...
# only run this step for derouter enterprise, we run this if enterprise/enterprise_ui/_enterprise.json exists
if [ ! -f "enterprise/enterprise_ui/enterprise_colors.json" ]; then
    echo "Admin UI - using default DeRouter UI"
    exit 0
fi
...
bash "$NVM_SCRIPT"
source ~/.nvm/nvm.sh
NODE_VERSION="$(cat ui/derouter-dashboard/.nvmrc)"
nvm install "v${NODE_VERSION}"
nvm use "v${NODE_VERSION}"


# cd in to /ui/derouter-dashboard
