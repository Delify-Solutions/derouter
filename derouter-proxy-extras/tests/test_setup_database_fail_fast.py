...
def test_v2_p3018_permission_error_raises_runtime_error(monkeypatch, tmp_path):
    ...
    with patch("derouter_proxy_extras.prisma_toolchain.run_prisma", side_effect=_fake_migrate_deploy_failure(1, stderr)):
...
def test_v1_default_still_calls_resolve_all_migrations(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("derouter_proxy_extras.prisma_toolchain.run_prisma", fake_run)
...
def test_v2_does_not_call_resolve_all_migrations(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("derouter_proxy_extras.prisma_toolchain.run_prisma", lambda *a, **kw: FakeResult())
...
def test_v2_p3018_deadlock_rolls_back_and_retries(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("derouter_proxy_extras.prisma_toolchain.run_prisma", _succeed_after(1, _DEADLOCK_P3018_STDERR))
...
def test_v2_p3018_persistent_deadlock_exhausts_attempts(monkeypatch, tmp_path):
    ...
    with patch(
        "derouter_proxy_extras.prisma_toolchain.run_prisma",
...
def test_v2_p3009_deadlocked_ledger_row_rolls_back_and_retries(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("derouter_proxy_extras.prisma_toolchain.run_prisma", _succeed_after(1, stderr))
...
def test_v2_bare_deadlock_stderr_retries(monkeypatch, tmp_path):
    ...
    _stub_v2_env(monkeypatch, tmp_path)
    monkeypatch.setattr(
        "derouter_proxy_extras.prisma_toolchain.run_prisma", _succeed_after(1, "Database error: deadlock detected")
    )
