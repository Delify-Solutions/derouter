...
``DEROUTER_SET_REPLICA_IDENTITY_FULL`` makes every migration run re-assert it.

The statement goes through the Prisma CLI rather than a Postgres driver because
``derouter-proxy-extras`` has no runtime dependencies, while the CLI is already
...
from derouter_proxy_extras.prisma_toolchain import prisma_command_timeout, run_prisma

REPLICA_IDENTITY_FULL_ENV_VAR = "DEROUTER_SET_REPLICA_IDENTITY_FULL"
...
def apply_replica_identity_full(
    ...
    """Set REPLICA IDENTITY FULL on every DeRouter table that is not already FULL.
    ...
    """
    logger.info("Applying REPLICA IDENTITY FULL to DeRouter tables")
    try:
        with tempfile.TemporaryDirectory(prefix="derouter_replica_identity_") as tmp_dir:
        ...
        logger.error("Timed out setting REPLICA IDENTITY FULL on DeRouter tables")
        return False
    except OSError as e:
        logger.error(
    ...
    logger.info("REPLICA IDENTITY FULL applied to DeRouter tables")
    return True
