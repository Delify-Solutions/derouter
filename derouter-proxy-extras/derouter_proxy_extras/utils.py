...
from derouter_proxy_extras import prisma_toolchain
from derouter_proxy_extras._logging import logger
from derouter_proxy_extras.prisma_toolchain import (
...
from derouter_proxy_extras.replica_identity import (
...
INDEX_REPAIR_ADVISORY_LOCK_KEY: Final = int.from_bytes(b"derouter", "big")
_TRANSIENT_INDEX_SUFFIX_RE: Final = re.compile(r"_cc(?:new|old)\d*$")
_INVALID_DEROUTER_INDEXES_SQL: Final = (
...
_SPEND_LOGS_ALTER_RE = re.compile(r'^ALTER\s+TABLE\s+"DeRouter_SpendLogs"\s', re.IGNORECASE)
_SPEND_LOGS_ARTIFACT_DROP_RE = re.compile(
    r'^DROP\s+TABLE\s+"DeRouter_SpendLogs_[^"]*"', re.IGNORECASE
)
...
PARTITIONED_SPEND_LOGS_PUSH_ERROR = (
    "DeRouter_SpendLogs is a partitioned table (see db_scripts/partition_spend_logs.sql), "
...
def filter_partitioned_spend_logs_diff(diff_sql: str) -> str:
    ...
    primary-key rewrite on "DeRouter_SpendLogs", which Postgres rejects on a
    partitioned table, and drops of runbook artifacts such as
    "DeRouter_SpendLogs_legacy"."""
    kept = tuple(
        filtered
...
class ProxyExtrasDBManager:
    ...
    def _get_prisma_dir() -> str:
        ...
        Set os.environ["DEROUTER_MIGRATION_DIR"] to a custom migrations directory, to support baselining db in read-only fs.
        """
        custom_migrations_dir = os.getenv("DEROUTER_MIGRATION_DIR")
    ...
    def _resolve_all_migrations(
        ...
        diff_dir = Path(tempfile.mkdtemp(prefix="derouter_migration_diff_"))
        ...
        if ProxyExtrasDBManager.spend_logs_is_partitioned():
            ...
            logger.info(
                "DeRouter_SpendLogs is partitioned; removed its primary-key "
    ...
    def spend_logs_is_partitioned() -> bool:
        """True when the connected database's DeRouter_SpendLogs is a
        ...
        try:
            ...
            logger.warning(
                "psycopg is not installed; skipping the DeRouter_SpendLogs "
                ...
                "derouter[extra_proxy] extra, which now includes psycopg."
            )
            return False
        ...
        try:
            with psycopg.connect(
                ...
                row = conn.execute(
                    ...
                    "WHERE c.relname = 'DeRouter_SpendLogs' "
                    "  AND n.nspname = %s",
                    (
    ...
    def _warn_if_db_ahead_of_head(migrations_dir: str) -> None:
        ...
        logger.warning(
            ...
            "migration this DeRouter version ships. This usually means the "
            "database was migrated by a newer DeRouter deployment. Some API "
    ...
    @staticmethod
    def _invalid_derouter_indexes(
        conn: "psycopg.Connection[tuple[str, str, str]]", schema: str
    ) -> tuple[_InvalidIndex, ...]:
        rows: Final = conn.execute(_INVALID_DEROUTER_INDEXES_SQL, (schema, "DeRouter\\_%")).fetchall()
    ...
    def repair_invalid_indexes(lock_timeout: str = "30s") -> bool:
        """Rebuild DeRouter indexes an interrupted CREATE INDEX CONCURRENTLY left
        ...
        try:
            ...
            logger.warning(
                ...
                "Install the derouter[extra_proxy] extra, which includes psycopg."
            )
            return False
        ...
        try:
            with psycopg.connect(cleaned_url, connect_timeout=10, autocommit=True) as conn:
                ...
                found: Final = ProxyExtrasDBManager._invalid_derouter_indexes(conn, schema)
                if not found:
                    return True
                logger.warning(
                ...
                if lock_row is None or not lock_row[0]:
                    ...
                    return False
                for index in ProxyExtrasDBManager._invalid_derouter_indexes(conn, schema):
                    ProxyExtrasDBManager._repair_index(conn, index)
                remaining: Final = ProxyExtrasDBManager._invalid_derouter_indexes(conn, schema)
        except psycopg.Error as e:
    ...
    def _setup_database_v2(use_migrate: bool) -> bool:
        ...
        caused schema thrashing when two DeRouter versions contended for the
    ...
    def apply_replica_identity_full_if_requested() -> bool:
        """
        Re-assert REPLICA IDENTITY FULL on DeRouter's tables when the operator
        opted in via DEROUTER_SET_REPLICA_IDENTITY_FULL.
    ...
    def setup_database(
        ...
        Uses migrations from derouter-proxy-extras package
                ...
                if use_migrate:
                    ...
                    try:
                        ...
                        if "P3009" in e.stderr:
                            ...
                            logger.info(
                                "Database schema is not empty, creating baseline migration. In read-only file system, please set an environment variable `DEROUTER_MIGRATION_DIR` to a writable directory to enable migrations. Learn more - https://router.delify.vn/docs/proxy/prod#read-only-file-system"
                            )
