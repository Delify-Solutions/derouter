...
help:
	...
	@echo "  make test-unit          - Run unit tests (tests/test_derouter)"
	...
	@echo ""
	@echo "Heavy targets (check, lint) queue for DEROUTER_GATE_SLOTS machine-wide"
...
bootstrap:
	...
	cd ui/derouter-dashboard && ../../scripts/with_dashboard_node.sh npm install --no-audit --no-fund
...
install-test-deps: install-proxy-dev
	...
	$(UV_RUN) prisma generate --schema derouter/proxy/schema.prisma
...
format: install-dev
	cd derouter && $(UV_RUN) ruff format --exclude '/enterprise/' . && cd ..

format-check: install-dev
	cd derouter && $(UV_RUN) ruff format --check --exclude '/enterprise/' . && cd ..
...
# only the derouter Python files changed vs the base are checked, so a pre-existing
# format issue elsewhere doesn't block an unrelated commit. Git pathspecs match
# recursively, so 'derouter/*.py' covers nested modules and the top-level files that
# CI's 'derouter/**/*.py' skips, which makes this target a superset of the CI step.
lint-format-check-changed: $(LINT_DEP_INSTALL) $(LINT_DEP_BASE)
	@base_ref=$$($(RESOLVE_BASE)) && \
	changed=$$(git diff --name-only --diff-filter=ACMR "$$base_ref...HEAD" -- 'derouter/*.py') && \
	files=$$(printf '%s\n' "$$changed" | grep -v '^derouter/enterprise/' || true) || exit $$?; \
	if [ -z "$$files" ]; then \
		echo "No changed derouter Python files to format-check."; \
	else \
...
lint-ruff: $(LINT_DEP_INSTALL)
	cd derouter && $(UV_RUN) ruff check . && cd ..
...
# derouter module-global mutation, credential-gated skips, conftest snapshot
...
check-circular-imports: $(LINT_DEP_INSTALL)
	cd derouter && $(UV_RUN) python ../tests/documentation_tests/test_circular_imports.py && cd ..

check-import-safety: $(LINT_DEP_INSTALL)
	@$(UV_RUN) python -c "from derouter import *; print('[from derouter import *] OK! no issues!');" || (echo '🚨 import failed, this means you introduced unprotected imports! 🚨'; exit 1)
...
# test-linting.yml (Python), test-derouter-ui-build.yml's frontend-lint (dashboard), and
...
test-rust-extension:
	...
	DEROUTER_RUST=1 DEROUTER_LOCAL_MODEL_COST_MAP=True \
	"$$temporary/venv/bin/python" -I -m pytest --import-mode=importlib -m requires_rust_extension tests/test_derouter_rust

test: install-test-deps
	$(UV_RUN) pytest tests/

test-unit: install-test-deps
	$(UV_RUN) pytest tests/test_derouter -x -vv -n 4

# Matrix test targets (matching CI workflow groups)
test-unit-llms: install-test-deps
	$(UV_RUN) pytest tests/test_derouter/llms --tb=short -vv -n 4 --durations=20

test-unit-proxy-guardrails: install-test-deps
	$(UV_RUN) pytest tests/test_derouter/proxy/guardrails tests/test_derouter/proxy/management_endpoints tests/test_derouter/proxy/management_helpers --tb=short -vv -n 4 --durations=20

test-unit-proxy-core: install-test-deps
	$(UV_RUN) pytest tests/test_derouter/proxy/auth tests/test_derouter/proxy/client tests/test_derouter/proxy/db tests/test_derouter/proxy/hooks tests/test_derouter/proxy/policy_engine --tb=short -vv -n 4 --durations=20

test-unit-proxy-misc: install-test-deps
	$(UV_RUN) pytest tests/test_derouter/proxy/_experimental tests/test_derouter/proxy/agent_endpoints tests/test_derouter/proxy/anthropic_endpoints tests/test_derouter/proxy/common_utils tests/test_derouter/proxy/discovery_endpoints tests/test_derouter/proxy/experimental tests/test_derouter/proxy/google_endpoints tests/test_derouter/proxy/health_endpoints tests/test_derouter/proxy/image_endpoints tests/test_derouter/proxy/middleware tests/test_derouter/proxy/openai_files_endpoint tests/test_derouter/proxy/pass_through_endpoints tests/test_derouter/proxy/prompts tests/test_derouter/proxy/public_endpoints tests/test_derouter/proxy/response_api_endpoints tests/test_derouter/proxy/shutdown tests/test_derouter/proxy/spend_tracking tests/test_derouter/proxy/ui_crud_endpoints tests/test_derouter/proxy/vector_store_endpoints tests/test_derouter/proxy/test_*.py --tb=short -vv -n 4 --durations=20

test-unit-integrations: install-test-deps
	$(UV_RUN) pytest tests/test_derouter/integrations --tb=short -vv -n 4 --durations=20

test-unit-core-utils: install-test-deps
	$(UV_RUN) pytest tests/test_derouter/derouter_core_utils --tb=short -vv -n 2 --durations=20

test-unit-other: install-test-deps
	$(UV_RUN) pytest tests/test_derouter/caching tests/test_derouter/responses tests/test_derouter/secret_managers tests/test_derouter/vector_stores tests/test_derouter/a2a_protocol tests/test_derouter/anthropic_interface tests/test_derouter/completion_extras tests/test_derouter/containers tests/test_derouter/enterprise tests/test_derouter/experimental_mcp_client tests/test_derouter/google_genai tests/test_derouter/images tests/test_derouter/interactions tests/test_derouter/passthrough tests/test_derouter/router_strategy tests/test_derouter/router_utils tests/test_derouter/types --tb=short -vv -n 4 --durations=20

test-unit-root: install-test-deps
	$(UV_RUN) pytest tests/test_derouter/test_*.py --tb=short -vv -n 4 --durations=20
...
test-integration: install-test-deps
	$(UV_RUN) pytest tests/ -k "not test_derouter"

test-unit-helm: install-helm-unittest
	helm unittest -f 'tests/*.yaml' helm/derouter-helm
