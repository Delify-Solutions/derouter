...
# client is imported from derouter as it's a decorator
from derouter import client
from derouter.constants import DEFAULT_IMAGE_ENDPOINT_MODEL
from derouter.constants import request_timeout as DEFAULT_REQUEST_TIMEOUT
...
from derouter.derouter_core_utils.llm_request_utils import flatten_form_field_values
from derouter.derouter_core_utils.mock_functions import mock_image_generation
from derouter.llms.base_llm import BaseImageEditConfig, BaseImageGenerationConfig
...
from derouter.utils import exception_type, get_derouter_params
...
# BFL handlers
from derouter.llms.black_forest_labs.image_edit.handler import bfl_image_edit
from derouter.llms.black_forest_labs.image_generation.handler import bfl_image_generation
from derouter.main import (
...
from derouter.types.images.main import ImageEditOptionalRequestParams
from derouter.types.llms.openai import ImageGenerationRequestQuality
from derouter.types.router import GenericDeRouterParams
from derouter.types.utils import (
    DEROUTER_IMAGE_VARIATION_PROVIDERS,
    LlmProviders,
    all_derouter_params,
)
...
def image_generation(
    ...
    **kwargs,
    ...
    try:
        ...
        model_response: ImageResponse = derouter.utils.ImageResponse()
        ...
        derouter_params: Final = all_derouter_params
        ...
        logging: Final[Logging] = derouter_logging_obj
        ...
        if custom_llm_provider == "azure":
            ...
            if azure_ad_token_provider is None:
                from derouter.llms.azure.common_utils import (
                    get_azure_ad_token_from_entra_id,
                )

                # Extract Azure AD credentials from derouter_params
                tenant_id: Final = derouter_params_dict.get("tenant_id")
                client_id: Final = derouter_params_dict.get("client_id")
                client_secret: Final = derouter_params_dict.get("client_secret")
                azure_scope = derouter_params_dict.get("azure_scope") or "https://cognitiveservices.azure.com/.default"
        ...
        #########################################################
            ...
            derouter.LlmProviders.RECRAFT,
            derouter.LlmProviders.AIML,
            derouter.LlmProviders.GEMINI,
            derouter.LlmProviders.FAL_AI,
            derouter.LlmProviders.STABILITY,
            derouter.LlmProviders.RUNWAYML,
            derouter.LlmProviders.VERTEX_AI,
            derouter.LlmProviders.OPENROUTER,
            ...
            # Resolve api_base from derouter.api_base if not explicitly provided
            _api_base: Final = api_base or derouter.api_base
            derouter_params_dict["api_base"] = _api_base
            ...
            or custom_llm_provider == LlmProviders.DEROUTER_PROXY.value
...
def image_variation(
    ...
    **kwargs,
    ...
    client: Final = kwargs.get("client", None)
    # get logging object
    derouter_logging_obj: Final = cast(DeRouterLoggingObj, kwargs.get("derouter_logging_obj"))

    # get the derouter params
    derouter_params: Final = get_derouter_params(**kwargs)
    # get the custom llm provider
    model, custom_llm_provider, dynamic_api_key, api_base = get_llm_provider(
        model=model,
        custom_llm_provider=derouter_params.get("custom_llm_provider", None),
    ...
    try:
        ...
        image_variation_provider: Final = DEROUTER_IMAGE_VARIATION_PROVIDERS(llm_provider)
    except ValueError:
        raise ValueError(
            f"Invalid image variation provider: {custom_llm_provider}. Supported providers are: {DEROUTER_IMAGE_VARIATION_PROVIDERS}"
        )
    ...
    if provider_config is None:
        raise ValueError(
            f"image variation provider has no known model info config - required for getting api keys, etc.: {custom_llm_provider}. Supported providers are: {DEROUTER_IMAGE_VARIATION_PROVIDERS}"
        )

    api_key: Final = provider_config.get_api_key(derouter_params.get("api_key", None))
    api_base = provider_config.get_api_base(derouter_params.get("api_base", None))

    if image_variation_provider == DEROUTER_IMAGE_VARIATION_PROVIDERS.OPENAI:
        if api_key is None:
        ...
        response = openai_image_variations.image_variations(
            ...
            model=model,
            image=image,
            timeout=derouter_params.get("timeout", None),
            custom_llm_provider=custom_llm_provider,
            logging_obj=derouter_logging_obj,
            optional_params={},
            derouter_params=derouter_params,
        )
    elif image_variation_provider == DEROUTER_IMAGE_VARIATION_PROVIDERS.TOPAZ:
        if api_key is None:
        ...
        response = base_llm_aiohttp_handler.image_variations(
            ...
            model=model,
            image=image,
            timeout=derouter_params.get("timeout", None) or DEFAULT_REQUEST_TIMEOUT,
            custom_llm_provider=custom_llm_provider,
            logging_obj=derouter_logging_obj,
            optional_params={},
            derouter_params=derouter_params,
...
def image_edit(
    ...
    **kwargs,
    ...
    try:
        ...
        derouter_params_list: Final = all_derouter_params
        default_params: Final = openai_params + derouter_params_list
        non_default_params: Final = {
            k: v for k, v in kwargs.items() if k not in default_params
        }  # model-specific params - pass them straight to the model/provider
        derouter_logging_obj: Final[DeRouterLoggingObj] = kwargs.get("derouter_logging_obj")
        ...
        if custom_llm_provider in derouter._custom_providers:
            custom_handler: CustomLLM | None = None
            for item in derouter.custom_provider_map:
                if item["provider"] == custom_llm_provider:
                    custom_handler = item["custom_handler"]
