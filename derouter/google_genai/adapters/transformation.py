...
from derouter.derouter_core_utils.json_validation_rule import normalize_tool_schema
...
class GoogleGenAIAdapter:
    """Adapter for transforming Google GenAI generate_content requests to/from derouter.completion format"""
    ...
    def translate_generate_content_to_completion(
        ...
        **kwargs,
        ...
        """
        Transform generate_content request to derouter completion format
        ...
        #########################################################
        # forward any derouter specific params
        #########################################################
        completion_request_dict = dict(completion_request)
        if derouter_params:
            completion_request_dict = self._add_generic_derouter_params_to_request(
    ...
    def _add_generic_derouter_params_to_request(
        self,
        completion_request_dict: _JsonDict,
        derouter_params: GenericDeRouterParams | None = None,
    ) -> _JsonDict:
        """Add generic derouter params to request. e.g add api_base, api_key, api_version, etc.
        ...
        Returns:
            Dict[str, Any]
        """
        allowed_fields: Final = GenericDeRouterParams.model_fields.keys()
        if derouter_params:
            derouter_dict: Final[_JsonDict] = derouter_params.model_dump(exclude_none=True)
            for key, value in derouter_dict.items():
    ...
    def translate_completion_to_generate_content(
        ...
        """
        Transform derouter completion response to Google GenAI generate_content format

        Args:
            response: ModelResponse from derouter.completion
    ...
    def translate_streaming_completion_to_generate_content(
        ...
        """
        Transform streaming derouter completion chunk to Google GenAI generate_content format

        Args:
            response: Streaming ModelResponse chunk from derouter.completion
