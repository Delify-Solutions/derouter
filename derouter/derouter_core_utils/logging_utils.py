...
if TYPE_CHECKING:
    ...
    from derouter import ModelResponse as _ModelResponse
    from derouter.derouter_core_utils.derouter_logging import (
        Logging as DeRouterLoggingObject,
    )

    DeRouterModelResponse = _ModelResponse
    Span = _Span | Any
else:
    DeRouterModelResponse = Any
...
def _iter_string_leaves(value: _StringTree) -> Iterator[str]:
    stack: Final[list[_StringTree]] = [value]  # mutable-ok: explicit stack, recursive functions are banned in derouter/
    while stack:
        match stack.pop():
            case str() as text:
                yield text
...
def _get_parent_otel_span_from_logging_obj(
    ...
    Args:
        logging_obj: The DeRouter logging object containing model call details
...
def convert_derouter_response_object_to_str(
    response_obj: object,
) -> str | None:
    """
    Get the string of the response object from DeRouter

    """
    if isinstance(response_obj, derouter.ModelResponse):
        response_str = ""
...
def _set_duration_in_model_call_details(
    logging_obj: Any,  # we're not guaranteed this will be `DeRouterLoggingObject`
    start_time: datetime,
    end_time: datetime,
):
