...
def _wrap_response_as_fake_stream(
    ...
    *,
    ...
    if isinstance(response, CustomStreamWrapper):
        return response
    if not isinstance(response, ModelResponse) or not isinstance(logging_obj, DeRouterLoggingObject):
        return response
...
def _add_agentic_loop_metadata(kwargs_for_followup: dict[str, object]) -> None:
    metadata = kwargs_for_followup.get("derouter_metadata")
    ...
    for key, value in kwargs_for_followup.items():
        if key.startswith("_agentic_loop") or key == "max_agentic_loops" or is_interception_internal_key(key):
            metadata[key] = value
    kwargs_for_followup["derouter_metadata"] = metadata
...
async def _execute_chat_completion_agentic_plan(
    *,
    ...
    try:
        response_followup = await derouter.acompletion(
...
async def maybe_run_chat_completion_agentic_loop(
    *,
    ...
    callbacks: Final = derouter.callbacks + (getattr(logging_obj, "dynamic_success_callbacks", None) or [])
    ...
    for callback in callbacks:
        ...
        try:
            ...
            verbose_logger.exception(
                "DeRouter.AgenticHookError: Exception in chat completion agentic gate: %s",
                str(e),
            )
            continue
        ...
        try:
            ...
            verbose_logger.exception(
                "DeRouter.AgenticHookError: Exception in chat completion agentic hooks: %s",
                str(e),
            )
