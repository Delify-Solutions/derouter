"""
Pulls the cost + context window + provider route for known models from https://github.com/Delify-Solutions/derouter/blob/main/model_prices_and_context_window.json

This can be disabled by setting the DEROUTER_LOCAL_MODEL_COST_MAP environment variable to True.

```
export DEROUTER_LOCAL_MODEL_COST_MAP=True
```
"""
...
class GetModelCostMap:
    ...
    def read_local_model_cost_map_bytes() -> bytes:
        return files("derouter").joinpath("model_prices_and_context_window_backup.json").read_bytes()
    ...
    def _check_is_valid_dict(fetched_map: dict) -> bool:
        ...
        if not isinstance(fetched_map, dict):
            verbose_logger.warning(
                "DeRouter: Fetched model cost map is not a dict (type=%s). Falling back to local backup.",
        ...
        if len(fetched_map) == 0:
            verbose_logger.warning(
                "DeRouter: Fetched model cost map is empty. Falling back to local backup.",
            )
            return False
    ...
    def _check_model_count_not_reduced(
        ...
        if fetched_count < min_model_count:
            verbose_logger.warning(
                "DeRouter: Fetched model cost map has only %d models (minimum=%d). "
        ...
        if backup_model_count > 0 and fetched_count < backup_model_count * max_shrink_ratio:
            verbose_logger.warning(
                "DeRouter: Fetched model cost map shrank significantly "
...
class ModelCostMapReloaded:
    model_cost_map: dict  # mutable-ok: adopted as derouter.model_cost, whose consumer contract is a plain mutable dict
...
def _next_retry_wait(
    ...
    verbose_logger.warning(
        "DeRouter: model cost map fetch attempt %d/%d failed (%s); retrying in %.1fs",
        attempt,
        max_attempts,
        outcome.reason,
        wait_seconds,
...
async def refetch_model_cost_map(
    ...
    """
    if os.getenv("DEROUTER_LOCAL_MODEL_COST_MAP", "").lower() == "true":
    ...
    if isinstance(result, ModelCostMapReloadUnavailable):
        verbose_logger.warning(
            "DeRouter: model cost map reload failed: %s. Keeping the currently loaded map.",
            result.reason,
        )
        return result
...
def _expand_model_aliases(model_cost: dict) -> dict:
    ...
    for model_name, model_info in model_cost.items():
        ...
        if not isinstance(aliases, list):
            verbose_logger.warning(
                "DeRouter model alias field for '%s' is not a list (got %s) — skipping.",
                model_name,
        ...
        for alias in aliases:
            if alias in model_cost:
                verbose_logger.warning(
                    "DeRouter model alias conflict: alias '%s' (from '%s') "
...
def get_model_cost_map(
    ...
    1. If ``DEROUTER_LOCAL_MODEL_COST_MAP`` is set, uses the local backup only.
    ...
    """
    _cost_map_source_info.loaded_at = datetime.now(timezone.utc)
    # Note: can't use get_secret_bool here — this runs during derouter.__init__
    # before derouter._key_management_settings is set.
    if os.getenv("DEROUTER_LOCAL_MODEL_COST_MAP", "").lower() == "true":
        _cost_map_source_info.source = "local"
        _cost_map_source_info.url = None
        _cost_map_source_info.is_env_forced = True
    ...
    if isinstance(result, ModelCostMapReloadUnavailable):
        verbose_logger.warning(
            "DeRouter: Failed to fetch remote model cost map from %s: %s. Falling back to local backup.",
            url,
            result.reason,
        )
        _cost_map_source_info.source = "local"
    ...
    if not GetModelCostMap.validate_model_cost_map(
        ...
        verbose_logger.warning(
            "DeRouter: Fetched model cost map failed integrity check. Using local backup instead. url=%s",
            url,
        )
        _cost_map_source_info.source = "local"
