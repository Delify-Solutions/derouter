...
from derouter.constants import MINIMUM_CUSTOM_KEY_LENGTH
...
def _build_secret_patterns() -> "re.Pattern[str]":
    patterns: Final[list[str]] = [
        ...
        r"dapi[0-9a-f]{32}",
        # Module-level provider keys logged as derouter.<provider>_key=<value>
        r"derouter\.[A-Za-z0-9_]*_key['\"]?\s*[:=]\s*['\"]?[^\s,'\"})\]{}>]+",
