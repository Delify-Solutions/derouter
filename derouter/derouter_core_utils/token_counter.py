...
from derouter.derouter_core_utils.default_encoding import encoding as default_encoding
from derouter.derouter_core_utils.url_utils import safe_get
...
from derouter.types.utils import Message, SelectTokenizerResponse
...
def get_modified_max_tokens(
    ...
    try:
        ...
        ## MODEL INFO
        _model_info: Final = derouter.get_model_info(model=model)

        max_output_tokens: Final = derouter.get_max_tokens(model=base_model)  # assume min context window is 4k tokens

        ## UNKNOWN MAX OUTPUT TOKENS - return user defined amount
        if max_output_tokens is None:
            return user_max_tokens

        input_tokens = derouter.token_counter(model=base_model, messages=messages)
        ...
        verbose_logger.debug(
            "derouter.derouter_core_utils.token_counter.py::get_modified_max_tokens() - user_max_tokens: %s",
            user_max_tokens,
        )
        ...
        verbose_logger.debug(
            "derouter.derouter_core_utils.token_counter.py::get_modified_max_tokens() - Error while checking max token limit: %s\nmodel=%s, base_model=%s",
            e,
            model,
            base_model,
        )
...
class _MessageCountParams:
    ...
    def __init__(
        ...
        from derouter.utils import print_verbose
        ...
        elif actual_model in derouter.open_ai_chat_completion_models or actual_model in derouter.azure_llms:
...
def _get_count_function(
    ...
    from derouter.utils import _select_tokenizer, print_verbose
...
def _fix_model_name(model: str) -> str:
    ...
    if model in derouter.azure_llms:
        # azure llms use gpt-35-turbo instead of gpt-3.5-turbo 🙃
        return model.replace("-35", "-3.5")
    elif model in derouter.open_ai_chat_completion_models:
        return model
    else:
...
def _count_anthropic_content(
    ...
    Uses TypedDict definitions from derouter.types.llms.anthropic to determine
