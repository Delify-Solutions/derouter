...
from derouter.derouter_core_utils.llm_cost_calc.tiered_pricing import (
...
def get_web_search_requests(server_tool_use: Any) -> int | None:
    ...
    See https://github.com/Delify-Solutions/derouter/issues/26153 — ``stream_chunk_builder``
...
def _generic_cost_per_character(
    ...
    try:
        ...
        verbose_logger.exception(
            "derouter.derouter_core_utils.llm_cost_calc.utils.py::cost_per_character(): Exception occured - %s\nDefaulting to None",
            e,
        )
...
def _get_cost_per_unit(model_info: ModelInfo, cost_key: str, default_value: float | None = 0.0) -> float | None:
    ...
    if isinstance(cost_per_unit, str):
        try:
            ...
            verbose_logger.exception(
                "derouter.derouter_core_utils.llm_cost_calc.utils.py::calculate_cost_per_component(): Exception occured - %s\nDefaulting to 0.0",
                cost_per_unit,
            )
            ...
            if suffix in cost_key:
                ...
                if isinstance(fallback_cost, str):
                    try:
                        ...
                        verbose_logger.exception(
                            "derouter.derouter_core_utils.llm_cost_calc.utils.py::_get_cost_per_unit(): Exception occured - %s\nDefaulting to 0.0",
                            fallback_cost,
                        )
...
def generic_cost_per_token(
    ...
    Input:
        - model: str, the model name without provider prefix
        - usage: DeRouter Usage block, containing anthropic caching information
    ...
    ## Ref: https://github.com/Delify-Solutions/derouter/issues/19680, #14874, #14875
...
def calculate_image_response_web_search_cost(
    ...
    from derouter.llms import get_cost_for_web_search_request
...
class CostCalculatorUtils:
    ...
    def route_image_generation_cost_calculator(
        ...
        from derouter.llms.azure_ai.image_generation.cost_calculator import (
            cost_calculator as azure_ai_image_cost_calculator,
        )
        from derouter.llms.bedrock.image_generation.cost_calculator import (
        ...
        from derouter.llms.recraft.cost_calculator import (
        ...
        if custom_llm_provider == derouter.LlmProviders.VERTEX_AI.value:
            if isinstance(completion_response, ImageResponse):
                return vertex_ai_image_cost_calculator(
                    model=model,
                    image_response=completion_response,
                )
        elif custom_llm_provider == derouter.LlmProviders.BEDROCK.value:
            if isinstance(completion_response, ImageResponse):
        ...
        elif custom_llm_provider == derouter.LlmProviders.RECRAFT.value:
            from derouter.llms.recraft.cost_calculator import (
                cost_calculator as recraft_image_cost_calculator,
            )
        ...
        elif custom_llm_provider == derouter.LlmProviders.AIML.value:
            from derouter.llms.aiml.image_generation.cost_calculator import (
        ...
        elif custom_llm_provider == derouter.LlmProviders.COMETAPI.value:
            from derouter.llms.cometapi.image_generation.cost_calculator import (
        ...
        elif custom_llm_provider == derouter.LlmProviders.GEMINI.value:
            if call_type in (
                CallTypes.image_edit.value,
                CallTypes.aimage_edit.value,
            ):
                from derouter.llms.gemini.image_edit.cost_calculator import (
        ...
        elif custom_llm_provider == derouter.LlmProviders.AZURE_AI.value:
            return azure_ai_image_cost_calculator(
                model=model,
                image_response=completion_response,
            )
        elif custom_llm_provider == derouter.LlmProviders.FAL_AI.value:
            from derouter.llms.fal_ai.cost_calculator import (
        ...
        elif custom_llm_provider == derouter.LlmProviders.RUNWAYML.value:
            from derouter.llms.runwayml.cost_calculator import (
            ...
            or custom_llm_provider == derouter.LlmProviders.AZURE.value
        ):
            # gpt-image models use token-based pricing.
            model_lower: Final = model.lower()
            if "gpt-image" in model_lower:
                from derouter.llms.openai.image_generation.cost_calculator import (
