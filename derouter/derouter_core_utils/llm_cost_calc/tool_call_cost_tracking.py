...
import derouter
from derouter.constants import OPENAI_FILE_SEARCH_COST_PER_1K_CALLS
...
class StandardBuiltInToolCostTracking:
    ...
    def _resolve_model_info(model: str, custom_llm_provider: str | None) -> tuple[ModelInfo | None, str | None]:
        ...
        if direct is not None:
            return direct, custom_llm_provider or direct["derouter_provider"]
        if "/" not in model:
        ...
        if by_prefix is None:
            return None, custom_llm_provider
        return by_prefix, by_prefix["derouter_provider"]
    ...
    def _handle_google_maps_grounding_cost(
        ...
        from derouter.llms import get_cost_for_google_maps_grounding_request
        from derouter.llms.gemini.cost_calculator import google_maps_grounding_requests
    ...
    def _safe_get_model_info(model: str, custom_llm_provider: str | None = None) -> ModelInfo | None:
        try:
            return derouter.get_model_info(model=model, custom_llm_provider=custom_llm_provider)
        except Exception:
            return None
    ...
    def get_cost_for_file_search(
        ...
        if provider == "azure":
            from derouter.constants import AZURE_FILE_SEARCH_COST_PER_GB_PER_DAY
    ...
    def get_cost_for_vector_store(
        ...
        if provider == "azure":
            from derouter.constants import AZURE_VECTOR_STORE_COST_PER_GB_PER_DAY
    ...
    def _get_code_interpreter_cost_from_model_map(
        ...
        try:
            ...
            model_info: Final = derouter.get_model_info(model=container_model, custom_llm_provider=provider)
            model_key = model_info.get("key") if isinstance(model_info, dict) else getattr(model_info, "key", None)

            if model_key and model_key in derouter.model_cost:
                return derouter.model_cost[model_key].get("code_interpreter_cost_per_session")
