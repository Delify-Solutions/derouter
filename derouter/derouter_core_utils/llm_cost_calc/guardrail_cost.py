...
from derouter.types.utils import CostBreakdown
...
def _bedrock_guardrail_pricing(aws_region_name: str | None) -> GuardrailPricing | None:
    ...
    for key in (regional_key, BEDROCK_GUARDRAIL_PRICING_KEY):
        if key is None or key not in derouter.model_cost:
            continue
        try:
            return GuardrailPricing.model_validate(derouter.model_cost[key])
        except ValidationError as e:
