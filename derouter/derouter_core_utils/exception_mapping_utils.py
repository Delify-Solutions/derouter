...
import derouter
from derouter._logging import _ENABLE_SECRET_REDACTION, _redact_string, verbose_logger
...
def _get_response_headers(original_exception: Exception) -> httpx.Headers | None:
    ...
    try:
        ...
        if not _response_headers:
            _response_headers = getattr(original_exception, "derouter_response_headers", None)
    except Exception:
        return None
...
def extract_and_raise_derouter_exception(
    response: Any | None,
    error_str: str,
    model: str,
    ...
    """
    Covers scenario where derouter sdk calling proxy.

    Enables raising the special errors raised by derouter, eg. ContextWindowExceededError.

    Relevant Issue: https://github.com/Delify-Solutions/derouter/issues/7259
    """
    pattern: Final = r"derouter\.\w+Error"
    ...
    if match:
        ...
        exception_name = exception_name.strip().replace("derouter.", "")
        raised_exception_obj: Final = getattr(derouter, exception_name, None)
...
def _map_openai_exception(
    *,
    ...
    if message is not None and isinstance(
        message, str
    ):  # done to prevent user-confusion. Relevant issue - https://github.com/Delify-Solutions/derouter/issues/1414
    ...
    if ExceptionCheckers.is_error_str_rate_limit(
        ...
        raise ContextWindowExceededError(
            ...
            model=model,
            response=getattr(original_exception, "response", None),
            derouter_debug_info=extra_information,
        )
        ...
        helpful_message: Final = (
            ...
            "   Learn more: https://router.delify.vn/docs/response_api#encrypted-content-affinity-multi-region-load-balancing"
        )
    ...
    *,
    ...
    if hasattr(original_exception, "status_code"):
        ...
        if original_exception.status_code == 401:
            ...
            raise derouter.InternalServerError(
                message=f"AnthropicException - {error_str}. Handle with `derouter.InternalServerError`.",
                llm_provider="anthropic",
                model=model,
                response=getattr(original_exception, "response", None),
            )
            ...
            raise derouter.ServiceUnavailableError(
                message=f"AnthropicException - {error_str}. Handle with `derouter.ServiceUnavailableError`.",
                llm_provider="anthropic",
                model=model,
                response=getattr(original_exception, "response", None),
            )
...
def _get_bedrock_mantle_context_window_message(error_str: str) -> str | None:
    ...
    message clients recognize as context overflow (derouter/derouter#36546).
    """
...
def _map_bedrock_exception(
    *,
    ...
    if (
        ...
        raise BadRequestError(
            message=f"BedrockException - {error_str}\n. Enable 'derouter.modify_params=True' (for PROXY do: `derouter_settings::modify_params: True`) to insert a dummy assistant message and fix this error.",
            model=model,
            llm_provider="bedrock",
            response=getattr(original_exception, "response", None),
        )
        ...
        raise BadRequestError(
            message=f"BedrockException - {error_str}\n. Pass in default user message via `completion(..,user_continue_message=)` or enable `derouter.modify_params=True`.\nFor Proxy: do via `derouter_settings::modify_params: True` or user_continue_message under `derouter_params`",
            model=model,
            llm_provider="bedrock",
            response=getattr(original_exception, "response", None),
        )
...
def _map_sagemaker_exception(
    *,
    ...
    if "Unable to locate credentials" in error_str:
        raise BadRequestError(
            message=f"derouter.BadRequestError: SagemakerException - {error_str}",
            model=model,
...
def _map_vertex_exception(
    *,
    ...
    if "Vertex AI API has not been used in project" in error_str or "Unable to find your project" in error_str:
        raise BadRequestError(
            message=f"derouter.BadRequestError: {custom_llm_provider}Exception - {error_str}",
            model=model,
            llm_provider=custom_llm_provider,
            response=httpx.Response(
                status_code=400,
    ...
    if "400 Request payload size exceeds" in error_str:
        ...
        raise derouter.InternalServerError(
            message=f"derouter.InternalServerError: {custom_llm_provider}Exception - {error_str}",
            model=model,
            llm_provider=custom_llm_provider,
            response=httpx.Response(
                status_code=500,
                content=str(original_exception),
                request=httpx.Request(method="completion", url="https://github.com/Delify-Solutions/derouter"),
            ),
            derouter_debug_info=extra_information,
        )
        ...
        raise RateLimitError(
            message=f"derouter.RateLimitError: {custom_llm_provider}Exception - {error_str}",
            model=model,
            llm_provider=custom_llm_provider,
            derouter_debug_info=extra_information,
            response=httpx.Response(
    ...
    if hasattr(original_exception, "status_code"):
        ...
        if original_exception.status_code == 429:
            raise RateLimitError(
                message=f"derouter.RateLimitError: {custom_llm_provider.capitalize()}Exception - {error_str}",
                model=model,
                llm_provider=custom_llm_provider,
                derouter_debug_info=extra_information,
                response=httpx.Response(
...
def _map_azure_exception(
    *,
    ...
    if "Internal server error" in error_str:
        ...
        from derouter.llms.azure.exception_mapping import (
...
def _response_or_stub(original_exception: _ProviderHTTPException, status_code: int) -> httpx.Response:
    ...
    return httpx.Response(
        status_code=status_code, request=httpx.Request(method="POST", url="https://router.delify.vn/docs")
    )
...
def exception_type(
    ...
    if any(isinstance(original_exception, exc_type) for exc_type in derouter.DEROUTER_EXCEPTION_TYPES):
    ...
    if derouter.suppress_debug_info is False:
        ...
        print(  # noqa: T201
            "DeRouter.Info: If you need to debug this error, use `derouter._turn_on_debug()'."
        )
        print()  # noqa: T201

    derouter_response_headers: Final = _get_response_headers(original_exception=original_exception)
    try:
        ...
        if model or custom_llm_provider:
            ...
            ################################################################################
            try:
                _api_base: Final = derouter.get_api_base(model=model, optional_params=extra_kwargs)
                messages: Final = derouter.get_first_chars_messages(kwargs=completion_kwargs)
                ...
                if _api_base:
                    extra_information += f"\nAPI Base: `{_api_base}`"
                if messages and len(messages) > 0 and derouter.redact_messages_in_exceptions is False:
                    extra_information += f"\nMessages: `{messages}`"
                ...
                # on derouter proxy add key name + team to exceptions
            ...
            ):  # handle special case where calling derouter proxy + exception str contains error message
            ...
            elif custom_llm_provider in derouter._openai_like_providers:
        ...
        if "BadRequestError.__init__() missing 1 required positional argument: 'param'" in str(
            ...
            """
            For unmapped exceptions - raise the exception with traceback - https://github.com/Delify-Solutions/derouter/issues/4201
            """
            exception_mapping_worked = True
        ...
        if exception_mapping_worked:
            setattr(e, "derouter_response_headers", derouter_response_headers)
            raise e
        else:
            for error_type in derouter.DEROUTER_EXCEPTION_TYPES:
            ...
            setattr(raised_exc, "derouter_response_headers", derouter_response_headers)
            raise raised_exc
    ...
    try:
        ...
        if logger_fn and callable(logger_fn):
            try:
                ...
                verbose_logger.debug(
                    "DeRouter.LoggingError: [Non-Blocking] Exception occurred while logging %s", traceback.format_exc()
                )
    except Exception:
        verbose_logger.debug(
            "DeRouter.LoggingError: [Non-Blocking] Exception occurred while logging %s", traceback.format_exc()
...
def _add_key_name_and_team_to_alert(request_info: str, metadata: dict) -> str:
    """
    Internal helper function for derouter proxy
