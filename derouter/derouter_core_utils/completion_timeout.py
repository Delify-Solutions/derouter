...
from derouter.constants import COMPLETION_HTTP_FALLBACK_SECONDS
...
class CompletionTimeout:
    ...
    def _fallback_when_no_explicit_timeout(
        ...
        ``global_timeout`` is the explicitly-configured ``derouter.request_timeout``
        (numeric / string) or ``None`` when it was never set. ``None`` falls back to
        :data:`~derouter.constants.COMPLETION_HTTP_FALLBACK_SECONDS`; any explicit value
    ...
    def resolve(
        ...
        *,
        ...
        1. ``model_timeout`` (call argument / merged ``derouter_params``)
        2. ``kwargs["timeout"]``
        3. ``kwargs["request_timeout"]``
        4. ``global_timeout`` (the explicitly-configured ``derouter.request_timeout``),
