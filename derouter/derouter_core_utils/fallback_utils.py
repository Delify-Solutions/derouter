...
async def async_completion_with_fallbacks(**kwargs):
    ...
    base_kwargs: Final = {**kwargs, **nested_kwargs, "derouter_call_id": derouter_call_id}

    # fields to remove
    base_kwargs.pop("model", None)  # Remove model as it will be set per fallback
    derouter_logging_obj: Final = base_kwargs.pop("derouter_logging_obj", None)
    ...
    raise Exception(
        f"{most_recent_exception_str}. All fallback attempts failed. Enable verbose logging with `derouter.set_verbose=True` for details."
    )
