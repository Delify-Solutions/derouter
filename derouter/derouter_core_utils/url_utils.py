...
Admins can opt out via two ``derouter`` globals (wired from proxy config):

- ``derouter.user_url_validation`` (bool, default True): master switch.
  When False, ``safe_get``/``async_safe_get`` perform a plain fetch with
  no DNS check, no block list, and no rewrite.
- ``derouter.user_url_allowed_hosts`` (List[str], default []): per-host
...
def _is_host_allowlisted(hostname: str, effective_port: int) -> bool:
    ...
    """
    configured: Final[list[str]] = getattr(derouter, "user_url_allowed_hosts", []) or []
    if not configured:
        return False
...
def validate_url(url: str) -> tuple[str, str]:
    ...
    ssl_verify: Final = getattr(derouter, "ssl_verify", True)
...
def safe_get(client: Any, url: str, **kwargs: Any) -> httpx.Response:
    ...
    When ``derouter.user_url_validation`` is False, validation is bypassed
    ...
    """
    if not getattr(derouter, "user_url_validation", True):
