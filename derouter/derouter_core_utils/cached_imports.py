"""
Cached imports module for DeRouter.
...
if TYPE_CHECKING:
    from derouter.derouter_core_utils.coroutine_checker import CoroutineChecker
    from derouter.derouter_core_utils.derouter_logging import Logging

# Global cache variables
_DeRouterLogging: type["Logging"] | None = None
_coroutine_checker: Optional["CoroutineChecker"] = None
_set_callbacks: Callable | None = None


def get_derouter_logging_class() -> type["Logging"]:
    """Get the cached DeRouter Logging class, initializing if needed."""
    global _DeRouterLogging
    if _DeRouterLogging is not None:
        return _DeRouterLogging
    from derouter.derouter_core_utils.derouter_logging import Logging

    _DeRouterLogging = Logging
    return _DeRouterLogging
...
def get_set_callbacks() -> Callable:
    ...
    from derouter.derouter_core_utils.derouter_logging import set_callbacks
...
def clear_cached_imports() -> None:
    ...
    global _DeRouterLogging, _coroutine_checker, _set_callbacks
    _DeRouterLogging = None
