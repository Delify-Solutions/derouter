"""
Pulls the latest DeRouter blog posts from the docs RSS feed.

Falls back to the bundled local backup on any failure.
RSS URL is configured via derouter.blog_posts_url (or DEROUTER_BLOG_POSTS_URL env var).

Disable remote fetching entirely:
    export DEROUTER_LOCAL_BLOG_POSTS=True
"""
...
class GetBlogPosts:
    """
    Fetches, validates, and caches DeRouter blog posts.
    ...
    def load_local_blog_posts() -> list[dict[str, str]]:
        ...
        content: Final = json.loads(files("derouter").joinpath("blog_posts.json").read_text(encoding="utf-8"))
    ...
    def validate_blog_posts(posts: list[dict[str, str]]) -> bool:
        ...
        if not isinstance(posts, list) or len(posts) == 0:
            verbose_logger.warning(
                "DeRouter: Parsed RSS feed has no valid posts. Falling back to local backup.",
            )
            return False
        return True
    ...
    def get_blog_posts(cls, url: str) -> list[dict[str, str]]:
        ...
        """
        if os.getenv("DEROUTER_LOCAL_BLOG_POSTS", "").lower() == "true":
        ...
        try:
            ...
            verbose_logger.warning(
                "DeRouter: Failed to fetch blog posts from %s: %s. Falling back to local backup.",
                url,
                str(e),
            )
            return cls.load_local_blog_posts()
