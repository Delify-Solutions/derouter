...
from derouter.types.utils import TRUSTED_CALLBACK_VARS_FIELD, StandardCallbackDynamicParams

_CLIENT_CALLBACK_METADATA_SLOTS: Final[tuple[str, ...]] = ("derouter_metadata", "metadata")
...
def iter_client_callback_metadata_dicts(
    ...
    if isinstance(derouter_params, dict):
        nested: Final = derouter_params.get("metadata")
        if isinstance(nested, dict):
            yield "derouter_params.metadata", nested
...
def _raise_env_reference_error(param: str, *, source: str) -> None:
    raise ValueError(
        ...
        "config.yaml under 'derouter_settings' / 'general_settings', or "
        "(b) pass the resolved secret value directly in the request.\n"
        "See https://router.delify.vn/docs/proxy/logging for server-side "
