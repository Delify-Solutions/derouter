...
from derouter.types.llms.databricks import DatabricksTool
...
from derouter.types.utils import Logprobs as TextCompletionLogprobs
...
def _safe_convert_created_field(created_value) -> int:
    ...
    (Unix timestamp with fractional seconds), but DeRouter expects an integer.
...
def convert_tool_call_to_json_mode(
    ...
    if _should_convert_tool_call_to_json_mode(
        ...
        if json_mode_content_str is not None:
            message: Final = derouter.Message(content=json_mode_content_str)
...
async def convert_to_streaming_response_async(
    ...
    if not response_object.get("choices"):
        ...
        raise APIError(
            status_code=500,
            message=(
                f"DeRouter: provider returned a response with no 'choices'. Raw keys: {list(response_object.keys())}"
            ),
            llm_provider="",
            model="",
        )
...
class DeRouterResponseObjectHandler:
    @staticmethod
    ...
    def convert_chat_to_text_completion(
        ...
        transformed_logprobs: Final = (
            DeRouterResponseObjectHandler._convert_provider_response_logprobs_to_text_completion_logprobs(
                response=response,
        ...
        if response_type == "completion" and (
            ...
            if not response_object.get("choices") or not isinstance(response_object["choices"], Iterable):
                ...
                raise APIError(
                    status_code=500,
                    message=(
                        "DeRouter: provider returned a response with no 'choices'. "
            ...
            for idx, choice in enumerate(response_object["choices"]):
                ...
                if tool_calls is not None and _should_convert_tool_call_to_json_mode(
                    ...
                    if json_mode_content_str is not None:
                        message = derouter.Message(content=json_mode_content_str)
                        finish_reason = "stop"
                if message is None:
            ...
            if "usage" in response_object and response_object["usage"] is not None:
                usage_object: Final = derouter.Usage(**response_object["usage"])
            ...
            return DeRouterResponseObjectHandler.convert_to_image_response(
