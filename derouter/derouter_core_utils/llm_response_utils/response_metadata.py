...
from derouter.constants import DEROUTER_DETAILED_TIMING
...
from derouter.derouter_core_utils.logging_utils import DeRouterLoggingObject
...
def response_timing_metrics(
    ...
    """``_response_ms`` for the whole call, plus ``derouter_overhead_time_ms`` when it can be derived.
    ...
    return {"_response_ms": total_response_time_ms, "derouter_overhead_time_ms": overhead_ms}


class ResponseMetadata:
    """
    Handles setting and managing `_hidden_params`, `response_time_ms`, and `derouter_overhead_time_ms` for DeRouter responses
    """
    ...
    def set_hidden_params(self, logging_obj: DeRouterLoggingObject, model: str | None, kwargs: dict) -> None:
        ...
        new_params: Final = {
            "derouter_call_id": getattr(logging_obj, "derouter_call_id", None),
            "api_base": get_api_base(model=model or "", optional_params=kwargs),
            "model_id": model_id,
            "response_cost": logging_obj._response_cost_calculator(
                result=self.result, derouter_model_name=model, router_model_id=model_id
            ),
    ...
    def set_timing_metrics(
        ...
        #########################################################
        # 1. Add _response_ms total duration and the DeRouter overhead within it
        ...
        #########################################################
        llm_api_duration_ms: Final = logging_obj.model_call_details.get("llm_api_duration_ms")
        if DEROUTER_DETAILED_TIMING and llm_api_duration_ms is not None:
...
def update_response_metadata(
    ...
    Updates response metadata, adds the following:
        ...
        - response._hidden_params["derouter_overhead_time_ms"]
