...
A ROUTING rule carries exactly one ``model_info`` key, ``derouter_provider``. It is
...
A CAPABILITY rule carries any ``model_info`` keys except ``derouter_provider``
...
ones on key conflicts, and the caller backfills ``derouter_provider`` with the
...
where a rule mixes ``derouter_provider`` with capability keys and may inherit a
...
``derouter_provider`` participates in first-hit inference) and a capability rule
...
``derouter_provider`` is not a string is invalid and is warned about and skipped
...
PROVIDER_KEY: Final = "derouter_provider"
...
def _compile_rule(rule: object) -> tuple[_CompiledRule, ...]:
    ...
    if not isinstance(pattern, str) or not isinstance(model_info, dict):
        verbose_logger.warning(
            "DeRouter: skipping malformed fallback generalization rule %s (needs string '%s' and dict '%s').",
    ...
    try:
        ...
        verbose_logger.warning(
            "DeRouter: skipping fallback generalization rule with invalid regex %r: %s",
            pattern,
            e,
        )
        return ()
    ...
    if not isinstance(provider, str):
        verbose_logger.warning(
            "DeRouter: skipping invalid fallback generalization rule %s: '%s' in '%s' must be a string.",
            rule.get(NAME_FIELD, pattern),
            PROVIDER_KEY,
            MODEL_INFO_FIELD,
        )
...
def set_fallback_generalizations(rules: list | None) -> None:
    ...
    a legacy rule mixing ``derouter_provider`` with capability keys installs as both
