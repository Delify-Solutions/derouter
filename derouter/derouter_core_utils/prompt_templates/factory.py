...
import derouter.types.llms
from derouter import verbose_logger
from derouter._uuid import uuid
from derouter.constants import REDACTED_BY_DEROUTER
from derouter.derouter_core_utils.url_utils import async_safe_get, safe_get
from derouter.llms.custom_httpx.http_handler import HTTPHandler, get_async_httpx_client
from derouter.types.files import get_file_extension_from_mime_type
from derouter.types.llms.anthropic import *
from derouter.types.llms.bedrock import CachePointBlock
from derouter.types.llms.bedrock import MessageBlock as BedrockMessageBlock
from derouter.types.llms.custom_http import httpxSpecialProvider
from derouter.types.llms.ollama import OllamaVisionModelObject
...
from derouter.types.llms.vertex_ai import FunctionCall as VertexFunctionCall
from derouter.types.llms.vertex_ai import FunctionResponse as VertexFunctionResponse
from derouter.types.llms.vertex_ai import PartType as VertexPartType
from derouter.types.utils import GenericImageParsingChunk
...
}  # similar to autogen. Only used if `derouter.modify_params=True`.
...
async def _afetch_and_extract_template(
    ...
    """
    from derouter.derouter_core_utils.prompt_templates.huggingface_template_handler import (
    ...
    if chat_template is None:
        ...
        if model in derouter.known_tokenizer_config:
            tokenizer_config = derouter.known_tokenizer_config[model]
        else:
            tokenizer_config = await get_config_fn(hf_model_name=model)
            derouter.known_tokenizer_config.update({model: tokenizer_config})
...
def convert_to_anthropic_image_obj(openai_image_url: str, format: str | None) -> GenericImageParsingChunk:
    ...
    except derouter.ImageFetchError:
        raise
    except Exception as e:
        raise Exception(
...
def anthropic_messages_pt_xml(messages: list):
    ...
    1. Anthropic supports roles like "user" and "assistant", (here derouter translates system-> assistant)
    2. The first message always needs to be of role "user"
    3. Each message must alternate between "user" and "assistant" (this is not addressed as now by derouter)
    ...
    if not new_messages or new_messages[0]["role"] != "user":
        if derouter.modify_params:
            new_messages.insert(0, {"role": "user", "content": [{"type": "text", "text": "."}]})
        else:
            raise Exception(
                "Invalid first message. Should always start with 'role'='user' for Anthropic. System prompt is sent separately for Anthropic. set 'derouter.modify_params = True' or 'derouter_settings:modify_params = True' on proxy, to insert a placeholder user message - '.' as the first message, "
            )
...
def convert_to_gemini_tool_call_result(
    ...
    """
    from derouter.types.llms.vertex_ai import BlobType
...
def convert_to_anthropic_tool_invoke(
    ...
    Fixes: https://github.com/Delify-Solutions/derouter/issues/17737
    """
...
def anthropic_messages_pt(
    ...
    # for Anthropic-shaped requests, independent of `derouter.modify_params`,
    ...
    if len(messages) == 0:
        if not derouter.modify_params:
            raise derouter.BadRequestError(
                message=f"Anthropic requires at least one non-system message. Either provide one, or set `derouter.modify_params = True` // `derouter_settings::modify_params: True` to add the dummy user message - {DEFAULT_USER_CONTINUE_MESSAGE_TYPED}.",
                model=model,
    ...
    while msg_i < len(messages):
        ...
        while msg_i < len(messages) and messages[msg_i]["role"] == "assistant":
            ...
            # Fixes: https://github.com/Delify-Solutions/derouter/issues/23047
...
from derouter.types.llms.cohere import (
    CallObject,
    ChatHistory,
    ChatHistoryChatBot,
    ChatHistorySystem,
...
def amazon_titan_pt(
    messages: list,
):  # format - https://github.com/Delify-Solutions/derouter/issues/1896
    """
    ...
    try:
        ...
        for message in messages:
            if isinstance(message["content"], str):
                ...
                # see https://router.delify.vn/docs/providers/openai#openai-vision-models
...
from derouter.types.llms.bedrock import (
    BedrockConverseReasoningContentBlock,
    BedrockConverseReasoningTextBlock,
    BedrockToolSpec,
    SearchResultBlock,
)
from derouter.types.llms.bedrock import ContentBlock as BedrockContentBlock
from derouter.types.llms.bedrock import DocumentBlock as BedrockDocumentBlock
from derouter.types.llms.bedrock import ImageBlock as BedrockImageBlock
from derouter.types.llms.bedrock import SourceBlock as BedrockSourceBlock
from derouter.types.llms.bedrock import ToolBlock as BedrockToolBlock
from derouter.types.llms.bedrock import ToolResultBlock as BedrockToolResultBlock
from derouter.types.llms.bedrock import (
    ToolResultContentBlock as BedrockToolResultContentBlock,
)
from derouter.types.llms.bedrock import ToolUseBlock as BedrockToolUseBlock
from derouter.types.llms.bedrock import VideoBlock as BedrockVideoBlock
...
class BedrockImageProcessor:
    ...
    def _validate_format(mime_type: str, image_format: str) -> str:
        ...
        supported_image_formats: Final = derouter.AmazonConverseConfig().get_supported_image_types()
        supported_doc_formats: Final = derouter.AmazonConverseConfig().get_supported_document_types()
        supported_video_formats: Final = derouter.AmazonConverseConfig().get_supported_video_types()
    ...
    def _get_document_format(mime_type: str, supported_doc_formats: list[str]) -> str:
        ...
        Relevant Issue: https://github.com/Delify-Solutions/derouter/issues/12260
        ...
        #################
        # derouter runs on docker containers and `mimetypes` depends on the installed mimetypes of the OS
            ...
            if "function" in tool:
                ...
                if not arguments or not arguments.strip():
                    ...
                    try:
                        ...
                        # Fixes: https://github.com/Delify-Solutions/derouter/issues/20543
                        ...
                        if parsed_objects:
                            ...
                            if tool.get("cache_control", None) is not None:
                                _cache_point_block = derouter.AmazonConverseConfig().get_cache_point_block(
                ...
                if tool.get("cache_control", None) is not None:
                    cache_point_block = derouter.AmazonConverseConfig().get_cache_point_block(
                        {"cache_control": tool["cache_control"]},
                        block_type="content_block",
                        model=model,
                    )
...
def _insert_assistant_continue_message(
    ...
    Conversation blocks and tool result blocks cannot be provided in the same turn. Issue: https://github.com/Delify-Solutions/derouter/issues/6053
    """
...
def get_user_message_block_or_continue_message(
    ...
    Relevant Issue: https://github.com/Delify-Solutions/derouter/issues/7169
    """
    content_block: Final = message.get("content", None)

    # Handle None case
    if content_block is None or (user_continue_message is None and derouter.modify_params is False):
...
def get_assistant_message_block_or_continue_message(
    ...
    # Handle Base case
    if content_block is None or (assistant_continue_message is None and derouter.modify_params is False):
        return skip_empty_text_blocks(message=message)
...
class BedrockConverseMessagesProcessor:
        ...
        while msg_i < len(messages):
            ...
            if user_content:
                if len(contents) > 0 and contents[-1]["role"] == "user":
                    if assistant_continue_message is not None or derouter.modify_params is True:
    ...
    def add_thinking_blocks_to_assistant_content(
        ...
        Relevant Issue: https://github.com/Delify-Solutions/derouter/issues/9063
        """
...
def make_valid_bedrock_tool_name(input_tool_name: str) -> str:
    ...
    if input_tool_name != valid_string:
        ...
        derouter.bedrock_tool_name_mappings.set_cache(key=valid_string, value=input_tool_name)

    return valid_string


def add_cache_point_tool_block(tool: dict, model: str | None = None) -> BedrockToolBlock | None:
    from derouter.llms.bedrock.common_utils import (
...
def _bedrock_tools_pt(tools: list, model: str | None = None) -> list[BedrockToolBlock]:
    ...
    """
    from derouter.derouter_core_utils.prompt_templates.common_utils import unpack_defs
    from derouter.llms.bedrock.common_utils import (
    ...
    for tool_idx, tool in enumerate(tools):
        ...
        if not (raw_name and str(raw_name).strip()):
            raw_name = f"derouter_unnamed_tool_{tool_idx}"

        # related issue: https://github.com/Delify-Solutions/derouter/issues/5007
...
def response_schema_prompt(model: str, response_schema: dict) -> str:
    ...
    if f"{model}/response_schema_prompt" in derouter.custom_prompt_dict:
        custom_prompt_details = derouter.custom_prompt_dict[
            f"{model}/response_schema_prompt"
        ]  # allow user to define custom response schema prompt by model
    elif "response_schema_prompt" in derouter.custom_prompt_dict:
        custom_prompt_details = derouter.custom_prompt_dict["response_schema_prompt"]
...
def prompt_factory(
    ...
    if custom_llm_provider == "ollama":
        ...
        if (
            ...
            or derouter.supports_vision(model=model)
            or derouter.supports_vision(model=custom_llm_provider + "/" + model)
        ):
            return _gemini_vision_convert_messages(messages=messages)
        else:
            return gemini_text_image_pt(messages=messages)
    elif custom_llm_provider == "mistral":
        return derouter.MistralConfig()._transform_messages(messages=messages, model=model)
        ...
        from derouter.llms.watsonx.chat.transformation import IBMWatsonXChatConfig
...
def _parse_tool_call_arguments(raw: object, tool_name: str | None, context: str) -> dict[str, object]:
    ...
    if not isinstance(raw, str):
        return {}
    normalized_raw: Final = "{}" if raw == REDACTED_BY_DEROUTER else raw
    from derouter.derouter_core_utils.prompt_templates.common_utils import (
        parse_tool_call_arguments,
    )
...
def resolve_structured_messages(
    ...
    route: Final = request_kwargs.get("derouter_metadata", {}).get("user_api_key_request_route")
    if route:
