...
from derouter.constants import MAX_IMAGE_URL_DOWNLOAD_SIZE_MB
from derouter.derouter_core_utils.url_utils import SSRFError, async_safe_get, safe_get
...
def _process_image_response(response: Response, url: str) -> str:
    if response.status_code != 200:
        raise derouter.ImageFetchError(
...
def _rejected_image_fetch(url: str, verdict: SSRFError) -> "derouter.ImageFetchError":
    verbose_logger.warning("Image fetch of %s rejected before any request went out: %s", url, verdict)
    return derouter.ImageFetchError(
...
async def async_convert_url_to_base64(url: str) -> str:
    ...
    client: Final = derouter.module_level_aclient
    for _ in range(3):
        try:
            ...
            pass
    raise derouter.ImageFetchError(f"Error: Unable to fetch image from URL after 3 attempts. url={url}")
