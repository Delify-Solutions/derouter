...
from derouter.router_utils.batch_utils import InMemoryFile
...
if TYPE_CHECKING:  # newer pattern to avoid importing pydantic objects on __init__.py
    from derouter.types.llms.anthropic import AnthropicInputSchema
...
if TYPE_CHECKING:
    from derouter.derouter_core_utils.derouter_logging import Logging as LoggingClass
...
def handle_any_messages_to_chat_completion_str_messages_conversion(
    ...
    Relevant Issue: https://github.com/Delify-Solutions/derouter/issues/9494
    """
    import json
...
def get_format_from_file_id(file_id: str | None) -> str | None:
    ...
    unified_file_id = derouter_proxy:{};unified_id,{}
...
def update_responses_input_with_model_file_ids(
    ...
    Args:
        ...
        model_file_id_mapping: Dictionary mapping derouter file IDs to provider file IDs
                               Format: {"derouter_file_id": {"model_id": "provider_file_id"}}
    """
    from derouter.proxy.openai_files_endpoints.common_utils import (
        _is_base64_encoded_unified_file_id,
        convert_b64_uid_to_unified_uid,
...
def _decode_vector_store_ids_in_tools(
    ...
    """
    Decodes unified (DeRouter-managed) vector_store_ids in file_search tools to
        ...
        for vs_id in vector_store_ids:
            ...
            if not provider_resource_id:
                verbose_logger.warning(
                    ...
                    "Ensure the vector store was created via DeRouter.",
                    vs_id,
                )
                decoded_ids.append(vs_id)
            else:
...
def unpack_legacy_defs(
    ...
    *,
    ...
    dangling pointers. See https://github.com/Delify-Solutions/derouter/issues/26692.
...
def flatten_top_level_schema_combinators(schema: Mapping[str, object]) -> Mapping[str, object]:
    ...
    400s through DeRouter. Branch properties merge without clobbering (the
...
def infer_content_type_from_url_and_content(
    ...
    Example:
        ...
        "image/png"
    """
    from derouter.derouter_core_utils.token_counter import get_image_type
...
DEROUTER_INTERNAL_MESSAGE_FIELDS: Final = frozenset({"thinking_blocks", "reasoning_content", "provider_specific_fields"})


def strip_derouter_internal_message_fields(message: AllMessageValues) -> AllMessageValues:
    """Drop the fields derouter attaches to assistant messages (e.g. when translating Anthropic thinking
    blocks) that OpenAI-compatible endpoints with strict schemas reject as extra inputs."""
    if DEROUTER_INTERNAL_MESSAGE_FIELDS.isdisjoint(message):
        return message
    return cast(  # cast-ok: same TypedDict minus internal keys
        ...
        {  # mutable-ok: provider transforms mutate message dicts in place downstream
            key: value for key, value in message.items() if key not in DEROUTER_INTERNAL_MESSAGE_FIELDS
        },
    )
...
def text_completion_prompt_to_messages(prompt: object) -> tuple[AllMessageValues, ...]:
    ...
    Mirrors what ``derouter.text_completion`` does on the real-time path: a
