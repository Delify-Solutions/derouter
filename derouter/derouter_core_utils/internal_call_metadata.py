...
from derouter.constants import INTERNAL_CALL_ORIGIN_METADATA_KEY, NON_INFERENCE_CALL_TYPES
from derouter.derouter_core_utils.initialize_dynamic_callback_params import initialize_standard_callback_dynamic_params
from derouter.types.utils import BACKGROUND_RESPONSE_COST_POLL_CALL_ORIGIN, InternalCallOrigin
...
``metadata`` and ``derouter_metadata``, ``get_derouter_metadata_from_kwargs`` returns the latter and
...
def is_unbilled_non_inference_call_from_params(
    ...
    """:func:`is_unbilled_non_inference_call` for callers holding raw ``derouter_params``.
    ...
    metadata: Final = (
        StandardLoggingPayloadSetup.merge_derouter_metadata(derouter_params) if derouter_params is not None else None
    )
...
def parent_session_kwargs(request_kwargs: Mapping[str, object] | None) -> Mapping[str, str]:
    ...
    return MappingProxyType(
        {k: v for k in ("derouter_session_id", "derouter_trace_id") if isinstance(v := kwargs.get(k), str)}
    )
