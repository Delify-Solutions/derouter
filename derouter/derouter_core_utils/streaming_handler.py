...
from derouter.derouter_core_utils.model_response_utils import (
    is_model_response_stream_empty,
)
from derouter.derouter_core_utils.redact_messages import DeRouterLoggingObject
from derouter.derouter_core_utils.thread_pool_executor import executor
from derouter.types.llms.openai import OpenAIChatCompletionChunk
...
from derouter.types.utils import GenericStreamingChunk as GChunk
...
class CustomStreamWrapper:
    def __init__(
        ...
        self.logging_obj: DeRouterLoggingObject = logging_obj
        self.completion_stream = completion_stream
        self.sent_first_chunk = False
        self.sent_last_chunk = False
        self._stream_created_time: float = time.time()

        derouter_params: Final[GenericDeRouterParams] = GenericDeRouterParams.model_validate(
            dict(**self.logging_obj.model_call_details.get("derouter_params", {}))
        )
        self.merge_reasoning_content_in_choices: bool = derouter_params.merge_reasoning_content_in_choices or False
        ...
        _model_info: Final[dict] = derouter_params.model_info or {}

        _api_base: Final = get_api_base(
            model=model or "",
            optional_params=self.logging_obj.model_call_details.get("derouter_params", {}),
        )

        self._hidden_params = {
            "model_id": (_model_info.get("id", None)),
            "api_base": _api_base,
        }  # returned as x-derouter-model-id response header in proxy
        ...
        # Snapshot assumes self._hidden_params is populated from derouter_params
    ...
    def _check_max_streaming_duration(self) -> None:
        ...
        from derouter.constants import DEROUTER_MAX_STREAMING_DURATION_SECONDS
    ...
    def raise_on_model_repetition(self) -> None:
        """
        Fixes - https://github.com/Delify-Solutions/derouter/issues/5158
        ...
        ):  # ignore empty content - https://github.com/Delify-Solutions/derouter/issues/5158#issuecomment-2287156946
            self._repeated_messages_count = 1
            return
        ...
        if self._repeated_messages_count >= derouter.REPEATED_STREAMING_CHUNK_LIMIT:
    ...
    def handle_baseten_chunk(self, chunk) -> str:
        try:
            ...
            verbose_logger.exception("derouter.CustomStreamWrapper.handle_baseten_chunk(): Exception occured - %s", e)
            return ""
    ...
    def model_response_creator(
        ...
        ):  # maintain same 'created' across all chunks - https://github.com/Delify-Solutions/derouter/issues/11437
    ...
    def _dispatch_provider_chunk(
        ...
        if (
            ...
            and self.custom_llm_provider in derouter._custom_providers
        ):
        ...
        if (
        ...
        ) or (self.custom_llm_provider and self.custom_llm_provider in derouter._custom_providers):
            if self.received_finish_reason is not None:
            ...
            if anthropic_response_obj["usage"] is not None:
                setattr(
                    model_response,
                    "usage",
                    derouter.Usage(**anthropic_response_obj["usage"]),
                )
                    ...
                    try:
                        ...
                        if "Part has no text." in str(e):
                            ...
                            try:
                                ...
                                raise e
                            _delta_obj: Final = derouter.utils.Delta(
                                content=None,
                                tool_calls=[
                                    {
            ...
            response_obj = cast(
                dict[str, object],
                derouter.CodestralTextCompletionConfig()._chunk_parser(chunk),
            )
            completion_obj["content"] = response_obj["text"]
            print_verbose(f"completion obj content: {completion_obj['content']}")
            if response_obj["is_finished"]:
            ...
            if response_obj["usage"] is not None:
                if isinstance(response_obj["usage"], dict):
                    ...
                    setattr(
                        model_response,
                        "usage",
                        derouter.Usage(**response_obj["usage"].model_dump()),
                    )
                    ...
                    if delta is not None and (delta.function_call is not None or delta.tool_calls is not None):
                        try:
                            ...
                            verbose_logger.exception(
                                "derouter.CustomStreamWrapper.chunk_creator(): Exception occured - %s", e
                            )
    ...
    def set_logging_event_loop(self, loop):
        """
        import derouter, asyncio

        loop = asyncio.get_event_loop() # 👈 gets the current event loop

        response = derouter.completion(.., stream=True)
    ...
    def run_success_logging_and_cache_storage(self, processed_chunk, cache_hit: bool):
        ...
        if derouter.disable_streaming_logging is True:
            """
            [NOT RECOMMENDED]
            Set this via `derouter.disable_streaming_logging = True`.
        ...
        derouter_params: Final = self.logging_obj.model_call_details.get("derouter_params", {})
        if self.logging_obj._is_sync_derouter_request(derouter_params):
    ...
    def _propagate_usage_cost_to_hidden_params(
        ...
        if _cost is not None:
            ...
            response._hidden_params["additional_headers"]["llm_provider-x-derouter-response-cost"] = _cost
        ...
        try:
            ...
            while True:
                ...
                if chunk is not None and chunk != b"":
                    ...
                    ## LOGGING
                    if not derouter.disable_streaming_logging:
                        executor.submit(
            ...
            if self.sent_last_chunk is True:
                try:
                    complete_streaming_response = derouter.stream_chunk_builder(
                        chunks=self.chunks,
    ...
    def fetch_sync_stream(self):
        if self.completion_stream is None and self.make_call is not None:
            ...
            self.completion_stream = self.make_call(client=derouter.module_level_client)
    ...
    async def fetch_stream(self):
        if self.completion_stream is None and self.make_call is not None:
            # Call make_call to get the completion stream
            self.completion_stream = await self.make_call(client=derouter.module_level_aclient)
    ...
    async def __anext__(self) -> "ModelResponseStream":
        ...
        if self.custom_llm_provider is not None and self.custom_llm_provider == "cached_response":
            cache_hit = True
        try:
            # Inside the try (not before it) so a raised derouter.Timeout flows
            ...
            ## ADD DEBUG INFORMATION - E.G. DEROUTER REQUEST TIMEOUT
            traceback_exception += f"\nDeRouter Default Request Timeout - {derouter.request_timeout}"
...
def calculate_total_usage(chunks: list[ModelResponse]) -> Usage:
    ...
    from derouter.derouter_core_utils.streaming_chunk_builder_utils import (
