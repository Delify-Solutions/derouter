...
def _provider_qualified(model: str) -> str | None:
    """`model` in the one spelling derouter itself resolves it to, or None if it maps to no
    provider.
    ...
    """
    try:
        stripped, provider, _, _ = derouter.get_llm_provider(model=model)
...
def judge_target(router: Router | None, model: str, team_id: str | None = None) -> JudgeTarget:
    ...
    composes); the SDK serves it because derouter recognises the provider; or nothing does,
    ...
    """
    served: Final = router.resolved_derouter_models(model, team_id=team_id) if router is not None else ()
    if served:
...
async def judge_acompletion(
    ...
    **params: object,
    ...
    return await derouter.acompletion(model=judge_model, messages=messages, num_retries=0, drop_params=True, **params)
