...
def _ensure_extra_body_is_safe(extra_body: dict | None) -> dict | None:
    ...
    Relevant Issue: https://github.com/Delify-Solutions/derouter/issues/4140
    """
...
def pick_cheapest_chat_models_from_llm_provider(custom_llm_provider: str, n=1):
    ...
    """
    if custom_llm_provider not in derouter.models_by_provider:
        return []

    known_models: Final = derouter.models_by_provider.get(custom_llm_provider, [])
    model_costs: Final = []
...
def get_proxy_server_request_headers(derouter_params: dict | None) -> dict:
    """
    Get the `proxy_server_request` headers from the derouter_params.\

    Use this if you want to access the request headers made to DeRouter proxy server.
    """
    if derouter_params is None:
        return {}

    proxy_request_headers: Final = (derouter_params.get("proxy_server_request") or {}).get("headers") or {}
