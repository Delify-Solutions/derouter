...
PTU_COST_ATTRIBUTION_ENV_VAR: Final = "DEROUTER_ENABLE_PTU_COST_ATTRIBUTION"
...
# charge.
CUSTOM_PRICING_FIELDS: Final = frozenset(f for f in CustomPricingDeRouterParams.model_fields if "cost" in f)
...
def ptu_identity_error(
    *, declared_id: str | None, taken: bool, current_id: str | None = None, model_name: str | None = None
    ...
    ``derouter_params``, so rotating a credential or editing an endpoint mints a second
    ...
    if not declared_id:
        return _named(
            ...
            "identified by a hash of its derouter_params, so rotating a credential bills the "
