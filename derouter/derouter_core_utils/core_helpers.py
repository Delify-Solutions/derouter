...
from derouter.types.llms.openai import AllMessageValues, OpenAIChatCompletionFinishReason
...
DROP_PARAMS_ENV_VAR: Final = "DEROUTER_DROP_PARAMS"
...
def _is_derouter_limit_rejection(exception: BaseException) -> bool:
    from derouter.exceptions import RateLimitErrorCategory

    derouter_limit_categories: Final = frozenset(
        (RateLimitErrorCategory.DEROUTER_RATE_LIMIT.value, RateLimitErrorCategory.DEROUTER_BATCH_RATE_LIMIT.value)
    )
    return getattr(exception, "category", None) in derouter_limit_categories


def _is_proxy_rejection(exception: BaseException) -> bool:
    if _is_derouter_limit_rejection(exception):
        return True
    try:
...
def is_expected_client_error(exception: BaseException | None) -> bool:
    ...
    client error and keeps its traceback: a mapped derouter exception carries
    ...
    so does any exception whose unified rate-limit ``category`` names derouter's
    ...
    derouter exceptions on .status_code.
    """
...
def coerce_token_limit(value: object) -> int | None:
    ...
    A deployment's model_info is registered into derouter.model_cost verbatim, so a
...
def add_missing_spend_metadata_to_derouter_metadata(derouter_metadata: dict, metadata: dict) -> dict:
    """
    Helper to get derouter metadata for spend tracking

    PATCH for issue where both `derouter_metadata` and `metadata` are present in the kwargs
    ...
    for key, value in metadata.items():
        if potential_spend_tracking_metadata_substring in key:
            derouter_metadata[key] = value
    return derouter_metadata


def get_metadata_variable_name_from_kwargs(
    kwargs: Mapping[str, object],
) -> Literal["metadata", "derouter_metadata"]:
    """
    ...
    """
    return "derouter_metadata" if "derouter_metadata" in kwargs else "metadata"


def get_or_create_metadata_bucket(
    request_data: dict,
) -> tuple[Literal["metadata", "derouter_metadata"], dict]:
    """
    Return the proxy-internal metadata bucket for this request, creating it if absent.

    Batch/file routes store proxy state in ``derouter_metadata`` so the OpenAI
...
def get_derouter_metadata_from_kwargs(kwargs: dict):
    """
    Helper to get derouter metadata from all derouter request kwargs

    Return `derouter_metadata` if it exists, otherwise return `metadata`
    """
    derouter_params: Final = kwargs.get("derouter_params", {})
    if derouter_params:
        metadata: Final = derouter_params.get("metadata", {})
        derouter_metadata = derouter_params.get("derouter_metadata", {})
        if derouter_metadata and metadata:
            derouter_metadata = add_missing_spend_metadata_to_derouter_metadata(derouter_metadata, metadata)
...
def _get_parent_otel_span_from_kwargs(
    ...
    try:
        ...
        if "derouter_parent_otel_span" in _metadata:
            return _metadata["derouter_parent_otel_span"]
        elif (
            derouter_params is not None
            and derouter_params.get("metadata") is not None
            and "derouter_parent_otel_span" in derouter_params.get("metadata", {})
        ):
            return derouter_params["metadata"]["derouter_parent_otel_span"]
        elif "derouter_parent_otel_span" in kwargs:
            return kwargs["derouter_parent_otel_span"]
        return None
...
def process_response_headers(
    ...
    preserve_derouter_internal_headers: bool = False,
) -> dict:
    """
    `preserve_derouter_internal_headers` must only be True when the input is a
    DeRouter-owned dict (e.g. `_hidden_params["additional_headers"]` that has
    ...
    remain False, otherwise a malicious provider returning `x-derouter-*` could
    spoof DeRouter-internal markers (e.g. `x-derouter-attempted-fallbacks`).
    ...
    always a raw provider response and can never be DeRouter-owned.
    """
    from derouter.types.utils import OPENAI_RESPONSE_HEADERS

    # Raw httpx.Headers objects come directly from provider HTTP responses and
    # must never be treated as DeRouter-owned, regardless of caller intent.
    _preserve: Final = preserve_derouter_internal_headers and isinstance(response_headers, dict)
    ...
    for k, v in response_headers.items():
        ...
        elif _preserve and k.startswith("x-derouter-"):
            # DeRouter's own internal headers (e.g. x-derouter-attempted-fallbacks,
            # x-derouter-model-group) are not LLM provider headers and must not be
...
def safe_deep_copy(data):
    ...
    The DeRouter request may contain objects that cannot be pickled/deep-copied
    ...
    if derouter.safe_memory_mode is True:
        return data

    derouter_parent_otel_span: object | None = None
    # Step 1: Remove the derouter_parent_otel_span
    derouter_parent_otel_span = None
    if isinstance(data, dict):
        # remove derouter_parent_otel_span since this is not picklable
        if "metadata" in data and "derouter_parent_otel_span" in data["metadata"]:
            derouter_parent_otel_span = data["metadata"].pop("derouter_parent_otel_span")
            data["metadata"]["derouter_parent_otel_span"] = "placeholder"
        if "derouter_metadata" in data and "derouter_parent_otel_span" in data["derouter_metadata"]:
            derouter_parent_otel_span = data["derouter_metadata"].pop("derouter_parent_otel_span")
            data["derouter_metadata"]["derouter_parent_otel_span"] = "placeholder"
    ...
    # Step 3: re-add the derouter_parent_otel_span after doing a deep copy
    if isinstance(data, dict) and derouter_parent_otel_span is not None:
        if "metadata" in data and "derouter_parent_otel_span" in data["metadata"]:
            data["metadata"]["derouter_parent_otel_span"] = derouter_parent_otel_span
        if "derouter_metadata" in data and "derouter_parent_otel_span" in data["derouter_metadata"]:
            data["derouter_metadata"]["derouter_parent_otel_span"] = derouter_parent_otel_span
    return new_data
...
def independent_snapshot(
    ...
    ``derouter.safe_memory_mode``. Unlike ``safe_deep_copy``, which can return
    ...
    Real proxy requests carry ``data["derouter_logging_obj"]`` (a ``Logging``
    ...
    sanitized: Final = {
        key: (
            {  # mutable-ok: same request-payload shape as data
                inner_key: ("placeholder" if inner_key == "derouter_parent_otel_span" else inner_value)
                for inner_key, inner_value in value.items()
            }
            if key in ("metadata", "derouter_metadata") and isinstance(value, dict)
            else value
        )
    ...
    def _copied_value(key: str, sanitized_value: object) -> object:
        ...
        if (
            key in ("metadata", "derouter_metadata")
            and isinstance(copied_value, dict)
            and isinstance(original_value, dict)
            and "derouter_parent_otel_span" in original_value
        ):
            return {  # mutable-ok: same request-payload shape as data
                **copied_value,
                "derouter_parent_otel_span": original_value["derouter_parent_otel_span"],
            }
        return copied_value
...
def filter_exceptions_from_params(data: object, max_depth: int = 20) -> Any:
    ...
    if obj_type_name in ["Logging", "DeRouterLoggingObj", "Router"]:
        return None
...
def filter_internal_params(data: dict, additional_internal_params: set | None = None) -> dict:
    """
    Filter out DeRouter internal parameters that shouldn't be sent to provider APIs.

    This removes internal/MCP-related parameters that are used by DeRouter internally
