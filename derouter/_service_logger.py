...
def _get_otel_v2_class() -> type[_ServiceSpanLogger] | None:
    ...
    Imported lazily: ``derouter.integrations.otel.logger`` imports the OpenTelemetry
    SDK at module scope, so importing it eagerly would break installs without the
    SDK. The V2 logger only exists when ``DEROUTER_OTEL_V2`` is enabled (which
...
class ServiceLogging(CustomLogger):
    """
    Separate class used for monitoring health of derouter-adjacent services (redis/postgres).
    """
    ...
    def __init__(self, mock_testing: bool = False) -> None:
        ...
        if "prometheus_system" in derouter.service_callback:
    ...
    def _resolve_otel_service_logger(self, callback: object) -> _ServiceSpanLogger | None:
        ...
        showed up under ``DEROUTER_OTEL_V2``. Match both the legacy and V2 types,
    ...
    async def async_service_success_hook(
        ...
        for callback in derouter.service_callback:
    ...
    async def async_post_call_failure_hook(
        ...
        """
        Hook to track failed derouter-service calls
        """
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        """
        Hook to track latency for derouter proxy llm api calls
        """
        try:
