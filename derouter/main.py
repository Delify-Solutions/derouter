# DeRouter main module: public completion, embedding, streaming, and moderation entrypoints.
#
# +-----------------------------------------------+
# |                                               |
...
from derouter._logging import _redact_string
...
# client must be imported from derouter as it's a decorator used at function definition time
from derouter import client
...
# Logging is imported lazily when needed to avoid loading derouter_logging at import time
if TYPE_CHECKING:
    from derouter.derouter_core_utils.derouter_logging import Logging
    from derouter.types.utils import TokenCountResponse
...
from derouter.exceptions import DeRouterUnknownProvider
from derouter.integrations.custom_logger import CustomLogger
from derouter.derouter_core_utils.asyncify import run_async_function
from derouter.derouter_core_utils.audio_utils.utils import (
    calculate_request_duration,
    get_audio_file_for_health_check,
)
from derouter.derouter_core_utils.chat_completion_agentic_loop import (
    maybe_run_chat_completion_agentic_loop,
)
from derouter.derouter_core_utils.completion_timeout import CompletionTimeout
...
from derouter.derouter_core_utils.health_check_utils import (
...
from derouter.derouter_core_utils.mock_functions import (
    mock_embedding,
...
from derouter.llms.base_llm import BaseConfig, BaseImageGenerationConfig
from derouter.llms.base_llm.base_model_iterator import (
...
from derouter.llms.openai.chat.gpt_5_transformation import OpenAIGPT5Config
...
from derouter.realtime_api.main import _realtime_health_check
from derouter.secret_managers.main import get_secret_bool, get_secret_str
from derouter.types.completion import (
...
from .derouter_core_utils.core_helpers import safe_deep_copy
from .derouter_core_utils.fallback_utils import (
    async_completion_with_fallbacks,
    completion_with_fallbacks,
)
from .derouter_core_utils.prompt_templates.common_utils import (
    add_system_prompt_to_messages,
    get_completion_messages,
    update_messages_with_model_file_ids,
)
from .derouter_core_utils.prompt_templates.factory import (
    custom_prompt,
...
from .derouter_core_utils.streaming_chunk_builder_utils import ChunkProcessor
...
class DeRouter:
    def __init__(
        ...
        *,
        ...
        max_retries: int | None = derouter.num_retries,
...
async def acompletion(
    ...
    # Per-request JSON schema validation (overrides derouter.enable_json_schema_validation)
    enable_json_schema_validation: bool | None = None,
    **kwargs,
) -> ModelResponse | CustomStreamWrapper:
    """
    Asynchronously executes a derouter.completion() call for any of derouter supported llms (example gpt-4, gpt-3.5-turbo, claude-2, command-nightly)

    Parameters:
        model (str): The name of the language model to use for text completion. see all supported LLMs: https://router.delify.vn/docs/providers/
    ...
    #########################################################
    #########################################################
    derouter_logging_obj: Final = kwargs.get("derouter_logging_obj", None)

    from derouter.integrations.anthropic_cache_control_hook import (
    ...
    if isinstance(derouter_logging_obj, DeRouterLoggingObj) and (
        derouter_logging_obj.should_run_prompt_management_hooks(
        ...
        (
            ...
            messages,
            _,
        ) = await derouter_logging_obj.async_get_chat_completion_prompt(
            model=model,
            messages=messages,
            non_default_params=kwargs,
            prompt_id=kwargs.get("prompt_id", None),
        ...
        # relevant issue: https://github.com/Delify-Solutions/derouter/issues/11404
        #########################################################
    ...
    fallbacks = fallbacks or derouter.model_fallbacks
    if fallbacks is not None:
        response = await async_completion_with_fallbacks(**completion_kwargs, kwargs={"fallbacks": fallbacks, **kwargs})
        if response is None:
            raise Exception(
                "No response from fallbacks. Got none. Turn on `derouter.set_verbose=True` to see more details."
            )
        return response
    ...
    try:
        ...
        if (
            ...
            response = derouter.OpenAITextCompletionConfig().convert_to_chat_model_response_object(
        ...
        if isinstance(response, derouter.ModelResponse):
            looped: Final = await maybe_run_chat_completion_agentic_loop(
                ...
                kwargs=kwargs,
                logging_obj=kwargs.get("derouter_logging_obj"),
                custom_llm_provider=custom_llm_provider,
                stream=bool(stream),
            )
...
def _handle_mock_potential_exceptions(
    ...
    elif isinstance(mock_response, str) and mock_response == "derouter.RateLimitError":
        raise derouter.RateLimitError(
            message="this is a mock rate limit error",
            llm_provider=getattr(mock_response, "llm_provider", custom_llm_provider or "openai"),
            model=model,
        )
    elif isinstance(mock_response, str) and mock_response == "derouter.ContextWindowExceededError":
        raise derouter.ContextWindowExceededError(
            message="this is a mock context window exceeded error",
            llm_provider=getattr(mock_response, "llm_provider", custom_llm_provider or "openai"),
            model=model,
        )
    elif isinstance(mock_response, str) and mock_response == "derouter.InternalServerError":
...
def mock_completion(
    ...
    **kwargs,
    ...
    Returns:
        derouter.ModelResponse: A ModelResponse simulating a completion response with the specified model, messages, and mock response.
    ...
    try:
        ...
        if isinstance(mock_response, str) and mock_response.startswith("Exception: mock_streaming_error"):
            mock_response = derouter.MockException(
        ...
        if n is None:
            ...
            _all_choices: Final = []
            for i in range(n):
                _choice = derouter.utils.Choices(
                    index=i,
                    message=derouter.utils.Message(content=mock_response, role="assistant"),
                )
        ...
        try:
            _, custom_llm_provider, _, _ = derouter.utils.get_llm_provider(model=model)
...
def _resolve_openai_api_base(api_base: str | None) -> str:
    ...
    set via ``derouter.api_base`` or ``OPENAI_BASE_URL``/``OPENAI_API_BASE`` is invisible to the gate,
...
def responses_api_bridge_check(
    ...
    if derouter.route_all_chat_openai_to_responses and custom_llm_provider == "openai":
    ...
    # does, so a custom base set via derouter.api_base or OPENAI_BASE_URL/OPENAI_API_BASE isn't misread
...
def _build_custom_pricing_entry(
    ...
    Collects all CustomPricingDeRouterParams fields present in kwargs and
    ...
    """
    entry: Final[dict] = {"derouter_provider": custom_llm_provider}

    for field_name in CustomPricingDeRouterParams.model_fields:
...
def _get_router_deployment_id(kwargs: dict) -> str | None:
    for metadata_key in ("derouter_metadata", "metadata"):
...
def _register_custom_pricing_for_request(
    ...
    """Register per-request custom pricing in derouter.model_cost.
    ...
    if deployment_id is None:
        derouter.register_model({shared_key: entry}, persist_across_reloads=False)
        return
    derouter.register_model(
        {
            deployment_id: entry,
            shared_key: CustomPricingDeRouterParams.strip_custom_pricing_fields(entry),
        },
...
def _complete_azure(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    headers = ctx.headers
    derouter_params: Final = ctx.derouter_params
    ...
    api_base = api_base or derouter.api_base or get_secret("AZURE_API_BASE")

    api_version = (
        api_version or derouter.api_version or get_secret_str("AZURE_API_VERSION") or derouter.AZURE_DEFAULT_API_VERSION
    )

    api_key = (
        api_key
        or derouter.api_key
        or derouter.azure_key
    ...
    azure_ad_token_provider_value: Final = derouter_params.get("azure_ad_token_provider", None)
    azure_ad_token_provider: Final = azure_ad_token_provider_value if callable(azure_ad_token_provider_value) else None

    headers = headers or derouter.headers
    ...
    if derouter.AzureOpenAIO1Config().is_o_series_model(model=_azure_detection_model):
        ## LOAD CONFIG - if set
        config = derouter.AzureOpenAIO1Config.get_config()
        ...
        ## LOAD CONFIG - if set
        config = derouter.AzureOpenAIConfig.get_config()
        for k, v in _provider_config_items(config):
            if (
                k not in optional_params
            ):  # completion(top_k=3) > azure_config(top_k=3) <- allows for dynamic variables to be passed in
...
def _complete_azure_text(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_base = api_base or derouter.api_base or get_secret_str("AZURE_API_BASE")
    ...
    api_version = api_version or derouter.api_version or get_secret_str("AZURE_API_VERSION")
    ...
    ## LOAD CONFIG - if set
    config: Final = derouter.AzureOpenAIConfig.get_config()
    for k, v in _provider_config_items(config):
        if (
            k not in optional_params
        ):  # completion(top_k=3) > azure_config(top_k=3) <- allows for dynamic variables to be passed in
...
def _complete_azure_ai(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    if azure_ai_route == "agents":
        from derouter.llms.azure_ai.agents import AzureAIAgentsConfig
    ...
    # Check if this is a Claude model - route to Azure Anthropic handler
        ...
        response = azure_anthropic_chat_completions.completion(
            ...
            api_base=api_base,
            acompletion=acompletion,
            custom_prompt_dict=derouter.custom_prompt_dict,
            model_response=model_response,
            print_verbose=print_verbose,
            optional_params=optional_params,
            derouter_params=derouter_params,
...
def _complete_text_completion_openai(
    ...
    api_key = api_key or derouter.api_key or derouter.openai_key or get_secret("OPENAI_API_KEY")

    headers = headers or derouter.headers

    ## LOAD CONFIG - if set
    config: Final = derouter.OpenAITextCompletionConfig.get_config()
    for k, v in _provider_config_items(config):
        if (
            k not in optional_params
        ):  # completion(top_k=3) > openai_text_config(top_k=3) <- allows for dynamic variables to be passed in
            optional_params[k] = v
    if derouter.organization:
        openai.organization = derouter.organization
    ...
    if optional_params.get("stream", False) is False and acompletion is False and text_completion is False:
        ...
        _response = derouter.OpenAITextCompletionConfig().convert_to_chat_model_response_object(
...
def _complete_fireworks_ai(
    ...
    optional_params: Final = (
        ...
        if isinstance(provider_config, derouter.FireworksAIConfig)
...
def _complete_groq(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_key = (
        api_key
        or derouter.api_key  # for deepinfra/perplexity/anyscale/friendliai we check in get_llm_provider and pass in the api key from there
        or derouter.groq_key
    ...
    ## LOAD CONFIG - if set
    config: Final = derouter.GroqChatConfig.get_config()
    for k, v in _provider_config_items(config):
        if (
            k not in optional_params
...
def _complete_bedrock_mantle(
    ...
    api_base = api_base or derouter.api_base or get_secret("BEDROCK_MANTLE_API_BASE")
    api_key = api_key or derouter.api_key or get_secret("BEDROCK_MANTLE_API_KEY")
    headers = headers or derouter.headers
    config: Final = derouter.BedrockMantleChatConfig.get_config()
    for k, v in _provider_config_items(config):
...
def _complete_a2a(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    (
        ...
        api_key,
        headers,
    ) = derouter.A2AConfig.resolve_agent_config_from_registry(
        model=model,
        api_base=api_base,
        api_key=api_key,
        headers=headers,
    ...
    api_base = api_base or derouter.api_base or get_secret_str("A2A_API_BASE")
...
def _complete_sap(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    headers = headers or derouter.headers
    ## LOAD CONFIG - if set
    config: Final = derouter.GenAIHubOrchestrationConfig.get_config()
    for k, v in _provider_config_items(config):
        if (
            k not in optional_params
        ):  # completion(top_k=3) > openai_config(top_k=3) <- allows for dynamic variables to be passed in
...
def _complete_cometapi(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_key = api_key or derouter.cometapi_key or get_secret_str("COMETAPI_KEY") or derouter.api_key

    api_base = api_base or derouter.api_base or get_secret_str("COMETAPI_API_BASE") or "https://api.cometapi.com/v1"
...
def _complete_minimax(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_key = api_key or get_secret_str("MINIMAX_API_KEY") or derouter.api_key

    api_base = api_base or derouter.api_base or get_secret_str("MINIMAX_API_BASE") or "https://api.minimax.io/v1"
...
def _complete_hosted_vllm(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_base = api_base or derouter.api_base or get_secret_str("HOSTED_VLLM_API_BASE")
...
def _complete_custom_openai(
    ...
    if custom_llm_provider == "github_copilot":
        from derouter.llms.github_copilot.authenticator import Authenticator
        from derouter.llms.github_copilot.common_utils import (
    ...
    if derouter.enable_preview_features and metadata is not None:  # [PREVIEW] allow metadata to be passed to OPENAI
    ...
    ## LOAD CONFIG - if set
    config: Final = derouter.OpenAIConfig.get_config()
    for k, v in _provider_config_items(config):
        if (
            k not in optional_params
        ):  # completion(top_k=3) > openai_config(top_k=3) <- allows for dynamic variables to be passed in
...
def _complete_mistral(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_key = api_key or derouter.api_key or get_secret("MISTRAL_API_KEY")
    api_base = api_base or derouter.api_base or get_secret("MISTRAL_API_BASE") or "https://api.mistral.ai/v1"
...
def _complete_replicate(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    replicate_key: Final = (
        api_key
        or derouter.replicate_key
        or derouter.api_key
        or get_secret("REPLICATE_API_KEY")
        or get_secret("REPLICATE_API_TOKEN")
    )

    api_base = api_base or derouter.api_base or get_secret("REPLICATE_API_BASE") or "https://api.replicate.com/v1"

    custom_prompt_dict = custom_prompt_dict or derouter.custom_prompt_dict
...
def _complete_anthropic_text(
    ...
    api_key = api_key or derouter.anthropic_key or derouter.api_key or os.environ.get("ANTHROPIC_API_KEY")
    custom_prompt_dict = custom_prompt_dict or derouter.custom_prompt_dict
    api_base = cast(
        str | None,
        api_base
    ...
    disable_url_suffix: Final = get_secret_bool("DEROUTER_ANTHROPIC_DISABLE_URL_SUFFIX")
    if api_base is not None and not disable_url_suffix and not api_base.endswith("/v1/complete"):
        ...
        verbose_logger.debug("DEROUTER_ANTHROPIC_DISABLE_URL_SUFFIX is set, skipping /v1/complete suffix")
...
def _complete_anthropic(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    if api_base is not None and not disable_url_suffix and not api_base.endswith("/v1/messages"):
        ...
        verbose_logger.debug("DEROUTER_ANTHROPIC_DISABLE_URL_SUFFIX is set, skipping /v1/messages suffix")
...
def _complete_nlp_cloud(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    nlp_cloud_key: Final = api_key or derouter.nlp_cloud_key or get_secret("NLP_CLOUD_API_KEY") or derouter.api_key

    api_base = api_base or derouter.api_base or get_secret("NLP_CLOUD_API_BASE") or "https://api.nlpcloud.io/v1/gpu/"
...
def _complete_aleph_alpha(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_base = (
        api_base or derouter.api_base or get_secret("ALEPH_ALPHA_API_BASE") or "https://api.aleph-alpha.com/complete"
    )
    ...
    model_response = aleph_alpha.completion(
        ...
        logger_fn=logger_fn,
        encoding=_get_encoding(),
        default_max_tokens_to_sample=derouter.max_tokens,
...
def _complete_cohere_chat(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    if cohere_route == "v2":
        api_base = api_base or derouter.api_base or get_secret_str("COHERE_API_BASE") or "https://api.cohere.com/v2/chat"
        # Remove v2/ prefix from model name for the actual API call
        if "v2/" in model:
            model = model.replace("v2/", "")
    else:
        api_base = api_base or derouter.api_base or get_secret_str("COHERE_API_BASE") or "https://api.cohere.ai/v1/chat"
...
def _complete_maritalk(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    maritalk_key: Final = api_key or derouter.maritalk_key or get_secret("MARITALK_API_KEY") or derouter.api_key

    api_base = api_base or derouter.api_base or get_secret("MARITALK_API_BASE") or "https://chat.maritaca.ai/api"
...
def _complete_amazon_nova(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_key = api_key or derouter.amazon_nova_api_key or get_secret_str("AMAZON_NOVA_API_KEY") or derouter.api_key
    api_base = (
        api_base or derouter.api_base or get_secret_str("AMAZON_NOVA_API_BASE") or "https://api.nova.amazon.com/v1"
    )
    return openai_like_chat_completion.completion(
        model=model,
        messages=messages,
...
def _complete_huggingface(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    huggingface_key: Final = (
        api_key
        or derouter.huggingface_key
        or os.environ.get("HF_TOKEN")
        or os.environ.get("HUGGINGFACE_API_KEY")
        or derouter.api_key
    )
    hf_headers: Final = headers or derouter.headers
    return base_llm_http_handler.completion(
        model=model,
        messages=messages,
        headers=hf_headers,
...
def _complete_compactifai(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_key = api_key or get_secret_str("COMPACTIFAI_API_KEY") or derouter.api_key
...
def _complete_databricks(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_key = (
        api_key
        or derouter.api_key  # for databricks we check in get_llm_provider and pass in the api key from there
        or derouter.databricks_key
...
def _complete_openrouter(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_base = api_base or derouter.api_base or get_secret_str("OPENROUTER_API_BASE") or "https://openrouter.ai/api/v1"

    api_key = (
        api_key
        or derouter.api_key
        or derouter.openrouter_key
        or get_secret_str("OPENROUTER_API_KEY")
        or get_secret_str("OR_API_KEY")
    )

    openrouter_site_url: Final = get_secret("OR_SITE_URL") or "https://router.delify.vn"
    ...
    _headers: Final = headers or derouter.headers
    if _headers:
    ...
    ## Load Config
    config: Final = derouter.OpenrouterConfig.get_config()
    for k, v in _provider_config_items(config):
        if k == "extra_body":
...
def _complete_vercel_ai_gateway(
    ...
    api_key = api_key or derouter.api_key or get_secret("VERCEL_AI_GATEWAY_API_KEY")

    vercel_site_url: Final = get_secret("VERCEL_SITE_URL") or "https://router.delify.vn"
    ...
    ## Load Config
    config: Final = derouter.VercelAIGatewayConfig.get_config()
    for k, v in _provider_config_items(config):
        if k == "extra_body":
...
def _complete_vertex_ai_beta(
    ...
    api_base = api_base or derouter.api_base or get_secret("GEMINI_API_BASE")
...
def _complete_vertex_ai(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_base = api_base or derouter.api_base or get_secret("VERTEXAI_API_BASE")

    new_params: Final = safe_deep_copy(optional_params or {})
    model_route: Final = get_vertex_ai_model_route(model=model, derouter_params=derouter_params)
    ...
    if model_route == VertexAIModelRoute.PARTNER_MODELS:
        ...
        from derouter.llms.vertex_ai.agent_engine.transformation import (
            VertexAgentEngineConfig,
        )

        vertex_agent_engine_config: Final = VertexAgentEngineConfig()

        # Update derouter_params with vertex credentials
        derouter_params["vertex_project"] = vertex_ai_project
        derouter_params["vertex_location"] = vertex_ai_location
        derouter_params["vertex_credentials"] = vertex_credentials
...
def _complete_predibase(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    tenant_id: Final = (
        ...
        or derouter.predibase_tenant_id
    ...
    api_key = api_key or derouter.api_key or derouter.predibase_key or get_secret("PREDIBASE_API_KEY")
...
def _complete_text_completion_codestral(
    ...
    api_key = api_key or derouter.api_key or get_secret("CODESTRAL_API_KEY")

    text_completion_model_response: Final = derouter.TextCompletionResponse(stream=stream)
...
def _complete_text_completion_inception(
    ...
    if passed_api_base is None or api_key:
        api_key = api_key or derouter.inception_key or get_secret_str("INCEPTION_API_KEY")
...
def _complete_bedrock(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    if "aws_bedrock_client" in optional_params:
        verbose_logger.warning(
            "'aws_bedrock_client' is a deprecated param. Please move to another auth method - https://router.delify.vn/docs/providers/bedrock#boto3---authentication."
        )
...
def _complete_ollama(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_base = derouter.api_base or api_base or get_secret("OLLAMA_API_BASE") or "http://localhost:11434"
...
def _complete_ollama_chat(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_key = api_key or derouter.ollama_key or os.environ.get("OLLAMA_API_KEY") or derouter.api_key
    if api_key is not None and "Authorization" not in headers:
        headers["Authorization"] = f"Bearer {api_key}"
...
def _complete_triton(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_base = derouter.api_base or api_base
    return base_llm_http_handler.completion(
        model=model,
        stream=stream,
        messages=messages,
...
def _complete_cloudflare(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_key = api_key or derouter.cloudflare_api_key or derouter.api_key or get_secret("CLOUDFLARE_API_KEY")
    api_base = api_base or derouter.api_base or get_secret("CLOUDFLARE_API_BASE")
...
def _complete_petals(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_base = api_base or derouter.api_base
...
def _complete_gdc(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_key = api_key or derouter.gdc_key or get_secret_str("GDC_API_KEY") or derouter.api_key
    api_base = api_base or derouter.gdc_api_base or get_secret_str("GDC_API_BASE") or derouter.api_base
...
def _complete_bytez(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_key = api_key or derouter.bytez_key or get_secret_str("BYTEZ_API_KEY") or derouter.api_key
...
def _complete_lemonade(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_key = api_key or derouter.lemonade_key or get_secret_str("LEMONADE_API_KEY") or derouter.api_key
...
def _complete_ovhcloud(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    api_key = api_key or derouter.ovhcloud_key or get_secret_str("OVHCLOUD_API_KEY") or derouter.api_key
...
def _complete_custom(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    url: Final = derouter.api_base or api_base or ""
    if url is None or url == "":
        raise ValueError("api_base not set. Set api_base or derouter.api_base for custom endpoints")

    """
    assume input to custom LLM api bases follow this format:
    resp = derouter.module_level_client.post(
        api_base,
        json={
    ...
    resp: Final = derouter.module_level_client.post(
        url,
        headers=headers,
        json={
            "model": model,
...
def _complete_custom_providers(
    ...
    for item in derouter.custom_provider_map:
        if item["provider"] == custom_llm_provider:
            custom_handler = item["custom_handler"]

    if custom_handler is None:
        raise DeRouterUnknownProvider(model=model, custom_llm_provider=custom_llm_provider)
...
def _complete_langgraph(ctx: _CompletionDispatchContext) -> _CompletionDispatchResult:
    ...
    (
        ...
        api_base=api_base or derouter.api_base,
        api_key=api_key or derouter.api_key,
    )
...
def completion(
    ...
    **kwargs,
    ...
    """
    Perform a completion() using any of derouter supported llms (example gpt-4, gpt-3.5-turbo, claude-2, command-nightly)
    Parameters:
        model (str): The name of the language model to use for text completion. see all supported LLMs: https://router.delify.vn/docs/providers/
        messages (List): A list of message objects representing the conversation context (default is an empty list).
    ...
    if not skip_mcp_handler and tools:
        from derouter.responses.mcp.chat_completions_handler import acompletion_with_mcp
        from derouter.responses.mcp.derouter_proxy_mcp_handler import (
            DeRouter_Proxy_MCP_Handler,
        )
        from derouter.types.llms.openai import ToolParam
        ...
        if DeRouter_Proxy_MCP_Handler._should_use_derouter_mcp_gateway(tools=tools_for_mcp):
    ...
    if derouter.proxy_auth is not None:
        try:
            proxy_headers: Final = _proxy_auth_headers(derouter.proxy_auth)
    ...
    derouter_system_prompt: Final = kwargs.get("derouter_system_prompt", None)
    ### COPY MESSAGES ### - related issue https://github.com/Delify-Solutions/derouter/discussions/4489
    ...
    derouter_params: dict[str, object] = {}  # used to prevent unbound var errors
    ## PROMPT MANAGEMENT HOOKS ##
    ...
    ### DEROUTER SYSTEM PROMPT ###
    if derouter_system_prompt:
        messages = add_system_prompt_to_messages(
            messages=messages,
            system_prompt=derouter_system_prompt,
    ...
    try:
        if base_url is not None:
            api_base = base_url
        is_router_call: Final = any("model_group" in (kwargs.get(k) or ()) for k in ("metadata", "derouter_metadata"))
        if is_router_call:
            max_retries = 0
        elif num_retries is not None:
            max_retries = num_retries
        logging: Final[DeRouterLoggingObj] = cast(DeRouterLoggingObj, derouter_logging_obj)
        fallbacks = fallbacks or derouter.model_fallbacks
        if fallbacks is not None:
        ...
        if model_list is not None:
            deployments: Final = [m["derouter_params"] for m in model_list if m["model_name"] == model]
            return derouter.batch_completion_models(  # pyright: ignore[reportReturnType]  # batch path returns a list of responses, outside completion()'s single-response return type
        ...
        setattr(model_response, "usage", derouter.Usage())
        if (
        ...
        model, custom_llm_provider, dynamic_api_key, api_base = get_llm_provider(
            ...
            api_key=api_key,
            derouter_params=(
                GenericDeRouterParams(**_supplemental_provider_params) if _supplemental_provider_params else None
            ),
        )
        ...
        if derouter.add_function_to_prompt and optional_params.get(
        ...
        # For logging - save the values of the derouter-specific params passed in
        derouter_params = get_derouter_params(
            ...
            custom_llm_provider=custom_llm_provider,
            api_base=api_base,
            derouter_call_id=kwargs.get("derouter_call_id", None),
            model_alias_map=derouter.model_alias_map,
            ...
            derouter_session_id=kwargs.get("derouter_session_id"),
            hf_model_name=hf_model_name,
            custom_prompt_dict=custom_prompt_dict,
            ...
            use_derouter_proxy=kwargs.get("use_derouter_proxy", False),
            api_version=api_version,
            ...
            max_retries=max_retries,
            timeout=timeout,
            derouter_request_debug=kwargs.get("derouter_request_debug", False),
            tpm=kwargs.get("tpm"),
            rpm=kwargs.get("rpm"),
        ...
        cast(DeRouterLoggingObj, logging).update_environment_variables(
            model=model,
            user=user,
        ...
        ## RESPONSES API BRIDGE LOGIC ## - check if model has 'mode: responses' in derouter.model_cost map
        ...
        if responses_api_model_info.get("mode") == "responses" and not skip_responses_api_bridge:
            from derouter.completion_extras import responses_api_bridge
            ...
            custom_llm_provider == "azure"
            and derouter.AzureOpenAIGPT5Config.is_model_gpt_5_model(_azure_detection_model)
        ):
        ...
        if custom_llm_provider == "azure":
            ...
            or custom_llm_provider in derouter.openai_text_completion_compatible_providers
            ...
            (model in derouter.open_ai_chat_completion_models and custom_llm_provider in (None, "openai"))
        ...
        elif "replicate" in model or custom_llm_provider == "replicate" or model in derouter.replicate_models:
            # Setting the relevant API KEY for replicate, replicate defaults to using os.environ.get("REPLICATE_API_TOKEN")
            response = _complete_replicate(_dispatch_ctx)
        elif "clarifai" in model or custom_llm_provider == "clarifai" or model in derouter.clarifai_models:
        ...
        elif custom_llm_provider == "petals" or model in derouter.petals_models:
            response = _complete_petals(_dispatch_ctx)
        elif custom_llm_provider == "snowflake" or model in derouter.snowflake_models:
        ...
        elif custom_llm_provider == "ovhcloud" or model in derouter.ovhcloud_models:
            response = _complete_ovhcloud(_dispatch_ctx)

        elif custom_llm_provider == "custom":
            response = _complete_custom(_dispatch_ctx)

        elif custom_llm_provider in derouter._custom_providers:  # Assume custom LLM provider
...
def completion_with_retries(*args, **kwargs):
    """
    Executes a derouter.completion() with 3 retries
    """
    try:
        import tenacity
    except Exception as e:
...
def responses_with_retries(*args, **kwargs):
    """
    Executes a derouter.responses() with retries
    """
    try:
        import tenacity
    except Exception as e:
    ...
    from derouter.responses.main import responses
...
async def aresponses_with_retries(*args, **kwargs):
    """
    Executes a derouter.aresponses() with retries
    """
    try:
        import tenacity
    except Exception as e:
    ...
    from derouter.responses.main import aresponses
...
def embedding(
    ...
    **kwargs,
    ...
    - derouter_call_id: The call ID for derouter logging.
    - derouter_logging_obj: The derouter logging object.
    ...
    derouter_params: Final = [
        "aembedding",
        "extra_headers",
    ] + all_derouter_params

    default_params: Final = openai_params + derouter_params
    ...
    derouter_params_dict: Final = get_derouter_params(**kwargs)

    logging: Final[DeRouterLoggingObj] = derouter_logging_obj
    logging.update_environment_variables(
        model=model,
        user=user,
        optional_params=optional_params,
        derouter_params=derouter_params_dict,
        custom_llm_provider=custom_llm_provider,
    )
    ...
    try:
        ...
        if azure is True or custom_llm_provider == "azure":
            ...
            api_key = api_key or derouter.api_key or derouter.azure_key or get_secret_str("AZURE_API_KEY")
            ...
            or custom_llm_provider == "nvidia_nim"
            or custom_llm_provider == "derouter_proxy"
            or (model in derouter.open_ai_embedding_models and custom_llm_provider is None)
        ):
            api_base = (
                api_base
                or derouter.api_base
            ...
            openai.organization = (
                derouter.organization
                or get_secret_str("OPENAI_ORGANIZATION")
                or None  # default - https://github.com/openai/openai-python/blob/284c1799070c723c6a553337134148a7ab088dd8/openai/util.py#L105
            )
            # set API KEY
            api_key = api_key or derouter.api_key or derouter.openai_key or get_secret_str("OPENAI_API_KEY")
            ...
            requested_encoding_format: Final = (
                ...
                or get_secret_str("DEROUTER_DEFAULT_EMBEDDING_ENCODING_FORMAT")
            )
            ...
            api_base = api_base or derouter.api_base or get_secret("DATABRICKS_API_BASE")

            # set API KEY
            api_key = api_key or derouter.api_key or derouter.databricks_key or get_secret("DATABRICKS_API_KEY")
            ...
            # set API KEY
            if api_key is None:
                api_key = derouter.api_key or get_secret_str("HOSTED_VLLM_API_KEY")
            ...
            api_base = api_base or derouter.api_base or get_secret_str("OPENAI_LIKE_API_BASE")

            # set API KEY
            if api_key is None:
                api_key = api_key or derouter.api_key or derouter.openai_like_key or get_secret_str("OPENAI_LIKE_API_KEY")
            ...
            api_key = api_key or derouter.huggingface_key or get_secret("HUGGINGFACE_API_KEY") or derouter.api_key
            ...
            gemini_api_key: Final = api_key or get_api_key_from_env() or derouter.api_key

            api_base = api_base or derouter.api_base or get_secret_str("GEMINI_API_BASE")
            ...
            api_base = (
                api_base or derouter.api_base or get_secret_str("VERTEXAI_API_BASE") or get_secret_str("VERTEX_API_BASE")
            )
            ...
            api_base = derouter.api_base or api_base or get_secret_str("OLLAMA_API_BASE") or "http://localhost:11434"
            ...
            api_key = api_key or derouter.api_key or get_secret_str("MISTRAL_API_KEY")
            response = openai_chat_completions.embedding(
                model=model,
                input=input,
                api_base=api_base,
            ...
            api_key = api_key or derouter.api_key or get_secret_str("FIREWORKS_AI_API_KEY")
            response = openai_chat_completions.embedding(
                model=model,
                input=input,
                api_base=api_base,
            ...
            api_key = api_key or derouter.api_key or get_secret_str("NEBIUS_API_KEY")
            api_base = api_base or derouter.api_base or get_secret_str("NEBIUS_API_BASE") or "api.studio.nebius.ai/v1"
            ...
            api_key = api_key or derouter.api_key or get_secret_str("WANDB_API_KEY")
            api_base = (
                api_base or derouter.api_base or get_secret_str("WANDB_API_BASE") or "https://api.inference.wandb.ai/v1"
            )
            ...
            api_key = api_key or derouter.api_key or get_secret_str("SAMBANOVA_API_KEY")
            api_base = (
                api_base or derouter.api_base or get_secret_str("SAMBANOVA_API_BASE") or "https://api.sambanova.ai/v1"
            )
            response = base_llm_http_handler.embedding(
                model=model,
                input=input,
            ...
            api_key = (
                api_key or derouter.api_key or get_secret_str("XINFERENCE_API_KEY") or "stub-xinference-key"
            )  # xinference does not need an api key, pass a stub key if user did not set one
            api_base = (
                api_base or derouter.api_base or get_secret_str("XINFERENCE_API_BASE") or "http://127.0.0.1:9997/v1"
            )
            response = openai_chat_completions.embedding(
                model=model,
                input=input,
            ...
            from derouter.llms.azure_ai.common_utils import get_azure_ai_entra_token
            ...
            api_key = (
                ...
                or get_azure_ai_entra_token(derouter_params=derouter_params_dict)
            )
            ...
            volcengine_key: Final = (
                api_key or derouter.api_key or get_secret_str("ARK_API_KEY") or get_secret_str("VOLCENGINE_API_KEY")
            )
            ...
            api_key = api_key or derouter.api_key or get_secret_str("OVHCLOUD_API_KEY")
            api_base = (
                api_base
                or derouter.api_base
                or get_secret_str("OVHCLOUD_API_BASE")
            ...
            api_key = api_key or derouter.cometapi_key or get_secret_str("COMETAPI_KEY") or derouter.api_key
            api_base = (
                api_base or derouter.api_base or get_secret_str("COMETAPI_API_BASE") or "https://api.cometapi.com/v1"
            )
            response = base_llm_http_handler.embedding(
                model=model,
                input=input,
            ...
            response = base_llm_http_handler.embedding(
                ...
                client=client,
                aembedding=aembedding,
                derouter_params={"ssl_verify": kwargs.get("ssl_verify", None)},
            )
        ...
        ## LOGGING
        derouter_logging_obj.post_call(
            input=input,
            api_key=api_key,
            original_response=str(e),
        )
...
async def atext_completion(*args, **kwargs) -> TextCompletionResponse | TextCompletionStreamWrapper:
    ...
    try:
        ...
        if (
            ...
            text_completion_response = derouter.utils.DeRouterResponseObjectHandler.convert_chat_to_text_completion(
...
def text_completion(
    ...
    **kwargs,
    ...
    # default values for all optional params are none, derouter only passes them to the llm when they are set to non None values
    ...
    if isinstance(prompt, list) and len(prompt) > 0 and isinstance(prompt[0], str):
        ...
        raise Exception(
            f"Unmapped prompt format. Your prompt is neither a list of strings nor a string. prompt={prompt}. File an issue - https://github.com/Delify-Solutions/derouter/issues"
        )
    ...
    if (
        ...
        if _model not in derouter.open_ai_chat_completion_models:
...
async def aadapter_completion(*, adapter_id: str, **kwargs) -> BaseModel | AdapterCompletionStreamWrapper | None:
    ...
    try:
        ...
        for item in derouter.adapters:
            if item["id"] == adapter_id:
                translation_obj = item["adapter"]

        if translation_obj is None:
            raise ValueError(
                f"No matching adapter given. Received 'adapter_id'={adapter_id}, derouter.adapters={derouter.adapters}"
            )
...
def moderation(input: str, model: str | None = None, api_key: str | None = None, **kwargs) -> OpenAIModerationResponse:
    ...
    return derouter.utils.DeRouterResponseObjectHandler.convert_to_moderation_response(
...
async def amoderation(
    ...
    **kwargs,
    ...
    # only supports open ai for now
    api_key = api_key or derouter.api_key or derouter.openai_key or get_secret_str("OPENAI_API_KEY")
    optional_params: Final = GenericDeRouterParams(**kwargs)
    derouter_logging_obj: Final[DeRouterLoggingObj | None] = kwargs.get("derouter_logging_obj", None)
    ...
    # update derouter_logging_obj with environment variables
    custom_llm_provider = custom_llm_provider or derouter.LlmProviders.OPENAI.value
    if derouter_logging_obj is not None:
        derouter_logging_obj.update_environment_variables(
            model=model,
...
def transcription(
    ...
    **kwargs,
    ...
    Allows router to load balance between them
    """
    derouter_call_id: Final = kwargs.get("derouter_call_id", None)
    proxy_server_request: Final = kwargs.get("proxy_server_request", None)
    model_info: Final = kwargs.get("model_info", None)
    metadata: Final = kwargs.get("metadata", None)
    ...
    if derouter_logging_obj:
        derouter_logging_obj.model_call_details["client"] = str(client)

    if max_retries is None:
        max_retries = openai.DEFAULT_MAX_RETRIES

    model_response: Final = derouter.utils.TranscriptionResponse()
    ...
    if custom_llm_provider in AZURE_OPENAI_AUDIO_PROVIDERS and provider_config is None:
    ...
    elif custom_llm_provider == "openai" or (custom_llm_provider in derouter.openai_compatible_providers):
        api_base = (
            api_base
            or derouter.api_base
            or get_secret("OPENAI_BASE_URL")
        ...
        from derouter.llms.soniox.audio_transcription.handler import (
        ...
        from derouter.llms.bedrock.audio_transcription import BedrockAudioTranscriptionRustDispatch
...
def speech(
    ...
    **kwargs,
    ...
    if timeout is None:
        timeout = derouter.request_timeout

    if max_retries is None:
        max_retries = derouter.num_retries or openai.DEFAULT_MAX_RETRIES
    derouter_params_dict: Final = get_derouter_params(metadata=metadata, api_key=api_key or dynamic_api_key, **kwargs)
    ...
    logging_obj: Final[DeRouterLoggingObj] = cast(DeRouterLoggingObj, kwargs.get("derouter_logging_obj"))
    ...
    if custom_llm_provider == "openai" or (
        custom_llm_provider in derouter.openai_compatible_providers
        ...
        api_key = (
            api_key
            or derouter.api_key  # for deepinfra/perplexity/anyscale we check in get_llm_provider and pass in the api key from there
            or derouter.openai_key
            or get_secret("OPENAI_API_KEY")
        )
        ...
        if model.startswith("speech/"):
            ...
            response = azure_config.dispatch_text_to_speech(
                ...
                voice=voice,
                optional_params=optional_params,
                derouter_params_dict=derouter_params_dict,
                logging_obj=logging_obj,
                timeout=timeout,
                extra_headers=extra_headers,
            ...
            api_version = api_version or derouter.api_version or get_secret("AZURE_API_VERSION")
        ...
        if isinstance(query_params, dict):
            derouter_params_dict[ElevenLabsTextToSpeechConfig.ELEVENLABS_QUERY_PARAMS_KEY] = query_params

        derouter_params_dict[ElevenLabsTextToSpeechConfig.ELEVENLABS_VOICE_ID_KEY] = voice_id

        if api_base is not None:
            derouter_params_dict["api_base"] = api_base
        if api_key is not None:
            derouter_params_dict["api_key"] = api_key
        ...
        generic_optional_params: Final = GenericDeRouterParams(**kwargs)
        ...
        # Store Vertex AI specific params in derouter_params_dict
        derouter_params_dict.update(
            {
    ...
    if response is None:
        raise Exception(
            f"Unable to map the custom llm provider={custom_llm_provider} to a known provider={derouter.provider_list}."
        )
    return response
...
async def ahealth_check(
    ...
    """
    from derouter.derouter_core_utils.cached_imports import get_derouter_logging_class
    from derouter.derouter_core_utils.health_check_helpers import HealthCheckHelpers

    # Use cached import helper to lazy-load Logging class (only loads when function is called)
    Logging: Final = get_derouter_logging_class()
    ...
    derouter_logging_obj: Final = Logging(
        ...
        derouter_call_id=str(uuid.uuid4()),
        start_time=datetime.datetime.now(),
        function_id=str(uuid.uuid4()),
        log_raw_request_response=True,
    )
    model_params["derouter_logging_obj"] = derouter_logging_obj
    ...
    try:
        ...
        if model in derouter.model_cost and mode is None:
            mode = derouter.model_cost[model].get("mode")
        ...
        if mode in mode_handlers:
            ...
            raise Exception(f"Mode {mode} not supported. See modes here: https://router.delify.vn/docs/proxy/health")
    except Exception as e:
        ...
        if mode is None:
            return {
                "error": f"error:{e}. Missing `mode`. Set the `mode` for the model - https://router.delify.vn/docs/proxy/health#embedding-models  \nstacktrace: {stack_trace}",
                "exception": e,
            }
...
####### HELPER FUNCTIONS ################
## Set verbose to true -> ```derouter.set_verbose = True```
def print_verbose(print_statement):
    try:
        verbose_logger.debug(print_statement)
        if derouter.set_verbose:
...
def config_completion(**kwargs):
    if derouter.config_path is not None:
        config_args: Final = read_config_args(derouter.config_path)
        # overwrite any args passed in with config args
        return completion(**kwargs, **config_args)
    else:
        raise ValueError(
            "No config path set, please set a config path using `derouter.config_path = 'path/to/config.json'`"
        )
...
def _stream_builder_response_cost(response: ModelResponse, logging_obj: Optional["Logging"]) -> float | None:
    ...
    try:
        return derouter.completion_cost(completion_response=response, custom_llm_provider=provider_hint)
    except Exception:
...
def _stream_builder_model_map_cost(response: ModelResponse) -> float | None:
    ...
    if not model_name or not isinstance(usage, Usage):
        return None
    try:
        prompt_cost, completion_tokens_cost = derouter.cost_per_token(model=model_name, usage_object=usage)
...
def stream_chunk_builder(
    ...
    try:
        ...
        if first_chunk_with_choices is not None and isinstance(
            first_chunk_with_choices["choices"][0], derouter.utils.TextChoices
        ):  # route to the text completion logic
        ...
        # See: https://github.com/Delify-Solutions/derouter/issues/19478
        image_chunks: Final = [
            chunk
            for chunk in chunks
            if chunk.get("choices")
        ...
        # See: https://github.com/Delify-Solutions/derouter/issues/17737
        ...
        verbose_logger.exception("derouter.main.py::stream_chunk_builder() - Exception occurred - %s", e)
        raise derouter.APIError(
            status_code=500,
            message="Error building chunks for logging/streaming usage calculation",
            llm_provider="",
...
async def acount_tokens(
    ...
    from derouter.types.utils import LlmProviders, TokenCountResponse
    ...
    local_count: Final = derouter.token_counter(
        model=model,
...
def _load_default_encoding() -> tiktoken.Encoding:
    from derouter._lazy_imports import _get_default_encoding
