...
PROVIDERS_PROXYING_AN_UNKNOWN_BACKEND: Final = frozenset({"derouter_proxy"})
...
from derouter.derouter_core_utils.reasoning_effort_utils import (
...
from derouter.types.utils import Choices, ModelResponse, StreamingChoices, Usage

from .streaming_iterator import AnthropicStreamWrapper

if TYPE_CHECKING:
    from derouter.derouter_core_utils.derouter_logging import Logging as DeRouterLoggingObject
    from derouter.types.llms.anthropic import ContentBlockContentBlockDict
...
class AnthropicAdapter:
    ...
    def translate_completion_input_params_with_tool_mapping(
        ...
        (
            translated_body,
            tool_name_mapping,
        ) = DeRouterAnthropicMessagesAdapter().translate_anthropic_to_openai(
    ...
    def translate_completion_output_params(
        ...
        """
        return DeRouterAnthropicMessagesAdapter().translate_openai_response_to_anthropic(
            response=response,
    ...
    def translate_completion_output_params_streaming(
        ...
        polyfill_result: PolyfillResult | None = None,
        is_async: bool = True,
        derouter_logging_obj: "DeRouterLoggingObject | None" = None,
        ...
        Args:
            ...
            is_async: When ``True`` (default, for back-compat with existing
                ...
                (e.g. ``derouter.anthropic.messages.create(stream=True)`` via
...
class DeRouterAnthropicMessagesAdapter:
    def __init__(self):
        pass
    ...
    def translate_thinking_for_model(
        ...
        """
        if DeRouterAnthropicMessagesAdapter.is_anthropic_claude_model(
            model
        ) or DeRouterAnthropicMessagesAdapter.is_bedrock_arn_model(model):
            return {"thinking": thinking}
        else:
            reasoning_effort: Final = DeRouterAnthropicMessagesAdapter.translate_anthropic_thinking_to_reasoning_effort(
                thinking
            )
            if reasoning_effort:
                return {
                    "reasoning_effort": DeRouterAnthropicMessagesAdapter._apply_reasoning_summary_wrapping(
    ...
    def translate_anthropic_tools_to_openai(
        ...
        for idx, tool in enumerate(tools):
            ...
            if raw_name is None or (isinstance(raw_name, str) and not str(raw_name).strip()):
                original_name = f"derouter_unnamed_tool_{idx}"
            else:
    ...
    def _add_additional_properties_false(schema: dict) -> None:
        ...
        if schema.get("type") == "object" and "properties" in schema:
            ...
            for prop in schema["properties"].values():
                DeRouterAnthropicMessagesAdapter._add_additional_properties_false(prop)

        # Handle array items
        if "items" in schema:
            DeRouterAnthropicMessagesAdapter._add_additional_properties_false(schema["items"])

        # Handle anyOf/oneOf/allOf
        for key in ("anyOf", "oneOf", "allOf"):
            if key in schema:
                for sub_schema in schema[key]:
                    DeRouterAnthropicMessagesAdapter._add_additional_properties_false(sub_schema)

        # Handle $defs / definitions
        for key in ("$defs", "definitions"):
            if key in schema:
                for def_schema in schema[key].values():
                    DeRouterAnthropicMessagesAdapter._add_additional_properties_false(def_schema)
    ...
    def _supports_prompt_cache_key(model: str | None, custom_llm_provider: str | None) -> bool:
        ...
        if custom_llm_provider in PROVIDERS_PROXYING_AN_UNKNOWN_BACKEND:
            return False
        supported_params: Final = derouter.get_supported_openai_params(
    ...
    def _translate_metadata_to_openai(
        ...
        *,
        ...
        if "derouter_metadata" in anthropic_message_request:
            # metadata will be passed to derouter.acompletion(), it's a derouter_param
            new_kwargs["metadata"] = anthropic_message_request.pop("derouter_metadata")
    ...
    def translate_anthropic_to_openai(
        ...
        *,
        ...
        ## CONVERT METADATA (user_id + derouter metadata)
    ...
    def _translate_streaming_openai_chunk_to_anthropic_content_block(
        ...
        from derouter.types.llms.anthropic import TextBlock
    ...
    def translate_streaming_openai_response_to_anthropic(
        ...
        if response.choices[0].finish_reason is not None:
            ...
            if getattr(response, "usage", None) is not None:
                derouter_usage_chunk: Usage | None = response.usage
            elif hasattr(response, "_hidden_params") and "usage" in response._hidden_params:
                derouter_usage_chunk = response._hidden_params["usage"]
            else:
                derouter_usage_chunk = None
            if derouter_usage_chunk is not None:
                usage_delta = self._translate_openai_usage_to_anthropic_usage_delta(derouter_usage_chunk)
            else:
