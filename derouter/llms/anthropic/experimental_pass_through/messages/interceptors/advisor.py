...
from derouter.derouter_core_utils.url_utils import validate_url
...
from derouter.types.llms.anthropic import ANTHROPIC_ADVISOR_TOOL_TYPE
...
class AdvisorOrchestrationHandler(MessagesInterceptor):
    ...
    async def handle(
        ...
        **kwargs,
        ...
        parent_request_id: Final[str] = str(kwargs.pop("derouter_call_id", None) or uuid.uuid4())
...
def _resolve_advisor_credentials(advisor_tool: dict) -> tuple[str | None, str | None]:
    ...
    ``derouter.ssl_verify`` disabled; otherwise it returns the URL unchanged
    ...
    if getattr(derouter, "ssl_verify", True) is False:
        raise ValueError(
            "advisor tool definition sets 'api_base' but the proxy has TLS verification "
            "disabled (derouter.ssl_verify=False), so a caller-supplied api_base can't be "
            "safely validated against DNS rebinding."
        )
    if getattr(derouter, "user_url_validation", True):
...
def _resolve_advisor_router(advisor_model: str) -> "Router | None":
    ...
    doesn't know about, so those keep resolving through ``derouter.anthropic_messages()``
    provider inference.
    """
    try:
