...
from derouter.types.llms.anthropic_messages.anthropic_request import AnthropicMetadata
...
from ..adapters.handler import DeRouterMessagesToCompletionTransformationHandler
from ..responses_adapters.handler import DeRouterMessagesToResponsesAPIHandler
...
def _responses_mode_is_lost_by_prefix_strip(
    ...
    ``derouter.completion`` re-derives the Responses bridge from the stripped id alone, so a
...
def _should_route_to_responses_api(
    ...
    Set ``derouter.use_chat_completions_url_for_anthropic_messages = True`` to
    opt out and route OpenAI/Azure requests through chat/completions instead.
    """
    if derouter.use_chat_completions_url_for_anthropic_messages:
        return False
...
async def _execute_pre_request_hooks(
    ...
    **kwargs,
    ...
    if not derouter.callbacks:
        return request_kwargs

    from derouter.integrations.custom_logger import CustomLogger as _CustomLogger
...
async def anthropic_messages(
    ...
    **kwargs,
    ...
    # web_search -> derouter_web_search — can keep a forced tool_choice in sync.
    ...
    # The derouter_params dict may have been overwritten by **kwargs in
    # _execute_pre_request_hooks, so fall back to get_llm_provider() if needed.
    if not custom_llm_provider:
        custom_llm_provider = request_kwargs.get("derouter_params", {}).get("custom_llm_provider")
        if not custom_llm_provider:
            try:
                _, custom_llm_provider, _, _ = derouter.get_llm_provider(model=model)
            except Exception:
                pass
    # Remove derouter_params from kwargs (only needed for hooks)
    request_kwargs.pop("derouter_params", None)
...
def validate_anthropic_api_metadata(metadata: dict | None = None) -> dict | None:
    ...
    If there are any derouter specific metadata fields, use `derouter_metadata` key to pass them.
    """
    if metadata is None:
        return None
...
def anthropic_messages_handler(
    ...
    **kwargs,
    ...
    # (derouter.messages.create -> anthropic_messages_handler) gets the same
    ...
    # ``_derouter_messages_presanitized`` to skip this redundant second
    # full-messages scan. Pop it so it never leaks into provider params.
    if not kwargs.pop("_derouter_messages_presanitized", False):
        messages = strip_empty_content_blocks_from_anthropic_messages(messages)
        messages = sanitize_tool_use_ids_in_anthropic_messages(messages)
        messages = flatten_unencrypted_web_search_results_in_anthropic_messages(messages)
    ...
    derouter_params: Final = GenericDeRouterParams(
        **kwargs,
        api_key=api_key,
        api_base=api_base,
        custom_llm_provider=custom_llm_provider,
    ...
    if derouter_logging_obj is not None:
        derouter_logging_obj.model_call_details["agentic_loop_params"] = {
            "model": original_model,
            "custom_llm_provider": custom_llm_provider,
        }
        derouter_logging_obj.model_call_details.setdefault("derouter_params", {})[CallTypes.aanthropic_messages.value] = (
            is_async
        )
        ...
        if kwargs.get("_websearch_interception_converted_stream", False):
            derouter_logging_obj.model_call_details["websearch_interception_converted_stream"] = True
    ...
    if derouter_params.mock_response and isinstance(derouter_params.mock_response, str):
        return mock_response(
            ...
            messages=messages,
            max_tokens=max_tokens,
            mock_response=derouter_params.mock_response,
        )

    # Expand derouter_proxy MCP references through the MCP gateway before dispatch, so every
    ...
    if not skip_mcp_handler and tools:
        from derouter.llms.anthropic.experimental_pass_through.messages.mcp_handler import (
        ...
        if DeRouter_Proxy_MCP_Handler._should_use_derouter_mcp_gateway(tools=tools):
    ...
    if anthropic_messages_provider_config is None:
        ...
        if _should_route_to_responses_api(custom_llm_provider, original_model, model):
            return DeRouterMessagesToResponsesAPIHandler.anthropic_messages_handler(
                max_tokens=max_tokens,
                messages=messages,
                model=original_model,
                metadata=metadata,
        ...
        return DeRouterMessagesToCompletionTransformationHandler.anthropic_messages_handler(
            max_tokens=max_tokens,
            messages=messages,
            model=original_model,
            metadata=metadata,
