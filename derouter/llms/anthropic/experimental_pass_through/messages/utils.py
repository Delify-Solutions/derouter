...
from derouter.types.llms.openai import ChatCompletionSystemMessage
...
def openai_chat_refusal_text(message_or_delta: object) -> str | None:
    ...
    read from ``refusal`` or from the ``provider_specific_fields`` DeRouter parks it
    ...
    def get_requested_anthropic_messages_optional_param(
        ...
        *,
        ...
        Args:
            ...
            drop_params: Per-request drop_params flag (also respects derouter.drop_params)
...
def mock_response(
    ...
    **kwargs,
    ...
    if mock_response == "derouter.InternalServerError":
        raise InternalServerError(
            message="this is a mock internal server error",
            llm_provider="anthropic",
            model=model,
        )
    elif mock_response == "derouter.ContextWindowExceededError":
        raise ContextWindowExceededError(
            message="this is a mock context window exceeded error",
            llm_provider="anthropic",
            model=model,
        )
    elif mock_response == "derouter.RateLimitError":
