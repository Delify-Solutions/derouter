...
from derouter.llms.anthropic.common_utils import ANTHROPIC_ERROR_STATUS_CODE_MAP
from derouter.proxy.pass_through_endpoints.success_handler import (
    PassThroughEndpointLogging,
)
from derouter.types.llms.anthropic_messages.anthropic_response import AnthropicMessagesResponse
...
class BaseAnthropicMessagesStreamingIterator:
    ...
    def __init__(
        ...
        self.derouter_logging_obj = derouter_logging_obj
    ...
    async def _handle_streaming_logging(self, collected_chunks: list[bytes], *, stream_teardown: bool = False):
        ...
        if self.completion_start_time is not None:
            self.derouter_logging_obj.completion_start_time = self.completion_start_time
            self.derouter_logging_obj.model_call_details["completion_start_time"] = self.completion_start_time
        logging_coroutine: Final = PassThroughStreamingHandler._route_streaming_logging_to_handler(
            derouter_logging_obj=self.derouter_logging_obj,
        ...
        deferred_dispatch_armed: Final = (
            getattr(self.derouter_logging_obj, "_on_deferred_stream_complete", None) is not None
        )
        if deferred_dispatch_armed and not stream_teardown:
            self.derouter_logging_obj._deferred_stream_complete_args = (logging_coroutine,)
            return
    ...
    def _dispatch_pending_deferred_logging(self) -> None:
        ...
        """
        deferred_cb: Final = getattr(self.derouter_logging_obj, "_on_deferred_stream_complete", None)
        deferred_args: Final = getattr(self.derouter_logging_obj, "_deferred_stream_complete_args", None)
        if deferred_cb is None or deferred_args is None:
            return
        self.derouter_logging_obj._on_deferred_stream_complete = None
        self.derouter_logging_obj._deferred_stream_complete_args = None
    ...
    async def _handle_pump_upstream_error(
        ...
        """
        from derouter.proxy.pass_through_endpoints.streaming_handler import PassThroughStreamingHandler
    ...
    async def _fire_detached_failure_hook(self, exc: Exception) -> None:
        ...
        on_detached_failure: Final = getattr(self.derouter_logging_obj, "_on_detached_stream_failure", None)
