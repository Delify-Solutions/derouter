...
if TYPE_CHECKING:
    ...
    from derouter.proxy.hooks.parallel_request_limiter_v3 import RateLimitDescriptor, RateLimitResponse
...
class _SummaryCallKwargs(TypedDict):
    ...
    derouter_metadata: ReadOnly[Mapping[str, object]]
...
async def _check_summary_model_access(
    ...
    ``derouter.proxy.auth.common_checks`` runs for the client-requested model:
    ...
    if user_id is not None and team_id is not None and prisma_client is not None:
        ...
        member_allowed_models: Final = (
            team_membership.derouter_budget_table.allowed_models
...
async def _check_summary_model_budget(
    ...
    if isinstance(model_max_budget, dict) and model_max_budget and token is not None:
        ...
        except derouter.BudgetExceededError:
            return False
        except Exception as e:
            verbose_logger.warning(
...
async def _check_summary_model_rate_limit(
    ...
    post-call success hook (via the propagated ``derouter_metadata``).
...
def _propagate_metadata(
    parent_derouter_metadata: Mapping[str, object] | None,
) -> dict[str, object]:
    ...
    """
    if not parent_derouter_metadata:
        return {}
    propagated: Final[dict[str, object]] = {}
    for key in _PROPAGATED_METADATA_KEYS:
        if key in parent_derouter_metadata:
            propagated[key] = parent_derouter_metadata[key]
    return propagated
...
def _count_effective_tokens(
    ...
    if compaction_block is not None:
        ...
        if content:
            total += derouter.token_counter(model=model, text=content)
    system_text: Final = _system_to_text(system)
    if system_text:
        total += derouter.token_counter(model=model, text=system_text)
    return total
...
def _build_summary_messages(
    ...
    try:
        openai_messages = DeRouterAnthropicMessagesAdapter().translate_anthropic_messages_to_openai(
            messages=cast(
                "list[AllAnthropicPassThroughMessageValues]",
                stripped,
            )
...
async def _call_summary_model(
    *,
    ...
    proxy's ``model_list``; falls back to ``derouter.acompletion`` if no router
    ...
    # must travel as ``derouter_metadata`` — that is the parameter the proxy's
    ...
    # than from ``derouter_metadata``, so without it the summary tokens would not
    ...
    return await derouter.acompletion(messages=[*summary_messages], **call_kwargs)
...
async def apply_compact_20260112(
    *,
    ...
    propagated_metadata: Final = _propagate_metadata(derouter_metadata)
