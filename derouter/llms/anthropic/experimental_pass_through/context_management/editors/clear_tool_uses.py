...
from derouter.types.llms.anthropic import AppliedEdit
...
def _trigger_met(
    ...
    current_tokens: Final = derouter.token_counter(
        model=model,
        messages=messages,
...
def apply_clear_tool_uses_20250919(
    *,
    ...
    if tokens_before is None:
        tokens_before = derouter.token_counter(model=model, messages=messages, tools=cast(Any, tools))
    tokens_after: Final = derouter.token_counter(model=model, messages=edited, tools=cast(Any, tools))
