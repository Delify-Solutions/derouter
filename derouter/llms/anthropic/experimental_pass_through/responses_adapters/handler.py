...
from ..utils import derouter_logging_obj_from_kwargs, local_model_name
from .streaming_iterator import AnthropicResponsesStreamWrapper
from .transformation import DeRouterAnthropicToResponsesAPIAdapter

AnthropicRequestMessages: TypeAlias = list[AllAnthropicMessageValues] | list[dict[str, object]]

_ADAPTER: Final = DeRouterAnthropicToResponsesAPIAdapter()


def _forwarded_kwargs(extra_kwargs: Mapping[str, object] | None) -> Mapping[str, object]:
    """The derouter-specific kwargs forwarded verbatim onto the Responses API request."""
...
def _build_responses_kwargs(
    *,
    ...
    """
    Build the kwargs dict to pass directly to derouter.responses() / derouter.aresponses().
    """
    ...
    # Forward derouter-specific kwargs (api_key, api_base, logging obj, etc.)
    excluded: Final = {"anthropic_messages"}
    for key, value in forwarded_kwargs.items():
        if key == "derouter_logging_obj" and value is not None:
            ...
            if isinstance(value, DeRouterLoggingObject):
...
class DeRouterMessagesToResponsesAPIHandler:
    """
    Handles Anthropic /v1/messages requests for OpenAI / Azure models by
    calling derouter.responses() / derouter.aresponses() directly and translating
    ...
    async def async_anthropic_messages_handler(
        ...
        **kwargs: object,
        ...
        result: Final = await derouter.aresponses(**responses_kwargs)
        ...
        if stream:
            wrapper: Final = AnthropicResponsesStreamWrapper(
                ...
                derouter_logging_obj=derouter_logging_obj_from_kwargs(responses_kwargs),
            )
    ...
    def anthropic_messages_handler(
        ...
        **kwargs: object,
        ...
        if _is_async:
            return DeRouterMessagesToResponsesAPIHandler.async_anthropic_messages_handler(
                max_tokens=max_tokens,
                messages=messages,
                model=model,
                context_management=context_management,
        ...
        result: Final = derouter.responses(**responses_kwargs)
