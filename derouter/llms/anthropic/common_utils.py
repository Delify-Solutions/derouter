...
class AnthropicModelInfo(BaseLLMModelInfo):
    ...
    def _apply_sampling_param(
        ...
        elif not (derouter.drop_params or drop_params):
        ...
        if not (derouter.drop_params or drop_params):
            raise derouter.utils.UnsupportedParamsError(
                message=(
                    ...
                    "`derouter.drop_params = True` to downgrade to 'auto' automatically."
                ),
                status_code=400,
            )
        derouter.verbose_logger.warning(DROP_FORCED_TOOL_CHOICE_WARNING, model)
        return True
    ...
    def _get_model_capability(model: str, key: str) -> bool | None:
        ...
        from derouter.utils import _get_bundled_model_cost_map

        try:
            candidates: Final = AnthropicModelInfo._model_map_lookup_candidates(model)
            for model_cost in (derouter.model_cost, _get_bundled_model_cost_map()):
    ...
    def _get_exact_model_capability(model: str, key: str) -> bool | None:
        ...
        """
        value: Final = derouter.model_cost.get(model, {}).get(key)
    ...
    def _get_provider_resolved_capability(model: str, key: str, custom_llm_provider: str) -> bool | None:
        ...
        try:
            resolved_model, resolved_provider, _, _ = derouter.get_llm_provider(
    ...
    def get_anthropic_beta_list(
        ...
        *,
        ...
        """
        from derouter.types.llms.anthropic import ANTHROPIC_EFFORT_BETA_HEADER
    ...
    def get_anthropic_headers(
        ...
        if tool_search_used or programmatic_tool_calling_used or input_examples_used:
            from derouter.types.llms.anthropic import ANTHROPIC_TOOL_SEARCH_BETA_HEADER
    ...
    def validate_environment(
        ...
        if api_base is None and isinstance(derouter_params, dict):
            api_base = derouter_params.get("api_base")
        use_bearer_for_custom_base: Final[bool] = bool(
            isinstance(derouter_params, dict) and derouter_params.get("use_bearer_for_custom_base", False)
        )
    ...
    def get_models(self, api_key: str | None = None, api_base: str | None = None) -> list[str]:
        ...
        derouter_model_names: Final = ["anthropic/" + model["id"] for model in models]
        return derouter_model_names
    ...
    def get_token_counter(self) -> BaseTokenCounter | None:
        ...
        """
        from derouter.llms.anthropic.count_tokens.token_counter import (
...
def flatten_unencrypted_web_search_results_in_anthropic_messages(  # mutable-ok: as sibling sanitizers
    ...
    mint, so blocks synthesized by DeRouter (websearch interception against a search
