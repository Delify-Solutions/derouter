...
from derouter.derouter_core_utils.prompt_templates.image_handling import (
    RemoteMedia,
...
from derouter.types.utils import Message as LitellmMessage
...
if TYPE_CHECKING:
    ...
    LoggingClass = DeRouterLoggingObj
else:
    LoggingClass = Any
...
# Single, internal-only key on ``derouter_params`` used to thread the per-
# request reverse map (sanitized -> original) from request build to response
# parsing. ``derouter_params`` is never serialized to a provider; ``optional_
...
class AnthropicConfig(AnthropicModelInfo, BaseConfig):
    ...
    def _maybe_drop_speed_param(
        ...
        if AnthropicConfig._model_supports_speed_param(model, custom_llm_provider):
            return
        if not (derouter.drop_params or drop_params):
            speed_value: Final = optional_params.get("speed")
            raise derouter.utils.UnsupportedParamsError(
                message=(
        ...
        derouter.verbose_logger.warning(
    ...
    def _raise_invalid_reasoning_effort(model: str, value: object, llm_provider: str) -> NoReturn:
        ...
        Raises:
            derouter.exceptions.BadRequestError: Always.
        """
    ...
    def filter_anthropic_output_schema(schema: dict[str, Any]) -> dict[str, Any]:
        ...
        Related issues:
        - https://github.com/Delify-Solutions/derouter/issues/19444
        """
    ...
    def get_json_schema_from_pydantic_object(self, response_format: Any | dict | None) -> dict | None:
        ...
        )  # Relevant issue: https://github.com/Delify-Solutions/derouter/issues/7755
    ...
    def _map_tool_helper(
        ...
        if tool["type"] == "function" or tool["type"] == "custom":
            ...
            from derouter.types.llms.anthropic import AnthropicToolSearchToolRegex
            ...
            from derouter.types.llms.anthropic import AnthropicToolSearchToolBM25
            ...
            from derouter.types.llms.anthropic import AnthropicAdvisorTool
    ...
    def _map_openai_mcp_server_tool(self, tool: OpenAIMcpServerTool) -> AnthropicMcpServerTool:
        from derouter.types.llms.anthropic import AnthropicMcpServerToolConfiguration
    ...
    def _map_stop_sequences(self, stop: str | list[str] | None) -> list[str] | None:
        ...
        if isinstance(stop, str):
            if (
                stop.isspace() and derouter.drop_params is True
            ...
            for v in stop:
                if (
                    v.isspace() and derouter.drop_params is True
                ):  # anthropic doesn't allow whitespace characters as stop-sequences
                    continue
                new_v.append(v)
            if len(new_v) > 0:
    ...
    def map_openai_params(
        ...
        #   2. the maps land on ``derouter_params`` (internal), never on
    ...
    async def async_transform_request(
        ...
        derouter_params: dict[str, object],  # mutable-ok: BaseConfig signature
    ...
    def transform_request(
        ...
        """
        ## VALIDATE REQUEST
        """Anthropic requires ``tools`` when messages include tool blocks; DeRouter injects a dummy tool if omitted (no ``modify_params`` needed)."""
        ...
        AnthropicConfig._maybe_drop_speed_param(
            model=model,
            optional_params=optional_params,
            drop_params=derouter.drop_params or derouter_params.get("drop_params") is True,
            custom_llm_provider=self.custom_llm_provider,
        )
        ...
        #     ``derouter_params`` (internal-only), never on ``optional_params``
        ...
        if _name_reverse_map and isinstance(derouter_params, dict):
            derouter_params[ANTHROPIC_TOOL_NAME_REVERSE_MAP_KEY] = _name_reverse_map
        ...
        ## Load Config
        config: Final = derouter.AnthropicConfig.get_config(model=model)
        ...
        _derouter_metadata: Final = derouter_params.get("metadata", None)
        if (
            _derouter_metadata
            and isinstance(_derouter_metadata, dict)
            and "user_id" in _derouter_metadata
            and _derouter_metadata["user_id"] is not None
            and _valid_user_id(_derouter_metadata["user_id"])
        ):
            optional_params["metadata"] = {"user_id": _derouter_metadata["user_id"]}
        ...
        # Remove internal DeRouter parameters that should not be sent to Anthropic API
        ...
        if top_k is not None:
            AnthropicConfig._apply_sampling_param(
                ...
                param="top_k",
                value=top_k,
                drop_params=derouter_params.get("drop_params") is True,
                output_key="top_k",
            )
    ...
    def transform_parsed_response(
        ...
        if (
            ...
            and not derouter.disable_add_prefix_to_prompt
        ):
    ...
    def transform_response(
        ...
        if isinstance(derouter_params, dict):
            _candidate: Final = derouter_params.get(ANTHROPIC_TOOL_NAME_REVERSE_MAP_KEY)
        ...
        try:
            if json_mode_content_str is not None:
                ...
                if isinstance(args, dict) and (values := args.get("values")) is not None:
                    _message = derouter.Message(content=json.dumps(values))
                    return _message
                else:
                    # a lot of the times the `values` key is not present in the tool response
                    # relevant issue: https://github.com/Delify-Solutions/derouter/issues/6741
                    _message = derouter.Message(content=json.dumps(args))
                    return _message
        except json.JSONDecodeError:
            # json decode error does occur, return the original tool response str
            return derouter.Message(content=json_mode_content_str)
        return None
