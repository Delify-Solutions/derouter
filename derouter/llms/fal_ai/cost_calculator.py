...
def _keyed_cost_per_image(model: str, optional_params: Mapping[str, object] | None) -> float | None:
    ...
    keyed_entry: Final = derouter.model_cost.get(f"fal_ai/{quality}/{size}/{model}")
...
def cost_calculator(
    ...
    model = model.removeprefix(f"{derouter.LlmProviders.FAL_AI.value}/")
    ...
    _model_info: Final = derouter.get_model_info(
        model=model,
        custom_llm_provider=derouter.LlmProviders.FAL_AI.value,
    )
