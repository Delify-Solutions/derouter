    ...
    try:
        ...
        for root, dirs, files in os.walk(llms_dir):
            ...
            if os.path.basename(root) == "guardrail_translation" and "__init__.py" in files:
                # Build the module path relative to derouter
                rel_path = os.path.relpath(root, os.path.dirname(llms_dir))
                module_path = "derouter." + rel_path.replace(os.sep, ".")
        ...
        try:
            from derouter.proxy._experimental.mcp_server.guardrail_translation import (
