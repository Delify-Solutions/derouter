"""
Automatic Code Execution Handler for DeRouter Skills

When `derouter_code_execution` tool is present, this handler automatically:
...
class DeRouterInternalTools(str, Enum):
    """
    Enum for internal DeRouter tools that are injected into requests.

    These tools are handled automatically by DeRouter hooks and are not
    passed to the underlying LLM provider directly.
    """

    CODE_EXECUTION = "derouter_code_execution"


def get_derouter_code_execution_tool() -> _OpenAIToolSpec:
    """
    Returns the derouter_code_execution tool definition in OpenAI format.
    ...
    return {
        "type": "function",
        "function": {
            "name": DeRouterInternalTools.CODE_EXECUTION.value,
...
def get_derouter_code_execution_tool_anthropic() -> _AnthropicToolSpec:
    """
    Returns the derouter_code_execution tool definition in Anthropic/messages API format.
...
DEROUTER_CODE_EXECUTION_TOOL: Final = get_derouter_code_execution_tool()


class CodeExecutionHandler:
    """
    Handles automatic code execution for DeRouter skills.
    ...
    async def execute_with_code_execution(
        ...
        **kwargs,
        ...
        2. If model calls derouter_code_execution, executes the code
        ...
        Args:
            ...
            tools: Tools including derouter_code_execution
            skill_files: Dict of skill files for execution
            skill_id: Optional skill ID for tracking
            **kwargs: Additional args for derouter.acompletion
...
def has_code_execution_tool(tools: list[_OpenAIToolSpec] | None) -> bool:
    """Check if derouter_code_execution tool is in the tools list."""
    if not tools:
        return False
    for tool in tools:
        func = tool.get("function", {})
        if func.get("name") == DeRouterInternalTools.CODE_EXECUTION.value:
            return True
    return False


def add_code_execution_tool(tools: list[_OpenAIToolSpec] | None) -> list[_OpenAIToolSpec]:
    """Add derouter_code_execution tool if not already present."""
    tools = tools or []
    if not has_code_execution_tool(tools):
        tools.append(DEROUTER_CODE_EXECUTION_TOOL)
    return tools
