"""Semantic ranking over the DeRouter-hosted skill registry, shared by GET /v1/skills?query= and the skill_search MCP tool."""
...
from derouter.llms.derouter_proxy.skills.constants import MAX_SKILLS_PER_SEARCH
...
def skill_search_text(skill: DeRouter_SkillsTable) -> str:
...
class SkillSearchIndex:
    ...
    async def search(
        self,
        query: str,
        skills: Sequence[DeRouter_SkillsTable],
        top_k: int,
        embed: Embedder,
        embedding_model: str,
...
async def search_skills(
    ...
    if embedding_model is None:
        return SkillSearchNotConfigured(
            reason="skill search needs derouter_settings.skill_search_embedding_model set to an embedding model from model_list"
        )
    if router is None:
...
async def search_hosted_skills(
    ...
    """GET /v1/skills?query= for the skills DeRouter hosts itself: only ``derouter_proxy`` has a registry to rank."""
    if custom_llm_provider != LlmProviders.DEROUTER_PROXY.value:
        return SkillSearchUnsupportedProvider(reason="query is only supported for custom_llm_provider=derouter_proxy")
    skills: Final = await DeRouterSkillsHandler.list_skills_for_search(user_api_key_dict=user_api_key_dict)
