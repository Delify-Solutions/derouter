...
implement the DeRouter BaseConfig interface.  Heavy-lifting lives in:

  - :mod:`derouter.llms.oci.chat.cohere`  — Cohere message/tool/response logic
  - :mod:`derouter.llms.oci.chat.generic` — Generic message/tool/response logic
  - :mod:`derouter.llms.oci.common_utils` — auth, signing, schema utilities
"""
...
import derouter
from derouter.constants import DEFAULT_OCI_CHAT_MAX_TOKENS
...
from derouter.llms.oci.chat.cohere import (
...
from derouter.llms.oci.chat.generic import (
...
from derouter.llms.oci.common_utils import (
    OCI_API_VERSION,
    OCIError,
...
from derouter.types.llms.oci import (
    CohereChatRequest,
...
class OCIChatConfig(BaseConfig):
    """DeRouter BaseConfig implementation for OCI Generative AI chat."""
    ...
    def __init__(self) -> None:
        ...
        # accepts ``topK`` and DeRouter commonly forwards ``top_k`` as a
    ...
    def map_openai_params(
        ...
        for key, value in {**non_default_params, **optional_params}.items():
            ...
            if alias is False:
                # max_retries is a derouter-level control param (derouter applies
                # retries itself); it is never a generation param OCI accepts, so
                # drop it silently. The derouter proxy injects it on every request,
            ...
            # OCI-mapped ``responseFormat`` so downstream derouter framework code
    ...
    def get_complete_url(
        ...
        base: Final = get_oci_base_url(optional_params, api_base or derouter.api_base)
    ...
    def _get_optional_params(self, vendor: OCIVendors, optional_params: dict, model: str = "") -> dict:
        ...
        # sane default when one is absent, mirroring derouter's Anthropic config.
