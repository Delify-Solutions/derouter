...
from derouter.derouter_core_utils.logging_utils import track_llm_api_timing
from derouter.derouter_core_utils.request_timeout_resolver import (
    get_configured_request_timeout,
)
from derouter.types.llms.custom_http import *

if TYPE_CHECKING:
    from derouter import LlmProviders
...
def get_default_headers() -> dict:
    ...
    - Default: `User-Agent: derouter/{version}`
    - Override: set `DEROUTER_USER_AGENT` to fully override the header value.
    """
    user_agent: Final = os.environ.get("DEROUTER_USER_AGENT")
    if user_agent is not None:
        return {"User-Agent": user_agent}

    return {"User-Agent": f"derouter/{version}"}
...
def _default_cached_client_timeout() -> httpx.Timeout:
    """Timeout for cached default httpx clients; honors an explicit derouter.request_timeout."""
...
def blocked_cookie_jar() -> CookieJar:
    ...
    DeRouter's outbound clients are pooled and shared by every caller, so a cookie one
...
_STREAMING_ERROR_BODY_READ_EXECUTOR: Final = concurrent.futures.ThreadPoolExecutor(
    max_workers=50,
    thread_name_prefix="derouter-streaming-error-body-read",
)
...
def _create_ssl_context(
    ...
    # Set SSL_ECDH_CURVE env var or derouter.ssl_ecdh_curve to 'X25519' to disable PQC
...
def get_ssl_verify(
    ...
    if ssl_verify is None:
        ssl_verify = os.getenv("SSL_VERIFY", derouter.ssl_verify)
...
def get_ssl_configuration(
    ...
    Args:
        ssl_verify: SSL verification setting. Can be:
            - None: Use default from environment/derouter settings
    ...
    ssl_security_level: Final = os.getenv("SSL_SECURITY_LEVEL", derouter.ssl_security_level)
    ssl_ecdh_curve: Final = os.getenv("SSL_ECDH_CURVE", derouter.ssl_ecdh_curve)
...
class AsyncHTTPHandler:
    ...
    def create_client(
        ...
        # /path/to/client.pem
        cert: Final = os.getenv("SSL_CERTIFICATE", derouter.ssl_certificate)
        ...
        # Get default headers (User-Agent, overridable via DEROUTER_USER_AGENT)
    ...
    async def post(
        ...
        timeout: float | httpx.Timeout | None = None,
        stream: bool = False,
        logging_obj: DeRouterLoggingObject | None = None,
        ...
        try:
            ...
            raise derouter.Timeout(
                ...
                llm_provider="derouter-httpx-handler",
                headers=headers,
            )
    ...
    # Mirrors DeRouterAiohttpTransport._background_close_tasks.
    ...
    def _aiohttp_session_bound_elsewhere(self, loop: asyncio.AbstractEventLoop) -> bool:
        ...
        from derouter.llms.custom_httpx.aiohttp_transport import (
            DeRouterAiohttpTransport,
        )

        transport: Final = getattr(self._client, "_transport", None)
        if not isinstance(transport, DeRouterAiohttpTransport):
            return False
    ...
    def _create_async_transport(
        ...
        shared_session: Optional["ClientSession"] = None,
    ) -> DeRouterAiohttpTransport | AsyncHTTPTransport | None:
        """
        - Creates a transport for httpx.AsyncClient
            - if derouter.force_ipv4 is True, it will return AsyncHTTPTransport with local_address="0.0.0.0"
            - [Default] It will return AiohttpTransport
            - Users can opt out of using AiohttpTransport by setting derouter.use_aiohttp_transport to False
    ...
    def _should_use_aiohttp_transport() -> bool:
        """
        AiohttpTransport is the default transport for derouter.
        ...
        ########################################################
        if (
            derouter.disable_aiohttp_transport is True
    ...
    def _get_ssl_connector_kwargs(
        ...
        connector_kwargs: Final[_TCPConnectorKwargs] = {
            "local_addr": ("0.0.0.0", 0) if derouter.force_ipv4 else None,
        }
    ...
    def _create_aiohttp_transport(
        ...
        ########################################################
        trust_env: bool = derouter.aiohttp_trust_env
        ...
        if shared_session is not None and not shared_session.closed:
            ...
            return DeRouterAiohttpTransport(
    ...
    def _create_httpx_proxy_mounts(
        transport: DeRouterAiohttpTransport | AsyncHTTPTransport | None,
        verify: VerifyTypes,
        cert: CertTypes | None,
...
class HTTPHandler:
    ...
    def _create_sync_transport(self) -> HTTPTransport | None:
        """
        Create an HTTP transport with IPv4 only if derouter.force_ipv4 is True.
        ...
        if derouter.force_ipv4:
            ...
            return getattr(derouter, "sync_transport", None)
...
def get_async_httpx_client(
    ...
    # derouter globals being fully populated during import time.
    ...
    if cache is None:
        ...
        setattr(derouter, "in_memory_llm_clients_cache", cache)
