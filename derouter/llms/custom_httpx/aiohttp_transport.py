...
class DeRouterAiohttpTransport(AiohttpTransport):
    """
    DeRouter wrapper around AiohttpTransport to handle %-encodings in URLs
    ...
    async def _get_proxy_settings(self, request: httpx.Request):
        proxy = None
        if not (derouter.disable_aiohttp_trust_env or str_to_bool(os.getenv("DISABLE_AIOHTTP_TRUST_ENV", "False"))):
            try:
