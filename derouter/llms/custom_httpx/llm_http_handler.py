...
from derouter._logging import _redact_string, verbose_logger
from derouter.anthropic_beta_headers_manager import update_headers_with_filtered_beta
from derouter.constants import REALTIME_WEBSOCKET_MAX_MESSAGE_SIZE_BYTES
...
from derouter.derouter_core_utils.audio_utils.subtitle_utils import (
    SUBTITLE_RESPONSE_FORMATS,
    synthesize_subtitle_document,
)
from derouter.derouter_core_utils.get_derouter_params import AWS_CREDENTIAL_KWARGS_KEYS
from derouter.derouter_core_utils.llm_request_utils import serialize_multipart_form_fields
from derouter.derouter_core_utils.realtime_errors import realtime_error_event, websocket_close_reason
from derouter.derouter_core_utils.realtime_streaming import RealTimeStreaming
...
from derouter.llms.base_llm.files.transformation import (
    BaseFilesConfig,
...
from derouter.llms.base_llm.ocr.transformation import BaseOCRConfig, OCRResponse
from derouter.llms.base_llm.realtime.http_transformation import BaseRealtimeHTTPConfig
...
from derouter.llms.base_llm.search.transformation import BaseSearchConfig, SearchResponse
from derouter.llms.base_llm.skills.transformation import BaseSkillsAPIConfig
from derouter.llms.base_llm.text_to_speech.transformation import BaseTextToSpeechConfig
...
from derouter.llms.custom_httpx.container_handler import raise_for_error_status
...
from derouter.types.containers.main import (
    ContainerFileListResponse,
    ContainerListResponse,
    ContainerObject,
    DeleteContainerResult,
)
from derouter.types.files import StreamingMediaUploadConfig, TwoStepFileUploadConfig
from derouter.types.integrations.custom_logger import (
...
from derouter.types.llms.anthropic_skills import (
    DeleteSkillResponse,
    ListSkillsResponse,
    Skill,
)
...
from derouter.types.vector_store_files import (
...
def _rust_responses_websocket_enabled(
    ...
    from derouter.rust_bridge.configuration import rust_enabled
...
if TYPE_CHECKING:
    ...
    from derouter.types.llms.openai_evals import (
        CancelEvalResponse,
        CancelRunResponse,
        DeleteEvalResponse,
        Eval,
...
def _google_genai_streaming_hidden_params(
    *,
    ...
    _model_info: Final[Mapping[str, object]] = dict(getattr(derouter_params, "model_info", None) or {})
...
def _custom_logger_callbacks(logging_obj: DeRouterLoggingObj) -> list["CustomLogger"]:
    from derouter.integrations.custom_logger import CustomLogger
    ...
    callbacks: Final = list(derouter.callbacks)
...
def _has_pre_call_deployment_hook(logging_obj: DeRouterLoggingObj) -> bool:
    from derouter.integrations.custom_logger import CustomLogger
...
def _aws_signing_overrides(optional_params: Mapping[str, Any], derouter_params: Mapping[str, Any]) -> Mapping[str, Any]:
    return MappingProxyType(
        {
            key: derouter_params[key]
            for key in AWS_CREDENTIAL_KWARGS_KEYS
            if optional_params.get(key) is None and derouter_params.get(key) is not None
        }
    )
...
def _collect_ws_project_quota_callbacks() -> tuple[ProjectQuotaCallback, ...]:
    ...
    Uses duck-typing on ``derouter.callbacks`` (rather than importing the
    ...
    callbacks: Final = cast(  # cast-ok: callback registry is inspected before protocol use
        Sequence[object], _derouter.callbacks
    )
    return tuple(
...
class BaseLLMHTTPHandler:
        ...
        for i in range(max(max_retry_on_unprocessable_entity_error, 1)):
            try:
                ...
                should_retry = provider_config.should_retry_llm_api_inside_llm_translation_on_http_error(
                    e=e, derouter_params=derouter_params
                )
    ...
    def completion(
        ...
        provider_config = provider_config or ProviderConfigManager.get_provider_chat_config(
            model=model, provider=derouter.LlmProviders(custom_llm_provider)
        )
        ...
        def sign_and_log(
            ...
            signed: Final = cast(  # cast-ok: sign_request is declared as a bare dict
                ...
                provider_config.sign_request(
                    ...
                    optional_params={
                        **optional_params,
                        **_aws_signing_overrides(optional_params, derouter_params),
                    },
                    request_data=data,
                    api_base=api_base,
                    api_key=api_key,
            ...
            if derouter_params.get("_websearch_interception_converted_stream", False):
    ...
    def make_sync_call(
        ...
        if client is None or not isinstance(client, HTTPHandler):
            sync_httpx_client = _get_httpx_client(
                {
                    "ssl_verify": derouter_params.get("ssl_verify", None),
                }
            )
        else:
            sync_httpx_client = client
    ...
    async def arerank(
        ...
        if client is None or not isinstance(client, AsyncHTTPHandler):
            async_httpx_client = get_async_httpx_client(llm_provider=derouter.LlmProviders(custom_llm_provider))
        else:
            async_httpx_client = client
        try:
    ...
    def _prepare_ocr_request(
        ...
        Returns: (headers, complete_url, data, files)
        """
        from derouter.llms.base_llm.ocr.transformation import OCRRequestData
    ...
    async def _async_post_anthropic_messages_with_http_error_retry(
        ...
        derouter_params_dict: Final = dict(derouter_params)
        optional_params_dict: Final = dict(derouter_params)
        for attempt_idx in range(max_attempts):
            try:
                ...
                should_retry = provider_config.should_retry_anthropic_messages_on_http_error(
                    e=e, derouter_params=derouter_params_dict
                )
    ...
    def _resolve_anthropic_messages_timeout(
        ...
        from derouter.utils import supports_httpx_timeout

        stream_timeout: Final = derouter_params.get("stream_timeout") if stream else None
        model_timeout: Final = stream_timeout if stream_timeout is not None else derouter_params.get("timeout")
        request_timeout: Final = derouter_params.get("request_timeout")
    ...
    async def async_anthropic_messages_handler(
        ...
        if client is None or not isinstance(client, AsyncHTTPHandler):
            async_httpx_client = get_async_httpx_client(llm_provider=derouter.LlmProviders.ANTHROPIC)
        else:
            async_httpx_client = client
        ...
        provider_specific_header: Final = cast(
            derouter.types.utils.ProviderSpecificHeader | Sequence[derouter.types.utils.ProviderSpecificHeader] | None,
        ...
        explicit_vertex_location: Final = VertexBase.explicit_vertex_ai_location(MappingProxyType(dict(derouter_params)))
        ...
        additional_drop_params: Final[list[str]] = derouter_params.get("additional_drop_params") or []
        if additional_drop_params:
            from derouter.derouter_core_utils.dot_notation_indexing import delete_nested_value
        ...
        headers, signed_json_body = anthropic_messages_provider_config.sign_request(
            headers=headers,
            optional_params=dict(derouter_params),  # dynamic aws_* params are passed under derouter_params
            request_data=request_body,
            api_base=request_url,
            api_key=api_key,
            stream=stream,
    ...
    async def _maybe_rust_anthropic_messages(
        *,
        ...
        from derouter.rust_bridge import messages as rust_messages_bridge
        ...
        response_obj["_hidden_params"] = {"additional_headers": {"x-derouter-rust": "true"}}
        return response_obj
    ...
    def _rust_anthropic_messages_fake_stream(
        ...
        hidden_params: Final = AnthropicMessagesStreamHiddenParams(additional_headers={"x-derouter-rust": "true"})
        return AnthropicMessagesStreamingResponse(
            completion_stream=completion_stream,
    ...
    def _run_sync_responses_pre_call_deployment_hook(
        ...
        *,
        ...
        updated_derouter_params: Final = GenericDeRouterParams(
            **{
                **dict(derouter_params),
                **{
                    key: value
    ...
    def response_api_handler(
        ...
        _is_async: bool = False,
        fake_stream: bool = False,
        derouter_metadata: dict[str, object] | None = None,
        shared_session: Optional["ClientSession"] = None,
    ) -> (
        ResponsesAPIResponse
        | BaseResponsesAPIStreamingIterator
        ...
        # with the same info as chat, including derouter_params.
        ...
        request_context["derouter_params"] = dict(derouter_params)
        ...
        headers, signed_body = responses_api_provider_config.sign_request(
            headers=headers,
            optional_params=dict(derouter_params),
            request_data=data,
            api_base=api_base,
        ...
        if self._has_agentic_completion_hook(logging_obj):
            agentic_kwargs: Final = dict(derouter_params)  # mutable-ok: agentic hooks mutate kwargs in place
    ...
    async def async_delete_response_api_handler(
        ...
        headers: Final = responses_api_provider_config.validate_environment(
            headers=extra_headers or {}, model="None", derouter_params=derouter_params
        )
    ...
    def create_file(
        ...
        if isinstance(transformed_request, dict) and "initial_request" in transformed_request:
            ...
            try:
                ...
                if initial_response_data:
                    derouter_params["initial_file_response"] = initial_response_data
            except Exception as e:
                raise self._handle_error(
                    e=e,
                    provider_config=provider_config,
        ...
        # Store the upload URL in derouter_params for the transformation method
        ...
        derouter_params_with_url: Final = dict(derouter_params)
        if "upload_url" not in derouter_params:
            derouter_params_with_url["upload_url"] = api_base

        return provider_config.transform_create_file_response(
            model=None,
            raw_response=upload_response,
            logging_obj=logging_obj,
            derouter_params=derouter_params_with_url,
        )
    ...
    def create_batch(
        ...
        timeout: float | httpx.Timeout | None = None,
        model: str | None = None,
    ) -> Union["DeRouterBatch", Coroutine[object, object, "DeRouterBatch"]]:
        """
        ...
        derouter_params_with_request: Final = {
            **derouter_params,
        ...
        return provider_config.transform_create_batch_response(
            ...
            logging_obj=logging_obj,
            derouter_params=derouter_params_with_request,
        )
    ...
    def retrieve_batch(
        ...
        transformed_request: Final = provider_config.transform_retrieve_batch_request(
            batch_id=batch_id,
            optional_params=derouter_params,
            derouter_params=derouter_params,
        )
    ...
    def compact_response_api_handler(
        ...
        headers = responses_api_provider_config.validate_environment(
            headers=extra_headers or {}, model=model, derouter_params=derouter_params
        )
    ...
    def retrieve_file(
        ...
        # Validate environment and get headers
        headers = provider_config.validate_environment(
            api_key=derouter_params.get("api_key"),
            headers=headers,
            model="",
            messages=[],
            optional_params={},
    ...
    def _get_agentic_loop_settings(kwargs: dict) -> tuple[int, int, list[str]]:
        ...
        configured: Final = validated_max_agentic_loops(
            kwargs.get("max_agentic_loops"), field="derouter_params.max_agentic_loops"
        )
    ...
    @staticmethod
    def _has_agentic_completion_hook(logging_obj: DeRouterLoggingObj) -> bool:
        """
        ...
        String entries in ``derouter.callbacks`` (e.g. ``"datadog"``) are
    ...
    @staticmethod
    def _server_fulfilled_tools_in_request(logging_obj: DeRouterLoggingObj, tools: object) -> frozenset[str]:
        """The request's tools that a registered callback fulfills server-side (e.g. ``headroom_retrieve``)."""
        if not isinstance(tools, list) or not tools:
            return frozenset()
        from derouter.derouter_core_utils.prompt_templates.factory import has_tool_with_name
    ...
    def _finalize_refused_agentic_response(response: object, tool_calls: object) -> object:
        ...
        The refused tool calls target tools DeRouter injected on the client's
    ...
    async def _execute_anthropic_agentic_plan(
        ...
        from derouter.anthropic_interface import messages as anthropic_messages
        ...
        internal_keys: Final = {"derouter_logging_obj"}
        ...
        if callback is not None:
            try:
                ...
                _call_id: Final = getattr(logging_obj, "derouter_call_id", "unknown")
                verbose_logger.exception(
                    "DeRouter.AgenticHookError: Exception in "
    ...
    async def _execute_responses_agentic_plan(
        ...
        try:
            response: ResponsesAPIResponse | BaseResponsesAPIStreamingIterator = await derouter.aresponses(
    ...
    async def _run_agentic_loop_cleanup(
        ...
        try:
            ...
            _call_id: Final = getattr(logging_obj, "derouter_call_id", "unknown")
            verbose_logger.exception(
                "DeRouter.AgenticHookError: Exception in async_agentic_loop_cleanup_hook [call_id=%s model=%s]: %s",
                _call_id,
                model,
                str(e),
            )
    ...
    async def _execute_chat_completion_agentic_plan(
        ...
        return await derouter.acompletion(
    ...
    def _maybe_wrap_in_fake_stream(
        self,
        response: _ResponseT,
        logging_obj: Optional["DeRouterLoggingObj"],
        api_surface: str,
    ...
    async def _call_agentic_completion_hooks(
        ...
        callbacks: Final = derouter.callbacks + (logging_obj.dynamic_success_callbacks or [])
        ...
        for callback in callbacks:
            ...
            try:
                ...
                verbose_logger.exception(
                    "DeRouter.AgenticHookError: Exception in async_should_run_agentic_loop [call_id=%s model=%s]: %s",
                    logging_obj.derouter_call_id,
                    model,
                    str(e),
                )
                continue
            ...
            try:
                ...
                verbose_logger.warning(
                    "DeRouter.AgenticLoopRefused: ending turn [call_id=%s model=%s]: %s",
                    logging_obj.derouter_call_id,
                    model,
                    str(e),
                )
            ...
            try:
                ...
                _call_id = getattr(logging_obj, "derouter_call_id", "unknown")
                verbose_logger.exception(
                    "DeRouter.AgenticHookError: Exception in agentic completion hooks [call_id=%s model=%s]: %s",
                    _call_id,
                    model,
                    str(e),
                )
    ...
    async def _call_agentic_chat_completion_hooks(
        ...
        for callback in callbacks:
            ...
            try:
                ...
                verbose_logger.exception(
                    "DeRouter.AgenticHookError: Exception in async_should_run_chat_completion_agentic_loop: %s",
                    str(e),
                )
                continue
            ...
            try:
                ...
                verbose_logger.exception("DeRouter.AgenticHookError: Exception in chat completion agentic hooks: %s", e)
    ...
    async def async_realtime(
        ...
        try:
            ...
            async with backend_ws:
                ...
                if derouter_metadata:
                    _request_data["derouter_metadata"] = derouter_metadata
                ...
                if not provider_config.requires_session_configuration():
                    synthetic_session: Final = provider_config.transform_session_created_event(
                        model=model,
                        logging_session_id=logging_obj.derouter_trace_id,
    ...
    async def _async_realtime_session_post(
        ...
        if client is None or not isinstance(client, AsyncHTTPHandler):
            async_httpx_client = get_async_httpx_client(
                llm_provider=derouter.LlmProviders.OPENAI,
            )
        else:
            async_httpx_client = client
    ...
    async def async_responses_websocket(
        ...
        **kwargs: Any,
        ...
        derouter_params: Final = GenericDeRouterParams.model_validate(
            {
                "api_base": api_base,
                "api_key": api_key,
                **kwargs,
        ...
        try:
            ...
            async def _backend_connection():
                if _rust_responses_websocket_enabled(custom_llm_provider):
                    from derouter.rust_bridge import responses_websocket as rust_responses_websocket
    ...
    def image_generation_handler(
        ...
        api_base: Final = image_generation_provider_config.get_complete_url(
            model=model,
            api_base=derouter_params.get("api_base", None),
            api_key=derouter_params.get("api_key", None),
            optional_params=image_generation_optional_request_params,
            derouter_params=dict(derouter_params),
        )
        ...
        model_response: Final[ImageResponse] = image_generation_provider_config.transform_image_generation_response(
            model=model,
            raw_response=response,
            model_response=derouter.ImageResponse(),
            logging_obj=logging_obj,
            request_data=data,
            optional_params=image_generation_optional_request_params,
            derouter_params=dict(derouter_params),
    ...
    def video_generation_handler(
        ...
        headers: Final = video_generation_provider_config.validate_environment(
            api_key=api_key or derouter_params.get("api_key", None),
    ...
    async def async_vector_store_search_handler(
        ...
        if isinstance(vector_store_provider_config, BaseDirectVectorStoreConfig):
            ...
            return await vector_store_provider_config.aexecute_search_vector_store_request(
                ...
                derouter_logging_obj=logging_obj,
                derouter_params=dict(derouter_params),  # mutable-ok: snapshot GenericDeRouterParams into the Mapping shape
        ...
        headers = vector_store_provider_config.validate_environment(
            headers=extra_headers or {}, derouter_params=derouter_params
        )
        ...
        if isinstance(vector_store_provider_config, BaseQueryEmbeddingVectorStoreConfig):
            (
                ...
                api_base=api_base,
                derouter_logging_obj=logging_obj,
                derouter_params=MappingProxyType(dict(derouter_params, timeout=timeout)),
                extra_body=extra_body,
                embedding_executor=embedding_executor,
            )
        else:
        ...
        all_optional_params: Final[dict[str, object]] = dict(derouter_params)
    ...
    def generate_content_handler(
        ...
        When _is_async=True, returns a coroutine instead of making the call directly.
        """
        from derouter.google_genai.streaming_iterator import (
