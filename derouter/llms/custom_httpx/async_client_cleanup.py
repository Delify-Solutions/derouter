...
async def close_derouter_async_clients():
    """
    Close all cached async HTTP clients to prevent resource leaks.

    This function iterates through all cached clients in derouter's in-memory cache
    ...
    cache_dict: Final = getattr(derouter.in_memory_llm_clients_cache, "cache_dict", {})
    ...
    if hasattr(derouter, "base_llm_aiohttp_handler"):
        base_handler: Final = getattr(derouter, "base_llm_aiohttp_handler", None)
...
def register_async_client_cleanup():
    ...
    def cleanup_wrapper():
        ...
        try:
            ...
            try:
                loop.run_until_complete(close_derouter_async_clients())
            finally:
