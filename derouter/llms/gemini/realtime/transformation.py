...
from derouter.utils import get_empty_usage
...
class GeminiRealtimeConfig(BaseRealtimeConfig):
    ...
    def _model_cost_entry(model: str) -> dict:
        entry = derouter.model_cost.get(model)
        if entry is None:
            stripped: Final = model.split("/", 1)[-1]
            entry = derouter.model_cost.get(stripped) or derouter.model_cost.get(f"gemini/{stripped}")
        return entry or {}
    ...
    def transform_response_done_event(
        ...
        responses_api_usage = DeRouterCompletionResponsesConfig._transform_chat_completion_usage_to_responses_usage(
    ...
    def transform_realtime_response(
        ...
        logging_session_id: Final = logging_obj.derouter_trace_id
        ...
        for key, value in list(json_message.items()):  # snapshot: handlers may mutate json_message
            ...
            if openai_event == OpenAIRealtimeEventTypes.SESSION_CREATED:
                ...
                tool_call_responses_api_usage = (
                    DeRouterCompletionResponsesConfig._transform_chat_completion_usage_to_responses_usage(
    ...
    def requires_session_configuration(self) -> bool:
        # Deferred setup opt-in: derouter.gemini_live_defer_setup = True
        return not derouter.gemini_live_defer_setup
