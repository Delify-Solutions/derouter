...
Implements the transformation between DeRouter's unified vector store API
...
class GeminiVectorStoreConfig(BaseVectorStoreConfig):
    ...
    def validate_environment(self, headers: dict, derouter_params: GenericDeRouterParams | None) -> dict:
        ...
        if derouter_params:
            api_key: Final = derouter_params.get("api_key") or get_api_key_from_env()
            if api_key:
    ...
    def transform_search_vector_store_request(
        ...
        # Get model from derouter_params or use default
        # Note: File Search requires gemini-2.5-flash or later
        model = derouter_params.get("model") or "gemini-2.5-flash"
        if model and model.startswith("gemini/"):
            model = model.replace("gemini/", "")

        api_key: Final = derouter_params.get("api_key") or GeminiModelInfo.get_api_key()
        if not api_key:
        ...
        derouter_logging_obj.model_call_details["vector_store_id"] = vector_store_id
    ...
    def transform_search_vector_store_response(
        ...
        try:
            ...
            query: Final[str] = derouter_logging_obj.model_call_details.get("query", "")
