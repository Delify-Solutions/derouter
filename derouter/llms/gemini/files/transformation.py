...
class GoogleAIStudioFilesHandler(GeminiModelInfo, BaseFilesConfig):
    ...
    def get_complete_url(
        ...
        final_api_key: Final = api_key or derouter_params.get("api_key") or self.get_api_key()
    ...
    def transform_retrieve_file_request(
        ...
        """
        api_key: Final = derouter_params.get("api_key") or self.get_api_key()
        if not api_key:
            raise ValueError("api_key is required")

        file_part: Final = self._normalize_gemini_file_id(file_id)

        api_base = self.get_api_base(derouter_params.get("api_base")) or "https://generativelanguage.googleapis.com"
    ...
    def transform_delete_file_request(
        ...
        Args:
            ...
            derouter_params: DeRouter parameters containing api_key

        Returns:
            tuple[str, dict]: (url, params) for the DELETE request
        """
        api_base: Final = self.get_api_base(derouter_params.get("api_base"))
        if not api_base:
            raise ValueError("api_base is required")
