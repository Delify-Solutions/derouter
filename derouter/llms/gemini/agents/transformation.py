...
from derouter.llms.gemini.common_utils import GeminiError, GeminiModelInfo
...
# Keys inside derouter_params that should be forwarded to the Gemini
# create-agent body verbatim.
_GEMINI_AGENT_BODY_KEYS: Final = ("base_agent", "instructions", "base_environment")

# DeRouter-internal keys that must never be forwarded to Gemini.
_DEROUTER_INTERNAL_KEYS: Final = frozenset(
    {
        "custom_llm_provider",
        "api_key",
        "api_base",
...
class GeminiAgentsConfig(BaseAgentsAPIConfig):
    ...
    def validate_environment(
        ...
        explicit_api_key: Final = derouter_params.get("api_key")
        ...
        if derouter_params.get("api_base") and not explicit_api_key:
            raise ValueError(
    ...
    def transform_list_request(
        ...
        if derouter_params.get("page_size"):
            params["pageSize"] = derouter_params["page_size"]
        if derouter_params.get("page_token"):
            params["pageToken"] = derouter_params["page_token"]
        return url, params
