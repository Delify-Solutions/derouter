...
from derouter.exceptions import AuthenticationError, BadRequestError, ServiceUnavailableError, Timeout
...
class _MongoDBSearchParams(BaseModel):
    """Typed view over the vector store's derouter_params; unrelated keys are ignored."""

    model_config = ConfigDict(frozen=True, extra="ignore")

    derouter_embedding_model: str | None = None
    derouter_embedding_config: Mapping[str, object] | None = None
    ...
    def require_embedding_model(self) -> str:
        if not self.derouter_embedding_model:
            raise config_error(
                "derouter_embedding_model is required in derouter_params for the MongoDB vector store. "
                ...
                "will be meaningless. Example: derouter_embedding_model: openai/text-embedding-3-small"
            )
        return self.derouter_embedding_model

    def require_database(self) -> str:
        if not self.mongodb_database:
            raise config_error(
                "mongodb_database is required in derouter_params for the MongoDB vector store. "
    ...
    def require_collection(self) -> str:
        if not self.mongodb_collection:
            raise config_error(
                "mongodb_collection is required in derouter_params for the MongoDB vector store. "
...
class MongoDBVectorStoreConfig(BaseQueryEmbeddingVectorStoreConfig):
    def __init__(self, embedding_executor: VectorStoreEmbeddingExecutor | None = None) -> None:
        self.embedding_executor: Final = embedding_executor or DeRouterVectorStoreEmbeddingExecutor()

    def get_auth_credentials(self, derouter_params: Mapping[str, object]) -> BaseVectorStoreAuthCredentials:
    ...
    @staticmethod
    def _reject_unknown_params(derouter_params: Mapping[str, object]) -> None:
        ...
        if derouter_params.get("mongodb_connection_string") is not None:
            raise config_error(
                "MongoDB vector stores now use the BETA sidecar. Move mongodb_connection_string to "
                "MONGODB_CONNECTION_STRING in the sidecar, remove it from DeRouter, and configure api_base and api_key."
            )
        unknown: Final = sorted(
            key for key in derouter_params if key.startswith(_MONGODB_PARAM_PREFIX) and key not in _KNOWN_MONGODB_PARAMS
        )
        if unknown:
            raise config_error(
    ...
    def validate_environment(
        self, headers: Mapping[str, object], derouter_params: GenericDeRouterParams | None
        ...
        self._reject_unknown_params(MappingProxyType(dict(derouter_params)))
        api_key: Final = derouter_params.api_key or get_secret_str("MONGODB_SIDECAR_API_KEY")
        if not api_key:
    ...
    def _params(
        ...
        cls._reject_unknown_params(derouter_params)
        if extra_body:
        ...
        try:
            params: Final = _MongoDBSearchParams.model_validate(derouter_params)
        ...
        cls._timeout_ms(derouter_params.get("timeout"))
        return params
    ...
    def _request(
        ...
        if not embedding_response.data:
            raise config_error(
                "The embedding model returned no embedding for the search query. Check derouter_embedding_model."
            )
    ...
    def transform_search_vector_store_request(
        ...
        params: Final = self._params(derouter_params, vector_store_search_optional_params, extra_body)
        query_text: Final = self._query_text(query)
        response: Final = (embedding_executor or self.embedding_executor).embed(
            params.require_embedding_model(), query_text, params.derouter_embedding_config or _EMPTY_EMBEDDING_CONFIG
        )
