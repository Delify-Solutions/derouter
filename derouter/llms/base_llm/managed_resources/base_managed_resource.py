...
class BaseManagedResource(ABC, Generic[ResourceObjectType]):
    ...
    def table_name(self) -> str:
        ...
        Example: 'derouter_managedfiletable', 'derouter_managedvectorstoretable'
        """
    ...
    def get_unified_resource_id_format(
        ...
        Example for files:
            "derouter_proxy:application/json;unified_id,{uuid};target_model_names,{models};..."
    ...
    async def create_resource_for_model(
        ...
        Args:
            ...
            derouter_parent_otel_span: OpenTelemetry span for tracing
    ...
    async def store_unified_resource_id(
        ...
        """
        verbose_logger.info("Storing DeRouter Managed %s with id=%s in cache", self.resource_type, unified_resource_id)
        ...
        verbose_logger.debug(
            "DeRouter Managed %s with id=%s stored in db: %s", self.resource_type, unified_resource_id, result
        )

    async def get_unified_resource_id(
        self,
        unified_resource_id: str,
        derouter_parent_otel_span: Span | None = None,
    ) -> dict[str, Any] | None:
        """
    ...
    async def delete_unified_resource_id(
        ...
        if initial_value is None:
            raise Exception(f"DeRouter Managed {self.resource_type} with id={unified_resource_id} not found")
    ...
    async def can_user_access_unified_resource_id(
        ...
        resource: Final = await self.get_unified_resource_id(unified_resource_id, derouter_parent_otel_span)
    ...
    async def get_model_resource_id_mapping(
        ...
        for resource_id in resource_ids:
            ...
            unified_resource_object = await self.get_unified_resource_id(resource_id, derouter_parent_otel_span)
    ...
    def get_unified_id_prefix(self) -> str:
        ...
        Returns:
            Prefix string (e.g., "derouter_proxy:")
        """
