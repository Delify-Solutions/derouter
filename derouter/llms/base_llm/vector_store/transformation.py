...
class DeRouterVectorStoreEmbeddingExecutor:
    def embed(self, model: str, query: str, configuration: Mapping[str, object]) -> EmbeddingResponse:
        import derouter

        return derouter.embedding(  # pyright: ignore[reportCallIssue, reportUnknownMemberType, reportUnknownVariableType]  # provider kwargs are intentionally dynamic
            model=model,
            input=[query],  # mutable-ok: DeRouter embedding requires a mutable input list
    ...
    async def aembed(self, model: str, query: str, configuration: Mapping[str, object]) -> EmbeddingResponse:
        ...
        return await derouter.aembedding(  # pyright: ignore[reportUnknownMemberType]  # provider kwargs are intentionally dynamic
            model=model,
            input=[query],  # mutable-ok: DeRouter embedding requires a mutable input list
            **dict(configuration),  # pyright: ignore[reportArgumentType]  # provider-specific embedding config is validated downstream  # mutable-ok: kwargs require a concrete dict
        )
...
def vector_store_request_metadata(kwargs: Mapping[str, object]) -> Mapping[str, object]:
    ...
    if isinstance(derouter_metadata, dict):
        return _REQUEST_METADATA.validate_python(derouter_metadata)
...
class RouterVectorStoreEmbeddingExecutor:
    ...
    def _router_serves(self, model: str) -> bool:
        ...
        resolved: Final = self.router.resolved_derouter_models(model, team_id if isinstance(team_id, str) else None)
        deployment_models: Final = (
            deployment.get("derouter_params", {}).get("model") for deployment in self.router.get_model_list() or ()
        )
    ...
    def embed(self, model: str, query: str, configuration: Mapping[str, object]) -> EmbeddingResponse:
        ...
        if not self._router_serves(model):
            return DeRouterVectorStoreEmbeddingExecutor().embed(model, query, embedding_kwargs)
    ...
    async def aembed(self, model: str, query: str, configuration: Mapping[str, object]) -> EmbeddingResponse:
        embedding_kwargs: Final = self._embedding_kwargs(configuration)
        if not self._router_serves(model):
            return await DeRouterVectorStoreEmbeddingExecutor().aembed(model, query, embedding_kwargs)
...
class BaseQueryEmbeddingVectorStoreConfig(BaseVectorStoreConfig):
    ...
    @staticmethod
    def query_embedding_model(derouter_params: Mapping[str, object]) -> str:
        embedding_model: Final = derouter_params.get("derouter_embedding_model")
        if isinstance(embedding_model, str) and embedding_model:
            return embedding_model
        raise ValueError(
            "derouter_embedding_model is required in derouter_params for this vector store. "
            "Example: derouter_params['derouter_embedding_model'] = 'openai/text-embedding-3-small'"
        )

    @staticmethod
    def query_embedding_configuration(derouter_params: Mapping[str, object]) -> Mapping[str, object]:
        configuration: Final = derouter_params.get("derouter_embedding_config")
        if isinstance(configuration, Mapping):
            return {str(key): value for key, value in configuration.items()}  # pyright: ignore[reportUnknownVariableType, reportUnknownArgumentType]  # derouter_params is an untyped dict, keys are re-validated as str here
    ...
    def query_embedding_executor(
        ...
        return DeRouterVectorStoreEmbeddingExecutor()
    ...
    def embed_query(
        ...
        model: Final = self.query_embedding_model(derouter_params)
        configuration: Final = self.query_embedding_configuration(derouter_params)
