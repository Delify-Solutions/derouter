...
# File-type inputs are preprocessed to this format in derouter/ocr/main.py.
DocumentType = dict[str, str]

OCRRequestFormat = Literal["derouter", "native"]

OCR_REQUEST_FORMATS: Final[tuple[OCRRequestFormat, ...]] = ("derouter", "native")
...
class OCRPageDimensions(DeRouterPydanticObjectBase):
...
class OCRPageImage(DeRouterPydanticObjectBase):
...
class OCRPage(DeRouterPydanticObjectBase):
...
class OCRUsageInfo(DeRouterPydanticObjectBase):
...
class OCRResponse(DeRouterPydanticObjectBase):
    """
...
class OCRRequestData(DeRouterPydanticObjectBase):
...
class BaseOCRConfig:
    ...
    def get_health_check_document(self) -> DocumentType:
        return {  # mutable-ok: derouter.aocr rejects any document that is not a dict
            "type": "document_url",
    ...
    def transform_ocr_request(
        ...
        **kwargs,
        ...
        the preprocessing in derouter/ocr/main.py.
