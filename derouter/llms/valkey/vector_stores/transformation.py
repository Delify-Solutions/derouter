...
from derouter.llms.valkey.common_utils import build_valkey_url, pack_vector
...
class _ValkeySearchParams(BaseModel):
    ...
    def require_embedding_model(self) -> str:
        if not self.derouter_embedding_model:
            raise ValueError(
                "derouter_embedding_model is required in derouter_params for the Valkey vector store. "
    ...
    def connection_url(self) -> str:
        if not self.valkey_host:
            raise ValueError(
                "valkey_host is required in derouter_params for the Valkey vector store. "
                "Set it on the vector store's derouter_params, e.g. valkey_host: my-valkey.example.com"
            )
...
class ValkeyVectorStoreConfig(BaseDirectVectorStoreConfig):
    def __init__(
        ...
        self.embedding_fn = embedding_fn if embedding_fn is not None else derouter.embedding
        self.aembedding_fn = aembedding_fn if aembedding_fn is not None else derouter.aembedding
    ...
    def execute_search_vector_store_request(
        ...
        params: Final = _ValkeySearchParams.model_validate(derouter_params)
        ...
        embedding_response: Final = (
            embedding_executor.embed(
                ...
                query_text,
                params.derouter_embedding_config or _EMPTY_EMBEDDING_CONFIG,
            )
