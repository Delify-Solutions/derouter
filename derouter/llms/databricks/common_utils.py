"""
Databricks integration utilities for DeRouter.
...
class DatabricksBase:
    ...
    def _build_user_agent(custom_user_agent: str | None = None) -> str:
        ...
        is extracted and prefixed to the derouter user agent with an underscore.
        The custom version is ignored; DeRouter's version is always used.
        ...
        Returns:
            ...
            - Default: "derouter/{version}"
            - With custom: "{partner}_derouter/{version}"

        Examples:
            - None -> "derouter/1.79.1"
            - "mycompany/1.0.0" -> "mycompany_derouter/1.79.1"
            - "partner_product/2.0.0" -> "partner_product_derouter/1.79.1"
            - "acme" -> "acme_derouter/1.79.1"
        """
        try:
        ...
        if custom_user_agent:
            ...
            if partner_name and partner_name.replace("_", "").replace("-", "").isalnum():
                return f"{partner_name}_derouter/{version}"

        # Default: just derouter
        return f"derouter/{version}"
    ...
    def _get_databricks_credentials(
        ...
        Also registers DeRouter as a partner for proper telemetry attribution
        ...
        try:
            ...
            # Register DeRouter as partner for Databricks telemetry attribution
            useragent.with_partner("derouter")
