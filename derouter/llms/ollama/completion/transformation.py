...
from derouter.types.llms.openai import AllMessageValues, ChatCompletionUsageBlock
...
class OllamaConfig(BaseConfig):
    ...
    def get_api_key() -> str | None:
        """Get API key from environment variables or derouter configuration"""
        import os
    ...
    def transform_response(
        ...
        if request_data.get("format", "") == "json":
            ...
            if not response_text or not response_text.strip():
                ...
                message = derouter.Message(content="")
                ...
                try:
                    ...
                    message = derouter.Message(content=content, reasoning_content=reasoning_content)
                    model_response.choices[0].message = message
                    model_response.choices[0].finish_reason = "stop"
        else:
            response_text = response_json.get("response", "")
        ...
        tokenizer: Final = encoding if encoding is not None else derouter.encoding
    ...
    def transform_request(
        ...
        custom_prompt_dict: Final = derouter_params.get("custom_prompt_dict") or derouter.custom_prompt_dict

        text_completion_request: Final = derouter_params.get("text_completion")
