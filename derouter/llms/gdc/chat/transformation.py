...
class GDCGeminiConfig(OpenAILikeChatConfig):
    supports_vertex_params: bool = True  # Tell DeRouter utilities not to strip vertex_ params
    ...
    def _resolve_project(self, optional_params: dict, derouter_params: dict) -> str | None:
        return (
    ...
    def _resolve_location(self, optional_params: dict, derouter_params: dict) -> str | None:
        return (
            derouter_params.get("vertex_location")
            or derouter_params.get("vertex_ai_location")
            or getattr(derouter, "vertex_location", None)
            or optional_params.get("vertex_location")
            or optional_params.get("vertex_ai_location")
        )

    def _effective_project(self, api_base: str, optional_params: dict, derouter_params: dict) -> str | None:
        match: Final = re.search(r"/v1/projects/([^/]+)", api_base)
        if match:
            return match.group(1)
        return self._resolve_project(optional_params, derouter_params)

    def _validate_path_id(self, value: str, field: str, model: str) -> str:
        if not self._PATH_ID_PATTERN.match(value):
            raise derouter.utils.AuthenticationError(
    ...
    def get_complete_url(
        ...
        api_base = api_base or derouter.gdc_api_base or derouter.api_base
        if not api_base:
            raise derouter.utils.AuthenticationError(
        ...
        project = self._resolve_project(optional_params, derouter_params)
        ...
        location = self._resolve_location(optional_params, derouter_params)
    ...
    def validate_environment(
        ...
        project = self._effective_project(api_base, optional_params, derouter_params)
        if not project:
            raise derouter.utils.AuthenticationError(
                message="project is required for GDC Gemini. Please pass vertex_project.",
                llm_provider="gdc",
        ...
        if creds is not None:
            ssl_verify: Final = self._read_env_bool(derouter_params.get("ssl_verify"), "SSL_VERIFY", default=True)
            if self._read_env_bool(derouter_params.get("gdc_token_caching"), "GDC_TOKEN_CACHING", default=False):
