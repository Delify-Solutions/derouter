...
class S3VectorsVectorStoreConfig(BaseQueryEmbeddingVectorStoreConfig, BaseAWSLLM):
    ...
    def get_complete_url(self, api_base: str | None, derouter_params: dict) -> str:
        aws_region_name: Final = self.get_aws_region_name_for_non_llm_api_calls(derouter_params.get("aws_region_name"))
        return f"https://s3vectors.{aws_region_name}.api.aws"

    @staticmethod
    def query_embedding_model(derouter_params: Mapping[str, object]) -> str:
        configured: Final = derouter_params.get("derouter_embedding_model") or derouter_params.get("embedding_model")
        return configured if isinstance(configured, str) and configured else _DEFAULT_QUERY_EMBEDDING_MODEL

    @staticmethod
    def _query_target(vector_store_id: str, derouter_params: Mapping[str, object]) -> tuple[str, str]:
        if ":" in vector_store_id:
            bucket_name, index_name = vector_store_id.split(":", 1)
            return bucket_name, index_name
        bucket_name_from_params: Final = derouter_params.get("vector_bucket_name")
        if not isinstance(bucket_name_from_params, str) or not bucket_name_from_params:
            raise ValueError(
                ...
                "or vector_bucket_name must be provided in derouter_params"
            )
    ...
    def _query_request(
        ...
        derouter_logging_obj.model_call_details["query"] = query_text
    ...
    def transform_search_vector_store_request(
        ...
        bucket_name, index_name = self._query_target(vector_store_id, derouter_params)
    ...
    def transform_search_vector_store_response(
        ...
        try:
            ...
            return VectorStoreSearchResponse(
                ...
                search_query=derouter_logging_obj.model_call_details.get("query", ""),
                data=results,
            )
