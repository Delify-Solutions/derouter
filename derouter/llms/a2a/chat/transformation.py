"""
A2A Protocol Transformation for DeRouter
"""
...
class A2AConfig(BaseConfig):
    ...
    def resolve_agent_config_from_registry(
        ...
        Args:
            ...
            optional_params: Dict to merge additional derouter_params into
        ...
        try:
            ...
            if agent:
                ...
                # Get api_key, headers, and other params from derouter_params
                if agent.derouter_params:
                    if api_key is None:
                        api_key = agent.derouter_params.get("api_key")

                    if headers is None:
                        agent_headers: Final = agent.derouter_params.get("headers")
                        if agent_headers:
                            headers = agent_headers

                    # Merge other derouter_params (timeout, max_retries, etc.)
                    for key, value in agent.derouter_params.items():
