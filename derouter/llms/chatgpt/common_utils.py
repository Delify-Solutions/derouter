...
def get_chatgpt_user_agent(originator: str) -> str:
    ...
    version: Final = _get_derouter_version()
...
def _normalize_derouter_params(derouter_params: Any | None) -> dict:
    ...
    if hasattr(derouter_params, "model_dump"):
        try:
            return derouter_params.model_dump()
        except Exception:
            return {}
    if hasattr(derouter_params, "dict"):
        try:
            return derouter_params.dict()
        except Exception:
            return {}
    return {}


def get_chatgpt_session_id(derouter_params: Any | None) -> str | None:
    params: Final = _normalize_derouter_params(derouter_params)
    ...
    if isinstance(metadata, dict):
        ...
        if value:
            return str(value)
    for key in ("derouter_trace_id", "derouter_call_id"):
        value = params.get(key)
        if value:
            return str(value)
    return None
...
def ensure_chatgpt_session_id(derouter_params: Any | None) -> str:
    return get_chatgpt_session_id(derouter_params) or str(uuid4())
