...
import derouter
from derouter.constants import REPLICATE_POLLING_DELAY_SECONDS
...
def completion(
    ...
    if "stream" in optional_params and optional_params["stream"] is True:
        ...
        for retry in range(derouter.DEFAULT_REPLICATE_POLLING_RETRIES):
            time.sleep(
                derouter.DEFAULT_REPLICATE_POLLING_DELAY_SECONDS + 2 * retry
            ...
            if response.status_code == 200 and response.json().get("status") in [
                ...
                continue
            return derouter.ReplicateConfig().transform_response(
                model=model,
...
async def async_completion(
    ...
    async_handler: Final = get_async_httpx_client(
        llm_provider=derouter.LlmProviders.REPLICATE,
        params={"timeout": 600.0},
    )
