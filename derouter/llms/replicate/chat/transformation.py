...
import derouter
from derouter.constants import REPLICATE_MODEL_NAME_WITH_ID_LENGTH
...
class ReplicateConfig(BaseConfig):
    ...
    def transform_request(
        ...
        ## Load Config
        config: Final = derouter.ReplicateConfig.get_config()
        ...
        if model in derouter.custom_prompt_dict:
            # check if the model has a registered custom prompt
            model_prompt_details: Final = derouter.custom_prompt_dict[model]
        ...
        if prompt is None or not isinstance(prompt, str):
            raise ReplicateError(
                status_code=400,
                message=f"DeRouter Error - prompt is not a string - {prompt}",
                headers={},
            )
    ...
    def transform_response(
        ...
        if raw_response_json.get("status") != "succeeded":
            raise ReplicateError(
                status_code=422,
                message=f"DeRouter Error - prediction not succeeded - {raw_response_json}",
    ...
    def get_prediction_url(self, response: httpx.Response) -> str:
        ...
        if prediction_url is None:
            raise ReplicateError(
                status_code=400,
                message=f"DeRouter Error - prediction url is None - {response_json}",
