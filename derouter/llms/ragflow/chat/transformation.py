...
class RAGFlowConfig(OpenAIConfig):
    ...
    def get_complete_url(
        ...
        Args:
            ...
            derouter_params: DeRouter parameters (may contain api_base)
            stream: Whether streaming is enabled

        Returns:
            Complete URL for the API call
        """
        # Get api_base from multiple sources: input param, derouter_params, environment, or global derouter setting
        if derouter_params and hasattr(derouter_params, "api_base") and derouter_params.api_base:
            api_base = api_base or derouter_params.api_base

        api_base = api_base or derouter.api_base or get_secret("RAGFLOW_API_BASE") or get_secret_str("RAGFLOW_API_BASE")

        if api_base is None:
            raise ValueError(
                "api_base is required for RAGFlow provider. Set it via api_base parameter, RAGFLOW_API_BASE environment variable, or derouter.api_base"
            )
    ...
    def _get_openai_compatible_provider_info(
        ...
        # The model name will be stored in derouter_params for use in requests
        _, _, actual_model = self._parse_ragflow_model(model)

        # Get api_base from multiple sources: input param, environment, or global derouter setting
        dynamic_api_base: Final = (
            api_base or derouter.api_base or get_secret("RAGFLOW_API_BASE") or get_secret_str("RAGFLOW_API_BASE")
        )

        # Get api_key from multiple sources: input param, environment, or global derouter setting
        dynamic_api_key: Final = api_key or derouter.api_key or get_secret_str("RAGFLOW_API_KEY")
    ...
    def validate_environment(
        ...
        Args:
            ...
            optional_params: Optional parameters
            derouter_params: DeRouter parameters (may contain api_key)
            api_key: API key (from input params)
        ...
        """
        # Use api_key from derouter_params if available, otherwise fall back to other sources
        if derouter_params and hasattr(derouter_params, "api_key") and derouter_params.api_key:
            api_key = api_key or derouter_params.api_key

        # Get api_key from multiple sources: input param, derouter_params, environment, or global derouter setting
        api_key = api_key or derouter.api_key or get_secret_str("RAGFLOW_API_KEY")
        ...
        try:
            _, _, actual_model = self._parse_ragflow_model(model)
            # Store the actual model name in derouter_params for use in transform_request
            derouter_params["_ragflow_actual_model"] = actual_model
        except ValueError:
    ...
    def transform_request(
        ...
        Args:
            ...
            messages: Chat messages
            optional_params: Optional parameters
            derouter_params: DeRouter parameters (may contain _ragflow_actual_model)
            headers: Request headers

        Returns:
            Transformed request dictionary
        """
        # Get the actual model name from derouter_params if available
        actual_model = derouter_params.get("_ragflow_actual_model")
        ...
        return super().transform_request(actual_model, messages, optional_params, derouter_params, headers)
