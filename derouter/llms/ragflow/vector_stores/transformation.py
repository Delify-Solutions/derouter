...
class RAGFlowVectorStoreConfig(BaseVectorStoreConfig):
    ...
    def get_auth_credentials(self, derouter_params: dict) -> BaseVectorStoreAuthCredentials:
        ...
        if api_key is None:
            raise ValueError("api_key is required (set RAGFLOW_API_KEY env var or pass in derouter_params)")
        return {
            "headers": {
    ...
    def validate_environment(self, headers: dict, derouter_params: GenericDeRouterParams | None) -> dict:
        ...
        api_key: Final = derouter_params.api_key or get_secret_str("RAGFLOW_API_KEY")

        if api_key is None:
            raise ValueError("RAGFLOW_API_KEY is required (set env var or pass in derouter_params)")
    ...
    def get_complete_url(
        ...
        - api_base in derouter_params
        - Default: http://localhost:9380
        """
        api_base = (
            api_base or derouter_params.get("api_base") or get_secret_str("RAGFLOW_API_BASE") or "http://localhost:9380"
        )
    ...
    def transform_create_vector_store_request(
        ...
        Maps DeRouter params to RAGFlow dataset creation parameters.
