...
from derouter.types.rerank import OptionalRerankParams, RerankRequest, RerankResponse
...
class CohereRerankConfig(BaseRerankConfig):
    ...
    def validate_environment(
        ...
        if api_key is None:
            api_key = get_secret_str("COHERE_API_KEY") or get_secret_str("CO_API_KEY") or derouter.cohere_key

        if api_key is None:
            raise ValueError(
                "Cohere API key is required. Please set 'COHERE_API_KEY' or 'CO_API_KEY' or 'derouter.cohere_key'"
            )
    ...
    def transform_rerank_response(
        ...
        No transformation required, derouter follows cohere API response format
        """
        try:
