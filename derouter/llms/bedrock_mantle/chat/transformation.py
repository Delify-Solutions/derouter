...
Auth: Bearer token (derouter_params.api_key, BEDROCK_MANTLE_API_KEY, or the
...
class BedrockMantleChatConfig(BedrockMantleAuthMixin, OpenAILikeChatConfig):
    ...
    def _get_openai_compatible_provider_info(
        ...
        region: Final = (
            (derouter_params.aws_region_name if derouter_params else None)
        ...
        api_base = (
            ...
            or f"https://bedrock-mantle.{region}.api.aws/{mantle_base_segment(model, derouter.model_cost)}"
        )
    ...
    def validate_environment(
        ...
        project_id: Final = derouter_params.get("aws_bedrock_project_id")
        if project_id:
