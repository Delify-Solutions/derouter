...
AWS_BEARER_TOKEN_BEDROCK, or derouter_params.api_key) when present; otherwise
...
class BedrockMantleResponsesAPIConfig(BedrockMantleAuthMixin, OpenAIResponsesAPIConfig):
    ...
    def get_complete_url(
        ...
        region: Final = self._resolve_region({**derouter_params, "api_base": api_base})
        ...
        # responses/main.py auto-injects into derouter_params.api_base), pin to the
    ...
    def validate_environment(self, headers: dict, model: str, derouter_params: GenericDeRouterParams | None) -> dict:
        ...
        bearer: Final = self._resolve_bearer_token(derouter_params.api_key)
        if bearer:
            headers["Authorization"] = f"Bearer {bearer}"
        if derouter_params.aws_bedrock_project_id:
            headers["OpenAI-Project"] = derouter_params.aws_bedrock_project_id
        return headers
        ...
        if not drop_params:
            raise derouter.utils.UnsupportedParamsError(
                ...
                message=(
                    ...
                    "Responses API only accepts 'auto' or 'default'. Set `drop_params: true` (derouter_settings "
                    "or this deployment's derouter_params) to have DeRouter drop it, or remove service_tier from "
