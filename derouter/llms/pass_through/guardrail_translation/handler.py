...
It uses the field targeting configuration from derouter_logging_obj
...
from derouter.proxy._types import PassThroughGuardrailSettings
...
class PassThroughEndpointHandler(BaseTranslation):
    ...
    Uses passthrough_guardrails_config from derouter_logging_obj
    to determine which fields to extract for guardrail processing.
    """

    def _get_guardrail_settings(
        self,
        derouter_logging_obj: Optional["DeRouterLoggingObj"],
        ...
        passthrough_config: Final = getattr(derouter_logging_obj, "passthrough_guardrails_config", None)
    ...
    def _extract_text_for_guardrail(
        ...
        """
        from derouter.proxy.pass_through_endpoints.jsonpath_extractor import (
            JsonPathExtractor,
        )
        ...
        payload_to_check: Final = {
            k: v for k, v in data.items() if not k.startswith("_") and k not in ("metadata", "derouter_logging_obj")
        }
    ...
    async def process_input_messages(
        ...
        derouter_logging_obj: Optional["DeRouterLoggingObj"] = None,
    ) -> Mapping[str, object]:
        """
        ...
        settings: Final = self._get_guardrail_settings(derouter_logging_obj, guardrail_name)
