...
This module bridges derouter's transcription dispatch to NVIDIA Riva's gRPC
...
``riva-client`` is imported lazily so ``derouter`` core remains usable
...
from derouter.llms.nvidia_riva.audio_transcription.audio_utils import (
...
from derouter.llms.nvidia_riva.common_utils import (
    NvidiaRivaException,
    grpc_error_to_derouter_exception,
)
...
_RIVA_INSTALL_HINT = "NVIDIA Riva client is not installed. Install with `pip install 'derouter[stt-nvidia-riva]'`."
...
class NvidiaRivaAudioTranscription:
    ...
    def _run_sync(
        ...
        if not api_base:
            raise NvidiaRivaException(
                ...
                message=(
                    ...
                    "`localhost:50051`). Set it in derouter_params or via "
        ...
        try:
            ...
            raise grpc_error_to_derouter_exception(e) from e
