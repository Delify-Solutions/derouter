...
from derouter.types.utils import ModelResponse, ModelResponseStream, ServerToolUse
...
class GroqChatConfig(OpenAILikeChatConfig):
    ...
    def get_supported_openai_params(self, model: str) -> list:
        ...
        if not (
            ...
            or derouter.supports_web_search(model=model, custom_llm_provider=self.custom_llm_provider)
        ):
            base_params.remove("web_search_options")

        try:
            if derouter.supports_reasoning(model=model, custom_llm_provider=self.custom_llm_provider):
    ...
    def _transform_messages(
        ...
        for idx, message in enumerate(messages):
            """
            1. Don't pass 'null' function_call assistant message to groq - https://github.com/Delify-Solutions/derouter/issues/5839
            """
    ...
    def map_openai_params(
        ...
        if _response_format is not None and isinstance(_response_format, dict):
            ...
            if json_schema is not None:
                ...
                if not derouter.supports_response_schema(model=model, custom_llm_provider="groq"):
                    ...
                    if "tools" in non_default_params:
                        raise derouter.BadRequestError(
                            ...
                            "DeRouter uses a tool-calling workaround for structured outputs on this model, "
...
class GroqChatCompletionStreamingHandler(OpenAIChatCompletionStreamingHandler):
    def chunk_parser(self, chunk: dict) -> ModelResponseStream:
        ...
        # Map Groq's 'reasoning' field to DeRouter's 'reasoning_content' field
        # Groq returns delta.reasoning, but DeRouter expects delta.reasoning_content
