...
from derouter.llms.openai.common_utils import BaseOpenAILLM
from derouter.secret_managers.get_azure_ad_token_provider import (
    get_azure_ad_token_provider,
)
from derouter.secret_managers.main import get_secret_str
from derouter.types.router import GenericDeRouterParams
from derouter.utils import _add_path_to_api_base
...
def get_azure_ad_token_from_oidc(
    ...
    client: Final = derouter.module_level_client
...
def get_azure_ad_token(
    ...
    Args:
        derouter_params: Dictionary containing authentication parameters
    ...
    azure_ad_token_provider = derouter_params.get("azure_ad_token_provider")
    azure_ad_token = derouter_params.get("azure_ad_token") or get_secret_str("AZURE_AD_TOKEN")
    tenant_id: Final = derouter_params.get("tenant_id") or os.getenv("AZURE_TENANT_ID")
    client_id: Final = derouter_params.get("client_id") or os.getenv("AZURE_CLIENT_ID")
    client_secret: Final = derouter_params.get("client_secret") or os.getenv("AZURE_CLIENT_SECRET")
    azure_username: Final = derouter_params.get("azure_username") or os.getenv("AZURE_USERNAME")
    azure_password: Final = derouter_params.get("azure_password") or os.getenv("AZURE_PASSWORD")
    scope = derouter_params.get("azure_scope") or os.getenv(
    ...
    # Try to get token provider from service principal or DefaultAzureCredential
    elif azure_ad_token_provider is None and derouter.enable_azure_ad_token_refresh is True:
        verbose_logger.debug(
        ...
        try:
            ...
            verbose_logger.error(
                "Error calling Azure AD token provider: %s. Follow docs - https://router.delify.vn/docs/providers/azure/#azure-ad-token-refresh---defaultazurecredential",
                e,
            )
            raise e

        #########################################################
        # If derouter.enable_azure_ad_token_refresh is True and no other token provider is available,
...
class BaseAzureLLM(BaseOpenAILLM):
    ...
    def _try_get_default_azure_credential_provider(
        ...
        """
        from derouter.types.secret_managers.get_azure_ad_token_provider import (
            AzureCredentialType,
        )
    ...
    def get_azure_openai_client(
        ...
        self.set_cached_openai_client(
            ...
            client_initialization_params=client_initialization_params,
            client_type="azure",
            derouter_owned_client=self.owns_wrapped_http_client(azure_client_params.get("http_client")),
        )
    ...
    def initialize_azure_sdk_client(
        ...
        azure_ad_token = derouter_params.get("azure_ad_token")

        # derouter_params sometimes contains the key, but the value is None
        # We should respect environment variables in this case
        tenant_id: Final = self._resolve_env_var(derouter_params, "tenant_id", "AZURE_TENANT_ID")
        client_id: Final = self._resolve_env_var(derouter_params, "client_id", "AZURE_CLIENT_ID")
        client_secret: Final = self._resolve_env_var(derouter_params, "client_secret", "AZURE_CLIENT_SECRET")
        azure_username: Final = self._resolve_env_var(derouter_params, "azure_username", "AZURE_USERNAME")
        azure_password: Final = self._resolve_env_var(derouter_params, "azure_password", "AZURE_PASSWORD")
        scope = self._resolve_env_var(derouter_params, "azure_scope", "AZURE_SCOPE")
        if scope is None:
            scope = "https://cognitiveservices.azure.com/.default"

        max_retries: Final = derouter_params.get("max_retries")
        timeout: Final = derouter_params.get("timeout")
        ...
        elif not api_key and azure_ad_token_provider is None and derouter.enable_azure_ad_token_refresh is True:
            verbose_logger.debug(
        ...
        if api_version is None:
            api_version = os.getenv("AZURE_API_VERSION", derouter.AZURE_DEFAULT_API_VERSION)
    ...
    def _init_azure_client_for_cloudflare_ai_gateway(
        ...
        tenant_id: Final = derouter_params.get("tenant_id", os.getenv("AZURE_TENANT_ID"))
        client_id: Final = derouter_params.get("client_id", os.getenv("AZURE_CLIENT_ID"))
        scope: Final = derouter_params.get(
            "azure_scope",
        ...
        if client is None:
            ...
            azure_client_params: Final[_AzureGatewayClientParams] = {
                "api_version": api_version,
                "base_url": f"{api_base}",
                "http_client": derouter.client_session,
    ...
    @staticmethod
    def _base_validate_azure_environment(headers: dict, derouter_params: GenericDeRouterParams | None) -> dict:
        ...
        azure_ad_token: Final = get_azure_ad_token(derouter_params)
        if azure_ad_token:
    ...
    def _get_base_azure_url(
        api_base: str | None,
        derouter_params: GenericDeRouterParams | Mapping[str, object] | None,
        ...
        Args:
            ...
            derouter_params: The derouter parameters.
        ...
        derouter_params = derouter_params or {}
        api_version: Final = cast(str | None, derouter_params.get("api_version")) or default_api_version
    ...
    def get_azure_v1_image_url(api_base: str, api_version: str | None, route: str) -> str | None:
        ...
        return BaseAzureLLM._get_base_azure_url(
            ...
            derouter_params=MappingProxyType({"api_version": api_version}),
            route=route,
        )
    ...
    def _resolve_env_var(
        self, derouter_params: Mapping[str, str | None], param_key: str, env_var_key: str
    ) -> str | None:
        ...
        derouter_params may contain the key with a None value, in which case we want
        to fallback to the environment variable.
        """
        param_value: Final = derouter_params.get(param_key)
...
def get_azure_credentials(
    ...
    """Resolve Azure credentials from params, derouter globals, and env vars."""
    resolved_api_base: Final = api_base or derouter.api_base or get_secret_str("AZURE_API_BASE")
    resolved_api_version: Final = api_version or derouter.api_version or get_secret_str("AZURE_API_VERSION")
