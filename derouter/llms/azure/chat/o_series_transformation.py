...
from derouter.utils import get_model_info, supports_reasoning
...
class AzureOpenAIO1Config(OpenAIOSeriesConfig):
    def get_supported_openai_params(self, model: str) -> list:
        ...
        """
        all_openai_params: Final = derouter.OpenAIGPTConfig().get_supported_openai_params(model=model)
    ...
    def _get_o_series_only_params(self, model: str) -> list:
        ...
        #########################################################
        # Case 1: If the model is recognized and in derouter model cost map
        # then check if it supports reasoning
        #########################################################
        if model in derouter.model_list_set:
    ...
    def should_fake_stream(
        ...
        if model and "o3" in model:  # o3 models support streaming - https://github.com/Delify-Solutions/derouter/issues/8274
            return False
    ...
    def transform_request(
        ...
        return super().transform_request(model, messages, flattened_params, derouter_params, headers)
