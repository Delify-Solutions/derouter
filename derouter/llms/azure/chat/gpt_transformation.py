...
from derouter.llms.openai.chat.gpt_5_transformation import GPT_REASONING_SERIES_MARKERS
from derouter.types.llms.azure import (
...
class AzureOpenAIConfig(BaseConfig):
    ...
    def requires_max_completion_tokens(cls, model: str) -> bool:
        ...
        the reasoning path by https://github.com/Delify-Solutions/derouter/issues/13781.
        """
                ...
                if api_version_year is None or api_version_month is None or api_version_day is None:
                    ...
                    if (
                        ...
                        if derouter.drop_params is True or (drop_params is not None and drop_params is True):
                            ...
                            raise UnsupportedParamsError(
                                status_code=400,
                                message=f"Azure does not support '{value}' as a {param} param, for api_version={api_version}. To drop 'tool_choice=required' for calls with this Azure API version, set `derouter.drop_params=True` or for proxy:\n\n`derouter_settings:\n drop_params: true`\nAzure API Reference: https://learn.microsoft.com/en-us/azure/ai-services/openai/reference#chat-completions",
                            )
                    else:
                        optional_params["tool_choice"] = value
