...
import derouter
from derouter.constants import AZURE_OPERATION_POLLING_TIMEOUT, DEFAULT_MAX_RETRIES
...
from derouter.derouter_core_utils.url_utils import SSRFError, assert_same_origin
                ...
                if isinstance(value, list):
                    for item in value:
                        if "file_id" in item:
                            ...
                            if derouter.drop_params is True:
                                pass
                            else:
                                raise derouter.utils.UnsupportedParamsError(
                                    message=f"Azure doesn't support {value}. To drop it from the call, set `derouter.drop_params = True.",
                                    status_code=400,
                                )
                else:
                    raise derouter.utils.UnsupportedParamsError(
...
class AzureChatCompletion(BaseAzureLLM, BaseLLM):
    ...
    def completion(
        ...
        try:
            ...
            if api_base is not None and "gateway.ai.cloudflare.com" in api_base:
            ...
            elif derouter.AzureOpenAIGPT5Config.is_model_gpt_5_model(model=derouter_params.get("base_model") or model):
                data = derouter.AzureOpenAIGPT5Config().transform_request(
                    model=model,
                    messages=messages,
                ...
                data = derouter.AzureOpenAIConfig().transform_request(
                    model=model,
                    messages=messages,
                    optional_params=optional_params,
                    derouter_params=derouter_params,
    ...
    def embedding(
        ...
        try:
            ...
            if max_retries is None:
                max_retries = derouter.DEFAULT_MAX_RETRIES
            ## LOGGING
            logging_obj.pre_call(
                input=input,
                api_key=api_key,
    ...
    def make_sync_azure_httpx_request(
        ...
        if client is None:
            ...
            sync_handler = HTTPHandler(**_params, client=derouter.client_session)
        else:
            sync_handler = client
    ...
    def image_generation(
        ...
        try:
            ...
            if (
                ...
                and derouter_params.get("base_model", None) is not None
            ):
                model_response._hidden_params["model"] = derouter_params.get("base_model", None)
            ...
            base_model: Final = derouter_params.get("base_model", None) if derouter_params else None
    ...
    def get_headers(
        ...
        client_session: Final = derouter.client_session or httpx.Client()
        if api_base is not None and "gateway.ai.cloudflare.com" in api_base:
