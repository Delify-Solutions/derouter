...
class MilvusVectorStoreConfig(BaseQueryEmbeddingVectorStoreConfig):
    ...
    Supports vector search with embeddings generated via derouter.embeddings.
    """
    ...
    def validate_environment(self, headers: dict, derouter_params: GenericDeRouterParams | None) -> dict:
        ...
        if derouter_params is not None:
            api_key = derouter_params.api_key or get_secret_str("MILVUS_API_KEY")

        if not api_key:
            raise ValueError(
                "MILVUS_API_KEY is not set. Either set it in the derouter_params or set the MILVUS_API_KEY environment variable."
            )
    ...
    def get_complete_url(
        ...
        if not api_base:
            raise ValueError(
                "Milvus API base URL is required. Set MILVUS_API_BASE environment variable or pass api_base in derouter_params."
            )
    ...
    def transform_search_vector_store_request(
        ...
        query_vector: Final = self.embed_query(query_text, derouter_params, embedding_executor)
    ...
    async def atransform_search_vector_store_request(
        ...
        query_text: Final = self.query_text(query)
        query_vector: Final = await self.aembed_query(query_text, derouter_params, embedding_executor)
        return self._search_request(
            vector_store_id,
            query_text,
            query_vector,
    ...
    def _search_request(
        ...
        scope: Final = {
            key: value
            for key, value in (
                ("dbName", derouter_params.get("milvus_db_name")),
                ("partitionNames", derouter_params.get("milvus_partition_names")),
            )
            if value
        }
        derouter_logging_obj.model_call_details["input"] = query_text
        derouter_logging_obj.model_call_details["embedding_model"] = derouter_params.get("derouter_embedding_model")
    ...
    def transform_search_vector_store_response(
        ...
        try:
            ...
            # Try to get text_field from optional_params first, then derouter_params
            optional_params: Final = derouter_logging_obj.model_call_details.get("optional_params", {})
            text_field = optional_params.get("milvus_text_field", "")

            # Fallback to derouter_params if not in optional_params

            if not text_field:
                text_field = derouter_logging_obj.model_call_details.get("derouter_params", {}).get(
                    "milvus_text_field", ""
                )
            ...
            return VectorStoreSearchResponse(
                ...
                search_query=derouter_logging_obj.model_call_details.get("input", ""),
                data=search_results,
            )
