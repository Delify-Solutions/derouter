...
from derouter.derouter_core_utils.derouter_logging import verbose_logger
...
from derouter.types.llms.bedrock import BedrockInvokeAnthropicMessagesRequest
...
class AmazonAnthropicClaudeMessagesConfig(
    ...
    WEBSEARCH_INTERCEPTION_DOCS_URL = "https://router.delify.vn/docs/integrations/websearch_interception"
    ...
    # is intentionally absent — it is DeRouter-internal, consumed via
    ...
    def _filter_context_management_for_bedrock_invoke(
        ...
        sends on every request) are DeRouter-internal: thinking is injected
        separately via ``_ensure_thinking_for_clear_thinking_context_management``,
        and forwarding the raw edit would trip Bedrock's
        ``"context_management: Extra inputs are not permitted"`` 400.

        Refs:
          * https://github.com/Delify-Solutions/derouter/issues/27532
          * https://docs.aws.amazon.com/bedrock/latest/userguide/model-parameters-anthropic-claude-messages-tool-use.md
        """
        cm: Final = anthropic_messages_request.get("context_management")
        if not isinstance(cm, dict):
    ...
    def _reject_unsupported_web_search_tools(anthropic_messages_request: dict[str, object], model: str) -> None:
        ...
        (``derouter_settings.callbacks: ["websearch_interception"]``), the tool
        ...
        raise derouter.BadRequestError(
            message=(
                ...
                "To use web search with this model, enable DeRouter's web search interception "
    ...
    def transform_anthropic_messages_request(
        ...
        # Ref: https://github.com/Delify-Solutions/derouter/issues/22797
    ...
    async def bedrock_sse_wrapper(
        ...
        """
        Bedrock invoke does not return SSE formatted data. This function is a wrapper to ensure derouter chunks are SSE formatted.
