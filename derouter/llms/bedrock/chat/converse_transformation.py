...
class AmazonConverseConfig(BaseConfig):
    ...
    def get_supported_openai_params(self, model: str) -> list[str]:
        ...
        if derouter.utils.supports_tool_choice(
            model=model, custom_llm_provider=self.custom_llm_provider
        ) or derouter.utils.supports_tool_choice(model=base_model, custom_llm_provider=self.custom_llm_provider):
        ...
        if tool_choice == "none":
            if derouter.drop_params is True or drop_params is True:
                ...
                raise derouter.utils.UnsupportedParamsError(
                    message=f"Bedrock doesn't support tool_choice={tool_choice}. To drop it from the call, set `derouter.drop_params = True.",
                    status_code=400,
                )
            ...
            raise derouter.utils.UnsupportedParamsError(
                message=f"Bedrock doesn't support tool_choice={tool_choice}. Supported tool_choice values=['auto', 'required', json object]. To drop it from the call, set `derouter.drop_params = True.",
                status_code=400,
            )
    ...
    def _supports_native_structured_outputs(model: str, custom_llm_provider: str | None = None) -> bool:
        ...
        which looks up the flag in ``derouter.model_cost`` via
        ``_get_model_info_helper``.
        Ref: https://docs.aws.amazon.com/bedrock/latest/userguide/structured-output.html
        """
        from derouter.utils import supports_native_structured_output
            ...
            if param == "thinking" and "openai.gpt-5" not in model:
                if (
                    ...
                    if capped is not None:
                        ...
                        derouter.verbose_logger.warning(DROP_UNSUPPORTED_ADAPTIVE_THINKING_WARNING, model)
                else:
    ...
    def _translate_response_format_param(
        ...
        if self._supports_native_structured_outputs(model, self.custom_llm_provider) and json_schema is not None:
            ...
            if (
                derouter.utils.supports_tool_choice(model=model, custom_llm_provider=self.custom_llm_provider)
    ...
    def _prepare_request_params(
        ...
        # These are DeRouter internal parameters, not API parameters
        ...
        if anthropic_output_config is not None and isinstance(anthropic_output_config, dict):
            ...
            if is_bedrock_application_inference_profile_arn(model):
                ...
                if derouter.drop_params is True and not AnthropicConfig._model_supports_effort_param(model, "bedrock"):
                    derouter.verbose_logger.warning(
                        DROP_UNSUPPORTED_OUTPUT_CONFIG_WARNING,
                        model,
                    )
    ...
    def _transform_request_helper(
        ...
        if "tools" not in optional_params and messages is not None and has_tool_call_blocks(messages):
            if derouter.modify_params:
                ...
                raise derouter.UnsupportedParamsError(
                    message="Bedrock doesn't support tool calling without `tools=` param specified. Pass `tools=` param OR set `derouter.modify_params = True` // `derouter_settings::modify_params: True` to add dummy tool to the request.",
                    model="",
                    llm_provider="bedrock",
                )
        ...
        # Related issues: https://github.com/Delify-Solutions/derouter/issues/14194
        if (
            ...
            for point in cache_injection_points:
                if point.get("location") == "tool_config":
                    ...
                    if derouter_params is not None:
                        ...
                        AnthropicCacheControlHook.record_gateway_injection(derouter_params, 1)
                    break
    ...
    async def _async_transform_request(
        ...
        bedrock_messages: Final = await BedrockConverseMessagesProcessor._bedrock_converse_messages_pt_async(
            ...
            model=model,
            llm_provider="bedrock_converse",
            user_continue_message=derouter_params.pop("user_continue_message", None),
        )

        request_metadata: Final = resolve_bedrock_request_metadata(
            derouter_params=derouter_params, caller_metadata=_data.get("requestMetadata")
        )
    ...
    def _translate_message_content(
        ...
        for idx, content in enumerate(content_blocks):
            ...
            if "toolUse" in content:
                ## check tool name was formatted by derouter
    ...
    def validate_environment(
        ...
        owned_names, metadata_headers = bedrock_request_metadata_headers(derouter_params)
