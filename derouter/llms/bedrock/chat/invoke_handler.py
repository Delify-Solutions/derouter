...
async def make_call(
    ...
    bedrock_invoke_provider: derouter.BEDROCK_INVOKE_PROVIDERS_LITERAL | None = None,
    stream_chunk_size: int | None = None,
) -> "tuple[MockResponseIterator | AsyncIterator[GChunk | ModelResponseStream | dict], httpx.Headers]":
    try:
        if client is None:
            client = get_async_httpx_client(
                llm_provider=derouter.LlmProviders.BEDROCK,
                params=(
                    {"ssl_verify": logging_obj.derouter_params.get("ssl_verify")}
                    if logging_obj and logging_obj.derouter_params and logging_obj.derouter_params.get("ssl_verify")
                    else None
                ),
...
class AmazonDeepSeekR1StreamDecoder(AWSEventStreamDecoder):
    def __init__(
        ...
        super().__init__(model=model)
        from derouter.llms.bedrock.chat.invoke_transformations.amazon_deepseek_transformation import (
...
class MockResponseIterator:  # for returning ai21 streaming responses
    ...
    def _handle_json_mode_chunk(
        ...
        if self.json_mode is True and tool_calls is not None:
            message: Final = derouter.AnthropicConfig()._convert_tool_response_to_message(tool_calls=tool_calls)
