...
class AmazonTitanConfig(AmazonInvokeConfig, BaseConfig):
    ...
    def _map_and_modify_arg(
        ...
        """
        filter params to fit the required provider format, drop those that don't fit if user sets `derouter.drop_params = True`.
        """
        filtered_stop = None
        if "stop" in supported_params and derouter.drop_params:
