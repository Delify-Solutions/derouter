...
def make_sync_call(
    ...
    if fake_stream:
        model_response: Final[ModelResponse] = derouter.AmazonConverseConfig()._transform_response(
            model=model,
            response=response,
...
class BedrockConverseLLM(BaseAWSLLM):
    ...
    async def async_streaming(
        ...
        request_data: Final = await derouter.AmazonConverseConfig()._async_transform_request(
            model=model,
            messages=messages,
        ...
        prepped: Final = self.get_request_headers(
            ...
            aws_region_name=derouter_params.get("aws_region_name") or "us-west-2",
    ...
    async def async_completion(
        ...
        if client is None or not isinstance(client, AsyncHTTPHandler):
            ...
            client = get_async_httpx_client(params=_params, llm_provider=derouter.LlmProviders.BEDROCK)
        else:
            client = client
        ...
        transformed_response: Final = derouter.AmazonConverseConfig()._transform_response(
            model=model,
            response=response,
    ...
    def completion(
        ...
        fake_stream = derouter.AmazonConverseConfig().should_fake_stream(
            fake_stream=fake_stream,
            model=model,
            stream=stream,
            custom_llm_provider="bedrock",
        ...
        derouter_params["aws_region_name"] = aws_region_name  # [DO NOT DELETE] important for async calls
        ...
        _data: Final = derouter.AmazonConverseConfig()._transform_request(
            model=model,
            messages=messages,
            optional_params=optional_params,
            derouter_params=derouter_params,
        ...
        sync_transformed_response: Final = derouter.AmazonConverseConfig()._transform_response(
            model=model,
            response=response,
            model_response=model_response,
            stream=stream if isinstance(stream, bool) else False,
