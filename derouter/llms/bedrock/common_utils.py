...
if TYPE_CHECKING:
    ...
    from derouter.types.llms.bedrock import BedrockCreateBatchRequest
...
from derouter.llms.base_llm.base_utils import BaseLLMModelInfo, BaseTokenCounter
...
_ERROR_REQUEST_URL: Final = "https://router.delify.vn/docs"
...
def merge_bedrock_aws_request_params(
    ...
    """
    request_params: Final = {**optional_params, **derouter_params}  # mutable-ok: AWS helpers require a plain dict
    has_static_deployment_credentials: Final = all(
        isinstance(derouter_params.get(key), str) and bool(derouter_params.get(key))
...
def _bedrock_model_supports(model: str, key: str) -> bool:
    from derouter.utils import _supports_factory
...
def normalize_custom_field_on_tools(request_body: dict) -> None:
    ...
    Ref: https://github.com/Delify-Solutions/derouter/issues/22847
    """
...
def ensure_bedrock_anthropic_messages_tool_names(request_body: dict) -> None:
    ...
    In-place: set ``name`` to ``derouter_unnamed_tool_{index}`` when missing or blank.
    """
    tools: Final = request_body.get("tools")
    if not tools or not isinstance(tools, list):
        return
    for i, tool in enumerate(tools):
        ...
        if name is None or (isinstance(name, str) and not name.strip()):
            tool["name"] = f"derouter_unnamed_tool_{i}"
...
def _get_bedrock_client_ssl_verify() -> bool | str:
    ...
    """
    from derouter.llms.custom_httpx.http_handler import get_ssl_verify
...
def init_bedrock_client(
    ...
    derouter_aws_region_name: Final = get_secret("AWS_REGION_NAME", None)
    ...
    resolved_region_name: Final = next(
        (
            candidate
            for candidate in (region_name, aws_region_name, derouter_aws_region_name, standard_aws_region_name)
    ...
    if aws_web_identity_token is not None and aws_role_name is not None and aws_session_name is not None:
        ...
        # aws_access_key_id is not None, assume user is trying to auth using derouter.completion
...
def get_bedrock_tool_name(response_tool_name: str) -> str:
    """
    If derouter formatted the input tool name, we need to convert it back to the original name.
    ...
    if response_tool_name in derouter.bedrock_tool_name_mappings.cache_dict:
        response_tool_name = derouter.bedrock_tool_name_mappings.cache_dict[response_tool_name]
...
def strip_bedrock_routing_prefix(model: str) -> str:
    """Strip DeRouter routing prefixes from model name."""
...
def bedrock_converse_supports_parallel_tool_use_config(model: str) -> bool:
    return any(
        (derouter.model_cost.get(candidate) or {}).get("supports_parallel_tool_use_config") is True
...
def bedrock_model_accepts_cache_points(model: str | None) -> bool:
    ...
    so existing caching setups never silently degrade. ``derouter.utils.supports_prompt_caching``
    ...
    entries: Final = tuple(
        entry
        for candidate in (model, get_bedrock_base_model(model))
        if (entry := derouter.model_cost.get(candidate)) is not None
    )
    if not entries:
        return True
...
def is_claude_4_5_on_bedrock(model: str) -> bool:
    ...
    """
    return any(
        (derouter.model_cost.get(candidate) or {}).get("cache_creation_input_token_cost_above_1hr") is not None
        for candidate in (model, get_bedrock_base_model(model))
    )
...
from derouter.llms.bedrock.count_tokens.bedrock_token_counter import BedrockTokenCounter
...
class BedrockModelInfo(BaseLLMModelInfo):
    ...
    @staticmethod
    def get_non_derouter_routing_model_name(model: str) -> str:
    ...
    def get_bedrock_route(
        ...
        alt_model: Final = BedrockModelInfo.get_non_derouter_routing_model_name(model=model)
        if base_model in derouter.bedrock_converse_models or alt_model in derouter.bedrock_converse_models:
            return "converse"
        return "invoke"
    ...
    def get_bedrock_provider_config_for_messages_api(
        ...
        All other routes should return None since they will go through derouter.completion
        """
        ...
        if BedrockModelInfo._explicit_claude_platform_route(model):
            return derouter.BedrockClaudePlatformMessagesConfig()

        #########################################################
        # Converse routes should go through derouter.completion()
        ...
        if BedrockModelInfo._explicit_mantle_route(model):
            from derouter.llms.bedrock.messages.mantle_transformation import (
        ...
        #########################################################
        # This goes through derouter.AmazonAnthropicClaude3MessagesConfig()
        # Since bedrock Invoke supports Native Anthropic Messages API
        #########################################################
        if "claude" in model:
            return derouter.AmazonAnthropicClaudeMessagesConfig()

        #########################################################
        # These routes will go through derouter.completion()
        #########################################################
        return None
...
def get_bedrock_chat_config(model: str):
    ...
    if bedrock_route == "claude_platform":
        return derouter.BedrockClaudePlatformConfig()
    elif bedrock_route == "converse" or bedrock_route == "converse_like":
        return derouter.AmazonConverseConfig()
    elif bedrock_route == "openai":
        return derouter.AmazonBedrockOpenAIConfig()
    elif bedrock_route == "agent":
        from derouter.llms.bedrock.chat.invoke_agent.transformation import (
            AmazonInvokeAgentConfig,
        )

        return AmazonInvokeAgentConfig()
    elif bedrock_route == "agentcore":
        from derouter.llms.bedrock.chat.agentcore.transformation import (
            AmazonAgentCoreConfig,
        )

        return AmazonAgentCoreConfig()
    elif bedrock_route == "mantle":
        from derouter.llms.bedrock.chat.mantle.transformation import (
            AmazonMantleConfig,
        )
    ...
    if bedrock_invoke_provider == "amazon":
        return derouter.AmazonTitanConfig()
    elif bedrock_invoke_provider == "anthropic":
        if base_model in derouter.AmazonAnthropicConfig.get_legacy_anthropic_model_names():
            return derouter.AmazonAnthropicConfig()
        else:
            return derouter.AmazonAnthropicClaudeConfig()
    elif bedrock_invoke_provider == "meta" or bedrock_invoke_provider == "llama":
        return derouter.AmazonLlamaConfig()
    elif bedrock_invoke_provider == "ai21":
        return derouter.AmazonAI21Config()
    elif bedrock_invoke_provider == "cohere":
        return derouter.AmazonCohereConfig()
    elif bedrock_invoke_provider == "mistral":
        return derouter.AmazonMistralConfig()
    elif bedrock_invoke_provider == "moonshot":
        return derouter.AmazonMoonshotConfig()
    elif bedrock_invoke_provider == "deepseek_r1":
        return derouter.AmazonDeepSeekR1Config()
    elif bedrock_invoke_provider == "nova":
        return derouter.AmazonInvokeNovaConfig()
    elif bedrock_invoke_provider == "qwen3":
        return derouter.AmazonQwen3Config()
    elif bedrock_invoke_provider == "qwen2":
        return derouter.AmazonQwen2Config()
    elif bedrock_invoke_provider == "twelvelabs":
        return derouter.AmazonTwelveLabsPegasusConfig()
    else:
        return derouter.AmazonInvokeConfig()
...
def _load_bedrock_response_stream_shape():
    ...
    try:
        ...
        verbose_logger.warning(
            "derouter: could not load bedrock-runtime response stream shape "
...
def resolve_s3_encryption_key_id(
    ...
    Precedence: `s3_encryption_key_id` in derouter_params, then optional_params
    (client-side / request params), then the AWS_S3_ENCRYPTION_KEY_ID env var.
    """
    candidates: Final = tuple(
        source.get("s3_encryption_key_id") for source in (derouter_params, optional_params) if source is not None
    )
...
class CommonBatchFilesUtils:
    ...
    def get_bedrock_model_id_from_derouter_model(self, model: str) -> str:
        """
        Extract the actual Bedrock model ID from DeRouter model name.

        Args:
            model: DeRouter model name (e.g., "bedrock/anthropic.claude-3-sonnet-20240229-v1:0")
    ...
    def extract_model_from_s3_file_path(self, s3_uri: str, optional_params: dict) -> str:
        ...
        Format: s3://bucket/derouter-bedrock-files-{model}-{uuid}.jsonl
        """
        # Check if model is provided in optional_params first
        if "model" in optional_params and optional_params["model"]:
            return self.get_bedrock_model_id_from_derouter_model(optional_params["model"])

        # Extract model from S3 URI path
        # Expected format: s3://bucket/derouter-bedrock-files-{model}-{uuid}.jsonl
        try:
            bucket, object_key = self.parse_s3_uri(s3_uri)

            # Extract model from object key if it follows our naming pattern
            if object_key.startswith("derouter-bedrock-files-"):
                # Remove prefix and suffix to get model part
                model_part: Final = object_key[22:]  # Remove "derouter-bedrock-files-"
    ...
    def generate_unique_job_name(self, model: str, prefix: str = "derouter") -> str:
        """
        ...
        # Example: derouter-batch-claude-266c398e
    ...
    def get_s3_bucket_and_key_from_config(
        ...
        key_prefix: str = "derouter",
    ) -> tuple:
        """
        Get S3 bucket and generate a unique key from configuration.

        Args:
            derouter_params: DeRouter parameters
        ...
        # Get bucket name
        bucket_name: Final = (
            derouter_params.get("s3_bucket_name") or optional_params.get("s3_bucket_name") or os.getenv(bucket_env_var)
        )
        if not bucket_name:
