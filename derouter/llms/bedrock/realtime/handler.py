...
from derouter.derouter_core_utils.realtime_streaming import DefaultLoggedRealTimeEventTypes
from derouter.types.llms.openai import OpenAIRealtimeEvents
...
def _should_log_event(openai_message: Mapping[str, object]) -> bool:
    logged_types: Final = (
        derouter.logged_real_time_event_types
        if derouter.logged_real_time_event_types is not None
        ...
        **kwargs: object,
        ...
        if credentials is None:
            raise BedrockError(
                status_code=401,
                message=(
                    "No AWS credentials found for Bedrock realtime. Set aws_* params in derouter_params "
