...
from derouter.constants import BEDROCK_INVOKE_PROVIDERS_LITERAL
from derouter.files.utils import FilesAPIUtils
from derouter.derouter_core_utils.aws_partition import get_aws_dns_suffix
from derouter.derouter_core_utils.cloud_storage_security import (
...
from derouter.types.llms.bedrock import BedrockBatchRecordKind
...
from derouter.types.utils import ExtractedFileData, LlmProviders, SpecialEnums
from derouter.utils import get_llm_provider

from ..base_aws_llm import BaseAWSLLM
from ..common_utils import BedrockError, merge_bedrock_aws_request_params, resolve_s3_encryption_key_id

# derouter_params key used to hand the SigV4-signed GET headers from
...
# derouter_params key carrying the size of the body uploaded to S3, handed from
...
def _strip_llm_routing_prefix(model: str) -> str:
    try:
        ...
        verbose_logger.exception(
            "derouter.llms.bedrock.files.transformation.py::_strip_llm_routing_prefix() - Error inferring custom_llm_provider - %s",
            e,
        )
        return model
...
def extract_s3_uri_from_file_id(file_id: str) -> str:
    ...
    Accepts either a base64-encoded DeRouter unified file id (whose decoded
    ...
    raise ValueError("file_id must be a managed DeRouter S3 file id")
...
def _trusted_s3_model_credentials(derouter_params: Mapping[str, object]) -> _TrustedS3ModelCredentials:
    trusted_model_credentials: Final = derouter_params.get("_derouter_internal_model_credentials")
...
def get_configured_s3_bucket_name(derouter_params: Mapping[str, object]) -> str:
    """
    ...
    """
    bucket_name: Final = _trusted_s3_model_credentials(derouter_params).s3_bucket_name or os.getenv("AWS_S3_BUCKET_NAME")
    if not bucket_name:
        raise ValueError(_S3_BUCKET_REQUIRED_ERROR)
    return bucket_name


def get_configured_s3_bucket_names(derouter_params: Mapping[str, object]) -> tuple[str, ...]:
    """
    ...
    """
    trusted: Final = _trusted_s3_model_credentials(derouter_params)
...
def _uploaded_object_size(derouter_params: Mapping[str, object], raw_response: Response) -> int:
    """
    ...
    """
    uploaded_size: Final = derouter_params.get(UPLOAD_CONTENT_LENGTH_PARAM)
...
class BedrockFilesConfig(BaseAWSLLM, BaseFilesConfig):
    ...
    def validate_environment(
        ...
        derouter_params: MutableMapping[str, object],
        api_key: str | None = None,
        api_base: str | None = None,
    ) -> dict:
        result: Final[dict[str, object]] = {}
        result.update(headers)
        signed_headers: Final = derouter_params.pop(S3_SIGNED_GET_HEADERS_PARAM, None)
    ...
    def _get_s3_object_name_from_batch_jsonl(
        ...
        named as: derouter-bedrock-files/{model}/{uuid}
        """
    ...
    def get_complete_file_url(
        ...
        """
        request_params: Final = merge_bedrock_aws_request_params(derouter_params, optional_params)
        bucket_name = derouter_params.get("s3_bucket_name") or os.getenv("AWS_S3_BUCKET_NAME")
        if not bucket_name:
            raise ValueError(
                "S3 bucket_name is required. Set 's3_bucket_name' in derouter_params or AWS_S3_BUCKET_NAME env var"
            )
        bucket_name, object_prefix = split_configured_cloud_bucket_name(bucket_name)

        s3_region_name: Final = derouter_params.get("s3_region_name") or optional_params.get("s3_region_name")
    ...
    def _lookup_provider_specific_field(model_id: str, field: str) -> str | None:
        ...
        `provider_specific_entry` dict via `derouter.get_model_info`.
    ...
    def _map_openai_embedding_to_bedrock_params(
        ...
        if not self._is_titan_v2_embed_model(model):
            ...
            raise NotImplementedError(
                ...
                "embedding models in https://github.com/Delify-Solutions/derouter/issues."
            )
    ...
    def _transform_text_completion_body_to_chat_body(
        ...
        `derouter.text_completion` does in real time has to happen here too.
        """
    ...
    def _map_openai_to_bedrock_params(
        ...
        if provider in self.CONVERSE_INVOKE_PROVIDERS:
            from derouter.llms.bedrock.chat.converse_transformation import (
    ...
    def _transform_openai_jsonl_content_to_bedrock_jsonl_content(
        ...
        for idx, _openai_jsonl_content in enumerate(openai_jsonl_content):
            ...
            resolved_model = derouter.model_alias_map.get(record_model, record_model)
    ...
    def transform_create_file_request(
        ...
        if FilesAPIUtils.is_batch_jsonl_file(
            ...
            derouter_params_model: Final = derouter_params.get("model")
            target_model: Final = model or (derouter_params_model if isinstance(derouter_params_model, str) else "")
        ...
        merged_params: Final = merge_bedrock_aws_request_params(derouter_params, optional_params)
        s3_region_name: Final = derouter_params.get("s3_region_name") or optional_params.get("s3_region_name")
        ...
        derouter_params["upload_url"] = api_base
        upload_content_length: Final = len(file_content.encode("utf-8"))
        derouter_params[UPLOAD_CONTENT_LENGTH_PARAM] = upload_content_length  # rebind-ok: same handoff as upload_url
    ...
    def _sign_s3_request(
        ...
        Reuses the exact pattern from derouter/integrations/s3_v2.py
        """
        try:
            import hashlib
    ...
    def _convert_https_url_to_s3_uri(self, https_url: str) -> tuple[str, str]:
        ...
        Example:
            Input: "https://s3.us-west-2.amazonaws.com/derouter-proxy/file.jsonl"
            Output: ("s3://derouter-proxy/file.jsonl", "file.jsonl")
        """
        import re
    ...
    def transform_create_file_response(
        ...
        upload_url: Final = derouter_params.get("upload_url")
        file_id: str = ""
        filename: str = ""
        if upload_url:
            # Convert HTTPS S3 URL to s3:// URI format
        ...
        return OpenAIFileObject(
            ...
            status="uploaded",
            bytes=_uploaded_object_size(derouter_params=derouter_params, raw_response=raw_response),
            object="file",
        )
    ...
    def transform_file_content_request(
        ...
        bucket_name, object_key = _validate_file_id_against_configured_buckets(
            s3_uri=s3_uri,
            configured_bucket_names=get_configured_s3_bucket_names(derouter_params),
            allow_legacy_cloud_file_ids=should_allow_legacy_cloud_file_ids(derouter_params),
        )

        # The shared file-content handler passes optional_params={}, so AWS
        # credentials/region arrive via derouter_params here (unlike the upload
        ...
        merged_params.update(derouter_params)
        ...
        derouter_params[S3_SIGNED_GET_HEADERS_PARAM] = self._sign_s3_get_request(
            api_base=url,
            aws_region_name=aws_region_name,
