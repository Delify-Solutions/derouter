"""
Resolve AWS Bedrock ``requestMetadata`` from DeRouter proxy identity and caller metadata.
...
sets ``derouter.bedrock_request_metadata_fields`` (``derouter_settings`` on the proxy).
...
_METADATA_PARAM_NAMES: Final[tuple[str, ...]] = ("metadata", "derouter_metadata")
...
def _allowed_fields() -> tuple[str, ...]:
    ...
    """
    configured: Final[object] = derouter.bedrock_request_metadata_fields
...
def _metadata_sources(derouter_params: Mapping[str, object] | None) -> tuple[Mapping[str, object], ...]:
    """``metadata`` on /v1/chat/completions, ``derouter_metadata`` on the DEROUTER_METADATA_ROUTES."""
    if derouter_params is None:
        return ()
    return tuple(
        source
        for name in _METADATA_PARAM_NAMES
        for source in (derouter_params.get(name),)
...
def resolve_bedrock_request_metadata(
    ...
    if not allowed_fields:
        return None
    sources: Final = _metadata_sources(derouter_params)
...
def bedrock_request_metadata_headers(
    ...
    if not bedrock_request_metadata_is_owned():
        return frozenset(), ()
    resolved: Final = resolve_bedrock_request_metadata(derouter_params)
    if resolved is None:
