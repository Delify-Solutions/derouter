...
lives in `derouter.llms.soniox.audio_transcription.handler`, because Soniox's
...
# Soniox-native kwargs the user can pass through `derouter.transcription(..., **kwargs)`
...
class SonioxAudioTranscriptionConfig(BaseAudioTranscriptionConfig):
    ...
    def get_supported_openai_params(self, model: str) -> list[OpenAIAudioTranscriptionOptionalParams]:
        ...
        # `response_format` is handled by DeRouter (Soniox doesn't support
        ...
        if not resolved_key:
            raise SonioxException(
                message=(
                    ...
                    "variable or pass api_key=... to derouter.transcription()."
                ),
                status_code=401,
                headers=None,
            )
