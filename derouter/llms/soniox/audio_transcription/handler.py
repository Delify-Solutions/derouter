...
`derouter.main.transcription()` routes Soniox requests directly to this
...
from derouter.llms.soniox.common_utils import (
...
class SonioxAudioTranscriptionHandler:
    ...
    def audio_transcriptions(
        ...
        ``derouter.transcription`` but is **not yet implemented** for the Soniox
        ...
        ``derouter.Router`` / ``num_retries`` mechanism for retry behaviour.
        """
    ...
    def _prepare(
        ...
        # response_format is handled by DeRouter post-processing, not Soniox.
    ...
    async def _async_audio_transcriptions(
        ...
        http_client: Final = (
            ...
            if isinstance(client, AsyncHTTPHandler)
                ...
                get_async_httpx_client(
                    llm_provider=derouter.LlmProviders.SONIOX,
                    params={"ssl_verify": derouter_params.get("ssl_verify", None)},
                )
            )
        )
