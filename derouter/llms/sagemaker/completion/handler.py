...
class SagemakerLLM(BaseAWSLLM):
    ...
    def completion(
        ...
        ## Load Config
        config: Final = derouter.SagemakerConfig.get_config()
        ...
        try:
            ...
            if "Inference Component Name header is required" in error_message:
                error_message += "\n pass in via `derouter.completion(..., model_id={InferenceComponentName})`"
        ...
        try:
            if client is None:
                client = get_async_httpx_client(
                    llm_provider=derouter.LlmProviders.SAGEMAKER
    ...
    async def async_streaming(
        ...
        if model_id is not None:  # Fixes https://github.com/Delify-Solutions/derouter/issues/8889
            prepared_request.headers.update({"X-Amzn-SageMaker-Inference-Component": model_id})

        if not prepared_request.body:
            raise ValueError("Prepared request body is empty")

        stream_logging_obj: Final[DeRouterLoggingObj] = logging_obj
    ...
    async def async_completion(
        ...
        timeout: Final = 300.0
        async_handler: Final = get_async_httpx_client(llm_provider=derouter.LlmProviders.SAGEMAKER)
