...
from derouter.types.llms.openai import AllMessageValues, ChatCompletionToolParam
...
class OpenAIChatCompletionsHandler(BaseTranslation):
    ...
    async def process_output_response(
        ...
        Args:
            response: DeRouter ModelResponse object
    ...
    async def process_output_streaming_response(
        ...
        Args:
            responses_so_far: List of DeRouter ModelResponseStream objects
            guardrail_to_apply: The guardrail instance to apply
            derouter_logging_obj: Optional logging object
            user_api_key_dict: User API key metadata to pass to guardrails
    ...
    async def _process_ended_stream(
        ...
        *,
        ...
        model_response: Final = cast(
            ModelResponse,
            stream_chunk_builder(chunks=responses_so_far, logging_obj=derouter_logging_obj),
        )
    ...
    def _accumulate_string_content_by_choice_index(
        ...
        for response in responses_so_far:
            for choice in response.choices:
                if isinstance(choice, derouter.StreamingChoices):
                    content = choice.delta.content
                elif isinstance(choice, derouter.Choices):
    ...
    def _has_text_content(self, response: Union["ModelResponse", "ModelResponseStream"]) -> bool:
        ...
        """
        from derouter.types.utils import ModelResponse, ModelResponseStream
        ...
        if isinstance(response, ModelResponse):
            ...
            for streaming_choice in response.choices:
                if isinstance(streaming_choice, derouter.StreamingChoices):
                    # Check for text content
    ...
    def _extract_output_text_images_and_tool_calls(
        ...
        elif isinstance(choice, derouter.StreamingChoices):
            content = choice.delta.content
