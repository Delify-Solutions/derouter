...
from derouter.llms.base_llm.base_utils import BaseLLMModelInfo
...
class OpenAIGPTConfig(BaseLLMModelInfo, BaseConfig):
    ...
    def is_openai_catalog_model(model: str) -> bool:
        ...
        return (
            model_for_check in derouter.open_ai_chat_completion_models
            or model_for_check in derouter.open_ai_text_completion_models
        )
    ...
    def _apply_common_transform_content_item(
        ...
        derouter_specific_params: Final = {"format"}
        if content_item.get("type") == "image_url":
            ...
            if isinstance(content_item["image_url"], str):
                ...
                new_image_url_obj: Final = ChatCompletionImageUrlObject(
                    **{k: v for k, v in content_item["image_url"].items() if k not in derouter_specific_params}
                )
            ...
            new_file_obj: Final = ChatCompletionFileObjectFile(
                **{k: v for k, v in file_obj.items() if k not in derouter_specific_params}
            )
    ...
    def _targets_openai_hosted_endpoint(
        ...
        if custom_llm_provider != "openai":
            return False
        resolved_api_base = api_base or derouter.api_base or os.getenv("OPENAI_BASE_URL") or os.getenv("OPENAI_API_BASE")
    ...
    def _should_preserve_cache_control_for_endpoint(
        ...
        (a DeRouter proxy, vLLM, an Anthropic-compatible gateway) via a custom
    ...
    def _flattened_tools_update_for_openai(
        ...
        provider: Final = derouter_params.get("custom_llm_provider")
        raw_api_base: Final = derouter_params.get("api_base")
    ...
    def transform_request(
        ...
        if not self._should_preserve_cache_control_for_endpoint(
            derouter_params.get("custom_llm_provider"), derouter_params.get("api_base")
        ):
        ...
        return {
            ...
            "messages": messages,
            **optional_params,
            **self._flattened_tools_update_for_openai(optional_params, derouter_params),
        }
    ...
    async def async_transform_request(
        ...
        if self.__class__._is_base_class:
            ...
            return self.transform_request(model, messages, optional_params, derouter_params, headers)
    ...
    def get_models(self, api_key: str | None = None, api_base: str | None = None) -> list[str]:
        ...
        response: Final = derouter.module_level_client.get(
    ...
    def get_api_key(api_key: str | None = None) -> str | None:
        return api_key or derouter.api_key or derouter.openai_key or get_secret_str("OPENAI_API_KEY")
...
class OpenAIUnknownModelConfig(OpenAIGPTConfig):
    """A model the openai provider does not recognize is typically a DeRouter proxy alias, so
...
class OpenAIChatCompletionStreamingHandler(BaseModelResponseIterator):
    def _map_reasoning_to_reasoning_content(self, choices: list) -> list:
        ...
        delta.reasoning, but DeRouter expects delta.reasoning_content.
