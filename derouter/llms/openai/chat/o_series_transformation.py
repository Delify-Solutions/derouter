...
Translations handled by DeRouter:
- modalities: image => drop param (if user opts in to dropping param)
- role: system ==> translate to role 'user'
- streaming => faked by DeRouter
...
class OpenAIOSeriesConfig(OpenAIGPTConfig):
                ...
                if temperature_value == 1:
                    ...
                    if derouter.drop_params is True or drop_params is True:
                        ...
                        raise derouter.utils.UnsupportedParamsError(
                            message=f"O-series models don't support temperature={temperature_value}. Only temperature=1 is supported. To drop unsupported openai params from the call, set `derouter.drop_params = True`",
                            status_code=400,
                        )
    ...
    def is_model_o_series_model(self, model: str) -> bool:
        ...
        return (
            ...
            and model[0] == "o"
            and model[1].isdigit()
            and model in derouter.open_ai_chat_completion_models
        )
