...
def _catalogue_declares_default_effort() -> bool:
    ...
    the one derouter gave before it existed. Scanning costs ~80us on the largest published map and
    only on the fallback path, which is noise beside the request it precedes.
    """
    return any(isinstance(entry, dict) and "default_reasoning_effort" in entry for entry in derouter.model_cost.values())
    ...
    def map_openai_params(
        ...
        if effective_effort == "xhigh":
            ...
            if not self._supports_reasoning_effort_level(model, effective_effort):
                if derouter.drop_params or drop_params:
        ...
        # Relevant issue: https://github.com/Delify-Solutions/derouter/issues/13381
        ################################################################
        if "max_tokens" in non_default_params:
            optional_params["max_completion_tokens"] = non_default_params.pop("max_tokens")
        ...
        if "temperature" in non_default_params:
            ...
            if temperature_value is not None:
                ...
                elif derouter.drop_params or drop_params:
                    pass
                else:
                    raise derouter.utils.UnsupportedParamsError(
                        message=(
