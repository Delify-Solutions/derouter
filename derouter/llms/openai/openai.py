...
from derouter.constants import DEFAULT_MAX_RETRIES
...
from derouter.derouter_core_utils.logging_utils import speech_request_body, track_llm_api_timing
...
class OpenAIConfig(BaseConfig):
    ...
    def get_supported_openai_params(self, model: str) -> list:
        ...
        elif derouter.openAIGPTAudioConfig.is_model_gpt_audio_model(model=model):
            return derouter.openAIGPTAudioConfig.get_supported_openai_params(model=model)
        else:
    ...
    def map_openai_params(
        ...
        if openaiOSeriesConfig.is_model_o_series_model(model=model):
            ...
            return derouter.openAIGPTAudioConfig.map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=model,
                drop_params=drop_params,
...
class OpenAIChatCompletion(BaseLLM, BaseOpenAILLM):
    ...
    def _get_openai_client(
        ...
        if client is None:
            ...
            self.set_cached_openai_client(
                ...
                client_initialization_params=client_initialization_params,
                client_type="openai",
                derouter_owned_client=self.owns_wrapped_http_client(http_client),
            )
            return _new_client
    ...
    async def make_openai_chat_completion_request(
        ...
        """
        Helper to:
        - call chat.completions.create.with_raw_response when derouter.return_response_headers is True
    ...
    async def _call_agentic_completion_hooks_openai(
        ...
        verbose_logger.debug("DeRouter.AgenticHooks: callbacks_count=%s", len(callbacks))
        tools: Final = optional_params.get("tools", [])
        # Avoid logging full tools payloads; they may contain sensitive parameters
        verbose_logger.debug(
            "DeRouter.AgenticHooks: tools_count=%s",
            len(tools) if isinstance(tools, list) else 1 if tools else 0,
        )
        # Get custom_llm_provider from derouter_params
        custom_llm_provider: Final = derouter_params.get("custom_llm_provider", "openai")
            ...
            try:
                if isinstance(callback, CustomLogger):
                    ...
                    if should_run:
                        ...
                        kwargs_with_provider = derouter_params.copy() if derouter_params else {}
                ...
                verbose_logger.exception(
                    "DeRouter.AgenticHookError: Exception in agentic completion hooks for OpenAI: %s", e
                )
        ...
        try:
            ...
            for _ in range(2):  # if call fails due to alternating messages, retry with reformatted message
                try:
                    ...
                    ## check if body contains unprocessable params - related issue https://github.com/Delify-Solutions/derouter/issues/4800
                    ...
                    if (
                        ...
                        derouter.remove_index_from_tool_calls(messages=messages)
                    else:
                        raise e
    ...
    def embedding(
        ...
        try:
            ...
            max_retries = max_retries or derouter.DEFAULT_MAX_RETRIES
            if not isinstance(max_retries, int):
                raise OpenAIError(status_code=422, message="max retries must be an int")
            ## LOGGING
            logging_obj.pre_call(
...
class OpenAIBatchesAPI(BaseLLM):
    ...
    async def acreate_batch(
        ...
        return DeRouterBatch.model_validate(response.model_dump())
    ...
    def create_batch(
        ...
        organization: str | None,
        client: OpenAI | AsyncOpenAI | None = None,
    ) -> DeRouterBatch | Coroutine[None, None, DeRouterBatch]:
        openai_client: Final[OpenAI | AsyncOpenAI | None] = self.get_openai_client(
            api_key=api_key,
            api_base=api_base,
            timeout=timeout,
...
class OpenAIAssistantsAPI(BaseLLM):
    ...
    def create_thread(
        ...
        Here's an example:
        ```
        from derouter.llms.openai.openai import OpenAIAssistantsAPI, MessageData
