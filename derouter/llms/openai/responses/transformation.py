...
from derouter.llms.openai.chat.gpt_5_transformation import is_gpt_reasoning_series_name
from derouter.responses.derouter_completion_transformation.custom_tools import TOOL_CALL_ITEM_ID_PREFIX_BY_TYPE
...
class _ReasoningSupportEntry(BaseModel):
    derouter_provider: str | None = None
...
def _bundled_openai_reasoning_models() -> frozenset[str]:
    ...
    return frozenset(
        ...
        if entry.derouter_provider == LlmProviders.OPENAI.value and entry.supports_reasoning is True
    )
...
class OpenAIResponsesAPIConfig(BaseResponsesAPIConfig):
    ...
    def _supports_reasoning_effort_none(model: str) -> bool:
        ...
        from derouter.utils import supports_none_reasoning_effort
    ...
    def map_openai_params(
        ...
        if (
            ...
            if drop_params or derouter.drop_params:
                ...
                raise derouter.UnsupportedParamsError(
                    message=(
                        ...
                        "To drop unsupported params set `derouter.drop_params = True`"
                    ),
                    status_code=400,
                )
        ...
        if self._is_gpt_5_model(model=model):
            ...
            if temperature is not None and temperature != 1:
                ...
                elif drop_params or derouter.drop_params:
    ...
    def _prepared_input_and_tools(
        ...
        sanitized_tools: Final = self._flatten_tool_schema_combinators_for_openai(
            model=model, tools=object_schema_tools, derouter_params=derouter_params
        )
    ...
    def _flatten_tool_schema_combinators_for_openai(
        ...
        if tools is None or self.custom_llm_provider not in _PROVIDERS_WITH_COMBINATOR_REJECTING_VALIDATOR:
            return tools
        gate_model: Final = self._combinator_gate_model(model=model, derouter_params=derouter_params)
    ...
    @staticmethod
    def _combinator_gate_model(model: str, derouter_params: GenericDeRouterParams) -> str:
        model_info: Final[object] = getattr(derouter_params, "model_info", None)
    ...
    def _handle_reasoning_item(self, item: dict[str, Any]) -> dict[str, Any]:
        ...
        Issue: https://github.com/Delify-Solutions/derouter/issues/13484
    ...
    def validate_environment(self, headers: dict, model: str, derouter_params: GenericDeRouterParams | None) -> dict:
        ...
        workload_identity_config: Final = (
            resolve_openai_workload_identity_config(api_key=api_key, api_base=derouter_params.api_base)
    ...
    @staticmethod
    def get_event_model_class(event_type: str) -> type[BaseDeRouterOpenAIResponseObject]:
        """
    ...
    def should_fake_stream(
        ...
        if model is not None:
            try:
                if (
                    derouter.utils.supports_native_streaming(
                        model=model,
