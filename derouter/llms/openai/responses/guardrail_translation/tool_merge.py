...
def _merged_original(
    ...
    if not guardrailed_group:
        return ()
    return tuple(
        DeRouterCompletionResponsesConfig.transform_chat_completion_tool_params_to_responses_api_tools(guardrailed_group)
    )
...
def merge_guardrailed_tools(
    ...
    appended: Final = DeRouterCompletionResponsesConfig.transform_chat_completion_tool_params_to_responses_api_tools(
