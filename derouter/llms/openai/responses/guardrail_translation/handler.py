...
from derouter.llms.base_llm.guardrail_translation.base_translation import (
    BaseTranslation,
    StreamingScanKey,
    StreamTransformSink,
)
...
from derouter.llms.openai.responses.guardrail_translation.tool_merge import merge_guardrailed_tools
...
def _rewritten_input_item(item: Mapping[str, object], rewritten: object) -> Mapping[str, object] | None:
    ...
    converted_items, _ = DeRouterResponsesTransformationHandler().convert_chat_completion_messages_to_responses_api(
...
def _input_item_provenance(
    ...
    solo_conversions: Final = tuple(
        DeRouterCompletionResponsesConfig.transform_responses_api_input_to_messages(
...
def _patch_or_convert_request_fields(
    ...
    input_items, converted_instructions = (
        DeRouterResponsesTransformationHandler().convert_chat_completion_messages_to_responses_api(structured_messages)
    )
...
class OpenAIResponsesHandler(BaseTranslation):
    ...
    async def process_input_messages(
        ...
        flattened_tool_groups: Final = tuple(
            form.chat_tools for form in DeRouterCompletionResponsesConfig.responses_tools_to_chat_forms(original_tools)
        )
    ...
    async def process_output_response(
        ...
        Args:
            response: DeRouter ResponsesAPIResponse object
            guardrail_to_apply: The guardrail instance to apply
            derouter_logging_obj: Optional logging object
        ...
        if texts_to_check or tool_calls_to_check:
            ...
            if "derouter_metadata" not in request_data:
                user_metadata: Final = self.transform_user_api_key_dict_to_metadata(user_api_key_dict)
                if user_metadata:
                    request_data["derouter_metadata"] = user_metadata
    ...
    async def process_output_streaming_response(
        ...
        # Accept both plain dicts and Pydantic models (BaseDeRouterOpenAIResponseObject
        ...
        if string_so_far:
            ...
            if deliver_ended_stream_rewrites and fallback_texts and tuple(fallback_texts) != (string_so_far,):
                from derouter.proxy.policy_engine.pipeline_executor import UndeliverableStreamRewrite
        ...
        if isinstance(output_item, OutputFunctionToolCall) or (
            ...
            if tool_calls_to_check is not None:
                ...
                try:
                    ...
                    tool_call_dict = DeRouterCompletionResponsesConfig.convert_response_function_tool_call_to_chat_completion_tool_call(
    ...
    def _standalone_block_events(exc: "ModifyResponseException") -> Sequence[ResponsesAPIStreamingResponse]:
        from derouter.responses.streaming_iterator import build_synthetic_response_events
        ...
        return (
            *_open_item_closing_events(responses_so_far),
            ...
            ContentPartAddedEvent(
                ...
                output_index=output_index,
                content_index=0,
                part=BaseDeRouterOpenAIResponseObject.model_validate(part),
            ),
            OutputTextDeltaEvent(
    ...
    if open_item.item_type != "message":
        return (
            OutputItemDoneEvent(
                ...
                item=BaseDeRouterOpenAIResponseObject.model_validate(
