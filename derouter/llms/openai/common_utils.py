...
class BaseOpenAILLM:
    ...
    def get_cached_openai_client(
        ...
        _cached_client: Final = derouter.in_memory_llm_clients_cache.get_cache(_cache_key)
        return _cached_client

    @staticmethod
    def owns_wrapped_http_client(http_client: httpx.Client | httpx.AsyncClient | None) -> bool:
        """Whether derouter may close an SDK client built around ``http_client``.

        ``_get_async_http_client`` / ``_get_sync_http_client`` hand back
        ``derouter.aclient_session`` / ``derouter.client_session`` when the caller
        ...
        SDK built its own http client, which derouter does own.
        """
        if http_client is None:
            return True
        return http_client is not derouter.aclient_session and http_client is not derouter.client_session
    ...
    def set_cached_openai_client(
        ...
        derouter_owned_client: bool = False,
    ):
        """Stores the OpenAI client in the in-memory cache for _DEFAULT_TTL_FOR_HTTPX_CLIENTS SECONDS

        ``derouter_owned_client`` says derouter built this client, so the cache may close it once it
        is evicted. A client the caller supplied stays open, since derouter does not own it.
        """
        _cache_key: Final = BaseOpenAILLM.get_openai_client_cache_key(
            client_initialization_params=client_initialization_params,
            client_type=client_type,
        ...
        derouter.in_memory_llm_clients_cache.set_cache(
            ...
            derouter_owned_client=derouter_owned_client,
        )
    ...
    def get_openai_client_cache_key(client_initialization_params: dict, client_type: Literal["openai", "azure"]) -> str:
        ...
        DEROUTER_CLIENT_SPECIFIC_PARAMS: Final = (
            "timeout",
            "max_retries",
            "organization",
            "api_base",
        ...
        openai_client_fields: Final = (
            ...
            + DEROUTER_CLIENT_SPECIFIC_PARAMS
        )
    ...
    def _get_async_http_client(
        ...
        if derouter.aclient_session is not None:
            return derouter.aclient_session

        if getattr(derouter, "network_mock", False):
            from derouter.llms.custom_httpx.mock_transport import MockOpenAITransport
    ...
    def _get_sync_http_client() -> httpx.Client | None:
        if derouter.client_session is not None:
            return derouter.client_session
...
def get_openai_credentials(
    ...
    """Resolve OpenAI credentials from params, derouter globals, and env vars."""
    ...
    resolved_organization = organization or derouter.organization or os.getenv("OPENAI_ORGANIZATION", None) or None
    resolved_api_key: Final = api_key or derouter.api_key or derouter.openai_key or os.getenv("OPENAI_API_KEY")
