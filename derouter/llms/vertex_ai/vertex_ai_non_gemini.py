...
from derouter.llms.bedrock.common_utils import ModelResponseIterator
from derouter.llms.custom_httpx.http_handler import _DEFAULT_TTL_FOR_HTTPX_CLIENTS
from derouter.types.llms.vertex_ai import *
from derouter.utils import CustomStreamWrapper, ModelResponse, Usage
...
def _get_client_from_cache(client_cache_key: str):
    return derouter.in_memory_llm_clients_cache.get_cache(client_cache_key)
...
def completion(
    ...
    try:
        ...
        raise VertexAIError(
            status_code=400,
            message="vertexai import failed please run `pip install google-cloud-aiplatform`. This is required for the 'vertex_ai/' route on DeRouter",
        )
    ...
    try:
        ...
        ## Load Config
        config: Final = derouter.VertexAIConfig.get_config()
        ...
        fake_stream = False
        if model in derouter.vertex_language_models or model in derouter.vertex_vision_models:
            llm_model: Any = _vertex_llm_model_object or GenerativeModel(model)
            mode = "vision"
            request_str += f"llm_model = GenerativeModel({model})\n"
        elif model in derouter.vertex_chat_models:
        ...
        elif model in derouter.vertex_text_models:
        ...
        elif model in derouter.vertex_code_text_models:
            llm_model = _vertex_llm_model_object or CodeGenerationModel.from_pretrained(model)
            mode = "text"
            request_str += f"llm_model = CodeGenerationModel.from_pretrained({model})\n"
            fake_stream = True
        elif model in derouter.vertex_code_chat_models:  # vertex_code_llm_models
        ...
        ## RESPONSE OBJECT
        if isinstance(completion_response, derouter.Message):
        ...
        ## CALCULATING USAGE
        if model in derouter.vertex_language_models and response_obj is not None:
            ...
            # init prompt tokens
            # this block attempts to get usage from response_obj if it exists, if not it uses the derouter token counter
        ...
        if isinstance(e, VertexAIError):
            raise e
        raise derouter.APIConnectionError(message=str(e), llm_provider="vertex_ai", model=model)
