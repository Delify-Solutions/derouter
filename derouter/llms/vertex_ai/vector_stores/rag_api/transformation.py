...
class VertexVectorStoreConfig(BaseVectorStoreConfig, VertexBase):
    ...
    def get_auth_credentials(self, derouter_params: Mapping[str, object]) -> BaseVectorStoreAuthCredentials:
        ...
        vertex_credentials: Final = self.get_vertex_ai_credentials(dict(derouter_params))
        vertex_project: Final = self.get_vertex_ai_project(dict(derouter_params))
    ...
    def validate_environment(
        self, headers: dict[str, str], derouter_params: GenericDeRouterParams | None
    ) -> dict[str, str]:
        """
        ...
        auth_headers: Final = self.get_auth_credentials(derouter_params.model_dump())
    ...
    def get_complete_url(
        ...
        """
        vertex_location: Final = self.get_vertex_ai_location(derouter_params)
        vertex_project: Final = self.get_vertex_ai_project(derouter_params)
    ...
    def transform_search_vector_store_request(
        ...
        #########################################################
        derouter_logging_obj.model_call_details["query"] = query
    ...
    def transform_search_vector_store_response(
        ...
        try:
            ...
            query_view: Final[_SearchQueryView] = {"query": derouter_logging_obj.model_call_details.get("query", "")}
    ...
    def transform_create_vector_store_request(
        ...
        request_body: Final[dict[str, object]] = {
            key: value
            for key, value in (
                ("display_name", vector_store_create_optional_params.get("name", "derouter-vector-store")),
                ("description", "Vector store created via DeRouter"),
                ("labels", metadata),
            )
            if value is not None
        }
