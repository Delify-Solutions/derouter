...
import derouter
from derouter.derouter_core_utils.prompt_templates.image_handling import RemoteMedia, inline_remote_image_urls
...
class VertexAIAnthropicConfig(AnthropicConfig):
    ...
    def is_supported_model(cls, model: str, custom_llm_provider: str) -> bool:
        ...
        if custom_llm_provider != "vertex_ai" and custom_llm_provider != "vertex_ai_beta":
            return False
        if "claude" in model.lower() or model in derouter.vertex_anthropic_models:
            return True
        return False
