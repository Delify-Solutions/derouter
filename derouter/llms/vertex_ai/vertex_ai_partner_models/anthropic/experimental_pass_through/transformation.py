...
class VertexAIPartnerModelsAnthropicMessagesConfig(AnthropicMessagesConfig, VertexBase):
    ...
    def validate_anthropic_messages_environment(
        ...
        """
        # Work on a local copy — router shallow-copies derouter_params so the caller's
        # headers dict may be the shared deployment extra_headers object.
        headers = dict(headers)
        vertex_ai_project: Final = VertexBase.safe_get_vertex_ai_project(derouter_params)
        vertex_ai_location: Final = VertexBase.safe_get_vertex_ai_location(derouter_params)

        vertex_credentials: Final = VertexBase.safe_get_vertex_ai_credentials(derouter_params)
