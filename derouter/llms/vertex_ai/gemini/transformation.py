...
from derouter.derouter_core_utils.prompt_templates.image_handling import RemoteMedia, async_inline_remote_media
from derouter.llms.custom_httpx.http_handler import AsyncHTTPHandler, HTTPHandler
from derouter.llms.vertex_ai.common_utils import pop_vertex_request_labels
from derouter.types.files import (
...
from derouter.types.utils import GenericImageParsingChunk, LlmProviders
...
# from derouter's HTTP stack apply (see Greptile review on PR #27278).
    ...
    try:
        ...
        if "gs://" in image_url:
            ...
            if explicit_gcs_format:
                ...
                # known MIME aliases; skip derouter's file-type registry.
...
def _gemini_convert_messages_with_history(
    ...
    if derouter_params:
        vertex_project = derouter_params.get("vertex_project") or derouter_params.get("vertex_ai_project")
        vertex_credentials = derouter_params.get("vertex_credentials") or derouter_params.get("vertex_ai_credentials")
    ...
    try:
        while msg_i < len(messages):
            ...
            if user_content:
                ...
                if has_text_in_content is False:
                    verbose_logger.warning(
                        "No text in user content. Adding a blank text to user content, to ensure Gemini doesn't fail the request. Relevant Issue - https://github.com/Delify-Solutions/derouter/issues/5515"
                    )
                    user_content.append(
                        PartType(text=" ")
            ...
            if msg_i == init_msg_i:  # prevent infinite loops
                raise Exception(
                    f"Invalid Message passed in - {messages[msg_i]}. File an issue https://github.com/Delify-Solutions/derouter/issues"
                )
        if len(tool_call_responses) > 0:
            contents.append(ContentType(role="user", parts=tool_call_responses))
        ...
        if len(contents) == 0:
            verbose_logger.warning("""
                ...
                but DeRouter does not check for missing messages.
                Setting an empty content to prevent an 400 error.
                Relevant Issue - https://github.com/Delify-Solutions/derouter/issues/9733
                """)
...
# Keys that DeRouter consumes internally and must never be forwarded to the
_DEROUTER_INTERNAL_EXTRA_BODY_KEYS: Final[frozenset] = frozenset({"cache", "tags"})
...
def _pop_and_merge_extra_body(data: RequestBody, optional_params: dict) -> None:
    ...
    if extra_body is not None:
        ...
        for k, v in extra_body.items():
            if k in _DEROUTER_INTERNAL_EXTRA_BODY_KEYS:
                continue
...
def _transform_request_body(
    ...
    # Check for any 'derouter_param_*' set during optional param mapping

    remove_keys: Final = []
    for k, v in optional_params.items():
        if k.startswith("derouter_param_"):
            derouter_params.update({k: v})
    ...
    try:
        if custom_llm_provider == "gemini":
            content = derouter.GoogleAIStudioGeminiConfig()._transform_messages(
                messages=messages, model=model, derouter_params=derouter_params
            )
        else:
            content = derouter.VertexGeminiConfig()._transform_messages(
                messages=messages, model=model, derouter_params=derouter_params
            )
        ...
        labels: Final = pop_vertex_request_labels(optional_params, derouter_params)
        ...
        can_send_cache_incompatible_fields: Final = cached_content is None or derouter.modify_params is False
...
def _transform_system_message(
    ...
    if len(system_content_blocks) > 0:
        ...
        # Relevant Issue - https://github.com/Delify-Solutions/derouter/issues/13769
        #########################################################
        if len(messages) == 0:
