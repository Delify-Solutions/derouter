...
from derouter.derouter_core_utils.json_fragment_accumulator import JSONFragmentAccumulator
...
from derouter.types.llms.gemini import BidiGenerateContentServerMessage
...
if TYPE_CHECKING:
    ...
    from derouter.types.utils import ModelResponseStream, StreamingChoices
...
class VertexGeminiConfig(VertexAIBaseConfig, BaseConfig):
    ...
    def get_json_schema_from_pydantic_object(self, response_format: type["BaseModel"] | dict | None) -> dict | None:
        ...
        See: https://github.com/Delify-Solutions/derouter/issues/21014
        """
    ...
    def map_tool_choice_values(self, model: str, tool_choice: str | dict) -> ToolConfig | None:
        if tool_choice == "none":
            ...
            raise derouter.utils.UnsupportedParamsError(
                message=f"VertexAI doesn't support tool_choice={tool_choice}. Supported tool_choice values=['auto', 'required', json object]. To drop it from the call, set `derouter.drop_params = True.",
                status_code=400,
            )
    ...
    def _transform_computer_use_config(self, computer_use_config: dict) -> dict:
        ...
        Args:
            computer_use_config: The computer use configuration from DeRouter
    ...
    def _extract_google_maps_retrieval_config(self, google_maps_config: dict) -> tuple[dict, dict | None]:
        ...
        Args:
            google_maps_config: The googleMaps tool configuration from DeRouter
    ...
    def _resolve_search_tool_conflict(
        ...
        Ref: https://github.com/Delify-Solutions/derouter/issues/23337
    ...
    def _map_function(self, value: list[dict], optional_params: dict) -> list[Tools]:
        ...
        for tool in value:
            ...
            # Handle tools with 'type' field (OpenAI spec compliance) Ignore this field -> https://github.com/Delify-Solutions/derouter/issues/14644#issuecomment-3342061838
            elif "type" in tool:
            ...
            if tool_name and (
                ...
                verbose_logger.warning(
                    "Invalid tool={}. Use `derouter.set_verbose` or `derouter --detailed_debug` to see raw request."
                )
        ...
        if model and VertexGeminiConfig._is_gemini_3_or_newer(model):
            if thinking_enabled:
                if thinking_budget == 0:
                    ...
                    if derouter.enable_gemini_default_thinking_level_low is True:
        ...
        if num_function_declarations > 1:
            raise derouter.utils.UnsupportedParamsError(
                message=(
                    ...
                    "`parallel_tool_calls=True`. If you want to drop this param, set `derouter.drop_params = True` or pass in `(.., drop_params=True)` in the requst - https://router.delify.vn/docs/completion/drop_params"
                ),
                status_code=400,
            )
    ...
    def map_openai_params(
        ...
        for param, value in non_default_params.items():
            if param == "temperature":
                ...
                optional_params["include_server_side_tool_invocations"] = True
        if derouter.vertex_ai_safety_settings is not None:
            optional_params["safety_settings"] = derouter.vertex_ai_safety_settings
    ...
    def get_finish_reason_mapping() -> dict[str, OpenAIChatCompletionFinishReason]:
        ...
        """
        from derouter.derouter_core_utils.core_helpers import _FINISH_REASON_MAP
    ...
    def is_candidate_token_count_inclusive(usage_metadata: UsageMetadata) -> bool:
        ...
        Addresses - https://github.com/Delify-Solutions/derouter/pull/10141#discussion_r2052272035
        """
    ...
    def _response_has_search_grounding(
        ...
        https://github.com/Delify-Solutions/derouter/discussions/33198
        """
    ...
    def _calculate_usage(
        ...
        ## See: https://github.com/Delify-Solutions/derouter/issues/18750
    ...
    def transform_response(
        ...
        try:
            ...
            raise VertexAIError(
                message=f"Error converting to valid response block={e}. File an issue if derouter error - https://github.com/Delify-Solutions/derouter/issues",
                status_code=422,
...
async def make_call(
    ...
    if client is None:
        client = get_async_httpx_client(
            llm_provider=derouter.LlmProviders.VERTEX_AI,
        )
...
class VertexLLM(VertexBase):
    ...
    async def async_completion(
        ...
        if client is None or not isinstance(client, AsyncHTTPHandler):
            client = get_async_httpx_client(params=_async_client_params, llm_provider=derouter.LlmProviders.VERTEX_AI)
        else:
            client = client
        ## LOGGING
        logging_obj.pre_call(
...
class ModelResponseIterator:
    ...
    def handle_valid_json_chunk(self, chunk: str) -> Optional["ModelResponseStream"]:
        ...
        try:
            ...
            # See: https://github.com/Delify-Solutions/derouter/issues/16562
    ...
    def handle_accumulated_json_chunk(self, chunk: str, is_final: bool = False) -> Optional["ModelResponseStream"]:
        message: Final = (derouter.CustomStreamWrapper._strip_sse_data_from_chunk(chunk) or "").replace("\n\n", "")
    ...
    def _common_chunk_parsing_logic(self, chunk: str) -> Optional["ModelResponseStream"]:
        try:
            chunk = derouter.CustomStreamWrapper._strip_sse_data_from_chunk(chunk) or ""
            if len(chunk) > 0:
                """
