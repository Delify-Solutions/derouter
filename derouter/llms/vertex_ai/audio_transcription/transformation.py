...
from derouter.llms.vertex_ai.common_utils import VertexAIError, validate_vertex_location
from derouter.llms.vertex_ai.vertex_llm_base import VertexBase
from derouter.types.llms.openai import (
    AllMessageValues,
    OpenAIAudioTranscriptionOptionalParams,
)
from derouter.types.llms.vertex_ai_speech_to_text import (
...
from derouter.types.utils import FileTypes, TranscriptionResponse
...
class VertexAIAudioTranscriptionConfig(BaseAudioTranscriptionConfig, VertexBase):
    ...
    def get_complete_url(
        ...
        location: Final = validate_vertex_transcription_location(
            self.safe_get_vertex_ai_location(derouter_params), default_location=DEFAULT_SPEECH_TO_TEXT_LOCATION
        )
        project_id: Final = validate_vertex_transcription_project_id(
            self.safe_get_vertex_ai_project(derouter_params) or self._resolve_project_id_from_credentials(derouter_params)
        )
    ...
    def _resolve_project_id_from_credentials(self, derouter_params: dict) -> str:
