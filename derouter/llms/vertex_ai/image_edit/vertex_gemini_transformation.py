...
from derouter.llms.vertex_ai.common_utils import get_vertex_base_url
from derouter.llms.vertex_ai.gemini.vertex_and_google_ai_studio_gemini import VertexLLM
...
from derouter.types.utils import FileTypes, ImageObject, ImageResponse, OpenAIImage
...
class VertexAIGeminiImageEditConfig(BaseImageEditConfig, VertexLLM):
    ...
    def _resolve_vertex_project(self) -> str | None:
        return (
            ...
            or getattr(derouter, "vertex_project", None)
    ...
    def _resolve_vertex_location(self) -> str | None:
        return (
            ...
            or getattr(derouter, "vertex_location", None)
    ...
    def _resolve_vertex_credentials(self) -> str | None:
        return (
            ...
            or getattr(derouter, "vertex_credentials", None)
    ...
    def validate_environment(
        ...
        _api_base: Final = derouter_params.get("api_base") or api_base
        if _api_base is not None:
            return headers

        # First check derouter_params (where vertex_ai_project/vertex_ai_credentials are passed)
        # then fall back to environment variables and other sources
        vertex_project: Final = self.safe_get_vertex_ai_project(derouter_params) or self._resolve_vertex_project()
        vertex_credentials = self.safe_get_vertex_ai_credentials(derouter_params) or self._resolve_vertex_credentials()
    ...
    def get_complete_url(
        ...
        # First check derouter_params (where vertex_ai_project/vertex_ai_location are passed)
        # then fall back to environment variables and other sources
        vertex_project: Final = self.safe_get_vertex_ai_project(derouter_params) or self._resolve_vertex_project()
        vertex_location: Final = self.safe_get_vertex_ai_location(derouter_params) or self._resolve_vertex_location()
