...
from derouter.types.llms.openai import BatchJobStatus, CreateBatchRequest
...
class VertexAIBatchTransformation:
    ...
    def transform_openai_batch_request_to_vertex_ai_batch_request(
        ...
        Transforms OpenAI Batch requests to Vertex AI Batch requests
        """
        request_display_name: Final = f"derouter-vertex-batch-{uuid.uuid4()}"
    ...
    def _get_gcs_uri_prefix_from_file(cls, input_file_id: str) -> str:
        ...
        Example:
        input_file_id: "gs://derouter-testing-bucket/vtx_batch.jsonl"
        returns: "gs://derouter-testing-bucket"

        input_file_id: "gs://derouter-testing-bucket/batches/vtx_batch.jsonl"
        returns: "gs://derouter-testing-bucket/batches"
        """
    ...
    def _get_model_from_gcs_file(cls, gcs_file_uri: str) -> str:
        ...
        When files are uploaded using DeRouter (/v1/files), the model is stored in the gcs file uri

        Why?
        - Because Vertex Requires the `model` param in create batch jobs request, but OpenAI does not require this


        gcs_file_uri format: gs://derouter-testing-bucket/derouter-vertex-files/publishers/google/models/gemini-1.5-flash-001/e9412502-2c91-42a6-8e61-f5c294cc0fc8
        ...
        if model is None:
            raise VertexAIError(
                ...
                message=(
                    ...
                    "Either upload the input file through DeRouter (POST /v1/files with "
    ...
    def _parse_model_from_gcs_file(cls, gcs_file_uri: str) -> str | None:
        ...
        override the model path DeRouter appended after it.
        """
    ...
    def is_unmanaged_gcs_batch_input_file_id(cls, input_file_id: str | None) -> bool:
        ...
        DeRouter-managed unified file id) with a `publishers/` model path that
