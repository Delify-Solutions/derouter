...
from derouter.types.llms.vertex_ai import VERTEX_CREDENTIALS_TYPES, VertexPartnerProvider
...
GOOGLE_IMPORT_ERROR_MESSAGE: Final = (
    "Google Cloud SDK not found. Install it with: pip install 'derouter[google]' or pip install google-cloud-aiplatform"
)
...
class VertexBase:
    ...
    def get_vertex_region(vertex_region: str | None, model: str) -> str:
        ...
        model_info: Final = derouter.model_cost.get(model_key, {})
        ...
        if credentials is not None:
            if isinstance(credentials, str):
                _is_path: Final = os.path.exists(
                    credentials
                )  # credentials is from server config (derouter_params), not user input
            ...
            if "type" in json_obj and json_obj["type"] == "external_account":
                ...
                if isinstance(environment_id, str) and "aws" in environment_id:
                    ...
                    from derouter.llms.vertex_ai.vertex_ai_aws_wif import (
                        VertexAIAwsWifAuth,
                    )
    ...
    def _check_custom_proxy(
        ...
        """
        for cloudflare ai gateway - https://github.com/Delify-Solutions/derouter/issues/4317
    ...
    @staticmethod
    def get_vertex_ai_project(derouter_params: dict) -> str | None:
        return (
            derouter_params.pop("vertex_project", None)
            or derouter_params.pop("vertex_ai_project", None)
            or derouter.vertex_project
            or get_secret_str("VERTEXAI_PROJECT")
        )

    @staticmethod
    def get_vertex_ai_credentials(derouter_params: dict) -> str | None:
        return (
            derouter_params.pop("vertex_credentials", None)
            or derouter_params.pop("vertex_ai_credentials", None)
            or get_secret_str("VERTEXAI_CREDENTIALS")
        )

    @staticmethod
    def get_vertex_ai_location(derouter_params: dict) -> str | None:
        return (
            derouter_params.pop("vertex_location", None)
            or derouter_params.pop("vertex_ai_location", None)
    ...
    @staticmethod
    def safe_get_vertex_ai_project(derouter_params: dict) -> str | None:
        """
        Safely get Vertex AI project without mutating the derouter_params dict.

        Unlike get_vertex_ai_project(), this does NOT pop values from the dict,
        making it safe to call multiple times with the same derouter_params.

        Args:
            derouter_params: Dictionary containing Vertex AI parameters

        Returns:
            Vertex AI project ID or None
        """
        return (
            derouter_params.get("vertex_project")
            or derouter_params.get("vertex_ai_project")
            or derouter.vertex_project
            or get_secret_str("VERTEXAI_PROJECT")
        )

    @staticmethod
    def safe_get_vertex_ai_credentials(derouter_params: dict) -> str | None:
        """
        Safely get Vertex AI credentials without mutating the derouter_params dict.
        ...
        """
        return (
            derouter_params.get("vertex_credentials")
            or derouter_params.get("vertex_ai_credentials")
            or get_secret_str("VERTEXAI_CREDENTIALS")
        )
    ...
    @staticmethod
    def safe_get_vertex_ai_location(derouter_params: Mapping[str, object]) -> str | None:
        """
        Safely get Vertex AI location without mutating the derouter_params dict.
        ...
        """
        return (
            VertexBase.explicit_vertex_ai_location(derouter_params)
            or derouter.vertex_location
            or get_secret_str("VERTEXAI_LOCATION")
            or get_secret_str("VERTEX_LOCATION")
        )
