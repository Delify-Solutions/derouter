...
from derouter.types.llms.vertex_ai import (
    VERTEX_AI_PROVIDER_METADATA_FIELDS,
    PartType,
    Schema,
)
from derouter.types.utils import TokenCountResponse
from derouter.utils import supports_response_schema, supports_system_messages
...
def get_vertex_ai_lyria_model_info(model: str) -> VertexAILyriaModelInfo | None:
    ...
    runtime_model_info: Final = _validate_vertex_ai_lyria_model_info(derouter.model_cost.get(model_key))
...
def get_vertex_ai_lyria_generation_cost(model: str) -> float | None:
    ...
    runtime_cost: Final = _vertex_ai_lyria_generation_cost(
        _validate_vertex_ai_lyria_model_info(derouter.model_cost.get(model_key))
    )
...
def redact_vertex_ai_metadata_from_derouter_params(model_call_details: dict) -> None:
    """
    success_handler() merges response._hidden_params into
    derouter_params.metadata['hidden_params'] before redaction runs, so the Vertex
...
def vertex_request_labels_from_derouter_params(
    derouter_params: dict | None,
) -> dict[str, str] | None:
    """
    Build Vertex/GCP billing labels from DeRouter user metadata on ``derouter_params``:
    ``metadata`` (``completion(..., metadata=...)``) or ``derouter_metadata``,
...
def pop_vertex_request_labels(
    ...
    ``derouter_params["metadata"]`` / ``derouter_params["derouter_metadata"]``
    ...
    if not labels:
        labels = vertex_request_labels_from_derouter_params(derouter_params)
...
def get_vertex_ai_model_route(model: str, derouter_params: dict | None = None) -> VertexAIModelRoute:
    """
    ...
    Args:
        ...
        derouter_params: Optional derouter parameters dict that may contain base_model for routing
    ...
    # Check base_model in derouter_params for gemini override
    if derouter_params and derouter_params.get("base_model") is not None:
        if "gemini" in derouter_params["base_model"]:
    ...
    if model.isdigit() and derouter_params and derouter_params.get("api_base"):
        return VertexAIModelRoute.GEMINI
...
def get_supports_system_message(
    ...
    try:
        ...
        if derouter.VertexGeminiConfig._is_model_gemini_spec_model(model):
            supports_system_message = True
    except Exception as e:
        verbose_logger.warning(
            "Unable to identify if system message supported. Defaulting to 'False'. Received error message - %s\nAdd it here - https://github.com/Delify-Solutions/derouter/blob/main/model_prices_and_context_window.json",
            e,
        )
...
def _get_embedding_url(
    ...
    try:
        model_info: Final = derouter.get_model_info(
...
def _get_vertex_url(
    ...
    model = derouter.VertexGeminiConfig.get_model_for_vertex_ai_url(model=model)
...
def _get_gemini_url(
    ...
    if mode == "chat":
        ...
        raise ValueError(
            "DeRouter's `gemini/` route does not support image generation yet. Let us know if you need this feature by opening an issue at https://github.com/Delify-Solutions/derouter/issues"
        )
    else:
...
def _check_text_in_content(parts: list[PartType]) -> bool:
    ...
    check that user_content has 'text' parameter.
        ...
        - Relevant Issue: https://github.com/Delify-Solutions/derouter/issues/5515
    """
...
def _filter_anyof_fields(schema_dict: dict[str, object]) -> dict[str, object]:
    """
    When anyof is present, only keep the anyof field and its contents - otherwise VertexAI will throw an error - https://github.com/Delify-Solutions/derouter/issues/11164
...
def construct_target_url(
    ...
    Handle cachedContent scenario - https://github.com/Delify-Solutions/derouter/issues/5460
...
class VertexAITokenCounter(BaseTokenCounter):
    ...
    async def count_tokens(
        ...
        count_tokens_params_request: Final = copy.deepcopy(deployment.get("derouter_params", {}))
        ...
        if VertexAIPartnerModels.is_vertex_partner_model(model_to_use):
            ...
            # Extract vertex-specific params from derouter_params
            partner_derouter_params: Final[dict[str, object]] = count_tokens_params_request
            vertex_project = partner_derouter_params.get("vertex_project") or partner_derouter_params.get(
                "vertex_ai_project"
            )

            vertex_location = partner_derouter_params.get("vertex_location") or partner_derouter_params.get(
                "vertex_ai_location"
            )
            ...
            vertex_location = partner_derouter_params.get("vertex_count_tokens_location") or vertex_location

            vertex_credentials: Final = partner_derouter_params.get("vertex_credentials") or partner_derouter_params.get(
            ...
            result = await partner_models_handler.count_tokens(
                ...
                derouter_params=partner_derouter_params,
                vertex_project=vertex_project,
                vertex_location=vertex_location,
            ...
            from derouter.llms.vertex_ai.count_tokens.handler import VertexAITokenCounter
