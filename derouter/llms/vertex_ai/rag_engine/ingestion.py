...
Uses:
- derouter.files.acreate_file for uploading files to GCS
- Vertex AI RAG Engine REST API for importing files into corpus (via httpx)

Key differences from OpenAI:
- Files must be uploaded to GCS first (via derouter.files.acreate_file)
...
if TYPE_CHECKING:
    ...
    from derouter.types.rag import RAGIngestOptions
...
class VertexAIRAGIngestion(BaseRAGIngestion):
    ...
    Uses derouter.files.acreate_file for GCS upload, then imports into RAG corpus.
    ...
    async def _upload_file_to_gcs(
        ...
        """
        Upload file to GCS using derouter.files.acreate_file.
        ...
        # Set GCS_BUCKET_NAME env var for derouter.files.create_file
        ...
        try:
            # Create file tuple for derouter.files.acreate_file
            file_tuple: Final = (filename, file_content, content_type)

            verbose_logger.debug(
                "Uploading file to GCS via derouter.files.acreate_file: %s (bucket: %s)", filename, self.gcs_bucket
            )

            # Upload to GCS using DeRouter's file upload
            response: Final = await derouter.acreate_file(
                file=file_tuple,
    ...
    def _build_transformation_config(self) -> object:
        ...
        from derouter.types.rag import RAGChunkingStrategy
    ...
    async def store(
        ...
        Vertex AI workflow:
        1. Upload file to GCS via derouter.files.acreate_file
