...
from derouter.llms.vertex_ai.gemini.transformation import _transform_request_body
...
from derouter.types.files import StreamingMediaUploadConfig
...
from derouter.types.llms.vertex_ai import GcsBucketResponse, GeminiEmbeddingInput
...
def _set_derouter_batch_custom_id_labels(labels: dict[str, str], custom_id: object) -> None:
    """
    Store OpenAI batch custom_id for Vertex batch correlation.

    ``derouter_custom_id`` is GCP-label-safe (may alter casing and characters).
    ``derouter_custom_id_raw`` encodes the original string for
    round-trip correlation in batch output transforms.
    """
    custom_id_str: Final = str(custom_id)
    labels["derouter_custom_id"] = _sanitize_gcp_label_value(custom_id_str)
    raw_label_chunks: Final = _encode_gcp_label_value_chunks(custom_id_str)
    labels["derouter_custom_id_raw"] = raw_label_chunks[0]
    for index, raw_label_chunk in enumerate(raw_label_chunks[1:], start=1):
        labels[f"derouter_custom_id_raw_{index}"] = raw_label_chunk


def _get_derouter_batch_custom_id(vertex_output_row: Mapping[str, object]) -> str:
    """
    ...
    labels instead (see `_set_derouter_batch_custom_id_labels`).
    """
    ...
    return _get_derouter_batch_custom_id_from_labels(labels)


def _get_derouter_batch_custom_id_from_labels(labels: Mapping[str, object] | None) -> str:
    """Prefer encoded custom_id when present (see _set_derouter_batch_custom_id_labels)."""
    if not labels:
        return "unknown"
    raw: Final = labels.get("derouter_custom_id_raw")
    if raw:
        raw_chunks: Final = [str(raw)]
        chunk_prefix: Final = "derouter_custom_id_raw_"
        ...
        if decoded is not None:
            return decoded
        return str(raw)
    return str(labels.get("derouter_custom_id", "unknown"))
...
def _split_vertex_batch_key(vertex_output_row: Mapping[str, object]) -> tuple[str, int, int]:
    ...
    key = vertex_output_row.get(_VERTEX_BATCH_KEY_FIELD)
    if key is None:
        return _get_derouter_batch_custom_id(vertex_output_row), 0, 1
...
def _model_from_managed_gcs_url(url: str) -> str | None:
    """
    Extracts the model from a DeRouter-managed Vertex batch GCS url.
...
def _openai_batch_jsonl_entry_to_vertex_rows(
    ...
    if custom_id is not None:
        ...
        _set_derouter_batch_custom_id_labels(vertex_request_body["labels"], custom_id)
...
class VertexAIFilesConfig(VertexBase, BaseFilesConfig):
    ...
    def validate_environment(
        ...
        if not api_key:
            api_key, _ = self.get_access_token(
                credentials=derouter_params.get("vertex_credentials"),
                project_id=derouter_params.get("vertex_project"),
            )
            if not api_key:
    ...
    def _get_gcs_object_name_from_batch_jsonl(
        ...
        named as: derouter-vertex-{model}-{uuid}
    ...
    def _get_configured_bucket_name(self, derouter_params: dict) -> str:
        bucket_name: Final = (
            derouter_params.get("gcs_bucket_name") or derouter_params.get("bucket_name") or os.getenv("GCS_BUCKET_NAME")
        )
        if not bucket_name:
    ...
    def get_complete_file_url(
        ...
        """
        if data.get("purpose") == "batch" and derouter_params.get("custom_endpoint"):
            raise VertexAIError(
                status_code=400,
                message=(
                    "Vertex AI batch prediction is not supported for `custom_endpoint` deployments. "
                    "The OpenAI-compatible custom endpoint path has no batch surface in DeRouter; "
        ...
        bucket_name = self._get_configured_bucket_name(derouter_params)
        ...
        configured_model: Final = derouter_params.get("model")
    ...
    def _parse_gcs_uri(self, file_id: str, derouter_params: dict | None = None) -> tuple[str, str]:
        """
        Validate a managed GCS file_id and return (bucket, url-encoded-object-path).
        """
        configured_bucket_name: Final = self._get_configured_bucket_name(derouter_params or {})
    ...
    def transform_retrieve_file_request(
        ...
        bucket, encoded_object = self._parse_gcs_uri(file_id, derouter_params)
    ...
    def transform_file_content_response(
        ...
        try:
            ...
            if getattr(derouter, "disable_vertex_batch_output_transformation", False):
    ...
    def _try_transform_vertex_batch_output_to_openai(
        ...
        {
          "request": {"contents": [...], "labels": {"derouter_custom_id": "request-1", "derouter_custom_id_raw": "..."}},
          "status": "",
    ...
    def _transform_single_vertex_batch_output_to_openai(
        ...
        """
        custom_id: Final = _get_derouter_batch_custom_id(vertex_output)
