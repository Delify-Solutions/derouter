...
Handles the transformation between DeRouter's OpenAI-compatible format and
...
from derouter.llms.vertex_ai.agent_engine.sse_iterator import (
...
class VertexAgentEngineConfig(BaseConfig, VertexBase):
    ...
    def get_complete_url(
        ...
        # Get project and location from derouter_params or environment
        vertex_project: Final = self.safe_get_vertex_ai_project(derouter_params)
        vertex_location: Final = self.safe_get_vertex_ai_location(derouter_params) or "us-central1"
        ...
        if not resource_path.startswith("projects/"):
            if not vertex_project:
                raise ValueError(
                    ...
                    "Set via derouter_params['vertex_project'] or VERTEXAI_PROJECT env var."
                )
    ...
    def _get_auth_headers(
        ...
        vertex_credentials: Final = self.safe_get_vertex_ai_credentials(derouter_params)
        vertex_project: Final = self.safe_get_vertex_ai_project(derouter_params)
    ...
    def _get_user_id(self, optional_params: dict) -> str:
        ...
        if user_id:
            return user_id
        # Generate a user ID
        return f"derouter-user-{str(uuid.uuid4())[:8]}"
    ...
    def validate_environment(
        ...
        auth_headers: Final = self._get_auth_headers(optional_params, derouter_params)
    ...
    def _calculate_usage(self, model: str, messages: list[AllMessageValues], content: str) -> Usage | None:
        """Calculate token usage using DeRouter's token counter."""
        try:
    ...
    def transform_response(
        ...
        """
        Transform Vertex Agent Engine response to DeRouter ModelResponse format.
