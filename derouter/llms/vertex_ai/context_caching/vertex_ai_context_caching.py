...
from derouter.constants import MINIMUM_PROMPT_CACHE_TOKEN_COUNT
...
from derouter.llms.openai.openai import AllMessageValues
from derouter.types.llms.vertex_ai import (
    CachedContentListAllResponseBody,
    VertexAICachedContentResponseObject,
)
from derouter.utils import is_prompt_caching_valid_prompt
...
local_cache_obj: Final = Cache(type=DeRouterCacheType.LOCAL)  # only used for calling 'get_cache_key' function
...
class ContextCachingEndpoints(VertexBase):
    ...
    async def async_check_and_create_cache(
        ...
        if client is None or not isinstance(client, AsyncHTTPHandler):
            client = get_async_httpx_client(params={"timeout": timeout}, llm_provider=derouter.LlmProviders.VERTEX_AI)
        else:
            client = client
