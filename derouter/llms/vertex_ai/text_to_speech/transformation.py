...
import derouter
from derouter.exceptions import UnsupportedParamsError
...
from derouter.types.llms.vertex_ai_text_to_speech import (
...
class VertexAITextToSpeechConfig(BaseTextToSpeechConfig, VertexBase):
    ...
    def dispatch_text_to_speech(
        ...
        derouter_params_dict: dict[str, object],
        ...
        **kwargs: object,
        ...
        vertex_credentials: Final = self.safe_get_vertex_ai_credentials(derouter_params_dict)
        vertex_project: Final = self.safe_get_vertex_ai_project(derouter_params_dict)
        vertex_location: Final = self.safe_get_vertex_ai_location(derouter_params_dict)
        ...
        # Store credentials in derouter_params for use in transform methods
    ...
    def _validate_vertex_input(
        ...
        if use_ssml:
            ...
            # DeRouter will auto-detect if text is in ssml format
    ...
    def transform_text_to_speech_request(
        ...
        """
        # Get Vertex AI credentials from derouter_params
        vertex_credentials: Final[VERTEX_CREDENTIALS_TYPES | None] = derouter_params.get("vertex_credentials")
        vertex_project: str | None = derouter_params.get("vertex_project")
        ...
        auth_header, _ = self._get_token_and_url(
            ...
            vertex_credentials=vertex_credentials,
            vertex_project=vertex_project,
            vertex_location=derouter_params.get("vertex_location"),
            stream=False,
            custom_llm_provider="vertex_ai_beta",
        ...
        # 1. derouter_params by dispatch method
        # 2. optional_params by map_openai_params
        voice_dict: Final = derouter_params.get("vertex_voice_dict") or optional_params.get("vertex_voice_dict")
...
class VertexAILyriaTextToSpeechConfig(VertexAITextToSpeechConfig):
            ...
            if drop_params or derouter.drop_params:
                ...
                raise UnsupportedParamsError(
                    ...
                    message=(
                        ...
                        "from the call, set `derouter.drop_params = True`"
                    ),
                )
            ...
            if drop_params or derouter.drop_params:
                ...
                raise UnsupportedParamsError(
                    ...
                    message=(
                        ...
                        "To drop unsupported openai params from the call, set `derouter.drop_params = True`"
                    ),
                )
    ...
    def get_complete_url(
        ...
        model: str,
        api_base: str | None,
        derouter_params: dict,  # mutable-ok: inherited provider interface accepts concrete DeRouter parameters
    ) -> str:
        base_model: Final = model.removeprefix("vertex_ai/")
        model_info: Final = self._get_model_info(model=model)
        configured_project: Final = self.safe_get_vertex_ai_project(derouter_params)
        project: Final = (
            self._ensure_access_token(
                credentials=self.safe_get_vertex_ai_credentials(derouter_params),
                project_id=None,
                custom_llm_provider="vertex_ai",
            )[1]
        ...
        if model_info["vertex_ai_audio_api"] == "lyria_interactions":
            from derouter.llms.vertex_ai.interactions.transformation import (
            ...
            return VertexAIInteractionsConfig(mint_access_token=mint_access_token).get_complete_url(
                api_base=api_base,
                model=base_model,
                derouter_params={  # mutable-ok: interactions dispatch expects a concrete parameter dictionary
                    **derouter_params,
                    "vertex_project": project,
                    "vertex_location": "global",
                },
            )
        location: Final = self.safe_get_vertex_ai_location(derouter_params) or self.get_default_vertex_location()
    ...
    def transform_text_to_speech_request(
        ...
        access_token, project = self._ensure_access_token(
            credentials=self.safe_get_vertex_ai_credentials(derouter_params),
            project_id=self.safe_get_vertex_ai_project(derouter_params),
            custom_llm_provider="vertex_ai",
        )
        headers.update(
