...
from derouter.derouter_core_utils.prompt_templates import factory as ptf
...
from derouter.types.llms.watsonx import WatsonXAPIParams, WatsonXCredentials
...
def generate_iam_token(api_key=None, **params) -> str:
    ...
    if result is None:
        ...
        response: Final = derouter.module_level_client.post(url=iam_token_url, data=data, headers=headers)
...
async def _aconvert_watsonx_messages_core(
    ...
    from derouter.types.llms.watsonx import WatsonXModelPattern
...
class IBMWatsonXMixin:
    ...
    def _add_api_version_to_url(self, url: str, api_version: str | None) -> str:
        api_version = api_version or derouter.WATSONX_DEFAULT_API_VERSION
