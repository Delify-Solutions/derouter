...
class VolcEngineResponsesAPIConfig(OpenAIResponsesAPIConfig):
    _SUPPORTED_OPTIONAL_PARAMS: list[str] = [
        ...
        "expire_at",
        "context_management",
        # DeRouter-internal metadata (not sent to provider)
        "metadata",
    ...
    def validate_environment(self, headers: dict, model: str, derouter_params: GenericDeRouterParams | None) -> dict:
        ...
        elif isinstance(derouter_params, dict):
            derouter_params = GenericDeRouterParams.model_validate(derouter_params)
    ...
    def map_openai_params(
        ...
        # DeRouter metadata is internal-only; don't send to provider
