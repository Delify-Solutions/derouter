...
from derouter.utils import _cached_get_model_info_helper
...
def github_copilot_supports_responses_api(model: str) -> bool:
    ...
    raw_info: Final = derouter.model_cost.get(key) if isinstance(key, str) else None
...
class GithubCopilotResponsesAPIConfig(OpenAIResponsesAPIConfig):
    ...
    def validate_environment(
        ...
        try:
            ...
            input_param: Final = self._get_input_from_params(derouter_params)
    ...
    def _get_input_from_params(self, derouter_params: GenericDeRouterParams | None) -> str | ResponseInputParam | None:
        """
        Extract input parameter from derouter_params.
        ...
        # Try to get input from derouter_params
        # This might be in different locations depending on how DeRouter structures it
        if hasattr(derouter_params, "input"):
            return derouter_params.input
