...
from derouter.constants import XAI_API_BASE
...
from derouter.llms.xai.common_utils import XAIModelInfo, xai_reported_cost_in_usd
...
if TYPE_CHECKING:
    from derouter.derouter_core_utils.derouter_logging import (
        Logging as _DeRouterLoggingObj,
    )
...
class XAIResponsesAPIConfig(OpenAIResponsesAPIConfig):
    ...
    def validate_environment(self, headers: dict, model: str, derouter_params: GenericDeRouterParams | None) -> dict:
        ...
        api_key = XAIModelInfo.get_api_key(derouter_params.api_key, legacy_generic_before_env=True)

        if not api_key:
            from derouter.llms.xai.oauth import (
                XAIOAuthAuthenticator,
                XAIOAuthError,
                should_use_xai_oauth,
            )

            if should_use_xai_oauth(derouter_params.model_dump()):
                try:
        ...
        if not api_key:
            raise ValueError(
                "XAI API key is required. Set api_key, derouter.xai_key, "
                "derouter.api_key, XAI_API_KEY, or use_xai_oauth=True."
            )
    ...
    def get_complete_url(
        ...
        """
        from derouter.llms.xai.oauth import XAIOAuthAuthenticator, should_use_xai_oauth

        api_key: Final = XAIModelInfo.get_api_key(derouter_params.get("api_key"), legacy_generic_before_env=True)
        if should_use_xai_oauth(derouter_params) and not api_key:
            api_base = XAIOAuthAuthenticator().get_api_base()
        else:
            api_base = api_base or derouter.api_base or get_secret_str("XAI_API_BASE") or XAI_API_BASE
