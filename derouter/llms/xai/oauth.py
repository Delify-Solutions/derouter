...
from derouter.llms.custom_httpx.http_handler import HTTPHandler, _get_httpx_client
...
class XAIOAuthAuthenticator:
    def __init__(self, http_client: httpx.Client | HTTPHandler | None = None) -> None:
        self.token_dir = get_secret_str("XAI_OAUTH_TOKEN_DIR") or os.path.expanduser("~/.config/derouter/xai_oauth")
    ...
    def get_access_token(self) -> str:
        ...
        if not auth_data:
            raise XAIOAuthLoginRequiredError("xAI OAuth login required. Run `derouter xai-oauth login`.")
        ...
        if not refresh_token:
            raise XAIOAuthLoginRequiredError("xAI OAuth refresh token missing. Run `derouter xai-oauth login`.")
...
def should_use_xai_oauth(derouter_params: Mapping[str, object] | None) -> bool:
    return bool((derouter_params or {}).get("use_xai_oauth"))
