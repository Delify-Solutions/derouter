...
from derouter.router_utils.reasoning_effort_capability import declared_reasoning_efforts_for_model
from derouter.types.llms.openai import AllMessageValues
from derouter.utils import supports_function_calling, supports_reasoning, supports_response_schema

from ...openai.chat.gpt_transformation import OpenAIGPTConfig

TOOL_CALLING_PARAMS: Final = ("tools", "tool_choice", "function_call")
DEROUTER_INTERNAL_ASSISTANT_FIELDS: Final = frozenset({"thinking_blocks", "provider_specific_fields"})
...
def _registry_verdict(model: str, flag: str, check: Callable[[str], bool]) -> bool | None:
    ...
    registry_entry: Final = derouter.model_cost.get(f"together_ai/{model}")
...
def _tool_params_to_drop(passed_params: Container[str], model: str, drop_params: bool) -> tuple[str, ...]:
    ...
    raise UnsupportedParamsError(
        status_code=500,
        message=f"together_ai does not support parameters: {', '.join(passed_tool_params)}, for model={model}. To drop it from the call, set `derouter.drop_params = True`.",
    )
...
def _drop_response_format(passed_params: Container[str], model: str, drop_params: bool) -> bool:
    ...
    raise UnsupportedParamsError(
        status_code=500,
        message=f"together_ai does not support parameters: response_format, for model={model}. To drop it from the call, set `derouter.drop_params = True`.",
    )


def _without_derouter_internal_fields(message: AllMessageValues) -> AllMessageValues:
    if message["role"] != "assistant" or DEROUTER_INTERNAL_ASSISTANT_FIELDS.isdisjoint(message):
        return message
    return cast(  # cast-ok: rebuilding the same TypedDict minus internal keys loses the narrowed type
        ...
        {  # mutable-ok: TypedDict rebuild minus internal keys
            key: value for key, value in message.items() if key not in DEROUTER_INTERNAL_ASSISTANT_FIELDS
        },
    )
...
class TogetherAIChatConfig(OpenAIGPTConfig):
    ...
    def _transform_messages(
        ...
        only derouter-internal fields are stripped before sending."""
        stripped: Final = [  # mutable-ok: super() requires a list
            _without_derouter_internal_fields(message) for message in messages
        ]
        if is_async:
