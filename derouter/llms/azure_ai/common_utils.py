...
def get_azure_ai_entra_token(derouter_params: Mapping[str, object] | None = None) -> str | None:
    """
    ...
    """
    from derouter.llms.azure.common_utils import get_azure_ad_token

    params = GenericDeRouterParams.model_validate(derouter_params) if derouter_params else GenericDeRouterParams()
...
def get_azure_ai_auth_headers(
    ...
    azure_ad_token = get_azure_ai_entra_token(derouter_params=derouter_params)
    if azure_ad_token:
    ...
    raise ValueError(
        ...
        "identity with `derouter.enable_azure_ad_token_refresh = True`)"
    )
...
class AzureFoundryModelInfo(BaseLLMModelInfo):
    ...
    def is_model_router_call(
        ...
        Prefers the response stamp, then the deployment's derouter model path, and only then the
        ...
        deployment_model: Final = (
            hidden_params.get("derouter_model_name") or hidden_params.get("model") if hidden_params is not None else None
        )
        return any(
    ...
    def get_api_base(api_base: str | None = None) -> str | None:
        return api_base or derouter.api_base or get_secret_str("AZURE_AI_API_BASE")

    @staticmethod
    def get_api_key(api_key: str | None = None) -> str | None:
        return api_key or derouter.api_key or get_secret_str("AZURE_AI_API_KEY")
    ...
    def get_azure_ai_config_for_model(model: str):
        ...
        if azure_ai_route == "model_router":
            from derouter.llms.azure_ai.azure_model_router.transformation import (
                AzureModelRouterConfig,
            )

            return AzureModelRouterConfig()
        elif "claude" in model.lower():
            from derouter.llms.azure_ai.anthropic.transformation import (
                AzureAnthropicConfig,
            )

            return AzureAnthropicConfig()
        else:
            from derouter.llms.azure_ai.chat.transformation import AzureAIStudioConfig
