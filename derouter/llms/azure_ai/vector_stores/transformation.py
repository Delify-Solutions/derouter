...
class AzureAIVectorStoreConfig(BaseQueryEmbeddingVectorStoreConfig, BaseAzureLLM):
    ...
    def validate_environment(self, headers: dict, derouter_params: GenericDeRouterParams | None) -> dict:
        basic_headers: Final = self._base_validate_azure_environment(headers, derouter_params)
    ...
    def get_complete_url(
        ...
        # Get search service name from derouter_params
        search_service_name: Final = derouter_params.get("azure_search_service_name")

        if not search_service_name:
            raise ValueError(
                "Azure AI Search service name is required. "
                "Provide it via derouter_params['azure_search_service_name'] or api_base parameter"
            )
    ...
    def _search_request(
        ...
        vector_field: Final = derouter_params.get("azure_search_vector_field", "contentVector")
        ...
        derouter_logging_obj.model_call_details["top_k"] = top_k
