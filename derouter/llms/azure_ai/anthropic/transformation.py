...
class AzureAnthropicConfig(AnthropicConfig):
    ...
    def validate_environment(
        ...
        derouter_params: dict | GenericDeRouterParams,
        ...
        # Convert dict to GenericDeRouterParams if needed
        if isinstance(derouter_params, dict):
            # Ensure api_key is included if provided
            if api_key and "api_key" not in derouter_params:
                derouter_params = {**derouter_params, "api_key": api_key}
            derouter_params_obj = GenericDeRouterParams(**derouter_params)
        else:
            derouter_params_obj = derouter_params or GenericDeRouterParams()
            # Set api_key if provided and not already set
            if api_key and not derouter_params_obj.api_key:
                derouter_params_obj.api_key = api_key
