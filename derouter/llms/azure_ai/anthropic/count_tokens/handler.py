...
class AzureAIAnthropicCountTokensHandler(AzureAIAnthropicCountTokensConfig):
    ...
    async def handle_count_tokens_request(
        ...
        Args:
            ...
            derouter_params: Optional DeRouter parameters
            timeout: Optional timeout for the request (defaults to derouter.request_timeout)
        ...
        try:
            ...
            # Use DeRouter's async httpx client
            async_client: Final = get_async_httpx_client(llm_provider=derouter.LlmProviders.AZURE_AI)

            # Use provided timeout or fall back to derouter.request_timeout
            request_timeout: Final = timeout if timeout is not None else derouter.request_timeout
