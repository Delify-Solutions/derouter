...
from derouter.constants import ANTHROPIC_TOKEN_COUNTING_BETA_VERSION
...
class AzureAIAnthropicCountTokensConfig(AnthropicCountTokensConfig):
    ...
    def get_required_headers(
        ...
        Args:
            ...
            derouter_params: Optional DeRouter parameters for additional auth config
        ...
        if "api_key" not in derouter_params:
            derouter_params["api_key"] = api_key

        derouter_params_obj: Final = GenericDeRouterParams.model_validate(derouter_params)

        # Get Azure auth headers (api-key or Authorization)
        azure_headers = BaseAzureLLM._base_validate_azure_environment(headers={}, derouter_params=derouter_params_obj)
