...
explicit flag is the only signal that the tier is a real one rather than derouter rounding the level
...
def _bare_model_entry(model_info: Mapping[str, object]) -> Mapping[str, object]:
    ...
    if not isinstance(key, str) or not isinstance(provider, str) or not key.startswith(f"{provider}/"):
        return _EMPTY_ENTRY
    entry: Final[Mapping[str, object] | None] = derouter.model_cost.get(key.removeprefix(f"{provider}/"))
...
def declared_reasoning_efforts_for_model(model: str, custom_llm_provider: str) -> tuple[str, ...] | None:
    ...
    """
    entry: Final = derouter.model_cost.get(f"{custom_llm_provider}/{model}") or derouter.model_cost.get(model)
...
def _supports_none_reasoning_effort(model_info: Mapping[str, object], flag: object) -> bool:
    ...
    if model_info.get("derouter_provider") != "azure":
