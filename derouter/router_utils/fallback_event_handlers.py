...
from derouter.derouter_core_utils.sensitive_data_masker import mask_sensitive_structure
...
from derouter.types.router import DeRouterParamsTypedDict
...
def _trigger_cooldown_for_failed_deployment(
    ...
    try:
        ...
        # timeout, which derouter.Timeout reports as status 408 regardless of the deployment's
        ...
        if kwargs.get("client_side_timeout") and cast_exception_status_to_int(exception_status) == 408:
            verbose_router_logger.debug(
                ...
                "x-derouter-timeout caused this 408, not deployment health."
            )
            return

        # Only Router._set_failed_deployment_id_on_exception()'s server-stamped id is
        # trusted here: a metadata-bucket lookup (e.g. "metadata"/"derouter_metadata")
        ...
        deployment_dict: Final = derouter_router.get_model_info(id=deployment_id)
        deployment_cooldown: Final = (
            _first_present(
                deployment_dict.get("model_info"), deployment_dict.get("derouter_params"), key="cooldown_time"
            )
        ...
        _get_retry_after: Final = (
            derouter.utils._get_retry_after_from_exception_header  # pyright: ignore[reportPrivateUsage] - as router.py
        )
        ...
        time_to_cooldown: Final = (
            ...
            if deployment_cooldown is not None and deployment_cooldown >= 0
                ...
                else derouter_router.cooldown_time
            )
        )

        increment_deployment_failures_for_current_minute(
            derouter_router_instance=derouter_router,
...
_ROUTER_METADATA_BUCKETS: Final = ("metadata", "derouter_metadata")
...
async def _is_fallback_target_authorized(
    ...
    access_check: Final = derouter_router.fallback_access_check
    ...
    if access_check is None or target is None or target == original_model_group:
        return True
    if await access_check(model=target, request_kwargs=kwargs, llm_router=derouter_router):
        return True
...
async def run_async_fallback(
    ...
    **kwargs,
    ...
    Args:
        derouter_router: The derouter router instance.
    ...
    for mg in fallback_model_group:
        ...
        if same_model_group_only and _get_fallback_target_model_group(mg) != original_model_group:
            ...
            continue
        if not await _is_fallback_target_authorized(derouter_router, mg, original_model_group, kwargs):
            continue
        ...
        try:
            # LOGGING
            kwargs = derouter_router.log_retry(kwargs=kwargs, e=original_exception)
            ...
            response = await derouter_router.async_function_with_fallbacks(*args, **kwargs)
            ...
            if logging_obj is not None and logging_obj.model_call_details.get("has_logged_async_failure", False):
                _trigger_cooldown_for_failed_deployment(
                    derouter_router=derouter_router,
                    kwargs=kwargs,
                    exception=e,
                )
...
async def log_success_fallback_event(original_model_group: str, kwargs: dict, original_exception: Exception):
    ...
    custom_loggers: Final = derouter.logging_callback_manager.get_custom_loggers_for_type(CustomLogger)
...
def _check_non_standard_fallback_format(fallbacks: list[Any] | None) -> bool:
    ...
    - List[Dict[<DeRouterParamsTypedDict>, Any]]: e.g. [{"model": "claude-3-haiku", "messages": [{"role": "user", "content": "Hey, how's it going?"}]}]
    ...
    if all(isinstance(item, str) for item in fallbacks):
        ...
        for item in fallbacks:
            for key in DeRouterParamsTypedDict.__annotations__:
                if key in item:
