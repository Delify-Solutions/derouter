...
def apply_quality_router_decision_headers(
    ...
    quality_header_fields: Final = (
        ("routed_model", "x-derouter-quality-router-model"),
        ("quality_tier", "x-derouter-quality-router-tier"),
        ("routed_via", "x-derouter-quality-router-via"),
        ("matched_keyword", "x-derouter-quality-router-keyword"),
        ("complexity_tier", "x-derouter-quality-router-complexity"),
    )
...
def get_fallback_errors_from_headers(
    ...
    existing_errors: Final = additional_headers.get("x-derouter-fallback-errors")
...
def add_retry_headers_to_response(
    ...
    retry_headers: Final[dict[str, object]] = {
        "x-derouter-attempted-retries": attempted_retries,
    }
    if max_retries is not None:
        retry_headers["x-derouter-max-retries"] = max_retries
...
def add_fallback_headers_to_response(
    ...
    fallback_headers: Final[dict[str, object]] = {
        "x-derouter-attempted-fallbacks": attempted_fallbacks,
    }
    ...
    additional_headers["x-derouter-fallback-errors"] = json.dumps(merged_errors)
    hidden_params["additional_headers"] = additional_headers
    _write_hidden_params(response, hidden_params)
    return response
