...
from derouter.router_utils.cooldown_callbacks import router_cooldown_event_callback
...
_ADVISOR_ORCHESTRATION_FAILURE_ATTR: Final = "_derouter_advisor_orchestration_failure"
...
_EXCEPTION_POLICY_FIELDS: Final[tuple[tuple[type, str], ...]] = (
    ...
    (derouter.ContentPolicyViolationError, "ContentPolicyViolationErrorAllowedFails"),
    (derouter.BadRequestError, "BadRequestErrorAllowedFails"),
    (derouter.AuthenticationError, "AuthenticationErrorAllowedFails"),
    (derouter.Timeout, "TimeoutErrorAllowedFails"),
    (derouter.RateLimitError, "RateLimitErrorAllowedFails"),
    (derouter.InternalServerError, "InternalServerErrorAllowedFails"),
    (derouter.ServiceUnavailableError, "ServiceUnavailableErrorAllowedFails"),
    (derouter.BadGatewayError, "BadGatewayErrorAllowedFails"),
    (derouter.NotFoundError, "NotFoundErrorAllowedFails"),
)
...
def _get_deployment_cooldown_policy(
    ...
    `cooldown_time`, they have no pre-existing `derouter_params` precedent): `derouter_params`
    gets copied wholesale into the actual provider call kwargs (see e.g.
    Router._image_generation's `data = deployment["derouter_params"].copy()`), so a new
    field placed there would leak into the outgoing LLM request instead of staying
    router-internal.
    """
    dep: Final = derouter_router_instance.get_model_info(id=deployment)
    if dep is None:
        return None, None
...
def _should_cooldown_based_on_deployment_policy(
    ...
    cooldown_time_override: Final = (
        _first_present(dep.get("model_info"), dep.get("derouter_params"), key="cooldown_time")
        if dep is not None
        else None
    )
...
def _has_explicit_allowed_fails_policy_for_exception(
    ...
    if deployment is None:
        return False
    dep_policy, _ = _get_deployment_cooldown_policy(derouter_router_instance, deployment)
...
def _is_cooldown_required(
    ...
    try:
        ...
        if exception_str is not None:  # don't cooldown on derouter api connection errors errors
...
def _should_run_cooldown_logic(
    ...
    - deployment is in derouter_router_instance.provider_default_deployment_ids
    - exception_status is not one that should be immediately retried (e.g. 401)
    """
    if deployment is None or derouter_router_instance.get_model_group(id=deployment) is None:
    ...
    if derouter_router_instance.disable_cooldowns:
    ...
    if deployment in derouter_router_instance.provider_default_deployment_ids:
...
def _should_cooldown_deployment(
    ...
    if (
        derouter_router_instance.allowed_fails_policy is None
        and _is_allowed_fails_set_on_router(derouter_router_instance=derouter_router_instance) is False
    ):
        num_successes_this_minute: Final = get_deployment_successes_for_current_minute(
            derouter_router_instance=derouter_router_instance, deployment_id=deployment
        )
        ...
        elif derouter._should_retry(status_code=cast_exception_status_to_int(exception_status)) is False:
            return True
...
def _set_cooldown_deployments(
    ...
    if _should_cooldown_deployment(
        ...
        derouter_router_instance.cooldown_cache.add_deployment_to_cooldown(
            model_id=deployment,
            original_exception=original_exception,
...
async def _async_get_cooldown_deployments(
    ...
    """
    model_ids: Final = derouter_router_instance.get_model_ids()
    cooldown_models: Final = await derouter_router_instance.cooldown_cache.async_get_active_cooldowns(
        model_ids=model_ids,
...
def _get_cooldown_deployments(derouter_router_instance: LitellmRouter, parent_otel_span: Span | None) -> list[str]:
    """
    ...
    cooldown_models: Final = derouter_router_instance.cooldown_cache.get_active_cooldowns(
        model_ids=model_ids, parent_otel_span=parent_otel_span
    )
...
def should_cooldown_based_on_allowed_fails_policy(
    ...
    """
    allowed_fails_from_policy: Final = derouter_router_instance.get_allowed_fails_from_policy(
    ...
    allowed_fails: Final = (
        ...
        if allowed_fails_override is not None
            ...
            if allowed_fails_from_policy is not None
            else derouter_router_instance.allowed_fails
        )
    )
    cooldown_time: Final = (
        cooldown_time_override
        if cooldown_time_override is not None
        else (derouter_router_instance.cooldown_time or DEFAULT_COOLDOWN_TIME_SECONDS)
    )
    ...
    updated_fails: Final = _increment_allowed_fails(
        cache=derouter_router_instance.cache, cache_key=cache_key, ttl=cooldown_time
    )
...
def _is_allowed_fails_set_on_router(
    ...
    """
    if derouter_router_instance.allowed_fails is None:
        return False
    if derouter_router_instance.allowed_fails != derouter.allowed_fails:
        return True
    return False
