...
from derouter.constants import DEFAULT_MINIMUM_PROMPT_CACHE_TOKEN_COUNT
...
from derouter.types.utils import CallTypes, StandardLoggingPayload
from derouter.utils import get_prompt_cache_min_tokens, is_prompt_caching_valid_prompt
...
def _get_min_token_count_for_deployments(healthy_deployments: list[dict]) -> int:
    ...
    return min(
        (
            get_prompt_cache_min_tokens(model=deployment["derouter_params"]["model"])
            for deployment in healthy_deployments
            if deployment.get("derouter_params", {}).get("model")
        ),
...
class PromptCachingDeploymentCheck(CustomLogger):
    ...
    async def async_filter_deployments(
        ...
        if messages is not None and is_prompt_caching_valid_prompt(
            ...
            ## `derouter.acompletion`, after a deployment has been picked, so the affinity key has to
            ...
            affinity_messages: Final = AnthropicCacheControlHook.messages_with_default_injections(
                ...
                models=(
                    deployment["derouter_params"]["model"]
                    for deployment in healthy_deployments
                    if isinstance(deployment.get("derouter_params"), dict) and deployment["derouter_params"].get("model")
                ),
                tools=(
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        if (
            ...
            verbose_logger.debug(
                "derouter.router_utils.pre_call_checks.prompt_caching_deployment_check: skipping adding model id to prompt caching cache, CALL TYPE IS NOT COMPLETION or ANTHROPIC MESSAGE"
            )
            return
        ...
        if messages is None or not isinstance(messages, list):
            verbose_logger.debug(
                "derouter.router_utils.pre_call_checks.prompt_caching_deployment_check: skipping adding model id to prompt caching cache, MESSAGES IS NOT A LIST"
            )
            return
        if model_id is None:
            verbose_logger.debug(
                "derouter.router_utils.pre_call_checks.prompt_caching_deployment_check: skipping adding model id to prompt caching cache, MODEL ID IS NONE"
            )
            return
