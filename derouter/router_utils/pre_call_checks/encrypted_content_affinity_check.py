...
2. **Items without IDs** (Codex): Wrap the encrypted_content with model_id metadata
   (e.g., ``derouter_enc:{base64_metadata};{original_encrypted_content}``)
...
from derouter.router_utils.cooldown_cache import CooldownCacheValue
...
class EncryptedContentAffinityCheck(CustomLogger):
    ...
    The ``model_id`` is decoded directly from the derouter-encoded item IDs –
    ...
    def _extract_model_id_from_input(request_input: object) -> str | None:
        """
        Scan ``input`` items for derouter-encoded encrypted-content markers and
        return the ``model_id`` embedded in the first one found.

        Checks both:
        1. Encoded item IDs (encitem_...) - for clients that send IDs
        2. Wrapped encrypted_content (derouter_enc:...) - for clients like Codex that don't send IDs
    ...
    def _encryption_boundary_key(
        ...
        ``DeRouter_Params``-style Pydantic instances, which define a custom
        ...
        """
        getter: Final = getattr(derouter_params, "get", None)
    ...
    def _find_deployments_on_same_encryption_boundary(
        ...
        if originating is None:
            return [], None
        boundary: Final = self._encryption_boundary_key(originating.derouter_params.model_dump(exclude_none=True))
        if boundary is None:
            return [], originating
        matches: Final = [
            d for d in healthy_deployments if self._encryption_boundary_key(d.get("derouter_params", {})) == boundary
        ]
    ...
    async def async_filter_deployments(
        ...
        """
        If the request ``input`` contains derouter-encoded item IDs, decode the
        ...
        # derouter_metadata already exists (Responses API path).  Using
        # setdefault would create an empty derouter_metadata dict for chat
        # completions / embeddings, which breaks tag-based routing because
        # _get_metadata_variable_name_from_kwargs would pick "derouter_metadata"
        # over "metadata" where tags are actually stored.
        if "derouter_metadata" in request_kwargs:
            request_kwargs["derouter_metadata"]["encrypted_content_affinity_enabled"] = True
        ...
        if cooldown is not None and str(cooldown.get("status_code")) == "429":
            ...
            return RateLimitError(
                ...
                response=httpx.Response(
                    ...
                    request=httpx.Request("POST", "https://router.delify.vn/"),
                ),
            )
