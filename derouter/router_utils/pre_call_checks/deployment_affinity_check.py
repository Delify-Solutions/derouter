...
from derouter.constants import SESSION_DEPLOYMENT_AFFINITY_TTL_METADATA_KEY, SESSION_ID_GENERATED_METADATA_KEY
...
class DeploymentAffinityCheck(CustomLogger):
    ...
    @staticmethod
    def _get_model_map_key_from_derouter_model_name(
        ...
        """
        if not derouter_model_name:
            return None

        if "/" not in derouter_model_name:
            return derouter_model_name

        provider_prefix, remainder = derouter_model_name.split("/", 1)
    ...
    def _get_model_map_key_from_deployment(deployment: dict) -> str | None:
        ...
        if isinstance(derouter_params, dict):
            base_model = derouter_params.get("base_model")
            if isinstance(base_model, str) and base_model:
                return base_model
            derouter_model_name: Final = derouter_params.get("model")
            if isinstance(derouter_model_name, str) and derouter_model_name:
                return DeploymentAffinityCheck._get_model_map_key_from_derouter_model_name(derouter_model_name)
    ...
    def _iter_metadata_dicts(request_kwargs: dict) -> list[dict]:
        ...
        Depending on the endpoint, Router may populate `metadata` or `derouter_metadata`.
    ...
    async def async_pre_call_deployment_hook(self, kwargs: dict[str, Any], call_type: CallTypes | None) -> dict | None:
        ...
        Why pre-call?
        - DeRouter runs async success callbacks via a background logging worker for performance.
