...
from derouter.types.router import RouterCacheEnum, RouterErrors
...
ITPM_RESERVED_KEY: Final = "_derouter_itpm_reserved"
OTPM_RESERVED_KEY: Final = "_derouter_otpm_reserved"
ITPM_CACHE_KEY: Final = "_derouter_itpm_cache_key"
OTPM_CACHE_KEY: Final = "_derouter_otpm_cache_key"
...
def get_deployment_io_token_limits(
    ...
    if itpm is None:
        itpm = derouter_params.get("itpm")
    if otpm is None:
        otpm = derouter_params.get("otpm")
    if itpm is None:
...
def _model_max_output_tokens(model_name: str) -> int | None:
    # derouter.get_model_info raises a bare Exception for an unrecognized model;
    # this lookup is a fallback default and must never fail the request.
    with contextlib.suppress(Exception):
        info: Final = derouter.get_model_info(model=model_name)
...
def _resolve_max_tokens(request_kwargs: dict[str, Any] | None, deployment: dict) -> int:
    ...
    model_name: Final = (deployment.get("derouter_params") or {}).get("model")
    if model_name:
...
def _stash_reservation_in_metadata(
    ...
    *,
    ...
    for channel in ("metadata", "derouter_metadata"):
...
def _reservation_channels(kwargs: Mapping[str, object] | None) -> tuple[object, ...]:
    ...
    channels win over derouter_params.metadata (so a top-level stash is never
    shadowed), which win over the standard_logging_object copy.
    """
    if not isinstance(kwargs, dict):
        return ()
    top_level: Final = (kwargs.get("metadata"), kwargs.get("derouter_metadata"))
    derouter_params: Final = _as_mutable_mapping(kwargs.get("derouter_params"))
    from_params: Final = () if derouter_params is None else (derouter_params.get("metadata"),)
...
def _rate_limit_error(limit_label: str, limit: int, current: float) -> derouter.RateLimitError:
    return derouter.RateLimitError(
...
def io_token_pre_call_check(
    ...
    _model: Final = (deployment.get("derouter_params") or {}).get("model") or ""
