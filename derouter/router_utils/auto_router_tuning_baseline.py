...
def router_identity(deployment: Mapping[str, object]) -> str | None:
    ...
    if not isinstance(model_name, str) or not model_name:
        return None
    derouter_params: Final = _mapping(deployment.get("derouter_params"))
    tags: Final = derouter_params.get("tags")
...
def heuristic_v1_router_fingerprint(deployment: Mapping[str, object]) -> tuple[str, str] | None:
    ...
    config: Final = derouter_params.get("complexity_router_config")
    if (
