...
from derouter.derouter_core_utils.get_llm_provider_logic import declared_authenticating_provider, get_llm_provider
...
class PatternMatchRouter:
    ...
    doc: https://router.delify.vn/docs/proxy/configs#provider-specific-wildcard-routing
    ...
    def _return_pattern_matched_deployments(self, matched_pattern: Match, deployments: list[dict]) -> list[dict]:
        ...
        for deployment in deployments:
            ...
            new_deployment["derouter_params"]["model"] = PatternMatchRouter.set_deployment_model_name(
                matched_pattern=matched_pattern,
                derouter_deployment_derouter_model=deployment["derouter_params"]["model"],
            )
    ...
    def set_deployment_model_name(
        ...
        derouter_deployment_derouter_model: str,
    ) -> str:
        """
        ...
        if "*" not in derouter_deployment_derouter_model:
            return derouter_deployment_derouter_model

        wildcard_count: Final = derouter_deployment_derouter_model.count("*")
        ...
        # Replace the corresponding wildcards in the derouter model pattern with extracted segments
        for segment in dynamic_segments:
            derouter_deployment_derouter_model = derouter_deployment_derouter_model.replace("*", segment, 1)
