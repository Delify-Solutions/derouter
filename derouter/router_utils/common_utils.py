...
def _is_proxy_admin_request(request_kwargs: Mapping[str, object] | None) -> bool:
    ...
    derouter_metadata_value: Final = request_kwargs.get("derouter_metadata")
    metadata: Final = metadata_value if isinstance(metadata_value, Mapping) else {}
    derouter_metadata: Final = derouter_metadata_value if isinstance(derouter_metadata_value, Mapping) else {}
...
def get_derouter_params_sensitive_credential_hash(derouter_params: dict) -> str:
    """
    Hash of the credential params, used for mapping the file id to the right model
    """
    sensitive_params: Final = CredentialDeRouterParams(**derouter_params)
...
def filter_team_based_models(
    ...
    request_team_id: Final = metadata.get("user_api_key_team_id") or derouter_metadata.get("user_api_key_team_id")
    if request_team_id is None and _is_proxy_admin_request(request_kwargs) and isinstance(healthy_deployments, list):
        requested_model: Final = (
            request_kwargs.get("model") or metadata.get("model_group") or derouter_metadata.get("model_group")
        )
...
def _deployment_supports_web_search(deployment: dict) -> bool:
    ...
    Note: Ideally we'd fall back to derouter.supports_web_search() but
...
def warn_on_provider_credential_mismatch(model_name: str, derouter_params: Mapping[str, object]) -> str | None:
    """
    ...
    or its provider cannot be resolved. Never raises: a deployment derouter cannot
    ...
    through ``derouter_credential_name`` resolves them after registration, so it
    ...
    if not isinstance(model, str) or not model:
        return None
    scoped: Final = tuple(param for param in PROVIDER_SCOPED_CREDENTIAL_PARAMS if derouter_params.get(param) is not None)
    if not scoped:
        return None
