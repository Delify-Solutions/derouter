...
from derouter.a2a_protocol.providers.watsonx_orchestrate.transformation import (
...
class WXOLitellmParams(TypedDict, total=False):
    """derouter_params keys read when routing an A2A request to watsonx Orchestrate."""
...
class WatsonxOrchestrateHandler:
    ...
    async def _get_bearer_token(
        ...
        if auth_mode == "ibm_cloud":
            ...
            if not username:
                raise ValueError("'username' is required in derouter_params when auth_mode='cp4d'")
    ...
    @staticmethod
    def _extract_derouter_params(derouter_params: WXOLitellmParams) -> WXORequestParams:
        cp4d_host: Final = derouter_params.get("cp4d_host") or ""
        instance_id: Final = derouter_params.get("instance_id") or ""
        wxo_agent_id: Final = derouter_params.get("wxo_agent_id") or ""
        api_key: Final = derouter_params.get("api_key") or ""

        if not cp4d_host:
            raise ValueError("'cp4d_host' is required in derouter_params for WXO agents")
        if not instance_id:
            raise ValueError("'instance_id' is required in derouter_params for WXO agents")
        if not wxo_agent_id:
            raise ValueError("'wxo_agent_id' is required in derouter_params for WXO agents")
        if not api_key:
            raise ValueError("'api_key' is required in derouter_params for WXO agents")
        ...
        return WXORequestParams(
            ...
            api_key=api_key,
            username=derouter_params.get("username") or None,
            auth_mode=derouter_params.get("auth_mode") or "cp4d",
            thread_id=derouter_params.get("thread_id") or None,
        )
    ...
    async def handle_non_streaming(
        ...
        derouter_params: WXOLitellmParams,
    ) -> dict[str, object]:
        wxo: Final = WatsonxOrchestrateHandler._extract_derouter_params(derouter_params)
