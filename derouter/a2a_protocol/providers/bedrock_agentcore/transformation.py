...
def _request_scoped_runtime_session_id(
    ...
    if not isinstance(context_id, str) or not context_id:
        return None
    return scope_session_to_principal(context_id, derouter_params.get(A2A_USER_API_KEY_HASH_PARAM))
...
class BedrockAgentCoreA2ATransformation:
    ...
    def get_url_and_signed_request(
        ...
        Args:
            ...
            derouter_params: Agent's derouter_params (model, api_key, etc.)
            ...
            agent_extra_headers: Per-request headers (from x-a2a-{agent}-* rewrite and
                ...
                ``api_key`` / ``runtimeUserId`` / ``runtimeSessionId`` in derouter_params
        ...
        # Build optional_params from derouter_params (everything except model and custom_llm_provider)
        optional_params: Final = {k: v for k, v in derouter_params.items() if k not in ("model", "custom_llm_provider")}
        ...
        session_id: Final = _validate_runtime_session_id(
            _request_scoped_runtime_session_id(params, derouter_params)
