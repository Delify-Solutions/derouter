...
"""
DeRouter A2A SDK functions.

Provides standalone functions with @client decorator for DeRouter logging integration.
"""
...
from derouter.a2a_protocol.streaming_iterator import A2AStreamingIterator
from derouter.a2a_protocol.utils import A2ARequestUtils
from derouter.constants import DEFAULT_A2A_AGENT_TIMEOUT
...
from derouter.a2a_protocol.card_resolver import (
    DeRouterA2ACardResolver,
    get_agent_card_url,
    normalize_agent_card_interfaces,
)
from derouter.a2a_protocol.exception_mapping_utils import (
    handle_a2a_localhost_retry,
    map_a2a_exception,
)
from derouter.a2a_protocol.exceptions import A2ALocalhostURLError

# Use our custom resolver instead of the default A2A SDK resolver
A2ACardResolver: Final = DeRouterA2ACardResolver
...
def _set_usage_on_logging_obj(
    ...
    """
    Set usage on derouter_logging_obj for standard logging payload.

    Args:
        kwargs: The kwargs dict containing derouter_logging_obj
        prompt_tokens: Number of input tokens
        completion_tokens: Number of output tokens
    """
    derouter_logging_obj: Final = kwargs.get("derouter_logging_obj")
    if isinstance(derouter_logging_obj, Logging):
        usage: Final = derouter.Usage(
            prompt_tokens=prompt_tokens,
            completion_tokens=completion_tokens,
            total_tokens=prompt_tokens + completion_tokens,
        )
        derouter_logging_obj.model_call_details["usage"] = usage
...
def _set_agent_id_on_logging_obj(
    ...
    """
    Set agent_id on derouter_logging_obj for SpendLogs tracking.
    ...
    if isinstance(derouter_logging_obj, Logging):
        ...
        derouter_logging_obj.model_call_details["agent_id"] = agent_id


_A2A_COST_PARAM_KEYS: Final = ("cost_per_query", "input_cost_per_token", "output_cost_per_token")


def _set_derouter_params_on_logging_obj(
    kwargs: Mapping[str, object],
    derouter_params: Mapping[str, object],
) -> None:
    """
    Merge the agent's pricing params into model_call_details["derouter_params"]
    so A2ACostCalculator can read them.

    The non-streaming path reuses the proxy-built logging object, whose
    derouter_params already carries metadata / proxy_server_request / user-key
    context, so merge the pricing keys in rather than replacing the dict.
    """
    logging_obj: Final = kwargs.get("derouter_logging_obj")
    if not isinstance(logging_obj, Logging):
        return

    cost_params: Final = {
        key: derouter_params[key] for key in _A2A_COST_PARAM_KEYS if derouter_params.get(key) is not None
    }
    if not cost_params:
        return

    logging_obj.model_call_details["derouter_params"] = {
        **(logging_obj.model_call_details.get("derouter_params") or {}),
        **cost_params,
    }
...
def _get_a2a_model_info(a2a_client: "A2AClientType", kwargs: Mapping[str, object]) -> str:
    ...
    Sets model info on the derouter_logging_obj if available.
    ...
    # Set on derouter_logging_obj if available (for standard logging payload)
    derouter_logging_obj: Final = kwargs.get("derouter_logging_obj")
    if isinstance(derouter_logging_obj, Logging):
        derouter_logging_obj.model = model
        derouter_logging_obj.custom_llm_provider = custom_llm_provider
        derouter_logging_obj.model_call_details["model"] = model
        derouter_logging_obj.model_call_details["custom_llm_provider"] = custom_llm_provider

    return agent_name


def _get_a2a_client_agent_card(a2a_client: "A2AClientType") -> Optional["AgentCard"]:
    agent_card = cast(Optional["AgentCard"], getattr(a2a_client, "_derouter_agent_card", None))
    if agent_card is not None:
        return agent_card
...
async def _send_message_via_completion_bridge(
    ...
    api_base: str | None,
    derouter_params: dict[str, object],
    agent_extra_headers: dict[str, str] | None = None,
) -> DeRouterSendMessageResponse:
    """
    Route a send_message through the DeRouter completion bridge (e.g. LangGraph, Bedrock AgentCore).
    ...
    return DeRouterSendMessageResponse.from_dict(response_dict, request_id=str(request.id))


def _get_a2a_call_context(a2a_client: "A2AClientType") -> Optional["A2ACallContextType"]:
    return getattr(a2a_client, "_derouter_call_context", None)
...
async def asend_message(
    ...
    **kwargs: object,
    ...
    Uses the @client decorator for DeRouter logging and tracking.
    If derouter_params contains custom_llm_provider, routes through the completion bridge.
    ...
    Args:
        ...
        derouter_params: Optional dict with custom_llm_provider, model, etc. for completion bridge
        agent_id: Optional agent ID for tracking in SpendLogs
        **kwargs: Additional arguments passed to the client decorator

    Returns:
        DeRouterSendMessageResponse (wraps a2a SendMessageResponse with _hidden_params)

    Example (standard A2A):
        ```python
        from derouter.a2a_protocol import asend_message, create_a2a_client
    ...
    Example (completion bridge with LangGraph):
        ```python
        from derouter.a2a_protocol import asend_message
        from a2a.types import SendMessageRequest, MessageSendParams
        from uuid import uuid4
        ...
        response = await asend_message(
            ...
            derouter_params={"custom_llm_provider": "langgraph", "model": "agent"},
        )
        ```
    """
    derouter_params = derouter_params or {}
    logging_obj: Final = kwargs.get("derouter_logging_obj")
    trace_id = getattr(logging_obj, "derouter_trace_id", None) if logging_obj else None
    custom_llm_provider: Final = derouter_params.get("custom_llm_provider")
    ...
    if a2a_client is None:
        ...
        extra_headers: Final[dict[str, str]] = {"X-DeRouter-Trace-Id": trace_id}
        if agent_id:
            extra_headers["X-DeRouter-Agent-Id"] = agent_id
        # Overlay agent-level headers (agent headers take precedence over DeRouter internal ones)
    ...
    # Wrap in DeRouter response type for _hidden_params support
    response: Final = DeRouterSendMessageResponse.from_a2a_response(a2a_response, request_id=str(request.id))
    ...
    _set_derouter_params_on_logging_obj(kwargs=kwargs, derouter_params=derouter_params)
...
def send_message(
    ...
    request: "SendMessageRequest",
    **kwargs: Any,
) -> DeRouterSendMessageResponse | Coroutine[object, object, DeRouterSendMessageResponse]:
    """
...
def _build_streaming_logging_obj(
    ...
    agent_name: str,
    agent_id: str | None,
    derouter_params: dict[str, object] | None,
    ...
    logging_obj: Final = Logging(
        ...
        derouter_call_id=str(request.id),
    ...
    _derouter_params: Final = derouter_params.copy() if derouter_params else {}
    if metadata:
        _derouter_params["metadata"] = metadata
    if proxy_server_request:
        _derouter_params["proxy_server_request"] = proxy_server_request

    logging_obj.derouter_params = _derouter_params
    logging_obj.optional_params = _derouter_params
    logging_obj.model_call_details["derouter_params"] = _derouter_params
...
async def asend_message_streaming(
    ...
    **kwargs: object,
    ...
    Example (completion bridge with LangGraph):
        ```python
        from derouter.a2a_protocol import asend_message_streaming
    ...
    _raw_logging_obj: Final = kwargs.get("derouter_logging_obj")
    logging_obj: Logging | None = _raw_logging_obj if isinstance(_raw_logging_obj, Logging) else None

    if a2a_client is None:
        if api_base is None:
            raise ValueError("Either a2a_client or api_base is required for standard A2A flow")
        logging_trace_id: Final = getattr(logging_obj, "derouter_trace_id", None) if logging_obj else None
...
async def create_a2a_client(
    ...
    Example:
        ```python
        from derouter.a2a_protocol import create_a2a_client, asend_message
    ...
    # Stash DeRouter-owned handles on the client so the localhost-retry path can reuse
    # the configured httpx client and this agent's headers without excavating
    # a2a-sdk private internals.
    a2a_client._derouter_httpx_client = httpx_client
    a2a_client._derouter_call_context = (  # pyright: ignore[reportAttributeAccessIssue]  # DeRouter-owned stash
        ClientCallContext(service_parameters=extra_headers)  # pyright: ignore[reportOptionalCall]  # SDK checked above
        if extra_headers
        else None
    )
    a2a_client._derouter_agent_card = agent_card
...
async def aget_agent_card(
    ...
    # Use DeRouter's cached httpx client
