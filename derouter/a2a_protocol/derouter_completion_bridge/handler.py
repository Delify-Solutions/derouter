"""
Handler for A2A to DeRouter completion bridge.

Routes A2A requests through derouter.acompletion based on custom_llm_provider.
...
from derouter.a2a_protocol.providers.config_manager import A2AProviderConfigManager
from derouter.interactions.agents.utils import merge_agent_headers
...
# derouter_params key carrying the authenticated principal (hashed virtual key) so
...
A2A_USER_API_KEY_HASH_PARAM: Final = "derouter_a2a_user_api_key_hash"

# Agent metadata fields stored in derouter_params that are not valid derouter.acompletion() kwargs
...
class A2ACompletionBridgeHandler:
    """
    Static methods for handling A2A requests via DeRouter completion.
    """
    ...
    def _build_completion_params(
        ...
        *,
        ...
        model: Final[str] = derouter_params.get("model", "agent")
        ...
        # Add derouter_params (contains api_key, client_id, client_secret, tenant_id, etc.)
        derouter_params_to_add: Final = {
            k: v
            for k, v in derouter_params.items()
            if k not in ("model", "custom_llm_provider") and k not in _AGENT_ONLY_PARAMS
        }
        completion_params.update(derouter_params_to_add)
        # Apply forward metadata AFTER the derouter_params merge so the helper
    ...
    async def handle_non_streaming(
        ...
        *,
        ...
        """
        Handle non-streaming A2A request via derouter.acompletion.

        Args:
            request_id: A2A JSON-RPC request ID
            params: A2A MessageSendParams containing the message
            derouter_params: Agent's derouter_params (custom_llm_provider, model, etc.)
    ...
    async def handle_streaming(
        ...
        *,
        ...
        """
        Handle streaming A2A request via derouter.acompletion with stream=True.
        ...
        # Call derouter.acompletion with streaming
        response: Final = await A2ACompletionBridgeHandler._acompletion(completion_params)
