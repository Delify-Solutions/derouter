...
Maps A2A SDK exceptions to DeRouter A2A exception types.
"""
...
from derouter.a2a_protocol.exceptions import (
    A2AAgentCardError,
    A2AConnectionError,
    A2AError,
    A2ALocalhostURLError,
)
from derouter.constants import CONNECTION_ERROR_PATTERNS
...
def map_a2a_exception(
    ...
    """
    Map an A2A SDK exception to a DeRouter A2A exception type.
    ...
    Returns:
        A mapped DeRouter A2A exception
...
async def handle_a2a_localhost_retry(
    ...
    # Reuse the httpx client and call context DeRouter attached at creation, since the
    ...
    httpx_client: Final = getattr(a2a_client, "_derouter_httpx_client", None)
    if httpx_client is None:
        raise RuntimeError(
            "Cannot retry A2A localhost URL fix: the client was not created by "
            "create_a2a_client, so no DeRouter httpx client is attached."
        )
    ...
    new_client._derouter_httpx_client = httpx_client
    new_client._derouter_call_context = getattr(  # pyright: ignore[reportAttributeAccessIssue]  # DeRouter-owned stash
        a2a_client, "_derouter_call_context", None
    )
    new_client._derouter_agent_card = agent_card
    return new_client
