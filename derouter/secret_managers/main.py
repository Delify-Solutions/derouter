...
from derouter.secret_managers.secret_manager_handler import get_secret_from_manager
...
def _get_oidc_allowed_credential_dirs() -> list[str]:
    ...
    override via the ``DEROUTER_OIDC_ALLOWED_CREDENTIAL_DIRS`` environment
    variable (comma-separated list of absolute paths).
    """
    override: Final = os.getenv("DEROUTER_OIDC_ALLOWED_CREDENTIAL_DIRS")
    raw_dirs: Final = (
...
def _resolve_oidc_file_path(requested_path: str) -> str:
    ...
    raise ValueError(
        ...
        "Set DEROUTER_OIDC_ALLOWED_CREDENTIAL_DIRS to extend the allowlist."
    )
...
def get_secret(
    ...
    key_management_system: Final = derouter._key_management_system
    key_management_settings: Final = derouter._key_management_settings
    secret = None
    ...
    try:
        if _should_read_secret_from_secret_manager() and derouter.secret_manager_client is not None:
            try:
...
def _should_read_secret_from_secret_manager() -> bool:
    ...
    """
    key_management_settings: Final = derouter._key_management_settings
    if derouter.secret_manager_client is None or key_management_settings is None:
        return False
