...
Relevant issue: https://github.com/Delify-Solutions/derouter/issues/1883
...
def load_aws_kms(use_aws_kms: bool | None):
    ...
    try:
        ...
        derouter.secret_manager_client = kms_client
        derouter._key_management_system = KeyManagementSystem.AWS_KMS
...
class AWSKeyManagementService_V2:
    ...
    def validate_environment(
        ...
        is_derouter_license_in_env: bool = False

        if (
            os.getenv("DEROUTER_LICENSE", None) is not None
            or os.getenv("DEROUTER_SECRET_AWS_KMS_DEROUTER_LICENSE", None) is not None
        ):
            is_derouter_license_in_env = True
        if is_derouter_license_in_env is False:
            raise ValueError(
                "AWSKeyManagementService V2 is an Enterprise Feature. Please add a valid DEROUTER_LICENSE to your envionment."
            )
...
def decrypt_env_var() -> dict[str, Any]:
    ...
    for k, v in os.environ.items():
        if (k is not None and isinstance(k, str) and k.lower().startswith("derouter_secret_aws_kms")) or (
            v is not None and isinstance(v, str) and v.startswith("aws_kms/")
        ):
            decrypted_value = aws_kms.decrypt_value(secret_name=k)
            # reset env var
            k = re.sub("derouter_secret_aws_kms_", "", k, flags=re.IGNORECASE)
