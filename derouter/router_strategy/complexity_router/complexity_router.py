...
from derouter.derouter_core_utils.internal_call_metadata import forwarded_internal_call_metadata
from derouter.derouter_core_utils.prompt_templates.common_utils import (
    as_openai_image_part,
    request_contains_image_content,
)
from derouter.derouter_core_utils.sensitive_data_masker import mask_credentials_in_payload
from derouter.llms.base_llm.base_utils import type_to_response_format_param
from derouter.router_strategy.adaptive_router.classifier import classify_prompt
...
if TYPE_CHECKING:
    ...
    from derouter.router_strategy.savings_baseline import Baseline
...
def _parent_session_kwargs(request_kwargs: Mapping[str, Any] | None) -> Mapping[str, Any]:
    ...
    return {k: kwargs[k] for k in ("derouter_session_id", "derouter_trace_id") if kwargs.get(k) is not None}
...
def _is_classifier_timeout(exc: BaseException) -> bool:
    ...
    # DeRouter still supports 3.10, where they are distinct exception classes.
    if isinstance(exc, (TimeoutError, asyncio.TimeoutError)):
        return True
    from derouter.exceptions import Timeout as DeRouterTimeout

    return isinstance(exc, DeRouterTimeout)
...
class ComplexityRouter(CustomLogger):
    ...
    def __init__(
        ...
        Args:
            ...
            derouter_router_instance: The DeRouter Router instance.
    ...
    def savings_baseline(self) -> Baseline | None:
        ...
        """
        from derouter.router_strategy.savings_baseline import resolve_baseline

        if not self._derive_savings_baseline:
            return None
        if not self._savings_baseline_derived:
            self._savings_baseline = resolve_baseline(self.derouter_router_instance, self._hardest_tier_models())
    ...
    def _build_routing_decision(
        ...
        *,
        ...
        tier_derouter_params: Mapping[str, object] | None = None,
        ...
        if tier_derouter_params:
            masked_tier_derouter_params: Final = mask_credentials_in_payload(tier_derouter_params)
            if isinstance(masked_tier_derouter_params, Mapping):
                decision["tier_derouter_params"] = masked_tier_derouter_params
        return decision
    ...
    async def _classify_with_plugin(
        ...
        from derouter.derouter_core_utils.prompt_templates.factory import resolve_structured_messages
        from derouter.types.router import RoutingContext
    ...
    async def _classify_with_llm(
        ...
        request_metadata = (request_kwargs or {}).get("derouter_metadata") or (request_kwargs or {}).get("metadata")
        ...
        response: Final[ModelResponse] = await asyncio.wait_for(
            self.derouter_router_instance.acompletion(
                model=llm_config.model,
                messages=messages_for_call,
                stream=False,
                response_format=response_format,
    ...
    def _derouter_params_for_model(self, tier: ComplexityTier | str | None, model: str) -> Mapping[str, object]:
        ...
        explicit: Final = entry.derouter_params if entry is not None else MappingProxyType({})
    ...
    def _ensure_adaptive_router(self) -> Any | None:
        ...
        from derouter.router_strategy.adaptive_router.config import (
        ...
        model_list: Final = getattr(self.derouter_router_instance, "model_list", None) or []
        name_to_indices: Final = getattr(self.derouter_router_instance, "model_name_to_deployment_indices", {}) or {}
        for name in available_models:
            ...
            lp = deployment.get("derouter_params") if isinstance(deployment, dict) else deployment.derouter_params
    ...
    def _soft_floor_pick(
        ...
        from derouter.router_strategy.adaptive_router.bandit import (
            normalized_cost,
            thompson_sample,
        )
        from derouter.router_strategy.adaptive_router.classifier import classify_prompt
    ...
    def _deployment_limit(
        ...
        params: Final = derouter_params if isinstance(derouter_params, Mapping) else EMPTY_MAPPING
        ...
        if declared_authenticating_provider(
            ...
            return None
        try:
            model_info: Final = self.derouter_router_instance.get_router_model_info(
    ...
    def _group_deployments(self, group: str) -> Sequence[Mapping[str, object]]:
        list_models: Final = getattr(self.derouter_router_instance, "get_model_list", None)
    ...
    async def _counted_request_tokens(
        ...
        try:
            counted: Final = await asyncify(derouter.token_counter)(
                messages=cast(list, resolved_messages)  # cast-ok: token_counter only iterates the sequence
            )
            return counted + (await asyncify(derouter.token_counter)(text=out_of_band) if out_of_band else 0)
    ...
    def _matched_escalation_keyword(self, user_message: str) -> str | None:
        ...
        Matching is a case-sensitive substring test so the default "DEROUTER ESCALATE"
    ...
    def _vision_verdicts(self, model_name: str) -> tuple[bool | None, ...]:
        ...
        """
        from derouter.utils import is_vision_explicitly_disabled, supports_vision
        ...
        def deployment_verdict(deployment: Mapping[str, Any]) -> bool | None:
            ...
            return model_verdict((deployment.get("derouter_params") or EMPTY_MAPPING).get("model") or model_name)

        deployments: Final = self.derouter_router_instance.get_model_list(model_name=model_name)
        if not deployments:
    ...
    async def _gate_response_modality(
        ...
        new_decision: Final = self._build_routing_decision(
            ...
            tier_derouter_params=self._derouter_params_for_model(new_tier, new_model),
            context_escalation_original_tier=(
                decision.get("context_escalation_original_tier") if decision is not None else None
            ),
        )
        from derouter.types.router import PreRoutingHookResponse as HookResponse

        return HookResponse(
            model=new_model,
            messages=response.messages,
            derouter_params=self._derouter_params_for_model(new_tier, new_model),
    ...
    async def _model_group_can_serve(
        ...
        from derouter.types.router import RouterRateLimitError, RouterRateLimitErrorBasic

        probe_kwargs: Final = dict(request_kwargs)  # mutable-ok: the owner pops routing keys off the dict it is handed
        try:
            deployments: Final = await self.derouter_router_instance.async_get_healthy_deployments(
                model=model_name,
    ...
    async def _gate_response_health(
        ...
        new_decision: Final = self._build_routing_decision(
            ...
            conversation_continuing=bool(decision.get("conversation_continuing", True)),
            tier_derouter_params=self._derouter_params_for_model(decided_tier, new_model),
        ...
        return response.model_copy(
            update={  # mutable-ok: model_copy types update as a plain dict
                "model": new_model,
                "derouter_params": self._derouter_params_for_model(decided_tier, new_model),
                "routing_decision": new_decision,
            }
        )
    ...
    def _get_or_create_semantic_routelayer(self) -> SemanticRouter:
        ...
        from derouter.router_strategy.auto_router.derouter_encoder import (
            DeRouterRouterEncoder,
        )
        ...
        routelayer: Final = SemanticRouter(
            routes=routes,
            encoder=DeRouterRouterEncoder(
                derouter_router_instance=self.derouter_router_instance,
    ...
    async def _semantic_tier_override(self, user_message: str, request_kwargs: dict) -> ComplexityTier | str | None:
        ...
        internally) so the caller's metadata/derouter_metadata flows into aembedding()
        and this spend is attributed and budget-checked against the originating key/team,
        the same as any other derouter call. SemanticRouter.acall() has no parameter to
        ...
        encoder: Final = cast(DeRouterRouterEncoder, routelayer.encoder)  # cast-ok: always the encoder we built above
        ...
        derouter_metadata: Final = forwarded_internal_call_metadata(
            request_kwargs.get("derouter_metadata"), AUTOROUTER_CLASSIFIER_CALL_ORIGIN
        )
        turn_off_message_logging: Final = _effective_turn_off_message_logging(request_kwargs)
    ...
    def _iter_metadata_dicts(request_kwargs: dict) -> list[dict]:
        """Metadata may land on `metadata` or `derouter_metadata` depending on the
    ...
    async def async_pre_routing_hook(
        ...
        if self.config.return_raw_model_name:
            metadata_key: Final = "derouter_metadata" if "derouter_metadata" in request_kwargs else "metadata"
        ...
        if cache_key is not None and pin_replay_allowed:
            pinned_value: Final = await self.derouter_router_instance.cache.async_get_cache(key=cache_key)
            ...
            if pinned_pin is not None:
                ...
                if routed_model is not None:
                    ...
                    await self.derouter_router_instance.cache.async_set_cache(
                        key=cache_key,
                    ...
                    session_tier_derouter_params: Final = self._derouter_params_for_model(routed_pin_tier, routed_model)
                        ...
                        await self._gate_response_health(
                            await self._gate_response_modality(
                                PreRoutingHookResponse(
                                    ...
                                    derouter_params=session_tier_derouter_params,
                                    routing_decision=self._build_routing_decision(
                                        ...
                                        escalated=escalated,
                                        conversation_continuing=conversation_continuing,
                                        tier_derouter_params=session_tier_derouter_params,
    ...
    async def _classify_and_route(
        ...
        if user_message is None and not classifier_images:
            ...
            default_tier_params: Final = self._derouter_params_for_model(fallback_tier, routed_model)
            return PreRoutingHookResponse(
                ...
                routing_decision=self._build_routing_decision(
                    ...
                    tier=fallback_tier,
                    conversation_continuing=conversation_continuing,
                    tier_derouter_params=default_tier_params,
                ),
                derouter_params=default_tier_params,
            )
        ...
        if plan_floor is not None and plan_mode_sentinel is not None and self._plan_mode_floor_is_top_tier():
            ...
            plan_tier_params: Final = self._derouter_params_for_model(plan_floor, routed_model)
            return PreRoutingHookResponse(
                model=routed_model,
                messages=messages if has_original_messages else None,
                routing_decision=self._build_routing_decision(
                    ...
                    escalated=False,
                    tier_derouter_params=plan_tier_params,
                ),
                derouter_params=plan_tier_params,
            )
        ...
        if override is not None:
            ...
            keyword_tier_derouter_params: Final = self._derouter_params_for_model(routed_tier, routed_model)
            ...
            return PreRoutingHookResponse(
                model=routed_model,
                messages=messages if has_original_messages else None,
                derouter_params=keyword_tier_derouter_params,
                routing_decision=self._build_routing_decision(
                    routed_model=routed_model,
                    conversation_continuing=conversation_continuing,
                    cause=keyword_cause,
                    ...
                    tier_derouter_params=keyword_tier_derouter_params,
                ),
            )
        ...
        if outcome.cause == "default_model_fallback" and fallback_model is not None and plan_mode_sentinel is None:
            ...
            fallback_tier_params: Final = self._derouter_params_for_model(None, fallback_model)
            return PreRoutingHookResponse(
                ...
                routing_decision=self._build_routing_decision(
                    ...
                    escalation_keyword=escalation_keyword,
                    escalated=False,
                    tier_derouter_params=fallback_tier_params,
                ),
                derouter_params=fallback_tier_params,
            )
        if self.config.adaptive:
        ...
        tier_derouter_params: Final = self._derouter_params_for_model(tier, routed_model)
        ...
        return PreRoutingHookResponse(
            ...
            routing_decision=self._build_routing_decision(
                ...
                tier_derouter_params=tier_derouter_params,
