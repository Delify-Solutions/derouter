...
from derouter.types.llms.openai import REASONING_EFFORT
from derouter.types.router import AdaptiveRouterWeights, ClassifierPlugin, RoutingPlugin
...
class ComplexityTierModel(BaseModel):
    ...
    model_name: str
    derouter_params: Annotated[Mapping[str, object], SkipValidation()] = Field(
        default_factory=lambda: MappingProxyType({})
    )

    @field_validator("derouter_params", mode="before")
    @classmethod
    def _freeze_derouter_params(cls, value: Mapping[str, object]) -> Mapping[str, object]:
        return MappingProxyType(dict(value))

    @field_serializer("derouter_params")
    def _serialize_derouter_params(self, value: Mapping[str, object]) -> Mapping[str, object]:
...
DEFAULT_ESCALATION_KEYWORDS: Final[list[str]] = ["DEROUTER ESCALATE"]
...
class ComplexityRouterConfig(BaseModel):
    ...
    escalation_keywords: list[str] | None = Field(
        ...
        description=(
            ...
            "model, but not choose which one). Defaults to ['DEROUTER ESCALATE'] when unset; "
    ...
    max_tokens_from_tier_model: bool = Field(
        ...
        description=(
            ...
            "max_completion_tokens or max_output_tokens in the tier's own derouter_params still "
    ...
    embedding_model: str | None = Field(
        default=None,
        description="Embedding model (DeRouter model name) used when semantic_keyword_matching is enabled",
    )
    ...
    def _normalize_tier_model_configs(cls, value: object) -> object:
        ...
        incoming_params: Final = (
            MappingProxyType(
                {
                    (tier, entry.model_name): entry.derouter_params
        ...
        tier_model_configs: Final = MappingProxyType(
            {
                tier: tuple(
                    entry.model_copy(
                        update=MappingProxyType(
                            {
                                "derouter_params": incoming_params.get((tier, entry.model_name), entry.derouter_params),
                            }
                        )
                    )
                ...
                if any(entry.derouter_params for entry in entries)
    ...
    def _validate_tier_param_placement(self) -> "ComplexityRouterConfig":
        ...
        A tier entry's ``derouter_params`` are request params for that deployment: the
        ...
        misplaced: Final = tuple(
            ...
            for entry in entries
            for key in sorted(frozenset(entry.derouter_params) & COMPLEXITY_ROUTER_CONFIG_KEYS)
        )
        if misplaced:
            raise ValueError(
                "tier entries carry complexity_router_config settings in their derouter_params, where the "
...
These names are disjoint from the OpenAI request params, from ``all_derouter_params``, and from the
``DeRouter_Params`` fields, so one of them appearing where a request param belongs is always a
