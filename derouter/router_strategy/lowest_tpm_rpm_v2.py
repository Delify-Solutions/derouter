...
from derouter.types.utils import DeRouterPydanticObjectBase, StandardLoggingPayload
from derouter.utils import get_utc_datetime, print_verbose
...
class RoutingArgs(DeRouterPydanticObjectBase):
...
class LowestTPMLoggingHandler_v2(BaseRoutingStrategy, CustomLogger):
    ...
    def pre_call_check(self, deployment: dict) -> dict | None:
        ...
        try:
            ...
            deployment_name: Final = deployment.get("derouter_params", {}).get("model")
            ...
            if deployment_rpm is None:
                deployment_rpm = deployment.get("derouter_params", {}).get("rpm")
            if deployment_rpm is None:
            ...
            if local_result is not None and local_result >= deployment_rpm:
                raise derouter.RateLimitError(
                    ...
                    llm_provider="",
                    model=deployment.get("derouter_params", {}).get("model"),
            ...
            if isinstance(e, derouter.RateLimitError):
                raise e
    ...
    async def async_pre_call_check(self, deployment: dict, parent_otel_span: Span | None) -> dict | None:
        ...
        Why? solves concurrency issue - https://github.com/Delify-Solutions/derouter/issues/2994
    ...
    def log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            model: Final = standard_logging_object["hidden_params"].get("derouter_model_name")
            ...
            verbose_logger.exception(
                "derouter.proxy.hooks.lowest_tpm_rpm_v2.py::log_success_event(): Exception occured - %s", e
            )
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            if standard_logging_object is None:
                raise ValueError("standard_logging_object not passed in.")
            model_group: Final = standard_logging_object.get("model_group")
            model: Final = standard_logging_object["hidden_params"]["derouter_model_name"]
            id = standard_logging_object.get("model_id")
            ...
            verbose_logger.exception(
                "derouter.proxy.hooks.lowest_tpm_rpm_v2.py::async_log_success_event(): Exception occured - %s", e
            )
    ...
    def _return_potential_deployments(
        ...
        for item, item_tpm in all_deployments.items():
            ...
            if _deployment_tpm is None:
                _deployment_tpm = _deployment.get("derouter_params", {}).get("tpm")
            if _deployment_tpm is None:
            ...
            if _deployment_rpm is None:
                _deployment_rpm = _deployment.get("derouter_params", {}).get("rpm")
            if _deployment_rpm is None:
    ...
    async def async_get_available_deployments(
        ...
        for m in healthy_deployments:
            if isinstance(m, dict):
                ...
                deployment_name = m.get("derouter_params", {}).get("model")
        ...
        try:
            ...
            for index, _deployment in enumerate(healthy_deployments):
                if isinstance(_deployment, dict):
                    ...
                    if _deployment_tpm is None:
                        _deployment_tpm = _deployment.get("derouter_params", {}).get("tpm", None)
                    if _deployment_tpm is None:
                    ...
                    if _deployment_rpm is None:
                        _deployment_rpm = _deployment.get("derouter_params", {}).get("rpm", None)
                    if _deployment_rpm is None:
