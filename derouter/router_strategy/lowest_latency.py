...
from derouter.derouter_core_utils.core_helpers import _get_parent_otel_span_from_kwargs, safe_divide_seconds
from derouter.types.utils import DeRouterPydanticObjectBase
...
class LowestLatencyLoggingHandler(CustomLogger):
    ...
    def log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            if kwargs["derouter_params"].get(metadata_field) is None:
                pass
            else:
                model_group: Final = kwargs["derouter_params"][metadata_field].get("model_group", None)

                id = (kwargs["derouter_params"].get("model_info") or {}).get("id", None)
    ...
    async def async_log_failure_event(self, kwargs, response_obj, start_time, end_time):
        ...
        try:
            ...
            if isinstance(_exception, derouter.Timeout):
                if kwargs["derouter_params"].get(metadata_field) is None:
                    pass
                else:
                    model_group: Final = kwargs["derouter_params"][metadata_field].get("model_group", None)
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            verbose_logger.exception(
                "derouter.router_strategy.lowest_latency.py::async_log_success_event(): Exception occured - %s", e
            )
