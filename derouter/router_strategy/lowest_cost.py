...
import derouter
from derouter import ModelResponse, token_counter, verbose_logger
...
class LowestCostLoggingHandler(CustomLogger):
    ...
    def log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            if kwargs["derouter_params"].get("metadata") is None:
                ...
                id = kwargs["derouter_params"].get("model_info", {}).get("id", None)
                ...
                if isinstance(response_obj, ModelResponse):
                    ...
                    if _usage is not None and isinstance(_usage, derouter.Usage):
            ...
            verbose_logger.exception(
                "derouter.router_strategy.lowest_cost.py::log_success_event(): Exception occured - %s", e
            )
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            verbose_logger.exception(
                "derouter.proxy.hooks.prompt_injection_detection.py::async_pre_call_hook(): Exception occured - %s", e
            )
    ...
    async def async_get_available_deployments(
        ...
        for item, item_map in all_deployments.items():
            ...
            _deployment_tpm = (
                ...
                or _deployment.get("derouter_params", {}).get("tpm", None)
            ...
            _deployment_rpm = (
                ...
                or _deployment.get("derouter_params", {}).get("rpm", None)
                or _deployment.get("model_info", {}).get("rpm", None)
                or float("inf")
            )
            item_derouter_model_name = _deployment.get("derouter_params", {}).get("model")
            item_derouter_model_cost_map = derouter.model_cost.get(item_derouter_model_name, {})

            # check if user provided input_cost_per_token and output_cost_per_token in derouter_params
            item_input_cost = None
            item_output_cost = None
            if _deployment.get("derouter_params", {}).get("input_cost_per_token", None):
                item_input_cost = _deployment.get("derouter_params", {}).get("input_cost_per_token")

            if _deployment.get("derouter_params", {}).get("output_cost_per_token", None):
                item_output_cost = _deployment.get("derouter_params", {}).get("output_cost_per_token")

            if item_input_cost is None:
                item_input_cost = item_derouter_model_cost_map.get("input_cost_per_token", 5.0)

            if item_output_cost is None:
                item_output_cost = item_derouter_model_cost_map.get("output_cost_per_token", 5.0)

            # if derouter["model"] is not in model_cost map -> use item_cost = $10
            ...
            _deployment_api_base = _deployment.get("derouter_params", {}).get("api_base", "")
