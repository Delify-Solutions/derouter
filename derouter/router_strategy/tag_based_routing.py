...
from derouter.derouter_core_utils.core_helpers import get_metadata_variable_name_from_kwargs
from derouter.types.router import ConsumedRequestTagsStamp, DeploymentTypedDict, RouterErrors
...
class _DeploymentLike(Protocol):
    @overload
    def get(self, key: Literal["derouter_params"], default: Mapping[str, object], /) -> _TagLitellmParamsLike: ...
    @overload
...
class _NestedLitellmParamsLike(Protocol):
    def get(
        self, key: Literal["metadata", "derouter_metadata"], default: Mapping[str, object], /
    ) -> _MetadataLike | None: ...
...
class _RequestKwargsLike(Protocol):
    ...
    def get(self, key: Literal["enable_tag_filtering"], /) -> bool | None: ...
    @overload
    def get(self, key: Literal["metadata", "derouter_metadata"], /) -> _MetadataLike | None: ...
    def __contains__(self, key: object, /) -> bool: ...
    @overload
    def __getitem__(self, key: Literal["metadata", "derouter_metadata"], /) -> _MetadataLike: ...
    @overload
    def __getitem__(self, key: Literal["derouter_params"], /) -> _NestedLitellmParamsLike: ...
...
def _match_deployment(
    ...
    deployment_tags: Final[Sequence[str] | None] = derouter_params.get("tags")
    deployment_tag_regex: Final[Sequence[str] | None] = derouter_params.get("tag_regex")
...
def _exclude_deployments(
    ...
    return [d for d in deployments if not excluded_set.intersection(d.get("derouter_params", {}).get("tags") or [])]
...
def _require_all_tags(
    ...
    return tuple(d for d in deployments if required_set.issubset(d.get("derouter_params", {}).get("tags") or []))


def _default_tagged_pool(
    deployments: _DeploymentPool,
) -> tuple[_DeploymentLike, ...]:
    defaults: Final = tuple(d for d in deployments if "default" in (d.get("derouter_params", {}).get("tags") or []))
    return defaults if defaults else tuple(deployments)


def _known_tag_values(deployments: _DeploymentPool) -> frozenset[str]:
    return frozenset(
        tag for d in deployments for tag in (d.get("derouter_params", MappingProxyType({})).get("tags") or ())
    )
...
def _inherited_constraint_sets(
    ...
    # merged in on top (see derouter_pre_call_utils.py), so a value present here is
...
def _tag_known_to_group(
    ...
    try:
        ...
        return False
    return any(
        tag_set.intersection(d.get("derouter_params", MappingProxyType({})).get("tags") or ()) for d in all_deployments
    )
...
async def get_deployments_for_tag(
    ...
    metadata_variable_name: Literal["metadata", "derouter_metadata"] = "metadata",
) -> _DeploymentPool:
    """
    ...
    if metadata_variable_name in request_kwargs:
        ...
        has_regex_deployments: Final = any(d.get("derouter_params", {}).get("tag_regex") for d in candidates)
        ...
        if has_positive_filter:
            ...
            default_deployments: Final = [
                d for d, _ in deployment_matches if "default" in (d.get("derouter_params", {}).get("tags") or ())
            ]
    ...
    _default_deployments_with_tags: Final = [
        ...
        if "default" in deployment.get("derouter_params", {}).get("tags", [])
    ]
...
def _get_tags_from_request_kwargs(
    ...
    metadata_variable_name: Literal["metadata", "derouter_metadata"] | None = None,
) -> list[str]:
    """
    ...
    Args:
        ...
        metadata_variable_name: Which metadata dict holds proxy metadata; resolved
            ...
            (``derouter_metadata``) read the same bucket the proxy wrote tags to
    ...
    if "derouter_params" in request_kwargs:
        derouter_params: Final = request_kwargs["derouter_params"]
        if not isinstance(derouter_params, Mapping):
            return []
        typed_derouter_params: Final[Mapping[str, object]] = derouter_params
        return _tags_in_metadata(typed_derouter_params.get(resolved_variable_name))
    return []
