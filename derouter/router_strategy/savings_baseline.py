...
def canonical_model(model: str, custom_llm_provider: str | None = None) -> str | None:
    ...
    try:
        resolved, provider, _, _ = derouter.get_llm_provider(model=model, custom_llm_provider=custom_llm_provider)
...
def _models_in(router: "Router", group_name: str) -> tuple[Baseline, ...]:
    ...
    `derouter_params.model` is not always a model: on Azure it is the deployment name,
