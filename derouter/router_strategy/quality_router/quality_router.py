...
`model_info.derouter_routing_preferences`.

Optional keyword override: deployments may also declare `keywords` in
`derouter_routing_preferences`. If any declared keyword appears in the user
...
class QualityRouter(CustomLogger):
    ...
    def _get_routing_preferences(self, deployment: object) -> dict[str, Any] | None:
        """
        Extract derouter_routing_preferences from a deployment, handling both
        ...
        if isinstance(deployment, dict):
            ...
            if isinstance(model_info, dict):
                return model_info.get("derouter_routing_preferences")
            # Pydantic ModelInfo nested in a dict.
            return getattr(model_info, "derouter_routing_preferences", None)
    ...
    def _build_tier_index(self) -> dict[int, list[str]]:
        ...
        missing `derouter_routing_preferences`.
        """
        ...
        for deployment in model_list:
            ...
            if raw_prefs is None:
                raise ValueError(
                    ...
                    f"but has no model_info.derouter_routing_preferences"
                )
            ...
            try:
                ...
                raise ValueError(f"QualityRouter: model '{name}' has invalid derouter_routing_preferences: {e}") from e
    ...
    def _stash_decision(
        ...
        lift it into response headers (`x-derouter-quality-router-*`). The same
