...
from derouter.caching.redis_cache import RedisPipelineIncrementOperation
from derouter.integrations.custom_logger import CustomLogger, Span
...
from derouter.router_utils.cooldown_callbacks import (
    _get_prometheus_logger_from_callbacks,
)
from derouter.types.llms.openai import AllMessageValues
from derouter.types.router import DeploymentTypedDict, DeRouter_Params, RouterErrors
from derouter.types.utils import BudgetConfig, GenericBudgetConfigType, StandardLoggingPayload
from derouter.types.utils import BudgetConfig as GenericBudgetInfo

DEFAULT_REDIS_SYNC_INTERVAL: Final = 1


class _DeRouterParamsDictView:
    """
    Lightweight attribute view over `derouter_params` dict.

    This avoids pydantic construction in request hot-path while preserving
    attribute-style access used by `derouter.get_llm_provider(...)`.
    """
...
class RouterBudgetLimiting(CustomLogger):
    def __init__(
        ...
        # Add self to derouter callbacks if it's a list
        if isinstance(derouter.callbacks, list):
            derouter.logging_callback_manager.add_derouter_callback(self)
    ...
    def _filter_out_deployments_above_budget(
        ...
        for idx, deployment in enumerate(healthy_deployments):
            ...
            if self.deployment_budget_config and is_within_budget:
                ...
                _derouter_params = deployment.get("derouter_params") or {}
                _derouter_model_name = _derouter_params.get("model")
                ...
                if model_id in deployment_configs:
                    ...
                    if config.max_budget and current_spend >= config.max_budget:
                        debug_msg = f"Exceeded budget for deployment model_name: {_model_name}, derouter_params.model: {_derouter_model_name}, model_id: {model_id}: {current_spend} >= {config.budget_duration}"
    ...
    async def _increment_spend_in_current_window(self, spend_key: str, response_cost: float, ttl: int):
        ...
        - Queues the increment operation to Redis Pipeline (using batched pipeline to optimize performance. Using Redis for multi instance environment of DeRouter)
        """
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        custom_llm_provider: Final[str] = kwargs.get("derouter_params", {}).get("custom_llm_provider", None)
    ...
    def _get_llm_provider_for_deployment(self, deployment: dict) -> str | None:
        try:
            deployment_derouter_params: Final = deployment.get("derouter_params") or {}

            if isinstance(deployment_derouter_params, DeRouter_Params):
                model = deployment_derouter_params.model or ""
                provider_resolution_params: Any = deployment_derouter_params
            elif isinstance(deployment_derouter_params, dict):
                model = deployment_derouter_params.get("model") or ""
                provider_resolution_params = _DeRouterParamsDictView(deployment_derouter_params)
            else:
                model = ""
                provider_resolution_params = _DeRouterParamsDictView({})

            _, custom_llm_provider, _, _ = derouter.get_llm_provider(
                model=str(model),
                derouter_params=provider_resolution_params,
            )
        except Exception:
    ...
    def should_init_router_budget_limiter(
        ...
        if derouter.tag_budget_config is not None:
            return True
        ...
        for _model in model_list:
            _derouter_params = _model.get("derouter_params", {})
            if _derouter_params.get("max_budget") or _derouter_params.get("budget_duration") is not None:
                return True
        return False
    ...
    def _init_deployment_budgets(
        ...
        for _model in model_list:
            ...
            _max_budget = _derouter_params.get("max_budget")
            _budget_duration = _derouter_params.get("budget_duration")
    ...
    def _init_tag_budgets(self):
        if derouter.tag_budget_config is None:
            return
        ...
        for _tag, _tag_budget_config in derouter.tag_budget_config.items():
