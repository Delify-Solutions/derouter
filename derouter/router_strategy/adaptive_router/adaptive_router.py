...
from derouter.router_strategy.adaptive_router.signals import (
    SessionState,
    SignalDelta,
    Turn,
...
from derouter.router_strategy.adaptive_router.update_queue import (
...
from derouter.repositories.table_repositories import AdaptiveRouterStateRepository
...
class AdaptiveRouter:
    ...
    async def async_pre_routing_hook(
        ...
        # it as the `x-derouter-adaptive-router-model` response header. We use
        # `metadata` (not a top-level kwarg) so the value doesn't leak into
        # `derouter.acompletion(**input_kwargs)`.
    ...
    def _extract_min_quality_tier(
        ...
        Precedence: headers (`x-derouter-min-quality-tier`) over metadata
