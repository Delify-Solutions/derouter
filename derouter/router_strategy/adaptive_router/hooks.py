...
def _resolve_session_key(kwargs: dict[str, Any]) -> str | None:
    ...
    Order:
      1. Honor a client-supplied session id (`derouter_session_id` on either
         `derouter_params` or `derouter_params.metadata`, or `session_id` on
    ...
    sid = derouter_params.get("derouter_session_id")
    if sid:
        return str(sid)
    metadata: Final = derouter_params.get("metadata") or {}
    if isinstance(metadata, dict):
        sid = metadata.get("session_id") or metadata.get("derouter_session_id")
        if sid:
            return str(sid)
...
class AdaptiveRouterPostCallHook(CustomLogger):
    """One hook instance per AdaptiveRouter. Registered into derouter.callbacks."""
    ...
    async def async_post_call_response_headers_hook(
        ...
        """
        Surface the chosen logical model as the `x-derouter-adaptive-router-model`
