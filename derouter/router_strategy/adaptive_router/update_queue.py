...
class AdaptiveRouterUpdateQueue:
    ...
    async def add_session_state(
        ...
        upsert this into DeRouter_AdaptiveRouterSession.
        """
    ...
    async def flush_state_to_db(self, prisma_client: Any) -> int:
        """
        Drain state aggregator and apply to DeRouter_AdaptiveRouterState.
    ...
    async def flush_session_to_db(self, prisma_client: Any) -> int:
        """
        Drain session aggregator and upsert into DeRouter_AdaptiveRouterSession.
        ...
        for key in sorted(batch.keys()):
            ...
            try:
                ...
                # `DeRouter_AdaptiveRouterSession` -> `derouter_adaptiveroutersession`
                # (single 's', not 'derouter_adaptiverouterssession').
