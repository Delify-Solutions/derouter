...
class _CallKwargs(TypedDict, total=False):
    derouter_params: ReadOnly[_LitellmParams | None]
...
def _deployment_ref(kwargs: Mapping[str, object]) -> tuple[str, str] | None:
    try:
        ...
        return None
    derouter_params: Final = call.get("derouter_params")
    metadata: Final = derouter_params.get("metadata") if derouter_params else None
    model_info: Final = derouter_params.get("model_info") if derouter_params else None
