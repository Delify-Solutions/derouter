...
from derouter.llms.xai.common_utils import XAIModelInfo
from derouter.secret_managers.main import get_secret_str
from derouter.types.llms.vertex_ai import VERTEX_CREDENTIALS_TYPES, VertexAccessTokenResolver
from derouter.types.realtime import (
...
from derouter.types.utils import CallTypes, LlmProviders
from derouter.utils import ProviderConfigManager

from ..derouter_core_utils.get_derouter_params import get_derouter_params
from ..derouter_core_utils.derouter_logging import Logging as DeRouterLogging
...
def _build_derouter_metadata(kwargs: dict) -> dict:
    """Build the derouter_metadata dict for guardrail checking (internal only, not forwarded to provider)."""
...
def _get_realtime_http_provider_config(
    ...
    raw_api_base: Final = dynamic_api_base or derouter_params.api_base
    raw_api_key: Final = dynamic_api_key or derouter_params.api_key
    ...
    if provider_config is not None:
        ...
        resolved_api_base = raw_api_base or derouter.api_base or "https://api.openai.com"
        resolved_api_key = (
            raw_api_key or derouter.api_key or derouter.openai_key or get_secret_str("OPENAI_API_KEY") or ""
        )
...
async def acreate_realtime_client_secret(
    ...
    **kwargs,
    ...
    derouter_logging_obj: Final[DeRouterLogging] = kwargs.get("derouter_logging_obj")
    ...
    derouter_logging_obj.update_from_kwargs(
        ...
        derouter_params={"api_base": resolved_api_base},
        custom_llm_provider=custom_llm_provider,
    )
    ...
    return await base_llm_http_handler.async_realtime_client_secret_handler(
        ...
        api_version=derouter_params.api_version,
    )
...
async def acreate_realtime_transcription_session(
    ...
    **kwargs,
    ...
    input_audio_transcription, turn_detection, …). ``model`` is a DeRouter-only
...
async def _arealtime(
    ...
    **kwargs,
    ...
    derouter_params_dict: Final = {**get_derouter_params(**kwargs), CallTypes.arealtime.value: True}
    ...
    if provider_config is not None:
        await base_llm_http_handler.async_realtime(
            ...
            derouter_metadata=_build_derouter_metadata(kwargs),
            query_params=query_params,
        )
    elif _custom_llm_provider in AZURE_OPENAI_AUDIO_PROVIDERS:
        api_base = dynamic_api_base or derouter_params.api_base or derouter.api_base or get_secret_str("AZURE_API_BASE")
        # set API KEY
        api_key = dynamic_api_key or derouter.api_key or derouter.openai_key or get_secret_str("AZURE_API_KEY")

        api_version = api_version or derouter_params.api_version or "2024-10-01-preview"

        realtime_protocol = (
            kwargs.get("realtime_protocol")
            or derouter_params.get("realtime_protocol")
            or os.environ.get("DEROUTER_AZURE_REALTIME_PROTOCOL")
        )
        ...
        resolved_azure_ad_token: Final = (
            None if api_key else get_azure_ad_token(GenericDeRouterParams(**kwargs, azure_ad_token=azure_ad_token))
        )
        ...
        api_base = dynamic_api_base or derouter_params.api_base or derouter.api_base or "https://api.openai.com/"
        # set API KEY
        api_key = dynamic_api_key or derouter.api_key or derouter.openai_key or get_secret_str("OPENAI_API_KEY")
        ...
        api_base = (
            dynamic_api_base or derouter_params.api_base or get_secret_str("XAI_API_BASE") or "https://api.x.ai/v1"
        )
        # set API KEY
        ...
        await base_llm_http_handler.async_realtime(
            ...
            api_base=dynamic_api_base or derouter_params.api_base,
            api_key=None,
            client=client,
            timeout=timeout,
            headers=headers,
...
def _is_transcription_only_realtime_model(model: str, custom_llm_provider: str) -> bool:
    try:
        model_info: Final = derouter.get_model_info(model=model, custom_llm_provider=custom_llm_provider)
...
def _azure_realtime_health_protocol(
    ...
    configured_raw: Final = (
        realtime_protocol or model_params.get("realtime_protocol") or os.environ.get("DEROUTER_AZURE_REALTIME_PROTOCOL")
    )
...
def _realtime_health_check_auth_headers(
    ...
    return azure_realtime.get_auth_headers(
        api_key=api_key,
        azure_ad_token=(None if api_key else get_azure_ad_token(GenericDeRouterParams(**model_params))),
    )
