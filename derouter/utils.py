"""Utility helpers for DeRouter core request handling and provider support."""
...
# What is this?
## Generic utils.py file. Problem-specific utils (e.g. 'cost calculation), should all be in `derouter_core_utils/`.
import sys
import textwrap
import threading
import time
...
import derouter
import derouter.derouter_core_utils

# audio_utils.utils is lazy-loaded - only imported when needed for transcription calls
import derouter.derouter_core_utils.json_validation_rule
from derouter._internal_context import is_internal_call
from derouter._lazy_imports import (
...
from derouter.derouter_core_utils.core_helpers import normalize_drop_params
from derouter.derouter_core_utils.fallback_generalizations import (
    match_capability_generalizations,
)
from derouter.derouter_core_utils.sensitive_data_masker import redact_credentials_in_payload
...
def _snapshot_exception_for_hook(exception: Exception) -> Exception:
    """
    A same-class copy of exception that skips __init__ (many derouter exceptions require
...
def _get_cached_caching_handler_response():
    ...
    if _CachingHandlerResponse is None:
        from derouter.caching.caching_handler import CachingHandlerResponse
...
def _get_cached_audio_utils():
    ...
    if _audio_utils_module is None:
        import derouter.derouter_core_utils.audio_utils.utils

        _audio_utils_module = derouter.derouter_core_utils.audio_utils.utils
...
try:
    # Python 3.9+
    with (
        resources.files("derouter.derouter_core_utils.tokenizers")
        .joinpath("anthropic_tokenizer.json")
        .open("r", encoding="utf-8") as f
    ):
        json_data = json.load(f)
except (ImportError, AttributeError, TypeError):
    with resources.open_text("derouter.derouter_core_utils.tokenizers", "anthropic_tokenizer.json") as f:
        json_data = json.load(f)
...
from derouter.llms.base_llm.base_utils import (
    BaseLLMModelInfo,
...
if TYPE_CHECKING:
    ...
    # their modules at runtime during `derouter` import.
    from derouter.caching.caching_handler import (
    ...
    from derouter.derouter_core_utils.cached_imports import (
        get_coroutine_checker,
        get_derouter_logging_class,
        get_set_callbacks,
    )
    from derouter.derouter_core_utils.core_helpers import (
        get_derouter_metadata_from_kwargs,
        map_finish_reason,
        process_response_headers,
    )
    from derouter.derouter_core_utils.credential_accessor import CredentialAccessor
    from derouter.derouter_core_utils.dot_notation_indexing import (
        delete_nested_value,
        is_nested_path,
    )
    ...
    from derouter.derouter_core_utils.exception_mapping_utils import exception_type
    from derouter.derouter_core_utils.get_derouter_params import (
        _get_base_model_from_derouter_call_metadata,
        get_derouter_params,
    )
    from derouter.derouter_core_utils.get_llm_provider_logic import (
        _is_non_openai_azure_model,
        get_llm_provider,
    )
    from derouter.derouter_core_utils.get_supported_openai_params import (
        get_supported_openai_params,
    )
    from derouter.derouter_core_utils.llm_request_utils import _ensure_extra_body_is_safe
    from derouter.derouter_core_utils.llm_response_utils.convert_dict_to_response import (
        DeRouterResponseObjectHandler,
        _handle_invalid_parallel_tool_calls,
        convert_to_model_response_object,
        convert_to_streaming_response,
        convert_to_streaming_response_async,
    )
    from derouter.derouter_core_utils.llm_response_utils.get_api_base import get_api_base
    from derouter.derouter_core_utils.llm_response_utils.get_formatted_prompt import (
        get_formatted_prompt,
    )
    from derouter.derouter_core_utils.llm_response_utils.get_headers import (
        get_response_headers,
    )
    from derouter.derouter_core_utils.llm_response_utils.response_metadata import (
        ResponseMetadata,
        update_response_metadata,
    )
    from derouter.derouter_core_utils.prompt_templates.common_utils import (
        _parse_content_for_reasoning,
    )
    from derouter.derouter_core_utils.redact_messages import (
        DeRouterLoggingObject,
        redact_message_input_output_from_logging,
    )
    from derouter.derouter_core_utils.rules import Rules
    from derouter.derouter_core_utils.streaming_handler import CustomStreamWrapper
    from derouter.derouter_core_utils.thread_pool_executor import executor
    from derouter.llms.base_llm.anthropic_messages.transformation import (
    ...
    from derouter.llms.base_llm.batches.transformation import BaseBatchesConfig
    from derouter.llms.base_llm.containers.transformation import BaseContainerConfig
    from derouter.llms.base_llm.embedding.transformation import BaseEmbeddingConfig
    from derouter.llms.base_llm.files.transformation import BaseFilesConfig
    from derouter.llms.base_llm.google_genai.transformation import (
        BaseGoogleGenAIGenerateContentConfig,
    )
    from derouter.llms.base_llm.image_edit.transformation import BaseImageEditConfig
    from derouter.llms.base_llm.image_generation.transformation import (
        BaseImageGenerationConfig,
    )
    from derouter.llms.base_llm.image_variations.transformation import (
        BaseImageVariationConfig,
    )
    from derouter.llms.base_llm.ocr.transformation import BaseOCRConfig
    from derouter.llms.base_llm.passthrough.transformation import BasePassthroughConfig
    from derouter.llms.base_llm.realtime.http_transformation import (
        BaseRealtimeHTTPConfig,
    )
    from derouter.llms.base_llm.realtime.transformation import BaseRealtimeConfig
    from derouter.llms.base_llm.rerank.transformation import BaseRerankConfig
    from derouter.llms.base_llm.sandbox.transformation import BaseSandboxConfig
    from derouter.llms.base_llm.search.transformation import BaseSearchConfig
    from derouter.llms.base_llm.text_to_speech.transformation import (
        BaseTextToSpeechConfig,
    )
    from derouter.llms.base_llm.vector_store.transformation import BaseVectorStoreConfig
    from derouter.llms.base_llm.vector_store_files.transformation import (
        BaseVectorStoreFilesConfig,
    )
    from derouter.llms.base_llm.videos.transformation import BaseVideoConfig
    from derouter.llms.bedrock.common_utils import BedrockModelInfo
    from derouter.llms.bedrock.embed.amazon_nova_transformation import (
        AmazonNovaEmbeddingConfig,
    )
    from derouter.llms.bedrock.embed.amazon_titan_g1_transformation import (
        AmazonTitanG1Config,
    )
    from derouter.llms.bedrock.embed.amazon_titan_multimodal_transformation import (
        AmazonTitanMultimodalEmbeddingG1Config,
    )
    from derouter.llms.bedrock.embed.amazon_titan_v2_transformation import (
        AmazonTitanV2Config,
    )
    from derouter.llms.bedrock.embed.cohere_transformation import (
        BedrockCohereEmbeddingConfig,
    )
    from derouter.llms.bedrock.embed.twelvelabs_marengo_transformation import (
        TwelveLabsMarengoEmbeddingConfig,
    )
    from derouter.llms.cohere.common_utils import CohereModelInfo
    from derouter.llms.custom_httpx.http_handler import AsyncHTTPHandler, HTTPHandler
    from derouter.llms.mistral.ocr.transformation import MistralOCRConfig
    from derouter.proxy._types import AllowedModelRegion
    from derouter.router_utils.get_retry_from_policy import (
    ...
    from derouter.types.rerank import RerankResponse
    from derouter.types.router import DeRouter_Params

from derouter.llms.base_llm.chat.transformation import BaseConfig
from derouter.llms.base_llm.completion.transformation import BaseTextCompletionConfig
from derouter.llms.base_llm.evals.transformation import BaseEvalsAPIConfig
from derouter.llms.base_llm.responses.transformation import BaseResponsesAPIConfig
from derouter.llms.base_llm.skills.transformation import BaseSkillsAPIConfig
...
if TYPE_CHECKING:
    from derouter import MockException
...
# https://router.delify.vn/docs/completion/output
# {
#   'choices': [
#      {
#         'finish_reason': 'stop',
...
def print_verbose(
    ...
    try:
        ...
        if derouter.set_verbose is True and logger_only is False:
...
def _print_verbose_is_active() -> bool:
    ...
    derouter._logging.set_verbose, while print_verbose's print reads derouter.set_verbose, and
    assigning the documented derouter.set_verbose = True rebinds only the latter."""
    return derouter.set_verbose is True or verbose_logger.isEnabledFor(logging.DEBUG)
...
def custom_llm_setup():
    ...
    """
    for custom_llm in derouter.custom_provider_map:
        if custom_llm["provider"] not in derouter.provider_list:
            derouter.provider_list.append(custom_llm["provider"])

        if custom_llm["provider"] not in derouter._custom_providers:
            derouter._custom_providers.append(custom_llm["provider"])
...
def _add_custom_logger_callback_to_specific_event(callback: str, logging_event: Literal["success", "failure"]) -> None:
    ...
    if callback not in derouter._known_custom_logger_compatible_callbacks:
        verbose_logger.debug(
            "Callback %s is not a valid custom logger compatible callback. Known list - %s",
            callback,
            derouter._known_custom_logger_compatible_callbacks,
        )
        return
    ...
    if callback_class:
        if logging_event == "success" and _custom_logger_class_exists_in_success_callbacks(callback_class) is False:
            derouter.logging_callback_manager.add_derouter_success_callback(callback_class)
            derouter.logging_callback_manager.add_derouter_async_success_callback(callback_class)
            if callback in derouter.success_callback:
                derouter.success_callback.remove(callback)  # remove the string from the callback list
            if callback in derouter._async_success_callback:
                derouter._async_success_callback.remove(callback)  # remove the string from the callback list
        elif logging_event == "failure" and _custom_logger_class_exists_in_failure_callbacks(callback_class) is False:
            derouter.logging_callback_manager.add_derouter_failure_callback(callback_class)
            derouter.logging_callback_manager.add_derouter_async_failure_callback(callback_class)
            if callback in derouter.failure_callback:
                derouter.failure_callback.remove(callback)  # remove the string from the callback list
            if callback in derouter._async_failure_callback:
                derouter._async_failure_callback.remove(callback)  # remove the string from the callback list
...
def _custom_logger_class_exists_in_success_callbacks(
    ...
    """
    Returns True if an instance of the custom logger exists in derouter.success_callback or derouter._async_success_callback

    e.g if `LangfusePromptManagement` is passed in, it will return True if an instance of `LangfusePromptManagement` exists in derouter.success_callback or derouter._async_success_callback

    Prevents double adding a custom logger callback to the derouter callbacks

    Matches on the exact class; an instance of a subclass does not count as registered
    """
    return any(type(cb) is type(callback_class) for cb in derouter.success_callback + derouter._async_success_callback)
...
def _custom_logger_class_exists_in_failure_callbacks(
    ...
    """
    Returns True if an instance of the custom logger exists in derouter.failure_callback or derouter._async_failure_callback

    e.g if `LangfusePromptManagement` is passed in, it will return True if an instance of `LangfusePromptManagement` exists in derouter.failure_callback or derouter._async_failure_callback

    Prevents double adding a custom logger callback to the derouter callbacks

    Matches on the exact class; an instance of a subclass does not count as registered
    """
    return any(type(cb) is type(callback_class) for cb in derouter.failure_callback + derouter._async_failure_callback)
...
def get_applied_guardrails(kwargs: dict[str, object]) -> list[str]:
    ...
    for callback in derouter.callbacks:
...
def load_credentials_from_list(kwargs: dict):
    ...
    credential_name: Final = kwargs.get("derouter_credential_name")
    if not credential_name:
        return
    credential: Final = CredentialAccessor.find_credential(credential_name)
    if credential is None:
        verbose_logger.warning(
            "derouter_credential_name=%s matched none of the %d loaded credentials; the request runs without it",
            credential_name,
            len(derouter.credential_list),
        )
        return
...
def get_dynamic_callbacks(
    ...
    returned_callbacks: Final = derouter.callbacks.copy()
...
def _restore_correlation_context_if_supported(logging_obj: object) -> None:
    ...
    object as derouter_logging_obj instead of a real Logging instance - this
...
) -> tuple[DeRouterLoggingObject, dict[str, Any]]:
    ### NOTICES ###
    if derouter.set_verbose is True:
        verbose_logger.warning(
            "`derouter.set_verbose` is deprecated. Please set `os.environ['DEROUTER_LOG'] = 'DEBUG'` for debug logs."
        )
    logging_obj: DeRouterLoggingObject | None = None  # rebind-ok: set to the real object further down on success
    try:
        ...
        get_coroutine_checker_fn: Final = derouter_utils.get_coroutine_checker
        ...
        if len(all_callbacks) > 0:
            for callback in all_callbacks:
                ...
                if isinstance(callback, str):
                    callback = derouter.derouter_core_utils.derouter_logging._init_custom_logger_compatible_class(
                        callback,
                        internal_usage_cache=None,
                        llm_router=None,
                    )
                    if callback is None or any(
                        type(cb) is type(callback) for cb in derouter._async_success_callback
                    ):  # don't double add a callback
                        continue
                if callback not in derouter.input_callback:
                    derouter.input_callback.append(callback)
                if callback not in derouter.success_callback:
                    derouter.logging_callback_manager.add_derouter_success_callback(callback)
                if callback not in derouter.failure_callback:
                    derouter.logging_callback_manager.add_derouter_failure_callback(callback)
                if callback not in derouter._async_success_callback:
                    derouter.logging_callback_manager.add_derouter_async_success_callback(callback)
                if callback not in derouter._async_failure_callback:
                    derouter.logging_callback_manager.add_derouter_async_failure_callback(callback)
            print_verbose(f"Initialized derouter callbacks, Async Success Callbacks: {derouter._async_success_callback}")

        if (
            len(derouter.input_callback) > 0 or len(derouter.success_callback) > 0 or len(derouter.failure_callback) > 0
        ) and len(callback_list) == 0:
            callback_list = list(set(derouter.input_callback + derouter.success_callback + derouter.failure_callback))
        ...
        if len(derouter.input_callback) > 0:
            removed_async_items = []
            for index, callback in enumerate(derouter.input_callback):
                if coroutine_checker.is_async_callable(callback):
                    derouter._async_input_callback.append(callback)
                    removed_async_items.append(index)

            # Pop the async items from input_callback in reverse order to avoid index issues
            for index in reversed(removed_async_items):
                derouter.input_callback.pop(index)
        if len(derouter.success_callback) > 0:
            removed_async_items = []
            for index, callback in enumerate(derouter.success_callback):
                if coroutine_checker.is_async_callable(callback):
                    derouter.logging_callback_manager.add_derouter_async_success_callback(callback)
                    removed_async_items.append(index)
                    ...
                    derouter.logging_callback_manager.add_derouter_async_success_callback(callback)
                    removed_async_items.append(index)
                elif callback in derouter._known_custom_logger_compatible_callbacks and isinstance(callback, str):
                    _add_custom_logger_callback_to_specific_event(callback, "success")

            # Pop the async items from success_callback in reverse order to avoid index issues
            for index in reversed(removed_async_items):
                derouter.success_callback.pop(index)

        if len(derouter.failure_callback) > 0:
            removed_async_items = []
            for index, callback in enumerate(derouter.failure_callback):
                if coroutine_checker.is_async_callable(callback):
                    derouter.logging_callback_manager.add_derouter_async_failure_callback(callback)
                    removed_async_items.append(index)
                elif callback in derouter._known_custom_logger_compatible_callbacks and isinstance(callback, str):
            ...
            for index in reversed(removed_async_items):
                derouter.failure_callback.pop(index)
        ### DYNAMIC CALLBACKS ###
        ...
        if add_breadcrumb:
            try:
                from derouter.derouter_core_utils.core_helpers import safe_deep_copy

                details_to_log = safe_deep_copy(kwargs)
            except Exception:
                details_to_log = kwargs

            if derouter.turn_off_message_logging:
                # make a copy of the _model_Call_details and log it
                details_to_log.pop("messages", None)
                details_to_log.pop("input", None)
                details_to_log.pop("prompt", None)
            add_breadcrumb(
                category="derouter.llm_call",
        ...
        if (
            ...
            ### PRE-CALL RULES ###
            Rules: Final = derouter_utils.Rules
            if (
            ...
            if isinstance(messages, list) and len(messages) > 0:
                try:
                    ...
                    from derouter.derouter_core_utils.prompt_templates.factory import (
            ...
            try:
                from derouter.google_genai.adapters.transformation import (
                    GoogleGenAIAdapter,
                )
                from derouter.derouter_core_utils.prompt_templates.common_utils import (
        ...
        if _is_streaming_request(
            ...
            stream = True
        get_derouter_logging_class: Final = getattr(sys.modules[__name__], "get_derouter_logging_class")
        # Victim for object pool
        logging_obj = get_derouter_logging_class()(  # rebind-ok: 2nd assignment to logging_obj (see initial None above)
            model=model,
            messages=messages,
            stream=stream,
            derouter_call_id=kwargs["derouter_call_id"],
        ...
        derouter_params: Final[dict[str, object]] = {"api_base": ""}
        if "metadata" in kwargs:
            derouter_params["metadata"] = kwargs["metadata"]
        if "derouter_metadata" in kwargs and isinstance(kwargs["derouter_metadata"], dict):
            derouter_params["derouter_metadata"] = kwargs["derouter_metadata"]
            # For endpoints like /v1/messages that use "derouter_metadata" instead
            # of "metadata" (to avoid conflicting with provider API metadata fields),
            # populate derouter_params["metadata"] so callbacks (e.g. Langfuse) that
            # read API key info from derouter_params["metadata"] see the fields.
            if not derouter_params.get("metadata"):
                derouter_params["metadata"] = kwargs["derouter_metadata"].copy()
        ...
        verbose_logger.exception("derouter.utils.py::function_setup() - [Non-Blocking] Error in function_setup")
        raise e


async def _client_async_logging_helper(
    logging_obj: DeRouterLoggingObject,
    result,
    start_time,
    end_time,
    ...
    ):  # don't log the parent event derouter.completion_with_fallbacks as a 'log_success_event', this will lead to double logging the same call - https://github.com/Delify-Solutions/derouter/issues/7477
        print_verbose(
...
def _get_wrapper_num_retries(kwargs: dict[str, Any], exception: Exception) -> tuple[int | None, dict[str, Any]]:
    ...
    if num_retries is None:
        num_retries = derouter.num_retries
    if kwargs.get("retry_policy", None):
        ...
        reset_retry_policy: Final = derouter_utils.reset_retry_policy
...
def check_coroutine(value) -> bool:
    get_coroutine_checker: Final = derouter_utils.get_coroutine_checker
                    ...
                    if isinstance(original_response, ModelResponse) and len(original_response.choices) > 0:
                        ...
                        if model_response is not None:
                            ...
                            _enable_json_schema_validation: Final = (
                                ...
                                else derouter.enable_json_schema_validation
                            )
                                ...
                                try:
                                    if (
                                        ...
                                        if json_response_format is not None:
                                            derouter.derouter_core_utils.json_validation_rule.validate_schema(
...
def client(original_function):
    ...
    def wrapper(*args, **kwargs):
        ...
        if _is_async_request(kwargs):
            ...
            if derouter.num_retries_per_request is not None:
                # check if previous_models passed in as ['derouter_params']['metadata]['previous_models']
                previous_models = (kwargs.get("metadata") or {}).get("previous_models", None)
                if previous_models is not None:
                    if derouter.num_retries_per_request <= len(previous_models):
            ...
            if _is_streaming_request(
                ...
                if "complete_response" in kwargs and kwargs["complete_response"] is True:
                    ...
                    for idx, chunk in enumerate(result):
                        chunks.append(chunk)
                    return derouter.stream_chunk_builder(chunks, messages=kwargs.get("messages", None))
                else:
                    return result

            return result

        # Prints Exactly what was passed to derouter function - don't execute any logic here - it should just print
        print_args_passed_to_derouter(original_function, args, kwargs)
        start_time: Final = datetime.datetime.now()
        result = None
        logging_obj: DeRouterLoggingObject | None = kwargs.get("derouter_logging_obj", None)

        # only set derouter_call_id if its not in kwargs
        if "derouter_call_id" not in kwargs:
            kwargs["derouter_call_id"] = str(uuid.uuid4())
        ...
        try:
            ...
            kwargs["derouter_logging_obj"] = logging_obj
            ...
            if derouter.max_budget:
                if derouter._current_cost > derouter.max_budget:
                    raise BudgetExceededError(
                        current_cost=derouter._current_cost,
                        max_budget=derouter.max_budget,
                    )
            ...
            print_verbose(
                f"SYNC kwargs[caching]: {kwargs.get('caching', False)}; derouter.cache: {derouter.cache}; kwargs.get('cache')['no-cache']: {kwargs.get('cache', {}).get('no-cache', False)}"
            )
            # if caching is false or cache["no-cache"]==True, don't run this
            if (
                (
                    (
                        (kwargs.get("caching", None) is None and derouter.cache is not None)
            ...
            if (
                ...
                and derouter.modify_params is True  # user is okay with params being modified
                and (
            ...
            if call_type == CallTypes.completion.value:
                num_retries = kwargs.get("num_retries", None) or derouter.num_retries or None
                if kwargs.get("retry_policy", None):
                    get_num_retries_from_retry_policy: Callable[..., int | None] = getattr(
                        sys.modules[__name__], "get_num_retries_from_retry_policy"
                    )
                    reset_retry_policy = derouter_utils.reset_retry_policy
                    num_retries = get_num_retries_from_retry_policy(
                        exception=e,
                        retry_policy=kwargs.get("retry_policy"),
                    )
                    kwargs["retry_policy"] = reset_retry_policy()  # prevent infinite loops
                derouter.num_retries = None  # set retries to None to prevent infinite loops
                context_window_fallback_dict: Final = kwargs.get("context_window_fallback_dict", {})

                _is_derouter_router_call = "model_group" in (
                    kwargs.get("metadata") or {}
                )  # check if call from derouter.router/proxy
                if (
                    num_retries and not _is_derouter_router_call
                ):  # only enter this if call is not from derouter router/proxy. router has it's own logic for retrying
                    if (
                        ...
                        return derouter.completion_with_retries(*args, **kwargs)
                elif (
                    isinstance(e, derouter.exceptions.ContextWindowExceededError)
                    and context_window_fallback_dict
                    and model in context_window_fallback_dict
                    and not _is_derouter_router_call
                ):
                    if len(args) > 0:
                ...
                if (
                    ...
                    if (
                        ...
                        kwargs["num_retries"] = num_retries
                        return derouter.responses_with_retries(*args, **kwargs)
    ...
    async def wrapper_async(*args, **kwargs):
        ...
        kwargs.pop("_is_derouter_internal_call", None)  # discard if injected
        _is_derouter_internal_call: Final = is_internal_call.get()
        ...
        try:
            ...
            if _is_debugging_on():
                print_verbose(
                    f"ASYNC kwargs[caching]: {kwargs.get('caching', False)}; derouter.cache: {derouter.cache}; kwargs.get('cache'): {kwargs.get('cache', None)}"
                )
            ...
            if not _is_derouter_internal_call:
            ...
            if logging_obj and not _is_derouter_internal_call:
                try:
                    logging_obj.failure_handler(
                        e, traceback_exception, start_time, end_time
                    )  # DO NOT MAKE THREADED - router retry fallback relies on this!
            ...
            if call_type == CallTypes.acompletion.value:
                ...
                if (
                    ...
                    try:
                        ...
                        result = await derouter.acompletion_with_retries(*args, **kwargs)
                    except Exception:
                        pass
                    else:
                        return result
                ...
                if (
                    ...
                    try:
                        ...
                        if isinstance(e, openai.RateLimitError):  # rate limiting specific error
                            ...
                            kwargs["retry_strategy"] = "constant_retry"
                        result = await derouter.aresponses_with_retries(*args, **kwargs)
                    except Exception:
                        pass
                    else:
                        return result
...
def _is_async_request(
    ...
    eg. derouter.acompletion, derouter.aimage_generation, derouter.acreate_batch, derouter._arealtime

    Args:
        kwargs (dict): The kwargs passed to the derouter function
...
def _is_streaming_request(
    ...
    Returns True if:
        - if "stream=True" in kwargs  (derouter chat completion, derouter text completion, derouter messages)
        - if call_type is generate_content_stream or agenerate_content_stream (derouter google genai)
    """
...
def _select_tokenizer_helper(model: str) -> SelectTokenizerResponse:
    if derouter.disable_hf_tokenizer_download is True:
...
def _return_huggingface_tokenizer(model: str) -> SelectTokenizerResponse | None:
    if model in derouter.cohere_models and "command-r" in model:
        # cohere
        cohere_tokenizer: Final = Tokenizer.from_pretrained("Xenova/c4ai-command-r-v01-tokenizer")
        return {"type": "huggingface_tokenizer", "tokenizer": cohere_tokenizer}
    # anthropic
    elif model in derouter.anthropic_models and "claude-3" not in model:
...
def decode(
    ...
    Args:
        skip_special_tokens: For HuggingFace tokenizers, keep the historical
            DeRouter round-trip behavior by omitting special tokens by default.
...
def token_counter(
    ...
    """
    The same as `derouter.derouter_core_utils.token_counter`.
    ...
    #########################################################
    if derouter.disable_token_counter is True:
        return 0
...
def supports_response_schema(model: str, custom_llm_provider: str | None = None) -> bool:
    ...
    ## GET LLM PROVIDER ##
    try:
        get_llm_provider: Final = derouter_utils.get_llm_provider
    ...
    PROVIDERS_GLOBALLY_SUPPORT_RESPONSE_SCHEMA: Final = [
        derouter.LlmProviders.PREDIBASE,
        derouter.LlmProviders.FIREWORKS_AI,
        derouter.LlmProviders.LM_STUDIO,
        derouter.LlmProviders.NEBIUS,
        derouter.LlmProviders.DATABRICKS,
    ]
    ...
    try:
        ...
        if model_info.get(key, False) is True:
            ...
            if bare_model_key is not None:
                bare_entry: Final = derouter.model_cost.get(bare_model_key) or {}
...
def declared_value_factory(model: str, custom_llm_provider: str | None, key: str) -> str | None:
    ...
    """
    try:
        resolved: Final = derouter.get_llm_provider(model=model, custom_llm_provider=custom_llm_provider)
        ...
        bare_entry: Final = derouter.model_cost.get(bare_model_key) if bare_model_key is not None else None
...
def get_supported_regions(model: str, custom_llm_provider: str | None = None) -> list[str] | None:
    ...
    try:
        ...
        model_cost_data: Final = derouter.model_cost.get(model_key, {})
...
def _resolve_builtin_model_cost_entry(key: str, provider: str) -> dict[str, object] | None:
    ...
    for candidate in candidates:
        entry = derouter.model_cost.get(candidate)
        if entry is not None and entry.get("derouter_provider") is not None:
            return dict(entry)
    return None
...
def _get_builtin_model_info_for_registration(model: str) -> ModelInfo | None:
    ...
    ``derouter.model_cost`` while matching a capability rule. A rule-derived entry
    ...
    try:
        ...
        return None
    if info["key"] in derouter.model_cost:
        return info
...
def set_live_deployment_replay(replay: Callable[[], None]) -> None:
    ...
    ``derouter.router`` installs this at import time. The seam exists because the
...
def reapply_runtime_model_cost_registrations() -> None:
    ...
    Adopting a new catalog replaces ``derouter.model_cost`` wholesale, which on
...
def register_model(
    ...
    *,
    ...
    if isinstance(model_cost, dict):
        ...
        loaded_model_cost = derouter.get_model_cost_map(url=model_cost)
    ...
    for key, value in loaded_model_cost.items():
        ...
        # ``get_model_info`` returns ``derouter_provider: None`` when the
        # provider is unknown (e.g. custom deployments registered via
        # ``Router.add_deployment``). Persisting that None into
        # ``derouter.model_cost`` causes ``_check_provider_match`` to drop
        # custom pricing on subsequent cost lookups.
        if existing_model.get("derouter_provider") is None:
            existing_model.pop("derouter_provider", None)
        ...
        _raw_entry = derouter.model_cost.get(model_cost_key)
        if _raw_entry is None:
            _raw_entry = derouter.model_cost.get(key)
        if _raw_entry is None:
            _raw_entry = {}
        ...
        derouter.model_cost.setdefault(model_cost_key, {}).update(updated_dictionary)

        # Invalidate case-insensitive lookup map since model_cost was modified
        _invalidate_model_cost_lowercase_map()

        verbose_logger.debug("added/updated model=%s in derouter.model_cost: %s", model_cost_key, model_cost_key)
        # add new model names to provider lists
        if value.get("derouter_provider") == "openai":
            if key not in derouter.open_ai_chat_completion_models:
                derouter.open_ai_chat_completion_models.add(key)
        elif value.get("derouter_provider") == "text-completion-openai":
            if key not in derouter.open_ai_text_completion_models:
                derouter.open_ai_text_completion_models.add(key)
        elif value.get("derouter_provider") == "cohere":
            if key not in derouter.cohere_models:
                derouter.cohere_models.add(key)
        elif value.get("derouter_provider") == "anthropic":
            if key not in derouter.anthropic_models:
                derouter.anthropic_models.add(key)
        elif value.get("derouter_provider") == "openrouter":
            split_string = key.split("/", 1)
            if split_string[-1] not in derouter.openrouter_models:
                derouter.openrouter_models.add(split_string[-1])
        elif value.get("derouter_provider") == "vercel_ai_gateway":
            if key not in derouter.vercel_ai_gateway_models:
                derouter.vercel_ai_gateway_models.add(key)
        elif value.get("derouter_provider") == "vertex_ai-text-models":
            if key not in derouter.vertex_text_models:
                derouter.vertex_text_models.add(key)
        elif value.get("derouter_provider") == "vertex_ai-code-text-models":
            if key not in derouter.vertex_code_text_models:
                derouter.vertex_code_text_models.add(key)
        elif value.get("derouter_provider") == "vertex_ai-chat-models":
            if key not in derouter.vertex_chat_models:
                derouter.vertex_chat_models.add(key)
        elif value.get("derouter_provider") == "vertex_ai-code-chat-models":
            if key not in derouter.vertex_code_chat_models:
                derouter.vertex_code_chat_models.add(key)
        elif value.get("derouter_provider") == "ai21":
            if key not in router.delify.vn21_models:
                router.delify.vn21_models.add(key)
        elif value.get("derouter_provider") == "nlp_cloud":
            if key not in derouter.nlp_cloud_models:
                derouter.nlp_cloud_models.add(key)
        elif value.get("derouter_provider") == "aleph_alpha":
            if key not in derouter.aleph_alpha_models:
                derouter.aleph_alpha_models.add(key)
        elif value.get("derouter_provider") == "bedrock":
            if key not in derouter.bedrock_models:
                derouter.bedrock_models.add(key)
        elif value.get("derouter_provider") == "novita":
            if key not in derouter.novita_models:
                derouter.novita_models.add(key)
    return model_cost
...
def get_optional_params_transcription(
    ...
    **kwargs,
    ...
    from derouter.constants import OPENAI_TRANSCRIPTION_PARAMS
    ...
    def _check_valid_arg(supported_params):
        if len(non_default_params.keys()) > 0:
            ...
            for k in keys:
                if (
                    drop_params is True or derouter.drop_params is True
                ) and k not in supported_params:  # drop the unsupported non-default values
                    non_default_params.pop(k, None)
                elif k not in supported_params:
                    raise UnsupportedParamsError(
                        status_code=500,
                        message=f"Setting user/encoding format is not supported by {custom_llm_provider}. To drop it from the call, set `derouter.drop_params = True`.",
                    )
            return non_default_params
    ...
    if custom_llm_provider == "openai" or custom_llm_provider == "azure":
        ...
        supported_params = derouter.GroqSTTConfig().get_supported_openai_params_stt()
        _check_valid_arg(supported_params=supported_params)
        optional_params = derouter.GroqSTTConfig().map_openai_params_stt(
    ...
    **kwargs,
    ...
    def _check_valid_arg(supported_params):
        if len(non_default_params.keys()) > 0:
            ...
            for k in keys:
                if (
                    derouter.drop_params is True or drop_params is True
                ) and k not in supported_params:  # drop the unsupported non-default values
                    non_default_params.pop(k, None)
                    ...
                    raise UnsupportedParamsError(
                        status_code=500,
                        message=f"Setting `{k}` is not supported by {custom_llm_provider}, {model}. To drop it from the call, set `derouter.drop_params = True`.",
                    )
            return non_default_params
    ...
    if provider_config is not None:
        ...
        or custom_llm_provider in derouter.openai_compatible_providers
    ):
        optional_params = non_default_params
    elif custom_llm_provider == "bedrock":
        config_class: Final = derouter.BedrockImageGeneration.get_config_class(model=model)
    ...
    **kwargs,
    ...
    def _check_valid_arg(supported_params: list | None):
        ...
        if unsupported_params:
            if derouter.drop_params is True or (drop_params is not None and drop_params is True):
                pass
            else:
                raise UnsupportedParamsError(
                    status_code=500,
                    message=f"{custom_llm_provider} does not support parameters: {unsupported_params}, for model={model}. To drop these, set `derouter.drop_params=True` or for proxy:\n\n`derouter_settings:\n drop_params: true`\n",
                )
    ...
    ## raise exception if non-default value passed for non-openai/azure embedding calls
        ...
        if (
            ...
            # Honor drop_params (per-call) and derouter.drop_params (global) the same
            # way `_check_valid_arg` does above. The raised error message itself
            # tells users to set `drop_params=True`, so respect it here.
            if derouter.drop_params is True or drop_params is True:
                non_default_params.pop("dimensions", None)
            else:
                raise UnsupportedParamsError(
                    status_code=500,
                    message="Setting dimensions is not supported for OpenAI `text-embedding-3` and later models. To drop it from the call, set `derouter.drop_params = True`.",
                )
        optional_params = non_default_params
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = derouter.TritonEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params={},
            model=model,
            drop_params=drop_params if drop_params is not None else False,
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = derouter.DatabricksEmbeddingConfig().map_openai_params(
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = derouter.nvidiaNimEmbeddingConfig.map_openai_params(
        ...
        (
            optional_params,
            kwargs,
        ) = derouter.VertexAITextEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params, optional_params={}, kwargs=kwargs
        )
    elif custom_llm_provider == "lm_studio":
        supported_params = derouter.LmStudioEmbeddingConfig().get_supported_openai_params()
        _check_valid_arg(supported_params=supported_params)
        optional_params = derouter.LmStudioEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params, optional_params={}
        )
    elif custom_llm_provider == "bedrock":
        ...
        if "amazon.titan-embed-text-v1" in model:
            object: (
                ...
                | TwelveLabsMarengoEmbeddingConfig
                | AmazonNovaEmbeddingConfig
            ) = derouter.AmazonTitanG1Config()
        elif "amazon.titan-embed-image-v1" in model:
            object = derouter.AmazonTitanMultimodalEmbeddingG1Config()
        elif "amazon.titan-embed-text-v2:0" in model:
            object = derouter.AmazonTitanV2Config()
        elif "cohere.embed" in model:
            object = derouter.BedrockCohereEmbeddingConfig()
        elif "twelvelabs" in model or "marengo" in model:
            object = derouter.TwelveLabsMarengoEmbeddingConfig(model=model)
        elif "nova" in model.lower():
            object = derouter.AmazonNovaEmbeddingConfig()
        else:  # unmapped model
            supported_params = []
            _check_valid_arg(supported_params=supported_params)
            final_params = {**kwargs}
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = derouter.MistralEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params, optional_params={}
        )
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = derouter.JinaAIEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params={},
            model=model,
            drop_params=drop_params if drop_params is not None else False,
        ...
        _check_valid_arg(supported_params=supported_params)
        if derouter.VoyageContextualEmbeddingConfig.is_contextualized_embeddings(model):
            optional_params = derouter.VoyageContextualEmbeddingConfig().map_openai_params(
                non_default_params=non_default_params,
                optional_params={},
                model=model,
                drop_params=drop_params if drop_params is not None else False,
            )
        elif derouter.VoyageMultimodalEmbeddingConfig.is_multimodal_embeddings(model):
            optional_params = derouter.VoyageMultimodalEmbeddingConfig().map_openai_params(
                non_default_params=non_default_params,
                optional_params={},
                model=model,
                drop_params=drop_params if drop_params is not None else False,
            ...
            optional_params = derouter.VoyageEmbeddingConfig().map_openai_params(
                non_default_params=non_default_params,
                optional_params={},
                model=model,
                drop_params=drop_params if drop_params is not None else False,
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = derouter.GenAIHubEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params={},
            model=model,
            drop_params=drop_params if drop_params is not None else False,
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = derouter.InfinityEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params={},
            model=model,
            drop_params=drop_params if drop_params is not None else False,
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = derouter.FireworksAIEmbeddingConfig().map_openai_params(
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = derouter.SambaNovaEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params={},
            model=model,
            drop_params=drop_params if drop_params is not None else False,
        ...
        _check_valid_arg(supported_params=supported_params)
        optional_params = derouter.OVHCloudEmbeddingConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params={},
            model=model,
            drop_params=drop_params if drop_params is not None else False,
        ...
        if len(non_default_params.keys()) > 0:
            if derouter.drop_params is True or drop_params is True:  # drop the unsupported non-default values
                ...
                raise UnsupportedParamsError(
                    status_code=500,
                    message=f"Setting {non_default_params} is not supported by {custom_llm_provider}. To drop it from the call, set `derouter.drop_params = True`.",
                )
    elif (
        custom_llm_provider != "openai"
        and custom_llm_provider != "azure"
        and custom_llm_provider not in derouter.openai_compatible_providers
    ):
        if len(non_default_params.keys()) > 0:
            if derouter.drop_params is True or drop_params is True:  # drop the unsupported non-default values
                keys = list(non_default_params.keys())
...
def _remove_additional_properties(schema):
    ...
    Relevant Issues: https://github.com/Delify-Solutions/derouter/issues/6136, https://github.com/Delify-Solutions/derouter/issues/6088
    """
...
def filter_out_derouter_params(kwargs: dict) -> dict:
    """
    Filter out DeRouter internal parameters from kwargs dict.

    Returns a new dict containing only non-DeRouter parameters that should be
    passed to external provider APIs.

    Args:
        kwargs: Dictionary that may contain DeRouter internal parameters

    Returns:
        Dictionary with DeRouter internal parameters filtered out

    Example:
        >>> kwargs = {"query": "test", "shared_session": session_obj, "metadata": {}}
        >>> filtered = filter_out_derouter_params(kwargs)
        >>> # filtered = {"query": "test"}
    """

    return {key: value for key, value in kwargs.items() if key not in all_derouter_params}
    ...
    def base_pre_process_non_default_params(
        ...
        for k, v in special_params.items():
            if k == "aws_bedrock_project_id":
                # sent as a request header (read from derouter_params by the
...
def pre_process_non_default_params(
    ...
    ):  # fixes https://github.com/Delify-Solutions/derouter/issues/4933
...
def pre_process_optional_params(passed_params: dict, non_default_params: dict, custom_llm_provider: str) -> dict:
    ...
    common_auth_dict: Final = derouter.common_cloud_provider_auth_params
    if custom_llm_provider in common_auth_dict["providers"]:
        ...
        if custom_llm_provider == "azure":
            optional_params = derouter.AzureOpenAIConfig().map_special_auth_params(
                non_default_params=passed_params, optional_params=optional_params
            )
        elif custom_llm_provider == "bedrock":
            optional_params = derouter.AmazonBedrockGlobalConfig().map_special_auth_params(
                non_default_params=passed_params, optional_params=optional_params
            )
        elif custom_llm_provider == "vertex_ai" or custom_llm_provider == "vertex_ai_beta":
            optional_params = derouter.VertexAIConfig().map_special_auth_params(
                non_default_params=passed_params, optional_params=optional_params
            )
        elif custom_llm_provider == "watsonx":
            optional_params = derouter.IBMWatsonXAIConfig().map_special_auth_params(
                non_default_params=passed_params, optional_params=optional_params
            )
    ...
    if "functions" in non_default_params or "function_call" in non_default_params or "tools" in non_default_params:
        if (
            ...
            if custom_llm_provider == "ollama":
                ...
                derouter.add_function_to_prompt = True  # so that main.py adds the function call to the prompt
            ...
            elif derouter.add_function_to_prompt:  # if user opts to add it to prompt instead
...
def get_optional_params(
    ...
    **kwargs,
    ...
    def _check_valid_arg(supported_params: list[str]):
        ...
        Args:
            supported_params: List[str] - supported params from the derouter config
        """
        verbose_logger.info("\nDeRouter completion() model= %s; provider = %s", model, custom_llm_provider)
        verbose_logger.debug("\nDeRouter: Params passed to completion() %s", passed_params)
        verbose_logger.debug("\nDeRouter: Non-Default params passed to completion() %s", non_default_params)
        unsupported_params: Final = {}
        for k in non_default_params:
            if k not in supported_params:
        ...
        if unsupported_params:
            if derouter.drop_params is True or (drop_params is not None and drop_params is True):
                ...
                raise UnsupportedParamsError(
                    status_code=500,
                    message=f"{custom_llm_provider} does not support parameters: {list(unsupported_params.keys())}, for model={model}. To drop these, set `derouter.drop_params=True` or for proxy:\n\n`derouter_settings:\n drop_params: true`\n. \n If you want to use these params dynamically send allowed_openai_params={list(unsupported_params.keys())} in your request.",
                )
    ...
    if custom_llm_provider == "anthropic":
        ...
        optional_params = derouter.AnthropicConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
        ...
        optional_params = derouter.AnthropicTextConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.CohereChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.TritonConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.MaritalkConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.ReplicateConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.PredibaseConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.HuggingFaceChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.TogetherAIChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        model in derouter.vertex_chat_models
        or model in derouter.vertex_code_chat_models
        or model in derouter.vertex_text_models
        or model in derouter.vertex_code_text_models
        or model in derouter.vertex_language_models
        or model in derouter.vertex_vision_models
    ):
        optional_params = derouter.VertexGeminiConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.GoogleAIStudioGeminiConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
    ...
    elif derouter.VertexAIAnthropicConfig.is_supported_model(model=model, custom_llm_provider=custom_llm_provider):
        optional_params = derouter.VertexAIAnthropicConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=bool(drop_params),
        ...
        if model in derouter.vertex_mistral_models:
            if "codestral" in model:
                optional_params = derouter.CodestralTextCompletionConfig().map_openai_params(
                    model=model,
                    non_default_params=non_default_params,
                    optional_params=optional_params,
                    drop_params=bool(drop_params),
                ...
                optional_params = derouter.MistralConfig().map_openai_params(
                    model=model,
                    non_default_params=non_default_params,
                    optional_params=optional_params,
                    drop_params=bool(drop_params),
                )
        elif model in derouter.vertex_ai_ai21_models:
            optional_params = derouter.VertexAIAi21Config().map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=model,
                drop_params=bool(drop_params),
            ...
            optional_params = derouter.VertexAILlama3Config().map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=model,
                drop_params=bool(drop_params),
        ...
        optional_params = derouter.SagemakerConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        if bedrock_route == "converse" or bedrock_route == "converse_like":
            optional_params = derouter.AmazonConverseConfig().map_openai_params(
                model=model,
                non_default_params=non_default_params,
                optional_params=optional_params,
                drop_params=bool(drop_params),
            ...
            optional_params = derouter.AmazonBedrockOpenAIConfig().map_openai_params(
                model=model,
                non_default_params=non_default_params,
                optional_params=optional_params,
                drop_params=bool(drop_params),
            ...
            if bedrock_base_model in derouter.AmazonAnthropicConfig.get_legacy_anthropic_model_names():
                optional_params = derouter.AmazonAnthropicConfig().map_openai_params(
                    non_default_params=non_default_params,
                    optional_params=optional_params,
                    model=model,
                    drop_params=bool(drop_params),
                ...
                optional_params = derouter.AmazonAnthropicClaudeConfig().map_openai_params(
                    non_default_params=non_default_params,
                    optional_params=optional_params,
                    model=model,
                    drop_params=bool(drop_params),
        ...
        optional_params = derouter.CloudflareChatConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.OllamaConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.OllamaChatConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.NLPCloudConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.PetalsConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.DeepInfraConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.InceptionTextCompletionConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.DatabricksConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.NvidiaNimConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.CerebrasConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.XAIChatConfig().map_openai_params(
            model=model,
            non_default_params=non_default_params,
            optional_params=optional_params,
        )
    elif custom_llm_provider == "ai21_chat" or custom_llm_provider == "ai21":
        optional_params = derouter.AI21ChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.FireworksAIConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.VolcEngineConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.HostedVLLMChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.VLLMConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.GroqChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.BedrockMantleChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.DeepSeekChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.TencentChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.OpenrouterConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.IBMWatsonXChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        for param in passed_params:
            if derouter.IBMWatsonXAIConfig().is_watsonx_text_param(param):
                raise ValueError(
                    f"DeRouter now defaults to Watsonx's `/text/chat` endpoint. Please use the `watsonx_text` provider instead, to call the `/text/generation` endpoint. Param: {param}"
                )
    elif custom_llm_provider == "watsonx_text":
        optional_params = derouter.IBMWatsonXAIConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.OpenAIConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        optional_params = derouter.NebiusConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
        ...
        if derouter.AzureOpenAIO1Config().is_o_series_model(model=_azure_detection_model):
            optional_params = derouter.AzureOpenAIO1Config().map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
            ...
            optional_params = derouter.AzureOpenAIGPT5Config().map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=_azure_detection_model,
                drop_params=bool(drop_params),
            ...
            verbose_logger.debug(
                "Azure optional params - api_version: api_version={}, derouter.api_version={}, os.environ['AZURE_API_VERSION']={}".format(
                    api_version, derouter.api_version, get_secret("AZURE_API_VERSION")
                )
            )
            api_version = (
                api_version
                or derouter.api_version
                or get_secret("AZURE_API_VERSION")
                or derouter.AZURE_DEFAULT_API_VERSION
            )
            optional_params = derouter.AzureOpenAIConfig().map_openai_params(
                non_default_params=non_default_params,
                optional_params=optional_params,
                model=_azure_detection_model,
        ...
        optional_params = derouter.OpenAILikeChatConfig().map_openai_params(
            non_default_params=non_default_params,
            optional_params=optional_params,
            model=model,
            drop_params=bool(drop_params),
...
def add_provider_specific_params_to_optional_params(
    ...
    if custom_llm_provider in ["openai", "azure", "text-completion-openai"] + derouter.openai_compatible_providers:
...
def _apply_openai_param_overrides(optional_params: dict, non_default_params: dict, allowed_openai_params: list):
    ...
    See https://github.com/Delify-Solutions/derouter/issues/25697
    """
...
def _get_deployment_order(deployment: dict | Any) -> int | None:
    ...
    Checks derouter_params first (static config), then model_info (dynamic/team
    models added via API where order lives in model_info, not derouter_params).
    """
    order = deployment.get("derouter_params", {}).get("order")
    if order is None:
...
def _get_model_region(custom_llm_provider: str, derouter_params: DeRouter_Params) -> str | None:
    """
    ...
    if custom_llm_provider == "vertex_ai":
        ...
        vertex_ai_location: Final = (
            derouter_params.vertex_location
            or derouter.vertex_location
        ...
        aws_region_name: Final = derouter_params.aws_region_name
        if aws_region_name is not None:
            return aws_region_name
    elif custom_llm_provider == "watsonx":
        watsonx_region_name: Final = derouter_params.watsonx_region_name
        if watsonx_region_name is not None:
            return watsonx_region_name
    return derouter_params.region_name


def _infer_model_region(derouter_params: DeRouter_Params) -> AllowedModelRegion | None:
    """
    ...
    """
    model, custom_llm_provider, _, _ = derouter.get_llm_provider(
        model=derouter_params.model, derouter_params=derouter_params
    )

    model_region: Final = _get_model_region(custom_llm_provider=custom_llm_provider, derouter_params=derouter_params)

    if model_region is None:
        verbose_logger.debug("Cannot infer model region for model: %s", derouter_params.model)
        return None

    if custom_llm_provider == "azure":
        eu_regions = derouter.AzureOpenAIConfig().get_eu_regions()
        us_regions = derouter.AzureOpenAIConfig().get_us_regions()
    elif custom_llm_provider == "vertex_ai":
        eu_regions = derouter.VertexAIConfig().get_eu_regions()
        us_regions = derouter.VertexAIConfig().get_us_regions()
    elif custom_llm_provider == "bedrock":
        eu_regions = derouter.AmazonBedrockGlobalConfig().get_eu_regions()
        us_regions = derouter.AmazonBedrockGlobalConfig().get_us_regions()
    elif custom_llm_provider == "watsonx":
        eu_regions = derouter.IBMWatsonXAIConfig().get_eu_regions()
        us_regions = derouter.IBMWatsonXAIConfig().get_us_regions()
    else:
        eu_regions = []
        us_regions = []
...
def _is_region_eu(derouter_params: DeRouter_Params) -> bool:
    """
    Return true/false if a deployment is in the EU
    """
    if derouter_params.region_name == "eu":
        return True

    ## Else - try and infer from model region
    model_region: Final = _infer_model_region(derouter_params=derouter_params)
    if model_region is not None and model_region == "eu":
        return True
    return False


def _is_region_us(derouter_params: DeRouter_Params) -> bool:
    """
    Return true/false if a deployment is in the US
    """
    if derouter_params.region_name == "us":
        return True
...
def is_region_allowed(derouter_params: DeRouter_Params, allowed_model_region: str) -> bool:
    """
    Return true/false if a deployment is in the EU
    """
    if derouter_params.region_name == allowed_model_region:
        return True
    return False


def get_model_region(derouter_params: DeRouter_Params, mode: str | None) -> str | None:
    """
    Pass the derouter params for an azure model, and get back the region
    """
    if (
        "azure" in derouter_params.model
        and isinstance(derouter_params.api_key, str)
        and isinstance(derouter_params.api_base, str)
    ):
        _model: Final = derouter_params.model.replace("azure/", "")
        response: Final[dict] = derouter.AzureChatCompletion().get_headers(
            model=_model,
            api_key=derouter_params.api_key,
            api_base=derouter_params.api_base,
            api_version=derouter_params.api_version or derouter.AZURE_DEFAULT_API_VERSION,
            timeout=10,
            mode=mode or "chat",
        )
...
def get_max_tokens(model: str) -> int | None:
    ...
    def _get_max_position_embeddings(model_name):
        ...
        try:
            ...
            response: Final = derouter.module_level_client.get(config_url, timeout=HF_CONFIG_FETCH_TIMEOUT_SECONDS)
    ...
    try:
        if model in derouter.model_cost:
            if "max_output_tokens" in derouter.model_cost[model]:
                return derouter.model_cost[model]["max_output_tokens"]
            elif "max_tokens" in derouter.model_cost[model]:
                return derouter.model_cost[model]["max_tokens"]
        get_llm_provider: Final = derouter_utils.get_llm_provider
        model, custom_llm_provider, _, _ = get_llm_provider(model=model)
        if custom_llm_provider == "huggingface":
            max_tokens: Final = _get_max_position_embeddings(model_name=model)
            return max_tokens
        if model in derouter.model_cost:  # check if extracted model is in model_list
            if "max_output_tokens" in derouter.model_cost[model]:
                return derouter.model_cost[model]["max_output_tokens"]
            elif "max_tokens" in derouter.model_cost[model]:
                return derouter.model_cost[model]["max_tokens"]
        ...
        raise Exception(
            f"Model {model} isn't mapped yet. Add it here - https://github.com/Delify-Solutions/derouter/blob/main/model_prices_and_context_window.json"
        )
...
def _invalidate_model_cost_lowercase_map() -> None:
    ...
    Call this whenever derouter.model_cost is modified to ensure the map is rebuilt.
...
def _rebuild_model_cost_lowercase_map() -> dict[str, str]:
    ...
    _model_cost_lowercase_map = {k.lower(): k for k in derouter.model_cost}
...
def _handle_stale_map_entry_rebuild(
    ...
    if matched_key is not None and matched_key in derouter.model_cost:
        return matched_key
    return None
...
def _handle_new_key_with_scan(
    ...
    """
    global _model_cost_lowercase_map
    for key in derouter.model_cost:
...
def _get_model_cost_key(potential_key: str) -> str | None:
    ...
    # Exact match (O(1))
    if potential_key in derouter.model_cost:
...
def _get_model_info_from_model_cost(key: str) -> dict:
    return derouter.model_cost[key]
...
def _check_provider_match(model_info: dict, custom_llm_provider: str | None) -> bool:
    ...
    A missing ``derouter_provider`` key and a ``derouter_provider`` set to
    ...
    if custom_llm_provider and (
        model_info.get("derouter_provider") is not None and model_info["derouter_provider"] != custom_llm_provider
    ):
        if (
            custom_llm_provider == "vertex_ai"
            and model_info["derouter_provider"].startswith("vertex_ai")
            or custom_llm_provider == "fireworks_ai"
            and model_info["derouter_provider"].startswith("fireworks_ai")
            or custom_llm_provider.startswith("bedrock")
            and model_info["derouter_provider"].startswith("bedrock")
        ):
            return True
        elif (
            custom_llm_provider == "derouter_proxy"
        ):  # derouter_proxy is a special case, it's not a provider, it's a proxy for the provider
            return True
        elif custom_llm_provider == "azure_ai" and model_info["derouter_provider"] in (
            "azure",
            "openai",
        ):
...
def _get_model_info_from_generalization(
    ...
    least one capability rule, with ``derouter_provider`` backfilled from the
    provider the caller requested. Rules lose to exact entries: if ANY candidate is
    an exact ``derouter.model_cost`` key (necessarily provider-mismatched, or the
    ...
    for candidate in candidates:
        ...
        return candidate, {**generalized_info, "derouter_provider": custom_llm_provider}
    return None
...
def _get_potential_model_names(model: str, custom_llm_provider: str | None) -> PotentialModelNamesAndCustomLLMProvider:
    ...
    if custom_llm_provider in ("bedrock", "bedrock_converse"):
        from derouter.llms.bedrock.common_utils import strip_bedrock_routing_prefix
...
def _get_model_info_helper(
    ...
    try:
        azure_llms: Final = {**derouter.azure_llms, **derouter.azure_embedding_models}
        if model in azure_llms:
            model = azure_llms[model]
        if custom_llm_provider is not None and custom_llm_provider == "vertex_ai_beta":
            custom_llm_provider = "vertex_ai"
        if custom_llm_provider is not None and custom_llm_provider == "vertex_ai":
            if "meta/" + model in derouter.vertex_llama3_models:
                model = "meta/" + model
            elif (
                model + "@latest" in derouter.vertex_mistral_models or model + "@latest" in derouter.vertex_ai_ai21_models
            ):
                model = model + "@latest"
        ##########################
        ...
        verbose_logger.debug("checking potential_model_names in derouter.model_cost: %s", potential_model_names)
        ...
        if custom_llm_provider == "huggingface":
            ...
            1. 'custom_llm_provider/model' in derouter.model_cost. Checks "groq/llama3-8b-8192" if model="llama3-8b-8192" and custom_llm_provider="groq"
            2. 'model' in derouter.model_cost. Checks "gemini-1.5-pro-002" in  derouter.model_cost if model="gemini-1.5-pro-002" and custom_llm_provider=None
            3. 'split_model' in derouter.model_cost. Checks "au.anthropic.claude-opus-4-8" in derouter.model_cost if model="bedrock/au.anthropic.claude-opus-4-8"
            4. 'combined_stripped_model_name' in derouter.model_cost. Checks if 'gemini/gemini-1.5-flash' in model map, if 'gemini/gemini-1.5-flash-001' given.
            5. 'stripped_model_name' in derouter.model_cost. Checks if 'ft:gpt-3.5-turbo' in model map, if 'ft:gpt-3.5-turbo:my-org:custom_suffix:id' given.
            6. 'provider_prefixed_model_name' in derouter.model_cost, for providers whose own model ids repeat the
               derouter provider name. Checks "perplexity/perplexity/glm-5.2" if model="perplexity/glm-5.2" and
               custom_llm_provider="perplexity", where 1-5 all read the leading "perplexity/" as the derouter prefix
            ...
            if _model_info is None or key is None:
                raise ValueError(
                    "This model isn't mapped yet. Add it here - https://github.com/Delify-Solutions/derouter/blob/main/model_prices_and_context_window.json"
                )
            ...
            returned_model_info: Final = ModelInfoBase(
                ...
                derouter_provider=_model_info.get("derouter_provider", custom_llm_provider),
        ...
        raise Exception(
            f"This model isn't mapped yet. model={model}, custom_llm_provider={custom_llm_provider}. Add it here - https://github.com/Delify-Solutions/derouter/blob/main/model_prices_and_context_window.json."
        )
...
def _build_model_info(
    ...
    supported_openai_params = derouter.get_supported_openai_params(model=model, custom_llm_provider=custom_llm_provider)
...
def get_model_info(
    ...
    Parameters:
    - model (str): The name of the model.
    - custom_llm_provider (str | null): the provider used for the model. If provided, used to check if the derouter model info is for that provider.

    Returns:
        dict: A dictionary containing the following information:
            key: Required[str] # the key in derouter.model_cost which is returned
            ...
            derouter_provider: Required[str]
            mode: Required[
                Literal[
...
def function_to_dict(input_function) -> dict:
    ...
    -------
    dictionnary
        A dictionnary to add to the list passed to `functions` parameter of `derouter.completion`
    """
...
def get_provider_fields(custom_llm_provider: str) -> list[ProviderField]:
    ...
    if custom_llm_provider == "databricks":
        return derouter.DatabricksConfig().get_required_params()

    elif custom_llm_provider == "ollama":
        return derouter.OllamaConfig().get_required_params()

    elif custom_llm_provider == "azure_ai":
        return derouter.AzureAIStudioConfig().get_required_params()
...
def validate_environment(
    ...
    if custom_llm_provider:
        ...
        if (
            model in derouter.open_ai_chat_completion_models
            or model in derouter.open_ai_text_completion_models
            or model in derouter.open_ai_embedding_models
            or model in derouter.openai_image_generation_models
            ...
            if "OPENAI_API_KEY" in os.environ:
                ...
                missing_keys.append("OPENAI_API_KEY")
        ## anthropic
        elif model in derouter.anthropic_models:
            if "ANTHROPIC_API_KEY" in os.environ or "ANTHROPIC_AUTH_TOKEN" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("ANTHROPIC_API_KEY")
        ## cohere
        elif model in derouter.cohere_models:
            if "COHERE_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("COHERE_API_KEY")
        ## replicate
        elif model in derouter.replicate_models:
            if "REPLICATE_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("REPLICATE_API_KEY")
        ## openrouter
            ...
            if "OPENROUTER_API_KEY" in os.environ:
                ...
                missing_keys.append("OPENROUTER_API_KEY")
        ## vercel_ai_gateway
        elif model in derouter.vercel_ai_gateway_models:
            if "VERCEL_AI_GATEWAY_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("VERCEL_AI_GATEWAY_API_KEY")
        ## datarobot
        elif model in derouter.datarobot_models:
            if "DATAROBOT_API_TOKEN" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("DATAROBOT_API_TOKEN")
        ## vertex - text + chat models
            ...
            model in derouter.vertex_chat_models
            or model in derouter.vertex_text_models
            or model in derouter.models_by_provider["vertex_ai"]
        ):
            if "VERTEXAI_PROJECT" in os.environ and "VERTEXAI_LOCATION" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.extend(["VERTEXAI_PROJECT", "VERTEXAI_LOCATION"])
        ## huggingface
        elif model in derouter.huggingface_models:
            if "HUGGINGFACE_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("HUGGINGFACE_API_KEY")
        ## ai21
        elif model in router.delify.vn21_models:
            if "AI21_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("AI21_API_KEY")
        ## together_ai
        elif model in derouter.together_ai_models:
            if "TOGETHERAI_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("TOGETHERAI_API_KEY")
        ## aleph_alpha
            ...
            if "ALEPH_ALPHA_API_KEY" in os.environ:
                ...
                missing_keys.append("ALEPH_ALPHA_API_KEY")
        ## baseten
        elif model in derouter.baseten_models:
            if "BASETEN_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("BASETEN_API_KEY")
        ## nlp_cloud
            ...
            if "NLP_CLOUD_API_KEY" in os.environ:
                ...
                missing_keys.append("NLP_CLOUD_API_KEY")
        elif model in derouter.novita_models:
            if "NOVITA_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("NOVITA_API_KEY")
        elif model in derouter.nebius_models:
            if "NEBIUS_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("NEBIUS_API_KEY")
        elif model in derouter.wandb_models:
            if "WANDB_API_KEY" in os.environ:
                keys_in_environment = True
            else:
                missing_keys.append("WANDB_API_KEY")
...
def acreate(*args, **kwargs):  ## Thin client to handle the acreate langchain call
    return derouter.acompletion(*args, **kwargs)


def valid_model(model):
    try:
        # for a given model name, check if the user has the right permissions to access the model
        if model in derouter.open_ai_chat_completion_models or model in derouter.open_ai_text_completion_models:
            openai.models.retrieve(model)
        else:
            messages: Final = [{"role": "user", "content": "Hello World"}]
            derouter.completion(model=model, messages=messages)
    except Exception:
...
def check_valid_key(model: str, api_key: str):
    """
    Checks if a given API key is valid for a specific model by making a derouter.completion call with max_tokens=10
    ...
    try:
        derouter.completion(model=model, messages=messages, api_key=api_key, max_tokens=10)
        return True
...
def register_prompt_template(
    ...
    ```
    import derouter
    derouter.register_prompt_template(
            model="llama-2",
    ...
    if tokenizer_config:
        for m in potential_models:
            derouter.known_tokenizer_config[m] = {
                "tokenizer": tokenizer_config,
                "status": "success",
            }
    else:
        for m in potential_models:
            derouter.custom_prompt_dict[m] = {
                "roles": roles,
                "initial_prompt_value": initial_prompt_value,
                "final_prompt_value": final_prompt_value,
            }

    return derouter.custom_prompt_dict
...
def mock_completion_streaming_obj(model_response, mock_response, model, n: int | None = None):
    if isinstance(mock_response, derouter.MockException):
        raise mock_response
    ...
    for i in range(0, len(mock_response), 3):
        ...
        if n is None:
            ...
            _all_choices = []
            for j in range(n):
                _streaming_choice = derouter.utils.StreamingChoices(
                    index=j,
                    delta=derouter.utils.Delta(role="assistant", content=mock_response[i : i + 3]),
                )
...
def trim_messages(
    ...
    Args:
        ...
        model: The DeRouter model being used (determines the token limit).
    ...
    try:
        if max_tokens is None:
            ...
            if model in derouter.model_cost:
                max_tokens_for_model: Final = derouter.model_cost[model].get(
                    "max_input_tokens", derouter.model_cost[model]["max_tokens"]
                )
                max_tokens = int(max_tokens_for_model * trim_ratio)
            else:
                # if user did not specify max (input) tokens
                # or passed an llm derouter does not know
        ...
        ## Handle Tool Call ## - check if last message is a tool response, return as is - https://github.com/Delify-Solutions/derouter/issues/4931
        tool_messages: Final = []
...
class AvailableModelsCache(InMemoryCache):
    ...
    def _get_cache_key(
        self,
        custom_llm_provider: str | None,
        derouter_params: DeRouter_Params | None,
    ) -> str:
        valid_str = ""

        if derouter_params is not None:
            valid_str = derouter_params.model_dump_json()
        if custom_llm_provider is not None:
    ...
    def get_cached_model_info(
        self,
        custom_llm_provider: str | None = None,
        derouter_params: DeRouter_Params | None = None,
    ) -> list[str] | None:
        """Get cached model info"""
        # Check if environment has changed
        if derouter_params is None and self._check_env_changed():
            self.cache_dict.clear()
            return None

        cache_key: Final = self._get_cache_key(custom_llm_provider, derouter_params)
...
def _infer_valid_provider_from_env_vars(
    ...
    for provider in derouter.provider_list:
        if custom_llm_provider and provider != custom_llm_provider:
            continue

        # edge case derouter has together_ai as a provider, it should be togetherai
        env_provider_1 = provider.replace("_", "")
        env_provider_2 = provider

        # derouter standardizes expected provider keys to
...
def _get_valid_models_from_provider_api(
    ...
    try:
        cached_result: Final = _model_cache.get_cached_model_info(custom_llm_provider, derouter_params)

        if cached_result is not None:
            return cached_result
        models: Final = provider_config.get_models(
            api_key=derouter_params.api_key if derouter_params is not None else None,
            api_base=derouter_params.api_base if derouter_params is not None else None,
        )

        _model_cache.set_cached_model_info(custom_llm_provider, derouter_params, models)
        return models
    except Exception as e:
...
def get_valid_models(
    ...
    try:
        ################################
        # init derouter_params
        #################################
        from derouter.types.router import DeRouter_Params

        if derouter_params is None:
            derouter_params = DeRouter_Params(model="")
        if api_key is not None:
            derouter_params.api_key = api_key
        if api_base is not None:
            derouter_params.api_base = api_base
        #################################

        check_provider_endpoint = check_provider_endpoint or derouter.check_provider_endpoint
        # get keys set in .env
        ...
        for provider in valid_providers:
            ...
            if provider == "azure":
                ...
                models_for_provider = copy.deepcopy(derouter.models_by_provider.get(provider, []))
...
def print_args_passed_to_derouter(original_function, args, kwargs):
    ...
    try:
        ...
        print_verbose(
            "\033[92mRequest to derouter:\033[0m",
        )
        if args and kwargs:
            print_verbose(f"\033[92mderouter.{original_function.__name__}({args_str}, {kwargs_str})\033[0m")
        elif args:
            print_verbose(f"\033[92mderouter.{original_function.__name__}({args_str})\033[0m")
        elif kwargs:
            print_verbose(f"\033[92mderouter.{original_function.__name__}({kwargs_str})\033[0m")
        else:
            print_verbose(f"\033[92mderouter.{original_function.__name__}()\033[0m")
        print_verbose("\n")  # new line after
    except Exception:
...
def _get_base_model_from_metadata(model_call_details=None):
    ...
    if derouter_params is not None:
        _base_model: Final = derouter_params.get("base_model", None)
        if _base_model is not None:
            return _base_model
        metadata: Final = derouter_params.get("metadata") or {}

        _get_base_model_from_derouter_call_metadata: Callable[..., str | None] = getattr(
            sys.modules[__name__], "_get_base_model_from_derouter_call_metadata"
        )
        base_model_from_metadata: Final = _get_base_model_from_derouter_call_metadata(metadata=metadata)
        if base_model_from_metadata is not None:
            return base_model_from_metadata

        # Also check derouter_metadata (used by Responses API and other generic API calls)
        derouter_metadata: Final = derouter_params.get("derouter_metadata", {})
        _get_base_model_from_derouter_call_metadata = getattr(
            sys.modules[__name__], "_get_base_model_from_derouter_call_metadata"
        )
        return _get_base_model_from_derouter_call_metadata(metadata=derouter_metadata)
    return None
...
def is_cached_message(message: AllMessageValues) -> bool:
    ...
    Can be disabled globally by setting derouter.disable_anthropic_gemini_context_caching_transform = True
    """
    # Check if context caching is disabled globally
    if derouter.disable_anthropic_gemini_context_caching_transform is True:
        return False
...
def any_assistant_message_has_thinking_blocks(
    ...
    Related issue: https://github.com/Delify-Solutions/derouter/issues/18926
    """
    for message in messages:
...
def last_assistant_with_tool_calls_has_no_thinking_blocks(
    ...
    Related issues: https://github.com/Delify-Solutions/derouter/issues/14194, https://github.com/Delify-Solutions/derouter/issues/9020
    """
...
def add_dummy_tool(custom_llm_provider: str) -> list[ChatCompletionToolParam]:
    ...
    Relevent Issues: https://github.com/Delify-Solutions/derouter/issues/5388, https://github.com/Delify-Solutions/derouter/issues/5747
    """
    return [
        ChatCompletionToolParam(
            type="function",
...
def validate_chat_completion_tool_choice(
    ...
    Prevents user errors like: https://github.com/Delify-Solutions/derouter/issues/7483
    """
...
def validate_openai_optional_params(stop: str | list[str] | None = None, **kwargs) -> str | list[str] | None:
    ...
    """
    if stop is not None and isinstance(stop, list) and not derouter.disable_stop_sequence_limit:
...
def _get_bundled_model_cost_map() -> dict[str, Any]:
    try:
        model_cost_path: Final = resources.files("derouter").joinpath("model_prices_and_context_window_backup.json")
...
def _get_model_cost_entry_for_provider_config(
    ...
    for model_key in candidate_keys:
        model_info = derouter.model_cost.get(model_key)
...
class ProviderConfigManager:
    ...
    def _build_provider_config_map() -> dict[LlmProviders, tuple[Callable, bool]]:
        ...
        return {
            ...
            LlmProviders.OPENAI: (lambda: derouter.OpenAIGPTConfig(), False),
            LlmProviders.ANTHROPIC: (lambda: derouter.AnthropicConfig(), False),
            ...
            LlmProviders.DEEPSEEK: (lambda: derouter.DeepSeekChatConfig(), False),
            LlmProviders.TENCENT: (lambda: derouter.TencentChatConfig(), False),
            LlmProviders.GROQ: (lambda: derouter.GroqChatConfig(), False),
            LlmProviders.BEDROCK_MANTLE: (
                lambda: derouter.BedrockMantleChatConfig(),
                False,
            ),
            LlmProviders.A2A: (lambda: derouter.A2AConfig(), False),
            LlmProviders.BYTEZ: (lambda: derouter.BytezChatConfig(), False),
            LlmProviders.DATABRICKS: (lambda: derouter.DatabricksConfig(), False),
            LlmProviders.XAI: (lambda: derouter.XAIChatConfig(), False),
            LlmProviders.ZAI: (lambda: derouter.ZAIChatConfig(), False),
            LlmProviders.LAMBDA_AI: (lambda: derouter.LambdaAIChatConfig(), False),
            LlmProviders.INCEPTION: (lambda: derouter.InceptionChatConfig(), False),
            LlmProviders.LLAMA: (lambda: derouter.LlamaAPIConfig(), False),
            LlmProviders.TEXT_COMPLETION_OPENAI: (
                lambda: derouter.OpenAITextCompletionConfig(),
                False,
            ),
            LlmProviders.SNOWFLAKE: (lambda: derouter.SnowflakeConfig(), False),
            LlmProviders.CLARIFAI: (lambda: derouter.ClarifaiConfig(), False),
            LlmProviders.ANTHROPIC_TEXT: (lambda: derouter.AnthropicTextConfig(), False),
            LlmProviders.VERTEX_AI_BETA: (lambda: derouter.VertexGeminiConfig(), False),
            LlmProviders.CLOUDFLARE: (lambda: derouter.CloudflareChatConfig(), False),
            LlmProviders.SAGEMAKER_CHAT: (lambda: derouter.SagemakerChatConfig(), False),
            LlmProviders.SAGEMAKER_NOVA: (lambda: derouter.SagemakerNovaConfig(), False),
            LlmProviders.SAGEMAKER: (lambda: derouter.SagemakerConfig(), False),
            LlmProviders.FIREWORKS_AI: (lambda: derouter.FireworksAIConfig(), False),
            LlmProviders.FRIENDLIAI: (lambda: derouter.FriendliaiChatConfig(), False),
            LlmProviders.WATSONX: (lambda: derouter.IBMWatsonXChatConfig(), False),
            LlmProviders.WATSONX_TEXT: (lambda: derouter.IBMWatsonXAIConfig(), False),
            LlmProviders.EMPOWER: (lambda: derouter.EmpowerChatConfig(), False),
            LlmProviders.MINIMAX: (lambda: derouter.MinimaxChatConfig(), False),
            LlmProviders.GITHUB: (lambda: derouter.GithubChatConfig(), False),
            LlmProviders.COMPACTIFAI: (lambda: derouter.CompactifAIChatConfig(), False),
            LlmProviders.GITHUB_COPILOT: (lambda: derouter.GithubCopilotConfig(), False),
            LlmProviders.CHATGPT: (lambda: derouter.ChatGPTConfig(), False),
            LlmProviders.GIGACHAT: (lambda: derouter.GigaChatConfig(), False),
            LlmProviders.RAGFLOW: (lambda: derouter.RAGFlowConfig(), False),
            LlmProviders.CUSTOM: (lambda: derouter.OpenAILikeChatConfig(), False),
            LlmProviders.CUSTOM_OPENAI: (lambda: derouter.OpenAILikeChatConfig(), False),
            LlmProviders.OPENAI_LIKE: (lambda: derouter.OpenAILikeChatConfig(), False),
            LlmProviders.AIOHTTP_OPENAI: (
                lambda: derouter.AiohttpOpenAIChatConfig(),
                False,
            ),
            LlmProviders.HOSTED_VLLM: (lambda: derouter.HostedVLLMChatConfig(), False),
            LlmProviders.LLAMAFILE: (lambda: derouter.LlamafileChatConfig(), False),
            LlmProviders.LM_STUDIO: (lambda: derouter.LMStudioChatConfig(), False),
            LlmProviders.GALADRIEL: (lambda: derouter.GaladrielChatConfig(), False),
            LlmProviders.REPLICATE: (lambda: derouter.ReplicateConfig(), False),
            LlmProviders.HUGGINGFACE: (lambda: derouter.HuggingFaceChatConfig(), False),
            LlmProviders.TOGETHER_AI: (lambda: derouter.TogetherAIChatConfig(), False),
            LlmProviders.OPENROUTER: (lambda: derouter.OpenrouterConfig(), False),
            LlmProviders.VERCEL_AI_GATEWAY: (
                lambda: derouter.VercelAIGatewayConfig(),
                False,
            ),
            LlmProviders.COMETAPI: (lambda: derouter.CometAPIConfig(), False),
            LlmProviders.DATAROBOT: (lambda: derouter.DataRobotConfig(), False),
            LlmProviders.GEMINI: (lambda: derouter.GoogleAIStudioGeminiConfig(), False),
            LlmProviders.AI21: (lambda: derouter.AI21ChatConfig(), False),
            LlmProviders.AI21_CHAT: (lambda: derouter.AI21ChatConfig(), False),
            LlmProviders.AZURE_TEXT: (lambda: derouter.AzureOpenAITextConfig(), False),
            LlmProviders.NLP_CLOUD: (lambda: derouter.NLPCloudConfig(), False),
            LlmProviders.OOBABOOGA: (lambda: derouter.OobaboogaConfig(), False),
            LlmProviders.OLLAMA_CHAT: (lambda: derouter.OllamaChatConfig(), False),
            LlmProviders.DEEPINFRA: (lambda: derouter.DeepInfraConfig(), False),
            LlmProviders.PERPLEXITY: (lambda: derouter.PerplexityChatConfig(), False),
            LlmProviders.MISTRAL: (lambda: derouter.MistralConfig(), False),
            LlmProviders.CODESTRAL: (lambda: derouter.MistralConfig(), False),
            LlmProviders.NVIDIA_NIM: (lambda: derouter.NvidiaNimConfig(), False),
            LlmProviders.CEREBRAS: (lambda: derouter.CerebrasConfig(), False),
            LlmProviders.BASETEN: (lambda: derouter.BasetenConfig(), False),
            LlmProviders.VOLCENGINE: (lambda: derouter.VolcEngineConfig(), False),
            LlmProviders.TEXT_COMPLETION_CODESTRAL: (
                lambda: derouter.CodestralTextCompletionConfig(),
                False,
            ),
            LlmProviders.TEXT_COMPLETION_INCEPTION: (
                lambda: derouter.InceptionTextCompletionConfig(),
                False,
            ),
            LlmProviders.SAMBANOVA: (lambda: derouter.SambanovaConfig(), False),
            LlmProviders.MARITALK: (lambda: derouter.MaritalkConfig(), False),
            LlmProviders.VLLM: (lambda: derouter.VLLMConfig(), False),
            LlmProviders.OLLAMA: (lambda: derouter.OllamaConfig(), False),
            LlmProviders.PREDIBASE: (lambda: derouter.PredibaseConfig(), False),
            LlmProviders.TRITON: (lambda: derouter.TritonConfig(), False),
            LlmProviders.PETALS: (lambda: derouter.PetalsConfig(), False),
            LlmProviders.SAP_GENERATIVE_AI_HUB: (
                lambda: derouter.GenAIHubOrchestrationConfig(),
                False,
            ),
            LlmProviders.FEATHERLESS_AI: (lambda: derouter.FeatherlessAIConfig(), False),
            LlmProviders.NOVITA: (lambda: derouter.NovitaConfig(), False),
            LlmProviders.NEBIUS: (lambda: derouter.NebiusConfig(), False),
            LlmProviders.WANDB: (lambda: derouter.WandbConfig(), False),
            LlmProviders.DASHSCOPE: (lambda: derouter.DashScopeChatConfig(), False),
            LlmProviders.QWENCLOUD: (lambda: derouter.QwenCloudChatConfig(), False),
            LlmProviders.QWEN_AI_PLATFORM: (
                lambda: derouter.QwenAIPlatformChatConfig(),
                False,
            ),
            LlmProviders.MODELSCOPE: (lambda: derouter.ModelScopeChatConfig(), False),
            LlmProviders.MOONSHOT: (lambda: derouter.MoonshotChatConfig(), False),
            LlmProviders.DOCKER_MODEL_RUNNER: (
                lambda: derouter.DockerModelRunnerChatConfig(),
                False,
            ),
            LlmProviders.V0: (lambda: derouter.V0ChatConfig(), False),
            LlmProviders.MORPH: (lambda: derouter.MorphChatConfig(), False),
            LlmProviders.DEROUTER_PROXY: (
                lambda: derouter.DeRouterProxyChatConfig(),
                False,
            ),
            LlmProviders.GRADIENT_AI: (lambda: derouter.GradientAIConfig(), False),
            LlmProviders.NSCALE: (lambda: derouter.NscaleConfig(), False),
            LlmProviders.HEROKU: (lambda: derouter.HerokuChatConfig(), False),
            LlmProviders.OCI: (lambda: derouter.OCIChatConfig(), False),
            LlmProviders.HYPERBOLIC: (lambda: derouter.HyperbolicChatConfig(), False),
            LlmProviders.OVHCLOUD: (lambda: derouter.OVHCloudChatConfig(), False),
            LlmProviders.AMAZON_NOVA: (lambda: derouter.AmazonNovaChatConfig(), False),
            ...
            LlmProviders.GDC: (
                lambda: derouter.GDCGeminiConfig(),
                False,
            ),
        }
    ...
    def _get_azure_config(model: str, base_model: str | None = None) -> BaseConfig:
        ...
        if derouter.AzureOpenAIO1Config().is_o_series_model(model=detection_model):
            return derouter.AzureOpenAIO1Config()
        if derouter.AzureOpenAIGPT5Config.is_model_gpt_5_model(model=detection_model):
            return derouter.AzureOpenAIGPT5Config()
        return derouter.AzureOpenAIConfig()

    @staticmethod
    def _get_azure_ai_config(model: str) -> BaseConfig:
        """Get Azure AI config based on model type."""
        from derouter.llms.azure_ai.common_utils import AzureFoundryModelInfo
    ...
    def _get_vertex_ai_config(model: str) -> BaseConfig:
        ...
        if "gemini" in model:
            return derouter.VertexGeminiConfig()
        elif "claude" in model:
            return derouter.VertexAIAnthropicConfig()
        elif "gpt-oss" in model:
            from derouter.llms.vertex_ai.vertex_ai_partner_models.gpt_oss.transformation import (
                VertexAIGPTOSSTransformation,
            )

            return VertexAIGPTOSSTransformation()
        elif model in derouter.vertex_mistral_models:
            if "codestral" in model:
                return derouter.CodestralTextCompletionConfig()
            return derouter.MistralConfig()
        elif model in derouter.vertex_ai_ai21_models:
            return derouter.VertexAIAi21Config()
        else:
            return derouter.VertexAILlama3Config()

    @staticmethod
    def _get_bedrock_config(model: str) -> BaseConfig:
        """Get Bedrock config based on model."""
        from derouter.llms.bedrock.common_utils import get_bedrock_chat_config
    ...
    def _get_cohere_config(model: str) -> BaseConfig:
        ...
        CohereModelInfo: Final = derouter_utils.CohereModelInfo
        route: Final = CohereModelInfo.get_cohere_route(model)
        if route == "v2":
            return derouter.CohereV2ChatConfig()
        return derouter.CohereChatConfig()

    @staticmethod
    def _get_langgraph_config() -> BaseConfig:
        """Get LangGraph config."""
        from derouter.llms.langgraph.chat.transformation import LangGraphConfig
    ...
    def _get_langflow_config() -> BaseConfig:
        """Get LangFlow config."""
        from derouter.llms.langflow.chat.transformation import LangFlowConfig
    ...
    def get_provider_chat_config(
        ...
        if provider == LlmProviders.OPENAI:
            from derouter.llms.openai.chat.gpt_transformation import (
                OpenAIGPTConfig,
                OpenAIUnknownModelConfig,
            )

            if derouter.openaiOSeriesConfig.is_model_o_series_model(model=model):
                return derouter.openaiOSeriesConfig
            if derouter.OpenAIGPT5Config.is_model_gpt_5_model(model=model):
                return derouter.OpenAIGPT5Config()
        ...
        from derouter.llms.openai_like.dynamic_config import create_config_class
    ...
    def get_provider_embedding_config(
        ...
        if (
            derouter.LlmProviders.VOYAGE == provider
            and derouter.VoyageContextualEmbeddingConfig.is_contextualized_embeddings(model)
        ):
            return derouter.VoyageContextualEmbeddingConfig()
        elif (
            derouter.LlmProviders.VOYAGE == provider
            and derouter.VoyageMultimodalEmbeddingConfig.is_multimodal_embeddings(model)
        ):
            return derouter.VoyageMultimodalEmbeddingConfig()
        elif derouter.LlmProviders.VOYAGE == provider:
            return derouter.VoyageEmbeddingConfig()
        elif derouter.LlmProviders.TRITON == provider:
            return derouter.TritonEmbeddingConfig()
        elif derouter.LlmProviders.WATSONX == provider:
            return derouter.IBMWatsonXEmbeddingConfig()
        elif derouter.LlmProviders.SAP_GENERATIVE_AI_HUB == provider:
            return derouter.GenAIHubEmbeddingConfig()
        elif derouter.LlmProviders.INFINITY == provider:
            return derouter.InfinityEmbeddingConfig()
        elif derouter.LlmProviders.SAMBANOVA == provider:
            return derouter.SambaNovaEmbeddingConfig()
        elif derouter.LlmProviders.OCI == provider:
            from derouter.llms.oci.embed.transformation import OCIEmbedConfig

            return OCIEmbedConfig()
        elif derouter.LlmProviders.COHERE == provider or derouter.LlmProviders.COHERE_CHAT == provider:
            from derouter.llms.cohere.embed.transformation import CohereEmbeddingConfig

            return CohereEmbeddingConfig()
        elif derouter.LlmProviders.JINA_AI == provider:
            from derouter.llms.jina_ai.embedding.transformation import (
                JinaAIEmbeddingConfig,
            )

            return JinaAIEmbeddingConfig()
        elif derouter.LlmProviders.VOLCENGINE == provider:
            from derouter.llms.volcengine.embedding.transformation import (
                VolcEngineEmbeddingConfig,
            )

            return VolcEngineEmbeddingConfig()
        elif provider in (
            derouter.LlmProviders.DASHSCOPE,
            derouter.LlmProviders.QWENCLOUD,
            derouter.LlmProviders.QWEN_AI_PLATFORM,
        ):
            from derouter.llms.dashscope.common_utils import (
                get_dashscope_family_embedding_config,
            )

            return get_dashscope_family_embedding_config(provider.value)
        elif derouter.LlmProviders.OVHCLOUD == provider:
            return derouter.OVHCloudEmbeddingConfig()
        elif derouter.LlmProviders.SNOWFLAKE == provider:
            return derouter.SnowflakeEmbeddingConfig()
        elif derouter.LlmProviders.COMETAPI == provider:
            return derouter.CometAPIEmbeddingConfig()
        elif derouter.LlmProviders.GITHUB_COPILOT == provider:
            return derouter.GithubCopilotEmbeddingConfig()
        elif derouter.LlmProviders.OPENROUTER == provider:
            from derouter.llms.openrouter.embedding.transformation import (
                OpenrouterEmbeddingConfig,
            )

            return OpenrouterEmbeddingConfig()
        elif derouter.LlmProviders.VERCEL_AI_GATEWAY == provider:
            from derouter.llms.vercel_ai_gateway.embedding.transformation import (
                VercelAIGatewayEmbeddingConfig,
            )

            return VercelAIGatewayEmbeddingConfig()
        elif derouter.LlmProviders.GIGACHAT == provider:
            return derouter.GigaChatEmbeddingConfig()
        elif derouter.LlmProviders.HOSTED_VLLM == provider:
            return derouter.HostedVLLMEmbeddingConfig()
        elif derouter.LlmProviders.SAGEMAKER == provider:
            from derouter.llms.sagemaker.embedding.transformation import (
                SagemakerEmbeddingConfig,
            )

            return SagemakerEmbeddingConfig.get_model_config(model)
        elif derouter.LlmProviders.PERPLEXITY == provider:
            return derouter.PerplexityEmbeddingConfig()
        return None
    ...
    def get_provider_rerank_config(
        ...
        if derouter.LlmProviders.COHERE == provider or derouter.LlmProviders.COHERE_CHAT == provider:
            if should_use_cohere_v1_client(api_base, present_version_params):
                return derouter.CohereRerankConfig()
            else:
                return derouter.CohereRerankV2Config()
        elif derouter.LlmProviders.AZURE_AI == provider:
            return derouter.AzureAIRerankConfig()
        elif derouter.LlmProviders.INFINITY == provider:
            return derouter.InfinityRerankConfig()
        elif derouter.LlmProviders.JINA_AI == provider:
            return derouter.JinaAIRerankConfig()
        elif derouter.LlmProviders.HOSTED_VLLM == provider:
            return derouter.HostedVLLMRerankConfig()
        elif derouter.LlmProviders.HUGGINGFACE == provider:
            return derouter.HuggingFaceRerankConfig()
        elif derouter.LlmProviders.DEEPINFRA == provider:
            return derouter.DeepinfraRerankConfig()
        elif derouter.LlmProviders.NVIDIA_NIM == provider:
            from derouter.llms.nvidia_nim.rerank.common_utils import (
                get_nvidia_nim_rerank_config,
            )

            return get_nvidia_nim_rerank_config(model)
        elif derouter.LlmProviders.VERTEX_AI == provider:
            return derouter.VertexAIRerankConfig()
        elif derouter.LlmProviders.FIREWORKS_AI == provider:
            return derouter.FireworksAIRerankConfig()
        elif derouter.LlmProviders.VOYAGE == provider:
            return derouter.VoyageRerankConfig()
        elif derouter.LlmProviders.WATSONX == provider:
            return derouter.IBMWatsonXRerankConfig()
        elif provider in (
            derouter.LlmProviders.DASHSCOPE,
            derouter.LlmProviders.QWENCLOUD,
            derouter.LlmProviders.QWEN_AI_PLATFORM,
    ...
    def _get_provider_anthropic_messages_config_cached(
        ...
        if derouter.LlmProviders.ANTHROPIC == provider:
            return derouter.AnthropicMessagesConfig()
        ...
        # This mapping ensures that the correct configuration is returned for BEDROCK.
        elif derouter.LlmProviders.BEDROCK == provider:
            from derouter.llms.bedrock.common_utils import BedrockModelInfo

            return BedrockModelInfo.get_bedrock_provider_config_for_messages_api(model)
        elif derouter.LlmProviders.VERTEX_AI == provider:
            if "claude" in model_lower:
                from derouter.llms.vertex_ai.vertex_ai_partner_models.anthropic.experimental_pass_through.transformation import (
            ...
            if "claude" in model_lower:
                from derouter.llms.azure_ai.anthropic.messages_transformation import (
                    AzureAnthropicMessagesConfig,
                )

                return AzureAnthropicMessagesConfig()
        elif derouter.LlmProviders.MINIMAX == provider:
            from derouter.llms.minimax.messages.transformation import (
                MinimaxMessagesConfig,
            )

            return MinimaxMessagesConfig()
        elif derouter.LlmProviders.DEEPSEEK == provider:
            from derouter.llms.deepseek.messages.transformation import (
                DeepSeekAnthropicMessagesConfig,
            )

            return DeepSeekAnthropicMessagesConfig()
        elif derouter.LlmProviders.TENCENT == provider:
            from derouter.llms.tencent.messages.transformation import (
            ...
            if "claude" in model_lower:
                from derouter.llms.github_copilot.messages.transformation import (
        ...
        if json_provider is not None and "/v1/messages" in json_provider.supported_endpoints:
            from derouter.llms.openai_like.messages.transformation import (
    ...
    def get_provider_audio_transcription_config(
        ...
        if (
            derouter.LlmProviders.AZURE == provider
            and model_cost_entry.get("audio_transcription_config") == "azure_speech"
        ):
            from derouter.llms.azure.audio_transcription.transformation import (
                AzureSpeechAudioTranscriptionConfig,
            )

            return AzureSpeechAudioTranscriptionConfig()
        elif derouter.LlmProviders.DEEPGRAM == provider:
            return derouter.DeepgramAudioTranscriptionConfig()
        elif derouter.LlmProviders.ELEVENLABS == provider:
        ...
        elif derouter.LlmProviders.OPENAI == provider:
            if "gpt-4o" in model:
                return derouter.OpenAIGPTAudioTranscriptionConfig()
            else:
                return derouter.OpenAIWhisperAudioTranscriptionConfig()
        elif derouter.LlmProviders.HOSTED_VLLM == provider:
            from derouter.llms.hosted_vllm.transcriptions.transformation import (
                HostedVLLMAudioTranscriptionConfig,
            )

            return HostedVLLMAudioTranscriptionConfig()
        elif derouter.LlmProviders.WATSONX == provider:
            from derouter.llms.watsonx.audio_transcription.transformation import (
        ...
        elif derouter.LlmProviders.SCALEWAY == provider:
        ...
        elif derouter.LlmProviders.MISTRAL == provider:
        ...
        elif derouter.LlmProviders.NVIDIA_RIVA == provider:
            from derouter.llms.nvidia_riva.audio_transcription.transformation import (
                NvidiaRivaAudioTranscriptionConfig,
            )

            return NvidiaRivaAudioTranscriptionConfig()
        elif derouter.LlmProviders.SONIOX == provider:
            from derouter.llms.soniox.audio_transcription.transformation import (
            ...
            if bare_vertex_model.startswith("gemini") and "transcribe" in bare_vertex_model:
                from derouter.llms.vertex_ai.audio_transcription.gemini_transcribe_transformation import (
                    VertexGeminiAudioTranscriptionConfig,
                )

                return VertexGeminiAudioTranscriptionConfig()
            from derouter.llms.vertex_ai.audio_transcription.transformation import (
                VertexAIAudioTranscriptionConfig,
            )

            return VertexAIAudioTranscriptionConfig()
        elif derouter.LlmProviders.GEMINI == provider:
            from derouter.llms.gemini.audio_transcription.transformation import (
    ...
    def get_provider_responses_api_config(
        ...
        from derouter.llms.openai_like.dynamic_config import (
    ...
    def _get_python_responses_api_config(
        ...
        if derouter.LlmProviders.OPENAI == provider:
            return derouter.OpenAIResponsesAPIConfig()
        elif derouter.LlmProviders.AZURE == provider:
            ...
            if is_o_series:
                return derouter.AzureOpenAIOSeriesResponsesAPIConfig()
            else:
                return derouter.AzureOpenAIResponsesAPIConfig()
        elif derouter.LlmProviders.XAI == provider:
            return derouter.XAIResponsesAPIConfig()
        elif derouter.LlmProviders.GITHUB_COPILOT == provider:
            from derouter.llms.github_copilot.responses.transformation import (
                github_copilot_supports_responses_api,
            )

            if model is None or github_copilot_supports_responses_api(model=model):
                return derouter.GithubCopilotResponsesAPIConfig()
            return None
        elif derouter.LlmProviders.CHATGPT == provider:
            return derouter.ChatGPTResponsesAPIConfig()
        elif derouter.LlmProviders.DEROUTER_PROXY == provider:
            return derouter.DeRouterProxyResponsesAPIConfig()
        elif derouter.LlmProviders.VOLCENGINE == provider:
            return derouter.VolcEngineResponsesAPIConfig()
        elif derouter.LlmProviders.MANUS == provider:
            return derouter.ManusResponsesAPIConfig()
        elif derouter.LlmProviders.PERPLEXITY == provider:
            return derouter.PerplexityResponsesConfig()
        elif derouter.LlmProviders.DATABRICKS == provider:
            # Databricks Responses API is only compatible with OpenAI GPT models
            if model and "gpt" in model.lower():
                return derouter.DatabricksResponsesAPIConfig()
            return None
        elif derouter.LlmProviders.OPENROUTER == provider:
            return derouter.OpenRouterResponsesAPIConfig()
        elif derouter.LlmProviders.HOSTED_VLLM == provider:
            return derouter.HostedVLLMResponsesAPIConfig()
        elif derouter.LlmProviders.FIREWORKS_AI == provider:
            return derouter.FireworksAIResponsesAPIConfig()
        elif derouter.LlmProviders.BEDROCK_MANTLE == provider:
            ...
            # /v1/responses.
            from derouter.llms.bedrock_mantle.common_utils import (
                mantle_base_segment,
                mantle_supports_responses,
            )

            if not model or not mantle_supports_responses(model, derouter.model_cost):
                return None
            return derouter.BedrockMantleResponsesAPIConfig(
                use_openai_path=mantle_base_segment(model, derouter.model_cost) == "openai/v1"
            )
        return None
    ...
    def get_provider_skills_api_config(
        ...
        """
        if derouter.LlmProviders.ANTHROPIC == provider:
            return derouter.AnthropicSkillsConfig()
        return None
    ...
    def get_provider_evals_api_config(
        ...
        """
        if derouter.LlmProviders.OPENAI == provider:
            from derouter.llms.openai.evals.transformation import OpenAIEvalsConfig
    ...
    def get_provider_text_completion_config(
        ...
        if LlmProviders.FIREWORKS_AI == provider:
            return derouter.FireworksAITextCompletionConfig()
        elif LlmProviders.TOGETHER_AI == provider:
            return derouter.TogetherAITextCompletionConfig()
        elif LlmProviders.TEXT_COMPLETION_INCEPTION == provider:
            return derouter.InceptionTextCompletionConfig()
        return derouter.OpenAITextCompletionConfig()
    ...
    def get_provider_model_info(
        ...
        if LlmProviders.FIREWORKS_AI == provider:
            return derouter.FireworksAIConfig()
        elif LlmProviders.OPENAI == provider:
            return derouter.OpenAIGPTConfig()
        elif LlmProviders.GEMINI == provider:
            return derouter.GeminiModelInfo()
        elif LlmProviders.VERTEX_AI == provider:
            from derouter.llms.vertex_ai.common_utils import VertexAIModelInfo

            return VertexAIModelInfo()
        elif LlmProviders.DEROUTER_PROXY == provider:
            return derouter.DeRouterProxyChatConfig()
        elif LlmProviders.TOPAZ == provider:
            return derouter.TopazModelInfo()
        elif LlmProviders.ANTHROPIC == provider:
            return derouter.AnthropicModelInfo()
        elif LlmProviders.XAI == provider:
            return derouter.XAIModelInfo()
        elif LlmProviders.OLLAMA == provider or LlmProviders.OLLAMA_CHAT == provider:
            # Dynamic model listing for Ollama server
            from derouter.llms.ollama.common_utils import OllamaModelInfo

            return OllamaModelInfo()
        elif LlmProviders.VLLM == provider or LlmProviders.HOSTED_VLLM == provider:
            from derouter.llms.vllm.common_utils import (
            ...
            return derouter.LemonadeChatConfig()
        elif LlmProviders.CLARIFAI == provider:
            return derouter.ClarifaiConfig()
    ...
    def get_provider_image_variation_config(
        ...
        if LlmProviders.OPENAI == provider:
            return derouter.OpenAIImageVariationConfig()
        elif LlmProviders.TOPAZ == provider:
            return derouter.TopazImageVariationConfig()
        return None
    ...
    def get_provider_files_config(
        ...
        if LlmProviders.GEMINI == provider:
            from derouter.llms.gemini.files.transformation import (
                GoogleAIStudioFilesHandler,  # experimental approach, to reduce bloat on __init__.py
            )

            return GoogleAIStudioFilesHandler()
        elif LlmProviders.VERTEX_AI == provider:
            from derouter.llms.vertex_ai.files.transformation import VertexAIFilesConfig

            return VertexAIFilesConfig()
        elif LlmProviders.BEDROCK == provider:
            from derouter.llms.bedrock.files.transformation import BedrockFilesConfig

            return BedrockFilesConfig()
        elif LlmProviders.MANUS == provider:
            from derouter.llms.manus.files.transformation import ManusFilesConfig

            return ManusFilesConfig()
        elif LlmProviders.ANTHROPIC == provider:
            from derouter.llms.anthropic.files.transformation import AnthropicFilesConfig
    ...
    def get_provider_batches_config(
        ...
        if LlmProviders.BEDROCK == provider:
            from derouter.llms.bedrock.batches.transformation import BedrockBatchesConfig
    ...
    def get_provider_vector_store_config(
        ...
        from derouter.integrations.vector_store_integrations.bedrock_vector_store import (
            BedrockVectorStore,
        )
    ...
    def get_provider_vector_stores_config(
        ...
        """
        if derouter.LlmProviders.OPENAI == provider:
            from derouter.llms.openai.vector_stores.transformation import (
                OpenAIVectorStoreConfig,
            )

            return OpenAIVectorStoreConfig()
        elif derouter.LlmProviders.AZURE == provider:
            from derouter.llms.azure.vector_stores.transformation import (
            ...
            if api_type == "rag_api" or api_type is None:  # default to rag_api
                from derouter.llms.vertex_ai.vector_stores.rag_api.transformation import (
                    VertexVectorStoreConfig,
                )

                return VertexVectorStoreConfig()
            elif api_type == "search_api":
                from derouter.llms.vertex_ai.vector_stores.search_api.transformation import (
                    VertexSearchAPIVectorStoreConfig,
                )

                return VertexSearchAPIVectorStoreConfig()
        elif derouter.LlmProviders.BEDROCK == provider:
            from derouter.llms.bedrock.vector_stores.transformation import (
                BedrockVectorStoreConfig,
            )

            return BedrockVectorStoreConfig()
        elif derouter.LlmProviders.PG_VECTOR == provider:
            from derouter.llms.pg_vector.vector_stores.transformation import (
                PGVectorStoreConfig,
            )

            return PGVectorStoreConfig()
        elif derouter.LlmProviders.AZURE_AI == provider:
            from derouter.llms.azure_ai.vector_stores.transformation import (
                AzureAIVectorStoreConfig,
            )

            return AzureAIVectorStoreConfig()
        elif derouter.LlmProviders.MILVUS == provider:
            from derouter.llms.milvus.vector_stores.transformation import (
                MilvusVectorStoreConfig,
            )

            return MilvusVectorStoreConfig()
        elif derouter.LlmProviders.GEMINI == provider:
            from derouter.llms.gemini.vector_stores.transformation import (
                GeminiVectorStoreConfig,
            )

            return GeminiVectorStoreConfig()
        elif derouter.LlmProviders.RAGFLOW == provider:
            from derouter.llms.ragflow.vector_stores.transformation import (
                RAGFlowVectorStoreConfig,
            )

            return RAGFlowVectorStoreConfig()
        elif derouter.LlmProviders.S3_VECTORS == provider:
            from derouter.llms.s3_vectors.vector_stores.transformation import (
                S3VectorsVectorStoreConfig,
            )

            return S3VectorsVectorStoreConfig()
        elif derouter.LlmProviders.VALKEY == provider:
            from derouter.llms.valkey.vector_stores.transformation import (
                ValkeyVectorStoreConfig,
            )

            return ValkeyVectorStoreConfig()
        elif derouter.LlmProviders.MONGODB == provider:
            from derouter.llms.mongodb.vector_stores.transformation import (
    ...
    def get_provider_vector_store_files_config(
        ...
        if derouter.LlmProviders.OPENAI == provider:
            from derouter.llms.openai.vector_store_files.transformation import (
    ...
    def get_provider_image_generation_config(
        ...
        if LlmProviders.OPENAI == provider:
            ...
            from derouter.llms.xinference.image_generation import (
            ...
            from derouter.llms.runwayml.image_generation import (
            ...
            from derouter.llms.black_forest_labs.image_generation import (
                get_black_forest_labs_image_generation_config,
            )

            return get_black_forest_labs_image_generation_config(model)
        elif LlmProviders.VERTEX_AI == provider:
            from derouter.llms.vertex_ai.image_generation import (
            ...
            from derouter.llms.openrouter.image_generation import (
            ...
            from derouter.llms.modelscope.image_generation import (
    ...
    def get_provider_video_config(
        ...
        if LlmProviders.OPENAI == provider:
            ...
            from derouter.llms.azure.videos.transformation import AzureVideoConfig
            ...
            from derouter.llms.vertex_ai.videos.transformation import VertexAIVideoConfig

            return VertexAIVideoConfig()
        elif LlmProviders.RUNWAYML == provider:
            from derouter.llms.runwayml.videos.transformation import RunwayMLVideoConfig

            return RunwayMLVideoConfig()
        elif LlmProviders.HOSTED_VLLM == provider:
            from derouter.llms.hosted_vllm.videos import get_hosted_vllm_video_config
    ...
    def get_provider_container_config(
        ...
        if LlmProviders.OPENAI == provider:
            from derouter.llms.openai.containers.transformation import (
                OpenAIContainerConfig,
            )

            return OpenAIContainerConfig()
        if provider in (LlmProviders.AZURE, LlmProviders.AZURE_TEXT):
            from derouter.llms.azure.containers.transformation import (
    ...
    def get_provider_realtime_config(
        ...
        if LlmProviders.GEMINI == provider:
            from derouter.llms.gemini.realtime.transformation import GeminiRealtimeConfig
    ...
    def get_provider_realtime_http_config(
        ...
        if LlmProviders.OPENAI == provider:
            from derouter.llms.openai.realtime.http_transformation import (
                OpenAIRealtimeHTTPConfig,
            )

            return OpenAIRealtimeHTTPConfig()
        if LlmProviders.AZURE == provider:
            from derouter.llms.azure.realtime.http_transformation import (
    ...
    def get_provider_image_edit_config(
        ...
        if LlmProviders.OPENAI == provider:
            from derouter.llms.openai.image_edit import get_openai_image_edit_config

            return get_openai_image_edit_config(model=model)
        elif LlmProviders.AZURE == provider:
            from derouter.llms.azure.image_edit.transformation import (
                AzureImageEditConfig,
            )

            return AzureImageEditConfig()
        elif LlmProviders.RECRAFT == provider:
            from derouter.llms.recraft.image_edit.transformation import (
                RecraftImageEditConfig,
            )

            return RecraftImageEditConfig()
        elif LlmProviders.BLACK_FOREST_LABS == provider:
            from derouter.llms.black_forest_labs.image_edit.transformation import (
                BlackForestLabsImageEditConfig,
            )

            return BlackForestLabsImageEditConfig()
        elif LlmProviders.AZURE_AI == provider:
            from derouter.llms.azure_ai.image_edit import get_azure_ai_image_edit_config

            return get_azure_ai_image_edit_config(model)
        elif LlmProviders.GEMINI == provider:
            from derouter.llms.gemini.image_edit import get_gemini_image_edit_config

            return get_gemini_image_edit_config(model)
        elif LlmProviders.DEROUTER_PROXY == provider:
            from derouter.llms.derouter_proxy.image_edit.transformation import (
                DeRouterProxyImageEditConfig,
            )

            return DeRouterProxyImageEditConfig()
        elif LlmProviders.VERTEX_AI == provider:
            from derouter.llms.vertex_ai.image_edit import (
                get_vertex_ai_image_edit_config,
            )

            return get_vertex_ai_image_edit_config(model)
        elif LlmProviders.STABILITY == provider:
            from derouter.llms.stability.image_edit import (
                get_stability_image_edit_config,
            )

            return get_stability_image_edit_config(model)
        elif LlmProviders.BEDROCK == provider:
            from derouter.llms.bedrock.image_edit.amazon_nova_canvas_image_edit_transformation import (
                get_bedrock_image_edit_config_for_model,
            )

            return get_bedrock_image_edit_config_for_model(model)
        elif LlmProviders.OPENROUTER == provider:
            from derouter.llms.openrouter.image_edit import (
    ...
    def get_provider_ocr_config(
        ...
        """
        from derouter.llms.vertex_ai.ocr.transformation import VertexAIOCRConfig

        # Special handling for Azure AI - distinguish between Mistral OCR and Document Intelligence
        if provider == derouter.LlmProviders.AZURE_AI:
            from derouter.llms.azure_ai.ocr.common_utils import get_azure_ai_ocr_config

            return get_azure_ai_ocr_config(model=model)

        if provider == derouter.LlmProviders.VERTEX_AI:
            from derouter.llms.vertex_ai.ocr.common_utils import get_vertex_ai_ocr_config

            return get_vertex_ai_ocr_config(model=model)

        if provider == derouter.LlmProviders.COHERE:
            from derouter.llms.cohere.ocr.transformation import CohereParseConfig

            return CohereParseConfig()

        if provider == derouter.LlmProviders.REDUCTO:
            from derouter.llms.reducto.ocr.transformation import (
        ...
        MistralOCRConfig: Final = derouter_utils.MistralOCRConfig
        PROVIDER_TO_CONFIG_MAP: Final = {
            derouter.LlmProviders.MISTRAL: MistralOCRConfig,
        }
    ...
    def get_provider_search_config(
        ...
        """
        from derouter.llms.apiserpent.search.transformation import (
            APISerpentSearchConfig,
        )
        from derouter.llms.azure.search.transformation import BingGroundingSearchConfig
        from derouter.llms.bedrock.search.transformation import AgentCoreSearchConfig
        from derouter.llms.brave.search.transformation import BraveSearchConfig
        from derouter.llms.dataforseo.search.transformation import DataForSEOSearchConfig
        from derouter.llms.duckduckgo.search.transformation import DuckDuckGoSearchConfig
        from derouter.llms.exa_ai.search.transformation import ExaAISearchConfig
        from derouter.llms.fastcrw.search.transformation import FastCRWSearchConfig
        from derouter.llms.firecrawl.search.transformation import FirecrawlSearchConfig
        from derouter.llms.google_pse.search.transformation import GooglePSESearchConfig
        from derouter.llms.linkup.search.transformation import LinkupSearchConfig
        from derouter.llms.nimble.search.transformation import NimbleSearchConfig
        from derouter.llms.parallel_ai.search.transformation import (
            ParallelAISearchConfig,
        )
        from derouter.llms.perplexity.search.transformation import PerplexitySearchConfig
        from derouter.llms.searchapi.search.transformation import SearchAPIConfig
        from derouter.llms.searxng.search.transformation import SearXNGSearchConfig
        from derouter.llms.serper.search.transformation import SerperSearchConfig
        from derouter.llms.tavily.search.transformation import TavilySearchConfig
        from derouter.llms.tinyfish.search.transformation import TinyfishSearchConfig
        from derouter.llms.you_com.search.transformation import YouComSearchConfig
    ...
    def get_provider_sandbox_config(
        ...
        """
        from derouter.llms.e2b.sandbox.transformation import E2BSandboxConfig
        from derouter.llms.opensandbox.sandbox.transformation import (
    ...
    def get_provider_text_to_speech_config(
        ...
        if derouter.LlmProviders.AZURE == provider:
            ...
            if model.startswith("speech/"):
                from derouter.llms.azure.text_to_speech.transformation import (
                    AzureAVATextToSpeechConfig,
                )

                return AzureAVATextToSpeechConfig()
        elif derouter.LlmProviders.ELEVENLABS == provider:
            from derouter.llms.elevenlabs.text_to_speech.transformation import (
                ElevenLabsTextToSpeechConfig,
            )

            return ElevenLabsTextToSpeechConfig()
        elif derouter.LlmProviders.RUNWAYML == provider:
            from derouter.llms.runwayml.text_to_speech.transformation import (
            ...
            if "gemini" in model:
                ...
                return None
            from derouter.llms.vertex_ai.text_to_speech.transformation import (
            ...
            from derouter.llms.minimax.text_to_speech.transformation import (
                MinimaxTextToSpeechConfig,
            )

            return MinimaxTextToSpeechConfig()
        elif derouter.LlmProviders.MISTRAL == provider:
            from derouter.llms.mistral.audio_speech.transformation import (
                MistralTextToSpeechConfig,
            )

            return MistralTextToSpeechConfig()
        elif derouter.LlmProviders.AWS_POLLY == provider:
            from derouter.llms.aws_polly.text_to_speech.transformation import (
    ...
    def get_provider_google_genai_generate_content_config(
        ...
        if derouter.LlmProviders.GEMINI == provider:
            from derouter.llms.gemini.google_genai.transformation import (
                GoogleGenAIConfig,
            )

            return GoogleGenAIConfig()
        elif derouter.LlmProviders.VERTEX_AI == provider:
            from derouter.llms.vertex_ai.google_genai.transformation import (
                VertexAIGoogleGenAIConfig,
            )
            from derouter.llms.vertex_ai.vertex_ai_partner_models.main import (
                VertexAIPartnerModels,
            )

            #########################################################
            # If Vertex Partner models like Anthropic, Mistral, etc. are used,
            # return None as we want this to go through the derouter.completion() adapter
...
def get_end_user_id_for_cost_tracking(
    ...
    service_type: Literal["derouter_logging", "prometheus"] = "derouter_logging",
) -> str | None:
    """
    Used for enforcing `disable_end_user_cost_tracking` param.

    service_type: "derouter_logging" or "prometheus" - used to allow prometheus only disable cost tracking.
    """
    get_derouter_metadata_from_kwargs: Final = getattr(sys.modules[__name__], "get_derouter_metadata_from_kwargs")
    _metadata: Final = cast(dict, get_derouter_metadata_from_kwargs(dict(derouter_params=derouter_params)))

    end_user_id: Final = cast(
        str | None,
        derouter_params.get("user_api_key_end_user_id") or _metadata.get("user_api_key_end_user_id"),
    )
    if derouter.disable_end_user_cost_tracking:
        return None

    #######################################
    # By default we don't track end_user on prometheus since we don't want to increase cardinality
    # by default derouter.enable_end_user_cost_tracking_prometheus_only is None, so we don't track end_user on prometheus
    #######################################
    if service_type == "prometheus":
        if derouter.enable_end_user_cost_tracking_prometheus_only is not True:
            return None
    return end_user_id
...
def get_standard_openai_params(params: Mapping[str, object]) -> dict:
    return {k: v for k, v in params.items() if k in derouter.OPENAI_CHAT_COMPLETION_PARAMS and v is not None}


def get_non_default_completion_params(kwargs: Mapping[str, object]) -> dict:
    openai_params: Final = derouter.OPENAI_CHAT_COMPLETION_PARAMS
    default_params: Final = openai_params + all_derouter_params
    non_default_params: Final = {
...
def get_non_default_transcription_params(kwargs: dict) -> dict:
    ...
    default_params: Final = OPENAI_TRANSCRIPTION_PARAMS + all_derouter_params
...
def return_raw_request(endpoint: CallTypes, kwargs: dict) -> RawRequestTypedDict:
    ...
    This is currently in BETA, and tested for `/chat/completions` -> `derouter.completion` calls.
    """
    from datetime import datetime

    from derouter.derouter_core_utils.derouter_logging import Logging

    derouter_logging_obj: Final = Logging(
        model="gpt-3.5-turbo",
    ...
    llm_api_endpoint: Final = getattr(derouter, endpoint.value)
    ...
    raw_request_typed_dict: Final = derouter_logging_obj.model_call_details.get("raw_request_typed_dict")
...
def jsonify_tools(tools: Sequence[object]) -> list[dict]:
    """
    Fixes https://github.com/Delify-Solutions/derouter/issues/9321
...
def __getattr__(name: str) -> Any:
    ...
    from derouter._lazy_imports import _get_lazy_import_registry
