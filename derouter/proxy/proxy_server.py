...
from derouter.constants import (
    ...
    DEROUTER_EMBEDDING_PROVIDERS_SUPPORTING_INPUT_ARRAY_OF_TOKENS,
    DEROUTER_SETTINGS_SAFE_DB_OVERRIDES,
    DEROUTER_UI_ALLOW_HEADERS,
    DEROUTER_UI_SESSION_DURATION,
...
from derouter.derouter_core_utils.safe_json_loads import safe_json_loads
...
from derouter.proxy.common_utils.cache_pydantic_utils import CacheCodec
from derouter.proxy.common_utils.callback_utils import (
    is_sensitive_callback_key,
    normalize_callback_names,
    process_callback,
    strip_callback_config,
)
from derouter.proxy.common_utils.realtime_utils import _realtime_request_body
from derouter.router_utils.add_retry_fallback_headers import (
    get_fallback_errors_from_headers,
    get_hidden_params_dict,
)
from derouter.router_utils.auto_router_model_naming import (
...
from derouter.router_utils.auto_router_tuning_baseline import (
...
messages: Final[list] = []
sys.path.insert(0, os.path.abspath("../.."))  # Adds the parent directory to the system path - for derouter local dev
...
try:
    ...
    raise ImportError(f"Missing dependency {e}. Run `pip install 'derouter[proxy]'`")
...
def generate_feedback_box():
    ...
    print(  # noqa: T201
        "\033[1;37m" + "# {:^59} #\033[0m".format("https://github.com/Delify-Solutions/derouter/issues/new")
    )
    print("\033[1;37m" + "#" + " " * box_width + "#\033[0m")  # noqa: T201
    print("\033[1;37m" + "#" + "-" * box_width + "#\033[0m")  # noqa: T201
    print()  # noqa: T201
    print(" Thank you for using DeRouter! - Krrish & Ishaan")  # noqa: T201
    print()  # noqa: T201
    print()  # noqa: T201
    print()  # noqa: T201
    print(  # noqa: T201
        "\033[1;31mGive Feedback / Get Help: https://github.com/Delify-Solutions/derouter/issues/new\033[0m"
    )
    print()  # noqa: T201
    print()  # noqa: T201
...
import derouter
import derouter._redis
from derouter import Router
from derouter._logging import _redact_string, verbose_proxy_logger, verbose_router_logger
from derouter.caching.caching import DualCache, RedisCache
from derouter.caching.redis_cluster_cache import RedisClusterCache
from derouter.constants import (
    ...
    DEROUTER_PROXY_ADMIN_NAME,
    DEROUTER_PROXY_BUDGET_NAME,
...
from derouter.exceptions import RejectedRequestError
from derouter.integrations.custom_guardrail import ModifyResponseException
from derouter.integrations.custom_logger import CustomLogger
from derouter.integrations.SlackAlerting.slack_alerting import SlackAlerting
from derouter.derouter_core_utils.agentic_loop_settings import (
    validated_max_agentic_loops,
)
from derouter.derouter_core_utils.asyncify import asyncify
from derouter.derouter_core_utils.audio_utils.utils import resolve_speech_media_type
...
from derouter.derouter_core_utils.realtime_errors import (
    realtime_error_event,
    websocket_close_reason,
)
from derouter.derouter_core_utils.sensitive_data_masker import (
    SensitiveDataMasker,
    mask_sensitive_keys,
)
...
from derouter.proxy._lazy_features import attach_lazy_features
from derouter.proxy._types import *
from derouter.proxy.analytics_endpoints.analytics_endpoints import (
    router as analytics_router,
)
from derouter.proxy.auth.auth_checks import (
    ExperimentalUIJWTToken,
    can_key_call_resolved_model,
    get_team_object,
    log_db_metrics,
)
from derouter.proxy.auth.auth_utils import (
    check_response_size_is_safe,
    is_request_body_safe,
    log_once_if_budget_reservation_disabled,
    warn_once_if_custom_auth_skips_common_checks,
)
from derouter.proxy.auth.fallback_model_access import router_fallback_access_check
from derouter.proxy.auth.handle_jwt import JWTHandler
from derouter.proxy.auth.derouter_license import AUTO_ROUTER_LICENSE_REMEDY, LicenseCheck
from derouter.proxy.auth.model_checks import (
...
from derouter.proxy.auth.password_policy import validate_password_policy
from derouter.proxy.auth.user_api_key_auth import (
    _fetch_global_spend_with_event_coordination,
    user_api_key_auth,
    user_api_key_auth_websocket,
)
from derouter.proxy.batches_endpoints.endpoints import router as batches_router

## Import All Misc routes here ##
from derouter.proxy.caching_routes import router as caching_router
from derouter.proxy.common_request_processing import (
...
from derouter.proxy.common_utils.auth_cache_invalidation_pubsub import (
    AuthCacheInvalidationSubscriber,
)
from derouter.proxy.common_utils.callback_utils import initialize_callbacks_on_proxy
from derouter.proxy.common_utils.config_sync_pubsub import ConfigSyncSubscriber
from derouter.proxy.common_utils.debug_utils import init_verbose_loggers
from derouter.proxy.common_utils.debug_utils import router as debugging_endpoints_router
from derouter.proxy.common_utils.encrypt_decrypt_utils import (
    decrypt_value_helper,
    encrypt_value_helper,
)
from derouter.proxy.common_utils.healthy_model_filter import (
    get_hidden_unhealthy_model_names,
    is_healthy_only_listing_default,
)
from derouter.proxy.common_utils.html_forms.ui_login import build_ui_login_form
from derouter.proxy.common_utils.http_parsing_utils import (
    _read_request_body,
    _safe_get_request_headers,
    check_file_size_under_limit,
    get_form_data,
)
from derouter.proxy.common_utils.load_config_utils import (
    get_config_file_contents_from_gcs,
    get_file_contents_from_s3,
)
from derouter.proxy.common_utils.model_deprecation import collect_model_deprecations
from derouter.proxy.common_utils.model_listing_utils import (
    TeamModelNameTranslator,
    configured_display_names,
)
from derouter.proxy.common_utils.openai_endpoint_utils import (
    remove_sensitive_info_from_deployment,
)
from derouter.proxy.common_utils.periodic_reload_schedule import (
...
from derouter.proxy.common_utils.proxy_state import ProxyState
from derouter.proxy.common_utils.reset_budget_job import ResetBudgetJob
from derouter.proxy.common_utils.scheduled_job_stagger import (
    apply_scheduled_job_stagger,
    attach_job_timing_logger,
    parse_stagger_settings,
    stagger_trigger,
)
from derouter.proxy.common_utils.swagger_utils import ERROR_RESPONSES
from derouter.proxy.common_utils.timezone_utils import (
    get_budget_reset_settings,
    get_budget_reset_time,
)
from derouter.proxy.common_utils.user_api_key_cache import (
    UserApiKeyCache,
    end_user_cache_key,
...
from derouter.proxy.config_resolvers import resolve_fields
from derouter.proxy.config_resolvers.alerting import (
    EMAIL_DESCRIPTORS,
    MS_TEAMS_DESCRIPTORS,
    SLACK_DESCRIPTORS,
)
from derouter.proxy.container_endpoints.endpoints import router as container_router
from derouter.proxy.credential_endpoints.endpoints import router as credential_router
from derouter.proxy.db.db_transaction_queue.spend_log_cleanup import (
    SPEND_LOG_CLEANUP_BOUND_SETTINGS,
    SpendLogCleanup,
)
from derouter.proxy.db.db_transaction_queue.window_spend_update_queue import (
    build_window_spend_transaction,
)
from derouter.proxy.db.exception_handler import (
    PrismaDBExceptionHandler,
    call_with_db_reconnect_retry,
)
from derouter.proxy.db.gateway_request_tracking import (
    GatewayRequestAccumulator,
    flush_gateway_requests,
)
from derouter.proxy.db.proxy_worker_heartbeat import (
    PROXY_WORKER_HEARTBEAT_INTERVAL_SECONDS,
    ProxyWorkerHeartbeat,
)
from derouter.proxy.db.spend_counter_reseed import END_USER_COUNTER_PREFIX, SpendCounterReseed
from derouter.proxy.discovery_endpoints import ui_discovery_endpoints_router
from derouter.proxy.fine_tuning_endpoints.endpoints import router as fine_tuning_router
from derouter.proxy.fine_tuning_endpoints.endpoints import set_fine_tuning_config
from derouter.proxy.google_endpoints.endpoints import router as google_router
from derouter.proxy.guardrails.init_guardrails import (
    init_guardrails_v2,
    initialize_guardrails,
)
from derouter.proxy.health_check import (
...
from derouter.proxy.health_endpoints._health_endpoints import router as health_router
from derouter.proxy.hooks.model_max_budget_limiter import (
    _PROXY_VirtualKeyModelMaxBudgetLimiter,
)
from derouter.proxy.hooks.prompt_injection_detection import (
    _OPTIONAL_PromptInjectionDetection,
)
from derouter.proxy.hooks.proxy_track_cost_callback import _ProxyDBLogger
from derouter.proxy.image_endpoints.endpoints import router as image_router
from derouter.proxy.list_api.common import (
    PROBLEM_TYPE_BASE,
    ManagementProblem,
    problem_response,
)
from derouter.proxy.derouter_pre_call_utils import add_derouter_data_to_request
from derouter.proxy.logging_endpoints.callback_logs_endpoints import (
    rust_control_plane_router,
)
from derouter.proxy.management_endpoints.auto_router_endpoints import (
    router as auto_router_management_router,
)
from derouter.proxy.management_endpoints.budget_management_endpoints import (
    router as budget_management_router,
)
from derouter.proxy.management_endpoints.cache_settings_endpoints import (
    router as cache_settings_router,
)
from derouter.proxy.management_endpoints.callback_management_endpoints import (
    router as callback_management_endpoints_router,
)
from derouter.proxy.management_endpoints.common_utils import (
    _user_has_admin_privileges,
    _user_has_admin_view,
    admin_can_invite_user,
)
from derouter.proxy.management_endpoints.coordination_redis_endpoints import (
    get_persisted_coordination_redis_settings,
)
from derouter.proxy.management_endpoints.coordination_redis_endpoints import (
    router as coordination_redis_settings_router,
)
from derouter.proxy.management_endpoints.cost_tracking_settings import (
    router as cost_tracking_settings_router,
)
from derouter.proxy.management_endpoints.customer_endpoints import (
    router as customer_router,
)
from derouter.proxy.management_endpoints.fallback_management_endpoints import (
    router as fallback_management_router,
)
from derouter.proxy.management_endpoints.gateway_request_endpoints import (
...
from derouter.proxy.management_endpoints.management_v1 import (
    router as management_v1_router,
)
from derouter.proxy.management_endpoints.management_v1.common import MANAGEMENT_V1_PREFIX
from derouter.proxy.management_endpoints.model_access_group_management_endpoints import (
    router as model_access_group_management_router,
)
from derouter.proxy.management_endpoints.model_management_endpoints import (
    _add_model_to_db,
    _add_team_model_to_db,
    _deduplicate_derouter_router_models,
    live_model_ids_snapshot,
)
from derouter.proxy.management_endpoints.model_management_endpoints import (
    router as model_management_router,
)
from derouter.proxy.management_endpoints.organization_endpoints import (
    router as organization_router,
)
from derouter.proxy.management_endpoints.router_settings_endpoints import (
    router as router_settings_router,
)
from derouter.proxy.management_endpoints.tag_management_endpoints import (
    router as tag_management_router,
)
from derouter.proxy.management_endpoints.team_callback_endpoints import (
    router as team_callback_router,
)
from derouter.proxy.management_endpoints.team_endpoints import router as team_router
from derouter.proxy.management_endpoints.team_endpoints import (
    update_team,
    validate_membership,
)
from derouter.proxy.management_endpoints.ui_sso import (
    get_disabled_non_admin_personal_key_creation,
)
from derouter.proxy.management_endpoints.ui_sso import router as ui_sso_router
from derouter.proxy.management_endpoints.user_agent_analytics_endpoints import (
    router as user_agent_analytics_router,
)
from derouter.proxy.management_endpoints.workflow_management_endpoints import (
...
from derouter.proxy.management_helpers.team_metadata_validation import (
    TEAM_METADATA_SCHEMA_REGISTRY,
    TEAM_METADATA_VALIDATOR_REGISTRY,
    parse_team_metadata_schema,
)
from derouter.proxy.memory.memory_endpoints import router as memory_router
from derouter.proxy.middleware.billable_request_metrics_middleware import (
    BillableRequestMetricsMiddleware,
    BillingRecorder,
)
from derouter.proxy.plugin_routes import (
    register_plugins_from_config,
)
from derouter.proxy.plugin_routes import (
    router as plugin_router,
)
from derouter.types.proxy.management_endpoints.management_v1 import ProblemDetail

try:
    from derouter.proxy.enterprise_billing.billing_metrics import (
...
from derouter.proxy.middleware.admission_control_middleware import (
    AdmissionControlMiddleware,
    admission_control_state,
    get_admission_control_settings,
)
from derouter.proxy.middleware.in_flight_requests_middleware import (
    InFlightRequestsMiddleware,
)
from derouter.proxy.middleware.per_request_root_path_middleware import (
    PerRequestRootPathMiddleware,
    get_server_root_paths,
)
from derouter.proxy.middleware.prometheus_auth_middleware import PrometheusAuthMiddleware
from derouter.proxy.middleware.request_size_limit_middleware import (
    RequestSizeLimitMiddleware,
)
from derouter.proxy.middleware.security_headers_middleware import (
    SecurityHeadersMiddleware,
)
from derouter.proxy.ocr_endpoints.endpoints import router as ocr_router
from derouter.proxy.openai_files_endpoints.files_endpoints import (
    router as openai_files_router,
)
from derouter.proxy.openai_files_endpoints.files_endpoints import (
    set_files_config,
)
from derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints import (
...
from derouter.proxy.pass_through_endpoints.pass_through_endpoints import (
    initialize_pass_through_endpoints,
)
from derouter.proxy.pass_through_endpoints.pass_through_endpoints import (
    router as pass_through_router,
)
from derouter.proxy.prometheus_cleanup import mark_dead_workers, mark_worker_exit
from derouter.proxy.public_endpoints import router as public_endpoints_router
from derouter.proxy.public_endpoints.public_v1 import router as public_v1_router
from derouter.proxy.rag_endpoints.endpoints import router as rag_router
from derouter.proxy.rerank_endpoints.endpoints import router as rerank_router
from derouter.proxy.response_api_endpoints.endpoints import router as response_router
from derouter.proxy.route_llm_request import route_request
from derouter.proxy.search_endpoints.endpoints import router as search_router
from derouter.proxy.shutdown.graceful_shutdown_manager import GracefulShutdownManager
from derouter.proxy.spend_tracking.budget_reservation import get_budget_window_start
from derouter.proxy.spend_tracking.spend_management_endpoints import (
    router as spend_management_router,
)
from derouter.proxy.spend_tracking.spend_tracking_utils import get_logging_payload
from derouter.proxy.types_utils.utils import get_instance_fn
from derouter.proxy.ui_crud_endpoints.proxy_setting_endpoints import (
    router as ui_crud_endpoints_router,
)
from derouter.proxy.ui_crud_endpoints.user_banner_endpoints import (
    router as user_banner_endpoints_router,
)
from derouter.proxy.utils import (
    PrismaClient,
    ProxyLogging,
    ProxyUpdateSpend,
    _cache_user_row,
    ...
    derouter_config_cache,
    migrate_passwords_to_scrypt_async,
    model_dump_with_preserved_fields,
    prefetch_config_params,
    update_spend,
)
from derouter.proxy.video_endpoints.endpoints import router as video_router
from derouter.repositories.base_repository import SupportsModelDump
from derouter.repositories.credentials_repository import CredentialsRepository
from derouter.repositories.prisma_protocols import TableActions
from derouter.router import (
    AssistantsTypedDict,
    Deployment,
    DeRouter_Params,
    ModelGroupInfo,
)
from derouter.scheduler import FlowItem, Scheduler
from derouter.secret_managers.aws_secret_manager import load_aws_kms
from derouter.secret_managers.google_kms import load_google_kms
from derouter.secret_managers.main import (
    get_secret,
    get_secret_bool,
    get_secret_str,
...
from derouter.types.integrations.slack_alerting import AlertType, SlackAlertingArgs
...
from derouter.types.proxy.management_endpoints.model_management_endpoints import (
    ModelGroupInfoProxy,
)
from derouter.types.proxy.management_endpoints.ui_sso import (
    DefaultTeamSSOParams,
    DeRouter_UpperboundKeyGenerateParams,
)
from derouter.types.proxy.model_deprecation import (
    DEFAULT_DEPRECATION_WARN_DAYS,
    ModelDeprecationResponse,
)
from derouter.types.realtime import RealtimeQueryParams
from derouter.types.router import (
    ClassifierPlugin,
    DeploymentTypedDict,
...
from derouter.types.router import ModelInfo as RouterModelInfo
from derouter.types.scheduler import DefaultPriorities
from derouter.types.secret_managers.main import (
    KeyManagementSettings,
    KeyManagementSystem,
)
from derouter.types.utils import CredentialItem, CustomHuggingfaceTokenizer, RawRequestTypedDict, StandardLoggingPayload
from derouter.types.utils import ModelInfo as ModelMapInfo
from derouter.utils import _add_custom_logger_callback_to_specific_event

try:
    from derouter._version import version
except Exception:
    version = "0.0.0"
derouter.suppress_debug_info = True
import json
...
try:
    # when using derouter cli
    from derouter.proxy import enterprise
except Exception:
    # when using derouter docker image
    try:
        import enterprise
    except Exception:
        pass
...
try:
    from derouter_enterprise.proxy.enterprise_routes import router as _enterprise_router
    from derouter_enterprise.proxy.proxy_server import EnterpriseProxyConfig
...
global_max_parallel_request_retries_env: Final[str | None] = os.getenv("DEROUTER_GLOBAL_MAX_PARALLEL_REQUEST_RETRIES")
...
def _redact_worker_config_for_logging(worker_config: str | dict[str, JsonValue] | None) -> JsonValue:
    ...
    (`DEROUTER_DISABLE_REDACT_SECRETS=true`, an older log sink, a downstream
...
global_max_parallel_request_retry_timeout_env: Final[str | None] = os.getenv(
    "DEROUTER_GLOBAL_MAX_PARALLEL_REQUEST_RETRY_TIMEOUT"
)
...
ui_message = f"👉 [```DeRouter Admin Panel on /ui```]({ui_link}). Create, Edit Keys with SSO. Having issues? Try [```Fallback Login```]({fallback_login_link})"
ui_message += "\n\n💸 [```DeRouter Model Cost Map```](https://models.router.delify.vn/)."

ui_message += f"\n\n🔎 [```DeRouter Model Hub```]({model_hub_link}). See available models on the proxy. [**Docs**](https://router.delify.vn/docs/proxy/ai_hub)"

custom_swagger_message: Final = (
    "[**Customize Swagger Docs**](https://router.delify.vn/docs/proxy/enterprise#swagger-docs---custom-routes--branding)"
)

### CUSTOM BRANDING [ENTERPRISE FEATURE] ###
_title: Final = os.getenv("DOCS_TITLE", "DeRouter API") if premium_user else "DeRouter API"
_description: Final = (
    os.getenv(
        "DOCS_DESCRIPTION",
...
async def _flush_spend_logs_queue_on_shutdown() -> None:
    ...
    try:
        from derouter.proxy.utils import drain_spend_logs_queue
...
async def proxy_shutdown_event(worker_heartbeat: ProxyWorkerHeartbeat | None = None) -> None:
    ...
    verbose_proxy_logger.info("Shutting down DeRouter Proxy Server")
    ...
    if derouter.cache is not None:
        await derouter.cache.disconnect()
    ...
    if "langfuse" in derouter.success_callback:
        try:
            # flush langfuse logs on shutdow
            from derouter.utils import langFuseLogger
...
async def proxy_startup_event(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    _startup_hooks_env: Final = os.environ.get("DEROUTER_WORKER_STARTUP_HOOKS", "")
    ...
    ## CHECK PREMIUM USER
    verbose_proxy_logger.debug("derouter.proxy.proxy_server.py::startup() - CHECKING PREMIUM USER - %s", premium_user)
    if premium_user is False:
        premium_user = _license_check.is_premium()

    ## CHECK MASTER KEY IN ENVIRONMENT ##
    master_key = get_secret_str("DEROUTER_MASTER_KEY")
    ### LOAD CONFIG ###
    ...
    if env_config_yaml is not None:
        ...
        if (
            ...
            or os.environ.get("DEROUTER_CONFIG_BUCKET_NAME") is not None
    ...
    if prisma_client is not None:
        ...
        async def _run_agent_grant_id_migration() -> None:
            from derouter.proxy.agent_endpoints.agent_registry import (
    ...
    db_coordination_redis_cache: Final = await ProxyStartupEvent._init_coordination_redis_from_db(
        derouter_settings=proxy_config.get_config_state().get("derouter_settings") or {},
    ...
    ## configured.
    try:
        from derouter.integrations.otel.model.config import is_otel_v2_enabled

        if is_otel_v2_enabled():
            from opentelemetry import trace as _otel_trace

            from derouter.integrations.otel.logger import (
                OpenTelemetryV2,
                publish_global_otel_v2_provider,
            )
            from derouter.derouter_core_utils.derouter_logging import _in_memory_loggers
    ...
    # Read derouter_settings from config for semantic filter initialization
    try:
        verbose_proxy_logger.debug("About to initialize semantic tool filter")
        _config: Final = proxy_config.get_config_state()
        _derouter_settings: Final = _config.get("derouter_settings", {})
        verbose_proxy_logger.debug("derouter_settings keys = %s", list(_derouter_settings.keys()))
        await ProxyStartupEvent._initialize_semantic_tool_filter(
            llm_router=llm_router,
            derouter_settings=_derouter_settings,
        )
    ...
    if prisma_client is not None and derouter.max_budget > 0:
    ...
    ## [Optional] Initialize Pyroscope continuous profiling (env: DEROUTER_ENABLE_PYROSCOPE=true)
...
# `is_secret` flags in derouter.proxy.config_resolvers and the
...
## DEROUTER_OTEL_V2). This MUST run at app-creation time — once the lifespan runs,
## the middleware stack is frozen and ``instrument_app`` raises "Cannot add
## middleware after an application has started". See
## ``derouter.integrations.otel.mount`` for the full rationale; the call is a safe
## no-op when the gate is off or the instrumentation package is unavailable.
from derouter.integrations.otel.mount import instrument_fastapi_app
...
def get_openapi_schema():
    ...
    from derouter.proxy.common_utils.openapi_schema_compat import (
    ...
    from derouter.proxy.common_utils.custom_openapi_spec import CustomOpenAPISpec
    ...
    from derouter.proxy._lazy_features import inject_lazy_stubs, loaded_lazy_modules
...
def custom_openapi():
    ...
    openai_routes: Final = DeRouterRoutes.openai_routes.value
...
_ProxyModelRow: TypeAlias = "prisma_models.DeRouter_ProxyModelTable"


def _config_param_table(client: PrismaClient | None) -> TableActions[_ConfigParamRow]:
    return cast(  # cast-ok: this is prisma's DeRouter_Config actions object, which parses its Json column to a mapping
...
def _get_cors_config(
    ...
    Args:
        cors_origins_env: Value of DEROUTER_CORS_ORIGINS (defaults to os.getenv).
        cors_credentials_env: Value of DEROUTER_CORS_ALLOW_CREDENTIALS (defaults to os.getenv).

    Returns:
        Tuple[List[str], bool]: (origins, allow_credentials)
    """
    _origins_raw: Final = cors_origins_env if cors_origins_env is not None else os.getenv("DEROUTER_CORS_ORIGINS")
    ...
    # Set DEROUTER_CORS_ALLOW_CREDENTIALS=true to explicitly restore the old behaviour
    ...
    _credentials_raw: Final = (
        cors_credentials_env if cors_credentials_env is not None else os.getenv("DEROUTER_CORS_ALLOW_CREDENTIALS")
    )
...
try:
    ...
    derouter_asset_prefix: Final = "/derouter-asset-prefix"
    ...
    def _is_ui_pre_restructured(ui_dir: str) -> bool:
        ...
        Returns True if:
        1. Marker file .derouter_ui_ready exists (created by Dockerfile), OR
        ...
        marker_file: Final = os.path.join(ui_dir, ".derouter_ui_ready")
    ...
    is_non_root: Final = os.getenv("DEROUTER_NON_ROOT", "").lower() == "true"

    # Determine runtime UI path
    # Priority: DEROUTER_UI_PATH env var > default path based on is_non_root
    if is_non_root:
        default_runtime_ui_path = "/var/lib/derouter/ui"
    else:
        default_runtime_ui_path = packaged_ui_path

    runtime_ui_path: Final = os.getenv("DEROUTER_UI_PATH", default_runtime_ui_path)
    ...
    if should_use_runtime_path:
        ...
        # Case 4: Runtime UI missing - try to populate
            ...
            if success:
                ...
                verbose_proxy_logger.warning(
                    "Failed to populate UI at %s: %s. Falling back to packaged UI at %s. For read-only deployments, pre-build UI in Dockerfile or set DEROUTER_UI_PATH to a writable emptyDir volume.",
                    runtime_ui_path,
                    error,
                    packaged_ui_path,
                )
                ...
                for filename in files:
                    ...
                    try:
                        ...
                        modified_content = content.replace(
                            f"{derouter_asset_prefix}",
                            f"{server_root_path}",
                        )

                        # Replace the /.well-known/derouter-ui-config with the server root path
                        modified_content = modified_content.replace(
                            "/derouter/.well-known/derouter-ui-config",
                            f"{server_root_path}/.well-known/derouter-ui-config",
                        )
    ...
    def _restructure_ui_html_files(ui_root: str) -> None:
        ...
        for current_root, _, files in os.walk(ui_root):
            ...
            if first_segment in {"_next", "derouter-asset-prefix"}:
                continue
...
app.add_middleware(
    ...
    allow_methods=["*"],
    allow_headers=["*"],
    expose_headers=DEROUTER_UI_ALLOW_HEADERS,
)
...
def mount_swagger_ui():
    ...
    from derouter.proxy._lazy_features import lazy_tag_to_prefix
...
derouter.logging_callback_manager.add_derouter_callback(model_max_budget_limiter)
...
# Tests that need to reset it can patch 'derouter.proxy.proxy_server._pkce_no_redis_warning_emitted'.
...
queue: Final[list] = []
derouter_proxy_budget_name: Final = DEROUTER_PROXY_BUDGET_NAME
derouter_proxy_admin_name = DEROUTER_PROXY_ADMIN_NAME
...
# DeRouter_DailyGatewayRequests by the update_gateway_requests scheduler job.
...
def load_from_azure_key_vault(use_azure_key_vault: bool = False):
    ...
    try:
        ...
        derouter.secret_manager_client = client
...
def cost_tracking():
    ...
    if prisma_client is not None:
        from derouter.integrations.shadow_eval_logger import ShadowEvalLogger

        derouter.logging_callback_manager.add_derouter_callback(_ProxyDBLogger())
        derouter.logging_callback_manager.add_derouter_async_success_callback(_ProxyDBLogger())
        derouter.logging_callback_manager.add_derouter_callback(ShadowEvalLogger())
...
async def get_current_spend(
    ...
    that row is missing or stale; end-user counters read ``DeRouter_EndUserTable``, the
...
async def _reconcile_budget_reservation_for_counter_update(
    ...
    from derouter.proxy.spend_tracking.budget_reservation import (
...
async def _enqueue_window_spend_row_update(
    ...
    """Queue this request's cost against the DeRouter_BudgetWindowSpend row for
    the window, so enforcement can read a maintained total instead of
    aggregating DeRouter_SpendLogs.

    request_started_at is this request's DeRouter_SpendLogs startTime; the flush
    ...
    the read path's DeRouter_SpendLogs fallback rather than rewritten per
    request.
    """
        ...
        try:
            for _id in user_ids:
                ...
                if cached_user is None:
                    ...
                    return
                existing_spend_obj = CacheCodec.deserialize(cached_user, DeRouter_UserTable)
                if existing_spend_obj is None:
                    return
                verbose_proxy_logger.debug(
                ...
                values_to_update_in_cache.append(
                    (
                        _id,
                        CacheCodec.serialize(existing_spend_obj, model_type=DeRouter_UserTable),
                    )
                )
            ## UPDATE GLOBAL PROXY ##
    ...
    async def _update_end_user_cache():
        ...
        try:
            ...
            if cached_end_user is None:
                # if user does not exist in DeRouter_UserTable, create a new user
                # do nothing if end-user not in api key cache
                return
            existing_spend_obj: Final = CacheCodec.deserialize(cached_end_user, DeRouter_EndUserTable)
            if existing_spend_obj is None:
                return
            verbose_proxy_logger.debug(
            ...
            values_to_update_in_cache.append(
                (
                    _id,
                    CacheCodec.serialize(existing_spend_obj, model_type=DeRouter_EndUserTable),
                )
            )
        except Exception as e:
            verbose_proxy_logger.warning(
    ...
    async def _update_team_cache():
        ...
        try:
            ...
            if cached_team is None:
                ...
                return
            existing_spend_obj: Final[DeRouter_TeamTableCachedObj | None] = CacheCodec.deserialize(
                cached_team, DeRouter_TeamTableCachedObj
            )
            if existing_spend_obj is None:
                return
            verbose_proxy_logger.debug(
            ...
            values_to_update_in_cache.append(
                (
                    _id,
                    CacheCodec.serialize(existing_spend_obj, model_type=DeRouter_TeamTableCachedObj),
                )
            )
        except Exception as e:
            verbose_proxy_logger.warning(
    ...
    async def _update_tag_cache():
        ...
        try:
            for tag_name in tags:
                ...
                existing_tag_obj = CacheCodec.deserialize(cached_tag, DeRouter_TagTable)
                ...
                values_to_update_in_cache.append(
                    (
                        cache_key,
                        CacheCodec.serialize(existing_tag_obj, model_type=DeRouter_TagTable),
                    )
                )
        except Exception as e:
            verbose_proxy_logger.warning(
    ...
    if local_object_updates:
        asyncio.create_task(
            user_api_key_cache.async_set_cache_pipeline(
                ...
                derouter_parent_otel_span=parent_otel_span,
                local_only=True,
            )
        )
...
def run_ollama_serve():
    try:
        ...
        verbose_proxy_logger.debug(
            "\n            DeRouter Warning: proxy started with `ollama` model\n`ollama serve` failed with Exception%s. \nEnsure you run `ollama serve`\n        ",
            e,
        )
...
def _schedule_background_health_check_db_save(
    ...
    from derouter.proxy.health_endpoints._health_endpoints import (
...
def _write_health_state_to_router_cache(
    ...
    is on.
    """
    from derouter.proxy.health_check import build_deployment_health_states
    from derouter.router_utils.cooldown_handlers import _set_cooldown_deployments
    from derouter.router_utils.router_callbacks.track_deployment_metrics import (
    ...
    try:
        ...
        for endpoint in unhealthy_endpoints:
            ...
            increment_deployment_failures_for_current_minute(
                derouter_router_instance=llm_router,
...
async def _run_background_health_check():
    ...
    if use_shared_health_check and redis_usage_cache is not None:
        from derouter.proxy.health_check_utils.shared_health_check_manager import (
...
# Fields in ``derouter_settings`` / ``general_settings`` whose values flow
...
_DB_OVERLAY_REMOTE_MODULE_STR_FIELDS: Final[dict[str, tuple[str, ...]]] = {
    "derouter_settings": ("post_call_rules",),
    "general_settings": (
        "custom_auth",
        "custom_key_generate",
        "custom_key_update",
...
def _scrub_guardrail_inner(inner: dict[str, JsonValue]) -> None:
    ...
    if isinstance(cbs, list):
        ...
        if len(cleaned) != len(cbs):
            verbose_proxy_logger.warning(
                "Refused %d remote-URL entries from DB-overlay derouter_settings.guardrails[...].callbacks",
                len(cbs) - len(cleaned),
            )
            inner["callbacks"] = cleaned
    if _is_remote_module_url(inner.get("guardrail")):
        verbose_proxy_logger.warning(
            "Refused remote-URL guardrail module from DB-overlay derouter_settings.guardrails[...].guardrail: %r",
            inner.get("guardrail"),
        )
        ...
        if isinstance(cpm, list):
            for item in cpm:
                if isinstance(item, dict) and _is_remote_module_url(item.get("custom_handler")):
                    verbose_proxy_logger.warning(
                        "Refused remote-URL custom_handler from DB-overlay derouter_settings.custom_provider_map: %r",
                        item.get("custom_handler"),
                    )
                    item["custom_handler"] = None
    # ``derouter_settings.guardrails`` is a list of single-key dicts in
    # v1 ({guardrail_name: {callbacks: [...], default_on: bool}}) or a
    # list of v2 entries ({guardrail_name, derouter_params: {guardrail:
    ...
    if section == "derouter_settings":
        ...
        if isinstance(guardrails, list):
            for entry in guardrails:
                ...
                lp = entry.get("derouter_params")
                if isinstance(lp, dict):
                    _scrub_guardrail_inner(lp)

    # ``general_settings.derouter_jwtauth.custom_validate`` is a nested
    # string field.
    if section == "general_settings":
        jwt: Final = sanitized.get("derouter_jwtauth")
        if isinstance(jwt, dict) and _is_remote_module_url(jwt.get("custom_validate")):
            verbose_proxy_logger.warning(
                "Refused remote-URL custom_validate from DB-overlay general_settings.derouter_jwtauth: %r",
...
def _apply_ssrf_general_settings(settings: Mapping[str, object]) -> None:
    if "user_url_allowed_hosts" in settings:
        derouter.user_url_allowed_hosts = cast(list[str], settings["user_url_allowed_hosts"])
    ...
    if user_url_validation is not None:
        derouter.user_url_validation = user_url_validation

    if "provider_url_destination_allowed_hosts" in settings:
        derouter.provider_url_destination_allowed_hosts = cast(
...
def _environment_has_redis_connection_target() -> bool:
    ...
    client.
    """
    redis_env_kwargs: Final = derouter._redis._redis_kwargs_from_environment()
    return (
...
def _build_redis_usage_cache_from_environment() -> RedisCache | None:
    ...
    if not _environment_has_redis_connection_target():
        return None
    return _build_redis_usage_cache(derouter._redis._redis_kwargs_from_environment())
...
def _attach_redis_usage_cache(redis_cache: RedisCache, enable_redis_auth_cache: bool) -> None:
    ...
    spend_counter_cache.attach_redis_cache(
        redis_cache,
        default_redis_ttl=derouter.default_redis_ttl,
    )
    ...
    if enable_redis_auth_cache is True:
        ...
        verbose_proxy_logger.info(
            ...
            "derouter_settings.enable_redis_auth_cache: true to share "
            "the auth cache across workers and reduce DB load."
        )
    derouter_config_cache.redis_cache = redis_cache
...
def resolve_routing_plugins(
    ...
    convention `derouter_settings.callbacks` uses, which resolves both local module
...
def validate_deployment_max_agentic_loops(model: Mapping[str, object]) -> None:
    ...
    Checked here rather than on `DeRouter_Params` because the proxy builds its
    ...
    """
    derouter_params: Final = model.get("derouter_params")
    if not isinstance(derouter_params, Mapping):
        return
    if "max_agentic_loops" not in derouter_params:
        return

    model_name: Final = model.get("model_name", "")
    validated_max_agentic_loops(
        derouter_params["max_agentic_loops"],
        field=f"derouter_params.max_agentic_loops on model {model_name!r}",
    )
...
def validate_deployment_complexity_router_placement(model: Mapping[str, object]) -> None:
    ...
    Checked here rather than on `DeRouter_Params` for the same reason as
    ...
    present_fields: Final = frozenset(
        field for field in STRATEGY_ROUTER_PARAM_FIELDS if derouter_params.get(field) is not None
    )
    if not carries_complexity_router_settings(str(derouter_params.get("model") or ""), present_fields):
        return
    violation: Final = validate_complexity_router_config_placement(derouter_params)
...
def pin_complexity_router_model_id(model: dict) -> None:  # mutable-ok: out-param, model_info is stamped in place
    """
    Stamps `model_info.id` from the raw derouter_params before plugin resolution swaps
    ...
    """
    derouter_params: Final = model.get("derouter_params")
    if not isinstance(derouter_params, dict) or not isinstance(derouter_params.get("complexity_router_config"), dict):
        return
    ...
    if model_info.get("id") is None:
        model_info["id"] = derouter.Router.generate_model_id(
...
def _swap_in_model_cost_map(new_model_cost_map: dict) -> int:
    """Adopt a freshly fetched cost map into this process's derouter state, return the model count"""
    derouter.model_cost = new_model_cost_map
    # Invalidate case-insensitive lookup map since model_cost was replaced
    _invalidate_model_cost_lowercase_map()
    # Repopulate provider model sets (e.g. derouter.anthropic_models) so that
    # wildcard patterns like "anthropic/*" include any newly added models.
    derouter.add_known_models(model_cost_map=new_model_cost_map)
...
class ProxyConfig:
    ...
    def __init__(self) -> None:
        ...
        from derouter.derouter_core_utils.get_model_cost_map import (
    ...
    async def save_config(self, new_config: dict, include_env_vars: bool = False):
        ...
        - if api_key is passed, save that to the local environment or connected secret manage (maybe expose `derouter.save_secret()`)
        """
    ...
    def _initialize_secret_manager_from_raw_config(
        ...
        """
        if derouter.secret_manager_client is not None:
            return
        ...
        if isinstance(raw_settings, dict):
            derouter._key_management_settings = KeyManagementSettings(
    ...
    def load_team_config(self, team_id: str):
        ...
        ## DEROUTER MODULE SETTINGS (e.g. derouter.drop_params=True,..)
        derouter_settings: Final = config.get("derouter_settings", {})
        all_teams_config: Final = derouter_settings.get("default_team_settings", None)
    ...
    def _init_coordination_redis(self, config: dict) -> RedisCache | None:
        ...
        derouter_settings: Final = config.get("derouter_settings") or {}
        ...
        _attach_redis_usage_cache(
            ...
            enable_redis_auth_cache=derouter_settings.get("enable_redis_auth_cache", False) is True,
        )
        verbose_proxy_logger.info(
    ...
    @staticmethod
    async def _init_coordination_redis_env_fallback(derouter_settings: Mapping[str, object]) -> RedisCache | None:
        """
        Last-resort coordination Redis, tried after an explicit
        `general_settings.coordination_redis` block and `derouter_settings.cache`
    ...
    def _init_cache(
        ...
        """
        from derouter import Cache

        if "default_in_memory_ttl" in cache_params:
            derouter.default_in_memory_ttl = cache_params["default_in_memory_ttl"]

        if "default_redis_ttl" in cache_params:
            derouter.default_redis_ttl = cache_params["default_redis_ttl"]

        derouter.cache = Cache(**cache_params)

        resolved_usage_cache = redis_usage_cache
        cache_backend: Final = derouter.cache.cache if derouter.cache is not None else None
        ...
        elif derouter_config_cache.redis_cache is None:
            verbose_proxy_logger.info("derouter_config_cache: no Redis configured; cluster-wide cache sharing disabled.")
    ...
    def switch_on_llm_response_caching(self):
        ...
        Router passes caching=self.cache_responses to derouter.completion()
        """
        global llm_router
        import derouter

        if llm_router is not None and derouter.cache is not None and llm_router.cache_responses is not True:
    ...
    async def get_config(self, config_file_path: str | None = None) -> dict:
        ...
        if os.environ.get("DEROUTER_CONFIG_BUCKET_NAME") is not None:
            bucket_name: Final = os.environ.get("DEROUTER_CONFIG_BUCKET_NAME")
            object_key: Final = os.environ.get("DEROUTER_CONFIG_BUCKET_OBJECT_KEY")
            bucket_type: Final = os.environ.get("DEROUTER_CONFIG_BUCKET_TYPE")
    ...
    def parse_search_tools(self, config: dict) -> list[SearchToolTypedDict] | None:
        ...
        print(  # noqa: T201
            "\033[32mDeRouter: Proxy initialized with Search Tools:\033[0m"
        )

        for search_tool in search_tools_raw:
            # Display loaded search tool
            search_tool_name = search_tool.get("search_tool_name", "")
            search_provider = search_tool.get("derouter_params", {}).get("search_provider", "")
            print(  # noqa: T201
                f"\033[32m    {search_tool_name} ({search_provider})\033[0m"
            )

            # Handle os.environ/ variables in derouter_params
            derouter_params = search_tool.get("derouter_params", {})
            if derouter_params:
                for k, v in derouter_params.items():
                    if isinstance(v, str) and v.startswith("os.environ/"):
                        ...
                        v = get_secret(_v)
                        derouter_params[k] = v
                search_tool["derouter_params"] = derouter_params
    ...
    def _load_environment_variables(self, config: dict):
        ...
        if environment_variables:
            ...
            # check if derouter_license in general_settings
            if "DEROUTER_LICENSE" in environment_variables:
                _license_check.license_str = os.getenv("DEROUTER_LICENSE", None)
                premium_user = _license_check.is_premium()

    def _warn_on_misplaced_jwt_keys(self, config: dict) -> tuple[str, ...]:
        misplaced_jwt_keys = tuple(key for key in ("enable_jwt_auth", "derouter_jwtauth") if key in config)
        ...
        verbose_proxy_logger.warning(
            ...
            "general_settings.derouter_jwtauth); placed at the top level they are silently "
            "dropped and JWT auth (and JWT-to-virtual-key mapping) will not engage.",
            ", ".join(misplaced_jwt_keys),
        )
        return misplaced_jwt_keys

    async def load_config(self, router: derouter.Router | None, config_file_path: str):
        """
        ...
        if callback_settings:
            derouter.callback_settings = callback_settings

        ## DEROUTER MODULE SETTINGS (e.g. derouter.drop_params=True,..)
        derouter_settings = config.get("derouter_settings", None)
        if derouter_settings is None:
            derouter_settings = {}
        if derouter_settings:
            ...
            from derouter.types.integrations.prometheus import validate_caller_identity_settings

            validate_caller_identity_settings(derouter_settings)

            # ANSI escape code for blue text
            blue_color_code: Final = "\033[94m"
            reset_color_code: Final = "\033[0m"
            for key, value in derouter_settings.items():
                if key == "cache" and value is True:
                    print(f"{blue_color_code}\nSetting Cache on Proxy")  # noqa: T201
                    from derouter.caching.caching import Cache

                    cache_params = {}
                    if "cache_params" in derouter_settings:
                        cache_params_in_config = derouter_settings["cache_params"]
                    ...
                    if derouter.cache is not None:
                        verbose_proxy_logger.debug("%sSet Cache on DeRouter Proxy%s", blue_color_code, reset_color_code)
                    ...
                    guardrail_name_config_map = initialize_guardrails(
                        ...
                        config_file_path=config_file_path,
                        derouter_settings=derouter_settings,
                    )

                    derouter.guardrail_name_config_map = guardrail_name_config_map

                elif key == "global_prompt_directory":
                    from derouter.integrations.dotprompt import (
                        set_global_prompt_directory,
                    )

                    set_global_prompt_directory(value)
                    verbose_proxy_logger.info(
                        "%sSet Global Prompt Directory on DeRouter Proxy%s", blue_color_code, reset_color_code
                    )
                elif key == "global_bitbucket_config":
                    from derouter.integrations.bitbucket import (
                        set_global_bitbucket_config,
                    )

                    set_global_bitbucket_config(value)
                    verbose_proxy_logger.info(
                        "%sSet Global BitBucket Config on DeRouter Proxy%s", blue_color_code, reset_color_code
                    )
                elif key == "global_gitlab_config":
                    from derouter.integrations.gitlab import set_global_gitlab_config

                    set_global_gitlab_config(value)
                    verbose_proxy_logger.info(
                        "%sSet Global Gitlab Config on DeRouter Proxy%s", blue_color_code, reset_color_code
                    )
                elif key == "priority_reservation_settings":
                    from derouter.types.utils import PriorityReservationSettings

                    derouter.priority_reservation_settings = PriorityReservationSettings(**value)
                elif key == "callbacks":
                    ...
                    from derouter.types.router import ModelGroupSettings

                    derouter.model_group_settings = ModelGroupSettings(**value)

                elif key == "post_call_rules":
                    derouter.post_call_rules = [get_instance_fn(value=value, config_file_path=config_file_path)]
                    verbose_proxy_logger.debug("derouter.post_call_rules: %s", derouter.post_call_rules)
                elif key == "max_budget":
                    derouter.max_budget = float(value)
                elif key == "max_internal_user_budget":
                    derouter.max_internal_user_budget = float(value)
                elif key == "default_max_internal_user_budget":
                    derouter.default_max_internal_user_budget = float(value)
                    if derouter.max_internal_user_budget is None:
                        derouter.max_internal_user_budget = derouter.default_max_internal_user_budget
                elif key == "default_internal_user_params" and isinstance(value, dict):
                    derouter.default_internal_user_params = (
                        {**value, "max_budget": float(value["max_budget"])}
                        if value.get("max_budget") is not None
                        else value
                    )
                    verbose_proxy_logger.debug(
                        "%s setting derouter.%s=%s%s",
                        blue_color_code,
                        key,
                        _redact_general_setting_value(key, derouter.default_internal_user_params, is_full_admin=False),
                        reset_color_code,
                    )
                elif key == "custom_provider_map":
                    from derouter.utils import custom_llm_setup

                    derouter.custom_provider_map = [
                        {
                    ...
                    for callback in value:
                        ...
                        if "." in callback:
                            derouter.logging_callback_manager.add_derouter_success_callback(
                                get_instance_fn(
                                    value=callback,
                                    config_file_path=config_file_path,
                                )
                            )
                        # these are derouter callbacks - "langfuse", "sentry", "wandb"
                        else:
                            derouter.logging_callback_manager.add_derouter_success_callback(callback)
                            if "prometheus" in callback:
                                from derouter.integrations.prometheus import (
                                    PrometheusLogger,
                                )
                    ...
                    print(  # noqa: T201
                        f"{blue_color_code} Initialized Success Callbacks - {derouter.success_callback} {reset_color_code}"
                    )
                    ...
                    for callback in value:
                        # user passed custom_callbacks.async_on_succes_logger. They need us to import a function
                        if "." in callback:
                            derouter.logging_callback_manager.add_derouter_failure_callback(
                                get_instance_fn(
                                    value=callback,
                                    config_file_path=config_file_path,
                                )
                    ...
                    print(  # noqa: T201
                        f"{blue_color_code} Initialized Failure Callbacks - {derouter.failure_callback} {reset_color_code}"
                    )
                    ...
                    reset_audit_log_callback_cache()
                    derouter.audit_log_callbacks = []

                    for callback in value:
                        if "." in callback:
                            derouter.audit_log_callbacks.append(
                                get_instance_fn(
                                    value=callback,
                                    config_file_path=config_file_path,
                                )
                            ...
                            derouter.audit_log_callbacks.append(callback)

                    _store_audit_logs = derouter_settings.get("store_audit_logs", derouter.store_audit_logs)
                    if is_audit_logging_enabled(store_audit_logs=_store_audit_logs):
                        print(  # noqa: T201
                            f"{blue_color_code} Initialized Audit Log Callbacks - {derouter.audit_log_callbacks} {reset_color_code}"
                        )
                    else:
                        verbose_proxy_logger.warning(
                    ...
                    # see usage here: https://router.delify.vn/docs/proxy/caching
                    pass
                elif key == "responses":
                    ...
                    derouter.max_ui_session_budget = float(value) if value is not None else None
                    verbose_proxy_logger.debug(
                        "%s setting derouter.max_ui_session_budget=%s%s",
                        blue_color_code,
                        derouter.max_ui_session_budget,
                        reset_color_code,
                    )
                    ...
                    if value is not None and isinstance(value, dict):
                        ...
                        derouter.upperbound_key_generate_params = DeRouter_UpperboundKeyGenerateParams(**value)
                    else:
                        raise Exception(f"Invalid value set for upperbound_key_generate_params - value={value}")
                elif key == "json_logs" and value is True:
                    derouter.json_logs = True
                    derouter._turn_on_json()
                    verbose_proxy_logger.debug(
                    ...
                    derouter.drop_params = drop_params_flag(value, "derouter_settings.drop_params", verbose_proxy_logger)
                else:
                    verbose_proxy_logger.debug(
                        "%s setting derouter.%s=%s%s",
                        blue_color_code,
                    ...
                    if key == "request_timeout":
                        derouter.request_timeout_explicitly_set = True
                    if key in {"s3_audit_callback_params", "s3_callback_params"}:
                        from derouter.integrations.s3_v2 import S3Logger as S3V2Logger
                        from derouter.derouter_core_utils.derouter_logging import (
                            _in_memory_loggers,
                        )
                        from derouter.proxy.management_helpers.audit_logs import (
        ...
        ## GENERAL SERVER SETTINGS (e.g. master key,..) # do this after initializing derouter, to ensure sentry logging works for proxylogging
        general_settings = config.get("general_settings", {})
        ...
        if general_settings:
            ...
            if key_management_settings is not None:
                derouter._key_management_settings = KeyManagementSettings(**key_management_settings)
            ...
            ### MASTER KEY ###
            master_key = general_settings.get("master_key", get_secret("DEROUTER_MASTER_KEY", None))

            if master_key and master_key.startswith("os.environ/"):
                master_key = get_secret(master_key)

            if master_key is not None and isinstance(master_key, str):
                derouter_master_key_hash = hash_token(master_key)
            else:
                verbose_proxy_logger.critical(
                    "DEROUTER_MASTER_KEY is not set! All requests will be treated as INTERNAL_USER with no admin access. Set DEROUTER_MASTER_KEY for production use."
                )
            ...
            if user_api_key_cache_ttl is not None:
                ...
                # Mirror TTL on Redis as well when ``derouter_settings.enable_redis_auth_cache``
            ...
            if use_pkce and redis_usage_cache is None:
                ...
                if not _pkce_no_redis_warning_emitted:
                    ...
                    verbose_proxy_logger.warning(
                        "GENERIC_CLIENT_USE_PKCE=true but Redis is not configured for DeRouter caching. "
            ...
            if cp_url and redis_usage_cache is None:
                ...
                if not _cp_no_redis_warning_emitted:
                    ...
                    verbose_proxy_logger.warning(
                        "control_plane_url is configured but Redis is not configured for DeRouter caching. "
            ...
            if allowed_ips is not None and premium_user is False:
                raise ValueError(
                    "allowed_ips is an Enterprise Feature. Please add a valid DEROUTER_LICENSE to your envionment."
                )
            ## BUDGET RESCHEDULER ##
            ...
            if _use_legacy_interactions_schema is not None:
                if isinstance(_use_legacy_interactions_schema, str):
                    derouter.use_legacy_interactions_schema = _use_legacy_interactions_schema.lower() == "true"
                else:
                    derouter.use_legacy_interactions_schema = bool(_use_legacy_interactions_schema)
            ...
            # check if derouter_license in general_settings
            if "derouter_license" in general_settings:
                _license_check.license_str = general_settings["derouter_license"]
                premium_user = _license_check.is_premium()

        router_params: Final[dict] = {
            "cache_responses": derouter.cache is not None,  # cache if user passed in cache values
        }
        ...
        if model_list:
            ...
            print(  # noqa: T201
                "\033[32mDeRouter: Proxy initialized with Config, Set models:\033[0m"
            )
            for model in model_list:
                ### LOAD FROM os.environ/ ###
                for k, v in model["derouter_params"].items():
                    if isinstance(v, str) and v.startswith("os.environ/"):
                        model["derouter_params"][k] = get_secret(v)
                ...
                complexity_router_config = model["derouter_params"].get("complexity_router_config")
                ...
                derouter_model_name = model["derouter_params"]["model"]
                derouter_model_api_base = model["derouter_params"].get("api_base", None)
                if "ollama" in derouter_model_name and derouter_model_api_base is None:
                    run_ollama_serve()
        ...
        if assistant_settings:
            for k, v in assistant_settings["derouter_params"].items():
                if isinstance(v, str) and v.startswith("os.environ/"):
                    _v = v.replace("os.environ/", "")
                    v = os.getenv(_v)
                    assistant_settings["derouter_params"][k] = v
        ...
        from derouter.sandbox.sandbox_tools import register_sandbox_tools
        ...
        if router_settings and isinstance(router_settings, dict):
            available_args: Final = [
                x for x in derouter.Router.get_valid_args() if x not in ROUTER_SETTINGS_MANAGED_OUTSIDE_CONFIG
            ]
        ...
        if prompts:
            from derouter.proxy.prompts.init_prompts import init_prompts
        ...
        derouter.credential_list = credential_list_dict
    ...
    async def _init_non_llm_configs(self, config: dict, config_file_path: str | None = None):
        ...
        if mcp_tools_config:
            from derouter.proxy._experimental.mcp_server.tool_registry import (
        ...
        if mcp_servers_config:
            ...
            # Get mcp_aliases from derouter_settings if available
            derouter_settings: Final = config.get("derouter_settings", {})
            mcp_aliases: Final = derouter_settings.get("mcp_aliases", None)
        ...
        if vector_store_registry_config:
            ...
            if derouter.vector_store_registry is None:
                derouter.vector_store_registry = VectorStoreRegistry()

            # Load vector stores from config
            derouter.vector_store_registry.load_vector_stores_from_config(vector_store_registry_config)
    ...
    async def _init_policy_engine(
        ...
        from derouter.proxy.policy_engine.init_policies import init_policies
        from derouter.proxy.policy_engine.policy_validator import PolicyValidator
    ...
    def _load_alerting_settings(self, general_settings: dict):
        ...
        for _alert in _alerting_callbacks:
            if _alert == "slack":
                ...
                if _alert in derouter._known_custom_logger_compatible_callbacks:
                    ...
                    if _logger is not None:
                        derouter.logging_callback_manager.add_derouter_callback(_logger)
    ...
    def initialize_secret_manager(
        ...
        if key_management_system is not None:
            if key_management_system == KeyManagementSystem.AZURE_KEY_VAULT.value:
                ...
                from derouter.secret_managers.custom_secret_manager_loader import (
    ...
    def get_model_info_with_id(self, model, db_model=False) -> RouterModelInfo:
        ...
        if premium_user is True:
            # seeing "created_at", "updated_at", "created_by", "updated_by" is a DeRouter Enterprise Feature
            ...
            for model in model_list:
                ...
                if model_id is None:
                    ...
                    model_id = llm_router.generate_model_id(
                        ...
                        derouter_params=model["derouter_params"],
                    )
                else:
    ...
    def _resolve_db_derouter_param(self, key: str, value: object) -> object:
    ...
    def _add_deployment(self, db_models: list) -> int:
        ...
        ## ADD MODEL LOGIC
        for m in db_models:
            _derouter_params = m.derouter_params
            if isinstance(_derouter_params, dict):
                # decrypt values
                for k, v in _derouter_params.items():
                    _derouter_params[k] = self._resolve_db_derouter_param(key=k, value=v)
                _derouter_params = DeRouter_Params.model_validate(_derouter_params)

            else:
                verbose_proxy_logger.error(
                    "Invalid model added to proxy db. Invalid derouter params. derouter_params=%s", _derouter_params
                )
            ...
            added = llm_router.upsert_deployment(
                deployment=Deployment(
                    ...
                    derouter_params=_derouter_params,
    ...
    def decrypt_model_list_from_db(self, new_models: list) -> list:
        ...
        for m in new_models:
            _derouter_params = m.derouter_params
            if isinstance(_derouter_params, BaseModel):
                _derouter_params = _derouter_params.model_dump()
            if isinstance(_derouter_params, dict):
                # decrypt values
                for k, v in _derouter_params.items():
                    _derouter_params[k] = self._resolve_db_derouter_param(key=k, value=v)
    ...
    def _add_callback_from_db_to_in_memory_derouter_callbacks(
        self,
        callback: str,
        ...
        """
        Helper method to add a single callback to derouter for specified event types.
        ...
        """
        if callback in derouter._known_custom_logger_compatible_callbacks:
    ...
    def _add_callbacks_from_db_config(self, config_data: dict) -> None:
        """
        Adds callbacks from DB config to derouter
        """
        derouter_settings: Final = config_data.get("derouter_settings", {}) or {}
        success_callbacks: Final = derouter_settings.get("success_callback", None)
        failure_callbacks: Final = derouter_settings.get("failure_callback", None)
        callbacks: Final = derouter_settings.get("callbacks", None)

        if success_callbacks is not None and isinstance(success_callbacks, list):
            for success_callback in success_callbacks:
                self._add_callback_from_db_to_in_memory_derouter_callbacks(
                    callback=success_callback,
                    event_types=["success"],
                    existing_callbacks=derouter.success_callback,
                )
        ...
        if failure_callbacks is not None and isinstance(failure_callbacks, list):
            for failure_callback in failure_callbacks:
                self._add_callback_from_db_to_in_memory_derouter_callbacks(
                    ...
                    event_types=["failure"],
                    existing_callbacks=derouter.failure_callback,
                )
        ...
        if callbacks is not None and isinstance(callbacks, list):
            for callback in callbacks:
                self._add_callback_from_db_to_in_memory_derouter_callbacks(
                    ...
                    existing_callbacks=derouter.callbacks,
                )
    ...
    async def _add_router_settings_from_db_config(
        ...
        """
        Adds router settings from DB config to derouter proxy
    ...
    def _add_general_settings_from_db_config(
        ...
        """
        Adds general settings from DB config to derouter proxy
    ...
    def _update_config_fields(
        ...
        elif param_name == "derouter_settings" and isinstance(db_param_value, dict):
            for key, value in db_param_value.items():
                if key in DEROUTER_SETTINGS_SAFE_DB_OVERRIDES:  # params that are safe to override with db values
                    setattr(derouter, key, value)
    ...
    async def _get_models_from_db(self, prisma_client: PrismaClient) -> Sequence[_ProxyModelRow] | None:
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "derouter.proxy_server.py::add_deployment() - Error getting new models from DB - %s", e
            )
            return None
    ...
    async def _add_deployment_locked(
        ...
        try:
            ...
            verbose_proxy_logger.exception("derouter.proxy.proxy_server.py::ProxyConfig:add_deployment - %s", e)
    ...
    async def _init_non_llm_objects_in_db(self, prisma_client: PrismaClient):
        ...
        await self._apply_safe_derouter_settings_overrides_from_db(prisma_client=prisma_client)

    async def _apply_safe_derouter_settings_overrides_from_db(self, prisma_client: PrismaClient) -> None:
        config_record: Final = await get_config_param(prisma_client, "derouter_settings")
        if config_record is None or config_record.param_value is None:
            return
        raw_settings: Final = config_record.param_value
        derouter_settings: Final = json.loads(raw_settings) if isinstance(raw_settings, str) else raw_settings
        if not isinstance(derouter_settings, dict):
            return
        for key, value in derouter_settings.items():
            if key in DEROUTER_SETTINGS_SAFE_DB_OVERRIDES:
                setattr(derouter, key, value)
    ...
    async def _init_semantic_filter_settings_in_db(self, prisma_client: PrismaClient):
        ...
        import derouter
        from derouter.proxy.hooks.mcp_semantic_filter import SemanticToolFilterHook
        ...
        try:
            ...
            derouter_settings = config_record.param_value
            if isinstance(derouter_settings, str):
                derouter_settings = json.loads(derouter_settings)

            mcp_semantic_filter_config: Final = derouter_settings.get("mcp_semantic_tool_filter", None)
            ...
            if hasattr(self, "_last_semantic_filter_config"):
                if self._last_semantic_filter_config == mcp_semantic_filter_config:
                    ...
                    active_hooks = derouter.logging_callback_manager.get_custom_loggers_for_type(SemanticToolFilterHook)
                    if active_hooks:
            ...
            derouter.logging_callback_manager.remove_callbacks_by_type(derouter.callbacks, SemanticToolFilterHook)
            ...
            if mcp_semantic_filter_config.get("enabled", False):
                ...
                if hook:
                    derouter.logging_callback_manager.add_derouter_callback(hook)
    ...
    async def _init_sso_settings_in_db(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "derouter.proxy.proxy_server.py::ProxyConfig:_init_sso_settings_in_db - %s", e
            )
    ...
    async def _init_hashicorp_vault_config_override(self, prisma_client: PrismaClient):
        ...
        """
        from derouter.proxy.management_endpoints.config_override_endpoints import (
    ...
    async def _check_and_reload_model_cost_map(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            reload_result = await refetch_model_cost_map(url=derouter.model_cost_map_url)
    ...
    async def _check_and_reload_anthropic_beta_headers(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            if should_reload:
                # Perform the reload
                from derouter.anthropic_beta_headers_manager import (
    ...
    def _get_prompt_spec_for_db_prompt(self, db_prompt):
        ...
        """
        from derouter.proxy.prompts.prompt_endpoints import create_versioned_prompt_spec

        return create_versioned_prompt_spec(db_prompt=db_prompt)

    async def _init_prompts_in_db(self, prisma_client: PrismaClient):
        from derouter.proxy.prompts.prompt_registry import IN_MEMORY_PROMPT_REGISTRY, registry_key_for_prompt
        from derouter.types.prompts.init_prompts import PromptSpec
        ...
        def parse_row(db_prompt: object) -> PromptSpec | None:
            try:
                ...
                verbose_proxy_logger.exception(
                    "derouter.proxy.proxy_server.py::ProxyConfig:_init_prompts_in_db - failed to parse prompt row %s: %s",
        ...
        try:
            ...
            for prompt_spec in parsed_specs:
                try:
                    ...
                    verbose_proxy_logger.exception(
                        "derouter.proxy.proxy_server.py::ProxyConfig:_init_prompts_in_db - failed to sync prompt %s: %s",
            ...
            verbose_proxy_logger.debug("derouter.proxy.proxy_server.py::ProxyConfig:_init_prompts_in_db - %s", e)

    async def _init_guardrails_in_db(self, prisma_client: PrismaClient):
        from derouter.proxy.guardrails.guardrail_registry import (
        ...
        try:
            async with GUARDRAIL_RECONCILE_LOCK:
                ...
                for guardrail in guardrails_in_db:
                    ...
                    try:
                        ...
                        verbose_proxy_logger.error(
                            "derouter.proxy.proxy_server.py::ProxyConfig:_init_guardrails_in_db - "
            ...
            verbose_proxy_logger.exception("derouter.proxy.proxy_server.py::ProxyConfig:_init_guardrails_in_db - %s", e)
    ...
    async def _init_policies_in_db(self, prisma_client: PrismaClient):
        ...
        """
        from derouter.proxy.policy_engine.attachment_registry import (
            get_attachment_registry,
        )
        from derouter.proxy.policy_engine.policy_registry import get_policy_registry
        ...
        try:
            ...
            verbose_proxy_logger.exception("derouter.proxy.proxy_server.py::ProxyConfig:_init_policies_in_db - %s", e)
    ...
    async def _init_tool_policy_in_db(self, prisma_client: PrismaClient):
        ...
        """
        from derouter.proxy.db.tool_registry_writer import get_tool_policy_registry
        ...
        try:
            ...
            verbose_proxy_logger.exception("derouter.proxy.proxy_server.py::ProxyConfig:_init_tool_policy_in_db - %s", e)
    ...
    async def _init_vector_stores_in_db(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            if derouter.vector_store_registry is None:
                derouter.vector_store_registry = VectorStoreRegistry(vector_stores=vector_stores)
            else:
                for vector_store in vector_stores:
                    derouter.vector_store_registry.add_vector_store_to_registry(vector_store=vector_store)
        except Exception as e:
            verbose_proxy_logger.exception(
                "derouter.proxy.proxy_server.py::ProxyConfig:_init_vector_stores_in_db - %s", e
            )

    async def _init_vector_store_indexes_in_db(self, prisma_client: PrismaClient):
        from derouter.vector_stores.vector_store_registry import VectorStoreIndexRegistry
        ...
        try:
            ...
            if derouter.vector_store_index_registry is None:
                derouter.vector_store_index_registry = VectorStoreIndexRegistry(
                    vector_store_indexes=vector_store_indexes
                )
            else:
                for vector_store_index in vector_store_indexes:
                    derouter.vector_store_index_registry.upsert_vector_store_index(vector_store_index=vector_store_index)
        except Exception as e:
            verbose_proxy_logger.exception(
                "derouter.proxy.proxy_server.py::ProxyConfig:_init_vector_stores_in_db - %s", e
            )
    ...
    async def _init_mcp_servers_in_db(self):
        from derouter.proxy._experimental.mcp_server.utils import is_mcp_available
        ...
        from derouter.proxy._experimental.mcp_server.oauth2_flow_backfill import (
            backfill_null_oauth2_flows,
        )
        from derouter.proxy._experimental.mcp_server.oauth_issuer_stamp_backfill import (
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "derouter.proxy.proxy_server.py::ProxyConfig:_init_mcp_servers_in_db backfill - %s", e
            )
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "derouter.proxy.proxy_server.py::ProxyConfig:_init_mcp_servers_in_db issuer stamp backfill - %s", e
            )

        try:
            await global_mcp_server_manager.reload_servers_from_database()
        except Exception as e:
            verbose_proxy_logger.exception("derouter.proxy.proxy_server.py::ProxyConfig:_init_mcp_servers_in_db - %s", e)
    ...
    async def reload_mcp_servers_from_db(self) -> None:
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "derouter.proxy.proxy_server.py::ProxyConfig:reload_mcp_servers_from_db - %s", e
            )
    ...
    async def _init_agents_in_db(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            verbose_proxy_logger.exception("derouter.proxy.proxy_server.py::ProxyConfig:_init_agents_in_db - %s", e)
    ...
    async def _init_search_tools_in_db(self, prisma_client: PrismaClient):
        ...
        from derouter.proxy.search_endpoints.search_tool_registry import (
            SearchToolRegistry,
        )
        from derouter.router_utils.search_api_router import SearchAPIRouter
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "derouter.proxy.proxy_server.py::ProxyConfig:_init_search_tools_in_db - %s", e
            )
    ...
    async def delete_credentials(self, db_credentials: list[CredentialItem]):
        ...
        Compare to the derouter.credential_list
        Delete any from derouter.credential_list that are not in the all-up list
        """
        ## CONFIG credentials ##
        config: Final = await self.get_config(config_file_path=user_config_file_path)
    ...
    async def get_credentials(self, prisma_client: PrismaClient):
        try:
            ...
            verbose_proxy_logger.exception(
                "derouter.proxy_server.py::get_credentials() - Error getting credentials from DB - %s", e
            )
            return []
...
async def initialize(
    ...
    global \
        ...
        user_custom_auth, \
        prisma_client
    from derouter.proxy.common_utils.banner import show_banner

    show_banner()
    if os.getenv("DEROUTER_DONT_SHOW_FEEDBACK_BOX", "").lower() != "true":
    ...
    if detailed_debug is True:
        ...
        # users can control proxy debugging using env variable = 'DEROUTER_LOG'
        derouter_log_setting: Final = os.environ.get("DEROUTER_LOG", "")
        if derouter_log_setting is not None:
            if derouter_log_setting.upper() == "INFO":
                import logging
                ...
                verbose_router_logger.setLevel(level=logging.INFO)  # set router logs to info
                verbose_proxy_logger.setLevel(level=logging.INFO)  # set proxy logs to info
            elif derouter_log_setting.upper() == "DEBUG":
                import logging
    ...
    if api_version:
        os.environ["AZURE_API_VERSION"] = api_version  # set this for azure - derouter can read this from the env
    ...
    if drop_params is True:  # derouter-specific param
        derouter.drop_params = True
        dynamic_config["general"]["drop_params"] = True
    if add_function_to_prompt is True:  # derouter-specific param
        derouter.add_function_to_prompt = True
        dynamic_config["general"]["add_function_to_prompt"] = True
    if max_budget:  # derouter-specific param
        derouter.max_budget = float(max_budget)
        dynamic_config["general"]["max_budget"] = derouter.max_budget
    if experimental:
        pass
...
async def async_assistants_data_generator(response, user_api_key_dict: UserAPIKeyAuth, request_data: dict):
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "derouter.proxy.proxy_server.async_assistants_data_generator(): Exception occured - %s", e
        )
        ...
        verbose_proxy_logger.debug(
            "\x1b[1;31mAn error occurred: %s\n\n Debug this by setting `--debug`, e.g. `derouter --model gpt-3.5-turbo --debug`",
            e,
        )
...
def _get_client_requested_model_for_streaming(request_data: dict) -> str:
    ...
    """
    requested_model = request_data.get("_derouter_client_requested_model")
...
def _get_streaming_fallback_metadata(
    ...
    if not _is_positive_int_like(additional_headers.get("x-derouter-attempted-fallbacks")):
        return False, None, []

    fallback_model: Final = additional_headers.get("x-derouter-model-group")
...
def _restamp_streaming_chunk_model(
    *,
    ...
    if not model_mismatch_logged and downstream_model != target_model:
        verbose_proxy_logger.debug(
            "derouter_call_id=%s: streaming chunk model mismatch - target=%r downstream=%r fallback_was_attempted=%s. Overriding chunk model to target.",
            request_data.get("derouter_call_id"),
            target_model,
            downstream_model,
    ...
    try:
        ...
        verbose_proxy_logger.error(
            "derouter_call_id=%s: failed to override chunk.model=%r on chunk_type=%s. error=%s",
            request_data.get("derouter_call_id"),
            target_model,
            type(chunk),
            str(e),
...
async def _apply_streaming_chunk_hooks(
    *,
    ...
    if isinstance(chunk, (ModelResponse, ModelResponseStream)):
        response_str: Final = derouter.get_response_string(response_obj=chunk)
...
def _keepalive_from_deployment_config(
    ...
    if isinstance(model_id, str) and model_id:
        ...
        return _DeploymentKeepaliveConfig(
            keepalive_seconds=getattr(deployment.derouter_params, "keepalive_seconds", None),
            allow_client_override=bool(getattr(deployment.derouter_params, "allow_client_keepalive_override", False)),
        )
    ...
    configs: Final = frozenset(
        (
            (deployment_dict.get("derouter_params") or _EMPTY_MAPPING).get("keepalive_seconds"),
            bool(
                (deployment_dict.get("derouter_params") or _EMPTY_MAPPING).get("allow_client_keepalive_override", False)
            ),
        )
...
def _resolve_keepalive_seconds(request_data: Mapping[str, object], response: object = None) -> float:
    ...
    # global `derouter_settings.sse_keepalive_ping_interval_seconds` applies; a
    ...
    raw: Final = (
        ...
        else derouter.sse_keepalive_ping_interval_seconds
    )
    try:
...
async def async_data_generator(
    ...
    try:
        ...
        is_raw_sse_stream: Final = bool(request_data.get("_derouter_raw_sse_stream"))
        strip_stream_usage: Final = bool(request_data.get("_derouter_strip_stream_usage"))
        raw_sse_buffer = ""
        ...
        if not request_data.get("_derouter_skip_openai_stream_done"):
            done_message: Final = "[DONE]"
            yield f"data: {done_message}\n\n"
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.async_data_generator(): Exception occured - %s", e)
        await proxy_logging_obj.post_call_failure_hook(
            user_api_key_dict=user_api_key_dict,
            original_exception=e,
            request_data=request_data,
...
def get_derouter_model_info(model: dict = {}):
    model_info: Final = model.get("model_info", {})
    model_to_lookup = model.get("derouter_params", {}).get("model", None)
    try:
        if "azure" in model_to_lookup or model_info.get("base_model"):
            model_to_lookup = model_info.get("base_model", None)
        derouter_model_info: Final = derouter.get_model_info(model_to_lookup)
        return derouter_model_info
    except Exception:
        # this should not block returning on /model/info
        # if derouter does not have info on the model it should return {}
        return {}
...
class ProxyStartupEvent:
    ...
    def _warn_budget_without_db(max_budget: float | None, prisma_client: PrismaClient | None) -> None:
        ...
        verbose_proxy_logger.warning(
            "A proxy-wide budget (derouter.max_budget=%s) is configured but no database is connected, "
    ...
    def _warn_if_mock_testing_params_enabled(general_settings: dict) -> None:
        ...
        from derouter.proxy.route_llm_request import (
    ...
    def _validate_redis_transaction_buffer_config(
        ...
        a Redis cache is properly configured in derouter_settings.
        """
        ...
        if _use_redis_transaction_buffer and redis_usage_cache is None:
            raise ValueError(
                ...
                "to not be tracked. Add a Redis cache in derouter_settings:\n\n"
                "derouter_settings:\n"
                "  cache: true\n"
                "  cache_params:\n"
                "    type: redis\n"
    ...
    async def _init_coordination_redis_from_db(
        derouter_settings: Mapping[str, object],
        llm_router: Router | None,
    ) -> RedisCache | None:
        """
    ...
    async def _initialize_semantic_tool_filter(
        cls,
        llm_router: Router | None,
        derouter_settings: dict[str, Any],
    ):
    ...
    def _initialize_jwt_auth(
        ...
        if general_settings.get("derouter_jwtauth", None) is not None:
            for k, v in general_settings["derouter_jwtauth"].items():
                if isinstance(v, str) and v.startswith("os.environ/"):
                    general_settings["derouter_jwtauth"][k] = get_secret(v)
            ...
            derouter_jwtauth = DeRouter_JWTAuth(
                config_file_path=user_config_file_path,
                **general_settings["derouter_jwtauth"],
            )
        else:
            derouter_jwtauth = DeRouter_JWTAuth()
    ...
    def _add_proxy_budget_to_db(cls):
        ...
        if derouter.budget_duration is None:
    ...
    async def _upsert_proxy_budget_with_reset_at_backfill(cls) -> None:
        ...
        await generate_key_helper_fn(
            request_type="user",
            table_name="user",
            user_id=DEROUTER_PROXY_BUDGET_NAME,
            duration=None,
            models=[],
            aliases={},
            config={},
            spend=0,
            max_budget=derouter.max_budget,
            budget_duration=derouter.budget_duration,
        ...
        # took the UPDATE path, and reset_budget_for_derouter_users never
        # matches them (NULL < now() is unknown in SQL) — so the proxy-wide
        # spend cap blocks forever once it's hit.
        if prisma_client is not None and derouter.budget_duration is not None:
            try:
                await UserRepository(prisma_client).table.update_many(
                    ...
                    data={
                        "budget_reset_at": get_budget_reset_time(budget_duration=derouter.budget_duration),
                        "spend": 0,
                    },
                )
            except Exception as e:
    ...
    async def _update_default_team_member_budget(cls):
        ...
        if derouter.default_internal_user_params is None:
            return

        _teams: Final = derouter.default_internal_user_params.get("teams") or []
    ...
    async def _load_heuristic_v1_tuning_baselines(
        ...
        try:
            config_table: Final = prisma_client.db.derouter_config
    ...
    async def initialize_scheduled_background_jobs(
        ...
        from derouter.proxy.utils import update_daily_tag_spend
        ...
        if general_settings.get("disable_spend_logs", False) is False:
            from derouter.proxy.utils import _monitor_spend_logs_queue
        ...
        ### PTU DAILY ROLLUP ###
        from derouter.proxy.spend_tracking.ptu_feature_flag import (
            is_ptu_cost_attribution_enabled,
        )

        if is_ptu_cost_attribution_enabled():
            from derouter.proxy.spend_tracking.ptu_flat_cost_rollup import (
                PTU_ROLLUP_JOB_ID,
        ...
        if llm_router is not None and PROXY_BATCH_POLLING_ENABLED:
            try:
                from derouter_enterprise.proxy.common_utils.check_batch_cost import (
                    CheckBatchCost,
                )
                ...
                verbose_proxy_logger.debug(
                    "Checking batch cost for DeRouter Managed Files is an Enterprise Feature. Skipping..."
                )

        ### CHECK RESPONSES COST ###
        if llm_router is not None and PROXY_BATCH_POLLING_ENABLED:
            try:
                from derouter_enterprise.proxy.common_utils.check_responses_cost import (
                    CheckResponsesCost,
                )
                ...
                verbose_proxy_logger.debug(
                    "Checking responses cost for DeRouter Managed Files is an Enterprise Feature. Skipping..."
                )
    ...
    async def _initialize_spend_tracking_background_jobs(cls, scheduler: AsyncIOScheduler):
        ...
        from derouter.proxy.spend_tracking.cloudzero_endpoints import is_cloudzero_setup
        ...
        from derouter.proxy.spend_tracking.vantage_endpoints import (
            ...
            if not is_vantage_setup_in_config() and await is_vantage_setup_in_db():
                try:
                    ...
                    if db_settings:
                        ...
                        derouter.logging_callback_manager.add_derouter_callback(vantage_logger)
                except Exception as e:
        ...
        ########################################################
        from derouter.integrations.mavvrik_focus.mavvrik_focus_logger import (  # noqa: PLC0415
            MavvrikFocusLogger,
        )
        ...
        ########################################################
        if derouter.prometheus_initialize_budget_metrics is True:
        ...
        ########################################################
        from derouter.constants import (
            DEROUTER_KEY_ROTATION_CHECK_INTERVAL_SECONDS,
            DEROUTER_KEY_ROTATION_ENABLED,
        )

        key_rotation_enabled: Final[bool | None] = str_to_bool(DEROUTER_KEY_ROTATION_ENABLED)
        verbose_proxy_logger.debug("key_rotation_enabled: %s", key_rotation_enabled)

        if key_rotation_enabled is True:
            try:
                from derouter.proxy.common_utils.key_rotation_manager import (
                    KeyRotationManager,
                )
                ...
                if prisma_client is not None:
                    ...
                    verbose_proxy_logger.debug(
                        "Key rotation background job scheduled every %s seconds (DEROUTER_KEY_ROTATION_ENABLED=true)",
                        DEROUTER_KEY_ROTATION_CHECK_INTERVAL_SECONDS,
                    )
                    scheduler.add_job(
                        key_rotation_manager.process_rotations,
                        "interval",
                        seconds=DEROUTER_KEY_ROTATION_CHECK_INTERVAL_SECONDS,
                        id="key_rotation_job",
                    )
                else:
            ...
            verbose_proxy_logger.debug("Key rotation disabled (set DEROUTER_KEY_ROTATION_ENABLED=true to enable)")
    ...
    async def _initialize_expired_ui_session_key_cleanup_background_job(cls, scheduler: AsyncIOScheduler):
        ...
        from derouter.constants import (
            ...
            DEROUTER_EXPIRED_UI_SESSION_KEY_CLEANUP_ENABLED,
            DEROUTER_EXPIRED_UI_SESSION_KEY_CLEANUP_INTERVAL_SECONDS,
        )
        ...
        if expired_ui_session_key_cleanup_enabled is True:
            try:
                from derouter.proxy.common_utils.expired_ui_session_key_cleanup_manager import (
                ...
                if prisma_client is not None:
                    ...
                    verbose_proxy_logger.debug(
                        "Expired UI session key cleanup background job scheduled every %s seconds (DEROUTER_EXPIRED_UI_SESSION_KEY_CLEANUP_ENABLED=true)",
                        DEROUTER_EXPIRED_UI_SESSION_KEY_CLEANUP_INTERVAL_SECONDS,
                    )
                    scheduler.add_job(
                        expired_ui_session_key_cleanup_manager.cleanup_expired_keys,
                        "interval",
                        seconds=DEROUTER_EXPIRED_UI_SESSION_KEY_CLEANUP_INTERVAL_SECONDS,
            ...
            verbose_proxy_logger.debug(
                ...
                "DEROUTER_EXPIRED_UI_SESSION_KEY_CLEANUP_ENABLED=true to enable)"
            )
    ...
    def _init_dd_tracer(cls):
        ...
        """
        from derouter.derouter_core_utils.dd_tracing import (
    ...
    def _init_pyroscope(cls):
        ...
        Off by default. Enable with DEROUTER_ENABLE_PYROSCOPE=true.
        ...
        """
        if not get_secret_bool("DEROUTER_ENABLE_PYROSCOPE", False):
            verbose_proxy_logger.debug(
                "DeRouter: Pyroscope profiling is disabled (set DEROUTER_ENABLE_PYROSCOPE=true to enable)."
            )
            return
        try:
            import pyroscope
            ...
            msg = (
                f"DeRouter: Pyroscope profiling started (app_name={app_name}, server_address={server_address}). "
            ...
            verbose_proxy_logger.warning(
                "DeRouter: DEROUTER_ENABLE_PYROSCOPE is set but the 'pyroscope-io' package is not installed. "
...
async def model_list(
    ...
    from derouter.llms.anthropic.common_utils import (
    ...
    from derouter.types.proxy.model_listing import ModelInfoResponse
    ...
    if should_expand_scope:
        ...
        from derouter.proxy.auth.model_checks import get_complete_model_list
...
async def model_info(
    ...
    # Use the actual derouter model from the deployment to get provider info
    _, provider, _, _ = derouter.get_llm_provider(model=deployment.derouter_params.model)
...
def _blocked_response_usage(original_response: object | None) -> "derouter.Usage":
    """
    ...
    if isinstance(usage, derouter.Usage):
        return usage
    return derouter.Usage(prompt_tokens=0, completion_tokens=0, total_tokens=0)
...
async def chat_completion(
    ...
    try:
        ...
        _logging_obj: Final = _data.get("derouter_logging_obj")
        await proxy_logging_obj.post_call_failure_hook(
            user_api_key_dict=user_api_key_dict,
            original_exception=e,
            request_data=_data,
        )
        _chat_response = derouter.ModelResponse()
        ...
        if data.get("stream", None) is not None and data["stream"] is True:
            _iterator = derouter.utils.ModelResponseIterator(model_response=_chat_response, convert_to_delta=True)
            _streaming_response = derouter.CustomStreamWrapper(
        ...
        if data.get("stream", None) is not None and data["stream"] is True:
            ...
            _streaming_response = derouter.CustomStreamWrapper(
                ...
                custom_llm_provider="cached_response",
                logging_obj=_data.get("derouter_logging_obj", None),
            )
            selected_data_generator = select_data_generator(
                response=_streaming_response,
                user_api_key_dict=user_api_key_dict,
        ...
        _usage: Final = derouter.Usage(prompt_tokens=0, completion_tokens=0, total_tokens=0)
...
async def completion(
    ...
    try:
        ...
        if _data.get("stream", None) is not None and _data["stream"] is True:
            _text_response: Final = derouter.ModelResponse()
            ...
            _iterator = derouter.utils.ModelResponseIterator(model_response=_text_response, convert_to_delta=True)
            _streaming_response = derouter.TextCompletionStreamWrapper(
                completion_stream=_iterator,
                model=e.model,
            )
            ...
            _response = derouter.TextCompletionResponse()
        ...
        if _data.get("stream", None) is not None and _data["stream"] is True:
            _chat_response: Final = derouter.ModelResponse()
            _usage = derouter.Usage(
                prompt_tokens=0,
                completion_tokens=0,
                total_tokens=0,
            )
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.completion(): Exception occured - %s", e)
        error_msg: Final = f"{e}"
        ...
        if (
            ...
            if llm_router is not None and data.get("model") in router_model_names:
                ...
                if deployment is not None:
                    derouter_params: Final = deployment.get("derouter_params", {}) or {}
                    derouter_model: Final = derouter_params.get("model", "")
                    # Check if this provider supports token arrays
                    supports_token_arrays: Final = derouter_model in derouter.open_ai_embedding_models or any(
                        derouter_model.startswith(provider)
                        for provider in DEROUTER_EMBEDDING_PROVIDERS_SUPPORTING_INPUT_ARRAY_OF_TOKENS
                    )
                    if not supports_token_arrays:
                        # non-openai/azure embedding model called with token input - decode tokens
                        input_list: Final = []
                        for i in data["input"]:
                            input_list.append(derouter.decode(model="gpt-3.5-turbo", tokens=i))
...
async def moderations(
    ...
    try:
        ...
        data = await add_derouter_data_to_request(
            data=data,
            request=request,
            general_settings=general_settings,
            user_api_key_dict=user_api_key_dict,
        ...
        ### ALERTING ###
        asyncio.create_task(
            proxy_logging_obj.update_request_status(derouter_call_id=data.get("derouter_call_id", ""), status="success")
        )
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.moderations(): Exception occured - %s", e)
...
async def audio_speech(
    ...
    try:
        ...
        derouter_call_id: Final = hidden_params.get("derouter_call_id", None) or ""
        ...
        verbose_proxy_logger.error("derouter.proxy.proxy_server.audio_speech(): Exception occured - %s", e)
...
async def audio_transcriptions(
    ...
    try:
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.audio_transcription(): Exception occured - %s", e)
        if isinstance(e, HTTPException):
            raise ProxyException(
...
async def realtime_websocket_endpoint(
    ...
    try:
        ...
        from derouter.derouter_core_utils.realtime_streaming import (
            REALTIME_SESSION_SUCCESS_LOGGED_KEY,
        )

        if not derouter_logging_obj.model_call_details.get(REALTIME_SESSION_SUCCESS_LOGGED_KEY):
            await _release_realtime_budget_reservation(user_api_key_dict)
...
async def get_assistants(
    ...
    try:
        ...
        # for now use custom_llm_provider=="openai" -> this will change as DeRouter adds more providers for acreate_batch
        if llm_router is None:
        ...
        verbose_proxy_logger.error("derouter.proxy.proxy_server.get_assistants(): Exception occured - %s", e)
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "message", str(e.detail)),
...
async def create_assistant(
    ...
    try:
        ...
        verbose_proxy_logger.error("derouter.proxy.proxy_server.create_assistant(): Exception occured - %s", e)
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "message", str(e.detail)),
...
async def delete_assistant(
    ...
    try:
        ...
        verbose_proxy_logger.error("derouter.proxy.proxy_server.delete_assistant(): Exception occured - %s", e)
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "message", str(e.detail)),
...
async def create_threads(
    ...
    try:
        ...
        verbose_proxy_logger.error("derouter.proxy.proxy_server.create_threads(): Exception occured - %s", e)
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "message", str(e.detail)),
...
async def get_thread(
    ...
    try:
        ...
        verbose_proxy_logger.error("derouter.proxy.proxy_server.get_thread(): Exception occured - %s", e)
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "message", str(e.detail)),
...
async def add_messages(
    ...
    try:
        ...
        verbose_proxy_logger.error("derouter.proxy.proxy_server.add_messages(): Exception occured - %s", e)
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "message", str(e.detail)),
...
async def get_messages(
    ...
    try:
        ...
        verbose_proxy_logger.error("derouter.proxy.proxy_server.get_messages(): Exception occured - %s", e)
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "message", str(e.detail)),
...
async def run_thread(
    ...
    try:
        ...
        verbose_proxy_logger.error("derouter.proxy.proxy_server.run_thread(): Exception occured - %s", e)
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "message", str(e.detail)),
...
from derouter.llms.base_llm.base_utils import BaseTokenCounter
from derouter.proxy.db.routing_prisma_wrapper import WriterPinnedClient
from derouter.repositories.config_repository import ConfigRepository
from derouter.repositories.model_repository import ModelRepository
from derouter.repositories.table_repositories import (
...
from derouter.repositories.team_repository import TeamRepository
from derouter.repositories.user_repository import UserRepository
...
def _get_provider_token_counter(
    ...
    from derouter.derouter_core_utils.get_llm_provider_logic import get_llm_provider

    full_model: Final = deployment.get("derouter_params", {}).get("model", "")
    model: str | None = None
    custom_llm_provider: str | None = None

    try:
        # Use existing DeRouter logic to determine provider
        model, custom_llm_provider, dynamic_api_key, api_base = get_llm_provider(
            model=full_model,
            custom_llm_provider=deployment.get("derouter_params", {}).get("custom_llm_provider"),
            api_base=deployment.get("derouter_params", {}).get("api_base"),
            api_key=deployment.get("derouter_params", {}).get("api_key"),
        )
        ...
        if full_model.startswith("anthropic/") or "anthropic" in full_model.lower():
            from derouter.llms.anthropic.common_utils import AnthropicModelInfo
...
async def token_counter(request: TokenCountRequest, call_endpoint: bool = False):
    ...
    if llm_router is not None:
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "derouter.proxy.proxy_server.token_counter(): Exception occured while getting deployment"
            )
    if deployment is not None:
        derouter_model_name = deployment.get("derouter_params", {}).get("model")
        model_info = deployment.get("model_info", {})
        load_credentials_from_list(deployment.get("derouter_params", {}))
        # remove the custom_llm_provider_prefix in the derouter_model_name
        if "/" in derouter_model_name:
            derouter_model_name = derouter_model_name.split("/", 1)[1]

    model_to_use: str = (
        derouter_model_name or request.model
    )  # use derouter model name, if it's not avalable then fallback to request.model
    ...
    # Default DeRouter token counting
    ...
    _tokenizer_used: Final = derouter.utils._select_tokenizer(model=model_to_use, custom_tokenizer=custom_tokenizer)
    ...
    total_tokens: Final = await asyncify(derouter.token_counter)(
        model=model_to_use,
        text=prompt,
...
async def supported_openai_params(model: str):
    """
    Returns supported openai params for a given derouter model name
    ...
    global llm_router
    try:
        resolved_models: Final = llm_router.resolved_derouter_models(model) if llm_router is not None else ()
        ...
        derouter_model, custom_llm_provider = (
            (target_model.removeprefix(f"{declared_provider}/"), declared_provider)
            if declared_provider is not None
            else derouter.get_llm_provider(model=target_model)[:2]
        )
        return {
            "supported_openai_params": derouter.get_supported_openai_params(
                model=derouter_model, custom_llm_provider=custom_llm_provider
            )
        }
    except Exception:
...
async def transform_request(request: TransformRequestBody):
    from derouter.utils import return_raw_request
...
async def non_admin_all_models(
    ...
    unique_models: Final = _deduplicate_derouter_router_models(models=all_models)
    return unique_models


def _add_team_models_to_all_models(
    team_db_objects_typed: list[DeRouter_TeamTable],
    llm_router: Router,
) -> dict[str, set[str]]:
    """
...
async def _add_access_group_models_to_team_models(
    ...
    eligible_teams: Final[list[DeRouter_TeamTable]] = []
...
async def get_all_team_models(
    ...
    if user_teams == "*":
        ...
        team_db_objects_typed = [
            DeRouter_TeamTable.model_validate(team_db_object.model_dump()) for team_db_object in team_db_objects
        ]
    else:
...
def get_direct_access_models(
    user_db_object: DeRouter_UserTable,
    llm_router: Router,
...
async def _populate_team_access_on_models(
    ...
    if _user_has_admin_view(user_api_key_dict):
        ...
        if user_db_object is not None:
            user_object: Final = DeRouter_UserTable.model_validate(user_db_object.model_dump())
...
def _enrich_model_info_with_derouter_data(
    model: dict[str, Any], debug: bool = False, llm_router: Router | None = None
) -> dict[str, Any]:
    """
    Enrich a model dictionary with derouter model info (pricing, context window, etc.)
    ...
    # read derouter model_prices_and_context_window.json to get the following:
    # input_cost_per_token, output_cost_per_token, max_tokens
    derouter_model_info = get_derouter_model_info(model=model)

    # 2nd pass on the model, try seeing if we can find model in derouter model_cost map
    if derouter_model_info == {}:
        # use derouter_param model_name to get model_info
        derouter_params = model.get("derouter_params", {})
        derouter_model = derouter_params.get("model", None)
        try:
            derouter_model_info = derouter.get_model_info(model=derouter_model)
        except Exception:
            derouter_model_info = {}
    ...
    if derouter_model_info == {}:
        ...
        derouter_model = derouter_params.get("model", None)
        if derouter_model:
            split_model: Final = derouter_model.split("/")
            if len(split_model) > 0:
                derouter_model = split_model[-1]
            try:
                derouter_model_info = derouter.get_model_info(model=derouter_model, custom_llm_provider=split_model[0])
            except Exception:
                derouter_model_info = {}
    for k, v in derouter_model_info.items():
        if k not in model_info:
            model_info[k] = v
    model["model_info"] = model_info
    # don't return the api key / vertex credentials
    # don't return the llm credentials
    model = remove_sensitive_info_from_deployment(model, excluded_keys={"derouter_credential_name"})
    return model
...
def _is_auto_router_model(model: Mapping[str, object]) -> bool:
    ...
    *semantic* auto-router strategy" and returns False for the complexity and adaptive
    ...
    if not isinstance(derouter_params, Mapping):
        return False
    derouter_model: Final = derouter_params.get("model")
    return isinstance(derouter_model, str) and derouter_model.startswith("auto_router/")
...
async def _load_team_object_for_model_filter(team_id: str, prisma_client: PrismaClient) -> DeRouter_TeamTable | None:
    ...
    try:
        ...
        if team_db_object is None:
            ...
            return None
        return DeRouter_TeamTable.model_validate(team_db_object.model_dump())
    except Exception as e:
...
async def _gather_team_accessible_model_ids(
    team_object: DeRouter_TeamTable,
    team_id: str,
    prisma_client: PrismaClient,
    llm_router: Router,
) -> set[str]:
...
async def model_info_v2(
    ...
    exclude_auto_routers: bool | None = fastapi.Query(
        False,
        description=(
            "Omit auto-router deployments (derouter model prefixed `auto_router/`). "
    ...
    Query parameters:
        ...
        modelId: Return a single deployment by DeRouter model id.
    ...
    ```json
    {
        "data": [
            {
                "model_name": "gpt-4",
                "derouter_params": {"model": "openai/gpt-4.1"},
                "model_info": {
                    "id": "abc123",
    ...
    if modelId is not None:
        ...
        if user_model is not None:
            # if user does not use a config.yaml, https://github.com/Delify-Solutions/derouter/issues/2061
    ...
    # Fill in model info based on config.yaml and derouter model_prices_and_context_window.json
    ...
    for i, _model in enumerate(all_models):
        all_models[i] = _enrich_model_info_with_derouter_data(
            model=_model,
    ...
    from derouter.proxy.agent_endpoints.model_list_helpers import (
...
async def model_metrics_exceptions(
    ...
    sql_query: Final = """
        WITH cte AS (
            SELECT
                CASE WHEN api_base = '' THEN derouter_model_name ELSE CONCAT(derouter_model_name, '-', api_base) END AS combined_model_api_base,
                exception_type,
                COUNT(*) AS num_rate_limit_exceptions
            FROM "DeRouter_ErrorLogs"
            WHERE
...
def _get_proxy_model_info(model: dict) -> dict:
    ...
    model["model_info"] = model_info
    # don't return the llm credentials
    model = remove_sensitive_info_from_deployment(deployment_dict=model, excluded_keys={"derouter_credential_name"})
...
async def model_info_v1(
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
    derouter_model_id: str | None = None,
    include_team_models: bool | None = fastapi.Query(
        False,
    ...
    Parameters:
        derouter_model_id: Optional[str] = None (this is the value of `x-derouter-model-id` returned in response headers)

        - When derouter_model_id is passed, it will return the info for that specific model
        - When derouter_model_id is not passed, it will return the info for all models
        ...
        - healthy_only: When true, hide models whose backing deployments are all marked
          ...
          unfiltered (fail open). Ignored when `derouter_model_id` is passed, since that
    ...
    if user_model is not None:
        # user is trying to get specific model from derouter router
        try:
            model_info: dict = cast(dict, derouter.get_model_info(model=user_model))
        except Exception:
            model_info = {}
        ...
        _deployment_info_dict = remove_sensitive_info_from_deployment(
            ...
            excluded_keys={"derouter_credential_name"},
        )
    ...
    if llm_model_list is None:
        raise HTTPException(
            status_code=500,
            detail={
                "error": "LLM Model List not loaded in. Make sure you passed models in your config.yaml or on the DeRouter Admin UI. - https://router.delify.vn/docs/proxy/configs"
            },
        )
    ...
    if llm_router is None:
        raise HTTPException(
            status_code=500,
            detail={
                "error": "LLM Router is not loaded in. Make sure you passed models in your config.yaml or on the DeRouter Admin UI. - https://router.delify.vn/docs/proxy/configs"
            },
        )
    ...
    if derouter_model_id is not None:
        # user is trying to get specific model from derouter router
        deployment_info: Final = llm_router.get_deployment(model_id=derouter_model_id)
        if deployment_info is None:
            raise HTTPException(
                status_code=400,
                detail={"error": f"Model id = {derouter_model_id} not found on derouter proxy"},
            )
    ...
    all_models = [
        _translate_model_name_for_response(_enrich_model_info_with_derouter_data(model=model, llm_router=llm_router))
        for model in all_models
    ]
...
def _get_model_group_info(
    ...
    if derouter.public_model_groups is not None:
        for mg in model_groups:
            if mg.model_group in derouter.public_model_groups:
...
async def model_group_info(
    ...
    """
    Get information about all the deployments on derouter proxy, including config.yaml descriptions (except api key and api base)
    ...
    Learn how to use and set wildcard models [here](https://router.delify.vn/docs/wildcard_routing)
    ...
    from derouter.proxy.utils import get_available_models_for_user
...
async def model_settings():
    ...
    for provider in derouter.provider_list:
        returned_list.append(
            ProviderInfo(
                name=provider,
                fields=derouter.get_provider_fields(custom_llm_provider=provider),
            )
        )
...
async def async_queue_request(
    ...
    try:
        ...
        data.pop("_derouter_strip_stream_usage", None)
        ...
        fastapi_response.headers.update({"x-derouter-priority": str(data["priority"])})
        return response
    except Exception as e:
        await proxy_logging_obj.post_call_failure_hook(
            user_api_key_dict=user_api_key_dict, original_exception=e, request_data=data
...
async def fallback_login(request: Request):
    ...
    PROXY_BASE_URL should be the your deployed proxy endpoint, e.g. PROXY_BASE_URL="https://derouter-production-7002.up.railway.app/"
    Example:
    """
    from derouter.proxy.proxy_server import ui_link
    ...
    hide_default_credentials_hint: Final = (
        os.getenv("DEROUTER_HIDE_DEFAULT_CREDENTIALS_HINT", "false").lower() == "true"
...
async def login(request: Request):
    ...
    from derouter.proxy.auth.login_utils import authenticate_user, create_ui_token_object, encode_ui_session_jwt
    from derouter.proxy.utils import get_custom_url
    ...
    # Build redirect URL
    derouter_dashboard_ui = get_custom_url(str(request.base_url))
    derouter_dashboard_ui = derouter_dashboard_ui.rstrip("/")
    derouter_dashboard_ui += "/ui?login=success"
    ...
    cp_return_to: Final = request.cookies.get("derouter_cp_return_to")
    if cp_return_to:
        try:
            ...
            verbose_proxy_logger.info("Ignoring stale derouter_cp_return_to cookie; landing on dashboard")
            resumed = None
        if resumed is not None:
            return resumed

    # Create redirect response with cookie
    redirect_response: Final = RedirectResponse(url=derouter_dashboard_ui, status_code=303)
    set_session_token_cookie(redirect_response, request, jwt_token)
    if cp_return_to:
        redirect_response.delete_cookie(key="derouter_cp_return_to")
...
async def login_v2(request: Request):
    global premium_user, general_settings, master_key
    from derouter.proxy.auth.login_utils import authenticate_user, create_ui_token_object, encode_ui_session_jwt
    from derouter.proxy.management_endpoints.ui_sso import set_session_token_cookie
    from derouter.proxy.utils import get_custom_url
    ...
    try:
        ...
        json_response: Final = JSONResponse(
            content={"redirect_url": derouter_dashboard_ui, "token": jwt_token},
            status_code=status.HTTP_200_OK,
        )
        set_session_token_cookie(json_response, request, jwt_token)
        return json_response
    except Exception as e:
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.login_v2(): Exception occurred - %s", e)
        if isinstance(e, ProxyException):
            raise e
...
async def login_v3(request: Request):
    ...
    try:
        ...
        cache_value: Final = {"token": jwt_token, "redirect_url": derouter_dashboard_ui}
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.login_v3(): Exception occurred - %s", e)
        if isinstance(e, ProxyException):
            raise e
        elif isinstance(e, HTTPException):
            raise ProxyException(
...
async def login_v3_exchange(request: Request):
    ...
    try:
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.login_v3_exchange(): Exception occurred - %s", e)
        raise ProxyException(
            message=str(e),
            type=ProxyErrorTypes.auth_error,
            param="None",
...
async def onboarding(invite_link: str, request: Request):
    ...
    from derouter.types.proxy.ui_sso import ReturnedUITokenObject

    if master_key is None:
        raise ProxyException(
            message="Master Key not set for Proxy. Please set Master Key to use Admin UI. Set `DEROUTER_MASTER_KEY` in .env or set general_settings:master_key in config.yaml.  https://router.delify.vn/docs/proxy/virtual_keys. If set, use `--detailed_debug` to debug issue.",
            type=ProxyErrorTypes.auth_error,
            param="master_key",
            code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
    ...
    utc_now_date: Final = derouter.utils.get_utc_datetime().date()
    ...
    derouter_dashboard_ui = get_custom_url(str(request.base_url))
    derouter_dashboard_ui = derouter_dashboard_ui.rstrip("/")
    derouter_dashboard_ui += "/ui/onboarding"
    import jwt

    user_email: Final = user_obj.user_email
    onboarding_token: Final = jwt.encode(
        {
            "token_type": "derouter_onboarding",
            "invitation_link": invite_link,
            "user_id": user_obj.user_id,
            "exp": derouter.utils.get_utc_datetime() + timedelta(minutes=15),
        },
        master_key,
        algorithm="HS256",
    )
    ...
    returned_ui_token_object: Final = ReturnedUITokenObject(
        ...
        premium_user=premium_user,
        auth_header_name=general_settings.get("derouter_key_header_name", "Authorization"),
    ...
    derouter_dashboard_ui += f"?token={jwt_token}&user_email={user_email}"
    return {
        "login_url": derouter_dashboard_ui,
        "token": jwt_token,
...
def _get_onboarding_claims_from_request(request: Request) -> dict:
    ...
    auth_header_name: Final = general_settings.get("derouter_key_header_name", "Authorization")
    ...
    try:
        await InvitationLinkRepository(prisma_client).table.update_many(
            ...
            data={
                "accepted_at": None,
                "is_accepted": False,
                "updated_at": derouter.utils.get_utc_datetime(),
                "updated_by": user_id,
            },
        )
    except Exception:
...
async def _generate_onboarding_ui_session_token(user_obj: _UserTableRow) -> str:
    ...
    response: Final = await generate_key_helper_fn(
        ...
        **{
            ...
            "duration": DEROUTER_UI_SESSION_DURATION,
            "key_max_budget": derouter.max_ui_session_budget,
            "models": [],
            "aliases": {},
            "config": {},
            "spend": 0,
...
async def claim_onboarding_link(data: InvitationClaim, request: Request):
    ...
    if (
        onboarding_claims.get("token_type") != "derouter_onboarding"
    ...
    current_time = derouter.utils.get_utc_datetime()
    async with prisma_client.db.tx() as tx:
        updated_count: Final = await tx.derouter_invitationlink.update_many(
        ...
        ### UPDATE USER OBJECT ###
        user_obj: Final[_UserTableRow | None] = await tx.derouter_usertable.update(
        ...
        #### MARK LINK AS USED
        current_time = derouter.utils.get_utc_datetime()
        await tx.derouter_invitationlink.update(
            where={"id": data.invitation_link},
            data={
...
def _serve_custom_ui_logo(candidate: str) -> Response | None:
    ...
    from derouter.proxy.common_utils.static_asset_utils import (
...
async def get_image(theme: Literal["light", "dark"] | None = None):
    ...
    # Priority: DEROUTER_ASSETS_PATH env var > default based on is_non_root
    default_assets_dir: Final = "/var/lib/derouter/assets" if is_non_root else current_dir
    assets_dir = os.getenv("DEROUTER_ASSETS_PATH", default_assets_dir)
...
async def get_favicon():
    ...
    favicon_url: Final = os.getenv("DEROUTER_FAVICON_URL", "")
    ...
    if favicon_url.startswith(("http://", "https://")):
        ...
        verbose_proxy_logger.warning(
            "DEROUTER_FAVICON_URL %r is not a supported image file or does not exist, falling back to default favicon",
            favicon_url,
        )
        if os.path.exists(default_favicon):
            return FileResponse(default_favicon, media_type="image/x-icon")
...
async def new_invitation(data: InvitationNew, user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth)):
    ...
    curl -X POST 'http://localhost:4000/invitation/new' \
        -H 'Content-Type: application/json' \
        -d '{
            "user_id": "1234" // 👈 id of user in 'DeRouter_UserTable'
        }'
    ```
    """
    try:
        from derouter.proxy.management_helpers.user_invitation import (
...
async def invitation_update(
    ...
    curl -X POST 'http://localhost:4000/invitation/update' \
        -H 'Content-Type: application/json' \
        -d '{
            "invitation_id": "1234" // 👈 id of invitation in 'DeRouter_InvitationTable'
...
async def update_config(
    ...
    Writes only the sections present in the request body to DeRouter_Config rows
    ...
    try:
        ...
        if isinstance(raw_router_settings, dict):
            supported_router_settings: Final = RUNTIME_UPDATABLE_ROUTER_SETTINGS | (
                frozenset(derouter.Router.get_valid_args()) - ROUTER_SETTINGS_MANAGED_OUTSIDE_CONFIG
            )
        ...
        # derouter_settings: merge existing + request, request wins (matching
        ...
        if config_info.derouter_settings is not None:
            existing = await _read_section("derouter_settings")
            before_derouter_settings: Final = copy.deepcopy(existing)
            updated_derouter_settings: Final = dict(config_info.derouter_settings)

            incoming_cb = updated_derouter_settings.get("success_callback")
            if isinstance(incoming_cb, list):
                updated_derouter_settings["success_callback"] = normalize_callback_names(incoming_cb)

            merged: Final = {**existing, **updated_derouter_settings}
            ...
            await _upsert_section("derouter_settings", merged)
            asyncio.create_task(
                create_config_audit_log(
                    "derouter_settings", "updated", before_derouter_settings, merged, user_api_key_dict
                )
            )
        ...
        verbose_proxy_logger.error("derouter.proxy.proxy_server.update_config(): Exception occured - %s", e)
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "detail", f"Authentication Error({e})"),
...
async def update_config_general_settings(
    ...
    """
    Update a specific field in derouter general settings
    """
    global prisma_client
    ## VALIDATION ##
    """
    ...
    if data.field_name in _GENERAL_SETTINGS_UI_DEROUTER_FIELDS:
        return await _persist_general_settings_ui_derouter_field(data.field_name, data.field_value, user_api_key_dict)
...
# Matches the cap on _redact_sensitive_derouter_params (the closest analog in the
...
def _dump_redacted_config(value: JsonValue | None, *, redact_all_values: bool = False) -> str | None:
    ...
    # team_endpoints.py and the DeRouter_AuditLogs validator, so a YAML-loaded
...
async def create_config_audit_log(
    ...
    """Record a system-wide settings change in DeRouter_AuditLog.
...
GeneralSettingsUIDeRouterValue = float | bool | str | None


class GeneralSettingsUIDeRouterFieldSpec(TypedDict):
...
_GENERAL_SETTINGS_UI_DEROUTER_FIELDS: Final[dict[str, GeneralSettingsUIDeRouterFieldSpec]] = {
...
def _general_settings_ui_derouter_default(
    spec: GeneralSettingsUIDeRouterFieldSpec,
) -> GeneralSettingsUIDeRouterValue:
...
def _validate_general_settings_ui_derouter_value(field_name: str, value: object) -> GeneralSettingsUIDeRouterValue:
    spec: Final = _GENERAL_SETTINGS_UI_DEROUTER_FIELDS[field_name]
    field_type: Final = spec["type"]
    if value is None or value == "":
        return _general_settings_ui_derouter_default(spec)
    match field_type:
        case "Boolean":
...
async def _persist_general_settings_ui_derouter_field(
    field_name: str, value: object, user_api_key_dict: UserAPIKeyAuth
) -> dict:
    validated: Final = _validate_general_settings_ui_derouter_value(field_name, value)
    config: Final = await proxy_config.get_config()
    before_value: Final = config.get("derouter_settings", {}).get(field_name)
    setattr(derouter, field_name, validated)
    if "derouter_settings" not in config:
        config["derouter_settings"] = {}
    config["derouter_settings"][field_name] = validated
...
async def _reset_general_settings_ui_derouter_field(field_name: str, user_api_key_dict: UserAPIKeyAuth) -> dict:
    config: Final = await proxy_config.get_config()
    before_value: Final = config.get("derouter_settings", {}).get(field_name)
    default_value: Final = _general_settings_ui_derouter_default(_GENERAL_SETTINGS_UI_DEROUTER_FIELDS[field_name])
    setattr(derouter, field_name, default_value)
    if "derouter_settings" in config:
        config["derouter_settings"].pop(field_name, None)
    await proxy_config.save_config(new_config=config)
...
async def get_config_list(
    ...
    db_derouter_settings_row: Final[_ConfigParamRow | None] = await _config_param_table(prisma_client).find_first(
        where={"param_name": "derouter_settings"}
    )
    db_derouter_settings: Final[dict] = (
        dict(db_derouter_settings_row.param_value)
        if db_derouter_settings_row is not None and db_derouter_settings_row.param_value is not None
        else {}
    )
    for derouter_field_name, spec in _GENERAL_SETTINGS_UI_DEROUTER_FIELDS.items():
        current_value: GeneralSettingsUIDeRouterValue = getattr(derouter, derouter_field_name, None)
        default_value = _general_settings_ui_derouter_default(spec)
        stored_in_db_derouter: bool | None
        if derouter_field_name in db_derouter_settings:
            stored_in_db_derouter = True
        elif current_value != default_value:
            stored_in_db_derouter = False
        else:
            stored_in_db_derouter = None
        return_val.append(
            ConfigList(
                field_name=derouter_field_name,
                field_type=spec["type"],
                field_description=spec["description"],
                field_value=current_value,
                stored_in_db=stored_in_db_derouter,
...
async def delete_config_general_settings(
    ...
    """
    Delete the db value of this field in derouter general settings. Resets it to it's initial default value on derouter.
    """
    global prisma_client
    ## VALIDATION ##
    """
    ...
    if data.field_name in _GENERAL_SETTINGS_UI_DEROUTER_FIELDS:
        return await _reset_general_settings_ui_derouter_field(data.field_name, user_api_key_dict)
...
async def delete_callback(
    ...
    try:
        ...
        derouter_settings: Final = config.get("derouter_settings", {})
        success_callbacks: Final = derouter_settings.get("success_callback", [])
        ...
        config.setdefault("derouter_settings", {})["success_callback"] = success_callbacks
        ...
        verbose_proxy_logger.error("derouter.proxy.proxy_server.delete_callback(): Exception occurred - %s", e)
        verbose_proxy_logger.debug(traceback.format_exc())
        raise ProxyException(
...
async def get_config(
    ...
    try:
        ...
        _derouter_settings: Final = config_data.get("derouter_settings", {})
        _general_settings: Final = config_data.get("general_settings", {})
        environment_variables: Final = config_data.get("environment_variables", {})

        is_full_admin: Final = user_api_key_dict.user_role == LitellmUserRoles.PROXY_ADMIN

        _success_callbacks = _derouter_settings.get("success_callback", [])
        _failure_callbacks = _derouter_settings.get("failure_callback", [])
        _success_and_failure_callbacks = _derouter_settings.get("callbacks", [])
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.get_config(): Exception occured - %s", e)
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "detail", f"Authentication Error({e})"),
                type=ProxyErrorTypes.auth_error,
...
async def config_yaml_endpoint(config_info: ConfigYAML):
    ...
    - **model_list**: *Optional[List[ModelParams]]* - A list of supported models on the server, along with model-specific configurations. ModelParams includes "model_name" (name of the model), "derouter_params" (derouter-specific parameters for the model), and "model_info" (additional info about the model such as id, mode, cost per token, etc).

    - **derouter_settings**: *Optional[dict]*: Settings for the derouter module. You can specify multiple properties like "drop_params", "set_verbose", "api_base", "cache".
...
async def get_model_cost_map_source(
    ...
    - is_env_forced: true if DEROUTER_LOCAL_MODEL_COST_MAP=True forced local usage
    ...
    try:
        ...
        model_count: Final = len(derouter.model_cost) if derouter.model_cost else 0
...
async def reload_anthropic_beta_headers(
    ...
    try:
        ...
        from derouter.anthropic_beta_headers_manager import reload_beta_headers_config
...
async def get_routes():
    ...
    """
    from derouter.proxy.common_utils.get_routes import GetRoutes
...
async def proxy_mcp_route(request: Request) -> Response:
    ...
    from derouter.proxy._experimental.mcp_server.mcp_context import (  # pyright: ignore[reportPrivateUsage]  # route-owned mode
        _mcp_proxy_mode,  # pyright: ignore[reportPrivateUsage]  # route-owned mode
    )
    from derouter.proxy._experimental.mcp_server.server import handle_streamable_http_mcp
    from derouter.proxy._experimental.mcp_server.utils import is_mcp_available
...
async def aggregate_mcp_route(request: Request):
    ...
    from derouter.proxy._experimental.mcp_server.server import (
...
async def _resolve_mcp_csv_tokens(csv_segment: str, client_ip: str | None) -> list[str]:
    ...
    """
    from derouter.constants import DEFAULT_MCP_NAMESPACE_CSV_MAX_TOKENS
    from derouter.proxy._experimental.mcp_server.mcp_server_manager import (
        global_mcp_server_manager,
    )
...
async def _is_mcp_access_group_cached(name: str) -> bool:
    ...
    """
    from derouter.constants import DEFAULT_MCP_ACCESS_GROUP_NEGATIVE_CACHE_TTL
...
async def dynamic_mcp_route(mcp_server_name: str, request: Request):
    ...
    try:
        ...
        from derouter.proxy.auth.ip_address_utils import IPAddressUtils
