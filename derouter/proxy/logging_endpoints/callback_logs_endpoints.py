...
through DeRouter's standard success/failure callback fan-out.

This exists for hosts that own a request outside the Python process — e.g. the
`derouter-rust` gateway proxying realtime websockets. Those hosts can't use the
...
from derouter.types.proxy.callback_logs_endpoints import (
    CallbackLogFailure,
    CallbackLogRecord,
    CallbackLogsRequest,
...
class CallbackLogsReplayer:
    """
    Replays finished logging payloads through DeRouter's callback fan-out.
    ...
    @staticmethod
    def _build_logging_obj(payload: dict[str, Any]) -> DeRouterLogging:
        """
        ...
        `response_cost`, and the `derouter_params.metadata` keys used for cost
        ...
        call_id: Final = payload.get("derouter_call_id") or payload.get("id") or str(uuid.uuid4())
        ...
        derouter_metadata: Final[dict[str, Any]] = {
