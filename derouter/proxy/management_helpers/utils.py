...
from derouter.proxy._types import (  # key request types; user request types; team request types; customer request types
    BudgetNewRequest,
...
from derouter.proxy.utils import PrismaClient, jsonify_object
...
class MemberWriteTx(Protocol):
    ...
    @property
    def derouter_usertable(self) -> _PrismaUserTable: ...

    @property
    def derouter_budgettable(self) -> _PrismaBudgetTable: ...

    @property
    def derouter_teammembership(self) -> _PrismaTeamMembershipTable: ...


def _user_table(prisma_client: PrismaClient, tx: MemberWriteTx | None) -> _PrismaUserTable:
    return tx.derouter_usertable if tx is not None else UserRepository(prisma_client).table


def _budget_table(prisma_client: PrismaClient, tx: MemberWriteTx | None) -> _PrismaBudgetTable:
    return tx.derouter_budgettable if tx is not None else BudgetRepository(prisma_client).table


def _team_membership_table(prisma_client: PrismaClient, tx: MemberWriteTx | None) -> _PrismaTeamMembershipTable:
    return tx.derouter_teammembership if tx is not None else TeamMembershipRepository(prisma_client).table
...
async def _find_users_by_email(
    ...
    if tx is not None:
        return await tx.derouter_usertable.find_many(where={"user_email": user_email})
...
async def _create_user_row(
    ...
    if tx is not None:
        return await _upsert_user_row(tx.derouter_usertable, str(user_data["user_id"]), jsonify_object(user_data))
...
def get_new_internal_user_defaults(user_id: str, user_email: str | None = None) -> dict[str, object]:
    user_info: Final = derouter.default_internal_user_params or {}

    returned_dict: Final[SSOUserDefinedValues] = {
        "models": user_info.get("models") or [],
        "max_budget": user_info.get("max_budget", derouter.max_internal_user_budget),
        "budget_duration": user_info.get("budget_duration", derouter.internal_user_budget_duration),
...
async def handle_budget_for_entity(
    ...
    Args:
        ...
        derouter_proxy_admin_name: Admin name for audit trail
    ...
    if existing_budget_id is None:
        if data_budget_id is not None:
            ...
            budget_row: Final = DeRouter_BudgetTable(**_budget_data)
...
# Fields on DeRouter_BudgetTable that represent the budget's *configuration*
...
async def add_new_member(
    ...
    tx: MemberWriteTx | None = None,
) -> tuple[DeRouter_UserTable, DeRouter_TeamMembership | None]:
    """
    ...
    """
    returned_user: DeRouter_UserTable | None = None
    returned_team_membership: DeRouter_TeamMembership | None = None
    ...
    if new_member.user_id is not None:
        ...
        if _returned_user is not None:
            returned_user = DeRouter_UserTable.model_validate(_returned_user.model_dump())
        ...
        if len(existing_user_row) == 0:
            ...
            returned_user = DeRouter_UserTable.model_validate(user_info.model_dump())
        elif len(existing_user_row) > 1:
            raise HTTPException(
                status_code=400,
    ...
    if _budget_id and returned_user is not None and returned_user.user_id is not None:
        ...
        returned_team_membership = DeRouter_TeamMembership.model_validate(_returned_team_membership.model_dump())
