...
from derouter.types.llms.openai import OpenAIFilesPurpose
from derouter.types.utils import SpecialEnums

if TYPE_CHECKING:
    from fastapi import Request
    from prisma.models import DeRouter_ManagedObjectTable
...
def get_models_from_unified_file_id(unified_file_id: str) -> list[str]:
    ...
    Example:
    unified_file_id = "derouter_proxy:application/octet-stream;unified_id,c4843482-b176-4901-8292-7523fd0f2c6e;target_model_names,gpt-4o-mini,gemini-2.0-flash"
...
def get_model_id_from_unified_batch_id(file_id: str) -> str | None:
    ...
    Expected format: derouter_proxy;model_id:{};llm_batch_id:{};llm_output_file_id:{}
    """
...
def encode_file_id_with_model(file_id: str, model: str, id_type: Literal["file", "batch"] = "file") -> str:
    ...
    Format: <prefix><base64(derouter:<original_id>;model,<model_name>)>
    ...
    Args:
        ...
        model: Model name from model_list (e.g., "gpt-4o-derouter")
    ...
    Examples:
        encode_file_id_with_model("file-abc123", "gpt-4o-derouter")
    ...
    """
    encoded_str: Final = f"derouter:{file_id};model,{model}"
...
def decode_model_from_file_id(encoded_id: str) -> str | None:
    ...
    try:
        ...
        decoded: Final = base64.urlsafe_b64decode(padded).decode()
        if decoded.startswith("derouter:") and ";model," in decoded:
...
def get_original_file_id(encoded_id: str) -> str:
    ...
    try:
        ...
        if decoded.startswith("derouter:") and ";model," in decoded:
            match: Final = re.search(r"derouter:([^;]+);model,", decoded)
            if match:
...
def extract_model_from_sources(
    ...
    2. Request headers (x-derouter-model)
    ...
    model_from_param = data.get("model") or request.query_params.get("model") or request.headers.get("x-derouter-model")
...
def get_team_provider_credentials(
    ...
    from derouter.proxy.auth.model_checks import get_complete_model_list, get_key_models
...
def add_internal_model_credentials(
    ...
    if credentials is None:
        return
    data["_derouter_internal_model_credentials"] = MappingProxyType(dict(credentials))
...
def validate_managed_files_requirement(
    ...
    """
    Enforce proxy-level managed files when derouter.require_managed_files is enabled.
    ...
    if derouter.require_managed_files is not True:
        return
    ...
    if not target_model_names:
        raise HTTPException(
            ...
            detail=(
                ...
                "in derouter_settings. Provide one or more model aliases via the "
    ...
    if model:
        raise HTTPException(
            ...
            detail=(
                ...
                "derouter_settings. Uploads must go through managed files using "
...
async def validate_managed_id_requirement(
    ...
    when ``derouter.require_managed_files`` is enabled, and authenticate managed ids against
    the caller's stored ownership record.

    Ownership is only recorded for DeRouter managed ids, so a raw provider id is forwarded to the
    ...
    if not _is_base64_encoded_unified_file_id(resource_id):
        raise HTTPException(
            ...
            detail=(
                ...
                f"derouter_settings. Use the DeRouter managed {resource_kind} id returned when the "
...
def _extract_model_param(request: "Request", request_body: dict) -> str | None:
    ...
    3. Header (x-derouter-model)
    """
    return request_body.get("model") or request.query_params.get("model") or request.headers.get("x-derouter-model")
...
def _model_name_for_batch_response(response: "DeRouterBatch") -> str | None:
    hidden_params: Final = getattr(response, "_hidden_params", None) or {}
...
def _batch_owner_auth_from_db_object(db_batch_object: "DeRouter_ManagedObjectTable") -> "UserAPIKeyAuth | None":
    from derouter.proxy._types import UserAPIKeyAuth
...
def apply_unified_file_ids(response: "DeRouterBatch", unified_id_by_raw_id: Mapping[str, str]) -> None:
...
async def ensure_batch_response_managed_file_ids(
    ...
    db_batch_object: "DeRouter_ManagedObjectTable | None" = None,
    ...
    for file_attr in ("output_file_id", "error_file_id"):
        ...
        try:
            ...
            await managed_files_obj.store_unified_file_id(
                ...
                file_object=None,
                derouter_parent_otel_span=getattr(effective_auth, "parent_otel_span", None),
...
async def get_batch_from_database(
    ...
    Returns:
        ...
        - response_batch: Parsed DeRouterBatch object (or None)
    """
    import json
    ...
    try:
        ...
        response: Final = DeRouterBatch.model_validate(batch_data)
...
def batch_cost_poller_is_active() -> bool:
    ...
    """
    from derouter.constants import PROXY_BATCH_POLLING_ENABLED
...
def _completed_batch_safe_to_retire(response: "DeRouterBatch") -> bool:
