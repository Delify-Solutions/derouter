...
import derouter
from derouter import CreateFileRequest, get_secret_str
...
from derouter.proxy.openai_files_endpoints.batch_file_validation import (
    check_batch_file_upload,
    raise_batch_file_validation_failure,
)
from derouter.proxy.openai_files_endpoints.batch_guardrails import (
    EMPTY_MAPPING,
    BatchScanResult,
...
from derouter.proxy.openai_files_endpoints.general_upload_validation import (
    MB,
...
from derouter.proxy.utils import ProxyLogging, is_known_model
from derouter.repositories.table_repositories import ManagedFileRepository
...
async def route_create_file(
    ...
    if target_storage and target_storage != "default":
        ...
        from derouter.proxy.openai_files_endpoints.storage_backend_service import (
    ...
    if model is not None:
        ...
        response = await derouter.acreate_file(
    ...
    # EXISTING: Deprecated loadbalancing approach (for backwards compatibility when not using managed files)
    elif derouter.enable_loadbalancing_on_batch_endpoints is True and is_router_model and router_model is not None:
        ...
        response = await derouter.acreate_file(**_create_file_request, custom_llm_provider=custom_llm_provider)
...
async def create_file(
    ...
    derouter_metadata: str | None = Form(default=None),
    ...
    try:
        ...
        extracted_derouter_metadata: Final[dict[str, Any] | None] = extract_nested_form_metadata(
            form_data=form_data_dict, prefix="derouter_metadata["
        )
        ...
        # Add derouter_metadata to data if provided (from form field)
        if extracted_derouter_metadata is not None:
            data["derouter_metadata"] = extracted_derouter_metadata
        ...
        # /v1/files stores its proxy metadata under derouter_metadata, not metadata
        request_metadata: Final = data.get("metadata") or data.get("derouter_metadata") or EMPTY_MAPPING
        ...
        if derouter.enable_loadbalancing_on_batch_endpoints is True:
        ...
        if scan_result is not None and scan_result.changes:
            response.derouter_batch_guardrail = scan_result.report()
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.create_file(): Exception occured - %s", e)
...
async def get_file_content(
    ...
    try:
        ...
        ## check if file_id is a derouter managed file
        ...
        if is_base64_unified_file_id:
            ...
            if is_managed_cloud_storage_uri(file_id):
                raise HTTPException(
                    status_code=400,
                    detail="Raw cloud storage file ids cannot be retrieved directly. Use the DeRouter managed file id returned when the file was created.",
                )
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.retrieve_file_content(): Exception occured - %s", e)
...
async def get_file(
    ...
    try:
        ...
        if should_route:
            ...
            response = await derouter.afile_retrieve(**data)
        ...
        ## EXISTING: check if file_id is a derouter managed file
            ...
            response = await derouter.afile_retrieve(
                custom_llm_provider=custom_llm_provider,
                file_id=file_id,
                **data,
            )
        ...
        verbose_proxy_logger.error("derouter.proxy.proxy_server.retrieve_file(): Exception occured - %s", e)
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "message", str(e.detail)),
...
async def delete_file(
    ...
    try:
        ...
        if should_route and credentials is not None:
            ...
            response = await derouter.afile_delete(
                custom_llm_provider=credentials["custom_llm_provider"],
                **data,
            )
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.delete_file(): Exception occured - %s", e)
        if isinstance(e, HTTPException):
            raise ProxyException(
                message=getattr(e, "message", str(e.detail)),
                type=openai_error_type(e, error_status_code(e, status.HTTP_400_BAD_REQUEST)),
...
async def list_files(
    ...
    try:
        ...
        if should_route and credentials is not None:
            ...
            response = await derouter.afile_list(
                custom_llm_provider=credentials["custom_llm_provider"],
                purpose=purpose,
                **data,
            )
        ...
        verbose_proxy_logger.error("derouter.proxy.proxy_server.list_files(): Exception occured - %s", e)
        verbose_proxy_logger.debug(traceback.format_exc())
        if isinstance(e, ProxyException):
            raise
        if isinstance(e, HTTPException):
