...
from derouter.proxy.guardrails.usage_tracking import guardrail_status_to_action
...
if TYPE_CHECKING:
    ...
    from derouter.types.guardrails import Guardrail

    _DbOrConfigGuardrail = prisma_models.DeRouter_GuardrailsTable | Guardrail
    _DailyMetricsRow = prisma_models.DeRouter_DailyGuardrailMetrics | prisma_models.DeRouter_DailyPolicyMetrics
...
) -> "TableActions[prisma_models.DeRouter_GuardrailsTable]":
    guardrails_table: Final[TableActions[prisma_models.DeRouter_GuardrailsTable]] = GuardrailsRepository(
        prisma_client
    ).table
...
def _policies_table(
    prisma_client: "PrismaClient",
) -> "TableActions[prisma_models.DeRouter_PolicyTable]":
    policies_table: Final[TableActions[prisma_models.DeRouter_PolicyTable]] = PolicyRepository(prisma_client).table
    return policies_table


def _daily_guardrail_metrics_table(
    prisma_client: "PrismaClient",
) -> "TableActions[prisma_models.DeRouter_DailyGuardrailMetrics]":
    metrics_table: Final[TableActions[prisma_models.DeRouter_DailyGuardrailMetrics]] = DailyGuardrailMetricsRepository(
        prisma_client
    ).table
...
def _daily_policy_metrics_table(
    prisma_client: "PrismaClient",
) -> "TableActions[prisma_models.DeRouter_DailyPolicyMetrics]":
    metrics_table: Final[TableActions[prisma_models.DeRouter_DailyPolicyMetrics]] = DailyPolicyMetricsRepository(
        prisma_client
    ).table
    return metrics_table
...
async def _find_daily_guardrail_metrics(
    prisma_client: "PrismaClient",
    where: "prisma_types.DeRouter_DailyGuardrailMetricsWhereInput",
) -> "Sequence[prisma_models.DeRouter_DailyGuardrailMetrics]":
    return await _daily_guardrail_metrics_table(prisma_client).find_many(where=where)


async def _find_daily_policy_metrics(
    prisma_client: "PrismaClient",
    where: "prisma_types.DeRouter_DailyPolicyMetricsWhereInput",
) -> "Sequence[prisma_models.DeRouter_DailyPolicyMetrics]":
    return await _daily_policy_metrics_table(prisma_client).find_many(where=where)


def _daily_guardrail_usage_units_table(
    prisma_client: "PrismaClient",
) -> "TableActions[prisma_models.DeRouter_DailyGuardrailUsageUnits]":
    units_table: Final[TableActions[prisma_models.DeRouter_DailyGuardrailUsageUnits]] = (
...
async def _find_daily_guardrail_usage_units(
    prisma_client: "PrismaClient",
    where: "prisma_types.DeRouter_DailyGuardrailUsageUnitsWhereInput",
) -> "Sequence[prisma_models.DeRouter_DailyGuardrailUsageUnits]":
    ...
    try:
        ...
        verbose_proxy_logger.warning(
            "Guardrail usage units are unavailable until the DeRouter_DailyGuardrailUsageUnits migration is applied: %s",
            e,
        )
        return ()


def _counter_name(row: "prisma_models.DeRouter_DailyGuardrailUsageUnits") -> str:
    return row.usage_unit


def _team_of(row: "prisma_models.DeRouter_DailyGuardrailUsageUnits") -> str:
    return row.team_id


def _key_of(row: "prisma_models.DeRouter_DailyGuardrailUsageUnits") -> str:
    return row.api_key


def _row_untracked_units(row: "prisma_models.DeRouter_DailyGuardrailUsageUnits") -> int:
...
def _row_tracked_cost(row: "prisma_models.DeRouter_DailyGuardrailUsageUnits") -> float | None:
...
def _sum_counter_units(rows: "Iterable[prisma_models.DeRouter_DailyGuardrailUsageUnits]") -> Mapping[str, int]:
...
def _sum_untracked_units(rows: "Iterable[prisma_models.DeRouter_DailyGuardrailUsageUnits]") -> Mapping[str, int]:
    ordered: Final = sorted(rows, key=_counter_name)
...
def _sum_tracked_cost(rows: "Iterable[prisma_models.DeRouter_DailyGuardrailUsageUnits]") -> float | None:
...
def _by(
    rows: "Sequence[prisma_models.DeRouter_DailyGuardrailUsageUnits]",
    key_of: "Callable[[prisma_models.DeRouter_DailyGuardrailUsageUnits], str]",
    reduce: "Callable[[Iterable[prisma_models.DeRouter_DailyGuardrailUsageUnits]], _T]",
) -> Mapping[str, _T]:
...
_GuardrailObjectField = Literal["derouter_params", "guardrail_info"]
...
def _guardrail_overview_rows(
    ...
    for g in guardrails:
        ...
        derouter_params = _to_dict(_get_guardrail_field(g, "derouter_params"))
        provider = _field_str(derouter_params, "guardrail", "Unknown")
...
def _policy_overview_rows(
    policies: "Sequence[prisma_models.DeRouter_PolicyTable]",
    agg: Mapping[str, _MetricTotals],
    prev_agg: Mapping[str, float],
) -> list[UsageOverviewRow]:
    rows: Final[list[UsageOverviewRow]] = []
...
async def guardrails_usage_overview(
    ...
    try:
        ...
        metrics: Final[Sequence[prisma_models.DeRouter_DailyGuardrailMetrics]] = await _find_daily_guardrail_metrics(
            prisma_client, where={"date": {"gte": start, "lte": end}}
        )

        # Previous period for trend
        start_prev: Final = (date.fromisoformat(start) - timedelta(days=7)).isoformat()
        metrics_prev: Sequence[prisma_models.DeRouter_DailyGuardrailMetrics] = await _find_daily_guardrail_metrics(
            prisma_client, where={"date": {"gte": start_prev, "lt": start}}
        )

        units_where: Final[prisma_types.DeRouter_DailyGuardrailUsageUnitsWhereInput] = {
...
async def guardrails_usage_detail(
    ...
    start_prev: Final = (date.fromisoformat(start) - timedelta(days=7)).isoformat()
    metrics_prev: Final[Sequence[prisma_models.DeRouter_DailyGuardrailMetrics]] = await _find_daily_guardrail_metrics(
        prisma_client,
        where={
            "guardrail_id": {"in": metric_ids},
    ...
    derouter_params: Final = _to_dict(_get_guardrail_field(guardrail, "derouter_params"))
...
def _build_usage_logs_where(
    ...
    start_date: str | None,
    end_date: str | None,
) -> "prisma_types.DeRouter_SpendLogGuardrailIndexWhereInput":
    where: Final[prisma_types.DeRouter_SpendLogGuardrailIndexWhereInput] = {}
    if guardrail_ids:
...
def _usage_log_entry_from_row(
    r: "prisma_models.DeRouter_SpendLogGuardrailIndex",
    sl: "prisma_models.DeRouter_SpendLogs",
...
def _input_snippet_for_log(sl: "prisma_models.DeRouter_SpendLogs") -> str | None:
...
async def guardrails_usage_logs(
    ...
    try:
        ...
        index_rows: Sequence[prisma_models.DeRouter_SpendLogGuardrailIndex] = await SpendLogGuardrailIndexRepository(
            prisma_client
        ).table.find_many(
            where=where,
        ...
        spend_logs: Final[Sequence[prisma_models.DeRouter_SpendLogs]] = await SpendLogsRepository(
            prisma_client
...
async def policies_usage_overview(
    ...
    try:
        ...
        metrics: Final[Sequence[prisma_models.DeRouter_DailyPolicyMetrics]] = await _find_daily_policy_metrics(
            prisma_client, where={"date": {"gte": start, "lte": end}}
        )
        metrics_prev: Final[Sequence[prisma_models.DeRouter_DailyPolicyMetrics]] = await _find_daily_policy_metrics(
            prisma_client,
            where={
                "date": {
