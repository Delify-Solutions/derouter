...
from derouter.constants import DEFAULT_MAX_RECURSE_DEPTH
...
from derouter.proxy.common_utils.path_utils import safe_join
from derouter.proxy.guardrails.guardrail_hooks.custom_code.sandbox import (
    build_sandbox_globals,
    compile_sandboxed,
)
from derouter.proxy.guardrails.guardrail_registry import GuardrailRegistry
from derouter.proxy.guardrails.usage_endpoints import router as guardrails_usage_router
...
from derouter.repositories.table_repositories import GuardrailsRepository
...
from derouter.types.proxy.guardrails.guardrail_hooks.hide_secrets import (
...
if TYPE_CHECKING:
    ...
    from prisma.models import DeRouter_GuardrailsTable
...
def _guardrails_table(prisma_client: "PrismaClient") -> "TableActions[DeRouter_GuardrailsTable]":
    return GuardrailsRepository(prisma_client).table


async def _create_guardrail_row(prisma_client: "PrismaClient", data: Mapping[str, object]) -> "DeRouter_GuardrailsTable":
...
) -> "Sequence[DeRouter_GuardrailsTable]":
...
def _get_guardrails_list_response(
    ...
    for guardrail in guardrails_config:
        derouter_params = guardrail.get("derouter_params") or {}
...
async def list_guardrails():
    ...
    👉 [Guardrail docs](https://router.delify.vn/docs/proxy/guardrails/quick_start)
...
async def list_guardrails_v2(
    ...
    """
    from derouter.derouter_core_utils.derouter_logging import _get_masked_values
    from derouter.proxy.guardrails.guardrail_registry import IN_MEMORY_GUARDRAIL_HANDLER
    ...
    try:
        ...
        for guardrail in guardrails:
            derouter_params: LitellmParams | dict | None = guardrail.get("derouter_params")
            derouter_params_dict = (
                derouter_params.model_dump(exclude_none=True)
                if isinstance(derouter_params, LitellmParams)
                else derouter_params
            ) or {}
            masked_derouter_params_dict = _get_masked_values(
                derouter_params_dict,
                unmasked_length=4,
                number_of_asterisks=4,
            )
            masked_derouter_params = (
                BaseLitellmParams(**masked_derouter_params_dict) if masked_derouter_params_dict else None
            )
            guardrail_configs.append(
                GuardrailInfoResponse(
                    guardrail_id=guardrail.get("guardrail_id"),
                    guardrail_name=guardrail.get("guardrail_name"),
                    derouter_params=masked_derouter_params,
                    guardrail_info=guardrail.get("guardrail_info"),
        ...
        # get guardrails initialized on derouter config.yaml
        ...
        for guardrail in in_memory_guardrails:
            ...
            if not is_admin:
                ...
                if g_team_id is not None and g_team_id not in caller_team_ids:
                    continue
            in_memory_derouter_params_raw = guardrail.get("derouter_params")
            in_memory_derouter_params_dict = (
                in_memory_derouter_params_raw.model_dump(exclude_none=True)
                if isinstance(in_memory_derouter_params_raw, LitellmParams)
                else in_memory_derouter_params_raw
            ) or {}
            masked_in_memory_derouter_params = _get_masked_values(
                in_memory_derouter_params_dict,
                unmasked_length=4,
                number_of_asterisks=4,
            )
            masked_in_memory_derouter_params_typed = (
                BaseLitellmParams(**masked_in_memory_derouter_params) if masked_in_memory_derouter_params else None
            )
            guardrail_configs.append(
                GuardrailInfoResponse(
                    guardrail_id=guardrail.get("guardrail_id"),
                    guardrail_name=guardrail.get("guardrail_name"),
                    derouter_params=masked_in_memory_derouter_params_typed,
...
class RegisterGuardrailRequest(BaseModel):
    ...
    guardrail_name: str
    derouter_params: dict[str, object]  # guardrail, mode, api_base required; api_key, headers, etc. optional
    guardrail_info: dict[str, object] | None = None
    team_id: str | None = None

    def get_derouter_params_dict(self) -> dict[str, Any]:
        return dict(self.derouter_params)
...
class GuardrailSubmissionItem(BaseModel):
    ...
    derouter_params: dict[str, object] | None = None
    guardrail_info: dict[str, object] | None = None
...
async def register_guardrail(
    ...
    [Generic Guardrail API](https://router.delify.vn/docs/adding_provider/generic_guardrail_api) format.
    ...
    params: Final = request.get_derouter_params_dict()
    if params.get("guardrail") != GENERIC_GUARDRAIL_API:
        raise HTTPException(
            status_code=400,
            detail=f"Only guardrails with derouter_params.guardrail={GENERIC_GUARDRAIL_API!r} are accepted for registration",
        )
    api_base: Final = params.get("api_base")
    if not api_base:
        raise HTTPException(
            status_code=400,
            detail="derouter_params.api_base is required for generic_guardrail_api",
        )
    parsed: Final = urlparse(api_base)
    if parsed.scheme not in ("http", "https"):
        raise HTTPException(
            status_code=400,
            detail="derouter_params.api_base must use http or https scheme",
        )
    if not parsed.hostname:
        raise HTTPException(
            status_code=400,
            detail="derouter_params.api_base must contain a valid hostname",
        )
    mode: Final = params.get("mode")
    if mode is None:
        raise HTTPException(
            status_code=400,
            detail="derouter_params.mode is required (e.g. pre_call, post_call)",
        )
    ...
    derouter_params_str: Final = safe_dumps(params)
    ...
    try:
        created: Final = await _create_guardrail_row(
            ...
            data={
                ...
                "derouter_params": derouter_params_str,
...
def _row_to_submission_item(row: "DeRouter_GuardrailsTable") -> GuardrailSubmissionItem:
    from derouter.derouter_core_utils.derouter_logging import _get_masked_values

    guardrail_info: Final = _parse_json_field(row.guardrail_info) or {}
    team_guardrail: Final = row.team_id is not None
    raw_params: Final = _parse_json_field(row.derouter_params) or {}
...
async def approve_guardrail_submission(
    ...
    try:
        ...
        derouter_params: Final = _parse_json_field(row.derouter_params)
        guardrail_info: Final = _parse_json_field(row.guardrail_info)
        if not derouter_params:
            raise HTTPException(
                status_code=500,
                detail="Guardrail derouter_params is missing or invalid",
            )
...
async def patch_guardrail(
    ...
    try:
        ...
        # Update derouter_params if default_on is provided or pii_entities_config is provided
        existing_derouter_params: Final = _as_str_object_mapping(dict(existing_guardrail.get("derouter_params", {})))
        derouter_params = LitellmParams(**existing_derouter_params)
        if request.derouter_params is not None:
            requested_derouter_params: Final = request.derouter_params.model_dump(exclude_unset=True)
            derouter_params_dict: Final = derouter_params.model_dump(exclude_unset=True)
            derouter_params_dict.update(requested_derouter_params)
            merged_derouter_params: Final = _as_str_object_mapping(derouter_params_dict)
            try:
                derouter_params = LitellmParams(**merged_derouter_params)
...
async def get_guardrail_info(guardrail_id: str):
    ...
    from derouter.proxy.guardrails.guardrail_registry import IN_MEMORY_GUARDRAIL_HANDLER
    from derouter.proxy.proxy_server import prisma_client
    from derouter.types.guardrails import GUARDRAIL_DEFINITION_LOCATION
    ...
    try:
        ...
        derouter_params: Final[LitellmParams | dict | None] = result.get("derouter_params")
        result_derouter_params_dict: Final = (
            derouter_params.model_dump(exclude_none=True)
            if isinstance(derouter_params, LitellmParams)
            else derouter_params
        ) or {}
        masked_derouter_params_dict: Final = _get_masked_values(
            result_derouter_params_dict,
            unmasked_length=4,
            number_of_asterisks=4,
        )
        masked_derouter_params = BaseLitellmParams(**masked_derouter_params_dict) if masked_derouter_params_dict else None
...
async def get_guardrail_ui_settings():
    ...
    """
    from derouter.proxy.guardrails.guardrail_hooks.derouter_content_filter.patterns import (
        PATTERN_CATEGORIES,
        get_available_content_categories,
        get_pattern_metadata,
    )
    from derouter.proxy.guardrails.guardrail_registry import guardrail_class_registry
...
async def test_custom_code_guardrail(
    ...
    👉 [Custom Code Guardrail docs](https://router.delify.vn/docs/proxy/guardrails/custom_code_guardrail)
...
def _patch_logging_obj_for_guardrail(derouter_logging_obj: _GuardrailLoggingObj, request: ApplyGuardrailRequest) -> None:
    """Configure the logging object so Langfuse/OTEL extract input and output correctly."""
    derouter_logging_obj.call_type = "pass_through_endpoint"
    derouter_logging_obj.model_call_details["call_type"] = "pass_through_endpoint"
    derouter_logging_obj.update_messages(
...
async def _emit_guardrail_success_logs(
    ...
    derouter_logging_obj: _GuardrailLoggingObj | None,
    data: dict,
    user_api_key_dict: UserAPIKeyAuth,
    response: ApplyGuardrailResponse,
    start_time: datetime,
) -> ApplyGuardrailResponse:
    """Fire proxy and DeRouter success hooks after a successful guardrail run.
    ...
    """
    from derouter.derouter_core_utils.thread_pool_executor import (
    ...
    if derouter_logging_obj is not None:
        ...
        try:
            thread_pool_executor.submit(
                derouter_logging_obj.success_handler,
...
async def apply_guardrail(
    ...
    from derouter.proxy.common_utils.registry_read_through import (
    ...
    try:
        ...
        if active_guardrail is None:
            raise HTTPException(
                status_code=404,
                detail=f"Guardrail '{request.guardrail_name}' not found. Please ensure the guardrail is configured in your DeRouter proxy.",
            )
        ...
        if derouter_logging_obj is not None:
            _patch_logging_obj_for_guardrail(derouter_logging_obj, request)
        ...
        if derouter_logging_obj is not None and not isinstance(e, HTTPException):
            try:
            ...
            try:
                thread_pool_executor.submit(
                    derouter_logging_obj.failure_handler,
                    e,
