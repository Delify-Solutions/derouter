...
def assemble_anthropic_sse_stream(
    ...
    try:
        assembled: Final = AnthropicPassthroughLoggingHandler._build_complete_streaming_response(  # pyright: ignore[reportPrivateUsage]  # the only SSE-to-ModelResponse assembler; reimplementing it here would fork the parser
            ...
            derouter_logging_obj=None,  # pyright: ignore[reportArgumentType]  # only forwarded to stream_chunk_builder, which accepts None
            model=model if isinstance(model, str) else "",
        )
    except Exception:  # noqa: BLE001  # stream_chunk_builder re-raises every assembly failure as derouter.APIError
        return None
...
def anthropic_sse_chunks_from_response(assembled: ModelResponse) -> tuple[bytes, ...]:
    ...
    anthropic_response: Final = DeRouterAnthropicMessagesAdapter().translate_openai_response_to_anthropic(
        response=assembled
    )
