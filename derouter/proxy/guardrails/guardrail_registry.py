# derouter/proxy/guardrails/guardrail_registry.py
...
from derouter.llms.base_llm.guardrail_translation.utils import (
...
from derouter.proxy.guardrails.guardrail_hooks.grayswan import (
    GraySwanGuardrail,
)
from derouter.proxy.guardrails.guardrail_hooks.grayswan import (
...
def _guardrail_table(prisma_client: PrismaClient) -> "TableActions[prisma_models.DeRouter_GuardrailsTable]":
...
def get_guardrail_initializer_from_hooks():
    ...
    try:
        ...
        for item in os.listdir(hooks_dir):
            ...
            module_path = f"derouter.proxy.guardrails.guardrail_hooks.{item}"
            try:
                # Import the module
...
class GuardrailRegistry:
    ...
    def get_initialized_guardrail_callback(self, guardrail_name: str) -> CustomGuardrail | None:
        ...
        """
        active_guardrails = derouter.logging_callback_manager.get_custom_loggers_for_type(callback_type=CustomGuardrail)
    ...
    async def add_guardrail_to_db(self, guardrail: Guardrail, prisma_client: PrismaClient):
        ...
        try:
            ...
            derouter_params_obj: Final = guardrail.get("derouter_params", {})
            if hasattr(derouter_params_obj, "model_dump"):
                derouter_params_dict = derouter_params_obj.model_dump()
            else:
                derouter_params_dict = dict(derouter_params_obj) if derouter_params_obj else {}
            derouter_params: Final[str] = safe_dumps(derouter_params_dict)
...
def _apply_configured_bool_overrides(instance: CustomGuardrail, derouter_params: LitellmParams) -> None:
    """Override the parallel/raw-scan flags only when ``derouter_params`` explicitly
    ...
    if derouter_params.run_in_parallel is not None:
        instance.run_in_parallel = bool(derouter_params.run_in_parallel)
    if derouter_params.scan_raw_request is not None:
        instance.scan_raw_request = bool(derouter_params.scan_raw_request)
...
def _configure_callback_scoping(
    custom_guardrail_callback: CustomGuardrail, guardrail_name: str, derouter_params: LitellmParams
) -> None:
    for scoping_param in (
        ...
        setattr(custom_guardrail_callback, scoping_param, getattr(derouter_params, scoping_param, None))
    ...
    _apply_configured_bool_overrides(custom_guardrail_callback, derouter_params)
...
class InMemoryGuardrailHandler:
    ...
    def initialize_guardrail(
        ...
        """
        Initialize a guardrail from a dictionary and add it to the derouter callback manager
        ...
        derouter_params_data: Final = guardrail["derouter_params"]
        verbose_proxy_logger.debug("derouter_params= %s", derouter_params_data)

        if isinstance(derouter_params_data, dict):
            derouter_params = LitellmParams(**derouter_params_data)
        else:
            derouter_params = derouter_params_data

        if "category_thresholds" in derouter_params_data and derouter_params_data["category_thresholds"]:
            lakera_category_thresholds: Final = LakeraCategoryThresholds(**derouter_params_data["category_thresholds"])
            derouter_params.category_thresholds = lakera_category_thresholds

        if derouter_params.api_key and derouter_params.api_key.startswith("os.environ/"):
            derouter_params.api_key = str(get_secret(derouter_params.api_key))

        if derouter_params.api_base and derouter_params.api_base.startswith("os.environ/"):
            derouter_params.api_base = str(get_secret(derouter_params.api_base))

        guardrail_type: Final = derouter_params.guardrail
        ...
        for custom_guardrail_callback in created_callbacks:
            _configure_callback_scoping(custom_guardrail_callback, guardrail["guardrail_name"], derouter_params)
    ...
    def _create_callbacks(
        ...
        if initializer:
            ...
            if "llm_router" in sig.parameters:
                return _as_callback_tuple(initializer(derouter_params, guardrail, llm_router))
            return _as_callback_tuple(initializer(derouter_params, guardrail))
    ...
    def initialize_custom_guardrail(
        ...
        This initializes it by adding it to the derouter callback manager
        """
        ...
        mode: Final = derouter_params.mode
        if mode is None:
            raise ValueError(
                f"mode is required for guardrail {guardrail_type} please set mode to one of the following: {', '.join(GuardrailEventHooks)}"
            )

        default_on: Final = derouter_params.default_on

        # Extract additional params from derouter_params to pass to custom guardrail
        ...
        if hasattr(derouter_params, "model_dump"):
            extra_params = derouter_params.model_dump(exclude_none=True)
        else:
            extra_params = dict(derouter_params) if derouter_params else {}
        ...
        derouter.logging_callback_manager.add_derouter_callback(_guardrail_callback)
    ...
    def update_in_memory_guardrail(
        ...
        """
        Update a guardrail in memory: a changed name or derouter_params rebuilds the
    ...
    def delete_in_memory_guardrail(self, guardrail_id: str) -> None:
        """
        Delete a guardrail in memory and remove from derouter callbacks.

        The callback is purged from every callback list, not just
        derouter.callbacks: request handling promotes guardrail callbacks into the
        success/failure/async lists, so removing it from only derouter.callbacks
        ...
        for custom_guardrail_callback in tracked_callbacks:
            derouter.logging_callback_manager.remove_callback_from_all_lists(custom_guardrail_callback)
    ...
    @staticmethod
    def _normalize_derouter_params_for_comparison(
        params: LitellmParams | Mapping[str, object] | None,
    ) -> Mapping[str, object] | None:
        """
        Render derouter_params to a canonical dict so an in-memory LitellmParams and
        ...
        if isinstance(params, dict):
            try:
                ...
                verbose_proxy_logger.warning(
                    "Could not normalize guardrail derouter_params for comparison; treating the guardrail as changed. Error: %s",
                    e,
                )
                return params
        return params
    ...
    def _has_guardrail_params_changed(self, guardrail_id: str, new_guardrail: Guardrail) -> bool:
        ...
        # Compare derouter_params
        existing_dict: Final = self._normalize_derouter_params_for_comparison(existing.get("derouter_params"))
        new_dict: Final = self._normalize_derouter_params_for_comparison(new_guardrail.get("derouter_params"))
        ...
        if existing_dict is not None and new_dict is not None:
            ...
            changed_fields = {"derouter_params": {"old": existing_dict, "new": new_dict}}
    ...
    def reinitialize_guardrail(
        ...
        Removes old callback from derouter.callbacks and creates fresh instance.
        ...
        # Initialize fresh (will add new callback to derouter.callbacks). If the new
...
########################################################
# In Memory Guardrail Handler for DeRouter Proxy
########################################################
