"""
Unified Guardrail, leveraging DeRouter's /applyGuardrail endpoint
...
from derouter.cost_calculator import _infer_call_type
...
from derouter.llms import get_guardrail_translation_mapping, load_guardrail_translation_mappings
...
if TYPE_CHECKING:
    ...
    # module-level cyclic import with derouter.integrations.custom_guardrail.
...
def _ensure_derouter_metadata(data: dict, user_api_key_dict: UserAPIKeyAuth) -> None:
    """Populate data['derouter_metadata'] from user_api_key_dict if absent."""
    if "derouter_metadata" not in data:
        from derouter.llms.base_llm.guardrail_translation.base_translation import (
            BaseTranslation,
        )
        ...
        if user_metadata:
            data["derouter_metadata"] = user_metadata
...
class UnifiedLLMGuardrails(CustomLogger):
    ...
    async def async_pre_call_hook(
        ...
        _ensure_derouter_metadata(data, user_api_key_dict)
    ...
    async def async_post_call_success_hook(
        ...
        # derouter.integrations.custom_guardrail.
        from derouter.integrations.custom_guardrail import ModifyResponseException
        from derouter.proxy.common_utils.callback_utils import (
            add_guardrail_to_applied_guardrails_header,
        )
        ...
        if call_type is None:
            ...
            logging_call_type: Final = (
                getattr(derouter_logging_obj, "call_type", None) if derouter_logging_obj is not None else None
            )
    ...
    async def async_post_call_streaming_iterator_hook(
        ...
        See Aim guardrail implementation for an example - https://github.com/Delify-Solutions/derouter/blob/d0e022cfacb8e9ebc5409bb652059b6fd97b45c0/derouter/proxy/guardrails/guardrail_hooks/aim.py#L168
