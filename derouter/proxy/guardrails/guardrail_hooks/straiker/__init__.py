...
def _get_config_value(derouter_params: "LitellmParams", optional_params: object, attribute_name: str) -> object:
...
def initialize_guardrail(derouter_params: "LitellmParams", guardrail: "Guardrail"):
    ...
    api_base: Final = derouter_params.api_base or "https://api.prod.straiker.ai"
    default_app: Final = getattr(derouter_params, "default_app", None) or getattr(derouter_params, "source", None)
    source: Final = default_app if isinstance(default_app, str) and default_app else "DeRouter Gateway"
    kwargs: Final[dict[str, object]] = {
        field: value
        for field in _OPTIONAL_INIT_FIELDS
        for value in [_get_config_value(derouter_params, optional_params, field)]
        if value is not None
    }
