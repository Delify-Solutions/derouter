...
from derouter.types.proxy.guardrails.guardrail_hooks.straiker import (
...
def _merged_metadata(request_data: dict) -> dict:
    return {
        ...
        **_as_dict(request_data.get("derouter_metadata")),
    }
...
def _extract_identity(request_data: dict) -> StraikerWebhookIdentity:
    ...
    return StraikerWebhookIdentity(
        derouter_key=_as_optional_str(meta.get("user_api_key_alias"))
        or _as_optional_str(meta.get("user_api_key_hash"))
        or _as_optional_str(meta.get("user_api_key_token")),
        derouter_team=_as_optional_str(meta.get("user_api_key_team_alias"))
        or _as_optional_str(meta.get("user_api_key_team_id")),
        derouter_user_id=_as_optional_str(meta.get("user_api_key_user_id")),
        derouter_user_email=_as_optional_str(meta.get("user_api_key_user_email")),
        derouter_org_id=_as_optional_str(meta.get("user_api_key_org_id")),
...
def _resolve_provider(request_data: dict, model: str | None) -> str | None:
    derouter_params: Final = _as_dict(request_data.get("derouter_params"))
    custom_llm_provider: Final = request_data.get("custom_llm_provider") or derouter_params.get("custom_llm_provider")
    ...
    try:
        _, provider, _, _ = get_llm_provider(
            model=model,
            api_base=request_data.get("api_base") or derouter_params.get("api_base"),
            api_key=request_data.get("api_key") or derouter_params.get("api_key"),
        )
...
def _resolve_destination(request_data: dict) -> str | None:
    derouter_params: Final = _as_dict(request_data.get("derouter_params"))
    api_base: Final = request_data.get("api_base") or derouter_params.get("api_base")
...
def _route_has_translation(request_data: dict) -> bool:
    ...
    route: Final = _as_dict(request_data.get("derouter_metadata")).get("user_api_key_request_route")
...
def _resolve_call_surface(logging_obj: DeRouterLoggingObj | None, request_data: dict) -> str:
    call_type: Final = (
        (getattr(logging_obj, "call_type", None) if logging_obj is not None else None)
        or request_data.get("call_type")
        or request_data.get("derouter_call_type")
    )
...
class StraikerGuardrail(CustomGuardrail):
    ...
    def _headers(self) -> dict[str, str]:
        ...
        return {
            ...
            "X-Straiker-Webhook-Format": "derouter",
            **extra,
        }
    ...
    def _build_context(
        ...
        return StraikerWebhookContext(
            ...
            derouter_call_id=getattr(logging_obj, "derouter_call_id", None) if logging_obj else None,
            derouter_trace_id=getattr(logging_obj, "derouter_trace_id", None) if logging_obj else None,
    ...
    def _build_envelope(
        ...
        *,
        ...
        call_id: Final = getattr(logging_obj, "derouter_call_id", None) if logging_obj else None
        event_id: Final = f"{call_id or 'derouter'}:{input_type}"
