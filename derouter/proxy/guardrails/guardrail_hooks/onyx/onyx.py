...
class OnyxGuardrail(CustomGuardrail):
    ...
    async def _validate_with_guard_server(
        ...
        response: Final = await self.async_handler.post(
            f"{self.api_base}/guard/evaluate/v1/{self.api_key}/derouter",
            json={
                "payload": payload,
    ...
    async def apply_guardrail(
        ...
        conversation_id: Final = logging_obj.derouter_call_id if logging_obj else str(uuid.uuid4())
    ...
    def get_config_model() -> type["GuardrailConfigModel"] | None:
        from derouter.types.proxy.guardrails.guardrail_hooks.onyx import (
