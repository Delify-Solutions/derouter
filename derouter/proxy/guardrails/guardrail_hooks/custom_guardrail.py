...
class myCustomGuardrail(CustomGuardrail):
    ...
    async def async_pre_call_hook(
        ...
        # In this guardrail, if a user inputs `derouter` we will mask it and then send it to the LLM
        _messages: Final = data.get("messages")
        if _messages:
            for message in _messages:
                _content = message.get("content")
                if isinstance(_content, str):
                    if "derouter" in _content.lower():
                        _content = _content.replace("derouter", "********")
    ...
    async def async_moderation_hook(
        ...
        # In this guardrail, if a user inputs `derouter` we will mask it.
        _messages: Final = data.get("messages")
        if _messages:
            for message in _messages:
                _content = message.get("content")
                if isinstance(_content, str):
                    if "derouter" in _content.lower():
                        raise ValueError("Guardrail failed words - `derouter` detected")
