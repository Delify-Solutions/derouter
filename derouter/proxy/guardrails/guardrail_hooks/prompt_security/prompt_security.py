...
class PromptSecurityGuardrail(CustomGuardrail):
    ...
    async def apply_guardrail(
        ...
        This method is called by DeRouter's guardrail framework for ALL endpoints:
        - /chat/completions
        - /responses
        - /messages (Anthropic)
        - /embeddings
    ...
    def _resolve_key_alias_from_request_data(request_data: dict) -> str | None:
        ...
        # Check derouter_metadata first (set by guardrail framework)
        derouter_metadata: Final = request_data.get("derouter_metadata", {})
        if derouter_metadata:
            alias = derouter_metadata.get("user_api_key_alias")
            if alias:
                return alias
    ...
    async def sanitize_file_content(
        ...
        except (asyncio.TimeoutError, httpx.TimeoutException, DeRouterTimeout) as exc:
    ...
    async def _sanitize_file_content(
        ...
        if user_api_key_alias:
            headers["X-DeRouter-Key-Alias"] = user_api_key_alias
    ...
    def get_config_model() -> type["GuardrailConfigModel"] | None:
        from derouter.types.proxy.guardrails.guardrail_hooks.prompt_security import (
