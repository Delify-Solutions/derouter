...
_EVALUATE_PATH: Final = "/v2/evaluate/derouter"
...
# again under ["metadata"]["headers"] / ["derouter_metadata"]["headers"], and again under
# ["metadata"]["requester_metadata"]["headers"], any of which can carry an Authorization or
# x-api-key value. DeRouter's own spend-log sanitizer excludes `secret_fields` for the same reason
...
class AliceVerdict(TypedDict):
    """Body returned by Alice's DeRouter evaluate endpoint."""
...
class AliceGuardrail(CustomGuardrail):
    ...
    DeRouter upgrade. A batch with nothing selectable at all (no `texts`, `images`, `tools`,
    ...
    several, so the application is named on the virtual key rather than in this config:
        ...
        curl $PROXY/key/generate -H "Authorization: Bearer $DEROUTER_MASTER_KEY" \\
    ...
    def get_config_model() -> type | None:
        from derouter.types.proxy.guardrails.guardrail_hooks.alice import (
