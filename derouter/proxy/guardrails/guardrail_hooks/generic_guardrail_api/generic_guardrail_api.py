...
from derouter.types.llms.openai import ChatCompletionToolParam
...
# Headers whose values are forwarded as-is (case-insensitive). Glob patterns supported (e.g. x-stainless-*, x-derouter*).
...
def _sanitize_inbound_headers(
    ...
    - Allowlist: default allowlist + extra_allowlist (from derouter_params.extra_headers); only these have values forwarded.
...
def _extract_inbound_headers(
    ...
    - metadata headers captured in derouter_pre_call_utils
    ...
    derouter_metadata_headers: Final = (request_data.get("derouter_metadata") or {}).get("headers")
    if derouter_metadata_headers:
        return _sanitize_inbound_headers(derouter_metadata_headers, extra_allowlist=extra_allowlist)
    ...
    if logging_obj and getattr(logging_obj, "model_call_details", None):
        try:
            ...
            headers = details.get("derouter_params", {}).get("metadata", {}).get("headers", None)
            if headers:
                return _sanitize_inbound_headers(headers, extra_allowlist=extra_allowlist)
        except Exception:
            pass
...
class GenericGuardrailAPI(CustomGuardrail):
    """
    Generic Guardrail API integration for DeRouter.

    This integration allows you to use any guardrail API that follows the
    DeRouter Basic Guardrail API spec without needing to write custom integration code.
        ...
        **kwargs,
        ...
        if not base_url:
            raise ValueError(
                ...
                "Set GENERIC_GUARDRAIL_API_BASE environment variable or pass it in derouter_params"
            )

        # Append the endpoint path if not already present
        if not base_url.endswith("/beta/derouter_basic_guardrail_api"):
            base_url = base_url.rstrip("/")
            self.api_base = f"{base_url}/beta/derouter_basic_guardrail_api"
        else:
    ...
    def _extract_user_api_key_metadata(self, request_data: dict) -> GenericGuardrailAPIMetadata:
        ...
        Args:
            request_data: Request data dictionary that may contain:
                ...
                - derouter_metadata (for output responses) with user_api_key_* fields
        ...
        # 1. For output responses: derouter_metadata (set by handlers with prefixed keys)
        # 2. For input requests: metadata (already present in request_data with prefixed keys)
        derouter_metadata: Final = request_data.get("derouter_metadata", {})
        top_level_metadata: Final = request_data.get("metadata", {})

        # Merge both sources, preferring derouter_metadata if both exist
        metadata_dict: Final = {**top_level_metadata, **derouter_metadata}
    ...
    def _fail_open_passthrough(
        ...
        *,
        ...
        verbose_proxy_logger.critical(
            ...
            "guardrail_name=%s api_base=%s input_type=%s derouter_call_id=%s derouter_trace_id=%s",
            status_suffix,
            getattr(self, "guardrail_name", None),
            getattr(self, "api_base", None),
            input_type,
            getattr(logging_obj, "derouter_call_id", None) if logging_obj else None,
            getattr(logging_obj, "derouter_trace_id", None) if logging_obj else None,
            exc_info=error,
        )
    ...
    async def apply_guardrail(
        ...
        try:
            ...
            guardrail_request: Final = GenericGuardrailAPIRequest(
                derouter_call_id=logging_obj.derouter_call_id if logging_obj else None,
                derouter_trace_id=logging_obj.derouter_trace_id if logging_obj else None,
                texts=texts,
