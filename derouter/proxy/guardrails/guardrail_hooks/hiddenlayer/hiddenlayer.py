...
from derouter.types.llms.openai import ChatCompletionToolCallChunk
from derouter.types.proxy.guardrails.guardrail_hooks.hiddenlayer import (
    HiddenlayerAction,
    HiddenlayerMessages,
)
from derouter.types.utils import ChatCompletionMessageToolCall, GenericGuardrailAPIInputs
...
def _logged_request_headers(logging_obj: _LoggedCallDetails) -> dict[str, str]:
    return logging_obj.model_call_details.get("derouter_params", {}).get("metadata", {}).get("headers", {})
...
class HiddenlayerGuardrail(CustomGuardrail):
    ...
    async def apply_guardrail(
        ...
        hl_request_metadata["requester_id"] = headers.get("hl-requester-id") or "DeRouter"
    ...
    async def _call_hiddenlayer(
        ...
        headers: Final = {
            ...
            "hl-runtime-edge-provider": "derouter",
...
class HiddenlayerGuardrailV2(CustomGuardrail):
    ...
    async def apply_guardrail(
        ...
        if "hl-requester-id" not in hl_headers:
            hl_headers["hl-requester-id"] = "DeRouter"
