"""
MCPJWTSigner — Built-in DeRouter guardrail for zero trust MCP authentication.

Signs outbound MCP requests with a DeRouter-issued RS256 JWT so that MCP servers
    ...
    guardrails:
      - guardrail_name: "mcp-jwt-signer"
        derouter_params:
          ...
          # Core signing config
          issuer: "https://my-derouter.example.com"   # optional
          ...
          # Supported: token:<claim>, derouter:user_id, derouter:email,
          #            derouter:end_user_id, derouter:team_id
          ...
          # FR-9: Debug headers
          debug_headers: false    # emit x-derouter-mcp-debug header when true
...
from derouter.types.guardrail_base_init import GuardrailBaseInitKwargs
...
class MCPJWTSigner(CustomGuardrail):
    """
    Built-in DeRouter guardrail that signs outbound MCP requests with a
    DeRouter-issued RS256 JWT, enabling zero trust authentication.
    ...
    The signed JWT carries:
      - iss: DeRouter issuer identifier
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # --- Core config ---
        self.issuer: str = (
            issuer or os.environ.get("MCP_JWT_ISSUER") or os.environ.get("DEROUTER_EXTERNAL_URL") or "derouter"
        )
        ...
        # --- FR-12: End-user identity mapping ---
        # Default chain: try incoming JWT sub, fall back to derouter user_id
    ...
    def _resolve_end_user_identity(
        ...
        Supported source prefixes:
          ...
          derouter:user_id     — from UserAPIKeyAuth.user_id
          derouter:email       — from UserAPIKeyAuth.user_email
          derouter:end_user_id — from UserAPIKeyAuth.end_user_id
          derouter:team_id     — from UserAPIKeyAuth.team_id
    ...
    def _build_claims(
        ...
        Args:
            user_api_key_dict: DeRouter auth context for the current request.
            data: Pre-call hook data dict (contains mcp_tool_name etc.).
            jwt_claims: Verified incoming IdP claims (FR-5), or DeRouter-decoded
        ...
        # email passthrough when available from DeRouter context
        ...
        act_sub: Final = team_id or org_id or "derouter-proxy"
    ...
    def _build_debug_header(claims: _DebugHeaderClaims, kid: str) -> str:
        """
        Build the x-derouter-mcp-debug header value.
    ...
    async def async_pre_call_hook(
        ...
        # Fall back to DeRouter-decoded JWT claims (available when proxy uses JWT auth).
        ...
        if self.debug_headers:
            ...
            new_headers["x-derouter-mcp-debug"] = self._build_debug_header(debug_claims, self._kid)
...
async def inject_mcp_jwt_headers_for_upstream(
    ...
    *,
    ...
    try:
        from derouter.proxy.proxy_server import (  # noqa: PLC0415
