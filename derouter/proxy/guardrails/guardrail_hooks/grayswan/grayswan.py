...
class GraySwanGuardrail(CustomGuardrail):
    ...
    which automatically works with all DeRouter endpoints:
    ...
    def _extract_inbound_headers(
        ...
        if not headers and logging_obj and getattr(logging_obj, "model_call_details", None):
            headers = (
                (logging_obj.model_call_details or {}).get("derouter_params", {}).get("metadata", {}).get("headers")
            )
    ...
    def _prepare_payload(
        ...
        cleaned_derouter_metadata: Final = dict(derouter_metadata) if isinstance(derouter_metadata, dict) else {}
        if inbound_headers:
            existing_headers: Final = cleaned_derouter_metadata.get("headers")
            cleaned_derouter_metadata["headers"] = (
                {**existing_headers, **inbound_headers} if isinstance(existing_headers, dict) else inbound_headers
            )
        if cleaned_derouter_metadata:
            sanitized: Final[object] = safe_json_loads(safe_dumps(cleaned_derouter_metadata), default={})
            if isinstance(sanitized, dict) and sanitized:
                payload["derouter_metadata"] = sanitized
