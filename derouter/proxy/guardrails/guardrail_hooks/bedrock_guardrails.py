...
from derouter.derouter_core_utils.api_route_to_call_types import get_call_types_for_route
from derouter.derouter_core_utils.core_helpers import redact_nested_match_and_regex_keys
...
from derouter.llms.anthropic.chat.guardrail_translation.handler import AnthropicMessagesHandler
from derouter.llms.base_llm.guardrail_translation.utils import (
    effective_scan_only_tool_results_for_guardrail,
)
from derouter.llms.bedrock.base_aws_llm import BaseAWSLLM, bedrock_bearer_token
from derouter.llms.custom_httpx.http_handler import (
    get_async_httpx_client,
    httpxSpecialProvider,
)
from derouter.proxy._types import UserAPIKeyAuth
from derouter.proxy.common_request_processing import serialize_http_exception_detail
from derouter.proxy.common_utils.sse_keepalive import keepalive_ping_has_fired
from derouter.proxy.guardrails.anthropic_sse import (
...
from derouter.types.llms.openai import AllMessageValues, ChatCompletionUserMessage
...
from derouter.types.utils import GenericGuardrailAPIInputs
...
class BedrockGuardrail(CustomGuardrail, BaseAWSLLM):
    ...
    def update_in_memory_derouter_params(self, derouter_params: LitellmParams) -> None:
        super().update_in_memory_derouter_params(derouter_params)
        self._set_streaming_params(BedrockGuardrailStreamingParams.from_extras(derouter_params.model_extra))
    ...
    def _build_response_content_items(self, response: object, has_grounding: bool) -> list[BedrockContentItem]:
        ...
        if not isinstance(response, derouter.ModelResponse):
            return items
        for choice in response.choices:
            if (
                isinstance(choice, derouter.Choices)
    ...
    def convert_to_bedrock_format(
        ...
        """
        Convert the derouter messages/response to the bedrock request format.
    ...
    def _select_messages_for_apply_guardrail(
        ...
        (https://github.com/Delify-Solutions/derouter/issues/23476).
        """
    ...
    async def make_bedrock_api_request(
        ...
        source: Literal["INPUT", "OUTPUT"],
        messages: list[AllMessageValues] | None = None,
        response: derouter.ModelResponse | None = None,
        ...
        if isinstance(assembled_model_response, ModelResponse):
            ...
            try:
                ...
                headers_flushed: Final = keepalive_ping_has_fired(
                    time.monotonic() - started_at, derouter.anthropic_sse_ping_interval_seconds
                )
    ...
    def _apply_masking_to_model_response(self, response: derouter.ModelResponse, masked_texts: list[str]) -> None:
        """
    ...
    def _incremental_scan_cache() -> DualCache:
        ...
        try:
            from derouter.proxy.proxy_server import proxy_logging_obj as _proxy_logging
