...
from derouter.types.llms.openai import AllMessageValues, OpenAIChatCompletionToolParam
from derouter.types.proxy.guardrails.guardrail_hooks.crowdstrike_aidr import (
...
def _merge_metadata_bags(request_data: Mapping[str, Any]) -> Mapping[str, Any] | None:
    ...
    present = False
    for bag in (request_data.get("metadata"), request_data.get("derouter_metadata")):
...
def streaming_params_from_derouter_params(
    derouter_params: LitellmParams,
) -> CrowdStrikeAIDRGuardrailConfigModelOptionalParams:
    extras: Final[Mapping[str, object]] = derouter_params.model_extra or {}
    nested: Final = derouter_params.optional_params
...
class CrowdStrikeAIDRHandler(CustomGuardrail):
        ...
        **kwargs,
        ...
        if not self.api_key:
            raise CrowdStrikeAIDRGuardrailMissingSecrets(
                "CrowdStrike AIDR API Key not found. Set CS_AIDR_TOKEN environment variable or pass it in derouter_params."
            )

        self.api_base = api_base or os.environ.get("CS_AIDR_BASE_URL")
        if not self.api_base:
            raise CrowdStrikeAIDRGuardrailMissingSecrets(
                "CrowdStrike AIDR API base URL is required. Set CS_AIDR_BASE_URL environment variable or pass it in derouter_params."
            )
    ...
    def update_in_memory_derouter_params(self, derouter_params: LitellmParams) -> None:
        ...
        self._set_streaming_params(streaming_params_from_derouter_params(derouter_params))
            ...
            if transformed_signal:
                raise HTTPException(
                    ...
                    detail={  # mutable-ok: one-shot HTTPException detail payload, never mutated after construction
                        "error": "CrowdStrike AIDR returned a transformed response derouter could not parse; "
