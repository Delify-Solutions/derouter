#!/usr/bin/env python3
"""
Azure Prompt Shield Native Guardrail Integrationfor DeRouter
"""
...
if TYPE_CHECKING:
    ...
    from derouter.types.proxy.guardrails.guardrail_hooks.azure.azure_prompt_shield import (
...
def _updated_param(derouter_params: "LitellmParams | dict", key: str) -> object:  # mutable-ok: DB dict
    ...
    if isinstance(derouter_params, Mapping):
        return derouter_params.get(key)
    return getattr(derouter_params, key, None)
...
class AzureContentSafetyPromptShieldGuardrail(AzureGuardrailBase, CustomGuardrail):
    """
    DeRouter Built-in Guardrail for Azure Content Safety Guardrail (Prompt Shield).
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Plain (non-Final) attributes: ``update_in_memory_derouter_params``
    ...
    def update_in_memory_derouter_params(self, derouter_params: "LitellmParams | dict") -> None:  # mutable-ok: DB dict
        ...
        """
        cost_tier: Final = _resolved_cost_tier(_updated_param(derouter_params, "cost_tier"))
        price: Final = _resolved_price(_updated_param(derouter_params, "price_per_1000_text_records"), cost_tier)
        resolved_credentials: dict[str, object] = {}  # mutable-ok: staged before mutation
        for cred_key in ("api_key", "api_base"):
            cred_value = _updated_param(derouter_params, cred_key)
