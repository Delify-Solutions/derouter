...
# |   https://github.com/Delify-Solutions/derouter/issues/  |
# +-----------------------------------------------+
#
...
from derouter.types.proxy.guardrails.guardrail_hooks.presidio import (
    PresidioAnalyzeRequest,
    PresidioAnalyzeResponseItem,
)
from derouter.types.utils import GuardrailStatus, StreamingChoices
...
class _OPTIONAL_PresidioPIIMasking(CustomGuardrail):
    ...
    def validate_environment(
        ...
        self.presidio_anonymizer_api_base: str | None = presidio_anonymizer_api_base or derouter.get_secret(
    ...
    async def async_post_call_success_hook(
        ...
        if self.output_parse_pii is False and derouter.output_parse_pii is False:
            return response
    ...
    def update_in_memory_derouter_params(self, derouter_params: LitellmParams) -> None:
        ...
        if derouter_params.pii_entities_config:
            self.pii_entities_config = derouter_params.pii_entities_config
        if derouter_params.presidio_score_thresholds:
            self.presidio_score_thresholds = derouter_params.presidio_score_thresholds
        if derouter_params.presidio_entities_deny_list:
            self.presidio_entities_deny_list = derouter_params.presidio_entities_deny_list
        if derouter_params.presidio_analyze_chunk_size_bytes is not None:
            ...
            self.presidio_analyze_chunk_size_bytes = self._coerce_analyze_chunk_size(
                derouter_params.presidio_analyze_chunk_size_bytes
            )
