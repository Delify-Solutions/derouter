...
if TYPE_CHECKING:
    ...
    from derouter.types.proxy.guardrails.guardrail_hooks.base import (
...
_SOURCE_TAG: Final = "integration:derouter"
...
def _resolve_call_id(logging_obj: object) -> str | None:
    ...
    This value ultimately derives from the client-settable ``x-derouter-call-id``
...
def _caller_scope(logging_obj: object) -> str:
    ...
    server-side (``metadata.user_api_key_auth``, derouter_pre_call_utils). Its
    ...
    ``derouter_metadata``), but the bare ``user_api_key`` *string* is never
    ...
    if not _is_str_object_dict(details):
        return ""
    derouter_params: Final = details.get("derouter_params")
    for container in (derouter_params, details):
...
def _scoped_store_key(logging_obj: object) -> str | None:
    ...
    ``x-derouter-call-id`` header and is echoed back in responses, so one caller
...
class CompresrGuardrail(CustomGuardrail):
    ...
    async def _call_compress(
        ...
        try:
            ...
            return None
        except (httpx.RequestError, derouter.Timeout) as e:
    ...
    def _sanitized_follow_up_kwargs(self, kwargs: dict) -> dict[str, object]:
        ...
        out: Final[dict[str, object]] = {
            k: v for k, v in kwargs.items() if not k.startswith("_compresr") and k != "derouter_logging_obj"
        }
