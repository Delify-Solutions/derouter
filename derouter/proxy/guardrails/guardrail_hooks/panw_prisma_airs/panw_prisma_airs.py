#!/usr/bin/env python3
"""
Palo Alto Networks Prisma AI Runtime Security (AIRS) Guardrail Integration for DeRouter
...
class PanwPrismaAirsHandler(CustomGuardrail):
    """
    DeRouter Built-in Guardrail for Palo Alto Networks Prisma AI Runtime Security (AIRS).
    ...
    Configuration:
        ...
        app_name: Application name for tracking in Prisma AIRS analytics (default: "DeRouter")
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Handle app_name: Default to "DeRouter", or prefix user's app_name with "DeRouter-"
        if app_name:
            self.app_name = f"DeRouter-{app_name}"
        else:
            self.app_name = "DeRouter"
    ...
    async def _call_panw_api(
        ...
        # Handle app_name: DeRouter by default, or DeRouter-{user_app_name} if user provides one
        user_app_name: Final = metadata.get("app_name") if metadata else None
        if user_app_name:
            app_name_value = f"DeRouter-{user_app_name}"
        else:
            app_name_value = self.app_name  # Defaults to "DeRouter"

        panw_metadata: Final[dict[str, object]] = {
            "app_user": (
                (metadata.get("app_user") or metadata.get("user") or "derouter_user") if metadata else "derouter_user"
            ),
            "ai_model": metadata.get("model", "unknown") if metadata else "unknown",
            "app_name": app_name_value,
            "source": "derouter_builtin_guardrail",
        }

        # Include user_ip if available (from DeRouter metadata or request)
        ...
        # Forward derouter_trace_id in AIRS metadata for session correlation
        if metadata and metadata.get("derouter_trace_id"):
            panw_metadata["derouter_trace_id"] = metadata["derouter_trace_id"]
        ...
        # Use per-request derouter_call_id as AIRS tr_id; keep derouter_trace_id in metadata.
        if call_id:
        ...
        try:
            # Use DeRouter's async HTTP client
    ...
    def _prepare_metadata_from_request(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        - app_name: Application identifier (will be prefixed with "DeRouter-")
        ...
        metadata: Final = {
            "user": data.get("user") or "derouter_user",
        ...
        # Include derouter_trace_id for session tracking.
        # Sources (checked in priority order):
        #   1. data["derouter_trace_id"]             — top-level body field
        #   2. metadata["derouter_trace_id"]         — user passes in request metadata
        #   3. metadata["trace_id"]                 — x-derouter-trace-id header
        #      (derouter_pre_call_utils stores it as "trace_id", not "derouter_trace_id")
        #   4. requester_metadata["derouter_trace_id"] — deep copy for /v1/messages routes
        trace_id: Final = (
            data.get("derouter_trace_id")
            or user_metadata.get("derouter_trace_id")
            or user_metadata.get("trace_id")
            or requester_meta.get("derouter_trace_id")
        )
        if trace_id:
            metadata["derouter_trace_id"] = trace_id
    ...
    async def _scan_raw_streaming_text(self, text: str, request_data: dict, start_time: datetime) -> None:
        ...
        scan_result: Final = await self._call_panw_api(
            ...
            is_response=True,
            metadata=metadata,
            call_id=request_data.get("derouter_call_id"),
        )
    ...
    def _check_and_mark_scanned(self, data: dict, scan_type: str) -> bool:
        ...
        """
        call_id = data.get("derouter_call_id")
        if not call_id:
            call_id = str(uuid.uuid4())
            data["derouter_call_id"] = call_id
            verbose_proxy_logger.warning(
                "PANW Prisma AIRS: derouter_call_id missing from request data, synthesized %s for %s scan deduplication",
                call_id,
                scan_type,
            )

        scan_key: Final = f"_panw_{scan_type}_scanned_{call_id}"
        derouter_metadata: Final = data.setdefault("derouter_metadata", {})

        if derouter_metadata.get(scan_key):
            verbose_proxy_logger.debug("PANW Prisma AIRS: Skipping duplicate %s-call scan", scan_type)
            return True  # Already scanned

        derouter_metadata[scan_key] = True
    ...
    async def apply_guardrail(
        ...
        # Resolve derouter_call_id: request_data first, then logging_obj fallback.
        # Post-call path reconstructs request_data as {"response": ...} without
        # derouter_call_id, but logging_obj.derouter_call_id is available.
        call_id = request_data.get("derouter_call_id")
        if not call_id and logging_obj:
            call_id = getattr(logging_obj, "derouter_call_id", None)
        if not call_id:
            ...
            if input_type == "request" and logging_obj is None and _mcp_tool:
                ...
                request_data["derouter_call_id"] = call_id
                verbose_proxy_logger.debug(
                ...
                request_data["derouter_call_id"] = call_id
                verbose_proxy_logger.warning(
                    "PANW Prisma AIRS: derouter_call_id missing from empty "
                ...
                request_data["derouter_call_id"] = call_id
                verbose_proxy_logger.warning(
                    "PANW Prisma AIRS: derouter_call_id missing, synthesized %s (input_type=%s)",
                    call_id,
                    input_type,
                )
        ...
        if logging_obj:
            _lp: Final = (getattr(logging_obj, "model_call_details", {}) or {}).get("derouter_params", {}) or {}
    ...
    def get_config_model() -> type["GuardrailConfigModel"] | None:
        from derouter.types.proxy.guardrails.guardrail_hooks.panw_prisma_airs import (
