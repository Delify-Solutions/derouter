"""Akto guardrail integration for DeRouter proxy.
...
AKTO_CONNECTOR_NAME: Final = "derouter"
DEFAULT_GUARDRAIL_TIMEOUT: Final = 5


class AktoGuardrail(CustomGuardrail):
    """DeRouter guardrail hook that validates and ingests LLM traffic via the Akto API."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not self.akto_base_url:
            raise ValueError("akto_base_url is required. Set AKTO_GUARDRAIL_API_BASE or pass it in derouter_params.")

        self.akto_api_key = akto_api_key or os.environ.get("AKTO_API_KEY", "")
        if not self.akto_api_key:
            raise ValueError("akto_api_key is required. Set AKTO_API_KEY or pass it in derouter_params.")
    ...
    def resolve_metadata_value(request_data: dict | None, key: str) -> str | None:
        """Look up a metadata value from derouter_metadata or metadata dicts."""
        if request_data is None:
            return None
        for dict_key in ("derouter_metadata", "metadata"):
    ...
    async def apply_guardrail(
        ...
        """Main entry point called by DeRouter's guardrail framework.
