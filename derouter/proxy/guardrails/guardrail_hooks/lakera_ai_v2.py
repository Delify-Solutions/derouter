...
from derouter.types.proxy.guardrails.guardrail_hooks.lakera_ai_v2 import (
    LakeraAIBreakdownItem,
    LakeraAIRequest,
    LakeraAIResponse,
)
from derouter.types.utils import CallTypesLiteral, GuardrailStatus, ModelResponse
...
def _build_lakera_inspection_messages(data: Mapping[str, object]) -> Sequence[Mapping[str, str]]:
    ...
    ``instructions`` field, placed first since derouter later converts it
...
class LakeraAIGuardrail(CustomGuardrail):
    ...
    def update_in_memory_derouter_params(self, derouter_params: LitellmParams) -> None:
        """
        The base implementation blindly ``setattr``s every field on ``derouter_params``
        ...
        The base setattr also writes ``derouter_params.mode`` onto a new ``self.mode``
        ...
        """
        new_event_hook: Final = derouter_params.mode or self.event_hook
        prospective_payload: Final = derouter_params.payload
        prospective_breakdown: Final = derouter_params.breakdown
        self._validate_advisory_config(
            on_flagged=derouter_params.on_flagged or self.on_flagged,
        ...
        super().update_in_memory_derouter_params(derouter_params=derouter_params)
