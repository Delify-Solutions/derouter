...
class VigilGuardGuardrail(CustomGuardrail):
    ...
    def _collect_metadata(
        self, request_data: dict, logging_obj: Optional["DeRouterLoggingObj"]
    ) -> Mapping[str, _MetadataValue]:
        sources: Final[list[dict]] = []
        if isinstance(request_data, dict):
            sources.append(request_data)
            for nested_key in ("metadata", "derouter_metadata"):
        ...
        if call_id:
            collected["derouter_call_id"] = call_id
    ...
    @staticmethod
    def _extract_call_id(request_data: dict, logging_obj: Optional["DeRouterLoggingObj"]) -> str | None:
        ...
        if isinstance(request_data, dict):
            ...
            if isinstance(metadata, dict):
                nested: Final = metadata.get("derouter_call_id")
