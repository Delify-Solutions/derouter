...
from derouter.compression.compress import get_protected_indices
from derouter.constants import HTTP_HANDLER_CONNECT_TIMEOUT_SECONDS
...
from derouter.proxy.guardrails.guardrail_hooks.content_text import (
    assistant_text_from_response,
    content_to_text,
    is_all_text_parts,
    merge_rewritten_text_parts,
)
from derouter.proxy.spend_tracking.compression_savings import HEADROOM_GUARDRAIL_PROVIDER
...
def _is_retrieve_tool_name(name: str | None) -> bool:
    ...
    Server-side the tool is ``headroom_retrieve``; exposed through DeRouter's MCP
...
def _retrieval_result_indices(
    ...
    (the DeRouter MCP gateway path), the client executes the call and sends the
...
def _protected_indices(
    ...
    ``get_protected_indices`` is derouter's own compression policy: the system
...
def _resolve_call_id(logging_obj: object, request_state: dict[str, object]) -> str | None:
    """Resolve the derouter_call_id shared by a request's pre-call hook and its
    ...
    logging_call_id: Final = getattr(logging_obj, "derouter_call_id", None)
    if isinstance(logging_call_id, str) and logging_call_id:
        return logging_call_id
    kwargs_call_id: Final = request_state.get("derouter_call_id")
...
class HeadroomGuardrail(CustomGuardrail):
    ...
    def update_in_memory_derouter_params(self, derouter_params: LitellmParams) -> None:
        ...
        self.timeout = self._resolve_timeout(derouter_params.timeout)
    ...
    async def _call_compress(
        ...
        except (httpx.ConnectError, httpx.TimeoutException, httpx.TransportError, derouter.Timeout) as e:
            return _CompressResult(
                self._handle_compress_failure(
                    messages,
    ...
    async def async_build_agentic_loop_plan(
        ...
        for tc in tool_calls:
            ...
            # Headroom /v1/compress call, scoped by derouter_call_id. Scoping by
        ...
        return AgenticLoopPlan(
            ...
            request_patch=AgenticLoopRequestPatch(
                ...
                kwargs={
                    k: v for k, v in kwargs.items() if not k.startswith("_headroom") and k != "derouter_logging_obj"
                },
            ),
