...
def _get_param(
    ...
    """Get a param from derouter_params, with fallback to raw guardrail derouter_params (for extra fields not on LitellmParams)."""
    value: Final = getattr(derouter_params, key, default)
...
def initialize_guardrail(
    ...
    blocked_languages: Final[list[str] | None] = cast(
        list[str] | None,
        _get_param(derouter_params, guardrail, "blocked_languages"),
    )
    action: Final = cast(
        Literal["block", "mask"],
        _get_param(derouter_params, guardrail, "action", "block"),
    )
    confidence_threshold: Final = float(
        cast(
            int | float | str,
            _get_param(derouter_params, guardrail, "confidence_threshold", 0.5),
        )
    )
    detect_execution_intent: Final = bool(_get_param(derouter_params, guardrail, "detect_execution_intent", True))
    mode: Final = _get_param(derouter_params, guardrail, "mode")
    ...
    instance: Final = BlockCodeExecutionGuardrail(
        ...
        default_on=bool(_get_param(derouter_params, guardrail, "default_on", False)),
    )
