...
from derouter.types.utils import LLMResponseTypes
...
class PillarGuardrail(CustomGuardrail):
    """
    Pillar Security Guardrail for DeRouter.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Note:
            DeRouter virtual key context (user_id, team_id, key_alias, etc.) is always
            automatically passed as X-DeRouter-* headers to enable application/user tracking.
        """
    ...
    def _prepare_headers(self, user_api_key_dict: UserAPIKeyAuth) -> dict[str, str]:
        ...
        # Always add DeRouter virtual key context headers (metadata excluded for security)
        context_mapping: Final = {
            "X-DeRouter-Key-Name": user_api_key_dict.key_name,
            "X-DeRouter-Key-Alias": user_api_key_dict.key_alias,
            "X-DeRouter-User-Id": user_api_key_dict.user_id,
            "X-DeRouter-User-Email": user_api_key_dict.user_email,
            "X-DeRouter-Team-Id": user_api_key_dict.team_id,
            "X-DeRouter-Team-Name": user_api_key_dict.team_alias,
            "X-DeRouter-Org-Id": user_api_key_dict.org_id,
        }
    ...
    def _extract_model_and_provider(self, data: dict) -> tuple[str, str]:
        ...
        # Use DeRouter's standard provider detection and model cleaning
        try:
    ...
    def _prepare_payload(self, data: dict) -> dict[str, Any]:
        ...
        # User ID: use DeRouter user field
    ...
    def get_config_model() -> type["GuardrailConfigModel"] | None:
        ...
        """
        from derouter.types.proxy.guardrails.guardrail_hooks.pillar import (
