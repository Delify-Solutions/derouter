...
Run all evals:
  pytest derouter/proxy/guardrails/guardrail_hooks/derouter_content_filter/guardrail_benchmarks/test_eval.py -v -s
...
class _LlmJudgeChecker:
    """Uses an LLM via derouter.completion() to classify messages."""
    ...
    def check(self, text: str) -> str:
        ...
        response: Final = derouter.completion(
            model=self.model,
            messages=[
...
def _llm_judge(model: str = "gpt-4o-mini"):
    """LLM-as-judge using derouter.completion(). Requires API key env var."""
