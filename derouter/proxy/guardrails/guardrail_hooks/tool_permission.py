...
from derouter import ChatCompletionToolParam
...
class ToolPermissionGuardrail(CustomGuardrail):
    ...
    def _load_rules(self, rules: list[Any] | None) -> None:
        ...
        both ``__init__`` and ``update_in_memory_derouter_params`` recompile from a
    ...
    def update_in_memory_derouter_params(self, derouter_params: LitellmParams | dict) -> None:
        ...
        """
        # ``derouter_params`` may arrive as the raw DB dict (the proxy ``cast()``s
