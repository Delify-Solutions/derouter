"""
Microsoft Purview DLP Guardrail for DeRouter.
...
class MicrosoftPurviewDLPGuardrail(PurviewGuardrailBase, CustomGuardrail):
    ...
    async def _check_content(
        ...
        try:
            ...
            correlation_id: Final = request_data.get("derouter_call_id") or str(uuid.uuid4())
    ...
    def _responses_api_input_to_str(self, data: dict[str, Any], raise_on_failure: bool = False) -> str | None:
        ...
        DeRouter transformation and then concatenated by ``get_prompt_text_for_dlp``.
    ...
    def logging_hook(self, kwargs: dict, result: object, call_type: str) -> tuple[dict, object]:
        ...
        In the proxy's async success path, derouter independently calls both
    ...
    async def async_logging_hook(self, kwargs: dict, result: object, call_type: str) -> tuple[dict, object]:
        ...
        try:
            ...
            if call_type in ("responses", "aresponses"):
                ...
                # before the generic ``messages`` branch.  derouter's logging
