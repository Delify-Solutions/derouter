...
class ZscalerAIGuard(CustomGuardrail):
    ...
    def _resolve_metadata_value(request_data: dict | None, key: str) -> str | None:
        ...
        During post-call: metadata is at request_data["derouter_metadata"][key]
        ...
        Also handles key name mapping for UserAPIKeyAuth fields:
            - key_alias -> user_api_key_key_alias (in derouter_metadata)
        ...
        # Check derouter_metadata first (set during post-call by guardrail framework)
        derouter_metadata: Final = request_data.get("derouter_metadata", {})
        if derouter_metadata:
            value = derouter_metadata.get(key)
            ...
            if key == "user_api_key_alias":
                value = derouter_metadata.get("user_api_key_key_alias")
                if value is not None:
                    return str(value).strip()
    ...
    def get_config_model() -> type["GuardrailConfigModel"] | None:
        from derouter.types.proxy.guardrails.guardrail_hooks.zscaler_ai_guard import (
