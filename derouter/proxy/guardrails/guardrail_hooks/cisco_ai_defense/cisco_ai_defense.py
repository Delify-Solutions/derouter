...
# DeRouter marks MCP guardrail calls with these call_type values; the proxy
...
class CiscoAIDefenseGuardrail(_CiscoAIDefenseMcpMixin, CustomGuardrail):
    ...
    def _build_headers(self) -> dict[str, str]:
        return {
            ...
            "User-Agent": f"derouter/{derouter_version}",
        }
    ...
    def _build_metadata(
        ...
        if derouter_call_id:
            metadata["client_transaction_id"] = str(derouter_call_id)
    ...
    def _finalize_inspection(
        ...
        ``response_obj`` is the DeRouter response object (or MCP tool-call
