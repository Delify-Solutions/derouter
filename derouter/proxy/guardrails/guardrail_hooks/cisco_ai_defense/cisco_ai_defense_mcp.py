...
class _CiscoAIDefenseMcpMixin:
    ...
    def _build_mcp_request_payload(
        ...
        if data.get("jsonrpc") == "2.0":
            return {
                "jsonrpc": "2.0",
                "id": (data.get("id") or data.get("derouter_call_id") or "derouter-mcp"),
        ...
        return {
            "jsonrpc": "2.0",
            "id": data.get("derouter_call_id") or "derouter-mcp",
            "method": "tools/call",
            "params": {
                "name": tool_name,
    ...
    def _build_mcp_response_payload(
        ...
        if response_id not in (None, "derouter-mcp"):
            payload["id"] = response_id
        elif payload.get("id") in (None, "derouter-mcp"):
            request_id: Final = request_data.get("derouter_call_id") or request_data.get("id")
            if request_id:
    ...
    def _hydrate_mcp_tool_context(request_data: dict[str, object]) -> None:
        ...
        if metadata is None:
            nested: Final = request_data.get("metadata") or request_data.get("derouter_metadata")
    ...
    def _normalize_mcp_response(response: object) -> dict[str, object] | None:
        ...
        if isinstance(response, dict):
            ...
            if isinstance(response.get("result"), dict):
                return {
                    "jsonrpc": "2.0",
                    "id": response.get("id") or "derouter-mcp",
