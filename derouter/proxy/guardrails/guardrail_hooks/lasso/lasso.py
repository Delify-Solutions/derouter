...
from derouter.integrations.custom_guardrail import dc as global_cache
from derouter.llms.custom_httpx.http_handler import (
    get_async_httpx_client,
    httpxSpecialProvider,
)
from derouter.proxy._types import UserAPIKeyAuth
from derouter.proxy.guardrails._content_utils import (
...
class LassoGuardrail(CustomGuardrail):
    """
    Lasso Security Guardrail integration for DeRouter.
    ...
    async def async_post_call_success_hook(
        ...
        if isinstance(response, derouter.ModelResponse):
            ...
            if response_messages:
                # Include derouter_call_id from original data for conversation_id consistency
                response_data: Final = {
                    "messages": response_messages,
                    "derouter_call_id": data.get("derouter_call_id"),
                }
    ...
    def _get_or_generate_conversation_id(self, data: dict, cache: DualCache) -> str:
        ...
        This method ensures session consistency by using derouter_call_id as a cache key.
        ...
        Example:
            ...
            >>> data = {"derouter_call_id": "call_123"}
            >>> conversation_id = guardrail._get_or_generate_conversation_id(data, cache)
            >>> # Returns consistent ID for same derouter_call_id

        Args:
            data: The request data containing derouter_call_id
        ...
        # Get the derouter_call_id which is consistent across all hooks for this request
        derouter_call_id: Final = data.get("derouter_call_id")

        if not derouter_call_id:
            # Fallback to generating a new ULID if no derouter_call_id available
            return self._generate_ulid()

        # Use derouter_call_id as cache key for conversation_id
        cache_key: Final = f"lasso_conversation_id:{derouter_call_id}"
    ...
    def _prepare_payload(
        ...
        payload: Final[dict[str, object]] = {
            ...
            # integration is attributed as "derouter" on the keys list.
            "source": {"type": "derouter"},
        }
    ...
    def get_config_model() -> type["GuardrailConfigModel"] | None:
        from derouter.types.proxy.guardrails.guardrail_hooks.lasso import (
