...
from derouter.router_utils.auto_router_model_naming import AUTO_ROUTER_MODEL_PREFIX
...
_suppressed_compression_guardrails: Final[contextvars.ContextVar[frozenset[str]]] = contextvars.ContextVar(
    "derouter_auto_router_suppressed_compression_guardrails", default=frozenset()
)
...
_model_hop_armed: Final[contextvars.ContextVar[bool]] = contextvars.ContextVar(
    "derouter_auto_router_model_hop_armed", default=False
)
...
def policy_from_derouter_params(derouter_params: Mapping[str, object]) -> AutoRouterCompressionPolicy | None:
    raw_routing: Final = derouter_params.get("auto_router_routing_compression")
    raw_model: Final = derouter_params.get("auto_router_model_compression")
...
def policy_for_model(
    ...
    markers: Final = tuple(
        ...
        if isinstance(derouter_params := deployment.get("derouter_params"), Mapping)  # pyright: ignore[reportUnnecessaryIsInstance]  # filters out non-Mapping
    ...
    candidates: Final = (policy_from_derouter_params(params) for params in (*tag_matched, *untagged))
...
def _active_compression_guardrails() -> tuple["CustomGuardrail", ...]:
    ...
    if not _compression_guardrail_classes():
        return ()
    active: Final = derouter.logging_callback_manager.get_custom_loggers_for_type(callback_type=CustomGuardrail)
...
async def arm_pre_call(
    ...
    if armed_model_hop:
        ...
        if policy.model not in existing:
            # A list: derouter_pre_call_utils isinstance-checks this key and drops a tuple.
