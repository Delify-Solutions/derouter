...
from derouter.batches.main import CancelBatchRequest, RetrieveBatchRequest
from derouter.proxy._types import *
from derouter.proxy.auth.user_api_key_auth import user_api_key_auth
from derouter.proxy.batches_endpoints.common_utils import validate_batch_list_limit
from derouter.proxy.common_request_processing import ProxyBaseLLMRequestProcessing
from derouter.proxy.common_utils.callback_utils import sanitize_openai_provider_metadata
...
from derouter.proxy.route_llm_request import raise_if_required_body_param_missing
from derouter.proxy.utils import handle_exception_on_proxy, is_known_model
from derouter.repositories.table_repositories import ManagedFileRepository
from derouter.types.llms.openai import DeRouterBatchCreateRequest
...
def _raise_not_found_when_openai_fallback_unservable(
    ...
    if requested_provider is not None:
        return
    if data.get("api_key") or derouter.api_key or derouter.openai_key or os.getenv("OPENAI_API_KEY"):
        return
...
async def create_batch(
    ...
    try:
        ...
        _create_batch_data: Final = DeRouterBatchCreateRequest(**data)
        ...
        if model_from_file_id is not None and input_file_id:
            ...
            response = await derouter.acreate_batch(
        ...
        ):  # derouter_proxy:application/octet-stream;unified_id,c4843482-b176-4901-8292-7523fd0f2c6e;target_model_names,gpt-4o-mini
            ...
            model_param: Final = (
                data.get("model") or request.query_params.get("model") or request.headers.get("x-derouter-model")
            )
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.create_batch(): Exception occured - %s", e)
...
async def retrieve_batch(
    ...
    try:
        ...
        if response is not None and response.status in [
            ...
            asyncio.create_task(
                proxy_logging_obj.update_request_status(
                    derouter_call_id=data.get("derouter_call_id", ""), status="success"
                )
            )
        ...
        if poller_owns_accounting:
            derouter_metadata = data.get("derouter_metadata")
            if not isinstance(derouter_metadata, dict):
                derouter_metadata = {}  # mutable-ok: the suppression flag must live inside derouter_metadata for the success handler to read it, and this request carried no mapping to extend
                data["derouter_metadata"] = derouter_metadata
            derouter_metadata["batch_ignore_default_logging"] = True
        ...
        elif derouter.enable_loadbalancing_on_batch_endpoints is True or unified_batch_id:
            if llm_router is None:
                raise HTTPException(
                    status_code=500,
                    detail={"error": "LLM Router not initialized. Ensure models added to proxy."},
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.retrieve_batch(): Exception occured - %s", e)
        raise handle_exception_on_proxy(e)
...
async def list_batches(
    ...
    try:
        ...
        if managed_files_obj is not None and hasattr(managed_files_obj, "list_user_batches"):
            ...
            response = await derouter.alist_batches(
                custom_llm_provider=credentials["custom_llm_provider"],
                after=after,
                limit=limit,
                **data,
        ...
        verbose_proxy_logger.error("derouter.proxy.proxy_server.retrieve_batch(): Exception occured - %s", e)
        raise handle_exception_on_proxy(e)
...
async def cancel_batch(
    ...
    try:
        ...
        if model_from_id is not None:
            ...
            response = await derouter.acancel_batch(
                custom_llm_provider=credentials["custom_llm_provider"],
                **data,
            )
        ...
        # SCENARIO 2: target_model_names based routing
            ...
            if model_id_from_batch is None:
                raise HTTPException(
                    status_code=400,
                    detail={"error": "Invalid DeRouter managed batch ID. Missing model_id."},
                )
