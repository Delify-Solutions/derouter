...
import derouter
from derouter.constants import DEFAULT_NUM_WORKERS_DEROUTER_PROXY
from derouter.proxy.db.query_engine_reaper import start_query_engine_reaper
...
def _deprioritize_script_dir_in_sys_path() -> None:
    """Stop ``derouter/proxy`` modules from shadowing installed packages.

    Running this file as a script puts its own directory at ``sys.path[0]``, so
    ``import a2a`` resolves to ``derouter/proxy/a2a`` instead of the ``a2a`` SDK
    ...
    import ...``) still need it. No-op under the ``derouter`` console script.
    """
...
config_filename: Final = "derouter.secrets"

derouter_mode: Final = os.getenv("DEROUTER_MODE", "DEV")  # "PRODUCTION", "DEV"
if derouter_mode == "DEV":
    load_dotenv()
...
class DeRouterDatabaseConnectionPool(Enum):
...
class ProxyInitializationHelpers:
    @staticmethod
    def _echo_derouter_version():
        pkg_version: Final = importlib.metadata.version("derouter")
        click.echo(f"\nDeRouter: Current Version = {pkg_version}\n")

    @staticmethod
    def _run_health_check(host, port):
        print("\nDeRouter: Health Testing models in config")
    ...
    def _run_test_chat_completion(
        ...
        click.echo(f"\nDeRouter: Making a test ChatCompletions request to your proxy. Model={request_model}")
        import openai
        ...
        click.echo(f"\nDeRouter: response from proxy {response}")

        print(f"\n DeRouter: Making a test ChatCompletions + streaming r equest to proxy. Model={request_model}")
        ...
        for chunk in stream_response:
            click.echo(f"DeRouter: streaming response from proxy {chunk}")
    ...
    def _get_default_unvicorn_init_args(
        ...
        import derouter
        from derouter._logging import _get_uvicorn_json_log_config

        uvicorn_args: Final = {
            "app": "derouter.proxy.proxy_server:app",
            "host": host,
            "port": port,
            "server_header": False,
        }
        ...
        if timeout_worker_healthcheck is not None:
            if "timeout_worker_healthcheck" in inspect.signature(uvicorn.Config.__init__).parameters:
                ...
                print(
                    f"\033[1;33mDeRouter Proxy: --timeout_worker_healthcheck "
    ...
    def _apply_uvicorn_max_requests_jitter(
        ...
        if max_requests_before_restart is None:
            print(
                "\033[1;33mDeRouter Proxy: --max_requests_before_restart_jitter "
        ...
        if "limit_max_requests_jitter" in inspect.signature(uvicorn.Config.__init__).parameters:
            ...
            print(
                f"\033[1;33mDeRouter Proxy: --max_requests_before_restart_jitter "
    ...
    def _patch_statreload_extra_paths(paths: Iterable[str | None]) -> bool:
        ...
        patched_paths = getattr(StatReload, "_derouter_patched_config_paths", None)
        if patched_paths is None:
            ...
            def _iter_with_extra(self):
                ...
                for path in StatReload._derouter_patched_config_paths:
                    if path.exists():
                        yield path

            StatReload.iter_py_files = _iter_with_extra
            StatReload._derouter_patched_config_paths = patched_paths
    ...
    def _configure_dev_reload(uvicorn_args: dict, config_path: str | None) -> None:
        ...
        os.environ["DEROUTER_DEV_ENV_HOT_RELOAD"] = "True"
        ...
        verbose_proxy_logger.warning(
            "DeRouter --reload: worker processes re-read .env with override, so .env "
    ...
    def _init_hypercorn_server(
        ...
        """
        Initialize derouter with `hypercorn`
        """
        import asyncio
        ...
        print(f"\033[1;32mDeRouter Proxy: Starting server on {host}:{port} using Hypercorn\033[0m\n")
        config: Final = Config()
        config.bind = [f"{host}:{port}"]

        if ssl_certfile_path is not None and ssl_keyfile_path is not None:
            print(
                f"\033[1;32mDeRouter Proxy: Using SSL with certfile: {ssl_certfile_path} and keyfile: {ssl_keyfile_path}\033[0m\n"
            )
    ...
    def _init_granian_server(
        ...
        Uses a string import path so workers load ``derouter.proxy.proxy_server:app``
        the same way as uvicorn's ``app=`` string target.
        """
        from granian import Granian
        from granian.constants import Interfaces

        print(f"\033[1;32mDeRouter Proxy: Starting server on {host}:{port} using Granian\033[0m\n")
        if max_requests_before_restart is not None:
            print(
                "\033[1;33mDeRouter: --max_requests_before_restart is not supported by Granian "
                "(Granian uses workers_lifetime in seconds, not a per-request limit).\033[0m\n"
            )
        if ciphers is not None:
            print("\033[1;33mDeRouter: --ciphers is not applied when using --run_granian.\033[0m\n")

        kwargs: Final[dict[str, Any]] = {
            "target": "derouter.proxy.proxy_server:app",
            "address": host,
            "port": port,
    ...
    def _run_gunicorn_server(
        ...
        class StandaloneApplication(gunicorn.app.base.BaseApplication):
            def __init__(self, app, options=None):
                ...
                print()
                print(
                    '\033[1;34mDeRouter: Test your local proxy with: "derouter --test" This runs an openai.ChatCompletion request to your proxy [In a new terminal tab]\033[0m\n'
                )
                print(f"\033[1;34mDeRouter: Curl Command Test for your local proxy\n {curl_command} \033[0m\n")
                print("\033[1;34mDocs: https://router.delify.vn/docs/simple_proxy\033[0m\n")
                print(f"\033[1;34mSee all Router/Swagger docs on http://0.0.0.0:{port} \033[0m\n")

            def load_config(self):
                # note: This Loads the gunicorn config - has nothing to do with DeRouter Proxy config
        ...
        print(f"\033[1;32mDeRouter Proxy: Starting server on {host}:{port} with {num_workers} workers\033[0m\n")
        ...
        if os.environ.get("PROMETHEUS_MULTIPROC_DIR"):
            from derouter.proxy.prometheus_cleanup import mark_worker_exit
    ...
    @staticmethod
    def _prometheus_callback_configured(derouter_settings: Mapping[str, object] | None) -> bool:
        if derouter_settings is None:
            return False
        configured: Final = tuple(
            derouter_settings.get(key) for key in ("callbacks", "success_callback", "failure_callback")
        )
        return any(
            setting == "prometheus"
    ...
    def _maybe_setup_prometheus_multiproc_dir(
        ...
        if prometheus_metrics_port is None and (
            num_workers <= 1 or not ProxyInitializationHelpers._prometheus_callback_configured(derouter_settings)
        ):
            return None

        from derouter.proxy.prometheus_cleanup import wipe_directory

        configured_dir: Final = os.environ.get("PROMETHEUS_MULTIPROC_DIR") or os.environ.get("prometheus_multiproc_dir")
        multiproc_dir: Final = configured_dir or os.path.join(tempfile.gettempdir(), "derouter_prometheus_multiproc")
        os.environ["PROMETHEUS_MULTIPROC_DIR"] = multiproc_dir

        os.makedirs(multiproc_dir, exist_ok=True)
        wipe_directory(multiproc_dir)
        action: Final = "Using existing" if configured_dir else "Auto-created"
        print(f"DeRouter: {action} PROMETHEUS_MULTIPROC_DIR={multiproc_dir}")
...
@click.option(
    "--num_workers",
    default=DEFAULT_NUM_WORKERS_DEROUTER_PROXY,
    help=(
        "Number of worker processes for uvicorn / gunicorn, or Granian worker processes "
        "(--workers). Default is 1 (from DEFAULT_NUM_WORKERS_DEROUTER_PROXY). "
...
@click.option(
    "--api_version",
    default=derouter.AZURE_DEFAULT_API_VERSION,
    help="For azure - pass in the api version.",
)
@click.option("--model", "-m", default=None, help="The model name to pass to derouter expects")
@click.option(
    "--alias",
    default=None,
    help='The alias for the model - use this to give a derouter model name (e.g. "huggingface/codellama/CodeLlama-7b-Instruct-hf") a more user-friendly name ("codellama")',
)
@click.option("--add_key", default=None, help="The model name to pass to derouter expects")
...
@click.option(
    ...
    "-c",
    default=None,
    help="Path to the proxy configuration file (e.g. config.yaml). Usage `derouter --config config.yaml`",
)
@click.option(
    "--max_budget",
    default=None,
...
@click.option(
    ...
    help=(
        ...
        "DeRouter versions contend for the same DB. Default is the v1 resolver."
    ),
...
def run_server(
    ...
    if cli_args:
        if cli_args == ("xai-oauth", "login"):
            from derouter.llms.xai.oauth import XAIOAuthAuthenticator
    ...
    if setup:
        from derouter.setup_wizard import run_setup_wizard
    ...
    if local:
        ...
        try:
            ...
            raise ModuleNotFoundError(f"Missing dependency {e}. Run `pip install 'derouter[proxy]'`")
        except ImportError as e:
            if "derouter[proxy]" in str(e):
                # user is missing a proxy dependency, ask them to pip install derouter[proxy]
                raise e
            else:
                # this is just a local/relative import error, user git cloned derouter
                from proxy_server import (
                    KeyManagementSettings,
                    ProxyConfig,
                    app,
    ...
    if version is True:
        ProxyInitializationHelpers._echo_derouter_version()
        return
    ...
    if test is True:
        ...
        if run_granian:
            try:
                ...
                raise ImportError(
                    ...
                    "Run `pip install granian` or `pip install 'derouter[proxy]'` "
            ...
            try:
                ...
                raise ImportError("uvicorn, gunicorn needs to be imported. Run - `pip install 'derouter[proxy]'`")
        ...
        from derouter.proxy.db.db_url_settings import DatabaseURLSettings
        ...
        from derouter.secret_managers.aws_secret_manager import decrypt_env_var
        ...
        if config is not None:
            ...
            try:
                ...
                raise ImportError("yaml needs to be imported. Run - `pip install 'derouter[proxy]'`")

            proxy_config: Final = ProxyConfig()
            _config: Final = asyncio.run(proxy_config.get_config(config_file_path=config))

            ### DEROUTER SETTINGS ###
            derouter_settings = _config.get("derouter_settings", None)
            if (
                derouter_settings is not None
                and "json_logs" in derouter_settings
                and derouter_settings["json_logs"] is True
            ):
                import derouter
            ...
            if database_url is None and os.getenv("DATABASE_URL") is None:
                ...
                from derouter.proxy.utils import construct_database_url_from_env_vars
            ...
            db_connection_pool_limit = general_settings.get(
                ...
                DeRouterDatabaseConnectionPool.database_connection_pool_limit.value,
            )
            ...
            if db_connection_timeout is None:
                db_connection_timeout = DeRouterDatabaseConnectionPool.database_connection_pool_timeout.value
            ...
            if database_url and database_url.startswith("os.environ/"):
                ...
                sys.path.insert(
                    0, os.path.abspath("../..")
                )  # Adds the parent directory to the system path - for derouter local dev
                import derouter
        ...
        if config is None:
            db_connection_pool_limit = DeRouterDatabaseConnectionPool.database_connection_pool_limit.value
            db_connection_timeout = DeRouterDatabaseConnectionPool.database_connection_pool_timeout.value

        if os.getenv("DATABASE_URL", None) is not None or os.getenv("DIRECT_URL", None) is not None:
            from derouter.proxy.db.db_url_settings import (
            ...
            for _db_env in ("DATABASE_URL", "DIRECT_URL"):
                ...
                if _bad_scheme is not None:
                    print(
                        f"\033[1;31mDeRouter Proxy: {unsupported_db_scheme_message(_db_env, _bad_scheme)}\033[0m",
                        file=sys.stderr,
                        flush=True,
                    )
                    sys.exit(1)
            ...
            if is_prisma_runnable:
                from derouter.proxy.db.check_migration import check_prisma_schema_diff
                ...
                if should_update_prisma_schema(general_settings.get("disable_prisma_schema_update")) is False:
                    ...
                    if not use_v2_migration_resolver:
                        print(
                            "\033[1;33mDeRouter Proxy: Using default (v1) migration resolver. "
                    ...
                    try:
                        ...
                        # partitioned DeRouter_SpendLogs.
                        print(
                            f"\033[1;31mDeRouter Proxy: Database migration cannot proceed. {e}\033[0m",
                            file=sys.stderr,
                            flush=True,
                        )
                        sys.exit(2)
                    if not setup_ok:
                        if enforce_prisma_migration_check:
                            print(
                                "\033[1;31mDeRouter Proxy: Database setup failed after multiple retries. "
                                "The proxy cannot start safely. Please check your database connection and migration status.\033[0m"
                            )
                            sys.exit(1)
                        else:
                            print(
                                "\033[1;33mDeRouter Proxy: Database migration failed but continuing startup. "
        ...
        prometheus_multiproc_dir: Final = ProxyInitializationHelpers._maybe_setup_prometheus_multiproc_dir(
            ...
            derouter_settings=derouter_settings if config else None,
            prometheus_metrics_port=prometheus_metrics_port,
        )

        # Skip server startup if requested (after all setup is done)
        if skip_server_startup:
            print("DeRouter: Setup complete. Skipping server startup as requested.")
            return

        if prometheus_metrics_port is not None and prometheus_multiproc_dir is not None:
            from derouter.proxy.prometheus_metrics_server import MetricsServerStartupError, start_metrics_server_process
            ...
            print(
                f"\033[1;32mDeRouter: Serving Prometheus metrics on {host}:{prometheus_metrics_port}/metrics "
