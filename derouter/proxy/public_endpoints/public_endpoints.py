...
from derouter.derouter_core_utils.get_blog_posts import (
    BlogPost,
    BlogPostsResponse,
    GetBlogPosts,
    get_blog_posts,
...
from derouter.repositories.table_repositories import ClaudeCodePluginRepository
from derouter.types.agents import AgentCard
from derouter.types.mcp import MCPPublicServer
from derouter.types.proxy.management_endpoints.model_management_endpoints import (
    ModelGroupInfoProxy,
)
from derouter.types.proxy.public_endpoints.public_endpoints import (
    AgentCreateInfo,
...
def _load_endpoints() -> list[_EndpointEntry]:
    raw: Final[_ProvidersFile] = json.loads(
        files("derouter").joinpath("provider_endpoints_support_backup.json").read_text(encoding="utf-8")
    )
...
async def public_model_hub():
    ...
    if derouter.public_model_groups is not None:
        model_groups = _get_model_group_info(
            ...
            all_models_str=derouter.public_model_groups,
            model_group=None,
        )
...
async def get_agents(request: Request):
    ...
    return [
        ...
        for agent in agents
        if not global_agent_registry.ids_for_agent(agent.agent_id).isdisjoint(derouter.public_agent_groups)
    ]
...
async def public_skill_hub():
    ...
    from derouter.proxy.anthropic_endpoints.claude_code_endpoints.claude_code_marketplace import (
        _get_prisma_client,
    )
    from derouter.types.proxy.claude_code_endpoints import (
        ListPluginsResponse,
        PluginListItem,
    )
...
async def public_model_hub_info():
    import derouter
    from derouter.proxy.proxy_server import _title, version
    ...
    return PublicModelHubInfo(
        ...
        useful_links=derouter.public_model_groups_links,
    )
...
async def get_supported_providers() -> list[str]:
    """
    Return a sorted list of all providers supported by DeRouter.
    """
...
async def get_derouter_model_cost_map():
    """
    Public endpoint to get the DeRouter model cost map.
    Returns pricing information for all supported models.
    """
    import derouter

    try:
        _model_cost_map: Final = derouter.model_cost
...
@router.get(
    "/public/derouter_blog_posts",
    tags=["public"],
    response_model=BlogPostsResponse,
)
async def get_derouter_blog_posts():
    """
    Public endpoint to get the latest DeRouter blog posts.

    Fetches from GitHub with a 1-hour in-process cache.
    Falls back to the bundled local backup on any failure.
    """
    try:
        posts_data = get_blog_posts(url=derouter.blog_posts_url)
    except Exception as e:
        verbose_logger.warning("DeRouter: get_derouter_blog_posts endpoint fallback triggered: %s", str(e))
...
def _load_bundled_autorouter_presets() -> Mapping[str, AutoRouterPresetRecord]:
    raw: Final = json.loads(
        files("derouter.proxy.public_endpoints").joinpath("autorouter_presets.json").read_text(encoding="utf-8")
    )
...
async def _resolve_autorouter_presets(
    ...
    if os.getenv("DEROUTER_LOCAL_AUTOROUTER_PRESETS", "").lower() == "true":
        return _load_bundled_autorouter_presets()
    try:
        return await fetch(url)
    except Exception as e:
        verbose_logger.warning(
            "DeRouter: failed to fetch auto-router presets from %s: %s. Serving the bundled catalog for the life of this process.",
            url,
            str(e),
        )
        return _load_bundled_autorouter_presets()
...
async def get_public_autorouter_presets() -> Mapping[str, AutoRouterPresetRecord]:
    ...
    Resolved once per process, like the model cost map: fetched from ``derouter.autorouter_presets_url``
    (override with ``DEROUTER_AUTOROUTER_PRESETS_URL``) on the first request, falling back to the
    catalog bundled with the package on any failure. Set ``DEROUTER_LOCAL_AUTOROUTER_PRESETS=True``
    to serve the bundled catalog only. A restart picks up a newly published catalog.
    """
    return await get_autorouter_presets(url=derouter.autorouter_presets_url)
...
async def get_supported_endpoints() -> SupportedEndpointsResponse:
    """
    Return the list of DeRouter proxy endpoints and which providers support each one.
...
async def get_agent_fields() -> list[AgentCreateInfo]:
    ...
    for agent in agent_create_fields:
        ...
        if inherit_from and inherit_from in provider_map:
            ...
            # Copy provider fields and mark them for inclusion in derouter_params
            inherited_fields = []
            for field in provider.get("credential_fields", []):
                field_copy = field.copy()
                field_copy["include_in_derouter_params"] = True
