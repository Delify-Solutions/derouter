...
from derouter.proxy.list_api.common import PROBLEM_TYPE_BASE, ManagementProblem
from derouter.proxy.list_api.in_memory import Cells, InMemoryListExecutor
...
from derouter.types.proxy.management_endpoints.management_v1 import (
    FacetListResponse,
    ListResponse,
    ProblemDetail,
)
...
def _published_rows() -> Sequence[ModelGroupInfoProxy]:
    ...
    if derouter.public_model_groups is None:
        return ()
    return tuple(
...
async def public_model_hub_list(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "derouter.proxy.public_endpoints.public_v1.model_hub.public_model_hub_list(): Exception occured - %s", e
        )
        raise ManagementProblem(
            ProblemDetail(
...
async def public_model_hub_facet(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "derouter.proxy.public_endpoints.public_v1.model_hub.public_model_hub_facet(): Exception occured - %s", e
        )
        raise ManagementProblem(
            ProblemDetail(
                type=f"{PROBLEM_TYPE_BASE}internal-server-error",
