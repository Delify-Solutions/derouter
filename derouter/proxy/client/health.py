...
class HealthManagementClient:
    """
    Client for interacting with the health endpoints of the DeRouter proxy server.
    """
    ...
    def get_readiness(self) -> dict[str, Any]:
        """
        Check the readiness of the DeRouter proxy server.
    ...
    def get_server_version(self) -> str | None:
        """
        Get the DeRouter server version from the readiness endpoint.

        Returns:
            Optional[str]: The server version if available, otherwise None.
        """
        readiness: Final = self.get_readiness()
        return readiness.get("derouter_version")
