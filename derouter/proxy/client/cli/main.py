...
from derouter.proxy.client.health import HealthManagementClient
...
def print_version(base_url: str, api_key: str | None):
    ...
    click.echo(f"DeRouter Proxy CLI Version: {derouter_version}")
    if base_url:
        click.echo(f"DeRouter Proxy Server URL: {base_url}")
    try:
        ...
        if server_version:
            click.echo(f"DeRouter Proxy Server Version: {server_version}")
        else:
            click.echo("DeRouter Proxy Server Version: (unavailable)")
...
@click.option(
    ...
    "show_version",
    is_flag=True,
    help="Show the DeRouter Proxy CLI and server version and exit.",
)
@click.option(
    "--base-url",
    envvar="DEROUTER_PROXY_URL",
    show_envvar=True,
    default=None,
    show_default="base_url from `lite config`, else http://localhost:4000",
    help="Base URL of the DeRouter proxy server",
)
@click.option(
    "--api-key",
    envvar="DEROUTER_PROXY_API_KEY",
    show_envvar=True,
    help="API key for authentication",
)
@click.pass_context
def cli(ctx: click.Context, show_version: bool, base_url: str | None, api_key: str | None) -> None:
    """DeRouter Proxy CLI - Manage your DeRouter proxy server"""
...
def version(ctx: click.Context):
    """Show the DeRouter Proxy CLI and server version."""
...
# Add the up/down commands (route Claude Code through the local DeRouter proxy)
cli.add_command(up)
