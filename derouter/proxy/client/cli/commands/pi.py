"""Sync a DeRouter provider into pi's models.json.
...
PI_PROVIDER_NAME: Final = "derouter"
DEROUTER_PROXY_API_KEY_ENV: Final = "DEROUTER_PROXY_API_KEY"
...
def provider_block(
    ...
    """openai-completions is the one API shape every DeRouter model serves.
    ...
    return {  # mutable-ok: JSON serialization requires a mutable object
        ...
        "apiKey": f"${DEROUTER_PROXY_API_KEY_ENV}",
...
def sync_models_json(
    ...
    """Replace only the derouter provider entry, leaving the rest of the file intact."""
    try:
