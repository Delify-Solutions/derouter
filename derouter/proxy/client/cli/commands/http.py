...
@click.group()
def http():
    """Make HTTP requests to the DeRouter proxy server"""
...
def request(
    ...
    Examples:
        derouter http request GET /models
        derouter http request POST /chat/completions -j '{"model": "gpt-4", "messages": [{"role": "user", "content": "Hello"}]}'
        derouter http request GET /health/test_connection -H "X-Custom-Header:value"
    """
