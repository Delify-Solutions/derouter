...
from derouter.derouter_core_utils.cli_keyring import SecretVault
from derouter.derouter_core_utils.cli_token_utils import is_cli_token_fresh
from derouter.derouter_core_utils.private_json import ensure_private_dir
...
def _ensure_fresh_login(ctx: click.Context) -> None:
    ...
    if not sys.stdin.isatty():
        raise UpError(
            f"No fresh DeRouter login found for this proxy. Run `{login_command}` first (apiKeyHelper "
            "reads this token on every Claude Code request)."
        )

    click.echo("No fresh DeRouter login found for this proxy; starting login...")
...
def up(ctx: click.Context) -> None:
    """Route every Claude Code session through your DeRouter proxy until stopped.
    ...
    click.echo(f"derouter: routing Claude Code through proxy at {base_url.rstrip('/')}")
