...
AUTOROUTE_DIR: Final = Path.home() / ".derouter" / "autorouter"
...
def missing_proxy_runtime_modules() -> tuple[str, ...]:
    ...
    ``launch_proxy`` runs the full ``derouter.proxy.proxy_cli`` server, whose dependencies live in
    the ``proxy`` extra, not the ``cli`` extra that installs the ``lite`` command. On a thin
    ``derouter[cli]`` install the subprocess dies with a bare ``ModuleNotFoundError``; detecting the
