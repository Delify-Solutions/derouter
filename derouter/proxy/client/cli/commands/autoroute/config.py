...
def _derouter_proxy_deployment(name: str, base_url: str, api_key: str) -> dict[str, JsonValue]:
    return {
        "model_name": name,
        "derouter_params": {
            "model": f"derouter_proxy/{name}",
            "api_base": base_url,
            "api_key": api_key,
        },
    }
...
def build_generated_model_list(config: AutorouteConfig) -> list[JsonValue]:
    ...
    to exactly one `derouter_proxy/<name>` deployment forwarding to the customer's real proxy,
    ...
    proxy_deployments: Final = [
        _derouter_proxy_deployment(name, config.base_url, config.api_key) for name in sorted(referenced_names)
    ]
    ...
    auto_router_derouter_params: Final[dict[str, JsonValue]] = {
    ...
    return [
        *proxy_deployments,
        {"model_name": AUTOROUTER_MODEL_NAME, "derouter_params": auto_router_derouter_params},
    ]
...
def build_generated_proxy_config(config: AutorouteConfig, master_key: str) -> dict[str, JsonValue]:
    ...
    master_key must live under general_settings, not derouter_settings -- the proxy server
    only ever reads general_settings.master_key (proxy_server.py:4530) to authenticate
    requests; a key placed under derouter_settings is silently ignored, leaving the proxy
