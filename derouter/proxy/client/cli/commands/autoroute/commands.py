...
def _ensure_master_key() -> str:
    ...
    general_settings.master_key only (a key under derouter_settings is silently ignored, which
...
def up(port: int) -> None:
    ...
    if missing:
        raise click.ClickException(
            "lite autoroute up launches a local derouter proxy, which needs the proxy runtime that the "
            f"thin `derouter[cli]` install does not include (missing: {', '.join(missing)}). Install the "
            "proxy runtime with `uv tool install --force 'derouter[proxy]'`, or to QA a branch, "
    ...
    if port == 4000:
        raise click.ClickException(
            "Port 4000 is the derouter proxy's own default and its launcher silently rebinds it to a random "
    ...
    click.echo(f"derouter: ephemeral auto-router proxy up at {base_url} (pid {process.pid})")
