...
KEYRING_INSTALL_HINT: Final = "pip install 'derouter[cli]'"
...
def _start_cli_sso_flow(base_url: str) -> CliSsoStartData:
    ...
    try:
        ...
        raise ValueError(
            ...
            "A proxy, load balancer, or auth gateway in front of DeRouter may be intercepting the request. "
...
def _get_cli_sso_poll_headers(poll_secret: str) -> dict[str, str]:
    return {"x-derouter-cli-poll-secret": poll_secret}
...
def login(ctx: click.Context, config_claude: bool, pkce: bool) -> None:
    """Login to DeRouter proxy using SSO authentication"""
...
def print_token(ctx: click.Context):
    ...
    expires after `DEROUTER_CLI_JWT_EXPIRATION_HOURS` (default 24h); a
