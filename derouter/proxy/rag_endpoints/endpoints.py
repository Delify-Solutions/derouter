"""
RAG Endpoints for DeRouter Proxy.
...
from derouter.proxy.rag_endpoints.upload_security import (
...
) -> Mapping[str, DeRouter_ManagedVectorStore]:
...
def _build_file_metadata_entry(
    ...
    Args:
        response: The response from derouter.aingest containing file_id
...
async def _save_vector_store_to_db_from_rag_ingest(
    ...
    Args:
        response: The response from derouter.aingest()
    ...
    # Extract derouter_vector_store_params for custom name and description
    derouter_vector_store_params: Final = ingest_options.get("derouter_vector_store_params", {})
    custom_vector_store_name: Final = derouter_vector_store_params.get("vector_store_name")
    custom_vector_store_description: Final = derouter_vector_store_params.get("vector_store_description")

    # Extract provider-specific params from vector_store_config to save as derouter_params
    ...
    try:
        ...
        if existing_vector_store is None:
            ...
            await create_vector_store_in_db(
                ...
                derouter_params=(provider_specific_params if provider_specific_params else None),
            ...
            from derouter.proxy.utils import safe_dumps
...
async def rag_ingest(
    ...
    try:
        ...
        # Call ingest
        response: Final = await derouter.aingest(
...
async def rag_query(
    ...
    try:
        ...
        # Merge derouter-managed vector store params (provider, region, embedding
        ...
        merged_retrieval_config: Final = {
            **retrieval_config,
            **store_data,
        }  # mutable-ok: derouter.aquery requires a plain dict payload
        ...
        def custom_headers_for(response: ModelResponse) -> Mapping[str, str]:
            ...
            return ProxyBaseLLMRequestProcessing.get_custom_headers(
                user_api_key_dict=user_api_key_dict,
                call_id=hidden_params.get("derouter_call_id", None) or "",
