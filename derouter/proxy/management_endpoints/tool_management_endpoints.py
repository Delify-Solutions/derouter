...
if TYPE_CHECKING:
    from prisma.models import DeRouter_DailyToolSpend as PrismaDailyToolSpendRow
    from prisma.models import DeRouter_ObjectPermissionTable as PrismaObjectPermissionRow
    from prisma.models import DeRouter_SpendLogs as PrismaSpendLogRow
    from prisma.models import DeRouter_SpendLogToolIndex as PrismaSpendLogToolIndexRow
    from prisma.models import DeRouter_TeamTable as PrismaTeamRow
    from prisma.models import DeRouter_VerificationToken as PrismaVerificationTokenRow
...
from derouter.types.tool_management import (
...
async def list_tools(
    ...
    """
    from derouter.proxy.db.tool_registry_writer import list_tools as db_list_tools
...
async def get_tool_spend(
    ...
    Reads the ``DeRouter_DailyToolSpend`` rollup, written at request time from invoked
...
async def get_tool_detail(
    ...
    """
    from derouter.proxy.db.tool_registry_writer import get_tool as db_get_tool
    from derouter.proxy.db.tool_registry_writer import list_overrides_for_tool
    from derouter.proxy.proxy_server import prisma_client
...
@router.get(
    ...
    tags=["tool management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=DeRouter_ToolTableRow,
)
async def get_tool(
    tool_name: str,
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
...
async def update_tool_policy(
    ...
    """
    from derouter.proxy.db.tool_registry_writer import (
