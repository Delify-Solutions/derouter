...
from derouter.derouter_core_utils.llm_judge import judge_target
...
from derouter.proxy.db.autorouter_session_rollup import AUTOROUTER_BENCHMARKS_SQL
...
from derouter.router_strategy.complexity_router import ComplexityRouter
...
def _verification_tokens(prisma_client: "PrismaClient") -> _VerificationTokenTable:
    return prisma_client.db.derouter_verificationtoken
...
def _user_rows(prisma_client: "PrismaClient") -> _UserRowsTable:
    return prisma_client.db.derouter_usertable


def _shadow_eval_jobs(prisma_client: "PrismaClient") -> _ShadowEvalJobTable:
    return prisma_client.db.derouter_shadowevaljob


def _shadow_eval_funnel(prisma_client: "PrismaClient") -> _ShadowEvalFunnelTable:
    return prisma_client.db.derouter_shadowevalfunnel  # pyright: ignore[reportAttributeAccessIssue]  # generated client


def _shadow_eval_attempts(prisma_client: "PrismaClient") -> _ShadowEvalAttemptTable:
    return prisma_client.db.derouter_shadowevalattempt
...
async def preview_auto_router_routing(
    ...
    **Example Request:**
    ...
    request_kwargs: Final = DeRouterProxyRequestSetup.add_user_api_key_auth_to_request_metadata(
...
def _strategy_router_key(deployment: object) -> tuple[str, str] | None:
    ...
    if not (isinstance(derouter_params, Mapping) and isinstance(router_name, str) and router_name):
        return None
    model: Final = derouter_params.get("model")
...
async def get_auto_router_benchmarks(
    ...
    Reads the DeRouter_AutoRouterSession rollup, folded once per request at spend-write time,
    so this endpoint never scans DeRouter_SpendLogs. A session is in the window when it
...
def _sdk_model_is_missing_anthropic_credentials(model: str) -> bool:
    _, provider, _, _ = derouter.get_llm_provider(model=model)
    if provider != "anthropic" or derouter.anthropic_key or derouter.api_key:
        return False
    from derouter.llms.anthropic.common_utils import AnthropicModelInfo
    from derouter.secret_managers.main import secret_manager_would_be_consulted
...
def _validate_plain_model(
    ...
    raise HTTPException(
        status_code=400,
        detail=(
            f"{field_name} '{model}' uses the DeRouter SDK but required credentials are not configured: "
...
def _router_arm_models(llm_router: "Router | None", router_name: str) -> tuple[tuple[str, str], ...]:
    ...
    return tuple(
        dict.fromkeys(
            ...
            for dependency in strategy_router_dependencies(deployment["derouter_params"])
...
_ATTEMPT_AGG_SELECT: Final = (
    ...
    + """
FROM "DeRouter_ShadowEvalAttempt"
...
_ATTEMPT_AGG_BY_ROUTER_SQL: Final = (
    ...
    + _ATTEMPT_AGG_COLUMNS
    + """
FROM "DeRouter_ShadowEvalAttempt" a
JOIN "DeRouter_ShadowEvalJob" j ON j.id = a.job_id
...
UPDATE "DeRouter_ShadowEvalJob" j SET stopped_at = (NOW() AT TIME ZONE 'utc')
WHERE j.target_type = $2 AND j.target_id = ANY($1::text[]) AND j.stopped_at IS NULL
  AND (
    j.ends_at <= (NOW() AT TIME ZONE 'utc')
    OR (SELECT COUNT(*) FROM "DeRouter_ShadowEvalAttempt" a WHERE a.job_id = j.id) >= j.max_turns
    OR (
      j.max_budget IS NOT NULL
      AND (SELECT COALESCE(SUM(a.judge_cost + a.shadow_cost + a.shadow_classifier_cost), 0) FROM "DeRouter_ShadowEvalAttempt" a WHERE a.job_id = j.id) >= j.max_budget
    )
  )
"""
...
FROM "DeRouter_ShadowEvalFunnel"
WHERE job_id = ANY($1::text[])
"""


_STOP_JOB_SQL: Final = """
UPDATE "DeRouter_ShadowEvalJob"
...
WHERE group_id = $1 AND stopped_by IS NULL
  ...
  AND EXISTS (
    SELECT 1 FROM "DeRouter_ShadowEvalJob" k
    WHERE k.group_id = $1 AND k.stopped_at IS NULL
      AND (SELECT COUNT(*) FROM "DeRouter_ShadowEvalAttempt" a WHERE a.job_id = k.id) < k.max_turns
      AND (
        k.max_budget IS NULL
        OR (SELECT COALESCE(SUM(a.judge_cost + a.shadow_cost + a.shadow_classifier_cost), 0) FROM "DeRouter_ShadowEvalAttempt" a WHERE a.job_id = k.id) < k.max_budget
      )
  )
"""
...
SELECT * FROM "DeRouter_ShadowEvalJob"
WHERE group_id IN (
    SELECT group_id FROM "DeRouter_ShadowEvalJob"
...
SELECT * FROM "DeRouter_ShadowEvalJob"
WHERE group_id IN (
    SELECT group_id FROM "DeRouter_ShadowEvalJob" WHERE target_type = $2 AND target_id = $3
    GROUP BY group_id ORDER BY MAX(created_at) DESC LIMIT $1::int
)
"""
...
class _LegRow(BaseModel):
    """One DeRouter_ShadowEvalJob row, validated off the untyped prisma record. A row is
