...
from derouter._redis import _redis_kwargs_from_environment
...
from derouter.repositories.table_repositories import CacheConfigRepository
...
def _resolve_cache_url_precedence(settings: Mapping[str, object]) -> dict[str, Any]:
    ...
    is unambiguous and matches runtime resolution in ``derouter._redis``
...
async def _emit_cache_settings_audit_log(
    *,
    ...
    exception.  Captured under ``DeRouter_CacheConfig`` so the row
    ...
    async def init_cache_settings_in_db(prisma_client: "PrismaClient", proxy_config):
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "derouter.proxy.management_endpoints.cache_settings_endpoints.py::CacheSettingsManager::init_cache_settings_in_db - %s",
                e,
            )
...
async def update_cache_settings(
    ...
    2. Saves to DeRouter_CacheConfig table
