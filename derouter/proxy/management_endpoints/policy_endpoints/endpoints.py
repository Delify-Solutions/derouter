...
from derouter.proxy.guardrails.guardrail_hooks.custom_code import (
...
from derouter.types.utils import GenericGuardrailAPIInputs, ModelResponse
...
async def apply_policies(
    ...
    proxy_logging_obj: "DeRouterLoggingObj",
...
async def test_policies_and_guardrails(
    ...
    """
    from derouter.derouter_core_utils.derouter_logging import Logging as DeRouterLoggingObj
    from derouter.proxy.proxy_server import chat_completion, proxy_logging_obj
    ...
    try:
        logging_obj: Final = cast(DeRouterLoggingObj, proxy_logging_obj)
...
async def list_policies(
    ...
    - Inheritance chain
    """
    from derouter.proxy.policy_engine.init_policies import get_policies_summary
...
POLICY_TEMPLATES_GITHUB_URL: Final = "https://raw.githubusercontent.com/Delify-Solutions/derouter/main/policy_templates.json"


def _load_policy_templates_from_local_backup() -> list:
    """Load policy templates from local backup file (derouter/policy_templates_backup.json)."""
...
async def get_policy_templates(
    ...
    Set DEROUTER_LOCAL_POLICY_TEMPLATES=true to skip GitHub and use local backup only.
    """
    use_local: Final = os.getenv("DEROUTER_LOCAL_POLICY_TEMPLATES", "").strip().lower() in (
        "true",
        "1",
        "yes",
    )
...
def _build_competitor_guardrail_definitions(
    ...
    for defn in enriched:
        ...
        if guardrail_name in blocked_words_map:
            defn["derouter_params"]["blocked_words"] = blocked_words_map[guardrail_name]
...
async def suggest_policy_templates(
    ...
    """
    from derouter.proxy.management_endpoints.policy_endpoints.ai_policy_suggester import (
        AiPolicySuggester,
    )
...
async def _test_guardrail_definitions(
    ...
    for guardrail_def in guardrail_definitions:
        ...
        derouter_params = guardrail_def.get("derouter_params", {})
        guardrail_type = derouter_params.get("guardrail", "")

        if guardrail_type != "derouter_content_filter":
            results.append(
        ...
        try:
            guardrail = ContentFilterGuardrail(
                guardrail_name=guardrail_name,
                patterns=derouter_params.get("patterns"),
                blocked_words=derouter_params.get("blocked_words"),
                categories=derouter_params.get("categories"),
                pattern_redaction_format=derouter_params.get("pattern_redaction_format"),
                default_on=derouter_params.get("default_on", False),
            )
