...
from derouter.proxy.spend_tracking.key_metadata_recovery import (
    attach_user_emails,
...
from derouter.repositories.table_repositories import DeletedVerificationTokenRepository
...
_PRISMA_TO_PG_TABLE: Final[Mapping[str, str]] = {
    "derouter_dailyuserspend": "DeRouter_DailyUserSpend",
    "derouter_dailyteamspend": "DeRouter_DailyTeamSpend",
    "derouter_dailyorganizationspend": "DeRouter_DailyOrganizationSpend",
    "derouter_dailyenduserspend": "DeRouter_DailyEndUserSpend",
    "derouter_dailyagentspend": "DeRouter_DailyAgentSpend",
    "derouter_dailytagspend": "DeRouter_DailyTagSpend",
}
...
def _adjust_dates_for_timezone(
    ...
    The aggregation table (e.g. DeRouter_DailyUserSpend) stores spend in whole-UTC-day
...
def _ptu_flat_cost_select(table_name: str) -> str:
    """Only DeRouter_DailyTeamSpend carries ptu_flat_cost; other daily tables emit a
    constant zero so the SpendMetrics.flat_cost response shape stays uniform."""
    if table_name == "derouter_dailyteamspend":
...
def _build_aggregated_sql_query(
    *,
    ...
    # only from DeRouter_DailyGatewayRequests. The remaining spend, token and
...
async def get_daily_activity(
    ...
    try:
        ...
        # DeRouter_DailyUserSpend and DeRouter_DailyTeamSpend) as a tiebreaker
