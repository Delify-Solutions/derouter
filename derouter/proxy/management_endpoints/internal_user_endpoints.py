...
These are members of a Team on DeRouter
...
from derouter.proxy.auth.auth_checks import get_team_object, get_user_object
...
from derouter.proxy.utils import handle_exception_on_proxy, hash_password
...
from derouter.types.proxy.management_endpoints.internal_user_endpoints import (
...
if TYPE_CHECKING:
    ...
    from derouter.proxy.proxy_server import PrismaClient
...
) -> "TableActions[prisma_models.DeRouter_UserTable]":
    user_table: Final[TableActions[prisma_models.DeRouter_UserTable]] = UserRepository(prisma_client).table
    return user_table


def _team_table(
    prisma_client: "PrismaClient | None",
) -> "TableActions[prisma_models.DeRouter_TeamTable]":
    team_table: Final[TableActions[prisma_models.DeRouter_TeamTable]] = TeamRepository(prisma_client).table
    return team_table


def _verification_token_table(
    prisma_client: "PrismaClient | None",
) -> "TableActions[prisma_models.DeRouter_VerificationToken]":
    token_table: Final[TableActions[prisma_models.DeRouter_VerificationToken]] = VerificationTokenRepository(
        prisma_client
    ).table
    return token_table
...
def _organization_membership_table(
    ...
    membership_table: Final[TableActions[prisma_models.DeRouter_OrganizationMembership]] = (
...
def _invitation_link_table(
    prisma_client: "PrismaClient | None",
) -> "TableActions[prisma_models.DeRouter_InvitationLink]":
    invitation_table: Final[TableActions[prisma_models.DeRouter_InvitationLink]] = InvitationLinkRepository(
        prisma_client
    ).table
...
def _organization_table(
    prisma_client: "PrismaClient | None",
) -> "TableActions[prisma_models.DeRouter_OrganizationTable]":
    organization_table: Final[TableActions[prisma_models.DeRouter_OrganizationTable]] = OrganizationRepository(
        prisma_client
    ).table
...
def _team_membership_table(
    prisma_client: "PrismaClient | None",
) -> "TableActions[prisma_models.DeRouter_TeamMembership]":
    team_membership_table: Final[TableActions[prisma_models.DeRouter_TeamMembership]] = TeamMembershipRepository(
        prisma_client
    ).table
...
def _update_internal_new_user_params(data_json: dict, data: NewUserRequest) -> dict:
    ...
    if derouter.default_internal_user_params and (
        data.user_role != LitellmUserRoles.PROXY_ADMIN.value and data.user_role != LitellmUserRoles.PROXY_ADMIN
    ):
        for key, value in derouter.default_internal_user_params.items():
    ...
    if data.user_role is not None and data.user_role == LitellmUserRoles.INTERNAL_USER.value:
        if derouter.max_internal_user_budget is not None and data_json.get("max_budget") is None:
            data_json["max_budget"] = derouter.max_internal_user_budget

        if derouter.internal_user_budget_duration is not None and data_json.get("budget_duration") is None:
            data_json["budget_duration"] = derouter.internal_user_budget_duration
    ...
    try:
        ...
        if e.status_code == 400 and ("already exists" in str(e) or "doesn't exist" in str(e)):
            verbose_proxy_logger.debug(
                "derouter.proxy.management_endpoints.internal_user_endpoints.new_user(): User already exists in team - %s",
                e,
            )
        else:
            verbose_proxy_logger.error(
                "derouter.proxy.management_endpoints.internal_user_endpoints._add_user_to_team(): "
...
def check_if_default_team_set() -> list[str] | list[NewUserRequestTeam] | None:
    if derouter.default_internal_user_params is None:
        return None
    teams: Final = derouter.default_internal_user_params.get("teams")
...
async def new_user(
    ...
    Internal Users can access DeRouter Admin UI to make keys, request access to models.
    ...
    - user_role: Optional[str] - Specify a user role - "proxy_admin", "proxy_admin_viewer", "internal_user", "internal_user_viewer", "team", "customer". Info about each role here: `https://github.com/Delify-Solutions/derouter/derouter/proxy/_types.py#L20`
    ...
    - aliases: Optional[dict] - Model aliases for the user - [Docs](https://derouter.vercel.app/docs/proxy/virtual_keys#model-aliases)
    ...
    - model_max_budget: Optional[dict] - Model-specific max budget for user. [Docs](https://router.delify.vn/docs/proxy/users#add-model-specific-budgets-to-keys)
    - budget_fallbacks: Optional[Dict[str, List[str]]] - Per-model fallback chain tried in order when that model's own `model_max_budget` is exceeded, e.g. {"gpt-4o": ["gpt-4o-mini"]}.
    - model_rpm_limit: Optional[float] - Model-specific rpm limit for user. [Docs](https://router.delify.vn/docs/proxy/users#add-model-specific-limits-to-keys)
    ...
    - model_tpm_limit: Optional[float] - Model-specific tpm limit for user. [Docs](https://router.delify.vn/docs/proxy/users#add-model-specific-limits-to-keys)
    ...
    - object_permission: Optional[DeRouter_ObjectPermissionBase] - internal user-specific object permission. Example - {"vector_stores": ["vector_store_1"], "mcp_servers": ["github"], "mcp_tool_permissions": {"github": ["list_issues"]}}. The MCP grants act as a ceiling on every key this user holds. IF null or {} then no object permission.
    ...
    """
    try:
        from derouter.proxy.proxy_server import _license_check, general_settings, prisma_client
...
def get_team_from_list(
    team_list: list[DeRouter_TeamTable] | list[TeamListResponseObject] | None,
    team_id: str,
) -> DeRouter_TeamTable | DeRouter_TeamMembership | None:
...
class _UserInfoDataClient(Protocol):
    @overload
    async def get_data(self, *, user_id: str) -> "prisma_models.DeRouter_UserTable | None": ...
    ...
    ) -> "Sequence[DeRouter_VerificationToken] | None": ...
...
async def _get_user_info_teams(
    prisma_client: "_UserInfoDataClient",
    user_id: str | None,
    user_info: "prisma_models.DeRouter_UserTable",
    user_api_key_dict: UserAPIKeyAuth,
...
def _build_user_info_response(
    user_id: str | None,
    user_info: Any | None,
    keys: Sequence[DeRouter_VerificationToken] | None,
    team_list: list[TeamListResponseObject],
...
async def user_info(
    ...
    ```
    """
    from derouter.proxy.proxy_server import model_max_budget_limiter, prisma_client
    ...
    try:
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.user_info(): Exception occured - %s", e)
        raise handle_exception_on_proxy(e)
...
async def _check_user_info_v2_access(
    ...
    if user_api_key_dict.user_id is not None:
        ...
        if caller_user is not None and caller_user.teams:
            ...
            for team in teams:
                team_obj = DeRouter_TeamTable.model_validate(team.model_dump())
...
async def user_info_v2(
    ...
    try:
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.user_info_v2(): Exception occured - %s", e)
        raise handle_exception_on_proxy(e)
...
async def _get_user_info_for_proxy_admin(user_api_key_dict: UserAPIKeyAuth):
    ...
    - get all teams in DeRouter_TeamTable
    - get all keys in DeRouter_VerificationToken table
    ...
    sql_query: Final = """
        SELECT 
            (SELECT json_agg(t.*) FROM "DeRouter_TeamTable" t) as teams,
            (SELECT json_agg(k.*) FROM "DeRouter_VerificationToken" k WHERE k.team_id != 'derouter-dashboard' OR k.team_id IS NULL) as keys
    """
    if prisma_client is None:
        raise Exception(
            "Database not connected. Connect a database to your proxy - https://router.delify.vn/docs/simple_proxy#managing-auth---virtual-keys"
    ...
    # cast all keys to DeRouter_VerificationToken
    keys_in_db: Final = []
    for key in _keys_in_db:
        key_payload = dict[str, object](key)
        if key_payload.get("models") is None:
            key_payload["models"] = []
        keys_in_db.append(DeRouter_VerificationToken.model_validate(key_payload))

    # cast all teams to DeRouter_TeamTable
    _teams_rows: Final[Sequence[Mapping[str, object]]] = results[0]["teams"] or []
    _teams_in_db: Final = sorted(
        (DeRouter_TeamTable.model_validate(team) for team in _teams_rows),
...
def _process_keys_for_user_info(
    keys: Sequence[DeRouter_VerificationToken] | None,
    all_teams: list[DeRouter_TeamTable] | list[TeamListResponseObject] | None,
):
    from derouter.constants import UI_SESSION_TOKEN_TEAM_ID
    from derouter.proxy.proxy_server import general_settings, derouter_master_key_hash
    ...
    if keys is None:
        ...
        for key in keys:
            if (
                key.token == derouter_master_key_hash
            ...
            # Filter out UI session tokens (team_id="derouter-dashboard")
            if _key.get("team_id") == UI_SESSION_TOKEN_TEAM_ID:
                continue

            if "team_id" in _key and _key["team_id"] is not None and _key["team_id"] != "derouter-dashboard":
...
def _update_internal_user_params(data_json: dict, data: UpdateUserRequest | UpdateUserRequestNoUserIDorEmail) -> dict:
    ...
    for k, v in data_json.items():
        if k == "max_budget":
            ...
            and k not in DeRouter_ManagementEndpoint_MetadataFields
    ...
    if "max_budget" not in non_default_values:
        if (
            is_internal_user and derouter.max_internal_user_budget is not None
        ):  # applies internal user limits, if user role updated
            non_default_values["max_budget"] = derouter.max_internal_user_budget

    if "budget_duration" not in non_default_values:  # applies internal user limits, if user role updated
        if is_internal_user and derouter.internal_user_budget_duration is not None:
            non_default_values["budget_duration"] = derouter.internal_user_budget_duration
            from derouter.proxy.common_utils.timezone_utils import get_budget_reset_time
...
async def _schedule_user_update_audit_log(
    ...
    user_api_key_dict: UserAPIKeyAuth,
    derouter_proxy_admin_name: str | None,
) -> None:
    from derouter.proxy.proxy_server import prisma_client
    ...
    try:
        ...
        if updated_user_row:
            user_row_typed: Final = DeRouter_UserTable.model_validate(updated_user_row.model_dump(exclude_none=True))
            asyncio.create_task(
                UserManagementEventHooks.create_internal_user_audit_log(
                    user_id=user_row_typed.user_id,
                    action="updated",
                    derouter_changed_by=derouter_changed_by or user_api_key_dict.user_id,
                    user_api_key_dict=user_api_key_dict,
...
def _check_user_update_authz(
    ...
    if existing_user_row is not None:
        typed_row: Final = DeRouter_UserTable.model_validate(existing_user_row.model_dump(exclude_none=True))
...
async def _invalidate_user_spend_counter_if_changed(
    ...
    if non_default_values.get("spend") is not None:
        from derouter.proxy.proxy_server import _invalidate_spend_counter
...
async def _update_single_user_helper(
    ...
    """
    from derouter.proxy.proxy_server import general_settings, derouter_proxy_admin_name, prisma_client
    ...
    if existing_user_row is not None:
        existing_user_row = DeRouter_UserTable.model_validate(existing_user_row.model_dump(exclude_none=True))
...
async def user_update(
    ...
    try:
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.user_update(): Exception occured - %s", e)
...
async def get_users(
    ...
    users: Final[Sequence[prisma_models.DeRouter_UserTable]] = await UserRepository(prisma_client).table.find_many(
        where=where,
        skip=skip,
        take=page_size,
    ...
    # Prepare response
    user_list: list[DeRouter_UserTableWithKeyCount] = []
    for user in users:
        user_dump = user.model_dump()
        user_dump["metadata"] = _redact_scim_enterprise_metadata(user_dump.get("metadata"))
        user_list.append(
            DeRouter_UserTableWithKeyCount.model_validate(
...
async def delete_user(
    ...
    if not caller_is_proxy_admin:
        caller_memberships: Final[Sequence[prisma_models.DeRouter_OrganizationMembership]] = (
    ...
    for user_id in data.user_ids:
        ...
        fetch_all_teams: Sequence[prisma_models.DeRouter_TeamTable] = await TeamRepository(
            prisma_client
        ...
        for team in fetch_all_teams:
            removed_team_members, new_team_members = _cleanup_members_with_roles(
                existing_team_row=DeRouter_TeamTable.model_validate(team.model_dump()),
...
async def add_internal_user_to_organization(
...
) -> "prisma_models.DeRouter_OrganizationMembership":
    """
    Helper function to add an internal user to an organization

    Adds the user to DeRouter_OrganizationMembership table
    ...
    try:
        ...
        new_membership: Final[prisma_models.DeRouter_OrganizationMembership] = await OrganizationMembershipRepository(
            prisma_client
        ).table.create(
            data={
                "user_id": user_id,
...
@router.get(
    ...
    include_in_schema=False,
    responses={
        200: {"model": list[DeRouter_UserTableFiltered]},
    },
)
async def ui_view_users(
    ...
    (stored in the ``derouter_uisettings`` table):
    ...
    * **Flag ON:**
    ...
    try:
        ...
        # Build where conditions based on provided parameters
        where_conditions: Final[prisma_types.DeRouter_UserTableWhereInput] = {}
        ...
        return [DeRouter_UserTableFiltered.model_validate(user.model_dump()) for user in users]
...
async def get_user_daily_activity(
    ...
    try:
        ...
        return await get_daily_activity(
            prisma_client=prisma_client,
            table_name="derouter_dailyuserspend",
