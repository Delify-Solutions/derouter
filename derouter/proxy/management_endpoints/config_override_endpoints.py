...
from derouter.repositories.table_repositories import ConfigOverridesRepository
...
def _redact_config(config: Mapping[str, object] | None) -> dict[str, str]:
    ...
    ``client_key`` etc.  Persisting them verbatim into ``DeRouter_AuditLogs``
...
async def _emit_config_override_audit_log(
    *,
    ...
    ``DeRouter_ConfigOverrides`` so the row co-locates with the table it
    mutates.
    """
    ...
    from derouter.proxy.proxy_server import derouter_proxy_admin_name
    ...
    task: Final = asyncio.create_task(
        create_audit_log_for_update(
            request_data=DeRouter_AuditLogs(
                ...
                changed_by=derouter_changed_by or user_api_key_dict.user_id or derouter_proxy_admin_name,
...
def _clear_hashicorp_vault_state(proxy_config: "ProxyConfig") -> None:
    ...
    _set_env_vars({})
    if derouter._key_management_system == KeyManagementSystem.HASHICORP_VAULT:
        derouter.secret_manager_client = None
        derouter._key_management_system = None
...
def _restore_cyberark_runtime(proxy_config: "ProxyConfig", env_values: Mapping[str, str | None]) -> None:
    ...
    if env_values.get("cyberark_api_base"):
        try:
            ...
            return
    if derouter._key_management_system != KeyManagementSystem.CYBERARK:  # pyright: ignore[reportPrivateUsage]  # proxy-internal helper, mirrors hashicorp endpoint usage
        return
    derouter.secret_manager_client = None
    derouter._key_management_system = None  # pyright: ignore[reportPrivateUsage]  # proxy-internal helper, mirrors hashicorp endpoint usage
...
async def test_hashicorp_vault_connection(
    ...
    client: Final = derouter.secret_manager_client
