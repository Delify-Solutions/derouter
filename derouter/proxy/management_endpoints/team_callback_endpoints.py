...
from derouter.proxy.common_utils.callback_config_validation import (
...
def _redact_callback_secrets(metadata: object) -> object:
    ...
    ``DeRouter_AuditLogs`` would let anyone with read access to the audit
...
async def add_team_callbacks(
    ...
    - callback_vars (StandardCallbackDynamicParams, required): A dictionary of variables to pass to the callback
        ...
        - newrelic_api_key: The ingest license key for the team's New Relic account; routes both LLM/agent traces and cost metrics to that account. Requires the proxy to run with DEROUTER_OTEL_V2=true, otherwise this callback is rejected with a 400
    ...
    try:
        ...
        await _verify_team_access(
            team_obj=DeRouter_TeamTable(**_existing_team.model_dump()),
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.add_team_callbacks(): Exception occured - %s", e)
        raise ProxyException(
...
async def delete_team_callback(
    ...
    try:
        ...
        verbose_proxy_logger.error("derouter.proxy.proxy_server.delete_team_callback(): Exception occurred - %s", e)
...
async def disable_team_logging(
    ...
    try:
        ...
        verbose_proxy_logger.error("derouter.proxy.proxy_server.disable_team_logging(): Exception occurred - %s", e)
        verbose_proxy_logger.debug(traceback.format_exc())
        raise ProxyException(
            message="Internal Server Error, " + str(e),
            type=ProxyErrorTypes.internal_server_error.value,
...
async def get_team_callbacks(
    ...
    try:
        ...
        verbose_proxy_logger.error("derouter.proxy.proxy_server.get_team_callbacks(): Exception occurred - %s", e)
        verbose_proxy_logger.debug(traceback.format_exc())
