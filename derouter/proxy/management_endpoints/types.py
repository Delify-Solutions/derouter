...
DEROUTER_USER_ROLE_HIERARCHY: Final = (
...
def highest_privilege_role(roles: Iterable[LitellmUserRoles]) -> LitellmUserRoles | None:
    ...
    ranked: Final = next((role for role in DEROUTER_USER_ROLE_HIERARCHY if role in resolved), None)
    return ranked if ranked is not None else min(resolved, key=lambda role: role.value)


def is_valid_derouter_user_role(role_str: str) -> bool:
    """
...
def get_derouter_user_role(role_str: object) -> LitellmUserRoles | None:
    """
