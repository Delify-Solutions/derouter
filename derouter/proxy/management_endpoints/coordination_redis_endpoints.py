...
from derouter.types.management_endpoints import (
...
def _coordination_redis_source(settings: Mapping[str, object] | None) -> CoordinationRedisSource | None:
    ...
    """
    from derouter.proxy.proxy_server import _environment_has_redis_connection_target
...
async def update_coordination_redis_settings(
    ...
    The settings are written to the `general_settings` row of DeRouter_Config,
    which startup merges over the yaml config; the proxy picks them up on its
    next restart.
    """
    from derouter.proxy.proxy_server import prisma_client, store_model_in_db
...
async def check_coordination_redis_connection(
    ...
    """
    from derouter.proxy.proxy_server import _build_redis_usage_cache
