...
from derouter.proxy._types import UserAPIKeyAuth, user_api_key_has_admin_view
...
from derouter.proxy.management_helpers.utils import handle_budget_for_entity
...
from derouter.types.tag_management import (
    TagConfig,
    TagDeleteRequest,
    TagInfoRequest,
    TagNewRequest,
...
if TYPE_CHECKING:
    ...
    from prisma.models import DeRouter_ProxyModelTable as PrismaProxyModelTable
    from prisma.models import DeRouter_TagTable as PrismaTagTable
...
class _TagRecord(Protocol):
    ...
    updated_at: datetime
    created_by: str | None
    derouter_budget_table: "PrismaBudgetTable | None"
...
async def get_deployments_by_model(model: str, llm_router: "Router") -> list["Deployment"]:
    ...
    return [
        Deployment(
            ...
            derouter_params=DeRouter_Params(**deployment["derouter_params"]),
...
async def _add_tag_to_deployment(deployment: "Deployment", tag: str):
    ...
    try:
        ...
        # Prisma returns derouter_params as dict (already parsed from JSON)
        ...
        if isinstance(existing_params, str):  # pyright: ignore[reportUnnecessaryIsInstance]  # prisma Json stub is str
            ...
            raise Exception(f"Unexpected derouter_params type: {type(existing_params)}")
        ...
        await ModelRepository(prisma_client).table.update(
            ...
            data={"derouter_params": json.dumps(existing_params)},
        )
    except Exception as e:
...
async def info_tag(
    ...
    try:
        ...
        for tag_record in tag_records:
            ...
            if hasattr(tag_record, "derouter_budget_table") and tag_record.derouter_budget_table:
                tag_dict["derouter_budget_table"] = tag_record.derouter_budget_table
...
async def get_tag_daily_activity(
    ...
    return await get_daily_activity(
        ...
        page=page,
        page_size=page_size,
        # metadata_metrics_func=None because derouter_dailytagspend rows are
