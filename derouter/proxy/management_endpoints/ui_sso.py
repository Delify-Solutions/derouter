...
/sso/callback - returns JWT Redirect Response that redirects to DeRouter UI
...
from derouter.derouter_core_utils.dot_notation_indexing import get_nested_value
...
from derouter.proxy.auth.auth_checks import ExperimentalUIJWTToken, get_user_object
...
from derouter.proxy.common_utils.admin_ui_utils import (
    admin_ui_disabled,
    show_missing_vars_in_env,
)
from derouter.proxy.common_utils.html_forms.jwt_display_template import (
...
from derouter.proxy.management_endpoints.internal_user_endpoints import new_user
from derouter.proxy.management_endpoints.sso import CustomMicrosoftSSO
from derouter.proxy.management_endpoints.sso.id_jag_assertion_capture import (
    id_jag_assertion_capture_gap,
)
from derouter.proxy.management_endpoints.sso.saml_sso import SAMLAuthHandler
from derouter.proxy.management_endpoints.sso_helper_utils import (
    check_is_admin_only_access,
    has_admin_ui_access,
)
from derouter.proxy.management_endpoints.team_endpoints import new_team, team_member_add
from derouter.proxy.management_endpoints.types import (
    DEROUTER_USER_ROLE_HIERARCHY,
    CustomOpenID,
    get_derouter_user_role,
    is_valid_derouter_user_role,
)
...
from derouter.repositories.table_repositories import SSOConfigRepository
...
from derouter.secret_managers.main import get_secret_bool, get_secret_str, str_to_bool
from derouter.types.proxy.management_endpoints.ui_sso import *  # noqa: F403
...
from derouter.types.proxy.ui_sso import ParsedOpenIDResult
...
class _TeamRowGrants(BaseModel):
    ...
    models: tuple[str, ...] = ()
    derouter_model_table: _TeamModelAliasTable | None = None
...
def _get_cli_sso_flow_or_raise(login_id: str | None, cache: DualCache) -> dict:
    if isinstance(login_id, str) and login_id.startswith("sk-"):
        raise HTTPException(
            status_code=400,
            detail=(
                "Your derouter CLI is out of date and uses a login flow this proxy no longer supports. "
                "Upgrade it with `pip install -U 'derouter[proxy]'` and run `derouter-proxy login` again."
            ),
        )
    ...
    if not isinstance(flow, dict) or "poll_secret_hash" not in flow:
        ...
        raise HTTPException(
            status_code=400,
            detail=(
                "CLI login session not found or expired. Run `derouter-proxy login` again. "
...
def _parse_cli_sso_claim_map() -> list[tuple[str, str]]:
    """
    Parse CLI_SSO_CLAIM_MAP / DEROUTER_CLI_SSO_CLAIM_MAP.
    ...
    on the DeRouter user's ``metadata`` JSON column.
    """
...
def build_cli_sso_attribution_metadata(
    ...
    Sources are configured via CLI_SSO_CLAIM_MAP / DEROUTER_CLI_SSO_CLAIM_MAP and
...
def _render_cli_sso_verification_page(
    ...
    <html>
      <head>
        <title>DeRouter CLI Login</title>
        <style>
          body {{
...
async def cli_sso_start(request: Request):
    from derouter.proxy.proxy_server import cli_sso_session_cache, general_settings
...
async def cli_sso_complete(request: Request, login_id: str):
    ...
    from derouter.proxy.proxy_server import cli_sso_session_cache
...
def determine_role_from_groups(
    ...
    for role in DEROUTER_USER_ROLE_HIERARCHY:
        ...
        if existing_role is None:
            ...
            if user_role is None:
                ...
                if user_role_from_token is not None:
                    user_role = get_derouter_user_role(user_role_from_token)
                    verbose_proxy_logger.debug(
...
async def _raise_if_sso_exceeds_free_user_limit(premium_user: bool, prisma_client: PrismaClient | None) -> None:
    ...
    if billable_users and billable_users > 5:
        raise ProxyException(
            message="You must be a DeRouter Enterprise user to use SSO for more than 5 users. If you have a license please set `DEROUTER_LICENSE` in your env. If you want to obtain a license meet with us here: https://enterprise.router.delify.vn/demo You are seeing this error message because You configured SSO (one of `MICROSOFT_CLIENT_ID`, `GOOGLE_CLIENT_ID`, `GENERIC_CLIENT_ID`, or SAML) in your env. Please unset it",
            type=ProxyErrorTypes.auth_error,
            param="premium_user",
            code=status.HTTP_403_FORBIDDEN,
        )
...
async def google_login(
    ...
    if source == DEROUTER_CLI_SOURCE_IDENTIFIER:
...
def generic_response_convertor(
    ...
    if user_role is None:
        ...
        if user_role_from_sso is not None:
            role: Final = get_derouter_user_role(user_role_from_sso)
            if role is not None:
                user_role = role
                verbose_proxy_logger.debug(
...
async def _setup_team_mappings() -> Optional["TeamMappings"]:
    ...
    from derouter.types.proxy.management_endpoints.ui_sso import TeamMappings

    team_mappings: TeamMappings | None = None
    try:
        from derouter.proxy.utils import get_prisma_client_or_throw
...
def _handle_generic_sso_error(
    ...
    if not pkce_configured and ("PKCE" in error_message or "code verifier" in error_message.lower()):
        ...
        detailed_message = (
            ...
            f"but it's not enabled in your DeRouter configuration.\n\n"
...
async def get_generic_sso_response(
    ...
    try:
        ...
        if code_verifier:
            ...
            # against the ``derouter_oauth_state`` cookie set on the
            ...
            cookie_state: Final = request.cookies.get("derouter_oauth_state")
...
async def add_missing_team_member(user_info: NewUserResponse | DeRouter_UserTable, sso_teams: list[str]):
    """
...
def get_disabled_non_admin_personal_key_creation():
    key_generation_settings: Final = derouter.key_generation_settings
...
) -> DeRouter_UserTable | NewUserResponse | None:
    try:
        ...
        user_info: DeRouter_UserTable | NewUserResponse | None = None
        ...
        verbose_proxy_logger.debug(
            "user_info: %s; derouter.default_internal_user_params: %s", user_info, derouter.default_internal_user_params
        )

        # Upsert SSO User to DeRouter DB
...
def _should_use_role_from_sso_response(sso_role: str | None) -> bool:
    ...
    if not is_valid_derouter_user_role(sso_role):
        verbose_proxy_logger.debug(
            "SSO role '%s' is not a valid DeRouter user role. Ignoring role from SSO response. See LitellmUserRoles enum for valid roles.",
            sso_role,
        )
        return False
    return True
...
def _build_sso_user_update_data(
    ...
    if sso_role is not None:
        ...
        # Only include if it's a valid DeRouter role
...
async def _sync_user_role_from_jwt_role_map(
    ...
    user_info: DeRouter_UserTable | NewUserResponse | None,
    prisma_client: PrismaClient,
    user_api_key_cache: UserApiKeyCache,
    user_defined_values: SSOUserDefinedValues | None,
) -> None:
    """
    Apply jwt_derouter_role_map during SSO login.

    When jwt_derouter_role_map is configured with sync_user_role_and_teams=True,
    ...
    if jwt_handler is None or received_response is None:
        return
    if not jwt_handler.derouter_jwtauth.sync_user_role_and_teams:
        return
    if not jwt_handler.derouter_jwtauth.jwt_derouter_role_map:
        return

    mapped_role: Final = jwt_handler.map_jwt_role_to_derouter_role(received_response)
    if mapped_role is None:
        return

    verbose_proxy_logger.info("SSO jwt_derouter_role_map matched role: %s", mapped_role.value)
...
async def auth_callback(request: Request, state: str | None = None):
    ...
    if state and state.startswith(f"{DEROUTER_CLI_SESSION_TOKEN_PREFIX}:"):
    ...
    cp_return_to: Final[str | None] = request.cookies.get("derouter_cp_return_to")
...
async def saml_callback(request: Request):
    ...
    if master_key is None:
        raise ProxyException(
            message="Master Key not set for Proxy. Set `DEROUTER_MASTER_KEY` in .env or general_settings:master_key in config.yaml.",
            type=ProxyErrorTypes.auth_error,
            param="master_key",
            code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
async def _build_cli_sso_user_defined_values(
    ...
    from derouter.proxy.proxy_server import user_custom_sso
...
async def cli_poll_key(
    key_id: str,
    team_id: str | None = None,
    x_derouter_cli_poll_secret: str | None = Header(default=None),
):
    """
    ...
    try:
        ...
        if not _verify_cli_sso_poll_secret(flow=flow, poll_secret=x_derouter_cli_poll_secret):
        ...
        if isinstance(session_data, dict):
            ...
            user_info: Final = DeRouter_UserTable(
                user_id=user_id,
...
async def insert_sso_user(
    ...
    """
    Helper function to create a New User in DeRouter DB after a successful SSO login

    Args:
        result_openid (OpenID): User information in OpenID format if the login was successful.
        user_defined_values (Optional[SSOUserDefinedValues], optional): DeRouter SSOValues / fields that were read
    ...
    if derouter.default_internal_user_params:
        # Preserve the SSO-extracted role if it's a valid DeRouter role,
        ...
        if _should_use_role_from_sso_response(sso_role):
            ...
            user_defined_values.update(derouter.default_internal_user_params)
    ...
    if user_defined_values.get("user_role") == LitellmUserRoles.INTERNAL_USER.value:
        if user_defined_values.get("max_budget") is None:
            user_defined_values["max_budget"] = derouter.max_internal_user_budget
        if user_defined_values.get("budget_duration") is None:
            user_defined_values["budget_duration"] = derouter.internal_user_budget_duration
...
async def get_ui_settings(request: Request):
    from derouter.proxy.proxy_server import general_settings, proxy_state

    _proxy_base_url: Final = os.getenv("PROXY_BASE_URL", None)
    _logout_url: Final = os.getenv("PROXY_LOGOUT_URL", None)
    _api_doc_base_url: Final = os.getenv("DEROUTER_UI_API_DOC_BASE_URL", None)
    ...
    return {
        ...
        "DEROUTER_UI_API_DOC_BASE_URL": _api_doc_base_url,
...
def _is_same_origin_return_path(return_to: str) -> bool:
    ...
    of the redirect ``Location`` and the ``derouter_cp_return_to`` cookie entirely, rather
...
async def _sso_return_to_redirect(
    ...
    Two arms, both clearing the one-shot ``derouter_cp_return_to`` cookie:
    ...
    if _is_same_origin_return_path(return_to):
        ...
        redirect_response.delete_cookie("derouter_cp_return_to")
...
def set_session_token_cookie(response: Response, request: Request, jwt_token: str) -> None:
    ...
    ``ui/derouter-dashboard/src/utils/cookieUtils.ts``), so marking it
...
def _persist_return_to_cookie(response: Response, return_to: str | None, request: Request) -> None:
    """Best-effort: persist a SAFE ``return_to`` on ``response`` as the one-shot ``derouter_cp_return_to``
...
class SSOAuthenticationHandler:
    ...
    async def get_sso_login_redirect(
        ...
        Args:
            ...
            request: Optional FastAPI request, used to drive the ``Secure``
                attribute on the ``derouter_oauth_state`` CSRF cookie.
        ...
        raise ValueError(
            "Unknown SSO provider. Please setup SSO with client IDs https://router.delify.vn/docs/proxy/admin_ui_sso"
        )
    ...
    async def get_generic_sso_redirect_response(
        ...
        from derouter.proxy.proxy_server import redis_usage_cache, user_api_key_cache

        with generic_sso:
            # State is bound to the caller's browser via a ``derouter_oauth_state``
    ...
    def _get_generic_sso_redirect_params(
        ...
        if state:
            ...
            # the derouter proxy cli sends the "state" parameter to the proxy server for auth. We should maintain the state parameter for the cli if it is provided
    ...
    async def upsert_sso_user(
        ...
        user_info: NewUserResponse | DeRouter_UserTable | None,
        user_email: str | None,
        user_defined_values: SSOUserDefinedValues | None,
        prisma_client: PrismaClient,
    ):
        """
        Connects the SSO Users to the User Table in DeRouter DB

        - If user on DeRouter DB, update the user_email and user_role (if SSO provides valid role) with the SSO values
        - If user not on DeRouter DB, insert the user into DeRouter DB
        """
        try:
            if user_info is not None:
                ...
                verbose_proxy_logger.info("user not in DB, inserting user into DeRouter DB")
                # user not in DB, insert User into DeRouter DB
            ...
            verbose_proxy_logger.exception("Error upserting SSO user into DeRouter DB: %s", e)
            return user_info
    ...
    async def add_user_to_teams_from_sso_response(
        ...
        The `team_ids` field is populated by derouter after processing the SSO response
        """
        if user_info is None:
            verbose_proxy_logger.debug("User not found in DeRouter DB, skipping team member addition")
            return
    ...
    @staticmethod
    async def create_derouter_team_from_sso_group(
        derouter_team_id: str,
        derouter_team_name: str | None = None,
    ):
        """
        Creates a Litellm Team from a SSO Group ID

        Your SSO provider might have groups that should be created on DeRouter

        Use this helper to create a Litellm Team from a SSO Group ID

        Args:
            derouter_team_id (str): The ID of the Litellm Team
            derouter_team_name (Optional[str]): The name of the Litellm Team
        """
        ...
        try:
            team_obj: Final = await _team_detail_db(TeamRepository(prisma_client)).find_first(
                where={"team_id": derouter_team_id}
            )
            verbose_proxy_logger.debug("Team object: %s", team_obj)

            # only create a new team if it doesn't exist
            if team_obj:
                verbose_proxy_logger.debug("Team already exists: %s - %s", derouter_team_id, derouter_team_name)
                return

            team_request: NewTeamRequest = NewTeamRequest(
                team_id=derouter_team_id,
                team_alias=derouter_team_name,
            )
            if derouter.default_team_params:
                team_request = SSOAuthenticationHandler._cast_and_deepcopy_derouter_default_team_params(
                    default_team_params=derouter.default_team_params,
                    derouter_team_id=derouter_team_id,
                    derouter_team_name=derouter_team_name,
            ...
            await new_team(
                ...
                user_api_key_dict=UserAPIKeyAuth(
                    token="",
                    key_alias=f"derouter.{MicrosoftSSOHandler.__name__}",
                ),
            )
        except Exception as e:
    ...
    @staticmethod
    def _cast_and_deepcopy_derouter_default_team_params(
        ...
        """
        Casts and deepcopies the derouter.default_team_params to a NewTeamRequest object

        - Ensures we create a new DefaultTeamSSOParams object
        - Handle the case where derouter.default_team_params is a dict or a DefaultTeamSSOParams object
        - Adds the derouter_team_id and derouter_team_name to the DefaultTeamSSOParams object
        """
        if isinstance(default_team_params, dict):
            _team_request: Final = deepcopy(default_team_params)
            _team_request["team_id"] = derouter_team_id
            _team_request["team_alias"] = derouter_team_name
            team_request = NewTeamRequest(**_team_request)
        elif isinstance(derouter.default_team_params, DefaultTeamSSOParams):
            _default_team_params: Final = deepcopy(derouter.default_team_params)
    ...
    def _get_cli_state(
        ...
        if source == DEROUTER_CLI_SOURCE_IDENTIFIER and key:
            if _is_valid_cli_sso_user_code(user_code):
                return f"{DEROUTER_CLI_SESSION_TOKEN_PREFIX}:{key}:{user_code}"
            return f"{DEROUTER_CLI_SESSION_TOKEN_PREFIX}:{key}"
        else:
            return None
    ...
    async def get_redirect_response_from_openid(
        ...
        max_internal_user_budget: Final = derouter.max_internal_user_budget
        internal_user_budget_duration: Final = derouter.internal_user_budget_duration
        ...
        # Sync user role from JWT claims via jwt_derouter_role_map (if configured).
        ...
        if user_defined_values is None:
            raise Exception(
                "Unable to map user identity to known values. 'user_defined_values' is None. File an issue - https://github.com/Delify-Solutions/derouter/issues"
            )
        ...
        derouter_dashboard_ui = get_custom_url(request_base_url=str(request.base_url), route="ui/")

        if get_secret_bool("EXPERIMENTAL_UI_LOGIN"):
            _user_info: DeRouter_UserTable | None = None
            if user_defined_values is not None and user_defined_values["user_id"] is not None:
                _user_info = DeRouter_UserTable(
                    user_id=user_defined_values["user_id"],
        ...
        from derouter.proxy.auth.login_utils import encode_ui_session_jwt
        ...
        if user_id is not None and isinstance(user_id, str):
            derouter_dashboard_ui += "?login=success"
        verbose_proxy_logger.info("Redirecting to %s", derouter_dashboard_ui)
...
class MicrosoftSSOHandler:
    ...
    # used for debugging to show the user groups derouter found from Graph API
    ...
    def get_user_role_from_app_roles(
        ...
        """
        Resolve the one role DeRouter stores for a user from their Entra app roles.
        ...
        """
        return get_derouter_user_role(tuple(app_roles or ()))
    ...
    @staticmethod
    async def create_derouter_teams_from_service_principal_team_ids(
        service_principal_teams: list[MicrosoftServicePrincipalTeam],
    ):
        """
        Creates Litellm Teams from the Service Principal Group IDs

        When a user sets a `SERVICE_PRINCIPAL_ID` in the env, derouter will fetch groups under that service principal and create Litellm Teams from them
        """
        ...
        for service_principal_team in service_principal_teams:
            derouter_team_id: str | None = service_principal_team.get("principalId")
            derouter_team_name: str | None = service_principal_team.get("principalDisplayName")
            if not derouter_team_id:
                verbose_proxy_logger.debug(
                    "Skipping team creation for %s because it has no principalId", derouter_team_name
                )
                continue

            await SSOAuthenticationHandler.create_derouter_team_from_sso_group(
                derouter_team_id=derouter_team_id,
                derouter_team_name=derouter_team_name,
            )
...
async def debug_sso_login(request: Request):
    ...
    if microsoft_client_id is not None or google_client_id is not None or generic_client_id is not None:
        if premium_user is not True:
            raise ProxyException(
                message="You must be a DeRouter Enterprise user to use SSO. If you have a license please set `DEROUTER_LICENSE` in your env. If you want to obtain a license meet with us here: https://enterprise.router.delify.vn/demo You are seeing this error message because You set one of `MICROSOFT_CLIENT_ID`, `GOOGLE_CLIENT_ID`, or `GENERIC_CLIENT_ID` in your env. Please unset this",
                type=ProxyErrorTypes.auth_error,
                param="premium_user",
                code=status.HTTP_403_FORBIDDEN,
            )
