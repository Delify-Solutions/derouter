"""
SAML 2.0 SSO for the DeRouter proxy admin UI.
...
from derouter.proxy.management_endpoints.types import CustomOpenID, get_derouter_user_role
...
_SAML_AUTHN_STATE_COOKIE: Final = "derouter_saml_authn"
...
_ROLE_ATTRIBUTE_CANDIDATES: Final = ("role", "roles", "derouter_role")
...
def _saml_unavailable_error() -> HTTPException:
    return HTTPException(
        ...
        detail=(
            ...
            "not installed. Re-install derouter with the saml extra: "
            "'pip install derouter[saml]'. The saml extra bundles the native "
    ...
    def _result_from_auth(auth: "OneLogin_Saml2_Auth") -> CustomOpenID:
        ...
        try:
            return CustomOpenID(
                ...
                provider="saml",
                team_ids=team_ids,
                user_role=get_derouter_user_role(role_values),
            )
