...
# Defined above the `derouter.proxy.*` imports so the name is bound even when
...
from derouter.proxy._types import (  # noqa: F401  re-exported
...
if TYPE_CHECKING:
    from derouter.proxy._types import NewProjectRequest, UpdateProjectRequest
...
def _is_user_team_admin(user_api_key_dict: UserAPIKeyAuth, team_obj: DeRouter_TeamTable) -> bool:
...
async def _is_user_org_admin_for_team(user_api_key_dict: UserAPIKeyAuth, team_obj: DeRouter_TeamTable) -> bool:
    """
...
async def _user_has_admin_privileges(
    ...
    from derouter.caching import DualCache as DualCacheImport
    from derouter.proxy.auth.auth_checks import get_user_object
...
def _org_admin_can_invite_user(
    admin_user_obj: DeRouter_UserTable,
    target_user_obj: DeRouter_UserTable,
) -> bool:
    """
...
def _set_object_metadata_field(
    ...
    """
    if field_name in DeRouter_ManagementEndpoint_MetadataFields_Premium and value:
...
async def _upsert_budget_and_membership(
    ...
    *,
    ...
    async def _disconnect():
        await tx.derouter_teammembership.update(
            where={"user_id_team_id": {"user_id": user_id, "team_id": team_id}},
            data={"derouter_budget_table": {"disconnect": True}},
        )

    if existing_budget_id is not None and not is_shared_default:
        existing_budget: Final = await tx.derouter_budgettable.find_unique(where={"budget_id": existing_budget_id})
    ...
    if is_shared_default:
        default_budget_row: Final = await tx.derouter_budgettable.find_unique(where={"budget_id": existing_budget_id})
    ...
    new_budget: Final = await tx.derouter_budgettable.create(
        data=create_data,
        include={"team_membership": True},
    )
    await tx.derouter_teammembership.upsert(
        where={
            "user_id_team_id": {
                "user_id": user_id,
                "team_id": team_id,
...
def _update_metadata_field(updated_kv: dict, field_name: str) -> None:
    ...
    """
    if field_name in DeRouter_ManagementEndpoint_MetadataFields_Premium:
