...
from derouter.repositories.table_repositories import ModelAccessGroupBudgetRepository
...
class _BudgetInclude(TypedDict):
    derouter_budget_table: ReadOnly[bool]
...
class _ModelAccessGroupBudgetRow(Protocol):
    ...
    @property
    def derouter_budget_table(self) -> _BudgetRow | None: ...
...
async def _model_access_group_budget_row(
    ...
    include: Final[_BudgetInclude] = {"derouter_budget_table": True}
...
def _budget_or_none(row: _ModelAccessGroupBudgetRow | None) -> AccessGroupBudget | None:
    budget: Final = row.derouter_budget_table if row is not None else None
    if budget is None:
        return None
...
async def _delete_model_access_group_budget_row(
    ...
    """
    Drop the group's budget row only, matching /tag/delete: the DeRouter_BudgetTable row survives
...
async def delete_access_group(
    ...
    try:
        ...
        # The DeRouter_BudgetTable row it linked is left alone, as /tag/delete leaves a tag's.
...
async def set_access_group_budget(
    ...
    actor: Final = user_api_key_dict.user_id or derouter_proxy_admin_name
