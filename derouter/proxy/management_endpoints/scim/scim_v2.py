"""
✨ SCIM v2 Endpoints for DeRouter Proxy using Internal User/Team Management
...
from derouter.models.user import SCIMPlaceholder
...
if TYPE_CHECKING:
    from prisma.models import DeRouter_VerificationToken as PrismaVerificationToken


class _UserTableClient(Protocol):
    async def find_first(self, where: Mapping[str, object]) -> DeRouter_UserTable | None: ...

    async def find_unique(self, where: Mapping[str, object]) -> DeRouter_UserTable | None: ...
    ...
    ) -> Sequence[DeRouter_UserTable]: ...

    async def update(self, where: Mapping[str, object], data: Mapping[str, object]) -> DeRouter_UserTable: ...

    async def delete(self, where: Mapping[str, object]) -> DeRouter_UserTable | None: ...

    async def count(self, where: Mapping[str, object] | None = None) -> int: ...


class _TeamTableClient(Protocol):
    async def find_unique(self, where: Mapping[str, object]) -> DeRouter_TeamTable | None: ...
    ...
    async def find_many(
        ...
        take: int | None = None,
        order: Mapping[str, str] | None = None,
    ) -> Sequence[DeRouter_TeamTable]: ...

    async def update(self, where: Mapping[str, object], data: Mapping[str, object]) -> DeRouter_TeamTable: ...

    async def delete(self, where: Mapping[str, object]) -> DeRouter_TeamTable | None: ...
...
class UserProvisionerHelpers:
    ...
    async def handle_existing_user_by_email(
        ...
        return await ScimTransformations.transform_derouter_user_to_scim_user(updated_user)
...
async def _check_user_exists(user_id: str) -> DeRouter_UserTable:
...
async def _check_team_exists(team_id: str) -> DeRouter_TeamTable:
...
def _build_scim_metadata(
    ...
    metadata: Final[dict[str, object]] = {
        "scim_metadata": DeRouter_UserScimMetadata(
...
async def _get_scim_upsert_user_setting() -> bool:
    """
    Get the scim_upsert_user setting from derouter_settings.
    ...
    try:
        ...
        scim_upsert_user: Final = derouter_settings.get("scim_upsert_user", True)
...
def _default_scim_user_role() -> ScimUserRole:
    ...
    if derouter.default_internal_user_params:
        configured_role: Final = derouter.default_internal_user_params.get("user_role")
...
async def _get_scim_admin_group() -> str | None:
    """
    Get the scim_admin_group setting from derouter_settings.
    ...
    try:
        ...
        config: Final = await proxy_config.get_config()
        derouter_settings: Final = config.get("derouter_settings", {}) or {}
        return derouter_settings.get("scim_admin_group") or None
    except Exception as e:
...
async def _classify_group_member(member: SCIMMember, prisma_client: PrismaClient) -> _ClassifiedGroupMember:
    ...
    A DeRouter team only holds users, so a member is dropped when it declares a type
    ...
    if undecidable:
        raise HTTPException(
            ...
            detail={
                ...
                "cannot be attributed. Send the DeRouter user ID as the member value, or resolve the duplicate."
            },
        )
...
async def _resolve_group_member_ids(
    ...
    """
    Resolve SCIM group members to DeRouter user ids, dropping members that are not users.

    Member ids are matched by ``user_id`` first, then by SSO identity or email. An
    id that resolves to nothing is created when derouter_settings.scim_upsert_user is
    ...
    for skipped in partition.skipped:
        verbose_proxy_logger.info(
            "SCIM: ignoring non-user group member '%s' (%s); DeRouter teams contain users only",
            skipped.value,
            skipped.reason,
        )
    ...
    if partition.ambiguous_values:
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Member ID '{partition.ambiguous_values[0]}' names more than one DeRouter user, so the "
...
async def _extract_group_member_ids(group: SCIMGroup) -> GroupMemberExtractionResult:
    ...
    Behavior depends on derouter_settings.scim_upsert_user:
...
async def _delete_rows_referencing_user(prisma_client: PrismaClient, *, user_id: str) -> None:
    """Drop rows whose foreign keys reference ``DeRouter_UserTable.user_id``.
    ...
    ``DeRouter_InvitationLink_user_id_fkey``).
    """
...
def _user_scim_active(user: DeRouter_UserTable) -> bool | None:
...
async def _create_user_if_not_exists(user_id: str, created_via: str = "scim_group") -> NewUserResponse | None:
    ...
    Returns:
        DeRouter_UserTable if user was created, None if creation failed
    """
    from derouter.proxy.management_endpoints.internal_user_endpoints import new_user
    ...
    try:
        ...
        if derouter.default_internal_user_params:
            default_role = derouter.default_internal_user_params.get("user_role")
...
async def _get_team_member_user_ids_from_team(team: DeRouter_TeamTable) -> list[str]:
    """
                    ...
                    subAttributes=[
                        ...
                        SCIMSchemaAttribute(
                            ...
                            description=(
                                ...
                                "Only members of type User are honored, DeRouter teams contain users only."
                            ),
                        ),
                    ],
                ),
...
async def get_users(
    ...
    try:
        ...
        if filter:
            # Okta locates users by userName before deprovisioning. DeRouter
        ...
        users: Final[Sequence[DeRouter_UserTable]] = await _table(UserRepository(prisma_client)).find_many(
        ...
        for user in users:
            scim_user = await ScimTransformations.transform_derouter_user_to_scim_user(user=user)
...
async def get_user(
    ...
    try:
        ...
        # Convert to SCIM format
        scim_user: Final = await ScimTransformations.transform_derouter_user_to_scim_user(user)
        return scim_user
...
async def create_user(
    ...
    try:
        ...
        scim_user: Final = await ScimTransformations.transform_derouter_user_to_scim_user(user=created_user)
        return scim_user
...
async def update_user(
    ...
    try:
        ...
        scim_user: Final = await ScimTransformations.transform_derouter_user_to_scim_user(updated_user)
...
async def delete_user(
    ...
    try:
        ...
        for team in teams:
            ...
            team_row = DeRouter_TeamTable.model_validate(team.model_dump())
...
def _placeholder_rejection(placeholder: DeRouter_UserTable, resolved: tuple[str, ...], key_count: int) -> str | None:
...
async def get_group(
    ...
    try:
        ...
        scim_group: Final = await ScimTransformations.transform_derouter_team_to_scim_group(team)
...
def _new_team_request_with_defaults(
    ...
    """Build the SCIM group's team request, applying derouter.default_team_params
...
async def create_group(
    ...
    try:
        ...
        scim_group: Final = await ScimTransformations.transform_derouter_team_to_scim_group(created_team)
        return scim_group
    except Exception as e:
        raise handle_exception_on_proxy(e)
...
async def update_group(
    ...
    try:
        ...
        scim_group: Final = await ScimTransformations.transform_derouter_team_to_scim_group(updated_team)
        return scim_group
...
async def _process_group_patch_operations(
    patch_ops: SCIMPatchOp, existing_team: DeRouter_TeamTable, prisma_client: PrismaClient
...
async def patch_group(
    ...
    try:
        ...
        refreshed_current: Final = (
            set(
                await _get_team_member_user_ids_from_team(DeRouter_TeamTable.model_validate(refreshed_team.model_dump()))
            )
            if refreshed_team
        ...
        # Convert to SCIM format and return
        scim_group: Final = await ScimTransformations.transform_derouter_team_to_scim_group(
            DeRouter_TeamTable.model_validate(updated_team.model_dump())
        )
        return scim_group
