...
class ScimTransformations:
    ...
    @staticmethod
    async def transform_derouter_user_to_scim_user(
        user: DeRouter_UserTable | NewUserResponse,
    ) -> SCIMUser:
    ...
    @staticmethod
    def _get_scim_user_name(user: DeRouter_UserTable | NewUserResponse) -> str:
        """
    ...
    @staticmethod
    def _get_scim_family_name(user: DeRouter_UserTable | NewUserResponse) -> str:
        """
        SCIM requires a family name with length > 0
        """
        metadata: Final = user.metadata or {}
        if "scim_metadata" in metadata:
            scim_metadata: Final[DeRouter_UserScimMetadata] = DeRouter_UserScimMetadata(**metadata["scim_metadata"])
    ...
    @staticmethod
    def _get_scim_given_name(user: DeRouter_UserTable | NewUserResponse) -> str:
        """
    ...
    @staticmethod
    async def transform_derouter_team_to_scim_group(
        team: DeRouter_TeamTable | dict,
    ) -> SCIMGroup:
        from derouter.proxy.proxy_server import prisma_client
    ...
    def _get_scim_member_value(member: Member) -> str:
        """The member's SCIM resource id, which DeRouter serves as user_id (RFC 7643 §8.7.1)."""
