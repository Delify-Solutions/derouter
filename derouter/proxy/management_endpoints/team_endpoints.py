...
from derouter.proxy._types import (
    ...
    DeleteTeamRequest,
    DeRouter_AuditLogs,
    DeRouter_DeletedTeamTable,
    DeRouter_ManagementEndpoint_MetadataFields,
    DeRouter_ManagementEndpoint_MetadataFields_Premium,
    DeRouter_ModelTable,
...
from derouter.proxy.common_utils.json_merge_patch import apply_json_merge_patch
from derouter.proxy.common_utils.user_api_key_cache import UserApiKeyCache
from derouter.proxy.management_endpoints.common_daily_activity import (
...
from derouter.proxy.management_helpers.object_permission_utils import (
...
from derouter.proxy.management_helpers.utils import (
    MemberWriteTx,
    add_new_member,
    management_endpoint_wrapper,
)
from derouter.proxy.utils import PrismaClient, ProxyLogging, handle_exception_on_proxy
...
from derouter.types.proxy.management_endpoints.common_daily_activity import (
...
class _TeamCreateTx(AccessGroupSyncTx, Protocol):
    @property
    def derouter_teamtable(self) -> "TableActions[prisma_models.DeRouter_TeamTable]": ...
...
class _MemberDeleteTx(Protocol):
    ...
    @property
    def derouter_usertable(self) -> "TableActions[prisma_models.DeRouter_UserTable]": ...

    @property
    def derouter_verificationtoken(self) -> "TableActions[prisma_models.DeRouter_VerificationToken]": ...
...
class _TeamDeleteTx(AccessGroupSyncTx, Protocol):
    ...
    @property
    def derouter_teammembership(self) -> "TableActions[prisma_models.DeRouter_TeamMembership]": ...


_STRIP_DELETED_TEAM_FROM_USERS_SQL: Final = """
UPDATE "DeRouter_UserTable" SET teams = array_remove(teams, $1) WHERE $1 = ANY(teams)
"""

_INCLUDE_MODEL_TABLE: Final = MappingProxyType({"derouter_model_table": True})


def _team_db(prisma_client: PrismaClient | None) -> "TableActions[prisma_models.DeRouter_TeamTable]":
    return TeamRepository(prisma_client).table


def _team_tx_db(tx: "Prisma") -> "TableActions[prisma_models.DeRouter_TeamTable]":
    return cast(  # cast-ok: generated actions type Json columns as str; TableActions widens inputs to Mapping
        "TableActions[prisma_models.DeRouter_TeamTable]", tx.derouter_teamtable
    )


def _team_membership_db(prisma_client: PrismaClient | None) -> "TableActions[prisma_models.DeRouter_TeamMembership]":
    return TeamMembershipRepository(prisma_client).table


def _user_db(prisma_client: PrismaClient | None) -> "TableActions[prisma_models.DeRouter_UserTable]":
    return UserRepository(prisma_client).table


def _model_db(prisma_client: PrismaClient | None) -> "TableActions[prisma_models.DeRouter_ModelTable]":
    return ModelTableRepository(prisma_client).table


def _org_db(prisma_client: PrismaClient | None) -> "TableActions[prisma_models.DeRouter_OrganizationTable]":
    return OrganizationRepository(prisma_client).table


def _org_membership_db(
    prisma_client: PrismaClient | None,
) -> "TableActions[prisma_models.DeRouter_OrganizationMembership]":
    return OrganizationMembershipRepository(prisma_client).table


def _budget_db(prisma_client: PrismaClient | None) -> "TableActions[prisma_models.DeRouter_BudgetTable]":
    return BudgetRepository(prisma_client).table


def _deleted_team_db(prisma_client: PrismaClient | None) -> "TableActions[prisma_models.DeRouter_DeletedTeamTable]":
    return DeletedTeamRepository(prisma_client).table


def _access_group_db(prisma_client: PrismaClient | None) -> "TableActions[prisma_models.DeRouter_AccessGroupTable]":
    return AccessGroupRepository(prisma_client).table


def _tokens_db(prisma_client: PrismaClient | None) -> "TableActions[prisma_models.DeRouter_VerificationToken]":
...
async def _refresh_cached_team(
    ...
    Every endpoint that mutates `derouter_teamtable` must call this so the
    cached `DeRouter_TeamTableCachedObj` used by `common_checks` stays in
    ...
    `derouter_teamtable`. It is converted to `DeRouter_TeamTableCachedObj`
    via `model_dump()` to match the cache shape `_cache_team_object`
    expects.
    """
    await _cache_team_object(
        team_id=team_row.team_id,
        team_table=DeRouter_TeamTableCachedObj.model_validate(team_row.model_dump()),
...
class TeamMemberBudgetHandler:
    ...
    async def create_team_member_budget_table(
        ...
        """
        from derouter.proxy._types import BudgetNewRequest
...
def _get_default_team_param(field: str) -> object:
    """
    Returns a default value for the given field from derouter.default_team_params config.
    ...
    """
    default_params: Final = derouter.default_team_params
...
def _is_available_team(team_id: str, user_api_key_dict: UserAPIKeyAuth) -> bool:
    if derouter.default_internal_user_params is None:
        return False
    if "available_teams" in derouter.default_internal_user_params:
        return team_id in derouter.default_internal_user_params["available_teams"]
    return False


async def get_all_team_memberships(
    prisma_client: PrismaClient, team_ids: list[str], user_id: str | None = None
) -> list[DeRouter_TeamMembership]:
    ...
    returned_tm: Final[list[DeRouter_TeamMembership]] = []
    for tm in team_memberships:
        returned_tm.append(DeRouter_TeamMembership.model_validate(tm.model_dump()))
...
def check_org_team_model_specific_limits(
    ...
    if org_table.derouter_budget_table is not None:
        entity_rpm_limit = org_table.derouter_budget_table.rpm_limit
        entity_tpm_limit = org_table.derouter_budget_table.tpm_limit
...
async def _check_org_team_limits(
    ...
    if (
        ...
        and org_table.derouter_budget_table is not None
        and org_table.derouter_budget_table.max_budget is not None
        and data.max_budget > org_table.derouter_budget_table.max_budget
    ):
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Team max_budget ({data.max_budget}) exceeds organization's max_budget ({org_table.derouter_budget_table.max_budget}). Organization: {org_table.organization_id}"
            },
        )
    ...
    if (
        data.tpm_limit is not None
        and org_table.derouter_budget_table is not None
        and org_table.derouter_budget_table.tpm_limit is not None
        and data.tpm_limit > org_table.derouter_budget_table.tpm_limit
    ):
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Team tpm_limit ({data.tpm_limit}) exceeds organization's tpm_limit ({org_table.derouter_budget_table.tpm_limit}). Organization: {org_table.organization_id}"
            },
        )
    ...
    if (
        data.rpm_limit is not None
        and org_table.derouter_budget_table is not None
        and org_table.derouter_budget_table.rpm_limit is not None
        and data.rpm_limit > org_table.derouter_budget_table.rpm_limit
    ):
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Team rpm_limit ({data.rpm_limit}) exceeds organization's rpm_limit ({org_table.derouter_budget_table.rpm_limit}). Organization: {org_table.organization_id}"
            },
        )
    ...
    # Convert teams to DeRouter_TeamTable objects
    team_objs: Final[list[DeRouter_TeamTable]] = []
    for team in teams:
        team_objs.append(DeRouter_TeamTable.model_validate(team.model_dump()))
...
@router.post(
    ...
    response_model=DeRouter_TeamTable,
)
@management_endpoint_wrapper
async def new_team(
    data: NewTeamRequest,
    ...
    derouter_changed_by: str | None = Header(
        None,
        description="The derouter-changed-by header enables tracking of actions performed by authorized users on behalf of other users, providing an audit trail for accountability",
    ),
):
    """
    Allow users to create a new team. Apply user permissions to their team.

    👉 [Detailed Doc on setting team budgets](https://router.delify.vn/docs/proxy/team_budgets)
    ...
    - budget_duration: Optional[str] - The duration of the budget for the team. Doc [here](https://router.delify.vn/docs/proxy/team_budgets)
    ...
    - tags: Optional[List[str]] - Tags for [tracking spend](https://derouter.vercel.app/docs/proxy/enterprise#tracking-spend-for-custom-tags) and/or doing [tag-based routing](https://derouter.vercel.app/docs/proxy/tag_routing).
    - prompts: Optional[List[str]] - List of prompts that the team is allowed to use.
    - organization_id: Optional[str] - The organization id of the team. Default is None. Create via `/organization/new`.
    - model_aliases: Optional[dict] - Model aliases for the team. [Docs](https://router.delify.vn/docs/proxy/team_based_routing#create-team-with-model-alias)
    - guardrails: Optional[List[str]] - Guardrails for the team. [Docs](https://router.delify.vn/docs/proxy/guardrails)
    - policies: Optional[List[str]] - Policies for the team. [Docs](https://router.delify.vn/docs/proxy/guardrails/guardrail_policies)
    - disable_global_guardrails: Optional[bool] - Whether to disable global guardrails for the key.
    - object_permission: Optional[DeRouter_ObjectPermissionBase] - team-specific object permission. Example - {"vector_stores": ["vector_store_1", "vector_store_2"], "agents": ["agent_1", "agent_2"], "agent_access_groups": ["dev_group"]}. IF null or {} then no object permission.
    - team_member_budget: Optional[float] - The maximum budget allocated to an individual team member.
    - team_member_budget_duration: Optional[str] - The duration of the budget for the team member. Doc [here](https://router.delify.vn/docs/proxy/team_budgets)
    ...
    - secret_manager_settings: Optional[dict] - Secret manager settings for the team. [Docs](https://router.delify.vn/docs/secret_managers/overview)
    ...
    try:
        ...
        if data.team_id is None:
            ...
            if data.team_id == UI_TEAM_ID:
                raise HTTPException(
                    status_code=400,
                    detail={
                        "error": f"team_id '{UI_TEAM_ID}' is reserved for DeRouter UI dashboard sessions and cannot be used for a real team. Please use a different team id."
                    },
                )
        ...
        # Apply defaults from derouter.default_team_params to null fields.
        ...
        if data.max_budget is None:
            if (
                isinstance(derouter.default_team_settings, list)
                and len(derouter.default_team_settings) > 0
                and isinstance(derouter.default_team_settings[0], dict)
            ):
                default_budget: Final = derouter.default_team_settings[0].get("max_budget")
        ...
        if data.model_aliases is not None and isinstance(data.model_aliases, dict):
            derouter_modeltable: Final = DeRouter_ModelTable(
            ...
            model_dict: Final = await _model_db(prisma_client).create({**derouter_modeltable.json(exclude_none=True)})
        ...
        ## ADD TO TEAM TABLE
        complete_team_data: Final = DeRouter_TeamTable(
            **data_json,
            model_id=_model_id,
        )

        # Set Management Endpoint Metadata Fields
        for field in DeRouter_ManagementEndpoint_MetadataFields_Premium:
        ...
        if complete_team_data.budget_duration is not None:
            from derouter.proxy.common_utils.timezone_utils import get_budget_reset_time
        ...
        async with prisma_client.db.tx() as tx:
            team_row: Final[prisma_models.DeRouter_TeamTable] = await tx.derouter_teamtable.create(
                ...
                create_audit_log_for_update(
                    request_data=DeRouter_AuditLogs(
                        ...
                        changed_by=get_audit_log_changed_by(
                            derouter_changed_by=derouter_changed_by,
                            user_api_key_dict=user_api_key_dict,
                            derouter_proxy_admin_name=derouter_proxy_admin_name,
                        ),
...
async def _create_team_update_audit_log(
    ...
    updated_kv: dict,
    team_id: str,
    derouter_changed_by: str | None,
    user_api_key_dict: UserAPIKeyAuth,
    derouter_proxy_admin_name: str,
) -> None:
    """
    ...
    Args:
        ...
        derouter_changed_by: Optional header indicating who made the change
        user_api_key_dict: User API key authentication details
        derouter_proxy_admin_name: Name of the proxy admin
    """
    from derouter.proxy.management_helpers.audit_logs import (
        create_audit_log_for_update,
        get_audit_log_changed_by,
...
async def _update_model_table(
    ...
    if data.model_aliases is not None and isinstance(data.model_aliases, dict):
        ...
        if model_id is None:
            model_dict = await _model_db(prisma_client).create(data={**derouter_modeltable.json(exclude_none=True)})
        else:
            model_dict = await _model_db(prisma_client).upsert(
                where={"id": model_id},
                data={
                    "update": {**derouter_modeltable.json(exclude_none=True)},
                    "create": {**derouter_modeltable.json(exclude_none=True)},
                },
            )
...
async def _auto_add_team_members_to_organization(
    ...
    organization: DeRouter_OrganizationTableWithMembers,
    prisma_client: PrismaClient,
) -> None:
    """
...
async def fetch_and_validate_organization(
...
) -> "prisma_models.DeRouter_OrganizationTable":
    """
    ...
    organization_row: Final = await _org_db(prisma_client).find_unique(
        ...
        include={"derouter_budget_table": True, "members": True, "teams": True},
    )
    ...
    organization: Final = DeRouter_OrganizationTableWithMembers.model_validate(organization_row.model_dump())
    validate_team_org_change(
        team=DeRouter_TeamTable.model_validate(existing_team_row.model_dump()),
...
def validate_team_org_change(
    ...
    if (
        team.max_budget
        and organization.derouter_budget_table
        and organization.derouter_budget_table.max_budget
        and team.max_budget > organization.derouter_budget_table.max_budget
    ):
        raise HTTPException(
            status_code=403,
            detail={
                "error": f"Cannot move team to organization. Team has max_budget {team.max_budget} that is greater than the organization's max_budget {organization.derouter_budget_table.max_budget}."
            },
        )
    ...
    if (
        team.tpm_limit
        and organization.derouter_budget_table
        and organization.derouter_budget_table.tpm_limit
        and team.tpm_limit > organization.derouter_budget_table.tpm_limit
    ):
        raise HTTPException(
            status_code=403,
            detail={
                "error": f"Cannot move team to organization. Team has tpm_limit {team.tpm_limit} that is greater than the organization's tpm_limit {organization.derouter_budget_table.tpm_limit}."
            },
        )
    if (
        team.rpm_limit
        and organization.derouter_budget_table
        and organization.derouter_budget_table.rpm_limit
        and team.rpm_limit > organization.derouter_budget_table.rpm_limit
    ):
        raise HTTPException(
            status_code=403,
            detail={
                "error": f"Cannot move team to organization. Team has rpm_limit {team.rpm_limit} that is greater than the organization's rpm_limit {organization.derouter_budget_table.rpm_limit}."
            },
        )
    return True
...
async def update_team(
    ...
    """
    try:
        from derouter.proxy.management_helpers.audit_logs import is_audit_logging_enabled
        from derouter.proxy.proxy_server import (
            derouter_proxy_admin_name,
            llm_router,
            prisma_client,
        ...
        await _verify_team_access(
            team_obj=DeRouter_TeamTable.model_validate(existing_team_row.model_dump()),
            user_api_key_dict=user_api_key_dict,
        )
...
async def handle_update_object_permission(data_json: dict, existing_team_row: _ObjectPermissionRow) -> dict:
    ...
    - IF there's no object_permission_id, then create a new entry in DeRouter_ObjectPermissionTable
    - IF there's an object_permission_id, then update the entry in DeRouter_ObjectPermissionTable
    """
    from derouter.proxy.proxy_server import prisma_client
...
async def _validate_team_member_add_permissions(
    user_api_key_dict: UserAPIKeyAuth,
    complete_team_data: DeRouter_TeamTable,
    data: TeamMemberAddRequest,
) -> None:
...
async def _process_team_members(
...
) -> tuple[list[DeRouter_UserTable], list[DeRouter_TeamMembership]]:
    ...
    """
    updated_users: Final[list[DeRouter_UserTable]] = []
    updated_team_memberships: Final[list[DeRouter_TeamMembership]] = []
...
def _resolve_member_identity(member: Member, updated_users: Sequence[DeRouter_UserTable]) -> Member:
...
def _member_already_in_team(member: Member, complete_team_data: DeRouter_TeamTable) -> bool:
    return any(
...
async def _add_team_members_to_team(
    ...
    user_api_key_dict: UserAPIKeyAuth,
    derouter_proxy_admin_name: str,
) -> tuple["prisma_models.DeRouter_TeamTable", list[DeRouter_UserTable], list[DeRouter_TeamMembership]]:
...
def _emit_team_members_metric(team: DeRouter_TeamTable) -> None:
...
async def _create_team_member_add_audit_logs(
    team_id: str,
    updated_users: Sequence[DeRouter_UserTable],
    existing_user_ids: frozenset[str],
    ...
    """
    from derouter.proxy.management_helpers.audit_logs import create_object_audit_log
...
async def team_member_add(
    ...
    complete_team_data: Final = DeRouter_TeamTable.model_validate(existing_team_row.model_dump())
...
async def team_member_delete(
    ...
    existing_team_row: Final = DeRouter_TeamTable.model_validate(_existing_team_row.model_dump())
    ...
    async with prisma_client.tx() as tx:
        ...
        removed_team_members, new_team_members = _cleanup_members_with_roles(
            existing_team_row=DeRouter_TeamTable(team_id=data.team_id, members_with_roles=fresh_members),
            data=data,
        )
        ...
        existing_user_rows: Final = await member_tx.derouter_usertable.find_many(where=key_val)
        ...
        keys_to_delete: Final = await member_tx.derouter_verificationtoken.find_many(
            where={
        ...
        for existing_user in stale_user_rows:
            await tx.derouter_usertable.update(
        ...
        for _uid in sorted(user_ids_to_delete):
            await tx.derouter_teammembership.delete_many(where={"team_id": data.team_id, "user_id": _uid})
        ...
        if user_ids_to_delete:
            ...
            await tx.derouter_verificationtoken.delete_many(
                where={
                    "user_id": {"in": sorted(user_ids_to_delete)},
                    "team_id": data.team_id,
                }
...
async def team_member_update(
    ...
    """
    from derouter.proxy.proxy_server import premium_user, prisma_client, user_api_key_cache
    ...
    if data.role == "admin" and not premium_user:
        ...
        raise HTTPException(
            status_code=400,
            detail="Assigning team admins is a premium feature. You must be a DeRouter Enterprise user to use this feature. If you have a license please set `DEROUTER_LICENSE` in your env. Get a 7 day trial key here: https://www.router.delify.vn/#trial. Pricing: https://www.router.delify.vn/#pricing",
        )
    if data.user_id is None and data.user_email is None:
        raise HTTPException(
            status_code=400,
...
def _check_not_resetting_own_spend(user_id: str, user_api_key_dict: UserAPIKeyAuth) -> None:
    ...
    unchecked, that admin could target their own DeRouter_TeamMembership row and
...
def _validate_team_member_reset_spend_value(
    ...
    max_budget: Final = membership.derouter_budget_table.max_budget if membership.derouter_budget_table else None
...
async def reset_team_member_spend_fn(
    ...
    budget and the team's own budget (DeRouter_TeamMembership.spend), so
    ...
    """
    from derouter.proxy.proxy_server import prisma_client, proxy_logging_obj, user_api_key_cache
    ...
    _membership_row: Final = await _team_membership_db(prisma_client).find_unique(
        ...
        include={"derouter_budget_table": True},  # mutable-ok: prisma client requires a plain dict include= argument
    )
    ...
    membership: Final = DeRouter_TeamMembership.model_validate(_membership_row.model_dump())
    ...
    return {  # mutable-ok: matches this router's established untyped-response-dict convention
        ...
        "max_budget": membership.derouter_budget_table.max_budget if membership.derouter_budget_table else None,
    }
...
async def delete_team(
    ...
    team_rows: Final[list[DeRouter_TeamTable]] = []
    for team_id in data.team_ids:
        ...
        team_row_pydantic = DeRouter_TeamTable.model_validate(team_row_base.model_dump())
    ...
    if is_audit_logging_enabled():
        ...
        for team_id in data.team_ids:
            team_row: DeRouter_TeamTable | None = await prisma_client.get_data(
    ...
    async with prisma_client.tx() as tx:
        ...
        await tx.derouter_teamtable.delete_many(where=delete_filter)
...
async def _sweep_deleted_team_references_tx(team_ids: Sequence[str], tx: _TeamDeleteTx) -> None:
    ...
    _ = await tx.derouter_teammembership.delete_many(where=membership_filter)


async def _invalidate_deleted_key_cache(
    keys: "Sequence[prisma_models.DeRouter_VerificationToken]",
    user_api_key_cache: UserApiKeyCache,
    proxy_logging_obj: ProxyLogging,
) -> None:
    """
...
async def _invalidate_deleted_team_cache(
    teams: Sequence[DeRouter_TeamTable],
    user_api_key_cache: UserApiKeyCache,
    proxy_logging_obj: ProxyLogging,
) -> None:
...
def _transform_teams_to_deleted_records(
    teams: list[DeRouter_TeamTable],
    user_api_key_dict: UserAPIKeyAuth,
    derouter_changed_by: str | None = None,
    ...
    for team in teams:
        ...
        deleted_record = DeRouter_DeletedTeamTable.model_validate(
            {
                **team_payload,
        ...
        for rel_key in (
            ...
            "object_permission",
            "id",
            "budget_limits",  # not in DeRouter_DeletedTeamTable schema
            "default_team_member_models",  # not in DeRouter_DeletedTeamTable schema
        ):
...
async def validate_membership(user_api_key_dict: UserAPIKeyAuth, team_table: DeRouter_TeamTable):
    if (
        user_api_key_dict.user_role == LitellmUserRoles.PROXY_ADMIN.value
...
async def _hydrate_member_emails(
    ...
    Look the missing ones up in ``DeRouter_UserTable`` (one indexed query) and fill them
    ...
    user_rows: Final[Sequence[prisma_models.DeRouter_UserTable]] = await _user_db(prisma_client).find_many(
        where={  # mutable-ok: Prisma query filters are dict-shaped
            "user_id": {  # mutable-ok: Prisma query filters are dict-shaped
    ...
    try:
        if prisma_client is None:
            raise HTTPException(
                ...
                detail={
                    "error": "Database not connected. Connect a database to your proxy - https://router.delify.vn/docs/simple_proxy#managing-auth---virtual-keys"
                },
            )
        if team_id is None:
            raise HTTPException(
        ...
        try:
            team_info: BaseModel | None = await _team_db(prisma_client).find_unique(
                ...
                include={"derouter_model_table": True, "object_permission": True},
            )
        ...
        await validate_membership(
            user_api_key_dict=user_api_key_dict,
            team_table=DeRouter_TeamTable.model_validate(team_info.model_dump()),
        )
        ...
        verbose_proxy_logger.error(
            "derouter.proxy.management_endpoints.team_endpoints.py::team_info - Exception occurred - %s\n%s",
            e,
...
async def team_member_me(
    ...
    return TeamMemberInfoResponse(
        ...
        derouter_budget_table=membership.derouter_budget_table,
    )
...
async def block_team(
    ...
    # Verify caller has access to manage this team
    await _verify_team_access(
        team_obj=DeRouter_TeamTable.model_validate(existing_team.model_dump()),
        user_api_key_dict=user_api_key_dict,
    )
...
async def list_available_teams(
    http_request: Request,
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
    response_model=list[DeRouter_TeamTable],
):
    from derouter.proxy.proxy_server import prisma_client
    ...
    available_teams = cast(
        list[str] | None,
        (
            derouter.default_internal_user_params.get("available_teams")
            if derouter.default_internal_user_params is not None
            else None
        ),
    )
    ...
    user_info_correct_type: Final = DeRouter_UserTable.model_validate(user_info.model_dump())
    ...
    available_teams_correct_type = [DeRouter_TeamTable.model_validate(team.model_dump()) for team in available_teams_db]
...
async def _batch_resolve_access_group_resources(
...
) -> "dict[str, prisma_models.DeRouter_AccessGroupTable]":
    """
    ...
    """
    from derouter.proxy.proxy_server import prisma_client as _prisma_client
...
def _convert_teams_to_response_models(
...
) -> list[TeamListItem | DeRouter_TeamTable | DeRouter_DeletedTeamTable]:
    """Convert raw Prisma team rows to response models."""
    team_list: Final[list[TeamListItem | DeRouter_TeamTable | DeRouter_DeletedTeamTable]] = []
    ...
    for team in teams:
        ...
        if use_deleted_table:
            team_list.append(DeRouter_DeletedTeamTable.model_validate(team_dict))
        else:
...
async def _get_keys_count_by_team(
    ...
    Runs a single GROUP BY against DeRouter_VerificationToken. The IN clause is
...
async def list_team_v2(
    ...
    if use_deleted_table:
        # DeRouter_DeletedTeamTable has no derouter_model_table relation, unlike below
...
async def _authorize_and_filter_teams(
    ...
    if allowed_org_ids is not None:
        ...
        return list(await _raw_team_db(TeamRepository(prisma_client)).find_many(include={"derouter_model_table": True}))
...
async def list_team(
    ...
    for team in filtered_response:
        _team_memberships: list[DeRouter_TeamMembership] = []
...
async def get_paginated_teams(
    ...
    page_size: int = 10,
    page: int = 1,
) -> tuple[list[DeRouter_TeamTable], int]:
    """
    ...
    Returns:
        Tuple[List[DeRouter_TeamTable], int] - (list of teams, total count)
    """
    try:
...
@router.get(
    ...
    responses={
        200: {"model": list[DeRouter_TeamTable]},
    },
)
async def ui_view_teams(
    ...
    Returns:
        List[DeRouter_SpendLogs]: Paginated list of matching user records
    """
    from derouter.proxy.proxy_server import prisma_client
...
def add_new_models_to_team(team_obj: DeRouter_TeamTable, new_models: list[str]) -> list[str]:
    """
...
async def team_model_add(
    ...
    team_obj: Final = DeRouter_TeamTable.model_validate(team_row.model_dump())
...
async def _compute_and_batch_updates(
    prisma_client, teams: "Sequence[prisma_models.DeRouter_TeamTable]", permissions_to_add: set
) -> int:
    ...
    if updates:
        ...
        for team_id, merged_perms in updates:
            batcher.derouter_teamtable.update(
                where={"team_id": team_id},
    ...
    *,
    ...
    if not _user_has_admin_view(user_api_key_dict) and team_ids_list and team_aliases:
        ...
        for team_alias in team_aliases:
            team_obj = DeRouter_TeamTable.model_validate(team_alias.model_dump())
...
async def get_team_daily_activity(
    ...
    return await get_daily_activity(
        prisma_client=prisma_client,
        table_name="derouter_dailyteamspend",
...
def _team_user_spend_sql(*, team_count: int, restrict_to_user: bool) -> str:
    ...
    return f"""
        ...
        FROM "DeRouter_SpendLogs" sl
        LEFT JOIN "DeRouter_UserTable" u ON u.user_id = sl."user"
