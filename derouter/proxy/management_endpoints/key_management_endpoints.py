...
from derouter.constants import (
    LENGTH_OF_DEROUTER_GENERATED_KEY,
...
from derouter.models.credentials import CredentialItem
from derouter.proxy._experimental.mcp_server.db import (
...
from derouter.proxy._types import Litellm_EntityType, DeRouter_VerificationToken, hash_token
...
from derouter.proxy.common_utils.callback_config_validation import logging_metadata_config_error
from derouter.proxy.common_utils.callback_utils import (
    decrypt_callback_vars,
    encrypt_callback_vars,
)
from derouter.proxy.common_utils.config_sync_pubsub import (
    coordination_redis_cache,
    publish_config_change,
)
from derouter.proxy.common_utils.rbac_utils import check_org_admin_can_generate_keys
...
from derouter.proxy.hooks.key_management_event_hooks import KeyManagementEventHooks
from derouter.proxy.hooks.model_max_budget_limiter import build_model_max_budget_usage
...
from derouter.proxy.management_helpers.utils import management_endpoint_wrapper
from derouter.proxy.spend_tracking.budget_reservation import get_budget_window_start
from derouter.proxy.spend_tracking.spend_tracking_utils import _is_master_key
...
from derouter.repositories.config_repository import ConfigParam, ConfigRepository
...
from derouter.secret_managers.base_secret_manager import raise_if_unsafe_secret_name
...
class _TxTables(Protocol):
    derouter_proxymodeltable: TableActions[object]


class _ModelParamsUpdate(TypedDict):
    derouter_params: ReadOnly["prisma.Json"]
...
class _KeyUpdateTx(Protocol):
    ...
    @property
    def derouter_budgettable(self) -> "TableActions[prisma_models.DeRouter_BudgetTable]": ...
...
) -> "TableActions[prisma_models.DeRouter_DeletedVerificationToken]":
    return DeletedVerificationTokenRepository(prisma_client).table


def _deprecated_verification_token_table(
    prisma_client: PrismaClient,
) -> "TableActions[prisma_models.DeRouter_DeprecatedVerificationToken]":
...
def _is_team_key(data: GenerateKeyRequest | DeRouter_VerificationToken):
    return data.team_id is not None


def _get_user_in_team(team_table: DeRouter_TeamTableCachedObj, user_id: str | None) -> Member | None:
    if user_id is None:
        return None
...
def _set_key_rotation_fields(
    ...
    if auto_rotate and rotation_interval:
        if (
            derouter._key_management_settings is not None
            and derouter._key_management_settings.store_virtual_keys is True
...
def _is_allowed_to_make_key_request(
    ...
    Relevant issue: https://github.com/Delify-Solutions/derouter/issues/7336
    """
    ...
    if team_id is not None:
        if user_api_key_dict.team_id is not None and user_api_key_dict.team_id == UI_TEAM_ID:
            return True  # handle https://github.com/Delify-Solutions/derouter/issues/7482
...
def _team_key_generation_check(
    ...
    if user_api_key_dict.user_role == LitellmUserRoles.PROXY_ADMIN.value:
        return True
    if derouter.key_generation_settings is not None and "team_key_generation" in derouter.key_generation_settings:
        _team_key_generation = derouter.key_generation_settings["team_key_generation"]
    else:
...
def _object_permission_to_dict(
    object_permission: DeRouter_ObjectPermissionBase | None,
...
def _personal_key_generation_check(user_api_key_dict: UserAPIKeyAuth, data: GenerateKeyRequest):
    ...
    if (
        derouter.key_generation_settings is None
        or derouter.key_generation_settings.get("personal_key_generation") is None
    ):
        return True

    _personal_key_generation: Final = derouter.key_generation_settings["personal_key_generation"]
...
def key_generation_check(
    team_table: DeRouter_TeamTableCachedObj | None,
    user_api_key_dict: UserAPIKeyAuth,
    data: GenerateKeyRequest,
    route: KeyManagementRoutes,
) -> bool:
    ...
    if is_team_key:
        if team_table is None and derouter.key_generation_settings is not None:
            raise HTTPException(
                status_code=400,
...
def handle_key_type(data: GenerateKeyRequest, data_json: dict) -> dict:
    ...
    if key_type == DeRouterKeyType.LLM_API:
        data_json["allowed_routes"] = ["llm_api_routes"]
    elif key_type == DeRouterKeyType.MANAGEMENT:
        data_json["allowed_routes"] = ["management_routes"]
    elif key_type == DeRouterKeyType.READ_ONLY:
...
def _validate_caller_can_change_key_ownership(
    ...
    ``_return_user_api_key_auth_obj`` resolves against ``derouter_usertable``
...
def _enforce_upperbound_key_params(
    ...
    if derouter.upperbound_key_generate_params is None:
        return

    for elem in data:
        key, value = elem
        upperbound_value = getattr(derouter.upperbound_key_generate_params, key, None)
...
async def _common_key_generation_helper(
    ...
    if derouter.default_key_generate_params is not None:
        for elem in _model_items(data):
            ...
            if (
                ...
                default_value = derouter.default_key_generate_params.get(key)
                if default_value is not None:
                    setattr(data, key, default_value)
            elif key == "models" and value == []:
                setattr(data, key, derouter.default_key_generate_params.get(key, []))
    ...
    try:
        from derouter_enterprise.proxy.management_endpoints.key_management_endpoints import (
        ...
        verbose_proxy_logger.debug(
            "derouter.proxy.proxy_server.generate_key_fn(): Enterprise key management params not applied - %s", e
        )

    # TODO: @ishaan-jaff: Migrate all budget tracking to use DeRouter_BudgetTable
    _budget_id = data.budget_id
    if prisma_client is not None and data.soft_budget is not None:
        # create the Budget Row for the DeRouter Verification Token
        budget_row: Final = DeRouter_BudgetTable(
        ...
        _budget: Final[prisma_models.DeRouter_BudgetTable] = await BudgetRepository(prisma_client).table.create(
            data={
                **new_budget,
    ...
    _default_object_permission: Final = (
        derouter.default_key_generate_params.get("object_permission")
        if derouter.default_key_generate_params is not None
        else None
    )
    ...
    if data.key is not None and not data.key.startswith("sk-"):
        ...
        raise HTTPException(
            status_code=400,
            detail={"error": f"Invalid key format. DeRouter Virtual Key must start with 'sk-'. Received: {_masked}"},
        )
    ...
    if data.key is not None and len(data.key) < MINIMUM_CUSTOM_KEY_LENGTH:
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Invalid key format. DeRouter Virtual Key must be at least {MINIMUM_CUSTOM_KEY_LENGTH} characters long."
            },
        )
...
def _check_key_model_specific_limits(
    keys: Sequence[DeRouter_VerificationToken],
    data: GenerateKeyRequest | UpdateKeyRequest,
...
async def _check_project_key_limits(
    ...
    if project_obj.derouter_budget_table is not None:
        project_max_budget = getattr(project_obj.derouter_budget_table, "max_budget", None)
...
async def generate_key_fn(
    ...
    Docs: https://router.delify.vn/docs/proxy/virtual_keys
    ...
    - aliases: Optional[dict] - Any alias mappings, on top of anything in the config.yaml model list. - https://router.delify.vn/docs/proxy/virtual_keys#managing-auth---upgradedowngrade-models
    - config: Optional[dict] - any key-specific configs, overrides config in config.yaml
    - spend: Optional[int] - Amount spent by key. Default is 0. Will be updated by proxy whenever key is used. https://router.delify.vn/docs/proxy/virtual_keys#managing-auth---tracking-spend
    ...
    - allowed_cache_controls: Optional[list] - List of allowed cache control values. Example - ["no-cache", "no-store"]. See all values - https://router.delify.vn/docs/proxy/caching#turn-on--off-caching-per-request
    ...
    - enforced_params: Optional[List[str]] - List of enforced params for the key (Enterprise only). [Docs](https://router.delify.vn/docs/proxy/enterprise#enforce-required-params-for-llm-requests)
    - prompts: Optional[List[str]] - List of prompts that the key is allowed to use.
    ...
    - object_permission: Optional[DeRouter_ObjectPermissionBase] - key-specific object permission. Example - {"vector_stores": ["vector_store_1", "vector_store_2"], "agents": ["agent_1", "agent_2"], "agent_access_groups": ["dev_group"]}. IF null or {} then no object permission.
    ...
    try:
        ...
        team_table: DeRouter_TeamTableCachedObj | None = None
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.generate_key_fn(): Exception occured - %s", e)
...
def prepare_metadata_fields(data: BaseModel, non_default_values: dict, existing_metadata: dict) -> dict:
    """
    Check DeRouter_ManagementEndpoint_MetadataFields (proxy/_types.py) for fields that are allowed to be updated
    """
    ...
    for reserved_field in DeRouter_Reserved_Metadata_Fields:
    ...
    try:
        for k, v in data_json.items():
            if k in DeRouter_ManagementEndpoint_MetadataFields:
                if isinstance(v, datetime):
                    casted_metadata[k] = v.isoformat()
                else:
                    casted_metadata[k] = v
            if k in DeRouter_ManagementEndpoint_MetadataFields_Premium:
        ...
        verbose_proxy_logger.exception(
            "derouter.proxy.proxy_server.prepare_metadata_fields(): Exception occured - %s", e
        )
...
async def _update_key_soft_budget(
    ...
    if existing_budget_id is not None:
        ...
        await db.derouter_budgettable.update(where=budget_where, data=budget_update)
    ...
    created_budget: Final = await db.derouter_budgettable.create(data=budget_create)
...
async def _update_key_row_with_soft_budget(
    ...
    async with prisma_client.tx() as tx:
        ...
        updated_row: Final = await tx.derouter_verificationtoken.update(
            where=key_where,
...
async def prepare_key_update_data(
    ...
    for k, v in data_json.items():
        if k in DeRouter_ManagementEndpoint_MetadataFields or k in DeRouter_ManagementEndpoint_MetadataFields_Premium:
            continue
...
def is_different_team(data: UpdateKeyRequest, existing_key_row: DeRouter_VerificationToken) -> bool:
...
async def _get_and_validate_existing_key(
    ...
    Returns:
        DeRouter_VerificationToken: The existing key row
    ...
    if token is not None:
        ...
        existing_key_row: Final[DeRouter_VerificationToken | None] = await _prisma_table(
    ...
    rows: Sequence[DeRouter_VerificationToken] = await _prisma_table(
        VerificationTokenRepository(prisma_client)
...
def _resolve_token_to_update(data: UpdateKeyRequest, existing_key_row: DeRouter_VerificationToken) -> str:
...
async def _process_single_key_update(
    ...
    existing_key_row: DeRouter_VerificationToken | None = None,
) -> dict[str, object]:
    """
    ...
    Args:
        ...
        derouter_changed_by: Optional header for tracking who made the change
        prisma_client: Prisma client instance
    ...
    team_obj: DeRouter_TeamTableCachedObj | None = None
...
async def _validate_mcp_servers_for_key_update(
    data: "UpdateKeyRequest",
    team_obj: Optional["DeRouter_TeamTableCachedObj"],
    existing_key_row: DeRouter_VerificationToken,
    prisma_client: PrismaClient | None,
    user_api_key_cache: UserApiKeyCache,
    is_proxy_admin: bool,
...
async def _validate_update_key_data(
    ...
    if data.object_permission is not None:
        ...
        if normalized_object_permission is not None:
            data.object_permission = DeRouter_ObjectPermissionBase(**normalized_object_permission)
...
async def update_key_fn(
    ...
    - prompts: Optional[List[str]] - List of prompts that the key is allowed to use.
    - blocked: Optional[bool] - Whether the key is blocked
    - aliases: Optional[dict] - Model aliases for the key - [Docs](https://derouter.vercel.app/docs/proxy/virtual_keys#model-aliases)
    ...
    try:
        ...
        changed_by: Final = user_api_key_dict.user_id or derouter_proxy_admin_name
        response: Final = (
        ...
        if data.spend is not None:
            from derouter.proxy.proxy_server import spend_counter_cache
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.update_key_fn(): Exception occured - %s", e)
...
def _build_failed_team_key_update(
    token: str,
    exception: Exception,
    existing_key_row: DeRouter_VerificationToken | None,
) -> FailedKeyUpdate:
...
async def bulk_update_team_keys(
    ...
    if user_api_key_dict.user_role != LitellmUserRoles.PROXY_ADMIN.value:
        auth_anchor: Final = (
            existing_keys[0]
            if existing_keys
            else DeRouter_VerificationToken(
...
async def validate_key_team_change(
    ...
    member_object: Final = _get_user_in_team(team_table=cast(DeRouter_TeamTableCachedObj, team), user_id=key.user_id)
    ...
    if (
        ...
        or TeamMemberPermissionChecks.does_team_member_have_permissions_for_endpoint(
            ...
            team_table=cast(DeRouter_TeamTableCachedObj, team),
...
async def delete_key_fn(
    ...
    try:
        ...
        # Normalize derouter_changed_by: if it's a Header object or not a string, convert to None
        if derouter_changed_by is not None and not isinstance(derouter_changed_by, str):
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.delete_key_fn(): Exception occured - %s", e)
        raise handle_exception_on_proxy(e)
...
async def _budget_window_usage(
    ...
    the DeRouter_BudgetWindowSpend row instead of a spend-log aggregate.
    """
...
async def info_key_fn_v2(
    ...
    **New endpoint**. Currently admin only.
    ...
    try:
        if prisma_client is None:
            raise Exception(
                "Database not connected. Connect a database to your proxy - https://router.delify.vn/docs/simple_proxy#managing-auth---virtual-keys"
            )
        if data is None:
            raise HTTPException(
        ...
        for k in key_info:
            ...
            budget_table = k_dict.get("derouter_budget_table") or {}
...
-H "Authorization: Bearer sk-test-example-key-123"
    ...
    try:
        ...
        budget_table: Final = key_info.get("derouter_budget_table") or {}
        if not model_max_budget and isinstance(budget_table, dict):
            model_max_budget = budget_table.get("model_max_budget") or {}
        ...
        if llm_router._is_model_access_group_for_wildcard_route(model_access_group=model):
            if not premium_user:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail={
                        "error": f"Setting a model access group on a wildcard model is only available for DeRouter Enterprise users.{CommonProxyErrors.not_premium_user.value}"
                    },
                )
...
async def generate_key_helper_fn(
    ...
    budget_id: float | None = None,  # budget id <-> DeRouter_BudgetTable
    ...
    object_permission_id: str | None = None,  # object_permission_id <-> DeRouter_ObjectPermissionTable
    ...
    from derouter.proxy.proxy_server import premium_user, prisma_client

    if prisma_client is None:
        raise Exception("Connect Proxy to database to generate keys - https://router.delify.vn/docs/proxy/virtual_keys ")

    if token is None:
        if key is not None:
            token = key
        else:
            token = f"sk-{secrets.token_urlsafe(LENGTH_OF_DEROUTER_GENERATED_KEY)}"
    ...
    try:
        ...
        if isinstance(saved_token["permissions"], str):
            if "get_spend_routes" in saved_token["permissions"] and premium_user is not True:
                raise ValueError("get_spend_routes permission is only available for DeRouter Enterprise users")
        ...
        if prisma_client is not None:
            if table_name is None or table_name == "user":  # do not auto-create users for `/key/generate`
                ...
                if query_type == "insert_data":
                    user_row = cast(  # cast-ok: table_name="user" is the insert_data branch returning the user row
                        "prisma_models.DeRouter_UserTable | None",
            ...
            if table_name is not None and table_name == "user":
                ...
                # the DeRouter_VerificationToken table will increase in size if we don't do this check
                return user_data
            ...
            key_data["derouter_budget_table"] = getattr(create_key_response, "derouter_budget_table", None)
        ...
        verbose_proxy_logger.error("derouter.proxy.proxy_server.generate_key_helper_fn(): Exception occured - %s", e)
...
async def can_modify_verification_token(
    ...
    if user_api_key_dict.api_key == DEROUTER_INTERNAL_JOBS_SERVICE_ACCOUNT_NAME:
        return True
...
async def delete_verification_tokens(
...
) -> tuple[dict | None, list[DeRouter_VerificationToken]]:
    """
    ...
    Returns:
        Tuple[Optional[Dict], List[DeRouter_VerificationToken]]:
            Optional[Dict]:
                - Number of deleted tokens
            List[DeRouter_VerificationToken]:
    ...
    try:
        if prisma_client:
            ...
            _keys_being_deleted: Final[list[DeRouter_VerificationToken]] = cast(  # cast-ok: find_many returns a list
                "list[DeRouter_VerificationToken]",
        ...
        verbose_proxy_logger.exception(
            "derouter.proxy.proxy_server.delete_verification_tokens(): Exception occured - %s", e
        )
        verbose_proxy_logger.debug(traceback.format_exc())
        raise e
...
def _transform_verification_tokens_to_deleted_records(
    ...
    for key in keys:
        ...
        deleted_record = DeRouter_DeletedVerificationToken.model_validate(
            {
                **key_payload,
...
async def _save_deleted_verification_token_records(
    ...
    if not records:
        return
    if tx is not None:
        await tx.derouter_deletedverificationtoken.create_many(data=records)
        return
...
async def _rotate_master_key(
    ...
    2. Decrypt the values
        - ModelTable
            - [{"model_name": "str", "derouter_params": {}}]
        - ConfigTable
    ...
    from derouter.proxy.proxy_server import proxy_config
    ...
    if models:
        ...
        verbose_proxy_logger.debug("Re-encrypting derouter_params on %s model rows", len(reencrypted_models))
        async with prisma_client.db.tx(timeout=timedelta(minutes=2)) as tx_ctx:
            ...
            for reencrypted_model in reencrypted_models:
                await tx.derouter_proxymodeltable.update_many(
                    data=_ModelParamsUpdate(derouter_params=prisma.Json(reencrypted_model.derouter_params)),
                    where=_ModelRowWhere(model_id=reencrypted_model.model_id),
                )
        await publish_config_change(redis_cache=coordination_redis_cache(), object_type="derouter_proxymodeltable")
    ...
    try:
        await rotate_mcp_server_credentials_master_key(
            prisma_client=prisma_client,
            touched_by=user_api_key_dict.user_id or DEROUTER_PROXY_ADMIN_NAME,
    ...
    try:
        ...
        credentials = None
    if credentials:
        from derouter.proxy.credential_endpoints.endpoints import update_db_credential
...
async def migrate_encryption_endpoint(
    ...
    """
    from derouter.proxy._types import CommonProxyErrors
    from derouter.proxy.management_endpoints.credential_migration import (
        migrate_encryption,
    )
    from derouter.proxy.proxy_server import prisma_client
    ...
    if data and data.new_key is not None:
        ...
        if not data.new_key.startswith("sk-"):
            raise HTTPException(
                ...
                detail={
                    "error": "New key must start with 'sk-'. This is to distinguish a key hash (used by derouter for logging / internal logic) from the actual key."
                },
            )
        ...
        new_token = f"sk-{secrets.token_urlsafe(LENGTH_OF_DEROUTER_GENERATED_KEY)}"
    return new_token
...
async def _insert_deprecated_key(
    ...
    """
    grace_period_value: Final = grace_period or os.getenv("DEROUTER_KEY_ROTATION_GRACE_PERIOD", "")
...
async def _execute_virtual_key_regeneration(
    *,
    ...
    from derouter.proxy.proxy_server import hash_token
    ...
    updated_token: Final[DeRouter_VerificationToken | None] = await _prisma_table(
        VerificationTokenRepository(prisma_client)
    ).update(
...
async def regenerate_key_fn(
    ...
    - data: Optional[RegenerateKeyRequest] - Request body containing optional parameters to update
        ...
        - guardrails: Optional[List[str]] - List of active guardrails for the key
        - blocked: Optional[bool] - Whether the key is blocked
        - grace_period: Optional[str] - Duration to keep old key valid after rotation (e.g. "24h", "2d"). Omitted = immediate revoke. Env: DEROUTER_KEY_ROTATION_GRACE_PERIOD
    ...
    try:
        ...
        if data is not None and (data.access_group_ids or data.object_permission is not None):
            regenerate_team_table: DeRouter_TeamTableCachedObj | None = None
...
def _validate_reset_spend_value(reset_to: object, key_in_db: DeRouter_VerificationToken) -> float:
    ...
    if key_in_db.derouter_budget_table is not None:
        budget_max_budget: Final[float | None] = getattr(key_in_db.derouter_budget_table, "max_budget", None)
...
async def _set_spend_counter_with_floor_and_broadcast(counter_key: str, value: float) -> None:
    ...
    """
    from derouter.proxy.proxy_server import SPEND_DB_FLOOR_CACHE_TTL_SECONDS, spend_counter_cache
...
async def _reset_key_budget_windows(
    ...
    `DeRouter_SpendLogs` rows inside `[window_start, now)` on every read below
...
async def validate_key_list_check(
    ...
    complete_user_info: Final = DeRouter_UserTable.model_validate(complete_user_info_db_obj.model_dump())
    ...
    if key_hash:
        try:
            key_info: Final[DeRouter_VerificationToken | None] = await _prisma_table(
                VerificationTokenRepository(prisma_client)
            ).find_unique(
...
async def _fetch_user_team_objects(
    complete_user_info: DeRouter_UserTable | None,
    prisma_client: PrismaClient,
) -> list[DeRouter_TeamTable]:
    ...
    return [DeRouter_TeamTable.model_validate(team.model_dump()) for team in teams]


def _get_admin_team_ids_from_objects(
    user_api_key_dict: UserAPIKeyAuth,
    team_objects: list[DeRouter_TeamTable],
) -> list[str]:
...
async def key_aliases(
    ...
    try:
        ...
        count_sql: Final = f'SELECT COUNT(*) AS count FROM "DeRouter_VerificationToken" WHERE {where_sql}'
        ...
        aliases_sql: Final = (
            f"SELECT key_alias"
            f' FROM "DeRouter_VerificationToken"'
            f" WHERE {where_sql}"
...
async def _list_key_helper(
    ...
    if use_deleted_table:
        ...
        keys = await VerificationTokenRepository(prisma_client).table.find_many(
            ...
            include={"object_permission": True, "derouter_budget_table": True},
        )
    ...
    # Prepare response
    key_list: Final[list[str | UserAPIKeyAuth | DeRouter_DeletedVerificationToken]] = []
    for key in keys:
        ...
        if return_full_object is True or (expand and "user" in expand):
            if use_deleted_table:
                ...
                key_list.append(DeRouter_DeletedVerificationToken.model_validate(key_dict))
            else:
                key_list.append(
...
) -> DeRouter_VerificationToken | None:
    """
...
async def test_key_logging(
    ...
    from derouter.proxy.proxy_server import general_settings, proxy_config
    ...
    try:
        data = {
            "model": "openai/derouter-key-health-test",
            "messages": [
                {
                    "role": "user",
                    "content": "Hello, this is a test from derouter /key/health. No LLM API call was made for this",
                }
            ],
        }
        ...
        await derouter.acompletion(**data)  # make mock completion call to trigger key based callbacks
    except Exception as e:
...
def _validate_key_alias_format(key_alias: str | None) -> None:
    ...
    ``derouter.enable_key_alias_format_validation``.

    The remaining charset/length rules are gated behind
    ``derouter.enable_key_alias_format_validation`` (default **False**). When disabled,
    ...
    if not derouter.enable_key_alias_format_validation:
        return
...
def validate_model_max_budget(model_max_budget: dict | None) -> None:
    ...
    try:
        ...
        if model_max_budget is not None:
            from derouter.proxy.proxy_server import CommonProxyErrors, premium_user
        ...
        raise ValueError(
            f"Invalid model_max_budget: {e}. Example of valid model_max_budget: https://router.delify.vn/docs/proxy/users"
        )
