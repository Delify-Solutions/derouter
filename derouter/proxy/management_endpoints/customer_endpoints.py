...
if TYPE_CHECKING:
    from prisma.models import DeRouter_BudgetTable as PrismaBudgetRow
    from prisma.models import DeRouter_EndUserTable as PrismaEndUserRow
...
from derouter.proxy.management_endpoints.common_utils import validate_budget_duration
...
from derouter.repositories.table_repositories import EndUserRepository
...
async def unblock_user(data: BlockUsers):
    ...
    if (
        not any(isinstance(x, _ENTERPRISE_BlockedUserList) for x in derouter.callbacks)
        or derouter.blocked_user_list is None
    ):
        raise HTTPException(
            status_code=400,
    ...
    if isinstance(derouter.blocked_user_list, list):
        for id in data.user_ids:
            derouter.blocked_user_list.remove(id)
    else:
        raise HTTPException(
            status_code=500,
    ...
    return {"blocked_users": derouter.blocked_user_list}
...
async def _handle_customer_object_permission_update(
    ...
    end_user_table_data_typed: DeRouter_EndUserTable | None,
...
async def new_end_user(
    ...
    - object_permission: Optional[DeRouter_ObjectPermissionBase] - Customer-specific object permissions to control access to resources.
        Supported fields:
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "derouter.proxy.management_endpoints.customer_endpoints.new_end_user(): Exception occured - %s", e
        )
...
async def end_user_info(
    ...
    Example curl:
    ```
    curl -X GET 'http://localhost:4000/customer/info?end_user_id=test-derouter-user-4' \
        -H 'Authorization: Bearer sk-1234'
    ```
    """
    try:
        ...
        verbose_proxy_logger.exception(
            "derouter.proxy.management_endpoints.customer_endpoints.end_user_info(): Exception occured - %s", e
        )
        raise handle_exception_on_proxy(e)
...
async def update_end_user(
    ...
    try:
        ...
        end_user_table_data: Final = await _typed_table(EndUserRepository(prisma_client)).find_first(
            where={"user_id": data.user_id}, include={"derouter_budget_table": True}
        )
        ...
        end_user_table_data_typed: Final = DeRouter_EndUserTable.model_validate(end_user_table_data.model_dump())

        ## Get budget table data ##
        end_user_budget_table: Final = end_user_table_data_typed.derouter_budget_table
        ...
        for k, v in non_default_values.items():
            ...
            elif k in DeRouter_BudgetTable.model_fields:
                budget_table_data[k] = v

            elif k in DeRouter_EndUserTable.model_fields:
                update_end_user_table_data[k] = v
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.update_end_user(): Exception occured - %s", e)
        raise handle_exception_on_proxy(e)
...
async def delete_end_user(
    ...
    try:
        ...
        verbose_proxy_logger.error("derouter.proxy.proxy_server.delete_end_user(): Exception occured - %s", e)
        raise handle_exception_on_proxy(e)
...
async def list_end_user(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "derouter.proxy.management_endpoints.customer_endpoints.list_end_user(): Exception occured - %s", e
        )
        raise handle_exception_on_proxy(e)
...
async def get_customer_daily_activity(
    ...
    return await get_daily_activity(
        prisma_client=prisma_client,
        table_name="derouter_dailyenduserspend",
