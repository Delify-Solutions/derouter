...
from derouter.types.utils import CostPerToken, LlmProvidersSet, ModelInfo
...
def _extract_custom_pricing(
    derouter_params: Mapping[str, object], model_info: Mapping[str, object]
) -> CostPerToken | None:
    """
    ...
    Pricing may live on ``derouter_params`` or ``model_info``; ``derouter_params``
    wins, matching the router's cost-map registration precedence.
    """
    sources: Final = (derouter_params, model_info)
...
def _lookup_model_info(model: str) -> ModelInfo | None:
    try:
        return derouter.get_model_info(model=model)
    except Exception:
        return None
...
def _resolve_model_for_cost_lookup(model: str) -> ResolvedCostModel:
    ...
    underlying derouter model name, provider, and any deployment-configured
    ...
    if llm_router is not None:
        try:
            ...
            if deployments and len(deployments) > 0:
                ...
                derouter_params: Final = first_deployment.get("derouter_params", {})
                ...
                custom_cost_per_token: Final = _extract_custom_pricing(derouter_params, model_info)

                # Check base_model first (needed for Azure custom deployment names)
                base_model: Final = model_info.get("base_model") or derouter_params.get("base_model")
                if base_model:
                ...
                resolved_model: Final = derouter_params.get("model")
                if resolved_model:
...
async def get_cost_discount_config(
    ...
    Returns the cost_discount_config from derouter_settings.
    """
    ...
    try:
        ...
        # Get cost_discount_config from derouter_settings
        derouter_settings: Final = config.get("derouter_settings", {})
        cost_discount_config: Final = derouter_settings.get("cost_discount_config", {})
...
async def update_cost_discount_config(
    ...
    Updates the cost_discount_config in derouter_settings.
    ...
    # Validate that all providers are valid DeRouter providers
    ...
    if invalid_providers:
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Invalid provider(s): {', '.join(invalid_providers)}. Must be valid DeRouter providers. See https://router.delify.vn/docs/providers for the full list."
            },
        )
    ...
    try:
        ...
        # Ensure derouter_settings exists
        if "derouter_settings" not in config:
            config["derouter_settings"] = {}

        # Update cost_discount_config
        config["derouter_settings"]["cost_discount_config"] = cost_discount_config

        # Save the updated config to DB
        await proxy_config.save_config(new_config=config)

        # Update in-memory derouter.cost_discount_config
        derouter.cost_discount_config = cost_discount_config
...
async def get_cost_margin_config(
    ...
    Returns the cost_margin_config from derouter_settings.
    """
    from derouter.proxy.proxy_server import prisma_client, proxy_config
    ...
    try:
        ...
        # Get cost_margin_config from derouter_settings
        derouter_settings: Final = config.get("derouter_settings", {})
        cost_margin_config: Final = derouter_settings.get("cost_margin_config", {})
...
async def update_cost_margin_config(
    ...
    Updates the cost_margin_config in derouter_settings.
    Margins can be:
    ...
    # Validate that all providers are valid DeRouter providers (except "global")
    invalid_providers: Final = []
    ...
    if invalid_providers:
        raise HTTPException(
            status_code=400,
            detail={
                "error": f"Invalid provider(s): {', '.join(invalid_providers)}. Must be valid DeRouter providers or 'global'. See https://router.delify.vn/docs/providers for the full list."
            },
        )
    ...
    try:
        ...
        config["derouter_settings"]["cost_margin_config"] = cost_margin_config

        # Save the updated config to DB
        await proxy_config.save_config(new_config=config)

        # Update in-memory derouter.cost_margin_config
        derouter.cost_margin_config = cost_margin_config
...
async def get_block_requests_for_models_without_pricing() -> BlockUnpricedModelsResponse:
    return BlockUnpricedModelsResponse(enabled=bool(derouter.block_requests_for_models_without_pricing))
...
async def update_block_requests_for_models_without_pricing(
    ...
    try:
        ...
        if "derouter_settings" not in config:
            config["derouter_settings"] = {}  # mutable-ok: config is a plain-dict payload for save_config
        config["derouter_settings"]["block_requests_for_models_without_pricing"] = request.enabled
        await proxy_config.save_config(new_config=config)

        derouter.block_requests_for_models_without_pricing = request.enabled
...
async def estimate_cost(
    ...
    derouter_logging_obj: Final = DeRouterLoggingObj(
        model=resolved_model,
        messages=[],
        stream=False,
        call_type="completion",
        start_time=None,
        derouter_call_id="cost-estimate",
    ...
    mapped_provider: Final = model_info.get("derouter_provider") if model_info is not None else None
