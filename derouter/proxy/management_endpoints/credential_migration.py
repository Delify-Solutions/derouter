...
async def _migrate_config_settings_row(
    ...
    """Migrate a single ``DeRouter_Config`` row whose ``param_value`` is a JSON
    dict with selected sensitive fields (vantage_settings / cloudzero_settings).
    """
    report: Final = LocationReport(location=param_name)
    record: Final = await prisma_client.db.derouter_config.find_unique(where={"param_name": param_name})
    ...
    if changed and not dry_run:
        await prisma_client.db.derouter_config.update(
...
async def _migrate_sso_config(prisma_client: object, dry_run: bool) -> LocationReport:
    """Migrate the ``DeRouter_SSOConfig`` row. All non-null fields are encrypted
    ...
    record: Final = await prisma_client.db.derouter_ssoconfig.find_unique(where={"id": "sso_config"})
    ...
    if changed and not dry_run:
        await prisma_client.db.derouter_ssoconfig.update(
            where={"id": "sso_config"},
...
async def _migrate_callback_vars_table(
    ...
    if table_name == "team":
        table = prisma_client.db.derouter_teamtable
        pk = "team_id"
    else:
        table = prisma_client.db.derouter_verificationtoken
        pk = "token"
    ...
    for row in rows or []:
        ...
        # Classify every callback-var value directly (strip the derouter_enc::
...
def _classify_callback_value(value: object) -> ValueClass:
    ...
    Encrypted callback vars carry the ``derouter_enc::`` marker in front of the
...
_COVERED_TABLE_SPECS: Final = [
    ("model_table", "derouter_proxymodeltable", ("derouter_params",), ()),
    ("credentials", "derouter_credentialstable", ("credential_values",), ()),
    ("mcp_server", "derouter_mcpservertable", ("credentials", "env_vars", "static_headers", "env"), ()),
    ("mcp_user_credentials", "derouter_mcpusercredentials", (), ("credential_b64",)),
    ("mcp_user_env_vars", "derouter_mcpuserenvvars", (), ("values_b64",)),
]
...
async def _scan_one_table(
    ...
    for row in rows or []:
        for col in json_columns:
            ...
            if raw is None:
                continue
            if db_attr == "derouter_mcpservertable" and col in ("static_headers", "env"):
...
async def _scan_config_env_vars(prisma_client: object) -> LocationReport:
    ...
    try:
        record: Final = await prisma_client.db.derouter_config.find_unique(where={"param_name": "environment_variables"})
...
async def _migrate_covered_tables(prisma_client: object, user_api_key_dict: object) -> list[LocationReport]:
    ...
    if current_key is None:
        raise RuntimeError(
            "Cannot migrate covered tables: no salt key / master key is set. Set DEROUTER_SALT_KEY before migrating."
        )
