...
"""
1. Create pass-through endpoints for any DEROUTER_BASE_URL/langfuse/<endpoint> map to LANGFUSE_BASE_URL/<endpoint>
"""
...
def _get_langfuse_proxy_credentials(
    *,
    ...
    return (
        dynamic_langfuse_public_key or derouter.utils.get_secret(secret_name="LANGFUSE_PUBLIC_KEY"),
        dynamic_langfuse_secret_key or derouter.utils.get_secret(secret_name="LANGFUSE_SECRET_KEY"),
    )
...
def _build_langfuse_proxy_target(
    *,
    ...
    if dynamic_host_supplied and getattr(derouter, "user_url_validation", True):
        try:
...
async def langfuse_proxy_route(
    ...
    """
    Call Langfuse via DeRouter proxy. Works with Langfuse SDK.

    [Docs](https://router.delify.vn/docs/pass_through/langfuse)
    """
