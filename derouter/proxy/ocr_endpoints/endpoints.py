...
from derouter.ocr.main import convert_file_document_to_url_document, get_mime_type
...
def _native_response(response: object, fastapi_response: Response) -> Response | None:
    ...
    DeRouter response headers (cost, call id, etc.) built for the normalized response.
    """
...
async def _parse_ocr_request_body(request: Request) -> dict[str, Any]:
    ...
    # JSON. These IDs are not scoped to the DeRouter proxy user/key, so an
...
async def ocr(
    ...
    **2. Multipart form file upload**:
    ...
    Response format is normalized to the DeRouter OCR schema by default. Providers
