...
from derouter.constants import MAX_POLICY_ESTIMATE_IMPACT_ROWS
...
async def _fetch_all_teams(prisma_client: object) -> "Sequence[prisma_models.DeRouter_TeamTable]":
...
def _filter_keys_by_tags(
    keys: "Sequence[prisma_models.DeRouter_VerificationToken]", tag_patterns: Sequence[str]
) -> tuple[list[str], int]:
...
def _filter_teams_by_tags(
    teams: "Sequence[prisma_models.DeRouter_TeamTable]", tag_patterns: Sequence[str]
) -> tuple[list[str], int]:
...
async def _find_affected_by_team_patterns(
    ...
    all_teams: "Sequence[prisma_models.DeRouter_TeamTable]",
