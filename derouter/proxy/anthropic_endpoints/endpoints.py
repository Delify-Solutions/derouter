...
from derouter.anthropic_interface.exceptions import AnthropicErrorResponse, AnthropicExceptionMapping
...
def _strip_total_tokens_from_anthropic_response(response: Any) -> None:
    """Remove the OpenAI-flavored `usage.total_tokens` field that DeRouter
...
async def anthropic_response(
    ...
    """
    Use `{PROXY_BASE_URL}/anthropic/v1/messages` instead - [Docs](https://router.delify.vn/docs/pass_through/anthropic_completion).
    ...
    try:
        ...
        # DeRouter adds internally. Anthropic's official /v1/messages spec
        # only defines input_tokens / output_tokens / cache_*_input_tokens;
        # total_tokens is an OpenAI convention. Default off
        # (`derouter.strip_anthropic_total_tokens = False`) to preserve
        ...
        if derouter.strip_anthropic_total_tokens:
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.anthropic_response(): Exception occured - %s", e)
        ...
        model_info: Final = derouter_metadata.get("model_info", {}) or {}
...
async def count_tokens(
    ...
    """
    from derouter.proxy.proxy_server import token_counter as internal_token_counter
    ...
    try:
        ...
        from derouter.proxy._types import TokenCountRequest
        ...
        verbose_proxy_logger.exception("derouter.proxy.anthropic_endpoints.count_tokens(): Exception occurred - %s", e)
