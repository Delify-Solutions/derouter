...
Plugins are stored as metadata + git source references in DeRouter database.
...
from derouter.proxy.common_utils.resource_ownership import is_proxy_admin
...
async def get_marketplace():
    ...
    Example:
        ...
        claude plugin install my-plugin@derouter
        ```
    """
    try:
        ...
        marketplace: Final = {
            "name": "derouter",
            "owner": {"name": "DeRouter", "email": "support@router.delify.vn"},
            "plugins": plugin_list,
        }
...
async def register_plugin(
    ...
    """
    Register a new plugin in the DeRouter marketplace.

    DeRouter acts as a registry/discovery layer. Plugins are hosted on
...
async def update_plugin(
    ...
    """
    Update an existing plugin in the DeRouter marketplace.
