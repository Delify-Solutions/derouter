...
async def _search_skills(
    ...
    to_response: Final = DeRouterSkillsTransformationHandler().db_skill_to_response
    match outcome:
...
async def create_skill(
    ...
    - Pass model via header: `x-derouter-model: claude-account-1`
    ...
    curl -X POST "http://localhost:4000/v1/skills?beta=true" \
      -H "Content-Type: multipart/form-data" \
      -H "Authorization: Bearer your-key" \
      -H "x-derouter-model: claude-account-1" \
      -F "display_title=My Skill" \
      -F "files[]=@skill.zip"
    ```
    ...
    model: Final = data.get("model") or request.query_params.get("model") or request.headers.get("x-derouter-model")
    if model:
        data["model"] = model
...
async def list_skills(
    ...
    query: Annotated[
        ...
        Query(
            ...
            "Only supported for custom_llm_provider=derouter_proxy. Requires "
            "derouter_settings.skill_search_embedding_model.",
        ),
    ] = None,
    top_k: Annotated[
        int,
    ...
    Pass `?custom_llm_provider=derouter_proxy&query=<task>` to rank the DeRouter-hosted skills you can
    access by semantic similarity instead of paging through the whole registry:
    ```bash
    curl "http://localhost:4000/v1/skills?custom_llm_provider=derouter_proxy&query=summarize+a+pdf&top_k=5" \
      -H "Authorization: Bearer your-key"
    ```
