...
from derouter.repositories.table_repositories import ManagedObjectRepository
...
_NEGATIVE_OWNER_SENTINEL: Final = "__derouter_container_no_owner__"
...
_NEGATIVE_STORED_ID_SENTINEL: Final = "__derouter_container_no_stored_id__"
...
# ``derouter_managedobjecttable``. The cache key is the sorted owner-scope
...
async def get_container_forwarding_params(
    ...
    if not (isinstance(model_id, str) and model_id):
        # Native upstream IDs (e.g. Azure ``cntr_<hex>``) carry no DeRouter
...
async def _get_container_owner(original_container_id: str, custom_llm_provider: str) -> str | None:
    ...
    table: Final = ManagedObjectRepository(prisma_client).table
    row: Final[prisma_models.DeRouter_ManagedObjectTable | None] = await table.find_first(
        where={
            "model_object_id": model_object_id,
            "file_purpose": CONTAINER_OBJECT_PURPOSE,
        }
...
async def _get_allowed_container_ids(
    ...
    table: Final = ManagedObjectRepository(prisma_client).table
    rows: Final[Sequence[prisma_models.DeRouter_ManagedObjectTable]] = await table.find_many(
        where={
            "file_purpose": CONTAINER_OBJECT_PURPOSE,
