"""
Plugin proxy routes for derouter.

Enables external services to register as plugins and be proxied through
the derouter proxy server.

Config (in derouter config.yaml general_settings):
  plugins:
    - name: my-plugin
...
Plugin iframe auth:
  ...
  derived as HMAC-SHA256(DEROUTER_SALT_KEY, plugin_name).  The claim carries no
  derouter bearer token, so a compromised plugin learns only the caller's
  identity, never their credential.  DEROUTER_SALT_KEY itself is never shared
...
from derouter.proxy._types import PluginConfig, SpecialHeaders, UserAPIKeyAuth
...
# Hop-by-hop headers (RFC 7230) and the derouter session cookie — never forwarded
...
def _configured_key_header_names() -> frozenset[str]:
    """The lowercased general_settings.derouter_key_header_name, if configured.
    ...
    if not isinstance(general_settings, dict):
        return frozenset()
    name: Final[object] = general_settings.get("derouter_key_header_name")
...
def _request_strip_headers() -> frozenset[str]:
    ...
    Every header user_api_key_auth accepts as a derouter credential is stripped —
    Authorization, x-api-key, API-Key, x-goog-api-key, Ocp-Apim-Subscription-Key,
    x-derouter-api-key, and any configured custom key header — so a plugin can
    never be handed the caller's live derouter key (confused-deputy escalation).
    """
    return _HOP_BY_HOP_STRIP | SpecialHeaders.derouter_credential_header_names() | _configured_key_header_names()
...
def _safe_response_headers(raw: "Mapping[str, str]") -> dict[str, str]:
    ...
    Plugin-controlled bytes are served from the derouter dashboard origin, so a
...
def issue_plugin_session_claim(plugin_name: str, user_id: str | None, user_role: str | None) -> str:
    ...
    contains NO derouter bearer token — the plugin can only derive the
...
async def plugin_auth_token(
    user_api_key_dict: UserAPIKeyAuth = Depends(user_api_key_auth),
    plugin_name: str = "derouter-platform-plugin",
) -> dict:
    """Issue a short-lived, audience-scoped plugin session claim.

    The claim contains {user_id, user_role, plugin, exp}.  It does NOT
    contain the caller's derouter bearer token — a compromised plugin can
    only learn the caller's identity, not impersonate them against the proxy.

    Encrypted with a key derived from HMAC(DEROUTER_SALT_KEY, plugin_name),
    so each plugin holds only its own key and cannot forge claims for others.

    Requires DEROUTER_SALT_KEY to be set; returns 503 otherwise.
    """
    if not os.getenv("DEROUTER_SALT_KEY"):
        raise HTTPException(
            status_code=503,
            detail="DEROUTER_SALT_KEY is not configured; plugin iframe auth unavailable.",
        )
...
async def plugin_proxy(
    ...
    The caller's derouter credential is stripped and replaced with the
    plugin's own plugin_key so plugins never receive a live derouter API key.
    """
    ...
    user_role: Final = getattr(user_api_key_dict, "user_role", None)
    if user_id:
        forward_headers["x-derouter-user-id"] = str(user_id)
    if user_role:
        forward_headers["x-derouter-user-role"] = str(user_role)
