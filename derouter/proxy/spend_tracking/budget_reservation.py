...
from derouter.derouter_core_utils.llm_cost_calc.tiered_pricing import select_tier_for_input, tier_rate
...
from derouter.proxy.auth.auth_utils import get_model_from_request
from derouter.proxy.auth.budget_throttle import should_throttle_budget_exceeded
...
async def _get_end_user_budget_counter(
    ...
    if end_user_object is not None:
        ...
        if max_budget is None:
            budget_table: Final = _get_value(end_user_object, "derouter_budget_table")
...
async def _get_tag_budget_counters(
    ...
    for tag_name in tag_names:
        ...
        if tag_object is None:
            continue
        budget_table = _get_value(tag_object, "derouter_budget_table")
        max_budget = _to_float(_get_value(budget_table, "max_budget"))
        if max_budget is None or max_budget <= 0:
            continue
        counters.append(
...
async def _get_model_access_group_budget_counters(
    ...
    """
    from derouter.proxy.auth.auth_checks import get_model_access_group_budgets_batch
...
async def _get_team_member_budget_counter(
    ...
    team_membership: DeRouter_TeamMembership | None = None
    if isinstance(cached_team_membership, DeRouter_TeamMembership):
        team_membership = cached_team_membership
    elif isinstance(cached_team_membership, dict):
        team_membership = DeRouter_TeamMembership(**cached_team_membership)

    team_member_budget: float | None = None
    if team_membership is not None and team_membership.derouter_budget_table is not None:
    ...
    team_member_spend = cast(DeRouter_TeamMembership, team_membership).spend if team_membership is not None else 0.0
    return _BudgetCounter(
...
async def _get_org_budget_counter(
    ...
    org_budget_table: Final = _get_value(org_table, "derouter_budget_table")
...
async def _counter_can_apply_adjustment(
    ...
    from derouter.proxy.proxy_server import read_spend_counter_cache_value
...
def _get_model_cost_info(
    ...
    return dict(derouter.get_model_info(model=model))
...
def _deployment_tiered_pricing_table(
    ...
    backend_model: Final = _get_value(_get_value(deployment, "derouter_params"), "model")
...
def _count_text_tokens(model: str, text: object) -> int:
    ...
    while stack:
        ...
        if isinstance(item, dict):
            token_count += derouter.token_counter(model=model, text=json.dumps(item))
            continue
        token_count += derouter.token_counter(model=model, text=str(item))
    return token_count
