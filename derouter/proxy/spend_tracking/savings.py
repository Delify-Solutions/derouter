...
from derouter.types.utils import ModelInfo, PromptTokensDetailsWrapper, Usage
...
def _resolve_model(model: str | None, custom_llm_provider: str | None) -> _ModelIdentity | None:
    ...
    if not model:
        return None
    try:
        resolved_model, provider, _, _ = derouter.get_llm_provider(model=model, custom_llm_provider=custom_llm_provider)
...
def _model_info(model: _ModelIdentity) -> ModelInfo | None:
    ...
    try:
        return derouter.get_model_info(model=model.model, custom_llm_provider=model.provider)
...
def marks_gateway_injection(metadata: Mapping[str, object] | None, model_id: str | None) -> bool:
    ...
    request shares one metadata bucket and one ``derouter_call_id``, so the deployment is
...
def compute_savings_spend(
    ...
    a value only when ``gateway_injected_cache`` is set, i.e. derouter itself added the
