...
from derouter.derouter_core_utils.internal_call_metadata import is_unbilled_non_inference_call
from derouter.derouter_core_utils.derouter_logging import (
    coerce_model_access_groups,
    is_valid_sha256_hash,
    request_model_access_groups_from_derouter_params,
)
from derouter.derouter_core_utils.safe_json_dumps import safe_dumps, strip_null_bytes
from derouter.proxy._types import SpendLogsMetadata, SpendLogsPayload, SpendLogsRouterMetadata
from derouter.proxy.spend_tracking.spend_log_error_logger import spend_log_error
from derouter.proxy.utils import PrismaClient, hash_token
...
def _get_spend_logs_metadata(
    ...
    derouter_overhead_time_ms: float | None = None,
    ...
    if metadata is None:
        return SpendLogsMetadata(
            ...
            derouter_gateway_injected_cache=None,
    ...
    clean_metadata["derouter_overhead_time_ms"] = derouter_overhead_time_ms
    clean_metadata["cost_breakdown"] = cost_breakdown
    clean_metadata["autorouter_savings"] = autorouter_savings
    clean_metadata["derouter_call_id"] = derouter_call_id
...
def get_request_model_access_groups(kwargs: Mapping[str, object] | None) -> tuple[str, ...]:
    ...
    if not isinstance(derouter_params, Mapping):
        return ()
    return request_model_access_groups_from_derouter_params(derouter_params)
...
def get_logging_payload(kwargs, response_obj, start_time, end_time) -> SpendLogsPayload:
    ...
    derouter_params: Final = kwargs.get("derouter_params", {})
    metadata: Final = get_derouter_metadata_from_kwargs(kwargs)
    ...
    if call_type in ["ocr", "aocr"]:
        ...
        if isinstance(_usage, derouter.Usage):
            usage = dict(_usage)
    ...
    if not usage and isinstance(_combined_usage, derouter.Usage):
    ...
    end_user_id = get_end_user_id_for_cost_tracking(derouter_params)
    ...
    _api_base: Final = derouter_params.get("api_base", "") or _sl_attribution_fallback(
    ...
    if standard_logging_payload is not None:
        ...
        derouter_overhead_time_ms = hidden_params.get("derouter_overhead_time_ms")
    ...
    derouter_call_id: Final = cast(
        str | None,
        kwargs.get("derouter_call_id") or derouter_params.get("derouter_call_id"),
    )
    ...
    clean_metadata = _get_spend_logs_metadata(
        ...
        derouter_overhead_time_ms=derouter_overhead_time_ms,
        cost_breakdown=(
        ...
        router_metadata=_get_router_metadata_for_spend_log(
            ...
            router_correlation_id=derouter_call_id,
        ),
    )
    ...
    if derouter.cache is not None:
        cache_key = derouter.cache.get_cache_key(**kwargs)
    else:
        cache_key = "Cache OFF"
...
def _omits_session_id_when_missing(metadata: Mapping[str, object] | None) -> bool:
    ...
    from both metadata buckets before stamping, which the merge of `derouter_metadata` into `metadata` makes
...
def _get_session_id_for_spend_log(
    ...
    """Under `omit` only `metadata.session_id`, the key Langfuse reads, counts as a session; `derouter_session_id` may
    ...
    if kwargs.get("derouter_trace_id") is not None:
        return str(kwargs.get("derouter_trace_id"))
        ...
        if isinstance(value, dict):
            ...
            if len(value) > max_string_length_prompt_in_db:
                ...
                truncated_value: Final = (
                    f"{value[:start_chars]}"
                    f"... ({DEROUTER_TRUNCATED_PAYLOAD_FIELD} skipped {skipped_chars} chars. "
                    f"{DEROUTER_TRUNCATION_DB_SAFEGUARD_NOTE}) ..."
                    f"{value[-end_chars:]}"
                )
...
def _sanitize_guardrail_information_for_spend_logs(
    ...
    ``classification``) before they land in ``DeRouter_SpendLogs.metadata``.

    Guardrail hooks may echo the LLM request payload back into
    ``guardrail_response``, and two first-party hooks
    (``block_code_execution``, ``derouter_content_filter``) inline user-prompt
...
def _sanitize_error_information_for_spend_logs(
    ...
    """
    Sanitize ``error_information`` before it lands in ``DeRouter_SpendLogs.metadata``.
    ...
    the untruncated error per ``DEROUTER_TRUNCATION_DB_SAFEGUARD_NOTE``.
    """
...
def _get_proxy_server_request_for_spend_logs_payload(
    ...
    """
    if _should_store_prompts_and_responses_in_spend_logs():
        _proxy_server_request: Final = cast(dict | None, derouter_params.get("proxy_server_request", {}))
        if _proxy_server_request is not None:
            ...
            if DEROUTER_TRUNCATED_PAYLOAD_FIELD in _request_body_json_str:
...
def _get_response_for_spend_logs_payload(
    ...
    if _should_store_prompts_and_responses_in_spend_logs():
        ...
        if DEROUTER_TRUNCATED_PAYLOAD_FIELD in result_str:
            verbose_proxy_logger.info(
