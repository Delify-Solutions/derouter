...
from derouter.proxy._types import ProviderBudgetResponse, ProviderBudgetResponseObject
...
def _spend_logs_table(prisma_client: PrismaClient) -> TableActions["prisma_models.DeRouter_SpendLogs"]:
...
async def _find_spend_logs(
    ...
    if len(rows) == take:
        http_response.headers["x-derouter-spend-logs-truncated"] = "true"
...
@router.get(
    ...
    dependencies=[Depends(user_api_key_auth)],
    responses={
        200: {"model": list[DeRouter_SpendLogs]},
    },
)
async def view_spend_tags(
    ...
    """
    DeRouter Enterprise - View Spend Per Request Tag
...
async def get_global_spend_provider(
    ...
    from derouter.proxy.proxy_server import llm_router, prisma_client
        ...
        for row in db_response:
            ...
            if llm_router is not None:
                ...
                if _deployment is not None:
                    try:
                        _, _provider, _, _ = derouter.get_llm_provider(
                            model=_deployment.derouter_params.model,
                            custom_llm_provider=_deployment.derouter_params.custom_llm_provider,
                            api_base=_deployment.derouter_params.api_base,
                            derouter_params=_deployment.derouter_params,
                        )
...
async def _resolve_org_spend_report_scope(
    ...
    """
    from derouter.proxy.management_endpoints.organization_endpoints import _verify_org_access
...
async def global_view_spend_tags(
    ...
    """
    DeRouter Enterprise - View Spend Per Request Tag. Used by DeRouter UI
...
async def _get_spend_report_for_time_range(
    ...
    try:
        ...
        FROM
            "DeRouter_SpendLogs" s
        LEFT JOIN
            "DeRouter_TeamTable" t ON s.team_id = t.team_id
        WHERE
...
async def calculate_spend(request: SpendCalculateRequest):
    ...
    Note: If you see a spend of $0.0 you need to set custom_pricing for your model: https://router.delify.vn/docs/proxy/custom_pricing
    ...
    try:
        ...
        from derouter.cost_calculator import CostPerToken
        ...
        if request.model is not None:
            ...
            1. user passes model, and model is defined on derouter config.yaml or in DB. use info on config or in DB in this case
            2. user passes model, and model is not defined on derouter config.yaml or in DB. Pass model as is to derouter.completion_cost
            3. user passes completion_response
            
            """
            if _model_in_llm_router is not None:
                _derouter_params: Final = _model_in_llm_router.get("derouter_params")
                _derouter_model_name: Final = _derouter_params.get("model")
                input_cost_per_token: Final = _derouter_params.get("input_cost_per_token")
                output_cost_per_token: Final = _derouter_params.get("output_cost_per_token")
            ...
            _completion_response: Final = derouter.ModelResponse(**request.completion_response)
...
async def ui_view_spend_logs(
    ...
    model_id: str | None = fastapi.Query(
        default=None,
        description="Filter logs by model ID (derouter model deployment id)",
    ),
    ...
    exclude_internal_health_checks: bool = fastapi.Query(
        default=False,
        description="Exclude DeRouter internal health check requests from results",
    ),
...
async def _ui_session_grouped_spend_logs(
    ...
    Sessions are derived on the fly from ``DeRouter_SpendLogs`` (no extra
...
async def ui_view_request_response_for_request_id(
    ...
    custom_loggers: Final = derouter.logging_callback_manager.get_active_additional_logging_utils_from_custom_logger()
    start_date_obj: datetime | None = None
    end_date_obj: datetime | None = None
    ...
    if prisma_client is not None:
        from derouter.proxy.spend_tracking.cold_storage_handler import (
            ColdStorageHandler,
        )
...
async def global_spend_reset():
    ...
    Globally reset spend for All API Keys and Teams, maintain DeRouter_SpendLogs

    1. DeRouter_SpendLogs will maintain the logs on spend, no data gets deleted from there
    2. DeRouter_VerificationTokens spend will be set = 0
    3. DeRouter_TeamTable spend will be set = 0
...
async def global_spend_logs(
    ...
    from derouter.integrations.prometheus_helpers.prometheus_api import (
...
async def global_spend():
    ...
    try:
        ...
        return {"spend": total_spend, "max_budget": derouter.max_budget}
    except Exception as e:
        error_trace: Final = traceback.format_exc()
        error_str: Final = str(e) + "\n" + error_trace
        if isinstance(e, HTTPException):
...
async def global_spend_key_internal_user(user_api_key_dict: UserAPIKeyAuth, limit: int = 10):
    ...
    sql_query: Final = """
        ...
        FROM 
            top_api_keys t
        LEFT JOIN 
            "DeRouter_VerificationToken" v ON t.api_key = v.token
        ORDER BY 
            t.total_spend DESC;
...
async def global_view_all_end_users():
    ...
    sql_query: Final = """
    SELECT DISTINCT end_user FROM "DeRouter_SpendLogs"
    """
...
async def provider_budgets() -> ProviderBudgetResponse:
    """
    Provider Budget Routing - Get Budget, Spend Details https://router.delify.vn/docs/proxy/provider_budget_routing
    ...
    try:
        ...
        if provider_budget_config is None:
            raise ValueError(
                "No provider budget config found. Please set a provider budget config in the router settings. https://router.delify.vn/docs/proxy/provider_budget_routing"
            )
...
async def _get_permitted_team_ids_for_spend_logs(
    ...
    for team_row in team_rows:
        team_obj = DeRouter_TeamTable.model_validate(team_row.model_dump())
