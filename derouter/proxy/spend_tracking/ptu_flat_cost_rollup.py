...
(``DeRouter_ProxyModelTable.model_info``): a deployment carrying ``ptu_count``
...
charges one hour that day). The amount is written to ``DeRouter_DailyTeamSpend``
...
from derouter.derouter_core_utils.ptu_pricing import ptu_terms
...
class _PTUDeployment:
    ...
    A ``DeRouter_ProxyModelTable`` row already has it. A router entry does not: its id
...
async def _upsert_ptu_daily_row(
    ...
    *,
    ...
    """Idempotent upsert of a sentinel-api_key row on DeRouter_DailyTeamSpend.
    ...
    await prisma_client.db.derouter_dailyteamspend.upsert(
        where=where,
...
def _config_deployments(router: object | None, *, owned_by_db: frozenset[str]) -> tuple[_PTUDeployment, ...]:
    """Deployments the router holds that no ``DeRouter_ProxyModelTable`` row owns.
...
async def _load_ptu_models(prisma_client: "PrismaClient", *, router: object | None) -> _LoadedDeployments:
    ...
    """
    rows: Final = await prisma_client.db.derouter_proxymodeltable.find_many()
...
async def _existing_sentinel_keys(
    ...
    *,
    ...
    rows: Final = await prisma_client.db.derouter_dailyteamspend.find_many(
...
async def _prune_unrefreshed_sentinel_rows(
    ...
    *,
    ...
    deletions: Final = tuple(
        [await prisma_client.db.derouter_dailyteamspend.delete_many(where=where) for where in filters]
    )
