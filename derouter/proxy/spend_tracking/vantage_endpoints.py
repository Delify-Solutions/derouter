...
from derouter.types.proxy.vantage_endpoints import (
...
class _VantageConfigRow(Protocol):
    """The ``DeRouter_Config`` row holding ``vantage_settings``, as this module reads it."""
...
def _get_registered_vantage_logger():
    """Return the VantageLogger already registered in derouter.callbacks, if any."""
    from derouter.integrations.vantage.vantage_logger import VantageLogger

    vantage_loggers: Final = derouter.logging_callback_manager.get_custom_loggers_for_type(callback_type=VantageLogger)
    if vantage_loggers:
...
async def vantage_dry_run_export(
    ...
    try:
        ...
        from derouter.integrations.focus.database import FocusDeRouterDatabase
        from derouter.integrations.focus.export_engine import FocusExportEngine
        from derouter.integrations.focus.transformer import FocusTransformer

        database: Final = FocusDeRouterDatabase()
...
async def delete_vantage_settings(
    ...
    try:
        ...
        derouter.logging_callback_manager.remove_callbacks_by_type(derouter.callbacks, VantageLogger)
