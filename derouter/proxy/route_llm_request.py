...
from derouter.router_utils.common_utils import _is_proxy_admin_request
...
# names are kept in sync with ``derouter.types.router.MockRouterTestingParams``
...
# import time) avoids a cyclic import: ``derouter.types.router`` imports
...
def _route_user_config_request(data: dict, route_type: str):
    ...
    valid_args: Final = derouter.Router.get_valid_args()
    filtered_config: Final = {k: v for k, v in router_config.items() if k in valid_args}

    user_router: Final = derouter.Router(**filtered_config)
...
def _raise_if_model_fully_blocked(llm_router: LitellmRouter, model_name: object, team_id: str | None) -> None:
    if not isinstance(model_name, str) or not model_name:
        return
    if not isinstance(llm_router, derouter.Router):
        return
    ...
    if llm_router._are_all_deployments_blocked(deployments):
        raise derouter.PermissionDeniedError(
            message="Model is blocked",
            model=model_name,
            llm_provider="",
            response=httpx.Response(
                status_code=403,
                request=httpx.Request(method="POST", url="https://github.com/Delify-Solutions/derouter"),
            ),
        )
...
def get_team_id_from_data(data: dict) -> str | None:
    """
    Get the team id from the data's metadata or derouter_metadata params.
    """
    if "metadata" in data and data["metadata"] is not None and "user_api_key_team_id" in data["metadata"]:
        ...
        "derouter_metadata" in data
        and data["derouter_metadata"] is not None
        and "user_api_key_team_id" in data["derouter_metadata"]
    ):
        return data["derouter_metadata"].get("user_api_key_team_id")
    return None
...
async def _route_request_single_attempt(  # noqa: ANN202  # returns unawaited provider coroutines; the inferred union keeps route_request's callers typed
    ...
    if "api_key" in data or "api_base" in data:
        if llm_router is not None:
            ...
            return getattr(derouter, f"{route_type}")(**data)
        ...
        # Evals API: always route to derouter directly (not through router)
            ...
            if model and llm_router:
                try:
                    ...
                    if not deployment_creds:
                        ...
                        if (
                            deployment
                            and deployment.derouter_params
                            and not llm_router._is_deployment_blocked(deployment)
                        ):
                            deployment_creds = deployment.derouter_params.model_dump(exclude_none=True)
        ...
        if team_model_name is not None:
            ...
            if llm_router.router_general_settings.pass_through_all_models:
                ...
                from derouter.proxy.agent_endpoints.a2a_routing import (
