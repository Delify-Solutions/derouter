...
from derouter.repositories.table_repositories import MemoryRepository
from derouter.repositories.team_repository import TeamRepository
from derouter.types.memory_management import (
    DeRouter_MemoryRow,
    MemoryCreateRequest,
...
def _memory_table(prisma_client: "PrismaClient") -> TableActions["prisma_models.DeRouter_MemoryTable"]:
...
def _row_to_model(row: "prisma_models.DeRouter_MemoryTable") -> DeRouter_MemoryRow:
    return DeRouter_MemoryRow(
...
async def _assert_write_access(
    prisma_client: "PrismaClient", row: "prisma_models.DeRouter_MemoryTable", user_api_key_dict: UserAPIKeyAuth
) -> None:
    """
...
@router.post(
    ...
    response_model=DeRouter_MemoryRow,
)
...
) -> "prisma_models.DeRouter_MemoryTable":
...
async def _find_visible_memory_or_none(
    prisma_client: "PrismaClient", key: str, user_api_key_dict: UserAPIKeyAuth
) -> "prisma_models.DeRouter_MemoryTable | None":
