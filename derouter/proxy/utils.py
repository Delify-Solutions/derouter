...
from derouter.constants import (
    ...
    DEROUTER_LOGGING_NO_UPSTREAM_LLM_CALL,
    MAX_TEAM_LIST_LIMIT,
...
from derouter.proxy.common_utils.openai_error_payload import openai_error_param
from derouter.proxy.spend_tracking.spend_log_error_logger import spend_log_error
...
from derouter.types.utils import CallTypes, CallTypesLiteral, ModelInfo, Usage

try:
    from derouter_enterprise.enterprise_callbacks.send_emails.base_email import (
        BaseEmailLogger,
    )
...
import derouter.derouter_core_utils.derouter_logging
...
from derouter._logging import _redact_string, verbose_proxy_logger
from derouter._service_logger import ServiceLogging, ServiceTypes
from derouter.caching.caching import DualCache, RedisCache
from derouter.caching.dual_cache import LimitedSizeOrderedDict
from derouter.exceptions import RejectedRequestError, SensitiveDataRouteException
...
from derouter.integrations.SlackAlerting.utils import _add_langfuse_trace_id_to_alert
...
from derouter.llms import load_guardrail_translation_mappings
from derouter.llms.custom_httpx.http_handler import AsyncHTTPHandler
from derouter.proxy._types import (
    AlertType,
    CallInfo,
    DeRouter_VerificationTokenView,
    Member,
    UserAPIKeyAuth,
)
from derouter.proxy.auth.route_checks import RouteChecks
from derouter.proxy.common_utils.config_sync_pubsub import publish_config_param_change
from derouter.proxy.common_utils.user_api_key_cache import UserApiKeyCache
from derouter.proxy.db.create_views import (
    create_missing_views,
    create_view_tolerating_race,
    should_create_missing_views,
)
from derouter.proxy.db.db_spend_update_writer import DBSpendUpdateWriter
from derouter.proxy.db.exception_handler import (
    PrismaDBExceptionHandler,
    call_with_db_reconnect_retry,
)
from derouter.proxy.db.log_db_metrics import log_db_metrics
from derouter.proxy.db.prisma_client import (
    PrismaWrapper,
    parse_iam_endpoint_from_url,
)
from derouter.proxy.db.routing_prisma_wrapper import RoutingPrismaWrapper
from derouter.proxy.db.spend_log_batching import (
    spend_log_queue_within_budget,
    spend_log_row_bytes,
    spend_log_write_batches,
)
from derouter.proxy.db.token_auth import (
    DatabaseTokenAuth,
    mint_database_token,
    resolve_database_token_auth,
)
from derouter.proxy.guardrails.guardrail_hooks.unified_guardrail.unified_guardrail import (
    UnifiedLLMGuardrails,
    resolve_endpoint_translation,
)
from derouter.proxy.hooks import PROXY_HOOKS, get_proxy_hook
from derouter.proxy.hooks.cache_control_check import _PROXY_CacheControlCheck
from derouter.proxy.hooks.max_budget_limiter import _PROXY_MaxBudgetLimiter
from derouter.proxy.hooks.parallel_request_limiter import (
    _PROXY_MaxParallelRequestsHandler,
)
from derouter.proxy.hooks.parallel_request_limiter_v3 import (
    _PROXY_MaxParallelRequestsHandler_v3,
)
from derouter.proxy.hooks.sensitive_data_routing import (
    _PROXY_SensitiveDataRoutingHandler,
)
from derouter.proxy.derouter_pre_call_utils import DeRouterProxyRequestSetup, add_guardrails_from_auth_metadata
from derouter.proxy.management_helpers.key_settings_audit import with_settings_updated_at
from derouter.proxy.policy_engine.pipeline_executor import PipelineExecutor
from derouter.repositories.budget_repository import BudgetRepository
...
from derouter.repositories.verification_token_repository import (
    VerificationTokenRepository,
)
from derouter.secret_managers.main import str_to_bool
from derouter.types.integrations.slack_alerting import DEFAULT_ALERT_TYPES
from derouter.types.mcp import (
    MCPDuringCallResponseObject,
    MCPPreCallRequestObject,
    MCPPreCallResponseObject,
)
from derouter.types.proxy.policy_engine.pipeline_types import PipelineExecutionResult
from derouter.types.utils import LLMResponseTypes, LoggedDeRouterParams
...
if TYPE_CHECKING:
    ...
    from prisma.actions import DeRouter_DeprecatedVerificationTokenActions
    from prisma.client import TransactionManager
    from prisma.models import DeRouter_DeprecatedVerificationToken
    ...
    from derouter.models.team import DeRouter_TeamTableCachedObj
    from derouter.proxy.db.autorouter_session_rollup import AutoRouterTurnTransaction
    from derouter.proxy.db.spend_log_tool_index import ToolUsageTransaction
    from derouter.repositories.prisma_protocols import TableActions
    from derouter.types.proxy.policy_engine.pipeline_types import GuardrailPipeline
...
class _EndUserSpendBatch(Protocol):
    @property
    def derouter_endusertable(self) -> _EndUserBatchTable: ...
...
def print_verbose(print_statement: object):
    """
    Prints the given `print_statement` to the console if `derouter.set_verbose` is True.
    ...
    if derouter.set_verbose:
        print(f"DeRouter Proxy: {_redact_string(str(print_statement))}")  # noqa: T201
...
class InternalUsageCache:
    ...
    async def async_get_cache(
        self,
        key: str,
        derouter_parent_otel_span: Span | None,
        ...
        **kwargs: object,
        ...
        return await self.dual_cache.async_get_cache(
            ...
            parent_otel_span=derouter_parent_otel_span,
            **kwargs,
        )
    ...
    async def async_set_cache(
        ...
        **kwargs: object,
        ...
        return await self.dual_cache.async_set_cache(
            ...
            value=value,
            local_only=local_only,
            derouter_parent_otel_span=derouter_parent_otel_span,
            **kwargs,
        )
...
def _accepts_derouter_call_info(cb: CustomLogger) -> bool:
    ...
    if key not in _CALLBACK_ACCEPTS_CALL_INFO:
        ...
        _CALLBACK_ACCEPTS_CALL_INFO[key] = "derouter_call_info" in sig.parameters
...
def _policy_state_metadata(data: Mapping[str, object]) -> Mapping[str, object]:
    ...
    The route decides the bucket (``derouter_metadata`` for ``/v1/messages``,
    ...
    ``derouter_metadata``. Pipeline slots are stripped from caller input before the
    policy engine runs, so whichever bucket carries them is the proxy's own write.
    """
    return next(
        (
            bucket
            for bucket in (data.get("metadata"), data.get("derouter_metadata"))
...
def _partition_post_call_callbacks() -> tuple[tuple[CustomGuardrail, ...], tuple[CustomLogger, ...]]:
    resolved: Final = tuple(
        derouter.derouter_core_utils.derouter_logging.get_custom_logger_compatible_class(
...
def _merge_pipeline_metadata_writes(
    ...
    """
    for bucket_key in ("metadata", "derouter_metadata"):
...
def _count_request_input_tokens(model: str, request_input: object, system_input: object) -> int:
    ...
    system_tokens: Final = derouter.token_counter(model=model, text=system_text) if system_text else 0
    if isinstance(request_input, str):
        return system_tokens + derouter.token_counter(model=model, text=request_input)
    ...
    if len(text_entries) == len(request_input):
        return system_tokens + derouter.token_counter(model=model, text="".join(text_entries))
    return system_tokens + derouter.token_counter(
...
def _failure_usage_to_lift(
    ...
    if not dispatched or model_call_details.get(DEROUTER_LOGGING_NO_UPSTREAM_LLM_CALL):
        return None
...
def _failure_fields_to_lift(request_data: Mapping[str, object]) -> Mapping[str, object]:
    """Failure-path callbacks run after ``derouter_logging_obj`` is popped from
    ...
    _logging_obj: Final = request_data.get("derouter_logging_obj")
...
class _CallbackCapabilities:
    """Cached per-hook capability flags derived from ``derouter.callbacks``.
    ...
    # Ordered the same as ``derouter.callbacks``; used to build the streaming
    ...
    # every string entry in ``derouter.callbacks``.
...
class ProxyLogging:
    ...
    def startup_event(
        ...
        self._init_derouter_callbacks(
            llm_router=llm_router
        )  # INITIALIZE DEROUTER CALLBACKS ON SERVER STARTUP <- do this to catch any logging errors on startup, not when calls are being made
        ...
        if updated_slack_alerting is True:
            ...
            if self.alerting is not None and ("slack" in self.alerting or "ms_teams" in self.alerting):
                ...
                if (
                    ...
                    derouter.logging_callback_manager.add_derouter_callback(self.slack_alerting_instance)
    ...
    def _add_proxy_hooks(self, llm_router: Router | None = None):
        """
        Add proxy hooks to derouter.callbacks
        """
        ...
        for hook in PROXY_HOOKS:
            ...
            derouter.logging_callback_manager.add_derouter_callback(proxy_hook_obj)
    ...
    def _init_derouter_callbacks(self, llm_router: Router | None = None):
        self._add_proxy_hooks(llm_router)
        derouter.logging_callback_manager.add_derouter_callback(self.service_logging_obj)
        ...
        # derouter.callbacks which caused double-counting of metrics.
        string_callbacks_to_replace: Final[dict[int, CustomLogger]] = {}

        for idx, callback in enumerate(derouter.callbacks):
            if isinstance(callback, str):
                initialized_callback = derouter.derouter_core_utils.derouter_logging._init_custom_logger_compatible_class(
        ...
        # Replace string entries in derouter.callbacks with initialized instances
        for idx, initialized_callback in string_callbacks_to_replace.items():
            derouter.callbacks[idx] = initialized_callback

        # Fan ``derouter.callbacks`` (the "all events" registry) out into the
        ...
        # notably pass-through endpoints — read ``derouter._async_success_callback``
        ...
        # Runs after load_config applied every derouter_settings key: logger __init__s read e.g. s3_callback_params
        success_callbacks: Final = tuple(cb for cb in derouter.success_callback if isinstance(cb, str))
        failure_callbacks: Final = tuple(cb for cb in derouter.failure_callback if isinstance(cb, str))
    ...
    async def update_request_status(self, derouter_call_id: str, status: Literal["success", "fail"]):
        ...
        await self.internal_usage_cache.async_set_cache(
            key=f"request_status:{derouter_call_id}",
            value=status,
            local_only=True,
    ...
    def _convert_mcp_to_llm_format(self, request_obj, kwargs: dict) -> dict:
        ...
        """
        from derouter.types.llms.openai import ChatCompletionUserMessage
    ...
    def _convert_llm_result_to_mcp_response(self, llm_result, request_obj) -> MCPPreCallResponseObject | None:
        ...
        """
        from derouter.types.mcp import MCPPreCallResponseObject
    ...
    def _create_mcp_request_object_from_kwargs(self, kwargs: dict) -> "MCPPreCallRequestObject":
        ...
        from derouter.types.mcp import MCPPreCallRequestObject
    ...
    async def _process_prompt_template(
        ...
        derouter_prompt_id: str | None = None
        if prompt_spec is not None:
            derouter_prompt_id = prompt_spec.derouter_params.prompt_id
    ...
    def _process_guardrail_metadata(self, data: dict) -> None:
        ...
        metadata_derouter: Final = data.get("derouter_metadata") or {}
        ...
        elif isinstance(metadata_derouter, dict) and "guardrails" in metadata_derouter:
            guardrails_in_metadata = metadata_derouter.get("guardrails", [])
        ...
        if guardrails_in_metadata and isinstance(guardrails_in_metadata, list):
            ...
            elif isinstance(metadata_derouter, dict) and "applied_guardrails" in metadata_derouter:
                applied_guardrails = metadata_derouter.get("applied_guardrails", [])
    ...
    async def pre_call_hook(
        ...
        derouter_logging_obj: Final = cast(Optional["DeRouterLoggingObj"], data.get("derouter_logging_obj", None))
        ...
        if (
            not guardrails_only
            and derouter_logging_obj is not None
            and prompt_id is not None
            and (call_type == "completion" or call_type == "acompletion" or call_type == "aresponses")
        ):
            from derouter.proxy.prompts.prompt_registry import parse_prompt_version
        ...
        # guarantee must hold even under derouter.safe_memory_mode, which
        ...
        try:
            ...
            # ``derouter.callbacks`` overrides ``async_pre_call_hook`` and no
    ...
    def _emit_guardrail_metrics(
        ...
        for prom_callback in derouter.callbacks:
    ...
    async def _run_guardrail_with_metrics(callback: object, coro: Awaitable[_T], hook_type: str) -> _T:
        ...
        `derouter_guardrail_latency_seconds` metric under `hook_type`, and
    ...
    # derouter.callbacks (length + each item's id) so we recompute when the
    ...
    def _callback_capabilities() -> "_CallbackCapabilities":
        """
        Inspect ``derouter.callbacks`` once and answer the per-hook capability
        questions used to short-circuit no-op work on the chat-completions hot
        path. Per-request callers iterated ``derouter.callbacks`` and called
        ...
        """
        callbacks: Final = derouter.callbacks
        ...
        for callback in callbacks:
            if isinstance(callback, str):
                resolved = derouter.derouter_core_utils.derouter_logging.get_custom_logger_compatible_class(
                    cast(_custom_logger_compatible_callbacks_literal, callback)
                )
            else:
                resolved = callback
    ...
    def _stream_requires_guardrail_translation(user_api_key_dict: UserAPIKeyAuth) -> bool:
        from derouter.derouter_core_utils.api_route_to_call_types import (
    ...
    async def during_call_hook(
        ...
        # ``derouter.callbacks`` plus an ``asyncio.gather([])`` round trip on
    ...
    async def alerting_handler(
        ...
        """
        Alerting based on thresholds: - https://github.com/Delify-Solutions/derouter/issues/1298
        ...
        for client in self.alerting:
            if client == "sentry":
                if derouter.utils.sentry_sdk_instance is not None:
                    derouter.utils.sentry_sdk_instance.capture_message(formatted_message)
                else:
    ...
    async def failure_handler(self, original_exception, duration: float, call_type: str, traceback_str=""):
        ...
        if derouter.utils.capture_exception:
            derouter.utils.capture_exception(error=original_exception)
    ...
    async def post_call_failure_hook(
        ...
        ### ALERTING ###
        await self.update_request_status(derouter_call_id=request_data.get("derouter_call_id", ""), status="fail")
        if AlertType.llm_exceptions in self.alert_types and not isinstance(
            ...
            Related issue - https://github.com/Delify-Solutions/derouter/issues/3395
            """
            derouter_debug_info: Final[str | None] = getattr(original_exception, "derouter_debug_info", None)
        ...
        request_data.pop("derouter_logging_obj", None)
        ...
        for callback in derouter.callbacks:
            try:
                ...
                if isinstance(callback, str):
                    _callback = derouter.derouter_core_utils.derouter_logging.get_custom_logger_compatible_class(
                        cast(_custom_logger_compatible_callbacks_literal, callback)
                    )
                else:
                    _callback = callback
    ...
    async def _handle_logging_proxy_only_error(
        ...
        """
        Handle logging for proxy only errors by calling `derouter_logging_obj.async_failure_handler`

        Is triggered when self._is_proxy_only_error() returns True
        """
        derouter_logging_obj: Logging | None = request_data.get("derouter_logging_obj", None)
        if derouter_logging_obj is None:
            from derouter._uuid import uuid

            request_data["derouter_call_id"] = str(uuid.uuid4())
            user_api_key_logged_metadata: Final = DeRouterProxyRequestSetup.get_sanitized_user_information_from_key(
                user_api_key_dict=user_api_key_dict
            )

            derouter_logging_obj, data = derouter.utils.function_setup(
        ...
        if derouter_logging_obj is not None:
            ...
            _derouter_params: Final = {}

            derouter_param_keys: Final = LoggedDeRouterParams.__annotations__.keys()
            for k, v in request_data.items():
                if k in derouter_param_keys:
                    _derouter_params[k] = v
                elif k not in ("model", "user", "derouter_logging_obj"):
            ...
            if "messages" in request_data and isinstance(request_data["messages"], list):
                ...
                derouter_logging_obj.model_call_details["messages"] = input
                if derouter_logging_obj.call_type != CallTypes.pass_through.value:
                ...
                derouter_logging_obj.model_call_details["prompt"] = input
                if derouter_logging_obj.call_type != CallTypes.pass_through.value:
                ...
                derouter_logging_obj.model_call_details["input"] = input
                if derouter_logging_obj.call_type != CallTypes.pass_through.value:
                    normalized_call_type = CallTypes.aembedding.value
            if normalized_call_type is not None:
                derouter_logging_obj.call_type = normalized_call_type
                derouter_logging_obj.model_call_details["call_type"] = normalized_call_type
            ...
            derouter_logging_obj.model_call_details[DEROUTER_LOGGING_NO_UPSTREAM_LLM_CALL] = True
    ...
    async def _dispatch_proxy_only_failure_handlers(
        ...
        derouter.log_client_error_tracebacks is set."""
        include_traceback: Final = derouter.log_client_error_tracebacks or not is_expected_client_error(
            original_exception
        )
        traceback_str: Final = traceback.format_exc() if include_traceback else ""
        await derouter_logging_obj.async_failure_handler(
            exception=original_exception,
            traceback_exception=traceback_str,
        )

        threading.Thread(
            target=derouter_logging_obj.failure_handler,
            args=(
                original_exception,
                traceback_str,
            ),
    ...
    async def post_call_success_hook(
        ...
        _, pipeline_response = await self._maybe_execute_pipelines(
            data=data,
            user_api_key_dict=user_api_key_dict,
            call_type=getattr(data.get("derouter_logging_obj"), "call_type", None) or "acompletion",
        ...
        for callback in caps.resolved_callbacks:
            ...
            response = await self._run_guardrail_with_metrics(
                ...
                handler_cls().process_output_response(
                    ...
                    derouter_logging_obj=request_data.get("derouter_logging_obj"),
                    user_api_key_dict=user_api_key_dict,
                    request_data=request_data,
                ),
                "post_mcp_call",
    ...
    async def post_call_response_headers_hook(
        ...
        try:
            # Build derouter_call_info — normalized routing metadata for callbacks
            derouter_call_info: Final = self._build_derouter_call_info(data=data, response=response)
            ...
            for callback in derouter.callbacks:
                ...
                if _callback is not None and isinstance(_callback, CustomLogger):
                    if _accepts_derouter_call_info(_callback):
                        result = await _callback.async_post_call_response_headers_hook(
                            data=data,
                            user_api_key_dict=user_api_key_dict,
                            response=response,
                            request_headers=request_headers,
                            derouter_call_info=derouter_call_info,
                        )
                    else:
                        # Backwards compat: callback doesn't accept derouter_call_info
                        result = await _callback.async_post_call_response_headers_hook(
                            data=data,
                            user_api_key_dict=user_api_key_dict,
                            response=response,
    ...
    @staticmethod
    def _build_derouter_call_info(data: dict, response: object) -> dict[str, object]:
        """
        Build a normalized dict of routing metadata from response._hidden_params
        and data, abstracting away the metadata vs derouter_metadata split.
        """
        hidden_params: Final = getattr(response, "_hidden_params", {}) or {}

        # model_info: check both metadata keys (chat uses "metadata", responses uses "derouter_metadata")
        model_info: Final = (
            (data.get("metadata") or {}).get("model_info")
            or (data.get("derouter_metadata") or {}).get("model_info")
            or {}
        )
    ...
    async def async_post_call_streaming_hook(
        ...
        if isinstance(response, (ModelResponse, ModelResponseStream)):
            response_str = derouter.get_response_string(response_obj=response)
        elif isinstance(response, dict) and self.is_a2a_streaming_response(response):
            from derouter.llms.a2a.common_utils import extract_text_from_a2a_response
    ...
    def _fire_deferred_stream_logging(request_data: dict) -> None:
        ...
        """
        logging_obj: Final = request_data.get("derouter_logging_obj")
...
async def _lookup_deprecated_key(
    ...
    try:
        deprecated_keys_table: Final[
            DeRouter_DeprecatedVerificationTokenActions[DeRouter_DeprecatedVerificationToken]
        ] = db.derouter_deprecatedverificationtoken
...
# DualCache for DeRouter_Config param_name reads.
# Redis layer is attached in proxy_server._init_cache.
DEROUTER_CONFIG_CACHE_TTL_SECONDS: Final[int] = int(os.environ.get("DEROUTER_CONFIG_PARAM_CACHE_TTL_SECONDS", "60"))
_CONFIG_CACHE_MISS: Final[str] = "__derouter_config_param_miss__"

derouter_config_cache: Final[DualCache] = DualCache(
    default_in_memory_ttl=DEROUTER_CONFIG_CACHE_TTL_SECONDS,
    default_redis_ttl=DEROUTER_CONFIG_CACHE_TTL_SECONDS,
)


class _ConfigRow:
    """Mimics the Prisma derouter_config row shape for cached entries."""
...
def _config_cache_key(param_name: str) -> str:
    return f"derouter_config:param:{param_name}"
...
async def get_config_param(prisma_client: "PrismaClient", param_name: str) -> Any | None:
    """Cached read of a DeRouter_Config row; returns row, _ConfigRow shim, or None."""
    cache_key: Final = _config_cache_key(param_name)
    cached: Final = await derouter_config_cache.async_get_cache(cache_key)
    if cached is not None:
    ...
    await derouter_config_cache.async_set_cache(cache_key, cache_value, ttl=DEROUTER_CONFIG_CACHE_TTL_SECONDS)
    return row


async def evict_config_param(param_name: str) -> None:
    await derouter_config_cache.async_delete_cache(_config_cache_key(param_name))


async def invalidate_config_param(param_name: str) -> None:
    """Evict from both cache layers; call after every DeRouter_Config write."""
...
async def prefetch_config_params(prisma_client: "PrismaClient | None", param_names: list[str]) -> None:
    """Batch-load DeRouter_Config rows into the cache with one find_many."""
    if not param_names:
        return
    try:
        config_table: Final = cast(  # cast-ok: ConfigRepository.table is prisma's derouter_config actions object
            "TableActions[prisma_models.DeRouter_Config]", ConfigRepository(prisma_client).table
        )
    ...
    for name in param_names:
        ...
        await derouter_config_cache.async_set_cache(
            _config_cache_key(name), cache_value, ttl=DEROUTER_CONFIG_CACHE_TTL_SECONDS
        )
...
class PrismaClient:
    ...
    async def check_view_exists(self):
        """
        Checks if the DeRouter_VerificationTokenView and MonthlyGlobalSpend exists in the user's db.

        DeRouter_VerificationTokenView: This view is used for getting the token + team data in user_api_key_auth
        ...
        try:
            ...
            required_view: Final = "DeRouter_VerificationTokenView"
            ...
            if ret[0]["view_count"] == expected_total_views:
                ...
                if ret[0]["view_names"] and required_view not in ret[0]["view_names"]:
                    ...
                    await create_view_tolerating_race(
                        ...
                        "DeRouter_VerificationTokenView",
                        """
                            CREATE VIEW "DeRouter_VerificationTokenView" AS
                            SELECT
                            v.*,
                            ...
                            FROM "DeRouter_VerificationToken" v
                            LEFT JOIN "DeRouter_TeamTable" t ON v.team_id = t.team_id;
                        """,
                    )
                else:
                    ...
                    if should_create_views:
                        ...
                        verbose_proxy_logger.warning(
                            "\n\n\x1b[93mNot all views exist in db, needed for UI 'Usage' tab. Missing=%s.\nRun 'create_views.py' from https://github.com/Delify-Solutions/derouter/tree/main/db_scripts to create missing views.\x1b[0m\n",
                            missing_views,
                        )
    ...
    async def get_generic_data(
        ...
        async def _do_query():
            if table_name == "users":
                ...
                config_table: Final = cast(  # cast-ok: ConfigRepository.table is prisma's derouter_config actions object
                    "TableActions[prisma_models.DeRouter_Config]", ConfigRepository(self).table
                )
        ...
        try:
            ...
            error_msg = f"DeRouter Prisma Client Exception get_generic_data: {e}"
    ...
    async def get_data(
        ...
        try:
            ...
            if (token is not None and table_name is None) or (table_name is not None and table_name == "key"):
                ...
                if query_type == "find_unique":
                    ...
                    if expires is not None:
                        ...
                        FROM
                            "DeRouter_UserTable" u
                        LEFT JOIN "DeRouter_VerificationToken" v ON u.user_id = v.user_id
                        GROUP BY
                            u.user_id
                ...
                if query_type == "find_unique":
                    response = await TeamRepository(self).table.find_unique(
                        where={"team_id": team_id},
                        include={"derouter_model_table": True},
                    )
                elif query_type == "find_all" and reset_at is not None:
                ...
                if query_type == "find_unique":
                    ...
                    sql_query = """
                        SELECT 
                            ...
                            b.max_budget AS derouter_budget_table_max_budget,
                            b.tpm_limit AS derouter_budget_table_tpm_limit,
                            b.rpm_limit AS derouter_budget_table_rpm_limit,
                            b.model_max_budget as derouter_budget_table_model_max_budget,
                            b.soft_budget as derouter_budget_table_soft_budget,
                        ...
                        FROM "DeRouter_VerificationToken" AS v
                        LEFT JOIN "DeRouter_TeamTable" AS t ON v.team_id = t.team_id
                        LEFT JOIN "DeRouter_TeamMembership" AS tm ON v.team_id = tm.team_id AND tm.user_id = v.user_id
                        LEFT JOIN "DeRouter_BudgetTable" AS b_tm ON tm.budget_id = b_tm.budget_id
                        LEFT JOIN "DeRouter_ModelTable" m ON t.model_id = m.id
                        LEFT JOIN "DeRouter_BudgetTable" AS b ON v.budget_id = b.budget_id
                        LEFT JOIN "DeRouter_ProjectTable" AS p ON v.project_id = p.project_id
                        LEFT JOIN "DeRouter_OrganizationTable" AS o ON v.organization_id = o.organization_id
                        LEFT JOIN "DeRouter_BudgetTable" AS b2 ON o.budget_id = b2.budget_id
                        WHERE v.token = $1
                    """
                    ...
                    if response is None and hashed_token is not None and check_deprecated:
                        ...
                        if active_token_id:
                            ...
                            # DeRouter_VerificationTokenView; the dict
                    ...
                    if response is not None:
                        ...
                        response = DeRouter_VerificationTokenView(**response, last_refreshed_at=time.time())
                        # for prisma we need to cast the expires time to str
                        if response.expires is not None and isinstance(response.expires, datetime):
                            response.expires = response.expires.isoformat()
                    return response
            ...
            prisma_query_info: Final = (
                f"DeRouter Prisma Client Exception: Error with `get_data`. Args passed in: {args_passed_in}"
            )
    ...
    async def insert_data(
        ...
        try:
            ...
            if table_name == "key":
                ...
                print_verbose("PrismaClient: Before upsert into derouter_verificationtoken")
                ...
                try:
                    ...
                    if (
                        "Foreign key constraint failed on the field: `DeRouter_UserTable_organization_id_fkey (index)`"
                        in str(e)
                    ):
                        raise HTTPException(
                            status_code=400,
                            detail={
                                "error": f"Foreign Key Constraint failed. Organization ID={db_data['organization_id']} does not exist in DeRouter_OrganizationTable. Create via `/organization/new`."
                            },
                        )
                    raise e
            ...
            error_msg: Final = f"DeRouter Prisma Client Exception in insert_data: {e}"
            print_verbose(error_msg)
            error_traceback: Final = error_msg + "\n" + traceback.format_exc()
            end_time: Final = time.time()
            _duration: Final = end_time - start_time
        ...
        try:
            ...
            if token is not None:
                ...
                for idx, t in enumerate(data_list):
                    ...
                    batcher.derouter_verificationtoken.update(
                        where={"token": t.token},
                        data={**data_json},
                    )
                ...
                for idx, user in enumerate(data_list):
                    ...
                    batcher.derouter_usertable.upsert(
                ...
                for enduser in data_list:
                    ...
                    batcher.derouter_endusertable.upsert(
                ...
                for budget in data_list:
                    ...
                    batcher.derouter_budgettable.upsert(
                ...
                for idx, team in enumerate(data_list):
                    ...
                    batcher.derouter_teamtable.upsert(
            ...
            error_msg: Final = f"DeRouter Prisma Client Exception - update_data: {e}"
            print_verbose(error_msg)
            error_traceback: Final = error_msg + "\n" + traceback.format_exc()
            end_time: Final = time.time()
            _duration: Final = end_time - start_time
    ...
    async def delete_data(
        ...
        try:
            ...
            error_msg: Final = f"DeRouter Prisma Client Exception - delete_data: {e}"
            print_verbose(error_msg)
            error_traceback: Final = error_msg + "\n" + traceback.format_exc()
            end_time: Final = time.time()
            _duration: Final = end_time - start_time
    ...
    async def connect(self):
        ...
        try:
            ...
            error_msg: Final = f"DeRouter Prisma Client Exception connect(): {e}"
    ...
    async def disconnect(self):
        ...
        try:
            ...
            error_msg: Final = f"DeRouter Prisma Client Exception disconnect(): {e}"
            print_verbose(error_msg)
            error_traceback: Final = error_msg + "\n" + traceback.format_exc()
            end_time: Final = time.time()
            _duration: Final = end_time - start_time
    ...
    async def health_check(self):
        ...
        try:
            ...
            error_msg: Final = f"DeRouter Prisma Client Exception health_check(): {e}"
            verbose_proxy_logger.warning(error_msg)
            error_traceback: Final = error_msg + "\n" + traceback.format_exc()
            end_time: Final = time.time()
            _duration: Final = end_time - start_time
    ...
    async def _get_spend_logs_row_count(self) -> int:
        """
        Get the row count from DeRouter_SpendLogs table using PostgreSQL system statistics.
        """
        ...
        async def _fetch_row_count() -> int:
            ...
            FROM pg_class
            WHERE oid = '"DeRouter_SpendLogs"'::regclass;
            """
        ...
        try:
            ...
            verbose_proxy_logger.error("Error getting DeRouter_SpendLogs row count: %s", e)
            return 0
    ...
    async def _set_spend_logs_row_count_in_proxy_state(self) -> None:
        """
        Set the `DeRouter_SpendLogs`row count in proxy state.

        This is used later to determine if we should run expensive UI Usage queries.
        """
        from derouter.proxy.proxy_server import proxy_state
    ...
    ) -> "Sequence[prisma_models.DeRouter_HealthCheckTable]":
        """
    ...
    async def get_all_latest_health_checks(self) -> "Sequence[prisma_models.DeRouter_HealthCheckTable]":
        """
...
class ProxyUpdateSpend:
                ...
                async with prisma_client.db.tx(timeout=timedelta(seconds=60)) as transaction:
                    ...
                    async with transaction.batch_() as batcher:
                        ...
                        for end_user_id, response_cost in sorted(end_user_list_transactions.items()):
                            if derouter.max_end_user_budget is not None:
                                pass
                            batcher.derouter_endusertable.upsert(
    ...
    def disable_spend_updates() -> bool:
        ...
        """
        from derouter.proxy.proxy_server import general_settings
...
async def _total_queued_spend_transactions(prisma_client: PrismaClient) -> int:
    ...
    from derouter.proxy.db.shadow_eval_funnel import pending_shadow_eval_funnel_events
...
async def update_spend_logs_job(
    ...
    try:
        from derouter.proxy.guardrails.usage_tracking import (
    ...
    # DeRouter_DailyToolSpend rollup. Never retried; a dropped batch is permanently
    ...
    try:
        from derouter.proxy.db.spend_log_tool_index import flush_tool_usage_transactions
    ...
    try:
        from derouter.proxy.db.autorouter_session_rollup import flush_autorouter_turn_transactions
    ...
    try:
        from derouter.proxy.db.shadow_eval_funnel import flush_shadow_eval_funnel
...
def _raise_failed_update_spend_exception(e: Exception, start_time: float, proxy_logging_obj: ProxyLogging):
    ...
    error_msg: Final = f"[Non-Blocking]DeRouter Prisma Client Exception - update spend logs: {e}"
    error_traceback: Final = error_msg + "\n" + traceback.format_exc()
    end_time: Final = time.time()
    _duration: Final = end_time - start_time
    asyncio.create_task(
...
def _check_and_merge_model_level_guardrails(
    ...
    Args:
        ...
        trust_client_model_info: If False, ignore metadata.model_info.id and
            ...
            pre_call path because add_derouter_data_to_request preserves
    ...
    metadata: Final = data.get("metadata") or {}
    derouter_metadata: Final = data.get("derouter_metadata") or {}
    ...
    # Medium on #29654).
    team_id: Final = metadata.get("user_api_key_team_id") or derouter_metadata.get("user_api_key_team_id")
    ...
    if model_id is not None:
        ...
        if deployment is None:
            return data
        deployment_guardrails: Final = deployment.derouter_params.get("guardrails")
        ...
        # don't know which deployment's derouter_params.guardrails will apply.
        ...
        for dep in deployments:
            derouter_params_dep = dep.get("derouter_params") or {}
            guardrails = derouter_params_dep.get("guardrails")
...
def _premium_user_check(feature: str | None = None):
    ...
    if feature:
        detail_msg = f"This feature is only available for DeRouter Enterprise users: {feature}. {CommonProxyErrors.not_premium_user.value}"
    else:
        detail_msg = (
            f"This feature is only available for DeRouter Enterprise users. {CommonProxyErrors.not_premium_user.value}"
        )
...
def is_known_vector_store_index(index_name: str) -> bool:
    ...
    if derouter.vector_store_index_registry is None:
        return False
    return index_name in derouter.vector_store_index_registry.get_vector_store_indexes()
...
def get_custom_url(request_base_url: str, route: str | None = None) -> str:
    ...
    from derouter.proxy.middleware.per_request_root_path_middleware import (  # noqa: PLC0415  # lazy: middleware imports utils
...
async def _get_validated_team_object(
    ...
    from derouter.proxy.management_endpoints.team_endpoints import validate_membership
...
) -> Optional["DeRouter_TeamTableCachedObj"]:
    from derouter.proxy.auth.auth_checks import get_team_object
...
async def _get_access_group_models(
    user_api_key_dict: "UserAPIKeyAuth",
    team_object: Optional["DeRouter_TeamTableCachedObj"],
    prisma_client: Optional["PrismaClient"],
    user_api_key_cache: Optional["UserApiKeyCache"],
    proxy_logging_obj: Optional["ProxyLogging"],
) -> tuple[str, ...]:
...
async def get_available_models_for_user(
    ...
    Args:
        ...
        llm_router: DeRouter router instance
...
def create_model_info_response(
    ...
    get_model_info: Callable[[str], ModelInfo] = derouter.get_model_info,
) -> ModelInfoResponse:
    """
    ...
    """
    from derouter.proxy.auth.model_checks import get_all_fallbacks
