...
Exposes Gemini's /v1beta/agents surface through the DeRouter proxy so that
...
def _enforce_caller_supplied_provider_key(
    ...
    *not* routed through ``model_list`` — the only credential source is either
    the per-request ``derouter_params_template`` or the env var fallback. Without
    ...
    raise HTTPException(
        ...
        detail=(
            ...
            "Gemini api_key (via 'derouter_params_template'). Falling back to "
...
def _merge_query_params_into_data(data: dict, request: Request) -> dict:
    ...
    JSON-encoded ``derouter_params_template`` query parameter and merge its
    ...
    This mirrors the ``derouter_params_template`` handling in
    ...
    .. code-block:: bash
        ...
        curl "http://localhost:4000/v1beta/agents?derouter_params_template=%7B%22api_key%22%3A%22AIza...%22%7D" \\
    ...
    headers. Use the ``derouter_params_template`` JSON body field on POST
    ...
    raw_template: Final = query_params.get("derouter_params_template")
    if raw_template:
        try:
...
async def create_gemini_agent(
    ...
    # Merge derouter_params_template (e.g. custom_llm_provider, api_key) into the request
    derouter_params_template: Final = data.pop("derouter_params_template", None) or {}
    if isinstance(derouter_params_template, dict):
        for key, value in derouter_params_template.items():
            if key not in data:
                data[key] = value
...
async def list_gemini_agents(
    ...
    ``derouter_params_template`` query parameter. Flat query parameters
...
async def get_gemini_agent(
    ...
    ```bash
    curl "http://localhost:4000/v1beta/agents/my-custom-slides-agent?derouter_params_template=%7B%22api_key%22%3A%22AIza...%22%7D" \\
        -H "Authorization: Bearer sk-..."
    ```
    """
    srv: Final = _proxy_server_imports()
...
async def delete_gemini_agent(
    ...
    ```bash
    curl -X DELETE "http://localhost:4000/v1beta/agents/my-custom-slides-agent?derouter_params_template=%7B%22api_key%22%3A%22AIza...%22%7D" \\
        -H "Authorization: Bearer sk-..."
    ```
    """
    srv: Final = _proxy_server_imports()
...
async def list_gemini_agent_versions(
    ...
    ```bash
    curl "http://localhost:4000/v1beta/agents/my-custom-slides-agent/versions?derouter_params_template=%7B%22api_key%22%3A%22AIza...%22%7D" \\
        -H "Authorization: Bearer sk-..."
    ```
    """
    srv: Final = _proxy_server_imports()
