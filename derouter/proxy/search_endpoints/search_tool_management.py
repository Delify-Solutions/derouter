...
from derouter.proxy.search_endpoints.search_tool_registry import SearchToolRegistry
from derouter.types.search import (
...
TeamObjectLookup: TypeAlias = Callable[[str, UserAPIKeyAuth], Awaitable[DeRouter_TeamTable]]
...
async def _team_object_from_db(team_id: str, user_api_key_dict: UserAPIKeyAuth) -> DeRouter_TeamTable:
...
def _allowlist_team_id(user_api_key_dict: UserAPIKeyAuth) -> str | None:
    ...
    never has a row in DeRouter_TeamTable (`/team/new` rejects it as a real team id), so looking it
...
async def _filter_visible_search_tools(
    ...
    from derouter.proxy.auth.auth_checks import can_user_view_search_tool

    allowlist_team_id: Final = _allowlist_team_id(user_api_key_dict)
    team_object: Final[DeRouter_TeamTable | None] = (
...
async def list_search_tools(
    ...
    try:
        ...
        for config_search_tool in config_search_tools:
            ...
            if tool_name:
                derouter_params_dict = dict(config_search_tool.get("derouter_params", {}))
                ...
                search_tool_configs.append(
                    SearchToolInfoResponse(
                        search_tool_id=None,
                        search_tool_name=tool_name,
                        derouter_params=masked_derouter_params_dict,
        ...
        for db_search_tool in search_tools_from_db:
            derouter_params_dict = dict(db_search_tool.get("derouter_params", {}))
            masked_derouter_params_dict = _get_masked_values(
                derouter_params_dict,
                unmasked_length=4,
                number_of_asterisks=4,
...
async def get_search_tool_info(search_tool_id: str):
    ...
    try:
        ...
        # Mask sensitive data
        derouter_params_dict: Final = dict(result.get("derouter_params", {}))
...
async def test_search_tool_connection(request: TestSearchToolConnectionRequest):
    ...
    """
    try:
        from derouter.search import asearch

        # Extract params from request
        derouter_params: Final = request.derouter_params
        search_provider: Final = derouter_params.get("search_provider")
        api_key: Final = derouter_params.get("api_key")
        api_base: Final = derouter_params.get("api_base")

        if not search_provider:
            raise HTTPException(status_code=400, detail="search_provider is required in derouter_params")
