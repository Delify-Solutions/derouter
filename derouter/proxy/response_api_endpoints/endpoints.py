...
async def responses_api(
    ...
    from derouter.proxy.response_polling.polling_handler import (
    ...
    if should_use_polling:
        from derouter.proxy.response_polling.background_streaming import (
        ...
        if data.get("background") and isinstance(response, ResponsesAPIResponse):
            if response.status in ["queued", "in_progress"]:
                ...
                managed_files_obj: Final = cast(
                    _PROXY_DeRouterManagedFiles | None,
...
async def cursor_model_list(
    ...
    """
    from derouter.proxy.proxy_server import model_list
...
async def cursor_chat_completions(
    ...
    ```
    """
    from derouter.completion_extras.derouter_responses_transformation.handler import (
    ...
    def cursor_data_generator(response, user_api_key_dict, request_data, request=None):
        ...
        # HiddenParamsAsyncIteratorWrapper around DeRouterCompletionStreamingIterator),
    ...
    try:
        ...
        if isinstance(response, ResponsesAPIResponse):
            logging_obj: Final = processor.data.get("derouter_logging_obj")
...
async def get_response(
    ...
    Supports both:
    - Polling IDs (derouter_poll_*): Returns cumulative cached content from background responses
    ...
    curl -X GET http://localhost:4000/v1/responses/derouter_poll_abc123 \
    -H "Authorization: Bearer sk-1234"
    ...
    from derouter.proxy.response_polling.polling_handler import ResponsePollingHandler
...
async def delete_response(
    ...
    Supports both:
    - Polling IDs (derouter_poll_*): Deletes from Redis cache
    - Provider response IDs: Passes through to provider API
...
def _responses_input_as_token_count_messages(
    ...
    transformed: Final = DeRouterCompletionResponsesConfig.transform_responses_api_input_to_messages(
        input=input_value,
...
async def cancel_response(
    ...
    Supports both:
    - Polling IDs (derouter_poll_*): Cancels background response and updates status in Redis
    - Provider response IDs: Passes through to provider API
    ...
    curl -X POST http://localhost:4000/v1/responses/derouter_poll_abc123/cancel \
    -H "Authorization: Bearer sk-1234"
