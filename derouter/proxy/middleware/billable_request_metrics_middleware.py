...
- ``GatewayRequestSink`` receives every classified request with its status and
  ...
  Not license-gated (see derouter.proxy.db.gateway_request_tracking). It is not
...
- ``BillingRecorder`` receives 2xx requests only and exports them for
  enterprise metering (see derouter.proxy.enterprise_billing.billing_metrics).
...
_MODEL_ID_HEADER: Final = b"x-derouter-model-id"
...
_PASSTHROUGH_PREFIXES: Final[tuple[str, ...]] = tuple(
    prefix
    for prefix in DeRouterRoutes.mapped_pass_through_routes.value
...
class BillableRequestMetricsMiddleware:
    ...
    status and the x-derouter-model-id header off the `http.response.start`
