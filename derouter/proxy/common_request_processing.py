...
import derouter
from derouter._logging import redact_internal_details_from_client_message, verbose_proxy_logger
from derouter._uuid import uuid
from derouter.constants import (
    DD_TRACER_STREAMING_CHUNK_YIELD_RESOURCE,
    DEFAULT_MAX_RECURSE_DEPTH,
    DEROUTER_DETAILED_TIMING,
    DEROUTER_HTTP_STATUS_CLIENT_DISCONNECTED,
...
from derouter.derouter_core_utils.core_helpers import get_or_create_metadata_bucket, is_expected_client_error
from derouter.derouter_core_utils.dd_tracing import NullTracer, tracer
from derouter.derouter_core_utils.get_supported_openai_params import (
    get_supported_openai_params,
)
from derouter.derouter_core_utils.internal_call_metadata import is_unbilled_non_inference_call_from_params
from derouter.derouter_core_utils.derouter_logging import Logging as DeRouterLoggingObj
from derouter.derouter_core_utils.llm_cost_calc.guardrail_cost import guardrail_information_cost
...
from derouter.derouter_core_utils.streaming_handler import (
...
from derouter.proxy.auth.auth_utils import check_response_size_is_safe
from derouter.proxy.common_utils.callback_utils import (
    get_logging_caching_headers,
    get_remaining_tokens_and_requests_from_request_data,
)
from derouter.proxy.common_utils.openai_error_payload import (
    attribute_of,
    error_status_code,
    openai_error_param,
    openai_error_type,
)
from derouter.proxy.common_utils.sse_keepalive import (
...
from derouter.proxy.guardrails.auto_router_compression import arm_pre_call as _arm_auto_router_compression
from derouter.proxy.route_llm_request import route_request
from derouter.proxy.utils import ProxyLogging, _check_and_merge_model_level_guardrails
from derouter.router import Router
from derouter.router_utils.add_retry_fallback_headers import get_hidden_params_dict
from derouter.router_utils.common_utils import resolve_model_group_alias
...
from derouter.llms.anthropic.chat.transformation import AnthropicConfig
...
if TYPE_CHECKING:
    ...
    ProxyConfig = Any
from derouter.proxy.anthropic_endpoints.streaming_model_restamp import (
...
_CLIENT_DISCONNECTED_ERROR_INFORMATION: Final[StandardLoggingPayloadErrorInformation] = {
    "error_code": str(DEROUTER_HTTP_STATUS_CLIENT_DISCONNECTED),
...
def _should_return_raw_model_name(request_data: dict[str, object]) -> bool:
    return any(
        ...
        for metadata in (request_data.get("metadata"), request_data.get("derouter_metadata"))
    )
...
async def _record_streaming_client_disconnect_if_needed(
    ...
    if logging_obj is not None:
        derouter_params: Final = logging_obj.model_call_details.setdefault("derouter_params", {})
        _lp_metadata = derouter_params.get("metadata")
        if _lp_metadata is None:
            _lp_metadata = {}
            derouter_params["metadata"] = _lp_metadata
    ...
    _rd_derouter_params = request_data.get("derouter_params")
    if _rd_derouter_params is None:
        _rd_derouter_params = {}
        request_data["derouter_params"] = _rd_derouter_params
    _rd_lp_metadata = _rd_derouter_params.get("metadata")
    if _rd_lp_metadata is None:
        _rd_lp_metadata = {}
        _rd_derouter_params["metadata"] = _rd_lp_metadata
    _apply_client_disconnect_metadata(_rd_lp_metadata)

    verbose_proxy_logger.debug(
        "Recorded streaming client disconnect with error_code=499 for derouter_call_id=%s",
...
def _assembled_model_is_the_name_the_client_asked_for(
    ...
    """
    client_requested_model: Final = request_data.get("_derouter_client_requested_model")
    stamped_model: Final = (
...
async def _bill_partial_streamed_spend_on_disconnect(request_data: dict, response: object) -> bool:
    ...
    """
    if derouter.disable_streaming_logging is True:
        return False
    logging_obj: Final = request_data.get("derouter_logging_obj")
    if not isinstance(logging_obj, DeRouterLoggingObj):
        return False
    ...
    if not isinstance(chunks, list) or not chunks:
        return False
    verbose_proxy_logger.debug(
        "Billing partial streamed spend for %s chunks after client disconnect, derouter_call_id=%s",
        len(chunks),
        request_data.get("derouter_call_id"),
    )
    messages: Final[object] = getattr(response, "messages", None)
    try:
        partial_response: Final = derouter.stream_chunk_builder(
            chunks=chunks,
...
def _derouter_model_supports_stream_options(derouter_model: str) -> bool:
    try:
        supported_params: Final = get_supported_openai_params(model=derouter_model)
...
def _model_deployments_support_stream_options(
    ...
    deployment_models: Final = (
        llm_router.resolved_derouter_models(model, team_id=team_id) if llm_router is not None else ()
    )
    candidate_models: Final = deployment_models if deployment_models else (model,)
    return all(_derouter_model_supports_stream_options(m) for m in candidate_models)
...
def _stream_usage_tracking_updates(
    ...
    scrub: Final = {"_derouter_strip_stream_usage": False} if "_derouter_strip_stream_usage" in data else {}
...
async def create_response(
    ...
    try:
        ...
        return JSONResponse(
            status_code=DEROUTER_HTTP_STATUS_CLIENT_DISCONNECTED,
            content={
                "error": {
                    "message": _CLIENT_DISCONNECT_DETAIL,
                    "type": "client_disconnect",
                    "param": None,
                    "code": str(DEROUTER_HTTP_STATUS_CLIENT_DISCONNECTED),
                }
            },
            headers=headers,
        )
...
def ttft_keepalive_interval(request_data: Mapping[str, object], llm_router: Router | None = None) -> float | None:
    ...
    return resolve_ttft_keepalive_interval(deployments, derouter.sse_keepalive_ping_interval_seconds)
...
async def open_sse_before_first_byte(
    ...
    as an HTTP error status, and DeRouter's own `x-derouter-*` response headers are
...
def _override_openai_response_model(
    *,
    ...
    DeRouter internally prefixes some provider/deployment model identifiers (e.g. `hosted_vllm/...`).
    ...
    Exceptions:
    1. If a fallback occurred (indicated by x-derouter-attempted-fallbacks header),
    ...
    if isinstance(hidden_params, dict):
        ...
        attempted_fallbacks: Final = fallback_headers.get("x-derouter-attempted-fallbacks", None)
        ...
        if hidden_params.get("fastest_response_batch_completion"):
            winning_model: Final = fallback_headers.get("x-derouter-model-group")
            if winning_model:
                verbose_proxy_logger.debug(
...
def _get_cost_breakdown_from_logging_obj(
    derouter_logging_obj: DeRouterLoggingObj | None,
    ...
    """
    if not derouter_logging_obj or not hasattr(derouter_logging_obj, "cost_breakdown"):
        return CostBreakdownHeaderValues()

    cost_breakdown: Final = derouter_logging_obj.cost_breakdown
    if not cost_breakdown:
        if derouter_logging_obj.call_type in NON_INFERENCE_CALL_TYPES and _totals_to_zero(response_cost):
...
def _classifier_cost_from_request_data(request_data: Mapping[str, object] | None) -> float | None:
    ...
    The pre-routing hook records the decision in `derouter_metadata` on messages/batch-style
    ...
    """
    from derouter.proxy.spend_tracking.savings import classifier_cost_from_decision
...
def _has_attribute_error_in_chain(exc: Exception) -> bool:
    ...
    Checks __cause__, __context__, and the derouter-specific original_exception
...
def _log_llm_api_exception(e: Exception) -> None:
    if getattr(e, "status_code", None) == 499 and getattr(e, "detail", None) == _CLIENT_DISCONNECT_DETAIL:
        verbose_proxy_logger.info(
            "derouter.proxy.proxy_server._handle_llm_api_exception(): client disconnected, upstream LLM request cancelled"
        )
        return
    log_fn: Final = (
        verbose_proxy_logger.error
        if is_expected_client_error(e) and not derouter.log_client_error_tracebacks
        else verbose_proxy_logger.exception
    )
    log_fn("derouter.proxy.proxy_server._handle_llm_api_exception(): Exception occured - %s", e)
...
def _timing_values(
    *,
    ...
    logging_obj: DeRouterLoggingObj | None,
...
class ProxyBaseLLMRequestProcessing:
    ...
    def get_custom_headers(
        *,
        ...
        derouter_logging_obj: DeRouterLoggingObj | None = None,
        ...
        **kwargs,
        ...
        cost_breakdown: Final = _get_cost_breakdown_from_logging_obj(
            derouter_logging_obj=derouter_logging_obj, response_cost=response_cost
        )
        ...
        model_name: Final = ProxyBaseLLMRequestProcessing._get_deployment_model_name(derouter_logging_obj)
        classifier_cost: Final = _classifier_cost_from_request_data(request_data)

        headers: Final = {
            "x-derouter-call-id": call_id,
            "x-derouter-model-id": model_id,
            "x-derouter-model-name": model_name,
            "x-derouter-cache-key": cache_key,
            "x-derouter-model-api-base": (
                api_base.split("?")[0] if api_base else None
            ),  # don't include query params, risk of leaking sensitive info
            "x-derouter-version": version,
            "x-derouter-model-region": model_region,
            "x-derouter-response-cost": str(response_cost),
            "x-derouter-response-cost-original": (
                str(cost_breakdown.original_cost) if cost_breakdown.original_cost is not None else None
            ),
            "x-derouter-response-cost-discount-amount": (
                str(cost_breakdown.discount_amount) if cost_breakdown.discount_amount is not None else None
            ),
            "x-derouter-response-cost-margin-amount": (
                str(cost_breakdown.margin_total_amount) if cost_breakdown.margin_total_amount is not None else None
            ),
            "x-derouter-response-cost-margin-percent": (
                str(cost_breakdown.margin_percent) if cost_breakdown.margin_percent is not None else None
            ),
            "x-derouter-response-cost-input": (
                str(cost_breakdown.input_cost) if cost_breakdown.input_cost is not None else None
            ),
            "x-derouter-response-cost-output": (
                str(cost_breakdown.output_cost) if cost_breakdown.output_cost is not None else None
            ),
            "x-derouter-response-cost-cache-read": (
                str(cost_breakdown.cache_read_cost) if cost_breakdown.cache_read_cost is not None else None
            ),
            "x-derouter-response-cost-cache-creation": (
                str(cost_breakdown.cache_creation_cost) if cost_breakdown.cache_creation_cost is not None else None
            ),
            "x-derouter-response-cost-reasoning": (
                str(cost_breakdown.reasoning_cost) if cost_breakdown.reasoning_cost is not None else None
            ),
            "x-derouter-response-cost-tool-usage": (
                str(cost_breakdown.tool_usage_cost) if cost_breakdown.tool_usage_cost is not None else None
            ),
            "x-derouter-classifier-cost": (str(classifier_cost) if classifier_cost is not None else None),
            "x-derouter-key-tpm-limit": str(user_api_key_dict.tpm_limit),
            "x-derouter-key-rpm-limit": str(user_api_key_dict.rpm_limit),
            "x-derouter-key-max-budget": str(user_api_key_dict.max_budget),
            "x-derouter-key-spend": str(updated_spend),
            "x-derouter-response-duration-ms": str(timing_values.get("_response_ms")),
            "x-derouter-overhead-duration-ms": str(timing_values.get("derouter_overhead_time_ms")),
            "x-derouter-callback-duration-ms": str(hidden_params.get("callback_duration_ms", None)),
            **(
                {
                    "x-derouter-timing-pre-processing-ms": str(hidden_params.get("timing_pre_processing_ms", None)),
                    "x-derouter-timing-llm-api-ms": str(hidden_params.get("timing_llm_api_ms", None)),
                    "x-derouter-timing-post-processing-ms": str(hidden_params.get("timing_post_processing_ms", None)),
                    "x-derouter-timing-message-copy-ms": str(hidden_params.get("timing_message_copy_ms", None)),
                }
                if DEROUTER_DETAILED_TIMING
                else {}
            ),
            "x-derouter-fastest_response_batch_completion": (
                str(fastest_response_batch_completion) if fastest_response_batch_completion is not None else None
            ),
            "x-derouter-timeout": str(timeout) if timeout is not None else None,
    ...
    @staticmethod
    async def build_derouter_proxy_success_headers_from_llm_response(
        *,
        response: object,
        request_data: dict,
        request: Request,
        ...
        """
        Build DeRouter proxy response headers for routes that call the LLM directly
        ...
        custom_headers: Final = ProxyBaseLLMRequestProcessing.get_custom_headers(
            user_api_key_dict=user_api_key_dict,
            call_id=logging_obj.derouter_call_id,
            model_id=model_id,
            cache_key=cache_key,
            api_base=api_base,
            version=version,
    ...
    async def common_processing_pre_call_logic(
    ...
    ) -> tuple[dict, DeRouterLoggingObj]:
        start_time: Final = datetime.now()  # start before calling guardrail hooks

        self.data = await add_derouter_data_to_request(
            data=self.data,
            request=request,
        ...
        # Calculate request queue time after add_derouter_data_to_request
        ...
        # derouter_request_total_latency_metric window of [start_time, end_time] --
        # otherwise the few lines of add_derouter_data_to_request's own work would
        ...
        # Store queue time in metadata after add_derouter_data_to_request to ensure it's preserved
        if queue_time_seconds is not None:
            from derouter.proxy.derouter_pre_call_utils import _get_metadata_variable_name
        ...
        if isinstance(self.data["model"], str) and self.data["model"] in derouter.model_alias_map:
            self.data["model"] = derouter.model_alias_map[self.data["model"]]
        ...
        self.data["derouter_call_id"] = request.headers.get("x-derouter-call-id", str(uuid.uuid4()))
        DDSpanTagger.tag_call_id(self.data.get("derouter_call_id"))
        ...
        logging_obj, self.data = derouter.utils.function_setup(
        ...
        self.data["derouter_logging_obj"] = logging_obj
        ...
        # via derouter_params.guardrails are only honored on post_call paths
        ...
        # and add_derouter_data_to_request preserves client-supplied
        ...
        # add_derouter_data_to_request predates that mutation.
    ...
    def _resolve_fallback_models(
        ...
        from derouter.router_utils.fallback_event_handlers import get_fallback_model_group
    ...
    def _get_model_id_from_response(hidden_params: Mapping[str, object], data: Mapping[str, object]) -> str:
        """Extract model_id from hidden_params with fallback to derouter_metadata."""
        model_id = hidden_params.get("model_id", None) or ""
        if not model_id:
            derouter_metadata: Final = data.get("derouter_metadata")
            model_info: Final = derouter_metadata.get("model_info") if isinstance(derouter_metadata, Mapping) else None
    ...
    def _get_deployment_model_name(
        ...
        ``derouter_params`` metadata as ``deployment``, so read it back from there.
        """
        derouter_params: Final = getattr(derouter_logging_obj, "derouter_params", None)
        if not isinstance(derouter_params, dict):
            return None
        for key in ("derouter_metadata", "metadata"):
            metadata = derouter_params.get(key, {}) or {}
    ...
    def _debug_log_request_payload(self) -> None:
        ...
        if len(_payload_str) > MAX_PAYLOAD_SIZE_FOR_DEBUG_LOG:
            verbose_proxy_logger.debug(
                "Request received by DeRouter: payload too large to log (%d bytes, limit %d). Keys: %s",
                len(_payload_str),
                MAX_PAYLOAD_SIZE_FOR_DEBUG_LOG,
    ...
    async def base_process_llm_request(
        ...
        ``derouter_settings.sse_keepalive_ping_interval_seconds``.
        """
    ...
    async def _process_llm_request(
        ...
        if skip_pre_call_logic:
            logging_obj = self.data.get("derouter_logging_obj")
            if logging_obj is None:
                raise ValueError(
                    "skip_pre_call_logic=True requires derouter_logging_obj to be set in data. "
        ...
        # Cache the result to avoid scanning derouter.callbacks twice.
        ...
        try:
            ...
            asyncio.create_task(
                proxy_logging_obj.update_request_status(
                    derouter_call_id=self.data.get("derouter_call_id", ""),
                    status="success",
                )
            )
            if self._is_streaming_request(
                ...
                if requested_model_from_client:
                    self.data["_derouter_client_requested_model"] = requested_model_from_client
                    ...
                    if self._is_streaming_response(response):
                        ...
                        return await create_response(
                            generator=wrap_sse_stream_with_keepalive_pings(
                                ...
                                ping_interval_seconds=derouter.anthropic_sse_ping_interval_seconds,
                            ),
        ...
        llm_cost_for_headers: Final = (
            0.0
            if is_unbilled_non_inference_call_from_params(logging_obj.call_type, logging_obj.derouter_params, response)
        ...
        if requested_model_from_client:
            _override_openai_response_model(
                ...
                log_context=f"derouter_call_id={logging_obj.derouter_call_id}",
    ...
    async def _record_container_owners_from_responses_if_needed(
        ...
        from derouter.proxy.container_endpoints.ownership import (
    ...
    async def _wrap_responses_stream_for_container_ownership(
        ...
        ``DeRouter_ManagedObjectTable`` row for any container created
    ...
    def _has_post_call_guardrails_for_passthrough(self) -> bool:
        ...
        from derouter.proxy.utils import _check_and_merge_model_level_guardrails
    ...
    def _passthrough_endpoint_has_stream_guardrail_handler(self) -> bool:
        ...
        leaves untouched).
        """
        from derouter.llms.pass_through.guardrail_translation.handler import (
    ...
    def _flush_deferred_async_logging(
        ...
        returns before the deferred block in derouter/utils.py), so this is a
        no-op there.
    ...
    def _arm_deferred_stream_dispatch(
        ...
        - Bridged /v1/responses (DeRouterCompletionStreamingIterator) shares
        ...
        from derouter.router_utils.add_retry_fallback_headers import HiddenParamsAsyncIteratorWrapper
        ...
        if isinstance(unwrapped, DeRouterCompletionStreamingIterator):
    ...
    async def _run_deferred_stream_guardrails(
        ...
        captured_logging_obj: DeRouterLoggingObj,
        ...
        try:
            from derouter.proxy.proxy_server import llm_router as _global_llm_router
            from derouter.proxy.utils import _check_and_merge_model_level_guardrails
            ...
            try:
                ...
                # _is_sync_derouter_request (which only recognizes a subset of
                # async markers stored in derouter_params).
                asyncio.create_task(
    ...
    async def _handle_llm_api_exception(
        ...
        derouter_debug_info: Final = getattr(e, "derouter_debug_info", "")
        verbose_proxy_logger.debug(
        ...
        _derouter_logging_obj: Final[DeRouterLoggingObj | None] = self.data.get("derouter_logging_obj", None)
        ...
        model_id: Final = self.maybe_get_model_id(_derouter_logging_obj)

        custom_headers: Final = ProxyBaseLLMRequestProcessing.get_custom_headers(
            user_api_key_dict=user_api_key_dict,
            call_id=(
                _derouter_logging_obj.derouter_call_id if _derouter_logging_obj else self.data.get("derouter_call_id")
            ),
            model_id=model_id,
            version=version,
            response_cost=0,
            ...
            request_data=self.data,
            timeout=timeout,
            derouter_logging_obj=_derouter_logging_obj,
    ...
    async def async_streaming_data_generator(
        ...
        *,
        ...
        cost_injection_enabled: Final = bool(getattr(derouter, "include_cost_in_streaming_usage", False))
        ...
        try:
            ...
            async for chunk in proxy_logging_obj.async_post_call_streaming_iterator_hook(
                ...
                if not fast_path:
                    ...
                    if isinstance(chunk, (ModelResponse, ModelResponseStream)):
                        response_str = derouter.get_response_string(response_obj=chunk)
                    ...
                    chunk = ProxyBaseLLMRequestProcessing._process_chunk_with_cost_injection(
                        chunk, model_name, request_data.get("derouter_logging_obj")
                    )
            ...
            verbose_proxy_logger.exception(
                "derouter.proxy.proxy_server.async_data_generator(): Exception occured - %s", e
            )
            transformed_exception: Final = await proxy_logging_obj.post_call_failure_hook(
                user_api_key_dict=user_api_key_dict,
                original_exception=e,
    ...
    def _process_chunk_with_cost_injection(
        chunk: bytes, model_name: str, derouter_logging_obj: DeRouterLoggingObj | None = None
    ) -> bytes: ...

    @overload
    @staticmethod
    def _process_chunk_with_cost_injection(
        chunk: object, model_name: str, derouter_logging_obj: DeRouterLoggingObj | None = None
    ) -> object: ...
    ...
    def _process_chunk_with_cost_injection(
        ...
        Args:
            ...
            derouter_logging_obj: The call's logging object, used for pricing

        Returns:
            The processed chunk with cost information injected if applicable
        """
        if not getattr(derouter, "include_cost_in_streaming_usage", False):
            return chunk

        try:
            if isinstance(chunk, dict):
                maybe_modified: Final = ProxyBaseLLMRequestProcessing._inject_cost_into_usage_dict(
                    chunk, model_name, derouter_logging_obj
                )
                ...
                try:
                    ...
                    if s.endswith(("\n\n", "\r\n\r\n")):
                        maybe_mod = ProxyBaseLLMRequestProcessing._inject_cost_into_sse_frame_str(
                            s, model_name, derouter_logging_obj
                        )
    ...
    def _inject_cost_into_sse_frame_str(
        frame_str: str, model_name: str, derouter_logging_obj: DeRouterLoggingObj | None = None
    ) -> str | None:
        """
        ...
        Args:
            ...
            model_name: Model name for cost calculation
            derouter_logging_obj: The call's logging object, forwarded for pricing
                ...
                if stripped_ln.startswith("data:"):
                    ...
                    if json_part and json_part != "[DONE]":
                        ...
                        maybe_modified = ProxyBaseLLMRequestProcessing._inject_cost_into_usage_dict(
                            obj, model_name, derouter_logging_obj
                        )
                        if maybe_modified is not None:
    ...
    def _completion_cost_or_none(
        ...
        try:
            return derouter.completion_cost(
    ...
    def _logging_obj_cost_or_none(
        model_response: ModelResponse, derouter_logging_obj: DeRouterLoggingObj
    ) -> float | None:
        ...
        breakdown_before: Final = getattr(derouter_logging_obj, "cost_breakdown", None)
        call_details: Final = getattr(derouter_logging_obj, "model_call_details", None)
        ...
        try:
            cost: Final = derouter_logging_obj._response_cost_calculator(result=model_response)  # pyright: ignore[reportPrivateUsage]  # reuse the call's own cost calc for pricing parity with the logging callback
        except Exception:  # noqa: BLE001  # a pricing failure falls back to model-name pricing instead of breaking the stream
            return None
        finally:
            if hasattr(derouter_logging_obj, "cost_breakdown"):
                derouter_logging_obj.cost_breakdown = breakdown_before
    ...
    def _streamed_usage_cost(
        ...
        cost_from_logging_obj: Final = (
            ProxyBaseLLMRequestProcessing._logging_obj_cost_or_none(model_response, derouter_logging_obj)
    ...
    def _inject_cost_into_usage_dict(
        obj: dict, model_name: str, derouter_logging_obj: DeRouterLoggingObj | None = None
    ) -> dict | None:
        """
    ...
    def maybe_get_model_id(self, _logging_obj: DeRouterLoggingObj | None) -> str | None:
        """
        ...
        model_id = None
        if _logging_obj:
            # 1. Try getting from derouter_params (updated during call)
            if hasattr(_logging_obj, "derouter_params") and _logging_obj.derouter_params:
                # First check direct model_info path (set by router.py with selected deployment)
                model_info = _logging_obj.derouter_params.get("model_info") or {}
                model_id = model_info.get("id", None)

                # Fallback to nested metadata path
                if not model_id:
                    metadata = _logging_obj.derouter_params.get("metadata") or {}
            ...
            if not model_id:
                ...
                if _kwargs:
                    derouter_params: Final = _kwargs.get("derouter_params", {})
                    # First check direct model_info path
                    model_info = derouter_params.get("model_info") or {}
                    model_id = model_info.get("id", None)
        ...
        # 3. Final fallback to self.data["derouter_metadata"] (for routes like /v1/responses that populate data before error)
        if not model_id:
            derouter_metadata: Final = self.data.get("derouter_metadata", {}) or {}
            model_info = derouter_metadata.get("model_info", {}) or {}
            model_id = model_info.get("id", None)
