...
from derouter.proxy.auth.auth_utils import is_request_body_safe
from derouter.proxy.auth.user_api_key_auth import user_api_key_auth
from derouter.proxy.common_utils.path_utils import safe_filename
from derouter.proxy.prompts.prompt_registry import (
    DEFAULT_PROMPT_ENVIRONMENT,
    get_base_prompt_id,
    get_version_number,
    prompt_environment_or_default,
)
from derouter.repositories.table_repositories import PromptRepository
...
from derouter.types.proxy.prompt_endpoints import TestPromptRequest
...
class _PromptRow(Protocol):
    ...
    @property
    def derouter_params(self) -> str | Mapping[str, object]: ...
    @property
...
class _PromptRowData(BaseModel):
    ...
    derouter_params: str | Mapping[str, object] | None = None
...
def create_versioned_prompt_spec(db_prompt: _PromptRow) -> PromptSpec:
    ...
    from derouter.types.prompts.init_prompts import PromptDeRouterParams

    row: Final = _PromptRowData.model_validate(db_prompt.model_dump())

    derouter_params_data: Final = row.derouter_params
    derouter_params_dict: Final[Mapping[str, object] | None] = (
        json.loads(derouter_params_data) if isinstance(derouter_params_data, str) else derouter_params_data
    )
    derouter_params: Final = PromptDeRouterParams.model_validate(derouter_params_dict)
...
AMBIGUOUS_PROMPT_DATA_ERROR: Final = (
    "derouter_params.prompt_id cannot be combined with prompt_data keyed by template name. "
    'Send a flat template, prompt_data={"content": "...", "metadata": {...}}, together with derouter_params.prompt_id, '
    'or send prompt_data={"<template_id>": {"content": "...", "metadata": {...}}} without derouter_params.prompt_id.'
)


def is_ambiguous_keyed_prompt_data(derouter_params: PromptDeRouterParams) -> bool:
    extra_fields: Final = derouter_params.model_extra or {}
    prompt_data: Final = extra_fields.get("prompt_data")
    if not derouter_params.prompt_id or not isinstance(prompt_data, dict):
        return False
    return bool(prompt_data) and "content" not in prompt_data


class PatchPromptRequest(BaseModel):
    derouter_params: PromptDeRouterParams | None = None
    prompt_info: PromptInfo | None = None
...
async def list_prompts(
    ...
    👉 [Prompt docs](https://router.delify.vn/docs/proxy/prompt_management)
        ...
        if prompts is not None:
            ...
            for original_prompt in all_prompts:
                ...
                prompt_copy = PromptSpec(
                    ...
                    derouter_params=original_prompt.derouter_params,
    ...
    if prisma_client is not None:
        ...
        for db_prompt in db_prompts:
            ...
            versioned_prompts.append(
                PromptSpec(
                    ...
                    derouter_params=spec.derouter_params,
        ...
        for prompt in prompt_versions:
            ...
            versioned_prompts.append(
                PromptSpec(
                    prompt_id=base_prompt_id,
                    derouter_params=prompt.derouter_params,
...
def _get_prompt_template(prompt_spec: PromptSpec, base_prompt_id: str) -> PromptTemplateBase | None:
    ...
    try:
        dotprompt_content: Final = prompt_spec.derouter_params.dotprompt_content
        if dotprompt_content:
            ...
            if parsed:
                return PromptTemplateBase(
                    derouter_prompt_id=base_prompt_id,
            ...
            if prompt_callback is not None:
                ...
                if integration_name == "dotprompt":
                    from derouter.integrations.dotprompt.dotprompt_manager import (
                        DotpromptManager,
                    )
                    ...
                    if isinstance(prompt_callback, DotpromptManager):
                        ...
                        if template is not None and len(template) == 1:
                            ...
                            return PromptTemplateBase(
                                derouter_prompt_id=template_id,
...
async def get_prompt_info(
    ...
    prompt_spec_response: Final = PromptSpec(
        ...
        derouter_params=prompt_spec.derouter_params,
...
async def create_prompt(
    ...
    if is_ambiguous_keyed_prompt_data(request.derouter_params):
    ...
    try:
        ...
        prompt_db_entry: Final = await _prompt_table(prisma_client).create(
            data={
                ...
                "derouter_params": request.derouter_params.model_dump_json(),
                "prompt_info": (
...
async def patch_prompt(
    ...
    - derouter_params: DeRouter parameters for the prompt
    ...
    if request.derouter_params is not None and is_ambiguous_keyed_prompt_data(request.derouter_params):
        raise HTTPException(status_code=400, detail=AMBIGUOUS_PROMPT_DATA_ERROR)
    ...
    try:
        ...
        updated_derouter_params: Final = (
            request.derouter_params if request.derouter_params is not None else current_spec.derouter_params
        )
        ...
        update_data: Final[dict[str, str]] = {
            "derouter_params": updated_derouter_params.model_dump_json(),
...
async def test_prompt(
    ...
    from derouter.integrations.dotprompt.prompt_manager import (
        PromptManager,
        PromptTemplate,
    )
