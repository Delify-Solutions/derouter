...
def get_prompt_initializer_from_integrations():
    ...
    try:
        ...
        for item in os.listdir(integrations_dir):
            ...
            module_path = f"derouter.integrations.{item}"
            try:
                # Import the module
...
class InMemoryPromptRegistry:
    ...
    def initialize_prompt(
        ...
        derouter.logging_callback_manager.add_derouter_callback(custom_prompt_callback)
    ...
    def _build_prompt_callback(self, prompt: PromptSpec) -> tuple[PromptSpec, CustomPromptManagement]:
        derouter_params_data: Final = prompt.derouter_params
        verbose_proxy_logger.debug("derouter_params= %s", derouter_params_data)

        if isinstance(derouter_params_data, dict):
            derouter_params = PromptDeRouterParams(**derouter_params_data)
        else:
            derouter_params = derouter_params_data

        prompt_integration: Final = derouter_params.prompt_integration
        ...
        custom_prompt_callback: Final = initializer(derouter_params, prompt)
    ...
    def reload_prompt(self, prompt: PromptSpec) -> PromptSpec | None:
        ...
        if stale_callback is not None:
            derouter.logging_callback_manager.remove_callback_from_all_lists(stale_callback)
        derouter.logging_callback_manager.add_derouter_callback(new_callback)
        self.IN_MEMORY_PROMPTS[registry_key] = parsed_prompt
    ...
    def sync_prompt_from_db(self, prompt: PromptSpec) -> PromptSpec | None:
        ...
        if existing.derouter_params == prompt.derouter_params and existing.prompt_info == prompt.prompt_info:
            return existing
