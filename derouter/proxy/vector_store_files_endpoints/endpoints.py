...
from derouter.proxy.auth.auth_checks import _can_object_call_model, can_key_call_model
...
from derouter.proxy.rag_endpoints.upload_security import safe_download_headers
...
async def _update_request_data_with_managed_file_id(
    ...
    1. Simple encoded file IDs (format: derouter:{file_id};model,{model})
    2. Unified managed file IDs (format: derouter_proxy:{mime};unified_id,{uuid};...;llm_output_file_id,{file_id};...)
    ...
    Args:
        ...
        llm_router: DeRouter router for credential lookup (required for managed files)
    ...
    if decoded_id:
        ...
        if parsed_id:
            ...
            # Format: derouter_proxy:...;llm_output_file_id,{actual_file_id};...
    ...
    # Fall back to simple encoded file ID handling (format: derouter:{file_id};model,{model})
    (
        should_route,
        model_used,
        original_file_id,
...
async def _update_request_data_with_model_routing_hint(
    ...
    user_controlled_model_hint: Final = request.query_params.get("model") or request.headers.get("x-derouter-model")
...
def _update_request_data_with_derouter_managed_vector_store_registry(
    data: dict,
    vector_store_id: str,
    llm_router: Optional["Router"] = None,
    managed_vector_store: DeRouter_ManagedVectorStore | None = None,
    ...
    Args:
        ...
        llm_router: DeRouter router for credential lookup (required for managed vector stores)
    ...
    if vector_store_to_run is None and should_lookup_registry and derouter.vector_store_registry is not None:
        vector_store_to_run = derouter.vector_store_registry.get_derouter_managed_vector_store_from_registry(
    ...
    if vector_store_to_run is not None:
        ...
        if "derouter_credential_name" in vector_store_to_run:
            data["derouter_credential_name"] = vector_store_to_run.get("derouter_credential_name")
        if "derouter_params" in vector_store_to_run:
            derouter_params: Final = vector_store_to_run.get("derouter_params", {}) or {}
            data.update(derouter_params)
...
async def vector_store_file_create(
    ...
    data = _update_request_data_with_derouter_managed_vector_store_registry(
        data=data,
        vector_store_id=vector_store_id,
        llm_router=llm_router,
