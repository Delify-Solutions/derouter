...
from derouter.constants import (
    ...
    DEROUTER_PROXY_MASTER_KEY_ALIAS,
...
from derouter.derouter_core_utils.url_utils import (
...
from derouter.proxy.auth.auth_utils import get_request_route
...
from derouter.proxy.common_utils.http_parsing_utils import _safe_get_request_headers
from derouter.types.integrations.anthropic_cache_control_hook import GATEWAY_INJECTED_CACHE_METADATA_KEY
...
_CREDENTIAL_HEADER_NAMES: Final = SpecialHeaders.derouter_credential_header_names() | frozenset(
...
# Excludes the two explicit derouter headers which are handled with higher priority.
_GENERIC_SESSION_ID_HEADER_RE: Final = re.compile(r"^x-.+-session-id$", re.IGNORECASE)
_EXPLICIT_SESSION_HEADERS: Final = frozenset({"x-derouter-trace-id", "x-derouter-session-id"})
...
def _stampable_key_hash(user_api_key_dict: UserAPIKeyAuth) -> str | None:
    ...
    if not user_api_key_dict.via_virtual_key or api_key is None:
        return None
    if api_key == DEROUTER_PROXY_MASTER_KEY_ALIAS or _SHA256_HEX_RE.fullmatch(api_key):
        return api_key
    return None
...
from derouter.secret_managers.main import get_secret_bool
from derouter.types.llms.anthropic import ANTHROPIC_API_HEADERS
...
if TYPE_CHECKING:
    from derouter.integrations.otel.model.destination import OtelDestination
    from derouter.proxy.proxy_server import ProxyConfig as _ProxyConfig
    from derouter.types.proxy.policy_engine import Policy, PolicyMatchContext
...
DEROUTER_METADATA_ROUTES: Final = (
    "batches",
    "bedrock",
    "/v1/messages",
    "responses",
...
DEROUTER_TRACE_CONTROL_METADATA_FIELDS: Final = frozenset(
    {
        "mask_input",
        "mask_output",
        "session_id",
...
_UNTRUSTED_ROOT_CONTROL_FIELDS: Final = (
    ...
    # and ``logger_fn`` are read by ``derouter.utils.function_setup`` and
    # appended to process-wide ``derouter.{input,success,failure,_async_*,
    # service}_callback`` lists / ``derouter.user_logger_fn`` — one request
    # poisons the worker for every subsequent caller.
    # ``derouter_disabled_callbacks`` is the inverse primitive: the
...
UNTRUSTED_REQUEST_HEADER_CONTROL_FIELDS: Final = frozenset(
    {
        "derouter-disable-message-redaction",
    }
)
...
# ``derouter.completion`` → ``register_model``) the process-wide
# ``derouter.model_cost`` map. Both effects belong to deployment configuration,
...
_CLIENT_PRICING_CONTROL_FIELDS: Final = frozenset(CustomPricingDeRouterParams.model_fields.keys())
...
def _reject_url_valued_destinations(data: dict[str, object]) -> None:
    ...
    ``derouter.provider_url_destination_allowed_hosts``.
    """
...
def reject_url_valued_destination(field: str, value: str) -> None:
    ...
    """
    allowed_hosts: Final = getattr(derouter, "provider_url_destination_allowed_hosts", []) or []
...
def _strip_client_callback_credentials(
    ...
    from team/key callback settings in ``add_derouter_data_to_request``.
    """
...
def _get_metadata_variable_name(request: Request) -> str:
    ...
    For all /thread or /assistant endpoints we need to call this "derouter_metadata"

    For ALL other endpoints we call this "metadata"
    """
    # Inline imports — auth_utils/route_checks participate in a proxy import cycle.
    from derouter.proxy.auth.auth_utils import get_request_route  # noqa: PLC0415

    path: Final = get_request_route(request)
    if "thread" in path or "assistant" in path:
        return "derouter_metadata"

    if any(route in path for route in DEROUTER_METADATA_ROUTES):
        return "derouter_metadata"
...
def _promoted_trace_control_fields(
    ...
    derouter_metadata: Mapping[str, object],
) -> tuple[tuple[str, object], ...]:
    """Return the caller's trace-control fields that ``derouter_metadata`` does not already set."""
    return tuple(
        (key, value)
        for key, value in requester_metadata.items()
        if key in DEROUTER_TRACE_CONTROL_METADATA_FIELDS and key not in derouter_metadata
    )
...
def _extract_generic_session_id_from_headers(
    ...
    The two explicit DeRouter headers (``x-derouter-trace-id`` /
    ``x-derouter-session-id``) are excluded here because they are handled with
...
def get_chain_id_from_headers(headers: dict[str, str] | None) -> str | None:
    ...
    Priority order:
    1. ``x-derouter-trace-id`` (explicit, highest priority)
    2. ``x-derouter-session-id`` (explicit)
    ...
    derouter_trace_id/derouter_session_id for spend logs and logging consistency.
    """
    if not headers:
        return None
    normalized: Final = {k.lower(): v for k, v in headers.items() if isinstance(k, str)}
    return (
        normalized.get("x-derouter-trace-id")
        or normalized.get("x-derouter-session-id")
...
def _is_llm_inference_route(request: Request) -> bool:
    ...
    return RouteChecks.is_llm_api_route(route=route) and not RouteChecks.check_route_access(
        route=route, allowed_routes=DeRouterRoutes.mcp_routes.value
    )
...
def apply_missing_session_id_policy(
    ...
    if policy == "omit":
        ...
        return
    if data.get("derouter_session_id") or metadata.get("session_id"):
        return
    match policy:
        case "generate":
            session_id: Final = str(data.get("derouter_trace_id") or metadata.get("trace_id") or uuid.uuid4())
            data["derouter_session_id"] = session_id  # rebind-ok: data is an out-param
            data.setdefault("derouter_trace_id", session_id)
            metadata["session_id"] = session_id
            metadata[SESSION_ID_GENERATED_METADATA_KEY] = True
        case "reject":
            raise ProxyException(
                message=(
                    "Request has no session id. Send an `x-derouter-session-id` header or `metadata.session_id`. "
...
def should_auto_drop_params_for_agentic_cli(user_agent: str, data: dict, proxy_config: ProxyConfig) -> bool:
    ...
    from the caller or in the operator's ``derouter_settings`` always wins
    over this default."""
    from derouter.llms.anthropic.common_utils import is_claude_code_user_agent
    ...
    derouter_settings: Final = config.get("derouter_settings") if isinstance(config, dict) else None
    return not (isinstance(derouter_settings, dict) and "drop_params" in derouter_settings)
...
def convert_key_logging_metadata_to_callback(
    ...
    ):  # assume 'success_and_failure' = derouter.callbacks
        if team_callback_settings_obj.success_callback is None:
            team_callback_settings_obj.success_callback = []
        if team_callback_settings_obj.failure_callback is None:
            team_callback_settings_obj.failure_callback = []
...
def _get_dynamic_logging_metadata(
    ...
    #########################################################################################
        ...
        callback_settings_obj = DeRouterProxyRequestSetup.add_team_based_callbacks_from_config(
...
def _dynamically_disabled_backends(
    ...
    before it skips a callback: the ``x-derouter-disable-callbacks`` header wins over the
    ...
    if derouter.allow_dynamic_callback_disabling is not True or not premium_user:
        return frozenset()
    ...
    disabled: Final = metadata.get("derouter_disabled_callbacks") if metadata else None
...
def resolve_tenant_otel_destinations(
    ...
    ``derouter_disabled_callbacks`` or the ``x-derouter-disable-callbacks`` header in
    ...
    from derouter.integrations.otel.presets.destinations import destination_for
...
def clean_headers(
    headers: Headers,
    derouter_key_header_name: str | None = None,
    forward_llm_provider_auth_headers: bool = False,
    authenticated_with_header: str | None = None,
) -> dict:
    """
    Removes derouter api key from headers

    Args:
        headers: Request headers
        derouter_key_header_name: Custom header name for DeRouter API key
        forward_llm_provider_auth_headers: Whether to forward provider auth headers
        authenticated_with_header: Which header was used for DeRouter authentication
            (e.g., "x-derouter-api-key", "authorization", "x-api-key")

    Returns:
        Cleaned headers dict
    """
    from derouter.llms.anthropic.common_utils import is_anthropic_oauth_key

    clean_headers: Final = {}
    derouter_key_lower: Final = derouter_key_header_name.lower() if derouter_key_header_name is not None else None
    for header, value in headers.items():
        ...
        # Special handling for x-api-key: forward it based on authenticated_with_header
            ...
            if derouter_key_lower and header_lower == derouter_key_lower:
                continue
            if header_lower == "authorization":
                continue
            # Never forward x-derouter-api-key (it's for proxy auth only)
            if header_lower == "x-derouter-api-key":
                continue
            clean_headers[header] = value
        # Check if header should be excluded: either in special headers cache or matches custom derouter key
        elif header_lower not in _SPECIAL_HEADERS_CACHE and (
            derouter_key_lower is None or header_lower != derouter_key_lower
        ):
            clean_headers[header] = value
...
class DeRouterProxyRequestSetup:
    @staticmethod
    def _get_timeout_from_request(headers: dict) -> float | None:
        ...
        const openaiProvider = createOpenAI({
            ...
            headers: {
                "x-derouter-timeout": "90"
            },
        });
        ```
        """
        timeout_header: Final = headers.get("x-derouter-timeout", None)
    ...
    def _get_stream_timeout_from_request(headers: dict) -> float | None:
        ...
        """
        stream_timeout_header: Final = headers.get("x-derouter-stream-timeout", None)
    ...
    def _get_keepalive_seconds_from_request(headers: Mapping[str, str]) -> float | None:
        ...
        """
        keepalive_seconds_header: Final = headers.get("x-derouter-keepalive-seconds", None)
    ...
    def _get_num_retries_from_request(headers: dict) -> int | None:
        ...
        Workaround for client request from Vercel's AI SDK.
        """
        num_retries_header: Final = headers.get("x-derouter-num-retries", None)
    ...
    def _get_spend_logs_metadata_from_request_headers(headers: dict) -> dict | None:
        ...
        spend_logs_metadata_header: Final = headers.get("x-derouter-spend-logs-metadata", None)
    ...
    def add_internal_user_from_user_mapping(
        ...
        if not user_header_mapping:
            return user_api_key_dict
        header_name: Final = DeRouterProxyRequestSetup.get_internal_user_header_from_mapping(user_header_mapping)
        if not header_name:
            return user_api_key_dict
        header_value: Final = DeRouterProxyRequestSetup._get_case_insensitive_header(headers, header_name)
        if header_value:
    ...
    def get_user_from_headers(headers: dict, general_settings: dict | None = None) -> str | None:
        ...
        user: Final = DeRouterProxyRequestSetup._get_case_insensitive_header(headers, header_name)
        if user is not None:
    ...
    def add_headers_to_llm_call(headers: dict, user_api_key_dict: UserAPIKeyAuth) -> dict:
        ...
        returned_headers: Final = DeRouterProxyRequestSetup._get_forwardable_headers(headers)

        if derouter.add_user_information_to_llm_headers is True:
            derouter_logging_metadata_headers: Final = DeRouterProxyRequestSetup.get_sanitized_user_information_from_key(
                user_api_key_dict=user_api_key_dict
            )
            for k, v in derouter_logging_metadata_headers.items():
                if v is None:
                    continue
                ...
                if isinstance(v, (dict, list)):
                    returned_headers[f"x-derouter-{k}"] = json.dumps(v)
                elif isinstance(v, (str, bytes)):
                    returned_headers[f"x-derouter-{k}"] = v
                else:
                    returned_headers[f"x-derouter-{k}"] = str(v)
    ...
    def add_headers_to_llm_call_by_model_group(data: dict, headers: dict, user_api_key_dict: UserAPIKeyAuth) -> dict:
        ...
        """
        from derouter.proxy.auth.auth_checks import _check_model_access_helper
        ...
        if (
            ...
            and derouter.model_group_settings is not None
            and derouter.model_group_settings.forward_client_headers_to_llm_api is not None
            and _check_model_access_helper(
                model=data_model,
                llm_router=llm_router,
                models=derouter.model_group_settings.forward_client_headers_to_llm_api,
                team_model_aliases=user_api_key_dict.team_model_aliases,
                team_id=user_api_key_dict.team_id,
            )  # handles aliases, wildcards, etc.
        ):
            _headers: Final = DeRouterProxyRequestSetup.add_headers_to_llm_call(headers, user_api_key_dict)
            if _headers != {}:
    ...
    def get_internal_user_header_from_mapping(user_header_mapping) -> str | None:
        ...
        for item in items:
            if not isinstance(item, dict):
                continue
            role = item.get("derouter_user_role")
    ...
    @staticmethod
    def add_derouter_data_for_backend_llm_call(
        *,
        headers: dict,
        ...
        if general_settings and general_settings.get("forward_client_headers_to_llm_api") is True:
            ...
            if _headers != {}:
                data["headers"] = _headers
        _organization: Final = DeRouterProxyRequestSetup.get_openai_org_id_from_headers(headers, general_settings)
        if _organization is not None:
            data["organization"] = _organization

        header_timeout: Final = DeRouterProxyRequestSetup._get_timeout_from_request(headers)
        if header_timeout is not None:
            data["timeout"] = header_timeout

        header_stream_timeout: Final = DeRouterProxyRequestSetup._get_stream_timeout_from_request(headers)
        ...
        num_retries: Final = DeRouterProxyRequestSetup._get_num_retries_from_request(headers)
        if num_retries is not None:
            data["num_retries"] = num_retries

        keepalive_seconds: Final = DeRouterProxyRequestSetup._get_keepalive_seconds_from_request(headers)
    ...
    @staticmethod
    def add_derouter_metadata_from_request_headers(
        headers: dict,
        data: dict,
        _metadata_variable_name: str,
    ) -> dict:
        """
        Add derouter metadata from request headers

        Relevant issue: https://github.com/Delify-Solutions/derouter/issues/14008
        """
        from derouter.proxy._types import LitellmMetadataFromRequestHeaders

        metadata_from_headers: Final = LitellmMetadataFromRequestHeaders()
        spend_logs_metadata: Final = DeRouterProxyRequestSetup._get_spend_logs_metadata_from_request_headers(headers)
        ...
        agent_id_from_header: Final = headers.get("x-derouter-agent-id")
        # Explicit derouter headers take precedence; fall back to any x-*-session-id header.
        ...
        if chain_id:
            ...
            data["derouter_session_id"] = chain_id
            data["derouter_trace_id"] = chain_id
            ...
            if session_id:
                ...
                data["derouter_session_id"] = session_id
        ...
        # explicit derouter headers nor the Anthropic-metadata path found
        # anything - but lets a caller's existing traceparent/baggage headers
        # (from real OTel instrumentation) correlate with derouter's own logs
        ...
        if "derouter_trace_id" not in data:
            ...
            if isinstance(traceparent, str):
                ...
                if trace_id_from_traceparent:
                    ...
                    data["derouter_trace_id"] = trace_id_from_traceparent  # rebind-ok: data is an out-param
                    verbose_proxy_logger.debug(
                        "Extracted trace_id from W3C traceparent header: %s", trace_id_from_traceparent
                    )
        if "derouter_session_id" not in data:
            ...
            if isinstance(baggage, str):
                ...
                if session_id_from_baggage:
                    ...
                    data["derouter_session_id"] = session_id_from_baggage  # rebind-ok: data is an out-param
    ...
    def add_management_endpoint_metadata_to_request_metadata(
        ...
        added_metadata: Final = {
            ...
            if k not in (DeRouter_ManagementEndpoint_MetadataFields_Premium + DeRouter_ManagementEndpoint_MetadataFields)
        }
    ...
    def add_key_level_controls(key_metadata: dict | None, data: dict, _metadata_variable_name: str):
        ...
        if "tags" in key_metadata and key_metadata["tags"] is not None:
            data[_metadata_variable_name]["tags"] = DeRouterProxyRequestSetup._merge_tags(
        ...
        ## KEY-LEVEL METADATA
        data = DeRouterProxyRequestSetup.add_management_endpoint_metadata_to_request_metadata(
            data=data,
    ...
    def add_team_based_callbacks_from_config(
        ...
        callback_vars_dict = {
            key: (derouter.utils.get_secret(value, default_value=value) or value if isinstance(value, str) else value)
    ...
    def add_request_tag_to_metadata(
        ...
        if "x-derouter-tags" in headers:
            if isinstance(headers["x-derouter-tags"], str):
                _tags: Final = headers["x-derouter-tags"].split(",")
                tags = [tag.strip() for tag in _tags]
            elif isinstance(headers["x-derouter-tags"], list):
                tags = headers["x-derouter-tags"]
    ...
    @staticmethod
    def pre_seed_derouter_metadata_for_route(
        request_data: dict,
        route: str,
    ) -> None:
        """Pre-seed ``derouter_metadata`` for routes that track tags there.

        Routes in ``DEROUTER_METADATA_ROUTES`` (e.g. Bedrock, ``/v1/messages``,
        responses, batches, files) store request-scoped tag metadata in
        ``derouter_metadata`` rather than the provider-facing ``metadata``
        field. ``get_metadata_variable_name_from_kwargs`` picks the target
        based on whether ``derouter_metadata`` is present, so it must be
        ...
        ``_tag_max_budget_check`` resolve to ``derouter_metadata``, leaving
        header tags invisible to per-tag budget enforcement.
        """
        if any(metadata_route in route for metadata_route in DEROUTER_METADATA_ROUTES):
            request_data.setdefault("derouter_metadata", {})
    ...
    def apply_key_tags_pre_auth(
        ...
        metadata["tags"] = DeRouterProxyRequestSetup._merge_tags(
    ...
    def apply_client_tag_policy_pre_auth(
        ...
        """
        Merge ``x-derouter-tags`` header tags into ``request_data`` BEFORE
        ...
        Why: ``add_derouter_data_to_request`` runs the equivalent merge
        ...
        # from (derouter_metadata vs metadata) so the merged tags are visible
...
def refresh_proxy_server_request_body_snapshot(
    ...
    ``add_derouter_data_to_request`` takes the initial snapshot before guardrails
    ...
    By the time a caller refreshes post-guardrail, ``derouter.utils.function_setup``
    has already stamped ``data["derouter_logging_obj"]`` with a live (non-serializable)
    ...
    _body_snapshot_exclude = (
        frozenset({"secret_fields", "proxy_server_request", "derouter_logging_obj"}) | _TRANSPORT_ONLY_CREDENTIAL_KEYS
    )
    proxy_server_request["body"] = {k: v for k, v in data.items() if k not in _body_snapshot_exclude}


async def add_derouter_data_to_request(
    data: dict,
    request: Request,
    user_api_key_dict: UserAPIKeyAuth,
    proxy_config: ProxyConfig,
    ...
    """
    Adds DeRouter-specific data to the request.
    ...
    from derouter.proxy.proxy_server import llm_router, premium_user
    from derouter.types.proxy.derouter_pre_call_utils import RedactedDict, SecretFields
    ...
    _raw_metadata_by_field: Final = {
        ...
        for _metadata_field in ("metadata", "derouter_metadata")
    ...
    if not forward_llm_auth:
        forward_llm_auth = getattr(derouter, "forward_llm_provider_auth_headers", False)
    # Determine which header was used for authentication
    # This enables forwarding provider keys (e.g., x-api-key) when they weren't used for DeRouter auth
    authenticated_with_header = None
    if "x-derouter-api-key" in request.headers:
        # If x-derouter-api-key is present, it was used for auth
    ...
    _headers: Final[dict[str, str]] = clean_headers(
        request.headers,
        derouter_key_header_name=(
            general_settings.get("derouter_key_header_name") if general_settings is not None else None
        ),
    ...
    # the top of user_api_key_auth (request.state.derouter_received_at): by the
    ...
    _derouter_received_at: Final[datetime | None] = getattr(request.state, "derouter_received_at", None)
    arrival_time: Final = _derouter_received_at.timestamp() if _derouter_received_at is not None else time.time()
    ...
    data.update(
        DeRouterProxyRequestSetup.add_derouter_data_for_backend_llm_call(
            headers=_headers,
            request_data=data,
            user_api_key_dict=user_api_key_dict,
    ...
    data = DeRouterProxyRequestSetup.add_headers_to_llm_call_by_model_group(
        data=data, headers=_headers, user_api_key_dict=user_api_key_dict
    )

    user_api_key_dict = DeRouterProxyRequestSetup.add_internal_user_from_user_mapping(
        general_settings, user_api_key_dict, _headers
    )

    # Parse user info from headers (fallback to general_settings.user_header_name)
    user: Final = DeRouterProxyRequestSetup.get_user_from_headers(_headers, general_settings)
    if user is not None:
    ...
    if derouter.overwrite_user_with_key_hash is True:
    ...
    for _meta_key in ("metadata", "derouter_metadata"):
    ...
    # Strip pricing overrides AFTER the derouter_metadata string-to-dict parse
    ...
    if not _allow_client_message_redaction_opt_out and derouter.turn_off_message_logging is True:
    ...
    if "metadata" in data and isinstance(data["metadata"], dict):
        ...
        if _metadata_variable_name == "derouter_metadata":
            data[_metadata_variable_name].update(
                _promoted_trace_control_fields(
                    ...
                    derouter_metadata=data[_metadata_variable_name],
                )
            )

    # Merge derouter_metadata into the metadata variable (preserving existing
    ...
    # into derouter_metadata — cannot cross-contaminate the admin-authoritative
    # metadata dict.
    if "derouter_metadata" in data and isinstance(data["derouter_metadata"], dict):
        for key, value in data["derouter_metadata"].items():
            if key not in data[_metadata_variable_name]:
                data[_metadata_variable_name][key] = value
        if _metadata_variable_name == "metadata":
            data["metadata"]["tags"] = DeRouterProxyRequestSetup._merge_tags(  # pyright: ignore[reportPrivateUsage]  # same-module helper, budget blocks the unsuppressed idiom sibling call sites use
                request_tags=data["metadata"].get("tags"),
                tags_to_add=data["derouter_metadata"].get("tags"),
            )
    if _metadata_variable_name == "metadata":
        data.pop("derouter_metadata", None)

    data = DeRouterProxyRequestSetup.add_user_api_key_auth_to_request_metadata(
        data=data,
        user_api_key_dict=user_api_key_dict,
        _metadata_variable_name=_metadata_variable_name,
    )
    data[_metadata_variable_name]["derouter_api_version"] = version
    ...
    ### KEY-LEVEL Controls
    key_metadata: Final = user_api_key_dict.metadata
    data = DeRouterProxyRequestSetup.add_key_level_controls(
    ...
    data[_metadata_variable_name]["derouter_received_at"] = getattr(request.state, "derouter_received_at", None)

    # OTEL Controls / Tracing
    # Add the OTEL Parent Trace before sending it DeRouter
    data[_metadata_variable_name]["derouter_parent_otel_span"] = user_api_key_dict.parent_otel_span
    ...
    # Merge caller-supplied tags (x-derouter-tags header, data["tags"] root-level)
    # into request metadata for tag-based routing and spend attribution.
    tags: Final = DeRouterProxyRequestSetup.add_request_tag_to_metadata(
        llm_router=llm_router,
        headers=_headers,
        data=data,
    )
    ...
    if _caller_body_tags:
        data[_metadata_variable_name]["tags"] = DeRouterProxyRequestSetup._merge_tags(  # rebind-ok: matches file idiom
            request_tags=data[_metadata_variable_name].get("tags"),
            tags_to_add=_caller_body_tags,
        )

    # caller_tags: exactly what this request itself supplied (x-derouter-tags header,
    # body "tags", or body "metadata.tags" on derouter_metadata routes), never
    ...
    if user_api_key_dict.metadata and "derouter_disabled_callbacks" in user_api_key_dict.metadata:
        disabled_callbacks: Final = user_api_key_dict.metadata["derouter_disabled_callbacks"]
        if disabled_callbacks and isinstance(disabled_callbacks, list):
            data["derouter_disabled_callbacks"] = disabled_callbacks
    ...
    verbose_proxy_logger.debug("[PROXY] returned data from derouter_pre_call_utils: %s", data)
    ...
    asyncio.create_task(
        service_logger_obj.async_service_success_hook(
            ...
            call_type="add_derouter_data_to_request",
...
def _update_model_if_team_alias_exists(
    ...
    if _ENABLE_TEAM_STALE_ALIAS_BYPASS is None:
        _ENABLE_TEAM_STALE_ALIAS_BYPASS = get_secret_bool("DEROUTER_ENABLE_TEAM_STALE_ALIAS_BYPASS", False)
    ...
    if is_stale_team_alias and llm_router:
        ...
        if key in llm_router.team_model_to_deployment_indices:
            ...
            _warn_stale_team_alias_once(
                ...
                "Set DEROUTER_ENABLE_TEAM_STALE_ALIAS_BYPASS=true to enable "
...
def _apply_credential_overrides_from_model_config(
    ...
    """
    # Feature flag gate — disabled by default, opt in with derouter.enable_model_config_credential_overrides = True
    if not derouter.enable_model_config_credential_overrides:
        return
    ...
    # deployment's derouter_params so multi-provider defaultconfig entries
...
def _resolve_provider_from_deployment(
    ...
    """
    Resolve a provider hint from the deployment's derouter_params when the
    ...
    for name in candidates:
        ...
        derouter_params: object = getattr(deployment, "derouter_params", None)
        if derouter_params is None:
            continue

        custom_provider = getattr(derouter_params, "custom_llm_provider", None)
        if isinstance(custom_provider, str) and custom_provider:
            return custom_provider

        deployment_model = getattr(derouter_params, "model", "")
...
def _resolve_credential_from_model_config(
    ...
    When a model-specific entry exists but contains no derouter_credentials,
    the function falls through to defaultconfig. This is intentional —
    an entry without derouter_credentials is treated as incomplete config,
    ...
    for model_config in (project_model_config, team_model_config):
        ...
        for name in model_names_to_try:
            ...
            if model_entry:
                ...
                verbose_proxy_logger.debug(
                    "model_config entry '%s' found but has no derouter_credentials, trying next candidate",
                    _safe_name,
                )
...
def _extract_credential_from_entry(entry: dict, provider: str | None = None) -> str | None:
    """
    Extract derouter_credentials from a model_config entry.

    Entry structure: {"azure": {"derouter_credentials": "name"}, ...}
    ...
    if provider and provider in entry:
        ...
        if isinstance(provider_config, dict):
            credential_name = provider_config.get("derouter_credentials")
            if credential_name:
                return credential_name
...
def _add_guardrails_from_key_or_team_metadata(
    ...
    """
    from derouter.proxy.utils import _premium_user_check
...
def _add_guardrails_from_policies_in_metadata(
    ...
    from derouter.proxy.policy_engine.policy_resolver import PolicyResolver
    from derouter.proxy.utils import _premium_user_check
    from derouter.types.proxy.policy_engine import PolicyMatchContext
...
def _is_policy_version_id(s: str) -> bool:
    ...
    from derouter.proxy.policy_engine.policy_registry import POLICY_VERSION_ID_PREFIX
...
def _match_and_track_policies(
    ...
    from derouter.proxy.policy_engine.policy_matcher import PolicyMatcher
    ...
    # Track policy attribution sources for x-derouter-policy-sources header
...
async def add_guardrails_from_policy_engine(
    ...
    """
    from derouter._logging import verbose_proxy_logger
    from derouter.proxy.common_utils.http_parsing_utils import get_tags_from_request_body
    from derouter.proxy.policy_engine.policy_registry import get_policy_registry
    from derouter.types.proxy.policy_engine import PolicyMatchContext
...
def _add_otel_traceparent_to_data(data: dict, request: Request):
    from derouter.proxy.proxy_server import open_telemetry_logger

    if data is None:
        return
    if open_telemetry_logger is None:
        # if user is not use OTEL don't send extra_headers
        # relevant issue: https://github.com/Delify-Solutions/derouter/issues/4448
        return

    if derouter.forward_traceparent_to_llm_provider is True:
        if request.headers:
            if "traceparent" in request.headers:
                # we want to forward this to the LLM Provider
                # Relevant issue: https://github.com/Delify-Solutions/derouter/issues/4419
