...
from derouter.constants import DEFAULT_STAGGER_WINDOW_SECONDS, MCP_STDIO_ALLOWED_COMMANDS
from derouter.derouter_core_utils.initialize_dynamic_callback_params import (
    validate_langfuse_environment_value,
    validate_no_callback_env_reference,
)
from derouter.types.integrations.compression_interception import (
...
from derouter.types.mcp import (
    MCPAuth,
    MCPAuthType,
    MCPCredentials,
    MCPTransport,
...
from derouter.types.mcp_server.mcp_server_manager import MCPInfo
from derouter.types.proxy.control_plane_endpoints import WorkerRegistryEntry
from derouter.types.router import RouterErrors, UpdateRouterConfig
from derouter.types.secret_managers.main import KeyManagementSystem
...
class DeRouterTeamRoles(enum.Enum):
    # team admin
    TEAM_ADMIN = "admin"
    # team member
    TEAM_MEMBER = "user"
...
class LitellmUserRoles(str, enum.Enum):
    ...
    def description(self):
        ...
        """
        descriptions: Final = {
            "proxy_admin": "admin over derouter proxy, has all permissions",
...
class LitellmTableNames(str, enum.Enum):
    """
    Enum for Table Names used by DeRouter
    """

    TEAM_TABLE_NAME = "DeRouter_TeamTable"
    USER_TABLE_NAME = "DeRouter_UserTable"
    KEY_TABLE_NAME = "DeRouter_VerificationToken"
    PROXY_MODEL_TABLE_NAME = "DeRouter_ProxyModelTable"
    MANAGED_FILE_TABLE_NAME = "DeRouter_ManagedFileTable"
    TOOL_TABLE_NAME = "DeRouter_ToolTable"
    CACHE_CONFIG_TABLE_NAME = "DeRouter_CacheConfig"
    CONFIG_OVERRIDES_TABLE_NAME = "DeRouter_ConfigOverrides"
    CONFIG_TABLE_NAME = "DeRouter_Config"
    SSO_CONFIG_TABLE_NAME = "DeRouter_SSOConfig"
    UI_SETTINGS_TABLE_NAME = "DeRouter_UISettings"


class Litellm_EntityType(enum.Enum):
    """
    Enum for types of entities on derouter
...
class DeRouterRoutes(enum.Enum):
    openai_route_names = [
        "chat_completion",
        "completion",
        "embeddings",
    ...
    public_routes = frozenset(
        (
            ...
            "/public/mcp_hub",
            "/public/skill_hub",
            "/public/derouter_model_cost_map",
        )
    )
...
class DeRouterPromptInjectionParams(DeRouterPydanticObjectBase):
...
class ProxyChatCompletionRequest(DeRouterPydanticObjectBase):
    """
    Pydantic model for chat completion requests that includes both OpenAI standard fields
    and DeRouter-specific parameters. This replaces the previous TypedDict version.
    """
    ...
    # DeRouter-specific metadata param (from original ChatCompletionRequest)
    metadata: dict[str, Any] | None = None

    # Optional DeRouter params
...
class ModelInfoDelete(DeRouterPydanticObjectBase):
    id: str


class ModelInfo(DeRouterPydanticObjectBase):
    id: str | None
...
class ProviderInfo(DeRouterPydanticObjectBase):
    name: str
    fields: list[ProviderField]


class BlockUsers(DeRouterPydanticObjectBase):
    user_ids: list[str]  # required


class ModelParams(DeRouterPydanticObjectBase):
    model_name: str
...
class DeRouter_ObjectPermissionBase(DeRouterPydanticObjectBase):
...
from derouter.models.team import BudgetLimitEntry as BudgetLimitEntry  # noqa: E402
from derouter.types.object_permission import (  # noqa: E402
    ObjectPermissionDict as ObjectPermissionDict,
)


class GenerateRequestBase(DeRouterPydanticObjectBase):
    """
    ...
    aliases: dict | None = {}
    object_permission: DeRouter_ObjectPermissionBase | None = None
...
class AllowedVectorStoreIndexItem(DeRouterPydanticObjectBase):
    index_name: str
...
class DeRouterKeyType(str, enum.Enum):
    """
...
class GenerateKeyRequest(KeyRequestBase):
    ...
    key_type: DeRouterKeyType | None = Field(
        default=DeRouterKeyType.DEFAULT,
...
class GenerateKeyResponse(KeyRequestBase):
    ...
    organization_id: str | None = None
    project_id: str | None = None
    derouter_budget_table: Any | None = None
    token: str | None = None
...
class ResetSpendRequest(DeRouterPydanticObjectBase):
    reset_to: float
...
class KeyRequest(DeRouterPydanticObjectBase):
...
from derouter.models.model import (  # noqa: E402
    DeRouter_ProxyModelTable as DeRouter_ProxyModelTable,
)
from derouter.models.team import DeRouter_ModelTable as DeRouter_ModelTable  # noqa: E402
...
from derouter.models.mcp_server import (  # noqa: E402
...
class NewMCPServerRequest(DeRouterPydanticObjectBase):
...
class UpdateMCPServerRequest(DeRouterPydanticObjectBase):
    server_id: str
    server_name: str | None = None
    alias: str | None = None
    description: str | None = None
...
from derouter.models.mcp_server import (  # noqa: E402
    DeRouter_MCPServerTable as DeRouter_MCPServerTable,
)


class MakeMCPServersPublicRequest(DeRouterPydanticObjectBase):
    mcp_server_ids: list[str]


class MCPUserCredentialRequest(DeRouterPydanticObjectBase):
    credential: str
    save: bool = True


class MCPUserCredentialResponse(DeRouterPydanticObjectBase):
    server_id: str
    has_credential: bool


class MCPOAuthUserCredentialRequest(DeRouterPydanticObjectBase):
...
class MCPOAuthUserCredentialStatus(DeRouterPydanticObjectBase):
...
class MCPUserCredentialListItem(DeRouterPydanticObjectBase):
...
class MCPUserEnvVarsRequest(DeRouterPydanticObjectBase):
    """Payload for storing the calling user's per-user env var values."""

    values: dict[str, str]


class MCPUserEnvVarSpec(DeRouterPydanticObjectBase):
...
class MCPUserEnvVarsStatus(DeRouterPydanticObjectBase):
...
class RejectMCPServerRequest(DeRouterPydanticObjectBase):
    review_notes: str | None = None


class MCPSubmissionsSummary(DeRouterPydanticObjectBase):
    total: int
    pending_review: int
    active: int
    rejected: int
    items: list["DeRouter_MCPServerTable"]


######## Skills API Types ########


class NewSkillRequest(DeRouterPydanticObjectBase):
    """Request to create a new skill in DeRouter database"""
...
class UpdateSkillRequest(DeRouterPydanticObjectBase):
...
from derouter.models.skills import (  # noqa: E402
    DeRouter_SkillsTable as DeRouter_SkillsTable,
)


class ListSkillsRequest(DeRouterPydanticObjectBase):
    """Request to list skills from DeRouter database"""

    limit: int | None = 20
    offset: int | None = 0


class NewUserRequestTeam(DeRouterPydanticObjectBase):
    team_id: str
...
class DeleteUserRequest(DeRouterPydanticObjectBase):
    user_ids: list[str]  # required
...
class BudgetNewRequest(DeRouterPydanticObjectBase):
...
class BudgetRequest(DeRouterPydanticObjectBase):
    budgets: list[str]


class BudgetDeleteRequest(DeRouterPydanticObjectBase):
    id: str


class CustomerBase(DeRouterPydanticObjectBase):
    user_id: str
    alias: str | None = None
    spend: float = 0.0
    ...
    budget_id: str | None = None
    derouter_budget_table: BudgetNewRequest | None = None
    blocked: bool = False
...
class UpdateCustomerRequest(DeRouterPydanticObjectBase):
    """
...
class DeleteCustomerRequest(DeRouterPydanticObjectBase):
    """
...
from derouter.models.team import Member as Member  # noqa: E402
from derouter.models.team import MemberBase as MemberBase  # noqa: E402
...
from derouter.models.team import TeamBase as TeamBase  # noqa: E402
...
class GlobalEndUsersSpend(DeRouterPydanticObjectBase):
    api_key: str | None = None
    startTime: datetime | None = None
    endTime: datetime | None = None


class UpdateTeamRequest(DeRouterPydanticObjectBase):
    """
...
class ResetTeamBudgetRequest(DeRouterPydanticObjectBase):
    """
...
class DeleteTeamRequest(DeRouterPydanticObjectBase):
    team_ids: list[str]  # required


class BlockTeamRequest(DeRouterPydanticObjectBase):
    team_id: str  # required


class BlockKeyRequest(DeRouterPydanticObjectBase):
    key: str  # required


class BlockModelRequest(DeRouterPydanticObjectBase):
    model_id: str  # required


class AddTeamCallback(DeRouterPydanticObjectBase):
    callback_name: str
...
class TeamCallbackDeleteResponseData(DeRouterPydanticObjectBase):
    team_id: str
    success_callbacks: tuple[str, ...]
    failure_callbacks: tuple[str, ...]


class TeamCallbackDeleteResponse(DeRouterPydanticObjectBase):
    status: Literal["success"]
    message: str
    data: TeamCallbackDeleteResponseData


class TeamCallbackMetadata(DeRouterPydanticObjectBase):
...
from derouter.models.object_permission import (  # noqa: E402
    DeRouter_ObjectPermissionTable as DeRouter_ObjectPermissionTable,
)
from derouter.models.team import (  # noqa: E402
    DeRouter_DeletedTeamTable as DeRouter_DeletedTeamTable,
)
from derouter.models.team import DeRouter_TeamTable as DeRouter_TeamTable  # noqa: E402
from derouter.models.team import (  # noqa: E402
    DeRouter_TeamTableCachedObj as DeRouter_TeamTableCachedObj,
)


class TeamRequest(DeRouterPydanticObjectBase):
    teams: list[str]


from derouter.models.budget import (  # noqa: E402
    DeRouter_BudgetTable as DeRouter_BudgetTable,
)
from derouter.models.budget import (  # noqa: E402
    DeRouter_BudgetTableFull as DeRouter_BudgetTableFull,
)
from derouter.models.budget import (  # noqa: E402
    DeRouter_TeamMemberTable as DeRouter_TeamMemberTable,
)


class NewOrganizationRequest(DeRouter_BudgetTable):
    organization_id: str | None = None
...
class OrganizationRequest(DeRouterPydanticObjectBase):
    organizations: list[str]


class DeleteOrganizationRequest(DeRouterPydanticObjectBase):
    organization_ids: list[str]  # required


class TeamDefaultSettings(DeRouterPydanticObjectBase):
    team_id: str

    model_config = ConfigDict(
        extra="allow"
    )  # allow params not defined here, these fall in derouter.completion(**kwargs)


class DynamoDBArgs(DeRouterPydanticObjectBase):
    ...
    region_name: str
    user_table_name: str = "DeRouter_UserTable"
    key_table_name: str = "DeRouter_VerificationToken"
    config_table_name: str = "DeRouter_Config"
    spend_table_name: str = "DeRouter_SpendLogs"
...
class PassThroughGuardrailSettings(DeRouterPydanticObjectBase):
    """
...
class PassThroughGenericEndpoint(DeRouterPydanticObjectBase):
    id: str | None = Field(
        default=None,
    ...
    path: str = Field(description="The route to be added to the DeRouter Proxy Server.")
    ...
    auth: bool = Field(
        default=True,
        description="Whether authentication is required for the pass-through endpoint. Defaults to True so a pass-through silently created without an explicit value still requires a valid DeRouter API key — set to False only if the endpoint is meant to be a public forwarder (e.g. an unauthenticated webhook target).",
    )
...
class PassThroughEndpointResponse(DeRouterPydanticObjectBase):
    endpoints: list[PassThroughGenericEndpoint]


class ConfigFieldUpdate(DeRouterPydanticObjectBase):
    field_name: str
    field_value: Any
    config_type: Literal["general_settings"]


class ConfigFieldDelete(DeRouterPydanticObjectBase):
    config_type: Literal["general_settings"]
    field_name: str


class CallbackDelete(DeRouterPydanticObjectBase):
    callback_name: str
...
class ConfigList(DeRouterPydanticObjectBase):
    field_name: str
    field_type: str
    field_description: str
    field_value: Any
...
class UserHeaderMapping(DeRouterPydanticObjectBase):
    """
    Map an incoming HTTP header to a DeRouter user role.
    """

    header_name: str
    derouter_user_role: Literal[
        LitellmUserRoles.INTERNAL_USER,
...
class PluginConfig(DeRouterPydanticObjectBase):
...
class CoordinationRedisNode(DeRouterPydanticObjectBase):
...
class CoordinationRedisParams(DeRouterPydanticObjectBase):
    """
    ...
    independently of the response-cache backend in `derouter_settings.cache_params`.
    """
...
class ScheduledJobStaggerSettings(DeRouterPydanticObjectBase):
    """
...
class ConfigGeneralSettings(DeRouterPydanticObjectBase):
    """
    ...
    database_extra_connection_params: dict[str, Any] | None = Field(
        ...
        description=(
            ...
            "`statement_cache_size`). Keys here override any default DeRouter sets."
        ),
    )
    ...
    custom_auth: str | None = Field(
        None,
        description="override user_api_key_auth with your own auth script - https://router.delify.vn/docs/proxy/virtual_keys#custom-auth",
    )
    max_parallel_requests: int | None = Field(
        None,
    ...
    missing_session_id: Literal["generate", "reject", "omit"] | None = Field(
        None,
        description="What to do with LLM API requests that carry no session id (x-derouter-session-id header, metadata.session_id, etc.). 'generate' stamps one id into derouter_session_id, derouter_trace_id and metadata.session_id so SpendLogs and logging callbacks agree; 'reject' returns 400; 'omit' leaves SpendLogs.session_id null, matching callbacks such as Langfuse that only record a client-established metadata.session_id. Unset keeps the legacy behavior where SpendLogs falls back to the trace id while callbacks get no session id.",
    )
    ...
    pass_through_endpoints: list[PassThroughGenericEndpoint] | None = Field(
        default=None,
        description="Set-up pass-through endpoints for provider-specific endpoints. Docs - https://router.delify.vn/docs/proxy/pass_through",
    )
    ...
    use_spend_logs_partitioning: bool | None = Field(
        None,
        description="If True and DeRouter_SpendLogs has been converted to a range-partitioned table (db_scripts/partition_spend_logs.sql), retention cleanup drops expired partitions instead of deleting rows, and pre-creates upcoming partitions. Default is False.",
    )
...
class ConfigYAML(DeRouterPydanticObjectBase):
    """
    ...
    derouter_settings: dict | None = Field(
        None,
        description="derouter Module settings. See __init__.py for all, example derouter.drop_params=True, derouter.set_verbose=True, derouter.api_base, derouter.cache",
    )
    ...
    router_settings: UpdateRouterConfig | None = Field(
        None,
        description="derouter router object settings. See router.py __init__ for all, example router.num_retries=5, router.timeout=5, router.max_retries=5, router.retry_after=5",
    )

    model_config = ConfigDict(protected_namespaces=())


from derouter.models.verification_token import (  # noqa: E402
    DeRouter_DeletedVerificationToken as DeRouter_DeletedVerificationToken,
)
from derouter.models.verification_token import (  # noqa: E402
    DeRouter_VerificationToken as DeRouter_VerificationToken,
)


class DeRouter_VerificationTokenView(DeRouter_VerificationToken):
    """
    Combined view of derouter verification token + derouter team table (select values)
    """
    ...
    def __init__(self, **kwargs):
        # Handle derouter_budget_table_* keys (budget table overrides when key value is None or empty)
        for key, value in list(kwargs.items()):
            if key.startswith("derouter_budget_table_") and value is not None:
                # Extract the corresponding attribute name
                attr_name = key.replace("derouter_budget_table_", "")
...
class UserAPIKeyAuth(DeRouter_VerificationTokenView):  # the expected response object for user api key auth
    """
    ...
    end_user_object_permission: DeRouter_ObjectPermissionTable | None = None
    ...
    team_object_permission: DeRouter_ObjectPermissionTable | None = None
    ...
    def check_api_key(cls, values):
        ...
        if values.get("api_key") is not None:
            values.update({"token": cls._safe_hash_derouter_api_key(values.get("api_key"))})
            if isinstance(values.get("api_key"), str):
                values.update({"api_key": cls._safe_hash_derouter_api_key(values.get("api_key"))})
        return values

    @classmethod
    def _safe_hash_derouter_api_key(cls, api_key: str) -> str:
        """
        Helper to ensure all logged keys are hashed
        Covers:
        1. Regular API keys from DeRouter DB
        2. JWT tokens used for connecting to DeRouter API
        """
        normalized = api_key
    ...
    @classmethod
    def get_derouter_internal_health_check_user_api_key_auth(cls) -> "UserAPIKeyAuth":
        """
        Returns a `UserAPIKeyAuth` object for the derouter internal health check service account.

        This is used to track number of requests/spend for health check calls.
        """
        from derouter.constants import LITTELM_INTERNAL_HEALTH_SERVICE_ACCOUNT_NAME
    ...
    @classmethod
    def get_derouter_cli_user_api_key_auth(cls) -> "UserAPIKeyAuth":
        """
        Returns a `UserAPIKeyAuth` object for the derouter internal health check service account.

        This is used to track number of requests/spend for health check calls.
        """
        from derouter.constants import LITTELM_CLI_SERVICE_ACCOUNT_NAME
    ...
    @classmethod
    def get_derouter_internal_jobs_user_api_key_auth(cls) -> "UserAPIKeyAuth":
        """
        Returns a `UserAPIKeyAuth` object for internal DeRouter jobs like key rotation.

        This is used to track actions performed by automated system jobs.
        """
        from derouter.constants import DEROUTER_INTERNAL_JOBS_SERVICE_ACCOUNT_NAME

        return cls(
            api_key=DEROUTER_INTERNAL_JOBS_SERVICE_ACCOUNT_NAME,
            team_id="system",
            key_alias=DEROUTER_INTERNAL_JOBS_SERVICE_ACCOUNT_NAME,
            team_alias="system",
            user_id="system",
...
def user_api_key_has_admin_view(user_api_key_dict: UserAPIKeyAuth) -> bool:
    ...
    Lives on _types.py so leaf modules (e.g. derouter.llms.base_llm.managed_resources)
    can use it without pulling in derouter.proxy.utils via management_endpoints.
    """
...
class UserInfoResponse(DeRouterPydanticObjectBase):
    user_id: str | None
...
class UserInfoV2Response(DeRouterPydanticObjectBase):
    """
...
from derouter.models.config import DeRouter_Config as DeRouter_Config  # noqa: E402
from derouter.models.organization_membership import (  # noqa: E402
    DeRouter_OrganizationMembershipTable as DeRouter_OrganizationMembershipTable,
)


class DeRouter_OrganizationTableUpdate(DeRouter_BudgetTable):
    ...
    @classmethod
    def set_model_info(cls, values):
        for field in DeRouter_ManagementEndpoint_MetadataFields:
...
class OrganizationUpdateRequestV2(DeRouterPydanticObjectBase):
    """
...
from derouter.models.organization import (  # noqa: E402
    DeRouter_OrganizationTable as DeRouter_OrganizationTable,
)
from derouter.models.user import DeRouter_UserTable as DeRouter_UserTable  # noqa: E402


class DeRouter_OrganizationTableWithMembers(DeRouter_OrganizationTable):
    """Returned by the /organization/info endpoint and /organization/list endpoint"""

    members: list[DeRouter_OrganizationMembershipTable] = []
    teams: list[DeRouter_TeamTable] = []
    derouter_budget_table: DeRouter_BudgetTable | None = None
    created_at: datetime
    updated_at: datetime


class NewOrganizationResponse(DeRouter_OrganizationTable):
    organization_id: str
    created_at: datetime
    updated_at: datetime
...
class ProjectBase(DeRouterPydanticObjectBase):
...
class NewProjectRequest(DeRouter_BudgetTable):
...
class UpdateProjectRequest(DeRouter_BudgetTable):
...
class DeleteProjectRequest(DeRouterPydanticObjectBase):
    """Request model for DELETE /project/delete"""

    project_ids: list[str]


from derouter.models.project import (  # noqa: E402
    DeRouter_ProjectTable as DeRouter_ProjectTable,
)


class NewProjectResponse(DeRouter_ProjectTable):
...
class DeRouter_ProjectTableCachedObj(DeRouter_ProjectTable):
    """Cached version for auth checks. Mirrors DeRouter_TeamTableCachedObj pattern."""

    last_refreshed_at: float | None = None


class DeRouter_UserTableFiltered(BaseModel):  # done to avoid exposing sensitive data
    user_id: str
    user_email: str | None = None


class DeRouter_UserTableWithKeyCount(DeRouter_UserTable):
    key_count: int = 0


from derouter.models.access_group import (  # noqa: E402
    DeRouter_AccessGroupTable as DeRouter_AccessGroupTable,
)
from derouter.models.end_user import (  # noqa: E402
    DeRouter_EndUserTable as DeRouter_EndUserTable,
)
from derouter.models.spend_logs import (  # noqa: E402
    DeRouter_ErrorLogs as DeRouter_ErrorLogs,
)
from derouter.models.spend_logs import (  # noqa: E402
    DeRouter_SpendLogs as DeRouter_SpendLogs,
)
from derouter.models.tag import DeRouter_TagTable as DeRouter_TagTable  # noqa: E402

AUDIT_ACTIONS = Literal["created", "updated", "deleted", "blocked", "unblocked", "rotated"]


class DeRouter_AuditLogs(DeRouterPydanticObjectBase):
    id: str
    updated_at: datetime
...
class DeRouter_SpendLogs_ResponseObject(DeRouterPydanticObjectBase):
    response: list[DeRouter_SpendLogs | Any] | None = None


class TokenCountRequest(DeRouterPydanticObjectBase):
    model: str
...
class CallInfo(DeRouterPydanticObjectBase):
...
class InvitationNew(DeRouterPydanticObjectBase):
    user_id: str


class InvitationUpdate(DeRouterPydanticObjectBase):
    invitation_id: str
    is_accepted: bool


class InvitationDelete(DeRouterPydanticObjectBase):
    invitation_id: str


class InvitationModel(DeRouterPydanticObjectBase):
    id: str
    user_id: str
    is_accepted: bool
...
class InvitationClaim(DeRouterPydanticObjectBase):
    invitation_link: str
    user_id: str
    password: str


class ConfigFieldInfo(DeRouterPydanticObjectBase):
    field_name: str
    field_value: Any


class CallbackOnUI(DeRouterPydanticObjectBase):
    derouter_callback_name: str
    derouter_callback_params: list | None
    ui_callback_name: str


class AllCallbacks(DeRouterPydanticObjectBase):
    langfuse: CallbackOnUI = CallbackOnUI(
        derouter_callback_name="langfuse",
        ui_callback_name="Langfuse",
        derouter_callback_params=[
            "LANGFUSE_PUBLIC_KEY",
            "LANGFUSE_SECRET_KEY",
            "LANGFUSE_HOST",
        ],
    ...
    azure_sentinel: CallbackOnUI = CallbackOnUI(
        derouter_callback_name="azure_sentinel",
    ...
    openmeter: CallbackOnUI = CallbackOnUI(
        derouter_callback_name="openmeter",
    ...
    custom_callback_api: CallbackOnUI = CallbackOnUI(
        derouter_callback_name="custom_callback_api",
        derouter_callback_params=["GENERIC_LOGGER_ENDPOINT", "GENERIC_LOGGER_HEADERS"],
        ui_callback_name="Custom Callback API",
    )

    generic_api: CallbackOnUI = CallbackOnUI(
        derouter_callback_name="generic_api",
        derouter_callback_params=["GENERIC_LOGGER_ENDPOINT", "GENERIC_LOGGER_HEADERS"],
        ui_callback_name="Custom Callback API",
    )

    datadog: CallbackOnUI = CallbackOnUI(
        derouter_callback_name="datadog",
        derouter_callback_params=["DD_API_KEY", "DD_SITE"],
        ui_callback_name="Datadog",
    )

    braintrust: CallbackOnUI = CallbackOnUI(
        derouter_callback_name="braintrust",
        derouter_callback_params=["BRAINTRUST_API_KEY", "BRAINTRUST_API_BASE"],
        ui_callback_name="Braintrust",
    )

    langsmith: CallbackOnUI = CallbackOnUI(
        derouter_callback_name="langsmith",
        derouter_callback_params=[
            "LANGSMITH_API_KEY",
            "LANGSMITH_PROJECT",
    ...
    traceloop: CallbackOnUI = CallbackOnUI(
        derouter_callback_name="traceloop",
        derouter_callback_params=[
            "TRACELOOP_API_KEY",
        ],
    ...
    galileo: CallbackOnUI = CallbackOnUI(
        derouter_callback_name="galileo",
        derouter_callback_params=[
            "GALILEO_API_KEY",
            "GALILEO_PROJECT_ID",
    ...
    newrelic: CallbackOnUI = CallbackOnUI(
        derouter_callback_name="newrelic",
...
class SpendLogsMetadata(TypedDict):
    ...
    derouter_overhead_time_ms: float | None  # DeRouter overhead time in milliseconds
    ...
    derouter_gateway_injected_cache: ReadOnly[str | None]
...
class SpanAttributes(str, enum.Enum):
    ...
    # I chose to not add a new dependency to derouter for this
...
class ManagementEndpointLoggingPayload(DeRouterPydanticObjectBase):
    route: str
    request_data: dict
...
class ProxyException(Exception):
    ...
    def __init__(
        ...
        # Related DeRouter issue: https://github.com/Delify-Solutions/derouter/discussions/4834
        self.code = str(code)
        ...
        # Should map to 429 errors https://github.com/Delify-Solutions/derouter/issues/2487
...
class CommonProxyErrors(str, enum.Enum):
    db_not_connected_error = (
        ...
        "See https://router.delify.vn/docs/proxy/virtual_keys"
    )
    no_llm_router = "No models configured on proxy"
    not_allowed_access = "Admin-only endpoint. Not allowed to access this."
    not_premium_user = "You must be a DeRouter Enterprise user to use this feature. If you have a license please set `DEROUTER_LICENSE` in your env. Get a 7 day trial key here: https://www.router.delify.vn/enterprise#trial. \nPricing: https://www.router.delify.vn/#pricing"
    max_parallel_request_limit_reached = "Crossed TPM / RPM / Max Parallel Request Limit"
    missing_enterprise_package = "Missing derouter-enterprise package. Please install it to use this feature. Run `pip install derouter-enterprise`"
    missing_enterprise_package_docker = "This uses the enterprise folder - only available on the Docker image."


class SpendCalculateRequest(DeRouterPydanticObjectBase):
    model: str | None = None
...
class VirtualKeyEvent(DeRouterPydanticObjectBase):
...
class CreatePassThroughEndpoint(DeRouterPydanticObjectBase):
    path: str
    target: str
    headers: dict


from derouter.models.team_membership import (  # noqa: E402
    DeRouter_TeamMembership as DeRouter_TeamMembership,
)

#### Organization / Team Member Requests ####


class MemberAddRequest(DeRouterPydanticObjectBase):
...
class OrgMemberAddRequest(DeRouterPydanticObjectBase):
...
class TeamAddMemberResponse(DeRouter_TeamTable):
    updated_users: list[DeRouter_UserTable]
    updated_team_memberships: list[DeRouter_TeamMembership]


class OrganizationAddMemberResponse(DeRouterPydanticObjectBase):
    organization_id: str
    updated_users: list[DeRouter_UserTable]
    updated_organization_memberships: list[DeRouter_OrganizationMembershipTable]


class MemberDeleteRequest(DeRouterPydanticObjectBase):
    user_id: str | None = None
    user_email: str | None = None
...
class MemberUpdateResponse(DeRouterPydanticObjectBase):
    user_id: str
    user_email: str | None = None
...
class TeamAccessGroupModelGrant(DeRouterPydanticObjectBase):
    access_group_id: str
...
class TeamInfoResponseObjectTeamTable(DeRouter_TeamTable):
    team_member_budget_table: DeRouter_BudgetTableFull | None = None
...
class TeamInfoResponseObject(TypedDict):
    ...
    keys: list
    team_memberships: list[DeRouter_TeamMembership]


class TeamListResponseObject(DeRouter_TeamTable):
    team_memberships: list[DeRouter_TeamMembership]
    keys: list  # list of keys that belong to the team


class KeyListResponseObject(TypedDict, total=False):
    keys: list[str | UserAPIKeyAuth | DeRouter_DeletedVerificationToken]
...
class CreateJWTKeyMappingRequest(DeRouterPydanticObjectBase):
    jwt_claim_name: str
    jwt_claim_value: str
    key: str
    description: str | None = None
...
class UpdateJWTKeyMappingRequest(DeRouterPydanticObjectBase):
    id: str
    key: str | None = None
    description: str | None = None
...
class DeleteJWTKeyMappingRequest(DeRouterPydanticObjectBase):
    id: str


class JWTKeyMappingResponse(DeRouterPydanticObjectBase):
    id: str
    jwt_claim_name: str
    jwt_claim_value: str
    description: str | None = None
...
class SpecialHeaders(enum.Enum):
    """Used by user_api_key_auth.py to get derouter key"""
    ...
    custom_derouter_api_key = "x-derouter-api-key"
    mcp_auth = "x-mcp-auth"
    mcp_servers = "x-mcp-servers"
    mcp_access_groups = "x-mcp-access-groups"

    @classmethod
    def derouter_credential_header_names(cls) -> "frozenset[str]":
        """Lowercased header names user_api_key_auth accepts as a derouter key.
        ...
        avoid leaking the caller's derouter credential downstream. The static
        custom-key header (general_settings.derouter_key_header_name) is runtime
...
class LitellmDataForBackendLLMCall(TypedDict, total=False):
    ...
    # `x-derouter-timeout`/`x-derouter-stream-timeout` headers, or a `timeout`/`request_timeout`/
...
class LitellmMetadataFromRequestHeaders(TypedDict, total=False):
    """
    Headers a user can pass that will get added to derouter metadata for the request
    """
...
class UserManagementEndpointParamDocStringEnums(str, enum.Enum):
    ...
    user_role_doc_str = """Optional[str] - Specify a user role - "proxy_admin", "proxy_admin_viewer", "internal_user", "internal_user_viewer", "team", "customer". Info about each role here: `https://github.com/Delify-Solutions/derouter/derouter/proxy/_types.py#L20`"""
    ...
    aliases_doc_str = """Optional[dict] - Model aliases for the user - [Docs](https://derouter.vercel.app/docs/proxy/virtual_keys#model-aliases)"""
    config_doc_str = """Optional[dict] - [DEPRECATED PARAM] User-specific config."""
    allowed_cache_controls_doc_str = """Optional[list] - List of allowed cache control values. Example - ["no-cache", "no-store"]. See all values - https://router.delify.vn/docs/proxy/caching#turn-on--off-caching-per-request-"""
    ...
    model_max_budget_doc_str = """Optional[dict] - Model-specific max budget for user. [Docs](https://router.delify.vn/docs/proxy/users#add-model-specific-budgets-to-keys)"""
    model_rpm_limit_doc_str = """Optional[float] - Model-specific rpm limit for user. [Docs](https://router.delify.vn/docs/proxy/users#add-model-specific-limits-to-keys)"""
    model_tpm_limit_doc_str = """Optional[float] - Model-specific tpm limit for user. [Docs](https://router.delify.vn/docs/proxy/users#add-model-specific-limits-to-keys)"""
    spend_doc_str = (
...
DeRouter_ManagementEndpoint_MetadataFields: Final = [
    "model_rpm_limit",
    "model_tpm_limit",
    "model_itpm_limit",
    "model_otpm_limit",
...
DeRouter_ManagementEndpoint_MetadataFields_Premium: Final = [
...
DeRouter_Reserved_Metadata_Fields: Final = [
    "service_account_id",
]


class ProviderBudgetResponseObject(DeRouterPydanticObjectBase):
    """
...
class ProviderBudgetResponse(DeRouterPydanticObjectBase):
    """
...
UI_TEAM_ID = "derouter-dashboard"


class JWTAuthBuilderResult(TypedDict):
    is_proxy_admin: bool
    team_object: DeRouter_TeamTable | None
    user_object: DeRouter_UserTable | None
    end_user_object: DeRouter_EndUserTable | None
    org_object: DeRouter_OrganizationTable | None
    token: str
    team_id: str | None
    user_id: str | None
    user_email: str | None
    ...
    org_id: str | None
    team_membership: DeRouter_TeamMembership | None
...
class OIDCPermissions(DeRouterPydanticObjectBase):
    models: list[str] | None = None
...
class JWTDeRouterRoleMap(BaseModel):
    jwt_role: str
    derouter_role: LitellmUserRoles
...
class UnregisteredJWTClientBehavior(str, enum.Enum):
    ...
    JWT claim value has no registered mapping in `derouter_jwtkeymapping`.
...
class JWTIssuerConfig(BaseModel):
    ...
    ``DeRouter_JWTAuth.issuers``, DeRouter validates it only against that
    ...
    jwks_url: str | None = Field(
        default=None,
        description="Issuer JWKS URL. If omitted, DeRouter uses the issuer's OIDC discovery document.",
    )
    ...
    disable_audience_validation: bool = Field(
        default=False,
        description="Explicitly disable audience validation for this issuer. Use only when the issuer cannot provide an audience suitable for DeRouter.",
    )
    user_id_jwt_field: str | None = Field(
        default=None,
        description="Issuer-specific claim path to normalize into DeRouter's user id.",
    )
    user_email_jwt_field: str | None = Field(
        default=None,
        description="Issuer-specific claim path to normalize into DeRouter's user email.",
    )
    team_id_jwt_field: str | None = Field(
        default=None,
        description="Issuer-specific claim path to normalize into DeRouter's team id.",
    )
    team_ids_jwt_field: str | None = Field(
        default=None,
        description="Issuer-specific claim path to normalize into DeRouter's team ids.",
    )
    org_id_jwt_field: str | None = Field(
        default=None,
        description="Issuer-specific claim path to normalize into DeRouter's organization id.",
    )
    end_user_id_jwt_field: str | None = Field(
        default=None,
        description="Issuer-specific claim path to normalize into DeRouter's end-user id.",
    )
...
class DeRouter_JWTAuth(DeRouterPydanticObjectBase):
    """
    A class to define the roles and permissions for a DeRouter Proxy w/ JWT Auth.
    ...
    - user_id_jwt_field: The field in the JWT token that stores the user id (maps to `DeRouterUserTable`). Use this for internal employees.
    - user_email_jwt_field: The field in the JWT token that stores the user email (maps to `DeRouterUserTable`). Use this for internal employees.
    - user_allowed_email_subdomain: If specified, only emails from specified subdomain will be allowed to access proxy.
    - end_user_id_jwt_field: The field in the JWT token that stores the end-user ID (maps to `DeRouterEndUserTable`). Turn this off by setting to `None`. Enables end-user cost tracking. Use this for external customers.
    ...
    - public_allowed_routes: list of allowed routes for authenticated but unknown derouter role jwt tokens.
    - enforce_rbac: If true, enforce RBAC for all routes.
    - custom_validate: A custom function to validates the JWT token.
    - oidc_userinfo_endpoint: OIDC UserInfo endpoint URL. When set along with oidc_userinfo_enabled, DeRouter will call this endpoint with the access token to retrieve user identity information.
    ...
    admin_jwt_scope: str = "derouter_proxy_admin"
    ...
    jwt_derouter_role_map: list[JWTDeRouterRoleMap] | None = None
    ...
    oidc_userinfo_endpoint: str | None = Field(
        default=None,
        description="OIDC UserInfo endpoint URL. If set, DeRouter will call this endpoint with the access token to retrieve user identity information.",
    )
    ...
    fallback_to_db_teams: bool = Field(
        ...
        description=(
            ...
            "team specified via the x-derouter-team-id request header (validated "
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        allowed_keys: Final = DeRouter_JWTAuth.__annotations__.keys()
...
class DefaultInternalUserParams(DeRouterPydanticObjectBase):
    """
...
from derouter.models.managed_files import (  # noqa: E402
    DeRouter_ManagedFileTable as DeRouter_ManagedFileTable,
)
from derouter.models.managed_files import (  # noqa: E402
    DeRouter_ManagedObjectTable as DeRouter_ManagedObjectTable,
)
from derouter.models.managed_files import (  # noqa: E402
    DeRouter_ManagedVectorStoresTable as DeRouter_ManagedVectorStoresTable,
)
from derouter.models.managed_files import (  # noqa: E402
    DeRouter_ManagedVectorStoreTable as DeRouter_ManagedVectorStoreTable,
)
...
class ResponseDeRouter_ManagedVectorStore(TypedDict, total=False):
    vector_store: DeRouter_ManagedVectorStoresTable


class CostEstimateRequest(DeRouterPydanticObjectBase):
...
class CostEstimateResponse(DeRouterPydanticObjectBase):
