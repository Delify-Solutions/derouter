...
from derouter.types.utils import CallTypesLiteral, LLMResponseTypes, SpecialEnums
...
def _rewrite_advertised_id(
    event: BaseDeRouterOpenAIResponseObject,
...
class ResponsesIDSecurity(CustomLogger):
    ...
    def _decrypt_response_id(self, response_id: str) -> tuple[str, str | None, str | None]:
        ...
        if decrypted_value.startswith(SpecialEnums.LITELM_MANAGED_FILE_ID_PREFIX.value):
            # Expected format: "derouter_proxy:responses_api:response_id:{response_id};user_id:{user_id}"
            parts: Final = decrypted_value.split(";")

            if len(parts) >= 2:
                # Extract response_id from "derouter_proxy:responses_api:response_id:{response_id}"
    ...
    def _get_signing_key(self) -> str | None:
        ...
        salt_key = os.getenv("DEROUTER_SALT_KEY", None)
        if salt_key is None:
            salt_key = master_key
        return salt_key
    ...
    def _encrypt_response_id(
        self,
        response: BaseDeRouterOpenAIResponseObject,
        user_api_key_dict: "UserAPIKeyAuth",
        ...
        if signing_key is None:
            verbose_proxy_logger.debug(
                ...
                "Please set DEROUTER_SALT_KEY environment variable or configure a master_key. "
                "Skipping response ID encryption. "
                "See: https://router.delify.vn/docs/proxy/prod#5-set-derouter-salt-key"
            )
            return response
        ...
        def encrypt(original_id: str) -> str:
            ...
            managed_id: Final = SpecialEnums.DEROUTER_MANAGED_RESPONSE_API_RESPONSE_ID_COMPLETE_STR.value.format(
                original_id,
    ...
    async def async_post_call_streaming_iterator_hook(
    ...
    ) -> AsyncGenerator[BaseDeRouterOpenAIResponseObject, None]:
        from derouter.proxy.proxy_server import general_settings
        ...
        async for chunk in response:
            if (
                isinstance(chunk, BaseDeRouterOpenAIResponseObject)
