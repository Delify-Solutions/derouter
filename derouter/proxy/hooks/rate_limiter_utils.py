...
from derouter.types.utils import PriorityReservationDict

PROXY_LLM_PROVIDER_FALLBACK: Final = "derouter_proxy"
...
def resolve_llm_provider_for_rate_limit(
    ...
    :func:`derouter.get_llm_provider` is invoked anywhere else in the request
    lifecycle — so the raised 429 would otherwise have an empty
    ``llm_provider`` field, making the resulting Prometheus
    ``derouter_proxy_failed_requests_metric`` show up with
    ``exception_class="RateLimitError"`` and no provider attribution.

    Resolution order:

    1. ``derouter.get_llm_provider(model)`` — covers raw provider/model
    ...
    2. **Router alias fallback** — nearly every real proxy deployment
       ...
       ``"tpm-locked"`` → ``derouter_params.model: openai/gpt-4o-mini``).
       ``get_llm_provider`` doesn't know router aliases, so without this
       step every alias call ended up labeled ``"derouter_proxy"``,
       defeating the field's purpose for the most common case.
    3. Defensive fallback to ``("", "derouter_proxy")`` — used only when
    ...
    try:
        resolved_model, custom_llm_provider, _, _ = derouter.get_llm_provider(
            model=model,
        )
        return (
...
def _resolve_provider_from_router_alias(
    ...
    registered, the deployment has no usable ``derouter_params.model``, or
    any underlying lookup raises. Callers fall through to the defensive
    ``derouter_proxy`` fallback in that case — never raising secondary
    ...
    try:
        ...
        for deployment in model_list:
            ...
            if deployment.get("model_name") != model:
                continue
            params = deployment.get("derouter_params")
