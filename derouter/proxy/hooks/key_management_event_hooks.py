...
from derouter.proxy.utils import _hash_token_if_needed

# NOTE: This is the prefix for all virtual keys stored in AWS Secrets Manager
DEROUTER_PREFIX_STORED_VIRTUAL_KEYS: Final = "derouter/"
...
class KeyManagementEventHooks:
    ...
    async def async_key_deleted_hook(
        data: KeyRequest,
        keys_being_deleted: list[DeRouter_VerificationToken],
        response: dict,
        user_api_key_dict: UserAPIKeyAuth,
        derouter_changed_by: str | None = None,
    ):
    ...
    async def _store_virtual_key_in_secret_manager(secret_name: str, secret_token: str, team_id: str | None = None):
        ...
        """
        if derouter._key_management_settings is not None:
            if derouter._key_management_settings.store_virtual_keys is True:
                from derouter.secret_managers.base_secret_manager import (
                    BaseSecretManager,
                )

                # store the key in the secret manager
                if isinstance(derouter.secret_manager_client, BaseSecretManager):
                    tags: Final = getattr(derouter._key_management_settings, "tags", None)
                    description: Final = getattr(derouter._key_management_settings, "description", None)
                    ...
                    await derouter.secret_manager_client.async_write_secret(
    ...
    async def _rotate_virtual_key_in_secret_manager(
        ...
        if derouter._key_management_settings is not None:
            if derouter._key_management_settings.store_virtual_keys is True:
                ...
                if isinstance(derouter.secret_manager_client, BaseSecretManager):
                    optional_params: Final = await KeyManagementEventHooks._get_secret_manager_optional_params(team_id)
                    await derouter.secret_manager_client.async_rotate_secret(
    ...
    def _get_secret_name(secret_name: str) -> str:
        if derouter._key_management_settings.prefix_for_stored_virtual_keys.endswith("/"):
            return f"{derouter._key_management_settings.prefix_for_stored_virtual_keys}{secret_name}"
        else:
            return f"{derouter._key_management_settings.prefix_for_stored_virtual_keys}/{secret_name}"
                ...
                if isinstance(derouter.secret_manager_client, BaseSecretManager):
                    ...
                    for key in keys_being_deleted:
                        if key.key_alias is not None:
                            ...
                            await derouter.secret_manager_client.async_delete_secret(
    ...
    def _is_email_sending_enabled() -> bool:
        ...
        try:
            ...
            initialized_email_loggers: Final = derouter.logging_callback_manager.get_custom_loggers_for_type(
    ...
    async def _send_key_created_email(response: dict):
        ...
        try:
            ...
            from derouter_enterprise.types.enterprise_callbacks.send_emails import (
        ...
        ##########################
        if "email" in general_settings.get("alerting", []):
            from derouter.proxy._types import WebhookEvent
