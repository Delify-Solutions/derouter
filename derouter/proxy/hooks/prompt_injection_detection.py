...
from derouter.constants import DEFAULT_PROMPT_INJECTION_SIMILARITY_THRESHOLD
...
from derouter.proxy._types import DeRouterPromptInjectionParams, UserAPIKeyAuth
from derouter.router import Router
from derouter.utils import get_formatted_prompt
...
class _OPTIONAL_PromptInjectionDetection(CustomLogger):
    ...
    def __init__(
        self,
        prompt_injection_params: DeRouterPromptInjectionParams | None = None,
    ):
    ...
    async def async_moderation_hook(
        ...
        if (
            ...
            if isinstance(response, derouter.ModelResponse) and isinstance(response.choices[0], derouter.Choices):
