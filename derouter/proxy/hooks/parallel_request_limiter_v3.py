...
from derouter import DualCache
from derouter._logging import verbose_proxy_logger
from derouter.constants import DYNAMIC_RATE_LIMIT_ERROR_THRESHOLD_PER_MINUTE, INTERNAL_CALL_ORIGIN_METADATA_KEY
...
from derouter.proxy.hooks.batch_enqueued_tokens import (
...
from derouter.proxy.hooks.rate_limiter_utils import resolve_llm_provider_for_rate_limit
from derouter.types.caching import RedisPipelineIncrementOperation
from derouter.types.llms.openai import BaseDeRouterOpenAIResponseObject, ResponseAPIUsage
...
# derouter.token_counter has no per-type handling for "input_audio" content
...
class RequestRateLimiterStash:
    ...
    Because the stash is context-inherited, nested DeRouter calls made inside
    the request (LLM-judge guardrails, silent experiments) would also see it
    from their own logging callbacks. ``owner_derouter_call_id`` pins the stash
    to the proxy request's ``derouter_call_id`` so those callbacks can tell the
    ...
    owner_derouter_call_id: str | None = None
...
_request_stash: Final[ContextVar[RequestRateLimiterStash | None]] = ContextVar(
    "derouter_v3_rate_limiter_request_stash", default=None
)
...
def claim_request_stash_for_data(data: dict) -> RequestRateLimiterStash:
    ...
    owner_call_id: Final = data.get("derouter_call_id")
    if isinstance(owner_call_id, str):
        stash.owner_derouter_call_id = owner_call_id
    return stash


def get_request_stash_for_call(derouter_call_id: str | None) -> RequestRateLimiterStash | None:
    stash: Final = _request_stash.get()
    if stash is None:
        return None
    if stash.owner_derouter_call_id is None or derouter_call_id is None:
        return stash
    return stash if derouter_call_id == stash.owner_derouter_call_id else None


def _call_id_from_callback_kwargs(kwargs: object) -> str | None:
    if not isinstance(kwargs, dict):
        return None
    call_id: Final = kwargs.get("derouter_call_id")
...
class _PROXY_MaxParallelRequestsHandler_v3(CustomLogger):
    ...
    def __init__(
        ...
        self.window_size = int(os.getenv("DEROUTER_RATE_LIMIT_WINDOW_SIZE", 60))
        ...
        self.tpm_reservation_enabled = os.getenv("DEROUTER_TPM_TOKEN_RESERVATION_ENABLED", "true").lower() == "true"
    ...
    def _translate_google_genai_native_request(
        ...
        if (
            ...
            return None
        from derouter.google_genai.adapters.transformation import GoogleGenAIAdapter
    ...
    def _add_mcp_per_key_rate_limit_descriptor(
        ...
        """
        from derouter.proxy.auth.auth_utils import get_key_mcp_rpm_limit
    ...
    def _add_mcp_per_team_rate_limit_descriptor(
        ...
        configured for the server being called.
        """
        from derouter.proxy.auth.auth_utils import get_team_mcp_rpm_limit
    ...
    def _get_session_id_from_data(self, data: dict) -> str | None:
        """Extract session_id from request metadata or derouter_session_id."""
        session_id = data.get("derouter_session_id")
        if session_id:
        ...
        session_id = derouter_metadata.get("session_id")
        if session_id:
            return str(session_id)
        return None
    ...
    def _responses_input_to_chat_messages(data: object) -> Sequence[object]:
        ...
        chat-completion-style messages via the standard DeRouter transformation
        ...
        if not isinstance(data, dict):
            return ()
        return DeRouterCompletionResponsesConfig.transform_responses_api_input_to_messages(
    ...
    def _estimate_precise_input_tokens(self, data: object, model: str | None, call_type: str | None = None) -> int:
        ...
        using ``derouter.token_counter`` -- the same approach the
        ...
        """
        from derouter import token_counter
    ...
    def _get_total_tokens_from_usage(
        ...
        rate limits. This aligns DeRouter's TPM calculation with provider behavior.
        """
        total_tokens = 0
        cached_tokens = 0
    ...
    def _response_usage(
        ...
        if isinstance(
            response_obj,
            (ModelResponse, EmbeddingResponse, TextCompletionResponse, BaseDeRouterOpenAIResponseObject),
        ):
    ...
    def _build_success_event_pipeline_operations(
        ...
        # 'metadata' and 'derouter_metadata' fields from derouter_params
        standard_logging_object: Final = kwargs.get("standard_logging_object") or {}
        request_metadata: Final = get_derouter_metadata_from_kwargs(kwargs)
        ...
        # Get total tokens from response. Responses DeRouter does not model
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        derouter_parent_otel_span: Final[Span | None] = _get_parent_otel_span_from_kwargs(kwargs)
        try:
    ...
    async def async_post_call_success_hook(self, data: dict, user_api_key_dict: UserAPIKeyAuth, response):
        ...
        try:
            ...
            stash: Final = get_request_stash()
            derouter_proxy_rate_limit_response: Final = stash.rate_limit_response if stash is not None else None

            if derouter_proxy_rate_limit_response is not None:
                ...
                if _hidden_params is not None and (
                    ...
                    _additional_headers: Final = self._merge_ratelimit_statuses_into_additional_headers(
                        ...
                        statuses=derouter_proxy_rate_limit_response["statuses"],
                    )
    ...
    async def _handle_batch_enqueued_post_call(self, user_api_key_dict: UserAPIKeyAuth, response: object) -> None:
        ...
        if view.status.lower() in BATCH_ENQUEUED_REFUND_STATUSES:
            ...
            if popped is not None:
                await self.batch_enqueued_token_store.refund(reservation=popped, derouter_parent_otel_span=span)
    ...
    async def async_post_call_failure_hook(
        ...
        async_log_failure_event is a derouter completion-level callback and
