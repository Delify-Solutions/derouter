"""
Per-Session Budget Limiter for DeRouter Proxy.

Enforces a dollar-amount cap per session (identified by `session_id` /
`x-derouter-trace-id`). After each successful LLM call the response cost is
accumulated against the session. When the accumulated spend exceeds
`max_budget_per_session` (configured in agent derouter_params), subsequent
...
class _PROXY_MaxBudgetPerSessionHandler(CustomLogger):
    ...
    Configuration (set in agent derouter_params):
    ...
    def __init__(self, internal_usage_cache: InternalUsageCache):
        ...
        self.ttl = int(
            os.getenv(
                "DEROUTER_MAX_BUDGET_PER_SESSION_TTL",
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        try:
            ...
            max_budget: Final = agent_derouter_params.get("max_budget_per_session")
    ...
    def _get_max_budget_per_session(self, user_api_key_dict: UserAPIKeyAuth) -> float | None:
        """Extract max_budget_per_session from agent derouter_params."""
        ...
        max_budget: Final = derouter_params.get("max_budget_per_session")
