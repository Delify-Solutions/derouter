...
import derouter
from derouter import ModelResponse, Router
...
def _get_priority_settings() -> "PriorityReservationSettings":
    ...
    The settings are lazy-loaded in derouter.__init__ and always return an instance.
    This helper provides proper type narrowing for mypy.
    """
    settings: Final = derouter.priority_reservation_settings
    if settings is None:
...
class _PROXY_DynamicRateLimitHandlerV3(CustomLogger):
    ...
    async def _get_saturation_value_from_cache(
        ...
        TTL is configurable via derouter.priority_reservation_settings.saturation_check_cache_ttl
    ...
    def _get_priority_weight(self, priority: str | None, model_info: ModelGroupInfo | None = None) -> float:
        """Get the weight for a given priority from derouter.priority_reservation"""
        weight: float = _get_priority_settings().default_priority
        if derouter.priority_reservation is None or priority not in derouter.priority_reservation:
            verbose_proxy_logger.debug("Priority Reservation not set for the given priority.")
        elif priority is not None and derouter.priority_reservation is not None:
            if os.getenv("DEROUTER_LICENSE", None) is None:
                verbose_proxy_logger.error(
                    "PREMIUM FEATURE: Reserving tpm/rpm by priority is a premium feature. Please add a 'DEROUTER_LICENSE' to your .env to enable this.\nGet a license: https://router.delify.vn/docs/proxy/enterprise."
                )
            else:
                value: Final = derouter.priority_reservation[priority]
    ...
    def _normalize_priority_weights(self, model_info: ModelGroupInfo) -> dict[str, float]:
        ...
        """
        if derouter.priority_reservation is None:
            return {}

        # Convert all values to percentages first
        weights: Final[dict[str, float]] = {}
        for k, v in derouter.priority_reservation.items():
    ...
    def _get_priority_allocation(
        ...
        """
        # Check if this key has an explicit priority in derouter.priority_reservation
        has_explicit_priority: Final = (
            priority is not None
            and derouter.priority_reservation is not None
            and priority in derouter.priority_reservation
        )
                ...
                if descriptor_key == "model_saturation_check":
                    raise ProxyRateLimitError(
                        ...
                        headers={
                            ...
                            "x-derouter-priority": priority or "default",
                        },
                ...
                if descriptor_key == "priority_model":
                    ...
                    raise ProxyRateLimitError(
                        ...
                        headers={
                            ...
                            "rate_limit_type": str(status["rate_limit_type"]),
                            "x-derouter-priority": priority or "default",
                            "x-derouter-saturation": f"{saturation:.2%}",
                        },
                        rate_limit_type=map_v3_rate_limit_type(status["rate_limit_type"]),
                        model=resolved_model,
                        llm_provider=llm_provider,
    ...
    async def async_post_call_success_hook(self, data: dict, user_api_key_dict: UserAPIKeyAuth, response):
        ...
        try:
            ...
            if isinstance(response, ModelResponse):
                ...
                additional_headers["x-derouter-priority"] = priority or "default"
                additional_headers["x-derouter-rate-limiter-version"] = "v3"
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        try:
            ...
            derouter_parent_otel_span: Final = _get_parent_otel_span_from_kwargs(kwargs)
            ...
            has_explicit_priority: Final = (
                ...
                and derouter.priority_reservation is not None
                and key_priority in derouter.priority_reservation
            )
