...
from derouter.constants import BATCH_ENQUEUED_TOKEN_LIMIT_METADATA_KEY, BATCH_ENQUEUED_TOKEN_TTL_SECONDS
...
class BatchEnqueuedTokenStore:
    ...
    async def reserve(
        ...
        return await self._reserve_in_memory(tokens=tokens, scopes=scopes, span=derouter_parent_otel_span)
    ...
    async def refund(
        ...
        async with self._lock:
            for scope in reservation.scopes:
                current = await self._get_local_counter(scope, derouter_parent_otel_span)
                ...
                if remaining <= 0:
                    ...
                    await self._set_local_counter(scope, remaining, derouter_parent_otel_span)
    ...
    async def pop_reservation(
        ...
        if redis_raw is not None and not redis_raw:
            ...
            await self._pop_local_record(batch_id, derouter_parent_otel_span)
            return None
        raw: Final = (
            redis_raw if redis_raw is not None else await self._pop_local_record(batch_id, derouter_parent_otel_span)
        )
        if raw is None:
            return None
        try:
