    ...
    async def async_set_cache_sadd(self, model: str, value: list):
        ...
        try:
            ...
            verbose_proxy_logger.exception(
                "derouter.proxy.hooks.dynamic_rate_limiter.py::async_set_cache_sadd(): Exception occured - %s", e
            )
            raise e
        ...
        try:
            ...
            if derouter.priority_reservation is None or priority not in derouter.priority_reservation:
                verbose_proxy_logger.error(
                    "Priority Reservation not set. priority=%s, but derouter.priority_reservation is %s.",
                    priority,
                    derouter.priority_reservation,
                )
            ...
            verbose_proxy_logger.exception(
                "derouter.proxy.hooks.dynamic_rate_limiter.py::check_available_usage: Exception occurred - %s", e
            )
        ...
        try:
            if isinstance(response, ModelResponse):
                ...
                response._hidden_params["additional_headers"] = {  # Add additional response headers - easier debugging
                    "x-derouter-model_group": model_info["model_name"],
                    "x-ratelimit-remaining-derouter-project-tokens": available_tpm,
                    "x-ratelimit-remaining-derouter-project-requests": available_rpm,
            ...
            verbose_proxy_logger.exception(
                "derouter.proxy.hooks.dynamic_rate_limiter.py::async_post_call_success_hook(): Exception occured - %s", e
            )
            return response
