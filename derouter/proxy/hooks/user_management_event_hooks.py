"""
Hooks that are triggered when a derouter user event occurs
"""
...
class UserManagementEventHooks:
    ...
    async def async_user_created_hook(
        ...
        """
        This hook is called when a new user is created on derouter
    ...
    async def async_send_user_invitation_email(
        ...
        event: Final = WebhookEvent(
            ...
            event_message="Welcome to DeRouter Proxy",
    ...
    async def _send_v2_user_invitation_emails(event: WebhookEvent, send_invite_email: bool | None) -> bool:
        ...
        email_loggers: Final = tuple(
            email_logger
            for email_logger in derouter.logging_callback_manager.get_custom_loggers_for_type(
    ...
    async def send_legacy_v1_user_invitation_email(
        ...
        if "email" not in general_settings.get("alerting", []):
            raise ValueError(
                "Email alerting not setup on config.yaml. Please set `alerting=['email']. \nDocs: https://router.delify.vn/docs/proxy/email`"
            )
