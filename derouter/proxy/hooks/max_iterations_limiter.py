"""
Max Iterations Limiter for DeRouter Proxy.

Enforces a per-session cap on the number of LLM calls an agentic loop can make.
Callers send a `session_id` with each request (via `x-derouter-session-id` header
or `metadata.session_id`), and this hook counts calls per session. When the count
exceeds `max_iterations` (configured in agent derouter_params or key metadata), returns 429.
...
class _PROXY_MaxIterationsHandler(CustomLogger):
    ...
    Configuration:
        - max_iterations: set in agent derouter_params (preferred)
          e.g. derouter_params={"max_iterations": 25}
          Falls back to key metadata max_iterations for backwards compatibility.
        - session_id: sent by caller via x-derouter-session-id header or
    ...
    def __init__(self, internal_usage_cache: InternalUsageCache):
        ...
        self.ttl = int(os.getenv("DEROUTER_MAX_ITERATIONS_TTL", DEFAULT_MAX_ITERATIONS_TTL))
    ...
    async def async_pre_call_hook(
        ...
        agent derouter_params. If the session has exceeded max_iterations, raises 429.
        """
    ...
    def _get_session_id(self, data: dict) -> str | None:
        ...
        # Also check derouter_metadata (used for /thread and /assistant endpoints)
    ...
    def _get_max_iterations(self, user_api_key_dict: UserAPIKeyAuth) -> int | None:
        """Extract max_iterations from agent derouter_params, with fallback to key metadata."""
        # Try agent derouter_params first
        ...
        if agent_id is not None:
            ...
            if agent is not None:
                derouter_params: Final = agent.derouter_params or {}
                max_iterations = derouter_params.get("max_iterations")
