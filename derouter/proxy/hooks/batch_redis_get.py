...
class _PROXY_BatchRedisRequests(CustomLogger):
    ...
    def __init__(self):
        if derouter.cache is not None:
            ...
            )  # map the derouter 'get_cache' function to our custom function
    ...
    async def async_pre_call_hook(
        ...
        try:
            ...
            Check if a key starting with `derouter:<api_key>:<call_type:` exists in-memory

            If no, then get relevant cache from redis
            """
            api_key: Final = user_api_key_dict.api_key

            cache_key_name: Final = f"derouter:{api_key}:{call_type}"
            ...
            if in_memory_cache_exists is False and derouter.cache is not None:
                """
                - Check if `derouter.Cache` is redis
                - Get the relevant values
                """
                if derouter.cache.type is not None and isinstance(derouter.cache.cache, RedisCache):
                    # Initialize an empty list to store the keys
                    keys = []
                    self.print_verbose(f"cache_key_name: {cache_key_name}")
                    # Use the SCAN iterator to fetch keys matching the pattern
                    keys = await derouter.cache.cache.async_scan_iter(pattern=cache_key_name, count=100)
                    ...
                    if len(keys) > 0:
                        key_value_dict = await derouter.cache.cache.async_batch_get_cache(key_list=keys)
            ...
            verbose_proxy_logger.error(
                "derouter.proxy.hooks.batch_redis_get.py::async_pre_call_hook(): Exception occured - %s", e
            )
    ...
    async def async_get_cache(self, *args, **kwargs):
        ...
        try:  # never block execution
            ...
            elif derouter.cache is not None:
            ...
            if cache_key is not None and self.in_memory_cache is not None and derouter.cache is not None:
                ...
                if cached_result is None:
                    cached_result = await derouter.cache.cache.async_get_cache(cache_key, *args, **kwargs)
                    if cached_result is not None:
                        await self.in_memory_cache.async_set_cache(cache_key, cached_result, ttl=60)
                return derouter.cache._get_cache_logic(cached_result=cached_result, max_age=max_age)
        except Exception:
            return None
