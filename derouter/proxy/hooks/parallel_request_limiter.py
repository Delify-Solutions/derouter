...
import derouter
from derouter import DualCache, EmbeddingResponse, ModelResponse, TextCompletionResponse
from derouter._logging import verbose_proxy_logger
from derouter.exceptions import RateLimitType
...
from derouter.proxy._types import CommonProxyErrors, CurrentItemRateLimit, UserAPIKeyAuth
...
class _PROXY_MaxParallelRequestsHandler(CustomLogger):
    ...
    def raise_rate_limit_error(
        ...
        """
        Raise a 429 with a retry-after header for derouter-proxy parallel-request limits.
        ...
        :class:`derouter.RateLimitError` (so callers can catch by category) and a
        ...
        callbacks). Falls back to ``llm_provider="derouter_proxy"`` when the
    ...
    async def async_pre_call_hook(
        ...
        if _tpm_limit_for_key_model is not None or _rpm_limit_for_key_model is not None:
            ...
            _remaining_limits_data: Final = {
                f"derouter-key-remaining-tokens-{_model}": _remaining_tokens,
                f"derouter-key-remaining-requests-{_model}": _remaining_requests,
            }
    ...
    async def async_log_success_event(self, kwargs, response_obj: object, start_time, end_time):
        ...
        derouter_parent_otel_span: Final[Span | None] = _get_parent_otel_span_from_kwargs(kwargs=kwargs)
        try:
            self.print_verbose("INSIDE parallel request limiter ASYNC SUCCESS LOGGING")

            global_max_parallel_requests: Final = kwargs["derouter_params"]["metadata"].get(
                "global_max_parallel_requests", None
            )
            user_api_key: Final = kwargs["derouter_params"]["metadata"]["user_api_key"]
            user_api_key_user_id: Final = kwargs["derouter_params"]["metadata"].get("user_api_key_user_id", None)
            user_api_key_team_id: Final = kwargs["derouter_params"]["metadata"].get("user_api_key_team_id", None)
            user_api_key_model_max_budget: Final = kwargs["derouter_params"]["metadata"].get(
                "user_api_key_model_max_budget", None
            )
            user_api_key_end_user_id: Final = kwargs.get("user")

            user_api_key_metadata: Final = kwargs["derouter_params"]["metadata"].get("user_api_key_metadata", {}) or {}
            user_api_key_team_metadata = kwargs["derouter_params"]["metadata"].get("user_api_key_team_metadata", None)
    ...
    async def async_log_failure_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            derouter_parent_otel_span: Final[Span | None] = _get_parent_otel_span_from_kwargs(kwargs=kwargs)
            _metadata: Final = kwargs["derouter_params"].get("metadata", {}) or {}
    ...
    async def get_internal_user_object(
        ...
        It uses the `get_user_object` function from `derouter.proxy.auth.auth_checks`
