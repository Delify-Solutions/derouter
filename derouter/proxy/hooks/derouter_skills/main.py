"""
Skills Injection Hook for DeRouter Proxy

Main hook that orchestrates skill processing:
- Fetches skills from DeRouter DB
- Injects SKILL.md content into system prompt
- Adds derouter_code_execution tool for automatic code execution
- Handles agentic loop internally when derouter_code_execution is called
...
- derouter_code_execution tool is added - when model calls it, DeRouter handles
  execution automatically and returns final response with file_ids

Usage:
    # Simple - DeRouter handles everything automatically via proxy
    ...
    response = await derouter.acompletion(
        ...
        container={"skills": [{"skill_id": "derouter_skill_abc123"}]},
    )
...
from derouter.llms.derouter_proxy.skills.constants import DEROUTER_SKILL_ID_PREFIX
from derouter.llms.derouter_proxy.skills.prompt_injection import (
    SkillPromptInjectionHandler,
)
from derouter.proxy._types import DeRouter_SkillsTable, UserAPIKeyAuth
from derouter.types.utils import CallTypes, CallTypesLiteral, LLMResponseTypes
...
class SkillsInjectionHook(CustomLogger):
    ...
    - Skills with 'derouter_skill_' prefix are fetched from DeRouter DB
    - For Anthropic models: native skills pass through, DeRouter skills converted to tools
    - For non-Anthropic models: DeRouter skills are converted to tools + execute_code tool

    Post-call (async_post_call_success_deployment_hook):
    - If response has derouter_code_execution tool call, automatically execute code
    - Continue conversation loop until model gives final response
    - Return response with generated files inline

    This hook is called automatically by derouter during completion calls.
    """
    ...
    async def async_pre_call_hook(
        ...
        2. Separate skills by prefix (derouter_skill_ vs native)
        3. Fetch DeRouter skills from database
        4. For Anthropic: keep native skills in container
        5. For non-Anthropic: convert DeRouter skills to tools, inject content, add execute_code
        """
        ...
        derouter_skills: Final[list[DeRouter_SkillsTable]] = []
        ...
        for skill in skills:
            ...
            if skill_id.startswith(DEROUTER_SKILL_ID_PREFIX):
                # Fetch from DeRouter DB
                db_skill = await self._fetch_skill_from_db(
                    skill_id,
                    user_api_key_dict=user_api_key_dict,
                )
                if db_skill:
                    derouter_skills.append(db_skill)
                else:
                    verbose_proxy_logger.warning("SkillsInjectionHook: Skill '%s' not found in DeRouter DB", skill_id)
            else:
        ...
        if len(derouter_skills) > 0:
            data = self._process_for_messages_api(
                data=data,
                derouter_skills=derouter_skills,
    ...
    def _process_for_messages_api(
        self,
        data: dict,
        derouter_skills: list[DeRouter_SkillsTable],
        ...
        # Add derouter_code_execution tool if we have skill files
        if all_skill_files:
            code_exec_tool: Final = get_derouter_code_execution_tool_anthropic()
            data["tools"] = data.get("tools", []) + [code_exec_tool]

            # Store skill files in derouter_metadata for automatic code execution
            data["derouter_metadata"] = data.get("derouter_metadata", {})
            data["derouter_metadata"]["_skill_files"] = all_skill_files
            data["derouter_metadata"]["_derouter_code_execution_enabled"] = True

        # Remove container (not supported by underlying providers)
        data.pop("container", None)

        verbose_proxy_logger.debug(
            "SkillsInjectionHook: Messages API - converted %s skills to Anthropic tools, injected %s skill contents, added derouter_code_execution tool with %s modules",
            len(derouter_skills),
            len(skill_contents),
    ...
    def _process_non_anthropic_model(
        ...
        if all_skill_files:
            ...
            data["tools"] = data.get("tools", []) + [get_derouter_code_execution_tool()]

            # Store skill files in derouter_metadata for automatic code execution
            # Using derouter_metadata instead of metadata to avoid conflicts with user metadata
            data["derouter_metadata"] = data.get("derouter_metadata", {})
            data["derouter_metadata"]["_skill_files"] = all_skill_files
            data["derouter_metadata"]["_derouter_code_execution_enabled"] = True
    ...
    async def _fetch_skill_from_db(
        ...
        """
        Fetch a skill from the DeRouter database.

        Args:
            skill_id: The skill ID (including the 'derouter_skill_' prefix)

        Returns:
            DeRouter_SkillsTable or None if not found
        """
        try:
            from derouter.llms.derouter_proxy.skills.handler import DeRouterSkillsHandler

            return await DeRouterSkillsHandler.fetch_skill_from_db(
                skill_id,
                user_api_key_dict=user_api_key_dict,
            )
    ...
    async def async_post_call_success_deployment_hook(
        ...
        If the response contains a tool call (derouter_code_execution or skill tool):
        ...
        derouter_metadata: Final = request_data.get("derouter_metadata") or {}
        metadata: Final = request_data.get("metadata") or {}

        code_exec_enabled: Final = derouter_metadata.get("_derouter_code_execution_enabled") or metadata.get(
            "_derouter_code_execution_enabled"
        )
        if not code_exec_enabled:
            return None

        # Get skill files
        skill_files_by_id: Final = derouter_metadata.get("_skill_files") or metadata.get("_skill_files", {})
        ...
        # Check if any tool call needs execution (derouter_code_execution or skill tool)
        has_executable_tool = False
        for tc in tool_calls:
            tool_name: str = tc.get("name", "")
            # Execute if it's derouter_code_execution OR a skill tool (derouter_skill_xxx)
            if tool_name == DeRouterInternalTools.CODE_EXECUTION.value or tool_name.startswith(DEROUTER_SKILL_ID_PREFIX):
    ...
    async def _execute_code_loop_messages_api(
        ...
        from derouter.llms.derouter_proxy.skills.sandbox_executor import (
        ...
        for iteration in range(self.max_iterations):
            ...
            for tc in tool_calls:
                ...
                # Execute if it's derouter_code_execution OR a skill tool
                if tool_name == DeRouterInternalTools.CODE_EXECUTION.value:
                    code = tool_input.get("code", "")
                    result = await self._execute_code(code, skill_files, executor, generated_files)
                elif tool_name.startswith(DEROUTER_SKILL_ID_PREFIX):
            ...
            try:
                current_response = await derouter.anthropic.acreate(
                    model=model,
                    messages=messages,
                    tools=tools,
...
"""
    ...
    async def _execute_code_loop(
        ...
        # These are either handled explicitly or are internal DeRouter fields
    ...
    async def _execute_code_tool(
        ...
        """Execute a derouter_code_execution tool call and return result string."""
        try:
    ...
    def _attach_files_to_response(
        ...
        Files are added to response._derouter_generated_files for easy access.
        ...
        if isinstance(response, dict):
            response["_derouter_generated_files"] = generated_files
        ...
        if hasattr(response, "model_extra"):
            ...
            response.model_extra["_derouter_generated_files"] = generated_files
