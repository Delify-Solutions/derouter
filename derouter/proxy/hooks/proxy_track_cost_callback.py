...
from derouter.batches.batch_utils import batch_cost_is_final
from derouter.constants import BACKGROUND_INTERACTION_COST_POLLING_ENABLED
...
from derouter.proxy.db.db_spend_update_writer import (
    debitable_model_access_groups,
    get_llm_router,
)
from derouter.proxy.derouter_pre_call_utils import DeRouterProxyRequestSetup
from derouter.proxy.spend_tracking.spend_log_error_logger import (
...
_UNATTRIBUTED_TRACKABLE_CALL_TYPES: Final[frozenset[str]] = frozenset(
    {
        ...
        # whatever DeRouter_ManagedObjectTable stored at create time, and all of it is
...
class _ProxyDBLogger(CustomLogger):
    ...
    async def async_post_call_failure_hook(
        ...
        if (
            isinstance(derouter_metadata_bucket, dict)
            and "standard_logging_guardrail_information" not in existing_metadata
        ):
            guardrail_info: Final = derouter_metadata_bucket.get("standard_logging_guardrail_information")
            if guardrail_info is not None:
                existing_metadata["standard_logging_guardrail_information"] = guardrail_info

        if "derouter_params" not in request_data:
            request_data["derouter_params"] = {}

        existing_derouter_params: Final = request_data.get("derouter_params", {})
        existing_derouter_metadata: Final = existing_derouter_params.get("metadata", {}) or {}

        # Preserve tags from existing metadata
        if existing_derouter_metadata.get("tags"):
            existing_metadata["tags"] = existing_derouter_metadata.get("tags")

        request_data["derouter_params"]["proxy_server_request"] = (
            request_data.get("proxy_server_request") or existing_derouter_params.get("proxy_server_request") or {}
        )
        request_data["derouter_params"]["metadata"] = existing_metadata

        # Preserve model name and custom_llm_provider
        if "model" not in request_data:
            request_data["model"] = existing_derouter_params.get("model") or request_data.get("model", "")
        if "custom_llm_provider" not in request_data:
            request_data["custom_llm_provider"] = existing_derouter_params.get(
                "custom_llm_provider"
            ) or request_data.get("custom_llm_provider", "")

        # Propagate standard_logging_object and derouter_trace_id from the Logging
        # instance so the failure row carries the same trace_id Langfuse received.
        _derouter_logging_obj: Final = request_data.get("derouter_logging_obj")
        if _derouter_logging_obj is not None:
            if not request_data.get("standard_logging_object"):
                request_data["standard_logging_object"] = getattr(_derouter_logging_obj, "model_call_details", {}).get(
                    "standard_logging_object"
                )
            if request_data.get("derouter_trace_id") is None:
                request_data["derouter_trace_id"] = getattr(_derouter_logging_obj, "derouter_trace_id", None)
        ...
        if _derouter_logging_obj is not None:
            obj_start: Final = getattr(_derouter_logging_obj, "start_time", None)
        ...
        recovered_stream_cost: Final = (
            ...
            if isinstance(request_data.get("combined_usage_object"), derouter.Usage)
            else 0.0
        )
    ...
    async def _PROXY_track_cost_callback(
        ...
        completion_response: derouter.ModelResponse | Any | None,  # response from completion
        start_time=None,
        ...
        try:
            ...
            end_user_id: Final = get_end_user_id_for_cost_tracking(derouter_params)
            metadata = get_derouter_metadata_from_kwargs(kwargs=kwargs)
            ...
            if (
                isinstance(completion_response, DeRouterBatch)
            ...
            if response_cost is not None:
                ...
                if kwargs.get("stream") is not True or (
                    ...
                    raise Exception(
                        f"Cost tracking failed for model={model}.\nDebug info - {cost_tracking_failure_debug_info}\nAdd custom pricing - https://router.delify.vn/docs/proxy/custom_pricing"
                    )
        except Exception as e:
            error_msg = f"Error in tracking cost callback - {e}\n Traceback:{traceback.format_exc()}"
            model = kwargs.get("model", "")
            metadata = get_derouter_metadata_from_kwargs(kwargs=kwargs)
            derouter_metadata: Final = kwargs.get("derouter_params", {}).get("derouter_metadata", {})
            old_metadata: Final = kwargs.get("derouter_params", {}).get("metadata", {})
    ...
    async def _enrich_failure_metadata_with_key_info(metadata: dict, resolve_missing_key_identity: bool = True) -> dict:
        ...
        2. Post-auth failures (provider errors, rate limits): key fields are populated
           but team_alias is missing because DeRouter_VerificationTokenView SQL view
...
def _write_spend_metadata_to_kwargs(kwargs: dict, metadata: dict) -> None:
    ...
    derouter_params: Final = kwargs.setdefault("derouter_params", {})
    for bucket_name in ("derouter_metadata", "metadata"):
        bucket = derouter_params.get(bucket_name)
...
def _is_unbilled_interaction_response(completion_response: object) -> bool:
    from derouter.interactions.background_cost_polling import missing_usage_is_expected
...
def _is_unbilled_in_progress_interaction(completion_response: object) -> bool:
    from derouter.interactions.background_cost_polling import is_pollable_background_interaction
    from derouter.types.interactions import InteractionsAPIResponse
...
async def _update_database_and_spend_counters(
    ...
    org_id: str | None,
    kwargs: dict,
    completion_response: derouter.ModelResponse | Any | None,
    start_time: Any,
    end_time: Any,
    response_cost: float,
