...
from derouter.types.llms.openai import DeRouterFineTuningJobCreate
        ...
        if isinstance(element, dict):
            for key, value in element.items():
                if isinstance(value, str) and value.startswith("os.environ/"):
                    element[key] = derouter.get_secret(value)
...
async def create_fine_tuning_job(
    ...
    fine_tuning_request: DeRouterFineTuningJobCreate,
    ...
    try:
        ...
        response: DeRouterFineTuningJob | None = None
        if training_file:
        ...
        if unified_file_id:
            ...
            response = cast(DeRouterFineTuningJob, await llm_router.acreate_fine_tuning_job(**data))
        ...
        ## ELSE, Route based on custom_llm_provider
            ...
            response = await derouter.acreate_fine_tuning_job(**data)

        if response is None:
            raise ValueError("Invalid request, No derouter managed file id or custom_llm_provider provided.")
        ...
        if _response is not None and isinstance(_response, DeRouterFineTuningJob):
            response = _response
        ...
        verbose_proxy_logger.exception(
            "derouter.proxy.proxy_server.create_fine_tuning_job(): Exception occurred - %s", e
        )
...
async def retrieve_fine_tuning_job(
    ...
    - `custom_llm_provider`: Name of the DeRouter provider
    ...
    try:
        ...
        if unified_finetuning_job_id:
            ...
            response = await derouter.aretrieve_fine_tuning_job(
                **data,
            )
        ...
        if response is None:
            raise HTTPException(
                status_code=400,
                detail="Invalid request, No derouter managed file id or custom_llm_provider provided.",
            )
        ...
        verbose_proxy_logger.exception(
            "derouter.proxy.proxy_server.retrieve_fine_tuning_job(): Exception occurred - %s", e
        )
        raise handle_exception_on_proxy(e)
...
async def list_fine_tuning_jobs(
    ...
    try:
        ...
        if target_model_names and isinstance(target_model_names, str):
            ...
            response = await derouter.alist_fine_tuning_jobs(
                **data,
                after=after,
                limit=limit,
            )
        ...
        verbose_proxy_logger.exception("derouter.proxy.proxy_server.list_fine_tuning_jobs(): Exception occurred - %s", e)
        raise handle_exception_on_proxy(e)
...
async def cancel_fine_tuning_job(
    ...
    try:
        ...
        if unified_finetuning_job_id:
            ...
            response = await derouter.acancel_fine_tuning_job(
                **data,
            )
        ...
        verbose_proxy_logger.exception(
            "derouter.proxy.proxy_server.cancel_fine_tuning_job(): Exception occurred - %s", e
        )
        raise handle_exception_on_proxy(e)
