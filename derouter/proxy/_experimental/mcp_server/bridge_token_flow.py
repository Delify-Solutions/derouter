"""Bridge token flow: derouter identity resolution and the DCR-bridge oauth_delegate mint/refresh pipeline."""
...
from derouter.proxy._experimental.mcp_server.oauth_utils import TOKEN_NO_CACHE_HEADERS
from derouter.types.mcp_server.mcp_server_manager import MCPServer

if TYPE_CHECKING:
    from derouter.models.user import DeRouter_UserTable
    from derouter.proxy._experimental.mcp_server.discoverable_endpoints import _BridgeAuthorizationCode
...
def _derouter_key_from_request(request: Request) -> str | None:
    """Return the DeRouter API key presented on the request, or ``None``.

    Accepts the key from ``x-derouter-api-key`` (what MCP clients such as Claude Desktop/Code
    send) as well as ``Authorization``; either may carry a bare token or ``Bearer <token>``.
    ``x-derouter-api-key`` wins when both are present, since ``Authorization`` may instead carry
    an OAuth/upstream bearer.
    """
    for header_value in (
        request.headers.get("x-derouter-api-key"),
...
class _ResolvedKey:
    """An active derouter key resolved from the token request: its hash (the value ``get_key_object``
...
"""Why a token request yielded no active derouter key, kept distinct so a caller statuses each truthfully
...
def _database_failure(exc: Exception) -> Literal["unavailable", "faulted"]:
    from derouter.proxy.db.exception_handler import (  # noqa: PLC0415  # inline import avoids a module-load circular import
...
async def _resolve_active_derouter_key(request: Request) -> "_ResolvedKey | _KeyResolutionFailure":
    """Resolve the presented derouter key to an active key record, or say precisely why not.
    ...
    gateway fault."""
    token: Final = _derouter_key_from_request(request)
    if not token:
        return "no_active_key"
    from derouter.proxy._types import hash_token  # noqa: PLC0415  # inline import avoids a module-load circular import
...
async def _reload_active_key_by_hash(key_hash: str) -> "_ResolvedKey | _KeyResolutionFailure":
    ...
    resolution (:func:`_resolve_active_derouter_key`, which hashes the presented key) and the refresh
    ...
    from derouter.proxy.auth.auth_checks import (  # noqa: PLC0415  # inline import avoids a module-load circular import
        get_key_object,
    )
    from derouter.proxy.db.exception_handler import (  # noqa: PLC0415  # inline import avoids a module-load circular import
        PrismaDBExceptionHandler,
    )
    from derouter.proxy.proxy_server import (  # noqa: PLC0415  # inline import avoids a module-load circular import
        prisma_client,
        user_api_key_cache,
    )
...
async def load_active_user_by_id(user_id: str) -> "DeRouter_UserTable | _KeyResolutionFailure":
    """Load a live derouter user by id, returning the record when the user is active or a precise
...
async def _extract_user_id_from_request(request: Request) -> str | None:
    """The derouter ``user_id`` for the token request, so a per-user token is stored under the same
    ...
    :func:`_resolve_active_derouter_key` directly."""
    resolved: Final = await _resolve_active_derouter_key(request)
...
def _bridge_grant_from_token_response(token_response: object) -> "UpstreamTokenGrant | _UpstreamGrantRejection":
    ...
    the cap."""
    from derouter.proxy._experimental.mcp_server.outbound_credentials.envelope import (  # noqa: PLC0415  # inline import avoids a module-load circular import
        UpstreamTokenGrant,
    )
...
class _BridgeMintReady:
    ...
    two-header client (resolved from the derouter key it presents) or a user_id subject for the
    ...
    match error:
        case "no_identity":
            status, code, desc = (
                ...
                "send a derouter credential (x-derouter-api-key or Authorization) on the token request",
            )
        ...
        case "identity_unresolvable":
            status, code, desc = (
                500,
                "server_error",
                "the gateway could not resolve the derouter identity for this request",
            )
        case "not_configured":
            status, code, desc = (
                500,
...
async def _prepare_bridge_mint(
    ...
    mint (master_key set), resolve the derouter identity, and derive the envelope keys. Returns a ready
    ...
    authorization code) and mints a user subject. The scripted two-header client presents a derouter key
    ...
    from derouter.proxy._experimental.mcp_server.outbound_credentials.bridge_credentials import (  # noqa: PLC0415  # inline import avoids a module-load circular import
    ...
    if bridge_identity is not None:
        identity = user_identity(server_id=mcp_server.server_id, user_id=bridge_identity.derouter_user_id)
...
async def _prepare_bridge_refresh(
    ...
    envelope, re-validate the sealed derouter identity so a revoked key cannot keep refreshing, and
