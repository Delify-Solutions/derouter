...
from derouter.proxy.auth.user_api_key_auth import (
    _get_bearer_token_or_received_api_key,  # pyright: ignore[reportPrivateUsage]  # shared x-derouter-api-key parser lives with user_api_key_auth
    _run_centralized_common_checks,
    user_api_key_auth,
)
from derouter.proxy.common_utils.http_parsing_utils import _read_request_body
...
def _is_derouter_auth_admission_error(exc: Exception) -> bool:
...
def _is_gateway_dcr_challenge_scope(
    ...
    if not _is_derouter_auth_admission_error(exc):
        return False
...
def _admission_failure_fallback(
    ...
    """Map a failed DeRouter admission to its anonymous fallback or challenge.
    ...
    if (
        ...
        and _is_derouter_auth_admission_error(exc)
...
class MCPRequestHandler:
    ...
    1. Authentication via DeRouter API keys
    ...
    DEROUTER_API_KEY_HEADER_NAME_PRIMARY = SpecialHeaders.custom_derouter_api_key.value
    DEROUTER_API_KEY_HEADER_NAME_SECONDARY = SpecialHeaders.openai_authorization.value

    # This is the header to use if you want DeRouter to use this header for authenticating to the MCP server
    DEROUTER_MCP_AUTH_HEADER_NAME = SpecialHeaders.mcp_auth.value

    DEROUTER_MCP_SERVERS_HEADER_NAME = SpecialHeaders.mcp_servers.value

    DEROUTER_MCP_ACCESS_GROUPS_HEADER_NAME = SpecialHeaders.mcp_access_groups.value
    ...
    async def process_mcp_request(
        ...
        # Check if there is an explicit DeRouter API key (primary header)
        has_explicit_derouter_key: Final = headers.get(MCPRequestHandler.DEROUTER_API_KEY_HEADER_NAME_PRIMARY) is not None

        derouter_api_key: Final = MCPRequestHandler.get_derouter_api_key_from_headers(headers) or ""
        ...
        mcp_servers_header: Final = headers.get(MCPRequestHandler.DEROUTER_MCP_SERVERS_HEADER_NAME)
        ...
        from derouter.proxy.auth.auth_utils import (  # noqa: PLC0415
            get_request_route,
        )
        ...
        if request_route.startswith("/.well-known/"):
        ...
        elif has_explicit_derouter_key:
            # An explicit x-derouter-api-key is always a DeRouter credential, even
            ...
            validated_user_api_key_auth = await user_api_key_auth(
                api_key=f"Bearer {_get_bearer_token_or_received_api_key(derouter_api_key)}",
                request=request,
            )
            ...
            # Authorization bearer is an upstream token, never a DeRouter key. Skip
            # DeRouter validation entirely — covering both the no-credential
            ...
            (
                ...
                derouter_api_key=derouter_api_key,
                mcp_server_auth_headers=mcp_server_auth_headers,
                request=request,
                route=request_route,
            )
            ...
            # token and admit under the live derouter user; downstream grant resolution
            ...
            # DeRouter credential, so a failed validation is a genuine 401/403 and
            # propagates unless a fallback in _admission_failure_fallback applies.
            try:
                validated_user_api_key_auth = await user_api_key_auth(api_key=derouter_api_key, request=request)
    ...
    def extract_target_server_names_from_path(path: str) -> list[str]:
        ...
        as targeting the delegate server (bypassing DeRouter auth) while
    ...
    def _target_servers_delegate_auth_to_upstream(
        ...
        Used by :meth:`process_mcp_request` to skip DeRouter API-key/SSO auth
        ...
        one non-delegated server) fall back to normal DeRouter auth.
        """
        ...
        # targets a non-delegate server, skipping DeRouter auth for it.
        ...
        for name in target_names:
            ...
            if getattr(server, "delegate_auth_to_upstream", False) is not True:
                return False
            # Never delegate for M2M (client_credentials) servers: DeRouter
            ...
            # tools authenticated as DeRouter's service account.
            #
    ...
    def _target_servers_are_true_passthrough(path: str, mcp_servers: list[str] | None, client_ip: str | None) -> bool:
        ...
        Used by :meth:`process_mcp_request` to skip DeRouter admission auth entirely: the gateway is a
        transparent proxy and the caller's ``Authorization`` is an upstream token, never a DeRouter key.
    ...
    async def _admit_dcr_bridge_authorization(
        ...
        try:
            admitted: Final = await user_api_key_auth(api_key=derouter_api_key, request=request)
        except (HTTPException, ProxyException) as exc:
            if not _is_derouter_auth_admission_error(exc):
                raise
            raise MCPRequestHandler._dcr_bridge_invalid_token_challenge(
    ...
    async def _admit_gateway_session(
        ...
        """Open a gateway DCR session bearer and admit the live derouter user it references.
    ...
    async def _run_pre_db_read_auth_checks(request: Request, route: str) -> None:
        ...
        from derouter.proxy.auth.auth_utils import pre_db_read_auth_checks
    ...
    async def _reload_admitted_principal(identity: EnvelopeIdentity) -> UserAPIKeyAuth:
        """Reload the live derouter record the envelope's subject references.

        Dispatches on the sealed subject type: a ``key_hash`` reloads the virtual key that
        minted the envelope (the scripted two-header client that presents a derouter key at the
    ...
    async def reload_admitted_user(user_id: str) -> UserAPIKeyAuth:
        ...
        from derouter.proxy.auth.auth_checks import get_object_permission, get_user_object
        ...
        try:
            ...
            # get_allowed_mcp_servers can grant the user their derouter-granted servers. Reuses the same
    ...
    def _get_mcp_auth_header_from_headers(headers: Headers) -> str | None:
        """
        Get the header passed to DeRouter to pass to downstream MCP servers

        By default derouter will check for the header `x-mcp-auth` by setting one of the following:
            1. `DEROUTER_MCP_CLIENT_SIDE_AUTH_HEADER_NAME` as an environment variable
            2. `mcp_client_side_auth_header_name` in the general settings on the config.yaml file

        Support this auth: https://router.delify.vn/docs/mcp#using-your-mcp-with-client-side-credentials

        If you want to use a different header name, you can set the `DEROUTER_MCP_CLIENT_SIDE_AUTH_HEADER_NAME` in the secret manager or `mcp_client_side_auth_header_name` in the general settings.
    ...
    def _get_mcp_server_auth_headers_from_headers(
        ...
        for header_name, header_value in headers.items():
            if header_name.lower().startswith(prefix):
                ...
                if (
                    header_name.lower() == MCPRequestHandler.DEROUTER_MCP_ACCESS_GROUPS_HEADER_NAME.lower()
                    or header_name.lower() == MCPRequestHandler.DEROUTER_MCP_SERVERS_HEADER_NAME.lower()
                ):
                    continue
    ...
    def get_mcp_client_side_auth_header_name() -> str:
        ...
        MCP_CLIENT_SIDE_AUTH_HEADER_NAME: str = MCPRequestHandler.DEROUTER_MCP_AUTH_HEADER_NAME
        if get_secret_str("DEROUTER_MCP_CLIENT_SIDE_AUTH_HEADER_NAME") is not None:
            MCP_CLIENT_SIDE_AUTH_HEADER_NAME = (
                get_secret_str("DEROUTER_MCP_CLIENT_SIDE_AUTH_HEADER_NAME") or MCP_CLIENT_SIDE_AUTH_HEADER_NAME
            )
    ...
    @staticmethod
    def get_derouter_api_key_from_headers(headers: Headers) -> str | None:
        """
        Get the Litellm API key from the headers using case-insensitive lookup

        1. Check if `x-derouter-api-key` is in the headers
        ...
        api_key: Final = headers.get(MCPRequestHandler.DEROUTER_API_KEY_HEADER_NAME_PRIMARY)
        if api_key:
            return api_key

        auth_header: Final = headers.get(MCPRequestHandler.DEROUTER_API_KEY_HEADER_NAME_SECONDARY)
        if auth_header:
            return auth_header
    ...
    @staticmethod
    async def _roster_team_object(team_id: str, auth: UserAPIKeyAuth) -> DeRouter_TeamTable | None:
        ...
        if prisma_client is None or not auth.user_id:
            return None
        try:
            team_obj: Final[DeRouter_TeamTable | None] = await get_team_object(
                team_id=team_id,
                prisma_client=prisma_client,
                user_api_key_cache=user_api_key_cache,
                parent_otel_span=auth.parent_otel_span,
    ...
    def _get_key_object_permission(
        ...
        get_key_object() in derouter/proxy/auth/auth_checks.py
        """
    ...
    async def _get_team_object_permission(
        ...
        get_team_object() in derouter/proxy/auth/auth_checks.py
        """
        from derouter.proxy.auth.auth_checks import get_team_object
        from derouter.proxy.proxy_server import (
            prisma_client,
    ...
    async def _get_key_access_group_mcp_server_extras(
        ...
        """
        Resolve the key's unified `access_group_ids` (DeRouter_AccessGroupTable) to
    ...
    @staticmethod
    async def _team_granted_servers(team_obj: DeRouter_TeamTable, team_access_group_servers: list[str]) -> set[str]:
    ...
    async def _load_named_object_permission(
        ...
        from derouter.proxy.auth.auth_checks import get_object_permission
        from derouter.proxy.proxy_server import proxy_logging_obj, user_api_key_cache
    ...
    async def _get_allowed_mcp_servers_for_agent(
        user_api_key_auth: UserAPIKeyAuth | None = None,
        agent_object_permission: DeRouter_ObjectPermissionTable | None = None,
    ) -> list[str]:
        """
    ...
    def get_mcp_access_groups_from_headers(headers: Headers) -> list[str] | None:
        ...
        """
        mcp_access_groups_header: Final = headers.get(MCPRequestHandler.DEROUTER_MCP_ACCESS_GROUPS_HEADER_NAME)
