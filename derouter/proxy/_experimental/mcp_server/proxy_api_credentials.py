...
from derouter.proxy._experimental.mcp_server.bridge_token_flow import load_active_user_by_id
...
async def mint_proxy_credential(
    ...
    key: Final = ExperimentalUIJWTToken.get_cli_jwt_auth_token(
        user_info=DeRouter_UserTable(user_id=user.user_id, user_role=user.user_role, models=user.models),
        team_id=team_id,
...
async def _team_details(teams: Sequence[str]) -> tuple[CliSsoTeamDetail, ...] | None:
    from derouter.proxy.proxy_server import prisma_client  # noqa: PLC0415  # rebound after startup, so read it per call
