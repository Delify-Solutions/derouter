...
from derouter.exceptions import BlockedPiiEntityError, GuardrailRaisedException
from derouter.experimental_mcp_client.client import MCPClient, MCPSigV4Auth, strip_auth_scheme, to_basic_credentials
...
from derouter.derouter_core_utils.url_utils import SSRFError, async_safe_get
...
from derouter.proxy._experimental.mcp_server.elicitation_handler import (
...
from derouter.proxy._experimental.mcp_server.oauth2_token_cache import (
...
from derouter.proxy._experimental.mcp_server.outbound_credentials.per_user_oauth_store import (
    LazyPerUserOAuthTokenStore,
)
from derouter.proxy._experimental.mcp_server.outbound_credentials.token_exchange_provider import (
...
from derouter.proxy._experimental.mcp_server.sampling_handler import (
...
from derouter.proxy.common_utils.user_api_key_cache import get_management_object_ttl
...
from derouter.repositories.table_repositories import MCPServerRepository
...
from derouter.types.mcp_server.mcp_server_manager import (
    MCPInfo,
    MCPOAuthMetadata,
    MCPServer,
)
...
if TYPE_CHECKING:
    ...
    from derouter.types.mcp_server.mcp_toolset import MCPToolset
...
_separator_probe_tool_name: Final = f"derouter{MCP_TOOL_PREFIX_SEPARATOR}probe"
...
_MCP_OAUTH_DISCOVERY_ON_STARTUP_ENV: Final = "DEROUTER_MCP_OAUTH_DISCOVERY_ON_STARTUP"
...
def _pinned_config_server_id(raw_server_id: object, server_name: str) -> str | None:
    ...
    Under ``DEROUTER_USE_SHORT_MCP_TOOL_PREFIX`` the tool prefix is derived from the server_id, so
...
def _config_identifier_owners(
    ...
    An alias is either set on the entry or mapped to it from ``derouter_settings.mcp_aliases``. Only
...
def _should_strip_caller_authorization(
    ...
    - **OAuth pass-through servers**: strip when the ``Authorization``
      header is actually the DeRouter API key — either because admission
      validated it (``user_api_key_auth.api_key`` is set) and the caller
      did NOT also supply ``x-derouter-api-key`` to disambiguate, or
    ...
    - **oauth_delegate servers**: admission always runs and there is no
      ...
      forwarded only when a distinct ``x-derouter-api-key`` carried
    ...
    has_explicit_derouter_admission_header: Final = _has_explicit_derouter_admission_header(raw_headers)
    if mcp_server.is_oauth_delegate:
        return not has_explicit_derouter_admission_header
    return _authorization_is_derouter_admission_credential(raw_headers, user_api_key_auth) or (
        user_api_key_auth is None and not has_explicit_derouter_admission_header
    )


DEROUTER_VIRTUAL_KEY_PREFIX: Final = "sk-"
...
def _has_explicit_derouter_admission_header(raw_headers: Mapping[str, str] | None) -> bool:
    """Admission only consumes a non-empty ``x-derouter-api-key``; an empty one falls back to ``Authorization``."""
    return bool(_raw_header_value(raw_headers, "x-derouter-api-key"))


def _authorization_is_derouter_admission_credential(
    raw_headers: Mapping[str, str] | None,
    user_api_key_auth: UserAPIKeyAuth | None,
) -> bool:
    """True when ``Authorization`` carries the DeRouter key admission validated.

    That is the case when no usable ``x-derouter-api-key`` was sent, or when the client repeated the
    same key in both headers.
    """
    if user_api_key_auth is None or not user_api_key_auth.api_key:
        return False
    admission_header: Final = _raw_header_value(raw_headers, "x-derouter-api-key")
...
def _client_forwarded_authorization_headers(
    ...
    ``_should_strip_caller_authorization`` says it was consumed as the DeRouter admission key. Shared by
...
def _warn_internal_delegate_pkce_if_applicable(server: MCPServer, *, source: str) -> None:
    ...
    verbose_logger.warning(
        ...
        "/authorize flow and complete PKCE without a DeRouter API key session; ensure the "
...
def _create_sampling_callback(user_api_key_auth: UserAPIKeyAuth | None = None):
    ...
    upstream MCP servers by routing them through derouter.acompletion().
    """
    ...
    async def _sampling_callback(
        ...
        return await handle_sampling_create_message(
            context=context,
            params=params,
            default_model=getattr(derouter, "default_mcp_sampling_model", None),
...
def _create_elicitation_callback():
    ...
    async def _elicitation_callback(context, params):
        ...
        from derouter.proxy._experimental.mcp_server.server import get_active_mcp_session
...
def _record_mcp_guardrail_evaluations(
    ...
    try:
        _sync_guardrail_info_to_logging_obj(synthetic_llm_data, derouter_logging_obj)
...
class MCPServerManager:
    ...
    def is_config_declared_server(self, server_id: str) -> bool:
        ...
        a DeRouter_MCPServerTable row is always resolved to that row first."""
    ...
    async def load_servers_from_config(
        ...
        Args:
            ...
            mcp_aliases: Optional dictionary mapping aliases to server names from derouter_settings
        """
    ...
    async def _hydrate_config_servers_dcr_clients(self) -> None:
        ...
        from derouter.proxy._experimental.mcp_server.discoverable_endpoints import (  # noqa: PLC0415  # circular import
    ...
    async def _register_openapi_tools(self, spec_path: str, server: MCPServer, base_url: str):
        ...
        """
        from derouter.proxy._experimental.mcp_server.openapi_to_mcp_generator import (
            build_input_schema,
    ...
    def remove_server(self, mcp_server: DeRouter_MCPServerTable):
        """
    ...
    def _resolve_env_vars_list(
        self,
        mcp_server: DeRouter_MCPServerTable,
        *,
        ...
        if env_vars_are_encrypted:
            from derouter.proxy._experimental.mcp_server.db import (  # noqa: PLC0415
    ...
    async def add_server(self, mcp_server: DeRouter_MCPServerTable):
    ...
    async def update_server(self, mcp_server: DeRouter_MCPServerTable):
    ...
    async def operator_open_server_ids(
        ...
        *,
        ...
        from derouter.proxy._experimental.mcp_server.mcp_context import (  # noqa: PLC0415
    ...
    async def get_allowed_mcp_servers(
        ...
        *,
        ...
        """
        from derouter.proxy.proxy_server import general_settings as proxy_general_settings
        ...
        try:
            ...
            # DeRouter granting access here does not bypass any security gate.
            ...
            if is_anonymous:
                delegate_server_ids: Final = [
                    ...
                    if (
                        ...
                        # unauthenticated caller would get DeRouter to proxy tool
    ...
    async def resolve_toolset_tool_permissions(
        ...
        """
        from derouter.proxy._experimental.mcp_server.toolset_db import list_mcp_toolsets
        from derouter.proxy.proxy_server import prisma_client, user_api_key_cache
    ...
    async def get_toolset_by_name_cached(
        ...
        from derouter.proxy._experimental.mcp_server.toolset_db import (
    ...
    def _extract_subject_token(
        ...
        """The caller's upstream identity token, or ``None`` when the bearer is a DeRouter key.

        Rejects the key admission validated and, because virtual keys always carry the ``sk-`` prefix,
        any other DeRouter key a client puts in ``Authorization`` next to ``x-derouter-api-key``.
        """
        if _authorization_is_derouter_admission_credential(raw_headers, user_api_key_auth):
            return None
        bearer: Final = MCPServerManager._extract_bearer_token(oauth2_headers, raw_headers)
        if bearer is not None and bearer.startswith(DEROUTER_VIRTUAL_KEY_PREFIX):
            return None
        return bearer
    ...
    async def _resolve_static_headers_with_env_vars(
        ...
        *,
        ...
        Per-user values come from the ``DeRouter_MCPUserEnvVars`` row for the
        calling user.
    ...
    async def _load_user_env_vars(
        ...
        *,
        ...
        from derouter.proxy.proxy_server import prisma_client  # noqa: PLC0415
    ...
    async def preflight_token_exchange(
        ...
        faithful exactly when no identity bearer was sent (a DeRouter key in ``Authorization`` is not one),
            ...
            if resolved_server.command:
                ...
                if (
                    ...
                    raise HTTPException(
                        ...
                        f"Add it to DEROUTER_MCP_STDIO_EXTRA_COMMANDS to allow this command.",
                    )
    ...
    async def _get_tools_from_server(
        ...
        try:
            ...
            if user_api_key_auth is not None and not server.spec_path:
                from derouter.proxy.guardrails.guardrail_hooks.mcp_jwt_signer.mcp_jwt_signer import (
                    get_mcp_jwt_signer,
    ...
    def _assign_unique_short_prefix(
        ...
        ``DEROUTER_USE_SHORT_MCP_TOOL_PREFIX`` is disabled, when the server
    ...
    async def pre_call_tool_check(
        ...
        ``derouter_logging_obj`` is the request's logger, and it is what lands a
        ...
        synthetic_llm_data["derouter_logging_obj"] = derouter_logging_obj
        ...
        try:
            ...
            _record_mcp_guardrail_evaluations(synthetic_llm_data, derouter_logging_obj)
    ...
    def _create_during_hook_task(
        ...
        ``derouter_logging_obj`` is the request's logger; see ``pre_call_tool_check``.
        ...
        from derouter.types.mcp import MCPDuringCallRequestObject
    ...
    async def _call_regular_mcp_tool(
        ...
        # the exact case of server alias/name (e.g., '1derouteragcgateway' vs '1DeRouterAGCGateway')
    ...
    async def _resolve_oauth2_headers_for_tool_call(
        ...
        try:
            from derouter.proxy._experimental.mcp_server.server import (  # noqa: PLC0415
    ...
    async def call_tool(
        ...
        Args:
            ...
            mcp_server_auth_headers: Optional dict of server-specific auth headers {server_alias: auth_value}
            proxy_logging_obj: Optional ProxyLogging object for hook integration
            derouter_logging_obj: Optional request logger the guardrail hooks record
    ...
    async def reload_servers_from_database(self):
        ...
        from derouter.proxy._experimental.mcp_server.db import DeRouter_MCPServerTable
        ...
        for row in raw_rows:
            try:
                server = DeRouter_MCPServerTable.model_validate(row.model_dump())
    ...
    def _is_server_accessible_from_ip(self, server: MCPServer, client_ip: str | None) -> bool:
        ...
        if server.available_on_public_internet:
            return True
        # Check backwards compat: derouter.public_mcp_servers
        public_ids: Final = set(derouter.public_mcp_servers or [])
    ...
    def get_public_mcp_servers(self) -> list[MCPServer]:
        ...
        Default (derouter.public_mcp_hub_strict_whitelist=True): mirrors
        /public/model_hub and /public/agent_hub — gates strictly on the
        derouter.public_mcp_servers whitelist. Returns an empty list when no
        ...
        Legacy (derouter.public_mcp_hub_strict_whitelist=False): preserves the
        ...
        """
        if derouter.public_mcp_hub_strict_whitelist:
            if derouter.public_mcp_servers is None:
                return []
            public_ids = set(derouter.public_mcp_servers)
            return [server for server in self.get_registry().values() if server.server_id in public_ids]

        public_ids = set(derouter.public_mcp_servers or [])
        return [
            server
            for server in self.get_registry().values()
    ...
    async def health_check_server(self, server_id: str, mcp_auth_header: str | None = None) -> DeRouter_MCPServerTable:
        """
    ...
    async def get_all_allowed_mcp_servers(
        ...
        list_mcp_servers: Final[list[DeRouter_MCPServerTable]] = []
    ...
    def _build_mcp_server_table(self, server: MCPServer) -> DeRouter_MCPServerTable:
        return DeRouter_MCPServerTable(
            server_id=server.server_id,
            server_name=server.server_name,
            alias=server.alias,
    ...
    async def get_all_mcp_servers_unfiltered(self) -> list[DeRouter_MCPServerTable]:
        ...
        servers: Final[list[DeRouter_MCPServerTable]] = []
        for server in registry.values():
    ...
    async def _run_health_checks(self, target_server_ids: list[str]) -> list[DeRouter_MCPServerTable]:
        if not target_server_ids:
            return []
