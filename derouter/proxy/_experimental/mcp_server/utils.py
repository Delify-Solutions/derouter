...
# ``tests/test_derouter/proxy/_experimental/mcp_server/test_mcp_server_identity_env.py``.
DEROUTER_MCP_SERVER_NAME: Final = os.environ.get("DEROUTER_MCP_SERVER_NAME", "derouter-mcp-server")
DEROUTER_MCP_SERVER_VERSION: Final = "1.0.0"
DEROUTER_MCP_SERVER_DESCRIPTION: Final = os.environ.get("DEROUTER_MCP_SERVER_DESCRIPTION", "MCP Server for DeRouter")
...
# ---------------------------------------------------------------------------
# When DEROUTER_USE_SHORT_MCP_TOOL_PREFIX is truthy the prefix attached to MCP
...
#     tool name happening to begin with the exact prefix DeRouter assigned
...
def is_short_mcp_tool_prefix_enabled() -> bool:
    ...
    """
    raw: Final = os.environ.get("DEROUTER_USE_SHORT_MCP_TOOL_PREFIX", "")
...
def sanitize_mcp_alias_for_header(alias: str) -> str:
    ...
    Must stay in sync with ui/derouter-dashboard/src/utils/mcpHeaderUtils.ts.
    """
...
def get_server_prefix(server: object) -> str:
    ...
    When the short-prefix mode is enabled (``DEROUTER_USE_SHORT_MCP_TOOL_PREFIX``)
...
def iter_known_tool_name_spellings(tool_name: str, server: MCPServer) -> Iterator[str]:
    ...
    alias and with ``DEROUTER_USE_SHORT_MCP_TOOL_PREFIX``.
    """
    yield tool_name
...
# Local rather than derouter.constants: this module deliberately imports no derouter
# package, so pulling one in for a single integer would drag in derouter/__init__.
...
def _custom_derouter_key_header_name() -> str | None:
    """``general_settings.derouter_key_header_name``, the deployment's custom header name for
    ...
    try:
        ...
        return None
    return general_settings.get("derouter_key_header_name") if general_settings else None
...
def _mcp_client_side_auth_header_name() -> str:
    ...
    try:
        ...
        return MCPRequestHandler.DEROUTER_MCP_AUTH_HEADER_NAME
...
def _upstream_credential_headers(header_names: Iterable[str]) -> frozenset[str]:
    ...
    non_credential: Final = frozenset(
        {
            MCPRequestHandler.DEROUTER_MCP_SERVERS_HEADER_NAME.lower(),
            MCPRequestHandler.DEROUTER_MCP_ACCESS_GROUPS_HEADER_NAME.lower(),
        }
    )
...
def build_synthetic_mcp_request(
    *,
    ...
    lets ``add_derouter_data_to_request`` derive ``metadata.headers``,
    ...
    ``derouter_key_header_name``, are dropped so they cannot reach a callback or a guardrail
    through the derived metadata even when a caller omits ``general_settings``.
    """
    from fastapi import Request

    custom_key_header: Final = _custom_derouter_key_header_name()
    excluded: Final = (
...
def logging_safe_mcp_headers(raw_headers: Mapping[str, str] | None) -> Mapping[str, str]:
    ...
    Client-controlled behaviour flags (``derouter-disable-message-redaction``) are dropped
    ...
    cleaned: Final = clean_headers(
        Headers(raw_headers),
        derouter_key_header_name=_custom_derouter_key_header_name(),
    )
