...
from derouter.proxy._experimental.mcp_server.oauth_utils import canonicalize_url_identity
...
async def backfill_discovery_stamped_issuers(prisma_client: PrismaClient) -> int:
    ...
    candidate_rows: Final[list[_MCPServerRow]] = await prisma_client.db.derouter_mcpservertable.find_many(
        where={
    ...
    for row in stamped:
        try:
            await prisma_client.db.derouter_mcpservertable.update(
