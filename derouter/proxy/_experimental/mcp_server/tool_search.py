...
from derouter.proxy.agent_endpoints.agent_search import DEFAULT_AGENT_SEARCH_TOP_K
from derouter.proxy.common_utils.semantic_text_index import (
    Embedder,
    EmbeddingFailed,
    SemanticTextIndex,
    router_embedder,
...
def mcp_tool_search_settings() -> MCPToolSearchSettings | ValidationError:
    try:
        return MCPToolSearchSettings.model_validate(derouter.mcp_tool_search or {})
...
_MCP_PROXY_IDENTITY_META_KEY: Final[str] = "router.delify.vn/proxy_tool_identity"
...
async def handle_skill_search(query: str, top_k: int, user_api_key_dict: UserAPIKeyAuth) -> CallToolResult:
    ...
    outcome: Final = await search_skills(
        query=query,
        skills=await DeRouterSkillsHandler.list_skills_for_search(user_api_key_dict),
        top_k=min(max(top_k, 1), MAX_SKILL_SEARCH_TOP_K),
        router=llm_router,
        embedding_model=derouter.skill_search_embedding_model,
...
async def handle_mcp_tool_search(
    ...
    if isinstance(settings, ValidationError):
        return _text_tool_result(
            f"derouter_settings.{MCP_TOOL_SEARCH_SETTINGS_KEY} is invalid: {settings}", is_error=True
        )
    if settings.embedding_model is not None and llm_router is None:
        return _text_tool_result(
            f"derouter_settings.{MCP_TOOL_SEARCH_SETTINGS_KEY}.embedding_model needs a model_list so it can be called",
            is_error=True,
        )
    ranker: Final = (
        SemanticToolRanker(
...
async def handle_mcp_proxy_tool(
    ...
    from derouter.proxy._experimental.mcp_server.server import (  # pyright: ignore[reportPrivateUsage]  # shared catalog owner
        _list_mcp_tools,  # pyright: ignore[reportPrivateUsage]  # shared catalog owner
    )
