...
def _resolve_proxy_base_url_env() -> str | None:
    ...
    if _warned_invalid_proxy_base_url != configured:
        verbose_logger.warning(
            ...
            "Set it to a full URL like https://derouter.example.com.",
            configured,
        )
...
def get_route_relative_request_path(scope: Scope) -> str:
    ...
    deployment they still carry the ``SERVER_ROOT_PATH`` prefix (``/derouter/{server}/mcp``) while
    every route shape compared against them is root-relative. Mirrors the segment-boundary strip in
    :func:`derouter.proxy.auth.auth_utils.get_request_route`, which the rest of the MCP auth path
    already routes through, so ``/derouterfoo`` is not truncated under ``root_path=/derouter``."""
...
def _build_trusted_redirect_rejection_message(
    ...
    leaking internal network topology (e.g. ``http://derouter-internal:4000``)
...
def _raise_trusted_redirect_uri_rejected(
    ...
    hint: Final = (
        ...
        "HTTPS origin (e.g. https://derouter.example.com), or enable "
