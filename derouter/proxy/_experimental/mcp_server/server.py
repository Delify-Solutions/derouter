...
from derouter.proxy._experimental.mcp_server.mcp_debug import MCPDebug
...
from derouter.proxy._experimental.mcp_server.utils import (
    DEROUTER_MCP_SERVER_DESCRIPTION,
    DEROUTER_MCP_SERVER_NAME,
    DEROUTER_MCP_SERVER_VERSION,
...
from derouter.types.mcp import MCPAuth, MCPSpecVersion
from derouter.types.mcp_server.mcp_server_manager import MCPInfo, MCPServer
from derouter.types.utils import CallTypes, StandardLoggingMCPToolCall
from derouter.utils import Rules, client, function_setup

if TYPE_CHECKING:
    from mcp.server.session import ServerSession as _McpServerSession

    from derouter.proxy._experimental.mcp_server.db import OAuthCredentialPayload
...
_MCP_TRANSPORT_SPAN_SCOPE_KEY: Final = "derouter_otel_transport_span"
_MCP_DESTINATIONS_SCOPE_KEY: Final = "derouter_otel_request_destinations"
...
# "mcp" requires python 3.10 or higher, but several derouter users use python 3.8
# We're making this conditional import to avoid breaking users who use python 3.8.
# TODO: Make this a util function for derouter client usage
...
def _mcp_meta_trace_carrier(req_ctx: object) -> dict[str, str] | None:
    ...
    stamps allowlisted baggage keys (``derouter.team.id``, ``derouter.metadata.*``,
...
def _otel_publish_request_destinations_on_scope(scope: Scope) -> None:
    try:
        from derouter.integrations.otel.plumbing.context import request_destinations
...
def _otel_reset_mcp_request_destinations(token: object) -> None:
    if token is None:
        return
    try:
        from derouter.integrations.otel.plumbing.context import reset_request_destinations
...
if MCP_AVAILABLE:
    ...
    from derouter.proxy._experimental.mcp_server.auth.derouter_auth_handler import (
    ...
    from derouter.proxy._experimental.mcp_server.sse_transport import SseServerTransport
    ...
    # required dependency for `derouter` pip package
    ######################################################
    ...
    server: Final[Server] = Server(
        name=DEROUTER_MCP_SERVER_NAME,
        version=DEROUTER_MCP_SERVER_VERSION,
    )
    ...
    async def handle_list_tools() -> "ListToolsResult | list[Tool]":
        ...
        try:
            ...
            from derouter.proxy._experimental.mcp_server.tool_search import (
    ...
    async def _dispatch_virtual_mcp_tool(
        ...
        """
        from derouter.llms.derouter_proxy.skills.skill_search import DEFAULT_SKILL_SEARCH_TOP_K
        from derouter.proxy._experimental.mcp_server.tool_search import (
        ...
        if _mcp_proxy_mode.get() and name in MCP_PROXY_TOOL_NAMES:
            ...
            try:
                proxy_result: Final = await handle_mcp_proxy_tool(
                    ...
                    oauth2_headers=oauth2_headers,
                    raw_headers=raw_headers,
                    derouter_logging_obj=proxy_logging_obj,
                )
            except Exception as exc:
                if proxy_logging_obj is not None:
                    from derouter.proxy.proxy_server import proxy_logging_obj as request_logging_obj
        ...
        return await handle_mcp_tool_call(
            ...
            oauth2_headers=oauth2_headers,
            raw_headers=raw_headers,
            derouter_logging_obj=virtual_logging_obj,
        )
    ...
    async def mcp_server_tool_call(name: str, arguments: dict[str, object] | None) -> CallToolResult:
        ...
        try:
            ...
            try:
                ...
                if chain_id:
                    body_data["derouter_trace_id"] = chain_id
                    body_data["derouter_session_id"] = chain_id
    ...
    def _client_has_per_server_auth_header(
        ...
        auth mode (never the DeRouter admission credential).
    ...
    async def _prefetch_oauth_creds_for_user(
        ...
        try:
            ...
            from derouter.proxy.utils import get_prisma_client_or_throw  # noqa: PLC0415
    ...
    async def _get_tools_from_mcp_servers(
        ...
        if log_list_tools_to_spendlogs:
            ...
            effective_derouter_trace_id: Final = derouter_trace_id or get_chain_id_from_headers(raw_headers)
            ...
            list_tools_request_data = {
                ...
                "derouter_call_id": list_tools_call_id,
                "derouter_trace_id": effective_derouter_trace_id,
                "metadata": {
            ...
            try:
                ...
                if derouter_logging_obj:
                    derouter_logging_obj.call_type = CallTypes.list_mcp_tools.value
                    derouter_logging_obj.model = "MCP: list_tools"
        ...
        try:
            ...
            async def _fetch_and_filter_server_tools(
                ...
                from derouter.proxy._experimental.mcp_server.outbound_credentials.adapter import (  # noqa: PLC0415
                    to_server_spec,
                )
                ...
                try:
                    ...
                    if mcp_proxy_mode:
                        from derouter.proxy._experimental.mcp_server.tool_search import with_mcp_proxy_identity
            ...
            if derouter_logging_obj:
                ...
                metadata_dict: Final = derouter_logging_obj.model_call_details.get("metadata")
                ...
                try:
                    await derouter_logging_obj.async_success_handler(
                        result=[
    ...
    async def _get_byok_credential(
        ...
        from derouter.proxy._experimental.mcp_server.db import get_user_credential
    ...
    async def execute_mcp_tool(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            **kwargs: Additional arguments (e.g., derouter_logging_obj)
        ...
        if requested_server is not None and not name_is_prefixed:
            ...
            # DeRouter prefix). The first segment is not a registered server
        ...
        if derouter_logging_obj:
            derouter_logging_obj.model_call_details["mcp_tool_call_metadata"] = standard_logging_mcp_tool_call
            derouter_logging_obj.model = f"MCP: {name}"
            derouter_logging_obj.model_call_details["model"] = f"MCP: {name}"
    ...
    async def _run_post_mcp_call_guardrails(
        ...
        return await proxy_logging_obj.post_mcp_call_hook(
            response=result,
            request_data=(
                derouter_logging_obj.model_call_details if derouter_logging_obj is not None else dict(request_data)
            ),
            user_api_key_dict=user_api_key_auth,
        )
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if derouter_logging_obj is not None:
                ...
                derouter_logging_obj.failure_handler(e, traceback_str, start_time, end_time)
                await derouter_logging_obj.async_failure_handler(e, traceback_str, start_time, end_time)
    ...
    async def _handle_stale_mcp_session(
        ...
        Fixes https://github.com/Delify-Solutions/derouter/issues/20992
        """
    ...
    async def _apply_toolset_scope(
        ...
        from derouter.proxy.management_endpoints.common_utils import _user_has_admin_view
        ...
        if existing_op is not None:
            ...
            updated_op = DeRouter_ObjectPermissionTable(
    ...
    async def _raise_preemptive_401_for_unauthenticated_servers(
        ...
        for server_name in mcp_servers or []:
            ...
            if server and server.auth_type == MCPAuth.oauth2:
                ...
                # Authorization is a DeRouter key (surfaced here as oauth2_headers)
                ...
                if getattr(server, "delegate_auth_to_upstream", False) is not True:
                    ...
                    # cannot vault without a derouter key on its token request.
            ...
            # so the client discovers the IdP, SSOs, and retries with a subject token, which DeRouter
            ...
            if server and server.auth_type == MCPAuth.oauth2_token_exchange and not oauth2_headers:
                from derouter.proxy._experimental.mcp_server.outbound_credentials.adapter import (  # noqa: PLC0415  # lazy: adapter pulls MCP subgraph
                    raise_token_exchange_challenge,
                )
                from derouter.proxy.middleware.per_request_root_path_middleware import (  # noqa: PLC0415  # lazy: middleware imports proxy utils
    ...
    def _get_forwarded_auth_from_scope(scope: Scope) -> str | None:
        ...
        Only returns the ``Authorization`` header when ``x-derouter-api-key`` is
        ...
        ``x-derouter-api-key`` is absent the ``Authorization`` header may itself
        be the DeRouter proxy API key (backward-compat path in
        ``MCPRequestHandler.process_mcp_request``), and forwarding it upstream
        would leak the proxy key to a third-party MCP server.
        """
        scope_headers: Final[Sequence[tuple[bytes, bytes]]] = scope.get("headers", [])
        has_derouter_key_header: Final = any(key.lower() == b"x-derouter-api-key" for key, _ in scope_headers)
        if not has_derouter_key_header:
            return None
    ...
    async def _probe_upstream_auth(
        ...
        probe_payload: Final = {
            "jsonrpc": "2.0",
            "id": "derouter-mcp-auth-probe",
            "method": "initialize",
            "params": {
                "protocolVersion": MCPSpecVersion.jun_2025.value,
                "capabilities": {},
                "clientInfo": {
                    "name": "derouter-mcp-auth-probe",
                    "version": "1.0.0",
                },
            },
        }
    ...
    async def _check_passthrough_upstream_auth(
        ...
        an upstream token (never a DeRouter key) when admission took the delegate bypass,
        ...
        group would have been admitted as a real DeRouter key, so probing it would leak that
        ...
        # The bare Authorization header (no x-derouter-api-key) is a valid upstream token
        ...
        # as a DeRouter key) is never probed with that key.
        ...
        for (srv, _, challenge_server_name), (probe_status, _) in zip(probe_targets, probe_results):
            if probe_status == 401:
                ...
                # the upstream IdP metadata proxied by DeRouter.
                www_authenticate = get_passthrough_www_authenticate(
                    scope=scope,
    ...
    async def handle_streamable_http_mcp(scope: Scope, receive: Receive, send: Send) -> None:
        ...
        try:
            ...
            # Inject masked debug headers when client sends x-derouter-mcp-debug: true
    ...
    app = FastAPI(
        title=DEROUTER_MCP_SERVER_NAME,
        description=DEROUTER_MCP_SERVER_DESCRIPTION,
        version=DEROUTER_MCP_SERVER_VERSION,
        lifespan=lifespan,
    )
