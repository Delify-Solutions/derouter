...
1. Per-user OAuth token rows exist for the server: only the interactive flow mints
   ...
   keys share the same table (``DeRouter_MCPUserCredentials``), so only rows whose
...
from derouter.proxy._experimental.mcp_server.db import _decode_oauth_payload, decrypt_credentials
...
class _MCPServerRow(Protocol):
    """The ``DeRouter_MCPServerTable`` columns this backfill reads."""
...
class _MCPUserCredentialRow(Protocol):
    """The ``DeRouter_MCPUserCredentials`` columns this backfill reads."""
...
def _mcp_server_table(prisma_client: PrismaClient) -> _MCPServerTable:
    ...
    return prisma_client.db.derouter_mcpservertable
...
def _mcp_user_credentials_table(prisma_client: PrismaClient) -> _MCPUserCredentialsTable:
    ...
    return prisma_client.db.derouter_mcpusercredentials
