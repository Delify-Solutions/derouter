...
async def post_client_credentials_grant(
    ...
    The httpx edge: derouter's handler raises ``HTTPStatusError`` itself on a 4xx/5xx, and every
    field the caller reads comes out of a validated ``TokenEndpointOutcome``.
    """
    from derouter.llms.custom_httpx.http_handler import (  # noqa: PLC0415  # defer heavy handler import to call time
        get_async_httpx_client,  # pyright: ignore[reportUnknownVariableType]  # handler factory params are coarsely typed
    )
    from derouter.types.llms.custom_http import httpxSpecialProvider  # noqa: PLC0415  # deferred with the handler import
...
def _prepare_grant(config: ClientCredentialsConfig) -> Result[_PreparedGrant, CredError]:
    ...
    from derouter.proxy._experimental.mcp_server.auth.token_endpoint_auth import (  # noqa: PLC0415  # keep package v1-free at import time
