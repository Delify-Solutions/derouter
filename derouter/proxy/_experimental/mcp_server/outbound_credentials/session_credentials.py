...
recovered derouter user (consumer), reloading the live user record and policy before
...
_SESSION_SIGNING_KEY_DOMAIN: Final = b"derouter-mcp-gateway:session-signing:"
...
def _resolve_key_material(value: str) -> str | None:
    if not value.startswith("os.environ/"):
        return value
    from derouter.secret_managers.main import get_secret_str  # noqa: PLC0415  # heavy import kept off the pure path
...
def active_session_signing_keys(master_key: str) -> SessionSigningKeys | SessionSigningConfigError:
    ...
    from derouter.proxy.proxy_server import general_settings  # noqa: PLC0415  # circular import at module load
