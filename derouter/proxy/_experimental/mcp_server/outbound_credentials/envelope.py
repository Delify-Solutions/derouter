...
A DCR-bridge client holds ONE bearer that must carry BOTH a derouter identity and the
upstream OAuth grant, with zero server-side storage. The gateway token endpoint mints a
derouter-signed envelope (:func:`mint_envelope`); the MCP edge validates it, recovers the
...
from derouter.proxy.common_utils.encrypt_decrypt_utils import decrypt_value, encrypt_value
...
ENVELOPE_ISSUER: Final = "derouter-mcp-bridge"
...
access envelope, and each renewal re-validates the sealed derouter key (revocation gates it) and is
...
"""Discriminator for what derouter principal the envelope binds the grant to.

``key_hash`` is a hashed virtual key (the scripted two-header client mints under the key it
presents at the token endpoint); ``user_id`` is a derouter user subject (the interactive DCR
...
class EnvelopeIdentity(BaseModel):
    """The derouter principal the envelope binds the inner grant to.

    ``subject`` is the principal identifier and ``subject_type`` says how to resolve it: a
    hashed derouter key (``key_hash``) or a derouter user id (``user_id``), never a raw
