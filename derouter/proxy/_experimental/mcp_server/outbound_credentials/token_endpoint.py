...
async def _post_form(endpoint: str, data: dict[str, str]) -> object:
    # derouter's httpx handler and httpx.Response are only partially typed; the token endpoint
    ...
    # `httpx.RequestError` (or derouter's `Timeout`, which the handler substitutes for
    # `httpx.TimeoutException`), and a non-JSON body raises `json.JSONDecodeError`; `fetch` maps
    # each to a CredError.
    client = get_async_httpx_client(llm_provider=httpxSpecialProvider.MCP)  # pyright: ignore[reportUnknownVariableType]  # derouter http handler is untyped
    response = await client.post(endpoint, data=data)  # pyright: ignore[reportUnknownMemberType,reportUnknownVariableType]  # derouter http handler is untyped
