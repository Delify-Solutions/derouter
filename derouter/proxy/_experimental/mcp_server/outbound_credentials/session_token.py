...
A DCR client that signs in through DeRouter SSO holds ONE bearer that carries ONLY a
derouter identity; unlike the :mod:`.envelope` bridge bearer it seals no upstream
credential, because the custody model vaults every upstream token server-side in
``DeRouter_MCPUserCredentials`` and egress resolves them by user at call time. The token
...
tell a gateway session from a derouter key, JWT, or bridge envelope before doing any
...
SESSION_ISSUER: Final = "derouter-mcp-gateway"
...
class SessionPrincipal(BaseModel):
    """The derouter user a session token identifies and the DCR client it was issued to.

    ``user_id`` is the SSO-established derouter user subject, never a credential: admission
