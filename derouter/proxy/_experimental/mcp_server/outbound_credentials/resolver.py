...
from derouter.proxy._experimental.mcp_server.outbound_credentials.sso_assertion_refresher import (
...
from derouter.proxy._experimental.mcp_server.outbound_credentials.token_endpoint import (
    ExchangedToken,
    ExchangedTokenCache,
    TokenEndpointClient,
)
...
class UpstreamCredentialProvider:
    ...
    def _passthrough(self, subject: Subject) -> Result[httpx.Auth, CredError]:
        ...
        The inbound token is the caller's already-disambiguated ``Authorization`` (never the DeRouter
    ...
    async def _id_jag_subject_token(self, subject: Subject) -> Result[str, CredError]:
        ...
        at DeRouter SSO login, which is what lets an agent holding a brokered DeRouter credential
        ...
        if not subject.subject_id:
            return Error(
                CredError.of_precondition_required(
                    ...
                    "identity token nor a resolved DeRouter user."
                )
            )
        try:
        ...
        if assertion_expired(assertion, datetime.now(timezone.utc)):
            return Error(
                CredError.of_precondition_required(
                    ...
                    "DeRouter SSO again to capture a current one."
                )
            )
