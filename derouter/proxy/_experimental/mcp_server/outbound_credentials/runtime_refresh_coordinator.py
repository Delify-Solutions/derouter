...
from derouter.proxy._experimental.mcp_server.outbound_credentials.redis_distributed_lock import (
    RedisDistributedLock,
)
from derouter.proxy._experimental.mcp_server.outbound_credentials.redis_refresh_coordinator import (
    RedisRefreshCoordinator,
)


def runtime_refresh_coordinator() -> RefreshCoordinator | None:
    from derouter.proxy.proxy_server import user_api_key_cache  # noqa: PLC0415  # runtime global
    ...
    redis_client: Final = redis_cache.init_async_client()  # pyright: ignore[reportUnknownMemberType,reportUnknownVariableType]  # derouter redis wrapper is untyped
    lock: Final = RedisDistributedLock(
        redis_client,  # pyright: ignore[reportArgumentType,reportUnknownArgumentType]  # derouter redis wrapper is untyped
