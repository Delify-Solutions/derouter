...
from derouter.constants import MCP_OAUTH2_TOKEN_CACHE_MAX_SIZE, MCP_SSO_ASSERTION_CACHE_TTL_SECONDS
...
async def ema_assertion_retention_enabled() -> bool:
    ...
    from derouter.proxy._experimental.mcp_server.mcp_server_manager import (  # noqa: PLC0415  # avoids import cycle
        global_mcp_server_manager,
    )
    from derouter.proxy.proxy_server import prisma_client  # noqa: PLC0415  # runtime global
    from derouter.types.mcp import MCPAuth  # noqa: PLC0415  # runtime global
    ...
    if prisma_client is None:
        return False
    row: Final = await prisma_client.db.derouter_mcpservertable.find_first(
...
async def persist_sso_identity_assertion(
    ...
    from derouter.proxy.common_utils.encrypt_decrypt_utils import encrypt_value_helper  # noqa: PLC0415  # runtime global
    from derouter.proxy.proxy_server import prisma_client  # noqa: PLC0415  # runtime global
    ...
    await prisma_client.db.derouter_ssoidentityassertion.upsert(
...
async def _read_assertion_from_db(user_id: str) -> SSOIdentityAssertion | None:
    from derouter.proxy.common_utils.encrypt_decrypt_utils import decrypt_value_helper  # noqa: PLC0415  # runtime global
    from derouter.proxy.proxy_server import prisma_client  # noqa: PLC0415  # runtime global

    if prisma_client is None:
        return None
    row: Final = await prisma_client.db.derouter_ssoidentityassertion.find_unique(where={"user_id": user_id})
    if row is None:
        return None
...
async def rotate_sso_identity_assertions_master_key(prisma_client: PrismaClient, new_master_key: str) -> None:
    ...
    from prisma.models import DeRouter_SSOIdentityAssertion as AssertionRow  # noqa: PLC0415  # generated at runtime

    from derouter.proxy.common_utils.encrypt_decrypt_utils import (  # noqa: PLC0415  # runtime global
    ...
    async def _rotate_row(row: AssertionRow) -> bool:
        ...
        await prisma_client.db.derouter_ssoidentityassertion.update(
            where={"user_id": row.user_id},
            data={"assertion_b64": re_encrypted},
        )
        return True

    rows: Final = await prisma_client.db.derouter_ssoidentityassertion.find_many()
