...
from derouter.proxy._experimental.mcp_server.auth.token_endpoint_auth import (
...
from derouter.proxy._experimental.mcp_server.faults import (
    CallerRejected,
    CredentialSource,
...
from derouter.proxy._experimental.mcp_server.gateway_dcr_flow import (
...
from derouter.proxy._experimental.mcp_server.proxy_api_credentials import (
...
from derouter.types.mcp import MCPAuth, MCPCredentials
from derouter.types.mcp_server.mcp_server_manager import MCPServer, MCPTokenEndpointAuthMethod
...
def encode_state_with_base_url(
    ...
    derouter_user_id: str | None = None,
    ...
    Args:
        ...
        derouter_user_id: The SSO-authenticated derouter user captured at the bridge authorize
        ...
        mcp_server_id: The server the flow targets, sealed alongside derouter_user_id (bridge) or
    ...
    state_data: Final = {
        ...
        "derouter_user_id": derouter_user_id,
...
class _BridgeAuthorizationCode(BaseModel):
    ...
    derouter_user_id: str = Field(min_length=1)
...
def seal_bridge_authorization_code(upstream_code: str, derouter_user_id: str, mcp_server_id: str) -> str:
    """Seal the upstream authorization code and the SSO-captured derouter user into a gateway
    ...
    exchange with the upstream), so a derouter identity captured in the browser at authorize survives
    ...
    payload: Final = json.dumps(
        {"upstream_code": upstream_code, "derouter_user_id": derouter_user_id, "mcp_server_id": mcp_server_id},
        sort_keys=True,
    )
...
def _session_cookie_user_id(request: Request) -> str | None:
    """The signed-in derouter user for a browser request, or ``None``. Thin wrapper so the
    aggregate DCR flow's verbs receive the identity as a plain value instead of parsing
    cookies themselves."""
    from derouter.proxy._experimental.mcp_server.byok_oauth_endpoints import (  # noqa: PLC0415  # circular import at module load
...
def _redirect_to_derouter_login(request: Request) -> RedirectResponse:
    """Send an unauthenticated browser through derouter login before the interactive bridge authorize
...
async def _store_per_user_token_server_side(
    ...
    """
    from derouter.proxy._experimental.mcp_server.oauth2_token_cache import (  # noqa: PLC0415
    ...
    from derouter.proxy._experimental.mcp_server.mcp_server_manager import (  # noqa: PLC0415
        global_mcp_server_manager,
    )
...
def _raise_if_not_oauth2(mcp_server: MCPServer) -> None:
    ...
    """
    from derouter.proxy._experimental.mcp_server.mcp_server_manager import (  # noqa: PLC0415  # circular import with mcp_server_manager at module load
...
async def authorize_with_server(
    ...
    # the loop, so the gateway can capture the derouter user here (from the browser's UI session) and
    ...
    # derouter key, so the browser session is the only identity source; without one there is nothing to
    ...
    if resolved_server.is_dcr_bridge and resolved_server.is_oauth_delegate:
        from derouter.proxy._experimental.mcp_server.byok_oauth_endpoints import (  # noqa: PLC0415  # inline import avoids a module-load circular import
            _user_id_from_session_cookie,
        )

        derouter_user_id = _user_id_from_session_cookie(request)
        if derouter_user_id is None:
            return _redirect_to_derouter_login(request)
    ...
    encoded_state: Final = encode_state_with_base_url(
        ...
        derouter_user_id=derouter_user_id,
        mcp_server_id=resolved_server.server_id if (derouter_user_id or ephemeral_dcr_client) else None,
...
async def exchange_token_with_server(
    ...
    if grant_type == "refresh_token":
        ...
        # the presented derouter key) and the envelope keys BEFORE the exchange consumes the single-use code.
        if is_bridge:
    ...
    # the calling client has provided a valid DeRouter identity.
    ...
    if resolved_server.needs_user_oauth_token:
        ...
        if user_id:
            ...
            verbose_logger.warning(
                "exchange_token_with_server: could not resolve a DeRouter user_id for the request, "
                ...
                "Ensure the request carries a valid DeRouter key (x-derouter-api-key or Authorization), "
...
async def _load_store_dcr_credentials(mcp_server: MCPServer) -> _PersistedDcrCredentials | None:
    ...
    (which has no DeRouter_MCPServerTable row). Returns None when the store has no usable client_id
    or the DB is unreachable."""
    from derouter.proxy._experimental.mcp_server.db import (  # noqa: PLC0415  # avoids circular import
        get_mcp_server_oauth_client_credentials,
    )
    from derouter.proxy.utils import get_prisma_client_or_throw  # noqa: PLC0415  # avoids circular import
...
async def hydrate_config_server_dcr_client(mcp_server: MCPServer) -> bool:
    ...
    refresh can authenticate. Config.yaml servers have no DeRouter_MCPServerTable row, so their
...
async def _resolve_persisted_dcr_client(
    mcp_server: MCPServer,
) -> tuple[Optional["DeRouter_MCPServerTable"], _PersistedDcrCredentials | None]:
    """Resolve a server's persisted DCR client using the same two-level rule the write path uses, so
    read and write always agree. First, whether the server HAS a DeRouter_MCPServerTable row: a row is
    ...
    from derouter.proxy._experimental.mcp_server.db import get_mcp_server  # noqa: PLC0415  # avoids circular import
    from derouter.proxy._experimental.mcp_server.mcp_server_manager import (  # noqa: PLC0415  # avoids circular import
        global_mcp_server_manager,
    )
    from derouter.proxy.utils import get_prisma_client_or_throw  # noqa: PLC0415  # avoids circular import
...
async def _persist_dcr_client_registration(
    ...
    ``DeRouter_MCPServerTable`` row when it has one, otherwise the server-scoped store when the server
    ...
    from derouter.proxy._types import UpdateMCPServerRequest  # noqa: PLC0415
    from derouter.proxy.utils import get_prisma_client_or_throw  # noqa: PLC0415
...
async def token_endpoint(
    ...
    2. Store the user's token in the db - and generate a DeRouter virtual key
    3. Return the token
    4. Return a virtual key in this response
    """
    if mcp_server_name is None and is_gateway_dcr_client_id(client_id):
        from derouter.proxy.proxy_server import (  # noqa: PLC0415  # circular import at module load
            master_key,
            user_api_key_cache,
        )
...
async def _vendor_credential_state(user_id: str, server_id: str) -> VendorCredentialState:
    ...
    from derouter.proxy._experimental.mcp_server.db import (  # noqa: PLC0415  # circular import at module load
        get_user_oauth_credential,
        oauth_grant_state,
    )
    from derouter.proxy.proxy_server import prisma_client  # noqa: PLC0415  # circular import at module load
...
async def authorize_complete(
    ...
    from derouter.proxy.proxy_server import user_api_key_cache  # noqa: PLC0415  # circular import at module load
...
async def introspect_endpoint(token: str = Form(...)) -> Response:
    ...
    secret. The caller authenticates with a DeRouter virtual key (section 2.1, enforced by
...
@router.get("/.well-known/derouter-cli-auth")
...
async def callback(
    ...
    try:
        ...
        # Interactive dcr_bridge oauth_delegate: the state carries the derouter user the authorize step
        ...
        derouter_user_id: Final = state_data.get("derouter_user_id")
        ...
        forwarded_code = code
        if isinstance(derouter_user_id, str) and derouter_user_id and isinstance(mcp_server_id, str) and mcp_server_id:
            forwarded_code = seal_bridge_authorization_code(
                upstream_code=code, derouter_user_id=derouter_user_id, mcp_server_id=mcp_server_id
            )
...
"""
    ...
    - DeRouter legacy pattern: /{server_name}/mcp (backward compatibility)
...
async def _build_oauth_protected_resource_response(
    ...
    if mcp_server_name:
        if use_standard_pattern:
            ...
            # DeRouter legacy pattern: /{server_name}/mcp
...
def _obo_protected_resource_response(mcp_server: MCPServer | None, resource_url: str) -> dict | None:
    ...
    The client SSOs with the IdP to obtain a subject token, which DeRouter then exchanges, so discovery
    points at the JWT-auth issuer(s) DeRouter trusts (the same IdP that issues and validates the
...
def _jwt_auth_issuers() -> list:
    """The OAuth issuer identifier(s) DeRouter's JWT auth trusts, for the OBO PRM authorization_servers.

    In token_exchange the IdP that issues the subject JWT is the same one DeRouter validates it
    against, so OBO discovery points clients at the JWT-auth issuer to obtain a subject token.
    Sourced from ``JWT_ISSUER`` and any configured ``derouter_jwtauth.issuers``.
    """
    import os  # noqa: PLC0415

    from derouter.proxy.proxy_server import general_settings  # noqa: PLC0415
    ...
    jwtauth: Final = general_settings.get("derouter_jwtauth") if isinstance(general_settings, dict) else None
...
# DeRouter legacy pattern: /.well-known/oauth-protected-resource/{server_name}/mcp
...
async def oauth_protected_resource_mcp(request: Request, mcp_server_name: str | None = None):
    """
    OAuth protected resource discovery endpoint using DeRouter legacy URL pattern.
...
# DeRouter legacy pattern and root endpoint
