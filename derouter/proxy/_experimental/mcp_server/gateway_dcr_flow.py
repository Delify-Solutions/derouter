...
2. ``GET /authorize``: validates the client and redirect URI, requires S256 PKCE, and
   interposes DeRouter sign-in. Without a session cookie the browser is sent through
...
4. ``POST /token``: exchanges the code (PKCE-verified, client- and redirect-bound,
   ...
   :mod:`.outbound_credentials.session_token`, re-validating that the derouter user is
...
from derouter.proxy.common_utils.html_forms.native_client_consent import (
...
def resolve_scoped_resource_server(request: Request, resource: str | None) -> MCPServer | None:
    ...
    if canonical == f"{base}/mcp" or not canonical.startswith(f"{base}/"):
        return None
    from derouter.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp import (  # noqa: PLC0415  # proxy import cycle
        MCPRequestHandler,
    )
    from derouter.proxy._experimental.mcp_server.mcp_server_manager import (  # noqa: PLC0415  # proxy import cycle
...
def aggregate_authorize(
    ...
    DeRouter sign-in, and hand the browser to the connect page with the flow sealed into a
    per-flow cookie.
...
def native_client_auth_contract(request: Request) -> NativeClientAuthContract:
    """The versioned discovery document at ``/.well-known/derouter-cli-auth``: everything a
    native client (in any language) needs to run the sign-in without reading DeRouter
...
def _open_flow_for(
    ...
    if flow is None or now.timestamp() >= flow.exp:
        return _oauth_error(400, "invalid_request", "unknown or expired connect flow")
    if session_user_id is None:
        return _oauth_error(401, "login_required", "sign in to DeRouter to finish connecting")
...
async def _flow_target(
    ...
    from derouter.proxy._experimental.mcp_server.mcp_server_manager import (  # noqa: PLC0415  # import cycle
        MCPServerManager,
        global_mcp_server_manager,
    )
...
class _SingleUseGuard:
    ...
    async def claim(self, key: str, ttl_seconds: int) -> ClaimOutcome:
        ...
        from derouter.proxy.proxy_server import redis_usage_cache  # noqa: PLC0415  # circular import at module load
...
async def aggregate_token(
    ...
    with that audience. Every path re-validates the derouter user live before minting, so a
...
async def introspect_gateway_token(
    ...
    (Kong, an API management layer) can validate a DeRouter-issued MCP session credential
    ...
    token, and a derouter user that is still live, so a deactivated user's outstanding
