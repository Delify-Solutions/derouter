...
from derouter.constants import MCP_PER_USER_TOKEN_EXPIRY_BUFFER_SECONDS
...
from derouter.types.mcp import MCPCredentials
...
class _UserEnvVarsTransactionClient(Protocol):
    derouter_mcpuserenvvars: "TableActions[prisma_db_models.DeRouter_MCPUserEnvVars]"
...
def decrypt_global_env_var_values(env_vars: Iterable[MCPEnvVar | dict[str, str]] | None) -> None:
    ...
    Accepts ``MCPEnvVar`` models (``DeRouter_MCPServerTable``) or plain dicts
    ...
    for entry in env_vars:
        ...
        if decrypted is None:
            ...
            verbose_proxy_logger.warning(
                "MCP global env var %s failed to decrypt (DEROUTER_SALT_KEY "
...
) -> "TableActions[prisma_db_models.DeRouter_MCPServerTable]":
    table: Final[TableActions[prisma_db_models.DeRouter_MCPServerTable]] = MCPServerRepository(prisma_client).table
    return table


def _verification_token_table_actions(
    prisma_client: PrismaClient,
) -> "TableActions[prisma_db_models.DeRouter_VerificationToken]":
    table: Final[TableActions[prisma_db_models.DeRouter_VerificationToken]] = VerificationTokenRepository(
        prisma_client
    ).table
    return table
...
def _team_table_actions(
    prisma_client: PrismaClient,
) -> "TableActions[prisma_db_models.DeRouter_TeamTable]":
    table: Final[TableActions[prisma_db_models.DeRouter_TeamTable]] = TeamRepository(prisma_client).table
    return table


def _oauth_client_table_actions(
    prisma_client: PrismaClient,
) -> "TableActions[prisma_db_models.DeRouter_MCPServerOAuthClient]":
    table: Final[TableActions[prisma_db_models.DeRouter_MCPServerOAuthClient]] = MCPServerOAuthClientRepository(
        prisma_client
    ).table
    return table
...
async def _db_find_mcp_server_rows(
    prisma_client: PrismaClient,
    where: "prisma_db_types.DeRouter_MCPServerTableWhereInput | None" = None,
) -> "Sequence[prisma_db_models.DeRouter_MCPServerTable]":
...
) -> "prisma_db_models.DeRouter_MCPServerTable | None":
...
async def _db_update_mcp_server_row(
    prisma_client: PrismaClient,
    server_id: str,
    data: "prisma_db_types.DeRouter_MCPServerTableUpdateInput",
) -> "prisma_db_models.DeRouter_MCPServerTable":
    row: Final[prisma_db_models.DeRouter_MCPServerTable | None] = await _mcp_server_table_actions(prisma_client).update(
...
def _user_credential_actions(
    prisma_client: PrismaClient,
) -> "TableActions[prisma_db_models.DeRouter_MCPUserCredentials]":
    table: Final[TableActions[prisma_db_models.DeRouter_MCPUserCredentials]] = MCPUserCredentialsRepository(
        prisma_client
    ).table
    return table
...
def _user_env_var_actions(
    prisma_client: PrismaClient,
) -> "TableActions[prisma_db_models.DeRouter_MCPUserEnvVars]":
    table: Final[TableActions[prisma_db_models.DeRouter_MCPUserEnvVars]] = prisma_client.db.derouter_mcpuserenvvars
    return table


async def _db_find_user_credential_row(
    prisma_client: PrismaClient, user_id: str, server_id: str
) -> "prisma_db_models.DeRouter_MCPUserCredentials | None":
...
async def _db_find_user_credential_rows(
    prisma_client: PrismaClient,
    where: "prisma_db_types.DeRouter_MCPUserCredentialsWhereInput | None" = None,
) -> "Sequence[prisma_db_models.DeRouter_MCPUserCredentials]":
...
async def _db_find_user_env_var_rows(
    prisma_client: PrismaClient,
    where: "prisma_db_types.DeRouter_MCPUserEnvVarsWhereInput | None" = None,
) -> "Sequence[prisma_db_models.DeRouter_MCPUserEnvVars]":
...
def _readable_mcp_servers(
    rows: Iterable["prisma_db_models.DeRouter_MCPServerTable"],
) -> Iterable[DeRouter_MCPServerTable]:
    for row in rows:
        try:
            table = DeRouter_MCPServerTable.model_validate(row.model_dump())
...
async def get_all_mcp_servers(
    ...
    """
    where: Final[prisma_db_types.DeRouter_MCPServerTableWhereInput] = (
...
async def get_mcp_server(prisma_client: PrismaClient, server_id: str) -> DeRouter_MCPServerTable | None:
    """
    ...
    if mcp_server is None:
        return None
    table: Final = DeRouter_MCPServerTable.model_validate(mcp_server.model_dump())
    decrypt_global_env_var_values(table.env_vars)
    return table


async def get_mcp_servers(prisma_client: PrismaClient, server_ids: Iterable[str]) -> list[DeRouter_MCPServerTable]:
    """
    Returns the matching mcp servers from the db with the server_ids
    """
    _mcp_servers: Final[Sequence[prisma_db_models.DeRouter_MCPServerTable]] = await _mcp_server_table_actions(
        prisma_client
    ).find_many(
        where={
...
async def get_mcp_servers_by_verificationtoken(prisma_client: PrismaClient, token: str) -> list[str]:
    ...
    verification_token_record: (
        prisma_db_models.DeRouter_VerificationToken | None
...
async def get_mcp_servers_by_team(prisma_client: PrismaClient, team_id: str) -> list[str]:
    ...
    """
    team_record: prisma_db_models.DeRouter_TeamTable | None = await _team_table_actions(prisma_client).find_unique(
        where={
            "team_id": team_id,
        },
        include={
...
) -> "Sequence[prisma_db_models.DeRouter_ObjectPermissionTable]":
    """
...
async def get_virtualkeys_for_mcp_server(
    prisma_client: PrismaClient, server_id: str
) -> "Sequence[prisma_db_models.DeRouter_VerificationToken]":
    """
    Get all the virtual keys that have access to the mcp server
    """
    virtual_keys: Final[
        Sequence[prisma_db_models.DeRouter_VerificationToken] | None
...
async def delete_mcp_server(
    ...
    if deleted_server is not None:
        ...
        try:
            credential_rows: Sequence[prisma_db_models.DeRouter_MCPUserCredentials] = await _user_credential_actions(
                prisma_client
    ...
    return deleted_server  # pyright: ignore[reportReturnType]  # prisma row, not domain DeRouter_MCPServerTable
...
async def create_mcp_server(
    ...
    return DeRouter_MCPServerTable.model_validate(new_mcp_server.model_dump())
...
async def create_draft_mcp_server(
    ...
    if existing is not None:
        ...
        return DeRouter_MCPServerTable.model_validate(existing.model_dump())
    ...
    try:
        ...
        if raced is None or raced.approval_status != MCPApprovalStatus.draft:
            raise
        return DeRouter_MCPServerTable.model_validate(raced.model_dump())
...
async def get_draft_mcp_server(
    ...
    table: Final = DeRouter_MCPServerTable.model_validate(draft_rows[0].model_dump())
    decrypt_global_env_var_values(table.env_vars)
    return table
...
async def update_mcp_server(
    ...
    return DeRouter_MCPServerTable.model_validate(updated_mcp_server.model_dump()) if updated_mcp_server else None
...
async def get_mcp_server_oauth_client_credentials(prisma_client: PrismaClient, server_id: str) -> object | None:
    ...
    DeRouter_MCPServerTable row, so their dynamically registered client lives here keyed
    by server_id. The returned value is the raw credentials blob for
    ``_get_persisted_dcr_credentials`` to parse."""
    row: Final[prisma_db_models.DeRouter_MCPServerOAuthClient | None] = await _oauth_client_table_actions(
        prisma_client
...
async def upsert_mcp_server_oauth_client_credentials(
    ...
    server-scoped store keyed by server_id, independent of any DeRouter_MCPServerTable row.
...
def _reencrypt_mcp_credentials_blob(
    ...
    if not credentials:
        return None
    from derouter.derouter_core_utils.safe_json_dumps import safe_dumps  # noqa: PLC0415  # avoids circular import
...
async def rotate_mcp_server_credentials_master_key(prisma_client: PrismaClient, touched_by: str, new_master_key: str):
    ...
    oauth_clients: Final[Sequence[prisma_db_models.DeRouter_MCPServerOAuthClient]] = await _oauth_client_table_actions(
        prisma_client
    ).find_many()
    oauth_updated = 0
...
def _decode_user_credential(stored: str) -> str | None:
    """Read back a value persisted in ``DeRouter_MCPUserCredentials.credential_b64``.
...
def _warn_undecryptable_credential(user_id: str, server_id: str) -> None:
    ...
    verbose_proxy_logger.warning(
        ...
        "previous DEROUTER_SALT_KEY); the user is treated as not connected and must re-authorize.",
        user_id,
        server_id,
    )
...
async def rotate_mcp_user_credentials_master_key(prisma_client: PrismaClient, new_master_key: str):
    """Re-encrypt every ``DeRouter_MCPUserCredentials`` row with ``new_master_key``.
...
async def rotate_mcp_user_env_vars_master_key(prisma_client: PrismaClient, new_master_key: str):
    """Re-encrypt every ``DeRouter_MCPUserEnvVars`` row with ``new_master_key``.
...
async def store_user_oauth_credential(
    ...
    if not skip_byok_guard:
        ...
        if existing is not None and _parse_oauth_payload(decoded) is None:
            ...
            # under a different DEROUTER_SALT_KEY, and one that decodes to nothing holds no
            ...
            verbose_proxy_logger.warning(
                ...
                "decrypted (likely written under a previous DEROUTER_SALT_KEY); replacing it with the "
...
async def purge_user_oauth_credentials_for_server(
    ...
    DeRouter_MCPUserCredentials also stores BYOK API keys in the same column; only rows whose payload
...
async def approve_mcp_server(
    ...
    table: Final = DeRouter_MCPServerTable.model_validate(updated.model_dump())
    decrypt_global_env_var_values(table.env_vars)
    return table
...
async def reject_mcp_server(
    ...
    now: Final = datetime.now(timezone.utc)
    data: Final[prisma_db_types.DeRouter_MCPServerTableUpdateInput] = {
...
async def get_mcp_submissions(
    ...
    """
    rows: Final[Sequence[prisma_db_models.DeRouter_MCPServerTable]] = await _mcp_server_table_actions(
        prisma_client
    ).find_many(
...
def _decode_user_env_vars(stored: str) -> dict[str, str]:
    ...
    if decrypted is None:
        if stored:
            verbose_proxy_logger.warning(
                "MCP per-user env vars failed to decrypt (DEROUTER_SALT_KEY "
...
async def merge_user_env_vars(
    ...
    async with _db_transaction_manager(prisma_client) as tx:
        ...
        row: Final[prisma_db_models.DeRouter_MCPUserEnvVars | None] = await tx.derouter_mcpuserenvvars.find_unique(
            where={"user_id_server_id": {"user_id": user_id, "server_id": server_id}}
        )
        ...
        await tx.derouter_mcpuserenvvars.upsert(
            where={"user_id_server_id": {"user_id": user_id, "server_id": server_id}},
            data={
                "create": {
                    "user_id": user_id,
