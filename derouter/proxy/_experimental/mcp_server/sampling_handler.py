...
routing them through DeRouter's internal completion infrastructure.
...
DeRouter acts as the LLM provider using its existing 100+ provider support,
...
def _resolve_model_from_preferences(
    ...
    Returns:
        A model string suitable for derouter.acompletion().
    """
    import derouter

    # Build list of available model names from proxy Router or derouter.model_list
    ...
    try:
        ...
        pass
    if not available_model_names and derouter.model_list:
        for entry in derouter.model_list:
    ...
    # Last resort - use DeRouter default or raise error
    default_sampling_model: Final[str | None] = getattr(derouter, "default_mcp_sampling_model", None)
    if default_sampling_model:
        verbose_logger.debug(
            "MCP sampling model resolution: using derouter.default_mcp_sampling_model='%s'",
            default_sampling_model,
        )
        return default_sampling_model
    raise ValueError(
        "No model could be resolved for MCP sampling. Please configure 'default_mcp_sampling_model' in your DeRouter configuration."
    )
...
def _select_model_by_priority(
    ...
    * **intelligencePriority** — higher means "prefer smarter models".
    ...
    for name in model_names:
        try:
            info = _derouter.get_model_info(name)
        except Exception:
            continue
...
def _convert_openai_response_to_mcp_result(
    ...
    """
    Convert a derouter completion response to MCP CreateMessageResult.
    Args:
        response: The derouter ModelResponse.
...
async def _check_model_access(model: str, user_api_key_auth: "UserAPIKeyAuth | None") -> Optional["ErrorData"]:
    ...
    if not _has_real_credential and not _is_admin:
        verbose_logger.warning(
            ...
            "auth context has no real DeRouter credential (possible "
        ...
        return ErrorData(
            code=-1,
            message=(
                "Model access denied: sampling requires a valid DeRouter "
    ...
    try:
        ...
        try:
            from derouter.proxy.proxy_server import llm_router as _llm_router
        except ImportError:
            _llm_router = None

        await can_key_call_model(
            model=model,
            llm_model_list=getattr(derouter, "model_list", None),
...
async def _run_budget_checks(
    ...
    global_proxy_spend: Final = getattr(derouter, "_global_proxy_spend", None)

    # Build request body and merge x-derouter-tags from MCP headers BEFORE
...
def _build_sampling_request(
    ...
    from derouter.proxy._experimental.mcp_server.utils import build_synthetic_mcp_request
...
async def _run_guardrails_and_call_llm(
    ...
    try:
        from derouter.proxy.proxy_server import proxy_logging_obj as _plo
    ...
    try:
        ...
        return await _AcompletionCall(fn=derouter.acompletion).fn(**completion_kwargs)
    except ImportError:
        return await _AcompletionCall(fn=derouter.acompletion).fn(**completion_kwargs)
...
async def handle_sampling_create_message(
    ...
    """
    Handle an MCP sampling/createMessage request by routing through DeRouter.
    ...
    try:
        ...
        verbose_logger.debug(
            "MCP sampling: calling derouter.acompletion with model=%s, num_messages=%d, has_tools=%s",
            model,
            len(openai_messages),
            bool(openai_tools),
        )
