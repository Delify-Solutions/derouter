...
from derouter.constants import MCP_CLIENT_TIMEOUT, MCP_TOOL_LISTING_TIMEOUT
...
def _connection_error_message(exc: BaseException, url: str | None, timeout_seconds: float) -> str:
    ...
    if isinstance(exc, TimeoutError):
        return (
            ...
            f"within {timeout_seconds:.0f}s. Check that the DeRouter proxy can reach this URL "
...
if MCP_AVAILABLE:
    ...
    from derouter.llms.derouter_proxy.skills.skill_search import (
    ...
    async def _handle_virtual_mcp_tool(
        ...
        from derouter.proxy.proxy_server import general_settings, proxy_config, proxy_logging_obj
    ...
    async def call_tool_rest_api(
        ...
        try:
            ...
            from derouter.proxy._experimental.mcp_server.tool_search import VIRTUAL_TOOL_NAMES
            ...
            result: Final = await execute_mcp_tool(
                ...
                derouter_logging_obj=data.get("derouter_logging_obj"),
    ...
    def _stage_server_test(new_mcp_server_request: NewMCPServerRequest, headers: Headers) -> _StagedServerTest:
        ...
        # Authorization doubles as the admission fallback (DEROUTER_API_KEY_HEADER_NAME_SECONDARY):
        # when the primary x-derouter-api-key header is absent, the Authorization value is the
        # caller's DeRouter key, not an upstream token, and must never be forwarded upstream.
        oauth2_headers: Final = (
            ...
            and headers.get(MCPRequestHandler.DEROUTER_API_KEY_HEADER_NAME_PRIMARY)
            else None
        )
    ...
    async def _execute_with_mcp_client(
        ...
        Args:
            ...
            oauth2_headers: Headers extracted from the incoming request (may contain the
                derouter API key — must NOT be forwarded for M2M servers).
        ...
        try:
            ...
            from derouter.proxy._experimental.mcp_server.outbound_credentials import (  # noqa: PLC0415
                UpstreamCredentialProvider,
            )
            from derouter.proxy._experimental.mcp_server.outbound_credentials.adapter import (  # noqa: PLC0415
                to_server_spec,
            )
            from derouter.proxy._experimental.mcp_server.outbound_credentials.oauth_token_store import (  # noqa: PLC0415
                OAuthToken,
            )
            from derouter.proxy._experimental.mcp_server.outbound_credentials.presented_token_store import (  # noqa: PLC0415
