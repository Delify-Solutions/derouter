"""
Agent endpoints for registering + discovering agents via DeRouter.
...
from derouter.proxy.agent_endpoints.agent_registry import (
    parse_agent_derouter_params,
    redact_sensitive_agent_derouter_params,
)
from derouter.proxy.agent_endpoints.agent_search import (
...
from derouter.proxy.agent_endpoints.auth.agent_permission_handler import accessible_agents
...
def _build_merged_agent_card(
    ...
    *,
    ...
    """Apply the DeRouter-fronting merge to ``upstream_card`` for ``agent_id``."""
...
async def _attach_keys_to_agents(agents: Sequence[AgentResponse], prisma_client) -> None:
    ...
    if not agent_ids:
        return
    key_rows: Final = await prisma_client.db.derouter_verificationtoken.find_many(
...
def _redact_agent_derouter_params_dict(
    derouter_params: Mapping[str, object],
) -> dict[str, object]:  # mutable-ok: AgentResponse.derouter_params is declared as a plain dict, not Mapping
    """Type-narrowing wrapper: a dict in always yields a dict back from
    ``redact_sensitive_agent_derouter_params``, which the function's general
    (possible-JSON-string, possibly-None) signature can't express."""
    return dict(  # mutable-ok: AgentResponse.derouter_params is declared as a plain dict, not Mapping
        parse_agent_derouter_params(redact_sensitive_agent_derouter_params(derouter_params))
    )
...
def _redact_sensitive_agent_fields(
    ...
    *,
    ...
    """
    Return copies of the given agents with credential-bearing derouter_params
    ...
    for agent in agents:
        ...
        if not is_admin:
            ...
            copy.keys = None
        if copy.derouter_params:
            copy.derouter_params = _redact_agent_derouter_params_dict(copy.derouter_params)
...
AGENT_HEALTH_CHECK_TIMEOUT_SECONDS: Final = float(os.environ.get("DEROUTER_AGENT_HEALTH_CHECK_TIMEOUT", "5.0"))
AGENT_HEALTH_CHECK_GATHER_TIMEOUT_SECONDS = float(os.environ.get("DEROUTER_AGENT_HEALTH_CHECK_GATHER_TIMEOUT", "30.0"))
...
async def _rank_agents_by_query(
    ...
    from derouter.proxy.proxy_server import llm_router, proxy_logging_obj
    ...
    outcome: Final = await search_agents(
        ...
        top_k=top_k,
        router=llm_router,
        embedding_model=derouter.agent_search_embedding_model,
...
async def get_agents(
    ...
    query: Annotated[
        ...
        Query(
            min_length=1,
            description="Describe the task in natural language to rank the agents you can reach by semantic similarity over their name, description, and skills. Each result carries a search_score. Requires derouter_settings.agent_search_embedding_model.",
        ),
    ] = None,
    top_k: Annotated[
        int,
    ...
    try:
        ...
        for agent in returned_agents:
            if agent.derouter_params is None:
                agent.derouter_params = {}
            agent.derouter_params["is_public"] = derouter.public_agent_groups is not None and not (
                global_agent_registry.ids_for_agent(agent.agent_id).isdisjoint(derouter.public_agent_groups)
            )

        # derouter_params secrets are always redacted; keys/headers stay
        # admin-only.
        is_admin: Final = (
        ...
        verbose_proxy_logger.exception("derouter.proxy.agent_endpoints.get_agents(): Exception occurred - %s", e)
...
async def create_agent(
    ...
    try:
        ...
        # Apply the DeRouter-fronting merge only when the admin actually
        ...
        # The caller is a proxy admin (enforced above); derouter_params
...
async def update_agent(
    ...
    try:
        ...
        # Re-apply the DeRouter-fronting merge — an update is a re-registration,
...
async def patch_agent(
    ...
    try:
        ...
        # patch updating just derouter_params/rate limits (``agent_card_params``
        ...
        # merge so DeRouter applies its security schemes and supported
...
async def make_agent_public(
    ...
    try:
        ...
        current_public_agent_groups: Final = list(derouter.public_agent_groups or [])
        ...
        config["derouter_settings"]["public_agent_groups"] = updated_public_agent_groups

        await proxy_config.save_config(new_config=config)

        derouter.public_agent_groups = updated_public_agent_groups
...
async def make_agents_public(
    ...
    try:
        ...
        if derouter.public_agent_groups is None:
            derouter.public_agent_groups = []
        ...
        derouter.public_agent_groups = request.agent_ids

        # Update config with new settings
        if "derouter_settings" not in config or config["derouter_settings"] is None:
            config["derouter_settings"] = {}

        config["derouter_settings"]["public_agent_groups"] = derouter.public_agent_groups

        # Save the updated config
        await proxy_config.save_config(new_config=config)

        verbose_proxy_logger.debug(
            "Updated public agent groups to: %s by user: %s", derouter.public_agent_groups, user_api_key_dict.user_id
        )

        return {
            "message": "Successfully updated public agent groups",
            "public_agent_groups": derouter.public_agent_groups,
            "updated_by": user_api_key_dict.user_id,
        }
    except HTTPException:
        raise
...
async def get_agent_daily_activity(
    ...
    return await get_daily_activity(
        prisma_client=prisma_client,
        table_name="derouter_dailyagentspend",
