...
from derouter.repositories.table_repositories import AgentsRepository, ObjectPermissionRepository
from derouter.types.agents import AgentConfig, AgentResponse, PatchAgentRequest
...
class AgentRecord(Protocol):
    ...
    @property
    def derouter_params(self) -> Mapping[str, object] | None: ...
...
def object_permission_table(
    ...
    table: Final[TableActions[prisma_models.DeRouter_ObjectPermissionTable]] = ObjectPermissionRepository(
        prisma_client
    ).table
    return table
...
_AGENT_PARAMS_ADAPTER: Final[TypeAdapter[dict[str, object]]] = TypeAdapter(
    dict[str, object]
)  # mutable-ok: safe_dumps() and AgentResponse.derouter_params both require a real dict, not a Mapping
_AGENT_PARAMS_SEQUENCE_ADAPTER: Final[TypeAdapter[tuple[object, ...]]] = TypeAdapter(tuple[object, ...])
_EMPTY_DEROUTER_PARAMS: Final[Mapping[str, object]] = MappingProxyType({})


def redact_sensitive_agent_derouter_params(derouter_params: object, _depth: int = 0) -> object:
    """
    Replace credential-bearing values in an agent's derouter_params with
    ...
    if isinstance(derouter_params, str):
        ...
        try:
            parsed_params: Final = _AGENT_PARAMS_ADAPTER.validate_json(derouter_params)
        except ValidationError:
            return REDACTED_BY_LITELM_STRING
        return json.dumps(_redact_agent_params_tree(parsed_params, _depth + 1))
    return _redact_agent_params_tree(derouter_params, _depth)


def _redact_agent_params_tree(value: object, _depth: int) -> object:
    """Structural recursion over an already-parsed derouter_params value: a
    ...
    }  # mutable-ok: consumed by json.dumps()/AgentResponse.derouter_params, both of which require a real dict


def parse_agent_derouter_params(value: object) -> Mapping[str, object]:
    """Normalize a stored derouter_params column to a read-only mapping.
...
def _restore_redacted_nested_value(incoming_value: object, existing_value: object, _depth: int) -> object:
    ...
    ``redact_sensitive_agent_derouter_params`` recurses into on read, so
    ...
    if isinstance(incoming_value, Mapping):
        ...
        existing_map: Final = (
            ...
            else _EMPTY_DEROUTER_PARAMS
        )
        return _restore_redacted_derouter_params(typed_incoming_map, existing_map, _depth + 1)
...
def _resolved_agent_param_value(
    ...
    """The value ``key`` should end up with in a restored derouter_params, or
...
def _restore_redacted_derouter_params(
    incoming: Mapping[str, object],
    existing: Mapping[str, object],
    _depth: int = 0,
) -> dict[str, object]:
    """Restore the real credential behind any derouter_params value the caller
...
class AgentRegistry:
    ...
    def get_public_agent_list(self) -> tuple[AgentResponse, ...]:
        public_agent_groups: Final = derouter.public_agent_groups
    ...
    async def migrate_legacy_grant_ids(
        self, table: "TableActions[prisma_models.DeRouter_ObjectPermissionTable]"
    ...
    async def add_agent_to_db(
        ...
        try:
            ...
            # Serialize derouter_params. A create has no stored row to restore a
            ...
            derouter_params_obj: Final = agent.get("derouter_params", {})
            derouter_params_dict: Final = _restore_redacted_derouter_params(
                _dump_agent_params(derouter_params_obj), _EMPTY_DEROUTER_PARAMS
            )
    ...
    async def patch_agent_in_db(
        ...
        try:
            ...
            if "derouter_params" in agent:
                existing_derouter_params: Final = parse_agent_derouter_params(existing_agent.get("derouter_params"))
                update_data["derouter_params"] = safe_dumps(
                    _restore_redacted_derouter_params(
                        _dump_agent_params(agent.get("derouter_params") or _EMPTY_DEROUTER_PARAMS),
                        existing_derouter_params,
                    )
                )
    ...
    async def update_agent_in_db(
        ...
        try:
            ...
            # A PUT fully replaces derouter_params from the request body, so the
            ...
            existing_derouter_params: Final = parse_agent_derouter_params(
                existing_row.derouter_params if existing_row is not None else None
            )

            # Serialize derouter_params
            derouter_params_obj: Final = agent.get("derouter_params", {})
            derouter_params_dict: Final = _restore_redacted_derouter_params(
                _dump_agent_params(derouter_params_obj), existing_derouter_params
            )
            derouter_params: Final[str] = safe_dumps(derouter_params_dict)
