...
registered with a ``databricks_oauth`` block in its ``derouter_params``, DeRouter
...
def parse_databricks_oauth_config(
    ...
    """Build a Databricks App OAuth config from an agent's ``derouter_params``.
    ...
    raw: Final = derouter_params.get(DATABRICKS_OAUTH_PARAM)
    if raw is None:
        return None
...
async def resolve_databricks_app_auth_header(
    ...
    """
    config: Final = parse_databricks_oauth_config(derouter_params)
    if config is None:
        return None
