...
"""
A2A Protocol endpoints for DeRouter Proxy.

Allows clients to invoke agents through DeRouter using the A2A protocol.
The A2A SDK can point to DeRouter's URL and invoke agents registered with DeRouter.
"""
...
from derouter.derouter_core_utils.url_utils import SSRFError, validate_url
from derouter.proxy._types import UserAPIKeyAuth
from derouter.proxy.a2a.version_convert import (
    A2AVersion,
...
from derouter.proxy.agent_endpoints.databricks_oauth import (
    DATABRICKS_OAUTH_PARAM,
    resolve_databricks_app_auth_header,
)
from derouter.proxy.agent_endpoints.utils import merge_agent_headers
...
from derouter.proxy.utils import ProxyLogging, get_custom_url
...
def _served_version(agent: "AgentResponse", request: Request, original_method: str | None = None) -> A2AVersion:
    """Protocol version DeRouter serves for this agent.
    ...
    return MappingProxyType(
        {
            name: value
            for name, value in (
                ("X-DeRouter-User-Id", user_api_key_dict.user_id),
                ("X-DeRouter-Team-Id", user_api_key_dict.team_id),
            )
            if value
        }
    )
...
def _forwarding_headers(
    ...
    passthrough: Final = tuple(
        ...
        if not name.lower().startswith("x-derouter-")
    )
    trace_id: Final = request_data.get("derouter_trace_id")
    trace: Final = (("X-DeRouter-Trace-Id", str(trace_id)),) if trace_id else ()
...
def _enforce_inbound_trace_id(agent: "AgentResponse", request: Request) -> None:
    """Raise 400 if agent requires x-derouter-trace-id on inbound calls and it is missing."""
    agent_derouter_params: Final = agent.derouter_params or {}
    if not agent_derouter_params.get("require_trace_id_on_calls_to_agent"):
        return
    ...
    if not trace_id:
        raise HTTPException(
            status_code=400,
            detail=(f"Agent '{agent.agent_id}' requires x-derouter-trace-id header on all inbound requests."),
        )
...
async def _a2a_sse_event_source(
    ...
    """
    from derouter.llms.custom_httpx.http_handler import get_async_httpx_client
    from derouter.types.agents import _normalize_a2a_jsonrpc_response
    from derouter.types.llms.custom_http import httpxSpecialProvider
...
def _sse_streaming_response(generator: AsyncGenerator[str, None]) -> StreamingResponse:
    ...
    keepalive_interval: Final = coerce_keepalive_interval(derouter.sse_keepalive_ping_interval_seconds)
...
async def _handle_stream_message(
    ...
    *,
    ...
    from derouter.a2a_protocol.main import A2A_SDK_AVAILABLE
...
async def get_agent_card(
    ...
    The URL in the agent card is rewritten to point to the DeRouter proxy,
    so all subsequent A2A calls go through DeRouter for logging and cost tracking.
    """
...
async def invoke_agent_a2a(
    ...
    try:
        ...
        if isinstance(params, dict):
            # extract any derouter params from the params - eg. 'guardrails'
            ...
            # collide with derouter's spend-tracking ``metadata`` kwarg and
            # silently drop the caller's A2A request-level metadata.
            params_to_remove: Final = []
            for key, value in params.items():
                if key in all_derouter_params and key not in {"id", "metadata"}:
        ...
        # Get derouter_params (may include custom_llm_provider for completion bridge)
        derouter_params: dict[str, object] = agent.derouter_params or {}
        ...
        # Set up data dict for derouter processing
        ...
        # Add derouter data (user_api_key, user_id, team_id, etc.)
        from derouter.proxy.common_request_processing import (
            ProxyBaseLLMRequestProcessing,
        )
        ...
        if derouter_params.get(DATABRICKS_OAUTH_PARAM):
            databricks_auth: Final = await resolve_databricks_app_auth_header(derouter_params)
            if databricks_auth:
                agent_extra_headers = {
        ...
        _agent_guardrails = derouter_params.get("guardrails")
        ...
        if method == "message/send":
            ...
            response = DeRouterSendMessageResponse.from_dict(result, request_id=request_id)
            response = await proxy_logging_obj.post_call_success_hook(
                user_api_key_dict=user_api_key_dict,
                data=data,
                response=response,
