# This file runs a health check for the LLM, used on derouter/proxy
...
ILLEGAL_DISPLAY_PARAMS: Final = [
    ...
    "derouter_metadata",  # internal tracking metadata with auth objects; not for display
]
...
def _resolve_health_check_mode(model_info: Mapping[str, object], derouter_params: Mapping[str, object]) -> str | None:
    """
    ...
    if not isinstance(model, str):
        return None
    try:
        return derouter.get_model_info(model=model).get("mode")
    except Exception:
        return None
...
def _clean_endpoint_data(endpoint_data: dict, details: bool | None = True):
    ...
    """
    endpoint_data.pop("derouter_logging_obj", None)
    return (
...
async def run_with_timeout(task, timeout):
    try:
        ...
        timeout_exception: Final = derouter.Timeout(
...
def _deployment_model(deployment: Mapping[str, object]) -> str | None:
    params: Final = deployment.get("derouter_params")
...
def _is_strategy_router_deployment(derouter_params: Mapping[str, object]) -> bool:
    """True for strategy-router deployments."""
    model: Final[object] = derouter_params.get("model", "")
...
def _dependency_deployments_to_probe(
    ...
    for _ in range(len(universe)):
        names = frozenset(
            ...
            if isinstance(params := deployment.get("derouter_params"), Mapping)
            for dependency in strategy_router_dependencies(params)
        )
        fresh_ids = (
...
async def _run_model_health_check(model: dict):
    ...
    if _is_strategy_router_deployment(derouter_params):
        return {}

    mode: Final = _resolve_health_check_mode(
        model_info,
        derouter_params,  # any-ok: untyped router config dict
    )
    derouter_params = _update_derouter_params_for_health_check(model_info, derouter_params)
...
async def _perform_health_check(
    ...
    for is_healthy, model in zip(results, model_list):
        ...
        if isinstance(is_healthy, dict) and "error" not in is_healthy:
            cleaned = _clean_endpoint_data({**derouter_params, **is_healthy}, details)
            if _model_id:
            ...
            cleaned = _clean_endpoint_data(derouter_params, details)
            if _model_id:
                cleaned["model_id"] = _model_id
...
def _deployment_model_string_for_health_check(derouter_params: dict) -> str:
    """Deployment model from derouter_params (before Bedrock rewrite).
    ...
    """
    return derouter_params.get("model") or ""


def _health_check_deployment_is_wildcard(derouter_params: dict) -> bool:
    return "*" in _deployment_model_string_for_health_check(derouter_params)


def _resolve_health_check_max_tokens(model_info: dict, derouter_params: dict) -> int | None:
    """
    ...
    2. For non-wildcard routes: health_check_max_tokens_reasoning / _non_reasoning
       from model_info based on derouter.supports_reasoning(derouter_params["model"])
    ...
    is_wildcard: Final = _health_check_deployment_is_wildcard(derouter_params)
    deployment_model: Final = _deployment_model_string_for_health_check(derouter_params)

    if not is_wildcard:
        try:
            is_reasoning = derouter.supports_reasoning(deployment_model)
        except Exception:
            is_reasoning = False
...
def _update_derouter_params_for_health_check(model_info: dict, derouter_params: dict) -> dict:
    """
    Update the derouter params for health check.

    - merges `model_info.health_check_params` into the probe request, so a deployment whose provider
      requires a payload field derouter does not synthesize (e.g. `mediaSource` for Bedrock TwelveLabs
    ...
    if mode in _HEALTH_CHECK_MODES_SUPPORTING_REASONING_EFFORT:
        ...
        if _hc_reasoning_effort is not None:
            derouter_params["reasoning_effort"] = _hc_reasoning_effort

    _health_check_model: Final = model_info.get("health_check_model", None)
    if _health_check_model is not None:
        derouter_params["model"] = _health_check_model
    if mode == "audio_speech":
        derouter_params["voice"] = model_info.get("health_check_voice", "alloy")
    ...
    if derouter_params["model"].startswith("bedrock/"):
        ...
        if not derouter_params.get("custom_llm_provider"):  # any-ok: untyped router dict
            derouter_params["custom_llm_provider"] = (  # any-ok: untyped router dict
                "bedrock"
            )
...
async def perform_health_check(
    ...
    if not model_list:
        if cli_model:
            model_list = [{"model_name": cli_model, "derouter_params": {"model": cli_model}}]
        else:
            if instrumentation_enabled:
                logger.debug(
