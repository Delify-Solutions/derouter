...
from derouter.types.llms.openai import BATCH_GUARDRAIL_RESPONSE_FIELD, OpenAIFileObject
from derouter.types.passthrough_endpoints.managed_id_rewriter import (
    ManagedFileIdReader,
    ManagedFileIdWriter,
    ManagedFileRow,
    ManagedFileTable,
...
# DeRouter_ManagedFileTable (flat_model_file_ids has no index), so a body packed
...
def _is_derouter_internal_key(key: object) -> bool:
    return isinstance(key, str) and key.startswith("derouter_")
...
async def rewrite_body_ids(
    ...
    managed IDs.  Skips derouter internal keys (``derouter_*``).
    ...
    async def _walk_mapping(node: dict[str, object], depth: int) -> dict[str, object]:
        ...
        for k, v in node.items():
            # Skip derouter internal injection keys (e.g. derouter_logging_obj)
            if _is_derouter_internal_key(k):
                result[k] = v
                continue
