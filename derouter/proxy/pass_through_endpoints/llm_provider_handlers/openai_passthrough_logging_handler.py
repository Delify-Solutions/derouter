...
from derouter.llms.openai.openai import OpenAIConfig
from derouter.llms.openai.openai import OpenAIConfig as OpenAIConfigType
...
from derouter.proxy.pass_through_endpoints.llm_provider_handlers.base_passthrough_logging_handler import (
...
from derouter.types.utils import EmbeddingResponse, ImageResponse, LlmProviders, PassthroughCallTypes
from derouter.utils import ModelResponse, TextCompletionResponse, convert_to_model_response_object
...
# `/v1/` is the OpenAI-v1 surface used by DeRouter's pass-through routing. Other
...
class OpenAIPassthroughLoggingHandler(BasePassthroughLoggingHandler):
    ...
    def _calculate_image_generation_cost(
        ...
        try:
            ...
            # Use DeRouter's default image cost calculator
    ...
    def _calculate_embeddings_cost(
        derouter_model_response: EmbeddingResponse,
        model: str,
    ...
    def _build_responses_api_response_and_cost(
        ...
        Returns (derouter_model_response, response_cost) — symmetric with the
        ...
        derouter_model_response: Final = responses_config.transform_response_api_response(
            model=model,
        ...
        return derouter_model_response, response_cost
        ...
        **kwargs,
        ...
        try:
            ...
            if is_chat_completions:
                ...
                # Preserve existing derouter_params to maintain metadata tags
                existing_derouter_params: Final = kwargs.get("derouter_params", {}) or {}
                derouter_model_response = provider_config.transform_response(
                    raw_response=httpx_response,
                    ...
                    derouter_params=existing_derouter_params,
                )
                ...
                derouter_model_response = convert_to_model_response_object(
                ...
                derouter_model_response = ImageResponse(
            ...
            if passthrough_logging_payload:
                ...
                if user:
                    kwargs["derouter_params"].setdefault("proxy_server_request", {}).setdefault("body", {})["user"] = (
                        user
                    )
    ...
    def _build_complete_streaming_response(
        ...
        try:
            ...
            from derouter.llms.openai.openai import OpenAIChatCompletionResponseIterator
    ...
    def _handle_logging_openai_collected_chunks(
        ...
        try:
            ...
            custom_llm_provider: Final = derouter_logging_obj.model_call_details.get("custom_llm_provider", "openai")
            # Calculate cost using DeRouter's cost calculator
            response_cost: Final = (
            ...
            # Preserve existing derouter_params to maintain metadata tags
            existing_derouter_params: Final = derouter_logging_obj.model_call_details.get("derouter_params", {}) or {}
            ...
            kwargs: Final = {
                ...
                "model": model,
                "custom_llm_provider": custom_llm_provider,
                "call_type": derouter_logging_obj.call_type,
                "messages": derouter_logging_obj.model_call_details.get("messages"),
                "derouter_params": existing_derouter_params.copy(),
            }

            # Extract user information for tracking
            passthrough_logging_payload: Final[PassthroughStandardLoggingPayload | None] = (
                derouter_logging_obj.model_call_details.get("passthrough_logging_payload")
            )
            if passthrough_logging_payload:
                user: Final = handler_instance._get_user_from_metadata(
                    passthrough_logging_payload=passthrough_logging_payload,
            ...
            derouter_logging_obj.model_call_details["response_cost"] = response_cost
