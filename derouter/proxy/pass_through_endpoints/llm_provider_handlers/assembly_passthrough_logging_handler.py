...
from derouter.types.passthrough_endpoints.assembly_ai import (
...
class AssemblyAIPassthroughLoggingHandler:
    def __init__(self):
        ...
        derouter needs to poll the GET /transcript/{transcript_id} endpoint to get the status of the transcript.
        """
    ...
    def _should_log_request(request_method: str) -> bool:
        """
        only POST transcription jobs are logged. derouter will POLL assembly to wait for the transcription to complete to log the complete response / cost
        """
