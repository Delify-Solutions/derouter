...
from derouter.constants import VERTEX_BATCH_PREDICTION_JOBS_ROUTE
...
class VertexPassthroughLoggingHandler:
    ...
    def vertex_passthrough_handler(
        ...
        **kwargs,
        ...
        if "predictLongRunning" in url_route:
            ...
            derouter_video_response: Final = vertex_video_config.transform_video_create_response(
                model=model,
            ...
            response_cost = derouter.completion_cost(
            ...
            instance_of_vertex_llm: Final = derouter.VertexGeminiConfig()
            derouter_model_response: Final[ModelResponse] = instance_of_vertex_llm.transform_response(
                model=model,
            ...
            from derouter.llms.vertex_ai.vertex_ai_partner_models import (
            ...
            derouter_prediction_response = ModelResponse()
            ...
            if vertex_publisher_or_api_spec is not None:
                ...
                derouter_prediction_response = vertex_ai_partner_model_config.transform_response(
                    model=model,
                    raw_response=httpx_response,
                    model_response=derouter_prediction_response,
                    logging_obj=logging_obj,
                    request_data={},
            ...
            kwargs = VertexPassthroughLoggingHandler._create_vertex_response_logging_payload_for_generate_content(
                derouter_model_response=derouter_prediction_response,
            ...
            return {
                "result": derouter_prediction_response,
                "kwargs": kwargs,
            }
        elif "search" in url_route:
            derouter_vs_response: Final = vertex_search_api_config.transform_search_vector_store_response(
                response=httpx_response,
                derouter_logging_obj=logging_obj,
            )
            response_cost = derouter.completion_cost(
                completion_response=derouter_vs_response,
            ...
            standard_pass_through_response_object: Final[StandardPassThroughResponseObject] = {
                "response": cast(dict, derouter_vs_response),
            }

            kwargs["response_cost"] = response_cost
            kwargs["model"] = "vertex_ai/search_api"
            logging_obj.model_call_details.setdefault("derouter_params", {})
            logging_obj.model_call_details["derouter_params"]["base_model"] = "vertex_ai/search_api"
            logging_obj.model_call_details["response_cost"] = response_cost
    ...
    def _handle_predict_response(
        ...
        from derouter.llms.vertex_ai.image_generation.image_generation_handler import (
            VertexImageGeneration,
        )
        from derouter.llms.vertex_ai.multimodal_embeddings.transformation import (
            VertexAIMultimodalEmbeddingConfig,
        )
        from derouter.types.utils import PassthroughCallTypes
        ...
        derouter_prediction_response: ModelResponse | EmbeddingResponse | ImageResponse = ModelResponse()
        ...
        if vertex_image_generation_class.is_image_generation_response(_json_response):
            derouter_prediction_response = vertex_image_generation_class.process_image_generation_response(
                _json_response,
            ...
            derouter_prediction_response = vertex_multimodal_config.transform_embedding_response(
                model=model,
                raw_response=httpx_response,
                model_response=derouter.EmbeddingResponse(),
                logging_obj=logging_obj,
                api_key="",
                request_data={},
                optional_params={},
            ...
            derouter_prediction_response = derouter.vertexAITextEmbeddingConfig.transform_vertex_response_to_openai(
                response=_json_response,
                model=model,
                model_response=derouter.EmbeddingResponse(),
            )
        if isinstance(derouter_prediction_response, derouter.EmbeddingResponse):
            derouter_prediction_response.model = model
        ...
        response_cost: Final = derouter.completion_cost(
            completion_response=derouter_prediction_response,
            model=model,
            custom_llm_provider="vertex_ai",
    ...
    def _handle_embed_content_response(
        ...
        from derouter.llms.vertex_ai.gemini_embeddings.batch_embed_content_transformation import (
        ...
        model_response: Final = derouter.EmbeddingResponse()
        if is_batch:
            derouter_embedding_response = process_batch_embed_response(
                input=input_text,
            ...
            derouter_embedding_response = process_embed_content_response(
                input=input_text,
                model_response=model_response,
                model=model,
        ...
        derouter_embedding_response.model = model
        logging_obj.model = model
        logging_obj.model_call_details["model"] = model
        ...
        response_cost: Final = derouter.completion_cost(
            completion_response=derouter_embedding_response,
            model=model,
        ...
        return {
            "result": derouter_embedding_response,
            "kwargs": kwargs,
        }
    ...
    def _handle_logging_vertex_collected_chunks(
        ...
        """
        Takes raw chunks from Vertex passthrough endpoint and logs them in derouter callbacks
        ...
        vertex_location: Final = get_vertex_location_from_url(url_route)
        if vertex_location is not None:
            derouter_logging_obj.optional_params["vertex_location"] = vertex_location
    ...
    def extract_model_name_from_vertex_path(vertex_model_path: str) -> str:
        ...
        Returns:
            The extracted model name for use with DeRouter
        """
    ...
    def _get_custom_llm_provider_from_url(url: str) -> str:
        ...
        if parsed_url.hostname and parsed_url.hostname.endswith("generativelanguage.googleapis.com"):
            return derouter.LlmProviders.GEMINI.value
        return derouter.LlmProviders.VERTEX_AI.value
    ...
    def batch_prediction_jobs_handler(
        ...
        **kwargs,
        ...
        from derouter.llms.vertex_ai.batches.transformation import (
        ...
        try:
            ...
            if httpx_response.status_code == 200 and "name" in _json_response:
                # Transform Vertex AI response to DeRouter batch format
                derouter_batch_response: Final = (
                ...
                # Format: base64(derouter_proxy;model_id:{};llm_batch_id:{})
                actual_model_id: Final = VertexPassthroughLoggingHandler.get_actual_model_id_from_router(model_name)

                unified_id_string: Final = SpecialEnums.DEROUTER_MANAGED_BATCH_COMPLETE_STR.value.format(
                ...
                VertexPassthroughLoggingHandler._store_batch_managed_object(
                    ...
                    batch_object=derouter_batch_response,
                ...
                derouter_model_response.id = str(uuid.uuid4())
                derouter_model_response.model = model_name
                derouter_model_response.object = "batch_prediction_job"
                derouter_model_response.created = int(start_time.timestamp())

                # Add batch-specific metadata to indicate this is a pending batch job
                derouter_model_response.choices = [
                    Choices(
                        finish_reason="stop",
                        index=0,
                        message={
                ...
                derouter_model_response = ModelResponse()
                derouter_model_response.id = str(uuid.uuid4())
                derouter_model_response.model = "vertex_ai_batch"
                derouter_model_response.object = "batch_prediction_job"
                derouter_model_response.created = int(start_time.timestamp())
        ...
        **kwargs,
        ...
        try:
            ...
            if managed_files_hook is not None and hasattr(managed_files_hook, "store_unified_object_id"):
                ...
                _request_metadata: Final = (kwargs.get("derouter_params", {}) or {}).get("metadata", {}) or {}
