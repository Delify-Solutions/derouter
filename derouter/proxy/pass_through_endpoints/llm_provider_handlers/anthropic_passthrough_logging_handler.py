...
from derouter.constants import ANTHROPIC_BATCHES_ROUTE
from derouter.derouter_core_utils.core_helpers import map_finish_reason
...
from derouter.llms.anthropic import get_anthropic_config
from derouter.llms.anthropic.chat.handler import (
...
class AnthropicPassthroughLoggingHandler:
    ...
    def anthropic_passthrough_handler(
        ...
        **kwargs,
        ...
        derouter_model_response: Final[ModelResponse] = anthropic_config().transform_response(
    ...
    @staticmethod
    def _resolve_costing_model(model: str, logging_obj: DeRouterLoggingObj) -> str:
        if model and model != "unknown":
            return model
        derouter_params: Final = (getattr(logging_obj, "model_call_details", {}) or {}).get("derouter_params", {}) or {}
        deployment_model: Final = derouter_params.get("model")
        if deployment_model and deployment_model != "unknown":
            return deployment_model
        model_group: Final = (derouter_params.get("metadata", {}) or {}).get("model_group")
        if model_group:
    ...
    def _resolve_logged_model(
        ...
        logged_model: Final = (
            ...
            else str(derouter_logging_obj.model_call_details.get("model") or "")
        )
    ...
    def record_partial_usage_for_failure(
        ...
        model: Final = AnthropicPassthroughLoggingHandler._resolve_logged_model(
            derouter_logging_obj, request_body, all_chunks
        )
        partial_response: Final = AnthropicPassthroughLoggingHandler._build_streaming_response_for_logging(
            derouter_logging_obj=derouter_logging_obj, request_body=request_body, all_chunks=all_chunks, model=model
        )
        usage: Final = cast(Usage | None, getattr(partial_response, "usage", None))
        if partial_response is None or usage is None:
            return
        derouter_logging_obj.record_partial_usage_for_failure(
            usage=usage,
            response_cost=AnthropicPassthroughLoggingHandler._cost_partial_stream_or_zero(
                partial_response=partial_response, model=model, logging_obj=derouter_logging_obj
            ),
        )

    @staticmethod
    def _cost_partial_stream_or_zero(
        partial_response: ModelResponse | TextCompletionResponse, model: str, logging_obj: DeRouterLoggingObj
    ) -> float:
        try:
            return AnthropicPassthroughLoggingHandler._compute_response_cost(
                derouter_model_response=partial_response,
    ...
    def _compute_response_cost(
        ...
        return derouter.completion_cost(
            ...
            custom_pricing=use_custom_pricing_for_model(
                derouter_params=(logging_obj.derouter_params if hasattr(logging_obj, "derouter_params") else None)
            ),
    ...
    def _recover_interrupted_stream_output_tokens(
        ...
        if not output_text:
            return
        try:
            recovered_output_tokens = derouter.token_counter(model=model, text=output_text, count_response_tokens=True)
        except Exception:
            verbose_proxy_logger.warning(
    ...
    def _create_anthropic_response_logging_payload(
        ...
        if logging_obj.model_call_details.get("stream") is True:
            logging_obj.model_call_details["complete_streaming_response"] = derouter_model_response
        try:
            model = AnthropicPassthroughLoggingHandler._resolve_costing_model(model, logging_obj)
            response_cost: Final = AnthropicPassthroughLoggingHandler._compute_response_cost(
                derouter_model_response=derouter_model_response, model=model, logging_obj=logging_obj
            )
            ...
            derouter_model_response.id = response_id or logging_obj.derouter_call_id
            derouter_model_response.model = model
            logging_obj.model_call_details["model"] = model
            if not logging_obj.model_call_details.get("custom_llm_provider"):
                logging_obj.model_call_details["custom_llm_provider"] = derouter.LlmProviders.ANTHROPIC.value
            return kwargs
    ...
    def _handle_logging_anthropic_collected_chunks(
        ...
        """
        Takes raw chunks from Anthropic passthrough endpoint and logs them in derouter callbacks
    ...
    def _build_complete_streaming_response_legacy(
        ...
        - Converts generic chunks to derouter chunks (OpenAI format)
        - Builds complete response from derouter chunks
        """
        ...
        complete_streaming_response: Final = derouter.stream_chunk_builder(
    ...
    def batch_creation_handler(
        ...
        **kwargs,
        ...
        from derouter.llms.anthropic.batches.transformation import AnthropicBatchesConfig
        from derouter.types.utils import Choices, SpecialEnums
        ...
        try:
            ...
            if httpx_response.status_code == 200 and "id" in _json_response:
                # Transform Anthropic response to DeRouter batch format
                anthropic_batches_config: Final = AnthropicBatchesConfig()
                derouter_batch_response: Final = anthropic_batches_config.transform_retrieve_batch_response(
                    model=None,
                    raw_response=httpx_response,
                    logging_obj=logging_obj,
                    derouter_params={},
                ...
                derouter_batch_response.status = "validating"
                ...
                derouter_model_response.object = "batch"
                ...
                derouter_model_response = ModelResponse()
                derouter_model_response.id = str(uuid.uuid4())
                derouter_model_response.model = "anthropic_batch"
                derouter_model_response.object = "batch"
                derouter_model_response.created = int(start_time.timestamp())
