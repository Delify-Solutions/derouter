...
from derouter.proxy._types import PassThroughEndpointLoggingResultValues
from derouter.proxy.common_request_processing import ProxyBaseLLMRequestProcessing
from derouter.proxy.common_utils.sse_keepalive import split_complete_sse_frames
from derouter.types.passthrough_endpoints.pass_through_endpoints import EndpointType
from derouter.types.utils import StandardPassThroughResponseObject, Usage
...
class PassThroughStreamingHandler:
    @staticmethod
    def _stamp_first_chunk_if_needed(derouter_logging_obj: DeRouterLoggingObj) -> None:
        if derouter_logging_obj.completion_start_time is None:
            derouter_logging_obj._update_completion_start_time(completion_start_time=datetime.now())
    ...
    def schedule_stream_failure_logging(
        ...
        try:
            GLOBAL_LOGGING_WORKER.ensure_initialized_and_enqueue(
                async_coroutine=derouter_logging_obj.dispatch_failure_handlers(
    ...
    def _record_partial_usage_for_failure(
        ...
        if endpoint_type == EndpointType.ANTHROPIC:
            AnthropicPassthroughLoggingHandler.record_partial_usage_for_failure(
                derouter_logging_obj=derouter_logging_obj, request_body=request_body, all_chunks=raw_bytes
            )
            return
    ...
    async def chunk_processor(
        ...
        cost_injection_active: Final = (
            bool(getattr(derouter, "include_cost_in_streaming_usage", False))
            and bool(model_name)
            and (
        ...
        try:
            if not cost_injection_active:
                ...
                async for chunk in response.aiter_bytes():
                    ...
                    PassThroughStreamingHandler._stamp_first_chunk_if_needed(derouter_logging_obj)
                    yield chunk
            else:
                ...
                async for chunk in response.aiter_bytes():
                    ...
                    if complete_frames:
                        yield ProxyBaseLLMRequestProcessing._process_chunk_with_cost_injection(
                            complete_frames, resolved_model_name, derouter_logging_obj
                        )
                if pending:
                    yield pending
            ...
            if (
                getattr(derouter_logging_obj, "_on_deferred_stream_complete", None) is not None
                and raw_bytes
                and response.status_code < 400
            ):
                logging_scheduled = True
                derouter_logging_obj._deferred_stream_complete_args = (_build_logging_coroutine(),)
        except Exception as e:
    ...
    async def _route_streaming_logging_to_handler(
        ...
        try:
            ...
            # and derouter_params lacks an async flag (e.g. aanthropic_messages).
            await derouter_logging_obj.dispatch_success_handlers(
                result=standard_logging_response_object,
                start_time=start_time,
                end_time=end_time,
                cache_hit=derouter_logging_obj.model_call_details.get("cache_hit") is True,
    ...
    def _extract_model_for_cost_injection(
        ...
        if hasattr(derouter_logging_obj, "model_call_details"):
            model = derouter_logging_obj.model_call_details.get("model")
            if model:
                return model
