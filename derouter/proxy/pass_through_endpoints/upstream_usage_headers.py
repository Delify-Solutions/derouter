...
Some pass-through targets invoke several models internally, so DeRouter cannot
price the request from the response body. Those targets report the totals for
the whole HTTP request in ``x-derouter-*`` response headers instead; DeRouter
...
UPSTREAM_RESPONSE_COST_HEADER: Final = "x-derouter-response-cost"
UPSTREAM_TOTAL_TOKENS_HEADER: Final = "x-derouter-total-tokens"

# model_call_details key holding what the upstream reported, so later stages of
# the success path can tell an upstream-reported cost apart from one DeRouter
# derived itself.
UPSTREAM_REPORTED_USAGE_KEY: Final = "_derouter_upstream_reported_usage"
...
def parse_upstream_reported_usage(headers: httpx.Headers) -> UpstreamReportedUsage | None:
    ...
    whose cost DeRouter derives from the response body instead).
    """
...
def apply_upstream_reported_usage(
    ...
    that reports cost but not tokens keeps the token count DeRouter derived on
...
def has_upstream_reported_usage(logging_obj: DeRouterLoggingObj) -> bool:
