"""
Codec for DeRouter passthrough-managed object IDs.

Plaintext format (before urlsafe-base64 encoding):
    derouter_proxy:passthrough;provider:{p};unified_id,{u};raw_id,{r}
...
unified-endpoint rows that share the same DeRouter_ManagedFileTable /
DeRouter_ManagedObjectTable.  ``_resolve_one`` in the rewriter module rejects
...
_PREFIX: Final = SpecialEnums.LITELM_MANAGED_FILE_ID_PREFIX.value  # "derouter_proxy"
...
def encode(provider: str, unified_uuid: str, raw_provider_id: str) -> str:
    ...
    plaintext: Final = SpecialEnums.DEROUTER_PASSTHROUGH_MANAGED_ID_COMPLETE_STR.value.format(
...
def decode(managed_id: str) -> ManagedIdPayload | None:
    ...
    # Must start with "derouter_proxy:passthrough;"
