...
class PassthroughEndpointRouter:
    ...
    def _resolve_matching_deployment_api_key(
        ...
        if derouter_params.get("use_in_pass_through") is not True:
            return None
        if self._get_deployment_provider(derouter_params) != custom_llm_provider:
            return None
        credential_name: Final = derouter_params.get("derouter_credential_name")
        credential_values: Final = (
            CredentialAccessor.get_credential_values(credential_name) if credential_name is not None else None
        )
        api_base: Final = _get_str_value(credential_values, "api_base") or derouter_params.get("api_base")
        ...
        if deployment_region != region_name:
            return None
        return _get_str_value(credential_values, "api_key") or derouter_params.get("api_key")

    def _get_deployment_provider(self, derouter_params: DeRouterParamsTypedDict) -> str | None:
    ...
    def get_vertex_credentials_from_router_deployments(self, model: str | None) -> VertexPassThroughCredentials | None:
        ...
        resolved: Final = tuple(
            ...
            if (credentials := self._resolve_vertex_deployment_credentials(deployment["derouter_params"])) is not None
        )
        matched: Final = next(
            (
                credentials
    ...
    def _resolve_vertex_deployment_credentials(
        self, derouter_params: DeRouterParamsTypedDict
    ) -> VertexPassThroughCredentials | None:
        if derouter_params.get("use_in_pass_through") is not True:
            return None
        if self._get_deployment_provider(derouter_params) != "vertex_ai":
            return None
        credential_name: Final = derouter_params.get("derouter_credential_name")
        credential_values: Final = (
            CredentialAccessor.get_credential_values(credential_name) if credential_name is not None else None
        )
        vertex_project: Final = _get_str_value(credential_values, "vertex_project") or derouter_params.get(
            "vertex_project"
        )
        vertex_location: Final = _get_str_value(credential_values, "vertex_location") or derouter_params.get(
            "vertex_location"
        )
        ...
        ) or derouter_params.get("vertex_credentials")
    ...
    def _deployment_matches_model(deployment: DeploymentTypedDict, model: str) -> bool:
        upstream_model: Final = deployment["derouter_params"].get("model")
        return model in (
    ...
    def get_vector_store_credentials(self, vector_store_id: str) -> DeRouter_ManagedVectorStore | None:
        """
        Get the vector store credentials for the given vector store id
        """
        if derouter.vector_store_registry is None:
            return None
        vector_store_to_run: Final[DeRouter_ManagedVectorStore | None] = (
            derouter.vector_store_registry.get_derouter_managed_vector_store_from_registry(
