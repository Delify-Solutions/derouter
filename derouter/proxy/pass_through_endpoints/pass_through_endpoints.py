...
from derouter.derouter_core_utils.initialize_dynamic_callback_params import validate_no_callback_env_reference
...
from derouter.proxy.utils import normalize_route_for_root_path
...
from derouter.types.utils import TRUSTED_CALLBACK_VARS_FIELD, Usage
...
async def set_env_variables_in_header(custom_headers: dict | None) -> dict | None:
    ...
    for key, value in custom_headers.items():
        # langfuse Api requires base64 encoded headers - it's simpleer to just ask derouter users to set their langfuse public and secret keys
...
async def chat_completion_pass_through_endpoint(
    ...
    try:
        ...
        verbose_proxy_logger.debug("Request received by DeRouter:\n%s", data)
        data["model"] = (
        ...
        if data["model"] in derouter.model_alias_map:
            data["model"] = derouter.model_alias_map[data["model"]]
        ...
        if "api_key" in data:
            llm_response = asyncio.create_task(derouter.aadapter_completion(**data))
        ...
        elif user_model is not None:  # `derouter --model <your-model-name>`
            llm_response = asyncio.create_task(derouter.aadapter_completion(**data))
        else:
            raise HTTPException(
...
class HttpPassThroughEndpointHelpers(BasePassthroughUtils):
    ...
    def get_response_headers(
        ...
        if derouter_call_id:
            return_headers["x-derouter-call-id"] = derouter_call_id
        if custom_headers:
    ...
    async def non_streaming_http_request_handler(
        ...
        if HttpPassThroughEndpointHelpers.is_multipart(request) is True and forward_multipart:
            ...
            # non-empty (pass_through_request always injects derouter_logging_obj, etc.).
    ...
    def _init_kwargs_for_pass_through_endpoint(
        ...
        """
        Filter out derouter params from the request body
        """
        from derouter.types.utils import all_derouter_params

        _parsed_body = _parsed_body or {}

        derouter_params_in_body: Final = {}
        for k in all_derouter_params:
            if k in _parsed_body:
                derouter_params_in_body[k] = _parsed_body.pop(k, None)

        _metadata = dict(
            DeRouterProxyRequestSetup.get_sanitized_user_information_from_key(user_api_key_dict=user_api_key_dict)
        )

        derouter_metadata: Final = derouter_params_in_body.pop("derouter_metadata", None)
        metadata: Final = derouter_params_in_body.pop("metadata", None)
        if derouter_metadata:
            _metadata.update(derouter_metadata)
        if metadata:
        ...
        _metadata["derouter_parent_otel_span"] = user_api_key_dict.parent_otel_span
        ...
        # DeRouter-managed one. Enforcement is therefore skipped on those routes, and
        # charging a counter anyway would track spend that nothing can refuse, and
        # would attribute it to a budget the operator scoped to a DeRouter model that
...
def _build_passthrough_failure_request_payload(
    ...
    Shared by the outer exception handler (DeRouter-internal failures) and
    upstream HTTP error logging, so both failure paths report the same shape
    of request data (model, custom_llm_provider, derouter_logging_obj, ...).
    ...
    if logging_obj is not None:
        request_payload["derouter_logging_obj"] = logging_obj
...
def _resolve_team_callback_wiring(
    ...
    Mirrors add_derouter_data_to_request: callback_vars are unpacked top-level
...
async def _log_passthrough_upstream_failure(
    ...
    """Fire DeRouter-side failure hooks (spend tracking, alerting callbacks) for
    ...
    have received had the error originated inside DeRouter.
    """
...
async def pass_through_request(
    ...
    """
    from derouter.exceptions import ModifyResponseException
    from derouter.derouter_core_utils.derouter_logging import Logging
    from derouter.proxy.pass_through_endpoints.passthrough_guardrails import (
        PassthroughGuardrailHandler,
    )
    from derouter.proxy.proxy_server import proxy_config, proxy_logging_obj

    #########################################################
    # Initialize variables
    #########################################################
    derouter_call_id: Final = str(uuid.uuid4())
    url: httpx.URL | None = None

    # parsed request body
    _parsed_body: dict | None = None
    # kwargs for pass through endpoint, contains metadata, derouter_params, call_type, derouter_call_id, passthrough_logging_payload
    ...
    try:
        ...
        state_raw_body: str | bytes | None = (
            getattr(_request_state, DEROUTER_PASS_THROUGH_RAW_BODY_STATE_KEY, None)
        ...
        if _parsed_body is None:
            _parsed_body = {}
        _parsed_body["derouter_logging_obj"] = logging_obj
        ...
        if custom_llm_provider:
            ...
            logging_obj.model_call_details["derouter_params"] = kwargs.get("derouter_params", {})
        ...
        logging_obj.update_environment_variables(
            ...
            user="unknown",
            optional_params={},
            derouter_params=kwargs["derouter_params"],
            call_type="pass_through_endpoint",
        )
        logging_obj.model_call_details["derouter_call_id"] = derouter_call_id
        ...
        if proxy_general_settings.get("passthrough_managed_object_ids", False) and _managed_id_provider is not None:
            ...
            if _passthrough_managed_hook is not None:
                from derouter.proxy.pass_through_endpoints.managed_id_rewriter import (
                    rewrite_body_ids,
                    rewrite_path_ids,
                    rewrite_query_ids,
                )
        ...
        if (
            ...
            from derouter.proxy.proxy_server import prisma_client as _list_prisma
        ...
        if CustomGuardrail._is_guardrail_intervention(e):
            ...
            verbose_proxy_logger.exception(
                "derouter.proxy.proxy_server.pass_through_endpoint(): Exception occured - %s", e
            )
        ...
        # add user_api_key_dict, derouter_call_id, passthrough_logging_payloa for logging
        if kwargs:
...
def _update_metadata_with_tags_in_header(request: Request, metadata: dict) -> dict:
    ...
    Checks both 'tags' and 'x-derouter-tags' headers
    """
    tags_to_add: Final = []
    ...
    _tags = request.headers.get("x-derouter-tags")
    if _tags:
        tags_to_add.extend([tag.strip() for tag in _tags.split(",")])
...
def create_pass_through_route(
    ...
    try:
        ...
        derouter.adapters = [{"id": adapter_id, "adapter": adapter}]
        ...
        async def endpoint_func(
            ...
            # Programmatic callers set DEROUTER_PASS_THROUGH_CUSTOM_BODY_STATE_KEY on
            ...
            async def _relay() -> Response:
                try:
                    ...
                    if hasattr(request.state, DEROUTER_PASS_THROUGH_CUSTOM_BODY_STATE_KEY):
                        delattr(request.state, DEROUTER_PASS_THROUGH_CUSTOM_BODY_STATE_KEY)
                    if hasattr(request.state, DEROUTER_PASS_THROUGH_RAW_BODY_STATE_KEY):
                        delattr(request.state, DEROUTER_PASS_THROUGH_RAW_BODY_STATE_KEY)
            ...
            return await open_sse_before_first_byte(
                _relay(),
                ping_interval_seconds=(derouter.sse_keepalive_ping_interval_seconds if is_stream else None),
            )

    setattr(endpoint_func, DEROUTER_PASS_THROUGH_ENDPOINT_MARKER, True)
...
def create_websocket_passthrough_route(
    ...
    """
    from derouter.proxy.auth.user_api_key_auth import user_api_key_auth_websocket
...
async def websocket_passthrough_request(
    ...
    logging_obj.update_environment_variables(
        ...
        user="unknown",
        optional_params={},
        derouter_params=dict(kwargs.get("derouter_params", {})),
        call_type="pass_through_endpoint",
    )
    logging_obj.model_call_details["derouter_call_id"] = derouter_call_id
    ...
    try:
        ...
        async with connect(
            ...
            #     success_kwargs.setdefault('derouter_params', {})
            #     success_kwargs['derouter_params'].update({
            #         'proxy_server_request': {
            #             'body': {
...
def _own_streamed_managed_ids(
    ...
    from derouter.proxy.proxy_server import general_settings, prisma_client, proxy_logging_obj
...
class InitPassThroughEndpointHelpers:
    ...
    def remove_endpoint_routes(endpoint_id: str):
        ...
        and clean up corresponding entries from DeRouterRoutes.openai_routes."""
...
async def _register_pass_through_endpoint(
    ...
    if auth_enforced:
        ...
        if path not in DeRouterRoutes.openai_routes.value:
            DeRouterRoutes.openai_routes.value.append(path)
    ...
    if endpoint_data.get("include_subpath", False) is True:
        if auth is not None and str(auth).lower() == "true":
            ...
            if wildcard_path not in DeRouterRoutes.openai_routes.value:
                DeRouterRoutes.openai_routes.value.append(wildcard_path)
...
def _get_pass_through_endpoints_from_config() -> list[PassThroughGenericEndpoint]:
    ...
    from derouter.proxy.proxy_server import config_passthrough_endpoints
...
async def _get_pass_through_endpoints_from_db(
    ...
    from derouter.proxy.proxy_server import get_config_general_settings
