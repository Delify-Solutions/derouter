"""
Pure logic for merging an upstream A2A agent card with DeRouter-specific overrides.

The merge produces the card that DeRouter exposes to A2A clients at
...
# Protocol versions DeRouter can serve to A2A clients. The admin pins one per agent;
...
DEROUTER_A2A_PROTOCOL_VERSION: Final = "1.0"
...
def resolve_served_protocol_version(card: Mapping[str, Any] | None) -> str:
    ...
    return normalized if normalized is not None else DEROUTER_A2A_PROTOCOL_VERSION


# Security scheme exposed by the DeRouter-fronted agent card. Always replaces
# whatever upstream advertised — the client must authenticate to the proxy,
# not the upstream agent.
DEROUTER_SECURITY_SCHEMES: Final[dict[str, dict[str, Any]]] = {
    "DeRouterKey": {
        "type": "http",
        "scheme": "bearer",
...
DEROUTER_SECURITY_REQUIREMENTS: Final[list[dict[str, list[str]]]] = [{"DeRouterKey": []}]

# Capabilities DeRouter can faithfully proxy today. Anything not in this set is
...
def _default_derouter_provider(proxy_base_url: str) -> dict[str, str]:
    return {"organization": "DeRouter Proxy", "url": proxy_base_url}
...
def merge_agent_card(
    ...
    *,
    ...
    """
    Build the DeRouter-fronted agent card.
    ...
    Args:
        ...
        proxy_base_url: Root URL of the DeRouter proxy, used as a fallback when
            we synthesize a provider record.
        name: User-supplied agent name from the DeRouter UI. Takes precedence
            over the upstream card's ``name``.
        description: User-supplied description from the DeRouter UI. Takes
    ...
    if not base.get("provider"):
        base["provider"] = _default_derouter_provider(proxy_base_url)
    ...
    base["securitySchemes"] = deepcopy(DEROUTER_SECURITY_SCHEMES)
    ...
    base["security"] = deepcopy(DEROUTER_SECURITY_REQUIREMENTS)
