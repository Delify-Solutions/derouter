...
"""
Normalize A2A JSON-RPC payloads to the protocol version DeRouter serves for an agent.

DeRouter fronts upstream agents and lets an admin pin the protocol version it speaks
...
from derouter.proxy.a2a.agent_card import normalize_protocol_version
...
def normalize_request_params(params: JsonDict, served: A2AVersion, *, method: str) -> JsonDict:
    ...
    Upstream agents in this proxy pivot on 0.3 wire format, so when DeRouter serves
...
def normalize_agent_card(card: JsonDict, target: A2AVersion) -> JsonDict:
    ...
    re-exposes upstream backend URLs that the DeRouter-fronting merge deliberately drops.
    """
