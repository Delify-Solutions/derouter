...
from derouter.proxy._types import CommonProxyErrors, UserAPIKeyAuth
...
from derouter.repositories.table_repositories import ManagedVectorStoreIndexRepository
    ...
    top_level: Final = MappingProxyType(
        {
            ...
            for key in ("custom_llm_provider", "derouter_credential_name")
            if key in vector_store
        }
    )
    derouter_params: Final = vector_store.get("derouter_params") or MappingProxyType({})
    return MappingProxyType({**top_level, **derouter_params})


async def _update_request_data_with_derouter_managed_vector_store_registry(
    data: dict,
    vector_store_id: str,
    user_api_key_dict: UserAPIKeyAuth | None = None,
) -> dict:
    """
    Update the request data with the derouter managed vector store registry.
    ...
    """
    vector_store_to_run: Final[DeRouter_ManagedVectorStore | None] = await get_derouter_managed_vector_store(
...
async def vector_store_search(
    ...
    data = await _update_request_data_with_derouter_managed_vector_store_registry(
