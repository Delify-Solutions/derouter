...
def _normalize_derouter_params(
    vector_store: DeRouter_ManagedVectorStore,
) -> DeRouter_ManagedVectorStore:
    derouter_params: Final = vector_store.get("derouter_params")
    if isinstance(derouter_params, str):
        normalized: Final = DeRouter_ManagedVectorStore(**dict(vector_store))
        try:
            parsed: Final = json.loads(derouter_params)
            normalized["derouter_params"] = parsed if isinstance(parsed, dict) else {}
        except (TypeError, ValueError):
            normalized["derouter_params"] = {}
        return normalized
    return vector_store
...
def assert_proxy_admin_for_vector_store_index_management(
    ...
    *,
    ...
    raise HTTPException(
        status_code=403,
        detail=(f"Only proxy admins can {operation} vector store indexes. Contact your DeRouter administrator."),
    )
...
async def _is_vector_store_granted(
    ...
    team_object_permission: DeRouter_ObjectPermissionTable | None = user_api_key_dict.team_object_permission
...
async def filter_listable_vector_stores(
    vector_stores: Iterable[DeRouter_ManagedVectorStore],
    user_api_key_dict: UserAPIKeyAuth,
) -> tuple[DeRouter_ManagedVectorStore, ...]:
...
async def get_derouter_managed_vector_store(
    vector_store_id: str,
) -> DeRouter_ManagedVectorStore | None:
    """
    Resolve a DeRouter-managed vector store from the registry or shared cache.
    ...
    DeRouter-managed multi-tenant stores.
    """
    ...
    if derouter.vector_store_registry is not None:
        try:
            ...
            if vector_store is not None:
                return _normalize_derouter_params(vector_store)
        except Exception as e:
    ...
    try:
        ...
        if not rows:
            return None
        return _normalize_derouter_params(DeRouter_ManagedVectorStore(**rows[0].model_dump()))
    except Exception as e:
        verbose_proxy_logger.warning(
...
async def assert_user_can_access_vector_store_id(
    ...
    """
    vector_store: Final = await get_derouter_managed_vector_store(vector_store_id=vector_store_id)
    if vector_store is not None:
