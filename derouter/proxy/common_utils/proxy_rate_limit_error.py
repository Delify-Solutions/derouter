"""
ProxyRateLimitError — a unified rate-limit exception used by derouter's
proxy-side hooks.

Background
----------
DeRouter previously surfaced rate-limit conditions through *several* unrelated
exception types:

* :class:`derouter.exceptions.RateLimitError` — raised by exception mapping when
...
* :class:`fastapi.HTTPException` (status 429) — raised directly by proxy hooks
  ...
  etc.
* :class:`derouter.llms.base_llm.chat.transformation.BaseLLMException` (status
...
distinguish *where* the rate limit originated (vendor vs. derouter, batch vs.
chat) without ad-hoc string-matching on the message.

This module provides a single proxy-side error class that:

1. Is a subclass of :class:`derouter.exceptions.RateLimitError`, so user code
...
3. Carries a :attr:`category` field (one of
   :class:`derouter.exceptions.RateLimitErrorCategory`) so callers can switch on
...
class ProxyRateLimitError(HTTPException, RateLimitError):
    """
    A 429 raised by derouter's proxy-side rate limiting hooks.

    This class deliberately inherits from BOTH
    :class:`derouter.exceptions.RateLimitError` and :class:`fastapi.HTTPException`
    ...
    * ``isinstance(e, HTTPException)`` (FastAPI / proxy_server.py route
    ...
    ``raise HTTPException(status_code=429, ...)`` for derouter-internal rate
    limits.
    ...
    category:
        ...
        ``DEROUTER_RATE_LIMIT`` since this class is only used by derouter's own
        proxy-side limiters; pass ``DEROUTER_BATCH_RATE_LIMIT`` for the batch
        limiter, etc.
    model / llm_provider:
    ...
    # value. Distinguishing vendor vs. derouter 429s is now the job of the
    ...
    def __init__(
        ...
        category: str | RateLimitErrorCategory = RateLimitErrorCategory.DEROUTER_RATE_LIMIT,
        rate_limit_type: str | RateLimitType | None = None,
        model: str | None = None,
        llm_provider: str | None = "derouter_proxy",
    ):
        ...
        model = model or ""
        llm_provider = llm_provider or "derouter_proxy"
        ...
        # Now initialize the derouter RateLimitError portion. We deliberately
