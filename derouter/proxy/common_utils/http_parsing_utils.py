...
from derouter.constants import MAX_REQUEST_BODY_SIZE_TO_REPAIR_MB
...
def check_file_size_under_limit(
    ...
    if llm_router is not None and request_data["model"] in router_model_names:
        try:
            ...
            if (
                deployment
                and deployment.derouter_params is not None
                and deployment.derouter_params.max_file_size_mb is not None
            ):
                max_file_size_mb = deployment.derouter_params.max_file_size_mb
        except Exception as e:
...
async def convert_upload_files_to_file_data(
    ...
    """
    Convert FastAPI UploadFile objects to file data tuples for derouter.

    Converts UploadFile objects to tuples of (filename, content, content_type)
    which is the format expected by httpx and derouter's HTTP handlers.
...
def extract_nested_form_metadata(
    form_data: Mapping[str, object], prefix: str = "derouter_metadata["
) -> dict[str, object]:
    """
    ...
    such as derouter_metadata[spend_logs_metadata][owner] = "value".
    ...
    Args:
        ...
        prefix: The prefix to look for in form keys (default: "derouter_metadata[")
    ...
    for key, value in form_data.items():
        ...
        # Example: "derouter_metadata[spend_logs_metadata][owner]" -> ["spend_logs_metadata", "owner"]
        try:
