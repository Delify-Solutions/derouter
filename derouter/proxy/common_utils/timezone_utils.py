...
import derouter
from derouter.derouter_core_utils.duration_parser import get_next_standardized_reset_time


class BudgetResetSettings(BaseModel):
    """Immutable, validated settings that govern when budgets reset.

    Parsed once from `derouter_settings` and injected into consumers (the reset
...
def get_budget_reset_timezone() -> str:
    """
    Get the budget reset timezone from derouter_settings.
    Falls back to UTC if not specified.

    derouter_settings values are set as attributes on the derouter module
    by proxy_server.py at startup (via setattr(derouter, key, value)).
    """
    return getattr(derouter, "timezone", None) or "UTC"


def get_budget_reset_settings() -> BudgetResetSettings:
    """Build validated reset settings from derouter_settings. Raises on a malformed
    `budget_reset_time`, which lets the proxy fail fast at startup."""
    return BudgetResetSettings(
        timezone=get_budget_reset_timezone(),
        reset_time_of_day=parse_budget_reset_time(getattr(derouter, "budget_reset_time", None)),
    )
