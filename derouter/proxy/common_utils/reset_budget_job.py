...
from derouter.proxy.db.budget_window_spend_writer import roll_window_spend_row
from derouter.proxy.db.db_transaction_queue.pod_lock_manager import PodLockManager
from derouter.proxy.db.exception_handler import call_with_db_reconnect_retry
...
from derouter.repositories.prisma_protocols import SpendLinkedTable
...
def _rollover_enabled() -> bool:
    return derouter.budget_rollover is True
...
def _enduser_carried_spend(row: _EndUserRow, caps: Mapping[str, float]) -> float:
    if not caps:
        return 0.0
    effective_budget_id: Final[str | None] = row.budget_id or derouter.max_end_user_budget_id
...
def _queue_enduser_resets(writes: LinkedSpendResetWrites, cascade: "_BudgetCascade") -> None:
    ...
    budget_id ride the default budget tier (derouter.max_end_user_budget_id).
    ...
    if not cascade.rollover_caps:
        ...
        return
    tiered: Final = tuple((row.budget_id or derouter.max_end_user_budget_id, row.user_id) for row in cascade.endusers)
    for budget_id, cap in cascade.rollover_caps.items():
        if not (
...
class _BudgetCascade:
    ...
    budgets: tuple[DeRouter_BudgetTableFull, ...] = ()
...
_WINDOW_SOURCES: Final[tuple[_WindowSource, ...]] = (
    _WindowSource(
        table="DeRouter_VerificationToken",
        id_column="token",
...
class ResetBudgetJob:
    ...
    async def reset_budget(
        ...
        try:
            await self.reset_budget_for_derouter_keys()
            await self.reset_budget_for_derouter_users()
            await self.reset_budget_for_derouter_teams()
            await self.reset_budget_for_derouter_budget_table()
    ...
    async def _collect_endusers_to_reset(self, budget_ids: Sequence[str]) -> tuple[_EndUserRow, ...]:
        ...
        if derouter.max_end_user_budget_id is None or derouter.max_end_user_budget_id not in budget_ids:
            return tuple(linked or ())
        return (*(linked or ()), *await self._get_endusers_with_no_budget_id())

    async def _collect_budget_cascade(self, budgets_to_reset: Sequence[DeRouter_BudgetTableFull]) -> _BudgetCascade:
        ...
        own schedule via reset_budget_for_derouter_keys(), so sweeping them here
    ...
    async def _reset_expired_budget_cascade(self) -> _BudgetCascadeCommitted | _BudgetCascadeFailed:
        ...
        try:
            budgets_to_reset: Final[Sequence[DeRouter_BudgetTableFull] | None] = await self._with_db_retry(
                lambda: self.prisma_client.get_data(
                    table_name="budget",
                    query_type="find_all",
                    reset_at=now,
    ...
    async def reset_budget_for_derouter_budget_table(self) -> None:
        """
        ...
        """
        await _run_phase_in_chunks(self._reset_budget_for_derouter_budget_table_chunk)

    async def _reset_budget_for_derouter_budget_table_chunk(self) -> _ChunkOutcome:
    ...
    async def _get_endusers_with_no_budget_id(
        ...
        rely on the default budget (derouter.max_end_user_budget_id) applied
        ...
        return [DeRouter_EndUserTable.model_validate(row.model_dump()) for row in rows]

    async def _write_key_reset_updates(self, updated_keys: list[DeRouter_VerificationToken]) -> None:
        """
        ...
        update input type for DeRouter_VerificationToken, and the failure
    ...
    async def _write_key_reset_updates_once(self, updated_keys: list[DeRouter_VerificationToken]) -> None:
    ...
    async def _write_user_reset_updates(self, updated_users: list[DeRouter_UserTable]) -> None:
        """
    ...
    async def _write_user_reset_updates_once(self, updated_users: list[DeRouter_UserTable]) -> None:
        async with spend_reset_unit_of_work(self.prisma_client.db.batch_) as uow:
    ...
    async def _write_team_reset_updates(self, updated_teams: list[DeRouter_TeamTable]) -> None:
        """
    ...
    async def _write_team_reset_updates_once(self, updated_teams: list[DeRouter_TeamTable]) -> None:
        async with spend_reset_unit_of_work(self.prisma_client.db.batch_) as uow:
    ...
    async def reset_budget_for_derouter_keys(self) -> None:
        """
        Resets the budget for all the derouter keys

        Catches Exceptions and logs them
        """
        await _run_phase_in_chunks(self._reset_budget_for_derouter_keys_chunk)

    async def _reset_budget_for_derouter_keys_chunk(self) -> _ChunkOutcome:
        now: Final = datetime.utcnow()
        start_time: Final = time.time()
        keys_to_reset: list[DeRouter_VerificationToken] | None = None
        try:
            ...
            updated_keys: Final[list[DeRouter_VerificationToken]] = []
            failed_keys: Final = []
    ...
    async def reset_budget_for_derouter_users(self) -> None:
        """
        Resets the budget for all DeRouter Internal Users if their budget has expired
        """
        await _run_phase_in_chunks(self._reset_budget_for_derouter_users_chunk)

    async def _reset_budget_for_derouter_users_chunk(self) -> _ChunkOutcome:
        now: Final = datetime.utcnow()
        start_time: Final = time.time()
        users_to_reset: list[DeRouter_UserTable] | None = None
        try:
            ...
            if users_to_reset is not None and len(users_to_reset) > 0:
                ...
                if updated_users:
                    ...
                    for u in updated_users:
                        ...
                        if user_id == DEROUTER_PROXY_BUDGET_NAME:
    ...
    async def reset_budget_for_derouter_teams(self) -> None:
        """
        Resets the budget for all DeRouter Internal Teams if their budget has expired
        """
        await _run_phase_in_chunks(self._reset_budget_for_derouter_teams_chunk)

    async def _reset_budget_for_derouter_teams_chunk(self) -> _ChunkOutcome:
        now: Final = datetime.utcnow()
        start_time: Final = time.time()
        teams_to_reset: list[DeRouter_TeamTable] | None = None
        try:
            ...
            updated_teams: Final[list[DeRouter_TeamTable]] = []
            failed_teams: Final = []
    ...
    async def _roll_window_spend_row(
        ...
        """Move this window's DeRouter_BudgetWindowSpend row onto the window
        ...
        DeRouter_SpendLogs, so a failure here must not stop the remaining
    ...
    async def _reset_budget_common(
        item: DeRouter_TeamTable | DeRouter_UserTable | DeRouter_VerificationToken,
