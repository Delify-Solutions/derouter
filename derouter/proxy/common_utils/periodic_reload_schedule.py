...
from derouter.proxy.utils import PrismaClient, evict_config_param
from derouter.repositories.config_repository import ConfigRepository

if TYPE_CHECKING:
    from prisma.models import DeRouter_Config
...
class _ConfigTable(Protocol):
    async def find_unique(self, where: Mapping[str, str]) -> "DeRouter_Config | None": ...

    async def upsert(self, where: Mapping[str, str], data: _ConfigUpsertData) -> "DeRouter_Config": ...
...
def parse_reload_schedule(row: "DeRouter_Config") -> ReloadSchedule:
