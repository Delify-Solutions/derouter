...
import derouter
from derouter import get_secret
...
from derouter.proxy._types import CommonProxyErrors, DeRouterPromptInjectionParams
...
# avoid double-encrypting if `DEROUTER_SALT_KEY` is rotated between writes.
_CALLBACK_VAR_ENCRYPTED_PREFIX: Final = "derouter_enc::"
...
def _classify_loaded_callback(entry: str, loaded: object) -> CustomLogger | Callable[..., object] | _CallbackLoadError:
    """
    Decide whether what a ``derouter_settings.callbacks`` dotted path resolved to can be dispatched.
...
def _raise_callback_load_error(error: _CallbackLoadError) -> NoReturn:
    ...
    match error:
        case _CallbackResolvedToClass():
            ...
            raise ValueError(
                f"derouter_settings.callbacks entry '{error.entry}' resolved to the class "
        ...
        case _CallbackNotDispatchable():
            raise ValueError(
                f"derouter_settings.callbacks entry '{error.entry}' resolved to "
...
def initialize_callbacks_on_proxy(
    ...
    if isinstance(value, list):
        ...
        for callback in value:  # ["presidio", <my-custom-callback>]
            ...
            if isinstance(callback, str) and callback in derouter._known_custom_logger_compatible_callbacks:
                ...
                presidio_logging_only: bool | None = derouter_settings.get("presidio_logging_only", None)
                ...
                try:
                    from derouter_enterprise.enterprise_callbacks.llama_guard import (
                ...
                from derouter.proxy.guardrails.guardrail_hooks.lakera_ai import (
                    lakeraAI_Moderation,
                )
                ...
                from derouter.proxy.guardrails.guardrail_hooks.aporia_ai.aporia_ai import (
                    AporiaGuardrail,
                )
                ...
                try:
                    from derouter_enterprise.enterprise_callbacks.llm_guard import (
                ...
                if "prompt_injection_params" in derouter_settings:
                    prompt_injection_params_in_config = derouter_settings["prompt_injection_params"]
                    prompt_injection_params = DeRouterPromptInjectionParams(**prompt_injection_params_in_config)
                ...
                from derouter.proxy.hooks.batch_redis_get import (
                ...
                from derouter.proxy.hooks.azure_content_safety import (
                    _PROXY_AzureContentSafety,
                )

                azure_content_safety_params = derouter_settings["azure_content_safety_params"]
                ...
                from derouter.integrations.datadog.datadog_cost_management import (
        ...
        if isinstance(derouter.callbacks, list):
            derouter.callbacks.extend(imported_list)
        else:
            derouter.callbacks = imported_list
    ...
    verbose_proxy_logger.debug("%s Initialized Callbacks - %s %s", blue_color_code, derouter.callbacks, reset_color_code)


def get_model_group_from_derouter_kwargs(kwargs: dict) -> str | None:
    _derouter_params: Final = kwargs.get("derouter_params", None) or {}
    _metadata: Final = _derouter_params.get(get_metadata_variable_name_from_kwargs(kwargs)) or {}
...
def get_remaining_tokens_and_requests_from_request_data(data: dict) -> dict[str, str]:
    """
    Helper function to return x-derouter-key-remaining-tokens-{model_group} and x-derouter-key-remaining-requests-{model_group}
    ...
    if remaining_requests:
        headers[f"x-derouter-key-remaining-requests-{h11_model_group_name}"] = remaining_requests
    ...
    if remaining_tokens:
        headers[f"x-derouter-key-remaining-tokens-{h11_model_group_name}"] = remaining_tokens
...
def get_logging_caching_headers(request_data: dict) -> dict | None:
    ...
    derouter_metadata_bucket: Final = request_data.get("derouter_metadata")
    if isinstance(metadata_bucket, dict):
        _metadata.update(metadata_bucket)
    if isinstance(derouter_metadata_bucket, dict):
        # Batch/file routes store proxy tracking in derouter_metadata while
        # user-facing metadata stays in metadata; merge both for headers.
        _metadata.update(derouter_metadata_bucket)
    headers: Final = {}
    if "applied_guardrails" in _metadata:
        headers["x-derouter-applied-guardrails"] = ",".join(_metadata["applied_guardrails"])

    scan_ids: Final = _metadata.get(GUARDRAIL_SCAN_IDS_METADATA_KEY)
    if scan_ids:
        headers["x-derouter-guardrail-scan-id"] = ",".join(scan_ids)

    if "applied_policies" in _metadata:
        headers["x-derouter-applied-policies"] = ",".join(_metadata["applied_policies"])
    ...
    if "policy_sources" in _metadata:
        ...
        if isinstance(sources, dict) and sources:
            ...
            headers["x-derouter-policy-sources"] = "; ".join(f"{name}={reason}" for name, reason in sources.items())

    if "semantic-similarity" in _metadata:
        headers["x-derouter-semantic-similarity"] = str(_metadata["semantic-similarity"])
...
DEROUTER_PROXY_INTERNAL_METADATA_KEYS: Final = frozenset(
    {
        "applied_policies",
        "applied_guardrails",
...
def sanitize_openai_provider_metadata(
    ...
    Strips DeRouter proxy-internal tracking fields that must not be forwarded to
    ...
    for key, value in metadata.items():
        if key in DEROUTER_PROXY_INTERNAL_METADATA_KEYS:
            continue
...
def add_guardrail_scan_id(request_data: dict, scan_id: str | None) -> None:
    ...
    audit trail. Ids recorded here become the x-derouter-guardrail-scan-id response header.
    """
    if not scan_id:
        return
    _, _metadata = get_or_create_metadata_bucket(request_data)
...
def add_policy_sources_to_metadata(request_data: dict, policy_sources: dict[str, str]):
    """
    Store policy match reasons in metadata for x-derouter-policy-sources header.
...
def add_guardrail_response_to_standard_logging_object(
    derouter_logging_obj: Optional["DeRouterLogging"],
    guardrail_response: StandardLoggingGuardrailInformation,
):
    if derouter_logging_obj is None:
        return
    standard_logging_object: Final[StandardLoggingPayload | None] = derouter_logging_obj.model_call_details.get(
        "standard_logging_object"
    )
...
def _encrypt_if_plaintext(key: str, value: object) -> object:
    ...
    try:
        ...
        # than crash the write. Dev environments without DEROUTER_SALT_KEY hit
