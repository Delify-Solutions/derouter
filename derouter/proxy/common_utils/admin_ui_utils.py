...
def show_missing_vars_in_env():
    ...
    if prisma_client is None and master_key is None:
        return HTMLResponse(
            content=missing_keys_form(missing_key_names="DATABASE_URL, DEROUTER_MASTER_KEY"),
            status_code=200,
        )
    ...
    if master_key is None:
        return HTMLResponse(
            content=missing_keys_form(missing_key_names="DEROUTER_MASTER_KEY"),
            status_code=200,
        )
    return None
...
def missing_keys_form(missing_key_names: str):
    missing_keys_html_form: Final = """
        ...
        <body>
            <div class="container">
                ...
                <pre>
    <span class="env-var">DEROUTER_MASTER_KEY="sk-1234"</span> <span class="comment"># Your master key for the proxy server. Can use this to send /chat/completion requests etc</span>
    <span class="env-var">DEROUTER_SALT_KEY="sk-XXXXXXXX"</span> <span class="comment"># Can NOT CHANGE THIS ONCE SET - It is used to encrypt/decrypt credentials stored in DB. If value of 'DEROUTER_SALT_KEY' changes your models cannot be retrieved from DB</span>
    ...
    <span class="env-var">STORE_MODEL_IN_DB="True"</span> <span class="comment"># Allow storing models in db</span>
            ...
            <p>Docs: <a href="https://router.delify.vn/docs/" target="_blank">https://router.delify.vn/docs/</a></p>
            </div>
        </body>
        </html>
    """
