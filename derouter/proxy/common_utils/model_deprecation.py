...
class _ResolvedDeprecation:
    ...
    derouter_model: str | None
    derouter_provider: str | None
...
def _cost_map_lookup(model_key: object) -> _ResolvedDeprecation | None:
    if not isinstance(model_key, str) or not model_key:
        return None
    entry: Final = derouter.model_cost.get(model_key)
    if not isinstance(entry, Mapping):
        return None
    parsed: Final = _parse_deprecation_date(entry.get("deprecation_date"))
    if parsed is None:
        return None
    provider: Final = entry.get("derouter_provider")
    return _ResolvedDeprecation(
        deprecation_date=parsed,
        derouter_model=model_key,
        derouter_provider=provider if isinstance(provider, str) else None,
    )
...
def _resolve_deployment_deprecation(
    ...
    raw_model: Final = _mapping_field(deployment, "derouter_params").get("model")

    override: Final = _parse_deprecation_date(model_info.get("deprecation_date"))
    if override is not None:
        provider: Final = model_info.get("derouter_provider")
        return _ResolvedDeprecation(
            deprecation_date=override,
            derouter_model=raw_model if isinstance(raw_model, str) else None,
            derouter_provider=provider if isinstance(provider, str) else None,
        )
...
def _format_entry(info: ModelDeprecationInfo) -> str:
    ...
    return (
        ...
        f"(provider: {_escape_slack_mrkdwn(info.derouter_provider) if info.derouter_provider else 'unknown'}, "
    ...
    return "\n".join(
        (
            ...
            *imminent_section,
            ...
            "https://router.delify.vn/docs/proxy/model_management for guidance.",
        )
    )
