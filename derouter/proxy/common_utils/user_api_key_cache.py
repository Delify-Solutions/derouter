...
from derouter.constants import DEFAULT_MANAGEMENT_OBJECT_IN_MEMORY_CACHE_TTL
...
def tag_registry_cache_key() -> str:
    """Cache key for the set of tag names that exist in ``DeRouter_TagTable``."""
    return "tag_registry"
...
def team_membership_auth_cache_key(team_id: str, user_id: str) -> str:
    """Cache key one team member's ``DeRouter_TeamMembership`` row is stored under for the admission check."""
    return f"{team_id}_{user_id}"


def team_membership_reservation_cache_key(user_id: str, team_id: str) -> str:
    """Cache key the pre-call budget reservation stores the same ``DeRouter_TeamMembership`` row under.
