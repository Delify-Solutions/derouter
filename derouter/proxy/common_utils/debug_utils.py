...
from derouter.constants import PYTHON_GC_THRESHOLD
...
if os.environ.get("DEROUTER_PROFILE", "false").lower() == "true":
    try:
        import objgraph
...
def _get_router_memory_stats(llm_router) -> Mapping[str, object]:
    """Get memory usage statistics for DeRouter router."""
    derouter_router_memory: dict[str, object] = {}
    try:
        if llm_router is not None:
            ...
            if hasattr(llm_router, "model_list") and llm_router.model_list:
                ...
                derouter_router_memory["model_list"] = {
            ...
            if hasattr(llm_router, "model_names") and llm_router.model_names:
                ...
                derouter_router_memory["model_names_set"] = {
            ...
            if hasattr(llm_router, "deployment_names") and llm_router.deployment_names:
                ...
                derouter_router_memory["deployment_names"] = {
            ...
            if hasattr(llm_router, "deployment_latency_map") and llm_router.deployment_latency_map:
                ...
                derouter_router_memory["deployment_latency_map"] = {
            ...
            if hasattr(llm_router, "fallbacks") and llm_router.fallbacks:
                ...
                derouter_router_memory["fallbacks"] = {
            ...
            derouter_router_memory["router_object"] = {
                "size_bytes": router_obj_size,
                "size_mb": round(router_obj_size / (1024 * 1024), 4),
            }

        else:
            derouter_router_memory = {"note": "Router not initialized"}
    except Exception as e:
        verbose_proxy_logger.debug("Error getting router memory info: %s", e)
        derouter_router_memory = {"error": str(e)}

    return derouter_router_memory
...
async def get_memory_details(
    ...
    derouter_router_memory: Final = _get_router_memory_stats(llm_router)
    ...
    return {
        ...
        "router_memory": derouter_router_memory,
    }
