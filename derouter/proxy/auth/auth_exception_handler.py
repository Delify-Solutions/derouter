...
import derouter
from derouter._logging import verbose_proxy_logger, verbose_proxy_stdout_logger
from derouter.constants import EMPTY_MAPPING
from derouter.integrations.otel.runtime import seed_request_identity
...
def _with_requester_ip_address(request_data: dict[str, object], requester_ip: str | None) -> dict[str, object]:
    """Auth gate rejections are raised before `add_derouter_data_to_request` records the
    caller IP, so their failure logs would otherwise carry no IP nor key/user identity."""
    if not requester_ip:
        return request_data
    key: Final = "derouter_metadata" if "derouter_metadata" in request_data else "metadata"
...
class UserAPIKeyAuthExceptionHandler:
    ...
    async def _handle_authentication_error(
        ...
        """
        Handles Connection Errors when reading a Virtual Key from DeRouter DB
        ...
        if (
            ...
            # rejections to WARNING on stdout (suppressible via DEROUTER_LOG=ERROR).
            log_extra: Final = {"requester_ip": requester_ip}
            is_invalid_virtual_key: Final = is_invalid_virtual_key_error(e)
            is_quiet_log: Final = is_invalid_virtual_key and not derouter.log_client_error_tracebacks
            ...
            logger.log(
                ...
                "derouter.proxy.proxy_server.user_api_key_auth(): Exception occured - %s\nRequester IP Address:%s",
                e,
                requester_ip,
                exc_info=True if derouter.log_client_error_tracebacks or not is_expected_client_error(e) else None,
                extra=log_extra,
            )
            ...
            if isinstance(e, derouter.BudgetExceededError) and not e.llm_provider:
