...
import derouter
from derouter.constants import DEROUTER_PROXY_ADMIN_NAME, DEROUTER_UI_SESSION_DURATION
...
from derouter.proxy.management_endpoints.internal_user_endpoints import user_update
...
def get_ui_credentials(master_key: str | None) -> tuple[str, str]:
    ...
    if ui_password is None:
        raise ProxyException(
            message="set Proxy master key to use UI. https://router.delify.vn/docs/proxy/virtual_keys. If set, use `--detailed_debug` to debug issue.",
...
async def authenticate_user(
    ...
    _user_row: DeRouter_UserTable | None = None
    user_role: (
        Literal[
    ...
    if secrets.compare_digest(username.encode("utf-8"), ui_username.encode("utf-8")) and secrets.compare_digest(
        ...
        key_user_id = DEROUTER_PROXY_ADMIN_NAME
        if (
            os.getenv("PROXY_ADMIN_ID", None) is not None and os.environ["PROXY_ADMIN_ID"] == user_id
        ) or user_id == DEROUTER_PROXY_ADMIN_NAME:
            # checks if user is admin
            key_user_id = os.getenv("PROXY_ADMIN_ID", DEROUTER_PROXY_ADMIN_NAME)
        ...
        if get_secret_bool("EXPERIMENTAL_UI_LOGIN"):
            ...
            user_info: DeRouter_UserTable | None = None
...
def _ui_session_exp_timestamp() -> int:
    """The ``exp`` claim (unix seconds) for a UI session cookie, ``DEROUTER_UI_SESSION_DURATION``
    ...
    ttl_seconds: Final = duration_in_seconds(DEROUTER_UI_SESSION_DURATION)
