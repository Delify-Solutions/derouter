...
from derouter.types.router import CredentialDeRouterParams, DeRouter_Params
from derouter.types.utils import LlmProviders
from derouter.utils import get_valid_models

_CREDENTIAL_DEROUTER_PARAM_FIELDS = set(CredentialDeRouterParams.model_fields)
...
def get_provider_models(provider: str, derouter_params: DeRouter_Params | None = None) -> list[str] | None:
    """
    Returns the list of known models by provider
    """
    if provider == "*":
        return get_valid_models(derouter_params=derouter_params)

    if provider in derouter.models_by_provider:
        provider_models: Final = get_valid_models(custom_llm_provider=provider, derouter_params=derouter_params)
...
def _hydrate_derouter_credential_name(
    ...
    credential_values: Final = CredentialAccessor.get_credential_values(derouter_params.derouter_credential_name)
    if not credential_values:
        return derouter_params

    derouter_params = derouter_params.model_copy()
    for key, value in credential_values.items():
        if key in _CREDENTIAL_DEROUTER_PARAM_FIELDS and getattr(derouter_params, key, None) is None:
            setattr(derouter_params, key, value)
    derouter_params.derouter_credential_name = None
    return derouter_params


def get_known_models_from_wildcard(wildcard_model: str, derouter_params: DeRouter_Params | None = None) -> list[str]:
    wildcard_model_to_expand: Final = (
        derouter_params.model
        if wildcard_model == "*"
        and derouter_params is not None
        and _check_wildcard_routing(derouter_params.model)
        and "/" in derouter_params.model
        else wildcard_model
    )
    try:
    ...
    # Use provider from derouter_params when available, otherwise from wildcard prefix
    # (e.g., "openai" from "openai/*" - needed for BYOK where wildcard isn't in router)
    if derouter_params is not None:
        try:
            provider = derouter_params.model.split("/", 1)[0]
        except ValueError:
            provider = wildcard_provider_prefix
    else:
        provider = wildcard_provider_prefix

    derouter_params = _hydrate_derouter_credential_name(derouter_params)

    wildcard_models = get_provider_models(provider=provider, derouter_params=derouter_params)
...
def expand_wildcard_deployments_for_model_info(
    ...
    on top of that: a wildcard deployment like model_name="*" / derouter_params.model="openai/*"
    ...
    for deployment in deployments:
        ...
        raw_params = deployment.get("derouter_params")
        derouter_params_dict: dict[str, Any] = raw_params if isinstance(raw_params, dict) else {}
        derouter_model = str(derouter_params_dict.get("model") or "")

        # Determine the wildcard pattern to expand.
        # Branch order matters: only fall to derouter_model when model_name is
        ...
        elif _check_wildcard_routing(model_name) and _check_wildcard_routing(derouter_model):
            wildcard_pattern = derouter_model
        ...
        try:
            derouter_params = DeRouter_Params.model_validate(derouter_params_dict) if derouter_params_dict else None
        except Exception:
            expanded.append(deployment)
            continue
        ...
        for name in expanded_names:
            ...
            row["model_name"] = name
            params = row.get("derouter_params")
                ...
                for router_model in model_list:
                    all_wildcard_models.extend(
                        get_known_models_from_wildcard(
                            ...
                            derouter_params=DeRouter_Params(**router_model["derouter_params"]),
                        )
                    )
            else:
                all_wildcard_models.extend(get_known_models_from_wildcard(wildcard_model=model, derouter_params=None))
