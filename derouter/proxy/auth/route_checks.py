...
class RouteChecks:
    ...
    def should_call_route(
        ...
        """
        try:
            from derouter_enterprise.proxy.auth.route_checks import EnterpriseRouteChecks
    ...
    def is_virtual_key_allowed_to_call_route(
        ...
        ## check if 'allowed_route' is a field name in DeRouterRoutes
        if any(allowed_route in DeRouterRoutes._member_names_ for allowed_route in valid_token.allowed_routes):
            for allowed_route in valid_token.allowed_routes:
                if allowed_route in DeRouterRoutes._member_names_:
                    if RouteChecks.check_route_access(
                        route=route,
                        allowed_routes=DeRouterRoutes._member_map_[allowed_route].value,
                    ):
                        if (
                    ...
                    if allowed_route == "llm_api_routes":
                        ...
                        if RouteChecks.check_route_access(
                            route=route,
                            allowed_routes=DeRouterRoutes.agent_management_routes.value,
                        ):
                            return True
    ...
    def _raise_admin_only_route_exception(
        ...
        Args:
            user_obj (Optional[DeRouter_UserTable]): The user object
    ...
    def non_proxy_admin_allowed_routes_check(
        ...
        if RouteChecks.is_auth_enforced_pass_through_route(
            ...
            route in DeRouterRoutes.global_spend_tracking_routes.value
            ...
            and RouteChecks.check_route_access(route=route, allowed_routes=DeRouterRoutes.internal_user_routes.value)
            or _user_is_org_admin(request_data=request_data, user_object=user_obj)
            and RouteChecks.check_route_access(route=route, allowed_routes=DeRouterRoutes.org_admin_allowed_routes.value)
            or _user_role == LitellmUserRoles.INTERNAL_USER_VIEW_ONLY.value
            and RouteChecks.check_route_access(
                route=route,
                allowed_routes=DeRouterRoutes.internal_user_view_only_routes.value,
            )
            or RouteChecks.check_route_access(route=route, allowed_routes=DeRouterRoutes.self_managed_routes.value)
        ):
            pass
    ...
    def is_llm_api_route(route: str) -> bool:
        ...
        if route in DeRouterRoutes.openai_routes.value:
            return True

        if route in DeRouterRoutes.anthropic_routes.value:
            return True

        if route in DeRouterRoutes.google_routes.value:
            return True

        if RouteChecks.check_route_access(route=route, allowed_routes=DeRouterRoutes.mcp_inference_routes.value):
            return True

        if RouteChecks.check_route_access(route=route, allowed_routes=DeRouterRoutes.agent_inference_routes.value):
            return True

        if route in DeRouterRoutes.derouter_native_routes.value:
            return True

        # fuzzy match routes like "/v1/threads/thread_49EIN5QF32s4mH20M7GFKdlZ"
        # Check for routes with placeholders or wildcard patterns
        for openai_route in DeRouterRoutes.openai_routes.value:
        ...
        for google_route in DeRouterRoutes.google_routes.value:
            if "{" in google_route:
                if RouteChecks._route_matches_pattern(route=route, pattern=google_route):
                    return True

        # Check for Anthropic routes with placeholders
        for anthropic_route in DeRouterRoutes.anthropic_routes.value:
        ...
        for _llm_passthrough_route in DeRouterRoutes.mapped_pass_through_routes.value:
    ...
    def is_management_route(route: str) -> bool:
        ...
        """
        return RouteChecks.check_route_access(route=route, allowed_routes=DeRouterRoutes.management_routes.value)
    ...
    def is_info_route(route: str) -> bool:
        ...
        """
        return route in DeRouterRoutes.info_routes.value
    ...
    def _check_proxy_admin_viewer_access(
        ...
        if RouteChecks.check_route_access(route=route, allowed_routes=DeRouterRoutes.management_routes.value):
        ...
        if RouteChecks.check_route_access(route=route, allowed_routes=DeRouterRoutes.admin_viewer_routes.value):
            return
        if RouteChecks.check_route_access(route=route, allowed_routes=DeRouterRoutes.global_spend_tracking_routes.value):
            return
