"""
This file handles authentication for the DeRouter Proxy.

it checks if the user passed a valid API Key to the DeRouter Proxy
...
from derouter.integrations.otel.runtime import phase_span, seed_request_identity
...
from derouter.proxy.auth.auth_exception_handler import UserAPIKeyAuthExceptionHandler
...
from derouter.proxy.auth.network import TrustedProxyConfig, resolve_network_context
from derouter.proxy.auth.oauth2_check import Oauth2Handler
from derouter.proxy.auth.oauth2_proxy_hook import handle_oauth2_proxy_request
from derouter.proxy.auth.resolvers import CredentialRef, Principal
from derouter.proxy.auth.resolvers.store import IdentityStore
from derouter.proxy.auth.route_checks import RouteChecks
from derouter.proxy.auth.trusted_proxy_utils import get_trusted_proxy_cidrs
from derouter.proxy.common_utils.cache_coordinator import EventDrivenCacheCoordinator
...
from derouter.repositories.table_repositories import TeamMembershipRepository
from derouter.secret_managers.main import get_secret_bool
from derouter.types.services import ServiceTypes

try:
    from derouter_enterprise.proxy.auth.user_api_key_auth import (
...
def _route_requires_auth_despite_public(route: str, general_settings: dict | None) -> bool:
    ...
    if normalized_route == "/metrics":
        return derouter.require_auth_for_metrics_endpoint is not False

    return False


custom_derouter_key_header: Final = APIKeyHeader(
    name=SpecialHeaders.custom_derouter_api_key.value,
    auto_error=False,
...
async def _check_key_model_budget_with_fallback(
    ...
    llm_router: derouter.Router | None = None,
) -> None:
    """
    ...
    except derouter.BudgetExceededError as e:
...
def _should_route_jwt_to_oauth2_override(token: str, jwt_handler: JWTHandler) -> bool:
    routing_overrides: Final = jwt_handler.derouter_jwtauth.routing_overrides
...
def _apply_budget_limits_to_end_user_params(
    ...
    budget_info: DeRouter_BudgetTable,
...
async def _fetch_global_spend_with_event_coordination(
    ...
    per request and is zeroed by ResetBudgetJob every ``derouter.budget_duration``.
    """

    async def _load_global_spend() -> float | None:
        proxy_budget_row: Final = await prisma_client.db.derouter_usertable.find_unique(
            where={"user_id": DEROUTER_PROXY_BUDGET_NAME}
        )
...
async def get_global_proxy_spend(
    ...
    if derouter.max_budget > 0 and prisma_client is not None:  # user set proxy max budget
        ...
        if global_proxy_spend is not None:
            user_info: Final = CallInfo(
                user_id=derouter_proxy_admin_name,
...
def get_api_key(
    custom_derouter_key_header: str | None,
    api_key: str,
    ...
    if isinstance(custom_derouter_key_header, str):
        passed_in_key = custom_derouter_key_header
        api_key = _get_bearer_token_or_received_api_key(custom_derouter_key_header)
        ...
        for endpoint in pass_through_endpoints:
            if endpoint.get("path", "") == route:
                ...
                if headers is not None:
                    header_key: str = headers.get("derouter_user_api_key", "")
...
async def check_api_key_for_custom_headers_or_pass_through_endpoints(
    ...
    if normalized_route is not None:
        for mapped_route in DeRouterRoutes.mapped_pass_through_routes.value:
            if normalized_route.startswith(mapped_route):
                is_mapped_pass_through_route = True
                break
    if is_mapped_pass_through_route:
        if request.headers.get("derouter_user_api_key") is not None:
            api_key = request.headers.get("derouter_user_api_key") or ""
        ...
        for endpoint in pass_through_endpoints:
            if isinstance(endpoint, dict) and endpoint.get("path", "") == route:
                ...
                if endpoint.get("custom_auth_parser") is not None and endpoint.get("custom_auth_parser") == "langfuse":
                    ...
                    # check the langfuse public key if it contains the derouter api key
                    import base64
                    ...
                    if headers is not None:
                        header_key = headers.get("derouter_user_api_key", "")
...
async def _auto_register_jwt_mapping(
    ...
    derouter_jwtkeymapping. The loser catches the conflict, deletes its orphaned
    ...
    # attempts to insert into DeRouter_UserTable with user_id=None, which fails
    ...
    # row in DeRouter_VerificationToken uses its hash, so hash here to get the FK
    # value referenced by DeRouter_JWTKeyMapping.token.
    token_hash = hash_token(key_data["token"])

    try:
        await prisma_client.db.derouter_jwtkeymapping.create(
            data={
        ...
        if "unique" in error_str or "p2002" in error_str:
            ...
            # just persisted to DeRouter_VerificationToken is orphaned — nothing
            ...
            try:
                await prisma_client.db.derouter_verificationtoken.delete(where={"token": token_hash})
    ...
    await user_api_key_cache.async_set_cache(
        key=cache_key,
        value=token_hash,
        ttl=jwt_handler.derouter_jwtauth.virtual_key_mapping_cache_ttl,
    )
...
async def _resolve_jwt_to_virtual_key(
    ...
    """
    virtual_key_claim_field: Final = jwt_handler.derouter_jwtauth.virtual_key_claim_field
    ...
    if claim_value is None:
        ...
        behavior = jwt_handler.derouter_jwtauth.unregistered_jwt_client_behavior
        if behavior in (
...
def _ensure_derouter_received_at_on_request_state(request: Request) -> datetime:
    """Idempotently stamp ``request.state.derouter_received_at`` with the moment
    derouter's own code started handling this request -- the first line of
    ``user_api_key_auth``, before any auth/pre-call work runs. This is the
    basis for the request-latency Prometheus metrics (see
    ``derouter/integrations/prometheus.py``), and unlike the OTEL SERVER span
    below, it is set unconditionally so those metrics don't depend on OTEL
    being configured.
    """
    existing_received_at: Final[datetime | None] = getattr(request.state, "derouter_received_at", None)
    if existing_received_at is not None:
        return existing_received_at
    received_at: Final = datetime.now(timezone.utc)
    try:
        request.state.derouter_received_at = received_at
    except Exception:
        pass
    return received_at
...
def _ensure_parent_otel_span_on_request_state(request: Request) -> None:
    ...
    start_time: Final = _ensure_derouter_received_at_on_request_state(request)

    if open_telemetry_logger is None:
        return
    if getattr(request.state, "parent_otel_span", None) is not None:
        return
    parent_otel_span: Final = open_telemetry_logger.create_derouter_proxy_request_started_span(
...
async def _user_api_key_auth_builder(
    ...
    azure_apim_header: str | None,
    request_data: dict,
    custom_derouter_key_header: str | None = None,
) -> UserAPIKeyAuth:
    ...
    start_time: Final = getattr(request.state, "derouter_received_at", None) or datetime.now(timezone.utc)
    try:
        request.state.derouter_received_at = start_time
    except Exception:
        pass
    ...
    try:
        with tracer.trace("derouter.proxy.auth.pre_db_read_auth_checks"):
        ...
        # if user wants to pass DeRouter_Master_Key as a custom header, example pass derouter keys as X-DeRouter-Key: Bearer sk-1234
        custom_derouter_key_header_name: Final = general_settings.get("derouter_key_header_name")
        if custom_derouter_key_header_name is not None:
            api_key = get_api_key_from_custom_header(
                request=request,
                custom_derouter_key_header_name=custom_derouter_key_header_name,
            )
        ...
        if enterprise_custom_auth is not None:
            with tracer.trace("derouter.proxy.auth.enterprise_custom_auth"):
            ...
            if response is not None and isinstance(response, UserAPIKeyAuth):
                ...
                if getattr(derouter, "enable_post_custom_auth_checks", False):
        ...
        ### DEROUTER-DEFINED AUTH FUNCTION ###
        #### IF JWT ####
        """
        DeRouter supports using JWTs.
        ...
        if not _route_requires_auth_despite_public(route=route, general_settings=general_settings) and (
            route in DeRouterRoutes.public_routes.value or route_in_additonal_public_routes(current_route=route)
        ):
        ...
        if general_settings.get("enable_jwt_auth", False) is True:
            ...
            if is_jwt:
                ...
                if jwt_handler.derouter_jwtauth.virtual_key_claim_field is not None:
                    # Decode JWT to get claims without running full auth_builder
                    jwt_claims: dict | None
                    if jwt_handler.derouter_jwtauth.oidc_userinfo_enabled and not is_jwt:
                ...
                if do_standard_jwt_auth:
                    with tracer.trace("derouter.proxy.auth.jwt_auth_builder"):
                    ...
                    team_membership: Final[DeRouter_TeamMembership | None] = result.get("team_membership", None)
                    ...
                    if model is not None and llm_router is not None:
                        from derouter.proxy.auth.auth_checks import _is_model_cost_zero
        ...
        if end_user_id:
            try:
                ...
                with tracer.trace("derouter.proxy.auth.get_end_user_object"):
                ...
                if _end_user_object is not None:
                    ...
                    if _end_user_object.derouter_budget_table is not None:
                        _apply_budget_limits_to_end_user_params(
                            end_user_params=end_user_params,
                            budget_info=_end_user_object.derouter_budget_table,
                            end_user_id=end_user_id,
                        )
                elif derouter.max_end_user_budget_id is not None:
                ...
                if isinstance(e, derouter.BudgetExceededError):
                    raise e
        ...
        if valid_token is None:
            ## Check CACHE
            try:
                with tracer.trace("derouter.proxy.auth.get_key_object_check_cache"):
        ...
        if (
            ...
            try:
                team_obj: Final[DeRouter_TeamTableCachedObj] = await get_team_object(
                    team_id=valid_token.team_id,
                    prisma_client=prisma_client,
                    user_api_key_cache=user_api_key_cache,
                    parent_otel_span=parent_otel_span,
        ...
        if is_master_key_valid:
            ...
            _user_api_key_obj = await _return_user_api_key_auth_obj(
                user_obj=None,
                user_role=LitellmUserRoles.PROXY_ADMIN,
                api_key=DEROUTER_PROXY_MASTER_KEY_ALIAS,
                parent_otel_span=parent_otel_span,
                valid_token_dict={
                    **end_user_params,
                    "user_id": derouter_proxy_admin_name,
        ...
        if route in DeRouterRoutes.master_key_only_routes.value:
        ...
        if valid_token is None:
            if isinstance(api_key, str):  # if generated token, make sure it starts with sk-.
                ...
                verbose_logger.warning(
                    "derouter.proxy.proxy_server.user_api_key_auth(): Warning - Key is not a string. Got type={}".format(
            ...
            try:
                with tracer.trace("derouter.proxy.auth.get_key_object_from_db"):
                    valid_token = IdentityStore.key_from_principal(
                        await IdentityStore(
                            prisma_client,
                            user_api_key_cache,
                ...
                if e.code == 401 or e.code == "401":
                    e.message = f"Authentication Error, Invalid proxy server token passed. Received API Key = {abbreviated_api_key}, Key Hash (Token) ={api_key}. Unable to find token in cache or `DeRouter_VerificationTokenTable`"
                raise e
        ...
        if valid_token is not None:
            ...
            if valid_token.user_id is not None:
                try:
                    with tracer.trace("derouter.proxy.auth.get_user_object"):
                    ...
                    verbose_logger.debug(
                        "derouter.proxy.auth.user_api_key_auth.py::user_api_key_auth() - Unable to get user from db/cache. Setting user_obj to None. Exception received - %s",
                        e,
                    )
                    user_obj = None
                ...
                if prisma_client is not None and _user_id is not None and _team_id is not None:
                    ...
                    if team_member_info is None:
                        ...
                        if _db_member is not None:
                            team_member_info = DeRouter_TeamMembership(**_db_member.model_dump())
                            await user_api_key_cache.async_set_cache(
                                key=_cache_key,
                    ...
                    if team_member_info is not None and team_member_info.derouter_budget_table is not None:
                        team_member_budget: Final = team_member_info.derouter_budget_table.max_budget
            ...
            if not skip_budget_checks:
                with tracer.trace("derouter.proxy.auth.budget_checks"):
            ...
            if valid_token.team_id is not None:
                try:
                    ...
                    with tracer.trace("derouter.proxy.auth.get_team_object"):
                    ...
                    _team_obj = DeRouter_TeamTableCachedObj(
                        team_id=valid_token.team_id,
            ...
            if derouter.max_budget > 0 and prisma_client is not None:  # user set proxy max budget
                cache_key: Final = GLOBAL_PROXY_SPEND_CACHE_KEY
                with tracer.trace("derouter.proxy.auth.get_global_proxy_spend"):
                    global_proxy_spend = await _fetch_global_spend_with_event_coordination(
                        cache_key=cache_key,
                        user_api_key_cache=user_api_key_cache,
                        prisma_client=prisma_client,
        ...
        # check if token is from derouter-ui, derouter ui makes keys to allow users to login with sso. These keys can only be used for DeRouter UI functions
...
async def _safe_fetch(label: str, awaitable):
    ...
    except (HTTPException, ProxyException, derouter.BudgetExceededError) as e:
        verbose_proxy_logger.debug(
...
def _team_obj_from_token(valid_token: UserAPIKeyAuth) -> DeRouter_TeamTableCachedObj:
...
async def _run_centralized_common_checks(
    ...
    # callers get 401.
    if route in DeRouterRoutes.public_routes.value or route_in_additonal_public_routes(current_route=route):
        return
    ...
    for r in (
        ...
        if isinstance(r, (ProxyException, derouter.BudgetExceededError)):
            raise r
    ...
    team_object: DeRouter_TeamTableCachedObj | None
    ...
    user_object: DeRouter_UserTable | None = None if isinstance(user_result, BaseException) else user_result
    project_object: Final[DeRouter_ProjectTableCachedObj | None] = (
        None if isinstance(project_result, BaseException) else project_result
    )
    end_user_object: Final[DeRouter_EndUserTable | None] = (
    ...
    # same user_id (e.g. derouter_proxy_admin_name = "default_user_id")
    ...
    if user_api_key_auth_obj.user_role == LitellmUserRoles.PROXY_ADMIN:
        user_object = DeRouter_UserTable(
            user_id=user_api_key_auth_obj.user_id or derouter_proxy_admin_name,
            user_role=LitellmUserRoles.PROXY_ADMIN,
    ...
    # Pin the metadata variable name (derouter_metadata vs metadata) before
    ...
    # _tag_max_budget_check read into `derouter_metadata`, hiding header
    # tags from per-tag budget enforcement on DEROUTER_METADATA_ROUTES.
    DeRouterProxyRequestSetup.pre_seed_derouter_metadata_for_route(
        request_data=request_data,
        route=route,
    )

    # Merge x-derouter-tags into request_data BEFORE common_checks runs.
...
async def _reserve_budget_after_common_checks(
    ...
    end_user_id: str | None = None,
    end_user_object: DeRouter_EndUserTable | None = None,
) -> None:
...
async def _authorize_authenticated_request(
    ...
    # ``derouter.validate_end_user_id_in_db``.
...
def _seed_request_destinations(user_api_key_dict: UserAPIKeyAuth, request: Request | None = None) -> None:
    ...
    ``x-derouter-disable-callbacks`` resolves to no destination.
    ...
    """
    try:
        from derouter.integrations.otel.logger import fan_out_provider
        from derouter.integrations.otel.plumbing.context import set_request_destinations
        from derouter.integrations.otel.plumbing.providers import deliverable_destinations
...
async def user_api_key_auth(
    ...
    custom_derouter_key_header: str | None = fastapi.Security(custom_derouter_key_header),
) -> UserAPIKeyAuth:
    """
...
def get_api_key_from_custom_header(request: Request, custom_derouter_key_header_name: str) -> str:
    """
    Get API key from custom header

    Args:
        request (Request): Request object
        custom_derouter_key_header_name (str): Custom header name

    Returns:
        Optional[str]: API key
    """
    api_key: str = ""
    # use this as the virtual key passed to derouter proxy
    custom_derouter_key_header_name = custom_derouter_key_header_name.lower()
    _headers: Final = {k.lower(): v for k, v in request.headers.items()}
    verbose_proxy_logger.debug(
        "searching for custom_derouter_key_header_name= %s, in headers=%s",
        custom_derouter_key_header_name,
        _headers,
    )
    custom_api_key: Final = _headers.get(custom_derouter_key_header_name)
    if custom_api_key:
        ...
        verbose_proxy_logger.exception(
            "No DeRouter Virtual Key pass. Please set header=%s: Bearer <api_key>", custom_derouter_key_header_name
        )
    return api_key
...
async def _lookup_end_user_and_apply_budget(
    ...
    try:
        ...
        if end_user_object is not None:
            ...
            if end_user_object.derouter_budget_table is not None:
                _apply_budget_limits_to_end_user_params(
                    end_user_params=end_user_params,
                    budget_info=end_user_object.derouter_budget_table,
