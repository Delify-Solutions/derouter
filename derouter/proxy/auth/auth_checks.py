...
from derouter.proxy.auth.budget_throttle import (
    budget_throttle_percentage,
    should_throttle_budget_exceeded,
)
from derouter.proxy.auth.route_checks import RouteChecks
from derouter.proxy.common_utils.auth_cache_invalidation_pubsub import publish_auth_cache_invalidation
...
from derouter.proxy.db.exception_handler import PrismaDBExceptionHandler
from derouter.proxy.guardrails.tool_name_extraction import (
...
from derouter.proxy.utils import PrismaClient, ProxyLogging, log_db_metrics
from derouter.repositories.budget_repository import BudgetRepository
from derouter.repositories.object_permission_repository import ObjectPermissionRepository
from derouter.repositories.organization_repository import OrganizationRepository
from derouter.repositories.prisma_protocols import RowT_co
from derouter.repositories.project_repository import ProjectRepository
...
from derouter.types.proxy.model_access_group_budget import ModelAccessGroupBudget
...
class _PrismaModelAccessGroupBudgetRow(Protocol):
    ...
    @property
    def derouter_budget_table(self) -> _PrismaMaxBudgetRow | None: ...
...
class _TeamMembershipModelScope(Protocol):
    @property
    def derouter_budget_table(self) -> _MemberModelScope | None: ...
...
def _member_allowed_models(membership: _TeamMembershipModelScope) -> Sequence[str]:
    ...
    budget_table: Final = membership.derouter_budget_table
...
all_routes: Final = DeRouterRoutes.openai_routes.value + DeRouterRoutes.management_routes.value
...
def _is_model_cost_zero(model: str | list[str] | None, llm_router: Router | None) -> bool:
    ...
    Args:
        ...
        llm_router: The DeRouter router instance
    ...
    for model_name in model_list:
        ...
        try:
            ...
            # See: https://github.com/Delify-Solutions/derouter/issues/24770
...
def _is_cost_explicitly_configured(model: str, llm_router: "Router") -> bool:
    ...
    set in its derouter.model_cost entry.
    ...
    for deployment in llm_router.model_list:
        ...
        if model_id is None:
            continue
        raw_entry = derouter.model_cost.get(model_id, {})
...
def _model_group_has_pricing(model: str, llm_router: "Router") -> bool:
    ...
    tiered_pricing in its derouter_params, even at zero, or when its resolved model info carries
    ...
    for deployment in llm_router.get_model_list(model_name=model) or ():
        derouter_params = deployment.get("derouter_params") or _EMPTY_COST_ENTRY
        if _entry_declares_price(derouter_params):
            return True
        ...
        model_info = llm_router.get_deployment_model_info(
            model_id=model_id, model_name=derouter_params.get("model") or ""
        )
...
def _group_declares_explicit_cost(model: str, llm_router: "Router") -> bool:
    ...
    derouter_params, and reaching that entry through the router's own resolution keeps an alias
    ...
    for deployment in llm_router.get_model_list(model_name=model) or ():
        ...
        if model_id is None:
            continue
        raw_entry = derouter.model_cost.get(model_id, _EMPTY_COST_ENTRY)
        if "input_cost_per_token" in raw_entry or "output_cost_per_token" in raw_entry:
            return True
    return False
...
def _unpriced_models_block_message(models: tuple[str, ...]) -> str:
    ...
    return (
        f"{subject} no pricing in the cost map, so derouter cannot price the request. "
...
async def _run_project_checks(
    project_object: DeRouter_ProjectTableCachedObj | None,
...
def _enforce_user_param_check(general_settings: dict, request: Request, request_body: dict, route: str) -> None:
    ...
    is_mcp_route: Final = route in DeRouterRoutes.mcp_routes.value or RouteChecks.check_route_access(
...
def _global_proxy_budget_check(global_proxy_spend: float | None, skip_budget_checks: bool, route: str) -> None:
    if (
        ...
        if math.isfinite(derouter.max_budget) and global_proxy_spend > derouter.max_budget:
            raise derouter.BudgetExceededError(
...
def _guardrail_modification_check(request_body: Mapping[str, object], team_object: DeRouter_TeamTable | None) -> None:
    """
    ...
    """
    from derouter.proxy.guardrails.guardrail_helpers import can_modify_guardrails
    ...
    def _coerce_to_dict(container: object) -> dict | None:
        ...
        no-modification, and ``add_derouter_data_to_request`` parses the string
    ...
    modifies: Final = (
        ...
        or _user_requested_modification(request_body.get("derouter_metadata"))
...
async def common_checks(
    ...
    project_object: DeRouter_ProjectTableCachedObj | None = None,
) -> bool:
    """
    ...
    7. [OPTIONAL] If 'derouter.max_budget' is set (>0), is proxy under budget
    ...
    unpriced_models: Final = (
        ...
        if derouter.block_requests_for_models_without_pricing and RouteChecks.is_llm_api_route(route=route)
        else ()
    )
    if unpriced_models:
        raise ProxyException(
    ...
    if _model and team_object:
        with tracer.trace("derouter.proxy.auth.common_checks.can_team_access_model"):
            try:
    ...
    if _model and team_object and valid_token and valid_token.user_id:
        with tracer.trace("derouter.proxy.auth.common_checks.check_team_member_model_access"):
    ...
    if valid_token is not None and valid_token.agent_id:
        from derouter.proxy.agent_endpoints.agent_registry import global_agent_registry
        ...
        if agent is not None:
            require_trace_id: Final = (agent.derouter_params or {}).get("require_trace_id_on_calls_by_agent")
            if require_trace_id:
                headers_dict: Final = dict(request.headers)
                trace_id: Final = get_chain_id_from_headers(headers_dict)
                if not trace_id:
                    raise ProxyException(
                        message="Requests made with this agent's key must include the x-derouter-trace-id header.",
                        type=ProxyErrorTypes.bad_request_error,
                        param=None,
                        code=status.HTTP_400_BAD_REQUEST,
                    )
    ...
    if _model and team_object is None and user_object is not None:
        with tracer.trace("derouter.proxy.auth.common_checks.can_user_call_model"):
    ...
    with tracer.trace("derouter.proxy.auth.common_checks.run_project_checks"):
    ...
    if not skip_all_budget_checks:
        ...
        budget_check_coros: Final = tuple(
            ...
            for coro in (
                ...
                if end_user_object is not None and end_user_object.derouter_budget_table is not None
                else None,
            )
            if coro is not None
        )

        with tracer.trace("derouter.proxy.auth.common_checks.budget_checks"):
    ...
    # 11. [OPTIONAL] Vector store checks - is the object allowed to access the vector store
    with tracer.trace("derouter.proxy.auth.common_checks.vector_store_access_check"):
    ...
    with tracer.trace("derouter.proxy.auth.common_checks.check_tools_allowlist"):
...
def _get_user_role(
    user_obj: DeRouter_UserTable | None,
...
def _is_api_route_allowed(
    ...
    request_data: dict,
    valid_token: UserAPIKeyAuth | None,
    user_obj: DeRouter_UserTable | None = None,
) -> bool:
    """
...
def _is_user_proxy_admin(user_obj: DeRouter_UserTable | None):
    if user_obj is None:
        return False
...
def _allowed_routes_check(user_route: str, allowed_routes: list) -> bool:
    ...
    - allowed_routes: List[str|DeRouterRoutes] - the list of allowed routes for the user. Entries are a route group name
      (e.g. "openai_routes"), an exact route, or a trailing-wildcard prefix (e.g. "/internal-models/*").
    """
    from starlette.routing import compile_path

    for allowed_route in allowed_routes:
        if allowed_route in DeRouterRoutes.__members__:
            for template in DeRouterRoutes[allowed_route].value:
...
def allowed_routes_check(
    ...
    user_route: str,
    derouter_proxy_roles: DeRouter_JWTAuth,
) -> bool:
    """
    ...
    if user_role == LitellmUserRoles.PROXY_ADMIN:
        is_allowed = _allowed_routes_check(
            ...
            allowed_routes=derouter_proxy_roles.admin_allowed_routes,
        )
        return is_allowed

    elif user_role == LitellmUserRoles.TEAM:
        if derouter_proxy_roles.team_allowed_routes is None:
            """
            ...
            return is_allowed
        elif derouter_proxy_roles.team_allowed_routes is not None:
            is_allowed = _allowed_routes_check(
                user_route=user_route,
                allowed_routes=derouter_proxy_roles.team_allowed_routes,
            )
            return is_allowed
    return False
...
def get_actual_routes(allowed_routes: list) -> list:
    ...
    for route_name in allowed_routes:
        try:
            route_value = DeRouterRoutes[route_name].value
...
async def get_default_end_user_budget(
...
) -> DeRouter_BudgetTable | None:
    """
    Fetches the default end user budget from the database if derouter.max_end_user_budget_id is configured.
    ...
    Returns:
        DeRouter_BudgetTable if configured and found, None otherwise
    """
    if prisma_client is None or derouter.max_end_user_budget_id is None:
        return None

    cache_key: Final = f"default_end_user_budget:{derouter.max_end_user_budget_id}"

    # Check cache first
    cached_budget: Final = await user_api_key_cache.async_get_cache(
        key=cache_key,
        model_type=DeRouter_BudgetTable,
    )
    ...
    try:
        budget_record: Final = await _dictable_table(BudgetRepository(prisma_client)).find_unique(
            where={"budget_id": derouter.max_end_user_budget_id}
        )

        if budget_record is None:
            verbose_proxy_logger.warning(
                "Default end user budget not found in database: %s", derouter.max_end_user_budget_id
            )
            return None

        _budget_obj: Final = DeRouter_BudgetTable.model_validate(budget_record.dict())
...
async def get_team_member_default_budget(
    ...
    Returns:
        DeRouter_BudgetTable if found, None otherwise
    """
    ...
    budget: Final = DeRouter_BudgetTable.model_validate(budget_record.dict())
    await user_api_key_cache.async_set_cache(
        key=cache_key,
        value=budget,
        model_type=DeRouter_BudgetTable,
...
async def _apply_default_budget_to_end_user(
    end_user_obj: DeRouter_EndUserTable,
    ...
    if end_user_obj.derouter_budget_table is not None:
        return end_user_obj

    # If no default budget configured, return as-is
    if derouter.max_end_user_budget_id is None:
        return end_user_obj
    ...
    if default_budget is not None:
        ...
        end_user_obj.derouter_budget_table = default_budget
        verbose_proxy_logger.debug(
            "Applied default budget %s to end user %s", derouter.max_end_user_budget_id, end_user_obj.user_id
        )
...
async def _check_end_user_budget(
    ...
    Raises:
        derouter.BudgetExceededError: If end user has exceeded their budget
    """
    if RouteChecks.is_info_route(route):
        return

    if end_user_obj.derouter_budget_table is None:
        return

    end_user_budget: Final = end_user_obj.derouter_budget_table.max_budget
...
async def _load_end_user_restricted_registry(
    ...
    """The set of end-user ids whose ``DeRouter_EndUserTable`` row carries a restriction."""
...
async def _end_user_is_known_unrestricted(
    ...
    """
    if (
        derouter.max_end_user_budget_id is not None
        or derouter.validate_end_user_id_in_db
...
async def get_end_user_object(
    ...
    Returns:
        DeRouter_EndUserTable if found, None otherwise
    """
    if prisma_client is None:
    ...
    cached_user_obj: Final = await user_api_key_cache.async_get_cache(
        key=_key,
        model_type=DeRouter_EndUserTable,
    )
    ...
    try:
        response: Final = await _dictable_table(EndUserRepository(prisma_client)).find_unique(
            ...
            include={"derouter_budget_table": True, "object_permission": True},
        )

        if response is None:
            raise Exception

        # Convert to DeRouter_EndUserTable object
        _response = DeRouter_EndUserTable.model_validate(response.dict())
...
async def resolve_and_validate_end_user_id(
    ...
    Default: pass-through. DeRouter's documented pattern is that the `user`
    field is an arbitrary caller-supplied identifier, so validation is
    opt-in behind ``derouter.validate_end_user_id_in_db`` to preserve
    backwards compatibility.

    When the flag is set: accept the id when it matches any of
      - DeRouter_EndUserTable.user_id
      - DeRouter_UserTable.user_id
      - DeRouter_UserTable.user_email (case-insensitive)

    If the id doesn't match but ``derouter.max_end_user_budget_id`` is set,
    ...
    if raw_end_user_id is None:
        return None
    if not derouter.validate_end_user_id_in_db:
    ...
    if cached == "valid":
        return raw_end_user_id
    if cached == "invalid":
        return raw_end_user_id if derouter.max_end_user_budget_id else None
    ...
    if is_valid:
        return raw_end_user_id
    # Preserve id so the caller can still apply derouter.max_end_user_budget_id.
    if derouter.max_end_user_budget_id:
        return raw_end_user_id
    return None
...
async def _load_tag_registry(
    ...
    """The set of tag names that have a row in ``DeRouter_TagTable``."""
...
async def _load_model_access_group_registry(
    ...
    """The set of model access group names that have a row in ``DeRouter_ModelAccessGroupBudgetTable``."""
...
def _model_access_group_budget(row: _PrismaModelAccessGroupBudgetRow) -> ModelAccessGroupBudget:
    budget_table: Final = row.derouter_budget_table
...
async def _fetch_uncached_tags(
    ...
    prisma_client: PrismaClient,
    user_api_key_cache: UserApiKeyCache,
) -> tuple[tuple[str, DeRouter_TagTable], ...]:
    ...
    try:
        ...
        fetched: Final = tuple((db_tag.tag_name, DeRouter_TagTable.model_validate(db_tag.dict())) for db_tag in db_tags)
        for fetched_name, fetched_obj in fetched:
            await user_api_key_cache.async_set_cache(
...
async def get_tag_objects_batch(
    ...
    2. Skipping tags that no ``DeRouter_TagTable`` row exists for, via the cached name registry
    ...
    Returns:
        Dictionary mapping tag_name to DeRouter_TagTable object
    """
    ...
    probed: Final = [
        (
            tag_name,
            await user_api_key_cache.async_get_cache(key=tag_cache_key(tag_name), model_type=DeRouter_TagTable),
        )
...
async def get_tag_object(
    ...
    Returns:
        DeRouter_TagTable object if found, None otherwise
    """
...
async def get_team_membership(
    ...
    parent_otel_span: Span | None = None,
    proxy_logging_obj: ProxyLogging | None = None,
) -> Optional["DeRouter_TeamMembership"]:
    """
    ...
    cached_membership_obj: Final = await user_api_key_cache.async_get_cache(
        key=_key,
        model_type=DeRouter_TeamMembership,
    )
    ...
    try:
        ...
        _response: Final = DeRouter_TeamMembership.model_validate(response.dict())
        await user_api_key_cache.async_set_cache(
            key=_key,
            value=_response,
            model_type=DeRouter_TeamMembership,
...
async def get_user_object(
    ...
    - if valid, return DeRouter_UserTable object with defined limits
    ...
    try:
        ...
        if response is None:
            if user_id_upsert:
                ...
                default_params: Final = derouter.default_internal_user_params or {}
        ...
        if response.organization_memberships is not None and len(response.organization_memberships) > 0:
            # dump each organization membership to type DeRouter_OrganizationMembershipTable
            _dumped_memberships: Final = [
                DeRouter_OrganizationMembershipTable.model_validate(membership.model_dump())
                for membership in response.organization_memberships
                if membership is not None
            ]
            response.organization_memberships = _dumped_memberships

        _response = DeRouter_UserTable.model_validate(dict(response))
...
async def _cache_team_object(
    ...
    await _cache_management_object(
        ...
        user_api_key_cache=user_api_key_cache,
        proxy_logging_obj=proxy_logging_obj,
        model_type=DeRouter_TeamTableCachedObj,
    )
...
async def invalidate_team_member_spend_state(
    ...
    Two independently-keyed cache entries hold the same DeRouter_TeamMembership
...
async def _get_team_db_check(
    ...
    if response is None and team_id_upsert:
        ...
        response = DeRouter_TeamTable.model_validate(created_team_dict)
    return response
...
async def _get_team_object_from_user_api_key_cache(
    ...
    _response: Final = DeRouter_TeamTableCachedObj.model_validate(response.dict())
...
) -> DeRouter_TeamTableCachedObj | None:
...
async def get_team_object(
    ...
    - if valid, return DeRouter_TeamTable object with defined limits
    - if not, then raise an error
    ...
    """
    if prisma_client is None:
        raise Exception("No DB Connected. See - https://router.delify.vn/docs/proxy/virtual_keys")
...
async def _cache_access_object(
    access_group_id: str,
    access_group_table: DeRouter_AccessGroupTable,
    user_api_key_cache: UserApiKeyCache,
    proxy_logging_obj: ProxyLogging | None = None,
):
    ...
    await user_api_key_cache.async_set_cache(
        ...
        model_type=DeRouter_AccessGroupTable,
...
async def get_access_object(
    ...
    - if valid, return DeRouter_AccessGroupTable object
    - if not, then raise an error
    ...
    try:
        ...
        _response: Final = DeRouter_AccessGroupTable.model_validate(response.dict())
...
async def get_team_object_by_alias(
    ...
    Returns:
        DeRouter_TeamTableCachedObj: The team object if found
    ...
    try:
        ...
        team: Final = teams[0]
        team_obj: Final = DeRouter_TeamTableCachedObj.model_validate(team.model_dump())
...
async def get_org_object_by_alias(
    ...
    Returns:
        DeRouter_OrganizationTable if found, None otherwise
    ...
    cached_org_obj: Final = await user_api_key_cache.async_get_cache(
        key=cache_key,
        model_type=DeRouter_OrganizationTable,
    )
    ...
    try:
        ...
        org: Final = orgs[0]
        org_obj: Final = DeRouter_OrganizationTable.model_validate(org.model_dump())
...
class ExperimentalUIJWTToken:
    @staticmethod
    def get_experimental_ui_login_jwt_auth_token(user_info: DeRouter_UserTable) -> str:
        ...
        # Experimental UI flow uses fixed 10-min expiry for security (does not use DEROUTER_UI_SESSION_DURATION)
    ...
    def get_cli_jwt_auth_token(
        ...
        The expiration time can be controlled via the DEROUTER_CLI_JWT_EXPIRATION_HOURS
        ...
        # Calculate expiration time (configurable via DEROUTER_CLI_JWT_EXPIRATION_HOURS env var)
...
async def get_object_permission(
    ...
    - if valid, return DeRouter_ObjectPermissionTable object
    - if not, then raise an error
    """
    if prisma_client is None:
        raise Exception("No DB Connected. See - https://router.delify.vn/docs/proxy/virtual_keys")
    ...
    deserialized_perm: Final = await user_api_key_cache.async_get_cache(
        key=key,
        model_type=DeRouter_ObjectPermissionTable,
    )
    ...
    try:
        ...
        _perm_obj: Final = DeRouter_ObjectPermissionTable.model_validate(response.dict())
        await user_api_key_cache.async_set_cache(
            key=key,
            value=_perm_obj,
            model_type=DeRouter_ObjectPermissionTable,
...
async def get_managed_vector_store_rows_by_uuids(
    ...
    parent_otel_span: Span | None = None,
    proxy_logging_obj: ProxyLogging | None = None,
) -> list[DeRouter_ManagedVectorStoresTable]:
    """
    ...
    result: Final[list[DeRouter_ManagedVectorStoresTable]] = []
    ...
    for uuid in uuids:
        ...
        deserialized_vs = await user_api_key_cache.async_get_cache(
            key=key,
            model_type=DeRouter_ManagedVectorStoresTable,
        )
    ...
    for row in rows:
        ...
        if not row_dict:
            continue
        cached_obj = DeRouter_ManagedVectorStoresTable.model_validate(row_dict)
...
async def get_org_object(
    ...
    - if valid, return DeRouter_OrganizationTable object
    - if not, then raise an error
    ...
    Args:
        ...
        parent_otel_span: Optional OpenTelemetry span
        proxy_logging_obj: Optional proxy logging object
        include_budget_table: If True, includes derouter_budget_table in the query
    """
    if prisma_client is None:
        raise Exception("No DB Connected. See - https://router.delify.vn/docs/proxy/virtual_keys")
    ...
    try:
        ...
        if include_budget_table:
            query_kwargs["include"] = {"derouter_budget_table": True}
    ...
    _org_obj: Final = DeRouter_OrganizationTable.model_validate(response.model_dump())
    # Cache the result
    await user_api_key_cache.async_set_cache(
        key=cache_key,
        value=_org_obj,
...
async def _get_resources_from_access_groups(
    ...
    if prisma_client is None or user_api_key_cache is None:
        ...
        from derouter.proxy.proxy_server import proxy_logging_obj as _proxy_logging_obj
        from derouter.proxy.proxy_server import user_api_key_cache as _user_api_key_cache
...
def _can_object_call_model(
    ...
    if model in derouter.model_alias_map:
        potential_models.append(derouter.model_alias_map[model])
...
async def can_key_call_model(
    ...
    valid_token: UserAPIKeyAuth,
    llm_router: derouter.Router | None,
) -> Literal[True]:
    """
    Checks if token can call a given model
...
async def can_key_call_resolved_model(
    ...
    team_object: DeRouter_TeamTableCachedObj | None = None
    ...
    if valid_token.team_id is not None:
        try:
            ...
            team_object = DeRouter_TeamTableCachedObj(
                team_id=valid_token.team_id,
...
async def get_authorized_resources_from_key_access_groups(
    ...
    """
    For each access_group_id on the key, fetch the DeRouter_AccessGroupTable row
...
def can_project_access_model(
    model: str | list[str],
    project_object: DeRouter_ProjectTableCachedObj,
    llm_router: Router | None,
) -> Literal[True]:
    """
...
def _search_tool_names_from_object_permission(
    object_permission: DeRouter_ObjectPermissionTable | None,
) -> list[str]:
...
# The shape abbreviate_api_key writes into DeRouter_VerificationToken.key_name. The
...
async def _virtual_key_max_budget_alert_check(
    ...
    if valid_token.max_budget is not None and valid_token.spend is not None and valid_token.spend > 0:
        ...
        alert_email_config: Final[dict[str, list[str]] | None] = _merge_budget_alert_email_configs(
            global_cfg=derouter.default_key_max_budget_alert_emails,
...
async def _check_team_member_budget(
    ...
    if (
        ...
        if (
            ...
            and team_membership.derouter_budget_table is not None
            and team_membership.derouter_budget_table.max_budget is not None
        ):
            team_member_budget = team_membership.derouter_budget_table.max_budget
        else:
...
async def _check_team_member_model_access(
    ...
    if (
        ...
        or team_membership.derouter_budget_table is None
        or not team_membership.derouter_budget_table.allowed_models
    ):
        return  # no per-member restriction — inherit team-level check

    member_allowed_models: Final[list[str]] = team_membership.derouter_budget_table.allowed_models
    try:
        _can_object_call_model(
            model=model,
            llm_router=llm_router,
...
async def _project_max_budget_check(
    ...
    max_budget = None
    if project_object.derouter_budget_table is not None:
        max_budget = project_object.derouter_budget_table.max_budget
...
async def _project_soft_budget_check(
    ...
    soft_budget = None
    if project_object.derouter_budget_table is not None:
        soft_budget = project_object.derouter_budget_table.soft_budget
...
async def get_project_object(
    ...
    project_obj: Final = DeRouter_ProjectTableCachedObj.model_validate(project_row.model_dump())
...
async def delete_cached_project_object(
    ...
    """
    Every endpoint that mutates derouter_projecttable must call this, or a stale project (e.g. a
    pre-update empty model allowlist) keeps being enforced until the TTL expires (LIT-3803).
    """
    from derouter.proxy.common_utils.auth_cache_invalidation_pubsub import evict_and_broadcast
...
async def _organization_max_budget_check(
    ...
    if org_table.derouter_budget_table is not None:
        org_max_budget = org_table.derouter_budget_table.max_budget
...
async def _tag_max_budget_check(
    ...
    for tag_name in tags:
        ...
        if tag_object.derouter_budget_table is not None and tag_object.derouter_budget_table.max_budget is not None:
            from derouter.proxy.proxy_server import get_current_spend

            tag_spend = await get_current_spend(
                counter_key=f"spend:tag:{tag_name}",
                fallback_spend=tag_object.spend or 0.0,
                max_budget=tag_object.derouter_budget_table.max_budget,
                fallback_authoritative=True,
            )
            if tag_spend <= tag_object.derouter_budget_table.max_budget:
                continue
            raise derouter.BudgetExceededError(
                current_cost=tag_spend,
                max_budget=tag_object.derouter_budget_table.max_budget,
                message=f"Budget has been exceeded! Tag={tag_name} Current cost: {tag_spend}, Max budget: {tag_object.derouter_budget_table.max_budget}",
...
async def vector_store_access_check(
    ...
    vector_store_ids_to_run: Final = derouter.vector_store_registry.get_vector_store_ids_to_run(
