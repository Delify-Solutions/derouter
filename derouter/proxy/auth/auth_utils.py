...
import derouter
from derouter import Router, constants, provider_list
...
from derouter.proxy.common_utils.http_parsing_utils import extract_nested_form_metadata
from derouter.types.passthrough_endpoints.pass_through_endpoints import (
    DEROUTER_PASS_THROUGH_ENDPOINT_MARKER,
)
from derouter.types.router import CONFIGURABLE_CLIENTSIDE_AUTH_PARAMS
...
def check_complete_credentials(request_body: dict) -> bool:
    ...
    if getattr(derouter, "user_url_validation", False):
...
# API calls. ``derouter_embedding_config`` is consumed by the Milvus
...
_NESTED_CONFIG_KEYS: Final[tuple[str, ...]] = ("derouter_embedding_config", "extra_body")
...
_NESTED_METADATA_KEYS: Final[tuple[str, ...]] = ("metadata", "derouter_metadata")
...
def _build_banned_observability_params() -> frozenset[str]:
    ...
    ``derouter/derouter_core_utils/initialize_dynamic_callback_params.py`` is
...
_BANNED_REQUEST_BODY_PARAMS: Final[tuple[str, ...]] = (
    ...
    *sorted(CustomPricingDeRouterParams.model_fields.keys()),
)
...
def _iter_fallback_targets(value: object, depth: int) -> Iterator[str | Mapping[str, object]]:
    if depth > 2 * derouter.ROUTER_MAX_FALLBACKS:
...
def _reject_url_valued_fallback_target(value: str) -> None:
    ...
    for candidate in provider_url_destination_candidates(value):
        ...
        raise ValueError(
            ...
            "`provider_url_destination_allowed_hosts` in derouter_settings."
        )
...
def is_request_body_safe(request_body: dict, general_settings: dict, llm_router: Router | None, model: str) -> bool:
    ...
    ``derouter_embedding_config.api_base`` (VERIA-6) without exposing a
    ...
    derouter_params: Final = _coerce_metadata_to_dict(request_body.get("derouter_params"))
    if derouter_params is not None:
        derouter_params_metadata: Final = _coerce_metadata_to_dict(derouter_params.get("metadata"))
        if derouter_params_metadata is not None:
            _check_banned_params(
                derouter_params_metadata,
                general_settings,
                llm_router,
                model,
            )
...
def _coerce_metadata_to_dict(value: Any) -> dict[str, Any] | None:
    ...
    Multipart/form-data and ``extra_body`` callers send ``derouter_metadata``
    as a JSON-encoded string; the proxy parses it into a dict later in
    ``add_derouter_data_to_request``, but the auth-time bouncer runs first
...
async def pre_db_read_auth_checks(
    ...
    """
    from derouter.proxy.proxy_server import general_settings, llm_router, premium_user
...
def route_in_additonal_public_routes(current_route: str):
    ...
    In order to use this the derouter config.yaml should have the following in general_settings:

    ```yaml
    general_settings:
        master_key: sk-1234
        public_routes: ["DeRouterRoutes.public_routes", "/spend/calculate", "/api/*"]
    ```
    """
    from derouter.proxy.auth.route_checks import RouteChecks
    from derouter.proxy.proxy_server import general_settings, premium_user
...
def _get_deployment_default_limit(model_name: str, field: str) -> int | None:
    ...
    for deployment in deployments:
        raw = deployment.get("derouter_params", {}).get(field)
        if raw is not None:
            try:
...
def custom_auth_common_checks_warning(
    *,
    ...
    return (
        ...
        "Docs: https://router.delify.vn/docs/proxy/custom_auth"
    )
...
def _coerce_user_id_to_str(value: Any) -> str | None:
    ...
    are only rejected when ``derouter.validate_end_user_id_in_db`` is enabled
    ...
    if isinstance(value, str):
        ...
        if derouter.validate_end_user_id_in_db and stripped[:1] in ("{", "["):
...
def get_end_user_id_from_request_body(request_body: dict, request_headers: dict | None = None) -> str | None:
    ...
    def _as_dict(value: Any) -> dict:
        # metadata / derouter_metadata can arrive as JSON strings from
    ...
    # Check 4: 'derouter_metadata.user' in request_body (commonly Anthropic)
    derouter_metadata: Final = _as_dict(request_body.get("derouter_metadata"))
    user_id_str = _coerce_user_id_to_str(derouter_metadata.get("user"))
    if user_id_str:
        return user_id_str
...
MODEL_ROUTING_HEADER_NAME: Final = "x-derouter-model"
...
def _extract_models_from_managed_resource_id(
    ...
    try:
        from derouter.llms.base_llm.managed_resources.utils import parse_unified_id
...
def request_dispatched_to_pass_through_endpoint(request: Request | None) -> bool:
    ...
    return getattr(endpoint, DEROUTER_PASS_THROUGH_ENDPOINT_MARKER, False) is True
...
def get_model_from_request(
    ...
    ``model`` field there names an upstream model, not a DeRouter-managed one, and
