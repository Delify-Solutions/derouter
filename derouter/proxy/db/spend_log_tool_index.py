...
job drains the queue into DeRouter_SpendLogToolIndex (per-request drill-down) and
DeRouter_DailyToolSpend (the per-day rollup the Cost Optimization card reads) in a
...
def build_tool_usage_transaction(
    ...
    same UTC day bucket as DeRouter_DailyUserSpend."""
    mcp_names: Final = (
    ...
    for attempt in range(n_retry_times + 1):
        try:
            async with prisma_client.db.batch_() as batcher:
                batcher.derouter_spendlogtoolindex.create_many(data=index_rows, skip_duplicates=True)
                for (date_key, tool_name), grouped in groupby(per_tool_day, key=lambda entry: (entry[0], entry[1])):
                    ...
                    batcher.derouter_dailytoolspend.upsert(
