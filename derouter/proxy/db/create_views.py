...
async def create_missing_views(db: SupportsRawQueries) -> None:
    """
    --------------------------------------------------
    NOTE: Copy of `derouter/db_scripts/create_views.py`.
    --------------------------------------------------
    ...
    try:
        ...
        await db.query_raw("""SELECT 1 FROM "DeRouter_VerificationTokenView" LIMIT 1""")
        verbose_logger.debug("DeRouter_VerificationTokenView Exists!")
    except Exception as e:
        ...
        await create_view_tolerating_race(
            ...
            """
                ...
                LEFT JOIN "DeRouter_ProjectTable" p ON v.project_id = p.project_id;
            """,
        )
    ...
    try:
        ...
        FROM
        "DeRouter_SpendLogs" L
        LEFT JOIN 
        "DeRouter_VerificationToken" V
        ON
        L."api_key" = V."token"
        WHERE
...
async def should_create_missing_views(db: SupportsRawQueries) -> bool:
    ...
    verbose_logger.debug("Estimated Row count of DeRouter_SpendLogs = %s", result)
    if (
        result
