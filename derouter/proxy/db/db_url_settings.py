...
``helm/derouter`` chart's ``derouter.serverEnv`` block
(``helm/derouter/templates/_helpers.tpl``). Both auth styles and both
...
def unsupported_db_scheme_message(env_var: str, scheme: str) -> str:
    ...
    return (
        f"{env_var} uses unsupported scheme '{scheme}'. DeRouter's database "
