...
class WindowSpendTransaction(TypedDict):
    ...
    DeRouter_SpendLogs rows from everything else, because the spend log writer
...
def to_naive_utc(value: datetime) -> datetime:
    """DeRouter_BudgetWindowSpend.window_start is TIMESTAMP(3), which holds naive UTC."""
...
class WindowSpendUpdateQueue(BaseUpdateQueue):
    ...
    def __init__(self) -> None:
        ...
        self.update_queue: asyncio.Queue[tuple[WindowSpendTransaction, ...]] = asyncio.Queue(
            maxsize=DEROUTER_ASYNCIO_QUEUE_MAXSIZE
        )
