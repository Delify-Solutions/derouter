...
from derouter.constants import SPEND_COUNTER_RESEED_LOCKS_MAX_SIZE
...
if TYPE_CHECKING:
    from prisma.types import DeRouter_EndUserTableWhereUniqueInput
...
class SpendCounterReseed:
    ...
    Counter key prefixes map to DB tables:
        spend:key:{token}                 -> DeRouter_VerificationToken.spend
        spend:team:{team_id}              -> DeRouter_TeamTable.spend
        spend:team_member:{uid}:{tid}     -> DeRouter_TeamMembership.spend
        spend:user:{user_id}              -> DeRouter_UserTable.spend
        spend:org:{org_id}                -> DeRouter_OrganizationTable.spend
    ...
    reset still caches the pre-reset end-user object, and DeRouter_EndUserTable
    ...
    async def end_user_from_db(prisma_client: Optional["PrismaClient"], counter_key: str) -> float | None:
        if prisma_client is None or not counter_key.startswith(END_USER_COUNTER_PREFIX):
            return None
        where: Final[DeRouter_EndUserTableWhereUniqueInput] = {"user_id": counter_key[len(END_USER_COUNTER_PREFIX) :]}
        try:
