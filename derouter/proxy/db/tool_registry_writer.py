"""
DB helpers for DeRouter_ToolTable — the global tool registry.
...
from derouter.proxy._types import ToolDiscoveryQueueItem
...
from derouter.repositories.table_repositories import ToolRepository
...
def _tool_table_actions(prisma_client: "PrismaClient") -> "TableActions[prisma_db_models.DeRouter_ToolTable]":
    table: Final[TableActions[prisma_db_models.DeRouter_ToolTable]] = ToolRepository(prisma_client).table
    return table


def _object_permission_table_actions(
    prisma_client: "PrismaClient",
) -> "TableActions[prisma_db_models.DeRouter_ObjectPermissionTable]":
    table: Final[TableActions[prisma_db_models.DeRouter_ObjectPermissionTable]] = ObjectPermissionRepository(
        prisma_client
    ).table
    return table


def _row_to_model(row: object) -> DeRouter_ToolTableRow:
    """Convert a Prisma model instance or dict to DeRouter_ToolTableRow."""
