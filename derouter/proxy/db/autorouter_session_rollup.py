...
DeRouter_AutoRouterSession with one conditional upsert per turn: the statement classifies
...
from derouter.proxy._types import DB_RETRY_SAFE_ERROR_TYPES
...
WITH windowed AS (
    SELECT * FROM "DeRouter_AutoRouterSession"
...
def turn_cache_facts(usage_object: Mapping[str, object] | None) -> TurnCacheFacts:
    from derouter.proxy.spend_tracking.savings import extract_cache_read_tokens
...
def _write_ttl_seconds(usage_object: Mapping[str, object] | None) -> int | None:
    ...
    """
    from derouter.proxy.spend_tracking.savings import extract_cache_creation_tokens
...
INSERT INTO "DeRouter_AutoRouterSession" AS t (
