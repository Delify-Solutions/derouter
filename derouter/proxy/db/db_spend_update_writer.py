...
from derouter.caching import RedisCache
from derouter.constants import (
    DB_DAILY_TAG_SPEND_UPDATE_JOB_NAME,
    DB_SPEND_UPDATE_JOB_NAME,
    INTERNAL_CALL_ORIGIN_METADATA_KEY,
)
from derouter.derouter_core_utils.derouter_logging import coerce_model_access_groups
...
from derouter.proxy.db.daily_spend_bulk_upsert import (
    DAILY_SPEND_TABLES,
    build_bulk_upsert,
    merge_by_conflict_key,
)
from derouter.proxy.db.db_transaction_queue.daily_spend_update_queue import (
    DailySpendUpdateQueue,
)
from derouter.proxy.db.db_transaction_queue.pod_lock_manager import PodLockManager
from derouter.proxy.db.db_transaction_queue.redis_update_buffer import RedisUpdateBuffer
from derouter.proxy.db.db_transaction_queue.spend_update_queue import SpendUpdateQueue
from derouter.proxy.db.db_transaction_queue.tool_discovery_queue import (
    ToolDiscoveryQueue,
)
...
from derouter.proxy.route_llm_request import ROUTE_ENDPOINT_MAPPING
from derouter.proxy.spend_tracking.compression_savings import (
    extract_compression_saved_tokens,
)
from derouter.proxy.spend_tracking.savings import (
...
from derouter.repositories.prisma_protocols import BatchTable
...
class _SpendBatch(Protocol):
    derouter_usertable: BatchTable
    derouter_verificationtoken: BatchTable
    derouter_teamtable: BatchTable
    derouter_teammembership: BatchTable
    derouter_organizationtable: BatchTable
    derouter_tagtable: BatchTable
    derouter_agentstable: BatchTable
    derouter_modelaccessgroupbudgettable: BatchTable
...
class DBSpendUpdateWriter:
    ...
    async def update_database(
        # DeRouter management object fields
        self,
        token: str | None,
        user_id: str | None,
        ...
        from derouter.proxy.utils import ProxyUpdateSpend, hash_token
        ...
        try:
            ...
            asyncio.create_task(
                self._batch_database_updates(
                    ...
                    prisma_client=prisma_client,
                    derouter_proxy_budget_name=derouter_proxy_budget_name,
                    payload=payload,
    ...
    async def _enqueue_tool_usage_transaction(
        ...
        try:
            if prisma_client is None:
                return
            from derouter.proxy.db.spend_log_tool_index import (
    ...
    async def _enqueue_autorouter_turn_transaction(
        ...
        try:
            ...
            if not isinstance(metadata, dict) or not metadata.get("routing_decision"):
                return
            from derouter.proxy.db.autorouter_session_rollup import (
    ...
    def _enqueue_tool_registry_upsert(
        ...
        for upsert into DeRouter_ToolTable via ToolDiscoveryQueue.
        ...
        try:
            ...
            from derouter.proxy.db.spend_log_tool_index import response_tool_call_names
    ...
    async def _batch_database_updates(
        ...
        *,
        ...
        end_user_id: str | None,
        prisma_client: PrismaClient | None,
        derouter_proxy_budget_name: str | None,
        payload: SpendLogsPayload,
    ...
    async def _update_user_db(
        ...
        - Update derouter-proxy-budget row (global proxy spend)
        """
        try:
            if prisma_client is not None:  # update
                user_ids: Final = [user_id]
                if derouter.max_budget > 0:  # track global proxy budget, if user set max budget
                    user_ids.append(derouter_proxy_budget_name)
    ...
    async def _insert_spend_log_to_db(
        ...
        if prisma_client is not None and spend_logs_url is not None or prisma_client is not None:
            from derouter.proxy.utils import enqueue_spend_logs, request_spend_log_flush
    ...
    async def _commit_window_spend_updates(
        ...
        """
        Commit per-budget-window spend increments to DeRouter_BudgetWindowSpend.
        ...
        DeRouter_SpendLogs, so a dropped increment would let the entity spend
        past its window limit after the next counter reseed.
        """
        from derouter.proxy.db.budget_window_spend_writer import (
    ...
    async def _flush_tool_discovery_queue(
        ...
        """Flush ToolDiscoveryQueue and batch-upsert new tools into DeRouter_ToolTable."""
        from derouter.proxy.db.tool_registry_writer import batch_upsert_tools
    ...
    async def _handle_spend_update_failure(
        ...
        from derouter.proxy.utils import _raise_failed_update_spend_exception
    ...
    async def _commit_spend_updates_to_db(
                    ...
                    async with _spend_update_tx(prisma_client) as transaction:
                        async with transaction.batch_() as batcher:
                            ...
                            for user_id, response_cost in sorted(user_list_transactions.items()):
                                batcher.derouter_usertable.update_many(
                    ...
                    async with _spend_update_tx(prisma_client) as transaction:
                        async with transaction.batch_() as batcher:
                            ...
                            for token, response_cost in sorted(key_list_transactions.items()):
                                batcher.derouter_verificationtoken.update_many(  # 'update_many' prevents error from being raised if no row exists
                                    where={"token": token},
                                    data={
                    ...
                    async with _spend_update_tx(prisma_client) as transaction:
                        async with transaction.batch_() as batcher:
                            ...
                            for team_id, response_cost in sorted(team_list_transactions.items()):
                                ...
                                batcher.derouter_teamtable.update_many(  # 'update_many' prevents error from being raised if no row exists
                    ...
                    async with _spend_update_tx(prisma_client) as transaction:
                        async with transaction.batch_() as batcher:
                            ...
                            for key, response_cost in sorted(team_member_list_transactions.items()):
                                ...
                                batcher.derouter_teammembership.update_many(  # 'update_many' prevents error from being raised if no row exists
                    ...
                    async with _spend_update_tx(prisma_client) as transaction:
                        async with transaction.batch_() as batcher:
                            ...
                            for org_id, response_cost in sorted(org_list_transactions.items()):
                                batcher.derouter_organizationtable.update_many(  # 'update_many' prevents error from being raised if no row exists
        ...
        await DBSpendUpdateWriter._update_entity_spend_in_db(
            ...
            table_accessor="derouter_tagtable",
        ...
        await DBSpendUpdateWriter._update_entity_spend_in_db(
            ...
            table_accessor="derouter_modelaccessgroupbudgettable",
        ...
        await DBSpendUpdateWriter._update_entity_spend_in_db(
            ...
            table_accessor="derouter_agentstable",
    ...
    async def _update_entity_spend_in_db(
        ...
        table_accessor: Literal["derouter_tagtable", "derouter_agentstable", "derouter_modelaccessgroupbudgettable"],
        where_field: str,
        n_retry_times: int,
        prisma_client: PrismaClient,
        proxy_logging_obj: ProxyLogging,
        ...
        Args:
            ...
            table_accessor: Prisma table accessor (e.g., prisma_client.db.derouter_teamtable)
    ...
    async def update_daily_user_spend(
        ...
        """
        Batch job to update DeRouter_DailyUserSpend table using in-memory daily_spend_transactions
        """
    ...
    async def update_daily_team_spend(
        ...
        """
        Batch job to update DeRouter_DailyTeamSpend table using in-memory daily_spend_transactions
        """
        await DBSpendUpdateWriter._update_daily_spend(
            n_retry_times=n_retry_times,
            prisma_client=prisma_client,
    ...
    async def update_daily_org_spend(
        ...
        """
        Batch job to update DeRouter_DailyOrganizationSpend table using in-memory daily_spend_transactions
        """
        await DBSpendUpdateWriter._update_daily_spend(
            n_retry_times=n_retry_times,
            prisma_client=prisma_client,
    ...
    async def update_daily_end_user_spend(
        ...
        """
        Batch job to update DeRouter_DailyEndUserSpend table using in-memory daily_spend_transactions
        """
        await DBSpendUpdateWriter._update_daily_spend(
            n_retry_times=n_retry_times,
            prisma_client=prisma_client,
    ...
    async def update_daily_agent_spend(
        ...
        """
        Batch job to update DeRouter_DailyAgentSpend table using in-memory daily_spend_transactions
        """
        await DBSpendUpdateWriter._update_daily_spend(
            n_retry_times=n_retry_times,
            prisma_client=prisma_client,
    ...
    async def update_daily_tag_spend(
        ...
        """
        Batch job to update DeRouter_DailyTagSpend table using in-memory daily_spend_transactions
        """
        await DBSpendUpdateWriter._update_daily_spend(
            n_retry_times=n_retry_times,
            prisma_client=prisma_client,
    ...
    async def _common_add_spend_log_transaction_to_daily_transaction(
        ...
        # admin UI has fully migrated to DeRouter_DailyGatewayRequests, which is now the
        ...
        # (see derouter/proxy/middleware/billable_request_metrics_middleware.py). The
