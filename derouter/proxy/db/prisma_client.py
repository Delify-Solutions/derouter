...
from derouter.proxy.db.db_url_settings import add_missing_query_params, connection_params_from_url
...
class PrismaManager:
    ...
    def _apply_replica_identity_full_if_requested() -> None:
        """
        `prisma db push` bypasses derouter-proxy-extras, so the opt-in
        REPLICA IDENTITY FULL step has to be driven from here too.

        derouter-proxy-extras is an optional install, so this is a no-op when it
        is absent.
        """
        try:
    ...
    def _raise_if_partitioned_spend_logs() -> None:
        """`prisma db push` rewrites a doc-partitioned DeRouter_SpendLogs
        ...
        derouter-proxy-extras is absent."""
        try:
            ...
            try:
                if use_migrate:
                    try:
                        ...
                        verbose_proxy_logger.error("\x1b[1;31mDeRouter: Failed to import proxy extras. Got %s\x1b[0m", e)
                        return False
