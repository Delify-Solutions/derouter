"""
Writer for DeRouter_BudgetWindowSpend.
...
instead of aggregating DeRouter_SpendLogs every time a window counter goes cold
...
Seeding a row that does not exist yet reads DeRouter_SpendLogs once and takes
...
_UPSERT_WINDOW_SPEND_SQL: Final = (
    ...
    "spend = CASE "
    'WHEN "DeRouter_BudgetWindowSpend".window_start >= EXCLUDED.window_start '
    'THEN "DeRouter_BudgetWindowSpend".spend + $6 '
    "ELSE EXCLUDED.spend "
    "END, "
    'window_start = GREATEST("DeRouter_BudgetWindowSpend".window_start, EXCLUDED.window_start), '
    "updated_at = ($7::timestamptz AT TIME ZONE 'UTC')"
)

_ROLL_WINDOW_SPEND_SQL: Final = (
    'UPDATE "DeRouter_BudgetWindowSpend" SET '
...
class WindowSpendLogsAggregate(Protocol):
    """Sums DeRouter_SpendLogs for one entity since window_start, split at the
...
async def spend_logs_seed_totals(
    ...
    """DeRouter_SpendLogs spend for one entity since window_start, both in full
...
def _exclusion_upper_bound(started_at: datetime) -> datetime:
    """DeRouter_SpendLogs.startTime is TIMESTAMP(3); floor to the second so a
...
async def _seed_base_for_missing_row(
    ...
    This is the DeRouter_SpendLogs aggregate the window counter reseed runs on
...
async def commit_window_spend_updates(
    ...
    """Apply aggregated window increments to DeRouter_BudgetWindowSpend.
