...
from derouter.derouter_core_utils.sensitive_data_masker import mask_sensitive_keys
from derouter.proxy._experimental.mcp_server.tool_search import MCP_TOOL_SEARCH_SETTINGS_KEY
from derouter.proxy._types import *
from derouter.proxy.auth.user_api_key_auth import user_api_key_auth
from derouter.proxy.config_resolvers.sso import (
    SSO_FIELD_ENV_VARS,
    SSO_SECRET_FIELDS,
    resolve_sso_config,
)
from derouter.proxy.spend_tracking.ptu_feature_flag import is_ptu_cost_attribution_enabled
from derouter.proxy.utils import invalidate_config_param
...
def _config_param_db(repo: ConfigRepository) -> TableActions[_ConfigParamRow]:
    return cast(  # cast-ok: prisma's DeRouter_Config actions object, whose Json column parses to a mapping
...
_UI_THEME_FIELD_ENV_VARS: Final[dict[str, str]] = {
    ...
    "favicon_url": "DEROUTER_FAVICON_URL",
}
...
# Extension point: packages outside OSS (e.g. derouter_enterprise) can
...
# client gets a clear signal that the feature requires DeRouter Enterprise.
...
async def get_internal_user_settings():
    """
    Get all SSO settings from the derouter_settings configuration.
...
async def get_default_team_settings():
    """
    Get the default team parameters (derouter_settings.default_team_params).
    Returns a structured object with values and descriptions for UI display.
    """
    from derouter.proxy.proxy_server import proxy_config
...
async def _update_derouter_setting(
    ...
    """
    Common utility function to update `derouter_settings` in both memory and config.

    Args:
        settings: The settings object to update
        settings_key: The key in derouter_settings to update
    ...
    # because get_config() may overwrite derouter.<key> with stale DB values
    # via DEROUTER_SETTINGS_SAFE_DB_OVERRIDES.
    config: Final = await proxy_config.get_config()
    before_value: Final = config.get("derouter_settings", {}).get(settings_key)

    # Update the in-memory settings (after get_config to avoid stale override)
    setattr(derouter, settings_key, in_memory_var)

    # Update config with new settings
    if "derouter_settings" not in config:
        config["derouter_settings"] = {}

    config["derouter_settings"][settings_key] = in_memory_var
...
async def update_internal_user_settings(
    ...
    return await _update_derouter_setting(
        settings=settings,
        settings_key="default_internal_user_params",
...
async def update_default_team_settings(
    ...
    """
    Update the default team parameters (derouter_settings.default_team_params).
...
async def get_ui_theme_settings():
    """
    Get UI theme configuration from the derouter_settings.
...
async def update_ui_theme_settings(
    ...
    # Load existing config
    config: Final = await proxy_config.get_config()
    before_theme: Final = config.get("derouter_settings", {}).get("ui_theme_config")

    # Convert theme config to dict
    theme_data: Final = theme_config.model_dump(exclude_none=True)

    # Store UI theme config in derouter_settings (where it's retrieved from)
    if "derouter_settings" not in config:
        config["derouter_settings"] = {}
    config["derouter_settings"]["ui_theme_config"] = theme_data
    ...
    env_updates: Final[dict[str, str | None]] = {
        ...
        "DEROUTER_FAVICON_URL": _clean(theme_config.favicon_url),
    }
    ...
    # Persist the theme config (derouter_settings). save_config defaults to
...
async def get_mcp_semantic_filter_settings(
    ...
    """
    from derouter.proxy.proxy_server import prisma_client, proxy_config
...
async def update_mcp_semantic_filter_settings(
    ...
    result: Final = await _update_derouter_setting(
        settings=settings,
        settings_key="mcp_semantic_tool_filter",
...
async def get_mcp_tool_search_settings(
    ...
    """
    Get the `derouter_settings.mcp_tool_search` configuration used by the native `mcp_tool_search` virtual tool.
    """
    from derouter.proxy.proxy_server import prisma_client, proxy_config
...
async def update_mcp_tool_search_settings(
    ...
    """
    Update `derouter_settings.mcp_tool_search` in the database.
    Settings will be picked up by all pods within approximately 10 seconds via background polling.
    """
    if user_api_key_dict.user_role != LitellmUserRoles.PROXY_ADMIN:
        raise HTTPException(
...
async def get_ui_settings():
    ...
    config: Final[dict[str, object]] = {"derouter_settings": {"ui_settings": ui_settings}}
...
async def update_ui_settings(
    ...
    if blocked_enterprise_keys:
        raise HTTPException(
            ...
            detail={
                "error": (
                    f"Setting(s) {blocked_enterprise_keys} are a DeRouter "
