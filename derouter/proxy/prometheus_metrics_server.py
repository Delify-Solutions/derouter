...
``python -m derouter.proxy.prometheus_metrics_server --host 0.0.0.0 --port 4001``.
"""
...
PID_HEADER: Final = "x-derouter-metrics-pid"
...
def build_metrics_app(multiproc_dir: str) -> FastAPI:
    ...
    app: Final = FastAPI(title="DeRouter Prometheus metrics", docs_url=None, redoc_url=None, openapi_url=None)
...
def _exit_when_parent_dies(parent_pid: int) -> None:
    ...
    threading.Thread(target=watch, name="derouter-metrics-parent-watchdog", daemon=True).start()
...
def start_metrics_server_process(host: str, port: int, multiproc_dir: str) -> subprocess.Popen[bytes]:
    ...
    process: Final = subprocess.Popen(
        (
            sys.executable,
            "-m",
            "derouter.proxy.prometheus_metrics_server",
            "--host",
            host,
            "--port",
            str(port),
...
def main(argv: Sequence[str] | None = None) -> None:
    parser: Final = argparse.ArgumentParser(
        description="Serve DeRouter Prometheus metrics from PROMETHEUS_MULTIPROC_DIR"
    )
