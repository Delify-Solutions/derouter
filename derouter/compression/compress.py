...
from derouter.compression.message_stubbing import (
    extract_key,
    stub_message,
    truncate_message,
)
...
from derouter.types.compression import CompressedResult
...
def compress(
    ...
    Parameters:
        ...
        call_type: The DeRouter call type whose message schema these messages
        ...
        embedding_model_params: Optional kwargs forwarded to
            ``derouter.embedding()`` when ``embedding_model`` is set.
        compression_cache: Passed through to ``derouter.embedding()`` for
    ...
    if embedding_model:
        from derouter.compression.scoring.embedding_scorer import (
