...
from derouter.caching.caching import S3Cache
from derouter.constants import CACHE_WRITE_SHUTDOWN_FLUSH_TIMEOUT_SECONDS
...
from derouter.types.caching import CachedEmbedding
...
if TYPE_CHECKING:
    ...
    from derouter.llms.anthropic.experimental_pass_through.messages.response_cache import (
...
def _drop_logging_obj_from_kwargs(request_kwargs: dict[str, object]) -> dict[str, object]:
    ...
    (``logging_obj._llm_caching_handler``), so keeping ``derouter_logging_obj``
    ...
    generation ignores derouter-internal params.
    """
    if "derouter_logging_obj" not in request_kwargs:
        return request_kwargs
    return {k: v for k, v in request_kwargs.items() if k != "derouter_logging_obj"}
    ...
    try:
        ...
        try:
            ...
            verbose_logger.warning(
                "DeRouter Cache: pending cache write failed during event loop shutdown: %s", flush_error
            )
        raise
...
class LLMCachingHandler:
    def __init__(
        ...
        from derouter.caching import DualCache, RedisCache
        ...
        if derouter.cache is not None and isinstance(derouter.cache.cache, RedisCache):
            self.dual_cache: DualCache | None = DualCache(
                redis_cache=derouter.cache.cache,
    ...
    async def _async_get_cache(
        ...
        if (
            (kwargs.get("caching", None) is None and derouter.cache is not None) or kwargs.get("caching", False) is True
        ) and (
            ...
            if derouter.cache is not None and self._is_call_type_supported_by_cache(original_function=original_function):
                ...
                if cached_result is not None and not isinstance(cached_result, list):
                    ...
                    self._update_derouter_logging_obj_environment(
                    ...
                    cache_key: Final = (
                        ...
                        or derouter.cache.get_cache_key(**self.request_kwargs)
                    )
                    ...
                    and derouter.cache is not None
                    and not isinstance(derouter.cache.cache, S3Cache)  # s3 doesn't support bulk writing. Exclude.
                ):
                    (
    ...
    def _sync_get_cache(
        ...
        if derouter.cache is not None and self._is_call_type_supported_by_cache(original_function=original_function):
            ...
            if new_kwargs.get("stream") is True and "cache_key" not in new_kwargs:
                new_kwargs["cache_key"] = derouter.cache.get_cache_key(**new_kwargs)
            self.request_kwargs = _drop_logging_obj_from_kwargs(new_kwargs)
            print_verbose("Checking Sync Cache")
            cached_result = derouter.cache.get_cache(**new_kwargs)
            ...
            for val in non_null_list:
                ...
                if cr is not None:
                    ...
                    if cached_prompt_tokens is not None:
                        ...
                        from derouter.utils import token_counter
    ...
    def _async_log_cache_hit_on_callbacks(
        ...
        Args:
            logging_obj (DeRouterLoggingObj): The logging object.
    ...
    async def _retrieve_from_cache(
        ...
        if call_type == CallTypes.aembedding.value:
            ...
            for idx, i in enumerate(new_kwargs["input"]):
                preset_cache_key = derouter.cache.get_cache_key(**{**new_kwargs, "input": i})
                tasks.append(
                    derouter.cache.async_get_cache(
            ...
            if derouter.cache._supports_async() is True:
                ## check if dual cache is supported ##
                self.preset_cache_key = request_cache_key or derouter.cache.get_cache_key(**request_kwargs)
                cached_result = await derouter.cache.async_get_cache(
                    dynamic_cache_object=self.dual_cache,
                ...
                self.preset_cache_key = request_cache_key or derouter.cache.get_cache_key(**request_kwargs)
                cached_result = derouter.cache.get_cache(
                    dynamic_cache_object=self.dual_cache,
                    cache_key=self.preset_cache_key,
                    **request_kwargs,
                )
    ...
    def _convert_cached_result_to_model_response(
        ...
        Returns:
            Optional[Any]:
        """
        from derouter.utils import convert_to_model_response_object
    ...
    async def async_set_cache(
        ...
        if self._should_store_result_in_cache(original_function=original_function, kwargs=new_kwargs):
            if (
                isinstance(result, derouter.ModelResponse)
                or isinstance(result, derouter.EmbeddingResponse)
    ...
    def sync_set_cache(
        ...
        if self._should_store_result_in_cache(original_function=self.original_function, kwargs=new_kwargs):
            derouter.cache.add_cache(result, **new_kwargs)
    ...
    def _should_store_result_in_cache(self, original_function: Callable, kwargs: dict[str, Any]) -> bool:
        ...
        """
        return (
            (derouter.cache is not None)
            and derouter.cache.supported_call_types is not None
            and (str(original_function.__name__) in derouter.cache.supported_call_types)
    ...
    def wrap_streaming_result_for_cache(
        ...
        if call_type not in (
            ...
            return result
        if derouter.cache is None or not self._should_store_result_in_cache(
    ...
    def _is_call_type_supported_by_cache(
        ...
        Defined on `derouter.types.utils.CallTypes`
    ...
    async def _add_streaming_response_to_cache(self, processed_chunk: ModelResponse):
        ...
        - If 'streaming_chunk' has a 'finish_reason' then assemble a derouter.ModelResponse object
    ...
    def _update_derouter_logging_obj_environment(
        self,
        logging_obj: DeRouterLoggingObj,
        model: str,
        kwargs: dict[str, Any],
        ...
        """
        Helper function to update the DeRouterLoggingObj environment variables.

        Args:
            logging_obj (DeRouterLoggingObj): The logging object to update.
        ...
        if derouter.cache is not None:
            derouter_params["preset_cache_key"] = derouter.cache._get_preset_cache_key_from_kwargs(**kwargs)
        else:
            derouter_params["preset_cache_key"] = None
