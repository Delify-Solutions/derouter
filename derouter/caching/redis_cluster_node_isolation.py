...
``get_derouter_async_redis_cluster_class`` returns a ``RedisCluster`` subclass that resets
...
def get_derouter_async_redis_cluster_class(  # noqa: C901  # supports redis-py version-specific cluster implementations
    ...
    if hasattr(node_class, "update_active_connections_for_reconnect"):
        verbose_logger.debug(
            ...
            "DeRouter's timeout-tolerant RedisCluster wrapper.",
            redis.__version__,
        )

        class DeRouterAsyncRedisClusterTimeoutTolerant(
            ...
            def __init__(
                ...
                **kwargs: object,  # kwargs-ok: passes redis-py's constructor kwargs through untouched
                ...
                self._derouter_initialize = False
                self._derouter_reinit_requests = 0
                self._derouter_tolerated_timeouts = 0
                super().__init__(*args, **kwargs)
                self._derouter_consecutive_timeouts: dict[  # mutable-ok: per-node counter updated on the command hot path
                    str, int
                ] = {}

            @property
            def _initialize(self) -> bool:
                return self._derouter_initialize

            @_initialize.setter
            def _initialize(self, value: bool) -> None:
                if value:
                    self._derouter_reinit_requests += 1
                self._derouter_initialize = value
            ...
            async def _execute_command(
                ...
                **kwargs: object,  # kwargs-ok: matches redis-py's own command dispatch signature
                ...
                outstanding_before: Final = self._derouter_reinit_requests - self._derouter_tolerated_timeouts
                pending_before: Final = self._derouter_initialize
                try:
                    result: Final = await super()._execute_command(target_node, *args, **kwargs)
                except _RedisTimeoutError:
                    timeouts: Final = self._derouter_consecutive_timeouts.get(target_node.name, 0) + 1
                    if timeouts >= _CONSECUTIVE_TIMEOUTS_BEFORE_REINIT:
                        self._derouter_consecutive_timeouts.pop(target_node.name, None)
                        raise
                    self._derouter_consecutive_timeouts[target_node.name] = timeouts
                    self._derouter_tolerated_timeouts += 1
                    if (
                        not pending_before
                        and self._derouter_reinit_requests - self._derouter_tolerated_timeouts == outstanding_before
                    ):
                        self._initialize = False
                    raise
                if self._derouter_consecutive_timeouts:
                    self._derouter_consecutive_timeouts.pop(target_node.name, None)
                return result

        return DeRouterAsyncRedisClusterTimeoutTolerant
    ...
    class DeRouterAsyncRedisCluster(
    ...
    return DeRouterAsyncRedisCluster
