...
for ``derouter.request_timeout`` seconds, 6000 by default, and a streaming
...
Only clients derouter itself created are closed; a client the caller supplied is
left alone because derouter does not own its lifecycle.
...
def _transport_of(client: object) -> object:
    """The httpx transport behind an SDK wrapper, a derouter handler, or a bare client."""
...
def _has_connection_in_flight(client: object) -> bool:
    ...
    Both connection backends derouter uses already account for the connections
...
class EvictedClientCloser:
    """Closes evicted, derouter-owned clients once they are idle and out of grace."""
    ...
    def mark_owned(self, client: object) -> None:
        """Record that derouter created this client, so it may be closed on eviction."""
        try:
            self._owned.add(client)
        except TypeError:
            pass  # values that cannot be weak-referenced are never derouter clients

    def _is_owned(self, client: object) -> bool:
        try:
            return client in self._owned
        except TypeError:
            return False  # unhashable values are never derouter clients
