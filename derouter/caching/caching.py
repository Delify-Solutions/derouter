...
from derouter.constants import CACHED_STREAMING_CHUNK_DELAY
from derouter.derouter_core_utils.model_param_helper import ModelParamHelper
from derouter.types.caching import *
from derouter.types.utils import EmbeddingResponse, all_derouter_params
...
#### DeRouter.Completion / Embedding Cache ####
class Cache:
    def __init__(
        self,
        type: DeRouterCacheType | None = DeRouterCacheType.LOCAL,
        mode: CacheMode
        ...
        **kwargs,
        ...
        Returns:
            None. Cache is set as a derouter param
        """
        if type == DeRouterCacheType.REDIS:
            # Check REDIS_CLUSTER_NODES env var if no explicit startup nodes
            if not redis_startup_nodes:
                _env_cluster_nodes: Final = derouter.get_secret("REDIS_CLUSTER_NODES")
        ...
        elif type == DeRouterCacheType.REDIS_SEMANTIC:
        ...
        elif type == DeRouterCacheType.VALKEY_SEMANTIC:
        ...
        elif type == DeRouterCacheType.QDRANT_SEMANTIC:
        ...
        elif type == DeRouterCacheType.LOCAL:
            self.cache = InMemoryCache()
        elif type == DeRouterCacheType.S3:
            self.cache = S3Cache(
        ...
        elif type == DeRouterCacheType.GCS:
            self.cache = GCSCache(
                bucket_name=gcs_bucket_name,
                path_service_account=gcs_path_service_account,
                gcs_path=gcs_path,
            )
        elif type == DeRouterCacheType.AZURE_BLOB:
            self.cache = AzureBlobCache(
                account_url=azure_account_url,
                container=azure_blob_container,
            )
        elif type == DeRouterCacheType.DISK:
            self.cache = DiskCache(disk_cache_dir=disk_cache_dir)
        if "cache" not in derouter.input_callback:
            derouter.input_callback.append("cache")
        if "cache" not in derouter.success_callback:
            derouter.logging_callback_manager.add_derouter_success_callback("cache")
        if "cache" not in derouter._async_success_callback:
            derouter.logging_callback_manager.add_derouter_async_success_callback("cache")
        ...
        if self.type == DeRouterCacheType.LOCAL and default_in_memory_ttl is not None:
            self.ttl = default_in_memory_ttl

        if (
            self.type == DeRouterCacheType.REDIS
            or self.type == DeRouterCacheType.REDIS_SEMANTIC
            or self.type == DeRouterCacheType.VALKEY_SEMANTIC
    ...
    def _is_semantic_cache(self) -> bool:
        return self.type in (
            DeRouterCacheType.REDIS_SEMANTIC,
            DeRouterCacheType.QDRANT_SEMANTIC,
            DeRouterCacheType.VALKEY_SEMANTIC,
        )
    ...
    def _get_semantic_cache_tenant_scope(self, kwargs: dict) -> str:
        derouter_params: Final[dict] = kwargs.get("derouter_params") or {}
        metadata_sources: Final[tuple[dict, ...]] = tuple(
            source.get(key) or {} for source in (kwargs, derouter_params) for key in ("metadata", "derouter_metadata")
        )
        scope_values: Final = (
    ...
    def get_cache_key(self, **kwargs) -> str:
        ...
        Args:
            **kwargs: kwargs to derouter.completion() or embedding()
        ...
        derouter_param_kwargs: Final = all_derouter_params
        ...
        for param in kwargs:
            ...
            elif param not in derouter_param_kwargs:  # check if user passed in optional param - e.g. top_k
                if derouter.enable_caching_on_provider_specific_optional_params is True:  # feature flagged for now
    ...
    def _get_model_param_value(self, kwargs: dict) -> str:
        ...
        1. If caching groups are set, then return the caching group as the model https://router.delify.vn/docs/routing#caching-across-model-groups
        2. Else if a model_group is set, then return the model_group as the model. This is used for all requests sent through the derouter.Router()
        3. Else use the `model` passed in kwargs
        """
        metadata: Final[dict] = kwargs.get("metadata", {}) or {}
        derouter_params: Final[dict] = kwargs.get("derouter_params", {}) or {}
        metadata_in_derouter_params: Final[dict] = derouter_params.get("metadata", {}) or {}
        model_group: Final[str | None] = metadata.get("model_group") or metadata_in_derouter_params.get("model_group")
    ...
    def _get_file_param_value(self, kwargs: dict) -> str:
        ...
        return (
            ...
            or derouter_params.get("file_name")
        )

    def _get_preset_cache_key_from_kwargs(self, **kwargs) -> str | None:
        """
        Get the preset cache key from kwargs["derouter_params"]
        ...
        """
        if kwargs:
            if "derouter_params" in kwargs:
                return kwargs["derouter_params"].get("preset_cache_key", None)
        return None
    ...
    def _set_preset_cache_key_in_kwargs(self, preset_cache_key: str, **kwargs) -> None:
        ...
        Placed in kwargs["derouter_params"]
        """
        if kwargs:
            if "derouter_params" in kwargs:
                kwargs["derouter_params"]["preset_cache_key"] = preset_cache_key
    ...
    def get_cache(self, dynamic_cache_object: BaseCache | None = None, **kwargs):
        ...
        Args:
            *args: args to derouter.completion() or embedding()
            **kwargs: kwargs to derouter.completion() or embedding()
    ...
    def add_cache(self, result, **kwargs):
        ...
        try:
            ...
            verbose_logger.exception("DeRouter Cache: Excepton add_cache: %s", e)
...
def enable_cache(
    ...
    **kwargs,
    ...
    Raises:
        None
    """
    print_verbose("DeRouter: Enabling Cache")
    if "cache" not in derouter.input_callback:
        derouter.input_callback.append("cache")
    if "cache" not in derouter.success_callback:
        derouter.logging_callback_manager.add_derouter_success_callback("cache")
    ...
    print_verbose(f"DeRouter: Cache enabled, derouter.cache={derouter.cache}")
    print_verbose(f"DeRouter Cache: {vars(derouter.cache)}")
...
def update_cache(
    ...
    **kwargs,
    ...
    """
    print_verbose("DeRouter: Updating Cache")
    derouter.cache = Cache(
        type=type,
        host=host,
        port=port,
    ...
    print_verbose(f"DeRouter: Cache Updated, derouter.cache={derouter.cache}")
    print_verbose(f"DeRouter Cache: {vars(derouter.cache)}")


def disable_cache():
    """
    Disable the cache used by DeRouter.

    This function disables the cache used by the DeRouter module. It removes the cache-related callbacks from the input_callback, success_callback, and _async_success_callback lists. It also sets the derouter.cache attribute to None.
    ...
    print_verbose("DeRouter: Disabling Cache")
    with suppress(ValueError):
        derouter.input_callback.remove("cache")
        derouter.success_callback.remove("cache")
        derouter._async_success_callback.remove("cache")

    derouter.cache = None
    print_verbose(f"DeRouter: Cache disabled, derouter.cache={derouter.cache}")
