"""
Redis Semantic Cache implementation for DeRouter
...
class RedisSemanticCache(BaseCache):
    ...
    DEFAULT_REDIS_INDEX_NAME: str = "derouter_semantic_cache_index"
    CACHE_KEY_FIELD_NAME: str = "derouter_cache_key"
    ...
    def _cache_hit_matches_key(self, cache_hit: Mapping[str, object], key: str) -> bool:
        # Pre-isolation entries with no ``derouter_cache_key`` field cannot be
    ...
    def _get_embedding(self, prompt: str, metadata: dict[str, object] | None = None) -> list[float]:
        ...
        """
        try:
            from derouter.proxy.proxy_server import llm_model_list, llm_router
        except ImportError:
            llm_model_list = None
            llm_router = None
