...
class LLMClientCache(InMemoryCache):
    ...
    until a generational collection runs. Instead a client derouter created is
    ...
    def set_cache(self, key: str, value: object, derouter_owned_client: bool = False, **kwargs):
        """``derouter_owned_client`` marks a client derouter built, so it may be closed once evicted."""
        if derouter_owned_client:
            self.evicted_client_closer.mark_owned(value)
        key = self.update_cache_key_with_event_loop(key)
        return super().set_cache(key, value, **kwargs)

    async def async_set_cache(self, key: str, value: object, derouter_owned_client: bool = False, **kwargs):
        if derouter_owned_client:
            self.evicted_client_closer.mark_owned(value)
        key = self.update_cache_key_with_event_loop(key)
