...
from derouter.derouter_core_utils.core_helpers import _get_parent_otel_span_from_kwargs
...
_swallowed_redis_failures: Final[ContextVar[int]] = ContextVar("derouter_swallowed_redis_failures", default=0)
...
def _redis_health_error_types() -> tuple[type, ...]:
    ...
    Imported lazily because this module is reachable from a base ``import derouter`` while
...
class RedisCache(BaseCache):
    # if users don't provider one, use the default derouter cache
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if derouter.default_redis_ttl is not None:
            super().__init__(default_ttl=int(derouter.default_redis_ttl))
        else:
    ...
    def init_async_client(
        ...
        from derouter import in_memory_llm_clients_cache
    ...
    def set_cache(self, key, value, **kwargs):
        ...
        try:
            ...
            print_verbose(f"derouter.caching.caching: set() - Got exception from REDIS : {e}")
    ...
    def increment_cache(self, key, value: int, ttl: float | None = None, **kwargs) -> int:
        ...
        try:
            ...
            _duration = end_time - start_time
            verbose_logger.error(
                "DeRouter Redis Caching: increment_cache() - Got exception from REDIS %s, Writing value=%s",
                str(e),
                value,
            )
            raise e
    ...
    def async_register_script(self, script: str) -> Callable[..., Awaitable[Any]]:
        ...
        async def run_script(
            ...
            async def execute() -> object:
                executor: Callable[..., Awaitable[Any]] | None = derouter.in_memory_llm_clients_cache.get_cache(
                    key=script_cache_key
                )
                if executor is None:
                    executor = self._register_script_for_current_loop(script)
                    derouter.in_memory_llm_clients_cache.set_cache(key=script_cache_key, value=executor)
    ...
    async def async_set_cache(self, key, value, **kwargs):
        ...
        if key is None:
            verbose_logger.debug(
                "DeRouter Redis Caching: async set() skipped — key is None, value=%r",
                value,
            )
            return None
        ...
        try:
            ...
            verbose_logger.error(
                "DeRouter Redis Caching: async set() - Got exception from REDIS %s, key=%r, value=%r",
                str(e),
                key,
                value,
            )
        ...
        try:
            ...
            verbose_logger.error(
                "DeRouter Redis Caching: async set() - Got exception from REDIS %s, Writing value=%s",
                str(e),
                value,
            )
    ...
    async def async_set_cache_pipeline(
        ...
        try:
            ...
            verbose_logger.error(
                "DeRouter Redis Caching: async set_cache_pipeline() - Got exception from REDIS %s, Writing value=%s",
                str(e),
                cache_value,
            )
            _record_swallowed_redis_failure(self._circuit_breaker, e)
    ...
    async def async_set_cache_sadd(self, key, value: list, ttl: float | None, **kwargs):
        ...
        try:
            ...
            # NON blocking - notify users Redis is throwing an exception
            verbose_logger.error(
                "DeRouter Redis Caching: async set_cache_sadd() - Got exception from REDIS %s, Writing value=%s",
                str(e),
                value,
            )
            _record_swallowed_redis_failure(self._circuit_breaker, e)
    ...
    async def async_increment(
        ...
        try:
            ...
            verbose_logger.error(
                "DeRouter Redis Caching: async async_increment() - Got exception from REDIS %s, Writing value=%s",
                str(e),
                value,
            )
            raise e
    ...
    def get_cache(self, key, parent_otel_span: Span | None = None, **kwargs):
        try:
            ...
            # NON blocking - notify users Redis is throwing an exception
            verbose_logger.error("derouter.caching.caching: get() - Got exception from REDIS: ", e)
    ...
    async def async_get_cache(self, key, parent_otel_span: Span | None = None, **kwargs):
        ...
        try:
            ...
            print_verbose(f"derouter.caching.caching: async get() - Got exception from REDIS: {e}")
            _record_swallowed_redis_failure(self._circuit_breaker, e)
    ...
    def sync_ping(self) -> bool:
        ...
        try:
            ...
            verbose_logger.error("DeRouter Redis Cache PING: - Got exception from REDIS : %s", e)
            raise e
    ...
    async def async_increment_pipeline(
        ...
        try:
            ...
            verbose_logger.error(
                "DeRouter Redis Caching: async increment_pipeline() - Got exception from REDIS %s",
                str(e),
            )
            raise e
    ...
    async def async_rpush(
        ...
        **kwargs,
        ...
        try:
            ...
            verbose_logger.error("DeRouter Redis Cache RPUSH: - Got exception from REDIS : %s", e)
            raise e
    ...
    async def async_rpush_pipeline(
        ...
        try:
            ...
            verbose_logger.error(
                "DeRouter Redis Caching: async_rpush_pipeline() - Got exception from REDIS %s",
                str(e),
            )
            raise e
    ...
    async def async_lpop(
        ...
        **kwargs,
        ...
        try:
            ...
            verbose_logger.error("DeRouter Redis Cache LPOP: - Got exception from REDIS : %s", e)
            raise e
    ...
    async def async_lpop_pipeline(
        ...
        try:
            ...
            verbose_logger.error(
                "DeRouter Redis Caching: async_lpop_pipeline() - Got exception from REDIS %s",
                str(e),
            )
            raise e
