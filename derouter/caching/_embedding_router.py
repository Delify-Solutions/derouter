...
applied. Otherwise fall back to a direct derouter embedding call.
...
def truncate_embedding_input(prompt: str, embedding_model: str, max_input_tokens: int | None) -> str:
    ...
    if max_input_tokens is None:
        return prompt
    tokens: Final[Sequence[int]] = derouter.encode(model=embedding_model, text=prompt)
    if len(tokens) <= max_input_tokens:
        return prompt
    truncated: Final[str] = derouter.decode(model=embedding_model, tokens=tokens[:max_input_tokens])
    return truncated
