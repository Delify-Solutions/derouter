...
## DeRouter versions of the OpenAI Exception Types
...
from derouter.types.utils import DeRouterCommonStrings
...
class RateLimitErrorCategory(str, enum.Enum):
    ...
    Use these values to switch on the rate limit source, e.g.::
        ...
        try:
            ...
            if e.category == RateLimitErrorCategory.DEROUTER_RATE_LIMIT:
                ...  # derouter's own limiter (key/team/user/model RPM/TPM/budget)
    ...
    DEROUTER_RATE_LIMIT = "derouter_rate_limit"
    """DeRouter's own rate limiter (key/team/user/model RPM/TPM, budget, parallel-requests, etc.) blocked the request."""

    DEROUTER_BATCH_RATE_LIMIT = "derouter_batch_rate_limit"
    """DeRouter's own batch rate limiter (token/request budget across a batch input file) blocked the request."""
...
class RateLimitType(str, enum.Enum):
    ...
    derouter's own limiters), while *type* tells them **which limit dimension**
...
def _get_minimal_error_response() -> httpx.Response:
    ...
    if _MINIMAL_ERROR_RESPONSE is None:
        _MINIMAL_ERROR_RESPONSE = httpx.Response(
            status_code=400,
            request=httpx.Request(method="GET", url="https://router.delify.vn"),
        )
...
class AuthenticationError(openai.AuthenticationError):
    def __init__(
        ...
        derouter_debug_info: str | None = None,
        max_retries: int | None = None,
        num_retries: int | None = None,
    ):
        self.status_code = 401
        self.message = f"derouter.AuthenticationError: {message}"
        self.llm_provider = llm_provider
        self.model = model
        self.derouter_debug_info = derouter_debug_info
        self.max_retries = max_retries
        self.num_retries = num_retries
        self.response = response or httpx.Response(
            status_code=self.status_code,
            request=httpx.Request(method="GET", url="https://router.delify.vn"),  # mock request object
        )
        super().__init__(
    ...
    def __str__(self):
        ...
        if self.num_retries:
            _message += f" DeRouter Retried: {self.num_retries} times"
        if self.max_retries:
            _message += f", DeRouter Max Retries: {self.max_retries}"
        return _message
...
class NotFoundError(openai.NotFoundError):
    def __init__(
        ...
        self.message = f"derouter.NotFoundError: {message}"
        self.model = model
        self.llm_provider = llm_provider
        self.derouter_debug_info = derouter_debug_info
        self.max_retries = max_retries
...
class BadRequestError(openai.BadRequestError):
    def __init__(
        ...
        self.message = f"derouter.BadRequestError: {message}"
        self.model = model
        self.llm_provider = llm_provider
        self.derouter_debug_info = derouter_debug_info
        self.max_retries = max_retries
...
class ImageFetchError(BadRequestError):
    def __init__(
        ...
        super().__init__(
            ...
            response=response,
            derouter_debug_info=derouter_debug_info,
...
class UnprocessableEntityError(openai.UnprocessableEntityError):
    def __init__(
        ...
        self.message = f"derouter.UnprocessableEntityError: {message}"
        self.model = model
        self.llm_provider = llm_provider
        self.derouter_debug_info = derouter_debug_info
        self.max_retries = max_retries
...
class Timeout(openai.APITimeoutError):
    def __init__(
        ...
        self.message = f"derouter.Timeout: {message}"
        self.model = model
        self.llm_provider = llm_provider
        self.derouter_debug_info = derouter_debug_info
        self.max_retries = max_retries
...
class PermissionDeniedError(openai.PermissionDeniedError):
    def __init__(
        ...
        self.message = f"derouter.PermissionDeniedError: {message}"
        self.llm_provider = llm_provider
        self.model = model
        self.derouter_debug_info = derouter_debug_info
        self.max_retries = max_retries
...
class RateLimitError(openai.RateLimitError):
    ...
    Every rate-limit condition surfaced by derouter — whether it originated from
    an upstream LLM provider, a vendor batch endpoint, or one of derouter's own
    ...
    def __init__(
        ...
        self.message = f"derouter.RateLimitError: {message}"
        self.llm_provider = llm_provider
        self.model = model
        self.derouter_debug_info = derouter_debug_info
        self.max_retries = max_retries
...
class ContextWindowExceededError(BadRequestError):
    def __init__(
        ...
        super().__init__(
            ...
            response=response,
            derouter_debug_info=self.derouter_debug_info,
        )  # Call the base class constructor with the parameters it needs

        # set after, to make it clear the raised error is a context window exceeded error
        self.message = f"derouter.ContextWindowExceededError: {self.message}"
...
class RejectedRequestError(BadRequestError):
    def __init__(
        ...
        self.status_code = 400
        self.message = f"derouter.RejectedRequestError: {message}"
        self.model = model
        self.llm_provider = llm_provider
        self.derouter_debug_info = derouter_debug_info
...
class ContentPolicyViolationError(BadRequestError):
    ...
    def __init__(
        ...
        self.status_code = 400
        self.message = f"derouter.ContentPolicyViolationError: {message}"
        self.model = model
        self.llm_provider = llm_provider
        self.derouter_debug_info = derouter_debug_info
...
class ServiceUnavailableError(openai.APIStatusError):
    def __init__(
        ...
        self.message = f"derouter.ServiceUnavailableError: {message}"
        self.llm_provider = llm_provider
        self.model = model
        self.derouter_debug_info = derouter_debug_info
        self.max_retries = max_retries
...
class BadGatewayError(openai.APIStatusError):
    def __init__(
        ...
        self.message = f"derouter.BadGatewayError: {message}"
        self.llm_provider = llm_provider
        self.model = model
        self.derouter_debug_info = derouter_debug_info
        self.max_retries = max_retries
...
class InternalServerError(openai.InternalServerError):
    def __init__(
        ...
        self.message = f"derouter.InternalServerError: {message}"
        self.llm_provider = llm_provider
        self.model = model
        self.derouter_debug_info = derouter_debug_info
        self.max_retries = max_retries
...
class APIError(openai.APIError):
    def __init__(
        ...
        self.message = f"derouter.APIError: {message}"
        self.llm_provider = llm_provider
        self.model = model
        self.derouter_debug_info = derouter_debug_info
        self.max_retries = max_retries
...
class APIConnectionError(openai.APIConnectionError):
    def __init__(
        ...
        self.message = f"derouter.APIConnectionError: {message}"
        self.llm_provider = llm_provider
        self.model = model
        self.status_code = 500
        self.derouter_debug_info = derouter_debug_info
...
class APIResponseValidationError(openai.APIResponseValidationError):
    def __init__(
        ...
        self.message = f"derouter.APIResponseValidationError: {message}"
        self.llm_provider = llm_provider
        self.model = model
        request: Final = httpx.Request(method="POST", url="https://api.openai.com/v1")
...
class JSONSchemaValidationError(APIResponseValidationError):
    def __init__(self, model: str, llm_provider: str, raw_response: str, schema: str) -> None:
        ...
        self.schema = schema
        self.model = model
        message = f"derouter.JSONSchemaValidationError: model={model}, returned an invalid response={raw_response}, for schema={schema}.\nAccess raw response with `e.raw_response`"
...
class UnsupportedParamsError(BadRequestError):
    def __init__(
        ...
        self.status_code = 400
        self.message = f"derouter.UnsupportedParamsError: {message}"
        self.model = model
        self.llm_provider = llm_provider
        self.derouter_debug_info = derouter_debug_info
...
DEROUTER_EXCEPTION_TYPES: Final = [
    AuthenticationError,
    NotFoundError,
    BadRequestError,
...
class BudgetExceededError(Exception):
    def __init__(
        ...
        self.category: str = RateLimitErrorCategory.DEROUTER_RATE_LIMIT.value
...
class MockException(openai.APIError):
    ...
    def __init__(
        ...
        self.status_code = status_code
        self.message = f"derouter.MockException: {message}"
        self.llm_provider = llm_provider
        self.model = model
        self.derouter_debug_info = derouter_debug_info
        self.max_retries = max_retries
...
class DeRouterUnknownProvider(BadRequestError):
    def __init__(self, model: str, custom_llm_provider: str | None = None):
        self.message = DeRouterCommonStrings.llm_provider_not_provided.value.format(
...
class MidStreamFallbackError(ServiceUnavailableError):
    def __init__(
        ...
        self.message = f"derouter.MidStreamFallbackError: {message}"
        self.model = model
        self.llm_provider = llm_provider
