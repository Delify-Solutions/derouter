...
Canonical definitions for ``derouter_teamtable`` (plus the shared Member and
...
class MemberBase(DeRouterPydanticObjectBase):
...
class BudgetLimitEntry(DeRouterPydanticObjectBase):
...
class DeRouter_ModelTable(DeRouterPydanticObjectBase):
    id: int | None = None
    model_aliases: str | dict | None = None
    created_by: str
    updated_by: str
    team: Optional["DeRouter_TeamTable"] = None

    model_config = ConfigDict(protected_namespaces=())


class TeamBase(DeRouterPydanticObjectBase):
...
class DeRouter_TeamTable(TeamBase):
    team_id: str
    ...
    derouter_model_table: DeRouter_ModelTable | None = None
...
class DeRouter_TeamTableCachedObj(DeRouter_TeamTable):
    last_refreshed_at: float | None = None


class DeRouter_DeletedTeamTable(DeRouter_TeamTable):
...
DeRouter_ModelTable.model_rebuild()
