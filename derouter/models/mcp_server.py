...
Canonical definition for ``derouter_mcpservertable``. Re-exported from
...
from derouter.types.mcp import MCPAuthType, MCPCredentials, MCPTransportType
...
class MCPEnvVar(DeRouterPydanticObjectBase):
    ...
    values are stored per-user in ``DeRouter_MCPUserEnvVars`` and supplied by
    each user.
    """
...
class DeRouter_MCPServerTable(DeRouterPydanticObjectBase):
    """Represents a DeRouter_MCPServerTable record"""
    ...
    # ``subject_token_type`` means DEFAULT_SUBJECT_TOKEN_TYPE (derouter.types.mcp),
    ...
    def decode_stored_secret_map(cls, value: object, info: ValidationInfo) -> Mapping[str, str] | None:
        from derouter.proxy.common_utils.encrypt_decrypt_utils import decode_secret_map
