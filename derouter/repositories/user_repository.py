"""
User repository for database operations on DeRouter_UserTable.
"""
...
from derouter.models.user import DeRouter_UserTable, SCIMPlaceholder
from derouter.repositories.base_repository import BaseRepository, DbRecord, record_to_dict
...
SELECT p.user_id AS placeholder_user_id,
       ...
       p.teams AS team_ids
FROM "DeRouter_UserTable" p
JOIN "DeRouter_UserTable" r
  ON r.user_id <> p.user_id
 AND (r.sso_user_id = p.user_id OR LOWER(r.user_email) = LOWER(p.user_id))
WHERE p.sso_user_id IS NULL
  AND NOT EXISTS (SELECT 1 FROM "DeRouter_VerificationToken" k WHERE k.user_id = p.user_id)
...
class UserRepository(BaseRepository[DeRouter_UserTable]):
    """Repository for user database operations."""

    @property
    def table(self) -> TableActions["prisma_models.DeRouter_UserTable"]:
        return self.prisma_client.db.derouter_usertable

    @property
    def model_class(self) -> type[DeRouter_UserTable]:
        return DeRouter_UserTable

    def _to_model(self, record: DbRecord | None) -> DeRouter_UserTable | None:
        """Convert a database record to a User model."""
        if record is None:
            return None

        return DeRouter_UserTable.model_validate(
            {
    ...
    async def find_by_id(self, id_value: str, id_field: str = "user_id") -> DeRouter_UserTable | None:
        return await super().find_by_id(id_value, id_field)

    async def find_by_email(self, user_email: str) -> DeRouter_UserTable | None:
        """Find a user by email."""
        records: Final = await self.find_many(where={"user_email": user_email})
        return records[0] if records else None

    async def find_by_sso_id(self, sso_user_id: str) -> DeRouter_UserTable | None:
        """Find a user by SSO ID."""
        return await self.find_by_id(sso_user_id, id_field="sso_user_id")

    async def find_by_organization_id(self, organization_id: str) -> list[DeRouter_UserTable]:
        """Find all users in an organization."""
        return await self.find_many(where={"organization_id": organization_id})

    async def find_by_team_id(self, team_id: str) -> list[DeRouter_UserTable]:
    ...
    async def delete_user(self, user_id: str) -> DeRouter_UserTable | None:
        """Delete a user."""
        return await self.delete(user_id, id_field="user_id")

    async def update_spend(self, user_id: str, spend: float) -> DeRouter_UserTable | None:
        """Update user spend."""
        return await self.update(user_id, {"spend": spend}, id_field="user_id")

    async def add_to_team(self, user_id: str, team_id: str) -> DeRouter_UserTable | None:
    ...
    async def remove_from_team(self, user_id: str, team_id: str) -> DeRouter_UserTable | None:
