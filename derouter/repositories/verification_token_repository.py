"""
VerificationToken repository for database operations on DeRouter_VerificationToken.
"""
...
if TYPE_CHECKING:
    from prisma.models import (
        DeRouter_DeletedVerificationToken as PrismaDeletedVerificationToken,
    )
    from prisma.models import (
        DeRouter_VerificationToken as PrismaVerificationToken,
    )
...
class VerificationTokenRepository(BaseRepository[DeRouter_VerificationToken]):
    ...
    def table(self) -> TableActions["PrismaVerificationToken"]:
        return self.prisma_client.db.derouter_verificationtoken

    @property
    def deleted_table(self) -> TableActions["PrismaDeletedVerificationToken"]:
        return self.prisma_client.db.derouter_deletedverificationtoken

    @property
    def model_class(self) -> type[DeRouter_VerificationToken]:
        return DeRouter_VerificationToken

    def _to_model(self, record: DbRecord | None) -> DeRouter_VerificationToken | None:
        ...
        return DeRouter_VerificationToken.model_validate(data)

    async def find_by_id(self, token: str, id_field: str = "token") -> DeRouter_VerificationToken | None:
        return await super().find_by_id(token, id_field)

    async def find_by_alias(self, key_alias: str) -> DeRouter_VerificationToken | None:
    ...
    async def find_by_user_id(self, user_id: str) -> list[DeRouter_VerificationToken]:
    ...
    async def find_by_team_id(self, team_id: str) -> list[DeRouter_VerificationToken]:
    ...
    async def find_by_project_id(self, project_id: str) -> list[DeRouter_VerificationToken]:
    ...
    async def find_active_tokens(self) -> list[DeRouter_VerificationToken]:
    ...
    async def delete_token(
        ...
        async with self.prisma_client.db.tx() as tx:
            await tx.derouter_deletedverificationtoken.create(data=archive_data)
            await tx.derouter_verificationtoken.delete(where={"token": token})

        return token_record

    def _build_archive_data(self, token: DeRouter_VerificationToken) -> dict[str, object]:
        """Build archive data with only columns present in DeRouter_DeletedVerificationToken.
        ...
        for field in ("object_permission", "derouter_budget_table", "budget_limits"):
            data.pop(field, None)
    ...
    async def update_spend(self, token: str, spend: float) -> DeRouter_VerificationToken | None:
        """Update token spend."""
        return await self.update(token, {"spend": spend}, id_field="token")

    async def update_last_active(self, token: str) -> DeRouter_VerificationToken | None:
        """Update the last_active timestamp."""
        return await self.update(token, {"last_active": datetime.utcnow()}, id_field="token")

    async def block_token(self, token: str, updated_by: str | None = None) -> DeRouter_VerificationToken | None:
        """Block a token."""
    ...
    async def unblock_token(self, token: str, updated_by: str | None = None) -> DeRouter_VerificationToken | None:
        """Unblock a token."""
