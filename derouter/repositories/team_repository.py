"""
Team repository for database operations on DeRouter_TeamTable.
"""
...
from derouter.models.team import DeRouter_TeamTable, Member
from derouter.repositories.base_repository import (
    BaseRepository,
    DbRecord,
    record_to_dict,
)
...
def _team_arrays(team: DeRouter_TeamTable) -> _TeamArrays:
...
class TeamRepository(BaseRepository[DeRouter_TeamTable]):
    """Repository for team database operations."""

    @property
    def table(self) -> TableActions["prisma_models.DeRouter_TeamTable"]:
        return self.prisma_client.db.derouter_teamtable

    @property
    def deleted_table(self) -> TableActions["prisma_models.DeRouter_DeletedTeamTable"]:
        return self.prisma_client.db.derouter_deletedteamtable

    @property
    def model_class(self) -> type[DeRouter_TeamTable]:
        return DeRouter_TeamTable

    def _to_model(self, record: DbRecord | None) -> DeRouter_TeamTable | None:
        ...
        return DeRouter_TeamTable.model_validate(data)
    ...
    async def get_members_with_roles_locked(self, tx: "Prisma", team_id: str) -> list[Member] | None:
        ...
        additionally take a row lock on ``DeRouter_TeamTable``, and the access-group
        ...
        rows: Final = await tx.query_raw(
            'SELECT members_with_roles FROM "DeRouter_TeamTable" WHERE team_id = $1',
            team_id,
        )
        if not rows:
            return None
    ...
    async def find_by_id(self, team_id: str, id_field: str = "team_id") -> DeRouter_TeamTable | None:
        return await super().find_by_id(team_id, id_field)

    async def find_by_alias(self, team_alias: str) -> DeRouter_TeamTable | None:
    ...
    async def find_by_organization_id(self, organization_id: str) -> list[DeRouter_TeamTable]:
    ...
    async def find_by_member(self, user_id: str) -> list[DeRouter_TeamTable]:
        """Find all teams where user is a member."""
        records: Final = await self.table.find_many(where={"members": {"has": user_id}})
        return self._to_model_list(records)

    async def find_by_admin(self, user_id: str) -> list[DeRouter_TeamTable]:
    ...
    async def delete_team(
        ...
        archive_data["derouter_changed_by"] = derouter_changed_by
        archive_data["deleted_at"] = datetime.utcnow()

        async with self.prisma_client.db.tx() as tx:
            await tx.derouter_deletedteamtable.create(data=archive_data)
            await tx.derouter_teamtable.delete(where={"team_id": team_id})

        return team

    def _build_archive_data(self, team: DeRouter_TeamTable) -> dict[str, object]:
        """Build archive data dict with only columns that exist in DeRouter_DeletedTeamTable."""
    ...
    async def update_spend(self, team_id: str, spend: float) -> DeRouter_TeamTable | None:
        """Update team spend."""
        return await self.update(team_id, {"spend": spend}, id_field="team_id")

    async def add_member(self, team_id: str, user_id: str) -> DeRouter_TeamTable | None:
    ...
    async def remove_member(self, team_id: str, user_id: str) -> DeRouter_TeamTable | None:
    ...
    async def add_admin(self, team_id: str, user_id: str) -> DeRouter_TeamTable | None:
    ...
    async def remove_admin(self, team_id: str, user_id: str) -> DeRouter_TeamTable | None:
    ...
    async def add_models(self, team_id: str, models: list[str]) -> DeRouter_TeamTable | None:
    ...
    async def remove_models(self, team_id: str, models: list[str]) -> DeRouter_TeamTable | None:
