"""
Project repository for database operations on DeRouter_ProjectTable.
"""
...
class ProjectRepository(BaseRepository[DeRouter_ProjectTable]):
    """Repository for project database operations."""

    @property
    def table(self) -> TableActions["prisma_models.DeRouter_ProjectTable"]:
        return self.prisma_client.db.derouter_projecttable

    @property
    def model_class(self) -> type[DeRouter_ProjectTable]:
        return DeRouter_ProjectTable

    async def find_by_id(self, project_id: str, id_field: str = "project_id") -> DeRouter_ProjectTable | None:
        return await super().find_by_id(project_id, id_field)

    async def find_by_alias(self, project_alias: str) -> DeRouter_ProjectTable | None:
        """Find a project by alias."""
        projects: Final = await self.find_many(where={"project_alias": project_alias})
        return projects[0] if projects else None

    async def find_by_team_id(self, team_id: str) -> list[DeRouter_ProjectTable]:
    ...
    async def delete_project(self, project_id: str) -> DeRouter_ProjectTable | None:
        """Delete a project."""
        return await self.delete(project_id, id_field="project_id")

    async def update_spend(self, project_id: str, spend: float) -> DeRouter_ProjectTable | None:
