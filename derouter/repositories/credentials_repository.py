"""
Credentials repository for database operations on DeRouter_CredentialsTable.

This is the only place that talks to ``derouter_credentialstable``. Encryption of
...
from derouter.repositories.base_repository import DbRecord, record_to_dict
from derouter.repositories.prisma_protocols import TableActions

if TYPE_CHECKING:
    from prisma import models as prisma_models

    _CredentialsTable: TypeAlias = TableActions[prisma_models.DeRouter_CredentialsTable]


class _PrismaCredentialsDb(Protocol):
    @property
    def derouter_credentialstable(self) -> "_CredentialsTable": ...
...
class CredentialsRepository:
    ...
    def table(self) -> "_CredentialsTable":
        return wrap_table_actions_for_config_sync(
            actions=self.prisma_client.db.derouter_credentialstable,
            table_name="derouter_credentialstable",
        )
    ...
    async def find_all(self) -> Sequence["prisma_models.DeRouter_CredentialsTable"]:
        return await self.table.find_many()

    async def create(self, data: Mapping[str, object]) -> "prisma_models.DeRouter_CredentialsTable":
    ...
    ) -> "prisma_models.DeRouter_CredentialsTable | None":
        return await self.table.update(where={"credential_name": credential_name}, data=data)

    async def delete_by_name(self, credential_name: str) -> "prisma_models.DeRouter_CredentialsTable | None":
