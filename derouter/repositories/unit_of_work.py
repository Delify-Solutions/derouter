...
from derouter.repositories.prisma_protocols import BatchTable, PrismaBatch
...
async def spend_reset_unit_of_work(new_batch: Callable[[], PrismaBatch]) -> AsyncGenerator[SpendResetUnitOfWork, None]:
    ...
    yield SpendResetUnitOfWork(
        keys=KeySpendResetWrites(table=batch.derouter_verificationtoken),
        users=UserSpendResetWrites(table=batch.derouter_usertable),
        teams=TeamSpendResetWrites(table=batch.derouter_teamtable),
    )
    await batch.commit()
...
async def budget_cascade_unit_of_work(
    ...
    yield BudgetCascadeUnitOfWork(
        team_memberships=LinkedSpendResetWrites(table=batch.derouter_teammembership),
        keys=LinkedSpendResetWrites(table=batch.derouter_verificationtoken),
        organizations=LinkedSpendResetWrites(table=batch.derouter_organizationtable),
        tags=LinkedSpendResetWrites(table=batch.derouter_tagtable),
        model_access_groups=LinkedSpendResetWrites(table=batch.derouter_modelaccessgroupbudgettable),
        endusers=LinkedSpendResetWrites(table=batch.derouter_endusertable),
        budgets=BudgetWindowWrites(table=batch.derouter_budgettable),
    )
    await batch.commit()
