# derouter/proxy/vector_stores/vector_store_registry.py
import json
...
from derouter.derouter_core_utils.core_helpers import remove_items_at_indices
...
from derouter.types.vector_stores import (
    ...
    DeRouter_ManagedVectorStoreIndex,
    DeRouter_ManagedVectorStoreListResponse,
    DeRouter_VectorStoreConfig,
...
class VectorStoreIndexRegistry:
    def __init__(self, vector_store_indexes: list[DeRouter_ManagedVectorStoreIndex] = []):
        self.vector_store_indexes: list[DeRouter_ManagedVectorStoreIndex] = vector_store_indexes

    def get_vector_store_indexes(self) -> list[DeRouter_ManagedVectorStoreIndex]:
        """
        Returns the vector store indexes
        """
        return self.vector_store_indexes

    def get_vector_store_index_by_name(self, vector_store_index_name: str) -> DeRouter_ManagedVectorStoreIndex | None:
        """
    ...
    def upsert_vector_store_index(self, vector_store_index: DeRouter_ManagedVectorStoreIndex):
        """
        Adds a vector store index to the registry.

        If it already exists, it will be updated.
        """
        for i, _vector_store_index in enumerate[DeRouter_ManagedVectorStoreIndex](self.vector_store_indexes):
    ...
    async def _get_vector_store_indexes_from_db(
    ...
    ) -> list[DeRouter_ManagedVectorStoreIndex]:
        """
        Get vector stores from the database
        """
        vector_stores_from_db: Final[list[DeRouter_ManagedVectorStoreIndex]] = []
        if prisma_client is not None:
            ...
            for vector_store in _vector_stores_from_db:
                ...
                _derouter_managed_vector_store = DeRouter_ManagedVectorStoreIndex(**_dict_vector_store)
                vector_stores_from_db.append(_derouter_managed_vector_store)
        return vector_stores_from_db


class VectorStoreRegistry:
    def __init__(self, vector_stores: list[DeRouter_ManagedVectorStore] = []):
        self.vector_stores: list[DeRouter_ManagedVectorStore] = vector_stores
        self.vector_store_ids_to_vector_store_map: dict[str, DeRouter_ManagedVectorStore] = {}
    ...
    ) -> DeRouter_ManagedVectorStore | None:
        """
    ...
    def get_derouter_managed_vector_store_from_registry(self, vector_store_id: str) -> DeRouter_ManagedVectorStore | None:
        """
    ...
    async def get_derouter_managed_vector_store_from_registry_or_db(
        ...
        vector_store: Final = self.get_derouter_managed_vector_store_from_registry(vector_store_id)
    ...
    def get_derouter_managed_vector_store_from_registry_by_name(
    ...
    def pop_vector_stores_to_run(
        self, non_default_params: dict, tools: list[dict] | None = None
    ) -> list[DeRouter_ManagedVectorStore]:
        """
        ...
        Returns:
            List of vector stores with tool parameters merged into derouter_params
        """
        ...
        vector_stores_to_run: Final[list[DeRouter_ManagedVectorStore]] = []
            ...
            for vector_store in self.vector_stores:
                if vector_store.get("vector_store_id") == vector_store_id:
                    ...
                    if vector_store_id in params_by_id:
                        existing_params = vector_store_copy.get("derouter_params", {}) or {}
                        ...
                        vector_store_copy["derouter_params"] = tool_params_dict
    ...
    async def pop_vector_stores_to_run_with_db_fallback(
        ...
        for vector_store_id in vector_store_ids:
            ...
            if vector_store is None and prisma_client is not None:
                try:
                    vector_store = await self.get_derouter_managed_vector_store_from_registry_or_db(
    ...
    def load_vector_stores_from_config(self, vector_stores_config: list[dict]):
        """
        Loads vector stores from the derouter proxy config.yaml
        """
        for vector_store_config in vector_stores_config:
            # cast to VectorStoreConfig
            derouter_vector_store_config = DeRouter_VectorStoreConfig(**vector_store_config)
            vector_store_name = derouter_vector_store_config.get("vector_store_name")
            vector_store_derouter_params: dict[str, Any] = derouter_vector_store_config.get("derouter_params") or {}

            vector_store_id = vector_store_derouter_params.get("vector_store_id")
            if vector_store_id is None:
                raise ValueError(
                    f"vector_store_id is required for initializing vector store, got vector_store_id={vector_store_id}"
                )
            custom_llm_provider = vector_store_derouter_params.get("custom_llm_provider")
            if custom_llm_provider is None:
                raise ValueError(
                    f"custom_llm_provider is required for initializing vector store, got custom_llm_provider={custom_llm_provider}"
                )

            derouter_managed_vector_store = DeRouter_ManagedVectorStore(
                vector_store_id=vector_store_id,
                custom_llm_provider=custom_llm_provider,
                derouter_params=vector_store_derouter_params,
                vector_store_name=vector_store_name,
                vector_store_description=vector_store_derouter_params.get("vector_store_description"),
                vector_store_metadata=vector_store_derouter_params.get("vector_store_metadata"),
                created_at=datetime.now(timezone.utc),
                updated_at=datetime.now(timezone.utc),
            )
            self.vector_stores.append(derouter_managed_vector_store)
    ...
    def list_all_vector_stores(self) -> DeRouter_ManagedVectorStoreListResponse:
        """
        List all vector stores in the required format

        Returns:
            DeRouter_ManagedVectorStoreListResponse: A standardized response with vector store data
        """
    ...
    def add_vector_store_to_registry(self, vector_store: DeRouter_ManagedVectorStore):
        """
    ...
    def update_vector_store_in_registry(self, vector_store_id: str, updated_data: DeRouter_ManagedVectorStore):
    ...
    async def _get_vector_stores_from_db(
        ...
        Get vector stores from the database
        """
        vector_stores_from_db: Final[list[DeRouter_ManagedVectorStore]] = []
        if prisma_client is not None:
            ...
            for vector_store in _vector_stores_from_db:
                _dict_vector_store = dict(vector_store)
                _derouter_managed_vector_store = DeRouter_ManagedVectorStore(**_dict_vector_store)
                vector_stores_from_db.append(_derouter_managed_vector_store)
        return vector_stores_from_db
    ...
    def get_credentials_for_vector_store(self, vector_store_id: str) -> dict[str, object]:
        ...
        for vector_store in self.vector_stores:
            if vector_store.get("vector_store_id") == vector_store_id:
                credentials = vector_store.get("derouter_credential_name")
                if credentials:
