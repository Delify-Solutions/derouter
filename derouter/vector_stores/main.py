"""
DeRouter SDK Functions for Creating and Searching Vector Stores
"""
...
from derouter.vector_stores.utils import VectorStoreRequestUtils
...
def create(
    ...
    **kwargs,
    ...
    try:
        ...
        ## MOCK RESPONSE LOGIC
        if derouter_params.mock_response and isinstance(derouter_params.mock_response, dict):
            return mock_vector_store_create_response(
                mock_response=VectorStoreCreateResponse(**derouter_params.mock_response)
            )
...
def search(
    ...
    **kwargs,
    ...
    try:
        ...
        if derouter.vector_store_registry is not None and vector_store_id is not None:
            try:
                registry_credentials = derouter.vector_store_registry.get_credentials_for_vector_store(vector_store_id)
                kwargs.update(registry_credentials)
            except Exception:
                pass

        # get llm provider logic
        derouter_params: Final = GenericDeRouterParams(vector_store_id=vector_store_id, **kwargs)

        ## MOCK RESPONSE LOGIC
        if derouter_params.mock_response and isinstance(derouter_params.mock_response, (str, builtins.list)):
            mock_results = None
            if isinstance(derouter_params.mock_response, builtins.list):
                mock_results = derouter_params.mock_response
