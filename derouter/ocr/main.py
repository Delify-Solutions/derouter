"""
Main OCR function for DeRouter.
"""
...
from derouter.llms.azure_ai.ocr.common_utils import (
...
from derouter.rust_bridge import ocr as rust_ocr_bridge
from derouter.rust_bridge.bindings import native_exception_types
...
def _prepare_ocr_request(
    ...
    derouter_logging_obj: Final = cast(DeRouterLoggingObj, kwargs.pop("derouter_logging_obj"))
    derouter_call_id: Final = cast(str | None, kwargs.get("derouter_call_id", None))
    ...
    derouter_params: Final = GenericDeRouterParams.model_validate(kwargs)
    ...
    if requested_format is not None:
        try:
            ...
            raise derouter.exceptions.UnsupportedParamsError(
...
def _rust_bridge_optional_params(
    ...
    if prepared_request.custom_llm_provider == "vertex_ai":
        vertex_project: Final = (
            prepared_request.derouter_params.get("vertex_project")
            or prepared_request.derouter_params.get("vertex_ai_project")
            or derouter.vertex_project
            or resolve_secret("VERTEXAI_PROJECT")
        )
        vertex_location: Final = (
            prepared_request.derouter_params.get("vertex_location")
            or prepared_request.derouter_params.get("vertex_ai_location")
...
def _prepare_rust_ocr_call(
    ...
    resolved_headers: Final = provider_config.validate_environment(
        ...
        derouter_params=prepared_request.derouter_params,
    )
    ...
    prepared_request.derouter_logging_obj.pre_call(
    ...
    **kwargs: object,
    ...
    try:
        ...
        response = base_llm_http_handler.ocr(
            ...
            logging_obj=prepared.derouter_logging_obj,
            api_key=prepared.api_key,
            api_base=prepared.api_base,
            custom_llm_provider=prepared.custom_llm_provider,
            aocr=True,
            ...
            derouter_params=prepared.derouter_params,
        )
