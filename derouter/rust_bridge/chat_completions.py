...
DeRouter's existing `ModelResponse` builder.
...
from derouter.llms.bedrock.request_metadata import bedrock_request_metadata_is_owned
from derouter.rust_bridge.configuration import rust_enabled
from derouter.rust_bridge.loader import get_native_bridge
from derouter.rust_bridge.timeouts import timeout_to_seconds
...
# `derouter_params` values are `object`, so validate the one this module reads
# rather than narrowing an unparameterized `Mapping` and typing the result Any.
_DEROUTER_METADATA_ADAPTER: Final = TypeAdapter(Mapping[str, object])

RUST_RESPONSE_HEADER: Final = "x-derouter-rust"
...
def _anthropic_user_id_reaches_the_body(derouter_params: Mapping[str, object] | None) -> bool:
    metadata: Final = derouter_params.get("metadata") if derouter_params is not None else None
    try:
        entries: Final = _DEROUTER_METADATA_ADAPTER.validate_python(metadata)
    except ValidationError:
        return False
    return entries.get("user_id") is not None


def _derouter_metadata_reaches_the_provider(
    custom_llm_provider: str | None, derouter_params: Mapping[str, object] | None
) -> bool:
    ...
    match custom_llm_provider:
        case "anthropic":
            return _anthropic_user_id_reaches_the_body(derouter_params)
        case "bedrock":
...
def rust_chat_completions_accepts(
    *,
    ...
    if not rust_enabled():
        return False
    if _derouter_metadata_reaches_the_provider(custom_llm_provider, derouter_params):
        verbose_logger.debug("Rust chat completions declined (derouter metadata user_id); using the Python path")
        return False
...
def _reraise_or_decline(
    ...
    *,
    ...
    the upstream status, which DeRouter's exception mapping already understands.
    """
    ...
    if isinstance(rust_error, upstream_failed):
        ...
        raise APIError(
            ...
            message=f"derouter rust chat completions: {message}",
