...
async def _fetch_interaction(context: BackgroundInteractionPollContext) -> InteractionsAPIResponse:
    from derouter.interactions import aget
...
def _is_settled(logging_obj: "DeRouterLoggingObj") -> bool:
    return logging_obj.model_call_details.get(_SETTLED_KEY) is True


def _claim_settlement(logging_obj: "DeRouterLoggingObj") -> bool:
    """
...
async def _release_open_budget_reservation(logging_obj: "DeRouterLoggingObj") -> None:
    """
    ...
    """
    metadata = get_derouter_metadata_from_kwargs(kwargs=logging_obj.model_call_details)
    budget_reservation = metadata.get("user_api_key_budget_reservation")
    if not isinstance(budget_reservation, dict):
        return

    from derouter.proxy.spend_tracking.budget_reservation import release_budget_reservation
...
async def _bill_settled_interaction(logging_obj: "DeRouterLoggingObj", response: InteractionsAPIResponse) -> None:
    """
...
def maybe_schedule_background_interaction_cost_polling(
    ...
    if not is_pollable_background_interaction(response):
        return None
    logging_obj = create_kwargs.get("derouter_logging_obj")
