...
def decode_managed_container_id_for_request(
...
) -> tuple[str, str, GenericDeRouterParams]:
    """Decode a DeRouter-managed container ID for upstream API calls.

    Returns:
        (original_container_id, resolved_provider, updated_derouter_params)
    """
    ...
    if decoded_model_id and not derouter_params.get("model_id"):
        derouter_params["model_id"] = decoded_model_id

    return original_container_id, custom_llm_provider, derouter_params
...
class ContainerRequestUtils:
    ...
    def encode_container_id_in_response(
        ...
        Encodes when:
        1. derouter_metadata contains model_info.id (indicating router/proxy usage), OR
        ...
        Args:
            ...
            derouter_metadata: Optional derouter_metadata dict that may contain model_info.id
        ...
        """
        # Extract model_id from derouter_metadata
