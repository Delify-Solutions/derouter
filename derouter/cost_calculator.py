...
from derouter.derouter_core_utils.llm_cost_calc.utils import (
    CostCalculatorUtils,
...
from derouter.llms.anthropic.cost_calculation import (
    cost_per_token as anthropic_cost_per_token,
)
from derouter.llms.azure.cost_calculation import (
    cost_per_token as azure_openai_cost_per_token,
)
from derouter.llms.azure_ai.cost_calculator import (
...
from derouter.llms.bedrock.cost_calculation import (
    cost_per_token as bedrock_cost_per_token,
)
from derouter.llms.databricks.cost_calculator import (
    cost_per_token as databricks_cost_per_token,
)
from derouter.llms.deepseek.cost_calculator import (
    cost_per_token as deepseek_cost_per_token,
)
from derouter.llms.fireworks_ai.cost_calculator import (
    cost_per_token as fireworks_ai_cost_per_token,
)
from derouter.llms.gemini.cost_calculator import cost_per_token as gemini_cost_per_token
from derouter.llms.lemonade.cost_calculator import (
    cost_per_token as lemonade_cost_per_token,
)
from derouter.llms.openai.cost_calculation import (
...
from derouter.llms.perplexity.cost_calculator import (
    cost_per_token as perplexity_cost_per_token,
)
from derouter.llms.tencent.cost_calculator import (
    cost_per_token as tencent_cost_per_token,
)
from derouter.llms.together_ai.cost_calculator import (
    get_model_params_and_category,
    has_together_registry_pricing,
)
from derouter.llms.vertex_ai.common_utils import get_vertex_ai_lyria_generation_cost
from derouter.llms.vertex_ai.cost_calculator import (
    cost_per_character as google_cost_per_character,
)
from derouter.llms.vertex_ai.cost_calculator import (
    cost_per_token as google_cost_per_token,
)
from derouter.llms.vertex_ai.cost_calculator import cost_router as google_cost_router
from derouter.llms.xai.cost_calculator import cost_per_token as xai_cost_per_token
...
from derouter.types.rerank import RerankBilledUnits, RerankResponse
...
def cost_per_token(
    ...
    model_cost_ref: Final = derouter.model_cost
    ...
    if caller_supplied_provider:
        ...
        _, custom_llm_provider, _, _ = derouter.get_llm_provider(model=model)
    ...
    if len(model_parts) > 1:
        ...
        model_without_prefix = model
    """
    Code block that formats model to lookup in derouter.model_cost
    ...
    if call_type == "speech" or call_type == "aspeech":
        ...
        speech_model_info = derouter.get_model_info(model=model_without_prefix, custom_llm_provider=custom_llm_provider)
        ...
        from derouter.search import search_provider_cost_per_query
        ...
        from derouter.llms.dashscope.cost_calculator import (
...
def get_replicate_completion_pricing(completion_response: dict, total_time=0.0):
    ...
    # for all derouter currently supported LLMs, almost all requests go to a100_80gb
...
def _get_provider_for_cost_calc(
    ...
    try:
        ...
        verbose_logger.debug(
            "derouter.cost_calculator.py::_get_provider_for_cost_calc() - Error inferring custom_llm_provider - %s", e
        )
        return None
...
def _select_model_name_for_cost_calc(
    ...
    if custom_pricing is True:
        if router_model_id is not None and router_model_id in derouter.model_cost:
            entry: Final = derouter.model_cost[router_model_id]
            if (
...
def _strip_unregistered_leading_segments(model: str, region_name: str | None) -> str:
    ...
    if "/".join(segments[1:]) in derouter.model_cost:
        return model
    ...
    return next((candidate for candidate in candidates if candidate in derouter.model_cost), model)
...
def _map_traffic_type_to_service_tier(traffic_type: str | None) -> str | None:
    """
    Map a Gemini usageMetadata.trafficType value to a DeRouter service_tier string.
...
def _get_usage_object(
    ...
    if isinstance(usage_obj, Usage):
        return usage_obj
    elif isinstance(usage_obj, dict) and derouter.AnthropicConfig.is_anthropic_usage_object(usage_obj):
        return derouter.AnthropicConfig().calculate_usage(usage_object=usage_obj, reasoning_content=None)
    elif (
...
def _is_known_usage_objects(usage_obj):
    ...
    return (
        isinstance(usage_obj, derouter.Usage)
...
def _infer_call_type(call_type: CallTypesLiteral | None, completion_response: Any) -> CallTypesLiteral | None:
    ...
    elif isinstance(completion_response, DeRouterSendMessageResponse):
        return "send_message"
...
def _apply_cost_discount(
    ...
    if custom_llm_provider and custom_llm_provider in derouter.cost_discount_config:
        discount_percent = derouter.cost_discount_config[custom_llm_provider]
...
def _apply_cost_margin(
    ...
    margin_config = None
    if custom_llm_provider and custom_llm_provider in derouter.cost_margin_config:
        margin_config = derouter.cost_margin_config[custom_llm_provider]
        if verbose_logger.isEnabledFor(logging.DEBUG):
            verbose_logger.debug("Found provider-specific margin config for %s: %s", custom_llm_provider, margin_config)
    elif "global" in derouter.cost_margin_config:
        margin_config = derouter.cost_margin_config["global"]
        if verbose_logger.isEnabledFor(logging.DEBUG):
        ...
        if verbose_logger.isEnabledFor(logging.DEBUG):
            verbose_logger.debug(
                ...
                custom_llm_provider,
                list(derouter.cost_margin_config.keys()),
            )
...
def _store_cost_breakdown_in_logging_obj(
    derouter_logging_obj: LitellmLoggingObject | None,
    ...
    Args:
        derouter_logging_obj: The logging object to store breakdown in
    ...
    try:
        ...
        derouter_logging_obj.set_cost_breakdown(
...
def completion_cost(
    ...
    router_model_id: str | None = None,
    derouter_logging_obj: LitellmLoggingObject | None = None,
    ### SERVICE TIER ###
    service_tier: str | None = None,  # for OpenAI service tier pricing
    ### DATA RESIDENCY ###
    data_residency: str | None = None,  # for OpenAI regional-processing uplift (e.g. "eu", "us")
    ...
    """
    Calculate the cost of a given completion call fot GPT-3.5-turbo, llama2, any derouter supported llm.

    Parameters:
        completion_response (derouter.ModelResponses): [Required] The response received from a DeRouter completion request.
    ...
    Exceptions:
        Raises exception if model not in the derouter model cost map. Register model, via custom pricing or PR - https://github.com/Delify-Solutions/derouter/blob/main/model_prices_and_context_window.json
        ...
        for idx, model in enumerate(potential_model_names):
            try:
                ...
                if completion_response is not None and (
                    ...
                    if isinstance(usage_obj, BaseModel) and not _is_known_usage_objects(usage_obj=usage_obj):
                        ...
                        setattr(
                            completion_response,
                            "usage",
                            derouter.Usage(**_usage_for_dump.model_dump()),
                        )
                    if usage_obj is None:
                        _usage = {}
                    elif isinstance(usage_obj, BaseModel):
                    ...
                    if derouter.AnthropicConfig.is_anthropic_usage_object(_usage):
                        _usage = (
                            derouter.AnthropicConfig()
                ...
                if call_type in _A2A_CALL_TYPES:
                    from derouter.a2a_protocol.cost_calculator import A2ACostCalculator

                    return A2ACostCalculator.calculate_a2a_cost(derouter_logging_obj=derouter_logging_obj)
                ...
                if custom_llm_provider is None:
                    try:
                        ...
                        verbose_logger.debug(
                            "derouter.cost_calculator.py::completion_cost() - Error inferring custom_llm_provider - %s",
                            e,
                        )
                if CostCalculatorUtils._call_type_has_image_response(call_type) and isinstance(
                    ...
                    if custom_pricing and derouter_logging_obj is not None:
                        _derouter_params = getattr(derouter_logging_obj, "derouter_params", None)
                        if _derouter_params is not None:
                            _video_model_info = next(
                                (
                                    model_info
                                    for _metadata_key in ("metadata", "derouter_metadata")
                                    if (model_info := (_derouter_params.get(_metadata_key) or {}).get("model_info"))
                                    is not None
                                ),
                                None,
                            )
                    ...
                    prompt_characters = derouter.utils._count_characters(text=prompt)
                    ...
                    completion_response, DeRouterRealtimeStreamLoggingObject
                ):
                    ...
                    from derouter.proxy._experimental.mcp_server.cost_calculator import (
                        MCPCostCalculator,
                    )

                    return MCPCostCalculator.calculate_mcp_tool_call_cost(derouter_logging_obj=derouter_logging_obj)
                # Calculate cost based on prompt_tokens, completion_tokens
                if (
                    "togethercomputer" in model or "together_ai" in model or custom_llm_provider == "together_ai"
                ) and not has_together_registry_pricing(model, derouter.model_cost):
                    model = get_model_params_and_category(model, call_type=CallTypes(call_type))

                # replicate llms are calculate based on time for request running
                # see https://replicate.com/pricing
                elif (model in derouter.replicate_models or "replicate" in model) and model not in derouter.model_cost:
                ...
                if custom_llm_provider is not None and custom_llm_provider == "vertex_ai":
                    ...
                    if len(messages) > 0:
                        prompt_string = derouter.utils.get_formatted_prompt(
                            data={"messages": messages}, call_type="completion"
                        )

                        prompt_characters = derouter.utils._count_characters(text=prompt_string)
                    if completion_response is not None and isinstance(completion_response, ModelResponse):
                        completion_string = derouter.utils.get_response_string(response_obj=completion_response)
                        completion_characters = derouter.utils._count_characters(text=completion_string)

                # Get the original request model for router detection
                request_model_for_cost = None
                if derouter_logging_obj is not None:
                    request_model_for_cost = derouter_logging_obj.model
                ...
                if custom_llm_provider == "azure_ai" and not azure_ai_is_model_router_name(model):
                    ...
                    if completion_response is not None:
                        ...
                        hidden_model = hidden_params.get("model") or hidden_params.get("derouter_model_name")
                        if (
                            hidden_model
                            and (
                ...
                original_cost = _final_cost
                if derouter.cost_discount_config:
                    (
                        _final_cost,
                        discount_percent,
                        discount_amount,
                ...
                # Apply margin from module-level config if configured
                if derouter.cost_margin_config:
                    (
                        _final_cost,
                        margin_percent,
                        margin_fixed_amount,
                ...
                verbose_logger.debug(
                    "derouter.cost_calculator.py::completion_cost() - Error calculating cost for model=%s - %s", model, e
                )
...
def get_response_cost_from_hidden_params(
    ...
    if additional_headers and "llm_provider-x-derouter-response-cost" in additional_headers:
        response_cost: Final = additional_headers["llm_provider-x-derouter-response-cost"]
        if response_cost is None:
            return None
        return float(additional_headers["llm_provider-x-derouter-response-cost"])
    return None
...
def ocr_cost(
    ...
    try:
        model_info: ModelInfo | None = derouter.get_model_info(model=model, custom_llm_provider=custom_llm_provider)
    except Exception:
        model_info = None
...
def vector_store_search_cost(
    ...
    if model is not None and "/" in model:
        api_type, custom_llm_provider, _, _ = derouter.get_llm_provider(
            model=model,
        )
...
def rerank_cost(
    ...
    """
    _, custom_llm_provider, _, _ = derouter.get_llm_provider(model=model, custom_llm_provider=custom_llm_provider)
...
def default_image_cost_calculator(
    ...
    for _model in models_to_check:
        if _model is not None and _model in derouter.model_cost:
            cost_info = derouter.model_cost[_model]
            break
...
def default_video_cost_calculator(
    ...
    Args:
        ...
        model_info (Optional[ModelInfo]): Deployment-level model info containing
            ...
            the global derouter.model_cost lookup.
    ...
    if model_info is not None:
        ...
        if cost_info is None and custom_llm_provider:
            ...
            if prefixed_model in derouter.model_cost:
                cost_info = derouter.model_cost[prefixed_model]
...
def batch_cost_calculator(
    ...
    Args:
        model_info: Optional deployment-level model info containing custom
            ...
            skips the global derouter.get_model_info() lookup so that
    ...
    if model_info is None:
        try:
            model_info = derouter.get_model_info(model=model, custom_llm_provider=custom_llm_provider)
        except Exception:
            model_info = None
    elif not any(
        ...
        try:
            global_info: Final = derouter.get_model_info(model=model, custom_llm_provider=custom_llm_provider)
            if global_info:
...
class RealtimeAPITokenUsageProcessor(BaseTokenUsageProcessor):
    ...
    def create_logging_realtime_object(
        ...
        return DeRouterRealtimeStreamLoggingObject(
            usage=usage,
            results=results,
        )
...
def _cost_map_entry_declares_pricing(model_name: str, custom_llm_provider: str) -> bool:
    entries: Final = (
        derouter.model_cost.get(model_name),
        derouter.model_cost.get(f"{custom_llm_provider}/{model_name}"),
    )
    return any(
...
def handle_realtime_stream_cost_calculation(
    ...
    potential_model_names.append(derouter_model_name)
...
def handle_realtime_transcription_cost_calculation(
    ...
    model_name: Final = _get_transcription_model_name_from_results(results) or derouter_model_name
    try:
        model_info = derouter.get_model_info(model=model_name, custom_llm_provider=custom_llm_provider)
    except Exception:
        model_info = None
