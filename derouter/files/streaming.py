...
from derouter.files.types import FileContentProvider

if TYPE_CHECKING:
    from derouter.derouter_core_utils.derouter_logging import (
        Logging as DeRouterLoggingObj,
    )
    from derouter.types.utils import StandardLoggingHiddenParams, StandardLoggingPayload


class FileContentStreamingResponse:
    """
    Iterator wrapper for file content streaming that carries DeRouter metadata
    ...
    def _sync_hidden_params(self) -> None:
        ...
        if self.logging_obj is not None:
            derouter_params = self.logging_obj.model_call_details.get("derouter_params", {}) or {}

        if "api_base" not in self._hidden_params and derouter_params.get("api_base"):
            self._hidden_params["api_base"] = derouter_params["api_base"]
        ...
        self._hidden_params["derouter_model_name"] = self.model
