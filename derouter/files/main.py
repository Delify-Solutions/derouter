...
FileRetrieveProvider = Literal[
    "openai", "azure", "gemini", "vertex_ai", "hosted_vllm", "derouter_proxy", "manus", "anthropic"
]
FileDeleteProvider = Literal["openai", "azure", "gemini", "derouter_proxy", "manus", "anthropic"]
FileListProvider = Literal["openai", "azure", "derouter_proxy", "manus", "anthropic"]
import derouter
from derouter import get_secret_str
from derouter.files.streaming import FileContentStreamingResponse
from derouter.files.types import FileContentProvider, FileContentStreamingResult
...
from derouter.llms.azure.common_utils import get_azure_credentials
from derouter.llms.azure.files.handler import AzureOpenAIFilesAPI
from derouter.llms.bedrock.files.handler import BedrockFilesHandler
...
from derouter.llms.openai.common_utils import get_openai_credentials
from derouter.llms.openai.openai import FileDeleted, FileObject, OpenAIFilesAPI
from derouter.llms.vertex_ai.files.handler import VertexAIFilesHandler
...
async def acreate_file(
    ...
    **kwargs,
    ...
    DeRouter Equivalent of POST: POST https://api.openai.com/v1/files
    """
    try:
...
def create_file(
    ...
    **kwargs,
    ...
    try:
        ...
        derouter_params_dict: Final = dict(**kwargs)
        logging_obj: Final = cast(DeRouterLoggingObj | None, kwargs.get("derouter_logging_obj"))
        ...
        if provider_config is not None:
            ...
            raise derouter.exceptions.BadRequestError(
                message=f"DeRouter doesn't support {custom_llm_provider} for 'create_file'. Only ['openai', 'azure', 'vertex_ai', 'manus', 'anthropic'] are supported.",
                model="n/a",
                llm_provider=custom_llm_provider,
                response=httpx.Response(
                    status_code=400,
                    content="Unsupported provider",
                    request=httpx.Request(method="create_file", url="https://github.com/Delify-Solutions/derouter"),
                ),
            )
        return response
    except Exception as e:
...
async def afile_retrieve(
    ...
    **kwargs,
    ...
    DeRouter Equivalent of GET https://api.openai.com/v1/files
    """
    try:
        loop: Final = asyncio.get_event_loop()
    ...
    try:
        ...
        if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
            ...
            if provider_config is not None:
                ...
                add_trusted_model_credentials_to_derouter_params(
                    derouter_params_dict=derouter_params_dict,
                    kwargs=kwargs,
                )
                derouter_params_dict["api_key"] = optional_params.api_key
                derouter_params_dict["api_base"] = optional_params.api_base
                ...
                if logging_obj is None:
                    ...
                    logging_obj = DeRouterLoggingObj(
                        ...
                        derouter_call_id=kwargs.get("derouter_call_id", str(uuid_module.uuid4())),
                ...
                raise derouter.exceptions.BadRequestError(
                    message=f"DeRouter doesn't support {custom_llm_provider} for 'file_retrieve'. Only 'openai', 'azure', 'manus', and 'anthropic' are supported.",
                    model="n/a",
                    llm_provider=custom_llm_provider,
                    response=httpx.Response(
                        status_code=400,
                        ...
                        request=httpx.Request(
                            ...
                            url="https://github.com/Delify-Solutions/derouter",
                        ),
                    ),
                )
...
async def afile_delete(
    ...
    **kwargs,
    ...
    DeRouter Equivalent of DELETE https://api.openai.com/v1/files
    """
    try:
        loop: Final = asyncio.get_event_loop()
    ...
    try:
        ...
        if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
            ...
            if provider_config is not None:
                ...
                raise derouter.exceptions.BadRequestError(
                    message=f"DeRouter doesn't support {custom_llm_provider} for 'file_delete'. Only 'openai', 'azure', 'gemini', 'manus', and 'anthropic' are supported.",
                    model="n/a",
                    llm_provider=custom_llm_provider,
                    response=httpx.Response(
                        status_code=400,
    ...
    **kwargs,
    ...
    try:
        ...
        if provider_config is not None:
            ...
            raise derouter.exceptions.BadRequestError(
                message=f"DeRouter doesn't support {custom_llm_provider} for 'file_list'. Only 'openai', 'azure', 'manus', and 'anthropic' are supported.",
                model="n/a",
                llm_provider=custom_llm_provider,
                response=httpx.Response(
                    status_code=400,
                    content="Unsupported provider",
                    request=httpx.Request(method="file_list", url="https://github.com/Delify-Solutions/derouter"),
                ),
            )
        return response
    except Exception as e:
    ...
    try:
        ...
        if stream and _should_sdk_support_streaming(custom_llm_provider):
            return file_content_streaming(
                ...
                timeout=timeout,
                logging_obj=cast(DeRouterLoggingObj | None, kwargs.get("derouter_logging_obj")),
                _is_async=_is_async,
                client=client,
            )
        ...
        if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
            ...
            response = bedrock_files_instance.file_content(
                ...
                file_content_request=_file_content_request,
                api_base=optional_params.api_base,
                optional_params=derouter_params_dict,
                timeout=timeout,
                max_retries=optional_params.max_retries,
            )
        else:
            raise derouter.exceptions.BadRequestError(
                message=f"DeRouter doesn't support {custom_llm_provider} for 'file_content'. Supported providers are 'openai', 'azure', 'vertex_ai', 'bedrock', 'manus', 'anthropic'.",
                model="n/a",
                llm_provider=custom_llm_provider,
                response=httpx.Response(
                    status_code=400,
...
def file_content_streaming(
    *,
    ...
    if logging_obj is not None:
        ...
        derouter_params: Final = logging_obj.model_call_details.get("derouter_params", {}) or {}
        if optional_params.api_base is not None:
            derouter_params["api_base"] = optional_params.api_base
        logging_obj.model_call_details["derouter_params"] = derouter_params
    ...
    if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
        ...
        raise derouter.exceptions.BadRequestError(
            message=f"DeRouter doesn't support {custom_llm_provider} for streaming 'file_content'. Supported providers are {sorted(OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS)}.",
            model="n/a",
            llm_provider=custom_llm_provider,
            response=httpx.Response(
                status_code=400,
