...
class VideoGenerationRequestUtils:
    ...
    def get_requested_video_generation_optional_param(
        ...
        base_params: Final = filter_out_derouter_params(kwargs=base_params_raw)

        cleaned_kwargs: Final = filter_out_derouter_params(kwargs={k: v for k, v in raw_kwargs.items() if v is not None})
        ...
        if merged_extra_body:
            merged_extra_body = filter_out_derouter_params(kwargs=merged_extra_body)
            if merged_extra_body:
