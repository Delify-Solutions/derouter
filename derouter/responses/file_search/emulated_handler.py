...
Flow:
  ...
  3. If the provider issues a file_search function_call, execute vector search
     via derouter.vector_stores.main.asearch().
...
from derouter.types.llms.openai import ResponseOutputItem, ResponsesAPIResponse
from derouter.types.vector_stores import VectorStoreSearchResult
...
FILE_SEARCH_FUNCTION_NAME: Final = "derouter_file_search"
...
def _build_function_tool(vector_store_ids: list[str]) -> dict[str, object]:
    ...
    file_search); DeRouter runs the actual vector search against the configured
    vector stores.
    ...
    DeRouterCompletionResponsesConfig transformation picks up name and description.
    """
    return {
        "type": "function",
...
async def _run_vector_searches(
    ...
    """
    import derouter.vector_stores.main as vs_main
...
async def aresponses_with_emulated_file_search(
    ...
    **kwargs: Any,  # kwargs-ok: `object` would surface the caller's partially-unknown dict at its call site
    ...
    # the parent derouter_logging_obj (propagated via kwargs) fires once at the end.
