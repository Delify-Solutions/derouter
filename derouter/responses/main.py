...
from derouter.integrations.anthropic_cache_control_hook import CARRY_UNMATCHED_MESSAGE_POINTS
...
from derouter.llms.openai_like.responses.transformation import OpenAILikeResponsesConfig
from derouter.responses.derouter_completion_transformation.handler import (
    DeRouterCompletionTransformationHandler,
)
...
if TYPE_CHECKING:
    from derouter.types.llms.openai import ResponseText
else:
...
from derouter.llms.openai.data_residency import infer_openai_data_residency
...
derouter_completion_transformation_handler: Final = DeRouterCompletionTransformationHandler()
#################################################
...
async def aresponses_api_with_mcp(
    ...
    **kwargs,
    ...
    When MCP tools with server_url="derouter_proxy" are provided, this function will:
    ...
    (
        mcp_tools_with_derouter_proxy,
        other_tools,
    ) = await DeRouter_Proxy_MCP_Handler._split_mcp_tools(tools)

    # Process MCP tools through the complete pipeline (fetch + filter + deduplicate + transform)
    # Extract user_api_key_auth from derouter_metadata (where it's added by add_user_api_key_auth_to_request_metadata)
    user_api_key_auth = kwargs.get("user_api_key_auth") or kwargs.get("derouter_metadata", {}).get("user_api_key_auth")
    ...
    (
        ...
        mcp_tools_with_derouter_proxy=mcp_tools_with_derouter_proxy,
        derouter_trace_id=kwargs.get("derouter_trace_id"),
        mcp_auth_header=mcp_auth_header,
        mcp_server_auth_headers=mcp_server_auth_headers,
        request_tags=DeRouter_Proxy_MCP_Handler._get_parent_request_tags(kwargs),
    )
    openai_tools: Final = DeRouter_Proxy_MCP_Handler._transform_mcp_tools_to_openai(original_mcp_tools)
    ...
    if stream and mcp_tools_with_derouter_proxy:
        # Generate MCP discovery events using the already processed tools
        from derouter._uuid import uuid
        from derouter.responses.mcp.mcp_streaming_iterator import (
        ...
        mcp_streaming_response: Final = DeRouter_Proxy_MCP_Handler._create_mcp_streaming_response(
            input=input,
            model=model,
            all_tools=all_tools,
            mcp_tools_with_derouter_proxy=mcp_tools_with_derouter_proxy,
    ...
    should_auto_execute = bool(mcp_tools_with_derouter_proxy) and DeRouter_Proxy_MCP_Handler._should_auto_execute_tools(
        mcp_tools_with_derouter_proxy=mcp_tools_with_derouter_proxy
    )

    # Prepare parameters for the initial call
    initial_call_params: Final = DeRouter_Proxy_MCP_Handler._prepare_initial_call_params(
    ...
    if should_auto_execute and isinstance(response, ResponsesAPIResponse):
        tool_calls: Final = DeRouter_Proxy_MCP_Handler._extract_tool_calls_from_response(response=response)

        if tool_calls:
            user_api_key_auth = kwargs.get("derouter_metadata", {}).get("user_api_key_auth")
            ...
            tool_results: Final = await DeRouter_Proxy_MCP_Handler._execute_tool_calls(
                tool_server_map=tool_server_map,
            ...
            if tool_results:
                persistence_disabled: Final = DeRouter_Proxy_MCP_Handler._is_persistence_disabled(call_params)

                follow_up_input: Final = DeRouter_Proxy_MCP_Handler._create_follow_up_input(
                    response=response,
                ...
                follow_up_call_params: Final = DeRouter_Proxy_MCP_Handler._prepare_follow_up_call_params(
                ...
                if stream:
                    tool_execution_events = DeRouter_Proxy_MCP_Handler._create_tool_execution_events(
                        tool_calls=tool_calls, tool_results=tool_results
                    )

                final_response = await DeRouter_Proxy_MCP_Handler._make_follow_up_call(
                ...
                # Add custom output elements to the final response (for non-streaming)
                    ...
                    final_response = DeRouter_Proxy_MCP_Handler._add_mcp_output_elements_to_response(
...
async def aresponses(
    ...
    **kwargs,
    ...
    try:
        ...
        ) and derouter_logging_obj.should_run_prompt_management_hooks(prompt_id=prompt_id, non_default_params=kwargs):
        ...
        if isinstance(response, ResponsesAPIResponse):
            ...
            # (mirrors derouter/main.py:1371 for chat completions)
...
def _resolve_prompt_swapped_provider(
    ...
    swapped_provider: Final = derouter.get_llm_provider(model=swapped_model)[1]
    if kwargs.get("api_key") is None and kwargs.get("api_base") is None:
        return swapped_provider
    try:
        original_provider: Final = custom_llm_provider or derouter.get_llm_provider(model=original_model)[1]
...
def _apply_prompt_management_to_responses_call(
    ...
    if isinstance(derouter_logging_obj, DeRouterLoggingObj) and derouter_logging_obj.should_run_prompt_management_hooks(
...
def _resolve_model_provider_for_responses(
    ...
    if custom_llm_provider is not None and not derouter_params.custom_llm_provider:
    ...
    if dynamic_api_key is not None:
        derouter_params.api_key = dynamic_api_key
    if dynamic_api_base is not None:
        derouter_params.api_base = dynamic_api_base
...
def _responses_try_dispatch_mcp_gateway(
    *,
    ...
    if skip_mcp_handler or not DeRouter_Proxy_MCP_Handler._should_use_derouter_mcp_gateway(tools=tools):
        return None
...
def _responses_try_dispatch_emulated_file_search(
    *,
    ...
    if not _has_file_search_tool(tools) or not (
        ...
        return None
    from derouter.responses.file_search.emulated_handler import (
        aresponses_with_emulated_file_search,
    )

    _internal_skip: Final = {"derouter_call_id", "aresponses"}
    ...
    **kwargs,
    ...
    try:
        ...
        if derouter_params.mock_response and isinstance(derouter_params.mock_response, str):
            return mock_responses_api_response(mock_response=derouter_params.mock_response)
        ...
        # Map reasoning_effort (from derouter_params/proxy config) to reasoning when not set
        if reasoning is None and "reasoning_effort" in local_vars:
            _mapped = DeRouterResponsesTransformationHandler()._map_reasoning_effort(local_vars.pop("reasoning_effort"))
        ...
        if _bridges_to_chat_completions(responses_api_provider_config, use_chat_completions_api):
            return derouter_completion_transformation_handler.response_api_handler(
                model=model,
                input=input,
        ...
        derouter_logging_obj.update_from_kwargs(
            ...
            derouter_params={
                **responses_api_request_params,
                ...
                "data_residency": infer_openai_data_residency(custom_llm_provider, derouter_params.api_base),
                "metadata": (kwargs["derouter_metadata"] if "derouter_metadata" in kwargs else kwargs.get("metadata")),
            },
            custom_llm_provider=custom_llm_provider,
        )

        # Decode any derouter-encoded encrypted-content item IDs back to their original IDs
...
def _build_derouter_metadata_for_ws(kwargs: dict) -> dict:
    metadata: Final[dict] = {**(kwargs.get("derouter_metadata") or {})}
...
async def _aresponses_websocket(
    ...
    **kwargs,
    ...
    derouter_params_dict["data_residency"] = infer_openai_data_residency(
        _custom_llm_provider,
        dynamic_api_base or derouter_params.api_base or derouter.api_base,
    )
    ...
    if _custom_llm_provider is not None:
        responses_api_provider_config = ProviderConfigManager.get_provider_responses_api_config(
            model=resolved_model,
            provider=derouter.LlmProviders(_custom_llm_provider),
        )

    resolved_api_base: Final = dynamic_api_base or derouter_params.api_base or derouter.api_base or None
    resolved_api_key: Final = (
        dynamic_api_key
        or derouter_params.api_key
        or derouter.api_key
    ...
    await base_llm_http_handler.async_responses_websocket(
        ...
        derouter_metadata=_build_derouter_metadata_for_ws(kwargs),
        custom_llm_provider=_custom_llm_provider,
        **remaining_kwargs,
    )
