...
from derouter.types.responses.main import DecodedResponseId
...
class ResponsesAPIRequestUtils:
    ...
    def _check_valid_arg(
        ...
        if unsupported_params:
            if derouter.drop_params is True or (drop_params is not None and drop_params is True):
                ...
                raise derouter.UnsupportedParamsError(
                    status_code=500,
    ...
    def get_optional_params_responses_api(
        ...
        """
        from derouter.utils import _apply_openai_param_overrides

        # Remove None values and internal parameters
        # Get supported parameters for the model
        supported_params: Final = responses_api_provider_config.get_supported_openai_params(model)

        should_drop_params: Final = derouter.drop_params or drop_params is True
    ...
    def get_requested_response_api_optional_param(
        ...
        """
        from derouter.utils import PreProcessNonDefaultParams
        ...
        # decode previous_response_id if it's a derouter encoded id
    ...
    def _update_responses_api_response_id_with_model_id(
        ...
        custom_llm_provider: str | None,
        derouter_metadata: dict[str, Any] | None = None,
    ) -> ResponsesAPIResponse: 
        ...
    ...
    def _update_responses_api_response_id_with_model_id(
        ...
        model_info: Final[dict[str, Any]] = derouter_metadata.get("model_info", {}) or {}
        ...
        if ResponsesAPIRequestUtils._is_derouter_encoded_response_id(response_id):
            return responses_api_response
        ...
        if derouter_metadata.get("encrypted_content_affinity_enabled"):
    ...
    def _build_encrypted_item_id(model_id: str, item_id: str) -> str:
        ...
        Format: ``encitem_{base64("derouter:model_id:{model_id};item_id:{original_id}")}``
        """
        assembled: Final = f"derouter:model_id:{model_id};item_id:{item_id}"
    ...
    def _decode_encrypted_item_id(encoded_id: str) -> dict[str, str] | None:
        """Decode a derouter-encoded encrypted-content item ID.

        Returns a dict with ``model_id`` and ``item_id`` keys, or ``None`` if
        the string is not a derouter-encoded item ID.
        """
        ...
        try:
            ...
            if len(parts) < 2:
                return None
            model_id: Final = parts[0].replace("derouter:model_id:", "")
    ...
    def _wrap_encrypted_content_with_model_id(encrypted_content: str, model_id: str) -> str:
        ...
        Format: ``derouter_enc:{base64("model_id:{model_id}")};{original_encrypted_content}``
        """
        metadata: Final = f"model_id:{model_id}"
        encoded_metadata: Final = base64.b64encode(metadata.encode("utf-8")).decode("utf-8")
        return f"derouter_enc:{encoded_metadata};{encrypted_content}"
    ...
    def _unwrap_encrypted_content_with_model_id(
        ...
        """
        if not wrapped_content.startswith("derouter_enc:"):
        ...
        try:
            ...
            metadata_b64 = parts[0].replace("derouter_enc:", "")
    ...
    def _restore_encrypted_content_item_ids_in_input(request_input: _RequestInputT) -> _RequestInputT:
        """Decode derouter-encoded item IDs in request input back to original IDs.
        ...
        2. Items with wrapped encrypted_content (derouter_enc:...)
        """
    ...
    def _build_responses_api_response_id(
        ...
        assembled_id: Final[str] = str(SpecialEnums.DEROUTER_MANAGED_RESPONSE_COMPLETE_STR.value).format(
    ...
    def _decode_responses_api_response_id(
        ...
        try:
            ...
            # Format: derouter:custom_llm_provider:{};model_id:{};response_id:{}
            custom_llm_provider = None
            model_id = None
            ...
            if len(parts) >= 3:  # Full format with custom_llm_provider, model_id, and response_id
                ...
                custom_llm_provider = custom_llm_provider_part.replace("derouter:custom_llm_provider:", "")
    ...
    @staticmethod
    def _is_derouter_encoded_response_id(response_id: str) -> bool:
    ...
    def decode_previous_response_id_to_original_previous_response_id(
        ...
        Why?
            - DeRouter encodes the `custom_llm_provider` and `model_id` into the `previous_response_id` this helps with maintaining session consistency when load balancing multiple deployments of the same model.
            - We cannot send the derouter encoded b64 to the upstream llm api, hence we decode it to the original `previous_response_id`
    ...
    def _build_container_id(
        ...
        Format: cntr_{base64("derouter:custom_llm_provider:{provider};model_id:{model};container_id:{original}")}
        """
        ...
        assembled_id = f"derouter:custom_llm_provider:{provider_part};model_id:{model_part};container_id:{container_id}"
    ...
    def _decode_container_id(container_id: str) -> DecodedResponseId:
        ...
        try:
            ...
            if not decoded_id.startswith("derouter:"):
                return DecodedResponseId(
                    custom_llm_provider=None,
                    model_id=None,
                    response_id=container_id,
            ...
            # Format: derouter:custom_llm_provider:{provider};model_id:{model};container_id:{container}
            # * for provider/model allows empty segments (missing router model_id).
            pattern: Final = r"^derouter:custom_llm_provider:([^;]*);model_id:([^;]*);container_id:(.+)$"
    ...
    def _encode_container_id_on_output_item(
        ...
        """Mutate one output item (dict or object): wrap raw ``container_id`` as DeRouter-managed.
