...
from derouter.types.utils import Message, ModelResponse
...
class _MCPCompletionKwargs(TypedDict, total=False, extra_items=object):
    """Extra keywords forwarded verbatim to ``derouter.acompletion``, which owns their contract."""
...
async def acompletion_with_mcp(
    ...
    **kwargs: Unpack[_MCPCompletionKwargs],  # kwargs-ok: forwarded verbatim to derouter.acompletion, which owns them
    ...
    """
    from derouter import acompletion as derouter_acompletion
    ...
    if not mcp_tools_with_derouter_proxy:
        ...
        return await derouter_acompletion(
            model=model,
            messages=messages,
            tools=tools,
    ...
    openai_tools: Final = DeRouter_Proxy_MCP_Handler._transform_mcp_tools_to_openai(
        deduplicated_mcp_tools,
        target_format="chat",
    )
    ...
    if not should_auto_execute:
        response = await derouter_acompletion(**base_call_args)
    ...
    if stream:
        ...
        initial_stream: Final = await derouter_acompletion(**initial_call_args)
        ...
        class MCPStreamingIterator:
            ...
            def __init__(
                ...
                self.derouter_trace_id = derouter_trace_id
            ...
            async def _drain_inner_stream(self):
                try:
                    ...
                    logging.getLogger("DeRouter").exception(
                ...
                if not self.stream_exhausted:
                    ...
                    try:
                        ...
                        if is_final:
                            ...
                            # _ProxyDBLogger → DeRouter_SpendLogs).  The CSW may
            ...
            async def _process_tool_calls(self):
                ...
                if isinstance(complete_response, ModelResponse):
                    ...
                    self.tool_calls = DeRouter_Proxy_MCP_Handler._extract_tool_calls_from_chat_response(
                        response=complete_response
                    )

                    if self.tool_calls:
                        # Execute tool calls
                        self.tool_results = await DeRouter_Proxy_MCP_Handler._execute_tool_calls(
            ...
            async def _prepare_follow_up_call(self):
                ...
                follow_up_messages: Final = DeRouter_Proxy_MCP_Handler._create_follow_up_messages_for_chat(
                ...
                # Import derouter here to ensure we get the patched version
                # This ensures the patch works correctly in tests
                import derouter

                follow_up_response: Final = await derouter.acompletion(**follow_up_call_args)
    ...
    # Make initial call
    initial_response: Final = await derouter_acompletion(**initial_call_args)

    if not isinstance(initial_response, ModelResponse):
        return initial_response

    # Extract tool calls from response
    tool_calls: Final = DeRouter_Proxy_MCP_Handler._extract_tool_calls_from_chat_response(response=initial_response)
    ...
    response = await derouter_acompletion(**follow_up_call_args)
    if isinstance(response, (ModelResponse, CustomStreamWrapper)):
        _add_mcp_metadata_to_response(
            response=response,
            openai_tools=openai_tools,
