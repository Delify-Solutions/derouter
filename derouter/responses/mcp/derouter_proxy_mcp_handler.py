...
from derouter.types.llms.openai import ToolParam as ResponsesToolParam
...
DEROUTER_PROXY_MCP_SERVER_URL: Final = "derouter_proxy"
DEROUTER_PROXY_MCP_SERVER_URL_PREFIX: Final = f"{DEROUTER_PROXY_MCP_SERVER_URL}/mcp/"
...
def _names_gateway_explicitly(tool: ToolParam) -> bool:
    return (_mcp_server_url(tool) or "").startswith(DEROUTER_PROXY_MCP_SERVER_URL)
...
class DeRouter_Proxy_MCP_Handler:
    """
    Helper class with static methods for MCP integration with Responses API.

    This handles when a user passes mcp server_url="derouter_proxy" in their tools.
    """
    ...
    def _get_parent_request_tags(kwargs: dict[str, Any] | None) -> list[str]:
        ...
        derouter_params: Final = kwargs.get("derouter_params") or kwargs
        proxy_server_request = derouter_params.get("proxy_server_request") or kwargs.get("proxy_server_request") or {}
    ...
    @staticmethod
    def _should_use_derouter_mcp_gateway(tools: Iterable[ToolParam] | None) -> bool:
        """True when a tool may name this gateway: server_url "derouter_proxy..." or an http(s) URL ending in
    ...
    async def _split_mcp_tools(
        ...
        return DeRouter_Proxy_MCP_Handler._parse_mcp_tools(
            [
                {**tool, "server_url": f"{DEROUTER_PROXY_MCP_SERVER_URL_PREFIX}{name}"}
    ...
    async def _get_mcp_tools_from_manager(
        ...
        mcp_tools_with_derouter_proxy: Iterable[Mapping[str, object]] | None,
        ...
        Args:
            ...
            mcp_tools_with_derouter_proxy: ToolParam objects with server_url starting with "derouter_proxy"
        ...
        mcp_servers: Final = [
            server_url.split("/")[-1]
            for _tool in (mcp_tools_with_derouter_proxy or ())
            for server_url in (_tool.get("server_url", "") if isinstance(_tool, dict) else "",)
            if isinstance(server_url, str) and server_url.startswith(DEROUTER_PROXY_MCP_SERVER_URL_PREFIX)
        ]
        ...
        if resolved_toolset_ids and user_api_key_auth is not None:
            user_api_key_auth = await DeRouter_Proxy_MCP_Handler._apply_toolset_permissions(
        ...
        listing: Final = await _get_tools_from_mcp_servers(
            ...
            derouter_trace_id=derouter_trace_id,
    ...
    def _filter_mcp_tools_by_allowed_tools(
        mcp_tools: list[MCPTool], mcp_tools_with_derouter_proxy: Sequence[Mapping[str, object]]
    ) -> list[MCPTool]:
        ...
        for tool_config in mcp_tools_with_derouter_proxy:
    ...
    async def _process_mcp_tools_to_openai_format(
        user_api_key_auth: "UserAPIKeyAuth | None",
        mcp_tools_with_derouter_proxy: Sequence[Mapping[str, object]],
        derouter_trace_id: str | None = None,
        request_tags: list[str] | None = None,
        ...
        Args:
            user_api_key_auth: User authentication info for access control
            mcp_tools_with_derouter_proxy: ToolParam objects with server_url starting with "derouter_proxy"
            derouter_trace_id: Optional trace ID for linking list_mcp_tools spend logs to parent request
        ...
        openai_tools: Final = DeRouter_Proxy_MCP_Handler._transform_mcp_tools_to_openai(deduplicated_mcp_tools)
    ...
    async def _process_mcp_tools_without_openai_transform(
        ...
        """
        if not mcp_tools_with_derouter_proxy:
            return [], {}
        ...
        (
            mcp_tools_fetched,
            allowed_mcp_servers,
        ) = await DeRouter_Proxy_MCP_Handler._get_mcp_tools_from_manager(
        ...
        filtered_mcp_tools: Final = DeRouter_Proxy_MCP_Handler._filter_mcp_tools_by_allowed_tools(
        ...
        (
            deduplicated_mcp_tools,
            tool_server_map,
        ) = DeRouter_Proxy_MCP_Handler._deduplicate_mcp_tools(filtered_mcp_tools, allowed_mcp_servers)
    ...
    def _should_auto_execute_tools(
        ...
        """
        references: Final = list(mcp_tools_with_derouter_proxy or [])
        if not references:
            return False
        ...
        for tool_call in tool_calls:
            ...
            try:
                (
                    ...
                    tool_arguments,
                    tool_call_id,
                ) = DeRouter_Proxy_MCP_Handler._extract_tool_call_details(tool_call)

                if not tool_name:
                    verbose_logger.warning("Tool call missing name: %s", tool_call)
                    continue

                parsed_arguments = DeRouter_Proxy_MCP_Handler._parse_tool_arguments(tool_arguments)
                ...
                tool_logging_call_id = derouter_call_id or str(uuid.uuid4())
                ...
                logging_request_data = {
                    ...
                    "derouter_call_id": tool_logging_call_id,
                ...
                if derouter_trace_id:
                    logging_request_data["derouter_trace_id"] = derouter_trace_id
                if request_tags:
                ...
                logging_request_data["derouter_logging_obj"] = derouter_logging_obj
                ...
                if derouter_logging_obj:
                    ...
                    derouter_logging_obj.model = f"MCP: {tool_name}"
                    derouter_logging_obj.call_type = CallTypes.call_mcp_tool.value
                ...
                if proxy_logging_obj:
                    result = await proxy_logging_obj.post_mcp_call_hook(
                        response=result,
                        request_data=(
                            derouter_logging_obj.model_call_details
                            if derouter_logging_obj
                ...
                if derouter_logging_obj:
                    try:
                        derouter_logging_obj.post_call(original_response=result)
                        await derouter_logging_obj.async_post_mcp_tool_call_hook(
                            kwargs=derouter_logging_obj.model_call_details,
                            response_obj=result,
                            start_time=start_time,
                ...
                result_text = DeRouter_Proxy_MCP_Handler._parse_mcp_result(result)
                tool_results.append(
                    {
                        "tool_call_id": tool_call_id,
                        "result": result_text,
                ...
                await DeRouter_Proxy_MCP_Handler._log_mcp_tool_failure(
                    proxy_logging_obj=proxy_logging_obj,
                    user_api_key_auth=typed_user_api_key_auth,
    ...
    def _create_follow_up_messages_for_chat(
        ...
        from derouter.utils import convert_list_message_to_dict
    ...
    def _create_mcp_streaming_response(
        ...
        mcp_tools_with_derouter_proxy: list[Mapping[str, object]],
        ...
        **kwargs,
        ...
        request_params: Final = DeRouter_Proxy_MCP_Handler._build_request_params(
            input=input,
            model=model,
            all_tools=all_tools,
        ...
        return MCPEnhancedStreamingIterator(
            ...
            or kwargs.get("derouter_metadata", {}).get("user_api_key_auth"),
    ...
    def _create_tool_execution_events(
        ...
        """
        from derouter._uuid import uuid
        from derouter.responses.mcp.mcp_streaming_iterator import create_mcp_call_events
    ...
    def _add_mcp_output_elements_to_response(
        ...
        from derouter._uuid import uuid
        from derouter.types.responses.main import GenericResponseOutputItem, OutputText
