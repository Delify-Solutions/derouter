...
if TYPE_CHECKING:
    ...
    from derouter.responses.mcp.derouter_proxy_mcp_handler import MCPToolResult
else:
    MCPTool = Any
...
async def create_mcp_list_tools_events(
    ...
    try:
        ...
        _mcp_servers: Final = [
            server_url.split("/")[-1]
            for tool in mcp_tools_with_derouter_proxy
            if isinstance(tool, dict)
            and "server_url" in tool
            and isinstance(server_url := tool.get("server_url"), str)
            and server_url.startswith("derouter_proxy/mcp/")
        ]
        ...
        from derouter.types.llms.openai import OutputItemDoneEvent

        # Extract server label from the first MCP tool config
        server_label = ""
        if mcp_tools_with_derouter_proxy:
            first_tool: Final = mcp_tools_with_derouter_proxy[0]
...
class MCPEnhancedStreamingIterator(BaseResponsesAPIStreamingIterator):
    ...
    def __init__(
        ...
        mcp_tools_with_derouter_proxy: Sequence[Mapping[str, object]] | None = None,
        user_api_key_auth: "UserAPIKeyAuth | None" = None,
        original_request_params: dict[str, Any] | None = None,
    ):
        # MCP setup
        self.mcp_tools_with_derouter_proxy = mcp_tools_with_derouter_proxy or []
        ...
        self.derouter_metadata = {}
        self.custom_llm_provider = self.original_request_params.get("custom_llm_provider", None)
        self.derouter_call_id = self.original_request_params.get("derouter_call_id")
        self.derouter_trace_id = self.original_request_params.get("derouter_trace_id")
    ...
    def _should_auto_execute_tools(self) -> bool:
        ...
        return DeRouter_Proxy_MCP_Handler._should_auto_execute_tools(self.mcp_tools_with_derouter_proxy)
    ...
    async def _create_initial_response_iterator(self) -> None:
        ...
        try:
            ...
            if hasattr(response, "__aiter__") or hasattr(response, "__iter__"):
                ...
                self.derouter_metadata = getattr(response, "derouter_metadata", {})
    ...
    async def _generate_tool_execution_events(self) -> None:
        ...
        try:
            ...
            if self.collected_response is not None:
                tool_calls = DeRouter_Proxy_MCP_Handler._extract_tool_calls_from_response(self.collected_response)
            else:
                tool_calls = []
            if not tool_calls:
                return
            ...
            tool_results: Final = await DeRouter_Proxy_MCP_Handler._execute_tool_calls(
                ...
                request_tags=DeRouter_Proxy_MCP_Handler._get_parent_request_tags(self.original_request_params),
            )
                ...
                output_item_done_event = OutputItemDoneEvent(
                    ...
                    item=BaseDeRouterOpenAIResponseObject(
                        **{
                            ...
                            "name": tool_name,
                            "output": result_text,
                            "server_label": "derouter",  # or extract from tool config
                        }
                    ),
                )
    ...
    async def _create_follow_up_iterator(self) -> None:
        ...
        try:
            ...
            if self.collected_response is not None:
                persistence_disabled: Final = DeRouter_Proxy_MCP_Handler._is_persistence_disabled(
