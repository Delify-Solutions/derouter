...
from derouter.responses.derouter_completion_transformation.custom_tools import (
...
from derouter.responses.streaming_iterator import ResponsesAPIStreamingIterator
...
from derouter.types.utils import Delta as ChatCompletionDelta
...
class DeRouterCompletionStreamingIterator(ResponsesAPIStreamingIterator):
    """
    ...
    def __init__(
        self,
        model: str,
        derouter_custom_stream_wrapper: derouter.CustomStreamWrapper,
        request_input: str | ResponseInputParam,
        responses_api_request: ResponsesAPIOptionalRequestParams,
        custom_llm_provider: str | None = None,
        derouter_metadata: dict | None = None,
    ):
        self.model: str = model
        self.derouter_custom_stream_wrapper: derouter.CustomStreamWrapper = derouter_custom_stream_wrapper
        ...
        self.derouter_metadata: dict | None = derouter_metadata or {}
        ...
        self.derouter_logging_obj = derouter_custom_stream_wrapper.logging_obj
        ...
        self.derouter_model_response: ModelResponse | TextCompletionResponse | None = None
        ...
        self._pending_tool_events: list[BaseDeRouterOpenAIResponseObject] = []
        ...
        self._pending_response_events: list[BaseDeRouterOpenAIResponseObject] = []
        ...
        self._namespace_tool_names = DeRouterCompletionResponsesConfig.namespace_tool_name_map(
        ...
        for tc in tool_calls:
            ...
            if call_id not in self._tool_args_by_call_id:
                ...
                event = OutputItemAddedEvent(
                    ...
                    item=BaseDeRouterOpenAIResponseObject(**item_kwargs),
                )
            ...
            if fn_args_delta:
                ...
                for i in range(0, len(fn_args_delta), chunk_size):
                    ...
                    self._sequence_number += 1
                    delta_event: BaseDeRouterOpenAIResponseObject = FunctionCallArgumentsDeltaEvent(
                        type=ResponsesAPIStreamEvents.FUNCTION_CALL_ARGUMENTS_DELTA,
                        item_id=self._tool_item_id_by_call_id.get(call_id, call_id),
                        output_index=output_index,
                        delta=delta_chunk,
                    )
                    # Add sequence_number as extra field (BaseDeRouterOpenAIResponseObject allows extra fields)
                    delta_event.__dict__["sequence_number"] = self._sequence_number
                    self._pending_tool_events.append(delta_event)

    def _queue_final_tool_call_done_events(self, derouter_complete_object: ModelResponse) -> None:
        """
        ...
        try:
            message: Final = derouter_complete_object.choices[0].message
    ...
    async def _aprime_response_id(self) -> None:
        ...
        while True:
            try:
                chunk = await self.derouter_custom_stream_wrapper.__anext__()
    ...
    def _prime_response_id(self) -> None:
        ...
        while True:
            try:
                chunk = self.derouter_custom_stream_wrapper.__next__()
    ...
    def _with_encoded_response_id(self, response: ResponsesAPIResponse) -> ResponsesAPIResponse:
        return ResponsesAPIRequestUtils._update_responses_api_response_id_with_model_id(
            ...
            derouter_metadata=self.derouter_metadata,
        )
    ...
    def _default_response_created_event_data(self) -> dict:
        ...
        if "tool_choice" in self.responses_api_request:
            ...
            response_created_event_data["tool_choice"] = (
                DeRouterCompletionResponsesConfig._transform_tool_choice(self.responses_api_request["tool_choice"])
                or "auto"
            )
        else:
    ...
    def create_output_item_added_event(self) -> OutputItemAddedEvent:
        ...
        event: Final = OutputItemAddedEvent(
            type=ResponsesAPIStreamEvents.OUTPUT_ITEM_ADDED,
            output_index=0,
            item=BaseDeRouterOpenAIResponseObject(
                **{
    ...
    def create_content_part_added_event(self) -> ContentPartAddedEvent:
        ...
        event: Final = ContentPartAddedEvent(
            ...
            output_index=0,
            content_index=0,
            part=BaseDeRouterOpenAIResponseObject(**{"type": "output_text", "text": "", "annotations": []}),
        )
        event.__dict__["sequence_number"] = self._sequence_number
        return event
    ...
    def create_derouter_model_response(self) -> ModelResponse | None:
        response: Final = cast(
            ModelResponse | None,
            stream_chunk_builder(
                chunks=self.collected_chat_completion_chunks,
                logging_obj=self.derouter_logging_obj,
            ),
        )
    ...
    def create_reasoning_summary_part_done_event(
        ...
        return ReasoningSummaryPartDoneEvent(
            ...
            sequence_number=sequence_number,
            summary_index=0,
            part=BaseDeRouterOpenAIResponseObject(
                **{
                    "type": "summary_text",
                    "text": reasoning_content,
                }
    ...
    def create_output_text_done_event(self, derouter_complete_object: ModelResponse) -> OutputTextDoneEvent:
        if self._cached_item_id is None:
            self._cached_item_id = f"msg_{uuid.uuid4()}"
        ...
        return OutputTextDoneEvent(
            ...
            output_index=0,
            content_index=0,
            text=getattr(derouter_complete_object.choices[0].message, "content", "") or "",
        )

    def create_output_content_part_done_event(self, derouter_complete_object: ModelResponse) -> ContentPartDoneEvent:
        if self._cached_item_id is None:
            self._cached_item_id = f"msg_{uuid.uuid4()}"

        text: Final = getattr(derouter_complete_object.choices[0].message, "content", "") or ""
        reasoning_content = getattr(derouter_complete_object.choices[0].message, "reasoning_content", "") or ""
        annotations: Final = getattr(derouter_complete_object.choices[0].message, "annotations", None)
    ...
    def create_output_item_done_event(self, derouter_complete_object: ModelResponse) -> OutputItemDoneEvent:
        if self._cached_item_id is None:
            self._cached_item_id = f"msg_{uuid.uuid4()}"

        text: Final = self.derouter_model_response.choices[0].message.content or ""
        annotations = getattr(self.derouter_model_response.choices[0].message, "annotations", None)
    ...
    def return_default_done_events(
        self, derouter_complete_object: ModelResponse
    ) -> BaseDeRouterOpenAIResponseObject | None:
        if self.sent_output_text_done_event is False:
            self.sent_output_text_done_event = True
            return self.create_output_text_done_event(derouter_complete_object)
        if self.sent_output_content_part_done_event is False:
            self.sent_output_content_part_done_event = True
            return self.create_output_content_part_done_event(derouter_complete_object)
        if self.sent_output_item_done_event is False:
            self.sent_output_item_done_event = True
            return self.create_output_item_done_event(derouter_complete_object)
        return None
    ...
    def common_done_event_logic(self, sync_mode: bool = True) -> BaseDeRouterOpenAIResponseObject:
        if not self.derouter_model_response or isinstance(self.derouter_model_response, TextCompletionResponse):
            self.derouter_model_response = self.create_derouter_model_response()
        if self.derouter_model_response:
            # If tool calls exist, emit tool events before finishing/response.completed.
            if isinstance(self.derouter_model_response, ModelResponse):
                self._queue_final_tool_call_done_events(self.derouter_model_response)
            if self._pending_tool_events:
                return self._pending_tool_events.pop(0)

            done_event: Final = self.return_default_done_events(self.derouter_model_response)
            if done_event:
                return done_event
        else:
            if sync_mode:
        ...
        response_completed_event: Final = self._emit_response_completed_event(self.derouter_model_response)
    ...
    async def __anext__(
        self,
    ) -> ResponsesAPIStreamingResponse | ResponseCompletedEvent | BaseDeRouterOpenAIResponseObject:
        try:
            while True:
        ...
        if chunk.choices and hasattr(chunk.choices[0].delta, "annotations"):
            ...
            if annotations and self.sent_annotation_events is False:
                ...
                if not hasattr(self, "_pending_annotation_events"):
                    response_annotations = DeRouterCompletionResponsesConfig._transform_chat_completion_annotations_to_response_output_annotations(
                        annotations=annotations
                    )
    ...
    def _get_delta_string_from_streaming_choices(self, choices: list[StreamingChoices]) -> str:
        ...
        It's unclear how users expect derouter to translate multiple-choices-per-chunk to the responses API output.
        """
    ...
    def _emit_response_completed_event(self, derouter_model_response: ModelResponse) -> ResponseCompletedEvent | None:
        if derouter_model_response:
            ...
            responses_api_response: Final = (
                DeRouterCompletionResponsesConfig.transform_chat_completion_response_to_responses_api_response(
                    ...
                    chat_completion_response=derouter_model_response,
