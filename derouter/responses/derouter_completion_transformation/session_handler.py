...
from derouter.proxy._types import SpendLogsMetadata, SpendLogsPayload
...
from derouter.types.utils import ChatCompletionMessageToolCall, Message, ModelResponse
...
def _normalize_redacted_tool_call_arguments(message: Message) -> None:
    ...
    for tool_call in message.tool_calls or []:
        if (function := getattr(tool_call, "function", None)) is not None and function.arguments == REDACTED_BY_DEROUTER:
            function.arguments = REDACTED_TOOL_CALL_ARGUMENTS_PLACEHOLDER
    function_call: Final = message.function_call
    if function_call is not None and function_call.arguments == REDACTED_BY_DEROUTER:
...
class ResponsesSessionHandler:
    ...
    async def get_chat_completion_message_history_for_previous_response_id(
        ...
        if len(all_spend_logs) > 0:
            derouter_session_id = all_spend_logs[0].get("session_id")
    ...
    async def extend_chat_completion_message_with_spend_log_payload(
        ...
        if response_input_param:
            chat_completion_messages = DeRouterCompletionResponsesConfig.transform_responses_api_input_to_messages(
    ...
    def _should_check_cold_storage_for_full_payload(
        ...
        1. `DEROUTER_TRUNCATED_PAYLOAD_FIELD` is in the proxy server request dict
        2. `derouter.cold_storage_custom_logger` is not None
        """
        from derouter.constants import DEROUTER_TRUNCATED_PAYLOAD_FIELD

        configured_cold_storage_custom_logger: Final = derouter.cold_storage_custom_logger
        ...
        if len(proxy_server_request_dict) == 0:
            return True
        if DEROUTER_TRUNCATED_PAYLOAD_FIELD in str(proxy_server_request_dict):
            return True
        return False
    ...
    async def get_all_spend_logs_for_previous_response_id(
        ...
        from derouter.proxy.proxy_server import disable_spend_logs, prisma_client
