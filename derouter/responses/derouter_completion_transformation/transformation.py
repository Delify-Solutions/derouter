"""
Handles transforming from Responses API -> DeRouter completion  (Chat Completion API)
"""
...
from derouter.caching import InMemoryCache
from derouter.constants import REDACTED_BY_DEROUTER, REDACTED_TOOL_CALL_ARGUMENTS_PLACEHOLDER
...
from derouter.responses.derouter_completion_transformation.session_handler import (
...
from derouter.types.responses.main import (
...
class ChatCompletionSession(TypedDict, total=False):
    ...
    derouter_session_id: str | None


########### End of Initialize Classes used for Responses API  ###########


class DeRouterCompletionResponsesConfig:
    @staticmethod
    def get_supported_openai_params(model: str) -> list:
        """
        DeRouter Adapter from OpenAI Responses API to Chat Completion API supports a subset of OpenAI Responses API params
        """
        return [
            "input",
            "model",
    ...
    def transform_responses_api_request_to_chat_completion_request(
        ...
        **kwargs,
        ...
        (
            tools,
            web_search_options,
        ) = DeRouterCompletionResponsesConfig.transform_responses_api_tools_to_chat_completion_tools(
            responses_api_request.get("tools") or []
        )

        if web_search_options is not None and DeRouterCompletionResponsesConfig._should_drop_derived_web_search_options(
            model=model, custom_llm_provider=custom_llm_provider
        ):
        ...
        if text_param:
            response_format = DeRouterCompletionResponsesConfig._transform_text_format_to_response_format(text_param)
        ...
        derouter_completion_request: dict = {
            "messages": DeRouterCompletionResponsesConfig.transform_responses_api_input_to_messages(
                input=input,
                responses_api_request=responses_api_request,
                replay_reasoning=True,
            ),
            "model": model,
            "tool_choice": DeRouterCompletionResponsesConfig._transform_tool_choice(
        ...
        if not tools:
            derouter_completion_request.pop("tool_choice", None)
            derouter_completion_request.pop("tools", None)

        # Responses API `Completed` events require usage, we pass `stream_options` to derouter.completion to include usage
        if stream is True:
            stream_options: Final = {
                "include_usage": True,
            }
            derouter_completion_request["stream_options"] = stream_options
            derouter_logging_obj: Final[DeRouterLoggingObj | None] = kwargs.get("derouter_logging_obj")
            if derouter_logging_obj:
                derouter_logging_obj.stream_options = stream_options

        # only pass non-None values
        derouter_completion_request = {k: v for k, v in derouter_completion_request.items() if v is not None}
        return derouter_completion_request
    ...
    def transform_responses_api_input_to_messages(
        ...
        if responses_api_request.get("instructions"):
            messages.append(
                DeRouterCompletionResponsesConfig.transform_instructions_to_system_message(
                    responses_api_request.get("instructions")
                )
            )

        messages.extend(
            DeRouterCompletionResponsesConfig._transform_response_input_param_to_chat_completion_message(
                input=input,
    ...
    async def async_responses_api_session_handler(
        ...
        """
        chat_completion_session = ChatCompletionSession(messages=[], derouter_session_id=None)
        ...
        _messages: Final = derouter_completion_request.get("messages") or []
        ...
        tools: Final = derouter_completion_request.get("tools") or []
        combined_messages = DeRouterCompletionResponsesConfig._ensure_tool_results_have_corresponding_tool_calls(
        ...
        if not combined_messages:
            ...
            if original_new_messages:
                ...
                raise derouter.BadRequestError(
                    ...
                    model=derouter_completion_request.get("model", ""),
                    llm_provider=derouter_completion_request.get("custom_llm_provider", ""),
                )

        derouter_completion_request["messages"] = combined_messages
        derouter_completion_request["derouter_trace_id"] = chat_completion_session.get("derouter_session_id")
        return derouter_completion_request
    ...
    def _transform_response_input_param_to_chat_completion_message(
        ...
        if isinstance(input, str):
            ...
            for _input in input:
                chat_completion_messages = (
                    DeRouterCompletionResponsesConfig._transform_responses_api_input_item_to_chat_completion_message(
                        input_item=_input,
                        replay_reasoning=replay_reasoning,
                    )
                )

                if DeRouterCompletionResponsesConfig._is_input_item_function_call(input_item=_input):
                        ...
                        if last_role == "assistant":
                            for new_msg in chat_completion_messages:
                                ...
                                if new_role == "assistant":
                                    ...
                                    for tc in new_tcs:
                                        DeRouterCompletionResponsesConfig._add_tool_call_to_assistant(last_msg, tc)
                            continue
                ...
                #########################################################
                if DeRouterCompletionResponsesConfig._is_input_item_tool_call_output(input_item=_input):
                ...
                merged_assistant = DeRouterCompletionResponsesConfig._merged_trailing_assistant_message(
                    messages=messages,
        ...
        if not replay_reasoning:
            return messages
        return DeRouterCompletionResponsesConfig._merge_reasoning_only_assistant_messages(messages)
    ...
    def _merge_reasoning_only_assistant_messages(
        ...
        _standalone: Final = DeRouterCompletionResponsesConfig._reasoning_only_assistant_message
    ...
    def _create_tool_call_chunk(
        ...
        function_raw = DeRouterCompletionResponsesConfig._get_mapping_or_attr_value(tool_use_definition, "function")
        function_name_raw: Final = DeRouterCompletionResponsesConfig._get_mapping_or_attr_value(function_raw, "name")
        function_arguments_raw = DeRouterCompletionResponsesConfig._get_mapping_or_attr_value(function_raw, "arguments")
        function: Final[dict[str, object]] = {
            "name": function_name_raw or "",
            "arguments": function_arguments_raw or "{}",
        }
        tool_use_id_raw: Final = DeRouterCompletionResponsesConfig._get_mapping_or_attr_value(tool_use_definition, "id")
        tool_use_id: Final[str] = str(tool_use_id_raw) if tool_use_id_raw is not None else str(tool_call_id)
        tool_use_type_raw = DeRouterCompletionResponsesConfig._get_mapping_or_attr_value(tool_use_definition, "type")
    ...
    def _normalize_tool_use_definition(tool_use_definition: object, tool_call_id: str) -> dict[object, object] | None:
        ...
        if isinstance(tool_use_definition, dict):
            ...
            tool_use_id_raw = DeRouterCompletionResponsesConfig._get_mapping_or_attr_value(tool_use_definition, "id")
            tool_use_type_raw = DeRouterCompletionResponsesConfig._get_mapping_or_attr_value(tool_use_definition, "type")
            function_raw = DeRouterCompletionResponsesConfig._get_mapping_or_attr_value(tool_use_definition, "function")
        ...
        if function_raw is not None and not isinstance(function_raw, dict):
            function_name_raw: Final = DeRouterCompletionResponsesConfig._get_mapping_or_attr_value(function_raw, "name")
            function_arguments_raw: Final = DeRouterCompletionResponsesConfig._get_mapping_or_attr_value(
    ...
    def _ensure_tool_results_have_corresponding_tool_calls(
        ...
        for i, message in enumerate(fixed_messages):
            ...
            prev_assistant_idx = DeRouterCompletionResponsesConfig._find_previous_assistant_idx(fixed_messages, i)
            ...
            if not tool_call_id and prev_assistant_idx is not None:
                ...
                tool_call_id = DeRouterCompletionResponsesConfig._recover_tool_call_id_from_assistant(
            ...
            if prev_assistant_idx is not None and tool_call_id:
                prev_assistant = fixed_messages[prev_assistant_idx]
                tool_calls = DeRouterCompletionResponsesConfig._get_tool_calls_list(prev_assistant)

                if not DeRouterCompletionResponsesConfig._check_tool_call_exists(tool_calls, tool_call_id):
                    _tool_use_definition: object = TOOL_CALLS_CACHE.get_cache(key=tool_call_id)

                    if not _tool_use_definition and tools:
                        _tool_use_definition = DeRouterCompletionResponsesConfig._reconstruct_tool_call_from_tools(
                            tool_call_id, tools
                        )

                    normalized_tool_use_definition = DeRouterCompletionResponsesConfig._normalize_tool_use_definition(
                        _tool_use_definition, tool_call_id
                    )

                    if normalized_tool_use_definition:
                        tool_call_chunk = DeRouterCompletionResponsesConfig._create_tool_call_chunk(
                            normalized_tool_use_definition,
                            tool_call_id,
                            len(tool_calls),
                        )
                        DeRouterCompletionResponsesConfig._add_tool_call_to_assistant(prev_assistant, tool_call_chunk)
    ...
    def _transform_responses_api_input_item_to_chat_completion_message(
        ...
        - ItemReference
        """
        if DeRouterCompletionResponsesConfig._is_input_item_tool_call_output(input_item):
            # handle executed tool call results
            return (
                DeRouterCompletionResponsesConfig._transform_responses_api_tool_call_output_to_chat_completion_message(
                    tool_call_output=input_item
                )
            )
        elif DeRouterCompletionResponsesConfig._is_input_item_function_call(input_item):
            # handle function call input items
            return DeRouterCompletionResponsesConfig._transform_responses_api_function_call_to_chat_completion_message(
            ...
            if not replay_reasoning:
                ...
                inspectable: Final[object] = (
                    ...
                    if DeRouterCompletionResponsesConfig._reasoning_text_from_content(input_item) is not None
                    else DeRouterCompletionResponsesConfig._reasoning_text_from_summary(input_item)
                ...
                return [  # mutable-ok: single message result
                    GenericChatCompletionMessage(
                        ...
                        content=DeRouterCompletionResponsesConfig._transform_responses_api_content_to_chat_completion_content(
                            inspectable
                        ),
                    )
                ]
            reasoning_text = DeRouterCompletionResponsesConfig._extract_reasoning_text_from_input_item(  # rebind-ok: extraction result
                input_item
            )
            thinking_blocks = DeRouterCompletionResponsesConfig._decode_thinking_blocks_from_input_item(  # rebind-ok: extraction result
                input_item
            )
            if not reasoning_text and not thinking_blocks:
                return []  # mutable-ok: empty drop result
            return [  # mutable-ok: single message result
                DeRouterCompletionResponsesConfig._reasoning_only_assistant_message(
    ...
    def _reasoning_text_from_summary(input_item: Mapping[str, object]) -> str | None:
        ...
        Guardrail traversal in derouter/proxy/guardrails/_content_utils.py
    ...
    def _extract_reasoning_text_from_input_item(input_item: Mapping[str, object]) -> str | None:
        ...
        """
        return DeRouterCompletionResponsesConfig._reasoning_text_from_content(
            input_item
        ) or DeRouterCompletionResponsesConfig._reasoning_text_from_summary(input_item)
    ...
    def _decode_thinking_blocks_from_input_item(
        ...
        DeRouter writes this field itself for providers whose reasoning is signed
        ...
        blocks: Final = tuple(
            ...
            if isinstance(block, Mapping) and DeRouterCompletionResponsesConfig._is_replayable_thinking_block(block)
        )
    ...
    def _transform_responses_api_function_call_to_chat_completion_message(
        ...
        if raw_arguments == REDACTED_BY_DEROUTER:
    ...
    def _transform_input_file_item_to_file_item(item: Mapping[str, object]) -> dict[str, object]:
        ...
        file_id: Final = DeRouterCompletionResponsesConfig._resolve_file_id(item)
        if file_id:
            ...
            for item in content:
                if isinstance(item, str):
                    ...
                    if item.get("type") == "input_file":
                        content_list.append(
                            DeRouterCompletionResponsesConfig._transform_input_file_item_to_file_item(item)
                        )
                    elif item.get("type") == "input_image":
                        image_block = _STR_KEY_DICT_ADAPTER.validate_python(
                            dict(DeRouterCompletionResponsesConfig._transform_input_image_item_to_image_item(item))
                        )
                        if "cache_control" in item:
                        ...
                        content_block: dict[str, object] = {
                            "type": DeRouterCompletionResponsesConfig._get_chat_completion_request_content_type(
    ...
    def _namespace_chat_tools(tool: NamespaceTool) -> tuple[ChatCompletionToolParam, ...]:
        ...
        if isinstance(namespace_tools, Sequence) and not isinstance(namespace_tools, (str, bytes)):
            return tuple(
                ...
                if (
                    chat_tool := DeRouterCompletionResponsesConfig._build_ns_chat_tool(
                        namespace,
        ...
        flat_tool: Final = DeRouterCompletionResponsesConfig._build_ns_chat_tool(
    ...
    def _responses_tool_to_chat_form(tool: Mapping[str, object]) -> ResponsesToolChatForm:
        ...
        if tool_type == "namespace":
            return ResponsesToolChatForm(
                chat_tools=DeRouterCompletionResponsesConfig._namespace_chat_tools(tool), web_search_options=None
            )
        if tool_type == "custom":
    ...
    def responses_tools_to_chat_forms(tools: ResponseTools) -> tuple[ResponsesToolChatForm, ...]:
        DeRouterCompletionResponsesConfig._validate_namespace_name_collisions(tools)
        return tuple(DeRouterCompletionResponsesConfig._responses_tool_to_chat_form(tool) for tool in tools or ())
    ...
    def transform_responses_api_tools_to_chat_completion_tools(
        ...
        if tools is None:
            return [], None
        forms: Final = DeRouterCompletionResponsesConfig.responses_tools_to_chat_forms(tools)
    ...
    def transform_chat_completion_tools_to_responses_tools(
        ...
        namespace_tool_names: Final = DeRouterCompletionResponsesConfig.namespace_tool_name_map(request_tools)
        ...
        for tool in all_chat_completion_tools:
            if tool.type == "function":
                ...
                if is_custom_tool_call(tool_name, custom_tool_names):
                    ...
                    restore_name = DeRouterCompletionResponsesConfig._restore_namespace_tool_name
    ...
    def convert_response_function_tool_call_to_chat_completion_tool_call(
        ...
        tool_call_dict: Final[dict[str, object]] = {
            "id": DeRouterCompletionResponsesConfig._tool_call_id_from_responses_item(
    ...
    def transform_chat_completion_response_to_responses_api_response(
        ...
        responses_api_response: Final[ResponsesAPIResponse] = ResponsesAPIResponse(
            ...
            output=DeRouterCompletionResponsesConfig._transform_chat_completion_choices_to_responses_output(
            ...
            reasoning=None,
            status=DeRouterCompletionResponsesConfig._map_chat_completion_finish_reason_to_responses_status(
                finish_reason
            ),
            text={},
            truncation=getattr(chat_completion_response, "truncation", None),
            usage=DeRouterCompletionResponsesConfig._transform_chat_completion_usage_to_responses_usage(
                chat_completion_response=chat_completion_response
            ),
    ...
    def _transform_chat_completion_choices_to_responses_output(
        ...
        responses_output.extend(
            DeRouterCompletionResponsesConfig._extract_reasoning_output_items(chat_completion_response, choices)
        )
        responses_output.extend(
            DeRouterCompletionResponsesConfig._extract_message_output_items(chat_completion_response, choices)
        )
        responses_output.extend(
            DeRouterCompletionResponsesConfig.transform_chat_completion_tools_to_responses_tools(
                chat_completion_response=chat_completion_response,
                responses_api_request=responses_api_request,
            )
        )
        ...
        tool_result_items = DeRouterCompletionResponsesConfig._extract_tool_result_output_items(chat_completion_response)
    ...
    def _extract_reasoning_output_items(
        ...
        for choice in choices:
            if hasattr(choice, "message") and choice.message:
                ...
                encrypted_content = DeRouterCompletionResponsesConfig._encode_thinking_blocks(message)
    ...
    def _extract_image_generation_output_items(
        ...
        for image_item in _DICT_ITEMS_LIST_ADAPTER.validate_python(images):
            ...
            base64_data = DeRouterCompletionResponsesConfig._extract_base64_from_data_url(image_url)
            ...
            if base64_data:
                image_generation_items.append(
                    OutputImageGenerationCall(
                        type="image_generation_call",
                        id=f"ig_{uuid.uuid4()}",
                        status=DeRouterCompletionResponsesConfig._map_finish_reason_to_image_generation_status(
                            choice.finish_reason
                        ),
                        result=base64_data,
                    )
    ...
    def _extract_message_output_items(
        ...
        for choice in choices:
            ...
            if hasattr(choice.message, "images") and choice.message.images:
                ...
                image_generation_items = DeRouterCompletionResponsesConfig._extract_image_generation_output_items(
                    choice=choice,
                )
                ...
                message_output_items.append(
                    GenericResponseOutputItem(
                        ...
                        content=[
                            DeRouterCompletionResponsesConfig._transform_chat_message_to_response_output_text(
                                choice.message
                            )
                        ],
                    )
    ...
    def _transform_responses_api_outputs_to_chat_completion_messages(
        ...
        for _output_item in output_items:
            ...
            if output_item.get("type") == "function_call":
                ...
                messages.append(
                    DeRouterCompletionResponsesConfig._transform_responses_output_tool_call_to_chat_completion_output_tool_call(
    ...
    def _transform_chat_message_to_response_output_text(
        ...
        transformed_annotations: Final = (
            DeRouterCompletionResponsesConfig._transform_chat_completion_annotations_to_response_output_annotations(
