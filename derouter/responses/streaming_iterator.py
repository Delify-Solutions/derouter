...
from derouter.constants import (
    EMPTY_MAPPING,
    DEROUTER_MAX_STREAMING_DURATION_SECONDS,
...
from derouter.responses.utils import ResponseAPILoggingUtils, ResponsesAPIRequestUtils
...
from derouter.utils import async_post_call_success_deployment_hook

if TYPE_CHECKING:
    from derouter.caching.caching_handler import LLMCachingHandler
    from derouter.proxy._types import UserAPIKeyAuth
    from derouter.types.responses.streaming_websocket import (
...
class _GetsLitellmParams(Protocol):
    def __call__(self, key: str, default: Mapping[str, object], /) -> DeRouter_Params: ...
...
def _typed_gets_derouter_params(fn: _GetsLitellmParams) -> _GetsLitellmParams:
    return fn
...
def _model_id_from_metadata(derouter_metadata: dict[str, object] | None) -> str | None:
    model_info: Final = derouter_metadata.get("model_info") if derouter_metadata else None
...
class BaseResponsesAPIStreamingIterator:
    ...
    def __init__(
        ...
        self.derouter_metadata = derouter_metadata
        self.custom_llm_provider = custom_llm_provider
        self.request_data: dict[str, object] = request_data or {}
        self.call_type: str | None = call_type

        # set hidden params for response headers (e.g., x-derouter-model-id)
        # This matches the stream wrapper in derouter/derouter_core_utils/streaming_handler.py
        _api_base: Final = get_api_base(
            model=model or "",
            optional_params=_typed_gets_derouter_params(self.logging_obj.model_call_details.get)("derouter_params", {}),
        )
        self._hidden_params: dict[str, object] = {
            "model_id": _model_id_from_metadata(derouter_metadata),
            "api_base": _api_base,
    ...
    def _check_max_streaming_duration(self) -> None:
        """Raise derouter.Timeout if the stream has exceeded DEROUTER_MAX_STREAMING_DURATION_SECONDS."""
        if DEROUTER_MAX_STREAMING_DURATION_SECONDS is None:
            return
        elapsed: Final = time.time() - self._stream_created_time
        if elapsed > DEROUTER_MAX_STREAMING_DURATION_SECONDS:
            raise derouter.Timeout(
                message=f"Stream exceeded max streaming duration of {DEROUTER_MAX_STREAMING_DURATION_SECONDS}s (elapsed {elapsed:.1f}s)",
                model=self.model or "",
    ...
    def _process_chunk(self, chunk: str) -> ResponsesAPIStreamingResponse | None:
        ...
        try:
            ...
            if _is_json_object(parsed_chunk):
                ...
                _stream_model_id: Final = _model_id_from_metadata(self.derouter_metadata)
                if _event_type in (
                ...
                if self.derouter_metadata and self.derouter_metadata.get("encrypted_content_affinity_enabled"):
                    ...
                    if event_type in (
                        ...
                        if item:
                            ...
                            if encrypted_content and isinstance(encrypted_content, str):
                                model_id: Final = _model_id_from_metadata(self.derouter_metadata)
                                if model_id:
    ...
    def _handle_logging_failed_response(self):
        ...
        exception: Final = derouter.APIError(
    ...
    def _maybe_raise_for_error_event(self, result: object) -> None:
        ...
        mapped_exception: Final = derouter.APIError(
    ...
    def _persist_completed_response_to_cache(self, *, is_async: bool) -> None:
        ...
        cache: Final = derouter.cache
        if cache is None:
            return

        cached_response: Final = response_obj.model_dump_json()
        if is_async:
            from derouter.caching.caching_handler import create_cache_write_task
    ...
    async def _call_post_streaming_deployment_hook(
        ...
        try:
            ...
            callbacks: Final[Sequence[object]] = getattr(derouter, "callbacks", None) or []
            hooks_ran = False
    ...
    def _run_post_success_hooks(self, end_time: datetime):
        ...
        try:
            ...
            pass
        if "derouter_params" not in request_payload:
            try:
                request_payload["derouter_params"] = getattr(self.logging_obj, "model_call_details", {}).get(
                    "derouter_params", {}
                )
            except Exception:
                request_payload["derouter_params"] = {}
...
def _stamp_responses_usage_cost(
    response_obj: ResponsesAPIResponse | None, logging_obj: DeRouterLoggingObj | None
) -> None:
    ...
    for output_index, output_item in enumerate(output_items):
        ...
        events.append(
            openai_types.OutputItemAddedEvent(
                ...
                output_index=output_index,
                item=openai_types.BaseDeRouterOpenAIResponseObject(**output_item_payload),
            )
        )
            ...
            for content_index, part in enumerate(content_parts):
                ...
                events.append(
                    openai_types.ContentPartAddedEvent(
                        ...
                        output_index=output_index,
                        content_index=content_index,
                        part=openai_types.BaseDeRouterOpenAIResponseObject(**part_payload),
                    )
                )
            ...
            for summary_index, summary in enumerate(summaries):
                ...
                events.append(
                    openai_types.ReasoningSummaryPartDoneEvent(
                        ...
                        sequence_number=sequence_number,
                        summary_index=summary_index,
                        part=openai_types.BaseDeRouterOpenAIResponseObject(**summary_payload),
                    )
                )
...
class ManagedResponsesWebSocketHandler:
    ...
    - Makes HTTP streaming calls via ``derouter.aresponses(stream=True)``
    ...
    This makes every provider that DeRouter can reach over HTTP available on
    ...
    def __init__(
        ...
        logging_obj: DeRouterLoggingObj,
        user_api_key_dict: UserAPIKeyAuth | None = None,
        derouter_metadata: Mapping[str, object] | None = None,
        ...
        **kwargs: object,
        ...
        self.logging_obj = logging_obj
        self.user_api_key_dict = user_api_key_dict
        self.derouter_metadata: Mapping[str, object] = derouter_metadata or {}
        raw_model_group: Final = self.derouter_metadata.get("model_group") or self.derouter_metadata.get(
    ...
    def _get_history_messages(self, previous_response_id: str) -> list[dict[str, object]]:
        ...
        DeRouter base64-encodes it into the ``resp_...`` format).
        """
    ...
    def _inject_credentials(self, call_kwargs: dict[str, object], model: str | None = None) -> None:
        ...
        # (e.g., connection is vertex_ai but event says openai/gpt-4), let derouter
        ...
        if self.derouter_metadata:
            call_kwargs["derouter_metadata"] = dict(self.derouter_metadata)
    ...
    def _update_proxy_request(call_kwargs: dict[str, Any], model: str) -> None:
        ...
        proxy_server_request = (call_kwargs.get("derouter_metadata") or {}).get("proxy_server_request") or {}
        ...
        if "derouter_metadata" not in call_kwargs:
            call_kwargs["derouter_metadata"] = {}
        call_kwargs["derouter_metadata"]["proxy_server_request"] = proxy_server_request
        call_kwargs.setdefault("derouter_params", {})
        call_kwargs["derouter_params"]["proxy_server_request"] = proxy_server_request

    async def _stream_and_forward(self, model: str, call_kwargs: dict[str, Any]) -> _MutableJsonObject | None:
        """
        Stream ``derouter.aresponses`` and forward every chunk over the WebSocket.
        ...
        stream_response: Final = await derouter.aresponses(model=model, **call_kwargs)
    ...
    async def _process_response_create(self, raw_message: str) -> None:
        """
        Parse one ``response.create`` event, call ``derouter.aresponses(stream=True)``,
        ...
        # derouter.aresponses fails in get_llm_provider. A genuinely different
