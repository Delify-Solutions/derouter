...
class LLMResponse(BaseModel):
    ...
    cost: float | None = Field(
        default=None,
        description="Total cost of the LLM call in USD as computed by DeRouter.",
    )
...
class GalileoObserve(CustomLogger):
    ...
    def _format_created_at(dt: object) -> str:
        ...
        if dt.tzinfo is None:
            # DeRouter often passes naive datetimes in local time; convert to UTC
    ...
    def _record_to_v2_span(
        ...
        *,
        ...
        span: Final[dict[str, object]] = {
            ...
            "trace_id": trace_id,
            "parent_id": trace_id,
            "name": record.get("node_type", "derouter"),
    ...
    @staticmethod
    def _get_chat_content_for_galileo(response_obj: derouter.ModelResponse) -> object:
    ...
    def _get_text_completion_content_for_galileo(
        response_obj: derouter.TextCompletionResponse,
    ) -> str | None:
        if response_obj.choices and len(response_obj.choices) > 0:
    ...
    def _get_galileo_input_output_content(
        ...
        if response_obj is not None and (
            call_type in ("embedding", "aembedding") or isinstance(response_obj, derouter.EmbeddingResponse)
        ):
        ...
        if response_obj is not None and isinstance(response_obj, derouter.ModelResponse):
        ...
        if response_obj is not None and isinstance(response_obj, derouter.TextCompletionResponse):
        ...
        if response_obj is not None and isinstance(response_obj, derouter.ImageResponse):
        ...
        if response_obj is not None and isinstance(response_obj, derouter.TranscriptionResponse):
        ...
        if response_obj is not None and isinstance(response_obj, derouter.RerankResponse):
    ...
    async def _async_log_success_event_impl(
        ...
        _call_type: Final[str] = str(slo.get("call_type") or kwargs.get("call_type") or "derouter")
