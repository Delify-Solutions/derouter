...
from derouter.integrations.custom_prompt_management import CustomPromptManagement
from derouter.types.prompts.init_prompts import PromptDeRouterParams, PromptSpec, SupportedPromptIntegrations
...
def set_global_gitlab_config(config: dict) -> None:
    ...
    derouter.global_gitlab_config = config
...
def prompt_initializer(derouter_params: "PromptDeRouterParams", prompt_spec: "PromptSpec") -> "CustomPromptManagement":
    ...
    """
    gitlab_config: Final = getattr(derouter_params, "gitlab_config", None)
...
def _gitlab_prompt_initializer(
    ...
    Expected fields on derouter_params:
    ...
    """
    # You can store arbitrary integration-specific config on PromptDeRouterParams.
    # If your dataclass doesn't have these attributes, add them or put inside
    # `derouter_params.extra` and pull them from there.
    gitlab_config: Final[dict[str, Any]] = getattr(derouter_params, "gitlab_config", None) or {}
    git_ref: Final[str | None] = getattr(derouter_params, "git_ref", None)
