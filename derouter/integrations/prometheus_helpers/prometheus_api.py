...
async def get_fallback_metric_from_prometheus() -> str:
    ...
    relevant_metrics: Final = [
        "derouter_deployment_successful_fallbacks_total",
        "derouter_deployment_failed_fallbacks_total",
    ]
...
async def get_daily_spend_from_prometheus(api_key: str | None) -> list[PrometheusDailySpend]:
    ...
    if api_key is None:
        query = "sum(delta(derouter_spend_metric_total[1d]))"
    else:
        quoted_api_key: Final = _quote_promql_string_literal(api_key)
        query = f"sum(delta(derouter_spend_metric_total{{hashed_api_key={quoted_api_key}}}[1d]))"
