...
class S3Logger:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            verbose_logger.debug("in init s3 logger - s3_callback_params %s", derouter.s3_callback_params)

            s3_use_team_prefix = False

            if derouter.s3_callback_params is not None:
                # read in .env variables - example os.environ/AWS_BUCKET_NAME
                for key, value in derouter.s3_callback_params.items():
                    if isinstance(value, str) and value.startswith("os.environ/"):
                        derouter.s3_callback_params[key] = derouter.get_secret(value)
                # now set s3 params from derouter.s3_logger_params
                s3_bucket_name = derouter.s3_callback_params.get("s3_bucket_name")
                s3_region_name = derouter.s3_callback_params.get("s3_region_name")
                s3_api_version = derouter.s3_callback_params.get("s3_api_version")
                s3_use_ssl = derouter.s3_callback_params.get("s3_use_ssl", True)
                s3_verify = derouter.s3_callback_params.get("s3_verify")
                s3_endpoint_url = derouter.s3_callback_params.get("s3_endpoint_url")
                s3_aws_access_key_id = derouter.s3_callback_params.get("s3_aws_access_key_id")
                s3_aws_secret_access_key = derouter.s3_callback_params.get("s3_aws_secret_access_key")
                s3_aws_session_token = derouter.s3_callback_params.get("s3_aws_session_token")
                s3_config = derouter.s3_callback_params.get("s3_config")
                s3_path = derouter.s3_callback_params.get("s3_path")
                s3_server_side_encryption = derouter.s3_callback_params.get("s3_server_side_encryption")
                s3_sse_kms_key_id = derouter.s3_callback_params.get("s3_sse_kms_key_id")
                # done reading derouter.s3_callback_params
                s3_use_team_prefix = bool(derouter.s3_callback_params.get("s3_use_team_prefix", False))
    ...
    def log_event(self, kwargs, response_obj, start_time, end_time, print_verbose):
        try:
            ...
            team_alias_prefix = ""
            if derouter.enable_preview_features and self.s3_use_team_prefix and team_alias is not None:
                team_alias_prefix = f"{team_alias}/"

            s3_file_name: Final = derouter.utils.get_logging_id(start_time, payload) or ""
