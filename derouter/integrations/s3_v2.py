...
from derouter.constants import DEFAULT_S3_BATCH_SIZE, DEFAULT_S3_FLUSH_INTERVAL_SECONDS
from derouter.integrations.s3 import (
...
class S3Logger(CustomBatchLogger, BaseAWSLLM):
        ...
        try:
            ...
            if s3_callback_params_override is not None:
                ...
                verbose_logger.debug(
                    ...
                    _masker.mask_dict(dict(derouter.s3_callback_params or {})),
                )
    ...
    def _init_s3_params(
        ...
        audit-log instance), otherwise falls back to `derouter.s3_callback_params`.
        Resolves `os.environ/X` markers into a local dict; never mutates the source.
        """
        if params_source is None:
            params_source = derouter.s3_callback_params or {}
        params: Final[dict] = {
            key: (derouter.get_secret(value) if isinstance(value, str) and value.startswith("os.environ/") else value)
    ...
    def create_s3_batch_logging_element(
        ...
        s3_file_name: Final = derouter.utils.get_logging_id(start_time, standard_logging_payload) or ""
        verbose_logger.debug(
