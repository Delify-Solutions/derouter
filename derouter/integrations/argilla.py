...
from derouter.types.integrations.argilla import (
...
class ArgillaLogger(CustomBatchLogger):
    def __init__(
        ...
        **kwargs,
        ...
        if derouter.argilla_transformation_object is None:
            raise Exception("'derouter.argilla_transformation_object' is required, to log your payload to Argilla.")
        self.validate_argilla_transformation_object(derouter.argilla_transformation_object)
        self.argilla_transformation_object = derouter.argilla_transformation_object
        ...
        _batch_size: Final = os.getenv("ARGILLA_BATCH_SIZE", None) or derouter.argilla_batch_size
        if _batch_size:
    ...
    def get_credentials_from_env(
        ...
        _credentials_dataset_name = argilla_dataset_name or os.getenv("ARGILLA_DATASET_NAME") or "derouter-completion"
        if _credentials_dataset_name is None:
            raise Exception("Invalid Argilla Dataset give. Value=None.")
        else:
            dataset_response: Final = derouter.module_level_client.get(
    ...
    def _send_batch(self):
        ...
        try:
            response: Final = derouter.module_level_client.post(
                url=url,
                json=self.log_queue,
                headers=headers,
            )
