...
class MlflowLogger(CustomLogger):
    ...
    def _handle_stream_event(self, kwargs, response_obj, start_time, end_time):
        ...
        if derouter_call_id not in self._stream_id_to_span:
            with self._lock:
                # Check again after acquiring lock
                if derouter_call_id not in self._stream_id_to_span:
                    # Start a new span for the first chunk of the stream
                    span = self._start_span_or_trace(kwargs, start_time)
                    self._stream_id_to_span[derouter_call_id] = span

        # Add chunk as event to the span
        span = self._stream_id_to_span[derouter_call_id]
        ...
        if final_response:
            ...
            try:
                ...
                with self._lock:
                    self._stream_id_to_span.pop(derouter_call_id, None)
    ...
    def _extract_attributes(self, kwargs):
        ...
        With the latest version of derouter, the standard_logging_object contains
    ...
    def _start_span_or_trace(self, kwargs, start_time):
        ...
        span_name: Final = f"derouter-{call_type}"
