"""Rubrik DeRouter Plugin for prompt/response moderation and batch logging."""
...
from derouter.types.llms.openai import AllMessageValues, ChatCompletionToolCallChunk
...
_WEBHOOK_PATH_LOGGING_BATCH: Final = "/v1/derouter/batch"
...
class _LogEventKwargs(TypedDict, total=False):
    ...
    derouter_call_id: ReadOnly[str]


class _HasCallId(Protocol):
    def get(self, key: Literal["derouter_call_id"], /) -> str | None: ...
...
class RubrikLogger(CustomGuardrail, CustomBatchLogger):
    def __init__(
        ...
        **kwargs,
        ...
        # guardrail config. Follow the standard derouter convention: omitted
        ...
        # derouter's ``unified_guardrail.async_post_call_streaming_iterator_hook``.
        ...
        # webhook round-trips. derouter reads this attribute via
        ...
        # ``streaming_buffer_until_moderated`` (derouter >= Delify-Solutions/derouter#31389)
        ...
        # (blocked). On older derouter this attribute is ignored and we fall
    ...
    async def aclose(self):
        ...
        from DeRouter's global HTTP-client cache (``get_async_httpx_client``
        ...
        other logger instance; let DeRouter manage their lifecycle instead.
        """
    ...
    async def apply_guardrail(
        ...
        derouter's guardrail-translation layer normalizes Anthropic and OpenAI
    ...
    async def _moderate_response(
        ...
        # ``derouter_call_id`` -- see ``_correlation_id``.
    ...
    async def _moderate_prompt(
        ...
        if not messages:
            # For non-chat request types (e.g. /v1/completions), derouter
    ...
    def _stash_block_context(
        ...
        - Stashes a reference to ``logging_obj`` on ``request_data`` under a
          ...
          ``derouter_logging_obj`` before iterating callbacks, so this key
          survives.
    ...
    def _flatten_messages_for_moderation(
        ...
        derouter normalizes Anthropic ``/v1/messages`` requests to OpenAI shape,
    ...
    def _build_prompt_moderation_payload(
        ...
        chat-completions request. ``structured_messages`` is derouter's
        ...
        # Fall back to derouter_call_id, the stable cross-provider join key the
        # response/tool path uses (see _correlation_id). DeRouter does not
        # populate request_data["correlation_key"]; it carries derouter_call_id.
        ...
        correlation_key: Final = request_data.get("correlation_key") or request_data.get("derouter_call_id")
        return {
    ...
    def _extract_request_data(
        ...
        # Don't forward proxy_server_request -- in derouter >=1.83 its ``body``
    ...
    def _correlation_id(
        ...
        Always ``derouter_call_id``. It is assigned at request start and is
        ...
        """
        return call_details.get("derouter_call_id") or (request_data or _EMPTY_MAPPING).get("derouter_call_id")

    @classmethod
    def _apply_correlation_id(cls, payload: _CorrelatablePayload, source: _HasCallId | _LogEventKwargs) -> None:
        """Pin ``payload["id"]`` to ``derouter_call_id`` in place so this log
        ...
        (``response_obj.get("id", derouter_call_id)``), a ``chatcmpl-*`` value
        for OpenAI, which would not correlate. ``derouter_call_id`` is assigned
        at request start and is identical across all log paths. Falls back to
        the existing id when ``derouter_call_id`` is somehow absent rather than
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        if kwargs.get("_rubrik_blocked"):
            verbose_logger.debug(
                "Rubrik: skipping success event for blocked request derouter_call_id=%s",
                kwargs.get("derouter_call_id"),
            )
            return
    ...
    async def async_post_call_failure_hook(
        ...
        exception we no-op; DeRouter's standard failure plumbing handles those.
        """
    ...
    async def _build_and_enqueue_block_event(
        ...
        try:
            ...
            call_id = call_details.get("derouter_call_id")
            ...
            verbose_logger.error(
                "Rubrik: failed to build blocked-tool payload for derouter_call_id=%s: %s. Event will NOT be logged.",
                call_id,
                e,
                exc_info=True,
            )
        ...
        try:
            ...
            verbose_logger.error(
                "Rubrik: failed to enqueue blocked-tool event for derouter_call_id=%s: %s.",
                call_id,
                e,
                exc_info=True,
            )
    ...
    def _prepare_block_failure_payload(
        ...
        - caller identity: ``_caller_metadata`` off the ``user_api_key_dict``
          the failure hook is handed. The enriched derouter metadata lives under
          ``call_details["derouter_params"]["metadata"]``, never at the top
    ...
    def _caller_metadata(user_api_key_dict: "UserAPIKeyAuth") -> StandardLoggingUserAPIKeyMetadata:
        ...
        The import is deferred because ``derouter/integrations/`` is SDK-side
        ...
        return DeRouterProxyRequestSetup.get_sanitized_user_information_from_key(user_api_key_dict)
    ...
    def _build_fallback_payload(
        ...
        return {
            "id": call_details.get("derouter_call_id"),
