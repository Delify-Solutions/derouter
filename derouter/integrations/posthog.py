...
from derouter.integrations.posthog_mock_client import (
...
from derouter.types.integrations.posthog import (
...
class PostHogDeRouterParams(TypedDict, total=False):
...
class PostHogLogKwargs(TypedDict, total=False):
    ...
    derouter_params: ReadOnly[PostHogDeRouterParams]
...
class PostHogLogger(CustomBatchLogger):
    ...
    def _add_custom_metadata_properties(self, properties: dict[str, object], kwargs: PostHogLogKwargs) -> None:
        ...
        derouter_internal_fields: Final = {
            "endpoint",
            "caching_groups",
            "user_api_key_hash",
            "user_api_key_alias",
        ...
        for key, value in metadata.items():
            if key not in derouter_internal_fields:
    ...
    def _extract_metadata(self, kwargs: PostHogLogKwargs) -> Mapping[str, object]:
        derouter_params: Final[PostHogDeRouterParams] = kwargs.get("derouter_params", {}) or {}
        metadata: Final[Mapping[str, object]] = derouter_params.get("metadata", {}) or {}
        return metadata
