...
from derouter.integrations.custom_batch_logger import CustomBatchLogger
from derouter.integrations.SlackAlerting.budget_alert_types import get_budget_alert_type
from derouter.integrations.SlackAlerting.hanging_request_check import (
...
from derouter.types.integrations.slack_alerting import *
...
class SlackAlerting(CustomBatchLogger):
    ...
    def _response_taking_too_long_callback_helper(
        ...
        try:
            ...
            api_base: Final = derouter.get_api_base(model=model, optional_params=derouter_params)
    ...
    async def response_taking_too_long_callback(
        ...
        if derouter.turn_off_message_logging or derouter.redact_messages_in_exceptions:
            messages = "Message not logged. derouter.redact_messages_in_exceptions=True"
        request_info = f"\nRequest Model: `{model}`\nAPI Base: `{api_base}`\nMessages: `{messages}`"
        slow_message = f"`Responses are slow - {round(time_difference_float, 2)}s response time > Alerting threshold: {self.alerting_threshold}s`"
        alerting_metadata: dict = {}
        if time_difference_float > self.alerting_threshold:
            # add deployment latencies to alert
            if kwargs is not None and "derouter_params" in kwargs and "metadata" in kwargs["derouter_params"]:
                _metadata: Final[dict] = kwargs["derouter_params"]["metadata"]
    ...
    async def send_daily_reports(self, router) -> bool:
        ...
        # format alert -> return the derouter model name + api base
        ...
        for i in range(len(top_5_failed)):
            ...
            if isinstance(_deployment, dict):
                deployment_name = _deployment["derouter_params"].get("model", "")
            else:
                return False

            api_base = derouter.get_api_base(
                model=deployment_name,
                optional_params=(_deployment["derouter_params"] if _deployment is not None else {}),
            )
            if api_base is None:
                api_base = ""
    ...
    async def budget_alerts(
        ...
        """
        ## PREVENTITIVE ALERTING ## - https://github.com/Delify-Solutions/derouter/issues/2727
...
"""
    ...
    async def region_outage_alerts(
        ...
        ### GET PROVIDER ###
        provider = deployment.derouter_params.custom_llm_provider
        if provider is None:
            model, provider, _, _ = derouter.get_llm_provider(model=model)

        ### GET REGION ###
        region_name = deployment.derouter_params.region_name
        if region_name is None:
            region_name = derouter.utils._get_model_region(
                custom_llm_provider=provider, derouter_params=deployment.derouter_params
            )
        ...
        try:
            ...
            if provider is None:
                try:
                    ...
                    provider = ""
            api_base: Final = derouter.get_api_base(model=model, optional_params=deployment.derouter_params)
    ...
    async def model_added_alert(self, model_name: str, derouter_model_name: str, passed_model_info: object):
        base_model_from_user: Final = getattr(passed_model_info, "base_model", None)
        model_info = {}
        base_model = ""
        if base_model_from_user is not None:
            model_info = derouter.model_cost.get(base_model_from_user, {})
            base_model = f"Base Model: `{base_model_from_user}`\n"
        else:
            model_info = derouter.model_cost.get(derouter_model_name, {})
        model_info_str = ""
...
"""
    ...
    async def send_model_deprecation_alert(
        ...
        from derouter.proxy.common_utils.model_deprecation import (
    ...
    async def send_key_created_or_user_invited_email(self, webhook_event: WebhookEvent) -> bool:
        try:
            ...
            if email_logo_url is None:
                email_logo_url = DEROUTER_LOGO_URL
            if email_support_contact is None:
                email_support_contact = DEROUTER_SUPPORT_CONTACT
            ...
            email_html_content = "Alert from DeRouter Server"
    ...
    async def send_email_alert_using_smtp(self, webhook_event: WebhookEvent, alert_type: str) -> bool:
        ...
        if webhook_event.event == "budget_crossed":
            ...
            <img src="{email_logo_url}" alt="DeRouter Logo" width="150" height="50" />
        ...
        if webhook_event.event_group == Litellm_EntityType.TEAM:
            from derouter.integrations.email_alerting import send_team_budget_alert
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        try:
            if "daily_reports" in self.alert_types:
                ...
                model_info: Final = derouter_params.get("model_info", {}) or {}
                model_id: Final = model_info.get("id", "") or ""
                response_s: Final[timedelta] = end_time - start_time

                final_value = response_s

                if isinstance(response_obj, derouter.ModelResponse) and (
    ...
    async def async_log_failure_event(self, kwargs, response_obj, start_time, end_time):
        ...
        _derouter_params: Final = kwargs.get("derouter_params", {})
        _model_info: Final = _derouter_params.get("model_info", {}) or {}
    ...
    async def send_user_spend_alerts(self, prisma_client: "PrismaClient | None" = None) -> None:
        ...
        from derouter.proxy.proxy_server import prisma_client as global_prisma_client

        client: Final = prisma_client if prisma_client is not None else global_prisma_client
        if client is None:
            return

        from derouter.integrations.SlackAlerting.user_spend_alerts import (
            evaluate_user_spend,
    ...
    async def _request_is_completed(self, request_data: dict | None) -> bool:
        ...
        if request_data.get("derouter_status", "") != "success" and request_data.get("derouter_status", "") != "fail":
            ## CHECK IF CACHE IS UPDATED
            derouter_call_id: Final = request_data.get("derouter_call_id", "")
            status: Final[str | None] = await self.internal_usage_cache.async_get_cache(
                key=f"request_status:{derouter_call_id}", local_only=True
            )
