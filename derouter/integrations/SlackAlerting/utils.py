...
if TYPE_CHECKING:
    from derouter.derouter_core_utils.derouter_logging import Logging as _Logging
...
async def _add_langfuse_trace_id_to_alert(
    ...
    -> derouter_call_id
    """
    if "langfuse" not in derouter.logging_callback_manager._get_all_callbacks():
        return None
    #########################################################
    # Only run if langfuse is added as a callback
    #########################################################

    if request_data is not None and request_data.get("derouter_logging_obj", None) is not None:
        trace_id: str | None = None
        derouter_logging_obj: Final[Logging] = request_data["derouter_logging_obj"]

        for _ in range(3):
            trace_id = derouter_logging_obj._get_trace_id(service_name="langfuse")
            if trace_id is not None:
                break
            await asyncio.sleep(3)  # wait 3s before retrying for trace id
        #########################################################
        langfuse_object: Final = derouter_logging_obj._get_callback_object(service_name="langfuse")
