...
class AlertingHangingRequestCheck:
    ...
    async def add_request_to_hanging_request_check(
        ...
        request_metadata: Final = get_derouter_metadata_from_kwargs(kwargs=request_data)
        ...
        if request_data.get("deployment", None) is not None and isinstance(request_data["deployment"], dict):
            api_base = derouter.get_api_base(
                model=model,
                optional_params=request_data["deployment"].get("derouter_params", {}),
            )

        hanging_request_data: Final = HangingRequestData(
            request_id=request_data.get("derouter_call_id", ""),
            model=model,
            api_base=api_base,
    ...
    async def send_hanging_request_alert(
        ...
        """
        from derouter.integrations.SlackAlerting.slack_alerting import AlertType
