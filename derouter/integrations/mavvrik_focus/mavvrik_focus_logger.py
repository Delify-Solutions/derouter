...
from derouter.constants import MAVVRIK_FOCUS_EXPORT_JOB_NAME
from derouter.integrations.focus.destinations.base import FocusTimeWindow
...
class MavvrikFocusLogger(FocusLogger):
    ...
    # data that has likely been cleaned up from DeRouter_DailyUserSpend.
    _MAX_CATCHUP_DAYS = 7
    ...
    async def _run_scheduled_export(self) -> None:
        ...
        """
        engine: Final = self._ensure_engine()
        from derouter.integrations.focus.destinations.mavvrik_destination import (  # noqa: PLC0415
        ...
        # DeRouter's DailyUserSpend rows for a given date keep getting updated_at
    ...
    async def initialize_mavvrik_focus_export_job(self) -> None:
        ...
        from derouter.proxy.proxy_server import proxy_logging_obj  # noqa: PLC0415
    ...
    async def init_mavvrik_focus_background_job(
        ...
        loggers: list[MavvrikFocusLogger] = [
            cb
            for cb in derouter.logging_callback_manager.get_custom_loggers_for_type(callback_type=MavvrikFocusLogger)
            if type(cb) is MavvrikFocusLogger
        ]
        if not loggers and "mavvrik" in derouter.callbacks:
            ...
            from derouter.derouter_core_utils.derouter_logging import (  # noqa: PLC0415
