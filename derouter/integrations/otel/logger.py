...
from derouter.integrations.otel.emitter import SpanEmitter, stamp_error
from derouter.integrations.otel.mappers import resolve_mappers
from derouter.integrations.otel.model.baggage import promoted_baggage
from derouter.integrations.otel.model.config import OpenTelemetryV2Config
from derouter.integrations.otel.model.metadata import (
    LLMCallEvent,
    RequestIdentity,
    auth_metadata,
...
from derouter.integrations.otel.model.semconv import Error
from derouter.integrations.otel.model.spans import SpanRole, span_role_for_service
from derouter.integrations.otel.model.utils import to_ns
...
from derouter.integrations.otel.plumbing.routing import TenantTracerCache
...
DEROUTER_TRACER_NAME: Final = "derouter"
...
_OTEL_MODULES: Final = (
    "derouter.integrations.otel",
    "derouter.integrations.opentelemetry",
)
...
class OpenTelemetryV2(CustomLogger):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.tracer: Tracer = get_tracer(self._tracer_provider, DEROUTER_TRACER_NAME)
        ...
        self._tenant_tracers = TenantTracerCache(self.config, callback_name, DEROUTER_TRACER_NAME)
    ...
    def _init_metrics(self, meter_provider: "MeterProvider | None") -> "GenAIMetricRecorder | None":
        ...
        meter: Final = get_meter(provider, DEROUTER_TRACER_NAME)
    ...
    def _init_events(self, logger_provider: LoggerProvider | None) -> "GenAIEventRecorder | None":
        ...
        if provider is None:
            return None
        return GenAIEventRecorder(get_event_logger(provider, DEROUTER_TRACER_NAME))
    ...
    def _init_otel_logger_on_derouter_proxy(self) -> None:
        try:
            from derouter.proxy import proxy_server
        except Exception:
            return
        try:
            self._register_in_callback_list(derouter.service_callback)
            self._register_in_callback_list(derouter.input_callback)
            self._register_in_callback_list(derouter._async_success_callback)
            self._register_in_callback_list(derouter._async_failure_callback)
        except Exception:
            pass
    ...
    def log_pre_api_call(self, model, messages, kwargs):
        ...
        ``DeRouterLoggingObj`` (a typed object) and closed in the async callback.
    ...
    def seed_request_identity(self, user_api_key_dict: object, model: str | None = None) -> None:
        ...
        ``DeRouterBaggageSpanProcessor``. Called once at the auth boundary (as soon
        ...
        try:
            ...
            if bag:
                ...
                if is_recordable_span(server_span):
                    ...
                    # ``create_derouter_proxy_request_started_span`` (e.g. the SDK
...
def select_global_otel_v2_logger(
    ...
    the first one (``_init_otel_logger_on_derouter_proxy`` sets
...
def _logger_class(config: OpenTelemetryV2Config) -> type[OpenTelemetryV2]:
    ...
    from derouter.integrations.otel.langfuse_logger import LangfuseOpenTelemetryV2
