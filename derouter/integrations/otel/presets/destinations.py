...
from derouter.derouter_core_utils.url_utils import is_url_destination_allowed_by_host
...
def _warn_host_not_allowlisted(host: str) -> None:
    ...
    verbose_logger.warning(
        ...
        "derouter_settings.provider_url_destination_allowed_hosts to permit it",
        host,
    )
...
def _langfuse_destination(params: StandardCallbackDynamicParams) -> "_Destination | None":
    ...
    if not is_url_destination_allowed_by_host(endpoint, derouter.provider_url_destination_allowed_hosts):
...
def _weave_destination(params: StandardCallbackDynamicParams) -> "_Destination | None":
    from derouter.integrations.weave.weave_otel import weave_otel_endpoint
...
def _newrelic_destination(params: StandardCallbackDynamicParams) -> "_Destination | None":
    from derouter.integrations.otel.presets.newrelic import newrelic_dynamic_endpoint
...
def destination_capable_backends() -> frozenset[str]:
    ...
    from derouter.integrations.otel.presets import DYNAMIC_HEADERS_BY_CALLBACK
