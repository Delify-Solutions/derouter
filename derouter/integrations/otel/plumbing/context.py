...
from derouter.integrations.otel.model.semconv import HTTP
...
_request_root_span: Final["ContextVar[Span | None]"] = ContextVar("derouter_otel_request_root_span", default=None)
...
_mcp_message_trace_carrier: Final["ContextVar[Mapping[str, str] | None]"] = ContextVar(
    "derouter_otel_mcp_message_trace_carrier", default=None
)
...
_mcp_message_transport_span: Final["ContextVar[Span | None]"] = ContextVar(
    "derouter_otel_mcp_message_transport_span", default=None
)
...
def resolve_mcp_span_context(
    ...
    *current message's* POST rather than whatever request happened to open the
    ...
    baggage processor stamps allowlisted baggage keys (``derouter.team.id``,
    ``derouter.metadata.*``, ...) onto the span as attributes, so honoring remote
...
_request_destinations: Final['ContextVar[tuple["OtelDestination", ...]]'] = ContextVar(
    "derouter_otel_request_destinations", default=()
)
...
#: ``derouter_settings: otel_tenant_destination_mode`` and its env equivalent.
ADDITIVE_DESTINATION_MODE: Final = "additive"
OTEL_TENANT_DESTINATION_MODE_ENV: Final = "DEROUTER_OTEL_TENANT_DESTINATION_MODE"
...
def tenant_destinations_are_additive() -> bool:
    ...
    configured: Final = derouter.otel_tenant_destination_mode or os.environ.get(OTEL_TENANT_DESTINATION_MODE_ENV)
