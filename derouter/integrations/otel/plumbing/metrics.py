...
the v1 :mod:`derouter.integrations.opentelemetry` integration so both engines emit
...
from derouter.integrations.otel.model.metadata import time_to_first_chunk_seconds
...
from derouter.integrations.otel.model.utils import to_seconds
...
def _provider_attributes(custom_llm_provider: object) -> Mapping[str, str]:
    ...
    ``gen_ai.system`` spelling is dual-emitted with the raw derouter provider
...
# ``derouter_overhead_time_ms``, ``cache_key``, ``usage_object`` and the provider's
...
class _ResponseView(Protocol):
    """The one read the recorder makes on a derouter response object."""
...
class _MetricKwargs(TypedDict, total=False):
    ...
    derouter_params: ReadOnly[Mapping[str, object] | None]
...
class GenAIMetricRecorder:
    ...
    def record_failure(
        ...
        nothing to measure here. derouter hands the failure callback no
