...
from derouter.integrations.otel.model.spans import DeRouterSpanKind
...
_SPAN_KIND_BY_ROLE_KIND: Final[dict[DeRouterSpanKind, SpanKind]] = {
    DeRouterSpanKind.SERVER: SpanKind.SERVER,
    DeRouterSpanKind.CLIENT: SpanKind.CLIENT,
    DeRouterSpanKind.INTERNAL: SpanKind.INTERNAL,
    DeRouterSpanKind.PRODUCER: SpanKind.PRODUCER,
    DeRouterSpanKind.CONSUMER: SpanKind.CONSUMER,
}


def to_otel_span_kind(kind: DeRouterSpanKind) -> SpanKind:
...
class DeRouterBaggageSpanProcessor(SpanProcessor):
    """Stamps an allowlisted set of Baggage entries onto every span at start."""

    def __init__(
        self,
        allowed_keys: Iterable[str],
        allowed_prefixes: tuple[str, ...] = (DeRouter.METADATA_PREFIX,),
    ) -> None:
...
def parse_headers(raw: str | None) -> dict[str, str]:
    ...
    own parser is used so derouter decodes exactly what the OTLP exporters do
...
def _exporter_from_spec(spec: ExporterSpec) -> SpanExporter:
    ...
    if kind in _OTLP_HTTP_JSON_KINDS:
        from derouter.integrations.otel.plumbing.otlp_json import OTLPJsonSpanExporter
...
class _DrainPool:
    ...
    def __init__(
        ...
        self._threads: Final = tuple(
            threading.Thread(target=self._drain_until_closed, daemon=True, name="derouter-otel-destination-drain")
    ...
    def submit(self, processor: SpanProcessor) -> None:
        ...
        threading.Thread(
            ...
            args=(processor,),
            daemon=True,
            name="derouter-otel-destination-drain-straggler",
        ).start()
...
_TENANT_OWNED_KEYS: Final = frozenset({GenAI.OPERATION_NAME, MCP.METHOD_NAME, DeRouter.GUARDRAIL_NAME})
...
# under the old convention and the new one, and derouter accepts a virtual key as a
# ``?key=`` query parameter.
...
def _guardrail_unreachable(attributes: Mapping[str, AttributeValue]) -> bool:
    return attributes.get(DeRouter.GUARDRAIL_STATUS) in _GUARDRAIL_UNREACHABLE_STATUSES


def _tenant_visible(key: str, database: bool, owned: bool, unreachable_guardrail: bool) -> bool:
    if key.startswith(_CAPTURED_HEADER_PREFIXES) or key in (DeRouterError.STACK_TRACE, _URL_QUERY_KEY):
        return False
    if database and key in _DATASTORE_ENDPOINT_KEYS:
        return False
    if unreachable_guardrail and key == DeRouter.GUARDRAIL_RESPONSE:
        return False
...
def get_event_logger(provider: SDKLoggerProvider, name: str = "derouter") -> EventLogger:
    return EventLoggerProvider(logger_provider=provider).get_event_logger(name, derouter_version)
...
def get_meter(provider: MeterProvider, name: str = "derouter") -> "Meter":
    return provider.get_meter(name, derouter_version)
...
def build_tracer_provider(
    ...
    if baggage_processor is None:
        baggage_processor = DeRouterBaggageSpanProcessor(allowed_keys=config.baggage_promoted_keys)
...
def get_tracer(provider: TracerProvider, name: str = "derouter") -> Tracer:
    # Stamp the instrumentation scope with the DeRouter package version so every
    ...
    return provider.get_tracer(name, derouter_version)
