...
from derouter.integrations.otel.mappers.base import AttributeMapper, SpanData
...
from derouter.integrations.otel.plumbing.providers import to_otel_span_kind
...
def _stamp_derouter_error_attributes(span: Span, error: SpanError) -> None:
    """Stamp derouter-specific error detail attributes. Emitted only when the
    corresponding field is populated so guardrail-shape errors carrying only a
    message aren't polluted with empty detail keys."""
    if error.code:
        span.set_attribute(DeRouterError.CODE, error.code)
    if error.stack_trace:
        span.set_attribute(DeRouterError.STACK_TRACE, error.stack_trace)
    if error.llm_provider:
        span.set_attribute(DeRouterError.LLM_PROVIDER, error.llm_provider)
...
def stamp_error(
    ...
    *,
    ...
    _stamp_derouter_error_attributes(span, error)
    if set_status:
...
class SpanEmitter:
    ...
    def finish_span(
        ...
        *,
        ...
        # span-level health signal derouter doesn't actually evaluate. Only a
