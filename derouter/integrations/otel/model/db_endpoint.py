"""OTel ``db.*`` / ``server.*`` attributes naming the database derouter talks to.
...
own metrics. These attributes name the real server on derouter's DB spans.
...
from derouter.integrations.otel.model.semconv import DB, Server
from derouter.integrations.otel.model.spans import POSTGRESQL, db_system
