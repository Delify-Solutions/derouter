...
from derouter.integrations.otel.model.baggage import (
...
OTEL_V2_ENV: Final = "DEROUTER_OTEL_V2"
...
class OpenTelemetryV2Config(BaseSettings):
    ...
    service_name: str = Field(default="derouter", validation_alias=AliasChoices("OTEL_SERVICE_NAME"))
    deployment_environment: str | None = Field(default=None, validation_alias=AliasChoices("OTEL_ENVIRONMENT_NAME"))

    enable_metrics: bool = Field(
        default=False,
        validation_alias=AliasChoices("DEROUTER_OTEL_INTEGRATION_ENABLE_METRICS"),
    )
    enable_events: bool = Field(
        default=False,
        validation_alias=AliasChoices("DEROUTER_OTEL_INTEGRATION_ENABLE_EVENTS"),
    )
    ...
    legacy_compat: bool = Field(default=True, validation_alias=AliasChoices("DEROUTER_OTEL_LEGACY_COMPAT"))
    ...
    baggage_promoted_keys: Annotated[list[str], NoDecode] = Field(
        ...
        validation_alias=AliasChoices("baggage_promoted_keys", "DEROUTER_OTEL_BAGGAGE_PROMOTED_KEYS"),
        description=(
            "Identity attribute keys written into Baggage and stamped on every "
            "child span (e.g. ``derouter.team.id``). Configure via the "
            "``DEROUTER_OTEL_BAGGAGE_PROMOTED_KEYS`` env var (comma-separated) or "
    ...
    baggage_metadata_keys: Annotated[list[str], NoDecode] = Field(
        ...
        validation_alias=AliasChoices("baggage_metadata_keys", "DEROUTER_OTEL_BAGGAGE_METADATA_KEYS"),
        description=(
            "Metadata sub-keys promoted under the ``derouter.metadata.*`` "
            "namespace. Configure via the ``DEROUTER_OTEL_BAGGAGE_METADATA_KEYS`` "
    ...
    baggage_team_metadata_keys: Annotated[list[str], NoDecode] = Field(
        ...
        validation_alias=AliasChoices("baggage_team_metadata_keys", "DEROUTER_OTEL_BAGGAGE_TEAM_METADATA_KEYS"),
        description=(
            "Sub-keys of the team's free-form metadata promoted under "
            "``derouter.team.metadata``. Empty by default so none of a team's "
            "metadata leaves the process until explicitly allowlisted. Configure "
            "via the ``DEROUTER_OTEL_BAGGAGE_TEAM_METADATA_KEYS`` env var "
            "(comma-separated) or "
    ...
    def _split_csv(cls, value: Any) -> Any:
        ...
        an operator write ``DEROUTER_OTEL_BAGGAGE_PROMOTED_KEYS=derouter.team.id,derouter.api_key.hash``.
