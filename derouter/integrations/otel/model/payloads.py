...
from derouter.integrations.otel.model.utils import (
    as_bool,
    as_float,
    as_int,
    as_str,
...
class LLMCost:
    ...
    ``cost_breakdown`` (``derouter.types.utils.CostBreakdown``).
...
class ServiceSpanData:
    ...
    # these are namespaced: the canonical vocabulary uses ``derouter.metadata.*``
...
class LLMCallSpanData:
    ...
    # generation, speech, transcription, OCR), and the derouter route itself, which
