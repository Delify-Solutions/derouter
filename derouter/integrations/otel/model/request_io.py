...
import derouter
from derouter.integrations.otel.mappers.utils import json_or_none
from derouter.proxy.guardrails.anthropic_sse import assemble_anthropic_sse_stream, is_raw_sse_stream
from derouter.types.llms.openai import ResponseCompletedEvent, ResponsesAPIResponse
...
def _assembled_chat_stream(chunks: Sequence[object], data: Mapping[str, object]) -> object:
    try:
        return derouter.stream_chunk_builder(  # pyright: ignore[reportUnknownMemberType]  # upstream types chunks as a bare list
            chunks=list(chunks),  # mutable-ok: stream_chunk_builder takes a list
            messages=_MESSAGES.validate_python(data.get("messages")),
        )
    except (derouter.APIError, ValidationError):
        return None
