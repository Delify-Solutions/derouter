...
``providers.DeRouterBaggageSpanProcessor`` reads Baggage at span start and stamps
...
from derouter.integrations.otel.model.metadata import RequestIdentity
from derouter.integrations.otel.model.semconv import GenAI, DeRouter
...
_PROMOTABLE: Final[dict[str, Callable[[RequestIdentity, str | None, tuple[str, ...]], str | None]]] = {
    DeRouter.TEAM_ID: lambda identity, model, team_metadata_keys: identity.team_id,
    DeRouter.TEAM_ALIAS: lambda identity, model, team_metadata_keys: identity.team_alias,
    DeRouter.TEAM_METADATA: lambda identity, model, team_metadata_keys: _filtered_team_metadata_json(
        identity.team_metadata, team_metadata_keys
    ),
    DeRouter.KEY_HASH: lambda identity, model, team_metadata_keys: identity.key_hash,
    DeRouter.END_USER: lambda identity, model, team_metadata_keys: identity.end_user,
    GenAI.REQUEST_MODEL: lambda identity, model, team_metadata_keys: model,
    DeRouter.PROVIDER_MODEL: lambda identity, model, team_metadata_keys: identity.provider_model,
}
...
BAGGAGE_PROMOTED_KEYS: Final[tuple[str, ...]] = (
    DeRouter.TEAM_ID,
    DeRouter.TEAM_ALIAS,
    DeRouter.TEAM_METADATA,
    DeRouter.KEY_HASH,
    GenAI.REQUEST_MODEL,
    DeRouter.PROVIDER_MODEL,
)

# Metadata sub-keys eligible for promotion under the ``derouter.metadata.*``
...
# ``derouter.team.metadata``. Empty by default: a team's metadata can hold
...
def promoted_baggage(
    ...
    sub-keys of ``identity.metadata`` to promote under ``derouter.metadata.*``;
    ``team_metadata_keys`` selects sub-keys of the team's metadata to promote
    under ``derouter.team.metadata``. Empty values are dropped.
    """
    ...
    for meta_key in metadata_keys:
        ...
        if value:
            out[f"{DeRouter.METADATA_PREFIX}{meta_key}"] = value
    return out
