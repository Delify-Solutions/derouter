...
without a semconv equivalent lives under the ``derouter.*`` vendor namespace.
"""
...
class GenAIOperation(str, Enum):
    ...
    The first block is the convention's own vocabulary. The ``DEROUTER_`` members
    ...
    unmapped providers. They stay under the ``derouter.`` prefix so a value the
    ...
    DEROUTER_VECTOR_STORE_MANAGEMENT = "derouter.vector_store_management"
    DEROUTER_VECTOR_STORE_FILE_MANAGEMENT = "derouter.vector_store_file_management"
    DEROUTER_RESPONSES_MANAGEMENT = "derouter.responses_management"
    DEROUTER_MODERATION = "derouter.moderation"
...
class MCPMethod(str, Enum):
    """Well-known values for ``mcp.method.name`` that derouter's MCP gateway
...
class MCP:
    ...
    ``METHOD_NAME`` is the only key derouter populates from a closed request today;
...
class DeRouterError:
    ...
    OTel semconv does not define these, so they live under the ``derouter.*``
    vendor namespace rather than squatting on the semconv-owned ``error.*``
    namespace."""

    CODE: Final = "derouter.provider.error.code"
    STACK_TRACE: Final = "derouter.provider.error.stack_trace"
    LLM_PROVIDER: Final = "derouter.provider.error.llm_provider"
...
class DeRouter:
    """Vendor-extension keys (no semconv equivalent). Always ``derouter.*``."""

    CALL_ID: Final = "derouter.call_id"
    # The derouter route that produced the call. Needed because the convention maps
    ...
    CALL_TYPE: Final = "derouter.call_type"
    COST_PREFIX: Final = "derouter.cost."
    METADATA_PREFIX: Final = "derouter.metadata."
    TEAM_ID: Final = "derouter.team.id"
    TEAM_ALIAS: Final = "derouter.team.alias"
    # The team's free-form metadata dict, JSON-serialized into a single value.
    TEAM_METADATA: Final = "derouter.team.metadata"
    KEY_HASH: Final = "derouter.api_key.hash"
    END_USER: Final = "derouter.end_user.id"
    # The model string derouter actually sent to the provider (the deployment's
    # ``derouter_params.model``), distinct from the user-facing ``gen_ai.request.model``.
    PROVIDER_MODEL: Final = "derouter.provider.model"
    REQUEST_STREAMING: Final = "derouter.request.streaming"
    REQUEST_ROUTE: Final = "derouter.request.route"
    TOOLS_DECLARED: Final = "derouter.request.tools.declared"
    GUARDRAIL_NAME: Final = "derouter.guardrail.name"
    GUARDRAIL_MODE: Final = "derouter.guardrail.mode"
    GUARDRAIL_STATUS: Final = "derouter.guardrail.status"
    GUARDRAIL_PROVIDER: Final = "derouter.guardrail.provider"
    GUARDRAIL_ACTION: Final = "derouter.guardrail.action"
    GUARDRAIL_RESPONSE: Final = "derouter.guardrail.response"
    GUARDRAIL_VIOLATION_CATEGORIES: Final = "derouter.guardrail.violation_categories"
    GUARDRAIL_CONFIDENCE_SCORE: Final = "derouter.guardrail.confidence_score"
    GUARDRAIL_RISK_SCORE: Final = "derouter.guardrail.risk_score"
    GUARDRAIL_MASKED_ENTITY_COUNT: Final = "derouter.guardrail.masked_entity_count"
    GUARDRAIL_DURATION: Final = "derouter.guardrail.duration"
    GUARDRAIL_ID: Final = "derouter.guardrail.id"
    GUARDRAIL_POLICY_TEMPLATE: Final = "derouter.guardrail.policy_template"
    GUARDRAIL_DETECTION_METHOD: Final = "derouter.guardrail.detection_method"
    # Provider-reported billable usage counters, JSON-serialized into one value.
    GUARDRAIL_USAGE: Final = "derouter.guardrail.usage"
    # Numeric USD cost of the guardrail invocation; lives under the derouter.cost.*
    # namespace (COST_PREFIX) beside the LLM call's derouter.cost.total.
    GUARDRAIL_COST: Final = "derouter.cost.guardrail"
    # Whether derouter.cost.guardrail is already inside derouter.cost.total (True,
    ...
    GUARDRAIL_COST_IN_SPEND: Final = "derouter.guardrail.cost_in_spend"
    SERVICE_NAME: Final = "derouter.service.name"
    SERVICE_CALL_TYPE: Final = "derouter.service.call_type"
    PREPROCESSING_MS: Final = "derouter.preprocessing.duration_ms"
    ...
    MCP_SERVER_NAME: Final = "derouter.mcp.server.name"
...
class Metric:
    ...
    consumer charting GenAI telemetry finds derouter's series where it looks for
    ...
    conventions; the ``gen_ai.client.response.*`` spellings derouter used for the
    ...
    ``OPERATION_DURATION``, which derouter already emits for the whole operation.
    """
...
# derouter ``custom_llm_provider`` -> ``gen_ai.provider.name`` value.
_PROVIDER_BY_DEROUTER: Final[dict[str, GenAIProvider]] = {
...
# derouter ``call_type`` -> ``gen_ai.operation.name``.
_OPERATION_BY_CALL_TYPE: Final[dict[str, GenAIOperation]] = {
    ...
    "get_responses": GenAIOperation.DEROUTER_RESPONSES_MANAGEMENT,
    "aget_responses": GenAIOperation.DEROUTER_RESPONSES_MANAGEMENT,
    "delete_responses": GenAIOperation.DEROUTER_RESPONSES_MANAGEMENT,
    "adelete_responses": GenAIOperation.DEROUTER_RESPONSES_MANAGEMENT,
    "cancel_responses": GenAIOperation.DEROUTER_RESPONSES_MANAGEMENT,
    "acancel_responses": GenAIOperation.DEROUTER_RESPONSES_MANAGEMENT,
    "list_input_items": GenAIOperation.DEROUTER_RESPONSES_MANAGEMENT,
    "alist_input_items": GenAIOperation.DEROUTER_RESPONSES_MANAGEMENT,
    "image_generation": GenAIOperation.GENERATE_CONTENT,
    "aimage_generation": GenAIOperation.GENERATE_CONTENT,
    "moderation": GenAIOperation.DEROUTER_MODERATION,
    "amoderation": GenAIOperation.DEROUTER_MODERATION,
    ...
    "vector_store_create": GenAIOperation.DEROUTER_VECTOR_STORE_MANAGEMENT,
    "avector_store_create": GenAIOperation.DEROUTER_VECTOR_STORE_MANAGEMENT,
    "vector_store_retrieve": GenAIOperation.DEROUTER_VECTOR_STORE_MANAGEMENT,
    "avector_store_retrieve": GenAIOperation.DEROUTER_VECTOR_STORE_MANAGEMENT,
    "vector_store_list": GenAIOperation.DEROUTER_VECTOR_STORE_MANAGEMENT,
    "avector_store_list": GenAIOperation.DEROUTER_VECTOR_STORE_MANAGEMENT,
    "vector_store_update": GenAIOperation.DEROUTER_VECTOR_STORE_MANAGEMENT,
    "avector_store_update": GenAIOperation.DEROUTER_VECTOR_STORE_MANAGEMENT,
    "vector_store_delete": GenAIOperation.DEROUTER_VECTOR_STORE_MANAGEMENT,
    "avector_store_delete": GenAIOperation.DEROUTER_VECTOR_STORE_MANAGEMENT,
    "vector_store_file_create": GenAIOperation.DEROUTER_VECTOR_STORE_FILE_MANAGEMENT,
    "avector_store_file_create": GenAIOperation.DEROUTER_VECTOR_STORE_FILE_MANAGEMENT,
    "vector_store_file_list": GenAIOperation.DEROUTER_VECTOR_STORE_FILE_MANAGEMENT,
    "avector_store_file_list": GenAIOperation.DEROUTER_VECTOR_STORE_FILE_MANAGEMENT,
    "vector_store_file_retrieve": GenAIOperation.DEROUTER_VECTOR_STORE_FILE_MANAGEMENT,
    "avector_store_file_retrieve": GenAIOperation.DEROUTER_VECTOR_STORE_FILE_MANAGEMENT,
    "vector_store_file_content": GenAIOperation.DEROUTER_VECTOR_STORE_FILE_MANAGEMENT,
    "avector_store_file_content": GenAIOperation.DEROUTER_VECTOR_STORE_FILE_MANAGEMENT,
    "vector_store_file_update": GenAIOperation.DEROUTER_VECTOR_STORE_FILE_MANAGEMENT,
    "avector_store_file_update": GenAIOperation.DEROUTER_VECTOR_STORE_FILE_MANAGEMENT,
    "vector_store_file_delete": GenAIOperation.DEROUTER_VECTOR_STORE_FILE_MANAGEMENT,
    "avector_store_file_delete": GenAIOperation.DEROUTER_VECTOR_STORE_FILE_MANAGEMENT,
}


# derouter ``call_type`` -> ``gen_ai.output.type``. Only the call types whose route
...
def resolve_provider(custom_llm_provider: str | None) -> str:
    """Map a derouter provider string to a ``gen_ai.provider.name`` value.
    ...
    if not custom_llm_provider:
        return ""
    mapped: Final = _PROVIDER_BY_DEROUTER.get(custom_llm_provider.lower())
...
def resolve_operation(call_type: str | None) -> GenAIOperation:
    """Map a derouter ``call_type`` to a ``gen_ai.operation.name`` value.
...
def resolve_output_type(call_type: str | None) -> GenAIOutputType | None:
    """Map a derouter ``call_type`` to a ``gen_ai.output.type`` value, or ``None``
