...
Every relevant field derouter exposes about a request — the user-facing model,
...
* ``model`` — already reconstructed (see ``reconstruct_model_name``) to the name
  derouter dispatched to the provider (the deployment, provider-prefixed).
* ``hidden_params.derouter_model_name`` — a secondary source for the dispatched
...
SDK path, which has no group), and ``derouter.provider.model`` is the *dispatched*
...
from derouter.integrations.otel.model.semconv import resolve_operation
from derouter.integrations.otel.model.utils import as_str, to_seconds
...
class RequestIdentity:
    ...
    # The model derouter dispatched to the provider. Only known once the call
    ...
    def from_payload(cls, payload: StandardLoggingPayload) -> RequestIdentity:
        ...
        return cls(
            call_id=as_str(payload.get("derouter_call_id")) or as_str(payload.get("id")),
...
class RequestContext:
    ...
    (on :attr:`identity`) is the model derouter dispatched to the provider; the two
...
class LLMCallEvent:
    ...
    derouter hands every callback an untyped ``kwargs`` god object. The fields the
    ...
    # The ``derouter_call_id`` correlating ``pre_call`` with the close callback.
    ...
    def from_dict(cls, kwargs: Mapping[str, Any]) -> LLMCallEvent:
        ...
        return cls(
            ...
            is_no_upstream_call=bool(kwargs.get(DEROUTER_LOGGING_NO_UPSTREAM_LLM_CALL)),
...
def auth_metadata(payload: StandardLoggingPayload | None, kwargs: Mapping[str, object]) -> Mapping[str, str] | None:
    ...
    Read from the payload once the call closes and from ``derouter_params`` at
...
def _metadata_dicts(
    ...
    ``derouter_metadata`` is the metadata field on the Anthropic-shaped routes;
    derouter copies it onto ``metadata``, but both are yielded so a route that
    ...
    params: Final = _as_str_mapping(kwargs.get("derouter_params"))
    if params is None:
        return
    yield from (
        metadata
...
def _call_id(payload: StandardLoggingPayload | None, kwargs: Mapping[str, Any]) -> str | None:
    ...
    if payload is not None:
        call_id: Final = as_str(payload.get("derouter_call_id")) or as_str(payload.get("id"))
        if call_id:
            return call_id
    return as_str(kwargs.get("derouter_call_id"))
...
def resolve_provider_model(payload: StandardLoggingPayload) -> str | None:
    """The model derouter dispatched to the provider, from the payload.

    Prefers the explicit ``hidden_params.derouter_model_name`` (set on call paths
    ...
    return (
        ...
        as_str(raw_meta.get("deployment")) or as_str(hidden.get("derouter_model_name")) or as_str(payload.get("model"))
    )
