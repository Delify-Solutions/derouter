...
from derouter.integrations.opentelemetry_utils.base_otel_llm_obs_attributes import (
...
from derouter.types.utils import CallTypes, StandardLoggingMCPToolCall, StandardLoggingPayload
...
def _safe_get(obj, key, default=None):
    ...
    plain dicts, derouter ``Usage`` (which exposes ``.get``), and raw OpenAI
    ...
    see https://github.com/Delify-Solutions/derouter/issues/13672.
    """
    if obj is None:
        return default
...
def _infer_open_inference_span_kind(call_type: str | None) -> str:
    """
    Map DeRouter call types to OpenInference span kinds.
    """
...
def _set_request_attributes(
    ...
    safe_set_attribute(
        ...
        derouter_params.get("custom_llm_provider", "Unknown"),
    )
...
def _emit_message_tool_calls(span: "Span", prefix: str, message) -> None:
    ...
    Accepts dicts or Pydantic message objects (e.g. ``derouter.Message``); the
...
def _set_session_and_user_attrs(span: "Span", kwargs: dict, standard_logging_payload) -> None:
    ...
    `trace_id` is still emitted under its own `derouter.trace_id` key so
    ...
    if trace_id:
        safe_set_attribute(span, "derouter.trace_id", str(trace_id))
    ...
    if team_id:
        safe_set_attribute(span, "derouter.team_id", str(team_id))
    team_alias: Final = metadata.get("user_api_key_team_alias")
    if team_alias:
        safe_set_attribute(span, "derouter.team_alias", str(team_alias))
    key_alias: Final = metadata.get("user_api_key_alias")
    if key_alias:
        safe_set_attribute(span, "derouter.key_alias", str(key_alias))
...
def _set_response_cost_attr(span: "Span", standard_logging_payload) -> None:
    ...
    surface the cost in their "Total Cost" column. DeRouter only tracks a
...
def _maybe_normalize_passthrough(
    ...
    # Respect DeRouter's central message-redaction contract. The normal
