...
from derouter.types.integrations.arize import ArizeConfig
...
class ArizeLogger(OpenTelemetry):
    ...
    def _init_tracing(self, tracer_provider):
        ...
        if tracer_provider is not None:
            self.tracer = tracer_provider.get_tracer("derouter")
            self.span_kind = SpanKind
            return

        provider: Final = TracerProvider(resource=self._get_derouter_resource(self.config))
        provider.add_span_processor(self._get_span_processor())
        self.tracer = provider.get_tracer("derouter")
        self.span_kind = SpanKind
