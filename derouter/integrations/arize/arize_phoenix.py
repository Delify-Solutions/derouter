...
from derouter.integrations.arize import _utils
from derouter.integrations.arize._utils import ArizeOTELAttributes
from derouter.types.integrations.arize_phoenix import ArizePhoenixConfig
...
if TYPE_CHECKING:
    ...
    from derouter.integrations.opentelemetry import OpenTelemetry as _OpenTelemetry
    from derouter.integrations.opentelemetry import (
        OpenTelemetryConfig as _OpenTelemetryConfig,
    )
    from derouter.types.integrations.arize import Protocol as _Protocol

    Protocol = _Protocol
    OpenTelemetryConfig = _OpenTelemetryConfig
    Span = _Span | Any
    OpenTelemetry = _OpenTelemetry
    DEROUTER_TRACER_NAME: str
else:
    Protocol = Any
    ...
    try:
        from derouter.integrations.opentelemetry import (
            DEROUTER_TRACER_NAME,
            OpenTelemetry,
        )
    except ImportError:
        DEROUTER_TRACER_NAME = "derouter"
        OpenTelemetry = None
...
class ArizePhoenixLogger(OpenTelemetry):
    ...
    def _get_derouter_resource_for_project(self, project_name: str):
        """
    ...
    def _build_tracer_provider_for_project(self, project_name: str) -> TracerProvider:
        ...
        provider: Final = TracerProvider(resource=self._get_derouter_resource_for_project(project_name))
    ...
    def _get_tracer_for(self, project_name: str) -> Tracer:
        ...
        with self._project_providers_lock:
            if project_name in self._project_providers:
                ...
                return self._project_providers[project_name].get_tracer(DEROUTER_TRACER_NAME)
        ...
        with self._project_providers_lock:
            ...
            return new_provider.get_tracer(DEROUTER_TRACER_NAME)
    ...
    def _iter_metadata_dicts_from_kwargs(kwargs: dict):
        """Yield request metadata dicts; standard_logging_object before derouter_params."""
        for key in ("standard_logging_object", "derouter_params"):
    ...
    def _is_proxy_request(kwargs: dict) -> bool:
        """True when the call is routed through the DeRouter proxy.

        Proxy mode is determined solely by the server-set ``proxy_server_request``
        field in ``derouter_params``.  Checking request metadata for
        ...
        return isinstance(derouter_params, dict) and bool(derouter_params.get("proxy_server_request"))
