...
from derouter.integrations._types.open_inference import (
    OpenInferenceSpanKindValues,
    SpanAttributes,
)
from derouter.integrations.custom_logger import CustomLogger
from derouter.integrations.opentelemetry_utils.gen_ai_semconv import (
    OTEL_SEMCONV_STABILITY_OPT_IN_ENV,
    OTELGenAISemconvMixin,
    OTELSemconvCategory,
    parse_semconv_opt_in,
)
from derouter.integrations.otel.model.db_endpoint import db_span_attributes
from derouter.integrations.otel.model.semconv import Metric
...
from derouter.secret_managers.main import get_secret_bool, str_to_bool
from derouter.types.services import ServiceLoggerPayload
...
if TYPE_CHECKING:
    ...
    from derouter.proxy.proxy_server import UserAPIKeyAuth as _UserAPIKeyAuth
...
DEROUTER_TRACER_NAME: Final = os.getenv("OTEL_TRACER_NAME", "derouter")
DEROUTER_METER_NAME: Final = os.getenv("DEROUTER_METER_NAME", "derouter")
DEROUTER_LOGGER_NAME: Final = os.getenv("DEROUTER_LOGGER_NAME", "derouter")
DEROUTER_PROXY_REQUEST_SPAN_NAME: Final = "Received Proxy Server Request"
...
PREPROCESSING_DURATION_MS_ATTRIBUTE: Final = "derouter.preprocessing.duration_ms"
TEAM_METADATA_ATTRIBUTE: Final = "derouter.team.metadata"
MODEL_GROUP_ATTRIBUTE: Final = "derouter.model_group"
PROVIDER_MODEL_ATTRIBUTE: Final = "derouter.provider.model"
...
# Remove the hardcoded DEROUTER_RESOURCE dictionary - we'll create it properly later
RAW_REQUEST_SPAN_NAME: Final = "raw_gen_ai_request"
DEROUTER_REQUEST_SPAN_NAME: Final = "derouter_request"
...
class OpenTelemetryConfig:
    ...
    # under ``derouter.team.metadata``. Empty by default so none of a team's
    ...
    def __post_init__(self) -> None:
        ...
        if not self.service_name:
            self.service_name = os.getenv("OTEL_SERVICE_NAME", "derouter")
        ...
        ) or _normalize_team_metadata_keys(os.getenv("DEROUTER_OTEL_BAGGAGE_TEAM_METADATA_KEYS"))
    ...
    def from_env(cls):
        ...
        enable_metrics: Final[bool] = os.getenv("DEROUTER_OTEL_INTEGRATION_ENABLE_METRICS", "false").lower() == "true"
        enable_events: Final[bool] = os.getenv("DEROUTER_OTEL_INTEGRATION_ENABLE_EVENTS", "false").lower() == "true"
        service_name: Final = os.getenv("OTEL_SERVICE_NAME", "derouter")
...
class OpenTelemetry(OTELGenAISemconvMixin, CustomLogger):
    def __init__(
        ...
        **kwargs,
        ...
        self._derouter_resource_memo: _Resource | None = None
        ...
        self._init_otel_logger_on_derouter_proxy()

    @staticmethod
    def _get_derouter_resource(config: OpenTelemetryConfig) -> "_Resource":
    ...
    def _derouter_resource(self) -> "_Resource":
        ...
        """
        memo: Final = self._derouter_resource_memo
        if memo is not None:
            return memo
        built: Final = self._get_derouter_resource(self.config)
        self._derouter_resource_memo = built
        return built

    def _init_otel_logger_on_derouter_proxy(self):
        """
        Initializes OpenTelemetry for derouter proxy server
        ...
        if "otel" not in derouter.service_callback and all(
            not isinstance(cb, OpenTelemetry) for cb in derouter.service_callback
        ):
            derouter.service_callback.append(self)
    ...
    def _resolve_capture_mode(self) -> str:
        ...
        Precedence:
          1. ``derouter.turn_off_message_logging=True`` forces ``NO_CONTENT``
    ...
    def _init_tracing(self, tracer_provider):
        ...
        def create_tracer_provider():
            provider: Final = TracerProvider(resource=self._derouter_resource())
        ...
        self.tracer = tracer_provider.get_tracer(DEROUTER_TRACER_NAME)
    ...
    def _init_metrics(self, meter_provider):
        ...
        def create_meter_provider():
            ...
            return MeterProvider(
                ...
                resource=self._derouter_resource(),
            )
        ...
        self._response_duration_histogram = meter.create_histogram(
            ...
            description="Total LLM API generation time (excludes DeRouter overhead)",
            unit="s",
        )
    ...
    def _init_logs(self, logger_provider):
        ...
        def create_logger_provider():
            provider: Final = OTLoggerProvider(resource=self._derouter_resource())
                ...
                for key, value in event_metadata.items():
                    ...
                    if isinstance(value, dict):
                        try:
                            ...
                            value = "derouter logging error - could_not_json_serialize"
                    self.safe_set_attribute(
                        span=service_logging_span,
                        key=key,
                        value=value,
    ...
    async def async_post_call_failure_hook(
        ...
        if parent_otel_span is not None:
            ...
            # records on the derouter_request child span). Inline import:
            # derouter_logging <-> integrations is circular.
    ...
    def _emit_guardrail_spans_from_request_data(
        ...
        # kwargs["derouter_params"]["metadata"]["_otel_internal"]. Pass the
    ...
    async def async_post_call_success_hook(
        ...
        derouter_logging_obj: Final = data.get("derouter_logging_obj")

        if derouter_logging_obj is not None and isinstance(derouter_logging_obj, DeRouterLogging):
            kwargs: Final = derouter_logging_obj.model_call_details
    ...
    def _cached_dynamic_tracer(
        ...
        with self._tracer_provider_cache_lock:
            ...
            if cached is not None:
                self._tracer_provider_cache.move_to_end(cache_key)
                return cached.provider.get_tracer(DEROUTER_TRACER_NAME)
        ...
        return winner.provider.get_tracer(DEROUTER_TRACER_NAME)
    ...
    def _get_tracer_with_dynamic_config(self, dynamic_config: OpenTelemetryConfig) -> "_Tracer":
        ...
        def _build() -> "_SDKTracerProvider":
            provider: Final = TracerProvider(resource=self._derouter_resource(), shutdown_on_exit=owns_exporter)
    ...
    def _emit_once(self, kwargs: dict, *scope: object) -> bool:
        ...
        1. **Handler-level dual-fire**: streaming code paths trigger both
           ...
           ``derouter_request`` spans. Scope: ``("success",)`` / ``("failure",)``.
        ...
        ``kwargs["derouter_params"]["metadata"]["_otel_internal"]`` so it is
        ...
        if not isinstance(derouter_params, dict):
            derouter_params = {}
            kwargs["derouter_params"] = derouter_params

        _metadata = derouter_params.get("metadata")
        if not isinstance(_metadata, dict):
            _metadata = {}
            derouter_params["metadata"] = _metadata
    ...
    def _end_proxy_span_from_kwargs(self, kwargs: dict, end_time) -> None:
        ...
        Only spans named ``DEROUTER_PROXY_REQUEST_SPAN_NAME`` are closed —
        externally provided spans must not be closed by DeRouter.
        """
        derouter_params: Final = kwargs.get("derouter_params", {}) or {}
        _metadata: Final = derouter_params.get("metadata", {}) or {}
        proxy_span = _metadata.get("derouter_parent_otel_span", None)

        # Fallback: check derouter_metadata (used by /v1/messages and other
        # DEROUTER_METADATA_ROUTES).
        if proxy_span is None:
            _derouter_metadata: Final = derouter_params.get("derouter_metadata", {}) or {}
            proxy_span = _derouter_metadata.get("derouter_parent_otel_span", None)

        if (
            proxy_span is not None
            and getattr(proxy_span, "name", None) == DEROUTER_PROXY_REQUEST_SPAN_NAME
    ...
    def _handle_success(self, kwargs, response_obj, start_time, end_time):
        """Create the derouter_request span then close the proxy span."""
        verbose_logger.debug(
        ...
        # OR if USE_OTEL_DEROUTER_REQUEST_SPAN is explicitly enabled
        should_create_primary_span: Final = parent_span is None or get_secret_bool("USE_OTEL_DEROUTER_REQUEST_SPAN")

        if should_create_primary_span:
            # Create a new derouter_request span
            span = self._start_primary_span(kwargs, response_obj, start_time, end_time, ctx)
            # Raw-request sub-span (if enabled) - child of derouter_request span
            ...
            # The child derouter_request span already carries all attributes;
        ...
        # External spans (from Langfuse, user code, HTTP headers, global context) must not be closed by DeRouter
        # However, proxy-created spans should be closed here.
        if (
            parent_span is not None
            and hasattr(parent_span, "name")
            and parent_span.name == DEROUTER_PROXY_REQUEST_SPAN_NAME
        ...
        # after all child spans (derouter_request, guardrail, raw_request)
    ...
    def _set_team_attributes_on_span(
        ...
        derouter_request trace carries them, not just the root span.
    ...
    def _set_team_attributes_on_proxy_span_from_kwargs(self, kwargs: dict) -> None:
        ...
        Guarded to the DeRouter-created proxy span (by name + recording) so
        externally provided parent spans are never mutated.
        """
        derouter_params: Final = kwargs.get("derouter_params") or {}
        metadata: Final = derouter_params.get("metadata") or {}
        proxy_span: Final = metadata.get("derouter_parent_otel_span")
        if (
            proxy_span is not None
            and getattr(proxy_span, "name", None) == DEROUTER_PROXY_REQUEST_SPAN_NAME
            and hasattr(proxy_span, "is_recording")
    ...
    def _set_inference_identity_attributes(
        ...
        # ``metadata`` and ``derouter_metadata`` are alternate names for the same
        # full metadata dict (the name varies by endpoint), so first-truthy wins.
        raw_metadata: Final = derouter_params.get("metadata") or derouter_params.get("derouter_metadata") or {}
        ...
        provider_model: Final = hidden_params.get("derouter_model_name") or standard_logging_payload.get("model")
        if provider_model:
    ...
    def _ensure_metric_attribute_filter(self) -> None:
        ...
        if attributes is None and self.callback_name in (None, "otel"):
            otel_settings: Final = (derouter.callback_settings or {}).get("otel") or {}
    ...
    def _record_metrics(self, kwargs, response_obj, start_time, end_time):
        ...
        params: Final = kwargs.get("derouter_params") or {}
    ...
    def _record_time_to_first_token_metric(self, kwargs: dict, common_attrs: dict):
        ...
        # This excludes DeRouter overhead and measures pure LLM API latency
    ...
    def _record_time_per_output_token_metric(
        ...
        if is_unbilled_non_inference_call_from_params(
            kwargs.get("call_type"), kwargs.get("derouter_params"), response_obj
        ):
            return
    ...
    def _record_response_duration_metric(
        ...
        This excludes DeRouter overhead and measures only the LLM provider's response time.
        Works for both streaming and non-streaming requests.

        Mirrors Prometheus's derouter_llm_api_latency_metric.
    ...
    def _emit_semantic_logs(self, kwargs, response_obj, span: Span):
        ...
        otel_logger: Final = self._logger_provider.get_logger(DEROUTER_LOGGER_NAME)

        parent_ctx: Final = span.get_span_context()
        provider: Final = (kwargs.get("derouter_params") or {}).get("custom_llm_provider", "Unknown")
    ...
    def _resolve_guardrail_context(
        ...
        never orphaned (Issue #5).  Priority:
          1. The derouter_request span that was just created
    ...
    def _handle_failure(self, kwargs, response_obj, start_time, end_time):
        ...
        # Always create if no parent span exists (backward compatibility)
        # OR if USE_OTEL_DEROUTER_REQUEST_SPAN is explicitly enabled
        should_create_primary_span: Final = parent_otel_span is None or get_secret_bool("USE_OTEL_DEROUTER_REQUEST_SPAN")

        span = None
        if should_create_primary_span:
            # Span 1: Request sent to derouter SDK
            otel_tracer: Final[Tracer] = self.get_tracer_to_use_for_request(kwargs)
            span_kwargs: Final[_StartSpanKwargs] = {
                "name": self._get_span_name(kwargs),
                "start_time": self._to_ns(start_time),
            ...
            # When parent span exists and USE_OTEL_DEROUTER_REQUEST_SPAN=false,
        ...
        if (
            ...
            and parent_otel_span.name == DEROUTER_PROXY_REQUEST_SPAN_NAME
        ):
    ...
    def set_tools_attributes(self, span: Span, tools):
        ...
        from derouter.proxy._types import SpanAttributes
    ...
    def set_attributes(self, span: Span, kwargs, response_obj: Any | None):
        try:
            if self.callback_name == "langtrace":
                from derouter.integrations.langtrace import LangtraceAttributes

                LangtraceAttributes().set_langtrace_attributes(span, kwargs, response_obj)
                return
            elif self.callback_name == "langfuse_otel":
                from derouter.integrations.langfuse.langfuse_otel import (
                    LangfuseOtelLogger,
                )
            ...
            provider_name: Final = derouter_params.get("custom_llm_provider", "Unknown")
            ...
            # the derouter call ID so every call type can be correlated
            # across DeRouter UI, Phoenix traces, and provider logs (Issue #8).
            ...
            derouter_call_id: Final = standard_logging_payload.get("derouter_call_id")
            if derouter_call_id:
                self.safe_set_attribute(
                    span=span,
                    key="derouter.call_id",
                    value=derouter_call_id,
                )
            ...
            usage: Final = (
                ...
                and not is_unbilled_non_inference_call_from_params(
                    kwargs.get("call_type"), derouter_params, response_obj
                )
                else None
            )
            if usage:
    ...
    def _transform_messages_to_otel_semantic_conventions(self, messages: list[dict] | str) -> list[dict]:
        """
        Transforms DeRouter/OpenAI style messages into OTEL GenAI 1.38 compliant format.
    ...
    def _to_dict(obj) -> dict | None:
        ...
        2. DeRouter's ``BaseDeRouterOpenAIResponseObject`` — exposes a
        ...
        if isinstance(obj, dict):
            return obj
        if hasattr(obj, "get"):
            # BaseDeRouterOpenAIResponseObject duck-type
            return obj
    ...
    def _transform_responses_api_output_to_otel(self, output: list) -> list[dict]:
        ...
        Items may be plain dicts, DeRouter wrapper objects (with ``.get()``),
    ...
    def set_raw_request_attributes(self, span: Span, kwargs, response_obj):
        try:
            ...
            # The parent derouter_request span already carries the standard
            # gen_ai.* / metadata.* attributes — duplicating them here doubles
            # storage and adds noise (Issue #3).
            derouter_params: Final = kwargs.get("derouter_params", {}) or {}
            custom_llm_provider: Final = derouter_params.get("custom_llm_provider", "Unknown")
            ...
            if _raw_response and isinstance(_raw_response, str):
                ...
                try:
                    ...
                    verbose_logger.debug(
                        "derouter.integrations.opentelemetry.py::set_raw_request_attributes() - raw_response not json string - %s",
                        _raw_response,
                    )
    ...
    def _get_span_name(self, kwargs) -> str:
        ...
        return DEROUTER_REQUEST_SPAN_NAME
    ...
    def _get_span_context(self, kwargs, default_span: Span | None = None):
        ...
        _metadata: Final = derouter_params.get("metadata", {}) or {}
        parent_otel_span = _metadata.get("derouter_parent_otel_span", None)

        # Fallback: check derouter_metadata (used by /v1/messages and other
        # DEROUTER_METADATA_ROUTES that store proxy-internal metadata
        # separately from the provider's native "metadata" field).
        if parent_otel_span is None:
            _derouter_metadata: Final = derouter_params.get("derouter_metadata", {}) or {}
            parent_otel_span = _derouter_metadata.get("derouter_parent_otel_span", None)
        ...
        if otel_exporter == "console":
            ...
            try:
                ...
                raise ImportError(
                    ...
                    "`opentelemetry-exporter-otlp` and `grpcio` (or `derouter[grpc]`)."
                ) from exc
    ...
    def create_derouter_proxy_request_started_span(
        self,
        start_time: datetime,
        headers: dict,
    ) -> Span | None:
        ...
        return self.tracer.start_span(
            name=DEROUTER_PROXY_REQUEST_SPAN_NAME,
            start_time=self._to_ns(start_time),
    ...
    def set_preprocessing_duration_attribute(self, span: Span | None, container: object) -> None:
        """
        Set ``derouter.preprocessing.duration_ms`` (proxy-receive -> first
        provider handoff) on the proxy SERVER span. ``derouter_received_at``
        ...
        _lp: Final = container.get("derouter_params")
        for _md in (
            (_lp or {}).get("metadata") if isinstance(_lp, dict) else None,
            container.get("metadata"),
            container.get("derouter_metadata"),
        ):
            if isinstance(_md, dict):
                received_at = received_at or _md.get("derouter_received_at")
