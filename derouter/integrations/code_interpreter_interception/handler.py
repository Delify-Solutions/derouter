...
from derouter.llms.base_llm.sandbox.transformation import (
    CodeExecutionResult,
    ContainerHandle,
)
from derouter.types.integrations.code_interpreter_interception import (
...
DEROUTER_CODE_EXECUTION_TOOL_NAME: Final = "derouter_code_execution"
...
_DEROUTER_METADATA_KEY: Final = "derouter_metadata"
...
def _resolve_sandbox_tool(sandbox_tool_name: str | None) -> SandboxToolParams | None:
    if sandbox_tool_name is None:
        return None
    try:
        from derouter.sandbox.sandbox_tools import resolve_sandbox_tool
    except ImportError:
        return None
...
class CodeInterpreterInterceptionLogger(CustomLogger):
    ...
    2. Detect ``derouter_code_execution`` function calls in the model response.
    ...
    def initialize_from_proxy_config(
        ...
        params: Final[CodeInterpreterInterceptionConfig] = (
            derouter_settings["code_interpreter_interception_params"]
            if "code_interpreter_interception_params" in derouter_settings
    ...
    def _strip_interception_metadata(kwargs: dict[str, object]) -> None:
        metadata: Final = kwargs.get(_DEROUTER_METADATA_KEY)
        ...
        if filtered_metadata:
            kwargs[_DEROUTER_METADATA_KEY] = filtered_metadata
        else:
            kwargs.pop(_DEROUTER_METADATA_KEY, None)

    @staticmethod
    def _write_interception_metadata(kwargs: dict[str, object]) -> None:
        existing: Final = kwargs.get(_DEROUTER_METADATA_KEY)
        ...
        kwargs[_DEROUTER_METADATA_KEY] = metadata
    ...
    def _tool_choice_targets_code_interpreter(tool_choice: object) -> bool:
        ...
        return (
            ...
            or choice.get("name") == DEROUTER_CODE_EXECUTION_TOOL_NAME
            or (isinstance(function, dict) and function.get("name") == DEROUTER_CODE_EXECUTION_TOOL_NAME)
        )
    ...
    def _resolve_provider(self, kwargs: dict[str, object]) -> str | None:
        ...
        if not isinstance(model, str):
            return None
        try:
            return derouter.get_llm_provider(model=model)[1]
        except Exception:
            return None
    ...
    def _filter_agentic_loop_kwargs(kwargs: dict[str, object]) -> dict[str, object]:
        return {
            ...
            if k not in {"derouter_logging_obj", "acompletion"}
    ...
    async def _create_container(self) -> tuple[ContainerHandle, SandboxToolParams | None]:
        ...
        container: Final = await derouter.acreate_sandbox(
    ...
    async def _run_code(
        ...
        return await derouter.arun_code(
            provider=params["sandbox_provider"],
            container=container,
            code=code,
            api_key=params.get("api_key"),
    ...
    async def _delete_container(self, container: ContainerHandle, params: SandboxToolParams | None) -> None:
        try:
            ...
            if params is None:
                return
            await derouter.adelete_sandbox(
                provider=params["sandbox_provider"],
                container=container,
                api_key=params.get("api_key"),
                api_base=params.get("api_base"),
    ...
    def _normalize_chat_completion_tool_call(
        ...
        if tool_call.type != "function" or tool_call.function.name != DEROUTER_CODE_EXECUTION_TOOL_NAME:
            return None
    ...
    def _is_code_execution_call(self, item: object) -> bool:
        if isinstance(item, dict):
            ...
            return (
                ...
                and item_mapping.get("name") == DEROUTER_CODE_EXECUTION_TOOL_NAME
            )
        item_type: Final[object] = getattr(item, "type", None)
        item_name: Final[object] = getattr(item, "name", None)
        return item_type == "function_call" and item_name == DEROUTER_CODE_EXECUTION_TOOL_NAME
