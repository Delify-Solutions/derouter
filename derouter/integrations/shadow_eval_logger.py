...
and appends one ``DeRouter_ShadowEvalAttempt`` row per arm (verdict or error) as the
...
from derouter.derouter_core_utils.internal_call_metadata import sanitized_forwardable_call_metadata
from derouter.derouter_core_utils.llm_judge import (
...
from derouter.types.management_endpoints.auto_router_endpoints import ShadowEvalDirection
from derouter.types.utils import SHADOW_EVAL_JUDGE_CALL_ORIGIN, SHADOW_EVAL_ROUTER_CALL_ORIGIN

if TYPE_CHECKING:
    from derouter.proxy.db.shadow_eval_funnel import ShadowEvalFunnelStage
...
def _proxy_wire_body(kwargs: Mapping[str, object]) -> Mapping[str, object]:
    ...
    request: Final = derouter_params.get("proxy_server_request") if isinstance(derouter_params, Mapping) else None
...
def _chat_request_from_anthropic_messages(
    ...
    adapter: Final = DeRouterAnthropicMessagesAdapter()
...
def _chat_request_from_responses(
    ...
    return _CHAT_REQUEST_ADAPTER.validate_python(
        DeRouterCompletionResponsesConfig.transform_responses_api_request_to_chat_completion_request(  # pyright: ignore[reportUnknownMemberType]  # transformer declares a bare dict return
            model=str(kwargs.get("model") or ""),
...
def _price_map_cost(response: object) -> float:
    ...
    try:
        return derouter.completion_cost(completion_response=response) or 0.0
...
async def _add_job_spend_to_counter(counter_key: str, cost: float) -> None:
    ...
    try:
        from derouter.proxy.proxy_server import increment_spend_counter
...
def _record_funnel_event(job_id: str, stage: "ShadowEvalFunnelStage") -> None:
    try:
        from derouter.proxy.db.shadow_eval_funnel import record_shadow_eval_funnel_event
...
class ShadowEvalLogger(CustomLogger):
    ...
    async def _active_jobs(self) -> Mapping[tuple[str, str], tuple[ActiveShadowEvalJob, ...]]:
        ...
        if prisma is None:
            return _EMPTY_JOBS
        try:
            records: Final = await prisma.db.derouter_shadowevaljob.find_many(
            ...
            grouped: Final = (
                await prisma.db.derouter_shadowevalattempt.group_by(
                    by=["job_id"],
                    count=True,
    ...
    async def async_log_success_event(
        ...
        try:
            ...
            if payload is None:
                return
            raw_meta: Final = get_derouter_metadata_from_kwargs(dict(kwargs))  # mutable-ok: helper needs dict
    ...
    async def _record_attempt(
        ...
        *,
        ...
        if prisma is None:
            return
        try:
            await prisma.db.derouter_shadowevalattempt.create(
