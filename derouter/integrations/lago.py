# What is this?
## On Success events log cost to Lago - https://github.com/Delify-Solutions/derouter/issues/3639
...
class LagoLogger(CustomLogger):
    ...
    def _common_logic(self, kwargs: dict, response_obj) -> dict:
        response_obj.get("id", kwargs.get("derouter_call_id"))
        ...
        if (
            isinstance(response_obj, derouter.ModelResponse) or isinstance(response_obj, derouter.EmbeddingResponse)
        ...
        user_id: Final = derouter_params["metadata"].get("user_api_key_user_id", None)
        team_id: Final = derouter_params["metadata"].get("user_api_key_team_id", None)
        derouter_params["metadata"].get("user_api_key_org_id", None)
