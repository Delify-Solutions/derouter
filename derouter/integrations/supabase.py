...
class Supabase:
    ...
    def __init__(self):
        ...
        if self.supabase_url is None or self.supabase_key is None:
            raise ValueError(
                "DeRouter Error, trying to use Supabase but url or key not passed. Create a table and set `derouter.supabase_url=<your-url>` and `derouter.supabase_key=<your-key>`"
            )
        self.supabase_client = supabase.create_client(self.supabase_url, self.supabase_key)

    def input_log_event(self, model, messages, end_user, derouter_call_id, print_verbose):
        try:
    ...
    def log_event(
        ...
        try:
            ...
            total_cost = derouter.completion_cost(completion_response=response_obj)
            ...
            if "choices" in response_obj:
                ...
                data, count = (
                    ...
                    .upsert(supabase_data_obj, on_conflict="derouter_call_id")
                    .execute()
                )
