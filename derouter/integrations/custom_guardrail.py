...
if TYPE_CHECKING:
    ...
    from derouter.llms.base_llm.guardrail_translation.base_translation import BaseTranslation
dc: Final = DualCache()
...
_guardrail_self_recorded: Final[contextvars.ContextVar[bool]] = contextvars.ContextVar(
    "derouter_guardrail_self_recorded", default=False
)
...
def _strict_guardrail_modes_enabled() -> bool:
    ...
    Set `DEROUTER_STRICT_GUARDRAIL_MODES=false` to keep the pre-LIT-4226 behavior
    ...
    """
    raw: Final = os.environ.get("DEROUTER_STRICT_GUARDRAIL_MODES")
    if raw is None:
        return True
...
def get_session_id_from_request_data(request_data: dict[str, Any]) -> str | None:
    """Extract session_id from request data (derouter_session_id or metadata)."""
    session_id = request_data.get("derouter_session_id")
    if session_id:
...
class CustomGuardrail(CustomLogger):
        ...
        if supported_event_hooks:
            ...
            try:
                ...
                verbose_logger.warning(
                    "%s. DEROUTER_STRICT_GUARDRAIL_MODES=false; continuing "
    ...
    def _get_admin_metadata(data: dict) -> dict:
        ...
        ``derouter_metadata`` depending on endpoint. Check both so a caller
    ...
    def _is_valid_response_type(self, result: object) -> bool:
        ...
        For non-DeRouter responses (like passthrough httpx.Response), returns True
    ...
    def get_guardrail_from_metadata(self, data: dict) -> list[str] | list[dict[str, DynamicGuardrailParams]]:
        ...
        # "derouter_metadata". We check the one that actually contains the "guardrails"
        # key so that a non-empty derouter_metadata without guardrails does not shadow
        ...
        for meta_key in ("metadata", "derouter_metadata"):
    ...
    def _deployment_hook_target(self) -> "CustomLogger":
        if not self.uses_apply_guardrail_interface() or self.use_native_lifecycle_hooks:
            return self
        try:
            from derouter.proxy.utils import unified_guardrail
        except ImportError as e:
            raise ImportError(
                f"Guardrail {self.guardrail_name or type(self).__name__} implements apply_guardrail, which needs "
                "the derouter proxy dependencies to run at the deployment level. "
                "Install them with: pip install 'derouter[proxy]'"
            ) from e
    ...
    async def async_pre_call_deployment_hook(self, kwargs: dict[str, Any], call_type: CallTypes | None) -> dict | None:
        ...
        derouter_guardrails: Final = kwargs.get("guardrails")
        if derouter_guardrails is None or not isinstance(derouter_guardrails, list):
            return kwargs
    ...
    async def async_post_call_success_deployment_hook(
        ...
        # should run guardrail
        derouter_guardrails: Final = request_data.get("guardrails")
        if derouter_guardrails is None or not isinstance(derouter_guardrails, list):
            return None
    ...
    async def async_logging_hook(
        ...
        from derouter.llms import get_guardrail_translation_mapping
        ...
        scratch_metadata: Final = {
            key: value
            for key, value in (derouter_params.get("metadata") or {}).items()
        ...
        if self.default_on is True and disable_global_guardrail is not True:
            if self._event_hook_is_event_type(event_type):
                if isinstance(self.event_hook, Mode):
                    try:
                        from derouter_enterprise.integrations.custom_guardrail import (
                            EnterpriseCustomGuardrailHelper,
                        )
                    except ImportError:
                        raise ImportError(
                            "Setting tag-based guardrails is only available in derouter-enterprise. You must be a premium user to use this feature."
                        )
    ...
    def get_guardrail_dynamic_request_body_params(self, request_data: dict) -> dict:
        ...
        Args:
            request_data: The original `request_data` passed to DeRouter Proxy
        """
    ...
    def add_standard_logging_guardrail_information_to_request_data(
        ...
        from derouter.types.utils import GuardrailMode
        ...
        try:
            from derouter.integrations.otel.logger import emit_guardrail_span
    ...
    def update_in_memory_derouter_params(self, derouter_params: LitellmParams) -> None:
        """
        Update the guardrails derouter params in memory
        """
        for key, value in vars(derouter_params).items():
    ...
    def get_guardrails_messages_for_call_type(
        ...
        #########################################################
        # /responses
        # User/System messages are stored in the "input" key, use derouter transformation to get the messages
        #########################################################
        if call_type == CallTypes.responses.value or call_type == CallTypes.aresponses.value:
            ...
            messages: Final = DeRouterCompletionResponsesConfig.transform_responses_api_input_to_messages(
                input=input_data,
...
def _append_slg_to_derouter_params(lp: object, entries: list) -> None:
    """Merge guardrail entries into a single derouter_params dict."""
...
def _sync_guardrail_info_to_logging_obj(request_data: dict, logging_obj: object) -> None:
    ...
    request_data["metadata"] or request_data["derouter_metadata"].  For
    passthrough routes (/v1/messages, /v1/responses) the spend-log payload is
    built from logging_obj.derouter_params["metadata"], which is a separate dict
    ...
    _append_slg_to_derouter_params(getattr(logging_obj, "derouter_params", None), entries)
    _append_slg_to_derouter_params(mcd.get("derouter_params"), entries)
...
def log_guardrail_information(func):
    ...
    async def async_wrapper(*args, **kwargs):
        ...
        logging_obj: Final = kwargs.get("logging_obj") or request_data.get("derouter_logging_obj")
