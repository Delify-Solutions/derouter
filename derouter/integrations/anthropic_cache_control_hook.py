...
Users can define
- `cache_control_injection_points` in the completion params and derouter will inject the cache control directives into the messages at the specified injection points.
...
from derouter.integrations.prompt_management_base import PromptManagementClient
...
from derouter.types.integrations.anthropic_cache_control_hook import (
...
def _model_map_prompt_cache_breakpoint_flag(model: str) -> bool | None:
    ...
    entries: Final = (derouter.model_cost.get(key) for key in (model, model.rsplit("/", 1)[-1]))
...
def targets_openai_api(api_base: object) -> bool:
    ...
    resolved: Final = next(
        (value for value in (api_base, derouter.api_base, *map(os.getenv, OPENAI_API_BASE_ENV_VARS)) if value),
        None,
    )
...
CARRY_UNMATCHED_MESSAGE_POINTS: Final = "_derouter_carry_unmatched_cache_control_points"
...
class AnthropicCacheControlHook(CustomPromptManagement):
    ...
    def get_chat_completion_prompt(
        ...
        stamped_dialect: Final = injection_points[0].get("_derouter_openai_dialect")
    ...
    def _default_control() -> ChatCompletionCachedContent:
        ...
        ``derouter.anthropic_prompt_caching_ttl`` override ("5m" or "1h").
        """
        import derouter

        ttl: Final = derouter.anthropic_prompt_caching_ttl
        if ttl == "5m" or ttl == "1h":
    ...
    def _stamped_as_judged(points: Sequence[CacheControlInjectionPoint]) -> Sequence[Mapping[str, object]]:
        ...
        """
        return AnthropicCacheControlHook._stamped(points, "_derouter_judged", True)
    ...
    def _should_stand_down(
        ...
        them, because by then the messages carry derouter's own injected marks
        and the judgment would misread those as client breakpoints.
        """
        if all(point.get("_derouter_judged") for point in points):
            return False
    ...
    def get_default_injection_points(
        ...
        """Default breakpoints when ``derouter.enable_anthropic_prompt_caching`` is on.
        ...
        if derouter.enable_anthropic_prompt_caching is not True and enable_prompt_caching is not True:
            return []
    ...
    def messages_with_default_injections(
        ...
        `derouter.acompletion`, so it has to reproduce the markers to derive the same cache key the
    ...
    def record_gateway_injection(
        ...
        Spend accounting only asks whether derouter acted, so what it needs is which
        ...
        request, and ``derouter_call_id`` is shared with it, so anything request-scoped
        ...
        name resolver, which answers on key presence: ``derouter_params`` declares
        ``derouter_metadata`` as None on every request, so the resolver names a bucket that
        ...
        bucket: Final = next(
            (
                candidate
                for candidate in (request_kwargs.get("derouter_metadata"), request_kwargs.get("metadata"))
    ...
    def maybe_inject_cache_control(
        ...
        ``derouter.enable_anthropic_prompt_caching`` or the per-request
