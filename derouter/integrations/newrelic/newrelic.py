...
Usage - Python SDK:
    import derouter
    derouter.callbacks = ["newrelic"]

    # Or with explicit configuration:
    from derouter.integrations.newrelic import NewRelicLogger
    derouter.callbacks = [NewRelicLogger(turn_off_message_logging=True)]
...
from derouter.types.utils import Message, ModelResponse, StandardLoggingPayload
...
class NewRelicLogger(CustomLogger):
    """
    New Relic logger for DeRouter to send AI monitoring events.
    ...
    def __init__(self, **kwargs):
        #########################################################
        # Handle newrelic_params set as derouter.newrelic_params
        #########################################################
        ...
        if not self.license_key or not self.app_name:
            ...
            verbose_logger.error(
                "New Relic Python agent not installed. Review the New Relic integration documentation at https://router.delify.vn/docs/observability/newrelic."
            )
            self.enabled = False
        else:
            try:
    ...
    def _get_newrelic_params(self) -> dict:
        """
        Get the newrelic_params from derouter.newrelic_params

        These are params specific to initializing the NewRelicLogger e.g. turn_off_message_logging
        """
        dict_newrelic_params: dict = {}
        if derouter.newrelic_params is not None:
            if isinstance(derouter.newrelic_params, NewRelicInitParams):
                dict_newrelic_params = derouter.newrelic_params.model_dump()
            elif isinstance(derouter.newrelic_params, dict):
                # only allow params that are of NewRelicInitParams
                dict_newrelic_params = NewRelicInitParams(**derouter.newrelic_params).model_dump()
    ...
    def _parse_bool_env(self, var_name: str, default: bool = False) -> bool:
        ...
        ``derouter/__init__.py`` and the standard library's
    ...
    def _get_derouter_version(self) -> str:
        """
        Get derouter version for supportability metrics.
        ...
        try:
            ...
            return version("derouter")
        except Exception as e:
            verbose_logger.warning("Unable to determine derouter version: %s", e)
            return "unknown"

    def _emit_supportability_metric(self):
        """
        Emit New Relic supportability metric for DeRouter usage.

        Per spec, this metric should be emitted at least once every 27 hours
        to indicate the library is in use. Format:
        Supportability/Python/ML/DeRouter/{version}

        This method updates _last_metric_emission_time and should
        be called within a lock when checking periodic emission.
        """
        try:
            derouter_version: Final = self._get_derouter_version()
            metric_name: Final = f"Supportability/Python/ML/DeRouter/{derouter_version}"
    ...
    def _get_trace_context(
        ...
        This integration runs in DeRouter's async logging worker, outside the
        ...
        Resolution order:
        1. W3C traceparent header (derouter_params.metadata.headers.traceparent) -
           what the agent would link to if we were in-transaction.
        2. StandardLoggingPayload.trace_id - DeRouter's internal trace for
    ...
    def _extract_completion_id(self, kwargs: dict, response_obj: ModelResponse) -> str:
        ...
        if not completion_id:
            completion_id = kwargs.get("derouter_call_id")
    ...
    def _get_vendor(
        ...
        if standard_logging_object:
            ...
            if vendor:
                return vendor
        derouter_params: Final = kwargs.get("derouter_params", {}) or {}
        return derouter_params.get("custom_llm_provider") or "derouter"
    ...
    def _get_duration(
        ...
        Resolution order:
        1. StandardLoggingPayload.response_time (already computed by DeRouter)
    ...
    def _extract_all_messages(
        ...
        # DeRouter's wider redaction decision (turn_off_message_logging, dynamic
        ...
        try:
            ...
            for message in messages:
                ...
                event_data: dict[str, object] = {
                    ...
                    "ingest_source": "derouter",
                    "token_count": 0,  # Per-message token counts are not available from DeRouter
                }
    ...
    def _record_error_metric(self):
        ...
        try:
            ...
            app: Final = _newrelic_agent.application()
            if app and app.enabled:
                app.record_custom_metric("LLM/DeRouter/Error", 1)
        except Exception as e:
    ...
    async def async_health_check(self) -> IntegrationHealthCheckStatus:
        ...
        `DeRouterConnectionTest` custom event so the user can confirm the
        end-to-end pipeline in the New Relic UI via NRQL:
        `SELECT * FROM DeRouterConnectionTest SINCE 1 hour ago`.

        The `DeRouterConnectionTest` event type is intentionally outside the
        ...
        try:
            ...
            app.record_custom_event(
                "DeRouterConnectionTest",
                {
                    "is_test_event": True,
                    "app_name": self.app_name,
                    "source": "derouter-proxy",
                    "timestamp": time.time(),
                },
            )
