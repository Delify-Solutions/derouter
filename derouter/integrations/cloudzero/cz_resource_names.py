...
"""CloudZero Resource Names (CZRN) generation and validation for DeRouter resources."""
...
class CZRNGenerator:
    """Generate CloudZero Resource Names (CZRNs) for DeRouter resources."""
    ...
    def create_from_derouter_data(self, row: dict[str, Any]) -> str:
        """Create a CZRN from DeRouter daily spend data.

        CZRN format: czrn:<service-type>:<provider>:<region>:<owner-account-id>:<resource-type>:<cloud-local-id>

        For DeRouter resources, we map:
        - service-type: 'derouter' (the service managing the LLM calls)
        - provider: The custom_llm_provider (e.g., 'openai', 'anthropic', 'azure')
        - region: 'cross-region' (DeRouter operates across regions)
    ...
    def _normalize_provider(self, provider: str) -> str:
        ...
        provider_map: Final = {
            derouter.LlmProviders.AZURE.value: "azure",
            derouter.LlmProviders.AZURE_AI.value: "azure",
            derouter.LlmProviders.ANTHROPIC.value: "anthropic",
            derouter.LlmProviders.BEDROCK.value: "aws",
            derouter.LlmProviders.VERTEX_AI.value: "gcp",
            derouter.LlmProviders.GEMINI.value: "google",
            derouter.LlmProviders.COHERE.value: "cohere",
            derouter.LlmProviders.HUGGINGFACE.value: "huggingface",
            derouter.LlmProviders.REPLICATE.value: "replicate",
            derouter.LlmProviders.TOGETHER_AI.value: "together-ai",
        }

        normalized: Final = provider.lower().replace("_", "-")

        # use derouter custom llm provider if not in provider_map
