...
"""Transform DeRouter data to CloudZero AnyCost CBF format."""
...
class CBFTransformer:
    """Transform DeRouter usage data to CloudZero Billing Format (CBF)."""
    ...
    def transform(self, data: pl.DataFrame) -> pl.DataFrame:
        """Transform DeRouter data to CBF format, dropping records with zero successful_requests or invalid CZRNs."""
        if data.is_empty():
            return pl.DataFrame()
    ...
    def _create_cbf_record(self, row: dict[str, object]) -> CBFRecord:
        """Create a single CBF record from DeRouter daily spend row."""
        ...
        resource_id: Final = self.czrn_generator.create_from_derouter_data(row)
