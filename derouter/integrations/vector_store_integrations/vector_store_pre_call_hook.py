...
from derouter.types.utils import CallTypes, StandardCallbackDynamicParams
...
class VectorStorePreCallHook(CustomLogger):
    ...
    2. Calls derouter.vector_stores.search() to get relevant context
    ...
    async def async_get_chat_completion_prompt(
        ...
        try:
            ...
            vector_stores_to_run: list[
                DeRouter_ManagedVectorStore
            ] = await derouter.vector_store_registry.pop_vector_stores_to_run_with_db_fallback(
            ...
            for vector_store_to_run in vector_stores_to_run:
                ...
                derouter_params_for_vector_store = vector_store_to_run.get("derouter_params", {}) or {}
                request_derouter_params = derouter_logging_obj.model_call_details.get("derouter_params", {})
                request_metadata = (
                    request_derouter_params.get("metadata", {}) if isinstance(request_derouter_params, dict) else {}
                )
                if llm_router is not None:
                    ...
                    search_function = cast(  # cast-ok: normalize SDK search callable
                        Callable[..., Awaitable[VectorStoreSearchResponse]],
                        derouter.vector_stores.asearch,
                    )
                try:
                    search_response = await search_function(
                        **{
                            ...
                            **derouter_params_for_vector_store,
                        },
                    )
            ...
            if derouter_logging_obj and all_search_results:
                derouter_logging_obj.model_call_details["search_results"] = all_search_results
    ...
    async def async_post_call_success_deployment_hook(
        ...
        try:
            ...
            derouter_logging_obj: Final = request_data.get("derouter_logging_obj")
            if not derouter_logging_obj:
                verbose_logger.debug("No derouter_logging_obj in request_data")
                return None

            verbose_logger.debug("model_call_details keys: %s", list(derouter_logging_obj.model_call_details.keys()))

            # Get search results from model_call_details (already in OpenAI format)
            search_results: Final[list[VectorStoreSearchResponse] | None] = derouter_logging_obj.model_call_details.get(
                "search_results"
            )
