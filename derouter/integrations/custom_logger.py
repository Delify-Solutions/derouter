...
from derouter.constants import DEFAULT_MAX_RECURSE_DEPTH_SENSITIVE_DATA_MASKER
from derouter.types.integrations.argilla import ArgillaItem
from derouter.types.integrations.custom_logger import AgenticLoopPlan
from derouter.types.llms.openai import AllMessageValues, ChatCompletionRequest
...
class CustomLogger:  # https://router.delify.vn/docs/observability/custom_callback#callback-class
    ...
    def get_callback_env_vars(callback_name: str | None = None) -> list[str]:
        ...
        try:
            from derouter.proxy._types import AllCallbacks
        except Exception:
            return []
        ...
        params: Final[Sequence[str] | None] = getattr(callback_info, "derouter_callback_params", None)
        if not params:
            return []
    ...
    #### ADAPTERS #### Allow calling 100+ LLMs in custom format - https://github.com/Delify-Solutions/derouter/pulls

    def translate_completion_input_params(self, kwargs) -> ChatCompletionRequest | None:
        """
        Translates the input params, from the provider's native format to the derouter.completion() format.
        """
    ...
    ):  # raise exception if invalid, return a str for the user to receive - if rejected, or return a modified dictionary for passing into derouter
        pass
    ...
    async def async_post_call_response_headers_hook(
        ...
        derouter_call_info: dict[str, object] | None = None,
    ) -> dict[str, str] | None:
        """
        ...
        Args:
            ...
            - derouter_call_info: Optional[Dict[str, Any]] - Normalized routing metadata:
    ...
    #### SINGLE-USE #### - https://router.delify.vn/docs/observability/custom_callback#using-your-custom-callback-function
    ...
    #########################################################
    # AGENTIC LOOP HOOKS (for derouter.messages + future completion support)
    #########################################################
    ...
    async def async_should_run_agentic_loop(
        ...
        - WebSearch: Intercept WebSearch tool calls for Bedrock/Claude, execute
          derouter.search(), return final answer with search results
        ...
        Flow:
        1. User calls derouter.messages.acreate(tools=[...])
    ...
    async def async_run_agentic_loop(
        ...
        2. Execute the tools (derouter.search, code execution, DB queries, etc.)
        3. Build assistant message with tool_use blocks
        4. Build user message with tool_result blocks containing results
        5. Make follow-up derouter.messages.acreate() call with results
        ...
        Args:
            ...
            logging_obj: DeRouter logging object
            stream: Whether response is streaming
            kwargs: Additional request parameters
        ...
        Example:
            ...
            search_results = await asyncio.gather(
                *[derouter.asearch(tc["input"]["query"]) for tc in tool_calls]
            )
    ...
    def _truncate_text(self, text: str, max_length: int) -> str:
        ...
        return (
            text[:max_length] + "...truncated by derouter, this logger does not support large content"
    ...
    def _select_metadata_field(self, request_kwargs: dict | None = None) -> str | None:
        ...
        1. If `derouter_metadata` is in the request kwargs, use it
        2. Otherwise, use `metadata`
        """
        from derouter.constants import DEROUTER_METADATA_FIELD, OLD_DEROUTER_METADATA_FIELD

        if request_kwargs is None:
            return None
        if DEROUTER_METADATA_FIELD in request_kwargs:
            return DEROUTER_METADATA_FIELD
        return OLD_DEROUTER_METADATA_FIELD
    ...
    def redact_standard_logging_payload_from_model_call_details(self, model_call_details: dict) -> dict:
        ...
        excluded_fields: Final[list[str] | None] = getattr(derouter, "standard_logging_payload_excluded_fields", None)
        ...
        if turn_off_message_logging:
            redacted_str: Final = "redacted-by-derouter"
