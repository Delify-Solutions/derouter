...
server-side using derouter router's search tools.
"""
...
from derouter.constants import DEROUTER_WEB_SEARCH_TOOL_NAME
...
from derouter.integrations.websearch_interception.tools import (
    ...
    get_derouter_web_search_tool_openai,
    get_derouter_web_search_tool_responses,
    is_anthropic_native_web_search_tool,
    is_web_search_tool,
    is_web_search_tool_chat_completion,
    is_web_search_tool_responses,
)
from derouter.integrations.websearch_interception.transformation import (
...
from derouter.types.integrations.websearch_interception import (
    AnthropicSearchQuery,
    AnthropicServerToolUseBlock,
    WebSearchInterceptionConfig,
)
from derouter.types.llms.anthropic import AnthropicThinkingParam
...
if TYPE_CHECKING:
    ...
    from derouter.utils import CustomStreamWrapper
...
class _SearchToolConfig(TypedDict, total=False):
    ...
    derouter_params: ReadOnly[_SearchToolLitellmParams | None]
...
class _DeploymentCallKwargsView(TypedDict):
    custom_llm_provider: ReadOnly[str]
    derouter_params: ReadOnly[_LitellmParamsProviderView]
    model: ReadOnly[str]
...
class WebSearchInterceptionLogger(CustomLogger):
    ...
    2. Executes derouter.asearch() for each query using router's search tools
    ...
    def __init__(
        ...
        Args:
            ...
            max_agentic_loops: How many follow-up model calls one intercepted request
                              ...
                              If None, DeRouter's default of 3 applies.
        """
        super().__init__()
    ...
    async def async_pre_call_deployment_hook(
        ...
        # Try top-level kwargs first, then nested derouter_params, then derive from model name
        call_kwargs_view: Final[_DeploymentCallKwargsView] = {
            "custom_llm_provider": kwargs.get("custom_llm_provider", ""),
            "derouter_params": kwargs.get("derouter_params", {}),
            "model": kwargs.get("model", ""),
        }
        custom_llm_provider = call_kwargs_view["custom_llm_provider"] or call_kwargs_view["derouter_params"].get(
            "custom_llm_provider", ""
        )
        if not custom_llm_provider:
            try:
                _, custom_llm_provider, _, _ = derouter.get_llm_provider(model=call_kwargs_view["model"])
            except Exception:
                custom_llm_provider = ""
        ...
        verbose_logger.debug("WebSearchInterception: Converting native web_search tools to DeRouter standard")
        ...
        # Convert native/custom web_search tools to DeRouter standard
        converted_tools: Final = []
        for tool in tools:
            if is_web_search_tool(tool):
                # Convert to DeRouter standard web search tool
                converted_tool = get_derouter_web_search_tool_openai()
                converted_tools.append(converted_tool)
                verbose_logger.debug(
                    "WebSearchInterception: Converted %s (type=%s) to %s",
                    tool.get("name", "unknown"),
                    tool.get("type", "none"),
                    DEROUTER_WEB_SEARCH_TOOL_NAME,
                )
            else:
    ...
    def _convert_responses_tools(
        ...
        """Convert Responses API web search tools to the DeRouter standard function tool."""
        if not any(is_web_search_tool_responses(tool) for tool in tools):
            return None

        verbose_logger.debug("WebSearchInterception: Converting Responses web_search tools to DeRouter standard")

        converted_tools: Final = [
            get_derouter_web_search_tool_responses() if is_web_search_tool_responses(tool) else tool for tool in tools
        ]
    ...
    def from_config_yaml(cls, config: WebSearchInterceptionConfig) -> "WebSearchInterceptionLogger":
        ...
        Args:
            config: Configuration dictionary from derouter_settings.websearch_interception_params
        ...
        Example:
            ...
            Usage:
                config = derouter_settings.get("websearch_interception_params", {})
    ...
    def _sync_forced_tool_choice(cls, tool_choice: object, converted_tools: Sequence[Mapping[str, object]]) -> object:
        """Repoint a forced ``tool_choice`` at ``derouter_web_search`` when it
        ...
        definition gets renamed to ``derouter_web_search``, an unrewritten
        ...
        if tool_choice.get("name") in converted_names:
            return tool_choice
        return {**tool_choice, "name": DEROUTER_WEB_SEARCH_TOOL_NAME}

    async def async_pre_request_hook(self, model: str, messages: list[dict], kwargs: dict) -> dict | None:
        """
        Pre-request hook to convert native web search tools to DeRouter standard.
        ...
        2. Convert them to DeRouter standard format (derouter_web_search)
        ...
        custom_llm_provider: Final = kwargs.get("derouter_params", {}).get("custom_llm_provider", "")
        ...
        # Convert native web search tools to DeRouter standard
        converted_tools: Final[list[dict[str, object]]] = []
        for tool in tools:
            if is_web_search_tool(tool):
                standard_tool = get_derouter_web_search_tool()
    ...
    async def async_should_run_agentic_loop(
        ...
        # Check if tools include any web search tool (DeRouter standard or native)
    ...
    async def async_should_run_chat_completion_agentic_loop(
        ...
        if not has_websearch_tool:
            verbose_logger.debug("WebSearchInterception: No derouter_web_search tool in request")
            return False, {}
    ...
    async def async_should_run_responses_agentic_loop(
        ...
        if not has_websearch_tool:
            verbose_logger.debug("WebSearchInterception: No derouter_web_search tool in responses request")
            return False, {}
    ...
    async def _build_responses_request_patch(
        ...
        """Execute derouter.asearch() and build a Responses API rerun patch."""
        search_tasks: Final = [
            (
            ...
            for item in (
                {
                    ...
                    "name": DEROUTER_WEB_SEARCH_TOOL_NAME,
    ...
    def _prepare_followup_kwargs(kwargs: dict) -> dict:
        ...
        ``derouter_logging_obj`` MUST be excluded so the follow-up call creates
        ...
        """
        _internal_keys: Final = {"derouter_logging_obj"}
        return {
    ...
    async def _build_anthropic_request_patch(
        ...
        """
        Execute derouter.search() and build follow-up request patch.
        ...
        _call_id: Final = getattr(logging_obj, "derouter_call_id", None) or kwargs.get("derouter_call_id", "unknown")
        ...
        try:
            ...
            try:
                ...
                verbose_logger.debug(
                    ...
                    "falling back to direct derouter.asearch() with perplexity"
                )
                llm_router = None

            search_tool: Final = self._select_search_tool_from_router(llm_router=llm_router)
            search_provider: str | None = None
            search_derouter_params: Mapping[str, object] = {}
            ...
            if search_tool is not None:
                ...
                tool_params: Final[_SearchToolLitellmParams] = search_tool.get("derouter_params", {}) or {}
                search_derouter_params = dict[str, object](tool_params)
    ...
    def _build_search_request_metadata(
        ...
        user_api_key_metadata: Final[StandardLoggingUserAPIKeyMetadata] = (
            DeRouterProxyRequestSetup.get_sanitized_user_information_from_key(user_api_key_dict=user_api_key_auth)
        )
        return {  # mutable-ok: derouter's metadata channel is a plain dict its logging path reads and enriches
    ...
    def _get_user_api_key_auth_from_kwargs(kwargs: Mapping[str, object] | None) -> "UserAPIKeyAuth | None":
        ...
        for metadata_key in ("metadata", "derouter_metadata"):
            metadata = derouter_params.get(metadata_key)
            if isinstance(metadata, dict) and metadata.get("user_api_key_auth") is not None:
                return metadata["user_api_key_auth"]
    ...
    def _select_search_tool_from_list(
        ...
        if self.search_tool_name:
            ...
            if matching_tools:
                search_provider = (matching_tools[0].get("derouter_params", {}) or {}).get("search_provider")
                verbose_logger.debug(
        ...
        if search_tools:
            ...
            search_provider = (first_tool.get("derouter_params", {}) or {}).get("search_provider")
            verbose_logger.debug(
    ...
    async def _build_chat_completion_request_patch(
        ...
        """Execute derouter.search() and build chat-completion rerun patch."""
    ...
    def initialize_from_proxy_config(
        derouter_settings: Mapping[str, WebSearchInterceptionConfig],
        ...
        Args:
            derouter_settings: Dictionary containing derouter_settings from proxy_config.yaml
        ...
        """
        # Get websearch_interception_params from derouter_settings or callback_specific_params
        websearch_params: WebSearchInterceptionConfig = {}
        if "websearch_interception_params" in derouter_settings:
            settings_view: Final[_WebSearchSettingsView] = {
                "websearch_interception_params": derouter_settings["websearch_interception_params"]
            }
