...
Transforms between Anthropic/OpenAI tool_use format and DeRouter search format.
"""
...
class WebSearchTransformation:
    ...
    - Anthropic tool_use format → DeRouter search requests
    - OpenAI tool_calls format → DeRouter search requests
    - DeRouter SearchResponse → Anthropic/OpenAI tool_result format
    """
    ...
    def _detect_from_responses_response(
        ...
        """Parse a Responses API response for ``derouter_web_search`` function calls.

        After pre-request conversion the native web search tool is replaced by a
        ``derouter_web_search`` function tool, so the model emits ``function_call``
        ...
        for item in output:
            ...
            if item_type != "function_call" or item_name != DEROUTER_WEB_SEARCH_TOOL_NAME:
                continue
    ...
    def _detect_from_non_streaming_response(
        ...
        for block in content:
            ...
            # ``derouter_web_search``; the bare ``web_search`` entry handles
            # callers that bypass our pre-request hooks (e.g. direct
            # derouter.acompletion). "WebSearch" is intentionally omitted —
    ...
    def transform_response(
        ...
        """
        Transform DeRouter search results to Anthropic/OpenAI tool_result format.
    ...
    def build_web_search_tool_result_block(
        ...
        Args:
            ...
            search_response: Structured ``SearchResponse`` from
                ``derouter.asearch()``. If None or empty, the block is still
    ...
    def format_search_response(result: SearchResponse) -> str:
        ...
        Args:
            result: SearchResponse from derouter.asearch()
