"""
DeRouter Web Search Tool Definition

This module defines the standard web search tool used across DeRouter.
...
def get_derouter_web_search_tool() -> dict[str, object]:
    """
    Get the standard DeRouter web search tool definition.
    ...
    Example:
        >>> tool = get_derouter_web_search_tool()
        >>> tool['name']
        'derouter_web_search'
    """
    return {
...
def get_derouter_web_search_tool_openai() -> dict[str, object]:
    """
    Get the standard DeRouter web search tool definition in OpenAI format.
...
def get_derouter_web_search_tool_responses() -> dict[str, object]:
    """
    Get the standard DeRouter web search tool definition in Responses API format.
...
def is_web_search_tool_responses(tool: Mapping[str, object]) -> bool:
    ...
    - The DeRouter standard function tool in Responses shape:
      ``{"type": "function", "name": "derouter_web_search"}``
    ...
    Example:
        ...
        True
        >>> is_web_search_tool_responses({"type": "function", "name": "derouter_web_search"})
        True
    ...
    if tool_type == "function":
        return tool.get("name") == DEROUTER_WEB_SEARCH_TOOL_NAME
...
def is_web_search_tool_chat_completion(tool: dict[str, Any]) -> bool:
    ...
    This is a stricter version that ONLY checks for the exact DeRouter web search tool name.
    Use this for Chat Completions API to avoid false positives with user-defined tools.

    Detects ONLY:
    - DeRouter standard: name == "derouter_web_search" (Anthropic format)
    - OpenAI format: type == "function" with function.name == "derouter_web_search"

    Args:
        tool: Tool dictionary to check

    Returns:
        True if tool is exactly the DeRouter web search tool

    Example:
        >>> is_web_search_tool_chat_completion({"name": "derouter_web_search"})
        True
        >>> is_web_search_tool_chat_completion({"type": "function", "function": {"name": "derouter_web_search"}})
        True
    ...
    # Check for OpenAI format: {"type": "function", "function": {"name": "derouter_web_search"}}
    if tool_type == "function" and "function" in tool:
        function_def: Final = tool.get("function", {})
        function_name: Final = function_def.get("name", "")
        if function_name == DEROUTER_WEB_SEARCH_TOOL_NAME:
            return True

    # Check for DeRouter standard tool (Anthropic format)
...
def is_anthropic_native_web_search_tool(tool: Mapping[str, object]) -> bool:
    ...
    without affecting clients that send the DeRouter standard tool.

    Returns False for the DeRouter standard tool (``derouter_web_search``),
...
def is_web_search_tool(tool: dict[str, Any]) -> bool:
    """
    Check if a tool is a web search tool (native or DeRouter standard).

    Detects:
    - DeRouter standard: name == "derouter_web_search"
    - OpenAI format: type == "function" with function.name == "derouter_web_search"
    ...
    Example:
        >>> is_web_search_tool({"name": "derouter_web_search"})
        True
        >>> is_web_search_tool({"type": "function", "function": {"name": "derouter_web_search"}})
        True
