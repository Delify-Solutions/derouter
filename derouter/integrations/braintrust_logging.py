...
from derouter.integrations.braintrust_mock_client import (
...
class BraintrustLogger(CustomLogger):
    ...
    async def create_default_project_and_experiment(self):
        project: Final = await self.global_braintrust_http_handler.post(
            f"{self.api_base}/project", headers=self.headers, json={"name": "derouter"}
        )
    ...
    def log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        try:
            derouter_call_id: Final = kwargs.get("derouter_call_id")
            ...
            dynamic_metadata: Final = derouter_params.get("metadata", {}) or {}
            ...
            if usage_obj and isinstance(usage_obj, derouter.Usage):
                derouter.utils.get_logging_id(start_time, response_obj)
                metrics = {
