...
from derouter.integrations.deepeval.api import Api, Endpoints, HttpMethods
from derouter.integrations.deepeval.types import (
    BaseApiSpan,
    SpanApiType,
    TraceApi,
    TraceSpanApiStatus,
)
from derouter.integrations.deepeval.utils import (
...
class DeepEvalLogger(CustomLogger):
    """Logs derouter traces to DeepEval's platform."""

    def __init__(self, *args, **kwargs):
        api_key: Final = os.getenv("CONFIDENT_API_KEY")
        self.derouter_environment = os.getenv("LITELM_ENVIRONMENT", "development")
        validate_environment(self.derouter_environment)
        if not api_key:
    ...
    def _prepare_trace_api(self, kwargs, response_obj, start_time, end_time, is_success):
        ...
        trace_api: Final = self._create_trace_api(
            ...
            start_time=_start_time,
            end_time=_end_time,
            derouter_environment=self.derouter_environment,
        )
    ...
    def _create_base_api_span(self, kwargs, standard_logging_object, start_time, end_time, is_success):
        ...
        return BaseApiSpan(
            ...
            name=("derouter_success_callback" if is_success else "derouter_failure_callback"),
    ...
    def _create_trace_api(
        ...
        return TraceApi(
            ...
            startTime=str(start_time),
            endTime=str(end_time),
            environment=derouter_environment,
        )
