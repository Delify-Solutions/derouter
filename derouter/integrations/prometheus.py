# used for /metrics endpoint on DeRouter Proxy
#### What this does ####
...
import derouter
from derouter._logging import print_verbose, verbose_logger
from derouter.exceptions import (
    validate_rate_limit_category,
    validate_rate_limit_type,
)
from derouter.integrations.custom_logger import CustomLogger
from derouter.integrations.prometheus_helpers import (
    PrometheusLabelFactoryContext,
    _get_cached_end_user_id_for_cost_tracking,
)
from derouter.integrations.prometheus_helpers.bounded_prometheus_series_tracker import (
...
from derouter.derouter_core_utils.service_tier_utils import (
    get_service_tier_from_standard_logging_payload,
)
from derouter.proxy._types import (
    DeRouter_DeletedVerificationToken,
    DeRouter_TeamTable,
    DeRouter_UserTable,
    UserAPIKeyAuth,
)
from derouter.repositories.base_repository import BaseRepository
from derouter.repositories.organization_repository import OrganizationRepository
...
class PrometheusLogger(CustomLogger):
    ...
    def get_instance() -> PrometheusLogger | None:
        """Find the PrometheusLogger instance from derouter.callbacks, if registered."""
        import derouter

        for cb in derouter.callbacks:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            # (e.g. ``derouter.prometheus_emit_stream_label``,
            # ``derouter.prometheus_emit_rate_limit_labels``) that can be
            ...
            _custom_buckets: Final = derouter.prometheus_latency_buckets
            ...
            self.derouter_proxy_failed_requests_metric = self._counter_factory(
                name="derouter_proxy_failed_requests_metric",
                documentation="Total number of failed responses from proxy - the client did not get a success response from derouter proxy",
                labelnames=self.get_labels_for_metric("derouter_proxy_failed_requests_metric"),
            )

            self.derouter_proxy_total_requests_metric = self._counter_factory(
                name="derouter_proxy_total_requests_metric",
                documentation="Total number of requests made to the proxy server - track number of client side requests",
                labelnames=self.get_labels_for_metric("derouter_proxy_total_requests_metric"),
            )

            # request latency metrics
            self.derouter_request_total_latency_metric = self._histogram_factory(
                "derouter_request_total_latency_metric",
                "End-to-end latency (seconds) for a request to DeRouter Proxy Server, from the moment "
                ...
                labelnames=self.get_labels_for_metric("derouter_request_total_latency_metric"),
                buckets=self.latency_buckets,
            )

            self.derouter_llm_api_latency_metric = self._histogram_factory(
                "derouter_llm_api_latency_metric",
                "Total latency (seconds) for a models LLM API call",
                labelnames=self.get_labels_for_metric("derouter_llm_api_latency_metric"),
                buckets=self.latency_buckets,
            )

            self.derouter_llm_api_time_to_first_token_metric = self._histogram_factory(
                ...
                #     "team_alias",
                # ],
                labelnames=self.get_labels_for_metric("derouter_llm_api_time_to_first_token_metric"),
                buckets=self.latency_buckets,
            )

            # Counter for spend
            self.derouter_spend_metric = self._counter_factory(
                "derouter_spend_metric",
                "Total spend on LLM requests",
                labelnames=self.get_labels_for_metric("derouter_spend_metric"),
            )

            # Counter for total_output_tokens
            self.derouter_tokens_metric = self._counter_factory(
                "derouter_total_tokens_metric",
                "Total number of input + output tokens from LLM requests",
                labelnames=self.get_labels_for_metric("derouter_total_tokens_metric"),
            )

            self.derouter_input_tokens_metric = self._counter_factory(
                "derouter_input_tokens_metric",
                "Total number of input tokens from LLM requests",
                labelnames=self.get_labels_for_metric("derouter_input_tokens_metric"),
            )

            self.derouter_output_tokens_metric = self._counter_factory(
                "derouter_output_tokens_metric",
                "Total number of output tokens from LLM requests",
                labelnames=self.get_labels_for_metric("derouter_output_tokens_metric"),
            )
            ...
            self.derouter_input_cached_tokens_metric = self._counter_factory(
                ...
                labelnames=self.get_labels_for_metric("derouter_input_cached_tokens_metric"),
            )

            self.derouter_input_cache_creation_tokens_metric = self._counter_factory(
                ...
                labelnames=self.get_labels_for_metric("derouter_input_cache_creation_tokens_metric"),
            )

            self.derouter_input_audio_tokens_metric = self._counter_factory(
                "derouter_input_audio_tokens_metric",
                "Audio input tokens reported in prompt_tokens_details.audio_tokens",
                labelnames=self.get_labels_for_metric("derouter_input_audio_tokens_metric"),
            )

            self.derouter_output_reasoning_tokens_metric = self._counter_factory(
                "derouter_output_reasoning_tokens_metric",
                "Reasoning tokens reported in completion_tokens_details.reasoning_tokens",
                labelnames=self.get_labels_for_metric("derouter_output_reasoning_tokens_metric"),
            )

            self.derouter_output_audio_tokens_metric = self._counter_factory(
                "derouter_output_audio_tokens_metric",
                "Audio output tokens reported in completion_tokens_details.audio_tokens",
                labelnames=self.get_labels_for_metric("derouter_output_audio_tokens_metric"),
            )

            self.derouter_video_duration_seconds_metric = self._counter_factory(
                "derouter_video_duration_seconds_metric",
                "Seconds of video generated, from usage.duration_seconds on video generation calls",
                labelnames=self.get_labels_for_metric("derouter_video_duration_seconds_metric"),
            )

            self.derouter_images_generated_metric = self._counter_factory(
                "derouter_images_generated_metric",
                "Number of images generated, from the image generation response",
                labelnames=self.get_labels_for_metric("derouter_images_generated_metric"),
            )

            # Remaining Budget for Team
            self.derouter_remaining_team_budget_metric = self._gauge_factory(
                "derouter_remaining_team_budget_metric",
                "Remaining budget for team",
                labelnames=self.get_labels_for_metric("derouter_remaining_team_budget_metric"),
            )

            # Max Budget for Team
            self.derouter_team_max_budget_metric = self._gauge_factory(
                "derouter_team_max_budget_metric",
                "Maximum budget set for team",
                labelnames=self.get_labels_for_metric("derouter_team_max_budget_metric"),
            )

            # Team Budget Reset At
            self.derouter_team_budget_remaining_hours_metric = self._gauge_factory(
                "derouter_team_budget_remaining_hours_metric",
                "Remaining days for team budget to be reset",
                labelnames=self.get_labels_for_metric("derouter_team_budget_remaining_hours_metric"),
            )

            # Number of members in a team
            self.derouter_team_members_metric = self._gauge_factory(
                "derouter_team_members_metric",
                "Number of members in a team",
                labelnames=self.get_labels_for_metric("derouter_team_members_metric"),
            )

            # Remaining Budget for Org
            self.derouter_remaining_org_budget_metric = self._gauge_factory(
                "derouter_remaining_org_budget_metric",
                "Remaining budget for org",
                labelnames=self.get_labels_for_metric("derouter_remaining_org_budget_metric"),
            )

            # Max Budget for Org
            self.derouter_org_max_budget_metric = self._gauge_factory(
                "derouter_org_max_budget_metric",
                "Maximum budget set for org",
                labelnames=self.get_labels_for_metric("derouter_org_max_budget_metric"),
            )

            # Org Budget Reset At
            self.derouter_org_budget_remaining_hours_metric = self._gauge_factory(
                "derouter_org_budget_remaining_hours_metric",
                "Remaining hours for org budget to be reset",
                labelnames=self.get_labels_for_metric("derouter_org_budget_remaining_hours_metric"),
            )

            # Remaining Budget for API Key
            self.derouter_remaining_api_key_budget_metric = self._gauge_factory(
                "derouter_remaining_api_key_budget_metric",
                "Remaining budget for api key",
                labelnames=self.get_labels_for_metric("derouter_remaining_api_key_budget_metric"),
            )

            # Max Budget for API Key
            self.derouter_api_key_max_budget_metric = self._gauge_factory(
                "derouter_api_key_max_budget_metric",
                "Maximum budget set for api key",
                labelnames=self.get_labels_for_metric("derouter_api_key_max_budget_metric"),
            )

            self.derouter_api_key_budget_remaining_hours_metric = self._gauge_factory(
                "derouter_api_key_budget_remaining_hours_metric",
                "Remaining hours for api key budget to be reset",
                labelnames=self.get_labels_for_metric("derouter_api_key_budget_remaining_hours_metric"),
            )

            # Remaining Budget for User
            self.derouter_remaining_user_budget_metric = self._gauge_factory(
                "derouter_remaining_user_budget_metric",
                "Remaining budget for user",
                labelnames=self.get_labels_for_metric("derouter_remaining_user_budget_metric"),
            )

            # Max Budget for User
            self.derouter_user_max_budget_metric = self._gauge_factory(
                "derouter_user_max_budget_metric",
                "Maximum budget set for user",
                labelnames=self.get_labels_for_metric("derouter_user_max_budget_metric"),
            )

            self.derouter_user_budget_remaining_hours_metric = self._gauge_factory(
                "derouter_user_budget_remaining_hours_metric",
                "Remaining hours for user budget to be reset",
                labelnames=self.get_labels_for_metric("derouter_user_budget_remaining_hours_metric"),
            )

            ########################################
            # DeRouter Virtual API KEY metrics
            ########################################

            # Remaining MODEL RPM limit for API Key
            self.derouter_remaining_api_key_requests_for_model = self._gauge_factory(
                "derouter_remaining_api_key_requests_for_model",
                "Remaining Requests API Key can make for model (model based rpm limit on key)",
                labelnames=self.get_labels_for_metric("derouter_remaining_api_key_requests_for_model"),
            )

            # Remaining MODEL TPM limit for API Key
            self.derouter_remaining_api_key_tokens_for_model = self._gauge_factory(
                "derouter_remaining_api_key_tokens_for_model",
                "Remaining Tokens API Key can make for model (model based tpm limit on key)",
                labelnames=self.get_labels_for_metric("derouter_remaining_api_key_tokens_for_model"),
            )

            self.derouter_api_key_rate_limit_allowed_metric = self._gauge_factory(
                ...
                labelnames=self.get_labels_for_metric("derouter_api_key_rate_limit_allowed_metric"),
            )

            self.derouter_api_key_rate_limit_used_metric = self._gauge_factory(
                ...
                labelnames=self.get_labels_for_metric("derouter_api_key_rate_limit_used_metric"),
            )

            self.derouter_team_rate_limit_allowed_metric = self._gauge_factory(
                ...
                labelnames=self.get_labels_for_metric("derouter_team_rate_limit_allowed_metric"),
            )

            self.derouter_team_rate_limit_used_metric = self._gauge_factory(
                ...
                labelnames=self.get_labels_for_metric("derouter_team_rate_limit_used_metric"),
            )
            ...
            self.derouter_remaining_requests_metric = self._gauge_factory(
                "derouter_remaining_requests_metric",
                "LLM Deployment Analytics - remaining requests for model, returned from LLM API Provider",
                labelnames=self.get_labels_for_metric("derouter_remaining_requests_metric"),
            )

            self.derouter_remaining_tokens_metric = self._gauge_factory(
                "derouter_remaining_tokens_metric",
                "remaining tokens for model, returned from LLM API Provider",
                labelnames=self.get_labels_for_metric("derouter_remaining_tokens_metric"),
            )

            self.derouter_overhead_latency_metric = self._histogram_factory(
                "derouter_overhead_latency_metric",
                "Latency overhead (milliseconds) added by DeRouter processing",
                labelnames=self.get_labels_for_metric("derouter_overhead_latency_metric"),
                buckets=self.latency_buckets,
            )

            self.derouter_overhead_with_guardrails_latency_metric = self._histogram_factory(
                "derouter_overhead_with_guardrails_latency_metric",
                "Total internal latency (seconds) added by DeRouter, including "
                "pre/post-call guardrails (excludes the LLM API call)",
                labelnames=self.get_labels_for_metric("derouter_overhead_with_guardrails_latency_metric"),
                buckets=self.latency_buckets,
            )

            # Request queue time metric
            self.derouter_request_queue_time_metric = self._histogram_factory(
                ...
                labelnames=self.get_labels_for_metric("derouter_request_queue_time_seconds"),
                buckets=self.latency_buckets,
            )

            # Guardrail metrics
            self.derouter_guardrail_latency_metric = self._histogram_factory(
            ...
            self.derouter_guardrail_errors_total = self._counter_factory(
            ...
            self.derouter_guardrail_requests_total = self._counter_factory(
            ...
            self.derouter_provider_remaining_budget_metric = self._gauge_factory(
                "derouter_provider_remaining_budget_metric",
                "Remaining budget for provider - used when you set provider budget limits",
                labelnames=["api_provider"],
            )

            # Metric for deployment state
            self.derouter_deployment_state = self._gauge_factory(
                "derouter_deployment_state",
                "LLM Deployment Analytics - The state of the deployment: 0 = healthy, 1 = partial outage, 2 = complete outage",
                labelnames=self.get_labels_for_metric("derouter_deployment_state"),
            )

            self.derouter_deployment_tpm_limit = self._gauge_factory(
                "derouter_deployment_tpm_limit",
                "Deployment TPM limit found in config",
                labelnames=self.get_labels_for_metric("derouter_deployment_tpm_limit"),
            )

            self.derouter_deployment_rpm_limit = self._gauge_factory(
                "derouter_deployment_rpm_limit",
                "Deployment RPM limit found in config",
                labelnames=self.get_labels_for_metric("derouter_deployment_rpm_limit"),
            )

            self.derouter_deployment_cooled_down = self._counter_factory(
                "derouter_deployment_cooled_down",
                "LLM Deployment Analytics - Number of times a deployment has been cooled down by DeRouter load balancing logic. exception_status is the status of the exception that caused the deployment to be cooled down",
                # labelnames=_logged_llm_labels + [EXCEPTION_STATUS],
                labelnames=self.get_labels_for_metric("derouter_deployment_cooled_down"),
            )

            self.derouter_deployment_success_responses = self._counter_factory(
                name="derouter_deployment_success_responses",
                documentation="LLM Deployment Analytics - Total number of successful LLM API calls via derouter",
                labelnames=self.get_labels_for_metric("derouter_deployment_success_responses"),
            )
            self.derouter_deployment_failure_responses = self._counter_factory(
                name="derouter_deployment_failure_responses",
                ...
                labelnames=self.get_labels_for_metric("derouter_deployment_failure_responses"),
            )

            self.derouter_deployment_total_requests = self._counter_factory(
                name="derouter_deployment_total_requests",
                documentation="LLM Deployment Analytics - Total number of LLM API calls via derouter - success + failure",
                labelnames=self.get_labels_for_metric("derouter_deployment_total_requests"),
            )

            # Deployment Latency tracking
            self.derouter_deployment_latency_per_output_token = self._histogram_factory(
                name="derouter_deployment_latency_per_output_token",
                documentation="LLM Deployment Analytics - Latency per output token",
                labelnames=self.get_labels_for_metric("derouter_deployment_latency_per_output_token"),
            )

            self.derouter_deployment_successful_fallbacks = self._counter_factory(
                ...
                self.get_labels_for_metric("derouter_deployment_successful_fallbacks"),
            )

            self.derouter_deployment_failed_fallbacks = self._counter_factory(
                ...
                self.get_labels_for_metric("derouter_deployment_failed_fallbacks"),
            )

            # Callback Logging Failure Metrics
            self.derouter_callback_logging_failures_metric = self._counter_factory(
                name="derouter_callback_logging_failures_metric",
                documentation="Total number of failures when emitting logs to callbacks (e.g. s3_v2, langfuse, etc)",
                labelnames=["callback_name"],
            )

            self.derouter_llm_api_failed_requests_metric = self._counter_factory(
                name="derouter_llm_api_failed_requests_metric",
                documentation="deprecated - use derouter_proxy_failed_requests_metric",
                labelnames=self.get_labels_for_metric("derouter_llm_api_failed_requests_metric"),
            )

            self.derouter_requests_metric = self._counter_factory(
                name="derouter_requests_metric",
                documentation="deprecated - use derouter_proxy_total_requests_metric. Total number of LLM calls to derouter - track total per API Key, team, user",
                labelnames=self.get_labels_for_metric("derouter_requests_metric"),
            )

            # Cache metrics
            self.derouter_cache_hits_metric = self._counter_factory(
                name="derouter_cache_hits_metric",
                documentation="Total number of DeRouter cache hits",
                labelnames=self.get_labels_for_metric("derouter_cache_hits_metric"),
            )

            self.derouter_cache_misses_metric = self._counter_factory(
                name="derouter_cache_misses_metric",
                documentation="Total number of DeRouter cache misses",
                labelnames=self.get_labels_for_metric("derouter_cache_misses_metric"),
            )

            self.derouter_cached_tokens_metric = self._counter_factory(
                name="derouter_cached_tokens_metric",
                documentation="Total tokens served from DeRouter cache",
                labelnames=self.get_labels_for_metric("derouter_cached_tokens_metric"),
            )

            # Provider prompt-caching metrics
            self.derouter_provider_cache_read_input_tokens_metric = self._counter_factory(
                name="derouter_provider_cache_read_input_tokens_metric",
                documentation="Total prompt/input tokens read from provider prompt cache (e.g. OpenAI/Anthropic/Gemini/Bedrock)",
                labelnames=self.get_labels_for_metric("derouter_provider_cache_read_input_tokens_metric"),
            )

            self.derouter_provider_cache_creation_input_tokens_metric = self._counter_factory(
                name="derouter_provider_cache_creation_input_tokens_metric",
                documentation="Total prompt/input tokens written to provider prompt cache (e.g. Anthropic/Bedrock)",
                labelnames=self.get_labels_for_metric("derouter_provider_cache_creation_input_tokens_metric"),
            )

            # User and Team count metrics
            self.derouter_total_users_metric = self._gauge_factory(
                "derouter_total_users",
                "Total number of users in DeRouter",
                labelnames=[],
            )

            self.derouter_active_users_metric = self._gauge_factory(
                "derouter_active_users",
                "Number of billable users in DeRouter (excludes SCIM-deactivated users)",
                labelnames=[],
            )

            self.derouter_teams_count_metric = self._gauge_factory(
                "derouter_teams_count",
                "Total number of teams in DeRouter",
                labelnames=[],
            )
            ...
            ########################################
            self.derouter_managed_batch_created_total = self._counter_factory(
                name="derouter_managed_batch_created_total",
            ...
            self.derouter_managed_file_size_bytes = self._gauge_factory(
            ...
            self.derouter_managed_batch_duration_seconds = self._histogram_factory(
            ...
            self.derouter_managed_file_created_total = self._counter_factory(
                name="derouter_managed_file_created_total",
            ...
            self.derouter_managed_file_deleted_total = self._counter_factory(
                name="derouter_managed_file_deleted_total",
                documentation="Total number of managed file deletions (success or blocked)",
                labelnames=["result"],
            )

            self.derouter_check_batch_cost_jobs_polled = self._gauge_factory(
                "derouter_check_batch_cost_jobs_polled",
                "Number of unprocessed batches found by the last CheckBatchCost poll",
                labelnames=[],
            )

            self.derouter_check_batch_cost_jobs_processed_total = self._counter_factory(
                name="derouter_check_batch_cost_jobs_processed_total",
                documentation="Total number of batches successfully cost-tracked by CheckBatchCost",
                labelnames=["model", "api_provider"],
            )

            self.derouter_check_batch_cost_errors_total = self._counter_factory(
                name="derouter_check_batch_cost_errors_total",
                documentation="Total number of errors in CheckBatchCost by error type",
                labelnames=["error_type"],
            )

            self.derouter_check_batch_cost_last_run_timestamp = self._gauge_factory(
            ...
            ########################################
            self.derouter_mcp_tool_calls_total = self._counter_factory(
                name="derouter_mcp_tool_calls_total",
                documentation="Total MCP tool calls, segmented by tool and server name",
                labelnames=self.get_labels_for_metric("derouter_mcp_tool_calls_total"),
            )

            self.derouter_mcp_tool_call_spend_metric = self._counter_factory(
                name="derouter_mcp_tool_call_spend_metric",
                documentation="Total spend on MCP tool calls, segmented by tool and server name",
                labelnames=self.get_labels_for_metric("derouter_mcp_tool_call_spend_metric"),
            )
    ...
    def _parse_prometheus_config(self) -> dict[str, list[str]]:
        ...
        import derouter
        from derouter.types.integrations.prometheus import PrometheusMetricsConfig

        config: Final = derouter.prometheus_metrics_config
    ...
    def _parse_exclude_config(self) -> tuple[frozenset[str], frozenset[str]]:
        ...
        exclude_metrics: Final = frozenset(derouter.prometheus_exclude_metrics or ())
        exclude_labels: Final = frozenset(derouter.prometheus_exclude_labels or ())
    ...
    def _all_defined_labels() -> frozenset[str]:
        ...
        custom_metadata_labels: Final = frozenset(
            _sanitize_prometheus_label_name(label) for label in derouter.custom_prometheus_metadata_labels
        )
        custom_tag_labels: Final = frozenset(
            _sanitize_prometheus_label_name(f"tag_{tag}") for tag in derouter.custom_prometheus_tags
        )
    ...
    def get_labels_for_metric(self, metric_name: DEFINED_PROMETHEUS_METRICS) -> list[str]:
        ...
        ``derouter.prometheus_emit_rate_limit_labels``,
        ``derouter.prometheus_emit_stream_label``) are flipped after the
    ...
    def _set_overhead_with_guardrails_metric(
        ...
        """Record derouter_overhead_with_guardrails_latency_metric (seconds): SDK overhead +
        pre/post-call guardrail time. Recorded outside the SDK-overhead gate so
        guardrail-only overhead is still captured when derouter_overhead_time_ms
        is 0 or absent.
        """
        derouter_overhead_time_ms: Final = standard_logging_payload["hidden_params"].get("derouter_overhead_time_ms")
        guardrail_overhead_seconds: Final = self._get_guardrail_overhead_seconds(standard_logging_payload)
        if derouter_overhead_time_ms is None and guardrail_overhead_seconds <= 0:
            return
        labels: Final = prometheus_label_factory(
            supported_enum_labels=self.get_labels_for_metric(
                metric_name="derouter_overhead_with_guardrails_latency_metric"
            ),
            enum_values=enum_values,
            label_context=label_context,
        )
        self.derouter_overhead_with_guardrails_latency_metric.labels(**labels).observe(
            ((derouter_overhead_time_ms or 0.0) / 1000) + guardrail_overhead_seconds
        )
    ...
    def _track_end_user_metric_series(
        ...
        max_series: Final = derouter.prometheus_end_user_metrics_max_series_per_metric
        ttl_seconds: Final = derouter.prometheus_end_user_metrics_ttl_seconds
        ...
        self._bounded_prometheus_series_tracker.track_series(
            ...
            cleanup_interval_seconds=derouter.prometheus_end_user_metrics_cleanup_interval_seconds,
        )
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        derouter_params: Final = kwargs.get("derouter_params", {}) or {}
        _metadata: Final = derouter_params.get("metadata") or {}
        get_end_user_id_for_cost_tracking: Final = _get_cached_end_user_id_for_cost_tracking()

        end_user_id: Final = get_end_user_id_for_cost_tracking(derouter_params, service_type="prometheus")
        ...
        enum_values: Final = UserAPIKeyLabelValues(
            ...
            stream=(str(standard_logging_payload.get("stream")) if derouter.prometheus_emit_stream_label else None),
        ...
        if user_api_key is not None and isinstance(user_api_key, str) and user_api_key.startswith("sk-"):
            from derouter.proxy.utils import hash_token
        ...
        # increment derouter_proxy_total_requests_metric for all successful requests
        ...
        PrometheusLogger._inc_labeled_counter(
            self,
            self.derouter_proxy_total_requests_metric,
    ...
    def _increment_token_metrics(
        ...
        PrometheusLogger._inc_labeled_counter(
            self,
            self.derouter_input_tokens_metric,
            "derouter_input_tokens_metric",
            enum_values,
            label_context=label_context,
        ...
        PrometheusLogger._inc_labeled_counter(
            self,
            self.derouter_output_tokens_metric,
            "derouter_output_tokens_metric",
            enum_values,
            label_context=label_context,
    ...
    def _increment_media_generation_metrics(
        ...
        media_metrics: Final[list[tuple[_MetricLike, DEFINED_PROMETHEUS_METRICS, object]]] = [
            (
                self.derouter_video_duration_seconds_metric,
                "derouter_video_duration_seconds_metric",
                usage_object.get("duration_seconds"),
            ),
            (
                self.derouter_images_generated_metric,
                "derouter_images_generated_metric",
    ...
    def _increment_cache_metrics(
        ...
        if cache_hit is None:
            # Historically these metrics only tracked DeRouter caching.
            ...
            PrometheusLogger._inc_labeled_counter(
                self,
                self.derouter_cache_hits_metric,
            ...
            if total_tokens > 0:
                PrometheusLogger._inc_labeled_counter(
                    self,
                    self.derouter_cached_tokens_metric,
            ...
            PrometheusLogger._inc_labeled_counter(
                self,
                self.derouter_cache_misses_metric,
                "derouter_cache_misses_metric",
                enum_values,
                label_context=label_context,
            )

        # Provider prompt caching metrics are independent of DeRouter cache_hit.
        ...
        if isinstance(usage_obj, dict):
            ...
            if provider_cache_read_tokens > 0:
                PrometheusLogger._inc_labeled_counter(
                    self,
                    self.derouter_provider_cache_read_input_tokens_metric,
            ...
            if provider_cache_creation_tokens > 0:
                PrometheusLogger._inc_labeled_counter(
                    self,
                    self.derouter_provider_cache_creation_input_tokens_metric,
    ...
    def _increment_mcp_tool_call_metrics(
        ...
        PrometheusLogger._inc_labeled_counter(
            self,
            self.derouter_mcp_tool_calls_total,
        ...
        if response_cost > 0:
            PrometheusLogger._inc_labeled_counter(
                self,
                self.derouter_mcp_tool_call_spend_metric,
    ...
    async def _increment_remaining_budget_metrics(
        ...
        if (
            isinstance(self.derouter_remaining_team_budget_metric, NoOpMetric)
            and isinstance(self.derouter_remaining_api_key_budget_metric, NoOpMetric)
            and isinstance(self.derouter_remaining_user_budget_metric, NoOpMetric)
            and isinstance(self.derouter_remaining_org_budget_metric, NoOpMetric)
        ):
            return
    ...
    def _get_remaining_from_v3_rate_limit_headers(
        ...
        of the ``derouter-key-remaining-*`` metadata keys the legacy limiter
    ...
    def _set_key_and_team_rate_limit_metrics(
        ...
        descriptor_gauges: Final[
            ...
            (
                "api_key",
                "derouter_api_key_rate_limit_allowed_metric",
                self.derouter_api_key_rate_limit_allowed_metric,
                self.derouter_api_key_rate_limit_used_metric,
            ),
            (
                "team",
                "derouter_team_rate_limit_allowed_metric",
                self.derouter_team_rate_limit_allowed_metric,
                self.derouter_team_rate_limit_used_metric,
            ),
        )
    ...
    def _set_virtual_key_rate_limit_metrics(
        ...
        from derouter.proxy.common_utils.callback_utils import (
            get_model_group_from_derouter_kwargs,
        )

        # Set remaining rpm/tpm for API Key + model
        # see parallel_request_limiter.py - variables are set there
        model_group: Final = get_model_group_from_derouter_kwargs(kwargs)
        remaining_requests_variable_name: Final = f"derouter-key-remaining-requests-{model_group}"
        remaining_tokens_variable_name: Final = f"derouter-key-remaining-tokens-{model_group}"
        standard_logging_payload: Final[StandardLoggingPayload | None] = kwargs.get("standard_logging_object")
        ...
        requests_labels: Final = prometheus_label_factory(
            supported_enum_labels=self.get_labels_for_metric("derouter_remaining_api_key_requests_for_model"),
            enum_values=enum_values,
            label_context=label_context,
        )
        self.derouter_remaining_api_key_requests_for_model.labels(**requests_labels).set(remaining_requests)

        tokens_labels: Final = prometheus_label_factory(
            supported_enum_labels=self.get_labels_for_metric("derouter_remaining_api_key_tokens_for_model"),
            enum_values=enum_values,
            label_context=label_context,
        )
        self.derouter_remaining_api_key_tokens_for_model.labels(**tokens_labels).set(remaining_tokens)
    ...
    def _set_latency_metrics(
        ...
        if (
            ...
            _ttft_labels: Final = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(
                    metric_name="derouter_llm_api_time_to_first_token_metric"
                ),
                enum_values=enum_values,
                label_context=label_context,
            )
            self.derouter_llm_api_time_to_first_token_metric.labels(**_ttft_labels).observe(time_to_first_token_seconds)
            self._track_end_user_metric_series(
                self.derouter_llm_api_time_to_first_token_metric,
                "derouter_llm_api_time_to_first_token_metric",
                _ttft_labels,
            )
        else:
        ...
        if api_call_total_time_seconds is not None:
            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(metric_name="derouter_llm_api_latency_metric"),
                enum_values=enum_values,
                label_context=label_context,
            )
            self.derouter_llm_api_latency_metric.labels(**_labels).observe(api_call_total_time_seconds)
            self._track_end_user_metric_series(
                self.derouter_llm_api_latency_metric,
                "derouter_llm_api_latency_metric",
                _labels,
            )
        ...
        _derouter_params: Final = kwargs.get("derouter_params", {}) or {}
        queue_time_seconds: Final = (_derouter_params.get("metadata") or {}).get("queue_time_seconds")
        ...
        if total_time_seconds is not None:
            ...
            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(metric_name="derouter_request_total_latency_metric"),
                enum_values=enum_values,
                label_context=label_context,
            )
            self.derouter_request_total_latency_metric.labels(**_labels).observe(_observed_total_time_seconds)
            self._track_end_user_metric_series(
                self.derouter_request_total_latency_metric,
                "derouter_request_total_latency_metric",
                _labels,
            )
        ...
        if queue_time_seconds is not None and queue_time_seconds >= 0:
            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(metric_name="derouter_request_queue_time_seconds"),
                enum_values=enum_values,
                label_context=label_context,
            )
            self.derouter_request_queue_time_metric.labels(**_labels).observe(queue_time_seconds)
            self._track_end_user_metric_series(
                self.derouter_request_queue_time_metric,
                "derouter_request_queue_time_seconds",
                _labels,
            )
    ...
    async def async_log_failure_event(self, kwargs, response_obj, start_time, end_time):
        ...
        try:
            ...
            PrometheusLogger._inc_labeled_counter(
                self,
                self.derouter_llm_api_failed_requests_metric,
    ...
    def _extract_api_provider_from_request_data(request_data: dict) -> str | None:
        ...
        ``derouter_params``, then any provider recovered onto a partial
        ...
        """
        derouter_params: Final = request_data.get("derouter_params") or {}
        provider = derouter_params.get("custom_llm_provider")
        if provider:
            return provider
        ...
        if provider:
            return provider
        model: Final = derouter_params.get("model") or request_data.get("model")
        if not model:
            return None
        try:
            return derouter.get_llm_provider(model=model)[1] or None
        except derouter.exceptions.BadRequestError:
            return None
    ...
    async def async_post_call_failure_hook(
        ...
        See :attr:`PrometheusMetricLabels.derouter_proxy_failed_requests_metric`
        ...
        try:
            ...
            enum_values: Final = UserAPIKeyLabelValues(
                ...
                stream=(str(request_data.get("stream")) if derouter.prometheus_emit_stream_label else None),
            )
            _label_ctx: Final = PrometheusLabelFactoryContext(enum_values)
            PrometheusLogger._inc_labeled_counter(
                self,
                self.derouter_proxy_failed_requests_metric,
    ...
    async def async_post_call_success_hook(self, data: dict, user_api_key_dict: UserAPIKeyAuth, response):
        ...
        Note: derouter_proxy_total_requests_metric is NOT incremented here to avoid
    ...
    def _extract_deployment_failure_label_values(self, request_kwargs: dict) -> dict[str, str | None]:
        ...
        sources in request_kwargs. Falls back to derouter_params metadata and
        ...
        _derouter_params: Final = request_kwargs.get("derouter_params", {}) or {}
        ...
        _derouter_params_metadata: Final = _derouter_params.get("metadata", {}) or {}

        # Extract user_api_key_auth if present (proxy injects this, skipped in merge)
        user_api_key_auth: Final = _derouter_params_metadata.get("user_api_key_auth")

        def _get_api_key_alias() -> str | None:
            val = _metadata.get("user_api_key_alias")
            if val is not None:
                return val
            val = _derouter_params_metadata.get("user_api_key_alias")
            if val is not None:
                return val
        ...
        def _get_user_email() -> str | None:
            ...
            from_params: Final = _derouter_params_metadata.get("user_api_key_user_email")
        ...
        def _get_team_id() -> str | None:
            ...
            if val is not None:
                return val
            val = _derouter_params_metadata.get("user_api_key_team_id")
            if val is not None:
                return val
            if user_api_key_auth is not None:
        ...
        def _get_team_alias() -> str | None:
            ...
            if val is not None:
                return val
            val = _derouter_params_metadata.get("user_api_key_team_alias")
            if val is not None:
                return val
            if user_api_key_auth is not None:
        ...
        def _get_hashed_api_key() -> str | None:
            ...
            if val is not None:
                return val
            val = _derouter_params_metadata.get("user_api_key_hash")
            if val is not None:
                return val
            if user_api_key_auth is not None:
        ...
        return {
            ...
            "client_ip": _metadata.get("requester_ip_address") or _derouter_params_metadata.get("requester_ip_address"),
            "user_agent": _metadata.get("user_agent") or _derouter_params_metadata.get("user_agent"),
        }
    ...
    def set_llm_deployment_failure_metrics(self, request_kwargs: dict):
        ...
        try:
            ...
            _derouter_params: Final = request_kwargs.get("derouter_params", {}) or {}
            derouter_model_name: Final = request_kwargs.get("model", None)
            ...
            # Fallback: model_id from derouter_metadata.model_info
            if model_id is None:
                _model_info: Final = (
                    (_derouter_params.get("derouter_metadata") or {}).get("model_info")
                    or (_derouter_params.get("metadata") or {}).get("model_info")
                    or {}
                )
                model_id = _model_info.get("id")

            # Fallback: model_group from derouter_metadata
            if model_group is None:
                model_group = (_derouter_params.get("derouter_metadata") or {}).get("model_group") or (
                    _derouter_params.get("metadata") or {}
                ).get("model_group")

            llm_provider: Final = _derouter_params.get("custom_llm_provider", None)
            ...
            # On DeRouter-side rejects (no deployment picked), route request_kwargs["model"]
            ...
            if deployment_selected:
                label_derouter_model_name = derouter_model_name
                label_model_id = model_id
                label_api_base = api_base
                label_api_provider = llm_provider
                label_requested_model = model_group or derouter_model_name
            else:
                label_derouter_model_name = ""
                label_model_id = ""
                label_api_base = ""
                label_api_provider = ""
                label_requested_model = (
                    _bounded_requested_model_label(derouter_model_name or model_group, router_originated=True) or ""
                )

            enum_values: Final = UserAPIKeyLabelValues(
                derouter_model_name=label_derouter_model_name,
            ...
            """
            log these labels
            ["derouter_model_name", "model_id", "api_base", "api_provider"]
            """
            # Only mark a deployment outage when one was actually picked.
            if deployment_selected:
                self.set_deployment_partial_outage(
                    derouter_model_name=derouter_model_name or "",
                    model_id=model_id,
                    api_base=api_base,
            ...
            if exception is not None:
                PrometheusLogger._inc_labeled_counter(
                    self,
                    self.derouter_deployment_failure_responses,
            ...
            PrometheusLogger._inc_labeled_counter(
                self,
                self.derouter_deployment_total_requests,
    ...
    def _set_deployment_tpm_rpm_limit_metrics(
        ...
        """
        tpm: Final = model_info.get("tpm") or derouter_params.get("tpm")
        rpm: Final = model_info.get("rpm") or derouter_params.get("rpm")

        if tpm is not None:
            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(metric_name="derouter_deployment_tpm_limit"),
                enum_values=UserAPIKeyLabelValues(
                    derouter_model_name=derouter_model_name,
                    model_id=model_id,
                    api_base=api_base,
                    api_provider=llm_provider,
                ),
            )
            self.derouter_deployment_tpm_limit.labels(**_labels).set(tpm)

        if rpm is not None:
            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(metric_name="derouter_deployment_rpm_limit"),
                enum_values=UserAPIKeyLabelValues(
                    derouter_model_name=derouter_model_name,
                    model_id=model_id,
                    api_base=api_base,
            ...
            self.derouter_deployment_rpm_limit.labels(**_labels).set(rpm)
    ...
    async def _async_set_router_remaining_metrics(
        ...
        """
        Populate ``derouter_remaining_tokens_metric`` /
        ``derouter_remaining_requests_metric`` from the router's internal usage
        ...
        try:
            ...
            if not already_have_tokens and remaining_tokens is not None:
                _labels = prometheus_label_factory(
                    supported_enum_labels=self.get_labels_for_metric(metric_name="derouter_remaining_tokens_metric"),
                    enum_values=enum_values,
                    label_context=label_context,
                )
                self.derouter_remaining_tokens_metric.labels(**_labels).set(remaining_tokens)

            if not already_have_requests and remaining_requests is not None:
                _labels = prometheus_label_factory(
                    supported_enum_labels=self.get_labels_for_metric(metric_name="derouter_remaining_requests_metric"),
                    enum_values=enum_values,
                    label_context=label_context,
                )
                self.derouter_remaining_requests_metric.labels(**_labels).set(remaining_requests)
        except Exception as e:
    ...
    def set_llm_deployment_success_metrics(
        ...
        try:
            ...
            _derouter_params: Final = request_kwargs.get("derouter_params", {}) or {}
            _metadata: Final = get_derouter_metadata_from_kwargs(request_kwargs)
            derouter_model_name: Final = request_kwargs.get("model", None)
            llm_provider: Final = _derouter_params.get("custom_llm_provider", None)
            _model_info: Final = _metadata.get("model_info") or {}
            model_id: Final = _model_info.get("id", None)

            if _model_info or _derouter_params:
            ...
            if derouter_overhead_time_ms := standard_logging_payload["hidden_params"].get("derouter_overhead_time_ms"):
                _labels = prometheus_label_factory(
                    supported_enum_labels=self.get_labels_for_metric(metric_name="derouter_overhead_latency_metric"),
                    enum_values=enum_values,
                    label_context=label_context,
                )
                self.derouter_overhead_latency_metric.labels(**_labels).observe(
                    derouter_overhead_time_ms / 1000
                )  # set as seconds
            ...
            """
            log these labels
            ["derouter_model_name", "requested_model", model_id", "api_base", "api_provider"]
            """
            ...
            PrometheusLogger._inc_labeled_counter(
                self,
                self.derouter_deployment_success_responses,
            ...
            if output_tokens is not None and output_tokens > 0:
                ...
                _labels = prometheus_label_factory(
                    supported_enum_labels=self.get_labels_for_metric(
                        metric_name="derouter_deployment_latency_per_output_token"
                    ),
                    enum_values=enum_values,
                    label_context=label_context,
                )
                self.derouter_deployment_latency_per_output_token.labels(**_labels).observe(latency_per_token)
    ...
    def _record_guardrail_metrics(
        ...
        try:
            # Record latency
            self.derouter_guardrail_latency_metric.labels(
            ...
            self.derouter_guardrail_requests_total.labels(
                guardrail_name=guardrail_name,
                status=status,
                hook_type=hook_type,
            ).inc()
            ...
            if status == "error" and error_type:
                self.derouter_guardrail_errors_total.labels(
                    guardrail_name=guardrail_name,
    ...
    def record_managed_batch_created(
        ...
        try:
            self.derouter_managed_batch_created_total.labels(
                model=model,
                api_provider=api_provider,
                user=user,
                user_email=user_email,
    ...
    def record_managed_file_size(
        ...
        try:
            self.derouter_managed_file_size_bytes.labels(
                purpose=purpose,
                file_type=file_type,
                model=model or "",
    ...
    def record_managed_batch_duration(
        ...
        try:
            self.derouter_managed_batch_duration_seconds.labels(
                model=model or "",
                api_provider=api_provider or "",
    ...
    def record_managed_file_created(
        ...
        try:
            self.derouter_managed_file_created_total.labels(
                model=model,
                api_provider=api_provider,
                user=user,
                user_email=user_email,
    ...
    def record_managed_file_deleted(self, result: str):
        ...
        try:
            self.derouter_managed_file_deleted_total.labels(result=result).inc()
        except Exception as e:
    ...
    def record_check_batch_cost_run(
        ...
        try:
            self.derouter_check_batch_cost_last_run_timestamp.set(time.time())
            self.derouter_check_batch_cost_jobs_polled.set(jobs_polled)

            if processed_models:
                for model, api_provider in processed_models:
                    self.derouter_check_batch_cost_jobs_processed_total.labels(
                        model=model or "",
                        api_provider=api_provider or "",
                    ).inc()
        except Exception as e:
    ...
    def record_check_batch_cost_error(self, error_type: str):
        try:
            self.derouter_check_batch_cost_errors_total.labels(
                error_type=error_type,
            ).inc()
        except Exception as e:
    ...
    def _get_exception_class_name(exception: Exception) -> str:
        ...
        # original value.
        try:
            from derouter.exceptions import BudgetExceededError
        except ImportError:
    ...
    def _extract_rate_limit_labels(
        ...
        exception that declares them (``derouter.RateLimitError`` and bare-
    ...
    async def log_success_fallback_event(self, original_model_group: str, kwargs: dict, original_exception: Exception):
        ...
        PrometheusLogger._inc_labeled_counter(
            self,
            self.derouter_deployment_successful_fallbacks,
            "derouter_deployment_successful_fallbacks",
            enum_values,
    ...
    async def log_failure_fallback_event(self, original_model_group: str, kwargs: dict, original_exception: Exception):
        ...
        PrometheusLogger._inc_labeled_counter(
            self,
            self.derouter_deployment_failed_fallbacks,
            "derouter_deployment_failed_fallbacks",
            enum_values,
            label_context=PrometheusLabelFactoryContext(enum_values),
        )

    def set_derouter_deployment_state(
        self,
        state: int,
        derouter_model_name: str,
        model_id: str | None,
        api_base: str | None,
        api_provider: str,
    ):
        ...
        ### get labels
        _labels: Final = prometheus_label_factory(
            supported_enum_labels=self.get_labels_for_metric(metric_name="derouter_deployment_state"),
            enum_values=UserAPIKeyLabelValues(
                derouter_model_name=derouter_model_name,
                model_id=model_id,
                api_base=api_base,
        ...
        self.derouter_deployment_state.labels(**_labels).set(state)
    ...
    def set_deployment_healthy(
        ...
        self.set_derouter_deployment_state(0, derouter_model_name, model_id, api_base, api_provider)
    ...
    def set_deployment_partial_outage(
        ...
        self.set_derouter_deployment_state(1, derouter_model_name, model_id, api_base, api_provider)
    ...
    def set_deployment_complete_outage(
        ...
        self.set_derouter_deployment_state(2, derouter_model_name, model_id, api_base, api_provider)
    ...
    def increment_deployment_cooled_down(
        ...
        """
        increment metric when derouter.Router / load balancing logic places a deployment in cool down
        """
        self.derouter_deployment_cooled_down.labels(
            _sanitize_prometheus_label_value(derouter_model_name),
    ...
    def increment_callback_logging_failure(
        ...
        """
        self.derouter_callback_logging_failures_metric.labels(callback_name=callback_name).inc()
    ...
    def track_provider_remaining_budget(self, provider: str, spend: float, budget_limit: float):
        ...
        """
        self.derouter_provider_remaining_budget_metric.labels(provider).set(
    ...
    async def _initialize_team_budget_metrics(self):
        ...
        async def fetch_teams(page_size: int, page: int) -> tuple[list[DeRouter_TeamTable], int | None]:
    ...
    async def _initialize_api_key_budget_metrics(self):
        ...
        """
        from derouter.constants import UI_SESSION_TOKEN_TEAM_ID
        ...
        async def fetch_keys(
            ...
            list[str | UserAPIKeyAuth | DeRouter_DeletedVerificationToken],
            int | None,
        ]:
    ...
    async def _initialize_user_budget_metrics(self):
        ...
        async def fetch_users(page_size: int, page: int) -> tuple[list[DeRouter_UserTable], int | None]:
    ...
    async def _initialize_org_budget_metrics(self):
        ...
        async def fetch_orgs(page_size: int, page: int) -> tuple[list, int | None]:
            ...
            orgs: Final = await _paginated_table(OrganizationRepository(prisma_client)).find_many(
                ...
                take=page_size,
                order={"created_at": "desc"},
                include={"derouter_budget_table": True},
            )
    ...
    async def initialize_remaining_budget_metrics(self):
        ...
        """
        from derouter.constants import PROMETHEUS_EMIT_BUDGET_METRICS_JOB_NAME
        from derouter.proxy.proxy_server import proxy_logging_obj
    ...
    async def _initialize_user_and_team_count_metrics(self):
        ...
        Updates:
        - derouter_total_users: Total count of users in the database
        - derouter_active_users: Count of billable users (excludes SCIM-deactivated)
        - derouter_teams_count: Total count of teams in the database
        """
        from derouter.proxy.proxy_server import prisma_client
        ...
        try:
            ...
            self.derouter_total_users_metric.set(total_users)
            verbose_logger.debug("Prometheus: set derouter_total_users to %s", total_users)

            billable_users: Final = await UserRepository(prisma_client).count_billable_users()
            self.derouter_active_users_metric.set(billable_users)
            verbose_logger.debug("Prometheus: set derouter_active_users to %s", billable_users)

            # Get total team count
            total_teams: Final = await _paginated_table(TeamRepository(prisma_client)).count()
            self.derouter_teams_count_metric.set(total_teams)
            verbose_logger.debug("Prometheus: set derouter_teams_count to %s", total_teams)
        except Exception as e:
            verbose_logger.exception("Error initializing user/team count metrics: %s", e)

    async def _set_key_list_budget_metrics(
        self, keys: list[str | UserAPIKeyAuth | DeRouter_DeletedVerificationToken]
    ) -> None:
    ...
    async def _set_team_list_budget_metrics(self, teams: list[DeRouter_TeamTable]):
        """Helper function to set budget metrics for a list of teams"""
        for team in teams:
            self._set_team_budget_metrics(team)

    async def _set_user_list_budget_metrics(self, users: list[DeRouter_UserTable]):
    ...
    async def _set_org_list_budget_metrics(self, orgs: list):
        ...
        for org in orgs:
            budget_table: _OrgBudgetRow | None = getattr(org, "derouter_budget_table", None)
    ...
    async def _set_team_budget_metrics_after_api_request(
        ...
        - Assemble a DeRouter_TeamTable object
            - looks up team info from db if not available in metadata
        - Set team budget metrics
        """
        if isinstance(self.derouter_remaining_team_budget_metric, NoOpMetric):
            return
    ...
    async def _assemble_team_object(
        ...
        - `budget_reset_at`
        """
        from derouter.proxy.auth.auth_checks import get_team_object
        from derouter.proxy.proxy_server import prisma_client, user_api_key_cache

        _total_team_spend: Final = (spend or 0) + response_cost
        team_object: Final = DeRouter_TeamTable(
            team_id=team_id,
            team_alias=team_alias,
    ...
    def _set_team_budget_metrics(
        ...
        _labels = prometheus_label_factory(
            supported_enum_labels=self.get_labels_for_metric(metric_name="derouter_remaining_team_budget_metric"),
            enum_values=enum_values,
        )
        self.derouter_remaining_team_budget_metric.labels(**_labels).set(
            self._safe_get_remaining_budget(
        ...
        if team.max_budget is not None:
            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(metric_name="derouter_team_max_budget_metric"),
                enum_values=enum_values,
            )
            self.derouter_team_max_budget_metric.labels(**_labels).set(team.max_budget)

        if team.budget_reset_at is not None:
            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(
                    metric_name="derouter_team_budget_remaining_hours_metric"
                ),
                enum_values=enum_values,
            )
            self.derouter_team_budget_remaining_hours_metric.labels(**_labels).set(
                self._get_remaining_hours_for_budget_reset(budget_reset_at=team.budget_reset_at)
            )

    def set_team_members_metric(self, team: DeRouter_TeamTable) -> None:
        ...
        _labels: Final = prometheus_label_factory(
            supported_enum_labels=self.get_labels_for_metric(metric_name="derouter_team_members_metric"),
            enum_values=enum_values,
        )
        self.derouter_team_members_metric.labels(**_labels).set(len(team.members_with_roles))
    ...
    async def _set_org_budget_metrics_after_api_request(
        ...
        """
        if isinstance(self.derouter_remaining_org_budget_metric, NoOpMetric):
            return

        if not org_id:
            return

        from derouter.proxy.auth.auth_checks import get_org_object
        from derouter.proxy.proxy_server import prisma_client, user_api_key_cache
        ...
        budget_table: Final = org_info.derouter_budget_table
    ...
    def _set_org_budget_metrics(
        ...
        _labels = prometheus_label_factory(
            supported_enum_labels=self.get_labels_for_metric(metric_name="derouter_remaining_org_budget_metric"),
            enum_values=enum_values,
        )
        self.derouter_remaining_org_budget_metric.labels(**_labels).set(
            self._safe_get_remaining_budget(
                max_budget=max_budget,
                spend=spend,
            )
        ...
        if max_budget is not None:
            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(metric_name="derouter_org_max_budget_metric"),
                enum_values=enum_values,
            )
            self.derouter_org_max_budget_metric.labels(**_labels).set(max_budget)

        if budget_reset_at is not None:
            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(
                    metric_name="derouter_org_budget_remaining_hours_metric"
                ),
                enum_values=enum_values,
            )
            self.derouter_org_budget_remaining_hours_metric.labels(**_labels).set(
    ...
    def _set_key_budget_metrics(self, user_api_key_dict: UserAPIKeyAuth):
        ...
        _labels = prometheus_label_factory(
            supported_enum_labels=self.get_labels_for_metric(metric_name="derouter_remaining_api_key_budget_metric"),
            enum_values=enum_values,
        )
        self.derouter_remaining_api_key_budget_metric.labels(**_labels).set(
            self._safe_get_remaining_budget(
        ...
        if user_api_key_dict.max_budget is not None:
            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(metric_name="derouter_api_key_max_budget_metric"),
                enum_values=enum_values,
            )
            self.derouter_api_key_max_budget_metric.labels(**_labels).set(user_api_key_dict.max_budget)

        if user_api_key_dict.budget_reset_at is not None:
            self.derouter_api_key_budget_remaining_hours_metric.labels(**_labels).set(
    ...
    async def _set_api_key_budget_metrics_after_api_request(
        ...
        if isinstance(self.derouter_remaining_api_key_budget_metric, NoOpMetric):
            return
    ...
    async def _assemble_key_object(
        ...
        """
        from derouter.proxy.auth.auth_checks import get_key_object
        from derouter.proxy.proxy_server import prisma_client, user_api_key_cache
    ...
    async def _set_user_budget_metrics_after_api_request(
        ...
        - Assemble a DeRouter_UserTable object
            - looks up user info from db if not available in metadata
        - Set user budget metrics
        """
        if isinstance(self.derouter_remaining_user_budget_metric, NoOpMetric):
            return
    ...
    async def _assemble_user_object(
        ...
        - `budget_reset_at`
        """
        from derouter.proxy.auth.auth_checks import get_user_object
        from derouter.proxy.proxy_server import prisma_client, user_api_key_cache

        _total_user_spend: Final = (spend or 0) + response_cost
        user_object: Final = DeRouter_UserTable(
            user_id=user_id,
    ...
    def _set_user_budget_metrics(
        ...
        _labels = prometheus_label_factory(
            supported_enum_labels=self.get_labels_for_metric(metric_name="derouter_remaining_user_budget_metric"),
            enum_values=enum_values,
        )
        self.derouter_remaining_user_budget_metric.labels(**_labels).set(
            self._safe_get_remaining_budget(
        ...
        if user.max_budget is not None:
            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(metric_name="derouter_user_max_budget_metric"),
                enum_values=enum_values,
            )
            self.derouter_user_max_budget_metric.labels(**_labels).set(user.max_budget)

        if user.budget_reset_at is not None:
            _labels = prometheus_label_factory(
                supported_enum_labels=self.get_labels_for_metric(
                    metric_name="derouter_user_budget_remaining_hours_metric"
                ),
                enum_values=enum_values,
            )
            self.derouter_user_budget_remaining_hours_metric.labels(**_labels).set(
    ...
    def initialize_budget_metrics_cron_job(scheduler: AsyncIOScheduler):
        ...
        """
        from derouter.constants import PROMETHEUS_BUDGET_METRICS_REFRESH_INTERVAL_MINUTES

        prometheus_loggers: Final[list[CustomLogger]] = derouter.logging_callback_manager.get_custom_loggers_for_type(
    ...
    def _mount_metrics_endpoint():
        ...
        from derouter.integrations.prometheus_metrics_endpoint import make_metrics_asgi_app
    ...
    *,
    ...
    if UserAPIKeyLabelNames.END_USER.value in filtered_labels:
        ...
        filtered_labels["end_user"] = get_end_user_id_for_cost_tracking(
            derouter_params={"user_api_key_end_user_id": enum_values.end_user},
...
def get_custom_labels_from_metadata(metadata: dict) -> dict[str, str]:
    ...
    """
    keys: Final = derouter.custom_prometheus_metadata_labels
...
def get_custom_labels_from_tags(tags: Sequence[str]) -> dict[str, str]:
    ...
    from derouter.types.integrations.prometheus import _sanitize_prometheus_label_name

    configured_tags: Final = derouter.custom_prometheus_tags
