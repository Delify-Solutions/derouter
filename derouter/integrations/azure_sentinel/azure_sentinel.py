...
DeRouter logs to the Log Analytics workspace using the Azure Monitor Logs Ingestion API.

Reference API: https://learn.microsoft.com/en-us/azure/azure-monitor/logs/logs-ingestion-api-overview

`async_log_success_event` - used by derouter proxy to send logs to Azure Sentinel
`async_log_failure_event` - used by derouter proxy to send failure logs to Azure Sentinel
...
from derouter.integrations.batch_utils import (
    BatchSendCancelled,
...
class AzureSentinelLogger(CustomBatchLogger):
    """
    Logger that sends DeRouter logs to Azure Sentinel via Azure Monitor Logs Ingestion API
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            stream_name (str, optional): Stream name from DCR (e.g., "Custom-DeRouter").
                If not provided, will use AZURE_SENTINEL_STREAM_NAME env var or default to "Custom-DeRouter".
        ...
        resolved_stream_name: Final = stream_name or os.getenv("AZURE_SENTINEL_STREAM_NAME") or "Custom-DeRouter"
    ...
    def _normalize_authority_host(authority_host: str) -> str:
        ...
        Accepts the scheme-qualified form derouter documents ("https://login.microsoftonline.us")
    ...
    async def async_log_audit_log_event(self, audit_log: StandardAuditLogPayload) -> None:
        """
        Async log DeRouter audit log events to Azure Sentinel.
