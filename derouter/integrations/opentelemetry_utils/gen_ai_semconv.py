...
default DeRouter output:

Request span:

- name is ``{operation} {model}`` (e.g. ``chat gpt-4``) instead of
  ``derouter_request``; span kind is ``CLIENT``.
...
# DeRouter optional_params key -> OTEL gen_ai semconv span attribute.
...
class OTELGenAISemconvMixin:
    ...
    - ``_gen_ai_operation_name`` -- DeRouter ``call_type`` -> spec
    ...
    def _gen_ai_operation_name(kwargs: dict) -> str:
        """Map a DeRouter ``call_type`` to spec ``gen_ai.operation.name``.
