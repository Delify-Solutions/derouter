...
derouter_content_retrieve tool calls server-side via the typed agentic loop plan.
"""
...
from derouter.compression import compress
...
DEROUTER_CONTENT_RETRIEVE_TOOL_NAME: Final = "derouter_content_retrieve"
...
def _record_compression_savings(kwargs: dict[str, object], savings: CompressionSavingsMetadata) -> None:
    """
    Attach savings to the request's derouter metadata so they land in the
    SpendLog row's metadata JSON under ``compression_savings``.

    ``/v1/messages`` requests carry proxy metadata under ``derouter_metadata``
    ...
    references.
    """
    existing: Final = kwargs.get("derouter_metadata")
    if isinstance(existing, dict):
        existing["compression_savings"] = savings
        return
    kwargs["derouter_metadata"] = {"compression_savings": savings}
...
class CompressionInterceptionLogger(CustomLogger):
    ...
    2. Inject derouter_content_retrieve tool and persist compressed cache by call_id.
    ...
    server_fulfilled_tool_names: ClassVar[frozenset[str]] = frozenset({DEROUTER_CONTENT_RETRIEVE_TOOL_NAME})
    ...
    def initialize_from_proxy_config(
        ...
        if "compression_interception_params" in derouter_settings:
            compression_params = derouter_settings["compression_interception_params"]
    ...
    async def async_pre_call_deployment_hook(
        ...
        if cache:
            ...
            call_id = cast(str | None, kwargs.get("derouter_call_id"))
            if not call_id:
                call_id = str(uuid.uuid4())
                kwargs["derouter_call_id"] = call_id
            ...
            + [
                {
                    "type": "tool_use",
                    "id": tc.get("id"),
                    "name": tc.get("name", DEROUTER_CONTENT_RETRIEVE_TOOL_NAME),
                    "input": tc.get("input", {}),
                }
    ...
    def _resolve_call_id(self, logging_obj: _AgenticLoopLoggingObj | None, kwargs: Mapping[str, object]) -> str | None:
        ...
        kwargs_call_id: Final = kwargs.get("derouter_call_id")
    ...
    def _extract_retrieval_tool_calls(
        ...
        for block in content:
            if isinstance(block, dict):
                ...
                if block_type == "tool_use" and block_name == DEROUTER_CONTENT_RETRIEVE_TOOL_NAME:
                    tool_calls.append(
                        {
                            "id": block.get("id"),
                            "type": "tool_use",
    ...
    def _has_retrieval_tool(self, tools: object) -> bool:
        ...
        for tool in tools:
            ...
            if tool.get("type") == "function" and isinstance(function, dict):
                if function.get("name") == DEROUTER_CONTENT_RETRIEVE_TOOL_NAME:
                    return True
            if tool.get("type") == "custom" and tool.get("name") == DEROUTER_CONTENT_RETRIEVE_TOOL_NAME:
                return True
        return False
