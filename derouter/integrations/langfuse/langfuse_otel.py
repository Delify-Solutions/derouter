...
from derouter.types.integrations.langfuse_otel import (
...
class LangfuseOtelLogger(OpenTelemetry):
    ...
    def _extract_langfuse_metadata(kwargs: dict) -> dict:
        """
        Extracts Langfuse metadata from the standard DeRouter kwargs structure.

        1. Reads kwargs["derouter_params"]["metadata"] if present and is a dict.
        ...
        try:
            from derouter.integrations.langfuse.langfuse import (
                LangFuseLogger as _LFLogger,
            )

            metadata = _LFLogger.add_metadata_from_header(derouter_params, metadata)
        except Exception:
    ...
    def _set_metadata_attributes(span: Span, metadata: dict):
        ...
        from derouter.integrations.arize._utils import safe_set_attribute
