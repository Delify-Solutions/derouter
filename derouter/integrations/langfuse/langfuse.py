...
from derouter.constants import MAX_LANGFUSE_INITIALIZED_CLIENTS
from derouter.integrations.langfuse.langfuse_mock_client import (
...
from derouter.derouter_core_utils.redact_messages import redact_user_api_key_info
...
from derouter.types.integrations.langfuse import *
from derouter.types.llms.openai import HttpxBinaryResponseContent, ResponsesAPIResponse
...
def _extract_cache_read_input_tokens(usage_obj) -> int:
    ...
    See: https://github.com/Delify-Solutions/derouter/issues/18520
...
def _logging_id(start_time: datetime | None, response_obj: object) -> str | None:
    ...
    return derouter.utils.get_logging_id(start_time, response_obj)
...
class LangFuseLogger:
    ...
    def __init__(
        ...
        if Version(self.langfuse_sdk_version) >= Version("2.6.0"):
            parameters["sdk_integration"] = "derouter"
    ...
    def safe_init_langfuse_client(self, parameters: dict) -> Langfuse:
        ...
        if derouter.initialized_langfuse_clients >= MAX_LANGFUSE_INITIALIZED_CLIENTS:
            raise Exception(
                f"Max langfuse clients reached: {derouter.initialized_langfuse_clients} is greater than {MAX_LANGFUSE_INITIALIZED_CLIENTS}"
            )
        langfuse_client: Final = Langfuse(**parameters)
        derouter.initialized_langfuse_clients += 1
        verbose_logger.debug("Created langfuse client number %s", derouter.initialized_langfuse_clients)
        return langfuse_client

    @staticmethod
    def add_metadata_from_header(derouter_params: dict, metadata: dict) -> dict[str, object]:
        """
        Adds metadata from proxy request headers to Langfuse logging if keys start with "langfuse_"
        and overwrites derouter_params.metadata if already included.
        ...
        if derouter_params.get("proxy_server_request") is None:
            return metadata

        if metadata is None:
            metadata = {}

        proxy_headers: Final = derouter_params.get("proxy_server_request", {}).get("headers", {}) or {}
    ...
    def log_event_on_langfuse(
        ...
        try:
            ...
            metadata = derouter_params.get("metadata", {}) or {}  # if derouter_params['metadata'] == None
            metadata = self.add_metadata_from_header(derouter_params, metadata)
    ...
    def _get_langfuse_input_output_content(
        ...
        if level == "ERROR" and status_message is not None and isinstance(status_message, str):
            ...
            kwargs.get("call_type", None) == "embedding" or isinstance(response_obj, derouter.EmbeddingResponse)
        ):
            input = prompt
            output = None
        elif response_obj is not None and isinstance(response_obj, derouter.ModelResponse):
            input = prompt
            output = self._get_chat_content_for_langfuse(response_obj)
        elif response_obj is not None and isinstance(response_obj, derouter.HttpxBinaryResponseContent):
            input = prompt
            output = "speech-output"
        elif response_obj is not None and isinstance(response_obj, derouter.TextCompletionResponse):
            input = prompt
            output = self._get_text_completion_content_for_langfuse(response_obj)
        elif response_obj is not None and isinstance(response_obj, derouter.ImageResponse):
            input = prompt
            output = response_obj.get("data", None)
        elif response_obj is not None and isinstance(response_obj, derouter.TranscriptionResponse):
            input = prompt
            output = response_obj.get("text", None)
        elif response_obj is not None and isinstance(response_obj, derouter.RerankResponse):
            input = prompt
            output = response_obj.results
        elif response_obj is not None and isinstance(response_obj, derouter.ResponsesAPIResponse):
            input = prompt
    ...
    def _log_langfuse_v1(
        ...
        trace: Final = self.Langfuse.trace(
            CreateTrace(
                name=metadata.get("generation_name", "derouter-completion"),
                input=input,
                output=output,
                userId=user_id,
            )
    ...
    def _log_langfuse_v2(
        ...
        try:
            ...
            # we clean out all extra derouter metadata params before logging
            ...
            if metadata_entries is not None:
                for key, value in metadata_entries.items():
                    # generate langfuse tags - Default Tags sent to Langfuse from DeRouter Proxy
                    if (
                        derouter.langfuse_default_tags is not None
                        and isinstance(derouter.langfuse_default_tags, list)
                        and key in derouter.langfuse_default_tags
                    ):
                        tags.append(f"{key}:{value}")

                    # clean derouter metadata before logging
            ...
            # Fallback to derouter_call_id if no trace_id found
            if trace_id is None:
                trace_id = kwargs.get("derouter_trace_id") or derouter_call_id
            ...
            update_trace_keys: Final = (
                requested_trace_keys if _as_steering_flag(derouter.langfuse_enable_update_trace_keys) else ()
            )
            ...
            masking_function: Final = derouter_params.get("_langfuse_masking_function") or clean_metadata.pop(
            ...
            if trace_name is None and existing_trace_id is None:
                # just log `derouter-{call_type}` as the trace name
                ## DO NOT SET TRACE_NAME if trace-id set. this can lead to overwriting of past traces.
                trace_name = f"derouter-{kwargs.get('call_type', 'completion')}"
            ...
            if existing_trace_id is not None:
                ...
                if "input" in update_trace_keys:
                    trace_params["input"] = masked_input if not mask_input else "redacted-by-derouter"
                if "output" in update_trace_keys:
                    trace_params["output"] = masked_output if not mask_output else "redacted-by-derouter"
            else:  # don't overwrite an existing trace
                trace_params = {
                    "id": trace_id,
                    "name": trace_name,
                    "session_id": session_id,
                    "input": masked_input if not mask_input else "redacted-by-derouter",
            ...
            if debug is True or (isinstance(debug, str) and debug.lower() == "true"):
                ...
                trace_params["metadata"] = {
                    **(trace_params.get("metadata") or _NO_METADATA),
                    "metadata_passed_to_derouter": debug_metadata,
                }
            ...
            candidate_enrichments: Final = (
                ("derouter_response_cost", cost, True),
            ...
            proxy_server_request: Final = derouter_params.get("proxy_server_request", None)
            ...
            if generation_name is None:
                ...
                # If using derouter proxy user `key_alias` if not None
                # If `key_alias` is None, just log `derouter-{call_type}` as the generation name
                _user_api_key_alias: Final = cast(str | None, clean_metadata.get("user_api_key_alias", None))
                generation_name = f"derouter-{cast(str, kwargs.get('call_type', 'completion'))}"
                if _user_api_key_alias is not None:
                    generation_name = f"derouter:{_user_api_key_alias}"
            ...
            generation_params = {
                ...
                "input": masked_input if not mask_input else "redacted-by-derouter",
                "output": masked_output if not mask_output else "redacted-by-derouter",
                "usage": usage,
            ...
            # Return the trace_id we set (which should be derouter_call_id when no explicit trace_id provided)
    ...
    def add_default_langfuse_tags(self, tags, kwargs, metadata):
        """
        Helper function to add derouter default langfuse tags

        - Special DeRouter tags:
            - cache_hit
            - cache_key

        """
        if derouter.langfuse_default_tags is not None and isinstance(derouter.langfuse_default_tags, list):
            if "cache_hit" in derouter.langfuse_default_tags:
                _cache_hit_value: Final = kwargs.get("cache_hit", False)
                tags.append(f"cache_hit:{_cache_hit_value}")
            if "cache_key" in derouter.langfuse_default_tags:
                _hidden_params: Final = metadata.get("hidden_params", {}) or {}
                _cache_key = _hidden_params.get("cache_key", None)
                if _cache_key is None and derouter.cache is not None:
                    # fallback to using "preset_cache_key"
                    _preset_cache_key: Final = derouter.cache._get_preset_cache_key_from_kwargs(**kwargs)
