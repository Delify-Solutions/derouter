# used for monitoring derouter services health on `/metrics` endpoint on DeRouter Proxy
#### What this does ####
#    On success + failure, log events to Prometheus for derouter / adjacent services (derouter, redis, postgres, llm api providers)
...
from derouter.types.services import (
...
class PrometheusServicesLogger:
    ...
    derouter_service_latency = None  # Class-level attribute to store the Histogram
    ...
    def create_histogram(self, service: str, type_of_request: str):
        metric_name: Final = f"derouter_{service}_{type_of_request}"
