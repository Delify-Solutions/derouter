...
from derouter.constants import REDACTED_BY_DEROUTER, REDACTED_BY_LITELM_STRING
...
from derouter.proxy.spend_tracking.savings import extract_cache_creation_tokens, extract_cache_read_tokens
from derouter.types.integrations.datadog_llm_obs import *
...
class DataDogLLMObsLogger(CustomBatchLogger):
    def __init__(self, **kwargs):
        try:
            ...
            # Use DEROUTER_DD_AGENT_HOST to avoid conflicts with ddtrace's DD_AGENT_HOST
            # Check for agent mode FIRST - agent mode doesn't require DD_API_KEY or DD_SITE
            dd_agent_host: Final = os.getenv("DEROUTER_DD_AGENT_HOST")
            ...
            #########################################################
            # Handle datadog_llm_observability_params set as derouter.datadog_llm_observability_params
            #########################################################
    ...
    def _configure_dd_agent(self, dd_agent_host: str):
        ...
        agent_port: Final = os.getenv("DEROUTER_DD_LLM_OBS_PORT", "8126")
    ...
    def _get_datadog_llm_obs_params(self) -> dict:
        """
        Get the datadog_llm_observability_params from derouter.datadog_llm_observability_params
        ...
        if derouter.datadog_llm_observability_params is not None:
            if isinstance(derouter.datadog_llm_observability_params, DatadogLLMObsInitParams):
                dict_datadog_llm_obs_params = derouter.datadog_llm_observability_params.model_dump(exclude_unset=True)
            elif isinstance(derouter.datadog_llm_observability_params, dict):
                # only allow params that are of DatadogLLMObsInitParams
                dict_datadog_llm_obs_params = DatadogLLMObsInitParams(
                    **derouter.datadog_llm_observability_params
    ...
    def create_llm_obs_payload(self, kwargs: dict, start_time: datetime, end_time: datetime) -> LLMObsPayload:
        ...
        raw_metadata: Final = kwargs.get("derouter_params", {}).get("metadata", {})
        ...
        payload: Final[LLMObsPayload] = LLMObsPayload(
            ...
            name=metadata.get("name", "derouter_llm_call"),
            meta=meta,
    ...
    def _assemble_metrics(self, standard_logging_payload: StandardLoggingPayload) -> LLMMetrics:
        ...
        derouter's normalized prompt count includes both (the invariant the cost calculator's custom
    ...
    def _get_dd_llm_obs_payload_metadata(
        ...
        """
        Fields to track in DD LLM Observability metadata from derouter standard logging payload
        """
    ...
    def _get_latency_metrics(self, standard_logging_payload: StandardLoggingPayload) -> DDLLMObsLatencyMetrics:
        ...
        # DeRouter overhead time
        hidden_params: Final = standard_logging_payload.get("hidden_params", {})
        derouter_overhead_ms: Final = hidden_params.get("derouter_overhead_time_ms")
        if derouter_overhead_ms is not None:
            latency_metrics["derouter_overhead_time_ms"] = derouter_overhead_ms
