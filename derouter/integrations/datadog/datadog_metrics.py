...
from derouter.types.integrations.datadog_metrics import (
    DatadogMetricPoint,
    DatadogMetricSeries,
    ...
    def _add_metrics_from_log(
        ...
        if api_call_start_time and end_time_dt:
            ...
            series_llm_latency: Final[DatadogMetricSeries] = {
                "metric": "derouter.llm_api.latency",
                "type": 3,  # gauge
        ...
        # 3. DeRouter Overhead Latency Metric (total - llm_api time)
        hidden_params: Final = log.get("hidden_params", {}) or {}
        derouter_overhead_time_ms: Final = hidden_params.get("derouter_overhead_time_ms")
        if derouter_overhead_time_ms is not None:
            overhead_tags: Final = self._extract_tags(log)  # no status_code on latency metric
            series_overhead: Final[DatadogMetricSeries] = {
                "metric": "derouter.overhead.latency",
                "type": 3,  # gauge
                "points": [
                    {
                        "timestamp": timestamp,
                        "value": derouter_overhead_time_ms / 1000,  # convert ms → seconds
                    }
                ],
                "tags": overhead_tags,
            }
        ...
        series_count: Final[DatadogMetricSeries] = {
            "metric": "derouter.llm_api.request_count",
            "type": 1,  # count
