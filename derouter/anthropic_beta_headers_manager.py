...
- Local file: Set DEROUTER_LOCAL_ANTHROPIC_BETA_HEADERS=True to use bundled config only

Environment Variables:
- DEROUTER_LOCAL_ANTHROPIC_BETA_HEADERS: Set to "True" to disable remote fetching
- DEROUTER_ANTHROPIC_BETA_HEADERS_URL: Custom URL for remote config (optional)
"""
    ...
    def load_local_beta_headers_config() -> dict:
        ...
        try:
            content: Final = json.loads(
                files("derouter").joinpath("anthropic_beta_headers_config.json").read_text(encoding="utf-8")
            )
            return content
    ...
    def _check_is_valid_dict(fetched_config: dict) -> bool:
        ...
        if not isinstance(fetched_config, dict):
            verbose_logger.warning(
                "DeRouter: Fetched beta headers config is not a dict (type=%s). Falling back to local backup.",
        ...
        if len(fetched_config) == 0:
            verbose_logger.warning(
                "DeRouter: Fetched beta headers config is empty. Falling back to local backup.",
            )
            return False
        ...
        if not has_provider:
            verbose_logger.warning(
                "DeRouter: Fetched beta headers config missing provider keys. Falling back to local backup.",
            )
            return False
...
def get_beta_headers_config(url: str) -> dict:
    ...
    1. If ``DEROUTER_LOCAL_ANTHROPIC_BETA_HEADERS`` is set, uses the local backup only.
    ...
    if os.getenv("DEROUTER_LOCAL_ANTHROPIC_BETA_HEADERS", "").lower() == "true":
        # verbose_logger.debug("Using local Anthropic beta headers config (DEROUTER_LOCAL_ANTHROPIC_BETA_HEADERS=True)")
    ...
    try:
        ...
        verbose_logger.warning(
            "DeRouter: Failed to fetch remote beta headers config from %s: %s. Falling back to local backup.",
            url,
            str(e),
        )
        return GetAnthropicBetaHeadersConfig.load_local_beta_headers_config()
    ...
    if not GetAnthropicBetaHeadersConfig.validate_beta_headers_config(fetched_config=content):
        verbose_logger.warning(
            "DeRouter: Fetched beta headers config failed integrity check. Using local backup instead. url=%s",
            url,
        )
        return GetAnthropicBetaHeadersConfig.load_local_beta_headers_config()
...
def _load_beta_headers_config() -> dict:
    ...
    from derouter import anthropic_beta_headers_url
