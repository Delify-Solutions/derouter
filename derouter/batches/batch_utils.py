...
from derouter.derouter_core_utils.llm_cost_calc.utils import parse_prompt_tokens_details
...
async def calculate_batch_cost_and_usage(
    ...
    if (
        ...
        and model_name
        and getattr(derouter, "disable_vertex_batch_output_transformation", False)
    ):
...
async def _handle_completed_batch(
    ...
    Args:
        ...
        derouter_params: Optional derouter parameters containing credentials (api_key, api_base, etc.)
    ...
    if batch.output_file_id is None:
        return BatchCostUsageResult(
            ...
            failed_requests=await count_error_file_failed_requests(
                batch, custom_llm_provider=custom_llm_provider, derouter_params=derouter_params
            ),
        )

    file_content = await _fetch_batch_output_file_content(batch, custom_llm_provider, derouter_params=derouter_params)
...
def _classify_output_line_stats(
    ...
    failed yields ``PROVIDER_FAILED``; a successful line derouter could not price
...
def calculate_vertex_ai_batch_cost_and_usage(
    ...
    Used only when ``derouter.disable_vertex_batch_output_transformation = True``.
...
async def _fetch_batch_managed_file_content(
    ...
    Args:
        file_id: The provider or unified (derouter-managed) file id to fetch
        custom_llm_provider: The LLM provider
        derouter_params: Optional derouter parameters containing credentials (api_key, api_base, etc.)
    ...
    # Build kwargs for afile_content with credentials from derouter_params
    ...
    credentials: Final = _extract_file_access_credentials(derouter_params)
...
async def _fetch_batch_output_file_content(
    ...
    return await _fetch_batch_managed_file_content(
        batch.output_file_id, custom_llm_provider=custom_llm_provider, derouter_params=derouter_params
    )
...
async def count_error_file_failed_requests(
    ...
    try:
        error_file_content = await _fetch_batch_managed_file_content(
            batch.error_file_id, custom_llm_provider=custom_llm_provider, derouter_params=derouter_params
        )
...
def _extract_file_access_credentials(derouter_params: dict | None) -> dict:
    """
    Extract credentials from derouter_params for file access operations.
    ...
    Args:
        derouter_params: Dictionary containing derouter parameters with credentials
    ...
    if derouter_params:
        ...
        credential_keys: Final = (
            ...
            "timeout",
            "max_retries",
            "_derouter_internal_model_credentials",
            *AWS_CREDENTIAL_KWARGS_KEYS,
        )
        for key in credential_keys:
            if key in derouter_params:
                credentials[key] = derouter_params[key]
