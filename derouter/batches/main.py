...
from derouter.derouter_core_utils.get_derouter_params import add_trusted_model_credentials_to_derouter_params
from derouter.derouter_core_utils.derouter_logging import Logging as DeRouterLoggingObj
from derouter.llms.anthropic.batches.handler import AnthropicBatchesHandler
from derouter.llms.azure.batches.handler import AzureBatchesAPI
from derouter.llms.bedrock.batches.handler import BedrockBatchesHandler
...
from derouter.llms.openai.openai import OpenAIBatchesAPI
from derouter.llms.vertex_ai.batches.handler import VertexAIBatchPrediction
...
def _resolve_timeout(
    optional_params: GenericDeRouterParams,
    kwargs: dict[str, Any],
    ...
    Args:
        optional_params: GenericDeRouterParams object containing timeout
...
async def acreate_batch(
    ...
    input_file_id: str,
    custom_llm_provider: Literal["openai", "azure", "vertex_ai", "bedrock", "hosted_vllm", "derouter_proxy"] = "openai",
    ...
    **kwargs,
    ...
    DeRouter Equivalent of POST: https://api.openai.com/v1/batches
    """
    try:
    ...
    try:
        ...
        try:
            ...
            verbose_logger.exception(
                "derouter.batches.main.py::create_batch() - Error inferring custom_llm_provider - %s", e
            )

        _is_async: Final = kwargs.pop("acreate_batch", False) is True
        derouter_params: Final = dict(GenericDeRouterParams(**kwargs))
        derouter_logging_obj: Final[DeRouterLoggingObj] = cast(DeRouterLoggingObj, kwargs.get("derouter_logging_obj", None))
        ### TIMEOUT LOGIC ###
        ...
        if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
            ...
            api_base = optional_params.api_base or derouter.api_base or get_secret_str("AZURE_API_BASE")
            api_version = optional_params.api_version or derouter.api_version or get_secret_str("AZURE_API_VERSION")
            ...
            vertex_ai_project: Final = (
                optional_params.vertex_project or derouter.vertex_project or get_secret_str("VERTEXAI_PROJECT")
            )
            vertex_ai_location: Final = (
                optional_params.vertex_location or derouter.vertex_location or get_secret_str("VERTEXAI_LOCATION")
            )
            ...
            raise derouter.exceptions.BadRequestError(
                message=f"DeRouter doesn't support custom_llm_provider={custom_llm_provider} for 'create_batch'",
                model="n/a",
                llm_provider=custom_llm_provider,
                response=httpx.Response(
                    status_code=400,
                    content="Unsupported provider",
                    request=httpx.Request(method="create_batch", url="https://github.com/Delify-Solutions/derouter"),
                ),
            )
        return response
    except Exception as e:
...
async def aretrieve_batch(
    ...
    custom_llm_provider: Literal[
        "openai", "azure", "vertex_ai", "bedrock", "hosted_vllm", "derouter_proxy", "anthropic"
    ] = "openai",
    metadata: dict[str, str] | None = None,
    extra_headers: dict[str, str] | None = None,
    extra_body: dict[str, str] | None = None,
    **kwargs,
    ...
    DeRouter Equivalent of GET https://api.openai.com/v1/batches/{batch_id}
    """
    try:
        loop: Final = asyncio.get_event_loop()
...
def _handle_retrieve_batch_providers_without_provider_config(
    ...
    if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
        ...
        api_base = optional_params.api_base or derouter.api_base or get_secret_str("AZURE_API_BASE")
        api_version: Final = optional_params.api_version or derouter.api_version or get_secret_str("AZURE_API_VERSION")
        ...
        api_key = optional_params.api_key or derouter.api_key or derouter.azure_key or get_secret_str("ANTHROPIC_API_KEY")
        ...
        raise derouter.exceptions.BadRequestError(
            message=(
                f"DeRouter doesn't support custom_llm_provider={custom_llm_provider} for 'retrieve_batch' without a `model` kwarg. "
            ...
            response=httpx.Response(
                status_code=400,
                content="Unsupported provider",
                request=httpx.Request(method="retrieve_batch", url="https://github.com/Delify-Solutions/derouter"),
            ),
        )
    return response
...
def retrieve_batch(
    ...
    **kwargs,
    ...
    try:
        ...
        derouter_params: Final = get_derouter_params(
            custom_llm_provider=custom_llm_provider,
            **kwargs,
        )
        add_trusted_model_credentials_to_derouter_params(derouter_params, kwargs)
    ...
    **kwargs,
    ...
    try:
        ...
        if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
            ...
            raise derouter.exceptions.BadRequestError(
                message="DeRouter doesn't support {} for 'list_batch'. Supported providers: {}.".format(
                    custom_llm_provider,
                ...
                response=httpx.Response(
                    status_code=400,
                    content="Unsupported provider",
                    request=httpx.Request(method="create_thread", url="https://github.com/Delify-Solutions/derouter"),
                ),
            )
        return response
    except Exception as e:
...
async def acancel_batch(
    batch_id: str,
    model: str | None = None,
    custom_llm_provider: Literal["openai", "azure", "vertex_ai", "bedrock", "derouter_proxy"] = "openai",
    metadata: dict[str, str] | None = None,
    extra_headers: dict[str, str] | None = None,
    extra_body: dict[str, str] | None = None,
    **kwargs,
    ...
    DeRouter Equivalent of POST https://api.openai.com/v1/batches/{batch_id}/cancel
    """
    try:
        loop: Final = asyncio.get_event_loop()
...
def cancel_batch(
    batch_id: str,
    model: str | None = None,
    custom_llm_provider: Literal["openai", "azure", "vertex_ai", "bedrock", "derouter_proxy"] | str = "openai",
    metadata: dict[str, str] | None = None,
    extra_headers: dict[str, str] | None = None,
    extra_body: dict[str, str] | None = None,
    **kwargs,
    ...
    try:
        try:
            ...
            verbose_logger.exception(
                "derouter.batches.main.py::cancel_batch() - Error inferring custom_llm_provider - %s", e
            )
        optional_params: Final = GenericDeRouterParams(**kwargs)
        derouter_params: Final = get_derouter_params(
            custom_llm_provider=custom_llm_provider,
        ...
        if custom_llm_provider in OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS:
            ...
            organization: Final = (
                optional_params.organization or derouter.organization or os.getenv("OPENAI_ORGANIZATION", None) or None
            )
            api_key = optional_params.api_key or derouter.api_key or derouter.openai_key or os.getenv("OPENAI_API_KEY")
            ...
            raise derouter.exceptions.BadRequestError(
                message=f"DeRouter doesn't support {custom_llm_provider} for 'cancel_batch'. Only 'openai', 'azure', 'vertex_ai', and 'bedrock' are supported.",
                model="n/a",
                llm_provider=custom_llm_provider,
                response=httpx.Response(
                    status_code=400,
                    content="Unsupported provider",
                    request=httpx.Request(method="cancel_batch", url="https://github.com/Delify-Solutions/derouter"),
                ),
            )
        return response
    except Exception as e:
...
def _handle_async_invoke_status(batch_id: str, aws_region_name: str, logging_obj=None, **kwargs) -> "DeRouterBatch":
    """
    ...
    from derouter.llms.bedrock.embed.embedding import BedrockEmbedding
    ...
    async def _async_get_status():
        ...
        # Transform response to a DeRouterBatch object
        ...
        # Map AWS status values to DeRouter expected values
