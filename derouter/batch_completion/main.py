...
import derouter
from derouter._logging import print_verbose
...
def batch_completion(
    ...
    **kwargs,
    ...
    """
    Batch derouter.completion function for a given model.
    ...
    if model.split("/", 1)[0] in derouter.provider_list:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            for sub_batch in chunks(batch_messages, 100):
                for message_list in sub_batch:
                    ...
                    future = executor.submit(derouter.completion, **kwargs_modified, **original_kwargs)
...
def batch_completion_models(*args, **kwargs):
    ...
    if "models" in kwargs:
        ...
        with ThreadPoolExecutor(max_workers=len(models)) as executor:
            for model in models:
                futures[model] = executor.submit(derouter.completion, *args, model=model, **kwargs)
        ...
        with ThreadPoolExecutor(max_workers=len(deployments)) as executor:
            for deployment in deployments:
                ...
                futures[deployment["model"]] = executor.submit(derouter.completion, **kwargs)
...
def batch_completion_models_all_responses(*args, **kwargs):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(models)) as executor:
        futures: Final = [executor.submit(derouter.completion, *args, model=model, **kwargs) for model in models]
