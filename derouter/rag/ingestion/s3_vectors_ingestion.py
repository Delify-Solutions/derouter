...
2. Uses DeRouter's embedding API (supports any provider)
...
class S3VectorsRAGIngestion(BaseRAGIngestion, BaseAWSLLM):
    ...
    3. Generate embeddings using DeRouter (supports any provider)
    ...
    async def _get_dimension_from_embedding_request(self) -> int:
        ...
        try:
            ...
            if self.router:
                ...
                response = await derouter.aembedding(model=model_name, input=[test_input])
    ...
    async def _ensure_config_initialized(self):
        ...
        if not self.index_name:
            ...
            self.index_name = f"derouter-index-{unique_id}"
    ...
    async def _sign_and_execute_request(
        ...
        Pattern from derouter/integrations/s3_v2.py
        """
        try:
            import requests
    ...
    async def embed(
        ...
        """
        Generate embeddings using DeRouter's embedding API.
        ...
        if self.router:
            ...
            response = await derouter.aembedding(model=embedding_model, input=input_chunks)
    ...
    async def query_vector_store(
        ...
        response = await derouter.aembedding(model=embedding_model, input=[query])
