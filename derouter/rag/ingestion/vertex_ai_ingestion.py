...
class VertexAIRAGIngestion(BaseRAGIngestion, VertexBase):
    ...
    def __init__(
        ...
        derouter_params: Final = dict(self.vector_store_config)

        # Get project, location, and credentials using VertexBase methods
        self.project_id = self.safe_get_vertex_ai_project(derouter_params)
        self.location = self.get_vertex_ai_location(derouter_params) or "us-central1"
        self.vertex_credentials = self.safe_get_vertex_ai_credentials(derouter_params)
    ...
    async def store(
        ...
        if not rag_corpus_id:
            rag_corpus_id = await self._create_rag_corpus(
                display_name=self.ingest_name or "derouter-rag-corpus",
