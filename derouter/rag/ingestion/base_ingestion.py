...
from derouter._uuid import uuid4
from derouter.constants import DEFAULT_CHUNK_OVERLAP, DEFAULT_CHUNK_SIZE
from derouter.derouter_core_utils.url_utils import async_safe_get
from derouter.llms.custom_httpx.http_handler import (
    get_async_httpx_client,
    httpxSpecialProvider,
)
from derouter.rag.ingestion.file_parsers import extract_text_from_pdf
from derouter.rag.text_splitters import RecursiveCharacterTextSplitter
from derouter.types.rag import RAGIngestOptions, RAGIngestResponse
...
class BaseRAGIngestion(ABC):
    ...
    def __init__(
        ...
        # Load credentials from derouter_credential_name if provided in vector_store config
    ...
    def _load_credentials_from_config(self) -> None:
        ...
        which will be resolved from derouter.credential_list. When a stored
        ...
        credential_name: Final = self.vector_store_config.get("derouter_credential_name")
        if credential_name and derouter.credential_list:
    ...
    async def ocr(
        ...
        if self.router is not None:
            ...
            ocr_response = await derouter.aocr(
                model=ocr_model,
                document={"type": doc_type, url_key: data_url},
            )
    ...
    async def embed(
        ...
        if self.router is not None:
            ...
            response = await derouter.aembedding(model=embedding_model, input=chunks)
