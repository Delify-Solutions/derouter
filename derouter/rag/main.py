...
from derouter.cost_calculator import vector_store_search_cost
...
from derouter.rag.rag_query import RAGQuery
from derouter.types.llms.openai import AllMessageValues
from derouter.types.rag import (
    RAGIngestOptions,
    RAGIngestResponse,
)
...
async def _execute_ingest_pipeline(
    ...
    Args:
        ...
        router: Optional DeRouter router for load balancing
...
async def aingest(
    ...
    **kwargs,
    ...
    Example:
        ```python
        response = await derouter.aingest(
            ingest_options={
                "vector_store": {
                    "custom_llm_provider": "openai",
                    "derouter_credential_name": "my-openai-creds",  # optional
                }
            },
...
async def _execute_query_pipeline(
    ...
    **kwargs,
    ...
    with _suppressed_sub_call_billing():
        search_response: Final = await derouter.vector_stores.asearch(
    ...
    if rerank and rerank.get("enabled"):
        ...
        if documents:
            with _suppressed_sub_call_billing():
                rerank_response = await derouter.arerank(
                    model=rerank["model"],
                    query=query_text,
                    documents=documents,
    ...
    if not stream and isinstance(response, ModelResponse):
        ...
        logging_obj: Final[object] = kwargs.get("derouter_logging_obj")
        if isinstance(logging_obj, DeRouterLoggingObj):
