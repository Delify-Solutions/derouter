"""
Public entrypoints for running model-generated code in a sandbox.

Low-level lifecycle:
    acreate_container -> arun_code -> adelete_container

High-level convenience:
    acode_interpreter_tool  (ephemeral: create -> run -> delete)

Each entrypoint is `@client`-decorated, so every operation is logged the same
way `derouter.asearch` is.
"""

from typing import Final

import derouter
from derouter.llms.base_llm.sandbox.transformation import (
    BaseSandboxConfig,
    CodeExecutionResult,
    ContainerHandle,
)
from derouter.types.utils import SandboxProviders
from derouter.utils import ProviderConfigManager, client

__all__ = [
    "acode_interpreter_tool",
    "acreate_sandbox",
    "adelete_sandbox",
    "arun_code",
]

_DEROUTER_INTERNAL_KWARGS: Final = {
    "derouter_logging_obj",
    "derouter_call_id",
    "derouter_trace_id",
    "derouter_metadata",
}


def _get_config(provider: str) -> BaseSandboxConfig:
    config: Final = ProviderConfigManager.get_provider_sandbox_config(SandboxProviders(provider))
    if config is None:
        raise ValueError(f"Code execution is not supported for provider: {provider}")
    return config


def _forward_kwargs(kwargs: dict) -> dict:
    return {k: v for k, v in kwargs.items() if k not in _DEROUTER_INTERNAL_KWARGS}


def _update_logging(kwargs: dict, provider: str, operation: str) -> None:
    logging_obj: Final = kwargs.get("derouter_logging_obj")
    if logging_obj is None:
        return
    logging_obj.update_from_kwargs(
        kwargs=kwargs,
        model=f"{provider}/{operation}",
        optional_params={},
        derouter_params={"derouter_call_id": kwargs.get("derouter_call_id")},
        custom_llm_provider=provider,
    )


@client
async def acreate_sandbox(
    provider: str,
    template: str | None = None,
    timeout: int | None = None,
    allow_internet_access: bool | None = None,
    api_key: str | None = None,
    api_base: str | None = None,
    **kwargs,
) -> ContainerHandle:
    _update_logging(kwargs, provider, "create_sandbox")
    return await _get_config(provider).acreate_sandbox(
        template=template,
        timeout=timeout,
        allow_internet_access=allow_internet_access,
        api_key=api_key,
        api_base=api_base,
        **_forward_kwargs(kwargs),
    )


@client
async def arun_code(
    provider: str,
    container: ContainerHandle | str,
    code: str,
    api_key: str | None = None,
    **kwargs,
) -> CodeExecutionResult:
    _update_logging(kwargs, provider, "run_code")
    return await _get_config(provider).arun_code(
        container=container,
        code=code,
        api_key=api_key,
        **_forward_kwargs(kwargs),
    )


@client
async def adelete_sandbox(
    provider: str,
    container: ContainerHandle | str,
    api_key: str | None = None,
    api_base: str | None = None,
    **kwargs,
) -> bool:
    _update_logging(kwargs, provider, "delete_sandbox")
    return await _get_config(provider).adelete_sandbox(
        container=container,
        api_key=api_key,
        api_base=api_base,
        **_forward_kwargs(kwargs),
    )


@client
async def acode_interpreter_tool(
    provider: str,
    code: str,
    template: str | None = None,
    timeout: int | None = None,
    api_key: str | None = None,
    api_base: str | None = None,
    **kwargs,
) -> CodeExecutionResult:
    _update_logging(kwargs, provider, "code_interpreter_tool")
    config: Final = _get_config(provider)
    forwarded: Final = _forward_kwargs(kwargs)

    container: Final = await config.acreate_sandbox(
        template=template,
        timeout=timeout,
        api_key=api_key,
        api_base=api_base,
        **forwarded,
    )
    try:
        return await config.arun_code(container=container, code=code, api_key=api_key, **forwarded)
    finally:
        try:
            await config.adelete_sandbox(container=container, api_key=api_key, api_base=api_base, **forwarded)
        except Exception as e:
            derouter._logging.verbose_logger.debug("sandbox: failed to delete ephemeral container: %s", e)
