...
from derouter.llms.azure.fine_tuning.handler import AzureOpenAIFineTuningAPI
from derouter.llms.openai.fine_tuning.handler import OpenAIFineTuningAPI
from derouter.llms.vertex_ai.fine_tuning.handler import VertexFineTuningAPI
from derouter.secret_managers.main import get_secret_str
from derouter.types.llms.openai import FineTuningJobCreate, Hyperparameters
from derouter.types.router import *
from derouter.types.utils import DeRouterFineTuningJob
from derouter.utils import client, supports_httpx_timeout
...
def create_fine_tuning_job(
    ...
    extra_body: dict[str, str] | None = None,
    **kwargs,
) -> DeRouterFineTuningJob | Coroutine[object, object, DeRouterFineTuningJob]:
    """
    ...
    try:
        ...
        if custom_llm_provider == "openai":
            ...
            response = openai_fine_tuning_apis_instance.create_fine_tuning_job(
                ...
                client=kwargs.get(
                    "client", None
                ),  # note, when we add this to `GenericDeRouterParams` it impacts a lot of other tests + linting
            )
        # Azure OpenAI
            ...
            raise derouter.exceptions.BadRequestError(
                message=f"DeRouter doesn't support {custom_llm_provider} for 'create_batch'. Only 'openai' is supported.",
                model="n/a",
    ...
    **kwargs,
    ...
    try:
        ...
        # Azure OpenAI
            ...
            raise derouter.exceptions.BadRequestError(
                message=f"DeRouter doesn't support {custom_llm_provider} for 'retrieve_fine_tuning_job'. Only 'openai' and 'azure' are supported.",
                model="n/a",
                llm_provider=custom_llm_provider,
                response=httpx.Response(
                    status_code=400,
