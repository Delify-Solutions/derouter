...
from derouter.passthrough.utils import CommonUtils
...
class AsyncPassthroughStreamingResponse(AsyncGenerator[bytes, bytes]):
    def __init__(
        ...
        self._derouter_logging_obj = derouter_logging_obj
    ...
    def _start_flush(self) -> None:
        ...
        try:
            task: Final = asyncio.create_task(
                self._derouter_logging_obj.async_flush_passthrough_collected_chunks(
...
class PassthroughStreamingResponse(Generator[bytes, bytes, None]):
    ...
    def _start_flush(self) -> None:
        ...
        from derouter.utils import executor

        try:
            executor.submit(
                self._derouter_logging_obj.flush_passthrough_collected_chunks,
                raw_bytes=self._raw_bytes,
                provider_config=self._provider_config,
            )
        except Exception as e:  # noqa: BLE001 # Safe catch-all for verbose logging
...
def llm_passthrough_route(
    ...
    **kwargs,
    ...
    derouter_logging_obj: Final = cast(
        DeRouterLoggingObj, kwargs.get("derouter_logging_obj")
    ...
    derouter_params_dict: Final = get_derouter_params(api_key=api_key, api_base=api_base, **kwargs)
    ...
    if client is None:
        ...
        from derouter.passthrough.timeout_utils import resolve_llm_passthrough_timeout
    ...
    # Add model_id to derouter_params if present in kwargs (for Bedrock Application Inference Profiles)
    if "model_id" in kwargs:
        derouter_params_dict["model_id"] = kwargs["model_id"]
    ...
    derouter_logging_obj.stream = is_streaming_request
    ...
    try:
        if _is_async:
            ...
            if hasattr(response, "iter_bytes") and is_streaming_request:
                return PassthroughStreamingResponse(response, derouter_logging_obj, provider_config)
            else:
                return response
    except Exception as e:
