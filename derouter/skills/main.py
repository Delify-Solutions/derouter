...
# Initialize DeRouter skills handler (lazy - only used when custom_llm_provider="derouter")
_derouter_skills_handler = None
...
def _get_derouter_skills_handler():
    """Lazy initialization of DeRouter skills handler to avoid import overhead."""
    global _derouter_skills_handler
    if _derouter_skills_handler is None:
        from derouter.llms.derouter_proxy.skills.transformation import (
            DeRouterSkillsTransformationHandler,
        )

        _derouter_skills_handler = DeRouterSkillsTransformationHandler()
    return _derouter_skills_handler
...
def create_skill(
    ...
    **kwargs,
    ...
    try:
        ...
        # Route to DeRouter DB if custom_llm_provider="derouter_proxy". description/instructions
        ...
        if custom_llm_provider == LlmProviders.DEROUTER_PROXY.value:
            return _get_derouter_skills_handler().create_skill_handler(
                display_title=display_title,
        ...
        headers = skills_api_provider_config.validate_environment(headers=headers, derouter_params=derouter_params)
        ...
        api_base: Final = AnthropicModelInfo.get_api_base(derouter_params.api_base)
...
def list_skills(
    ...
    **kwargs,
    ...
    try:
        ...
        # Route to DeRouter DB if custom_llm_provider="derouter_proxy"
        if custom_llm_provider == LlmProviders.DEROUTER_PROXY.value:
            return _get_derouter_skills_handler().list_skills_handler(
                limit=limit or 20,
                offset=0,
                user_api_key_dict=_get_user_api_key_auth_from_kwargs(kwargs),
                _is_async=_is_async,
...
def get_skill(
    ...
    **kwargs,
    ...
    try:
        ...
        # Route to DeRouter DB if custom_llm_provider="derouter_proxy"
        if custom_llm_provider == LlmProviders.DEROUTER_PROXY.value:
            return _get_derouter_skills_handler().get_skill_handler(
                skill_id=skill_id,
                user_api_key_dict=_get_user_api_key_auth_from_kwargs(kwargs),
                _is_async=_is_async,
                logging_obj=derouter_logging_obj,
...
def delete_skill(
    ...
    **kwargs,
    ...
    try:
        ...
        # Route to DeRouter DB if custom_llm_provider="derouter_proxy"
        if custom_llm_provider == LlmProviders.DEROUTER_PROXY.value:
            return _get_derouter_skills_handler().delete_skill_handler(
                skill_id=skill_id,
                user_api_key_dict=_get_user_api_key_auth_from_kwargs(kwargs),
                _is_async=_is_async,
                logging_obj=derouter_logging_obj,
