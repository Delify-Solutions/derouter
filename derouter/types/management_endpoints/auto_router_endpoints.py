...
class ShadowEvalSlice(BaseModel):
    ...
    real_spend: float = Field(
        ...
        description=(
            ...
            "classifier when it routed, excluding turns derouter's response cache served for free"
        ),
    )
    ...
    cache_hit_turns: int = Field(
        default=0,
        description=(
            "Judged turns derouter's response cache served, excluded from both spends: an adopted router "
