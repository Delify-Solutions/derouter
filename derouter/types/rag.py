...
class OpenAIVectorStoreOptions(TypedDict, total=False):
    ...
    Example (with credentials):
        {"custom_llm_provider": "openai", "derouter_credential_name": "my-openai-creds"}
    """
    ...
    # Credentials (loaded from derouter.credential_list if derouter_credential_name is provided)
    derouter_credential_name: str | None  # Credential name to load from derouter.credential_list
    api_key: str | None  # Direct API key (alternative to derouter_credential_name)
    api_base: str | None  # Direct API base (alternative to derouter_credential_name)
...
class BedrockVectorStoreOptions(TypedDict, total=False):
    ...
    Example (with credentials):
        {"custom_llm_provider": "bedrock", "derouter_credential_name": "my-aws-creds"}
...
class VertexAIVectorStoreOptions(TypedDict, total=False):
    ...
    - Files are uploaded to GCS via derouter.files.create_file, then imported into RAG corpus
...
class S3VectorsVectorStoreOptions(TypedDict, total=False):
    ...
    Embeddings are generated using DeRouter's embedding API (supports any provider).
    """
...
class RAGIngestOptions(TypedDict, total=False):
    ...
    Example (OpenAI):
        from derouter.types.rag import RAGIngestOptions, OpenAIVectorStoreOptions
    ...
    Example (Bedrock):
        from derouter.types.rag import RAGIngestOptions, BedrockVectorStoreOptions
