...
class MCPToolSearchSettings(BaseModel):
    """`derouter_settings.mcp_tool_search`: how the native `mcp_tool_search` virtual tool ranks the caller's tools."""
...
class MCPCredentials(TypedDict, total=False):
    ...
    """
    Which upstream header carries the credential DeRouter resolves for this server. Omitted when
...
``ADMIN_CONFIG_CREDENTIAL_KEYS`` in ``ui/derouter-dashboard/src/components/mcp_tools/types.tsx``."""
