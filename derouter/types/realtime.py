...
class RealtimeClientSecretRequest(BaseModel):
    ...
    DeRouter also accepts a top-level `model` field for routing when
    session.model is absent (DeRouter extension, not forwarded to OpenAI).
    """

    expires_after: RealtimeExpiresAfter | None = None
    session: RealtimeSessionConfig | None = None
    # DeRouter-only routing hint — stripped before forwarding upstream
    model: str | None = None
...
class RealtimeTranscriptionSessionRequest(BaseModel):
    ...
    for routing is taken from the DeRouter-only top-level `model` hint, falling
