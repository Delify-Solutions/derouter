...
class DeRouter_VectorStoreConfig(TypedDict, total=False):
    """Parameters for initializing a vector store on Litellm proxy config.yaml"""

    vector_store_name: str
    derouter_params: dict[str, Any] | None


class DeRouter_ManagedVectorStore(TypedDict, total=False):
    """DeRouter managed vector store object - this is is the object stored in the database"""
...
class DeRouter_ManagedVectorStoreListResponse(TypedDict, total=False):
    """Response format for listing vector stores"""

    object: Literal["list"]  # Always "list"
    data: list[DeRouter_ManagedVectorStore]
...
class IndexCreateDeRouterParams(BaseModel):
...
class IndexCreateRequest(BaseModel):
    index_name: str
    derouter_params: IndexCreateDeRouterParams
...
class DeRouter_ManagedVectorStoreIndex(BaseModel):
    """DeRouter managed vector store index object - this is is the object stored in the database"""
...
class IndexListResponse(BaseModel):
    ...
    data: tuple[DeRouter_ManagedVectorStoreIndex, ...]
