"""
derouter.Router Types - includes RouterConfig, UpdateRouterConfig, ModelInfo etc
"""
...
class ModelConfig(BaseModel):
    model_name: str
    derouter_params: CompletionRequest | EmbeddingRequest
    tpm: int
    rpm: int
...
class RouterConfig(BaseModel):
    ...
    default_derouter_params: dict[str, str] | None = {}
...
class RetryPolicy(BaseModel):
    ...
    https://router.delify.vn/docs/exception_mapping
    """
...
class ModelInfo(MirroredPricingParams):
    ...
    # admin-toggled pause flag; mirrors DeRouter_ProxyModelTable.blocked
    blocked: bool | None = None

    # Bounds live on the model rather than derouter.constants: names there reach
    # derouter/__init__ through several modules' star re-exports, and a Final rebound that
...
class CredentialDeRouterParams(BaseModel):
    api_key: str | None = None
    api_base: str | None = None
    api_version: str | None = None
    ## AZURE OAUTH ##
    # Without this field, ``get_deployment_credentials_with_provider``
    # round-trips ``derouter_params`` through a strict Pydantic dump and
...
class GenericDeRouterParams(CredentialDeRouterParams, CustomPricingDeRouterParams):
    """
    DeRouter Params without 'model' arg (used across completion / assistants api)
    """
    ...
    use_derouter_proxy: bool | None = False
...
class DeRouter_Params(GenericDeRouterParams):
    """
    DeRouter Params with 'model' requirement - used for completions
    """
...
class updateDeRouterParams(GenericDeRouterParams):
    # This class is used to update the DeRouter_Params
...
class updateDeployment(BaseModel):
    ...
    derouter_params: updateDeRouterParams | None = None
...
class DeRouterParamsTypedDict(TypedDict, total=False):
    model: str
    ...
    derouter_credential_name: str | None
    ## UNIFIED PROJECT/REGION ##
...
class DeploymentTypedDict(TypedDict, total=False):
    ...
    derouter_params: Required[DeRouterParamsTypedDict]
    model_info: dict
    ...
    def __init__(
        ...
        **params,
        ...
        for key in SPECIAL_MODEL_INFO_PARAMS:  # ensures custom pricing info is consistently in 'model_info'
            field = getattr(derouter_params, key, None)
...
class AssistantsTypedDict(TypedDict):
    ...
    derouter_params: DeRouterParamsTypedDict


class SearchToolDeRouterParams(TypedDict, total=False):
    """
    DeRouter params for search tools.
...
class GuardrailDeRouterParams(TypedDict, total=False):
    """
    DeRouter params for guardrails.
    """
...
class GuardrailTypedDict(TypedDict, total=False):
    ...
    derouter_params: Required[GuardrailDeRouterParams]
...
class CustomRoutingStrategyBase:
    async def async_get_available_deployment(
        ...
        Returns:
            Returns an element from derouter.router.model_list
...
class RouterGeneralSettings(BaseModel):
    ...
    pass_through_all_models: bool = Field(
        default=False
    )  # if passed a model not llm_router model list, pass through the request to derouter.acompletion/embedding
...
class DeRouter_RouterFileObject(TypedDict, total=False):
    """
    Tracking the derouter params hash, used for mapping the file id to the right model
    """

    derouter_params_sensitive_credential_hash: str
...
class RoutingContext(BaseModel):
    ...
    only selects a deployment whose `derouter_params.model` survives the pipeline.
