...
Prisma's generated client is untyped at the ``derouter`` boundary, so the row
...
if TYPE_CHECKING:
    from derouter.models.managed_files import DeRouter_ManagedFileTable
...
class ManagedResourceRow(Protocol):
    """Columns shared by ``DeRouter_ManagedFileTable`` and ``DeRouter_ManagedObjectTable`` rows."""
...
class ManagedFileIdReader(Protocol):
    ...
    async def get_unified_file_id(
        self,
        file_id: str,
        derouter_parent_otel_span: object = None,
    ) -> DeRouter_ManagedFileTable | None: ...
...
class ManagedFileIdWriter(Protocol):
    ...
    async def store_unified_file_id(
        ...
        derouter_parent_otel_span: object,
