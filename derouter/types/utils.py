...
from derouter.types.llms.base import (
    BaseDeRouterOpenAIResponseObject,
    DeRouterPydanticObjectBase,
)
from derouter.types.mcp import MCPServerCostInfo

from ..derouter_core_utils.core_helpers import map_finish_reason, process_response_headers
from .agents import DeRouterSendMessageResponse
...
class DeRouterCommonStrings(Enum):
    redacted_by_derouter = "redacted by derouter. 'derouter.turn_off_message_logging=True'"
    llm_provider_not_provided = "Unmapped LLM provider for this endpoint. You passed model={model}, custom_llm_provider={custom_llm_provider}. Check supported provider and route: https://router.delify.vn/docs/providers"
...
class ModelInfo(ModelInfoBase, total=False):
    """
    Model info for a given model, this is information found in derouter.model_prices_and_context_window.json
    """
...
class ChatCompletionTokenLogprob(OpenAIObject):
    ...
    def ensure_top_logprobs_is_list(cls, v):
        ...
        Fixes https://github.com/Delify-Solutions/derouter/issues/21932
        """
        if v is None:
            return []
        return v
        ...
        **params,
        ...
        if message is None:
            ...
            if isinstance(message, Message):
                ...
                # Normalize provider/OpenAI SDK message models into DeRouter's Message type.
...
class StreamingChoices(OpenAIObject):
    def __init__(
        ...
        **params,
        ...
        # https://github.com/Delify-Solutions/derouter/issues/8455
...
class ImageUsageInputTokensDetails(BaseDeRouterOpenAIResponseObject):
    image_tokens: int
...
class ImageUsage(BaseDeRouterOpenAIResponseObject):
    input_tokens: int
...
class ImageResponse(OpenAIImageResponse, BaseDeRouterOpenAIResponseObject):
    _hidden_params: dict = {}

    usage: ImageUsage | None = None
    """
    Users might use derouter with older python versions, we don't want this to break for them.
...
class LoggedDeRouterParams(TypedDict, total=False):
...
class StandardLoggingUserAPIKeyMetadata(TypedDict):
    user_api_key_hash: str | None  # hash of the derouter virtual key used
...
class StandardLoggingMCPToolCall(TypedDict, total=False):
    ...
    (this is to render the logo on the logs page on derouter ui)
    """
...
"""Which internal derouter feature originated a billed sub-call, so a spend log row
...
class StandardLoggingRoutingDecision(TypedDict, total=False):
    ...
    tier_derouter_params: Mapping[str, object]  # writable-ok: Pydantic warns on ReadOnly TypedDict fields
...
class StandardLoggingHiddenParams(TypedDict):
    ...
    derouter_overhead_time_ms: float | None
    ...
    derouter_model_name: str | None  # the model name sent to the provider by derouter
    usage_object: dict | None
...
class StandardLoggingPayloadErrorInformation(TypedDict, total=False):
    ...
    #   string values defined by `derouter.exceptions.RateLimitErrorCategory`
    #   (vendor_rate_limit, vendor_batch_rate_limit, derouter_rate_limit,
    #   derouter_batch_rate_limit). None for non-rate-limit exceptions.
    ...
    #   the string values defined by `derouter.exceptions.RateLimitType`
...
# prompt substrings (``block_code_execution`` and ``derouter_content_filter``). Every other field
...
class StandardLoggingPayload(TypedDict):
    ...
    session_id: str  # End-user/conversation session id (derouter_session_id), independent of trace_id
    derouter_call_id: str | None  # UUID returned in x-derouter-call-id response header
    call_type: str
    stream: bool | None
    response_cost: float
...
class StandardCallbackDynamicParams(TypedDict, total=False):
    ...
    derouter_disabled_callbacks: list[str] | None


class MirroredPricingParams(BaseModel):
    """Pricing overrides that ``Deployment.__init__`` mirrors from ``derouter_params``
...
class CustomPricingDeRouterParams(MirroredPricingParams):
    ## CUSTOM PRICING ##
    ...
    # This allows any model_info parameter to be set in derouter_params
    ...
    def strip_custom_pricing_fields(cls, model_info: dict[str, Any]) -> dict[str, Any]:
        ...
        ``{provider}/{model}`` key in ``derouter.model_cost``, so one deployment's
...
SHARED_BACKEND_MODEL_INFO_FIELDS: Final[frozenset[str]] = (
    ...
    - frozenset(CustomPricingDeRouterParams.model_fields)
...
def shared_backend_model_info(model_info: dict[str, Any]) -> dict[str, Any]:
    ...
    key in ``derouter.model_cost``: cost-map schema fields (``ModelInfoBase``) minus
...
# in all_derouter_params so they are treated as DeRouter-level and excluded from
...
# scrubber.
agentic_loop_internal_derouter_params: Final = [
    "_agentic_loop_depth",
...
# settings. Listed in all_derouter_params for the same reason as the agentic-loop
# fields above: an unrecognized top-level key is swept into extra_body and sent to
# the provider.
TRUSTED_CALLBACK_VARS_FIELD: Final = "derouter_trusted_callback_vars"

# Bedrock managed-batch deployment config, read from derouter_params by the batch and
...
bedrock_batch_derouter_params: Final = (
    "aws_batch_role_arn",
    "s3_bucket_name",
    "s3_region_name",
...
all_derouter_params = (
    agentic_loop_internal_derouter_params
    + [TRUSTED_CALLBACK_VARS_FIELD, *bedrock_batch_derouter_params]
    + [
        "metadata",
        "derouter_metadata",
        "keepalive_seconds",
    ...
    + list(CustomPricingDeRouterParams.model_fields.keys())
)
...
OPENAI_COMPATIBLE_BATCH_AND_FILES_PROVIDERS: set[str] = {
    ...
    LlmProviders.DEROUTER_PROXY.value,
}

ListBatchesSupportedProvider = Literal["openai", "azure", "hosted_vllm", "derouter_proxy", "vertex_ai"]
...
class DeRouterLoggingBaseClass:
    """
...
class TokenCountResponse(DeRouterPydanticObjectBase):
    total_tokens: int
    request_model: str
    model_used: str
    tokenizer_type: str
...
class DEROUTER_IMAGE_VARIATION_PROVIDERS(Enum):
    """
...
class DeRouterFineTuningJob(FineTuningJob):
    _hidden_params: dict = {}
    seed: int | None = None
...
class DeRouterRealtimeStreamLoggingObject(DeRouterPydanticObjectBase):
...
from derouter.models.credentials import (  # noqa: E402
    CreateCredentialItem as CreateCredentialItem,
)
from derouter.models.credentials import CredentialBase as CredentialBase  # noqa: E402
from derouter.models.credentials import CredentialItem as CredentialItem  # noqa: E402
...
class SpecialEnums(Enum):
    LITELM_MANAGED_FILE_ID_PREFIX = "derouter_proxy"
    DEROUTER_MANAGED_FILE_COMPLETE_STR = (
        "derouter_proxy:{};unified_id,{};target_model_names,{};llm_output_file_id,{};llm_output_file_model_id,{}"
    )

    DEROUTER_MANAGED_RESPONSE_COMPLETE_STR = "derouter:custom_llm_provider:{};model_id:{};response_id:{}"

    DEROUTER_MANAGED_BATCH_COMPLETE_STR = "derouter_proxy;model_id:{};llm_batch_id:{}"

    DEROUTER_MANAGED_RESPONSE_API_RESPONSE_ID_COMPLETE_STR = (
        "derouter_proxy:responses_api:response_id:{};user_id:{};team_id:{}"
    )

    DEROUTER_MANAGED_GENERIC_RESPONSE_COMPLETE_STR = "derouter_proxy;model_id:{};generic_response_id:{}"  # generic implementation of 'managed batches' - used for finetuning and any future work.

    DEROUTER_MANAGED_VIDEO_COMPLETE_STR = "derouter:custom_llm_provider:{};model_id:{};video_id:{}"

    DEROUTER_PASSTHROUGH_MANAGED_ID_COMPLETE_STR = "derouter_proxy:passthrough;provider:{};unified_id,{};raw_id,{}"
...
class PriorityReservationDict(TypedDict, total=False):
    ...
    Used in derouter.priority_reservation to specify how much capacity to reserve
...
class PriorityReservationSettings(BaseModel):
    ...
    The priority_reservation mapping is configured separately via derouter.priority_reservation.
    """

    default_priority: float = Field(
        default=0.25,
        description="Priority level to assign to API keys without explicit priority metadata. Should match a key in derouter.priority_reservation.",
    )
