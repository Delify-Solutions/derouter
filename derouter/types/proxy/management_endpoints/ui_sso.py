...
from derouter.proxy._types import KeyManagementRoutes, LitellmUserRoles
from derouter.types.utils import DeRouterPydanticObjectBase


class DeRouter_UpperboundKeyGenerateParams(DeRouterPydanticObjectBase):
    """
...
class AccessControl_UI_AccessMode(DeRouterPydanticObjectBase):
...
class RoleMappings(DeRouterPydanticObjectBase):
    """
    Configuration for mapping SSO groups to DeRouter roles.
    ...
    roles: dict[LitellmUserRoles, list[str]] = Field(
        ...
        description="Mapping of DeRouter role names to arrays of SSO group names. Example: {'proxy_admin': ['group-1', 'group-2'], 'proxy_admin_viewer': ['group-3']}",
    )


class TeamMappings(DeRouterPydanticObjectBase):
    """
...
class SSOConfig(DeRouterPydanticObjectBase):
    """
    ...
    role_mappings: RoleMappings | None = Field(
        default=None,
        description="Configuration for mapping SSO groups to DeRouter roles based on group claims in the SSO token",
    )
...
class DefaultTeamSSOParams(DeRouterPydanticObjectBase):
    """
    Default parameters applied to every /team/new call for fields not explicitly provided in the request.
    `models` is the exception: it only applies to teams automatically created by DeRouter via SSO Groups.
    """
