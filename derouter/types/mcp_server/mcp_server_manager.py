...
class MCPServer(BaseModel):
    ...
    # Which upstream header carries the credential DeRouter resolves for this server (the minted
    ...
    # bypass DeRouter API-key/SSO auth (and the pre-emptive 401) so the client
    ...
    # Resolved short-ID tool prefix when DEROUTER_USE_SHORT_MCP_TOOL_PREFIX is
    ...
    def is_true_passthrough(self) -> bool:
        """True for the transparent-proxy mode: DeRouter performs no admission auth and forwards the
    ...
    def is_oauth_delegate(self) -> bool:
        """True for the delegated-upstream-OAuth mode: DeRouter still admits the caller (API key / SSO /
