...
class BudgetManager:
    def __init__(
        ...
        self.api_base = api_base or "https://api.router.delify.vn"
    ...
    def load_data(self):
        if self.client_type == "local":
            ...
            response = derouter.module_level_client.post(url, headers=self.headers, json=data)
    ...
    def projected_cost(self, model: str, messages: list, user: str):
        ...
        prompt_tokens: Final = derouter.token_counter(model=model, text=text)
        prompt_cost, _ = derouter.cost_per_token(model=model, prompt_tokens=prompt_tokens, completion_tokens=0)
    ...
    def update_cost(
        ...
        if model and input_text and output_text:
            prompt_tokens = derouter.token_counter(model=model, messages=[{"role": "user", "content": input_text}])
            completion_tokens = derouter.token_counter(model=model, messages=[{"role": "user", "content": output_text}])
            (
                prompt_tokens_cost_usd_dollar,
                completion_tokens_cost_usd_dollar,
            ) = derouter.cost_per_token(
                model=model,
            ...
            cost = derouter.completion_cost(completion_response=completion_obj)
            model = completion_obj["model"]  # if this throws an error try, model = completion_obj['model']
        else:
            raise ValueError(
                "Either a chat completion object or the text response needs to be passed in. Learn more - https://router.delify.vn/docs/budget_manager"
            )
