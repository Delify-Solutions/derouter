"""
DeRouter Proxy uses this MCP Client to connnect to other MCP servers.
"""
...
def missing_streamable_http_client_error() -> ImportError:
    return ImportError(
        ...
        "Fix with: pip install 'derouter[mcp]' (or upgrade mcp directly: pip install -U mcp)"
    )
...
from derouter.constants import MCP_CLIENT_TIMEOUT, MCP_NPM_CACHE_DIR, MCP_TOOL_LISTING_TIMEOUT
from derouter.experimental_mcp_client.tools import list_tools_with_pagination
from derouter.llms.custom_httpx.http_handler import get_ssl_configuration
from derouter.types.llms.custom_http import VerifyTypes
...
class MCPSigV4Auth(httpx.Auth):
    ...
    def _assume_role(
        ...
        session_name: Final = aws_session_name or f"derouter-mcp-{int(time.time())}"
...
class MCPClient:
    ...
    def _create_transport_context(
        ...
        headers = self._get_auth_headers()
        httpx_client_factory = self._create_httpx_client_factory()
        verbose_logger.debug("derouter headers for streamable_http_client: %s", headers)
    ...
    def _get_safe_stdio_env(self, provided_env: dict[str, str] | None) -> dict[str, str] | None:
        ...
        to avoid leaking sensitive DeRouter keys (OPENAI_API_KEY, etc.) to sub-processes.
        """
    ...
    def _create_httpx_client_factory(self) -> Callable[..., httpx.AsyncClient]:
        """
        Create a custom httpx client factory that uses DeRouter's SSL configuration.
        ...
        def factory(
            *,
            ...
            """Create an httpx.AsyncClient with DeRouter's SSL configuration."""
