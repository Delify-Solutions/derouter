...
from derouter.derouter_core_utils.env_utils import get_env_int, get_env_int_in_range, get_env_int_or_none

DEFAULT_HEALTH_CHECK_PROMPT: Final = str(os.getenv("DEFAULT_HEALTH_CHECK_PROMPT", "test from derouter"))
...
DEFAULT_NUM_WORKERS_DEROUTER_PROXY: Final = int(os.getenv("DEFAULT_NUM_WORKERS_DEROUTER_PROXY", 1))
...
_max_stream_duration_env: Final = os.getenv("DEROUTER_MAX_STREAMING_DURATION_SECONDS", None)
DEROUTER_MAX_STREAMING_DURATION_SECONDS: Final = (
...
REDACTED_BY_DEROUTER: Final = "redacted-by-derouter"
...
# Headers: x-derouter-timing-{pre-processing,llm-api,post-processing,message-copy}-ms
DEROUTER_DETAILED_TIMING: Final = os.getenv("DEROUTER_DETAILED_TIMING", "false").lower() == "true"
...
MCP_CLIENT_TIMEOUT: Final = float(os.getenv("DEROUTER_MCP_CLIENT_TIMEOUT", "60.0"))
MCP_TOOL_LISTING_TIMEOUT: Final = float(os.getenv("DEROUTER_MCP_TOOL_LISTING_TIMEOUT", "30.0"))
MCP_METADATA_TIMEOUT: Final = float(os.getenv("DEROUTER_MCP_METADATA_TIMEOUT", "10.0"))
MCP_HEALTH_CHECK_TIMEOUT: Final = float(os.getenv("DEROUTER_MCP_HEALTH_CHECK_TIMEOUT", "10.0"))
...
# Extend via DEROUTER_MCP_STDIO_EXTRA_COMMANDS env var (comma-separated).
_MCP_STDIO_EXTRA_COMMANDS: Final = os.getenv("DEROUTER_MCP_STDIO_EXTRA_COMMANDS", "")
...
DEROUTER_UI_ALLOW_HEADERS: Final = [
    "x-derouter-semantic-filter",
    "x-derouter-semantic-filter-tools",
    "x-derouter-adaptive-router-model",
    "x-derouter-applied-guardrails",
    "x-derouter-guardrail-scan-id",
    "x-derouter-cache-key",
]
...
# Override with DEROUTER_MAX_CALLBACKS env var for large deployments (e.g., many teams with guardrails)
MAX_CALLBACKS: Final = get_env_int("DEROUTER_MAX_CALLBACKS", 100)
...
LOGS_GUARDRAIL_INFORMATION_MARKER: Final = "_derouter_logs_guardrail_information"
...
# The earliest an evicted, derouter-created client may be closed. A request handed the
...
REDIS_UPDATE_BUFFER_KEY: Final = "derouter_spend_update_buffer"
REDIS_DAILY_SPEND_UPDATE_BUFFER_KEY: Final = "derouter_daily_spend_update_buffer"
REDIS_DAILY_TEAM_SPEND_UPDATE_BUFFER_KEY: Final = "derouter_daily_team_spend_update_buffer"
REDIS_DAILY_ORG_SPEND_UPDATE_BUFFER_KEY: Final = "derouter_daily_org_spend_update_buffer"
REDIS_DAILY_END_USER_SPEND_UPDATE_BUFFER_KEY: Final = "derouter_daily_end_user_spend_update_buffer"
REDIS_DAILY_AGENT_SPEND_UPDATE_BUFFER_KEY: Final = "derouter_daily_agent_spend_update_buffer"
REDIS_DAILY_TAG_SPEND_UPDATE_BUFFER_KEY: Final = "derouter_daily_tag_spend_update_buffer"
REDIS_WINDOW_SPEND_UPDATE_BUFFER_KEY: Final = "derouter_window_spend_update_buffer"
MAX_REDIS_BUFFER_DEQUEUE_COUNT: Final = int(os.getenv("MAX_REDIS_BUFFER_DEQUEUE_COUNT", 100))
# Bounds asyncio.Queue() instances (log queues, spend update queues, etc.) to prevent unbounded memory growth
DEROUTER_ASYNCIO_QUEUE_MAXSIZE: Final = int(os.getenv("DEROUTER_ASYNCIO_QUEUE_MAXSIZE", 1000))
...
# Must be < DEROUTER_ASYNCIO_QUEUE_MAXSIZE; if set higher the aggregation logic will never fire.
MAX_SIZE_IN_MEMORY_QUEUE: Final = int(os.getenv("MAX_SIZE_IN_MEMORY_QUEUE", int(DEROUTER_ASYNCIO_QUEUE_MAXSIZE * 0.8)))
...
#### Networking settings ####
# Sentinel used when `REQUEST_TIMEOUT` is unset: `derouter.request_timeout` keeps this
# value so longer-running surfaces (Router `timeout or derouter.request_timeout`,
...
DEROUTER_HTTP_STATUS_CLIENT_DISCONNECTED: Final = 499
...
# DeRouter standard web search tool name
# Used for web search interception across providers
DEROUTER_WEB_SEARCH_TOOL_NAME: Final = "derouter_web_search"
...
DEROUTER_EMBEDDING_PROVIDERS_SUPPORTING_INPUT_ARRAY_OF_TOKENS: Final = [
    "openai",
    "azure",
    "hosted_vllm",
    "nebius",
...
)  # these have been tested on extensively. But by default all text2text-generation and text-generation models are supported by liteLLM. - https://router.delify.vn/docs/providers
...
X_DEROUTER_DISABLE_CALLBACKS: Final = "x-derouter-disable-callbacks"
DEROUTER_METADATA_FIELD: Final = "derouter_metadata"
OLD_DEROUTER_METADATA_FIELD: Final = "metadata"
...
SESSION_ID_GENERATED_METADATA_KEY: Final = "derouter_session_id_generated"
SESSION_ID_OMITTED_METADATA_KEY: Final = "derouter_session_id_omitted"
DEROUTER_TRUNCATED_PAYLOAD_FIELD: Final = "derouter_truncated"
DEROUTER_TRUNCATION_DB_SAFEGUARD_NOTE: Final = (
...
DEROUTER_TRUNCATION_STDOUT_SAFEGUARD_NOTE: Final = (
...
########################### DeRouter Proxy Specific Constants ###########################
########################################################################################
...
STANDARD_CUSTOMER_ID_HEADERS: Final = [
    "x-derouter-customer-id",
    "x-derouter-end-user-id",
]
...
)  # by default all derouter proxy keys have a soft budget of 50.0
# makes it clear this is a rate limit error for a derouter virtual key
RATE_LIMIT_ERROR_MESSAGE_FOR_VIRTUAL_KEY: Final = "DeRouter Virtual Key user_api_key_hash"
# Prefix of the 401 raised when a submitted virtual key is not shaped like one.
INVALID_VIRTUAL_KEY_ERROR_MESSAGE: Final = "DeRouter Virtual Key expected"
...
# be used to classify.
INVALID_VIRTUAL_KEY_ERROR_MARKER: Final = "_derouter_invalid_virtual_key_error"
...
LITTELM_INTERNAL_HEALTH_SERVICE_ACCOUNT_NAME: Final = "derouter-internal-health-check"
LITTELM_CLI_SERVICE_ACCOUNT_NAME: Final = "derouter-cli"
DEROUTER_INTERNAL_JOBS_SERVICE_ACCOUNT_NAME: Final = "derouter_internal_jobs"
...
DEROUTER_PROXY_MASTER_KEY_ALIAS: Final = "derouter_proxy_master_key"
...
DEROUTER_LOGGING_NO_UPSTREAM_LLM_CALL: Final = "derouter_no_upstream_llm_call"
...
DEROUTER_KEY_ROTATION_ENABLED: Final = os.getenv("DEROUTER_KEY_ROTATION_ENABLED", "false")
DEROUTER_KEY_ROTATION_CHECK_INTERVAL_SECONDS: Final = int(
    os.getenv("DEROUTER_KEY_ROTATION_CHECK_INTERVAL_SECONDS", 86400)
)  # 24 hours default
DEROUTER_KEY_ROTATION_GRACE_PERIOD: Final[str] = os.getenv(
    "DEROUTER_KEY_ROTATION_GRACE_PERIOD", ""
)  # Duration to keep old key valid after rotation (e.g. "24h", "2d"); empty = immediate revoke (default)
DEROUTER_KEY_ROTATION_LOCK_TTL_SECONDS: Final = int(
    os.getenv("DEROUTER_KEY_ROTATION_LOCK_TTL_SECONDS", 600)
)  # 10 minutes default — caps the deadlock window if a pod crashes mid-rotation
UI_SESSION_TOKEN_TEAM_ID: Final = "derouter-dashboard"
DEROUTER_EXPIRED_UI_SESSION_KEY_CLEANUP_ENABLED = os.getenv("DEROUTER_EXPIRED_UI_SESSION_KEY_CLEANUP_ENABLED", "false")
DEROUTER_EXPIRED_UI_SESSION_KEY_CLEANUP_INTERVAL_SECONDS: Final = int(
    os.getenv("DEROUTER_EXPIRED_UI_SESSION_KEY_CLEANUP_INTERVAL_SECONDS", 86400)
)  # 24 hours default
DEROUTER_EXPIRED_UI_SESSION_KEY_CLEANUP_BATCH_SIZE: Final = int(
    os.getenv("DEROUTER_EXPIRED_UI_SESSION_KEY_CLEANUP_BATCH_SIZE", 1000)
)
DEROUTER_PROXY_ADMIN_NAME: Final = "default_user_id"
DEROUTER_PROXY_BUDGET_NAME: Final = "derouter-proxy-budget"
GLOBAL_PROXY_SPEND_CACHE_KEY: Final = f"{DEROUTER_PROXY_ADMIN_NAME}:spend"

########################### CLI SSO AUTHENTICATION CONSTANTS ###########################
DEROUTER_CLI_SOURCE_IDENTIFIER: Final = "derouter-cli"
DEROUTER_CLI_SESSION_TOKEN_PREFIX: Final = "derouter-session-token"
...
# Support both CLI_JWT_EXPIRATION_HOURS and DEROUTER_CLI_JWT_EXPIRATION_HOURS for backwards compatibility
CLI_JWT_EXPIRATION_HOURS: Final = int(
    os.getenv("CLI_JWT_EXPIRATION_HOURS") or os.getenv("DEROUTER_CLI_JWT_EXPIRATION_HOURS") or 24
)
...
CLI_SSO_CLAIM_MAP: Final = os.getenv("CLI_SSO_CLAIM_MAP") or os.getenv("DEROUTER_CLI_SSO_CLAIM_MAP") or ""
...
DEROUTER_UI_SESSION_DURATION: Final = os.getenv("DEROUTER_UI_SESSION_DURATION", "24h")
...
KEY_ROTATION_JOB_NAME: Final = "derouter_key_rotation_job"
EXPIRED_UI_SESSION_KEY_CLEANUP_JOB_NAME: Final = "derouter_expired_ui_session_key_cleanup_job"
...
LENGTH_OF_DEROUTER_GENERATED_KEY: Final = int(os.getenv("LENGTH_OF_DEROUTER_GENERATED_KEY", 16))
...
DEROUTER_SETTINGS_SAFE_DB_OVERRIDES: Final = [
    ...
    # Every field editable from the Admin UI (proxy_server._GENERAL_SETTINGS_UI_DEROUTER_FIELDS)
    # must be listed here so a DB write from one worker overrides the live derouter attribute on
...
SPECIAL_DEROUTER_AUTH_TOKEN: Final = ["ui-token"]
...
# Providers that support advisor_20260301 natively (no DeRouter orchestration needed).
...
# PTU reservation rollup writes rows to DeRouter_DailyTeamSpend with this
