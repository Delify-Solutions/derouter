...
This module implements lazy loading for DeRouter attributes. Instead of importing
everything when the module loads, we only import things when they're actually used.

How it works:
1. When someone accesses `derouter.some_attribute`, Python calls __getattr__ in __init__.py
...
This makes importing derouter much faster because we don't load heavy dependencies
...
def get_derouter_globals() -> dict[str, object]:
    """
    Get the globals dictionary of the derouter module.

    This is where we cache imported attributes so we don't import them twice.
    When you do `derouter.some_function`, it gets stored in this dictionary.
    """
    return sys.modules["derouter"].__dict__
...
def _get_utils_globals() -> dict[str, object]:
    ...
    This is where we cache imported attributes so we don't import them twice.
    When you do `derouter.utils.some_function`, it gets stored in this dictionary.
    """
    return sys.modules["derouter.utils"].__dict__


def _get_module_level_client_timeout(derouter_globals: Mapping[str, Any]) -> "float | httpx.Timeout | None":
    """Read the configured `derouter.request_timeout` used for the module level http clients."""
    return derouter_globals.get("request_timeout")
...
def _get_default_encoding() -> "Encoding":
    ...
    This avoids importing `derouter.derouter_core_utils.default_encoding` (and thus tiktoken)
    at `derouter` import time. The encoding is cached after the first import.
    ...
    if _default_encoding is None:
        from derouter.derouter_core_utils.default_encoding import encoding
...
def _get_modified_max_tokens() -> "Callable[..., int | None]":
    ...
    This avoids importing `derouter.derouter_core_utils.token_counter` at `derouter` import time.
    ...
    if _get_modified_max_tokens_func is None:
        from derouter.derouter_core_utils.token_counter import (
...
def _get_lazy_import_registry() -> dict[str, Callable[[str], object]]:
    ...
    if _LAZY_IMPORT_REGISTRY is None:
        ...
        for name in DEROUTER_LOGGING_NAMES:
            _LAZY_IMPORT_REGISTRY[name] = _lazy_import_derouter_logging
...
def _generic_lazy_import(name: str, import_map: Mapping[str, tuple[str, str]], category: str) -> object:
    ...
    _globals: Final = get_derouter_globals()
    ...
    # If module_path starts with ".", it's a relative import (needs package="derouter")
    # Otherwise it's an absolute import (like "derouter.caching.caching")
    if module_path.startswith("."):
        module = importlib.import_module(module_path, package="derouter")
    else:
...
def _lazy_import_types_utils(name: str) -> object:
    """Handler for types from derouter.types.utils (BudgetConfig, ImageObject, etc.)"""
...
def _lazy_import_derouter_logging(name: str) -> object:
    """Handler for derouter_logging module (Logging, modify_integration)"""
    return _generic_lazy_import(name, _DEROUTER_LOGGING_IMPORT_MAP, "Litellm logging")
...
def _lazy_import_utils_module(name: str) -> object:
    ...
    _get_utils_globals() instead of get_derouter_globals() for caching.
    """
...
def _lazy_import_http_handlers(name: str) -> object:
    ...
    if name == "module_level_aclient":
        ...
        provider_id: Final = cast(Any, "derouter_module_level_client")
