...
if TYPE_CHECKING:
    from derouter import CustomStreamWrapper, DeRouterLoggingObj, ModelResponse
...
class ResponsesToCompletionBridgeHandler:
    def __init__(self):
        from .transformation import DeRouterResponsesTransformationHandler

        super().__init__()
        self.transformation_handler = DeRouterResponsesTransformationHandler()

    @staticmethod
    def _resolve_stream_flag(optional_params: dict, derouter_params: dict) -> bool:
        stream = optional_params.get("stream")
        if stream is None:
            stream = derouter_params.get("stream", False)
        return bool(stream)
    ...
    def validate_input_kwargs(self, kwargs: dict) -> ResponsesToCompletionBridgeHandlerInputKwargs:
        ...
        derouter_params: Final = typed_kwargs.get("derouter_params")
    ...
    def completion(
        ...
        derouter_params: Final = validated_kwargs["derouter_params"]
        ...
        # `sanitized_derouter_params` already carries both, so passing them again
        ...
        stream: Final = self._resolve_stream_flag(optional_params, derouter_params)
