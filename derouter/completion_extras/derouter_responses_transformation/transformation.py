...
from derouter.llms.base_llm.base_model_iterator import BaseModelResponseIterator
from derouter.llms.base_llm.bridges.completion_transformation import (
    CompletionTransformationBridge,
)
from derouter.responses.sse_output_recovery import (
    parse_sse_json_chunk,
    record_output_item_chunk,
    record_output_text_chunk,
)
from derouter.responses.utils import ResponsesAPIRequestUtils, normalize_responses_api_stream_options
...
from derouter.types.utils import GenericStreamingChunk, ModelResponseStream
...
if TYPE_CHECKING:
    ...
    from derouter.types.utils import Choices
...
def _tool_call_dict_from_output_item(item: Mapping[str, Any], index: int) -> _ChatToolCallDict:
    ...
    tool_call_dict: Final = _ChatToolCallDict(
        id=DeRouterCompletionResponsesConfig._tool_call_id_from_responses_item(item.get("id"), item.get("call_id")),
        type="function",
...
class DeRouterResponsesTransformationHandler(CompletionTransformationBridge):
    """
    Handler for transforming /chat/completions api requests to derouter.responses requests
    """
            ...
            for content_item in content_list:
                if isinstance(content_item, dict):
                    ...
                    if content_type == "output_text":
                        ...
                        annotations = DeRouterResponsesTransformationHandler._convert_annotations_to_chat_format(
    ...
    def _build_sanitized_derouter_params(self, derouter_params: dict) -> dict[str, object]:
        """Build sanitized derouter_params with merged metadata."""
        ...
        sanitized: Final[dict[str, object]] = {
            key: value for key, value in derouter_params.items() if key not in responses_optional_param_keys
        }
        legacy_metadata: Final = derouter_params.get("metadata")
        existing_derouter_metadata: Final = derouter_params.get("derouter_metadata")
        merged_derouter_metadata: Final[dict[str, object]] = {}
        if isinstance(legacy_metadata, dict):
            merged_derouter_metadata.update(legacy_metadata)
        if isinstance(existing_derouter_metadata, dict):
            merged_derouter_metadata.update(existing_derouter_metadata)
        if merged_derouter_metadata:
            sanitized["derouter_metadata"] = merged_derouter_metadata
        else:
            sanitized.pop("derouter_metadata", None)
        return sanitized
    ...
    def transform_request(
        ...
        stream: Final = optional_params.get("stream") or derouter_params.get("stream", False)
        ...
        setattr(derouter_logging_obj, "call_type", CallTypes.responses.value)

        sanitized_derouter_params: Final = self._build_sanitized_derouter_params(derouter_params)

        request_data: Final = {
            "model": api_model,
            "input": input_items,
            "derouter_logging_obj": derouter_logging_obj,
            **sanitized_derouter_params,
            "client": client,
        }
    ...
    def _convert_response_output_to_choices(
        ...
        for item in output_items:
            if isinstance(item, ResponseReasoningItem):
                ...
                tool_call_dict = (
                    DeRouterCompletionResponsesConfig.convert_apply_patch_tool_call_to_chat_completion_tool_call(
                        tool_call_item=item,
                        index=tool_call_index,
                    )
                )
                ...
                # openai SDK class above: typed ResponseCustomToolCall and derouter's own
    ...
    @classmethod
    def _recover_output_items_from_logging(cls, logging_obj: "DeRouterLoggingObj") -> list[dict[str, object]]:
    ...
    def _convert_content_to_responses_format(
        ...
        from derouter.types.llms.openai import ChatCompletionImageObject
    ...
    def _map_reasoning_effort(self, reasoning_effort: str | Reasoning) -> Reasoning | None:
        ...
        # Priority: derouter.reasoning_auto_summary flag > DEROUTER_REASONING_AUTO_SUMMARY env var
        auto_summary_enabled: Final = (
            derouter.reasoning_auto_summary or os.getenv("DEROUTER_REASONING_AUTO_SUMMARY", "false").lower() == "true"
        )
...
class OpenAiResponsesToChatCompletionStreamIterator(BaseModelResponseIterator):
    ...
    def translate_responses_chunk_to_openai_stream(
        ...
        """
        from derouter.types.llms.openai import ChatCompletionToolCallFunctionChunk
