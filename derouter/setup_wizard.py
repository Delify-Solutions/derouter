...
# flake8: noqa: T201
"""
DeRouter Interactive Setup Wizard
...
from derouter.utils import check_valid_key
...
class SetupWizard:
    ...
    def _wizard() -> None:
        ...
        config_path: Final = Path(os.getcwd()) / "derouter_config.yaml"
        try:
    ...
    def _print_welcome() -> None:
        try:
            version = importlib.metadata.version("derouter")
        except Exception:
            version = "unknown"
        print()
        print(orange(DEROUTER_ASCII.rstrip("\n")))
        print(f"  {orange('Welcome')} to {bold('DeRouter')} {grey('v' + version)}")
        print()
        print(_divider())
        print()
    ...
    def _collect_keys(providers: list[dict]) -> dict[str, str]:
        ...
        print(grey("  Tip: add derouter_config.yaml to .gitignore to avoid committing secrets."))
        print()
        ...
        for p in providers:
            ...
            if p.get("needs_api_base"):
                ...
                if api_base:
                    env_vars[f"_DEROUTER_AZURE_AI_API_BASE_{p['id'].upper()}"] = api_base
                deployment = _styled_input(f"  {blue('❯')} Azure deployment name {grey('(e.g. my-gpt4o)')}: ")
                if deployment:
                    env_vars[f"_DEROUTER_AZURE_DEPLOYMENT_{p['id'].upper()}"] = deployment
    ...
    def _validate_and_report(provider: dict, api_key: str) -> str:
        """
        Validate credentials using derouter.utils.check_valid_key and print result.
    ...
    def _build_config(
        ...
        for p in providers:
            ...
            if p["id"] == "azure":
                deployment = env_copy.pop(f"_DEROUTER_AZURE_DEPLOYMENT_{p['id'].upper()}", "")
                if not deployment:
            ...
            for model in models:
                ...
                if p.get("api_base"):
                    ...
                    azure_base_key = f"_DEROUTER_AZURE_AI_API_BASE_{p['id'].upper()}"
        ...
        real_vars: Final = {k: v for k, v in env_copy.items() if not k.startswith("_DEROUTER_")}
        if real_vars:
    ...
    def _print_success(config_path: Path, port: int, master_key: str) -> None:
        ...
        print()
        print(f"    {grey('$')} derouter --config {config_path} --port {port}")
        print()
    ...
    def _offer_start(config_path: Path, port: int, master_key: str) -> None:
        ...
        if start not in ("", "y", "yes"):
            print()
            print(f"  Run {bold(f'derouter --config {config_path}')} whenever you're ready.")
            print()
        ...
        derouter_bin: Final = os.path.join(scripts_dir or "", "derouter")
        try:
            os.execlp(
                derouter_bin,
                derouter_bin,
            ...
            print(f"  Run manually:  derouter --config {config_path} --port {port}\n")
...
def run_setup_wizard() -> None:
    """Run the interactive setup wizard. Called by `derouter --setup`."""
    SetupWizard.run()
