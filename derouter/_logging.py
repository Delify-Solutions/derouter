...
from derouter.constants import (
    ...
    DEROUTER_TRUNCATION_STDOUT_SAFEGUARD_NOTE,
    MAX_STRING_LENGTH_STDOUT_LOG,
)
from derouter.derouter_core_utils.env_utils import get_env_int
...
from derouter.derouter_core_utils.secret_redaction import (
...
def _sanitize_correlation_id(value: str) -> str:
    ...
    derouter_session_id, x-derouter-trace-id) is stamped into log lines.
...
_ENABLE_SECRET_REDACTION: Final = os.getenv("DEROUTER_DISABLE_REDACT_SECRETS", "").lower() != "true"
...
def redact_secrets(value: str) -> str:
    ...
    derouter_core_utils/redact_messages.py, which redacts LLM prompt/response
...
def _stdout_truncation_marker(skipped_chars: int) -> str:
    return (
        ...
        f"{DEROUTER_TRUNCATION_STDOUT_SAFEGUARD_NOTE}) ..."
    )
...
class LevelRoutingStreamHandler(logging.StreamHandler):
    ...
    Invalid-key warnings route to stdout so DEROUTER_LOG=ERROR can suppress them.
    """
...
def _parse_json_logs_env(value: str | None) -> bool:
    ...
    Matches the reader in derouter-proxy-extras/_logging.py. The previous
...
log_level: Final = os.getenv("DEROUTER_LOG", "DEBUG")
...
verbose_proxy_logger = logging.getLogger("DeRouter Proxy")
...
verbose_router_logger = logging.getLogger("DeRouter Router")
verbose_logger = logging.getLogger("DeRouter")
...
def _redact_third_party_loggers() -> None:
    """Extend secret redaction to records derouter does not emit directly.

    derouter's own loggers are covered by the filter on their shared handler, but a
    derouter value can also reach a log record through a dependency that logs on its
    own logger. Those records never pass through a derouter handler.
...
def _get_loggers_to_initialize():
    ...
    all_callbacks: Final = set(derouter.success_callback + derouter.failure_callback)
...
def _get_uvicorn_json_log_config():
    ...
    """
    json_formatter_class: Final = "derouter._logging.JsonFormatter"
