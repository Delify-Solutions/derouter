...
# Load .env before any other derouter imports so env vars (e.g. DEROUTER_UI_SESSION_DURATION) are available
...
def _dev_env_hot_reload_enabled() -> bool:
    ...
    return os.getenv("DEROUTER_DEV_ENV_HOT_RELOAD") == "True"


if os.getenv("DEROUTER_MODE", "DEV") == "DEV":
...
from derouter.types.integrations.datadog import DatadogInitParams
from derouter.types.integrations.newrelic import NewRelicInitParams
from derouter.derouter_core_utils.core_helpers import drop_params_env_flag
...
derouter_mode = os.getenv("DEROUTER_MODE", "DEV")  # "PRODUCTION", "DEV"
...
# major release; opt in early with `derouter.expose_router_debug_in_errors
# = False`.
...
email: Optional[str] = (
    None  # Not used anymore, will be removed in next MAJOR release - https://github.com/Delify-Solutions/derouter/discussions/648
)
token: Optional[str] = (
    None  # Not used anymore, will be removed in next MAJOR release - https://github.com/Delify-Solutions/derouter/discussions/648
)
...
modify_params = bool(os.getenv("DEROUTER_MODIFY_PARAMS", False))
use_chat_completions_url_for_anthropic_messages: bool = bool(
    os.getenv("DEROUTER_USE_CHAT_COMPLETIONS_URL_FOR_ANTHROPIC_MESSAGES", False)
...
# DeRouter injects into non-streaming /v1/messages responses, bringing the
...
# backward compatibility for clients that read the DeRouter-shaped
# `usage.total_tokens` today. Planned to flip to True in a future major
# release; opt in early via Python:
#   `derouter.strip_anthropic_total_tokens = True`
# Or via `derouter_settings.strip_anthropic_total_tokens: true` in
# config.yaml.
...
route_all_chat_openai_to_responses: bool = (
    os.getenv("DEROUTER_ROUTE_ALL_CHAT_OPENAI_TO_RESPONSES", "false").lower() == "true"
...
gemini_live_defer_setup: bool = os.getenv("DEROUTER_GEMINI_LIVE_DEFER_SETUP", "false").lower() == "true"
use_legacy_interactions_schema: bool = (
    os.getenv("DEROUTER_USE_LEGACY_INTERACTIONS_SCHEMA", "false").lower() == "true"
...
use_derouter_proxy: bool = False  # when True, requests will be sent to the specified derouter proxy endpoint
...
enable_anthropic_prompt_caching: bool = os.getenv("DEROUTER_ENABLE_ANTHROPIC_PROMPT_CACHING", "false").lower() == "true"
_anthropic_prompt_caching_ttl_env: Optional[str] = os.getenv("DEROUTER_ANTHROPIC_PROMPT_CACHING_TTL")
...
# Proxy Authentication - auto-obtain/refresh OAuth2/JWT tokens for DeRouter Proxy
...
caching: bool = False  # Not used anymore, will be removed in next MAJOR release - https://github.com/Delify-Solutions/derouter/discussions/648
caching_with_models: bool = False  # # Not used anymore, will be removed in next MAJOR release - https://github.com/Delify-Solutions/derouter/discussions/648
cache: Optional["Cache"] = None  # cache object <- use this - https://router.delify.vn/docs/caching
...
default_soft_budget: float = DEFAULT_SOFT_BUDGET  # by default all derouter proxy keys have a soft budget of 50.0
...
model_fallbacks: Optional[List] = None  # Deprecated for 'derouter.fallbacks'
model_cost_map_url: str = os.getenv(
    "DEROUTER_MODEL_COST_MAP_URL",
    "https://raw.githubusercontent.com/Delify-Solutions/derouter/main/model_prices_and_context_window.json",
)
blog_posts_url: str = os.getenv(
    "DEROUTER_BLOG_POSTS_URL",
    "https://router.delify.vn/blog/rss.xml",
)
anthropic_beta_headers_url: str = os.getenv(
    "DEROUTER_ANTHROPIC_BETA_HEADERS_URL",
    "https://raw.githubusercontent.com/Delify-Solutions/derouter/main/derouter/anthropic_beta_headers_config.json",
)
autorouter_presets_url: str = os.getenv(
    "DEROUTER_AUTOROUTER_PRESETS_URL",
    "https://raw.githubusercontent.com/Delify-Solutions/derouter/main/derouter/proxy/public_endpoints/autorouter_presets.json",
)
...
upperbound_key_generate_params: Optional[DeRouter_UpperboundKeyGenerateParams] = None
...
# DeRouter_EndUserTable / DeRouter_UserTable. Values that do not resolve to a
...
# `derouter_proxy_failed_requests_metric`. Off by default to preserve the
...
# are ready to split 429s by source (vendor vs. derouter) and dimension
...
force_ipv4: bool = False  # when True, derouter will force ipv4 for all LLM requests. Some users have seen httpx ConnectionError when using ipv6.
...
def _populate_provider_model_sets(model_cost_map: Dict) -> None:
    for key, value in model_cost_map.items():
        if value.get("derouter_provider") == "openai" and not is_openai_finetune_model(key):
        ...
        elif value.get("derouter_provider") == "datarobot":
        ...
        elif value.get("derouter_provider") == "vertex_ai-language-models":
            vertex_language_models.add(key)
        elif value.get("derouter_provider") == "vertex_ai-vision-models":
        ...
        elif value.get("derouter_provider") == "vertex_ai-embedding-models":
            vertex_embedding_models.add(key)
        elif value.get("derouter_provider") == "vertex_ai-anthropic_models":
            key = key.replace("vertex_ai/", "")
            vertex_anthropic_models.add(key)
        elif value.get("derouter_provider") == "vertex_ai-llama_models":
            key = key.replace("vertex_ai/", "")
            vertex_llama3_models.add(key)
        elif value.get("derouter_provider") == "vertex_ai-deepseek_models":
            key = key.replace("vertex_ai/", "")
            vertex_deepseek_models.add(key)
        elif value.get("derouter_provider") == "vertex_ai-mistral_models":
            key = key.replace("vertex_ai/", "")
            vertex_mistral_models.add(key)
        elif value.get("derouter_provider") == "vertex_ai-ai21_models":
            key = key.replace("vertex_ai/", "")
            vertex_ai_ai21_models.add(key)
        elif value.get("derouter_provider") == "vertex_ai-image-models":
            key = key.replace("vertex_ai/", "")
            vertex_ai_image_models.add(key)
        elif value.get("derouter_provider") == "vertex_ai-video-models":
            key = key.replace("vertex_ai/", "")
            vertex_ai_video_models.add(key)
        elif value.get("derouter_provider") == "vertex_ai-openai_models":
            key = key.replace("vertex_ai/", "")
            vertex_openai_models.add(key)
        elif value.get("derouter_provider") == "vertex_ai-minimax_models":
            key = key.replace("vertex_ai/", "")
            vertex_minimax_models.add(key)
        elif value.get("derouter_provider") == "vertex_ai-moonshot_models":
            key = key.replace("vertex_ai/", "")
            vertex_moonshot_models.add(key)
        elif value.get("derouter_provider") == "vertex_ai-zai_models":
            key = key.replace("vertex_ai/", "")
        ...
        elif value.get("derouter_provider") == "bedrock" and value.get("mode") == "guardrail":
            pass
        elif value.get("derouter_provider") == "bedrock" and not is_bedrock_pricing_only_model(key):
            bedrock_models.add(key)
        elif value.get("derouter_provider") == "bedrock_converse":
            bedrock_converse_models.add(key)
        elif value.get("derouter_provider") == "deepinfra":
            deepinfra_models.add(key)
        elif value.get("derouter_provider") == "perplexity":
            perplexity_models.add(key)
        elif value.get("derouter_provider") == "watsonx":
            watsonx_models.add(key)
        elif value.get("derouter_provider") == "gemini":
            gemini_models.add(key)
        elif value.get("derouter_provider") == "fireworks_ai":
            # ignore the 'up-to', '-to-' model names -> not real models. just for cost tracking based on model params.
            if "-to-" not in key and "fireworks-ai-default" not in key:
                fireworks_ai_models.add(key)
        elif value.get("derouter_provider") == "fireworks_ai-embedding-models":
            # ignore the 'up-to', '-to-' model names -> not real models. just for cost tracking based on model params.
            if "-to-" not in key:
                fireworks_ai_embedding_models.add(key)
        elif value.get("derouter_provider") == "text-completion-codestral":
            text_completion_codestral_models.add(key)
        elif value.get("derouter_provider") == "text-completion-inception":
            text_completion_inception_models.add(key)
        elif value.get("derouter_provider") == "xai":
            xai_models.add(key)
        elif value.get("derouter_provider") == "zai":
            zai_models.add(key)
        elif value.get("derouter_provider") == "fal_ai":
            fal_ai_models.add(key)
        elif value.get("derouter_provider") == "deepseek":
            deepseek_models.add(key)
        elif value.get("derouter_provider") == "tencent":
            tencent_models.add(key)
        elif value.get("derouter_provider") == "runwayml":
            runwayml_models.add(key)
        elif value.get("derouter_provider") == "meta_llama":
            llama_models.add(key)
        elif value.get("derouter_provider") == "nscale":
            nscale_models.add(key)
        elif value.get("derouter_provider") == "azure_ai":
            azure_ai_models.add(key)
        elif value.get("derouter_provider") == "voyage":
            voyage_models.add(key)
        elif value.get("derouter_provider") == "infinity":
            infinity_models.add(key)
        elif value.get("derouter_provider") == "databricks":
            databricks_models.add(key)
        elif value.get("derouter_provider") == "cloudflare":
            cloudflare_models.add(key)
        elif value.get("derouter_provider") == "codestral":
            codestral_models.add(key)
        elif value.get("derouter_provider") == "friendliai":
            friendliai_models.add(key)
        elif value.get("derouter_provider") == "palm":
            palm_models.add(key)
        elif value.get("derouter_provider") == "groq":
            groq_models.add(key)
        elif value.get("derouter_provider") == "azure":
            azure_models.add(key)
        elif value.get("derouter_provider") == "azure_anthropic":
            azure_anthropic_models.add(key)
        elif value.get("derouter_provider") == "anyscale":
            anyscale_models.add(key)
        elif value.get("derouter_provider") == "cerebras":
            cerebras_models.add(key)
        elif value.get("derouter_provider") == "galadriel":
            galadriel_models.add(key)
        elif value.get("derouter_provider") == "nvidia_nim":
            nvidia_nim_models.add(key)
        elif value.get("derouter_provider") == "nvidia_riva":
            nvidia_riva_models.add(key)
        elif value.get("derouter_provider") == "soniox":
            soniox_models.add(key)
        elif value.get("derouter_provider") == "sambanova":
            sambanova_models.add(key)
        elif value.get("derouter_provider") == "sambanova-embedding-models":
            sambanova_embedding_models.add(key)
        elif value.get("derouter_provider") == "novita":
            novita_models.add(key)
        elif value.get("derouter_provider") == "nebius-chat-models":
            nebius_models.add(key)
        elif value.get("derouter_provider") == "nebius-embedding-models":
            nebius_embedding_models.add(key)
        elif value.get("derouter_provider") == "aiml":
            aiml_models.add(key)
        elif value.get("derouter_provider") == "assemblyai":
            assemblyai_models.add(key)
        elif value.get("derouter_provider") == "jina_ai":
            jina_ai_models.add(key)
        elif value.get("derouter_provider") == "snowflake":
            snowflake_models.add(key)
        elif value.get("derouter_provider") == "gradient_ai":
            gradient_ai_models.add(key)
        elif value.get("derouter_provider") == "featherless_ai":
            featherless_ai_models.add(key)
        elif value.get("derouter_provider") == "deepgram":
            deepgram_models.add(key)
        elif value.get("derouter_provider") == "elevenlabs":
            elevenlabs_models.add(key)
        elif value.get("derouter_provider") == "heroku":
            heroku_models.add(key)
        elif value.get("derouter_provider") == "dashscope":
            dashscope_models.add(key)
        elif value.get("derouter_provider") == "qwencloud":
            qwencloud_models.add(key)
        elif value.get("derouter_provider") == "qwen_ai_platform":
            qwen_ai_platform_models.add(key)
        elif value.get("derouter_provider") == "modelscope":
            modelscope_models.add(key)
        elif value.get("derouter_provider") == "moonshot":
            moonshot_models.add(key)
        elif value.get("derouter_provider") == "publicai":
            publicai_models.add(key)
        elif value.get("derouter_provider") == "darkbloom":
            darkbloom_models.add(key)
        elif value.get("derouter_provider") == "v0":
            v0_models.add(key)
        elif value.get("derouter_provider") == "morph":
            morph_models.add(key)
        elif value.get("derouter_provider") == "lambda_ai":
            lambda_ai_models.add(key)
        elif value.get("derouter_provider") == "inception":
            inception_models.add(key)
        elif value.get("derouter_provider") == "hyperbolic":
            hyperbolic_models.add(key)
        elif value.get("derouter_provider") == "black_forest_labs":
            black_forest_labs_models.add(key)
        elif value.get("derouter_provider") == "recraft":
            recraft_models.add(key)
        elif value.get("derouter_provider") == "cometapi":
            cometapi_models.add(key)
        elif value.get("derouter_provider") == "oci":
            oci_models.add(key)
        elif value.get("derouter_provider") == "volcengine":
            volcengine_models.add(key)
        elif value.get("derouter_provider") == "wandb":
            wandb_models.add(key)
        elif value.get("derouter_provider") == "ovhcloud":
            ovhcloud_models.add(key)
        elif value.get("derouter_provider") == "ovhcloud-embedding-models":
            ovhcloud_embedding_models.add(key)
        elif value.get("derouter_provider") == "lemonade":
            lemonade_models.add(key)
        elif value.get("derouter_provider") == "docker_model_runner":
            docker_model_runner_models.add(key)
        elif value.get("derouter_provider") == "amazon_nova":
            amazon_nova_models.add(key)
        elif value.get("derouter_provider") == "stability":
            stability_models.add(key)
        elif value.get("derouter_provider") == "github_copilot":
            github_copilot_models.add(key)
        elif value.get("derouter_provider") == "chatgpt":
            chatgpt_models.add(key)
        elif value.get("derouter_provider") == "minimax":
            minimax_models.add(key)
        elif value.get("derouter_provider") == "aws_polly":
            aws_polly_models.add(key)
        elif value.get("derouter_provider") == "gigachat":
            gigachat_models.add(key)
        elif value.get("derouter_provider") == "llamagate":
            llamagate_models.add(key)
        elif value.get("derouter_provider") == "reducto":
            reducto_models.add(key)
        elif value.get("derouter_provider") == "bedrock_mantle":
            bedrock_mantle_models.add(key)


def add_known_models(model_cost_map: Optional[Dict] = None):
    """Fold `model_cost_map` (defaults to `derouter.model_cost`) into the per-provider model sets,
...
from derouter.types.secret_managers.main import KeyManagementSettings
...
# PublicAI now uses JSON-based configuration (see derouter/llms/openai_like/providers.json)
...
from .llms.custom_httpx.async_client_cleanup import close_derouter_async_clients
...
# Interactions API is available as derouter.interactions module
# Usage: derouter.interactions.create(), derouter.interactions.get(), etc.
...
import derouter.anthropic_interface as anthropic
...
### CLI UTILITIES ###
from derouter.derouter_core_utils.cli_token_utils import get_derouter_gateway_api_key
...
if TYPE_CHECKING:
    from derouter.types.utils import ModelInfo as _ModelInfoType
    ...
    from derouter.llms.openai.completion.transformation import (
    ...
    from .llms.derouter_proxy.responses.transformation import (
        DeRouterProxyResponsesAPIConfig as DeRouterProxyResponsesAPIConfig,
    )
    ...
    from .llms.derouter_proxy.chat.transformation import (
        DeRouterProxyChatConfig as _DeRouterProxyChatConfig,
    )
    ...
    DeRouterProxyChatConfig: Type[_DeRouterProxyChatConfig]
    ...
    from derouter.caching.llm_caching_handler import LLMClientCache
    from derouter.types.llms.bedrock import COHERE_EMBEDDING_INPUT_TYPES
    ...
    from derouter.types.guardrails import GuardrailItem
    ...
    from derouter.caching.caching import InMemoryCache

    bedrock_tool_name_mappings: InMemoryCache

    # Azure exception class (lazy-loaded)
    from derouter.llms.azure.common_utils import AzureOpenAIError
    ...
    from derouter.types.integrations.datadog_llm_obs import DatadogLLMObsInitParams

    # Logging callback manager class and instance (lazy-loaded)
    from derouter.derouter_core_utils.logging_callback_manager import (
...
# When DEROUTER_DISABLE_LAZY_LOADING is set, lazy-loaded attributes are loaded at import time
# For now, this only affects encoding (tiktoken) as it was the only reported issue
# See: https://github.com/Delify-Solutions/derouter/issues/18659
# This ensures encoding is initialized before VCR starts recording HTTP requests
if os.getenv("DEROUTER_DISABLE_LAZY_LOADING", "").lower() in ("1", "true", "yes", "on"):
...
def __getattr__(name: str) -> Any:
    ...
    if name == "encoding":
        from ._lazy_imports import get_derouter_globals

        _globals = get_derouter_globals()
    ...
    if name == "_service_logger":
        ...
        if "_service_logger" not in _globals:
            ...
            import derouter._service_logger

            _globals["_service_logger"] = derouter._service_logger
    ...
    if name in [
        ...
        from derouter.evals.main import (
            acreate_eval,
            alist_evals,
            aget_eval,
            aupdate_eval,
...
# ALL_DEROUTER_RESPONSE_TYPES is lazy-loaded via __getattr__ to avoid loading utils at import time
