...
import derouter
import derouter.derouter_core_utils.exception_mapping_utils
from derouter import get_secret_str
from derouter._logging import verbose_router_logger
from derouter._uuid import uuid
from derouter.caching.caching import (
    DualCache,
    InMemoryCache,
    RedisCache,
    RedisClusterCache,
...
from derouter.derouter_core_utils.asyncify import asyncify, run_async_function
...
from derouter.derouter_core_utils.coroutine_checker import coroutine_checker
from derouter.derouter_core_utils.credential_accessor import CredentialAccessor
from derouter.derouter_core_utils.dd_tracing import tracer
...
from derouter.derouter_core_utils.ptu_pricing import (
...
from derouter.derouter_core_utils.request_timeout_resolver import (
    get_configured_request_timeout,
)
from derouter.derouter_core_utils.secret_redaction import redact_string
...
from derouter.router_strategy.budget_limiter import RouterBudgetLimiting
from derouter.router_strategy.least_busy import LeastBusyLoggingHandler
from derouter.router_strategy.lowest_cost import LowestCostLoggingHandler
from derouter.router_strategy.lowest_latency import LowestLatencyLoggingHandler
from derouter.router_strategy.lowest_tpm_rpm import LowestTPMLoggingHandler
from derouter.router_strategy.lowest_tpm_rpm_v2 import LowestTPMLoggingHandler_v2
from derouter.router_strategy.simple_shuffle import simple_shuffle
from derouter.router_strategy.tag_based_routing import (
...
from derouter.router_utils.batch_utils import (
    _get_router_metadata_variable_name,
    replace_model_in_jsonl,
    should_replace_model_in_jsonl,
)
from derouter.router_utils.client_initalization_utils import InitalizeCachedClient
from derouter.router_utils.clientside_credential_handler import (
    get_dynamic_derouter_params,
    is_clientside_credential,
)
from derouter.router_utils.common_utils import (
...
from derouter.router_utils.cooldown_cache import CooldownCache
...
from derouter.router_utils.fallback_event_handlers import (
...
from derouter.router_utils.handle_error import (
    async_raise_no_deployment_exception,
    send_llm_exception_alert,
)
from derouter.router_utils.health_state_cache import DeploymentHealthCache
from derouter.router_utils.pre_call_checks.deployment_affinity_check import (
    DeploymentAffinityCheck,
    warn_on_unknown_model_group_affinity_flags,
)
from derouter.router_utils.pre_call_checks.io_token_rate_limit_check import (
...
from derouter.router_utils.pre_call_checks.model_rate_limit_check import (
    ModelRateLimitingCheck,
)
from derouter.router_utils.pre_call_checks.prompt_caching_deployment_check import (
    PromptCachingDeploymentCheck,
)
from derouter.router_utils.reasoning_effort_capability import (
...
from derouter.types.router import (
    ...
    VALID_DEROUTER_ENVIRONMENTS,
    AlertingConfig,
    AllowedFailsPolicy,
    AssistantsTypedDict,
    AutoRouterCapabilityLimit,
    ConsumedRequestTagsStamp,
    CredentialDeRouterParams,
...
if TYPE_CHECKING:
    ...
    from derouter.exceptions import MidStreamFallbackError
    from derouter.responses.streaming_iterator import (
        BaseResponsesAPIStreamingIterator,
    )
    from derouter.router_strategy.adaptive_router.adaptive_router import (
        AdaptiveRouter,
    )
    from derouter.router_strategy.auto_router.auto_router import (
        AutoRouter,
        PreRoutingHookResponse,
    )
    from derouter.router_strategy.complexity_router.complexity_router import (
        ComplexityRouter,
    )
    from derouter.router_strategy.quality_router.quality_router import (
        QualityRouter,
    )
    from derouter.types.llms.anthropic_messages.anthropic_response import (
        AnthropicMessagesResponse,
    )
    from derouter.types.llms.base import BaseDeRouterOpenAIResponseObject
    from derouter.types.llms.openai import (
        ResponseAPIUsage,
        ResponseInputParam,
...
def model_info_is_active_for_environment(model_info: Mapping[str, object] | None) -> bool:
    ...
    `supported_environments` loads only on pods whose DEROUTER_ENVIRONMENT is in that list.
    ...
    if not isinstance(supported_environments, (list, tuple)):
        raise ValueError(
            f"supported_environments must be a list of {VALID_DEROUTER_ENVIRONMENTS}. "
            f"but set as: {supported_environments} for model_info: {model_info}"
        )
    derouter_environment: Final = get_secret_str(secret_name="DEROUTER_ENVIRONMENT")
    if derouter_environment is None:
        raise ValueError("Set 'supported_environments' for model but not 'DEROUTER_ENVIRONMENT' set in .env")

    if derouter_environment not in VALID_DEROUTER_ENVIRONMENTS:
        raise ValueError(
            f"DEROUTER_ENVIRONMENT must be one of {VALID_DEROUTER_ENVIRONMENTS}. but set as: {derouter_environment}"
        )

    for _env in supported_environments:
        if _env not in VALID_DEROUTER_ENVIRONMENTS:
            raise ValueError(
                f"supported_environments must be one of {VALID_DEROUTER_ENVIRONMENTS}. but set as: {_env} "
                f"for model_info: {model_info}"
            )

    if derouter_environment in supported_environments:
        return True
    return False
...
def _anthropic_stream_should_drop_pre_content_ping(chunk: object, has_generated_content: bool) -> bool:
    ...
    from derouter.llms.anthropic.experimental_pass_through.messages.streaming_iterator import is_anthropic_ping_chunk
...
def _anthropic_stream_error_is_gateway_verdict(chunk: object) -> bool:
    ...
    from derouter.llms.anthropic.experimental_pass_through.messages.agentic_streaming_iterator import (
...
def _anthropic_stream_commits_now(chunk: object, has_generated_content: bool, buffered_chunk_count: int) -> bool:
    ...
    """
    from derouter.llms.anthropic.experimental_pass_through.messages.streaming_iterator import (
...
class FallbackAwareStreamWrapper(CustomStreamWrapper):
    ...
    def adopt_fallback_response_headers(
        ...
        if fallback_hidden_params:
            self._hidden_params = {  # mutable-ok: the rest of derouter writes into _hidden_params
                **fallback_hidden_params,
...
class Router:
    ...
    def __init__(
        ...
        default_derouter_params: dict | None = None,  # default params for Router.chat.completion.create
        ...
        Args:
            ...
            default_derouter_params (dict): Default parameters for Router.chat.completion.create. Defaults to {}.
        ...
        Returns:
            Router: An instance of the derouter.Router class.
        ...
        from derouter._service_logger import ServiceLogging

        self.service_logger_obj: ServiceLogging = ServiceLogging()
        derouter.suppress_debug_info = True  # prevents 'Give Feedback/Get help' message from being emitted on Router - Relevant Issue: https://github.com/Delify-Solutions/derouter/issues/5942
        ...
        self.deployment_names: list = []  # names of models under derouter_params. ex. azure/chatgpt-v-2
        ...
        if cache_responses:
            if derouter.cache is None:
                ...
                derouter.cache = derouter.Cache(type=cache_type, **cache_config)
        ...
        # ``derouter.proxy.auth.auth_checks._is_model_cost_zero``.
        ...
        if model_list is not None:
            ...
            for m in model_list:
                if "model" in m["derouter_params"]:
                    self.deployment_latency_map[m["derouter_params"]["model"]] = 0
        else:
        ...
        if allowed_fails is not None:
            ...
            self.allowed_fails = derouter.allowed_fails
        ...
        elif derouter.num_retries is not None:
            self.num_retries = derouter.num_retries
        else:
            self.num_retries = openai.DEFAULT_MAX_RETRIES

        if max_fallbacks is not None:
            self.max_fallbacks = max_fallbacks
        elif derouter.max_fallbacks is not None:
            self.max_fallbacks = derouter.max_fallbacks
        else:
            self.max_fallbacks = derouter.ROUTER_MAX_FALLBACKS

        self._explicit_timeout = timeout  # None when user did not pass timeout
        self.timeout = timeout or derouter.request_timeout
        ...
        _fallbacks = fallbacks or derouter.fallbacks

        self.validate_fallbacks(fallback_param=_fallbacks)
        ### set fallbacks
        self.fallbacks = _fallbacks

        if default_fallbacks is not None or derouter.default_fallbacks is not None:
            _fallbacks = default_fallbacks or derouter.default_fallbacks
            if self.fallbacks is not None:
                self.fallbacks.append({"*": _fallbacks})
            else:
                self.fallbacks = [{"*": _fallbacks}]

        self.context_window_fallbacks = context_window_fallbacks or derouter.context_window_fallbacks

        _content_policy_fallbacks: Final = content_policy_fallbacks or derouter.content_policy_fallbacks
        ...
        default_derouter_params = default_derouter_params or {}
        self.chat = derouter.Chat(params=default_derouter_params, router_obj=self)

        # default derouter args
        self.default_derouter_params = default_derouter_params
        self.default_derouter_params.setdefault("timeout", timeout)
        self.default_derouter_params.setdefault("max_retries", 0)
        self.default_derouter_params.setdefault("metadata", {}).update({"caching_groups": caching_groups})
        ...
        if self._normalize_strategy(routing_strategy) == "lar1":
            from derouter.router_strategy.lar1_routing import apply_lar1_routing_strategy
        ...
        ## USAGE TRACKING ##
        if isinstance(derouter._async_success_callback, list):
            derouter.logging_callback_manager.add_derouter_async_success_callback(self.deployment_callback_on_success)
        else:
            derouter.logging_callback_manager.add_derouter_async_success_callback(self.deployment_callback_on_success)
        if isinstance(derouter.success_callback, list):
            derouter.logging_callback_manager.add_derouter_success_callback(self.sync_deployment_callback_on_success)
        else:
            derouter.success_callback = [self.sync_deployment_callback_on_success]
        if isinstance(derouter._async_failure_callback, list):
            derouter.logging_callback_manager.add_derouter_async_failure_callback(
                self.async_deployment_callback_on_failure
            )
        else:
            derouter._async_failure_callback = [self.async_deployment_callback_on_failure]
        ## COOLDOWNS ##
        if isinstance(derouter.failure_callback, list):
            derouter.logging_callback_manager.add_derouter_failure_callback(self.deployment_callback_on_failure)
        else:
            derouter.failure_callback = [self.deployment_callback_on_failure]
    ...
    def discard(self):
        ...
        derouter.logging_callback_manager.remove_callback_from_list_by_object(derouter._async_success_callback, self)
        derouter.logging_callback_manager.remove_callback_from_list_by_object(derouter.success_callback, self)
        derouter.logging_callback_manager.remove_callback_from_list_by_object(derouter._async_failure_callback, self)
        derouter.logging_callback_manager.remove_callback_from_list_by_object(derouter.failure_callback, self)
        derouter.logging_callback_manager.remove_callback_from_list_by_object(derouter.input_callback, self)
        derouter.logging_callback_manager.remove_callback_from_list_by_object(derouter.service_callback, self)
        derouter.logging_callback_manager.remove_callback_from_list_by_object(derouter.callbacks, self)
        ...
        if self.optional_callbacks is not None:
            for callback in self.optional_callbacks:
                derouter.logging_callback_manager.remove_callback_from_list_by_object(
                    derouter.callbacks, callback, require_self=False
                )
    ...
    def _validate_routing_strategy(self, routing_strategy: RoutingStrategy | str | None) -> None:
        # See: https://github.com/Delify-Solutions/derouter/issues/11330
    ...
    def _build_strategy_selector(
        ...
        match self._normalize_strategy(strategy):
            case RoutingStrategy.LEAST_BUSY.value:
                ...
                if register_callbacks:
                    if isinstance(derouter.input_callback, list):
                        derouter.logging_callback_manager.add_derouter_input_callback(selector)
                    else:
                        derouter.input_callback = [selector]
        ...
        if selector is not None and register_callbacks and isinstance(derouter.callbacks, list):
            derouter.logging_callback_manager.add_derouter_callback(selector)

        return selector

    def _unregister_router_selectors(self, selectors: Sequence[object]) -> None:
        """
        Drop router-owned strategy selectors from derouter's global callback
        ...
        if not selector_ids:
            return
        if isinstance(derouter.callbacks, list):
            derouter.callbacks = [c for c in derouter.callbacks if id(c) not in selector_ids]
        if isinstance(derouter.input_callback, list):
            derouter.input_callback = [c for c in derouter.input_callback if id(c) not in selector_ids]
    ...
    def _override_selector_pre_call_check(
        ...
        """
        Override selectors are not in `derouter.callbacks`, so the pre-call check that
    ...
    def initialize_assistants_endpoint(self):
        ...
        self.acreate_assistants = self.factory_function(derouter.acreate_assistants)
        self.adelete_assistant = self.factory_function(derouter.adelete_assistant)
        self.aget_assistants = self.factory_function(derouter.aget_assistants)
        self.acreate_thread = self.factory_function(derouter.acreate_thread)
        self.aget_thread = self.factory_function(derouter.aget_thread)
        self.a_add_message = self.factory_function(derouter.a_add_message)
        self.aget_messages = self.factory_function(derouter.aget_messages)
        self.arun_thread = self.factory_function(derouter.arun_thread)

    def _initialize_core_endpoints(self):
        """Helper to initialize core router endpoints."""
        self.amoderation = self.factory_function(derouter.amoderation, call_type="moderation")
        self.aanthropic_messages = self.factory_function(derouter.anthropic_messages, call_type="anthropic_messages")
        self.anthropic_messages = self.factory_function(derouter.anthropic_messages, call_type="anthropic_messages")
        self.agenerate_content = self.factory_function(derouter.agenerate_content, call_type="agenerate_content")
        self.aadapter_generate_content = self.factory_function(
            derouter.aadapter_generate_content, call_type="aadapter_generate_content"
        )
        self.aresponses = self.factory_function(derouter.aresponses, call_type="aresponses")
        self.afile_delete = self.factory_function(derouter.afile_delete, call_type="afile_delete")
        self.afile_content = self.factory_function(derouter.afile_content, call_type="afile_content")
        self.responses = self.factory_function(derouter.responses, call_type="responses")
        self.aget_responses = self.factory_function(derouter.aget_responses, call_type="aget_responses")
        self.acancel_responses = self.factory_function(derouter.acancel_responses, call_type="acancel_responses")
        self.acompact_responses = self.factory_function(derouter.acompact_responses, call_type="acompact_responses")
        self.adelete_responses = self.factory_function(derouter.adelete_responses, call_type="adelete_responses")
        self.alist_input_items = self.factory_function(derouter.alist_input_items, call_type="alist_input_items")
        self._arealtime = self.factory_function(derouter._arealtime, call_type="_arealtime")
        self.acreate_realtime_client_secret = self.factory_function(
            derouter.acreate_realtime_client_secret, call_type="acreate_realtime_client_secret"
        )
        self.arealtime_calls = self.factory_function(derouter.arealtime_calls, call_type="arealtime_calls")
        self.acreate_realtime_transcription_session = self.factory_function(
            derouter.acreate_realtime_transcription_session, call_type="acreate_realtime_transcription_session"
        )
        self._aresponses_websocket = self.factory_function(
            derouter._aresponses_websocket, call_type="_aresponses_websocket"
        )
        self.acreate_fine_tuning_job = self.factory_function(
            derouter.acreate_fine_tuning_job, call_type="acreate_fine_tuning_job"
        )
        self.acancel_fine_tuning_job = self.factory_function(
            derouter.acancel_fine_tuning_job, call_type="acancel_fine_tuning_job"
        )
        self.alist_fine_tuning_jobs = self.factory_function(
            derouter.alist_fine_tuning_jobs, call_type="alist_fine_tuning_jobs"
        )
        self.aretrieve_fine_tuning_job = self.factory_function(
            derouter.aretrieve_fine_tuning_job, call_type="aretrieve_fine_tuning_job"
        )
        self.afile_list = self.factory_function(derouter.afile_list, call_type="alist_files")
        self.aimage_edit = self.factory_function(derouter.aimage_edit, call_type="aimage_edit")
        self.allm_passthrough_route = self.factory_function(
            derouter.allm_passthrough_route, call_type="allm_passthrough_route"
        )
    ...
    def _initialize_vector_store_endpoints(self):
        ...
        from derouter.vector_stores.main import (
            adelete,
            alist,
            aretrieve,
            asearch,
    ...
    def _initialize_vector_store_file_endpoints(self):
        ...
        from derouter.vector_store_files.main import (
            acreate as avector_store_file_create_fn,
        )
        from derouter.vector_store_files.main import (
            adelete as avector_store_file_delete_fn,
        )
        from derouter.vector_store_files.main import alist as avector_store_file_list_fn
        from derouter.vector_store_files.main import (
        ...
        from derouter.vector_store_files.main import list as vector_store_file_list_fn
        from derouter.vector_store_files.main import (
    ...
    def _initialize_google_genai_endpoints(self):
        ...
        from derouter.google_genai import (
            agenerate_content,
    ...
    def _initialize_container_endpoints(self):
        ...
        from derouter.containers import (
            acreate_container,
            adelete_container,
            alist_containers,
            aretrieve_container,
        ...
        from derouter.containers.endpoint_factory import (
    ...
    def _initialize_skills_endpoints(self):
        ...
        self.acreate_skill = self.factory_function(derouter.acreate_skill, call_type="acreate_skill")
        self.alist_skills = self.factory_function(derouter.alist_skills, call_type="alist_skills")
        self.aget_skill = self.factory_function(derouter.aget_skill, call_type="aget_skill")
        self.adelete_skill = self.factory_function(derouter.adelete_skill, call_type="adelete_skill")

    def _initialize_interactions_endpoints(self):
        """Initialize Google Interactions API endpoints."""
        from derouter.interactions import acancel as acancel_interaction
        from derouter.interactions import acreate as acreate_interaction
        from derouter.interactions import adelete as adelete_interaction
        from derouter.interactions import aget as aget_interaction
        from derouter.interactions import cancel as cancel_interaction
        from derouter.interactions import create as create_interaction
        from derouter.interactions import delete as delete_interaction
        from derouter.interactions import get as get_interaction
    ...
    def _initialize_managed_agents_endpoints(self):
        ...
        from derouter.interactions.agents import acreate as acreate_agent
        from derouter.interactions.agents import adelete as adelete_agent
        from derouter.interactions.agents import aget as aget_agent
        from derouter.interactions.agents import alist as alist_agents
        from derouter.interactions.agents import alist_versions as alist_agent_versions
        from derouter.interactions.agents import create as create_agent
        from derouter.interactions.agents import delete as delete_agent
        from derouter.interactions.agents import get as get_agent
        from derouter.interactions.agents import list as list_agents
        from derouter.interactions.agents import list_versions as list_agent_versions
    ...
    def _add_encrypted_content_affinity_check(self, enable_global_affinity: bool) -> None:
        from derouter.router_utils.pre_call_checks.encrypted_content_affinity_check import (
        ...
        if enable_global_affinity or EncryptedContentAffinityCheck.has_model_group_affinity_enabled(
            ...
            if existing_ec_callback is not None:
                ...
                derouter.logging_callback_manager.add_derouter_callback(ec_callback)

            _move_before_deployment_affinity(self.optional_callbacks, ec_callback)
            _move_before_deployment_affinity(derouter.callbacks, ec_callback)
    ...
    def _ensure_deployment_affinity_callback(self) -> None:
        ...
        derouter.logging_callback_manager.add_derouter_callback(affinity_callback)
    ...
    def _remove_optional_callbacks_of_type(self, callback_cls: type[CustomLogger]) -> None:
        ...
        if any(
            ...
            return
        for cb in tuple(derouter.callbacks):
            if type(cb) is callback_cls:
                derouter.logging_callback_manager.remove_callback_from_list_by_object(
                    derouter.callbacks, cb, require_self=False
                )
    ...
    def print_deployment(self, deployment: dict):
        ...
        try:
            ...
            derouter_params: Final[dict] = _deployment_copy["derouter_params"]

            if derouter.redact_user_api_key_info:
                masker: Final = SensitiveDataMasker(visible_prefix=2, visible_suffix=0)
                _deployment_copy["derouter_params"] = masker.mask_dict(derouter_params)
            elif "api_key" in derouter_params:
                derouter_params["api_key"] = derouter_params["api_key"][:2] + "*" * 10
    ...
    ) -> dict[str, object]:  # mutable-ok: derouter's request pipeline consumes a mutable kwargs mapping
    ...
    def _is_classifier_internal_call(kwargs: Mapping[str, object]) -> bool:
        ...
        derouter_metadata: Final = kwargs.get("derouter_metadata")
        return any(
            isinstance(candidate, Mapping)
            and candidate.get(INTERNAL_CALL_ORIGIN_METADATA_KEY) == AUTOROUTER_CLASSIFIER_CALL_ORIGIN
            for candidate in (metadata, derouter_metadata)
        )
    ...
    def _drop_unsupported_classifier_reasoning_effort(
        ...
        if self._deployment_accepts_param(deployment, model, "reasoning_effort"):
            return
        verbose_router_logger.warning(
            "derouter.router.py: dropping classifier reasoning_effort for model=%s because the selected deployment does not support it",
            model,
        )
    ...
    def _completion(self, model: str, messages: list[dict[str, str]], **kwargs) -> ModelResponse | CustomStreamWrapper:
        ...
        try:
            ...
            # Make a local copy of derouter_params to avoid mutating the Router's state
            derouter_params: Final = self._deployment_params_with_request_reasoning_override(
                deployment["derouter_params"], kwargs
            )
            silent_model: Final = derouter_params.pop("silent_model", None)
            ...
            model_name = derouter_params["model"]
            ...
            response: Final = derouter.completion(**input_kwargs)
            verbose_router_logger.info("derouter.completion(model=%s)\x1b[32m 200 OK\x1b[0m", model_name)

            ## CHECK CONTENT FILTER ERROR ##
            if isinstance(response, ModelResponse):
                _should_raise = self._should_raise_content_policy_error(model=model, response=response, kwargs=kwargs)
                if _should_raise:
                    raise derouter.ContentPolicyViolationError(
            ...
            verbose_router_logger.info("derouter.completion(model=%s)\x1b[31m Exception %s\x1b[0m", model_name, e)
    ...
    def _get_silent_experiment_kwargs(self, **kwargs) -> dict:
        ...
        silent_kwargs["metadata"].pop("derouter_parent_otel_span", None)
        ...
        silent_kwargs.pop("derouter_call_id", None)
        silent_kwargs.pop("derouter_logging_obj", None)
    ...
    def _combine_fallback_usage(
        ...
        from derouter.cost_calculator import BaseTokenUsageProcessor
    ...
    async def _acompletion_streaming_iterator(
        ...
        class FallbackStreamWrapper(FallbackAwareStreamWrapper):
            def __init__(self, async_generator: AsyncGenerator):
                ...
                # Preserve hidden params (including derouter_overhead_time_ms) from original response
        ...
        async def stream_with_fallbacks():
            ...
            try:
                ...
                from derouter.main import stream_chunk_builder
    ...
    def _extract_partial_responses_usage(
        ...
        Two sources, in priority order:
          1. The bridge path (DeRouterCompletionStreamingIterator) accumulates
        ...
        if isinstance(source_iterator, DeRouterCompletionStreamingIterator):
    ...
    def _combine_responses_fallback_usage(
        fallback_item: "BaseDeRouterOpenAIResponseObject",
    ...
    async def _aresponses_streaming_iterator(
        ...
        DeRouterCompletionStreamingIterator when the Responses API is
        ...
        class FallbackResponsesStreamWrapper(BaseResponsesAPIStreamingIterator):
            ...
            def __init__(self, async_generator: AsyncGenerator):
                ...
                #
                # The bridge path (DeRouterCompletionStreamingIterator used by
                ...
                # bridge stores the logging object as `derouter_logging_obj`.
                # base class declares non-Optional types for these
                # fields but the bridge path (DeRouterCompletionStreamingIterator)
                ...
                self.logging_obj = getattr(
                    source_iterator,
                    "logging_obj",
                    getattr(source_iterator, "derouter_logging_obj", None),
                )
                self.finished = False
                ...
                self.derouter_metadata = getattr(source_iterator, "derouter_metadata", None)
        ...
        async def stream_with_fallbacks():
            ...
            try:
                ...
                try:
                    ...
                    # under "derouter_metadata" (not the default "metadata") —
                    # see _ageneric_api_call_with_fallbacks. Mirroring that
                    # here ensures model_group, model_group_alias, and trace
                    # ids land in the same key derouter.aresponses reads from.
                    self._update_kwargs_before_fallbacks(
                        model=model_group,
                        kwargs=initial_kwargs,
                        metadata_variable_name="derouter_metadata",
                    )
                    fallback_response = await self.async_function_with_fallbacks_common_utils(
                        e=e,
                        disable_fallbacks=False,
    ...
    async def _acompletion(
        ...
        try:
            ...
            _response: Final = derouter.acompletion(**input_kwargs)

            logging_obj: Final[DeRouterLogging | None] = kwargs.get("derouter_logging_obj", None)
            ...
            async with contextlib.AsyncExitStack() as deployment_slot:
                ...
                verbose_router_logger.info("derouter.acompletion(model=%s)\x1b[32m 200 OK\x1b[0m", model_name)
                # debug how often this deployment picked
            ...
            deployment_request_timeout_param: Final = _timeout_debug_deployment_dict.get("derouter_params", {}).get(
                "request_timeout", None
            )
            deployment_timeout_param = _timeout_debug_deployment_dict.get("derouter_params", {}).get("timeout", None)
            if derouter.expose_router_debug_in_errors:
            ...
            verbose_router_logger.info("derouter.acompletion(model=%s)\x1b[31m Exception %s\x1b[0m", model_name, e)
    ...
    def _update_kwargs_before_fallbacks(
        ...
        honour the precedence request > deployment derouter_params > derouter_settings while an
        absent num_retries still means "the caller did not ask for one".
        """
        kwargs.setdefault("derouter_trace_id", str(uuid.uuid4()))
    ...
    def _set_deployment_num_retries_on_exception(self, exception: Exception, deployment: dict) -> None:
        """
        Set num_retries from deployment derouter_params on the exception.
        ...
        derouter_params: Final = deployment.get("derouter_params", {})
        dep_num_retries: Final = derouter_params.get("num_retries")
    ...
    def _update_kwargs_with_default_derouter_params(
        self, kwargs: dict, metadata_variable_name: str | None = "metadata"
    ) -> None:
        """
        Adds default derouter params to kwargs, if set.

        Handles inserting this as either "metadata" or "derouter_metadata" depending on the metadata_variable_name
        """
        # 1) copy your defaults and pull out metadata
        defaults: Final = self.default_derouter_params.copy()
        ...
        # 3) merge in metadata, this handles inserting this as either "metadata" or "derouter_metadata"
    ...
    def _handle_clientside_credential(
        ...
        derouter_params: Final = deployment["derouter_params"].copy()
        dynamic_derouter_params: Final = get_dynamic_derouter_params(derouter_params=derouter_params, request_kwargs=kwargs)
        ...
        _model_id: Final = self.generate_model_id(model_group=model_group, derouter_params=dynamic_derouter_params)
        ...
        deployment_pydantic_obj: Final = Deployment(
            ...
            derouter_params=DeRouter_Params(**dynamic_derouter_params),
    ...
    def _merge_tools_from_deployment(deployment: dict, kwargs: dict) -> None:
        """
        Merge tools from deployment derouter_params with request kwargs.
        ...
        """
        dep_params_raw: Final = deployment.get("derouter_params", {}) or {}
    ...
    def _update_kwargs_with_deployment(
        ...
        deployment_derouter_model_name = deployment["derouter_params"]["model"]
        deployment_api_base = deployment["derouter_params"].get("api_base")
        ...
        if is_clientside_credential(request_kwargs=kwargs):
            ...
            deployment_derouter_model_name = deployment_pydantic_obj.derouter_params.model
            deployment_api_base = deployment_pydantic_obj.derouter_params.api_base
        ...
        kwargs.setdefault(metadata_variable_name, {}).update(
            {
                "deployment": deployment_derouter_model_name,
                "model_info": model_info,
        ...
        deployment_tags: Final = deployment.get("derouter_params", {}).get("tags")
        if deployment_tags:
        ...
        credential_name: Final = deployment.get("derouter_params", {}).get("derouter_credential_name")
        if credential_name:
        ...
        if function_name == "_ageneric_api_call_with_fallbacks":
            from derouter.passthrough.timeout_utils import (
            ...
            kwargs["timeout"] = resolve_llm_passthrough_timeout(
                kwargs=kwargs,
                derouter_params=deployment["derouter_params"],
                router_timeout=_router_timeout,
            )
        else:
            kwargs["timeout"] = self._get_timeout(kwargs=kwargs, data=deployment["derouter_params"])

        self._update_kwargs_with_default_derouter_params(kwargs=kwargs, metadata_variable_name=metadata_variable_name)
    ...
    def _get_stream_timeout(self, kwargs: dict, data: dict) -> float | int | None:
        ...
        return (
            ...
            or data.get("stream_timeout", None)  # timeout set on derouter_params for this deployment
            or self.stream_timeout  # timeout set on router
            or self.request_timeout  # derouter_settings.request_timeout (per-attempt)
            or self.default_derouter_params.get("stream_timeout", None)
        )
    ...
    def _get_non_stream_timeout(self, kwargs: dict, data: dict) -> float | int | None:
        ...
        timeout: Final = (
            ...
            or data.get("timeout", None)  # timeout set on derouter_params for this deployment
            or data.get("request_timeout", None)  # timeout set on derouter_params for this deployment
            or self.request_timeout  # derouter_settings.request_timeout (per-attempt)
            or self.timeout  # timeout set on router (router_settings.timeout)
            or self.default_derouter_params.get("timeout", None)
        )
        return timeout
    ...
    async def abatch_completion(
        ...
        **kwargs,
        ...
        1. Batch Process 1 request to N models on derouter.Router. Pass messages as List[Dict[str, str]] to use this
        2. Batch Process N requests to M models on derouter.Router. Pass messages as List[List[Dict[str, str]]] to use this
        ...
        ```
            response = await router.abatch_completion(
                ...
                messages=[
                    {"role": "user", "content": "is derouter becoming a better product ?"}
                ],
                max_tokens=15,
            )
        ```
    ...
    async def abatch_completion_one_model_multiple_requests(
        ...
        """
        Async Batch Completion - Batch Process multiple Messages to one model_group on derouter.Router
        ...
        **kwargs,
        ...
        if make_request:
            try:
                ...
                _response._hidden_params["additional_headers"].update({"x-derouter-request-prioritization-used": True})
                return _response
            except Exception as e:
            ...
            try:
                ...
                if isinstance(_response._hidden_params, dict):
                    ...
                    _response._hidden_params["additional_headers"].update(
                        {"x-derouter-request-prioritization-used": True}
                    )
                return _response
            except Exception as e:
                setattr(e, "priority", priority)
    ...
    def _is_prompt_management_model(self, model: str) -> bool:
        ...
        derouter_model: Final = model_list[0]["derouter_params"].get("model", None)
        if derouter_model is None or "/" not in derouter_model:
            return False

        split_derouter_model: Final = derouter_model.split("/")[0]
        return split_derouter_model in derouter._known_custom_logger_compatible_callbacks
    ...
    async def _prompt_management_factory(
        ...
        derouter_logging_object = kwargs.get("derouter_logging_obj", None)
        if derouter_logging_object is None:
            derouter_logging_object, kwargs = function_setup(
                **{
        ...
        derouter_logging_object = cast(DeRouterLogging, derouter_logging_object)
        ...
        data: Final = prompt_management_deployment["derouter_params"].copy()

        derouter_model: Final = data.get("model", None)

        # derouter_agent/ prefix only strips the model name, no prompt_id needed
        is_derouter_agent_model: Final = isinstance(derouter_model, str) and derouter_model.startswith("derouter_agent/")

        prompt_id = kwargs.get("prompt_id") or prompt_management_deployment["derouter_params"].get("prompt_id", None)
        prompt_variables: Final = kwargs.get("prompt_variables") or prompt_management_deployment["derouter_params"].get(
            "prompt_variables", None
        )
        prompt_label: Final = kwargs.get("prompt_label", None) or prompt_management_deployment["derouter_params"].get(
            "prompt_label", None
        )

        if not is_derouter_agent_model and (prompt_id is None or not isinstance(prompt_id, str)):
        ...
        (
            ...
            messages,
            optional_params,
        ) = derouter_logging_object.get_chat_completion_prompt(
        ...
        kwargs["model"] = model
        kwargs["messages"] = messages
        kwargs["derouter_logging_obj"] = derouter_logging_object
        ...
        if _model_list is None or len(_model_list) == 0:  # if direct call to model
            ...
            return await derouter.acompletion(**kwargs)
    ...
    def _image_generation(self, prompt: str, model: str, **kwargs):
        ...
        try:
            ...
            self._update_kwargs_with_deployment(deployment=deployment, kwargs=kwargs)
            data: Final = deployment["derouter_params"].copy()
            ...
            response: Final = derouter.image_generation(
                **{
                    **data,
                    "prompt": prompt,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info("derouter.image_generation(model=%s)\x1b[32m 200 OK\x1b[0m", model_name)
            return response
        except Exception as e:
            verbose_router_logger.info("derouter.image_generation(model=%s)\x1b[31m Exception %s\x1b[0m", model_name, e)
            if model_name is not None:
                self.fail_calls[model_name] += 1
    ...
    async def _aimage_generation(self, prompt: str, model: str, **kwargs):
        ...
        try:
            ...
            self.total_calls[model_name] += 1
            response = derouter.aimage_generation(
                **{
                    **data,
                    "prompt": prompt,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info("derouter.aimage_generation(model=%s)\x1b[32m 200 OK\x1b[0m", model_name)
            return response
        except Exception as e:
            verbose_router_logger.info("derouter.aimage_generation(model=%s)\x1b[31m Exception %s\x1b[0m", model_name, e)
            if model_name is not None:
                self.fail_calls[model_name] += 1
            self._stamp_retry_skip_deployment_id(e, kwargs)
            raise e
    ...
    async def _atranscription(self, file: FileTypes, model: str, **kwargs):
        ...
        try:
            ...
            self.total_calls[model_name] += 1
            response = derouter.atranscription(
                **{
                    **data,
                    "file": file,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info("derouter.atranscription(model=%s)\x1b[32m 200 OK\x1b[0m", model_name)
            return response
        except Exception as e:
            verbose_router_logger.info("derouter.atranscription(model=%s)\x1b[31m Exception %s\x1b[0m", model_name, e)
            if model_name is not None:
                self.fail_calls[model_name] += 1
            self._stamp_retry_skip_deployment_id(e, kwargs)
            raise e
    ...
    async def _aspeech(self, model: str, input: str, voice: str | None = None, **kwargs):
        ...
        try:
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info("derouter.aspeech(model=%s)\x1b[32m 200 OK\x1b[0m", model_name)
            return response
        except Exception as e:
            verbose_router_logger.info("derouter.aspeech(model=%s)\x1b[31m Exception %s\x1b[0m", model_name, e)
            if model_name is not None:
                self.fail_calls[model_name] += 1
            self._stamp_retry_skip_deployment_id(e, kwargs)
            raise e
    ...
    async def _arerank(self, model: str, **kwargs):
        ...
        try:
            ...
            response: Final = await derouter.arerank(
                **{
                    **data,
                    "caching": self.cache_responses,
                    "client": model_client,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info("derouter.arerank(model=%s)\x1b[32m 200 OK\x1b[0m", model_name)
            return response
        except Exception as e:
            verbose_router_logger.info("derouter.arerank(model=%s)\x1b[31m Exception %s\x1b[0m", model_name, e)
            if model_name is not None:
                self.fail_calls[model_name] += 1
            self._stamp_retry_skip_deployment_id(e, kwargs)
            raise e
    ...
    def text_completion(
        ...
        **kwargs,
        ...
        try:
            ...
            data: Final = deployment["derouter_params"].copy()
            for k, v in self.default_derouter_params.items():
                if k not in kwargs:  # prioritize model-specific params > default router params
                    kwargs[k] = v
                elif k == "metadata":
                    kwargs[k].update(v)

            # call via derouter.completion()
            return derouter.text_completion(**{**data, "prompt": prompt, "caching": self.cache_responses, **kwargs})
        except Exception as e:
            raise e
    ...
    async def _atext_completion(self, model: str, prompt: str, **kwargs):
        try:
            ...
            response = derouter.atext_completion(
                **{
                    **data,
                    "prompt": prompt,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info("derouter.atext_completion(model=%s)\x1b[32m 200 OK\x1b[0m", model_name)
            return response
        except Exception as e:
            verbose_router_logger.info("derouter.atext_completion(model=%s)\x1b[31m Exception %s\x1b[0m", model, e)
    ...
    async def _aadapter_completion(self, adapter_id: str, model: str, **kwargs):
        try:
            ...
            response = derouter.aadapter_completion(
                **{
                    **data,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info("derouter.aadapter_completion(model=%s)\x1b[32m 200 OK\x1b[0m", model_name)
            return response
        except Exception as e:
            verbose_router_logger.info("derouter.aadapter_completion(model=%s)\x1b[31m Exception %s\x1b[0m", model, e)
            if model is not None:
                self.fail_calls[model] += 1
            self._stamp_retry_skip_deployment_id(e, kwargs)
            raise e
    ...
    async def _aguardrail_helper(
        ...
        **kwargs,
        ...
        verbose_router_logger.debug(
            "Selected guardrail deployment: %s", selected_guardrail.get("derouter_params", {}).get("guardrail")
        )
    ...
    async def _ageneric_api_call_with_fallbacks(self, model: str, original_function: Callable, **kwargs):
        """
        Helper function to make a generic LLM API call through the router, this allows you to use retries/fallbacks with derouter router
        """
        try:
            kwargs["model"] = model
            kwargs["original_generic_function"] = original_function
            kwargs["original_function"] = self._ageneric_api_call_with_fallbacks_helper
            self._update_kwargs_before_fallbacks(model=model, kwargs=kwargs, metadata_variable_name="derouter_metadata")
            verbose_router_logger.debug(
    ...
    def _add_deployment_model_to_endpoint_for_llm_passthrough_route(
        ...
        if "endpoint" in kwargs and kwargs["endpoint"]:
            ...
            from derouter import get_llm_provider
    ...
    async def _ageneric_api_call_with_fallbacks_helper(self, model: str, original_generic_function: Callable, **kwargs):
        ...
        try:
            ...
            if self._should_raise_anthropic_refusal_error(
                ...
                from derouter.llms.anthropic.experimental_pass_through.messages.utils import (
    ...
    async def _aresponses_with_streaming_fallbacks(
        ...
        # attempt mutates nested dicts in place — notably `derouter_metadata`,
        ...
        kwargs.setdefault("derouter_metadata", {})  # mutable-ok: shared bucket  # rebind-ok: stamp must be readable here

        fallback_kwargs: Final[dict[str, object]] = kwargs.copy()
        if isinstance(fallback_kwargs.get("derouter_metadata"), dict):
            fallback_kwargs["derouter_metadata"] = safe_deep_copy(fallback_kwargs["derouter_metadata"])
    ...
    async def _aanthropic_messages_streaming_iterator(
        ...
        shapes land here:
          - the completion-bridge path (deployments with no native
            ...
            DeRouterMessagesToCompletionTransformationHandler) already
                ...
                async for chunk in source_iterator:
                    ...
                    if retriable_pending_error:
                        ...
                        raise MidStreamFallbackError(
                            ...
                            model=model,
                            llm_provider="anthropic",
                            original_exception=derouter.exceptions.APIError(
    ...
    async def _aanthropic_messages_fallback_attempt(
        ...
        try:
            ...
            fallback_trigger: Final[Exception] = (
                e.original_exception if isinstance(e.original_exception, derouter.ContentPolicyViolationError) else e
            )
    ...
    async def _aanthropic_messages_with_streaming_fallbacks(
        ...
        **kwargs: object,  # kwargs-ok: forwarded verbatim to original_function, shape varies per call site
        ...
        # SAME derouter_metadata/metadata dicts a shallow .copy() would still
    ...
    def _generic_api_call_with_fallbacks(self, model: str, original_function: Callable, **kwargs):
        """
        Make a generic LLM API call through the router, this allows you to use retries/fallbacks with derouter router
        Args:
            model: The model to use
            original_function: The handler function to call (e.g., derouter.completion)
    ...
    def _embedding(self, input: str | list, model: str, **kwargs):
        ...
        try:
            ...
            response: Final = derouter.embedding(
                **{
                    **data,
                    "input": input,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info("derouter.embedding(model=%s)\x1b[32m 200 OK\x1b[0m", model_name)
            return response
        except Exception as e:
            verbose_router_logger.info("derouter.embedding(model=%s)\x1b[31m Exception %s\x1b[0m", model_name, e)
            if model_name is not None:
                self.fail_calls[model_name] += 1
            self._stamp_retry_skip_deployment_id(e, kwargs)
            raise e
    ...
    async def _aembedding(self, input: str | list, model: str, **kwargs):
        ...
        try:
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info("derouter.aembedding(model=%s)\x1b[32m 200 OK\x1b[0m", model_name)
            return response
        except Exception as e:
            verbose_router_logger.info("derouter.aembedding(model=%s)\x1b[31m Exception %s\x1b[0m", model_name, e)
            if model_name is not None:
                self.fail_calls[model_name] += 1
            self._stamp_retry_skip_deployment_id(e, kwargs)
            raise e
    ...
    async def _acreate_file(
        ...
        **kwargs,
        ...
        try:
            from derouter.router_utils.common_utils import add_model_file_id_mappings
            ...
            async def create_file_for_deployment(deployment: dict) -> OpenAIFileObject:
                ...
                if custom_llm_provider == LlmProviders.DEROUTER_PROXY.value:
                ...
                if (
                    ...
                    kwargs_copy.setdefault("derouter_metadata", {})["gcs_bucket_name"] = data["gcs_bucket_name"]
                response = derouter.acreate_file(
                    **{
                        **data,
                        "custom_llm_provider": custom_llm_provider,
                        "caching": self.cache_responses,
                ...
                self.success_calls[model_name] += 1
                verbose_router_logger.info("derouter.acreate_file(model=%s)\x1b[32m 200 OK\x1b[0m", model_name)
            ...
            verbose_router_logger.exception(
                "derouter.acreate_file(model=%s, %s)\x1b[31m Exception %s\x1b[0m", model, kwargs, e
            )
            if model is not None:
                self.fail_calls[model] += 1
            raise e
    ...
    async def avector_store_create(
        ...
        **kwargs,
        ...
        try:
            ...
            if model is None:
                from derouter.vector_stores.main import acreate
            ...
            from derouter.vector_stores import acreate as avector_store_create_sdk
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info("derouter.avector_store_create(model=%s)\x1b[32m 200 OK\x1b[0m", model_name)

            return response
        except Exception as e:
            verbose_router_logger.exception(
                "derouter.avector_store_create(model=%s)\x1b[31m Exception %s\x1b[0m", model, e
            )
            if model is not None:
                self.fail_calls[model] += 1
            raise e
    ...
    async def _acreate_batch(
        ...
        **kwargs,
        ...
        try:
            ...
            response = derouter.acreate_batch(
                **{
                    **data,
                    "custom_llm_provider": custom_llm_provider,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info("derouter.acreate_batch(model=%s)\x1b[32m 200 OK\x1b[0m", model_name)

            return response
        except Exception as e:
            verbose_router_logger.exception(
                "derouter._acreate_batch(model=%s, %s)\x1b[31m Exception %s\x1b[0m", model, kwargs, e
            )
            if model is not None:
                self.fail_calls[model] += 1
            self._stamp_retry_skip_deployment_id(e, kwargs)
        ...
        try:
            ...
            async def try_retrieve_batch(model_name: DeploymentTypedDict):
                try:
                    ...
                    model: Final = model_name["derouter_params"].get("model")
                    data: Final = model_name["derouter_params"].copy()
                    custom_llm_provider = data.get("custom_llm_provider")
                    if model is None:
                        raise Exception(f"Model not found in derouter_params for deployment: {model_name}")
                    ...
                    return await derouter.aretrieve_batch(
                        **{
                            **data,
                            "custom_llm_provider": custom_llm_provider,
                            **new_kwargs,
            ...
            if results is not None:
                if isinstance(results, DeRouterBatch):
                    return results
                elif isinstance(results, list):
                    for result in results:
                        if isinstance(result, DeRouterBatch):
                            return result
    ...
    async def _acancel_batch(
        ...
        **kwargs,
        ...
        try:
            ...
            data.pop("derouter_credential_name", None)
            model_name: Final = data["model"]
            ...
            response = derouter.acancel_batch(
                **{
                    **data,
                    "custom_llm_provider": custom_llm_provider,
                    "caching": self.cache_responses,
            ...
            self.success_calls[model_name] += 1
            verbose_router_logger.info("derouter.acancel_batch(model=%s)\x1b[32m 200 OK\x1b[0m", model_name)

            return response
        except Exception as e:
            verbose_router_logger.exception(
                "derouter._acancel_batch(model=%s, %s)\x1b[31m Exception %s\x1b[0m", model, kwargs, e
            )
            if model is not None:
                self.fail_calls[model] += 1
            self._stamp_retry_skip_deployment_id(e, kwargs)
    ...
    async def alist_batches(
        ...
        **kwargs,
        ...
        async def try_retrieve_batch(model: DeploymentTypedDict):
            try:
                # Update kwargs with the current model name or any other model-specific adjustments
                return await derouter.alist_batches(**{**model["derouter_params"], **kwargs})
            except Exception:
                return None
    ...
    async def _pass_through_moderation_endpoint_factory(
        ...
        **kwargs,
        ...
        if kwargs.get("model") and self.get_model_list(model_name=kwargs["model"]):
            ...
            kwargs["model"] = deployment["derouter_params"]["model"]
            data: Final = deployment["derouter_params"].copy()
            self._update_kwargs_with_deployment(
                deployment=deployment,
                kwargs=kwargs,
    ...
    async def _init_containers_api_endpoints(
        ...
        **kwargs,
        ...
        DeRouter-managed container IDs (``cntr_...``) encode ``model_id`` and provider
        ...
        if isinstance(container_id, str):
            ...
            # is a native upstream ID (e.g. Azure hex cntr_) that carries no DeRouter
    ...
    async def _init_interactions_api_endpoints(
        ...
        **kwargs,
        ...
        # must not enter model-group lookup (agent name is not a DeRouter deployment).
        if kwargs.get("model"):
            return await self._ageneric_api_call_with_fallbacks(
                original_function=original_function,
                **kwargs,
        ...
        **kwargs,
        ...
        if custom_llm_provider is None:
            if self.assistants_config is not None:
                ...
                kwargs.update(self.assistants_config["derouter_params"])
            else:
                raise Exception(
    ...
    async def _maybe_run_weighted_failover(
        ...
        cooldown_ids = set(await _async_get_cooldown_deployments(derouter_router_instance=self, parent_otel_span=None))
        ...
        try:
            ...
            # derouter provider errors derive from openai.APIError; if every
    ...
    async def async_function_with_fallbacks_common_utils(
        ...
        _skip_order_fallback: Final = isinstance(
            e,
            (derouter.ContextWindowExceededError, derouter.ContentPolicyViolationError),
        )
        ...
        _order_set: Final[set] = {
            derouter.utils._get_deployment_order(d)
            for d in all_deployments
            if derouter.utils._get_deployment_order(d) is not None
        }
        ...
        try:
            ...
            if isinstance(e, derouter.ContextWindowExceededError):
                if context_window_fallbacks is not None:
                    ...
                    if derouter.expose_router_debug_in_errors:
                        e.message += f"\n{error_message}"
            elif isinstance(e, derouter.ContentPolicyViolationError):
            ...
            if fallbacks is not None and lookup_groups:
                ...
                if fallback_model_group is None:
                    ...
                    if hasattr(original_exception, "message") and derouter.expose_router_debug_in_errors:
            ...
            verbose_router_logger.error(
                "derouter.router.py::async_function_with_fallbacks() - "
    ...
    async def async_function_with_fallbacks(self, *args, **kwargs):
        ...
        if not isinstance(kwargs.get("attempted_targets"), AttemptedFallbackTargets):
            ...
            _sibling_metadata_key: Final = (
                "metadata" if _fallback_metadata_key == "derouter_metadata" else "derouter_metadata"
            )
    ...
    def _handle_mock_testing_fallbacks(
        ...
        """
        Helper function to raise a derouter Error for mock testing purposes.

        Raises:
            derouter.InternalServerError: when `mock_testing_fallbacks=True` passed in request params
            derouter.ContextWindowExceededError: when `mock_testing_context_fallbacks=True` passed in request params
            derouter.ContentPolicyViolationError: when `mock_testing_content_policy_fallbacks=True` passed in request params
        """
        ...
        if (
            ...
            raise derouter.InternalServerError(
                model=model_group,
                llm_provider="",
            ...
            raise derouter.ContextWindowExceededError(
                model=model_group,
                llm_provider="",
    ...
    def _deployment_ids_to_skip_on_retry(exception: Exception, already_skipped: object) -> tuple[str, ...]:
        ...
        if not failed_deployment_id or not isinstance(status_code, int):
            return ()
        if derouter._should_retry(status_code):  # pyright: ignore[reportPrivateUsage]  # as in should_retry_this_error
            return ()
    ...
    async def async_function_with_retries(self, *args, **kwargs):
        ...
        _metadata: Final[dict] = kwargs.get("derouter_metadata", kwargs.get("metadata")) or {}
        ...
        try:
            ...
            if type(original_exception) in derouter.DEROUTER_EXCEPTION_TYPES:
    ...
    def _handle_mock_testing_rate_limit_error(self, kwargs: dict, model_group: str | None = None):
        """
        Helper function to raise a mock derouter.RateLimitError error for testing purposes.

        Raises:
            derouter.RateLimitError error when `mock_testing_rate_limit_error=True` passed in request params
        """
        ...
        if available_models is not None and len(available_models) == 1:
            num_retries = cast(int | None, available_models[0]["derouter_params"].get("num_retries"))

        if mock_testing_rate_limit_error is not None and mock_testing_rate_limit_error is True:
            verbose_router_logger.info(
                "derouter.router.py::_mock_rate_limit_error() - Raising mock RateLimitError for model=%s", model_group
            )
    ...
    def should_retry_this_error(
        ...
        if isinstance(error, derouter.ContextWindowExceededError) and context_window_fallbacks is not None:
            raise error

        if isinstance(error, derouter.ContentPolicyViolationError) and content_policy_fallbacks is not None:
            raise error

        status_code: Final = getattr(error, "status_code", None)
        if status_code is not None and not derouter._should_retry(status_code):
            # 401/403 are special cases - allow retry if multiple deployments exist (handled below)
            if status_code not in (401, 403):
                raise error

        if isinstance(error, derouter.NotFoundError):
            raise error
    ...
    def _time_to_sleep_before_retry(
        ...
        if hasattr(e, "derouter_response_headers"):
            response_headers = e.derouter_response_headers

        if response_headers is not None:
            timeout = derouter._calculate_retry_after(
                remaining_retries=remaining_retries,
                max_retries=num_retries,
    ...
    async def deployment_callback_on_success(
        ...
        """
        from derouter.types.caching import RedisPipelineIncrementOperation
        ...
        try:
            ...
            if kwargs["derouter_params"].get("metadata") is None:
                pass
            else:
                deployment_name: Final = kwargs["derouter_params"]["metadata"].get(
                    "deployment", None
                )  # stable name - works for wildcard routes as well
                # Get model_group and id from kwargs like the sync version does
                model_group: Final = kwargs["derouter_params"]["metadata"].get("model_group", None)
                model_info: Final = kwargs["derouter_params"].get("model_info", {}) or {}
                ...
                if deployment_info is None:
                    ...
                    ## check tpm/rpm in derouter_params
                    tpm_derouter_params: Final = deployment_info.derouter_params.tpm
                    rpm_derouter_params: Final = deployment_info.derouter_params.rpm
                ...
                if (
                    tpm is None
                    and rpm is None
                    and tpm_derouter_params is None
                    and rpm_derouter_params is None
            ...
            verbose_router_logger.debug(
                "derouter.router.Router::deployment_callback_on_success(): Exception occured - %s", e
            )
    ...
    def deployment_callback_on_failure(
        ...
        try:
            ...
            # Cache derouter_params to avoid repeated dict lookups
            derouter_params: Final = kwargs.get("derouter_params", {})
            _model_info: Final = derouter_params.get("model_info", {})

            exception_headers: Final = derouter.derouter_core_utils.exception_mapping_utils._get_response_headers(
            ...
            deployment_cooldown: Final = _first_present(
                _model_info if isinstance(_model_info, dict) else None, derouter_params, key="cooldown_time"
            )

            header_cooldown = None
            if exception_headers is not None:
                header_cooldown = derouter.utils._get_retry_after_from_exception_header(
            ...
            if isinstance(_model_info, dict):
                ...
                result: Final = _set_cooldown_deployments(
                    ...
                    requested_model_group=(get_derouter_metadata_from_kwargs(kwargs) or {}).get("model_group"),
    ...
    async def async_deployment_callback_on_failure(
        ...
        deployment_name: Final = kwargs["derouter_params"]["metadata"].get(
            "deployment", None
        )  # handles wildcard routes - by giving the original name sent to `derouter.completion`
        model_group: Final = kwargs["derouter_params"]["metadata"].get("model_group", None)
        model_info: Final = kwargs["derouter_params"].get("model_info", {}) or {}
        id = model_info.get("id", None)
        if model_group is None or id is None:
    ...
    def _get_metadata_variable_name_from_kwargs(self, kwargs: dict) -> Literal["metadata", "derouter_metadata"]:
        """
        Helper to return what the "metadata" field should be called in the request data

        - New endpoints return `derouter_metadata`
        - Old endpoints return `metadata`

        Context:
        - DeRouter used `metadata` as an internal field for storing metadata
        - OpenAI then started using this field for their metadata
        - DeRouter is now moving to using `derouter_metadata` for our metadata
        """
    ...
    def log_retry(self, kwargs: dict, e: Exception) -> dict:
        ...
        """
        _metadata_var: Final = "derouter_metadata" if "derouter_metadata" in kwargs else "metadata"
    ...
    def _get_healthy_deployments(self, model: str, parent_otel_span: Span | None):
        ...
        unhealthy_deployments: Final = _get_cooldown_deployments(
            derouter_router_instance=self, parent_otel_span=parent_otel_span
        )
    ...
    def routing_strategy_pre_call_checks(self, deployment: dict):
        ...
        """
        for _callback in derouter.callbacks:
    ...
    async def async_routing_strategy_pre_call_checks(
        ...
        deployment: dict,
        parent_otel_span: Span | None,
        logging_obj: DeRouterLogging | None = None,
    ):
        """
        ...
        for _callback in derouter.callbacks:
            if isinstance(_callback, CustomLogger):
                ...
                except derouter.RateLimitError as e:
                    ## LOG FAILURE EVENT
    ...
    @staticmethod
    def generate_model_id(model_group: str, derouter_params: dict) -> str:  # mutable-ok: hashed read-only
        """
        Helper function to consistently generate the same id for a deployment

        - create a string from all the derouter params
        - hash
        - use hash as id
        """
    ...
    def _inherit_builtin_cache_pricing(model_info: dict, backend_model: str, custom_llm_provider: str | None) -> None:
        ...
        if all(model_info.get(f) is not None for f in cache_fields):
            return
        try:
            backend_info: Final = derouter.get_model_info(model=backend_model, custom_llm_provider=custom_llm_provider)
        except Exception:
            return
    ...
    def _inherit_builtin_base_rates_for_off_peak(
        ...
        The raw ``derouter.model_cost`` entry is the copy source rather than
        ...
        try:
            ...
            return
        backend_entry: Final = derouter.model_cost.get(backend_info.get("key") or "")
    ...
    def _inherit_builtin_tiered_output_rate(
        ...
        ``derouter.model_cost`` entry holding only the supplied fields, and the
    ...
    def _create_deployment(
        ...
        *,
        ...
        - None: If the deployment is not active for the current environment (if 'supported_environments' is set in derouter_params)
        """
        try:
            ...
            zeroed_pricing: Final = zeroed_ptu_pricing(_model_info, _derouter_params) if config_sourced else None
            derouter_params: Final[DeRouter_Params] = DeRouter_Params(
                **(  # pyright: ignore[reportArgumentType]  # untyped merged dict; already true for every field here
                    _derouter_params
                    if zeroed_pricing is None
                    else MappingProxyType({**_derouter_params, **zeroed_pricing})
                )
            )
            warn_on_provider_credential_mismatch(model_name=_model_name, derouter_params=_derouter_params)
            deployment = Deployment(
                **deployment_info,
                model_name=_model_name,
            ...
            for field in CustomPricingDeRouterParams.model_fields:
                if deployment.derouter_params.get(field) is not None:
                    _model_info[field] = deployment.derouter_params[field]

            Router._inherit_builtin_base_rates_for_off_peak(
                model_info=_model_info,
                backend_model=deployment.derouter_params.model,
                custom_llm_provider=deployment.derouter_params.custom_llm_provider,
            )
            ...
            ## REGISTER MODEL INFO IN DEROUTER MODEL COST MAP
            Router._register_deployment_in_model_cost(
                model_id=deployment.model_info.id,
                model_info=_model_info,
                model=deployment.derouter_params.model,
                custom_llm_provider=deployment.derouter_params.custom_llm_provider,
            )
            ...
            _tag_regex: Final = deployment.derouter_params.get("tag_regex") or []
            ...
            if self.ignore_invalid_deployments:
                if isinstance(e, derouter.BadRequestError):
                    self._provider_unresolved_deployments = (
                        *self._provider_unresolved_deployments,
                        partial(
                            ...
                            _derouter_params=_derouter_params,
    ...
    def _is_auto_router_deployment(self, derouter_params: DeRouter_Params) -> bool:
        """
        Check if the deployment is an auto-router deployment (semantic router).

        Returns True if the derouter_params model starts with "auto_router/"
        ...
        """
        return classify_strategy_router_model(derouter_params.model) == "semantic"
    ...
    def _deployment_tags(deployment: Deployment) -> tuple[str, ...]:
        ...
        return tuple(deployment.derouter_params.tags or ())
    ...
    def init_auto_router_deployment(self, deployment: Deployment):
        ...
        """
        from derouter.router_strategy.auto_router.auto_router import AutoRouter

        auto_router_config_path: Final[str | None] = deployment.derouter_params.auto_router_config_path
        auto_router_config: Final[str | None] = deployment.derouter_params.auto_router_config
        if auto_router_config_path is None and auto_router_config is None:
            raise ValueError(
                "auto_router_config_path or auto_router_config is required for auto-router deployments. Please set it in the derouter_params"
            )

        default_model: Final[str | None] = deployment.derouter_params.auto_router_default_model
        if default_model is None:
            raise ValueError(
                "auto_router_default_model is required for auto-router deployments. Please set it in the derouter_params"
            )

        embedding_model: Final[str | None] = deployment.derouter_params.auto_router_embedding_model
        if embedding_model is None:
            raise ValueError(
                "auto_router_embedding_model is required for auto-router deployments. Please set it in the derouter_params"
            )

        max_input_chars: Final[int | None] = deployment.derouter_params.auto_router_max_input_chars
    ...
    def _is_complexity_router_deployment(self, derouter_params: DeRouter_Params) -> bool:
        """
        Check if the deployment is a complexity-router deployment.

        Returns True if the derouter_params model starts with "auto_router/complexity_router"
        """
        return classify_strategy_router_model(derouter_params.model) == "complexity"
    ...
    def init_complexity_router_deployment(self, deployment: Deployment):
        ...
        # subclass that imports derouter internals which depend on router.py.
        # This matches the AutoRouter pattern in init_auto_router_deployment above.
        from derouter.router_strategy.complexity_router.complexity_router import (
            ComplexityRouter,
        )
        from derouter.router_strategy.complexity_router.config import (
            ComplexityRouterConfig,
        )

        complexity_router_config: Final[dict | None] = deployment.derouter_params.complexity_router_config
        ...
        default_model: str | None = deployment.derouter_params.complexity_router_default_model
        ...
        if default_model is None:
            raise ValueError(
                ...
                "or configure tiers in complexity_router_config. Please set it in the derouter_params"
            )
    ...
    def _is_adaptive_router_deployment(self, derouter_params: DeRouter_Params) -> bool:
        """True when this deployment opts in via the `auto_router/adaptive_router` model prefix."""
        return classify_strategy_router_model(derouter_params.model) == "adaptive"

    def _deployment_participates_in_adaptive_routing(self, derouter_params: DeRouter_Params) -> bool:
        ...
        if self._is_adaptive_router_deployment(derouter_params=derouter_params):
            return True
        if not self._is_complexity_router_deployment(derouter_params=derouter_params):
            return False
        config: Final = derouter_params.complexity_router_config
        if not config:
            return False
    ...
    def _unregister_pre_routing_strategy_for_deployment(self, deployment: Deployment) -> None:
        ...
        """
        if not deployment.derouter_params.model.startswith("auto_router/"):
            return
    ...
    def _finalize_adaptive_router_if_configured(self) -> None:
        ...
        for entry in self.model_list or []:
            lp = entry.get("derouter_params") if isinstance(entry, dict) else entry.derouter_params
            ...
            deployment = Deployment(
                ...
                derouter_params=(lp if not isinstance(lp, dict) else DeRouter_Params(**lp)),
    ...
    def _sync_adaptive_router_hooks(self) -> None:
        ...
        from derouter.router_strategy.adaptive_router.hooks import (
            AdaptiveRouterPostCallHook,
        )

        for callback in derouter.logging_callback_manager.get_custom_loggers_for_type(AdaptiveRouterPostCallHook):
            derouter.logging_callback_manager.remove_callback_from_all_lists(callback)
    ...
    def init_adaptive_router_deployment(self, deployment: Deployment) -> None:
        ...
        """
        # Local import: AdaptiveRouter -> hooks -> classifier all import derouter
        ...
        raw_config: Final = deployment.derouter_params.adaptive_router_config
        ...
        for name in config.available_models:
            ...
            # model_info is the conventional pricing location elsewhere in DeRouter; derouter_params wins if set.
            lp = d.get("derouter_params") if isinstance(d, dict) else d.derouter_params
    ...
    def _is_quality_router_deployment(self, derouter_params: DeRouter_Params) -> bool:
        """
        Check if the deployment is a quality-router deployment.

        Returns True if the derouter_params model starts with "auto_router/quality_router".
        """
        return classify_strategy_router_model(derouter_params.model) == "quality"
    ...
    def init_quality_router_deployment(self, deployment: Deployment):
        ...
        quality_router_config: Final[dict | None] = deployment.derouter_params.quality_router_config

        default_model: str | None = deployment.derouter_params.quality_router_default_model
        ...
        if default_model is None:
            raise ValueError(
                ...
                "or set default_model in quality_router_config. Please configure it in the derouter_params"
            )
    ...
    def deployment_is_active_for_environment(self, deployment: Deployment) -> bool:
        ...
        Requires `DEROUTER_ENVIRONMENT` to be set in .env. Valid values for environment:
            - development
            - staging
            - production

        Raises:
        - ValueError: If DEROUTER_ENVIRONMENT is not set in .env or not one of the valid values
    ...
    def set_model_list(self, model_list: list):
        ...
        for model in original_model_list:
            ...
            _derouter_params = model.pop("derouter_params")
            ## check if derouter params in os.environ
            if isinstance(_derouter_params, dict):
                for k, v in _derouter_params.items():
                    if isinstance(v, str) and v.startswith("os.environ/"):
                        _derouter_params[k] = get_secret(v)
            ...
            if "id" not in _model_info:
                _id = self.generate_model_id(_model_name, _derouter_params)
                _model_info["id"] = _id

            if _derouter_params.get("organization", None) is not None and isinstance(
                _derouter_params["organization"], list
            ):  # Addresses https://github.com/Delify-Solutions/derouter/issues/3949
                for org in _derouter_params["organization"]:
                    _derouter_params["organization"] = org
                    self._create_deployment(
    ...
    def _add_deployment(self, deployment: Deployment) -> Deployment:
        ...
        derouter_model: Final = deployment.derouter_params.model
        if isinstance(deployment.derouter_params.drop_params, str):
            verbose_router_logger.warning(
                "model=%s drop_params=%r is not a flag value, treating it as unset",
                deployment.model_name,
                deployment.derouter_params.drop_params,
            )
        is_prompt_management_model = False

        if "/" in derouter_model:
            split_derouter_model: Final = derouter_model.split("/")[0]
            if split_derouter_model in derouter._known_custom_logger_compatible_callbacks:
        ...
        if is_prompt_management_model:
            ...
            (
                ...
                model=deployment.derouter_params.model,
                custom_llm_provider=deployment.derouter_params.get("custom_llm_provider", None),
                api_base=deployment.derouter_params.api_base,
            )
            # done reading model["derouter_params"]
            # Check if provider is supported: either in enum or JSON-configured
            if custom_llm_provider not in derouter.provider_list and not JSONProviderRegistry.exists(
                custom_llm_provider
            ):
                raise Exception(f"Unsupported provider - {custom_llm_provider}")

        #### DEPLOYMENT NAMES INIT ########
        self.deployment_names.append(deployment.derouter_params.model)
        ############ Users can either pass tpm/rpm as a derouter_param or a router param ###########
        # for get_available_deployment, we use the derouter_param["rpm"]
        # in this snippet we also set rpm to be a derouter_param
        if deployment.derouter_params.rpm is None and getattr(deployment, "rpm", None) is not None:
            deployment.derouter_params.rpm = getattr(deployment, "rpm")

        if deployment.derouter_params.tpm is None and getattr(deployment, "tpm", None) is not None:
            deployment.derouter_params.tpm = getattr(deployment, "tpm")

        # Check if user is trying to use model_name == "*"
        # this is a catch all model for their specific api key
        # if deployment.model_name == "*":
        #     if deployment.derouter_params.model == "*":
        #         # user wants to pass through all requests to derouter.acompletion for unknown deployments
        ...
        data_sources: Final = deployment.derouter_params.get("dataSources", []) or []
        ...
        #     derouter_router_instance=self, model=deployment.to_json(exclude_none=True)
        # )
        ...
        #########################################################
        if self._is_auto_router_deployment(derouter_params=deployment.derouter_params):
            self.init_auto_router_deployment(deployment=deployment)

        #########################################################
        # Check if this is a complexity-router deployment
        #########################################################
        if self._is_complexity_router_deployment(derouter_params=deployment.derouter_params):
        ...
        #########################################################
        if self._is_quality_router_deployment(derouter_params=deployment.derouter_params):
    ...
    def _initialize_deployment_for_pass_through(self, deployment: Deployment, custom_llm_provider: str):
        """
        Optional: Register vertex credentials for pass-through endpoints if `deployment.derouter_params.use_in_pass_through` is True
        ...
        """
        if deployment.derouter_params.use_in_pass_through is not True:
            return
        ...
        credential_name: Final = deployment.derouter_params.derouter_credential_name
        credential_values: Final = (
            CredentialAccessor.get_credential_values(credential_name) if credential_name is not None else {}
        )
        vertex_project: Final = credential_values.get("vertex_project") or deployment.derouter_params.vertex_project
        vertex_location: Final = credential_values.get("vertex_location") or deployment.derouter_params.vertex_location
        vertex_credentials: Final = (
            credential_values.get("vertex_credentials") or deployment.derouter_params.vertex_credentials
        )

        if vertex_project is None or vertex_location is None:
            raise ValueError(
                "vertex_project, and vertex_location must be set in derouter_params for pass-through endpoints."
            )
    ...
    def add_deployment(self, deployment: Deployment) -> Deployment | None:
        ...
        warn_on_provider_credential_mismatch(
            model_name=deployment.model_name,
            derouter_params=deployment.derouter_params.model_dump(exclude_none=True),
        )
        ...
        for field in CustomPricingDeRouterParams.model_fields:
            field_value = deployment.derouter_params.get(field)
        ...
        # Register custom pricing in derouter.model_cost.
        ...
        try:
            if _deployment_on_router is not None:
                ...
                if (
                    ...
                    and deployment.derouter_params == _deployment_on_router.derouter_params
                    and deployment.model_info == _deployment_on_router.model_info
                ):
                    # No need to update
                    return None

                # if there is a new derouter param -> then update the deployment
            ...
            if self._deployment_participates_in_adaptive_routing(derouter_params=deployment.derouter_params) or (
                _deployment_on_router is not None
                and self._deployment_participates_in_adaptive_routing(
                    derouter_params=_deployment_on_router.derouter_params
                )
            ):
                self._finalize_adaptive_router_if_configured()
            return deployment
    ...
    def _backend_cost_map_keys(model: str, custom_llm_provider: str | None) -> tuple[str, ...]:
        """The ``derouter.model_cost`` keys a deployment's shared backend info is registered under."""
    ...
    def _deployment_model_cost_payload(deployment: Deployment) -> dict:  # mutable-ok: cost-map entry
        """The ``model_info`` a deployment contributes to ``derouter.model_cost``.

        Custom pricing lives on ``derouter_params`` rather than ``model_info``, and
    ...
    def _register_deployment_in_model_cost(
        *,
        ...
        """Write a deployment's metadata into ``derouter.model_cost``.
        ...
        if classify_strategy_router_model(model) is not None:
            model_info = {  # mutable-ok: filtered copy of the caller's entry, handed straight to register_model
                k: v for k, v in model_info.items() if k not in CustomPricingDeRouterParams.model_fields
            }
        ...
        existing_shared_mode: Final = (cast(dict | None, derouter.model_cost.get(backend_key, {})) or {}).get("mode")
    ...
    def _get_router_deployment_budget_limiter(
        ...
        registered on derouter.callbacks are not mistaken for router deployment budgets.
        """
    ...
    def _deployment_has_budget_limits(self, deployment: Deployment) -> bool:
        return (
            deployment.derouter_params.get("max_budget") is not None
            and deployment.derouter_params.get("budget_duration") is not None
    ...
    def get_deployment_credentials(self, model_id: str) -> dict | None:
        ...
        Returns None if the deployment is paused via `DeRouter_ProxyModelTable.blocked`,
        ...
        if deployment is None or self._is_deployment_blocked(deployment):
            return None
        return CredentialDeRouterParams.model_validate(
            deployment.derouter_params.model_dump(exclude_none=True)
    ...
    def get_deployment_credentials_with_provider(
        ...
        Args:
            model_id: Model ID or model name from model_list (e.g., "gpt-4o-derouter")
        ...
        Returns:
            ...
            paused via `DeRouter_ProxyModelTable.blocked` (so passthrough callers
            cannot bypass an admin pause by resolving credentials directly).

        Example:
            credentials = router.get_deployment_credentials_with_provider("gpt-4o-derouter")
        ...
        credentials: Final = CredentialDeRouterParams.model_validate(
            deployment.derouter_params.model_dump(exclude_none=True)
        ).model_dump(exclude_none=True)

        # Resolve derouter_credential_name to actual credentials
        if deployment.derouter_params.derouter_credential_name is not None:
            credential_values: Final = CredentialAccessor.get_credential_values(
                deployment.derouter_params.derouter_credential_name
            )
            if not credential_values:
                verbose_router_logger.warning(
                    "Credential '%s' not found in credential_list", deployment.derouter_params.derouter_credential_name
                )
            credentials.update(credential_values)
            # Remove the credential name since we've resolved it
            credentials.pop("derouter_credential_name", None)

        credentials["model"] = deployment.derouter_params.model

        # Add custom_llm_provider
        if deployment.derouter_params.custom_llm_provider:
            credentials["custom_llm_provider"] = deployment.derouter_params.custom_llm_provider
        elif "/" in deployment.derouter_params.model:
            # Extract provider from "provider/model" format
            credentials["custom_llm_provider"] = deployment.derouter_params.model.split("/")[0]
        else:
    ...
    def get_router_model_info(
        ...
        Augment derouter info with additional params set in `model_info`.
        ...
        if base_model is None:
            base_model = (deployment.get("derouter_params") or {}).get("base_model", None)

        model = base_model

        ## GET PROVIDER - reuse DeRouter_Params if already constructed
        derouter_params_data: Final = deployment.get("derouter_params")
        derouter_params: DeRouter_Params
        if isinstance(derouter_params_data, DeRouter_Params):
            derouter_params = derouter_params_data
        elif isinstance(derouter_params_data, dict) and "model" in derouter_params_data:
            derouter_params = DeRouter_Params(**derouter_params_data)
        else:
            raise ValueError(
                f"Deployment missing valid derouter_params. "
                f"Got: {type(derouter_params_data).__name__}, "
                f"deployment_id: {(deployment.get('model_info') or {}).get('id', 'unknown')}"
            )
        _model, custom_llm_provider, _, _ = derouter.get_llm_provider(
        ...
        if custom_llm_provider == "azure" and base_model is None:
            ...
            # derouter.model_cost as a zeroed stub, so membership alone can't
            ...
            _fallback_entry = derouter.model_cost.get(_azure_fallback_key)
            ...
            if _fallback_resolves:
                ...
                verbose_router_logger.error(
                    "Could not identify azure model '%s'. Set azure 'base_model' for accurate max tokens, cost tracking, etc.- https://router.delify.vn/docs/proxy/cost_tracking#spend-tracking-for-azure-openai-models",
                    _model,
                )
                    ...
                    for potential_model in potential_models:
                        try:
                            if (potential_model.get("model_info") or {}).get("id") == (
                                ...
                                model = (potential_model.get("derouter_params") or {}).get("model")
                                break
                        except Exception:
                            pass

        ## GET DEROUTER MODEL INFO - raises exception, if model is not mapped
        if model is None:
            # Handle case where base_model is None (e.g., Azure models without base_model set)
            # Use the original model from derouter_params
            model = _model
        ...
        model_info: Final = derouter.get_model_info(model=model_info_name)
        if model_info is None:
            return model_info
    ...
    def get_model_info(self, id: str) -> dict | None:
        ...
        Returns
        - dict: the model in list with 'model_name', 'derouter_params', Optional['model_info']
    ...
    def get_deployment_model_info(self, model_id: str, model_name: str) -> ModelInfo | None:
        ...
        2. If not, check if derouter model name is in model info
        ...
        derouter_model_name_model_info: ModelInfo | None = None

        try:
            custom_model_info = copy.deepcopy(derouter.model_cost.get(model_id))
        except Exception:
            pass

        try:
            derouter_model_name_model_info = derouter.get_model_info(model=model_name)
        except Exception:
            pass
        ...
        try:
            if custom_model_info is not None:
                ...
                if base_model is not None:
                    ## update derouter model info with base model info
                    base_model_info: Final = copy.deepcopy(derouter.get_model_info(model=base_model))
        ...
        if custom_model_info is not None and derouter_model_name_model_info is not None:
            # (1) It has both custom model_info set and exists in the built-in map
            # merge with custom overriding built-in
            model_info = cast(
                ModelInfo,
                _update_dictionary(
                    copy.deepcopy(cast(dict, derouter_model_name_model_info)),
                    custom_model_info,
                ),
            )
        elif derouter_model_name_model_info is not None:
            # (2) Built-in only — no custom pricing to merge
            model_info = copy.deepcopy(derouter_model_name_model_info)
    ...
    def _set_model_group_info(self, model_group: str, user_facing_model_group_name: str) -> ModelGroupInfo | None:
        ...
        for model in model_list:
            ...
            derouter_params = DeRouter_Params(**model["derouter_params"])
            # get configurable clientside auth params
            configurable_clientside_auth_params = derouter_params.configurable_clientside_auth_params

            # Cache nested dict access to avoid repeated temporary dict allocations
            model_derouter_params = model.get("derouter_params", {})
            ...
            if _deployment_tpm is None:
                _deployment_tpm = model_derouter_params.get("tpm", None)
            if _deployment_tpm is None:
            ...
            try:
                ...
                if model_id is not None:
                    model_info = self.get_deployment_model_info(model_id=model_id, model_name=derouter_params.model)
                else:
                    model_info = None
            except Exception:
                model_info = None
            ...
            # get llm provider
            derouter_model, llm_provider = "", ""
            try:
                derouter_model, llm_provider, _, _ = derouter.get_llm_provider(
                    model=derouter_params.model,
                    custom_llm_provider=derouter_params.custom_llm_provider,
                )
            except derouter.exceptions.BadRequestError as e:
                verbose_router_logger.error("derouter.router.py::get_model_group_info() - %s", e)

            if model_info is None:
                supported_openai_params = derouter.get_supported_openai_params(
                    model=derouter_model, custom_llm_provider=llm_provider
                )
                ...
                model_info = ModelMapInfo(
                    ...
                    derouter_provider=llm_provider,
                    mode=mode,
    ...
    async def get_model_group_usage(self, model_group: str) -> tuple[int | None, int | None]:
        ...
        for model in model_list:
            ...
            derouter_model: str | None = model["derouter_params"].get(
                "model"
            )  # USE THE MODEL SENT TO derouter.completion() - consistent with how global_router cache is written.
            if id is None or derouter_model is None:
                continue
            tpm_keys.append(
    ...
    async def get_model_group_io_token_usage(self, model_group: str) -> tuple[int | None, int | None]:
        ...
        for model in model_list:
            ...
            derouter_model: str | None = model["derouter_params"].get("model")
            if model_id is None or derouter_model is None:
                continue
            itpm_keys.append(
    ...
    async def set_response_headers(
        ...
        additional_headers["x-derouter-model-group"] = model_group
    ...
    def _build_model_id_to_deployment_index_map(self, model_list: list):
        ...
        for _, model in enumerate(model_list):
            ...
            if model_id is None:
                ...
                model_id = self.generate_model_id(model_name, derouter_params)
    ...
    def resolve_model_name_from_model_id(self, model_id: str | None) -> str | None:
        ...
        so that derouter_params can be automatically injected from the model config.

        Strategy:
        1. First, check if model_id directly matches a model_name or deployment ID
        2. If not, search through router's model_list to find a match by derouter_params.model
        3. Return the model_name if found, None otherwise

        Args:
            model_id: The model_id extracted from decoded video_id
                     (could be model_name or derouter_params.model value)
        ...
        # Strategy 2: Search through router's model_list to find by derouter_params.model
        all_models: Final = self.get_model_list(model_name=None)
        if not all_models:
            return None

        for deployment in all_models:
            derouter_params = deployment.get("derouter_params", {})
            actual_model = derouter_params.get("model")
    ...
    TIER_PARAMS_NEVER_DROPPED: Final = frozenset(all_derouter_params) | frozenset(
        {
    ...
    def _deployment_accepts_param(deployment: DeploymentTypedDict, group: str, param: str) -> bool:
        deployment_params: Final = deployment.get("derouter_params")
        ...
        try:
            ...
            supported: Final = derouter.get_supported_openai_params(
                model=model,
                custom_llm_provider=custom_llm_provider,
            ...
            verbose_router_logger.debug(
                "derouter.router.py::_deployment_accepts_param: keeping %s for model=%s. Got - %s", param, group, e
            )
            return True
    ...
    def _tier_params_the_target_accepts(
        ...
        A tier's derouter_params are an operator override applied to every request the tier routes,
        ...
        derouter control among its supported params, so "no deployment declares it" means derouter
        consumes it rather than that the target refuses it, and dropping one changes derouter's own
        ...
        overlay, the request, or a deployment's own derouter_params is never a drop candidate.
        """
        deployments: Final = self.get_model_list(model_name=model) or ()
        if not deployments:
            return tier_params
        ...
        if not unsupported:
            return tier_params
        verbose_router_logger.warning(
            "derouter.router.py: dropping tier params %s for model=%s, no deployment behind it declares them",
    ...
    def resolved_derouter_models(self, model_name: str, team_id: str | None = None) -> tuple[str, ...]:
        ...
        return tuple(
            ...
            if isinstance(derouter_model := deployment.get("derouter_params", {}).get("model"), str) and derouter_model
        )
                ...
                if var in _int_settings:
                    ...
                    if var == "routing_strategy":
                        ...
                        if _existing_router_settings["routing_strategy"] != value:
                            if value == "lar1":
                                from derouter.router_strategy.lar1_routing import (
    ...
    def _get_client(self, deployment, kwargs, client_type=None):
        ...
        if client_type == "max_parallel_requests":
            ...
            if client is None:
                InitalizeCachedClient.set_max_parallel_requests_client(derouter_router_instance=self, model=deployment)
                client = self.cache.get_cache(key=cache_key, local_only=True, parent_otel_span=parent_otel_span)
            return client
    ...
    def _count_pre_call_check_tokens(
        ...
        DeRouterCompletionResponsesConfig transform so the same token_counter path covers
        ...
        if messages is not None:
            ...
            return derouter.token_counter(messages=counted_messages, tools=tools)
        if input is not None:
            ...
            input_messages: Final = DeRouterCompletionResponsesConfig.transform_responses_api_input_to_messages(
                input=typed_input,
                responses_api_request={"instructions": instructions} if instructions is not None else {},
            )
            return derouter.token_counter(
    ...
    def _deployment_max_input_tokens(self, model: str, deployment: Mapping[str, object]) -> int | None:
        ...
        try:
            ...
            verbose_router_logger.debug(
                "derouter.router.py::_deployment_max_input_tokens: skipping deployment. Got - %s", e
            )
            return None
    ...
    async def _acount_pre_call_check_tokens(
        ...
        try:
            ...
            verbose_router_logger.error(
                "derouter.router.py::_acount_pre_call_check_tokens: failed to count tokens. Got - %s", e
            )
            return None
    ...
    def _pre_call_checks(
        ...
        - model context window < message length. For azure openai models, requires 'base_model' is set. - https://router.delify.vn/docs/proxy/cost_tracking#spend-tracking-for-azure-openai-models
        ...
        for idx, deployment in enumerate(_returned_deployments):
            # Cache nested dict access to avoid repeated temporary dict allocations
            _derouter_params = deployment.get("derouter_params", {})
            ...
            try:
                ...
                if base_model is None:
                    base_model = _derouter_params.get("base_model", None)
                _deployment_model = base_model or _derouter_params.get("model", None)
                ...
                if isinstance(max_input_tokens, int) and has_countable_input:
                    if input_tokens is None:
                        ...
                        try:
                            ...
                            verbose_router_logger.error(
                                "derouter.router.py::_pre_call_checks: failed to count tokens. Returning initial list of deployments. Got - %s",
                                e,
                            )
                            return _returned_deployments
            ...
            if isinstance(model_group_cache, dict) and self.routing_strategy != "usage-based-routing-v2":
                ...
                if isinstance(_derouter_params, dict) and _derouter_params.get("rpm", None) is not None:
                    if isinstance(_derouter_params["rpm"], int) and _derouter_params["rpm"] <= current_request:
                        invalid_model_indices.add(idx)
            ...
            if request_kwargs is not None and request_kwargs.get("allowed_model_region") is not None:
                ...
                if allowed_model_region is not None:
                    if not is_region_allowed(
                        derouter_params=DeRouter_Params(**_derouter_params),
            ...
            if request_kwargs is not None and derouter.drop_params is False:
                ...
                try:
                    ...
                    verbose_router_logger.debug(
                        "derouter.router.py::_pre_call_checks: skipping supported-params check for model=%s. Got - %s",
                        _dep_model_for_params,
                        e,
                    )
                    continue
                ...
                if supported_openai_params is None:
                    ...
                    non_default_params = derouter.utils.get_non_default_params(passed_params=request_kwargs)
        ...
        if len(invalid_model_indices) == len(_returned_deployments):
            ...
            if _rate_limit_error is True:  # allow generic fallback logic to take place
                ...
                raise derouter.ContextWindowExceededError(
                    message=f"derouter._pre_call_checks: Context Window exceeded for given call. No models have context window large enough for this call.\n{_potential_error_str}",
                    model=model,
                    llm_provider="",
                )
    ...
    def _get_model_from_alias(self, model: str) -> str | None:
        ...
        Returns:
        - str, the derouter model name
        - None, if model is not in model group alias
        """
        return resolve_model_group_alias(self.model_group_alias, model)

    def _get_deployment_by_derouter_model(self, model: str) -> list:
        """
        Get the deployment by derouter model.
        """
        return [m for m in self.model_list if m["derouter_params"]["model"] == model]
    ...
    def _try_early_resolve_deployments_for_model_not_in_names(
        ...
        if self.default_deployment is not None:
            # Shallow copy with nested derouter_params copy (100x+ faster than deepcopy)
            updated_deployment: Final = self.default_deployment.copy()
            updated_deployment["derouter_params"] = self.default_deployment["derouter_params"].copy()
            updated_deployment["derouter_params"]["model"] = model
    ...
    def _is_strategy_marker_deployment(deployment: Mapping[str, object]) -> bool:
        ...
        if not isinstance(derouter_params, Mapping):
            return False
        deployment_model: Final = derouter_params.get("model")
    ...
    def _common_checks_available_deployment(
        ...
        if request_kwargs is not None:
            ...
            derouter_metadata: Final = request_kwargs.get("derouter_metadata") or {}
            request_team_id = metadata.get("user_api_key_team_id") or derouter_metadata.get("user_api_key_team_id")
        # check if aliases set on derouter model alias map
        if specific_deployment is True:
            return model, self._get_deployment_by_derouter_model(model=model)
        elif self.has_model_id(model):
            deployment: Final = self.get_deployment(model_id=model)
            if deployment is not None:
                deployment_model: Final = deployment.derouter_params.model
                return deployment_model, deployment.model_dump(exclude_none=True)
            raise ValueError(
                f"DeRouter Router: Trying to call specific deployment, but Model ID :{model} does not exist in Model ID map"
            )
        ...
        if len(healthy_deployments) == 0:
            ...
            # _get_deployment_by_derouter_model does not re-apply that filter.
            if _pre_model_access_group_filter_len == 0:
                _derouter_model_deployments: Final = self._get_deployment_by_derouter_model(model=model)
                healthy_deployments = self._filter_deployments_by_model_access_groups(
                    model=model,
                    healthy_deployments=_derouter_model_deployments,
                    request_kwargs=request_kwargs,
                    request_team_id=request_team_id,
                )
                # If the derouter-model lookup produced candidates that access-group
                ...
                if len(_derouter_model_deployments) > 0 and len(healthy_deployments) == 0:
        ...
        if derouter.model_alias_map and model in derouter.model_alias_map:
            model = derouter.model_alias_map[
                model
    ...
    def _filter_deployments_by_model_access_groups(
        ...
        metadata: Final = request_kwargs.get("metadata") or {}
        derouter_metadata: Final = request_kwargs.get("derouter_metadata") or {}
        user_api_key_auth: Final = metadata.get("user_api_key_auth") or derouter_metadata.get("user_api_key_auth")
        ...
        *OR*
        ...
        if isinstance(healthy_deployments, dict):
            if (healthy_deployments.get("model_info") or {}).get("blocked") is True:
                raise derouter.ServiceUnavailableError(
        ...
        healthy_deployments = derouter.utils.get_order_filtered_deployments(
        ...
        healthy_deployments = derouter.utils.get_excluded_filtered_deployments(
        ...
        healthy_deployments = (
            derouter.utils.get_excluded_filtered_deployments(
    ...
    def _tier_ceiling_under_the_surface_name(
        tier_derouter_params: Mapping[str, object], responses_call: bool
    ) -> Mapping[str, object]:
        ...
        ``max_output_tokens``, and derouter already renames ``max_tokens`` to
        ...
        carried: Final = tuple(
            ...
            if key in tier_derouter_params
        )
        if not carried:
            return tier_derouter_params
        return MappingProxyType(
            {
                **{k: v for k, v in tier_derouter_params.items() if k not in OUTPUT_TOKEN_CEILING_PARAMS},
                surface_key: tier_derouter_params[carried[0]],
            }
        )
    ...
    def _pin_tier_params_onto_request(
        self,
        model: str,
        tier_derouter_params: Mapping[str, object] | None,
        request_kwargs: dict,
        responses_call: bool,
    ) -> bool:
        """Apply a routing strategy's per-tier derouter_params on top of the request and report
        ...
        if not tier_derouter_params:
            return False
        accepted_tier_params: Final = self._tier_params_the_target_accepts(model, tier_derouter_params, request_kwargs)
    ...
    def _restore_client_ceiling_no_tier_pins(request_kwargs: MutableMapping[str, object]) -> None:
        ...
        stamped: Final = next(
            (
                ...
                for bucket in (request_kwargs.get("metadata"), request_kwargs.get("derouter_metadata"))
    ...
    def _drop_client_carriers_a_tier_pin_supersedes(
        ...
        tier_derouter_params: Mapping[str, object],
    ) -> None:
        """Tier derouter_params are deliberate operator overrides, but provider
        ...
        if not OUTPUT_TOKEN_CEILING_PARAMS.isdisjoint(tier_derouter_params):
            ...
            for carrier in OUTPUT_TOKEN_CEILING_PARAMS:
                request_kwargs.pop(carrier, None)
        if "reasoning_effort" not in tier_derouter_params:
            return
    ...
    async def async_get_available_deployment(
        ...
        try:
            ...
            tier_pins_ceiling: Final = self._pin_tier_params_onto_request(
                model=model,
                tier_derouter_params=pre_routing_hook_response.derouter_params if pre_routing_hook_response else None,
                request_kwargs=request_kwargs,
            ...
            if request_kwargs is not None:
                logging_obj: Final = request_kwargs.get("derouter_logging_obj", None)
    ...
    async def async_get_available_deployment_for_pass_through(
        ...
        try:
            ...
            if isinstance(healthy_deployments, dict):
                ...
                derouter_params: Final = healthy_deployments.get("derouter_params", {})
                if derouter_params.get("use_in_pass_through"):
                    await self._async_override_selector_pre_call_check(
                        strategy, strategy_selector, healthy_deployments, parent_otel_span
                    )
                    return healthy_deployments
    ...
    async def _run_routing_plugins(
        ...
        candidate_models: Final = list(self.resolved_derouter_models(model))
    ...
    def _filter_by_routing_plugin_candidates(
        ...
        filtered: Final = [d for d in healthy_deployments if d.get("derouter_params", {}).get("model") in candidate_set]
    ...
    def _claude_code_session_router_cache_key(self, request_kwargs: Mapping[str, object]) -> str | None:
        ...
        if session_id is None or _CLAUDE_CODE_SESSION_ID_RE.fullmatch(session_id) is None:
            return None
        metadata_name: Final = "derouter_metadata" if "derouter_metadata" in request_kwargs else "metadata"
    ...
    async def async_pre_routing_hook(
        ...
        Used for the derouter auto-router to modify the request before the routing decision is made.
        ...
        from derouter.proxy.guardrails.auto_router_compression import (
        ...
        # called - apply the router marker's own derouter_params to the request,
        ...
        # outbound LLM call downstream by derouter.types.utils.all_derouter_params,
        ...
        # Per-tier `derouter_params` on the hook response are deliberate overrides
        ...
        tier_param_keys: Final = (
            tuple(pre_routing_hook_response.derouter_params or ()) if pre_routing_hook_response is not None else ()
        )
    ...
    def _forwardable_alias_marker_params(
        ...
        marker_params: Final = tuple(
            ...
            if isinstance(derouter_params := self.model_list[idx].get("derouter_params", {}), dict)
            and str(derouter_params.get("model", "")).startswith(AUTO_ROUTER_MODEL_PREFIX)
        )
        ...
        return tuple(
            ...
            and key not in CustomPricingDeRouterParams.model_fields
    ...
    def _forwarded_alias_marker_keys_the_deployment_sets(
        ...
        deployment_derouter_params: Final = deployment.get("derouter_params")
        if not isinstance(deployment_derouter_params, Mapping) or not isinstance(forwarded_keys, tuple):
            return ()
        return tuple(
            key
            for key in forwarded_keys
            if isinstance(key, str) and Router._deployment_sets_derouter_param(deployment_derouter_params, key)
        )

    @staticmethod
    def _deployment_sets_derouter_param(deployment_derouter_params: Mapping[str, object], key: str) -> bool:
        value: Final = deployment_derouter_params.get(key)
        if value is None:
            return False
        field: Final = DeRouter_Params.model_fields.get(key)
    ...
    def _stamp_or_clear_metadata_key(request_kwargs: dict, key: str, value: object | None) -> None:
        ...
        proxy-internal metadata": it picks `derouter_metadata` when present (so the
    ...
    def _redact_prompt_text_if_needed(
        ...
        if not should_redact_message_logging(
            {
                "derouter_params": request_kwargs,
    ...
    def get_available_deployment(
        ...
        if len(healthy_deployments) == 0:
            ...
            _cooldown_list = _get_cooldown_deployments(derouter_router_instance=self, parent_otel_span=parent_otel_span)
    ...
    def _filter_blocked_deployments(self, healthy_deployments: list[dict]) -> list[dict]:
        """
        Filters out deployments that an admin has paused via `DeRouter_ProxyModelTable.blocked`.
    ...
    def _filter_pass_through_deployments(self, healthy_deployments: list[dict]) -> list[dict]:
        ...
        pass_through_deployments: Final = [
            deployment
            for deployment in healthy_deployments
            if deployment.get("derouter_params", {}).get("use_in_pass_through", False)
        ]
    ...
    def get_allowed_fails_from_policy(self, exception: Exception):
        ...
        if (
            isinstance(exception, derouter.AuthenticationError)
        ...
        if isinstance(exception, derouter.Timeout) and allowed_fails_policy.TimeoutErrorAllowedFails is not None:
            return allowed_fails_policy.TimeoutErrorAllowedFails
        if (
            isinstance(exception, derouter.RateLimitError)
            and allowed_fails_policy.RateLimitErrorAllowedFails is not None
        ):
            return allowed_fails_policy.RateLimitErrorAllowedFails
        if (
            isinstance(exception, derouter.ContentPolicyViolationError)
        ...
        if (
            isinstance(exception, derouter.ServiceUnavailableError)
        ...
        if (
            isinstance(exception, derouter.BadGatewayError)
            and allowed_fails_policy.BadGatewayErrorAllowedFails is not None
        ):
            return allowed_fails_policy.BadGatewayErrorAllowedFails
        if isinstance(exception, derouter.NotFoundError) and allowed_fails_policy.NotFoundErrorAllowedFails is not None:
    ...
    def _initialize_alerting(self):
        ...
        derouter.logging_callback_manager.add_derouter_callback(_slack_alerting_logger)
        derouter.logging_callback_manager.add_derouter_success_callback(
            _slack_alerting_logger.response_taking_too_long_callback
        )
        verbose_router_logger.info("\033[94m\nInitialized Alerting for derouter.Router\033[0m\n")

    def set_custom_routing_strategy(self, CustomRoutingStrategy: CustomRoutingStrategyBase):
        """
        Sets get_available_deployment and async_get_available_deployment on an instanced of derouter.Router

        Use this to set your custom routing strategy

        Args:
            CustomRoutingStrategy: derouter.router.CustomRoutingStrategyBase
        """
