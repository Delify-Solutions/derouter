...
Runs `prisma migrate deploy` against the DeRouter writer database using the
recovery logic in `derouter_proxy_extras.ProxyExtrasDBManager.setup_database`
...
Env vars:
  ...
  USE_V2_MIGRATION_RESOLVER     "false" → fall back to the v1 resolver
                                ...
                                two DeRouter versions contend for the same DB.
...
from derouter_proxy_extras.utils import ProxyExtrasDBManager, str_to_bool
