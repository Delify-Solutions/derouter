...
use derouter_core::routing_utils::provider::{CustomLlmProvider, get_custom_llm_provider};
...
pub(crate) fn prepare_ocr_call(request: OcrRequest<'_>) -> PreparedOcrCall {
    ...
    let config = ocr_provider_config(&custom_llm_provider, &model)
        .ok_or_else(|| derouter_core::Error::InvalidProvider(custom_llm_provider.clone()))
        .and_then(|config| {
...
fn validate_request_format(
    config: &'static dyn derouter_core::ocr::transformation::OcrProviderConfig,
    optional_params: &Map<String, Value>,
    provider: &str,
) -> Result<(), derouter_core::Error> {
    ...
    match format.as_str() {
        ...
        Some("native") => Err(derouter_core::Error::InvalidRequest(format!(
            "`req_format=native` is not supported for provider {provider}"
        ))),
        _ => Err(derouter_core::Error::InvalidRequest(format!(
            "Invalid `req_format`: {format}. Expected `derouter` or `native`"
        ))),
    }
}
