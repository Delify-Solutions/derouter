...
//! `derouter/integrations/custom_logger.py::CustomLogger`.
//!
...
impl CustomLoggerRunner {
    ...
    pub async fn async_log_success_event(
        ...
        for logger in &self.loggers {
            ...
            {
                report.dropped += 1;
                eprintln!("derouter-ai-gateway: async_log_success_event dropped: {err}");
            }
        }
        report
    }
    ...
    pub async fn async_log_failure_event(
        ...
        for logger in &self.loggers {
            ...
            {
                report.dropped += 1;
                eprintln!("derouter-ai-gateway: async_log_failure_event dropped: {err}");
            }
        }
        report
    }
...
mod tests {
    ...
    struct RecordedEvent {
        ...
        derouter_call_id: Option<String>,
        ...
        fn async_log_success_event<'a>(
            ...
            Box::pin(async move {
                self.events.lock().unwrap().push(RecordedEvent {
                    ...
                    derouter_call_id: model_call_details.derouter_call_id.clone(),
    ...
    fn payload(call_type: &str, model: &str, provider: &str) -> StandardLoggingPayload {
        StandardLoggingPayload {
            ...
            derouter_call_id: format!("call_{call_type}"),
    ...
    async fn rust_custom_logger_reads_success_payload_for_ocr() {
        ...
        assert_eq!(
            ...
            vec![RecordedEvent {
                ...
                derouter_call_id: Some("call_ocr".to_string()),
    ...
    async fn rust_custom_logger_reads_failure_payload_for_non_ocr_call_type() {
        ...
        assert_eq!(
            ...
            vec![RecordedEvent {
                ...
                derouter_call_id: Some("call_acompletion".to_string()),
                user_id: Some("user".to_string()),
    ...
    fn with_standard_logging_payload_keeps_top_level_fields_in_sync() {
        ...
        assert_eq!(details.derouter_call_id, Some("call_ocr".to_string()));
    }
}
