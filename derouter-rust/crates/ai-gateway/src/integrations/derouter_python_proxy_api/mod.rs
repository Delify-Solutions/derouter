//! A `CustomLogger` that ships finished events to the DeRouter Python proxy's
...
/// Ships realtime logging events to the DeRouter Python proxy.
pub struct DeRouterPythonProxyAPILogger {
    sink: Sender<LogRecord>,
}

impl DeRouterPythonProxyAPILogger {
    ...
    /// Build a logger from the environment: `DEROUTER_PROXY_BASE_URL` (default
    /// `http://localhost:4000`) and `DEROUTER_MASTER_KEY`.
    ///
    /// `DEROUTER_PROXY_BASE_URL` is treated as the full base and the route is
    /// appended verbatim, so if the proxy runs under a `SERVER_ROOT_PATH`
    /// (e.g. served at `https://host/derouter`), include it in the base
    /// (`DEROUTER_PROXY_BASE_URL=https://host/derouter`) and the POST lands at
    /// `https://host/derouter/v1/rust_control_plane/logs`.
    pub fn from_env() -> Arc<Self> {
        let base = std::env::var("DEROUTER_PROXY_BASE_URL")
            .ok()
            .filter(|value| !value.trim().is_empty())
            .unwrap_or_else(|| DEFAULT_PROXY_BASE_URL.to_string());
        let key = std::env::var("DEROUTER_MASTER_KEY").unwrap_or_default();
        Self::start(base, key)
    }
...
impl CustomLogger for DeRouterPythonProxyAPILogger {
...
async fn flush(client: &Client, url: &str, master_key: &str, batch: &mut Vec<LogRecord>) {
    ...
    match response {
        ...
        Ok(resp) => {
            eprintln!(
                "derouter-ai-gateway: callback logs POST returned {} to {url}",
                resp.status()
            );
        }
        Err(err) => {
            eprintln!("derouter-ai-gateway: callback logs POST failed to {url}: {err}");
        }
    }
}
