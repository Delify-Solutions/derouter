...
/// (`derouter.proxy.utils.hash_token`).
///
/// STRICT REQUIREMENT: a raw key (`DEROUTER_MASTER_KEY`, a virtual key, …) must
...
/// `DeRouter_SpendLogs.api_key`, so realtime spend joins with the rest of DeRouter.
    ...
    async fn from_request_parts(
        ...
        let Some(expected) = state.master_key.as_deref() else {
            return Err((
                ...
                "gateway auth not configured (set DEROUTER_MASTER_KEY)".to_string(),
            ));
        };
        let provided = parts
            .headers
...
mod tests {
    ...
    fn hash_token_matches_python_sha256_hexdigest() {
        ...
        // the proxy stores in DeRouter_SpendLogs.api_key.
        assert_eq!(
            hash_token("sk-1234"),
