...
use derouter_core::call_lifecycle::{CallLifecycle, CallLifecycleContext};
use derouter_core::responses::instrumentation::{
    ResponsesWsCallbackPayload, ResponsesWsInstrumentation, ResponsesWsLogOutcome,
    ResponsesWsMetadata,
};
use derouter_core::responses::types::ResponsesWsEvent;
...
pub async fn run<In, Out>(
    router: &derouter_core::router::Router,
    model: &str,
...
fn logging_values(
    payload: derouter_core::responses::instrumentation::ResponsesWsLogPayload,
    ...
    let details = ModelCallDetails::from_standard_logging_payload(
        crate::integrations::types::StandardLoggingPayload {
            id: payload.id,
            derouter_call_id: payload.derouter_call_id,
