...
use derouter_core::responses::types::{ResponsesErrorFrame, ResponsesWsEvent, ResponsesWsEventType};
use derouter_core::router::Router as ModelRouter;
...
fn validate_model(router: &ModelRouter, model: &str) -> Result<(), (StatusCode, String)> {
    ...
    if deployment.derouter_params.model.contains('/')
        && !deployment.derouter_params.model.starts_with("openai/")
    {
        return Err((
            StatusCode::BAD_REQUEST,
