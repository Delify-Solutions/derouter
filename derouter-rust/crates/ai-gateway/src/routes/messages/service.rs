...
use derouter_core::Error;
use derouter_core::constants::ANTHROPIC_MESSAGES_PROVIDER;
use derouter_core::messages::types::MessagesRequest;
use derouter_core::messages::{messages, messages_stream};
use derouter_core::router::Router;
...
#[tracing::instrument(
    ...
    target = "derouter::function_trace",
    level = "trace",
    skip_all
)]
pub async fn run(
    ...
    let provider_model = deployment.derouter_params.model.as_str();
    ...
    let request = MessagesRequest {
        ...
        body,
        api_key: deployment.derouter_params.api_key.as_deref(),
        api_base: deployment.derouter_params.api_base.as_deref(),
        custom_llm_provider,
        extra_headers,
        timeout: None,
    };
