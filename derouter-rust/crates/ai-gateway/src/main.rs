//! DeRouter AI Gateway — a minimal Axum server fronting the Rust router.
//!
...
//! the binary needs lives in the library (`derouter_ai_gateway`); `main` just
//! wires startup.

use std::sync::Arc;

use derouter_ai_gateway::io::realtime_pool::{PoolConfig, RealtimePool, upstream_key};
use derouter_ai_gateway::routes;
use derouter_ai_gateway::state::AppState;
#[cfg(feature = "python-config")]
use derouter_config::load_model_list;
use derouter_core::router::{Deployment, DeRouterParams, Router};

use derouter_ai_gateway::integrations::custom_logger::CustomLogger;
use derouter_ai_gateway::integrations::derouter_python_proxy_api::DeRouterPythonProxyAPILogger;
...
async fn main() {
    ...
    let master_key: Option<Arc<str>> = std::env::var("DEROUTER_MASTER_KEY")
        .ok()
    ...
    if master_key.is_none() {
        eprintln!(
            "warning: DEROUTER_MASTER_KEY is not set; /v1/realtime will reject all requests (fail closed)"
        );
    }
    ...
    // tokio runtime. `from_env` reads DEROUTER_PROXY_BASE_URL + DEROUTER_MASTER_KEY.
    let proxy_logger = DeRouterPythonProxyAPILogger::from_env();
    ...
    eprintln!("derouter-ai-gateway listening on {host}:{port}");
...
fn register_deployments(router: &Router, pool: &RealtimePool) {
    for deployment in router.deployments() {
        let params = &deployment.derouter_params;
...
/// Build the router. With the `python-config` feature and `DEROUTER_CONFIG_PATH`
...
fn build_router() -> Router {
    #[cfg(feature = "python-config")]
    if let Ok(config_path) = std::env::var("DEROUTER_CONFIG_PATH") {
