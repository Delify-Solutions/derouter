...
//!
//! Per `derouter-rust/CLAUDE.md`, magic numbers and fixed strings live here
//! (the Rust mirror of Python's `derouter/constants.py`), not inline in feature
//! modules. Env-overridable tunables keep their `DEFAULT_*` value here; the env
//! read + fallback happens at the host/config layer.

/// Default DeRouter control-plane base URL for request-log egress when
/// `DEROUTER_PROXY_BASE_URL` is unset.
