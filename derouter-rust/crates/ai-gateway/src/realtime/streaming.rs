...
//!
//! Mirrors Python `derouter.realtime_api.main.RealTimeStreaming`: it observes the
...
use derouter_core::realtime::types::RealtimeEvent;
...
pub struct RealTimeStreaming {
    ...
    /// `derouter_call_id` are set to that value so the Python writer logs the same
    ...
    derouter_call_id: String,
    /// See the request-id rule above — mirrors `derouter_call_id`.
    id: String,
    model: String,
...
impl RealTimeStreaming {
    /// Create a collector for one session. `derouter_call_id` is the gateway's
    ...
    /// `derouter_call_id`, replacing the gateway-generated fallback.
    fn on_session(&mut self, event: &RealtimeEvent) {
            ...
            && !id.is_empty()
        {
            ...
            self.derouter_call_id = id.to_string();
        }
    ...
    pub fn build_payload(&self) -> StandardLoggingPayload {
        StandardLoggingPayload {
            id: self.id.clone(),
            derouter_call_id: self.derouter_call_id.clone(),
...
mod tests {
    ...
    async fn observe_accumulates_model_and_tokens_then_logs() {
        ...
        // derouter_call_id (replacing the "call_abc" gateway fallback), so the
        // SpendLogs request_id is always the OpenAI session id.
        assert_eq!(payload.id, "sess_001");
        assert_eq!(payload.derouter_call_id, "sess_001");
    ...
    fn blank_session_id_and_model_keep_the_gateway_fallbacks() {
        ...
        assert_eq!(payload.derouter_call_id, "call_fallback");
        ...
        assert_eq!(payload.derouter_call_id, "sess_002");
        assert_eq!(payload.model, "gpt-realtime");
    }
