...
//!   rule that a route's entrypoint and handler live in `derouter-core` (see
//!   `derouter_core::messages`) and move there as they are touched.
//! - [`io`]: compatibility exports and realtime WebSocket splice helpers.
//! - The server modules ([`auth`], [`routes`], [`state`]) and anything pulling
//!   `axum` are gated behind the `server` feature, which the `derouter-ai-gateway`
//!   binary turns on.
