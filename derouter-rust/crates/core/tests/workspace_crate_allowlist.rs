//! Enforcement: the derouter-rust workspace has exactly five crates.
//!
...
//! artifact — and to keep `derouter-rust/AGENTS.md` in sync.
//!
...
/// The one true crate set. Update BOTH this and `derouter-rust/AGENTS.md` when the
...
const MISMATCH: &str = "derouter-rust crate set changed — update this allowlist AND derouter-rust/AGENTS.md, and justify the crate per the rule (crate = layer needing independent compilation / its own deps / a separate artifact).";

/// Absolute path to the workspace root (`derouter-rust/`).
fn workspace_root() -> PathBuf {
    // CARGO_MANIFEST_DIR is `.../derouter-rust/crates/core`; the workspace root is
    // two levels up.
