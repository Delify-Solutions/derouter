...
/// Per-deployment call parameters, mirroring Python's `derouter_params`.
#[derive(Clone, Debug, Deserialize)]
pub struct DeRouterParams {
...
pub struct Deployment {
    ...
    pub derouter_params: DeRouterParams,
}
...
mod tests {
    ...
    fn deserializes_from_model_list_entry() {
        let entry = r#"{
            ...
            "derouter_params": {"model": "openai/gpt-realtime", "api_base": "https://x"}
        }"#;
        let deployment: Deployment = serde_json::from_str(entry).expect("valid entry");
        assert_eq!(deployment.model_name, "gpt-realtime");
        assert_eq!(deployment.derouter_params.model, "openai/gpt-realtime");
        assert_eq!(deployment.derouter_params.api_key, None);
        assert_eq!(
            deployment.derouter_params.api_base.as_deref(),
            Some("https://x")
        );
    }
}
