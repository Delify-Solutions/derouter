//! Minimal Rust port of DeRouter's `router.py` deployment selection.
//!
//! A [`Router`] is built from a `model_list` of [`Deployment`]s
//! (`{ model_name, derouter_params: { model, api_key, api_base } }`) and selects
...
pub use deployment::{Deployment, DeRouterParams};
