...
fn normalize_req_format(req_format: &Value) -> Result<String, Error> {
    match req_format.as_str() {
        Some(value @ ("native" | "derouter")) => Ok(value.to_string()),
        _ => Err(Error::InvalidRequest(format!(
            "Invalid `req_format` for Azure Document Intelligence: {req_format:?}. Expected 'native' or 'derouter'."
        ))),
    }
}
...
mod tests {
    ...
    #[case::derouter(Map::from_iter([("req_format".to_string(), json!("derouter"))]))]
