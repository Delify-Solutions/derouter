...
use derouter_core::router::Deployment;
use pyo3::prelude::*;
...
pub fn load_model_list(config_path: &Path) -> Result<Vec<Deployment>, Error> {
    Python::attach(|python| {
        let model_list = python
            .import("derouter.proxy.read_model_list")
...
mod tests {
    ...
    fn parses_resolved_model_list() {
        ...
        assert_eq!(
            deployments[0].derouter_params.api_key.as_deref(),
            Some("resolved-secret")
        );
        assert_eq!(
            deployments[0].derouter_params.api_base.as_deref(),
            Some("https://api.example.test/v1")
        );
        assert_eq!(deployments[1].derouter_params.api_key, None);
        assert_eq!(deployments[1].derouter_params.api_base, None);
    }

    #[test]
    fn malformed_model_list_returns_parsing_error() {
        let error = parse_model_list(r#"[{"model_name":"missing-params"}]"#)
            .expect_err("missing derouter_params should fail");
