...
function createRequestContextResponse() {
  // AgentService asks every run for client context. derouter has no IDE file
      ...
      try {
        while (!finished) {
          ...
          pending = decodeAgentFrames(pending, (payload) => {
            ...
            // Return an empty context; derouter is not coupled to an editor.
            if (serverMessage.has(2)) {
              ...
              if (execRequest.has(10)) {
                ...
                // (shell, read, write, …) that derouter cannot service. Fail the
