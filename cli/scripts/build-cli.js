...
const cliAppDir = process.env.DEROUTER_CLI_APP_DIR || path.join(cliDir, "app");
...
function resolveStandaloneBuild(appDir, buildDistDir) {
  ...
  // NEXT_TRACING_ROOT_MODE=workspace, e.g. standalone/derouter/server.js.
...
function buildCliPackage() {
  console.log("📦 Building derouter CLI package with Next.js...\n");
  ...
  // Strip better-sqlite3 (native) — it lives in ~/.derouter/runtime to avoid
  ...
  if (fs.existsSync(betterDir)) {
    ...
    console.log("✅ Stripped better-sqlite3 (lives in ~/.derouter/runtime)");
  }
  console.log("");
