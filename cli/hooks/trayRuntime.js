...
function cleanupLegacySystray({ silent = false } = {}) {
  // 1) Runtime dir: ~/.derouter/runtime/node_modules/systray (or %APPDATA% on Win)
  // 2) npm global nested: <npm_prefix>/node_modules/derouter/node_modules/systray
  ...
  for (const dir of targets) {
    if (fs.existsSync(dir)) {
      try {
        ...
        if (!silent) console.log(`[derouter][runtime] removed legacy systray: ${dir}`);
      } catch (e) {
        if (!silent) console.warn(`[derouter][runtime] failed to remove ${dir}: ${e.message}`);
      }
    }
  }
}
...
function chmodSystrayBin({ silent = false } = {}) {
  ...
  try {
    ...
    if (!silent) console.warn(`[derouter][runtime] chmod tray bin failed: ${e.message}`);
  }
}
