...
async function buildCodexHeader() {
  ...
  const { installed, hasderouter, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasderouter, settings } = result.data;
  ...
  // Extract derouter custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:derouter-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract derouter provider config
  const provider = settings?.models?.providers?.["derouter"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("derouter/") ? primary.replace("derouter/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasderouter, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use derouter\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
