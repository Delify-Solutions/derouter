...
function buildMenuItems(port, autostartEnabled) {
  return [
    { title: `derouter (Port ${port})`, tooltip: "Server is running", enabled: false },
  ...
  try {
    ...
    if (isV2) {
      ...
      trayInstance.ready().catch((err) => {
        process.stderr.write(`[derouter] tray failed to start: ${err && err.message ? err.message : err}\n`);
      });
    } else {
    ...
    process.stderr.write(`[derouter] tray init error: ${err.message}\n`);
    return null;
  }
}
