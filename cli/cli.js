...
// Subcommands (`derouter xai video …`) run against an already-running gateway
...
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.derouter/runtime
...
// Identifiers for killAllAppProcesses - only kill derouter specifically
const PROCESS_IDENTIFIERS = [
  'derouter'  // Only package name - avoid killing other apps
];
...
function getAppDataDir() {
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "derouter")
    : path.join(os.homedir(), ".derouter");
}
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running derouter/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "derouter" in cmdline.
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("derouter") && (cmd.includes("cli.js") || cmd.includes("\\derouter") || cmd.includes("/derouter")))
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running derouter/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "derouter".
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("derouter") && (cmd.includes("cli.js") || cmd.includes("/derouter")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
...
function startServer(updatePromise) {
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 derouter is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 derouter is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "derouter", "db.json") : path.join(".derouter", "db.json"));
