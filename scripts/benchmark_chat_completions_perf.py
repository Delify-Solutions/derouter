#!/usr/bin/env python3
"""Benchmark DeRouter proxy /v1/chat/completions overhead and streaming TTFT.

The script can run a local OpenAI-compatible mock provider plus a DeRouter proxy
...
Compare another checkout:
    uv run python scripts/benchmark_chat_completions_perf.py \
        --label v1.83.14-stable --derouter-dir /tmp/derouter-v1.83.14-stable
"""
...
def get_git_revision(derouter_dir: Path) -> str:
    try:
...
def extract_overhead_header(headers: aiohttp.typedefs.LooseHeaders) -> Optional[float]:
    raw_value = headers.get("x-derouter-overhead-duration-ms")  # type: ignore[union-attr]
...
def print_summary(
    ...
    print(
        "x-derouter-overhead-duration-ms p50: "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Checkout directory used to start the DeRouter proxy",
    )
    parser.add_argument(
        "--proxy-command",
        default="uv run derouter",
        help="Command used to start the proxy inside --derouter-dir",
    )
...
async def async_main() -> None:
    args = parse_args()
    derouter_dir = Path(args.derouter_dir).resolve()
    revision = get_git_revision(derouter_dir)
    ...
    with tempfile.TemporaryDirectory(prefix="derouter-perf-") as tmp_dir_name:
