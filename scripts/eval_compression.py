...
The harness runs each problem in two modes:
  ...
  2. **compressed** — prompt is padded with distractor context, then
     ``derouter.compress()`` removes the noise before sending.
...
def call_llm(model: str, messages: list[dict]) -> dict:
    """Call model via derouter. Returns dict with response text and usage."""
    t0 = time.time()
...
def run_benchmark(
    ...
    print(f"\n  Compressed (derouter.compress → then call model):")
    print(
