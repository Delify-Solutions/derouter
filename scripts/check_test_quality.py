...
TQ002   Mock-echo: a test that patches something and whose every assertion only
        ...
        (respx / MockTransport) rather than patching derouter internals.
...
TQ003   `sys.path.insert(...)` inside the test tree. pytest's rootdir handling and
        the installed package already make `derouter` importable, so these are
...
TQ004   Raw `os.environ[...] = ...` assignment. The write outlives the test and
        ...
        at teardown.
TQ005   `derouter.<attr> = ...` module-global mutation. The SDK's module globals are
...
TQ006   A `pytest.skip` reached only when a credential-shaped environment variable is
        ...
        these use.
TQ008   A `patch(...)` whose target is a `derouter.` internal. Patching the SDK's own
        ...
        a third-party client, a transport, or anything outside `derouter.` is untouched.
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Call) and _dotted_name(node.func) == "sys.path.insert":
            yield Violation(
                ...
                f"make derouter importable (suppress: `# {SUPPRESSION_TOKEN}: <reason>`)",
            )
...
def _derouter_attribute_targets(target: ast.expr) -> Iterator[ast.Attribute]:
    if isinstance(target, ast.Tuple):
        for element in target.elts:
            yield from _derouter_attribute_targets(element)
        return
    if isinstance(target, ast.Attribute) and _dotted_name(target.value) == "derouter":
        yield target
...
def iter_global_mutation_violations(path: Path, tree: ast.Module) -> Iterator[Violation]:
    for node in ast.walk(tree):
        ...
        for target in targets:
            for attribute in _derouter_attribute_targets(target):
                yield Violation(
                    path,
                    attribute.lineno,
                    "TQ005",
                    f"derouter.{attribute.attr} is a process-wide global; writing it here is what the "
...
def _sdk_import_bindings(tree: ast.Module) -> Iterator[tuple[str, str]]:
    """(local name, dotted path) for every import that binds something under `derouter`."""
    for node in ast.walk(tree):
...
def _sdk_aliases(tree: ast.Module) -> Mapping[str, str]:
    """Local names bound to something under `derouter`, mapped to the path they stand for.

    `from derouter.llms.openai.chat import handler` then `patch.object(handler.X, ...)`
...
def _saves_sdk_attribute_by_key(node: ast.AST) -> Iterator[ast.Subscript]:
    """Every `<dict>["name"] = <something read off derouter>`, whatever the dict is called.
...
def _saves_sdk_attributes_in_loop(node: ast.For) -> bool:
    ...
    The read is often bound to a local first (`val = getattr(derouter, attr)`) and only
...
def iter_conftest_inventory_violations(path: Path, tree: ast.Module) -> Iterator[Violation]:
    ...
    for name, line in sorted(seen.items(), key=lambda item: item[1]):
        yield Violation(
            ...
            line,
            "TQ007",
            f"`derouter.{name}` is saved and restored around every test in this tree; the list is an "
