...
Requires:
  - DeRouter proxy running on http://localhost:4000 with adaptive_router configured
    (see derouter/proxy/example_config_yaml/adaptive_router_example.yaml).
  - Postgres reachable via DATABASE_URL (same one the proxy uses).
  - DEROUTER_PROXY_KEY env var set (a valid key with permission to send requests).
...
Optional env:
  DEROUTER_PROXY_URL    (default: http://localhost:4000)
...
PROXY_URL: str = os.environ.get("DEROUTER_PROXY_URL", "http://localhost:4000")
try:
    PROXY_KEY: str = os.environ["DEROUTER_PROXY_KEY"]
except KeyError:
    print(
        "ERROR: DEROUTER_PROXY_KEY env var must be set (a proxy key with /chat/completions perms).",
        file=sys.stderr,
    )
    sys.exit(2)
...
async def _post_chat(
    ...
    body = {
        ...
        "metadata": {"derouter_session_id": session_id},
    }
