...
LIT010  Variable assignment without a `Final` declaration. Every local and module-level
        ...
        `TypeAlias` declarations, and module-level names in `derouter/__init__.py`:
        that namespace is the SDK's runtime-settable config surface (users follow
        the documented `derouter.api_key = ...` pattern and the proxy rebinds these
...
Usage
-----
    python check_type_discipline.py derouter/ tests/
...
CONFIG_SURFACE_PARTS = ("derouter", "__init__.py")
...
def _is_config_surface(path: Path) -> bool:
    """The SDK's runtime-settable config module: `derouter.<name> = ...` is the
