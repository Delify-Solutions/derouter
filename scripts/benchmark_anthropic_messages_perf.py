#!/usr/bin/env python3
"""Benchmark DeRouter proxy /v1/messages (Anthropic Messages API) streaming.
...
``message_stop``) and a DeRouter proxy from any checkout, so commits/branches
can be compared without depending on real provider latency.

Example:
    uv run python scripts/benchmark_anthropic_messages_perf.py \
        --label baseline --proxy-command ".venv/bin/derouter"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--derouter-dir", default=str(Path.cwd()))
    parser.add_argument("--proxy-command", default="uv run derouter")
...
async def async_main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="derouter-anthropic-perf-") as tmp_dir_name:
