...
dashboard="$repo_root/ui/derouter-dashboard"
...
cat >&2 <<EOF
with_dashboard_node: node ${current:-missing} does not meet ui/derouter-dashboard's engines floor (>= $floor) and neither nvm nor fnm is available to switch automatically.
Fix it with one of:
