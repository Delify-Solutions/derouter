...
const notice = (candidates: readonly number[], createdAt: string, overrides: Partial<Comment> = {}): Comment => ({
  id: 900,
  body: `<!-- derouter:potential-duplicate candidates=${candidates.join(",")}, -->\n**Potential duplicate detected**`,
...
const config: SweepConfig = { repo: "Delify-Solutions/derouter", graceDays: 3, dryRun: false, now: NOW };
...
describe("candidateNumbers", () => {
  test("reads only the marker field, keeps older issues, sorted ascending and deduplicated", () => {
    const body = "<!-- derouter:potential-duplicate candidates=40,10,30,10, -->\n- #1 - see #1 (100% similar)";
...
describe("sweepIssue", () => {
  ...
  function fakeApi(
    ...
    const api: GitHubApi = {
      request: async <T>(method: string, path: string, body?: object): Promise<T> => {
        ...
        if (path.startsWith("/repos/Delify-Solutions/derouter/issues/35/comments")) {
          return comments as T;
        }
        const reactionsPath = path.match(/^\/repos\/Delify-Solutions\/derouter\/issues\/comments\/(\d+)\/reactions/);
        if (reactionsPath) {
          return (reactionsByNotice[Number(reactionsPath[1])] ?? []) as T;
        }
        if (path === "/repos/Delify-Solutions/derouter/issues/10") {
  ...
  test("a real run comments, labels, then closes with the duplicate reason", async () => {
    ...
    expect(writes.map((write) => write.split(" ").slice(0, 2).join(" "))).toEqual([
      "POST /repos/Delify-Solutions/derouter/issues/35/comments",
      "POST /repos/Delify-Solutions/derouter/issues/35/labels",
      "PATCH /repos/Delify-Solutions/derouter/issues/35",
    ]);
...
describe("sweepClosedIssue", () => {
  function fakeApi(issueBody: Issue, comments: readonly Comment[]): { readonly api: GitHubApi; readonly writes: readonly string[] } {
    ...
    const api: GitHubApi = {
      request: async <T>(method: string, path: string, body?: object): Promise<T> => {
        ...
        if (path === "/repos/Delify-Solutions/derouter/issues/35") {
  ...
  test("a real run unlabels, reopens, then explains", async () => {
    ...
    expect(verdict).toEqual({ kind: "reopen" });
    expect(writes).toEqual([
      "DELETE /repos/Delify-Solutions/derouter/issues/35/labels/duplicate undefined",
      'PATCH /repos/Delify-Solutions/derouter/issues/35 {"state":"open"}',
      `POST /repos/Delify-Solutions/derouter/issues/35/comments {"body":"${REOPEN_COMMENT}"}`,
    ]);
  });
...
describe("readConfig", () => {
  test("defaults to a real run with a 3-day grace period", () => {
    const parsed = readConfig({ GITHUB_TOKEN: "t", GITHUB_REPOSITORY: "Delify-Solutions/derouter" }, NOW);
    expect(parsed).toEqual({ token: "t", repo: "Delify-Solutions/derouter", graceDays: 3, dryRun: false, now: NOW });
  });
  ...
  test("refuses a missing token, a malformed repository, or a bad grace period", () => {
    ...
    expect(() => readConfig({ GITHUB_TOKEN: "t", GITHUB_REPOSITORY: "derouter" }, NOW)).toThrow("owner/repo");
