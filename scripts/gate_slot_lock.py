...
~/.cache/derouter/gate-slots by default, $DEROUTER_GATE_SLOT_DIR to override.
A holder's lock dies with its process, so a crash leaves nothing to clean up.

$DEROUTER_GATE_SLOTS sets the slot count (default 2); 0 disables locking.
...
DEROUTER_GATE_SLOT_HELD, and nested acquisitions under that marker are no-ops,
...
HELD_MARKER_ENV: Final = "DEROUTER_GATE_SLOT_HELD"
SLOT_COUNT_ENV: Final = "DEROUTER_GATE_SLOTS"
SLOT_DIR_ENV: Final = "DEROUTER_GATE_SLOT_DIR"
...
def _slot_dir() -> Path:
    ...
    return Path(override) if override else Path.home() / ".cache" / "derouter" / "gate-slots"
