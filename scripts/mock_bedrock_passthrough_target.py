#!/usr/bin/env python3
"""
Minimal HTTP target for testing DeRouter **Bedrock pass-through** (`/bedrock/...` on the proxy).
...
Wire DeRouter to this host (use **one** of these patterns):
  ...
  2) Environment (see derouter BaseAWSLLM.get_runtime_endpoint)::
...
Then call the proxy, e.g. (model_name must match config)::
  ...
  curl -sS -X POST "http://127.0.0.1:4000/bedrock/model/mock-bedrock-claude/converse" \
    -H "Authorization: Bearer $DEROUTER_KEY" -H "Content-Type: application/json" \
...
async def invoke_with_response_stream(
    ...
    ``InvokeModelWithResponseStream`` / DeRouter's Bedrock invoke streaming path.
    """
    _ = await request.body()
