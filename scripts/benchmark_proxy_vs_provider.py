#!/usr/bin/env python3
"""
Benchmark script comparing DeRouter proxy vs direct provider endpoint.
...
1. Basic Usage (Sequential, Recommended):
   ...
   export DEROUTER_PROXY_URL='http://localhost:4000/chat/completions'
   export PROVIDER_URL='https://api.openai.com/v1/chat/completions'
   export DEROUTER_PROXY_API_KEY='sk-1234'
...
REQUIRED ENVIRONMENT VARIABLES:
  - DEROUTER_PROXY_URL: Full URL to DeRouter proxy chat completions endpoint
  - PROVIDER_URL: Full URL to direct provider chat completions endpoint

OPTIONAL ENVIRONMENT VARIABLES:
  - DEROUTER_PROXY_API_KEY: API key for DeRouter proxy (if auth required)
...
def compare_results(
    ...
    print(f"\n{'='*60}")
    print(f"Comparison: DeRouter Proxy vs Direct Provider")
    print(f"{'='*60}")
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark DeRouter proxy vs direct provider endpoint",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Environment Variables (required):
  DEROUTER_PROXY_URL    - URL of the DeRouter proxy endpoint (e.g., http://localhost:4000/chat/completions)
  PROVIDER_URL         - URL of the direct provider endpoint (e.g., https://api.openai.com/v1/chat/completions)
  DEROUTER_PROXY_API_KEY - API key for DeRouter proxy (optional, but may be required)
...
Examples:
  ...
  python scripts/benchmark_proxy_vs_provider.py --no-warmup
    ...
    DEROUTER_PROXY_URL = os.getenv("DEROUTER_PROXY_URL")
    PROVIDER_URL = os.getenv("PROVIDER_URL")
    DEROUTER_PROXY_API_KEY = os.getenv("DEROUTER_PROXY_API_KEY", "")
    PROVIDER_API_KEY = os.getenv("PROVIDER_API_KEY", "")

    # Validate required environment variables
    if not DEROUTER_PROXY_URL:
        print("Error: DEROUTER_PROXY_URL environment variable is required")
        print(
            "   Example: export DEROUTER_PROXY_URL='https://your-proxy.com/chat/completions'"
        )
        sys.exit(1)
    ...
    # Headers for DeRouter proxy
    proxy_headers = {
        "Content-Type": "application/json",
    }
    if DEROUTER_PROXY_API_KEY:
        proxy_headers["Authorization"] = f"Bearer {DEROUTER_PROXY_API_KEY}"
    else:
        print(
            "Warning: DEROUTER_PROXY_API_KEY not set, requests may fail if authentication is required"
        )
    ...
    print("=" * 60)
    print("DeRouter Proxy vs Provider Benchmark")
    print("=" * 60)
    print(f"Configuration (from environment variables):")
    print(f"  Proxy URL:    {DEROUTER_PROXY_URL}")
    print(f"  Provider URL: {PROVIDER_URL}")
    print(
        f"  Proxy API Key: {'Set' if DEROUTER_PROXY_API_KEY else 'Not set (may cause auth errors)'}"
    )
    print(
    ...
    print_results("DeRouter Proxy", final_proxy_results)
    ...
    if args.runs > 1:
        print_run_variance("DeRouter Proxy", all_proxy_results)
