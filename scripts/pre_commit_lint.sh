...
# The per-area checks:
#   - derouter/ Python  -> `make lint` (test-linting.yml's lint job)
...
#   - dashboard        -> prettier + eslint + lint budgets (test-derouter-ui-build.yml's frontend-lint)
...
# DEROUTER_GATE_SLOT_HELD, so this re-exec happens exactly once and everything this
...
if [ -z "${DEROUTER_GATE_SLOT_HELD:-}" ]; then
...
derouter_py_pattern='^derouter/.*\.py$'
...
spec_pattern='^(derouter/(proxy|types)/.*|ui/derouter-dashboard/(scripts/gen-api-types\.mjs|package\.json|package-lock\.json|src/lib/http/schema\.d\.ts))$'
ui_prettier_pattern='^ui/derouter-dashboard/.*\.(js|jsx|ts|tsx|mjs|cjs|json|css|scss|md|mdx|yml|yaml|html)$'
ui_eslint_pattern='^ui/derouter-dashboard/.*\.(js|jsx|ts|tsx|mjs|cjs)$'

derouter_py_files=$(scope_match "$derouter_py_pattern")
...
fmt_files=$(printf '%s\n' "$derouter_py_files" | grep -v '^derouter/enterprise/' | existing_files)
# check-ui-api-types.yml triggers on any file under derouter/proxy or derouter/types
...
if [ -n "$staged" ]; then
    ...
    warn_skipped "Python lint (make lint)" "$derouter_py_pattern" "$derouter_py_files"
...
lint_dashboard() {
    (
        ...
        eslint_rel=()
        while IFS= read -r f; do
            [ -n "$f" ] && prettier_rel+=("${f#ui/derouter-dashboard/}")
        done <<EOF
$ui_prettier_files
EOF
        while IFS= read -r f; do
            [ -n "$f" ] && eslint_rel+=("${f#ui/derouter-dashboard/}")
        done <<EOF
$ui_eslint_files
EOF
        cd ui/derouter-dashboard
...
python_checks() {
    ...
    # predates the staged change, so format-check the scoped derouter files directly to
    # cover a brand-new commit before it lands.
    if [ -n "$fmt_files" ]; then
        echo "check: ruff format --check (scoped derouter files)"
...
if [ -n "$e2e_py_files" ] && [ -z "$derouter_py_files" ]; then
...
dashboard_checks() {
    ...
    if [ ! -d ui/derouter-dashboard/node_modules ]; then
        echo "✗ ui/derouter-dashboard/node_modules is missing; dashboard lint cannot run." >&2
        bootstrap_hint
        return 1
    fi
    lint_dashboard || { echo "✗ Dashboard lint failed. See above; format with: (cd ui/derouter-dashboard && npm run format)." >&2; return 1; }
}
...
genapi_checks() {
    ...
    # gen-api-types.mjs imports derouter.proxy.proxy_server, which needs the proxy deps
    ...
    if [ ! -d ui/derouter-dashboard/node_modules ]; then
        echo "✗ ui/derouter-dashboard/node_modules is missing; the gen:api sync check cannot run." >&2
        bootstrap_hint
        status=1
        ...
        status=1
    elif ! uv run --no-sync python -m derouter.proxy._lazy_openapi_snapshot; then
        echo "✗ Could not regenerate the lazy OpenAPI snapshot (python -m derouter.proxy._lazy_openapi_snapshot failed)." >&2
        status=1
    elif ( cd ui/derouter-dashboard && DEROUTER_PYTHON="uv run --no-sync python" npm run gen:api ); then
        if ! git diff --quiet -- derouter/proxy/_lazy_openapi_snapshot.json; then
            echo "✗ The lazy OpenAPI snapshot is stale; regenerated derouter/proxy/_lazy_openapi_snapshot.json. Stage it and commit; re-run make check only if other checks failed too." >&2
            status=1
        fi
        if ! git diff --quiet -- ui/derouter-dashboard/src/lib/http/schema.d.ts; then
...
if [ -n "$test_tree_files" ] && [ -z "$derouter_py_files" ]; then
...
echo "check: summary"
summary_item "Python lint (make lint)" "$derouter_py_files" "no derouter/ Python files in scope"
...
summary_item "dashboard API-type sync (npm run gen:api)" "$spec_files" "no derouter/proxy, derouter/types, or generator files in scope"

if [ -z "$derouter_py_files$e2e_py_files$test_tree_files$ui_prettier_changed$ui_eslint_changed$spec_files" ]; then
