...
# Script to create a branch with derouter_ prefix from a contributor's branch
# Usage: ./create_derouter_branch.sh [source_branch] [new_branch_name]
# 
# Examples:
#   ./create_derouter_branch.sh branch-name
#   ./create_derouter_branch.sh remote:branch-name
#   ./create_derouter_branch.sh codgician:ghcopilot-costmap
#
...
#   - Create a new branch with derouter_ prefix
#
...
if [[ "$SOURCE_BRANCH" == *:* ]]; then
    ...
    if ! git remote | grep -q "^${REMOTE_NAME}$"; then
        ...
        if git remote add "$REMOTE_NAME" "https://github.com/${REMOTE_NAME}/derouter.git" 2>/dev/null; then
            print_success "Added remote '$REMOTE_NAME'"
        else
            print_error "Failed to add remote '$REMOTE_NAME'. Please add it manually:"
            print_info "  git remote add $REMOTE_NAME https://github.com/${REMOTE_NAME}/derouter.git"
            exit 1
        fi
    fi
...
# Remove derouter_ prefix if it already exists
if [[ "$NEW_BRANCH_NAME" == derouter_* ]]; then
    NEW_BRANCH_NAME="${NEW_BRANCH_NAME#derouter_}"
    print_warning "Removed existing derouter_ prefix from branch name"
fi

# Add derouter_ prefix
NEW_BRANCH_NAME="derouter_${NEW_BRANCH_NAME}"
