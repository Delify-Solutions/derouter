...
#
# Usage:
#   export DEROUTER_API_KEY="sk-..."   # required, use an admin key
#   ./scripts/test_guardrails_register_endpoints.sh
#   BASE_URL=http://localhost:4000 DEROUTER_API_KEY="sk-..." ./scripts/test_guardrails_register_endpoints.sh
#
set -euo pipefail

BASE_URL="${BASE_URL:-http://localhost:4000}"
API_KEY="${DEROUTER_API_KEY:-}"
...
if [[ -z "$API_KEY" ]]; then
  echo "Error: DEROUTER_API_KEY is not set. Use an admin key to test list/approve/reject."
  exit 1
fi
