 ...
 *
 * The Python interpreter must have derouter installed. Override which one via
 * DEROUTER_PYTHON (CI passes "uv run --no-sync python"); defaults to python3.
 */
...
const specDir = mkdtempSync(join(tmpdir(), "derouter-openapi-"));
const specPath = join(specDir, "openapi.json");

const python = (process.env.DEROUTER_PYTHON ?? "python3").split(" ");
