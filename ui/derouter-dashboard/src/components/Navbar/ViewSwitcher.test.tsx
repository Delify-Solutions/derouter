...
describe("ViewSwitcher", () => {
  ...
  it("labels the button from the active plugin and lists AI Gateway + each plugin", async () => {
    state.plugins = [
      { name: "derouter-platform-plugin", display_name: "Chat UI", url: "http://localhost:3300" },
      { name: "obs", display_name: "Observability", url: "http://localhost:9000" },
    ];
    state.mode = "derouter-platform-plugin";
    render(<ViewSwitcher />);
  ...
  it("switches plugin mode when a mode entry is picked", async () => {
    state.plugins = [{ name: "derouter-platform-plugin", display_name: "Chat UI", url: "http://localhost:3300" }];
    state.mode = "ai-gateway";
    render(<ViewSwitcher />);
    ...
    expect(state.setMode).toHaveBeenCalledWith("derouter-platform-plugin");
    expect(assignSpy).not.toHaveBeenCalled();
  });
