...
const sameMappings = (left: readonly ModelMapping[], right: readonly ModelMapping[]): boolean =>
  ...
  left.every(
    (mapping, index) =>
      mapping.public_name === right[index].public_name && mapping.derouter_model === right[index].derouter_model,
  );
...
const publicNameTooltipContent = (
  <div className="flex flex-col gap-2 text-left font-normal">
    <div>The name you specify in your API calls to DeRouter Proxy</div>
    <div>
      <strong>Example:</strong> If you name your public model <code className={tooltipCodeClassName}>example-name</code>
      , and choose <code className={tooltipCodeClassName}>openai/qwen-plus-latest</code> as the DeRouter model
    </div>
    <div>
      <strong>Usage:</strong> You make an API call to the DeRouter proxy with{" "}
      <code className={tooltipCodeClassName}>model = &quot;example-name&quot;</code>
    </div>
    <div>
      <strong>Result:</strong> DeRouter sends <code className={tooltipCodeClassName}>qwen-plus-latest</code> to the
      provider
    </div>
  </div>
);
...
const PublicNameInput: React.FC<{ readonly index: number; readonly value: string }> = ({ index, value }) => {
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const derouterParams = form.getValues("derouter_extra_params") as string | undefined;
    const wantsAnthropic1m =
      selectedProvider === Providers.Anthropic && typed.endsWith("-1m") && (derouterParams ?? "").trim() === "";

    if (wantsAnthropic1m) {
      form.setValue("derouter_extra_params", ANTHROPIC_1M_HEADERS);
    }
...
const columns: ColumnDef<ModelMapping>[] = [
  ...
  {
    id: "derouter_model",
    accessorKey: "derouter_model",
    header: () => (
      <span className="flex items-center">
        DeRouter Model Name
        <SimpleTooltip content={<div>The model name DeRouter will send to the LLM API</div>} width="360px" />
      </span>
    ),
  },
];
  ...
  useEffect(() => {
    if (customModelName && selectedModels.includes("custom")) {
      ...
      const updatedMappings = currentMappings.map((mapping) => {
        if (mapping.public_name === "custom" || mapping.derouter_model === "custom") {
          if (selectedProvider === Providers.Azure) {
            return {
              public_name: customModelName,
              derouter_model: `azure/${customModelName}`,
            };
          }
          return {
            public_name: customModelName,
            derouter_model: customModelName,
          };
        }
        return mapping;
      });
      ...
      const shouldUpdateMappings =
        ...
        !selectedModels.every((model) =>
          currentMappings.some((mapping) => {
            if (model === "custom") {
              return mapping.derouter_model === "custom" || mapping.derouter_model === customModelName;
            }
            if (selectedProvider === Providers.Azure) {
              return mapping.derouter_model === `azure/${model}`;
            }
            return mapping.derouter_model === model;
          }),
        );
      ...
      if (shouldUpdateMappings) {
        const mappings = selectedModels.map((model: string) => {
          ...
          if (selectedProvider === Providers.Azure) {
            return {
              public_name: model,
              derouter_model: `azure/${model}`,
            };
          }
          return {
            public_name: model,
            derouter_model: model,
          };
        });
  ...
  return (
    <MountedFormField
      ...
      label={
        <span className="flex items-center">
          Model Mappings
          <SimpleTooltip content="Map public model names to DeRouter model names for load balancing" />
        </span>
      }
      required
    ...
    >
      {(control) => (
        <DataTable
          ...
          columns={columns}
          getRowId={(row) => row.derouter_model}
          size="compact"
        />
      )}
    </MountedFormField>
