...
interface DeRouterModelNameFieldProps {
  selectedProvider: Providers;
  providerModels: string[];
  getPlaceholder: (provider: Providers) => string;
}

const DeRouterModelNameField: React.FC<DeRouterModelNameFieldProps> = ({
  selectedProvider,
  providerModels,
  getPlaceholder,
}) => {
      ...
      ? [
          {
            ...
            derouter_model: `azure/${deploymentName}`,
          },
        ]
      : [];
      ...
      if (mapping.public_name === "custom" || mapping.derouter_model === "custom") {
        if (selectedProvider === Providers.Azure) {
          return {
            ...
            derouter_model: `azure/${customName}`,
          };
        }
        return {
          public_name: customName,
          derouter_model: customName,
        };
      }
      return mapping;
    });
  ...
  return (
    <>
      <MountedFormField
        name="model"
        label={labelWithHint("DeRouter Model Name(s)", "The model name DeRouter will send to the LLM API")}
        required
        rules={{
          validate: {
      ...
      <div className="grid grid-cols-24">
        <p className="col-start-11 col-span-14 text-sm mb-3 mt-1">
          {selectedProvider === Providers.Azure
            ? "Your deployment name will be saved as the public model name, and DeRouter will use 'azure/deployment-name' internally"
            : "The model name DeRouter will send to the LLM API"}
        </p>
      </div>
    </>
  );
...
export default DeRouterModelNameField;
