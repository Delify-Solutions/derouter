...
export const createCredentialFromModel = (provider: string, modelData: any): CredentialItem => {
  ...
  providerFields.forEach((field) => {
    const value = modelData.derouter_params[field.key];
    if (value !== undefined) {
...
const ProviderSpecificFields: React.FC<ProviderSpecificFieldsProps> = ({ selectedProvider }) => {
  ...
  const cacheEntries = React.useMemo(() => {
    ...
    providerMetadata.forEach((providerInfo) => {
      ...
      if (providerInfo.derouter_provider) {
        entries[providerInfo.derouter_provider] = mappedFields;
      }
    });
    return entries;
  }, [providerMetadata]);
  ...
  const allFields = React.useMemo(() => {
    ...
    const providerInfo = providerMetadata.find(
      (p) =>
        ...
        p.derouter_provider === selectedProvider,
    );
    if (!providerInfo) {
      return [];
    }
    ...
    if (providerInfo.derouter_provider) {
      providerFieldsByDisplayName[providerInfo.derouter_provider] = mapped;
    }
    return mapped;
          ...
          {field.key === "base_model" && (
            <div className="grid grid-cols-24">
              <p className="col-start-11 col-span-10 text-sm mb-2">
                ...
                <a
                  href="https://github.com/Delify-Solutions/derouter/blob/main/model_prices_and_context_window.json"
                  target="_blank"
