/**
 * Maps the auto router's compression form state to the two flat derouter_params keys
 * the backend reads (derouter.proxy.guardrails.auto_router_compression), and back.
 *
 ...
 * derouter/proxy/guardrails/auto_router_compression.py. Both are selectable per hop. */
...
export const hydrateAutoRouterCompression = (derouterParams: {
  auto_router_routing_compression?: string | null;
  auto_router_model_compression?: string | null;
}): AutoRouterCompressionState => {
  const storedRouting = derouterParams.auto_router_routing_compression ?? undefined;
  const storedModel = derouterParams.auto_router_model_compression ?? undefined;

  // Only neither key set means the section was never touched. The backend treats
  // either key on its own as an authoritative policy (policy_from_derouter_params), so
