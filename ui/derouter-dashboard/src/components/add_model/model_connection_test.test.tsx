...
const preparedRequest = [{ derouterParamsObj: { model: "openai/gpt-4o-mini" }, modelInfoObj: { mode: "chat" } }];
...
describe("ModelConnectionTest", () => {
  ...
  it("shows a cleaned provider error, request details, and copies the curl command", async () => {
    ...
    vi.mocked(testConnectionRequest).mockResolvedValue({
      status: "error",
      result: {
        error: "derouter.AuthenticationError: invalid api key stack trace: hidden",
