...
const ModelConnectionTest: React.FC<ModelConnectionTestProps> = ({
  ...
  const testModelConnection = async () => {
    ...
    try {
      ...
      const { derouterParamsObj, modelInfoObj } = result[0];
      const response = await testConnectionRequest(accessToken, derouterParamsObj, modelInfoObj, modelInfoObj?.mode);
  ...
  return (
    <div className="rounded-lg bg-background p-6">
      ...
      <Button
        ...
        className="px-0"
        nativeButton={false}
        render={<a href="https://router.delify.vn/docs/providers" target="_blank" rel="noopener noreferrer" />}
      >
