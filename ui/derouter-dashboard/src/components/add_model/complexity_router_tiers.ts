...
const asTierEntryObject = (entry: unknown): { model_name: string; derouter_params: TierModelParams } | undefined => {
  const record = asRecord(entry);
  if (record === undefined || typeof record.model_name !== "string" || !record.model_name) return undefined;
  return { model_name: record.model_name, derouter_params: asRecord(record.derouter_params) ?? {} };
};

/**
 * A complexity tier maps to `str | object | list[str | object]` on the backend
 * (derouter/router_strategy/complexity_router/config.py: string/object = pin; list = random pick;
 * an object is `{model_name, derouter_params}`), and the router widens a bare value to a list.
 *
...
const tierEntriesWithParams = (value: unknown): [string, TierModelParams][] =>
  (Array.isArray(value) ? value : [value])
    ...
    .filter((entry): entry is { model_name: string; derouter_params: TierModelParams } => entry !== undefined)
    .filter((entry) => Object.keys(entry.derouter_params).length > 0)
    .map((entry) => [entry.model_name, entry.derouter_params]);
...
): Record<string, { model_name: string; derouter_params: TierModelParams }[]> | undefined => {
  ...
  const serialized = Object.entries(tierModelParams)
    .map(([tier, byModel]) => {
      ...
      const entries = Object.entries(byModel)
        ...
        .map(([model_name, derouter_params]) => ({ model_name, derouter_params }));
