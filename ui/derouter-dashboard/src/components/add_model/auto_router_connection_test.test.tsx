...
describe("AutoRouterConnectionTest", () => {
  ...
  it("renders the provider error message (derouter prefix stripped) for a failing target while others pass", async () => {
    const mock = await getMock();
    mock.mockImplementation((_token, modelGroup) =>
      Promise.resolve(
        modelGroup === "claude-sonnet-4"
          ? { status: "error", error: "derouter.AuthenticationError: invalid api key" }
          : { status: "success" },
      ),
    );
    ...
    expect(screen.getByTestId("test-error-message")).not.toHaveTextContent("derouter.AuthenticationError");
    expect(screen.getAllByTestId("test-status-success")).toHaveLength(3);
  });

  it("renders a non-derouter error string verbatim", async () => {
    const mock = await getMock();
