...
// (derouter/router_strategy/complexity_router/config.py:255, :441). These cases assert the
...
// The backend also accepts `{model_name, derouter_params}` entries and splits them into the
...
describe("normalizeTierModels object entries", () => {
  it("reads model_name from an object entry the way the backend does", () => {
    expect(normalizeTierModels([{ model_name: "opus", derouter_params: { reasoning_effort: "high" } }, "mini"])).toEqual(
      ["opus", "mini"],
    );
  });
  ...
  it("drops an object without a model_name", () => {
    expect(normalizeTierModels([{ derouter_params: { reasoning_effort: "high" } }])).toEqual([]);
  });
});
...
describe("hydrateTierModelParams", () => {
  it("reads the sibling tier_model_configs key", () => {
    expect(
      hydrateTierModelParams(
        { MEDIUM: ["opus"] },
        { MEDIUM: [{ model_name: "opus", derouter_params: { reasoning_effort: "medium" } }] },
      ),
  ...
  it("reads inline object entries out of tiers", () => {
    expect(
      hydrateTierModelParams(
        { COMPLEX: [{ model_name: "opus", derouter_params: { reasoning_effort: "high" } }] },
        undefined,
      ),
  ...
  it("lets tier_model_configs beat an inline entry for the same tier and model", () => {
    expect(
      hydrateTierModelParams(
        { MEDIUM: [{ model_name: "opus", derouter_params: { reasoning_effort: "low" } }] },
        { MEDIUM: [{ model_name: "opus", derouter_params: { reasoning_effort: "medium" } }] },
      ),
    ).toEqual({ MEDIUM: { opus: { reasoning_effort: "medium" } } });
  });
  ...
  it("hydrates to undefined when nothing carries params, so an untouched save stays byte-identical", () => {
    expect(
      hydrateTierModelParams({ SIMPLE: ["mini"], MEDIUM: [{ model_name: "opus", derouter_params: {} }] }, undefined),
    ).toBeUndefined();
  });
});
...
describe("serializeTierModelConfigs", () => {
  ...
  it("carries params keys this editor has no control for", () => {
    expect(
      ...
      MEDIUM: [{ model_name: "opus", derouter_params: { reasoning_effort: "medium", max_tokens: 512 } }],
    });
  });
  ...
  it("passes tiers this editor does not render through untouched", () => {
    expect(serializeTierModelConfigs(tiers, { DEEP_RESEARCH: { opus: { reasoning_effort: "xhigh" } } })).toEqual({
      DEEP_RESEARCH: [{ model_name: "opus", derouter_params: { reasoning_effort: "xhigh" } }],
    });
  });

  it("round-trips what hydration produced", () => {
    const stored = { MEDIUM: [{ model_name: "opus", derouter_params: { reasoning_effort: "medium" } }] };
