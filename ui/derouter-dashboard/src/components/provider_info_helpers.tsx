...
export const getProviderLogoAndName = (providerValue: string): { logo: string; displayName: string } => {
  ...
  // Resolve by the derouter provider slug (e.g. "bedrock_mantle"); fall back to
...
export const getProviderModels = (provider: Providers, modelMap: any): Array<string> => {
  ...
  if (providerKey && typeof modelMap === "object") {
    Object.entries(modelMap).forEach(([key, value]) => {
      if (value !== null && typeof value === "object" && "derouter_provider" in (value as object)) {
        const derouterProvider = (value as any)["derouter_provider"];
        const isPrefixVariant =
          typeof derouterProvider === "string" &&
          (derouterProvider.startsWith(`${custom_llm_provider}_`) ||
            derouterProvider.startsWith(`${custom_llm_provider}-`));
        if (
          derouterProvider === custom_llm_provider ||
          (isPrefixVariant && !standaloneSubproviderSlugs.has(derouterProvider))
        ) {
    ...
    if (providerKey == Providers.Cohere) {
      Object.entries(modelMap).forEach(([key, value]) => {
        if (
          value !== null &&
          typeof value === "object" &&
          "derouter_provider" in (value as object) &&
          (value as any)["derouter_provider"] === "cohere_chat"
        ) {
          providerModels.push(key);
        }
      });
    ...
    if (providerKey == Providers.SageMaker) {
      Object.entries(modelMap).forEach(([key, value]) => {
        if (
          ...
          typeof value === "object" &&
          "derouter_provider" in (value as object) &&
          (value as any)["derouter_provider"] === "sagemaker_chat"
        ) {
          providerModels.push(key);
        }
      });
