...
// by PR #36615) and derouter_params.complexity_router_default_model (what the backend reads). Only
// the marker proves an operator picked it, because before #36615 every save wrote a tier-derived
// value into derouter_params. So with no marker, a derouter_params value counts as a pin only when
...
export const hydratePinnedDefaultModel = (
  ...
  derouterParamsDefaultModel: string | null | undefined,
  ...
  const externalOverride = derouterParamsDefaultModel?.trim();
...
const EditAutoRouterModal: React.FC<EditAutoRouterModalProps> = ({
  ...
  const isComplexityRouterModel = isComplexityRouter(modelData?.derouter_params);
  ...
  const initializeForm = () => {
    ...
    try {
      if (isComplexityRouterModel) {
        ...
        let parsedConfig = modelData.derouter_params?.complexity_router_config || {};
        ...
        const hydratedComplexityRouterConfig = hydrateComplexityRouterConfig(
          parsedConfig,
          modelData.derouter_params?.complexity_router_default_model,
        );
        ...
        setAutoRouterCompression(
          hydrateAutoRouterCompression({
            auto_router_routing_compression: modelData.derouter_params?.auto_router_routing_compression,
            auto_router_model_compression: modelData.derouter_params?.auto_router_model_compression,
          }),
        );
      ...
      if (modelData.derouter_params?.auto_router_config) {
        if (typeof modelData.derouter_params.auto_router_config === "string") {
          parsedConfig = JSON.parse(modelData.derouter_params.auto_router_config);
        } else {
          parsedConfig = modelData.derouter_params.auto_router_config;
        }
      }
      ...
      form.reset({
        auto_router_name: modelData.model_name,
        auto_router_default_model: modelData.derouter_params?.auto_router_default_model || "",
        auto_router_embedding_model: modelData.derouter_params?.auto_router_embedding_model || "",
        model_access_group: modelData.model_info?.access_groups || [],
      });
    } catch (error) {
  ...
  const saveValues = async (values: EditAutoRouterFormValues) => {
    if (isComplexityRouterModel) {
      ...
      // raises in that case (derouter/router.py), so block it rather than saving a router that
      // fails at init.
      ...
      const updatedConfig = buildUpdatedComplexityRouterConfig(
        modelData.derouter_params?.complexity_router_config,
      ...
      await modelPatchUpdateCall(
        accessToken,
        { model_name: values.auto_router_name, derouter_params: updatedLitellmParams, model_info: updatedModelInfo },
      ...
      onSuccess({
        ...
        derouter_params: updatedLitellmParams,
    ...
    // Prepare the updated derouter_params
    const updatedLitellmParams = {
      ...modelData.derouter_params,
