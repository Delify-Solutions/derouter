...
const savedConfig = () => {
  ...
  return payload?.derouter_params?.complexity_router_config;
};
  ...
  const renderLlmModal = () =>
    renderWithProviders(
      <EditAutoRouterModal
        ...
        modelData={{
          ...MODEL_DATA,
          derouter_params: { ...MODEL_DATA.derouter_params, complexity_router_config: STORED_LLM_CONFIG },
        }}
        accessToken="token"
        userRole="Admin"
      />,
  ...
  const renderModal = () =>
    renderWithProviders(
      <EditAutoRouterModal
        ...
        modelData={{
          ...MODEL_DATA,
          derouter_params: { ...MODEL_DATA.derouter_params, complexity_router_config: STORED_CONFIG },
        }}
        accessToken="token"
        userRole="Admin"
      />,
  ...
  const renderWithStoredConfig = (complexity_router_config: Record<string, unknown>) =>
    renderWithProviders(
      <EditAutoRouterModal
        ...
        onCancel={vi.fn()}
        onSuccess={vi.fn()}
        modelData={{ ...MODEL_DATA, derouter_params: { ...MODEL_DATA.derouter_params, complexity_router_config } }}
        accessToken="token"
        userRole="Admin"
      />,
    );
  ...
  const renderCustomModal = () =>
    renderWithProviders(
      <EditAutoRouterModal
        ...
        modelData={{
          ...MODEL_DATA,
          derouter_params: { ...MODEL_DATA.derouter_params, complexity_router_config: STORED_CUSTOM_CONFIG },
        }}
        accessToken="token"
        userRole="Admin"
      />,
...
describe("EditAutoRouterModal default model", () => {
  ...
  const savedDefaultModel = () => {
    const [, payload] = modelPatchUpdateCall.mock.calls.at(-1) ?? [];
    return payload?.derouter_params?.complexity_router_default_model;
  };
  ...
  // No config blob marker — only derouter_params.complexity_router_default_model, as an untouched
  ...
  // Greptile P1 on #36615: with no config blob marker, a derouter_params default that merely
  ...
  it("treats a derouter_params default matching tier-derivation as unpinned, not a frozen-in pin", async () => {
    const user = userEvent.setup();
  ...
  // Greptile P1 on #36615: a derouter_params default that diverges from tier-derivation could only
  ...
  it("treats a diverging derouter_params default as an external pin and preserves it", async () => {
    const user = userEvent.setup();
  ...
  // hydratePinnedDefaultModel), so it wins even over a derouter_params value that disagrees —
  // e.g. a stale value from before the operator most recently changed the pin.
  it("prefers the config blob marker over a diverging derouter_params value", async () => {
    const user = userEvent.setup();
    renderWithProviders(
      <EditAutoRouterModal
        isVisible
        ...
        modelData={{
          ...
          derouter_params: {
            ...
            complexity_router_default_model: "stale-derouter-params-value",
          },
        }}
        accessToken="token"
        userRole="Admin"
...
describe("EditAutoRouterModal with a stored custom tier set", () => {
  const CUSTOM_STORED = {
    ...
    tier_model_configs: {
      SECURITY_REVIEW: [{ model_name: "gpt-4o-mini", derouter_params: { reasoning_effort: "high" } }],
    },
  };
  ...
  const renderCustomModal = () =>
    renderWithProviders(
      <EditAutoRouterModal
        ...
        modelData={{
          ...MODEL_DATA,
          derouter_params: { ...MODEL_DATA.derouter_params, complexity_router_config: CUSTOM_STORED },
        }}
        accessToken="token"
        userRole="Admin"
      />,
...
describe("EditAutoRouterModal prompt compression", () => {
  ...
  const savedLitellmParams = () => {
    const [, payload] = modelPatchUpdateCall.mock.calls.at(-1) ?? [];
    return payload?.derouter_params;
  };
  ...
  const renderWithStoredCompression = (compression?: {
    ...
    renderWithProviders(
      <EditAutoRouterModal
        ...
        modelData={{
          ...MODEL_DATA,
          derouter_params: { ...MODEL_DATA.derouter_params, ...compression },
        }}
        accessToken="token"
        userRole="Admin"
      />,
