...
let mockHealthDataImpl = (): { derouter_version?: string } | undefined => ({ derouter_version: "1.99.0" });
...
describe("SidebarAccountMenu", () => {
  ...
  beforeEach(() => {
    ...
    mockHealthDataImpl = () => ({ derouter_version: "1.99.0" });
  ...
  it("should render a clickable version badge linking to the release notes", async () => {
    ...
    expect(versionLink).toHaveAttribute("href", "https://router.delify.vn/release_notes");
  ...
  it("should show the bouncing icon by default", async () => {
    ...
    renderWithProviders(<SidebarAccountMenu onLogout={mockOnLogout} />);
    await openMenu(user);
    expect(screen.getByTitle("Thanks for using DeRouter!")).toBeInTheDocument();
  });
  ...
  it("should hide the bouncing icon when Hide Bouncing Icon is enabled", async () => {
    ...
    renderWithProviders(<SidebarAccountMenu onLogout={mockOnLogout} />);
    await openMenu(user);
    expect(screen.queryByTitle("Thanks for using DeRouter!")).not.toBeInTheDocument();
  });
