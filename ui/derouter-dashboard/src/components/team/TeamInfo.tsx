...
const TeamInfoView: React.FC<TeamInfoProps> = ({
  ...
  const handleMemberCreate = async (values: any) => {
    try {
      ...
      if (error?.raw?.detail?.error?.includes("Assigning team admins is a premium feature")) {
        errMsg = "Assigning admins is an enterprise-only feature. Please upgrade your DeRouter plan to enable this.";
  ...
  const handleTeamUpdate = async (values: any) => {
    try {
      ...
      const previousModelAliases = info.derouter_model_table?.model_aliases ?? {};
  ...
  const globalGuardrails = allGuardrails.filter((g) => g.derouter_params?.default_on);
  const otherGuardrails = allGuardrails.filter((g) => !g.derouter_params?.default_on);
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            {canEditTeam && !isEditing && (
              <Button
                variant="outline"
                onClick={() => {
                  setTeamModelAliases(info.derouter_model_table?.model_aliases ?? {});
                  startEditing();
                }}
              >
                <Pencil />
          ...
          {isEditing && isGuardrailsLoading ? (
                ...
                <FieldGroup>
                  ...
                  <FormField
                    ...
                    label={labelWithDocsHint(
                      ...
                      "https://router.delify.vn/docs/proxy/guardrails/quick_start",
                    )}
                  >
                    {({ id, value, onChange }) => (
                      <GuardrailsSelect
                  ...
                  {canViewPolicies && (
                    <FormField
                      ...
                      label={labelWithDocsHint(
                        ...
                        "https://router.delify.vn/docs/proxy/guardrails/guardrail_policies",
                      )}
                    >
                      {({ id, value, onChange }) => (
                        <TagsInput
                  ...
                  >
                    {({ ref, value, ...field }) => (
                      <Textarea
                        ...
                        value={value ?? ""}
                        rows={6}
                        placeholder='{"namespace": "admin", "mount": "secret", "path_prefix": "derouter"}'
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                {(() => {
                  const aliasEntries = Object.entries(info.derouter_model_table?.model_aliases ?? {});
