...
export default function TeamMemberTab({
  ...
  const getUserBudget = (userId: string | null): number | null => {
    ...
    const membership = teamData.team_memberships.find((tm) => tm.user_id === userId);
    return membership?.derouter_budget_table?.max_budget ?? null;
  };
  ...
  const getUserRateLimits = (userId: string | null): string => {
    ...
    const membership = teamData.team_memberships.find((tm) => tm.user_id === userId);
    const rpmLimit = membership?.derouter_budget_table?.rpm_limit;
    const tpmLimit = membership?.derouter_budget_table?.tpm_limit;
  ...
  const getUserAllowedModels = (userId: string | null): string[] | null => {
    if (!userId) return null;
    const membership = teamData.team_memberships.find((tm) => tm.user_id === userId);
    const models = membership?.derouter_budget_table?.allowed_models;
  ...
  const getUserBudgetReset = (userId: string | null): string | null => {
    if (!userId) return null;
    const membership = teamData.team_memberships.find((tm) => tm.user_id === userId);
    return membership?.derouter_budget_table?.budget_reset_at ?? null;
  };
    ...
    <MemberTable
      ...
      onEdit={(record) => {
        ...
        const enhancedMember = {
          ...record,
          max_budget_in_team: membership?.derouter_budget_table?.max_budget ?? null,
          tpm_limit: membership?.derouter_budget_table?.tpm_limit ?? null,
          rpm_limit: membership?.derouter_budget_table?.rpm_limit ?? null,
          budget_duration: membership?.derouter_budget_table?.budget_duration || null,
          allowed_models: membership?.derouter_budget_table?.allowed_models || [],
        };
