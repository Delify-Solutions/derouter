...
vi.mock("./Navbar/CommunityEngagementButtons/CommunityEngagementButtons", () => ({
  CommunityEngagementButtons: () => (
    <div data-testid="community-engagement-buttons">
      <a href="https://www.router.delify.vn/support" target="_blank" rel="noopener noreferrer">
        Join Slack
      </a>
      <a href="https://github.com/Delify-Solutions/derouter" target="_blank" rel="noopener noreferrer">
        Star us on GitHub
      </a>
    </div>
  ),
...
describe("Navbar", () => {
  ...
  it("should link the logo to the UI home route rather than the proxy origin", () => {
    ...
    expect(screen.getByRole("link", { name: /derouter brand/i })).toHaveAttribute("href", "/ui");
  });

  it("pairs the logo with a dark-mode variant that swaps on the dark class", () => {
    renderWithProviders(<Navbar {...defaultProps} />);

    const [light, dark] = Array.from(screen.getByRole("link", { name: /derouter brand/i }).querySelectorAll("img"));
  ...
  it("should show version badge when health data contains version", () => {
    mockUseHealthReadinessDetailsImpl = () => ({ data: { derouter_version: "1.0.0" } });
  ...
  it("should use custom logo from theme context", () => {
    ...
    const logoImg = screen.getByAltText("DeRouter Brand");
