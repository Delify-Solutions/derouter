...
const buildCurlSnippet = (group: RoutingGroup, baseUrl: string): string =>
  ...
  -H 'Authorization: Bearer $DEROUTER_API_KEY' \\
  -d '{
...
const client = new OpenAI({
  apiKey: process.env.DEROUTER_API_KEY,
  baseURL: "${baseUrl}",
});
  ...
  return (
    <div className="border-y bg-muted/40 px-4 py-4">
      ...
      <p className="mb-3 text-sm text-muted-foreground">
        Callers request any model in the group by name; DeRouter picks a deployment behind the scenes using the{" "}
