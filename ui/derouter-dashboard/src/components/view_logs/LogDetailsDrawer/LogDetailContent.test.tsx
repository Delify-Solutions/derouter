...
describe("LogDetailContent", () => {
  ...
  it("should link the Response Cache tooltip to the response caching docs", async () => {
    ...
    expect(await screen.findByRole("link", { name: "Docs" })).toHaveAttribute(
      "href",
      "https://router.delify.vn/docs/proxy/caching",
    );
  });
  ...
  it("should link the prompt cache tooltips to the prompt caching docs", async () => {
    ...
    expect(await screen.findByRole("link", { name: "Docs" })).toHaveAttribute(
      "href",
      "https://router.delify.vn/docs/completion/prompt_caching",
    );
  });
  ...
  it("should display DeRouter Overhead when derouter_overhead_time_ms is in metadata", () => {
    render(
      <LogDetailContent
        logEntry={createLogEntry({
          metadata: {
    ...
    expect(screen.getByText("DeRouter Overhead")).toBeInTheDocument();
    expect(screen.getByText("42.50 ms")).toBeInTheDocument();
  });

  it("should not display DeRouter Overhead when derouter_overhead_time_ms is absent from metadata", () => {
    render(<LogDetailContent logEntry={createLogEntry({ metadata: { status: "success" } })} />);

    expect(screen.queryByText("DeRouter Overhead")).not.toBeInTheDocument();
  });
