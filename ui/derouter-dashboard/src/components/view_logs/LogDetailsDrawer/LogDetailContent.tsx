...
const RESPONSE_CACHE_TOOLTIP =
  "Whether this request was served from DeRouter's response cache (e.g. Redis / in-memory), skipping the LLM provider call entirely. This is separate from provider prompt caching; a Miss here does not mean prompt caching failed.";
const RESPONSE_CACHE_DOCS_URL = "https://router.delify.vn/docs/proxy/caching";
const CACHE_KEY_TOOLTIP =
  "The key DeRouter computed for this request in the response cache. Requests with the same cache key share a cached response; a different key means the request content did not match any cached entry.";
const PROMPT_CACHE_DOCS_URL = "https://router.delify.vn/docs/completion/prompt_caching";
      ...
      <Card size="sm" style={{ marginBottom: 0 }}>
        ...
        <CardContent>
          <DescriptionList>
            ...
            {metadata?.derouter_overhead_time_ms !== undefined && metadata.derouter_overhead_time_ms !== null && (
              <DescriptionItem label="DeRouter Overhead">
                {metadata.derouter_overhead_time_ms.toFixed(2)} ms
              </DescriptionItem>
            )}
