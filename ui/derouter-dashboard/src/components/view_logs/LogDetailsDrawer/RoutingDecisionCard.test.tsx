...
describe("RoutingDecisionCard", () => {
  ...
  it("shows the escalation keyword", () => {
    render(
      <RoutingDecisionCard decision={{ ...heuristic, escalated: true, escalation_keyword: "DEROUTER ESCALATE" }} />,
    );
    expect(screen.getByText('Yes, keyword "DEROUTER ESCALATE"')).toBeInTheDocument();
  });
  ...
  it("still shows the ask when escalation had nowhere higher to go", () => {
    ...
    // asked to escalate.
    render(
      <RoutingDecisionCard decision={{ ...heuristic, escalated: false, escalation_keyword: "DEROUTER ESCALATE" }} />,
    );
    expect(screen.getByText('Requested via "DEROUTER ESCALATE"; already at the highest tier')).toBeInTheDocument();
  });
