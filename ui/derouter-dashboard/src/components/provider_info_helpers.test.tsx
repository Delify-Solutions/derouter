...
describe("provider_info_helpers", () => {
  describe("getProviderLogoAndName", () => {
    ...
    it("should resolve the zai (Z.AI) provider value to the Z.AI display name", () => {
      // Regression test for https://github.com/Delify-Solutions/derouter/issues/25482 —
  ...
  describe("getProviderModels", () => {
    ...
    it("should return models matching the provider from modelMap", () => {
      const modelMap = {
        "gpt-3.5-turbo": { derouter_provider: "openai" },
        "gpt-4": { derouter_provider: "openai" },
        "claude-3-opus": { derouter_provider: "anthropic" },
      };
      const result = getProviderModels(Providers.OpenAI, modelMap);
      expect(result).toEqual(["gpt-3.5-turbo", "gpt-4"]);
    });

    it("should return models whose derouter_provider is a prefix-anchored variant of the provider", () => {
      const modelMap = {
        "anthropic-text-model": { derouter_provider: "anthropic_text" },
        "claude-3-opus": { derouter_provider: "anthropic" },
      };
    ...
    it("should not leak vertex_ai-anthropic_models into the Anthropic provider", () => {
      const modelMap = {
        "claude-3-opus": { derouter_provider: "anthropic" },
        "vertex_ai/claude-3-5-sonnet": { derouter_provider: "vertex_ai-anthropic_models" },
        "vertex_ai/claude-haiku-4-5": { derouter_provider: "vertex_ai-anthropic_models" },
      };
      const result = getProviderModels(Providers.Anthropic, modelMap);
    ...
    it("should not leak vertex_ai-openai_models into the OpenAI provider", () => {
      const modelMap = {
        "gpt-4": { derouter_provider: "openai" },
        "vertex_ai/openai-something": { derouter_provider: "vertex_ai-openai_models" },
      };
      const result = getProviderModels(Providers.OpenAI, modelMap);
    ...
    it("should include all vertex_ai variants when called with 'Vertex_AI' provider key", () => {
      const modelMap = {
        "vertex_ai/gemini-pro": { derouter_provider: "vertex_ai" },
        "vertex_ai/claude-3-5-sonnet": { derouter_provider: "vertex_ai-anthropic_models" },
        "vertex_ai/text-bison": { derouter_provider: "vertex_ai-text-models" },
        "vertex_ai_beta/something": { derouter_provider: "vertex_ai_beta" },
        "anthropic-native": { derouter_provider: "anthropic" },
      };
    ...
    it("should include bedrock converse but exclude standalone bedrock_mantle when called with 'Bedrock' provider key", () => {
      const modelMap = {
        "bedrock-base": { derouter_provider: "bedrock" },
        "bedrock-converse-model": { derouter_provider: "bedrock_converse" },
        "bedrock_mantle/openai.gpt-5.4": { derouter_provider: "bedrock_mantle" },
        "openai-model": { derouter_provider: "openai" },
      };
    ...
    it("should return only bedrock_mantle models when called with 'BedrockMantle' provider key", () => {
      ...
      const modelMap = {
        "bedrock_mantle/openai.gpt-oss-120b": { derouter_provider: "bedrock_mantle" },
        "bedrock_mantle/openai.gpt-5.5": { derouter_provider: "bedrock_mantle" },
        "bedrock-base": { derouter_provider: "bedrock" },
        "bedrock-converse-model": { derouter_provider: "bedrock_converse" },
      };
    ...
    it("should include fireworks_ai-embedding-models when called with 'FireworksAI' provider key", () => {
      const modelMap = {
        "fireworks-base": { derouter_provider: "fireworks_ai" },
        "fireworks-embed": { derouter_provider: "fireworks_ai-embedding-models" },
        "openai-model": { derouter_provider: "openai" },
      };
    ...
    it("should filter out models without derouter_provider property", () => {
      const modelMap = {
        "gpt-3.5-turbo": { derouter_provider: "openai" },
    ...
    it("should include both cohere and cohere_chat models for Cohere provider", () => {
      const modelMap = {
        "cohere-model-1": { derouter_provider: "cohere" },
        "cohere-model-2": { derouter_provider: "cohere_chat" },
        "other-model": { derouter_provider: "openai" },
      };
    ...
    it("should include sagemaker_chat models for SageMaker provider", () => {
      const modelMap = {
        "sagemaker-model-1": { derouter_provider: "sagemaker_chat" },
        "sagemaker-model-2": { derouter_provider: "sagemaker" },
        "other-model": { derouter_provider: "openai" },
      };
    ...
    it("should handle modelMap with non-object values", () => {
      const modelMap = {
        ...
        "number-model": 123,
        "valid-model": { derouter_provider: "openai" },
      };
      const result = getProviderModels(Providers.OpenAI, modelMap);
    ...
    it("should handle multiple providers correctly", () => {
      const modelMap = {
        "gpt-3.5-turbo": { derouter_provider: "openai" },
        "claude-3-opus": { derouter_provider: "anthropic" },
        "groq-model": { derouter_provider: "groq" },
      };
...
describe("getProviderLogoAndName under a custom server_root_path", () => {
  ...
  it("returns the bundled logo URL untouched under a sub-path mount", async () => {
    vi.resetModules();
    vi.doMock("@/lib/serverRootPath", () => ({ serverRootPath: "/derouter" }));
    ...
    expect(logo.startsWith("/derouter")).toBe(false);
  });
