...
export const pricingIssueUrl = (unpriced: UsageUnits, provider?: string): string => {
  ...
  const params = new URLSearchParams({
    ...
    "the-feature": `DeRouter has no price for these ${subject} usage units, so the Guardrails Monitor leaves them out of the cost: ${Object.keys(unpriced).join(", ")}`,
  });
  return `https://github.com/Delify-Solutions/derouter/issues/new?${params.toString()}`;
};
