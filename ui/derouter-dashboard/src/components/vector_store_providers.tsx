...
export enum VectorStoreProviders {
  ...
  PgVector = "PostgreSQL pgvector (DeRouter Connector)",
...
export const vectorStoreProviderFields: Record<string, VectorStoreFieldConfig[]> = {
  ...
  pg_vector: [
    {
      name: "api_base",
      label: "API Base",
      tooltip: "Enter the base URL of your deployed derouter-pgvector server (e.g., http://your-server:8000)",
    ...
    },
    {
      name: "api_key",
      label: "API Key",
      tooltip: "Enter the API key from your deployed derouter-pgvector server",
  ...
  mongodb: [
    ...
    },
    {
      ...
      label: "Embedding Model",
      tooltip:
        "The embedding model on this proxy that created the vectors already stored in your collection. DeRouter embeds every search query with it, so it must be the same model. A different model of the same size will not error, it will just return wrong results. Add it under Models first if it is not listed",
      placeholder: "text-embedding-3-small",
      required: true,
      type: "select",
    },
    ...
    },
    {
      ...
      label: "Text Field",
      tooltip:
        "The field in each document that holds its readable text. DeRouter returns this text in search results, and it accepts a dotted path such as metadata.body (default: text)",
      placeholder: "text",
      required: false,
      type: "text",
      initialValue: "text",
    },
    {
      ...
      tooltip:
        "How many nearest neighbours MongoDB examines before returning the top results. Higher is more accurate and slower. Leave blank to let DeRouter scale it with the requested result count",
      placeholder: "100",
      required: false,
      type: "text",
    },
  ...
  valkey: [
    ...
    },
    {
      ...
      label: "Embedding Model",
      tooltip:
        "The embedding model on this proxy that was used to create the embeddings already stored in your Valkey index. DeRouter uses it to embed each search query, so it must be the same model or results will be wrong. Add it under Models first if it is not listed",
      placeholder: "text-embedding-3-small",
      required: true,
      type: "select",
    },
    {
      ...
      label: "Text Field",
      tooltip:
        "The field in each stored document that holds its readable text. DeRouter returns this text in search results. Must match how your documents were stored (default: text)",
      placeholder: "text",
      required: false,
      type: "text",
      initialValue: "text",
    },
    {
      ...
      label: "Vector Field Name",
      tooltip:
        "The field in each stored document that holds its embedding. DeRouter searches against this field, so it must match the field your index was created on (default: embedding)",
      placeholder: "embedding",
      required: false,
      type: "text",
      initialValue: "embedding",
