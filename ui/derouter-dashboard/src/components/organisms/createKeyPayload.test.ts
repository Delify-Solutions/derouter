...
describe("metadata", () => {
  ...
  it("maps disabled callbacks from display names to internal names", () => {
    expect(payloadOf(build({ key_alias: "my-key" }, { disabledCallbacks: ["Langfuse"] }))).toStrictEqual(
      aliasOnly({ metadata: '{"derouter_disabled_callbacks":["langfuse"]}' }),
    );
  });
  ...
  it("keeps every metadata contributor in one object", () => {
    expect(
      ...
      duration: null,
      metadata:
        '{"team":"core","service_account_id":"svc-key","logging":[{"callback_name":"otel"}],"derouter_disabled_callbacks":["datadog"]}',
    });
  });
});
