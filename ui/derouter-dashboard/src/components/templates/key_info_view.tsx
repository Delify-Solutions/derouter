...
// Premium fields (from DeRouter_ManagementEndpoint_MetadataFields_Premium in
// derouter/proxy/_types.py) that the key-edit form submits as arrays/strings, where
  ...
  const handleKeyUpdate = async (formValues: Record<string, any>) => {
    try {
      ...
      const previousSoftBudget =
        (currentKeyData.derouter_budget_table as { soft_budget?: number | null } | null | undefined)?.soft_budget ??
        null;
        ...
        try {
          ...
          formValues.metadata = {
            ...
            ...(formValues.disabled_callbacks?.length > 0
              ? {
                  derouter_disabled_callbacks: mapDisplayToInternalNames(formValues.disabled_callbacks),
                }
              : {}),
          };
        } catch (error) {
  ...
  return (
    <div className="w-full h-full overflow-y-auto p-4">
      <KeyInfoHeader
        ...
        regenerateTooltip={
          !premiumUser ? "This is a DeRouter Enterprise feature, and requires a valid key to use." : undefined
        }
      />
          ...
          <TabsContent value="overview" keepMounted>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <LoggingSettingsView
                ...
                disabledCallbacks={
                  Array.isArray(currentKeyData.metadata?.derouter_disabled_callbacks)
                    ? mapInternalToDisplayNames(currentKeyData.metadata.derouter_disabled_callbacks)
                    : []
                }
                variant="card"
              />
