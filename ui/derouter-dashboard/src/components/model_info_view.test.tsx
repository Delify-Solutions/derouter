...
describe("ModelInfoView", () => {
  ...
  it("should pass model_info.id to disambiguate duplicate model_name deployments", async () => {
    ...
    // Signature: (accessToken, derouter_params, model_info, mode)
  ...
  it("keeps the edit form and its touched fields alive across a tab switch", async () => {
    ...
    expect(mockModelPatchUpdateCall.mock.calls[0][1].derouter_params.input_cost_per_token).toBeCloseTo(5 / 1_000_000);
  });
  ...
  it("should display model information in overview tab", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText("DeRouter Model")).toBeInTheDocument();
  ...
  it("should display form fields in edit mode", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByPlaceholderText("Enter DeRouter model name")).toBeInTheDocument();
    });
  });
  ...
  it("should display DeRouter Params section", async () => {
    render(<ModelInfoView {...DEFAULT_ADMIN_PROPS} />, { wrapper });
    await waitFor(() => {
      expect(screen.getByText("DeRouter Params")).toBeInTheDocument();
    });
  });
  ...
  it("should keep selector credential and ignore derouter_credential_name from DeRouter Params json", async () => {
    const user = userEvent.setup();
    render(<ModelInfoView {...DEFAULT_ADMIN_PROPS} />, { wrapper });
    ...
    const derouterParamsInput = screen
      .getAllByRole("textbox")
      .find(
        (input) =>
          input.tagName === "TEXTAREA" && (input as HTMLTextAreaElement).value.includes('"custom_llm_provider"'),
      );
    expect(derouterParamsInput).toBeDefined();
    if (!derouterParamsInput) {
      return;
    }
    expect((derouterParamsInput as HTMLTextAreaElement).value).not.toContain("derouter_credential_name");
    await user.clear(derouterParamsInput);
    await user.paste(`{"derouter_credential_name":"from-json","timeout":42}`);
    ...
    expect(updatePayload.derouter_params.derouter_credential_name).toBe("selected-credential");
    expect(updatePayload.derouter_params.derouter_credential_name).not.toBe("from-json");
  });
  ...
  it("should not include vector_store_ids in update payload when model has none", async () => {
    ...
    // vector_store_ids: [] into derouter_params, which then propagated to
    ...
    const updatePayload = mockModelPatchUpdateCall.mock.calls[0][1];
    expect(updatePayload.derouter_params).not.toHaveProperty("vector_store_ids");
  });
  ...
  describe("PTU cost attribution gate", () => {
    const ptuModelData = {
      ...
      derouter_params: { ...defaultModelData.derouter_params, input_cost_per_token: 0, output_cost_per_token: 0 },
      model_info: {
        ...defaultModelData.model_info,
        team_id: "team-1",
  ...
  it("blocks the save when the DeRouter Params box does not hold valid JSON", async () => {
    const user = userEvent.setup();
    render(<ModelInfoView {...DEFAULT_ADMIN_PROPS} />, { wrapper });
  ...
  it("should not include input_cost_per_token or output_cost_per_token in update payload when user does not touch cost fields", async () => {
    ...
    // input_cost_per_token: 0 and output_cost_per_token: 0 into derouter_params,
    ...
    const updatePayload = mockModelPatchUpdateCall.mock.calls[0][1];
    expect(updatePayload.derouter_params).not.toHaveProperty("input_cost_per_token");
    expect(updatePayload.derouter_params).not.toHaveProperty("output_cost_per_token");
  });
  ...
  it("never re-sends a masked secret on save (regression: masked auth value must not overwrite the real secret)", async () => {
    ...
    // A plain save re-PATCHes the whole derouter_params blob; if the masked value were
    ...
    const updatePayload = mockModelPatchUpdateCall.mock.calls[0][1];
    expect(updatePayload.derouter_params.azure_ad_token).not.toBe(maskedSecret);
    // No masked value may appear anywhere in the outbound params.
    expect(JSON.stringify(updatePayload.derouter_params)).not.toContain("**");
  });
  ...
  it("should display health check model field for wildcard models", async () => {
    const wildcardModelData = {
      ...defaultModelData,
      derouter_params: {
        ...defaultModelData.derouter_params,
        model: "openai/gpt-4*",
      },
    };
  ...
  // marker — init_complexity_router_deployment (derouter/router.py) never reads it, falling back
  // to tier-derivation instead when derouter_params.complexity_router_default_model is absent.
  // Probing the blob field here would test a model the running router never calls.
  it("ignores an unused config blob pin when derouter_params has no default, matching the backend's own tier-derivation fallback", async () => {
    const complexityRouterModelData = {
      ...defaultModelData,
      derouter_params: {
        ...defaultModelData.derouter_params,
  ...
  describe("Edit Auto Router affordance", () => {
    const withRouter = (derouterParams: Record<string, unknown>) => {
      mockUseModelsInfo.mockReturnValue({
        data: { data: [{ ...defaultModelData, derouter_params: { ...derouterParams } }] },
        isLoading: false,
        error: null,
      });
    };
  ...
  describe("auto-router header actions", () => {
    const withParams = (derouterParams: Record<string, unknown>) => {
      mockUseModelsInfo.mockReturnValue({
        data: { data: [{ ...defaultModelData, derouter_params: { ...derouterParams } }] },
        isLoading: false,
        error: null,
  ...
  describe("payload parity pins", () => {
    ...
    const save = async (user: ReturnType<typeof userEvent.setup>) => {
      ...
      return mockModelPatchUpdateCall.mock.calls[0][1] as {
        model_name: string;
        derouter_params: Record<string, unknown>;
    ...
    it("routes each edited field into its own payload key", async () => {
      ...
      await user.clear(screen.getByPlaceholderText("Enter DeRouter model name"));
      await user.type(screen.getByPlaceholderText("Enter DeRouter model name"), "gpt-4o");
      ...
      expect(payload.derouter_params).toMatchObject({
        model: "gpt-4o",
    ...
    it("routes each typed collection field into its own payload key", async () => {
      ...
      expect(payload.derouter_params.guardrails).toEqual(["content_filter"]);
      expect(payload.derouter_params.tags).toEqual(["production_tag"]);
    });
    ...
    it("sends the edited DeRouter extra params", async () => {
      const user = userEvent.setup();
      await enterEditMode(user);
      ...
      expect(payload.derouter_params.drop_params).toBe(true);
    });
    ...
    it("sends the credential picked in the selector", async () => {
      ...
      expect(payload.derouter_params.derouter_credential_name).toBe("other-credential");
    });
    ...
    it("sends the vector stores picked in the knowledge base selector", async () => {
      ...
      expect(payload.derouter_params.vector_store_ids).toEqual(["vs-beta"]);
    });

    it("sends the health check model picked for a wildcard deployment", async () => {
      const wildcard = {
        ...defaultModelData,
        derouter_params: { ...defaultModelData.derouter_params, model: "openai/gpt-4*" },
      };
    ...
    it("keeps a pricing field in the payload after the operator types a value and restores the original", async () => {
      ...
      expect(payload.derouter_params.input_cost_per_token).toBe(0.00003);
      expect(payload.derouter_params.cache_read_input_token_cost).toBe(0.00003);
    });
    ...
    it("clears a pricing override with an explicit null once the field is emptied", async () => {
      ...
      expect(payload.derouter_params.input_cost_per_token).toBeNull();
      expect(payload.derouter_params).not.toHaveProperty("cache_read_input_token_cost");
    });
    ...
    describe("cache control injection points", () => {
      const withCachePoints = (points: unknown) => {
        const data = {
          ...defaultModelData,
          derouter_params: { ...defaultModelData.derouter_params, cache_control_injection_points: points },
        };
      ...
      it("omits the key when the deployment has none and the operator leaves the toggle alone", async () => {
        ...
        expect(payload.derouter_params).not.toHaveProperty("cache_control_injection_points");
      });
      ...
      it("round-trips the stored injection points on an untouched save", async () => {
        ...
        expect(payload.derouter_params.cache_control_injection_points).toEqual([{ location: "message", role: "user" }]);
      });
      ...
      it("adds a typed index as a string, matching what the deployment already stores", async () => {
        ...
        expect(payload.derouter_params.cache_control_injection_points).toEqual([{ location: "message", index: "2" }]);
      });
    });
    ...
    it("carries an edited input cost and the model identifier onto the wire", async () => {
      ...
      expect(payload.derouter_params.input_cost_per_token).toBe(5 / 1_000_000);
    });
    ...
    it.fails(
      ...
      async () => {
        ...
        expect(payload).toStrictEqual({ derouter_params: { input_cost_per_token: 5 / 1_000_000 } });
      },
    );
