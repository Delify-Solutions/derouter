...
export interface ModelEditFormValues {
  ...
  derouter_credential_name?: string;
  derouter_extra_params?: string;
  model_info?: string;
}
...
const modelEditShape = {
  ...
  derouter_credential_name: textish,
  derouter_extra_params: textish,
  model_info: textish,
};
...
const buildSchema = (ptuEnabled: boolean, isFieldTouched: (field: TouchedPricingField) => boolean) =>
  z.object(modelEditShape).superRefine((values, ctx) => {
    ...
    if (values.derouter_extra_params && !isJson(values.derouter_extra_params)) {
      reject("derouter_extra_params", "Please enter valid JSON");
    }
...
export const toModelEditFormValues = (localModelData: any, isWildcardModel: boolean): ModelEditFormValues => ({
  ...
  derouter_model_name: localModelData.derouter_model_name,
  api_base: localModelData.derouter_params.api_base,
  custom_llm_provider: localModelData.derouter_params.custom_llm_provider,
  organization: localModelData.derouter_params.organization,
  tpm: localModelData.derouter_params.tpm,
  rpm: localModelData.derouter_params.rpm,
  max_retries: localModelData.derouter_params.max_retries,
  timeout: localModelData.derouter_params.timeout,
  stream_timeout: localModelData.derouter_params.stream_timeout,
  input_cost: perMillionTokens(
    localModelData.derouter_params.input_cost_per_token,
    localModelData.model_info?.input_cost_per_token,
  ),
  output_cost: perMillionTokens(
    localModelData.derouter_params?.output_cost_per_token,
  ...
  cache_read_cost: perMillionTokens(
    localModelData.derouter_params?.cache_read_input_token_cost,
    localModelData.model_info?.cache_read_input_token_cost,
  ),
  cache_write_cost: perMillionTokens(
    localModelData.derouter_params?.cache_creation_input_token_cost,
    localModelData.model_info?.cache_creation_input_token_cost,
  ),
  cache_control: localModelData.derouter_params?.cache_control_injection_points ? true : false,
  cache_control_injection_points: localModelData.derouter_params?.cache_control_injection_points || [],
  model_access_group: Array.isArray(localModelData.model_info?.access_groups)
    ? localModelData.model_info.access_groups
    : [],
  guardrails: Array.isArray(localModelData.derouter_params?.guardrails) ? localModelData.derouter_params.guardrails : [],
  vector_store_ids:
    Array.isArray(localModelData.derouter_params?.vector_store_ids) &&
    localModelData.derouter_params.vector_store_ids.length > 0
      ? localModelData.derouter_params.vector_store_ids
      : undefined,
  tags: Array.isArray(localModelData.derouter_params?.tags) ? localModelData.derouter_params.tags : [],
  ...
  derouter_credential_name: localModelData.derouter_params?.derouter_credential_name || "",
  derouter_extra_params: JSON.stringify(
    Object.fromEntries(
      Object.entries(localModelData.derouter_params || {}).filter(
        ([key, value]) => key !== "derouter_credential_name" && !isMaskedSecret(value),
      ),
    ),
    null,
    2,
...
const displayCost = (localModelData: any, field: TouchedPricingField): string => {
  ...
  const rate = localModelData?.derouter_params?.[param] ?? localModelData?.model_info?.[info];
        ...
        <div className="space-y-4">
          <div className="space-y-4">
            ...
            {textField(
              "derouter_model_name",
              "DeRouter Model Name",
              "Enter DeRouter model name",
              localModelData.derouter_model_name,
            )}
            ...
            {textField("api_base", "API Base", "Enter API base", localModelData.derouter_params?.api_base)}
            {textField(
              "custom_llm_provider",
              "Custom LLM Provider",
              "Enter custom LLM provider",
              localModelData.derouter_params?.custom_llm_provider,
            )}
            {textField(
              "organization",
              "Organization",
              "Enter organization",
              localModelData.derouter_params?.organization,
            )}

            {numberField("tpm", "TPM (Tokens per Minute)", "Enter TPM", localModelData.derouter_params?.tpm)}
            {numberField("rpm", "RPM (Requests per Minute)", "Enter RPM", localModelData.derouter_params?.rpm)}
            {numberField("max_retries", "Max Retries", "Enter max retries", localModelData.derouter_params?.max_retries)}
            {numberField("timeout", "Timeout (seconds)", "Enter timeout", localModelData.derouter_params?.timeout)}
            {numberField(
              "stream_timeout",
              "Stream Timeout (seconds)",
              "Enter stream timeout",
              localModelData.derouter_params?.stream_timeout,
            )}
            ...
            <div>
              <FieldLabel>
                ...
                <DocsHint
                  ...
                  href="https://router.delify.vn/docs/proxy/guardrails/quick_start"
                />
              </FieldLabel>
              {isEditing ? (
                tagsField(
                ...
                <Display>
                  <ChipList values={localModelData.derouter_params?.guardrails} emptyLabel="No guardrails assigned" />
                </Display>
              )}
            </div>
            ...
            <div>
              <FieldLabel>
                ...
                <DocsHint
                  ...
                  href="https://router.delify.vn/docs/completion/knowledgebase"
                />
              </FieldLabel>
              {isEditing ? (
                ...
                <Display>
                  <ChipList
                    values={localModelData.derouter_params?.vector_store_ids}
            ...
            <div>
              ...
              {isEditing ? (
                ...
                <Display>
                  <ChipList values={localModelData.derouter_params?.tags} emptyLabel="No tags assigned" />
                </Display>
              )}
            </div>
            ...
            <div>
              ...
              {isEditing ? (
                <FormField control={form.control} name="derouter_credential_name">
                ...
                <Display>{localModelData.derouter_params?.derouter_credential_name || "Manual"}</Display>
              )}
            </div>
            ...
            {isEditing ? (
              ...
              <div>
                ...
                <Display>
                  {localModelData.derouter_params?.cache_control_injection_points ? (
                    <div>
                      <p>Enabled</p>
                      <div className="mt-2">
                        {localModelData.derouter_params.cache_control_injection_points.map((point: any, i: number) => (
            ...
            <div>
              <FieldLabel>
                DeRouter Params
                <DocsHint
                  text="Optional derouter params used for making a derouter.completion() call. Some params are automatically added by DeRouter."
                  href="https://router.delify.vn/docs/completion/input"
                />
              </FieldLabel>
              {isEditing ? (
                <FormField control={form.control} name="derouter_extra_params">
                  {({ value, ...control }) => (
                    <Textarea
                      {...control}
                      value={(value as string) ?? ""}
                ...
                <Display>
                  <pre className="mt-1 overflow-auto rounded-sm bg-muted p-2 text-xs">
                    {JSON.stringify(localModelData.derouter_params, null, 2)}
                  </pre>
                </Display>
              )}
            </div>
