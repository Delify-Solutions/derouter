    ...
    mockGetCallbacksCall.mockResolvedValue({
      ...
      available_callbacks: {
        langfuse: {
          ...
          derouter_callback_params: ["LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "LANGFUSE_HOST"],
    ...
    mockGetCallbacksCall.mockResolvedValue({
      ...
      available_callbacks: {
        otel: {
          ...
          derouter_callback_params: ["OTEL_EXPORTER", "OTEL_EXPORTER_OTLP_PROTOCOL", "OTEL_ENDPOINT", "OTEL_HEADERS"],
  ...
  it("should show the saved export protocol in the edit modal and keep it on an unchanged save", async () => {
    ...
    await waitFor(() => {
      expect(vi.mocked(setCallbacksCall)).toHaveBeenCalledWith("token", {
        ...
        derouter_settings: { success_callback: ["otel"] },
      });
    });
  });
