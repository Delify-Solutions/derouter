...
const buildComplexityRouterTestTargets = (
  ...
  const rawConfig = modelData?.derouter_params?.complexity_router_config;
  ...
  // Mirrors init_complexity_router_deployment (derouter/router.py): derouter_params wins, otherwise
  ...
  const effectiveDefaultModel = modelData?.derouter_params?.complexity_router_default_model || undefined;
...
export default function ModelInfoView({
  ...
  const getProviderFromModel = (model: string) => {
    if (modelCostMapData !== null && modelCostMapData !== undefined) {
      if (typeof modelCostMapData == "object" && model in modelCostMapData) {
        return modelCostMapData[model]["derouter_provider"];
      }
    }
    return "openai";
  };
  ...
  const isAutoRouterModel = hasAutoRouterEditor(modelData?.derouter_params);
  ...
  const isAnyAutoRouter = isAutoRouterDeployment(modelData?.derouter_params);
  const deleteLabel = isAnyAutoRouter ? "Delete Auto-Router" : "Delete Model";
  const isComplexityRouterModel = isComplexityRouterParams(modelData?.derouter_params);

  const usingExistingCredential =
    modelData?.derouter_params?.derouter_credential_name != null &&
    modelData?.derouter_params?.derouter_credential_name != undefined;

  // Initialize localModelData from modelData when available
  useEffect(() => {
    if (modelData && !localModelData) {
      let processedModelData = modelData;
      if (!processedModelData.derouter_model_name) {
        processedModelData = {
          ...processedModelData,
          derouter_model_name:
            processedModelData?.derouter_params?.derouter_model_name ??
            processedModelData?.derouter_params?.model ??
      ...
      if (processedModelData?.derouter_params?.cache_control_injection_points) {
  ...
  useEffect(() => {
    ...
    const getModelInfo = async () => {
      ...
      if (specificModelData && !specificModelData.derouter_model_name) {
        specificModelData = {
          ...specificModelData,
          derouter_model_name:
            specificModelData?.derouter_params?.derouter_model_name ??
            specificModelData?.derouter_params?.model ??
      ...
      // Check if cache control is enabled
      if (specificModelData?.derouter_params?.cache_control_injection_points) {
        setShowCacheControl(true);
      }
    };
  ...
  const handleModelUpdate = async (
    ...
    try {
      ...
      // Parse DeRouter extra params from JSON text area
      let parsedExtraParams: Record<string, any> = {};
      try {
        parsedExtraParams = values.derouter_extra_params ? JSON.parse(values.derouter_extra_params) : {};
        delete parsedExtraParams.derouter_credential_name;
      } catch (e) {
        toast.fromError("Invalid JSON in DeRouter Params");
        setIsSaving(false);
        return;
      }
      ...
      let updatedLitellmParams: Record<string, any> = {
        ...parsedExtraParams,
        model: values.derouter_model_name,
      ...
      if (values.derouter_credential_name) {
        updatedLitellmParams.derouter_credential_name = values.derouter_credential_name;
      } else {
        delete updatedLitellmParams.derouter_credential_name;
      }
      ...
      const updateData = {
        ...
        derouter_params: safeLitellmParams,
      ...
      const updatedModelData = {
        ...localModelData,
        model_name: values.model_name,
        derouter_model_name: values.derouter_model_name,
        derouter_params: safeLitellmParams,
        model_info: updatedModelInfo,
      };
    ...
    try {
      ...
      const response = await testConnectionRequest(
        accessToken,
        {
          ...
          model: localModelData.derouter_model_name,
        },
        {
  ...
  const isWildcardModel = modelData.derouter_model_name.includes("*");
  const wildcardProvider = modelData.derouter_model_name.split("/")[0];
  const healthCheckModelOptions =
    modelHubData?.data
      ?.filter(
        (model: any) =>
          model.providers?.includes(wildcardProvider) && model.model_group !== modelData.derouter_model_name,
      )
    ...
    <div className="p-4">
        ...
        <div>
          <TabsContent value="overview" keepMounted>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
              ...
              <Card className="block p-6">
                <p className="text-sm">DeRouter Model</p>
                <div className="mt-2 overflow-hidden">
                  <SimpleTooltip content={modelData.derouter_model_name || "Not Set"} className="w-full min-w-0">
                    <div className="break-all text-sm font-medium leading-relaxed cursor-pointer">
                      {modelData.derouter_model_name || "Not Set"}
                    </div>
                  </SimpleTooltip>
                </div>
              </Card>
      ...
      <DeleteResourceModal
        ...
        resourceInformation={[
          ...
          {
            label: "DeRouter Model Name",
            value: modelData?.derouter_model_name || "Not Set",
          },
          {
            label: "Provider",
      ...
      {isCredentialModalOpen && !usingExistingCredential ? (
        ...
        <Dialog open={isCredentialModalOpen} onOpenChange={(open) => !open && setIsCredentialModalOpen(false)}>
          <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto">
            ...
            <p className="text-sm">{modelData.derouter_params.derouter_credential_name}</p>
            <DialogFooter>
