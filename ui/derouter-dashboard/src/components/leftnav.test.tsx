...
describe("Sidebar (leftnav)", () => {
  ...
  it("should link the logo to the UI home route rather than the proxy origin", () => {
    ...
    expect(screen.getByRole("link", { name: /derouter home/i })).toHaveAttribute("href", "/ui");
  });

  it("pairs the logo with a dark-mode variant that swaps on the dark class", () => {
    renderWithProviders(<Sidebar {...defaultProps} />);

    const [light, dark] = Array.from(screen.getByRole("link", { name: /derouter home/i }).querySelectorAll("img"));
  ...
  it("falls back to the light logo when a configured dark logo fails to load", () => {
    ...
    const [, dark] = Array.from(screen.getByRole("link", { name: /derouter home/i }).querySelectorAll("img"));
