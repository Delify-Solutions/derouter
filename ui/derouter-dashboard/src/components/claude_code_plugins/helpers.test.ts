...
describe("formatInstallCommand", () => {
  it("produces a /plugin install command scoped to the derouter marketplace", () => {
    expect(formatInstallCommand({ name: "my-plugin" })).toBe("/plugin install my-plugin@derouter");
  });

  it("uses the plugin name as the identifier", () => {
    expect(formatInstallCommand({ name: "code-review" })).toBe("/plugin install code-review@derouter");
  });
});
