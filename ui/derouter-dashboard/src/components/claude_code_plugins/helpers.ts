 ...
 * "derouter" to match the name the proxy returns in marketplace.json.
 */
 ...
 * Installs the named plugin from the "derouter" marketplace registered in settings.json.
 */
export const formatInstallCommand = (plugin: { name: string }): string => `/plugin install ${plugin.name}@derouter`;
