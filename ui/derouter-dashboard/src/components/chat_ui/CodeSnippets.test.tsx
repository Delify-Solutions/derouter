...
describe("CodeSnippets", () => {
  ...
  describe("base URL selection", () => {
    it("should use DEROUTER_UI_API_DOC_BASE_URL when provided", () => {
      const customBaseUrl = "https://custom-doc.example.com";
      const code = generateCodeSnippet({
        ...baseParams,
        proxySettings: {
          DEROUTER_UI_API_DOC_BASE_URL: customBaseUrl,
        },
      });
      expect(code).toContain(`base_url="${customBaseUrl}"`);
    });

    it("should use PROXY_BASE_URL when DEROUTER_UI_API_DOC_BASE_URL is not provided", () => {
    ...
    it("should prioritize DEROUTER_UI_API_DOC_BASE_URL over PROXY_BASE_URL when both are provided", () => {
      const customBaseUrl = "https://custom-doc.example.com";
      const proxyBaseUrl = "https://proxy.example.com";
      const code = generateCodeSnippet({
        ...baseParams,
    ...
    it("should fallback to PROXY_BASE_URL when DEROUTER_UI_API_DOC_BASE_URL is empty string", () => {
      const proxyBaseUrl = "https://proxy.example.com";
      const code = generateCodeSnippet({
        ...baseParams,
        proxySettings: {
          DEROUTER_UI_API_DOC_BASE_URL: "",
          PROXY_BASE_URL: proxyBaseUrl,
        },
      });
      expect(code).toContain(`base_url="${proxyBaseUrl}"`);
    ...
    it("should fallback to PROXY_BASE_URL when DEROUTER_UI_API_DOC_BASE_URL is whitespace only", () => {
      const proxyBaseUrl = "https://proxy.example.com";
      const code = generateCodeSnippet({
        ...baseParams,
        proxySettings: {
    ...
    it("should fallback to window.location.origin when DEROUTER_UI_API_DOC_BASE_URL is null", () => {
      const code = generateCodeSnippet({
        ...baseParams,
        proxySettings: {
          DEROUTER_UI_API_DOC_BASE_URL: null,
        },
      });
      expect(code).toContain(`base_url="http://localhost:4000"`);
    });

    it("should use DEROUTER_UI_API_DOC_BASE_URL for Azure SDK", () => {
      const customBaseUrl = "https://custom-doc.example.com";
      const code = generateCodeSnippet({
        ...baseParams,
        selectedSdk: "azure",
    ...
    it("should use PROXY_BASE_URL for Azure SDK when DEROUTER_UI_API_DOC_BASE_URL is not provided", () => {
      const proxyBaseUrl = "https://proxy.example.com";
      const code = generateCodeSnippet({
        ...baseParams,
        selectedSdk: "azure",
