...
export async function makeOpenAIChatCompletionRequest(
  ...
  if (tags && tags.length > 0) {
    headers["x-derouter-tags"] = tags.join(",");
  }
        ...
        selectedMCPServers.forEach((serverId) => {
          if (serverId.startsWith("toolset:")) {
            ...
            tools.push({
              ...
              server_url: `derouter_proxy/mcp/${encodeURIComponent(toolsetName)}`,
              require_approval: "never",
            });
          } else {
            ...
            tools.push({
              type: "mcp",
              server_label: "derouter",
              server_url: `derouter_proxy/mcp/${serverName}`,
              require_approval: "never",
    ...
    const response: AsyncIterable<ChatCompletionChunk> | ChatCompletionChunk[] = streamingEnabled
      ...
      : await (async () => {
          const nonStreamingResponse = await client.chat.completions
            ...
            .withResponse();
          servedFromResponseCache = nonStreamingResponse.response.headers.get("x-derouter-cache-key") !== null;
