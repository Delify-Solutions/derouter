...
describe("chat_completion", () => {
  ...
  it("should configure MCP tools per server with restrictions", async () => {
    ...
    expect(firstTool.server_label).toBe("derouter");
    expect(firstTool.server_url).toBe("derouter_proxy/mcp/alpha");
    ...
    expect(secondTool.server_label).toBe("derouter");
    expect(secondTool.server_url).toBe("derouter_proxy/mcp/Beta");
  ...
  it("flags a non-streaming response-cache hit even though it replays provider prompt-cache usage", async () => {
    mockCreate.mockReturnValueOnce(
      nonStreamingResponse(
        ...
        { "x-derouter-cache-key": "cache-key-abc" },
      ),
    );
