...
/**
 * Build the derouter_proxy MCP reference blocks for a playground request.
 *
  ...
  return selectedMCPServers.map((serverId) => {
    if (serverId.startsWith(TOOLSET_PREFIX)) {
      ...
      return {
        ...
        server_url: `derouter_proxy/mcp/${toolsetName}`,
        require_approval: "never",
      };
    }
    ...
    return {
      ...
      server_url: `derouter_proxy/mcp/${routeName}`,
      require_approval: "never",
