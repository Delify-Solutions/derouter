...
vi.mock("@/lib/serverRootPath", () => ({ serverRootPath: "/derouter" }));
...
describe("Logo", () => {
  ...
  it("resolves a backend asset path through the server root path in src mode", () => {
    ...
    expect(img).toHaveAttribute("src", "/derouter/ui/assets/logos/github.svg");
  });
  ...
  it("swaps to the letter avatar and warns with the failing URL on image error", () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("/derouter/ui/assets/logos/github.svg"));
    warnSpy.mockRestore();
  });
