          ...
          <form onSubmit={(event) => event.preventDefault()} noValidate style={{ marginTop: 16 }}>
            <FieldGroup>
              <FormField
                ...
                description="Used in URLs and config. No spaces. E.g. derouter-platform-plugin"
              >
                {({ ref, ...field }) => <Input {...field} ref={ref} placeholder="derouter-platform-plugin" />}
              </FormField>
              ...
              <FormField
                ...
                name="plugin_key"
                label="Plugin Key"
                description="Optional. The plugin's own credential, injected as Authorization: Bearer <key> only when derouter reverse-proxies API calls to the plugin's backend (/plugin-proxy/<name>/*). Leave blank for plugins that use the forwarded derouter user token (e.g. iframe plugins) — that path uses the user's token, not this key."
              >
                {({ ref, ...field }) => (
                  <InputGroup>
                    <InputGroupInput
