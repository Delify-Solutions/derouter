...
describe("daily activity helpers", () => {
  ...
  it("always includes exclude_team_ids but only adds team_ids when given", async () => {
    ...
    expect(urlWithoutTeams.searchParams.get("exclude_team_ids")).toBe("derouter-dashboard");
    ...
    expect(urlWithTeams.searchParams.get("exclude_team_ids")).toBe("derouter-dashboard");
  });
});
...
describe("UI config and public endpoints", () => {
  ...
  it("should use proxyBaseURL and server_root_path for /public/providers/fields when server_root_path is defined", async () => {
    ...
    const mockFetch = setupMockFetch([
      { url: "/derouter/.well-known/derouter-ui-config", data: uiConfig },
  ...
  it("should return UI config from getUiConfig", async () => {
    ...
    const mockFetch = setupMockFetch([{ url: "/derouter/.well-known/derouter-ui-config", data: uiConfig }]);
    ...
    const configCall = mockFetch.mock.calls.find((call) =>
      (call[0] as string).includes("/derouter/.well-known/derouter-ui-config"),
    );
  ...
  it("updates serverRootPath so path-based nav links carry the root path", async () => {
    const uiConfig = {
      server_root_path: "/derouter",
      proxy_base_url: "https://example.com",
    };

    setupMockFetch([{ url: "/derouter/.well-known/derouter-ui-config", data: uiConfig }]);

    await Networking.getUiConfig();

    expect(Networking.serverRootPath).toBe("/derouter");
    expect(uiHref("api-reference")).toBe("/derouter/ui/api-reference");
  });
});
...
describe("buildModelGroupTestRequest", () => {
  it("builds a chat completion request with NO max_tokens (reasoning models 400 on a tiny cap)", () => {
    ...
    expect(body).toEqual({ model: "o3", messages: [{ role: "user", content: "test from derouter" }] });
  ...
  it("builds an embeddings request for embedding mode", () => {
    ...
    expect(body).toEqual({ model: "text-embedding-3-small", input: "test from derouter" });
  });
...
describe("testMCPToolsListRequest auth headers", () => {
  ...
  it("sends the derouter key under a custom derouter_key_header_name even when an upstream OAuth token uses Authorization", async () => {
    Networking.setGlobalLitellmHeaderName("x-derouter-key");
    ...
    expect(headers["x-derouter-key"]).toBe("Bearer sk-key");
    expect(headers["Authorization"]).toBe("Bearer upstream-oauth-token");
  });

  it("Bearer-prefixes x-derouter-api-key when it is the configured key header (raw values fail _get_bearer_token)", async () => {
    Networking.setGlobalLitellmHeaderName("x-derouter-api-key");
    const mockFetch = captureFetch();

    await Networking.testMCPToolsListRequest("sk-key", {}, "upstream-oauth-token");

    const headers = sentHeaders(mockFetch);
    expect(headers["x-derouter-api-key"]).toBe("Bearer sk-key");
    expect(headers["Authorization"]).toBe("Bearer upstream-oauth-token");
  });
  ...
  it("never clobbers the upstream OAuth token on default deployments", async () => {
    ...
    const headers = sentHeaders(mockFetch);
    expect(headers["Authorization"]).toBe("Bearer upstream-oauth-token");
    expect(headers["x-derouter-api-key"]).toBe("sk-key");
  });

  it("sends the derouter key as the bearer on default deployments without an OAuth token", async () => {
    const mockFetch = captureFetch();
