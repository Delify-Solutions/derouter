...
describe("resolveApiBase", () => {
  describe("same-origin (no explicit base)", () => {
    ...
    it("prefixes a relative base with the server root path", () => {
      expect(resolveApiBase({ serverRootPath: "/derouter" })).toBe("/derouter");
    });
  });
  ...
  describe("explicit base (split-origin / dev)", () => {
    ...
    it("appends the server root path to the explicit base", () => {
      expect(resolveApiBase({ explicitBase: "http://localhost:4000", serverRootPath: "/derouter" })).toBe(
        "http://localhost:4000/derouter",
      );
    });
  });
  ...
  describe("dedup — base already includes the root path", () => {
    it("does not double the root path", () => {
      expect(resolveApiBase({ explicitBase: "http://localhost:4000/derouter", serverRootPath: "/derouter" })).toBe(
        "http://localhost:4000/derouter",
      );
    });
  });
  ...
  describe("multi-segment root paths", () => {
    it("appends a nested root path", () => {
      expect(resolveApiBase({ explicitBase: "http://localhost:4000", serverRootPath: "/team/derouter" })).toBe(
        "http://localhost:4000/team/derouter",
      );
    });

    it("dedups a nested root path", () => {
      expect(
        resolveApiBase({ explicitBase: "http://localhost:4000/team/derouter", serverRootPath: "/team/derouter" }),
      ).toBe("http://localhost:4000/team/derouter");
    });
  });
});
