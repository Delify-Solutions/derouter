...
describe("logoTreatmentFor", () => {
  ...
  it("resolves through a bundler fingerprint in the filename", () => {
    expect(logoTreatmentFor("/derouter-asset-prefix/_next/static/media/openrouter.1xk7748-_jixf.svg")).toBe("invert");
  });

  it("resolves a bundled asset served under a proxy root path", () => {
    expect(logoTreatmentFor("/derouter/ui/assets/logos/notion.svg")).toBe("invert");
  });
