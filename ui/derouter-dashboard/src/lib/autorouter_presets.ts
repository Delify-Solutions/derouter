...
export const deploymentRefsFromModelInfo = (
  rows: readonly {
    ...
    derouter_params?: { model?: string | null; base_model?: string | null } | null;
    model_info?: { base_model?: string | null } | null;
  }[],
): DeploymentModelRef[] =>
  rows.flatMap((row) => {
    const underlyingModels = [
      row.derouter_params?.model,
      row.derouter_params?.base_model,
  ...
  const resolveParamKeys = (params: TierModelParamsByTier | undefined): TierModelParamsByTier | undefined =>
    ...
    Object.fromEntries(
      Object.entries(params).map(([tier, byModel]) => [
        tier,
        Object.entries(byModel).reduce<Record<string, TierModelParams>>((byResolved, [model, derouterParams]) => {
          const resolved = resolve(model);
          return { ...byResolved, [resolved]: { ...byResolved[resolved], ...derouterParams } };
        }, {}),
      ]),
    );
