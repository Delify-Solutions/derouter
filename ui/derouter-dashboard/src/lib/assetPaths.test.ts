...
describe("withServerRoot", () => {
  ...
  it("prefixes a custom server root path", () => {
    expect(withServerRoot("/ui/assets/logos/openai.svg", "/derouter")).toBe("/derouter/ui/assets/logos/openai.svg");
  });

  it("normalizes a trailing or missing slash in the root", () => {
    expect(withServerRoot("/ui/assets/logos/x.svg", "/derouter/")).toBe("/derouter/ui/assets/logos/x.svg");
...
describe("resolveLogoSrc", () => {
  ...
  it("returns undefined for empty values", async () => {
    const { resolveLogoSrc } = await importWithRoot("/derouter");
  ...
  it("passes bundled /_next/static/ asset URLs through untouched even under a sub-path mount", async () => {
    ...
    expect(resolveLogoSrc("/derouter-asset-prefix/_next/static/media/openai_small.abc123.svg")).toBe(
      "/derouter-asset-prefix/_next/static/media/openai_small.abc123.svg",
    );
    expect(resolveLogoSrc("/derouter/_next/static/media/openai_small.abc123.svg")).toBe(
      "/derouter/_next/static/media/openai_small.abc123.svg",
    );
  });

  it("still prefixes a backend path that merely contains a /_next/ lookalike segment", async () => {
    const { resolveLogoSrc } = await importWithRoot("/derouter");
    expect(resolveLogoSrc("/ui/assets/logos/_next/logo.svg")).toBe("/derouter/ui/assets/logos/_next/logo.svg");
  });

  it("roots a local asset path using the live server root path", async () => {
    const { resolveLogoSrc } = await importWithRoot("/derouter");
    expect(resolveLogoSrc("/ui/assets/logos/github.svg")).toBe("/derouter/ui/assets/logos/github.svg");
  });
  ...
  it("does not double-prefix a stored value that already carries the server root path", async () => {
    const { resolveLogoSrc } = await importWithRoot("/derouter");
    expect(resolveLogoSrc("/derouter/ui/assets/logos/custom.svg")).toBe("/derouter/ui/assets/logos/custom.svg");
    expect(resolveLogoSrc("/derouter")).toBe("/derouter");
  });

  it("still prefixes a path whose first segment merely starts with the root path text", async () => {
    const { resolveLogoSrc } = await importWithRoot("/derouter");
    expect(resolveLogoSrc("/derouter-docs/logo.png")).toBe("/derouter/derouter-docs/logo.png");
  });
});
