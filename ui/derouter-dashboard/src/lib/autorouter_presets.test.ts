...
describe("autorouter_presets", () => {
  ...
  // key. For tier_model_configs that silently drops one model's derouter_params; catch it in the
  ...
  it("pins the anthropic preset's reasoning tier to Fable 5.1 at high thinking", () => {
    ...
    expect(config.tier_model_configs).toEqual({
      REASONING: [{ model_name: "claude-fable-5-1", derouter_params: { reasoning_effort: "high" } }],
    });
  });
  ...
  it("pins the lite preset's per-tier reasoning efforts", () => {
    expect(getPresetByKey("lite")!.complexity_router_config.tier_model_configs).toEqual({
      MEDIUM: [{ model_name: "muse-spark-1.2", derouter_params: { reasoning_effort: "xhigh" } }],
      COMPLEX: [{ model_name: "kimi-k3", derouter_params: { reasoning_effort: "max" } }],
    });
  });
  ...
  it("pins the OpenAI preset to the Luna, Terra, Sol, and Astra progression", () => {
    ...
    expect(preset.complexity_router_config.tier_model_configs).toEqual({
      REASONING: [{ model_name: "gpt-6-astra", derouter_params: { reasoning_effort: "xhigh" } }],
    });
    const prefill = buildPresetPrefill(preset.complexity_router_config, groupsOnly(getRequiredModelsInPreset(preset)));
    expect(prefill.complexityRouterConfig.tier_model_params).toEqual({
  ...
  describe("deploymentRefsFromModelInfo", () => {
    it("keeps derouter_params.model and model_info.base_model, drops rows with neither or no name", () => {
      const refs = deploymentRefsFromModelInfo([
        ...
        { model_name: "no-underlying", derouter_params: {}, model_info: {} },
    ...
    it("lets an azure deployment resolve through base_model declared under derouter_params", () => {
      const availability = buildModelAvailability(
        ["azure-lp"],
        deploymentRefsFromModelInfo([
          {
            model_name: "azure-lp",
            derouter_params: { model: "azure/opaque-deployment-name", base_model: "azure/gpt-5.4" },
          },
        ]),
      );
    ...
    it("lets an azure deployment resolve through its admin-declared base_model", () => {
      const availability = buildModelAvailability(
        ...
        deploymentRefsFromModelInfo([
          {
            model_name: "azure-prod",
            derouter_params: { model: "azure/opaque-deployment-name" },
            model_info: { base_model: "azure/gpt-5.4" },
          },
        ]),
      );
  ...
  describe("buildPresetPrefill", () => {
    ...
    it("prefills the per-model derouter_params a preset carries in tier_model_configs", () => {
      const config = {
        tiers: { SIMPLE: ["gpt-5-nano"], MEDIUM: [], COMPLEX: [], REASONING: ["o3"] },
        tier_model_configs: {
          REASONING: [{ model_name: "o3", derouter_params: { reasoning_effort: "high" } }],
        },
        classifier_type: "heuristic" as const,
        classification_mode: "every_request" as const,
        session_affinity: false,
    ...
    it("rewrites a param key to the same registered spelling its tier entry was rewritten to", () => {
      const config = {
        ...
        tier_model_configs: {
          REASONING: [{ model_name: "claude-sonnet-4-5", derouter_params: { reasoning_effort: "high" } }],
        },
        classifier_type: "heuristic" as const,
        classification_mode: "every_request" as const,
        session_affinity: false,
    ...
    it("merges rather than drops params when two spellings resolve to the same registered model", () => {
      const config = {
        ...
        tier_model_configs: {
          REASONING: [
            { model_name: "claude-sonnet-4-5", derouter_params: { reasoning_effort: "high", temperature: 0.2 } },
            { model_name: "claude-sonnet-4.5", derouter_params: { reasoning_effort: "low" } },
          ],
        },
        classifier_type: "heuristic" as const,
        classification_mode: "every_request" as const,
