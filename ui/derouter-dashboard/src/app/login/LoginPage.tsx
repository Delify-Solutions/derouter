...
function LoginPageContent() {
  ...
  useEffect(() => {
    ...
    if (ssoCode) {
      const rawWorkerUrl = localStorage.getItem("derouter_worker_url");
          ...
          <CardContent>
            <div className="flex w-full flex-col gap-4">
              <div className="text-center">
                <h2 className="text-3xl font-semibold text-foreground">🚅 DeRouter</h2>
              </div>
        ...
        <CardContent>
          <TooltipProvider>
            <div className="flex w-full flex-col gap-4">
              ...
              <div className="text-center">
                <h3 className="text-2xl font-semibold text-foreground">{t("login.title_alt", { defaultValue: "Login" })}</h3>
              ...
              {!uiConfig?.hide_default_credentials_hint && (
                <Alert variant="info">
                  <Info />
                  <AlertTitle>{t("login.defaultCredentials", { defaultValue: "Default Credentials" })}</AlertTitle>
                  <AlertDescription>
                    <p className="text-sm">
                      {t("login.defaultCredentialsDesc", { defaultValue: "By default, Username is" })}{" "}
                      <code className="bg-muted px-1 py-0.5 rounded-sm text-xs">admin</code>{" "}
                      {t("login.and", { defaultValue: "and" })}{" "}
                      {t("login.passwordIs", { defaultValue: "Password is your set DeRouter Proxy" })}
                      <code className="bg-muted px-1 py-0.5 rounded-sm text-xs">MASTER_KEY</code>.
                    </p>
                    <p className="mt-2 text-sm">
                      {t("login.needSetCreds", { defaultValue: "Need to set UI credentials or SSO?" })}{" "}
                      <a href="https://router.delify.vn/docs/proxy/ui" target="_blank" rel="noopener noreferrer">
                        {t("login.checkDocs", { defaultValue: "Check the documentation" })}
                      </a>
                      .
                    </p>
                  </AlertDescription>
              ...
              <form onSubmit={form.handleSubmit(handleSubmit)}>
                <FieldGroup>
                  ...
                  <Button type="submit" size="lg" disabled={isLoginLoading} className="w-full">
                    ...
                    {isLoginLoading ? t("login.signingIn") : t("login.signIn")}
                  </Button>
                  ...
                  {!uiConfig?.sso_configured ? (
                    <Tooltip>
                      ...
                      <TooltipContent>{t("login.configureSso")}</TooltipContent>
                    </Tooltip>
                  ) : (
                    <Button
                      type="button"
                      ...
                      onClick={() => {
                        ...
                        if (selectedWorker) {
                          ...
                          localStorage.setItem("derouter_selected_worker_id", selectedWorkerId!);
                          switchToWorkerUrl(selectedWorker.url);
                        }
