        ...
        <div className="space-y-6 pt-4">
          <div className="flex items-start justify-between gap-4">
            ...
            <MountedFormField name="allow_all_keys" defaultValue={mcpServer?.allow_all_keys ?? false} className="mb-0">
              {(control) => <Switch aria-label="Allow All DeRouter Keys" {...switchControl(control)} />}
            </MountedFormField>
          </div>
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <span className="text-sm font-medium text-foreground flex items-center">
                  ...
                  <SimpleTooltip content="When on, DeRouter skips its own API key/SSO check for this server and lets the client complete PKCE directly with the upstream MCP server. Only honored when Auth Type is oauth2. No spend tracking or per-key rate limiting will run on this route.">
                    <Info className="ml-2 size-4 text-info hover:text-info/80 cursor-help" />
                  </SimpleTooltip>
                </span>
                <p className="text-sm text-muted-foreground mt-1">
                  Bypass DeRouter auth so clients authenticate directly with the upstream OAuth MCP server.
                </p>
              </div>
              <MountedFormField
          ...
          {showInternalDelegatePkceWarning && (
            <Alert variant="warning" className="mb-2">
              ...
              <AlertDescription>
                ...
                able to reach the upstream OAuth2 /authorize flow without a DeRouter session. Ensure your upstream
