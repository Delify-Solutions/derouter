...
const FeatureCard: React.FC<FeatureCardProps> = ({
  ...
  const getHeadersConfig = () => {
    const headers: Record<string, any> = {
      "x-derouter-api-key": "Bearer YOUR_DEROUTER_API_KEY",
    };
    ...
    <div className="flex w-full flex-col gap-6">
      <div className="bg-linear-to-r from-success/15 to-success/5 p-6 rounded-lg border border-success/15">
        <div className="flex items-center gap-3 mb-3">
          ...
          <h4 className="mb-0 text-xl font-semibold text-success">DeRouter Proxy API Integration</h4>
        </div>
        <span className="text-success">
          Connect to DeRouter Proxy Responses API for seamless tool integration with multiple model providers
        </span>
      </div>
      ...
      <div className="flex w-full flex-col gap-6">
        <FeatureCard
          ...
          description="Configure your DeRouter Proxy Virtual Key for authentication"
        >
          <div className="flex w-full flex-col gap-4">
            <div>
              <span>Get your Virtual Key from your DeRouter Proxy dashboard or contact your administrator</span>
            </div>
            <CodeBlock title="Environment Variable" code='export DEROUTER_API_KEY="sk-..."' copyKey="derouter-env" />
          </div>
        </FeatureCard>
        ...
        <FeatureCard
          ...
          description="Connection details for your DeRouter MCP server"
        >
          <CodeBlock title="Server URL" code={`${proxyBaseUrl}/mcp`} copyKey="derouter-server-url" />
        </FeatureCard>

        <FeatureCard
          icon={<Code className="text-success" size={16} />}
          title="Implementation Example"
          description="Complete cURL example for using the DeRouter Proxy Responses API"
...
--header "Authorization: Bearer $DEROUTER_VIRTUAL_KEY" \\
--data '{
    "model": "gpt-4",
    "tools": [
        {
            ...
            "headers": {
                "x-derouter-api-key": "Bearer YOUR_DEROUTER_VIRTUAL_KEY",
    ...
    <div className="flex w-full flex-col gap-6">
      <div className="bg-linear-to-r from-info/15 to-info/5 p-6 rounded-lg border border-info/15">
        ...
        <span className="text-info">
          Connect OpenAI Responses API to your DeRouter MCP server for seamless tool integration
        </span>
      </div>
    ...
    <div className="flex w-full flex-col gap-6">
      <div className="bg-linear-to-r from-purple-50 to-blue-50 p-6 rounded-lg border border-purple-100 dark:from-purple-950 dark:to-blue-950 dark:border-purple-900">
        ...
        <span className="text-purple-700 dark:text-purple-300">
          Use tools directly from Cursor IDE with DeRouter MCP. Enable your AI assistant to perform real-world tasks
  ...
  const StreamableHTTPTab = () => (
    <div className="flex w-full flex-col gap-6">
      <div className="bg-linear-to-r from-success/15 to-success/5 p-6 rounded-lg border border-success/15">
        ...
        <span className="text-success">
          Connect to DeRouter MCP using HTTP transport. Compatible with any MCP client that supports HTTP streaming.
        </span>
      </div>
      ...
      <div className="flex w-full flex-col gap-6">
        <div>
          ...
          <p className="text-lg text-muted-foreground">
            Use tools directly from any MCP client with DeRouter MCP. Enable your AI assistant to perform real-world
        ...
        <Tabs defaultValue="openai" className="w-full">
          <TabsList variant="line" className="mt-8 mb-6 h-auto w-full justify-start rounded-none border-b p-0">
            <div className="flex rounded-lg bg-muted p-1">
              ...
              <TabsTrigger value="derouter" className="flex-none px-6 py-3">
                <span className="flex items-center gap-2 font-medium">
                  <Zap size={18} />
                  DeRouter Proxy
                </span>
          ...
          <TabsContent value="derouter" keepMounted className="mt-6">
            <DeRouterProxyTab />
          </TabsContent>
