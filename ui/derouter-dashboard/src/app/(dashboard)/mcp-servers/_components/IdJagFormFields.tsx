...
const IdJagFormFields: React.FC<IdJagFormFieldsProps> = ({ isEditing = false }) => {
  ...
  return (
    <>
      <MountedFormField
        label={
          <FieldLabel
            ...
            tooltip="Your IdP org authorization server's token endpoint. DeRouter exchanges the user's identity assertion here for an ID-JAG assertion (RFC 8693 with requested_token_type=urn:ietf:params:oauth:token-type:id-jag)."
          />
        }
      ...
      <MountedFormField
        label={
          <FieldLabel
            ...
            tooltip="The upstream resource authorization server's token endpoint. DeRouter posts the ID-JAG assertion here as an RFC 7523 jwt-bearer grant to get the access token the MCP server accepts."
          />
        }
      ...
      <MountedFormField
        label={<FieldLabel label="Client ID" tooltip="OAuth2 client ID DeRouter authenticates as on both legs." />}
      ...
      <MountedFormField
        label={
          <FieldLabel
            label="Client Secret"
            tooltip="Authenticates DeRouter as the OAuth client via client_secret_post. Leave blank when using a private key instead; a private key takes precedence over this secret."
          />
        }
