...
describe("CreateMCPServer", () => {
  ...
  describe("when HTTP transport is selected", () => {
    ...
    it("should warn that DeRouter auth is disabled when True Passthrough is selected", async () => {
      await selectHttpTransport();

      await selectOption("Authentication", "True Passthrough (no DeRouter auth)");

      await waitFor(() => {
        expect(
          screen.getByText("True Passthrough disables DeRouter authentication for this server"),
        ).toBeInTheDocument();
      });
    });
    ...
    it("should not show the True Passthrough warning when OAuth Delegate is selected", async () => {
      ...
      expect(
        screen.queryByText("True Passthrough disables DeRouter authentication for this server"),
      ).not.toBeInTheDocument();
    });

    it.each([["True Passthrough (no DeRouter auth)"], ["OAuth Delegate (client-supplied upstream token)"]])(
    ...
    it.each([
      ["true_passthrough", "True Passthrough (no DeRouter auth)"],
