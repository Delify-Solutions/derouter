  ...
  <>
    ...
    <MountedFormField
      label={
        <FieldLabel
          label="AWS Role ARN"
          tooltip="Optional. IAM role ARN to assume via STS before signing. If set, DeRouter calls sts:AssumeRole to get temporary credentials. Uses ambient credentials (IAM role, env vars) as the source identity unless explicit keys are also provided."
        />
      }
    ...
    >
      {(control) => (
        <Input
          {...textControl(control)}
          placeholder="derouter-prod (optional, auto-generated if blank)"
          className={fieldClassName}
        />
      )}
    </MountedFormField>
