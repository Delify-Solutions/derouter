 ...
 * that server, so callers reach the upstream without a DeRouter identity.
 */
export default function TruePassthroughWarning({ authType }: { authType?: string | null }) {
  ...
  return (
    <Alert className="mb-4">
      <TriangleAlert />
      <AlertTitle>True Passthrough disables DeRouter authentication for this server</AlertTitle>
      <AlertDescription>
        Anyone who can reach the gateway can call this server without a DeRouter key. The caller&apos;s Authorization
        ...
        should still authenticate to DeRouter.
      </AlertDescription>
    </Alert>
  );
}
