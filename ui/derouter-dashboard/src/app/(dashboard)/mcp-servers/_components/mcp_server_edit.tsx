...
export const EDIT_OAUTH_UI_STATE_KEY = "derouter-mcp-oauth-edit-state";
  ...
  const {
    ...
    onTokenReceived: (token) => {
      ...
      if (isClientForwardedTokenMode(getEffectiveAuthType())) {
        ...
        toast.success(
          "Token held for this browser session. Tools can now be loaded and configured; the token is not saved to DeRouter.",
        );
        return;
      }
                ...
                <MountedFormField
                  label={
                    <span className="text-sm font-medium text-foreground flex items-center">
                      ...
                      <SimpleTooltip content="Maximum number of tool calls DeRouter will run against this server at the same time. Additional calls wait for a free slot. Leave blank for no limit.">
                        <Info className="ml-2 size-4 text-info hover:text-info/80 cursor-help" />
                      </SimpleTooltip>
                    </span>
                  }
                  ...
                  <>
                    {!oauthFlowTypeValue && !isDelegateAuth && (
                      <Alert variant="warning" className="mb-4 rounded-lg">
                        ...
                        <AlertDescription>
                          Choose Machine-to-Machine (M2M) or Interactive (PKCE) so DeRouter authenticates it the way you
                          intend, then save. Until it is set, DeRouter falls back to interactive per-user auth and treats
                ...
                {!isStdioTransport && isAwsSigV4AuthType && (
                  <>
                    <p className="text-sm text-muted-foreground mb-2">
                      ...
                      <a
                        href="https://router.delify.vn/docs/mcp_aws_sigv4"
                        target="_blank"
                    ...
                    <MountedFormField
                      label={
                        <span className="text-sm font-medium text-foreground flex items-center">
                          AWS Role ARN
                          <SimpleTooltip content="Optional. IAM role ARN to assume via STS before signing. If set, DeRouter calls sts:AssumeRole to get temporary credentials.">
                            <Info className="ml-2 size-4 text-info hover:text-info/80 cursor-help" />
                          </SimpleTooltip>
                        </span>
                      }
