  ...
  return (
    <div className="rounded-lg border border-dashed border-border p-4 space-y-2 mb-4">
      <p className="text-sm text-muted-foreground">
        Callers bring their own upstream token for this auth type, so DeRouter never stores tokens. To preview tools and
        configure the tool allowlist, authorize against the upstream here: the token stays in this browser session only
        and is never saved to DeRouter. An OAuth app configured below IS saved with the server, so internal users who
