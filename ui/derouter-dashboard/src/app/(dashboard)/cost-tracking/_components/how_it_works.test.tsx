...
describe("HowItWorks", () => {
  ...
  it("should show the response header names for discount verification", () => {
    renderWithProviders(<HowItWorks />);
    expect(screen.getByText("x-derouter-response-cost")).toBeInTheDocument();
    expect(screen.getByText("x-derouter-response-cost-original")).toBeInTheDocument();
    expect(screen.getByText("x-derouter-response-cost-discount-amount")).toBeInTheDocument();
  });
