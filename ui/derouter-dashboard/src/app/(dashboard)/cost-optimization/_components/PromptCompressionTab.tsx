      ...
      <Card>
        ...
        <CardContent>
          <p className="mb-4 text-sm text-muted-foreground">
            Headroom is a native DeRouter guardrail that compresses your prompts before they reach the model, so you pay
            for fewer input tokens. The tokens it removes are priced and shown on the Usage tab as compression savings.{" "}
            <a
              href="https://router.delify.vn/docs/proxy/headroom"
              target="_blank"
              ...
              {guardrails.map((guardrail) => (
                <li key={guardrail.guardrail_id} className="flex items-center justify-between py-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{guardrail.derouter_params?.api_base ?? ""}</p>
                  </div>
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                      guardrail.derouter_params?.default_on
                        ? "bg-success/15 text-success"
                        : "bg-muted text-muted-foreground"
                    }`}
                  >
                    {guardrail.derouter_params?.default_on ? "Always on" : "Opt-in"}
                  </span>
                </li>
              ))}
            </ul>
            ...
            <form onSubmit={form.handleSubmit(handleAdd)} noValidate>
              <FieldGroup>
                ...
                <FormField
                  ...
                  label={labelWithHint(
                    "Headroom API base",
                    "Base URL of your Headroom compression service (DeRouter calls its /v1/compress endpoint)",
                  )}
              ...
              <div className="mt-6 mb-4 rounded-lg border border-warning/20 bg-warning/10 p-3">
                <p className="text-sm text-warning">
                  ...
                  team is a DeRouter Enterprise feature. Get a trial key{" "}
                  <a
                    href="https://www.router.delify.vn/#pricing"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="underline"
                  >
