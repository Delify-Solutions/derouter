...
const project: ProjectResponse = {
  ...
  updated_by: "user-1",
  derouter_budget_table: { max_budget: 50 },
...
describe("EditProjectModal submit payload", () => {
  ...
  it("never sends server-only fields from the loaded record", async () => {
    ...
    expect(variables().params).not.toHaveProperty("derouter_budget_table");
  });
  ...
  it("omits max_budget when the project has no budget row", async () => {
    const user = setup();
    renderModal({ ...project, derouter_budget_table: null } as unknown as ProjectResponse);
    await screen.findByDisplayValue("My Project");
