      ...
      if (accessToken) {
        ...
        if (settings) {
          setProxySettings({
            ...
            DEROUTER_UI_API_DOC_BASE_URL: settings.DEROUTER_UI_API_DOC_BASE_URL,
          });
        }
      }
    };
      ...
      <Tabs defaultValue="chat" className="flex min-h-0 min-w-0 flex-1 flex-col gap-0 overflow-hidden">
        ...
        <TabsContent value="agent-builder" className="mt-0 h-full data-hidden:hidden" keepMounted>
          ...
          <AgentBuilderView
            ...
            disabledPersonalKeyCreation={disabledPersonalKeyCreation}
            proxySettings={proxySettings}
            customProxyBaseUrl={proxySettings?.DEROUTER_UI_API_DOC_BASE_URL ?? proxySettings?.PROXY_BASE_URL}
          />
        </TabsContent>
      </Tabs>
    </div>
