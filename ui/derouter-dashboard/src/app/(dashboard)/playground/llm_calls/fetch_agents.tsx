...
/** MCP tool entry in the same format as chat completions API (derouter_params.tools) */
...
/** Agent model from /model/info where derouter_params.model starts with "derouter_agent/" */
export interface AgentModel {
  model_name: string;
  derouter_params: {
    model: string;
    derouter_system_prompt?: string;
...
/**
 * Fetches available derouter_agent models from /v2/model/info.
 * Filters for models where derouter_params.model starts with "derouter_agent/".
 */
export const fetchAvailableAgentModels = async (
  ...
  try {
    ...
    const agentModels: AgentModel[] = list
      .filter(
        (m: { derouter_params?: { model?: string } }) =>
          typeof m?.derouter_params?.model === "string" && m.derouter_params.model.startsWith("derouter_agent/"),
      )
      .map((m: any) => ({
        model_name: m.model_name ?? m.model_group ?? "",
        derouter_params: {
          ...m.derouter_params,
          model: m.derouter_params.model,
          derouter_system_prompt: m.derouter_params?.derouter_system_prompt,
          tools: Array.isArray(m.derouter_params?.tools) ? m.derouter_params.tools : undefined,
        },
