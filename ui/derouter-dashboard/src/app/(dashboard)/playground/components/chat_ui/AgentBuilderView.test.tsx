...
const AGENTS: AgentModel[] = [
  {
    ...
    derouter_params: { model: "derouter_agent/gpt-4o", derouter_system_prompt: "Be helpful.", temperature: 0.3 },
    model_info: { id: "agent-1" },
  },
  {
    model_name: "research-agent",
    derouter_params: { model: "derouter_agent/claude-sonnet-4" },
...
const waitForRoster = async () => {
  await screen.findByRole("button", { name: "support-agent derouter_agent" });
};
...
describe("AgentBuilderView", () => {
  ...
  it("lists every agent and opens the first one's configuration", async () => {
    ...
    expect(screen.getByRole("button", { name: "research-agent derouter_agent" })).toBeInTheDocument();
  ...
  it("loads the configuration of whichever agent is picked", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "research-agent derouter_agent" }));
  ...
  it("creates the agent under the derouter_agent prefix", async () => {
    const user = userEvent.setup();
    renderView();
    await waitForRoster();
    ...
    expect(payload.derouter_params.model).toBe("derouter_agent/gpt-4o");
  });
  ...
  it("attaches the MCP servers the agent should reach", async () => {
    ...
    await waitFor(() => expect(modelPatchUpdateCall).toHaveBeenCalled());
    expect(modelPatchUpdateCall.mock.calls[0][1].derouter_params.tools).toEqual([
      { type: "mcp", server_label: "derouter", server_url: "derouter_proxy/mcp/github", require_approval: "never" },
    ]);
  });
