...
export interface AgentBuilderViewProps {
  ...
  proxySettings?: {
    ...
    DEROUTER_UI_API_DOC_BASE_URL?: string | null;
  };
  apiKey?: string;
...
function getConnectTabBaseUrl(
  ...
  const customDocBaseUrl = proxySettings?.DEROUTER_UI_API_DOC_BASE_URL;
...
-H 'x-derouter-api-key: ${apiKeyForCurl}' \\
-d '{
  "model": "${agentName}",
  "stream": true,
  "stream_options": {
...
function parseUnderlyingModel(derouterModel: string | undefined): string | undefined {
  if (!derouterModel || !derouterModel.startsWith("derouter_agent/")) return undefined;
  return derouterModel.slice("derouter_agent/".length) || undefined;
}

const MCP_TOOLS_PREFIX = "derouter_proxy/mcp/";
...
export default function AgentBuilderView({
  ...
  useEffect(() => {
    if (selectedAgent && !isNewAgent) {
      ...
      setDraftSystemPrompt(selectedAgent.derouter_params?.derouter_system_prompt ?? "");
      const underlying = parseUnderlyingModel(selectedAgent.derouter_params?.model);
      setDraftUnderlyingModel(underlying ?? modelGroups[0]?.model_group);
      const p = selectedAgent.derouter_params as { temperature?: number; max_tokens?: number } | undefined;
      ...
      const rawTools = selectedAgent.derouter_params?.tools;
  ...
  }, [selectedId, isNewAgent, selectedAgent?.model_name, selectedAgent?.derouter_params?.tools]);
  ...
  const handleSaveAgent = async () => {
    ...
    try {
      const response = await modelCreateCall(accessToken, {
        ...
        derouter_params: {
          model: `derouter_agent/${draftUnderlyingModel}`,
          derouter_system_prompt: draftSystemPrompt.trim() || undefined,
                ...
                {agentModels.map((agent) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-[10px] text-muted-foreground truncate">derouter_agent</div>
                    </button>
                  );
                })}
                <button
