...
/**
 * Get versioned prompt ID from derouter_params (preserves version)
 */
export const getVersionedPromptId = (promptData?: PromptSpec): string => {
  const baseId = getBasePromptId(promptData);
  const versionedId = (promptData?.derouter_params as any)?.prompt_id || baseId;
  return versionedId;
};
...
export const extractModel = (prompt: PromptSpec): string | null => {
  try {
    ...
    // Try to extract model from derouter_params directly
    if (params?.model) {
      return params.model;
    }
