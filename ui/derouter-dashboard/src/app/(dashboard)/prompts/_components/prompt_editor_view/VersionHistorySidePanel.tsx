...
const VersionHistorySidePanel: React.FC<VersionHistorySidePanelProps> = ({
  ...
  const getVersionNumber = (prompt: PromptSpec) => {
    // Use explicit version field if available, otherwise try to extract from derouter_params.prompt_id
    if (prompt.version) {
      return `v${prompt.version}`;
    }

    // Fallback: try to extract from derouter_params.prompt_id
    const versionedId = (prompt.derouter_params as any)?.prompt_id || prompt.prompt_id;
