...
describe("isAutoRouterDeployment", () => {
  ...
  it.each(cases)("returns %s -> %s", (_label, derouterParamsModel, expected) => {
    expect(isAutoRouterDeployment({ model_name: "some-group", derouter_params: { model: derouterParamsModel } })).toBe(
      expected,
    );
  });

  it("returns false when derouter_params is absent", () => {
...
describe("selectAutoRouterModelGroups", () => {
  it("keeps only the public model_name of auto-router deployments", () => {
    const deployments: AutoRouterCandidateDeployment[] = [
      ...
      { model_name: "claude-haiku", derouter_params: { model: "anthropic/claude-haiku-4-5" } },
      { model_name: "claude-sonnet", derouter_params: { model: "anthropic/claude-sonnet-4-5" } },
      { model_name: "cheap-router", derouter_params: { model: "auto_router/adaptive_router" } },
    ];
  ...
  it("drops auto-router deployments that have no public model_name", () => {
    expect(
      selectAutoRouterModelGroups([{ model_name: "", derouter_params: { model: "auto_router/complexity_router" } }]),
    ).toEqual(new Set());
  });
...
describe("selectPlainModelGroups", () => {
  ...
  it("drops a group name that also fronts an auto-router deployment", () => {
    const deployments: AutoRouterCandidateDeployment[] = [
      { model_name: "shared-name", derouter_params: { model: "auto_router/complexity_router" } },
      { model_name: "shared-name", derouter_params: { model: "anthropic/claude-sonnet-4-5" } },
    ];
  ...
  it("drops deployments that have no public model_name", () => {
    expect(selectPlainModelGroups([{ model_name: "", derouter_params: { model: "openai/gpt-4o" } }])).toEqual(new Set());
  });
});
    ...
    (modelInfoCall as any).mockImplementation((_t: string, _u: string, _r: string, page: number) => {
      if (page === 1) {
        return Promise.resolve({
          data: [{ model_name: "claude-haiku", derouter_params: { model: "anthropic/claude-haiku-4-5" } }],
          total_count: 3,
          current_page: 1,
          total_pages: 3,
          size: 1000,
      ...
      if (page === 2) {
        return Promise.resolve({
          data: [{ model_name: "claude-sonnet", derouter_params: { model: "anthropic/claude-sonnet-4-5" } }],
          total_count: 3,
          current_page: 2,
          total_pages: 3,
          size: 1000,
      ...
      return Promise.resolve({
        data: [{ model_name: "late-router", derouter_params: { model: "auto_router/complexity_router" } }],
        total_count: 3,
        current_page: 3,
        total_pages: 3,
        size: 1000,
