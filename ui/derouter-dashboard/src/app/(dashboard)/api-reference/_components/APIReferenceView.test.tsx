...
describe("APIReferenceView", () => {
  ...
  it("uses the API doc base url when provided", () => {
    const apiDocUrl = "https://docs.derouter.test";
    render(<APIReferenceView proxySettings={{ DEROUTER_UI_API_DOC_BASE_URL: apiDocUrl }} />);
  ...
  it("falls back to the proxy base url when the docs url is missing", () => {
    const proxyUrl = "https://proxy.derouter.test";
  ...
  it("prefers the docs url when both urls are provided", () => {
    const apiDocUrl = "https://docs-preferred.derouter.test";
    const proxyUrl = "https://proxy-backup.derouter.test";

    render(
      <APIReferenceView
        proxySettings={{
          DEROUTER_UI_API_DOC_BASE_URL: apiDocUrl,
  ...
  it("renders the page title, blurb and docs link", () => {
    render(<APIReferenceView proxySettings={{ PROXY_BASE_URL: "https://proxy.derouter.test" }} />);

    expect(screen.getByText("OpenAI Compatible Proxy: API Reference")).toBeInTheDocument();
    expect(screen.getByText(/DeRouter is OpenAI Compatible/)).toBeInTheDocument();

    const docsLink = screen.getByRole("link", { name: /API Reference Docs/ });
    expect(docsLink).toHaveAttribute("href", "https://router.delify.vn/docs/proxy/user_keys");
