...
const healthyResponse = {
  ...
  derouter_cache_params: JSON.stringify({ type: "redis", supported_call_types: ["acompletion"] }),
  health_check_cache_params: JSON.stringify({
    redis_version: "7.2.1",
    namespace: "derouter-ns",
...
const errorPayload = {
  ...
  derouter_cache_params: { type: "redis" },
...
describe("CacheHealthTab", () => {
  ...
  it("shows the Redis detail rows when the cache type is redis", async () => {
    ...
    expect(screen.getByText("derouter-ns")).toBeInTheDocument();
  });
  ...
  it("omits the Redis detail rows for a non-redis cache type", async () => {
    renderTab({
      healthCheckResponse: {
        status: "healthy",
        ping_response: true,
        derouter_cache_params: JSON.stringify({ type: "local" }),
        health_check_cache_params: JSON.stringify({}),
      },
    });
