...
const GuardrailInfoView: React.FC<GuardrailInfoProps> = ({ guardrailId, onClose, accessToken, isAdmin }) => {
  ...
  const fetchGuardrailInfo = async () => {
    try {
      ...
      if (response.derouter_params?.pii_entities_config) {
        const piiConfig = response.derouter_params.pii_entities_config;
  ...
  useEffect(() => {
    ...
    form.setValue("default_on", guardrailData.derouter_params?.default_on);
    form.setValue(
      "skip_system_message_choice",
      skipSystemMessageToChoice(guardrailData.derouter_params?.skip_system_message_in_guardrail),
    );
    form.setValue(
      "skip_tool_message_choice",
      skipToolMessageToChoice(guardrailData.derouter_params?.skip_tool_message_in_guardrail),
    );
    form.setValue(
      "guardrail_info",
      guardrailData.guardrail_info ? JSON.stringify(guardrailData.guardrail_info, null, 2) : "",
    );
    if (guardrailData.derouter_params?.optional_params) {
      form.setValue("optional_params", guardrailData.derouter_params.optional_params);
    }
  }, [guardrailData, guardrailProviderSpecificParams, form]);

  const resetToolPermissionEditor = useCallback(() => {
    if (guardrailData?.derouter_params?.guardrail === "tool_permission") {
      setToolPermissionConfig({
        rules: (guardrailData.derouter_params?.rules as ToolPermissionConfig["rules"]) || [],
        default_action: (
          (guardrailData.derouter_params?.default_action || "deny") as ToolPermissionConfig["default_action"]
        ).toLowerCase() as ToolPermissionConfig["default_action"],
        on_disallowed_action: (
          (guardrailData.derouter_params?.on_disallowed_action ||
            "block") as ToolPermissionConfig["on_disallowed_action"]
        ).toLowerCase() as ToolPermissionConfig["on_disallowed_action"],
        violation_message_template: guardrailData.derouter_params?.violation_message_template || "",
      });
    } else {
  ...
  const handleGuardrailUpdate = async (values: GuardrailFormValues) => {
    try {
      ...
      if (values.default_on !== guardrailData.derouter_params?.default_on) {
        updateData.derouter_params.default_on = values.default_on;
      }

      const prevSkipChoice = skipSystemMessageToChoice(guardrailData.derouter_params?.skip_system_message_in_guardrail);
      ...
      if (nextSkipChoice !== undefined && nextSkipChoice !== prevSkipChoice) {
        if (nextSkipChoice === "inherit") {
          updateData.derouter_params.skip_system_message_in_guardrail = null;
        } else if (nextSkipChoice === "yes") {
          updateData.derouter_params.skip_system_message_in_guardrail = true;
        } else {
          updateData.derouter_params.skip_system_message_in_guardrail = false;
        }
      }

      const prevSkipToolChoice = skipToolMessageToChoice(guardrailData.derouter_params?.skip_tool_message_in_guardrail);
      ...
      if (nextSkipToolChoice !== undefined && nextSkipToolChoice !== prevSkipToolChoice) {
        if (nextSkipToolChoice === "inherit") {
          updateData.derouter_params.skip_tool_message_in_guardrail = null;
        } else if (nextSkipToolChoice === "yes") {
          updateData.derouter_params.skip_tool_message_in_guardrail = true;
        } else {
          updateData.derouter_params.skip_tool_message_in_guardrail = false;
        }
      }
      ...
      const originalPiiConfig = guardrailData.derouter_params?.pii_entities_config || {};
      ...
      if (JSON.stringify(originalPiiConfig) !== JSON.stringify(newPiiEntitiesConfig)) {
        updateData.derouter_params.pii_entities_config = newPiiEntitiesConfig;
      }

      // Only add Content Filter patterns if there are changes
      if (guardrailData.derouter_params?.guardrail === "derouter_content_filter" && hasUnsavedContentFilterChanges) {
        ...
        updateData.derouter_params.patterns = formattedData.patterns;
        updateData.derouter_params.blocked_words = formattedData.blocked_words;
        updateData.derouter_params.categories = formattedData.categories;
        updateData.derouter_params.competitor_intent_config = formattedData.competitor_intent_config ?? null;
      }

      if (guardrailData.derouter_params?.guardrail === "tool_permission") {
        const originalRules = guardrailData.derouter_params?.rules || [];
        const currentRules = toolPermissionConfig.rules || [];
        const rulesChanged = JSON.stringify(originalRules) !== JSON.stringify(currentRules);

        const originalDefault = (guardrailData.derouter_params?.default_action || "deny").toLowerCase();
        ...
        const originalOnDisallowed = (guardrailData.derouter_params?.on_disallowed_action || "block").toLowerCase();
        ...
        const originalMessage = guardrailData.derouter_params?.violation_message_template || "";
        ...
        if (toolPermissionDirty || rulesChanged || defaultChanged || onDisallowedChanged || messageChanged) {
          updateData.derouter_params.rules = currentRules;
          updateData.derouter_params.default_action = currentDefault;
          updateData.derouter_params.on_disallowed_action = currentOnDisallowed;
          updateData.derouter_params.violation_message_template = currentMessage || null;
        }
      }
      ...
      const currentProvider = Object.keys(guardrail_provider_map).find(
        (key) => guardrail_provider_map[key] === guardrailData.derouter_params?.guardrail,
      );

      // Use pre-fetched provider params to copy recognised params
      const isToolPermissionGuardrail = guardrailData.derouter_params?.guardrail === "tool_permission";
      if (guardrailProviderSpecificParams && currentProvider && !isToolPermissionGuardrail) {
        ...
        allowedParams.forEach((paramName) => {
          ...
          const originalValue = guardrailData.derouter_params?.[paramName];
          ...
          if (hasChanged) {
            if (paramValue !== undefined && paramValue !== null && paramValue !== "") {
              ...
              updateData.derouter_params[paramName] = paramValue;
              ...
              updateData.derouter_params[paramName] = null;
            }
          }
        });
      }

      // Remove empty derouter_params object if no parameters were changed
      if (Object.keys(updateData.derouter_params).length === 0) {
        delete updateData.derouter_params;
      }
  ...
  const { logo, displayName } = getGuardrailLogoAndName(guardrailData.derouter_params?.guardrail || "");
      ...
      <Tabs defaultValue="overview">
        ...
        <div>
          ...
          <TabsContent value="overview" keepMounted>
              ...
              <Card className="block p-6">
                ...
                <div className="mt-2">
                  <h3 className="text-lg font-medium">
                    {formatGuardrailMode(guardrailData.derouter_params?.mode) || "-"}
                  </h3>
                  <Badge variant={guardrailData.derouter_params?.default_on ? "secondary" : "outline"}>
                    {guardrailData.derouter_params?.default_on ? "Default On" : "Default Off"}
                  </Badge>
                </div>
              </Card>
            ...
            {guardrailData.derouter_params?.pii_entities_config &&
              Object.keys(guardrailData.derouter_params.pii_entities_config).length > 0 && (
                <Card className="block mt-6 p-6">
                  <div className="flex justify-between items-center">
                    <p className="font-medium">PII Protection</p>
                    <Badge variant="secondary">
                      {Object.keys(guardrailData.derouter_params.pii_entities_config).length} PII entities configured
                    </Badge>
                  </div>
                </Card>
              )}
                ...
                <Card className="block mt-6 p-6">
                  ...
                  <div className="border rounded-lg overflow-hidden shadow-xs">
                    ...
                    <div className="max-h-[400px] overflow-y-auto">
                      {Object.entries(guardrailData.derouter_params?.pii_entities_config).map(([key, value]) => (
            ...
            {guardrailData.derouter_params?.guardrail === "tool_permission" && (
              <Card className="block mt-6 p-6">
            ...
            {/* Custom Code Display */}
            {guardrailData.derouter_params?.guardrail === "custom_code" && guardrailData.derouter_params?.custom_code && (
              <Card className="block mt-6 p-6">
                ...
                <div className="relative rounded-lg overflow-hidden border border-gray-700 bg-[#1e1e1e]">
                  ...
                  >
                    <code>{guardrailData.derouter_params.custom_code}</code>
                  </pre>
                </div>
              </Card>
            )}
            ...
            <TabsContent value="settings" keepMounted>
              <Card className="block p-6">
                <div className="flex justify-between items-center mb-4">
                  ...
                  {!isEditing &&
                    !isConfigGuardrail &&
                    (guardrailData.derouter_params?.guardrail === "custom_code" ? (
                ...
                {isEditing ? (
                      ...
                      <FieldGroup>
                        ...
                        <GuardrailField
                          ...
                          label={labelWithHint(
                            ...
                            "Unified guardrails: omit role: system from guardrail input (LLM still gets full messages). Use global default follows derouter_settings.skip_system_message_in_guardrail.",
                          )}
                        >
                        ...
                        <GuardrailField
                          ...
                          label={labelWithHint(
                            ...
                            "Unified guardrails: omit role: tool from guardrail input (LLM still gets full messages). Use global default follows derouter_settings.skip_tool_message_in_guardrail.",
                          )}
                        >
                          {(fieldControl) => <SkipMessageSelect control={fieldControl} />}
                        </GuardrailField>
                        {guardrailData.derouter_params?.guardrail === "presidio" && (
                          <>
                        ...
                        {guardrailData.derouter_params?.guardrail === "tool_permission" ? (
                          ...
                          <>
                            ...
                            <GuardrailProviderFields
                              ...
                              value={guardrailData.derouter_params}
                            />
                              ...
                              (() => {
                                ...
                                return (
                                  <GuardrailOptionalParams
                                    ...
                                    control={form.control}
                                    values={guardrailData.derouter_params}
                                  />
                                );
                              })()}
                          </>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div>{formatGuardrailMode(guardrailData.derouter_params?.mode) || "-"}</div>
                    </div>
                    <div>
                      <p className="font-medium">Default On</p>
                      <Badge variant={guardrailData.derouter_params?.default_on ? "secondary" : "outline"}>
                        {guardrailData.derouter_params?.default_on ? "Yes" : "No"}
                      </Badge>
                    </div>
                    ...
                    {guardrailData.derouter_params?.pii_entities_config &&
                      Object.keys(guardrailData.derouter_params.pii_entities_config).length > 0 && (
                        <div>
                          ...
                          <div className="mt-2">
                            <Badge variant="secondary">
                              {Object.keys(guardrailData.derouter_params.pii_entities_config).length} PII entities
                              configured
                            </Badge>
                          </div>
                        </div>
        ...
        editData={
          guardrailData
            ? ({
                ...
                derouter_params: guardrailData.derouter_params,
