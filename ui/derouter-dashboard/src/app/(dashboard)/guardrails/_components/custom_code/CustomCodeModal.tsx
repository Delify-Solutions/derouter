...
const CustomCodeModal: React.FC<CustomCodeModalProps> = ({ visible, onClose, onSuccess, accessToken, editData }) => {
  ...
  const TEST_INPUT_EXAMPLES = {
    ...
    pre_mcp_call: {
      ...
      data: {
        texts: ['Tool: read_wiki_structure\nArguments: {"repoName": "Delify-Solutions/derouter"}'],
        images: [],
        tools: [
          {
            type: "function",
            function: {
              ...
              parameters: {
                type: "object",
                properties: {
                  repoName: { type: "string", description: "Repository name, e.g. Delify-Solutions/derouter" },
                },
                required: ["repoName"],
              },
            },
  ...
  useEffect(() => {
    if (visible) {
      if (editData) {
        ...
        setMode(normalizeMode(editData.derouter_params?.mode));
        setDefaultOn(editData.derouter_params?.default_on || false);
        setCode(editData.derouter_params?.custom_code || CODE_TEMPLATES.empty.code);
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditMode && editData) {
        ...
        const existingMode = normalizeMode(editData.derouter_params?.mode);
        const modeChanged = mode.length !== existingMode.length || mode.some((m, i) => m !== existingMode[i]);
        if (modeChanged) {
          updateData.derouter_params.mode = mode;
        }
        if (defaultOn !== editData.derouter_params?.default_on) {
          updateData.derouter_params.default_on = defaultOn;
        }
          ...
          <div className="w-[180px]">
            ...
            >
              ...
              <SelectContent>
                ...
                <button
                  type="button"
                  onClick={() => window.open("https://models.router.delify.vn/guardrails", "_blank")}
        ...
        <div className="mt-4 flex gap-6">
          ...
          <div className="flex min-w-0 flex-1 flex-col">
            ...
            <div className="mt-3 flex shrink-0 items-center justify-between rounded-lg border border-info/20 bg-linear-to-r from-blue-50 to-indigo-50 p-4 dark:from-blue-950 dark:to-indigo-950">
              ...
              <Button size="sm" onClick={() => window.open("https://github.com/Delify-Solutions/derouter-guardrails", "_blank")}>
                <ExternalLink />
                Contribute Template
              </Button>
            </div>
