...
describe("Guardrail Info", () => {
  ...
  it("should handle content filter updates correctly", async () => {
    ...
    // derouter_params might be undefined if empty, which is correct.
    // If it exists, ensure patterns/blocked_words are not in it.
    if (firstCallArgs.derouter_params) {
      expect(firstCallArgs.derouter_params.patterns).toBeUndefined();
      expect(firstCallArgs.derouter_params.blocked_words).toBeUndefined();
    }
    ...
    expect(secondCallArgs.derouter_params).toBeDefined();
    expect(secondCallArgs.derouter_params.patterns).toEqual(["new_pattern"]);
    expect(secondCallArgs.derouter_params.blocked_words).toEqual(["new_word"]);
  });
  ...
  it("keeps the settings panel mounted while the overview tab is active", async () => {
    vi.mocked(networking.getGuardrailInfo).mockResolvedValue({
      guardrail_id: "123",
      guardrail_name: "Test Guardrail",
      derouter_params: { guardrail: "presidio", mode: "pre_call", default_on: true },
      created_at: "2024-01-01T00:00:00Z",
      updated_at: "2024-01-01T00:00:00Z",
      guardrail_definition_location: "database",
    });
