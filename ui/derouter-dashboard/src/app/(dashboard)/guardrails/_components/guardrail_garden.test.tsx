...
const DEROUTER_CARDS = ALL_CARDS.filter((c) => c.category === "derouter");
...
describe("GuardrailGarden", () => {
  ...
  it("should render both sections with their descriptions", () => {
    ...
    expect(screen.getByText("DeRouter Content Filter")).toBeInTheDocument();
    expect(
      screen.getByText(
        "Built-in guardrails powered by DeRouter. Zero latency, no external dependencies, no additional cost.",
      ),
    ).toBeInTheDocument();
  ...
  it("should show a capped set of derouter cards behind a show all toggle", async () => {
    const user = userEvent.setup();
    renderGarden();

    expect(screen.getByText(`Show all (${DEROUTER_CARDS.length})`)).toBeInTheDocument();
    expect(screen.getByText(DEROUTER_CARDS[0].name)).toBeInTheDocument();
    expect(screen.queryByText(DEROUTER_CARDS[DEROUTER_CARDS.length - 1].name)).not.toBeInTheDocument();

    await user.click(screen.getByText(`Show all (${DEROUTER_CARDS.length})`));

    expect(screen.getByText("Show less")).toBeInTheDocument();
    expect(screen.getByText(DEROUTER_CARDS[DEROUTER_CARDS.length - 1].name)).toBeInTheDocument();
  });
