...
const guardrail = (derouterParams: Record<string, unknown>, guardrailInfo?: Record<string, unknown>) => ({
  guardrail_id: "123",
  guardrail_name: "Test Guardrail",
  derouter_params: { guardrail: "bedrock", mode: "pre_call", default_on: true, ...derouterParams },
...
describe("GuardrailInfoView update payload characterization", () => {
  ...
  it("sends only guardrail_name and drops the empty derouter_params object", async () => {
    const user = userEvent.setup({ delay: null });
    renderView();
    await openEditor(user);
  ...
  it("never leaks the seeded guardrail, mode or created_at keys into derouter_params", async () => {
    const user = userEvent.setup({ delay: null });
    renderView();
    await openEditor(user);
    ...
    await waitFor(() => expect(networking.updateGuardrailCall).toHaveBeenCalledTimes(1));
    expect(lastPayload()).toEqual({ derouter_params: { guardrailIdentifier: "gr-new" } });
  });
  ...
  it("sends null for a provider param the user cleared", async () => {
    ...
    await waitFor(() => expect(networking.updateGuardrailCall).toHaveBeenCalledTimes(1));
    expect(lastPayload()).toEqual({ derouter_params: { api_key: null } });
  });
  ...
  it("maps the skip system message choice to an explicit boolean", async () => {
    ...
    await waitFor(() => expect(networking.updateGuardrailCall).toHaveBeenCalledTimes(1));
    expect(lastPayload()).toEqual({ derouter_params: { skip_system_message_in_guardrail: true } });
  });
  ...
  it("reads a value typed into the optional params section out of the nested optional_params object", async () => {
    ...
    await waitFor(() => expect(networking.updateGuardrailCall).toHaveBeenCalledTimes(1));
    expect(lastPayload()).toEqual({ derouter_params: { severity_threshold: 4 } });
  });
  ...
  it("clears a stored nested param to null because no form field ever binds it", async () => {
    ...
    await waitFor(() => expect(networking.updateGuardrailCall).toHaveBeenCalledTimes(1));
    expect(lastPayload()).toEqual({ derouter_params: { guardrailIdentifier: "gr-new", tuning: null } });
  });
  ...
  it("sends an unnormalised numeric provider field as the raw string the input produced", async () => {
    ...
    await waitFor(() => expect(networking.updateGuardrailCall).toHaveBeenCalledTimes(1));
    expect(lastPayload()).toEqual({ derouter_params: { max_tokens: "5" } });
  });
