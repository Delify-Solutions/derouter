  ...
  const handleSubmit = async () => {
    try {
      ...
      if (skipForCreate !== undefined) {
        guardrailData.derouter_params.skip_system_message_in_guardrail = skipForCreate;
      }
      ...
      if (skipToolForCreate !== undefined) {
        guardrailData.derouter_params.skip_tool_message_in_guardrail = skipToolForCreate;
      }
      ...
      if (providerKey === "PresidioPII" && selectedEntities.length > 0) {
        ...
        guardrailData.derouter_params.pii_entities_config = piiEntitiesConfig;

        // Add Presidio API bases if provided
        if (values.presidio_analyzer_api_base) {
          guardrailData.derouter_params.presidio_analyzer_api_base = values.presidio_analyzer_api_base;
        }
        if (values.presidio_anonymizer_api_base) {
          guardrailData.derouter_params.presidio_anonymizer_api_base = values.presidio_anonymizer_api_base;
        }
      }
      ...
      if (shouldRenderContentFilterConfigSettings(providerKey)) {
        ...
        if (selectedPatterns.length > 0) {
          guardrailData.derouter_params.patterns = selectedPatterns.map((p) => ({
        ...
        if (blockedWords.length > 0) {
          guardrailData.derouter_params.blocked_words = blockedWords.map((w) => ({
            keyword: w.keyword,
            action: w.action,
        ...
        if (selectedContentCategories.length > 0) {
          guardrailData.derouter_params.categories = selectedContentCategories.map((c) => ({
            category: c.category,
            enabled: true,
            action: c.action,
        ...
        if (hasCompetitorIntent && competitorIntentConfig) {
          guardrailData.derouter_params.competitor_intent_config = {
      ...
      // Add config values to the guardrail_info if provided
        ...
        try {
          ...
          // For some guardrails, the config values need to be in derouter_params
      ...
      if (guardrailProvider === "llm_as_a_judge") {
        ...
        guardrailData.derouter_params.judge_model = values.judge_model;
        guardrailData.derouter_params.overall_threshold = values.overall_threshold ?? 80;
        guardrailData.derouter_params.on_failure = values.on_failure ?? "block";
        guardrailData.derouter_params.criteria = criteria.map((c) => ({
          name: c.name,
      ...
      if (guardrailProvider === "tool_permission") {
        ...
        guardrailData.derouter_params.rules = toolPermissionConfig.rules;
        guardrailData.derouter_params.default_action = toolPermissionConfig.default_action;
        guardrailData.derouter_params.on_disallowed_action = toolPermissionConfig.on_disallowed_action;
        if (toolPermissionConfig.violation_message_template) {
          guardrailData.derouter_params.violation_message_template = toolPermissionConfig.violation_message_template;
        }
      }
      ...
      if (shouldRenderContentFilterConfigSettings(providerKey)) {
        if (endSessionAfterNFails !== undefined && endSessionAfterNFails > 0) {
          guardrailData.derouter_params.end_session_after_n_fails = endSessionAfterNFails;
        }
        if (onViolation && selectedEndpointType === "realtime") {
          guardrailData.derouter_params.on_violation = onViolation;
        }
        if (realtimeViolationMessage.trim()) {
          guardrailData.derouter_params.realtime_violation_message = realtimeViolationMessage.trim();
        }
      }
      ...
      // Skip for providers that handle their own derouter_params (llm_as_a_judge, tool_permission, content filter, PII)
      if (providerParams && selectedProvider && guardrailProvider !== "llm_as_a_judge") {
        ...
        allowedParams.forEach((paramName) => {
          ...
          if (paramValue !== undefined && paramValue !== null && paramValue !== "") {
            guardrailData.derouter_params[paramName] = paramValue;
          }
        });
      }
      ...
      <FieldGroup>
        ...
        <GuardrailField
          ...
          label={labelWithHint(
            ...
            "Unified guardrails only: omit role: system from guardrail evaluation input (OpenAI chat + Anthropic messages). The model still receives full messages. Use global default follows derouter_settings.skip_system_message_in_guardrail.",
          )}
        >
        ...
        <GuardrailField
          ...
          label={labelWithHint(
            ...
            "Unified guardrails only: omit role: tool from guardrail evaluation input (OpenAI chat + Anthropic messages). The model still receives full messages. Use global default follows derouter_settings.skip_tool_message_in_guardrail.",
          )}
        >
          {(fieldControl) => <SkipMessageSelect control={fieldControl} />}
        </GuardrailField>
