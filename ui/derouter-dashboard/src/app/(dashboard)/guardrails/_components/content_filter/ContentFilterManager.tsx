...
const ContentFilterManager: React.FC<ContentFilterManagerProps> = ({
  ...
  useEffect(() => {
    if (guardrailData?.derouter_params?.patterns) {
      const patterns = guardrailData.derouter_params.patterns.map((p: any, index: number) => ({
        id: `pattern-${index}`,
    ...
    if (guardrailData?.derouter_params?.blocked_words) {
      const words = guardrailData.derouter_params.blocked_words.map((w: any, index: number) => ({
        id: `word-${index}`,
        keyword: w.keyword,
    ...
    if (guardrailData?.derouter_params?.categories?.length > 0) {
      const contentCategoriesMap = guardrailSettings?.content_filter_settings?.content_categories
        ...
        : {};
      const categories = guardrailData.derouter_params.categories.map((c: any, index: number) => {
    ...
    const cic = guardrailData?.derouter_params?.competitor_intent_config;
  ...
  if (guardrailData?.derouter_params?.guardrail !== "derouter_content_filter") {
    return null;
  }
