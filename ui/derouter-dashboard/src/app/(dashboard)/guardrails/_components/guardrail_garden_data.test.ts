...
import { ALL_CARDS, DEROUTER_CONTENT_FILTER_CARDS, PARTNER_GUARDRAIL_CARDS } from "./guardrail_garden_data";
...
describe("guardrail_garden_data logos", () => {
  ...
  it("uses the DeRouter logo for every content filter card", () => {
    for (const card of DEROUTER_CONTENT_FILTER_CARDS) {
      expect(card.logo, `card ${card.id}`).toContain("derouter_logo.jpg");
    }
  });
