...
const submitGuardrailSchema = z.object({
  ...
  extra_derouter_params: z.string().superRefine((value, ctx) => {
    if (!value) return;
    try {
...
function submissionToTeamGuardrail(item: GuardrailSubmissionItem): TeamGuardrail {
  const params = item.derouter_params ?? {};
        ...
        <div className="space-y-4">
          ...
          <div className="border border-info/15 bg-info/10 rounded-lg p-3">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-semibold text-info">Forward DeRouter API Key</span>
              </div>
              <Toggle enabled={g.forwardKey} onToggle={onToggleForwardKey} disabled={!isAdmin} />
            </div>
            <p className="text-xs text-info leading-relaxed">
              When enabled, the caller&apos;s DeRouter API key is forwarded as an{" "}
          ...
          <div className="flex items-start gap-2 bg-muted border border-border rounded-lg p-3">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              ...
              <a
                href="https://router.delify.vn/docs/adding_provider/generic_guardrail_api"
                target="_blank"
                rel="noopener noreferrer"
                className="text-info hover:underline"
              >
                DeRouter Generic Guardrail API docs
              </a>{" "}
...
export function TeamGuardrailsTab({ accessToken }: TeamGuardrailsTabProps) {
  ...
  const handleSubmitGuardrail = submitForm.handleSubmit(async (values) => {
    const derouter_params: Record<string, unknown> = {
      ...(values.extra_derouter_params ? JSON.parse(values.extra_derouter_params) : {}),
  ...
  async function toggleForwardKey(id: string) {
    ...
    try {
      await updateGuardrailCall(accessToken, id, {
        derouter_params: { forward_api_key: newValue },
      });
  ...
  async function updateCustomHeaders(id: string, customHeaders: { key: string; value: string }[]) {
    ...
    try {
      await updateGuardrailCall(accessToken, id, {
        derouter_params: { headers: headersObj },
      });
      setGuardrails((prev) =>
        prev.map((x) =>
          x.id === id
  ...
  async function updateExtraHeaders(id: string, extraHeaders: string[]) {
    ...
    try {
      await updateGuardrailCall(accessToken, id, {
        derouter_params: { extra_headers: extraHeaders },
      });
            ...
            <form onSubmit={handleSubmitGuardrail}>
              <FieldGroup>
                ...
                <FormField
                  ...
                  name="extra_derouter_params"
                  label={labelWithHint(
                    "Additional derouter_params (optional)",
                    "JSON object merged into derouter_params. e.g. forward_api_key, headers, model, unreachable_fallback",
                  )}
                >
                  {({ ref, ...field }) => (
                    <Textarea
