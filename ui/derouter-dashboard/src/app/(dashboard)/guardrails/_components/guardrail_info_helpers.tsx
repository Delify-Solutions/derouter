...
import derouterLogo from "../../../../../public/assets/logos/derouter_logo.jpg";
...
export const populateGuardrailProviders = (providerParamsResponse: Record<string, any>) => {
  ...
  providers.LlmAsAJudge = "DeRouter LLM as a Judge";
...
export const guardrail_provider_map: Record<string, string> = {
  ...
  Bedrock: "bedrock",
  Lakera: "lakera_v2",
  LitellmContentFilter: "derouter_content_filter",
...
export const shouldRenderContentFilterConfigSettings = (provider: string | null) => {
  ...
  const currentProviders = getGuardrailProviders();
  const providerEnum = currentProviders[provider as keyof typeof currentProviders];
  return providerEnum === "DeRouter Content Filter";
};
...
export const guardrailLogoMap = {
  ...
  "DeRouter Content Filter": derouterLogo.src,
  "DeRouter LLM as a Judge": derouterLogo.src,
  "Hide Secrets": derouterLogo.src,
  Akto: aktoLogo.src,
...
/** Tri-state UI value for `derouter_params.skip_system_message_in_guardrail` (inherit = use global). */
...
/** Tri-state UI value for `derouter_params.skip_tool_message_in_guardrail` (inherit = use global). */
