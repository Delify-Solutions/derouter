...
vi.mock("@/components/networking", () => ({
  proxyBaseUrl: "",
  getGlobalLitellmHeaderName: () => "x-derouter-api-key",
}));
...
describe("WorkflowRuns (migrated onto shared DataTable)", () => {
  ...
  it("sends the configured derouter key header on every fetch instead of hardcoding Authorization", async () => {
    const user = userEvent.setup();
    const fetchSpy = mockFetch(RUNS);
    vi.stubGlobal("fetch", fetchSpy);
    render(<WorkflowRuns accessToken="tok" />);
    ...
    for (const [url, init] of fetchSpy.mock.calls as [string, RequestInit][]) {
      expect(init.headers, url).toEqual({ "x-derouter-api-key": "Bearer tok" });
    }
  });
});
