...
const TagEditForm: React.FC<TagEditFormProps> = ({ tag, seedBudgetFields, userModels, onCancel, onSave }) => {
  ...
  const form = useZodForm(tagEditSchema, {
    defaultValues: {
      ...
      models: tag.models,
      max_budget: seedBudgetFields ? tag.derouter_budget_table?.max_budget : undefined,
      budget_duration: seedBudgetFields ? tag.derouter_budget_table?.budget_duration : undefined,
    },
  });
        ...
        <CollapsibleContent className="px-4 pb-4">
          ...
          <div className="mt-4 rounded-md border border-border bg-muted p-3">
            <p className="text-sm text-muted-foreground">
              ...
              <a
                href="https://github.com/Delify-Solutions/derouter/issues/new"
                target="_blank"
    ...
    <div className="p-4">
      ...
      {isEditing ? (
        ...
        <div className="space-y-6">
          ...
          {tagDetails.derouter_budget_table && (
            <Card>
              <CardContent>
                <CardTitle>Budget & Rate Limits</CardTitle>
                <div className="space-y-4 mt-4">
                  {tagDetails.derouter_budget_table.max_budget !== undefined &&
                    tagDetails.derouter_budget_table.max_budget !== null && (
                      <div>
                        <p className="font-medium">Max Budget</p>
                        <p>${tagDetails.derouter_budget_table.max_budget}</p>
                      </div>
                    )}
                  {tagDetails.derouter_budget_table.budget_duration && (
                    <div>
                      <p className="font-medium">Budget Duration</p>
                      <p>{tagDetails.derouter_budget_table.budget_duration}</p>
                    </div>
                  )}
                  {tagDetails.derouter_budget_table.tpm_limit !== undefined &&
                    tagDetails.derouter_budget_table.tpm_limit !== null && (
                      <div>
                        <p className="font-medium">TPM Limit</p>
                        <p>{tagDetails.derouter_budget_table.tpm_limit.toLocaleString()}</p>
                      </div>
                    )}
                  {tagDetails.derouter_budget_table.rpm_limit !== undefined &&
                    tagDetails.derouter_budget_table.rpm_limit !== null && (
                      <div>
                        <p className="font-medium">RPM Limit</p>
                        <p>{tagDetails.derouter_budget_table.rpm_limit.toLocaleString()}</p>
                      </div>
                    )}
                </div>
              </CardContent>
