...
interface SearchConnectionTestProps {
  derouterParams: Record<string, any>;
  accessToken: string;
  onTestComplete?: () => void;
}

const SearchConnectionTest: React.FC<SearchConnectionTestProps> = ({ derouterParams, accessToken, onTestComplete }) => {
  ...
  useEffect(() => {
    const runTest = async () => {
      setIsLoading(true);
      try {
        const result = await testSearchToolConnection(accessToken, derouterParams);
    ...
    runTest();
  }, [accessToken, derouterParams, onTestComplete]);
  ...
  const getCleanErrorMessage = (errorMsg: string) => {
    ...
    const cleanedError = mainError.replace(/^derouter\.(.*?)Error:\s*/, "");
      ...
      <div className="rounded-lg bg-card p-6">
        <div className="flex flex-col items-center justify-center px-5 py-8">
          ...
          <p className="text-base text-foreground">
            Testing connection to {derouterParams.search_provider || "search provider"}...
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="rounded-lg bg-card p-6">
        ...
        <div className="flex items-center justify-center px-5 py-8">
          ...
          <div className="ml-3">
            <p className="text-lg font-medium text-success">
              Connection to {derouterParams.search_provider} successful!
            </p>
        ...
        <div>
          <div className="mb-5 flex items-center">
            ...
            <p className="text-lg font-medium text-destructive">
              Connection to {derouterParams.search_provider || "search provider"} failed
            </p>
          </div>
      ...
      <div className="flex items-center justify-between">
        <a
          href="https://router.delify.vn/docs/search"
          target="_blank"
