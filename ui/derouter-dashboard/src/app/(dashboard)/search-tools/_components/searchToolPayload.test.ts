...
describe("buildSearchToolPayload", () => {
  it("nests the provider under derouter_params", () => {
    expect(buildSearchToolPayload(minimal).derouter_params.search_provider).toBe("perplexity");
  });
  ...
  it("leaves every optional param undefined so it never reaches the wire body", () => {
    expect(JSON.stringify(buildSearchToolPayload(minimal))).toBe(
      '{"search_tool_name":"tool","derouter_params":{"search_provider":"perplexity"}}',
    );
  });
  ...
  it("passes an api key through untouched", () => {
    expect(buildSearchToolPayload({ ...minimal, api_key: "sk-secret" }).derouter_params.api_key).toBe("sk-secret");
  });

  it("keeps an explicitly emptied api key as an empty string, so the backend clears it", () => {
    expect(buildSearchToolPayload({ ...minimal, api_key: "" }).derouter_params.api_key).toBe("");
  });
  ...
  it("sends the same wire body with an api key and a description as it did before the params were pruned", () => {
    expect(
      ...
      '{"search_tool_name":"tool","derouter_params":{"search_provider":"perplexity","api_key":"sk-secret"},"search_tool_info":{"description":"finds things"}}',
    );
  });
});
