...
const SearchTools: React.FC<SearchToolsProps> = ({ accessToken, userRole, userID }) => {
  ...
  const handleEditOpen = (toolId: string) => {
    ...
    const editFormValues: EditSearchToolFormValues = {
      ...
      search_provider: tool.derouter_params.search_provider,
      api_key: tool.derouter_params.api_key,
  ...
  const providerInfo = toolToDelete
    ? availableProviders.find((p) => p.provider_name === toolToDelete.derouter_params.search_provider)
    : null;
        ...
        resourceInformation={
          toolToDelete
            ? [
                ...
                {
                  label: "Provider",
                  value: providerInfo?.ui_friendly_name || toolToDelete.derouter_params.search_provider,
                },
