...
describe("AgentsPanel", () => {
  ...
  it("should drop previous rows when the fetch for a new token fails", async () => {
    vi.mocked(networking.getAgentsList)
      .mockResolvedValueOnce({
        agents: [
          { agent_id: "stale", agent_name: "Stale Agent", derouter_params: { model: "gpt-4" }, spend: 0, keys: [] },
        ],
      })
  ...
  it("should ignore a superseded response so it cannot overwrite the current token's rows", async () => {
    let resolveFirst: (value: {
      agents: { agent_id: string; agent_name: string; derouter_params: { model: string }; spend: number; keys: [] }[];
    }) => void = () => {};
    vi.mocked(networking.getAgentsList)
      .mockImplementationOnce(
        () =>
      ...
      .mockResolvedValueOnce({
        agents: [
          { agent_id: "current", agent_name: "Current Agent", derouter_params: { model: "gpt-4" }, spend: 0, keys: [] },
        ],
      });
    ...
    await act(async () => {
      resolveFirst({
        agents: [
          { agent_id: "stale", agent_name: "Superseded Agent", derouter_params: { model: "gpt-4" }, spend: 0, keys: [] },
        ],
      });
    });
