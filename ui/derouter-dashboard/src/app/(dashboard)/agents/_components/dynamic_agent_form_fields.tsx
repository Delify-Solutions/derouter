...
export const buildDynamicAgentData = (values: AgentFormValues, agentTypeInfo: AgentCreateInfo): AgentRequestPayload => {
  const derouterParams: Record<string, unknown> = {
    ...(agentTypeInfo.derouter_params_template || {}),
  };

  for (const field of agentTypeInfo.credential_fields) {
    const value = values[field.key];
    if (value && field.include_in_derouter_params !== false) {
      derouterParams[field.key] = value;
    }
  }

  if (values.cost_per_query) {
    derouterParams.cost_per_query = parseFloat(String(values.cost_per_query));
  }
  if (values.input_cost_per_token) {
    derouterParams.input_cost_per_token = parseFloat(String(values.input_cost_per_token));
  }
  if (values.output_cost_per_token) {
    derouterParams.output_cost_per_token = parseFloat(String(values.output_cost_per_token));
  }

  if (agentTypeInfo.model_template) {
    derouterParams.model = agentTypeInfo.credential_fields.reduce((model, field) => {
