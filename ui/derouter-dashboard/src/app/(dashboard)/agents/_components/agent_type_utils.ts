...
/**
 * Detects the agent type from an agent's derouter_params.
...
export const detectAgentType = (agent: Agent): string => {
  const model = agent.derouter_params?.model || "";
  const customProvider = agent.derouter_params?.custom_llm_provider;
 ...
 * Extracts values from derouter_params based on the agent type metadata.
 */
export const parseDynamicAgentForForm = (agent: Agent, agentTypeInfo: AgentCreateInfo): Record<string, any> => {
  ...
  const templateValues =
    agentTypeInfo.model_template && agent.derouter_params?.model
      ? extractModelTemplateValues(agentTypeInfo.model_template, agent.derouter_params.model)
      : {};

  // Extract credential field values from derouter_params
  for (const field of agentTypeInfo.credential_fields) {
    if (field.include_in_derouter_params !== false) {
      values[field.key] = agent.derouter_params?.[field.key] || field.default_value || "";
    } else if (templateValues[field.key] !== undefined) {
      // For fields not in derouter_params (like agent_runtime_arn), recover from the model string
      values[field.key] = templateValues[field.key];
    }
  }

  // Extract cost configuration
  values.cost_per_query = agent.derouter_params?.cost_per_query;
  values.input_cost_per_token = agent.derouter_params?.input_cost_per_token;
  values.output_cost_per_token = agent.derouter_params?.output_cost_per_token;
