...
const A2A_AGENT = {
  ...
  derouter_params: { model: "gpt-4o", make_public: true, cost_per_query: 0.5 },
  tpm_limit: 111,
  rpm_limit: 222,
...
const langgraphInfo: AgentCreateInfo = {
  ...
  derouter_params_template: { custom_llm_provider: "langgraph" },
  ...
  credential_fields: [
    { key: "api_base", label: "API Base", field_type: "text", required: true },
    {
      ...
      field_type: "text",
      required: false,
      include_in_derouter_params: false,
    },
  ],
};
...
const bedrockAgentcoreInfo: AgentCreateInfo = {
  ...
  logo_url: "/b.png",
  use_a2a_form_fields: false,
  derouter_params_template: { custom_llm_provider: "bedrock" },
...
describe("AgentInfoView update payload", () => {
  ...
  it("sends the loaded values of every panel the user opens", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /DeRouter Parameters/ }));
  ...
  it("preserves the full AgentCore runtime ARN (including the resource id after runtime/) across an unedited save", async () => {
    ...
    expect((patchedPayload().derouter_params as Record<string, unknown>).model).toBe(
