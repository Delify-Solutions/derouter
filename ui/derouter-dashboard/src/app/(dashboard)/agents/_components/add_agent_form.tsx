...
const AddAgentForm: React.FC<AddAgentFormProps> = ({ visible, onClose, accessToken, onSuccess, teams }) => {
  ...
  const buildAgentData = (values: AgentFormValues): AgentRequestPayload | null => {
    ...
    if (selectedAgentTypeInfo.derouter_params_template) {
      agentData.derouter_params = {
        ...agentData.derouter_params,
        ...selectedAgentTypeInfo.derouter_params_template,
      };
    }
    const credentialParams = Object.fromEntries(
      selectedAgentTypeInfo.credential_fields
        .filter((field) => values[field.key] && field.include_in_derouter_params !== false)
        .map((field) => [field.key, values[field.key]]),
    );
    if (Object.keys(credentialParams).length > 0) {
      agentData.derouter_params = { ...agentData.derouter_params, ...credentialParams };
    }
  ...
  const handleCreateAgent = async () => {
    ...
    try {
      ...
      // Wire trace-id flags and budget controls into agent derouter_params (before create call)
      ...
      if (selectedGuardrails.length > 0) {
        agentData.derouter_params = { ...agentData.derouter_params, guardrails: selectedGuardrails };
      }
      ...
      <div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <span className="text-sm font-medium text-foreground">
                Require x-derouter-trace-id on calls TO this agent
              </span>
          ...
          <div className="flex items-center justify-between">
            <div>
              <span className="text-sm font-medium text-foreground">
                Require x-derouter-trace-id on calls BY this agent
              </span>
              <p className="mt-1 text-xs text-muted-foreground">
                Requires LLM/MCP calls made by this agent to include x-derouter-trace-id for session tracking.
              </p>
            </div>
            <Switch
        ...
        <div className="space-y-4">
          {!requireTraceIdOutbound && (
            <div className="rounded-lg border border-warning/20 bg-warning/10 p-3 text-sm text-warning">
              Enable &quot;Require x-derouter-trace-id on calls BY this agent&quot; in Tracing to configure budgets and
              rate limits.
            </div>
          )}
          ...
          <p className="text-xs text-muted-foreground">
            Rate limits per session (x-derouter-trace-id). Each session gets its own counters.
          </p>
          <div className="grid grid-cols-2 gap-4">
