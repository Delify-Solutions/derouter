  ...
  basic: {
    ...
    fields: [
      ...
      },
      {
        ...
        defaultValue: "1.0",
        tooltip:
          "The A2A protocol version DeRouter serves to clients for this agent. DeRouter converts the upstream agent's responses to this version, so clients always see the version you pick here regardless of the original agent's version.",
        helpText:
          "DeRouter serves this version to clients and converts the upstream agent's responses to match it, regardless of the original agent's version.",
      },
    ],
  },
  skills: {
...
export const buildAgentDataFromForm = (values: any, existingAgent?: any) => {
  ...
  if (Object.keys(params).length > 0) {
    agentData.derouter_params = params;
  }
...
export const parseAgentForForm = (agent: any) => {
  ...
  return {
    ...
    model: agent.derouter_params?.model,
    make_public: agent.derouter_params?.make_public,
    cost_per_query: agent.derouter_params?.cost_per_query,
    input_cost_per_token: agent.derouter_params?.input_cost_per_token,
    output_cost_per_token: agent.derouter_params?.output_cost_per_token,
