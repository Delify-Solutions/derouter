...
describe("AddModelPanel submit payload contract", () => {
  ...
  it("registers four more keys as undefined once Advanced Settings opens", async () => {
    ...
    expect(lastCreatedModel()).toStrictEqual({
      model_name: "gpt-4o",
      derouter_params: { ...alwaysMounted, ...advancedOpenExtras },
      model_info: { ...baseModelInfo },
    });
  });

  it("merges typed DeRouter Params into derouter_params", async () => {
    const { user, openAdvanced, fillRequired, submit } = await setup();
    await fillRequired();
    await openAdvanced();
    await user.type(screen.getByLabelText("DeRouter Params"), '{{"rpm": 7}');
    await submit();

    expect(lastCreatedModel()).toStrictEqual({
      model_name: "gpt-4o",
      derouter_params: { ...alwaysMounted, ...advancedOpenExtras, rpm: 7 },
      model_info: { ...baseModelInfo },
    });
  });
  ...
  it("restores the typed value when the section is expanded again", async () => {
    ...
    await closeAdvanced();
    await openAdvanced();
    expect(screen.getByLabelText("DeRouter Params")).toHaveValue('{"rpm": 7}');
...
describe("AddModelPanel empty-string skip", () => {
  ...
  it("sends a typed api_base, so the binding behind the next case is known to be live", async () => {
    ...
    expect(lastCreatedModel().derouter_params).toStrictEqual({
      ...alwaysMounted,
  ...
  it("omits api_base entirely once it is cleared, rather than sending an empty string", async () => {
    ...
    const params = lastCreatedModel().derouter_params;
...
describe("AddModelPanel validation gates", () => {
  ...
  it("blocks the submit when DeRouter Params is not valid JSON", async () => {
    mockPtuEnabled.mockReturnValue(false);
    const { user, openAdvanced, fillRequired, submitExpectingRejection } = await setup();
    await fillRequired();
    await openAdvanced();
...
describe("AddModelPanel behaviours the removed Advanced Settings form instance never drove", () => {
  ...
  it("leaves DeRouter Params untouched when pass through routes is switched on", async () => {
    const { user, openAdvanced, fillRequired, submit } = await setup();
    await fillRequired();
    await openAdvanced();
    await user.click(screen.getByRole("switch", { name: "Use in pass through routes" }));
    expect(screen.getByLabelText("DeRouter Params")).toHaveValue("");

    await submit();

    expect(lastCreatedModel()).toStrictEqual({
      model_name: "gpt-4o",
      derouter_params: { ...alwaysMounted, ...advancedOpenExtras, use_in_pass_through: true },
      model_info: { ...baseModelInfo },
    });
  });
