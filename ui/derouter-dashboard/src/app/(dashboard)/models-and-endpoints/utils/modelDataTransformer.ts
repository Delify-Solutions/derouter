...
export const transformModelData = (rawModelData: any, getProviderFromModel: (model: string) => string) => {
  ...
  for (let i = 0; i < transformedData.length; i++) {
    ...
    let derouter_model_name = curr_model?.derouter_params?.model;
    let custom_llm_provider = curr_model?.derouter_params?.custom_llm_provider;
    ...
    // Check if derouter_model_name is null or undefined
    if (derouter_model_name) {
      // Split derouter_model_name based on "/"
      let splitModel = derouter_model_name.split("/");
      ...
      if (!provider) {
        provider = splitModel.length === 1 ? getProviderFromModel(derouter_model_name) : firstElement;
      }
    } else {
      // derouter_model_name is null or undefined, default provider to openai
      provider = "-";
    }
    ...
    if (curr_model?.derouter_params) {
      cleanedLitellmParams = Object.fromEntries(
        Object.entries(curr_model?.derouter_params).filter(([key]) => key !== "model" && key !== "api_base"),
      );
    }
    ...
    transformedData[i].derouter_model_name = derouter_model_name;
    ...
    transformedData[i].api_base = curr_model?.derouter_params?.api_base;
