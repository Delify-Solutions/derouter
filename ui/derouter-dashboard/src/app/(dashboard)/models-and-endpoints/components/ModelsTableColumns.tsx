...
export const CREDENTIALS_COLUMN_ID = "derouter_credential_name";
...
function ModelInformationCell({ model, displayName }: { model: ModelData; displayName: string }) {
  const derouterModelName = model.derouter_model_name || "-";
    ...
    <HoverCard>
      ...
      >
        ...
        <span className="flex min-w-0 flex-col gap-0.5">
          ...
          <span className="max-w-60 truncate font-mono text-xs text-muted-foreground" title={derouterModelName}>
            {derouterModelName}
          </span>
        </span>
      </HoverCardTrigger>
      <HoverCardContent align="start" className="w-80">
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-0.5">
            <span className="text-xs text-muted-foreground">DeRouter Model Name</span>
            <span className="flex min-w-0 items-center gap-1.5">
              <span className="truncate font-mono text-sm text-foreground" title={derouterModelName}>
                {derouterModelName}
              </span>
              <button
                type="button"
                aria-label="Copy DeRouter model name"
                data-testid={`copy-derouter-model-name-${model.model_info.id}`}
                className="shrink-0 cursor-pointer text-muted-foreground hover:text-foreground"
                onClick={() => void copyToClipboard(derouterModelName, "DeRouter model name copied")}
              >
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1">
              ...
              <span className="text-xs text-muted-foreground">
                Credentials saved in DeRouter that can be added to models repeatedly.
              </span>
            </div>
            <div className="flex flex-col gap-1">
...
export const getModelsTableColumns = ({
  ...
  },
  {
    ...
    accessorFn: (row) => row.derouter_params?.derouter_credential_name ?? "",
    meta: { title: "Credentials" },
    header: () => <CredentialsHeader />,
    enableSorting: false,
    size: 180,
    minSize: 110,
    cell: ({ row }) => <CredentialsCell credentialName={row.original.derouter_params?.derouter_credential_name} />,
  },
  {
