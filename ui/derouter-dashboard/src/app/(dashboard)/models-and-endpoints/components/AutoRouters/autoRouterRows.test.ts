...
describe("autoRouterRows", () => {
    ...
    const row = toAutoRouterRow(
      {
        ...
        derouter_params: {
          ...complexityDeployment.derouter_params,
          complexity_router_config: {
  ...
  it("survives an unparseable config instead of throwing", () => {
    const row = toAutoRouterRow(
      {
        model_name: "broken",
        derouter_params: { model: "auto_router/broken", auto_router_config: "{not json" },
        model_info: { id: "bid-1" },
      },
      0,
      ADMIN,
  ...
  it("falls back to a stable synthetic id when the deployment has no model_info id", () => {
    const rows = toAutoRouterRows(
      [
        { model_name: "a", derouter_params: { model: "auto_router/a" } },
        { model_name: "b", derouter_params: { model: "auto_router/b" } },
      ],
      ADMIN,
      null,
    );
  ...
  const rowFor = (model: string, dbModel: boolean) =>
    toAutoRouterRow(
      { model_name: "r", derouter_params: { model }, model_info: { id: "x", db_model: dbModel } },
      0,
      ADMIN,
      null,
    );
