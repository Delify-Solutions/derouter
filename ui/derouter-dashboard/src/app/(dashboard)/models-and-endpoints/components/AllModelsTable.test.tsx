...
const makeModel = (overrides: Partial<ModelData> = {}): ModelData =>
  ({
    ...
    derouter_model_name: "openai/gpt-4",
    provider: "openai",
...
describe("AllModelsTable", () => {
  ...
  it("renders the model name over the derouter model name", () => {
    render(<AllModelsTable {...baseProps} />);
  ...
  it("renders a reusable credential by name and falls back to Manual", () => {
    const { rerender } = render(
      <AllModelsTable
        {...baseProps}
        data={[makeModel({ derouter_params: { model: "openai/gpt-4", derouter_credential_name: "openai-prod" } })]}
      />,
    );
