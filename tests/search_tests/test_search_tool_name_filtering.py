...
The search_tool_name parameter is used internally by DeRouter to identify
...
def test_search_tool_name_in_all_derouter_params():
    """
    Test that search_tool_name is in all_derouter_params.

    If missing, it gets passed to provider APIs causing errors.
    """
    assert "search_tool_name" in all_derouter_params


def test_filter_out_search_tool_name():
    """
    Test that filter_out_derouter_params correctly filters search_tool_name.
    """
    kwargs = {
    ...
    assert "derouter_call_id" not in filtered
