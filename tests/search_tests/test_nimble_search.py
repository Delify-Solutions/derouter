...
class TestNimbleSearchTransformation:
    ...
    Transformation details are unit-tested in tests/test_derouter/llms/nimble/search/.
    """
    ...
    def test_nimble_search_request_and_response(self):
        ...
        assert call_kwargs["headers"]["X-Client-Source"] == "derouter"
    ...
    def test_provider_specific_params_survive_to_the_wire(self):
        """Nimble-native params must not be eaten by `filter_out_derouter_params`."""
        with patch(
            "derouter.llms.custom_httpx.http_handler.HTTPHandler.post",
            return_value=_mock_response(),
        ) as mock_post:
    ...
    def test_nimble_search_tracks_cost(self):
        with patch(
            ...
            response = derouter.search(query="pricing check", search_provider="nimble")
