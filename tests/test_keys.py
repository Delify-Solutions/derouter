...
async def update_proxy_budget(session):
    ...
    data = {"user_id": "derouter-proxy-budget", "spend": 0}
...
async def chat_completion_streaming(session, key, model="gpt-4"):
    ...
    prompt_tokens = derouter.token_counter(model="gpt-35-turbo", messages=messages)
    data = {
        "model": model,
        "messages": messages,
        "stream": True,
    ...
    completion_tokens = derouter.token_counter(
...
async def test_key_model_list(model_access, model_access_level, model_endpoint):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        team_id = "derouter_dashboard_{}".format(uuid.uuid4())
