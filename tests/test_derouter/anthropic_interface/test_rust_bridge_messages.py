...
rust_messages = importlib.import_module("derouter.rust_bridge.messages")
...
def test_messages_wrapper_returns_none_when_bridge_absent(monkeypatch):
    monkeypatch.setattr(
        importlib.import_module("derouter.rust_bridge"),
        "get_native_bridge",
        lambda: None,
    )
    derouter.rust(True)
...
def _gate(**overrides):
    kwargs = {
        "custom_llm_provider": "azure_ai",
        "derouter_params": GenericDeRouterParams(api_key="sk-azure"),
...
async def test_gate_skips_rust_when_flag_absent():
    ...
    response = await _gate(derouter_params=GenericDeRouterParams(api_key="sk-azure"))
...
async def test_gate_invokes_rust_for_native_anthropic_provider():
    ...
    response = await _gate(
        ...
        derouter_params=GenericDeRouterParams(api_key="sk-ant"),
        api_key="sk-ant",
...
async def test_fake_stream_wraps_rust_response_as_anthropic_sse():
    ...
    assert stream._hidden_params["additional_headers"] == {"x-derouter-rust": "true"}
