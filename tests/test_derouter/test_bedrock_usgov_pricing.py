...
Reference: https://github.com/Delify-Solutions/derouter/issues/27120
"""
...
def test_usgov_east_haiku_profile_mirrors_in_region_row(model_data):
    ...
    assert profile["derouter_provider"] == "bedrock_converse"
    assert {k: v for k, v in profile.items() if k != "derouter_provider"} == {
        k: v for k, v in in_region.items() if k != "derouter_provider"
    }
...
def _non_pricing_fields(info):
    return {k: v for k, v in info.items() if "cost" not in k and k not in ("derouter_provider", "source")}
