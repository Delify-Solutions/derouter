"""Tests for DeRouterSendMessageResponse JSON-RPC normalization."""
...
def test_from_dict_backfills_id_on_agent_error_response():
    ...
    response = DeRouterSendMessageResponse.from_dict(
...
def test_from_dict_preserves_integer_id_echoed_by_upstream():
    ...
    response = DeRouterSendMessageResponse.from_dict(payload, request_id="r1")
...
def test_backfilled_id_keeps_the_request_id_type():
    ...
    from_int = DeRouterSendMessageResponse.from_dict(agent_error, request_id=7)
    from_str = DeRouterSendMessageResponse.from_dict(agent_error, request_id="7")
...
def test_id_accepts_every_member_of_the_json_rpc_union_and_nothing_else():
    ...
    for accepted in ("s1", 42, 0, None):
        assert DeRouterSendMessageResponse(id=accepted).id == accepted
    ...
    for rejected in (True, False, 1.5, ["a"], {"a": 1}):
        try:
            DeRouterSendMessageResponse(id=rejected)
        except Exception:
            continue
...
def test_boolean_id_is_never_relayed_as_an_integer():
    ...
    echoed = DeRouterSendMessageResponse.from_dict({"id": True, "jsonrpc": "2.0", "result": {}})
    backfilled = DeRouterSendMessageResponse.from_dict(agent_error, request_id=True)
