"""Tests for derouter/a2a_protocol/main.py non-streaming send behavior."""
...
import derouter
from derouter.a2a_protocol.main import _send_message, _stream_messages, create_a2a_client
...
async def test_streaming_trace_id_prefers_logging_trace_id():
    """The streaming X-DeRouter-Trace-Id must use the logging object's trace id (same
    ...
    from derouter.a2a_protocol import main as a2a_main
    ...
    logging_obj.derouter_trace_id = "trace-from-logging"
    ...
    assert captured["extra_headers"]["X-DeRouter-Trace-Id"] == "trace-from-logging"
...
def test_streaming_logging_obj_carries_call_type_into_model_call_details():
    ...
    from derouter.a2a_protocol.main import _build_streaming_logging_obj
...
async def _seed_shared_a2a_client(card=_AGENT_CARD, rpc_reply=_RPC_REPLY) -> _RequestRecorder:
    ...
    derouter.in_memory_llm_clients_cache.set_cache(key=_a2a_client_cache_key(DEFAULT_A2A_AGENT_TIMEOUT), value=handler)
...
def isolated_client_cache():
    ...
    yield derouter.in_memory_llm_clients_cache
...
async def test_callers_with_different_headers_reuse_one_pooled_client(isolated_client_cache):
    ...
    assert client_a._derouter_httpx_client is recorder.client
    assert client_b._derouter_httpx_client is recorder.client
    assert client_none._derouter_httpx_client is recorder.client
...
async def test_lowercase_protocol_binding_card_round_trips_the_langgraph_dialect(isolated_client_cache):
    ...
    interface = a2a_client._derouter_agent_card.supported_interfaces[0]
