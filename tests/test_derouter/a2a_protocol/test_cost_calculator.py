...
async def test_asend_message_uses_cost_per_query():
    ...
    derouter.callbacks = [cost_logger]

    # Mock A2A client
    mock_client = MagicMock()
    mock_client._derouter_agent_card = MagicMock()
    mock_client._derouter_agent_card.name = "test-agent"

    mock_request = _make_send_message_request("test-123")

    # Call asend_message with cost_per_query
    with patch(
        "derouter.a2a_protocol.main._execute_a2a_send_with_retry",
...
@pytest.mark.asyncio
async def test_asend_message_uses_cost_per_query_from_derouter_params_dict():
    """
    Proxy passes agent pricing as the derouter_params dict param (not top-level
...
async def test_asend_message_uses_input_output_cost_per_token():
    ...
    derouter.callbacks = [token_cost_logger]
...
async def test_asend_message_passes_agent_id_to_callback():
    ...
    derouter.callbacks = [agent_id_logger]
...
class MetadataLogger(CustomLogger):
    ...
    def __init__(self):
        ...
        self.derouter_params: Optional[dict] = None
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        self.derouter_params = kwargs.get("derouter_params", {})
        self.metadata = self.derouter_params.get("metadata", {})
...
async def test_asend_message_streaming_propagates_metadata():
    ...
    derouter.logging_callback_manager.add_derouter_async_success_callback(metadata_logger)
    ...
    chunks = []
    with patch(
        "derouter.a2a_protocol.main._stream_messages",
...
async def test_asend_message_streaming_triggers_callbacks():
    ...
    derouter.logging_callback_manager.add_derouter_async_success_callback(callback_logger)
    derouter.logging_callback_manager.add_derouter_success_callback(callback_logger)
