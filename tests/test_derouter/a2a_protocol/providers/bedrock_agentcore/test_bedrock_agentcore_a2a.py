...
- Handler passes derouter_params and allows api_base=None
"""
...
class TestTransformation:
    ...
    def test_json_rpc_envelope_structure(self):
        ...
        from derouter.a2a_protocol.providers.bedrock_agentcore.transformation import (
        ...
        url, headers, body = (
            BedrockAgentCoreA2ATransformation.get_url_and_signed_request(
                ...
                derouter_params=SAMPLE_DEROUTER_PARAMS,
                method="message/send",
            )
        )
    ...
    def test_custom_session_id_header(self):
        ...
        params_with_session = {**SAMPLE_DEROUTER_PARAMS, "runtimeSessionId": "a" * 40}
        _, headers, _ = BedrockAgentCoreA2ATransformation.get_url_and_signed_request(
            request_id="req-001",
            params=SAMPLE_PARAMS,
            derouter_params=params_with_session,
        )
    ...
    def test_agent_extra_headers_signed_for_sigv4(self):
        ...
        with patch(
            "derouter.llms.bedrock.chat.agentcore.transformation.AmazonAgentCoreConfig._sign_request",
            new=fake_sign,
        ):
            BedrockAgentCoreA2ATransformation.get_url_and_signed_request(
                request_id="req-001",
                params=SAMPLE_PARAMS,
                derouter_params=derouter_params_no_key,
    ...
    def test_reserved_headers_filtered_from_agent_extra_headers(self):
        ...
        _, headers, _ = BedrockAgentCoreA2ATransformation.get_url_and_signed_request(
            request_id="req-001",
            params=SAMPLE_PARAMS,
            derouter_params=derouter_params_with_user,
            agent_extra_headers={
        ...
        # Runtime user id is the value set from derouter_params, NOT the spoof.
...
def _session_header(params: dict, derouter_params: dict) -> str:
    from derouter.a2a_protocol.providers.bedrock_agentcore.transformation import (
        BedrockAgentCoreA2ATransformation,
    )
...
class TestRequestScopedRuntimeSession:
    ...
    def test_context_id_scoped_to_calling_key(self):
        ...
        derouter_params = {**SAMPLE_DEROUTER_PARAMS, A2A_USER_API_KEY_HASH_PARAM: KEY_HASH}
        assert _session_header(_params_with_context(CONTEXT_ID), derouter_params) == _scoped(CONTEXT_ID, KEY_HASH)

    def test_context_id_used_verbatim_without_principal(self):
        assert _session_header(_params_with_context(CONTEXT_ID), SAMPLE_DEROUTER_PARAMS) == CONTEXT_ID

    def test_same_context_id_reuses_session_and_other_context_isolated(self):
        first = _session_header(_params_with_context(CONTEXT_ID), SAMPLE_DEROUTER_PARAMS)
        second = _session_header(_params_with_context(CONTEXT_ID), SAMPLE_DEROUTER_PARAMS)
    ...
    def test_same_context_id_from_different_keys_is_isolated(self):
        ...
        caller_one = _session_header(params, {**SAMPLE_DEROUTER_PARAMS, A2A_USER_API_KEY_HASH_PARAM: KEY_HASH})
        caller_two = _session_header(
            params, {**SAMPLE_DEROUTER_PARAMS, A2A_USER_API_KEY_HASH_PARAM: "hashed-key-of-caller-two"}
        )
    ...
    def test_context_id_takes_precedence_over_configured_session(self):
        derouter_params = {**SAMPLE_DEROUTER_PARAMS, "runtimeSessionId": "a" * 40}
        assert _session_header(_params_with_context(CONTEXT_ID), derouter_params) == CONTEXT_ID

    def test_configured_session_is_fallback_without_context_id(self):
        derouter_params = {**SAMPLE_DEROUTER_PARAMS, "runtimeSessionId": "a" * 40}
        assert _session_header(SAMPLE_PARAMS, derouter_params) == "a" * 40
        assert _session_header(_params_with_context(""), derouter_params) == "a" * 40

    def test_no_context_id_and_no_config_generates_new_session_per_request(self):
        first = _session_header(SAMPLE_PARAMS, SAMPLE_DEROUTER_PARAMS)
        second = _session_header(SAMPLE_PARAMS, SAMPLE_DEROUTER_PARAMS)
        assert first != second
    ...
    def test_invalid_context_id_rejected_with_clear_error(self, context_id):
        ...
        with pytest.raises(derouter.BadRequestError, match="Invalid AgentCore runtime session id") as exc_info:
            _session_header(_params_with_context(context_id), SAMPLE_DEROUTER_PARAMS)
    ...
    def test_scoped_context_id_shorter_than_33_rejected(self):
        ...
        derouter_params = {**SAMPLE_DEROUTER_PARAMS, A2A_USER_API_KEY_HASH_PARAM: KEY_HASH}
        with pytest.raises(derouter.BadRequestError, match=_scoped("c" * 15, KEY_HASH)):
            _session_header(_params_with_context("c" * 15), derouter_params)
        assert _session_header(_params_with_context("c" * 16), derouter_params) == _scoped("c" * 16, KEY_HASH)

    def test_invalid_configured_session_rejected(self):
        import derouter

        derouter_params = {**SAMPLE_DEROUTER_PARAMS, "runtimeSessionId": "too-short"}
        with pytest.raises(derouter.BadRequestError, match="Invalid AgentCore runtime session id"):
            _session_header(SAMPLE_PARAMS, derouter_params)

    def test_non_string_context_id_falls_back(self):
        derouter_params = {**SAMPLE_DEROUTER_PARAMS, "runtimeSessionId": "a" * 40}
        assert _session_header(_params_with_context(12345), derouter_params) == "a" * 40
    ...
    async def test_context_id_session_header_on_outbound_non_streaming_post(self, httpx_transport):
        ...
        from derouter.a2a_protocol.providers.bedrock_agentcore.config import (
...
class TestNonStreaming:
    ...
    async def test_json_rpc_body_sent_to_agentcore(self):
        ...
        with patch(
            "derouter.a2a_protocol.providers.bedrock_agentcore.handler.get_async_httpx_client"
        ) as mock_get_client:
...
class TestConfigManager:
    ...
    def test_bedrock_returns_config(self):
        ...
        from derouter.a2a_protocol.providers.config_manager import (
...
class TestHandlerIntegration:
    """Test handler.py changes — derouter_params passed through, api_base not required."""

    @pytest.mark.asyncio
    async def test_provider_config_receives_derouter_params(self):
        """Verify handler passes derouter_params to provider config via kwargs."""
        from derouter.a2a_protocol.derouter_completion_bridge.handler import (
        ...
        with patch(
            "derouter.a2a_protocol.derouter_completion_bridge.handler.A2AProviderConfigManager.get_provider_config",
