"""Tests for derouter/a2a_protocol/exception_mapping_utils.py."""
...
from derouter.a2a_protocol import exception_mapping_utils as emu
...
async def test_localhost_retry_reuses_stashed_httpx_client():
    """The retry must reuse the httpx client DeRouter attached at creation (it carries
    ...
    a2a_client._derouter_httpx_client = stashed_httpx_client
    ...
    assert new_client._derouter_httpx_client is stashed_httpx_client
...
async def test_localhost_retry_carries_the_agents_call_context_onto_the_new_client():
    ...
    a2a_client = MagicMock()
    a2a_client._derouter_httpx_client = object()
    a2a_client._derouter_call_context = stashed_context
    new_client = MagicMock()
    ...
    assert result._derouter_call_context is stashed_context


@pytest.mark.asyncio
async def test_localhost_retry_raises_when_no_stashed_client():
    """An externally-supplied client has no DeRouter httpx handle; the retry must fail
    with a clear error instead of excavating a2a-sdk internals."""
    a2a_client = MagicMock(spec=[])  # no _derouter_httpx_client attribute
...
async def test_localhost_retry_raises_when_agent_card_is_none():
    ...
    a2a_client = MagicMock()
    a2a_client._derouter_httpx_client = MagicMock()
...
def test_get_a2a_client_agent_card_reads_sdk_private_card():
    from derouter.a2a_protocol.main import _get_a2a_client_agent_card
...
async def test_stream_with_retry_raises_after_localhost_retries_exhausted():
    ...
    from derouter.a2a_protocol.main import _execute_a2a_stream_with_retry
    ...
    with (
        ...
        patch(
            "derouter.a2a_protocol.main.handle_a2a_localhost_retry",
