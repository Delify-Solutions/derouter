...
_CONFIG = {"paths_to_mutate": ["derouter/proxy/management_endpoints/"], "tests_dir": ["tests/"]}
...
def test_survivors_are_read_out_of_the_verdicts_they_came_with(monkeypatch):
    class _Proc:
        stdout = (
            "derouter.proxy.management_endpoints.key_management_endpoints.x_1: killed\n"
            "derouter.proxy.management_endpoints.key_management_endpoints.x_2: survived\n"
            "derouter.proxy.management_endpoints.key_management_endpoints.x_3: no tests\n"
    ...
    assert results.survivors == (
        "derouter.proxy.management_endpoints.key_management_endpoints.x_2",
    )
...
def test_every_multi_word_verdict_mutmut_can_emit_still_counts(monkeypatch):
    class _Proc:
        stdout = "".join(
            f"derouter.proxy.management_endpoints.key_management_endpoints.x_{i}: {verdict}\n"
