...
import derouter
from derouter.passthrough.main import allm_passthrough_route, llm_passthrough_route
...
def test_bedrock_application_inference_profile_url_encoding():
    ...
    with (
        patch(
            "derouter.utils.ProviderConfigManager.get_provider_passthrough_config",
            return_value=mock_provider_config,
        ),
        patch(
            "derouter.derouter_core_utils.get_derouter_params.get_derouter_params",
            return_value={},
        ),
        patch(
...
def test_llm_passthrough_route_propagates_allm_passthrough_route_to_logging_obj():
    ...
    `get_derouter_params(**kwargs)` and land in the logging object's
    `derouter_params`. Without that, `_is_sync_derouter_request` misclassifies
    ...
    captured_derouter_params: dict = {}

    def _capture_update_env(*args, **kwargs):
        captured_derouter_params.clear()
        captured_derouter_params.update(kwargs.get("derouter_params") or {})
    ...
    assert captured_derouter_params.get("allm_passthrough_route") is True
    assert LitellmLogging._is_sync_derouter_request(captured_derouter_params) is False
