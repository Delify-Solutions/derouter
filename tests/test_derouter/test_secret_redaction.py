...
from derouter.derouter_core_utils.secret_redaction import redact_internal_details, redact_string
...
def _enable_redaction():
    ...
    with patch("derouter._logging._ENABLE_SECRET_REDACTION", True):
        yield


def _capture_logger_output(fn):
    """Run fn with all derouter loggers wired to a StringIO buffer, return output."""
    buf = StringIO()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "postgresql://derouter:host%3A5432%2F%3FAction%3Dconnect%26X-Amz-Signature%3D"
        + "f" * 540
        + "s3cret@db.host:5432/derouter",
    ],
)
...
def test_disable_redaction_passes_secrets_through():
    """When DEROUTER_DISABLE_REDACT_SECRETS=true, secrets pass through."""
    with patch("derouter._logging._ENABLE_SECRET_REDACTION", False):
        record = logging.LogRecord(
            name="test",
            level=logging.DEBUG,
            pathname="",
...
def test_xai_key_redaction_catches_proxy_log_and_config_dump():
    ...
    cases = [
        ("setting derouter.xai_key=xai-test-secret-123456", "xai-test-secret-123456"),
...
def test_module_level_provider_key_redaction_catches_proxy_log_format():
    ...
    cases = [
        ("setting derouter.groq_key=gsk-test-secret-123456", "gsk-test-secret-123456"),
        (
            "setting derouter.openai_key=openai-test-secret-123456",
...
def test_key_name_redaction_catches_secrets_in_dict_repr():
    ...
    cases = [
        ...
        # database_url
        "'database_url': 'postgres://admin:password@db.example.com:5432/derouter'",
        # JSON format
        '"database_url": "postgres://admin:password@db.example.com:5432/derouter"',
        # access_token
...
def test_key_name_redaction_in_general_settings_dict():
    ...
    when logged through the named derouter loggers."""
...
def _capture_from_logger(logger_name: str, emit: Callable[[logging.Logger], None]) -> str:
    ...
    A handler on the root logger stands in for a log-shipping sink derouter does not own.
...
def test_redaction_survives_uvicorn_logging_reconfiguration():
    ...
    The config below is written out rather than imported from the one derouter ships on
...
@pytest.mark.parametrize(
    "text,leaked",
    (
        ("config file /etc/derouter/secrets/db.yaml", "/etc/derouter/secrets/db.yaml"),
        ("home dir /Users/admin/.derouter/master_key.txt", "/Users/admin/.derouter/master_key.txt"),
        ("cache at /var/cache/derouter/tokens.db", "/var/cache/derouter/tokens.db"),
...
def test_redact_internal_details_leaves_public_hostnames_and_routes_alone():
    """derouter's own error messages rely on routes like /v1/models staying legible."""
    safe_strings = (
...
def test_redact_internal_details_layers_on_top_of_credential_redaction():
    text = "postgresql://derouter_internal:S3cr3tPGPass@10.20.30.40:5432/derouter_prod"
    result = redact_internal_details(text)
...
def test_redact_internal_details_drops_embedded_traceback():
    ...
    message = f"derouter.APIConnectionError: MinimaxException - socket hung up\n{raw_tb}"

    result = redact_internal_details(message)

    assert result == "derouter.APIConnectionError: MinimaxException - socket hung up"
...
def test_redact_internal_details_from_client_message_respects_disable_flag():
    with patch("derouter._logging._ENABLE_SECRET_REDACTION", False):  # test-quality-ok: the opt-out flag is the SUT
        text = "config file /etc/derouter/secrets/db.yaml"
