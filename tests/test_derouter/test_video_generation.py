...
import derouter
from derouter.cost_calculator import default_video_cost_calculator
...
from derouter.types.videos.main import VideoObject, VideoResponse
from derouter.videos import main as videos_main
from derouter.videos.main import (
    avideo_generation,
    avideo_status,
    video_generation,
    video_status,
...
class TestVideoGeneration:
    ...
    def test_video_generation_cost_calculation(self):
        ...
        with open(cost_map_path, "r") as f:
            derouter.model_cost = json.load(f)
    ...
    def test_video_generation_cost_with_custom_model_info(self):
        ...
        instead of looking up the global derouter.model_cost map.

        Related: https://github.com/Delify-Solutions/derouter/issues/21907
        """
        model_info = {
    ...
    def test_video_generation_cost_custom_pricing_through_completion_cost(self):
        """Test that custom video pricing flows through completion_cost via derouter_logging_obj.

        This tests the full cost calculation path: completion_cost extracts model_info
        from derouter_logging_obj.derouter_params.metadata.model_info and passes it to
        the video cost calculator.

        Related: https://github.com/Delify-Solutions/derouter/issues/21907
        """
        from derouter.cost_calculator import completion_cost
        ...
        # Create mock derouter_logging_obj with custom pricing
        mock_logging_obj = MagicMock()
        mock_logging_obj.derouter_params = {
            "metadata": {
                "model_info": {
                    "output_cost_per_video_per_second": 0.05,
                }
    ...
    def test_completion_cost_video_custom_pricing_under_derouter_metadata(self):
        """Video routes store deployment model_info under derouter_metadata, not metadata.

        Regression for https://github.com/Delify-Solutions/derouter/issues/36483: custom video
    ...
    def test_completion_cost_video_resolution_tiers_from_cost_map(self, monkeypatch):
        ...
        with open(local_map_path, "r") as f:
            monkeypatch.setattr(derouter, "model_cost", json.load(f))
    ...
    def test_video_generation_uses_api_key_from_derouter_params(self):
        """Test that video generation handler uses api_key from derouter_params when function parameter is None."""
        ...
        with patch.object(config, "validate_environment") as mock_validate:
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch(
                        "derouter.llms.custom_httpx.llm_http_handler._get_httpx_client",
                    ...
                    # Verify validate_environment was called with api_key from derouter_params
    ...
    def test_video_generation_url_generation(self):
        ...
        url = config.get_complete_url(
            model="sora-2", api_base="https://api.openai.com/v1", derouter_params={}
        )
    ...
    def test_video_generation_unsupported_parameters(self):
        ...
        from derouter.videos.utils import VideoGenerationRequestUtils
...
class TestVideoLogging:
    ...
    async def test_video_generation_logging(self):
        ...
        derouter.logging_callback_manager._reset_all_callbacks()
        derouter.callbacks = [custom_logger]
        ...
        with patch.object(
            ...
            response = await derouter.avideo_generation(
...
def test_video_content_handler_passes_variant_to_url():
    ...
    with patch(
        ...
        result = handler.video_content_handler(
            ...
            custom_llm_provider="openai",
            derouter_params=GenericDeRouterParams(api_base="https://api.openai.com/v1"),
            logging_obj=MagicMock(),
            timeout=5.0,
            api_key="sk-test",
            client=mock_client,
...
def test_video_content_respects_api_base_and_api_key_from_kwargs():
    ...
    from derouter.videos.main import video_content

    # Mock the handler to capture derouter_params
    captured_derouter_params = None

    def capture_derouter_params(*args, **kwargs):
        nonlocal captured_derouter_params
        captured_derouter_params = kwargs.get("derouter_params")
        return b"mp4-bytes"

    with patch("derouter.videos.main.base_llm_http_handler") as mock_handler:
        mock_handler.video_content_handler = capture_derouter_params
    ...
    # Verify that api_base and api_key from kwargs were included in derouter_params
    assert captured_derouter_params is not None
    assert (
        captured_derouter_params.get("api_base")
        == "https://test-resource.openai.azure.com/"
    )
    assert captured_derouter_params.get("api_key") == "test-api-key-from-db"
    assert result == b"mp4-bytes"
...
def test_encode_video_id_with_provider_handles_azure_video_prefix():
    ...
    """
    from derouter.types.videos.utils import (
...
class TestVideoListTransformation:
    ...
    def test_transform_video_list_response_encodes_first_id_and_last_id(self):
        ...
        from derouter.types.videos.utils import decode_video_id_with_provider
...
class TestVideoEndpointsProxyLitellmParams:
    """Test that video proxy endpoints (status, content, remix) respect derouter_params from proxy config."""

    @pytest.fixture
    def client_with_vertex_config(self, monkeypatch):
        """Create a test client with a proxy config that includes Vertex AI model with derouter_params."""
        import asyncio
        import tempfile
    ...
    @pytest.mark.asyncio
    async def test_video_status_respects_derouter_params(
        self,
        client_with_vertex_config,
        mock_video_generation_response,
        mock_video_status_response,
    ):
        """Test that video_status endpoint uses derouter_params from proxy config."""
        from unittest.mock import AsyncMock, MagicMock, patch
        ...
        with patch("derouter.proxy.proxy_server.llm_router", mock_router_instance):
            with patch(
    ...
    @pytest.mark.asyncio
    async def test_video_content_respects_derouter_params(
        self,
        client_with_vertex_config,
        mock_video_generation_response,
        mock_video_content_response,
    ):
        """Test that video_content endpoint uses derouter_params from proxy config."""
        from unittest.mock import AsyncMock, MagicMock, patch
...
def test_video_remix_handler_uses_api_key_from_derouter_params():
    """Sync remix handler should fall back to derouter_params api_key when api_key param is None."""
    handler = BaseLLMHTTPHandler()
    config = OpenAIVideoConfig()
...
@pytest.mark.asyncio
async def test_async_video_remix_handler_uses_api_key_from_derouter_params():
    """Async remix handler should fall back to derouter_params api_key when api_key param is None."""
    handler = BaseLLMHTTPHandler()
    config = OpenAIVideoConfig()
...
def test_video_remix_handler_prefers_explicit_api_key():
    """Sync remix handler should prefer explicit api_key over derouter_params."""
    handler = BaseLLMHTTPHandler()
    config = OpenAIVideoConfig()
...
class TestVideoCreateCharacter:
    ...
    def test_video_create_character_transform_response(self):
        ...
        from derouter.types.videos.main import CharacterObject
    ...
    def test_video_create_character_mock_response(self):
        ...
        from derouter.types.videos.main import CharacterObject
        from derouter.videos.main import video_create_character
...
class TestVideoGetCharacter:
    ...
    def test_video_get_character_mock_response(self):
        ...
        from derouter.types.videos.main import CharacterObject
        from derouter.videos.main import video_get_character
...
class TestVideoEdit:
    ...
    def test_video_edit_mock_response(self):
        ...
        from derouter.videos.main import video_edit
...
class TestVideoExtension:
    ...
    def test_video_extension_mock_response(self):
        ...
        from derouter.videos.main import video_extension
...
def test_video_create_character_target_model_names_returns_encoded_id(
    ...
    from derouter.types.videos.utils import decode_character_id_with_provider
...
async def test_edit_and_extension_read_cached_body_after_auth_consumes_stream(
    ...
    import derouter.proxy.video_endpoints.endpoints as endpoints
