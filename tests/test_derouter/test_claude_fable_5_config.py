...
Foundry), and the ``supports_adaptive_thinking`` flag is what makes DeRouter send
...
def test_fable_5_present_in_bundled_backup():
    ...
    ``DEROUTER_LOCAL_MODEL_COST_MAP=True``) — it must carry the same entries as
...
def test_fable_5_all_variants_carry_adaptive_thinking_flag(cost_map):
    ...
    so adaptive is the only valid thinking shape DeRouter can emit for it."""
