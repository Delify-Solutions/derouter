...
model key in derouter.model_cost.
...
from derouter.derouter_core_utils.ptu_pricing import ptu_config_error
...
def _simulate_price_data_reload(fetched_catalog):
    """Drive what a price data reload does to this process's derouter state.

    Mirrors `derouter.proxy.proxy_server._swap_in_model_cost_map`, which is the
    ...
    """
    derouter.model_cost = fetched_catalog
    _invalidate_model_cost_lowercase_map()
    reapply_runtime_model_cost_registrations()
...
def test_should_not_pollute_shared_key_with_zero_cost_pricing():
    ...
    builtin_info = derouter.get_model_info(model=backend_model)
...
def test_should_store_full_pricing_under_deployment_model_id():
    ...
    retrievable via the unique model_id key in derouter.model_cost.
    """
    backend_model = "vertex_ai/gemini-2.5-flash"
    ...
    entry = derouter.model_cost.get("deployment-zero-check")
...
def test_responses_prefix_stripped_alias_registered_for_model_list():
    """
    Register ``derouter.model_cost`` under the backend key with ``responses/`` and
    ...
    assert "azure/responses/gpt-strip-test-a1b2c3d4" in derouter.model_cost
    assert "azure/gpt-strip-test-a1b2c3d4" in derouter.model_cost
    assert derouter.model_cost["azure/gpt-strip-test-a1b2c3d4"].get("supports_native_streaming") is True
...
def test_responses_prefix_stripped_alias_registered_for_add_deployment():
    ...
    assert "azure/responses/gpt-add-strip-e5f6a7b8" in derouter.model_cost
    assert "azure/gpt-add-strip-e5f6a7b8" in derouter.model_cost
    assert derouter.model_cost["azure/gpt-add-strip-e5f6a7b8"].get("supports_native_streaming") is True
...
def test_should_not_downgrade_chatgpt_shared_key_mode_with_alias_override():
    ...
    """
    from derouter.main import responses_api_bridge_check

    backend_model = "chatgpt/gpt-5.4"
    model_keys = {
        backend_model: copy.deepcopy(derouter.model_cost.get(backend_model)),
        "chatgpt-shared-mode-base": copy.deepcopy(derouter.model_cost.get("chatgpt-shared-mode-base")),
        "chatgpt-shared-mode-alias": copy.deepcopy(derouter.model_cost.get("chatgpt-shared-mode-alias")),
    }

    try:
        backend_entry = copy.deepcopy(model_keys[backend_model]) or {}
        backend_entry["derouter_provider"] = "chatgpt"
        backend_entry["mode"] = "responses"
        derouter.model_cost[backend_model] = backend_entry
        _invalidate_model_cost_lowercase_map()
        ...
        assert derouter.model_cost[backend_model]["mode"] == "responses"
        assert "mode" in derouter.model_cost[backend_model]
...
def test_partial_custom_pricing_inherits_builtin_cache_pricing():
    ...
    model_keys = {
        deploy_id: derouter.model_cost.get(deploy_id),
        backend_model: copy.deepcopy(derouter.model_cost.get(backend_model)),
    }
    try:
        Router(
        ...
        entry = derouter.model_cost[deploy_id]
...
def test_inherit_builtin_base_rates_for_off_peak_fills_missing_rates():
    ...
    builtin_info = derouter.get_model_info(model=backend_model, custom_llm_provider="openai")
    off_peak_block = {
...
def test_inherit_builtin_base_rates_for_off_peak_carries_companion_billing_fields():
    ...
    raw_entry = derouter.model_cost[backend_model]
...
def test_inherit_builtin_base_rates_for_off_peak_tiered_only_backend_stores_no_zero():
    ...
    raw_tiers = derouter.model_cost[backend_model]["tiered_pricing"]
...
def test_custom_pricing_field_denylist_covers_all_builtin_pricing_fields():
    ...
    CustomPricingDeRouterParams enumerating every per-deployment pricing field.
    ...
    from derouter.types.utils import CustomPricingDeRouterParams, ModelInfoBase
    ...
    denylisted_fields = set(CustomPricingDeRouterParams.model_fields.keys())

    uncovered = sorted(builtin_pricing_fields - denylisted_fields)
    assert not uncovered, (
        "ModelInfoBase pricing fields missing from CustomPricingDeRouterParams; "
...
def test_tiered_pricing_override_isolated_from_sibling_via_model_info_lookup():
    ...
    via derouter.get_model_info (the path /model/info uses) without seeing the
    override.
    """
    ...
    model_keys = {
        "lit3897-tiered-custom": derouter.model_cost.get("lit3897-tiered-custom"),
        "lit3897-tiered-sibling": derouter.model_cost.get("lit3897-tiered-sibling"),
        backend_model: copy.deepcopy(derouter.model_cost.get(backend_model)),
    }
    try:
        Router(
        ...
        shared = derouter.get_model_info(model=backend_model)
        ...
        custom_entry = derouter.model_cost["lit3897-tiered-custom"]
...
def test_custom_pricing_isolated_from_sibling_via_proxy_model_info_path():
    ...
    ticket config where the override is set on derouter_params.
    """
    ...
    model_keys = {
        "lit3897-proxy-custom": derouter.model_cost.get("lit3897-proxy-custom"),
        "lit3897-proxy-sibling": derouter.model_cost.get("lit3897-proxy-sibling"),
        backend_model: copy.deepcopy(derouter.model_cost.get(backend_model)),
    }
    try:
        router = Router(
...
def test_custom_model_info_metadata_not_leaked_to_shared_backend_key():
    ...
    derouter.model_cost (served raw by /public/derouter_model_cost_map);
    ...
    model_keys = {
        key: copy.deepcopy(derouter.model_cost.get(key))
    ...
    try:
        ...
        entry_a = derouter.model_cost["lit4544-deploy-a"]
        assert entry_a["additionalProp1"] == {"restricted": False, "model_location": "EU"}
        entry_b = derouter.model_cost["lit4544-deploy-b"]
...
def test_add_deployment_does_not_leak_custom_metadata_to_shared_backend_key():
    ...
    model_keys = {key: copy.deepcopy(derouter.model_cost.get(key)) for key in (*shared_keys, deploy_id)}
    try:
        router = Router(model_list=[])
        ...
        assert derouter.model_cost[deploy_id]["access_via_team_ids"] == ["team-dynamic"]
    finally:
        _restore_model_cost_entries(model_keys)
...
def test_shared_backend_model_info_keeps_schema_fields_and_drops_the_rest():
    ...
    """
    from derouter.types.utils import shared_backend_model_info
...
def test_capability_flags_propagate_from_deployment_model_info_to_shared_key():
    ...
    derouter.model_cost and document proxy model_info as an override path for
    ...
    model_keys = {key: copy.deepcopy(derouter.model_cost.get(key)) for key in (bare_model, backend_model, deploy_id)}
    try:
        Router(
            model_list=[
                {
        ...
        shared_entry = derouter.model_cost.get(backend_model) or {}
        ...
        assert mantle_supports_responses(bare_model, derouter.model_cost) is True
        assert mantle_base_segment(bare_model, derouter.model_cost) == "openai/v1"
    finally:
        _restore_model_cost_entries(model_keys)
...
def test_wildcard_zero_cost_request_does_not_poison_named_deployment_pricing():
    ...
    model_keys = {
        shared_key: copy.deepcopy(derouter.model_cost.get(shared_key)),
        "text-embedding-3-small": copy.deepcopy(derouter.model_cost.get("text-embedding-3-small")),
        "openai/*": copy.deepcopy(derouter.model_cost.get("openai/*")),
        "lit3991-named": derouter.model_cost.get("lit3991-named"),
        "lit3991-wildcard": derouter.model_cost.get("lit3991-wildcard"),
    }
    builtin_input_cost = derouter.get_model_info(model=shared_key)["input_cost_per_token"]
    ...
    try:
        ...
        assert derouter.get_model_info(model=shared_key)["input_cost_per_token"] == builtin_input_cost, (
        ...
        named_cost = derouter.completion_cost(completion_response=named_response, call_type="embedding")
...
def test_price_data_reload_preserves_router_registered_model_info(monkeypatch):
    """
    A price-data reload replaces derouter.model_cost wholesale. Deployment
    ...
    try:
        _simulate_price_data_reload(
            {"gpt-4o": {"derouter_provider": "openai", "mode": "chat"}},
        )
...
def test_deleted_deployments_are_not_replayed_onto_later_reloads(monkeypatch):
    ...
    router = Router(
        model_list=[
            {
                "model_name": "doomed",
                "derouter_params": {"model": "hosted_vllm/shared-backend"},
            ...
            {
                "model_name": "solo",
                "derouter_params": {"model": "hosted_vllm/solo-backend"},
    ...
    try:
        ...
        assert "doomed-id" not in derouter.model_cost
        assert "solo-id" not in derouter.model_cost
        assert "hosted_vllm/solo-backend" not in derouter.model_cost

        surviving = derouter.model_cost["kept-id"]
        assert surviving["max_input_tokens"] == 222
        assert "hosted_vllm/shared-backend" in derouter.model_cost
    finally:
        derouter.model_cost = saved_model_cost
        _invalidate_model_cost_lowercase_map()
...
def test_deleting_a_deployment_leaves_catalog_pricing_for_its_backend_model(monkeypatch):
    ...
    backend_model = "gemini/gemini-2.5-pro"
    catalog_entry = derouter.get_model_info(model=backend_model)
    ...
    saved_catalog = derouter.model_cost
    fetched_catalog = copy.deepcopy(derouter.model_cost)
    try:
        router = Router(
            model_list=[
                {
                    "model_name": "doomed-gemini",
                    "derouter_params": {"model": backend_model, "api_key": "sk-fake"},
        ...
        assert "doomed-gemini-id" not in derouter.model_cost
        assert derouter.model_cost[backend_model]["input_cost_per_token"] == catalog_input_cost
    finally:
        derouter.model_cost = saved_catalog
        _invalidate_model_cost_lowercase_map()
...
def test_repointing_a_deployment_drops_its_previous_backend_key(monkeypatch):
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "hosted_vllm/old-backend"},
    ...
    try:
        router.upsert_deployment(
            deployment=Deployment(
                model_name="moving-target",
                derouter_params=DeRouter_Params(model="hosted_vllm/new-backend"),
        ...
        assert "hosted_vllm/old-backend" not in derouter.model_cost
        assert "hosted_vllm/new-backend" in derouter.model_cost
        assert "moving-target-id" in derouter.model_cost
    finally:
        derouter.model_cost = saved_model_cost
        _invalidate_model_cost_lowercase_map()
    ...
    try:
        kept = Router(
            model_list=[
                {
                    "model_name": "kept",
                    "derouter_params": {"model": "hosted_vllm/kept-backend"},
            ...
            model_list=[
                {
                    ...
                    "derouter_params": {"model": "hosted_vllm/throwaway-backend"},
        ...
        assert "throwaway-router-id" not in derouter.model_cost
        assert "hosted_vllm/throwaway-backend" not in derouter.model_cost
        assert derouter.model_cost["kept-router-id"]["max_input_tokens"] == 4242
        assert "hosted_vllm/kept-backend" in derouter.model_cost
...
def test_a_reload_rebuilds_exactly_what_a_fresh_boot_registered():
    ...
    custom pricing carried on derouter_params, and the cache pricing inherited from
    ...
    try:
        ...
        at_boot = copy.deepcopy(derouter.model_cost["priced-id"])
        ...
        rebuilt = derouter.model_cost["priced-id"]
    ...
    try:
        router = Router(
            model_list=[
                {
                    "model_name": "healthy",
                    "derouter_params": {"model": "hosted_vllm/healthy-backend"},
                    "model_info": {"id": "healthy-id", "max_input_tokens": 777},
                }
            ],
        )
        router.model_list.insert(0, {"derouter_params": {}})

        derouter.model_cost = {"gpt-4o": {"derouter_provider": "openai", "mode": "chat"}}
        _invalidate_model_cost_lowercase_map()
        router._replay_model_cost_registrations()

        assert derouter.model_cost["healthy-id"]["max_input_tokens"] == 777
    finally:
        derouter.model_cost = saved_model_cost
        _invalidate_model_cost_lowercase_map()


def test_deployment_model_cost_payload_folds_in_derouter_params_pricing():
    """
    Custom pricing is configured on derouter_params but has to land in the
...
def test_register_deployment_in_model_cost_writes_both_key_families():
    ...
    """
    model_keys = {
        "both-families-id": copy.deepcopy(derouter.model_cost.get("both-families-id")),
        "hosted_vllm/both-families-backend": copy.deepcopy(derouter.model_cost.get("hosted_vllm/both-families-backend")),
    }
    try:
        Router._register_deployment_in_model_cost(
            model_id="both-families-id",
            model_info={"id": "both-families-id", "max_input_tokens": 999, "derouter_provider": "hosted_vllm"},
            model="hosted_vllm/both-families-backend",
            custom_llm_provider=None,
        )

        assert derouter.model_cost["both-families-id"]["max_input_tokens"] == 999
        shared = derouter.model_cost["hosted_vllm/both-families-backend"]
...
def test_reload_keeps_custom_pricing_configured_on_derouter_params_for_a_db_model():
    """
    A deployment added at runtime, which is what /model/new does, configures its
    custom pricing on derouter_params rather than on model_info. A price data
    ...
    try:
        ...
        assert derouter.model_cost["db-priced-id"]["input_cost_per_token"] == 0.000123
        ...
        assert derouter.model_cost["db-priced-id"]["input_cost_per_token"] == 0.000123
        assert derouter.model_cost["db-priced-id"]["output_cost_per_token"] == 0.000456
    finally:
        derouter.model_cost = saved_catalog
        _invalidate_model_cost_lowercase_map()
...
def test_replay_live_router_model_cost_rebuilds_every_live_router():
    ...
    """
    from derouter.router import _replay_live_router_model_cost
    ...
    try:
        first = Router(
            model_list=[
                {
                    "model_name": "first",
                    "derouter_params": {"model": "hosted_vllm/first-backend"},
        ...
        second = Router(
            model_list=[
                {
                    "model_name": "second",
                    "derouter_params": {"model": "hosted_vllm/second-backend"},
        ...
        assert derouter.model_cost["first-id"]["max_input_tokens"] == 111
        assert derouter.model_cost["second-id"]["max_input_tokens"] == 222
...
def test_strategy_router_alias_pricing_never_enters_model_cost(monkeypatch):
    ...
    def _assert_alias_unpriced():
        entry = derouter.model_cost.get("strategy-alias-id")
...
def test_inherit_builtin_tiered_output_rate_fills_the_backend_flat_rate():
    ...
    backend_rate = derouter.get_model_info(model="claude-haiku-4-5", custom_llm_provider="anthropic")[
...
def test_inherit_builtin_tiered_output_rate_never_stores_a_synthesized_zero():
    ...
    """
    backend_info = derouter.get_model_info(model="qwen-flash", custom_llm_provider="dashscope")
...
def _ptu_router(model_info=None, derouter_params=None, ptu_enabled=True):
    """A router built the way loading config.yaml builds one."""
    with patch.dict(os.environ, {"DEROUTER_ENABLE_PTU_COST_ATTRIBUTION": "True" if ptu_enabled else ""}, clear=False):
        return Router(
            model_list=[
                {
...
def test_a_config_ptu_deployment_bills_nothing_per_token():
    ...
    router = _ptu_router(derouter_params={"input_cost_per_token": 5e-06, "output_cost_per_token": 1.5e-05})
    entry = router.model_list[0]

    assert entry["derouter_params"]["input_cost_per_token"] == 0.0
    assert entry["derouter_params"]["output_cost_per_token"] == 0.0
    assert entry["model_info"]["input_cost_per_token"] == 0.0
    assert derouter.model_cost[entry["model_info"]["id"]]["input_cost_per_token"] == 0.0
...
def test_a_config_ptu_deployment_imports_no_cache_rate_from_its_backend(backend):
    ...
    builtin = derouter.get_model_info(model=backend)
    assert any(builtin.get(field) for field in cache_fields), "backend publishes no cache pricing to leak"

    router = _ptu_router(derouter_params={"model": backend})
    priced = derouter.model_cost[router.model_list[0]["model_info"]["id"]]
...
def test_zeroing_a_ptu_deployment_leaves_its_backend_model_priced():
    ...
    builtin = derouter.get_model_info(model=backend)["input_cost_per_token"]
    assert builtin > 0

    _ptu_router(derouter_params={"model": backend})

    assert derouter.get_model_info(model=backend)["input_cost_per_token"] == builtin
...
def test_the_registered_id_is_the_one_the_operator_declared():
    ...
    priced = _ptu_router(derouter_params=params, ptu_enabled=False).model_list[0]["model_info"]["id"]
    zeroed = _ptu_router(derouter_params=params).model_list[0]["model_info"]["id"]
...
def test_a_database_backed_deployment_is_left_alone():
    ...
    assert entry["derouter_params"].get("input_cost_per_token") is None
...
def test_nothing_is_zeroed_while_the_feature_is_off():
    ...
    entry = _ptu_router(derouter_params={"input_cost_per_token": 5e-06}, ptu_enabled=False).model_list[0]

    assert entry["derouter_params"]["input_cost_per_token"] == 5e-06
...
def test_an_incomplete_reservation_is_refused_rather_than_served(dropped):
    ...
    with pytest.raises(ValueError, match="PTU configuration on model 'gpt") as raised:
        _ptu_router(model_info=incomplete, derouter_params={"input_cost_per_token": 5e-06})
...
def test_an_incomplete_reservation_is_left_alone_while_the_feature_is_off(dropped):
    ...
    incomplete = {k: v for k, v in _PTU_MODEL_INFO.items() if k != dropped}
    entry = _ptu_router(
        model_info=incomplete, derouter_params={"input_cost_per_token": 5e-06}, ptu_enabled=False
    ).model_list[0]
...
def test_the_proxy_drops_the_deployment_rather_than_failing_to_boot(dropped):
    ...
    incomplete = {k: v for k, v in _PTU_MODEL_INFO.items() if k != dropped}
    with patch.dict(os.environ, {"DEROUTER_ENABLE_PTU_COST_ATTRIBUTION": "True"}, clear=False):
        router = Router(
            model_list=[
                {
                    "model_name": "gpt-4o-ptu",
                    "derouter_params": {"model": "anthropic/claude-sonnet-4-5-20250929", "api_key": "sk-not-used"},
...
def test_nested_custom_model_info_does_not_pollute_shared_backend():
    ...
    builtin_info = copy.deepcopy(derouter.get_model_info(model=backend_model))
    expected_nested = copy.deepcopy(builtin_info["search_context_cost_per_query"])
    model_keys = {
        backend_model: copy.deepcopy(derouter.model_cost.get(backend_model)),
        custom_id: copy.deepcopy(derouter.model_cost.get(custom_id)),
        sibling_id: copy.deepcopy(derouter.model_cost.get(sibling_id)),
    }
    try:
        router = Router(
            model_list=[
                {
                    ...
                    "derouter_params": {"model": backend_model, "api_key": "fake-key"},
                    "model_info": {
                        "id": custom_id,
        ...
        assert derouter.model_cost[backend_model]["search_context_cost_per_query"] == expected_nested
...
def test_base_model_custom_info_does_not_pollute_cached_base_model():
    ...
    base_model_info = copy.deepcopy(derouter.get_model_info(model=base_model))
    model_keys = {
        "azure/gpt-4o": copy.deepcopy(derouter.model_cost.get("azure/gpt-4o")),
        deployment_id: copy.deepcopy(derouter.model_cost.get(deployment_id)),
    }
    try:
        router = Router(
            model_list=[
        ...
        assert derouter.get_model_info(model=base_model) == base_model_info
    finally:
        _restore_model_cost_entries(model_keys)
        derouter.get_model_info.cache_clear()
...
def test_builtin_only_deployment_info_is_not_the_cached_object():
    ...
    derouter.get_model_info.cache_clear()
    model_keys = {deployment_id: copy.deepcopy(derouter.model_cost.get(deployment_id))}
    try:
        cached_info = derouter.get_model_info(model=backend_model)
...
def test_custom_only_deployment_info_is_not_the_registry_entry():
    ...
    model_keys = {
        unknown_backend: copy.deepcopy(derouter.model_cost.get(unknown_backend)),
        deployment_id: copy.deepcopy(derouter.model_cost.get(deployment_id)),
    }
    try:
        router = Router(
            model_list=[
                {
                    ...
                    "derouter_params": {"model": unknown_backend, "api_key": "fake-key"},
                    "model_info": {"id": deployment_id, "search_context_cost_per_query": dict(nested_pricing)},
                }
            ],
        )
        registry_entry = derouter.model_cost[deployment_id]
...
def test_router_model_info_deep_copies_nested_cached_metadata():
    ...
    derouter.get_model_info.cache_clear()
    try:
        cached_info = derouter.get_model_info(model=model)
        ...
        merged_info = router.get_router_model_info(
            deployment={
                "model_name": "search",
                "derouter_params": {"model": "gpt-4o-search-preview"},
        ...
        assert derouter.get_model_info(model=model)["search_context_cost_per_query"] == expected_nested
    finally:
        derouter.get_model_info.cache_clear()
...
def test_a_reservation_without_a_declared_id_is_refused():
    """Left underived the id is a hash of the resolved derouter_params, so rotating the
                ...
                model_list=[
                    {
                        ...
                        "derouter_params": {"model": "azure/gpt-4o", "api_key": "k", "api_base": "https://e.azure.com"},
                        "model_info": dict(_PTU_MODEL_INFO),
                    },
                    {
                        "model_name": "azure-ptu-west",
                        "derouter_params": {"model": "azure/gpt-4o", "api_key": "k", "api_base": "https://w.azure.com"},
                        "model_info": dict(_PTU_MODEL_INFO),
                    },
                ]
            )
...
def test_a_duplicate_id_is_caught_whatever_yaml_parsed_it_as(declared):
    ...
    def entry(name, region):
        return {
            "model_name": name,
            "derouter_params": {"model": "azure/gpt-4o", "api_key": "k", "api_base": f"https://{region}.azure.com"},
...
def _ptu_warnings(caplog):
    return tuple(
        ...
        if record.name == "DeRouter Router" and record.levelno == logging.WARNING and "PTU" in record.getMessage()
    )
...
def test_a_reservation_declared_while_the_feature_is_off_is_warned_about(caplog):
    ...
    assert "DEROUTER_ENABLE_PTU_COST_ATTRIBUTION" in warnings[0]
...
def test_a_reservation_dropped_by_a_typo_is_warned_about(caplog, typo):
    ...
    """
    with patch.dict(os.environ, {"DEROUTER_ENABLE_PTU_COST_ATTRIBUTION": ""}, clear=False):
        with caplog.at_level(logging.WARNING, logger="DeRouter Router"):
            router = Router(
                ignore_invalid_deployments=True,
                model_list=[
            ...
            Router(
                model_list=[
                    ...
                    },
                    {
                        ...
                        "derouter_params": {"model": "azure/gpt-4o", "api_key": "k", "api_base": "https://p.azure.com"},
                        "model_info": {"id": "plain"},
                    },
                ]
            )
...
def _simulate_price_data_reload_with_provider_sets(monkeypatch, fetched_catalog):
    ...
    monkeypatch.setattr(derouter, "model_cost", fetched_catalog)
    _invalidate_model_cost_lowercase_map()
    derouter.add_known_models(model_cost_map=fetched_catalog)
    reapply_runtime_model_cost_registrations()
...
def test_a_config_deployment_dropped_by_a_stale_cost_map_comes_back_on_reload(monkeypatch):
    ...
    try:
            ...
            model_list=[
                {
                    ...
                    "derouter_params": {"model": backend, "api_key": "k"},
                    "model_info": {"id": "new-id", "access_groups": ["team-models"]},
                },
                {
                    "model_name": "control-model",
                    "derouter_params": {"model": "hosted_vllm/control-backend", "api_key": "k"},
        ...
        fresh_catalog = {**derouter.model_cost, backend: {"derouter_provider": "openai", "mode": "chat"}}
        ...
        assert "new-id" in derouter.model_cost
    finally:
        derouter.open_ai_chat_completion_models.discard(backend)
        derouter.models_by_provider["openai"].discard(backend)
...
def test_a_config_deployment_dropped_for_a_permanent_reason_is_not_retried_on_reload(monkeypatch):
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "vertex_ai/gemini-2.5-flash", "use_in_pass_through": True},
    ...
    _simulate_price_data_reload_with_provider_sets(monkeypatch, dict(derouter.model_cost))
