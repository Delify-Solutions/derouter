"""
Tests for derouter.acount_tokens() public API.
"""
...
def test_acount_tokens_routes_to_openai():
    ...
    with patch(
        "derouter.llms.openai.responses.count_tokens.token_counter.openai_count_tokens_handler.handle_count_tokens_request",
        new_callable=AsyncMock,
        return_value={"input_tokens": 15},
    ):
        result = asyncio.run(
            derouter.acount_tokens(
                model="openai/gpt-4o",
...
def test_acount_tokens_routes_to_anthropic():
    ...
    with patch(
        "derouter.llms.anthropic.count_tokens.token_counter.anthropic_count_tokens_handler.handle_count_tokens_request",
        new_callable=AsyncMock,
