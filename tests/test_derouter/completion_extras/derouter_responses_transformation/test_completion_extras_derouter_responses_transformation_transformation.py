...
def test_convert_chat_completion_messages_to_responses_api_image_input():
    ...
    handler = DeRouterResponsesTransformationHandler()
...
and I learn to carry this small calm home."""
    ...
    derouter_params = {"acompletion": False, "api_key": None}
    encoding = Mock()
...
def test_recover_output_items_merges_text_only_items_at_distinct_indices():
    ...
    recovered = (
        DeRouterResponsesTransformationHandler._recover_output_items_from_raw_sse(
            raw_sse
        )
    )
...
def test_map_reasoning_effort_adds_summary_detailed(monkeypatch):
    ...
    original_flag = derouter.reasoning_auto_summary
    original_env = os.environ.get("DEROUTER_REASONING_AUTO_SUMMARY")

    try:
        # Test 1: Default behavior (flag=False, no env var) - NO summary
        derouter.reasoning_auto_summary = False
        if "DEROUTER_REASONING_AUTO_SUMMARY" in os.environ:
            del os.environ["DEROUTER_REASONING_AUTO_SUMMARY"]
        ...
        print("✓ DEROUTER_REASONING_AUTO_SUMMARY env var works correctly")
        ...
        from derouter.types.llms.openai import Reasoning
        ...
        derouter.reasoning_auto_summary = original_flag
        if original_env is not None:
            monkeypatch.setenv("DEROUTER_REASONING_AUTO_SUMMARY", original_env)
        elif "DEROUTER_REASONING_AUTO_SUMMARY" in os.environ:
            del os.environ["DEROUTER_REASONING_AUTO_SUMMARY"]
...
def test_apply_patch_tool_call_converted_to_chat_completion_tool_call():
    ...
    This is a regression test for a bug where derouter.completion() with a
    ...
    native derouter.responses() path worked correctly.
    """
...
def test_map_optional_params_preserves_reasoning_summary():
    ...
    from derouter.types.llms.openai import ResponsesAPIOptionalRequestParams
...
def test_transform_request_bedrock_mantle_tools_keeps_reasoning_effort(monkeypatch, reasoning_effort):
    ...
    monkeypatch.setattr(derouter, "reasoning_auto_summary", False)
    monkeypatch.delenv("DEROUTER_REASONING_AUTO_SUMMARY", raising=False)
    handler: Final = DeRouterResponsesTransformationHandler()
    ...
    result: Final = handler.transform_request(
        ...
        derouter_params={"custom_llm_provider": "bedrock_mantle"},
        headers={},
...
def test_convert_chat_completion_file_type_to_input_file():
    ...
    Regression test for https://github.com/Delify-Solutions/derouter/issues/23588
    """
    from derouter.completion_extras.derouter_responses_transformation.transformation import (
        DeRouterResponsesTransformationHandler,
    )
...
def test_convert_response_output_custom_tool_call_to_tool_calls_choice():
    ...
    choices = DeRouterResponsesTransformationHandler._convert_response_output_to_choices([item])
...
def test_convert_response_output_accumulates_raw_tool_calls_into_one_choice():
    ...
    choices = DeRouterResponsesTransformationHandler._convert_response_output_to_choices(
        items,
...
def test_convert_response_output_generic_pydantic_message_item():
    """derouter's completion bridge (used for non-Responses-native providers behind the
...
def test_assistant_message_with_tool_calls_keeps_its_content():
    """Regression for https://github.com/Delify-Solutions/derouter/issues/24985.
...
def _derouter_encoded_response_id(upstream_id: str) -> str:
...
def test_transform_response_keeps_upstream_id_and_provider_extras():
    ...
    raw_response = ResponsesAPIResponse.model_validate(
        {
            "id": _derouter_encoded_response_id("resp_azure_123"),
            "created_at": 1734366691,
            "object": "response",
            "model": "gpt-5.6",
            "status": "completed",
    ...
    result = DeRouterResponsesTransformationHandler().transform_response(
        model="gpt-5.6",
        raw_response=raw_response,
        model_response=model_response,
        logging_obj=Mock(),
...
def test_bridged_response_is_priced_by_the_reported_service_tier():
    ...
    pricing = derouter.model_cost["gpt-5.4"]
    ...
    cost = derouter.completion_cost(completion_response=result, custom_llm_provider="openai")
...
def test_streaming_chunks_carry_the_upstream_response_id():
    ...
    encoded_id = _derouter_encoded_response_id("resp_azure_stream")
    events = [
