"""
Regression test for https://github.com/Delify-Solutions/derouter/issues/28505 -
...
Root cause: the bridge handler called `derouter.responses()` /
`derouter.aresponses()` without passing the already-resolved
...
def test_sync_completion_forwards_custom_llm_provider():
    ...
    handler.transformation_handler.transform_request.return_value = {
        "model": "openai/openai/openai/gpt-5.5",
        "input": [],
        # `_build_sanitized_derouter_params` spreads `custom_llm_provider` from
        # `derouter_params` into request_data on the real bridge path.  Seed
    ...
    with (
        ...
        assert kwargs.get("custom_llm_provider") == "openai", (
            "sync bridge must forward custom_llm_provider to derouter.responses() "
...
async def test_async_completion_forwards_custom_llm_provider():
    ...
    with (
        ...
        patch("derouter.aresponses", _fake_aresponses),
    ):
        try:
        ...
        assert _fake_aresponses.kwargs.get("custom_llm_provider") == "openai", (
            "async bridge must forward custom_llm_provider to derouter.aresponses() "
            "so the downstream get_llm_provider() call does not re-strip the "
            "provider prefix on a provider/provider/model deployment string"
        )
