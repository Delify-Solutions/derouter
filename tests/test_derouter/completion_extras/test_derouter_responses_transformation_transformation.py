...
def test_transform_request_with_response_format():
    ...
    derouter_logging_obj = MockLoggingObj()
...
def test_transform_request_strips_internal_metadata_to_derouter_metadata():
    handler = DeRouterResponsesTransformationHandler()
    messages = [{"role": "user", "content": "Hello"}]
    optional_params = {}
    derouter_params = {
    ...
    assert result["derouter_metadata"]["user_api_key_auth"]["id"] == "abc"
...
def test_transform_request_preserves_user_metadata():
    ...
    derouter_params = {"metadata": {"internal_key": "secret"}}
    ...
    assert result["derouter_metadata"]["internal_key"] == "secret"
