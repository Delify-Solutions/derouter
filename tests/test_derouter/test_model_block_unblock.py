...
def _setup_model_block_mocks(monkeypatch, *, updated_blocked: bool):
    ...
    mock_prisma_client.db.derouter_proxymodeltable = model_table
...
async def test_model_block_endpoint_sets_blocked_true(monkeypatch):
    ...
    result = await block_model(
        ...
        derouter_changed_by="operator@example.com",
    )
    ...
    assert (
        mock_audit_log.call_args.kwargs["derouter_changed_by"] == "operator@example.com"
    )
...
def test_router_returns_no_healthy_deployment_when_model_is_fully_blocked():
    router = derouter.Router(
        model_list=[
            {
                "model_name": "gpt-4o",
                "derouter_params": {"model": "openai/gpt-4o-0"},
                "model_info": {"id": "dep-0", "blocked": True},
            },
            {
                "model_name": "gpt-4o",
                "derouter_params": {"model": "openai/gpt-4o-1"},
...
async def test_route_request_returns_403_when_model_is_fully_blocked(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.proxy.route_llm_request.add_shared_session_to_data",
        AsyncMock(return_value=None),
    )

    with pytest.raises(derouter.PermissionDeniedError) as exc_info:
        await route_request(
            data={"model": "gpt-4o"},
            llm_router=router,
            user_model=None,
...
async def test_model_block_surfaces_wholesale_reload_failure(monkeypatch):
    ...
    from derouter.proxy.management_endpoints.model_management_endpoints import block_model
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", wiped_router)
...
async def test_model_block_surfaces_model_dropped_by_reload(monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", dropped_router)
