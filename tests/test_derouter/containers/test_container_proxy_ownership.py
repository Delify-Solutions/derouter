...
from derouter.types.containers.main import ContainerListResponse, ContainerObject
...
def _owned_containers_in_db(monkeypatch, *model_object_ids: str) -> AsyncMock:
    ...
    monkeypatch.setattr(
        ownership,
        "_get_prisma_client",
        AsyncMock(return_value=SimpleNamespace(db=SimpleNamespace(derouter_managedobjecttable=table))),
    )
    return table
...
async def test_should_validate_owner_and_forward_decoded_id_for_multipart_upload(
    ...
    monkeypatch.setitem(sys.modules, "derouter.proxy.proxy_server", proxy_server_stub)
...
async def test_should_forward_decoded_container_id_for_proxy_retrieve(monkeypatch):
    from derouter.proxy.container_endpoints import endpoints
