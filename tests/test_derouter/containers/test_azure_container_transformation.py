...
import derouter
from derouter.llms.azure.containers.transformation import AzureContainerConfig
...
class TestAzureContainerConfig:
    ...
    def setup_method(self):
        ...
        self.logging_obj = DeRouterLogging(
            model="",
            messages=[],
            stream=False,
            call_type="create_container",
            start_time=None,
            derouter_call_id="test_call_id",
    ...
    def test_get_complete_url_strips_responses_path_and_preserves_api_version(self):
        ...
        url = self.config.get_complete_url(
            api_base=api_base,
            derouter_params={"api_version": "2024-08-01-preview"},
        )
        ...
        ), "must use version from api_base, not derouter_params"
        assert (
    ...
    def test_get_complete_url_raises_without_api_base(self, monkeypatch):
        ...
        with pytest.raises(ValueError, match="api_base is required"):
            self.config.get_complete_url(api_base=None, derouter_params={})
    ...
    def test_proxy_handler_factory_returns_azure_config(self):
        from derouter.proxy.container_endpoints.handler_factory import (
...
class TestAzureContainerKnownFailureRegressions:
    ...
    2. **Bare resource root** — ``AZURE_API_BASE`` is only the host (no ``?``). The
       query appears only after DeRouter builds the full container base; downstream
    ...
    async def test_proxy_process_request_forwards_decoded_container_id(
        ...
        from derouter.proxy.container_endpoints import handler_factory
    ...
    async def test_regression_binary_file_request_routes_through_proxy_processor(
        ...
        async def _mock_base_process_llm_request(
            ...
            **kwargs,
            ...
            captured["data"] = self.data
            captured["route_type"] = route_type
            fastapi_response.headers["x-derouter-call-id"] = "call-123"
            return b"csv-bytes"
        ...
        assert response.headers["x-derouter-call-id"] == "call-123"
    ...
    async def test_regression_multipart_upload_request_uses_provider_from_managed_id(
        ...
        from derouter.proxy.common_utils import http_parsing_utils
        from derouter.proxy.container_endpoints import handler_factory
    ...
    async def test_regression_get_container_forwarding_params_sets_model_id_for_managed_id(
        ...
        DeRouter-managed encoded container ID and include it in the forwarding
    ...
    async def test_regression_get_container_forwarding_params_recovers_model_id_for_native_id(
        ...
        from derouter.proxy.container_endpoints import ownership
        from derouter.proxy.container_endpoints.ownership import (
            get_container_forwarding_params,
        )
        ...
        prisma_client = SimpleNamespace(
            db=SimpleNamespace(derouter_managedobjecttable=table)
        )
        monkeypatch.setattr(
            ownership,
            "_get_prisma_client",
    ...
    async def test_regression_native_azure_container_id_uses_forwarded_model_id(
        ...
        """Native Azure container IDs (cntr_ + hex, no DeRouter payload) must
        ...
        assert called_with.get("model") == "deployment-uuid-123", (
            ...
            "model_id when the container_id carries no DeRouter routing payload"
        )
