...
from derouter.containers.main import (
    create_container,
    acreate_container,
    list_containers,
    alist_containers,
...
class TestContainerIntegration:
    ...
    @patch("derouter.llms.custom_httpx.llm_http_handler.HTTPHandler")
    ...
    @pytest.mark.asyncio
    @patch("derouter.llms.custom_httpx.llm_http_handler.AsyncHTTPHandler")
    ...
    def test_container_workflow_simulation(self):
        ...
        with patch("derouter.containers.main.base_llm_http_handler") as mock_handler:
    ...
    def test_error_handling_integration(self):
        ...
        import importlib
        import derouter.containers.main as containers_main_module

        # Reload the module to ensure it has a fresh reference to base_llm_http_handler
        # after conftest reloads derouter
        importlib.reload(containers_main_module)

        # Re-import the function after reload
        from derouter.containers.main import create_container as create_container_fresh

        with patch("derouter.containers.main.base_llm_http_handler") as mock_handler:
            # Simulate an API error
            mock_handler.container_create_handler.side_effect = derouter.APIError(
                status_code=400,
    ...
    def test_provider_support(self, provider):
        ...
        # Reload the module to ensure it has a fresh reference to base_llm_http_handler
        # after conftest reloads derouter (same pattern as test_error_handling_integration)
        importlib.reload(containers_main_module)
