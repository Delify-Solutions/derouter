...
from derouter.derouter_core_utils import thread_pool_executor as thread_pool_executor_module
from derouter.responses import streaming_iterator as responses_streaming_iterator_module
...
def _make_logging_obj() -> LitellmLogging:
    logging_obj = LitellmLogging(
        ...
        derouter_call_id="lit-4210-test",
        function_id="lit-4210-test",
    )
    logging_obj.model_call_details["derouter_params"] = {"aresponses": True}
    return logging_obj
...
async def test_custom_logger_only_never_submits_sync_success_handler(recording_executor):
    ...
    derouter.success_callback = [recorder]
    derouter._async_success_callback = [recorder]
...
async def test_sync_callbacks_run_only_after_async_handler_completes(recording_executor):
    ...
    derouter.success_callback = [recorder, sync_callback]
    derouter._async_success_callback = [recorder]
