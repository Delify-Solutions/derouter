...
def test_logging_object_not_popped():
    """
    Test that derouter_logging_obj is not popped from kwargs.
    ...
    from derouter.responses import main as responses_module

    # Get the source code of the responses function
    source = inspect.getsource(responses_module.responses)

    # Check that .pop("derouter_logging_obj") is NOT used
    # The bug was using kwargs.pop("derouter_logging_obj") which removes it
    assert 'kwargs.pop("derouter_logging_obj")' not in source, (
        "FAIL: Found kwargs.pop('derouter_logging_obj') in responses() function. "
        "This causes duplicate spend logs. Use kwargs.get('derouter_logging_obj') instead."
    )

    # Check that .get("derouter_logging_obj") IS used
    assert 'kwargs.get("derouter_logging_obj")' in source, (
        "FAIL: Expected kwargs.get('derouter_logging_obj') but not found. "
...
async def test_async_no_duplicate_spend_logs():
    ...
    class SpendLogCounter(CustomLogger):
        ...
        async def async_log_success_event(
            ...
            if derouter_call_id == self.tracking_id:
                self.log_count += 1
    ...
    derouter.callbacks = original_callbacks + [spend_logger]

    try:
        # Call responses API with Anthropic model using mock_response
        # Pass our unique ID as derouter_call_id to track this specific request
        response = await derouter.aresponses(
            ...
            derouter_call_id=test_request_id,
        )
