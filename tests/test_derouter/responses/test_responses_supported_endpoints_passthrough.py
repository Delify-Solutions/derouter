...
from derouter.responses.main import _resolve_responses_api_provider_config
...
def _respx_interceptable_httpx_client(monkeypatch):
    monkeypatch.setattr(derouter, "num_retries", 0)
...
async def test_opt_in_forwards_streaming_responses_natively(monkeypatch):
    """The router registers each deployment in `derouter.model_cost`; an unregistered model is
    treated as non-streaming and would be faked, so mirror that registration here."""
    monkeypatch.setitem(derouter.model_cost, "custom_openai/my-model", {"derouter_provider": "custom_openai"})
...
async def test_prompt_swap_to_other_provider_drops_deployment_opt_in(monkeypatch):
    ...
    result = await derouter.aresponses(
        ...
        input="hi",
        prompt_id="p1",
        derouter_logging_obj=_prompt_manager_swapping_to(SWAPPED_MODEL),
        model_info=OPT_IN,
    )
...
async def test_mode_responses_chat_completion_reaches_native_responses(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        {"mode": "responses", "derouter_provider": "custom_openai"},
    )
