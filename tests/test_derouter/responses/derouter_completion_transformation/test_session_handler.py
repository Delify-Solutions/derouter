...
import derouter
from derouter.responses.derouter_completion_transformation import session_handler
...
async def test_get_chat_completion_message_history_for_previous_response_id():
    ...
    with patch.object(
        ...
        assert "derouter_session_id" in result

        # Verify session_id is extracted correctly
        assert result["derouter_session_id"] == "a96757c4-c6dc-4c76-b37e-e7dfa526b701"
...
async def test_get_chat_completion_message_history_empty_spend_logs():
    ...
    with patch.object(
        ...
        assert result.get("derouter_session_id") is None
...
async def test_e2e_cold_storage_successful_retrieval():
    ...
    mock_spend_logs = [
        {
            ...
            "proxy_server_request": '{"derouter_truncated": true}',  # Truncated payload
            "response": {
    ...
    with (
        ...
        assert result.get("derouter_session_id") == "session-456"
...
async def test_e2e_cold_storage_fallback_to_truncated_payload():
    ...
    with (
        ...
        # Verify result structure
        assert result.get("derouter_session_id") == "session-999"
        assert len(result.get("messages", [])) >= 1  # At least the assistant response
...
async def test_should_check_cold_storage_for_full_payload():
    ...
    proxy_request_with_truncated_pdf = {
        ...
        "stream": True,
        "derouter_trace_id": "16b86861-c120-4ecb-865b-4d2238bfd8f0",
    }
    ...
    with patch("derouter.cold_storage_custom_logger", return_value="s3"):
...
async def test_get_chat_completion_message_history_empty_response_dict():
    ...
    with patch.object(
        ...
        assert result["derouter_session_id"] == "test-session"
...
def instant_session_lookup_retries(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(derouter.constants, "RESPONSES_SESSION_LOOKUP_RETRY_INTERVAL", 0.0)
...
async def test_message_history_retries_a_spend_log_the_batch_writer_has_not_flushed_yet(
    ...
    with patch("derouter.proxy.proxy_server.prisma_client", fake_prisma_client):
        result = await ResponsesSessionHandler.get_chat_completion_message_history_for_previous_response_id(
            request_id
        )
    ...
    assert result["derouter_session_id"] == session_id
...
async def test_session_lookup_stops_retrying_once_the_budget_is_spent(
    ...
    assert spend_logs == []
    assert len(fake_prisma_client.db.calls) == derouter.constants.RESPONSES_SESSION_LOOKUP_MAX_ATTEMPTS
...
async def test_session_lookup_does_not_retry_when_spend_logs_are_disabled(
    ...
    with patch("derouter.proxy.proxy_server.prisma_client", fake_prisma_client), patch(
        "derouter.proxy.proxy_server.disable_spend_logs", True
    ):
        spend_logs = await ResponsesSessionHandler.get_all_spend_logs_for_previous_response_id(
            "chatcmpl-does-not-exist"
        )
...
def test_normalize_redacted_arguments_skips_custom_tool_calls():
    ...
    message = Message(
        ...
        tool_calls=[
            ...
            {"id": "call_f", "type": "function", "function": {"name": "get_weather", "arguments": "redacted-by-derouter"}},
        ],
    )
