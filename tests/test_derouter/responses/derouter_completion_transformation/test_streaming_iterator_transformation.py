...
derouter/responses/derouter_completion_transformation/streaming_iterator.py.

Ensures that when the underlying chat-completions stream includes tool_calls deltas,
DeRouter emits Responses API streaming events (output_item.added + function_call_arguments.*).
...
def _build_iterator(chunks) -> DeRouterCompletionStreamingIterator:
    return DeRouterCompletionStreamingIterator(
        model="claude-haiku-4-5",
        derouter_custom_stream_wrapper=_FakeStreamWrapper(chunks),
...
def test_tool_call_delta_is_emitted_as_responses_events():
    iterator = DeRouterCompletionStreamingIterator(
        model="test-model",
        derouter_custom_stream_wrapper=AsyncMock(),
...
def test_tool_calls_present_only_in_final_response_are_emitted_before_completed():
    ...
    # We bypass the stream builder and directly set iterator.derouter_model_response.
    ...
    iterator.derouter_model_response = response
