...
config), it must tag the forwarded ``derouter.completion`` / ``derouter.acompletion``
...
module-level ``derouter.completion`` / ``derouter.acompletion`` are patched to
...
def test_sync_fallback_tags_skip_responses_api_bridge():
    ...
    with patch("derouter.completion", fake_completion):
...
async def test_async_fallback_tags_skip_responses_api_bridge():
    ...
    with patch("derouter.acompletion", fake_acompletion):
