...
blocks back through ``encrypted_content``, which DeRouter itself writes as a
...
def _transform_item(item):
    return DeRouterCompletionResponsesConfig._transform_responses_api_input_item_to_chat_completion_message(
        input_item=item, replay_reasoning=True
    )


def _transform_input(input_items):
    return DeRouterCompletionResponsesConfig._transform_response_input_param_to_chat_completion_message(
...
class TestReasoningInputItemHandler:
    ...
    def test_reasoning_item_with_opaque_encrypted_content_dropped(self):
        """An encrypted blob DeRouter did not write cannot be forwarded."""
...
class TestReasoningInputItemMerging:
    ...
    def test_reasoning_merged_into_assistant_with_existing_reasoning_content(self):
        ...
        messages = DeRouterCompletionResponsesConfig._merge_reasoning_only_assistant_messages(
            [
...
class TestEncryptedReasoningRoundTrip:
    """``encrypted_content`` DeRouter wrote decodes back into thinking blocks."""
    ...
    def test_encoded_thinking_blocks_decode_back(self):
        ...
        encoded = DeRouterCompletionResponsesConfig._encode_thinking_blocks(message)
        decoded = DeRouterCompletionResponsesConfig._decode_thinking_blocks_from_input_item(
...
class TestInspectionCallersStillSeeReasoningText:
    ...
    def test_reasoning_moves_off_content_only_for_provider_bound_callers(self):
        ...
        provider_bound = DeRouterCompletionResponsesConfig.transform_responses_api_input_to_messages(
