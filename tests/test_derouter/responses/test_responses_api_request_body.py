"""
Test that derouter.responses() / derouter.aresponses() send the expected request body
...
def _expected_dir() -> Path:
    """Path to expected_responses_api_request folder (sibling of test_derouter/responses)."""
...
async def test_aresponses_context_management_and_shell_request_body_matches_expected():
    """
    Call derouter.aresponses() with context_management and shell tool;
...
async def test_aresponses_azure_shell_tool_request_body_matches_expected():
    """
    Call derouter.aresponses() on the Azure route with the shell tool;
...
async def test_aresponses_azure_shell_tool_400_maps_to_bad_request_error():
    ...
    derouter must surface that as derouter.BadRequestError carrying the provider message.
    """
    error_body = {
        "error": {
...
async def test_aresponses_strips_responses_routing_prefix_from_openai_model(model, custom_llm_provider):
    """
    `responses/` is DeRouter routing sugar, never part of the provider model id.
...
async def test_aresponses_websocket_strips_responses_routing_prefix_from_openai_model():
    ...
    from derouter.responses.main import _aresponses_websocket

    with patch.object(
        import_module("derouter.responses.main").base_llm_http_handler, "async_responses_websocket",
        new_callable=AsyncMock,
    ) as mock_ws:
...
async def test_aresponses_derouter_proxy_target_sends_no_openai_markers():
    body = await _aresponses_body_with_system_point(model="derouter_proxy/gpt-5.6", api_base=_CUSTOM_API_BASE)
    assert body["input"] == _INJECTION_POINT_INPUT
    assert "prompt_cache_options" not in body
...
async def test_instructions_are_marked_when_the_bridge_builds_the_system_message(points, content):
    ...
    derouter's own marks as client breakpoints and stands the whole configuration down.
    """
