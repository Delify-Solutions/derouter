...
class TestUseResponsesApiBridgeFlag:
    ...
    def test_bridge_used_when_use_chat_completions_api_true(
        ...
        mock_get_config.return_value = derouter.OpenAIResponsesAPIConfig()
    ...
    @patch.object(import_module("derouter.responses.main").base_llm_http_handler, "response_api_handler")
    @patch.object(
        import_module("derouter.responses.main").ProviderConfigManager, "get_provider_responses_api_config"
    )
    ...
    async def test_allowed_openai_params_forwarded_through_bridge(
        ...
        explicitly forwarded to the bridge; otherwise derouter.acompletion raises
    ...
    @patch.object(import_module("derouter.responses.file_search.emulated_handler"), "_call_aresponses")
    @patch.object(
        import_module("derouter.responses.main").ProviderConfigManager, "get_provider_responses_api_config"
    )
    ...
    @patch("derouter.vector_stores.main.asearch")
    @patch.object(
        import_module("derouter.responses.main").ProviderConfigManager, "get_provider_responses_api_config"
    )
    async def test_bridge_flag_prevents_native_responses_endpoint_call(
        ...
        result = await derouter.aresponses(
            model="openai/my-local-model",
            input="Search for information",
            tools=[
                {
