...
def _make_logging_obj(
    ...
    """Return a mock DeRouterLoggingObj pre-configured for prompt management."""
    if merged_optional_params is None:
        merged_optional_params = {}
    logging_obj = MagicMock()
    logging_obj.__class__ = DeRouterLoggingObj
...
def _patch_responses_dispatch():
    ...
    return [
        ...
        patch.object(
            import_module("derouter.responses.mcp.derouter_proxy_mcp_handler").DeRouter_Proxy_MCP_Handler, "_should_use_derouter_mcp_gateway",
            return_value=False,
        ),
        patch.object(
...
def test_resolve_prompt_swapped_provider_raises_cross_provider_with_credentials():
    import derouter
    from derouter.responses.main import _resolve_prompt_swapped_provider

    with pytest.raises(derouter.BadRequestError, match="Refusing to send"):
...
def test_sync_prompt_swap_resolves_credentials_for_swapped_provider(monkeypatch: pytest.MonkeyPatch):
    ...
    with patch.object(  # test-quality-ok: handler boundary stub proves creds resolve for the swapped provider without network
        import_module("derouter.responses.main").base_llm_http_handler, "response_api_handler", return_value=MagicMock()
    ) as mock_handler:
        derouter.responses(input="hi", model="xai/grok-4", prompt_id="p1", derouter_logging_obj=logging_obj)
    ...
    assert handler_kwargs["derouter_params"].api_base is None
    assert handler_kwargs["derouter_params"].api_key != "sk-xai-test"


def test_sync_prompt_swap_cross_provider_with_credentials_raises():
    import derouter
    from derouter.responses.main import _apply_prompt_management_to_responses_call
