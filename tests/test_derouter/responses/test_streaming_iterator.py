...
def _logging_obj_stub() -> Mock:
    logging_obj = Mock(spec=DeRouterLoggingObj)
...
def test_stream_cache_write_completes_when_asyncio_run_closes_the_loop(monkeypatch):
    ...
    logging_obj = SimpleNamespace(
        model_call_details={"derouter_params": {}},
    ...
    monkeypatch.setattr(derouter, "cache", _SlowWriteCache())
...
def test_run_post_success_hooks_does_not_report_generation_time_as_overhead():
    ...
    that duration is token generation, not DeRouter overhead."""
    logging_obj = _logging_obj_stub()
    logging_obj.model_call_details = {"derouter_params": {}, "llm_api_duration_ms": 200.0}
    ...
    assert "derouter_overhead_time_ms" not in iterator.completed_response._hidden_params
...
def test_stamp_responses_usage_cost_stamps_computed_cost():
    from derouter.responses.streaming_iterator import _stamp_responses_usage_cost
...
def _capture_dispatch(logged: list):
    ...
    ``Mock(spec=DeRouterLoggingObj).dispatch_success_handlers`` is an AsyncMock whose side effect
