...
raise derouter.APIError directly; 429 and 5xx are wrapped in
...
def _make_iterator() -> BaseResponsesAPIStreamingIterator:
    ...
    mock_logging_obj.model_call_details = {"derouter_params": {}}
...
def test_maybe_raise_for_error_event_wraps_unknown_error_in_mid_stream_fallback():
    ...
    assert isinstance(exc_info.value.original_exception, derouter.APIError)
...
def test_handle_logging_failed_response_maps_rate_limit_to_429():
    ...
    with (
        patch.object(import_module("derouter.responses.streaming_iterator"), "run_async_function") as mock_run_async,
        patch.object(import_module("derouter.responses.streaming_iterator"), "executor"),
    ):
        iterator._handle_logging_failed_response()
    logged_exception = mock_run_async.call_args.kwargs["exception"]
    assert isinstance(logged_exception, derouter.APIError)
...
def test_handle_logging_failed_response_records_usage_and_cost():
    ...
    with (
        patch.object(import_module("derouter.responses.streaming_iterator"), "run_async_function"),
        patch.object(import_module("derouter.responses.streaming_iterator"), "executor"),
    ):
        iterator._handle_logging_failed_response()
    combined_usage = iterator.logging_obj.model_call_details["combined_usage_object"]
    assert isinstance(combined_usage, derouter.Usage)
