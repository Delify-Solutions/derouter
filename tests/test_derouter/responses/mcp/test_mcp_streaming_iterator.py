...
import derouter  # noqa: F401 - ensures derouter.responses.main is registered in sys.modules
from derouter.responses.mcp.mcp_streaming_iterator import (
    MAX_MCP_TOOL_CALL_ROUNDS,
    MCPEnhancedStreamingIterator,
)
from derouter.types.llms.openai import ResponsesAPIResponse, ResponsesAPIStreamEvents

# `derouter.__init__` re-exports a function named `responses`, which shadows the
# `derouter.responses` subpackage as an attribute — `import derouter.responses.main`
...
responses_main_module = sys.modules["derouter.responses.main"]
...
def _make_iterator(initial_chunks) -> MCPEnhancedStreamingIterator:
    return MCPEnhancedStreamingIterator(
        ...
        mcp_tools_with_derouter_proxy=[{"require_approval": "never"}],
        user_api_key_auth=None,
        original_request_params={
            "model": "gpt-4",
            "input": "what is berriai/derouter?",
            "tools": [{"type": "mcp"}],
        },
    )
