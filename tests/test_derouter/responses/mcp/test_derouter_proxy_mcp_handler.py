...
from derouter.proxy._experimental.mcp_server.faults.list_outcomes import AggregateToolListing
from derouter.responses import main as responses_main
from derouter.responses.mcp import derouter_proxy_mcp_handler as mcp_handler_module
...
from derouter.types.responses.main import OutputFunctionToolCall
...
def _setup_mcp_call_environment(monkeypatch: pytest.MonkeyPatch) -> AsyncMock:
    ...
    monkeypatch.setitem(sys.modules, "derouter.proxy.proxy_server", proxy_module)
...
def test_deduplicate_mcp_tools_single_allowed_server():
    ...
    deduped, server_map = DeRouter_Proxy_MCP_Handler._deduplicate_mcp_tools(
        tools,
        ["everything"],
    )
...
def test_deduplicate_mcp_tools_prefixed_names(tool_name, expected_server):
    ...
    _, server_map = DeRouter_Proxy_MCP_Handler._deduplicate_mcp_tools(
        tools,
        ["alpha", "beta"],
    )
...
def test_extract_tool_calls_from_chat_response_handles_tool_calls():
    ...
    tool_calls = DeRouter_Proxy_MCP_Handler._extract_tool_calls_from_chat_response(
        response
    )
...
def test_create_follow_up_messages_for_chat_appends_tool_results():
    ...
    follow_up = DeRouter_Proxy_MCP_Handler._create_follow_up_messages_for_chat(
        original_messages,
        response,
        tool_results,
    )
...
def test_transform_mcp_tools_to_openai_uses_chat_format(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.experimental_mcp_client.tools.transform_mcp_tool_to_openai_tool",
        fake_transform_chat,
    )
    monkeypatch.setattr(
        "derouter.experimental_mcp_client.tools.transform_mcp_tool_to_openai_responses_api_tool",
        fake_transform_responses,
    )

    chat_tools = DeRouter_Proxy_MCP_Handler._transform_mcp_tools_to_openai(
        ["tool"], target_format="chat"
    )
    resp_tools = DeRouter_Proxy_MCP_Handler._transform_mcp_tools_to_openai(["tool"])
...
def test_create_follow_up_input_handles_response_function_tool_call():
    ...
    follow_up = DeRouter_Proxy_MCP_Handler._create_follow_up_input(
...
async def test_execute_tool_calls_strips_server_prefix(monkeypatch):
    ...
    await DeRouter_Proxy_MCP_Handler._execute_tool_calls(
...
async def test_execute_tool_calls_strips_prefix_when_alias_differs_from_server_name(
    ...
    from derouter.proxy._experimental.mcp_server import mcp_server_manager as _msm
...
async def test_execute_tool_calls_logs_failure_via_post_call_failure_hook(monkeypatch):
    ...
    results = await DeRouter_Proxy_MCP_Handler._execute_tool_calls(
        tool_server_map={tool_name: "deepwiki"},
        tool_calls=tool_calls,
        user_api_key_auth=user_auth,
        derouter_call_id="cid",
        derouter_trace_id="tid",
    )
...
@pytest.mark.asyncio
async def test_execute_tool_calls_passes_derouter_call_id_and_trace_id_to_function_setup(
    monkeypatch,
):
    """
    Regression test for ae4d92ad...:
    Ensure derouter_call_id / derouter_trace_id are forwarded into function_setup kwargs.
    """
    ...
    # NOTE: Don't patch via dotted string path here because `derouter.responses`
    # is a function attribute on the `derouter` package (shadowing the submodule),
    # which breaks monkeypatch's importpath resolution.
    handler_module = importlib.import_module(
        "derouter.responses.mcp.derouter_proxy_mcp_handler"
    )
    ...
    assert captured.get("derouter_call_id") == "cid"
    assert captured.get("derouter_trace_id") == "tid"


@pytest.mark.asyncio
async def test_execute_tool_calls_threads_logging_obj_into_call_tool(monkeypatch):
    """The Responses-API MCP path must hand the request's derouter_logging_obj to
    ...
    guardrail evaluations in the monitor. Drop the derouter_logging_obj kwarg on the
    ...
    handler_module = importlib.import_module("derouter.responses.mcp.derouter_proxy_mcp_handler")
    monkeypatch.setattr(
        handler_module,
        "function_setup",
    ...
    assert call_tool_mock.await_count == 1
    assert call_tool_mock.await_args is not None
    assert call_tool_mock.await_args.kwargs["derouter_logging_obj"] is sentinel_logging_obj
...
async def test_get_mcp_tools_from_manager_enables_list_tools_logging(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.proxy._experimental.mcp_server.server._get_tools_from_mcp_servers",
        mock_get_tools,
    )
    ...
    user_auth = types.SimpleNamespace(api_key="test_key", user_id="test_user")
    tools, _server_names = await DeRouter_Proxy_MCP_Handler._get_mcp_tools_from_manager(
        user_api_key_auth=user_auth,
        mcp_tools_with_derouter_proxy=[
            {"type": "mcp", "server_url": "derouter_proxy/mcp/deepwiki"}
        ],
    )
...
def test_get_parent_request_tags_from_metadata():
    tags = DeRouter_Proxy_MCP_Handler._get_parent_request_tags(
        {"metadata": {"tags": ["team-a", "prod"]}}
    )
    assert tags == ["team-a", "prod"]


def test_get_parent_request_tags_from_nested_derouter_params():
    tags = DeRouter_Proxy_MCP_Handler._get_parent_request_tags(
        {
...
async def test_execute_tool_calls_exposes_sanitized_client_headers_to_logging(monkeypatch):
    ...
    await DeRouter_Proxy_MCP_Handler._execute_tool_calls(
        ...
        tool_calls=[{"id": "call-1", "function": {"name": tool_name, "arguments": "{}"}}],
        user_api_key_auth=None,
        raw_headers={"x-nuid": "nuid-1", "x-derouter-api-key": "sk-proxy", "cookie": "s=1"},
    )
...
def test_extract_tool_call_details_reads_anthropic_tool_use_input():
    ...
    tool_use_block = {
        ...
        "input": {"repoName": "Delify-Solutions/derouter"},
    }

    name, arguments, call_id = DeRouter_Proxy_MCP_Handler._extract_tool_call_details(tool_use_block)

    assert name == "read_wiki_structure"
    assert call_id == "toolu_01ABC"
    assert arguments == {"repoName": "Delify-Solutions/derouter"}
    assert DeRouter_Proxy_MCP_Handler._parse_tool_arguments(arguments) == {"repoName": "Delify-Solutions/derouter"}
...
def test_extract_tool_call_details_still_prefers_openai_arguments():
    ...
    name, arguments, call_id = DeRouter_Proxy_MCP_Handler._extract_tool_call_details(openai_tool_call)
...
EXPLICIT_GATEWAY_TOOL = {"type": "mcp", "server_label": "github", "server_url": "derouter_proxy/mcp/github"}
...
async def test_gateway_served_names_matches_alias_server_name_name_access_group_and_toolset():
    from derouter.responses.mcp.derouter_proxy_mcp_handler import _gateway_served_names
...
async def test_gateway_served_names_matches_server_id_short_prefix_and_alias_case_like_the_gateway():
    from derouter.proxy._experimental.mcp_server.utils import compute_short_server_prefix
    from derouter.responses.mcp.derouter_proxy_mcp_handler import _gateway_served_names
...
async def test_routes_through_gateway_flags_explicit_and_served_tools_only():
    ...
    flags = await DeRouter_Proxy_MCP_Handler.routes_through_gateway(
...
async def test_split_mcp_tools_leaves_external_mcp_path_urls_for_the_provider():
    ...
    gateway_tools, other_tools = await DeRouter_Proxy_MCP_Handler._split_mcp_tools(
...
async def test_split_mcp_tools_repoints_served_proxy_urls_at_the_gateway():
    ...
    assert gateway_tools == [{**served_tool, "server_url": "derouter_proxy/mcp/my-toolset"}]
...
def test_should_use_gateway_still_triggers_on_http_mcp_path():
    assert DeRouter_Proxy_MCP_Handler._should_use_derouter_mcp_gateway([ZAPIER_TOOL]) is True
    assert DeRouter_Proxy_MCP_Handler._should_use_derouter_mcp_gateway([EXPLICIT_GATEWAY_TOOL]) is True
    assert DeRouter_Proxy_MCP_Handler._should_use_derouter_mcp_gateway([FUNCTION_TOOL]) is False
    assert DeRouter_Proxy_MCP_Handler._should_use_derouter_mcp_gateway(None) is False
...
async def test_aresponses_api_with_mcp_forwards_unserved_external_mcp_tool_to_the_provider(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "derouter.proxy.proxy_server", types.SimpleNamespace(prisma_client=None))
...
def test_is_persistence_disabled(call_params: dict[str, Any], expected: bool):
    assert DeRouter_Proxy_MCP_Handler._is_persistence_disabled(call_params) is expected
...
async def test_mcp_follow_up_call_is_stateless_when_store_is_false(
    ...
    monkeypatch.setattr(
        DeRouter_Proxy_MCP_Handler, "_process_mcp_tools_without_openai_transform", staticmethod(fake_process)
    )
    monkeypatch.setattr(DeRouter_Proxy_MCP_Handler, "_execute_tool_calls", staticmethod(fake_execute))

    await responses_main.aresponses_api_with_mcp(
        input="hi",
        model="gpt-5",
        tools=[{"type": "mcp", "server_url": "derouter_proxy", "require_approval": "never"}],
        store=store,
