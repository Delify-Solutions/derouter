...
from derouter.responses.mcp import chat_completions_handler
from derouter.responses.mcp.chat_completions_handler import (
...
async def test_acompletion_with_mcp_returns_normal_completion_without_tools(
    ...
    with patch("derouter.acompletion", mock_acompletion):
...
async def test_acompletion_with_mcp_without_auto_execution_calls_model(monkeypatch):
    ...
    monkeypatch.setattr(
        DeRouter_Proxy_MCP_Handler,
        "_should_use_derouter_mcp_gateway",
...
async def test_acompletion_with_mcp_auto_exec_performs_follow_up(monkeypatch):
    ...
    # Patch derouter.acompletion at module level to catch function-level imports
    with (
        patch("derouter.acompletion", mock_acompletion_func),
        patch.object(
            chat_completions_handler,
            "derouter_acompletion",
...
async def test_acompletion_with_mcp_adds_metadata_to_streaming(monkeypatch):
    ...
    """
    from derouter.utils import CustomStreamWrapper
    from derouter.types.utils import ModelResponseStream, StreamingChoices, Delta
    from derouter.derouter_core_utils.derouter_logging import Logging

    tools = [{"type": "mcp", "server_url": "derouter_proxy/mcp/local"}]
...
async def test_acompletion_with_mcp_streaming_initial_call_is_streaming(monkeypatch):
    ...
    # Patch derouter.acompletion at module level to catch function-level imports
    with (
        patch("derouter.acompletion", mock_acompletion),
        patch.object(
            chat_completions_handler,
            "derouter_acompletion",
            mock_acompletion,
...
async def test_execute_tool_calls_sets_proxy_server_request_arguments(monkeypatch):
    ...
    # NOTE: avoid monkeypatch string path here because `derouter.responses` is also
    # exported as a function on the top-level `derouter` package, which can confuse
    ...
    monkeypatch.setattr(
        "derouter.proxy._experimental.mcp_server.mcp_server_manager.global_mcp_server_manager.call_tool",
        mock_call_tool,
    )
    ...
    result = await DeRouter_Proxy_MCP_Handler._execute_tool_calls(
