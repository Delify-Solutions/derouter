...
failed when master_key was None. [https://github.com/Delify-Solutions/derouter/issues/16428]
"""
...
async def test_add_deployment_without_master_key():
    ...
    with patch("derouter.proxy.proxy_server.master_key", None):
        ...
        mock_prisma_client.db.derouter_config = MagicMock()
...
async def test_add_deployment_without_salt_key_or_master_key(monkeypatch):
    """
    Test that add_deployment() works when both master_key and DEROUTER_SALT_KEY are None.
    ...
    """
    # Remove DEROUTER_SALT_KEY from environment
    ...
    with patch("derouter.proxy.proxy_server.master_key", None):
        ...
        mock_prisma_client.db = MagicMock()
        mock_prisma_client.db.derouter_config = MagicMock()
        mock_prisma_client.db.derouter_config.find_first = AsyncMock(
            return_value=None
        )
