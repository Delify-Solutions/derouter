...
the result is written back into ``derouter.model_cost``.
...
def _restore_model_cost():
    ...
    original = dict(derouter.model_cost)
    try:
        yield
    finally:
        derouter.model_cost.clear()
        derouter.model_cost.update(original)
...
def _sparse_router_value(model_cost_key: str) -> Dict[str, Any]:
    ...
    # pricing (derouter/router.py:_create_deployment).
    return {
...
def test_first_registration_leaves_sparse_entry_without_cost_keys():
    ...
    derouter.register_model({key: {"derouter_provider": "openai"}})

    entry = derouter.model_cost.get(key, {})
...
def test_second_registration_does_not_persist_synthesized_zero_costs():
    ...
    payload = {key: {"derouter_provider": "openai"}}
    derouter.register_model(payload)
    derouter.register_model(payload)
...
def test_real_pricing_for_known_model_survives_re_registration():
    ...
    base_in = derouter.model_cost["gpt-4o-mini"]["input_cost_per_token"]
    base_out = derouter.model_cost["gpt-4o-mini"]["output_cost_per_token"]
    assert base_in > 0 and base_out > 0

    derouter.register_model({"gpt-4o-mini": {"derouter_provider": "openai"}})
    derouter.register_model({"gpt-4o-mini": {"derouter_provider": "openai"}})

    assert derouter.model_cost["gpt-4o-mini"]["input_cost_per_token"] == base_in
    assert derouter.model_cost["gpt-4o-mini"]["output_cost_per_token"] == base_out
...
def test_router_double_init_keeps_db_model_entry_sparse():
    ...
    after_first = dict(derouter.model_cost.get(deployment_id, {}))

    Router(model_list=model_list)
    after_second = dict(derouter.model_cost.get(deployment_id, {}))
