...
class TestExceptionMessageFormatting:
    ...
    def test_bad_request_error_message_format(self):
        """BadRequestError should format message with derouter prefix."""
        error = BadRequestError(
            message="test error",
            model="gpt-4",
            llm_provider="azure",
        ...
        assert "derouter.BadRequestError" in error.message
    ...
    def test_content_policy_violation_error_message_format(self):
        ...
        assert "derouter.ContentPolicyViolationError" in error.message
        assert "test error" in error.message
    ...
    def test_context_window_exceeded_error_message_format(self):
        ...
        assert "derouter.ContextWindowExceededError" in error.message
        assert "test error" in error.message
...
class TestExceptionAttributes:
    ...
    def test_bad_request_error_attributes(self):
        ...
        error = BadRequestError(
            ...
            model="gpt-4",
            llm_provider="azure",
            derouter_debug_info="debug info",
            max_retries=3,
            num_retries=1,
        )
        ...
        assert error.derouter_debug_info == "debug info"
    ...
    def test_midstream_fallback_error_status_code_propagation(self):
        ...
        assert midstream_error.message == "derouter.MidStreamFallbackError: stream broke"
        assert midstream_error.args == ("derouter.MidStreamFallbackError: stream broke",)
