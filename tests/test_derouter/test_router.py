...
from derouter.llms.bedrock.common_utils import BedrockError
...
from derouter.types.llms.openai import ChatCompletionRequest
...
from derouter.router_strategy import simple_shuffle
from derouter.types.router import DeploymentTypedDict, RetryPolicy
...
def test_update_kwargs_does_not_mutate_defaults_and_merges_metadata():
    ...
    router.default_derouter_params = {
        "foo": "bar",
        "metadata": {"baz": 123},
    }
    original = copy.deepcopy(router.default_derouter_params)
    kwargs: dict = {}

    # invoke the helper
    router._update_kwargs_with_default_derouter_params(
        kwargs=kwargs,
        metadata_variable_name="derouter_metadata",
    )

    # 1) router.defaults must be unchanged
    assert router.default_derouter_params == original

    # 2) non‑metadata keys get merged
    assert kwargs["foo"] == "bar"

    # 3) metadata lands under "metadata"
    assert kwargs["derouter_metadata"] == {"baz": 123}
...
def test_router_model_group_encrypted_content_affinity_callback_registration():
    ...
    original_callbacks = list(derouter.callbacks)
    derouter.callbacks = []
    router = None
    ...
    try:
        ...
        assert derouter.callbacks.index(encrypted_content_callbacks[0]) < (
            derouter.callbacks.index(deployment_callback)
        )
...
async def test_encrypted_content_affinity_model_group_config_is_additive():
    ...
    target_deployment = {
        "model_name": model_group,
        "derouter_params": {"model": "openai/gpt-5.1-codex"},
    ...
    assert request_kwargs["derouter_metadata"]["encrypted_content_affinity_enabled"]
    ...
    assert (
        ...
        not in disabled_request_kwargs["derouter_metadata"]
    )
    ...
    assert global_request_kwargs["derouter_metadata"][
        "encrypted_content_affinity_enabled"
    ]
...
async def test_encrypted_content_affinity_takes_priority_over_user_key_affinity():
    ...
    try:
        ...
        assert derouter.callbacks.index(encrypted_content_callback) < (
            derouter.callbacks.index(deployment_callback)
        )
        ...
        request_kwargs = {
            "input": [{"type": "reasoning", "id": encoded_id}],
            "derouter_metadata": {"user_api_key_hash": user_api_key_hash},
        }
...
async def test_arouter_with_tags_and_fallbacks():
    ...
    with pytest.raises(derouter.InternalServerError):
...
async def test_async_router_acreate_file():
    ...
    router = derouter.Router(
        model_list=[
            ...
            {"model_name": "gpt-3.5-turbo", "derouter_params": {"model": "gpt-4o-mini"}},
        ],
    )

    with patch("derouter.acreate_file", return_value=MagicMock()) as mock_acreate_file:
...
async def test_async_router_acreate_file_with_jsonl():
    ...
    router = derouter.Router(
        model_list=[
            ...
            {
                "model_name": "gpt-3.5-turbo-router",
                "derouter_params": {"model": "gpt-4o-mini"},
            },
        ],
    )
...
async def test_async_router_acreate_file_does_not_fall_back_across_model_groups():
    ...
    def fail_azure(*args: object, **kwargs: object) -> MagicMock:
        if kwargs.get("model") == "azure/my-azure-deployment":
            raise derouter.APIConnectionError(
    ...
    with patch("derouter.acreate_file", side_effect=fail_azure) as mock_acreate_file:
        with pytest.raises(derouter.APIConnectionError):
...
async def test_async_router_acancel_batch_does_not_fall_back_across_model_groups(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(derouter, "disable_aiohttp_transport", True)
    router = derouter.Router(
        model_list=[
            {
                "model_name": "azure-gpt",
            ...
            {
                "model_name": "openai-gpt",
                "derouter_params": {"model": "gpt-4o-mini", "api_key": "dummy-key"},
            },
        ],
...
@pytest.mark.asyncio
async def test_async_router_acreate_file_forwards_target_model_names_to_derouter_proxy():
    import json
    from io import BytesIO
    from unittest.mock import MagicMock, patch
        ...
        model_list=[
            {
                ...
                "derouter_params": {
                    "model": "derouter_proxy/gpt-4.1-batch",
    ...
    with patch("derouter.acreate_file", return_value=MagicMock()) as mock_acreate_file:
        ...
        assert call_kwargs["custom_llm_provider"] == "derouter_proxy"
...
@pytest.mark.asyncio
async def test_async_router_acreate_file_derouter_proxy_sends_target_model_names_in_multipart_form():
    import json
    from io import BytesIO
...
async def test_async_router_afile_content_uses_deployment_custom_llm_provider():
    ...
    with patch(
        "derouter.llms.azure.files.handler.AzureOpenAIFilesAPI.afile_content",
...
async def test_arouter_async_get_healthy_deployments():
    ...
    assert result[0]["derouter_params"]["model"] == "gpt-3.5-turbo"


@pytest.mark.asyncio
@patch("derouter.amoderation")
async def test_arouter_amoderation_with_credential_name(mock_amoderation):
    """
    Test that router.amoderation passes derouter_credential_name to the underlying derouter.amoderation call
    """
        ...
        model_list=[
            {
                ...
                "derouter_params": {
                    ...
                    "derouter_credential_name": "my-custom-auth",
                },
            },
        ],
    )
    ...
    assert call_kwargs["derouter_credential_name"] == "my-custom-auth"
...
def test_arouter_ignore_invalid_deployments():
    ...
    """
    from derouter.types.router import Deployment

    router = derouter.Router(
        model_list=[
            {
                "model_name": "gpt-3.5-turbo",
                "derouter_params": {"model": "my-bad-model"},
            },
        ],
    ...
    router.upsert_deployment(
        Deployment(
            model_name="gpt-3.5-turbo",
            derouter_params={"model": "my-bad-model"},  # type: ignore
...
async def test_arouter_aretrieve_batch():
    ...
    with patch.object(
        derouter, "aretrieve_batch", return_value=AsyncMock()
...
async def test_arouter_aretrieve_file_content():
    ...
    with patch.object(
        derouter, "afile_content", return_value=AsyncMock()
    ) as mock_afile_content:
        router = derouter.Router(
            model_list=[
                {
...
async def test_arouter_filter_team_based_models():
    ...
    router.add_deployment(
        Deployment(
            model_name="gpt-3.5-turbo",
            derouter_params={"model": "gpt-3.5-turbo"},  # type: ignore
            model_info={"tpm": 1000, "rpm": 1000},
        )
    )
...
def test_arouter_responses_api_bridge():
    ...
    with patch.object(derouter, "responses", return_value=AsyncMock()) as mock_responses:
        ...
        model_list=[
            {
                ...
                "derouter_params": {
                    ...
                    "mock_response": "derouter.InternalServerError",
                },
            },
            {
...
async def test_ageneric_api_call_deployment_model_overrides_alias():
    ...
    with model="vertex_ai/gemini-2.5-flash", the underlying derouter function must receive
...
async def test_ageneric_api_call_resolves_realtime_session_model():
    ...
    fills it with the pre-routing model group name. The underlying derouter function reads session.model first,
...
def test_with_router_resolved_session_model(session, expected):
    from derouter.router import _with_router_resolved_session_model
...
def test_cached_get_model_group_info():
    ...
    """
    from derouter.types.router import Deployment, DeRouter_Params

    router = derouter.Router(
        model_list=[
            {
                "model_name": "gpt-4",
                "derouter_params": {"model": "gpt-4", "api_key": "fake"},
    ...
    router.add_deployment(
        Deployment(
            model_name="gpt-4",
            derouter_params=DeRouter_Params(model="gpt-4", api_key="fake2"),
...
def test_cost_value_as_float(value, expected):
    from derouter.router import _cost_value_as_float
...
def test_get_model_access_groups_caching():
    ...
    router.add_deployment(
        Deployment(
            model_name="gpt-3.5",
            derouter_params=DeRouter_Params(model="gpt-3.5-turbo"),
...
def test_get_model_access_groups_cache_invalidation_set_model_list():
    ...
    router.set_model_list(
        [
            {
                "model_name": "claude-3",
                "derouter_params": {"model": "anthropic/claude-3-opus-20240229"},
...
def test_get_model_access_groups_cache_invalidation_upsert_deployment():
    ...
    router.upsert_deployment(
        Deployment(
            ...
            derouter_params=DeRouter_Params(model="gpt-4-turbo"),
...
async def test_acompletion_streaming_iterator():
    ...
    router = derouter.Router(
        model_list=[
            {
                "model_name": "gpt-4",
                "derouter_params": {"model": "gpt-4", "api_key": "fake-key-1"},
            },
            {
                "model_name": "gpt-3.5-turbo",
                "derouter_params": {"model": "gpt-3.5-turbo", "api_key": "fake-key-2"},
            },
        ],
...
async def test_acompletion_streaming_iterator_reraises_original_exception_when_available():
    ...
    from derouter.exceptions import MidStreamFallbackError, RateLimitError

    router = derouter.Router(
        model_list=[
            {
                "model_name": "gpt-4",
                "derouter_params": {"model": "gpt-4", "api_key": "fake-key"},
            }
        ],
        set_verbose=True,
    )
...
async def test_acompletion_streaming_iterator_preserves_hidden_params():
    ...
    original CustomStreamWrapper so that x-derouter-overhead-duration-ms (and
    ...
    # update_response_metadata (derouter_overhead_time_ms, _response_ms, etc.)
    mock_response = MagicMock()
    mock_response.model = "gpt-4"
    mock_response.custom_llm_provider = "openai"
    mock_response.logging_obj = MagicMock()
    mock_response._hidden_params = {
        "derouter_overhead_time_ms": 12.34,
        "_response_ms": 500.0,
    ...
    assert result._hidden_params.get("derouter_overhead_time_ms") == 12.34, (
        "derouter_overhead_time_ms must be preserved — "
        "this is what drives x-derouter-overhead-duration-ms in streaming responses"
    )
    assert result._hidden_params.get("derouter_call_id") == "test-call-id"
...
async def test_acompletion_streaming_iterator_preserves_response_headers():
    ...
    assert additional_headers["llm_provider-x-derouter-model-id"] == "spoofed"
    assert "x-derouter-model-id" not in additional_headers
...
def test_adopt_fallback_response_headers_replaces_rather_than_merges():
    ...
    from derouter.router import FallbackAwareStreamWrapper, Router
...
def test_adopt_fallback_response_headers_keeps_identity_when_fallback_has_none():
    ...
    Publishing no ``x-derouter-*`` header at all for a request the fallback served is
...
async def test_acompletion_streaming_iterator_adopts_fallback_response_headers():
    ...
    class FallbackStream:
        def __init__(self):
            ...
            self._chunks = iter([derouter.ModelResponseStream(choices=[{"index": 0, "delta": {"content": "OK"}}])])
...
def test_completion_streaming_iterator_preserves_hidden_params():
    ...
    mock_response.logging_obj = MagicMock()
    mock_response._hidden_params = {
        "derouter_overhead_time_ms": 42.0,
        "derouter_call_id": "test-sync-call",
    }
    ...
    assert result._hidden_params.get("derouter_overhead_time_ms") == 42.0
    assert result._hidden_params.get("derouter_call_id") == "test-sync-call"
...
def test_completion_streaming_iterator_reraises_mid_chunk_error_with_no_text_content():
    ...
    from derouter.exceptions import MidStreamFallbackError
    from derouter.types.utils import Delta, StreamingChoices
    ...
    reasoning_chunk = derouter.ModelResponseStream(
...
def _make_responses_iterator(
    *,
    ...
    DeRouterCompletionStreamingIterator so the wrapper's bridge-path
...
def _make_router_with_fallback(primary="gpt-4", secondary="gpt-3.5-turbo"):
    return derouter.Router(
        model_list=[
            {
                "model_name": primary,
                "derouter_params": {"model": primary, "api_key": "k1"},
            },
            {
                "model_name": secondary,
                "derouter_params": {"model": secondary, "api_key": "k2"},
            },
        ],
...
async def test_aresponses_streaming_iterator_fallback():
    ...
    with patch.object(
        ...
        wrapped = await router._aresponses_streaming_iterator(
            ...
            initial_kwargs={
                ...
                "stream": True,
                "input": "Hi",
                "original_generic_function": derouter.aresponses,
            },
        )
    ...
    assert fbk["original_generic_function"] is derouter.aresponses
    assert call_kwargs["model_group"] == "anthropic/claude-sonnet-4-6"
    assert call_kwargs["disable_fallbacks"] is False


@pytest.mark.asyncio
async def test_aresponses_streaming_iterator_writes_derouter_metadata_on_fallback():
    """Regression: model_group must land under "derouter_metadata" (the key
    derouter.aresponses reads), not the default "metadata"."""
    from derouter.exceptions import MidStreamFallbackError
    ...
    assert "derouter_metadata" in fbk, "wrong metadata_variable_name"
    assert fbk["derouter_metadata"]["model_group"] == "gpt-4"
    assert "model_group" not in fbk.get(
        "metadata", {}
    ), "model_group leaked into 'metadata' instead of 'derouter_metadata'"
...
async def test_aresponses_streaming_iterator_combines_partial_usage():
    ...
    with (
        patch(
            "derouter.main.stream_chunk_builder",
...
def _midstream_rate_limit_error():
    rate_limit_error = derouter.RateLimitError(
...
def test_pre_call_checks_skips_token_count_without_max_input_tokens(monkeypatch):
    ...
    router = derouter.Router(
        model_list=[
            {"model_name": "m", "derouter_params": {"model": "gpt-3.5-turbo"}},
        ],
    ...
    calls = []
    monkeypatch.setattr(
        derouter, "token_counter", lambda *a, **k: calls.append(1) or 1000
    )

    deployments = [
        {"derouter_params": {"model": "gpt-3.5-turbo"}, "model_info": {"id": "d1"}},
        {"derouter_params": {"model": "gpt-3.5-turbo"}, "model_info": {"id": "d2"}},
    ]
...
def test_pre_call_checks_uses_precounted_tokens(monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(
        derouter, "token_counter", lambda *a, **k: calls.append(1) or 1
    )
...
def test_deployment_max_input_tokens_survives_an_unmappable_deployment(monkeypatch):
    ...
    unmapped = {"derouter_params": {"model": "gpt-3.5-turbo"}, "model_info": {"id": "unmapped"}}
    mapped = {"derouter_params": {"model": "gpt-3.5-turbo"}, "model_info": {"id": "mapped"}}
...
async def test_async_get_healthy_deployments_never_recounts_on_the_loop(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "token_counter", exploding_counter)
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "anthropic/claude-3-5-haiku-20241022", "mock_response": "hi"},
...
def test_get_deployment_model_info_base_model_flow():
    ...
    mock_derouter_model_name_info = {
        ...
        "derouter_provider": "test_provider",
        "mode": "completion",
    ...
    with patch.object(
        derouter, "model_cost", {"test-custom-model": mock_custom_model_info}
    ):
        with patch.object(derouter, "get_model_info") as mock_get_model_info:
            # Configure mock returns
            mock_get_model_info.side_effect = lambda model: {
                "gpt-3.5-turbo": mock_base_model_info,
                "test-model": mock_derouter_model_name_info,
            }.get(model)
            ...
            # 2. The result of step 1 gets merged into derouter_model_name_info (custom+base override derouter)
            ...
            assert result["derouter_provider"] == "openai"  # From base model
            assert (
                result["mode"] == "chat"
            )  # From base model (overrides derouter "completion")

            # The key field comes from base model since both base and derouter have it
            # and base model info overrides derouter model name info in final merge
            assert (
                result["key"] == "gpt-3.5-turbo"
            )  # From base model (overrides derouter key)
    ...
    with patch.object(
        ...
        with patch.object(derouter, "get_model_info") as mock_get_model_info:
            ...
            assert result["max_tokens"] == 2048  # From derouter model name info
            assert result["custom_field"] == "custom_value"  # From custom model
            assert result["mode"] == "completion"  # From derouter model name info

    # Test Case 3: No custom model info, only derouter model name info
    with patch.object(derouter, "model_cost", {}):  # Empty model cost
        with patch.object(derouter, "get_model_info") as mock_get_model_info:
            mock_get_model_info.side_effect = lambda model: {
                "test-model": mock_derouter_model_name_info,
            }.get(model)
            ...
            # Result should be just the derouter model name info
            assert result is not None
            assert result == mock_derouter_model_name_info
        ...
        with patch.object(derouter, "get_model_info") as mock_get_model_info:
            ...
            def mock_get_model_info_side_effect(model):
                if model == "invalid-base-model":
                    ...
                    return mock_derouter_model_name_info
                return None
    ...
    with patch.object(derouter, "model_cost", {}):
        with patch.object(
            derouter, "get_model_info", side_effect=Exception("Not found")
        ):
            result = router.get_deployment_model_info(
    ...
    # Test Case 6: custom_model_info present but derouter_model_name_model_info is None
    ...
    with patch.object(
        ...
        with patch.object(derouter, "get_model_info") as mock_get_model_info:
            ...
            # Should return custom_model_info even when derouter_model_name_model_info is None
            assert result is not None
    ...
    # Test Case 7: custom_model_info with base_model but derouter_model_name_model_info None
...
def test_get_deployment_model_info_base_model_merge_priority():
    ...
    mock_derouter_model_name_info = {
        ...
        "mode": "completion",
        "derouter_only_field": "derouter_value",
    }

    with patch.object(
        derouter, "model_cost", {"custom-model-id": mock_custom_model_info}
    ):
        with patch.object(derouter, "get_model_info") as mock_get_model_info:
            mock_get_model_info.side_effect = lambda model: {
            ...
            # 3. Result from steps 1-2 overrides derouter_model_name_info
            ...
            assert (
                result["derouter_provider"] == "openai"
            )  # From base model (not in custom)
            assert (
                result["base_only_field"] == "base_value"
            )  # From base model (not in custom)

            # Fields that should come from derouter model name info (not overridden by custom+base)
            assert (
                result["mode"] == "completion"
            )  # From derouter model name info (not in custom or base)
            assert (
                result["derouter_only_field"] == "derouter_value"
            )  # From derouter model name info (not in custom or base)

            # Key comes from base model since both base and derouter have key fields
            # and the merged custom+base overrides derouter in the final merge
...
async def test_router_acompletion_with_unknown_model_and_default_fallback():
    ...
    router = derouter.Router(model_list=model_list, default_fallbacks=["gpt-4o"])
...
async def test_router_unknown_model_error_message_renders_model_name_literally():
    ...
    router = derouter.Router(
        model_list=[
            {
                "model_name": "gpt-4o",
                "derouter_params": {"model": "azure/gpt-4o-real", "api_key": "fake-key"},
            }
        ]
    )
...
def test_get_deployment_credentials_with_provider_aws_bedrock_runtime_endpoint():
    ...
    aws_bedrock_runtime_endpoint from deployment derouter_params to credentials.
    """
    router = derouter.Router(
        model_list=[
            {
...
def test_get_deployment_credentials_with_provider_includes_bucket_name():
    """
    Regression: bucket_name must survive the CredentialDeRouterParams filter so
...
def test_get_deployment_credentials_with_provider_resolves_credential_name():
    ...
    derouter_credential_name to actual credential values (for UI-created models).
    """
    from derouter.types.utils import CredentialItem

    # Setup credential list with a test credential
    derouter.credential_list = [
        CredentialItem(
        ...
        model_list=[
            {
                ...
                "derouter_params": {
                    "model": "azure/gpt-4",
                    "derouter_credential_name": "test-azure-cred",
                },
            }
        ],
    )
    ...
    assert "derouter_credential_name" not in credentials
...
def _team_wildcard_model(api_key: str, model_id: str = "team-wildcard-id") -> dict:
    return {
        ...
        "derouter_params": {"model": "openai/*", "api_key": api_key},
        "model_info": {
            "id": model_id,
            "team_id": "team-1",
            "team_public_model_name": "openai/*",
...
def test_get_deployment_credentials_with_provider_team_wildcard_priority():
    ...
    router = derouter.Router(
        model_list=[
            {
                "model_name": "openai/*",
                "derouter_params": {"model": "openai/*", "api_key": "global-key"},
            },
...
def test_deployment_usable_by_team_helpers():
    ...
    assert picked.derouter_params.vertex_project == "shared-project"
    ...
    assert owner_picked.derouter_params.vertex_project == "team-b-project"
...
def test_get_deployment_credentials_with_provider_skips_other_team_wildcard():
    ...
    router = derouter.Router(
        model_list=[
            {
                "model_name": "openai/*",
                "derouter_params": {"model": "openai/*", "api_key": "team-b-key"},
                "model_info": {
                    "id": "team-b-wildcard",
                    "team_id": "team-b",
                    "team_public_model_name": "openai/*",
...
def test_team_wildcard_credentials_not_usable_after_delete_deployment():
    ...
    """
    router = derouter.Router(model_list=[_team_wildcard_model(api_key="old-key")])
...
def test_global_wildcard_pattern_router_evicts_stale_entry_on_upsert_and_delete():
    ...
    router = derouter.Router(
        model_list=[
            {
                "model_name": "openai/*",
                "derouter_params": {"model": "openai/openai/*", "api_key": "sk-old"},
    ...
    router.upsert_deployment(
        Deployment(
            model_name="openai/*",
            derouter_params=DeRouter_Params(model="openai/*", api_key="sk-new"),
            model_info={"id": "global-wildcard"},
        )
    )
    ...
    assert [m["derouter_params"]["api_key"] for m in matches] == ["sk-new"]
...
def test_pattern_match_router_remove_deployment():
    ...
    """
    from derouter.router_utils.pattern_match_deployments import PatternMatchRouter

    pattern_router = PatternMatchRouter()
    pattern_router.add_pattern(
        "openai/*",
        {"derouter_params": {"model": "openai/*", "api_key": "key-a"}, "model_info": {"id": "dep-a"}},
    )
    pattern_router.add_pattern(
        "openai/*",
        {"derouter_params": {"model": "openai/*", "api_key": "key-b"}, "model_info": {"id": "dep-b"}},
    )
...
def test_get_available_guardrail_single_deployment():
    ...
    guardrail_config = {
        ...
        "derouter_params": {"guardrail": "custom", "mode": "pre_call"},
        "id": "guardrail-1",
    }
...
async def test_anthropic_messages_call_type_is_cached():
    ...
    from derouter.router_utils.prompt_caching_cache import PromptCachingCache
...
def test_update_kwargs_with_deployment_propagates_model_tags():
    """
    Test that deployment-level tags from derouter_params are merged into
    ...
    See: https://github.com/Delify-Solutions/derouter/issues/XXXX
    """
    router = derouter.Router(
        model_list=[
            {
        ...
        model_list=[
            {
                ...
                "derouter_params": {
                    ...
                    "mock_response": "derouter.ContextWindowExceededError",
                },
...
def test_update_kwargs_with_deployment_merges_tools():
    """
    Test that when both deployment derouter_params and request have tools,
...
def test_credential_name_injected_as_tag():
    """
    Test that derouter_credential_name from deployment derouter_params
...
def test_credential_name_not_injected_when_absent():
    """
    Test that when no derouter_credential_name is set, tags are unchanged.
    """
    router = derouter.Router(
        model_list=[
            {
...
def test_update_kwargs_with_deployment_model_info_in_derouter_metadata():
    """For generic_api_call, model_info with pricing must go to derouter_metadata.

    Routes like /messages and /responses use generic_api_call which stores
    model_info under derouter_metadata. Regression test for #23185.
    """
    router = derouter.Router(
        model_list=[
            {
    ...
    assert "derouter_metadata" in kwargs
    model_info = kwargs["derouter_metadata"]["model_info"]
...
def test_combine_fallback_usage():
    ...
    chunk = derouter.ModelResponseStream(
        id="test",
        model="gpt-4o",
        choices=[],
...
async def test_acompletion_streaming_iterator_does_not_log_success_on_terminal_failure():
    ...
    """
    from derouter.exceptions import MidStreamFallbackError
    from derouter.types.utils import Delta, StreamingChoices, Usage
    ...
    def _make_interrupted_model_response():
        partial_chunk = derouter.ModelResponseStream(
            id="chatcmpl-partial-1",
            created=1742056047,
            model="gpt-4",
            object="chat.completion.chunk",
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "gpt-3.5-turbo", "api_key": "fake"},
                "model_info": {
                    "team_id": "team-a",
...
def test_access_group_filter_empty_does_not_bypass_via_derouter_model_fallback(
    monkeypatch: pytest.MonkeyPatch,
):
    """
    When access-group filtering removes all candidates, _get_deployment_by_derouter_model
    must not run: it does not re-apply access groups and could return blocked deployments
    that share the same derouter_params.model as the request model string.
    ...
    derouter-model fallback would return both rows because ``derouter_params.model`` matches.
    """
    from derouter.proxy._types import UserAPIKeyAuth
    ...
    with pytest.raises(derouter.BadRequestError):
...
def test_access_group_block_via_derouter_model_branch_does_not_use_default_fallback(
    monkeypatch: pytest.MonkeyPatch,
):
    """
    When the by-name lookup returns no deployments and the derouter-model fallback
...
def test_try_early_resolve_deployments_for_model_not_in_names():
    ...
    """
    router_in_names = derouter.Router(
        model_list=[
            {
                "model_name": "gpt-5",
                "derouter_params": {
    ...
    pattern_router = derouter.Router(
        model_list=[
            {
                "model_name": "openai/*",
                "derouter_params": {
    ...
    default_router = derouter.Router(
        model_list=[
            {
    ...
    assert default_deployment["derouter_params"]["model"] == "brand-new-model"
    # The original default_deployment must not be mutated.
    assert (
        default_router.default_deployment["derouter_params"]["model"]
...
def _router_with_two_deployments(blocked_flags):
    ...
    for idx, blocked in enumerate(blocked_flags):
        model_list.append(
            {
                "model_name": "gpt-4o",
                "derouter_params": {"model": f"openai/gpt-4o-{idx}"},
                "model_info": {"id": f"dep-{idx}", "blocked": blocked},
            }
        )
    return derouter.Router(model_list=model_list)
...
def test_get_fully_blocked_model_names_treats_missing_key_as_unblocked():
    ...
    router = derouter.Router(
        model_list=[
            {
                "model_name": "gpt-4o",
                "derouter_params": {"model": "openai/gpt-4o"},
                "model_info": {"id": "dep-0"},
            }
        ]
    )
...
async def test_async_get_fully_unhealthy_model_names_noop_with_allowed_fails_policy():
    from derouter.types.router import AllowedFailsPolicy
...
def test_get_available_deployment_raises_when_addressed_dict_is_blocked():
    ...
    router = _router_with_two_deployments([True, True])
    with pytest.raises(derouter.ServiceUnavailableError):
...
def test_pass_through_deployment_api_key_resolves_via_get_credentials():
    from derouter.proxy.pass_through_endpoints.passthrough_endpoint_router import (
...
def test_is_deployment_blocked_static_helper_reflects_blocked_flag():
    ...
    assert derouter.Router._is_deployment_blocked(blocked_dep) is True
    assert derouter.Router._is_deployment_blocked(unblocked_dep) is False

    # No model_info on deployment object → treated as not blocked
    assert derouter.Router._is_deployment_blocked(object()) is False
    missing_blocked = types.SimpleNamespace()
    assert (
        derouter.Router._is_deployment_blocked(
...
class TestRouterRequestTimeoutPropagation:
    """derouter_settings.request_timeout must act as an independent per-attempt timeout.
    ...
    def explicit_request_timeout(self):
        original_value = derouter.request_timeout
        original_flag = derouter.request_timeout_explicitly_set
        derouter.request_timeout = 300
        derouter.request_timeout_explicitly_set = True
        try:
            yield 300
        finally:
            derouter.request_timeout = original_value
            derouter.request_timeout_explicitly_set = original_flag
    ...
    def test_request_timeout_none_when_not_explicitly_configured(self):
        original_value = derouter.request_timeout
        original_flag = derouter.request_timeout_explicitly_set
        derouter.request_timeout = derouter.constants.DEFAULT_REQUEST_TIMEOUT_SECONDS
        derouter.request_timeout_explicitly_set = False
        try:
            router = self._make_router(timeout=330)
            assert router.timeout == 330
...
def _make_deferred_stream_wrapper(make_call_fn):
    ...
    logging_obj = MagicMock()
    logging_obj.model_call_details = {"derouter_params": {}}
...
async def test_acompletion_deferred_stream_error_propagates_through_acompletion():
    ...
    """
    import derouter as _derouter

    rate_limit_err = _derouter.RateLimitError(
        message="Resource exhausted",
        llm_provider="vertex_ai",
    ...
    with patch(
        "derouter.acompletion",
        new_callable=AsyncMock,
        return_value=deferred_wrapper,
    ):
        with pytest.raises(_derouter.RateLimitError):
    ...
    model_name = router.model_list[0]["derouter_params"]["model"]
...
async def test_acompletion_deferred_stream_preserves_original_headers_on_error():
    ...
    with patch(
        ...
        with pytest.raises(_derouter.RateLimitError) as exc_info:
            await router._acompletion(
                model="vertex_ai/gemini-2.0-flash",
                messages=[{"role": "user", "content": "Hello"}],
                stream=True,
...
def test_completion_deferred_stream_error_propagates_through_completion():
    ...
    with patch("derouter.completion", return_value=deferred_wrapper):
        with pytest.raises(_derouter.RateLimitError):
            router._completion(
                model="vertex_ai/gemini-2.0-flash",
                messages=[{"role": "user", "content": "Hello"}],
...
def test_completion_deferred_stream_skipped_when_stream_already_set():
    ...
    router = derouter.Router(
        model_list=[
            {
                "model_name": "my-model",
                "derouter_params": {"model": "openai/gpt-4o", "api_key": "sk-fake"},
            }
        ],
    )

    with patch("derouter.completion", return_value=already_set_wrapper):
...
class TestAdvisorSubCallCooldown:
            ...
            model_list=[
                {
                    ...
                    "derouter_params": {"model": "bedrock/us.anthropic.claude-opus-4-8"},
                    "model_info": {"id": "dep-1"},
                }
            ],
        )
    ...
    def _kwargs(self, exception):
        return {
            "exception": exception,
            "derouter_params": {"model_info": {"id": "dep-1"}, "metadata": {}},
        }

    def _auth_error(self):
        return derouter.AuthenticationError(
...
def test_stream_chunks_have_generated_content_detects_text_and_non_text():
    from derouter.router import _stream_chunks_have_generated_content
    from derouter.types.utils import (
    ...
    def _chunk(delta):
        return derouter.ModelResponseStream(
            id="chatcmpl-1",
            model="gpt-4",
            object="chat.completion.chunk",
...
def test_get_configured_token_limits_reads_deployment_model_info():
    router = derouter.Router(
        model_list=[
            {
                "model_name": "my-custom-model",
                "derouter_params": {"model": "openai/some-unmapped-model"},
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "openai/some-unmapped-tts-model"},
...
async def test_acreate_batch_disable_fallbacks_surfaces_owning_provider_error():
    ...
    owning_provider_error = derouter.BadRequestError(
    ...
    with patch.object(router, "_acreate_batch", mock_create):
        with pytest.raises(derouter.BadRequestError, match="24h"):
...
async def test_acreate_batch_surfaces_owning_provider_error_without_disable_fallbacks():
    ...
    from derouter.types.utils import DeRouterBatch
    ...
    with patch.object(router, "_acreate_batch", _acreate_batch):
        with pytest.raises(derouter.APIConnectionError, match="openai is unreachable"):
            await router.acreate_batch(
                model="owning-model",
                input_file_id="file-owned-by-openai",
                endpoint="/v1/chat/completions",
...
async def test_acreate_batch_still_falls_back_within_the_owning_model_group():
    ...
    with patch.object(derouter, "acreate_batch", new=_acreate_batch):
        with pytest.raises(derouter.BadRequestError) as raised:
            await router.acreate_batch(
                model="my-gpt",
...
async def test_acreate_batch_request_bedrock_tags_override_deployment_tags():
    ...
    from derouter.llms.bedrock.common_utils import CommonBatchFilesUtils
    ...
    with patch.object(
        ...
        "derouter.llms.custom_httpx.llm_http_handler.get_async_httpx_client",
...
async def test_avector_store_search_injects_router():
    ...
    """
    from derouter.types.vector_stores import VectorStoreSearchResponse
    ...
    with patch("derouter.vector_stores.main.asearch", new=mock_asearch):  # test-quality-ok: the SDK call is the only place the injected router kwarg is observable
        router = derouter.Router(
            model_list=[
                {
                    "model_name": "gpt-3.5-turbo",
                    "derouter_params": {"model": "openai/gpt-3.5-turbo", "api_key": "test-key"},
                }
            ]
        )
...
async def test_avector_store_create_does_not_inject_router():
    ...
    with patch("derouter.vector_stores.main.acreate", new=mock_acreate):  # test-quality-ok: the SDK call is the only place a leaked router kwarg would surface
...
def test_vector_store_search_injects_router():
    ...
    # attribute before constructing the Router.
    with patch("derouter.vector_stores.main.search", new=mock_search):  # test-quality-ok: the SDK call is the only place the injected router kwarg is observable
        router = derouter.Router(
            model_list=[
                {
                    "model_name": "gpt-3.5-turbo",
...
def test_vector_store_create_does_not_inject_router():
    ...
    # attribute before constructing the Router.
    with patch("derouter.vector_stores.main.create", new=mock_create):  # test-quality-ok: the SDK call is the only place a leaked router kwarg would surface
        router = derouter.Router(
            model_list=[
                {
                    "model_name": "gpt-3.5-turbo",
...
class TestPreRoutingStrategyRegistryLifecycle:
    ...
    @classmethod
    def _router_with_complexity_router(cls, default_model: str = "gpt-4o") -> "derouter.Router":
        return derouter.Router(
            model_list=[
                {"model_name": "gpt-4o", "derouter_params": {"model": "gpt-4o"}},
                {"model_name": "gpt-4o-mini", "derouter_params": {"model": "gpt-4o-mini"}},
                {
                    "model_name": "smart-router",
                    "derouter_params": cls._complexity_router_params(default_model),
    ...
    @staticmethod
    def _model_names(router: "derouter.Router") -> list:
    ...
    def test_upsert_of_edited_router_keeps_it_routable(self):
        ...
        router.upsert_deployment(
            deployment=Deployment(
                model_name="smart-router",
                derouter_params=DeRouter_Params(**self._complexity_router_params("gpt-4o-mini")),
    ...
    def test_unchanged_upsert_leaves_router_untouched(self):
        ...
        for _ in range(3):
            router.upsert_deployment(
                deployment=Deployment(
                    model_name="smart-router",
                    derouter_params=DeRouter_Params(**self._complexity_router_params("gpt-4o")),
                    model_info=ModelInfo(id="router-1", db_model=True),
                )
            )
        ...
        router = derouter.Router(
            model_list=[
                ...
                {"model_name": "gpt-4o-mini", "derouter_params": {"model": "gpt-4o-mini"}},
                {
                    ...
                    "derouter_params": self._complexity_router_params("gpt-4o", tags=["team-a"]),
                    "model_info": {"id": "router-a"},
                },
                {
                    "model_name": "shared-router",
                    "derouter_params": self._complexity_router_params("gpt-4o-mini", tags=["team-b"]),
        ...
        router = derouter.Router(
            model_list=[
                ...
                {"model_name": "gpt-4o-mini", "derouter_params": {"model": "gpt-4o-mini"}},
                {
                    ...
                    "derouter_params": self._complexity_router_params("gpt-4o"),
    ...
    def test_upsert_of_edited_adaptive_router_rebuilds_it(self):
        ...
        router = derouter.Router(
            model_list=[
                {"model_name": "gpt-4o", "derouter_params": {"model": "openai/gpt-4o"}},
                {"model_name": "gpt-4o-mini", "derouter_params": {"model": "openai/gpt-4o-mini"}},
                {
                    "model_name": "adaptive-router",
                    "derouter_params": adaptive_params(["gpt-4o-mini"]),
                    "model_info": {"id": "router-1", "db_model": True},
                },
            ],
            ignore_invalid_deployments=True,
        ...
        router.upsert_deployment(
            deployment=Deployment(
                model_name="adaptive-router",
                derouter_params=DeRouter_Params(**adaptive_params(["gpt-4o", "gpt-4o-mini"])),
                model_info=ModelInfo(id="router-1", db_model=True),
            )
        )
    ...
    def test_delete_repairs_indices_even_when_strategy_release_fails(self):
        ...
        router.model_list[idx] = {"model_name": "smart-router", "derouter_params": None}
    ...
    def test_delete_of_adaptive_enabled_complexity_router_frees_both_registries(self):
        ...
        import derouter as derouter_module
        ...
        hooks = derouter_module.logging_callback_manager.get_custom_loggers_for_type(AdaptiveRouterPostCallHook)
        assert len(hooks) == 1
        ...
        remaining_hooks = derouter_module.logging_callback_manager.get_custom_loggers_for_type(
    ...
    def test_upsert_of_edited_quality_router_keeps_it_routable(self):
        ...
        router = derouter.Router(
            model_list=[
                ...
                {"model_name": "gpt-4o-mini", "derouter_params": {"model": "openai/gpt-4o-mini"}},
                {
                    ...
                    "derouter_params": quality_params("gpt-4o"),
                    "model_info": {"id": "router-1", "db_model": True},
                },
            ],
            ignore_invalid_deployments=True,
        ...
        router.upsert_deployment(
            deployment=Deployment(
                model_name="quality-router",
                derouter_params=DeRouter_Params(**quality_params("gpt-4o-mini")),
                model_info=ModelInfo(id="router-1", db_model=True),
            )
        )
    ...
    @classmethod
    def _router_with_hybrid_router(cls) -> "derouter.Router":
        return derouter.Router(
            model_list=[
                {"model_name": "gpt-4o", "derouter_params": {"model": "openai/gpt-4o"}},
                {"model_name": "gpt-4o-mini", "derouter_params": {"model": "openai/gpt-4o-mini"}},
                {
                    "model_name": "hybrid-router",
                    "derouter_params": cls._hybrid_router_params({"SIMPLE": "gpt-4o-mini", "MEDIUM": "gpt-4o"}),
                    "model_info": {"id": "router-1", "db_model": True},
                },
            ],
            ignore_invalid_deployments=True,
    ...
    def test_unregister_pre_routing_strategy_scopes_the_drop_by_tags(self):
        ...
        from derouter.types.router import TaggedPreRoutingStrategy
        ...
        assert derouter.Router._unregister_pre_routing_strategy(registry, "m", ("team-b",)) is False
        assert len(registry["m"]) == 2

        assert derouter.Router._unregister_pre_routing_strategy(registry, "m", ("team-a",)) is True
        assert [entry.tags for entry in registry["m"]] == [()]

        assert derouter.Router._unregister_pre_routing_strategy(registry, "m", ()) is True
    ...
    def test_unregister_for_deployment_ignores_non_router_deployments(self):
        ...
        router._unregister_pre_routing_strategy_for_deployment(
            deployment=Deployment(
                model_name="smart-router",
                derouter_params=DeRouter_Params(model="openai/gpt-4o"),
...
def test_model_info_is_active_for_environment_matrix(monkeypatch):
    ...
    from derouter.router import model_info_is_active_for_environment
    ...
    monkeypatch.setenv("DEROUTER_ENVIRONMENT", "development")
    ...
    monkeypatch.delenv("DEROUTER_ENVIRONMENT")
    with pytest.raises(ValueError, match="DEROUTER_ENVIRONMENT"):
...
def test_pre_call_checks_uses_deployment_model_when_model_info_lookup_raises(monkeypatch):
    ...
    """
    monkeypatch.setattr(derouter, "drop_params", False)

    router = derouter.Router(
        model_list=[
            {
                "model_name": "custom-alias",
                "derouter_params": {"model": "hosted_vllm/not-in-the-catalog"},
            }
        ],
        enable_pre_call_checks=True,
    )
    ...
    seen: list[tuple] = []
    original_get_supported_openai_params = derouter.get_supported_openai_params
    ...
    monkeypatch.setattr(derouter, "get_supported_openai_params", _record)
...
def test_pre_call_checks_keeps_deployment_when_provider_is_unresolvable(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "get_llm_provider", _raise_no_provider)

    deployments = [
        {
            "derouter_params": {"model": "some-unresolvable-model"},
            "model_info": {"id": "d1"},
        }
    ]
    result = router._pre_call_checks(
...
class TestUpsertDeploymentRollback:
    ...
    def test_failed_upsert_keeps_previous_deployment_serving(self):
            ...
            model_list=[
                {
                    ...
                    "derouter_params": {"model": "gpt-4o", "api_key": "sk-old"},
        ...
        result = router.upsert_deployment(
            deployment=Deployment(
                model_name="prod-model",
                derouter_params=DeRouter_Params(model="auto_router/broken"),
        ...
        assert restored.derouter_params.model == "gpt-4o"
    ...
    def test_failed_fresh_add_returns_none_without_restore(self):
        ...
        router = derouter.Router(model_list=[], ignore_invalid_deployments=True)
...
class TestUpsertDeploymentRename:
    ...
    `upsert_deployment` compared only `derouter_params` and `model_info`. A rename with no
    ...
    @staticmethod
    def _router() -> "derouter.Router":
        return derouter.Router(
            model_list=[
                {
                    "model_name": "old-name",
                    "derouter_params": {"model": "openai/gpt-4o", "api_key": "sk-test"},
    ...
    def _deployment(model_name: str, tpm: int | None = None):
        ...
        return Deployment(
            model_name=model_name,
            derouter_params=DeRouter_Params(model="openai/gpt-4o", api_key="sk-test", tpm=tpm),
    ...
    def test_rename_alongside_another_edit_still_updates(self):
        ...
        renamed = router.get_deployment(model_id="rename-1")
        assert renamed is not None
        assert renamed.derouter_params.tpm == 1234
...
class TestConsumedRequestTagsStamp:
    ...
    class _RewriteStrategy:
        ...
        async def async_pre_routing_hook(
            ...
            from derouter.types.router import PreRoutingHookResponse

            return PreRoutingHookResponse(model=self.rewrite_to, messages=messages)

    @classmethod
    def _router(cls, marker_tags=("route",)) -> "derouter.Router":
        from derouter.types.router import TaggedPreRoutingStrategy

        router = derouter.Router(
            model_list=[
                {"model_name": "gpt4o", "derouter_params": {"model": "openai/gpt-4o"}},
                {"model_name": "gemini-flash", "derouter_params": {"model": "gemini/gemini-3.6-flash"}},
            ],
            enable_tag_filtering=True,
        )
        router.auto_routers = {
    ...
    async def test_stamps_the_rewritten_group_when_request_tags_selected_the_router(self):
        from derouter.constants import CONSUMED_REQUEST_TAGS_METADATA_KEY
        from derouter.types.router import ConsumedRequestTagsStamp
    ...
    @pytest.mark.asyncio
    async def test_stamps_into_derouter_metadata_when_the_request_uses_that_bucket(self):
        from derouter.constants import CONSUMED_REQUEST_TAGS_METADATA_KEY
        from derouter.types.router import ConsumedRequestTagsStamp

        router = self._router()
        request_kwargs = {"derouter_metadata": {"tags": ["route"]}}

        await router.async_pre_routing_hook(model="gpt4o", request_kwargs=request_kwargs)

        assert request_kwargs["derouter_metadata"][CONSUMED_REQUEST_TAGS_METADATA_KEY] == ConsumedRequestTagsStamp(
            model_group="gemini-flash", tags=("route",)
        )
...
class TestClaudeCodeSubagentSessionRouterBinding:
            ...
            model_list=[
                {
                    ...
                    "derouter_params": {"model": "openai/gpt-4o-mini", "mock_response": cheap_response},
                },
                {
                    "model_name": "expensive-model",
                    "derouter_params": {"model": "openai/gpt-4o", "mock_response": "expensive response"},
                },
            ],
            fallbacks=fallbacks,
            num_retries=0,
    ...
    async def test_subagent_can_fallback_to_its_original_requested_model(self):
        router = self._router(
            cheap_response="derouter.RateLimitError",
    ...
    async def test_anthropic_subagent_four_fallback_hops_use_each_current_model_chain(self):
                ...
                *(
                    {
                        ...
                        "derouter_params": {
                            ...
                            "mock_response": "derouter.RateLimitError",
                        },
                    }
        ...
        main_kwargs["derouter_metadata"] = main_kwargs.pop("metadata")
        await router.async_pre_routing_hook(model="smart-router", request_kwargs=main_kwargs)
        subagent_kwargs = self._request_kwargs(agent_id="agent-1234")
        subagent_kwargs["derouter_metadata"] = subagent_kwargs.pop("metadata")
...
class TestAutoRouterMaxInputCharsWiring:
    ...
    @staticmethod
    def _router(**extra_params) -> "derouter.Router":
    ...
    @staticmethod
    def _registered_auto_router(router: "derouter.Router"):
    ...
    def test_should_fall_back_to_the_shared_default_when_the_deployment_omits_it(self):
        from derouter.constants import DEFAULT_AUTO_ROUTER_MAX_INPUT_CHARS
...
class TestTaggedAutoRouterOnSharedModelName:
    ...
    @classmethod
    def _router(cls, marker_tags, include_plain_sibling: bool, enable_tag_filtering: bool) -> "derouter.Router":
        pytest.importorskip("semantic_router", reason="auto-router needs the semantic-router extra")
        marker = {
            "model_name": "gpt4o",
            "derouter_params": {
        ...
        plain = {"model_name": "gpt4o", "derouter_params": {"model": "openai/gpt-4o"}}
        tier = {"model_name": "gemini-flash", "derouter_params": {"model": "gemini/gemini-3.6-flash"}}
        router = derouter.Router(
    ...
    @staticmethod
    async def _hook_response(router: "derouter.Router", request_kwargs: dict):
    ...
    async def test_untagged_selection_never_lands_on_the_marker_deployment(self):
        ...
        for _ in range(20):
            ...
            assert deployment["derouter_params"]["model"] == "openai/gpt-4o"

    def test_deployment_without_derouter_params_mapping_is_not_a_marker(self):
        assert derouter.Router._is_strategy_marker_deployment({"model_name": "gpt4o"}) is False
...
class TestAutoRouterSharedModelNameConnectionParams:
    ...
    @classmethod
    def _router(cls, plain_entry_first: bool) -> "derouter.Router":
        pytest.importorskip("semantic_router", reason="auto-router needs the semantic-router extra")
        plain = {
            "model_name": "gpt4o",
            "derouter_params": {
        ...
        tier = {
            ...
            "derouter_params": {"model": "gemini/gemini-3.6-flash", "api_key": "sk-tier-key"},
        }
        shared_name_entries = [plain, marker] if plain_entry_first else [marker, plain]
        router = derouter.Router(model_list=[*shared_name_entries, tier])
        router.auto_routers["gpt4o"][0].strategy.routelayer = cls._FixedRouteLayer()
        return router
...
class TestGetAllowedFailsFromPolicy:
    def _make_router(self, **policy_kwargs) -> derouter.Router:
        from derouter.types.router import AllowedFailsPolicy

        return derouter.Router(
            model_list=[{"model_name": "gpt-4", "derouter_params": {"model": "gpt-4", "api_key": "fake"}}],
    ...
    def test_no_policy_returns_none(self):
        ...
        assert router.get_allowed_fails_from_policy(derouter.RateLimitError("429", "openai", "gpt-4")) is None

    def test_internal_server_error_allowed_fails(self):
        router = self._make_router(InternalServerErrorAllowedFails=7)
        exc = derouter.InternalServerError("500", "openai", "gpt-4")
    ...
    def test_service_unavailable_error_allowed_fails(self):
        ...
        exc = derouter.ServiceUnavailableError("503", "openai", "gpt-4")
    ...
    def test_bad_gateway_error_allowed_fails(self):
        ...
        exc = derouter.BadGatewayError("502", "openai", "gpt-4")
    ...
    def test_not_found_error_allowed_fails(self):
        ...
        exc = derouter.NotFoundError("404", "openai", "gpt-4")
    ...
    def test_unmatched_exception_returns_none(self):
        ...
        exc = derouter.RateLimitError("429", "openai", "gpt-4")
...
async def _drive_cyclic_fallback(router, capture, recorder=None, **request_kwargs):
    ...
    try:
        ...
        if recorder is not None:
            derouter.callbacks.remove(recorder)
...
def test_stamp_or_clear_metadata_key_writes_and_clears_both_buckets():
    request_kwargs = {"metadata": {}}
    derouter.Router._stamp_or_clear_metadata_key(request_kwargs=request_kwargs, key="probe", value=7)
    assert request_kwargs["metadata"]["probe"] == 7

    stale_kwargs = {"metadata": {"probe": 7}, "derouter_metadata": {"probe": 7}}
    derouter.Router._stamp_or_clear_metadata_key(request_kwargs=stale_kwargs, key="probe", value=None)
    assert "probe" not in stale_kwargs["metadata"]
    assert "probe" not in stale_kwargs["derouter_metadata"]
...
def test_complexity_router_registers_affinity_callback_for_deployment_pin(complexity_router_config, expect_callback):
    ...
    try:
        ...
        for cb in router.optional_callbacks or []:
            derouter.logging_callback_manager.remove_callback_from_all_lists(cb)
...
def test_ensure_deployment_affinity_callback_is_idempotent():
    ...
    router = derouter.Router(model_list=[])
    try:
...
def test_get_router_model_info_does_not_wipe_cached_pricing():
    ...
    expected = copy.deepcopy(derouter.get_model_info(model="anthropic/claude-sonnet-4-5"))

    router = derouter.Router(model_list=[])
    merged = router.get_router_model_info(
        deployment=Deployment(
            model_name="sonnet",
            derouter_params=DeRouter_Params(model="claude-sonnet-4-5", custom_llm_provider="anthropic"),
            model_info=ModelInfo(id="sonnet-1"),
        ),
        received_model_name="sonnet",
    )

    assert derouter.get_model_info(model="anthropic/claude-sonnet-4-5") == expected
...
def test_get_router_model_info_keeps_explicit_pricing_overrides():
    ...
    assert derouter.get_model_info(model="anthropic/claude-sonnet-4-5")["input_cost_per_token"] != 1e-08
...
class TestModelGroupAliasReachesPreRoutingStrategies:
    ...
    @classmethod
    def _router(cls, registry_name: str | None) -> "derouter.Router":
        from derouter.types.router import TaggedPreRoutingStrategy
        ...
        router = derouter.Router(
            model_list=[
                ...
                {
                    "model_name": "gemini-flash",
                    "derouter_params": {"model": "gemini/gemini-3.6-flash", "mock_response": "routed by the tier"},
                },
            ],
    ...
    async def test_alias_deployment_selection_lands_on_the_tier_never_the_marker(self):
        ...
        assert deployment["derouter_params"]["model"] == "gemini/gemini-3.6-flash"
    ...
    def test_a_group_of_only_markers_is_not_a_callable_model(self):
        ...
        with pytest.raises(derouter.BadRequestError, match="strategy router marker"):
...
class TestAutoRouterCompressionDecoupling:
    ...
    def _router(self, marker_derouter_params: dict) -> tuple[derouter.Router, "_RecordingStrategy"]:
        from derouter.types.router import TaggedPreRoutingStrategy
    ...
    def registered_guardrail(self, monkeypatch):
        from derouter.proxy.guardrails import guardrail_registry
        ...
        derouter.logging_callback_manager.add_derouter_callback(guardrail)
        yield guardrail
        derouter.logging_callback_manager.remove_callback_from_all_lists(guardrail)
    ...
    async def test_same_compression_still_compresses_routing_when_nothing_armed_it(self, registered_guardrail):
        ...
        from derouter.proxy.guardrails import auto_router_compression
...
class TestAzureBaseModelFallbackLogging:
    ...
    def test_map_known_deployment_name_resolves_without_error_log(self):
        ...
        with patch(
            "derouter.router.verbose_router_logger.error"
        ) as mock_error:
        ...
        assert model_info["max_input_tokens"] == derouter.model_cost["azure/gpt-4o"]["max_input_tokens"]
        assert model_info["input_cost_per_token"] == derouter.model_cost["azure/gpt-4o"]["input_cost_per_token"]
    ...
    def test_explicit_base_model_still_wins(self):
        ...
        assert model_info["max_input_tokens"] == derouter.model_cost["azure/gpt-4o-mini"]["max_input_tokens"]
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "anthropic/opus-like"},
                "model_info": {"id": "opus-like-deployment"},
            },
            {
                "model_name": "smart-group",
                "derouter_params": {"model": "openai/mini-like"},
...
def test_model_group_info_reasoning_efforts_are_unknown_when_any_deployment_is_off_the_map():
    ...
    router = derouter.Router(
        model_list=[
            ...
            {
                "model_name": "smart-group",
                "derouter_params": {"model": "openai/unmapped-model"},
    ...
    def _model_info(model_id: str, model_name: str):
        ...
        return {"key": model_name, "derouter_provider": "openai", "mode": None, "supports_reasoning": None}
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "together_ai/zai-org/GLM-5.3-Flash", "api_key": "fake-key"},
            }
        ]
    )
...
def test_model_group_info_reasoning_efforts_empty_on_a_mapped_non_reasoning_deployment():
    ...
    router = derouter.Router(
        model_list=[
            ...
            {
                "model_name": "mixed-group",
                "derouter_params": {"model": "openai/plain-chat"},
    ...
    def _model_info(model_id: str, model_name: str):
        ...
        return {"key": model_name, "derouter_provider": "openai", "mode": "chat", "supports_reasoning": None}
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "openai/first-reasoner"},
                "model_info": {"id": "first-deployment"},
            },
            {
                "model_name": "declared-group",
                "derouter_params": {"model": "openai/second-reasoner"},
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "openai/lone-reasoner"},
...
def test_model_group_info_reasoning_efforts_are_unknown_for_an_operator_declared_mode():
    ...
    from derouter.router_utils.reasoning_effort_capability import resolve_supported_reasoning_efforts

    mapped_model = "openai/gpt-5.6-sol"
    expected = resolve_supported_reasoning_efforts(
        derouter.get_model_info(model=mapped_model),
    ...
    router = derouter.Router(
        model_list=[
            {
                "model_name": "smart-group",
                "derouter_params": {"model": mapped_model, "api_key": "sk-fake"},
                "model_info": {"id": "mapped-deployment"},
            },
            {
                "model_name": "smart-group",
                "derouter_params": {"model": "openai/a-model-the-map-never-heard-of", "api_key": "sk-fake"},
...
class TestAddDeploymentApiBaseProviderResolution:
    ...
    def test_bare_model_with_unknown_api_base_still_raises(self):
        with pytest.raises(derouter.BadRequestError, match="LLM Provider NOT provided"):
            derouter.Router(
                model_list=[
                    {
                        "model_name": "mystery",
    ...
    def test_explicit_custom_llm_provider_beats_api_base_endpoint_match(self):
        ...
        assert deployment.derouter_params.custom_llm_provider == "openai"
...
async def test_aanthropic_messages_fallback_attempt_raises_original_exception_on_double_failure():
    ...
    router = _anthropic_messages_make_router()
    error = MidStreamFallbackError(message="overloaded", model="primary", llm_provider="anthropic")
    original_exception = derouter.APIError(
    ...
    with patch.object(
        ...
        with pytest.raises(derouter.APIError) as exc_info:
...
async def test_aanthropic_messages_with_streaming_fallbacks_deep_copies_nested_metadata():
    ...
    nested derouter_metadata/metadata dict objects with the primary attempt.
    ...
    async def fake_original(**_kwargs):
        ...
        # derouter_metadata in place, as the real helper does.
    ...
    with patch.object(
        ...
        with patch.object(
            ...
            await router._aanthropic_messages_with_streaming_fallbacks(
                ...
                model="primary",
                stream=True,
                derouter_metadata=primary_metadata,
            )

    fallback_kwargs = streaming_iter_kwargs["initial_kwargs"]
    assert fallback_kwargs["derouter_metadata"] is not primary_metadata
    assert "deployment" not in fallback_kwargs["derouter_metadata"]
...
async def test_aanthropic_messages_with_streaming_fallbacks_deep_copies_metadata_field():
    ...
    call sites use instead of `derouter_metadata`."""
    router = _anthropic_messages_make_router()
...
async def test_factory_function_anthropic_messages_uses_streaming_fallback_dispatch():
    ...
    router = _anthropic_messages_make_router()
    wrapped = router.factory_function(derouter.anthropic_messages, call_type="anthropic_messages")
...
async def test_async_function_with_fallbacks_stamps_zero_attempted_fallbacks():
    ...
    the requested model group in metadata, mirroring the x-derouter-attempted-fallbacks header."""
    router = derouter.Router(
        model_list=[
            {
                "model_name": "gpt-3.5-turbo",
...
@pytest.mark.asyncio
async def test_async_function_with_fallbacks_stamps_route_bucket_not_derouter_metadata():
    ...
    metadata = {}
    derouter_metadata = {"client_key": "client_value"}

    await router.acompletion(
        model="gpt-3.5-turbo",
        messages=[{"role": "user", "content": "hey"}],
        metadata=metadata,
        derouter_metadata=derouter_metadata,
    )

    assert metadata["attempted_fallbacks"] == 0
    assert metadata["original_model_group"] == "gpt-3.5-turbo"
    assert derouter_metadata["client_key"] == "client_value"
    assert "attempted_fallbacks" not in derouter_metadata
    assert "original_model_group" not in derouter_metadata
...
async def test_async_function_with_fallbacks_stamps_despite_forged_reentry_params():
    ...
    derouter params cannot skip the entry stamp; only the router's own in-process
...
async def test_async_function_with_fallbacks_skips_stamp_on_genuine_reentrant_hop():
    ...
    from derouter.router_utils.fallback_event_handlers import AttemptedFallbackTargets
...
def _record_router_acompletion_kwargs(router: derouter.Router) -> list:
...
async def test_async_function_with_fallbacks_scrubs_spoofed_values_from_sibling_bucket():
    """Spend logs read a truthy derouter_metadata dict in preference to metadata, so spoofed
    ...
    downstream_sibling = downstream_calls[0]["derouter_metadata"]
...
async def test_async_function_with_fallbacks_scrubs_sibling_bucket_in_place():
    ...
    caller_snapshot = copy.deepcopy(derouter_metadata)
    downstream_calls = _record_router_acompletion_kwargs(router)
    ...
    assert len(downstream_calls) == 1
    assert downstream_calls[0]["derouter_metadata"] is derouter_metadata
    assert "attempted_fallbacks" not in derouter_metadata
    assert "original_model_group" not in derouter_metadata
    assert derouter_metadata["client_key"] == caller_snapshot["client_key"]


@pytest.mark.asyncio
async def test_async_function_with_fallbacks_stamps_aliased_buckets_on_every_call():
    """One dict object passed as both metadata and derouter_metadata: the first call's own
    ...
    for _ in range(3):
        await router.acompletion(
            ...
            derouter_metadata=shared_metadata,
        )

    assert len(downstream_calls) == 3
    for call_kwargs in downstream_calls:
        assert call_kwargs["derouter_metadata"] is shared_metadata
        assert call_kwargs["metadata"] is shared_metadata
        assert call_kwargs["derouter_metadata"]["attempted_fallbacks"] == 0
        assert call_kwargs["derouter_metadata"]["original_model_group"] == "chat-group"
...
async def test_run_async_fallback_keeps_caller_metadata_keys_on_the_wire(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "enable_preview_features", True)
    caller_metadata = {f"user_key_{i}": f"value_{i}" for i in range(16)}
    router = derouter.Router(
        model_list=[
            {
                "model_name": "primary-group",
                "derouter_params": {"model": "gpt-3.5-turbo", "api_key": "sk-test"},
            },
            {
...
async def test_run_async_fallback_two_hop_chain_reports_entry_group_and_hop_count():
    ...
    router = derouter.Router(
        model_list=[
            {
                "model_name": "group-a",
                "derouter_params": {"model": "gpt-3.5-turbo", "mock_response": "derouter.InternalServerError"},
            },
            {
                "model_name": "group-b",
                "derouter_params": {"model": "gpt-3.5-turbo", "mock_response": "derouter.InternalServerError"},
            ...
            {
                "model_name": "group-c",
                "derouter_params": {"model": "gpt-3.5-turbo", "mock_response": "ok"},
            },
        ],
...
def _permission_denied_error() -> derouter.PermissionDeniedError:
    return derouter.PermissionDeniedError(
...
def test_permission_denied_error_is_not_retried_against_a_single_deployment():
    router = derouter.Router(
        model_list=[
            {"model_name": "gpt-4o", "derouter_params": {"model": "openrouter/openai/gpt-4o", "api_key": "sk-test"}},
        ]
    )

    with pytest.raises(derouter.PermissionDeniedError):
...
def test_permission_denied_error_is_retried_when_other_deployments_exist():
    router = derouter.Router(
        model_list=[
            {"model_name": "gpt-4o", "derouter_params": {"model": "openrouter/openai/gpt-4o", "api_key": "sk-test"}},
            {"model_name": "gpt-4o", "derouter_params": {"model": "openai/gpt-4o", "api_key": "sk-test"}},
        ]
    )
...
def _resolution_router() -> Router:
    return Router(
        model_list=[
            {"model_name": "pinned", "derouter_params": {"model": "openai/gpt-4o", "api_key": "sk-test"}},
            {"model_name": "pooled", "derouter_params": {"model": "openai/gpt-4o-mini", "api_key": "sk-test"}},
            {"model_name": "pooled", "derouter_params": {"model": "anthropic/claude-haiku-4-5", "api_key": "sk-test"}},
            {"model_name": "bedrock/*", "derouter_params": {"model": "bedrock/*", "api_key": "sk-test"}},
        ],
...
def test_resolved_derouter_models_answers_through_every_channel_a_request_uses(
    ...
    """
    assert set(_resolution_router().resolved_derouter_models(model_name)) == set(expected)


class TestTierParamsTheTargetAccepts:
    """A tier's derouter_params are applied to every request that tier routes, so one the target
    ...
    def force_local_model_cost(self, monkeypatch):
        from derouter.derouter_core_utils.get_model_cost_map import GetModelCostMap

        monkeypatch.setattr(derouter, "model_cost", GetModelCostMap.load_local_model_cost_map())

    @staticmethod
    def _router(model: str) -> derouter.Router:
        return derouter.Router(
            model_list=[{"model_name": "tiered", "derouter_params": {"model": model, "api_key": "sk-x"}}]
        )
    ...
    def test_keeps_derouter_controls_the_provider_never_lists(self, control, value):
        """No provider lists a derouter control among its supported params, so "no deployment
        declares it" means derouter consumes it, not that the target refuses it. Dropping
    ...
    def test_declared_param_allowlist_ignores_malformed_declarations(self):
        ...
        assert derouter.Router._declared_param_allowlist({"allowed_openai_params": ["reasoning_effort", 3]}) == frozenset(
            {"reasoning_effort"}
        )
        assert derouter.Router._declared_param_allowlist({"allowed_openai_params": "reasoning_effort"}) == frozenset()
        assert derouter.Router._declared_param_allowlist({}) == frozenset()

    def test_deployment_accepts_param_honors_deployment_allowlist(self):
        deployment = {
            "model_name": "x",
            "derouter_params": {"model": "novita/moonshotai/kimi-k3", "allowed_openai_params": ["reasoning_effort"]},
        }

        assert derouter.Router._deployment_accepts_param(deployment, "x", "reasoning_effort") is True
    ...
    def test_keeps_a_param_any_deployment_in_the_group_declares(self):
        ...
        router = derouter.Router(
            model_list=[
                {"model_name": "tiered", "derouter_params": {"model": "novita/moonshotai/kimi-k3", "api_key": "k"}},
                {"model_name": "tiered", "derouter_params": {"model": "fireworks_ai/kimi-k3", "api_key": "k"}},
            ]
        )
    ...
    def test_deployment_accepts_param_honors_base_model(self):
        ...
        by_model_info = {
            "model_name": "x",
            "derouter_params": {"model": "azure/my-gpt5-deploy"},
            "model_info": {"base_model": "azure/gpt-5"},
        }
        by_derouter_params = {
            "model_name": "x",
            "derouter_params": {"model": "azure/my-gpt5-deploy", "base_model": "azure/gpt-5"},
        }
        without_hint = {"model_name": "x", "derouter_params": {"model": "azure/my-gpt5-deploy"}}

        assert derouter.Router._deployment_accepts_param(by_model_info, "x", "reasoning_effort") is True
        assert derouter.Router._deployment_accepts_param(by_derouter_params, "x", "reasoning_effort") is True
        assert derouter.Router._deployment_accepts_param(without_hint, "x", "reasoning_effort") is False

    def test_deployment_accepts_param_reads_the_provider(self):
        deployment = {"model_name": "x", "derouter_params": {"model": "fireworks_ai/kimi-k3"}}

        assert derouter.Router._deployment_accepts_param(deployment, "x", "reasoning_effort") is True

    def test_deployment_accepts_param_is_false_when_the_provider_omits_it(self):
        deployment = {"model_name": "x", "derouter_params": {"model": "novita/moonshotai/kimi-k3"}}

        assert derouter.Router._deployment_accepts_param(deployment, "x", "reasoning_effort") is False

    @pytest.mark.parametrize(
        "deployment",
        [{"model_name": "x"}, {"model_name": "x", "derouter_params": {}}, {"model_name": "x", "derouter_params": {"model": "not-a-real-provider/nope"}}],
    )
    ...
    def test_deployment_accepts_param_never_asks_a_provider_whose_lookup_authenticates(
        self, derouter_params, monkeypatch
    ):
        ...
        monkeypatch.setattr(derouter, "get_llm_provider", _record)
        deployment = {"model_name": "x", "derouter_params": derouter_params}
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "mistral/voxtral-mini-tts-2603"},
            }
        ]
    )
...
async def test_router_aspeech_without_voice_keeps_deployment_default_voice(respx_mock, monkeypatch):
    ...
    router = Router(
        model_list=[
            {
                "model_name": "voxtral-tts",
                "derouter_params": {"model": "mistral/voxtral-mini-tts-2603", "voice": "en_paul_neutral"},
            }
        ]
    )
...
class TestRequestReasoningEffortOverride:
    def test_drop_effort_from_nested_carrier_preserves_other_nested_values(self):
        ...
        derouter.Router._pop_effort_from_nested_carrier(params, "output_config")

        assert params == {"output_config": {"format": "json"}}

    @pytest.mark.parametrize("metadata_key", ["metadata", "derouter_metadata"])
    def test_is_classifier_internal_call_recognizes_both_metadata_carriers(self, metadata_key):
        ...
        assert derouter.Router._is_classifier_internal_call(kwargs) is True
        assert derouter.Router._is_classifier_internal_call({metadata_key: {}}) is False
    ...
    def test_removes_every_deployment_native_effort_carrier_without_mutating_shared_config(self):
        ...
        sanitized = derouter.Router._deployment_params_with_request_reasoning_override(
    ...
    def test_omitted_override_preserves_deployment_defaults(self, request_kwargs):
        ...
        assert (
            derouter.Router._deployment_params_with_request_reasoning_override(deployment_params, request_kwargs)
            == deployment_params
        )
    ...
    async def test_280_concurrent_overrides_never_mutate_or_leak_through_shared_deployment_params(self):
        ...
        results = await asyncio.gather(
            *(
                asyncio.to_thread(
                    derouter.Router._deployment_params_with_request_reasoning_override,
                    deployment_params,
    ...
    def test_only_classifier_calls_drop_effort_for_an_unsupported_fallback(self, metadata, should_drop):
        ...
        deployment: DeploymentTypedDict = {
            "model_name": "fallback",
            "derouter_params": {"model": "openai/gpt-4o-mini"},
        }
...
class TestPreRoutingTierDrivesFallbacks:
    ...
    class _TierRouter(derouter.Router):
        async def async_pre_routing_hook(
            self, model, request_kwargs, messages=None, input=None, specific_deployment=False
        ):
            from derouter.types.router import PreRoutingHookResponse
    ...
    @classmethod
    def _router(cls, fallbacks) -> "derouter.Router":
        return cls._TierRouter(
            model_list=[
                {
                    "model_name": "smart-router",
                    "derouter_params": {"model": "openai/gpt-4o-mini", "api_key": "sk-x"},
                },
                {
                    "model_name": "tier1",
                    "derouter_params": {
    ...
    async def test_a_chain_keyed_on_the_router_name_is_not_used(self):
        ...
        with pytest.raises(derouter.RateLimitError):
...
async def test_prompt_management_factory_marks_injection_for_every_deployment(monkeypatch):
    ...
    async def _capture_acompletion(**kwargs):
        ...
        return derouter.ModelResponse()

    monkeypatch.setattr(derouter, "acompletion", _capture_acompletion)
    ...
    bucket = captured.get("derouter_metadata") or captured["metadata"]
    assert captured["model_info"]["id"] == "provisional-dep"
    assert bucket["derouter_gateway_injected_cache"] == ""
...
@pytest.mark.parametrize(
    ...
    [
        ({"ServiceUnavailableErrorRetries": 0}, 503, derouter.ServiceUnavailableError, 1),
        ({"ServiceUnavailableErrorRetries": 1}, 503, derouter.ServiceUnavailableError, 2),
        ({"InternalServerErrorRetries": 0}, 500, derouter.InternalServerError, 1),
        ({"DefaultRetries": 0}, 502, derouter.BadGatewayError, 1),
        ({"DefaultRetries": 0, "ServiceUnavailableErrorRetries": 1}, 503, derouter.ServiceUnavailableError, 2),
        ({"ServiceUnavailableErrorRetries": 0}, 502, derouter.BadGatewayError, 3),
        ({"BadRequestErrorRetries": 2}, 400, derouter.BadRequestError, 3),
    ],
)
...
async def test_router_retry_policy_400_retries_on_sibling_deployment(
    ...
    assert response._hidden_params["additional_headers"]["x-derouter-attempted-retries"] == 1


_UPSTREAM_400 = {"message": "upstream refused this request", "type": "invalid_request_error", "code": "bad_request"}


def _retry_skip_deployment(deployment_id, host, derouter_params=None, model_info=None):
    return {
        "model_name": "gpt-5.6",
        "derouter_params": {
            "model": "openai/gpt-5.6",
...
def test_router_deployment_ids_to_skip_on_retry(status_code, failed_deployment_id, already_skipped, expected):
    ...
    assert derouter.Router._deployment_ids_to_skip_on_retry(exception, already_skipped) == expected
...
def test_router_retry_skip_stamp_feeds_deployment_ids_to_skip_on_retry(
    ...
    derouter.Router._stamp_retry_skip_deployment_id(exception, kwargs)

    assert derouter.Router._deployment_ids_to_skip_on_retry(exception, None) == expected
...
def test_router_as_retry_skipped_deployment_ids_keeps_only_a_tuple_of_strings(value, expected):
    from derouter.router import _as_retry_skipped_deployment_ids
...
async def test_router_retry_policy_400_keeps_upstream_error_on_order_fallback_hop(monkeypatch: pytest.MonkeyPatch):
    ...
    router = derouter.Router(
        model_list=[
            _retry_skip_deployment("order1", "order1", derouter_params={"order": 1}),
            _retry_skip_deployment("order2", "order2", derouter_params={"order": 2}),
        ],
        num_retries=2,
        retry_policy={"BadRequestErrorRetries": 2},
        disable_cooldowns=True,
    ...
    router = derouter.Router(
        model_list=[
            _retry_skip_deployment(
                "tagged", "tagged", derouter_params={"tags": ["free"]}, model_info={"enable_tag_filtering": True}
            ),
...
async def test_router_retry_policy_400_never_returns_to_a_deployment_that_already_refused(
    ...
    router = derouter.Router(
        model_list=[
            _retry_skip_deployment("first-refuser", "first-refuser", derouter_params={"weight": 1}),
            _retry_skip_deployment("second-refuser", "second-refuser", derouter_params={"weight": 0}),
            _retry_skip_deployment("accepting", "accepting", derouter_params={"weight": 0}),
        ],
        num_retries=3,
...
class _PassthroughAdapter(CustomLogger):
    ...
    def translate_completion_output_params(self, response: derouter.ModelResponse) -> derouter.ModelResponse:
        return response


def _lit_7114_router(derouter_model: str) -> derouter.Router:
    api_base_suffix: Final = "" if derouter_model.startswith("cohere/") else "/v1"
    return derouter.Router(
        model_list=[
            {
                "model_name": "gpt-5.6",
...
_LIT_7114_ASYNC_ENTRYPOINTS: Final[
    Mapping[str, tuple[str, str, int, Mapping[str, object] | bytes, Callable[[derouter.Router], Awaitable[object]]]]
] = {
    "aembedding": (
...
async def test_router_retry_moves_off_the_refusing_deployment_on_every_async_entrypoint(
    ...
    derouter_model, path, refusal_status, success_body, call = _LIT_7114_ASYNC_ENTRYPOINTS[entrypoint]
    monkeypatch.setattr(derouter, "disable_aiohttp_transport", True)
    monkeypatch.setattr(derouter, "adapters", [{"id": "lit-7114", "adapter": _PassthroughAdapter()}])
    router: Final = _lit_7114_router(derouter_model)
...
_LIT_7114_SYNC_ENTRYPOINTS: Final[
    Mapping[str, tuple[str, str, Mapping[str, object], Callable[[derouter.Router], object]]]
] = {
    "embedding": (
        "openai/text-embedding-3-large",
        "/v1/embeddings",
...
def test_router_retry_policy_400_moves_off_the_refusing_deployment_on_every_sync_entrypoint(
    ...
    derouter_model, path, success_body, call = _LIT_7114_SYNC_ENTRYPOINTS[entrypoint]
    monkeypatch.setattr(derouter, "disable_aiohttp_transport", True)
    router: Final = _lit_7114_router(derouter_model)
...
def _make_failure_logging_obj():
    return DeRouterLogging(
        model="gpt-5.6",
        messages=[{"role": "user", "content": "hi"}],
        stream=False,
        call_type="acompletion",
        start_time=datetime.now(),
        derouter_call_id="lit-6960",
        function_id="f",
    )
...
@pytest.mark.parametrize(
    "hook_error",
    [
        derouter.RateLimitError(message="rpm exceeded", llm_provider="openai", model="gpt-5.6"),
...
async def test_async_routing_strategy_pre_call_checks_failure_logging_is_coordinated(hook_error):
    ...
    router = derouter.Router(
        model_list=[{"model_name": "gpt-5.6", "derouter_params": {"model": "openai/gpt-5.6", "api_key": "sk-fake"}}]
    )
    deployment = router.model_list[0]
    logging_obj = _make_failure_logging_obj()

    with patch.object(derouter, "callbacks", [_RaisingPreCallCheck()]):  # test-quality-ok: router reads this global
...
async def test_async_callback_filter_deployments_failure_logging_is_coordinated():
    ...
    with patch.object(derouter, "callbacks", [_RaisingFilter()]):  # test-quality-ok: router reads this global
        await _assert_router_failure_logging_is_coordinated(
            logging_obj,
...
async def test_async_get_available_deployment_failure_logging_is_coordinated():
    ...
    await _assert_router_failure_logging_is_coordinated(
        ...
        lambda: router.async_get_available_deployment(
            ...
            request_kwargs={"derouter_logging_obj": logging_obj},
            messages=[{"role": "user", "content": "hi"}],
        ),
        derouter.BadRequestError,
    )
...
async def test_router_deployment_drop_params_string_true_is_honored(monkeypatch):
    ...
    assert deployment is not None
    assert deployment.derouter_params.drop_params is True
...
def test_router_warns_when_a_deployment_drop_params_string_is_not_a_flag(value, caplog):
    with caplog.at_level(logging.WARNING, logger="DeRouter Router"):
        router = Router(
            model_list=[
                {
                    "model_name": "gpt-5-nano",
                    "derouter_params": {"model": "openai/gpt-5-nano", "api_key": "sk-fake", "drop_params": value},
                }
            ]
        )
    ...
    deployment = router.get_deployment_by_model_group_name(model_group_name="gpt-5-nano")
    assert deployment is not None
    assert deployment.derouter_params.drop_params == value
