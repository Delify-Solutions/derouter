...
def test_patch_decorator_counts_as_installing_a_patch(tmp_path):
    source = (
        ...
        "\n"
        "\n"
        "@mock.patch('derouter.completion')\n"
...
def test_a_test_with_no_assertions_is_tq001_not_tq002(tmp_path):
    source = (
        ...
        "\n"
        "def test_nothing():\n"
        "    with patch('derouter.completion'):\n"
        "        run()\n"
    )
...
def test_derouter_global_write_is_flagged(tmp_path):
    assert _codes(tmp_path, "import derouter\n\nderouter.drop_params = True\n") == ["TQ005"]


def test_derouter_augmented_global_write_is_flagged(tmp_path):
    assert _codes(tmp_path, "import derouter\n\nderouter.num_retries += 1\n") == ["TQ005"]


def test_derouter_attribute_read_is_not_flagged(tmp_path):
    assert _codes(tmp_path, "import derouter\n\nvalue = derouter.drop_params\n") == []
...
_SNAPSHOT_CONFTEST = """import derouter
import pytest
...
def restore_globals():
    ...
    for attr in ("api_base", "num_retries"):
        original_state[attr] = getattr(derouter, attr)
    yield
...
def test_the_names_come_from_the_loop_tuple_as_well_as_the_direct_keys(tmp_path):
    ...
    assert sorted(reported) == ["derouter.api_base", "derouter.drop_params", "derouter.num_retries"]
...
def test_a_snapshot_entry_is_suppressible_with_a_reason(tmp_path):
    source = _SNAPSHOT_CONFTEST.replace(
        'original_state["drop_params"] = derouter.drop_params',
        'original_state["drop_params"] = derouter.drop_params  # test-quality-ok: owned by the SDK config surface',
    )
    assert _conftest_codes(tmp_path, source) == ["TQ007", "TQ007"]


_NAMED_MAPPING_CONFTEST = """import derouter
import pytest
...
def test_a_save_loop_over_a_module_level_dict_counts_its_keys(tmp_path):
    ...
    assert sorted(reported) == [
        "derouter.api_base",
        "derouter.drop_params",
        "derouter.num_retries",
        "derouter.set_verbose",
    ]


def test_a_named_iterable_that_is_not_a_module_constant_is_skipped_quietly(tmp_path):
    source = _NAMED_MAPPING_CONFTEST.replace("for attr in _EXTRA_ATTRS:", "for attr in dir(derouter):")
    reported = [v.message.split("`")[1] for v in checker.check_file(_written(tmp_path, source))]
    assert sorted(reported) == ["derouter.num_retries", "derouter.set_verbose"]
...
_HELPER_DICT_CONFTEST = """import derouter
import pytest
...
def test_a_snapshot_built_in_a_helper_under_any_dict_name_is_counted(tmp_path):
    ...
    assert sorted(reported) == ["derouter.callbacks", "derouter.success_callback"]


def test_the_read_may_sit_a_statement_above_the_store(tmp_path):
    # `val = getattr(derouter, attr)` then `state[attr] = val.copy()` is the common
    # shape; requiring the store itself to read derouter loses every one of them.
...
def test_patching_a_deep_sdk_path_is_flagged(tmp_path):
    source = (
        "from unittest.mock import patch\n\n\n"
        "def test_x():\n"
        '    with patch("derouter.llms.openai.chat.handler.OpenAIChatCompletion.completion"):\n'
        "        assert True\n"
    )
    assert "TQ008" in _codes(tmp_path, source)
...
def test_patch_object_rooted_at_the_sdk_is_flagged(tmp_path):
    source = (
        "import derouter\nfrom unittest.mock import patch\n\n\n"
        "def test_x():\n"
        '    with patch.object(derouter, "api_key", "x"):\n'
        "        assert True\n"
    )
    assert "TQ008" in _codes(tmp_path, source)
...
def test_patch_object_on_a_from_imported_sdk_module_is_flagged(tmp_path):
    source = (
        "from derouter.llms.openai.chat import handler\nfrom unittest.mock import patch\n\n\n"
        "def test_x():\n"
...
def test_patch_object_on_an_aliased_sdk_module_is_flagged(tmp_path):
    source = (
        "import derouter.llms.openai.chat.handler as oai\nfrom unittest.mock import patch\n\n\n"
        "def test_x():\n"
...
def test_patch_object_on_a_renamed_sdk_symbol_is_flagged(tmp_path):
    source = (
        "from derouter.utils import get_llm_provider as glp\nfrom unittest.mock import patch\n\n\n"
        "def test_x():\n"
...
def test_the_reported_target_is_the_resolved_sdk_path(tmp_path):
    ...
    assert reported
    assert "derouter.llms.openai.chat.handler.OpenAIChatCompletion" in reported[0]
...
def test_a_name_merely_starting_with_derouter_is_not_the_sdk(tmp_path):
    source = (
        "from unittest.mock import patch\n\n\n"
        "def test_x():\n"
        '    with patch("derouter_enterprise.thing.go"):\n'
        "        assert True\n"
    )
    assert "TQ008" not in _codes(tmp_path, source)
...
def test_an_sdk_patch_can_be_suppressed(tmp_path):
    source = (
        "from unittest.mock import patch\n\n\n"
        "def test_x():\n"
        '    with patch("derouter.completion"):  # test-quality-ok: pinning the router seam\n'
        "        assert True\n"
    )
    assert "TQ008" not in _codes(tmp_path, source)
