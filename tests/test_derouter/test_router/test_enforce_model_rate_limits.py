...
class TestModelRateLimitingCheck:
    ...
    def test_get_deployment_limits_from_derouter_params(self):
        """Test extracting limits from derouter_params."""
        check = ModelRateLimitingCheck(dual_cache=MagicMock())

        deployment = {
            "derouter_params": {"model": "gpt-4", "tpm": 2000, "rpm": 20},
            "model_info": {"id": "test-id"},
        }
    ...
    def test_log_success_event_increments_cache(self):
        ...
        kwargs = {
            "standard_logging_object": {
                "model_id": "test-id",
                "total_tokens": 50,
                "hidden_params": {"derouter_model_name": "gpt-4"},
            }
        }
...
class TestRouterWithEnforceModelRateLimits:
    ...
    def test_router_optional_callbacks_contains_model_rate_limiting(self):
        ...
        # Find the ModelRateLimitingCheck in derouter.callbacks
        found = False
        ...
        assert found, "ModelRateLimitingCheck should be in derouter.callbacks"
...
class TestModelRateLimitConcurrency:
    ...
    async def test_concurrent_requests_respect_rpm_limit(self):
        ...
        failures = [r for r in results if isinstance(r, derouter.RateLimitError)]
