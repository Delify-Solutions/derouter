...
def test_basedpyright_counts_per_rule_from_json_not_warnings():
    ...
    payload = json.dumps(
        {
            "generalDiagnostics": [
                _bpr(f"{ROOT}/derouter/utils.py", "error", "reportUnknownVariableType"),
                _bpr(f"{ROOT}/derouter/utils.py", "error", "reportUnknownVariableType"),
                _bpr(f"{ROOT}/derouter/main.py", "error", "reportArgumentType"),
                _bpr(f"{ROOT}/derouter/main.py", "warning", "reportUnusedImport"),
            ]
        }
    )
...
def test_basedpyright_error_without_a_rule_is_bucketed():
    payload = json.dumps(
        {"generalDiagnostics": [_bpr(f"{ROOT}/derouter/x.py", "error", None)]}
    )
...
def test_symlinked_root_keeps_diagnostics_in_tree(tmp_path):
    ...
    payload = json.dumps(
        {
            "generalDiagnostics": [
                _bpr(link / "derouter" / "x.py", "error", "reportArgumentType")
            ]
        }
    )
...
def test_origin_slug_parsing_supports_ssh_and_https_github_forms():
    assert gate.parse_origin_slug("git@github.com:Delify-Solutions/derouter.git") == "Delify-Solutions/derouter"
    assert gate.parse_origin_slug("git@github.com:Delify-Solutions/derouter") == "Delify-Solutions/derouter"
    assert gate.parse_origin_slug("https://github.com/Delify-Solutions/derouter.git") == "Delify-Solutions/derouter"
    assert gate.parse_origin_slug("https://github.com/Delify-Solutions/derouter") == "Delify-Solutions/derouter"
    assert gate.parse_origin_slug("https://github.com/Delify-Solutions/derouter/") == "Delify-Solutions/derouter"


def test_origin_slug_parsing_rejects_non_github_urls():
    assert gate.parse_origin_slug("https://gitlab.com/Delify-Solutions/derouter.git") is None
    assert gate.parse_origin_slug("git@bitbucket.org:Delify-Solutions/derouter.git") is None
