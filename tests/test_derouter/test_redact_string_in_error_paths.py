...
from derouter._logging import _ENABLE_SECRET_REDACTION, _redact_string
...
class TestRouterFallbackFailureTracebackRedaction:
    ...
    async def test_fallback_failure_does_not_leak_secret_via_exc_info(self, caplog):
        ...
        router = derouter.Router(
            model_list=[
                ...
                },
                {
                    ...
                    "derouter_params": {"model": "anthropic/claude-3-haiku-20240307", "api_key": "fake-key"},
                },
            ],
        )
        ...
        with patch(
            "derouter.router.run_async_fallback",
            ...
            try:
                ...
                with caplog.at_level(logging.DEBUG, logger="DeRouter Router"):
...
class TestGeminiIngestionHeaders:
    ...
    async def test_create_file_search_store_sends_header(self):
        ...
        with patch(
            "derouter.rag.ingestion.gemini_ingestion.get_async_httpx_client",
