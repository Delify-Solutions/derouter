...
``derouter/`` backup, and verify ``get_model_info`` surfaces them, so the
...
def _main_path() -> str:
    # This test lives at ``tests/test_derouter/``; the primary price map sits at
    # the repo root, two directories up. Resolve it relative to this file so the
    # test works regardless of where ``derouter`` itself is installed (e.g. a pip
...
class TestCommandR7bPricingModelInfo:
    ...
    def test_get_model_info_costs(self):
        # Patch derouter.model_cost with the local backup so the test is not
        # dependent on the remote fetch hitting a not-yet-merged main branch.
        original = derouter.model_cost
        try:
            derouter.model_cost = _load_json(_backup_path())
