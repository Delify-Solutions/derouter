"""
Tests for the RAG query pipeline in derouter/rag/main.py.

The RAG pipeline forwards its kwargs (including the parent derouter_logging_obj)
...
async def test_aquery_single_billing_event_carries_completion_usage_and_cost(use_router):
    """
    derouter.aquery must produce exactly one success event, and that event must
    ...
    derouter.callbacks = [recording_logger]

    router_kwargs = {}
    if use_router:
        router_kwargs["router"] = derouter.Router(
            model_list=[
                {
    ...
    try:
        response = await derouter.aquery(
            model="gpt-4o-mini",
...
async def test_aquery_response_hidden_params_carry_completion_cost():
    ...
    params, so the proxy can return the x-derouter-response-cost header.
    """
    response = await derouter.aquery(
        model="gpt-4o-mini",
...
async def test_aquery_billed_cost_includes_priced_vector_store_search():
    ...
    try:
        with patch("derouter.rag.main.vector_store_search_cost", return_value=(0.002, 0.0)):
            response = await derouter.aquery(
                model="gpt-4o-mini",
                messages=[{"role": "user", "content": "hello"}],
                retrieval_config={"vector_store_id": "vs_test_123", "custom_llm_provider": "openai"},
...
async def test_aquery_with_rerank_bills_once_and_folds_rerank_cost():
    ...
    try:
        with patch("derouter.arerank", side_effect=fake_arerank):
            response = await derouter.aquery(
                model="gpt-4o-mini",
                messages=[{"role": "user", "content": "hello"}],
                retrieval_config={"vector_store_id": "vs_test_123", "custom_llm_provider": "openai"},
...
async def test_aquery_streaming_bills_sub_call_costs_into_final_event():
    ...
    try:
        with (
            patch("derouter.rag.main.vector_store_search_cost", return_value=(0.002, 0.0)),
            patch("derouter.arerank", side_effect=fake_arerank),
        ):
            response = await derouter.aquery(
                model="gpt-4o-mini",
                messages=[{"role": "user", "content": "hello"}],
...
async def test_aquery_forwards_provider_retrieval_config_and_router_to_search():
    ...
    with patch("derouter.vector_stores.asearch", new=fake_search):  # test-quality-ok: asearch is the boundary the forwarding contract under test targets
        response = await derouter.aquery(
            model="gpt-4o-mini",
            messages=[{"role": "user", "content": "hello"}],
            retrieval_config={
...
async def test_aquery_forwards_vector_store_params_to_search_but_not_completion():
    ...
    store's derouter_params) must reach the search call wholesale, including the
    ...
    with (
        ...
        patch("derouter.acompletion", new=fake_completion),  # test-quality-ok: the completion boundary under test
    ):
        await derouter.aquery(
            model="gpt-4o-mini",
            messages=[{"role": "user", "content": "hello"}],
