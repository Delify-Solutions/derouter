...
def _logging_obj(
    ...
    logging_obj = LitellmLogging(
        ...
        derouter_call_id="bg-interactions-call-id",
...
def _logging_obj_with_reservation(reservation: dict) -> LitellmLogging:
    return _logging_obj(derouter_params={"metadata": {"user_api_key_budget_reservation": reservation}})
...
async def test_schedule_creates_poll_task_for_in_progress_create():
    ...
    task = maybe_schedule_background_interaction_cost_polling(
        ...
        create_kwargs={"derouter_logging_obj": logging_obj},
        custom_llm_provider="gemini",
    )
...
@pytest.mark.parametrize(
    ...
    [
        (_response("completed", with_usage=True), {"derouter_logging_obj": "placeholder"}),
        (_response("in_progress", with_usage=False), {}),
        ("not a response", {"derouter_logging_obj": "placeholder"}),
    ],
)
async def test_schedule_skips_non_pollable_results(response, create_kwargs):
    if create_kwargs.get("derouter_logging_obj") == "placeholder":
        create_kwargs = {"derouter_logging_obj": _logging_obj()}
...
def _register_poll(logging_obj: LitellmLogging, poll_fetch=None) -> asyncio.Task:
    import derouter.interactions.background_cost_polling as bg
...
async def test_schedule_respects_kill_switch(monkeypatch):
    import derouter.interactions.background_cost_polling as module
...
def test_every_status_the_api_can_return_is_either_pollable_or_terminal():
    ...
    """
    from derouter.interactions.background_cost_polling import _POLLABLE_STATUSES, _TERMINAL_STATUSES
