...
Tests the derouter.interactions.create() and related methods against the Google AI Studio API.

Per OpenAPI spec: https://ai.google.dev/static/api/interactions.openapi.json

Run with: pytest tests/test_derouter/interactions/test_google_interactions_integration.py -v
"""
...
import derouter
import derouter.interactions as interactions
import openai
...
class TestGoogleInteractionsCreate:
    """Tests for creating interactions via derouter.interactions.create()."""
...
class TestGoogleInteractionsErrorHandling:
    ...
    def test_missing_model_and_agent(self, api_key):
        ...
        with pytest.raises((ValueError, derouter.APIConnectionError)):
            interactions.create(
                input="Hello",
                api_key=api_key,
            )
