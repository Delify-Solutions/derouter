...
_PATCH_GET_API_KEY = "derouter.llms.gemini.common_utils.GeminiModelInfo.get_api_key"
...
class TestValidateEnvironment:
    def test_sets_x_goog_api_key_header(self, config):
        derouter_params = GenericDeRouterParams(api_key="test-api-key-123")
    ...
    def test_no_api_key_skips_header(self, config):
        derouter_params = GenericDeRouterParams(api_key=None)
    ...
    def test_no_derouter_params_skips_header(self, config):
        headers = config.validate_environment(
            headers={},
            model="gemini-2.5-flash",
    ...
    def test_preserves_existing_headers(self, config):
        derouter_params = GenericDeRouterParams(api_key="test-key")
    ...
    def test_api_revision_new_schema_by_default(self, config, monkeypatch: pytest.MonkeyPatch):
        ...
        monkeypatch.setattr(derouter, "use_legacy_interactions_schema", False)
        headers = config.validate_environment(
            headers={}, model="gemini-2.5-flash", derouter_params=None
        )
    ...
    def test_api_revision_legacy_schema_when_flag_set(self, config, monkeypatch: pytest.MonkeyPatch):
        ...
        monkeypatch.setattr(derouter, "use_legacy_interactions_schema", True)
        headers = config.validate_environment(
            headers={}, model="gemini-2.5-flash", derouter_params=None
        )
    ...
    def test_url_excludes_api_key(self, config):
        with patch(_PATCH_GET_API_KEY, return_value="secret-key"):
            url = config.get_complete_url(
                api_base=None,
                model="gemini-2.5-flash",
                derouter_params={"api_key": "secret-key"},
            )
...
class TestStreamingIterator:
    def _make_iterator(
        ...
        original = derouter.use_legacy_interactions_schema
        derouter.use_legacy_interactions_schema = use_legacy
        try:
            return DeRouterResponsesInteractionsStreamingIterator(
                model="gpt-5.4",
                derouter_custom_stream_wrapper=MagicMock(),
                request_input="hi",
                optional_params={},
            )
        finally:
            derouter.use_legacy_interactions_schema = original
    ...
    def test_stop_iteration_fallback_emits_completion_event(self):
        ...
        original = derouter.use_legacy_interactions_schema
        derouter.use_legacy_interactions_schema = False
        try:
            it = DeRouterResponsesInteractionsStreamingIterator(
                model="gpt-5.4",
                derouter_custom_stream_wrapper=sync_iter,
                request_input="hi",
                optional_params={},
            )
        finally:
    ...
    def test_url_excludes_key(
        ...
        with patch(_PATCH_GET_API_KEY, return_value="secret-key"):
            url, params = getattr(config, method_name)(
                ...
                derouter_params=GenericDeRouterParams(api_key="secret-key"),
                headers={},
            )
