"""
Tests for DeRouter Responses bridge provider.

Inherits from BaseInteractionsTest to run the same test suite against
the derouter_responses bridge provider, which calls derouter.responses() internally.
"""
...
from derouter.types.interactions import Turn
from tests.test_derouter.interactions.base_interactions_test import (
    BaseInteractionsTest,
)


class TestDeRouterResponsesBridge(BaseInteractionsTest):
    """Test DeRouter Responses bridge using the base test suite."""

    def get_model(self) -> str:
        """Return the model string for the bridge provider.

        The bridge provider uses derouter.responses() internally, so we can
        use any model that derouter.responses() supports (e.g., gpt-4o).
        """
        return "gpt-4o"
...
class TestBridgeInputTransformation:
    ...
    def test_step_input_maps_roles_and_content_types(self):
        transformed = DeRouterResponsesInteractionsConfig._transform_interactions_input_to_responses_input(
            [
    ...
    def test_string_input_passes_through(self):
        transformed = DeRouterResponsesInteractionsConfig._transform_interactions_input_to_responses_input("Hello")
