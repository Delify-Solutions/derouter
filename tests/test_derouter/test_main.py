...
import derouter
from derouter import main as derouter_main
...
def clear_client_cache():
    ...
    """
    cache = getattr(derouter, "in_memory_llm_clients_cache", None)
...
async def test_url_with_format_param(model, sync_mode, monkeypatch):
    from derouter import acompletion, completion
    from derouter.llms.custom_httpx.http_handler import AsyncHTTPHandler, HTTPHandler
    from derouter.derouter_core_utils.prompt_templates import factory as prompt_factory
...
def test_strip_input_examples_for_non_anthropic_providers():
    ...
    assert not derouter_main._should_allow_input_examples(
        custom_llm_provider="openai", model="gpt-4o-mini"
    )

    cleaned = derouter_main._drop_input_examples_from_tools(tools=tools)
...
async def test_extra_body_with_fallback(
    ...
    """
    test regression for https://github.com/Delify-Solutions/derouter/issues/8425.
...
async def test_openai_env_base(
    ...
    try:
        response = await derouter.acompletion(model=model, messages=messages)
...
def test_responses_api_bridge_check_strips_responses_prefix():
    ...
    with patch("derouter.main._get_model_info_helper") as mock_get_model_info:
...
def test_responses_api_bridge_check_gpt_5_4_pro():
    ...
    Regression test for https://github.com/Delify-Solutions/derouter/issues/23014
...
def test_responses_api_bridge_check_gpt_5_6_tools_with_default_reasoning_routes_to_responses(
    ...
    monkeypatch.setattr(derouter, "api_base", None)
...
def test_responses_api_bridge_check_dict_effort_none_stays_chat():
    """The escape hatch must honor derouter's dict form: {"effort": "none"} means reasoning off."""
    from derouter.main import responses_api_bridge_check
...
def test_responses_api_bridge_check_custom_api_base_via_global_with_unset_effort_stays_chat(monkeypatch):
    """
    A custom base set through the derouter.api_base global (not the call arg) is resolved the
    ...
    monkeypatch.setattr(derouter, "api_base", "http://vllm.internal:8000/v1")
    with patch("derouter.main._get_model_info_helper") as mock_get_model_info:
        mock_get_model_info.return_value = {"max_tokens": 128000}
        model_info, model = responses_api_bridge_check(
            model="gpt-5.6",
...
@patch("derouter.completion_extras.responses_api_bridge.completion")
...
def test_completion_optional_params_base_model(
    ...
    with patch("derouter.main.get_optional_params") as mock_get_optional_params:
...
def test_responses_bridge_preserves_reasoning_summary_without_effort(
    ...
    with patch.object(derouter, "route_all_chat_openai_to_responses", True):
        derouter.completion(
            model="gpt-4o",
            messages=[{"role": "user", "content": "ok"}],
            reasoningSummary="auto",
...
def test_responses_api_bridge_check_global_flag_default_false():
    ...
    with patch.object(derouter, "route_all_chat_openai_to_responses", False):
        with patch("derouter.main._get_model_info_helper") as mock_get_model_info:
            mock_get_model_info.return_value = {"max_tokens": 4096}
            model_info, model = responses_api_bridge_check(
                model="gpt-4o",
...
async def test_async_mock_delay():
    ...
    from derouter import acompletion
...
def test_stream_chunk_builder_thinking_blocks():
    ...
    from derouter.types.utils import Delta, ModelResponseStream, StreamingChoices
...
from derouter.llms.openai.openai import OpenAIChatCompletion
...
async def test_retrying() -> None:
    ...
    with (
        ...
        pytest.raises(derouter.InternalServerError, match="DeRouter Retried: 10 times"),
    ):
...
def test_anthropic_disable_url_suffix_env_var():
    """Test that DEROUTER_ANTHROPIC_DISABLE_URL_SUFFIX prevents /v1/messages suffix."""
    import os
    from unittest.mock import MagicMock, patch
    ...
    with patch.dict(os.environ, {"ANTHROPIC_API_BASE": "https://api.example.com"}):
        ...
        with patch("derouter.main.anthropic_chat_completions") as mock_anthropic:
    ...
    with patch.dict(
        os.environ,
        {
            ...
            "DEROUTER_ANTHROPIC_DISABLE_URL_SUFFIX": "true",
        },
    ):
        actual_api_base = None
...
def test_anthropic_text_disable_url_suffix_env_var():
    """Test that DEROUTER_ANTHROPIC_DISABLE_URL_SUFFIX prevents /v1/complete suffix for anthropic_text."""
    import os
    from unittest.mock import MagicMock, patch
    ...
    with patch.dict(os.environ, {"ANTHROPIC_API_BASE": "https://api.example.com"}):
        ...
        with patch("derouter.main.base_llm_http_handler") as mock_handler:
...
def test_image_edit_merges_headers_and_extra_headers():
    from derouter.images.main import base_llm_http_handler
    ...
    with (
        patch(
            "derouter.images.main.ProviderConfigManager.get_provider_image_edit_config",
...
def test_mock_completion_stream_with_model_response():
    ...
    from derouter import completion
    from derouter.types.utils import Choices, Message, ModelResponse, Usage
...
class TestCallTypesOCR:
    ...
    Fixes https://github.com/Delify-Solutions/derouter/issues/17381
    """
...
def test_stream_chunk_builder_text_completion_combines_text_and_usage():
    from derouter.main import stream_chunk_builder_text_completion
...
def test_completion_forwards_store_and_prompt_cache_key_to_openai():
    """
    Regression test for https://github.com/Delify-Solutions/derouter/issues/33184
...
async def test_acompletion_forwards_store_and_prompt_cache_key_to_openai():
    ...
    https://github.com/Delify-Solutions/derouter/issues/33184
    """
...
def test_completion_forwards_store_and_prompt_cache_key_to_mcp_gateway():
    ...
    """
    with patch.object(
        import_module("derouter.responses.mcp.chat_completions_handler"), "acompletion_with_mcp"
    ) as mock_mcp:
        result = derouter.completion(
            model="openai/gpt-4o",
            messages=[{"role": "user", "content": "Hello"}],
            tools=[{"type": "mcp", "server_url": "derouter_proxy"}],
            store=False,
            prompt_cache_key="test-cache-key",
        )
...
@pytest.mark.parametrize(
    ...
    [
        {
            "aws_session_name": "derouter-gcp",
            "aws_role_name": "arn:aws:iam::123456789012:role/derouter-bedrock-role",
...
def restore_model_registry():
    """derouter.model_cost and the provider name sets are module-global.

    register_model merges into the existing entry in place, hence the deep copy.
    """
    model_cost = copy.deepcopy(derouter.model_cost)
    openai_models = set(derouter.open_ai_chat_completion_models)
    yield
    derouter.model_cost.clear()
    derouter.model_cost.update(model_cost)
    derouter.open_ai_chat_completion_models.clear()
    derouter.open_ai_chat_completion_models.update(openai_models)


def test_openai_model_name_does_not_outrank_explicit_provider():
    """`gemini/gpt-4o` goes to Google, not to derouter's OpenAI handler.

    completion() checks `model in derouter.open_ai_chat_completion_models` ahead of
    ...
    """
    assert "gpt-4o" in derouter.open_ai_chat_completion_models
...
def test_mislabelled_pricing_entry_does_not_reroute_provider(restore_model_registry):
    ...
    An entry claiming derouter_provider "openai" adds its name to
    ...
    assert "gemini-2.5-pro" in derouter.open_ai_chat_completion_models
    client, post = _gemini_client_returning_a_reply()
...
def _priced_at(prompt_tokens, completion_tokens):
    prices = derouter.model_cost[STREAM_COST_MODEL]
    return (
...
def test_a_streamed_response_bills_the_usage_the_provider_reported(local_cost_map):
    rebuilt = derouter.stream_chunk_builder(
        chunks=[
            _text_chunk("Hello"),
            _text_chunk(" there"),
    ...
    cost = derouter.completion_cost(completion_response=rebuilt, model=STREAM_COST_MODEL)
...
def test_streaming_and_not_streaming_bill_the_same_usage_the_same(local_cost_map):
    ...
    ) == pytest.approx(derouter.completion_cost(completion_response=whole, model=STREAM_COST_MODEL))
...
def _record_speech_success(payload: dict[str, object]) -> _RecordedSpeechSuccess:
    ...
    return _RecordedSpeechSuccess(
        ...
        spend_metadata=get_derouter_metadata_from_kwargs(payload),
...
async def test_aspeech_gemini_bridge_keeps_proxy_metadata_for_spend_tracking(
    ...
    expected_prompt_cost, expected_completion_cost = derouter.cost_per_token(
        model="gemini/gemini-2.5-flash-preview-tts",
...
def test_stream_chunk_builder_sets_hidden_response_cost_for_known_model():
    ...
    response: Final = derouter.stream_chunk_builder(chunks=chunks, messages=[{"role": "user", "content": "hi"}])

    assert response is not None
    prompt_cost, completion_cost = derouter.cost_per_token(model="gpt-4o", usage_object=response.usage)
...
def test_stream_chunk_builder_prices_proxy_alias_via_model_map():
    ...
    prompt_cost, completion_cost = derouter.cost_per_token(model="claude-opus-5", usage_object=response.usage)
    expected_cost: Final = prompt_cost + completion_cost
    assert expected_cost > 0
    assert response._hidden_params["response_cost"] == pytest.approx(expected_cost)


def _stream_builder_logging_obj(model: str = "gpt-4o", custom_llm_provider: str = "openai") -> DeRouterLogging:
    logging_obj: Final = DeRouterLogging(
        model=model,
        messages=[{"role": "user", "content": "hi"}],
        stream=True,
...
def test_stream_chunk_builder_stamps_streaming_usage_cost_by_default(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(derouter, "include_cost_in_streaming_usage", False)
    chunks: Final = [
        _stream_builder_text_chunk("gpt-4o", "Hello "),
        _stream_builder_text_chunk("gpt-4o", "world.", finish_reason="stop"),
    ]

    response: Final = derouter.stream_chunk_builder(
...
def test_stream_chunk_builder_skips_stamp_when_cost_is_unpriceable():
    ...
    logging_obj: Final = DeRouterLogging(
        ...
        derouter_call_id="stream-builder-alias-unpriceable",
        function_id="1",
    )
...
def test_speech_mistral_dispatches_and_decodes_audio(respx_mock: respx.MockRouter, monkeypatch: pytest.MonkeyPatch):
    ...
    response: Final = derouter.speech(
        model="mistral/voxtral-mini-tts-2603",
        input="hello from derouter",
...
def test_azure_ai_transcription_on_a_foundry_host_uses_the_azure_openai_deployment_route(
    ...
    response: Final = derouter.transcription(
