...
from derouter._redis_credential_provider import (
...
from derouter.constants import REDIS_CLUSTER_HEALTH_CHECK_INTERVAL
...
def test_pretty_print_never_expands_credential_provider(capsys):
    ...
    with patch(  # test-quality-ok: enable the debug-only printer without changing process-wide logger state
        "derouter._redis.verbose_logger.isEnabledFor", return_value=True
    ):
        _pretty_print_redis_config(
            redis_kwargs={
                "host": "redis-host",
...
@patch(  # test-quality-ok: redis-py >= 6 keeps no cluster_error_retry_attempts attribute on the built client, so the constructor call is the only place the value is observable
    "derouter.caching.redis_cluster_node_isolation.get_derouter_async_redis_cluster_class"
)
...
@patch("derouter.caching.redis_cluster_node_isolation.get_derouter_async_redis_cluster_class")
...
def test_get_redis_async_client_with_connection_pool():
    ...
    with (
        patch("derouter._redis.async_redis.Redis") as mock_redis,
        patch("derouter._redis._get_redis_client_logic") as mock_logic,
    ):
...
def test_gcp_iam_credential_provider_get_credentials():
    ...
    with patch(
        "derouter._redis_credential_provider._generate_gcp_iam_access_token",
        return_value="tok-1",
    ) as mock_gen:
...
def test_gcp_iam_credential_provider_refreshes_on_expiry():
    ...
    import derouter._redis_credential_provider as cred_module
...
def test_get_redis_async_client_gcp_cluster_uses_credential_provider():
    ...
    with (
        ...
        patch("derouter._redis._get_redis_client_logic", return_value=redis_kwargs),
    ):
...
@patch("derouter._redis.init_redis_cluster")
...
@patch("derouter._redis.redis.Sentinel")
...
@patch("derouter._redis.async_redis.Sentinel")
...
@patch("derouter._redis.redis.Redis.from_url")
...
def test_connection_pool_falsy_ssl_uses_plain_connection(falsy_ssl, monkeypatch):
    ...
    with patch("derouter._redis.async_redis.BlockingConnectionPool") as mock_pool:
...
def test_init_arg_names_sees_through_decorated_inits():
    ...
    from derouter._redis import _init_arg_names
...
def test_url_allowlist_always_carries_socket_timeouts():
    ...
    """
    from derouter._redis import _get_redis_url_kwargs
...
def test_async_url_client_authenticates_through_credential_provider(markers, provider_cls):
    ...
    with patch("derouter._redis._get_redis_client_logic", return_value=redis_kwargs):
        client = get_redis_async_client()
...
def test_async_sentinel_keeps_the_credential_provider_off_the_monitors(markers, provider_cls, sentinel_password):
    ...
    with patch("derouter._redis.async_redis.Sentinel") as mock_sentinel_cls:
        with patch("derouter._redis._get_redis_client_logic", return_value=redis_kwargs):
            get_redis_async_client()
