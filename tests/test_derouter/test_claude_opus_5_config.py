...
DeRouter: the cacheable-prefix minimum drops to 512 tokens, and Bedrock's Opus 5
...
``derouter.anthropic_models`` at import, which is what lets a bare
...
from derouter.constants import BEDROCK_CONVERSE_MODELS
...
def test_opus_5_bedrock_rejects_strict_tools(model_name, local_model_cost_map):
    ...
    Without the flag DeRouter forwards ``strict`` and every tool call 400s."""
    from derouter.llms.bedrock.common_utils import bedrock_converse_supports_strict_tools
...
def test_opus_5_prompt_cache_minimum_is_512(local_model_cost_map):
    ...
    from derouter.utils import get_prompt_cache_min_tokens
...
def test_opus_5_present_in_bundled_backup():
    ...
    ``DEROUTER_LOCAL_MODEL_COST_MAP=True``); it must carry the same entries as the
