"""
Unit tests for derouter.compress().
"""
...
import derouter
from derouter.compression.scoring.bm25 import bm25_score_messages
from derouter.compression.scoring.embedding_scorer import embedding_score_messages
from derouter.compression.content_detection import detect_content_type
from derouter.compression.message_stubbing import extract_key, stub_message
from derouter.compression.retrieval_tool import build_retrieval_tool
...
def test_stub_message():
    ...
    assert "derouter_content_retrieve" in stubbed["content"]
...
def test_retrieval_tool_schema():
    ...
    assert tool["function"]["name"] == "derouter_content_retrieve"
...
def test_compress_below_trigger_passthrough():
    ...
    result = derouter.compress(messages, model="gpt-4o", call_type=CALL_TYPE)
...
def test_compress_above_trigger():
    ...
    assert result["tools"][0]["function"]["name"] == "derouter_content_retrieve"
...
def test_compress_anthropic_list_content_is_boundary_stable():
    ...
    assert result["tools"][0]["name"] == "derouter_content_retrieve"
...
def test_compress_forwards_embedding_model_params(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.compression.scoring.embedding_scorer.embedding_score_messages",
...
def test_embedding_scorer_forwards_embedding_model_params(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "embedding", fake_embedding)
...
def test_compress_anthropic_drops_irrelevant_tool_exchange_span(monkeypatch):
    compress_module = importlib.import_module("derouter.compression.compress")
...
def test_compress_anthropic_keeps_relevant_tool_exchange_span(monkeypatch):
    ...
    assert "derouter_content_retrieve" in result["messages"][0]["content"]
