...
def test_undated_azure_audio_alias_matches_dated_entry(undated, dated):
    undated_info = derouter.get_model_info(undated)
    dated_info = derouter.get_model_info(dated)
    ...
    assert undated_info.get("derouter_provider") == "azure"
...
def test_undated_azure_audio_alias_is_in_the_root_cost_map(undated, dated):
    """`local_model_cost_map` pins `derouter.model_cost` to the packaged backup, but a
    ...
    assert root_map[undated] == derouter.model_cost[undated], (
