"""
Unit test for https://github.com/Delify-Solutions/derouter/issues/22285

Verifies that extra_headers passed to image_generation() are forwarded
to the OpenAI SDK on the openai/derouter_proxy/openai_compatible_providers
code paths.
"""
...
class TestImageGenerationExtraHeaders:
    ...
    @patch("derouter.images.main.openai_chat_completions")
    def test_extra_headers_forwarded_to_openai_image_generation(
        ...
        """
        mock_image_response = derouter.utils.ImageResponse(
            created=1234567890,
