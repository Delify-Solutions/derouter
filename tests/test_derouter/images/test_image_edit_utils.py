...
class MockImageEditConfig(BaseImageEditConfig):
    ...
    def get_complete_url(self, model: str, api_base: str, derouter_params: dict) -> str:
    ...
    def validate_environment(
        ...
        derouter_params: Optional[dict] = None,
...
class TestImageEditRequestUtilsDropParams:
    def setup_method(self):
        ...
        self._original_drop_params = getattr(derouter, "drop_params", None)

    def teardown_method(self):
        if self._original_drop_params is None:
            if hasattr(derouter, "drop_params"):
                delattr(derouter, "drop_params")
        else:
            derouter.drop_params = self._original_drop_params
    ...
    def test_unsupported_params_raises_without_drop(self):
        ...
        with pytest.raises(derouter.UnsupportedParamsError) as exc_info:
...
class TestImageEditCustomPricing:
    """
    Regression tests for https://github.com/Delify-Solutions/derouter/issues/22244

    image_edit must forward model_info and metadata into derouter_params
    ...
    def test_image_edit_passes_model_info_to_logging(self):
        ...
        image_edit should include model_info and metadata in derouter_params.
        """
        from derouter.images.main import image_edit
        ...
        def capturing_update(**update_kwargs):
            captured_derouter_params.update(update_kwargs.get("derouter_params", {}))
            inner_kwargs = update_kwargs.get("kwargs", {})
            if "metadata" in inner_kwargs:
                captured_derouter_params["metadata"] = inner_kwargs["metadata"]
        ...
        with (
            patch(
                "derouter.images.main.get_llm_provider",
        ...
        assert "model_info" in captured_derouter_params
        assert captured_derouter_params["model_info"] == custom_model_info
        assert "metadata" in captured_derouter_params
        assert captured_derouter_params["metadata"] == custom_metadata
    ...
    def test_custom_pricing_not_detected_without_model_info(self):
        derouter_params = {"derouter_call_id": "test-call-id"}
...
class TestImageEditHandlerCredentialsForwarding:
    ...
    image_edit handler must forward derouter_params to validate_environment,
    ...
    def test_vertex_gemini_image_edit_reads_credentials_from_derouter_params(self):
        """
        VertexAIGeminiImageEditConfig.validate_environment should read
        vertex_ai_project/vertex_ai_credentials from derouter_params first.
        """
        from derouter.llms.vertex_ai.image_edit.vertex_gemini_transformation import (
    ...
    def test_vertex_imagen_image_edit_reads_credentials_from_derouter_params(self):
        """
        VertexAIImagenImageEditConfig.validate_environment should read
        vertex_ai_project/vertex_ai_credentials from derouter_params first.
        """
        from derouter.llms.vertex_ai.image_edit.vertex_imagen_transformation import (
    ...
    def test_vertex_imagen_get_complete_url_reads_project_and_location_from_derouter_params(
        self,
    ):
        """
        VertexAIImagenImageEditConfig.get_complete_url should read
        vertex_ai_project and vertex_ai_location from derouter_params,
    ...
    def test_validate_environment_signature_includes_derouter_params(self):
        """
        All image_edit config validate_environment methods should accept
        derouter_params to allow credentials to be forwarded from the handler.
        """
        import inspect
        ...
        from derouter.llms.openai.image_edit.transformation import (
        ...
        for config in configs:
            ...
            assert "derouter_params" in params, (
                f"{config.__class__.__name__}.validate_environment "
                "missing derouter_params parameter"
            )
