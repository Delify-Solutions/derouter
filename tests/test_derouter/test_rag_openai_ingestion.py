...
async def _run_openai_existing_file_id_attach_test():
    ...
    with (
        patch(
            "derouter.rag.ingestion.openai_ingestion.vector_store_file_acreate",
            new_callable=AsyncMock,
        ) as mock_attach,
        patch(
            "derouter.rag.ingestion.openai_ingestion.derouter.acreate_file",
            new_callable=AsyncMock,
        ) as mock_upload,
    ):
...
async def _run_openai_existing_file_id_requires_vector_store_id_test():
    ...
    with (
        patch(
            "derouter.rag.ingestion.openai_ingestion.vector_store_acreate",
            new_callable=AsyncMock,
