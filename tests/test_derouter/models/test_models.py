...
from derouter.models.credentials import CreateCredentialItem, CredentialItem
...
from derouter.models.team import (
...
from derouter.models.verification_token import (
...
class TestModel:
    def test_model_creation(self):
        model = DeRouter_ProxyModelTable(
            model_id="test-model-id",
            model_name="gpt-4",
            derouter_params={"model": "gpt-4", "api_key": "test"},
    ...
    def test_is_blocked(self):
        model_blocked = DeRouter_ProxyModelTable(
            model_id="m1", model_name="test", derouter_params={}, blocked=True
        )
        model_unblocked = DeRouter_ProxyModelTable(
            model_id="m2", model_name="test", derouter_params={}, blocked=False
        )
    ...
    def test_parses_json_string_fields(self):
        ...
        assert model.derouter_params == {"model": "gpt-4"}
    ...
    def test_response_type_adapter_accepts_pydantic_row(self):
        ...
        model = TypeAdapter(DeRouter_ProxyModelTable | None).validate_python(
            row,
    ...
    def test_team_helpers_none_when_no_model_info(self):
        model = DeRouter_ProxyModelTable(
            model_id="m1", model_name="gpt-4", derouter_params={}, model_info=None
        )
...
class TestObjectPermission:
    def test_object_permission_creation(self):
        perm = DeRouter_ObjectPermissionTable(
...
class TestOrganization:
    def test_organization_creation(self):
        org = DeRouter_OrganizationTable(
...
class TestTeam:
    ...
    def test_members_with_roles_empty_dict_coerced(self):
        team = DeRouter_TeamTable(team_id="t3", members_with_roles={})
...
class TestUser:
    ...
    def test_is_over_budget(self):
        user = DeRouter_UserTable(user_id="u1", max_budget=100.0, spend=150.0)
        user_no_budget = DeRouter_UserTable(user_id="u2", spend=1000.0)

        assert user.is_over_budget()
        assert not user_no_budget.is_over_budget()

    def test_has_model_access(self):
        user_with_models = DeRouter_UserTable(user_id="u1", models=["gpt-4"])
        user_no_models = DeRouter_UserTable(user_id="u2", models=[])
    ...
    def test_password_hash_excluded_from_serialization(self):
        from derouter.proxy._types import DeRouter_UserTableWithKeyCount

        secret = "$2b$12$abcdefghijklmnopqrstuv"
        user = DeRouter_UserTable(user_id="u1", user_email="a@b.c", password=secret)

        assert user.password == secret
        assert "password" not in user.model_dump()
        assert "password" not in user.model_dump_json()

        with_keys = DeRouter_UserTableWithKeyCount(
...
class TestVerificationToken:
    def test_verification_token_creation(self):
        token = DeRouter_VerificationToken(
            token="sk-test123",
            key_name="Test Key",
            user_id="user-123",
            team_id="team-123",
    ...
    def test_expires_accepts_string_and_datetime(self):
        as_str = DeRouter_VerificationToken(token="t1", expires="2024-12-31T23:59:59Z")
        as_dt = DeRouter_VerificationToken(token="t2", expires=datetime.utcnow())
    ...
    def test_deleted_verification_token(self):
        deleted = DeRouter_DeletedVerificationToken(
            token="t1",
            deleted_by="admin",
            deleted_at=datetime.utcnow(),
        )
...
class TestConfigTable:
    def test_config_creation(self):
        cfg = DeRouter_Config(param_name="general_settings", param_value={"k": "v"})
...
class TestSkillsTable:
    ...
    def test_skills_defaults(self):
        skill = DeRouter_SkillsTable(skill_id="s2")
...
class TestTagTable:
    ...
    def test_tag_set_model_info_coerces_none(self):
        tag = DeRouter_TagTable(tag_name="t", spend=None, models=None)
...
class TestEndUserTable:
    ...
    def test_end_user_spend_coerced_when_none(self):
        eu = DeRouter_EndUserTable(user_id="eu2", blocked=True, spend=None)
        assert eu.spend == 0.0
...
class TestBudgetTableFull:
    def test_full_adds_server_managed_fields(self):
        now = datetime.now()
        budget = DeRouter_BudgetTableFull(
    ...
    def test_full_requires_created_at(self):
        with pytest.raises(ValidationError):
            DeRouter_BudgetTableFull(budget_id="b1")


class TestTeamMemberTable:
    def test_tracks_user_within_team(self):
        member = DeRouter_TeamMemberTable(
...
class TestTeamMembership:
    def test_safe_get_limits_with_budget_table(self):
        membership = DeRouter_TeamMembership(
            user_id="u1",
            team_id="t1",
            derouter_budget_table=DeRouter_BudgetTable(rpm_limit=100, tpm_limit=2000),
        )
    ...
    def test_safe_get_limits_without_budget_table(self):
        membership = DeRouter_TeamMembership(user_id="u1", team_id="t1")
    ...
    def test_full_budget_variant_parsed_for_server_fields(self):
        ...
        assert isinstance(membership.derouter_budget_table, DeRouter_BudgetTableFull)
        assert membership.safe_get_team_member_rpm_limit() == 7


class TestMCPServerTable:
    def test_mcp_server_defaults(self):
        server = DeRouter_MCPServerTable(server_id="s1", transport="sse")
    ...
    def test_mcp_server_requires_transport(self):
        with pytest.raises(ValidationError):
            DeRouter_MCPServerTable(server_id="s1")


class TestSpendLogs:
    def test_spend_logs_creation(self):
        log = DeRouter_SpendLogs(
            request_id="r1",
            api_key="sk-1",
...
class TestManagedTables:
    def test_managed_file_table(self):
        table = DeRouter_ManagedFileTable(
            unified_file_id="f1",
    ...
    def test_managed_vector_stores_table(self):
        table = DeRouter_ManagedVectorStoresTable(
            vector_store_id="vs1",
