...
def load_native(native_path: Path) -> object:
    module_spec: Final = importlib.util.spec_from_file_location("derouter.rust_bridge._native", native_path)
...
def verify_wheel(wheel: Path) -> int:
    with tempfile.TemporaryDirectory() as temporary_directory, zipfile.ZipFile(wheel) as archive:
        ...
        native_members: Final = tuple(
            member
            for member in archive.infolist()
            if member.filename.startswith("derouter/rust_bridge/_native.") and member.filename.endswith(".so")
        )
