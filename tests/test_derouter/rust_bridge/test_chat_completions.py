...
class TestGate:
    def test_declines_when_the_deployment_did_not_opt_in(self, monkeypatch):
        monkeypatch.delenv("DEROUTER_RUST", raising=False)
        gate = _RecordingDecline()
        bridge.set_rust_chat_completions(decline=gate)
        assert _accepts(derouter_params={}) is False
        assert _accepts(derouter_params=None) is False
    ...
    def test_process_enable_applies_without_request_override(self):
        ...
        assert _accepts(derouter_params={}) is True

    def test_the_env_var_opts_in_without_a_per_model_flag(self, monkeypatch):
        monkeypatch.setenv("DEROUTER_RUST", "true")
        bridge.set_rust_chat_completions(decline=_RecordingDecline())
        assert _accepts(derouter_params={}) is True
    ...
    def test_declines_an_anthropic_request_carrying_a_derouter_metadata_user_id(self, monkeypatch):
        ...
        gate = _RecordingDecline()
        bridge.set_rust_chat_completions(decline=gate)
        assert _accepts(derouter_params={"metadata": {"user_id": "u-123"}}) is False
        ...
        assert (
            _accepts(
                ...
                derouter_params={"metadata": {"user_id": "u-123"}},
            )
            is True
        )
        assert _accepts(derouter_params={"metadata": {"trace_id": "t-1"}}) is True
        assert _accepts(derouter_params={"metadata": {"user_id": None}}) is True
        assert _accepts(derouter_params={"metadata": None}) is True
    ...
    def test_declines_a_bedrock_request_while_the_proxy_owns_request_metadata(self, monkeypatch):
        ...
        Converse body from `derouter_params`, and owning that field also means
        ...
        monkeypatch.setattr(derouter, "bedrock_request_metadata_fields", ["user_api_key_team_id"])
...
class TestSyncCall:
    def test_builds_a_model_response_and_stamps_the_rust_header(self):
        ...
        assert result.id == original_id, "the rust path must keep the chatcmpl id derouter already minted"
