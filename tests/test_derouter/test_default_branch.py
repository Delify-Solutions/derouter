...
def remote_and_clone(tmp_path: Path) -> tuple[Path, Path]:
    ...
    seed.mkdir()
    _git(seed, "init", "-q", "-b", "derouter_internal_staging")
    (seed / "scripts").mkdir()
    for name in (
        "default_branch.py",
    ...
    _git(remote, "symbolic-ref", "HEAD", "refs/heads/derouter_internal_staging")
...
def test_existing_single_branch_clone_follows_remote_switch(remote_and_clone: tuple[Path, Path]) -> None:
    ...
    assert before.stdout.strip() == "origin/derouter_internal_staging"
    ...
    assert _git(repo, "symbolic-ref", "refs/remotes/origin/HEAD").endswith("/derouter_internal_staging")
...
@pytest.mark.parametrize("base_ref", ["HEAD", "origin/derouter_internal_staging"])
def test_explicit_base_works_without_remote_access(
    ...
    assert "No changed derouter Python files" in checked.stdout
...
def test_budget_ratchet_compares_against_new_default(remote_and_clone: tuple[Path, Path]) -> None:
    ...
    _git(repo, "checkout", "-qb", "derouter_feature", "origin/main")
    ...
    overridden: Final = subprocess.run(
        [*command, "--base", "origin/derouter_internal_staging"],
        cwd=repo,
        capture_output=True,
        text=True,
        check=False,
...
def test_migration_freshness_refuses_stale_branch_after_switch(remote_and_clone: tuple[Path, Path]) -> None:
    ...
    overridden: Final = _freshness(repo, "derouter_internal_staging")
...
def test_migration_freshness_refuses_unavailable_remote(remote_and_clone: tuple[Path, Path]) -> None:
    ...
    explicit: Final = _freshness(repo, "derouter_internal_staging")
    assert explicit.returncode == 3
    assert "git fetch origin derouter_internal_staging" in explicit.stderr
