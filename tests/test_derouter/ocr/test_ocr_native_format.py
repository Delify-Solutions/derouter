...
from derouter.ocr.main import _PreparedOCRRequest, _rust_ocr_supported
...
@pytest.mark.parametrize("optional_params", [{}, {"req_format": "derouter"}])
...
async def test_native_format_rejected_for_provider_without_support_as_bad_request():
    with pytest.raises(derouter.BadRequestError, match="not supported for provider") as exc_info:
        await derouter.aocr(
...
async def test_unknown_format_rejected_for_provider_without_support_as_bad_request():
    with pytest.raises(derouter.BadRequestError, match="Invalid `req_format`") as exc_info:
        await derouter.aocr(
            model="mistral/mistral-ocr-latest",
            document=DOCUMENT,
            api_key="fake-key",
