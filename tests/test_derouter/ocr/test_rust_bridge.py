...
# `derouter/__init__.py` does `from .ocr.main import *`, which binds the `ocr`
# function onto `derouter.ocr` and shadows the submodule, so import the modules
# explicitly via importlib rather than attribute traversal.
ocr_main = importlib.import_module("derouter.ocr.main")
rust_bridge = importlib.import_module("derouter.rust_bridge.ocr")
rust_bridge_bindings = importlib.import_module("derouter.rust_bridge.bindings")
rust_bridge_loader = importlib.import_module("derouter.rust_bridge.loader")
...
class RecordingLogging:
    """A spy standing in for ``DeRouterLoggingObj`` to capture ``pre_call``."""
...
def build_prepared_request(
    *,
    ...
    return ocr_main._PreparedOCRRequest(
        ...
        derouter_logging_obj=logging_obj or RecordingLogging(),
    )
...
def test_native_bridge_loader_returns_none_when_extension_absent(monkeypatch):
    ...
    def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name == "derouter.rust_bridge" and "_native" in fromlist:
            raise ImportError
...
def test_native_bridge_available_reflects_loader(monkeypatch):
    fake_module = types.ModuleType("derouter.rust_bridge._native")
...
def test_load_rust_ocr_none_when_extension_absent(monkeypatch):
    ...
    derouter.rust(True)  # no impl injected; extension isn't built in CI
    assert rust_bridge.load_rust_ocr() is None
    assert rust_bridge.load_rust_aocr() is None


def test_load_rust_ocr_uses_compiled_extension(monkeypatch):
    """With no injected impl but a packaged ``derouter.rust_bridge._native`` importable,
    ...
    derouter.rust(True)  # enabled, no impl injected -> import the extension
...
def test_ocr_exception_type_uses_resolved_provider_context(
    ...
    monkeypatch.setattr(ocr_main.derouter, "exception_type", fake_exception_type)
    derouter.rust(True)
    rust_bridge._OCR.override(RaisingBridge())

    with pytest.raises(CapturedException):
        derouter.ocr(model=MODEL, document=DOCUMENT, api_key="sk-test")
...
async def test_aocr_exception_type_uses_resolved_provider_context(
    ...
    with pytest.raises(CapturedException):
        await derouter.aocr(model=MODEL, document=DOCUMENT, api_key="sk-test")
...
def test_ocr_forwards_timeout_to_rust(fake_bridge):
    ...
    derouter.ocr(model=MODEL, document=DOCUMENT, api_key="sk-test", timeout=12.5)
...
def test_ocr_falls_back_to_python_when_bridge_unavailable(monkeypatch):
    ...
    derouter.rust(True)  # enabled, but load_rust_ocr() returns None in CI
    ...
    response = derouter.ocr(model=MODEL, document=DOCUMENT, api_key="sk-test")
...
def test_ocr_provider_configs_expose_api_key_env_vars():
    from derouter.llms.azure_ai.ocr.document_intelligence.transformation import (
    ...
    from derouter.llms.vertex_ai.ocr.deepseek_transformation import (
