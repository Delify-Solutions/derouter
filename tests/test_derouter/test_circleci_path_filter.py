...
DOCS = ["README.md", "docs/my_website/index.mdx", "derouter/anywhere.md"]
CLIENT = ["ui/derouter-dashboard/src/App.tsx"]
BACKEND = ["derouter/main.py"]
CI = [".github/workflows/test-derouter-ui-unit.yml"]
...
def test_markdown_under_ui_counts_as_client_not_docs() -> None:
    assert classify("client", ["ui/derouter-dashboard/README.md"]) == "run"
    assert classify("backend", ["ui/derouter-dashboard/README.md"]) == "skip"
    assert classify("ui", ["ui/derouter-dashboard/README.md"]) == "run"
...
def _pr_repo(tmp_path: Path, feature_files: dict[str, str]) -> Path:
    ...
    (work / "derouter_core.py").write_text("x\n")
    _git(work, "add", ".")
    _git(work, "commit", "-qm", "base")
    _git(work, "push", "-q", "origin", "main")
    _git(work, "checkout", "-q", "-b", "derouter_feature")
...
def test_path_filter_runs_backend_pr(tmp_path: Path) -> None:
    work = _pr_repo(tmp_path, {"derouter/new.py": "y\n"})
    result = _run_path_filter(work, tmp_path, "backend", SCRIPTS_DIR)
    assert result.returncode == 0
