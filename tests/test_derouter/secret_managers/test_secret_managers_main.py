...
def test_oidc_google_success():
    ...
    with patch("derouter.secret_managers.main.oidc_cache", mock_oidc_cache):
        with patch(
            "derouter.secret_managers.main._get_oidc_http_handler",
            mock_get_http_handler,
        ):
            with patch(
                "derouter.secret_managers.main.HTTPHandler",
...
@patch("derouter.secret_managers.main.oidc_cache")
@patch("derouter.secret_managers.main._get_oidc_http_handler")
...
@patch("derouter.secret_managers.main.get_azure_ad_token_provider")
...
def test_oidc_file_success(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("DEROUTER_OIDC_ALLOWED_CREDENTIAL_DIRS", str(tmp_path))
...
def test_oidc_file_rejects_path_outside_allowlist(tmp_path, monkeypatch):
    ...
    allowed_dir.mkdir()
    monkeypatch.setenv("DEROUTER_OIDC_ALLOWED_CREDENTIAL_DIRS", str(allowed_dir))
...
def test_secret_manager_would_be_consulted_matches_get_secret(
    ...
    """
    asked = []
    monkeypatch.setattr(derouter, "secret_manager_client", _SpySecretManager(asked))
