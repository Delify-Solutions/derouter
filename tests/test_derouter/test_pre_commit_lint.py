...
def _sandbox(tmp_path: Path) -> tuple[Path, Path]:
    repo = tmp_path / "repo"
    (repo / "derouter" / "proxy").mkdir(parents=True)
    (repo / "derouter" / "foo.py").write_text("x = 1\n")
    (repo / "derouter" / "proxy" / "spec.py").write_text("y = 2\n")
    dashboard = repo / "ui" / "derouter-dashboard"
...
def _set_base_ref(repo: Path, branch: str = "derouter_internal_staging") -> None:
...
@pytest.mark.parametrize("branch", ["derouter_internal_staging", "main"])
...
def test_nothing_staged_includes_untracked_files_in_scope(tmp_path: Path) -> None:
    ...
    _commit_all(repo, "base")
    _set_base_ref(repo)
    (repo / "derouter" / "brand_new.py").write_text("z = 3\n")
    proc = _run(repo, bin_dir, {})
    assert proc.returncode == 0, proc.stdout + proc.stderr
    assert "derouter/brand_new.py" in proc.stdout
    assert "linting Python" in proc.stdout
...
def test_staged_deletion_triggers_checks_without_feeding_missing_files_to_tools(tmp_path: Path) -> None:
    repo, bin_dir = _sandbox(tmp_path)
    _commit_all(repo, "base")
    subprocess.run(["git", "rm", "-q", "derouter/foo.py"], cwd=repo, check=True)
    proc = _run(repo, bin_dir, {})
    assert proc.returncode == 0, proc.stdout + proc.stderr
...
def test_deleted_dashboard_file_still_triggers_dashboard_lint(tmp_path: Path) -> None:
    ...
    _commit_all(repo, "base")
    _set_base_ref(repo)
    (repo / "ui" / "derouter-dashboard" / "src" / "app.ts").unlink()
...
def test_partial_staging_warns_which_checks_were_skipped(tmp_path: Path) -> None:
    ...
    (repo / "derouter" / "foo.py").write_text("x = 4\n")
    proc = _run(repo, bin_dir, {})
    assert proc.returncode == 0, proc.stdout + proc.stderr
...
def test_staged_files_matching_no_check_print_an_explicit_noop_note_and_nonempty_log(tmp_path: Path) -> None:
    ...
    assert "skipped: Python lint (make lint) (no derouter/ Python files in scope)" in log
...
def test_tests_changed_alongside_derouter_files_defer_to_make_lint(tmp_path: Path) -> None:
    repo, bin_dir = _sandbox(tmp_path)
    _commit_all(repo, "base")
    args_dir = tmp_path / "args"
    args_dir.mkdir()
    _stage_file(repo, "derouter/foo.py", "x = 2\n")
    _stage_file(repo, "tests/test_a.py", "def test_a() -> None: ...\n")
...
def test_run_queues_through_the_machine_wide_gate_slot_lock(tmp_path: Path) -> None:
    ...
    proc = _run(repo, bin_dir, {"DEROUTER_GATE_SLOT_DIR": str(lock_dir)})
    assert proc.returncode == 0, proc.stdout + proc.stderr
...
def test_run_under_a_held_slot_skips_reacquiring_the_gate_lock(tmp_path: Path) -> None:
    ...
    proc = _run(
        repo,
        bin_dir,
        {"DEROUTER_GATE_SLOT_DIR": str(lock_dir), "DEROUTER_GATE_SLOT_HELD": "1"},
    )
    assert proc.returncode == 0, proc.stdout + proc.stderr
...
def test_hook_symlink_install_still_resolves_the_slot_lock_helper(tmp_path: Path) -> None:
    ...
    proc = subprocess.run(
        ...
        capture_output=True,
        text=True,
        env=_env(repo, bin_dir, {"DEROUTER_GATE_SLOT_DIR": str(lock_dir)}),
        timeout=120,
    )
    assert proc.returncode == 0, proc.stdout + proc.stderr
    assert (lock_dir / "slot-0.lock").exists()
