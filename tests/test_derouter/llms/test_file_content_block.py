...
- derouter/llms/vertex_ai/gemini/transformation.py
- derouter/llms/gemini/chat/transformation.py
- derouter/derouter_core_utils/prompt_templates/common_utils.py
  (migrate_file_to_image_url raises on missing `file`; file-id helpers skip non-OpenAI shapes)
- derouter/derouter_core_utils/prompt_templates/factory.py (Bedrock + Anthropic)
- derouter/llms/openai/chat/gpt_transformation.py
"""
...
def test_gemini_convert_messages_malformed_file_raises_bad_request():
    ...
    with pytest.raises(derouter.BadRequestError, match="missing the required 'file' field"):
        _gemini_convert_messages_with_history(
            messages=_malformed(),
...
def test_google_ai_studio_transform_messages_http_file_id_converts_to_base64(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.llms.gemini.chat.transformation.convert_url_to_base64",
...
def test_google_ai_studio_transform_messages_http_file_id_convert_failure_leaves_file_unchanged(
    ...
    def _raise(_url: str) -> str:
        raise derouter.ImageFetchError("simulated fetch failure")
...
def test_bedrock_async_process_file_message_malformed_raises_bad_request():
    ...
    async def _run() -> None:
        with pytest.raises(
            derouter.BadRequestError, match="missing the required 'file' field"
        ):
