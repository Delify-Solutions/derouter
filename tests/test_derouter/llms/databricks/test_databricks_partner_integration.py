...
from derouter.llms.databricks.common_utils import DatabricksBase, DatabricksException
...
class TestBuildUserAgent:
    ...
    def test_default_user_agent(self):
        """No custom user agent returns derouter/{version}."""
        ua = DatabricksBase._build_user_agent(None)
        assert ua.startswith("derouter/")
    ...
    def test_custom_user_agent_with_version(self):
        ...
        assert ua.startswith("mycompany_derouter/")
        # Verify the version is derouter's, not the custom one
        assert "/1.0.0" not in ua or "mycompany_derouter/1.0.0" not in ua
    ...
    def test_custom_user_agent_with_underscore(self):
        ...
        assert ua.startswith("my_company_derouter/")
    ...
    def test_custom_user_agent_with_hyphen(self):
        ...
        assert ua.startswith("my-company_derouter/")

    def test_custom_user_agent_ignores_custom_version(self):
        """Custom version is ignored, derouter version is used."""
        ua = DatabricksBase._build_user_agent("partner/99.99.99")
        parts = ua.split("/")
        assert parts[0] == "partner_derouter"
    ...
    def test_partner_with_numbers(self):
        ...
        assert ua.startswith("company123_derouter/")
...
class TestValidateEnvironmentUserAgent:
    ...
    def test_default_user_agent(self, monkeypatch):
        ...
        assert headers["User-Agent"].startswith("derouter/")
    ...
    def test_custom_user_agent_via_param(self, monkeypatch):
        ...
        assert headers["User-Agent"].startswith("mycompany_derouter/")
...
class TestSDKPartnerTelemetry:
    ...
    def test_sdk_partner_registered(self):
        ...
        with patch.dict(
            ...
            mock_useragent.with_partner.assert_called_once_with("derouter")
...
class TestUserAgentFromEnvironment:
    ...
    def test_user_agent_from_databricks_env_var(self, monkeypatch):
        ...
        assert headers["User-Agent"].startswith("envpartner_derouter/")
    ...
    def test_custom_param_takes_precedence(self, monkeypatch):
        ...
        assert headers["User-Agent"].startswith("parampartner_derouter/")


class TestDeRouterCompletionUserAgent:
    """Test User-Agent is correctly passed through DeRouter completion calls."""

    def test_completion_passes_user_agent_to_headers(self):
        """derouter.completion() correctly passes user_agent to request headers."""
        ...
        with patch.object(
            ...
            return_value=(
                "https://test.net/serving-endpoints/chat/completions",
                {
                    ...
                    "User-Agent": "testpartner_derouter/1.0.0",
                },
            ),
        ) as mock_validate:
...
class TestDeRouterEmbeddingUserAgent:
    """Test User-Agent is correctly passed through DeRouter embedding calls."""

    def test_embedding_passes_user_agent_to_headers(self):
        """derouter.embedding() correctly passes user_agent to request headers."""
        from derouter.llms.databricks.embed.handler import DatabricksEmbeddingHandler
        ...
        with patch.object(
            ...
            return_value=(
                ...
                {
                    "Authorization": "Bearer test",
                    "User-Agent": "embedpartner_derouter/1.0.0",
                },
            ),
        ) as mock_validate:
            with patch(
                "derouter.llms.openai_like.embedding.handler.OpenAILikeEmbeddingHandler.embedding"
            ):
                try:
                    handler.embedding(
                        model="databricks/test-model",
