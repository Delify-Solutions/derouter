"""
End-to-End Tests for Databricks DeRouter Integration
====================================================
...
DeRouter Integration Tests:
    This test file includes tests for different ways of calling Databricks via DeRouter:
    
    1. DeRouter SDK Direct - Using derouter.completion() with user_agent parameter
    2. LangChain + DeRouter - Using ChatDeRouter wrapper (requires langchain-community)
    3. DeRouter Async - Using derouter.acompletion() async API
    4. DeRouter Streaming - Using derouter.completion() with stream=True
    5. DeRouter Embedding - Using derouter.embedding() with user_agent parameter
    
    All tests use the CUSTOM_USER_AGENT value from the config file and call
    Databricks endpoints through DeRouter's unified interface.
...
Optional Dependencies (for DeRouter integration tests):
    - pip install langchain-derouter  # For LangChain tests (recommended)

Setup:
    1. Copy the template to create your config file:
       cp databricks_config.template.txt ~/.databricks_derouter_config.txt
...
Run with:
    cd /path/to/derouter
    python tests/test_derouter/llms/databricks/test_databricks_e2e.py
...
# Add the derouter package to path
sys.path.insert(
    0, os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../.."))
)

# Config file path - can be overridden with DATABRICKS_TEST_CONFIG env var
DEFAULT_CONFIG_PATH = os.path.expanduser("~/.databricks_derouter_config.txt")
...
def setup_environment(config: dict, auth_method: str):
    ...
    if auth_method == "oauth":
        ...
        # But we still need to pass api_base to derouter, so set it if provided
        if "DATABRICKS_API_BASE" in config:
            os.environ["DATABRICKS_API_BASE"] = config["DATABRICKS_API_BASE"]
...
def test_user_agent_building():
    ...
    print(f"  Default: {ua}")
    assert ua.startswith("derouter/"), f"Expected derouter/, got {ua}"
    print("  ✓ Default user agent works")

    # Test 2: With partner
    ua = DatabricksBase._build_user_agent("mycompany/1.0.0")
    print(f"  With partner: {ua}")
    assert ua.startswith("mycompany_derouter/"), f"Expected mycompany_derouter/, got {ua}"
    ...
    assert ua.startswith("acme_derouter/"), f"Expected acme_derouter/, got {ua}"
    ...
    try:
        response = derouter.completion(
            ...
            messages=[
                {
                    "role": "user",
                    "content": "Say 'Hello, DeRouter test!' in exactly those words.",
                }
            ],
            max_tokens=50,
            temperature=0.1,
...
def test_chat_completion_default_user_agent(config: dict):
    ...
    saved_derouter_ua = os.environ.pop("DEROUTER_USER_AGENT", None)
    ...
    print(f"  Model: {full_model}")
    print(f"  Expected User-Agent: derouter/{version}")
    ...
    try:
        ...
        print(
            f"  Note: Check Databricks Query History to verify User-Agent is 'derouter/{version}'"
        )
        return True
        ...
        if saved_derouter_ua:
            os.environ["DEROUTER_USER_AGENT"] = saved_derouter_ua
...
def test_chat_completion_with_custom_user_agent(config: dict):
    ...
    print(f"  Expected User-Agent: testpartner_derouter/{version}")
    ...
    try:
        response = derouter.completion(
            ...
            max_tokens=10,
            user_agent="testpartner/2.0.0",  # This should result in testpartner_derouter/{version}
        )

        print(f"  Response: {response.choices[0].message.content}")
        print("  ✓ Custom user-agent test passed!")
        print(
            f"  Note: Check Databricks Query History to verify User-Agent is 'testpartner_derouter/{version}'"
        )
        return True
...
def test_chat_completion_with_env_user_agent(config: dict):
    ...
    print(f"  Expected User-Agent: {test_partner}_derouter/{version}")
    ...
    try:
        ...
        print(
            f"  Note: Check Databricks Query History to verify User-Agent is '{test_partner}_derouter/{version}'"
        )
        return True
...
# ==============================================================================
# SDK INTEGRATION TESTS - Different ways of calling Databricks via DeRouter
# ==============================================================================


def test_derouter_sdk_with_config_user_agent(config: dict):
    """
    Test 1: DeRouter SDK with custom user agent from config file.

    This test uses the DeRouter SDK directly with the CUSTOM_USER_AGENT
    specified in the databricks config file.
    """
    print("\n" + "=" * 60)
    print("TEST: DeRouter SDK with Config User-Agent")
    print("=" * 60)
    ...
    try:
        response = derouter.completion(
            model=full_model,
            messages=[{"role": "user", "content": "Say 'DeRouter SDK test' only."}],
            max_tokens=20,
            temperature=0.1,
        ...
        print("  ✓ DeRouter SDK with config user-agent test passed!")
        return True

    except Exception as e:
        print(f"  ✗ DeRouter SDK test failed: {e}")
        return False


def test_langchain_derouter_with_user_agent(config: dict):
    """
    Test 2: LangChain with DeRouter integration.

    This test uses LangChain's ChatDeRouter wrapper to call Databricks
    with custom user agent from config.

    Requires: pip install langchain-derouter (recommended)
              or: pip install langchain langchain-community (deprecated)
    """
    print("\n" + "=" * 60)
    print("TEST: LangChain + DeRouter with Config User-Agent")
    print("=" * 60)
    ...
    # Try the new langchain-derouter package first, fall back to deprecated import
    ChatDeRouter = None
    HumanMessage = None

    try:
        from langchain_derouter import ChatDeRouter
        from langchain_core.messages import HumanMessage

        print("  Using: langchain-derouter package (recommended)")
    except ImportError:
        try:
            ...
            with warnings.catch_warnings():
                ...
                from langchain_community.chat_models import ChatDeRouter
                from langchain_core.messages import HumanMessage
            print(
                "  Using: langchain-community (deprecated, consider: pip install langchain-derouter)"
            )
        except ImportError:
            print("  Skipped: langchain-derouter not installed")
            print("  Install with: pip install langchain-derouter")
            return None
    ...
    try:
        ...
        print(f"  Response: {content}")
        print("  ✓ LangChain + DeRouter with config user-agent test passed!")
        return True

    except Exception as e:
        print(f"  ✗ LangChain + DeRouter test failed: {e}")
        import traceback
...
def test_derouter_async_completion(config: dict):
    """
    Test 3: DeRouter Async Completion API with custom User-Agent.

    This test uses DeRouter's async completion API (acompletion) to call
    Databricks with custom user agent from config.
    """
    print("\n" + "=" * 60)
    print("TEST: DeRouter Async Completion with Config User-Agent")
    print("=" * 60)
    ...
    async def run_async_completion():
        response = await derouter.acompletion(
            model=full_model,
            messages=[{"role": "user", "content": "Say 'DeRouter async test' only."}],
            max_tokens=20,
            temperature=0.1,
    ...
    try:
        ...
        content = response.choices[0].message.content
        print(f"  Response: {content}")
        print("  ✓ DeRouter async completion with config user-agent test passed!")
        return True

    except Exception as e:
        print(f"  ✗ DeRouter async completion test failed: {e}")
        import traceback
...
def test_derouter_streaming_completion(config: dict):
    """
    Test 4: DeRouter Streaming Completion with custom User-Agent.

    This test uses DeRouter's streaming completion API to call
    Databricks with custom user agent from config.
    """
    print("\n" + "=" * 60)
    print("TEST: DeRouter Streaming Completion with Config User-Agent")
    print("=" * 60)
    ...
    try:
        ...
        response = derouter.completion(
            model=full_model,
            messages=[
                {"role": "user", "content": "Say 'DeRouter streaming test' only."}
            ],
            max_tokens=20,
            temperature=0.1,
            user_agent=custom_ua,
        ...
        print("  ✓ DeRouter streaming completion with config user-agent test passed!")
        return True

    except Exception as e:
        print(f"  ✗ DeRouter streaming completion test failed: {e}")
        import traceback
...
def test_derouter_embedding_with_user_agent(config: dict):
    """
    Test 5: DeRouter Embedding API with custom User-Agent.

    This test uses DeRouter's embedding API to call Databricks
    with custom user agent from config.
    """
    print("\n" + "=" * 60)
    print("TEST: DeRouter Embedding with Config User-Agent")
    print("=" * 60)
    ...
    try:
        response = derouter.embedding(
            model=full_model,
            input=["Hello, this is a DeRouter embedding test with custom user agent!"],
            user_agent=custom_ua,
        )
        ...
        if data:
            ...
            print("  ✓ DeRouter embedding with config user-agent test passed!")
            return True
        else:
            print("  ✗ DeRouter embedding test failed: No data in response")
            return False

    except Exception as e:
        print(f"  ✗ DeRouter embedding test failed: {e}")
...
def main():
    print("=" * 60)
    print("DATABRICKS DEROUTER INTEGRATION TESTS")
    print("=" * 60)
    ...
    # Test 1: DeRouter SDK with config user agent
    all_results.append(
        (
            "DeRouter SDK with Config User-Agent",
            test_derouter_sdk_with_config_user_agent(config),
        )
    )

    # Test 2: LangChain + DeRouter with config user agent
    all_results.append(
        (
            "LangChain + DeRouter with Config User-Agent",
            test_langchain_derouter_with_user_agent(config),
        )
    )

    # Test 3: DeRouter Async Completion with config user agent
    all_results.append(
        (
            "DeRouter Async Completion with Config User-Agent",
            test_derouter_async_completion(config),
        )
    )

    # Test 4: DeRouter Streaming Completion with config user agent
    all_results.append(
        (
            "DeRouter Streaming Completion with Config User-Agent",
            test_derouter_streaming_completion(config),
        )
    )

    # Test 5: DeRouter Embedding with config user agent
    all_results.append(
        (
            "DeRouter Embedding with Config User-Agent",
            test_derouter_embedding_with_user_agent(config),
        )
    )
