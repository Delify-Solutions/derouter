...
def _search(config, client=None, query="what is derouter", optional_params=None, derouter_params=None, executor=None):
    return config.execute_search_vector_store_request(
        ...
        derouter_params={"derouter_embedding_model": "openai/text-embedding-3-small", **(derouter_params or {})},
...
def test_sync_search_uses_request_embedding_executor_without_overwriting_explicit_config():
    ...
    _search(
        ...
        derouter_params={
            ...
            "derouter_embedding_config": embedding_config,
        },
        executor=executor,
    )

    assert executor.captured == ("team-embedding-alias", "what is derouter", embedding_config)
...
def test_sync_search_honors_custom_field_names():
    ...
    response = _search(
        config,
        derouter_params={"valkey_embedding_field": "emb", "valkey_text_field": "chunk"},
    )
...
def test_sync_search_maps_response_with_inverted_score_sorted_best_first():
    ...
    assert response["search_query"] == "what is derouter"
...
def test_sync_search_expands_embedding_config_into_kwargs():
    ...
    _search(
        config,
        derouter_params={"derouter_embedding_config": {"api_key": "sk-test", "api_base": "https://embed.example.com"}},
    )
...
async def test_async_search_builds_knn_query_and_maps_response():
    ...
    response = await config.aexecute_search_vector_store_request(
        ...
        derouter_logging_obj=MagicMock(),
        derouter_params={
            "derouter_embedding_model": "openai/text-embedding-3-small",
            "derouter_embedding_config": {"api_key": "sk-async"},
        },
    )
