"""
Unit tests for derouter/llms/oci/chat/generic.py — error paths and stream handling.
"""
...
def _register_oci_gpt5_in_catalog():
    ...
    ``derouter.model_cost`` was populated from the bundled
    ...
    added = []
    for key in needed:
        if key not in derouter.model_cost:
            derouter.model_cost[key] = {
                "derouter_provider": "oci",
                "mode": "chat",
...
class TestGpt5MaxCompletionTokens:
    ...
    def test_helper_covers_openai_models_absent_from_catalog(self):
        ...
        faster than the derouter catalog tracks them. The vendor-prefix rule
        ...
        for name in (
            ...
            assert f"oci/{name.removeprefix('oci/')}" not in derouter.model_cost
    ...
    def test_payload_serializes_max_completion_tokens(self):
        from derouter.types.llms.oci import OCIChatRequestPayload
