...
from derouter.llms.oci.chat.transformation import (
    OCIChatConfig,
    OCIRequestWrapper,
    version,
)
...
class TestOCIChatConfig:
    def test_validate_environment_with_oci_region(self, supplied_params):
        ...
        assert result["user-agent"] == f"derouter/{version}"
    ...
    def test_transform_request_json_schema_without_body_raises_generic(self):
        ...
        from derouter.llms.oci.common_utils import OCIError
    ...
    def test_transform_response_simple_text(self):
        ...
        assert isinstance(result.choices[0], derouter.Choices)
        ...
        assert isinstance(result.usage, derouter.Usage)  # type: ignore
    ...
    def test_transform_response_with_tool_calls(self):
        ...
        model_response = ModelResponse(
            choices=[derouter.Choices(index=0, message=derouter.Message())]
        )
        ...
        assert isinstance(tool_call, derouter.utils.ChatCompletionMessageToolCall)
        ...
        assert isinstance(usage, derouter.Usage)  # type: ignore
...
class TestOCIReasoningEffort:
    ...
    def _build_chat_request(self, model: str, optional_params: dict) -> dict:
        ...
        from derouter.llms.oci.chat.transformation import OCIChatConfig
    ...
    def test_reasoning_tokens_extracted_from_usage(self):
        ...
        Usage.completion_tokens_details.reasoning_tokens."""
        from derouter.llms.oci.chat.generic import handle_generic_response
...
class TestOCIChatConfigErrorPaths:
    ...
    def test_map_openai_params_max_retries_dropped_without_drop_params(self, model):
        """max_retries is a derouter control param, not a generation param. It
        must be dropped silently (no raise) even when drop_params is False, so
        the derouter proxy (which injects max_retries on every request) does not
        500 every OCI call.
        """
        config = OCIChatConfig()
        result = config.map_openai_params(
...
import pytest
from unittest.mock import MagicMock
from derouter.llms.oci.common_utils import OCIError, sign_with_manual_credentials
...
class TestOCIImageUrlTransformation:
    ...
    Fixes: https://github.com/Delify-Solutions/derouter/issues/18270
    Fixes: https://github.com/Delify-Solutions/derouter/issues/19589
    """
