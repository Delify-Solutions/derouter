...
from derouter.llms.groq.chat.transformation import GroqChatConfig
...
def _groq_completion_with_mocked_response(response_json: dict) -> derouter.ModelResponse:
    client = HTTPHandler()
...
class TestGroqWebSearchCost:
    ...
    def test_browser_search_priced_per_search(self, model: str, search_context_size: str):
        cost = StandardBuiltInToolCostTracking.get_cost_for_web_search(
            ...
            model_info=derouter.get_model_info(model=model, custom_llm_provider="groq"),
        )
        assert cost == 0.005
