...
import derouter
from derouter.llms.fireworks_ai.responses.transformation import FireworksAIResponsesAPIConfig
from derouter.responses.file_search.emulated_handler import should_use_emulated_file_search
from derouter.types.llms.openai import InputTokensDetails, ResponseAPIUsage, ResponseInputParam, ResponsesAPIResponse
...
HTTPX_CLIENT_FACTORY: Final = "derouter.llms.custom_httpx.llm_http_handler._get_httpx_client"
...
class _GeneratedSessionMetadata(TypedDict):
    derouter_session_id_generated: ReadOnly[bool]
...
def test_responses_call_hits_native_endpoint_with_mcp_tool_untouched() -> None:
    ...
    with patch(HTTPX_CLIENT_FACTORY, return_value=client):
        response: Final = derouter.responses(
            model="fireworks_ai/accounts/fireworks/models/kimi-k3",
            input="What is derouter?",
...
def test_responses_call_expands_bare_model_name_to_fireworks_resource() -> None:
    ...
    with patch(HTTPX_CLIENT_FACTORY, return_value=client):
        derouter.responses(model="fireworks_ai/glm-5p3", input="hi", api_key="fw-test-key")
...
def test_file_search_tools_take_derouter_emulated_search_not_fireworks() -> None:
...
def test_responses_call_sends_session_affinity_for_caller_session_id() -> None:
    client: Final = _mock_http_client(_fireworks_response("accounts/fireworks/models/kimi-k3"))
    with patch(HTTPX_CLIENT_FACTORY, return_value=client):
        derouter.responses(model="fireworks_ai/kimi-k3", input="hi", api_key="fw-test-key", derouter_session_id="sess-42")
...
def test_responses_call_maps_provider_errors_to_fireworks_ai() -> None:
    ...
    with patch(HTTPX_CLIENT_FACTORY, return_value=client), pytest.raises(derouter.AuthenticationError) as raised:
        derouter.responses(model="fireworks_ai/kimi-k3", input="hi", api_key="fw-bad-key")
...
def test_responses_call_skips_session_affinity_for_proxy_generated_session_id() -> None:
    client: Final = _mock_http_client(_fireworks_response("accounts/fireworks/models/kimi-k3"))
    generated: Final[_GeneratedSessionMetadata] = {"derouter_session_id_generated": True}
    with patch(HTTPX_CLIENT_FACTORY, return_value=client):
        derouter.responses(
            model="fireworks_ai/kimi-k3",
            input="hi",
...
def test_get_complete_url(api_base: str | None, expected: str) -> None:
    assert FireworksAIResponsesAPIConfig().get_complete_url(api_base=api_base, derouter_params=NO_PARAMS) == expected
...
def test_responses_call_reads_fireworks_api_base_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    client: Final = _mock_http_client(_fireworks_response("accounts/fireworks/models/kimi-k3"))
    with patch(HTTPX_CLIENT_FACTORY, return_value=client):
        derouter.responses(model="fireworks_ai/kimi-k3", input="hi", api_key="fw-test-key")
...
def test_validate_environment_reads_every_fireworks_key_name(monkeypatch: pytest.MonkeyPatch, env_name: str) -> None:
    ...
    headers: Final = FireworksAIResponsesAPIConfig().validate_environment(
        headers=NO_HEADERS, model="accounts/fireworks/models/kimi-k3", derouter_params=GenericDeRouterParams()
    )
...
def test_validate_environment_prefers_explicit_api_key(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    headers: Final = FireworksAIResponsesAPIConfig().validate_environment(
        headers=NO_HEADERS,
        model="accounts/fireworks/models/kimi-k3",
        derouter_params=GenericDeRouterParams(api_key="explicit"),
    )
...
def test_validate_environment_without_any_key_raises() -> None:
    with pytest.raises(ValueError, match="FIREWORKS_API_KEY"):
        FireworksAIResponsesAPIConfig().validate_environment(
            headers=NO_HEADERS, model="accounts/fireworks/models/kimi-k3", derouter_params=None
        )
...
def test_delete_responses_maps_fireworks_message_only_body_to_deleted_result() -> None:
    ...
    with patch(HTTPX_CLIENT_FACTORY, return_value=client):
        result: Final = derouter.delete_responses(
