...
import derouter
from derouter import supports_reasoning, supports_vision
...
def test_validate_environment_sets_session_affinity_from_derouter_session_id():
    ...
    headers = config.validate_environment(
        ...
        messages=[],
        optional_params={},
        derouter_params={"derouter_session_id": "session-123"},
        api_key="test-key",
    )
...
def test_validate_environment_sets_session_affinity_from_metadata_session_id():
    ...
    headers = config.validate_environment(
        ...
        messages=[],
        optional_params={},
        derouter_params={"metadata": {"session_id": "metadata-session-123"}},
        api_key="test-key",
    )
...
def test_validate_environment_sets_session_affinity_from_session_id():
    ...
    headers = config.validate_environment(
        ...
        messages=[],
        optional_params={},
        derouter_params={"session_id": "session-id-123"},
        api_key="test-key",
    )
...
def test_validate_environment_ignores_trace_id_for_session_affinity():
    ...
    derouter_trace_id defaults to a fresh uuid4 per request, so pinning
    ...
    headers = config.validate_environment(
        ...
        messages=[],
        optional_params={},
        derouter_params={"derouter_trace_id": "trace-id-123"},
        api_key="test-key",
    )
...
def test_validate_environment_prefers_session_id_over_trace_id():
    ...
    headers = config.validate_environment(
        ...
        optional_params={},
        derouter_params={
            "derouter_session_id": "session-123",
            "derouter_trace_id": "trace-id-123",
        },
        api_key="test-key",
    )
...
def test_get_fireworks_session_id_prefers_derouter_session_id_over_trace_id():
    assert (
        get_fireworks_session_id(
            {"derouter_session_id": "session-123", "derouter_trace_id": "trace-123"}
        )
        == "session-123"
    )
...
def test_get_fireworks_session_id_ignores_proxy_generated_session_id():
    ...
    assert (
        get_fireworks_session_id(
            {
                "derouter_session_id": "generated-1",
                "derouter_trace_id": "generated-1",
...
def test_get_provider_info_omits_false_supports_reasoning(monkeypatch):
    ...
    monkeypatch.setitem(derouter.model_cost, model, {"supports_reasoning": False})
...
def test_get_models_url_no_double_v1(api_base, expected_url_prefix):
    ...
    with (
        ...
        patch(
            "derouter.llms.fireworks_ai.chat.transformation.get_secret_str",
...
def test_transform_messages_helper_removes_provider_specific_fields():
    ...
    out = config._transform_messages_helper(
        messages, model="fireworks/test", derouter_params={}
    )
    for msg in out:
...
def test_transform_messages_helper_strips_thinking_blocks():
    ...
    out = config._transform_messages_helper(
        messages, model="accounts/fireworks/models/glm-5p1", derouter_params={}
    )
...
def test_transform_messages_helper_rejects_file_blocks():
    ...
    with pytest.raises(
        ...
        config._transform_messages_helper(
            messages, model="accounts/fireworks/models/kimi-k2p6", derouter_params={}
        )
...
def test_transform_messages_helper_rejects_non_vision_image_inputs():
    ...
    with pytest.raises(derouter.BadRequestError, match="does not support image inputs"):
        config._transform_messages_helper(
            messages, model="accounts/fireworks/models/glm-5p2", derouter_params={}
        )
...
def test_transform_messages_helper_allows_vision_image_inputs():
    ...
    out = config._transform_messages_helper(
        messages, model="accounts/fireworks/models/minimax-m3", derouter_params={}
    )
    assert out == messages
...
def test_image_inputs_not_rejected_for_fuzzy_non_vision_match():
    ...
    out = config._transform_messages_helper(
        messages, model=custom_model, derouter_params={}
    )
    assert out == messages
...
def test_get_provider_info_vision_from_model_cost(monkeypatch):
    ...
    monkeypatch.setitem(derouter.model_cost, no_vision_model, {})
...
def test_streaming_surfaces_fireworks_response_fields():
    ...
    streamed chunks' provider_specific_fields (derouter rebuilds each streamed
    ...
    derouter.completion(stream=True) path also covers the get_model_response_iterator
...
def test_streaming_preserves_selected_model_for_private_accounting():
    ...
    assembled = derouter.stream_chunk_builder(chunks=chunks)
    assert assembled is not None
    assert assembled.model == requested_route
    assert assembled._hidden_params["provider_response_model"] == selected_model
    selected_model_info = derouter.model_cost[f"fireworks_ai/{selected_model}"]
    expected_cost = (
