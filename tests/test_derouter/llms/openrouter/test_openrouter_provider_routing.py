...
configures such a model in DeRouter they use the double-prefixed form
...
See: https://github.com/Delify-Solutions/derouter/issues/16353
"""
...
class TestOpenRouterNativeModelRouting:
    ...
    def test_double_prefixed_strips_once(self, input_model, expected_model):
        ...
        result_model, provider, _, _ = derouter.get_llm_provider(model=input_model)
    ...
    def test_bridge_double_call_preserves_native_model(self, input_model):
        ...
        model_first, provider, _, _ = derouter.get_llm_provider(model=input_model)
        assert provider == "openrouter"
        ...
        model_second, provider2, _, _ = derouter.get_llm_provider(
            model=model_first,
    ...
    def test_wildcard_deployment_strips_routing_prefix(self):
        """openrouter/* proxy deployments pass custom_llm_provider; strip DeRouter prefix."""
        result_model, provider, _, _ = derouter.get_llm_provider(
