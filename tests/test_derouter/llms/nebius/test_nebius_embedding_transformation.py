...
def mock_embedding_response(*args, **kwargs):
    """Mock response mimicking derouter.embedding output."""

    class MockResponse:
        def __init__(self):
            self.data = [{"embedding": [0.1, 0.2, 0.3]}]  # Example embedding vector
            self.usage = derouter.Usage()  # Mock Usage object
...
def test_nebius_embeddings():
    ...
    with patch("derouter.embedding", side_effect=mock_embedding_response) as mock_embed:
