...
from derouter.llms.hosted_vllm.videos.transformation import (
...
def test_get_complete_url_appends_videos():
    ...
    assert (
        config.get_complete_url(model="MiniMax-H3", api_base="http://localhost:8091", derouter_params={})
        == "http://localhost:8091/v1/videos"
    )
    assert (
        config.get_complete_url(model="MiniMax-H3", api_base="http://localhost:8091/v1", derouter_params={})
        == "http://localhost:8091/v1/videos"
    )
    assert (
        config.get_complete_url(model="MiniMax-H3", api_base="http://localhost:8091/v1/", derouter_params={})
...
def test_get_complete_url_requires_api_base():
    ...
    with pytest.raises(ValueError, match="api_base not set"):
        config.get_complete_url(model="MiniMax-H3", api_base=None, derouter_params={})
...
def test_validate_environment_uses_provided_api_key():
    ...
    headers = config.validate_environment(
        headers={"X-Test": "1"},
        model="MiniMax-H3",
        derouter_params=GenericDeRouterParams(api_key="my-custom-key"),
    )
...
def test_video_generation_posts_multipart_not_json():
    ...
    response = derouter.video_generation(
        model="hosted_vllm/MiniMax-H3",
        prompt="three cats march into a bedroom playing tiny brass instruments",
