...
def test_crusoe_model_list_populated(monkeypatch):
    ...
    original_env = os.environ.get("DEROUTER_LOCAL_MODEL_COST_MAP")
    try:
        ...
        for model in expected:
            assert model in derouter.model_cost, f"{model} not found in model_cost"
            assert derouter.model_cost[model].get("derouter_provider") == "crusoe"
    finally:
        derouter.model_cost = original_model_cost
        if original_env is None:
            os.environ.pop("DEROUTER_LOCAL_MODEL_COST_MAP", None)
        else:
            monkeypatch.setenv("DEROUTER_LOCAL_MODEL_COST_MAP", original_env)
