...
def _search_kwargs(**overrides):
    kwargs = {
        ...
        "derouter_logging_obj": _logging_obj(),
        "derouter_params": {},
        "extra_body": None,
    }
...
class TestS3VectorsVectorStoreConfig:
    ...
    def test_get_complete_url(self):
        ...
        url = config.get_complete_url(None, derouter_params)
    ...
    @pytest.mark.parametrize(
        ("derouter_params", "expected_model"),
        [
            ({}, "text-embedding-3-small"),
            ({"embedding_model": ""}, "text-embedding-3-small"),
            ({"embedding_model": "my-embedding-model"}, "my-embedding-model"),
            ({"derouter_embedding_model": "shared-key-model"}, "shared-key-model"),
            (
                {"derouter_embedding_model": "shared-key-model", "embedding_model": "legacy-alias"},
                "shared-key-model",
            ),
        ],
    )
    def test_query_embedding_model_accepts_embedding_model_alias(self, derouter_params, expected_model):
        assert S3VectorsVectorStoreConfig.query_embedding_model(derouter_params) == expected_model
        ...
        _, request_body = await config.atransform_search_vector_store_request(
            **_search_kwargs(
                ...
                derouter_params={
                    ...
                    "derouter_embedding_config": {"api_key": "store-key"},
                },
                embedding_executor=executor,
            )
        )
    ...
    async def test_atransform_search_router_executor_carries_request_metadata(self):
        ...
        _, request_body = await config.atransform_search_vector_store_request(
            **_search_kwargs(
                derouter_params={"embedding_model": "team-embeddings"},
    ...
    async def test_atransform_search_default_model_falls_back_to_the_sdk(self):
        ...
        router.get_model_list.return_value = [
            {"model_name": "team-embeddings", "derouter_params": {"model": "openai/text-embedding-3-small"}}
        ]
        router.resolved_derouter_models.return_value = []
        ...
        with patch("derouter.aembedding", new=mock_bare):  # test-quality-ok: stubs the bare-embedding fallback whose call the test asserts on
            _, request_body = await config.atransform_search_vector_store_request(
                **_search_kwargs(
                    embedding_executor=RouterVectorStoreEmbeddingExecutor(router=router, metadata=request_metadata)
                )
    ...
    async def test_atransform_search_without_executor_uses_bare_embedding(self):
        ...
        with patch("derouter.aembedding", new=mock_bare):  # test-quality-ok: stubs the bare-embedding fallback whose request body the test asserts on
    ...
    def test_transform_search_without_executor_uses_bare_embedding_sync(self):
        ...
        with patch("derouter.embedding", new=mock_bare):  # test-quality-ok: stubs the bare-embedding fallback whose request body the test asserts on
            _, request_body = config.transform_search_vector_store_request(
                **_search_kwargs(derouter_params={"embedding_model": "my-embedding-model"})
            )
    ...
    def test_transform_search_request_bucket_from_derouter_params(self):
        config = S3VectorsVectorStoreConfig()

        _, request_body = config.transform_search_vector_store_request(
            **_search_kwargs(
                vector_store_id="only-index",
                derouter_params={"vector_bucket_name": "params-bucket"},
