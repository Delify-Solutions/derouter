...
from derouter.llms.predibase.chat.handler import PredibaseChatCompletion
from derouter.llms.predibase.chat.transformation import PredibaseConfig
from derouter.llms.predibase.common_utils import PredibaseError
from derouter.utils import Choices, Message, ModelResponse
...
def test_predibase_transform_request_custom_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.llms.predibase.chat.transformation.custom_prompt",
...
def test_predibase_get_complete_url_stream_and_non_stream():
    config = PredibaseConfig()
    derouter_params = {"predibase_tenant_id": "tenant-123"}
...
def test_predibase_get_complete_url_with_tenant_id_key():
    ...
    url = config.get_complete_url(
        ...
        model="predibase-model",
        optional_params={},
        derouter_params={"tenant_id": "tenant-xyz"},
    )
...
def test_predibase_transform_response_success_best_of(monkeypatch):
    ...
    monkeypatch.setattr("derouter.token_counter", lambda messages: 5)
...
def test_predibase_transform_response_best_of_with_empty_generated_text(monkeypatch):
    ...
    monkeypatch.setattr("derouter.token_counter", lambda messages: 1)
...
def test_predibase_transform_response_empty_output_sets_completion_tokens_zero(monkeypatch):
    ...
    logging_obj = Mock()
    encoding = Mock()
    monkeypatch.setattr("derouter.token_counter", lambda messages: 3)
...
def test_predibase_transform_response_usage_fallbacks(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.token_counter", lambda messages: (_ for _ in ()).throw(RuntimeError())
    )
...
async def test_predibase_async_completion_uses_default_config_when_none(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.llms.predibase.chat.handler.get_async_httpx_client",
        lambda **kwargs: async_handler,
    )

    default_config = Mock()
    default_config.transform_response.return_value = _build_model_response()
    monkeypatch.setattr("derouter.PredibaseConfig", lambda: default_config)
...
def test_predibase_completion_sync_returns_transform_response(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.module_level_client.post",
