...
class TestMetaProviderConfig:
    def test_meta_in_provider_list(self):
        ...
        assert "meta" in derouter.provider_list
...
class TestMetaReasoningParams:
    ...
    def test_reasoning_effort_mapped_through(self):
        cfg = derouter.ProviderConfigManager.get_provider_chat_config(
            model="muse-spark-1.1", provider=derouter.LlmProviders.META
        )
...
class TestMetaAnthropicMessages:
    def test_meta_resolves_native_messages_config(self):
        ...
        cfg = derouter.ProviderConfigManager.get_provider_anthropic_messages_config(
            model="muse-spark-1.1", provider=derouter.LlmProviders.META
        )
    ...
    def test_json_provider_without_messages_endpoint_resolves_none(self):
        cfg = derouter.ProviderConfigManager.get_provider_anthropic_messages_config(
            model="some-model", provider=derouter.LlmProviders.PINSTRIPES
        )
        assert cfg is None
