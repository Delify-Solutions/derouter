...
from derouter.llms.openai_like import dynamic_config
from derouter.llms.openai_like.dynamic_config import create_responses_config_class
from derouter.llms.openai_like.json_loader import SimpleProviderConfig
...
class TestValidateEnvironment:
    def test_explicit_api_key_becomes_a_bearer_header(self):
        ...
        headers = config.validate_environment(
            headers={}, model="m", derouter_params=GenericDeRouterParams(api_key="sk-explicit")
        )
    ...
    def test_api_key_falls_back_to_the_configured_env_var(self, monkeypatch):
        ...
        headers = config.validate_environment(headers={}, model="m", derouter_params=None)
    ...
    def test_explicit_key_wins_over_the_env_var(self, monkeypatch):
        ...
        headers = config.validate_environment(
            headers={}, model="m", derouter_params=GenericDeRouterParams(api_key="sk-wins")
        )
    ...
    def test_no_key_anywhere_leaves_the_header_unset(self, monkeypatch):
        ...
        assert config.validate_environment(headers={}, model="m", derouter_params=None) == {}
    ...
    def test_existing_headers_are_preserved(self):
        ...
        headers = config.validate_environment(
            headers={"X-Trace": "abc"},
            model="m",
            derouter_params=GenericDeRouterParams(api_key="sk-1"),
        )
...
class TestGetCompleteUrl:
    def test_explicit_api_base_gets_the_responses_suffix(self):
        ...
        assert config.get_complete_url(api_base="https://host/v1", derouter_params={}) == "https://host/v1/responses"
    ...
    def test_falls_back_to_the_api_base_env_var(self, monkeypatch):
        ...
        assert config.get_complete_url(api_base=None, derouter_params={}) == "https://from-env/v1/responses"
    ...
    def test_falls_back_to_the_configured_base_url_last(self, monkeypatch):
        ...
        assert config.get_complete_url(api_base=None, derouter_params={}) == "https://api.example.com/v1/responses"
    ...
    def test_explicit_api_base_wins_over_the_env_var(self, monkeypatch):
        ...
        assert (
            config.get_complete_url(api_base="https://explicit/v1", derouter_params={})
