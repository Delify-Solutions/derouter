...
class TestCognitionProviderIdentity:
    def test_cognition_is_a_registered_provider(self):
        ...
        assert "cognition" in derouter.provider_list
    ...
    def test_cognition_in_openai_compatible_providers(self):
        from derouter.constants import openai_compatible_providers
...
class TestCognitionCostTracking:
    ...
    def test_lightning_is_five_times_the_standard_tier(self):
        standard = derouter.get_model_info(model="cognition/swe-1.7")
        lightning = derouter.get_model_info(model="cognition/swe-1.7-lightning")
    ...
    def test_supported_endpoints_matrix(self):
        matrix = json.loads((Path(derouter.__file__).parent / "provider_endpoints_support_backup.json").read_text())
    ...
    async def test_router_spend_is_attributed_to_cognition_pricing(self):
        ...
        router = Router(
            model_list=[
                {
                    "model_name": "swe",
                    "derouter_params": {"model": "cognition/swe-1.7", "api_key": "sk-test"},
                }
            ]
        )
            ...
            model_list=[
                {
                    ...
                    "derouter_params": {"model": "cognition/swe-1.7-lightning", "api_key": "sk-test"},
                }
            ]
        )
