...
class TestJSONProviderRegistryResponsesAPI:
    ...
    def test_provider_with_responses_endpoint(self):
        ...
        from derouter.llms.openai_like.json_loader import (
...
class TestCreateResponsesConfigClass:
    ...
    def test_generated_class_get_complete_url_with_override(self):
        ...
        url = config.get_complete_url(
            api_base="https://custom.api.com/v1", derouter_params={}
        )
    ...
    def test_generated_class_validate_environment(self):
        ...
        with patch(
            "derouter.llms.openai_like.dynamic_config.get_secret_str",
            return_value="sk-test-key-123",
        ):
            headers = config.validate_environment(
                headers={}, model="test-model", derouter_params=None
            )
        assert headers["Authorization"] == "Bearer sk-test-key-123"

    def test_generated_class_validate_environment_derouter_params_override(self):
        """api_key from derouter_params takes precedence over env"""
        from derouter.llms.openai_like.dynamic_config import (
            create_responses_config_class,
        )
        ...
        derouter_params = GenericDeRouterParams(api_key="sk-override-key")
        headers = config.validate_environment(
            headers={}, model="test-model", derouter_params=derouter_params
        )
...
class TestProviderConfigManagerResponsesAPI:
    ...
    def test_python_class_takes_priority_over_json(self):
        ...
        from derouter.llms.perplexity.responses.transformation import (
