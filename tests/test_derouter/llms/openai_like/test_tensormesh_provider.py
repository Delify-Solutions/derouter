...
class TestTensormeshProviderConfig:
    ...
    def test_tensormesh_in_provider_list(self):
        ...
        assert "tensormesh" in derouter.provider_list
    ...
    def test_tensormesh_text_completion_enabled(self):
        ...
        assert "tensormesh" in derouter.openai_text_completion_compatible_providers

    def test_tensormesh_responses_api_enabled(self):
        """Tensormesh declares /v1/responses in supported_endpoints, so derouter
...
class TestTensormeshCostMap:
    """The serverless models are registered in the cost map so DeRouter can
    ...
    def test_models_registered_with_capabilities(self):
        for model in TENSORMESH_MODELS:
            ...
            assert info["derouter_provider"] == "tensormesh"
            assert info["mode"] == "chat"
            assert derouter.supports_function_calling(model) is True, model
            assert derouter.supports_response_schema(model) is True, model
            assert derouter.model_cost[model]["supports_tool_choice"] is True, model
            assert derouter.model_cost[model]["supports_prompt_caching"] is True, model
    ...
    def test_reasoning_flag_matches_expected_set(self):
        ...
        for model in TENSORMESH_MODELS:
            assert derouter.supports_reasoning(model) is (model in reasoning_models), model

    def test_cost_is_wired_and_cache_reads_are_free(self):
        prompt_cost, completion_cost = derouter.cost_per_token(
            model="tensormesh/openai/gpt-oss-120b",
        ...
        assert (
            derouter.model_cost["tensormesh/openai/gpt-oss-120b"][
