...
Source: derouter/llms/github_copilot/responses/transformation.py
"""
...
def use_local_model_cost_map(monkeypatch: pytest.MonkeyPatch):
    """Pin derouter.model_cost to the bundled local backup so tests don't depend
    ...
    monkeypatch.setattr(
        derouter, "model_cost", get_model_cost_map(url=derouter.model_cost_map_url)
    )
...
class TestGithubCopilotResponsesAPITransformation:
    ...
    @patch("derouter.llms.github_copilot.responses.transformation.Authenticator")
    def test_github_copilot_responses_endpoint_url(self, mock_authenticator_class):
        ...
        url = config.get_complete_url(api_base=None, derouter_params={})
        assert (
        ...
        custom_url = config.get_complete_url(
            api_base="https://custom.githubcopilot.com", derouter_params={}
        )
        assert (
        ...
        url_with_slash = config.get_complete_url(
            api_base="https://api.githubcopilot.com/", derouter_params={}
        )
        assert (
    ...
    def test_validate_environment_default_headers(self, mock_authenticator_class):
        ...
        headers = config.validate_environment(
            headers={}, model="gpt-5.1-codex", derouter_params={}
        )
    ...
    def test_validate_environment_user_headers_override(self, mock_authenticator_class):
        ...
        headers = config.validate_environment(
            headers=custom_headers, model="gpt-5.1-codex", derouter_params={}
        )
    ...
    def test_validate_environment_with_vision_header(self, mock_authenticator_class):
        ...
        # Create mock derouter_params with input attribute
        mock_derouter_params = MagicMock()
        mock_derouter_params.input = [
            {
                "role": "user",
                "content": [{"type": "input_image", "data": "base64..."}],
            }
        ...
        headers = config.validate_environment(
            headers={}, model="gpt-5.1-codex", derouter_params=mock_derouter_params
        )
...
class TestGithubCopilotResponsesAPIRouting:
    ...
    @patch(
        "derouter.llms.github_copilot.responses.transformation._cached_get_model_info_helper"
    )
    ...
    def test_returns_config_when_mode_unset_but_endpoints_have_responses(self):
        ...
        from the raw ``derouter.model_cost`` entry; a mock-based test would
        mask that.
        """
        derouter.register_model(
            {
                "github_copilot/test-endpoints-only-model": {
                    "derouter_provider": "github_copilot",
                    "max_tokens": 1,
    ...
    def test_user_override_via_register_model(self, mock_get_info):
        ...
        ``derouter.register_model`` (called by the router) into the merged
...
class TestGithubCopilotReasoningStreamItemIdNormalization:
    ...
    def _config(self):
        with patch(
            "derouter.llms.github_copilot.responses.transformation.Authenticator"
        ):
