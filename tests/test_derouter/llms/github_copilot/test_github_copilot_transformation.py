...
import derouter.llms.github_copilot.chat.transformation
from derouter import Choices, Message, ModelResponse, Usage, acompletion, completion
from derouter.exceptions import AuthenticationError
from derouter.llms.github_copilot.authenticator import Authenticator
from derouter.llms.github_copilot.chat.transformation import GithubCopilotConfig
...
@patch("derouter.llms.github_copilot.authenticator.Authenticator.get_api_key")
@patch("derouter.main.openai_chat_completions.completion")
@patch("derouter.llms.openai.openai.OpenAIChatCompletion.completion")
...
def test_transform_messages_disable_copilot_system_to_assistant(monkeypatch):
    ...
    # Save original value
    original_flag = derouter.disable_copilot_system_to_assistant
    try:
        # Case 1: Flag is False (default, conversion happens)
        derouter.disable_copilot_system_to_assistant = False
        config = GithubCopilotConfig()
        messages = [
        ...
        derouter.disable_copilot_system_to_assistant = True
        out = config._transform_messages(
            [m.copy() for m in messages], model="github_copilot/gpt-4"
        )
        ...
        derouter.disable_copilot_system_to_assistant = original_flag
...
class TestGithubCopilotTransformParsedResponseDict:
    ...
    See: https://github.com/Delify-Solutions/derouter/issues/30927
    """
...
@patch("derouter.llms.openai.openai.OpenAIChatCompletion._get_openai_client")
@patch(
