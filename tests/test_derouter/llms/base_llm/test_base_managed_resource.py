...
from derouter.llms.base_llm.managed_resources.base_managed_resource import (
    BaseManagedResource,
)
...
class _StubResource(BaseManagedResource):
    ...
    def table_name(self) -> str:
        return "derouter_test_resource_table"
    ...
    async def create_resource_for_model(
        self, llm_router, model, request_data, derouter_parent_otel_span
    ):
        return {"id": "test"}
...
def _make_resource(records: List = None) -> _StubResource:
    ...
    prisma.db = MagicMock()
    setattr(prisma.db, "derouter_test_resource_table", table)
...
async def test_list_admin_query_is_unscoped():
    ...
    table = resource.prisma_client.db.derouter_test_resource_table
...
async def test_list_user_filters_by_user_id():
    ...
    where = resource.prisma_client.db.derouter_test_resource_table.find_many.await_args.kwargs[
        "where"
    ]
...
async def test_list_identity_less_caller_returns_empty_without_query():
    ...
    resource.prisma_client.db.derouter_test_resource_table.find_many.assert_not_awaited()
