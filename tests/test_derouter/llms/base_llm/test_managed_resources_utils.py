"""
Tests for `derouter.llms.base_llm.managed_resources.utils.extract_model_id_from_unified_id`.
...
def test_extract_model_id_returns_value_when_field_is_first():
    ...
    unified_id = "derouter_proxy:vector_store;model_id,first-field-value;unified_id,abc"
...
def test_extract_model_id_returns_none_for_file_id_without_model_id_field():
    ...
    File-IDs constructed via `DEROUTER_MANAGED_FILE_COMPLETE_STR` have
...
def test_extract_model_id_returns_none_when_field_absent():
    assert (
        extract_model_id_from_unified_id(
            "derouter_proxy:other;unified_id,abc;some_field,whatever"
        )
        is None
    )
