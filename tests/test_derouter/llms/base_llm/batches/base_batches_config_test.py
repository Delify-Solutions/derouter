...
`derouter.llms.base_llm.batches.transformation.BaseBatchesConfig` gets a shared
...
e.g. every `transform_retrieve_batch_response` returns a real `DeRouterBatch`
...
class BatchesConfigContractTests:
    ...
    def test_contract__retrieve_response_is_valid_derouter_batch(self):
