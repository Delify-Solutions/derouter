...
def setup_mocks(monkeypatch):
    ...
    with (
        patch(
            "derouter.llms.azure.common_utils.get_azure_ad_token_from_entra_id"
        ) as mock_entra_token,
        patch(
            "derouter.llms.azure.common_utils.get_azure_ad_token_from_username_password"
        ) as mock_username_password_token,
        patch(
            "derouter.llms.azure.common_utils.get_azure_ad_token_from_oidc"
        ) as mock_oidc_token,
        patch(
            "derouter.llms.azure.common_utils.get_azure_ad_token_provider"
        ) as mock_token_provider,
        patch("derouter.llms.azure.common_utils.derouter") as mock_derouter,
        patch("derouter.llms.azure.common_utils.verbose_logger") as mock_logger,
        patch(
            "derouter.llms.azure.common_utils.select_azure_base_url_or_endpoint"
        ) as mock_select_url,
    ):
        # Configure mocks
        mock_derouter.AZURE_DEFAULT_API_VERSION = "2023-05-15"
        mock_derouter.enable_azure_ad_token_refresh = False
...
def test_initialize_with_oidc_token(setup_mocks, monkeypatch):
    ...
    result = BaseAzureLLM().initialize_azure_sdk_client(
        derouter_params={"azure_ad_token": "oidc/test-token"},
        api_key=None,
        api_base="https://test.openai.azure.com",
        model_name="gpt-4",
        api_version=None,
...
def test_initialize_with_enable_token_refresh(setup_mocks, monkeypatch):
    ...
    monkeypatch.delenv("AZURE_CLIENT_SECRET", raising=False)
    monkeypatch.delenv("AZURE_TENANT_ID", raising=False)
    setup_mocks["derouter"].enable_azure_ad_token_refresh = True
...
async def test_ensure_initialize_azure_sdk_client_always_used(call_type):
    ...
    patch_target = (
        "derouter.llms.azure.common_utils.BaseAzureLLM.initialize_azure_sdk_client"
    )
    if call_type == CallTypes.arerank:
        patch_target = (
            "derouter.rerank_api.main.azure_rerank.initialize_azure_sdk_client"
        )
    elif (
        ...
        patch_target = (
            "derouter.batches.main.azure_batches_instance.initialize_azure_sdk_client"
        )
    elif (
        ...
        patch_target = (
            "derouter.assistants.main.azure_assistants_api.initialize_azure_sdk_client"
        )
    elif call_type == CallTypes.acreate_file or call_type == CallTypes.afile_content:
        patch_target = (
            "derouter.files.main.azure_files_instance.initialize_azure_sdk_client"
        )
    elif (
    ...
    with patch(patch_target) as mock_init_azure:
        ...
        derouter_params = azure_calls[0].kwargs["derouter_params"]
        print("derouter_params", derouter_params)

        assert (
            "azure_ad_token" in derouter_params
...
async def test_ensure_initialize_azure_sdk_client_always_used_azure_text(call_type):
    ...
    patch_target = "derouter.main.azure_text_completions.initialize_azure_sdk_client"
...
async def test_azure_client_reuse(function_name, is_async, args):
    ...
    client_path = (
        "derouter.llms.azure.common_utils.AsyncAzureOpenAI"
        if is_async
        else "derouter.llms.azure.common_utils.AzureOpenAI"
    )
        ...
        for _ in range(10):
            try:
                ...
                if is_async:
                    ...
                    func = getattr(derouter, f"a{function_name}")
                    await func(**args)
                else:
                    func = getattr(derouter, function_name)
                    func(**args)
            except Exception:
...
async def test_azure_client_cache_separates_sync_and_async():
    ...
    derouter.in_memory_llm_clients_cache._cache = {}
    ...
    with (
        patch("derouter.llms.azure.common_utils.AzureOpenAI") as mock_sync_client_class,
        patch(
            "derouter.llms.azure.common_utils.AsyncAzureOpenAI"
...
def test_scope_always_string_in_initialize_azure_sdk_client(setup_mocks, monkeypatch):
    ...
    # Test case 1: scope is None in derouter_params
    result = base_llm.initialize_azure_sdk_client(
        derouter_params={"azure_scope": None},
        api_key="test-api-key",
        api_base="https://test.openai.azure.com",
        model_name="gpt-4",
        api_version="2023-06-01",
    ...
    result = base_llm.initialize_azure_sdk_client(
        derouter_params={"azure_scope": custom_scope},
        api_key="test-api-key",
        api_base="https://test.openai.azure.com",
        model_name="gpt-4",
        api_version="2023-06-01",
...
def test_with_existing_token_provider(setup_mocks):
    ...
    derouter_params = GenericDeRouterParams(azure_ad_token_provider=token_provider)

    token = get_azure_ad_token(derouter_params)
...
def test_with_existing_azure_ad_token(setup_mocks):
    ...
    derouter_params = GenericDeRouterParams(azure_ad_token="test-token")
...
def test_with_existing_azure_ad_token_from_env(setup_mocks):
    ...
    with patch("derouter.llms.azure.common_utils.get_secret_str") as mock_get_secret_str:
...
def test_token_provider_returns_non_string(setup_mocks):
    ...
    derouter_params = GenericDeRouterParams(azure_ad_token_provider=non_string_provider)

    # Call the function and expect a TypeError
    with pytest.raises(TypeError) as excinfo:
        get_azure_ad_token(derouter_params)
...
def test_token_provider_raises_exception(setup_mocks):
    ...
    derouter_params = GenericDeRouterParams(azure_ad_token_provider=error_provider)
...
def test_azure_v1_api_with_azure_ad_token_provider(api_version):
    ...
    Regression: https://github.com/Delify-Solutions/derouter/issues/27945 — before the fix
...
def test_azure_v1_client_cache_separates_distinct_ad_providers(api_version):
    ...
    with patch.object(base_llm, "initialize_azure_sdk_client") as mock_init:
        ...
        client_a = base_llm.get_azure_openai_client(
            ...
            api_base=api_base,
            api_version=api_version,
            derouter_params={"azure_ad_token_provider": provider_a},
            _is_async=True,
        )
    ...
    with patch.object(base_llm, "initialize_azure_sdk_client") as mock_init:
        ...
        client_b = base_llm.get_azure_openai_client(
            ...
            api_base=api_base,
            api_version=api_version,
            derouter_params={"azure_ad_token_provider": provider_b},
            _is_async=True,
        )
...
def test_azure_v1_client_cache_reuses_for_identical_ad_config(api_version):
    ...
    with patch.object(base_llm, "initialize_azure_sdk_client") as mock_init:
        ...
        client_a = base_llm.get_azure_openai_client(
            ...
            api_base=api_base,
            api_version=api_version,
            derouter_params={"azure_ad_token_provider": provider},
            _is_async=True,
        )
        client_b = base_llm.get_azure_openai_client(
            api_key=None,
...
def test_evicting_an_azure_client_built_on_the_callers_session_leaves_it_open(monkeypatch):
    """`initialize_azure_sdk_client` puts `derouter.aclient_session` on the SDK client.

    That session belongs to the caller. `AsyncAzureOpenAI.close()` closes whatever
    http client it was handed, so treating the wrapper as derouter's to close would
    ...
    monkeypatch.setattr(derouter, "aclient_session", shared_session)
    monkeypatch.setattr(
        derouter,
    ...
    wrapper = BaseAzureLLM().get_azure_openai_client(
        ...
        api_base="https://derouter.openai.azure.com",
...
def test_an_azure_client_derouter_built_its_own_http_client_for_is_still_closed(monkeypatch):
    """The ownership check must not turn the reclaim off for the ordinary case."""
    from derouter.caching.evicted_client_closer import EvictedClientCloser
    from derouter.caching.llm_caching_handler import LLMClientCache

    closer = EvictedClientCloser(grace_seconds=0.0)
    monkeypatch.setattr(derouter, "aclient_session", None)
    monkeypatch.setattr(derouter, "client_session", None)
    monkeypatch.setattr(
        derouter,
        "in_memory_llm_clients_cache",
        LLMClientCache(evicted_client_closer=closer),
    ...
    assert closer.pending_count == 1, "derouter built this client's http client, so it owns it"
