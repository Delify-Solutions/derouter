...
import derouter
from derouter.llms.azure.image_edit.transformation import AzureImageEditConfig
...
def test_validate_environment_prefers_derouter_params_api_key():
    config = AzureImageEditConfig()
    headers = config.validate_environment(
        headers={},
        model="gpt-image-1",
        api_key=None,
        derouter_params={"api_key": "from-params"},
    )
    assert headers.get("api-key") == "from-params"
    assert "Authorization" not in headers


def test_validate_environment_derouter_params_api_key_beats_positional_arg():
    """
    Precedence pin: when both ``api_key`` (positional) and
    ``derouter_params["api_key"]`` are set with different values, the
    ``derouter_params`` value wins.

    This matches the convention used by every other Azure
    ``validate_environment`` (videos, vector_stores, responses, ...), where
    ``derouter_params.api_key`` is the source of truth and the positional
    kwarg only fills in when ``derouter_params`` lacks a key. In production
    the only caller (``llm_http_handler.image_edit``) sources both values
    from the same ``derouter_params.api_key``, so this only matters for
    direct callers.
    """
    config = AzureImageEditConfig()
    headers = config.validate_environment(
...
def test_validate_environment_falls_back_to_aad_bearer_when_no_api_key():
    ...
    with (
        patch(
            "derouter.llms.azure.common_utils.get_azure_ad_token",
            return_value="fake-aad-token",
        ),
        patch(
            "derouter.llms.azure.common_utils.get_secret_str",
            return_value=None,
        ),
        patch("derouter.api_key", None),
        patch("derouter.azure_key", None),
    ):
        headers = config.validate_environment(
            headers={},
            model="gpt-image-1",
...
def test_azure_finalize_image_edit_strips_model_after_openai_transform():
    ...
    resolved = config.get_complete_url(
        ...
        derouter_params=derouter_params.model_dump(exclude_none=True),
    )
...
#   derouter_params["api_version"]
#     > derouter.api_version (module-global)
#     > AZURE_API_VERSION env var
#     > derouter.AZURE_DEFAULT_API_VERSION
#
# Before this fallback chain existed, image edit only read ``derouter_params``
...
# in ``derouter/llms/azure/common_utils.py`` already had this fallback.
# ---------------------------------------------------------------------------
...
def test_api_version_uses_derouter_params_first(monkeypatch):
    monkeypatch.setattr(derouter, "api_version", "from-global", raising=False)
    monkeypatch.setenv("AZURE_API_VERSION", "from-env")

    url = AzureImageEditConfig().get_complete_url(
        model=_FALLBACK_MODEL,
        api_base=_FALLBACK_API_BASE,
        derouter_params={"api_version": "from-params"},
    )

    assert _query_params(url) == {"api-version": "from-params"}


def test_api_version_falls_back_to_derouter_global(monkeypatch):
    monkeypatch.setattr(derouter, "api_version", "from-global", raising=False)
    monkeypatch.setenv("AZURE_API_VERSION", "from-env")
...
def test_api_version_falls_back_to_env_var(monkeypatch):
    monkeypatch.setattr(derouter, "api_version", None, raising=False)
    monkeypatch.setenv("AZURE_API_VERSION", "from-env")
...
def test_api_version_falls_back_to_azure_default(monkeypatch):
    ...
    assert _query_params(url) == {"api-version": derouter.AZURE_DEFAULT_API_VERSION}
...
def test_api_version_in_api_base_query_is_preserved(monkeypatch):
    ...
    url = AzureImageEditConfig().get_complete_url(
        ...
        derouter_params={"api_version": "would-be-overridden"},
    )
...
def test_v1_api_version_uses_v1_route_and_keeps_model(monkeypatch):
    ...
    for api_version in ("v1", "preview", "latest"):
        url = config.get_complete_url(
            model=_FALLBACK_MODEL,
            api_base=_FALLBACK_API_BASE,
            derouter_params={"api_version": api_version},
        )
...
def test_v1_api_version_from_global_uses_v1_route(monkeypatch):
    monkeypatch.setattr(derouter, "api_version", "preview", raising=False)
    monkeypatch.delenv("AZURE_API_VERSION", raising=False)
...
def test_dated_api_version_still_uses_deployment_route(monkeypatch):
    ...
    url = AzureImageEditConfig().get_complete_url(
        model=_FALLBACK_MODEL,
        api_base=_FALLBACK_API_BASE,
        derouter_params={"api_version": "2024-10-21"},
    )
