"""Unit tests for ``AzureBatchesAPI`` (derouter/llms/azure/batches/handler.py).
...
the client type, and parses the SDK response into ``DeRouterBatch``.
...
We mock only true boundaries:
  * ``get_azure_openai_client`` - the credential/client-construction seam. We
    ...
    _is_async / derouter_params) forwarded to it.
  * the returned Azure OpenAI client's ``batches.*`` methods - the network call.
    ...
    into ``DeRouterBatch`` (sibling SDK methods asserted NOT called).
...
from derouter.llms.azure.azure import AsyncAzureOpenAI, AzureOpenAI  # noqa: E402
from derouter.llms.azure.batches.handler import AzureBatchesAPI  # noqa: E402
from derouter.types.utils import DeRouterBatch  # noqa: E402

GET_CLIENT = "derouter.llms.azure.batches.handler.AzureBatchesAPI.get_azure_openai_client"
...
def _batch_dict(batch_id: str = "batch-123", status: str = "completed") -> dict:
    """A minimal-but-valid dict for ``DeRouterBatch(**response.model_dump())``."""
    return {
        "id": batch_id,
        "completion_window": "24h",
...
def test_create_sync_forwards_auth_to_client_seam(handler):
    ...
    # derouter_params defaults to {} (not None) when not supplied.
    assert kw["derouter_params"] == {}
    ...
    # RESULT: parsed into DeRouterBatch from the SDK response's model_dump.
    assert isinstance(result, DeRouterBatch)
    assert result.id == "batch-123"
    assert result.status == "completed"
    assert result.output_file_id == "file-out-xyz"


def test_create_sync_passes_derouter_params_through(handler):
    client = _sync_client()
    client.batches.create.return_value = _sdk_response(_batch_dict())
    ...
    assert get_client.call_args.kwargs["derouter_params"] == lp
...
# =========================================================================== #
# list_batches  (returns the raw SDK response, NOT a DeRouterBatch)
# =========================================================================== #
...
def test_list_sync_forwards_after_limit_and_returns_raw_response(handler):
    ...
    # list returns the SDK response untouched (no DeRouterBatch parsing).
