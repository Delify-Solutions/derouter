...
from derouter.llms.custom_httpx.http_handler import get_shared_realtime_ssl_context
...
async def test_async_realtime_uses_max_size_parameter():
    ...
    This verifies the fix for: https://github.com/Delify-Solutions/derouter/issues/15747
    """
    from derouter.constants import REALTIME_WEBSOCKET_MAX_MESSAGE_SIZE_BYTES
    from derouter.llms.azure.realtime.handler import AzureOpenAIRealtime
    ...
    with (
        ...
        patch(
            "derouter.llms.azure.realtime.handler.RealTimeStreaming"
...
async def test_realtime_protocol_env_var_fallback():
    """
    Test that DEROUTER_AZURE_REALTIME_PROTOCOL env var is used as fallback.
    Fixes #22127: no way to set realtime_protocol from config.
    """
    from derouter.realtime_api.main import _arealtime
    from derouter.types.router import GenericDeRouterParams

    with patch.dict(os.environ, {"DEROUTER_AZURE_REALTIME_PROTOCOL": "v1"}):
        # Create a GenericDeRouterParams without realtime_protocol
...
@pytest.mark.asyncio
async def test_realtime_protocol_from_derouter_params():
    """
    Test that realtime_protocol is read from derouter_params (config.yaml extra field).
    Fixes #22127: realtime_protocol in derouter_params was not used.
    """
    from derouter.types.router import GenericDeRouterParams

    # Simulate config.yaml with realtime_protocol as an extra field
    derouter_params = GenericDeRouterParams(realtime_protocol="GA")
    assert derouter_params.get("realtime_protocol") == "GA"
...
async def test_async_realtime_uses_bearer_token_when_no_api_key():
    ...
    Regression test for https://github.com/Delify-Solutions/derouter/issues/34654
    """
    from derouter.llms.azure.realtime.handler import AzureOpenAIRealtime
...
async def test_arealtime_resolves_azure_ad_token_when_no_api_key(monkeypatch):
    ...
    def fake_get_azure_ad_token(derouter_params):
        captured_params["tenant_id"] = derouter_params.get("tenant_id")
...
async def test_arealtime_does_not_resolve_azure_ad_token_when_api_key_present(monkeypatch):
    ...
    def fail_get_azure_ad_token(derouter_params):
...
async def test_realtime_health_check_uses_bearer_token_when_no_api_key(monkeypatch):
    ...
    monkeypatch.setattr(
        realtime_main,
        "get_azure_ad_token",
        lambda derouter_params: "my-entra-token",
    )
...
async def test_arealtime_forwards_deployment_azure_ad_token(monkeypatch):
    ...
    **kwargs, so it must still reach the handler.
    ...
    monkeypatch.delenv("AZURE_API_KEY", raising=False)
    monkeypatch.setattr(realtime_main.derouter, "api_key", None)
