...
class TestAzureVideoConfig:
    ...
    @patch("derouter.llms.azure.common_utils.derouter")
    def test_validate_environment_with_api_key(self, mock_derouter):
        """Test environment validation with provided API key - should use api-key header for Azure."""
        # Since validate_environment passes derouter_params=None, it relies on derouter.api_key or derouter.azure_key
        mock_derouter.api_key = self.api_key
        mock_derouter.azure_key = None
    ...
    @patch("derouter.llms.azure.common_utils.get_secret_str")
    @patch("derouter.llms.azure.common_utils.derouter")
    def test_validate_environment_without_api_key(self, mock_derouter, mock_get_secret):
        """Test environment validation without provided API key - should fallback to secret manager."""
        mock_derouter.api_key = None
        mock_derouter.azure_key = None
    ...
    def test_get_complete_url(self):
        ...
        url = self.config.get_complete_url(
            model=self.model, api_base=self.api_base, derouter_params=derouter_params
        )
    ...
    def test_url_construction_with_trailing_slash(self):
        ...
        derouter_params = {"api_base": api_base_with_slash}
    ...
    def test_url_construction_without_trailing_slash(self):
        ...
        derouter_params = {"api_base": api_base_without_slash}
    ...
    @patch("derouter.llms.azure.common_utils.derouter")
    def test_azure_specific_environment_validation(self, mock_derouter):
        """Test Azure-specific environment validation with different key sources."""
        # Test with azure_key
        mock_derouter.api_key = None
        mock_derouter.azure_key = "azure-test-key"
        mock_derouter.openai_key = None
