...
from derouter.llms.azure.responses.o_series_transformation import (
...
@pytest.mark.serial
def test_validate_environment_api_key_within_derouter_params():
    azure_openai_responses_apiconfig = AzureOpenAIResponsesAPIConfig()
    derouter_params = GenericDeRouterParams(api_key="test-api-key")

    result = azure_openai_responses_apiconfig.validate_environment(
        headers={}, model="", derouter_params=derouter_params
    )
...
@pytest.mark.serial
def test_validate_environment_api_key_within_derouter():
    azure_openai_responses_apiconfig = AzureOpenAIResponsesAPIConfig()

    with patch("derouter.api_key", "test-api-key"):
...
@pytest.mark.serial
def test_validate_environment_azure_key_within_derouter():
    azure_openai_responses_apiconfig = AzureOpenAIResponsesAPIConfig()

    with patch("derouter.azure_key", "test-azure-key"):
        derouter_params = GenericDeRouterParams()
        result = azure_openai_responses_apiconfig.validate_environment(
            headers={}, model="", derouter_params=derouter_params
        )
...
def test_validate_environment_azure_key_within_headers():
    ...
    result = azure_openai_responses_apiconfig.validate_environment(
        headers=headers, model="", derouter_params=derouter_params
    )
...
def test_get_complete_url():
    ...
    """
    azure_openai_responses_apiconfig = AzureOpenAIResponsesAPIConfig()
    api_base = "https://derouter8397336933.openai.azure.com"
    derouter_params = {"api_version": "2024-05-01-preview"}

    result = azure_openai_responses_apiconfig.get_complete_url(
        api_base=api_base, derouter_params=derouter_params
    )

    expected = "https://derouter8397336933.openai.azure.com/openai/responses?api-version=2024-05-01-preview"
...
def test_response_id_path_requests_encode_response_id():
    ...
    api_base = (
        "https://derouter8397336933.openai.azure.com/openai/responses"
    ...
    assert (
        url
        == "https://derouter8397336933.openai.azure.com/openai/responses/..%2F..%2Fresponses%2Fother%3Fx%3D1%23frag/cancel?api-version=2024-05-01-preview"
    )
    assert params == {}
...
def test_provider_config_manager_o_series_selection():
    ...
    o_series_config = ProviderConfigManager.get_provider_responses_api_config(
        provider=derouter.LlmProviders.AZURE, model="o_series/gpt-o1"
    )
    ...
    regular_config = ProviderConfigManager.get_provider_responses_api_config(
        provider=derouter.LlmProviders.AZURE, model="gpt-4o"
    )
    ...
    default_config = ProviderConfigManager.get_provider_responses_api_config(
        provider=derouter.LlmProviders.AZURE, model=None
    )
...
class TestAzureResponsesAPIConfig:
    ...
    def test_azure_get_complete_url_with_default_api_version(self):
        ...
        from derouter.constants import AZURE_DEFAULT_RESPONSES_API_VERSION
        ...
        expected_url = f"https://derouter8397336933.openai.azure.com/openai/v1/responses?api-version={AZURE_DEFAULT_RESPONSES_API_VERSION}"
        assert result_no_version == expected_url

        # Test with empty derouter_params - should use default
        ...
        result_none_version = self.config.get_complete_url(
            api_base=base_url,
            derouter_params={"api_version": None},
        )
    ...
    def test_azure_cancel_response_api_request(self):
        ...
        derouter_params = GenericDeRouterParams(api_version="2024-05-01-preview")
    ...
    def test_azure_list_input_items_request_url_path_before_query(self):
        ...
        url, params = self.config.transform_list_input_items_request(
            response_id="resp_test123",
            api_base=api_base,
            derouter_params=GenericDeRouterParams(api_version="2025-03-01-preview"),
            headers={},
        )
    ...
    def test_azure_flattens_via_base_model_for_arbitrary_deployment_name(self):
        result = self.config.transform_responses_api_request(
            ...
            input="hi",
            response_api_optional_request_params={"tools": [self._anyof_tool()]},
            derouter_params=GenericDeRouterParams(model_info={"base_model": "azure/gpt-4o"}),
            headers={},
        )
    ...
    def test_azure_keeps_combinators_for_gpt5_base_model(self):
        ...
        result = self.config.transform_responses_api_request(
            ...
            derouter_params=GenericDeRouterParams(model_info={"base_model": "azure/gpt-5.4-mini"}),
            headers={},
        )
...
def local_model_cost_map(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(derouter, "model_cost", get_model_cost_map(url=derouter.model_cost_map_url))
    derouter.add_known_models(model_cost_map=derouter.model_cost)
