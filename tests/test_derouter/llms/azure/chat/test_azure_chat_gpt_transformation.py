...
from derouter.llms.azure.chat.gpt_5_transformation import AzureOpenAIGPT5Config
...
def test_azure_gpt_5_chat_stays_off_the_reasoning_path(model: str) -> None:
    """https://github.com/Delify-Solutions/derouter/issues/13781: gpt-5-chat* is a regular chat model."""
    mapped: Final = _MAPPED_PARAMS.validate_python(
        get_optional_params(
            model=model,
    ...
    supported: Final = _SUPPORTED_PARAMS.validate_python(
        derouter.get_supported_openai_params(model=model, custom_llm_provider="azure")
    )
...
def test_azure_gpt_5_takes_the_reasoning_path() -> None:
    ...
    supported: Final = _SUPPORTED_PARAMS.validate_python(
        derouter.get_supported_openai_params(model="gpt-5", custom_llm_provider="azure")
    )
...
class TestAzureToolSchemaCombinatorFlattening:
    ...
    def _transform(self, config, model, tools):
        return config.transform_request(
            ...
            derouter_params={"custom_llm_provider": "azure"},
            headers={},
        )
