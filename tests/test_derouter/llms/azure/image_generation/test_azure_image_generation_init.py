...
from derouter.llms.azure.azure import AzureChatCompletion
from derouter.llms.azure.image_generation.http_utils import (
...
from derouter.utils import get_optional_params_image_gen
...
def test_azure_image_generation_flattens_extra_body():
    ...
    This test verifies the fix for: https://github.com/Delify-Solutions/derouter/issues/16059
...
def test_azure_image_generation_creates_token_provider_from_credentials():
    ...
    azure_ad_token_provider from credentials in derouter_params if it's not already provided.
    """
    ...
    if azure_ad_token_provider is None:
        tenant_id = derouter_params_dict.get("tenant_id")
        client_id = derouter_params_dict.get("client_id")
        client_secret = derouter_params_dict.get("client_secret")
        azure_scope = (
            derouter_params_dict.get("azure_scope")
...
def test_azure_image_generation_headers_without_api_key():
    ...
    """
    from derouter.images.main import image_generation
...
def test_azure_image_generation_base_model_vs_deployment_name():
    ...
    gpt-image-2; see DeRouter #26316). ``base_model`` in derouter_params is still used
    internally for logging / hidden params.

    Example config:
      model: azure/gpt-image-15  # deployment name (URL only)
      base_model: gpt-image-1.5  # optional, for DeRouter metadata
    """
...
async def test_azure_aimage_generation_base_model_vs_deployment_name():
    ...
    with patch(
        "derouter.llms.azure.azure.get_async_httpx_client", return_value=mock_client
    ):
        logging_obj = MagicMock()
        logging_obj.pre_call = MagicMock()
        logging_obj.post_call = MagicMock()
