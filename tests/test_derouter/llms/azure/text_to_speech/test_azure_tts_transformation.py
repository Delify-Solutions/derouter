...
import derouter
from derouter.llms.azure.text_to_speech.transformation import AzureAVATextToSpeechConfig
...
def test_get_complete_url_cognitive_services(
    ...
    url = azure_tts_config.get_complete_url(
        model="azure-tts", api_base=api_base, derouter_params={}
    )
...
def test_get_complete_url_missing_api_base(
    ...
    with pytest.raises(ValueError, match="api_base is required"):
        azure_tts_config.get_complete_url(
            model="azure-tts", api_base=None, derouter_params={}
        )
...
def test_derouter_speech_with_ssml_passthrough(mock_post):
    """
    Test that derouter.speech passes SSML through to Azure AVA without transformation
    """
    raw_ssml = """<speak version='1.0' xmlns='http://www.w3.org/2001/10/synthesis' xml:lang='en-US'>
    <voice name='en-US-JennyNeural'>
        <prosody rate='fast' pitch='high'>
