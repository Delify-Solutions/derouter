...
def test_transform_search_response_real_fixture_dedupes_and_preserves_order():
    ...
    assert [r.url for r in resp.results] == [
        "https://github.com/Delify-Solutions/derouter/releases",
        "https://releasealert.dev/github/Delify-Solutions/derouter",
        "https://api.github.com/repos/Delify-Solutions/derouter/releases/latest",
    ]
    assert resp.results[0].title == "Releases · Delify-Solutions/derouter - GitHub"
    assert resp.results[1].title == "Delify-Solutions/derouter on GitHub | Release Alert"


def test_transform_search_response_real_fixture_snippets_are_the_cited_claims():
    resp = _config().transform_search_response(_resp(REAL_FIXTURE), logging_obj=Mock())

    assert resp.results[0].snippet.startswith("- On the GitHub **Releases** page for Delify-Solutions/derouter")
...
def test_transform_search_response_web_search_mode_zeroes_per_query_cost():
    payload = _message_response("claim", [_citation("https://example.com", "Example", 0, 5)])
    resp = _config().transform_search_response(_resp(payload), logging_obj=Mock())
    assert resp._hidden_params["additional_headers"]["llm_provider-x-derouter-response-cost"] == 0.0
