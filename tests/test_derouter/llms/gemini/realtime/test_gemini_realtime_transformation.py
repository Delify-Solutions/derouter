...
def test_gemini_realtime_transformation_session_created():
    ...
    logging_obj.derouter_trace_id = "123"
...
def test_session_created_does_not_overwrite_session_configuration_request():
    ...
    logging_obj = MagicMock()
    logging_obj.derouter_trace_id = "trace_123"
...
def test_gemini_realtime_transformation_content_delta():
    ...
    session_created_message_str = json.dumps(session_created_message)
    logging_obj = MagicMock()
    logging_obj.derouter_trace_id.return_value = "123"
...
def test_gemini_model_turn_event_mapping():
    from derouter.types.llms.openai import OpenAIRealtimeEventTypes
...
def test_gemini_realtime_tool_call_transformation():
    ...
    logging_obj = MagicMock()
    logging_obj.derouter_trace_id = "test-trace-123"
...
def test_gemini_requires_session_configuration_feature_flag(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "gemini_live_defer_setup", False, raising=False)
    assert config.requires_session_configuration() is True

    # Opt-in behavior: defer setup until client sends session.update
    monkeypatch.setattr(derouter, "gemini_live_defer_setup", True, raising=False)
...
def test_gemini_realtime_function_call_output_transformation():
    ...
    logging_obj = MagicMock()
    logging_obj.derouter_trace_id = "trace_func_output"
...
def test_gemini_empty_tool_call_does_not_crash_websocket():
    ...
    config = GeminiRealtimeConfig()
    logging_obj = MagicMock()
    logging_obj.derouter_trace_id = "trace_empty_tool_call"
...
def test_gemini_empty_tool_call_with_sibling_usage_metadata_does_not_crash():
    ...
    config = GeminiRealtimeConfig()
    logging_obj = MagicMock()
    logging_obj.derouter_trace_id = "trace_empty_tool_call_with_sibling"
...
def test_gemini_tool_call_response_done_includes_usage_from_sibling_metadata():
    ...
    config = GeminiRealtimeConfig()
    logging_obj = MagicMock()
    logging_obj.derouter_trace_id = "trace_tool_call_usage"
...
def test_gemini_tool_call_response_done_falls_back_to_empty_usage():
    ...
    config = GeminiRealtimeConfig()
    logging_obj = MagicMock()
    logging_obj.derouter_trace_id = "trace_tool_call_no_usage"
...
def test_gemini_tool_call_id_to_name_evicts_oldest_when_capped():
    ...
    config = GeminiRealtimeConfig()
    logging_obj = MagicMock()
    logging_obj.derouter_trace_id = "trace_lru"
...
def test_gemini_standalone_usage_metadata_does_not_crash_websocket():
    ...
    config = GeminiRealtimeConfig()
    logging_obj = MagicMock()
    logging_obj.derouter_trace_id = "trace_usage_only"
...
def test_gemini_standalone_usage_metadata_is_attributed_to_next_tool_call_response_done():
    ...
    config = GeminiRealtimeConfig()
    logging_obj = MagicMock()
    logging_obj.derouter_trace_id = "trace_standalone_usage_then_tool_call"
...
def test_gemini_standalone_usage_metadata_is_attributed_to_next_response_done():
    ...
    config = GeminiRealtimeConfig()
    logging_obj = MagicMock()
    logging_obj.derouter_trace_id = "trace_standalone_usage_then_turn_complete"
...
def test_gemini_in_frame_usage_metadata_clears_pending_buffer():
    ...
    logging_obj = MagicMock()
    logging_obj.derouter_trace_id = "trace_in_frame_clears_buffer"
...
def test_gemini_post_tool_bare_turn_complete_followed_by_answer():
    ...
    config = GeminiRealtimeConfig()
    logging_obj = MagicMock()
    logging_obj.derouter_trace_id = "trace_post_tool_bare_turn_complete"
...
def patch_gemini_audio_cost_map_entries(monkeypatch):
    ...
    derouter.model_cost is fetched from main branch at import time, so in CI
    ...
    for m in native_audio_models:
        entry = dict(derouter.model_cost.get(m, {}))
        entry["gemini_native_audio"] = True
        monkeypatch.setitem(derouter.model_cost, m, entry)
...
def test_gemini_response_done_bills_audio_output_tokens_at_audio_rate(monkeypatch):
    ...
    cost = handle_realtime_stream_cost_calculation(
        ...
        derouter_model_name="gemini-2.5-flash-native-audio-preview-12-2025",
    )
...
def patch_gemini_transcribe_live_cost_map_entry(monkeypatch):
    ...
    for m in ["gemini-3.5-transcribe-live", "gemini/gemini-3.5-transcribe-live"]:
        ...
        monkeypatch.setitem(derouter.model_cost, m, entry)
    chat_entry = dict(derouter.model_cost.get("gemini-2.5-flash", {}))
    chat_entry["supported_output_modalities"] = ["text"]
    monkeypatch.setitem(derouter.model_cost, "gemini-2.5-flash", chat_entry)
...
def test_generation_complete_without_prior_delta_keeps_turn_usage(patch_gemini_audio_cost_map_entries):
    ...
    from derouter.types.realtime import RealtimeResponseTransformInput
...
def test_transcribe_live_completed_event_carries_estimated_usage(patch_gemini_transcribe_live_cost_map_entry):
    """Gemini Live sends no usageMetadata for transcribe sessions, so DeRouter bills
    ...
    from derouter.types.llms.gemini import BidiGenerateContentServerMessage
    from derouter.types.realtime import RealtimeInputAudioTranscriptionUsage, RealtimeResponseTransformInput
...
def test_unbilled_usage_on_session_close_flushes_trailing_audio(patch_gemini_transcribe_live_cost_map_entry):
    ...
    from derouter.types.realtime import RealtimeInputAudioTranscriptionUsage
