...
def bundled_cost_map(monkeypatch):
    ...
    derouter caches model-info lookups, so swapping ``model_cost`` only takes
...
class TestParallelAISearch:
    ...
    async def test_default_mode_is_basic(self):
        """v1 defaults to 'advanced' server-side; derouter must send 'basic' to keep v1beta's default tier and cost tracking accurate."""
        with patch(
            "derouter.llms.custom_httpx.http_handler.AsyncHTTPHandler.post",
            new_callable=AsyncMock,
        ) as mock_post:
