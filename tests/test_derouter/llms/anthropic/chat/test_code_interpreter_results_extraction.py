...
def test_extract_tool_result_output_items_from_pydantic_objects():
    ...
    result = DeRouterCompletionResponsesConfig._extract_tool_result_output_items(resp)
    assert len(result) == 2
...
def test_in_place_substitution_preserves_ordering():
    ...
    tool_result_items = (
        DeRouterCompletionResponsesConfig._extract_tool_result_output_items(resp)
    )
...
def test_end_to_end_streaming_chunks_to_code_interpreter_output():
    ...
    tool_result_items = (
        DeRouterCompletionResponsesConfig._extract_tool_result_output_items(assembled)
    )
