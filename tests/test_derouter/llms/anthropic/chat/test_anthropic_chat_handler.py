...
from derouter.llms.anthropic.chat.handler import ModelResponseIterator, make_call
...
from derouter.types.responses.main import OutputCodeInterpreterCall


@pytest.mark.asyncio
async def test_make_call_passes_logging_obj_to_client_post():
    """make_call must pass logging_obj to client.post so track_llm_api_timing can set llm_api_duration_ms for derouter_overhead_time_ms."""
...
def test_anthropic_completion_streaming_usage_matches_non_streaming_with_thinking():
    ...
    try:
        ...
        non_stream_response = derouter.completion(**request_kwargs, stream=False)
        ...
        content_chunks = []
        stream_usage = None
        for chunk in derouter.completion(
            **request_kwargs,
            stream=True,
...
def test_partial_json_chunk_accumulation():
    ...
    This tests the fix for https://github.com/Delify-Solutions/derouter/issues/17473
...
def test_web_search_tool_result_no_extra_tool_calls():
    ...
    This tests the fix for https://github.com/Delify-Solutions/derouter/issues/17254
...
def test_web_search_tool_result_captured_in_provider_specific_fields():
    ...
    This tests the fix for https://github.com/Delify-Solutions/derouter/issues/17737
...
def test_web_fetch_tool_result_captured_in_provider_specific_fields():
    ...
    This tests the fix for https://github.com/Delify-Solutions/derouter/issues/18137
...
class TestRustChatCompletionsHook:
    ...
    def _reset_bridge(self, monkeypatch):
        from derouter.rust_bridge import chat_completions as bridge

        monkeypatch.setenv("DEROUTER_RUST", "1")
    ...
    def test_rust_true_serves_the_call_and_stamps_the_header(self):
        ...
        assert response._hidden_params["additional_headers"] == {"x-derouter-rust": "true"}
    ...
    def test_without_the_opt_in_the_core_is_never_consulted(self, monkeypatch):
        monkeypatch.setenv("DEROUTER_RUST", "0")
        from derouter.llms.anthropic.chat.handler import AnthropicChatCompletion
        ...
        with patch.object(
            ...
            try:
                AnthropicChatCompletion().completion(
                    **self._completion_kwargs(derouter_params={})
                )
            except Exception:
    ...
    async def test_the_async_path_serves_the_rust_response_without_the_fallback(self):
        ...
        assert result._hidden_params["additional_headers"] == {"x-derouter-rust": "true"}
    ...
    def test_pre_call_logging_still_fires_when_rust_is_not_involved(self, monkeypatch):
        ...
        with patch.object(
            ...
            try:
                AnthropicChatCompletion().completion(
                    **self._completion_kwargs(derouter_params={}, logging_obj=logging_obj)
                )
            except Exception:
                pass
