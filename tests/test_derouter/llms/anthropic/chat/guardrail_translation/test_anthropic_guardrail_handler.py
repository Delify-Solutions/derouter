...
from derouter.llms.base_llm.guardrail_translation.base_translation import StreamingScanKey
...
class TestAnthropicMessagesHandlerStreamingRequestData:
    ...
    async def test_terminal_chunk_passes_assembled_response_and_metadata(self):
        ...
        with (
            ...
            patch(
                "derouter.llms.anthropic.chat.guardrail_translation.handler.AnthropicPassthroughLoggingHandler._build_complete_streaming_response",
        ...
        assert (
            guardrail.request_data["derouter_metadata"]["user_api_key_user_id"] == "u-1"
        )
...
class TestAnthropicMessagesHandlerInputProcessing:
    """Test input processing preserves derouter_metadata for dynamic guardrails."""

    @pytest.mark.asyncio
    async def test_process_input_messages_preserves_derouter_metadata_guardrails(self):
        handler = AnthropicMessagesHandler()
        ...
        assert data.get("derouter_metadata", {}).get("guardrails")
    ...
    async def test_compaction_rewrite_to_only_system_messages_is_rejected(self):
        ...
        with patch.object(derouter, "modify_params", False):
            with pytest.raises(derouter.BadRequestError, match="at least one non-system message"):
                await handler.process_input_messages(data=data, guardrail_to_apply=guardrail)
    ...
    async def test_compaction_rewrite_to_only_system_messages_repaired_with_modify_params(
        ...
        with patch.object(derouter, "modify_params", True):
            await handler.process_input_messages(data=data, guardrail_to_apply=guardrail)
    ...
    async def test_process_input_messages_with_anthropic_native_tools(self):
        ...
        result = await handler.process_input_messages(
            data=data, guardrail_to_apply=guardrail, derouter_logging_obj=MagicMock()
        )
...
class TestAnthropicMessagesIncrementalScan:
    ...
    def _bedrock_guardrail(self):
        from derouter.proxy.guardrails.guardrail_hooks.bedrock_guardrails import BedrockGuardrail
    ...
    def _data(self, messages, session_id):
        return {
            ...
            "derouter_session_id": session_id,
        }
