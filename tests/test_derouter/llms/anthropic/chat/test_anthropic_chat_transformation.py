...
from derouter.types.llms.anthropic import ANTHROPIC_BETA_HEADER_VALUES
from derouter.types.utils import ServerToolUse, Usage
...
def test_calculate_usage():
    ...
    Fixes https://github.com/Delify-Solutions/derouter/issues/9812
    """
    config = AnthropicConfig()
...
def test_streaming_iterator_persists_served_speed_across_usage_chunks():
    ...
    """
    from derouter.llms.anthropic.chat.handler import ModelResponseIterator
...
def test_calculate_usage_aggregates_cache_creation_split_across_iterations():
    ...
    """
    from derouter.llms.anthropic.cost_calculation import cost_per_token
    ...
    info = derouter.get_model_info(model="claude-opus-4-8", custom_llm_provider="anthropic")
...
def test_calculate_usage_nulls(usage_object, expected_usage):
    ...
    Fixes https://github.com/Delify-Solutions/derouter/issues/11920
    """
    config = AnthropicConfig()
...
def test_web_search_tool_transformation():
    from derouter.types.llms.openai import OpenAIWebSearchOptions
...
def test_anthropic_memory_tool_auto_adds_beta_header():
    """
    Tests that DeRouter automatically adds the required 'anthropic-beta' header
...
def test_anthropic_beta_header_merging_with_output_format():
    ...
    This is a regression test for: https://github.com/Delify-Solutions/derouter/issues/...
...
def test_effort_validation():
    ...
    with pytest.raises(
        derouter.exceptions.BadRequestError, match="Invalid effort value"
    ):
        config.transform_request(
            model="claude-opus-4-5-20251101",
            messages=messages,
...
def test_max_effort_rejected_for_opus_45():
    ...
    with pytest.raises(
        derouter.exceptions.BadRequestError,
...
def test_vertex_anthropic_raises_on_speed_without_drop_params(monkeypatch):
    ...
    with pytest.raises(derouter.utils.UnsupportedParamsError, match="drop_params"):
...
def test_get_max_tokens_for_model_claude_35():
    ...
    Fixes: https://github.com/Delify-Solutions/derouter/issues/8835
    """
    config = AnthropicConfig()

    # Claude 3.5 Sonnet should return 8192
    with patch(
        "derouter.llms.anthropic.chat.transformation.get_max_tokens",
        return_value=8192,
    ):
...
def test_transform_request_injects_dummy_tool_without_tools_param():
    ...
    DeRouter must inject a dummy tool without ``derouter.modify_params``.
    """
    config = AnthropicConfig()
    prev_modify_params = derouter.modify_params
    derouter.modify_params = False
    try:
        messages = [
            {"role": "user", "content": "Hello"},
            {
        ...
        derouter.modify_params = prev_modify_params
...
def test_calculate_usage_completion_tokens_details_always_populated():
    ...
    Fixes: https://github.com/Delify-Solutions/derouter/issues/18772
...
def test_reasoning_effort_garbage_raises_bad_request(effort):
    ...
    with pytest.raises(derouter.exceptions.BadRequestError):
...
def test_output_config_effort_empty_string_raises_bad_request():
    ...
    with pytest.raises(derouter.exceptions.BadRequestError, match="Invalid effort"):
        config.transform_request(
            model="claude-opus-4-7",
...
def test_code_execution_tool_results_extraction():
    ...
    Related to: https://github.com/Delify-Solutions/derouter/issues/xxxxx
    """
    import httpx
...
def test_fast_mode_cost_calculation():
    ...
    with (
        patch(
            "derouter.llms.anthropic.cost_calculation.generic_cost_per_token"
        ) as mock_cost,
        patch("derouter.get_model_info") as mock_info,
    ):
...
def test_map_tool_helper_enforces_object_type_when_missing():
    ...
    has parameters without a 'type' field (common with MCP servers), DeRouter
...
def test_map_tool_helper_enforces_object_type_when_wrong_type():
    ...
    DeRouter should normalize it to type:"object" for Anthropic compatibility.
    """
    config = AnthropicConfig()
...
def test_extract_response_content_thinking_block_null_thinking():
    ...
    or missing. Regression test for https://github.com/Delify-Solutions/derouter/issues/24026
    """
    config = AnthropicConfig()
...
def test_strip_advisor_blocks_when_no_advisor_tool():
    ...
    """
    from derouter.llms.anthropic.common_utils import strip_advisor_blocks_from_messages
...
def test_basic_sanitize_anthropic_tool_name_replaces_invalid_chars():
    from derouter.llms.anthropic.chat.transformation import (
...
def test_map_openai_params_does_not_pollute_optional_params_with_internal_keys():
    ...
    Anthropic (``data = {**optional_params}``). It MUST NOT carry DeRouter-
...
def test_transform_request_does_not_leak_internal_keys_into_body():
    ...
    Anthropic Messages fields -- no DeRouter coordination state."""
    config = AnthropicConfig()
    tools = [
        {
            "type": "function",
    ...
    # Body must not contain any DeRouter-internal keys.
    ...
    # Reverse map landed on derouter_params (NOT optional_params, NOT body).
    rmap = derouter_params["_anthropic_tool_name_map"]
    assert (
...
def test_transform_request_no_reverse_map_when_all_names_valid():
    """If every name is already valid, ``derouter_params`` stays clean
    ...
    assert "_anthropic_tool_name_map" not in derouter_params
...
# https://github.com/Delify-Solutions/derouter/issues/26692.
# -----------------------------------------------------------------------------
...
def test_translate_system_message_strips_billing_header_for_bedrock():
    from derouter.llms.bedrock.claude_platform.transformation import (
...
def test_anthropic_messages_request_strips_billing_header_for_minimax():
    from derouter.llms.minimax.messages.transformation import MinimaxMessagesConfig
    from derouter.types.router import GenericDeRouterParams
...
@pytest.mark.parametrize(
    ...
    [
        ("derouter.llms.anthropic.chat.transformation", "AnthropicConfig", False),
        (
            "derouter.llms.anthropic.experimental_pass_through.messages.transformation",
            "AnthropicMessagesConfig",
            False,
        ),
        (
            "derouter.llms.bedrock.claude_platform.transformation",
            "BedrockClaudePlatformConfig",
            True,
        ),
        (
            "derouter.llms.bedrock.chat.invoke_transformations.anthropic_claude3_transformation",
            "AmazonAnthropicClaudeConfig",
            True,
        ),
        (
            "derouter.llms.vertex_ai.vertex_ai_partner_models.anthropic.transformation",
            "VertexAIAnthropicConfig",
            True,
        ),
        (
            "derouter.llms.azure_ai.anthropic.transformation",
            "AzureAnthropicConfig",
            True,
        ),
        ("derouter.llms.minimax.messages.transformation", "MinimaxMessagesConfig", True),
        (
            "derouter.llms.azure_ai.anthropic.messages_transformation",
            "AzureAnthropicMessagesConfig",
            True,
        ),
        (
            "derouter.llms.deepseek.messages.transformation",
            "DeepSeekAnthropicMessagesConfig",
            True,
        ),
        (
            "derouter.llms.vertex_ai.vertex_ai_partner_models.anthropic.experimental_pass_through.transformation",
...
def test_sampling_param_gating_driven_by_model_map_flag(monkeypatch):
    ...
    monkeypatch.setitem(
        derouter.model_cost, "claude-zeta-9", {"supports_sampling_params": False}
    )
    monkeypatch.setitem(
        derouter.model_cost, "claude-fable-5-test", {"supports_sampling_params": True}
    )
    config = AnthropicConfig()
...
def test_top_k_dropped_at_transform_for_models_that_removed_it():
    ...
    result = config.transform_request(
        ...
        derouter_params={"drop_params": True},
        headers={},
    )
...
def test_forced_tool_choice_raises_clean_error_on_fable_5_1_without_drop_params(
    ...
    with pytest.raises(derouter.utils.UnsupportedParamsError, match="forced tool use"):
        config.map_openai_params(
...
def test_forced_tool_choice_gating_driven_by_model_map_flag(local_model_cost_map, monkeypatch):
    ...
    monkeypatch.setitem(derouter.model_cost, "claude-zeta-9", {"supports_forced_tool_use": False})
    config = AnthropicConfig()
...
def test_response_format_tool_path_skips_forced_tool_choice_when_unsupported(local_model_cost_map, monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        {"derouter_provider": "anthropic", "mode": "chat", "supports_forced_tool_use": False},
    )
    config = AnthropicConfig()
