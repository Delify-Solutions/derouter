"""
Unit tests for derouter/llms/anthropic/batches/handler.py
...
parsed result is the DeRouterBatch the transform produced.
...
tests/test_derouter/batches/test_main.py.
"""
...
def patched_client():
    ...
    with patch(
        "derouter.llms.anthropic.batches.handler.get_async_httpx_client",
...
def test_retrieve_batch_sync_runs_to_result(handler, patched_client):
    # _is_async=False -> asyncio.run(...) returns the resolved DeRouterBatch.
