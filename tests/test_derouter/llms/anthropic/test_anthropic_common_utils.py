...
def test_is_claude_code_one_shot_subagent_request(messages, system, expected):
    from derouter.llms.anthropic.common_utils import is_claude_code_one_shot_subagent_request
...
class TestValidateEnvironmentOAuth:
    ...
    def test_custom_api_base_via_param(self):
        """validate_environment uses Bearer when use_bearer_for_custom_base is set in derouter_params."""
        from derouter.llms.anthropic.common_utils import AnthropicModelInfo
        ...
        updated_headers = config.validate_environment(
            ...
            messages=[{"role": "user", "content": "Hello"}],
            optional_params={},
            derouter_params={"use_bearer_for_custom_base": True},
    ...
    def test_custom_api_base_via_derouter_params(self):
        """validate_environment uses Bearer when api_base and use_bearer_for_custom_base are in derouter_params."""
        from derouter.llms.anthropic.common_utils import AnthropicModelInfo
        ...
        updated_headers = config.validate_environment(
            ...
            messages=[{"role": "user", "content": "Hello"}],
            optional_params={},
            derouter_params={"api_base": "https://custom-gateway.com", "use_bearer_for_custom_base": True},
            api_key="custom-api-key",
            api_base=None,
        )
...
class TestProxyOAuthHeaderForwarding:
    ...
    def test_clean_headers_preserves_oauth_authorization(self):
        ...
        from derouter.proxy.derouter_pre_call_utils import clean_headers
    ...
    def test_clean_headers_forwards_x_api_key_when_authenticated_with_derouter_key(self):
        """clean_headers should forward x-api-key when user authenticated with x-derouter-api-key and forward_llm_provider_auth_headers=True."""
        from starlette.datastructures import Headers
        ...
        # x-derouter-api-key should be excluded (special header)
        assert "x-derouter-api-key" not in cleaned
        assert cleaned["content-type"] == "application/json"

    def test_clean_headers_excludes_x_api_key_when_used_for_auth(self):
        """clean_headers should exclude x-api-key when it was used for DeRouter authentication."""
        from starlette.datastructures import Headers
        ...
        raw_headers = Headers(
            raw=[
                (b"x-api-key", b"sk-derouter-proxy-key"),
                (b"content-type", b"application/json"),
            ]
        )
        cleaned = clean_headers(raw_headers, authenticated_with_header="x-api-key")

        # x-api-key should be excluded (was used for DeRouter auth)
        assert "x-api-key" not in cleaned
        assert cleaned["content-type"] == "application/json"
    ...
    def test_clean_headers_forwards_x_api_key_when_authenticated_with_authorization(
        ...
        raw_headers = Headers(
            raw=[
                (b"authorization", b"Bearer sk-derouter-proxy-key"),
                (b"x-api-key", b"sk-ant-api03-client-key"),
                (b"content-type", b"application/json"),
            ]
        )
    ...
    def test_clean_headers_forwards_x_api_key_with_forward_flag_and_derouter_auth(
        self,
    ):
    ...
    def test_clean_headers_authorization_not_forwarded_when_used_for_derouter_auth(
        self,
    ):
        """Authorization Bearer (DeRouter key) must never be forwarded to the LLM provider.

        When a user sends their DeRouter key as 'Authorization: Bearer sk-1234' and
        ...
        raw_headers = Headers(
            raw=[
                (b"authorization", b"Bearer sk-1234-derouter-proxy-key"),
                (b"x-api-key", b"sk-ant-api03-real-anthropic-key"),
                (b"content-type", b"application/json"),
            ]
        )
        # Authorization was the header used for DeRouter auth
        cleaned = clean_headers(
            raw_headers,
            forward_llm_provider_auth_headers=True,
            authenticated_with_header="authorization",
    ...
    def test_clean_headers_oauth_authorization_forwarded_when_not_used_for_derouter_auth(
        self,
    ):
        """OAuth Authorization header IS forwarded when x-derouter-api-key was used for proxy auth."""
        ...
        # x-derouter-api-key was used for DeRouter auth; Authorization carries the Anthropic OAuth token
        with patch(
            "derouter.llms.anthropic.common_utils.is_anthropic_oauth_key",
            return_value=True,
        ):
            cleaned = clean_headers(
                raw_headers,
...
class TestPassthroughAuthToken:
    ...
    def test_passthrough_missing_credentials_raises_authentication_error(self):
        ...
        config = AnthropicMessagesConfig()
        with mock_patch.dict("os.environ", {}, clear=True):
            with pytest.raises(derouter.AuthenticationError, match="Missing Anthropic API Key"):
...
class TestAnthropicThinkingSignatureSelfHeal:
    ...
    def test_is_empty_thinking_block(self):
        from derouter.llms.anthropic.common_utils import is_empty_thinking_block
    ...
    def test_is_empty_unsigned_thinking_block(self):
        ...
        from derouter.llms.anthropic.common_utils import is_empty_unsigned_thinking_block
    ...
    def test_flatten_unencrypted_web_search_results_keeps_snippet_evidence(self):
        ...
        assert "Web search results for 'latest derouter version':" in flattened
        assert "URL: https://github.com/Delify-Solutions/derouter/releases" in flattened
    ...
    def test_normalize_anthropic_tool_use_id_strips_thought_signature(self):
        ...
        from derouter.llms.anthropic.common_utils import normalize_anthropic_tool_use_id
...
class TestCapabilityProbeUsesCallerProvider:
    ...
    def test_exact_bedrock_entry_flag_is_authoritative_for_bedrock_caller(self, local_model_cost_map, monkeypatch):
        ...
        monkeypatch.setitem(derouter.model_cost[self.BEDROCK_MODEL], "supports_adaptive_thinking", False)
