...
import derouter
from derouter.anthropic_interface import messages
...
def test_anthropic_experimental_pass_through_messages_handler():
    """
    Test that api key is passed to derouter.responses for OpenAI models.
    OpenAI and Azure models are routed directly to the Responses API.
    """
    from derouter.llms.anthropic.experimental_pass_through.messages.handler import (
        anthropic_messages_handler,
    )

    with patch("derouter.responses", return_value="test-response") as mock_responses:
        try:
            anthropic_messages_handler(
                max_tokens=100,
...
async def test_openai_model_does_not_forward_stream_options_to_responses_api():
    ...
    with patch.object(AsyncHTTPHandler, "post", new_callable=AsyncMock) as mock_post:
        ...
        await derouter.anthropic.messages.acreate(
            max_tokens=100,
            messages=[{"role": "user", "content": "Hello, how are you?"}],
...
def test_anthropic_experimental_pass_through_messages_handler_dynamic_api_key_and_api_base_and_custom_values():
    """
    Test that api key, api base, and extra kwargs are forwarded to derouter.completion for Azure models.
    ...
    with patch("derouter.completion", return_value=MagicMock()) as mock_completion:
        try:
            anthropic_messages_handler(
                max_tokens=100,
                messages=[{"role": "user", "content": "Hello, how are you?"}],
...
async def test_anthropic_messages_sanitizes_empty_text_blocks_before_dispatch():
    ...
    from derouter.llms.anthropic.experimental_pass_through.messages import handler
...
def test_anthropic_experimental_pass_through_messages_handler_custom_llm_provider():
    """
    Test that derouter.completion is called when a custom LLM provider is given.

    Provider resolution now happens exactly once, inside derouter.completion itself
    (Delify-Solutions/derouter#37716), so the handler passes the original unresolved model through.
    """
    from derouter.llms.anthropic.experimental_pass_through.messages.handler import (
        anthropic_messages_handler,
    )

    with patch("derouter.completion", return_value="test-response") as mock_completion:
        try:
            anthropic_messages_handler(
                max_tokens=100,
                messages=[{"role": "user", "content": "Hello, how are you?"}],
        ...
        # Assert that derouter.completion was called when using a custom LLM provider
        mock_completion.assert_called_once()
...
async def test_bedrock_converse_budget_tokens_preserved():
    ...
    """
    # Mock derouter.acompletion which is called internally by anthropic_messages_handler
    ...
    with patch("derouter.acompletion", new_callable=AsyncMock) as mock_acompletion:
...
def test_openai_model_with_thinking_converts_to_reasoning():
    ...
    derouter.responses() is called with `reasoning` properly set.
    """
    from derouter.llms.anthropic.experimental_pass_through.messages.handler import (
        anthropic_messages_handler,
    )
    ...
    with patch("derouter.responses", return_value="test-response") as mock_responses:
        ...
        assert "reasoning" in call_kwargs, "reasoning should be passed to derouter.responses"
        ...
        assert "thinking" not in call_kwargs, "thinking should NOT be passed directly to derouter.responses"
...
class TestThinkingParameterTransformation:
    ...
    def test_claude_model_preserves_thinking_with_budget_tokens(self):
        ...
        from derouter.llms.anthropic.experimental_pass_through.adapters.transformation import (
            DeRouterAnthropicMessagesAdapter,
        )

        thinking = {"type": "enabled", "budget_tokens": 5000}
        result = DeRouterAnthropicMessagesAdapter.translate_thinking_for_model(
            thinking=thinking,
            model="bedrock/converse/us.anthropic.claude-sonnet-4-20250514-v1:0",
        )
    ...
    def test_translate_thinking_for_model_summary_when_enabled(self):
        ...
        original = derouter.reasoning_auto_summary
        try:
            derouter.reasoning_auto_summary = True
            thinking = {"type": "enabled", "budget_tokens": 5000}
            result = DeRouterAnthropicMessagesAdapter.translate_thinking_for_model(
                thinking=thinking,
                model="openai/gpt-5.2",
            ...
            derouter.reasoning_auto_summary = original
...
class TestThinkingSummaryPreservation:
    ...
    def test_thinking_summary_concise_preserved_for_openai(self):
        ...
        from derouter.llms.anthropic.experimental_pass_through.adapters.handler import (
            DeRouterMessagesToCompletionTransformationHandler,
        )

        thinking = {"type": "enabled", "budget_tokens": 5000, "summary": "concise"}
        completion_kwargs = {"model": "openai/gpt-5.1", "reasoning_effort": "medium"}
        DeRouterMessagesToCompletionTransformationHandler._route_openai_thinking_to_responses_api_if_needed(
    ...
    def test_no_summary_by_default_string_reasoning(self):
        ...
        original = derouter.reasoning_auto_summary
        try:
            derouter.reasoning_auto_summary = False
            completion_kwargs = {
                "model": "responses/gpt-5.2",
                "custom_llm_provider": "openai",
    ...
    def test_summary_added_when_env_var_set(self, monkeypatch):
        """When DEROUTER_REASONING_AUTO_SUMMARY env var is true, summary is added."""
        import derouter
        from derouter.llms.anthropic.experimental_pass_through.adapters.handler import (
            DeRouterMessagesToCompletionTransformationHandler,
        )
        ...
        try:
            derouter.reasoning_auto_summary = False
            monkeypatch.setenv("DEROUTER_REASONING_AUTO_SUMMARY", "true")
            completion_kwargs = {
                "model": "responses/gpt-5.2",
                "custom_llm_provider": "openai",
                "reasoning_effort": "high",
            ...
            derouter.reasoning_auto_summary = original
            os.environ.pop("DEROUTER_REASONING_AUTO_SUMMARY", None)
    ...
    def test_responses_adapter_preserves_summary(self):
        ...
        from derouter.llms.anthropic.experimental_pass_through.responses_adapters.transformation import (
            DeRouterAnthropicToResponsesAPIAdapter,
        )

        thinking = {"type": "enabled", "budget_tokens": 5000, "summary": "concise"}
        result = DeRouterAnthropicToResponsesAPIAdapter.translate_thinking_to_reasoning(thinking)
...
def test_handler_skips_strip_when_presanitized():
    ...
    with patch.object(
        ...
        result = handler.anthropic_messages_handler(
            ...
            custom_llm_provider="anthropic",
            mock_response="hi there",
            _derouter_messages_presanitized=True,
        )
...
def test_handler_flattens_replayed_unencrypted_web_search_results():
    ...
    with patch.object(
        ...
        handler.anthropic_messages_handler(
            max_tokens=10,
            messages=[
                {"role": "user", "content": "latest derouter version?"},
                {
                    "role": "assistant",
                    "content": [
                        {
                            ...
                            "id": "srvtoolu_1",
                            "name": "web_search",
                            "input": {"query": "latest derouter version"},
                        },
                        {
                            ...
                            "content": [
                                {
                                    ...
                                    "url": "https://github.com/Delify-Solutions/derouter/releases",
                                    "title": "Releases",
                                    "page_age": None,
                                    "encrypted_content": "",
...
def test_presanitized_flag_not_leaked_to_provider_params():
    ...
    assert "_derouter_messages_presanitized" not in captured.get("optional", {})
    assert "_derouter_messages_presanitized" not in captured.get("kwargs", {})
...
async def test_async_wrapper_sets_presanitized_and_sanitizes_once():
    ...
    def fake_handler(*args, **kwargs):
        captured["messages"] = kwargs.get("messages")
        captured["presanitized"] = kwargs.get("_derouter_messages_presanitized")
        return "stub"
...
def _gate_stubs(monkeypatch):
    ...
    monkeypatch.setattr(
        handler.DeRouterMessagesToResponsesAPIHandler,
        "anthropic_messages_handler",
        staticmethod(fake_translation),
    )
    monkeypatch.setattr(
        handler.DeRouterMessagesToCompletionTransformationHandler,
        "anthropic_messages_handler",
        staticmethod(fake_translation),
    )
...
def test_first_party_claude_4_8_plus_cost_map_entries_carry_mid_conversation_system_flag():
    ...
    cost_map_path = os.path.join(os.path.dirname(derouter.__file__), "model_prices_and_context_window_backup.json")
    ...
    missing = [
        ...
        and info.get("derouter_provider") == "anthropic"
        and "claude" in key
...
async def test_messages_strips_provider_prefix_exactly_once(requested_model, expected_wire_model, expected_url):
    """
    Delify-Solutions/derouter#37716: only the leading provider segment may be stripped on the way upstream.
    ...
    with (
        ...
        pytest.raises(derouter.exceptions.InternalServerError),
    ):
        await derouter.anthropic.messages.acreate(
            max_tokens=100,
...
async def test_messages_streaming_reports_provider_local_model(requested_model, expected_reported_model):
    """
    Delify-Solutions/derouter#37716: the wire keeps every segment, so ``message_start`` must still
    ...
    with patch("derouter.acompletion", new=AsyncMock(return_value=_EmptyStream())):
        stream = await derouter.anthropic.messages.acreate(
            max_tokens=100,
            messages=[{"role": "user", "content": "ping"}],
            model=requested_model,
            api_key="test-api-key",
...
def test_messages_sync_streaming_reports_provider_local_model():
    ...
    with patch("derouter.completion", new=MagicMock(return_value=iter(()))):
        stream = derouter.anthropic.messages.create(
            max_tokens=100,
            messages=[{"role": "user", "content": "ping"}],
...
class _SuccessPayloadCapture(CustomLogger):
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        if kwargs.get("derouter_call_id") == self.tracking_id:
...
def capture_success_payloads(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [capture])
    return capture
...
class TestMessagesStreamingSuccessLogging:
    ...
    async def test_responses_bridge_streaming_emits_success_logging(self, capture_success_payloads):
        ...
        from derouter.llms.anthropic.experimental_pass_through.responses_adapters.handler import (
            DeRouterMessagesToResponsesAPIHandler,
        )
        ...
        with patch(
            ...
            sse_stream = await DeRouterMessagesToResponsesAPIHandler.async_anthropic_messages_handler(
                max_tokens=100,
                messages=self.MESSAGES,
                model="openai/gpt-4o-mini",
                stream=True,
                custom_llm_provider="openai",
                derouter_call_id=capture_success_payloads.tracking_id,
            )
    ...
    async def test_chat_completions_bridge_streaming_emits_success_logging(self, capture_success_payloads):
        ...
        derouter.use_chat_completions_url_for_anthropic_messages. Its router lookup is
        ...
        with patch(
            "derouter.llms.anthropic.experimental_pass_through.adapters.handler._proxy_router_fallback",
            return_value=None,
        ):
            sse_stream = await DeRouterMessagesToCompletionTransformationHandler.async_anthropic_messages_handler(
                max_tokens=100,
                messages=self.MESSAGES,
                model="openai/gpt-4o-mini",
                stream=True,
...
@pytest.mark.parametrize(
    ...
    [
        (401, "authentication_error", derouter.AuthenticationError),
        (403, "permission_error", derouter.PermissionDeniedError),
    ],
)
