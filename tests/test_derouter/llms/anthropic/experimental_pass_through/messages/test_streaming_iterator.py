...
from derouter.llms.anthropic.experimental_pass_through.messages import streaming_iterator as streaming_iterator_module
...
class _RecordingLoggingIterator(BaseAnthropicMessagesStreamingIterator):
    def __init__(self, derouter_logging_obj: DeRouterLoggingObj, request_body: dict):
        super().__init__(derouter_logging_obj=derouter_logging_obj, request_body=request_body)
...
def _make_logging_obj(test_name: str, failure_recorder: _FailureRecorder | None = None) -> DeRouterLoggingObj:
...
def _make_iterator(test_name: str) -> BaseAnthropicMessagesStreamingIterator:
    return BaseAnthropicMessagesStreamingIterator(
        derouter_logging_obj=_make_logging_obj(test_name),
        request_body={},
    )
...
async def test_async_sse_wrapper_excludes_synthetic_error_event_from_logged_chunks():
    ...
    iterator = _RecordingLoggingIterator(
        derouter_logging_obj=_make_logging_obj("test_synthetic_error_not_logged"),
        request_body={},
    )
    chunks = await _collect(iterator, _truncated_stream())
...
async def test_async_sse_wrapper_skips_logging_on_disconnect_before_first_chunk():
    iterator = _RecordingLoggingIterator(
        derouter_logging_obj=_make_logging_obj("test_disconnect_before_first_chunk"),
        request_body={},
    )
...
async def test_async_sse_wrapper_bills_full_stream_after_client_disconnect():
    ...
    iterator = _RecordingLoggingIterator(
        derouter_logging_obj=_make_logging_obj("test_bills_full_stream_after_disconnect"),
        request_body={},
    )
...
async def test_async_sse_wrapper_bills_full_stream_when_client_reads_all():
    ...
    iterator = _RecordingLoggingIterator(
        derouter_logging_obj=_make_logging_obj("test_bills_full_stream_happy_path"),
        request_body={},
    )
...
async def test_async_sse_wrapper_dispatches_deferred_logging_when_client_disconnects_mid_tail():
    ...
    iterator = BaseAnthropicMessagesStreamingIterator(derouter_logging_obj=logging_obj, request_body={})
...
async def test_async_sse_wrapper_reraises_upstream_error_to_connected_client():
    ...
    iterator = _RecordingLoggingIterator(
        derouter_logging_obj=_make_logging_obj("test_reraises_upstream_error", recorder),
        request_body={},
    )
    detached_hook = _DetachedFailureRecorder()
    iterator.derouter_logging_obj._on_detached_stream_failure = detached_hook
...
async def test_async_sse_wrapper_logs_failure_on_upstream_error_after_disconnect():
    ...
    recorder = _FailureRecorder()
    iterator = _RecordingLoggingIterator(
        derouter_logging_obj=_make_logging_obj("test_failure_logged_on_late_error", recorder),
        request_body={},
    )
    detached_hook = _DetachedFailureRecorder()
    iterator.derouter_logging_obj._on_detached_stream_failure = detached_hook
...
async def test_async_sse_wrapper_logs_failure_when_queued_error_is_never_consumed():
    ...
    recorder = _FailureRecorder()
    iterator = _RecordingLoggingIterator(
        derouter_logging_obj=_make_logging_obj("test_failure_logged_on_unconsumed_queued_error", recorder),
        request_body={},
    )
    detached_hook = _DetachedFailureRecorder()
    iterator.derouter_logging_obj._on_detached_stream_failure = detached_hook
...
async def test_async_sse_wrapper_bills_partial_when_detached_drain_cap_reached(monkeypatch):
    ...
    iterator = _RecordingLoggingIterator(derouter_logging_obj=_make_logging_obj("drain_cap_full"), request_body={})
    try:
...
async def test_async_sse_wrapper_bills_partial_when_detached_drains_disabled(monkeypatch):
    ...
    iterator = _RecordingLoggingIterator(derouter_logging_obj=_make_logging_obj("drains_disabled"), request_body={})
    gen = iterator.async_sse_wrapper(_long_stream())
    await gen.__anext__()  # message_start
    await gen.__anext__()  # first delta
    await gen.aclose()  # client disconnects; 100+ chunks remain upstream
...
async def test_async_sse_wrapper_aborts_upstream_when_detached_drain_cap_reached(monkeypatch):
    ...
    iterator = _RecordingLoggingIterator(derouter_logging_obj=_make_logging_obj("abort_upstream_at_cap"), request_body={})
    try:
...
async def test_async_sse_wrapper_drains_detached_when_cap_available(monkeypatch):
    ...
    iterator = _RecordingLoggingIterator(derouter_logging_obj=_make_logging_obj("drain_cap_free"), request_body={})
...
async def test_normal_end_with_deferred_dispatch_armed_parks_logging_coroutine(monkeypatch):
    ...
    iterator.derouter_logging_obj._on_deferred_stream_complete = _noop_deferred_dispatch

    await _collect(iterator, _stream_of(COMPLETE_STREAM_EVENTS))

    parked = getattr(iterator.derouter_logging_obj, "_deferred_stream_complete_args", None)
...
async def test_client_disconnect_enqueues_immediately_even_when_deferred_dispatch_armed(monkeypatch):
    ...
    assert getattr(iterator.derouter_logging_obj, "_deferred_stream_complete_args", None) is None
...
async def test_normal_end_parks_deferred_logging_even_when_sentinel_enqueue_backpressured(monkeypatch):
    ...
    iterator.derouter_logging_obj._on_deferred_stream_complete = _deferred_stream_complete
    ...
    for _ in range(100):
        if worker.enqueued or getattr(iterator.derouter_logging_obj, "_deferred_stream_complete_args", None):
            break
        await asyncio.sleep(0.01)
...
async def test_relay_teardown_dispatches_deferred_billing_when_sentinel_never_consumed(monkeypatch):
    ...
    for _ in range(100):
        if getattr(iterator.derouter_logging_obj, "_deferred_stream_complete_args", None) is not None:
            break
        await asyncio.sleep(0.01)
    ...
    assert getattr(iterator.derouter_logging_obj, "_deferred_stream_complete_args", None) is None
    assert getattr(iterator.derouter_logging_obj, "_on_deferred_stream_complete", None) is None
