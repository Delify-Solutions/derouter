...
import derouter
from derouter.caching.caching import Cache, DeRouterCacheType
...
@pytest.fixture
def local_cache():
    previous_cache = derouter.cache
    derouter.cache = Cache(type=DeRouterCacheType.LOCAL)
    yield derouter.cache
    derouter.cache = previous_cache
...
async def test_non_streaming_request_is_served_from_cache(local_cache, request_kwargs, monkeypatch):
    ...
    first = await derouter.anthropic_messages(**request_kwargs)
    await asyncio.sleep(0)
    second = await derouter.anthropic_messages(**request_kwargs)
...
async def test_cache_key_separates_different_system_prompts(local_cache, request_kwargs, monkeypatch):
    ...
    first = await derouter.anthropic_messages(**request_kwargs, system="Always answer ALPHA")
    await asyncio.sleep(0)
    second = await derouter.anthropic_messages(**request_kwargs, system="Always answer BETA")
...
async def test_cache_key_separates_anthropic_native_params(local_cache, request_kwargs, monkeypatch, anthropic_param):
    ...
    await derouter.anthropic_messages(**request_kwargs)
    await asyncio.sleep(0)
    await derouter.anthropic_messages(**request_kwargs, **anthropic_param)
...
async def test_streaming_request_is_replayed_from_cache(local_cache, request_kwargs, monkeypatch):
    ...
    first = await _collect(await derouter.anthropic_messages(**request_kwargs, stream=True))
    second_stream = await derouter.anthropic_messages(**request_kwargs, stream=True)
...
async def test_streaming_cache_is_not_shared_with_non_streaming(local_cache, request_kwargs, monkeypatch):
    ...
    await _collect(await derouter.anthropic_messages(**request_kwargs, stream=True))
    non_streaming = await derouter.anthropic_messages(**request_kwargs)
...
async def test_failed_stream_is_not_cached(local_cache, request_kwargs, monkeypatch):
    ...
    failed = await _collect(await derouter.anthropic_messages(**request_kwargs, stream=True))
    replayed = await _collect(await derouter.anthropic_messages(**request_kwargs, stream=True))
...
async def test_multibyte_utf8_split_across_chunks_streams_and_caches(local_cache, request_kwargs, monkeypatch):
    ...
    first = await _collect(await derouter.anthropic_messages(**request_kwargs, stream=True))
    second = await _collect(await derouter.anthropic_messages(**request_kwargs, stream=True))
...
async def test_abandoned_stream_is_not_cached(local_cache, request_kwargs, monkeypatch):
    ...
    partial_stream = await derouter.anthropic_messages(**request_kwargs, stream=True)
