...
MCP_REFERENCE = {
    ...
    "server_url": "derouter_proxy/mcp/deepwiki",
    "require_approval": "never",
}


def test_anthropic_messages_handler_routes_derouter_proxy_mcp_to_the_gateway():
    """
    Regression test (LIT-4517): /v1/messages must expand a derouter_proxy MCP
    reference through the MCP gateway.

    Given: A /v1/messages request whose tools carry a derouter_proxy MCP reference
    ...
    """
    with patch(
        "derouter.llms.anthropic.experimental_pass_through.messages.mcp_handler.anthropic_messages_with_mcp",
    ...
    assert routed.called, "A derouter_proxy MCP reference must be dispatched to the MCP gateway"
...
def test_anthropic_messages_handler_leaves_native_tools_alone():
    ...
    assert not routed.called, "Only derouter_proxy MCP references belong to the gateway"
...
async def test_anthropic_messages_with_mcp_forwards_the_callers_mcp_credentials():
    ...
    """
    from derouter.llms.anthropic.experimental_pass_through.messages import mcp_handler
    ...
    with patch.object(MCPRequestContext, "resolve", return_value=context), patch.object(
        mcp_handler.DeRouter_Proxy_MCP_Handler
        if hasattr(mcp_handler, "DeRouter_Proxy_MCP_Handler")
        else __import__(
            "derouter.responses.mcp.derouter_proxy_mcp_handler", fromlist=["DeRouter_Proxy_MCP_Handler"]
        ).DeRouter_Proxy_MCP_Handler,
        "_process_mcp_tools_without_openai_transform",
        new=process,
    ), patch.object(
        import_module("derouter.responses.mcp.derouter_proxy_mcp_handler").DeRouter_Proxy_MCP_Handler, "_execute_tool_calls",
        new=execute,
    ), patch(
        "derouter.anthropic_messages", new=AsyncMock(side_effect=responses)
    ):
    ...
    assert listing["derouter_trace_id"] == "trace-123"
    ...
    assert execution["derouter_call_id"] == "call-456"
    assert execution["derouter_trace_id"] == "trace-123"
...
async def test_anthropic_messages_with_mcp_stops_when_every_tool_call_is_skipped():
    ...
    with patch.object(
        ...
        import_module("derouter.responses.mcp.derouter_proxy_mcp_handler").DeRouter_Proxy_MCP_Handler, "_process_mcp_tools_without_openai_transform",
        new=AsyncMock(return_value=([], {})),
    ), patch.object(
        import_module("derouter.responses.mcp.derouter_proxy_mcp_handler").DeRouter_Proxy_MCP_Handler, "_execute_tool_calls",
        new=AsyncMock(return_value=[]),
    ), patch(
        "derouter.anthropic_messages", new=anthropic_messages_mock
    ):
