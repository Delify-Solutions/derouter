...
When reasoning_auto_summary is enabled (via derouter.reasoning_auto_summary or
DEROUTER_REASONING_AUTO_SUMMARY env var), the handler injects
...
def _call_handler_and_capture_optional_params(thinking=None, **extra_kwargs):
    ...
    with patch(
        "derouter.llms.anthropic.experimental_pass_through.messages.handler."
...
class TestReasoningAutoSummaryMessages:
    ...
    def test_adaptive_thinking_gets_display_summarized(self):
        ...
        with patch.object(derouter, "reasoning_auto_summary", True):
    ...
    def test_no_injection_when_flag_false(self):
        ...
        with patch.object(derouter, "reasoning_auto_summary", False):
            params = _call_handler_and_capture_optional_params(
                thinking={"type": "enabled", "budget_tokens": 10000}
            )
        thinking = params.get("thinking", {})
    ...
    def test_env_var_enables_auto_summary(self):
        """DEROUTER_REASONING_AUTO_SUMMARY=true env var enables the feature."""
        with patch.object(derouter, "reasoning_auto_summary", False), patch.dict(
            os.environ, {"DEROUTER_REASONING_AUTO_SUMMARY": "true"}
        ):
            params = _call_handler_and_capture_optional_params(
                thinking={"type": "adaptive", "budget_tokens": 5000}
            )
