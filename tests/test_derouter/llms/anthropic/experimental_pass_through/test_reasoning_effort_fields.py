...
class TestModelRegistryReasoningEffortFields:
    ...
    low/medium/high/xhigh/max), so DeRouter degrades ``minimal`` to ``low``
...
class TestAdapterAdaptiveThinking:
    ...
    def test_responses_adapter_adaptive_with_output_config(self):
        ...
        result = DeRouterAnthropicToResponsesAPIAdapter.translate_thinking_to_reasoning(
            thinking={"type": "adaptive"},
...
def declared_effort_entry(local_model_cost_map, request):
    ...
    derouter.model_cost[key] = {
        "derouter_provider": "synthetic",
        "mode": "chat",
