...
async def test_streaming_message_start_reports_the_provider_local_model(requested_model, expected_reported_model):
    """
    Delify-Solutions/derouter#37716 sends the caller's unresolved id down this bridge so the provider
    ...
    with patch.object(derouter, "aresponses", AsyncMock(return_value=empty_stream())):
        sse = await DeRouterMessagesToResponsesAPIHandler.async_anthropic_messages_handler(
            max_tokens=1024,
            messages=MESSAGES,
...
async def test_streaming_hands_the_logging_object_the_message_id_the_caller_is_streamed(
    ...
    logging_obj = Logging(
        ...
        derouter_call_id="6825beef-0000-4000-8000-000000000003",
        function_id="1234",
    )
