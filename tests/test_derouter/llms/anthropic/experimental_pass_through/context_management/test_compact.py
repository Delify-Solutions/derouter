...
import derouter
from derouter.llms.anthropic.experimental_pass_through.context_management import (
    AnthropicContextManagementError,
    apply_context_management,
)
from derouter.llms.anthropic.experimental_pass_through.context_management.editors.compact import (
...
async def test_trigger_at_minimum_does_not_raise():
    ...
    with patch(
        "derouter.llms.anthropic.experimental_pass_through.context_management.editors.compact._read_summary_model_setting",
        return_value=None,
    ):
...
async def test_slice_only_path_with_existing_compaction_block():
    ...
    with (
        ...
        patch("derouter.token_counter", return_value=500),  # well under threshold
    ):
        result = await apply_compact_20260112(
            model=MODEL,
            messages=messages,
...
async def test_slice_only_no_compaction_block_under_threshold():
    ...
    with (
        ...
        patch("derouter.token_counter", return_value=500),
    ):
        result = await apply_compact_20260112(
            model=MODEL,
            messages=messages,
...
async def test_full_summary_path():
    ...
    with (
        ...
        patch("derouter.token_counter", return_value=200_000),  # over 150k threshold
        patch(
            "derouter.llms.anthropic.experimental_pass_through.context_management.editors.compact._call_summary_model",
...
async def test_full_summary_path_uses_router_when_available():
    """When llm_router is provided, its acompletion method is called instead of derouter."""
    messages = _simple_messages()
    ...
    with (
        ...
        patch("derouter.token_counter", return_value=200_000),
    ):
        result = await apply_compact_20260112(
            model=MODEL,
            messages=messages,
...
async def test_derouter_metadata_propagated_to_summary_call():
    """Auth fields from the proxy ``derouter_metadata`` are forwarded to the summary call."""
    messages = _simple_messages()
    mock_response = _make_mock_response("<summary>Summary</summary>")
    parent_derouter_metadata = {
        "user_api_key": "sk-test",
        "user_api_key_team_id": "team-123",
        "user_api_key_user_id": "user-456",
        "derouter_call_id": "call-789",
    ...
    with (
        ...
        await apply_compact_20260112(
            ...
            system=None,
            edit_spec=_EDIT_SPEC_DEFAULT,
            derouter_metadata=parent_derouter_metadata,
        )
...
async def test_summary_call_sends_default_max_tokens():
    ...
    ``summary_call_failed``.
    """
    from derouter.llms.anthropic.experimental_pass_through.context_management.constants import (
...
async def test_summary_model_denied_when_user_scope_excludes_it():
    ...
    with (
        ...
        patch(
            "derouter.proxy.auth.auth_checks.get_project_object",
            AsyncMock(return_value=None),
        ),
...
async def test_summary_model_denied_when_key_over_model_budget():
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.model_max_budget_limiter", limiter),
    ):
        result = await apply_compact_20260112(
            model=MODEL,
            messages=messages,
...
async def test_run_polyfill_runs_when_derouter_drop_params_true(monkeypatch):
    """drop_params must not disable the polyfill: context_management is a
    DeRouter-supported param (polyfilled where not native), and drop_params only
...
async def _call_async_adapter_handler(**handler_kwargs: Any):
    ...
    with patch("derouter.acompletion", side_effect=_capture_acompletion):
        response = await DeRouterMessagesToCompletionTransformationHandler.async_anthropic_messages_handler(
            max_tokens=128,
            messages=_tool_use_messages(),
            model=MODEL,
            context_management=_CLEAR_TOOL_USES_SPEC,
            derouter_router=MagicMock(),
            **handler_kwargs,
        )
...
async def test_async_handler_runs_polyfill_when_derouter_drop_params_true(monkeypatch):
    """Regression (LIT-3768): proxy-wide derouter.drop_params=True silently
...
def _call_sync_adapter_handler(**handler_kwargs: Any):
    ...
    with patch("derouter.completion", side_effect=_capture_completion):
        response = DeRouterMessagesToCompletionTransformationHandler.anthropic_messages_handler(
            max_tokens=128,
            messages=_tool_use_messages(),
            model=MODEL,
            context_management=_CLEAR_TOOL_USES_SPEC,
            derouter_router=None,
            **handler_kwargs,
        )
    return response, captured
...
def test_sync_handler_runs_polyfill_when_derouter_drop_params_true(monkeypatch):
    """Proxy-wide derouter.drop_params=True must not skip the polyfill on the
...
async def test_prepare_context_managed_request_forwards_proxy_derouter_metadata():
    """The handler must hand the polyfill the proxy ``derouter_metadata`` (which
    ...
    class _RouterStub:
        async def acompletion(self, **kwargs):
            captured_summary_metadata.update(kwargs.get("derouter_metadata", {}))
    ...
    assert captured_summary_metadata.get("derouter_call_id") == "call-1"
...
def test_anthropic_context_management_error_format():
    ...
    from derouter.anthropic_interface.exceptions import AnthropicExceptionMapping
...
def test_endpoint_returns_anthropic_400_on_context_management_error():
    ...
    from derouter.proxy.anthropic_endpoints.endpoints import router
    ...
    with patch.dict(sys.modules, {"derouter.proxy.proxy_server": mock_proxy_server}):
        with patch(
            "derouter.proxy.anthropic_endpoints.endpoints.ProxyBaseLLMRequestProcessing"
        ) as mock_cls:
