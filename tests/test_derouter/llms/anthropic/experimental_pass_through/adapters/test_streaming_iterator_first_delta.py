...
When DeRouter proxies ``client -> /v1/messages -> /v1/chat/completions`` and a
...
def test_streaming_chat_refusal_parked_in_provider_specific_fields_is_emitted():
    ...
    them) hand DeRouter the refusal as an unrecognized field, which lands in
...
def _full_snapshot_signature_chunks() -> List[MagicMock]:
    """Mirror derouter's real Anthropic streaming: incremental ``thinking_delta``
...
def test_mixed_chunk_with_both_reasoning_fields_keeps_text_sync():
    """DeRouter bridges often set ``reasoning_content`` AND ``thinking_blocks``
