...
from derouter.types.llms.openai import ChatCompletionAssistantToolCall
...
def test_translate_chat_refusal_to_anthropic_response():
    ...
    result = DeRouterAnthropicMessagesAdapter().translate_openai_response_to_anthropic(response)
...
def test_translate_streaming_openai_chunk_to_anthropic_content_block():
    ...
    (
        block_type,
        content_block_start,
    ) = DeRouterAnthropicMessagesAdapter()._translate_streaming_openai_chunk_to_anthropic_content_block(
        choices=choices
    )
...
def test_translate_anthropic_messages_to_openai_thinking_blocks():
    ...
    adapter = DeRouterAnthropicMessagesAdapter()
...
def test_translate_anthropic_messages_to_openai_sets_reasoning_content():
    ...
    result = DeRouterAnthropicMessagesAdapter().translate_anthropic_messages_to_openai(messages=anthropic_messages)
...
def test_translate_anthropic_messages_to_openai_preserves_midturn_system_correction(
    ...
    result = DeRouterAnthropicMessagesAdapter().translate_anthropic_messages_to_openai(
        messages=messages,
...
def test_translate_anthropic_to_openai_orders_top_level_and_midturn_system():
    ...
    """
    openai_request, _ = DeRouterAnthropicMessagesAdapter().translate_anthropic_to_openai(
...
def test_translate_anthropic_to_openai_gives_each_claude_code_session_its_own_prompt_cache_key():
    """Delify-Solutions/derouter#39145: the first 64 chars of Claude Code's user_id are the per-install device_id."""
    keys = tuple(
...
def test_translate_anthropic_to_openai_skips_prompt_cache_key_for_chained_derouter_proxy():
    assert "prompt_cache_key" in derouter.get_supported_openai_params(
        model="xai", custom_llm_provider="derouter_proxy"
    )
    openai_request = _translate_with_metadata("derouter_proxy/xai", {"user_id": CLAUDE_CODE_USER_ID}, "derouter_proxy")
    assert openai_request["user"] == CLAUDE_CODE_USER_ID
    assert "prompt_cache_key" not in openai_request
...
def test_translate_streaming_openai_chunk_to_anthropic_with_partial_json():
    ...
    (
        type_of_content,
        content_block_delta,
    ) = DeRouterAnthropicMessagesAdapter()._translate_streaming_openai_chunk_to_anthropic(
        choices=choices
    )
...
def test_translate_streaming_openai_chunk_to_anthropic_emits_signature_when_thinking_and_signature():
    ...
    derouter's Anthropic streaming handler emits the ``signature_delta`` event as an
...
def test_streaming_chunk_with_both_text_and_tool_calls_issue_18238():
    ...
    choices = [
        StreamingChoices(
            ...
            delta=Delta(
                provider_specific_fields=None,
                content="Here is some text for derouter",
                role=None,
                function_call=None,
                tool_calls=[
                    ChatCompletionDeltaToolCall(
...
def test_is_bedrock_arn_model(model, expected):
    ...
    assert DeRouterAnthropicMessagesAdapter.is_bedrock_arn_model(model) is expected


def test_cache_control_preserved_for_bedrock_arn_inference_profile():
    """
    Regression for https://github.com/Delify-Solutions/derouter/issues/26625
...
def test_cache_control_fix_does_not_broaden_claude_detection():
    ...
    """
    assert (
        DeRouterAnthropicMessagesAdapter.is_anthropic_claude_model(
            CACHE_CONTROL_BEDROCK_ARN_MODEL
        )
        is False
    )
...
def test_thinking_preserved_for_bedrock_arn_inference_profile():
    ...
    assert DeRouterAnthropicMessagesAdapter.translate_thinking_for_model(thinking, CACHE_CONTROL_BEDROCK_ARN_MODEL) == {
        "thinking": thinking
    }
...
def test_adaptive_thinking_output_config_effort_preserved_for_claude_model(model):
    ...
    """
    from derouter.types.llms.anthropic import AnthropicMessagesRequest
...
def test_translate_anthropic_tools_to_openai_fills_missing_tool_name():
    ...
    assert result[0]["function"]["name"] == "derouter_unnamed_tool_0"
    assert result[1]["function"]["name"] == "derouter_unnamed_tool_1"
...
def test_translate_anthropic_tools_to_openai_passes_openai_function_tools_through():
    """A tool already in OpenAI function format must pass through unchanged instead of becoming derouter_unnamed_tool_N."""
    openai_tool = {
        "type": "function",
        "function": {
            "name": "get_weather",
...
def test_translate_openai_response_to_anthropic_input_tokens_excludes_cached_tokens():
    ...
    """
    from derouter.types.utils import PromptTokensDetailsWrapper
...
def test_translate_openai_usage_to_anthropic_cache_tokens_from_dict_details_with_integral_floats():
    ...
    anthropic_usage = DeRouterAnthropicMessagesAdapter._translate_openai_usage_to_anthropic_usage_delta(
        usage
    )
...
class TestTranslateAnthropicOutputFormatToOpenAI:
    ...
    def setup_method(self):
        self.adapter = DeRouterAnthropicMessagesAdapter()
...
class TestAnthropicStreamWrapperToolArgs:
    """
    Regression test for https://github.com/Delify-Solutions/derouter/issues/24134
    ...
    def _make_stream_wrapper(self, chunks):
        from derouter.llms.anthropic.experimental_pass_through.adapters.streaming_iterator import (
...
def test_translate_openai_response_to_anthropic_with_polyfill_compaction_block():
    ...
    from derouter.llms.anthropic.experimental_pass_through.context_management.result import (
        PolyfillResult,
    )
...
def _run_chat_completions_pipeline(anthropic_messages):
    ...
    request = OpenAIGPTConfig().transform_request(
        model="gpt-5.4-mini", messages=translated, optional_params={}, derouter_params={}, headers={}
    )
...
def test_translate_openai_response_to_anthropic_maps_gemini_web_search_usage():
    ...
    anthropic_response = DeRouterAnthropicMessagesAdapter().translate_openai_response_to_anthropic(
...
def test_translate_openai_response_to_anthropic_maps_server_tool_use_web_search_usage():
    from derouter.types.utils import ServerToolUse
...
def test_completion_cost_on_translated_anthropic_response_includes_web_search():
    ...
    cost_with_search = derouter.completion_cost(
        completion_response=with_search,
        model="gemini/gemini-3-flash-preview",
        call_type="anthropic_messages",
    )
    cost_without_search = derouter.completion_cost(
        completion_response=without_search,
        model="gemini/gemini-3-flash-preview",
        call_type="anthropic_messages",
    )

    per_query_cost = derouter.model_cost["gemini/gemini-3-flash-preview"]["search_context_cost_per_query"][
...
def test_a_databricks_target_trades_its_thinking_display_for_the_tier():
    ...
    silent. It only takes `output_config` when derouter sends one, which this bridge cannot do for a
...
def test_a_chained_derouter_proxy_target_still_takes_the_tier():
    ...
    openai_request, _ = adapter.translate_anthropic_to_openai(
        anthropic_message_request=AnthropicMessagesRequest(
            model="derouter_proxy/claude-sonnet-4-6",
            max_tokens=1024,
            messages=[{"role": "user", "content": "hi"}],
            thinking={"type": "adaptive"},
            output_config={"effort": "max"},
