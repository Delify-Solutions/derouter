...
def _prepare(model: str, extra_kwargs: dict[str, object], thinking: dict[str, object] | None = None):
    completion_kwargs, _ = DeRouterMessagesToCompletionTransformationHandler._prepare_completion_kwargs(
        max_tokens=1024,
        messages=MESSAGES,
        model=model,
...
def test_prepare_completion_kwargs_skips_prompt_cache_key_for_chained_derouter_proxy():
    completion_kwargs = _prepare("derouter_proxy/xai", {"custom_llm_provider": "derouter_proxy"})
    assert completion_kwargs["user"] == CLAUDE_CODE_USER_ID
    assert "prompt_cache_key" not in completion_kwargs
