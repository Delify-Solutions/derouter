...
def test_can_handle_edge_cases():
    from derouter.llms.anthropic.experimental_pass_through.messages.interceptors.advisor import (
...
async def test_loop_no_advisor_call():
    ...
    with patch(
        "derouter.llms.anthropic.experimental_pass_through.messages.interceptors.advisor._call_messages_handler",
...
async def test_max_uses_none_falls_back_to_default():
    ...
    import derouter.constants as _c
    from derouter.llms.anthropic.experimental_pass_through.messages.interceptors.advisor import (
        AdvisorMaxIterationsError,
        AdvisorOrchestrationHandler,
    )
...
async def _run_advisor_and_capture_subcall_kwargs():
    ...
    with (
        ...
        patch(
            "derouter.llms.anthropic.experimental_pass_through.messages.interceptors.advisor.validate_url",
        ),
    ):
        h = AdvisorOrchestrationHandler()
        await h.handle(
...
async def test_advisor_creds_dropped_when_proxy_opt_in_disabled():
    ...
    with patch(
        "derouter.llms.anthropic.experimental_pass_through.messages.interceptors.advisor._allow_client_side_advisor_credentials",
        return_value=False,
    ):
...
def _fake_proxy_server(general_settings: Dict):
    """A stand-in derouter.proxy.proxy_server module exposing general_settings.

    The real proxy_server pulls in heavy optional deps that may be absent in a
    unit-test environment, so the gate's
    ``from derouter.proxy.proxy_server import general_settings`` is satisfied by
    injecting this lightweight module into sys.modules.
    """
    import types

    module = types.ModuleType("derouter.proxy.proxy_server")
...
def test_allow_client_side_advisor_credentials_reads_proxy_flag():
    ...
    for settings, expected in cases:
        with patch.dict(
            sys.modules,
            {"derouter.proxy.proxy_server": _fake_proxy_server(settings)},
        ):
...
def test_allow_client_side_advisor_credentials_defaults_true_outside_proxy():
    ...
    def _blocked_import(name, *args, **kwargs):
        if name == "derouter.proxy.proxy_server":
            raise ImportError("proxy server unavailable")
        return real_import(name, *args, **kwargs)

    with patch.dict(sys.modules):
        sys.modules.pop("derouter.proxy.proxy_server", None)
...
def test_advisor_gate_propagates_non_import_errors():
    ...
    from derouter.llms.anthropic.experimental_pass_through.messages.interceptors import (
        advisor,
    )

    original = sys.modules.get("derouter.proxy.proxy_server")

    class _Broken:
        def __getattr__(self, _name):
            raise RuntimeError("partial proxy boot")

    sys.modules["derouter.proxy.proxy_server"] = _Broken()
    try:
        ...
        if original is None:
            ...
            sys.modules["derouter.proxy.proxy_server"] = original
...
async def test_advisor_ignores_tool_credentials_when_clientside_disabled():
    ...
    with patch.dict(
        sys.modules,
        {
            "derouter.proxy.proxy_server": _fake_proxy_server(
...
def test_resolve_advisor_credentials_skips_validation_when_url_validation_disabled():
    ...
    with (
        ...
        patch.object(derouter, "user_url_validation", False),
        patch(
            "derouter.llms.anthropic.experimental_pass_through.messages.interceptors.advisor.validate_url",
...
def test_resolve_advisor_credentials_rejects_api_base_when_ssl_verify_disabled():
    ...
    with (
        ...
        patch.object(derouter, "ssl_verify", False),
    ):
...
async def test_advisor_sub_call_failure_is_tagged():
    ...
    from derouter.router_utils.cooldown_handlers import is_advisor_orchestration_failure
    ...
    async def mock_call(model, messages, tools, stream, max_tokens, **kwargs):
        ...
        raise derouter.AuthenticationError(  # advisor sub-call: 401
...
async def test_executor_failure_is_not_tagged():
    ...
    async def mock_call(model, messages, tools, stream, max_tokens, **kwargs):
        raise derouter.AuthenticationError(  # executor (first call) fails
...
def _router_with_advisor_deployment(
    ...
    The recorder replaces ``derouter.anthropic_messages`` before construction
    ...
    with patch("derouter.anthropic_messages", new=recorder):
        return Router(
            model_list=[
                {
