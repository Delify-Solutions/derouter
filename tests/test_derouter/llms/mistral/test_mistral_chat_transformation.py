...
from derouter.llms.mistral.chat.transformation import (
...
class TestMistralStripsOutputOnlyFields:
    ...
    DeRouter attaches ``reasoning_content`` / ``thinking_blocks`` to assistant
    responses, so replaying an assistant turn verbatim must not forward them.
    Regression for https://github.com/Delify-Solutions/derouter/issues/30835.
    """
...
def test_mistral_transform_request_hoists_tool_message_image():
    ...
    request = MistralConfig().transform_request(
        model="mistral-medium-2508", messages=messages, optional_params={}, derouter_params={}, headers={}
    )
