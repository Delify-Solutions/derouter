"""
Cost tests for Mistral OCR models against the real derouter cost map
...
from derouter.llms.base_llm.ocr.transformation import OCRPage, OCRResponse, OCRUsageInfo
...
BACKUP_COST_MAP = REPO_ROOT / "derouter" / "model_prices_and_context_window_backup.json"
...
def test_model_info_ocr4_price(model: str) -> None:
    info = derouter.get_model_info(model=f"mistral/{model}", custom_llm_provider="mistral")
...
def test_ocr3_pricing_entry(cost_map_path: Path) -> None:
    ...
    assert info["derouter_provider"] == "mistral"
...
def test_ocr3_model_info_price(local_model_cost_map) -> None:
    info = derouter.get_model_info(model=OCR3_MODEL, custom_llm_provider="mistral")
    assert info["ocr_cost_per_page"] == OCR3_COST_PER_PAGE
...
def test_azure_doc_ai_annotation_pages_fall_back_to_ocr_rate(local_model_cost_map) -> None:
    info = derouter.get_model_info(model=AZURE_DOC_AI_MODEL, custom_llm_provider="azure_ai")
...
def test_azure_ocr4_bills_ocr_and_annotation_pages_at_their_own_rates(local_model_cost_map) -> None:
    info = derouter.get_model_info(model="azure_ai/mistral-ocr-4-0", custom_llm_provider="azure_ai")
    assert info["ocr_cost_per_page"] == OCR4_COST_PER_PAGE
