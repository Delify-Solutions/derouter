...
from derouter.llms.openai.common_utils import BaseOpenAILLM, is_openai_backed_api_base
...
async def test_openai_client_reuse(function_name, is_async, args):
    ...
    client_path = (
        "derouter.llms.openai.openai.AsyncOpenAI"
        if is_async
        else "derouter.llms.openai.openai.OpenAI"
    )
...
def test_precomputed_init_params_match_inspect_signature():
    ...
    from derouter.llms.openai.common_utils import (
...
def test_evicting_a_client_built_on_the_callers_session_leaves_that_session_open(monkeypatch):
    """`derouter.aclient_session` belongs to the caller, who goes on using it.

    `_get_async_http_client` hands that session straight back, so the SDK client
    derouter builds around it is only a wrapper. The SDK's `close()` closes
    whatever http client it was given, so treating the wrapper as derouter's to
...
def test_a_client_derouter_built_its_own_http_client_for_is_still_closed(monkeypatch):
...
def test_mapped_response_still_bills_the_prompt_the_provider_processed(provider):
    ...
    assert response.usage.completion_tokens == 0
    assert derouter.completion_cost(completion_response=response) > 0
...
async def test_async_streaming_output_limit_400_maps_to_length_truncated_stream(provider):
    stream = await derouter.acompletion(
...
async def test_async_genuine_bad_request_still_raises(provider, stream):
    async def _call_and_drain():
        result = await derouter.acompletion(
