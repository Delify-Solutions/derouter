"""
Tests that data_residency is correctly populated on the derouter logging
object's derouter_params for OpenAI Responses paths, even when
...
def _capture_logging_obj():
    ...
    real_init = derouter.Logging.__init__
...
def test_responses_eu_api_base_sets_data_residency():
    ...
    up on the logging object's derouter_params so the cost calculator can apply
    the regional uplift."""
    ...
    with (
        ...
        patch.object(derouter.Logging, "__init__", init_spy),
    ):
        derouter.responses(
            model="gpt-4.1",
            input="hi",
    ...
    assert logging_obj.derouter_params.get("data_residency") == "eu"
...
def test_responses_us_api_base_sets_data_residency():
    ...
    logging_obj = captured["logging_obj"]
    assert logging_obj.derouter_params.get("data_residency") == "us"
...
def test_responses_global_api_base_leaves_data_residency_none():
    ...
    logging_obj = captured["logging_obj"]
    assert logging_obj.derouter_params.get("data_residency") is None
