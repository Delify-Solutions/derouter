...
from derouter.llms.azure.responses.transformation import AzureOpenAIResponsesAPIConfig
...
class TestOpenAIResponsesAPIConfig:
    ...
    def test_validate_environment(self):
        ...
        derouter_params = GenericDeRouterParams(api_key=api_key)
        result = self.config.validate_environment(
            headers=headers, model=self.model, derouter_params=derouter_params
        )
        ...
        with patch("derouter.api_key", "derouter_api_key"):
            derouter_params = GenericDeRouterParams()
            result = self.config.validate_environment(
                headers=headers, model=self.model, derouter_params=derouter_params
            )
            ...
            assert "Authorization" in result
            assert result["Authorization"] == "Bearer derouter_api_key"

        # Test with existing headers
        headers = {"Content-Type": "application/json"}

        with patch("derouter.openai_key", "openai_key"):
            with patch("derouter.api_key", None):
                derouter_params = GenericDeRouterParams()
                result = self.config.validate_environment(
                    headers=headers, model=self.model, derouter_params=derouter_params
                )
        ...
        with patch("derouter.api_key", None):
            with patch("derouter.openai_key", None):
                with patch(
                    "derouter.llms.openai.responses.transformation.get_secret_str",
    ...
    def test_get_complete_url(self):
        ...
        # Test with derouter.api_base
        with patch("derouter.api_base", "https://derouter-api-base.example.com/v1"):
            result = self.config.get_complete_url(
                api_base=None,
                derouter_params={},
            )

            assert result == "https://derouter-api-base.example.com/v1/responses"

        # Test with environment variable
        with patch("derouter.api_base", None):
            with patch(
                "derouter.llms.openai.responses.transformation.get_secret_str",
    ...
    def test_get_event_model_class_generic_event(self):
        ...
        from derouter.types.llms.openai import GenericEvent
    ...
    def test_transform_streaming_response_coalesces_null_error_code(self):
        ...
        """
        from derouter.types.llms.openai import ErrorEvent
    ...
    def test_transform_streaming_response_missing_required_fields_response_created(
        ...
        Reproduces https://github.com/Delify-Solutions/derouter/issues/20570
        """
        from derouter.types.llms.openai import ResponseCreatedEvent
    ...
    def test_transform_streaming_response_missing_required_fields_output_text_delta(
        ...
        Reproduces https://github.com/Delify-Solutions/derouter/issues/20570
        """
        from derouter.types.llms.openai import OutputTextDeltaEvent
    ...
    def test_transform_streaming_response_missing_required_fields_content_part_added(
        ...
        Reproduces https://github.com/Delify-Solutions/derouter/issues/20570
        """
        from derouter.types.llms.openai import ContentPartAddedEvent
...
class TestAzureResponsesAPIConfig:
    ...
    def test_azure_get_complete_url_with_version_types(self):
        ...
        base_url = "https://derouter8397336933.openai.azure.com"

        # Test with preview version - should use openai/v1/responses
        result_preview = self.config.get_complete_url(
            api_base=base_url,
            derouter_params={"api_version": "preview"},
        )
        assert (
            result_preview
            == "https://derouter8397336933.openai.azure.com/openai/v1/responses?api-version=preview"
        )

        # Test with latest version - should use openai/v1/responses
        result_latest = self.config.get_complete_url(
            api_base=base_url,
            derouter_params={"api_version": "latest"},
        )
        assert (
            result_latest
            == "https://derouter8397336933.openai.azure.com/openai/v1/responses?api-version=latest"
        )

        # Test with date-based version - should use openai/responses
        result_date = self.config.get_complete_url(
            api_base=base_url,
            derouter_params={"api_version": "2025-01-01"},
        )
        assert (
            result_date
            == "https://derouter8397336933.openai.azure.com/openai/responses?api-version=2025-01-01"
        )
...
class TestTransformListInputItemsRequest:
    ...
    def setup_method(self):
        ...
        self.derouter_params = GenericDeRouterParams()
    ...
    @patch("derouter.router.Router")
    def test_mock_derouter_router_with_transform_list_input_items_request(
        self, mock_router
    ):
        """Mock test using derouter.router for transform_list_input_items_request"""
        # Setup mock router
    ...
    @patch("derouter.list_input_items")
    def test_mock_derouter_list_input_items_integration(self, mock_list_input_items):
        """Test integration with derouter.list_input_items function"""
        # Setup mock response
        mock_response = {
            "object": "list",
            "data": [
...
class TestPhaseParameter:
    ...
    def _make_output_text(text: str):
        from derouter.types.responses.main import OutputText
    ...
    def test_generic_response_output_item_accepts_phase_commentary(self):
        from derouter.types.responses.main import GenericResponseOutputItem
...
class TestFlattenToolSchemaCombinatorsWiring:
    ...
    def test_non_openai_subclass_does_not_flatten(self):
        from derouter.llms.hosted_vllm.responses.transformation import HostedVLLMResponsesAPIConfig
...
class TestReasoningFollowsModelSupport:
    ...
    def test_a_cost_map_older_than_this_release_never_strips_a_known_reasoning_model(
        ...
        lagging = {
            ...
            for name, entry in derouter.model_cost.items()
        }
        monkeypatch.setattr(derouter, "model_cost", lagging)
        mapped = OpenAIResponsesAPIConfig().map_openai_params(
    ...
    def test_without_drop_params_the_error_is_derouters_400(self, local_model_cost_map, monkeypatch):
    ...
    def test_reasoning_without_an_effort_passes_through_on_non_reasoning_models(
        ...
        monkeypatch.setattr(derouter, "drop_params", drop_params)
        mapped = OpenAIResponsesAPIConfig().map_openai_params(
    ...
    def test_an_explicit_supports_reasoning_false_beats_the_bundled_floor(self, local_model_cost_map, monkeypatch):
        ...
        monkeypatch.setattr(derouter, "model_cost", overridden)
        mapped = OpenAIResponsesAPIConfig().map_openai_params(
            response_api_optional_params={"reasoning": {"effort": "medium"}},
            model="o3",
            drop_params=True,
