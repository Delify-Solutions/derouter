...
from derouter.llms.azure.realtime.http_transformation import AzureRealtimeHTTPConfig
...
from derouter.llms.openai.realtime.http_transformation import OpenAIRealtimeHTTPConfig
from derouter.types.realtime import RealtimeTranscriptionSessionRequest
...
def test_request_passthrough_excludes_routing_hint():
    """Unknown fields pass through; the derouter-only `model` hint is not forwarded."""
    req = RealtimeTranscriptionSessionRequest(
        model="openai/gpt-realtime-whisper",
...
async def test_sdk_fn_routes_openai_transcription_session(monkeypatch):
    """
    derouter.acreate_realtime_transcription_session resolves the OpenAI provider
    ...
    result = await derouter.acreate_realtime_transcription_session(
        model="openai/gpt-realtime-whisper",
    ...
    # The derouter-only routing hint must not be forwarded upstream.
