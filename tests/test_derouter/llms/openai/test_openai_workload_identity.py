...
import derouter
from derouter.llms.derouter_proxy.responses.transformation import DeRouterProxyResponsesAPIConfig
from derouter.llms.openai.common_utils import BaseOpenAILLM, OpenAIError
...
from derouter.llms.openai.workload_identity import (
...
class TestResolveConfig:
    ...
    def test_foreign_derouter_api_base_disables(
        self, wif_env: OpenAIWorkloadIdentityConfig, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr(derouter, "api_base", "https://my-vllm.internal/v1")
        assert resolve_openai_workload_identity_config(api_key=None, api_base=None) is None
...
class TestResponsesValidateEnvironment:
    ...
    def test_mints_bearer_when_wif_configured(self, wif_env: OpenAIWorkloadIdentityConfig) -> None:
        ...
        headers: Final = OpenAIResponsesAPIConfig().validate_environment(
            headers={}, model="gpt-4o-mini", derouter_params=GenericDeRouterParams()
        )
    ...
    def test_static_key_wins(self, wif_env: OpenAIWorkloadIdentityConfig) -> None:
        headers: Final = OpenAIResponsesAPIConfig().validate_environment(
            headers={}, model="gpt-4o-mini", derouter_params=GenericDeRouterParams(api_key="sk-responses")
        )
    ...
    def test_foreign_api_base_skips_wif(self, wif_env: OpenAIWorkloadIdentityConfig) -> None:
        headers: Final = OpenAIResponsesAPIConfig().validate_environment(
            headers={},
            model="gpt-4o-mini",
            derouter_params=GenericDeRouterParams(api_base="https://my-vllm.internal/v1"),
        )
    ...
    def test_privatelink_api_base_mints_bearer(self, wif_env: OpenAIWorkloadIdentityConfig) -> None:
        ...
        headers: Final = OpenAIResponsesAPIConfig().validate_environment(
            headers={},
            model="gpt-4o-mini",
            derouter_params=GenericDeRouterParams(api_base="https://southcentralus.privatelink.api.openai.com/v1"),
        )
        assert headers["Authorization"] == "Bearer exchanged-bearer-token"

    def test_derouter_proxy_subclass_never_mints_wif(self, wif_env: OpenAIWorkloadIdentityConfig) -> None:
        headers: Final = DeRouterProxyResponsesAPIConfig().validate_environment(
            headers={}, model="gpt-4o-mini", derouter_params=GenericDeRouterParams()
        )
        assert headers["Authorization"] == "Bearer None"
