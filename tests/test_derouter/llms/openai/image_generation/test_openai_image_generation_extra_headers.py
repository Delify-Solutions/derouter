...
Verifies that extra_headers passed to derouter.image_generation() /
derouter.aimage_generation() are forwarded to the OpenAI API client as
...
class TestImageGenerationEntryPointHeaders:
    """Test that derouter.image_generation() passes headers through to the OpenAI provider."""

    @pytest.mark.asyncio
    async def test_extra_headers_reach_openai_provider(self):
        """End-to-end: extra_headers from derouter.aimage_generation() reach OpenAI images.generate()."""
        import derouter
