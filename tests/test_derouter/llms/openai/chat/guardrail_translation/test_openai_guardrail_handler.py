...
class TestOpenAIChatCompletionsHandlerToolCallsOutput:
    ...
    async def test_extract_tool_calls_and_content_from_output(self):
                        ...
                        tool_calls=[
                            ChatCompletionMessageToolCall(
                                ...
                                function=Function(
                                    name="web_search",
                                    arguments=json.dumps(
                                        {"keywords": "derouter documentation"}
                                    ),
                                ),
                            )
                        ],
        ...
        response_tool_call = response.choices[0].message.tool_calls[0]
        args = json.loads(response_tool_call.function.arguments)
        assert args["keywords"] == "DEROUTER DOCUMENTATION"
...
class TestIncrementalScanRespectsSkipFlags:
    ...
    applies derouter_params to the callback (they are not constructor kwargs).
    """
    ...
    async def test_first_turn_scans_no_system_or_tool_content(self):
        ...
        data = {"messages": self._messages(), "derouter_session_id": "skip-flags-turn1"}
    ...
    async def test_second_turn_scans_only_new_eligible_content(self):
        ...
        with patch.object(guardrail, "make_bedrock_api_request", new_callable=AsyncMock) as mock_api:
            ...
            await handler.process_input_messages(
                data={"messages": self._messages(), "derouter_session_id": session},
                guardrail_to_apply=guardrail,
            )
            mock_api.reset_mock()
            await handler.process_input_messages(
                data={"messages": self._messages(followup), "derouter_session_id": session},
                guardrail_to_apply=guardrail,
            )
            assert mock_api.call_count == 1
            scanned = [m["content"] for m in mock_api.call_args.kwargs["messages"]]
