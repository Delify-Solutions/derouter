"""
Tests for OpenAI GPT transformation (derouter/llms/openai/chat/gpt_transformation.py)
"""
...
import derouter
from derouter.derouter_core_utils.prompt_templates.common_utils import TOOL_RESULT_IMAGE_BOUNDARY
...
class TestOpenAIGPTConfig:
    ...
    def test_user_param_supported_for_responses_api_models(self):
        ...
        Regression test for: https://github.com/Delify-Solutions/derouter/issues/17633
...
class TestGetOptionalParamsIntegration:
    """Integration tests using derouter.get_optional_params()"""
    ...
    def test_reasoning_effort_supported_for_unknown_model_alias(self):
        """An openai/-routed model derouter doesn't recognize is likely a proxy alias:
    ...
    def test_reasoning_effort_still_rejected_for_known_non_reasoning_model(self):
        ...
        with pytest.raises(derouter.utils.UnsupportedParamsError):
            get_optional_params(
                model="gpt-4o",
                custom_llm_provider="openai",
                reasoning_effort="low",
...
class TestOpenAIChatCompletionStreamingHandler:
    ...
    def test_chunk_parser_raises_on_in_body_error_payload(self):
        ...
        ends the stream (https://github.com/Delify-Solutions/derouter/issues/25492)."""
...
class TestCacheControlPreservationForCustomEndpoint:
    """
    Regression tests for https://github.com/Delify-Solutions/derouter/issues/30319
    ...
    openai provider pointed at a cache_control-aware endpoint (a DeRouter proxy,
    ...
    def _clean_openai_base_env(self, monkeypatch):
        ...
        monkeypatch.setattr(derouter, "api_base", None, raising=False)
    ...
    async def test_async_transform_request_strips_for_real_openai(self):
        body = await self.config.async_transform_request(
            ...
            messages=self._cache_controlled_messages(),
            optional_params={},
            derouter_params={"custom_llm_provider": "openai", "api_base": None},
            headers={},
        )
        assert all("cache_control" not in m for m in body["messages"])
...
class TestToolSchemaCombinatorFlatteningForOpenAI:
    ...
    def _transform(self, config, model, derouter_params, tools):
