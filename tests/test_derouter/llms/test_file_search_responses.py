...
class TestUpdateResponsesToolsWithModelFileIds:
    ...
    def test_B2_code_interpreter_mapping_still_works(self):
        ...
        model_id = "model-abc"
        file_id = "derouter_managed_file_001"
...
class TestFileSearchGuardInResponsesMain:
    ...
    def test_has_file_search_tool_true(self):
        from derouter.responses.main import _has_file_search_tool
    ...
    def test_E2_no_provider_config_routes_to_emulated_handler(self):
        ...
        with (
            patch.object(
                import_module("derouter.responses.main").derouter, "get_llm_provider",
                return_value=("claude-sonnet-4-5", "anthropic", None, None),
            ),
            patch.object(
                import_module("derouter.responses.main"), "update_responses_input_with_model_file_ids",
                return_value="hello",
            ),
            patch.object(
                import_module("derouter.responses.main"), "update_responses_tools_with_model_file_ids",
                return_value=tools,
            ),
            patch.object(
                import_module("derouter.responses.main").ProviderConfigManager, "get_provider_responses_api_config",
                return_value=None,
            ),
            patch.object(
                import_module("derouter.responses.main").ResponsesAPIRequestUtils, "get_requested_response_api_optional_param",
                return_value={},
            ),
            patch.object(
                import_module("derouter.responses.main"), "run_async_function", return_value=expected
            ) as run_async_mock,
        ):
            result = responses(
                input="hello",
    ...
    def test_E3_non_native_provider_config_routes_to_emulated_handler(self):
        ...
        from derouter.llms.base_llm.responses.transformation import (
...
class TestManagedFilesVectorStoreAccess:
    def _make_hook(self):
        ...
        from derouter_enterprise.proxy.hooks.managed_files import (
            _PROXY_DeRouterManagedFiles as ManagedFiles,
        )
    ...
    def _make_vs_row(self, vector_store_id: str, team_id: Optional[str]) -> Any:
        ...
        return DeRouter_ManagedVectorStoresTable(
...
class TestEmulatedFileSearchHandler:
    """Tests for derouter/responses/file_search/emulated_handler.py"""
    ...
    def test_H1_file_search_replaced_with_function_tool(self):
        ...
        assert new_tools[0]["name"] == "derouter_file_search"
    ...
    async def test_H11_emulated_full_flow_provider_calls_tool(self):
        ...
        with (
            patch.object(
                import_module("derouter.responses.file_search.emulated_handler"), "_call_aresponses",
                new=AsyncMock(side_effect=[first_resp, final_resp]),
            ),
            patch(
                "derouter.vector_stores.main.asearch",
    ...
    async def test_H16_model_chosen_id_outside_request_is_not_searched(self, caplog):
        ...
        with (
            patch.object(
                import_module("derouter.responses.file_search.emulated_handler"),
                "_call_aresponses",
                new=AsyncMock(side_effect=[first_resp, final_resp]),
            ),
            patch("derouter.vector_stores.main.asearch", new=mock_asearch),  # test-quality-ok: asserts store searched
