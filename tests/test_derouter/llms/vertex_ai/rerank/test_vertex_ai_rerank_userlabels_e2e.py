...
These tests go through the full `derouter.rerank()` call path with the HTTP
layer mocked, so they catch plumbing bugs (e.g. `derouter_params` losing
...
import derouter
import derouter.llms.vertex_ai.rerank.transformation
...
def _patch_vertex_auth():
    return patch.object(
        derouter.llms.vertex_ai.rerank.transformation.VertexAIRerankConfig,
...
def test_rerank_userlabels_propagates_from_metadata_sync(clean_vertex_env):
    """
    `derouter.rerank(metadata={"requester_metadata": {...}})` must end up as
    ...
    assert "userLabels" in body, (
        ...
        "lost between derouter.rerank() and transform_rerank_request. "
...
def test_rerank_userlabels_propagates_from_metadata_async(clean_vertex_env):
    """Same as the sync test, but through `derouter.arerank`."""
    captured = {}
