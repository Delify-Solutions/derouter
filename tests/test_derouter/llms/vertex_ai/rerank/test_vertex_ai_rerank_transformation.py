...
from derouter.llms.vertex_ai.rerank.transformation import VertexAIRerankConfig
...
class TestVertexAIRerankTransform:
    ...
    def test_get_complete_url(self):
        ...
        importlib.reload(derouter) in conftest.py.
        """
        ...
        # Test with derouter.vertex_project
        with patch.dict(os.environ, {}, clear=True):
            import derouter

            # Set vertex_project attribute if it doesn't exist
            if not hasattr(derouter, "vertex_project"):
                derouter.vertex_project = None
            original_project = derouter.vertex_project
            derouter.vertex_project = "derouter-project-456"
            # Reset mock call count
            mock_ensure_access_token.reset_mock()
            mock_ensure_access_token.return_value = (
                "mock-token",
                "derouter-project-456",
            )
            try:
                url = self.config.get_complete_url(api_base=None, model=self.model)
                expected_url = "https://discoveryengine.googleapis.com/v1/projects/derouter-project-456/locations/global/rankingConfigs/default_ranking_config:rank"
                assert url == expected_url
            finally:
                derouter.vertex_project = original_project
    ...
    def test_transform_rerank_request_user_labels_from_metadata(self):
        ...
        request_data = self.config.transform_rerank_request(
            ...
            headers={},
            derouter_params={
                "metadata": {"requester_metadata": {"app": "derouter", "tier": "1"}}
            },
        )
        assert request_data["userLabels"] == {"app": "derouter", "tier": "1"}
    ...
    def test_validate_environment_preserves_optional_params_for_get_complete_url(
        ...
        with patch(
            "derouter.llms.vertex_ai.rerank.transformation.get_secret_str",
            return_value=None,
        ):
