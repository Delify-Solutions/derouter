...
from derouter.llms.vertex_ai.batches.transformation import VertexAIBatchTransformation


def test_output_file_id_uses_predictions_jsonl_with_output_info():
    response = {
        "outputInfo": {
            "gcsOutputDirectory": "gs://test-bucket/derouter-vertex-files/publishers/google/models/gemini-2.5-pro/prediction-model-123"
        }
    }
    ...
    assert (
        output_file_id
        == "gs://test-bucket/derouter-vertex-files/publishers/google/models/gemini-2.5-pro/prediction-model-123/predictions.jsonl"
    )
...
def test_output_file_id_falls_back_to_output_uri_prefix_with_predictions_jsonl():
    response = {
        ...
        "outputConfig": {
            "gcsDestination": {
                "outputUriPrefix": "gs://test-bucket/derouter-vertex-files/publishers/google/models/gemini-2.5-pro/prediction-model-456"
            }
        },
    }
    ...
    assert (
        output_file_id
        == "gs://test-bucket/derouter-vertex-files/publishers/google/models/gemini-2.5-pro/prediction-model-456/predictions.jsonl"
    )
...
def test_vertex_ai_cancel_batch():
    ...
    with patch(
        "derouter.llms.vertex_ai.batches.handler._get_httpx_client"
    ) as mock_client:
...
@pytest.mark.asyncio
async def test_derouter_cancel_batch_vertex_ai():
    """Test that derouter.cancel_batch works with vertex_ai provider"""
    mock_response = MagicMock()
    mock_response.id = "batch_123"
    mock_response.status = "cancelling"

    with patch("derouter.batches.main.vertex_ai_batches_instance") as mock_instance:
        mock_instance.cancel_batch.return_value = mock_response

        response = derouter.cancel_batch(
            batch_id="batch_123",
