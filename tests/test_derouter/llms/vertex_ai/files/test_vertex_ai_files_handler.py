...
from derouter.types.llms.openai import FileContentRequest, HttpxBinaryResponseContent
...
class TestVertexAIFilesHandler:
    ...
    def test_extract_bucket_and_object_from_file_id_standard_path(self):
        ...
        file_id = "gs%3A%2F%2Ftest-bucket%2Fderouter-vertex-files%2Ftest-folder%2Fsub-folder%2Ftest-file.txt"
        ...
        expected_object = "derouter-vertex-files/test-folder/sub-folder/test-file.txt"
    ...
    def test_extract_bucket_and_object_from_file_id_rejects_no_gs_prefix(self):
        ...
        file_id = "test-bucket%2Fderouter-vertex-files%2Ftest-file.txt"
    ...
    def test_extract_bucket_and_object_from_file_id_rejects_wrong_bucket(self):
        file_id = "gs%3A%2F%2Fother-bucket%2Fderouter-vertex-files%2Ftest-file.txt"
    ...
    async def test_afile_content_success(self):
        ...
        # Setup test data
        file_id = "gs%3A%2F%2Ftest-bucket%2Fderouter-vertex-files%2Fuploads%2Fabc-test-file.txt"
        ...
        with (
            ...
            assert call_args.kwargs["object_name"] == "derouter-vertex-files/uploads/abc-test-file.txt"
        ...
        with (
            ...
            with pytest.raises(
                ...
                match=re.escape(
                    ...
                    "gs://test-bucket/derouter-vertex-files/uploads/abc-test-file.txt"
                ),
            ):
                await self.handler.afile_content(
                    file_content_request=file_content_request,
    ...
    def test_resolve_read_gcs_config_falls_back_to_env(self, monkeypatch):
        ...
        bucket, service_account = self.handler._resolve_read_gcs_config(derouter_params={}, vertex_credentials=None)
    ...
    def test_resolve_read_gcs_config_serializes_dict_credentials(self, monkeypatch):
        ...
        _, service_account = self.handler._resolve_read_gcs_config(
            derouter_params={"gcs_bucket_name": "my-model-bucket"},
    ...
    async def test_afile_content_honors_per_model_bucket_over_env(self, monkeypatch):
        ...
        file_id = "gs%3A%2F%2Fmy-model-bucket%2Fderouter-vertex-files%2Fuploads%2Fabc-batch-output.jsonl"
        file_content_request = FileContentRequest(file_id=file_id, extra_headers=None, extra_body=None)
        ...
        assert mock_download.call_args.kwargs["object_name"] == "derouter-vertex-files/uploads/abc-batch-output.jsonl"
