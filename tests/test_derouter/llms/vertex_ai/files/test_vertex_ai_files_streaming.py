...
from derouter.llms.base_llm.files.transformation import BaseFileUploadStream
...
from derouter.types.llms.openai import CreateFileRequest
...
class TestStreamingOutputParity:
    def test_transform_create_file_request_returns_streaming_body_parity(self):
        ...
        out = cfg.transform_create_file_request(
            model="", create_file_data=request, optional_params={}, derouter_params={}
        )
...
class TestFileLikeInputNotPartiallyConsumed:
    ...
    def test_filehandle_create_file_keeps_first_entry(self):
        ...
        cfg.get_complete_file_url(
            ...
            model="",
            optional_params={},
            derouter_params={"gcs_bucket_name": "test-bucket"},
        ...
        assert _get_derouter_batch_custom_id_from_labels(first_labels) == "request-0"
...
class TestPathSourcedStreaming:
    ...
    def test_transform_from_path_matches_legacy_and_keeps_all_rows(self, tmp_path):
        ...
        out = cfg.transform_create_file_request(model="", create_file_data=data, optional_params={}, derouter_params={})
        assert isinstance(out, dict) and "streaming_media_upload" in out
    ...
    def test_path_source_peak_stays_below_payload(self, tmp_path):
        ...
        def run():
            ...
            out = cfg.transform_create_file_request(
                model="", create_file_data=data, optional_params={}, derouter_params={}
            )
...
_GCS_OBJECT_JSON = {
    "id": "test-bucket/derouter-vertex-files/x/123",
    "name": "derouter-vertex-files/x",
    "size": "0",
