...
from derouter.llms.vertex_ai.files.transformation import (
    VertexAIFilesConfig,
    _get_derouter_batch_custom_id_from_labels,
    _openai_batch_jsonl_entry_to_vertex_rows,
    _sanitize_gcp_label_value,
)
from derouter.types.llms.openai import OpenAIFileObject, HttpxBinaryResponseContent
...
class TestParseGcsUri:
    ...
    def test_should_parse_standard_gs_uri(self, config):
        file_id = "gs://my-bucket/derouter-vertex-files/path/to/object.jsonl"
        bucket, encoded = config._parse_gcs_uri(file_id, derouter_params={"gcs_bucket_name": "my-bucket"})
        assert bucket == "my-bucket"
        assert encoded == urllib.parse.quote("derouter-vertex-files/path/to/object.jsonl", safe="")

    def test_should_parse_uri_with_nested_publisher_path(self, config):
        uri = "gs://derouter-local/derouter-vertex-files/publishers/google/models/gemini-2.0-flash-001/abc-123"
        bucket, encoded = config._parse_gcs_uri(uri, derouter_params={"gcs_bucket_name": "derouter-local"})
        assert bucket == "derouter-local"
        expected_path = "derouter-vertex-files/publishers/google/models/gemini-2.0-flash-001/abc-123"
        assert encoded == urllib.parse.quote(expected_path, safe="")

    def test_should_handle_url_encoded_input(self, config):
        encoded_uri = urllib.parse.quote("gs://my-bucket/derouter-vertex-files/some/path", safe="")
        bucket, encoded = config._parse_gcs_uri(encoded_uri, derouter_params={"gcs_bucket_name": "my-bucket"})
        assert bucket == "my-bucket"
        assert encoded == urllib.parse.quote("derouter-vertex-files/some/path", safe="")

    def test_should_reject_bucket_only(self, config):
        with pytest.raises(ValueError, match="object name"):
            config._parse_gcs_uri("gs://my-bucket", derouter_params={"gcs_bucket_name": "my-bucket"})

    def test_should_reject_no_gs_prefix(self, config):
        with pytest.raises(ValueError, match="gs://"):
            config._parse_gcs_uri(
                "my-bucket/derouter-vertex-files/object.txt",
                derouter_params={"gcs_bucket_name": "my-bucket"},
            )

    def test_should_reject_unmanaged_object_path(self, config):
        with pytest.raises(ValueError, match="DeRouter-managed"):
            config._parse_gcs_uri(
    ...
    def test_should_allow_legacy_object_path_with_trusted_server_flag(self, config):
        ...
        bucket, encoded = config._parse_gcs_uri(
            ...
            derouter_params={
                "gcs_bucket_name": "my-bucket",
                "_derouter_internal_model_credentials": trusted_credentials,
            },
        )
    ...
    def test_should_reject_user_supplied_legacy_flag_snapshot(self, config):
        with pytest.raises(ValueError, match="DeRouter-managed"):
            config._parse_gcs_uri(
                ...
                derouter_params={
                    "gcs_bucket_name": "my-bucket",
                    "_derouter_internal_model_credentials": {"allow_legacy_cloud_file_ids": True},
                },
            )
    ...
    def test_should_reject_unconfigured_bucket(self, config):
        with pytest.raises(ValueError, match="configured storage bucket"):
            config._parse_gcs_uri(
                "gs://other-bucket/derouter-vertex-files/object.txt",
                derouter_params={"gcs_bucket_name": "my-bucket"},
            )
    ...
    def test_should_ignore_request_metadata_bucket_and_sanitize_filename(self, config):
        url = config.get_complete_file_url(
            ...
            derouter_params={
                ...
                "derouter_metadata": {"gcs_bucket_name": "attacker-bucket"},
            },
            data={
        ...
        assert object_name.startswith("derouter-vertex-files/uploads/")
...
class TestBatchObjectNaming:
    def test_should_store_publisher_model_under_publishers_path(self, config):
        ...
        assert object_name.startswith("derouter-vertex-files/publishers/google/models/gemini-2.5-flash/")
    ...
    def test_should_store_fine_tuned_endpoint_under_endpoints_path(self, config):
        ...
        assert object_name.startswith("derouter-vertex-files/endpoints/7768560373388541952/")
    ...
    def test_should_reject_batch_upload_for_custom_endpoint_deployment(self, config):
        ...
        with pytest.raises(VertexAIError) as exc_info:
            config.get_complete_file_url(
                ...
                model="",
                optional_params={},
                derouter_params={"gcs_bucket_name": "my-bucket", "custom_endpoint": True},
                data={
...
class TestTransformRetrieveFile:
    def test_should_build_correct_gcs_metadata_url(self, config):
        file_id = "gs://my-bucket/derouter-vertex-files/path/to/file.jsonl"
        url, params = config.transform_retrieve_file_request(
            file_id=file_id,
            optional_params={},
            derouter_params={"gcs_bucket_name": "my-bucket"},
        )
        expected_encoded = urllib.parse.quote("derouter-vertex-files/path/to/file.jsonl", safe="")
...
class TestTransformFileContent:
    def test_should_build_gcs_media_download_url(self, config):
        ...
        encoded = urllib.parse.quote("derouter-vertex-files/path/to/file.jsonl", safe="")
    ...
    def test_should_not_mutate_caller_logging_obj_for_batch_output_transform(self, config, monkeypatch):
            ...
            content=json.dumps(
                {
                    ...
                    "request": {"labels": {"derouter_custom_id": "request-1"}},
                    "response": {
        ...
        def mock_transform_single(
            ...
            return {"custom_id": vertex_output["request"]["labels"]["derouter_custom_id"]}
    ...
    def test_should_skip_batch_output_transformation_when_opt_out_flag_set(self, config, monkeypatch):
        """When `derouter.disable_vertex_batch_output_transformation` is True the
        ...
        monkeypatch.setattr(derouter, "disable_vertex_batch_output_transformation", True, raising=False)
...
class TestTransformDeleteFile:
    ...
    def test_should_return_file_deleted_with_reconstructed_id(self, config):
        ...
        encoded_name = urllib.parse.quote(
            "derouter-vertex-files/publishers/google/models/gemini-2.0-flash-001/abc",
            safe="",
        )
        ...
        assert result.object == "file"
        assert result.id == "gs://my-bucket/derouter-vertex-files/publishers/google/models/gemini-2.0-flash-001/abc"
    ...
    def test_should_include_bucket_in_nested_object_path(self, config):
        ...
        encoded_object = urllib.parse.quote(
            "derouter-vertex-files/publishers/google/models/gemini-2.0-flash-001/abc-123",
            safe="",
        )
        ...
        assert result.id == (
            "gs://prod-bucket/derouter-vertex-files/publishers/google/models/gemini-2.0-flash-001/abc-123"
        )
...
class TestVertexBatchOutputTransformation:
    ...
    def test_transform_successful_vertex_batch_output(self, config):
        ...
        vertex_output = {
            ...
            "request": {
                ...
                "labels": {"derouter_custom_id": "request-1"},
            },
            "response": {
                "candidates": [
                    {
    ...
    def test_transform_error_vertex_batch_output(self, config):
        ...
        vertex_output = {
            ...
            "request": {
                "contents": [{"role": "user", "parts": [{"text": "Hello world!"}]}],
                "labels": {"derouter_custom_id": "request-error"},
            },
            "response": {},
        }
    ...
    def test_transform_exception_path_sets_response_null(self, config):
        ...
        vertex_output = {
            ...
            "request": {
                "contents": [{"role": "user", "parts": [{"text": "Hello world!"}]}],
                "labels": {"derouter_custom_id": "request-boom"},
            },
    ...
    def test_transform_vertex_batch_output_legacy_labels_only_sanitized(self, config):
        """Older DeRouter batches only stored derouter_custom_id (sanitized); read path still works."""
        vertex_output = {
            "status": "",
            "processed_time": "2024-11-01T18:13:16.826+00:00",
            "request": {
                "contents": [{"role": "user", "parts": [{"text": "Hello world!"}]}],
                "labels": {"derouter_custom_id": "myrequest-1"},
            },
            "response": {
                "candidates": [
                    {
        ...
        vertex_outputs = [
            ...
            },
            {
                ...
                "request": {
                    ...
                    "labels": {"derouter_custom_id": "request-2"},
                },
                "response": {
                    "candidates": [
                        {
        ...
        vertex_outputs = [
            {
                "status": "",
                "processed_time": "2024-11-01T18:13:16.826+00:00",
                "request": {"labels": {"derouter_custom_id": "blocked-request"}},
                "response": {
            ...
            },
            {
                "status": "",
                "processed_time": "2024-11-01T18:13:17.826+00:00",
                "request": {"labels": {"derouter_custom_id": "request-2"}},
        ...
        vertex_outputs = [
            {
                "status": "",
                "processed_time": "2024-11-01T18:13:16.826+00:00",
                "request": {"labels": {"derouter_custom_id": f"request-{i}"}},
                "response": {"candidates": [{"content": {"parts": [{"text": "ok"}]}}]},
            }
            for i in range(2)
        ]
        ...
        def vertex_row(index: int) -> dict:
            return {
                ...
                "request": {
                    ...
                    "labels": {"derouter_custom_id": f"r-{index}"},
                },
                "response": {
                    "candidates": [
                        {
...
class TestVertexBatchCustomIdLabels:
    ...
    def test_custom_id_added_to_labels_in_vertex_request(self):
        ...
        assert "derouter_custom_id" in vertex_request["request"]["labels"]
        assert vertex_request["request"]["labels"]["derouter_custom_id"] == "request-1"
        raw_label = vertex_request["request"]["labels"]["derouter_custom_id_raw"]
    ...
    def test_long_custom_id_round_trips_across_raw_label_chunks(self):
        ...
        assert "derouter_custom_id_raw_1" in labels_a
        assert "derouter_custom_id_raw_1" in labels_b
        assert labels_a["derouter_custom_id_raw"] == labels_b["derouter_custom_id_raw"]
        assert labels_a["derouter_custom_id_raw_1"] != labels_b["derouter_custom_id_raw_1"]
        assert _get_derouter_batch_custom_id_from_labels(labels_a) == custom_id_a
        assert _get_derouter_batch_custom_id_from_labels(labels_b) == custom_id_b
    ...
    def test_multiple_requests_each_get_their_own_label(self):
        ...
        for i, vertex_request in enumerate(vertex_jsonl_content):
            ...
            assert vertex_request["request"]["labels"]["derouter_custom_id"] == expected_custom_id
            raw_label = vertex_request["request"]["labels"]["derouter_custom_id_raw"]
    ...
    def test_end_to_end_custom_id_round_trip(self):
        ...
        assert vertex_input[0]["request"]["labels"]["derouter_custom_id"] == "myrequest-1"
        raw_label = vertex_input[0]["request"]["labels"]["derouter_custom_id_raw"]
...
class TestConfiguredBucketNameResolution:
    ...
    def test_legacy_kwarg_survives_get_derouter_params(self):
        ...
        params = get_derouter_params(bucket_name="my-legacy-bucket")
...
class TestVertexEmbeddingsBatchInputTranslation:
    ...
    def test_should_keep_chat_completions_lines_on_generate_content_path(self):
        ...
        assert row["request"]["labels"]["derouter_custom_id"] == "request-1"
        assert "key" not in row

    def test_should_keep_lines_without_a_url_on_generate_content_path(self):
        """`url` is optional on a batch line, and chat is the shape DeRouter has always assumed."""
        (row,) = _wrap_entries(
            [
                {
                    "custom_id": "request-1",
...
class TestVertexEmbeddingsBatchOutputTranslation:
    ...
    def test_should_resolve_model_from_managed_gcs_object_path(self, config):
        object_path = urllib.parse.quote(
            "derouter-vertex-files/publishers/google/models/gemini-embedding-2/"
