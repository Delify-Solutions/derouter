...
class TestVertexAIFilesIntegration:
    """Test integration of Vertex AI files with main derouter API"""

    @pytest.mark.asyncio
    async def test_derouter_afile_content_vertex_ai_provider(self):
        """Test derouter.afile_content with vertex_ai provider"""
        ...
        with patch(
            "derouter.files.main.base_llm_http_handler.retrieve_file_content",
            new_callable=MagicMock,
        ) as mock_retrieve:
            # Make it return a coroutine for async path
            mock_retrieve.return_value = mock_result

            result = await derouter.afile_content(
                file_id=file_id,
    ...
    def test_derouter_file_content_vertex_ai_provider(self):
        """Test derouter.file_content with vertex_ai provider (sync)"""
        file_id = "gs%3A%2F%2Ftest-bucket%2Ftest-file.txt"
        expected_content = b"test file content"
    ...
    def test_derouter_file_content_vertex_ai_with_model_provider_detection(self):
        """Test derouter.file_content with model parameter for provider detection"""
        file_id = "gs%3A%2F%2Ftest-bucket%2Ftest-file.txt"
        expected_content = b"test file content"
        ...
        with patch(
            ...
            with patch("derouter.files.main.get_llm_provider") as mock_get_provider:
                ...
                # Call derouter.file_content with model to trigger provider detection
                result = derouter.file_content(
                    file_id=file_id,
    ...
    def test_derouter_file_content_vertex_ai_error_cases(self):
        ...
        with patch(
            "derouter.files.main.ProviderConfigManager.get_provider_files_config",
            return_value=None,
        ):
            with pytest.raises(ValueError, match="file_id is required"):
                derouter.file_content(
