...
Maps to: derouter/llms/vertex_ai/vertex_gemma_models/transformation.py
"""
...
def _reset_derouter_http_client_cache():
...
class TestVertexGemmaCompletion:
    """Test completion flow for Vertex AI Gemma models using derouter.acompletion()"""

    @pytest.mark.asyncio
    async def test_acompletion_basic_request(self):
        """
        Test derouter.acompletion() with Vertex AI Gemma model
        ...
        Expected DeRouter Response: Standard OpenAI format
        """
        ...
        with (
            patch("derouter.llms.custom_httpx.http_handler.get_async_httpx_client") as mock_get_client,
            patch(
                "derouter.llms.vertex_ai.vertex_gemma_models.main.VertexAIGemmaModels._ensure_access_token",
            ...
            # Validate DeRouter Response (OpenAI format)
    ...
    async def test_acompletion_error_handling(self):
        """
        Test derouter.acompletion() error handling when Vertex returns invalid response
        ...
        with (
            ...
            # Should raise exception (wrapped as BadRequestError by DeRouter)
    ...
    async def test_acompletion_fake_streaming(self):
        ...
        with (
            ...
            # Call derouter.acompletion() with stream=True
            response = await derouter.acompletion(
                model="vertex_ai/gemma/gemma-3-12b-it-1222199011122",
    ...
    def test_transform_request_strips_context_management(self):
        ...
        """
        from derouter.llms.vertex_ai.vertex_gemma_models.transformation import (
            VertexGemmaConfig,
        )
    ...
    def test_sync_completion_makes_http_call(self):
        ...
        derouter.completion() and asserts a fully parsed response comes back,
        ...
        with (
            patch("derouter.llms.vertex_ai.vertex_gemma_models.transformation._get_httpx_client") as mock_get_client,
            patch(
                "derouter.llms.vertex_ai.vertex_gemma_models.main.VertexAIGemmaModels._ensure_access_token",
                return_value=("fake-access-token", "PROJECT_ID"),
            ),
