...
import derouter
from derouter.interactions.utils import get_provider_interactions_api_config
from derouter.llms.gemini.interactions.transformation import (
...
@pytest.fixture
def derouter_params():
    return GenericDeRouterParams(vertex_project="test-proj", vertex_credentials="creds.json")
...
class TestRegistration:
    ...
    def test_lazy_import_resolves(self):
        assert derouter.VertexAIInteractionsConfig is VertexAIInteractionsConfig
...
class TestValidateEnvironment:
    def test_sets_bearer_auth_without_gemini_headers(self, config, minter, derouter_params):
    ...
    def test_caller_authorization_wins(self, config, derouter_params):
        headers = config.validate_environment(
...
class TestGetCompleteUrl:
    def test_defaults_to_global_v1beta1(self, config, derouter_params):
    ...
    def test_stream_appends_alt_sse(self, config, derouter_params):
        url = config.get_complete_url(
            api_base=None,
            model="gemini-omni-flash-preview",
            derouter_params=dict(derouter_params),
    ...
    def test_multi_region_location_uses_rep_host(self, config):
        url = config.get_complete_url(
            api_base=None,
            model="gemini-omni-flash-preview",
            derouter_params={"vertex_project": "test-proj", "vertex_location": "us"},
        )
    ...
    def test_regional_location_uses_regional_host(self, config):
        url = config.get_complete_url(
            api_base=None,
            model="gemini-omni-flash-preview",
            derouter_params={"vertex_project": "test-proj", "vertex_location": "us-central1"},
        )
    ...
    def test_location_env_fallback_is_ignored(self, config, monkeypatch):
        ...
        url = config.get_complete_url(
            api_base=None,
            model="gemini-omni-flash-preview",
            derouter_params={"vertex_project": "test-proj"},
        )

        assert url == GLOBAL_BASE

    def test_api_base_override(self, config, derouter_params):
        url = config.get_complete_url(
    ...
    def test_project_resolved_from_credentials_when_not_passed(self, config, monkeypatch):
        ...
        monkeypatch.setattr(derouter, "vertex_project", None)

        url = config.get_complete_url(
            api_base=None,
            model="gemini-omni-flash-preview",
            derouter_params={"vertex_credentials": "creds.json"},
        )
    ...
    def test_invalid_location_rejected(self, config):
        with pytest.raises(ValueError, match="Invalid vertex_location"):
            config.get_complete_url(
                api_base=None,
                model="gemini-omni-flash-preview",
                derouter_params={"vertex_project": "test-proj", "vertex_location": "evil.com#"},
            )
...
class TestInteractionByIdRequests:
    def test_get_url(self, config, derouter_params):
    ...
    def test_get_url_encodes_interaction_id(self, config, derouter_params):
    ...
    def test_delete_url(self, config, derouter_params):
    ...
    def test_cancel_url(self, config, derouter_params):
