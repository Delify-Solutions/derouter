...
class TestVertexAIGeminiImageEditTransformation:
    ...
    def test_validate_environment_with_derouter_params(self) -> None:
        """Test validate_environment uses credentials from derouter_params"""
        with patch.object(
            self.config,
    ...
    def test_get_complete_url_from_derouter_params(self) -> None:
        """Test vertex_project/vertex_location read from derouter_params first"""
        url = self.config.get_complete_url(
            model="gemini-2.5-flash",
            api_base=None,
            derouter_params={
    ...
    def test_get_complete_url_derouter_params_overrides_env(self) -> None:
        """Test derouter_params takes precedence over environment variables"""
        with patch.dict(
            os.environ,
            {
