...
def test_gemini_batch_embeddings_with_custom_api_base_and_auth_header():
    ...
    with (
        ...
        patch(
            "derouter.llms.vertex_ai.gemini_embeddings.batch_embed_content_handler.GoogleBatchEmbeddings._ensure_access_token",
            side_effect=mock_auth_token,
        ),
        patch(
            "derouter.llms.vertex_ai.gemini_embeddings.batch_embed_content_handler.GoogleBatchEmbeddings._get_token_and_url"
        ) as mock_get_token,
    ):
...
def test_gemini_multimodal_embedding_e2e():
    """Test end-to-end multimodal embedding call through derouter.embedding()."""
    client = HTTPHandler()
