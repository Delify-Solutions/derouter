...
def test_process_gemini_media_gcs_without_extension_errors_and_metadata_mock():
    with patch(
        "derouter.llms.vertex_ai.gemini.transformation._get_gcs_object_content_type",
        return_value=None,
    ):
        with pytest.raises(derouter.BadRequestError) as exc:
...
def test_process_gemini_media_rejects_gcs_metadata_mime_not_supported_by_gemini():
    ...
    with patch(
        "derouter.llms.vertex_ai.gemini.transformation._get_gcs_object_content_type",
        return_value="application/x-derouter-unit-test-unknown-mime",
    ):
        with pytest.raises(
...
def test_is_valid_gcs_bucket_name_matrix(bucket, expected):
    from derouter.llms.vertex_ai.gemini.transformation import _is_valid_gcs_bucket_name
...
def test_get_gcs_object_content_type_explicit_vertex_success_and_token_failure():
    from derouter.llms.vertex_ai.gemini import transformation as gt
    ...
    with (
        ...
        patch(
            "derouter.llms.vertex_ai.gemini.transformation._get_gcs_metadata_http_handler",
            return_value=http,
        ),
    ):
        assert (
...
def test_get_gcs_object_content_type_http_error_explicit_vs_anonymous():
    ...
    with (
        ...
        with pytest.raises(derouter.BadRequestError, match="HTTP 403") as ei:
            gt._get_gcs_object_content_type(
...
def test_async_transform_request_body_offloads_extensionless_gs_not_plain_text():
    from derouter.llms.vertex_ai.gemini import transformation as gemini_transformation
    ...
    with (
        ...
        patch(
            "derouter.llms.vertex_ai.context_caching.vertex_ai_context_caching."
    ...
    async def run_plain():
        with patch(
            "derouter.llms.vertex_ai.gemini.transformation.asyncify",
