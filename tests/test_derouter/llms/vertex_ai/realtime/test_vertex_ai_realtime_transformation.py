...
import derouter
from derouter.llms.vertex_ai.realtime.transformation import VertexAIRealtimeConfig
...
def patch_native_audio_cost_map_entry(monkeypatch):
    ...
    """
    entry = dict(derouter.model_cost.get(_NATIVE_AUDIO_MODEL, {}))
    entry["gemini_native_audio"] = True
    monkeypatch.setitem(derouter.model_cost, _NATIVE_AUDIO_MODEL, entry)
...
async def test_vertex_realtime_text_in_text_out():
    ...
    logging_obj.derouter_trace_id = "test-trace-id"
