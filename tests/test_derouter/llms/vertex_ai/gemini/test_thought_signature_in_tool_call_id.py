...
When using OpenAI clients (instead of DeRouter SDK), provider_specific_fields are not preserved.
...
def test_get_thought_signature_backward_compatibility():
    ...
    # Test with provider_specific_fields (DeRouter SDK scenario)
    tool = {
        "id": "call_abc123",
        "type": "function",
        "function": {
...
def test_openai_client_e2e_flow(enable_preview_features):
    ...
    1. DeRouter receives response from Gemini with thought signature
    2. DeRouter embeds signature in tool call ID (if preview features enabled)
    3. OpenAI client sends message back with same tool call ID
    4. DeRouter extracts signature from ID/provider_specific_fields and sends to Gemini
    """
    test_signature = ""
    ...
    # Step 2: DeRouter transforms to OpenAI format
    function, tools, _ = VertexGeminiConfig._transform_parts(
        parts=gemini_parts,
        cumulative_tool_call_idx=0,
        is_function_call=False,
    ...
    # Step 4: DeRouter converts back to Gemini format, extracting signature
