...
def test_extra_body_cache_not_forwarded_to_vertex_ai():
    """
    'cache' inside extra_body is a DeRouter-internal proxy caching control.
    ...
    optional_params = {
        "extra_body": {
            "cache": {"use-cache": True, "ttl": 86400},  # DeRouter-internal
...
def test_extra_body_tags_not_forwarded_to_vertex_ai():
    """
    'tags' inside extra_body is a DeRouter-internal param for logging/tracking.
    It must NOT be forwarded to the Vertex AI request body.
    Documented in derouter_proxy.md: "Send tags by including them in the extra_body parameter"
    """
    messages = [{"role": "user", "content": "test"}]
    optional_params = {
        "extra_body": {
...
def test_metadata_to_labels_vertex_only():
    ...
    result = _transform_request_body(
        ...
        custom_llm_provider="gemini",
        derouter_params=derouter_params.copy(),
        cached_content=None,
    )
    assert "labels" not in result
...
def _parallel_tool_calls_signed_via_id(*signatures):
    """Parallel tool calls in the shape DeRouter actually hands back to clients.
...
def test_file_data_field_order():
    ...
    from derouter.llms.vertex_ai.gemini.transformation import _process_gemini_media
...
def test_gemini_files_api_uri_without_format():
    ...
    Related issue: https://github.com/Delify-Solutions/derouter/issues/24907
    """
    from derouter.llms.vertex_ai.gemini.transformation import _process_gemini_media
...
def test_assistant_message_with_images_field():
    ...
    messages = [
        ...
        {
            "role": "assistant",
            "content": "Here's your banana in a DeRouter costume!",
            "images": [
                {
    ...
    assert (
        ...
        == "Generate an image of a banana wearing a costume that says DeRouter"
    )
    ...
    assert text_part["text"] == "Here's your banana in a DeRouter costume!"
...
def test_assistant_message_with_images_using_message_object():
    ...
    # Create messages using Message object (as returned by DeRouter)
...
def test_function_response_has_user_role():
    ...
    must be sent with role="user". Previously, DeRouter omitted the role field
    entirely, causing 400 errors from the Gemini API.

    Fixes: https://github.com/Delify-Solutions/derouter/issues/22003
    Fixes: https://github.com/Delify-Solutions/derouter/issues/20690
    """
    messages = [
