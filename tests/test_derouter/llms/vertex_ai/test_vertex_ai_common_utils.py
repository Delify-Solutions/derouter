...
from derouter.llms.vertex_ai.common_utils import (
    ...
    vertex_request_labels_from_derouter_params,
)
...
async def test_get_supports_system_message():
    ...
    from derouter.llms.vertex_ai.common_utils import get_supports_system_message
...
def test_build_vertex_schema():
    ...
    from derouter.llms.vertex_ai.common_utils import _build_vertex_schema
...
def test_process_items_with_excessive_nesting():
    ...
    # generate a schema with excessive nesting
    from derouter.llms.vertex_ai.common_utils import process_items
...
def test_get_vertex_url_global_region(stream, expected_endpoint_suffix):
    ...
    # Mock derouter.VertexGeminiConfig.get_model_for_vertex_ai_url to return model as is
    # as we are not testing that part here, just the URL construction
    with patch(
        "derouter.VertexGeminiConfig.get_model_for_vertex_ai_url",
...
def test_vertex_filter_format_uri():
    ...
    from derouter.llms.vertex_ai.common_utils import filter_schema_fields
...
def test_convert_schema_types_type_array_conversion():
    ...
    Relevant issue: https://github.com/Delify-Solutions/derouter/issues/14091
    """
    from derouter.llms.vertex_ai.common_utils import _convert_schema_types
...
def test_fix_enum_empty_strings():
    ...
    """
    from derouter.llms.vertex_ai.common_utils import _fix_enum_empty_strings
...
def test_get_vertex_model_id_from_url():
    ...
    from derouter.llms.vertex_ai.common_utils import get_vertex_model_id_from_url
...
def test_construct_target_url_with_version_prefix():
    ...
    from derouter.llms.vertex_ai.common_utils import construct_target_url
...
def test_fix_enum_types():
    ...
    """
    from derouter.llms.vertex_ai.common_utils import _fix_enum_types
...
async def test_vertex_ai_token_counter_routes_partner_models():
    ...
    from derouter.llms.vertex_ai.common_utils import VertexAITokenCounter
    ...
    with patch(
        "derouter.llms.vertex_ai.vertex_ai_partner_models.main.VertexAIPartnerModels.count_tokens"
...
async def test_vertex_ai_token_counter_routes_gemini_models():
    ...
    with patch(
        "derouter.llms.vertex_ai.count_tokens.handler.VertexAITokenCounter.acount_tokens"
...
def test_add_object_type_schema_with_no_properties_and_no_type():
    ...
    """
    from derouter.llms.vertex_ai.common_utils import add_object_type
...
def test_vertex_request_labels_from_derouter_params_extracts_requester_metadata():
    assert vertex_request_labels_from_derouter_params(None) is None
    assert vertex_request_labels_from_derouter_params({}) is None
    assert vertex_request_labels_from_derouter_params({"metadata": None}) is None
    lp = {"metadata": {"requester_metadata": {"team": "analytics", "count": 3}}}
    assert vertex_request_labels_from_derouter_params(lp) == {"team": "analytics"}


def test_vertex_request_labels_from_derouter_params_accepts_derouter_metadata():
    lp = {"derouter_metadata": {"requester_metadata": {"team": "platform", "count": 3}}}
    assert vertex_request_labels_from_derouter_params(lp) == {"team": "platform"}


def test_vertex_request_labels_prefers_metadata_over_derouter_metadata():
    lp = {
        "metadata": {"requester_metadata": {"source": "metadata"}},
        "derouter_metadata": {"requester_metadata": {"source": "derouter_metadata"}},
    }
    assert vertex_request_labels_from_derouter_params(lp) == {"source": "metadata"}


def test_pop_vertex_request_labels_prefers_explicit_labels_then_metadata():
    optional = {"labels": {"env": "prod"}}
    derouter_params = {"metadata": {"requester_metadata": {"team": "x"}}}
    assert pop_vertex_request_labels(optional, derouter_params) == {"env": "prod"}
    assert "labels" not in optional

    optional2: dict = {}
    assert pop_vertex_request_labels(optional2, derouter_params) == {"team": "x"}

    optional3 = {"labels": {"team": 123}}
    assert pop_vertex_request_labels(optional3, derouter_params) == {"team": "x"}


def test_pop_vertex_request_labels_uses_derouter_metadata_when_metadata_absent():
    optional: dict = {}
    derouter_params = {
        "derouter_metadata": {"requester_metadata": {"team": "from_derouter_meta"}}
    }
    assert pop_vertex_request_labels(optional, derouter_params) == {
        "team": "from_derouter_meta"
    }
...
def test_vertex_text_embedding_request_includes_labels_from_metadata():
    ...
    req = derouter.vertexAITextEmbeddingConfig.transform_openai_request_to_vertex_embedding_request(
        input="hi",
        optional_params={},
...
def test_get_vertex_ai_lyria_model_info_resolves_audio_api(model, expected_api):
    from derouter.llms.vertex_ai.common_utils import get_vertex_ai_lyria_model_info
...
def test_get_vertex_ai_lyria_model_info_falls_back_to_bundled_map(monkeypatch):
    ...
    stale_runtime_model_cost = {
        key: value for key, value in derouter.model_cost.items() if not key.startswith("vertex_ai/lyria")
    }
    monkeypatch.setattr(derouter, "model_cost", stale_runtime_model_cost)
