...
import derouter.derouter_core_utils.prompt_templates
import derouter.derouter_core_utils.prompt_templates.factory
...
import derouter
from derouter import get_optional_params
...
def test_logprobs_unit_test():
    from derouter import VertexGeminiConfig
...
def test_process_gemini_media():
    ...
    from derouter.llms.vertex_ai.gemini.transformation import _process_gemini_media
    from derouter.types.llms.vertex_ai import FileDataType
...
def test_vertex_embedding_url(model, expected_url):
    ...
    Relevant issue: https://github.com/Delify-Solutions/derouter/issues/6482

    When a fine-tuned embedding model is used, the URL is different from the standard one.
    """
    from derouter.llms.vertex_ai.common_utils import _get_vertex_url
...
def mock_convert_url_to_base64():
    with patch(
        "derouter.derouter_core_utils.prompt_templates.factory.convert_url_to_base64",
    ) as mock:
...
def test_aaavertex_embeddings_distances(
    ...
    with (
        ...
        patch.object(
            derouter.main.vertex_multimodal_embedding,
    ...
    with (
        ...
        text_response = derouter.embedding(
            model="vertex_ai/multimodalembedding@001",
            input=[input_string],
            client=vertex_client,
        )
