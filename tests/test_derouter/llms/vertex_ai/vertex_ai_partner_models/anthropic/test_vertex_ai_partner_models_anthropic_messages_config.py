...
from derouter.llms.vertex_ai.vertex_ai_partner_models.main import VertexAIPartnerModels
...
def test_provider_config_manager_reuses_vertex_anthropic_messages_config_instance():
    ...
    try:
        first_config = ProviderConfigManager.get_provider_anthropic_messages_config(
            model="claude-opus-4-6",
            provider=derouter.LlmProviders.VERTEX_AI,
        )
...
def test_vertex_claude_completion_does_not_mutate_shared_extra_headers():
    """Regression: router shallow-copies derouter_params so extra_headers is a shared
    ...
    shared_extra_headers = {}  # simulates deployment["derouter_params"]["extra_headers"]
    ...
    with (
        ...
        patch(
            "derouter.llms.anthropic.chat.AnthropicChatCompletion.completion",
...
def test_messages_thinking_shape_follows_exact_vertex_entry_flag(local_model_cost_map, monkeypatch):
    ...
    monkeypatch.setitem(
        derouter.model_cost["vertex_ai/claude-opus-4-8"], "supports_adaptive_thinking", False
    )
...
def test_vertex_claude_4_8_plus_cost_map_entries_carry_mid_conversation_system_flag():
    ...
    missing = [
        ...
        and str(info.get("derouter_provider", "")).startswith("vertex_ai")
        and "claude" in key
