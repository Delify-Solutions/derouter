...
_GEMMA_MODEL_COST_ENTRY = {
    "vertex_ai/google/gemma-4-26b-a4b-it-maas": {
        "derouter_provider": "vertex_ai-openai_models",
...
def test_gemma_maas_supports_function_calling():
    ...
    with patch.dict(derouter.model_cost, _GEMMA_MODEL_COST_ENTRY, clear=False):
        assert (
...
def test_gemma_maas_supports_vision():
    ...
    with patch.dict(derouter.model_cost, _GEMMA_MODEL_COST_ENTRY, clear=False):
        assert (
            derouter.utils.supports_vision(
                model="vertex_ai/google/gemma-4-26b-a4b-it-maas"
            )
            is True
        )
...
# (derouter.llms.custom_httpx.http_handler.AsyncHTTPHandler).  This works
...
async def test_vertex_ai_gemma_global_endpoint_url():
    ...
    with (
        patch(
            "derouter.llms.custom_httpx.http_handler.AsyncHTTPHandler"
        ) as mock_http_handler,
        patch(
            "derouter.llms.vertex_ai.vertex_ai_partner_models.main.VertexAIPartnerModels._ensure_access_token",
...
async def test_vertex_ai_gemma_function_calling_passthrough():
    ...
    with (
        ...
        patch.dict(derouter.model_cost, _GEMMA_MODEL_COST_ENTRY, clear=False),
    ):
        mock_http_handler.return_value.post = AsyncMock(return_value=mock_response)
