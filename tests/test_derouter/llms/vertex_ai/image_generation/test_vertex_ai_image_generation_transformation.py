...
from derouter.llms.vertex_ai.image_generation.vertex_gemini_transformation import (
    VertexAIGeminiImageGenerationConfig,
)
from derouter.llms.vertex_ai.image_generation.vertex_imagen_transformation import (
...
class TestVertexAIGeminiImageGenerationConfig:
    ...
    def test_map_openai_params_image_config_non_dict_warns_and_drops(self):
        ...
        with patch("derouter.llms.vertex_ai.image_generation.vertex_gemini_transformation.verbose_logger") as mock_log:
            result = self.config.map_openai_params(
...
class TestVertexAIImagenImageGenerationConfig:
    ...
    def test_transform_image_generation_request_labels_from_metadata(self):
        """Billing labels from derouter_params.metadata.requester_metadata on predict body."""
        request = self.config.transform_image_generation_request(
            model="imagegeneration@006",
            prompt="A cat",
            optional_params={},
            derouter_params={"metadata": {"requester_metadata": {"team": "platform", "env": "prod"}}},
            headers={},
        )
