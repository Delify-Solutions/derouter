"""
Unit tests for ``VertexAIBatchPrediction`` (derouter/llms/vertex_ai/batches/handler.py).
...
and parses the JSON into ``DeRouterBatch`` (or the OpenAI list shape). POST-backed
...
We mock only true I/O / auth seams:
  ...
  * the httpx client factories (``_get_httpx_client`` /
    ...
    what URL/headers/body, and that the response is parsed into the derouter
...
from derouter.llms.vertex_ai.batches.handler import (  # noqa: E402
    VertexAIBatchPrediction,
)
from derouter.llms.vertex_ai.common_utils import VertexAIError  # noqa: E402
from derouter.types.utils import DeRouterBatch  # noqa: E402

HMOD = "derouter.llms.vertex_ai.batches.handler"
...
def _make_handler() -> VertexAIBatchPrediction:
    ...
    """
    h = VertexAIBatchPrediction(gcs_bucket_name="derouter-testing-bucket")
...
def test_create_batch_sync_posts_and_parses():
    ...
    assert isinstance(out, DeRouterBatch)
...
ENDPOINT_CREATE_DATA = {
    "input_file_id": f"gs://bucket/derouter-vertex-files/endpoints/{ENDPOINT_ID}/file-uuid"
}
...
def test_retrieve_batch_sync_invokes_logging_pre_call():
    ...
    h = _make_handler()
    logging_obj = MagicMock(spec=DeRouterLogging)
