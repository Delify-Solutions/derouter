...
(derouter/llms/vertex_ai/batches/transformation.py).
...
responses back into ``DeRouterBatch`` / OpenAI list shapes. Unlike anthropic /
...
from derouter.llms.vertex_ai.batches.transformation import (  # noqa: E402
    VertexAIBatchTransformation,
)
from derouter.llms.vertex_ai.common_utils import (  # noqa: E402
    VertexAIError,
    _convert_vertex_datetime_to_openai_datetime,
)
from derouter.types.utils import DeRouterBatch  # noqa: E402

T = VertexAIBatchTransformation

INPUT_FILE = (
    "gs://derouter-testing-bucket/derouter-vertex-files/publishers/google/"
    "models/gemini-1.5-flash-001/e9412502-2c91-42a6-8e61-f5c294cc0fc8"
)

ENDPOINT_ID = "7768560373388541952"
ENDPOINT_INPUT_FILE = (
    f"gs://derouter-testing-bucket/derouter-vertex-files/endpoints/{ENDPOINT_ID}/"
...
def test_transform_openai_request_builds_full_vertex_job():
    with patch(
        "derouter.llms.vertex_ai.batches.transformation.uuid.uuid4",
        return_value="fixed-uuid",
    ):
        job = T.transform_openai_batch_request_to_vertex_ai_batch_request({"input_file_id": INPUT_FILE})

    assert job["displayName"] == "derouter-vertex-batch-fixed-uuid"
...
def test_transform_vertex_response_full_mapping():
    ...
    assert isinstance(batch, DeRouterBatch)
...
def test_get_gcs_uri_prefix_root():
    assert (
        T._get_gcs_uri_prefix_from_file("gs://derouter-testing-bucket/vtx_batch.jsonl") == "gs://derouter-testing-bucket"
    )


def test_get_gcs_uri_prefix_nested():
    assert (
        T._get_gcs_uri_prefix_from_file("gs://derouter-testing-bucket/batches/vtx_batch.jsonl")
        == "gs://derouter-testing-bucket/batches"
    )
...
def test_get_model_from_gcs_file_publisher_path_wins_over_endpoints_prefix():
    ...
    DeRouter appended after it."""
    uri = "gs://bucket/team-endpoints/999/derouter-vertex-files/publishers/google/models/gemini-1.5-flash-001/uuid"
...
def test_get_model_from_gcs_file_last_endpoints_segment_wins():
    ...
    DeRouter stored; an earlier prefix segment must not shadow it."""
    uri = f"gs://bucket/endpoints/999/derouter-vertex-files/endpoints/{ENDPOINT_ID}/uuid"
