...
from derouter.utils import ProviderConfigManager, get_optional_params_transcription
...
class TestGetCompleteUrl:
    def test_defaults_to_us_regional_host(self, config):
        url = config.get_complete_url(
            ...
            model="chirp_3",
            optional_params={},
            derouter_params={"vertex_project": "test-project"},
        )
    ...
    def test_uses_vertex_location_for_regional_host(self, config):
        url = config.get_complete_url(
            ...
            model="chirp_3",
            optional_params={},
            derouter_params={"vertex_project": "test-project", "vertex_location": "eu"},
        )
    ...
    def test_global_location_uses_unprefixed_host(self, config):
        url = config.get_complete_url(
            ...
            model="chirp_3",
            optional_params={},
            derouter_params={"vertex_project": "test-project", "vertex_location": "global"},
        )
    ...
    def test_valid_location_netloc_always_google(self, config, location, expected_netloc):
        url = config.get_complete_url(
            ...
            model="chirp_3",
            optional_params={},
            derouter_params={"vertex_project": "test-project", "vertex_location": location},
        )
    ...
    def test_malicious_location_is_rejected(self, config, malicious_location):
        ...
        with pytest.raises(VertexAIError):
            config.get_complete_url(
                ...
                model="chirp_3",
                optional_params={},
                derouter_params={"vertex_project": "test-project", "vertex_location": malicious_location},
            )
    ...
    def test_malicious_project_is_rejected(self, config, malicious_project):
        with pytest.raises(VertexAIError):
            config.get_complete_url(
                ...
                model="chirp_3",
                optional_params={},
                derouter_params={"vertex_project": malicious_project, "vertex_location": "us"},
            )
...
class TestValidateEnvironment:
    def test_sets_oauth_headers(self):
        ...
        headers = StubbedConfig().validate_environment(
            ...
            messages=[],
            optional_params={},
            derouter_params={"vertex_project": "resolved-project"},
        )
...
class TestProviderRouting:
    ...
    def test_unsupported_response_format_raises(self, response_format):
        with pytest.raises(derouter.utils.UnsupportedParamsError, match="response_format"):
            get_optional_params_transcription(
                model="chirp_3",
                custom_llm_provider="vertex_ai",
                response_format=response_format,
...
class TestModelCostEntry:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "derouter/model_prices_and_context_window_backup.json",
        ],
    )
    def test_chirp_3_registered_as_audio_transcription(self, cost_map_path):
        ...
        assert entry["derouter_provider"] == "vertex_ai"
