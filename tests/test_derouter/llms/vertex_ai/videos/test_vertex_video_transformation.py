...
from derouter.llms.openai.cost_calculation import video_generation_cost
from derouter.llms.vertex_ai.videos.transformation import (
    VertexAIVideoConfig,
...
class TestVertexAIVideoConfig:
    ...
    def test_get_complete_url(self):
        ...
        url = self.config.get_complete_url(
            model="vertex_ai/veo-002", api_base=None, derouter_params=derouter_params
        )
    ...
    def test_get_complete_url_missing_project(self, monkeypatch):
        ...
        with pytest.raises(ValueError, match="vertex_project is required"):
            self.config.get_complete_url(
                model="veo-002", api_base=None, derouter_params={}
            )
    ...
    def test_get_complete_url_default_location(self):
        ...
        url = self.config.get_complete_url(
            model="veo-002", api_base=None, derouter_params=derouter_params
        )
    ...
    def test_veo_31_lite_provider_routing_from_local_model_map(
        ...
        vertex_video_models = {
            ...
            if info.get("derouter_provider") == "vertex_ai-video-models"
        }
        monkeypatch.setattr(derouter, "vertex_ai_video_models", vertex_video_models)
    ...
    def test_transform_video_create_request_with_image(self):
        ...
        with patch(
            "derouter.llms.vertex_ai.videos.transformation.ImageEditRequestUtils.get_image_content_type",
    ...
    def test_map_openai_size_does_not_infer_resolution_for_existing_veo_3(
        ...
        model_cost = _load_model_cost_map(BACKUP_MODEL_COST_PATH)
        monkeypatch.setitem(derouter.model_cost, model_key, dict(model_cost[model_key]))
