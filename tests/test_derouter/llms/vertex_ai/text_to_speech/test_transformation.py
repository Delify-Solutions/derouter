...
class TestVertexAITextToSpeechConfig:
    ...
    def test_transform_text_to_speech_request_body(self, mock_get_token, mock_ensure_token):
        ...
        # Test with voice dict in derouter_params (as set by dispatch)
...
class TestVertexAILyriaTextToSpeechConfig:
    ...
    def test_get_complete_url_for_lyria_3(self):
        ...
        url = config.get_complete_url(
            ...
            api_base=None,
            derouter_params={"vertex_project": "music-project"},
        )
    ...
    def test_derouter_speech_dispatches_to_lyria_api(
        self,
        model,
        response_format,
        response_json,
        ...
        with (
            patch.object(  # test-quality-ok: derouter.speech has no seam for Vertex token minting
                VertexAILyriaTextToSpeechConfig,
                "_ensure_access_token",
                return_value=("mock-token", "music-project"),
            ),
            patch(  # test-quality-ok: derouter.speech has no seam for the HTTP handler
                "derouter.llms.custom_httpx.llm_http_handler.HTTPHandler.post",
                return_value=mock_response,
            ) as mock_post,
        ):
            response = derouter.speech(
...
@patch("derouter.llms.custom_httpx.llm_http_handler.HTTPHandler.post")
@patch.object(VertexAITextToSpeechConfig, "_ensure_access_token")
@patch.object(VertexAITextToSpeechConfig, "_get_token_and_url")
def test_derouter_speech_vertex_ai_chirp(mock_get_token, mock_ensure_token, mock_post):
    """
    Test that derouter.speech(model="vertex_ai/chirp") sends the correct URL and request body
    """
    # Mock authentication
    mock_ensure_token.return_value = ("mock-token", "test-project")
    mock_get_token.return_value = ("mock-token", "mock-url")
