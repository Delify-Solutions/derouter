...
class TestInternalToolName:
    def test_code_execution_tool_name_is_stable(self):
        assert DeRouterInternalTools.CODE_EXECUTION.value == "derouter_code_execution"

    def test_enum_is_str_subclass_so_it_serializes_as_the_bare_name(self):
        assert isinstance(DeRouterInternalTools.CODE_EXECUTION, str)


class TestOpenAIToolSchema:
    def test_schema_matches_openai_function_tool_contract_exactly(self):
        assert get_derouter_code_execution_tool() == {
            "type": "function",
            "function": {
    ...
    def test_returns_a_fresh_dict_each_call_so_callers_cannot_mutate_the_shared_one(self):
        first = get_derouter_code_execution_tool()
        first["function"]["name"] = "clobbered"
        assert get_derouter_code_execution_tool()["function"]["name"] == "derouter_code_execution"

    def test_singleton_matches_the_factory(self):
        assert DEROUTER_CODE_EXECUTION_TOOL == get_derouter_code_execution_tool()


class TestAnthropicToolSchema:
    def test_schema_matches_anthropic_messages_tool_contract_exactly(self):
        assert get_derouter_code_execution_tool_anthropic() == {
            "name": "derouter_code_execution",
            "description": _DESCRIPTION,
            "input_schema": {
                "type": "object",
    ...
    def test_anthropic_shape_is_flat_and_carries_no_openai_only_keys(self):
        tool = get_derouter_code_execution_tool_anthropic()
    ...
    def test_returns_a_fresh_dict_each_call(self):
        get_derouter_code_execution_tool_anthropic()["name"] = "clobbered"
        assert get_derouter_code_execution_tool_anthropic()["name"] == "derouter_code_execution"

    def test_both_surfaces_agree_on_name_and_description(self):
        openai_tool = get_derouter_code_execution_tool()
        anthropic_tool = get_derouter_code_execution_tool_anthropic()
