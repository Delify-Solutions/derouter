...
from derouter.proxy.common_utils.semantic_text_index import Vector, cosine_similarity

CALLER: Final = UserAPIKeyAuth(api_key="hashed-caller-key", team_id="team-1", user_id="user-1")

TRANSLATOR: Final = DeRouter_SkillsTable(
...
def _embedding_router() -> MagicMock:
    ...
    router.aembedding = AsyncMock(
        side_effect=lambda model, input, metadata: derouter.EmbeddingResponse(
            model=model,
...
class TestSkillSearchText:
    ...
    def test_missing_fields_fall_back_to_whatever_is_present(self) -> None:
        assert skill_search_text(DeRouter_SkillsTable(skill_id="bare", display_title="bare")) == "bare"

    def test_all_fields_absent_is_an_empty_string(self) -> None:
        assert skill_search_text(DeRouter_SkillsTable(skill_id="empty")) == ""

    def test_oversized_instructions_are_cut_so_one_skill_cannot_blow_up_the_embedding_batch(self) -> None:
        bloated = DeRouter_SkillsTable(
...
class TestSkillSearchIndex:
    ...
    async def test_least_recently_searched_skills_are_evicted_once_the_index_is_full(self) -> None:
        ...
        embedder = FixedDimensionEmbedder(3)
        newcomer = DeRouter_SkillsTable(skill_id="newcomer", display_title="Newcomer")
        await index.search("q", SKILLS, top_k=5, embed=embedder, embedding_model="m")
    ...
    async def test_deleted_skills_stop_occupying_the_index_after_enough_new_ones(self) -> None:
        ...
        for generation in range(50):
            skill = DeRouter_SkillsTable(skill_id=f"gen-{generation}", display_title=f"Generation {generation}")
...
def accessible_skills(monkeypatch: pytest.MonkeyPatch) -> AsyncMock:
    ...
    monkeypatch.setattr(
        "derouter.llms.derouter_proxy.skills.handler.DeRouterSkillsHandler.list_skills_for_search", list_for_search
    )
...
def key_limits(monkeypatch: pytest.MonkeyPatch) -> MagicMock:
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.proxy_logging_obj", limits)
    return limits
...
def embedding_router(monkeypatch: pytest.MonkeyPatch, key_limits: MagicMock) -> MagicMock:
    ...
    monkeypatch.setattr(derouter, "skill_search_embedding_model", "text-embedding-3-small")
    return router
...
class TestHandleSkillSearchMCP:
    ...
    async def test_top_k_is_clamped_to_the_same_ceiling_as_rest(
        ...
        from derouter.llms.derouter_proxy.skills.skill_search import MAX_SKILL_SEARCH_TOP_K
        from derouter.proxy._experimental.mcp_server.tool_search import handle_skill_search
