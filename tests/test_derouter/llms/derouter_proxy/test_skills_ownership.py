...
from derouter.llms.derouter_proxy.skills import handler as skills_handler
...
from derouter.proxy.common_utils import resource_ownership
from derouter.skills import main as skills_main
...
def _skill(skill_id: str, created_by: str | None) -> DeRouter_SkillsTable:
    return DeRouter_SkillsTable(
        skill_id=skill_id,
        display_title="skill",
...
def test_should_extract_skill_auth_from_supported_metadata_fields():
    ...
    assert (
        skills_main._get_user_api_key_auth_from_kwargs(
            {"metadata": {}, "derouter_metadata": {"user_api_key_auth": auth}}
        )
        is auth
    )
...
def test_should_forward_skill_auth_through_sdk_entrypoints(monkeypatch):
    ...
    monkeypatch.setattr(skills_main, "_get_derouter_skills_handler", lambda: handler)
...
async def test_should_forward_skill_auth_through_transformation_handler(monkeypatch):
    handler = DeRouterSkillsTransformationHandler()
    auth = UserAPIKeyAuth(user_id="user-1")

    create_skill = AsyncMock(return_value=_skill("derouter_skill_created", "user-1"))
    list_skills = AsyncMock(return_value=[_skill("derouter_skill_listed", "user-1")])
    get_skill = AsyncMock(return_value=_skill("derouter_skill_got", "user-1"))
    delete_skill = AsyncMock(return_value={"id": "derouter_skill_deleted"})
    monkeypatch.setattr(DeRouterSkillsHandler, "create_skill", create_skill)
    monkeypatch.setattr(DeRouterSkillsHandler, "list_skills", list_skills)
    monkeypatch.setattr(DeRouterSkillsHandler, "get_skill", get_skill)
    monkeypatch.setattr(DeRouterSkillsHandler, "delete_skill", delete_skill)
    ...
    deleted = await handler._async_delete_skill(
        "derouter_skill_deleted",
        user_api_key_dict=auth,
    )

    assert created.id == "derouter_skill_created"
    assert [skill.id for skill in listed.data] == ["derouter_skill_listed"]
    assert got.id == "derouter_skill_got"
    assert deleted.id == "derouter_skill_deleted"
...
async def test_should_store_team_owner_for_keys_without_user_id(monkeypatch):
    ...
    prisma_client = type(
        "Prisma", (), {"db": type("DB", (), {"derouter_skillstable": table})()}
    )()
    monkeypatch.setattr(
    ...
    skill = await DeRouterSkillsHandler.create_skill(
...
async def test_should_reject_skill_create_for_identityless_proxy_auth(monkeypatch):
    ...
    with pytest.raises(ValueError, match="identity scope"):
        await DeRouterSkillsHandler.create_skill(
            data=NewSkillRequest(display_title="skill"),
            user_api_key_dict=auth,
        )
...
async def test_should_filter_list_skills_to_authenticated_owner_scopes(monkeypatch):
    table = AsyncMock()
    table.find_many.return_value = [_skill("derouter_skill_owner", "user-1")]
    prisma_client = type(
        "Prisma", (), {"db": type("DB", (), {"derouter_skillstable": table})()}
    )()
    monkeypatch.setattr(
    ...
    skills = await DeRouterSkillsHandler.list_skills(user_api_key_dict=auth)

    assert [skill.skill_id for skill in skills] == ["derouter_skill_owner"]
...
async def test_should_hide_skill_from_different_owner(monkeypatch):
    table = AsyncMock()
    table.find_unique.return_value = _skill("derouter_skill_other", "user-2")
    prisma_client = type(
        "Prisma", (), {"db": type("DB", (), {"derouter_skillstable": table})()}
    )()
    monkeypatch.setattr(
    ...
    with pytest.raises(ValueError, match="Skill not found"):
        await DeRouterSkillsHandler.get_skill(
            "derouter_skill_other",
            user_api_key_dict=auth,
        )
...
async def test_should_hide_unowned_skill_by_default(monkeypatch):
    table = AsyncMock()
    table.find_unique.return_value = _skill("derouter_skill_unowned", None)
    prisma_client = type(
        "Prisma", (), {"db": type("DB", (), {"derouter_skillstable": table})()}
    )()
    monkeypatch.setattr(
...
async def test_list_skills_excludes_unowned_for_non_admin(monkeypatch):
    ...
    auth = UserAPIKeyAuth(user_id="user-1")
    await DeRouterSkillsHandler.list_skills(user_api_key_dict=auth)
...
async def test_should_scope_skill_injection_fetch_to_authenticated_user(monkeypatch):
    ...
    monkeypatch.setattr(DeRouterSkillsHandler, "fetch_skill_from_db", fetch)
...
async def test_load_skill_uses_cache_after_first_db_hit(monkeypatch):
    ...
    fake_skill = Mock(created_by="user-1", skill_id="derouter_skill_a")
    table = AsyncMock()
    ...
    monkeypatch.setattr(
        skills_handler.DeRouterSkillsHandler,
        "_get_prisma_client",
        AsyncMock(return_value=prisma_client),
    )
    ...
    for _ in range(3):
        assert (
            await skills_handler.DeRouterSkillsHandler._load_skill("derouter_skill_a")
            is fake_skill
        )
...
async def test_load_skill_caches_negative_lookups(monkeypatch):
    ...
    assert await skills_handler.DeRouterSkillsHandler._load_skill("missing") is None
    assert await skills_handler.DeRouterSkillsHandler._load_skill("missing") is None
    assert table.find_unique.await_count == 1
...
async def test_delete_skill_invalidates_cache(monkeypatch):
    ...
    await skills_handler.DeRouterSkillsHandler._load_skill("derouter_skill_a")
    assert skills_handler._SKILL_CACHE.get_cache("derouter_skill_a") is fake_skill

    auth = UserAPIKeyAuth(user_id="user-1")
    await skills_handler.DeRouterSkillsHandler.delete_skill(
        "derouter_skill_a", user_api_key_dict=auth
    )

    # Post-delete, the cache holds the negative sentinel — not the stale row.
    assert (
        skills_handler._SKILL_CACHE.get_cache("derouter_skill_a")
