...
from derouter.llms.tinyfish.search.transformation import (
...
class TestTransformSearchRequest:
    ...
    def test_arbitrary_param_passed_through(self):
        ...
        # The passthrough loop should forward it verbatim without DeRouter needing
        # to know about it.
        config = TinyfishSearchConfig()
        result = config.transform_search_request(
...
class TestGetCompleteUrl:
    def test_default_api_base(self):
        config = TinyfishSearchConfig()
        with patch(
            "derouter.llms.tinyfish.search.transformation.get_secret_str",
            return_value=None,
        ):
...
class TestTransformSearchResponse:
    ...
    def test_missing_required_fields_default_to_empty_string(self):
        # title/url/snippet are required by DeRouter's SearchResult schema.
    ...
    def test_top_level_future_extras_flow_through(self):
        ...
        # (design contract: no DeRouter code change needed for new fields).
        config = TinyfishSearchConfig()
        body = {
            "results": [
    ...
    def test_response_headers_future_headers_flow_through(self):
        ...
        # through without a DeRouter code change.
        config = TinyfishSearchConfig()
        mock_response = _make_mock_response(
            MOCK_TINYFISH_RESPONSE,
            headers={
    ...
    def test_response_headers_strips_x_derouter_spoof(self):
        # A provider setting `x-derouter-*` in its response must not be able to
        # spoof DeRouter-internal markers via _hidden_params["additional_headers"].
        # The raw copy preserves the header (opt-in debug view); the sanitized
        # copy prefixes it with `llm_provider-` so bare `x-derouter-*` markers
        # can't be spoofed (values still survive under the prefixed key for
        # observability).
        config = TinyfishSearchConfig()
        mock_response = _make_mock_response(
            MOCK_TINYFISH_RESPONSE,
            headers={"x-derouter-attempted-fallbacks": "spoofed", "X-Request-ID": "r1"},
        )
        result = config.transform_search_response(
            raw_response=mock_response, logging_obj=None
        )
        # Raw view still has the spoof.
        assert result._hidden_params["headers"]["x-derouter-attempted-fallbacks"] == "spoofed"
        # Sanitized view: the spoof survives only under the llm_provider- prefix
        # (never under the bare x-derouter-* key that DeRouter downstream trusts).
        additional = result._hidden_params["additional_headers"]
        assert "x-derouter-attempted-fallbacks" not in additional
        assert additional.get("llm_provider-x-derouter-attempted-fallbacks") == "spoofed"

    def test_fetch_field_rides_through_to_search_result(self):
        # Mirrors TinyFish Search's per-result `fetch` nested object.
        # Confirms `fetch=...` requests surface their content to DeRouter
...
class TestErrorHandling:
    ...
    def test_wrap_error_returns_attributed_baselm_exception_directly(self):
        ...
        # they hit BaseSearchConfig.get_error_class via DeRouter core.
        config = TinyfishSearchConfig()
    ...
    def test_schema_mismatch_wraps_with_attribution(self):
        ...
        # DeRouter's SearchResponse contract (e.g. missing top-level `results`),
