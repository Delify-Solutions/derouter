...
from derouter.llms.bedrock.chat.invoke_transformations.amazon_openai_transformation import (
    AmazonBedrockOpenAIConfig,
)
from derouter.llms.bedrock.chat.invoke_transformations.base_invoke_transformation import (
    AmazonInvokeConfig,
)
from derouter.llms.bedrock.messages.invoke_transformations.anthropic_claude3_transformation import (
    AmazonAnthropicClaudeMessagesConfig,
)
from derouter.llms.bedrock.request_metadata import (
...
def derouter_params(metadata_key, **metadata):
    return {metadata_key: dict(metadata)}


def converse_body(derouter_params_value, optional_params=None):
    return AmazonConverseConfig()._transform_request(
        model=MODEL,
        messages=MESSAGES,
        optional_params=dict(optional_params or {}),
        derouter_params=dict(derouter_params_value),
    )


def converse_body_async(derouter_params_value, optional_params=None):
...
def test_feature_off_by_default_leaves_body_and_headers_untouched(setting, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(derouter, "bedrock_request_metadata_fields", setting)
    params = derouter_params("metadata", spend_logs_metadata={"team": "x"}, **IDENTITY)
    ...
    assert BEDROCK_REQUEST_METADATA_HEADER not in AmazonInvokeConfig().validate_environment(
        headers={}, model=MODEL, messages=MESSAGES, optional_params={}, derouter_params=dict(params)
    )
...
def test_resolver_reads_both_metadata_variable_names(metadata_key, monkeypatch: pytest.MonkeyPatch):
    """`/v1/chat/completions` populates `metadata`; the DEROUTER_METADATA_ROUTES populate
    `derouter_metadata`. Reading only one silently forwards nothing on the other route."""
    monkeypatch.setattr(derouter, "bedrock_request_metadata_fields", ALL_FIELDS)
    params = derouter_params(metadata_key, spend_logs_metadata={"cost_center": "cc-1"}, **IDENTITY)
...
def test_invoke_messages_header_reads_both_metadata_variable_names(metadata_key, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(derouter, "bedrock_request_metadata_fields", ALL_FIELDS)
    params = derouter_params(metadata_key, **IDENTITY)

    headers, _ = AmazonAnthropicClaudeMessagesConfig().validate_anthropic_messages_environment(
        headers={}, model=MODEL, messages=MESSAGES, optional_params={}, derouter_params=params
    )
...
def test_identity_survives_a_caller_filling_every_slot(
    ...
    monkeypatch.setattr(derouter, "bedrock_request_metadata_fields", field_order)
    ...
    if client_source == "spend_logs_metadata":
        params, optional_params = derouter_params("metadata", spend_logs_metadata=client_pairs, **IDENTITY), {}
    else:
        params, optional_params = derouter_params("metadata", **IDENTITY), {"requestMetadata": client_pairs}
...
def test_a_field_repeated_in_the_allow_list_does_not_consume_a_client_slot(
    ...
    params = derouter_params("metadata", spend_logs_metadata={key: "v" for key in client_keys}, **IDENTITY)
...
def test_caller_cannot_forge_or_shadow_a_reserved_identity_key(
    ...
    if client_source == "spend_logs_metadata":
        params, optional_params = derouter_params("metadata", spend_logs_metadata=forged, **IDENTITY), {}
    else:
        params, optional_params = derouter_params("metadata", **IDENTITY), {"requestMetadata": forged}
...
def test_caller_supplied_violation_still_raises_bad_request(monkeypatch: pytest.MonkeyPatch):
    ...
    with pytest.raises(derouter.exceptions.BadRequestError):
        converse_body(
            derouter_params("metadata", **IDENTITY),
...
def test_non_string_and_absent_identity_values_are_dropped(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(derouter, "bedrock_request_metadata_fields", ALL_FIELDS + ["user_api_key_spend"])
    params = derouter_params("metadata", user_api_key_alias="prod-key", user_api_key_spend=1.25)
...
def test_email_is_separately_opt_in(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(derouter, "bedrock_request_metadata_fields", ["user_api_key_alias", "user_api_key_team_alias"])
    assert (
        "user_api_key_user_email"
        not in converse_body(derouter_params("metadata", **identity_with_email))["requestMetadata"]
    )

    monkeypatch.setattr(derouter, "bedrock_request_metadata_fields", ALL_FIELDS)
    assert converse_body(derouter_params("metadata", **identity_with_email))["requestMetadata"] == identity_with_email


def test_resolver_returns_none_when_nothing_survives(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(derouter, "bedrock_request_metadata_fields", ALL_FIELDS)
    assert resolve_bedrock_request_metadata(derouter_params=None) is None
    assert resolve_bedrock_request_metadata(derouter_params={"metadata": {"unrelated": "x"}}) is None


def test_invoke_header_is_json_encoded_and_signed(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(derouter, "bedrock_request_metadata_fields", ALL_FIELDS)
    params = derouter_params("metadata", spend_logs_metadata={"cost_center": "cc-1"}, **IDENTITY)
...
def test_a_caller_supplied_guardrail_header_still_wins(monkeypatch: pytest.MonkeyPatch):
    ...
    headers = AmazonInvokeConfig().validate_environment(
        ...
        derouter_params=derouter_params("metadata", **IDENTITY),
    )
...
def test_converse_still_sets_the_bearer_authorization_header(monkeypatch: pytest.MonkeyPatch):
    ...
    headers = AmazonConverseConfig().validate_environment(
        ...
        messages=MESSAGES,
        optional_params={},
        derouter_params=dict(derouter_params("metadata", **IDENTITY)),
...
def test_a_caller_cannot_forge_the_request_metadata_header(driver, caller_header_name, monkeypatch: pytest.MonkeyPatch):
    ...
    headers = driver({caller_header_name: FORGED}, derouter_params("metadata", **IDENTITY))
...
def test_a_caller_cannot_forge_the_header_when_the_resolver_yields_nothing(driver, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(derouter, "bedrock_request_metadata_fields", ALL_FIELDS)
    unresolvable = derouter_params("metadata", user_api_key_alias="O'Brien's key", user_api_key_team_alias="x" * 300)
...
def test_a_caller_cannot_keep_reserved_body_keys_when_the_resolver_yields_nothing(
    ...
    body = driver(derouter_params("metadata"), {"requestMetadata": {forged_key: "FORGED"}})
...
def test_caller_body_metadata_is_left_alone_when_forwarding_is_off(driver, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(derouter, "bedrock_request_metadata_fields", None)
    caller_supplied = {"user_api_key_team_alias": "caller-set", "cost_center": "cc-9"}

    body = driver(derouter_params("metadata", **IDENTITY), {"requestMetadata": caller_supplied})
...
def test_a_caller_header_is_left_alone_when_forwarding_is_off(driver, monkeypatch: pytest.MonkeyPatch):
    ...
    headers = driver({BEDROCK_REQUEST_METADATA_HEADER: FORGED}, derouter_params("metadata", **IDENTITY))
