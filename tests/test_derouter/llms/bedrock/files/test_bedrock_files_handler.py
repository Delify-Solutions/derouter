...
class TestBedrockFilesHandler:
    ...
    def test_should_parse_direct_managed_s3_uri(self):
        bucket, key = self.handler._parse_s3_uri(
            s3_uri="s3://safe-bucket/derouter-bedrock-files-model-id-abc.jsonl",
            configured_bucket_name="safe-bucket",
        )

        assert bucket == "safe-bucket"
        assert key == "derouter-bedrock-files-model-id-abc.jsonl"

    def test_should_parse_managed_batch_output_uri(self):
        bucket, key = self.handler._parse_s3_uri(
            s3_uri="s3://safe-bucket/derouter-batch-outputs/job/",
            configured_bucket_name="safe-bucket",
        )

        assert bucket == "safe-bucket"
        assert key == "derouter-batch-outputs/job/"
    ...
    def test_should_reject_arbitrary_bucket(self):
        with pytest.raises(ValueError, match="configured storage bucket"):
            self.handler._parse_s3_uri(
                s3_uri="s3://other-bucket/derouter-bedrock-files-model-id-abc.jsonl",
                configured_bucket_name="safe-bucket",
            )
    ...
    def test_should_reject_dot_segment_key(self):
        with pytest.raises(ValueError, match="invalid path segment"):
            self.handler._parse_s3_uri(
                s3_uri="s3://safe-bucket/derouter-bedrock-files/../secret.jsonl",
                configured_bucket_name="safe-bucket",
            )
    ...
    def test_should_extract_unified_managed_s3_uri(self):
        file_id = _encode_unified_file_id(
            "s3://safe-bucket/derouter-batch-outputs/job/output.jsonl"
        )
...
def test_should_forward_trusted_model_credentials_to_bedrock_provider_config():
    ...
    with patch.object(
        ...
        response = files_main.file_content(
            file_id="s3://safe-bucket/derouter-bedrock-files/file.jsonl",
            custom_llm_provider="bedrock",
            _derouter_internal_model_credentials=trusted_credentials,
        )

    assert response is mock_response
    derouter_params = mock_retrieve_file_content.call_args.kwargs["derouter_params"]
    assert derouter_params["_derouter_internal_model_credentials"] is trusted_credentials
    assert "s3_bucket_name" not in derouter_params
...
def test_should_forward_trusted_model_credentials_to_retrieve_provider_config():
    ...
    assert response is mock_response
    derouter_params = mock_retrieve_file.call_args.kwargs["derouter_params"]
    assert derouter_params["_derouter_internal_model_credentials"] is trusted_credentials
...
async def test_afile_content_assumes_role_with_external_id(monkeypatch):
    ...
    optional_params = {
        "_derouter_internal_model_credentials": MappingProxyType({"s3_bucket_name": "safe-bucket"}),
        "aws_region_name": "us-east-1",
        "aws_access_key_id": "AKIAFILESDOWNLOADCALLER",
        "aws_secret_access_key": "pod-caller-secret",
        "aws_role_name": "arn:aws:iam::999999999999:role/derouter-files-download-role",
        "aws_session_name": "derouter-files-download-session",
    ...
    with patch.object(boto3, "client", side_effect=fake_boto3_client) as mock_boto3_client:
        response = await BedrockFilesHandler().afile_content(
            file_content_request={"file_id": "s3://safe-bucket/derouter-bedrock-files-model-id-abc.jsonl"},
