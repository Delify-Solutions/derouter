...
from derouter.llms.bedrock.files.transformation import BedrockJsonlFilesTransformation
...
class TestBedrockFilesTransformation:
    ...
    def test_get_complete_file_url_respects_s3_region_name(self):
        """
        s3_region_name in derouter_params must be used when building the S3 URL.
        ...
        derouter_params = {
            "s3_bucket_name": "derouter-batch-352026",
        ...
        assert "derouter-batch-352026" in url
    ...
    def test_get_complete_file_url_sanitizes_untrusted_filename(self):
        ...
        url = config.get_complete_file_url(
            ...
            derouter_params={"s3_bucket_name": "safe-bucket"},
            data=create_file_data,
        )

        parsed_url = urlparse(url)
        object_key = unquote(parsed_url.path).split("/safe-bucket/", 1)[1]
        assert object_key.startswith("derouter-bedrock-files/")
    ...
    def test_batch_object_name_sanitizes_model_path(self):
        ...
        assert object_name.startswith("derouter-bedrock-files-")
    ...
    def _signed_upload_request(self, derouter_params: dict) -> dict:
        from derouter.llms.bedrock.files.transformation import BedrockFilesConfig
        ...
        request = config.transform_create_file_request(
            ...
            derouter_params={"s3_bucket_name": "derouter-batch-bucket", **derouter_params},
        )
    ...
    def test_create_file_response_reports_uploaded_object_size(self):
        ...
        config = BedrockFilesConfig()
        derouter_params: dict = {"s3_bucket_name": "derouter-batch-bucket"}
        jsonl_content = json.dumps(
            {
                "custom_id": "req-1",
                "method": "POST",
...
class TestBedrockFilesEmbeddingTransformation:
    ...
    def test_titan_v2_accepted_when_registry_silent(self, mocker):
        ...
        mocker.patch("derouter.get_model_info", side_effect=Exception("not mapped"))
        assert BedrockFilesConfig._is_titan_v2_embed_model(
            "us.amazon.titan-embed-text-v2:0"
        )
        assert BedrockFilesConfig._is_titan_v2_embed_model(
    ...
    def test_lookup_provider_specific_field_helper(self, mocker):
        ...
        mocker.patch("derouter.get_model_info", return_value="not a dict")
        assert (
            BedrockFilesConfig._lookup_provider_specific_field("anything", "any")
            is None
        )
        ...
        mocker.patch("derouter.get_model_info", return_value={"mode": "embedding"})
        assert (
            BedrockFilesConfig._lookup_provider_specific_field(
                "anything", "bedrock_invocation_schema"
            )
...
class TestBedrockFileContentTransformation:
    ...
    S3_URI = "s3://my-bucket/derouter-batch-outputs/job-123/input.jsonl.out"
    EXPECTED_URL = "https://s3.us-west-2.amazonaws.com/my-bucket/derouter-batch-outputs/job-123/input.jsonl.out"

    def _derouter_params(self) -> dict:
        return {
    ...
    def test_transform_file_content_request_signs_s3_get(self, monkeypatch):
        ...
        from derouter.llms.bedrock.files.transformation import (
            S3_SIGNED_GET_HEADERS_PARAM,
            BedrockFilesConfig,
        )

        monkeypatch.setenv("AWS_S3_BUCKET_NAME", "my-bucket")
        derouter_params = self._derouter_params()
        ...
        signed_headers = derouter_params[S3_SIGNED_GET_HEADERS_PARAM]
        content_hashes = {
            value
        ...
        with pytest.raises(ValueError, match="configured storage bucket"):
            BedrockFilesConfig().transform_file_content_request(
                file_content_request={
                    "file_id": "s3://other-bucket/derouter-batch-outputs/job/x.jsonl.out"
                },
                optional_params={},
                derouter_params=self._derouter_params(),
            )
    ...
    def test_extract_s3_uri_rejects_non_managed_file_id(self):
        ...
        with pytest.raises(ValueError, match="managed DeRouter S3 file id"):
    ...
    def _trusted(self, **deployment_derouter_params) -> dict:
        """Build the trusted snapshot the way the proxy does: deployment
        derouter_params funneled through ``CredentialDeRouterParams`` (the strict
        ...
        snapshot = CredentialDeRouterParams(**deployment_derouter_params).model_dump(
            exclude_none=True
        )
        params = self._derouter_params()
        params["_derouter_internal_model_credentials"] = MappingProxyType(snapshot)
        return params
    ...
    def test_retrieves_from_distinct_output_bucket(self, monkeypatch):
        ...
        url, _ = BedrockFilesConfig().transform_file_content_request(
            file_content_request={
                "file_id": "s3://out-bucket/derouter-batch-outputs/job/in.jsonl.out"
            },
            optional_params={},
            derouter_params=self._trusted(
        ...
        assert (
            url
            == "https://s3.us-west-2.amazonaws.com/out-bucket/derouter-batch-outputs/job/in.jsonl.out"
        )
    ...
    def test_output_bucket_falls_back_to_env(self, monkeypatch):
        ...
        url, _ = BedrockFilesConfig().transform_file_content_request(
            file_content_request={
                "file_id": "s3://env-out-bucket/derouter-batch-outputs/job/in.jsonl.out"
            },
            optional_params={},
            derouter_params=self._derouter_params(),
        )
        ...
        assert (
            url
            == "https://s3.us-west-2.amazonaws.com/env-out-bucket/derouter-batch-outputs/job/in.jsonl.out"
        )
    ...
    def test_input_bucket_still_validates_when_output_bucket_set(self, monkeypatch):
        ...
        url, _ = BedrockFilesConfig().transform_file_content_request(
            file_content_request={
                "file_id": "s3://in-bucket/derouter-batch-outputs/job/in.jsonl.out"
            },
            optional_params={},
            derouter_params=self._trusted(
                s3_bucket_name="in-bucket", s3_output_bucket_name="out-bucket"
        ...
        assert (
            url
            == "https://s3.us-west-2.amazonaws.com/in-bucket/derouter-batch-outputs/job/in.jsonl.out"
        )
    ...
    def test_bucket_resolved_from_trusted_model_credentials(self, monkeypatch):
        ...
        monkeypatch.delenv("AWS_S3_BUCKET_NAME", raising=False)
        derouter_params = self._derouter_params()
        derouter_params["_derouter_internal_model_credentials"] = MappingProxyType(
    ...
    def test_s3_region_name_wins_for_content_signing(self, monkeypatch):
        ...
        monkeypatch.setenv("AWS_S3_BUCKET_NAME", "my-bucket")
        derouter_params = self._derouter_params()
        derouter_params["s3_region_name"] = "eu-west-1"
        ...
        authorization = derouter_params[S3_SIGNED_GET_HEADERS_PARAM]["Authorization"]
    ...
    def test_validate_environment_merges_and_pops_signed_get_headers(self):
        ...
        assert S3_SIGNED_GET_HEADERS_PARAM not in derouter_params
    ...
    def test_file_content_end_to_end_sends_signed_get(self, monkeypatch):
        """derouter.file_content must issue a SigV4-signed GET and return the S3 object bytes."""
        import httpx
        import respx
        ...
        with respx.mock:
            ...
            response = derouter.file_content(
                file_id=self.S3_URI,
                custom_llm_provider="bedrock",
                **self._derouter_params(),
            )
    ...
    async def test_afile_content_end_to_end_sends_signed_get(self, monkeypatch):
        """Async variant: derouter.afile_content over the same signed GET path."""
        import httpx
        import respx
        ...
        with respx.mock:
            ...
            response = await derouter.afile_content(
                file_id=self.S3_URI,
                custom_llm_provider="bedrock",
                **self._derouter_params(),
            )
...
class TestBedrockFilesS3SignatureEncoding:
    ...
    def test_file_content_signs_spaced_object_key_the_way_s3_does(
        ...
        url, _ = BedrockFilesConfig().transform_file_content_request(
            file_content_request={
                "file_id": "s3://my-bucket/LLM AI Projects/derouter-bedrock-files-model-abc.jsonl"
            },
            optional_params={},
            derouter_params=derouter_params,
        )
        ...
        self._assert_signed_the_way_s3_reads_it(
            ...
            url=url,
            body=None,
            headers=derouter_params[S3_SIGNED_GET_HEADERS_PARAM],
        )
...
def test_sign_s3_request_assumes_role_with_external_id(monkeypatch):
    ...
    optional_params = {
        ...
        "aws_role_name": "arn:aws:iam::999999999999:role/derouter-files-put-role",
        "aws_session_name": "derouter-files-put-session",
    ...
    with patch.object(boto3, "client", return_value=FakeSTSClient()):
        signed_headers, _signed_body = BedrockFilesConfig()._sign_s3_request(
            ...
            api_base="https://s3.us-east-1.amazonaws.com/safe-bucket/derouter-bedrock-files-model-id-abc.jsonl",
            optional_params=optional_params,
        )
    ...
    request_params = _BedrockS3RequestParams.model_validate(
        {
            ...
            "aws_secret_access_key": "pod-caller-secret",
            "aws_role_name": "arn:aws:iam::999999999999:role/derouter-files-get-role",
            "aws_session_name": "derouter-files-get-session",
