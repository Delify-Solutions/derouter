...
from derouter.llms.bedrock.image_edit.handler import BedrockImageEdit
from derouter.llms.bedrock.image_edit.stability_transformation import (
...
def ensure_nova_canvas_image_edit_model_cost_flags(monkeypatch):
    """Routing uses ``supports_nova_canvas_image_edit`` on ``derouter.model_cost``.
    ...
    for key in (
        ...
        entry = derouter.model_cost.get(key) or {}
        ...
        monkeypatch.setitem(
            ...
            {
                **entry,
                "derouter_provider": entry.get("derouter_provider", "bedrock"),
...
def test_provider_config_manager_bedrock_dispatches_to_nova_transform_outpainting():
    ...
    body, _ = cfg.transform_image_edit_request(
        ...
        derouter_params={},  # type: ignore[arg-type]
        headers={},
    )
...
def test_prepare_request_bearer_token_never_runs_the_sigv4_credential_chain(monkeypatch):
    ...
    request = BedrockImageEdit()._prepare_request(
        ...
        prompt="make it warmer",
        optional_params={"aws_region_name": "us-west-2", "aws_profile_name": "derouter-no-such-aws-profile"},
        api_base=None,
        extra_headers=None,
        logging_obj=Mock(),
        api_key=None,
