...
def local_model_cost_map(monkeypatch):
    ...
    original_converse_models = set(derouter.bedrock_converse_models)
    ...
    try:
        derouter.bedrock_converse_models.update(
            key
            for key, value in derouter.model_cost.items()
            if isinstance(value, dict)
            and value.get("derouter_provider") == "bedrock_converse"
        )
        yield
    finally:
        derouter.bedrock_converse_models.clear()
        derouter.bedrock_converse_models.update(original_converse_models)
        derouter.get_model_info.cache_clear()
...
def test_proxy_cost_calculation_scenario():
    ...
    model = "derouter_proxy/bedrock/us.anthropic.claude-3-5-haiku-20241022-v1:0"

    # Test model info lookup works
    model_info = _get_model_info_helper(
        model=model, custom_llm_provider="derouter_proxy"
    )
    ...
    cost = completion_cost(
        completion_response=response, model=model, custom_llm_provider="derouter_proxy"
    )
