...
from derouter.llms.bedrock.common_utils import BedrockModelInfo, get_bedrock_chat_config
from derouter.llms.bedrock.chat.mantle.transformation import AmazonMantleConfig
...
def test_mantle_validate_environment_without_project_id():
    ...
    headers = config.validate_environment(
        ...
        messages=[{"role": "user", "content": "Hello"}],
        optional_params={},
        derouter_params={"aws_bedrock_project_id": None},
    )
...
def test_mantle_completion_sends_workspace_header_and_clean_body():
    ...
    with patch("derouter.llms.custom_httpx.http_handler.HTTPHandler.post", mock_post):
...
async def test_mantle_anthropic_messages_sends_workspace_header_and_clean_body():
    ...
    try:
        ...
        await derouter.close_derouter_async_clients()
