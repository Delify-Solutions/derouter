...
def test_bedrock_image_prepare_request_with_arn() -> None:
    ...
    with (
        patch(
            "derouter.llms.bedrock.image_generation.image_handler.BedrockImageGeneration._get_boto_credentials_from_optional_params"
        ),
        patch(
            "derouter.llms.bedrock.image_generation.image_handler.BedrockImageGeneration.get_request_headers"
        ),
    ):
