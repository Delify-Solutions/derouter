...
import derouter
from derouter.llms.bedrock.embed.twelvelabs_marengo_transformation import TwelveLabsMarengoEmbeddingConfig
...
# Test data
test_input = "Hello world from derouter"
...
def test_bedrock_embedding_with_api_key_bearer_token(model, input_type, embed_response):
    ...
    with patch.object(client, "post") as mock_post:
        ...
        response = derouter.embedding(**kwargs)

        assert isinstance(response, derouter.EmbeddingResponse)
...
def test_bedrock_embedding_with_sigv4():
    ...
    with patch(
        "derouter.llms.bedrock.embed.embedding.BedrockEmbedding.embeddings"
    ) as mock_bedrock_embed:
        mock_embedding_response = derouter.EmbeddingResponse()
...
def test_twelvelabs_input_type_parameter_mapping():
    ...
    with patch.object(client, "post") as mock_post:
        ...
        # Test with input_type parameter (new DeRouter parameter)
        response = derouter.embedding(
            model=model,
            input=test_input,
            client=client,
...
def test_bedrock_embedding_header_forwarding(model, embed_response):
    ...
    Relevant Issue: https://github.com/Delify-Solutions/derouter/pull/16042
    """
    client = HTTPHandler()
    test_api_key = "test-bearer-token-12345"
...
def test_load_credentials_assumes_role_with_external_id(monkeypatch):
    ...
    optional_params = {
        ...
        "aws_region_name": "us-east-1",
        "aws_role_name": "arn:aws:iam::999999999999:role/derouter-embed-role",
        "aws_session_name": "derouter-embed-session",
...
def test_marengo_3_text_image_without_media_source_is_a_bad_request():
    with pytest.raises(derouter.BadRequestError, match=r"text_image.*media_source"):
        derouter.embedding(
            model="bedrock/us.twelvelabs.marengo-embed-3-0-v1:0",
            input="a duck on water",
            aws_region_name="us-east-1",
