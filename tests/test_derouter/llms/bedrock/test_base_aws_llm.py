...
import derouter
from derouter.llms.bedrock.base_aws_llm import (
    AwsAuthError,
    BaseAWSLLM,
...
BASE_AWS_LLM_PATH = os.path.join(
    os.path.dirname(__file__), "../../../../derouter/llms/bedrock/base_aws_llm.py"
)
...
def test_web_identity_token_oidc_reference_still_resolved():
    ...
    env = _os_environ_without_aws_keys()
    with patch.dict(os.environ, env, clear=True), patch(
        "derouter.llms.bedrock.base_aws_llm.get_secret", return_value=None
    ):
...
def test_boto3_init_tracer_wrapping():
    ...
    Ensures observability of boto3 calls in derouter.
    """
...
def test_auth_functions_tracer_wrapping():
    ...
    Ensures observability of AWS authentication calls in derouter.
    """
    # Get the source code of base_aws_llm.py
    with open(BASE_AWS_LLM_PATH, "r") as f:
        content = f.read()
...
def test_get_aws_region_name_boto3_fallback():
    ...
    with patch("derouter.llms.bedrock.base_aws_llm.get_secret") as mock_get_secret:
...
def test_role_assumption_without_session_name():
    ...
    with patch("boto3.client", return_value=mock_sts_client):
        ...
        # Session name should be auto-generated with format "derouter-session-{timestamp}"
        assert call_args[1]["RoleSessionName"].startswith("derouter-session-")
...
def test_web_identity_token_sts_client_uses_build_sts_client_kwargs():
    ...
    with patch.dict(os.environ, {"AWS_REGION": "eu-west-1"}, clear=True):
        with patch("boto3.client", return_value=mock_sts_client) as mock_boto3_client:
            with patch(
                "derouter.llms.bedrock.base_aws_llm.get_secret",
...
def test_converse_handler_external_id_extraction():
    ...
    from derouter.llms.bedrock.chat.converse_handler import BedrockConverseLLM
    ...
    with patch.object(
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch("derouter.AmazonConverseConfig") as mock_config:
                    ...
                    with patch.object(
                        ...
                        with patch(
                            "derouter.llms.custom_httpx.http_handler._get_httpx_client"
                        ) as mock_client:
    ...
    def test_forwarded_headers_still_added_back_after_signing(self):
        prepped = self._prepare(
            headers={
                ...
                "user-agent": "derouter-test-client",
            },
            data=json.dumps({"messages": []}),
        )
        assert prepped.headers["anthropic-version"] == "bedrock-2023-05-31"
        assert prepped.headers["user-agent"] == "derouter-test-client"
