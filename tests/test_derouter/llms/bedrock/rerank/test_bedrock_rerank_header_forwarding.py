...
import derouter
from derouter.llms.bedrock.base_aws_llm import Boto3CredentialsInfo
...
def test_bedrock_rerank_header_forwarding_sync(model):
    ...
    with (
        ...
        patch(  # test-quality-ok: boto credential lookup needs live AWS; the HTTP boundary is already a MockTransport
            "derouter.llms.bedrock.rerank.handler.BedrockRerankHandler._get_boto_credentials_from_optional_params",
        ...
        try:
            ...
            assert isinstance(response, derouter.RerankResponse)
...
def test_bedrock_rerank_forwarded_headers_excluded_from_sigv4_signature():
    """
    A forwarded header like x-forwarded-for can be rewritten between DeRouter
...
async def test_bedrock_rerank_records_llm_api_duration():
    ...
    proxy can emit x-derouter-overhead-duration-ms / x-derouter-timing-* on /rerank."""
    import httpx
