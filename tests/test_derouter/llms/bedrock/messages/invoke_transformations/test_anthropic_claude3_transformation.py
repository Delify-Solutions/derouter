...
# Ensure the project root is on the import path so `derouter` can be imported when
...
async def test_bedrock_sse_wrapper_encodes_dict_chunks():
    ...
    async for chunk in cfg.bedrock_sse_wrapper(
        ...
        derouter_logging_obj=DeRouterLoggingObj(
            ...
            derouter_call_id="test_bedrock_sse_wrapper_encodes_dict_chunks",
...
async def test_bedrock_sse_wrapper_appends_error_event_when_stream_truncates_mid_tool_use():
    ...
    async for chunk in cfg.bedrock_sse_wrapper(
        ...
        derouter_logging_obj=DeRouterLoggingObj(
            ...
            call_type="chat",
            start_time=datetime.now(),
            derouter_call_id="test_bedrock_sse_wrapper_truncated_tool_use",
...
async def test_bedrock_sse_wrapper_no_error_event_when_stream_ends_with_message_stop():
    ...
    async for chunk in cfg.bedrock_sse_wrapper(
        ...
        derouter_logging_obj=DeRouterLoggingObj(
            ...
            call_type="chat",
            start_time=datetime.now(),
            derouter_call_id="test_bedrock_sse_wrapper_complete_stream",
...
async def test_bedrock_sse_wrapper_keeps_usage_in_message_start_and_message_delta():
    ...
    async for chunk in cfg.bedrock_sse_wrapper(
        ...
        derouter_logging_obj=DeRouterLoggingObj(
            ...
            call_type="chat",
            start_time=datetime.now(),
            derouter_call_id="test_bedrock_sse_wrapper_keeps_usage_in_both_events",
...
async def test_bedrock_sse_wrapper_preserves_cache_usage_with_invocation_metrics():
    ...
    async for chunk in cfg.bedrock_sse_wrapper(
        ...
        derouter_logging_obj=DeRouterLoggingObj(
            ...
            call_type="chat",
            start_time=datetime.now(),
            derouter_call_id="test_bedrock_sse_wrapper_preserves_cache_usage",
...
def test_ensure_bedrock_anthropic_messages_tool_names():
    ...
    assert request["tools"][0]["name"] == "derouter_unnamed_tool_0"
    assert request["tools"][1]["name"] == "derouter_unnamed_tool_1"
    assert request["tools"][2]["name"] == "derouter_unnamed_tool_2"
...
def test_bedrock_invoke_messages_transform_adds_name_when_tool_missing_name():
    ...
    assert result["tools"][0]["name"] == "derouter_unnamed_tool_0"
...
def test_bedrock_messages_strips_output_config():
    ...
    Regression test for: https://github.com/Delify-Solutions/derouter/issues/22797
    """
    ...
    with patch(
        "derouter.llms.bedrock.common_utils._bedrock_model_supports",
        return_value=False,
    ):
        result = cfg.transform_anthropic_messages_request(
            model="anthropic.claude-3-haiku-20240307-v1:0",
...
def test_bedrock_messages_strips_context_management():
    ...
    Bedrock Invoke when it carries only DeRouter-internal edits (e.g.
...
def test_bedrock_messages_preserves_compact_context_management_and_adds_beta():
    ...
    Ref: https://github.com/Delify-Solutions/derouter/issues/27532
    """
    from derouter.types.router import GenericDeRouterParams
...
def test_bedrock_messages_filters_unsupported_context_management_edits():
    """
    Mixed edit lists must drop the DeRouter-internal ``clear_thinking_20251015``
...
async def test_unified_bedrock_messages_cache_on_start_only_never_negative_cost():
    ...
    logging_obj = DeRouterLoggingObj(
        ...
        call_type="chat",
        start_time=datetime.now(),
        derouter_call_id="test_cache_on_start_only_never_negative_cost",
...
async def test_unified_bedrock_messages_sse_usage_and_cost_claude_sonnet_46():
    ...
    logging_obj = DeRouterLoggingObj(
        ...
        call_type="chat",
        start_time=datetime.now(),
        derouter_call_id="test_unified_bedrock_messages_sse_cost",
...
def test_bedrock_claude_4_8_plus_cost_map_entries_carry_mid_conversation_system_flag():
    ...
    missing = [
        ...
        and str(info.get("derouter_provider", "")).startswith("bedrock")
...
def local_beta_headers_config(monkeypatch):
    ...
    monkeypatch.setenv("DEROUTER_LOCAL_ANTHROPIC_BETA_HEADERS", "True")
    reload_beta_headers_config()
    yield
    monkeypatch.delenv("DEROUTER_LOCAL_ANTHROPIC_BETA_HEADERS", raising=False)
    reload_beta_headers_config()
...
def test_bedrock_messages_filters_clear_thinking_keeps_clear_tool_uses(
    ...
    """
    LIT-3393: ``clear_thinking_20251015`` remains DeRouter-internal (consumed via
    ...
    ], "clear_thinking_20251015 must still be stripped (DeRouter-internal)"
...
def test_filter_and_transform_beta_headers_passes_context_management_for_bedrock(
    ...
    Pinned to the bundled local config via ``DEROUTER_LOCAL_ANTHROPIC_BETA_HEADERS``
    ...
    """
    from derouter.anthropic_beta_headers_manager import filter_and_transform_beta_headers
...
def test_bedrock_messages_tool_search_model_map_flag_is_authoritative(local_model_cost_map, monkeypatch):
    ...
    monkeypatch.setitem(derouter.model_cost[model], "supports_tool_search", False)
...
def test_bedrock_messages_thinking_shape_follows_exact_bedrock_entry_flag(
    ...
    monkeypatch.setitem(derouter.model_cost[model], "supports_adaptive_thinking", False)
    derouter.get_model_info.cache_clear()
...
def test_bedrock_invoke_messages_rejects_server_web_search_tool(tool_type: str):
    ...
    with pytest.raises(derouter.BadRequestError) as exc_info:
        cfg.transform_anthropic_messages_request(
            model="us.anthropic.claude-haiku-4-5-20251001-v1:0",
            messages=[{"role": "user", "content": "search the web for derouter"}],
            anthropic_messages_optional_request_params={
                "max_tokens": 128,
    ...
    assert "https://router.delify.vn/docs/integrations/websearch_interception" in str(exc_info.value)
...
def test_bedrock_invoke_messages_allows_converted_websearch_function_tool():
    ...
    (derouter_web_search); that converted shape must pass through untouched."""
    from derouter.types.router import GenericDeRouterParams
    ...
    assert result["tools"][0]["name"] == "derouter_web_search"
...
async def test_bedrock_sse_wrapper_dispatches_logging_on_client_disconnect():
    ...
    logging_obj = DeRouterLoggingObj(
        ...
        call_type="chat",
        start_time=datetime.now(),
        derouter_call_id="test_bedrock_sse_wrapper_disconnect_logging",
        function_id="test_bedrock_sse_wrapper_disconnect_logging",
    )
    wrapped = cfg.bedrock_sse_wrapper(_gated_stream(), derouter_logging_obj=logging_obj, request_body={})
