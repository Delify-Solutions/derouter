...
import derouter
from derouter import ModelResponse, RateLimitError, completion
from derouter.llms.bedrock.chat.converse_transformation import AmazonConverseConfig
from derouter.types.llms.bedrock import ConverseTokenUsageBlock
...
def test_bedrock_converse_1h_cache_write_billed_at_1h_rate(monkeypatch):
    ...
    prompt_cost, completion_cost = derouter.cost_calculator.cost_per_token(model=model, usage_object=openai_usage)
    model_info = derouter.get_model_info(model=model)
...
def test_get_supported_openai_params_bedrock_converse():
    ...
    Note: This test is critical for routing, if we ever remove `derouter.BEDROCK_CONVERSE_MODELS`,
    please update this test to read `bedrock_converse` models from the model cost map.
    """
    for model in derouter.BEDROCK_CONVERSE_MODELS:
...
def test_tool_config_cachepoint_is_credited_only_where_it_is_placed(tools, expected_marker):
    ...
    data = AmazonConverseConfig()._transform_request_helper(
        ...
        optional_params=optional_params,
        messages=[{"role": "user", "content": "hi"}],
        derouter_params={"metadata": bucket, "derouter_metadata": None, "model_info": {"id": "dep-bedrock"}},
    )

    placed = "cachePoint" in json.dumps(data.get("toolConfig", {}))
    assert placed is (expected_marker is not None)
    assert bucket.get("derouter_gateway_injected_cache") == expected_marker
...
def test_parallel_tool_calls_config_kept_for_sonnet_5(monkeypatch):
    old_env = os.environ.get("DEROUTER_LOCAL_MODEL_COST_MAP")
    old_cost = derouter.model_cost
    monkeypatch.setenv("DEROUTER_LOCAL_MODEL_COST_MAP", "True")
    ...
    try:
        ...
        derouter.model_cost = old_cost
        if old_env is None:
            os.environ.pop("DEROUTER_LOCAL_MODEL_COST_MAP", None)
        else:
            monkeypatch.setenv("DEROUTER_LOCAL_MODEL_COST_MAP", old_env)
...
async def test_tool_message_search_results_maps_to_bedrock_search_result_block():
    ...
    messages = [
        ...
        },
        {
            ...
            "content": "Apptio is a company that makes calls to Bedrock using passthrough APIs via DeRouter",
            "search_results": [
                {
                    ...
                    "content": [
                        {
                            "text": "Apptio is a company that makes calls to Bedrock using passthrough APIs via DeRouter"
                        }
                    ],
...
def test_extract_search_results_text_counts_hidden_tool_payload():
    ...
    from derouter.derouter_core_utils.token_counter import token_counter
...
def test_empty_assistant_message_handling():
    ...
    """
    # Import the derouter module that factory.py uses to ensure we patch the correct reference
    import derouter.derouter_core_utils.prompt_templates.factory as factory_module
    from derouter.derouter_core_utils.prompt_templates.factory import (
        _bedrock_converse_messages_pt,
    )
    ...
    # Use patch to ensure we modify the derouter reference that factory.py actually uses
    # This avoids issues with module reloading during parallel test execution
    with patch.object(factory_module.derouter, "modify_params", True):
        result = _bedrock_converse_messages_pt(
            messages=messages,
            model="anthropic.claude-haiku-4-5-20251001-v1:0",
            llm_provider="bedrock_converse",
...
def test_thinking_with_max_completion_tokens():
    ...
    from derouter.constants import DEFAULT_MAX_TOKENS
...
def test_drop_thinking_param_when_thinking_blocks_missing():
    ...
    Related issue: https://github.com/Delify-Solutions/derouter/issues/14194
    """
    from derouter.utils import last_assistant_with_tool_calls_has_no_thinking_blocks

    # Save original modify_params setting
    original_modify_params = derouter.modify_params
    ...
    try:
        ...
        derouter.modify_params = original_modify_params
...
def test_supports_native_structured_outputs(monkeypatch):
    ...
    cost JSON (derouter.model_cost), not a hardcoded model set.
    """
    old_env = os.environ.get("DEROUTER_LOCAL_MODEL_COST_MAP")
    old_cost = derouter.model_cost
    monkeypatch.setenv("DEROUTER_LOCAL_MODEL_COST_MAP", "True")
...
def test_parallel_tool_calls_flag_decoupled_from_ttl_pricing(monkeypatch):
    ...
    """
    from derouter.llms.bedrock.common_utils import is_claude_4_5_on_bedrock

    config = AmazonConverseConfig()
    model = "anthropic.claude-parallel-tool-use-only"
    monkeypatch.setitem(derouter.model_cost, model, {"supports_parallel_tool_use_config": True})
...
def test_transform_response_with_both_json_tool_call_and_real_tool():
    ...
    Fixes https://github.com/Delify-Solutions/derouter/issues/18381
    """
    from derouter.llms.bedrock.chat.converse_transformation import AmazonConverseConfig
    from derouter.types.utils import ModelResponse
...
def test_cache_control_injection_tool_config_honors_ttl_for_supported_model(monkeypatch):
    ...
    reads `cache_creation_input_token_cost_above_1hr` from derouter.model_cost)
...
def test_cache_control_injection_tool_config_honors_ttl_for_regional_model_lacking_own_pricing(monkeypatch):
    ...
    derouter.model_cost = derouter.get_model_cost_map(url="")
    try:
        assert "cache_creation_input_token_cost_above_1hr" not in derouter.model_cost["jp.anthropic.claude-opus-4-7"]
        assert "cache_creation_input_token_cost_above_1hr" in derouter.model_cost["anthropic.claude-opus-4-7"]
        config = AmazonConverseConfig()
        messages = [
            {"role": "user", "content": "What is the weather?"},
        ]
...
def test_tool_config_cachepoint_not_placed_or_credited_for_model_without_prompt_caching(monkeypatch):
    ...
    assert "derouter_gateway_injected_cache" not in bucket
...
def test_transform_response_does_not_leak_body_on_parse_failure():
    ...
    with patch(
        "derouter.llms.bedrock.chat.converse_transformation.ConverseResponseBlock",
...
def test_adaptive_thinking_dropped_when_max_tokens_too_small_converse():
    ...
    from derouter.constants import ANTHROPIC_MIN_THINKING_BUDGET_TOKENS
