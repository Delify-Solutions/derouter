...
class TestAgentCoreAcceptHeader:
    ...
    def test_accept_header_in_completion_request_jwt(self):
        ...
        when using JWT auth through derouter.completion().
...
class TestAgentCoreStreamingJsonFallback:
    ...
    def test_sync_streaming_with_json_response(self):
        ...
        Exercises the full path through derouter.completion().
        """
        from derouter.llms.custom_httpx.http_handler import HTTPHandler
    ...
    async def test_async_streaming_with_json_response(self):
        ...
        derouter.acompletion(stream=True).
        """
...
class TestAgentCoreMultimodalContent:
    ...
    When the ``forward_multimodal_content`` derouter param is set, the OpenAI
    ...
    def test_forward_flag_via_derouter_params(self, config, transform_kwargs):
        """The opt-in flag is also honored when set in derouter_params."""
        content = [
            {"type": "text", "text": "hi"},
            {
                "type": "image_url",
        ...
        kwargs = {
            **transform_kwargs,
            "derouter_params": {"forward_multimodal_content": True},
        }
        payload = config.transform_request(messages=messages, **kwargs)
        assert payload["content"] == content
        assert payload["content"] is not content
