...
from derouter.llms.bedrock.realtime.handler import BedrockRealtime
from derouter.llms.bedrock.realtime.transformation import BedrockRealtimeConfig
...
class FakeLogging:
    def __init__(self, trace_id="trace-nova-sonic"):
        self.derouter_trace_id = trace_id
...
class TestBedrockRealtimeHandler:
    ...
    async def test_logged_event_types_star_collects_every_forwarded_event(self, monkeypatch):
        monkeypatch.setattr(derouter, "logged_real_time_event_types", "*")
        handler = BedrockRealtime()
        stream = ScriptedBedrockStream(
            [
                json.dumps({"event": {"userSpeechStart": {}}}),
    ...
    async def test_trailing_usage_after_last_done_is_dispatched_for_spend(self, stub_aws_sdk_client, monkeypatch):
        import derouter.llms.bedrock.realtime.handler as handler_module
...
class TestBedrockRealtimeAwsAuth:
    """AWS auth params passed via derouter_params must reach the Smithy client config (LIT-3923 regression)"""

    @pytest.mark.asyncio
    async def test_static_credentials_from_derouter_params_reach_smithy_config(self, stub_aws_sdk_client):
        handler = BedrockRealtime()
        websocket = RealtimeClientWS()
        ...
        await handler.async_realtime(
            ...
            logging_obj=FakeLogging(),
            aws_region_name="us-east-1",
            aws_access_key_id="derouter-params-access-key",
            aws_secret_access_key="derouter-params-secret-key",
            aws_session_token="derouter-params-session-token",
        )

        config_kwargs = stub_aws_sdk_client["config_kwargs"]
        assert config_kwargs["aws_access_key_id"] == "derouter-params-access-key"
        assert config_kwargs["aws_secret_access_key"] == "derouter-params-secret-key"
        assert config_kwargs["aws_session_token"] == "derouter-params-session-token"
