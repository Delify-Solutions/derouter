...
def _reset_bedrock_response_stream_shape_cache():
    ...
    import derouter.llms.bedrock.common_utils as mod
...
def test_bedrock_response_stream_shape_loaded_on_first_access():
    ...
    from derouter.llms.bedrock.common_utils import get_bedrock_response_stream_shape
...
def test_bedrock_event_stream_decoder_base_uses_module_shape():
    ...
    """
    from derouter.llms.bedrock.common_utils import BedrockEventStreamDecoderBase
...
def test_context_window_suffix_stripped_for_cost_lookup():
    ...
    """
    from derouter.llms.bedrock.common_utils import get_bedrock_base_model
...
def test_capability_lookups_fall_back_to_base_model_when_regional_entry_lacks_field(monkeypatch):
    ...
    regional = f"eu.{base}"
    monkeypatch.setitem(derouter.model_cost, regional, {"input_cost_per_token": 1e-06})
    monkeypatch.setitem(
        derouter.model_cost,
        base,
        {
...
def test_merge_bedrock_aws_request_params_strips_caller_identity_when_deployment_has_static_credentials():
    from derouter.llms.bedrock.common_utils import merge_bedrock_aws_request_params
...
def test_merge_bedrock_aws_request_params_keeps_caller_credentials_without_static_deployment_credentials():
    ...
    merged = merge_bedrock_aws_request_params(
        derouter_params={"aws_region_name": "us-west-2"},
        optional_params={
            "aws_access_key_id": "caller-key",
...
def test_sign_aws_request_assumes_role_with_external_id(monkeypatch):
    ...
    optional_params = {
        ...
        "aws_role_name": "arn:aws:iam::999999999999:role/derouter-batch-sign-role",
        "aws_session_name": "derouter-batch-sign-session",
    ...
    with patch.object(boto3, "client", return_value=FakeSTSClient()):
        signed_headers, signed_data = CommonBatchFilesUtils().sign_aws_request(
            ...
            data={"jobName": "derouter-batch-job"},
    ...
    assert signed_data == b'{"jobName": "derouter-batch-job"}'
...
def _bedrock_chat_error_configs():
    from derouter.llms.bedrock.chat.agentcore.transformation import AmazonAgentCoreConfig
    from derouter.llms.bedrock.chat.converse_transformation import AmazonConverseConfig
    from derouter.llms.bedrock.chat.invoke_agent.transformation import AmazonInvokeAgentConfig
    from derouter.llms.bedrock.chat.invoke_transformations.amazon_moonshot_transformation import (
...
def test_error_response_text_reads_a_read_response():
    ...
    from derouter.llms.bedrock.common_utils import error_response_text
...
def _bedrock_httpx_status_error_sites():
    ...
    sites = []
    for path in sorted(pathlib.Path("derouter/llms/bedrock").rglob("*.py")):
...
def _bedrock_mantle_error_configs():
    from derouter.llms.bedrock_mantle.chat.transformation import BedrockMantleChatConfig
    from derouter.llms.bedrock_mantle.responses.transformation import BedrockMantleResponsesAPIConfig
...
def _bedrock_configs_with_get_error_class():
    ...
    llms_root = pathlib.Path(inspect.getfile(derouter)).parent / "llms"
    configs = []
    for package in ("bedrock", "bedrock_mantle"):
        for path in sorted((llms_root / package).rglob("*.py")):
            module_name = "derouter.llms." + ".".join(path.relative_to(llms_root).with_suffix("").parts)
