...
from derouter.llms.bedrock.batches.handler import (  # noqa: E402
...
OUTPUT_PREFIX = "s3://my-bucket/derouter-batch-outputs/derouter-bedrock-files-qwen-uuid/"
...
def _fake_boto3_response(status: str = "Completed", end_time=END_TIME):
    return {
        "jobArn": JOB_ARN,
        "jobName": "derouter-bedrock-files-qwen-uuid",
...
def test_derouter_cancel_batch_dispatches_to_bedrock(patched_boto3):
    import derouter

    fake_client, _ = patched_boto3
    fake_client.get_model_invocation_job.return_value = _fake_boto3_response(status="Stopped")

    batch = derouter.cancel_batch(batch_id=JOB_ARN, custom_llm_provider="bedrock")
