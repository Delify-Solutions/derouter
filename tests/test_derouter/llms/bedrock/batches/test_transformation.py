...
def test_create_request_builds_s3_input_output_and_arn(config):
    with patch.object(
        ...
        return_value="derouter-batch-deadbeef",
    ...
    assert bedrock_request["jobName"] == "derouter-batch-deadbeef"
    ...
    assert (
        ...
        == "s3://out-bucket/derouter-batch-outputs/derouter-batch-deadbeef/"
    )
...
def test_create_request_defaults_output_bucket_to_input_bucket(config):
    with patch.object(
        config.common_utils,
        "generate_unique_job_name",
        return_value="derouter-batch-cafef00d",
    ), patch.object(config.common_utils, "sign_aws_request") as mock_sign:
        ...
        config.transform_create_batch_request(
            ...
            optional_params={},
            derouter_params={"aws_batch_role_arn": "arn:aws:iam::1:role/r"},
        )
    bedrock_request = mock_sign.call_args.kwargs["data"]
    assert (
        bedrock_request["outputDataConfig"]["s3OutputDataConfig"]["s3Uri"]
        == "s3://same-bucket/derouter-batch-outputs/derouter-batch-cafef00d/"
    )
...
def test_create_request_omits_kms_key_when_absent(config):
    with patch.object(
        ...
        "derouter.llms.bedrock.common_utils.get_secret_str",
        return_value=None,
    ):
        mock_sign.return_value = ({}, b"{}")
        config.transform_create_batch_request(
...
def test_create_request_forwards_bedrock_tags_from_derouter_params(config):
    tags = [
...
def test_create_request_empty_derouter_params_tags_do_not_fall_through(config):
    with patch.object(
        config.common_utils,
        "generate_unique_job_name",
        return_value="derouter-batch-1",
...
# --------------------------------------------------------------------------- #
# transform_create_batch_response - status mapping + DeRouterBatch shape
# --------------------------------------------------------------------------- #
...
def test_retrieve_request_builds_encoded_arn_url(config):
    with patch.object(config.common_utils, "sign_aws_request") as mock_sign:
        ...
        result = config.transform_retrieve_batch_request(
            batch_id=ARN, optional_params={}, derouter_params={}
        )
...
def test_retrieve_request_rejects_non_arn(config):
    with pytest.raises(ValueError, match="Expected ARN"):
        config.transform_retrieve_batch_request(
            batch_id="abc1234567", optional_params={}, derouter_params={}
        )
...
def test_retrieve_request_rejects_short_arn(config):
    with pytest.raises(ValueError, match="Invalid ARN format"):
        config.transform_retrieve_batch_request(
            batch_id="arn:aws:bedrock:us-west-2", optional_params={}, derouter_params={}
        )
...
def test_retrieve_request_rejects_bad_region(config):
    ...
    with pytest.raises(ValueError, match="Invalid region in ARN"):
        config.transform_retrieve_batch_request(
            batch_id=bad, optional_params={}, derouter_params={}
        )
...
def test_retrieve_response_enriches_metadata(config):
    ...
    assert out.metadata["jobName"] == "derouter-batch-1"
...
from tests.test_derouter.llms.base_llm.batches.base_batches_config_test import (  # noqa: E402
