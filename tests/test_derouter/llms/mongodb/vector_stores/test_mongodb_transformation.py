...
from derouter.llms.mongodb.vector_stores.transformation import MongoDBVectorStoreConfig
from derouter.types.utils import EmbeddingResponse
from derouter.types.vector_stores import VectorStoreSearchOptionalRequestParams, VectorStoreSearchResponse

BASE_PARAMS: Final = {
    "api_base": "https://sidecar.example/prefix",
    "api_key": "test-sidecar-key",
    "derouter_embedding_model": "embedding-alias",
...
async def test_search_preserves_embedding_and_http_contract(
    ...
    params: Final = {
        **BASE_PARAMS,
        ...
        "derouter_embedding_config": {"dimensions": 3},
        "timeout": 0.75,
    }
    kwargs: Final = {
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("  ", {}, {}),
        ("x" * 32_001, {}, {}),
        ("travel", {"derouter_embedding_model": None}, {}),
...
def test_invalid_search_is_rejected_before_embedding(
    ...
    executor: Final = RecordingEmbeddingExecutor()
    config: Final = MongoDBVectorStoreConfig(executor)
    with pytest.raises(derouter.BadRequestError) as error:
        config.transform_search_vector_store_request(
            ...
            api_base=BASE_PARAMS["api_base"],
            derouter_logging_obj=MagicMock(),
            derouter_params={**BASE_PARAMS, **overrides},
        )
...
@pytest.mark.parametrize(
    ...
    [
        (400, {"error": {"message": "Index is not queryable"}}, derouter.BadRequestError),
        (401, {}, derouter.AuthenticationError),
        (408, {}, derouter.Timeout),
        (503, {}, derouter.ServiceUnavailableError),
        (200, {}, derouter.ServiceUnavailableError),
        (200, {**RESULT, "data": [{"score": "wrong"}]}, derouter.ServiceUnavailableError),
        (0, {}, derouter.Timeout),
        (-1, {}, derouter.BadRequestError),
        (-2, {"api_base": "http://sidecar.example"}, derouter.BadRequestError),
        (-2, {"api_base": "http://10.0.0.10:8080"}, derouter.BadRequestError),
        (-2, {"api_base": "http://localhost:8080"}, derouter.BadRequestError),
        (200, RESULT, None),
    ],
)
@pytest.mark.parametrize("asynchronous", [False, True])
...
async def test_public_sdk_preserves_http_errors_response_and_timeout(
    ...
    if status == -1:
        if asynchronous:
            with pytest.raises(derouter.BadRequestError, match="search-only"):
                await derouter.vector_stores.acreate(custom_llm_provider="mongodb")
        else:
            with pytest.raises(derouter.BadRequestError, match="search-only"):
                derouter.vector_stores.create(custom_llm_provider="mongodb")
        return
    if status == -2:
        rejected_params: Final = {**BASE_PARAMS, "api_base": str(body["api_base"])}
        if asynchronous:
            with pytest.raises(derouter.BadRequestError, match="requires HTTPS"):
                await derouter.vector_stores.asearch(
                    vector_store_id="policy_index",
                    query="travel policy",
    ...
    async with httpx.AsyncClient(transport=httpx.MockTransport(respond)) as async_transport:
        with httpx.Client(transport=httpx.MockTransport(respond)) as transport:
            ...
            async def search() -> VectorStoreSearchResponse:
                ...
                if asynchronous:
                    return await derouter.vector_stores.asearch(**kwargs)
                return derouter.vector_stores.search(**kwargs)
