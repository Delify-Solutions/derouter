...
from derouter.llms.langflow.a2a import merge_a2a_session_into_derouter_params


def test_merge_a2a_session_into_derouter_params():
    merged = merge_a2a_session_into_derouter_params(
...
def test_merge_a2a_session_is_scoped_per_principal():
    ...
    key_a = merge_a2a_session_into_derouter_params(base, params, "hash-a")["session_id"]
    key_a_again = merge_a2a_session_into_derouter_params(base, params, "hash-a")[
        "session_id"
    ]
    key_b = merge_a2a_session_into_derouter_params(base, params, "hash-b")["session_id"]
...
async def test_langflow_a2a_config_passes_session_id_to_completion():
    from derouter.a2a_protocol.providers.langflow.config import LangFlowA2AConfig
...
@pytest.mark.asyncio
async def test_langflow_a2a_config_requires_derouter_params_non_streaming():
    from derouter.a2a_protocol.providers.langflow.config import LangFlowA2AConfig

    with pytest.raises(ValueError, match="derouter_params is required"):
        await LangFlowA2AConfig().handle_non_streaming(
            request_id="req-1",
...
@pytest.mark.asyncio
async def test_langflow_a2a_config_requires_derouter_params_streaming():
    from derouter.a2a_protocol.providers.langflow.config import LangFlowA2AConfig
