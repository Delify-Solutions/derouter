...
import derouter
from derouter.llms.gdc.chat.transformation import GDCGeminiConfig
...
class TestGDCGeminiConfig:
    ...
    def test_get_complete_url_preformed_base_is_authoritative_over_derouter_params(self):
        config = GDCGeminiConfig()
    ...
    def test_validate_environment_strips_audience_trailing_slash(self):
        ...
        with patch(
            ...
            config.validate_environment(
                ...
                messages=[],
                optional_params={},
                derouter_params={"vertex_project": TEST_PROJECT},
                api_key=TEST_API_KEY,
    ...
    def test_validate_environment_missing_api_base(self, monkeypatch):
        ...
        monkeypatch.setattr(derouter, "gdc_api_base", None, raising=False)
        config = GDCGeminiConfig()
        with pytest.raises(Exception, match="api_base/host is required for GDC Gemini"):
            config.validate_environment(
                headers={},
    ...
    def test_validate_environment_quota_header_pinned_to_preformed_url(self):
        ...
        with patch(
            ...
            headers = config.validate_environment(
                ...
                messages=[],
                optional_params={"vertex_project": "attacker-proj"},
                derouter_params={"vertex_project": "override-proj"},
                api_key=TEST_API_KEY,
                api_base=preformed,
            )
    ...
    def test_transform_request(self):
        ...
        data = config.transform_request(
            ...
            derouter_params={"ssl_verify": True},
            headers={},
        )
...
class TestCompleteGDC:
    @patch("derouter.main.base_llm_http_handler.completion")
    def test_complete_gdc_resolves_key_and_base(self, mock_completion, monkeypatch):
        from derouter.main import gdc_transformation

        mock_completion.return_value = MagicMock()
        monkeypatch.setattr(derouter, "gdc_key", "resolved-key", raising=False)
        monkeypatch.setattr(
            derouter, "gdc_api_base", "https://resolved-base.com", raising=False
        )
        monkeypatch.setattr(derouter, "api_base", None, raising=False)
    ...
    def test_complete_gdc_prefers_gdc_api_base_over_global(
        ...
        monkeypatch.setattr(derouter, "gdc_key", "resolved-key", raising=False)
        monkeypatch.setattr(
            derouter, "gdc_api_base", "https://gdc-specific.com", raising=False
        )
        monkeypatch.setattr(
            derouter, "api_base", "https://other-provider.com", raising=False
        )
