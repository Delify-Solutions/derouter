...
async def test_sap_chat(
    ...
    with (
        patch(
            "derouter.llms.sap.chat.transformation.GenAIHubOrchestrationConfig.deployment_url",
            new_callable=PropertyMock,
            return_value=fake_deployment_url,
        ),
        patch(
            "derouter.llms.sap.chat.transformation.get_token_creator",
        ...
        if sync_mode:
            response = derouter.completion(model=model, messages=messages)
        else:
...
async def test_sap_streaming(
    ...
    with (
        ...
        stream = derouter.completion(model=model, messages=messages, stream=True)
