...
from derouter.integrations.code_interpreter_interception.handler import (
    CodeInterpreterInterceptionLogger,
    DEROUTER_CODE_EXECUTION_TOOL_NAME,
)
from derouter.llms.base_llm.audio_transcription.transformation import (
    AudioTranscriptionRequestData,
    BaseAudioTranscriptionConfig,
)
from derouter.llms.base_llm.chat.transformation import BaseConfig, BaseLLMException
...
from derouter.llms.custom_httpx.llm_http_handler import (
    BaseLLMHTTPHandler,
...
from derouter.types.utils import ImageObject, ImageResponse, ModelResponse, TranscriptionResponse
...
def test_response_api_handler_runs_responses_pre_call_hook_before_transform():
    ...
    old_callbacks = list(derouter.callbacks)
    derouter.callbacks = [CodeInterpreterInterceptionLogger()]
    try:
        response = handler.response_api_handler(
            model="gpt-5",
            input="use code",
            ...
            custom_llm_provider="openai",
            derouter_params=GenericDeRouterParams(api_key="sk-test"),
            logging_obj=logging_obj,
            client=client,
        )
    finally:
        derouter.callbacks = old_callbacks
    ...
    assert any(
        tool.get("type") == "function" and tool.get("name") == DEROUTER_CODE_EXECUTION_TOOL_NAME for tool in tools
    )
    hook_derouter_params = transform_kwargs["derouter_params"]
    assert hook_derouter_params.get(_ACTIVE_KEY) is True
    assert hook_derouter_params.get(_SANDBOX_KEY)
...
async def test_async_response_api_handler_streaming_passes_logging_obj_to_post():
    ...
    x-derouter-overhead-duration-ms (the non-streaming site is pinned by
...
async def test_async_responses_records_llm_api_duration():
    ...
    x-derouter-overhead-duration-ms on /v1/responses (mirrors the arerank regression test)."""
    ...
    response = await derouter.aresponses(
    ...
    assert response._hidden_params["derouter_overhead_time_ms"] is not None
    assert response._hidden_params["_response_ms"] >= response._hidden_params["derouter_overhead_time_ms"]
...
def test_has_agentic_completion_hook_detection(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [_PlainLogger()])
    assert handler._has_agentic_completion_hook(logging_obj) is False
    ...
    monkeypatch.setattr(derouter, "callbacks", [_AgenticLogger()])
    ...
    monkeypatch.setattr(derouter, "callbacks", [_DerivedAgenticLogger()])
    assert handler._has_agentic_completion_hook(logging_obj) is True
    ...
    monkeypatch.setattr(derouter, "callbacks", ["fake_string_callback"])
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(derouter, "callbacks", ["unknown_callback"])
    monkeypatch.setattr(
        "derouter.derouter_core_utils.derouter_logging.get_custom_logger_compatible_class",
        lambda name: None,
    )
...
async def test_async_anthropic_messages_handler_extra_headers():
    ...
    with patch(
        "derouter.derouter_core_utils.get_provider_specific_headers.ProviderSpecificHeaderUtils.get_provider_specific_headers"
...
@pytest.mark.asyncio
async def test_async_anthropic_messages_handler_passes_derouter_metadata():
    """Ensure derouter_metadata from kwargs is forwarded via update_from_kwargs.

    Routes like /messages store model_info under kwargs['derouter_metadata'].
    ...
    assert "derouter_metadata" in kwargs_arg
    assert kwargs_arg["derouter_metadata"]["model_info"] == custom_model_info


@pytest.mark.asyncio
async def test_async_anthropic_messages_handler_forwards_router_model_info():
    """Ensure router deployment model_info is forwarded into derouter_params.
    ...
    look up the deployment id via kwargs['derouter_params']['model_info']['id'].
    If async_anthropic_messages_handler builds its own derouter_params dict
    ...
    call_kwargs = mock_logging_obj.update_from_kwargs.call_args
    derouter_params_arg = (
        call_kwargs.kwargs.get("derouter_params", call_kwargs[1].get("derouter_params", {}))
        if call_kwargs.kwargs
        else call_kwargs[1].get("derouter_params", {})
    )

    assert derouter_params_arg.get("model_info") == deployment_model_info
...
def test_google_genai_streaming_hidden_params_model_info_and_router_fallback():
    ...
    from_model_info = _google_genai_streaming_hidden_params(
        ...
        derouter_params=GenericDeRouterParams(model_info={"id": "info-id"}),
        logging_obj=logging_obj,
...
def test_async_delete_responses_omits_body_for_azure():
    ...
    async def run():
        with patch.object(AsyncHTTPHandler, "delete", new=fake_async_delete):
            await derouter.adelete_responses(
...
@pytest.mark.parametrize(
    "derouter_params_kwargs, stream, global_timeout, expected",
    [
...
def test_resolve_anthropic_messages_timeout(
    monkeypatch, derouter_params_kwargs, stream, global_timeout, expected
):
    from derouter.constants import DEFAULT_REQUEST_TIMEOUT_SECONDS

    if global_timeout is None:
        monkeypatch.setattr(
            "derouter.request_timeout",
            float(DEFAULT_REQUEST_TIMEOUT_SECONDS),
            raising=False,
        )
        monkeypatch.setattr(
            "derouter.request_timeout_explicitly_set",
            False,
            raising=False,
        )
    else:
        monkeypatch.setattr("derouter.request_timeout", global_timeout, raising=False)
        monkeypatch.setattr(
            "derouter.request_timeout_explicitly_set", True, raising=False
        )

    resolved = BaseLLMHTTPHandler._resolve_anthropic_messages_timeout(
        derouter_params=GenericDeRouterParams(**derouter_params_kwargs),
        stream=stream,
        custom_llm_provider="anthropic",
    )
...
async def test_async_anthropic_messages_handler_forwards_request_timeout(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [])
    monkeypatch.setattr(derouter, "request_timeout", float(DEFAULT_REQUEST_TIMEOUT_SECONDS))
    monkeypatch.setattr(derouter, "request_timeout_explicitly_set", False)
    handler = BaseLLMHTTPHandler()
    ...
    result = await handler.async_anthropic_messages_handler(
        ...
        anthropic_messages_optional_request_params={},
        custom_llm_provider="anthropic",
        derouter_params=GenericDeRouterParams(request_timeout=0.3),
        logging_obj=logging_obj,
        client=mock_client,
        kwargs={},
    )
...
async def test_async_anthropic_messages_handler_forwards_stream_timeout(monkeypatch):
    ...
    await handler.async_anthropic_messages_handler(
        ...
        anthropic_messages_optional_request_params={},
        custom_llm_provider="anthropic",
        derouter_params=GenericDeRouterParams(timeout=9.0, stream_timeout=0.7),
        logging_obj=logging_obj,
        client=mock_client,
        stream=True,
        kwargs={},
...
def _make_responses_handler_call(signed_body):
    ...
    handler.response_api_handler(
        ...
        derouter_params=GenericDeRouterParams(aws_region_name="us-east-2"),
...
async def test_async_anthropic_messages_handler_passes_api_key_to_agentic_hooks():
    ...
    with (
        ...
        patch("derouter.llms.custom_httpx.llm_http_handler.get_async_httpx_client"),
        patch("derouter.derouter_core_utils.get_provider_specific_headers.ProviderSpecificHeaderUtils.get_provider_specific_headers", return_value=None),
    ):
        result = await handler.async_anthropic_messages_handler(
            model="claude-haiku",
            messages=[{"role": "user", "content": "hi"}],
            ...
            custom_llm_provider="anthropic",
            derouter_params=GenericDeRouterParams(api_key="sk-real-anthropic-key"),
            logging_obj=mock_logging_obj,
...
class _JSONBodyAudioTranscriptionConfig(BaseAudioTranscriptionConfig):
    ...
    def get_complete_url(self, api_base, api_key, model, optional_params, derouter_params, stream=None):
        return "https://transcription.example/recognize"

    def transform_audio_transcription_request(self, model, audio_file, optional_params, derouter_params):
...
async def test_async_retrieve_file_content_raises_on_http_error():
    ...
    with pytest.raises(AnthropicError) as exc_info:
        await handler.async_retrieve_file_content(
            ...
            derouter_params={"api_key": "sk-test"},
            headers={},
            logging_obj=Mock(),
            client=client,
        )
...
async def test_aget_responses_surfaces_upstream_error_status_instead_of_500():
    ...
    with pytest.raises(derouter.NotFoundError) as excinfo:
        await derouter.aget_responses(
...
async def test_alist_input_items_surfaces_upstream_error_status():
    ...
    with pytest.raises(derouter.NotFoundError) as excinfo:
        await derouter.alist_input_items(
            response_id="resp_abc",
            custom_llm_provider="azure",
            api_base="https://test.openai.azure.com",
            api_key="test-key",
...
async def test_anthropic_invalid_thinking_signature_retry_resigns_bedrock_request(monkeypatch):
    ...
    from derouter.llms.bedrock.messages.invoke_transformations.anthropic_claude3_transformation import (
...
def test_aws_signing_overrides_only_fills_missing_credentials():
    from derouter.llms.custom_httpx.llm_http_handler import _aws_signing_overrides
...
class TestServerFulfilledToolsInRequest:
    ...
    def test_interception_callbacks_declare_their_retrieval_tools(self):
        from derouter.integrations.compression_interception.handler import (
            DEROUTER_CONTENT_RETRIEVE_TOOL_NAME,
            CompressionInterceptionLogger,
        )
        from derouter.proxy.guardrails.guardrail_hooks.headroom.headroom import (
...
def test_vector_store_search_handler_direct_config_sync_skips_http():
    ...
    with patch("derouter.llms.custom_httpx.llm_http_handler._get_httpx_client") as mock_get_client:
        result = handler.vector_store_search_handler(
            ...
            derouter_params=GenericDeRouterParams(valkey_host="localhost"),
            logging_obj=logging_obj,
            timeout=12.5,
            _is_async=False,
        )
    ...
    assert isinstance(call["derouter_params"], dict)
    assert call["derouter_params"]["valkey_host"] == "localhost"
...
async def test_vector_store_search_handler_direct_config_async_skips_http():
    ...
    with patch("derouter.llms.custom_httpx.llm_http_handler.get_async_httpx_client") as mock_get_client:
    ...
    assert config.async_calls[0]["derouter_params"]["valkey_host"] == "localhost"
...
def _direct_vector_store_debug_logging_obj():
    ...
    logging_obj = LitellmLogging(
        ...
        derouter_call_id="vs-debug-call-id",
...
def test_direct_vector_store_search_debug_log_omits_stored_credentials(caplog, is_async):
    ...
    assert result is stub_response
    raw_request = logging_obj.model_call_details["derouter_params"]["metadata"]["raw_request"]
...
async def test_async_anthropic_messages_handler_carries_deployment_vertex_location_for_pricing(monkeypatch):
    ...
    async def logging_obj_after_handler(generic_params):
        logging_obj = Logging(
            ...
            derouter_call_id="vertex-messages-location",
            function_id="f",
        )
        logging_obj.update_environment_variables(
            model="vertex_ai/claude-haiku-4-5@20251001",
        ...
        with contextlib.suppress(Exception):
            await handler.async_anthropic_messages_handler(
                ...
                custom_llm_provider="vertex_ai",
                derouter_params=generic_params,
                logging_obj=logging_obj,
                client=AsyncMock(),
                kwargs={},
            )
    ...
    global_deployment = await logging_obj_after_handler(GenericDeRouterParams(vertex_location="global"))
    assert global_deployment.derouter_params["vertex_location"] == "global"
    assert (
        _resolve_vertex_location_for_cost(
            custom_llm_provider="vertex_ai",
            derouter_params=global_deployment.derouter_params,
    ...
    unconfigured_deployment = await logging_obj_after_handler(GenericDeRouterParams())
    assert "vertex_location" not in unconfigured_deployment.derouter_params
...
def test_the_rust_responses_websocket_needs_openai_and_process_enablement(
    ...
    from derouter.rust_bridge import configuration

    configuration.reset_rust_configuration()
    monkeypatch.setenv("DEROUTER_RUST", "1" if enabled else "0")
...
def test_a_callback_that_overrides_the_deployment_hook_is_detected(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [_DeploymentHookLogger()])
    assert _has_pre_call_deployment_hook(logging_obj) is True

    monkeypatch.setattr(derouter, "callbacks", [_InheritsTheHook()])
    assert _has_pre_call_deployment_hook(logging_obj) is True
...
def test_only_callbacks_that_can_charge_a_frame_are_collected_for_ws_quota(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [plain, decoy])
    assert _collect_ws_project_quota_callbacks() == ()

    monkeypatch.setattr(derouter, "callbacks", [plain, quota, decoy])
...
async def test_async_rerank_records_llm_api_duration():
    ...
    x-derouter-overhead-duration-ms / x-derouter-timing-* on /rerank."""
    ...
    response = await derouter.arerank(
...
def _video_create_call_kwargs(config, **optional_params):
    return {
        ...
        "custom_llm_provider": "openai",
        "derouter_params": GenericDeRouterParams(api_key="sk-test", api_base="https://video.example/v1"),
        "logging_obj": Mock(),
        "timeout": 10.0,
    }
...
def test_video_generation_without_file_sends_multipart_form_data():
    ...
    vLLM-Omni) reject the JSON body DeRouter used to send when no
...
def test_container_retrieve_handler_raises_upstream_error_status_and_message():
    from derouter.llms.openai.containers.transformation import OpenAIContainerConfig
...
async def test_async_container_list_handler_raises_upstream_error_status_and_message():
    ...
    with pytest.raises(BaseLLMException) as exc_info:
        await BaseLLMHTTPHandler().async_container_list_handler(
            container_provider_config=OpenAIContainerConfig(),
            derouter_params=GenericDeRouterParams(api_key="sk-rejected"),
            logging_obj=Mock(),
...
class _TransformRecordingConfig(BaseConfig):
    ...
    def validate_environment(
        self, headers, model, messages, optional_params, derouter_params, api_key=None, api_base=None
    ):
        return {}

    def transform_request(self, model, messages, optional_params, derouter_params, headers):
        self.transform_calls.append("sync")
        return {"transformed_by": "sync"}

    async def async_transform_request(self, model, messages, optional_params, derouter_params, headers):
    ...
    def get_model_response_iterator(self, streaming_response, sync_stream, json_mode=False):
        return derouter.OpenAIGPTConfig().get_model_response_iterator(
...
def _streaming_logging_obj():
    ...
    logging_obj = Logging(
        ...
        start_time=time.time(),
        derouter_call_id="async-transform-stream",
        function_id="f",
    )
    logging_obj.update_environment_variables(
        model="stub-model", user="", optional_params={}, derouter_params={}, custom_llm_provider="openai"
    )
    return logging_obj
...
class _ImageEditRecordingConfig(BaseImageEditConfig):
    ...
    def validate_environment(self, headers, model, api_key=None, derouter_params=None, api_base=None):
        return {}

    def get_complete_url(self, model, api_base, derouter_params):
    ...
    def transform_image_edit_request(
        self, model, prompt, image, image_edit_optional_request_params, derouter_params, headers
    ):
        self.transform_calls.append("sync")
        return {"transformed_by": "sync"}, []
