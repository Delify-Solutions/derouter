...
import derouter
from derouter.llms.custom_httpx.aiohttp_transport import DeRouterAiohttpTransport
...
async def test_async_post_streaming_status_error_should_not_wait_forever_for_body(
    ...
    monkeypatch.setattr(
        "derouter.llms.custom_httpx.http_handler._STREAMING_ERROR_BODY_READ_TIMEOUT_SECONDS",
        0.01,
    )

    derouter_handler = AsyncHTTPHandler()
    await derouter_handler.client.aclose()
    derouter_handler.client = httpx.AsyncClient(transport=httpx.MockTransport(mock_handler))
    try:
        with pytest.raises(MaskedHTTPStatusError) as exc_info:
            await asyncio.wait_for(
                derouter_handler.post(
...
def test_sync_post_streaming_status_error_should_not_wait_forever_for_body(
    ...
    derouter_handler = HTTPHandler()
    derouter_handler.client.close()
    derouter_handler.client = httpx.Client(transport=httpx.MockTransport(mock_handler))
    try:
        with pytest.raises(MaskedHTTPStatusError) as exc_info:
            derouter_handler.post(
                "https://vertex.example/streamRawPredict",
...
async def test_ssl_security_level(monkeypatch):
    ...
    with patch.dict(os.environ, clear=True):
        ...
        try:
            # Get the transport (should be DeRouterAiohttpTransport)
            transport = client.client._transport
            assert isinstance(transport, DeRouterAiohttpTransport)
...
async def test_force_ipv4_transport(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(derouter, "force_ipv4", True)
...
async def test_aiohttp_disabled_transport(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(derouter, "disable_aiohttp_transport", True)
    monkeypatch.setattr(derouter, "force_ipv4", False)
...
async def test_ssl_verification_with_aiohttp_transport(monkeypatch: pytest.MonkeyPatch):
    ...
    We validate that the ssl settings for a derouter transport match what a ssl verify=False aiohttp client would have.
    ...
    derouter_async_client = AsyncHTTPHandler(ssl_verify=False)

    try:
        transport = derouter_async_client.client._transport
        assert isinstance(transport, DeRouterAiohttpTransport)
        ...
        try:
            ...
            # assert both derouter transport and aiohttp session have ssl_verify=False
            assert transport_connector._ssl == aiohttp_connector._ssl
        finally:
            await aiohttp_session.close()
    finally:
        await derouter_async_client.close()
...
async def test_ssl_verification_with_shared_session(monkeypatch: pytest.MonkeyPatch):
    ...
    to the DeRouterAiohttpTransport constructor.
...
def test_get_ssl_configuration():
    ...
    from derouter.llms.custom_httpx.http_handler import _ssl_context_cache
...
def test_shared_session_parameter_in_acompletion():
    ...
    import inspect
    from derouter.main import acompletion
...
def test_shared_session_parameter_in_completion():
    ...
    import inspect
    from derouter.main import completion
...
@pytest.mark.parametrize(
    "env_curve,derouter_curve,expected_curve,should_call",
    [
        # env_curve: SSL_ECDH_CURVE env var | derouter_curve: derouter.ssl_ecdh_curve variable
        ...
        (None, "secp384r1", "secp384r1", True),  # derouter variable only
...
def test_ssl_ecdh_curve(env_curve, derouter_curve, expected_curve, should_call, monkeypatch):
    ...
    with patch.dict(os.environ, clear=True):
        ...
        monkeypatch.setattr(derouter, "ssl_ecdh_curve", derouter_curve)
...
def test_default_user_agent_is_derouter_version(monkeypatch):
    from derouter._version import version
    from derouter.llms.custom_httpx.http_handler import get_default_headers

    monkeypatch.delenv("DEROUTER_USER_AGENT", raising=False)

    assert get_default_headers()["User-Agent"] == f"derouter/{version}"


def test_user_agent_can_be_overridden_via_env_var(monkeypatch):
    from derouter.llms.custom_httpx.http_handler import get_default_headers

    monkeypatch.setenv("DEROUTER_USER_AGENT", "Claude Code")
...
def test_user_agent_env_var_can_be_empty_string(monkeypatch):
    ...
    monkeypatch.setenv("DEROUTER_USER_AGENT", "")
...
class TestDefaultCachedClientTimeoutHonorsRequestTimeout:
    """Cached default httpx clients must fall back to an explicit derouter.request_timeout.

    Regression for LIT-2369: get_async_httpx_client / _get_httpx_client hardcoded a
    600s default and never consulted derouter.request_timeout, so provider calls with
    ...
    def test_default_when_request_timeout_unset(self, monkeypatch: pytest.MonkeyPatch):
        ...
        monkeypatch.setattr(derouter, "request_timeout", derouter.constants.DEFAULT_REQUEST_TIMEOUT_SECONDS)
    ...
    def test_uses_explicit_request_timeout(self, monkeypatch: pytest.MonkeyPatch):
        ...
        monkeypatch.setattr(derouter, "request_timeout", 300)
        monkeypatch.setattr(derouter, "request_timeout_explicitly_set", True)
    ...
    def test_cached_async_client_built_with_explicit_request_timeout(self, monkeypatch: pytest.MonkeyPatch):
        ...
        monkeypatch.setattr(derouter, "request_timeout", 300)
        monkeypatch.setattr(derouter, "request_timeout_explicitly_set", True)
        derouter.in_memory_llm_clients_cache = LLMClientCache()
...
async def test_init_held_async_handler_survives_evicted_client_close():
    from derouter.caching.evicted_client_closer import EvictedClientCloser
    from derouter.caching.llm_caching_handler import LLMClientCache

    cache = LLMClientCache(evicted_client_closer=EvictedClientCloser(grace_seconds=0))
    handler = AsyncHTTPHandler(timeout=42.5)
    held_client = handler.client
    cache.set_cache("init-held-handler", handler, derouter_owned_client=True, ttl=0)
...
def fresh_llm_client_cache():
    ...
    previous = getattr(derouter, "in_memory_llm_clients_cache", None)
    derouter.in_memory_llm_clients_cache = LLMClientCache()
    try:
        yield
    finally:
        derouter.in_memory_llm_clients_cache = previous


def test_sole_referrer_handler_may_close_but_a_sharing_one_may_not():
    from derouter.llms.custom_httpx.http_handler import _handler_may_close_client
...
def test_client_handed_out_by_sync_cache_survives_eviction_and_collection(fresh_llm_client_cache):
    ...
    assert not consumer_client.is_closed
    assert derouter.in_memory_llm_clients_cache.get_cache("httpx_client") is handler

    derouter.in_memory_llm_clients_cache = LLMClientCache()
    del handler
    gc.collect()

    assert derouter.in_memory_llm_clients_cache.get_cache("httpx_client") is None
...
async def test_async_client_never_replays_one_upstreams_cookie_to_another():
    """DeRouter's async clients are pooled and shared by every caller, so a cookie one
...
async def test_aiohttp_session_never_replays_one_upstreams_cookie_to_another():
    """The httpx jar is not the only one. AiohttpTransport is derouter's default transport
    ...
    assert isinstance(transport, DeRouterAiohttpTransport), "aiohttp is no longer the default transport"
...
async def test_sync_close_helper_respects_session_ownership():
    ...
    assert isinstance(owned_transport, DeRouterAiohttpTransport)
    owned_session = owned_transport._get_valid_client_session()

    baseline = set(DeRouterAiohttpTransport._background_close_tasks)
    owned_handler._dispose_wrapped_aiohttp_session()
    scheduled = DeRouterAiohttpTransport._background_close_tasks - baseline
    await asyncio.gather(*scheduled)
    ...
    assert isinstance(shared_transport, DeRouterAiohttpTransport)
...
async def test_async_handler_honours_proxy_env_for_every_transport(
    ...
    monkeypatch.delenv("no_proxy", raising=False)
    monkeypatch.setattr(derouter, "disable_aiohttp_transport", disable_aiohttp_transport)
    monkeypatch.setattr(derouter, "force_ipv4", force_ipv4)
...
async def test_force_ipv4_httpx_transport_honours_no_proxy(keepalive_server, monkeypatch: pytest.MonkeyPatch):
    """NO_PROXY hosts must still go direct when the proxy mounts are supplied by derouter instead of httpx."""
