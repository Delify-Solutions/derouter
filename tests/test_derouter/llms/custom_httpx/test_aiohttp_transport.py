...
async def test_aclose_does_not_close_shared_session():
    ...
    try:
        transport = DeRouterAiohttpTransport(client=session, owns_session=False)
...
async def test_aclose_closes_owned_session():
    ...
    session = aiohttp.ClientSession()
    transport = DeRouterAiohttpTransport(client=session, owns_session=True)
    await transport.aclose()
...
async def test_handle_async_request_uses_env_proxy(monkeypatch):
    ...
    transport = DeRouterAiohttpTransport(client=lambda: FakeSession())  # type: ignore
...
async def test_handle_async_request_proxy_cache_per_host(monkeypatch):
    ...
    transport = DeRouterAiohttpTransport(client=factory)  # type: ignore
...
async def test_handle_async_request_sock_read_timeout_triggers():
    """
    Ensure that DeRouterAiohttpTransport raises httpx.TimeoutException
...
async def test_handle_async_request_streaming_does_not_timeout_on_total_duration():
    """
    Ensure that DeRouterAiohttpTransport does NOT timeout on long-running
...
async def test_closed_shared_session_rebuild_uses_injected_session_factory():
    ...
    transport = DeRouterAiohttpTransport(
...
async def test_fallback_recreate_closes_previous_session():
    ...
    transport = DeRouterAiohttpTransport(client=lambda: aiohttp.ClientSession())
    transport.client = old_session

    with patch(
        "derouter.llms.custom_httpx.aiohttp_transport.asyncio.get_running_loop",
...
async def test_close_task_strongly_referenced_until_done():
    ...
    assert DeRouterAiohttpTransport._background_close_tasks, "close task must be strongly referenced while pending"
    for _ in range(5):
        await asyncio.sleep(0)
    assert old_session.closed
    assert not DeRouterAiohttpTransport._background_close_tasks, "completed close tasks must be pruned from the registry"
...
def test_threadsafe_close_done_callback_tolerates_cancelled_future():
    ...
    DeRouterAiohttpTransport._on_threadsafe_close_done(future)
...
async def test_internal_dns_cancellation_maps_to_connect_error():
    ...
    transport = DeRouterAiohttpTransport(client=session)
    try:
