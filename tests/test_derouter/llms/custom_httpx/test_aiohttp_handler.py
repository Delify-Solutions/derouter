...
import derouter
from derouter.llms.custom_httpx.aiohttp_handler import BaseLLMAIOHTTPHandler
...
class TestBaseLLMAIOHTTPHandler:
    ...
    @patch("derouter.llms.custom_httpx.aiohttp_handler.aiohttp.ClientSession")
    ...
    def test_init_with_transport(self):
        ...
        mock_transport = Mock(spec=DeRouterAiohttpTransport)
    ...
    async def test_create_client_session_default_honors_global_ssl_verify_false(
        ...
        """Regression test for LIT-3369: `derouter.ssl_verify = False` (set via
        `derouter_settings.ssl_verify: false`) must reach the default session's
    ...
    def test_get_or_create_transport(self):
        ...
        if result is not None:
            ...
            assert isinstance(result, DeRouterAiohttpTransport)
        else:
