...
async def test_atexit_cleanup():
    ...
    # Use the actual global base_llm_aiohttp_handler from derouter
    handler = derouter.base_llm_aiohttp_handler
    session = handler._get_async_client_session()
...
def test_new_event_loop_atexit():
    ...
    try:
        new_loop.run_until_complete(close_derouter_async_clients())
    finally:
        new_loop.close()
