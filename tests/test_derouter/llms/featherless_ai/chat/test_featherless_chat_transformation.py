...
from derouter.llms.featherless_ai.chat.transformation import FeatherlessAIConfig
...
class TestFeatherlessAIConfig:
    ...
    def test_map_openai_params_with_tool_choice(self):
        ...
        optional_params = {}
        with pytest.raises(Exception, match="derouter\\.UnsupportedParamsError: Featherless AI doesn't") as excinfo:
    ...
    def test_featherless_ai_completion_mock(self, respx_mock):
                ...
                "choices": [
                    {
                        ...
                        "message": {
                            "role": "assistant",
                            "content": '```python\nprint("Hi from DeRouter!")\n```\n\nThis simple Python code prints a greeting message from DeRouter.',
                        },
                        "finish_reason": "stop",
                    }
                ],
        ...
        # Make the actual API call through DeRouter
        response = completion(
            model=model,
            messages=[
                {"role": "user", "content": "write code for saying hi from DeRouter"}
            ],
            api_key=api_key,
            api_base=api_base,
        )
        ...
        assert "Hi from DeRouter" in response.choices[0].message.content
