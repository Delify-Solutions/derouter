...
import derouter.llms.xai.oauth as xai_oauth_module
...
from derouter.utils import get_optional_params, validate_environment
...
def test_chat_config_wraps_flagged_oauth_errors_as_authentication_error(
    ...
    with pytest.raises(derouter.AuthenticationError) as exc_info:
        XAIChatConfig().validate_environment(
            ...
            messages=[],
            optional_params={},
            derouter_params={"use_xai_oauth": True},
            api_key=None,
        )

    assert exc_info.value.llm_provider == "xai"
    assert "derouter xai-oauth login" in str(exc_info.value)
...
def test_xai_oauth_flag_uses_xai_optional_param_mapping():
    derouter_params = GenericDeRouterParams(use_xai_oauth=True)
    ...
    assert derouter_params.use_xai_oauth is True
...
def test_responses_config_endpoint_url_uses_oauth_authenticator(monkeypatch):
    ...
    assert config.get_complete_url(
        api_base=None, derouter_params={"use_xai_oauth": True}
    ...
    assert (
        config.get_complete_url(
            api_base="https://custom.example.com/v1/",
            derouter_params={"api_key": "", "use_xai_oauth": True},
        )
        == "https://xai.example.com/v1/responses"
    )
    assert (
        config.get_complete_url(
            api_base="https://custom.example.com/v1/",
            derouter_params={"api_key": "caller-api-key"},
        )
...
def test_proxy_cli_xai_oauth_login_uses_single_authenticator(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.llms.xai.oauth.XAIOAuthAuthenticator", FakeAuthenticator
    )
