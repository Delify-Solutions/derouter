...
def test_surviving_xai_chat_models_still_serve_chat_completions(cost_map: dict):
    ...
    chat_models = [
        ...
        if isinstance(value, dict) and value.get("derouter_provider") == "xai" and value.get("mode") == "chat"
    ]
...
def test_both_cost_maps_agree_on_xai_entries():
    ...
    xai_keys = {k for k, v in prices.items() if isinstance(v, dict) and v.get("derouter_provider") == "xai"}
    assert xai_keys
