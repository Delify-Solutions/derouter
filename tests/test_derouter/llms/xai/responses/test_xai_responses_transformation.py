...
Source: derouter/llms/xai/responses/transformation.py
"""
...
import derouter
from derouter.llms.xai.cost_calculator import cost_per_token
...
class TestXAIResponsesAPITransformation:
    ...
    def test_xai_responses_endpoint_url(self):
        ...
        custom_url = config.get_complete_url(api_base="https://custom.x.ai/v1", derouter_params={})
        assert custom_url == "https://custom.x.ai/v1/responses", f"Expected custom endpoint, got {custom_url}"

        # Test with trailing slash
        url_with_slash = config.get_complete_url(api_base="https://api.x.ai/v1/", derouter_params={})
...
class TestXAIResponsesWebSearchBilling:
    ...
    def test_completion_cost_bills_web_search_calls(self):
        with_search = derouter.completion_cost(
            completion_response=self._transform(include_web_search=True),
            model="xai/grok-4",
            custom_llm_provider="xai",
        )
        without_search = derouter.completion_cost(
...
class TestXAIResponsesReportedCost:
    """xAI reports what it charged; the transformation moves it to where derouter bills from.
