...
import derouter
from derouter.llms.bedrock_mantle.responses.transformation import (
...
class TestBedrockMantleResponsesURL:
    def test_url_uses_region_from_env(self, monkeypatch):
        ...
        url = cfg.get_complete_url(api_base=None, derouter_params={})
    ...
    def test_url_region_from_aws_region_name_derouter_params(self, monkeypatch):
        monkeypatch.delenv("BEDROCK_MANTLE_REGION", raising=False)
        monkeypatch.delenv("BEDROCK_MANTLE_API_BASE", raising=False)
        ...
        url = cfg.get_complete_url(
            api_base=None,
            derouter_params={"aws_region_name": "us-east-2"},
        )
        assert url == "https://bedrock-mantle.us-east-2.api.aws/openai/v1/responses"
...
class TestBedrockMantleGetLlmProviderRegion:
    def test_get_llm_provider_uses_supplemental_derouter_params(
    ...
    def test_get_llm_provider_uses_aws_region_from_derouter_params(
        self, monkeypatch, local_cost_map
    ):
        monkeypatch.delenv("BEDROCK_MANTLE_REGION", raising=False)
        monkeypatch.delenv("BEDROCK_MANTLE_API_BASE", raising=False)
...
class TestBedrockMantleResponsesAuth:
    def test_config_api_key_takes_priority(self, monkeypatch):
        ...
        headers = cfg.validate_environment(
            ...
            derouter_params=GenericDeRouterParams(api_key="config-key"),
        )
    ...
    def test_env_key_fallback(self, monkeypatch):
        ...
        cfg = BedrockMantleResponsesAPIConfig()
        headers = cfg.validate_environment(
            headers={}, model="openai.gpt-5.5", derouter_params=GenericDeRouterParams()
        )
    ...
    def test_no_project_id_no_openai_project_header(self):
        ...
        headers = cfg.validate_environment(
            headers={},
            model="openai.gpt-5.5",
            derouter_params=GenericDeRouterParams(api_key="fake-key"),
        )
    ...
    def test_file_search_routes_to_emulation(self):
        ...
        # file_search so DeRouter's emulation handles it instead of forwarding.
...
class TestBedrockMantleResponsesTools:
    ...
    def test_dropped_tools_are_logged_at_warning_level(self):
        ...
        cfg = BedrockMantleResponsesAPIConfig()
        with patch(
            "derouter.llms.bedrock_mantle.responses.transformation.verbose_logger.warning"
        ) as mock_warning:
...
class TestBedrockMantleResponsesWebSearch:
    ...
    def test_cost_map_advertises_web_search_support(self, model):
        assert derouter.supports_web_search(model=model) is True
...
class TestBedrockMantleServiceTier:
    ...
    def test_unsupported_service_tier_raises_when_drop_params_false(self, tier):
        cfg = BedrockMantleResponsesAPIConfig()
        with pytest.raises(derouter.UnsupportedParamsError) as excinfo:
            cfg.map_openai_params(
                response_api_optional_params={"service_tier": tier},
                model="openai.gpt-5.5",
                drop_params=False,
...
class TestBedrockMantleCodexAdditionalTools:
    ...
    def test_hoist_is_logged_at_debug_level(self):
        ...
        with patch(
            "derouter.llms.bedrock_mantle.responses.transformation.verbose_logger.debug"
        ) as mock_debug:
            self._transform(
                input=[
                    {"type": "additional_tools", "role": "developer", "tools": self._CODEX_TOOLS},
...
class TestBedrockMantleResponsesRegistry:
    ...
    def test_unmapped_frontier_model_falls_through_to_none(self, restore_model_cost):
        ...
        derouter.model_cost.pop("bedrock_mantle/openai.gpt-6", None)
    ...
    def test_price_map_flag_routes_non_gpt_name_to_openai_path(
        ...
        from derouter.utils import ProviderConfigManager, register_model

        register_model(
            {
                "bedrock_mantle/somelab.frontier-x": {
                    "derouter_provider": "bedrock_mantle",
                    "mode": "responses",
                    "use_openai_responses_path": True,
                }
            }
    ...
    def test_gpt_5_5_price_map_declares_openai_responses_path(self, local_cost_map):
        ...
        assert (
            derouter.model_cost["bedrock_mantle/openai.gpt-5.5"].get(
                "use_openai_responses_path"
            )
            is True
        )
        assert (
            derouter.model_cost["bedrock_mantle/openai.gpt-5.4"].get(
                "use_openai_responses_path"
            )
            is True
        )
    ...
    def test_unmapped_model_degrades_to_none_without_crashing(self, restore_model_cost):
        ...
        derouter.model_cost.pop("bedrock_mantle/somelab.unmapped-model", None)
        derouter.get_model_info.cache_clear()
        cfg = ProviderConfigManager.get_provider_responses_api_config(
            provider="bedrock_mantle",
    ...
    def test_register_model_restore_undoes_existing_key_overwrite(self):
        ...
        snapshot = copy.deepcopy(derouter.model_cost)
        derouter.get_model_info.cache_clear()
        try:
            register_model(
                {
            ...
            derouter.model_cost.update(snapshot)
            derouter.get_model_info.cache_clear()
...
class TestMantleBaseSegment:
    ...
    def test_base_segment(self, model, model_cost, expected):
        from derouter.llms.bedrock_mantle.common_utils import mantle_base_segment
...
class TestMantleSupportsResponses:
    ...
    def test_supports_responses(self, model, model_cost, expected):
        from derouter.llms.bedrock_mantle.common_utils import mantle_supports_responses
...
class TestBedrockMantlePerModelResponsesURL:
    ...
    def _url_for(self, model, region="us-east-2"):
        ...
        return cfg.get_complete_url(
            api_base=None, derouter_params={"aws_region_name": region}
        )
...
def restore_model_cost():
    """Snapshot derouter.model_cost so register_model edits don't leak across tests.

    register_model mutates the global derouter.model_cost, and get_model_info is
    ...
    1. The snapshot must be a deepcopy. register_model overwrites an existing key
       via `derouter.model_cost.setdefault(key, {}).update(...)`, mutating the
    ...
    2. The restore must be in place (clear + update the SAME dict object), not a
       reassignment. The conftest autouse `isolate_derouter_state` fixture
       snapshots `derouter.model_cost` by reference and restores that reference on
       its teardown, which runs after this one. Reassigning `derouter.model_cost`
    ...
    """
    original_model_cost = copy.deepcopy(derouter.model_cost)
    derouter.get_model_info.cache_clear()
    try:
        yield
    finally:
        derouter.model_cost.clear()
        derouter.model_cost.update(original_model_cost)
        derouter.get_model_info.cache_clear()
...
def local_cost_map(monkeypatch):
    ...
    ``derouter.model_cost`` is populated once at import time (here, from the
...
class TestBedrockMantleResponsesSigV4:
    ...
    def test_bearer_arg_takes_priority_over_mantle_env_key(self, monkeypatch):
        # The passed api_key (e.g. derouter_params.api_key) must win over the env
    ...
    def test_assume_role_path_produces_sigv4_headers(self, monkeypatch):
        ...
        headers, _ = cfg.sign_request(
            ...
            optional_params={
                ...
                "aws_session_name": "derouter-test",
                "aws_region_name": "us-east-2",
            },
            request_data={"input": "hi"},
            api_base="https://bedrock-mantle.us-east-2.api.aws/openai/v1/responses",
        ...
        assert call["aws_session_name"] == "derouter-test"
        assert headers["Authorization"].startswith("AWS4-HMAC-SHA256")
        assert "/us-east-2/bedrock/aws4_request" in headers["Authorization"]
    ...
    def test_url_region_and_sigv4_region_agree_from_derouter_params(self, monkeypatch):
        ...
        cfg = BedrockMantleResponsesAPIConfig(aws_signer=BaseAWSLLM())
        url = cfg.get_complete_url(api_base=None, derouter_params=params)
        assert (
    ...
    def test_injected_default_region_base_does_not_override_aws_region_name(
        ...
        derouter_params.api_base = https://bedrock-mantle.<DEFAULT>.api.aws/v1 (default
        ...
        cfg = BedrockMantleResponsesAPIConfig(aws_signer=BaseAWSLLM())
        url = cfg.get_complete_url(api_base=injected_base, derouter_params=params)
        assert url == "https://bedrock-mantle.us-east-2.api.aws/openai/v1/responses"
...
class TestBedrockMantleResponsesPricing:
    ...
    def test_models_registered(self, local_cost_map):
        assert "bedrock_mantle/openai.gpt-5.5" in derouter.bedrock_mantle_models
        assert "bedrock_mantle/openai.gpt-5.4" in derouter.bedrock_mantle_models
