...
from derouter.llms.bedrock.passthrough.transformation import BedrockPassthroughConfig
from derouter.llms.bedrock_mantle.passthrough.transformation import BedrockMantlePassthroughConfig
from derouter.llms.custom_httpx.http_handler import HTTPHandler
from derouter.passthrough.main import llm_passthrough_route
...
def test_mantle_api_base_only_lends_its_region_to_the_runtime_url(no_ambient_aws):
    url, base_url = BedrockMantlePassthroughConfig().get_complete_url(
        ...
        derouter_params={"api_base": MANTLE_API_BASE},
    )
...
def test_explicit_region_and_non_mantle_api_base_are_kept(no_ambient_aws):
    ...
    url, base_url = BedrockMantlePassthroughConfig().get_complete_url(
        ...
        endpoint=INVOKE_ENDPOINT,
        request_query_params=None,
        derouter_params={"api_base": vpc_endpoint, "aws_region_name": "us-east-1"},
    )
...
def test_lookalike_mantle_host_api_base_is_kept(no_ambient_aws, lookalike_host):
    url, base_url = BedrockMantlePassthroughConfig().get_complete_url(
        ...
        endpoint=INVOKE_ENDPOINT,
        request_query_params=None,
        derouter_params={"api_base": lookalike_host},
    )
...
@pytest.mark.parametrize(
    ("derouter_params", "env", "expected_bearer"),
    [
...
def test_sign_request_uses_the_deployment_bearer_token(no_ambient_aws, monkeypatch, derouter_params, env, expected_bearer):
