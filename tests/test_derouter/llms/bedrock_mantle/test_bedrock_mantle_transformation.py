...
def local_cost_map(monkeypatch):
    ...
    original_bedrock_mantle_models = set(derouter.bedrock_mantle_models)
    try:
        ...
        derouter.bedrock_mantle_models.clear()
        derouter.bedrock_mantle_models.update(original_bedrock_mantle_models)
        derouter.get_model_info.cache_clear()
...
class TestBedrockMantleProviderRegistration:
    ...
    def test_provider_in_provider_list(self):
        assert "bedrock_mantle" in derouter.provider_list

    def test_models_loaded(self, monkeypatch):
        monkeypatch.setenv("DEROUTER_LOCAL_MODEL_COST_MAP", "true")
        derouter.add_known_models()
        assert len(derouter.bedrock_mantle_models) > 0
        assert "bedrock_mantle/openai.gpt-oss-120b" in derouter.bedrock_mantle_models
        assert "bedrock_mantle/openai.gpt-oss-20b" in derouter.bedrock_mantle_models
        assert (
            "bedrock_mantle/openai.gpt-oss-safeguard-120b"
            in derouter.bedrock_mantle_models
        )
        assert (
...
class TestBedrockMantleConfig:
    ...
    def test_aws_region_name_param_overrides_env(self, monkeypatch):
        ...
        api_base, _ = cfg._get_openai_compatible_provider_info(
            None, None, derouter_params=GenericDeRouterParams(aws_region_name="us-east-2")
        )
    ...
    def test_get_llm_provider_uses_aws_region_name_for_responses(
        ...
        monkeypatch.delenv("BEDROCK_MANTLE_API_BASE", raising=False)
        monkeypatch.delenv("AWS_REGION", raising=False)
        _, provider, _, api_base = derouter.get_llm_provider(
            model="openai.gpt-5.5",
            custom_llm_provider="bedrock_mantle",
...
class TestBedrockMantleChatAuth:
    ...
    def test_completion_no_bearer_signs_with_sigv4_end_to_end(self, monkeypatch):
        ...
        with patch(
            "derouter.llms.custom_httpx.http_handler.HTTPHandler.post", mock_post
        ):
...
class TestBedrockMantleProjectHeader:
    def test_validate_environment_sets_openai_project_header(self):
        ...
        headers = cfg.validate_environment(
            ...
            optional_params={},
            derouter_params={"aws_bedrock_project_id": "proj_abc123def456"},
            api_key="fake-key",
        )
...
class TestBedrockMantleProviderResolution:
    def test_get_llm_provider_resolves_correctly(self):
        model, provider, _, _ = derouter.get_llm_provider(
...
class TestBedrockMantlePricing:
    ...
    def test_safeguard_models_have_larger_output_tokens(self, monkeypatch):
        monkeypatch.setenv("DEROUTER_LOCAL_MODEL_COST_MAP", "true")
        derouter.add_known_models()
        info_120b = derouter.get_model_info("bedrock_mantle/openai.gpt-oss-120b")
        info_safeguard = derouter.get_model_info(
            "bedrock_mantle/openai.gpt-oss-safeguard-120b"
        )
...
def test_gemma_4_models_register_under_bedrock_mantle(local_cost_map, model_id):
    ...
    assert full_model_name in derouter.bedrock_mantle_models

    resolved_model, provider, _, _ = derouter.get_llm_provider(full_model_name)
    assert provider == "bedrock_mantle"
