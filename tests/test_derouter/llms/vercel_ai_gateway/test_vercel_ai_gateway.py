...
from derouter.llms.vercel_ai_gateway.chat.transformation import VercelAIGatewayConfig
...
def test_vercel_ai_gateway_in_provider_lists():
    ...
    assert "vercel_ai_gateway" in derouter.openai_compatible_providers
    assert "vercel_ai_gateway" in derouter.provider_list
    assert "https://ai-gateway.vercel.sh/v1" in derouter.openai_compatible_endpoints
...
def test_vercel_ai_gateway_models_endpoint():
    ...
    with patch("derouter.module_level_client.get") as mock_get:
