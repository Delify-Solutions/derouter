...
from derouter.llms.ollama.completion.handler import ollama_aembeddings, ollama_embeddings
...
def test_ollama_embeddings(mock_response_data, mock_embedding_response, mock_encoding):
    with (
        patch("derouter.module_level_client.post") as mock_post,
        patch("derouter.OllamaConfig.get_config", return_value={"truncate": 512}),
    ):
...
async def test_ollama_aembeddings(
    ...
    with (
        patch(
            "derouter.module_level_aclient.post", return_value=mock_response
        ) as mock_post,
        patch("derouter.OllamaConfig.get_config", return_value={"truncate": 512}),
    ):
...
def test_prompt_eval_fallback_when_missing(mock_embedding_response, mock_encoding):
    ...
    with (
        patch("derouter.module_level_client.post") as mock_post,
        patch("derouter.OllamaConfig.get_config", return_value={}),
    ):
