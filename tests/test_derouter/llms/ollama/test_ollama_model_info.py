...
class TestOllamaModelInfo:
    ...
    def test_get_models_does_not_leak_server_key_to_provided_api_base(
        ...
        monkeypatch.setattr(derouter, "api_key", "global-provider-key")
        monkeypatch.setattr(derouter, "openai_key", "global-openai-key")
        monkeypatch.setattr(httpx, "get", mock_get)
...
class TestOllamaGetModelInfo:
    ...
    def test_get_model_info_uses_provided_api_base(self, monkeypatch):
        ...
        monkeypatch.setattr("derouter.module_level_client.post", mock_post)
    ...
    def test_derouter_get_model_info_does_not_leak_server_key_to_provided_api_base(
        self, monkeypatch
    ):
    ...
    def test_derouter_get_model_info_forwards_explicit_api_key_to_provided_base(
        self, monkeypatch
    ):
        """An explicit api_key passed to derouter.get_model_info must reach the provided base."""
        captured_headers = []
    ...
    def test_derouter_get_model_info_does_not_cache_on_api_key(self, monkeypatch):
        ...
        """
        from derouter.utils import _cached_get_model_info
    ...
    def test_get_model_info_graceful_fallback_on_connection_error(self, monkeypatch):
        ...
        assert result["derouter_provider"] == "ollama"
    ...
    def test_derouter_get_model_info_uses_provider_hook_for_unknown_model(
        self, monkeypatch
    ):
    ...
    def test_derouter_get_model_info_keeps_static_map_for_known_model(self, monkeypatch):
        ...
        monkeypatch.setattr("derouter.module_level_client.post", mock_post)
        try:
            model_info = derouter.get_model_info("ollama/llama2")
        finally:
            derouter.get_model_info.cache_clear()

        assert model_info["key"] == "ollama/llama2"
        assert model_info["derouter_provider"] == "ollama"
...
class TestOllamaAuthHeaders:
    ...
    def test_ollama_completion_with_api_key_adds_auth_header(self, monkeypatch):
        ...
        with patch(
            "derouter.main.base_llm_http_handler.completion", side_effect=mock_completion
        ):
            try:
