...
import derouter
from derouter import completion, get_llm_provider
from derouter.llms.dashscope.chat.transformation import DashScopeChatConfig
from derouter.llms.dashscope.cost_calculator import (
    cost_per_token as dashscope_cost_per_token,
)
from derouter.llms.dashscope.embed.transformation import DashScopeEmbeddingConfig
from derouter.llms.dashscope.image_generation.transformation import (
    DashScopeImageGenerationConfig,
)
from derouter.llms.dashscope.qwen_ai_platform import (
...
from derouter.llms.dashscope.qwencloud import (
    QWENCLOUD_API_BASE,
...
from derouter.llms.dashscope.rerank.transformation import DashScopeRerankConfig
from derouter.types.utils import LlmProviders, Usage
...
class TestQwenBrandCostParity:
    ...
    def test_get_model_info(self, brand):
        model_info = derouter.get_model_info(f"{brand['provider']}/qwen-max")
        dashscope_info = derouter.get_model_info("dashscope/qwen-max")
        assert model_info["derouter_provider"] == brand["provider"]
    ...
    def test_tiered_pricing_matches_dashscope(self, brand):
        ...
        assert brand_costs == dashscope_costs
        tier_2 = derouter.get_model_info(f"{brand['provider']}/qwen-flash")["tiered_pricing"][1]
    ...
    def test_public_cost_per_token_routes_to_dashscope_calculator(self, brand):
        brand_costs = derouter.cost_per_token(
            model=f"{brand['provider']}/qwen-max",
            prompt_tokens=1000,
            completion_tokens=500,
            custom_llm_provider=brand["provider"],
        )
        dashscope_costs = derouter.cost_per_token(
...
class TestQwenBrandCompletionMock:
    ...
    def test_completion_hits_brand_default_host(self, respx_mock, brand, monkeypatch):
        ...
        respx_mock.post(f"{brand['default_base']}/chat/completions").respond(
            json={
                ...
                "choices": [
                    {
                        "index": 0,
                        "message": {"role": "assistant", "content": "Hey from DeRouter!"},
                        "finish_reason": "stop",
                    }
                ],
                "usage": {
        ...
        response = completion(
            ...
            messages=[{"role": "user", "content": "say hey from DeRouter"}],
            api_key="fake-brand-key",
        )

        assert response.choices[0].message.content == "Hey from DeRouter!"
