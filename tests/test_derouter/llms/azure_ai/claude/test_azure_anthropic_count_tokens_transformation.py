...
class TestAzureAIAnthropicCountTokensConfig:
    ...
    def test_get_required_headers_with_derouter_params(self):
        """
        Test that get_required_headers works with derouter_params.
        """
        config = AzureAIAnthropicCountTokensConfig()
        api_key = "test-key"
        derouter_params = {"api_key": "param-key", "custom_field": "value"}

        headers = config.get_required_headers(
            api_key=api_key, derouter_params=derouter_params
        )

        # x-api-key should use the direct api_key parameter
        assert headers["x-api-key"] == api_key
        # Azure api-key should come from derouter_params
