...
class TestAzureAnthropicConfig:
    ...
    def test_validate_environment_with_dict_derouter_params(self):
        """Test validate_environment with dict derouter_params"""
        config = AzureAnthropicConfig()
        headers = {}
        model = "claude-sonnet-4-5"
        messages = [{"role": "user", "content": "Hello"}]
        optional_params = {}
        derouter_params = {"api_key": "test-api-key"}
        api_key = "test-api-key"

        with patch(
            "derouter.llms.azure.common_utils.BaseAzureLLM._base_validate_azure_environment"
        ) as mock_validate:
            ...
            # Verify that dict was converted to GenericDeRouterParams
            call_args = mock_validate.call_args
            assert isinstance(call_args[1]["derouter_params"], GenericDeRouterParams)
            assert call_args[1]["derouter_params"].api_key == "test-api-key"
            assert "anthropic-version" in result

    def test_validate_environment_with_generic_derouter_params(self):
        """Test validate_environment with GenericDeRouterParams object"""
        config = AzureAnthropicConfig()
        headers = {}
        model = "claude-sonnet-4-5"
        messages = [{"role": "user", "content": "Hello"}]
        ...
        with patch(
            ...
            # Verify that GenericDeRouterParams was passed through
            call_args = mock_validate.call_args
            assert isinstance(call_args[1]["derouter_params"], GenericDeRouterParams)
            assert "anthropic-version" in result

    def test_validate_environment_sets_api_key_in_derouter_params(self):
        """Test that api_key parameter is set in derouter_params if provided"""
        config = AzureAnthropicConfig()
        headers = {}
        model = "claude-sonnet-4-5"
        messages = [{"role": "user", "content": "Hello"}]
        optional_params = {}
        derouter_params = {}  # Empty dict, no api_key
        ...
        with patch(
            ...
            # Verify that api_key was set in derouter_params
            call_args = mock_validate.call_args
            assert call_args[1]["derouter_params"].api_key == "provided-api-key"
    ...
    def test_transform_request_removes_unsupported_params(self):
        ...
        These parameters are DeRouter-internal and not supported by Azure AI Anthropic endpoint.
    ...
    def test_invalid_output_config_effort_raises_via_extra_body(self):
        ...
        with pytest.raises(derouter.exceptions.BadRequestError) as exc_info:
