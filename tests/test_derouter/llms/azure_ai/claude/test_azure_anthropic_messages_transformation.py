...
class TestAzureAnthropicMessagesConfig:
    ...
    def test_validate_anthropic_messages_environment_with_dict_derouter_params(self):
        """Test validate_anthropic_messages_environment with dict derouter_params"""
        config = AzureAnthropicMessagesConfig()
        headers = {}
    ...
    def test_get_complete_url_raises_error_when_api_base_missing(self):
        ...
        with patch("derouter.secret_managers.main.get_secret_str", return_value=None):
...
def test_messages_thinking_shape_follows_exact_azure_entry_flag(local_model_cost_map, monkeypatch):
    ...
    monkeypatch.setitem(
        derouter.model_cost["azure_ai/claude-opus-4-8"], "supports_adaptive_thinking", False
    )
    derouter.get_model_info.cache_clear()
    assert derouter.model_cost["claude-opus-4-8"]["supports_adaptive_thinking"] is True
...
def test_azure_claude_4_8_plus_cost_map_entries_carry_mid_conversation_system_flag():
    ...
    cost_map_path = os.path.join(
        os.path.dirname(derouter.__file__), "model_prices_and_context_window_backup.json"
    )
    ...
    missing = [
        ...
        and info.get("derouter_provider") == "azure_ai"
