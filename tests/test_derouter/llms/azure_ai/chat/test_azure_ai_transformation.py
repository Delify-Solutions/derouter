...
def test_azure_ai_validate_environment_with_azure_ad_token():
    ...
    Regression test for https://github.com/Delify-Solutions/derouter/issues/20759
    """
    import derouter
    ...
    with (
        ...
        patch.object(derouter, "api_key", None),
        patch.object(derouter, "azure_key", None),
    ):
        headers = config.validate_environment(
            headers={},
            model="Kimi-K2.5",
...
def test_azure_model_router_response_shows_actual_model():
    ...
    result = config.transform_response(
        ...
        optional_params={},
        derouter_params={"model": "azure_ai/model-router"},  # Original request model
        encoding=None,
        api_key="test-key",
        json_mode=False,
    )
...
def test_azure_model_router_stamps_selected_model_on_hidden_params():
    ...
    result = AzureModelRouterConfig().transform_response(
        ...
        optional_params={},
        derouter_params={"model": "azure_ai/model_router/smart-pick"},
        encoding=None,
        api_key="test-key",
        json_mode=False,
    )
...
def test_drop_tool_level_extra_fields_strips_copilot_mcp_server_name():
    ...
    DeRouter should strip the field and retry automatically.
    """
    import httpx
...
def test_azure_ai_strips_non_openai_spec_message_fields():
    """
    Regression for https://github.com/Delify-Solutions/derouter/issues/33961.
...
def test_azure_ai_stripping_does_not_mutate_caller_messages():
    """
    The stripping must not touch the caller's messages. DeRouter reuses the same
