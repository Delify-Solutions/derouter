...
def _azure_ai_response(response_model: str, derouter_model_name: str | None = None) -> ModelResponse:
    ...
    response._hidden_params = (
        ...
        if derouter_model_name is None
        else {"custom_llm_provider": "azure_ai", "derouter_model_name": derouter_model_name}
    )
    return response
...
class TestAzureModelRouterFlatCost:
    ...
    def test_public_cost_per_token_keeps_the_request_model_keyword(self) -> None:
        routed_prompt_cost, routed_completion_cost = _routed_model_cost()
        prompt_cost, completion_cost_usd = derouter.cost_per_token(
            model=ROUTED_MODEL,
            custom_llm_provider="azure_ai",
    ...
    def test_flat_cost_reads_the_fee_from_the_deployment_named_entry(self) -> None:
        derouter.register_model(
            {"azure_ai/model-router": {"input_cost_per_token": 2e-07, "derouter_provider": "azure_ai", "mode": "chat"}}
        )
...
class TestAzureModelRouterCostBreakdown:
    ...
    def test_routed_response_named_by_hidden_params_charges_the_fee_once(self) -> None:
        ...
        cost = completion_cost(
            completion_response=_azure_ai_response(ROUTED_MODEL, derouter_model_name="azure_ai/model-router"),
            model=ROUTED_MODEL,
            custom_llm_provider="azure_ai",
            derouter_logging_obj=logging_obj,
        )
