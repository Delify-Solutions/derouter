...
This test file mirrors derouter/llms/pg_vector/vector_stores/transformation.py
...
from derouter.llms.pg_vector.vector_stores.transformation import PGVectorStoreConfig
...
class TestPGVectorStoreConfig:
    ...
    def test_validate_environment_with_api_key_in_params(self):
        """
        Test that validate_environment works when api_key is provided in derouter_params.

        This test validates that API key from params is correctly set in headers.
        """
        config = PGVectorStoreConfig()
        derouter_params = GenericDeRouterParams(api_key="test_pg_vector_key_123")
        headers = {}

        result_headers = config.validate_environment(headers, derouter_params)
    ...
    def test_validate_environment_missing_api_key(self):
        ...
        with pytest.raises(ValueError, match='PG Vector API key is required\\. Set PG_VECTOR_API_KEY') as exc_info:
            config.validate_environment(headers, derouter_params)
    ...
    def test_get_complete_url_with_api_base(self):
        ...
        result_url = config.get_complete_url(api_base, derouter_params)
    ...
    def test_get_complete_url_missing_api_base(self):
        ...
        with pytest.raises(ValueError, match='PG Vector API base URL is required\\. Set') as exc_info:
            config.get_complete_url(None, derouter_params)
    ...
    def test_config_methods_with_mock_data(self):
        ...
        derouter_params = GenericDeRouterParams(api_key="test_key")
        headers = config.validate_environment({}, derouter_params)
    ...
    def test_environment_variable_support(self):
        ...
        with patch.dict(os.environ, {"PG_VECTOR_API_KEY": "env_api_key_123"}):
            derouter_params = GenericDeRouterParams()  # No API key in params
        ...
        with patch.dict(os.environ, {"PG_VECTOR_API_KEY": "env_key"}):
            derouter_params = GenericDeRouterParams(api_key="param_key")
    ...
    @patch("derouter.llms.custom_httpx.http_handler.HTTPHandler.post")
    def test_pg_vector_search_request_construction(self, mock_post):
        ...
        original_registry = getattr(derouter, "vector_store_registry", None)
        ...
        try:
            ...
            mock_response.json.return_value = {
                ...
                "search_query": ["what are remote working hours for Delify-Solutions"],
                "data": [
                    {
                        "file_id": "file_123",
            ...
            query = "what are remote working hours for Delify-Solutions"

            # Call derouter vector store search
            ...
            try:
                response = derouter.vector_stores.search(
                    ...
                    mock_response=None,  # Explicitly disable DeRouter's automatic mocking
                )
            ...
            derouter.vector_store_registry = original_registry
