"""
Integration tests for Volcengine embedding following DeRouter testing patterns
...
class TestVolcEngineEmbedding(BaseLLMEmbeddingTest):
    """Test Volcengine embedding integration following DeRouter patterns"""

    def get_custom_llm_provider(self) -> derouter.LlmProviders:
        return derouter.LlmProviders.VOLCENGINE
    ...
    async def test_basic_embedding(self, sync_mode):
        ...
        with (
            patch("derouter.embedding") as mock_embedding,
            patch("derouter.aembedding") as mock_aembedding,
        ):
    ...
    for scenario in error_scenarios:
        with patch("derouter.embedding") as mock_embedding:
            ...
            with pytest.raises(Exception, match=f"(?i){scenario['expected_error_pattern']}") as exc_info:
                derouter.embedding(input=["test"], **test_params)
