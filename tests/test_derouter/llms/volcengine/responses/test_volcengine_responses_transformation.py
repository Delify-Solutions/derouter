...
class TestVolcengineResponsesAPITransformation:
    ...
    def test_get_complete_url_variants(self):
        ...
        default_url = config.get_complete_url(api_base=None, derouter_params={})
        assert default_url == "https://ark.cn-beijing.volces.com/api/v3/responses"

        api_base_with_api = config.get_complete_url(api_base="https://custom.volc.com/api/v3", derouter_params={})
        assert api_base_with_api == "https://custom.volc.com/api/v3/responses"

        api_base_full = config.get_complete_url(api_base="https://custom.volc.com/api/v3/responses", derouter_params={})
    ...
    @pytest.mark.parametrize(
        "derouter_params, expected_key",
        [
            ({"api_key": "dict-key"}, "dict-key"),
            (GenericDeRouterParams(api_key="attr-key"), "attr-key"),
        ],
    )
    def test_validate_environment_uses_api_key(self, monkeypatch, derouter_params, expected_key):
        ...
        headers = config.validate_environment(headers={}, model="volcengine/demo-model", derouter_params=derouter_params)
    ...
    def test_validate_environment_raises_without_key(self, monkeypatch):
        ...
        with pytest.raises(ValueError, match='Volcengine API key is required\\. Set ARK_API_KEY /'):
            config.validate_environment(headers={}, model="volcengine/demo", derouter_params={})
    ...
    def test_error_class_returns_volcengine_error(self):
        ...
        # module reloading during parallel test execution (conftest reloads derouter)
