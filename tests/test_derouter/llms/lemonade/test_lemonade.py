...
import derouter
from derouter.llms.lemonade.chat.transformation import LemonadeChatConfig
...
def test_get_openai_compatible_provider_info(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "lemonade_key", None)
...
def test_get_openai_compatible_provider_info_skips_env_key_for_custom_base(
    ...
    monkeypatch.setattr(derouter, "lemonade_key", "configured-lemonade-key")
    monkeypatch.setattr(derouter, "api_key", None)
    config = LemonadeChatConfig()
...
def test_get_openai_compatible_provider_info_ignores_global_api_key(monkeypatch):
    ...
    monkeypatch.delenv("LEMONADE_API_KEY", raising=False)
    monkeypatch.setattr(derouter, "lemonade_key", None)
    monkeypatch.setattr(derouter, "api_key", "global-openai-key")
    config = LemonadeChatConfig()
...
def test_get_models_does_not_leak_lemonade_key_to_custom_base(monkeypatch):
    ...
    with patch.object(
        derouter.module_level_client, "get", return_value=response
    ) as mock_get:
...
def test_get_model_info_uses_loaded_context_size():
    ...
    assert model_info["derouter_provider"] == "lemonade"
...
def test_get_model_info_falls_back_when_server_unavailable():
    ...
    with patch.object(
        derouter.module_level_client, "get", side_effect=Exception("boom")
    ):
        model_info = config.get_model_info(
            model="lemonade/Qwen3.6-35B-A3B-GGUF",
            api_base="http://lemonade.test/v1",
...
def test_get_model_info_reads_context_from_provider_specific_entry():
    ...
    with patch.object(derouter.module_level_client, "get", return_value=response):
        model_info = config.get_model_info(
            model="lemonade/Qwen3.6-35B-A3B-GGUF",
            api_base="http://lemonade.test/v1",
        )
...
def test_derouter_get_model_info_does_not_leak_lemonade_key_to_custom_base(
    monkeypatch,
):
...
def test_derouter_get_model_info_forwards_explicit_lemonade_key_to_custom_base(
    monkeypatch,
):
...
def test_derouter_get_model_info_uses_lemonade_api_base():
    """Test that DeRouter model info is wired to Lemonade's model metadata API."""
    response = MagicMock()
    response.status_code = 200
    response.json.return_value = {
        "id": "Qwen3.6-35B-A3B-GGUF",
