...
import derouter
from derouter.llms.inception.completion.transformation import (
...
def test_text_completion_inception_in_provider_lists():
    ...
    assert "text-completion-inception" in derouter.provider_list


def test_inception_get_supported_openai_params_dispatch():
    """derouter.get_supported_openai_params routes the FIM provider to our config"""
    params = derouter.get_supported_openai_params(
...
def test_inception_validate_environment(provider):
    ...
    with mock.patch.dict(os.environ, {}, clear=True):
        result = derouter.validate_environment(model)
...
async def test_inception_fim_async():
    ...
    with mock.patch("httpx.AsyncClient.send", new=fake_asend):
        r = await derouter.atext_completion(
            model="text-completion-inception/mercury-edit-2",
...
def test_inception_fim_does_not_leak_global_api_key():
    """
    Regression: the global derouter.api_key (commonly an OpenAI key) must not be
    forwarded to Inception. Only an Inception-specific key (param,
    derouter.inception_key, or INCEPTION_API_KEY) may be sent to the Inception base.
    """
    ...
    with mock.patch.dict(
        ...
        with mock.patch.object(derouter, "inception_key", None):
            with mock.patch.object(derouter, "api_key", "sk-global-should-not-leak"):
                with mock.patch("httpx.Client.send", new=fake_send):
