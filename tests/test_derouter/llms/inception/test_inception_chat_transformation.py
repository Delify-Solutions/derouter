...
import derouter
from derouter.llms.inception.chat.transformation import InceptionChatConfig
...
def test_inception_get_openai_compatible_provider_info():
    ...
    with mock.patch.dict(os.environ, {}, clear=True):
        with mock.patch.object(derouter, "inception_key", None):
...
def test_inception_key_module_attr_fallback():
    """derouter.inception_key is used when no param/env key is provided"""
    config = InceptionChatConfig()
    with mock.patch.dict(os.environ, {}, clear=True):
        with mock.patch.object(derouter, "inception_key", "module-attr-key"):
...
def test_inception_does_not_leak_key_to_caller_api_base():
    ...
    with mock.patch.dict(
        ...
        with mock.patch.object(derouter, "inception_key", "module-secret"):
...
def test_inception_in_provider_lists():
    assert "inception" in derouter.openai_compatible_providers
    assert "inception" in derouter.provider_list
    assert "https://api.inceptionlabs.ai/v1" in derouter.openai_compatible_endpoints
...
def test_inception_model_list_populated(monkeypatch):
    ...
    derouter.inception_models = set()
    derouter.add_known_models()

    assert "inception/mercury-2" in derouter.inception_models
    for model in derouter.inception_models:
