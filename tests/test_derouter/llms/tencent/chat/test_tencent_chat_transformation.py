...
from derouter.llms.tencent.chat.transformation import TencentChatConfig
...
def test_minimax_m3_cost_map_entry_marks_adaptive_thinking():
    ...
    for filename in ("model_prices_and_context_window.json", "derouter/model_prices_and_context_window_backup.json"):
        ...
        assert entry["derouter_provider"] == "tencent"
...
def test_provider_info_falls_back_to_default_base():
    ...
    with patch("derouter.llms.tencent.chat.transformation.get_secret_str", return_value=None):
...
def test_provider_info_reads_env_secrets():
    ...
    with patch(
        "derouter.llms.tencent.chat.transformation.get_secret_str",
