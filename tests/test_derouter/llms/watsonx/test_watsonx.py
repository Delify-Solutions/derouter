    ...
    def _call(
        ...
        if patch_token_call:
            ...
            with (
                ...
                patch.object(
                    derouter.module_level_client, "post", return_value=mock_response
                ) as mock_get,
            ):
                try:
                    completion(
...
def test_watsonx_gpt_oss_prompt_transformation(monkeypatch):
    ...
    This test calls derouter.completion (sync) and verifies what gets sent in the final POST request body.
    ...
    monkeypatch.setitem(derouter.known_tokenizer_config, hf_model, mock_tokenizer_config)
    ...
    with (
        patch.object(client, "post") as mock_post,
        patch.object(
            derouter.module_level_client, "post", return_value=mock_token_response
        ),
    ):
        try:
            completion(
...
async def test_watsonx_gpt_oss_uses_async_http_handler():
    ...
    with patch(
        "derouter.derouter_core_utils.prompt_templates.huggingface_template_handler.get_async_httpx_client",
...
async def test_watsonx_text_gpt_oss_async_completion_fetches_hf_template_off_the_event_loop(
    ...
    from derouter.derouter_core_utils.prompt_templates import huggingface_template_handler
    ...
    if tokenizer_config_cached:
        ...
        monkeypatch.setattr(derouter, "known_tokenizer_config", {hf_model: cached_config})
        expected_fetch = f"https://huggingface.co/{hf_model}/raw/main/chat_template.jinja"
    else:
        monkeypatch.setattr(derouter, "known_tokenizer_config", {})
...
def test_watsonx_chat_completion_with_reasoning_effort(monkeypatch):
    ...
    # Call derouter.completion with the new parameter
    with (
        patch.object(client, "post") as mock_post,
        patch.object(
            derouter.module_level_client, "post", return_value=mock_token_response
