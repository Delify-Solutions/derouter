...
import derouter
from derouter.cost_calculator import completion_cost, cost_per_token
...
class TestPerplexityCostCalculator:
    ...
    def setup_model_cost_map(self):
        ...
        try:
            ...
            derouter.model_cost = model_cost_map
    ...
    def test_missing_model_info_fields(self):
        ...
        with patch(
            "derouter.llms.perplexity.cost_calculator.get_model_info"
    ...
    def test_integration_with_main_cost_calculator(self):
        """Test integration with the main DeRouter cost calculator."""
        usage = Usage(
            prompt_tokens=100,
            completion_tokens=50,
            total_tokens=150,
    ...
    def test_uses_perplexity_provided_cost_when_available(self):
        ...
        request_cost (fixed per-request fee) that DeRouter cannot calculate.
        """
    ...
    def test_uses_perplexity_provided_cost_when_normalized_to_float(self):
        ...
        (derouter/types/llms/openai.py) already flattens Perplexity's
        `usage.cost.total_cost` dict down to a plain float before
        `_transform_response_api_usage_to_chat_usage` (derouter/responses/utils.py) copies
    ...
    def _register_off_peak_model(self, off_peak_pricing: OffPeakPricing) -> None:
        derouter.model_cost[f"perplexity/{self.OFF_PEAK_MODEL}"] = {
            "derouter_provider": "perplexity",
            "mode": "chat",
