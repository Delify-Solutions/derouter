...
from derouter.llms.reducto.common import (
...
async def test_parse_v3_rejects_plain_http_urls(disable_aiohttp_transport):
    with pytest.raises(derouter.BadRequestError, match="upload the file first"):
        await derouter.aocr(
...
def test_upload_bytes_sync_uses_shared_client(monkeypatch):
    ...
    monkeypatch.setattr(derouter.module_level_client, "post", sync_post)
...
async def test_upload_bytes_async_uses_shared_aclient(monkeypatch):
    ...
    monkeypatch.setattr(derouter.module_level_aclient, "post", async_post)
...
def test_extract_file_id_or_bytes_raises_on_malformed_data_uri():
    with pytest.raises(derouter.BadRequestError, match="Invalid Reducto data URI"):
        extract_file_id_or_bytes("data:application/pdf", model="reducto/parse-v3")

    with pytest.raises(derouter.BadRequestError, match="Invalid Reducto base64 payload"):
