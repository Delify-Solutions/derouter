...
from derouter.llms.gigachat.passthrough.transformation import GigaChatPassthroughConfig
...
class TestGigaChatPassthroughConfig:
    ...
    @patch(  # test-quality-ok: patching derouter internal for unit test isolation
        "derouter.llms.gigachat.passthrough.transformation.get_secret_str"
    )
    ...
    def test_get_complete_url_no_api_base_raises(self):
        ...
        config = GigaChatPassthroughConfig()
        with patch(
            "derouter.llms.gigachat.passthrough.transformation.get_secret_str",  # test-quality-ok: patching derouter internal for unit test isolation
            return_value=None,
        ):
            with patch(
                "derouter.llms.gigachat.passthrough.transformation.GIGACHAT_BASE_URL",  # test-quality-ok: patching derouter internal for unit test isolation
                None,
            ):
    ...
    @patch(  # test-quality-ok: patching derouter internal for unit test isolation
        "derouter.llms.gigachat.passthrough.transformation.get_access_token"
    )
    ...
    def test_logging_non_streaming_chat_raises_when_no_config(self):
        ...
        with patch(
            "derouter.utils.ProviderConfigManager.get_provider_chat_config",  # test-quality-ok: patching derouter internal for unit test isolation
            return_value=None,
        ):
    ...
    def test_logging_non_streaming_embedding_raises_when_no_config(self):
        ...
        with patch(
            "derouter.utils.ProviderConfigManager.get_provider_embedding_config",  # test-quality-ok: patching derouter internal for unit test isolation
            return_value=None,
        ):
            with pytest.raises(ValueError, match="No provider config found for model"):
                config.logging_non_streaming_response(
    ...
    def test_handle_logging_collected_chunks_with_model_response_stream_chunk(self):
        ...
        with patch(
            "derouter.llms.gigachat.passthrough.transformation.GigaChatModelResponseIterator.chunk_parser",  # test-quality-ok: patching derouter internal for unit test isolation
