...
from derouter_params/env, credential validation, caching, and error handling.
"""
...
from derouter.llms.gigachat import authenticator
from derouter.llms.gigachat.authenticator import (
    GigaChatAuthError,
...
AUTH_MODULE = "derouter.llms.gigachat.authenticator"
...
class TestGetAccessTokenSync:
    def test_returns_token_from_derouter_params(self):
        token = get_access_token(derouter_params={"gigachat_access_token": "param-token"})
    ...
    @patch(f"{AUTH_MODULE}.get_secret_str", return_value=None)
    def test_derouter_params_override_scope_and_auth_url(self, mock_get_secret, mock_creds, mock_scope, mock_auth_url, mock_request):  # test-quality-ok: mock-echo of internal wiring
...
class TestGetAccessTokenAsync:
    @pytest.mark.asyncio
    async def test_returns_token_from_derouter_params(self):
        token = await get_access_token_async(
            derouter_params={"gigachat_access_token": "param-token"}
        )
        assert token == "param-token"
        authenticator._token_cache.get_cache.assert_not_called()
    ...
    @patch(f"{AUTH_MODULE}._get_credentials", return_value="env-creds")
    @patch(f"{AUTH_MODULE}.get_secret_str", return_value=None)
    async def test_derouter_params_override_scope_and_auth_url(  # test-quality-ok: mock-echo of internal wiring
        self, mock_get_secret, mock_creds, mock_scope, mock_auth_url, mock_request
    ):
        mock_request.return_value = ("token", _future_expires_at_ms())
...
class TestRequestTokenSyncErrorMapping:
    ...
    def test_http_status_error_maps_to_auth_error(self, mock_get_client):
        ...
        from derouter.llms.gigachat.authenticator import _request_token_sync
...
class TestRequestTokenAsyncErrorMapping:
    ...
    async def test_http_status_error_maps_to_auth_error(self, mock_get_client):
        ...
        from derouter.llms.gigachat.authenticator import _request_token_async
...
class TestParseTokenResponse:
    ...
    def test_parses_tok_exp_fields(self):
        from derouter.llms.gigachat.authenticator import _parse_token_response
