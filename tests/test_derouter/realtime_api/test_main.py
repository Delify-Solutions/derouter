...
from derouter.realtime_api.main import _with_resolved_session_model
...
async def test_azure_health_check_stays_on_ga_when_deployment_registration_overwrites_mode(
    ...
    polluted = {**derouter.model_cost["azure/gpt-realtime-whisper"], "mode": "realtime"}
    monkeypatch.setitem(derouter.model_cost, "azure/gpt-realtime-whisper", polluted)
    connect = _CapturingConnect()
    with patch("websockets.connect", connect):
        assert await realtime_main._realtime_health_check(
            model="gpt-realtime-whisper",
