...
async def test_stream_response_when_stream_requested_async():
    ...
    with patch.object(
        ...
        with patch("derouter.acompletion", return_value=mock_stream):
            # Call the handler with stream=True
            result = (
...
def test_stream_transformation_error_sync():
    ...
    with patch.object(
        ...
        # Patch derouter.completion directly to prevent real API calls
        with patch("derouter.completion", return_value=mock_stream):
...
async def test_stream_transformation_error_async():
    ...
    with patch.object(
        ...
        # Mock derouter.acompletion at the module level where it's imported
        # We need to patch it in the handler module, not in derouter itself
        with patch("derouter.google_genai.adapters.handler.derouter") as mock_derouter:
            # Use AsyncMock for async function
            mock_derouter.acompletion = AsyncMock(return_value=mock_stream)
            # Call the handler with stream=True and expect a ValueError
            with pytest.raises(
                ValueError, match="Failed to transform streaming response"
            ):
