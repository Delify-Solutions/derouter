...
from derouter.google_genai.main import agenerate_content
...
def test_completion_to_generate_content_with_tool_calls():
    ...
    from derouter.types.utils import Choices, ModelResponse, Usage
...
def test_streaming_partial_tool_calls_accumulation():
    ...
    assert (
        function_call["args"]["path"] == "/Users/ishaanjaffer/Github/derouter/README.md"
...
def test_completion_to_generate_content_transformation():
    ...
    from derouter.google_genai.adapters.transformation import GoogleGenAIAdapter
    from derouter.types.llms.openai import ChatCompletionAssistantMessage
    from derouter.types.utils import Choices, ModelResponse, Usage
...
def test_api_base_and_api_key_passthrough(function_name, is_async, is_stream):
    """Test that api_base and api_key parameters are passed through to derouter.completion/acompletion when using generate_content"""
    import asyncio
    ...
    if function_name == "generate_content":
        from derouter.google_genai.main import generate_content as test_function
    elif function_name == "agenerate_content":
        from derouter.google_genai.main import agenerate_content as test_function
    elif function_name == "generate_content_stream":
        from derouter.google_genai.main import generate_content_stream as test_function
    elif function_name == "agenerate_content_stream":
        from derouter.google_genai.main import agenerate_content_stream as test_function
    ...
    # Mock the appropriate derouter function (completion vs acompletion)
    mock_target = "derouter.acompletion" if is_async else "derouter.completion"
    ...
    with unittest.mock.patch(mock_target) as mock_completion:
        ...
        # Verify that the appropriate derouter function was called
        mock_completion.assert_called_once()

        # Get the arguments passed to derouter.completion/acompletion
...
def test_shared_schema_normalization_utilities():
    ...
    from derouter.derouter_core_utils.json_validation_rule import (
...
async def test_google_generate_content_with_openai():
    ...
    with unittest.mock.patch(
        "derouter.google_genai.main.derouter.acompletion",
        ...
        # Print the request args sent to derouter.completion
        call_args, call_kwargs = mock_completion.call_args
        print("Arguments sent to derouter.completion:")
        ...
        #########################################################
        # validate only expected fields were sent to derouter.completion
        passed_fields = set(call_kwargs.keys())
        # remove any GenericDeRouterParams fields
        passed_fields = passed_fields - set(GenericDeRouterParams.model_fields.keys())
...
def test_get_gemini_url_excludes_api_key():
    ...
    """
    from derouter.llms.vertex_ai.common_utils import _get_gemini_url
