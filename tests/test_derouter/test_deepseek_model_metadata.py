...
def _load_backup_json() -> dict:
    ...
    backup_path = os.path.join(
        os.path.dirname(derouter.__file__),
...
class TestBareModelFallback:
    ...
    def test_fallback_uses_bare_entry(self):
        ...
        original = derouter.model_cost.get(key, {}).get("supports_response_schema")
        try:
            # Simulate the pre-fix state: field missing from prefixed entry
            if key in derouter.model_cost:
                derouter.model_cost[key].pop("supports_response_schema", None)
            ...
            # Restore
            if key in derouter.model_cost and original is not None:
                derouter.model_cost[key]["supports_response_schema"] = original
    ...
    def test_no_fallback_when_explicitly_false(self):
        ...
        original = derouter.model_cost.get(key, {}).get("supports_function_calling")
        try:
            if key in derouter.model_cost:
                derouter.model_cost[key]["supports_function_calling"] = False
            result = _supports_factory(
            ...
            if key in derouter.model_cost and original is not None:
                derouter.model_cost[key]["supports_function_calling"] = original
