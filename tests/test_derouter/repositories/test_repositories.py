...
from derouter.models.base import DomainModel
...
from derouter.repositories.object_permission_repository import (
...
class MockPrismaClient:
    ...
    def __init__(self):
        self.db = MagicMock()
        self.db.derouter_budgettable = MockTable()
        self.db.derouter_proxymodeltable = MockTable(pk_field="model_id")
        self.db.derouter_teamtable = MockTable()
        self.db.derouter_deletedteamtable = MockTable()
        self.db.derouter_usertable = MockTable()
        self.db.derouter_verificationtoken = MockTable()
        self.db.derouter_deletedverificationtoken = MockTable()
        self.db.derouter_config = MockTable()
        self.db.derouter_organizationtable = MockTable()
        self.db.derouter_projecttable = MockTable(pk_field="project_id")
        self.db.derouter_objectpermissiontable = MockTable(
            pk_field="object_permission_id"
        )
        self.db.derouter_credentialstable = MockTable()
...
class TestBaseRepository:
    ...
    def test_prisma_client_none_raises(self):
        class TestRepo(BaseRepository[DeRouter_BudgetTable]):
            @property
            def table(self):
                return None
    ...
    async def test_find_many(self, prisma_client):
        ...
        prisma_client.db.derouter_budgettable._records = {
    ...
    def test_record_to_dict_branches(self):
        from derouter.repositories.base_repository import record_to_dict
...
class TestBudgetRepository:
    ...
    async def test_update_budget(self, repo):
        ...
        repo._prisma_client.db.derouter_budgettable._records["budget-1"] = {
            "budget_id": "budget-1",
            "max_budget": 100.0,
        }
    ...
    async def test_delete_budget(self, repo):
        ...
        assert "budget-1" not in repo._prisma_client.db.derouter_budgettable._records
...
class TestModelRepository:
    ...
    async def test_find_all_except_serializes_exclusion_for_prisma(self) -> None:
        ...
        client: Final = SimpleNamespace(
            db=SimpleNamespace(derouter_proxymodeltable=SimpleNamespace(find_many=find_many))
        )
        ...
        query: Final = QueryBuilder(
            method="find_many",
            model=prisma_models.DeRouter_ProxyModelTable,
    ...
    def test_table_is_wrapped_for_config_sync(self, repo):
        ...
        assert table._actions is repo.prisma_client.db.derouter_proxymodeltable

    @pytest.mark.asyncio
    @patch(
        "derouter.repositories.model_repository.encrypt_value_helper",
        side_effect=lambda v, **kw: f"encrypted_{v}",
    )
    @patch(
        "derouter.repositories.model_repository.decrypt_value_helper",
    ...
    async def test_create_model_encrypts_params(self, mock_decrypt, mock_encrypt, repo):
        model = await repo.create_model(
            model_name="gpt-4",
            derouter_params={"api_key": "sk-secret"},
            created_by="test-user",
        )
    ...
    async def test_update_model_all_fields(self, mock_decrypt, mock_encrypt, repo):
        repo._prisma_client.db.derouter_proxymodeltable._records["model-full"] = {
            "model_id": "model-full",
            "model_name": "old-name",
            "derouter_params": '{"api_key": "old"}',
            "blocked": False,
        }
        updated = await repo.update_model(
            ...
            updated_by="admin",
            model_name="new-name",
            derouter_params={"api_key": "new-key"},
    ...
    async def test_find_all(self, mock_decrypt, repo):
        repo._prisma_client.db.derouter_proxymodeltable._records = {
            "m1": {
                "model_id": "m1",
                "model_name": "gpt-4",
            ...
            "m2": {
                "model_id": "m2",
                "model_name": "claude-3",
                "derouter_params": '{"model": "claude-3"}',
                "blocked": False,
            },
        }
    ...
    async def test_update_model(self, mock_decrypt, mock_encrypt, repo):
        repo._prisma_client.db.derouter_proxymodeltable._records["m1"] = {
            "model_id": "m1",
            "model_name": "gpt-4",
            "derouter_params": '{"model": "gpt-4"}',
            "blocked": False,
...
class TestTeamRepository:
    ...
    async def test_update_team(self, repo):
        repo._prisma_client.db.derouter_teamtable._records["team-1"] = {
            "team_id": "team-1",
            "team_alias": "Test",
            "admins": [],
            "members": [],
    ...
    async def test_update_team_all_fields(self, repo):
        repo._prisma_client.db.derouter_teamtable._records["team-full"] = {
            "team_id": "team-full",
            "team_alias": "Test",
            "admins": [],
            "members": [],
...
class TestUserRepository:
    ...
    async def test_update_user(self, repo):
        repo._prisma_client.db.derouter_usertable._records["user-1"] = {
            "user_id": "user-1",
            "teams": [],
            "models": [],
        }
    ...
    async def test_find_by_sso_id(self, repo):
        repo._prisma_client.db.derouter_usertable._records["sso-123"] = {
            "user_id": "user-1",
            "sso_user_id": "sso-123",
            "teams": [],
            "models": [],
...
class TestVerificationTokenRepository:
    ...
    async def test_update_token(self, repo):
        repo._prisma_client.db.derouter_verificationtoken._records["sk-test"] = {
            "token": "sk-test",
            "blocked": False,
        }
...
class TestOrganizationRepository:
    ...
    async def test_update_organization(self, repo):
        repo._prisma_client.db.derouter_organizationtable._records["org-1"] = {
            "organization_id": "org-1",
    ...
    async def test_update_organization_all_fields(self, repo):
        repo._prisma_client.db.derouter_organizationtable._records["org-full"] = {
...
class TestProjectRepository:
    ...
    async def test_update_project(self, repo):
        repo._prisma_client.db.derouter_projecttable._records["proj-1"] = {
            "project_id": "proj-1",
    ...
    async def test_update_project_all_fields(self, repo):
        repo._prisma_client.db.derouter_projecttable._records["proj-full"] = {
...
class TestObjectPermissionRepository:
    ...
    async def test_update_permission(self, repo):
        repo._prisma_client.db.derouter_objectpermissiontable._records["perm-1"] = {
            "object_permission_id": "perm-1",
            "models": ["gpt-3.5-turbo"],
        }
    ...
    async def test_update_permission_all_fields(self, repo):
        repo._prisma_client.db.derouter_objectpermissiontable._records["perm-full"] = {
...
class TestCredentialsRepository:
    ...
    def test_table_is_wrapped_for_config_sync(self, repo):
        ...
        table = repo.table
        assert isinstance(table, _PublishOnWriteActions)
        assert table._actions is repo.prisma_client.db.derouter_credentialstable
    ...
    async def test_find_by_name_returns_stored_values_without_decryption(self, repo):
        repo._prisma_client.db.derouter_credentialstable._records["my-key"] = {
    ...
    async def test_find_all(self, repo):
        repo._prisma_client.db.derouter_credentialstable._records["k1"] = {
            "credential_name": "k1",
            "credential_values": {"api_key": "a"},
            "credential_info": {},
        }
        repo._prisma_client.db.derouter_credentialstable._records["k2"] = {
            "credential_name": "k2",
...
class TestConfigRepository:
    ...
    async def test_get_param(self, repo):
        repo._prisma_client.db.derouter_config._records["general_settings"] = {
    ...
    async def test_delete_param(self, repo):
        repo._prisma_client.db.derouter_config._records["test_param"] = {
    ...
    async def test_delete_param_nonexistent(self, repo):
        ...
        repo._prisma_client.db.derouter_config.delete = mock_delete
        result = await repo.delete_param("nonexistent")
        assert result is False

    @pytest.mark.asyncio
    async def test_get_all_params(self, repo):
        repo._prisma_client.db.derouter_config._records = {
    ...
    async def test_reconcile_config_with_db_values(self, repo):
        ...
        repo._prisma_client.db.derouter_config._records["router_settings"] = {
    ...
    @pytest.mark.asyncio
    @patch("derouter.repositories.config_repository.decrypt_value_helper")
    async def test_reconcile_config_with_environment_variables(
        ...
        repo._prisma_client.db.derouter_config._records["environment_variables"] = {
...
class TestVerificationTokenRepositoryExtended:
    ...
    async def test_find_active_tokens(self, repo):
        repo._prisma_client.db.derouter_verificationtoken._records["sk-active"] = {
            "token": "sk-active",
            "blocked": False,
            "expires": None,
        }
    ...
    async def test_delete_token_with_audit(self, repo):
        repo._prisma_client.db.derouter_verificationtoken._records["sk-delete"] = {
            "token": "sk-delete",
            "key_name": "Delete Me",
            "spend": 0.0,
        }
        ...
        class MockTx:
            def __init__(self, client):
                self.derouter_deletedverificationtoken = (
                    client.db.derouter_deletedverificationtoken
                )
                self.derouter_verificationtoken = client.db.derouter_verificationtoken
    ...
    async def test_delete_token_archive_serialization(self, repo):
        ...
        that don't exist on DeRouter_DeletedVerificationToken."""
        repo._prisma_client.db.derouter_verificationtoken._records["sk-arch"] = {
            "token": "sk-arch",
            "key_name": "Archive Me",
        ...
        archived = list(
            repo._prisma_client.db.derouter_deletedverificationtoken._records.values()
        )[0]
        ...
        assert (
            "sk-arch" not in repo._prisma_client.db.derouter_verificationtoken._records
        )
    ...
    async def test_find_by_id_maps_org_and_budget_columns(self, repo):
        ...
        repo._prisma_client.db.derouter_verificationtoken._records["sk-read"] = {
            "token": "sk-read",
    ...
    async def test_to_model_with_json_fields(self, repo):
        repo._prisma_client.db.derouter_verificationtoken._records["sk-json"] = {
            "token": "sk-json",
            "aliases": '{"alias1": "value1"}',
            "config": '{"setting": "val"}',
            "permissions": '{"read": true}',
            ...
            "derouter_budget_table": '{"budget_id": "b1"}',
        }
...
class TestTeamRepositoryExtended:
    ...
    async def test_delete_team_with_audit(self, repo):
        repo._prisma_client.db.derouter_teamtable._records["team-delete"] = {
            "team_id": "team-delete",
        ...
        class MockTx:
            def __init__(self, client):
                self.derouter_deletedteamtable = client.db.derouter_deletedteamtable
                self.derouter_teamtable = client.db.derouter_teamtable
    ...
    @pytest.mark.asyncio
    async def test_to_model_with_json_fields(self, repo):
        repo._prisma_client.db.derouter_teamtable._records["team-json"] = {
            "team_id": "team-json",
            "metadata": '{"key": "value"}',
            "model_spend": '{"gpt-4": 10.0}',
            "model_max_budget": '{"gpt-4": 100.0}',
...
class TestUserRepositoryExtended:
    ...
    async def test_delete_user_simple(self, repo):
        repo._prisma_client.db.derouter_usertable._records["user-delete"] = {
            "user_id": "user-delete",
    ...
    async def test_update_user_all_fields(self, repo):
        repo._prisma_client.db.derouter_usertable._records["user-update"] = {
            "user_id": "user-update",
            "teams": [],
            "models": [],
        }
    ...
    async def test_find_by_id_decodes_only_the_json_encoded_columns(self, repo):
        repo._prisma_client.db.derouter_usertable._records["user-json"] = {
            "user_id": "user-json",
...
class TestProjectRepositoryExtended:
    ...
    async def test_delete_project_simple(self, repo):
        repo._prisma_client.db.derouter_projecttable._records["proj-delete"] = {
...
class TestBudgetRepositoryExtended:
    ...
    async def test_update_budget_all_fields(self, repo):
        repo._prisma_client.db.derouter_budgettable._records["budget-update"] = {
...
class TestModelRepositoryExtended:
    ...
    async def test_find_by_team_id(self, mock_decrypt, repo):
        repo._prisma_client.db.derouter_proxymodeltable._records["model-1"] = {
            "model_id": "model-1",
            "model_name": "gpt-4",
        ...
        repo._prisma_client.db.derouter_proxymodeltable._records["model-2"] = {
            "model_id": "model-2",
            "model_name": "claude-3",
            "derouter_params": '{"api_key": "sk-other"}',
...
class TestBaseRepositoryExtended:
    ...
    async def test_find_many_with_pagination(self, repo):
        repo._prisma_client.db.derouter_budgettable._records = {
            "b1": {"budget_id": "b1", "max_budget": 100.0},
            "b2": {"budget_id": "b2", "max_budget": 200.0},
...
class TestTeamRepositoryArchiveData:
    ...
    def test_build_archive_data_minimal_fields(self, repo):
        ...
        team = DeRouter_TeamTable(team_id="team-minimal")
    ...
    def test_build_archive_data_with_all_valid_fields(self, repo):
        ...
        from derouter.models.team import Member
...
class TestPrismaTableRepository:
    def test_table_property_returns_named_delegate(self):
        ...
        assert agents.table._actions is prisma_client.db.derouter_agentstable
        assert policy.table._actions is prisma_client.db.derouter_policytable
        assert agents.table._actions is not policy.table._actions

    def test_table_access_raises_without_db(self):
        from derouter.repositories.table_repositories import SpendLogsRepository
    ...
    CONFIG_SYNCED_TABLE_NAMES = frozenset(
        {
            "derouter_agentstable",
            "derouter_cacheconfig",
            "derouter_configoverrides",
            "derouter_guardrailstable",
            "derouter_managedvectorstoreindextable",
            "derouter_managedvectorstorestable",
            "derouter_mcpservertable",
            "derouter_policyattachmenttable",
            "derouter_policytable",
            "derouter_prompttable",
            "derouter_searchtoolstable",
            "derouter_ssoconfig",
        }
    )

    def test_each_repository_binds_its_own_table_name(self):
        import derouter.repositories.table_repositories as tr
        from derouter.proxy.common_utils.config_sync_pubsub import (
            _PublishOnWriteActions,
        )
        ...
        for repo_cls in repos:
            ...
            assert name.startswith("derouter_")
...
class _ScimAwareUserTable:
    """Fake DeRouter_UserTable whose count() applies the JSON `where` filter the
...
class TestCountBillableUsers:
    def _repo(self, metadatas: List[Optional[Dict[str, Any]]]) -> UserRepository:
        client = MockPrismaClient()
        client.db.derouter_usertable = _ScimAwareUserTable(metadatas)
        return UserRepository(client)
    ...
    async def test_floors_at_zero_when_deactivated_exceeds_total(self):
        ...
        client = MockPrismaClient()
        client.db.derouter_usertable = _RacyTable()
