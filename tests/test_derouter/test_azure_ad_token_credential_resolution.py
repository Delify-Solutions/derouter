...
by model_id::
    ...
    return CredentialDeRouterParams(
        **deployment.derouter_params.model_dump(exclude_none=True)
    ).model_dump(exclude_none=True)

That re-validation is strict. Any field NOT declared on
``CredentialDeRouterParams`` gets dropped on the way through, even when
it was present on the original ``derouter_params``.
...
1. ``CredentialDeRouterParams`` directly accepts and round-trips
   ``azure_ad_token``.
2. ``Router.get_deployment_credentials_with_provider`` preserves
   ``azure_ad_token`` from a deployment's ``derouter_params``.
"""
...
class TestCredentialDeRouterParamsAzureAdToken:
    def test_azure_ad_token_round_trips_through_model_dump(self):
        ...
        assert dumped["azure_ad_token"] == "oauth-bearer-token-xyz", (
            "azure_ad_token dropped from CredentialDeRouterParams.model_dump() — "
            "every callsite that round-trips derouter_params through this class "
    ...
    def test_azure_ad_token_is_optional(self):
        ...
        params = CredentialDeRouterParams(api_key="sk-static")
        dumped = params.model_dump(exclude_none=True)
    ...
    def test_round_trip_preserves_full_credential_shape(self):
        ...
        rebuilt = CredentialDeRouterParams(
...
class TestRouterCredentialResolutionS3OutputBucket:
    ...
    the deployment's ``derouter_params`` through ``CredentialDeRouterParams``. If
