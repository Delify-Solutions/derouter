...
from derouter.utils import get_excluded_filtered_deployments
...
class TestGetExcludedFilteredDeployments:
    ...
    def test_handles_missing_model_info(self):
        deps = [
            {"model_name": "x", "derouter_params": {"model": "gpt-4o"}},  # no model_info
            _make_dep("b"),
        ]
        result = get_excluded_filtered_deployments(deps, excluded_deployment_ids=["b"])
        assert len(result) == 1
...
def test_set_failed_deployment_id_on_exception():
    router = Router(
        model_list=[
            {
                "model_name": "test-model",
                "derouter_params": {"model": "gpt-4o", "api_key": "key"},
                "model_info": {"id": "dep-a"},
            }
        ],
    )
...
async def test_ageneric_api_call_with_fallbacks_helper_stamps_failed_deployment_id():
    ...
    depending on which metadata bucket ("metadata" vs "derouter_metadata") it uses."""
    router = Router(
        model_list=[
            {
                "model_name": "test-model",
                "derouter_params": {"model": "openai/gpt-4o", "api_key": "test-key"},
                "model_info": {"id": "dep-a"},
            }
        ],
    )
...
async def test_ageneric_api_call_with_fallbacks_helper_stamps_dynamic_id_for_clientside_credentials():
    ...
    with pytest.raises(RuntimeError) as exc_info:
        await router._ageneric_api_call_with_fallbacks_helper(
            ...
            derouter_metadata={"model_group": "test-model"},
        )
...
async def test_acompletion_stamps_dynamic_id_for_clientside_credentials():
    ...
    with patch("derouter.acompletion", new_callable=AsyncMock, side_effect=RuntimeError("boom")):
        with pytest.raises(RuntimeError) as exc_info:
...
async def test_acompletion_stamps_dynamic_id_for_clientside_credentials_on_timeout():
    """Same bug as the RuntimeError case above, but for the separate `except derouter.Timeout`
    ...
    (`derouter.Timeout` is what `x-derouter-timeout` maps to) stamping the shared static id."""
    router = Router(
        model_list=[
            {
                "model_name": "test-model",
    ...
    timeout_exc = derouter.Timeout(message="boom", model="test-model", llm_provider="openai")
    with patch("derouter.acompletion", new_callable=AsyncMock, side_effect=timeout_exc):
...
def test_completion_stamps_dynamic_id_for_clientside_credentials():
    ...
    with patch("derouter.completion", side_effect=RuntimeError("boom")):
        with pytest.raises(RuntimeError) as exc_info:
            router._completion(
                model="test-model",
                messages=[{"role": "user", "content": "Hello"}],
...
async def test_maybe_run_weighted_failover_returns_none_without_failed_id():
    router = Router(
        model_list=[
            {
                "model_name": "test-model",
                "derouter_params": {"model": "gpt-4o", "api_key": "key", "weight": 1},
                "model_info": {"id": "A"},
            },
            {
                "model_name": "test-model",
...
async def test_maybe_run_weighted_failover_persists_excluded_ids_to_kwargs(monkeypatch):
    ...
    router = Router(
        model_list=[
            {
                "model_name": "test-model",
                "derouter_params": {"model": "gpt-4o", "api_key": "k", "weight": 1},
                "model_info": {"id": "A"},
            },
            {
                "model_name": "test-model",
    ...
    monkeypatch.setattr("derouter.router.run_async_fallback", _stub_run_async_fallback)
...
async def test_maybe_run_weighted_failover_skips_when_remaining_all_in_cooldown(
    ...
    monkeypatch.setattr("derouter.router.run_async_fallback", _should_not_be_called)
