...
def test_embedding_openai_omits_encoding_format_when_client_omits_it(respx_mock: respx.MockRouter) -> None:
    ...
    response: Final = derouter.embedding(model="openai/text-embedding-3-small", input=["hello"], api_key="sk-test")
...
def test_embedding_openai_explicit_encoding_format_wins_over_env_var(
    ...
    monkeypatch.setenv("DEROUTER_DEFAULT_EMBEDDING_ENCODING_FORMAT", "float")
    mock_route: Final = _mock_openai_embedding_route(respx_mock)
...
def test_embedding_openai_env_var_sets_default_encoding_format(
    ...
    monkeypatch.setenv("DEROUTER_DEFAULT_EMBEDDING_ENCODING_FORMAT", env_value)
    mock_route: Final = _mock_openai_embedding_route(respx_mock)

    derouter.embedding(model="openai/text-embedding-3-small", input=["hello"], api_key="sk-test")
...
def test_embedding_openai_env_none_omits_encoding_format(
    ...
    monkeypatch.setenv("DEROUTER_DEFAULT_EMBEDDING_ENCODING_FORMAT", env_none)
    mock_route: Final = _mock_openai_embedding_route(respx_mock)
...
async def test_aembedding_openai_omits_encoding_format_when_client_omits_it(
    ...
    response: Final = await derouter.aembedding(model="openai/text-embedding-3-small", input=["hello"], api_key="sk-test")
...
def test_embedding_openai_omitted_encoding_format_maps_provider_errors(
    ...
    assert int(exc_info.value.derouter_response_headers["retry-after"]) == 42
