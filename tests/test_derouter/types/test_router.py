...
from derouter.types.utils import CustomPricingDeRouterParams, MirroredPricingParams
...
def test_special_model_info_params_cannot_drift_from_the_mirror():
    ...
    assert set(SPECIAL_MODEL_INFO_PARAMS) <= set(CustomPricingDeRouterParams.model_fields)
    assert set(SPECIAL_MODEL_INFO_PARAMS) <= set(DeRouter_Params.model_fields)
...
def test_custom_pricing_params_keeps_every_field_it_had():
    ...
    CustomPricingDeRouterParams, whose model_fields drive custom-pricing detection."""
    for field in (
        ...
        assert field in CustomPricingDeRouterParams.model_fields


@pytest.mark.parametrize("field", SPECIAL_MODEL_INFO_PARAMS)
def test_deployment_mirrors_pricing_from_derouter_params_onto_model_info(field):
    value = [{"range": [0, 128000], "input_cost_per_token": 3e-06}] if field == "tiered_pricing" else 3e-06
    deployment = Deployment(
        model_name="my-model",
        derouter_params=DeRouter_Params(model="gpt-4o", **{field: value}),
    )
...
def test_deployment_mirrors_tiered_pricing_onto_model_info():
    """
    Regression: tiered_pricing set under a deployment's derouter_params was silently
    ...
    deployment = Deployment(
        model_name="my-model",
        derouter_params=DeRouter_Params(model="anthropic/claude-haiku-4-5", tiered_pricing=tiers),
    )
...
def test_drop_params_coerces_flags_and_keeps_unresolved_strings(value, expected):
    assert GenericDeRouterParams(drop_params=value).drop_params == expected
...
def test_drop_params_ignores_non_flag_non_string_values_with_a_warning(value, caplog):
    with caplog.at_level(logging.WARNING, logger="DeRouter"):
        assert GenericDeRouterParams(drop_params=value).drop_params is None
...
def test_drop_params_flags_and_strings_log_nothing(value, caplog):
    with caplog.at_level(logging.WARNING, logger="DeRouter"):
        GenericDeRouterParams(drop_params=value)
    assert caplog.text == ""
