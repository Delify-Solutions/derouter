...
class TestAssistantMessageImageUrlContent:
    ...
    def test_assistant_message_image_url_preserved_in_all_message_values(self):
        ...
        conversation = [
            {
                "role": "user",
                "content": "Generate an image of a banana wearing a DeRouter costume",
            },
...
class TestResponsesAPIReasoningNullFields:
    ...
    When a provider returns reasoning items without these fields, DeRouter's
...
def test_normalize_fine_tuning_job_dict_maps_azure_pending():
    from derouter.llms.openai.fine_tuning.handler import _normalize_fine_tuning_job_dict
...
class TestOpenAIFileObjectBatchGuardrailSerialization:
    """The proxy-only `derouter_batch_guardrail` key must reach the wire only when something set it."""
    ...
    @staticmethod
    def _report():
        from derouter.types.llms.openai import BatchGuardrailRecord, BatchGuardrailReport
    ...
    def test_key_absent_when_unset(self, mode):
        assert "derouter_batch_guardrail" not in self._file_object().model_dump(mode=mode)

    @pytest.mark.parametrize("mode", ["python", "json"])
    def test_key_present_when_set(self, mode):
        dumped = self._file_object(derouter_batch_guardrail=self._report()).model_dump(mode=mode)
        assert dumped["derouter_batch_guardrail"]["submitted_records"] == 3

    def test_nested_nulls_of_a_set_report_survive(self):
        """`exclude_none=True` was rejected as the fix because it would strip these."""
        dumped = self._file_object(derouter_batch_guardrail=self._report()).model_dump(mode="json")
        assert dumped["derouter_batch_guardrail"]["modified_records"] == [
    ...
    def test_by_alias_dump_also_omits_the_key(self):
        ...
        assert "derouter_batch_guardrail" not in self._file_object().model_dump(mode="json", by_alias=True)
    ...
    def test_round_trip_of_a_set_report_is_lossless(self):
        ...
        original = self._file_object(derouter_batch_guardrail=self._report())
    ...
    def test_serialization_json_schema_still_describes_the_model(self):
        ...
        assert "derouter_batch_guardrail" in schema["properties"]

    def test_key_omitted_inside_a_file_list_page(self):
        from derouter.types.llms.openai import FileListPage

        page = FileListPage(object="list", data=[self._file_object()], has_more=False)
        assert "derouter_batch_guardrail" not in page.model_dump(mode="json")["data"][0]
