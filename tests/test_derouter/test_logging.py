...
from derouter.constants import DEROUTER_TRUNCATED_PAYLOAD_FIELD
...
def test_json_formatter_includes_extra_attributes():
    ...
    record = logging.LogRecord(
        ...
        pathname="",
        lineno=0,
        msg="POST Request Sent from DeRouter",
        args=(),
        exc_info=None,
    )
    ...
    output = formatter.format(record)
    obj = json.loads(output)
    assert obj["message"] == "POST Request Sent from DeRouter"
...
def test_json_formatter_parses_embedded_python_dict_repr():
    ...
    msg = (
        ...
        "'derouter_params': {'api_key': 'sk**********', 'tpm': 1000000, 'rpm': 2000, "
        "'use_in_pass_through': False, 'use_derouter_proxy': False, "
    ...
    assert obj["derouter_params"]["api_key"] == "REDACTED"
    assert obj["derouter_params"]["tpm"] == 1000000
    assert obj["derouter_params"]["use_in_pass_through"] is False
...
def test_json_formatter_includes_component_field():
    ...
    This allows filtering by component (e.g. "DeRouter Proxy") in Datadog / third-party log services.
    """
    formatter = JsonFormatter()
    for logger_name in ("DeRouter Proxy", "DeRouter Router", "DeRouter"):
        record = logging.LogRecord(
            name=logger_name,
            level=logging.ERROR,
...
def test_json_formatter_includes_logger_field():
    ...
    record = logging.LogRecord(
        name="DeRouter Proxy",
        level=logging.INFO,
        pathname="/app/derouter/proxy/proxy_server.py",
        lineno=123,
...
DEROUTER_LOGGER_NAMES = frozenset(
...
DEROUTER_PACKAGE_ROOT = Path(__file__).resolve().parents[2] / "derouter"
...
def _is_logging_call(node: ast.AST) -> bool:
    return (
        ...
        and _receiver_name(node.func.value) in DEROUTER_LOGGER_NAMES
    )
...
def test_logging_calls_do_not_build_their_message_eagerly():
    ...
    offenders = tuple(
        offender
        for path in sorted(DEROUTER_PACKAGE_ROOT.rglob("*.py"))
        for offender in _eager_logging_calls(
            path.read_text(encoding="utf-8"), path.relative_to(DEROUTER_PACKAGE_ROOT.parent)
        )
    )
...
def test_trace_id_and_session_id_are_redacted_when_credential_shaped(monkeypatch):
    """A caller-controlled trace_id/session_id (e.g. from x-derouter-trace-id or a W3C
...
_MARKER_RE = re.compile(rf"\.\.\. \({DEROUTER_TRUNCATED_PAYLOAD_FIELD} skipped (\d+) chars\..*?\) \.\.\.", re.S)
...
def test_oversized_info_record_is_truncated(monkeypatch):
    ...
    payload = "p" * 100_000
    record = _make_record(logging.INFO, "derouter.acompletion(model=%s) Exception %s", ("gpt-4", payload))

    assert StdoutLogTruncationFilter().filter(record) is True

    message = record.getMessage()
    assert DEROUTER_TRUNCATED_PAYLOAD_FIELD in message
    assert len(message) <= 500
    assert message.startswith("derouter.acompletion(model=gpt-4) Exception ppp")
...
def test_oversized_traceback_is_truncated(monkeypatch):
    ...
    assert DEROUTER_TRUNCATED_PAYLOAD_FIELD in record.exc_text
...
def test_falsy_exc_info_is_not_formatted(monkeypatch):
    ...
    assert DEROUTER_TRUNCATED_PAYLOAD_FIELD in record.getMessage()
...
def test_oversized_error_is_truncated_end_to_end(monkeypatch, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="DeRouter Router"):
        verbose_router_logger.info("derouter.acompletion(model=%s) Exception %s", "gpt-4", "p" * 100_000)

    emitted = "".join(record.getMessage() for record in caplog.records)
    assert DEROUTER_TRUNCATED_PAYLOAD_FIELD in emitted
...
def test_uvicorn_access_logger_redacts_a_credential_it_is_handed():
    """Registration happens at derouter import; without it the filter never runs."""
...
def test_access_redaction_survives_the_uvicorn_json_log_config():
    """derouter hands uvicorn a dictConfig when json_logs is on. dictConfig clears a
