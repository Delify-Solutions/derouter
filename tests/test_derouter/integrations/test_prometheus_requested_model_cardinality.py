        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "openai/gpt-4o-mini", "api_key": "fake-key"},
            },
            {
                "model_name": "openai/*",
...
async def test_unknown_models_collapse_to_one_series_on_proxy_request_metrics(router):
    ...
    for metric in (
        logger.derouter_proxy_failed_requests_metric,
        logger.derouter_proxy_total_requests_metric,
    ):
...
async def test_team_alias_and_team_wildcard_models_keep_their_own_labels(team_router):
    ...
    with patch("derouter.proxy.proxy_server.llm_router", team_router, create=True):  # test-quality-ok: production reads proxy_server.llm_router lazily, no injection seam
...
async def test_unknown_models_collapse_to_other_when_router_is_unavailable():
    ...
    with patch("derouter.proxy.proxy_server.llm_router", None, create=True):  # test-quality-ok: production reads proxy_server.llm_router lazily, no injection seam
        await _fire_proxy_failure(logger, "agent-typo-no-router")
        await _fire_proxy_failure(logger, "gpt-4o-mini")

    assert _requested_model_values(logger.derouter_proxy_failed_requests_metric) == {
        UNRECOGNIZED_REQUESTED_MODEL_LABEL
    }
...
async def test_sdk_router_originated_metrics_keep_labels_without_proxy_router():
    ...
    assert _requested_model_values(logger.derouter_deployment_failure_responses) == {"sdk-deployment-group"}
    assert _requested_model_values(logger.derouter_deployment_failed_fallbacks) == {"sdk-fallback-group"}


@pytest.mark.asyncio
async def test_sdk_fallback_labels_survive_non_import_errors_from_proxy_module(monkeypatch):
    logger = PrometheusLogger()
    broken_proxy_module = types.ModuleType("derouter.proxy.proxy_server")
    ...
    monkeypatch.setitem(sys.modules, "derouter.proxy.proxy_server", broken_proxy_module)  # test-quality-ok: reproduces a proxy_server import raising non-ImportError, no injection seam
...
def test_unknown_models_collapse_to_one_series_on_deployment_metrics(router):
    ...
    for metric in (
        logger.derouter_deployment_failure_responses,
        logger.derouter_deployment_total_requests,
    ):
        assert _requested_model_values(metric) == {
            UNRECOGNIZED_REQUESTED_MODEL_LABEL,
            "gpt-4o-mini",
...
async def test_fallback_event_requested_model_is_bounded(router):
    ...
    assert _requested_model_values(logger.derouter_deployment_failed_fallbacks) == {
        UNRECOGNIZED_REQUESTED_MODEL_LABEL,
        "gpt-4o-mini",
    }
    assert _requested_model_values(logger.derouter_deployment_successful_fallbacks) == {
        UNRECOGNIZED_REQUESTED_MODEL_LABEL
    }
