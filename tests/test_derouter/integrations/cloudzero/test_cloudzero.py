...
class TestCloudZeroHourlyExport:
    ...
    async def test_hourly_export(self):
        ...
        with (
            patch.object(
                DeRouterDatabase, "_ensure_prisma_client"
            ) as mock_prisma_client_getter,
            patch.object(CloudZeroStreamer, "send_batched") as send_batched_mock,
            patch("derouter.integrations.cloudzero.cloudzero.datetime") as mock_datetime,
        ):
            ...
            fake_db.derouter_verificationtoken.find_many = AsyncMock(return_value=[])
            fake_db.derouter_deletedverificationtoken.find_many = AsyncMock(return_value=[])
            fake_db.derouter_usertable.find_many = AsyncMock(return_value=[])
...
class TestDeRouterDatabaseUsageData:
    @pytest.mark.asyncio
