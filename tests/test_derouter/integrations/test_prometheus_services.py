...
from derouter.integrations.prometheus_services import (
...
def test_is_metric_registered_does_not_use_registry_collect():
    ...
    for i in range(80):
        Counter(
            f"derouter_service_{i}_total_requests",
            "Total requests",
            labelnames=["service"],
            registry=registry,
        )
        Histogram(
            f"derouter_service_{i}_latency",
            "Latency",
            labelnames=["service"],
            registry=registry,
        )
    ...
    for _ in range(30):
        pl.is_metric_registered("derouter_service_0_latency")
        pl.is_metric_registered("derouter_service_79_total_requests")
...
def test_create_gauge_new():
    ...
    assert pl._get_metric("derouter_test_service_size") is gauge
