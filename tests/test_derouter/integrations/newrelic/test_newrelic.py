...
import derouter
import derouter.integrations.newrelic.newrelic as nr_module
...
# derouter's own startup imports), leaving _newrelic_agent=None. Point it at
...
def make_kwargs(
    ...
    """Build a minimal kwargs dict representative of a derouter callback invocation."""
    headers = {}
    if traceparent:
...
class TestNewRelicLoggerInit:
    ...
    def test_constructor_kwargs_take_priority_over_global_params(self):
        ...
        derouter.newrelic_params which defaults turn_off_message_logging to False."""
        from derouter.types.integrations.newrelic import NewRelicInitParams

        with patch("newrelic.agent.register_application"):
            with patch.dict(os.environ, NR_ENV):
                with patch(
                    "derouter.newrelic_params",
    ...
    def test_newrelic_params_plain_dict_branch(self):
        """derouter.newrelic_params can be a plain dict; it should be validated
...
class TestGetTraceContext:
    ...
    def test_extracts_trace_id_from_mixed_case_traceparent_header(self):
        ...
        kwargs = make_kwargs()
        kwargs["derouter_params"]["metadata"]["headers"] = {
            "Traceparent": "00-4bf92f3577b34da6a3ce929d0e0e4736-00f067aa0ba902b7-00"
        }
        trace_id = self.logger._get_trace_context(kwargs)
        assert trace_id == "4bf92f3577b34da6a3ce929d0e0e4736"
    ...
    def test_parse_failure_falls_through_to_synthetic_uuid(self):
        ...
        kwargs = {"derouter_params": {"metadata": {"headers": "not-a-dict"}}}
        trace_id = self.logger._get_trace_context(kwargs)
        assert trace_id is not None
...
class TestExtractAllMessagesRespectsLitellmRedaction:
    ...
    ``derouter.turn_off_message_logging=True`` (or the per-request equivalents),
    async streaming callbacks receive an unredacted
    ``async_complete_streaming_response``. Without consulting DeRouter's
    ...
    def test_enable_redaction_header_blocks_content(self):
        ...
        kwargs = make_kwargs(messages=[{"role": "user", "content": "user prompt"}])
        kwargs["derouter_params"]["metadata"]["headers"] = {
            "x-derouter-enable-message-redaction": True,
        }
        self._assert_no_content(logger, kwargs)
...
class TestStreamingResponse:
    ...
    When DeRouter assembles a streaming response, some providers produce a
...
class TestExplicitNoneValues:
    ...
    def test_trace_context_derouter_params_none(self):
        kwargs = make_kwargs()
        kwargs["derouter_params"] = None
        trace_id = self.logger._get_trace_context(kwargs)
        assert trace_id is not None  # falls back to UUID

    def test_trace_context_metadata_none(self):
        kwargs = make_kwargs()
        kwargs["derouter_params"] = {"metadata": None}
        trace_id = self.logger._get_trace_context(kwargs)
        assert trace_id is not None

    def test_trace_context_headers_none(self):
        kwargs = make_kwargs()
        kwargs["derouter_params"] = {"metadata": {"headers": None}}
        trace_id = self.logger._get_trace_context(kwargs)
        assert trace_id is not None
...
class TestProcessSuccess:
    def test_records_summary_and_message_events(self):
        ...
        assert summary_data["ingest_source"] == "derouter"
...
class TestRecordErrorMetric:
    ...
    def test_calls_record_custom_metric(self):
        ...
        mock_app.record_custom_metric.assert_called_once_with("LLM/DeRouter/Error", 1)
...
class TestEmitSupportabilityMetric:
    ...
    def test_records_metric_with_correct_name_and_value(self):
        ...
        with patch("newrelic.agent.application", return_value=mock_app):
            with patch.object(
                self.logger, "_get_derouter_version", return_value="1.80.0"
            ):
                self.logger._emit_supportability_metric()
        mock_app.record_custom_metric.assert_called_once_with(
            "Supportability/Python/ML/DeRouter/1.80.0", 1
        )
    ...
    def test_updates_last_emission_time(self):
        ...
        with patch("newrelic.agent.application", return_value=mock_app):
            with patch(
                "derouter.integrations.newrelic.newrelic.time.time",
...
class TestGetLitellmVersion:
    ...
    def test_returns_unknown_on_exception(self):
        with patch("importlib.metadata.version", side_effect=Exception("no package")):
            result = self.logger._get_derouter_version()
...
class TestExtractCompletionId:
    ...
    def test_uses_derouter_call_id_when_response_has_no_id(self):
        result = self.logger._extract_completion_id(
            kwargs={"derouter_call_id": "call-abc-123"},
            response_obj={},
        )
...
class TestStandardLoggingPayloadPreference:
    ...
    def test_vendor_from_slo(self):
        ...
        kwargs = {"derouter_params": {"custom_llm_provider": "kwargs-provider"}}
        assert (
