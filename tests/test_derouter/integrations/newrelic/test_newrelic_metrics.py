...
from derouter.integrations.newrelic.newrelic_metrics import (
    NewRelicMetricsLogger,
    _bucket_metrics,
    build_metric_payload,
)
from derouter.types.integrations.newrelic import (
...
class TestQueueAndFlush:
    ...
    async def test_sent_payload_window_starts_at_last_flush_time(self):
        ...
        with patch("derouter.integrations.newrelic.newrelic_metrics.time.time", return_value=2_010.0):
...
class TestErrorPolicy:
    ...
    async def test_403_drops_batch_and_names_permanent_credential_failure(self):
        ...
        with patch("derouter.integrations.newrelic.newrelic_metrics.verbose_logger") as mock_logger:
            await logger.async_send_batch()
...
class TestStopFlag:
    ...
    async def test_eviction_drains_queued_records(self):
        ...
        from derouter.derouter_core_utils.specialty_caches.dynamic_logging_cache import (
            DynamicLoggingCache,
        )
...
async def test_terminal_drop_leaves_untried_late_arrival_for_next_drain():
    ...
    logger = _make_logger()
    logger.stop()
    from derouter.types.integrations.newrelic import NEWRELIC_METRICS_MAX_DRAIN_PASSES
