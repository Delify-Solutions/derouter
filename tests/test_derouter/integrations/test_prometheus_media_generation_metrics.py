...
class TestMediaGenerationMetricsRegistration:
    ...
    def test_metrics_share_output_token_label_set(self):
        assert (
            PrometheusMetricLabels.derouter_video_duration_seconds_metric
            == PrometheusMetricLabels.derouter_output_tokens_metric
        )
        assert (
            PrometheusMetricLabels.derouter_images_generated_metric
            == PrometheusMetricLabels.derouter_output_tokens_metric
        )

    def test_runtime_label_set_matches_output_tokens_metric(self):
        """Full parity with derouter_output_tokens_metric, including the org labels
        appended via _org_label_metrics, so existing token dashboards can be cloned."""
        expected = PrometheusMetricLabels.get_labels("derouter_output_tokens_metric")
        for name in MEDIA_GENERATION_METRICS:
...
class TestIncrementMediaGenerationMetrics:
    def test_video_duration_incremented(self, sample_enum_values):
        ...
        logger.derouter_video_duration_seconds_metric.labels().inc.assert_called_once_with(8.0)
        logger.derouter_images_generated_metric.labels.assert_not_called()
    ...
    def test_image_count_incremented(self, sample_enum_values):
        ...
        logger.derouter_images_generated_metric.labels().inc.assert_called_once_with(2.0)
        logger.derouter_video_duration_seconds_metric.labels.assert_not_called()
