...
async def test_add_metrics_from_log(clean_env):
    ...
    # (no overhead metric because payload has no hidden_params derouter_overhead_time_ms)
    assert len(logger.log_queue) == 3

    metrics = {s["metric"]: s for s in logger.log_queue}

    # Total latency ~2s
    total = metrics["derouter.request.total_latency"]
    assert total["type"] == 3  # gauge
    assert abs(total["points"][0]["value"] - 2.0) < 0.1

    # LLM API latency ~1s
    llm = metrics["derouter.llm_api.latency"]
    assert llm["type"] == 3  # gauge
    assert abs(llm["points"][0]["value"] - 1.0) < 0.1

    # Request count
    count = metrics["derouter.llm_api.request_count"]
...
async def test_overhead_latency_metric_emitted(clean_env):
    """Test that derouter.overhead.latency is emitted when hidden_params contains derouter_overhead_time_ms."""
    logger = DatadogMetricsLogger(batch_size=100, start_periodic_flush=False)
    ...
    payload = StandardLoggingPayload(
        ...
        model="gpt-4o",
        hidden_params={
            "derouter_overhead_time_ms": 250.0,  # 250 ms of overhead
        },
    )
    ...
    assert (
        "derouter.overhead.latency" in metrics
    ), f"Expected 'derouter.overhead.latency' in emitted metrics, got: {list(metrics.keys())}"
    overhead = metrics["derouter.overhead.latency"]
...
async def test_overhead_latency_metric_absent_when_no_hidden_params(clean_env):
    """Test that derouter.overhead.latency is NOT emitted when hidden_params has no overhead value."""
    logger = DatadogMetricsLogger(batch_size=100, start_periodic_flush=False)
    ...
    payload = StandardLoggingPayload(
        custom_llm_provider="openai",
        model="gpt-4o",
        # No hidden_params / no derouter_overhead_time_ms
    )
    ...
    metrics = {s["metric"]: s for s in logger.log_queue}
    assert "derouter.overhead.latency" not in metrics
...
async def test_async_log_failure_event_extracts_status_code(clean_env):
    ...
    count_series = next(
        (s for s in logger.log_queue if s["metric"] == "derouter.llm_api.request_count"),
        None,
    )
...
async def test_async_send_batch(clean_env):
    ...
    logger.log_queue = [
        {
            "metric": "derouter.request.total_latency",
            "type": 3,
    ...
    assert payload["series"][0]["metric"] == "derouter.request.total_latency"
