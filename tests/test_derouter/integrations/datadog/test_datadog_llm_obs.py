...
from derouter.types.utils import StandardLoggingGuardrailInformation
...
def test_cache_tokens_are_reported_as_span_metrics(logger: DataDogLLMObsLogger) -> None:
    ...
    derouter's normalized prompt count includes both cache categories, so the three cache
...
def test_cache_metrics_come_from_the_normalized_field_not_the_anthropic_one(logger: DataDogLLMObsLogger) -> None:
    """
    derouter normalizes every provider's cache counters into prompt_tokens_details.
...
def test_every_spelling_of_cache_write_tokens_is_read(
    ...
    """A raw usage dict that bypassed derouter's normalizer can carry any provider's spelling."""
...
def test_redacted_payload_keeps_metrics_and_removes_sensitive_fields(logger: DataDogLLMObsLogger) -> None:
    ...
    assert result["meta"]["input"]["messages"][0]["content"] == "redacted-by-derouter"
    assert result["meta"]["output"]["messages"][0]["content"] == "redacted-by-derouter"
...
def test_a_failure_span_redacts_its_messages(logger: DataDogLLMObsLogger) -> None:
    ...
    assert result["meta"]["input"]["messages"] == [{"role": "user", "content": "redacted-by-derouter"}]
...
def test_redaction_keeps_the_conversation_shape_without_its_content() -> None:
    ...
    assert result["meta"]["input"]["messages"] == [
        ...
        {"role": "assistant", "content": "redacted-by-derouter"},
    ]
    assert result["meta"]["output"]["messages"] == [{"role": "assistant", "content": "redacted-by-derouter"}]
...
def test_redaction_drops_unrecognized_and_malformed_message_roles() -> None:
    ...
    assert result["meta"]["input"]["messages"] == [
        {"role": "", "content": "redacted-by-derouter"},
        {"role": "", "content": "redacted-by-derouter"},
        {"role": "", "content": "redacted-by-derouter"},
        {"role": "agent", "content": "redacted-by-derouter"},
    ]
...
def test_a_caller_supplied_redaction_header_cannot_blank_the_guardrail_record(
    ...
    payload["derouter_params"] = {"metadata": {"headers": {"x-derouter-enable-message-redaction": "true"}}}

    span = _span_json(logger, payload)
    record = span["meta"]["metadata"]["guardrail_information"][0]

    assert span["meta"]["input"]["messages"] == [{"role": "user", "content": "redacted-by-derouter"}]
    assert record["guardrail_status"] == "guardrail_intervened"
    assert record["masked_entity_count"] == {"EMAIL": 2}
...
def test_a_ddtrace_integer_parent_id_is_forwarded_as_its_string(logger: DataDogLLMObsLogger) -> None:
    ...
    kwargs["derouter_params"]["metadata"]["parent_id"] = 8675309
    start = datetime(2026, 9, 1, 12, 0, 0)
