...
async def test_async_log_failure_event_accepts_custom_metadata_labels(
    ...
    samples = _metric_samples("derouter_llm_api_failed_requests_metric_total")
    assert any(
...
def test_virtual_key_rate_limit_metrics_accept_custom_metadata_labels(
    ...
    samples = _metric_samples("derouter_remaining_api_key_requests_for_model")
    assert any(
        sample.labels.get("metadata_department") == "engineering"
        and sample.labels.get("metadata_environment") == "production"
        and sample.value == 3
...
def test_virtual_key_rate_limit_metrics_preserve_zero_remaining_values(
    ...
    metadata = {
        "model_group": "gpt-4o-mini",
        "derouter-key-remaining-requests-gpt-4o-mini": 0,
        "derouter-key-remaining-tokens-gpt-4o-mini": 0,
    }
    kwargs = {
        "derouter_params": {
            "metadata": metadata,
    ...
    request_samples = _metric_samples("derouter_remaining_api_key_requests_for_model")
    token_samples = _metric_samples("derouter_remaining_api_key_tokens_for_model")
