...
import derouter
from derouter.constants import MAX_S3_OBJECT_DOWNLOAD_FILENAME_BYTES, MAX_S3_OBJECT_KEY_BYTES
from derouter.integrations.s3 import S3Logger
...
def _run_log_event(callback_params: dict, response_id: str = "chatcmpl-test-id") -> MagicMock:
    original = derouter.s3_callback_params
    derouter.s3_callback_params = callback_params
    try:
        ...
        derouter.s3_callback_params = original
...
def test_put_object_keeps_the_configured_path_intact_when_only_the_id_has_to_shrink():
    ...
    long_path = "derouter-prod-logs/" + "t" * 921
    mock_s3_client = _run_log_event(
