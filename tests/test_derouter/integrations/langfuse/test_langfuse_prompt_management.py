...
class TestLangfusePromptManagement:
    ...
    def test_log_failure_event_runs_async_logger(self):
        langfuse_prompt_management = LangfusePromptManagement()
        with patch(
            "derouter.integrations.langfuse.langfuse_prompt_management.run_async_function"
        ) as mock_run_async:
    ...
    def test_langfuse_client_init_passes_dedicated_httpx_client(self):
        ...
        with (
            patch(
                "derouter.integrations.langfuse.langfuse_prompt_management.resolve_langfuse_credentials",
                return_value=("pk-1234", "sk-1234", "https://localhost"),
            ),
            patch(
                "derouter.integrations.langfuse.langfuse_prompt_management.LangFuseLogger._get_langfuse_flush_interval",
                return_value=1,
            ),
            patch.dict("sys.modules", {"langfuse": self._mock_langfuse}),
            patch(
                "derouter.llms.custom_httpx.http_handler.get_ssl_configuration",
                return_value=False,
            ) as mock_get_ssl,
        ):
