...
from derouter.types.utils import Choices, StandardCallbackDynamicParams
...
def test_set_usage_outputs_pydantic_completion_usage():
    """
    Regression test for https://github.com/Delify-Solutions/derouter/issues/13672
    ...
    from derouter.integrations.arize._utils import _set_usage_outputs
...
def test_set_usage_outputs_pydantic_response_api_usage():
    ...
    from derouter.integrations.arize._utils import _set_usage_outputs
    from derouter.types.llms.openai import OutputTokensDetails

    # Build an object that mimics openai ResponsesAPI usage but lacks `.get`
    # (uses a plain class — not BaseDeRouterOpenAIResponseObject)
...
async def test_arize_dynamic_params():
    ...
    derouter.callbacks = [test_arize_logger]
...
def test_passthrough_call_type_resolves_to_llm_span_kind():
    ...
    from derouter.integrations._types.open_inference import OpenInferenceSpanKindValues
    from derouter.integrations.arize._utils import _infer_open_inference_span_kind
...
def test_arize_emits_session_and_user_attrs_from_metadata():
    ...
    assert attrs["derouter.team_id"] == "team_7"
    assert attrs["derouter.team_alias"] == "alpha"
    assert attrs["derouter.key_alias"] == "key_alpha"
...
def test_arize_does_not_use_trace_id_as_session_id_fallback():
    ...
    own `derouter.trace_id` key instead.
    """
    from unittest.mock import MagicMock
    ...
    assert attrs["derouter.trace_id"] == "trace-xyz-123"
...
def test_arize_passthrough_bedrock_anthropic_normalization():
    ...
    kwargs = {
        ...
        "optional_params": {},
        "derouter_params": {"custom_llm_provider": "bedrock"},
    }
    ArizeLogger.set_arize_attributes(span, kwargs, response_obj)
    attrs = _collect_calls(span)
...
def test_arize_passthrough_call_type_does_not_run_on_chat_completion():
    ...
    from derouter.integrations.arize._utils import _maybe_normalize_passthrough
...
def test_arize_coerce_response_obj_passes_dicts_through_untouched():
    ...
    from derouter.integrations.arize._utils import _coerce_response_obj_for_attrs
...
def test_arize_mcp_call_tool_result_does_not_break_attribute_setting():
    ...
    kwargs = {
        ...
        "optional_params": {},
        "derouter_params": {"custom_llm_provider": "mcp"},
    }
