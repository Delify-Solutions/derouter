...
class TestResolveProjectName:
    ...
    def test_extracts_phoenix_name_from_derouter_params_metadata(self):
        kwargs = {
            "derouter_params": {
...
class TestProjectNameNotOnSpan:
    ...
    @patch("derouter.integrations.arize._utils.set_attributes")
...
class TestGetLitellmResourceForProject:
    ...
    def test_project_attrs_win_over_otel_resource_attributes_env(self):
        ...
        with patch.dict(
            ...
            resource = logger._get_derouter_resource_for_project("dynamic-proj")
    ...
    def test_preserves_deployment_environment_from_config(self):
        ...
        resource = logger._get_derouter_resource_for_project("my-proj")
...
class TestTracerResolutionAndCache:
    ...
    def test_standard_logging_metadata_wins_over_derouter_params(self):
        kwargs = {
            "standard_logging_object": {
...
class TestPhoenixTraceHandling:
    ...
    def test_handle_failure_sets_error_status_on_request_span(self):
        ...
        spans = exporter.get_finished_spans()
        request_spans = [s for s in spans if s.name == DEROUTER_REQUEST_SPAN_NAME]
    ...
    def test_proxy_mode_parent_and_child_share_trace_id(self):
        ...
        assert "derouter_proxy_request" in span_names
        assert DEROUTER_REQUEST_SPAN_NAME in span_names
...
def test_arize_phoenix_client_sanitize_id_rejects_traversal():
    from derouter.integrations.arize.arize_phoenix_client import _sanitize_id
...
def test_arize_phoenix_client_get_prompt_version_rejects_traversal():
    from derouter.integrations.arize.arize_phoenix_client import ArizePhoenixClient
