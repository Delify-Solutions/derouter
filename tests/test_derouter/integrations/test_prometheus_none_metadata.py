"""
Unit tests for Prometheus handling of None metadata in derouter_params.

When the Responses API sends streaming requests, derouter_params.metadata
...
class TestNoneMetadataHandling:
    ...
    derouter_params["metadata"] = None, causing:
    ...
    def test_set_llm_deployment_success_metrics_with_none_metadata(
        ...
        derouter_params.metadata is None.
        """
        request_kwargs = {
            "derouter_params": {
    ...
    def test_set_llm_deployment_success_metrics_with_missing_derouter_params(
        self, prometheus_logger
    ):
        """
        set_llm_deployment_success_metrics should not raise when
        derouter_params is missing entirely.
        """
        request_kwargs = {
            "model": "gpt-4o",
            "standard_logging_object": {
    ...
    def test_set_llm_deployment_success_metrics_with_derouter_metadata_key(
        self, prometheus_logger
    ):
        """
        set_llm_deployment_success_metrics should pick up derouter_metadata
        when metadata is None, using get_derouter_metadata_from_kwargs.
        """
        request_kwargs = {
            "derouter_params": {
                "metadata": None,
                "derouter_metadata": {"model_info": {"id": "test-model-id"}},
                "custom_llm_provider": "openai",
            },
            "model": "gpt-4o",
            "standard_logging_object": {
        ...
        # Should not raise, and should pick up derouter_metadata
        prometheus_logger.set_llm_deployment_success_metrics(
            request_kwargs=request_kwargs,
            start_time=datetime.now(),
            end_time=datetime.now(),
