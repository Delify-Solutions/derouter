...
def reset_redact_flag():
    ...
    original = derouter.redact_user_api_key_info
    yield
    derouter.redact_user_api_key_info = original
...
class TestLangsmithRedactUserApiKeyInfo:
    """Verify derouter.redact_user_api_key_info is honored for LangSmith."""
    ...
    def test_redact_disabled_keeps_user_api_key_fields(self, reset_redact_flag):
        ...
        derouter.redact_user_api_key_info = False
        logger = self._logger()
    ...
    def test_redact_enabled_strips_user_api_key_info_from_inputs(self, reset_redact_flag):
        ...
        data = logger._prepare_log_data(
            kwargs={"derouter_params": {"metadata": metadata}, "standard_logging_object": payload},
            response_obj=None,
            start_time=1.0,
            end_time=2.0,
            credentials=credentials,
...
class TestLangsmithRootRunIdConsistency:
    ...
    fans the header value out into derouter metadata as both trace_id and
    ...
    def _prepare(self, request_metadata):
        ...
        return logger._prepare_log_data(
            kwargs={
                "derouter_params": {"metadata": request_metadata},
                "standard_logging_object": payload,
            },
            response_obj=None,
            start_time=1.0,
