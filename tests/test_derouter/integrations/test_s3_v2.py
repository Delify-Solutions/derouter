...
class TestS3V2UnitTests:
    ...
    def test_s3_v2_source_code_analysis(self):
        ...
        from derouter.integrations import s3_v2
    ...
    @patch("derouter.integrations.s3_v2.CustomBatchLogger.periodic_flush")
    def test_s3_v2_endpoint_url(self, mock_periodic_flush, mock_create_task):
        """testing s3 endpoint url"""
        from unittest.mock import AsyncMock, MagicMock

        from derouter.types.integrations.s3_v2 import s3BatchLoggingElement
        ...
        expected_minio_url = (
            "https://minio.example.com:9000/derouter-logs/2025-09-14/test-key.json"
        )
        assert (
        ...
        with patch(
            "derouter.integrations.s3_v2._get_httpx_client",
...
async def test_s3_verify_false_handling(monkeypatch: pytest.MonkeyPatch):
    ...
    with patch("asyncio.create_task"):
        with patch(
            "derouter.integrations.s3_v2.get_async_httpx_client"
        ) as mock_get_client:
...
def test_s3_object_key_prefix_combinations(
    ...
    """
    with patch("derouter.integrations.s3_v2.get_s3_object_key") as mock_get_key:
...
def test_s3_object_key_sanitizes_slashes_in_file_name():
    ...
    assert key == (
        "DeRouterAPPLogs/myteam/2026-02-11/"
...
def test_s3_object_key_at_the_byte_limit_is_left_alone():
    ...
    from derouter.constants import MAX_S3_OBJECT_KEY_BYTES
    from derouter.integrations.s3 import get_s3_object_key
...
def test_s3_object_download_filename_is_bounded_for_oversized_response_id():
    ...
    from derouter.constants import MAX_S3_OBJECT_DOWNLOAD_FILENAME_BYTES
    from derouter.integrations.s3 import get_s3_object_download_filename
...
def test_s3_callback_params_override_uses_alternate_dict(monkeypatch):
    ...
    the override dict instead of `derouter.s3_callback_params`."""
    import derouter
...
def test_s3_callback_params_override_does_not_mutate_inputs(monkeypatch):
    ...
    or `derouter.s3_callback_params`."""
    import derouter

    monkeypatch.setenv("MY_AUDIT_BUCKET", "resolved-bucket")
    override = {"s3_bucket_name": "os.environ/MY_AUDIT_BUCKET"}
    monkeypatch.setattr(derouter, "s3_callback_params", {"s3_bucket_name": "os.environ/MY_AUDIT_BUCKET"})
    ...
    assert (
        derouter.s3_callback_params["s3_bucket_name"] == "os.environ/MY_AUDIT_BUCKET"
    )


def test_s3_callback_params_override_none_falls_back_to_global(monkeypatch):
    """No override → behaves exactly as today (reads `derouter.s3_callback_params`)."""
    import derouter

    monkeypatch.setattr(derouter, "s3_callback_params", {"s3_bucket_name": "from-global"})
    logger = S3Logger()
...
def test_s3_sse_kms_key_id_read_from_audit_override_params(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "s3_callback_params", {"s3_bucket_name": "normal-logs-bucket"})
    logger = S3Logger(
        s3_callback_params_override={
...
def test_sync_upload_signs_object_key_with_space_the_way_s3_does():
    ...
    with patch("derouter.integrations.s3_v2._get_httpx_client", return_value=mock_sync_client):
...
def _s3_logger_for_region(region_name: str) -> S3Logger:
    ...
    logger.s3_bucket_name = "my-derouter-audit"
...
@pytest.mark.parametrize(
    ...
    [
        (
            "cn-northwest-1",
            "https://my-derouter-audit.s3.cn-northwest-1.amazonaws.com.cn/2025-01-01/key.json",
        ),
        (
            "us-gov-west-1",
            "https://my-derouter-audit.s3.us-gov-west-1.amazonaws.com/2025-01-01/key.json",
        ),
        (
            "us-east-1",
            "https://my-derouter-audit.s3.us-east-1.amazonaws.com/2025-01-01/key.json",
        ),
    ],
)
