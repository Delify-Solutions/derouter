...
import derouter
from derouter.constants import SLACK_MODEL_DEPRECATION_LOCK_ID
from derouter.integrations.SlackAlerting.slack_alerting import SlackAlerting
from derouter.proxy._types import AlertType
from derouter.types.integrations.slack_alerting import SlackAlertingCacheKeys
...
DEAD_MODEL_COST = {
    "dead-model": {"deprecation_date": "2020-01-01", "derouter_provider": "openai"}
}
...
async def test_should_alert_once_the_alert_type_and_router_arrive_after_startup(
    ...
    with (
        ...
        patch(
            "derouter.integrations.SlackAlerting.slack_alerting.asyncio.sleep",
...
async def test_should_retry_on_the_next_poll_when_the_lock_claim_fails(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "model_cost", DEAD_MODEL_COST)
    alerting = SlackAlerting(
        alerting=["slack"], alert_types=[AlertType.model_deprecation_warnings]
    )
