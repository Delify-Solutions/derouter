...
async def test_langfuse_not_initialized_returns_none_early():
    ...
    derouter.logging_callback_manager = LoggingCallbackManager()

    # Create request data that would normally trigger processing
    request_data = {"derouter_logging_obj": MagicMock(), "trace_id": "test-trace-id"}
    ...
    # Verify the derouter_logging_obj was never accessed (early return)
    request_data["derouter_logging_obj"].assert_not_called()
