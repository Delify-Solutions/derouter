...
from derouter.constants import INTERNAL_CALL_ORIGIN_METADATA_KEY
from derouter.integrations.shadow_eval_logger import (
...
def _prisma(jobs=(), attempt_counts=(), attempt_costs=()) -> MagicMock:
    ...
    prisma = MagicMock()
    prisma.db.derouter_shadowevaljob.find_many = AsyncMock(return_value=list(jobs))
    prisma.db.derouter_shadowevalattempt.group_by = AsyncMock(
        return_value=[
            {
                "job_id": job_id,
                "_count": {"_all": count},
    ...
    prisma.db.derouter_shadowevalattempt.create = AsyncMock()
    return prisma
...
def _router(
    ...
    router.get_model_list = MagicMock(return_value=[{"derouter_params": {"model": "openai/gpt-4o-mini"}}])
...
def _success_kwargs(
    ...
    return {
        ...
        "derouter_params": {"metadata": request_metadata or {}},
...
class TestSurfaceNormalization:
    ...
    request through derouter's own transformations and judges only text-final turns."""
    ...
    async def test_anthropic_messages_arm_normalizes_blocks_and_system(self):
        ...
        prisma.db.derouter_shadowevalattempt.create.assert_called_once()
    ...
    async def test_anthropic_bridge_path_recovers_system_from_proxy_wire_body(self):
        ...
        hook_kwargs["derouter_params"]["proxy_server_request"] = {
    ...
    async def test_anthropic_arm_translates_wire_body_params_not_logged_optional_params(self):
        ...
        transport and derouter keys never do."""
        hook_kwargs = _success_kwargs(call_type="anthropic_messages")
        hook_kwargs["messages"] = [{"role": "user", "content": [{"type": "text", "text": "hi"}]}]
        ...
        hook_kwargs["derouter_params"]["proxy_server_request"] = {
            "body": {
                ...
                "derouter_metadata": {"user_api_key_hash": "key-hash"},
            }
        }
    ...
    async def test_turns_with_nothing_to_compare_are_skipped_without_consuming_budget(
        ...
        prisma.db.derouter_shadowevalattempt.create.assert_not_called()
...
def test_call_cost_prefers_the_billed_figure_over_the_public_price_map(monkeypatch):
    ...
    from derouter.integrations.shadow_eval_logger import _call_cost

    monkeypatch.setattr(derouter_module, "completion_cost", lambda completion_response: 0.005)
    stamped = MagicMock()
    stamped._hidden_params = {"response_cost": 0.04}
    assert _call_cost(stamped) == 0.04

    from derouter.types.utils import HiddenParams
...
class TestSuccessHookSkipChain:
    async def test_happy_path_writes_exactly_one_attempt_row(self, monkeypatch: pytest.MonkeyPatch):
        ...
        assert "stream" not in shadow_call
        create = prisma.db.derouter_shadowevalattempt.create
        ...
        assert prisma.db.derouter_shadowevaljob.find_many.await_count == 0
    ...
    async def test_shadow_call_messages_survive_in_place_provider_rewrites(self, monkeypatch: pytest.MonkeyPatch):
        ...
        row = prisma.db.derouter_shadowevalattempt.create.call_args.kwargs["data"]
        assert row["error"] is None
    ...
    async def test_pipeline_continues_judging_after_a_failed_attempt(self, monkeypatch: pytest.MonkeyPatch):
        ...
        rows = [c.kwargs["data"] for c in prisma.db.derouter_shadowevalattempt.create.call_args_list]
    ...
    async def test_completed_pipelines_hold_turn_budget_within_a_cache_generation(self):
        ...
        assert prisma.db.derouter_shadowevalattempt.create.await_count == 1
    ...
    async def test_a_sibling_pod_sees_spend_through_the_shared_counter(self, monkeypatch):
        ...
        assert prisma_a.db.derouter_shadowevalattempt.create.await_count == 1
        prisma_b.db.derouter_shadowevalattempt.create.assert_not_called()
    ...
    async def test_v1_messages_surface_forwards_identity_from_derouter_metadata(self):
        """/v1/messages stores identity in derouter_params.derouter_metadata, so the hook
        ...
        hook_kwargs = _success_kwargs()
        hook_kwargs["derouter_params"] = {
            "derouter_metadata": {"user_api_key_hash": "key-hash", "user_api_key_team_id": "team-1"}
        }
...
class TestTargetMatching:
    ...
    async def test_jwt_shaped_traffic_matches_team_and_user_jobs_but_no_key_job(self, target, sampled):
        ...
        if sampled:
            prisma.db.derouter_shadowevalattempt.create.assert_awaited_once()
            assert prisma.db.derouter_shadowevalattempt.create.call_args.kwargs["data"]["job_id"] == "job-1"
        else:
            router.acompletion.assert_not_called()
            prisma.db.derouter_shadowevalattempt.create.assert_not_called()
    ...
    async def test_an_event_matching_a_key_job_and_a_team_job_fires_both(self):
        ...
        rows = [call.kwargs["data"] for call in prisma.db.derouter_shadowevalattempt.create.call_args_list]
...
class TestModelScope:
    ...
    async def test_scope_admits_only_the_named_groups_and_counts_nothing_else(self, requested, sampled):
        ...
        assert prisma.db.derouter_shadowevalattempt.create.await_count == (1 if sampled else 0)
    ...
    async def test_an_alias_and_its_target_are_one_group_on_both_sides(self, scoped_to, requested):
        ...
        prisma.db.derouter_shadowevalattempt.create.assert_awaited_once()
        assert prisma.db.derouter_shadowevaljob.find_many.await_count == 1
    ...
    async def test_a_repointed_alias_applies_to_the_next_request_without_a_cache_refill(self):
        ...
        rows = [call.kwargs["data"]["request_id"] for call in prisma.db.derouter_shadowevalattempt.create.call_args_list]
...
class TestActiveJobsCache:
    async def test_cache_miss_reads_db_once_then_serves_from_cache(self):
        ...
        assert prisma.db.derouter_shadowevaljob.find_many.await_count == 1
        where = prisma.db.derouter_shadowevaljob.find_many.call_args.kwargs["where"]
        assert where["stopped_at"] is None
        assert "gt" in where["ends_at"]
        count_where = prisma.db.derouter_shadowevalattempt.group_by.call_args.kwargs["where"]
    ...
    async def test_no_active_jobs_is_cached_too(self):
        ...
        assert await logger._active_jobs() == {}
        assert prisma.db.derouter_shadowevaljob.find_many.await_count == 1
        prisma.db.derouter_shadowevalattempt.group_by.assert_not_called()

    async def test_db_fault_returns_empty_without_caching_the_fault(self):
        prisma = _prisma()
        prisma.db.derouter_shadowevaljob.find_many = AsyncMock(side_effect=RuntimeError("db blip"))
        logger = ShadowEvalLogger(
            router_provider=lambda: None,
            prisma_provider=lambda: prisma,
            jobs_cache=InMemoryCache(max_size_in_memory=4, default_ttl=60),
        ...
        assert await logger._active_jobs() == {}
        assert await logger._active_jobs() == {}
        assert prisma.db.derouter_shadowevaljob.find_many.await_count == 2
...
class TestShadowPipeline:
    ...
    async def test_failures_become_error_rows_and_keep_billed_judge_cost(
        ...
        monkeypatch.setattr(derouter_module, "completion_cost", lambda completion_response: 0.007)
        prisma = _prisma()
    ...
    async def _judge_error(self, router: MagicMock, monkeypatch: pytest.MonkeyPatch) -> str:
        ...
        return prisma.db.derouter_shadowevalattempt.create.call_args.kwargs["data"]["error"]
    ...
    async def _judged_shadow_row(self, router: MagicMock, shadow_params: dict | None = None) -> dict:
        ...
        return prisma.db.derouter_shadowevalattempt.create.call_args.kwargs["data"]
    ...
    async def test_a_pipeline_error_after_the_shadow_call_keeps_its_billed_cost(self, monkeypatch: pytest.MonkeyPatch):
        ...
        import derouter as derouter_module
        import derouter.integrations.shadow_eval_logger as shadow_eval_module
...
class TestDirection:
    ...
    async def test_direction_decides_which_traffic_is_sampled(self, job, routed_by, attempt_rows):
        ...
        assert prisma.db.derouter_shadowevalattempt.create.await_count == attempt_rows
...
class TestMultiRouterArms:
    async def test_every_arm_judges_the_same_request_and_stamps_its_own_row(self):
        ...
        rows = [call.kwargs["data"] for call in prisma.db.derouter_shadowevalattempt.create.await_args_list]
...
async def test_judge_call_resolves_its_arm_under_the_shadowed_keys_team(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    import derouter
    from derouter.derouter_core_utils.llm_judge import judge_acompletion
    ...
    monkeypatch.setattr(derouter, "acompletion", sdk)
...
class TestSamplingFunnel:
    ...
    async def test_a_lost_sampling_dice_roll_counts_not_sampled(self):
        from derouter.integrations.shadow_eval_logger import _sample_hits
        ...
        prisma.db.derouter_shadowevalattempt.create.assert_not_awaited()
