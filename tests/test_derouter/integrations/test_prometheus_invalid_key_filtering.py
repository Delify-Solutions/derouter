...
class TestInvalidAPIKeyDetection:
    ...
    def test_auth_error_message_detection(self, prometheus_logger):
        exception = AssertionError(
            "DeRouter Virtual Key expected. Received=invalid-key-12345, expected to start with 'sk-'."
        )
        assert (
...
class TestAsyncHooks:
    ...
    async def test_post_call_failure_hook_skips_401(
        ...
        with (
            patch.object(
                prometheus_logger, "derouter_proxy_failed_requests_metric"
            ) as mock_failed,
            patch.object(
                prometheus_logger, "derouter_proxy_total_requests_metric"
            ) as mock_total,
        ):
    ...
    async def test_log_failure_event_skips_401(self, prometheus_logger):
        ...
        with (
            patch.object(
                prometheus_logger, "derouter_llm_api_failed_requests_metric"
            ) as mock_failed,
            patch.object(
