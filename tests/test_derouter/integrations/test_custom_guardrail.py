...
from derouter.proxy._types import CallTypes, UserAPIKeyAuth
from derouter.types.guardrails import GuardrailEventHooks, Mode
from derouter.types.utils import GenericGuardrailAPIInputs, GuardrailTracingDetail
...
class TestCustomGuardrailDeploymentHook:
    ...
    def test_mark_pre_call_hook_ran_uses_derouter_metadata(self):
        """The marker is recorded in derouter_metadata when that is the metadata
        bucket in use, and is then visible to the skip check."""
        from derouter.constants import PRE_CALL_EXECUTED_GUARDRAILS_KEY

        guardrail = CustomGuardrail(guardrail_name="g1")
        kwargs = {"derouter_metadata": {}}

        guardrail.mark_pre_call_hook_ran(kwargs)

        assert kwargs["derouter_metadata"][PRE_CALL_EXECUTED_GUARDRAILS_KEY]
...
class TestCustomGuardrailShouldRunGuardrail:
    def test_should_run_guardrail_with_derouter_metadata(self):
        """Test that should_run_guardrail works with derouter_metadata pattern"""
        ...
        # Test with guardrails in derouter_metadata
        data = {
            "model": "gpt-3.5-turbo",
            "derouter_metadata": {"guardrails": ["test_guardrail"]},
        }
    ...
    def test_should_run_guardrail_no_matching_guardrail(self):
        ...
        data = {
            "model": "gpt-3.5-turbo",
            "derouter_metadata": {"guardrails": ["different_guardrail"]},
        }
    ...
    def test_should_run_guardrail_with_disable_global_guardrail(self):
        ...
        # Test 5: Admin config in metadata isn't shadowed by user-supplied derouter_metadata
        data_cross_key = {
            "model": "gpt-3.5-turbo",
            "messages": [{"role": "user", "content": "test"}],
            "metadata": {"user_api_key_metadata": {"disable_global_guardrails": True}},
            "derouter_metadata": {"request_tags": ["user-supplied"]},
        }
        result = custom_guardrail.should_run_guardrail(data=data_cross_key, event_type=GuardrailEventHooks.pre_call)
        assert result is False, "Admin config in metadata must not be shadowed by user-supplied derouter_metadata"
        ...
        data_post_strip = {
            ...
            "messages": [{"role": "user", "content": "test"}],
            "metadata": {"user_api_key_metadata": {"disable_global_guardrails": True}},
            "derouter_metadata": {},  # post-strip: attacker payload removed
        }
...
class TestGuardrailLoggingAggregation:
    ...
    def test_appends_to_derouter_metadata(self):
        request_data = {
            "derouter_metadata": {"standard_logging_guardrail_information": [{"guardrail_name": "derouter_existing"}]}
        }

        self._invoke_add_log(request_data)

        info = request_data["derouter_metadata"]["standard_logging_guardrail_information"]
        assert isinstance(info, list)
        assert len(info) == 2
        assert info[1]["guardrail_name"] == "test_guardrail"
    ...
    def test_caller_metadata_does_not_divert_the_entry_from_the_reader(self):
        ...
        spend log never reads. Routes in DEROUTER_METADATA_ROUTES (/v1/messages,
        /v1/responses, batches, files) seed `derouter_metadata`, and Claude Code sends
        `metadata.user_id`, so both keys are present on the same request."""
        request_data = {
            "metadata": {"user_id": "device-account-session"},
            "derouter_metadata": {"user_api_key_hash": "abc"},
        }
    ...
    def test_entry_and_applied_guardrails_header_share_one_bucket(self):
        """The x-derouter-applied-guardrails writer and the guardrail-info writer must
...
class TestGuardrailOtelSpanEmission:
    ...
    def test_emits_span_for_recorded_entry(self, monkeypatch):
        captured = []
        monkeypatch.setattr(
            "derouter.integrations.otel.logger.emit_guardrail_span",
            captured.append,
        )
    ...
    def test_span_emission_failure_does_not_break_recording(self, monkeypatch):
        ...
        monkeypatch.setattr("derouter.integrations.otel.logger.emit_guardrail_span", _boom)
...
class TestCustomGuardrailPassthroughSupport:
    ...
    def test_is_valid_response_type_with_typeddict_error(self):
        ...
        # Create a valid DeRouter response object
...
class TestInjectAdvisoryMessage:
    ...
    def test_prefers_instructions_over_input_for_responses_api(self):
        """
        Veria-ai finding on Delify-Solutions/derouter#34940: "instructions" is the
    ...
    def test_returns_false_and_does_not_mutate_when_messages_also_present_alongside_structured_input(self):
        """
        Bugbot finding on Delify-Solutions/derouter#34940: a request can carry both a
...
class TestEventTypeLogging:
    ...
    async def test_log_guardrail_information_infers_event_type_from_async_pre_call_hook(
        ...
        """
        from derouter.integrations.custom_guardrail import log_guardrail_information
        from derouter.types.guardrails import GuardrailEventHooks
...
class TestApplyGuardrailStyleDeploymentDispatch:
    ...
    async def test_fails_closed_when_proxy_extras_missing(self):
        ...
        with patch.dict(sys.modules, {"derouter.proxy.utils": None}):
            with pytest.raises(ImportError, match="derouter\\[proxy\\]"):
...
class TestOnlyScanNewMessages:
    ...
    async def test_disabled_returns_none(self):
        ...
        result = await guardrail.filter_new_texts_for_session(
            texts=["hi"],
            request_data={"derouter_session_id": "s1"},
            cache=self._cache(),
        )
        assert result is None
    ...
    async def test_dedupes_previously_scanned_texts(self):
        guardrail = self._guardrail()
        cache = self._cache()
        request = {"derouter_session_id": "sess-dedupe"}
    ...
    async def test_no_new_texts_returns_empty(self):
        guardrail = self._guardrail()
        cache = self._cache()
        request = {"derouter_session_id": "sess-empty"}
        texts = ["only message"]
    ...
    async def test_modified_earlier_text_is_rescanned(self):
        guardrail = self._guardrail()
        cache = self._cache()
        request = {"derouter_session_id": "sess-edit"}
        original = ["original"]
    ...
    async def test_blocked_scan_does_not_persist_hashes(self):
        guardrail = self._guardrail()
        cache = self._cache()
        request = {"derouter_session_id": "sess-blocked"}
    ...
    async def test_scanned_hashes_written_with_fixed_ttl(self):
        ...
        from derouter.constants import GUARDRAIL_SCANNED_MESSAGES_CACHE_TTL_SECONDS

        guardrail = self._guardrail()
        cache = self._cache()
        cache.async_set_cache = AsyncMock()
        request = {"derouter_session_id": "sess-ttl"}
    ...
    @pytest.mark.asyncio
    async def test_session_id_from_derouter_metadata_is_used_for_dedupe(self):
        guardrail = self._guardrail()
        cache = self._cache()
        request = {"derouter_metadata": {"session_id": "sess-lmeta"}}
        texts = ["shared message"]
    ...
    async def test_mark_texts_scanned_disabled_does_not_persist(self):
        ...
        await guardrail.mark_texts_scanned(texts=["a"], request_data={"derouter_session_id": "s1"}, cache=cache)
...
def _guardrail_entries(request_data: dict) -> list:
    container = request_data.get("metadata") or request_data.get("derouter_metadata") or {}
...
class _UndecoratedGuardrail(CustomGuardrail):
    ...
    async def apply_guardrail(
        ...
        logging_obj: Optional["DeRouterLoggingObj"] = None,
    ) -> GenericGuardrailAPIInputs:
        from derouter.exceptions import GuardrailRaisedException
...
def _logged_call(messages: list | str) -> tuple[dict, object]:
    ...
    kwargs = {
        ...
        "derouter_params": {"metadata": {"user_api_key_user_id": "u1"}},
        "optional_params": {},
...
class TestLoggingOnlyApplyGuardrail:
    ...
    async def test_content_filter_accepts_logging_only_and_records_detection(
        ...
        from derouter.types.guardrails import BlockedWord, ContentFilterAction, GuardrailEventHooks
    ...
    async def test_runs_apply_guardrail_observe_only_and_records_verdict(self):
        ...
        assert "standard_logging_guardrail_information" not in kwargs["derouter_params"]["metadata"]
    ...
    async def test_appends_to_pre_call_verdicts_without_duplicating_them(self):
        ...
        kwargs["derouter_params"]["metadata"]["standard_logging_guardrail_information"] = [pre_call_entry]
        ...
        assert kwargs["derouter_params"]["metadata"]["standard_logging_guardrail_information"] == [pre_call_entry]
    ...
    async def test_async_success_handler_records_verdict_in_standard_logging_object(self):
        ...
        logging_obj.update_environment_variables(
            derouter_params={"metadata": {}}, optional_params={}, model="gpt-5.4-mini", custom_llm_provider="openai"
        )
...
class TestCustomGuardrailPostCallSuccessDeploymentHook:
    ...
    which starved every later callback in derouter.callbacks (notably the lazily-appended
