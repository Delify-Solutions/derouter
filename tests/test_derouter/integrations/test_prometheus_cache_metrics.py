...
Run with: uv run pytest tests/test_derouter/integrations/test_prometheus_cache_metrics.py -v
"""
...
class TestPrometheusCacheMetrics:
    ...
    def test_cache_metrics_defined_in_types(self):
        ...
        from derouter.types.integrations.prometheus import DEFINED_PROMETHEUS_METRICS
        from typing import get_args

        defined_metrics = get_args(DEFINED_PROMETHEUS_METRICS)

        assert "derouter_cache_hits_metric" in defined_metrics
        assert "derouter_cache_misses_metric" in defined_metrics
        assert "derouter_cached_tokens_metric" in defined_metrics
        assert "derouter_provider_cache_read_input_tokens_metric" in defined_metrics
        assert "derouter_provider_cache_creation_input_tokens_metric" in defined_metrics
    ...
    def test_cache_metric_labels_defined(self):
        ...
        assert hasattr(PrometheusMetricLabels, "derouter_cache_hits_metric")
        assert hasattr(PrometheusMetricLabels, "derouter_cache_misses_metric")
        assert hasattr(PrometheusMetricLabels, "derouter_cached_tokens_metric")
        assert hasattr(
            PrometheusMetricLabels, "derouter_provider_cache_read_input_tokens_metric"
        )
        assert hasattr(
        ...
        for label in expected_labels:
            assert label in PrometheusMetricLabels.derouter_cache_hits_metric
            assert label in PrometheusMetricLabels.derouter_cache_misses_metric
            assert label in PrometheusMetricLabels.derouter_cached_tokens_metric
            assert (
                label
                in PrometheusMetricLabels.derouter_provider_cache_read_input_tokens_metric
            )
            assert (
                label
                in PrometheusMetricLabels.derouter_provider_cache_creation_input_tokens_metric
            )
    ...
    def test_increment_cache_metrics_on_cache_hit(self, sample_enum_values):
        ...
        # Create mock metrics
        mock_logger.derouter_cache_hits_metric = MagicMock()
        mock_logger.derouter_cache_misses_metric = MagicMock()
        mock_logger.derouter_cached_tokens_metric = MagicMock()
        mock_logger.derouter_provider_cache_read_input_tokens_metric = MagicMock()
        mock_logger.derouter_provider_cache_creation_input_tokens_metric = MagicMock()
        ...
        mock_logger.derouter_cache_hits_metric.labels.assert_called()
        mock_logger.derouter_cache_hits_metric.labels().inc.assert_called_once()

        # Verify cached tokens metric was incremented with total_tokens
        mock_logger.derouter_cached_tokens_metric.labels.assert_called()
        mock_logger.derouter_cached_tokens_metric.labels().inc.assert_called_once_with(
            100
        )

        # Verify cache misses metric was NOT called
        mock_logger.derouter_cache_misses_metric.labels.assert_not_called()

        # Verify provider prompt caching metrics were incremented
        mock_logger.derouter_provider_cache_read_input_tokens_metric.labels().inc.assert_called_once_with(
            25
        )
        mock_logger.derouter_provider_cache_creation_input_tokens_metric.labels().inc.assert_called_once_with(
            10
        )
    ...
    def test_increment_cache_metrics_on_cache_miss(self, sample_enum_values):
        ...
        mock_logger.derouter_cache_misses_metric.labels.assert_called()
        mock_logger.derouter_cache_misses_metric.labels().inc.assert_called_once()

        # Verify cache hits and cached tokens metrics were NOT called
        mock_logger.derouter_cache_hits_metric.labels.assert_not_called()
        mock_logger.derouter_cached_tokens_metric.labels.assert_not_called()

        # Provider prompt caching metrics should still be emitted
        mock_logger.derouter_provider_cache_read_input_tokens_metric.labels().inc.assert_called_once_with(
            20
        )
        mock_logger.derouter_provider_cache_creation_input_tokens_metric.labels.assert_not_called()
    ...
    def test_provider_cache_read_does_not_fallback_on_explicit_zero(
        ...
        mock_logger.derouter_provider_cache_read_input_tokens_metric.labels.assert_not_called()
    ...
    def test_provider_cache_creation_fallback_to_cache_creation_tokens(
        ...
        """Normalized derouter usage dumps carry cache_creation_tokens in
