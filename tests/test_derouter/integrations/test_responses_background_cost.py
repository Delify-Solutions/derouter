...
def _check_responses_cost_module_available():
    """Check if derouter_enterprise.proxy.common_utils.check_responses_cost module is available"""
    try:
        from derouter_enterprise.proxy.common_utils.check_responses_cost import (  # noqa: F401
            CheckResponsesCost,
        )
...
@pytest.mark.skipif(
    ...
    reason="derouter_enterprise.proxy.common_utils.check_responses_cost module not available (enterprise-only feature)",
)
class TestCheckResponsesCost:
    ...
    async def test_check_responses_cost_no_jobs(
        ...
        mock_prisma_client.db.derouter_managedobjecttable.find_many.assert_called_once_with(
            where={
    ...
    async def test_check_responses_cost_with_completed_job(
        ...
        # Mock derouter.aget_responses to return completed response
    ...
    async def test_check_responses_cost_error_handling(
        ...
        # Mock derouter.aget_responses to raise an exception
        with patch(
