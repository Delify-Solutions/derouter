...
``derouter.callback_settings['otel']['attributes']``, which the proxy populates
...
from derouter.constants import (  # noqa: E402
    DEROUTER_LOGGING_NO_UPSTREAM_LLM_CALL,
)
from derouter.integrations.otel.logger import OpenTelemetryV2  # noqa: E402
from derouter.integrations.otel.model.config import (  # noqa: E402
    OpenTelemetryV2Config,
)
from derouter.integrations.otel.plumbing.metrics import (  # noqa: E402
    GenAIMetricRecorder,
    create_genai_metrics,
)
from derouter.integrations.otel.plumbing.providers import (  # noqa: E402
...
def _build_call(
    ...
    derouter could not attribute to a provider."""
    ...
    kwargs = {
        "model": "gpt-4o-mini",
        "call_type": call_type,
        "derouter_params": ({"custom_llm_provider": provider} if provider is not None else {}),
        ...
        "standard_logging_object": {
            ...
            "hidden_params": {"derouter_call_id": "abc", "model_id": "m-1"},
        },
    }
    response_obj = {
        "usage": {
...
def test_two_calls_differing_only_per_request_share_one_series():
    ...
    for index, cost in enumerate((RESPONSE_COST, RESPONSE_COST * 3)):
        ...
        kwargs["standard_logging_object"]["hidden_params"] = {
            ...
            "derouter_call_id": f"call-{index}",
            "cache_key": f"cache-{index}",
            "response_cost": cost,
            "derouter_overhead_time_ms": 1.5 + index,
...
def test_deprecated_gen_ai_system_is_dual_emitted_verbatim():
    """gen_ai.system keeps its raw derouter provider value alongside the new key
...
def test_no_provider_attribute_when_provider_is_absent():
    """A call derouter could not attribute to a provider carries no provider label
...
@pytest.mark.parametrize(
    "call_type,expected",
    [
        ("avector_store_create", "derouter.vector_store_management"),
        ("avector_store_delete", "derouter.vector_store_management"),
        ("avector_store_file_create", "derouter.vector_store_file_management"),
        ("avector_store_file_list", "derouter.vector_store_file_management"),
    ],
)
...
def _build_failure(
    *,
    ...
    Mirrors what derouter actually hands ``async_log_failure_event``: no
...
def _drive_failure(reader, callback_settings_attributes=None, **failure_kwargs):
    ...
    previous = derouter.callback_settings
    if callback_settings_attributes is not None:
        derouter.callback_settings = {"otel": {"attributes": callback_settings_attributes}}
    try:
