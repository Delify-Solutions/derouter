...
import derouter
from derouter.integrations.otel import (
...
from derouter.integrations.otel.model import spans as spans_mod
from derouter.integrations.otel.model.payloads import (
    LLMCallSpanData,
    RequestIdentity,
    _upstream_address_port,
)
from derouter.integrations.otel.model.spans import (
    SPAN_REGISTRY,
    DeRouterSpanKind,
    SpanRole,
    child_roles,
...
def test_registry_hierarchy_shape():
    ...
    assert SPAN_REGISTRY[SpanRole.LLM_CALL].kind is DeRouterSpanKind.CLIENT
    ...
    assert SPAN_REGISTRY[SpanRole.MCP_TOOL_CALL].kind is DeRouterSpanKind.CLIENT
    assert SPAN_REGISTRY[SpanRole.MCP_LIST_TOOLS].kind is DeRouterSpanKind.CLIENT
    ...
    assert SPAN_REGISTRY[SpanRole.PROXY_REQUEST].kind is DeRouterSpanKind.SERVER
    ...
    assert SPAN_REGISTRY[SpanRole.DB_CALL].kind is DeRouterSpanKind.CLIENT
    assert SPAN_REGISTRY[SpanRole.SERVICE].kind is DeRouterSpanKind.INTERNAL
...
def test_attribute_keys_are_unique_across_namespaces():
    from derouter.integrations.otel import MCP, Client, JsonRpc, DeRouterError, Network

    # prefixes are allowed to be substrings; exact keys must not collide.
    exact = set()
    for cls in (GenAI, Error, DeRouterError, Server, HTTP, DB, MCP, JsonRpc, Network, Client):
...
def test_mcp_attribute_vocabulary_is_complete():
    ...
    """
    from derouter.integrations.otel import MCP, Client, JsonRpc, Network
...
def test_vector_store_management_is_not_chat(call_type):
    ...
    the chat series."""
    assert resolve_operation(call_type) is GenAIOperation.DEROUTER_VECTOR_STORE_MANAGEMENT
    assert resolve_operation(call_type).value == "derouter.vector_store_management"
...
def test_vector_store_file_management_is_not_chat(call_type):
    ...
    assert resolve_operation(call_type) is GenAIOperation.DEROUTER_VECTOR_STORE_FILE_MANAGEMENT
    assert resolve_operation(call_type).value == "derouter.vector_store_file_management"
...
def test_responses_management_is_not_chat(call_type):
    ...
    assert resolve_operation(call_type) is GenAIOperation.DEROUTER_RESPONSES_MANAGEMENT
    assert resolve_operation(call_type).value == "derouter.responses_management"
...
_NON_CHAT_ROUTES: Final = (
    ...
    ("moderation", GenAIOperation.DEROUTER_MODERATION, None),
)
...
def test_non_chat_route_spans_carry_semconv_name_and_modality(call_type, operation, output_type):
    ...
    ``derouter.call_type`` now that several routes share one operation."""
    ...
    assert attrs[DeRouter.CALL_TYPE] == call_type
...
def test_vendor_operation_values_are_namespaced():
    """A vendor value must stay under the ``derouter.`` prefix: an unprefixed invented
    ...
    vendor = [op for op in GenAIOperation if op.name.startswith("DEROUTER_")]
    assert vendor, "no vendor operation values defined"
    assert all(op.value.startswith("derouter.") for op in vendor)
...
def test_agent_message_is_an_invoke_agent_operation(call_type):
    ...
    ``derouter/a2a_protocol/main.py`` stamps ``asend_message_streaming`` on the
...
def test_every_call_type_the_a2a_package_stamps_is_an_agent_operation():
    ...
    a2a_package = Path(derouter.__file__).parent / "a2a_protocol"
    stamped = {
        call_type
        for source in a2a_package.rglob("*.py")
        for call_type in re.findall(r'call_type="([^"]+)"', source.read_text())
    }
    assert stamped, "no call_type literals found in derouter/a2a_protocol"
    unmapped = {
...
def test_mcp_method_values_match_wire_format():
    from derouter.integrations.otel import MCP, MCPMethod
...
def test_mcp_tool_call_adapter_extracts_fields():
    from derouter.integrations.otel import MCPToolCallSpanData
...
def test_is_mcp_tool_call_detection():
    from derouter.integrations.otel import is_mcp_tool_call
...
def test_mcp_tool_call_span_name():
    from derouter.integrations.otel import MCPToolCallSpanData
    from derouter.integrations.otel.model.spans import mcp_tool_call_span_name
...
def test_request_identity_prefers_canonical_team_keys():
    from derouter.integrations.otel.model.payloads import RequestIdentity
...
def test_llm_span_carries_proxy_request_route():
    ...
    assert attrs[DeRouter.REQUEST_ROUTE] == "/v1/responses/{response_id}"
...
def test_llm_span_falls_back_to_the_logged_route_without_a_server_span():
    ...
    assert GenAIMapper().map(data)[DeRouter.REQUEST_ROUTE] == "/v1/responses"
...
def test_llm_span_omits_request_route_off_the_proxy():
    ...
    assert DeRouter.REQUEST_ROUTE not in attrs


def test_guardrail_span_data_block_carries_verdict_and_error():
    from derouter.integrations.otel.model.payloads import GuardrailSpanData
...
def test_request_context_splits_group_from_dispatched_model():
    ...
    deployment: ``gen_ai.request.model`` is the group, ``derouter.provider.model``
    is the dispatched (provider-prefixed) deployment model."""
    from derouter.integrations.otel.model.metadata import RequestContext
...
def test_request_context_prefers_explicit_dispatched_model():
    """``hidden_params.derouter_model_name`` is the authoritative dispatched model
...
def test_v2_flag_resolved_once_not_per_call(monkeypatch):
    ...
    from derouter.integrations.otel.model import config as config_mod
...
def test_config_legacy_compat_env_toggle(monkeypatch):
    monkeypatch.setenv("DEROUTER_OTEL_LEGACY_COMPAT", "false")
...
def test_promoted_baggage_is_bounded_allowlist():
    ...
    assert promoted[DeRouter.TEAM_ID] == "t1"
    assert promoted[DeRouter.TEAM_ALIAS] == "team one"
    assert promoted[GenAI.REQUEST_MODEL] == "gpt-4o"
    # allowlisted metadata sub-key is promoted under the derouter.metadata.* prefix
    assert promoted[f"{DeRouter.METADATA_PREFIX}user_api_key_org_id"] == "org1"
