...
from derouter.integrations.otel.model.baggage import BAGGAGE_PROMOTED_KEYS  # noqa: E402
from derouter.integrations.otel.model.spans import SpanRole  # noqa: E402
...
def _engine_and_exporter(config=None):
    ...
    tracer = providers.get_tracer(provider, "derouter-baggage-test")
...
def test_identity_promoted_onto_every_span():
    ...
    for span in spans:
        ...
        assert span.attributes.get(DeRouter.TEAM_ALIAS) == "team one"
...
def test_team_metadata_promoted_only_for_allowlisted_subkeys():
    ...
    assert json.loads(span.attributes[DeRouter.TEAM_METADATA]) == {"tier": "gold"}
    # provider model is distinct from the user-facing request model
    assert span.attributes.get(DeRouter.PROVIDER_MODEL) == "azure/my-deployment"
    assert span.attributes.get(GenAI.REQUEST_MODEL) == "gpt-4o"
...
def test_team_metadata_not_promoted_by_default():
    ...
    bag = promoted_baggage(data.identity, data.request_model, BAGGAGE_PROMOTED_KEYS)
    assert DeRouter.TEAM_METADATA not in bag
...
def test_team_metadata_not_promoted_when_key_excluded_from_promoted_keys():
    ...
    ``derouter.team.metadata`` itself isn't in ``promoted_keys``."""
    data = LLMCallSpanData.from_standard_logging_payload(_payload())
    bag = promoted_baggage(
        data.identity,
        data.request_model,
...
def test_empty_team_metadata_is_dropped():
    ...
    bag = promoted_baggage(data.identity, data.request_model, BAGGAGE_PROMOTED_KEYS)
    assert DeRouter.TEAM_METADATA not in bag
    assert bag[DeRouter.PROVIDER_MODEL] == "gpt-4o"
...
def test_allowlisted_metadata_subkey_promoted_blob_excluded():
    ...
    assert (
        span.attributes.get(f"{DeRouter.METADATA_PREFIX}user_api_key_org_id") == "org1"
    )
...
def test_arbitrary_upstream_baggage_not_promoted():
    engine, exporter = _engine_and_exporter()
    ctx = ctx_mod.set_request_baggage(
        {DeRouter.TEAM_ID: "t1", "some.upstream.key": "leak"}
    )
    engine.emit(SpanRole.SERVICE, ServiceSpanData("redis", call_type="set"), ctx)
    (span,) = exporter.get_finished_spans()
    assert span.attributes.get(DeRouter.TEAM_ID) == "t1"
...
def test_baggage_processor_allowlist_can_be_widened():
    cfg = OpenTelemetryV2Config(
        exporter="in_memory",
        baggage_promoted_keys=[DeRouter.TEAM_ID, "custom.key"],
    )
    engine, exporter = _engine_and_exporter(cfg)
    ctx = ctx_mod.set_request_baggage({"custom.key": "v", DeRouter.TEAM_ALIAS: "ta"})
    ...
    assert DeRouter.TEAM_ALIAS not in span.attributes
