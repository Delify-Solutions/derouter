...
from derouter.integrations.otel.plumbing import context as ctx_mod  # noqa: E402
from derouter.integrations.otel.plumbing import providers  # noqa: E402
from derouter.integrations.otel.model.config import OpenTelemetryV2Config  # noqa: E402
from derouter.integrations.otel.mappers.genai import GenAIMapper  # noqa: E402
from derouter.integrations.otel.mappers.legacy import LegacyMapper  # noqa: E402
from derouter.integrations.otel.plumbing.metrics import (
    create_genai_metrics,
)  # noqa: E402
from derouter.integrations.otel.model.payloads import (  # noqa: E402
    GuardrailSpanData,
    LLMCallSpanData,
    LLMCost,
    LLMRequestParams,
...
from derouter.integrations.otel.model.semconv import GenAI, GenAIOperation
...
from derouter.integrations.otel.model.utils import (  # noqa: E402
    as_bool,
    as_float,
    as_int,
    as_str,
...
def test_validate_registry_detects_role_mismatch():
    bad = {SpanRole.LLM_CALL: SpanSpec(SpanRole.SERVICE, DeRouterSpanKind.CLIENT, None)}
    with pytest.raises(ValueError, match="mismatched role"):
        validate_registry(bad)


def test_validate_registry_detects_unknown_parent():
    bad = {SpanRole.LLM_CALL: SpanSpec(SpanRole.LLM_CALL, DeRouterSpanKind.CLIENT, parent=SpanRole.PROXY_REQUEST)}
...
def test_genai_mapper_cost_breakdown():
    from derouter.integrations.otel.model.semconv import DeRouter
    ...
    attrs = GenAIMapper().map(data)
    assert attrs[f"{DeRouter.COST_PREFIX}total"] == 0.012
    assert attrs[f"{DeRouter.COST_PREFIX}input"] == 0.004
    assert attrs[f"{DeRouter.COST_PREFIX}output"] == 0.006
    assert attrs[f"{DeRouter.COST_PREFIX}cache_read"] == 0.001
    assert attrs[f"{DeRouter.COST_PREFIX}cache_creation"] == 0.0
    assert attrs[f"{DeRouter.COST_PREFIX}tool_usage"] == 0.0005
    assert attrs[f"{DeRouter.COST_PREFIX}original"] == 0.013
    assert attrs[f"{DeRouter.COST_PREFIX}discount_amount"] == 0.001
    assert attrs[f"{DeRouter.COST_PREFIX}discount_percent"] == 0.077
    assert attrs[f"{DeRouter.COST_PREFIX}margin_total_amount"] == 0.0
    # Components the source did not report are omitted, not zero-filled.
    assert f"{DeRouter.COST_PREFIX}margin_fixed_amount" not in attrs
    assert f"{DeRouter.COST_PREFIX}margin_percent" not in attrs
...
def test_genai_mapper_cost_breakdown_absent():
    ...
    attrs = GenAIMapper().map(_full_llm_call())
    assert attrs[f"{DeRouter.COST_PREFIX}total"] == 0.002
    assert not any(k.startswith(DeRouter.COST_PREFIX) and k != f"{DeRouter.COST_PREFIX}total" for k in attrs)
...
def test_genai_mapper_guardrail_and_service():
    ...
    assert g[DeRouter.GUARDRAIL_NAME] == "presidio"
    assert g[DeRouter.GUARDRAIL_MODE] == "pre"

    # A datastore service (redis) also gets db.* semconv.
    s = GenAIMapper().map(ServiceSpanData("redis", call_type="set"))
    assert s[DeRouter.SERVICE_NAME] == "redis"
    assert s[DeRouter.SERVICE_CALL_TYPE] == "set"
    ...
    assert internal[DeRouter.SERVICE_NAME] == "router"
...
def test_genai_mapper_guardrail_billing_attrs():
    ...
    attrs = GenAIMapper().map(data)
    assert attrs[DeRouter.GUARDRAIL_COST] == 0.00456
    assert DeRouter.GUARDRAIL_COST == "derouter.cost.guardrail"
    assert attrs[DeRouter.GUARDRAIL_USAGE] == data.usage_json
    ...
    assert DeRouter.GUARDRAIL_COST not in unbilled
    assert DeRouter.GUARDRAIL_USAGE not in unbilled
...
def test_get_baggage_attributes_roundtrip():
    ctx = ctx_mod.set_request_baggage({"derouter.team.id": "t1"})
    assert ctx_mod.get_baggage_attributes(ctx)["derouter.team.id"] == "t1"
...
def test_to_otel_span_kind_covers_all():
    assert providers.to_otel_span_kind(DeRouterSpanKind.SERVER) is SpanKind.SERVER
    assert providers.to_otel_span_kind(DeRouterSpanKind.CLIENT) is SpanKind.CLIENT
    assert providers.to_otel_span_kind(DeRouterSpanKind.INTERNAL) is SpanKind.INTERNAL
    assert providers.to_otel_span_kind(DeRouterSpanKind.PRODUCER) is SpanKind.PRODUCER
    assert providers.to_otel_span_kind(DeRouterSpanKind.CONSUMER) is SpanKind.CONSUMER
...
def test_parse_headers_keeps_unencoded_values_working():
    ...
    Vendors that document a bare space, and derouter's own presets, must survive
...
def test_baggage_processor_lifecycle_noops():
    proc = providers.DeRouterBaggageSpanProcessor(allowed_keys=["derouter.team.id"])
...
def _emit_error_span(message, error_type="derouter.APIError"):
    from derouter.integrations.otel.emitter import SpanEmitter
...
def _exception_event(span):
    from derouter.integrations.otel.model.semconv import ExceptionEvent
...
def test_error_message_recorded_as_full_exception_event_untruncated():
    ...
    from derouter.integrations.otel.model.semconv import Error, ExceptionEvent

    long_message = "boom: " + "x" * 5000
    span = _emit_error_span(long_message, error_type="derouter.APIError")
    ...
    assert event.attributes[ExceptionEvent.TYPE] == "derouter.APIError"
    ...
    assert span.attributes[Error.TYPE] == "derouter.APIError"
...
def test_error_details_stamped_as_span_attributes_for_labels_ingest():
    """OTel-defined keys and derouter-specific detail keys both ride span
    ...
    from derouter.integrations.otel.model.semconv import Error, ExceptionEvent, DeRouterError
    from derouter.integrations.otel.emitter import SpanEmitter
    ...
    data = LLMCallSpanData(
        ...
        finish_reasons=(),
        error=SpanError(
            error_type="derouter.BadRequestError",
    ...
    assert span.attributes[Error.TYPE] == "derouter.BadRequestError"
    assert span.attributes[Error.MESSAGE] == "400: violated moderation policy"
    # DeRouter-specific detail keys, under the ``derouter.provider.error.*``
    # vendor namespace, not defined by OTel semconv.
    assert span.attributes[DeRouterError.CODE] == "400"
    assert span.attributes[DeRouterError.STACK_TRACE] == "File proxy_server.py line 8570 ..."
    assert span.attributes[DeRouterError.LLM_PROVIDER] == "openai"
...
def test_error_details_omitted_when_span_error_carries_only_message():
    ...
    from derouter.integrations.otel.model.semconv import Error, DeRouterError
    ...
    # DeRouter-specific detail keys aren't stamped when the SpanError doesn't
    # carry them.
    assert DeRouterError.CODE not in span.attributes
    assert DeRouterError.STACK_TRACE not in span.attributes
    assert DeRouterError.LLM_PROVIDER not in span.attributes


def test_error_attribute_keys_are_pinned():
    """``error.type`` and ``error.message`` come from the semconv ``error.*``
    registry; the derouter-specific detail keys are vendor keys under
    ``derouter.provider.error.*``. Pins the exact strings so the emitted
    vocabulary can't drift silently."""
    from derouter.integrations.otel.model.semconv import Error, DeRouterError

    assert Error.TYPE == "error.type"
    assert Error.MESSAGE == "error.message"
    assert DeRouterError.CODE == "derouter.provider.error.code"
    assert DeRouterError.STACK_TRACE == "derouter.provider.error.stack_trace"
    assert DeRouterError.LLM_PROVIDER == "derouter.provider.error.llm_provider"
...
def _engine_with_event_recorder():
    ...
    from derouter.integrations.otel.emitter import SpanEmitter
    from derouter.integrations.otel.plumbing.events import GenAIEventRecorder
...
def test_resolve_logger_provider_reuses_operator_sdk_global(monkeypatch):
    ...
    built by derouter, so they land wherever the operator's other logs land."""
    from opentelemetry import _logs
    from opentelemetry.sdk._logs.export import InMemoryLogExporter
...
def test_sanitize_event_metadata_drops_objects_dumps_and_secrets():
    from derouter.integrations.otel.model.payloads import sanitize_event_metadata
...
def test_genai_mapper_guardrail_cost_in_spend_attr():
    ...
    billed guardrail cost (already inside derouter.cost.total) from a report-only
    ...
    assert attrs[DeRouter.GUARDRAIL_COST_IN_SPEND] is False
    assert DeRouter.GUARDRAIL_COST_IN_SPEND == "derouter.guardrail.cost_in_spend"

    billed = dict(entry)
    del billed["guardrail_cost_in_spend"]
    assert DeRouter.GUARDRAIL_COST_IN_SPEND not in GenAIMapper().map(GuardrailSpanData.from_logging_entry(billed))
