...
from derouter.integrations.otel.model.config import (
    ExporterOwner,
    ExporterSpec,
...
from derouter.integrations.otel.plumbing import providers as otel_providers
from derouter.integrations.otel.plumbing.context import (
    destination_backends,
    request_destinations,
    set_request_destinations,
)
from derouter.integrations.otel.plumbing.providers import (
...
from derouter.integrations.otel.plumbing.routing import TenantTracerCache, get_tracer
from derouter.integrations.otel.presets.arize import arize_preset
from derouter.integrations.otel.presets.destinations import (
    destination_capable_backends,
    destination_for,
)
from derouter.integrations.otel.presets.langfuse import langfuse_preset
from derouter.proxy._types import AddTeamCallback, UserAPIKeyAuth
...
def allow_test_hosts(monkeypatch):
    ...
    monkeypatch.setattr(
        derouter, "provider_url_destination_allowed_hosts", ["team.local", "key.local", "x"], raising=False
    )
...
def emit(provider: TracerProvider, name: str = "chat gpt-4") -> None:
    with get_tracer(provider, "derouter").start_as_current_span(name):
        pass
...
class TestRoutingMode:
    ...
    def _additive(monkeypatch):
        monkeypatch.setattr(derouter, "otel_tenant_destination_mode", "additive", raising=False)

    @staticmethod
    def _tree(provider):
        tracer = get_tracer(provider, "derouter")
    ...
    def test_the_env_var_turns_additive_on_without_a_config_file(self, monkeypatch):
        monkeypatch.setattr(derouter, "otel_tenant_destination_mode", None, raising=False)
        monkeypatch.setenv("DEROUTER_OTEL_TENANT_DESTINATION_MODE", "Additive")
        global_exporter, dest_exporter = InMemorySpanExporter(), InMemorySpanExporter()
        provider = wired_provider(dest_exporter, global_exporter)
    ...
    def test_an_unrecognized_mode_stays_on_override(self, monkeypatch):
        monkeypatch.setattr(derouter, "otel_tenant_destination_mode", "both", raising=False)
        global_exporter, dest_exporter = InMemorySpanExporter(), InMemorySpanExporter()
        provider = wired_provider(dest_exporter, global_exporter)
    ...
    def test_the_operators_own_langfuse_and_a_team_naming_it_are_one_account(self, monkeypatch):
        ...
        monkeypatch.setattr(derouter, "provider_url_destination_allowed_hosts", ["lf.internal"], raising=False)
...
class TestFanOut:
    ...
    def test_the_operators_database_endpoint_does_not_ride_along_to_the_tenant(self):
                ...
                db_span.set_attributes(
                    {
                        ...
                        "server.port": 15400,
                        "db.namespace": "derouter",
                        "error.type": "PrismaError",
                        "error.message": unreachable,
                        "error": unreachable,
                        "derouter.provider.error.stack_trace": f"Traceback: {unreachable}",
                    }
                )
        ...
        assert operator_db.attributes["db.namespace"] == "derouter"
    ...
    def test_a_guardrails_failure_text_does_not_ride_along_to_the_tenant(self, failure_status):
        ...
        def run():
            ...
            with tracer.start_as_current_span("POST /v1/chat/completions"):
                with tracer.start_as_current_span("execute_guardrail pii") as down:
                    down.set_attributes(
                        {
                            "derouter.guardrail.name": "pii",
                            "derouter.guardrail.status": failure_status,
                            "derouter.guardrail.response": unreachable,
                        }
                    )
                with tracer.start_as_current_span("execute_guardrail toxicity") as up:
                    up.set_attributes(
                        {
                            "derouter.guardrail.name": "toxicity",
                            "derouter.guardrail.status": "guardrail_intervened",
                            "derouter.guardrail.response": verdict,
                        }
                    )
        ...
        assert tenant["execute_guardrail toxicity"].attributes["derouter.guardrail.response"] == verdict
        assert operator["execute_guardrail pii"].attributes["derouter.guardrail.response"] == unreachable
                ...
                server_span.set_attributes(
                    {
                        ...
                        "http.request.header.x_derouter_api_key": (bearer,),
    ...
    def test_the_proxys_own_error_text_does_not_ride_along_to_the_tenant(self):
        ...
        install = "/srv/derouter/.venv/lib/python3.13/site-packages/opentelemetry/trace/__init__.py"
            ...
            span.set_attributes(
                {
                    ...
                    "derouter.provider.error.code": "500",
                    "derouter.provider.error.stack_trace": f"Traceback\n  File {install}\n{message}",
                }
            )
            span.add_event(
                "exception",
        ...
        for name in ("POST /v1/chat/completions", "auth /v1/chat/completions"):
            ...
            assert dict(proxy_span.attributes) == {"error.type": "ProxyException", "derouter.provider.error.code": "500"}
        ...
        assert "derouter.provider.error.stack_trace" not in llm_span.attributes
        ...
        for name, message in (("auth /v1/chat/completions", unreachable), ("chat claude-haiku", provider_error)):
            ...
            assert install in operator[name].attributes["derouter.provider.error.stack_trace"]
    ...
    def test_a_tenants_service_name_is_layered_onto_the_operators_resource(self):
        ...
        dest = InMemorySpanExporter()
        provider = TracerProvider(
            resource=Resource({"service.name": "derouter-proxy", "deployment.environment.name": "prod"})
        )
        provider.add_span_processor(TenantFanOutSpanProcessor(processor_factory=lambda _d: SimpleSpanProcessor(dest)))
...
class TestProviderWiring:
    ...
    def test_concurrent_anchoring_attaches_exactly_one_fan_out(self):
        ...
        from derouter.integrations.otel.plumbing.providers import attach_tenant_fan_out
    ...
    def test_auth_seeds_the_request_with_destinations_the_registered_logger_can_deliver(
        ...
        from derouter.proxy import proxy_server
        from derouter.proxy.auth.user_api_key_auth import _seed_request_destinations
...
class TestRouting:
    def test_an_overridden_backend_is_not_detached_onto_a_second_provider(self):
        ...
        cache = TenantTracerCache(config, "langfuse_otel", "derouter")
        default = get_tracer(TracerProvider(), "derouter")
    ...
    def test_a_service_name_does_not_detach_a_backend_the_destination_does_not_name(self, callback_name):
        ...
        cache = TenantTracerCache(config, callback_name, "derouter")
        default = get_tracer(TracerProvider(), "derouter")
        auth_metadata = {"otel_service_name": "team-checkout"}
    ...
    def test_a_backend_pointed_at_its_own_account_still_routes_next_to_another_backend_destination(
        ...
        cache = TenantTracerCache(config, owner.value, "derouter")
        default = get_tracer(TracerProvider(), "derouter")
...
class TestDestinationResolution:
    ...
    def test_the_destination_header_string_survives_the_exporter_round_trip(self):
        from derouter.integrations.otel.plumbing.providers import parse_headers
...
class TestPresetDegradation:
    ...
    def test_a_credential_less_proxy_builds_the_gated_logger_beside_a_v2_carrier(self, monkeypatch):
        from derouter.derouter_core_utils.derouter_logging import _maybe_construct_otel_v2
...
class TestTenantConfigAgreement:
    ...
    def _v2_on(self, monkeypatch):
        monkeypatch.setenv("DEROUTER_OTEL_V2", "true")
        monkeypatch.setattr(
            derouter, "provider_url_destination_allowed_hosts", ["team.local", "key.local"], raising=False
        )
        is_otel_v2_enabled.cache_clear()
        yield
        is_otel_v2_enabled.cache_clear()
    ...
    def premium(self, monkeypatch):
        ...
        monkeypatch.setattr(derouter, "allow_dynamic_callback_disabling", True)

    @pytest.mark.usefixtures("premium")
    def test_a_backend_the_key_disabled_resolves_to_no_destination(self):
        """Dispatch skips a callback named in the key's ``derouter_disabled_callbacks``,
        so the fan-out must not deliver to it either."""
        auth = UserAPIKeyAuth(
            metadata={"derouter_disabled_callbacks": ["Langfuse_OTEL"]},
            team_metadata={"logging": [self._entry("http://team.local")]},
        )
    ...
    def test_the_disable_header_wins_over_the_key_list(self, header, resolved):
        ...
        destinations = resolve_tenant_otel_destinations(auth, {"x-derouter-disable-callbacks": header})
    ...
    def test_a_non_premium_proxy_ignores_the_disabled_list_like_dispatch_does(self, monkeypatch):
        ...
        assert resolve_tenant_otel_destinations(auth, {"x-derouter-disable-callbacks": "langfuse_otel"}) != ()
...
class TestEvictionSafety:
    ...
    def test_a_processor_still_exporting_a_span_is_not_closed_under_it(self):
        ...
        from derouter.integrations.otel.plumbing.providers import _MAX_CACHED_DESTINATION_PROCESSORS
    ...
    def test_drain_workers_are_daemons(self):
        ...
        self._fan_out()
        workers = [t for t in threading.enumerate() if t.name.startswith("derouter-otel-destination-drain")]
    ...
    def _drain_workers():
        ...
        return len([t for t in threading.enumerate() if t.name.startswith("derouter-otel-destination-drain")])
    ...
    def test_shutdown_retires_the_drain_workers(self):
        ...
        from derouter.integrations.otel.plumbing.providers import _DRAIN_WORKERS
    ...
    def test_a_submit_racing_close_is_never_stranded_behind_the_sentinels(self):
        ...
        from derouter.integrations.otel.plumbing.providers import _DrainPool
...
class TestCredentialGatedExporters:
    def test_layering_a_second_preset_does_not_eat_the_first_gated_exporter(self, monkeypatch):
        ...
        credential_less_proxy(monkeypatch)
        from derouter.integrations.otel.presets.utils import credential_gated_exporters
...
class TestTenantHostSsrfGuard:
    ...
    def _guard_on(self, monkeypatch):
        from derouter.integrations.otel.presets.destinations import _warn_host_not_allowlisted

        monkeypatch.setattr(derouter, "provider_url_destination_allowed_hosts", [], raising=False)
    ...
    def test_userinfo_naming_an_allowlisted_host_does_not_smuggle_a_second_one(self, monkeypatch):
        ...
        monkeypatch.setattr(derouter, "provider_url_destination_allowed_hosts", ["collector.example.com"], raising=False)
    ...
    def test_the_operator_can_allowlist_its_teams_internal_langfuse(self, monkeypatch):
        monkeypatch.setattr(derouter, "provider_url_destination_allowed_hosts", ["127.0.0.1:9111"], raising=False)
    ...
    def test_an_allowlisted_host_is_taken_without_resolving_it(self, monkeypatch):
        ...
        monkeypatch.setattr(derouter, "provider_url_destination_allowed_hosts", ["lf.invalid"], raising=False)
    ...
    def test_a_rejected_host_is_warned_about_once(self, caplog):
        with caplog.at_level("WARNING", logger="DeRouter"):
            for _ in range(3):
