...
from derouter.integrations.otel.emitter import SpanEmitter  # noqa: E402
from derouter.integrations.otel.emitter import stamp_error  # noqa: E402
from derouter.integrations.otel.mappers.utils import (  # noqa: E402
...
from derouter.integrations.otel.model.spans import SPAN_REGISTRY, SpanRole  # noqa: E402
...
def _engine(legacy_compat=True):
    ...
    tracer = providers.get_tracer(provider, "derouter-test")
...
def test_llm_call_span_cost_breakdown():
    ...
    assert a[f"{DeRouter.COST_PREFIX}total"] == 0.002
    # Per-component breakdown now rides the span.
    assert a[f"{DeRouter.COST_PREFIX}input"] == 0.004
    assert a[f"{DeRouter.COST_PREFIX}output"] == 0.006
    assert a[f"{DeRouter.COST_PREFIX}cache_read"] == 0.001
    # Unreported components are omitted, not zero-filled.
    assert f"{DeRouter.COST_PREFIX}margin_total_amount" not in a


def test_tracer_scope_carries_derouter_version():
    ...
    (span,) = exporter.get_finished_spans()
    assert span.instrumentation_scope.version == derouter_version
...
def test_llm_call_span_golden():
    ...
    assert a[DeRouter.CALL_ID] == "call_1"
    assert a["derouter.cost.total"] == 0.002
...
def test_stamp_error_writes_full_attribute_set_and_event():
    ...
    assert s.attributes["derouter.provider.error.code"] == "401"
    assert s.attributes["derouter.provider.error.stack_trace"] == "tb"
    assert s.attributes["derouter.provider.error.llm_provider"] == "anthropic"
...
def test_dedup_cache_is_bounded(monkeypatch):
    ...
    long-running proxy."""
    from derouter.integrations.otel import emitter as emitter_mod
    ...
    for i in range(10):
        engine.emit(
            ...
            LLMCallSpanData.from_standard_logging_payload(
                _payload(derouter_call_id=f"call_{i}")
            ),
        )
...
def test_service_error_span():
    from derouter.integrations.otel.model.payloads import SpanError
...
def test_guardrail_block_span_is_error_and_carries_verdict():
    ...
    a = span.attributes
    assert a[DeRouter.GUARDRAIL_STATUS] == "guardrail_intervened"
    assert a[DeRouter.GUARDRAIL_PROVIDER] == "openai"
    assert "violence" in a[DeRouter.GUARDRAIL_RESPONSE]  # the verdict rides the span
    assert a[DeRouter.GUARDRAIL_MASKED_ENTITY_COUNT] == 2
...
def test_many_tools_do_not_evict_core_attributes():
    ...
    assert a[DeRouter.TOOLS_DECLARED] == 127
...
def test_tool_definitions_kept_in_full_below_the_cap():
    ...
    assert a[DeRouter.TOOLS_DECLARED] == 3
    for idx in range(3):
...
def _tool_span(mapper_names, tool_count):
    ...
    provider, exporter = providers.in_memory_provider(cfg)
    engine = SpanEmitter(providers.get_tracer(provider, "derouter-test"), cfg)
    engine.emit(
        SpanRole.LLM_CALL,
