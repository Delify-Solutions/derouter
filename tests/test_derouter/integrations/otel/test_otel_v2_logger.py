...
from derouter.integrations.otel import (  # noqa: E402
    GenAI,
    DeRouter,
    OpenTelemetryV2Config,
)
from derouter.integrations.otel.plumbing import providers  # noqa: E402
from derouter.integrations.otel.plumbing.context import (  # noqa: E402
...
from derouter.integrations.otel.model.config import ExporterSpec  # noqa: E402
from derouter.integrations.otel.model.spans import (  # noqa: E402
    DEROUTER_PROXY_REQUEST_SPAN_NAME,
    SpanRole,
)
from derouter.integrations.otel.model.utils import to_ns, to_seconds  # noqa: E402
...
def _reset_request_root_span():
    ...
    """
    from derouter.integrations.otel.plumbing import context as _otel_context
...
def _kwargs(payload=None):
    return {
        # ``derouter_call_id`` (here carried inside the payload) correlates the
...
def test_async_log_success_event_emits_llm_call_span():
    ...
    assert span.attributes[DeRouter.CALL_ID] == "call_1"
...
def test_llm_call_span_reports_the_server_spans_route():
    """``derouter.request.route`` is the anchored server span's own ``http.route``,
    ...
    assert llm_span.attributes[DeRouter.REQUEST_ROUTE] == "/engines/{model:path}/chat/completions"
...
def test_llm_call_span_omits_the_route_without_a_server_span():
    ...
    _emit_llm(logger)
    (span,) = exporter.get_finished_spans()
    assert DeRouter.REQUEST_ROUTE not in span.attributes
...
def test_failed_llm_call_span_reports_the_server_spans_route():
    ...
    llm_span = next(s for s in exporter.get_finished_spans() if s.kind is SpanKind.CLIENT)
    assert llm_span.attributes[DeRouter.REQUEST_ROUTE] == "/v1/responses/{response_id}"
...
def test_deferred_llm_call_span_reports_the_server_spans_route():
    ...
    llm_span = next(s for s in exporter.get_finished_spans() if s.kind is SpanKind.CLIENT)
    assert llm_span.attributes[DeRouter.REQUEST_ROUTE] == "/v1/messages"
...
def test_enable_events_records_operation_exception_through_failure_callback():
    ...
    from derouter.integrations.otel.model.semconv import ExceptionEvent, GenAIEvent
...
def test_missing_standard_logging_object_is_noop():
    ...
    logger, exporter = _logger()
    asyncio.run(
        logger.async_log_success_event({"derouter_params": {}}, None, None, None)
    )
    assert exporter.get_finished_spans() == ()
...
def test_provider_auth_failure_span_carries_stack_trace():
    ...
    ``derouter.provider.error.stack_trace`` alongside code and llm_provider."""
    from derouter.exceptions import AuthenticationError
    ...
    assert span.attributes["derouter.provider.error.code"] == "401"
    assert span.attributes["derouter.provider.error.llm_provider"] == "anthropic"
    assert "test_otel_v2_logger" in span.attributes["derouter.provider.error.stack_trace"]
...
def test_evicted_carrier_completed_call_emits_one_deferred_span():
    ...
    assert spans[0].attributes[DeRouter.CALL_ID] == "call_1"
...
def _logger_capturing():
    from derouter.integrations.otel.model.config import CaptureMessageContent
...
def test_mcp_tool_call_emits_client_span():
    ...
    assert span.attributes[DeRouter.MCP_SERVER_NAME] == "weather-mcp"
    assert span.attributes[DeRouter.CALL_ID] == "mcp_1"
    assert span.status.status_code is StatusCode.UNSET
...
def test_mcp_tool_call_metadata_read_from_nested_metadata_not_top_level():
    ...
    assert DeRouter.MCP_SERVER_NAME not in span.attributes
...
def test_mcp_list_tools_emits_client_span():
    ...
    assert span.attributes[DeRouter.CALL_ID] == "mcp_list_1"
    assert span.status.status_code is StatusCode.UNSET
...
def test_mcp_span_ignores_client_supplied_baggage(make_payload, span_name):
    ...
    let a client spoof a span's identity (e.g. ``derouter.team.id``). The propagator
    ...
    assert span.attributes[DeRouter.TEAM_ID] == "t1"
    assert "derouter.metadata.user_api_key_user_id" not in span.attributes
...
def test_llm_span_parents_to_ambient_server_span():
    ...
    context, so it nests under it natively (no ``derouter_parent_otel_span``)."""
    logger, exporter = _logger()
    server = logger._emitter.start_span(
        SpanRole.PROXY_REQUEST, DEROUTER_PROXY_REQUEST_SPAN_NAME
    )
...
def test_synthetic_error_log_produces_no_llm_span():
    ...
    ``DEROUTER_LOGGING_NO_UPSTREAM_LLM_CALL``, it must open no carrier and emit no
    LLM-call span — even though the failure callback also fires."""
    from derouter.constants import DEROUTER_LOGGING_NO_UPSTREAM_LLM_CALL
    ...
    kwargs[DEROUTER_LOGGING_NO_UPSTREAM_LLM_CALL] = True
    with trace.use_span(server, end_on_exit=False):
        with logger.start_phase_span("auth /chat/completions"):
            logger.log_pre_api_call(model="gpt-4o", messages=[], kwargs=kwargs)
...
def test_create_request_started_span_captures_anchor():
    """``create_derouter_proxy_request_started_span`` doubles as the anchor capture
    point: the active server span becomes the request root for later spans."""
    from derouter.integrations.otel.plumbing.context import request_root_span
    ...
    with trace.use_span(server, end_on_exit=False):
        returned = logger.create_derouter_proxy_request_started_span(
...
def test_async_post_call_failure_hook_stamps_error_on_root_span():
    ...
    logger, exporter = _logger()
    server = logger._emitter.start_span(SpanRole.PROXY_REQUEST, DEROUTER_PROXY_REQUEST_SPAN_NAME)
    set_request_root_span(server)
    exc = _proxy_exc("derouter.BadRequestError: messages is required", 400)
    result = asyncio.run(
    ...
    assert span.attributes["derouter.provider.error.code"] == "400"
    assert span.status.status_code is StatusCode.ERROR
...
def test_async_post_call_failure_hook_stamps_the_mcp_messages_own_transport():
    ...
    logger, exporter = _logger()
    session_opener = logger._emitter.start_span(SpanRole.PROXY_REQUEST, DEROUTER_PROXY_REQUEST_SPAN_NAME)
    this_message = logger._emitter.start_span(SpanRole.PROXY_REQUEST, DEROUTER_PROXY_REQUEST_SPAN_NAME)
...
def test_async_post_call_failure_hook_skips_a_transport_that_already_answered():
    ...
    logger, exporter = _logger()
    anchor = logger._emitter.start_span(SpanRole.PROXY_REQUEST, DEROUTER_PROXY_REQUEST_SPAN_NAME)
    answered = logger._emitter.start_span(SpanRole.PROXY_REQUEST, DEROUTER_PROXY_REQUEST_SPAN_NAME)
    answered.end()
...
def test_record_error_attributes_on_span_decorates_without_ending():
    ...
    assert span.attributes["derouter.provider.error.code"] == "422"
    assert span.status.status_code is StatusCode.ERROR
...
def test_start_phase_span_stamps_error_attributes_on_failure():
    ...
    assert auth.attributes["derouter.provider.error.code"] == "401"
...
def test_real_logging_pre_call_opens_span_end_to_end():
    """Regression guard: a real ``DeRouterLoggingObj.pre_call`` must fire
    ``log_pre_api_call`` on the V2 logger (via ``derouter.input_callback``), so the
    ...
    monkeypatch.setattr(derouter, "input_callback", [logger], raising=False)
    try:
        logging_obj = Logging(
            model="gpt-4o",
            ...
            derouter_call_id="call_e2e",
            function_id="fn",
        )
        # The wrapper always runs this before pre_call — it's what seeds
        # ``derouter_params`` and ``derouter_call_id`` into ``model_call_details``
...
def test_baggage_identity_promoted_onto_llm_call():
    ...
    (span,) = exporter.get_finished_spans()
    assert span.attributes[DeRouter.TEAM_ID] == "t1"
    assert span.attributes[DeRouter.TEAM_ALIAS] == "team one"
    assert span.attributes[GenAI.REQUEST_MODEL] == "gpt-4o"
...
def test_provider_model_and_team_metadata_on_real_boundary_flow():
    ...
    - ``derouter.team.metadata`` (filtered to the allowlisted sub-keys) is known
      at auth, so it rides identity Baggage seeded there onto EVERY span
      (server + LLM call).
    - ``derouter.provider.model`` is only known once routing picks a deployment
    ...
    srv = spans[DEROUTER_PROXY_REQUEST_SPAN_NAME]
    # provider model: on the LLM call span, NOT the server span
    assert llm.attributes[DeRouter.PROVIDER_MODEL] == "azure/my-deployment"
    assert DeRouter.PROVIDER_MODEL not in srv.attributes
    # team metadata: on every span, JSON-serialized
    expected = {"tier": "gold", "cost_center": "42"}
    assert json.loads(llm.attributes[DeRouter.TEAM_METADATA]) == expected
    assert json.loads(srv.attributes[DeRouter.TEAM_METADATA]) == expected
...
def test_pre_call_hook_seeds_baggage_onto_server_and_child_spans():
    ...
    assert redis.attributes[DeRouter.TEAM_ID] == "t1"
    assert redis.attributes[DeRouter.KEY_HASH] == "hash1"
    assert redis.attributes[f"{DeRouter.METADATA_PREFIX}user_api_key_user_id"] == "u1"
    srv = spans[DEROUTER_PROXY_REQUEST_SPAN_NAME]
    assert (
        srv.attributes[DeRouter.TEAM_ID] == "t1"
    )  # stamped directly on the server span
    assert srv.attributes[f"{DeRouter.METADATA_PREFIX}user_api_key_user_id"] == "u1"
...
def test_async_service_success_hook_emits_service_span():
    ...
    assert span.attributes[DeRouter.SERVICE_NAME] == "redis"
    assert span.attributes[DeRouter.SERVICE_CALL_TYPE] == "set"
    # Canonical (V2) namespaced metadata key
    assert span.attributes[f"{DeRouter.METADATA_PREFIX}key1"] == "val1"
...
def test_postgres_db_span_names_the_database_server_not_the_prisma_engine():
    ...
    assert span.attributes["server.address"] == "derouter-prod.abc123.us-east-1.rds.amazonaws.com"
    assert span.attributes["server.port"] == 6432
    assert span.attributes["db.namespace"] == "derouter|reporting"
...
def test_async_service_failure_hook_marks_error_status():
    ...
    assert span.attributes[DeRouter.SERVICE_NAME] == "postgres"
...
def test_internal_service_call_is_internal_kind_without_db_attrs():
    ...
    assert span.attributes[DeRouter.SERVICE_NAME] == "reset_budget_job"
...
def test_service_span_inherits_parent_when_provided():
    ...
    assert (
        ...
        == by_name[DEROUTER_PROXY_REQUEST_SPAN_NAME].get_span_context().span_id
    )
...
def test_create_proxy_request_started_span_returns_ambient_span():
    ...
    assert (
        logger.create_derouter_proxy_request_started_span(
    ...
    with trace.use_span(server, end_on_exit=False):
        got = logger.create_derouter_proxy_request_started_span(
...
def test_proxy_global_first_registered_wins(monkeypatch):
    """``_init_otel_logger_on_derouter_proxy`` claims the global only when empty."""
    proxy_server = pytest.importorskip("derouter.proxy.proxy_server")
...
def test_select_global_otel_v2_logger_reuses_existing_preset_logger():
    ...
    """
    from derouter.integrations.otel.logger import select_global_otel_v2_logger
...
def test_publish_global_otel_v2_provider_sets_selected_logger_provider(monkeypatch):
    ...
    not some other.
    """
    from derouter.integrations.otel import logger as otel_logger
    from derouter.integrations.otel.logger import publish_global_otel_v2_provider
...
def test_registers_into_derouter_service_callback(monkeypatch):
    """The logger must mutate ``derouter.service_callback`` in place. An empty
    ...
    pytest.importorskip("derouter.proxy.proxy_server")
    monkeypatch.setattr(derouter, "service_callback", [], raising=False)
    cfg = OpenTelemetryV2Config(exporter="in_memory")
    tp = providers.build_tracer_provider(cfg)

    first = OpenTelemetryV2(config=cfg, tracer_provider=tp)
    assert first in derouter.service_callback

    # A second OTel logger sees one is already registered and does not duplicate.
    OpenTelemetryV2(config=cfg, tracer_provider=tp)
    otel_registrations = [
        cb
        for cb in derouter.service_callback
        if cb.__class__.__module__.startswith("derouter.integrations.otel")
    ]
    assert len(otel_registrations) == 1


def test_registers_into_derouter_input_callback(monkeypatch):
    """The logger must land in ``derouter.input_callback`` — the list
    ...
    pytest.importorskip("derouter.proxy.proxy_server")
    monkeypatch.setattr(derouter, "input_callback", [], raising=False)
    cfg = OpenTelemetryV2Config(exporter="in_memory")
    tp = providers.build_tracer_provider(cfg)

    first = OpenTelemetryV2(config=cfg, tracer_provider=tp)
    assert first in derouter.input_callback

    OpenTelemetryV2(config=cfg, tracer_provider=tp)
    otel_registrations = [
        cb
        for cb in derouter.input_callback
        if cb.__class__.__module__.startswith("derouter.integrations.otel")
    ]
    assert len(otel_registrations) == 1
...
def test_registers_into_async_success_and_failure_callbacks(monkeypatch):
    """The logger must self-register into ``derouter._async_success_callback`` and
    ``derouter._async_failure_callback`` — the lists ``Logging.async_success_handler``
    ...
    proxy's ``derouter.callbacks`` fan-out to populate them is not enough: a logger
    that reached derouter via ``service_callback`` / ``success_callback`` (or was
    created after the fan-out ran) is absent from ``derouter.callbacks``, so on a
    ...
    pytest.importorskip("derouter.proxy.proxy_server")
    monkeypatch.setattr(derouter, "_async_success_callback", [], raising=False)
    monkeypatch.setattr(derouter, "_async_failure_callback", [], raising=False)
    cfg = OpenTelemetryV2Config(exporter="in_memory")
    tp = providers.build_tracer_provider(cfg)

    first = OpenTelemetryV2(config=cfg, tracer_provider=tp)
    assert first in derouter._async_success_callback
    assert first in derouter._async_failure_callback
...
def test_boundary_span_closes_without_proxy_fanout(monkeypatch):
    ...
    registered ONLY via its own ``__init__`` (no ``derouter.callbacks`` fan-out, as
    ...
    monkeypatch.setattr(derouter, "_async_success_callback", [], raising=False)
    monkeypatch.setattr(derouter, "_async_failure_callback", [], raising=False)
    # Crucially: the logger is NOT in derouter.callbacks, so the proxy fan-out would
    ...
    logging_obj.model_call_details["derouter_call_id"] = "pt_leak"
...
def test_module_level_emit_guardrail_span_routes_to_registered_logger(monkeypatch):
    ...
    import derouter.integrations.otel.logger as otel_logger
...
def test_invalid_metric_filter_logged_once_records_nothing(caplog, monkeypatch):
    ...
    with caplog.at_level(logging.ERROR, logger="DeRouter"):
...
def test_valid_metric_filter_records_six_metrics(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callback_settings", {}, raising=False)
...
def test_key_team_auth_metadata_routes_llm_span_to_phoenix_project():
    ...
    kwargs = {
        ...
        "derouter_params": {"metadata": {"user_api_key_auth_metadata": auth_md}},
    }
    _emit_llm(logger, kwargs)
...
def test_client_request_metadata_cannot_route_phoenix_project():
    ...
    kwargs = {
        "standard_logging_object": payload,
        "derouter_params": {"metadata": {"phoenix_project_name": "attacker-project"}},
    }
    _emit_llm(logger, kwargs)
...
def test_project_routing_resolves_at_pre_call_before_payload_exists():
    ...
    so the destination project must resolve from ``derouter_params`` alone — the
    ...
    with trace.use_span(server, end_on_exit=False):
        logger.log_pre_api_call(
            model="gpt-4o",
            messages=[],
            kwargs={"derouter_call_id": "call_1", "derouter_params": derouter_params},
        )
    server.end()
...
def test_evicted_provider_still_exports_span_opened_before_eviction(monkeypatch):
    ...
    from derouter.integrations.otel.plumbing import routing as routing_mod
    ...
    with trace.use_span(server, end_on_exit=False):
        logger.log_pre_api_call(
            model="gpt-4o",
            messages=[],
            kwargs={"derouter_call_id": "call_a", "derouter_params": {"metadata": md_a}},
        )
    ...
    _emit_llm(
        logger,
        {
            "standard_logging_object": _payload(derouter_call_id="call_b", metadata=md_b),
            "derouter_params": {"metadata": md_b},
        },
    )
    ...
    asyncio.run(
        logger.async_log_success_event(
            {
                "standard_logging_object": _payload(derouter_call_id="call_a", metadata=md_a),
                "derouter_params": {"metadata": md_a},
            },
            None,
            None,
            None,
...
def test_boundary_open_with_no_payload_ends_provisional_span():
    ...
    asyncio.run(
        logger.async_log_success_event(
            {**kwargs, "standard_logging_object": None, "derouter_call_id": "call_1"}, None, None, None
        )
    )
    server.end()
...
def test_provisional_close_then_payload_close_does_not_duplicate():
    ...
    asyncio.run(
        logger.async_log_success_event(
            {**kwargs, "standard_logging_object": None, "derouter_call_id": payload["derouter_call_id"]},
            None,
            None,
            None,
        )
