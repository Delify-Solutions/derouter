...
from derouter.integrations.otel.presets import (
...
def _cache(callback_name, exporters=None):
    ...
    return TenantTracerCache(cfg, callback_name, "derouter")
...
def test_tenant_service_name_precedence_and_blanks():
    from derouter.integrations.otel.plumbing.routing import tenant_service_name
...
def test_newrelic_dynamic_endpoint_resolves_from_fixed_table():
    from derouter.integrations.otel.presets import dynamic_otlp_endpoint
...
def test_credential_routed_span_roots_new_trace_and_links_back():
    ...
    from derouter.integrations.otel.logger import _request_trace_links
    ...
    default = default_provider.get_tracer("derouter")
...
def test_requires_headers_spec_skipped_without_headers():
    from derouter.integrations.otel.plumbing.providers import build_tracer_provider
    ...
    assert [type(p).__name__ for p in processors] == ["DeRouterBaggageSpanProcessor"]
