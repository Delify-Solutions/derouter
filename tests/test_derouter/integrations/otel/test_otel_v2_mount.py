...
(gated by DEROUTER_OTEL_V2). The mount logic lives in
``derouter.integrations.otel.mount``; this exercises both that module's public
...
from derouter.integrations.otel.mount import (  # noqa: E402
...
def test_gate_toggles_with_env(monkeypatch):
    ...
    monkeypatch.setenv("DEROUTER_OTEL_V2", "1")
    is_otel_v2_enabled.cache_clear()
...
def test_llm_span_route_is_read_off_the_server_span(monkeypatch):
    ...
    ``create_derouter_proxy_request_started_span`` call the proxy makes per request,
    ...
    def _anchor_then_read(key):
        logger.create_derouter_proxy_request_started_span(start_time=datetime.now(timezone.utc), headers=None)
