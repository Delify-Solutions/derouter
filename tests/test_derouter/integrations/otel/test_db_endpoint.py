"""Tests for derouter/integrations/otel/model/db_endpoint.py
...
from derouter.integrations.otel.model.db_endpoint import (
    DatabaseEndpoint,
    db_span_attributes,
...
LOCAL_DSN = "postgresql://llmproxy:dbpassword9090@localhost:5432/derouter"
REMOTE_DSN = "postgresql://llmproxy:s3cr3t@derouter-prod.abc123.us-east-1.rds.amazonaws.com:6432/derouter?schema=reporting&sslmode=require"
REPLICA_DSN = "postgresql://reader:r3ad0nly@derouter-prod-ro.abc123.us-east-1.rds.amazonaws.com/derouter_replica"
...
def test_local_prisma_engine_endpoint_is_the_postgres_server_not_the_engine():
    assert parse_database_endpoint(LOCAL_DSN) == DatabaseEndpoint(
        address="localhost", port=5432, namespace="derouter"
    )
...
def test_remote_endpoint_keeps_host_port_and_schema_qualified_namespace():
    assert parse_database_endpoint(REMOTE_DSN) == DatabaseEndpoint(
        address="derouter-prod.abc123.us-east-1.rds.amazonaws.com",
        port=6432,
        namespace="derouter|reporting",
    )
...
def test_read_replica_dsn_parses_to_the_replica_host_and_database():
    assert parse_database_endpoint(REPLICA_DSN) == DatabaseEndpoint(
        address="derouter-prod-ro.abc123.us-east-1.rds.amazonaws.com",
        port=5432,
        namespace="derouter_replica",
    )
...
def test_default_schema_is_not_spelled_out_in_the_namespace():
    ...
    assert parse_database_endpoint("postgresql://u:p@db.internal/derouter?schema=public") == parse_database_endpoint(
        "postgresql://u:p@db.internal/derouter"
    )
...
def test_unix_socket_host_parameter_wins_over_the_netloc():
    ...
    assert parse_database_endpoint(
        "postgresql://u:p@localhost:5432/derouter?host=/cloudsql/proj:us-east1:inst"
    ) == DatabaseEndpoint(address="/cloudsql/proj:us-east1:inst", port=5432, namespace="derouter")


def test_socket_only_dsn_without_a_netloc_host_still_resolves():
    assert parse_database_endpoint("postgresql:///derouter?host=/var/run/postgresql") == DatabaseEndpoint(
        address="/var/run/postgresql", port=5432, namespace="derouter"
    )


def test_percent_encoded_database_name_is_decoded():
    endpoint = parse_database_endpoint("postgresql://u:p@db.internal/derouter%20prod")
    assert endpoint is not None and endpoint.namespace == "derouter prod"


MISPARSED_AUTHORITY_DSNS = (
    ("postgresql://derouter:/kJ8xQz+9wT@db.internal:5432/derouter", "kJ8xQz+9wT"),
    ("postgresql://derouter:12345/aBcD@db.internal:5432/derouter", "aBcD"),
    # '#' sends the tail to the fragment and '?' to the query, so the path is
    # empty and only the stranded userinfo '@' reveals the mis-split.
    ("postgresql://derouter:12345#aBcD@db.internal/derouter", "aBcD"),
    ("postgresql://derouter:12345?aBcD@db.internal/derouter", "aBcD"),
    # A '?'-stranded tail that happens to parse as parameters, including one
    # that hijacks the host= parameter into server.address.
    ("postgresql://derouter:12345?a=aBcD@db.internal/derouter", "aBcD"),
    ("postgresql://derouter:12345?host=aBcD@db.internal/derouter", "aBcD"),
    ...
    ("postgresql://derouter:12345/aBcD?x=1@db.internal/derouter", "aBcD"),
)
...
def test_extra_path_segment_yields_no_endpoint():
    ...
    assert parse_database_endpoint("postgresql://db.internal:5432/derouter/extra") is None
...
def test_a_mis_split_authority_never_exports_the_database_username(dsn):
    ...
    attrs = _resolve("postgres", "get_data", database_url=dsn)
    assert "server.address" not in attrs
    assert "derouter" not in " ".join(str(v) for v in attrs.values())


@pytest.mark.parametrize(
    "dsn",
    [
        "postgresql://db.internal:5432/derouter?application_name=svc@prod",
        "postgresql://db.internal:5432/derouter?user=admin@company.com",
    ],
)
...
def test_host_and_port_query_parameters_are_honoured_together():
    assert parse_database_endpoint("postgresql://ignored/derouter?host=real.internal&port=6543") == DatabaseEndpoint(
        address="real.internal", port=6543, namespace="derouter"
    )
...
def test_percent_encoded_password_still_resolves_the_endpoint():
    ...
    assert parse_database_endpoint("postgresql://derouter:pa%2Fssw0rd@db.internal:5432/derouter") == DatabaseEndpoint(
        address="db.internal", port=5432, namespace="derouter"
    )


def test_hostless_socket_dsn_still_names_the_database():
    """``postgresql:///derouter`` is a valid local-socket DSN that Prisma accepts,
    so the database is knowable even though no server address is."""
    assert parse_database_endpoint("postgresql:///derouter") == DatabaseEndpoint(
        address=None, port=None, namespace="derouter"
    )


def test_hostless_socket_dsn_emits_namespace_without_a_server():
    attrs = _resolve("postgres", "get_data", database_url="postgresql:///derouter")
    assert attrs["db.namespace"] == "derouter"
    assert "server.address" not in attrs
...
def test_prisma_default_schema_is_left_implicit():
    endpoint = parse_database_endpoint("postgresql://u:p@db.internal/derouter?schema=public")
    assert endpoint is not None and endpoint.namespace == "derouter"
...
def test_a_non_default_schema_stays_in_the_namespace(spelling):
    ...
    endpoint = parse_database_endpoint(f"postgresql://u:p@db.internal/derouter?schema={spelling}")
    assert endpoint is not None and endpoint.namespace == f"derouter|{spelling}"


def test_postgres_scheme_alias_is_accepted():
    assert parse_database_endpoint("postgres://u:p@db.internal/derouter") == DatabaseEndpoint(
        address="db.internal", port=5432, namespace="derouter"
    )
...
@pytest.mark.parametrize(
    ...
    [
        None,
        "",
        "mysql://u:p@db.internal:3306/derouter",
        "postgresql://u:p@db.internal:not-a-port/derouter",
        "not a url at all",
    ],
)
...
def test_postgres_service_span_carries_system_operation_and_endpoint():
    assert _resolve("postgres", "get_data", database_url=REMOTE_DSN) == {
        ...
        "server.address": "derouter-prod.abc123.us-east-1.rds.amazonaws.com",
        "server.port": 6432,
        "db.namespace": "derouter|reporting",
    }
...
def test_batch_write_service_is_also_attributed_to_postgres():
    ...
    assert attrs["server.address"] == "derouter-prod.abc123.us-east-1.rds.amazonaws.com"
...
def test_a_runtime_endpoint_change_is_reflected_on_the_next_span():
    ...
    assert moved["server.address"] == "derouter-prod.abc123.us-east-1.rds.amazonaws.com"
