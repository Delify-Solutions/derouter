...
Before the fix the parent span is never wired in, so the derouter_request span
...
foreign trace; the USE_OTEL_DEROUTER_REQUEST_SPAN variants additionally assert
the derouter_request child is parented to the SERVER root.
"""
...
def otel_success_callback(otel_with_exporter, monkeypatch):
    ...
    for success callbacks (derouter._async_success_callback), so the real
    logging path drives it."""
    otel, exporter = otel_with_exporter
    monkeypatch.setattr(derouter, "callbacks", [otel])
    monkeypatch.setattr(derouter, "_async_success_callback", [otel])
...
def _build_logging_obj_wired_to_root(
...
) -> Tuple[DeRouterLoggingObj, dict, datetime]:
    ...
    logging_obj = DeRouterLoggingObj(
        ...
        derouter_call_id="lit-3443-call",
        function_id="1245",
    )
    ...
    logging_obj.model_call_details["derouter_call_id"] = "lit-3443-call"
...
def _assert_root_closed_and_no_orphan(exporter, root_span, where):
    ...
    server_spans = [s for s in finished if s.name == DEROUTER_PROXY_REQUEST_SPAN_NAME]
    assert server_spans, (
...
def _assert_child_parented_to_root(exporter, root_span, where):
    ...
    children = [
        s
        for s in finished
        if s.name != DEROUTER_PROXY_REQUEST_SPAN_NAME
        and s.parent is not None
        and s.parent.span_id == root_ctx.span_id
    ]
    assert children, (
        f"{where}: no derouter_request child parented to the SERVER root — "
...
def test_non_streaming_passthrough_links_to_server_root(
    ...
    if use_request_span:
        monkeypatch.setenv("USE_OTEL_DEROUTER_REQUEST_SPAN", "true")
...
def test_client_body_metadata_cannot_clobber_parent_span(
    ...
    derouter_parent_otel_span key must not override the real parent span. The
    ...
    the derouter_request span would orphan."""
    monkeypatch.setenv("USE_OTEL_DEROUTER_REQUEST_SPAN", "true")
    _otel, exporter = otel_success_callback
    root = server_span_factory("/anthropic/v1/messages")
    ...
    logging_obj, kwargs, start_time = _build_logging_obj_wired_to_root(
        root,
        stream=False,
        extra_body={"metadata": {"derouter_parent_otel_span": "not-a-real-span"}},
    )
    end_time = datetime.now()
    asyncio.run(
        logging_obj.async_success_handler(
...
def test_init_kwargs_internal_keys_resist_client_metadata(server_span_factory):
    ...
    derouter_parent_otel_span keys must not override the authenticated values.
    ...
    body = {
        ...
        "metadata": {
            ...
            "derouter_parent_otel_span": "not-a-real-span",
        },
    }
    logging_obj = DeRouterLoggingObj(
        model="unknown",
        ...
        call_type="pass_through_endpoint",
        start_time=datetime.now(),
        derouter_call_id="lit-3443-clobber",
        function_id="1245",
    )
    payload = PassthroughStandardLoggingPayload(
        url=URL_ROUTE, request_body=body, request_method="POST"
    ...
    md = kwargs["derouter_params"]["metadata"]
    ...
    assert md["derouter_parent_otel_span"] is real_span
