...
import derouter
from derouter.integrations.langfuse import langfuse as langfuse_module
...
class TestLangfuseUsageDetails(unittest.TestCase):
    def setUp(self):
        ...
        self._original_langfuse_clients_count = derouter.initialized_langfuse_clients
        ...
        def log_event_on_langfuse(
            ...
            return self._log_langfuse_v2(
                user_id=user_id,
                metadata=kwargs.get("derouter_params", {}).get("metadata", {}),
    ...
    def tearDown(self):
        ...
        derouter.initialized_langfuse_clients = self._original_langfuse_clients_count
    ...
    def test_log_langfuse_v2_handles_null_usage_values(self):
        ...
        with (
            patch(
                "derouter.integrations.langfuse.langfuse._add_prompt_to_generation_params",
            ...
            kwargs = {
                ...
                "derouter_params": {"metadata": {}},
                "optional_params": {},
                "derouter_call_id": "test-call-id-null-usage",
    ...
    def test_log_langfuse_v2_uses_derouter_trace_id_fallback_over_call_id(self):
        """
        When standard_logging_object has no trace_id, but kwargs contains
        derouter_trace_id (the same ID the DB stores as Session ID), Langfuse
        should use derouter_trace_id — NOT derouter_call_id. This ensures the
        trace_id in Langfuse matches the Session ID shown in DeRouter logs.
        """
        payload = self._build_standard_logging_payload()  # no trace_id
        kwargs = self._build_langfuse_kwargs(payload)
        kwargs["derouter_trace_id"] = "trace-id-from-kwargs"
        self.last_trace_kwargs = {}
        ...
        # derouter_trace_id should be preferred over derouter_call_id
    ...
    def _drive_with_canary(self, extra_metadata=None, hidden_params=None):
        ...
        with patch(
            ...
            self.logger._log_langfuse_v2(
                ...
                response_obj=None,
                level="INFO",
                derouter_call_id="canary-call-id",
            )
    ...
    def test_debug_langfuse_dump_carries_no_credentials(self):
        ...
        dumped = self.last_trace_kwargs["metadata"]["metadata_passed_to_derouter"]
        assert "user_api_key_auth" not in dumped
        assert self.CANARY not in self._emitted_payload_text()

    def test_raw_request_metadata_reaches_the_emitted_blob_through_no_key(self):
        """
        The emitted blob is the allowlist plus derouter enrichments, nothing else.
    ...
    def test_caller_nested_metadata_cannot_erase_a_derouter_enrichment(self):
        """
        ...
        dict for real, so a caller naming a key derouter_response_cost would otherwise
        blank out the cost derouter computed. Enrichments are layered after the dedupe.
        """
        payload = self._build_standard_logging_payload(trace_id="canary-trace-id")
        payload["metadata"]["requester_metadata"] = {"derouter_response_cost": "caller-value", "api_base": "caller"}
        kwargs = {**self._build_langfuse_kwargs(payload), "response_cost": 0.25}
        ...
        with patch(
            ...
            self.logger._log_langfuse_v2(
                user_id="user-1",
                metadata=metadata,
                derouter_params={"metadata": metadata, "api_base": "https://real-api-base"},
                output=None,
                start_time=datetime.datetime(2024, 1, 1, 12, 0, 0),
                end_time=datetime.datetime(2024, 1, 1, 12, 0, 1),
                kwargs=kwargs,
        ...
        assert generation_metadata["derouter_response_cost"] == 0.25
    ...
    def test_denied_steering_keys_and_enrichments(self):
        ...
        derouter-computed and must survive the move off clean_metadata.
        """
        generation_metadata = self._drive_with_canary()
    ...
    def test_redact_user_api_key_info_still_strips_the_emitted_blob(self):
        ...
        """
        with patch.object(derouter, "redact_user_api_key_info", True):
            generation_metadata = self._drive_with_canary()
    ...
    def test_failure_trace_survives_a_missing_standard_logging_object(self):
        ...
        show. The trace is still emitted with the derouter_trace_id fallback, and the
        ...
        kwargs = {
            ...
            "cache_hit": False,
            "messages": [],
            "derouter_trace_id": "trace-id-failure",
        }
        self.last_trace_kwargs = {}
        self.mock_langfuse_trace.generation.reset_mock()
        ...
        with patch(
            ...
            trace_id, _ = self.logger._log_langfuse_v2(
                ...
                response_obj=None,
                level="ERROR",
                derouter_call_id="call-id-different",
            )
    ...
    def test_log_langfuse_v2_session_id_passed_as_trace_session_id(self):
        ...
        with patch(
            ...
            self.logger._log_langfuse_v2(
                ...
                derouter_params={"metadata": {"session_id": "my-session-abc"}},
                output=None,
                start_time=datetime.datetime.utcnow(),
                end_time=datetime.datetime.utcnow(),
                kwargs=kwargs,
    ...
    def test_log_langfuse_v2_session_id_preserved_for_error_level(self):
        ...
        with patch(
            ...
            self.logger._log_langfuse_v2(
                ...
                derouter_params={"metadata": {"session_id": "error-session-xyz"}},
                ...
                response_obj=None,
                level="ERROR",
                derouter_call_id="call-id-err-789",
            )
...
def test_failure_handler_langfuse_kwargs_excludes_original_response():
    ...
    logging_obj = Logging(
        ...
        call_type="completion",
        start_time=datetime.datetime.utcnow(),
        derouter_call_id="test-call-id-failure",
    ...
    original_failure_callback = derouter.failure_callback
    derouter.failure_callback = ["langfuse"]

    try:
        # Mock LangFuseHandler to return our capturing mock logger
        with patch(
            "derouter.derouter_core_utils.derouter_logging.LangFuseHandler"
        ...
        langfuse_metadata = langfuse_kwargs.get("derouter_params", {}).get(
            "metadata", {}
        )
        assert (
        ...
        derouter.failure_callback = original_failure_callback
...
async def test_async_log_failure_event_logs_to_langfuse():
    ...
    with (
        ...
        with patch(
            "derouter.integrations.langfuse.langfuse_prompt_management.LangFuseHandler"
        ) as mock_handler:
            ...
            kwargs = {
                ...
                "derouter_call_id": "call-fail-123",
                "user": "test-user",
    ...
    with (
        ...
        with patch(
            ...
            kwargs = {
                ...
                "derouter_call_id": "call-no-slo-456",
                "user": "test-user",
...
def test_max_langfuse_clients_limit():
    ...
    original_initialized_langfuse_clients = derouter.initialized_langfuse_clients
    with (
        patch.dict("sys.modules", {"langfuse": mock_langfuse}),
        patch.object(langfuse_module, "MAX_LANGFUSE_INITIALIZED_CLIENTS", 2),
    ):
        # Reset the counter
        derouter.initialized_langfuse_clients = 0
        ...
        assert derouter.initialized_langfuse_clients == 1
        ...
        assert derouter.initialized_langfuse_clients == 2
    ...
    derouter.initialized_langfuse_clients = original_initialized_langfuse_clients
...
def _build_langfuse_logger(monkeypatch) -> LangFuseLogger:
    ...
    monkeypatch.setattr(derouter, "initialized_langfuse_clients", 0)
...
def test_dynamic_langfuse_environment_triggers_dynamic_logger():
    from derouter.integrations.langfuse.langfuse_handler import LangFuseHandler
...
def test_langfuse_sdk_client_survives_httpx_cache_eviction(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "in_memory_llm_clients_cache", LLMClientCache())
...
_LANGFUSE_REDACTED = "redacted-by-derouter"
...
def test_update_trace_keys_header_applies_every_key_when_enabled(flag):
    ...
    with patch.object(derouter, "langfuse_enable_update_trace_keys", flag):
...
def test_update_trace_keys_input_and_output_are_gated_too():
    ...
    with patch.object(derouter, "langfuse_enable_update_trace_keys", True):
