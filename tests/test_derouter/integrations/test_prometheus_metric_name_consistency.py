...
Related issue: https://github.com/Delify-Solutions/derouter/issues/18221
"""
...
def test_remaining_requests_metric_name_in_defined_metrics():
    """
    Test that derouter_remaining_requests_metric is defined in DEFINED_PROMETHEUS_METRICS.
    ...
    assert (
        "derouter_remaining_requests_metric" in defined_metrics
    ), "derouter_remaining_requests_metric should be in DEFINED_PROMETHEUS_METRICS"


def test_remaining_tokens_metric_name_in_defined_metrics():
    """
    Test that derouter_remaining_tokens_metric is defined in DEFINED_PROMETHEUS_METRICS.
    ...
    defined_metrics = get_args(DEFINED_PROMETHEUS_METRICS)
    assert (
        "derouter_remaining_tokens_metric" in defined_metrics
    ), "derouter_remaining_tokens_metric should be in DEFINED_PROMETHEUS_METRICS"
...
def test_prometheus_metric_labels_have_remaining_metrics():
    ...
    ), "Labels for derouter_remaining_requests_metric should be a list"
    assert isinstance(
        remaining_tokens_labels, list
    ), "Labels for derouter_remaining_tokens_metric should be a list"

    # These metrics should have api_provider and api_base labels
    assert (
        "api_provider" in remaining_requests_labels
    ), "derouter_remaining_requests_metric should have api_provider label"
    assert (
        "api_base" in remaining_requests_labels
    ), "derouter_remaining_requests_metric should have api_base label"
    assert (
        "api_provider" in remaining_tokens_labels
    ), "derouter_remaining_tokens_metric should have api_provider label"
    assert (
        "api_base" in remaining_tokens_labels
    ), "derouter_remaining_tokens_metric should have api_base label"
...
def test_all_defined_metrics_have_consistent_naming():
    ...
    for metric_name in defined_metrics:
        # All metrics should start with 'derouter_'
        assert metric_name.startswith(
            "derouter_"
        ), f"Metric {metric_name} should start with 'derouter_'"
