...
async def test_anthropic_cache_control_hook_system_message(monkeypatch: pytest.MonkeyPatch):
    ...
    with patch.dict(
        ...
        monkeypatch.setattr(derouter, "callbacks", [anthropic_cache_control_hook])
...
async def test_anthropic_cache_control_hook_out_of_bounds_logging(monkeypatch: pytest.MonkeyPatch):
    ...
    with patch.dict(
        ...
        with patch("derouter.integrations.anthropic_cache_control_hook.verbose_logger") as mock_logger:
            with patch.object(client, "post", return_value=mock_response) as mock_post:
                messages = [
...
async def test_anthropic_cache_control_hook_multiple_content_items_last_only(monkeypatch: pytest.MonkeyPatch):
    ...
    This verifies the fix for https://github.com/Delify-Solutions/derouter/issues/15696
    """
    with patch.dict(
        os.environ,
        {
...
def test_gemini_cache_control_injection_points_detected():
    ...
    """
    from derouter.llms.vertex_ai.context_caching.transformation import (
        separate_cached_messages,
    )
    from derouter.utils import is_cached_message
...
async def test_cache_control_hook_bedrock_payload_caps_cachepoints_at_four(monkeypatch: pytest.MonkeyPatch):
    ...
    with patch.dict(
        ...
        monkeypatch.setattr(derouter, "callbacks", [AnthropicCacheControlHook()])
...
def test_cache_control_hook_reserves_slot_for_tool_config_point():
    ...
    # stamped so re-entries never re-judge it against derouter's own marks.
    assert non_default_params["cache_control_injection_points"] == [
        {"location": "tool_config", "_derouter_judged": True}
    ]
...
class TestEnableAnthropicPromptCaching:
    """Auto-injected default breakpoints via derouter.enable_anthropic_prompt_caching."""
    ...
    def test_disabled_by_default(self):
        assert derouter.enable_anthropic_prompt_caching is False
        assert self._points() == []
    ...
    def test_non_anthropic_providers_never_injected(self, monkeypatch, model, provider):
        ...
        from derouter.utils import supports_prompt_caching
    ...
    def test_ttl_override_applied(self, monkeypatch, ttl):
        monkeypatch.setattr(derouter, "enable_anthropic_prompt_caching", True)
        monkeypatch.setattr(derouter, "anthropic_prompt_caching_ttl", ttl)
    ...
    def test_messages_with_default_injections_leaves_the_caller_list_untouched(self, monkeypatch):
        ...
        the request is sent. Marking in place would leak derouter's breakpoints into the caller's
...
class TestClaudeCodeOneShotAutoCaching:
    ...
    def test_skips_defaults_and_attribution_for_one_shot_subagent(self, monkeypatch, system, tools):
        ...
        assert "derouter_gateway_injected_cache" not in kwargs["derouter_metadata"]
    ...
    def test_keeps_defaults_for_reusable_subagents(self, monkeypatch, messages, system, tools):
        ...
        assert kwargs["derouter_metadata"]["derouter_gateway_injected_cache"] == ""
...
class TestConfiguredInjectionPointsStandDown:
    ...
    def test_judged_remainder_survives_reentry_despite_injected_marks(self):
        ...
        derouter's own marks as client ones and drop that remainder."""
        remainder = [{"location": "tool_config", "_derouter_judged": True}]
    ...
    def test_v1_messages_stands_down_for_root_cache_control(self, monkeypatch, configured):
        ...
        kwargs = {"cache_control": root_cache_control, "derouter_metadata": {}}
        if configured is not None:
    ...
    def test_v1_messages_reentry_flow_preserves_tool_config_remainder(self):
        ...
        now carry derouter's own marks."""
        ...
        expected_remainder = [{"location": "tool_config", "_derouter_judged": True}]
...
class TestAnthropicPromptCachingEnvVars:
    ...
    auto-caching without a config file. Each case re-imports derouter in a subprocess
    so the env is read fresh without contaminating this process's module graph.
    """

    @staticmethod
    def _import_derouter_with_env(env_override: dict) -> Tuple[bool, Optional[str]]:
        env = os.environ.copy()
        env.pop("DEROUTER_ENABLE_ANTHROPIC_PROMPT_CACHING", None)
        env.pop("DEROUTER_ANTHROPIC_PROMPT_CACHING_TTL", None)
        env.update(env_override)
        script = textwrap.dedent(
            """
            import json, derouter
            print(json.dumps([derouter.enable_anthropic_prompt_caching, derouter.anthropic_prompt_caching_ttl]))
            """
        )
    ...
    def test_unset_env_leaves_auto_caching_off(self):
        assert self._import_derouter_with_env({}) == (False, None)

    @pytest.mark.parametrize("value", ["true", "True", "TRUE"])
    def test_env_enables_auto_caching_case_insensitively(self, value):
        enabled, _ = self._import_derouter_with_env({"DEROUTER_ENABLE_ANTHROPIC_PROMPT_CACHING": value})
    ...
    def test_ttl_env_is_applied(self, value):
        _, ttl = self._import_derouter_with_env({"DEROUTER_ANTHROPIC_PROMPT_CACHING_TTL": value})
        assert ttl == value
...
class TestChatPathProviderStamp:
    ...
    def test_derouter_proxy_target_keeps_anthropic_style_markers(self):
        out, params = self._seed_and_run("derouter_proxy/gpt-5.6", None)
    ...
    def test_global_derouter_api_base_keeps_anthropic_style_markers(self, monkeypatch):
        monkeypatch.setattr(derouter, "api_base", self.CUSTOM_API_BASE)
        out, params = self._seed_and_run("gpt-5.6", None)
        assert out[0] == self.ANTHROPIC_STYLE
        assert "prompt_cache_options" not in params
    ...
    def test_seed_stamps_the_dialect_decision(self, api_base, expected):
        ...
        assert params["cache_control_injection_points"][0]["_derouter_openai_dialect"] is expected

    def test_stamp_is_authoritative_over_request_params(self):
        points = [{**self.POINTS[0], "_derouter_openai_dialect": False}]
...
class TestMessagesPathApiBaseGate:
    ...
    def test_derouter_proxy_target_keeps_cache_control(self):
        block, kwargs = self._inject("gpt-5.6", api_base=self.CUSTOM_API_BASE, custom_llm_provider="derouter_proxy")
...
class TestPromptCacheBreakpointCapability:
    ...
    def _bundled_model_map(self, monkeypatch):
        bundled = os.path.join(os.path.dirname(derouter.__file__), "model_prices_and_context_window_backup.json")
        with open(bundled) as handle:
            monkeypatch.setattr(derouter, "model_cost", json.load(handle))
        derouter.utils._cached_get_model_info_helper.cache_clear()
        yield
        derouter.utils._cached_get_model_info_helper.cache_clear()

    def test_public_helper_reads_the_model_map(self):
        from derouter.utils import supports_prompt_cache_breakpoint
    ...
    def test_model_map_flags_every_openai_gpt_5_6_entry(self, model):
        assert derouter.model_cost[model]["derouter_provider"] == "openai"
        assert derouter.model_cost[model]["supports_prompt_cache_breakpoint"] is True

    def test_listed_model_uses_the_model_map_flag(self, monkeypatch):
        flagged = {**derouter.model_cost["gpt-4.1"], "supports_prompt_cache_breakpoint": True}
        monkeypatch.setitem(derouter.model_cost, "gpt-4.1", flagged)
    ...
    def test_listed_gpt_5_6_without_the_flag_falls_back_to_the_version_rule(self, monkeypatch):
        unflagged = {k: v for k, v in derouter.model_cost["gpt-5.6"].items() if k != "supports_prompt_cache_breakpoint"}
        monkeypatch.setitem(derouter.model_cost, "gpt-5.6", unflagged)
    ...
    def test_listed_model_flagged_false_is_not_eligible(self, monkeypatch):
        monkeypatch.setitem(
            derouter.model_cost, "gpt-5.6", {**derouter.model_cost["gpt-5.6"], "supports_prompt_cache_breakpoint": False}
        )
    ...
    def test_listed_gpt_model_without_the_flag_follows_the_version_rule(self):
        assert "supports_prompt_cache_breakpoint" not in derouter.model_cost["gpt-4.1"]
    ...
    def test_unlisted_model_falls_back_to_the_version_rule(self, model, expected):
        assert model not in derouter.model_cost
        assert supports_openai_prompt_cache_breakpoint(model) is expected
...
class TestRecordGatewayInjection:
    ...
    KEY = "derouter_gateway_injected_cache"
    DEPLOYMENT = "dep-abc"
    ...
    def test_records_only_an_actual_injection(self):
        ...
        the messages; neither is derouter adding a breakpoint."""
    ...
    def test_a_point_this_pass_did_not_place_is_not_claimed(self):
        ...
        breakpoint reaches the wire. Claiming it credited derouter on request shapes that
    ...
    def test_a_later_pass_cannot_unset_an_earlier_injection(self):
        kwargs: dict = {"derouter_metadata": {"user_api_key": "k"}, "model_info": {"id": self.DEPLOYMENT}}
        AnthropicCacheControlHook.record_gateway_injection(kwargs, 2)
        AnthropicCacheControlHook.record_gateway_injection(kwargs, 0)
        assert kwargs["derouter_metadata"][self.KEY] == self.DEPLOYMENT
    ...
    def test_an_every_deployment_mark_survives_a_later_per_deployment_stamp(self):
        ...
        kwargs: dict = {"derouter_metadata": {self.KEY: ""}, "model_info": {"id": self.DEPLOYMENT}}
        AnthropicCacheControlHook.record_gateway_injection(kwargs, 1)
        assert kwargs["derouter_metadata"][self.KEY] == ""
    ...
    def test_a_pre_choice_pass_stamps_the_sentinel_over_a_provisional_deployment(self):
        ...
        kwargs: dict = {"derouter_metadata": {}, "model_info": {"id": self.DEPLOYMENT}}
    ...
    def test_a_per_deployment_mark_still_follows_the_latest_leg(self):
        kwargs: dict = {"derouter_metadata": {self.KEY: "dep-old"}, "model_info": {"id": self.DEPLOYMENT}}
        AnthropicCacheControlHook.record_gateway_injection(kwargs, 1)
        assert kwargs["derouter_metadata"][self.KEY] == self.DEPLOYMENT
    ...
    def test_v1_messages_stand_down_leaves_no_marker(self, monkeypatch):
        ...
        assert self.KEY not in kwargs["derouter_metadata"]
