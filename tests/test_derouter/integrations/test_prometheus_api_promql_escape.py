...
async def test_get_daily_spend_does_not_pass_raw_quote_into_query():
    from derouter.integrations.prometheus_helpers import prometheus_api
    ...
    assert rendered_query.startswith(
        'sum(delta(derouter_spend_metric_total{hashed_api_key="'
    )
    ...
    inner = rendered_query[
        len('sum(delta(derouter_spend_metric_total{hashed_api_key="') : -len('"}[1d]))')
    ]
...
async def test_get_daily_spend_with_no_api_key_uses_unfiltered_query():
    ...
    assert captured["params"]["query"] == "sum(delta(derouter_spend_metric_total[1d]))"
...
async def test_get_daily_spend_legitimate_hashed_key_unchanged():
    ...
    assert (
        ...
        == f'sum(delta(derouter_spend_metric_total{{hashed_api_key="{legit_key}"}}[1d]))'
    )
