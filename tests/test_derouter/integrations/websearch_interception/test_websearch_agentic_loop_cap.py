...
The web search interception loop injects an internal tool (derouter_web_search)
...
from derouter.derouter_core_utils.agentic_loop_settings import DEFAULT_MAX_AGENTIC_LOOPS
...
INTERNAL_TOOL_NAME = "derouter_web_search"
...
def only_the_callbacks_these_tests_register(monkeypatch):
    ...
    object, so a logger another test left on derouter.callbacks would join the
...
class TestOuterFramePostHookStillRuns:
    ...
    async def test_parent_frame_injects_its_blocks_after_the_cap_trips(self, monkeypatch):
        ...
        monkeypatch.setattr("derouter.anthropic_interface.messages.acreate", fake_acreate)
...
class TestBothCeilingKnobsAreValidated:
    ...
    def test_a_per_deployment_non_integer_names_the_field_it_came_from(self):
        with pytest.raises(TypeError, match=r"derouter_params\.max_agentic_loops must be an integer"):
    ...
    def test_each_knob_names_its_own_config_field(self):
        ...
        with pytest.raises(ValueError, match=r"derouter_params\.max_agentic_loops"):
            BaseLLMHTTPHandler._get_agentic_loop_settings(kwargs={"max_agentic_loops": 0})
