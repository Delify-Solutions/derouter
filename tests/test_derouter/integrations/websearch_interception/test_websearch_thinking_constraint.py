...
Covers:
  ...
  - M2-I5/I8: derouter_logging_obj excluded from follow-up kwargs to prevent SpendLog dedup
    ...
    return [
        {
            ...
            "type": "tool_use",
            "name": "web_search",
            "input": {"query": "derouter spend tracking"},
        }
    ]
...
# ---------------------------------------------------------------------------
# M2-I5 / M2-I8: derouter_logging_obj excluded from follow-up kwargs
# ---------------------------------------------------------------------------


class TestLoggingObjExcludedFromFollowUp:
    """Verify derouter_logging_obj is NOT forwarded to the follow-up acreate() call.
    ...
    @pytest.mark.asyncio
    async def test_derouter_logging_obj_excluded_from_anthropic_followup(self):
        """The Anthropic messages follow-up must NOT receive derouter_logging_obj."""
        logger = WebSearchInterceptionLogger(enabled_providers=["bedrock"])
        captured_kwargs: Dict[str, Any] = {}
        ...
        # derouter_logging_obj must be absent from the follow-up call
        assert "derouter_logging_obj" not in captured_kwargs
    ...
    async def test_websearch_flags_also_excluded(self):
        """Both _websearch_interception flags and derouter_logging_obj must be excluded."""
        logger = WebSearchInterceptionLogger(enabled_providers=["bedrock"])
        captured_kwargs: Dict[str, Any] = {}
...
class TestFollowUpErrorScenarios:
    ...
    async def test_metadata_preserved_after_logging_obj_exclusion(self):
        ...
        even after derouter_logging_obj is excluded — so the new logging_obj from
        ...
        with (
            ...
            await logger._execute_agentic_loop(
                ...
                kwargs={
                    ...
                    "derouter_call_id": "call-abc-123",
                },
            )

        # derouter_logging_obj excluded
        assert "derouter_logging_obj" not in captured_kwargs
        # But ALL proxy metadata must be preserved
        assert captured_kwargs.get("metadata") == proxy_metadata
        assert captured_kwargs.get("derouter_call_id") == "call-abc-123"
