...
class TestIsAnthropicNativeWebSearchTool:
    ...
    def test_rejects_derouter_standard(self):
        assert not is_anthropic_native_web_search_tool(
            {"name": "derouter_web_search", "input_schema": {}}
        )

    def test_rejects_openai_function_shape(self):
        assert not is_anthropic_native_web_search_tool(
            {"type": "function", "function": {"name": "derouter_web_search"}}
        )
...
class TestBuildWebSearchToolResultBlock:
    ...
    def test_shape_with_results(self):
        ...
        assert first["url"] == "https://router.delify.vn/"
        assert first["title"] == "DeRouter Docs"
...
class TestPreRequestHookFlagsNativeTools:
    ...
    @pytest.mark.asyncio
    async def test_derouter_standard_tool_does_not_set_flag(self):
        logger = WebSearchInterceptionLogger(enabled_providers=["bedrock"])
        kwargs = {
            "tools": [{"name": "derouter_web_search", "input_schema": {}}],
            "derouter_params": {"custom_llm_provider": "bedrock"},
        }
        out = await logger.async_pre_request_hook(
            model="bedrock/claude", messages=[], kwargs=kwargs
...
class TestBuildPlanAttachesBlocks:
    ...
    async def test_metadata_carries_blocks_when_flag_set(self):
        ...
        assert blocks[0]["input"] == {"query": "what is derouter"}
        assert blocks[1]["tool_use_id"] == blocks[0]["id"]
        assert blocks[1]["content"][0]["url"] == "https://router.delify.vn/"
...
class TestShortCircuitEmitsNativeBlocks:
    ...
    async def test_native_tool_short_circuit_emits_blocks(self):
        ...
        assert tool_result["content"][0]["url"] == "https://router.delify.vn/"

    @pytest.mark.asyncio
    async def test_derouter_standard_tool_short_circuit_stays_text_only(self):
...
class TestLegacyPathMatchesNewPath:
    ...
    async def test_legacy_path_injects_when_flag_set(self):
        ...
        with (
            ...
            patch(
                "derouter.integrations.websearch_interception.handler.anthropic_messages.acreate",
