...
from derouter.proxy._types import DeRouter_ObjectPermissionTable, DeRouter_TeamTable, ProxyException, UserAPIKeyAuth
from derouter.types.utils import LlmProviders


def test_initialize_from_proxy_config():
    """Test initialization from proxy config with derouter_settings"""
    derouter_settings = {
...
async def test_async_build_agentic_loop_plan_returns_request_patch():
    ...
    kwargs = {
        ...
        "derouter_logging_obj": object(),
    }

    plan = await logger.async_build_agentic_loop_plan(
        tools=tools_dict,
        model="claude-3-5-sonnet",
        messages=[{"role": "user", "content": "search DeRouter"}],
        response=None,
        anthropic_messages_provider_config=None,
        anthropic_messages_optional_request_params={
            "max_tokens": 1024,
            "tools": [{"name": "derouter_web_search"}],
        },
    ...
    assert "derouter_logging_obj" not in plan.request_patch.kwargs
...
@pytest.mark.asyncio
async def test_execute_search_passes_selected_search_tool_derouter_params(monkeypatch):
    import derouter
    ...
    monkeypatch.setattr(derouter, "asearch", mock_asearch)

    await logger._execute_search(
        "what is derouter",
        kwargs={"derouter_params": {"metadata": {"user_api_key_auth": user_api_key_auth}}},
    )

    forwarded_kwargs = mock_asearch.await_args.kwargs
    assert forwarded_kwargs["query"] == "what is derouter"
...
async def test_execute_search_attributes_spend_to_the_calling_key(monkeypatch):
    ...
    import derouter
    from derouter.proxy import proxy_server
    from derouter.proxy.hooks.proxy_track_cost_callback import _should_track_cost_callback
    ...
    router.search_tools = [
        {
            "search_tool_name": "perplexity-sonar-pro",
            "derouter_params": {"search_provider": "perplexity", "api_key": "fake-key"},
        }
    ]
    mock_asearch = AsyncMock(return_value=SearchResponse(object="search", results=[]))
    user_api_key_auth = UserAPIKeyAuth(
    ...
    forwarded_metadata = mock_asearch.await_args.kwargs["derouter_metadata"]
...
async def test_execute_search_without_proxy_auth_context_stays_sdk_only(monkeypatch):
    ...
    await logger._execute_search("what is derouter", kwargs={"derouter_params": {}})

    assert "derouter_metadata" not in mock_asearch.await_args.kwargs
...
async def test_execute_search_enforces_team_search_tool_permission(monkeypatch):
    ...
    monkeypatch.setattr(proxy_server, "llm_router", router)
    monkeypatch.setattr(derouter, "asearch", mock_asearch)
    monkeypatch.setattr("derouter.proxy.auth.auth_checks.get_team_object", mock_get_team_object)
...
async def test_async_pre_call_deployment_hook_provider_from_top_level_kwargs():
    ...
    Regression test for bug where the hook only checked kwargs["derouter_params"]["custom_llm_provider"]
    but the router places custom_llm_provider at the top level of kwargs.
    """
    logger = WebSearchInterceptionLogger(enabled_providers=["bedrock"])

    # Simulate kwargs as they arrive from the router path:
    # custom_llm_provider is at the TOP LEVEL (not nested under derouter_params)
    kwargs = {
    ...
    # The web_search tool should be converted to derouter_web_search (OpenAI format)
    assert any(
        t.get("type") == "function" and t.get("function", {}).get("name") == "derouter_web_search"
...
@pytest.mark.asyncio
async def test_async_pre_call_deployment_hook_nested_derouter_params_fallback():
    """Test that the hook still works when custom_llm_provider is in nested derouter_params.

    This is the Anthropic experimental pass-through path where derouter_params is
...
async def test_async_pre_call_deployment_hook_provider_derived_from_model_name():
    ...
    in kwargs at all — neither at top-level nor in derouter_params. The hook must derive
...
async def test_deployment_hook_converts_stream_and_logging_obj_syncs():
    ...
    kwargs = {
        ...
        "messages": [{"role": "user", "content": "Search for DeRouter"}],
        "tools": [
            {"type": "web_search_20250305", "name": "web_search", "max_uses": 3},
        ],
        "custom_llm_provider": "bedrock",
...
def test_sync_forced_tool_choice_repoints_converted_web_search():
    ...
    web_search tool must be repointed to derouter_web_search after conversion.

    Native clients (e.g. Claude Code) send
    tool_choice={"type": "tool", "name": "web_search"}. The tool definition is
    renamed to derouter_web_search, so an unrewritten tool_choice points at a
    ...
    converted_tools = [
        {
            "type": "function",
            "function": {"name": DEROUTER_WEB_SEARCH_TOOL_NAME, "parameters": {}},
        }
    ]
    ...
    assert result == {"type": "tool", "name": DEROUTER_WEB_SEARCH_TOOL_NAME}
...
def test_sync_forced_tool_choice_leaves_existing_tool_untouched():
    ...
    converted_tools = [{"name": DEROUTER_WEB_SEARCH_TOOL_NAME}]

    result = WebSearchInterceptionLogger._sync_forced_tool_choice(
        {"type": "tool", "name": DEROUTER_WEB_SEARCH_TOOL_NAME}, converted_tools
    )
...
async def test_pre_request_hook_syncs_forced_tool_choice():
    ...
    kwargs = {
        "derouter_params": {"custom_llm_provider": "anthropic"},
        "tools": [{"type": "web_search_20250305", "name": "web_search"}],
