...
Tests that the websearch_interception callback intercepts derouter_web_search
...
async def test_responses_hook_detects_function_call():
    """async_should_run_responses_agentic_loop detects a derouter_web_search function_call."""
    ...
    should_run, tools_dict = await logger.async_should_run_responses_agentic_loop(
        ...
        tools=[{"type": "function", "name": "derouter_web_search"}],
        stream=False,
...
async def test_build_responses_plan_produces_responses_input():
    ...
    with patch.object(
        ...
        plan = await logger.async_build_responses_agentic_loop_plan(
            ...
            optional_params={
                "tools": [{"type": "function", "name": "derouter_web_search"}],
                "tool_choice": {"type": "function", "name": "derouter_web_search"},
            },
    ...
    assert patch_obj.tools == [{"type": "function", "name": "derouter_web_search"}]
...
async def test_deployment_hook_converts_native_responses_web_search_tool():
    ...
    into the flat derouter_web_search function tool (Responses shape, not the
    ...
    assert tool["name"] == "derouter_web_search"
