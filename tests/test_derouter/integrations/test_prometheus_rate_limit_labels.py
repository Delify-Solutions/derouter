...
1. ``derouter_proxy_failed_requests_metric`` now carries
   ``rate_limit_category`` and ``rate_limit_type`` labels populated from
   :class:`derouter.RateLimitError` (vendor + ``ProxyRateLimitError``
...
2. ``_get_exception_class_name`` keeps emitting the literal string
   ...
   that key off ``exception_class="HTTPException"`` for derouter-internal
...
def test_should_include_rate_limit_labels_on_failed_requests_metric():
    ...
    original = derouter.prometheus_emit_rate_limit_labels
    try:
        ...
        derouter.prometheus_emit_rate_limit_labels = original
...
def test_should_omit_rate_limit_labels_by_default_for_back_compat():
    ...
    dashboards / recording rules keyed on `derouter_proxy_failed_requests_metric`
    keep matching after upgrade."""
    import derouter

    assert derouter.prometheus_emit_rate_limit_labels is False
    labels = PrometheusMetricLabels.get_labels("derouter_proxy_failed_requests_metric")
...
def test_should_accept_rate_limit_fields_on_user_api_key_label_values():
    enum_values = UserAPIKeyLabelValues(
        rate_limit_category="derouter_rate_limit",
        rate_limit_type="requests",
    )
    assert enum_values.rate_limit_category == "derouter_rate_limit"
...
def test_should_extract_derouter_category_and_type_for_proxy_rate_limit_error():
    ...
    category, rate_limit_type = PrometheusLogger._extract_rate_limit_labels(err)
    assert category == "derouter_rate_limit"
...
def test_should_extract_budget_dimension_for_budget_exceeded_error():
    ...
    # `derouter.BudgetExceededError` (a bare Exception subclass), which sets
    ...
    err = derouter.BudgetExceededError(current_cost=0.5, max_budget=0.1)
    category, rate_limit_type = PrometheusLogger._extract_rate_limit_labels(err)
    assert category == "derouter_rate_limit"
...
@pytest.mark.parametrize(
    ...
    [
        (
            RateLimitErrorCategory.DEROUTER_RATE_LIMIT,
...
def test_should_emit_legacy_http_exception_label_for_proxy_rate_limit_error():
    ...
    thread + PR #27687 review). Distinguishing vendor vs. derouter 429s
...
async def test_should_populate_rate_limit_labels_for_proxy_rate_limit_error_on_failure_hook():
    ...
    """
    with patch(
        "derouter.integrations.prometheus.PrometheusLogger.__init__", return_value=None
    ):
        logger = PrometheusLogger()
        logger.derouter_proxy_failed_requests_metric = MagicMock()
        logger.derouter_proxy_total_requests_metric = MagicMock()
    ...
    with patch(
        "derouter.integrations.prometheus.prometheus_label_factory"
...
def _logger_with_mock_virtual_key_gauges() -> PrometheusLogger:
    with patch(
        ...
        logger = PrometheusLogger()
    logger.derouter_remaining_api_key_requests_for_model = MagicMock()
    logger.derouter_remaining_api_key_tokens_for_model = MagicMock()
...
def _kwargs_with_v3_rate_limit_headers(additional_headers: dict) -> dict:
    return {
        "derouter_params": {"metadata": {"model_group": "gpt-4o-mini"}},
...
def test_should_read_v3_remaining_headers_when_metadata_keys_absent():
    ...
    the legacy ``derouter-key-remaining-*`` metadata keys, so the gauges were
    ...
    logger.derouter_remaining_api_key_requests_for_model.labels.return_value.set.assert_called_once_with(
        42
    )
    logger.derouter_remaining_api_key_tokens_for_model.labels.return_value.set.assert_called_once_with(
        900
    )
...
def test_should_prefer_legacy_metadata_keys_over_v3_headers():
    ...
    kwargs["derouter_params"]["metadata"].update(
        {
            "derouter-key-remaining-requests-gpt-4o-mini": 3,
            "derouter-key-remaining-tokens-gpt-4o-mini": 200,
        }
    )
...
async def test_should_emit_key_and_team_rate_limit_allowed_and_used_from_v3_headers():
    ...
    try:
        ...
        assert _collected_samples("derouter_api_key_rate_limit_allowed_metric") == {
            key_requests: 10,
            key_tokens: 20000,
        }
        assert _collected_samples("derouter_api_key_rate_limit_used_metric") == {
            key_requests: 3,
            key_tokens: 53,
        }
        assert _collected_samples("derouter_team_rate_limit_allowed_metric") == {
            team_requests: 50,
            team_tokens: 40000,
        }
        assert _collected_samples("derouter_team_rate_limit_used_metric") == {
            team_requests: 3,
            team_tokens: 40,
        }
    finally:
...
async def test_should_emit_only_the_dimensions_the_limiter_enforced():
    ...
    try:
        ...
        assert _collected_samples("derouter_api_key_rate_limit_allowed_metric") == {key_requests: 10}
        assert _collected_samples("derouter_api_key_rate_limit_used_metric") == {key_requests: 0}
        assert _collected_samples("derouter_team_rate_limit_allowed_metric") == {}
        assert _collected_samples("derouter_team_rate_limit_used_metric") == {}
    finally:
        _clear_prometheus_registry()
...
async def test_should_drop_key_and_team_series_once_the_limiter_stops_reporting_a_limit():
    ...
    try:
        ...
        assert _collected_samples("derouter_api_key_rate_limit_allowed_metric") == {}
        assert _collected_samples("derouter_api_key_rate_limit_used_metric") == {}
        assert _collected_samples("derouter_team_rate_limit_allowed_metric") == {team_requests: 50}
        assert _collected_samples("derouter_team_rate_limit_used_metric") == {team_requests: 4}
    finally:
        _clear_prometheus_registry()
