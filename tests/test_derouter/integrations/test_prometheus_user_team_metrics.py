...
class TestPrometheusUserTeamCountMetrics:
    ...
    def test_user_team_count_metrics_initialization(self, prometheus_logger):
        ...
        assert hasattr(prometheus_logger, "derouter_total_users_metric")
        assert hasattr(prometheus_logger, "derouter_teams_count_metric")

        # Verify the metrics are not None
        assert prometheus_logger.derouter_total_users_metric is not None
        assert prometheus_logger.derouter_teams_count_metric is not None

    def test_user_count_metric_has_no_labels(self, prometheus_logger):
        """Test that derouter_total_users metric has no labels (as specified)"""
        metric = prometheus_logger.derouter_total_users_metric
        ...
        try:
            ...
            pytest.fail(f"derouter_total_users_metric should not require labels: {e}")

    def test_teams_count_metric_has_no_labels(self, prometheus_logger):
        """Test that derouter_teams_count metric has no labels (as specified)"""
        metric = prometheus_logger.derouter_teams_count_metric
        ...
        try:
            ...
            pytest.fail(f"derouter_teams_count_metric should not require labels: {e}")
    ...
    def test_user_count_metric_accepts_various_values(self, prometheus_logger):
        ...
        for value in test_values:
            try:
                ...
                pytest.fail(
                    f"derouter_total_users_metric should accept value {value}: {e}"
                )
    ...
    def test_team_count_metric_accepts_various_values(self, prometheus_logger):
        ...
        for value in test_values:
            try:
                ...
                pytest.fail(
                    f"derouter_teams_count_metric should accept value {value}: {e}"
                )
    ...
    def test_user_count_metric_with_zero(self, prometheus_logger):
        ...
        try:
            ...
            pytest.fail(f"derouter_total_users_metric should handle zero: {e}")
    ...
    def test_team_count_metric_with_zero(self, prometheus_logger):
        ...
        try:
            ...
            pytest.fail(f"derouter_teams_count_metric should handle zero: {e}")
    ...
    def test_metrics_can_be_updated_multiple_times(self, prometheus_logger):
        ...
        user_metric = prometheus_logger.derouter_total_users_metric
        team_metric = prometheus_logger.derouter_teams_count_metric
    ...
    def test_metrics_can_be_collected_by_prometheus(self, prometheus_logger):
        ...
        # Set some values
        prometheus_logger.derouter_total_users_metric.set(100)
        prometheus_logger.derouter_teams_count_metric.set(20)
        ...
        assert (
            "derouter_total_users" in metrics or "derouter_total_users_total" in metrics
        )
        assert (
            "derouter_teams_count" in metrics or "derouter_teams_count_total" in metrics
        )
    ...
    def test_active_users_metric_initialized(self, prometheus_logger):
        """derouter_active_users gauge must exist alongside derouter_total_users."""
        assert hasattr(prometheus_logger, "derouter_active_users_metric")
        assert prometheus_logger.derouter_active_users_metric is not None

    @pytest.mark.asyncio
    async def test_initialize_counts_total_and_active_users(self, prometheus_logger):
        """derouter_total_users counts every row; derouter_active_users counts only
        billable (non SCIM-deactivated) users."""
        import sys

        prometheus_logger.derouter_total_users_metric = MagicMock()
        prometheus_logger.derouter_active_users_metric = MagicMock()
        ...
        mock_prisma.db.derouter_usertable.count = _user_count
        mock_prisma.db.derouter_teamtable.count = AsyncMock(return_value=4)

        mock_proxy_server = MagicMock()
        mock_proxy_server.prisma_client = mock_prisma

        with patch.dict(sys.modules, {"derouter.proxy.proxy_server": mock_proxy_server}):
            await prometheus_logger._initialize_user_and_team_count_metrics()

        prometheus_logger.derouter_total_users_metric.set.assert_called_once_with(10)
        prometheus_logger.derouter_active_users_metric.set.assert_called_once_with(8)
        prometheus_logger.derouter_teams_count_metric.set.assert_called_once_with(4)
    ...
    def test_metrics_have_correct_type(self, prometheus_logger):
        ...
        assert hasattr(prometheus_logger.derouter_total_users_metric, "set")
        assert hasattr(prometheus_logger.derouter_teams_count_metric, "set")

        # Gauges have set() method, Counters only have inc()
        assert callable(prometheus_logger.derouter_total_users_metric.set)
        assert callable(prometheus_logger.derouter_teams_count_metric.set)
...
async def test_set_team_budget_metrics_after_api_request_no_inf_when_metadata_budget_none(
    ...
    set_call_args = (
        prometheus_logger.derouter_remaining_team_budget_metric.labels().set.call_args
    )
    assert (
...
async def test_assemble_user_object_uses_db_max_budget_when_metadata_is_none(
    ...
    with patch("derouter.proxy.auth.auth_checks.get_user_object") as mock_get_user:
...
def test_set_user_budget_metrics_default_no_email_alias_labels(
    ...
    derouter.prometheus_user_budget_label_include_email_alias = False
    ...
    prometheus_logger.derouter_user_max_budget_metric = MagicMock()
    prometheus_logger.derouter_user_budget_remaining_hours_metric = MagicMock()

    prometheus_logger._set_user_budget_metrics(user)

    prometheus_logger.derouter_remaining_user_budget_metric.labels.assert_called_once_with(
        user="user-abc-123",
    )
...
def test_set_user_budget_metrics_includes_user_email_and_alias_labels_when_opted_in():
    ...
    derouter.prometheus_user_budget_label_include_email_alias = True
    ...
    try:
        ...
        prometheus_logger.derouter_remaining_user_budget_metric.labels().set.assert_called_once_with(
            75.0
        )
        prometheus_logger.derouter_user_max_budget_metric.labels.assert_called_once_with(
            user="user-abc-123",
            user_email="alice@example.com",
            user_alias="Alice",
        )
        prometheus_logger.derouter_user_budget_remaining_hours_metric.labels.assert_called_once_with(
            user="user-abc-123",
            user_email="alice@example.com",
            user_alias="Alice",
        )
...
async def test_set_user_budget_metrics_after_api_request_no_inf_when_metadata_budget_none(
    ...
    set_call_args = (
        prometheus_logger.derouter_remaining_user_budget_metric.labels().set.call_args
    )
    assert (
        set_call_args is not None
...
def test_per_request_metrics_emit_all_identity_labels(prometheus_logger):
    ...
    try:
        ...
        label_kwargs = prometheus_logger.derouter_requests_metric.labels.call_args.kwargs
        ...
        from derouter.types.integrations.prometheus import PrometheusMetricLabels
        ...
        derouter.custom_prometheus_metadata_labels = ["org_id"]
        labels = PrometheusMetricLabels.get_labels("derouter_requests_metric")
...
def test_org_budget_metrics_initialized(prometheus_logger):
    ...
    assert hasattr(prometheus_logger, "derouter_remaining_org_budget_metric")
    assert hasattr(prometheus_logger, "derouter_org_max_budget_metric")
    assert hasattr(prometheus_logger, "derouter_org_budget_remaining_hours_metric")
    assert prometheus_logger.derouter_remaining_org_budget_metric is not None
    assert prometheus_logger.derouter_org_max_budget_metric is not None
    assert prometheus_logger.derouter_org_budget_remaining_hours_metric is not None
...
def test_set_org_budget_metrics_remaining_budget(prometheus_logger):
    ...
    prometheus_logger.derouter_org_max_budget_metric = MagicMock()
    prometheus_logger.derouter_org_budget_remaining_hours_metric = MagicMock()
    ...
    set_call = prometheus_logger.derouter_remaining_org_budget_metric.labels().set
...
def test_set_org_budget_metrics_max_budget(prometheus_logger):
    ...
    prometheus_logger.derouter_org_max_budget_metric.labels().set.assert_called_once_with(
        1000.0
    )
...
def test_set_org_budget_metrics_no_max_budget(prometheus_logger):
    ...
    prometheus_logger.derouter_org_max_budget_metric.labels().set.assert_not_called()
...
def test_set_org_budget_metrics_remaining_hours(prometheus_logger):
    ...
    prometheus_logger.derouter_org_budget_remaining_hours_metric.labels().set.assert_called_once()
...
async def test_set_org_budget_metrics_after_api_request(prometheus_logger):
    ...
    org_mock.derouter_budget_table = budget_mock
    ...
    with (
        patch.dict(sys.modules, {"derouter.proxy.proxy_server": mock_proxy_server}),
        patch(
    ...
    remaining_call = (
        prometheus_logger.derouter_remaining_org_budget_metric.labels().set.call_args
    )
...
async def test_set_org_budget_metrics_after_api_request_no_org_id(prometheus_logger):
    ...
    prometheus_logger.derouter_remaining_org_budget_metric.labels().set.assert_not_called()
    prometheus_logger.derouter_org_max_budget_metric.labels().set.assert_not_called()
    prometheus_logger.derouter_org_budget_remaining_hours_metric.labels().set.assert_not_called()
...
async def test_initialize_org_budget_metrics(prometheus_logger):
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_organizationtable.find_many = AsyncMock(
        return_value=[org_mock]
    )
    mock_prisma.db.derouter_organizationtable.count = AsyncMock(return_value=1)
    ...
    prometheus_logger.derouter_remaining_org_budget_metric.labels().set.assert_called_once()
    prometheus_logger.derouter_org_max_budget_metric.labels().set.assert_called_once_with(
        500.0
    )
...
def test_default_latency_buckets(prometheus_logger):
    ...
    from derouter.types.integrations.prometheus import LATENCY_BUCKETS
...
def test_custom_latency_buckets():
    """prometheus_latency_buckets in derouter settings overrides the defaults."""
    import derouter
    from prometheus_client import REGISTRY

    custom_buckets = [0.1, 0.5, 1.0, 5.0, 10.0]
    original = derouter.prometheus_latency_buckets
    ...
    for collector in list(REGISTRY._collector_to_names.keys()):
        try:
            ...
            pass
    try:
        derouter.prometheus_latency_buckets = custom_buckets
        logger = PrometheusLogger()
        assert logger.latency_buckets == tuple(custom_buckets)
    finally:
        derouter.prometheus_latency_buckets = original
        for collector in list(REGISTRY._collector_to_names.keys()):
            try:
                REGISTRY.unregister(collector)
            except Exception:
...
class TestSetTeamMembersMetric:
    """derouter_team_members_metric tracks the current member count per team."""
    ...
    def test_metric_initialized(self, prometheus_logger):
        assert hasattr(prometheus_logger, "derouter_team_members_metric")
        assert prometheus_logger.derouter_team_members_metric is not None
    ...
    def test_teams_isolated_by_label(self, prometheus_logger):
        ...
        team_one = DeRouter_TeamTable(
            team_id="team-1",
            team_alias="One",
            members_with_roles=[Member(user_id="a", role="user")],
        )
        team_two = DeRouter_TeamTable(
            team_id="team-2",
            team_alias="Two",
            members_with_roles=[
