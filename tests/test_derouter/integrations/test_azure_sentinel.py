...
from derouter.types.integrations.azure_sentinel import AZURE_SENTINEL_MAX_PAYLOAD_SIZE_BYTES
from derouter.types.utils import StandardAuditLogPayload, StandardLoggingPayload
...
async def test_azure_sentinel_sends_audit_log_payload_to_ingestion_api():
    ...
    assert "Custom-DeRouter" in api_call_args.kwargs["url"]
...
async def test_azure_sentinel_flushes_standard_and_audit_logs_separately():
    ...
    with patch("asyncio.create_task", side_effect=_close_periodic_flush_task):
        logger = AzureSentinelLogger(
            dcr_immutable_id="dcr-test123456789",
            stream_name="Custom-DeRouter-Standard",
            audit_stream_name="Custom-DeRouter-Audit",
            endpoint="https://test-dce.eastus-1.ingest.monitor.azure.com",
            tenant_id="test-tenant-id",
            client_id="test-client-id",
            client_secret="test-client-secret",
    ...
    assert "Custom-DeRouter-Standard" in standard_call.kwargs["url"]
    assert json.loads(standard_call.kwargs["data"].decode("utf-8")) == [
        standard_payload
    ]
    assert "Custom-DeRouter-Audit" in audit_call.kwargs["url"]
...
async def test_azure_sentinel_audit_stream_name_from_env_var(monkeypatch):
    ...
    monkeypatch.setenv("AZURE_SENTINEL_STREAM_NAME", "Custom-DeRouter-Standard")
    monkeypatch.setenv("AZURE_SENTINEL_AUDIT_STREAM_NAME", "Custom-DeRouter-Audit")
    ...
    assert logger.audit_stream_name == "Custom-DeRouter-Audit"
    assert "streams/Custom-DeRouter-Audit" in logger.audit_api_endpoint
    assert "streams/Custom-DeRouter-Standard" in logger.api_endpoint
    ...
    with patch("asyncio.create_task", side_effect=_close_periodic_flush_task):
        explicit_logger = AzureSentinelLogger(
            ...
            client_id="test-client-id",
            client_secret="test-client-secret",
            audit_stream_name="Custom-DeRouter-Explicit",
        )

    assert explicit_logger.audit_stream_name == "Custom-DeRouter-Explicit"
...
def _rejected(status_code, *, raised):
    """derouter's http handler calls raise_for_status, so a real rejection arrives raised, not returned."""
    request = Request("POST", "https://example.com")
...
async def test_azure_sentinel_halves_the_batch_on_413(queue_attr, send_method, build_payloads, raised):
    ...
    derouter's http handler raises MaskedHTTPStatusError on a 4xx, so the raised path is the one
