...
Run with:
    uv run pytest tests/test_derouter/integrations/test_prometheus_token_detail_metrics.py -v
"""
...
class TestTokenDetailMetricsRegistration:
    ...
    def test_input_detail_metrics_share_input_label_set(self):
        ...
        assert (
            PrometheusMetricLabels.derouter_input_cached_tokens_metric
            == PrometheusMetricLabels.derouter_input_tokens_metric
        )
        assert (
            PrometheusMetricLabels.derouter_input_cache_creation_tokens_metric
            == PrometheusMetricLabels.derouter_input_tokens_metric
        )
        assert (
            PrometheusMetricLabels.derouter_input_audio_tokens_metric
            == PrometheusMetricLabels.derouter_input_tokens_metric
        )

    def test_output_detail_metrics_share_output_label_set(self):
        assert (
            PrometheusMetricLabels.derouter_output_reasoning_tokens_metric
            == PrometheusMetricLabels.derouter_output_tokens_metric
        )
        assert (
            PrometheusMetricLabels.derouter_output_audio_tokens_metric
            == PrometheusMetricLabels.derouter_output_tokens_metric
        )
...
class TestIncrementTokenDetailMetrics:
    ...
    def test_increments_all_present_token_types(self, sample_enum_values):
        ...
        logger.derouter_input_cached_tokens_metric.labels().inc.assert_called_once_with(
            40.0
        )
        logger.derouter_input_cache_creation_tokens_metric.labels().inc.assert_called_once_with(
            25.0
        )
        logger.derouter_input_audio_tokens_metric.labels().inc.assert_called_once_with(
            15.0
        )
        logger.derouter_output_reasoning_tokens_metric.labels().inc.assert_called_once_with(
            60.0
        )
        logger.derouter_output_audio_tokens_metric.labels().inc.assert_called_once_with(
            10.0
        )
    ...
    def test_skips_metrics_when_value_is_none(self, sample_enum_values):
        ...
        logger.derouter_input_cached_tokens_metric.labels.assert_not_called()
        logger.derouter_input_cache_creation_tokens_metric.labels.assert_not_called()
        logger.derouter_input_audio_tokens_metric.labels().inc.assert_called_once_with(
            12.0
        )
        logger.derouter_output_reasoning_tokens_metric.labels.assert_not_called()
        logger.derouter_output_audio_tokens_metric.labels.assert_not_called()
