...
def test_user_email_in_required_metrics():
    ...
    - derouter_proxy_total_requests_metric (already had it)
    - derouter_proxy_failed_requests_metric (added)
    - derouter_input_tokens_metric (added)
    - derouter_output_tokens_metric (added)
    - derouter_requests_metric (already had it)
    - derouter_spend_metric (added)
    """
...
def test_api_provider_in_spend_and_requests_metrics():
    ...
    for metric_name in ["derouter_spend_metric", "derouter_requests_metric"]:
        labels = PrometheusMetricLabels.get_labels(metric_name)
        assert (
...
def test_api_provider_in_token_latency_and_request_metrics():
    ...
    / async_post_call_failure_hook) as derouter_spend_metric and
    derouter_requests_metric, which already carry api_provider. They were the odd
...
def test_api_provider_value_flows_through_label_factory():
    ...
    enum_values = UserAPIKeyLabelValues(
        ...
        derouter_model_name="claude-sonnet-4",
...
def test_extract_api_provider_from_request_data_failure_path():
    ...
    resolved custom_llm_provider on derouter_params, fall back to a partial
    ...
    assert extract({"derouter_params": {"custom_llm_provider": "bedrock"}}) == "bedrock"
    assert (
        extract({"standard_logging_object": {"custom_llm_provider": "vertex_ai"}})
        == "vertex_ai"
    )
    # derouter_params wins over standard_logging_object when both are present
    assert (
        extract(
            {
    ...
    assert extract({"derouter_params": {"model": "anthropic/claude-haiku-4-5"}}) == "anthropic"
...
def test_extract_api_provider_swallows_unknown_model_but_logs_unexpected_errors():
    ...
    with patch.object(
        derouter,
        "get_llm_provider",
        side_effect=derouter.exceptions.BadRequestError(
            message="no provider", model="x", llm_provider="y"
        ),
    ):
        with patch("derouter.integrations.prometheus.verbose_logger") as mock_logger:
            assert extract({"model": "x"}) is None
            mock_logger.debug.assert_not_called()

    # Unexpected error: must be logged and still return None (never raised)
    with patch.object(derouter, "get_llm_provider", side_effect=RuntimeError("boom")):
        with patch("derouter.integrations.prometheus.verbose_logger") as mock_logger:
            assert extract({"model": "x"}) is None
...
def test_requested_model_in_spend_and_requests_metrics():
    ...
    derouter_spend_metric and derouter_requests_metric must expose the
...
def test_route_normalization_for_responses_api():
    ...
    Issue: https://github.com/Delify-Solutions/derouter/issues/XXXX
    ...
    """
    from derouter.proxy.auth.auth_utils import normalize_request_route
    ...
    responses_routes = [
        ...
        ("/v1/responses/derouter_poll_xyz", "/v1/responses/{response_id}"),
    ]
...
def test_prometheus_metrics_use_normalized_routes():
    ...
    labels = prometheus_label_factory(
        supported_enum_labels=prometheus_logger.get_labels_for_metric(
            metric_name="derouter_proxy_total_requests_metric"
        ),
        enum_values=enum_values,
    )
...
def test_prometheus_label_value_sanitization_unicode_paragraph_separator():
    ...
    from derouter.types.integrations.prometheus import _sanitize_prometheus_label_value
...
async def test_success_hook_emits_api_provider_value_on_token_metric():
    ...
    the collected derouter_total_tokens_metric sample actually carries
    ...
    payload = {
        ...
        "hidden_params": {"derouter_overhead_time_ms": None, "additional_headers": None},
    }
    ...
    try:
        ...
        samples = _collected_samples("derouter_total_tokens_metric_total")
        assert samples, "expected derouter_total_tokens_metric to be emitted"
...
async def test_failure_hook_emits_api_provider_value_on_failed_requests_metric():
    ...
    emit derouter_proxy_failed_requests_metric with api_provider="openai". This
    ...
    try:
        ...
        samples = _collected_samples("derouter_proxy_failed_requests_metric_total")
        assert samples, "expected derouter_proxy_failed_requests_metric to be emitted"
