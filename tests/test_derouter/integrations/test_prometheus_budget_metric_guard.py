...
def cleanup_prometheus_registry():
    old_config = derouter.prometheus_metrics_config
    for collector in list(REGISTRY._collector_to_names.keys()):
        try:
            ...
            pass
    yield
    derouter.prometheus_metrics_config = old_config
    for collector in list(REGISTRY._collector_to_names.keys()):
        try:
            REGISTRY.unregister(collector)
        except Exception:
...
def make_logger_with_budget_metrics_disabled() -> PrometheusLogger:
    derouter.prometheus_metrics_config = _BUDGET_EXCLUDED_CONFIG
...
class TestBudgetGaugesAreNoopWhenExcluded:
    def test_team_gauge_is_noop(self):
        ...
        assert isinstance(logger.derouter_remaining_team_budget_metric, NoOpMetric)

    def test_api_key_gauge_is_noop(self):
        logger = make_logger_with_budget_metrics_disabled()
        assert isinstance(logger.derouter_remaining_api_key_budget_metric, NoOpMetric)

    def test_user_gauge_is_noop(self):
        logger = make_logger_with_budget_metrics_disabled()
        assert isinstance(logger.derouter_remaining_user_budget_metric, NoOpMetric)

    def test_org_gauge_is_noop(self):
        logger = make_logger_with_budget_metrics_disabled()
        assert isinstance(logger.derouter_remaining_org_budget_metric, NoOpMetric)

    def test_gauges_are_real_when_all_metrics_enabled(self):
        logger = make_logger_with_all_metrics_enabled()
        assert not isinstance(logger.derouter_remaining_team_budget_metric, NoOpMetric)
        assert not isinstance(logger.derouter_remaining_api_key_budget_metric, NoOpMetric)
        assert not isinstance(logger.derouter_remaining_user_budget_metric, NoOpMetric)
        assert not isinstance(logger.derouter_remaining_org_budget_metric, NoOpMetric)
...
class TestPerEntityGuards:
    ...
    async def test_team_guard_skips_lookup_when_team_gauge_is_noop(self):
        ...
        logger = make_logger_with_all_metrics_enabled()
        logger.derouter_remaining_team_budget_metric = NoOpMetric()
    ...
    async def test_key_guard_skips_lookup_when_key_gauge_is_noop(self):
        ...
        logger = make_logger_with_all_metrics_enabled()
        logger.derouter_remaining_api_key_budget_metric = NoOpMetric()
    ...
    async def test_user_guard_skips_lookup_when_user_gauge_is_noop(self):
        ...
        logger = make_logger_with_all_metrics_enabled()
        logger.derouter_remaining_user_budget_metric = NoOpMetric()
