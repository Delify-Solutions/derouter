...
Run with:
    uv run pytest tests/test_derouter/integrations/test_prometheus_mcp_tool_metrics.py -v
"""
...
def _make_mock_logger():
    ...
    logger.get_labels_for_metric = MagicMock(
        return_value=PrometheusMetricLabels.derouter_mcp_tool_calls_total,
    )
    return logger
...
class TestMCPMetricRegistration:
    ...
    def test_mcp_labels_include_tool_and_server_name(self):
        labels = PrometheusMetricLabels.derouter_mcp_tool_calls_total
    ...
    def test_spend_metric_shares_label_set_with_calls_metric(self):
        assert (
            PrometheusMetricLabels.derouter_mcp_tool_call_spend_metric
            == PrometheusMetricLabels.derouter_mcp_tool_calls_total
        )
        assert (
            PrometheusMetricLabels.derouter_mcp_tool_call_spend_metric
            is not PrometheusMetricLabels.derouter_mcp_tool_calls_total
        )
...
class TestIncrementMCPToolCallMetrics:
    def test_increments_calls_counter_when_mcp_metadata_present(self):
        ...
        logger.derouter_mcp_tool_calls_total.labels.assert_called_once()
        logger.derouter_mcp_tool_calls_total.labels().inc.assert_called_once_with(1.0)
    ...
    def test_increments_spend_counter_when_cost_positive(self):
        ...
        logger.derouter_mcp_tool_call_spend_metric.labels.assert_called_once()
        logger.derouter_mcp_tool_call_spend_metric.labels().inc.assert_called_once_with(0.01)
    ...
    def test_skips_spend_counter_when_cost_zero(self):
        ...
        logger.derouter_mcp_tool_calls_total.labels.assert_called_once()
        logger.derouter_mcp_tool_call_spend_metric.labels.assert_not_called()
    ...
    def test_label_values_carry_tool_and_server_name(self):
        ...
        labels_passed = logger.derouter_mcp_tool_calls_total.labels.call_args
