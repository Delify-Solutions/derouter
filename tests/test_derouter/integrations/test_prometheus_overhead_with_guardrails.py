"""
Unit tests for derouter_overhead_with_guardrails_latency_metric.

The metric reports total internal latency DeRouter adds around the provider
call = SDK overhead (derouter_overhead_time_ms) + pre/post-call guardrail
...
def test_overhead_with_guardrails_recorded_when_only_guardrails_no_sdk_overhead(monkeypatch):
    ...
    logger.derouter_overhead_with_guardrails_latency_metric = mock_metric

    payload = StandardLoggingPayload(
        hidden_params={},  # no derouter_overhead_time_ms
        guardrail_information=[
...
def test_overhead_with_guardrails_recorded_when_sdk_overhead_is_zero(monkeypatch):
    ...
    payload = StandardLoggingPayload(
        hidden_params={"derouter_overhead_time_ms": 0.0},
        guardrail_information=[
            {"guardrail_mode": GuardrailEventHooks.pre_call, "duration": 0.1}
        ],
    )
...
def test_overhead_with_guardrails_metric_is_registered():
    ...
    logger = PrometheusLogger()
    assert logger.derouter_overhead_with_guardrails_latency_metric is not None

    registered = [
        name
        for name in REGISTRY._names_to_collectors
        if name.startswith("derouter_overhead_with_guardrails_latency_metric")
    ]
    assert registered, "derouter_overhead_with_guardrails_latency_metric not registered"
