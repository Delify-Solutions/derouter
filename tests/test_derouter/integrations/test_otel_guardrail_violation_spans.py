...
def _bedrock_block_response():
    ...
    Mirrors the shape in ``derouter/types/proxy/guardrails/guardrail_hooks/
...
def _kwargs_with_guardrail(
    *,
    ...
    consumes. ``derouter_params.metadata`` is the SAME dict that the proxy's
...
class TestGuardrailSpanOnViolation(unittest.TestCase):
    ...
    derouter_request span must both appear with the correct status."""

    def test_handle_failure_creates_derouter_request_and_guardrail_spans(self):
        ...
        self.assertEqual(
            len(derouter_spans),
            1,
            "Expected exactly one derouter_request span on guardrail block",
        )
        ...
        # Guardrail span must be a child of the derouter_request span
        ...
        self.assertEqual(
            ...
            derouter_spans[0].context.span_id,
        )
    ...
    def test_post_call_failure_hook_emits_span_when_caller_sends_metadata(self):
        """On routes that seed ``derouter_metadata`` the guardrail entry lives there,
    ...
    def test_handle_failure_and_post_call_failure_hook_dedupe(self):
        ...
        request_data['metadata'] and kwargs['derouter_params']['metadata']
