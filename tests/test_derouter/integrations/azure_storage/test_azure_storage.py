...
from derouter.integrations.azure_storage.azure_storage import (
    AzureBlobStorageLogger,
    _cached_credential_chain_token_provider,
)
from derouter.types.secret_managers.get_azure_ad_token_provider import AzureCredentialType
...
async def test_async_upload_payload_to_azure_blob_storage(mock_env_vars):
    ...
    """
    with (
        patch("derouter.integrations.azure_storage.azure_storage.get_async_httpx_client") as mock_get_client,
        patch("derouter.integrations.azure_storage.azure_storage.get_azure_ad_token_from_entra_id") as mock_get_token,
    ):
...
async def test_async_upload_payload_uses_configured_endpoint_suffix(mock_gov_env_vars):
    ...
    """
    with patch("derouter.integrations.azure_storage.azure_storage.get_async_httpx_client") as mock_get_client:
        mock_http_client = AsyncMock()
        mock_response = MagicMock()
        mock_http_client.put.return_value = mock_response
        mock_http_client.patch.return_value = mock_response
...
async def test_upload_authenticates_through_the_credential_chain_under_workload_identity(
    ...
    with patch(  # test-quality-ok: REST client is created inside the method; assert emitted request headers
        "derouter.integrations.azure_storage.azure_storage.get_async_httpx_client"
    ) as mock_get_client:
        mock_http_client = AsyncMock()
...
def test_default_chain_provider_is_storage_scoped_and_built_once_per_process():
    ...
    with (
        patch(  # test-quality-ok: assert the default factory's fixed scope and credential type without constructing Azure SDK credentials
            "derouter.integrations.azure_storage.azure_storage.get_azure_ad_token_provider",
...
async def test_client_secret_auth_still_uses_the_storage_scoped_service_principal(mock_env_vars):
    ...
    with (
        patch(  # test-quality-ok: assert the storage scope passed to the shared token factory without making an external auth call
            "derouter.integrations.azure_storage.azure_storage.get_azure_ad_token_from_entra_id",
