...
Strategy
  ...
  - success path  -> ``log_success_event`` -> derouter_request +
...
def _server_span(otel):
    ...
    return otel.create_derouter_proxy_request_started_span(
...
def _success_kwargs(call_type, server_span, with_guardrail=True):
    ...
    return {
        ...
        "derouter_params": {
            "metadata": {
                "derouter_parent_otel_span": server_span,
                "user_api_key_team_id": TEAM_ID,
                "user_api_key_team_alias": TEAM_ALIAS,
            }
        },
...
# ---------------------------------------------------------------------------
# LLM success cells: derouter_request + raw_gen_ai_request + guardrail spans
# ---------------------------------------------------------------------------
...
class TestLLMFailureCells(unittest.TestCase):
    ...
    def _assert_all(self, spans, where):
        for name in ("Failed Proxy Server Request", DEROUTER_PROXY_REQUEST_SPAN_NAME):
