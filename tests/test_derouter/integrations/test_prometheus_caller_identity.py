...
def reset_prometheus_settings(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(derouter, "prometheus_deployment_and_latency_caller_identity", "api_key_alias")
    monkeypatch.setattr(derouter, "prometheus_metrics_config", None)
    monkeypatch.setattr(derouter, "prometheus_exclude_metrics", None)
    monkeypatch.setattr(derouter, "prometheus_exclude_labels", None)
...
def _set_caller_identity(monkeypatch: pytest.MonkeyPatch, mode: str) -> None:
    monkeypatch.setattr(derouter, "prometheus_deployment_and_latency_caller_identity", mode)
...
def test_repeated_label_resolution_does_not_mutate_class_level_or_shared_lists(
    ...
    total_request_labels = PrometheusMetricLabels.derouter_deployment_total_requests
    success_labels = PrometheusMetricLabels.derouter_deployment_success_responses
    ...
    assert PrometheusMetricLabels.derouter_deployment_total_requests is total_request_labels
    assert PrometheusMetricLabels.derouter_deployment_success_responses is success_labels
    assert success_labels is total_request_labels
...
def test_label_resolution_rejects_non_string_class_labels(monkeypatch: pytest.MonkeyPatch):
    ...
    with pytest.raises(TypeError, match=r"Prometheus labels .* must be strings"):
        PrometheusMetricLabels.get_labels("derouter_deployment_total_requests")
...
def test_include_labels_validation_matches_caller_identity_mode(
    ...
    assert logger.get_labels_for_metric("derouter_deployment_total_requests") == include_labels
...
def test_exclude_labels_can_remove_supported_identity_labels(
    ...
    _set_caller_identity(monkeypatch, mode)
    monkeypatch.setattr(derouter, "prometheus_exclude_labels", exclude_labels)

    logger = PrometheusLogger()
    labels = logger.get_labels_for_metric("derouter_deployment_total_requests")
...
def test_non_target_metric_label_schema_is_unchanged(monkeypatch: pytest.MonkeyPatch, mode: str):
    baseline = list(PrometheusMetricLabels.derouter_overhead_with_guardrails_latency_metric)
    _set_caller_identity(monkeypatch, mode)

    actual = PrometheusMetricLabels.get_labels("derouter_overhead_with_guardrails_latency_metric")
...
def test_successful_request_emits_configured_identity_on_real_counter_and_histogram_samples(
    ...
    enum_values = UserAPIKeyLabelValues(
        ...
        derouter_model_name="deployment-model",
        model_id="deployment-id",
        api_base="https://api.example.com",
        api_provider="openai",
    ...
    sample_names = (
        "derouter_deployment_total_requests_total",
        "derouter_deployment_success_responses_total",
        "derouter_request_total_latency_metric_count",
        "derouter_llm_api_latency_metric_count",
        "derouter_llm_api_time_to_first_token_metric_count",
        "derouter_request_queue_time_seconds_count",
        "derouter_overhead_latency_metric_count",
        "derouter_deployment_latency_per_output_token_count",
    )
...
def test_deployment_failure_email_fallbacks_reach_both_real_counters(
    ...
    scrape = generate_latest(REGISTRY).decode()
    for sample_name in (
        "derouter_deployment_failure_responses_total",
        "derouter_deployment_total_requests_total",
    ):
        samples = _sample_labels(scrape, sample_name)
        assert len(samples) == 1, sample_name
...
async def test_proxy_config_loads_caller_identity_before_initializing_callbacks(tmp_path: Path):
    ...
    def capture_mode(*args: object, **kwargs: object) -> None:
        observed_modes.append(derouter.prometheus_deployment_and_latency_caller_identity)

    with patch(  # test-quality-ok: callback interception verifies schema selection before construction
        "derouter.proxy.proxy_server.initialize_callbacks_on_proxy", side_effect=capture_mode
    ):
        await ProxyConfig().load_config(router=None, config_file_path=str(config_path))

    assert observed_modes == ["both"]
    assert derouter.prometheus_deployment_and_latency_caller_identity == "both"
...
def test_validate_mode_returns_each_accepted_value_and_defaults_to_api_key_alias(
    ...
    monkeypatch.delattr(derouter, "prometheus_deployment_and_latency_caller_identity")
...
def test_validate_mode_rejects_invalid_values_and_names_accepted_ones(
    ...
    monkeypatch.setattr(derouter, "prometheus_deployment_and_latency_caller_identity", invalid_mode)
...
def test_validate_caller_identity_settings_stores_each_valid_mode(mode: str):
    ...
    assert derouter.prometheus_deployment_and_latency_caller_identity == mode
...
def test_user_email_mode_conflict_error_names_every_conflicting_metric_and_only_those():
    metrics_config = [
        {
            "group": "non_target",
            "metrics": ["derouter_overhead_with_guardrails_latency_metric"],
            "include_labels": ["api_key_alias"],
        },
        {
            "group": "target_pair",
            "metrics": ["derouter_deployment_total_requests", "derouter_llm_api_latency_metric"],
            "include_labels": ["api_key_alias"],
        },
        {
            "group": "target_single",
            "metrics": ["derouter_request_queue_time_seconds"],
            "include_labels": ["api_key_alias"],
        },
    ]
    ...
    assert "derouter_overhead_with_guardrails_latency_metric" not in message
    assert "prometheus_deployment_and_latency_caller_identity" in message
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            "user_email",
            [{"group": "g", "metrics": ["derouter_deployment_total_requests"], "include_labels": None}],
        ),
...
def _write_proxy_config(tmp_path: Path, derouter_settings: dict[str, object]) -> Path:
                ...
                "model_list": [
                    {
                        ...
                        "derouter_params": {"model": "openai/gpt-4", "api_key": "test-key"},
                    }
                ],
...
async def test_proxy_config_fails_boot_before_callbacks_on_invalid_caller_identity_config(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    derouter_settings: dict[str, object],
):
    from derouter.proxy.proxy_server import ProxyConfig
    ...
    config_path = _write_proxy_config(tmp_path, derouter_settings)
...
def test_failed_init_leaves_registry_clean_so_a_corrected_retry_succeeds(
    ...
    logger = PrometheusLogger()
    assert "user_email" in logger.get_labels_for_metric("derouter_deployment_total_requests")
...
def test_label_validation_error_names_mode_setting_for_identity_labels_on_target_metric(
    ...
    error = LabelValidationError(
        metric_name="derouter_deployment_total_requests",
