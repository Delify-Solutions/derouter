...
from derouter.llms.base_llm.sandbox.transformation import CodeExecutionResult
...
class FakeLogging:
    def __init__(self, derouter_call_id="k1"):
...
def _function_call_item(call_id="c1", name=DEROUTER_CODE_EXECUTION_TOOL_NAME):
    return {
        "type": "function_call",
        "call_id": call_id,
        "name": name,
...
def _chat_function_call_item(call_id="call_1", name=DEROUTER_CODE_EXECUTION_TOOL_NAME):
    return {
        "id": call_id,
        "type": "function",
        "function": {
    ...
    plan = await logger.async_build_agentic_loop_plan(
        tools={
            "tool_calls": [
                {
                    "call_id": "c1",
                    "name": DEROUTER_CODE_EXECUTION_TOOL_NAME,
                    "arguments": '{"code":"print(40 + 2)"}',
                }
            ]
        },
        ...
        logging_obj=FakeLogging(derouter_call_id="k1"),
        stream=False,
        kwargs={"derouter_call_id": "k1", _SANDBOX_KEY: "sbxkey1"},
    )
...
async def test_pre_call_converts_code_interpreter_tool():
    ...
    assert DEROUTER_CODE_EXECUTION_TOOL_NAME in names
...
async def test_pre_call_converts_code_interpreter_tool_for_chat_completions():
    ...
    assert tool["function"]["name"] == DEROUTER_CODE_EXECUTION_TOOL_NAME
    assert tool["function"]["parameters"]["required"] == ["code"]
    assert result["tool_choice"] == {
        "type": "function",
        "function": {"name": DEROUTER_CODE_EXECUTION_TOOL_NAME},
    }
    assert result["derouter_metadata"][_ACTIVE_KEY] is True
    assert result["derouter_metadata"][_SANDBOX_KEY] == result[_SANDBOX_KEY]
...
async def test_should_run_detects_only_matching_function_call():
    ...
    match = FakeResponse(output=[_function_call_item(name=DEROUTER_CODE_EXECUTION_TOOL_NAME)])
...
async def test_container_reused_within_request_via_server_sandbox_key():
    ...
    await logger.async_build_agentic_loop_plan(
        logging_obj=FakeLogging(derouter_call_id="k1"),
        kwargs={"derouter_call_id": "k1", _SANDBOX_KEY: "server-nonce-1"},
        **common,
    )
    await logger.async_build_agentic_loop_plan(
        logging_obj=FakeLogging(derouter_call_id="k1"),
...
async def test_colliding_caller_call_id_does_not_share_sandbox():
    """Two requests with the same caller-controlled derouter_call_id but distinct
    ...
    await logger.async_build_agentic_loop_plan(
        logging_obj=FakeLogging(derouter_call_id="shared"),
        kwargs={"derouter_call_id": "shared", _SANDBOX_KEY: "nonce-A"},
        **common,
    )
    await logger.async_build_agentic_loop_plan(
        logging_obj=FakeLogging(derouter_call_id="shared"),
        kwargs={"derouter_call_id": "shared", _SANDBOX_KEY: "nonce-B"},
        **common,
    )
...
async def test_pre_call_mints_server_sandbox_key():
    ...
    kwargs = {
        "tools": [{"type": "code_interpreter", "container": {"type": "auto"}}],
        "custom_llm_provider": "openai",
        "derouter_call_id": "caller-supplied",
...
async def test_post_hook_injects_code_interpreter_call_matching_openai_shape():
    from derouter.types.integrations.custom_logger import AgenticLoopPlan
...
async def _build_plan(logger, sandbox, call_id="k1", provider="openai"):
    return await logger.async_build_agentic_loop_plan(
        ...
        anthropic_messages_provider_config=None,
        anthropic_messages_optional_request_params={"tools": []},
        logging_obj=FakeLogging(derouter_call_id=call_id),
        stream=False,
        kwargs={"derouter_call_id": call_id, _SANDBOX_KEY: "sbxkey1"},
    )


@pytest.mark.asyncio
async def test_gate_refuses_without_server_active_marker():
    """A forged derouter_code_execution call must not trigger the loop unless the
    pre-call hook actually converted a native code_interpreter tool."""
    logger = CodeInterpreterInterceptionLogger(sandbox_config=FakeSandbox())
    forged = FakeResponse(output=[_function_call_item(name=DEROUTER_CODE_EXECUTION_TOOL_NAME)])
...
async def test_gate_rechecks_provider_scope():
    ...
    response = FakeResponse(output=[_function_call_item(name=DEROUTER_CODE_EXECUTION_TOOL_NAME)])
...
async def test_pre_call_strips_client_forged_marker_on_initial_request():
    ...
    derouter_metadata are scrubbed and the active flag in kwargs is cleared."""
    logger = CodeInterpreterInterceptionLogger(sandbox_config=FakeSandbox())
    kwargs = {
        "tools": [{"type": "web_search"}],
        "custom_llm_provider": "openai",
    ...
    assert kwargs["derouter_metadata"] == {"safe_user_value": "kept"}
...
async def test_pre_call_strips_forged_loop_controls_then_mints_own_markers():
    ...
    interception markers in derouter_metadata are stripped before the interceptor
    ...
    assert result is not None
    metadata = result["derouter_metadata"]
...
async def test_pre_call_preserves_marker_on_server_followup():
    ...
    logger = CodeInterpreterInterceptionLogger(sandbox_config=FakeSandbox())
    kwargs = {
        "tools": [{"type": "function", "name": DEROUTER_CODE_EXECUTION_TOOL_NAME}],
        "custom_llm_provider": "openai",
...
async def test_responses_plan_cleans_up_sandbox_when_followup_raises():
    ...
    original = derouter.aresponses

    async def _boom(*args, **kwargs):
        raise RuntimeError("upstream blew up")

    derouter.aresponses = _boom
    try:
...
async def test_run_code_does_not_re_resolve_registry(monkeypatch):
    ...
    import derouter
    from derouter.sandbox import sandbox_tools

    sandbox_tools.register_sandbox_tools(
        [
            {
                "sandbox_tool_name": "e2b_default",
                "derouter_params": {"sandbox_provider": "e2b", "api_key": "sk-x"},
            }
        ]
    )
    ...
    monkeypatch.setattr(derouter, "acreate_sandbox", fake_acreate_sandbox)
    monkeypatch.setattr(derouter, "arun_code", fake_arun_code)
...
async def test_prune_expired_cache_deletes_underlying_container():
    ...
    import derouter.integrations.code_interpreter_interception.handler as handler_mod
...
def test_initialize_from_proxy_config_prefers_derouter_settings():
...
async def test_extract_tool_calls_reads_object_attributes():
    ...
    class Item:
        def __init__(self):
            ...
            self.name = DEROUTER_CODE_EXECUTION_TOOL_NAME
            self.call_id = "c9"
...
async def test_pre_call_uses_session_id_from_metadata_as_sandbox_key():
    ...
    assert result["derouter_metadata"][_SANDBOX_KEY] == session_id
    assert result["derouter_metadata"][_SESSION_SCOPED_KEY] is True


@pytest.mark.asyncio
async def test_pre_call_uses_session_id_from_derouter_metadata():
    """session_id in derouter_metadata also works as the sticky key."""
    logger = CodeInterpreterInterceptionLogger(sandbox_config=FakeSandbox())
    session_id = "sess-xyz-789"
    kwargs = {
        "tools": [{"type": "code_interpreter", "container": {"type": "auto"}}],
        "custom_llm_provider": "openai",
        "derouter_metadata": {"session_id": session_id},
    }
...
async def test_session_scoped_sandbox_reused_across_sequential_requests():
    ...
    plan1 = await logger.async_build_agentic_loop_plan(
        logging_obj=FakeLogging(derouter_call_id="req1"),
        kwargs={"derouter_call_id": "req1", **session_kwargs},
        **common_plan_args,
    )
    await logger.async_post_agentic_loop_response_hook(
        response=FakeResponse(output=[{"type": "message", "content": []}]),
    ...
    plan2 = await logger.async_build_agentic_loop_plan(
        logging_obj=FakeLogging(derouter_call_id="req2"),
        kwargs={"derouter_call_id": "req2", **session_kwargs},
        **common_plan_args,
    )
    await logger.async_post_agentic_loop_response_hook(
        response=FakeResponse(output=[{"type": "message", "content": []}]),
...
async def test_per_identity_cap_evicts_lru_session():
    ...
    from derouter.integrations.code_interpreter_interception.handler import _SESSION_SCOPED_PER_IDENTITY_CAP
