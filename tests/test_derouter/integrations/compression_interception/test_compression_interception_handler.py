...
async def test_pre_call_hook_compresses_messages_and_injects_tool(monkeypatch):
        ...
        "tools": [
            {
                "type": "function",
                "function": {
                    "name": "derouter_content_retrieve",
                    "parameters": {
                        "type": "object",
    ...
    # The handler does ``from derouter.compression import compress`` at module
    # scope, so we must patch the binding on the handler module — patching
    # ``derouter.compress`` has no effect on the already-bound reference.
    monkeypatch.setattr(
    ...
    assert "derouter_content_retrieve" in tool_names
    assert result["derouter_call_id"] in logger._compression_cache_by_call_id
...
async def test_pre_call_hook_below_trigger_does_not_inject_empty_tools(monkeypatch):
    ...
    assert result.get("derouter_call_id") not in logger._compression_cache_by_call_id
...
async def test_build_agentic_loop_plan_returns_request_patch():
    ...
    logging_obj.derouter_call_id = call_id
    ...
    plan = await logger.async_build_agentic_loop_plan(
        ...
        anthropic_messages_optional_request_params={
            "max_tokens": 1024,
            "tools": [{"name": "derouter_content_retrieve"}],
        },
...
async def test_should_run_agentic_loop_with_custom_type_tools():
    ...
    derouter_content_retrieve as a custom-typed tool (e.g. Claude Code tool list)
    and the model response includes a matching tool_use block."""
    logger = CompressionInterceptionLogger()

    # Exact tools payload produced by Claude Code – derouter_content_retrieve is
...
async def test_build_agentic_loop_plan_missing_key_fallback():
    ...
    logging_obj = MagicMock()
    logging_obj.derouter_call_id = "missing_call"
...
@pytest.mark.asyncio
async def test_pre_call_hook_records_compression_savings_in_derouter_metadata(monkeypatch):
    """
    When compression fires, the hook must record tokens_before/after/saved into
    the request's derouter_metadata IN PLACE (the proxy and logging object hold
    ...
    derouter_metadata = {"user_api_key": "hashed-key", "user_api_key_user_id": "u1"}
    kwargs = {
    ...
    assert result is not None
    assert result["derouter_metadata"] is derouter_metadata
    assert derouter_metadata["compression_savings"] == {
        "tokens_before": 12000,
        "tokens_after": 5000,
        "tokens_saved": 7000,
        "source": "compression_interception",
    }
    assert derouter_metadata["user_api_key"] == "hashed-key"


@pytest.mark.asyncio
async def test_pre_call_hook_creates_derouter_metadata_when_absent(monkeypatch):
    """SDK-direct calls have no derouter_metadata dict yet; the hook creates it."""
    logger = CompressionInterceptionLogger()
    monkeypatch.setattr(
        "derouter.integrations.compression_interception.handler.compress",
    ...
    assert result is not None
    assert result["derouter_metadata"]["compression_savings"]["tokens_saved"] == 200
...
async def test_pre_call_hook_invalid_token_counts_fail_open(monkeypatch, original_tokens, compressed_tokens):
    ...
    derouter_metadata = {"user_api_key": "hashed-key"}
    kwargs = {
        "model": "claude-sonnet-5",
        "messages": [{"role": "user", "content": "ctx"}],
        "derouter_metadata": derouter_metadata,
    ...
    assert result is not None
    assert "compression_savings" not in derouter_metadata
