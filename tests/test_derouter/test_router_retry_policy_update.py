...
from derouter.router_utils.pre_call_checks.model_rate_limit_check import ModelRateLimitingCheck
from derouter.router_utils.pre_call_checks.prompt_caching_deployment_check import PromptCachingDeploymentCheck
from derouter.types.router import RetryPolicy, UpdateRouterConfig


@pytest.fixture(autouse=True)
def isolate_derouter_callbacks():
    callbacks_before: Final = derouter.callbacks.copy()
    yield
    derouter.callbacks = callbacks_before  # test-quality-ok: required callback-state restoration fixture
...
def test_update_router_config_accepts_retry_policy_payload():
    ...
    that is what /config/update writes to the DeRouter_Config row."""
    payload = {
        "retry_policy": {
...
def _build_router() -> derouter.Router:
...
def test_update_settings_clears_omitted_toggleable_pre_call_checks():
    ...
    assert not any(isinstance(callback, PromptCachingDeploymentCheck) for callback in derouter.callbacks)
...
def test_remove_optional_pre_call_check_removes_local_and_global_callbacks():
    ...
    assert not any(type(callback) is PromptCachingDeploymentCheck for callback in derouter.callbacks)
...
def test_remove_optional_pre_call_check_keeps_global_callback_for_another_router():
    ...
    assert any(type(callback) is PromptCachingDeploymentCheck for callback in derouter.callbacks)
...
# triggers — ``POST /config/update`` writes the DeRouter_Config row,
...
class _FakeConfigTable:
    """Stand-in for ``prisma_client.db.derouter_config``.
...
async def test_config_update_persists_and_reads_back_retry_policy(monkeypatch):
    ...
    from derouter.proxy._types import ConfigYAML, LitellmUserRoles, UserAPIKeyAuth
    ...
    prisma_client.db.derouter_config = fake_table
