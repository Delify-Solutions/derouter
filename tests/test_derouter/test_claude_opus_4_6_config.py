...
def test_claude_4_6_australia_region_uses_au_prefix_not_apac():
    ...
    assert (
        "au.anthropic.claude-opus-4-6-v1" in derouter.bedrock_converse_models
    ), "au.anthropic.claude-opus-4-6-v1 not registered in bedrock_converse_models"

    # Verify apac. is NOT registered for this model
    assert (
        "apac.anthropic.claude-opus-4-6-v1" not in derouter.bedrock_converse_models
    ), "apac.anthropic.claude-opus-4-6-v1 should not be in bedrock_converse_models"

    # Verify the au. model is registered in bedrock_converse_models
    assert (
        "au.anthropic.claude-sonnet-4-6" in derouter.bedrock_converse_models
    ), "au.anthropic.claude-sonnet-4-6 not registered in bedrock_converse_models"

    # Verify apac. is NOT registered for this model
    assert (
        "apac.anthropic.claude-sonnet-4-6" not in derouter.bedrock_converse_models
...
def test_opus_4_6_bedrock_converse_registration():
    assert "anthropic.claude-opus-4-6-v1" in derouter.BEDROCK_CONVERSE_MODELS
    assert "global.anthropic.claude-opus-4-6-v1" in derouter.bedrock_converse_models
    assert "us.anthropic.claude-opus-4-6-v1" in derouter.bedrock_converse_models
    assert "eu.anthropic.claude-opus-4-6-v1" in derouter.bedrock_converse_models
    assert "au.anthropic.claude-opus-4-6-v1" in derouter.bedrock_converse_models
