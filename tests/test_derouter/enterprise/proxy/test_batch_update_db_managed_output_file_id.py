...
def _build_prisma_mock(db_batch_object=None):
    mock = MagicMock()
    mock.db.derouter_managedfiletable.find_first = AsyncMock(return_value=None)
    mock.db.derouter_managedobjecttable.find_first = AsyncMock(return_value=db_batch_object)
    mock.db.derouter_managedobjecttable.update = AsyncMock()
    return mock
...
async def test_update_batch_in_database_stores_unified_output_file_id():
    ...
    unified_batch_id = (
        "derouter_proxy;model_id:my-model;llm_batch_id:batch_managed_ids_test"
    )
    ...
    stored = json.loads(
        mock_prisma.db.derouter_managedobjecttable.update.call_args.kwargs["data"][
            "file_object"
        ]
    )
...
async def test_cancel_path_registers_output_file_under_batch_owner():
    ...
    await update_batch_in_database(
        batch_id="batch_managed_ids_test",
        unified_batch_id="derouter_proxy;model_id:my-model;llm_batch_id:batch_managed_ids_test",
        response=response,
        managed_files_obj=mock_managed_files,
        prisma_client=mock_prisma,
        verbose_proxy_logger=MagicMock(),
...
async def test_update_batch_skips_lookup_when_db_batch_object_supplied():
    ...
    mock_prisma.db.derouter_managedobjecttable.find_first.assert_not_called()
    forwarded_auth = mock_managed_files.store_unified_file_id.call_args.kwargs[
        "user_api_key_dict"
    ]
...
async def test_update_batch_derives_model_id_from_unified_batch_id():
    ...
    await update_batch_in_database(
        batch_id="batch_managed_ids_test",
        unified_batch_id="derouter_proxy;model_id:model-from-batch-id;llm_batch_id:batch_managed_ids_test",
        response=response,
        managed_files_obj=mock_managed_files,
        prisma_client=mock_prisma,
        verbose_proxy_logger=MagicMock(),
...
async def test_ensure_batch_response_resolves_model_name_from_unified_file_id():
    ...
    response = _build_batch_response(
        ...
        hidden_params={
            "model_id": "my-model",
            "unified_file_id": "derouter_proxy:application/octet-stream;unified_id,abc;target_model_names,gpt-4o-mini,gemini-2.0-flash",
        },
    )
...
def _in_memory_managed_files():
    """Build a real _PROXY_DeRouterManagedFiles whose prisma upsert hits an in-memory row."""
    ...
    prisma = MagicMock()
    prisma.db.derouter_managedobjecttable = table
    ...
    return (
        _PROXY_DeRouterManagedFiles(internal_usage_cache=cache, prisma_client=prisma),
        store,
    )
...
async def test_store_unified_object_id_resolves_org_through_the_cached_team():
    ...
    await user_api_key_cache.async_set_cache(
        ...
        value=DeRouter_TeamTableCachedObj(team_id="team-cached", organization_id="org-via-team"),
    ...
    instance.prisma_client.db.derouter_teamtable.find_unique.assert_not_awaited()
...
async def test_store_unified_object_id_resolves_org_from_the_db_when_the_team_is_not_cached():
    ...
    instance, store = _in_memory_managed_files()
    instance.prisma_client.db.derouter_teamtable.find_unique = AsyncMock(
        return_value=DeRouter_TeamTable(team_id="team-uncached", organization_id="org-via-db")
    )
...
async def test_store_unified_object_id_attribution_columns_are_write_once():
    ...
    upsert_data = instance.prisma_client.db.derouter_managedobjecttable.upsert.call_args.kwargs["data"]
...
async def test_store_unified_object_id_omits_unset_columns():
    ...
    create_data = instance.prisma_client.db.derouter_managedobjecttable.upsert.call_args.kwargs["data"]["create"]
