"""
Tests for enterprise/derouter_enterprise/proxy/hooks/managed_files.py
...
from derouter.proxy.openai_files_endpoints.common_utils import BATCH_CREATE_HIDDEN_PARAM
from derouter.types.llms.openai import FileListPage, OpenAIFileObject
...
def _make_batch_response(
    ...
    """Create a DeRouterBatch response with hidden params set as the router would."""
    batch = DeRouterBatch(
        id=batch_id,
...
def _make_managed_files_over_rows(rows):
    ...
    managed_files.prisma_client.db.derouter_managedfiletable = table
    return managed_files, table


def _make_managed_files_instance():
    """Create a _PROXY_DeRouterManagedFiles with storage methods mocked out."""
    ...
    instance = _PROXY_DeRouterManagedFiles(
...
async def test_should_pass_credentials_to_afile_retrieve():
    ...
    with (
        patch("derouter.afile_retrieve", mock_afile_retrieve),
...
async def test_get_user_created_file_ids_skips_rows_without_file_object():
    managed_files = _make_managed_files_instance()
    managed_files.prisma_client.db.derouter_managedfiletable.find_many = AsyncMock(
        return_value=[
            MagicMock(
...
async def test_get_user_created_file_ids_remaps_stored_raw_provider_id_to_unified_id():
    ...
    Regression test for https://github.com/Delify-Solutions/derouter/issues/35362.
    """
...
async def test_afile_list_returns_owner_scoped_managed_files():
    ...
    managed_files.prisma_client.db.derouter_managedfiletable.find_many.assert_awaited_once_with(
...
async def test_should_skip_non_file_unified_id_on_output_file_id():
    ...
    batch_unified = (
        base64.urlsafe_b64encode(
            b"derouter_proxy;model_id:openai/openai/gpt-5.5-batch;llm_batch_id:batch_abc"
        )
        .decode()
        .rstrip("=")
    )
    ...
    with patch("derouter.afile_retrieve", AsyncMock()) as mock_afile_retrieve:
        await managed_files.async_post_call_success_hook(
            data={},
            user_api_key_dict=user_api_key_dict,
            response=batch_response,
...
async def test_afile_content_passes_trusted_model_credentials_to_router():
    ...
    trusted_credentials = call_kwargs["_derouter_internal_model_credentials"]
...
async def test_afile_content_error_reports_unified_id_not_provider_uri():
    ...
    managed_files = _make_managed_files_instance()
    unified_file_id = "derouter_proxy_unified_id_abc"
    s3_uri = "s3://my-bucket/derouter-batch-outputs/job-123/input.jsonl.out"
    managed_files.get_model_file_id_mapping = AsyncMock(
        return_value={unified_file_id: {"model-123": s3_uri}}
    )
    ...
    with pytest.raises(Exception, match='DeRouter Managed File object with') as exc_info:
        await managed_files.afile_content(
            file_id=unified_file_id,
            derouter_parent_otel_span=None,
            llm_router=mock_router,
...
def _make_real_managed_files_instance():
    """Create a _PROXY_DeRouterManagedFiles with a real store_unified_file_id but
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_managedfiletable.upsert = AsyncMock()
    mock_prisma.db.derouter_managedfiletable.create = AsyncMock(
...
def _make_object_store_instance():
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_managedobjecttable.upsert = AsyncMock()
    mock_prisma.db.derouter_managedobjecttable.update_many = AsyncMock()
...
async def test_poll_refreshes_batch_state_without_claiming_the_row():
    ...
    mock_prisma.db.derouter_managedobjecttable.upsert.assert_not_awaited()
    update_many = mock_prisma.db.derouter_managedobjecttable.update_many
...
async def test_create_still_upserts_and_claims_attribution():
    ...
    mock_prisma.db.derouter_managedobjecttable.update_many.assert_not_awaited()
    upsert = mock_prisma.db.derouter_managedobjecttable.upsert
...
async def test_default_callers_still_create_their_rows():
    ...
    mock_prisma.db.derouter_managedobjecttable.upsert.assert_awaited_once()
    mock_prisma.db.derouter_managedobjecttable.update_many.assert_not_awaited()
...
async def test_store_unified_file_id_is_idempotent_via_upsert():
    ...
    file_id = "derouter_proxy_unified_output_id_abc"
    ...
    mock_prisma.db.derouter_managedfiletable.create.assert_not_awaited()
    upsert_mock = mock_prisma.db.derouter_managedfiletable.upsert
...
async def test_concurrent_first_registrations_converge_on_one_row():
    ...
    upserted_row_keys = {
        ...
        for upsert_call in mock_prisma.db.derouter_managedfiletable.upsert.await_args_list
    }
...
def _b64_unified_input_file_id(target_model_names: str) -> str:
    unified_input_file_id = (
        "derouter_proxy:application/octet-stream;unified_id,input-uuid;"
...
async def test_hook_mint_prefers_input_file_target_model_names():
    ...
    with (
        patch("derouter.afile_retrieve", AsyncMock(return_value=_make_file_object())),
        patch("derouter.proxy.proxy_server.llm_router", mock_router),
    ):
        await managed_files.async_post_call_success_hook(
            data={},
...
async def test_cost_job_and_retrieve_paths_mint_identical_unified_output_file_ids():
    ...
    managed_files, mock_prisma = _make_real_managed_files_instance()
    mock_prisma.db.derouter_managedfiletable.find_first = AsyncMock(return_value=None)
    unified_input_file_id = _b64_unified_input_file_id("model-a")

    retrieve_response = DeRouterBatch(
        id="batch-123",
        completion_window="24h",
        created_at=1700000000,
        endpoint="/v1/chat/completions",
...
async def test_batch_create_hook_persists_creating_key_and_tags():
    ...
    await managed_files.async_post_call_success_hook(
        data={"derouter_metadata": {"tags": ["env:prod", "team:ml"], "user_api_key": creator.api_key}},
        user_api_key_dict=creator,
...
async def test_batch_retrieve_hook_does_not_claim_attribution():
    ...
    await managed_files.async_post_call_success_hook(
        data={"derouter_metadata": {"tags": ["poller:tag"]}},
        user_api_key_dict=UserAPIKeyAuth(api_key="sk-the-poller", user_id="bob", parent_otel_span=None),
        response=retrieve_response,
    )
