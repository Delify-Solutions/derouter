...
def _make_unified_file_id(file_id: str = "file-abc123") -> str:
    ...
    raw = f"derouter_proxy:application/json;unified_id,test-{file_id};target_model_names,azure-gpt-4;llm_output_file_id,{file_id};llm_output_file_model_id,model-123"
    return base64.urlsafe_b64encode(raw.encode()).decode().rstrip("=")


def _make_unified_batch_id(batch_id: str = "batch-123") -> str:
    """Create a base64-encoded unified batch ID."""
    raw = f"derouter_proxy;model_id:model-deploy-xyz;llm_batch_id:{batch_id};llm_output_file_id:file-output"
    return base64.urlsafe_b64encode(raw.encode()).decode().rstrip("=")
...
def _make_managed_files_instance_with_batches(
    ...
    """
    Create a _PROXY_DeRouterManagedFiles instance with mocked DB and batches.
    ...
    mock_prisma.db.derouter_managedfiletable.delete = AsyncMock(
        return_value=mock_file_record
    )
...
def test_is_batch_polling_enabled_when_job_registered():
    ...
    with patch("derouter.proxy.proxy_server.scheduler", mock_scheduler):
...
def test_is_batch_polling_disabled_when_no_scheduler():
    ...
    with patch("derouter.proxy.proxy_server.scheduler", None):
        assert instance._is_batch_polling_enabled() is False
