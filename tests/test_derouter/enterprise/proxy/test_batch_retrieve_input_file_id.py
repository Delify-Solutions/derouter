...
input_file_id instead of the DeRouter unified ID.
"""
...
DECODED_UNIFIED_INPUT_FILE_ID = "derouter_proxy:application/octet-stream;unified_id,test-uuid;target_model_names,azure-gpt-4"
...
DECODED_UNIFIED_BATCH_ID = "derouter_proxy;model_id:model-xyz;llm_batch_id:batch-123"
...
async def test_should_resolve_raw_input_file_id_to_unified():
    ...
    mock_prisma.db.derouter_managedobjecttable.find_first = AsyncMock(return_value=mock_db_object)
    mock_prisma.db.derouter_managedfiletable.find_first = AsyncMock(return_value=mock_managed_file)

    from derouter.proxy.openai_files_endpoints.common_utils import get_batch_from_database
