...
from derouter_enterprise.enterprise_callbacks.send_emails.endpoints import (
    _get_email_settings,
...
def mock_prisma_client():
    ...
    mock_db.derouter_config = mock_config
...
async def test_get_email_settings_empty_db(mock_prisma_client):
    ...
    mock_prisma_client.db.derouter_config.find_unique = mock_find_unique
...
async def test_save_email_settings_new_entry(mock_prisma_client):
    ...
    mock_prisma_client.db.derouter_config.upsert = mock_upsert
...
async def test_get_email_event_settings(mock_prisma_client, mock_user_api_key_auth):
    ...
    # Setup mocks
    with mock.patch("derouter.proxy.proxy_server.prisma_client", mock_prisma_client):
        with mock.patch(
            "derouter_enterprise.enterprise_callbacks.send_emails.endpoints._get_email_settings",
...
async def test_update_event_settings(mock_prisma_client, mock_user_api_key_auth):
    ...
    with mock.patch("derouter.proxy.proxy_server.prisma_client", mock_prisma_client):
        with mock.patch(
            ...
            with mock.patch(
                "derouter_enterprise.enterprise_callbacks.send_emails.endpoints._save_email_settings",
...
async def test_endpoint_with_no_prisma_client(mock_user_api_key_auth):
    ...
    with mock.patch("derouter.proxy.proxy_server.prisma_client", None):
        # Test get endpoint
