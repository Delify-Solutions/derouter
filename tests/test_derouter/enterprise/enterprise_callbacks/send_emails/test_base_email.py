...
from derouter.integrations.email_templates.email_footer import EMAIL_FOOTER
from derouter.proxy._types import CallInfo, Litellm_EntityType, WebhookEvent
from derouter.constants import EMAIL_BUDGET_ALERT_TTL
...
async def test_send_key_created_email(
    ...
    with mock.patch.dict(
        os.environ,
        {
            "EMAIL_LOGO_URL": "https://derouter-listing.s3.amazonaws.com/derouter_logo.png",
        ...
        assert call_args["from_email"] == BaseEmailLogger.DEFAULT_DEROUTER_EMAIL
        assert call_args["to_email"] == ["test@example.com"]
        assert call_args["subject"] == "DeRouter: Test Key Created"
...
async def test_send_user_invitation_email(
    ...
    with mock.patch.dict(
        ...
        assert call_args["subject"] == "DeRouter: User Invitation"
...
async def test_send_key_rotated_email(
    ...
    with mock.patch.dict(
        ...
        assert call_args["from_email"] == BaseEmailLogger.DEFAULT_DEROUTER_EMAIL
        assert call_args["to_email"] == ["test@example.com"]
        assert call_args["subject"] == "DeRouter: API Key Rotated"
...
async def test_get_invitation_link(base_email_logger):
    ...
    mock_prisma.db.derouter_invitationlink.find_many = mock_find_many

    with mock.patch("derouter.proxy.proxy_server.prisma_client", mock_prisma):
        ...
        mock_prisma.db.derouter_invitationlink.find_many = mock_find_many_empty
        result = await base_email_logger._get_invitation_link(
            user_id="test-user", base_url="http://test.com"
        )
        assert result == "http://test.com"
...
async def test_get_invitation_link_uses_existing_when_available(base_email_logger):
    ...
    mock_prisma.db.derouter_invitationlink.find_many = mock_find_many_existing
...
async def test_get_invitation_link_creates_new_when_list_is_none(base_email_logger):
    ...
    mock_prisma.db.derouter_invitationlink.find_many = mock_find_many_none
...
async def test_get_email_params_user_invitation(
    ...
    with mock.patch.dict(
        ...
        with mock.patch.object(
            ...
            assert (
                result.logo_url
                == "https://derouter-listing.s3.amazonaws.com/derouter_logo.png"
            )
...
async def test_get_email_params_non_premium_user(mock_env_vars):
    ...
    with patch("derouter.proxy.proxy_server.premium_user", False):
        ...
        assert email_params.subject == "DeRouter: New User Invitation"
        ...
        assert key_params.subject == "DeRouter: API Key Created"
...
async def test_get_email_params_default_templates(monkeypatch):
    ...
    with patch("derouter.proxy.proxy_server.premium_user", True):
        ...
        assert invitation_params.subject == "DeRouter: New User Invitation"
...
async def test_send_soft_budget_alert_email(
    ...
    with mock.patch.dict(
        ...
        assert (
            call_args["subject"]
            == "DeRouter: Soft Budget Crossed - Total Soft Budget: $100.0"
        )
...
async def test_get_email_params_soft_budget_crossed(
    ...
    with mock.patch.dict(
        ...
        assert (
            result.subject == "DeRouter: Soft Budget Crossed - Total Soft Budget: $100.0"
        )
        assert result.recipient_email == "test@example.com"
...
async def test_send_soft_budget_alert_email_uses_custom_signature(
    ...
    with mock.patch.dict(
        ...
        assert "The DeRouter team" not in html_body
