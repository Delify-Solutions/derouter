...
def test_add_fallback_headers_serializes_fallback_errors():
    ...
    assert result is response
    assert response._hidden_params["additional_headers"][
        "x-derouter-attempted-fallbacks"
    ] == 1
    assert (
        json.loads(
            response._hidden_params["additional_headers"]["x-derouter-fallback-errors"]
        )
        == fallback_errors
    )
...
def test_add_retry_headers_to_streaming_wrapper():
    ...
    assert response._hidden_params["additional_headers"] == {
        "x-existing": "keep",
        "x-derouter-attempted-retries": 2,
        "x-derouter-max-retries": 3,
    }
...
def test_add_fallback_headers_when_no_existing_additional_headers():
    ...
    assert result is response
    assert response._hidden_params["additional_headers"]["x-derouter-attempted-fallbacks"] == 2
...
def test_get_fallback_errors_from_headers_existing_list_passthrough():
    ...
    result = get_fallback_errors_from_headers({"x-derouter-fallback-errors": errors})
    assert result == errors


def test_get_fallback_errors_from_headers_invalid_json_returns_empty():
    result = get_fallback_errors_from_headers(
        {"x-derouter-fallback-errors": "not-valid-json-{"}
    )
    assert result == []
