...
import derouter
from derouter.proxy.health_check import run_with_timeout
...
def _make_model(model_id: str, model_name: str = "gpt-4") -> dict:
    return {
        ...
        "derouter_params": {"model": model_name, "api_key": "fake-key"},
...
class TestAhealthCheckExceptionPreservation:
    ...
    async def test_run_with_timeout_returns_timeout_exception(self):
        """run_with_timeout should return a derouter.Timeout in the 'exception' key on timeout."""
        import asyncio
        ...
        assert isinstance(result["exception"], derouter.Timeout)
...
class TestHealthCheckEndpointExceptionPropagation:
    ...
    async def test_raw_exception_from_gather_in_map(self):
        ...
        raw_exc = derouter.RateLimitError(
        ...
        with patch(
            "derouter.proxy.health_check._run_model_health_check",
            side_effect=raw_exc,
        ):
            healthy, unhealthy, exc_map = await _perform_health_check(model_list)
...
class TestGetAllowedFailsFromPolicyWithHealthCheckExceptions:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (derouter.Timeout, "TimeoutErrorAllowedFails", 5),
            (derouter.AuthenticationError, "AuthenticationErrorAllowedFails", 3),
            (derouter.RateLimitError, "RateLimitErrorAllowedFails", 10),
            (
                derouter.ContentPolicyViolationError,
                "ContentPolicyViolationErrorAllowedFails",
                2,
            ),
            (derouter.BadRequestError, "BadRequestErrorAllowedFails", 7),
        ],
    )
...
class TestHealthCheckCooldownIntegration:
    ...
    def test_health_check_failure_triggers_cooldown_at_threshold(self):
        ...
        auth_exc = derouter.AuthenticationError(
...
class TestWriteHealthStateIntegration:
    ...
    def test_unhealthy_endpoint_triggers_set_cooldown(self):
        ...
        import derouter.proxy.proxy_server as proxy_module
        from derouter.proxy.proxy_server import _write_health_state_to_router_cache
        ...
        with patch.object(proxy_module, "llm_router", router):
            with patch(
                "derouter.router_utils.cooldown_handlers._set_cooldown_deployments"
            ) as mock_cooldown:
    ...
    def test_unhealthy_endpoint_increments_failure_counter(self):
        ...
        rate_exc = derouter.RateLimitError(
        ...
        with patch.object(proxy_module, "llm_router", router):
            with patch(
                "derouter.router_utils.router_callbacks.track_deployment_metrics.increment_deployment_failures_for_current_minute"
            ) as mock_increment:
                with patch(
                    "derouter.router_utils.cooldown_handlers._set_cooldown_deployments"
                ):
