...
import derouter
from derouter.router_utils.get_retry_from_policy import get_num_retries_from_retry_policy
from derouter.types.router import RetryPolicy

_EXCEPTION_FOR_FIELD: Final = MappingProxyType(
    {
        "BadRequestErrorRetries": derouter.BadRequestError,
        "AuthenticationErrorRetries": derouter.AuthenticationError,
        "TimeoutErrorRetries": derouter.Timeout,
        "RateLimitErrorRetries": derouter.RateLimitError,
        "ContentPolicyViolationErrorRetries": derouter.ContentPolicyViolationError,
        "InternalServerErrorRetries": derouter.InternalServerError,
        "ServiceUnavailableErrorRetries": derouter.ServiceUnavailableError,
    }
)
...
def test_subclass_prefers_its_own_field_over_the_parent_field():
    ...
    assert (
        get_num_retries_from_retry_policy(exception=_error(derouter.ContentPolicyViolationError), retry_policy=policy)
        == 1
    )
    assert get_num_retries_from_retry_policy(exception=_error(derouter.BadRequestError), retry_policy=policy) == 5
...
@pytest.mark.parametrize("exception_type", (derouter.BadGatewayError, derouter.NotFoundError))
...
def test_specific_field_wins_over_default_retries():
    ...
    assert get_num_retries_from_retry_policy(exception=_error(derouter.RateLimitError), retry_policy=policy) == 3
    assert get_num_retries_from_retry_policy(exception=_error(derouter.BadGatewayError), retry_policy=policy) == 0
...
def test_default_retries_applies_when_the_specific_field_is_unset():
    ...
    assert (
        get_num_retries_from_retry_policy(exception=_error(derouter.ServiceUnavailableError), retry_policy=policy) == 2
    )


def test_empty_policy_matches_nothing():
    assert (
        get_num_retries_from_retry_policy(exception=_error(derouter.ServiceUnavailableError), retry_policy=RetryPolicy())
        is None
    )
    assert (
        get_num_retries_from_retry_policy(exception=_error(derouter.ServiceUnavailableError), retry_policy=None) is None
    )
...
def test_dict_policy_is_accepted():
    assert (
        get_num_retries_from_retry_policy(
            exception=_error(derouter.ServiceUnavailableError),
...
def test_model_group_policy_replaces_the_global_policy():
    exception: Final = _error(derouter.ServiceUnavailableError)
