...
from derouter.constants import ROUTER_FALLBACK_ERROR_DETAIL_MAX_CHARS
...
class TestFilterTeamBasedModels:
    ...
    def test_filter_team_based_models_team_match_derouter_metadata(
        self, sample_deployments_with_teams
    ):
        """Test filtering when team_id is in derouter_metadata"""
        request_kwargs = {"derouter_metadata": {"user_api_key_team_id": "team-b"}}
        result = filter_team_based_models(sample_deployments_with_teams, request_kwargs)
    ...
    def test_filter_team_based_models_priority_metadata_over_derouter(
        self, sample_deployments_with_teams
    ):
        """Test that metadata.user_api_key_team_id takes priority over derouter_metadata.user_api_key_team_id"""
        request_kwargs = {
            "metadata": {
                "user_api_key_team_id": "team-a",  # This should take priority
                "derouter_metadata": {"user_api_key_team_id": "team-b"},
            }
        }
        result = filter_team_based_models(sample_deployments_with_teams, request_kwargs)

        # Should filter based on team-a (from metadata, not derouter_metadata)
        expected_ids = ["deployment-1", "deployment-3", "deployment-4"]
        result_ids = [d.get("model_info", {}).get("id") for d in result]
        assert sorted(result_ids) == sorted(expected_ids)
...
class TestDeploymentSupportsWebSearch:
    ...
    def test_no_config_defaults_to_true(self):
        ...
        deployment = {"derouter_params": {"model": "gpt-4"}, "model_info": {"id": "123"}}
        assert _deployment_supports_web_search(deployment) is True
...
def test_filter_deployments_by_model_access_groups_access_group_only_key():
    ...
    router = Router(
        model_list=[
            {
                "model_name": "gpt-5",
                "derouter_params": {"model": "openai/gpt-5.1", "api_key": "key-1"},
                "model_info": {"access_groups": ["AG1"]},
            },
            {
                "model_name": "gpt-5",
                "derouter_params": {"model": "openai/gpt-4o", "api_key": "key-2"},
...
class TestAddModelFileIdMappings:
    ...
    def test_should_extract_model_info_id_when_given_single_deployment_dict(self):
        ...
        deployment's own keys (`model_name`, `derouter_params`, `model_info`).
        """
        deployment = {
            "model_name": "gpt-4",
            "derouter_params": {"model": "gpt-4", "api_key": "sk-test"},
    ...
    def test_should_handle_batch_model_when_id_matches_model_name(self):
        ...
        `DeRouter_ManagedFileTable` Pydantic validation passes.
        """
        deployment = {
        ...
        # Bug case would have produced keys ["model_name", "derouter_params",
        # "model_info"] with non-string values.
        assert result == {"openai/openai/gpt-5.5-batch": "file-batch-1"}
        assert "derouter_params" not in result
...
class TestWarnOnProviderCredentialMismatch:
    ...
    another is silently broken: derouter ignores the credentials and sends the
    ...
    def test_warns_when_aws_params_sit_on_an_anthropic_model(self):
        warning = warn_on_provider_credential_mismatch(
            ...
            derouter_params={"model": "claude-sonnet-5", "aws_region_name": "eu-central-1"},
        )
    ...
    def test_silent_when_no_provider_scoped_credentials_are_set(self):
        assert (
            warn_on_provider_credential_mismatch(
                model_name="gpt-5.5", derouter_params={"model": "gpt-5.5"}
            )
            is None
        )
    ...
    def test_vertex_params_name_vertex_not_bedrock(self):
        ...
        warning = warn_on_provider_credential_mismatch(
            ...
            derouter_params={"model": "claude-sonnet-5", "vertex_project": "my-project"},
        )
    ...
    def test_silent_for_a_model_derouter_cannot_classify(self):
        ...
        assert (
            warn_on_provider_credential_mismatch(
                model_name="mystery",
                derouter_params={"model": "not-a-real-provider-model-xyz", "aws_region_name": "us-east-1"},
            )
            is None
        )
    ...
    def test_silent_for_every_aws_family_route(self, model):
        ...
        assert (
            warn_on_provider_credential_mismatch(
                ...
                derouter_params={"model": model, "aws_region_name": "us-east-1"},
            )
            is None
        )
    ...
    def test_silent_when_credentials_come_from_a_named_credential(self):
        ...
        assert (
            warn_on_provider_credential_mismatch(
                ...
                derouter_params={
                    "model": "bedrock/invoke/us.anthropic.claude-haiku-4-5-20251001-v1:0",
                    "derouter_credential_name": "my-aws-creds",
                },
            )
            is None
        )
