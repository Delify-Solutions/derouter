...
def test_strategy_router_dependencies(derouter_params, expected):
    found = strategy_router_dependencies(derouter_params)
...
def test_complexity_default_model_param_wins_over_the_config_field():
    """ComplexityRouter overwrites config.default_model with the derouter_params one, so the
...
def test_is_complexity_router_model(model: str | None, expected: bool) -> None:
    from derouter.router_utils.auto_router_model_naming import is_complexity_router_model
...
def test_gated_capability_of(derouter_params: Mapping[str, object], expected_key: str | None) -> None:
    ...
    """
    capability = gated_capability_of(derouter_params)
...
def test_count_capability_routers_counts_only_its_own_capability(capability) -> None:
    ...
    def row(name: str, config: Mapping[str, object] | None) -> Mapping[str, object]:
        ...
        return {"model_name": name, "derouter_params": params}
    ...
    rows: list[Mapping[str, object]] = [
        row("a", mine_first),
        row("b", theirs),
        {"model_name": "c", "derouter_params": {"model": "openai/gpt-4o"}},
        row("d", mine_second),
        {"model_name": "e"},
        {"model_name": "f", "derouter_params": "not a mapping"},
    ]
