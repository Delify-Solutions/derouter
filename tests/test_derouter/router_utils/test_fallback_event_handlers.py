...
import derouter
from derouter.router_utils.cooldown_handlers import mark_advisor_orchestration_failure
...
async def test_run_async_fallback_adds_errors_when_opted_in():
    ...
    assert additional_headers["x-derouter-attempted-fallbacks"] == 1
    assert json.loads(additional_headers["x-derouter-fallback-errors"]) == [
        {
...
async def test_run_async_fallback_omits_errors_without_opt_in():
    ...
    additional_headers = response._hidden_params["additional_headers"]
    assert additional_headers["x-derouter-attempted-fallbacks"] == 1
    assert "x-derouter-fallback-errors" not in additional_headers
...
async def test_run_async_fallback_raises_when_all_fallbacks_fail():
    with pytest.raises(RuntimeError, match="fallback model also failed"):
        await run_async_fallback(
            derouter_router=AlwaysFailRouter(),
            fallback_model_group=["fallback-model"],
            original_model_group="primary-model",
...
async def test_run_async_fallback_skips_original_model_group():
    ...
    assert response._hidden_params["additional_headers"]["x-derouter-attempted-fallbacks"] == 1
...
async def test_run_async_fallback_records_batch_model_group_outside_provider_metadata():
    ...
    batch, so the router's own model_group belongs in derouter_metadata."""
    router = AttemptRecordingRouter()
    ...
    await run_async_fallback(
        ...
        derouter_metadata={"model_group": "openai-group"},
        original_function=_acreate_batch,
    )

    assert router.received_kwargs["metadata"] == {"caller": "nightly-job"}
    assert router.received_kwargs["derouter_metadata"]["model_group"] == "openai-group"
...
class TestTriggerCooldownForFailedDeployment:
    def test_calls_set_cooldown_deployments_with_stamped_deployment_id(self):
        ...
        exc = derouter.RateLimitError("Rate limit", "openai", "gpt-4")
        exc.failed_deployment_id = "fallback-deployment"

        with patch("derouter.router_utils.fallback_event_handlers._set_cooldown_deployments") as mock_set_cooldown:
            _trigger_cooldown_for_failed_deployment(derouter_router=mock_router, kwargs={}, exception=exc)
    ...
    def test_does_not_trust_caller_supplied_metadata_bucket(self):
        ...
        with patch("derouter.router_utils.fallback_event_handlers._set_cooldown_deployments") as mock_set_cooldown:
            _trigger_cooldown_for_failed_deployment(derouter_router=mock_router, kwargs=kwargs, exception=exc)
    ...
    def test_increments_failure_counter_before_cooldown_check(self):
        ...
        with (
            patch("derouter.router_utils.fallback_event_handlers._set_cooldown_deployments") as mock_set_cooldown,
            patch(
                "derouter.router_utils.fallback_event_handlers.increment_deployment_failures_for_current_minute"
            ) as mock_increment,
        ):
            _trigger_cooldown_for_failed_deployment(derouter_router=mock_router, kwargs={}, exception=exc)

            mock_increment.assert_called_once_with(
                derouter_router_instance=mock_router, deployment_id="fallback-deployment"
            )
            mock_set_cooldown.assert_called_once()
    ...
    def test_no_op_when_deployment_id_missing(self):
        ...
        with patch("derouter.router_utils.fallback_event_handlers._set_cooldown_deployments") as mock_set_cooldown:
            _trigger_cooldown_for_failed_deployment(
                derouter_router=mock_router, kwargs={}, exception=RuntimeError("no metadata")
            )
    ...
    def test_uses_deployment_derouter_params_cooldown_time_override(self):
        mock_router = MagicMock()
        mock_router.cooldown_time = 300.0
        mock_router.get_model_info.return_value = {"derouter_params": {"cooldown_time": 30.0}}
    ...
    def test_uses_response_header_when_no_deployment_config(self):
        ...
        mock_router = MagicMock()
        mock_router.cooldown_time = 60.0
        mock_router.get_model_info.return_value = {"derouter_params": {}}

        exc = RuntimeError("upstream error")
        exc.failed_deployment_id = "fallback-deployment"
        exc.derouter_response_headers = httpx.Headers({"retry-after": "45"})
    ...
    def test_silently_catches_exceptions(self):
        ...
        with patch(
            "derouter.router_utils.fallback_event_handlers._set_cooldown_deployments",
    ...
    def test_skips_request_scoped_404_on_generic_api_call(self):
        ...
        exc = derouter.NotFoundError("not found", "openai", "gpt-4")
        exc.failed_deployment_id = "fallback-deployment"
    ...
    def test_skips_client_side_timeout_408(self):
        """The proxy's x-derouter-timeout header lets a caller set an arbitrarily short
        timeout, which derouter.Timeout reports as status 408 regardless of the
        ...
        exc = derouter.Timeout(message="timeout", model="gpt-4", llm_provider="openai")
        exc.failed_deployment_id = "fallback-deployment"
    ...
    def test_still_cools_down_408_without_client_side_timeout_flag(self):
        ...
        408 that did not come from x-derouter-timeout (no client_side_timeout in kwargs)
...
class TestRunAsyncFallbackTriggersCooldown:
    ...
    async def test_triggers_cooldown_when_has_logged_async_failure_is_true(self):
        with patch(
            "derouter.router_utils.fallback_event_handlers._trigger_cooldown_for_failed_deployment"
        ) as mock_trigger:
            with pytest.raises(RuntimeError, match="fallback model also failed"):
                await run_async_fallback(
                    derouter_router=self.RouterWithLoggingKwarg(),
                    fallback_model_group=["fallback-model"],
                    original_model_group="primary-model",
                    original_exception=RuntimeError("original request failed"),
                    max_fallbacks=3,
                    fallback_depth=0,
                    derouter_logging_obj=self._logging_obj(has_logged_async_failure=True),
                )
        ...
        with patch(
            ...
            with pytest.raises(RuntimeError, match="fallback model also failed"):
                await run_async_fallback(
                    ...
                    max_fallbacks=3,
                    fallback_depth=0,
                    derouter_logging_obj=self._logging_obj(has_logged_async_failure=False),
                )
...
class TestPreRoutingSelectionCarriesToFallbacks:
    ...
    def test_selection_is_recorded_in_the_derouter_metadata_bucket(self):
        kwargs = {"model": "smart-router", "derouter_metadata": {}}
...
class TestPreRoutingSelectionIsPerHop:
    ...
    def test_both_buckets_are_cleared(self):
        kwargs = {
            ...
            "derouter_metadata": {"pre_routing_selected_model": "tier2"},
        }
...
class TestOrderedFallbackLookupGroups:
    def test_tier_first_then_requested_group_deduped(self):
        ...
        kwargs = {"derouter_metadata": {PRE_ROUTING_SELECTED_MODEL_KEY: "tier1"}}
