...
def _router(
    ...
    *,
    ...
    derouter_params: dict[str, object] = {"model": model}
    if config is not None:
        derouter_params["complexity_router_config"] = dict(config)
    if tags is not None:
        derouter_params["tags"] = tags
    row: dict[str, object] = {"model_name": name, "derouter_params": derouter_params}
...
class TestTuningFingerprint:
    ...
    def test_tier_model_overrides_change_the_fingerprint(self) -> None:
        ...
        with_override = tuning_fingerprint(
            {"tiers": {"SIMPLE": {"model_name": "x", "derouter_params": {"temperature": 0.1}}}}
        )
    ...
    def test_a_release_changing_a_shipped_default_is_not_an_operator_edit(self, monkeypatch) -> None:
        ...
        import derouter.router_strategy.complexity_router.config as config_module
        ...
        assert tuning_fingerprint(pinned["derouter_params"]["complexity_router_config"]) != DEFAULT_TUNING_FINGERPRINT
...
class TestRouterIdentity:
    def test_db_rows_key_on_model_id_and_yaml_rows_on_name_and_tags(self) -> None:
        ...
        assert router_identity({"derouter_params": {"model": "auto_router/complexity_router"}}) is None
