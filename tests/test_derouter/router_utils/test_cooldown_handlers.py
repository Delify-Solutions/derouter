...
class TestResolveAllowedFailsFromPolicy:
    ...
    def test_content_policy_violation_not_shadowed_by_bad_request_error(self):
        ...
        exc = derouter.ContentPolicyViolationError("flagged content", "openai", "gpt-4")
        assert _resolve_allowed_fails_from_policy(policy, exc) == 2
...
class TestShouldCooldownBasedOnDeploymentPolicy:
    ...
    def test_policy_match_uses_exception_type_as_cache_key_suffix(self):
        ...
        exc = derouter.RateLimitError("429", "openai", "gpt-4")
        router = self._make_router({"derouter_params": {}, "model_info": {}})

        with patch("derouter.router_utils.cooldown_handlers.should_cooldown_based_on_allowed_fails_policy") as mock_sc:
    ...
    def test_partial_policy_without_dep_allowed_fails_defers_for_uncovered_exception(self):
        ...
        policy = {"RateLimitErrorAllowedFails": 0}
        exc = derouter.Timeout("timed out", "openai", "gpt-4")
        router = self._make_router({"derouter_params": {}, "model_info": {}})
    ...
    def test_cooldown_time_from_model_info_passed_through(self):
        exc = derouter.RateLimitError("429", "openai", "gpt-4")
        router = self._make_router({"derouter_params": {}, "model_info": {"cooldown_time": 120.0}})
    ...
    def test_cooldown_time_from_derouter_params_used_as_fallback(self):
        ...
        set it."""
        exc = derouter.RateLimitError("429", "openai", "gpt-4")
        router = self._make_router({"derouter_params": {"cooldown_time": 120.0}, "model_info": {}})
    ...
    def test_cooldown_time_from_model_info_takes_priority_over_derouter_params(self):
        exc = derouter.RateLimitError("429", "openai", "gpt-4")
        router = self._make_router({"derouter_params": {"cooldown_time": 120.0}, "model_info": {"cooldown_time": 15.0}})
...
class TestShouldCooldownBasedOnAllowedFailsPolicy:
    ...
    def test_fail_counter_is_shared_across_router_instances(self):
        ...
        exc = derouter.AuthenticationError("401", "openai", "gpt-4")

        results = [
            should_cooldown_based_on_allowed_fails_policy(
                derouter_router_instance=workers[i % 2],
                deployment="dep-1",
                original_exception=exc,
                allowed_fails_override=5,
            )
...
class TestRoutingGroupCooldownAlternatives:
    ...
    def test_group_call_429_cools_down_member_with_alternatives(self):
        from derouter.router_utils.cooldown_handlers import _should_cooldown_deployment
