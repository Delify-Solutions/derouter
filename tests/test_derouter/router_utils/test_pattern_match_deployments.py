"""Behavior pins for ``derouter/router_utils/pattern_match_deployments.py``."""
...
from derouter.router_utils.pattern_match_deployments import PatternMatchRouter, PatternUtils


def _wildcard_deployment(model_name: str) -> dict:
    return {"model_name": model_name, "derouter_params": {"model": model_name}}


def _matched_models(matches: list[dict] | None) -> list[str]:
    return [deployment["derouter_params"]["model"] for deployment in matches or []]
...
def test_route_never_sorts_and_the_most_specific_pattern_still_wins_after_registry_changes():
    ...
    router.add_pattern("openai/gpt-*", {"model_name": "openai/gpt-*", "derouter_params": {"model": "azure/gpt-*"}})
