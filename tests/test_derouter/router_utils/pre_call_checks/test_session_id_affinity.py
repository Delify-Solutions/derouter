    ...
    healthy_deployments = [
        {
            ...
            "derouter_params": {"model": "model_1"},
            "model_info": {"id": "deployment-1"},
        },
        {
            "model_name": "model_group",
            "derouter_params": {"model": "model_2"},
...
async def test_proxy_generated_session_id_does_not_pin_a_deployment():
    ...
    healthy_deployments = [
        {"model_name": "model_group", "derouter_params": {"model": "model_1"}, "model_info": {"id": "deployment-1"}},
        {"model_name": "model_group", "derouter_params": {"model": "model_2"}, "model_info": {"id": "deployment-2"}},
    ]
...
async def _one_turn(router, model, session_id, key_hash):
    ...
    with (
        ...
        response = await router.aresponses(
            ...
            derouter_metadata={"session_id": session_id, "user_api_key_hash": key_hash},
        )
...
async def test_pre_routing_hook_stamps_or_clears_the_marker_per_attempt(
    ...
    try:
        request_kwargs = {
            ...
            "derouter_metadata": {SESSION_DEPLOYMENT_AFFINITY_TTL_METADATA_KEY: 111},
        }
        ...
        if expect_marker:
            assert request_kwargs["derouter_metadata"][SESSION_DEPLOYMENT_AFFINITY_TTL_METADATA_KEY] == 777
        else:
            assert SESSION_DEPLOYMENT_AFFINITY_TTL_METADATA_KEY not in request_kwargs["metadata"]
            assert SESSION_DEPLOYMENT_AFFINITY_TTL_METADATA_KEY not in request_kwargs["derouter_metadata"]
    finally:
        _cleanup_router_callbacks(router)
