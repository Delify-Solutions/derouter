...
from derouter.utils import get_prompt_cache_min_tokens, is_prompt_caching_valid_prompt, token_counter
...
async def test_affinity_key_matches_the_messages_auto_caching_actually_sends(monkeypatch, local_model_cost_map):
    ...
    `derouter.acompletion`, which runs after routing, so at filter time the messages carried no
...
async def test_repeated_auto_cached_prefix_pins_to_one_deployment(monkeypatch, local_model_cost_map):
    ...
    router = derouter.Router(
        model_list=[
            {
                "model_name": MODEL_GROUP_ALIAS,
                "derouter_params": {"model": AUTO_CACHING_MODEL, "api_key": "sk-fake"},
...
async def test_tool_marked_cache_control_keeps_routing_off_another_requests_prefix(monkeypatch, local_model_cost_map):
    ...
    will not carry derouter's breakpoints. Routing must see the tools as well. Ignore them and it
...
async def test_wildcard_route_resolves_underlying_model_minimum(local_model_cost_map):
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "anthropic/*", "api_key": "sk-fake"},
    ...
    assert deployments[0]["derouter_params"]["model"] == "anthropic/claude-opus-4-6"
