...
1. **Items with IDs**: item IDs for output items with `encrypted_content` are rewritten to
   `encitem_{base64("derouter:model_id:{model_id};item_id:{original_id}")}`.

2. **Items without IDs** (Codex): encrypted_content itself is wrapped with model_id metadata:
   `derouter_enc:{base64("model_id:{model_id}")};{original_encrypted_content}`.
...
class TestEncryptedContentWrapping:
    def test_wrap_and_unwrap_encrypted_content(self):
        ...
        assert wrapped.startswith("derouter_enc:")
...
async def test_encrypted_content_affinity_tracks_and_routes():
    ...
    with (
        patch(
            "derouter.llms.custom_httpx.llm_http_handler.BaseLLMHTTPHandler.async_response_api_handler",
            new_callable=AsyncMock,
            return_value=mock_resp,
        ),
        patch(
            "derouter.router_strategy.simple_shuffle.random.choice",
...
@pytest.mark.asyncio
async def test_encrypted_content_affinity_does_not_create_derouter_metadata_for_chat():
    """
    For chat completions / embeddings, request_kwargs uses 'metadata' (not
    'derouter_metadata').  The affinity check must NOT create a spurious
    'derouter_metadata' key, because that would cause
    _get_metadata_variable_name_from_kwargs to return 'derouter_metadata'
    and tag-based routing would look for tags in the wrong dict.
    """
    check = EncryptedContentAffinityCheck()
    deployments = [
        {"model_info": {"id": "dep-1"}, "derouter_params": {"model": "gpt-4"}},
    ]
    ...
    # Must not inject derouter_metadata
    assert "derouter_metadata" not in request_kwargs
...
@pytest.mark.asyncio
async def test_encrypted_content_affinity_preserves_derouter_metadata_for_responses():
    """
    For Responses API calls, derouter_metadata already exists.  The affinity
    check should set the flag there and preserve existing keys.
    """
    check = EncryptedContentAffinityCheck()
    deployments = [
        {"model_info": {"id": "dep-1"}, "derouter_params": {"model": "gpt-5.1-codex"}},
    ]
    request_kwargs = {
        "derouter_metadata": {"model_info": {"id": "dep-1"}},
    }
    ...
    assert (
        request_kwargs["derouter_metadata"]["encrypted_content_affinity_enabled"] is True
    )
    assert request_kwargs["derouter_metadata"]["model_info"] == {"id": "dep-1"}
...
async def test_affinity_falls_back_to_same_boundary_on_alias_switch():
    """
    LIT-2531 alias path: gpt-5.2-codex is a DeRouter alias that points at the
...
def test_boundary_fallback_no_router_ref_returns_empty():
    ...
    healthy = [
        {
            "model_info": {"id": "dep-1"},
            "derouter_params": {"api_base": "https://x", "api_key": "k"},
        }
    ]
...
def test_boundary_key_accepts_pydantic_derouter_params_instance():
    """
    Regression: ``_encryption_boundary_key`` must accept any object exposing
    dict-style ``.get()`` (incl. ``DeRouter_Params`` Pydantic instances) — not
    just plain dicts.

    A stricter ``isinstance(dict)`` guard would silently return ``None`` for a
    ``DeRouter_Params`` value, drop the deployment from boundary matching, and
    ...
    pydantic_params = DeRouter_Params(
        model="azure/gpt-5.3-codex",
...
def _make_originating_mock(api_base: str, api_key: str):
    ...
    originating.derouter_params.model_dump.return_value = {
        "api_base": api_base,
        "api_key": api_key,
    }
    return originating
...
async def test_affinity_raises_service_unavailable_when_origin_cooled_for_non_429():
    ...
    """
    from derouter.exceptions import ServiceUnavailableError
    from derouter.router_utils.pre_call_checks.encrypted_content_affinity_check import (
        EncryptedContentAffinityCheck,
    )
