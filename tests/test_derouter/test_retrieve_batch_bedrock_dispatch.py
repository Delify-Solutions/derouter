"""Cover the Bedrock-ARN dispatch in ``derouter.batches.main.retrieve_batch``.
...
def mock_handlers():
    ...
    We patch at the import site (derouter.batches.main) rather than the
    definition site so the ``BedrockBatchesHandler`` reference inside
    ``retrieve_batch`` resolves to our mocks.
    """
    fake_batch = MagicMock(name="DeRouterBatch")
    with (
        patch(
            "derouter.batches.main.BedrockBatchesHandler._handle_async_invoke_status",
            return_value=fake_batch,
        ) as async_invoke,
        patch(
            "derouter.batches.main.BedrockBatchesHandler._handle_model_invocation_job_status",
            return_value=fake_batch,
        ) as mij,
    ):
...
def test_async_invoke_arn_routes_to_async_invoke_handler(mock_handlers):
    ...
    result = derouter.retrieve_batch(
