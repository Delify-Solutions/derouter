...
from derouter.proxy._types import DeRouter_VerificationTokenView, UserAPIKeyAuth
...
class TestProjectAliasOnTypes:
    ...
    def test_verification_token_view_has_project_alias(self):
        token_view = DeRouter_VerificationTokenView(
            token="test-token",
            project_id="proj-123",
    ...
    def test_verification_token_view_project_alias_defaults_none(self):
        token_view = DeRouter_VerificationTokenView(token="test-token")
        assert token_view.project_alias is None

    def test_user_api_key_auth_inherits_project_alias(self):
        """UserAPIKeyAuth extends DeRouter_VerificationTokenView, so it gets project_alias."""
        auth = UserAPIKeyAuth(
            api_key="sk-test",
            project_id="proj-1",
...
class TestProjectAliasThroughMetadataPipeline:
    ...
    def test_get_sanitized_user_information_includes_project_alias(self):
        ...
        result = DeRouterProxyRequestSetup.get_sanitized_user_information_from_key(
    ...
    def test_end_to_end_project_alias_flow(self):
        ...
        sanitized = DeRouterProxyRequestSetup.get_sanitized_user_information_from_key(
