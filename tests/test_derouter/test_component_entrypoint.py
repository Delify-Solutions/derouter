...
TERRAFORM_ECS = REPO_ROOT / "terraform" / "derouter" / "aws" / "ecs.tf"
TERRAFORM_CLOUDRUN = REPO_ROOT / "terraform" / "derouter" / "gcp" / "cloudrun.tf"
...
def _run_entrypoint(
    ...
    _write_stubs(bin_dir, ("ddtrace-run", "uvicorn", "derouter"))
...
def test_ddtrace_enabled_wraps_the_command_and_disables_the_openai_integration(
    ...
    it before any derouter code runs, so derouter's in-process `patch_all(..., openai=False)`
...
def test_gating_matches_the_monolithic_entrypoint_and_get_secret_bool(
    ...
    assert monolith[0] == ("exec=ddtrace-run" if traced else "exec=derouter")
    assert monolith[2] == expected_openai
    assert monolith[1] == ("args=derouter --port 4000" if traced else "args=--port 4000")
...
def test_build_from_pip_image_launches_derouter_through_the_prod_entrypoint(
    use_ddtrace: str | None, traced: bool, tmp_path: Path
) -> None:
    """Run the build_from_pip image's ENTRYPOINT + CMD through the script it actually COPYs.

    The image used to `ENTRYPOINT ["derouter"]`, so `USE_DDTRACE` was inert there at any spelling.
    ...
    assert len(entrypoint) == 1, (
        f"{BUILD_FROM_PIP_DOCKERFILE} ENTRYPOINT must be the bare script so CMD reaches derouter"
    )
    ...
    cmd_str = " ".join(cmd)
    assert recorded == (
        "exec=ddtrace-run" if traced else "exec=derouter",
        f"args=derouter {cmd_str}" if traced else f"args={cmd_str}",
