...
Run in docker: pytest tests/test_derouter/test_dashscope_image_generation.py -v
"""
...
def test_get_model_info_mode_is_image_generation(
    ...
    prev_env = os.environ.get("DEROUTER_LOCAL_MODEL_COST_MAP")
    prev_model_cost = derouter.model_cost
    try:
        ...
        if prev_env is None:
            ...
            os.environ["DEROUTER_LOCAL_MODEL_COST_MAP"] = prev_env
        derouter.model_cost = prev_model_cost
...
class TestDashScopeImageGenerationConfig:
    ...
    def test_validate_environment_raises_without_key(self):
        with patch(
            "derouter.llms.dashscope.image_generation.transformation.get_secret_str",
            return_value=None,
        ):
...
# ---------------------------------------------------------------------------
# 6. End-to-end flow via derouter.image_generation (HTTP mocked)
# ---------------------------------------------------------------------------
...
def test_derouter_image_generation_dashscope_end_to_end(model: str):
    mock_response_body = {
        "output": {
            "choices": [
                {
