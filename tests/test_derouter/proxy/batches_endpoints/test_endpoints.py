"""
Routing-contract tests for derouter/proxy/batches_endpoints/endpoints.py
...
fails loudly:
  ...
  1. DISPATCH      - exactly which downstream seam fired (derouter.acreate_batch
  ...
  3. SEAM PAYLOAD  - the *entire* kwargs dict forwarded to the provider call is
                     exact-matched. Because DeRouterBatchCreateRequest is a
...
import derouter
import derouter.proxy.batches_endpoints.endpoints as endpoints
...
from derouter.types.llms.openai import BatchJobStatus
from derouter.types.utils import CredentialItem, DeRouterBatch
...
def no_openai_creds(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "api_key", None)
    monkeypatch.setattr(derouter, "openai_key", None)
...
class Harness:
    ...
    def acreate_kwargs(self) -> Dict[str, Any]:
        """Exact kwargs forwarded to derouter.acreate_batch."""
        assert self.derouter_acreate.call_count == 1
        return dict(self.derouter_acreate.call_args.kwargs)
...
def harness():
    ...
    derouter_acreate = AsyncMock(return_value=make_batch())
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(patch.object(derouter, "acreate_batch", derouter_acreate))
        stack.enter_context(patch.object(derouter, "enable_loadbalancing_on_batch_endpoints", False))
        ...
        h = Harness(
            ...
            derouter_acreate=derouter_acreate,
            router=router,
            logging=logging,
...
async def test_create__model_encoded_file_id(harness):
    ...
    # 1. DISPATCH - model-credential path fired via derouter, router did not.
    assert harness.derouter_acreate.call_count == 1
...
async def test_create__model_encoded_file_id__encodes_output_and_error_ids(harness):
    ...
    harness.derouter_acreate.return_value = make_batch(
        id="batch-xyz",
...
async def test_create__model_from_header(harness):
    ...
    await call_create(harness, headers={"x-derouter-model": "vertex-model"})
...
async def test_create__body_model_beats_header_and_query(harness):
    ...
    await call_create(
        harness,
        headers={"x-derouter-model": "vertex-model"},
...
async def test_create__fallback_no_creds_404(harness, no_openai_creds):
    ...
    harness.derouter_acreate.assert_not_called()
    harness.router_acreate.assert_not_called()
...
async def test_create__unified_file_id_single_model_disables_cross_model_fallbacks(harness):
    set_body(
        harness,
        {
            "input_file_id": "derouter_proxy_unified_id",
            "endpoint": "/v1/chat/completions",
            "completion_window": "24h",
    ...
    # DISPATCH - router fired, direct derouter did not.
    ...
    assert resp.input_file_id == "derouter_proxy_unified_id"
...
async def test_create__unified_file_id_resolves_real_storage_url(harness):
    """A base64 unified_file_id is a DeRouter-internal token, not a real
    ...
    up from DeRouter_ManagedFileTable and substituted before dispatch.

    Regression lock on the lookup key: DeRouter_ManagedFileTable.unified_file_id
    ...
    derouter_proxy:... string. Querying with the decoded string never matches
    ...
    fake_db_file = MagicMock(
        storage_url="gs://bucket/derouter-vertex-files/publishers/google/models/gemini-2.0/abc",
    )
    ...
    find_first.assert_awaited_once_with(where={"unified_file_id": "derouter_proxy_unified_id"})
    assert resp.input_file_id == "derouter_proxy_unified_id"
    assert resp._hidden_params["unified_file_id"] == "unified-xyz"
...
async def test_create__unified_file_id_db_error_falls_back_to_raw_id(harness):
    ...
    assert harness.router_kwargs()["input_file_id"] == "derouter_proxy_unified_id"
...
async def test_create__multi_model_unified_file_with_loadbalancing_keeps_router_branch(harness):
    ...
    with (
        patch.object(derouter, "enable_loadbalancing_on_batch_endpoints", True),
        patch.object(endpoints, "_is_base64_encoded_unified_file_id", return_value="unified-xyz"),
...
async def test_create__loadbalancing_routes_to_router(harness):
    ...
    harness.is_known_model.return_value = True
    with patch.object(derouter, "enable_loadbalancing_on_batch_endpoints", True):
        await call_create(harness)
...
@pytest.mark.asyncio
@pytest.mark.parametrize("field", ["metadata", "derouter_metadata"])
...
def install_managed_files_hook(harness: Harness) -> AsyncMock:
    ...
    managed_files = _PROXY_DeRouterManagedFiles(MagicMock(async_set_cache=AsyncMock()), prisma_client=prisma_client)
...
def assert_ownership_registered_for_team_a(prisma_client: AsyncMock, batch_id: str) -> None:
    upsert = prisma_client.db.derouter_managedobjecttable.upsert
...
async def test_create__unified_file_id_registers_ownership_for_creator(harness):
    unified_input_file_id = base64.urlsafe_b64encode(
        b"derouter_proxy:application/octet-stream;unified_id,input-uuid;target_model_names,gpt-4o-mini"
    ).decode()
    set_body(
        harness,
        {
...
async def test_create__exception_calls_failure_hook(harness, openai_env_creds):
    ...
    harness.derouter_acreate.side_effect = ValueError("provider boom")
...
#   (derouter.aretrieve_batch via model creds, llm_router.aretrieve_batch, or   #
#   derouter.aretrieve_batch via env-var provider) and writes the fresh state   #
...
UNIFIED_BATCH_ID = "derouter_proxy;model_id:gpt-4o-mini;llm_batch_id:batch-raw-xyz"
...
class RetrieveHarness:
    ...
    def aretrieve_kwargs(self) -> Dict[str, Any]:
        """Exact kwargs forwarded to derouter.aretrieve_batch."""
        assert self.derouter_aretrieve.call_count == 1
        return dict(self.derouter_aretrieve.call_args.kwargs)
...
def retrieve_harness():
    ...
    derouter_aretrieve = AsyncMock(return_value=make_batch())
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(patch.object(derouter, "aretrieve_batch", derouter_aretrieve))
        stack.enter_context(patch.object(derouter, "enable_loadbalancing_on_batch_endpoints", False))
        stack.enter_context(patch.object(proxy_server, "llm_router", router))
        stack.enter_context(patch.object(proxy_server, "proxy_logging_obj", logging))
        stack.enter_context(patch.object(proxy_server, "general_settings", {}))
        ...
        yield RetrieveHarness(
            ...
            derouter_aretrieve=derouter_aretrieve,
            router=router,
            logging=logging,
            creds_resolver=router.get_deployment_credentials_with_provider,
...
# --------------------------------------------------------------------------- #
# SCENARIO 1 - batch id encoded with model. derouter.aretrieve_batch via the
...
async def test_retrieve__model_encoded_id(retrieve_harness):
    ...
    # 1. DISPATCH - model-credential path fired via derouter, router did not.
    assert retrieve_harness.derouter_aretrieve.call_count == 1
...
async def test_retrieve__model_encoded_id__encodes_output_and_error_ids(
    ...
    retrieve_harness.derouter_aretrieve.return_value = make_batch(
        id="batch-xyz",
        output_file_id="file-out-raw",
        error_file_id="file-err-raw",
    )
...
async def test_retrieve__unified_batch_id_routes_to_router(retrieve_harness):
    ...
    retrieve_harness.derouter_aretrieve.assert_not_called()
...
async def test_retrieve__exception_calls_failure_hook(retrieve_harness, openai_env_creds):
    retrieve_harness.derouter_aretrieve.side_effect = ValueError("provider boom")
...
#     2. model from body/query/header -> derouter.alist_batches + id encode     #
#     3. target_model_names (param or body) -> llm_router.alist_batches        #
#     4. fallback -> derouter.alist_batches via env-var custom_llm_provider     #
#                                                                             #
...
class ListHarness:
    ...
    def alist_kwargs(self) -> Dict[str, Any]:
        assert self.derouter_alist.call_count == 1
        return dict(self.derouter_alist.call_args.kwargs)
...
def list_harness():
    ...
    provider_from_headers = MagicMock(return_value=None)
    provider_from_query = MagicMock(return_value=None)
    derouter_alist = AsyncMock(return_value=FakeListPage([]))
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(patch.object(derouter, "alist_batches", derouter_alist))
        stack.enter_context(patch.object(proxy_server, "llm_router", router))
        stack.enter_context(patch.object(proxy_server, "proxy_logging_obj", logging))
        stack.enter_context(patch.object(proxy_server, "general_settings", {}))
        stack.enter_context(patch.object(proxy_server, "proxy_config", MagicMock()))
...
async def test_list__managed_files_path(list_harness):
    ...
    list_harness.derouter_alist.assert_not_called()
...
async def test_list__model_from_body_routes_and_encodes(list_harness):
    list_harness.derouter_alist.return_value = FakeListPage([make_batch(id="batch-1"), make_batch(id="batch-2")])

    resp = await call_list(list_harness, body={"model": "azure/gpt-4o"})

    assert list_harness.derouter_alist.call_count == 1
    list_harness.router_alist.assert_not_called()
...
async def test_list__exception_calls_failure_hook(list_harness):
    list_harness.derouter_alist.side_effect = ValueError("provider boom")
...
#     1. model-encoded batch id -> derouter.acancel_batch via model creds       #
#     2. unified batch id       -> llm_router.acancel_batch (model+batch_id    #
#                                  parsed out of the unified id)               #
#     3. fallback               -> derouter.acancel_batch via env-var provider  #
...
class CancelHarness:
    ...
    def acancel_kwargs(self) -> Dict[str, Any]:
        assert self.derouter_acancel.call_count == 1
        return dict(self.derouter_acancel.call_args.kwargs)
...
def cancel_harness():
    ...
    pre_call = AsyncMock(side_effect=lambda **kw: (data_holder["data"], MagicMock()))
    # add_derouter_data_to_request is a passthrough that returns the data it got.
    add_data = AsyncMock(side_effect=lambda **kw: kw["data"])
    get_headers = MagicMock(return_value={})
    provider_from_headers = MagicMock(return_value=None)
    provider_from_query = MagicMock(return_value=None)
    derouter_acancel = AsyncMock(return_value=make_batch())
    update_batch_in_db = AsyncMock(return_value=None)
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(patch.object(endpoints, "update_batch_in_database", update_batch_in_db))
        stack.enter_context(patch.object(derouter, "acancel_batch", derouter_acancel))
        stack.enter_context(patch.object(derouter, "enable_loadbalancing_on_batch_endpoints", False))
        stack.enter_context(patch.object(proxy_server, "llm_router", router))
        stack.enter_context(patch.object(proxy_server, "proxy_logging_obj", logging))
        stack.enter_context(patch.object(proxy_server, "general_settings", {}))
        ...
        stack.enter_context(patch.object(proxy_server, "version", "test-version"))
        stack.enter_context(patch.object(proxy_server, "prisma_client", MagicMock()))
        stack.enter_context(patch.object(proxy_server, "add_derouter_data_to_request", add_data))
        ...
        yield CancelHarness(
            ...
            provider_from_headers=provider_from_headers,
            provider_from_query=provider_from_query,
            derouter_acancel=derouter_acancel,
            router=router,
            logging=logging,
            creds_resolver=router.get_deployment_credentials_with_provider,
            update_batch_in_db=update_batch_in_db,
...
# --------------------------------------------------------------------------- #
# SCENARIO 1 - model-encoded batch id -> derouter.acancel_batch via model creds.
# --------------------------------------------------------------------------- #
...
async def test_cancel__model_encoded_id(cancel_harness):
    ...
    # DISPATCH - model-credential path via derouter; router untouched.
    assert cancel_harness.derouter_acancel.call_count == 1
...
async def test_cancel__unified_batch_id_routes_to_router(cancel_harness):
    ...
    # DISPATCH - router fired, derouter did not, no creds lookup.
    assert cancel_harness.router_acancel.call_count == 1
    cancel_harness.derouter_acancel.assert_not_called()
...
async def test_cancel__unified_missing_model_id_400(cancel_harness):
    ...
    with patch.object(
        ...
        return_value="derouter_proxy;llm_batch_id:batch-xyz",
    ):
        with pytest.raises(ProxyException) as exc:
...
async def test_cancel__exception_calls_failure_hook(cancel_harness, openai_env_creds):
    cancel_harness.derouter_acancel.side_effect = ValueError("provider boom")
...
# project because derouter_credential_name never reached the call).
# =========================================================================== #
...
async def test_create__provider_only_resolves_named_vertex_credentials(harness):
    ...
    with patch.object(derouter, "credential_list", [VERTEX_NAMED_CREDENTIAL]):
...
async def test_create__provider_only_ignores_other_provider_deployments(harness):
    ...
    openai_only_router = Router(
        model_list=[
            {
                "model_name": "gpt-4o",
                "derouter_params": {"model": "openai/gpt-4o", "api_key": "sk-global-openai"},
            }
        ]
    )
...
#                                                                              #
# Ownership rows only exist for DeRouter managed ids. A raw provider id sent to #
...
def _unified_batch_id(model_id: str = "azure/gpt-4o", batch_id: str = "batch-provider-id") -> str:
    ...
    unified = SpecialEnums.DEROUTER_MANAGED_BATCH_COMPLETE_STR.value.format(model_id, batch_id)
...
def _unified_file_id() -> str:
    ...
    unified = SpecialEnums.DEROUTER_MANAGED_FILE_COMPLETE_STR.value.format(
...
async def test_create__raw_input_file_id_rejected_when_managed_files_required(harness):
    ...
    with patch.object(derouter, "require_managed_files", True):
        with pytest.raises(ProxyException) as exc:
            await call_create(harness)
...
async def test_create__raw_input_file_id_allowed_when_managed_files_not_required(harness, openai_env_creds):
    ...
    with patch.object(derouter, "require_managed_files", False):
        await call_create(harness)
...
async def test_retrieve__managed_batch_defers_cost_to_the_poller_when_it_is_running(retrieve_harness):
    ...
    metadata = retrieve_harness.router.aretrieve_batch.await_args.kwargs.get("derouter_metadata") or {}
...
async def test_retrieve__raw_batch_id_is_untouched_by_the_poller_handoff(retrieve_harness, openai_env_creds):
    ...
    metadata = retrieve_harness.derouter_aretrieve.await_args.kwargs.get("derouter_metadata") or {}
    assert metadata.get("batch_ignore_default_logging") is None
