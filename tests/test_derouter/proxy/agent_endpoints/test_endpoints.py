...
from derouter.proxy.agent_endpoints import endpoints as agent_endpoints
from derouter.proxy.agent_endpoints.auth.agent_permission_handler import (
    RestrictedAgentAccess,
)
from derouter.proxy.agent_endpoints.endpoints import (
...
def _sample_agent_config() -> dict:
    return {
        ...
        "derouter_params": {"make_public": False},
    }
...
def mock_prisma_client():
    with patch("derouter.proxy.proxy_server.prisma_client") as mock:
        yield mock


@pytest.fixture
def mock_user_api_key_auth():
    with patch("derouter.proxy.agent_endpoints.endpoints.user_api_key_auth") as mock:
...
def test_update_agent_success(mock_prisma_client, mock_user_api_key_auth, monkeypatch):
    ...
    mock_prisma_client.db.derouter_agentstable.find_unique = AsyncMock(
...
def test_update_agent_not_found(
    ...
    mock_prisma_client.db.derouter_agentstable.find_unique = AsyncMock(return_value=None)
...
async def test_get_agent_daily_activity_admin_param_passing(monkeypatch):
    ...
    mock_prisma.db.derouter_agentstable.find_many = AsyncMock(return_value=[])
    ...
    assert kwargs["table_name"] == "derouter_dailyagentspend"
...
async def test_get_agent_daily_activity_with_agent_names(monkeypatch):
    ...
    mock_prisma.db.derouter_agentstable.find_many = AsyncMock(
...
async def test_attach_keys_to_agents_groups_by_agent_and_omits_secret():
    ...
    where = mock_prisma.db.derouter_verificationtoken.find_many.call_args.kwargs["where"]
...
class TestAgentByIdKeyRedaction:
    ...
    def _get_as(self, role: LitellmUserRoles):
        ...
        with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
            mock_prisma.db.derouter_agentstable.find_unique = AsyncMock(
                return_value=None
            )
            mock_prisma.db.derouter_verificationtoken.find_many = AsyncMock(
...
class TestAgentRBACProxyAdminViewOnly:
    ...
    cannot fan out health checks beyond their allowlist). derouter_params
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch):
        from derouter.proxy.agent_endpoints import agent_registry as ar_mod
        ...
        self.agents = [
            AgentResponse(
                ...
                agent_card_params=_sample_agent_card_params(),
                derouter_params={"api_key": SENTINEL_AGENT_API_KEY},
            )
            for index in (1, 2)
        ]
        self.mock_registry = MagicMock()
        ...
        monkeypatch.setattr(
            "derouter.proxy.agent_endpoints.auth.agent_permission_handler.AgentRequestHandler.resolve_agent_access",
    ...
    def test_should_still_redact_secrets_for_view_only_admin(self):
        ...
        stripped; derouter_params secrets never appear in either response."""
        ...
        assert admin_by_id["agent-1"]["derouter_params"]["api_key"] == REDACTED_BY_LITELM_STRING
...
class TestAgentRBACProxyAdmin:
    ...
    def test_create_agent_applies_derouter_merge_to_stored_card(self):
        """The card stored in the DB must reflect the DeRouter-fronting merge."""
        with patch("derouter.proxy.proxy_server.prisma_client"):
            self.mock_registry.get_agent_by_name = MagicMock(return_value=None)
            self.mock_registry.add_agent_to_db = AsyncMock(
                return_value=_sample_agent_response()
            )
            ...
            # Security scheme is the DeRouter scheme.
            assert "DeRouterKey" in stored_card["securitySchemes"]
    ...
    def test_create_agent_response_never_echoes_secret(self):
        ...
        with patch("derouter.proxy.proxy_server.prisma_client"):  # test-quality-ok: proxy_server module global is the endpoint's only injection point
            self.mock_registry.get_agent_by_name = MagicMock(return_value=None)
            self.mock_registry.add_agent_to_db = AsyncMock(
            ...
            body = resp.json()
            assert body["derouter_params"]["aws_secret_access_key"] == REDACTED_BY_LITELM_STRING
            assert body["derouter_params"]["model"] == "bedrock/agentcore/my-agent"

    def test_update_agent_response_never_echoes_secret(self):
        """LIT-6736: PUT /v1/agents/{id} must not echo the stored secret back."""
        with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:  # test-quality-ok: proxy_server module global is the endpoint's only injection point
            mock_prisma.db.derouter_agentstable.find_unique = AsyncMock(
                return_value={
                    "agent_id": "agent-123",
                    "agent_name": "Existing Agent",
            ...
            self.mock_registry.update_agent_in_db = AsyncMock(
                return_value=AgentResponse(
                    ...
                    agent_name="Test Agent",
                    agent_card_params=_sample_agent_card_params(),
                    derouter_params={"aws_secret_access_key": SENTINEL_AGENT_API_KEY},
                )
            )
            ...
            resp = self.admin_client.put(
                ...
                json={
                    "agent_name": "Test Agent",
                    "agent_card_params": _sample_agent_card_params(),
                    "derouter_params": {"aws_secret_access_key": REDACTED_BY_LITELM_STRING},
                },
                headers={"Authorization": "Bearer k"},
            )
            ...
            assert resp.status_code == 200
            assert SENTINEL_AGENT_API_KEY not in resp.text
            assert resp.json()["derouter_params"]["aws_secret_access_key"] == REDACTED_BY_LITELM_STRING
...
class TestAgentRoutesIncludesAgentIdPattern:
    ...
    def test_should_include_agent_id_pattern(self):
        ...
        assert "/v1/agents/{agent_id}" in DeRouterRoutes.agent_routes.value
...
class TestCheckAgentUrlHealth:
    ...
    async def test_should_return_healthy_when_no_url(self):
        from derouter.proxy.agent_endpoints.endpoints import _check_agent_url_health
    ...
    @pytest.mark.asyncio
    @patch("derouter.proxy.agent_endpoints.endpoints.get_async_httpx_client")
...
class _DbBackedProxyConfig:
    """Round-trips `derouter_settings` through the DB overlay the proxy applies on every
    `get_config()`, which is what re-assigns the `derouter.public_*` globals in production.

    Storage goes through JSON the way the `derouter_config` row does, so every read hands back
    freshly built values instead of the objects the endpoint still holds a reference to."""

    def __init__(self, stored_derouter_settings: dict[str, object] | None = None) -> None:
        self.stored_derouter_settings_json: str = json.dumps(stored_derouter_settings or {})

    async def get_config(self) -> dict[str, dict[str, object]]:
        from derouter.proxy.proxy_server import ProxyConfig

        config: Final[dict[str, dict[str, object]]] = {"derouter_settings": {}}
        db_param_value: Final[dict[str, object]] = json.loads(self.stored_derouter_settings_json)
    ...
    async def save_config(self, new_config: dict[str, dict[str, object]]) -> None:
        self.stored_derouter_settings_json = json.dumps(new_config.get("derouter_settings") or {})
...
def test_make_agent_public_twice_keeps_both_agents_public(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    import derouter
    from derouter.proxy.agent_endpoints import agent_registry as agent_registry_module
    from derouter.proxy.agent_endpoints.agent_registry import AgentRegistry
    ...
    monkeypatch.setattr(derouter, "public_agent_groups", None)
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", MagicMock())
    monkeypatch.setattr("derouter.proxy.proxy_server.proxy_config", _DbBackedProxyConfig())
