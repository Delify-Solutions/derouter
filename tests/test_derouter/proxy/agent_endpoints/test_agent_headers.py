...
async def test_no_headers():
    ...
    call_kwargs = mock_asend.call_args.kwargs
    headers = call_kwargs.get("agent_extra_headers")
    assert headers == {"X-DeRouter-User-Id": "u1"}
...
async def test_databricks_oauth_header_injected():
    ...
    from derouter.proxy.agent_endpoints import databricks_oauth
    ...
    with patch(
        "derouter.proxy.agent_endpoints.databricks_oauth.get_async_httpx_client",
...
async def test_non_databricks_agent_skips_oauth_resolution():
    ...
    mock_agent.derouter_params = {"require_trace_id_on_calls_to_agent": False}
    mock_request = _make_mock_request()

    with patch(
        "derouter.proxy.agent_endpoints.a2a_endpoints.resolve_databricks_app_auth_header",
        new_callable=AsyncMock,
    ) as mock_resolve:
        mock_asend = await _invoke(mock_agent, mock_request, None)
    ...
    headers = mock_asend.call_args.kwargs.get("agent_extra_headers")
    assert headers == {"x-custom": "v", "X-DeRouter-User-Id": "u1"}
...
async def test_convention_header_blocked_by_case_variant_static():
    ...
    headers = mock_asend.call_args.kwargs.get("agent_extra_headers")
    assert headers is not None
    assert headers == {"Authorization": "Bearer admin-token", "X-DeRouter-User-Id": "u1"}
    assert "authorization" not in headers


# ---------------------------------------------------------------------------
# Completion bridge: configured derouter_params.extra_headers win
...
async def test_bridge_caller_header_cannot_shadow_configured_header():
    ...
    admin-configured ``Authorization`` from ``derouter_params.extra_headers``."""
