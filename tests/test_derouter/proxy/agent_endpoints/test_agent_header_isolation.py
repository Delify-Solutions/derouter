...
async def _fake_create_client(agent_card, client_config=None, **kwargs):
    ...
    if client_config is not None:
        client._derouter_httpx_client = client_config.httpx_client
    return client
...
async def test_create_a2a_client_leaves_the_shared_client_untouched():
    ...
    tests/test_derouter/a2a_protocol/test_main.py.
    """
    pytest.importorskip("a2a.client")
    from derouter.a2a_protocol.main import create_a2a_client
    ...
    with (
        patch("derouter.a2a_protocol.main.A2A_SDK_AVAILABLE", True),
        patch(
            "derouter.a2a_protocol.main.get_async_httpx_client",
            side_effect=_fake_get_async_httpx_client_factory(captured_calls),
        ),
        patch(
            "derouter.a2a_protocol.main.create_client",
            new=AsyncMock(side_effect=_fake_create_client),
        ),
        patch(
            "derouter.a2a_protocol.main.A2ACardResolver",
