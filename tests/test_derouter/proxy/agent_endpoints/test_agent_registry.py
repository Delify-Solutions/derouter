...
from derouter.proxy.agent_endpoints.agent_registry import (
    ...
    _restore_redacted_derouter_params,
...
async def test_update_agent_in_db_clears_static_headers_and_extra_headers_when_omitted():
    ...
    mock_prisma.db.derouter_agentstable.update = mock_update
    mock_prisma.db.derouter_agentstable.find_unique = AsyncMock(return_value=None)
...
def test_public_agent_groups_holding_the_legacy_id_still_mark_the_config_agent_public(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "public_agent_groups", [legacy_id])
    assert [a.agent_id for a in registry.get_public_agent_list()] == [agent.agent_id]

    monkeypatch.setattr(derouter, "public_agent_groups", ["unrelated-id"])
...
async def test_update_agent_in_db_raises_when_row_deleted_mid_update():
    ...
    mock_prisma.db.derouter_agentstable.find_unique = AsyncMock(
        return_value=SimpleNamespace(derouter_params={}, object_permission_id=None)
    )
    mock_prisma.db.derouter_agentstable.update = AsyncMock(return_value=None)
...
async def test_delete_agent_from_db_raises_when_row_already_gone():
    ...
    registry: Final = AgentRegistry()
    mock_prisma: Final = MagicMock()
    mock_prisma.db.derouter_agentstable.delete = AsyncMock(return_value=None)
...
# ---------- LIT-6736: agent derouter_params secret redaction ----------


def test_redact_sensitive_agent_derouter_params_masks_secrets_keeps_the_rest():
    ...
    redacted = redact_sensitive_agent_derouter_params(
        {
...
def test_redact_sensitive_agent_derouter_params_recurses_into_nested_dicts():
...
def test_redact_sensitive_agent_derouter_params_handles_none_and_json_string():
    assert redact_sensitive_agent_derouter_params(None) is None

    serialized = json.dumps({"api_key": SENTINEL_AWS_SECRET_ACCESS_KEY, "model": "gpt-4"})
    redacted = redact_sensitive_agent_derouter_params(serialized)
...
def test_redact_sensitive_agent_derouter_params_recurses_into_lists_of_dicts():
...
def test_redact_sensitive_agent_derouter_params_redacts_secrets_inside_model_list():
    """The exact shape flagged in review: derouter_params.model_list, where each
    entry carries its own nested derouter_params with a provider credential."""
    redacted = redact_sensitive_agent_derouter_params(
        {
            "model_list": [
                {
                    "model_name": "gpt-4",
                    "derouter_params": {"api_key": SENTINEL_AWS_SECRET_ACCESS_KEY, "model": "gpt-4"},
                },
                {
                    "model_name": "claude",
                    "derouter_params": {
    ...
    assert SENTINEL_AWS_SECRET_ACCESS_KEY not in json.dumps(redacted)
    assert redacted["model_list"][0]["derouter_params"]["api_key"] == REDACTED_BY_LITELM_STRING
    assert redacted["model_list"][0]["derouter_params"]["model"] == "gpt-4"
    assert redacted["model_list"][1]["derouter_params"]["aws_secret_access_key"] == REDACTED_BY_LITELM_STRING
    assert redacted["model_list"][1]["derouter_params"]["model"] == "bedrock/claude"


def test_restore_redacted_derouter_params_preserves_secret_inside_model_list():
        ...
        "model_list": [
            {
                ...
                "derouter_params": {"api_key": REDACTED_BY_LITELM_STRING, "model": "gpt-4"},
            },
        ],
    }

    restored = _restore_redacted_derouter_params(incoming, existing)

    assert SENTINEL_AWS_SECRET_ACCESS_KEY == restored["model_list"][0]["derouter_params"]["api_key"]
    assert restored["model_list"][0]["model_name"] == "gpt-4-renamed"
    assert restored["agent_name"] == "my-agent-renamed"


def test_restore_redacted_derouter_params_matches_list_entries_by_position():
    ...
    existing = {
        "model_list": [
            {"model_name": "gpt-4", "derouter_params": {"api_key": SENTINEL_AWS_SECRET_ACCESS_KEY}},
            {"model_name": "claude", "derouter_params": {"api_key": "other-" + SENTINEL_AWS_SECRET_ACCESS_KEY}},
        ],
    }
    incoming = {
        "model_list": [
            # Same index (0) now holds what used to be at index 1's entry.
            {"model_name": "claude", "derouter_params": {"api_key": REDACTED_BY_LITELM_STRING}},
        ],
    }

    restored = _restore_redacted_derouter_params(incoming, existing)

    assert restored["model_list"][0]["derouter_params"]["api_key"] == SENTINEL_AWS_SECRET_ACCESS_KEY


def test_restore_redacted_derouter_params_recovers_a_whole_subtree_collapsed_by_the_depth_cap():
...
def test_redact_sensitive_agent_derouter_params_does_not_reinterpret_plain_string_values_as_json():
    ...
    redacted = redact_sensitive_agent_derouter_params({"model": "bedrock/agentcore/my-agent", "is_public": True})
...
async def test_add_agent_to_db_drops_a_sentinel_value_instead_of_storing_the_placeholder():
    ...
    mock_prisma.db.derouter_agentstable.create = mock_create
    ...
    stored_params: Final = json.loads(mock_create.call_args.kwargs["data"]["derouter_params"])
...
async def test_update_agent_in_db_preserves_secret_when_echoed_back_redacted():
    ...
    stored_params: Final = json.loads(mock_update.call_args.kwargs["data"]["derouter_params"])
...
async def test_update_agent_in_db_preserves_secret_when_key_omitted_entirely():
    ...
    mock_prisma.db.derouter_agentstable.find_unique = AsyncMock(
        return_value=SimpleNamespace(
            derouter_params={"aws_secret_access_key": SENTINEL_AWS_SECRET_ACCESS_KEY},
            object_permission_id=None,
        )
    )
    updated_agent = MagicMock()
    ...
    await registry.update_agent_in_db(
        ...
        agent={
            "agent_name": "Test Agent",
            "agent_card_params": _sample_agent_card_params(),
            "derouter_params": {"model": "bedrock/agentcore/my-agent"},
        },
        prisma_client=mock_prisma,
        updated_by="test-user",
    )
...
async def test_update_agent_in_db_clears_secret_on_explicit_empty_value():
    ...
    await registry.update_agent_in_db(
        ...
        agent={
            "agent_name": "Test Agent",
            "agent_card_params": _sample_agent_card_params(),
            "derouter_params": {"aws_secret_access_key": ""},
        },
        prisma_client=mock_prisma,
        updated_by="test-user",
    )
...
@pytest.mark.asyncio
async def test_patch_agent_in_db_preserves_secret_when_derouter_params_omitted():
    """A PATCH that only renames the agent must not touch (let alone drop) the
    stored derouter_params secret."""
    registry: Final = AgentRegistry()
    mock_prisma: Final = MagicMock()
    ...
    assert "derouter_params" not in update_data


@pytest.mark.asyncio
async def test_patch_agent_in_db_preserves_secret_when_echoed_back_redacted():
    """A PATCH that includes derouter_params (e.g. to flip an unrelated flag)
