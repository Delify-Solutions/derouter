...
def _mock_http_handler(access_token="tok-abc", expires_in=3600, post_error=None):
    """Return a mock that mirrors derouter's ``AsyncHTTPHandler`` contract.
...
async def test_fetch_token_posts_client_credentials_with_basic_auth():
    ...
    # header because derouter's AsyncHTTPHandler.post has no ``auth`` parameter.
...
async def test_resolve_returns_bearer_header():
    ...
    with patch(
        ...
        header = await resolve_databricks_app_auth_header(derouter_params)
