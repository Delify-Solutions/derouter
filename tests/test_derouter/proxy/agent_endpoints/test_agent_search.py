...
from derouter.proxy.agent_endpoints.auth.agent_permission_handler import RestrictedAgentAccess
from derouter.proxy.agent_endpoints.endpoints import router, user_api_key_auth
...
def registry(monkeypatch: pytest.MonkeyPatch) -> MagicMock:
    from derouter.proxy.agent_endpoints import agent_registry as registry_module
    ...
    monkeypatch.setattr("derouter.proxy.agent_endpoints.endpoints.global_agent_search_index", AgentSearchIndex())
...
def embedding_router(monkeypatch: pytest.MonkeyPatch) -> MagicMock:
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.proxy_logging_obj", _pass_through_key_limits())
    monkeypatch.setattr(derouter, "agent_search_embedding_model", "text-embedding-3-small")
    return router
...
class TestGetAgentsQuery:
    ...
    def test_missing_embedding_model_is_a_400(
        ...
        monkeypatch.setattr(derouter, "agent_search_embedding_model", None)
        response = _client(LitellmUserRoles.PROXY_ADMIN).get(
