...
Tests that invoke_agent_a2a properly integrates with add_derouter_data_to_request.
"""
...
AddDeRouterData = Callable[..., Awaitable[dict[str, object]]]
...
@pytest.mark.asyncio
async def test_invoke_agent_a2a_adds_derouter_data():
    """
    Test that invoke_agent_a2a calls add_derouter_data_to_request
    and the resulting data includes proxy_server_request.
    """
    from derouter.proxy._types import UserAPIKeyAuth

    # Track the data passed to add_derouter_data_to_request
    captured_data = {}

    async def mock_add_derouter_data(data, **kwargs):
        # Simulate what add_derouter_data_to_request does
    ...
    mock_agent.derouter_params = None
    ...
    # This test focuses on DeRouter integration, not A2A protocol correctness,
    ...
    # Note: add_derouter_data_to_request is called from common_request_processing,
    # so we need to patch it there, not at derouter_pre_call_utils
    with (
        patch(
            "derouter.proxy.agent_endpoints.a2a_endpoints._get_agent",
            return_value=mock_agent,
        ),
        patch(
            "derouter.proxy.common_request_processing.add_derouter_data_to_request",
            side_effect=mock_add_derouter_data,
        ) as mock_add_data,
        patch(
            "derouter.a2a_protocol.create_a2a_client",
            new_callable=AsyncMock,
        ),
        patch(
            "derouter.a2a_protocol.asend_message",
            new_callable=AsyncMock,
        ...
        patch(
            "derouter.proxy.proxy_server.version",
            "1.0.0",
        ),
        patch.dict(
            sys.modules,
        ...
        patch(
            "derouter.a2a_protocol.main.A2A_SDK_AVAILABLE",
            True,
        ),
    ):
        from derouter.proxy.agent_endpoints.a2a_endpoints import invoke_agent_a2a
        ...
        # Verify add_derouter_data_to_request was called
...
async def test_invoke_agent_a2a_injects_authenticated_key_hash_for_bridge():
    ...
    derouter_params so provider configs (e.g. LangFlow) can scope provider-side
    ...
    with (
        ...
        patch(
            "derouter.proxy.agent_endpoints.auth.agent_permission_handler.AgentRequestHandler.is_agent_allowed",
        ...
        patch("derouter.proxy.proxy_server.proxy_config", MagicMock()),
        patch("derouter.proxy.proxy_server.version", "1.0.0"),
        patch("derouter.a2a_protocol.main.A2A_SDK_AVAILABLE", True),
        patch.dict(sys.modules, {"a2a": MagicMock(), "a2a.types": MagicMock()}),
    ):
        from derouter.proxy.agent_endpoints.a2a_endpoints import invoke_agent_a2a
    ...
    assert (
        captured.get("derouter_params", {}).get(A2A_USER_API_KEY_HASH_PARAM)
...
def _base_patches(
    agent: MagicMock, add_derouter_data: AddDeRouterData | None = None
    ...
    return [
        ...
        patch(
            "derouter.proxy.common_request_processing.add_derouter_data_to_request",
            new=AsyncMock(side_effect=add_derouter_data or _add_proxy_data),
        ),
        patch("derouter.proxy.proxy_server.general_settings", {}),
        patch("derouter.proxy.proxy_server.proxy_config", MagicMock()),
        patch("derouter.proxy.proxy_server.version", "1.0.0"),
...
async def _invoke_message_method(
    ...
    add_derouter_data: AddDeRouterData | None = None,
) -> CapturedAgentCall:
    ...
    with ExitStack() as stack:
        for p in _base_patches(_make_agent_mock(), add_derouter_data):
            stack.enter_context(p)
        stack.enter_context(patch("derouter.a2a_protocol.main.A2A_SDK_AVAILABLE", True))
        if is_send:
            stack.enter_context(patch.dict(sys.modules, {"a2a": MagicMock(), "a2a.types": mock_a2a_types}))
            stack.enter_context(patch("derouter.a2a_protocol.asend_message", new=downstream))
        else:
            stack.enter_context(
                patch("derouter.proxy.agent_endpoints.a2a_endpoints._handle_stream_message", new=downstream)
            )
...
async def test_message_methods_forward_caller_identity_headers(method: str):
    ...
    assert forwarded_headers.get("X-DeRouter-User-Id") == "user-abc"
    assert forwarded_headers.get("X-DeRouter-Team-Id") == "team-xyz"
...
async def test_message_methods_caller_identity_headers_cannot_be_spoofed(method: str):
    mock_request = _make_request_mock(method, _HELLO_MESSAGE_PARAMS)
    mock_request.headers = {
        "x-a2a-test-agent-x-derouter-user-id": "attacker-user",
        "x-a2a-test-agent-x-derouter-team-id": "attacker-team",
    }
    ...
    forwarded_headers = captured.agent_extra_headers or {}
    assert (
        forwarded_headers.get("X-DeRouter-User-Id") == "real-user"
    ), "authenticated user id must not be overridden by forwarded client headers"
    assert (
        forwarded_headers.get("X-DeRouter-Team-Id") == "real-team"
...
async def test_message_methods_forward_key_bound_identity_not_pre_call_rewrite(method: str):
    ...
    async def apply_user_header_mapping(data: dict[str, object], **kwargs: object) -> dict[str, object]:
        DeRouterProxyRequestSetup.add_internal_user_from_user_mapping(
    ...
    captured = await _invoke_message_method(
        method, mock_request, user_api_key_dict, add_derouter_data=apply_user_header_mapping
    )

    assert user_api_key_dict.user_id == "header-mapped-user", "precondition: pre-call rewrite ran"
    forwarded_headers = captured.agent_extra_headers or {}
    assert forwarded_headers.get("X-DeRouter-User-Id") == "key-user"
    assert forwarded_headers.get("X-DeRouter-Team-Id") == "key-team"
...
async def test_task_methods_forward_jsonrpc(method: str, params: dict):
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(
            patch(
                "derouter.llms.custom_httpx.http_handler.get_async_httpx_client",
                return_value=mock_handler,
            )
        )
        stack.enter_context(
            patch(
                "derouter.proxy.agent_endpoints.a2a_endpoints.validate_url",
...
async def test_task_methods_extract_derouter_params_before_forwarding(method: str):
    from derouter.proxy._types import UserAPIKeyAuth
...
async def test_subscribe_to_task_runs_post_call_streaming_guardrail():
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(patch.object(derouter, "callbacks", [guardrail]))
...
async def test_task_method_failure_hook_uses_enriched_request_data():
    ...
    assert failure_data.get("derouter_call_id")
...
async def test_get_agent_card_uses_proxy_base_url_when_set(monkeypatch):
    ...
    with ExitStack() as stack:
        ...
        from derouter.proxy.agent_endpoints.a2a_endpoints import get_agent_card
    ...
    body = json.loads(response.body.decode())
    assert body["url"] == "https://derouter.example.com/a2a/test-agent"
    assert (
        body["supportedInterfaces"][0]["url"]
        == "https://derouter.example.com/a2a/test-agent"
    )
...
async def test_get_extended_agent_card_uses_proxy_base_url_when_set(monkeypatch):
    ...
    body = json.loads(response.body.decode())
    assert body["result"]["url"] == "https://derouter.example.com/a2a/test-agent"
...
def test_build_merged_agent_card_uses_proxy_base_url_for_supported_interfaces(
    ...
    from derouter.proxy.agent_endpoints.endpoints import _build_merged_agent_card
    ...
    assert merged["supportedInterfaces"][0]["url"] == (
        "https://derouter.example.com/a2a/jenkins_agent"
    )
...
def test_build_message_send_params_accepts_wire_and_a2a_10(params):
    from derouter.proxy.agent_endpoints.a2a_endpoints import _build_message_send_params
...
async def test_handle_stream_message_rejects_invalid_params_with_32602():
    from derouter.proxy.agent_endpoints.a2a_endpoints import _handle_stream_message
...
async def test_handle_stream_message_frames_events_as_sse():
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(
            patch(
                "derouter.a2a_protocol.asend_message_streaming",
                new=fake_stream,
            )
        )
...
async def test_handle_stream_message_sdk_unavailable_frames_error_as_sse():
    ...
    with patch("derouter.a2a_protocol.main.A2A_SDK_AVAILABLE", False):
        response = await _handle_stream_message(
            api_base="http://upstream.local",
            request_id="req-1",
...
async def test_handle_stream_message_proxy_hook_path_frames_events_as_sse():
    ...
    with ExitStack() as stack:
        stack.enter_context(patch("derouter.a2a_protocol.main.A2A_SDK_AVAILABLE", True))
        stack.enter_context(
            patch("derouter.a2a_protocol.asend_message_streaming", new=fake_stream)
        )
...
def test_normalize_response_wraps_flat_message_result_for_1_0():
    from derouter.proxy.a2a.version_convert import normalize_jsonrpc_response
...
async def test_forward_jsonrpc_sse_fallback_error_uses_jsonrpc_error_code():
    ...
    with patch(
        ...
        from derouter.proxy.agent_endpoints.a2a_endpoints import _forward_jsonrpc_sse
...
async def test_task_methods_forward_caller_identity_headers():
    """Task operations must forward X-DeRouter-User-Id and X-DeRouter-Team-Id so the
    ...
    assert posted_headers.get("X-DeRouter-User-Id") == "user-abc"
    assert posted_headers.get("X-DeRouter-Team-Id") == "team-xyz"
...
async def test_task_methods_forward_trace_header(method: str):
    ...
    async def add_proxy_data_with_trace(data, **kwargs):
        ...
        data["derouter_trace_id"] = "trace-123"
        return data
    ...
    assert forwarded_headers.get("X-DeRouter-Trace-Id") == "trace-123"
...
def test_push_notification_config_set_rejects_private_dns_resolution():
    ...
    from derouter.proxy.agent_endpoints.a2a_endpoints import (
...
async def test_caller_identity_headers_cannot_be_spoofed_via_forwarded_headers():
    """A client must not be able to override X-DeRouter-User-Id / X-DeRouter-Team-Id
    by including x-a2a-<agent>-x-derouter-user-id in their request headers.
    ...
    posted_headers = mock_handler.post.call_args.kwargs.get("headers") or {}
    assert (
        posted_headers.get("X-DeRouter-User-Id") == "real-user"
    ), "authenticated user id must not be overridden by forwarded client headers"
    assert (
        posted_headers.get("X-DeRouter-Team-Id") == "real-team"
    ), "authenticated team id must not be overridden by forwarded client headers"
...
def test_served_version_config_governs_over_header():
    from derouter.proxy.agent_endpoints.a2a_endpoints import _served_version
...
async def test_forward_jsonrpc_sse_pings_while_the_upstream_agent_is_still_silent(
    ...
    monkeypatch.setattr(derouter, "sse_keepalive_ping_interval_seconds", 0.05)
...
async def test_forward_jsonrpc_sse_is_untouched_while_keepalives_are_unconfigured(
    ...
    monkeypatch.setattr(derouter, "sse_keepalive_ping_interval_seconds", None)
