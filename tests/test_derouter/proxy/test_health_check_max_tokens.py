...
from derouter.proxy import health_check as hc_module
...
@pytest.mark.asyncio
async def test_update_derouter_params_max_tokens_default(monkeypatch):
    """
    ...
    updated_params = _update_derouter_params_for_health_check(model_info, derouter_params)

    assert updated_params["max_tokens"] == 16


@pytest.mark.asyncio
async def test_update_derouter_params_max_tokens_custom():
    """
...
@pytest.mark.asyncio
async def test_update_derouter_params_max_tokens_wildcard():
    """
...
async def test_ahealth_check_wildcard_models_respects_max_tokens():
    ...
    with (
        patch(
            "derouter.derouter_core_utils.llm_request_utils.pick_cheapest_chat_models_from_llm_provider",
            return_value=["gpt-4o-mini"],
        ),
        patch("derouter.acompletion", new_callable=AsyncMock),
    ):
...
async def test_background_health_check_max_tokens_env_var(monkeypatch):
    ...
    model_info = {}
    derouter_params = {"model": "azure/gpt-4"}
...
def test_resolve_health_check_max_tokens_reasoning_specific_model_info():
    ...
    with patch.object(hc_module.derouter, "supports_reasoning", return_value=False):
        assert _resolve_health_check_max_tokens(model_info, derouter_params) == 2

    with patch.object(hc_module.derouter, "supports_reasoning", return_value=True):
        assert _resolve_health_check_max_tokens(model_info, derouter_params) == 64
...
def test_explicit_health_check_max_tokens_beats_reasoning_specific():
    ...
    with patch.object(hc_module.derouter, "supports_reasoning", return_value=True):
        assert _resolve_health_check_max_tokens(model_info, derouter_params) == 9
...
def test_reasoning_specific_falls_through_when_wrong_branch_only(monkeypatch):
    ...
    derouter_params = {"model": "openai/o1"}

    with patch.object(hc_module.derouter, "supports_reasoning", return_value=True):
        assert _resolve_health_check_max_tokens(model_info, derouter_params) == 16
...
async def test_background_split_env_reasoning_vs_non_reasoning(monkeypatch):
    ...
    with patch.object(hc_module.derouter, "supports_reasoning", return_value=False):
        updated = _update_derouter_params_for_health_check(model_info, derouter_params)
        assert updated["max_tokens"] == 16

    derouter_params2 = {"model": "openai/o1"}
    with patch.object(hc_module.derouter, "supports_reasoning", return_value=True):
        updated2 = _update_derouter_params_for_health_check(model_info, derouter_params2)
...
async def test_reasoning_env_precedence_over_global(monkeypatch):
    ...
    model_info = {}
    derouter_params = {"model": "openai/gpt-5.4"}
...
def test_wildcard_ignores_reasoning_split_model_info(monkeypatch):
    ...
    assert _resolve_health_check_max_tokens(model_info, derouter_params) is None


# ---------------------------------------------------------------------------
# image_generation must not receive max_tokens.
#
# _update_derouter_params_for_health_check injected `max_tokens` for every
...
def test_image_generation_mode_skips_max_tokens():
    ...
    derouter_params = {"model": "openai/dall-e-3", "api_key": "sk-test"}
...
def test_health_check_max_tokens_value_is_ignored_for_non_chat_modes():
    ...
    derouter_params = {"model": "openai/dall-e-3"}
...
def test_chat_style_modes_inject_max_tokens(mode):
    updated = _update_derouter_params_for_health_check({"mode": mode}, {"model": f"openai/dummy-{mode}"})
...
def test_explicit_override_true_forces_injection_outside_allowlist():
    ...
    derouter_params = {"model": "openai/some-future-image-model"}
...
def test_explicit_override_false_suppresses_injection_inside_allowlist():
    ...
    derouter_params = {"model": "openai/strict-schema-chat"}
...
def test_update_derouter_params_health_check_reasoning_effort():
    ...
    derouter_params = {"model": "openai/gpt-5", "api_key": "x"}
    out = _update_derouter_params_for_health_check(model_info, dict(derouter_params))
    assert out.get("reasoning_effort") == "low"

    model_info = {"mode": "chat", "health_check_reasoning_effort": "none"}
    out = _update_derouter_params_for_health_check(model_info, {"model": "openai/gpt-5", "api_key": "x"})
    ...
    out = _update_derouter_params_for_health_check(model_info, {"model": "openai/gpt-5.1", "api_key": "x"})
    assert out.get("reasoning_effort") == {"effort": "none", "summary": "auto"}

    model_info = {"mode": "embedding", "health_check_reasoning_effort": "low"}
    out = _update_derouter_params_for_health_check(model_info, {"model": "text-embedding-3-small", "api_key": "x"})
    assert "reasoning_effort" not in out

    model_info = {}
    out = _update_derouter_params_for_health_check(model_info, {"model": "openai/gpt-4o", "api_key": "x"})
    assert "reasoning_effort" not in out
...
def test_bedrock_embedding_without_explicit_mode_skips_max_tokens(deployment_model, expected_request_model):
    ...
    updated = _update_derouter_params_for_health_check({}, {"model": deployment_model})
...
def test_bedrock_chat_without_mode_still_injects_max_tokens_and_pins_provider():
    ...
    updated = _update_derouter_params_for_health_check(
...
async def test_run_model_health_check_threads_resolved_mode_to_ahealth_check():
    ...
    model = {
        "derouter_params": {"model": "bedrock/amazon.titan-embed-text-v2:0"},
        "model_info": {},
    }

    with patch.object(hc_module.derouter, "ahealth_check", fake_ahealth_check):
...
def test_health_check_params_lose_to_dedicated_health_check_knobs():
    ...
    updated = _update_derouter_params_for_health_check(model_info, {"model": "openai/dummy"})
...
def test_health_check_params_ignored_when_not_a_dict(bad_value, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="derouter.proxy.health_check"):
        updated = _update_derouter_params_for_health_check(
...
async def _pegasus_health_check_request_body(
    ...
    with respx.mock(assert_all_called=True) as respx_mock:
        ...
        result = await derouter.ahealth_check(derouter_params, mode="chat")
...
def test_strategy_router_reds_when_a_dependency_name_matches_no_deployment():
    ...
    marker["derouter_params"]["complexity_router_config"]["tiers"]["SIMPLE"] = "typo-group"
...
def test_narrowing_by_an_id_that_matches_nothing_keeps_the_whole_list():
    ...
    deployments = [{"model_name": "a", "derouter_params": {"model": "openai/a"}, "model_info": {"id": "a-1"}}]
...
def test_a_router_routing_to_a_healthy_router_stays_green():
    ...
    child["derouter_params"]["complexity_router_config"]["tiers"]["SIMPLE"] = "dead-group"
