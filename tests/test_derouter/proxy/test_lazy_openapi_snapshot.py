...
from derouter.proxy._lazy_openapi_snapshot import SnapshotResult, _normalize_operation_ids, main


def test_generate_snapshot_uses_shared_operation_id_reservations(monkeypatch):
    from derouter.proxy import _lazy_openapi_snapshot
    ...
    fake_lazy_features_module = ModuleType("derouter.proxy._lazy_features")
    ...
    monkeypatch.setitem(sys.modules, "derouter.proxy._lazy_features", fake_lazy_features_module)
    ...
    fake_proxy_server_module = ModuleType("derouter.proxy.proxy_server")
    ...
    monkeypatch.setitem(sys.modules, "derouter.proxy.proxy_server", fake_proxy_server_module)
...
def test_generate_snapshot_registers_transitively_imported_modules(monkeypatch):
    ...
    fake_app = SimpleNamespace(title="DeRouter test", version="0.0.0", routes=[])
...
def test_generate_snapshot_reports_features_whose_import_fails(monkeypatch):
    ...
    fake_lazy_features_module.LAZY_FEATURES = [
        ...
        LazyFeature(
            name="broken",
            module_path="derouter.proxy.this_module_does_not_exist",
