...
from derouter.proxy.proxy_cli import ProxyInitializationHelpers, run_server
...
class TestProxyInitializationHelpers:
    ...
    @patch("click.echo")
    def test_echo_derouter_version(self, mock_echo, mock_version):
        # Setup
        ...
        # Assert
        mock_version.assert_called_once_with("derouter")
        mock_echo.assert_called_once_with("\nDeRouter: Current Version = 1.0.0\n")
    ...
    def test_get_default_unvicorn_init_args(self):
        ...
        assert args["app"] == "derouter.proxy.proxy_server:app"
        ...
        with patch("derouter.json_logs", True):
            args = ProxyInitializationHelpers._get_default_unvicorn_init_args(
                "localhost", 8000
            )
    ...
    def test_patch_statreload_extra_paths_yields_config_and_py(self, tmp_path):
        ...
        if hasattr(StatReload, "_derouter_patched_config_paths"):
            StatReload._derouter_patched_config_paths.clear()
    ...
    def test_configure_dev_reload_watches_env_and_sets_override_flag(
        ...
        if hasattr(StatReload, "_derouter_patched_config_paths"):
            StatReload._derouter_patched_config_paths.clear()
        monkeypatch.delenv("DEROUTER_DEV_ENV_HOT_RELOAD", raising=False)
        ...
        uvicorn_args: dict = {}
        with patch("derouter._logging.verbose_proxy_logger.warning") as mock_warning:
            ProxyInitializationHelpers._configure_dev_reload(
                uvicorn_args, str(config_file)
            )

        assert os.environ["DEROUTER_DEV_ENV_HOT_RELOAD"] == "True"
    ...
    def test_dev_env_hot_reload_enabled_reads_flag(self, monkeypatch):
        ...
        monkeypatch.setenv("DEROUTER_DEV_ENV_HOT_RELOAD", "True")
        assert derouter._dev_env_hot_reload_enabled() is True

        monkeypatch.setenv("DEROUTER_DEV_ENV_HOT_RELOAD", "false")
        assert derouter._dev_env_hot_reload_enabled() is False
    ...
    def test_init_granian_server(self, mock_print, mock_granian_cls):
        ...
        assert call_kwargs["target"] == "derouter.proxy.proxy_server:app"
    ...
    @patch("derouter.proxy.db.prisma_client.PrismaManager.setup_database")
    @patch(
        "derouter.proxy.db.prisma_client.should_update_prisma_schema", return_value=False
    )
    def test_skip_server_startup(
        ...
        from derouter.proxy.proxy_cli import run_server
        ...
        with (
            ...
            patch.dict(
                "sys.modules",
                {
                    ...
                    "derouter.proxy.proxy_server": mock_proxy_module,
                },
            ),
            patch(
                "derouter.proxy.proxy_cli.ProxyInitializationHelpers._get_default_unvicorn_init_args"
            ) as mock_get_args,
        ):
    ...
    @patch("atexit.register")
    @patch("subprocess.Popen")
    @patch("derouter.proxy.db.prisma_client.PrismaManager.setup_database")  # test-quality-ok: run_server always wires the DB; same isolation as the sibling CLI tests above
    ...
    def test_prometheus_metrics_port_starts_separate_metrics_process(
        ...
        """--prometheus_metrics_port must spawn `python -m derouter.proxy.prometheus_metrics_server` on --host
        ...
        def child_metrics(request: httpx.Request) -> httpx.Response:
            ...
            return httpx.Response(200, headers={"x-derouter-metrics-pid": "4242"}, content=b"")
        ...
        with (
            ...
            assert spawned[1:3] == ["-m", "derouter.proxy.prometheus_metrics_server"]
    ...
    def test_db_timeout_settings_are_forwarded_to_pool_timeout(
        ...
        with (
            ...
            patch(
                "derouter.proxy.proxy_cli.append_query_params",
    ...
    def test_build_db_connection_url_params_defaults(self):
        from derouter.proxy.proxy_cli import _build_db_connection_url_params
    ...
    def test_proxy_default_api_version_uses_azure_default(
        ...
        """Proxy default api_version should match derouter.AZURE_DEFAULT_API_VERSION for consistency."""
        from click.testing import CliRunner
        ...
        with (
            ...
            assert call_kwargs["api_version"] == derouter.AZURE_DEFAULT_API_VERSION
    ...
    def test_keepalive_timeout_flag(
        ...
        with (
            ...
            patch(
                "derouter.proxy.proxy_cli.ProxyInitializationHelpers._is_port_in_use",
                return_value=False,
            ),
        ):
            mock_get_args.return_value = {
    ...
    @patch("derouter.proxy.proxy_cli.ProxyInitializationHelpers._run_gunicorn_server")
    @patch("uvicorn.run")
    @patch("builtins.print")
    @patch("derouter.proxy.db.prisma_client.PrismaManager.setup_database")
    ...
    def test_gunicorn_options_include_max_requests_jitter(self):
        ...
        with (
            ...
            patch(  # test-quality-ok: option tests must not start a thread or change the pytest worker's child ownership
                "derouter.proxy.proxy_cli.start_query_engine_reaper"
            ),
        ):
    ...
    def test_run_server_no_config_passed(self, mock_print, mock_uvicorn_run):
        ...
        async def mock_get_config(config_file_path=None):
            return {"general_settings": {}, "derouter_settings": {}}
        ...
        # derouter.proxy.proxy_server import at line 820 of proxy_cli.py has heavy
            ...
            with (
                patch.dict(
                    "sys.modules",
                    {
                        ...
                        # Also mock derouter.proxy.proxy_server to prevent the real
                        # import at line 820 of proxy_cli.py which has heavy side
                        # effects (FastAPI app init, logging setup, etc.)
                        "derouter.proxy.proxy_server": mock_proxy_server_module,
                    },
                ),
                patch(
                    "derouter.proxy.proxy_cli.ProxyInitializationHelpers._get_default_unvicorn_init_args"
...
class TestRunServerDbSetup:
    ...
    @patch("atexit.register")
    @patch("derouter.proxy.db.prisma_client.PrismaManager.setup_database")
    @patch("derouter.proxy.db.check_migration.check_prisma_schema_diff")
    @patch("derouter.proxy.db.prisma_client.should_update_prisma_schema")
    ...
    def test_startup_exits_on_non_postgres_database_url(
        ...
        clean_env["DATABASE_URL"] = "sqlite:///data/derouter.db"
    ...
    def test_v2_migration_resolver_opts_in_via_env_var(
        ...
        The Helm migrations Job runs `python derouter/proxy/prisma_migration.py`,
...
class TestWorkerStartupHooks:
    """Tests for the DEROUTER_WORKER_STARTUP_HOOKS mechanism in proxy_startup_event."""
    ...
    async def test_should_run_worker_startup_hooks(self):
        ...
        env_overrides = {
            "DEROUTER_WORKER_STARTUP_HOOKS": "tests.test_derouter.proxy.test_proxy_cli:_dummy_hook",
        }
    ...
    async def test_should_run_async_worker_startup_hook(self):
        ...
        env_overrides = {
            "DEROUTER_WORKER_STARTUP_HOOKS": "tests.test_derouter.proxy.test_proxy_cli:_dummy_async_hook",
        }
        clean_env = {
            k: v
            for k, v in os.environ.items()
    ...
    async def test_should_raise_on_failing_worker_startup_hook(self):
        ...
        env_overrides = {
            "DEROUTER_WORKER_STARTUP_HOOKS": "tests.test_derouter.proxy.test_proxy_cli:_failing_hook",
        }
        clean_env = {
            k: v
            for k, v in os.environ.items()
    ...
    def test_should_skip_when_no_hooks_set(self):
        """When DEROUTER_WORKER_STARTUP_HOOKS is not set, no hooks are executed."""
        global _dummy_hook_called
        _dummy_hook_called = False

        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("DEROUTER_WORKER_STARTUP_HOOKS", None)
            # The hook block should be skipped entirely when env var is absent
            assert "DEROUTER_WORKER_STARTUP_HOOKS" not in os.environ
            # Verify that an empty env var value also results in no hook execution
            assert os.environ.get("DEROUTER_WORKER_STARTUP_HOOKS", "") == ""
    ...
    async def test_should_run_multiple_hooks(self):
        ...
        hooks = (
            "tests.test_derouter.proxy.test_proxy_cli:_dummy_hook,"
            "tests.test_derouter.proxy.test_proxy_cli:_dummy_async_hook"
        )
        env_overrides = {
            "DEROUTER_WORKER_STARTUP_HOOKS": hooks,
        }
        clean_env = {
            k: v
            for k, v in os.environ.items()
...
class TestPostgresStatementTimeoutOptions:
    ...
    def test_pg_options_with_timeouts(self, existing, statement_timeout, lock_timeout, expected):
        from derouter.proxy.proxy_cli import _pg_options_with_timeouts
...
def _run_server_and_capture_urls(
    ...
    with (
        ...
        patch("atexit.register"),
        patch("derouter.proxy.db.prisma_client.should_update_prisma_schema", return_value=False),
        patch("derouter.proxy.db.check_migration.check_prisma_schema_diff"),
    ):
        run_server.main(
...
class TestMaxIdleConnectionLifetimeDefault:
    ...
    def test_idle_lifetime_params_prefers_configured_value(self):
        from derouter.proxy.db.db_url_settings import idle_lifetime_params
...
class TestTokenAuthCliFlags:
    ...
    def _invoke_with_azure_host(self, args):
        ...
        from derouter.proxy.db.token_auth import build_azure_entra_token_provider
        from derouter.proxy.proxy_cli import run_server
        ...
        clean_env["DATABASE_USER"] = "derouter@contoso.onmicrosoft.com"
        clean_env["DATABASE_NAME"] = "derouter_db"
