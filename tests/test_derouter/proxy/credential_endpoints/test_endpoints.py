...
def credential_store():
    ...
    def install(
        ...
        **repository_calls: AsyncMock,
        ...
        patch("derouter.proxy.proxy_server.prisma_client", MagicMock() if connected else None).start()
        patch("derouter.proxy.proxy_server.master_key", "sk-test-master").start()
        patch.object(derouter, "credential_list", list(in_memory)).start()
        repository = patch("derouter.proxy.credential_endpoints.endpoints.CredentialsRepository").start()
...
def test_delete_credential_still_answers_200_and_drops_the_credential_from_memory(credential_store):
    ...
    assert response.status_code == 200, response.text
    assert response.json()["success"] is True
    assert [credential.credential_name for credential in derouter.credential_list] == ["keeper"]
...
def test_delete_credential_leaves_a_credential_that_only_exists_in_memory_in_place(credential_store):
    ...
    assert [credential.credential_name for credential in derouter.credential_list] == ["from-config-yaml"]
