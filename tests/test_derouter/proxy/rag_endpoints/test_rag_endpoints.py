...
def test_rag_query_returns_response_cost_header(client_internal_user):
    ...
    x-derouter-response-cost response header, like /v1/chat/completions does.
    """
    ...
    ), patch("derouter.vector_store_registry", None), patch(
        "derouter.proxy.proxy_server.prisma_client", None
    ):
        response = client_internal_user.post(
            "/v1/rag/query",
            json={
    ...
    assert response.headers.get("x-derouter-response-cost") == "3.45e-06"
...
def test_rag_query_stream_returns_event_stream(client_internal_user):
    ...
    async def fake_aquery(**kwargs):
        return await derouter_module.acompletion(
            model="gpt-4o-mini",
            messages=[{"role": "user", "content": "What is the codename?"}],
    ...
    ), patch("derouter.vector_store_registry", None), patch("derouter.proxy.proxy_server.prisma_client", None):
        response = client_internal_user.post(
            "/v1/rag/query",
            json={
                "model": "gpt-4o-mini",
...
def test_rag_query_stream_pings_while_retrieval_is_still_running(client_internal_user, monkeypatch):
    ...
    monkeypatch.setattr(derouter_module, "sse_keepalive_ping_interval_seconds", 0.05)
    ...
    with (
        patch(  # test-quality-ok: the handler calls the module-level derouter.aquery directly; no injection seam
            "derouter.proxy.rag_endpoints.endpoints.derouter.aquery",
            new=AsyncMock(side_effect=slow_aquery),
        ),
        patch("derouter.vector_store_registry", None),  # test-quality-ok: proxy module global, no injection seam
        patch("derouter.proxy.proxy_server.prisma_client", None),  # test-quality-ok: proxy module global, no injection seam
    ):
        response = client_internal_user.post(
            "/v1/rag/query",
            json={
...
def test_rag_query_stream_keeps_response_headers_when_retrieval_beats_the_keepalive(
    ...
    monkeypatch.setattr(derouter_module, "sse_keepalive_ping_interval_seconds", 5)

    async def fast_aquery(**kwargs):
        response = await derouter_module.acompletion(
            model="gpt-4o-mini",
            messages=[{"role": "user", "content": "What is the codename?"}],
            mock_response="The codename is AZURE-FALCON-42.",
            stream=True,
...
def test_rag_query_merges_managed_store_params(client_internal_user):
    ...
    mock_registry.get_derouter_managed_vector_store_from_registry.return_value = mock_vector_store
    ...
    with patch(  # test-quality-ok: aquery is the endpoint's downstream boundary; the forwarded config is what the test asserts
        ...
        new_callable=AsyncMock,
        return_value=mock_response,
    ) as mock_aquery, patch.object(derouter, "vector_store_registry", mock_registry), patch(  # test-quality-ok: seeds the managed-store registry the merge under test reads and grants access so real store resolution runs
        "derouter.proxy.vector_store_endpoints.utils.can_user_access_vector_store",
...
def test_rag_query_store_params_win_over_user_retrieval_config(client_internal_user):
    ...
    mock_vector_store = {
        "vector_store_id": "s3-store",
        "custom_llm_provider": "s3_vectors",
        "derouter_params": {"aws_region_name": "eu-west-1"},
    }
    mock_registry = MagicMock()
    mock_registry.get_derouter_managed_vector_store_from_registry.return_value = mock_vector_store
...
def test_rag_query_forwards_managed_store_credentials_to_search(client_internal_user):
    ...
    mock_vector_store = {
        ...
        "derouter_params": {
            ...
            "api_key": "root:Milvus",
            "derouter_embedding_model": "multilingual-e5-large",
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "openai/gpt-4o-mini", "api_key": "sk-test", "mock_response": "hi"},
            }
        ]
    )

    with (
        patch("derouter.vector_stores.asearch", new=fake_search),  # test-quality-ok: the search boundary under test
        patch.object(derouter, "vector_store_registry", mock_registry),  # test-quality-ok: seeds the store under test
        patch("derouter.proxy.proxy_server.llm_router", router),  # test-quality-ok: mock-response router for completion
    ...
    assert search_kwargs["derouter_embedding_model"] == "multilingual-e5-large"
...
def _multipart_ingest_request(*, filename: str, content: bytes, content_type: str):
    ...
    boundary = "derouteruploadtestboundary"
    head = (
        f"--{boundary}\r\n"
...
class TestVectorStoreUploadControls:
    ...
    async def test_clean_text_upload_gets_server_generated_filename(self):
        from derouter.proxy.rag_endpoints.endpoints import parse_rag_ingest_request
        from derouter.proxy.rag_endpoints.upload_security import EicarTestMalwareScanner
