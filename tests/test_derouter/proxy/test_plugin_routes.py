...
from derouter.proxy.plugin_routes import list_plugins, register_plugins_from_config
...
def test_safe_response_headers_sandbox_and_strips_wire_headers() -> None:
    ...
    from derouter.proxy.plugin_routes import _safe_response_headers
...
def test_derouter_credential_header_names_covers_every_auth_header() -> None:
    """The canonical strip set must list every header user_api_key_auth accepts
    as a derouter key, so a new auth header can't silently start leaking."""
    from derouter.proxy._types import SpecialHeaders

    assert SpecialHeaders.derouter_credential_header_names() == {
        "authorization",
        "api-key",
        "x-api-key",
        "x-goog-api-key",
...
def test_every_derouter_auth_header_is_stripped_before_forwarding() -> None:
    """A plugin must never receive any header that authenticates against derouter,
    only the hop-by-hop set and benign headers are forwarded."""
    from derouter.proxy.plugin_routes import _request_strip_headers

    strip = _request_strip_headers()
    incoming = {
        "Authorization": "Bearer sk-derouter",
        "API-Key": "sk-derouter",
        "X-Api-Key": "sk-derouter",
        "X-Goog-Api-Key": "sk-derouter",
        "Ocp-Apim-Subscription-Key": "sk-derouter",
        "X-Litellm-Api-Key": "sk-derouter",
        "Cookie": "derouter_session=abc",
...
def test_configured_custom_key_header_is_stripped() -> None:
    """A custom general_settings.derouter_key_header_name must also be stripped,
    ...
    proxy_server.general_settings = {"derouter_key_header_name": "X-My-Tenant-Key"}
    try:
