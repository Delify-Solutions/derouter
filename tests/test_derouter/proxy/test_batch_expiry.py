...
def _make_batch_response() -> DeRouterBatch:
...
def test_output_expires_after_passthrough():
    ...
    with patch("derouter.batches.main.openai_batches_instance") as mock_instance:
        mock_instance.create_batch.side_effect = capturing_create
        derouter.create_batch(
            completion_window="24h",
            endpoint="/v1/chat/completions",
            input_file_id="file-abc123",
...
class TestBatchEndpointTeamOverride:
    ...
    def _post_batch(
        ...
        monkeypatch.setattr(derouter, "acreate_batch", mock_acreate_batch)
...
class TestBatchEndpointPolicyMetadata:
    """Batch create must not forward DeRouter policy tracking via OpenAI metadata."""
    ...
    def test_create_batch_does_not_forward_applied_policies_metadata(
        ...
        assert (
            "global-baseline" in captured_kwargs["derouter_metadata"]["applied_policies"]
        )
