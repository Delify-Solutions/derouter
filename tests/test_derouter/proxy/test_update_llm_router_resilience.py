...
def _make_db_model(model_name: str, model_id: str):
    ...
    record.derouter_params = {"model": model_name}
...
class TestDeleteDeploymentKeepsPluginConfigModels:
    """Regression: _delete_deployment re-reads the raw config and hashes derouter_params to
    ...
    async def test_plugin_bearing_config_model_survives_reconcile_and_stale_ids_still_evict(self, tmp_path):
        ...
        resolve_complexity_router_plugins(
            model_name="smart-router",
            complexity_router_config=resolved_entry["derouter_params"]["complexity_router_config"],
        ...
        with (
            ...
            patch("derouter.proxy.proxy_server.llm_router", router),
            patch("derouter.proxy.proxy_server.user_config_file_path", config_file_path),
            patch("derouter.proxy.proxy_server.premium_user", False),
        ):
    ...
    def test_pin_respects_an_explicit_model_id(self):
        from derouter.proxy.proxy_server import pin_complexity_router_model_id
    ...
    def test_pin_is_a_noop_without_a_complexity_router_config(self):
        ...
        entry = {"model_name": "gpt-4o-mini", "derouter_params": {"model": "gpt-4o-mini"}}
        pin_complexity_router_model_id(entry)
