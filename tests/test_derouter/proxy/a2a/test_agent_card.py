"""Unit tests for the pure merge logic in derouter/proxy/a2a/agent_card.py."""
...
def test_unsupported_protocol_version_defaults_to_1_0():
    # The fixture card pins "0.9", which DeRouter does not serve; it falls back to
    # the default rather than advertising a version the proxy can't honor.
    merged = merge_agent_card(
        _full_upstream_card(), proxy_url=PROXY_URL, proxy_base_url=PROXY_BASE
    )
    assert merged["protocolVersion"] == DEROUTER_A2A_PROTOCOL_VERSION
...
def test_replaces_security_schemes_and_requirements():
    ...
    assert merged["securitySchemes"] == DEROUTER_SECURITY_SCHEMES
    assert merged["security"] == DEROUTER_SECURITY_REQUIREMENTS
    ...
    assert merged["supportedInterfaces"] == [
        {
            ...
            "protocolVersion": DEROUTER_A2A_PROTOCOL_VERSION,
        }
    ]
...
def test_falls_back_to_derouter_provider_when_upstream_lacks_one():
    sparse = {"name": "x", "description": "y", "version": "1"}
    merged = merge_agent_card(sparse, proxy_url=PROXY_URL, proxy_base_url=PROXY_BASE)
    assert merged["provider"] == {
        "organization": "DeRouter Proxy",
        "url": PROXY_BASE,
    }
...
def test_resolve_served_protocol_version_falls_back_for_unsupported():
    ...
    assert resolve_served_protocol_version(None) == DEROUTER_A2A_PROTOCOL_VERSION
