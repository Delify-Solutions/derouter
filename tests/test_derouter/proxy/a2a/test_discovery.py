...
from derouter.proxy.a2a.endpoints import router as a2a_router
...
async def test_fetch_uses_first_path_that_returns_200():
    ...
    with patch(
        "derouter.proxy.a2a.discovery.get_async_httpx_client", return_value=fake_client
    ):
...
def test_discover_admin_returns_raw_card():
    ...
    with patch(
        "derouter.proxy.a2a.endpoints.fetch_well_known_card",
...
def test_discover_forwards_mode_and_params_to_fetcher():
    ...
    with patch("derouter.proxy.a2a.endpoints.fetch_well_known_card", new=fetch_stub):
        resp = client.post(
            "/v1/a2a/discover",
            json={
