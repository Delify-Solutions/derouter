...
from derouter.secret_managers.main import get_secret_str as real_get_secret_str
...
def _patch_pyroscope_grafana_secrets(user: Optional[str], token: Optional[str]):
    ...
    return patch(
        "derouter.proxy.proxy_server.get_secret_str",
        side_effect=side_effect,
    )


def test_init_pyroscope_returns_cleanly_when_disabled():
    """When DEROUTER_ENABLE_PYROSCOPE is false, _init_pyroscope returns without error."""
    with (
        patch(
            "derouter.proxy.proxy_server.get_secret_bool",
            return_value=False,
        ),
        patch.dict(
            os.environ,
            {"DEROUTER_ENABLE_PYROSCOPE": "false"},
            clear=False,
        ),
    ):
...
def test_init_pyroscope_raises_when_enabled_but_missing_app_name():
    """When DEROUTER_ENABLE_PYROSCOPE is true but PYROSCOPE_APP_NAME is not set, raises ValueError."""
    ...
    with (
        ...
        patch.dict(
            os.environ,
            {
                "DEROUTER_ENABLE_PYROSCOPE": "true",
                "PYROSCOPE_APP_NAME": "",
...
def test_init_pyroscope_raises_when_enabled_but_missing_server_address():
    """When DEROUTER_ENABLE_PYROSCOPE is true but PYROSCOPE_SERVER_ADDRESS is not set, raises ValueError."""
    mock_pyroscope = _mock_pyroscope_module()
    with (
        patch(
            "derouter.proxy.proxy_server.get_secret_bool",
