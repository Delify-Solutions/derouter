...
``derouter_settings.success_callback: ["s3://attacker/m.i"]`` via
...
def test_derouter_settings_callback_list_strips_remote_urls(field):
    overlay = {field: ["langfuse", "s3://attacker/m.i", "gcs://attacker/m.i"]}
    cleaned = _scrub_db_overlay_remote_module_loads("derouter_settings", overlay)
...
def test_derouter_settings_post_call_rules_str_stripped():
...
def test_derouter_settings_guardrails_v1_callbacks_stripped():
...
def test_derouter_settings_guardrails_v2_callbacks_and_guardrail_stripped():
    # v2 shape: {guardrail_name, derouter_params: {guardrail: "module.path", callbacks: [...]}}
    overlay = {
        "guardrails": [
            {
    ...
    cleaned = _scrub_db_overlay_remote_module_loads("derouter_settings", overlay)
    lp = cleaned["guardrails"][0]["derouter_params"]
    assert lp["guardrail"] is None
    assert lp["callbacks"] == ["lakera"]
    assert lp["mode"] == "pre_call"


def test_derouter_settings_guardrails_local_dotted_name_preserved():
    overlay = {
        "guardrails": [
            {
                "guardrail_name": "custom",
...
def test_derouter_settings_guardrails_non_list_passthrough():
    cleaned = _scrub_db_overlay_remote_module_loads(
        "derouter_settings", {"guardrails": "not-a-list"}
    )
...
def test_derouter_jwtauth_custom_validate_stripped():
    overlay = {
        "derouter_jwtauth": {
    ...
    cleaned = _scrub_db_overlay_remote_module_loads("general_settings", overlay)
    assert cleaned["derouter_jwtauth"]["custom_validate"] is None
    # Sibling fields preserved.
    assert cleaned["derouter_jwtauth"]["user_id_jwt_field"] == "sub"
...
def test_non_dict_overlay_passthrough():
    ...
    assert _scrub_db_overlay_remote_module_loads("derouter_settings", 100.0) == 100.0
    assert _scrub_db_overlay_remote_module_loads("derouter_settings", None) is None
...
def test_scrub_does_not_mutate_input():
    ...
    _scrub_db_overlay_remote_module_loads("derouter_settings", original)
