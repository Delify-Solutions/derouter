...
unaffected — the documented ``derouter_settings.callbacks:
...
from derouter.proxy.types_utils.utils import get_instance_fn  # noqa: E402
...
def test_remote_url_with_config_file_path_is_allowed():
    ...
    # working.
    with patch(
        "derouter.proxy.types_utils.utils._load_instance_from_remote_storage",
        return_value="loaded",
    ) as mock_loader:
        result = get_instance_fn(
            value="s3://my-bucket/m.inst",
            config_file_path="/etc/derouter/config.yaml",
        )

    assert result == "loaded"
    mock_loader.assert_called_once_with(
        "s3://my-bucket/m.inst", "/etc/derouter/config.yaml"
    )
...
def test_dotted_module_path_is_unaffected_by_gate():
    ...
    with patch(
        "derouter.proxy.types_utils.utils.importlib.import_module"
    ) as mock_import:
...
def test_pass_through_route_threads_config_file_path():
    ...
    from derouter.proxy.pass_through_endpoints import pass_through_endpoints as pte

    # ``get_instance_fn`` is imported lazily inside the function — patch
    # at the source so the deferred import resolves to the mock.
    with patch(
        "derouter.proxy.types_utils.utils.get_instance_fn", return_value=object()
    ) as mock_get:
...
def test_mcp_tool_registry_threads_config_file_path():
    ...
    from derouter.proxy._experimental.mcp_server import tool_registry as tr
    ...
    mock_get.assert_called_once_with(
        "s3://bucket/mod.handler", "/etc/derouter/config.yaml"
    )
