...
def mock_redis_success(mocker):
    ...
    mocker.patch.object(derouter, "cache", mock_cache)
    return mock_cache
...
def test_cache_ping_with_complex_objects(mock_redis_success, mocker):
    ...
    assert data["status"] == "healthy"
    assert "derouter_cache_params" in data

    # Verify complex objects were converted to strings
    cache_params = json.loads(data["derouter_cache_params"])
...
def test_cache_ping_failure(mock_redis_failure):
    ...
    assert "derouter_cache_params" in error_details
...
def test_cache_ping_no_cache_initialized():
    ...
    try:
        ...
        assert (
            error_details["message"] == "Cache not initialized. derouter.cache is None"
        )
    finally:
...
def test_cache_ping_health_check_includes_only_cache_attributes(mock_redis_success):
    """
    Ensure that the /cache/ping endpoint only pulls HealthCheckCacheParams from derouter.cache.cache,
    and not from other attributes on derouter.cache.
    """
    ...
    # The unrelated field we attached at the top-level of derouter.cache should *not* be present
    assert (
    ...
    ), "Expected field on `derouter.cache.cache` was not found in health_check_cache_params"
