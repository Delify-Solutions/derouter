...
def pytest_runtest_setup(item):
    """
    Snapshot module-level globals on derouter.proxy.proxy_server before any
...
def _reset_graceful_shutdown_state():
    ...
    from derouter.proxy.shutdown.graceful_shutdown_manager import (
...
def build_minimal_proxy_config(
    ...
    if cache_config:
        config["derouter_settings"].update(cache_config)

    # Add success_callback if provided (for realistic readiness endpoint)
    if init_options.get("success_callback") is not None:
        config["derouter_settings"]["success_callback"] = init_options[
            "success_callback"
        ]

    # Add any other derouter_settings from init_options
    excluded_keys = {
        "master_key",
        "debug",
        "success_callback",
    ...
    for key, value in init_options.items():
        if key not in excluded_keys and key not in config["derouter_settings"]:
            config["derouter_settings"][key] = value
