...
Related PR: https://github.com/Delify-Solutions/derouter/pull/12577
...
def _make_token(token: str, user_id: str = "user-123") -> DeRouter_VerificationToken:
...
def _mock_prisma(keys, deleted_tokens):
    ...
    mock = AsyncMock()
    mock.db.derouter_verificationtoken.find_many = AsyncMock(return_value=keys)
    mock.delete_data = AsyncMock(return_value=deleted_tokens)
    mock.db.derouter_deletedverificationtoken.create_many = AsyncMock()
    return mock
...
async def test_delete_tokens_non_admin_token_not_in_db_returns_failed_tokens(
    ...
    mock_prisma.db.derouter_verificationtoken.find_many = AsyncMock(return_value=[key1])
    mock_prisma.delete_data = AsyncMock(return_value=["hashed-token-1"])
    mock_prisma.db.derouter_deletedverificationtoken.create_many = AsyncMock()
