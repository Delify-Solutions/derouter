...
from derouter.proxy.common_utils.callback_config_validation import cross_entry_family_error
...
def _patch_prisma(existing_team: MagicMock):
    ...
    mock_prisma.db.derouter_teamtable.update = AsyncMock(return_value=updated_row)
    return mock_prisma
...
def stub_team_cache_refresh():
    ...
    That helper validates a real Prisma row into DeRouter_TeamTableCachedObj,
    ...
    """
    with patch(
        "derouter.proxy.management_endpoints.team_callback_endpoints._refresh_cached_team",
...
def patched_prisma():
    with (
        patch("derouter.proxy.proxy_server.prisma_client") as mock_client,
        patch(
        ...
        mock_client.db.derouter_teamtable.update = AsyncMock()
        yield mock_client
...
async def test_add_team_callbacks_rejects_unauthorized_caller(
    ...
    patched_prisma.db.derouter_teamtable.update.assert_not_called()
...
async def test_proxy_admin_can_add_team_callbacks(patched_prisma):
    ...
    patched_prisma.db.derouter_teamtable.update.assert_awaited_once()
...
async def test_disable_team_logging_emits_audit_log_when_enabled(monkeypatch):
    ...
    with (
        ...
        patch(
            "derouter.proxy.management_helpers.audit_logs.create_audit_log_for_update",
            new=capture,
        ),
    ):
        await disable_team_logging(
...
async def test_add_team_callbacks_emits_audit_log_when_enabled(monkeypatch):
    ...
    with (
        ...
        await add_team_callbacks(
            ...
            team_id="team-1",
            user_api_key_dict=_admin_auth(),
            derouter_changed_by="ops-on-call",
        )
        import asyncio
...
async def test_add_team_callbacks_writes_encrypted_callback_vars(monkeypatch):
    ...
    written = json.loads(
        mock_prisma.db.derouter_teamtable.update.await_args.kwargs["data"]["metadata"]
    )
...
async def test_get_team_callbacks_returns_callbacks_registered_via_post(monkeypatch):
    ...
    with (
        ...
        row.metadata = json.loads(mock_prisma.db.derouter_teamtable.update.await_args.kwargs["data"]["metadata"])
    ...
    assert "derouter_enc::" not in json.dumps(response)
...
async def test_get_team_callbacks_decrypts_vars_stored_under_non_sensitive_keys(monkeypatch):
    ...
    the decrypt step that value comes back as an unusable derouter_enc:: blob.
    """
    from derouter.proxy.common_utils.callback_utils import _CALLBACK_VAR_ENCRYPTED_PREFIX, is_sensitive_callback_key
...
async def test_get_team_callbacks_masks_values_that_fail_to_decrypt(monkeypatch):
    ...
    """
    from derouter.proxy.common_utils.callback_utils import _CALLBACK_VAR_ENCRYPTED_PREFIX
    from derouter.proxy.common_utils.encrypt_decrypt_utils import encrypt_value_helper

    monkeypatch.setenv("DEROUTER_SALT_KEY", "test-salt-32-bytes-aaaaaaaaaaaaaa")
    stale = _CALLBACK_VAR_ENCRYPTED_PREFIX + encrypt_value_helper("tenant-project")
    monkeypatch.setenv("DEROUTER_SALT_KEY", "test-salt-32-bytes-bbbbbbbbbbbbbb")
...
async def test_disable_team_logging_stops_callbacks_registered_via_api():
    ...
    row would do.
    """
    from derouter.proxy.derouter_pre_call_utils import _get_dynamic_logging_metadata
    ...
    written = json.loads(mock_prisma.db.derouter_teamtable.update.await_args.kwargs["data"]["metadata"])
...
async def test_disable_team_logging_refreshes_cached_team(stub_team_cache_refresh):
    ...
    assert refreshed is mock_prisma.db.derouter_teamtable.update.return_value
...
async def test_delete_team_callback_404s_for_unregistered_callback():
    ...
    mock_prisma.db.derouter_teamtable.update.assert_not_called()
...
async def test_add_team_callbacks_rejects_team_deleted_before_write():
    ...
    """
    mock_prisma = _patch_prisma(_team_row(team_id="team-1", metadata={}))
    mock_prisma.db.derouter_teamtable.update = AsyncMock(return_value=None)
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.master_key", None),  # test-quality-ok: proxy_server module global is the endpoint's only injection point
    ):
        with pytest.raises(HTTPException) as exc:
            await add_team_callbacks(
                data=data,
    ...
    mock_prisma.db.derouter_teamtable.update.assert_called_once()
...
async def test_delete_team_callback_route_accepts_team_ids_containing_slashes():
    ...
    from derouter.proxy.management_endpoints.team_callback_endpoints import router
...
async def test_unknown_team_is_indistinguishable_from_no_access(call_handler, unauthorized_caller):
    ...
    """
    with patch("derouter.proxy.proxy_server.prisma_client") as mock_client:  # test-quality-ok: the handler imports prisma_client from proxy_server at call time, so there is no seam to inject through
