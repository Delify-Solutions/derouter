...
from derouter.proxy.auth.user_api_key_auth import UserAPIKeyAuth, user_api_key_auth
from derouter.proxy.management_endpoints.customer_endpoints import router
from derouter.types.proxy.management_endpoints.common_daily_activity import (
    SpendAnalyticsPaginatedResponse,
)
from derouter.types.proxy.management_endpoints.customer_endpoints import (
    BlockUsersResponse,
    CustomerResponse,
...
def test_update_customer_success(mock_prisma_client, mock_user_api_key_auth):
    ...
    mock_end_user = DeRouter_EndUserTable(user_id="test-user-1", alias="Test User", blocked=False)
    updated_mock_end_user = DeRouter_EndUserTable(user_id="test-user-1", alias="Updated Test User", blocked=False)

    # Mock the find_first response
    mock_prisma_client.db.derouter_endusertable.find_first = AsyncMock(return_value=mock_end_user)

    # Mock the update response
    mock_prisma_client.db.derouter_endusertable.update = AsyncMock(return_value=updated_mock_end_user)
...
def test_update_customer_unblock(mock_prisma_client, mock_user_api_key_auth):
    mock_end_user = DeRouter_EndUserTable(user_id="test-user-1", blocked=True)
    updated_mock_end_user = DeRouter_EndUserTable(user_id="test-user-1", blocked=False)
    ...
    update_mock = mock_prisma_client.db.derouter_endusertable.update
...
def test_update_customer_keeps_blocked_when_omitted(mock_prisma_client, mock_user_api_key_auth):
    ...
    """
    mock_end_user = DeRouter_EndUserTable(user_id="test-user-1", blocked=True)
    updated_mock_end_user = DeRouter_EndUserTable(user_id="test-user-1", blocked=True)
...
def test_update_customer_not_found(mock_prisma_client, mock_user_api_key_auth):
    ...
    mock_prisma_client.db.derouter_endusertable.find_first = AsyncMock(return_value=None)
...
def test_delete_customer_not_found(mock_prisma_client, mock_user_api_key_auth):
    ...
    mock_prisma_client.db.derouter_endusertable.find_many = AsyncMock(return_value=[])
...
def test_error_schema_consistency(mock_prisma_client, mock_user_api_key_auth):
    ...
    mock_prisma_client.db.derouter_endusertable.create = AsyncMock(
...
def test_update_customer_response_preserves_budget_id(mock_prisma_client, mock_user_api_key_auth):
    ...
    DeRouter_EndUserTable must NOT drop it, so budget_id stays in DeRouter_EndUserTable.
    """
    existing = DeRouter_EndUserTable(user_id="cust-1", blocked=False)
    updated = DeRouter_EndUserTable(user_id="cust-1", blocked=False, budget_id="budget-123")
    mock_prisma_client.db.derouter_endusertable.find_first = AsyncMock(return_value=existing)
    mock_prisma_client.db.derouter_endusertable.update = AsyncMock(return_value=updated)
...
def test_update_customer_response_keeps_nested_budget_server_fields(mock_prisma_client, mock_user_api_key_auth):
    ...
    mock_prisma_client.db.derouter_endusertable.find_first = AsyncMock(return_value=existing)
    mock_prisma_client.db.derouter_endusertable.update = AsyncMock(return_value=raw_row)
    ...
    assert response.status_code == 200
    budget = response.json()["derouter_budget_table"]
...
def test_block_customer_success_serializes_through_response_model(mock_prisma_client, mock_user_api_key_auth):
    ...
    """
    blocked_row = DeRouter_EndUserTable(user_id="blocked-1", blocked=True)
    mock_prisma_client.db.derouter_endusertable.upsert = AsyncMock(return_value=blocked_row)
...
def test_delete_customer_success_serializes_through_response_model(mock_prisma_client, mock_user_api_key_auth):
    ...
    """
    existing = [
        DeRouter_EndUserTable(user_id="u1", blocked=False),
        DeRouter_EndUserTable(user_id="u2", blocked=False),
    ]
    mock_prisma_client.db.derouter_endusertable.find_many = AsyncMock(return_value=existing)
    mock_prisma_client.db.derouter_endusertable.delete_many = AsyncMock(return_value=2)
...
async def test_get_customer_daily_activity_admin_param_passing(monkeypatch):
    ...
    from derouter.proxy.management_endpoints import customer_endpoints
    ...
    assert kwargs["table_name"] == "derouter_dailyenduserspend"
...
async def test_get_customer_daily_activity_with_end_user_aliases(monkeypatch):
    ...
    mock_prisma_client.db.derouter_endusertable.find_many = AsyncMock(return_value=[mock_end_user1, mock_end_user2])
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
...
async def test_get_customer_daily_activity_non_admin_is_rejected(monkeypatch):
    ...
    DeRouter_DailyEndUserSpend table.

    DeRouter_EndUserTable has no per-tenant ownership column, so non-admin
...
async def test_get_customer_daily_activity_service_account_key_is_rejected(monkeypatch):
    ...
    return the full DeRouter_DailyEndUserSpend table with no WHERE clause.
    """
    from derouter.proxy.management_endpoints import customer_endpoints
    from derouter.proxy.management_endpoints.customer_endpoints import (
        get_customer_daily_activity,
...
def test_char_info_body(mock_prisma_client, mock_user_api_key_auth):
    mock_prisma_client.db.derouter_endusertable.find_first = AsyncMock(return_value=_row(_FULL_DB_ROW))
...
def test_char_list_body(mock_prisma_client, mock_user_api_key_auth):
    mock_prisma_client.db.derouter_endusertable.find_many = AsyncMock(return_value=[_row(_FULL_DB_ROW)])
...
def test_char_new_body(mock_prisma_client, mock_user_api_key_auth):
    mock_prisma_client.db.derouter_endusertable.create = AsyncMock(return_value=_row(_FULL_DB_ROW))
...
def test_customer_new_rejects_a_duration_that_never_advances(
    ...
    mock_prisma_client.db.derouter_endusertable.create.assert_not_awaited()


def test_customer_new_accepts_a_normal_duration(mock_prisma_client, mock_user_api_key_auth):
    mock_prisma_client.db.derouter_endusertable.create = AsyncMock(return_value=_row(_FULL_DB_ROW))
    mock_prisma_client.db.derouter_budgettable.create = AsyncMock(
...
def test_char_update_body(mock_prisma_client, mock_user_api_key_auth):
    mock_prisma_client.db.derouter_endusertable.find_first = AsyncMock(
        return_value=_row({"user_id": "c1", "blocked": False})
    )
    mock_prisma_client.db.derouter_endusertable.update = AsyncMock(return_value=_row(_FULL_DB_ROW))
    response = client.post(
        "/customer/update",
...
def test_char_delete_body(mock_prisma_client, mock_user_api_key_auth):
    mock_prisma_client.db.derouter_endusertable.find_many = AsyncMock(
        return_value=[
            DeRouter_EndUserTable(user_id="c1", blocked=False),
            DeRouter_EndUserTable(user_id="c2", blocked=False),
        ]
    )
    mock_prisma_client.db.derouter_endusertable.delete_many = AsyncMock(return_value=2)
    response = client.post(
...
def _end_user_cache_doubles():
    ...
    with (
        patch("derouter.proxy.proxy_server.user_api_key_cache", recording_cache),
        patch(
...
def test_customer_block_invalidates_end_user_and_registry_caches(mock_prisma_client, mock_user_api_key_auth):
    ...
    mock_prisma_client.db.derouter_endusertable.upsert = AsyncMock(
        return_value=DeRouter_EndUserTable(user_id="c1", blocked=True)
    )
