...
import derouter
from derouter.proxy.management_endpoints.cost_tracking_settings import router
...
class TestCostTrackingSettings:
    ...
    async def test_update_cost_discount_config_success(self):
        ...
        with (
            ...
            patch.object(derouter, "cost_discount_config", {}),
        ):
            # Make request
            ...
            # Verify derouter.cost_discount_config was updated
            assert derouter.cost_discount_config == test_discount_config
...
class TestResolveModelForCostLookup:
    ...
    def test_resolves_base_model_for_azure_deployment(self):
        ...
        instead of the raw derouter_params.model (Azure deployment name).
        """
    ...
    def test_falls_back_to_derouter_params_model_when_no_base_model(self):
        """
        When no base_model is set, should fall back to derouter_params.model.
        """
        from derouter.proxy.management_endpoints.cost_tracking_settings import (
            _resolve_model_for_cost_lookup,
        )
    ...
    def test_resolves_base_model_from_derouter_params(self):
        """
        When base_model is in derouter_params (not model_info),
    ...
    def test_returns_custom_llm_provider_on_base_model_path(self):
        """base_model path: the custom_llm_provider from derouter_params is
    ...
    def test_returns_custom_llm_provider_on_resolved_model_path(self):
        ...
        derouter_params is returned alongside derouter_params.model."""
        from derouter.proxy.management_endpoints.cost_tracking_settings import (
            _resolve_model_for_cost_lookup,
        )
    ...
    def test_resolves_base_model_when_deployment_has_no_derouter_params(self):
        """A deployment can omit derouter_params entirely; base_model from
        model_info must still resolve (the .get default must be {} not None,
        else the later derouter_params.get(...) raises and resolution is lost)."""
        from derouter.proxy.management_endpoints.cost_tracking_settings import (
            _resolve_model_for_cost_lookup,
        )
    ...
    def test_resolves_model_when_deployment_has_no_model_info(self):
        """A deployment can omit model_info entirely; derouter_params.model must
...
class TestEstimateCostOnPremProvider:
    ...
    async def test_estimate_cost_onprem_model_without_pricing(self):
        ...
        """
        from derouter.proxy._types import CostEstimateRequest
        from derouter.proxy.management_endpoints.cost_tracking_settings import (
            estimate_cost,
        )
    ...
    async def test_estimate_cost_onprem_model_with_model_info_pricing(self):
        ...
        deployments store it) must be honored, not just derouter_params pricing.
    ...
    @pytest.mark.asyncio
    async def test_estimate_cost_derouter_params_pricing_overrides_model_info(self):
        """
        When pricing is set in both places, derouter_params wins, matching the
...
class TestBlockRequestsForModelsWithoutPricing:
    ...
    async def test_get_reflects_in_memory_flag(self):
        with patch.object(derouter, "block_requests_for_models_without_pricing", True):
            response = client.get(
    ...
    async def test_patch_persists_and_updates_flag(self):
        ...
        with (
            ...
            patch.object(derouter, "block_requests_for_models_without_pricing", False),
        ):
            response = client.patch(
                "/config/block_requests_for_models_without_pricing",
                headers={"Authorization": "Bearer sk-1234"},
            ...
            assert response.status_code == 200
            assert response.json() == {"enabled": True}
            assert derouter.block_requests_for_models_without_pricing is True

        saved_config = mock_proxy_config.save_config.call_args.kwargs["new_config"]
        assert saved_config["derouter_settings"]["block_requests_for_models_without_pricing"] is True
    ...
    def test_peer_workers_pick_up_persisted_flag_on_config_reload(self):
        ...
        persisted value up when they reload derouter_settings from the DB."""
        from derouter.proxy.proxy_server import ProxyConfig

        with patch.object(derouter, "block_requests_for_models_without_pricing", False):
    ...
    async def test_periodic_db_sync_applies_flag_to_peer_worker(self, loads_config_overrides):
        ...
        with (
            ...
            patch("derouter.proxy.proxy_server.get_config_param", AsyncMock(return_value=config_record)),
        ):
            await ProxyConfig()._init_non_llm_objects_in_db(prisma_client=MagicMock())

            assert derouter.block_requests_for_models_without_pricing is True
            assert not hasattr(derouter, "unsafe_key")

    @pytest.mark.asyncio
    async def test_patch_requires_store_model_in_db(self):
        with (
            patch("derouter.proxy.proxy_server.prisma_client", MagicMock()),
            patch("derouter.proxy.proxy_server.proxy_config", AsyncMock()),
...
async def _estimate(mock_router: MagicMock | None, model: str = AN_ALIAS, **overrides: int):
    from derouter.proxy._types import CostEstimateRequest
    from derouter.proxy.management_endpoints.cost_tracking_settings import estimate_cost
    ...
    with patch(  # test-quality-ok: proxy_server module global is the endpoint's only injection point
        "derouter.proxy.proxy_server.llm_router", mock_router
    ):
