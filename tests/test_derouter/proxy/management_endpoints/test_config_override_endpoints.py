...
from derouter.proxy._types import KeyManagementSystem, LitellmUserRoles, UserAPIKeyAuth
...
from derouter.types.proxy.management_endpoints.config_overrides import (
    CyberArkConfig,
...
def _make_mock_db():
    ...
    prisma = MagicMock()
    prisma.db.derouter_configoverrides = mock
    return prisma, mock
...
async def test_hashicorp_vault_crud_lifecycle(client, monkeypatch):
    ...
    old_client, old_kms = derouter.secret_manager_client, derouter._key_management_system
    _set_admin()
    ...
    try:
        ...
        derouter.secret_manager_client = MagicMock()
        derouter._key_management_system = KeyManagementSystem.HASHICORP_VAULT
        r = client.delete(VAULT_URL)
        assert r.status_code == 200
        assert os.environ.get("HCP_VAULT_ADDR") is None
        assert derouter.secret_manager_client is None
        ...
        derouter.secret_manager_client = old_client
        derouter._key_management_system = old_kms
        _cleanup()
...
async def test_hashicorp_vault_validation_errors_and_access_control(
    ...
    try:
        ...
        aws = MagicMock()
        derouter.secret_manager_client = aws
        derouter._key_management_system = KeyManagementSystem.AWS_SECRET_MANAGER
        assert client.delete(VAULT_URL).status_code == 200
        assert derouter.secret_manager_client is aws
        assert derouter._key_management_system == KeyManagementSystem.AWS_SECRET_MANAGER
    ...
    try:
        ...
        r = client.post(
            ...
            json={
                ...
                "cyberark_username": "derouter-user",
        ...
        derouter.secret_manager_client = MagicMock()  # test-quality-ok: endpoint hot-reloads derouter globals; test must set and restore them
        derouter._key_management_system = KeyManagementSystem.CYBERARK  # test-quality-ok: endpoint hot-reloads derouter globals; test must set and restore them
        ...
        derouter.secret_manager_client = old_client  # test-quality-ok: endpoint hot-reloads derouter globals; test must set and restore them
        derouter._key_management_system = old_kms  # test-quality-ok: endpoint hot-reloads derouter globals; test must set and restore them
        _cleanup()
...
async def test_cyberark_validation_errors_and_access_control(client, monkeypatch):
    ...
    try:
        ...
        aws = MagicMock()
        derouter.secret_manager_client = aws  # test-quality-ok: endpoint hot-reloads derouter globals; test must set and restore them
        derouter._key_management_system = KeyManagementSystem.AWS_SECRET_MANAGER  # test-quality-ok: endpoint hot-reloads derouter globals; test must set and restore them
        assert client.delete(CYBERARK_URL).status_code == 200
        assert derouter.secret_manager_client is aws
...
async def test_cyberark_persist_failure_restores_hashicorp_manager(client, monkeypatch):
    ...
    def _fake_init(key_management_system):
        derouter._key_management_system = (  # test-quality-ok: endpoint hot-reloads derouter globals; test must set and restore them
    ...
    try:
        ...
        derouter._key_management_system = KeyManagementSystem.HASHICORP_VAULT  # test-quality-ok: endpoint hot-reloads derouter globals; test must set and restore them
        ...
        assert r.status_code == 500
        assert derouter._key_management_system == KeyManagementSystem.HASHICORP_VAULT
        assert (
...
async def test_cyberark_test_connection(client, monkeypatch):
    ...
    from derouter.secret_managers.cyberark_secret_manager import CyberArkSecretManager
    ...
    try:
        # Not configured → 400
        derouter.secret_manager_client = None  # test-quality-ok: endpoint hot-reloads derouter globals; test must set and restore them
        ...
        derouter.secret_manager_client = mock_manager  # test-quality-ok: endpoint hot-reloads derouter globals; test must set and restore them
        ...
        with patch(  # test-quality-ok: patching proxy-internal collaborator to isolate the endpoint
            "derouter.proxy.management_endpoints.config_override_endpoints.get_async_httpx_client",
...
class TestHashicorpVaultAuditLog:
    ...
    ``derouter.store_audit_logs`` is True, with credential values redacted
    ...
    async def test_post_action_is_updated_when_row_exists_with_null_config_value(
        ...
        """A row can exist in ``derouter_configoverrides`` with a NULL
