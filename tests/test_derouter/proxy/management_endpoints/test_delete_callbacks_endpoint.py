...
class MockPrismaClient:
    def __init__(self):
        ...
        self.db.derouter_config.upsert = AsyncMock()

        # Mock config retrieval for get_config/callbacks
        self.db.derouter_config.find_first = AsyncMock(side_effect=self._mock_find_first)
    ...
    async def _mock_find_first(self, where=None):
        ...
        if where and "param_name" in where:
            ...
            if param_name == "derouter_settings":
                return MagicMock(
                    param_name="derouter_settings",
                    param_value=self.config_data["derouter_settings"],
                )
    ...
    def remove_callback_from_config(self, callback_name):
        ...
        if "success_callback" in self.config_data["derouter_settings"]:
            callbacks = self.config_data["derouter_settings"]["success_callback"]
