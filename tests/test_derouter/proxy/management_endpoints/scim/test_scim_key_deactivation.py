...
def _build_prisma_with_keys(user_keys, mock_user=None, updated_user=None):
    ...
    if mock_user is not None:
        mock_db.derouter_usertable.find_unique = AsyncMock(return_value=mock_user)
    if updated_user is not None:
        mock_db.derouter_usertable.update = AsyncMock(return_value=updated_user)
    mock_db.derouter_usertable.delete = AsyncMock(return_value=None)
    mock_db.derouter_teamtable.find_unique = AsyncMock(return_value=None)

    mock_db.derouter_verificationtoken.find_many = AsyncMock(return_value=user_keys)
    mock_db.derouter_verificationtoken.update_many = AsyncMock(return_value=None)
    mock_db.derouter_verificationtoken.update = AsyncMock(return_value=None)
    mock_db.derouter_invitationlink.delete_many = AsyncMock(return_value=None)
    mock_db.derouter_organizationmembership.delete_many = AsyncMock(return_value=None)
    mock_db.derouter_teammembership.delete_many = AsyncMock(return_value=None)
...
async def test_set_user_keys_blocked_flips_state_and_invalidates_cache():
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client", mock_client),
        ...
        patch(
            "derouter.proxy.management_endpoints.scim.scim_v2._delete_cache_key_object",
    ...
    assert mock_db.derouter_verificationtoken.update.await_count == 2
    update_calls = mock_db.derouter_verificationtoken.update.await_args_list
    seen_tokens = set()
...
async def test_set_user_keys_blocked_noop_when_no_matching_keys():
    ...
    assert flipped == 0
    mock_db.derouter_verificationtoken.update.assert_not_called()
    mock_db.derouter_verificationtoken.update_many.assert_not_called()
...
async def test_set_user_keys_unblocked_skips_admin_blocked_keys():
    ...
    assert flipped == 1
    mock_db.derouter_verificationtoken.update.assert_awaited_once()
    update_kwargs = mock_db.derouter_verificationtoken.update.await_args.kwargs
...
async def test_scim_delete_user_blocks_keys_before_deleting_user():
    ...
    mock_db.derouter_usertable.delete.assert_awaited_once_with(
...
async def test_scim_delete_user_clears_fk_referenced_rows_before_user_delete():
    ...
    call_order: list = []
    mock_db.derouter_invitationlink.delete_many = AsyncMock(
        side_effect=lambda **kw: call_order.append(("invitation", kw)) or None
    )
    mock_db.derouter_organizationmembership.delete_many = AsyncMock(
        side_effect=lambda **kw: call_order.append(("orgmembership", kw)) or None
    )
    mock_db.derouter_teammembership.delete_many = AsyncMock(
        side_effect=lambda **kw: call_order.append(("teammembership", kw)) or None
    )
    mock_db.derouter_usertable.delete = AsyncMock(
    ...
    mock_db.derouter_invitationlink.delete_many.assert_awaited_once()
    inv_kwargs = mock_db.derouter_invitationlink.delete_many.await_args.kwargs
    assert inv_kwargs == {
        "where": {
            "OR": [
                {"user_id": user_id},
    ...
    mock_db.derouter_organizationmembership.delete_many.assert_awaited_once_with(
        where={"user_id": user_id}
    )
    mock_db.derouter_teammembership.delete_many.assert_awaited_once_with(
        where={"user_id": user_id}
    )
...
async def test_scim_patch_user_no_active_change_does_not_touch_keys():
    ...
    mock_db.derouter_verificationtoken.find_many.assert_not_called()
    mock_db.derouter_verificationtoken.update_many.assert_not_called()
...
async def test_scim_put_user_omitting_active_preserves_deactivated_state():
    ...
    user_id = "scim-user"
    deactivated = DeRouter_UserTable(
        user_id=user_id,
        user_email="x@example.com",
        user_alias=None,
        teams=[],
    ...
    mock_db.derouter_verificationtoken.update.assert_not_called()
    mock_db.derouter_verificationtoken.update_many.assert_not_called()
    mock_db.derouter_usertable.update.assert_awaited_once()
    update_kwargs = mock_db.derouter_usertable.update.await_args.kwargs
