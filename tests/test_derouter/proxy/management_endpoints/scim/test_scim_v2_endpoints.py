...
from derouter.proxy.management_endpoints.scim.scim_v2 import (
    SCIMRosterSyncError,
...
from derouter.types.proxy.management_endpoints.scim_v2 import (
...
async def test_create_user_existing_user_conflict(mocker):
    ...
    mock_prisma_client.db.derouter_usertable = mocker.MagicMock()
    mock_prisma_client.db.derouter_usertable.find_unique = AsyncMock(return_value={"user_id": "existing-user"})
    mock_prisma_client.db.derouter_usertable.find_many = AsyncMock(return_value=())

    # Mock the _get_prisma_client_or_raise_exception to return our mock
    mocker.patch(
        "derouter.proxy.management_endpoints.scim.scim_v2._get_prisma_client_or_raise_exception",
        AsyncMock(return_value=mock_prisma_client),
    )

    mocked_new_user = mocker.patch(
        "derouter.proxy.management_endpoints.scim.scim_v2.new_user",
        AsyncMock(),
    )
...
async def test_create_user_defaults_to_viewer(mocker, monkeypatch):
    ...
    monkeypatch.setattr("derouter.default_internal_user_params", None, raising=False)
    ...
    mocker.patch(
        "derouter.proxy.management_endpoints.scim.scim_v2.ScimTransformations.transform_derouter_user_to_scim_user",
...
async def test_create_user_uses_default_internal_user_params_role(mocker, monkeypatch):
    ...
    monkeypatch.setattr("derouter.default_internal_user_params", default_params, raising=False)
...
async def test_create_user_if_not_exists_defers_to_default_team(mocker: MockerFixture, monkeypatch):
    ...
    new_user_mock = mocker.patch(  # test-quality-ok: new_user is imported inside the helper, not injectable
        "derouter.proxy.management_endpoints.internal_user_endpoints.new_user",
...
async def test_scim_create_user_respects_default_role_set_via_ui(mocker, monkeypatch):
    ...
    The UI saves the setting via _update_derouter_setting, which should update
    derouter.default_internal_user_params in memory. Then SCIM create_user
    ...
    """
    from derouter.proxy._types import DefaultInternalUserParams
    from derouter.proxy.ui_crud_endpoints.proxy_setting_endpoints import (
        _update_derouter_setting,
    )
    ...
    # Mock the proxy_config and store_model_in_db that _update_derouter_setting needs
    mock_proxy_config = mocker.MagicMock()
    mock_proxy_config.get_config = AsyncMock(return_value={"derouter_settings": {}})
    ...
    await _update_derouter_setting(
        settings=settings,
    ...
    assert derouter.default_internal_user_params is not None, (
        "BUG: _update_derouter_setting did not update derouter.default_internal_user_params in memory. "
        "The local variable reassignment (in_memory_var = ...) doesn't propagate back."
    )
    assert derouter.default_internal_user_params.get("user_role") == LitellmUserRoles.INTERNAL_USER
    ...
    assert called_args.user_role == LitellmUserRoles.INTERNAL_USER, (
        ...
        f"in-memory variable was not updated by _update_derouter_setting."
    )
...
async def test_get_users_filters_username_by_exposed_scim_username_for_okta(mocker):
    ...
    DeRouter exposes SCIM userName from user_email, so the lookup must match
    ...
    mock_prisma_client.db = mocker.MagicMock()
    mock_prisma_client.db.derouter_usertable = mocker.MagicMock()
    mock_prisma_client.db.derouter_usertable.find_many = AsyncMock(return_value=[user])
    mock_prisma_client.db.derouter_usertable.count = AsyncMock(return_value=1)
    ...
    mock_prisma_client.db.derouter_usertable.find_many.assert_awaited_once_with(
        where=expected_where,
        skip=0,
        take=10,
        order={"created_at": "desc"},
    )
    mock_prisma_client.db.derouter_usertable.count.assert_awaited_once_with(where=expected_where)
...
async def test_handle_existing_user_by_email_no_existing_user(mocker):
    ...
    assert result is None
    mock_prisma_client.db.derouter_usertable.find_first.assert_called_once_with(where={"user_email": "test@example.com"})
...
async def test_handle_existing_user_by_email_existing_user_updated(mocker):
    ...
    mock_prisma_client.db = mocker.MagicMock()
    mock_prisma_client.db.derouter_usertable = mocker.MagicMock()
    mock_prisma_client.db.derouter_usertable.find_first = AsyncMock(return_value=existing_user)
    mock_prisma_client.db.derouter_usertable.update = AsyncMock(return_value=updated_user)
    ...
    mock_membership = mocker.patch(
        "derouter.proxy.management_endpoints.scim.scim_v2._handle_team_membership_changes",
        AsyncMock(),
    )
    ...
    update_calls = mock_prisma_client.db.derouter_usertable.update.call_args_list
...
async def test_handle_existing_user_by_email_roster_changes_use_existing_user_id(mocker):
    ...
    mock_prisma_client.db.derouter_usertable = mocker.MagicMock()
    mock_prisma_client.db.derouter_usertable.find_first = AsyncMock(return_value=existing_user)
    mock_prisma_client.db.derouter_usertable.update = AsyncMock(return_value={})

    mock_team_member_add = mocker.patch(
        "derouter.proxy.management_endpoints.scim.scim_v2.team_member_add",
        AsyncMock(),
    )
    mock_team_member_delete = mocker.patch(
        "derouter.proxy.management_endpoints.scim.scim_v2.team_member_delete",
        AsyncMock(),
    )
    mocker.patch(
        "derouter.proxy.management_endpoints.scim.scim_v2.ScimTransformations.transform_derouter_user_to_scim_user",
    ...
    assert mock_prisma_client.db.derouter_usertable.update.await_args.kwargs["where"] == {"user_id": "oidc-sub-123"}
...
async def test_handle_existing_user_by_email_syncs_roster_and_dedups_teams(mocker):
    ...
    duplicates) and the team roster (members_with_roles / DeRouter_TeamMembership)
...
async def test_handle_existing_user_by_email_roster_add_failure_blocks_teams_write(mocker):
    ...
    assert mock_prisma_client.db.derouter_usertable.update.await_count == 0
...
async def test_handle_team_membership_changes_no_changes(mocker):
    ...
    mock_patch_team_membership = mocker.patch(
        "derouter.proxy.management_endpoints.scim.scim_v2.patch_team_membership",
        AsyncMock(),
    )
...
async def test_update_user_success(mocker):
    ...
    mocker.patch(
        "derouter.proxy.management_endpoints.scim.scim_v2._check_user_exists",
    ...
    mock_prisma_client.db.derouter_usertable.update.assert_called_once()
    call_args = mock_prisma_client.db.derouter_usertable.update.call_args
...
async def test_update_user_without_groups_preserves_memberships_and_role(mocker, monkeypatch, groups):
    ...
    async def mock_get_config():
        return {"derouter_settings": {"scim_admin_group": "derouter-admins"}}

    monkeypatch.setattr(proxy_config, "get_config", mock_get_config)
    monkeypatch.setattr("derouter.default_internal_user_params", None, raising=False)

    existing_user = mocker.MagicMock()
    existing_user.teams = ["derouter-admins", "engineering"]
    existing_user.metadata = {}
    ...
    mock_prisma_client.db = mocker.MagicMock()
    mock_prisma_client.db.derouter_usertable = mocker.MagicMock()
    mock_prisma_client.db.derouter_usertable.update = AsyncMock(return_value={"user_id": "okta-user"})
    ...
    update_data = mock_prisma_client.db.derouter_usertable.update.call_args.kwargs["data"]
    assert update_data["teams"] == ["derouter-admins", "engineering"]
...
async def test_update_group_metadata_serialization_issue(mocker):
    ...
    """
    from derouter.proxy.management_endpoints.scim.scim_v2 import update_group
    from derouter.types.proxy.management_endpoints.scim_v2 import SCIMGroup, SCIMMember
    ...
    mock_prisma_client = mocker.MagicMock()
    mock_prisma_client.db = mocker.MagicMock()
    mock_prisma_client.db.derouter_teamtable = mocker.MagicMock()
    mock_prisma_client.db.derouter_usertable = mocker.MagicMock()

    # Mock team operations
    mock_prisma_client.db.derouter_teamtable.find_unique = AsyncMock(return_value=mock_existing_team)
    mock_prisma_client.db.derouter_teamtable.update = AsyncMock(return_value=mock_updated_team)
    ...
    mock_prisma_client.db.derouter_usertable.find_unique = AsyncMock(return_value=mock_user)
    mock_prisma_client.db.derouter_usertable.find_many = AsyncMock(return_value=())
    mock_prisma_client.db.derouter_usertable.update = AsyncMock(return_value=mock_user)
    ...
    mocker.patch(
        "derouter.proxy.management_endpoints.scim.scim_v2.ScimTransformations.transform_derouter_team_to_scim_group",
    ...
    mock_prisma_client.db.derouter_teamtable.update.assert_called_once()
...
async def test_update_group_e2e(mocker):
    ...
    from derouter.proxy.management_endpoints.scim.scim_transformations import (
        ScimTransformations,
    )
    ...
    existing_team = DeRouter_TeamTable(
        team_id=group_id,
    ...
    updated_team = DeRouter_TeamTable(
        team_id=group_id,
    ...
    mocker.patch.object(
        ScimTransformations,
        "transform_derouter_team_to_scim_group",
    ...
    ScimTransformations.transform_derouter_team_to_scim_group.assert_called_once_with(updated_team)


def _rows_by_exact_id(
    user_row: Callable[[Mapping[str, str]], DeRouter_UserTable | MagicMock | None],
) -> Callable[..., tuple[DeRouter_UserTable | MagicMock, ...]]:
    ...
    def rows(where: Mapping[str, object], take: int | None = None) -> tuple[DeRouter_UserTable | MagicMock, ...]:
...
def _user_row_for(where: Mapping[str, str]) -> DeRouter_UserTable:
    return DeRouter_UserTable(user_id=where["user_id"])
...
async def test_create_group_with_nonexistent_users_rejects(mocker, monkeypatch):
    ...
    async def mock_get_config():
        return {"derouter_settings": {"scim_upsert_user": False}}
    ...
    mock_prisma_client.db.derouter_usertable.find_first = AsyncMock(return_value=None)
    mock_prisma_client.db.derouter_usertable.find_many = AsyncMock(side_effect=_rows_by_exact_id(mock_user_lookup))
...
async def test_update_group_with_nonexistent_users_rejects(mocker, monkeypatch):
    ...
    mock_prisma_client.db.derouter_teamtable.find_unique = AsyncMock(side_effect=mock_team_lookup)
    ...
    mocker.patch(
        "derouter.proxy.management_endpoints.scim.scim_v2._check_team_exists",
...
async def test_create_group_with_nonexistent_users_creates_when_flag_true(mocker, monkeypatch):
    ...
    async def mock_get_config():
        return {"derouter_settings": {"scim_upsert_user": True}}
    ...
    mock_create_user = mocker.patch(
        "derouter.proxy.management_endpoints.scim.scim_v2._create_user_if_not_exists",
    ...
    mock_new_team = mocker.patch(
        "derouter.proxy.management_endpoints.scim.scim_v2.new_team",
...
async def test_create_user_grants_admin_when_in_scim_admin_group(mocker, monkeypatch):
    ...
    scim_user = SCIMUser(
        ...
        groups=[SCIMUserGroup(value="derouter-admins", display="DeRouter Admins")],
    )
...
async def test_update_user_demotes_admin_when_removed_from_scim_admin_group(mocker, monkeypatch):
    ...
    existing_user = mocker.MagicMock()
    existing_user.teams = ["derouter-admins"]
    existing_user.metadata = {}
...
async def test_update_user_demotes_when_default_params_lack_user_role(mocker, monkeypatch):
    ...
    monkeypatch.setattr(proxy_config, "get_config", mock_get_config)
    monkeypatch.setattr("derouter.default_internal_user_params", {"max_budget": 10}, raising=False)
...
async def test_patch_user_demotes_admin_when_removed_from_scim_admin_group(mocker, monkeypatch):
    ...
    mock_prisma_client.db.derouter_usertable.update = AsyncMock(return_value=updated_user)
    mock_prisma_client.db.derouter_teamtable = mocker.MagicMock()
    mock_prisma_client.db.derouter_teamtable.find_unique = AsyncMock(return_value=engineering_team)
...
async def test_patch_user_grants_admin_by_team_display_name(mocker, monkeypatch):
    ...
    admin_team.team_alias = "DeRouter Admins"
    ...
    mock_prisma_client.db.derouter_usertable.update = AsyncMock(return_value=updated_user)
    mock_prisma_client.db.derouter_teamtable = mocker.MagicMock()
    mock_prisma_client.db.derouter_teamtable.find_unique = AsyncMock(return_value=admin_team)
...
def _scim_admin_prisma(mocker, *, user_teams):
    ...
    prisma.db.derouter_usertable = mocker.MagicMock()
    prisma.db.derouter_usertable.find_unique = AsyncMock(return_value=user)
    prisma.db.derouter_usertable.find_many = AsyncMock(return_value=())
    prisma.db.derouter_usertable.update = AsyncMock(return_value=user)
    prisma.db.derouter_teamtable = mocker.MagicMock()
    prisma.db.derouter_teamtable.find_unique = AsyncMock(side_effect=_team_find_unique)
    return prisma
...
async def test_recompute_scim_member_roles_demotes_when_not_in_admin_group(mocker, monkeypatch):
    ...
    call_args = prisma.db.derouter_usertable.update.call_args
    assert call_args[1]["data"]["user_role"] == LitellmUserRoles.INTERNAL_USER_VIEW_ONLY
...
async def test_recompute_scim_member_roles_grants_when_in_admin_group(mocker, monkeypatch):
    ...
    prisma = _scim_admin_prisma(mocker, user_teams=["derouter-admins"])
...
async def test_update_group_recomputes_roles_for_changed_members(mocker):
    ...
    mock_prisma_client.db.derouter_teamtable = mocker.MagicMock()
    mock_prisma_client.db.derouter_teamtable.find_unique = AsyncMock(return_value=existing_team)
    mock_prisma_client.db.derouter_teamtable.update = AsyncMock(return_value=existing_team)
    mock_prisma_client.db.derouter_usertable = mocker.MagicMock()
    mock_prisma_client.db.derouter_usertable.find_unique = AsyncMock(return_value=mocker.MagicMock())
    mock_prisma_client.db.derouter_usertable.find_many = AsyncMock(return_value=())
    ...
    recompute_mock = mocker.patch(
        "derouter.proxy.management_endpoints.scim.scim_v2._recompute_scim_member_roles",
        AsyncMock(),
    )
    mocker.patch.object(
        ScimTransformations,
...
async def test_delete_group_recomputes_roles_for_members(mocker):
    ...
    mock_prisma_client.db.derouter_teamtable = mocker.MagicMock()
    mock_prisma_client.db.derouter_teamtable.find_unique = AsyncMock(return_value=existing_team)
    mock_prisma_client.db.derouter_teamtable.delete = AsyncMock()
    mock_prisma_client.db.derouter_usertable = mocker.MagicMock()
    mock_prisma_client.db.derouter_usertable.find_unique = AsyncMock(return_value=member)
    mock_prisma_client.db.derouter_usertable.find_many = AsyncMock(return_value=())
    mock_prisma_client.db.derouter_usertable.update = AsyncMock()
...
async def test_handle_existing_user_by_email_applies_role_when_admin_group_set(mocker):
    ...
    mock_prisma_client.db.derouter_usertable = mocker.MagicMock()
    mock_prisma_client.db.derouter_usertable.find_first = AsyncMock(return_value=existing_user)
    mock_prisma_client.db.derouter_usertable.update = AsyncMock(return_value={"user_id": "new-user-id"})
    mocker.patch(
        "derouter.proxy.management_endpoints.scim.scim_v2.ScimTransformations.transform_derouter_user_to_scim_user",
        AsyncMock(return_value=mocker.MagicMock()),
    )
...
async def test_create_user_existing_email_upsert_demotes_when_admin_group_set(mocker, monkeypatch):
    ...
    mock_prisma_client.db.derouter_usertable.find_many = AsyncMock(return_value=())
    mock_prisma_client.db.derouter_usertable.find_first = AsyncMock(return_value=existing_user)
    mock_prisma_client.db.derouter_usertable.update = AsyncMock(return_value={"user_id": "returning-user"})
...
async def test_process_group_patch_operations_add_retains_existing_members(mocker, monkeypatch):
    ...
    mock_prisma_client.db.derouter_usertable.find_many = AsyncMock(return_value=(mocker.MagicMock(user_id="new-user"),))
...
async def test_process_group_patch_operations_remove_uses_members_with_roles(mocker, monkeypatch):
    ...
    mock_prisma_client.db = mocker.MagicMock()
    mock_prisma_client.db.derouter_usertable = mocker.MagicMock()
    mock_prisma_client.db.derouter_usertable.find_unique = AsyncMock(return_value=mocker.MagicMock(user_id="drop-user"))
    mock_prisma_client.db.derouter_usertable.find_many = AsyncMock(return_value=())
...
async def test_get_groups_reports_members_from_members_with_roles(mocker):
    ...
    mock_prisma_client.db = mocker.MagicMock()
    mock_prisma_client.db.derouter_teamtable = mocker.MagicMock()
    mock_prisma_client.db.derouter_teamtable.find_many = AsyncMock(return_value=[team])
    mock_prisma_client.db.derouter_teamtable.count = AsyncMock(return_value=1)
    mock_prisma_client.db.derouter_usertable = mocker.MagicMock()
...
async def test_apply_group_patch_updates_does_not_write_legacy_members(mocker):
    ...
    mock_prisma_client.db.derouter_teamtable.update = AsyncMock(return_value=updated)
    ...
    mock_prisma_client.db.derouter_teamtable.update.assert_awaited_once()
    written = mock_prisma_client.db.derouter_teamtable.update.call_args.kwargs["data"]
...
def _mock_prisma_for_delete_user(mocker, team):
    ...
    mock_prisma_client.db.derouter_usertable = mocker.MagicMock()
    mock_prisma_client.db.derouter_usertable.delete = AsyncMock()
...
def _patch_delete_user_dependencies(mocker, mock_prisma_client, existing_user):
    ...
    mocker.patch(
        "derouter.proxy.management_endpoints.scim.scim_v2._set_user_keys_blocked",
        AsyncMock(),
    )
    mocker.patch(
        "derouter.proxy.management_endpoints.scim.scim_v2._delete_rows_referencing_user",
        AsyncMock(),
    )
...
async def test_delete_user_prunes_members_with_roles(mocker):
    ...
    mock_prisma_client.db.derouter_usertable.delete.assert_awaited_once()
...
async def test_delete_user_surfaces_prune_failure_and_keeps_user(mocker):
    ...
    mock_prisma_client.db.derouter_usertable.delete.assert_not_awaited()
...
async def test_patch_group_add_applies_delta_and_keeps_concurrent_add(mocker):
    ...
    mock_prisma_client.db.derouter_teamtable.update = AsyncMock(return_value=final_team)
    mock_prisma_client.db.derouter_usertable = mocker.MagicMock()
    mock_prisma_client.db.derouter_usertable.find_many = AsyncMock(side_effect=_rows_by_exact_id(_user_row_for))
...
async def test_process_group_patch_remove_filtered_path_without_value(mocker):
    ...
    prisma_client.db.derouter_usertable = mocker.MagicMock()
    prisma_client.db.derouter_usertable.find_unique = AsyncMock(return_value=DeRouter_UserTable(user_id="user-1"))
    prisma_client.db.derouter_usertable.find_many = AsyncMock(return_value=())
...
async def test_process_group_patch_add_filtered_path_without_value(mocker):
    ...
    prisma_client.db = mocker.MagicMock()
    prisma_client.db.derouter_usertable = mocker.MagicMock()
    prisma_client.db.derouter_usertable.find_many = AsyncMock(return_value=(DeRouter_UserTable(user_id="user-3"),))
...
def _member_resolution_prisma(
    ...
    *,
    ...
    def team_row(team_id: str) -> DeRouter_TeamTable | None:
        if team_id in teams:
            return DeRouter_TeamTable(team_id=team_id, metadata={SCIM_MANAGED_TEAM_METADATA_KEY: True})
        if team_id in unmanaged_teams:
            return DeRouter_TeamTable(team_id=team_id, metadata={})
        return None

    def user_row(where: Mapping[str, str]) -> DeRouter_UserTable | None:
        user_id: Final = where["user_id"]
        if user_id in users:
            return DeRouter_UserTable(user_id=user_id)
        return None

    prisma_client = mocker.MagicMock()
    prisma_client.db = mocker.MagicMock()
    prisma_client.db.derouter_usertable = mocker.MagicMock()
    prisma_client.db.derouter_usertable.find_unique = AsyncMock(side_effect=user_row)
    ...
    def identity_rows(where: Mapping[str, object], take: int | None = None) -> tuple[DeRouter_UserTable, ...]:
        ...
        return tuple(DeRouter_UserTable(user_id=user_id) for user_id in (found[:take] if take else found))

    def team_lookup(where: Mapping[str, str]) -> DeRouter_TeamTable | None:
        team_id: Final = where["team_id"]
        return team_row(team_id)

    prisma_client.db.derouter_usertable.find_first = AsyncMock(return_value=None)
    prisma_client.db.derouter_usertable.find_many = AsyncMock(side_effect=identity_rows)
    prisma_client.db.derouter_teamtable = mocker.MagicMock()
    prisma_client.db.derouter_teamtable.find_unique = AsyncMock(side_effect=team_lookup)
...
async def test_update_group_ignores_nested_group_members(mocker, scim_upsert_user_enabled):
    ...
    prisma_client.db.derouter_teamtable.update = AsyncMock(return_value=existing_team)
    mocker.patch(
        "derouter.proxy.management_endpoints.scim.scim_v2._get_prisma_client_or_raise_exception",
...
async def test_process_group_patch_team_match_needs_scim_provenance(
    ...
    prisma_client = _member_resolution_prisma(mocker, users=set(), teams=set())
    prisma_client.db.derouter_teamtable.find_unique = AsyncMock(
        return_value=DeRouter_TeamTable(team_id="child-team", metadata=team_metadata)
    )
    create_user_mock = mocker.patch(
        "derouter.proxy.management_endpoints.scim.scim_v2._create_user_if_not_exists",
...
async def test_create_group_applies_default_team_params(
    ...
    """SCIM-created teams must honor derouter_settings.default_team_params, including
    models, the same way SSO auto-created teams do."""
    import derouter
    from derouter.types.proxy.management_endpoints.ui_sso import DefaultTeamSSOParams
...
async def test_update_group_stamps_scim_provenance(mocker, scim_upsert_user_enabled):
    ...
    written = json.loads(prisma_client.db.derouter_teamtable.update.call_args.kwargs["data"]["metadata"])
...
async def test_resolve_group_member_ids_matches_sso_user_id(mocker, scim_upsert_user_enabled):
    ...
    assert prisma_client.db.derouter_usertable.find_many.await_args_list == [_identity_lookup("member-sub")]
...
async def test_resolve_group_member_ids_matches_user_email(mocker, scim_upsert_user_enabled):
    ...
    assert prisma_client.db.derouter_usertable.find_many.await_args_list == [_identity_lookup("member@example.com")]
...
async def test_resolve_group_member_ids_reads_the_exact_id_when_two_other_accounts_fill_the_lookup(
    ...
    create_user_mock.assert_not_called()
    assert prisma_client.db.derouter_usertable.find_many.await_args_list == [_identity_lookup("shared")]
    prisma_client.db.derouter_usertable.find_unique.assert_awaited_once_with(where={"user_id": "shared"})
...
async def test_resolve_group_member_ids_reads_the_user_table_once_per_member(mocker, scim_upsert_user_enabled):
    ...
    prisma_client.db.derouter_usertable.find_unique.assert_not_awaited()
    assert prisma_client.db.derouter_usertable.find_many.await_args_list == [
...
async def test_update_user_roster_add_failure_propagates_and_skips_teams_write(mocker):
    ...
    delete_mock = mocker.patch("derouter.proxy.management_endpoints.scim.scim_v2.team_member_delete", AsyncMock())
    ...
    mock_prisma_client.db.derouter_usertable.update.assert_not_called()
...
async def test_handle_group_membership_changes_attempts_every_member_and_names_failures(mocker, failing_member):
    ...
    mocker.patch("derouter.proxy.management_endpoints.scim.scim_v2.team_member_add", add_mock)
    mocker.patch("derouter.proxy.management_endpoints.scim.scim_v2.team_member_delete", delete_mock)
...
async def test_roster_sync_error_status_follows_unanimous_failures(
    ...
    mocker.patch("derouter.proxy.management_endpoints.scim.scim_v2.team_member_add", AsyncMock(side_effect=add_member))
...
async def test_resolve_group_member_ids_admits_member_created_concurrently(mocker, scim_upsert_user_enabled):
    ...
    prisma_client = _member_resolution_prisma(mocker, users=set(), teams=set())
    prisma_client.db.derouter_usertable.find_unique = AsyncMock(return_value=DeRouter_UserTable(user_id="raced-user"))
    mocker.patch(
        "derouter.proxy.management_endpoints.scim.scim_v2._create_user_if_not_exists",
        AsyncMock(return_value=None),
    )
...
async def test_patch_group_404s_when_team_deleted_mid_request(mocker):
    ...
    mock_prisma_client.db = mocker.MagicMock()
    mock_prisma_client.db.derouter_teamtable = mocker.MagicMock()
    mock_prisma_client.db.derouter_teamtable.find_unique = AsyncMock(side_effect=[snapshot_team, None, None])
...
def _shadowed_tenant_rows() -> tuple[DeRouter_UserTable, ...]:
    """A placeholder keyed by the raw member value, and the real account that value names by SSO id."""
    return (
        DeRouter_UserTable(user_id=_SHADOW_MEMBER_VALUE, user_email=_SHADOW_MEMBER_VALUE, teams=[_SHADOWED_GROUP]),
        DeRouter_UserTable(user_id=_SHADOWED_ACCOUNT, user_email="alice@example.com", sso_user_id=_SHADOW_MEMBER_VALUE),
    )
...
def _shadow_tenant_prisma(
    mocker: MockerFixture,
    *,
    rows: Sequence[DeRouter_UserTable],
    ...
    users: Final[dict[str, DeRouter_UserTable]] = {row.user_id: row for row in rows}
    team: Final = DeRouter_TeamTable(
    ...
    async def find_unique(where: Mapping[str, str]) -> DeRouter_UserTable | None:
        return users.get(where["user_id"])

    def clause_matches(row: DeRouter_UserTable, clause: Mapping[str, object]) -> bool:
    ...
    async def identity_rows(where: Mapping[str, object], take: int | None = None) -> tuple[DeRouter_UserTable, ...]:
        clauses: Final = where["OR"]
        assert isinstance(clauses, list)
    ...
    async def delete(where: Mapping[str, str]) -> DeRouter_UserTable | None:
    ...
    async def team_lookup(where: Mapping[str, str]) -> DeRouter_TeamTable | None:
        return team if where["team_id"] == team.team_id else None

    prisma_client = mocker.MagicMock()
    prisma_client.db = mocker.MagicMock()
    prisma_client.db.derouter_usertable = mocker.MagicMock()
    prisma_client.db.derouter_usertable.find_unique = AsyncMock(side_effect=find_unique)
    prisma_client.db.derouter_usertable.find_many = AsyncMock(side_effect=identity_rows)
    prisma_client.db.derouter_usertable.delete = AsyncMock(side_effect=delete)
    prisma_client.db.derouter_teamtable = mocker.MagicMock()
    prisma_client.db.derouter_teamtable.find_unique = AsyncMock(side_effect=team_lookup)
    prisma_client.db.derouter_teamtable.update = AsyncMock(return_value=team)
    prisma_client.db.derouter_verificationtoken = mocker.MagicMock()
    prisma_client.db.derouter_verificationtoken.find_many = AsyncMock(side_effect=keys_for)
    prisma_client.db.derouter_invitationlink = mocker.MagicMock(delete_many=AsyncMock(return_value=0))
    prisma_client.db.derouter_organizationmembership = mocker.MagicMock(delete_many=AsyncMock(return_value=0))
    prisma_client.db.derouter_teammembership = mocker.MagicMock(delete_many=AsyncMock(return_value=0))
    return prisma_client
...
async def test_merge_placeholder_hands_the_group_to_the_shadowed_account(mocker, shadowed_tenant):
    ...
    team_member_add_mock = (
        mocker.patch(  # test-quality-ok: roster helpers are module-level, not injectable into the endpoint
            "derouter.proxy.management_endpoints.scim.scim_v2.team_member_add", AsyncMock()
        )
    )
    team_member_delete_mock = (
        mocker.patch(  # test-quality-ok: roster helpers are module-level, not injectable into the endpoint
            "derouter.proxy.management_endpoints.scim.scim_v2.team_member_delete", AsyncMock()
        )
    )
    ...
    shadowed_tenant.db.derouter_teammembership.delete_many.assert_awaited_once_with(
        where={"user_id": _SHADOW_MEMBER_VALUE}
    )
    shadowed_tenant.db.derouter_usertable.delete.assert_awaited_once_with(where={"user_id": _SHADOW_MEMBER_VALUE})
...
async def test_merge_placeholder_keeps_the_placeholder_when_the_roster_write_fails(mocker, shadowed_tenant):
    ...
    team_member_delete_mock.assert_not_awaited()
    shadowed_tenant.db.derouter_usertable.delete.assert_not_awaited()
    assert await shadowed_tenant.db.derouter_usertable.find_unique(where={"user_id": _SHADOW_MEMBER_VALUE}) is not None
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            (*_shadowed_tenant_rows(), DeRouter_UserTable(user_id="real-2", user_email=_SHADOW_MEMBER_VALUE.upper())),
            {},
...
async def test_merge_placeholder_refuses_rows_that_are_not_a_lone_placeholder(
    ...
    prisma_client.db.derouter_usertable.delete.assert_not_awaited()
