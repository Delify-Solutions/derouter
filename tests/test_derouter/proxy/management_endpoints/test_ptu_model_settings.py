...
from derouter.llms.gemini.cost_calculator import cost_per_web_search_request
...
class TestPartialPtuEditsUseTheMergedView:
    ...
    def test_a_genuinely_startless_configuration_is_still_rejected(self):
        ...
        bare = Deployment(model_name="gpt-4o", derouter_params=DeRouter_Params(model="openai/gpt-4o"))
        merged = _merged_ptu_model_info(
            db_model=bare,
...
class TestTeamModelUpdateValidatesBeforeWriting:
    ...
    async def _run(db_model, patch_data, monkeypatch, touched=None):
        import derouter.proxy.management_endpoints.model_management_endpoints as mme
...
class TestPtuCostAttributionGate:
    """PTU config is only writable once an operator sets DEROUTER_ENABLE_PTU_COST_ATTRIBUTION.
...
class TestAddNewModelPtuGate:
    ...
    def _patched_proxy(model_id: str):
        ...
        proxy_server = "derouter.proxy.proxy_server"
        endpoints = "derouter.proxy.management_endpoints.model_management_endpoints"
    ...
    def _ptu_deployment(model_id: str) -> Deployment:
        return Deployment(
            model_name="ptu-model",
            derouter_params=DeRouter_Params(model="openai/gpt-4.1-nano", api_key="fake-key"),
            model_info=ModelInfo(
                id=model_id,
                team_id="team-1",
                ptu_count=15,
...
class TestPtuDeploymentsAreNotBilledPerToken:
    ...
    def _flag_on(self, monkeypatch):
        monkeypatch.setenv(PTU_COST_ATTRIBUTION_ENV_VAR, "true")
        # update_db_model encrypts every derouter_params value it is handed, and the salt falls
        # back to the master key the proxy sets at boot, which no unit test has.
        monkeypatch.setenv("DEROUTER_SALT_KEY", "test-salt-key")

    @staticmethod
    def _zeroed(model_info=None, derouter_params=None, supplied=None):
        return _ptu_zeroed_pricing(
            model_info=model_info if model_info is not None else {},
            derouter_params=derouter_params if derouter_params is not None else {},
            supplied=supplied if supplied is not None else {},
        )

    def test_a_deployment_without_ptu_config_keeps_its_pricing(self):
        assert self._zeroed(model_info={"team_id": "t"}, derouter_params={"input_cost_per_token": 5e-07}) == {}
    ...
    def test_search_context_already_on_the_row_is_zeroed_in_place(self):
        ...
        override = self._zeroed(model_info=self.PTU, derouter_params=stored)
    ...
    def test_tiered_pricing_already_on_the_row_is_emptied_not_zeroed(self):
        ...
        assert priced.derouter_params.tiered_pricing == []
        assert priced.model_info.tiered_pricing == []

        written = update_db_model(
            db_model=Deployment(
                model_name="tiered",
                derouter_params=DeRouter_Params(model="openai/gpt-4o", tiered_pricing=tiers),
        ...
        for blob in ("model_info", "derouter_params"):
    ...
    def test_a_price_already_on_the_row_is_zeroed_rather_than_refused(self):
        ...
        zeroed = self._zeroed(model_info={**self.PTU, "input_cost_per_second": 3.0}, derouter_params={})
    ...
    async def test_a_refused_price_does_not_leave_the_team_changed(self):
        ...
        patch = updateDeployment(
            derouter_params=updateDeRouterParams(model="openai/gpt-4o", input_cost_per_token=5e-07),
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(patch_ctx("derouter.proxy.proxy_server.premium_user", True))
            with pytest.raises(HTTPException) as exc:
    ...
    def test_a_setting_that_is_not_a_charge_is_left_alone(self):
        """CustomPricingDeRouterParams also carries an embedding's output vector size and the
        ...
        assert priced.derouter_params.get("output_vector_size") == 1536
        assert priced.derouter_params.get("regional_processing_uplift_multiplier_eu") == 1.15
        assert priced.derouter_params.get("input_cost_per_token") == 0
    ...
    def test_removing_ptu_config_releases_every_rate_it_zeroed(self):
        ...
        on = update_db_model(
            db_model=Deployment(
                model_name="audio",
                derouter_params=DeRouter_Params(model="azure/whisper", input_cost_per_second=0.006),
        ...
        assert json.loads(on["derouter_params"])["input_cost_per_second"] == 0

        off = update_db_model(
            db_model=Deployment(
                model_name="audio",
                derouter_params=DeRouter_Params(**json.loads(on["derouter_params"])),
        ...
        assert "input_cost_per_second" not in json.loads(off["derouter_params"])
    ...
    def test_removing_ptu_config_releases_a_zeroed_search_context_table(self):
        ...
        assert json.loads(on["derouter_params"])["search_context_cost_per_query"] == dict.fromkeys(
        ...
        assert "search_context_cost_per_query" not in json.loads(off["derouter_params"])
    ...
    def test_the_cost_map_contributes_no_price_to_a_priced_ptu_deployment(self, backend):
        ...
        priced = _ptu_priced_deployment(
            Deployment(
                ...
                derouter_params=DeRouter_Params(model=backend, api_key="fake-key"),
                model_info=ModelInfo(
                    id="dep-ptu",
                    team_id="team-1",
                    ptu_effective_from=datetime.datetime(2020, 1, 1, tzinfo=datetime.timezone.utc),
    ...
    def test_the_cost_map_tiers_contribute_no_price_to_a_priced_ptu_deployment(self):
        ...
        priced = _ptu_priced_deployment(
            Deployment(
                model_name="ptu-deployment",
                derouter_params=DeRouter_Params(model="dashscope/qwen-flash", api_key="fake-key"),
                model_info=ModelInfo(
                    id="dep-ptu",
                    team_id="team-1",
                    ptu_effective_from=datetime.datetime(2020, 1, 1, tzinfo=datetime.timezone.utc),
    ...
    def test_the_zeroed_pricing_does_not_waive_budget_enforcement(self):
        ...
        priced = _ptu_priced_deployment(
            Deployment(
                ...
                derouter_params=DeRouter_Params(model="gemini/gemini-2.5-flash", api_key="fake-key"),
                model_info=ModelInfo(
                    id="dep-ptu",
                    team_id="team-1",
    ...
    def test_an_unrelated_patch_heals_a_deployment_stored_before_this_rule(self):
        """Both blobs, because derouter_params wins over model_info wherever the two are merged."""
        written = update_db_model(
            db_model=_deployment_with_stored_ptu(),
            updated_patch=updateDeployment(model_name="gpt-4o-renamed"),
        )
    ...
    def test_an_unrelated_patch_of_a_ptu_row_that_carries_a_price_is_not_refused(self):
        ...
        priced_ptu = Deployment(
            model_name="gpt-4o",
            derouter_params=DeRouter_Params(model="openai/gpt-4o", input_cost_per_token=5e-07),
            model_info=ModelInfo(
                id="dep-0",
                team_id="t",
                ptu_effective_from=datetime.datetime(2020, 1, 1, tzinfo=datetime.timezone.utc),
        ...
        assert json.loads(written["derouter_params"])["input_cost_per_token"] == 0
    ...
    def test_removing_ptu_config_hands_per_token_billing_back(self):
        ...
        written = update_db_model(
            db_model=Deployment(
                model_name="gpt-4o",
                derouter_params=DeRouter_Params(model="openai/gpt-4o", **zeros),
                model_info=ModelInfo(
                    id="dep-0",
                    team_id="t",
                    ptu_effective_from=datetime.datetime(2020, 1, 1, tzinfo=datetime.timezone.utc),
    ...
    def test_a_deployment_that_never_had_ptu_keeps_a_price_its_operator_set_to_zero(self):
        ...
        free = Deployment(
            ...
            derouter_params=DeRouter_Params(model="openai/gpt-4o", input_cost_per_token=0.0),
    ...
    async def test_model_new_stores_zero_pricing_on_both_blobs(self):
        ...
        assert all(written.derouter_params.get(field) == 0 for field in _PTU_ZEROED_PRICING_FIELDS)
        assert written.derouter_params.tiered_pricing == []
    ...
    async def test_model_new_refuses_a_priced_ptu_deployment(self):
        ...
        deployment = base.model_copy(
            update={"derouter_params": base.derouter_params.model_copy(update={"input_cost_per_token": 5e-07})}
        )
