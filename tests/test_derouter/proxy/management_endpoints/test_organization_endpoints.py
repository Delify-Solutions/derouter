...
async def test_organization_update_object_permissions_existing_permission(monkeypatch):
    ...
    existing_organization_row = DeRouter_OrganizationTable(
    ...
    mock_prisma_client.db.derouter_objectpermissiontable.find_unique = AsyncMock(return_value=existing_object_permission)

    # Mock upsert operation
    updated_permission = MagicMock()
    updated_permission.object_permission_id = "existing_perm_id_123"
    mock_prisma_client.db.derouter_objectpermissiontable.upsert = AsyncMock(return_value=updated_permission)
...
async def test_get_organization_daily_activity_admin_param_passing(monkeypatch):
    ...
    from derouter.proxy.management_endpoints import organization_endpoints
    from derouter.proxy.management_endpoints.organization_endpoints import (
    ...
    # Mock prisma client
    mock_prisma_client = AsyncMock()
    mock_prisma_client.db.derouter_organizationtable.find_many = AsyncMock(return_value=[])
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)

    # Admin view -> skip membership restriction
    monkeypatch.setattr(
        "derouter.proxy.management_endpoints.organization_endpoints._user_has_admin_view",
        lambda _: True,
    )
    ...
    assert kwargs["table_name"] == "derouter_dailyorganizationspend"
...
async def test_get_organization_daily_activity_non_admin_defaults_to_admin_orgs(
    ...
    mock_prisma_client = AsyncMock()
    mock_prisma_client.db.derouter_organizationtable.find_many = AsyncMock(return_value=[])
    mock_prisma_client.db.derouter_organizationmembership.find_many = AsyncMock(
        return_value=[
...
async def test_organization_update_object_permissions_no_existing_permission(
    ...
    DeRouter_ObjectPermissionTable and the organization is updated with the new object_permission_id.
    """
    from unittest.mock import AsyncMock, MagicMock
    ...
    existing_organization_row_no_perm = DeRouter_OrganizationTable(
    ...
    mock_prisma_client.db.derouter_objectpermissiontable.find_unique = AsyncMock(return_value=None)

    # Mock upsert to create new record
    new_permission = MagicMock()
    new_permission.object_permission_id = "new_perm_id_456"
    mock_prisma_client.db.derouter_objectpermissiontable.upsert = AsyncMock(return_value=new_permission)

    data_json = {
        "object_permission": DeRouter_ObjectPermissionBase(vector_stores=["brand_new_store"]).model_dump(
...
async def test_organization_update_object_permissions_missing_permission_record(
    ...
    existing_organization_row_missing_perm = DeRouter_OrganizationTable(
    ...
    data_json = {
        "object_permission": DeRouter_ObjectPermissionBase(vector_stores=["recreated_store"]).model_dump(
            exclude_unset=True, exclude_none=True
        ),
...
async def test_list_organization_filter_by_org_id(monkeypatch):
    ...
    mock_prisma_client.db.derouter_organizationtable.find_many = AsyncMock(return_value=[mock_org1])
    ...
    mock_prisma_client.db.derouter_organizationtable.find_many.assert_called_once()
    call_args = mock_prisma_client.db.derouter_organizationtable.find_many.call_args
    assert call_args.kwargs["where"] == {"organization_id": "org-123"}
    assert call_args.kwargs["include"] == {
        "derouter_budget_table": True,
        "members": True,
        "teams": True,
    }
...
async def test_list_organization_filter_by_org_alias(monkeypatch):
    ...
    # Mock find_many to return filtered results
    mock_prisma_client.db.derouter_organizationtable.find_many = AsyncMock(return_value=[mock_org1, mock_org2])
...
async def test_organization_info_includes_user_email(monkeypatch):
    ...
    membership = DeRouter_OrganizationMembershipTable(**raw_membership)
...
def patched_org_prisma():
    ...
    with (
        ...
        patch(
            "derouter.proxy.management_endpoints.organization_endpoints.get_user_object",
        ...
        mock_prisma.db.derouter_organizationtable.find_unique = AsyncMock(return_value=victim_row)
        yield mock_prisma
...
async def test_organization_member_update_rejects_unauthorized_caller(patched_org_prisma, unauthorized_caller):
    from derouter.proxy._types import OrganizationMemberUpdateRequest
    from derouter.proxy.management_endpoints.organization_endpoints import (
...
async def test_organization_member_delete_rejects_unauthorized_caller(patched_org_prisma, unauthorized_caller):
    from derouter.proxy._types import OrganizationMemberDeleteRequest
    from derouter.proxy.management_endpoints.organization_endpoints import (
...
def test_v2_model_rejects_invalid_body(body):
    ...
    from derouter.proxy._types import OrganizationUpdateRequestV2
...
async def _run_update_organization_v2(
    ...
    *,
    ...
    from derouter.proxy.utils import jsonify_object
    ...
    mock_prisma_client.db.derouter_organizationtable.find_unique = AsyncMock(return_value=existing_org)
    mock_prisma_client.db.derouter_organizationtable.update = AsyncMock(return_value=MagicMock())
    mock_prisma_client.db.derouter_budgettable.update = AsyncMock()
    ...
    tx = MagicMock()
    tx.derouter_organizationtable = mock_prisma_client.db.derouter_organizationtable
    tx.derouter_budgettable = mock_prisma_client.db.derouter_budgettable
    tx.derouter_objectpermissiontable.upsert = AsyncMock()
    mock_prisma_client.db.tx = MagicMock(return_value=_FakeTxContext(tx))
    mock_prisma_client.tx = tx

    call_order = MagicMock()
    call_order.attach_mock(tx.derouter_objectpermissiontable.upsert, "permission_upsert")
    call_order.attach_mock(mock_prisma_client.db.derouter_organizationtable.update, "org_update")
...
def test_v2_update_route_is_public_in_openapi():
    ...
    from derouter.proxy.proxy_server import get_openapi_schema
...
async def test_v2_update_clears_tpm_limit_and_metadata(monkeypatch):
    ...
    budget_write = prisma.db.derouter_budgettable.update.await_args
    ...
    write_data = prisma.db.derouter_organizationtable.update.await_args.kwargs["data"]
...
async def test_v2_update_untouched_fields_not_written(monkeypatch):
    ...
    prisma.db.derouter_budgettable.update.assert_not_awaited()
    write_data = prisma.db.derouter_organizationtable.update.await_args.kwargs["data"]
...
async def test_v2_rejects_null_clear_of_non_nullable_fields(monkeypatch):
    ...
    from derouter.proxy._types import LitellmUserRoles, OrganizationUpdateRequestV2, UserAPIKeyAuth
    from derouter.proxy.management_endpoints.organization_endpoints import update_organization_v2

    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", AsyncMock())
    auth = UserAPIKeyAuth(user_role=LitellmUserRoles.PROXY_ADMIN, user_id="admin-1")
...
async def test_v2_rejects_negative_integer_limits(monkeypatch: pytest.MonkeyPatch, field: str):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", prisma_mock)
    ...
    prisma_mock.db.derouter_budgettable.update.assert_not_awaited()
    prisma_mock.db.derouter_organizationtable.update.assert_not_awaited()
...
async def test_v2_rejects_caller_without_org_access(monkeypatch):
    ...
    assert exc.value.status_code == 403
    mock_prisma_client.db.derouter_organizationtable.update.assert_not_awaited()
...
async def test_v2_wires_object_permission_onto_org_write(monkeypatch):
    ...
    upsert = prisma.tx.derouter_objectpermissiontable.upsert.await_args.kwargs
...
async def test_v2_object_permission_upsert_runs_inside_transaction(monkeypatch):
    ...
    prisma.tx.derouter_objectpermissiontable.upsert.assert_awaited_once()
    prisma.db.derouter_objectpermissiontable.upsert.assert_not_awaited()

    upsert = prisma.tx.derouter_objectpermissiontable.upsert.await_args.kwargs
    linked_id = prisma.db.derouter_organizationtable.update.await_args.kwargs["data"]["object_permission_id"]
...
async def test_v2_clears_object_permission_when_sent_null(monkeypatch):
    ...
    prisma.tx.derouter_objectpermissiontable.upsert.assert_not_awaited()
    prisma.db.derouter_objectpermissiontable.find_unique.assert_not_awaited()
    write_data = prisma.db.derouter_organizationtable.update.await_args.kwargs["data"]
...
async def test_v2_writes_budget_and_org_in_one_transaction(monkeypatch):
    ...
    prisma.db.derouter_budgettable.update.assert_awaited_once()
    prisma.db.derouter_organizationtable.update.assert_awaited_once()
...
async def test_v2_serializes_model_max_budget_on_budget_write(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.proxy.management_endpoints.key_management_endpoints.validate_model_max_budget",
        lambda _: None,
    )
    ...
    written = prisma.db.derouter_budgettable.update.await_args.kwargs["data"]["model_max_budget"]
...
async def _run_legacy_update_organization(
    ...
    from derouter.proxy._types import LitellmUserRoles, UserAPIKeyAuth
    from derouter.proxy.management_endpoints import organization_endpoints
    from derouter.proxy.management_endpoints.organization_endpoints import update_organization
    from derouter.proxy.utils import jsonify_object
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"organization_id": "org-1", "derouter_budget_table": {"tpm_limit": None}},
    ],
)
async def test_legacy_update_clears_tpm_limit_when_sent_null(monkeypatch, body):
    ...
    assert "tpm_limit" not in prisma.db.derouter_organizationtable.update.await_args.kwargs["data"]
...
async def test_legacy_update_without_budget_fields_skips_budget_write(monkeypatch):
    ...
    prisma.db.derouter_budgettable.update.assert_not_awaited()
    assert prisma.db.derouter_organizationtable.update.await_args.kwargs["data"]["organization_alias"] == "renamed"
...
def test_build_budget_write_data_recomputes_reset_at_on_duration():
    ...
    from derouter.proxy.management_endpoints.organization_endpoints import build_budget_write_data
...
async def test_get_organization_daily_activity_non_admin_without_org_admin_role_sees_nothing(
    ...
    mock_prisma_client.db.derouter_organizationtable.find_many = org_table_find_many
    mock_prisma_client.db.derouter_organizationmembership.find_many = AsyncMock(return_value=[])
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
...
async def test_find_member_if_email_missing_row_raises_documented_400():
    ...
    assert exc_info.value.detail == {
        "error": (
            ...
            "non-existent user_email in DeRouter_UserTable. Use 'user_id' instead."
        )
    }
...
async def test_new_organization_rejects_shared_alias_tool_permission_key():
    ...
    from derouter.proxy._types import DeRouter_ObjectPermissionBase, NewOrganizationRequest
    from derouter.proxy.management_endpoints.organization_endpoints import (
    ...
    prisma_client.db.derouter_objectpermissiontable.create = AsyncMock()
    data = NewOrganizationRequest(
        organization_alias="org",
        object_permission=DeRouter_ObjectPermissionBase(mcp_tool_permissions={"wiki": ["ask_question"]}),
    )
    ...
    prisma_client.db.derouter_objectpermissiontable.create.assert_not_called()
...
def test_v2_update_organization_is_in_openapi_schema():
    ...
    from derouter.proxy.management_endpoints.organization_endpoints import router
