...
from derouter.proxy.management_endpoints.policy_endpoints import apply_policies
...
class TestApplyPoliciesEarlyReturn:
    ...
    async def test_returns_inputs_unchanged_when_registry_not_initialized(
        ...
        with patch(
            "derouter.proxy.management_endpoints.policy_endpoints.endpoints.get_policy_registry",
    ...
    async def test_returns_inputs_unchanged_when_resolved_guardrails_empty(
        ...
        from derouter.types.proxy.policy_engine import ResolvedPolicy
        ...
        with (
            ...
            patch(
                "derouter.proxy.management_endpoints.policy_endpoints.endpoints.PolicyResolver.resolve_policy_guardrails",
...
class TestApplyPoliciesWithGuardrails:
    ...
    async def test_applies_single_guardrail_and_returns_modified_inputs(
        ...
        with (
            ...
            patch(
                "derouter.proxy.management_endpoints.policy_endpoints.endpoints.GuardrailRegistry",
...
# ---------------------------------------------------------------------------
from derouter.proxy.management_endpoints.policy_endpoints import (
...
class TestBuildCompetitorGuardrailDefinitions:
    ...
    def test_populates_blocked_words_for_known_guardrail_names(self):
        ...
        assert len(name_blocker["derouter_params"]["blocked_words"]) > 0
        ...
        assert len(rec_filter["derouter_params"]["blocked_words"]) > 0

    def test_does_not_modify_unknown_guardrail_names(self):
        definitions = [
            {
                "guardrail_name": "some-other-guardrail",
                "derouter_params": {"blocked_words": ["original"]},
            },
        ]
        result = _build_competitor_guardrail_definitions(
            definitions, ["Delta"], "Emirates"
        )
        assert result[0]["derouter_params"]["blocked_words"] == ["original"]
    ...
    def test_does_not_mutate_original_definitions(self):
        ...
        assert definitions[0]["derouter_params"]["blocked_words"] == []
    ...
    def test_handles_input_and_output_blocker_variants(self):
        ...
        for defn in result:
            assert len(defn["derouter_params"]["blocked_words"]) > 0
