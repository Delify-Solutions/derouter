...
def _make_team(team_id="team-1", organization_id="org-1") -> DeRouter_TeamTable:
...
def _patch_org_admin_deps(get_user_return):
    ...
    return (
        ...
        patch("derouter.proxy.proxy_server.prisma_client", MagicMock(), create=True),
        patch("derouter.proxy.proxy_server.proxy_logging_obj", MagicMock(), create=True),
        patch(
            "derouter.proxy.proxy_server.user_api_key_cache", MagicMock(), create=True
        ),
    )
...
class TestUserIsOrgAdminRouteCheck:
    ...
    def test_user_list_in_self_managed_routes(self):
        ...
        assert "/user/list" in DeRouterRoutes.self_managed_routes.value
