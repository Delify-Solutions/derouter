...
async def test_create_access_group_with_model_ids_tags_only_specific_deployments():
    ...
    Fixes: https://github.com/Delify-Solutions/derouter/issues/21544
    """
    from derouter.proxy._types import LitellmUserRoles, UserAPIKeyAuth
    from derouter.proxy.management_endpoints.model_access_group_management_endpoints import (
        create_model_group,
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.prisma_client", mock_prisma),
        patch(
            "derouter.proxy.management_endpoints.model_access_group_management_endpoints.clear_cache",
    ...
    mock_prisma.db.derouter_proxymodeltable.find_unique.assert_called_once_with(
        where={"model_id": "deploy-A"}
    )
    assert mock_prisma.db.derouter_proxymodeltable.update.call_count == 1
    update_call = mock_prisma.db.derouter_proxymodeltable.update.call_args
...
async def test_create_access_group_with_model_names_tags_all_deployments():
    ...
    mock_router = Router(
        model_list=[
            {
                "model_name": "gpt-4o",
                "derouter_params": {"model": "gpt-4o", "api_key": "fake-key"},
    ...
    assert mock_prisma.db.derouter_proxymodeltable.update.call_count == 3
...
async def test_create_access_group_invalid_model_id_returns_400():
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_proxymodeltable.find_many = AsyncMock(return_value=[])
    mock_prisma.db.derouter_proxymodeltable.find_unique = AsyncMock(return_value=None)
...
async def test_create_access_group_surfaces_dropped_models():
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_proxymodeltable.find_many = AsyncMock(return_value=[])
    mock_prisma.db.derouter_proxymodeltable.find_unique = AsyncMock(return_value=deploy_a)
    mock_prisma.db.derouter_proxymodeltable.update = AsyncMock()
    ...
    with (
        patch("derouter.proxy.proxy_server.llm_router", wiped_router),
        patch("derouter.proxy.proxy_server.prisma_client", mock_prisma),
        patch(
            "derouter.proxy.management_endpoints.model_access_group_management_endpoints.clear_cache",
            new=AsyncMock(return_value=ReconcileOutcome(still_desired=None, live_after=None)),
...
async def test_create_access_group_trusts_reload_snapshot_over_post_lock_fresh_read():
    ...
    with (
        patch("derouter.proxy.proxy_server.llm_router", concurrently_wiped_router),
        patch("derouter.proxy.proxy_server.prisma_client", mock_prisma),
        patch(
            "derouter.proxy.management_endpoints.model_access_group_management_endpoints.clear_cache",
            new=AsyncMock(
...
async def test_delete_access_group_ignores_models_that_were_already_dead():
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_proxymodeltable.find_many = AsyncMock(return_value=[deploy_broken])
    mock_prisma.db.derouter_proxymodeltable.update = AsyncMock()
    mock_prisma.db.derouter_modelaccessgroupbudgettable.delete = AsyncMock(return_value=None)
    ...
    with (
        patch("derouter.proxy.proxy_server.llm_router", never_served_router),
        patch("derouter.proxy.proxy_server.prisma_client", mock_prisma),
        patch(
            "derouter.proxy.management_endpoints.model_access_group_management_endpoints.clear_cache",
            new=AsyncMock(return_value=ReconcileOutcome(still_desired=None, live_after=None)),
...
async def test_create_access_group_read_through_recovers_model_created_on_sibling_replica():
    ...
    db_row = SimpleNamespace(
        ...
        derouter_params={"model": "openai/gpt-4o", "api_key": "fake", "mock_response": "hi"},
        model_info={},
        blocked=False,
    )
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_proxymodeltable.find_many = AsyncMock(side_effect=[[db_row], [], [db_row]])
    mock_prisma.db.derouter_proxymodeltable.update = AsyncMock()
    ...
    assert mock_prisma.db.derouter_proxymodeltable.find_many.await_args_list[0].kwargs["where"] == {
        "model_name": model_name
    }
...
class _FakeAccessGroupBudgetRow:
    ...
    spend: float = 0.0
    derouter_budget_table: _FakeBudgetRow | None = None
...
class _FakeBudgetTable:
    """Stands in for derouter_budgettable so a test can see whether a budget row was created,
...
class _FakeAccessGroupBudgetTable:
    """Stands in for derouter_modelaccessgroupbudgettable, resolving `include` against the fake
...
class _FakePrismaClient:
    def __init__(self, journal: list[str], deployments=()) -> None:
        ...
        self.db = SimpleNamespace(
            derouter_budgettable=self.budget_table,
            derouter_modelaccessgroupbudgettable=self.access_group_budget_table,
            derouter_proxymodeltable=self.model_table,
        )
...
def _proxy_with_stubbed_reload(prisma):
    ...
    with (
        ...
        patch(  # test-quality-ok: live_model_ids_snapshot() reads the llm_router global; the endpoint takes no router
            "derouter.proxy.proxy_server.llm_router", never_served_router
        ),
...
async def test_delete_access_group_budget_drops_the_row_and_spares_the_shared_budget():
    """The group row goes; the DeRouter_BudgetTable row it linked survives, as /tag/delete leaves a
...
async def test_deleting_the_access_group_strips_deployments_before_dropping_the_budget():
    ...
    DeRouter_BudgetTable row survives here too."""
    from derouter.proxy.management_endpoints.model_access_group_management_endpoints import (
        delete_access_group,
    )
