...
from prisma.actions import DeRouter_VerificationTokenActions
...
from derouter.types.tag_management import TagDeleteRequest, TagInfoRequest, TagNewRequest

client = TestClient(app)


class FakeVerificationTokenTable:
    """Stand-in for ``prisma_client.db.derouter_verificationtoken``.
    ...
    async def find_many(self, **kwargs: object) -> tuple[Mock, ...]:
        inspect.signature(DeRouter_VerificationTokenActions.find_many).bind(
            self, **kwargs
        )
...
async def test_create_and_get_tag():
    ...
    try:
        with (
            ...
            patch(
                "derouter.proxy.proxy_server.derouter_proxy_admin_name", "default_user_id"
            ),
            patch(
                "derouter.proxy.management_endpoints.tag_management_endpoints.get_deployments_by_model"
            ...
            mock_db.derouter_tagtable.find_unique = AsyncMock(return_value=None)

            # Mock find_many for model lookup
            mock_db.derouter_proxymodeltable.find_many = AsyncMock(return_value=[])
            ...
            mock_db.derouter_tagtable.create = AsyncMock(return_value=created_tag)
            ...
            retrieved_tag.derouter_budget_table = None
            mock_db.derouter_tagtable.find_many = AsyncMock(return_value=[retrieved_tag])
...
async def test_update_tag():
    ...
    try:
        with (
            ...
            mock_db.derouter_tagtable.find_unique = AsyncMock(return_value=existing_tag)
            ...
            mock_db.derouter_tagtable.update = AsyncMock(return_value=updated_tag)
...
async def test_delete_tag():
    ...
    try:
        with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
            ...
            # Mock delete
            mock_db.derouter_tagtable.delete = AsyncMock(return_value=existing_tag)
            ...
            mock_db.derouter_tagtable.delete.assert_called_once()
    finally:
        # Clean up dependency overrides
        app.dependency_overrides.clear()
...
async def test_list_tags_with_dynamic_tags():
    ...
    try:
        with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
            ...
            stored_tag.derouter_budget_table = None
            mock_db.derouter_tagtable.find_many = AsyncMock(return_value=[stored_tag])

            # Setup dynamic tags via group_by — includes one that overlaps with stored
            mock_db.derouter_dailytagspend.group_by = AsyncMock(
                return_value=[
                    {
                        "tag": "dynamic-tag-1",
...
async def test_list_tags_no_dynamic_tags():
    ...
    try:
        with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
            ...
            mock_db.derouter_dailytagspend.group_by = AsyncMock(return_value=[])
...
async def test_internal_user_list_tags_only_returns_tags_used_by_their_keys():
    ...
    try:
        with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
            ...
            mock_db.derouter_verificationtoken = fake_token_table
            ...
            mock_db.derouter_dailytagspend.group_by.assert_awaited_once_with(
                by=["tag"],
                where={
                    "tag": {"not": None},
            ...
            mock_db.derouter_tagtable.find_many.assert_awaited_once_with(
...
async def test_internal_user_list_tags_does_not_500_on_unsupported_prisma_kwarg():
    ...
    "DeRouter_VerificationTokenActions.find_many() got an unexpected keyword
    argument 'select'".
    """
    from unittest.mock import AsyncMock, Mock
    ...
    try:
        with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
            ...
            mock_db.derouter_dailytagspend.group_by = AsyncMock(return_value=[])
            mock_db.derouter_tagtable.find_many = AsyncMock(return_value=[])
...
async def test_list_tags_with_date_range_filters_dynamic_tags():
    ...
    try:
        with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
            ...
            mock_db.derouter_dailytagspend.group_by = group_by_mock
...
async def test_internal_user_tag_daily_activity_is_scoped_to_their_keys():
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma,
        patch(
            "derouter.proxy.management_endpoints.tag_management_endpoints.get_daily_activity",
...
async def test_get_deployments_by_model_name():
    ...
    mock_router.get_model_list.return_value = [
        {
            "model_name": "gpt-3.5-turbo",
            "derouter_params": {"model": "gpt-3.5-turbo", "api_key": "test-key"},
...
async def test_add_tag_to_deployment_preserves_encrypted_fields():
    ...
    with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
        ...
        mock_db.derouter_proxymodeltable.find_unique = AsyncMock(return_value=db_model)

        # Mock update
        mock_db.derouter_proxymodeltable.update = AsyncMock(return_value=db_model)
        ...
        mock_db.derouter_proxymodeltable.find_unique.assert_called_once_with(
            where={"model_id": "model-123"}
        )

        # Verify update was called with preserved encrypted fields
        update_call = mock_db.derouter_proxymodeltable.update.call_args
        assert update_call[1]["where"] == {"model_id": "model-123"}

        # Parse the updated derouter_params
        updated_params = json.loads(update_call[1]["data"]["derouter_params"])
...
async def test_add_tag_to_deployment_with_string_params():
    """
    Test that _add_tag_to_deployment handles string derouter_params correctly
    """
    from unittest.mock import AsyncMock, Mock
    ...
    with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
        ...
        # Mock the database model with derouter_params as string
        db_model = Mock()
        db_model.model_id = "model-456"
        db_model.derouter_params = json.dumps(
            {
                "model": "claude-3",
        ...
        deployment = Deployment(
            model_name="claude-3",
            derouter_params=DeRouter_Params(model="claude-3"),
...
async def test_add_tag_to_deployment_no_duplicate_tags():
    ...
    with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
        ...
        deployment = Deployment(
            model_name="gpt-4",
            derouter_params=DeRouter_Params(model="gpt-4"),
...
async def test_add_tag_to_deployment_model_not_found():
    ...
    with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
        ...
        mock_db.derouter_proxymodeltable.find_unique = AsyncMock(return_value=None)

        # Create deployment
        deployment = Deployment(
            model_name="nonexistent-model",
            derouter_params=DeRouter_Params(model="nonexistent-model"),
