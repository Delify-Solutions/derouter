...
from derouter.proxy.management_endpoints.usage_endpoints.ai_usage_chat import (
    TOOL_HANDLERS,
    TOOLS_ADMIN,
    TOOLS_BASE,
...
class TestStreamUsageAiChat:
    ...
    async def test_stream_emits_status_events(self):
        ...
        with (
            patch(
                "derouter.proxy.management_endpoints.usage_endpoints.ai_usage_chat.derouter"
            ) as mock_derouter,
            patch(
                "derouter.proxy.management_endpoints.usage_endpoints.ai_usage_chat._fetch_usage_data",
                new_callable=AsyncMock,
            ) as mock_fetch,
        ):
            mock_derouter.acompletion = AsyncMock(
                side_effect=[
                    mock_first_response,
                    mock_stream(),
                ]
    ...
    async def test_stream_handles_team_tool(self):
        ...
        with (
            ...
            patch(
                "derouter.proxy.management_endpoints.usage_endpoints.ai_usage_chat._fetch_team_usage_data",
                new_callable=AsyncMock,
            ) as mock_fetch,
        ):
            mock_derouter.acompletion = AsyncMock(
    ...
    async def test_stream_handles_error(self):
        with patch(
            ...
            mock_derouter.acompletion = AsyncMock(side_effect=Exception("LLM error"))
    ...
    async def test_non_admin_enforces_user_id(self):
        ...
        with (
            ...
            patch.dict(
                "derouter.proxy.management_endpoints.usage_endpoints.ai_usage_chat.TOOL_HANDLERS",
                {
                    "get_usage_data": {
                        "fetch": mock_fetch,
...
class TestUsageAiChatServiceAccountGuard:
    ...
    async def test_non_admin_with_user_id_none_is_rejected(self):
        ...
        from derouter.proxy.management_endpoints.usage_endpoints.endpoints import (
            ChatMessage,
            UsageAIChatRequest,
            usage_ai_chat,
        )
...
class TestUsageAiChatKeepalive:
    async def _collect_endpoint_body(self, monkeypatch, interval, delay=0.3) -> tuple[list[bytes], dict]:
        ...
        with patch(  # test-quality-ok: the stream calls the module-level derouter.acompletion directly; no injection seam
            "derouter.proxy.management_endpoints.usage_endpoints.ai_usage_chat.derouter.acompletion",
