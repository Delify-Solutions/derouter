...
from derouter.proxy.management_endpoints.policy_endpoints.endpoints import (
...
async def test_unsupported_guardrail_type():
    """Non-derouter_content_filter types should return unsupported."""
    guardrail_defs = [
        {
...
class TestEnrichPolicyTemplateStreamKeepalive:
    async def _collect_endpoint_body(self, monkeypatch, interval, delay=0.3) -> tuple[list[bytes], dict]:
        ...
        import derouter
        import derouter.proxy.management_endpoints.policy_endpoints.endpoints as policy_endpoints
