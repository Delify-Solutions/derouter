...
from derouter.proxy.common_utils.encrypt_decrypt_utils import encrypt_value_helper
...
from derouter.types.router import Deployment, DeRouter_Params, ModelInfo, updateDeployment, updateDeRouterParams
...
class MockPrismaClient:
    ...
    @property
    def derouter_teamtable(self):
        return self

    @property
    def derouter_proxymodeltable(self):
        return self
...
class TestModelManagementAuthChecks:
    ...
    async def test_can_user_make_team_model_call_non_premium_fails(self):
        ...
        with pytest.raises(Exception, match='You must be a DeRouter Enterprise user to use this feature\\.') as exc_info:
    ...
    async def test_allow_team_model_action_success(self):
        ...
        model_params = Deployment(
            ...
            derouter_params=DeRouter_Params(model="test_model", team_id="test_team"),
    ...
    def test_can_user_attach_credential_admin_success(self):
        result = ModelManagementAuthChecks.can_user_attach_credential(
            derouter_params=DeRouter_Params(model="test_model", derouter_credential_name="shared-credential"),
            user_api_key_dict=self.admin_user,
        )
        assert result is True
    ...
    def test_can_user_attach_credential_without_credential_allows_any_role(self):
        result = ModelManagementAuthChecks.can_user_attach_credential(
            derouter_params=DeRouter_Params(model="test_model"),
            user_api_key_dict=self.team_admin_user,
        )
        assert result is True
    ...
    def test_can_user_attach_credential_unchanged_existing_allows_any_role(self):
        result = ModelManagementAuthChecks.can_user_attach_credential(
            derouter_params=DeRouter_Params(model="test_model", derouter_credential_name="shared-credential"),
            user_api_key_dict=self.team_admin_user,
            existing_derouter_params=DeRouter_Params(model="test_model", derouter_credential_name="shared-credential"),
        )
        assert result is True
    ...
    def test_can_user_attach_credential_unchanged_encrypted_existing_allows_any_role(self, monkeypatch):
        ...
        result = ModelManagementAuthChecks.can_user_attach_credential(
            derouter_params=DeRouter_Params(model="test_model", derouter_credential_name="shared-credential"),
            user_api_key_dict=self.team_admin_user,
            existing_derouter_params=DeRouter_Params(model="test_model", derouter_credential_name=encrypted_name),
        )
        assert result is True
    ...
    async def test_add_new_model_rejects_credential_attach_for_non_admin(self):
        ...
        with (
            patch("derouter.proxy.proxy_server.prisma_client", mock_prisma),  # test-quality-ok: endpoint reads proxy server globals with no injection seam
            patch("derouter.proxy.proxy_server.store_model_in_db", True),  # test-quality-ok: endpoint reads proxy server globals with no injection seam
            patch("derouter.proxy.proxy_server.premium_user", True),  # test-quality-ok: endpoint reads proxy server globals with no injection seam
            patch(  # test-quality-ok: prior auth check needs a live DB; only the credential check is under test
                "derouter.proxy.management_endpoints.model_management_endpoints.ModelManagementAuthChecks.can_user_make_model_call",
            ...
            with pytest.raises(ProxyException) as exc_info:
                await add_new_model(
                    model_params=Deployment(
                        ...
                        derouter_params=DeRouter_Params(
                            model="openai/gpt-4o", derouter_credential_name="shared-credential"
                        ),
            ...
            assert exc_info.value.code == "403"
            mock_prisma.db.derouter_proxymodeltable.create.assert_not_called()
    ...
    async def test_patch_model_rejects_credential_attach_for_non_admin(self):
        ...
        from derouter.types.router import updateDeRouterParams
        ...
        with (
            patch("derouter.proxy.proxy_server.prisma_client", MagicMock()),  # test-quality-ok: endpoint reads proxy server globals with no injection seam
            patch("derouter.proxy.proxy_server.llm_router", MagicMock()),  # test-quality-ok: endpoint reads proxy server globals with no injection seam
            patch("derouter.proxy.proxy_server.store_model_in_db", True),  # test-quality-ok: endpoint reads proxy server globals with no injection seam
            patch("derouter.proxy.proxy_server.premium_user", True),  # test-quality-ok: endpoint reads proxy server globals with no injection seam
            patch(  # test-quality-ok: stubs the DB row fetch; only the credential check is under test
                "derouter.proxy.management_endpoints.model_management_endpoints.get_db_model",
            ...
            patch(  # test-quality-ok: asserts the DB write is never reached on rejection
                "derouter.proxy.management_endpoints.model_management_endpoints._update_team_model_in_db",
                new=AsyncMock(),
            ) as mock_update,
        ):
            with pytest.raises(ProxyException) as exc_info:
                await patch_model(
                    ...
                    patch_data=updateDeployment(
                        derouter_params=updateDeRouterParams(
                            model="openai/gpt-4o", derouter_credential_name="shared-credential"
                        )
                    ),
                    user_api_key_dict=self.team_admin_user,
...
class MockPrismaDB:
    ...
    async def find_many(self, include=None):
        ...
        return [DeRouter_ModelTable(**aliases) for aliases in self.model_aliases_list]
...
class MockPrismaWrapper:
    def __init__(self, model_aliases_list):
        self.derouter_modeltable = MockPrismaDB(model_aliases_list)
...
class TestDeleteTeamModelAlias:
    ...
    async def test_delete_team_model_alias_success(self):
        ...
        # Verify results
        mock_db = mock_prisma.db.derouter_modeltable
        assert (
...
class TestClearCache:
    ...
    async def test_clear_cache_success(self):
        ...
        with (
            ...
            patch("derouter.proxy.proxy_server.proxy_logging_obj", mock_logging),
            patch("derouter.proxy.proxy_server.verbose_proxy_logger"),
        ):
            await clear_cache()
        ...
        mock_router.model_list = [
            {
                ...
                "derouter_params": {"model": "auto_router/db-auto-router"},
            },
            {
                "model_name": "config-router",
                "model_info": {"id": "config-model-1", "db_model": False},
                "derouter_params": {"model": "auto_router/complexity_router"},
            },
            {
...
class TestClearCachePreservesConfigRouters:
    ...
    async def test_config_router_sharing_name_with_regular_db_model_is_preserved(self):
        ...
        """
        from derouter.proxy.management_endpoints.model_management_endpoints import clear_cache
        ...
        mock_router.model_list = [
            {
                ...
                "model_info": {"id": "db-model-1", "db_model": True},
                "derouter_params": {"model": "openai/gpt-4o"},  # a regular model, NOT a router
            },
        ]
        mock_router.delete_deployment = MagicMock(return_value=True)
        ...
        mock_router.model_list = [
            {
                ...
                "derouter_params": {"model": "auto_router/quality_router/q1"},
            },
            {
                "model_name": "a1",
                "model_info": {"id": "db-a", "db_model": True},
                "derouter_params": {"model": "auto_router/adaptive_router/a1"},
            },
        ]
        mock_router.delete_deployment = MagicMock(return_value=True)
        mock_router.auto_routers = {}
...
class TestDeleteModelClearsRouterRegistry:
    ...
    async def test_delete_model_releases_only_the_deleted_routers_slot(self):
        ...
        """
        import derouter
        from derouter.proxy.management_endpoints.model_management_endpoints import ModelInfoDelete
        from derouter.proxy.management_endpoints.model_management_endpoints import (
        ...
        db_row = DeRouter_ProxyModelTable(
            model_id=model_id,
            model_name="smart-router",
            derouter_params={"model": "auto_router/complexity_router"},
            model_info={"id": model_id},
        ...
        mock_prisma.db.derouter_proxymodeltable = AsyncMock()
        mock_prisma.db.query_raw = AsyncMock(return_value=[])
        mock_prisma.db.derouter_proxymodeltable.find_unique = AsyncMock(return_value=db_row)
        mock_prisma.db.derouter_proxymodeltable.delete = AsyncMock(return_value=db_row)

        real_router = derouter.Router(
            model_list=[
        ...
        _PS = "derouter.proxy.proxy_server"
        with (
...
class TestUpdateModel:
    ...
    async def test_update_model_clears_cache_after_db_write(self):
        ...
        the in-memory router after persisting to DeRouter_ProxyModelTable, otherwise
        model-level guardrails (and any other derouter_params change) silently no-op
        ...
        from derouter.types.router import (
            ModelInfo,
            updateDeployment,
            updateDeRouterParams,
        )

        model_id = "db-model-under-test"

        existing_row = MagicMock()
        existing_row.derouter_params = {
            "model": "openai/gpt-4o-mini",
            "api_key": "sk-existing",
        }
        existing_row.model_dump.return_value = {
            "model_name": "gpt-4o-mini",
            "derouter_params": existing_row.derouter_params,
            "model_info": {"id": model_id},
        }
        ...
        mock_prisma = MagicMock()
        mock_prisma.db.derouter_proxymodeltable.find_unique = AsyncMock(
            return_value=existing_row
        )
        mock_prisma.db.derouter_proxymodeltable.update = AsyncMock(
        ...
        with (
            ...
            patch(
                "derouter.proxy.management_endpoints.model_management_endpoints.encrypt_value_helper",
                side_effect=lambda value: value,
            ),
            patch(
                "derouter.proxy.management_endpoints.model_management_endpoints.clear_cache",
                new=AsyncMock(
            ...
            await update_model(
                model_params=updateDeployment(
                    derouter_params=updateDeRouterParams(guardrails=["g1"]),
                    model_info=ModelInfo(id=model_id),
                ),
                user_api_key_dict=admin_user,
            )

            mock_prisma.db.derouter_proxymodeltable.update.assert_awaited_once()
            mock_clear_cache.assert_awaited_once_with()


class TestUpdatePublicModelGroups:
    """Test that update_public_model_groups correctly sets derouter.public_model_groups
    ...
    async def test_public_model_groups_set_after_get_config(self):
        ...
        sets derouter.public_model_groups to the old DB value. The endpoint must set
        ...
        # Simulate get_config() overwriting derouter.public_model_groups with old DB value
        async def mock_get_config(*args, **kwargs):
            # This simulates _update_config_from_db calling setattr(derouter, "public_model_groups", old_value)
            derouter.public_model_groups = old_db_models
            return {"derouter_settings": {"public_model_groups": old_db_models}}
        ...
        original_value = getattr(derouter, "public_model_groups", None)
        try:
            with (
                patch(
                ...
                patch(
                    "derouter.proxy.proxy_server.store_model_in_db",
                    True,
                ),
            ):
            ...
            assert derouter.public_model_groups == new_models
            assert result["public_model_groups"] == new_models
        finally:
            derouter.public_model_groups = original_value
    ...
    async def test_useful_links_set_after_get_config(self):
        ...
        async def mock_get_config(*args, **kwargs):
            derouter.public_model_groups_links = old_links
            return {"derouter_settings": {"public_model_groups_links": old_links}}
        ...
        original_value = getattr(derouter, "public_model_groups_links", None)
        try:
            with patch(
                "derouter.proxy.proxy_server.proxy_config",
                mock_proxy_config,
            ...
            assert derouter.public_model_groups_links == new_links
            assert result["useful_links"] == new_links
        finally:
            derouter.public_model_groups_links = original_value
...
class TestTeamModelSiblingRouting:
    ...
    async def test_no_model_aliases_written_for_team_models(self):
        ...
        from derouter.types.router import ModelInfo
        ...
        for api_base in ["https://eastus.example.com", "https://westus.example.com"]:
            ...
            with (
                patch(
                    "derouter.proxy.management_endpoints.model_management_endpoints._add_model_to_db",
                    side_effect=mock_add_model_to_db,
                ),
                patch(
                    "derouter.proxy.management_endpoints.model_management_endpoints.team_model_add",
                    mock_team_model_add,
                ),
            ):
    ...
    async def test_router_finds_all_sibling_team_deployments(self):
        ...
        api_bases = {d["derouter_params"]["api_base"] for d in healthy}
        assert api_bases == {
...
class TestTeamModelUpdate:
    ...
    async def test_patch_model_with_team_id_creates_proper_setup(self):
        ...
        with (
            ...
            patch(
                "derouter.proxy.management_endpoints.model_management_endpoints.update_team"
            ) as mock_update_team,
        ):
    ...
    async def test_rename_preserves_old_name_when_siblings_exist(self):
        ...
        db_model = Deployment(
            ...
            derouter_params=DeRouter_Params(model="azure/gpt-4o-mini"),
            model_info=ModelInfo(
        ...
        with (
            patch(
                "derouter.proxy.management_endpoints.model_management_endpoints.team_model_delete"
            ) as mock_delete,
            patch(
                "derouter.proxy.management_endpoints.model_management_endpoints.team_model_add"
            ) as mock_add,
    ...
    async def test_a_refused_row_write_leaves_the_team_untouched(self):
        ...
        with (
            patch(  # test-quality-ok: the team auth check needs a live DB; the write order is what is under test
                "derouter.proxy.management_endpoints.model_management_endpoints.ModelManagementAuthChecks.allow_team_model_action",
                AsyncMock(return_value=True),
            ),
            patch("derouter.proxy.proxy_server.premium_user", True),  # test-quality-ok: team models are premium-gated through a proxy global with no injection seam
    ...
    def test_get_public_model_name_28382_dashboard_echo_preserves_public_name(self):
        ...
        db_model = Deployment(
            ...
            derouter_params=DeRouter_Params(model="azure/gpt-5.2-low-rpm-testing"),
            model_info=ModelInfo(
                team_id="test-team",
    ...
    def test_get_public_model_name_top_level_rename_wins_over_stale_model_info(self):
        ...
        db_model = Deployment(
            ...
            derouter_params=DeRouter_Params(model="azure/gpt-4.1"),
            model_info=ModelInfo(
    ...
    def test_get_public_model_name_last_resort_returns_db_model_name(self):
        ...
        db_model = Deployment(
            ...
            derouter_params=DeRouter_Params(model="azure/legacy"),
            model_info=ModelInfo(team_id="test-team"),
        )
        patch_data = updateDeployment(
            model_info=ModelInfo(team_id="test-team"),
...
class TestModelInfoEndpoint:
    ...
    async def test_model_info_accessible_model_success(self):
        ...
        from derouter.proxy.proxy_server import model_info
        ...
        with (
            ...
            patch(
                "derouter.proxy.utils.get_available_models_for_user",
                new=AsyncMock(return_value=["gpt-4", "claude-3", "gpt-3.5-turbo"]),
            ),
            patch("derouter.get_llm_provider", return_value=(None, "openai", None, None)),
        ):
            ...
            mock_router.get_deployment_by_model_group_name.return_value = Deployment(
                model_name="gpt-4",
                derouter_params=DeRouter_Params(model="openai/gpt-4"),
    ...
    async def test_model_info_team_model_access(self):
        ...
        with (
            ...
            patch("derouter.get_llm_provider", return_value=(None, "custom", None, None)),
        ):
            mock_router.get_fully_blocked_model_names.return_value = set()
            mock_router.get_model_list.return_value = []
            mock_router.get_configured_token_limits.return_value = (None, None)
            mock_router.get_deployment_by_model_group_name.return_value = Deployment(
                ...
                derouter_params=DeRouter_Params(model="custom/team-model-1"),
...
class TestAddAndDeleteModelLifecycle:
    ...
    async def test_add_then_delete_model(self):
        ...
        # Build a real DeRouter_ProxyModelTable for the DB mock to return
        db_row = DeRouter_ProxyModelTable(
            model_id=model_id,
            model_name="lifecycle-model",
            derouter_params={"model": "openai/gpt-4.1-nano"},
            model_info={"id": model_id},
        ...
        mock_prisma.db.derouter_proxymodeltable = AsyncMock()
        mock_prisma.db.query_raw = AsyncMock(return_value=[])
        mock_prisma.db.derouter_proxymodeltable.create = AsyncMock(return_value=db_row)
        mock_prisma.db.derouter_proxymodeltable.find_unique = AsyncMock(
            return_value=db_row
        )
        mock_prisma.db.derouter_proxymodeltable.delete = AsyncMock(return_value=db_row)
        ...
        _PS = "derouter.proxy.proxy_server"
        _ENCRYPT = "derouter.proxy.management_endpoints.model_management_endpoints.encrypt_value_helper"
        with (
            patch(f"{_PS}.prisma_client", mock_prisma),
            patch(f"{_PS}.store_model_in_db", True),
            ...
            from derouter.proxy.proxy_server import ProxyException
...
class TestDeleteTeamBYOKModelGhost:
    ...
    never creates a derouter_modeltable alias row. delete_model used to strip
    ...
    async def test_delete_strips_public_name_and_refreshes_cache(self):
        ...
        mock_prisma.db.derouter_proxymodeltable.find_many = AsyncMock(return_value=[])
        mock_prisma.db.derouter_teamtable = AsyncMock()
        ...
        mock_prisma.db.derouter_modeltable = AsyncMock()
        mock_prisma.db.derouter_modeltable.find_many = AsyncMock(return_value=[])
        ...
        _PS = "derouter.proxy.proxy_server"
        _MOD = "derouter.proxy.management_endpoints.model_management_endpoints"
        with (
            patch(f"{_PS}.prisma_client", mock_prisma),
            patch(f"{_PS}.store_model_in_db", True),
            patch(f"{_PS}.premium_user", True),
        ...
        mock_prisma.db.derouter_teamtable.update.assert_awaited_once()
        update_kwargs = mock_prisma.db.derouter_teamtable.update.await_args.kwargs
        ...
        mock_prisma.db.derouter_modeltable.find_many.assert_awaited()
    ...
    async def test_delete_non_internal_team_model_still_scans_aliases(self):
        ...
        mock_prisma.db.derouter_teamtable = AsyncMock()
        mock_prisma.db.derouter_teamtable.find_unique = AsyncMock(return_value=team_row)
        mock_prisma.db.derouter_teamtable.update = AsyncMock(return_value=team_row)
        mock_prisma.db.derouter_modeltable = AsyncMock()
    ...
    async def test_delete_keeps_public_name_when_sibling_backs_it(self):
        ...
        mock_prisma.db.derouter_proxymodeltable.delete = AsyncMock(
            return_value=deleted_row
        )
        # After the deleted replica's row is gone, the sibling still backs the public name.
        mock_prisma.db.derouter_proxymodeltable.find_many = AsyncMock(
        ...
        mock_prisma.db.derouter_teamtable.update.assert_not_awaited()
    ...
    async def test_delete_legacy_team_model_scrubs_stale_alias_and_keeps_gateway_access(
        ...
        mock_prisma.db.derouter_teamtable.update = AsyncMock(return_value=team_row)
        mock_prisma.db.derouter_modeltable = AsyncMock()
        mock_prisma.db.derouter_modeltable.find_many = AsyncMock(
            return_value=[alias_row]
        )
        mock_prisma.db.derouter_modeltable.update = AsyncMock()
        ...
        mock_prisma.db.derouter_modeltable.update.assert_awaited_once()
        alias_update_kwargs = mock_prisma.db.derouter_modeltable.update.await_args.kwargs
    ...
    async def test_delete_replica_keeps_alias_while_surviving_replica_serves_it(self):
        ...
        assert "deleted successfully" in result["message"]
        mock_prisma.db.derouter_modeltable.find_many.assert_not_awaited()
        mock_prisma.db.derouter_modeltable.update.assert_not_awaited()
        mock_prisma.db.derouter_teamtable.update.assert_not_awaited()
...
class TestDeleteModelTeamAuth:
    ...
    def _orphaned_model_mocks(self, team_id, model_id):
        ...
        mock_prisma.db.derouter_teamtable = AsyncMock()
        mock_prisma.db.derouter_teamtable.find_unique = AsyncMock(return_value=None)
        mock_prisma.db.derouter_teamtable.update = AsyncMock()
        mock_prisma.db.derouter_modeltable = AsyncMock()
        mock_prisma.db.derouter_modeltable.find_many = AsyncMock(return_value=[])
        return mock_prisma
    ...
    async def test_proxy_admin_can_delete_model_when_team_deleted(self):
        ...
        assert "deleted successfully" in result["message"]
        mock_prisma.db.derouter_proxymodeltable.delete.assert_awaited_once()
    ...
    async def test_non_admin_cannot_delete_model_when_team_deleted(self):
        ...
        mock_prisma.db.derouter_proxymodeltable.delete.assert_not_awaited()
    ...
    async def test_live_team_delete_looks_up_team_once(self):
        ...
        assert str(exc_info.value.code) == "403"
        assert mock_prisma.db.derouter_teamtable.find_unique.await_count == 1
        mock_prisma.db.derouter_proxymodeltable.delete.assert_not_awaited()
...
class _TxPrismaClient:
    ...
    def __init__(self, rows):
        ...
        tx = MagicMock()
        tx.derouter_proxymodeltable = self._table
        outer = self
...
class TestUpdateDBModelKeepsLegacyDropParams:
    def test_partial_patch_keeps_encrypted_string_drop_params(self, monkeypatch):
        ...
        from derouter.proxy.management_endpoints.model_management_endpoints import update_db_model
        ...
        result = update_db_model(
            db_model=legacy_row,
            updated_patch=updateDeployment(derouter_params=updateDeRouterParams(api_key="sk-new")),
        )

        stored = json.loads(result["derouter_params"])
        assert decrypt_value_helper(value=stored["drop_params"], key="drop_params") == "true"


def _build_db_model_with_pricing():
    """Wildcard deployment with custom pricing in derouter_params; Deployment.__init__
...
class TestUpdateDBModelClearPricing:
    ...
    `derouter_params` and `model_info` (SPECIAL_MODEL_INFO_PARAMS are mirrored
    ...
    def test_clear_input_cost_removes_from_both_blobs(self):
        ...
        result = update_db_model(
            ...
            updated_patch=updateDeployment(
                derouter_params=updateDeRouterParams(input_cost_per_token=None)
            ),
        )

        params = json.loads(result["derouter_params"])
    ...
    def test_clear_output_cost_removes_from_both_blobs(self):
        ...
        result = update_db_model(
            db_model=_build_db_model_with_pricing(),
            updated_patch=updateDeployment(
                derouter_params=updateDeRouterParams(output_cost_per_token=None)
            ),
        )
    ...
    def test_non_null_pricing_update_still_works(self):
        ...
        result = update_db_model(
            db_model=_build_db_model_with_pricing(),
            updated_patch=updateDeployment(
                derouter_params=updateDeRouterParams(input_cost_per_token=0.000005)
            ),
        )
    ...
    def test_omitted_pricing_field_is_preserved(self):
        ...
        result = update_db_model(
            db_model=_build_db_model_with_pricing(),
            updated_patch=updateDeployment(
                derouter_params=updateDeRouterParams(output_cost_per_token=0.000007)
            ),
        )
    ...
    def test_null_on_non_pricing_field_does_not_clear(self):
        ...
        result = update_db_model(
            db_model=db_model,
            updated_patch=updateDeployment(
                derouter_params=updateDeRouterParams(api_base=None)
            ),
        )
    ...
    def test_clear_survives_model_info_passthrough_with_old_pricing(self):
        ...
        re-serializes the source blob. The derouter_params null must beat the
        model_info merge — i.e. the clear runs after both merges, not between.
        """
        from derouter.proxy.management_endpoints.model_management_endpoints import (
            update_db_model,
        )
        from derouter.types.router import ModelInfo, updateDeRouterParams
    ...
    def test_clear_via_model_info_clears_both_blobs(self):
        ...
        via the model_info patch should clear it from derouter_params as well."""
        from derouter.proxy.management_endpoints.model_management_endpoints import (
            update_db_model,
        )
        from derouter.types.router import ModelInfo
    ...
    def test_clear_cache_read_cost_removes_from_both_blobs(self):
        ...
        result = update_db_model(
            db_model=db_model,
            updated_patch=updateDeployment(
                derouter_params=updateDeRouterParams(cache_read_input_token_cost=None)
            ),
        )
    ...
    def test_clear_cache_write_cost_removes_from_both_blobs(self):
        ...
        result = update_db_model(
            db_model=db_model,
            updated_patch=updateDeployment(
                derouter_params=updateDeRouterParams(cache_creation_input_token_cost=None)
            ),
        )
...
class TestPatchModelBlockedAuthGate:
    ...
    async def test_team_admin_cannot_toggle_blocked(self):
        ...
        existing_row = MagicMock()
        existing_row.derouter_params = {"model": "openai/gpt-4o-mini"}
        existing_row.model_dump.return_value = {
            "model_name": "gpt-4o-mini",
            "derouter_params": existing_row.derouter_params,
            "model_info": {"id": "m1"},
        ...
        with (
            patch("derouter.proxy.proxy_server.prisma_client", mock_prisma),
            patch("derouter.proxy.proxy_server.llm_router", MagicMock(**{"get_model_ids.return_value": ["m1"]})),
            patch("derouter.proxy.proxy_server.store_model_in_db", True),
            patch("derouter.proxy.proxy_server.premium_user", True),
            patch(
                "derouter.proxy.management_endpoints.model_management_endpoints.ModelManagementAuthChecks.can_user_make_model_call",
...
class TestPatchModelRowDeletedBeforeWrite:
    ...
    async def test_patch_model_404s_when_update_returns_none(self):
        ...
        mock_prisma.db.derouter_proxymodeltable.update = AsyncMock(return_value=None)

        with (
            patch("derouter.proxy.proxy_server.prisma_client", mock_prisma),  # test-quality-ok: proxy_server module global is the endpoint's only injection point
            patch("derouter.proxy.proxy_server.llm_router", MagicMock(**{"get_model_ids.return_value": ["m1"]})),  # test-quality-ok: proxy_server module global is the endpoint's only injection point
            patch("derouter.proxy.proxy_server.store_model_in_db", True),  # test-quality-ok: proxy_server module global is the endpoint's only injection point
...
class TestWriteSurfacesReloadDrop:
    ...
    def test_reload_serving_verdict_matrix(self, monkeypatch):
        ...
        monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", live_router)
...
class TestConcurrentModelWritesDoNotEvictEachOther:
    ...
    async def test_reconciles_serialize_so_no_stale_snapshot_can_evict(self, monkeypatch):
        ...
        from derouter.proxy._types import ReconcileOutcome
        from derouter.proxy.proxy_server import ProxyConfig
    ...
    def test_verdict_trusts_the_lock_captured_snapshot_over_a_live_reread(self, monkeypatch):
        ...
        mid_wipe_router = derouter.Router(model_list=[])
        monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", mid_wipe_router)
...
class TestDeleteEvictionsHoldTheReconcileLock:
    ...
    async def _assert_evicts_under_lock(monkeypatch, call_endpoint, model_id: str) -> None:
        ...
        """
        lock = asyncio.Lock()
        monkeypatch.setattr("derouter.proxy.proxy_server.MODEL_RECONCILE_LOCK", lock)
    ...
    async def test_delete_model_waits_for_an_in_flight_reconcile(self, monkeypatch):
        ...
        prisma = MagicMock()
        prisma.db.derouter_proxymodeltable = table
        ...
        monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", router)
    ...
    async def test_delete_team_models_waits_for_an_in_flight_reconcile(self, monkeypatch):
        ...
        tx = MagicMock()
        tx.derouter_proxymodeltable.find_many = AsyncMock(return_value=[deleted_row])
        tx.derouter_proxymodeltable.delete_many = AsyncMock(return_value=1)
        ...
        monkeypatch.setattr(
            "derouter.proxy.management_endpoints.model_management_endpoints.publish_config_change",
            AsyncMock(return_value=None),
        )
        monkeypatch.setattr(
            "derouter.proxy.management_endpoints.model_management_endpoints.coordination_redis_cache",
...
class TestStrategyRouterWriteValidation:
    """Management write paths must reject derouter_params.model values that would
    ...
    def _stored_complexity_params(self) -> DeRouter_Params:
    ...
    def _db_complexity_router(self, model_id: str) -> Deployment:
        return Deployment(
            ...
            derouter_params=self._stored_complexity_params(),
            model_info={"id": model_id},
        )
    ...
    def test_double_prefix_rejected_against_stored_params(self):
        ...
        violation = _strategy_router_write_violation(
            incoming_params=updateDeRouterParams(model="auto_router/auto_router/complexity_router"),
    ...
    def test_prefix_strip_rejected_against_stored_params(self):
        ...
        violation = _strategy_router_write_violation(
            incoming_params=updateDeRouterParams(model="complexity_router"),
            existing_params=self._stored_complexity_params(),
        )
        assert violation is not None
    ...
    def test_patch_without_model_is_not_judged(self):
        ...
        assert (
            _strategy_router_write_violation(
                incoming_params=updateDeRouterParams(rpm=10),
                existing_params=self._stored_complexity_params(),
            )
            is None
        )
    ...
    def test_model_less_patch_cannot_attach_router_config_to_a_regular_model(self, config: dict[str, object]) -> None:
        ...
        violation = _strategy_router_write_violation(
            incoming_params=updateDeRouterParams(complexity_router_config=config),
            existing_params=DeRouter_Params(model="openai/gpt-4o-mini"),
        )
    ...
    def test_effective_params_decrypts_a_stored_complexity_router_model(self, monkeypatch) -> None:
        ...
        monkeypatch.setenv("DEROUTER_SALT_KEY", "test-salt")
        encrypted_model = encrypt_value_helper("auto_router/complexity_router")
        effective_params = _effective_complexity_router_params(
            updateDeRouterParams(complexity_router_config={"tiers": {"SIMPLE": "gpt-4o-mini"}}),
            DeRouter_Params(model=encrypted_model),
        )
    ...
    def test_model_less_patch_keeps_a_complexity_router_in_scope(self) -> None:
        ...
        assert (
            _strategy_router_write_violation(
                incoming_params=updateDeRouterParams(
    ...
    def test_restore_of_corrupted_row_is_allowed(self):
        ...
        corrupted = DeRouter_Params(
            model="auto_router/auto_router/complexity_router",
            complexity_router_config={"tiers": {"SIMPLE": "gpt-4o-mini"}},
        )
        assert (
            _strategy_router_write_violation(
                incoming_params=updateDeRouterParams(model="auto_router/complexity_router"),
    ...
    def test_create_with_empty_keyword_rule_rejected(self):
        ...
        violation = _strategy_router_write_violation(
            incoming_params=DeRouter_Params(
                model="auto_router/complexity_router",
    ...
    def test_config_only_patch_is_judged_on_the_config_alone(self):
        ...
        violation = _strategy_router_write_violation(
            ...
            existing_params=DeRouter_Params(model="c2VjcmV0-encrypted-at-rest"),
        )
        assert violation is not None
        assert "complexity_router_config is invalid" in violation
        ...
        with (
            ...
            with pytest.raises(ProxyException) as exc_info:
                await patch_model(
                    model_id=model_id,
                    patch_data=updateDeployment(
                        derouter_params=updateDeRouterParams(model="auto_router/auto_router/complexity_router")
                    ),
                    user_api_key_dict=admin,
                )
        ...
        with (
            ...
            with pytest.raises(ProxyException) as exc_info:
                await add_new_model(
                    model_params=Deployment(
                        model_name="my-auto-router",
                        derouter_params=DeRouter_Params(model="auto_router/complexity_router"),
    ...
    def test_settings_beside_the_config_rejected_on_a_patch_of_a_stored_router(self, stored_field):
        ...
        violation = _strategy_router_write_violation(
            incoming_params=updateDeRouterParams(tier_boundaries={"simple_medium": 0.1}),
            existing_params=DeRouter_Params(model="auto_router/complexity_router", **{stored_field: stored}),
        )
        assert violation is not None
        assert "tier_boundaries" in violation
    ...
    def _live_router_holding_one_capability(limit: int | None, config: Mapping[str, object]) -> Router:
        return Router(
            model_list=[
                {"model_name": "gpt-4o-mini", "derouter_params": {"model": "openai/gpt-4o-mini", "api_key": "k"}},
                {
                    "model_name": "held",
                    "derouter_params": {
                        "model": "auto_router/complexity_router",
    ...
    class _FakeTx:
        ...
        def __init__(self, db_models: list[str], tuning_rows: list[dict[str, object]] | None = None) -> None:
            ...
            self.derouter_proxymodeltable = MagicMock(
                create=AsyncMock(),
                update=AsyncMock(),
                find_many=AsyncMock(side_effect=self._find_many),
            )

        async def _find_many(self, where: object = None) -> tuple[DeRouter_ProxyModelTable, ...]:
            json.dumps(where)  # prisma serializes the filter with json.dumps and rejects a mappingproxy
            return tuple(DeRouter_ProxyModelTable.model_validate(row) for row in self.tuning_rows)
    ...
    def test_effective_complexity_router_config(
        ...
        incoming_params = None if incoming is None else updateDeRouterParams(
            complexity_router_config=None if incoming == "no-config" else incoming
        )
        existing_params = None if existing is None else updateDeRouterParams(complexity_router_config=existing)
    ...
    async def test_auto_router_capability_slot_matrix(
        ...
        from derouter.router_utils.auto_router_model_naming import gated_capability_of
        ...
        with (
            patch("derouter.proxy.proxy_server._license_check.auto_router_capability_limit", lambda: limit),  # test-quality-ok: the guard reads the proxy license singleton with no injection seam
            patch("derouter.proxy.proxy_server.llm_router", live_router),  # test-quality-ok: the guard reads the proxy router global with no injection seam
        ...
        if expected == "plain":
            ...
            fake.derouter_proxymodeltable.create.assert_awaited_once_with(data={})
            assert fake.tx_obj.raw_calls == []
            return
        assert handle is fake.tx_obj.derouter_proxymodeltable
        published.assert_awaited_once_with(redis_cache=None, object_type="derouter_proxymodeltable")
    ...
    def _db_router_row(model_id: str, config: Mapping[str, object]) -> dict[str, object]:
        return {
            ...
            "derouter_params": {"model": "auto_router/complexity_router", "complexity_router_config": dict(config)},
            "model_info": {"id": model_id, "db_model": True},
        }
    ...
    async def test_slot_enforces_baseline_relative_tuning_quota(
        ...
        from derouter.proxy.management_endpoints.model_management_endpoints import _auto_router_capability_slot
        from derouter.router_utils.auto_router_tuning_baseline import snapshot_tuning_baselines
        ...
        with (
            patch("derouter.proxy.proxy_server._license_check.auto_router_capability_limit", lambda: limit),  # test-quality-ok: the guard reads the proxy license singleton with no injection seam
            patch("derouter.proxy.proxy_server.llm_router", None),  # test-quality-ok: the guard reads the proxy router global with no injection seam
            patch("derouter.proxy.proxy_server.heuristic_v1_tuning_baselines", baselines),  # test-quality-ok: baselines are a startup-loaded proxy global with no injection seam
        ):
            if expected == "refused":
                with pytest.raises(HTTPException) as exc_info:
    ...
    async def test_add_new_model_refuses_a_second_tuned_heuristic_v1_router_without_a_model_id(self) -> None:
        ...
        from derouter.proxy._types import ProxyException
        from derouter.proxy.management_endpoints.model_management_endpoints import add_new_model
        from derouter.router_utils.auto_router_tuning_baseline import snapshot_tuning_baselines
        ...
        with (
            patch("derouter.proxy.proxy_server.prisma_client", fake),  # test-quality-ok: endpoint reads proxy server globals with no injection seam
            patch("derouter.proxy.proxy_server.llm_router", None),  # test-quality-ok: endpoint reads proxy server globals with no injection seam
            patch("derouter.proxy.proxy_server.store_model_in_db", True),  # test-quality-ok: endpoint reads proxy server globals with no injection seam
            patch("derouter.proxy.proxy_server.premium_user", True),  # test-quality-ok: endpoint reads proxy server globals with no injection seam
            patch("derouter.proxy.proxy_server._license_check.auto_router_capability_limit", lambda: 1),  # test-quality-ok: endpoint reads proxy server globals with no injection seam
            patch("derouter.proxy.proxy_server.heuristic_v1_tuning_baselines", baselines),  # test-quality-ok: baselines are a startup-loaded proxy global with no injection seam
            ...
            fake.tx_obj.derouter_proxymodeltable.create.assert_not_awaited()
            fake.derouter_proxymodeltable.create.assert_not_awaited()
    ...
    async def test_slot_skips_tuning_quota_when_no_baseline_is_loaded(self) -> None:
        ...
        fake = self._FakeDb([])
        with (
            patch("derouter.proxy.proxy_server._license_check.auto_router_capability_limit", lambda: 1),  # test-quality-ok: the guard reads the proxy license singleton with no injection seam
            patch("derouter.proxy.proxy_server.llm_router", None),  # test-quality-ok: the guard reads the proxy router global with no injection seam
            patch("derouter.proxy.proxy_server.heuristic_v1_tuning_baselines", None),  # test-quality-ok: baselines are a startup-loaded proxy global with no injection seam
        ):
    ...
    async def test_team_model_bookkeeping_runs_after_the_slot_is_released(self) -> None:
        ...
        from derouter.proxy.management_endpoints.model_management_endpoints import _add_team_model_to_db
        from derouter.types.router import ModelInfo
        ...
        deployment = Deployment(
            ...
            derouter_params=DeRouter_Params(model="auto_router/complexity_router", complexity_router_config=self._V2),
    ...
    async def test_model_less_patch_rejects_router_config_on_a_regular_model(self) -> None:
        ...
        from derouter.proxy._types import ProxyException
        from derouter.proxy.management_endpoints.model_management_endpoints import patch_model
        from derouter.types.router import updateDeRouterParams
        ...
        regular = Deployment(
            ...
            derouter_params=DeRouter_Params(model="openai/gpt-4o-mini"),
            model_info={"id": model_id},
        )
        fake = self._FakeDb([])
        with (
            patch("derouter.proxy.proxy_server.prisma_client", fake),  # test-quality-ok: endpoint reads proxy globals with no injection seam
            patch("derouter.proxy.proxy_server.llm_router", None),  # test-quality-ok: config-based lookup must be absent to drive the stored-row branch
            patch("derouter.proxy.proxy_server.store_model_in_db", True),  # test-quality-ok: endpoint reaches its DB-write branch only with this process setting
            patch("derouter.proxy.proxy_server.premium_user", True),  # test-quality-ok: authorization branch reads the proxy-wide premium flag
            ...
            with pytest.raises(ProxyException) as exc_info:
                await patch_model(
                    model_id=model_id,
                    patch_data=updateDeployment(
                        derouter_params=updateDeRouterParams(complexity_router_config=self._CUSTOM_TIERS)
                    ),
                    user_api_key_dict=admin,
                )
        ...
        assert fake.tx_obj.raw_calls == []
        assert fake.tx_obj.derouter_proxymodeltable.update.await_count == 0
        assert fake.derouter_proxymodeltable.update.await_count == 0
    ...
    async def test_model_less_legacy_update_rejects_router_config_on_a_regular_model(self) -> None:
        ...
        from derouter.proxy._types import ProxyException
        from derouter.proxy.management_endpoints.model_management_endpoints import update_model
        from derouter.types.router import ModelInfo, updateDeRouterParams
        ...
        existing_row.derouter_params = regular.derouter_params.model_dump()
    ...
    async def test_patch_model_refuses_switching_another_router_to_heuristic_v2(self) -> None:
        ...
        with (
            ...
            with pytest.raises(HTTPException) as exc_info:
                await patch_model(
                    model_id=model_id,
                    patch_data=updateDeployment(derouter_params=updateDeRouterParams(complexity_router_config=self._V2)),
                    user_api_key_dict=admin,
                )
            assert exc_info.value.status_code == 403
            fake.tx_obj.derouter_proxymodeltable.update.assert_not_awaited()
            fake.derouter_proxymodeltable.update.assert_not_awaited()
    ...
    async def test_update_model_refuses_switching_another_router_to_heuristic_v2(self) -> None:
        ...
        existing_row.derouter_params = existing_row.model_dump.return_value["derouter_params"]
        ...
        with (
            ...
            with pytest.raises(ProxyException) as exc_info:
                await update_model(
                    model_params=updateDeployment(
                        derouter_params=updateDeRouterParams(complexity_router_config=self._V2),
                        model_info=ModelInfo(id=model_id),
                    ),
                    user_api_key_dict=admin,
                )
    ...
    async def test_update_model_rejects_prefix_strip(self):
        ...
        mock_prisma = MagicMock()
        mock_prisma.db.derouter_proxymodeltable.find_unique = AsyncMock(return_value=existing_row)
        mock_prisma.db.derouter_proxymodeltable.update = AsyncMock()
        ...
        with (
            ...
            with pytest.raises(ProxyException) as exc_info:
                await update_model(
                    model_params=updateDeployment(
                        derouter_params=updateDeRouterParams(model="complexity_router"),
                        model_info=ModelInfo(id=model_id),
                    ),
                    user_api_key_dict=admin,
                )
            assert "does not start with" in str(exc_info.value.message)
            mock_prisma.db.derouter_proxymodeltable.update.assert_not_awaited()
...
class TestAutoRouterClassifierDefaultPrompt:
    ...
    async def test_returns_the_prompt_the_router_would_send(self):
        ...
        from derouter.router_strategy.complexity_router import classification_system_prompt
    ...
    async def test_rubric_preset_selects_the_calibration_examples(self):
        ...
        from derouter.router_strategy.complexity_router import ClassificationRubric, classification_system_prompt
    ...
    async def test_built_in_opening_preview_uses_the_built_in_tiers(self):
        ...
        from derouter.router_strategy.complexity_router import ClassificationRubric, built_in_tier_classification_prompt
        from derouter.router_strategy.complexity_router.config import ComplexityRouterConfig
    ...
    async def test_custom_tier_examples_preview_matches_what_the_router_would_send(self):
        from derouter.router_strategy.complexity_router import custom_tier_classification_prompt
        from derouter.router_strategy.complexity_router.config import TierDefinition
    ...
    async def test_a_built_in_name_without_a_description_resolves_the_shipped_criteria(self):
        ...
        from derouter.router_strategy.complexity_router import ComplexityTier
        from derouter.router_strategy.complexity_router.complexity_router import _CLASSIFICATION_TIER_CRITERIA
    ...
    def test_the_prompt_preview_is_readable_by_an_admin_viewer_like_the_get_beside_it(self):
        ...
        assert "/auto_router/classifier/default_prompt" in DeRouterRoutes.admin_viewer_routes.value
...
class TestEnforceRpmTpmOnModelAdd:
    def test_passes_when_disabled_even_without_limits(self):
        assert (
            _raise_if_rate_limits_required_but_missing(
                derouter_params=DeRouter_Params(model="azure/gpt-5.2"),
                enforced=False,
            )
            is None
        )
    ...
    def test_passes_when_enabled_and_both_set(self):
        assert (
            _raise_if_rate_limits_required_but_missing(
                derouter_params=DeRouter_Params(model="azure/gpt-5.2", rpm=10, tpm=1000),
                enforced=True,
            )
            is None
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (DeRouter_Params(model="azure/gpt-5.2"), "rpm and tpm"),
            (DeRouter_Params(model="azure/gpt-5.2", rpm=10), "tpm"),
            (DeRouter_Params(model="azure/gpt-5.2", tpm=1000), "rpm"),
            (DeRouter_Params(model="azure/gpt-5.2", rpm=0, tpm=1000), "rpm"),
            (DeRouter_Params(model="azure/gpt-5.2", rpm=10, tpm=-1), "tpm"),
        ],
    )
    def test_raises_when_enabled_and_missing(self, params, expected_missing):
        ...
        with pytest.raises(ProxyException) as exc_info:
            _raise_if_rate_limits_required_but_missing(derouter_params=params, enforced=True)
...
class TestBlockModelResponseSerialization:
    ...
    def test_block_routes_serialize_prisma_row_to_200(self, route, blocked):
        ...
        row_fields = {
            "model_id": "m-block-1",
            "model_name": "gpt-4o-mini",
            "derouter_params": json.dumps({"model": "openai/gpt-4o-mini", "api_key": "encrypted-value"}),
        ...
        existing_row = prisma_models.DeRouter_ProxyModelTable(blocked=not blocked, **row_fields)
        updated_row = prisma_models.DeRouter_ProxyModelTable(blocked=blocked, **row_fields)

        mock_prisma = MagicMock()
        mock_prisma.db.derouter_proxymodeltable.find_unique = AsyncMock(return_value=existing_row)
        mock_prisma.db.derouter_proxymodeltable.update = AsyncMock(return_value=updated_row)
        ...
        try:
            with (
                ...
                patch(  # test-quality-ok: proxy_server module global is the endpoint's only injection point
                    "derouter.proxy.proxy_server.llm_router",
                    MagicMock(**{"get_model_ids.return_value": ["m-block-1"]}),
                ),
                patch("derouter.proxy.proxy_server.redis_usage_cache", None),  # test-quality-ok: proxy_server module global is the endpoint's only injection point
                ...
                patch(  # test-quality-ok: audit logging is a background side effect outside this test's contract
                    "derouter.proxy.management_endpoints.model_management_endpoints.create_object_audit_log",
                    new=AsyncMock(return_value=None),
                ),
            ):
        ...
        assert body["derouter_params"] == {"model": "openai/gpt-4o-mini", "api_key": "encrypted-value"}
...
class TestAccessGroupModelSync:
    ...
    _PS = "derouter.proxy.proxy_server"
    _MOD = "derouter.proxy.management_endpoints.model_management_endpoints"
    _INVALIDATE = "derouter.proxy.management_helpers.access_group_model_sync.invalidate_access_group_caches"
    ...
    def _prisma_with_row(model_id: str, model_name: str, deployment_count: int):
        row = DeRouter_ProxyModelTable(
            model_id=model_id,
            model_name=model_name,
            derouter_params={"model": "openai/gpt-5.6"},
            model_info={"id": model_id},
            created_by="admin",
            updated_by="admin",
        )
        ...
        mock_prisma.db.derouter_proxymodeltable = AsyncMock()
        mock_prisma.db.derouter_proxymodeltable.find_unique = AsyncMock(return_value=row)
        mock_prisma.db.derouter_proxymodeltable.update = AsyncMock(return_value=row)
        mock_prisma.db.derouter_proxymodeltable.delete = AsyncMock(return_value=row)
        return mock_prisma
    ...
    def _access_group_updates(mock_prisma):
        return [
            ...
            if call.args[0].startswith('UPDATE "DeRouter_AccessGroupTable"')
        ]
    ...
    async def test_patch_model_rename_rewrites_the_groups_that_named_the_model(self):
        ...
        written = mock_prisma.db.derouter_proxymodeltable.update.await_args.kwargs["data"]
    ...
    async def test_delete_model_drops_the_name_from_groups_when_nothing_backs_it(self):
        from derouter.proxy.management_endpoints.model_management_endpoints import ModelInfoDelete, delete_model
        ...
        with self._endpoint_env(mock_prisma, router) as invalidate:
            await update_model(
                model_params=updateDeployment(
                    ...
                    derouter_params=updateDeRouterParams(model="openai/gpt-5.6"),
