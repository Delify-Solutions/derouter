...
from derouter.proxy._types import UserAPIKeyAuth  # Import UserAPIKeyAuth
from derouter.proxy._types import (
    ...
    DeRouter_OrganizationTableWithMembers,
...
from derouter.proxy.management_helpers.access_group_team_sync import (
    TEAM_ADVISORY_LOCK_SQL,
)
from derouter.proxy.management_helpers.team_member_permission_checks import (
    TeamMemberPermissionChecks,
)
from derouter.proxy.proxy_server import app
from derouter.router import Router
from derouter.types.proxy.management_endpoints.team_endpoints import (
...
def _wire_team_create_tx(prisma_client):
    ...
    async def _tx():
        yield SimpleNamespace(
            derouter_teamtable=prisma_client.db.derouter_teamtable,
...
def _wire_member_delete_tx(prisma_client):
    ...
    async def _query_raw(sql, team_id):
        if sql != TEAM_ADVISORY_LOCK_SQL:
            team_row = await prisma_client.db.derouter_teamtable.find_unique(where={"team_id": team_id})
...
def _wire_team_delete_tx(prisma_client):
    ...
    tx = SimpleNamespace(
        derouter_teamtable=prisma_client.db.derouter_teamtable,
        derouter_teammembership=prisma_client.db.derouter_teammembership,
        query_raw=AsyncMock(return_value=[]),
...
mock_prisma_client.db.derouter_teamtable = MagicMock()
mock_prisma_client.db.derouter_teamtable.update = AsyncMock()
mock_prisma_client.db.derouter_auditlog = MagicMock()
mock_prisma_client.db.derouter_auditlog.create = AsyncMock()
...
def mock_db_client():
    with patch(
        "derouter.proxy.proxy_server.prisma_client", mock_prisma_client
...
async def test_validate_team_org_change_same_org_id():
    ...
    # Mock organization
    organization = MagicMock(spec=DeRouter_OrganizationTableWithMembers)
    organization.organization_id = org_id
    organization.models = []
    organization.derouter_budget_table = MagicMock()
    organization.derouter_budget_table.max_budget = (
        50.0  # This would normally fail validation
    )
    organization.derouter_budget_table.tpm_limit = (
        500  # This would normally fail validation
    )
    organization.derouter_budget_table.rpm_limit = (
    ...
    with patch(
        "derouter.proxy.management_endpoints.team_endpoints.can_org_access_model"
...
async def test_validate_team_org_change_members_in_org():
    ...
    # Mock organization with members (using DeRouter_OrganizationMembershipTable structure)
    organization = MagicMock(spec=DeRouter_OrganizationTableWithMembers)
    organization.organization_id = new_org_id
    organization.models = []
    organization.derouter_budget_table = None

    # Create mock organization members - these should match team members
    org_member_1 = MagicMock(spec=DeRouter_OrganizationMembershipTable)
    org_member_1.user_id = user_id_1
    org_member_2 = MagicMock(spec=DeRouter_OrganizationMembershipTable)
...
async def test_get_team_permissions_list_success(mock_db_client, mock_admin_auth):
    ...
    with patch(
        "derouter.proxy.management_endpoints.team_endpoints.get_team_object",
...
async def test_update_team_permissions_success(mock_db_client, mock_admin_auth):
    ...
    mock_existing_team_row = MagicMock(spec=DeRouter_TeamTable)
    ...
    mock_updated_team_row = MagicMock(spec=DeRouter_TeamTable)
    ...
    with patch(
        ...
        mock_db_client.db.derouter_teamtable.update = AsyncMock(
        ...
        mock_db_client.db.derouter_teamtable.update.assert_awaited_once_with(
...
async def test_new_team_rejects_a_duration_that_never_advances(
    ...
    from derouter.proxy._types import NewTeamRequest
    from derouter.proxy.management_endpoints.team_endpoints import new_team

    mock_db_client.db = MagicMock()
    mock_team_create = AsyncMock()
    mock_db_client.db.derouter_teamtable = MagicMock()
    mock_db_client.db.derouter_teamtable.create = mock_team_create
...
async def test_update_team_rejects_a_duration_that_never_advances(
    ...
    from derouter.proxy._types import UpdateTeamRequest
    from derouter.proxy.management_endpoints.team_endpoints import update_team

    mock_db_client.db = MagicMock()
    mock_find_unique = AsyncMock(return_value=None)
    mock_db_client.db.derouter_teamtable = MagicMock()
    mock_db_client.db.derouter_teamtable.find_unique = mock_find_unique
...
async def test_new_team_with_object_permission(mock_db_client, mock_admin_auth):
    ...
    1. Creating a record in derouter_objectpermissiontable
    ...
    mock_db_client.db.derouter_objectpermissiontable = MagicMock()
    mock_db_client.db.derouter_objectpermissiontable.create = mock_object_perm_create

    # Mock model table creation
    mock_db_client.db.derouter_modeltable = MagicMock()
    mock_db_client.db.derouter_modeltable.create = AsyncMock(
    ...
    mock_db_client.db.derouter_teamtable.create = mock_team_create
    _wire_team_create_tx(mock_db_client)
    mock_db_client.db.derouter_teamtable.count = mock_team_count
    mock_db_client.db.derouter_teamtable.update = AsyncMock(
        return_value=team_create_result
    )

    # Mock user table
    mock_db_client.db.derouter_usertable = MagicMock()
    mock_db_client.db.derouter_usertable.update = AsyncMock(return_value=MagicMock())

    from fastapi import Request

    from derouter.proxy._types import DeRouter_ObjectPermissionBase, NewTeamRequest
    from derouter.proxy.management_endpoints.team_endpoints import new_team

    # Build request with object_permission
    team_request = NewTeamRequest(
        team_alias="my-team",
        object_permission=DeRouter_ObjectPermissionBase(vector_stores=["my-vector"]),
    )
...
async def test_new_team_with_mcp_tool_permissions(mock_db_client, mock_admin_auth):
    ...
    2. The field is properly stored in the DeRouter_ObjectPermissionTable
    ...
    mock_db_client.db.derouter_objectpermissiontable = MagicMock()
    mock_db_client.db.derouter_objectpermissiontable.create = mock_obj_perm_create
    mock_db_client.db.derouter_mcpservertable.find_many = AsyncMock(return_value=[])
    ...
    mock_db_client.db.derouter_teamtable = MagicMock()
    mock_db_client.db.derouter_teamtable.create = AsyncMock(
        return_value=team_create_result
    )
    _wire_team_create_tx(mock_db_client)
    mock_db_client.db.derouter_teamtable.count = AsyncMock(return_value=0)
    mock_db_client.db.derouter_teamtable.update = AsyncMock(
        return_value=team_create_result
    )
    ...
    team_request = NewTeamRequest(
        team_alias="mcp-team",
        object_permission=DeRouter_ObjectPermissionBase(
...
async def test_new_team_disable_auto_add_proxy_admin_flag(
    ...
    with patch(
        ...
        "derouter.proxy.management_endpoints.team_endpoints._add_team_members_to_team",
        new_callable=AsyncMock,
    ) as mock_add_members:
        await new_team(
...
async def test_team_update_object_permissions_existing_permission(monkeypatch):
    ...
    object_permission_id, the existing DeRouter_ObjectPermissionTable record is updated
    ...
    from derouter.proxy._types import DeRouter_ObjectPermissionBase, DeRouter_TeamTable
    from derouter.proxy.management_endpoints.team_endpoints import (
    ...
    existing_team_row = DeRouter_TeamTable(
        team_id="test_team_id",
    ...
    mock_prisma_client.db.derouter_objectpermissiontable.find_unique = AsyncMock(
    ...
    mock_prisma_client.db.derouter_objectpermissiontable.upsert = AsyncMock(
    ...
    mock_prisma_client.db.derouter_objectpermissiontable.find_unique.assert_called_once_with(
        where={"object_permission_id": "existing_perm_id_123"}
    )
    mock_prisma_client.db.derouter_objectpermissiontable.upsert.assert_called_once()
...
async def test_team_update_object_permissions_no_existing_permission(monkeypatch):
    ...
    DeRouter_ObjectPermissionTable and the team is updated with the new object_permission_id.
    """
    from unittest.mock import AsyncMock, MagicMock
    ...
    existing_team_row_no_perm = DeRouter_TeamTable(
...
async def test_team_update_object_permissions_missing_permission_record(monkeypatch):
    ...
    a new entry is created in the DeRouter_ObjectPermissionTable with the new permissions.
    """
    from unittest.mock import AsyncMock, MagicMock
    ...
    existing_team_row_missing_perm = DeRouter_TeamTable(
...
def test_team_member_add_duplication_check_raises_proxy_exception():
    ...
    existing_team_row = MagicMock(spec=DeRouter_TeamTable)
...
async def test_add_team_member_budget_table_success():
    ...
    """
    from derouter.proxy._types import TeamInfoResponseObjectTeamTable
    from derouter.proxy.management_endpoints.team_endpoints import (
    ...
    mock_prisma_client.db.derouter_budgettable.find_unique.assert_called_once_with(
...
async def test_add_team_member_budget_table_exception_handling():
    ...
    with patch(
        "derouter.proxy.management_endpoints.team_endpoints.verbose_proxy_logger"
    ) as mock_logger:
        # Call the function
        result = await _add_team_member_budget_table(
...
async def test_add_team_member_budget_table_budget_not_found():
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_budgettable.find_unique = AsyncMock(return_value=None)
...
def test_add_new_models_to_team():
    ...
    """
    from derouter.proxy._types import SpecialModelNames
    from derouter.proxy.management_endpoints.team_endpoints import add_new_models_to_team

    team_obj = MagicMock(spec=DeRouter_TeamTable)
    team_obj.models = []
...
def _make_team_member_add_request(
    ...
    from derouter.proxy._types import Member, TeamMemberAddRequest
...
async def test_validate_team_member_add_permissions_non_admin():
    ...
    with (
        patch(
            "derouter.proxy.management_endpoints.team_endpoints._is_user_team_admin",
            return_value=False,
        ),
        patch(
            "derouter.proxy.management_endpoints.team_endpoints._is_available_team",
            return_value=False,
        ),
    ):
        with pytest.raises(HTTPException) as exc_info:
...
async def test_update_team_member_permissions_blocks_non_admin_via_available_team(
    ...
    existing_row = MagicMock(spec=DeRouter_TeamTable)
...
async def test_process_team_members_single_member():
    ...
    """
    from derouter.proxy._types import DeRouter_TeamMembership, DeRouter_UserTable
    from derouter.proxy.management_endpoints.team_endpoints import _process_team_members

    # Mock dependencies
    mock_prisma_client = MagicMock()
    mock_team = MagicMock(spec=DeRouter_TeamTable)
    ...
    mock_user = MagicMock(spec=DeRouter_UserTable)
    mock_user.user_id = "new-user-123"
    mock_membership = MagicMock(spec=DeRouter_TeamMembership)
    ...
    with patch(
        "derouter.proxy.management_endpoints.team_endpoints.add_new_member",
        new_callable=AsyncMock,
        ...
        users, memberships = await _process_team_members(
            ...
            derouter_proxy_admin_name="admin",
        )
...
async def test_process_team_members_multiple_members():
    ...
    mock_users = [MagicMock(spec=DeRouter_UserTable) for _ in range(2)]
    mock_memberships = [MagicMock(spec=DeRouter_TeamMembership) for _ in range(2)]
...
async def test_update_team_members_list_single_member():
    ...
    """
    from derouter.proxy._types import DeRouter_UserTable
    from derouter.proxy.management_endpoints.team_endpoints import (
...
async def test_add_team_members_reconciles_against_freshly_locked_row():
    ...
    stale_snapshot = DeRouter_TeamTable(
    ...
    tx.derouter_teamtable.update = AsyncMock(side_effect=_capture_update)
    ...
    with patch(
        "derouter.proxy.management_endpoints.team_endpoints._process_team_members",
...
async def test_add_team_members_runs_member_writes_on_the_lock_holding_transaction():
    ...
    tx.derouter_teamtable.update = AsyncMock(
        return_value=DeRouter_TeamTable(team_id="team-pool", members_with_roles=[])
    )
    tx.derouter_usertable.upsert = AsyncMock(return_value=added_user)
    tx.derouter_usertable.update_many = AsyncMock()
    tx.derouter_budgettable.create = AsyncMock(return_value=created_budget)
    tx.derouter_teammembership.create = AsyncMock(return_value=membership)
    ...
    _, updated_users, updated_team_memberships = await _add_team_members_to_team(
        ...
        complete_team_data=DeRouter_TeamTable(team_id="team-pool", members_with_roles=[]),
        prisma_client=cast(object, prisma_client),
        user_api_key_dict=UserAPIKeyAuth(user_role=LitellmUserRoles.PROXY_ADMIN),
        derouter_proxy_admin_name="admin",
    )
...
async def test_add_team_members_writes_nothing_when_the_team_is_deleted_mid_request():
    ...
    prisma_client.db.derouter_teammembership.delete_many = AsyncMock()
    ...
    with patch(
        ...
        with pytest.raises(HTTPException) as exc_info:
            await _add_team_members_to_team(
                ...
                complete_team_data=DeRouter_TeamTable(team_id="team-deleted-mid-add", members_with_roles=[]),
                prisma_client=cast(object, prisma_client),
                user_api_key_dict=UserAPIKeyAuth(user_role=LitellmUserRoles.PROXY_ADMIN),
                derouter_proxy_admin_name="admin",
            )
    ...
    tx.derouter_teamtable.update.assert_not_awaited()
    prisma_client.db.execute_raw.assert_not_awaited()
    prisma_client.db.derouter_teammembership.delete_many.assert_not_awaited()
...
async def test_team_model_add_delete_refresh_team_cache(endpoint_name):
    ...
    DB. Without a cache refresh, the in-memory `DeRouter_TeamTableCachedObj`
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma_client,
        patch("derouter.proxy.proxy_server.user_api_key_cache") as mock_cache,
        patch("derouter.proxy.proxy_server.proxy_logging_obj") as mock_logging,
        patch(
            "derouter.proxy.management_endpoints.team_endpoints._cache_team_object",
            new_callable=AsyncMock,
        ) as mock_cache_team,
    ):
        mock_prisma_client.db.derouter_teamtable.find_unique = AsyncMock(
        ...
        update_call_kwargs = (
            mock_prisma_client.db.derouter_teamtable.update.call_args.kwargs
        )
...
async def test_team_write_404s_when_row_vanishes_before_update(endpoint_name):
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma_client,  # test-quality-ok: proxy_server module global is the endpoint's only injection point
        patch("derouter.proxy.proxy_server.user_api_key_cache"),  # test-quality-ok: proxy_server module global is the endpoint's only injection point
        patch("derouter.proxy.proxy_server.proxy_logging_obj"),  # test-quality-ok: proxy_server module global is the endpoint's only injection point
        ...
        mock_prisma_client.db.derouter_teamtable.update = AsyncMock(return_value=None)
        mock_prisma_client.db.execute_raw = AsyncMock(return_value=None)
...
async def test_update_team_team_member_budget_not_passed_to_db(
    ...
    """
    Test that 'team_member_budget' is never passed to prisma_client.db.derouter_teamtable.update
    ...
    from derouter.proxy._types import LitellmUserRoles, UpdateTeamRequest, UserAPIKeyAuth
    from derouter.proxy.management_endpoints.team_endpoints import update_team
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma_client,
        patch("derouter.proxy.proxy_server.llm_router") as mock_llm_router,
        patch("derouter.proxy.proxy_server.user_api_key_cache") as mock_cache,
        patch("derouter.proxy.proxy_server.proxy_logging_obj") as mock_logging,
        patch("derouter.proxy.proxy_server.derouter_proxy_admin_name", "admin"),
        patch(
            "derouter.proxy.management_endpoints.team_endpoints._cache_team_object"
        ) as mock_cache_team,
        patch(
            "derouter.proxy.management_endpoints.team_endpoints.TeamMemberBudgetHandler.upsert_team_member_budget_table"
        ...
        assert mock_prisma_client.db.derouter_teamtable.update.called

        # Get the call arguments
        call_args = mock_prisma_client.db.derouter_teamtable.update.call_args
        ...
        mock_prisma_client.db.derouter_teamtable.update.reset_mock()
...
async def test_create_team_member_budget_table():
    ...
    from derouter.proxy._types import LitellmUserRoles, NewTeamRequest, UserAPIKeyAuth
    from derouter.proxy.management_endpoints.team_endpoints import (
        TeamMemberBudgetHandler,
    )
    ...
    with patch(
        "derouter.proxy.management_endpoints.budget_management_endpoints.new_budget",
        new_callable=AsyncMock,
    ) as mock_new_budget:
...
async def test_upsert_team_member_budget_table_existing_budget():
    ...
    from derouter.proxy._types import LitellmUserRoles, DeRouter_TeamTable, UserAPIKeyAuth
    from derouter.proxy.management_endpoints.team_endpoints import (
        TeamMemberBudgetHandler,
    )
    ...
    team_table = MagicMock(spec=DeRouter_TeamTable)
    ...
    with patch(
        "derouter.proxy.management_endpoints.budget_management_endpoints.update_budget",
        new_callable=AsyncMock,
...
async def test_upsert_team_member_budget_table_clears_duration_kept_budget(mock_db_client):
    ...
    mock_db_client.db.derouter_budgettable.update = AsyncMock(
    ...
    written = mock_db_client.db.derouter_budgettable.update.call_args.kwargs["data"]
...
async def test_create_team_member_budget_table_explicit_null_duration_does_not_inherit_team_duration(
    ...
    mock_db_client.db.derouter_budgettable.create = AsyncMock(
    ...
    written = mock_db_client.db.derouter_budgettable.create.call_args.kwargs["data"]
    assert written["max_budget"] == 100.0
...
async def test_update_team_with_team_member_budget_duration(
    ...
    with (
        ...
        assert mock_prisma_client.db.derouter_teamtable.update.called
        update_call_args = mock_prisma_client.db.derouter_teamtable.update.call_args
...
async def test_backfill_team_member_budget_entries_creates_missing_memberships():
    ...
    Regression test for: https://github.com/Delify-Solutions/derouter/issues/25506
    """
    from unittest.mock import AsyncMock, MagicMock

    from derouter.proxy._types import Member
    from derouter.proxy.management_endpoints.team_endpoints import (
        TeamMemberBudgetHandler,
    )
    ...
    mock_prisma.db.derouter_teammembership.find_many = AsyncMock(
        return_value=[existing_membership]
    )
    mock_prisma.db.derouter_teammembership.create_many = AsyncMock(return_value=None)
    mock_prisma.db.derouter_teammembership.update_many = AsyncMock(return_value=0)
    ...
    mock_prisma.db.derouter_teammembership.find_many.assert_awaited_once_with(
        where={"team_id": team_id}
    )

    # create_many should only create an entry for user-B (user-A already has one)
    mock_prisma.db.derouter_teammembership.create_many.assert_awaited_once_with(
    ...
    mock_prisma.db.derouter_teammembership.find_many.reset_mock()
    mock_prisma.db.derouter_teammembership.create_many.reset_mock()
    mock_prisma.db.derouter_teammembership.update_many.reset_mock()
...
async def test_backfill_team_member_budget_entries_no_op_when_all_exist():
    ...
    mock_prisma.db.derouter_teammembership.create_many.assert_not_awaited()
...
async def test_backfill_team_member_budget_entries_populates_null_budget_id_on_existing_rows():
    ...
    mock_prisma.db.derouter_teammembership.create_many = AsyncMock(return_value=None)
    mock_prisma.db.derouter_teammembership.update_many = AsyncMock(return_value=2)
    ...
    mock_prisma.db.derouter_teammembership.create_many.assert_not_awaited()
    mock_prisma.db.derouter_teammembership.update_many.assert_awaited_once_with(
...
async def test_backfill_team_member_budget_entries_empty_members():
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_teammembership.find_many = AsyncMock(return_value=[])
    mock_prisma.db.derouter_teammembership.create_many = AsyncMock(return_value=None)
    ...
    mock_prisma.db.derouter_teammembership.find_many.assert_not_awaited()
    mock_prisma.db.derouter_teammembership.create_many.assert_not_awaited()
...
async def test_bulk_team_member_add_success():
    ...
    from derouter.proxy.management_endpoints.team_endpoints import bulk_team_member_add
    ...
    mock_user_1 = MagicMock(spec=DeRouter_UserTable)
    ...
    mock_user_2 = MagicMock(spec=DeRouter_UserTable)
    ...
    mock_membership_1 = MagicMock(spec=DeRouter_TeamMembership)
    ...
    mock_membership_2 = MagicMock(spec=DeRouter_TeamMembership)
    ...
    with patch(
        "derouter.proxy.management_endpoints.team_endpoints.team_member_add",
        new_callable=AsyncMock,
...
async def test_bulk_team_member_add_all_users_flag():
    ...
    """
    from derouter.proxy._types import DeRouter_UserTable, TeamAddMemberResponse
    from derouter.proxy.management_endpoints.team_endpoints import bulk_team_member_add
    ...
    with (
        ...
        mock_prisma.db.derouter_usertable.find_many = AsyncMock(
        ...
        mock_prisma.db.derouter_usertable.find_many.assert_called_once_with(
...
async def test_bulk_team_member_add_no_db_connection():
    ...
    with patch("derouter.proxy.proxy_server.prisma_client", None):
        mock_auth = UserAPIKeyAuth(user_role=LitellmUserRoles.PROXY_ADMIN)
...
async def test_list_team_v2_security_check_non_admin_user():
    ...
    from derouter.proxy.management_endpoints.team_endpoints import list_team_v2
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma_client,
        patch("derouter.proxy.proxy_server.user_api_key_cache"),
        patch("derouter.proxy.proxy_server.proxy_logging_obj"),
        patch(
            "derouter.proxy.management_endpoints.team_endpoints.get_user_object",
            new_callable=AsyncMock,
            return_value=None,
        ),
    ):
...
async def test_list_team_v2_security_check_non_admin_user_own_teams():
    ...
    with (
        ...
        mock_db.derouter_teamtable.find_many = AsyncMock(return_value=mock_teams)
        mock_db.derouter_teamtable.count = AsyncMock(return_value=2)
        mock_db.derouter_verificationtoken.group_by = AsyncMock(return_value=[])
...
async def test_list_team_v2_security_check_admin_user():
    ...
    with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma_client:
        # Mock prisma client and database operations
        mock_db = Mock()
        mock_prisma_client.db = mock_db
...
async def test_list_team_v2_with_status_deleted():
    ...
    with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma_client:
        ...
        mock_db.derouter_deletedteamtable.find_many = AsyncMock(
            return_value=[mock_deleted_team1, mock_deleted_team2]
        )
        mock_db.derouter_deletedteamtable.count = AsyncMock(return_value=2)

        # Mock regular teams table (should NOT be called)
        mock_db.derouter_teamtable.find_many = AsyncMock(return_value=[])
        mock_db.derouter_teamtable.count = AsyncMock(return_value=0)
        ...
        mock_db.derouter_deletedteamtable.find_many.assert_called_once()
        mock_db.derouter_deletedteamtable.count.assert_called_once()

        # Verify that regular table was NOT queried
        mock_db.derouter_teamtable.find_many.assert_not_called()
        mock_db.derouter_teamtable.count.assert_not_called()
...
@pytest.mark.asyncio
async def test_list_team_v2_includes_derouter_model_table():
    """
    Regression test for GH #26312: GET /v2/team/list must eagerly load the
    derouter_model_table relation for active teams, same as /team/info and
    /team/list, or a team's model_aliases always read back as null from this
    endpoint. Deleted teams are excluded: DeRouter_DeletedTeamTable has no such
    relation in the Prisma schema, so requesting it there raises
    UnknownRelationalFieldError against a real database.

    The fake find_many below only attaches derouter_model_table when its own
    ...
    def _team_row(team_id: str, include) -> Mock:
        model_table = (
            ...
            if (include or {}).get("derouter_model_table")
            else None
        )
        return Mock(
            team_id=team_id,
            model_dump=lambda: {
                "team_id": team_id,
                "team_alias": "t",
                "derouter_model_table": model_table,
            },
        )

    with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma_client:  # test-quality-ok: this file's DB-mock convention
        mock_db = Mock()
        mock_prisma_client.db = mock_db

        mock_db.derouter_teamtable.find_many = AsyncMock(
            side_effect=lambda **kw: [_team_row("team_1", kw.get("include"))]
        )
        mock_db.derouter_teamtable.count = AsyncMock(return_value=1)
        mock_db.derouter_verificationtoken.group_by = AsyncMock(return_value=[])
        ...
        assert result["teams"][0].derouter_model_table is not None
        assert result["teams"][0].derouter_model_table.model_aliases == {"my-fast-model": "fake-model"}

        mock_db.derouter_deletedteamtable.find_many = AsyncMock(
            side_effect=lambda **kw: [_team_row("team_2", kw.get("include"))]
        )
        mock_db.derouter_deletedteamtable.count = AsyncMock(return_value=1)
        ...
        assert "include" not in mock_db.derouter_deletedteamtable.find_many.call_args.kwargs
...
async def test_list_team_v2_org_admin_sees_org_teams():
    ...
    with (
        ...
        mock_db.derouter_teamtable.find_many = AsyncMock(return_value=[mock_team])
        mock_db.derouter_teamtable.count = AsyncMock(return_value=1)
        mock_db.derouter_verificationtoken.group_by = AsyncMock(return_value=[])
        ...
        where = mock_db.derouter_teamtable.find_many.call_args.kwargs["where"]
...
async def test_list_team_v2_org_admin_own_user_id_sees_all_org_teams():
    ...
    Regression test for https://github.com/Delify-Solutions/derouter/issues/30215
    """
    from datetime import datetime
    from unittest.mock import AsyncMock, Mock, patch
...
async def test_list_team_v2_org_admin_with_user_id_returns_user_teams():
    ...
    mock_org_admin = DeRouter_UserTable(
        user_id="org_admin_user",
        teams=["team_1"],
        organization_memberships=[
            DeRouter_OrganizationMembershipTable(
    ...
    mock_target_user = DeRouter_UserTable(
        user_id="target_user",
        teams=["team_X", "team_Y"],
    )
...
async def test_list_team_v2_with_invalid_status():
    ...
    with patch("derouter.proxy.proxy_server.prisma_client", mock_prisma_client):
...
async def test_list_team_v2_search_builds_or_clause():
    ...
    with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma_client:
        ...
        find_many_kwargs = mock_db.derouter_teamtable.find_many.call_args.kwargs
...
async def test_list_team_v2_search_composes_with_user_id_filter():
    ...
    with (
        ...
        patch(
            "derouter.proxy.management_endpoints.team_endpoints._get_org_admin_org_ids",
...
async def test_list_team_v2_populates_keys_count():
    ...
    batched group_by against DeRouter_VerificationToken.
    """
    from unittest.mock import AsyncMock, Mock, patch
    ...
    with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma_client:
        ...
        mock_db.derouter_teamtable.find_many = AsyncMock(return_value=[team_a, team_b])
        mock_db.derouter_teamtable.count = AsyncMock(return_value=2)
        mock_db.derouter_verificationtoken.group_by = AsyncMock(
            return_value=[
        ...
        group_by_kwargs = mock_db.derouter_verificationtoken.group_by.call_args.kwargs
...
async def test_list_team_v2_keys_count_skipped_for_empty_page():
    ...
    with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma_client:
        ...
        assert result["teams"] == []
        mock_db.derouter_verificationtoken.group_by.assert_not_called()


@pytest.mark.asyncio
async def test_list_team_v2_keys_count_skipped_for_deleted_status():
    """
    The deleted-table branch returns DeRouter_DeletedTeamTable items, which do
...
async def test_team_member_delete_cleans_membership(mock_db_client, mock_admin_auth):
    """
    Verify that /team/member_delete removes the corresponding DeRouter_TeamMembership row
    so the same user can be re-added without unique constraint issues.
    """
    from derouter.proxy._types import TeamMemberDeleteRequest
    from derouter.proxy.management_endpoints.team_endpoints import team_member_delete
    ...
    mock_db_client.db.derouter_teamtable.find_unique = AsyncMock(
        return_value=mock_team_row
    )
    mock_db_client.db.derouter_teamtable.update = AsyncMock(return_value=mock_team_row)

    # User row to allow removal from user's teams list
    mock_user_row = MagicMock()
    mock_user_row.user_id = test_user_id
    mock_user_row.teams = [test_team_id]
    mock_db_client.db.derouter_usertable.find_many = AsyncMock(
        return_value=[mock_user_row]
    )
    mock_db_client.db.derouter_usertable.update = AsyncMock(return_value=MagicMock())

    # Membership deletion should be called
    mock_db_client.db.derouter_teammembership = MagicMock()
    mock_db_client.db.derouter_teammembership.delete_many = AsyncMock(
        return_value=MagicMock()
    )

    # Verification token deletion should be called
    mock_db_client.db.derouter_verificationtoken = MagicMock()
    mock_db_client.db.derouter_verificationtoken.find_many = AsyncMock(return_value=[])
    mock_db_client.db.derouter_verificationtoken.delete_many = AsyncMock(
        return_value=MagicMock()
    )
    ...
    mock_db_client.db.derouter_teammembership.delete_many.assert_awaited_with(
...
async def test_team_member_delete_cleans_verification_tokens(
    ...
    mock_db_client.db.derouter_verificationtoken.delete_many.assert_awaited_once_with(
        where={
...
async def test_team_member_delete_reads_on_the_lock_holding_transaction(
    ...
    mock_db_client.db.derouter_usertable.find_many = pooled_user_read
    mock_db_client.db.derouter_verificationtoken.find_many = pooled_token_read

    tx = MagicMock()
    tx.query_raw = AsyncMock(return_value=[{"members_with_roles": [roster_entry]}])
    tx.derouter_teamtable.update = AsyncMock(return_value=mock_team_row)
    tx.derouter_usertable.find_many = AsyncMock(return_value=[user_row])
    tx.derouter_usertable.update = AsyncMock()
    tx.derouter_teammembership.delete_many = AsyncMock()
    tx.derouter_verificationtoken.find_many = AsyncMock(return_value=[])
    tx.derouter_verificationtoken.delete_many = AsyncMock()
    ...
    tx.derouter_usertable.find_many.assert_awaited_once_with(
        where={"user_id": {"in": [test_user_id]}}
    )
    tx.derouter_verificationtoken.find_many.assert_awaited_once_with(
    ...
    tx.derouter_usertable.update.assert_awaited_once_with(
        where={"user_id": test_user_id}, data={"teams": {"set": []}}
    )
    tx.derouter_teammembership.delete_many.assert_awaited_once_with(
        where={"team_id": test_team_id, "user_id": test_user_id}
    )
...
async def test_team_member_delete_by_email_the_user_row_does_not_carry(
    ...
    if user_row_exists:
        mock_db_client.db.derouter_usertable.update.assert_awaited_once_with(
            where={"user_id": test_user_id},
            data={"teams": {"set": []}},
        )
    else:
        mock_db_client.db.derouter_usertable.update.assert_not_awaited()

    mock_db_client.db.derouter_teammembership.delete_many.assert_awaited_once_with(
        where={"team_id": test_team_id, "user_id": test_user_id}
    )
...
async def test_team_member_delete_still_rejects_a_user_the_team_has_no_trace_of(
    ...
    mock_db_client.db.derouter_usertable.update.assert_not_awaited()
    mock_db_client.db.derouter_teammembership.delete_many.assert_not_awaited()
...
async def test_team_member_delete_is_atomic_across_its_four_writes(
    ...
    mock_db_client.db.derouter_usertable.update = AsyncMock(
    ...
    mock_db_client.db.derouter_teamtable.update.assert_awaited_once()
    ...
    mock_db_client.db.derouter_teammembership.delete_many.assert_not_awaited()
    mock_db_client.db.derouter_verificationtoken.delete_many.assert_not_awaited()
...
async def test_new_team_max_budget_exceeds_user_max_budget():
    ...
    from derouter.proxy._types import NewTeamRequest, ProxyException, UserAPIKeyAuth
    from derouter.proxy.management_endpoints.team_endpoints import new_team
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma,
        patch("derouter.proxy.proxy_server._license_check") as mock_license,
        patch("derouter.proxy.proxy_server.user_api_key_cache") as mock_cache,
        patch("derouter.proxy.proxy_server.derouter_proxy_admin_name", "admin"),
        patch(
            "derouter.proxy.proxy_server.create_audit_log_for_update", new=AsyncMock()
        ) as mock_audit,
    ):
        # Setup basic mocks
        mock_prisma.db.derouter_teamtable.count = AsyncMock(return_value=0)
        ...
        mock_user_obj = DeRouter_UserTable(
            user_id="non-admin-user-123",
            max_budget=100.0,
        )
...
async def test_new_team_max_budget_within_user_limit():
    ...
    from derouter.proxy._types import NewTeamRequest, UserAPIKeyAuth
    from derouter.proxy.management_endpoints.team_endpoints import new_team
    ...
    with (
        ...
        mock_prisma.db.derouter_teamtable.create = AsyncMock(
            return_value=mock_created_team
        )
        _wire_team_create_tx(mock_prisma)
        mock_prisma.db.derouter_teamtable.update = AsyncMock(
            return_value=mock_created_team
        )

        # Mock model table
        mock_prisma.db.derouter_modeltable = MagicMock()
        mock_prisma.db.derouter_modeltable.create = AsyncMock(
            return_value=MagicMock(id="model123")
        )
        ...
        mock_prisma.db.derouter_usertable = MagicMock()
        mock_prisma.db.derouter_usertable.upsert = AsyncMock(return_value=mock_user)
        mock_prisma.db.derouter_usertable.update_many = AsyncMock()
        mock_prisma.db.derouter_usertable.find_unique = AsyncMock(return_value=mock_user)
        mock_prisma.db.derouter_usertable.update = AsyncMock(return_value=mock_user)
        ...
        mock_prisma.db.derouter_teammembership = MagicMock()
        mock_prisma.db.derouter_teammembership.create = AsyncMock(
...
async def test_new_team_org_scoped_budget_bypasses_user_limit():
    ...
    with (
        ...
        patch(
            "derouter.proxy.management_endpoints.team_endpoints.get_org_object"
        ) as mock_get_org,
    ):
        # Setup mocks
        mock_prisma.db.derouter_teamtable.count = AsyncMock(return_value=0)
        ...
        mock_org = MagicMock(spec=DeRouter_OrganizationTable)
        mock_org.organization_id = "test-org-123"
        mock_org.max_budget = 100.0
        mock_org.models = ["gpt-4", "gpt-3.5-turbo", "claude-3-opus"]
        mock_org.derouter_budget_table = None  # No budget table for this test
...
async def test_new_team_org_scoped_models_bypasses_user_limit():
    ...
    with (
        ...
        mock_org.max_budget = 100.0
        mock_org.models = ["gpt-4", "gpt-3.5-turbo", "claude-3-opus"]
        mock_org.derouter_budget_table = None
        mock_get_org.return_value = mock_org
...
async def test_new_team_standalone_validates_against_user_models(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "default_team_settings", None)
    monkeypatch.setattr(derouter, "default_team_params", None)
...
async def test_update_team_standalone_budget_raise_blocked_for_team_admin():
    ...
    with (
        ...
        mock_prisma.db.derouter_teamtable.find_unique = AsyncMock(
            return_value=mock_existing_team
        )
...
async def test_update_team_standalone_budget_raise_allowed_for_proxy_admin(
    ...
    from derouter.proxy._types import UpdateTeamRequest, UserAPIKeyAuth
    from derouter.proxy.management_endpoints.team_endpoints import update_team
    ...
    with (
        ...
        mock_updated_team.derouter_model_table = None
...
async def test_update_team_standalone_unchanged_budget_allowed(
    ...
    This is the DeRouter UI scenario: the UI sends the full team object on every
...
async def test_new_team_org_scoped_tpm_rpm_bypasses_user_limit():
    ...
    with (
        ...
        # Mock team creation
        mock_created_team = MagicMock(spec=DeRouter_TeamTable)
...
async def test_update_team_org_scoped_tpm_rpm_bypasses_user_limit(
    ...
    with (
        ...
        # Mock team update
        mock_updated_team = MagicMock(spec=DeRouter_TeamTable)
...
async def test_update_team_guardrails_with_org_id(
    ...
    with (
        ...
        mock_prisma.db.derouter_organizationtable.find_unique = AsyncMock(
        ...
        mock_prisma.db.derouter_organizationmembership.find_many = AsyncMock(
        ...
        with patch("derouter.proxy.proxy_server.llm_router", mock_router):
            ...
            assert mock_prisma.db.derouter_organizationtable.find_unique.call_count >= 1

            # Get the first call (from fetch_and_validate_organization)
            first_call_kwargs = (
                mock_prisma.db.derouter_organizationtable.find_unique.call_args_list[
                    0
                ].kwargs
            )
...
def test_transform_teams_to_deleted_records():
    ...
    records = _transform_teams_to_deleted_records(
        ...
        derouter_changed_by="admin-user",
    )
    ...
    assert all("derouter_changed_by" in record for record in records)
...
async def test_save_deleted_team_records():
    ...
    mock_prisma_client.db.derouter_deletedteamtable.create_many = mock_create_many
...
async def test_persist_deleted_team_records():
    ...
    assert records[0]["derouter_changed_by"] == "admin-user"
...
async def test_delete_team_persists_deleted_teams(
    ...
    from derouter.proxy._types import DeleteTeamRequest
    ...
    mock_prisma_client.db.derouter_teamtable.find_unique = mock_find_unique
    ...
    mock_prisma_client.db.derouter_deletedteamtable.create_many = mock_create_many_teams

    mock_create_many_keys = AsyncMock()
    mock_prisma_client.db.derouter_deletedverificationtoken.create_many = (
        mock_create_many_keys
    )

    mock_find_many_keys = AsyncMock(return_value=[])
    mock_prisma_client.db.derouter_verificationtoken.find_many = mock_find_many_keys

    # delete_team now deletes team BYOK models inside a transaction; this team has none.
    mock_tx = AsyncMock()
    mock_tx.derouter_proxymodeltable.find_many = AsyncMock(return_value=[])
    ...
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.prisma_client",
        mock_prisma_client,
    )
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.create_audit_log_for_update",
        AsyncMock(),
    )
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.derouter_proxy_admin_name",
        "admin",
    )
    monkeypatch.setattr(
        "derouter.proxy.management_endpoints.team_endpoints.team_member_delete",
...
async def test_delete_team_sweeps_references_outside_members_with_roles(
    ...
    doomed_team = DeRouter_TeamTable(
        team_id="team-doomed",
    ...
    mock_prisma_client = AsyncMock()
    mock_prisma_client.db.derouter_teamtable.find_unique = AsyncMock(return_value=doomed_team)
    mock_prisma_client.delete_data = AsyncMock(return_value={"deleted_keys": 0})
    mock_prisma_client.db.derouter_deletedteamtable.create_many = AsyncMock()
    mock_prisma_client.db.derouter_deletedverificationtoken.create_many = AsyncMock()
    mock_prisma_client.db.derouter_verificationtoken.find_many = AsyncMock(return_value=[])

    mock_execute_raw = AsyncMock()
    mock_prisma_client.db.execute_raw = mock_execute_raw
    mock_membership_delete_many = AsyncMock()
    mock_prisma_client.db.derouter_teammembership.delete_many = mock_membership_delete_many
    mock_prisma_client.db.derouter_teamtable.delete_many = AsyncMock(side_effect=record_cache_state_then_delete)
    ...
    for cached_team_id, cached_alias in (
        ...
        cached_obj = DeRouter_TeamTableCachedObj(
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
    monkeypatch.setattr("derouter.proxy.proxy_server.user_api_key_cache", fresh_cache)
    monkeypatch.setattr("derouter.proxy.proxy_server.create_audit_log_for_update", AsyncMock())
    monkeypatch.setattr("derouter.proxy.proxy_server.derouter_proxy_admin_name", "admin")
...
async def test_delete_team_evicts_the_auth_cache_of_the_keys_it_deletes(
    ...
    """
    from derouter.proxy._types import DeleteTeamRequest, DeRouter_VerificationToken
    from derouter.proxy.common_utils.user_api_key_cache import UserApiKeyCache
    ...
    team_key = DeRouter_VerificationToken(token="hashed-doomed-key", team_id="team-doomed")

    mock_prisma_client = AsyncMock()
    mock_prisma_client.db.derouter_teamtable.find_unique = AsyncMock(return_value=team)
    mock_prisma_client.delete_data = AsyncMock(return_value={"deleted_teams": ["team-doomed"]})
    mock_prisma_client.db.derouter_deletedteamtable.create_many = AsyncMock()
    mock_prisma_client.db.derouter_deletedverificationtoken.create_many = AsyncMock()
    mock_prisma_client.db.derouter_verificationtoken.find_many = AsyncMock(return_value=[team_key])
    mock_prisma_client.db.execute_raw = AsyncMock()
    mock_prisma_client.db.derouter_teammembership.delete_many = AsyncMock()
...
async def test_delete_team_failing_locked_sweep_rolls_back_the_delete_and_leaves_the_cache_alone(
    ...
    fresh_cache = UserApiKeyCache()
    cached_obj = DeRouter_TeamTableCachedObj(team_id="team-doomed", team_alias="doomed-team")
...
async def test_delete_team_broadcasts_cache_invalidation_to_other_workers(
    ...
    mock_prisma_client.db.derouter_deletedteamtable.create_many = AsyncMock()
    mock_prisma_client.db.derouter_deletedverificationtoken.create_many = AsyncMock()
    mock_prisma_client.db.derouter_verificationtoken.find_many = AsyncMock(
        return_value=[DeRouter_VerificationToken(token="hashed-doomed-key", team_id="team-doomed")]
    )
    mock_prisma_client.db.execute_raw = AsyncMock()
    mock_prisma_client.db.derouter_teammembership.delete_many = AsyncMock()
    ...
    monkeypatch.setattr("derouter.proxy.auth.auth_checks.publish_auth_cache_invalidation", record_publish)
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
    monkeypatch.setattr("derouter.proxy.proxy_server.user_api_key_cache", UserApiKeyCache())
    monkeypatch.setattr("derouter.proxy.proxy_server.create_audit_log_for_update", AsyncMock())
    monkeypatch.setattr("derouter.proxy.proxy_server.derouter_proxy_admin_name", "admin")
...
async def test_delete_team_survives_a_failing_cache_backend(
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
    monkeypatch.setattr("derouter.proxy.proxy_server.user_api_key_cache", UserApiKeyCache())
    monkeypatch.setattr("derouter.proxy.proxy_server.proxy_logging_obj", exploding_logging_obj)
    monkeypatch.setattr("derouter.proxy.proxy_server.create_audit_log_for_update", AsyncMock())
    monkeypatch.setattr("derouter.proxy.proxy_server.derouter_proxy_admin_name", "admin")
    ...
    mock_prisma_client.db.derouter_teamtable.delete_many.assert_any_await(where={"team_id": {"in": ["team-doomed"]}})
...
async def test_team_member_delete_persists_deleted_keys(monkeypatch):
    ...
    key1 = DeRouter_VerificationToken(
    ...
    key2 = DeRouter_VerificationToken(
    ...
    mock_prisma_client.db.derouter_teamtable.find_unique = mock_find_unique_team
    ...
    mock_prisma_client.db.derouter_usertable.find_many = mock_find_many_user

    mock_update_team = AsyncMock()
    mock_prisma_client.db.derouter_teamtable.update = mock_update_team

    mock_update_user = AsyncMock()
    mock_prisma_client.db.derouter_usertable.update = mock_update_user

    mock_delete_membership = AsyncMock()
    mock_prisma_client.db.derouter_teammembership.delete_many = mock_delete_membership

    mock_find_many_keys = AsyncMock(return_value=[key1, key2])
    mock_prisma_client.db.derouter_verificationtoken.find_many = mock_find_many_keys

    mock_delete_keys = AsyncMock()
    mock_prisma_client.db.derouter_verificationtoken.delete_many = mock_delete_keys
...
async def test_get_team_daily_activity_member_with_permission_sees_all_spend(
    ...
    # Mock user info
    mock_user_info = DeRouter_UserTable(
        user_id=user_id,
        teams=[team_id],
        max_budget=1000.0,
        spend=0.0,
    ...
    # Setup mocks
    mock_db_client.db.derouter_teamtable.find_many = AsyncMock(return_value=[mock_team])
    ...
    with patch(
        ...
        with patch(
            "derouter.proxy.management_endpoints.team_endpoints.get_daily_activity",
            new_callable=AsyncMock,
            ...
            if (
                hasattr(mock_db_client.db.derouter_verificationtoken, "find_many")
                and mock_db_client.db.derouter_verificationtoken.find_many.called
            ):
...
async def test_get_team_daily_activity_member_without_permission_filters_by_keys(
    ...
    # Setup mocks
    mock_db_client.db.derouter_teamtable.find_many = AsyncMock(return_value=[mock_team])
    mock_db_client.db.derouter_verificationtoken.find_many = AsyncMock(
    ...
    with patch(
        ...
        with patch(
            ...
            mock_db_client.db.derouter_verificationtoken.find_many.assert_called_once()
...
async def test_update_team_with_router_settings(
    ...
    mock_db_client.db.derouter_teamtable = MagicMock()
    mock_db_client.db.derouter_teamtable.find_unique = mock_team_find_unique
    mock_db_client.db.derouter_teamtable.update = mock_team_update
...
async def test_get_team_daily_activity_non_admin_filters_by_user_api_keys(
    ...
    with patch(
        ...
        with patch(
            ...
            mock_db_client.db.derouter_verificationtoken.find_many.assert_called_once()
            api_key_call_kwargs = (
                mock_db_client.db.derouter_verificationtoken.find_many.call_args[1]
            )
...
async def test_validate_and_populate_member_user_info_only_email_provided():
    ...
    mock_prisma_client.db.derouter_usertable.find_first = AsyncMock(
    ...
    mock_prisma_client.db.derouter_usertable.find_first.assert_called_once_with(
...
async def test_validate_and_populate_member_user_info_only_user_id_not_found():
    ...
    mock_prisma_client.db.derouter_usertable.find_unique = AsyncMock(return_value=None)
    ...
    mock_prisma_client.db.derouter_usertable.find_unique.assert_called_once_with(
...
async def test_list_available_teams_returns_empty_list_when_none_configured():
    ...
    with patch("derouter.proxy.proxy_server.prisma_client", mock_prisma_client):
        ...
        original = derouter.default_internal_user_params
        derouter.default_internal_user_params = None
        ...
        derouter.default_internal_user_params = {"some_other_param": "value"}
        result = await list_available_teams(
            http_request=mock_request,
            user_api_key_dict=mock_user_key,
        )
        ...
        derouter.default_internal_user_params = original
...
async def test_list_team_v1_batches_key_queries():
    ...
    from derouter.proxy.management_endpoints.team_endpoints import list_team
    ...
    # Two teams
    team1 = DeRouter_TeamTable(team_id="team-1", team_alias="Team One")
    team2 = DeRouter_TeamTable(team_id="team-2", team_alias="Team Two")
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma_client,
        patch(
            "derouter.proxy.management_endpoints.team_endpoints._authorize_and_filter_teams",
            new_callable=AsyncMock,
            return_value=[team1, team2],
        ),
        patch(
            "derouter.proxy.management_endpoints.team_endpoints.get_all_team_memberships",
            new_callable=AsyncMock,
            return_value=[],
        ),
    ):
...
async def test_create_team_member_budget_table_with_duration():
    ...
    from derouter.proxy._types import NewTeamRequest, UserAPIKeyAuth, LitellmUserRoles
    from derouter.proxy.management_endpoints.team_endpoints import (
        TeamMemberBudgetHandler,
    )
...
class TestBatchResolveAccessGroupResources:
    ...
    async def test_single_access_group(self):
        ...
        fake_prisma.db.derouter_accessgrouptable.find_many = AsyncMock(
            return_value=[fake_row]
        )

        with patch("derouter.proxy.proxy_server.prisma_client", fake_prisma):
    ...
    async def test_deduplicates_input_ids(self):
        ...
        fake_prisma = MagicMock()
        fake_prisma.db.derouter_accessgrouptable.find_many = fake_find_many
...
async def test_verify_team_access_denies_unauthorized_user():
    ...
    with patch(
        "derouter.proxy.management_endpoints.team_endpoints._is_user_org_admin_for_team",
        new_callable=AsyncMock,
        return_value=False,
    ):
        with pytest.raises(HTTPException) as exc_info:
...
async def test_update_team_rejects_unauthorized_caller():
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma_client,
        patch("derouter.proxy.proxy_server.llm_router"),
        patch("derouter.proxy.proxy_server.user_api_key_cache"),
        patch("derouter.proxy.proxy_server.proxy_logging_obj"),
        patch("derouter.proxy.proxy_server.derouter_proxy_admin_name", "admin"),
        patch(
...
def _build_team_for_me(team_id, members):
    """Real DeRouter_TeamTableCachedObj as get_team_object would return."""
    return DeRouter_TeamTableCachedObj(
        team_id=team_id,
        team_alias="team-vec",
...
def _build_membership_for_me(user_id, team_id, *, spend=12.34, max_budget=100.0):
    """Real DeRouter_TeamMembership as get_team_membership would return."""
    return DeRouter_TeamMembership(
        user_id=user_id,
        team_id=team_id,
        budget_id="b-1",
        spend=spend,
        total_spend=spend,
        derouter_budget_table=DeRouter_BudgetTableFull(
            budget_id="b-1",
            max_budget=max_budget,
            soft_budget=None,
            tpm_limit=500,
...
def _patch_member_me_helpers(*, team, membership=None, user=None):
    ...
    return (
        ...
        patch(
            "derouter.proxy.management_endpoints.team_endpoints.get_team_membership",
...
async def test_team_member_me_returns_caller_membership(mock_db_client):
    ...
    from derouter.proxy.management_endpoints.team_endpoints import team_member_me
    ...
    user = DeRouter_UserTable(user_id=caller_id, user_email=caller_id, max_budget=None)
    ...
    assert response.derouter_budget_table is not None
    assert response.derouter_budget_table.max_budget == 100.0
    ...
    # DeRouter_BudgetTable which would silently strip this field.
    assert response.derouter_budget_table.budget_reset_at == datetime(
...
async def test_team_member_me_returns_defaults_when_no_membership_row(mock_db_client):
    """
    Caller is in members_with_roles but has no DeRouter_TeamMembership row yet
    ...
    assert response.derouter_budget_table is None
...
async def test_new_team_encrypts_callback_vars(
    ...
    from derouter.proxy._types import NewTeamRequest
    from derouter.proxy.common_utils.callback_utils import decrypt_callback_vars
    from derouter.proxy.management_endpoints.team_endpoints import new_team
    from derouter.proxy.utils import PrismaClient

    monkeypatch.setenv("DEROUTER_SALT_KEY", "test-salt-32-bytes-aaaaaaaaaaaaaa")
...
async def test_new_team_blocks_non_admin_passthrough_routes(mock_db_client):
    ...
    from derouter.proxy._types import NewTeamRequest, ProxyException
    from derouter.proxy.management_endpoints.team_endpoints import new_team

    with patch(
        "derouter.proxy.management_endpoints.team_endpoints._check_user_team_limits",
...
async def test_update_team_blocks_non_admin_passthrough_routes(mock_db_client):
    ...
    from derouter.proxy._types import ProxyException, UpdateTeamRequest
    from derouter.proxy.management_endpoints.team_endpoints import update_team

    existing = MagicMock()
    existing.model_dump.return_value = {"team_id": "t1"}
    mock_db_client.db.derouter_teamtable.find_unique = AsyncMock(return_value=existing)

    with patch(
        "derouter.proxy.management_endpoints.team_endpoints._verify_team_access",
        AsyncMock(return_value=None),
    ):
        with pytest.raises(ProxyException) as exc:
            await update_team(
...
def test_set_budget_reset_at_clears_when_budget_duration_null():
    ...
    """
    from derouter.proxy._types import UpdateTeamRequest
    from derouter.proxy.management_endpoints.team_endpoints import _set_budget_reset_at
...
async def test_team_info_forwards_key_limit_to_get_data():
    ...
    from derouter.proxy.management_endpoints import team_endpoints

    mock_prisma = MagicMock()
    mock_prisma.db.derouter_teamtable.find_unique = AsyncMock(
        return_value=DeRouter_TeamTable(team_id="team-1")
    )
...
async def test_team_info_returns_model_aliases():
    """/team/info must join DeRouter_ModelTable so the response exposes the team's
    current model aliases; without the ``derouter_model_table`` include the field
    ...
    team_row = DeRouter_TeamTable(
        team_id="team-1",
        derouter_model_table=DeRouter_ModelTable(
            id=1,
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_teamtable.find_unique = AsyncMock(return_value=team_row)
    mock_prisma.get_data = AsyncMock(return_value=[])
    ...
    include = mock_prisma.db.derouter_teamtable.find_unique.await_args.kwargs["include"]
    assert include["derouter_model_table"] is True

    derouter_model_table = response["team_info"].derouter_model_table
    assert derouter_model_table is not None
    assert derouter_model_table.model_aliases == {"gpt-4o": "gpt-4o-team-1"}
...
async def test_update_model_table_clears_aliases_with_empty_map():
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_modeltable.create = AsyncMock()
    mock_prisma.db.derouter_modeltable.upsert = AsyncMock(
    ...
    returned_model_id = await _update_model_table(
        ...
        prisma_client=mock_prisma,
        user_api_key_dict=user_api_key_dict,
        derouter_proxy_admin_name="default_user_id",
    )

    mock_prisma.db.derouter_modeltable.upsert.assert_awaited_once()
    upsert_kwargs = mock_prisma.db.derouter_modeltable.upsert.await_args.kwargs
    ...
    mock_prisma.db.derouter_modeltable.create.reset_mock()
    mock_prisma.db.derouter_modeltable.upsert.reset_mock()
    ...
    mock_prisma.db.derouter_modeltable.create.assert_not_called()
    mock_prisma.db.derouter_modeltable.upsert.assert_not_called()
...
class TestEmitTeamMembersMetric:
    ...
    def restore_callbacks(self):
        ...
        original = derouter.callbacks
        yield
        derouter.callbacks = original
    ...
    def test_emits_with_team_when_logger_registered(self, restore_callbacks):
        ...
        derouter.callbacks = [fake_logger]
...
async def test_new_team_rejects_reserved_ui_session_team_id():
    """
    /team/new must reject team_id "derouter-dashboard" (UI_TEAM_ID): it is the
    ...
    from derouter.proxy._types import UI_TEAM_ID, NewTeamRequest
    from derouter.proxy.management_endpoints.team_endpoints import new_team
...
# and asserts on the exact dict handed to derouter_teamtable.update.
# ---------------------------------------------------------------------------
...
async def _drive_team_write(
    ...
    *,
    ...
    with (
        _patch("derouter.proxy.proxy_server.prisma_client") as pc,
        _patch("derouter.proxy.proxy_server.llm_router", None),
        _patch("derouter.proxy.proxy_server.user_api_key_cache", MagicMock()),
        _patch("derouter.proxy.proxy_server.proxy_logging_obj", MagicMock()),
        _patch("derouter.proxy.proxy_server.derouter_proxy_admin_name", "admin"),
        _patch(
            "derouter.proxy.management_endpoints.team_endpoints._refresh_cached_team",
            new=AsyncMock(),
        ),
    ):
        pc.db.derouter_teamtable.find_unique = AsyncMock(
            return_value=None if find_returns_none else existing
        )
        pc.db.derouter_teamtable.update = AsyncMock(
            return_value=DeRouter_TeamTable(team_id=_PATCH_TEAM_ID, team_alias="t")
        )
        pc.jsonify_team_object = MagicMock(side_effect=lambda db_data: db_data)
        if mock_sink is not None:
            mock_sink["update"] = pc.db.derouter_teamtable.update
        ...
        return result, pc.db.derouter_teamtable.update
...
async def _written_metadata_with_budget(kind, body):
    ...
    from derouter.proxy._types import DeRouter_BudgetTable

    with (
        patch("derouter.proxy.proxy_server.premium_user", True),  # test-quality-ok: proxy_server module global is the endpoint's only injection point
        patch(  # test-quality-ok: update_team imports update_budget at call time; the module attribute is its only seam
            "derouter.proxy.management_endpoints.budget_management_endpoints.update_budget",
            AsyncMock(return_value=DeRouter_BudgetTable(budget_id="budget-existing-123")),
        ),
    ):
...
def test_patch_rejects_a_malformed_body_with_422(kwargs):
    ...
    from derouter.proxy._types import PatchTeamRequest
...
async def test_patch_enforces_team_access_via_delegation():
    ...
    from derouter.proxy._types import LitellmUserRoles, ProxyException, UserAPIKeyAuth
...
async def test_patch_returns_full_team_object_not_wrapper():
    ...
    assert isinstance(result, DeRouter_TeamTable)
...
async def test_new_team_validator_runs_without_metadata_and_rejection_blocks_create():
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma,
        patch("derouter.proxy.proxy_server._license_check") as mock_license,
        patch("derouter.proxy.proxy_server.user_api_key_cache", MagicMock()),
        patch("derouter.proxy.proxy_server.derouter_proxy_admin_name", "admin"),
        ...
        mock_prisma.db.derouter_teamtable.create.assert_not_awaited()
...
async def test_new_team_validator_accept_proceeds_to_create(mock_db_client, mock_admin_auth):
    ...
    mock_db_client.db.derouter_teamtable = MagicMock()
    mock_db_client.db.derouter_teamtable.create = AsyncMock(return_value=team_create_result)
    _wire_team_create_tx(mock_db_client)
    mock_db_client.db.derouter_teamtable.count = AsyncMock(return_value=0)
    mock_db_client.db.derouter_teamtable.update = AsyncMock(return_value=team_create_result)
    mock_db_client.db.derouter_usertable = MagicMock()
    mock_db_client.db.derouter_usertable.update = AsyncMock(return_value=MagicMock())
    ...
    mock_db_client.db.derouter_teamtable.create.assert_awaited_once()


@pytest.mark.asyncio
async def test_new_team_rejection_precedes_model_alias_write():
    """A rejected create must not leave an orphaned DeRouter_ModelTable row:
    ...
    with (
        ...
        mock_prisma.db.derouter_teamtable.create = AsyncMock()
        _wire_team_create_tx(mock_prisma)
        mock_prisma.db.derouter_modeltable.create = AsyncMock(return_value=MagicMock(id="model-1"))
        mock_license.is_team_count_over_limit.return_value = False
        ...
        mock_prisma.db.derouter_modeltable.create.assert_not_awaited()
        mock_prisma.db.derouter_teamtable.create.assert_not_awaited()
...
def _patch_body_to_update_request(body: dict):
    ...
    from derouter.proxy._types import PatchTeamRequest, UpdateTeamRequest
...
async def test_get_all_team_memberships_validates_rows():
    from derouter.proxy._types import DeRouter_TeamMembership
    from derouter.proxy.management_endpoints.team_endpoints import (
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_teammembership.find_many = AsyncMock(return_value=[membership_row])

    result = await get_all_team_memberships(mock_prisma_client, ["team-1"], user_id="member-1")

    assert len(result) == 1
    assert isinstance(result[0], DeRouter_TeamMembership)
    assert result[0].user_id == "member-1"
    assert result[0].team_id == "team-1"
    assert result[0].spend == 2.5
    find_many_kwargs = mock_prisma_client.db.derouter_teammembership.find_many.call_args.kwargs
...
async def test_list_available_teams_filters_joined_and_validates_rows(monkeypatch):
    ...
    import derouter
    from derouter.proxy.management_endpoints.team_endpoints import list_available_teams
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_usertable.find_unique = AsyncMock(return_value=user_row)
    mock_prisma_client.db.derouter_teamtable.find_many = AsyncMock(return_value=[open_team_row])
    ...
    assert len(result) == 1
    assert isinstance(result[0], DeRouter_TeamTable)
    assert result[0].team_id == "team-open"
    assert result[0].team_alias == "open team"
    find_many_kwargs = mock_prisma_client.db.derouter_teamtable.find_many.call_args.kwargs
    assert find_many_kwargs["where"] == {"team_id": {"in": ["team-open"]}}


@pytest.mark.asyncio
async def test_get_team_metadata_schema_returns_configured_fields():
    from derouter.proxy.management_endpoints.team_endpoints import get_team_metadata_schema
    from derouter.proxy.management_helpers.team_metadata_validation import (
...
def test_get_team_metadata_schema_route_requires_auth():
    ...
    with patch("derouter.proxy.proxy_server.master_key", "sk-1234"):
...
def test_team_metadata_schema_route_is_readable_by_non_admins():
    ...
    assert "/team/metadata_schema" in DeRouterRoutes.info_routes.value
    assert "/team/metadata_schema" in DeRouterRoutes.management_routes.value
...
async def test_resolve_existing_member_user_ids_matches_caller_supplied_user_ids():
    ...
    prisma_client = MagicMock()
    find_many = AsyncMock(
        return_value=[DeRouter_UserTable(user_id="by-id", max_budget=None, spend=0.0, user_email=None, models=[])]
    )

    with patch("derouter.proxy.management_endpoints.team_endpoints.UserRepository") as repo:
        repo.return_value.table.find_many = find_many
...
def _user_row(user_id: str, user_email: str | None) -> DeRouter_UserTable:
    return DeRouter_UserTable(
...
async def test_hydrate_member_emails_fills_in_emails_the_roster_snapshot_never_captured():
    ...
    """
    from derouter.proxy.management_endpoints.team_endpoints import _hydrate_member_emails
...
def test_pre_existing_user_ids_counts_ids_filled_in_by_member_resolution():
    ...
    """
    from derouter.proxy.management_endpoints.team_endpoints import _pre_existing_user_ids
...
def test_members_audit_value_serializes_to_a_json_object():
    ...
    from derouter.proxy.management_endpoints.team_endpoints import _members_audit_value
...
async def test_team_member_add_audits_a_user_created_from_a_list_payload(monkeypatch):
    ...
    """
    from derouter.proxy._types import TeamMemberAddRequest
    from derouter.proxy.management_endpoints.team_endpoints import team_member_add
    ...
    mock_prisma_client = AsyncMock()
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
    monkeypatch.setattr("derouter.proxy.proxy_server.premium_user", True)
    monkeypatch.setattr("derouter.proxy.proxy_server.derouter_proxy_admin_name", "default_user_id")

    team_row = DeRouter_TeamTable(team_id=team_id, members_with_roles=[])
    created_user = DeRouter_UserTable(
    ...
    with (
        ...
        patch(
            "derouter.proxy.management_endpoints.team_endpoints._validate_team_member_add_permissions",
            new_callable=AsyncMock,
        ),
        patch(
            "derouter.proxy.management_endpoints.team_endpoints._validate_and_populate_member_user_info",
            new_callable=AsyncMock,
        ),
        patch(
            "derouter.proxy.management_endpoints.team_endpoints._resolve_existing_member_user_ids",
            new_callable=AsyncMock,
        ...
        patch(
            "derouter.proxy.management_endpoints.team_endpoints._create_team_member_add_audit_logs",
            new_callable=AsyncMock,
        ) as mock_audit,
    ):
...
def _wire_update_team(stack, existing_metadata):
    ...
    mock_prisma_client = stack.enter_context(patch("derouter.proxy.proxy_server.prisma_client"))
    stack.enter_context(patch("derouter.proxy.proxy_server.llm_router"))
    stack.enter_context(patch("derouter.proxy.proxy_server.user_api_key_cache"))
    stack.enter_context(patch("derouter.proxy.proxy_server.proxy_logging_obj"))
    stack.enter_context(patch("derouter.proxy.proxy_server.derouter_proxy_admin_name", "admin"))
    stack.enter_context(patch("derouter.proxy.management_endpoints.team_endpoints._cache_team_object"))
    ...
    mock_prisma_client.db.derouter_teamtable.find_unique = AsyncMock(return_value=existing_team)

    updated_team = MagicMock()
    updated_team.team_id = "test_team_id"
    updated_team.model_dump.return_value = {"team_id": "test_team_id"}
    mock_prisma_client.db.derouter_teamtable.update = AsyncMock(return_value=updated_team)
...
async def test_update_team_output_token_estimate_unchanged_allows_team_admin_edit(
    ...
    assert prisma.db.derouter_teamtable.update.called
...
async def test_get_team_daily_activity_aggregated_scopes_and_flags(mock_db_client):
    ...
    with patch(
        ...
        with patch(
            "derouter.proxy.management_endpoints.team_endpoints.get_daily_activity_aggregated",
            new_callable=AsyncMock,
        ) as mock_aggregated:
            ...
            assert call_kwargs["table_name"] == "derouter_dailyteamspend"
...
def _wire_new_team_prisma(mock_db_client):
    ...
    mock_db_client.db.derouter_teamtable = MagicMock()
    mock_db_client.db.derouter_teamtable.count = AsyncMock(return_value=0)
    mock_db_client.db.derouter_teamtable.create = AsyncMock(return_value=created_team)
    _wire_team_create_tx(mock_db_client)
    mock_db_client.db.derouter_teamtable.update = AsyncMock(return_value=created_team)
    mock_db_client.db.derouter_usertable = MagicMock()
    mock_db_client.db.derouter_usertable.update = AsyncMock(return_value=MagicMock())

    return mock_db_client.db.derouter_teamtable.create
...
async def test_new_team_explicit_null_budget_duration_beats_configured_default(
    ...
    monkeypatch.setattr(derouter, "default_team_settings", None)
    monkeypatch.setattr(derouter, "default_team_params", {"budget_duration": "30d"})
...
async def test_new_team_explicit_null_max_budget_still_takes_configured_default(
    ...
    monkeypatch.setattr(derouter, "default_team_settings", None)
    monkeypatch.setattr(derouter, "default_team_params", {"max_budget": 100.0})
    mock_team_create = _wire_new_team_prisma(mock_db_client)
        ...
        class _Tx:
            async def __aenter__(self):
                ...
                return SimpleNamespace(
                    ...
                    derouter_teamtable=SimpleNamespace(create=outer._create_team),
                )
...
async def test_update_team_syncs_access_group_assigned_team_ids_in_both_directions(
    ...
    from derouter.proxy._types import DeRouter_AccessGroupTable
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client") as prisma,
        patch("derouter.proxy.proxy_server.llm_router"),
        patch("derouter.proxy.proxy_server.user_api_key_cache"),
        patch("derouter.proxy.proxy_server.proxy_logging_obj"),
        patch("derouter.proxy.proxy_server.derouter_proxy_admin_name", "admin"),
        patch("derouter.proxy.management_endpoints.team_endpoints._refresh_cached_team"),
        patch(
            "derouter.proxy.management_helpers.access_group_team_sync.invalidate_access_group_cache",
            new_callable=AsyncMock,
        ) as invalidate_cache,
    ):
        prisma.db.derouter_teamtable.find_unique = AsyncMock(return_value=existing_team)
        prisma.db.derouter_teamtable.update = AsyncMock(return_value=updated_team)
    ...
    async def _get_access_object(*, access_group_id, **_kwargs):
        ...
        return DeRouter_AccessGroupTable(
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.user_api_key_cache", MagicMock()),
        patch("derouter.proxy.proxy_server.proxy_logging_obj", MagicMock()),
        patch(
            "derouter.proxy.auth.auth_checks.get_access_object",
            new_callable=AsyncMock,
        ...
        authorized_models = await get_authorized_resources_from_key_access_groups(
            ...
            team_object=DeRouter_TeamTable(team_id="team-a", models=[]),
...
async def test_new_team_and_delete_team_both_drive_the_mirror(
    ...
    from derouter.proxy._types import DeleteTeamRequest, NewTeamRequest
    from derouter.proxy.management_endpoints.team_endpoints import delete_team, new_team
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.user_api_key_cache"),
        patch("derouter.proxy.proxy_server.proxy_logging_obj"),
        patch("derouter.proxy.management_endpoints.team_endpoints._add_team_members_to_team", new_callable=AsyncMock),
        patch(
            "derouter.proxy.management_helpers.access_group_team_sync.invalidate_access_group_cache",
            new_callable=AsyncMock,
        ) as invalidate_cache,
    ):
        prisma.db.derouter_teamtable.find_unique = AsyncMock(return_value=None)
        prisma.db.derouter_teamtable.count = AsyncMock(return_value=0)
        prisma.db.tx = fake_db.tx
        prisma.jsonify_team_object = MagicMock(side_effect=lambda db_data: db_data)
    ...
    team_row = DeRouter_TeamTable(team_id="team-gone", models=[], access_group_ids=["ag-1"])

    with (
        patch("derouter.proxy.proxy_server.prisma_client") as prisma,
        patch("derouter.proxy.proxy_server.derouter_proxy_admin_name", "admin"),
        patch("derouter.proxy.proxy_server.llm_router", None),
        patch("derouter.proxy.management_endpoints.team_endpoints._persist_deleted_team_records", new_callable=AsyncMock),
        patch("derouter.proxy.management_endpoints.team_endpoints._verify_team_access", new_callable=AsyncMock),
        patch(
            "derouter.proxy.management_endpoints.team_endpoints.sync_team_access_group_membership",
            new_callable=AsyncMock,
        ) as sync,
    ):
        prisma.db.derouter_teamtable.find_unique = AsyncMock(return_value=team_row)
        prisma.db.derouter_verificationtoken.find_many = AsyncMock(return_value=[])
        prisma.delete_data = AsyncMock(return_value=[team_row])
        prisma.db.execute_raw = AsyncMock(return_value=0)
        prisma.db.derouter_teammembership.delete_many = AsyncMock(return_value=0)
...
async def test_invalidate_access_group_cache_deletes_the_cached_object():
    ...
    with (
        patch("derouter.proxy.proxy_server.user_api_key_cache", cache),
        patch("derouter.proxy.proxy_server.proxy_logging_obj", logging_obj),
        patch(
            "derouter.proxy.management_helpers.access_group_team_sync._delete_cache_access_object",
            new_callable=AsyncMock,
        ) as delete_cached,
    ):
...
def test_validate_team_member_reset_spend_value_rejects_non_numeric():
    with pytest.raises(HTTPException) as exc:
        _validate_team_member_reset_spend_value(
            ...
            membership=DeRouter_TeamMembership(user_id="u1", team_id="t1", spend=10.0),
        )
...
def test_validate_team_member_reset_spend_value_rejects_above_max_budget():
    with pytest.raises(HTTPException) as exc:
        _validate_team_member_reset_spend_value(
            reset_to=10.0,
            membership=DeRouter_TeamMembership(
                user_id="u1",
                team_id="t1",
                spend=10.0,
                derouter_budget_table=DeRouter_BudgetTable(budget_id="b1", max_budget=5.0),
            ),
        )
    assert exc.value.status_code == 400
...
async def test_reset_team_member_spend_fn_success(monkeypatch):
    ...
    membership_row = DeRouter_TeamMembership(
        user_id="member-1",
        team_id="team-1",
        spend=10.0,
        derouter_budget_table=DeRouter_BudgetTable(budget_id="b1", max_budget=50.0),
    )
    mock_prisma_client.db.derouter_teammembership.find_unique = AsyncMock(return_value=membership_row)
    mock_prisma_client.db.derouter_teammembership.update = AsyncMock(return_value=membership_row)

    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
    monkeypatch.setattr("derouter.proxy.proxy_server.user_api_key_cache", real_cache)
    monkeypatch.setattr("derouter.proxy.proxy_server.proxy_logging_obj", mock_proxy_logging_obj)
    monkeypatch.setattr("derouter.proxy.proxy_server.spend_counter_cache", real_spend_counter_cache)

    with patch(  # test-quality-ok: no live DB here; matches this file's established convention for endpoint-logic unit tests
        "derouter.proxy.management_endpoints.team_endpoints.get_team_object",
        AsyncMock(return_value=DeRouter_TeamTable(team_id="team-1")),
    ):
    ...
    mock_prisma_client.db.derouter_teammembership.update.assert_awaited_once_with(
...
async def test_reset_team_member_spend_fn_membership_not_found(monkeypatch):
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_teammembership.find_unique = AsyncMock(return_value=None)
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
    monkeypatch.setattr("derouter.proxy.proxy_server.user_api_key_cache", MagicMock())
    monkeypatch.setattr("derouter.proxy.proxy_server.proxy_logging_obj", MagicMock())
...
async def test_reset_team_member_spend_fn_forbidden_for_non_admin(monkeypatch):
    ...
    with patch(  # test-quality-ok: no live DB here; matches this file's established convention for endpoint-logic unit tests
        "derouter.proxy.management_endpoints.team_endpoints.get_team_object",
        AsyncMock(return_value=DeRouter_TeamTable(team_id="team-1", members_with_roles=[])),
    ):
        with pytest.raises(HTTPException) as exc:
            await reset_team_member_spend_fn(
                team_id="team-1",
...
async def test_reset_team_member_spend_fn_team_admin_cannot_reset_own_spend(monkeypatch):
    ...
    with patch(  # test-quality-ok: no live DB here; matches this file's established convention for endpoint-logic unit tests
        "derouter.proxy.management_endpoints.team_endpoints.get_team_object",
        AsyncMock(
            return_value=DeRouter_TeamTable(
                team_id="team-1",
    ...
    assert exc.value.status_code == 403
    mock_prisma_client.db.derouter_teammembership.update.assert_not_called()
...
async def test_reset_team_member_spend_fn_proxy_admin_can_reset_own_spend(monkeypatch):
    ...
    membership_row = DeRouter_TeamMembership(user_id="admin-user", team_id="team-1", spend=10.0)
    mock_prisma_client.db.derouter_teammembership.find_unique = AsyncMock(return_value=membership_row)
    mock_prisma_client.db.derouter_teammembership.update = AsyncMock(return_value=membership_row)
...
async def test_team_member_update_invalidates_team_member_spend_state_when_budget_patch_applied(monkeypatch):
    ...
    team_row = DeRouter_TeamTable(team_id="team-1", metadata={}, members_with_roles=[])
    team_info_response = {
        "team_info": team_row,
        "team_memberships": [DeRouter_TeamMembership(user_id="member-1", team_id="team-1", budget_id=None)],
    }

    mock_prisma_client.db.derouter_teamtable.find_unique = AsyncMock(return_value=team_row)
    ...
    with (
        patch(  # test-quality-ok: no live DB here; matches this file's established convention for endpoint-logic unit tests
            "derouter.proxy.management_endpoints.team_endpoints.team_info",
            AsyncMock(return_value=team_info_response),
        ),
        patch(  # test-quality-ok: no live DB here; matches this file's established convention for endpoint-logic unit tests
            "derouter.proxy.management_endpoints.team_endpoints._upsert_budget_and_membership",
            AsyncMock(),
        ),
    ):
...
def _team_spend_by_user_caller(user_id: str, teams: list[str]) -> DeRouter_UserTable:
    return DeRouter_UserTable(
...
async def test_get_team_spend_by_user_admin_groups_spend_logs_by_team_and_user(mock_db_client):
    from derouter.proxy.management_endpoints.team_endpoints import get_team_spend_by_user
    ...
    mock_db_client.db.derouter_teamtable.find_many = AsyncMock(return_value=[alpha, beta])
    ...
    assert 'FROM "DeRouter_SpendLogs" sl' in sql
...
async def test_get_team_spend_by_user_team_admin_sees_every_member(mock_db_client):
    ...
    alpha = _team_spend_by_user_team("team-alpha", "Team Alpha", Member(user_id="alice", role="admin"), [])
    mock_db_client.db.derouter_teamtable.find_many = AsyncMock(return_value=[alpha])
    mock_db_client.db.query_raw = AsyncMock(return_value=[])
    mock_db_client.db.derouter_usertable.find_unique = AsyncMock(
