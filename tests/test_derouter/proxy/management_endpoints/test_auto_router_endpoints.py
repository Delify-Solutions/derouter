...
from derouter.types.management_endpoints.auto_router_endpoints import (
...
ROUTER_MODEL_LIST = [
    {"model_name": name, "derouter_params": {"model": "openai/gpt-4o-mini", "api_key": "fake-key"}}
...
class TestAutoRouterBenchmarks:
    from derouter.proxy.management_endpoints.auto_router_endpoints import _SessionAggRow
    ...
    async def _benchmarks(
        ...
        from derouter.proxy import proxy_server
        from derouter.proxy.management_endpoints.auto_router_endpoints import get_auto_router_benchmarks
    ...
    def test_overall_hit_rate_counts_hits_independently_of_bucketing(self):
        from derouter.proxy.management_endpoints.auto_router_endpoints import _benchmark_totals
    ...
    def test_summed_totals_carry_no_tier_map_because_names_are_router_scoped(self):
        from derouter.proxy.management_endpoints.auto_router_endpoints import _summed_agg_row
    ...
    async def test_a_malformed_deployment_is_skipped_rather_than_failing_the_dashboard(
        ...
        response = await self._benchmarks(
            ...
            model_list=[
                ...
                {"model_name": "", "derouter_params": {"model": "auto_router/complexity_router"}},
                {"model_name": 7, "derouter_params": {"model": "auto_router/complexity_router"}},
                {"model_name": "no-model", "derouter_params": {}},
                {"model_name": "unreadable-model", "derouter_params": {"model": None}},
            ],
        )
    ...
    def test_the_listed_kinds_match_the_router_types_traffic_can_record(self):
        ...
        from derouter.router_utils.auto_router_model_naming import StrategyRouterKind
        from derouter.types.utils import StandardLoggingRoutingDecision
...
from derouter.types.management_endpoints.auto_router_endpoints import SHADOW_EVAL_TURN_VALVE, StartShadowEvalRequest
...
def _shadow_router() -> Router:
    ...
    return Router(
        model_list=[
            {"model_name": "cheap", "derouter_params": {"model": "openai/gpt-4o-mini", "api_key": "fake"}},
            {"model_name": "mid", "derouter_params": {"model": "openai/gpt-4o", "api_key": "fake"}},
            {"model_name": "pricey", "derouter_params": {"model": "openai/o3", "api_key": "fake"}},
            {"model_name": "prefixed-tier", "derouter_params": {"model": "openai/gpt-4o", "api_key": "fake"}},
            {"model_name": "bare-tier", "derouter_params": {"model": "gpt-4o", "api_key": "fake"}},
            {"model_name": "house-sonnet", "derouter_params": {"model": "anthropic/claude-sonnet-5", "api_key": "fake"}},
            {
                "model_name": "model_name_team-a_x",
                "derouter_params": {"model": "anthropic/claude-sonnet-5", "api_key": "fake"},
...
def _shadow_prisma(
    ...
    prisma.db.derouter_verificationtoken.find_many = AsyncMock(side_effect=find_tokens)
    ...
    prisma.db.derouter_teamtable.find_many = AsyncMock(side_effect=find_teams)
    prisma.db.derouter_usertable.find_many = AsyncMock(side_effect=find_users)
    ...
    prisma.db.derouter_shadowevaljob.find_many = AsyncMock(side_effect=find_many_legs)
    prisma.db.derouter_shadowevaljob.create_many = AsyncMock(return_value=1)
    prisma.db.derouter_shadowevaljob.update_many = AsyncMock(return_value=1)
    prisma.db.derouter_shadowevalattempt.find_first = AsyncMock(return_value=None)
    prisma.db.derouter_shadowevalfunnel.create_many = AsyncMock(return_value=1)
    prisma.attempt_rows = []
    ...
    async def query_raw(sql: str, *params: object):
        ...
        if 'FROM "DeRouter_ShadowEvalFunnel"' in sql:
...
def _configure_anthropic_sdk_judge(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(derouter, "anthropic_key", "sk-test")
...
async def test_start_shadow_eval_writes_one_leg_per_key_in_one_statement(monkeypatch: pytest.MonkeyPatch):
    ...
    prisma.db.derouter_shadowevaljob.create_many.assert_awaited_once()
    rows = prisma.db.derouter_shadowevaljob.create_many.call_args.kwargs["data"]
...
async def test_start_shadow_eval_rejects_an_unconfigured_router_in_the_set(monkeypatch: pytest.MonkeyPatch):
    ...
    prisma.db.derouter_shadowevaljob.create_many.assert_not_called()
...
async def test_start_shadow_eval_rejects_an_uncredentialed_sdk_judge(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(derouter, "anthropic_key", None)
...
async def test_start_shadow_eval_accepts_an_sdk_judge_when_anthropic_secret_lookup_is_available(
    ...
    from derouter.types.secret_managers.main import KeyManagementSettings, KeyManagementSystem
    ...
    monkeypatch.setattr(derouter, "anthropic_key", None)
    monkeypatch.setattr(derouter, "api_key", None)
    monkeypatch.setattr(derouter, "secret_manager_client", AnthropicSecretManager())
    monkeypatch.setattr(derouter, "_key_management_system", KeyManagementSystem.CUSTOM)
    monkeypatch.setattr(derouter, "_key_management_settings", KeyManagementSettings(access_mode="read_only"))
    monkeypatch.delenv("ANTHROPIC_API_KEY", raising=False)
    monkeypatch.delenv("ANTHROPIC_AUTH_TOKEN", raising=False)
...
async def test_start_shadow_eval_accepts_a_judge_that_serves_neither_arm(
    ...
    monkeypatch.setattr(derouter, "api_key", "sk-test")
    import derouter.proxy.proxy_server as proxy_server
...
async def test_start_shadow_eval_accepts_a_team_public_scope_for_a_user_target(monkeypatch: pytest.MonkeyPatch):
    ...
    assert prisma.db.derouter_shadowevaljob.create_many.call_args.kwargs["data"][0]["models"] == ["house-judge"]
...
async def test_start_shadow_eval_concurrent_unique_violation_is_a_409(monkeypatch: pytest.MonkeyPatch):
    ...
    _configure_anthropic_sdk_judge(monkeypatch)
    prisma = _shadow_prisma()
    prisma.db.derouter_shadowevaljob.create_many = AsyncMock(
...
async def test_get_shadow_eval_job_pools_counts_and_slices_results_per_key(monkeypatch: pytest.MonkeyPatch):
    ...
    prisma.db.derouter_shadowevalattempt.find_first = AsyncMock(return_value=MagicMock(error="judge call failed: boom"))
    monkeypatch.setattr(proxy_server, "prisma_client", prisma)
    ...
    error_where = prisma.db.derouter_shadowevalattempt.find_first.call_args.kwargs["where"]
...
async def test_get_shadow_eval_job_slices_results_per_router(monkeypatch: pytest.MonkeyPatch):
    ...
    assert 'JOIN "DeRouter_ShadowEvalJob" j ON j.id = a.job_id' in router_sql
...
async def test_list_shadow_eval_jobs_collapses_legs_into_jobs_newest_first(monkeypatch: pytest.MonkeyPatch):
    ...
    group_reads = [
        call
        for call in prisma.db.derouter_shadowevaljob.find_many.call_args_list
...
def test_stopped_by_migration_backfills_every_job_that_displayed_stopped():
    ...
    import derouter_proxy_extras

    sql = (
        Path(derouter_proxy_extras.__file__).parent
        / "migrations"
...
def test_max_budget_migration_is_additive_and_leaves_legacy_rows_null():
    ...
    assert 'ALTER TABLE "DeRouter_ShadowEvalJob" ADD COLUMN     "max_budget" DOUBLE PRECISION' in sql
    assert (
...
async def test_stop_rejects_a_job_that_already_spent_its_budget(monkeypatch: pytest.MonkeyPatch):
    ...
    prisma.db.derouter_shadowevaljob.update_many.assert_not_called()
...
async def test_shadow_eval_responses_name_every_shadowed_key(monkeypatch: pytest.MonkeyPatch):
    ...
    batched_where = prisma.db.derouter_verificationtoken.find_many.call_args.kwargs["where"]
...
def test_every_shadow_eval_sql_constant_speaks_naive_utc():
    ...
    import derouter.proxy.management_endpoints.auto_router_endpoints as module
...
async def test_a_stop_racing_the_last_budgeted_attempt_reports_completed_not_stopped(
    ...
    assert exc.value.status_code == 400
    assert "already completed" in exc.value.detail
    assert prisma.db.derouter_shadowevaljob.find_many.await_args.kwargs["where"] == {"group_id": "job-1"}
...
async def test_start_shadow_eval_matches_a_bare_public_judge_name_to_a_prefixed_tier(
    ...
    derouter resolves it to; the tier is served by its deployment under the name the admin
...
async def test_start_shadow_eval_matches_a_prefixed_judge_name_to_a_bare_tier_deployment(
    ...
    An admin may configure a deployment as plain `gpt-4o` and derouter infers the provider.
...
async def test_start_shadow_eval_seeds_a_zero_funnel_row_per_leg(monkeypatch: pytest.MonkeyPatch):
    ...
    created = prisma.db.derouter_shadowevaljob.create_many.call_args.kwargs["data"]
    leg_ids = sorted(row["id"] for row in created)
    assert len(leg_ids) == 2 and all(leg_ids)
    seeded = prisma.db.derouter_shadowevalfunnel.create_many.call_args.kwargs
