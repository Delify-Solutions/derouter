"""
Tests for derouter/proxy/management_endpoints/common_utils.py
...
from derouter.proxy.management_endpoints.common_utils import _has_non_empty_value
...
class TestUpdateMetadataFieldsEmptyCollections:
    ...
    @patch("derouter.proxy.management_endpoints.common_utils._premium_user_check")
...
class TestOrgAdminCanInviteUser:
    ...
    def _make_membership(self, org_id: str, user_role: str):
        ...
        return DeRouter_OrganizationMembershipTable(
            user_id="u",
    ...
    def test_org_admin_can_invite_user_parametrized(
        ...
        target_user = DeRouter_UserTable(
            user_id="target",
            organization_memberships=[
...
class TestTeamAdminCanInviteUser:
    ...
    async def test_team_admin_can_invite_user_parametrized(
        ...
        admin_user = DeRouter_UserTable(user_id="admin", teams=admin_teams)
        target_user = DeRouter_UserTable(user_id="target", teams=target_teams)
        ...
        mock_prisma.db.derouter_teamtable.find_many = AsyncMock(return_value=teams)
    ...
    async def test_team_admin_can_invite_user_no_shared_team(self):
        ...
        mock_auth = MagicMock()
        mock_auth.user_id = "admin"
        admin_user = DeRouter_UserTable(user_id="admin", teams=[])
        target_user = DeRouter_UserTable(user_id="target", teams=["t1"])
...
class TestSetObjectMetadataField:
    ...
    def test_set_object_metadata_field_parametrized(
        ...
        team = DeRouter_TeamTable(team_id="t1", metadata={})
        with patch(
            "derouter.proxy.management_endpoints.common_utils._premium_user_check"
        ) as mock_premium:
    ...
    def test_set_object_metadata_field_initializes_metadata_if_none(self):
        ...
        team = DeRouter_TeamTable(team_id="t1", metadata=None)
        with patch(
            "derouter.proxy.management_endpoints.common_utils._premium_user_check"
        ):
    ...
    def test_mcp_rpm_limit_is_hoisted_into_metadata(self):
        ...
        DeRouter_ManagementEndpoint_MetadataFields to move the request field into
        ...
        """
        from derouter.proxy._types import DeRouter_ManagementEndpoint_MetadataFields

        assert "mcp_rpm_limit" in DeRouter_ManagementEndpoint_MetadataFields
...
class TestIsUserOrgAdminForTeam:
    ...
    async def test_get_user_object_called_with_caller_identity(self):
        ...
        with patch(
            "derouter.proxy.proxy_server.prisma_client", fake_prisma
        ), patch(
            "derouter.proxy.proxy_server.user_api_key_cache", fake_cache
        ), patch(
            "derouter.proxy.proxy_server.proxy_logging_obj", fake_logging
        ), patch(
            "derouter.proxy.auth.auth_checks.get_user_object", mock_get_user
        ):
...
class TestUserHasAdminPrivilegesGuard:
    ...
    async def test_no_user_lookup_when_prisma_is_none(self):
        ...
        mock_get_user = AsyncMock(return_value=None)
        with patch("derouter.proxy.auth.auth_checks.get_user_object", mock_get_user):
            result = await _user_has_admin_privileges(
...
class TestTeamAdminCanInviteUserQuery:
    ...
    async def test_find_many_queries_admin_teams_with_exact_where(self):
        ...
        mock_auth = MagicMock()
        mock_auth.user_id = "admin"
        admin_user = DeRouter_UserTable(user_id="admin", teams=["t1", "t2"])
        target_user = DeRouter_UserTable(user_id="target", teams=["t2"])
        ...
        mock_prisma.db.derouter_teamtable.find_many = find_many
