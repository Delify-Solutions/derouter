...
from derouter.types.search import SearchToolInfoResponse
...
async def test_list_search_tools_db_only(monkeypatch):
    ...
    db_tools = [
        {
            ...
            "derouter_params": {"search_provider": "perplexity", "api_key": "sk-test"},
    ...
    with patch(
        "derouter.proxy.search_endpoints.search_tool_management.SEARCH_TOOL_REGISTRY",
        mock_registry,
    ):
        # Mock prisma_client
        ...
        with patch("derouter.proxy.proxy_server.prisma_client", mock_prisma):
            ...
            with patch("derouter.proxy.proxy_server.proxy_config", mock_proxy_config):
                # Mock auth
                ...
                try:
                    ...
                    assert tool["derouter_params"]["api_key"] != "sk-test"
                    assert "****" in tool["derouter_params"]["api_key"]
                    assert tool["derouter_params"]["search_provider"] == "perplexity"
                finally:
        ...
        with patch("derouter.proxy.proxy_server.prisma_client", mock_prisma):
            ...
            with patch("derouter.proxy.proxy_server.proxy_config", mock_proxy_config):
                ...
                try:
                    ...
                    # Verify masking
                    assert "tv****ey" in tool["derouter_params"]["api_key"]
                finally:
                    app.dependency_overrides.pop(user_api_key_auth, None)
    ...
    db_tools = [
        {
            ...
            "derouter_params": {"search_provider": "perplexity", "api_key": "sk-db"},
    ...
    config_tools = [
        {
            ...
            "derouter_params": {"search_provider": "tavily", "api_key": "tvly-config"},
            "search_tool_info": {"description": "Config tool - duplicate"},
        },
        {
            "search_tool_name": "unique-config-tool",  # Unique - should be included
            "derouter_params": {"search_provider": "tavily", "api_key": "tvly-unique"},
        ...
        with patch("derouter.proxy.proxy_server.prisma_client", mock_prisma):
            ...
            with patch("derouter.proxy.proxy_server.proxy_config", mock_proxy_config):
                ...
                try:
                    ...
                    assert db_tool["derouter_params"]["api_key"] != "sk-db"
                    assert "****" in db_tool["derouter_params"]["api_key"]
                    assert db_tool["derouter_params"]["search_provider"] == "perplexity"
...
async def test_list_search_tools_datetime_conversion(monkeypatch):
    ...
    db_tools = [
        ...
        },
        {
            ...
            "derouter_params": {"search_provider": "tavily", "api_key": "tvly-test"},
        ...
        with patch("derouter.proxy.proxy_server.prisma_client", mock_prisma):
            ...
            with patch("derouter.proxy.proxy_server.proxy_config", mock_proxy_config):
                ...
                try:
                    ...
                    # Verify masking of sensitive values
                    assert tool1["derouter_params"]["api_key"] != "sk-test"
                    assert "****" in tool1["derouter_params"]["api_key"]
                    ...
                    # Verify masking of sensitive values
                    assert tool2["derouter_params"]["api_key"] != "tvly-test"
                    assert "****" in tool2["derouter_params"]["api_key"]
                    ...
                    # Verify masking of sensitive values
                    assert tool3["derouter_params"]["api_key"] != "sk-test"
                    assert "****" in tool3["derouter_params"]["api_key"]
                finally:
                    app.dependency_overrides.pop(user_api_key_auth, None)
        ...
        with patch("derouter.proxy.proxy_server.prisma_client", mock_prisma):
            ...
            with patch("derouter.proxy.proxy_server.proxy_config", mock_proxy_config):
                ...
                try:
                    ...
                    # Verify masking of sensitive values
                    assert (
                        data["search_tools"][0]["derouter_params"]["api_key"]
                        != "sk-test"
                    )
                    assert (
                        "****" in data["search_tools"][0]["derouter_params"]["api_key"]
                    )
                finally:
                    app.dependency_overrides.pop(user_api_key_auth, None)
        ...
        with patch("derouter.proxy.proxy_server.prisma_client", mock_prisma):
            ...
            with patch("derouter.proxy.proxy_server.proxy_config", mock_proxy_config):
                ...
                try:
                    ...
                    assert tool1 is not None
                    assert (
                        tool1["derouter_params"]["api_key"] != "pplx-sk-1234567890abcdef"
                    )
                    assert "****" in tool1["derouter_params"]["api_key"]
                    assert tool1["derouter_params"]["search_provider"] == "perplexity"
                    assert (
                        tool1["derouter_params"]["api_base"]
                    ...
                    assert tool2 is not None
                    assert tool2["derouter_params"]["api_key"] != "tvly-secret-key-12345"
                    assert "****" in tool2["derouter_params"]["api_key"]
                    assert tool2["derouter_params"]["search_provider"] == "tavily"
                    ...
                    assert tool3 is not None
                    assert (
                        tool3["derouter_params"]["access_token"]
                        != "token-abcdefghijklmnop"
                    )
                    assert "****" in tool3["derouter_params"]["access_token"]
                    assert tool3["derouter_params"]["secret_key"] != "secret-xyz123"
                    assert "****" in tool3["derouter_params"]["secret_key"]
                    ...
                    assert tool4["derouter_params"]["max_results"] == 10
                    assert tool4["derouter_params"]["timeout"] == 30
                    assert tool4["derouter_params"]["search_provider"] == "custom"
                finally:
                    app.dependency_overrides.pop(user_api_key_auth, None)
...
async def test_get_all_search_tools_from_db_retries_on_transport_error():
    ...
    mock_prisma_client.db.derouter_searchtoolstable.find_many = AsyncMock(
...
def _scoping_db_tools():
    return [
        ...
        },
        {
            ...
            "derouter_params": {"search_provider": "exa", "api_key": "exa-secret-3"},
...
async def test_list_search_tools_scoped_to_key_object_permission():
    ...
    leaked = {t["derouter_params"].get("api_base") for t in tools}
...
async def test_list_search_tools_dashboard_session_key_does_not_look_up_the_ui_team():
    ...
    ``derouter-dashboard``, which has no row in DeRouter_TeamTable. Resolving it as a real team
...
async def test_filter_visible_search_tools_dashboard_session_still_honors_key_allowlist():
    ...
    """
    from derouter.constants import UI_SESSION_TOKEN_TEAM_ID
    from derouter.proxy.search_endpoints.search_tool_management import (
...
# derouter.proxy.proxy_server module globals at call time and reaches its DB layer through a
...
def _search_tool_row(name: str, provider: str = "tavily") -> dict:
    return {
        ...
        "search_tool_name": name,
        "derouter_params": {"search_provider": provider, "api_key": "sk-test"},
...
def _fake_registry(db_rows: list) -> MagicMock:
    ...
    async def _add(search_tool, **_):
        row = _search_tool_row(
            ...
            provider=search_tool.get("derouter_params", {}).get("search_provider", "tavily"),
        )
        db_rows.append(row)
        return row
...
def _live_router_and_db(db_rows: list):
    ...
    with contextlib.ExitStack() as stack:
        stack.enter_context(patch("derouter.proxy.proxy_server.prisma_client", MagicMock()))  # test-quality-ok: proxy globals are the only seam; see the module note above
        stack.enter_context(patch("derouter.proxy.proxy_server.proxy_config", proxy_config))  # test-quality-ok: proxy globals are the only seam; see the module note above
        stack.enter_context(patch("derouter.proxy.proxy_server.llm_router", fake_router))  # test-quality-ok: proxy globals are the only seam; see the module note above
        stack.enter_context(
    ...
    with _live_router_and_db([_search_tool_row("tavily-search", provider="tavily")]) as fake_router:
        response = TestClient(app).put(
            ...
            json={
                "search_tool": {
                    "search_tool_name": "tavily-search",
                    "derouter_params": {"search_provider": "exa_ai"},
                }
            },
        )

    assert response.status_code == 200
    assert fake_router.search_tools[0]["derouter_params"]["search_provider"] == "exa_ai"
...
async def test_create_search_tool_survives_a_failing_router_refresh():
    ...
    with _live_router_and_db([]):
        with patch(  # test-quality-ok: forcing the refresh to fail needs the refresh itself replaced
            "derouter.proxy.proxy_server.ProxyConfig.reload_search_tools_from_db",
