...
def salt_key(monkeypatch):
    monkeypatch.setenv("DEROUTER_SALT_KEY", "sk-migration-salt-1234")
...
def _config_prisma(record):
    """Build an AsyncMock prisma client whose derouter_config returns `record`."""
    client = MagicMock()
    client.db.derouter_config.find_unique = AsyncMock(return_value=record)
    client.db.derouter_config.update = AsyncMock()
    return client
...
async def test_vantage_walker_migrates_legacy_field(salt_key, monkeypatch):
    ...
    client.db.derouter_config.update.assert_awaited_once()
    written = json.loads(
        client.db.derouter_config.update.call_args.kwargs["data"]["param_value"]
    )
...
async def test_vantage_walker_idempotent_no_write(salt_key, monkeypatch):
    ...
    client.db.derouter_config.update.assert_not_awaited()
...
async def test_sso_walker_real_run_migrates_and_clears_residual(salt_key, monkeypatch):
    ...
    client = MagicMock()
    client.db.derouter_ssoconfig.find_unique = AsyncMock(return_value=record)
    client.db.derouter_ssoconfig.update = AsyncMock()

    report = await cm._migrate_sso_config(client, dry_run=False)

    assert report.migrated == 1
    assert report.legacy == 0  # migrated -> no longer residual
    client.db.derouter_ssoconfig.update.assert_awaited_once()
...
async def test_sso_walker_dry_run_reports_residual_not_migrated(salt_key, monkeypatch):
    ...
    assert report.legacy == 1
    assert report.migrated == 0
    client.db.derouter_ssoconfig.update.assert_not_awaited()
...
async def test_check_reports_residual_legacy(salt_key, monkeypatch):
    ...
    client.db.derouter_teamtable.find_many = AsyncMock(return_value=[])
    client.db.derouter_verificationtoken.find_many = AsyncMock(return_value=[])
    client.db.derouter_ssoconfig.find_unique = AsyncMock(return_value=None)
    client.db.derouter_config.update = AsyncMock()
    ...
    client.db.derouter_config.find_unique = AsyncMock(side_effect=_find_unique)

    report = await cm.check_encryption(client)

    assert report.residual_legacy == 1
    client.db.derouter_config.update.assert_not_awaited()  # read-only
...
async def test_callback_vars_walker_migrates_team_metadata(salt_key, monkeypatch):
    ...
    client = MagicMock()
    client.db.derouter_teamtable.find_many = AsyncMock(return_value=[team_row])
    client.db.derouter_teamtable.update = AsyncMock()
    ...
    client.db.derouter_teamtable.update.assert_awaited_once()
    written = json.loads(
        client.db.derouter_teamtable.update.call_args.kwargs["data"]["metadata"]
    )
...
async def test_callback_vars_walker_dry_run_reports_legacy(salt_key, monkeypatch):
    ...
    assert report.migrated == 0
    client.db.derouter_teamtable.update.assert_not_awaited()
...
async def test_check_reports_callback_var_legacy_with_gate_off(salt_key, monkeypatch):
    ...
    client.db.derouter_verificationtoken.find_many = AsyncMock(return_value=[])
    client.db.derouter_ssoconfig.find_unique = AsyncMock(return_value=None)
    client.db.derouter_config.find_unique = AsyncMock(return_value=None)
    client.db.derouter_teamtable.update = AsyncMock()
    ...
    client.db.derouter_teamtable.update.assert_not_awaited()  # read-only
...
async def test_scan_covered_tables_classifies_legacy_and_v2(salt_key, monkeypatch):
    ...
    client = MagicMock()
    _empty_covered_tables(client)
    client.db.derouter_proxymodeltable.find_many = AsyncMock(
        return_value=[
            SimpleNamespace(derouter_params={"api_key": legacy, "model": "gpt-4"})
        ]
    )
    client.db.derouter_credentialstable.find_many = AsyncMock(
...
async def test_check_classifies_mcp_secret_maps(
    ...
    plaintext: Final = {"Authorization": "v2:gcm:operator-text", "CUSTOM": "derouter_enc::literal\n café "}
    ...
    _empty_covered_tables(client)
    client.db.derouter_mcpservertable.find_many = AsyncMock(return_value=[row])
    client.db.derouter_mcpservertable.update = AsyncMock()
    client.db.derouter_teamtable.find_many = AsyncMock(return_value=[])
    client.db.derouter_verificationtoken.find_many = AsyncMock(return_value=[])
    client.db.derouter_ssoconfig.find_unique = AsyncMock(return_value=None)
    client.db.derouter_config.find_unique = AsyncMock(return_value=None)
    ...
    client.db.derouter_mcpservertable.update.assert_not_awaited()
...
async def test_check_counts_covered_table_residual(salt_key, monkeypatch):
    ...
    client.db.derouter_config.update = AsyncMock()
    client.db.derouter_proxymodeltable.find_many = AsyncMock(
        return_value=[SimpleNamespace(derouter_params={"api_key": legacy})]
    )
...
async def test_migrate_covered_tables_reports_real_counts(salt_key, monkeypatch):
    ...
    row = SimpleNamespace(derouter_params={"api_key": legacy})
    client = MagicMock()
    _empty_covered_tables(client)
    client.db.derouter_proxymodeltable.find_many = AsyncMock(return_value=[row])
    client.db.derouter_config.find_unique = AsyncMock(return_value=None)

    async def fake_rotate(**kwargs):
        # Stand in for _rotate_master_key: re-encrypt the model api_key in place.
        row.derouter_params["api_key"] = v2
