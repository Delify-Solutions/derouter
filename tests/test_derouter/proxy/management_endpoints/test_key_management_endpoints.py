...
from derouter.proxy._types import (
    ...
    DeRouter_ProjectTableCachedObj,
...
from derouter.proxy.auth.auth_checks import _delete_cache_key_object, _project_cache_key
from derouter.proxy.auth.user_api_key_auth import UserAPIKeyAuth
...
async def test_list_keys():
    ...
    mock_prisma_client.db.derouter_verificationtoken.find_many = mock_find_many
    args = {
    ...
    assert json.dumps({"team_id": {"not": "derouter-dashboard"}}) in json.dumps(
        where_condition
    )
...
async def test_list_keys_include_created_by_keys():
    ...
    mock_prisma_client.db.derouter_verificationtoken.find_many = mock_find_many
    mock_prisma_client.db.derouter_verificationtoken.count = mock_count
...
async def test_key_token_handling(monkeypatch):
    ...
    mock_insert_data = AsyncMock(
        return_value=MagicMock(
            token="hashed_token_123", derouter_budget_table=None, object_permission=None
        )
    )
    mock_prisma_client.insert_data = mock_insert_data
    mock_prisma_client.db = MagicMock()
    mock_prisma_client.db.derouter_verificationtoken = MagicMock()
    mock_prisma_client.db.derouter_verificationtoken.find_unique = AsyncMock(
        return_value=None
    )
    mock_prisma_client.db.derouter_verificationtoken.find_many = AsyncMock(
        return_value=[]
    )
    mock_prisma_client.db.derouter_verificationtoken.count = AsyncMock(return_value=0)
    mock_prisma_client.db.derouter_verificationtoken.update = AsyncMock(
        return_value=MagicMock(
            token="hashed_token_123", derouter_budget_table=None, object_permission=None
        )
    )

    from derouter.proxy._types import GenerateKeyRequest, LitellmUserRoles
    from derouter.proxy.auth.user_api_key_auth import UserAPIKeyAuth
    from derouter.proxy.management_endpoints.key_management_endpoints import (
        generate_key_fn,
    )
...
async def test_budget_reset_and_expires_at_first_of_month(monkeypatch):
    ...
    mock_prisma_client = AsyncMock()
    mock_insert_data = AsyncMock(
        return_value=MagicMock(token="hashed_token_123", derouter_budget_table=None)
    )
    mock_prisma_client.insert_data = mock_insert_data
    mock_prisma_client.db = MagicMock()
    mock_prisma_client.db.derouter_verificationtoken = MagicMock()
...
async def test_key_generation_with_object_permission(monkeypatch):
    ...
    # Mock the prisma_client.db.derouter_objectpermissiontable.create call
    mock_object_permission_create = AsyncMock(
        return_value=MagicMock(object_permission_id="objperm123")
    )
    mock_prisma_client.db = MagicMock()
    mock_prisma_client.db.derouter_objectpermissiontable = MagicMock()
    mock_prisma_client.db.derouter_objectpermissiontable.create = (
...
async def test_generate_key_debug_log_never_contains_raw_token(monkeypatch, caplog):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
    monkeypatch.setattr(
        "derouter.proxy.management_endpoints.key_management_endpoints.get_ui_settings_cached",
...
async def test_generate_key_personal_non_admin_denied_for_team_scoped_fields(
    ...
    mock_prisma_client.db = MagicMock()
    mock_prisma_client.db.derouter_objectpermissiontable = MagicMock()
    mock_prisma_client.db.derouter_objectpermissiontable.create = AsyncMock(
    ...
    mock_prisma_client.db.derouter_objectpermissiontable.create.assert_not_called()
...
async def test_update_key_personal_non_admin_denied_vector_stores(monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.user_api_key_cache",
        MagicMock(),
    )
    ...
    data = UpdateKeyRequest(
        ...
        object_permission=DeRouter_ObjectPermissionBase(vector_stores=["acme_vs"]),
    )
...
async def test_update_key_grandfathers_existing_mcp_servers(monkeypatch):
    ...
    mock_prisma.db.derouter_mcpservertable.find_many = AsyncMock(return_value=[])
    ...
    monkeypatch.setattr(
        "derouter.proxy._experimental.mcp_server.mcp_server_manager.global_mcp_server_manager",
        mock_mgr,
    )
    ...
    result = await _validate_mcp_servers_for_key_update(
        data=UpdateKeyRequest(
            key="sk-team-key",
            object_permission=DeRouter_ObjectPermissionBase(mcp_servers=["server-a"]),
        ),
        team_obj=team_obj,
        existing_key_row=existing_key_row,
...
async def test_update_key_personal_non_admin_denied_access_groups(
    ...
    from derouter.proxy._types import LitellmUserRoles, UpdateKeyRequest
    from derouter.proxy.auth.user_api_key_auth import UserAPIKeyAuth
    from derouter.proxy.management_endpoints.key_management_endpoints import (
        _validate_update_key_data,
    )
...
async def test_key_generation_with_mcp_tool_permissions(monkeypatch):
    ...
    mock_prisma_client.db = MagicMock()
    mock_prisma_client.db.derouter_objectpermissiontable = MagicMock()
    mock_prisma_client.db.derouter_objectpermissiontable.create = mock_create
    mock_prisma_client.db.derouter_mcpservertable.find_many = AsyncMock(return_value=[])
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
    monkeypatch.setattr(
        "derouter.proxy.management_endpoints.key_management_endpoints.validate_key_mcp_servers_against_team",
        AsyncMock(
...
async def test_key_update_object_permissions_existing_permission(monkeypatch):
    ...
    existing_key_row = DeRouter_VerificationToken(
...
async def test_key_update_object_permissions_no_existing_permission(monkeypatch):
    ...
    DeRouter_ObjectPermissionTable and the key is updated with the new object_permission_id.
    """
    from unittest.mock import AsyncMock, MagicMock
    ...
    existing_key_row_no_perm = DeRouter_VerificationToken(
...
async def test_key_update_object_permissions_missing_permission_record(monkeypatch):
    ...
    existing_key_row_missing_perm = DeRouter_VerificationToken(
...
async def test_key_info_returns_object_permission(monkeypatch):
    ...
    from derouter.proxy._types import DeRouter_VerificationToken
    from derouter.proxy.management_endpoints.key_management_endpoints import info_key_fn
    ...
    mock_key_info = MagicMock(spec=DeRouter_VerificationToken)
    ...
    mock_key_info.derouter_budget_table = None
...
async def test_get_new_token_with_valid_key(monkeypatch):
    ...
    from derouter.proxy._types import RegenerateKeyRequest
    from derouter.proxy.management_endpoints.key_management_endpoints import (
        get_new_token,
    )
...
async def test_generate_key_fn_rejects_short_custom_key(monkeypatch, short_key):
    ...
    mock_prisma_client.db = MagicMock()
    mock_prisma_client.db.derouter_verificationtoken = MagicMock()
    mock_prisma_client.db.derouter_verificationtoken.find_unique = AsyncMock(return_value=None)
    mock_prisma_client.db.derouter_verificationtoken.find_many = AsyncMock(return_value=[])

    from derouter.proxy._types import GenerateKeyRequest, LitellmUserRoles, ProxyException
    from derouter.proxy.auth.user_api_key_auth import UserAPIKeyAuth
    from derouter.proxy.management_endpoints.key_management_endpoints import (
        generate_key_fn,
    )
...
async def test_generate_key_fn_accepts_custom_key_at_minimum_length(monkeypatch):
    ...
    mock_prisma_client = AsyncMock()
    mock_insert_data = AsyncMock(
        return_value=MagicMock(token="hashed_token_123", derouter_budget_table=None, object_permission=None)
    )
    mock_prisma_client.insert_data = mock_insert_data
    mock_prisma_client.db = MagicMock()
    mock_prisma_client.db.derouter_verificationtoken = MagicMock()
...
async def test_generate_service_account_works_with_team_id():
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.llm_router") as mock_router,
        patch("derouter.proxy.proxy_server.premium_user", False),
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints.generate_key_helper_fn"
...
async def test_generate_key_throttle_allowed_for_admin():
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client", AsyncMock()),
...
async def test_update_service_account_requires_team_id():
    ...
    existing_key = DeRouter_VerificationToken(token="hashed", team_id=None)
...
async def test_update_service_account_works_with_team_id():
    ...
    existing_key = DeRouter_VerificationToken(token="hashed")
...
async def test_update_key_enable_prompt_caching_folds_into_metadata(flag_value):
    ...
    existing_key = DeRouter_VerificationToken(
...
async def test_prepare_key_update_data_duration_never_expires():
    ...
    from derouter.proxy._types import UpdateKeyRequest
    from derouter.proxy.management_endpoints.key_management_endpoints import (
        prepare_key_update_data,
    )
...
async def test_generate_service_account_key_endpoint_validation():
    ...
    with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
        # Mock team not found
        mock_find_unique = AsyncMock(return_value=None)
        mock_prisma.db.derouter_teamtable.find_unique = mock_find_unique
...
async def test_unblock_key_supports_both_sk_and_hashed_tokens(monkeypatch):
    ...
    from derouter.proxy._types import BlockKeyRequest
    from derouter.proxy.management_endpoints.key_management_endpoints import unblock_key
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.user_api_key_cache", mock_user_api_key_cache
    )
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.proxy_logging_obj", mock_proxy_logging_obj
    )
    monkeypatch.setattr("derouter.proxy.proxy_server.hash_token", mock_hash_token)
    monkeypatch.setattr(
        "derouter.store_audit_logs", False
    ...
    monkeypatch.setattr(
        "derouter.proxy.management_endpoints.key_management_endpoints._delete_cache_key_object",
    ...
    sk_token_call = mock_prisma_client.db.derouter_verificationtoken.update.call_args.kwargs
    ...
    mock_prisma_client.db.derouter_verificationtoken.update.reset_mock()
    ...
    hashed_token_call = mock_prisma_client.db.derouter_verificationtoken.update.call_args.kwargs
...
async def test_unblock_key_invalid_key_format(monkeypatch):
    ...
    from derouter.proxy._types import BlockKeyRequest
    from derouter.proxy.management_endpoints.key_management_endpoints import unblock_key
    from derouter.proxy.utils import ProxyException
...
async def test_block_key_nonexistent_key_returns_404(monkeypatch):
    ...
    """
    from derouter.proxy._types import BlockKeyRequest
    from derouter.proxy.management_endpoints.key_management_endpoints import block_key
    ...
    mock_prisma_client.db.derouter_verificationtoken.update.assert_not_called()
...
async def test_update_key_nonexistent_key_returns_404(monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", None)
    monkeypatch.setattr("derouter.proxy.proxy_server.premium_user", True)
...
def _setup_update_key_mocks(monkeypatch, mock_prisma_client):
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
    monkeypatch.setattr("derouter.proxy.proxy_server.user_api_key_cache", AsyncMock())
...
async def test_update_key_rejects_a_duration_that_never_advances(monkeypatch, bad_duration):
    ...
    key_in_db = DeRouter_VerificationToken(token=hashed_token, user_id="test-user")
...
async def test_update_key_by_alias_only(monkeypatch):
    ...
    hashed_token = "0d62f396c1317066f55a96086517047c737087c61eb2bf016b72e6298927b15b"
    key_in_db = DeRouter_VerificationToken(
        token=hashed_token,
    ...
    mock_prisma_client.db.derouter_verificationtoken.find_first = AsyncMock(
        return_value=None
    )
    mock_prisma_client.update_data = AsyncMock(
    ...
    mock_prisma_client.db.derouter_verificationtoken.find_many.assert_called_once_with(
        where={"key_alias": "prod-alias"}, take=2
    )
    assert request_data.key == hashed_token
    mock_prisma_client.db.derouter_verificationtoken.find_unique.assert_not_called()
...
async def test_update_key_by_duplicate_alias_returns_400(monkeypatch):
    ...
    rows = [
        DeRouter_VerificationToken(token="hashed-token-1", key_alias="dup-alias"),
        DeRouter_VerificationToken(token="hashed-token-2", key_alias="dup-alias"),
    ]
    mock_prisma_client = AsyncMock()
    mock_prisma_client.db.derouter_verificationtoken.find_many = AsyncMock(
        return_value=rows
...
async def test_update_key_with_key_and_alias_selects_by_key(monkeypatch):
    ...
    mock_prisma_client.db.derouter_verificationtoken.find_many.assert_not_called()
    mock_prisma_client.db.derouter_verificationtoken.find_unique.assert_called_once()
...
async def test_block_key_existing_key_succeeds(monkeypatch):
    ...
    mock_prisma_client.db.derouter_verificationtoken.find_unique.assert_called_once_with(
        where={"token": test_hashed_token}
    )
    mock_prisma_client.db.derouter_verificationtoken.update.assert_called_once()
    block_call = mock_prisma_client.db.derouter_verificationtoken.update.call_args.kwargs
...
async def test_validate_key_team_change_with_member_permissions():
    ...
    from derouter.proxy._types import KeyManagementRoutes
    ...
    with patch(
        "derouter.proxy.management_endpoints.key_management_endpoints.can_team_access_model",
        new_callable=AsyncMock,
    ):
        with patch(
            "derouter.proxy.management_endpoints.key_management_endpoints._get_user_in_team"
        ) as mock_get_user:
            with patch(
                "derouter.proxy.management_endpoints.key_management_endpoints._is_user_team_admin"
            ) as mock_is_admin:
                with patch(
                    "derouter.proxy.management_endpoints.key_management_endpoints.TeamMemberPermissionChecks.does_team_member_have_permissions_for_endpoint"
                ) as mock_has_perms:
...
async def test_update_key_fn_auto_rotate_enable():
    ...
    from derouter.proxy._types import DeRouter_VerificationToken, UpdateKeyRequest
    from derouter.proxy.management_endpoints.key_management_endpoints import (
        prepare_key_update_data,
    )
...
async def test_check_team_key_limits_no_existing_keys():
    ...
    team_table = DeRouter_TeamTableCachedObj(
...
async def test_check_team_key_limits_on_update_excludes_self():
    ...
    """
    from derouter.proxy._types import hash_token as _ht
...
def test_check_team_key_model_specific_limits_rpm_overallocation():
    ...
    existing_key1 = DeRouter_VerificationToken(
        token="test-token-1",
        user_id="test-user-1",
        team_id="test-team-456",
        metadata={
    ...
    existing_key2 = DeRouter_VerificationToken(
        token="test-token-2",
        user_id="test-user-2",
        team_id="test-team-456",
        metadata={
...
async def test_generate_key_with_object_permission():
    ...
    mock_prisma_client.db.derouter_objectpermissiontable = MagicMock()
    mock_prisma_client.db.derouter_objectpermissiontable.create = mock_object_perm_create
...
async def test_generate_key_team_member_inherits_org_skips_membership_check():
    ...
    ``DeRouter_OrganizationMembership`` row on top of that broke the normal admin
    ...
    with (
        ...
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints.validate_key_search_tools_against_team",
            new_callable=AsyncMock,
        ),
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints._validate_caller_can_assign_key_org",
            mock_validate_org,
        ),
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints.get_org_object",
            new_callable=AsyncMock,
            return_value=MagicMock(derouter_budget_table=None),
        ),
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints._check_org_key_limits",
            new_callable=AsyncMock,
        ),
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints.generate_key_helper_fn",
...
async def test_generate_key_foreign_org_with_mismatched_team_still_enforces_membership():
    ...
    with (
        ...
        patch(
            "derouter_enterprise.proxy.management_endpoints.key_management_endpoints.apply_enterprise_key_management_params",
...
async def test_check_org_key_limits_no_existing_keys():
    ...
    org_table = DeRouter_OrganizationTable(
        ...
        created_by="admin",
        updated_by="admin",
        derouter_budget_table=DeRouter_BudgetTable(
            budget_id="budget-123",
            tpm_limit=20000,
            rpm_limit=2000,
        ),
...
def test_transform_verification_tokens_to_deleted_records():
    ...
    assert "derouter_budget_table" not in record1
    assert "derouter_organization_table" not in record1
...
async def test_persist_deleted_verification_tokens():
    ...
    key = DeRouter_VerificationToken(
        token="hashed-token-1",
        user_id="user-123",
        team_id="team-456",
        key_alias="test-key",
...
async def test_delete_verification_tokens_persists_deleted_keys(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.proxy.management_endpoints.key_management_endpoints._hash_token_if_needed",
        mock_hash_token,
    )
    monkeypatch.setattr(
        "derouter.proxy.management_endpoints.key_management_endpoints.hash_token",
        mock_hash_token,
    )
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.prisma_client",
...
async def test_delete_key_fn_persists_deleted_keys(monkeypatch):
    from derouter.proxy._types import KeyRequest
    from derouter.proxy.management_endpoints.key_management_endpoints import (
        delete_key_fn,
        delete_verification_tokens,
    )
    ...
    monkeypatch.setattr(
        "derouter.proxy.management_endpoints.key_management_endpoints.delete_verification_tokens",
    ...
    monkeypatch.setattr(
        "derouter.proxy.management_endpoints.key_management_endpoints.KeyManagementEventHooks.async_key_deleted_hook",
        AsyncMock(),
    )
...
async def test_can_delete_verification_token_proxy_admin_team_key(monkeypatch):
    ...
    key_info = DeRouter_VerificationToken(
        token="test-token",
        user_id="other-user",
        team_id="test-team-123",
    )
    ...
    monkeypatch.setattr(
        "derouter.proxy.management_endpoints.key_management_endpoints.get_team_object",
...
async def test_list_keys_with_expand_user():
    ...
    mock_prisma_client.db.derouter_verificationtoken.count = mock_count_keys
    mock_prisma_client.db.derouter_usertable.find_many = mock_find_many_users
    ...
    with patch(
        "derouter.proxy.management_endpoints.key_management_endpoints.attach_object_permission_to_dict",
...
async def test_list_keys_with_status_deleted():
    ...
    mock_prisma_client.db.derouter_deletedverificationtoken.find_many = (
        mock_find_many_deleted
    )
    mock_prisma_client.db.derouter_deletedverificationtoken.count = mock_count_deleted
    mock_prisma_client.db.derouter_verificationtoken.find_many = mock_find_many_regular
    mock_prisma_client.db.derouter_verificationtoken.count = mock_count_regular
...
async def test_list_keys_with_invalid_status():
    ...
    from derouter.proxy.management_endpoints.key_management_endpoints import list_keys
    from derouter.proxy.utils import ProxyException
...
async def test_list_keys_non_admin_user_id_auto_set():
    ...
    with patch("derouter.proxy.proxy_server.prisma_client", mock_prisma_client):
        with patch(
            "derouter.proxy.management_endpoints.key_management_endpoints.validate_key_list_check",
            return_value=mock_user_info,
        ):
            with patch(
                "derouter.proxy.management_endpoints.key_management_endpoints.get_admin_team_ids",
                return_value=[],
            ):
                with patch(
                    "derouter.proxy.management_endpoints.key_management_endpoints._list_key_helper",
...
def _make_member_team_table(
    ...
    """Build a DeRouter_TeamTable with a single member, suitable for list_keys tests."""
    from derouter.proxy._types import DeRouter_TeamTable, Member
...
async def _invoke_list_keys_and_capture_helper_kwargs(
    ...
    *,
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client", mock_prisma_client),
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints.validate_key_list_check",
            return_value=mock_user_info,
        ),
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints._fetch_user_team_objects",
...
async def test_get_and_validate_existing_key():
    ...
    mock_prisma_client = AsyncMock()
    mock_key = DeRouter_VerificationToken(
        token="test-key-123",
        user_id="user-123",
        models=["gpt-4"],
        team_id=None,
...
async def test_process_single_key_update():
    ...
    """
    from derouter.types.proxy.management_endpoints.key_management_endpoints import (
    ...
    with patch(
        "derouter.proxy.management_endpoints.key_management_endpoints.prepare_key_update_data"
    ) as mock_prepare:
        mock_prepare.return_value = {"max_budget": 100.0, "tags": ["production"]}

        # Mock TeamMemberPermissionChecks
        with patch(
            "derouter.proxy.management_endpoints.key_management_endpoints.TeamMemberPermissionChecks.can_team_member_execute_key_management_endpoint"
            ...
            with patch(
                ...
                # Mock hash_token (imported from derouter.proxy._types)
                with patch("derouter.proxy._types.hash_token") as mock_hash:
                    ...
                    with patch(
                        ...
                        with patch(
                            "derouter.proxy.management_endpoints.key_management_endpoints.KeyManagementEventHooks.async_key_updated_hook"
                        ):
...
async def test_bulk_update_keys_success(monkeypatch):
    ...
    # Mock existing keys
    existing_key_1 = DeRouter_VerificationToken(
        token="test-key-1",
        user_id="user-123",
        models=["gpt-4"],
        team_id=None,
    ...
    existing_key_2 = DeRouter_VerificationToken(
        token="test-key-2",
        user_id="user-123",
        models=["gpt-3.5-turbo"],
        team_id=None,
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", mock_llm_router)
...
async def test_reset_key_spend_success(monkeypatch):
    ...
    updated_key = DeRouter_VerificationToken(
        token=hashed_key,
        user_id="test-user",
        spend=50.0,
        max_budget=200.0,
    ...
    with (
        patch("derouter.proxy.proxy_server.hash_token") as mock_hash_token,
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints._check_proxy_or_team_admin_for_key"
        ) as mock_check_admin,
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints._delete_cache_key_object"
        ) as mock_delete_cache,
        ...
        monkeypatch.setattr(
            "derouter.proxy.proxy_server.spend_counter_cache",
...
async def test_reset_key_spend_resets_budget_windows(monkeypatch):
    ...
    DeRouter_SpendLogs rows inside the still-open window, merely zeroing that
    ...
    with (
        patch("derouter.proxy.proxy_server.hash_token") as mock_hash_token,  # test-quality-ok: no HTTP boundary; same pattern as test_reset_key_spend_success
    ...
    assert mock_prisma_client.db.derouter_verificationtoken.update.call_count == 2
    window_update_call = mock_prisma_client.db.derouter_verificationtoken.update.call_args_list[1]
...
async def test_delete_cache_key_object_broadcasts_invalidation(monkeypatch):
    ...
    with patch(  # test-quality-ok: pub/sub broadcast to other workers has no HTTP boundary to fake
        "derouter.proxy.auth.auth_checks.publish_auth_cache_invalidation"
    ) as mock_publish:
...
async def test_reset_key_spend_key_not_found(monkeypatch):
    ...
    with patch("derouter.proxy.proxy_server.hash_token") as mock_hash_token:
...
async def test_reset_key_spend_hashed_key(monkeypatch):
    ...
    with (
        ...
        assert response["spend"] == 50.0
        mock_prisma_client.db.derouter_verificationtoken.find_unique.assert_called_once_with(
            where={"token": hashed_key}, include={"derouter_budget_table": True}
        )
...
async def test_validate_key_list_check_team_admin_success():
    ...
    mock_prisma_client.db.derouter_usertable.find_unique = AsyncMock(
...
async def test_validate_key_list_check_key_hash_authorized():
    ...
    with patch(
        "derouter.proxy.management_endpoints.key_management_endpoints._can_user_query_key_info"
    ) as mock_can_query:
...
async def test_validate_key_list_check_key_hash_row_missing():
    ...
    mock_prisma_client = AsyncMock()
    mock_prisma_client.db.derouter_usertable.find_unique = AsyncMock(
        return_value=DeRouter_UserTable(
            user_id="test-user",
            user_email="test@example.com",
            teams=[],
            organization_memberships=[],
...
async def test_validate_key_list_check_proxy_admin_viewer_skips_db_lookup():
    ...
    assert result is None
    mock_prisma_client.db.derouter_usertable.find_unique.assert_not_awaited()
...
async def test_key_with_budget_id_does_not_store_budget_duration():
    ...
    reset_budget_for_derouter_budget_table() resets them when the tier resets.
    ...
    mock_prisma.db.derouter_budgettable.find_unique.assert_not_called()
...
async def test_key_does_not_override_explicit_budget_duration():
    ...
    mock_prisma.db.derouter_budgettable.find_unique = AsyncMock(
...
@pytest.mark.asyncio
@patch(
    "derouter.proxy.management_endpoints.key_management_endpoints.rotate_mcp_server_credentials_master_key"
)
async def test_rotate_master_key_reencrypts_model_params_in_place(
    ...
    on DeRouter_ProxyModelTable. Rotation used to rebuild the table via
    ...
    audit columns. Rotation must instead update only derouter_params (the sole
    ...
    mock_prisma_client.db.derouter_proxymodeltable.find_many = AsyncMock(
        return_value=[mock_model]
    )

    # Mock transaction context manager
    mock_tx = AsyncMock()
    mock_tx.derouter_proxymodeltable = MagicMock()
    mock_tx.derouter_proxymodeltable.delete_many = AsyncMock()
    mock_tx.derouter_proxymodeltable.create_many = AsyncMock()
    mock_tx.derouter_proxymodeltable.update_many = AsyncMock()
    ...
    mock_prisma_client.db.derouter_config.find_many = AsyncMock(return_value=[])

    # Mock credentials table — no credentials
    mock_prisma_client.db.derouter_credentialstable.find_many = AsyncMock(
        return_value=[]
    )
    ...
    with patch(
        "derouter.proxy.proxy_server.proxy_config",
        mock_proxy_config,
    ):
    ...
    mock_tx.derouter_proxymodeltable.delete_many.assert_not_called()
    mock_tx.derouter_proxymodeltable.create_many.assert_not_called()

    mock_tx.derouter_proxymodeltable.update_many.assert_called_once()
    call_args = mock_tx.derouter_proxymodeltable.update_many.call_args
    ...
    assert set(update_data.keys()) == {"derouter_params"}, (
        "rotation must touch only the encrypted derouter_params column; writing any "
    ...
    assert isinstance(
        update_data["derouter_params"], prisma.Json
    ), f"derouter_params should be prisma.Json for update_many(), got {type(update_data['derouter_params'])}"
    reencrypted_params = update_data["derouter_params"].data
...
async def test_default_key_generate_params_duration(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "default_key_generate_params", {"duration": "180d"})
...
async def test_default_key_generate_params_object_permission_applied_when_absent(
    ...
    created_data = mock_prisma_client.db.derouter_objectpermissiontable.create.call_args.kwargs["data"]
...
async def test_default_key_generate_params_object_permission_merges_partial(
    ...
    """
    import derouter
    from derouter.proxy._types import DeRouter_ObjectPermissionBase
    ...
    request = GenerateKeyRequest(
        object_permission=DeRouter_ObjectPermissionBase(agents=["agent-1"])
    )
    await _common_key_generation_helper(
        data=request,
        user_api_key_dict=UserAPIKeyAuth(
...
async def test_get_member_team_ids():
    ...
    mock_prisma_client.db.derouter_teamtable.find_many = AsyncMock(
...
async def test_generate_key_with_agent_id():
    ...
    from derouter.proxy._types import GenerateKeyRequest
...
async def test_generate_key_helper_fn_agent_id():
    ...
    import derouter.proxy.management_endpoints.key_management_endpoints as km
...
class TestValidateKeyAliasFormat:
    ...
    def reset_key_alias_flag(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(derouter, "enable_key_alias_format_validation", False)
    ...
    def test_validate_key_alias_format_valid(self, monkeypatch):
        ...
        monkeypatch.setattr(derouter, "enable_key_alias_format_validation", True)
        # Valid cases
...
async def test_check_org_key_limits_on_update_overallocation():
    ...
    """
    from derouter.proxy._types import hash_token as _hash_token
...
async def test_block_key_allowed_for_team_admin(monkeypatch):
    ...
    team_obj = DeRouter_TeamTableCachedObj(
        team_id=team_id,
        members_with_roles=[
...
async def test_update_key_non_budget_fields_allowed_for_internal_user(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.proxy.management_endpoints.key_management_endpoints._enforce_unique_key_alias",
...
async def test_update_key_throttle_on_budget_exceeded_rejected_for_internal_user(
    ...
    mock_prisma_client.get_data = AsyncMock(return_value=mock_existing_key)
    mock_prisma_client.db.derouter_verificationtoken.find_unique = AsyncMock(return_value=mock_existing_key)

    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
    monkeypatch.setattr("derouter.proxy.proxy_server.user_api_key_cache", mock_user_api_key_cache)
...
async def test_update_key_throttle_unchanged_allows_non_budget_edit_for_internal_user(
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.hash_token", lambda token: test_hashed_token)
...
async def test_update_key_non_budget_rejects_cross_user_modification(monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.premium_user", True)
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.hash_token", lambda t: test_hashed_token
    )
...
async def test_update_key_team_member_with_permission_can_update_non_budget(
    ...
    monkeypatch.setattr(
        "derouter.proxy.management_helpers.team_member_permission_checks.get_team_object",
        mock_get_team_object,
    )
    monkeypatch.setattr(
        "derouter.proxy.management_endpoints.key_management_endpoints._enforce_unique_key_alias",
...
class TestLIT1884KeyGenerateValidation:
    ...
    async def test_internal_user_generate_key_no_user_id_auto_assigns(self):
        ...
        with (
            ...
            patch("derouter.proxy.proxy_server.user_custom_key_generate", None),
            patch(
                "derouter.proxy.management_endpoints.key_management_endpoints._common_key_generation_helper",
                new_callable=AsyncMock,
                return_value=MagicMock(),
            ),
        ):
    ...
    def test_key_generation_check_non_admin_no_team_table_raises(self):
        ...
        with patch.object(derouter, "key_generation_settings", None):
            with pytest.raises(HTTPException) as exc_info:
                key_generation_check(
                    team_table=None,
                    user_api_key_dict=user_api_key_dict,
...
class TestKeyOwnerPrivilegeEscalation:
    ...
    async def test_assigned_user_blocked_from_any_edit(self):
        ...
        with patch(
            "derouter.proxy.management_endpoints.key_management_endpoints._check_key_admin_access",
            mock_check,
        ):
            with pytest.raises(HTTPException) as exc_info:
                await _validate_update_key_data(
...
class TestKeyAliasSkipValidationOnUnchanged:
    ...
    def mock_prisma(self):
        prisma = MagicMock()
        prisma.db = MagicMock()
        prisma.db.derouter_verificationtoken = MagicMock()
    ...
    def existing_key_with_legacy_alias(self):
        ...
        return DeRouter_VerificationToken(
            token="hashed_token_123",
    ...
    async def test_update_key_unchanged_legacy_alias_passes(
        ...
        import re
        from derouter.proxy.management_endpoints import key_management_endpoints as mod
...
def test_enforce_upperbound_no_config_is_noop(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "upperbound_key_generate_params", None)
...
def _make_regenerate_mock_prisma():
    ...
    mock_prisma_client.db.derouter_verificationtoken.create = AsyncMock(
        return_value=None
    )
...
async def test_execute_virtual_key_regeneration_rejects_over_limit_duration(monkeypatch):
    ...
    with (
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints.get_new_token",
            new_callable=AsyncMock,
            return_value="sk-newtoken1234ab12",
        ),
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints._insert_deprecated_key",
            new_callable=AsyncMock,
        ),
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints._delete_cache_key_object",
    ...
    assert mock_prisma_client.db.derouter_verificationtoken.update.await_count == 0
...
async def test_execute_virtual_key_regeneration_allows_within_limit_duration(monkeypatch):
    ...
    with (
        ...
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints.KeyManagementEventHooks.async_key_rotated_hook",
            new_callable=AsyncMock,
        ),
    ):
        await _execute_virtual_key_regeneration(
    ...
    assert mock_prisma_client.db.derouter_verificationtoken.update.await_count == 1
...
async def test_regenerate_evicts_jwt_key_mapping_cache_so_next_jwt_call_gets_new_token():
    ...
    from derouter.proxy._types import DeRouter_JWTAuth
    from derouter.proxy.auth.auth_method import AuthMethod
    from derouter.proxy.auth.resolvers.models import CredentialRef
    from derouter.proxy.auth.resolvers.store import IdentityStore
    from derouter.proxy.auth.user_api_key_auth import _resolve_jwt_to_virtual_key
    from derouter.proxy.management_endpoints.key_management_endpoints import (
        _execute_virtual_key_regeneration,
    )
    ...
    existing_key = _make_regenerate_existing_key()
    mock_prisma_client = _make_regenerate_mock_prisma()
    mock_prisma_client.db.derouter_jwtkeymapping.find_many = AsyncMock(
        return_value=[MagicMock(jwt_claim_name="sub", jwt_claim_value="user1")]
    )
    mock_prisma_client.db.derouter_jwtkeymapping.find_first = AsyncMock(
    ...
    with (
        ...
        patch(  # test-quality-ok: captures the cross-worker broadcast without a redis instance
            "derouter.proxy.common_utils.auth_cache_invalidation_pubsub.publish_auth_cache_invalidation",
            publish_mock,
        ),
    ):
        await _execute_virtual_key_regeneration(
    ...
    mock_prisma_client.db.derouter_jwtkeymapping.find_many.assert_awaited_once_with(where={"token": "abc123"})
    ...
    jwt_handler.derouter_jwtauth = DeRouter_JWTAuth(
    ...
    with patch(  # test-quality-ok: DB-backed resolve; fake asserts it receives the rotated hash
        "derouter.proxy.auth.resolvers.store.IdentityStore.resolve",
        new_callable=AsyncMock,
...
class TestAllowedRoutesCallerPermission:
    ...
    async def test_non_admin_update_key_with_allowed_routes_rejected(self):
        ...
        with (
            patch("derouter.proxy.proxy_server.prisma_client", mock_prisma_client),
            patch("derouter.proxy.proxy_server.user_api_key_cache", MagicMock()),
            patch("derouter.proxy.proxy_server.user_custom_key_update", None),
            patch("derouter.proxy.proxy_server.llm_router", None),
            patch("derouter.proxy.proxy_server.premium_user", True),
            patch("derouter.proxy.proxy_server.proxy_logging_obj", MagicMock()),
            patch(
                "derouter.proxy.management_endpoints.key_management_endpoints._get_and_validate_existing_key",
                new_callable=AsyncMock,
                return_value=MagicMock(),
            ),
        ):
    ...
    async def test_non_admin_regenerate_key_explicit_empty_allowed_routes_rejected(self):
        ...
        with patch("derouter.proxy.proxy_server.premium_user", True):
            with pytest.raises(ProxyException) as exc_info:
    ...
    async def test_non_admin_regenerate_key_allowed_routes_rejected_before_enterprise_gate(self):
        ...
        with patch("derouter.proxy.proxy_server.premium_user", False):
            with pytest.raises(ProxyException) as exc_info:
                await regenerate_key_fn(
                    key=None,
                    data=data,
...
def test_jinja_prompt_manager_is_sandboxed():
    ...
    from derouter.integrations.dotprompt.prompt_manager import PromptManager
...
_BULK_PKG = "derouter.proxy.management_endpoints.key_management_endpoints"


def _make_team_key(token: str, team_id: str = "team-abc") -> DeRouter_VerificationToken:
    return DeRouter_VerificationToken(
        token=token,
        user_id="user-123",
        models=[],
...
def _setup_team_keys_mocks(
    ...
    *,
    ...
    mock_prisma.db.derouter_verificationtoken.find_many = AsyncMock(
        return_value=[] if find_many is None else find_many
    )
    if find_unique is not None:
        mock_prisma.db.derouter_verificationtoken.find_unique = find_unique
    if update_data is not None:
        mock_prisma.update_data = update_data
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma)
    monkeypatch.setattr("derouter.proxy.proxy_server.user_api_key_cache", MagicMock())
    monkeypatch.setattr("derouter.proxy.proxy_server.proxy_logging_obj", MagicMock())
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", MagicMock())
    monkeypatch.setattr("derouter.proxy.proxy_server.user_custom_key_update", None)
    monkeypatch.setattr(
...
async def _call_as_admin(data):
    ...
    return await bulk_update_team_keys(
        data=data, user_api_key_dict=_admin(), derouter_changed_by=None
    )
...
async def test_bulk_update_team_keys_success_with_key_ids(monkeypatch):
    ...
    assert len(response.successful_updates) == 2
    assert len(response.failed_updates) == 0
    where = mock.db.derouter_verificationtoken.find_many.await_args.kwargs["where"]
...
def test_handle_key_type_persists_key_type_and_derives_routes():
    ...
    from derouter.proxy._types import GenerateKeyRequest, DeRouterKeyType
    from derouter.proxy.management_endpoints.key_management_endpoints import (
        handle_key_type,
    )

    cases = {
        DeRouterKeyType.MANAGEMENT: ("management", ["management_routes"]),
        DeRouterKeyType.READ_ONLY: ("read_only", ["info_routes"]),
        DeRouterKeyType.LLM_API: ("llm_api", ["llm_api_routes"]),
    }
...
def test_handle_key_type_default_persists_type_without_forcing_routes():
    ...
    data = GenerateKeyRequest(key_type=DeRouterKeyType.DEFAULT)
    out = handle_key_type(data, {"allowed_routes": ["/scim/*"], "key_type": DeRouterKeyType.DEFAULT})
...
async def test_bulk_update_team_keys_hashes_raw_sk_key_ids(monkeypatch):
    ...
    row = DeRouter_VerificationToken(
...
async def test_bulk_update_team_keys_does_not_log_raw_sk_token_on_failure(
    ...
    with caplog.at_level(logging.ERROR, logger="DeRouter Proxy"):
        response = await _call_as_admin(
            BulkUpdateTeamKeysRequest(
                team_id="team-abc",
                key_ids=[raw_sk],
...
async def test_regenerate_premium_gate_requires_actual_master_key():
    ...
    with (
        patch("derouter.proxy.proxy_server.premium_user", False),
        patch("derouter.proxy.proxy_server.master_key", "sk-the-real-master-key"),
        patch("derouter.proxy.proxy_server.prisma_client", AsyncMock()),
...
async def test_regenerate_premium_gate_allows_actual_master_key_holder():
    ...
    with (
        patch("derouter.proxy.proxy_server.premium_user", False),
        patch("derouter.proxy.proxy_server.master_key", master),
        patch("derouter.proxy.proxy_server.prisma_client", AsyncMock()),
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints._rotate_master_key",
            new_callable=AsyncMock,
        ),
    ):
...
async def test_regenerate_applies_normalized_mcp_object_permission():
    ...
    data = RegenerateKeyRequest(
        key="sk-old",
        object_permission=DeRouter_ObjectPermissionBase(mcp_servers=["server-alias"]),
    )
    existing_key = _make_regenerate_existing_key()
    mock_prisma_client = AsyncMock()
    mock_repo = MagicMock()
    ...
    with (
        patch("derouter.proxy.proxy_server.premium_user", True),
        patch("derouter.proxy.proxy_server.master_key", None),
        patch("derouter.proxy.proxy_server.prisma_client", mock_prisma_client),
        patch("derouter.proxy.proxy_server.proxy_logging_obj", MagicMock()),
        patch("derouter.proxy.proxy_server.user_api_key_cache", MagicMock()),
        patch("derouter.proxy.proxy_server.hash_token", lambda token: "hashed-old"),
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints.VerificationTokenRepository",
        ...
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints.TeamMemberPermissionChecks.enforce_member_can_assign_access_groups",
        ),
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints.can_modify_verification_token",
            new_callable=AsyncMock,
            return_value=True,
        ),
        patch(
        ...
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints.validate_key_vector_stores_against_team",
            new_callable=AsyncMock,
        ),
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints._persist_deleted_verification_tokens",
            new_callable=AsyncMock,
        ),
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints._execute_virtual_key_regeneration",
            execute_mock,
        ),
    ):
        await regenerate_key_fn(
...
async def test_ghsa_q775_upperbound_default_not_rejected():
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.user_custom_key_generate", None),
        patch(
            "derouter.upperbound_key_generate_params",
...
async def test_ghsa_q775_default_key_generate_params_not_rejected():
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.user_custom_key_generate", None),
        patch(
            "derouter.default_key_generate_params",
            {"max_budget": 50.0},
        ),
        patch(
            "derouter.proxy.management_endpoints.key_management_endpoints._common_key_generation_helper",
...
async def test_ghsa_q775_ui_session_token_team_key_exempt_from_budget_ceiling():
    """
    Regression: a UI/CLI session token (team_id=derouter-dashboard) creating a
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.llm_router", None),
        patch("derouter.proxy.proxy_server.premium_user", False),
        patch("derouter.proxy.proxy_server.derouter_proxy_admin_name", "default_user_id"),
    ):
        try:
            await _common_key_generation_helper(
                data=data,
...
async def test_ghsa_q775_ui_session_token_personal_key_still_capped():
    ...
    to personal keys. A UI/CLI session token (team_id=derouter-dashboard) creating a
...
async def test_ghsa_q775_default_team_id_does_not_grant_session_token_exemption():
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.premium_user", False),
        patch("derouter.proxy.proxy_server.derouter_proxy_admin_name", "default_user_id"),
        patch("derouter.default_key_generate_params", {"team_id": "injected-team"}),
    ):
        with pytest.raises((HTTPException, ProxyException)) as exc_info:
            await _common_key_generation_helper(
                data=data,
...
async def test_info_key_fn_includes_model_max_budget_usage(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.model_max_budget_limiter.dual_cache",
...
async def test_info_key_fn_v2_includes_model_max_budget_usage(monkeypatch):
    ...
    from derouter.proxy._types import KeyRequest, DeRouter_VerificationToken
    from derouter.proxy.management_endpoints.key_management_endpoints import (
        info_key_fn_v2,
    )
    ...
    mock_key = MagicMock(spec=DeRouter_VerificationToken)
...
async def test_info_key_fn_budget_table_fallback(monkeypatch):
    """When model_max_budget is empty on the key but set in derouter_budget_table,
...
async def test_info_key_fn_reports_budget_limits_usage(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.get_current_spend", mock_get_current_spend
    )
...
async def test_build_budget_limits_usage_pydantic_windows(monkeypatch):
    ...
    from derouter.models.team import BudgetLimitEntry
    from derouter.proxy.management_endpoints.key_management_endpoints import (
        _build_budget_limits_usage,
    )
...
async def _list_keys_capture_helper_kwargs(user_api_key_dict, **list_kwargs):
    ...
    with patch("derouter.proxy.proxy_server.prisma_client", AsyncMock()):
        with patch(
            "derouter.proxy.management_endpoints.key_management_endpoints.validate_key_list_check",
            return_value=mock_user_info,
        ):
...
async def test_cli_session_token_delegation_ceiling_blocked_by_team_budget():
    team = DeRouter_TeamTableCachedObj(team_id="team-1", max_budget=50.0)
...
async def test_budget_limits_window_cannot_exceed_caller_max_budget(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.proxy.management_endpoints.key_management_endpoints.derouter.default_key_generate_params",
        None,
        raising=False,
    )
    caller = UserAPIKeyAuth(
...
@pytest.mark.asyncio
@patch(
    "derouter.proxy.management_endpoints.key_management_endpoints.rotate_sso_identity_assertions_master_key"
)
@patch(
    "derouter.proxy.management_endpoints.key_management_endpoints.rotate_mcp_user_env_vars_master_key"
)
@patch(
    "derouter.proxy.management_endpoints.key_management_endpoints.rotate_mcp_user_credentials_master_key"
)
@patch(
    "derouter.proxy.management_endpoints.key_management_endpoints.rotate_mcp_server_credentials_master_key"
)
async def test_rotate_master_key_rotates_sso_identity_assertions(
    ...
    mock_prisma_client = AsyncMock()
    mock_prisma_client.db = MagicMock()
    mock_prisma_client.db.derouter_proxymodeltable.find_many = AsyncMock(return_value=[])
    mock_tx = AsyncMock()
    mock_tx.derouter_proxymodeltable = MagicMock()
    mock_tx.derouter_proxymodeltable.delete_many = AsyncMock()
    mock_tx.derouter_proxymodeltable.create_many = AsyncMock()
...
async def test_check_encryption_endpoint_rejects_proxy_admin_viewer():
    ...
    from derouter.proxy.management_endpoints import credential_migration as cm
    from derouter.proxy.management_endpoints.key_management_endpoints import (
    ...
    with patch("derouter.proxy.proxy_server.prisma_client", MagicMock()), patch.object(
...
def _wire_update_key_fn(monkeypatch, existing_key):
    ...
    mock_prisma_client.db.derouter_verificationtoken.find_unique = AsyncMock(return_value=existing_key)
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.premium_user", True)
    monkeypatch.setattr("derouter.store_audit_logs", False)
    monkeypatch.setattr("derouter.proxy.proxy_server.hash_token", lambda token: existing_key.token)
...
async def test_execute_virtual_key_regeneration_stamps_settings_updated_at():
    ...
    sent = mock_prisma_client.db.derouter_verificationtoken.update.call_args.kwargs["data"]
...
def _wire_key_generation_prisma(monkeypatch):
    created_key = MagicMock(token="hashed_token_123", derouter_budget_table=None, object_permission=None)
    ...
    mock_prisma_client.db.derouter_verificationtoken.find_many = AsyncMock(return_value=[])
    mock_prisma_client.db.derouter_verificationtoken.count = AsyncMock(return_value=0)
    mock_prisma_client.db.derouter_verificationtoken.update = AsyncMock(return_value=created_key)
...
async def test_key_generate_explicit_null_budget_duration_beats_default_key_generate_params(monkeypatch):
    ...
    """
    monkeypatch.setattr(derouter, "default_key_generate_params", {"budget_duration": "30d"})
...
async def test_key_generate_explicit_null_budget_duration_cannot_bypass_upperbound(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "default_key_generate_params", None)
    monkeypatch.setattr(
        derouter,
        "upperbound_key_generate_params",
        DeRouter_UpperboundKeyGenerateParams(budget_duration="30d"),
    )
    mock_insert_data = _wire_key_generation_prisma(monkeypatch)
...
async def test_key_generate_omitted_budget_duration_still_filled_by_upperbound(monkeypatch):
    ...
    assert key_row["budget_duration"] == "30d"
    assert key_row["budget_reset_at"] is not None
from derouter.proxy.management_helpers.access_group_key_sync import (
    _ATTACH_KEY_SQL,
    _DETACH_KEY_SQL,
    _REPOINT_KEY_SQL,
)
...
async def _authorized_models_for_key(access_groups, token, key_access_group_ids):
    ...
    from derouter.proxy._types import DeRouter_AccessGroupTable, DeRouter_TeamTable
...
async def test_update_key_syncs_access_group_assigned_key_ids_in_both_directions(
    ...
    with (
        ...
        patch(
            "derouter.proxy.management_helpers.access_group_key_sync._invalidate_access_group_cache",
            new_callable=AsyncMock,
        ) as invalidate_cache,
    ):
        await update_key_fn(
...
async def test_generate_key_records_token_in_its_access_groups(monkeypatch):
    ...
    created_key.derouter_budget_table = None
...
async def _cache_with_project(project_id: str, project_models: list[str]) -> UserApiKeyCache:
    ...
    await user_api_key_cache.async_set_cache(
        ...
        value=DeRouter_ProjectTableCachedObj(project_id=project_id, team_id="team-lit-5823", models=project_models),
        model_type=DeRouter_ProjectTableCachedObj,
    )
...
async def test_update_key_soft_budget_updates_existing_budget_row():
    ...
    existing_key = DeRouter_VerificationToken(token="test-token", budget_id="budget-123")
    mock_db = MagicMock()
    mock_db.derouter_budgettable.update = AsyncMock()
    mock_db.derouter_budgettable.create = AsyncMock()
    ...
    mock_db.derouter_budgettable.update.assert_awaited_once_with(
...
async def test_update_key_soft_budget_creates_budget_row_when_key_has_none():
    ...
    existing_key = DeRouter_VerificationToken(token="test-token", budget_id=None)
    created_row = MagicMock()
    created_row.budget_id = "budget-new"
    mock_db = MagicMock()
    mock_db.derouter_budgettable.create = AsyncMock(return_value=created_row)
    mock_db.derouter_budgettable.update = AsyncMock()
    ...
    mock_db.derouter_budgettable.create.assert_awaited_once_with(
...
async def test_update_key_soft_budget_noop_when_clearing_without_budget_row():
    ...
    assert result is None
    mock_db.derouter_budgettable.create.assert_not_awaited()
    mock_db.derouter_budgettable.update.assert_not_awaited()
...
async def test_update_key_row_with_soft_budget_updates_budget_and_key_in_transaction():
    ...
    tx = MagicMock()
    tx.derouter_budgettable.create = AsyncMock(return_value=created_row)
    tx.derouter_verificationtoken.update = AsyncMock(return_value=updated_row)
    ...
    tx.derouter_verificationtoken.update.assert_awaited_once()
    update_call = tx.derouter_verificationtoken.update.await_args
...
async def test_update_key_row_with_soft_budget_propagates_transaction_error():
    ...
    tx = MagicMock()
    tx.derouter_budgettable.create = AsyncMock(return_value=created_row)
    tx.derouter_verificationtoken.update = AsyncMock(side_effect=RuntimeError("update failed"))
    tx_context = MagicMock()
    tx_context.__aenter__ = AsyncMock(return_value=tx)
    tx_context.__aexit__ = AsyncMock(return_value=None)
    prisma_client = MagicMock()
...
def test_key_request_blank_organization_id_is_unset():
    from derouter.proxy._types import RegenerateKeyRequest, UpdateKeyRequest
