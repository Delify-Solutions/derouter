...
# Patching ``derouter.proxy.proxy_server.prisma_client`` imports that module, whose
# module-level setup reads DATABASE_URL and DEROUTER_MASTER_KEY. Tier-zero runners
...
_THROWAWAY_ENV = {
    ...
    "DEROUTER_MASTER_KEY": "sk-test-gateway-request-endpoints",
}
...
@pytest.fixture
def frozen_clock():
    with patch("derouter.proxy.management_endpoints.gateway_request_endpoints.datetime") as clock:
...
class TestGatewayDailyActivityEndpoint:
    ...
    async def test_refuses_every_non_admin_role(self, role):
        with patch("derouter.proxy.proxy_server.prisma_client", _prisma_returning([])):
    ...
    async def test_totals_and_breakdowns_come_from_the_same_rows(self):
        ...
        with patch("derouter.proxy.proxy_server.prisma_client", _prisma_returning(rows)):
    ...
    async def test_a_null_result_set_is_not_an_error(self):
        ...
        with patch("derouter.proxy.proxy_server.prisma_client", client):
            response = await get_gateway_daily_activity(user_api_key_dict=_admin())
        assert response.total_successful_requests == 0
