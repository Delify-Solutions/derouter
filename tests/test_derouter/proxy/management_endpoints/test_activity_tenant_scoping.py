...
async def test_team_activity_requires_admin_on_every_requested_team():
    ...
    # Mock prisma client
    prisma = MagicMock()
    prisma.db.derouter_teamtable.find_many = AsyncMock(
        return_value=[
    ...
    prisma.db.derouter_verificationtoken.find_many = AsyncMock(return_value=[user_keys])
...
async def test_agent_activity_admin_unscoped():
    ...
    from derouter.proxy.agent_endpoints import endpoints

    admin = UserAPIKeyAuth(user_id="root", user_role=LitellmUserRoles.PROXY_ADMIN.value)

    prisma = MagicMock()
    prisma.db.derouter_agentstable.find_many = AsyncMock(return_value=[])
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.prisma_client", prisma),
        patch(
            "derouter.proxy.agent_endpoints.endpoints.check_feature_access_for_user",
            new=AsyncMock(return_value=None),
        ),
        patch(
            "derouter.proxy.agent_endpoints.endpoints.get_daily_activity",
            new=AsyncMock(side_effect=_fake_get_daily_activity),
        ),
    ):
...
async def test_agent_activity_non_admin_no_perms_falls_back_to_owned():
    ...
    prisma.db.derouter_agentstable.find_many = AsyncMock(side_effect=[owned, owned])
...
async def test_agent_activity_keyless_caller_does_not_query_created_by_null():
    ...
    prisma.db.derouter_agentstable.find_many.assert_not_called()
