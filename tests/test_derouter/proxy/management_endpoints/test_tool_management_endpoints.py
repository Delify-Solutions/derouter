...
Patches target the source modules (derouter.proxy.db.tool_registry_writer.*
and derouter.proxy.proxy_server.prisma_client) because the endpoint code
...
from prisma.actions import DeRouter_TeamTableActions


from derouter.proxy.management_endpoints.tool_management_endpoints import router
from derouter.types.tool_management import DeRouter_ToolTableRow
...
) -> DeRouter_ToolTableRow:
    now = datetime.now(timezone.utc)
    return DeRouter_ToolTableRow(
        tool_id="uuid-1",
        tool_name=tool_name,
        origin=origin,
...
class FakeTeamTable:
    """Stand-in for ``prisma_client.db.derouter_teamtable``.
    ...
    async def find_unique(self, **kwargs: object) -> Optional[MagicMock]:
        inspect.signature(DeRouter_TeamTableActions.find_unique).bind(self, **kwargs)
    ...
    async def update_many(self, **kwargs: object) -> int:
        inspect.signature(DeRouter_TeamTableActions.update_many).bind(self, **kwargs)
...
def _team_policy_prisma(team_table: FakeTeamTable) -> MagicMock:
    prisma = MagicMock()
    prisma.db.derouter_teamtable = team_table
    prisma.db.derouter_objectpermissiontable.create = AsyncMock()
    prisma.db.derouter_objectpermissiontable.delete = AsyncMock()
    return prisma
...
def _rollup_prisma(group_rows: list, daily_rows: list | None = None) -> MagicMock:
    ...
    prisma.db.derouter_spendlogs.find_many = AsyncMock(return_value=[])
    prisma.db.derouter_spendlogtoolindex.find_many = AsyncMock(return_value=[])
    prisma.db.derouter_dailytoolspend.group_by = AsyncMock(return_value=group_rows)
    prisma.db.derouter_dailytoolspend.find_many = AsyncMock(return_value=daily_rows or [])
    return prisma
...
class TestToolManagementEndpoints:
    ...
    @patch(
        "derouter.proxy.db.tool_registry_writer.list_tools",
        new_callable=AsyncMock,
    )
    @patch("derouter.proxy.proxy_server.prisma_client", _MOCK_PRISMA)
    ...
    @patch(
        "derouter.proxy.db.tool_registry_writer.get_tool",
        new_callable=AsyncMock,
    )
    @patch("derouter.proxy.proxy_server.prisma_client", _MOCK_PRISMA)
    ...
    @patch(
        "derouter.proxy.db.tool_registry_writer.update_tool_policy",
        new_callable=AsyncMock,
    )
    @patch("derouter.proxy.proxy_server.prisma_client", _MOCK_PRISMA)
    ...
    @patch(
        "derouter.proxy.db.tool_registry_writer.add_tool_to_object_permission_blocked",
        new_callable=AsyncMock,
    )
    def test_update_tool_policy_for_team_does_not_500_on_unsupported_prisma_kwarg(self, mock_block):
        ...
        "DeRouter_TeamTableActions.find_unique() got an unexpected keyword argument
        ...
        with patch("derouter.proxy.proxy_server.prisma_client", _team_policy_prisma(team_table)), patch(
            "derouter.proxy.db.tool_registry_writer.get_tool_policy_registry"
        ) as mock_registry:
    ...
    def test_tool_spend_route_not_shadowed_by_get_tool(self):
        ...
        with patch("derouter.proxy.proxy_server.prisma_client", prisma):
    ...
    def test_tool_spend_daily_restricted_to_top_tools_and_capped(self):
        from derouter.constants import TOOL_SPEND_TOP_TOOLS
        ...
        with patch("derouter.proxy.proxy_server.prisma_client", prisma):
            resp = self.client.get("/v1/tool/spend?start_date=2026-07-01&end_date=2026-07-02")
        assert resp.status_code == 200
        group_kwargs = prisma.db.derouter_dailytoolspend.group_by.await_args.kwargs
        assert group_kwargs["take"] == TOOL_SPEND_TOP_TOOLS
        assert group_kwargs["order"] == {"_sum": {"spend": "desc"}}
        daily_where = prisma.db.derouter_dailytoolspend.find_many.await_args.kwargs["where"]
    ...
    def test_tool_spend_skips_daily_query_when_no_tools(self):
        ...
        with patch("derouter.proxy.proxy_server.prisma_client", prisma):
            resp = self.client.get("/v1/tool/spend?start_date=2026-07-01&end_date=2026-07-02")
        assert resp.status_code == 200
        prisma.db.derouter_dailytoolspend.find_many.assert_not_awaited()
    ...
    def test_tool_spend_reads_rollup_only_never_spendlogs(self):
        ...
        # entirely from DeRouter_DailyToolSpend; any query_raw or SpendLogs table
        ...
        prisma.db.derouter_spendlogs.find_many.assert_not_awaited()
        prisma.db.derouter_spendlogtoolindex.find_many.assert_not_awaited()
        prisma.db.derouter_dailytoolspend.group_by.assert_awaited_once()
    ...
    def test_tool_spend_windows_rollup_by_inclusive_date_strings(self):
        ...
        with patch("derouter.proxy.proxy_server.prisma_client", prisma):
            resp = self.client.get("/v1/tool/spend?start_date=2026-07-01&end_date=2026-07-02")
        assert resp.status_code == 200
        where = prisma.db.derouter_dailytoolspend.group_by.await_args.kwargs["where"]
    ...
    def test_tool_spend_malformed_date_returns_400(self, query: str):
        ...
        prisma.db.derouter_dailytoolspend.group_by.assert_not_awaited()
