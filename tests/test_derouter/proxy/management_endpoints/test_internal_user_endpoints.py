...
from derouter.proxy.management_endpoints.internal_user_endpoints import (
    DeRouter_UserTableWithKeyCount,
...
async def test_ui_view_users_with_null_email(mocker, caplog):
    ...
    mock_prisma_client.db.derouter_usertable.find_many = mock_find_many

    # Flag OFF by default
    mocker.patch(
        "derouter.proxy.ui_crud_endpoints.proxy_setting_endpoints.get_ui_settings_cached",
        return_value={},
    )

    mocker.patch("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
    ...
    assert response == [
        DeRouter_UserTableFiltered(user_id="test-user-null-email", user_email=None)
    ]
...
async def test_ui_view_users_org_admin_filtered_by_org(mocker):
    ...
    """
    from derouter.proxy._types import DeRouter_OrganizationMembershipTable
    ...
    mocker.patch("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
    mocker.patch("derouter.proxy.proxy_server.user_api_key_cache", mocker.MagicMock())
    mocker.patch("derouter.proxy.proxy_server.proxy_logging_obj", mocker.MagicMock())
    ...
    mocker.patch(
        "derouter.proxy.management_endpoints.internal_user_endpoints.get_user_object",
...
async def test_ui_view_users_flag_on_team_admin_org_team(mocker):
    ...
    mocker.patch(
        "derouter.proxy.management_endpoints.internal_user_endpoints.get_team_object",
...
async def test_get_users_includes_timestamps(mocker):
    ...
    mock_prisma_client.db.derouter_usertable.find_many = mock_find_many
    mock_prisma_client.db.derouter_usertable.count = mock_count
    ...
    mocker.patch(
        "derouter.proxy.management_endpoints.internal_user_endpoints.get_user_key_counts",
...
def test_update_internal_user_params_email():
    ...
    Test that _update_internal_user_params correctly processes an email-only update
    """
    from derouter.proxy._types import UpdateUserRequest
    from derouter.proxy.management_endpoints.internal_user_endpoints import (
        _update_internal_user_params,
    )
...
def test_update_internal_user_params_reset_spend_and_max_budget():
    """
    Relevant Issue: https://github.com/Delify-Solutions/derouter/issues/10495
    """
    from derouter.proxy._types import UpdateUserRequest
    from derouter.proxy.management_endpoints.internal_user_endpoints import (
        _update_internal_user_params,
...
async def test_new_user_rejects_a_duration_that_never_advances(mocker, bad_duration):
    ...
    mocker.patch("derouter.proxy.proxy_server.prisma_client", MagicMock())
    duplicate_check = mocker.patch(
        "derouter.proxy.management_endpoints.internal_user_endpoints._check_duplicate_user_id",
        new=AsyncMock(),
    )
...
async def test_new_user_license_over_limit(mocker):
    ...
    from derouter.proxy._types import NewUserRequest, UserAPIKeyAuth
    from derouter.proxy.management_endpoints.internal_user_endpoints import new_user
    ...
    mocker.patch(
        "derouter.proxy.management_endpoints.internal_user_endpoints._check_duplicate_user_email",
    ...
    mocker.patch("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
    mocker.patch("derouter.proxy.proxy_server._license_check", mock_license_check)
...
async def test_new_user_license_gate_counts_only_billable_users(mocker):
    ...
    """
    from derouter.proxy.auth.derouter_license import LicenseCheck
    ...
    mocker.patch("derouter.proxy.proxy_server._license_check", license_check)

    key_gen = mocker.patch(
        "derouter.proxy.management_endpoints.internal_user_endpoints.generate_key_helper_fn",
    ...
    def _prisma(total, deactivated):
        ...
        client.db.derouter_usertable.count = _count
        return client
    ...
    mocker.patch(
        "derouter.proxy.proxy_server.prisma_client", _prisma(total=5, deactivated=3)
    )
    ...
    key_gen.reset_mock()
    mocker.patch(
        "derouter.proxy.proxy_server.prisma_client", _prisma(total=3, deactivated=0)
    )
...
async def test_update_single_user_non_admin_permissions_rejected(mocker):
    ...
    mock_prisma_client = mocker.MagicMock()
    mocker.patch("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
    mocker.patch("derouter.proxy.proxy_server.derouter_proxy_admin_name", "admin")
...
async def test_user_info_url_encoding_plus_character(mocker):
    ...
    from derouter.proxy._types import DeRouter_UserTable, UserAPIKeyAuth
    from derouter.proxy.management_endpoints.internal_user_endpoints import user_info

    # Mock the prisma client
    mock_prisma_client = mocker.MagicMock()

    # Create a real DeRouter_UserTable instance (BaseModel) so isinstance check passes
...
async def test_user_info_no_user_id_view_only_admin_gets_proxy_admin_payload(mocker):
    ...
    from derouter.proxy._types import LitellmUserRoles, UserAPIKeyAuth, UserInfoResponse
    from derouter.proxy.management_endpoints.internal_user_endpoints import user_info
    ...
    mocker.patch(
        "derouter.proxy.management_endpoints.internal_user_endpoints._get_user_info_for_proxy_admin",
...
async def test_new_user_default_teams_flow(mocker):
    ...
    """
    import derouter
    from derouter.proxy._types import NewUserRequest, NewUserRequestTeam, UserAPIKeyAuth
    from derouter.proxy.management_endpoints.internal_user_endpoints import new_user
    ...
    mock_prisma_client.db.derouter_usertable.find_unique = mocker.AsyncMock(
    ...
    try:
        ...
        mocker.patch(
            "derouter.proxy.management_endpoints.internal_user_endpoints.UserManagementEventHooks.async_user_created_hook",
        ...
        # is defined in derouter/__init__.py and delattr-ing it breaks parallel tests)
...
def test_update_internal_new_user_params_proxy_admin_role():
    ...
    """
    import derouter
    from derouter.proxy._types import LitellmUserRoles, NewUserRequest
    from derouter.proxy.management_endpoints.internal_user_endpoints import (
...
def test_update_internal_new_user_params_no_role_specified():
    ...
    """
    import derouter
    from derouter.proxy._types import NewUserRequest
    from derouter.proxy.management_endpoints.internal_user_endpoints import (
        _update_internal_new_user_params,
    )
...
async def test_check_duplicate_user_email_case_insensitive(mocker):
    ...
    mock_prisma_client.db.derouter_usertable.find_first = mock_find_first_duplicate
    ...
    mock_prisma_client.db.derouter_usertable.find_first = mock_find_first_no_duplicate
...
def test_process_keys_for_user_info_filters_dashboard_keys(monkeypatch):
    """
    Test that _process_keys_for_user_info filters out keys with team_id='derouter-dashboard'

    UI session tokens (team_id='derouter-dashboard') should be excluded from user info responses
    ...
    # Mock general_settings and derouter_master_key_hash (they're imported from proxy_server)
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.general_settings",
        {},
    )
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.derouter_master_key_hash",
        "different-hash",
    )
...
def test_get_users_search_matches_user_id_or_email(mocker):
    ...
    mock_prisma_client.db.derouter_usertable.find_many = mock_find_many
    mock_prisma_client.db.derouter_usertable.count = mock_count
    mock_prisma_client.db.derouter_verificationtoken.count = mock_key_count
...
def test_generate_request_base_validator():
    ...
    """
    from derouter.proxy._types import GenerateRequestBase
...
async def test_get_user_daily_activity_non_admin_cannot_view_other_users(monkeypatch):
    ...
    with patch(
        "derouter.proxy.management_endpoints.internal_user_endpoints.get_daily_activity",
...
async def test_get_user_daily_activity_aggregated_rejects_service_account_caller(
    ...
    monkeypatch.setattr(
        "derouter.proxy.management_endpoints.internal_user_endpoints.get_daily_activity_aggregated",
        mock_get_daily_agg,
    )
...
async def test_delete_user_cleans_up_created_by_invitation_links(mocker):
    ...
    """
    from derouter.proxy._types import DeleteUserRequest, UserAPIKeyAuth
    from derouter.proxy.management_endpoints.internal_user_endpoints import delete_user
    ...
    mock_prisma_client.db.derouter_teamtable.find_many = mocker.AsyncMock(
        return_value=[]
    )

    # Mock all delete_many calls
    mock_prisma_client.db.derouter_verificationtoken.delete_many = mocker.AsyncMock(
        return_value=0
    )
    mock_prisma_client.db.derouter_invitationlink.delete_many = mocker.AsyncMock(
        return_value=1
    )
    mock_prisma_client.db.derouter_organizationmembership.delete_many = mocker.AsyncMock(
        return_value=0
    )
    mock_prisma_client.db.derouter_teammembership.delete_many = mocker.AsyncMock(
        return_value=0
    )
    mock_prisma_client.db.derouter_usertable.delete_many = mocker.AsyncMock(
        return_value=1
    )
    ...
    mock_prisma_client.db.derouter_invitationlink.delete_many.assert_called_once()
    call_kwargs = mock_prisma_client.db.derouter_invitationlink.delete_many.call_args
...
async def test_delete_user_rejects_org_admin_deleting_outside_scope(mocker):
    ...
    mock_prisma_client.db.derouter_organizationmembership.find_many = mocker.AsyncMock(
    ...
    assert (
        not hasattr(
            mock_prisma_client.db.derouter_verificationtoken.delete_many, "mock_calls"
        )
        or len(mock_prisma_client.db.derouter_verificationtoken.delete_many.mock_calls)
        == 0
    )
...
async def test_user_update_rejects_silent_create_for_non_proxy_admin(mocker):
    ...
    from derouter.proxy._types import UpdateUserRequest, UserAPIKeyAuth
    from derouter.proxy.management_endpoints.internal_user_endpoints import (
        _update_single_user_helper,
    )
    ...
    mock_prisma_client.db.derouter_usertable.find_first = mocker.AsyncMock(
        return_value=None
    )
    mocker.patch("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
...
async def test_user_info_v2_proxy_admin_can_query_any_user(mocker):
    ...
    from derouter.proxy._types import UserInfoV2Response
    from derouter.proxy.management_endpoints.internal_user_endpoints import user_info_v2
...
async def test_user_info_v2_team_admin_can_query_team_member(mocker):
    ...
    from derouter.proxy._types import DeRouter_TeamTable, UserInfoV2Response
    from derouter.proxy.management_endpoints.internal_user_endpoints import user_info_v2
...
async def test_admin_user_update_spend_invalidates_counter(mocker):
    ...
    mock_invalidate = mocker.patch(
        "derouter.proxy.proxy_server._invalidate_spend_counter",
...
async def test_resolve_user_email_metadata_maps_page_user_ids_to_email(mocker):
    ...
    mock_prisma_client.db.derouter_usertable.find_many = find_many
...
async def test_add_new_user_to_default_team_propagates_max_budget_in_team(mocker):
    ...
    from derouter.proxy._types import NewUserRequestTeam
    from derouter.proxy.management_endpoints.internal_user_endpoints import (
...
async def test_get_user_info_for_proxy_admin_validates_keys_and_teams():
    ...
    assert all(isinstance(team, DeRouter_TeamTable) for team in result.teams)
...
def _object_permission_mocks(mocker, existing_object_permission_id=None):
    ...
    mock_prisma_client.db.derouter_objectpermissiontable.find_unique = mocker.AsyncMock(
        return_value=None
    )
    mock_prisma_client.db.derouter_objectpermissiontable.upsert = mocker.AsyncMock(
        return_value=SimpleNamespace(object_permission_id="perm-new")
    )
    mock_prisma_client.db.derouter_mcpservertable.find_many = mocker.AsyncMock(return_value=[])
    mock_prisma_client.update_data = mocker.AsyncMock(
...
async def test_user_update_persists_mcp_entitlement_and_links_it(mocker):
    ...
    mocker.patch("derouter.proxy.proxy_server.user_api_key_cache", cache)
    ...
    upsert_kwargs = mock_prisma_client.db.derouter_objectpermissiontable.upsert.call_args.kwargs
...
async def test_admin_can_clear_a_users_mcp_entitlement(mocker):
    ...
    assert "object_permission" not in written
    mock_prisma_client.db.derouter_objectpermissiontable.upsert.assert_not_called()
...
async def test_new_user_persists_the_requested_mcp_entitlement(mocker):
    ...
    mock_prisma_client = mocker.MagicMock()
    mock_prisma_client.db.derouter_objectpermissiontable.create = mocker.AsyncMock(
    ...
    mock_prisma_client.db.derouter_usertable.count = mocker.AsyncMock(return_value=0)
    mocker.patch("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
    mocker.patch(
        "derouter.proxy.management_endpoints.internal_user_endpoints.check_if_default_team_set",
        return_value=None,
    )
    ...
    mocker.patch(
        "derouter.proxy.hooks.user_management_event_hooks.UserManagementEventHooks.async_user_created_hook",
        new=mocker.AsyncMock(),
    )
    ...
    created = mock_prisma_client.db.derouter_objectpermissiontable.create.call_args.kwargs["data"]
    assert json.loads(created["mcp_tool_permissions"]) == {"github": ["list_issues"]}
...
async def test_user_info_v2_returns_the_mcp_entitlement(mocker):
    ...
    mocker.patch("derouter.proxy.proxy_server.prisma_client", mocker.MagicMock())
    mocker.patch(
        "derouter.proxy.management_endpoints.internal_user_endpoints._check_user_info_v2_access",
...
async def test_user_new_persists_model_max_budget(
    ...
    """
    from derouter.proxy.management_endpoints import key_management_endpoints
...
def _admin_prisma(mocker):
    ...
    mocker.patch(  # test-quality-ok: same module-global mocking every test in this file already uses
        "derouter.proxy.proxy_server.derouter_proxy_admin_name", "admin"
    )
    return mock_prisma_client
...
async def test_user_update_rejects_weak_password(_admin_prisma):
    ...
    _admin_prisma.db.derouter_usertable.find_first.assert_not_called()
...
async def test_user_update_hashes_and_persists_strong_password(_admin_prisma, mocker):
    ...
    existing_user.user_id = "target-user"
    mock_prisma_client.db.derouter_usertable.find_first = mocker.AsyncMock(return_value=existing_user)
