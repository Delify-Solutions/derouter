...
class TestConfigFieldsDefaultTeamParams:
    ...
    def test_default_team_params_applied_from_db(self, monkeypatch):
        """default_team_params in DB is set on derouter module during config load."""
    ...
    def test_default_team_params_merged_into_config_dict(self):
        ...
        assert result["derouter_settings"]["default_team_params"] == {"max_budget": 100.0}
        # Existing keys preserved
        assert result["derouter_settings"]["cache"] is False

    def test_default_team_params_not_applied_when_absent(self, monkeypatch):
        """When DB derouter_settings has no default_team_params, it stays None."""
        monkeypatch.setattr(derouter, "default_team_params", None)
        ...
        assert derouter.default_team_params is None
    ...
    def test_default_team_params_overrides_yaml_value(self, monkeypatch):
        ...
        merged = result["derouter_settings"]["default_team_params"]
...
class TestNewTeamDefaultParamsApplied:
    """Tests that /team/new applies defaults from derouter.default_team_params."""
    ...
    def setup_mocks(self, monkeypatch):
        ...
        mock_prisma.db.derouter_teamtable = MagicMock()
    ...
    def _make_org(self, organization_id: str, max_budget: float | None = None) -> DeRouter_OrganizationTable:
        return DeRouter_OrganizationTable(
            organization_id=organization_id,
            budget_id="budget-id",
            created_by="admin-user",
            updated_by="admin-user",
            derouter_budget_table=None if max_budget is None else DeRouter_BudgetTable(max_budget=max_budget),
        )
    ...
    async def test_default_organization_applied_and_validated(self, monkeypatch):
        ...
        monkeypatch.setattr(derouter, "default_team_params", {"organization_id": "default-org"})
    ...
    async def test_nonexistent_default_organization_returns_400(self, monkeypatch):
        ...
        monkeypatch.setattr(derouter, "default_team_params", {"organization_id": "deleted-org"})
    ...
    async def test_explicit_budget_validated_against_default_org_budget(self, monkeypatch):
        ...
        without it derouter_budget_table is None and every budget comparison is skipped."""
        from derouter.proxy.management_endpoints.team_endpoints import new_team

        monkeypatch.setattr(derouter, "default_team_params", {"organization_id": "capped-org"})
...
# ---------------------------------------------------------------------------
# _update_derouter_setting: setattr ordering
# ---------------------------------------------------------------------------


class TestUpdateLitellmSettingOrdering:
    """Tests that _update_derouter_setting sets in-memory value AFTER get_config,
    so stale DB values from DEROUTER_SETTINGS_SAFE_DB_OVERRIDES don't overwrite it."""
    ...
    async def test_setattr_not_overwritten_by_get_config(self, monkeypatch):
        ...
        monkeypatch.setattr(derouter, "default_team_params", stale_value)

        # get_config will overwrite derouter.default_team_params with stale DB value
        async def mock_get_config():
            # Simulate what _update_config_from_db does for safe overrides
            derouter.default_team_params = stale_value
            return {
                "derouter_settings": {
        ...
        result = await _update_derouter_setting(
        ...
        assert derouter.default_team_params == expected

        # Saved config should contain the new value
        assert len(saved_configs) == 1
        saved_settings = saved_configs[0]["derouter_settings"]["default_team_params"]
...
# ---------------------------------------------------------------------------
# DEROUTER_SETTINGS_SAFE_DB_OVERRIDES contains default_team_params
# ---------------------------------------------------------------------------
...
class TestSafeDbOverrides:
    ...
    def test_default_team_params_in_safe_overrides(self):
        ...
        assert "default_team_params" in DEROUTER_SETTINGS_SAFE_DB_OVERRIDES

    def test_default_internal_user_params_in_safe_overrides(self):
        """Sanity: default_internal_user_params was already in the list."""
        from derouter.constants import DEROUTER_SETTINGS_SAFE_DB_OVERRIDES

        assert "default_internal_user_params" in DEROUTER_SETTINGS_SAFE_DB_OVERRIDES
...
class TestBulkUpdateTeamMemberPermissions:
    ...
    async def test_all_teams_appends_preserving_existing(self, monkeypatch):
        ...
        mock_prisma.db.derouter_teamtable.find_many = AsyncMock(return_value=[team_a, team_b])
        ...
        calls = mock_batcher.derouter_teamtable.update.call_args_list
        assert len(calls) == 2
    ...
    async def test_all_teams_skips_teams_that_already_have_permission(self, monkeypatch):
        ...
        mock_prisma = MagicMock()
        mock_prisma.db.derouter_teamtable.find_many = AsyncMock(return_value=[team_has, team_missing])
        mock_prisma.db.batch_ = MagicMock(return_value=mock_batcher)
        monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma)
    ...
    async def test_all_teams_pagination(self, monkeypatch):
        ...
        mock_prisma = MagicMock()
        mock_prisma.db.derouter_teamtable.find_many = AsyncMock(side_effect=[page1, page2])
        mock_prisma.db.batch_ = MagicMock(return_value=mock_batcher)
        monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma)
        ...
        find_calls = mock_prisma.db.derouter_teamtable.find_many.call_args_list
    ...
    async def test_team_ids_updates_only_specified_teams(self, monkeypatch):
        ...
        find_call = mock_prisma.db.derouter_teamtable.find_many.call_args
    ...
    async def test_team_ids_returns_404_for_missing_teams(self, monkeypatch):
        ...
        mock_prisma.db.derouter_teamtable.find_many = AsyncMock(return_value=[team_a])
        monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma)
    ...
    async def test_empty_permissions_list_is_noop(self, monkeypatch):
        ...
        mock_prisma.db.derouter_teamtable.find_many.assert_not_called()
