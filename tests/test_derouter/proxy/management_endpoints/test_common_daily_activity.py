...
async def test_get_daily_activity_order_has_id_tiebreaker():
    ...
    ``DeRouter_DailyUserSpend`` or ``DeRouter_DailyTeamSpend`` -- a busy
...
async def test_get_api_key_metadata_returns_active_key_metadata():
    ...
    mock_prisma.db.derouter_verificationtoken.find_many = AsyncMock(return_value=[mock_active_key])
...
async def test_get_api_key_metadata_falls_back_to_deleted_keys():
    ...
    mock_prisma.db.derouter_deletedverificationtoken.find_many = AsyncMock(return_value=[mock_deleted_key])
    ...
    mock_prisma.db.derouter_deletedverificationtoken.find_many.assert_called_once_with(
...
async def test_get_api_key_metadata_deleted_table_not_queried_when_all_keys_found():
    ...
    mock_prisma.db.derouter_verificationtoken.find_many = AsyncMock(return_value=[mock_active_key])
    mock_prisma.db.derouter_deletedverificationtoken = MagicMock()
    mock_prisma.db.derouter_deletedverificationtoken.find_many = AsyncMock(return_value=[])
    ...
    mock_prisma.db.derouter_deletedverificationtoken.find_many.assert_not_called()
...
async def test_get_api_key_metadata_deleted_table_error_handled_gracefully():
    ...
    mock_prisma.db.derouter_deletedverificationtoken.find_many = AsyncMock(side_effect=Exception("Table not found"))
...
async def test_get_api_key_metadata_regenerated_key_uses_most_recent_deleted_record():
    ...
    mock_prisma.db.derouter_deletedverificationtoken.find_many = AsyncMock(return_value=[mock_deleted_1, mock_deleted_2])
...
async def test_get_api_key_metadata_recovers_double_hashed_key_via_reverse_hash():
    ...
    assert '"DeRouter_VerificationToken"' in digest_sql
...
async def test_get_api_key_metadata_permanent_miss_never_pages_tokens_or_reads_spend_logs():
    ...
    mock_prisma.db.derouter_verificationtoken.find_many = AsyncMock(return_value=[])
    mock_prisma.db.derouter_deletedverificationtoken.find_many = AsyncMock(return_value=[])
    mock_prisma.db.derouter_usertable.find_many = AsyncMock(return_value=[])
    ...
    assert not any("DeRouter_SpendLogs" in sql for sql in issued_sql)
    token_lookups = (
        mock_prisma.db.derouter_verificationtoken.find_many.call_args_list
        + mock_prisma.db.derouter_deletedverificationtoken.find_many.call_args_list
    )
...
async def test_get_daily_activity_recovers_a_session_key_alias_from_spend_logs_around_the_page_dates():
    ...
    mock_prisma.db.derouter_dailyuserspend = mock_table
    mock_prisma.db.derouter_verificationtoken.find_many = AsyncMock(return_value=[])
    mock_prisma.db.derouter_deletedverificationtoken.find_many = AsyncMock(return_value=[])
    mock_prisma.db.derouter_usertable.find_many = AsyncMock(
...
def test_key_metadata_includes_recovered_user_email():
    from derouter.proxy.management_endpoints.common_daily_activity import _key_metadata
...
def test_update_breakdown_metrics_includes_user_email():
    from derouter.proxy.management_endpoints.common_daily_activity import update_breakdown_metrics
    from derouter.types.proxy.management_endpoints.common_daily_activity import BreakdownMetrics
...
async def test_tag_daily_activity_metadata_totals_not_zero():
    ...
    NULL.  Rows in derouter_dailytagspend are pre-aggregated and always have
    ...
    mock_prisma.db.derouter_dailytagspend = mock_table
    mock_prisma.db.derouter_verificationtoken = MagicMock()
    mock_prisma.db.derouter_verificationtoken.find_many = AsyncMock(return_value=[])
...
def _daily_user_spend_record(*, user_id, api_key, spend, model="gpt-4", model_group="gpt-4"):
    """A DeRouter_DailyUserSpend row as the per-user breakdown reads it."""
...
async def test_model_groups_breakdown_keys_by_public_name_with_model_fallback():
    ...
    from the breakdown. The models breakdown keeps the upstream derouter names.
    """
    mock_prisma = MagicMock()
    mock_prisma.db = MagicMock()
...
def test_ptu_sentinel_excluded_from_key_breakdown_but_flat_cost_aggregates(ptu_cost_attribution_enabled):
    from derouter.constants import PTU_SENTINEL_API_KEY
    from derouter.proxy.management_endpoints.common_daily_activity import update_breakdown_metrics
    from derouter.types.proxy.management_endpoints.common_daily_activity import BreakdownMetrics
...
def _grouping_row(
    ...
    *,
    ...
    from derouter.proxy.management_endpoints.common_daily_activity import _GroupingSetsRow
...
def test_grouping_sets_dispatcher_keeps_an_all_zero_legacy_provider_bucket():
    """DeRouter_DailyTeamSpend predates its api_requests column; the migration that added it
...
def _daily_team_row(api_key, *, spend=0.0, ptu_flat_cost=0.0):
    """A DeRouter_DailyTeamSpend row as the paginated read path receives it from find_many."""
...
class TestPtuCostAttributionDisabled:
    """With DEROUTER_ENABLE_PTU_COST_ATTRIBUTION unset, both read paths report zero flat
    ...
    sentinel rows, then disable it, and those rows stay in DeRouter_DailyTeamSpend
    ...
    def test_aggregated_path_reports_zero_flat_cost(self):
        from derouter.proxy.management_endpoints.common_daily_activity import _GROUP_GRAND_TOTAL
    ...
    async def test_team_daily_activity_endpoint_reports_zero_flat_cost(self):
        ...
        mock_prisma.db.derouter_verificationtoken = MagicMock()
        mock_prisma.db.derouter_verificationtoken.find_many = AsyncMock(return_value=[])
        mock_prisma.db.derouter_dailyteamspend = mock_table
...
class TestFlagIsNotReadOnTheHotPath:
    ...
    def _count_flag_reads(records):
        import derouter.proxy.management_endpoints.common_daily_activity as cda
        from derouter.types.proxy.management_endpoints.common_daily_activity import BreakdownMetrics
...
def test_spend_logs_window_pads_min_minus_one_day_and_max_plus_two_days():
    from derouter.proxy.management_endpoints.common_daily_activity import _spend_logs_window
