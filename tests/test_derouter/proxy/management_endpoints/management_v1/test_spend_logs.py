...
from derouter.proxy._types import DeRouterRoutes, LitellmUserRoles
...
from derouter.proxy.management_endpoints.management_v1 import router
...
def mock_prisma_client(monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", prisma_client)
...
def test_reads_spend_logs_not_the_end_user_table(mock_prisma_client, as_proxy_admin):
    ...
    assert '"DeRouter_SpendLogs"' in sql
    assert "DeRouter_EndUserTable" not in sql
...
def test_caps_the_rows_it_scans(mock_prisma_client, as_proxy_admin):
    ...
    from derouter.proxy.management_endpoints.management_v1.spend_logs import (
...
def test_problem_type_is_an_identifier_not_a_dead_docs_link(mock_prisma_client, as_proxy_admin):
    ...
    human-readable documentation at that address, and https://router.delify.vn/errors/
    ...
    assert "router.delify.vn" not in problem_type
...
def test_is_reachable_by_every_role_that_can_open_the_logs_page(role):
    ...
    for facet_path in (END_USERS_PATH, USERS_PATH):
        for allowed in (
            DeRouterRoutes.internal_user_routes.value,
            DeRouterRoutes.internal_user_view_only_routes.value,
        ):
        ...
        if role in (LitellmUserRoles.INTERNAL_USER, LitellmUserRoles.INTERNAL_USER_VIEW_ONLY):
            allowed_routes = (
                ...
                else DeRouterRoutes.internal_user_view_only_routes.value
            )
            assert RouteChecks.check_route_access(route=facet_path, allowed_routes=allowed_routes)
        else:
            assert facet_path in DeRouterRoutes.admin_viewer_routes.value
