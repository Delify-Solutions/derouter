...
from derouter.proxy.list_api.list_framework import (
    Compare,
    ScopeWhere,
    build_query_plan,
)
from derouter.proxy.management_endpoints.management_v1 import router
from derouter.proxy.management_endpoints.management_v1.budgets import (
    BUDGETS_LIST_SPEC,
    BudgetListItem,
)
...
def test_the_spec_serves_what_the_row_model_declares():
    ...
    from derouter.proxy.management_endpoints.management_v1.budgets import SELECTED_COLUMNS
...
def test_is_reachable_by_the_roles_that_can_open_the_budgets_page():
    ...
    assert BUDGETS_PATH in DeRouterRoutes.admin_viewer_routes.value
    assert ("/budget/list" in DeRouterRoutes.admin_viewer_routes.value) == (
        BUDGETS_PATH in DeRouterRoutes.admin_viewer_routes.value
    )
