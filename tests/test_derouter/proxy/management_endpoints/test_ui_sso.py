...
from derouter.proxy._types import DeRouter_UserTable, NewUserResponse
from derouter.proxy.auth.handle_jwt import JWTHandler
from derouter.proxy.management_endpoints.sso import CustomMicrosoftSSO
from derouter.proxy.management_endpoints.types import CustomOpenID
...
def test_microsoft_sso_handler_openid_from_response_with_custom_attributes():
    ...
    # Act
    with (
        patch("derouter.constants.MICROSOFT_USER_EMAIL_ATTRIBUTE", "custom_email_field"),
        patch(
            "derouter.constants.MICROSOFT_USER_DISPLAY_NAME_ATTRIBUTE",
            "custom_display_name",
        ),
        patch("derouter.constants.MICROSOFT_USER_ID_ATTRIBUTE", "custom_id_field"),
        patch(
            "derouter.constants.MICROSOFT_USER_FIRST_NAME_ATTRIBUTE", "custom_first_name"
        ),
        patch(
            "derouter.constants.MICROSOFT_USER_LAST_NAME_ATTRIBUTE", "custom_last_name"
        ),
        patch(
            "derouter.proxy.management_endpoints.ui_sso.MICROSOFT_USER_EMAIL_ATTRIBUTE",
            "custom_email_field",
        ),
        patch(
            "derouter.proxy.management_endpoints.ui_sso.MICROSOFT_USER_DISPLAY_NAME_ATTRIBUTE",
            "custom_display_name",
        ),
        patch(
            "derouter.proxy.management_endpoints.ui_sso.MICROSOFT_USER_ID_ATTRIBUTE",
            "custom_id_field",
        ),
        patch(
            "derouter.proxy.management_endpoints.ui_sso.MICROSOFT_USER_FIRST_NAME_ATTRIBUTE",
            "custom_first_name",
        ),
        patch(
            "derouter.proxy.management_endpoints.ui_sso.MICROSOFT_USER_LAST_NAME_ATTRIBUTE",
            "custom_last_name",
        ),
    ):
        result = MicrosoftSSOHandler.openid_from_response(
...
async def test_get_user_groups_from_graph_api():
    ...
    with patch(
        "derouter.proxy.management_endpoints.ui_sso.get_async_httpx_client"
    ) as mock_client:
...
async def test_default_team_params(team_params):
    """
    When derouter.default_team_params is set, it should be used to create a new team
    """
    # Arrange
    derouter.default_team_params = team_params
    ...
    mock_prisma.db.derouter_teamtable.find_first = AsyncMock(return_value=None)
    ...
    with patch("derouter.proxy.proxy_server.prisma_client", mock_prisma):
        # Act
        team_id = str(uuid.uuid4())
        await MicrosoftSSOHandler.create_derouter_teams_from_service_principal_team_ids(
        ...
        mock_prisma.db.derouter_teamtable.create.assert_called_once()
        create_call_args = mock_prisma.db.derouter_teamtable.create.call_args.kwargs[
            "data"
        ]
...
async def test_default_team_params_organization_id_reaches_sso_created_team(team_params):
    ...
    mock_org = DeRouter_OrganizationTable(
    ...
    with patch("derouter.proxy.proxy_server.prisma_client", mock_prisma), patch(
        ...
        mock_prisma.db.derouter_teamtable.create.assert_called_once()
        create_call_args = mock_prisma.db.derouter_teamtable.create.call_args.kwargs["data"]
...
async def test_create_team_without_default_params():
    """
    Test team creation when derouter.default_team_params is None
    Should create team with just the basic required fields
    """
    # Arrange
    derouter.default_team_params = None
...
def test_apply_user_info_values_to_sso_user_defined_values():
    from derouter.proxy._types import DeRouter_UserTable, SSOUserDefinedValues
    from derouter.proxy.management_endpoints.ui_sso import (
...
def test_build_sso_user_update_data_with_valid_role():
    ...
    from derouter.proxy.management_endpoints.types import CustomOpenID
    from derouter.proxy.management_endpoints.ui_sso import _build_sso_user_update_data
...
def test_generic_response_convertor_normalizes_email():
    ...
    """
    from derouter.proxy.management_endpoints.ui_sso import generic_response_convertor
...
async def test_upsert_sso_user_updates_role_for_existing_user():
    ...
    updated in the DeRouter database on subsequent logins, not just at initial user creation.
    """
    from derouter.proxy._types import DeRouter_UserTable, LitellmUserRoles
    from derouter.proxy.management_endpoints.types import CustomOpenID
    from derouter.proxy.management_endpoints.ui_sso import SSOAuthenticationHandler
    ...
    existing_user = DeRouter_UserTable(
        user_id="test-user-123",
        user_email="test@example.com",
    ...
    call_args = mock_prisma.db.derouter_usertable.update_many.call_args
...
async def test_upsert_sso_user_does_not_update_invalid_role():
    ...
    If the SSO returns a role that is not a valid DeRouter role, it should be ignored
...
async def test_get_user_info_from_db_user_exists():
    ...
    """
    from derouter.proxy.management_endpoints.ui_sso import get_user_info_from_db
    ...
    with patch(
        "derouter.proxy.management_endpoints.ui_sso.get_user_object"
...
async def test_get_user_info_from_db_user_not_exists_creates_user():
    ...
    """
    from derouter.proxy._types import NewUserResponse, SSOUserDefinedValues
    from derouter.proxy.management_endpoints.ui_sso import get_user_info_from_db
    ...
    with (
        patch(
            "derouter.proxy.management_endpoints.ui_sso.get_existing_user_info_from_db",
            return_value=None,  # User doesn't exist
        ) as mock_get_existing,
        patch(
            "derouter.proxy.management_endpoints.ui_sso.SSOAuthenticationHandler.upsert_sso_user",
            return_value=mock_new_user,
        ) as mock_upsert,
        patch(
            "derouter.proxy.management_endpoints.ui_sso.SSOAuthenticationHandler.add_user_to_teams_from_sso_response",
        ) as mock_add_teams,
    ):
        # Act
...
async def test_get_user_info_from_db_user_exists_updates_user():
    ...
    updated_user = DeRouter_UserTable(
        user_id="existing-user-456",
...
async def test_check_and_update_if_proxy_admin_id():
    ...
    # Mock Prisma client
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_usertable.update = AsyncMock()
    ...
    with patch.dict(os.environ, {"PROXY_ADMIN_ID": test_user_id}):
        ...
        mock_prisma.db.derouter_usertable.update.assert_called_once_with(
...
async def test_check_and_update_if_proxy_admin_id_already_admin():
    ...
    with patch.dict(os.environ, {"PROXY_ADMIN_ID": test_user_id}):
        ...
        # Assert
        assert updated_role == LitellmUserRoles.PROXY_ADMIN.value
        mock_prisma.db.derouter_usertable.update.assert_not_called()
...
async def test_get_generic_sso_response_with_additional_headers():
    ...
    """
    from derouter.proxy.management_endpoints.ui_sso import get_generic_sso_response
...
def test_merge_sso_token_claims_precedence_and_invalid_tokens():
    ...
    from derouter.proxy.management_endpoints.ui_sso import _merge_sso_token_claims
...
async def test_get_generic_sso_response_pkce_merges_token_claims_and_excludes_credentials(monkeypatch):
    ...
    request = StarletteRequest(
        {
            ...
            "headers": [(b"cookie", b"derouter_oauth_state=test-state")],
        }
    )
    ...
    with (
        patch("derouter.proxy.proxy_server.redis_usage_cache", None),
        patch("derouter.proxy.proxy_server.user_api_key_cache", pkce_cache),
        patch(
            "derouter.proxy.management_endpoints.ui_sso.get_async_httpx_client",
...
class TestAuthCallbackRouting:
    ...
    def test_cli_state_detection_and_routing(self):
        ...
        from derouter.constants import DEROUTER_CLI_SESSION_TOKEN_PREFIX

        # Test CLI state detection logic
        cli_state = f"{DEROUTER_CLI_SESSION_TOKEN_PREFIX}:cli-test1234567890"

        # This mimics the logic in auth_callback
        if cli_state and cli_state.startswith(f"{DEROUTER_CLI_SESSION_TOKEN_PREFIX}:"):
    ...
    def test_non_cli_state_routing(self):
        ...
        for state in non_cli_states:
            ...
            should_route_to_cli = state and state.startswith(
                f"{DEROUTER_CLI_SESSION_TOKEN_PREFIX}:"
            )
...
class TestGoogleLoginCLIIntegration:
    ...
    def test_google_login_cli_state_generation(self):
        ...
        assert cli_state.startswith("derouter-session-token:")
...
class TestSSOHandlerIntegration:
    ...
    def test_get_redirect_url_for_sso(self):
        ...
        mock_request.base_url = "https://test.router.delify.vn/"
        ...
        assert redirect_url.startswith("https://test.router.delify.vn")
...
class TestSSOStateHandling:
    ...
    def test_get_cli_state_valid(self):
        ...
        state = SSOAuthenticationHandler._get_cli_state(
            source="derouter-cli", key="cli-test1234567890"
        )

        assert state is not None
        assert state.startswith("derouter-session-token:")
    ...
    def test_get_cli_state_no_key(self):
        ...
        state = SSOAuthenticationHandler._get_cli_state(source="derouter-cli", key=None)
    ...
    def test_get_cli_state_ignores_existing_key(self):
        ...
        assert state == "derouter-session-token:cli-new-key-1234567890"
    ...
    def test_get_cli_state_without_existing_key(self):
        ...
        state = SSOAuthenticationHandler._get_cli_state(
            source="derouter-cli", key="cli-new-key-789123456", existing_key=None
        )

        assert state is not None
        assert state.startswith("derouter-session-token:")
        assert "cli-new-key-789123456" in state
        assert state == "derouter-session-token:cli-new-key-789123456"
...
class TestStateRouting:
    ...
    def test_cli_state_detection(self):
        ...
        cli_state = f"{DEROUTER_CLI_SESSION_TOKEN_PREFIX}:cli-test1234567890"
        assert cli_state.startswith(f"{DEROUTER_CLI_SESSION_TOKEN_PREFIX}:")
    ...
    def test_cli_state_parsing_uses_single_login_id(self):
        ...
        cli_state = f"{DEROUTER_CLI_SESSION_TOKEN_PREFIX}:cli-new-key-456123"
    ...
    def test_cli_state_parsing_without_extra_segments(self):
        ...
        cli_state = f"{DEROUTER_CLI_SESSION_TOKEN_PREFIX}:cli-new-key-999123"
    ...
    def test_non_cli_state_detection(self):
        ...
        for state in test_states:
            if state:
                assert not state.startswith(f"{DEROUTER_CLI_SESSION_TOKEN_PREFIX}:")
            else:
                assert state != f"{DEROUTER_CLI_SESSION_TOKEN_PREFIX}:"
...
class TestHTMLIntegration:
    ...
    def test_html_render_utils_import(self):
        ...
        from derouter.proxy.common_utils.html_forms.cli_sso_success import (
...
class TestCustomUISSO:
    ...
    def test_enterprise_import_error_handling(self):
        ...
        with patch("derouter.proxy.proxy_server.premium_user", True):
            with patch(
                "derouter.proxy.proxy_server.user_custom_ui_sso_sign_in_handler",
                MagicMock(),
            ):
                with patch.dict(
                    "sys.modules",
                    {"derouter_enterprise.proxy.auth.custom_sso_handler": None},
                ):
                    ...
                    async def mock_google_login():
                        ...
                        try:
                            from derouter_enterprise.proxy.auth.custom_sso_handler import (  # noqa: F401
                            ...
                            raise ValueError(
                                "Enterprise features are not available. Custom UI SSO sign-in requires DeRouter Enterprise."
                            )
    ...
    async def test_handle_custom_ui_sso_sign_in_success(self):
        ...
        from derouter_enterprise.proxy.auth.custom_sso_handler import (
            EnterpriseCustomSSOHandler,
        )

        from derouter.integrations.custom_sso_handler import CustomSSOLoginHandler

        # Mock request with custom headers
        mock_request = MagicMock(spec=Request)
        mock_request.headers = {
            "x-derouter-user-id": "test_user_123",
            "x-derouter-user-email": "test@example.com",
    ...
    async def test_handle_custom_ui_sso_sign_in_rejects_untrusted_proxy(self):
        ...
        mock_request = MagicMock(spec=Request)
        mock_request.headers = {
            "x-derouter-user-id": "admin",
            "x-derouter-user-email": "admin@example.com",
        }
        mock_request.base_url = "https://test.router.delify.vn/"
    ...
    async def test_custom_ui_sso_handler_execution_with_real_class(self):
        ...
        class TestCustomSSOHandler(CustomSSOLoginHandler):
            ...
            async def handle_custom_ui_sso_sign_in(self, request: Request) -> OpenID:
                ...
                return OpenID(
                    id=request_headers_dict.get("x-derouter-user-id", "default_user"),
                    email=request_headers_dict.get(
                        "x-derouter-user-email", "default@test.com"
                    ),
                    first_name="Custom",
                    last_name="Handler",
        ...
        mock_request = MagicMock(spec=Request)
        mock_request.headers = {
            "x-derouter-user-id": "custom_test_user_456",
            "x-derouter-user-email": "custom@example.com",
            "x-forwarded-for": "10.0.0.1",
        }
        mock_request.base_url = "https://custom.router.delify.vn/"
...
class TestCLIKeyRegenerationFlow:
    ...
    async def test_cli_sso_start_creates_bound_flow(self):
        ...
        with (
            patch("derouter.proxy.proxy_server.user_api_key_cache", mock_cache),
            patch("derouter.proxy.proxy_server.cli_sso_session_cache", mock_cache),
        ):
    ...
    async def test_cli_sso_start_rate_limits_by_client_ip(self):
        ...
        from derouter.proxy.management_endpoints.ui_sso import cli_sso_start
    ...
    async def test_cli_sso_start_returns_verification_uri_complete_when_enabled(self):
        ...
        from derouter.constants import DEROUTER_CLI_SOURCE_IDENTIFIER
        from derouter.proxy.management_endpoints.ui_sso import cli_sso_start
        ...
        assert query["source"] == [DEROUTER_CLI_SOURCE_IDENTIFIER]
    ...
    def test_get_cli_state_appends_user_code_for_prefill(self):
        ...
        from derouter.constants import (
            DEROUTER_CLI_SESSION_TOKEN_PREFIX,
            DEROUTER_CLI_SOURCE_IDENTIFIER,
        )
        from derouter.proxy.management_endpoints.ui_sso import SSOAuthenticationHandler

        manual_state = SSOAuthenticationHandler._get_cli_state(
            source=DEROUTER_CLI_SOURCE_IDENTIFIER, key="cli-abc123"
        )
        prefill_state = SSOAuthenticationHandler._get_cli_state(
            source=DEROUTER_CLI_SOURCE_IDENTIFIER,
            key="cli-abc123",
            user_code="WXYZ-2345",
        )

        assert manual_state == f"{DEROUTER_CLI_SESSION_TOKEN_PREFIX}:cli-abc123"
        assert (
            prefill_state == f"{DEROUTER_CLI_SESSION_TOKEN_PREFIX}:cli-abc123:WXYZ-2345"
        )
        assert (
    ...
    def test_get_cli_state_drops_malformed_user_code(self):
        ...
        manual_only = f"{DEROUTER_CLI_SESSION_TOKEN_PREFIX}:cli-abc123"
    ...
    async def test_cli_sso_callback_prefills_user_code_on_verify_page(self):
        ...
        with (
            ...
            patch(
                "derouter.proxy.management_endpoints.ui_sso.get_user_info_from_db",
    ...
    async def test_cli_sso_complete_verifies_user_code(self):
        ...
        with (
            ...
            patch("derouter.proxy.proxy_server.cli_sso_session_cache", mock_cache),
            patch(
                "derouter.proxy.common_utils.html_forms.cli_sso_success.render_cli_sso_success_page",
    ...
    async def test_cli_sso_callback_stores_session(self):
        ...
        mock_prisma = MagicMock()
        mock_prisma.db.derouter_teamtable.find_many = AsyncMock(
            return_value=[
                MagicMock(
    ...
    async def test_cli_poll_key_returns_teams_for_selection(self):
        ...
        with (
            ...
            result = await cli_poll_key(
                key_id=session_key,
                team_id=None,
                x_derouter_cli_poll_secret="poll-secret",
            )
    ...
    async def test_auth_callback_routes_to_cli(self):
        ...
        cli_state = f"{DEROUTER_CLI_SESSION_TOKEN_PREFIX}:cli-new-session-key-456"
        ...
        with (
            patch(
                "derouter.proxy.management_endpoints.ui_sso.cli_sso_callback"
            ) as mock_cli_callback,
            patch("derouter.proxy.proxy_server.prisma_client", MagicMock()),
            patch("derouter.proxy.proxy_server.master_key", "test-master-key"),
            patch("derouter.proxy.proxy_server.general_settings", {}),
            patch("derouter.proxy.proxy_server.jwt_handler", MagicMock()),
            ...
            patch(
                "derouter.proxy.management_endpoints.ui_sso.GoogleSSOHandler.get_google_callback_response",
    ...
    async def test_auth_callback_forwards_prefill_user_code_from_state(self):
        ...
        mock_request.query_params = {"code": "some-auth-code"}
        cli_state = (
            f"{DEROUTER_CLI_SESSION_TOKEN_PREFIX}:cli-new-session-key-456:WXYZ-2345"
        )
        mock_result = {"user_id": "test-user", "email": "test@example.com"}
    ...
    def test_get_redirect_url_does_not_include_existing_key_in_url(self):
        ...
        with patch(
            "derouter.proxy.utils.get_custom_url", return_value="https://test.router.delify.vn"
        ):
            ...
            assert "https://test.router.delify.vn/sso/callback" == redirect_url
    ...
    async def test_cli_poll_key_generates_jwt_with_team(self):
        ...
        with (
            patch("derouter.proxy.proxy_server.user_api_key_cache", mock_cache),
            patch("derouter.proxy.proxy_server.cli_sso_session_cache", mock_cache),
            patch("derouter.proxy.proxy_server.prisma_client"),
            patch(
                "derouter.proxy.auth.auth_checks.ExperimentalUIJWTToken.get_cli_jwt_auth_token",
                return_value=mock_jwt_token,
            ) as mock_get_jwt,
            patch(
                "derouter.proxy.auth.auth_checks.get_user_object",
                new=AsyncMock(return_value=mock_user_info),
            ),
            patch(
                "derouter.proxy.auth.auth_checks.get_team_object",
    ...
    async def test_fetch_cli_sso_team_details_projects_team_grants(self):
        ...
        prisma_client = MagicMock()
        prisma_client.db.derouter_teamtable.find_many = find_many

        details = await fetch_cli_sso_team_details(
            prisma_client=prisma_client, teams=["team-a"]
        )

        assert find_many.await_args.kwargs["include"] == {"derouter_model_table": True}
    ...
    async def test_fetch_cli_sso_team_details_separates_lookup_failure_from_no_teams(self):
        ...
        failing_client.db.derouter_teamtable.find_many = AsyncMock(
        ...
        empty_client.db.derouter_teamtable.find_many = AsyncMock(return_value=[])
        assert (
            await fetch_cli_sso_team_details(
...
async def test_get_ui_settings_includes_api_doc_base_url():
    ...
    from derouter.proxy.management_endpoints.ui_sso import get_ui_settings
    ...
    with patch.dict(
        os.environ,
        {
            "DEROUTER_UI_API_DOC_BASE_URL": "https://custom.docs",
        },
    ):
        response = await get_ui_settings(mock_request)
        assert response["DEROUTER_UI_API_DOC_BASE_URL"] == "https://custom.docs"
...
class TestGenericResponseConvertorUserRole:
    ...
    def test_generic_response_convertor_extracts_valid_user_role(self):
        """
        Test that generic_response_convertor extracts a valid DeRouter user role
...
class TestGetGenericSSORedirectParams:
    ...
    def test_state_priority_cli_state_provided(self):
        ...
        # Arrange
        cli_state = "derouter-session-token:cli-test1234567890"
...
class TestPKCEFunctionality:
    ...
    async def test_pkce_redis_multi_pod_verifier_roundtrip(self):
        ...
        with patch.dict(os.environ, {"GENERIC_CLIENT_USE_PKCE": "true"}):
            with patch("derouter.proxy.proxy_server.redis_usage_cache", mock_redis):
                with patch(
                    "derouter.proxy.proxy_server.user_api_key_cache", mock_in_memory
                ):
    ...
    async def test_pkce_fallback_in_memory_roundtrip_when_redis_none(self):
        ...
        with patch.dict(os.environ, {"GENERIC_CLIENT_USE_PKCE": "true"}):
            with patch("derouter.proxy.proxy_server.redis_usage_cache", None):
                with patch(
                    "derouter.proxy.proxy_server.user_api_key_cache", mock_in_memory
                ):
    ...
    async def test_pkce_prepare_token_exchange_returns_nothing_when_no_state(self):
        ...
        with (
            patch("derouter.proxy.proxy_server.redis_usage_cache", mock_redis),
            patch("derouter.proxy.proxy_server.user_api_key_cache", mock_in_memory),
    ...
    async def test_delete_pkce_verifier_swallows_deletion_errors(self):
        ...
        with (
            patch("derouter.proxy.proxy_server.redis_usage_cache", None),
            patch("derouter.proxy.proxy_server.user_api_key_cache", failing_cache),
        ):
...
class TestAddMissingTeamMember:
    ...
    async def test_add_missing_team_member_with_new_user_response_teams_none(self):
        ...
        """
        from derouter.proxy._types import NewUserResponse
        from derouter.proxy.management_endpoints.ui_sso import add_missing_team_member
        ...
        with patch(
            "derouter.proxy.management_endpoints.ui_sso.create_team_member_add_task"
        ) as mock_add_task:
    ...
    @pytest.mark.asyncio
    async def test_add_missing_team_member_with_derouter_user_table_empty_teams(self):
        """
        Control test: When a DeRouter_UserTable has teams=[] (existing user, no teams),
        add_missing_team_member() should add the user to SSO teams.

        This test PASSES because DeRouter_UserTable defaults teams to [] not None.
        """
        from derouter.proxy._types import DeRouter_UserTable
        from derouter.proxy.management_endpoints.ui_sso import add_missing_team_member
    ...
    async def test_add_user_to_teams_from_sso_response_new_user(self):
        ...
        with patch(
            "derouter.proxy.management_endpoints.ui_sso.add_missing_team_member"
        ) as mock_add_member:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # Working case: DeRouter_UserTable with teams=[]
            pytest.param(
                lambda uid: DeRouter_UserTable(user_id=uid, teams=[]),
                [],
                ["team-1", "team-2"],
            ...
            pytest.param(
                lambda uid: DeRouter_UserTable(user_id=uid, teams=["team-1"]),
                ["team-1"],
...
async def test_role_mappings_override_default_internal_user_params():
    ...
    """
    from derouter.proxy._types import NewUserResponse, SSOUserDefinedValues
    from derouter.proxy.management_endpoints.ui_sso import insert_sso_user

    # Save original default_internal_user_params
    original_default_params = getattr(derouter, "default_internal_user_params", None)
    ...
    try:
        ...
        with patch(
            "derouter.proxy.management_endpoints.ui_sso.new_user",
        ...
        # the attribute is defined in derouter/__init__.py and delattr-ing it breaks parallel tests)
        derouter.default_internal_user_params = original_default_params
...
async def test_sso_role_preserved_without_role_mappings():
    ...
    GENERIC_USER_ROLE_ATTRIBUTE (not from DeRouter's role_mappings feature).
    ...
    try:
        ...
        # derouter.__getattr__ has no handler for this name)
        derouter.default_internal_user_params = original_default_params
...
async def test_setup_team_mappings():
    ...
    mock_prisma.db.derouter_ssoconfig.find_unique = AsyncMock(
        return_value=mock_sso_config
    )

    with patch(
        "derouter.proxy.utils.get_prisma_client_or_throw",
        ...
        mock_prisma.db.derouter_ssoconfig.find_unique.assert_called_once_with(
            where={"id": "sso_config"}
        )


# ============================================================================
# Tests for get_derouter_user_role with list inputs (Keycloak returns lists)
# ============================================================================


def test_get_derouter_user_role_with_string():
    """Test that get_derouter_user_role works with a plain string."""
    from derouter.proxy._types import LitellmUserRoles
    from derouter.proxy.management_endpoints.types import get_derouter_user_role

    result = get_derouter_user_role("proxy_admin")
    assert result == LitellmUserRoles.PROXY_ADMIN


def test_get_derouter_user_role_with_list():
    """
    Test that get_derouter_user_role handles list inputs.
...
def test_get_derouter_user_role_with_empty_list():
    """Test that get_derouter_user_role returns None for empty lists."""
    from derouter.proxy.management_endpoints.types import get_derouter_user_role

    result = get_derouter_user_role([])
    assert result is None


def test_get_derouter_user_role_with_invalid_role():
    """Test that get_derouter_user_role returns None for invalid roles."""
    from derouter.proxy.management_endpoints.types import get_derouter_user_role

    result = get_derouter_user_role("not_a_real_role")
    assert result is None
...
def test_get_derouter_user_role_picks_highest_privilege_regardless_of_order(role_claim):
    """A multi-valued role claim resolves to the most privileged role, not the first one listed."""
    from derouter.proxy._types import LitellmUserRoles
    from derouter.proxy.management_endpoints.types import get_derouter_user_role

    assert get_derouter_user_role(role_claim) == LitellmUserRoles.PROXY_ADMIN
...
def test_get_derouter_user_role_keeps_org_spend_visibility_for_mixed_roles(role_claim):
    """
    ...
    assert get_derouter_user_role(role_claim) == LitellmUserRoles.PROXY_ADMIN_VIEW_ONLY


def test_get_derouter_user_role_ignores_unrecognised_entries():
    """Roles DeRouter does not know about are skipped rather than swallowing the whole claim."""
    from derouter.proxy._types import LitellmUserRoles
    from derouter.proxy.management_endpoints.types import get_derouter_user_role

    assert get_derouter_user_role(["some_idp_group", "internal_user"]) == LitellmUserRoles.INTERNAL_USER
    assert get_derouter_user_role(["some_idp_group", "another_group"]) is None


def test_get_derouter_user_role_list_lookup_is_case_insensitive():
    from derouter.proxy._types import LitellmUserRoles
    from derouter.proxy.management_endpoints.types import get_derouter_user_role

    assert get_derouter_user_role(["INTERNAL_USER", "Proxy_Admin"]) == LitellmUserRoles.PROXY_ADMIN
...
def test_get_derouter_user_role_is_deterministic_for_unranked_roles(role_claim):
    """Roles outside the privilege hierarchy still resolve the same way in either claim order."""
    from derouter.proxy._types import LitellmUserRoles
    from derouter.proxy.management_endpoints.types import get_derouter_user_role

    assert get_derouter_user_role(role_claim) == LitellmUserRoles.ORG_ADMIN
...
def test_get_derouter_user_role_prefers_a_ranked_role_over_an_unranked_one(role_claim):
    """
    ...
    assert get_derouter_user_role(role_claim) == LitellmUserRoles.INTERNAL_USER


def test_get_derouter_user_role_returns_none_for_non_string_claims():
    from derouter.proxy.management_endpoints.types import get_derouter_user_role

    assert get_derouter_user_role(None) is None
    assert get_derouter_user_role({"role": "proxy_admin"}) is None
...
def test_process_sso_jwt_access_token_extracts_role_from_access_token():
    ...
    # Call with GENERIC_USER_ROLE_ATTRIBUTE pointing to derouter_role
    with patch.dict(os.environ, {"GENERIC_USER_ROLE_ATTRIBUTE": "derouter_role"}):
        process_sso_jwt_access_token(
...
def test_process_sso_jwt_access_token_resolves_highest_privilege_role(role_claim):
    ...
    access_token_str = pyjwt.encode(
        {"sub": "user-123", "email": "mixed@test.com", "derouter_role": role_claim},
        "secret",
        algorithm="HS256",
    )
    result = CustomOpenID(
...
def test_process_sso_jwt_access_token_with_role_mappings():
    ...
    from derouter.proxy._types import LitellmUserRoles
    from derouter.types.proxy.management_endpoints.ui_sso import RoleMappings
...
class TestCliSsoAttributionMetadata:
    ...
    def test_parse_cli_sso_claim_map(self, monkeypatch):
        from derouter.proxy.management_endpoints import ui_sso
    ...
    async def test_cli_sso_callback_passes_user_defined_values_for_new_users(self):
        ...
        with (
            ...
            patch("derouter.proxy.proxy_server.user_api_key_cache", mock_cache),
            patch("derouter.proxy.proxy_server.cli_sso_session_cache", mock_cache),
            patch("derouter.proxy.proxy_server.user_custom_sso", None),
        ):
    ...
    async def test_cli_sso_callback_persists_attribution_metadata(self, monkeypatch):
        ...
        mock_prisma = MagicMock()
        mock_prisma.db.derouter_usertable.find_unique = AsyncMock(
        ...
        mock_prisma.db.derouter_usertable.update_many.assert_awaited_once()
        update_data = mock_prisma.db.derouter_usertable.update_many.call_args.kwargs[
            "data"
        ]
...
class TestSyncUserRoleFromJwtRoleMap:
    ...
    def _make_jwt_handler():
        ...
        handler.update_environment(
            ...
            derouter_jwtauth=DeRouter_JWTAuth(
                ...
                jwt_derouter_role_map=[
                    JWTDeRouterRoleMap(
                        jwt_role="my-admin",
                        derouter_role=LitellmUserRoles.PROXY_ADMIN,
                    ),
                    JWTDeRouterRoleMap(
                        jwt_role="my-viewer",
                        derouter_role=LitellmUserRoles.INTERNAL_USER,
                    ),
                ],
            ),
        )
    ...
    def _make_sso_values(user_role=None):
        from derouter.proxy._types import SSOUserDefinedValues
    ...
    async def test_existing_user_role_updated_in_db_and_cache(self):
        ...
        cache = DualCache()
        prisma = AsyncMock()
        prisma.db.derouter_usertable.update = AsyncMock()
        user_id = "testuser@example.com"
        ...
        prisma.db.derouter_usertable.update.assert_called_once_with(
    ...
    async def test_same_role_no_db_write(self):
        ...
        prisma.db.derouter_usertable.update.assert_not_called()
...
class TestPKCEStateCookieBinding:
    ...
    access token.  Fix: set ``derouter_oauth_state`` HttpOnly cookie on
    ...
    async def test_redirect_response_sets_oauth_state_cookie_when_pkce_enabled(self):
        ...
        ``derouter_oauth_state`` on the redirect response when PKCE is on so
        ...
        cookie_str = next(
            (c for c in cookie_headers if "derouter_oauth_state=" in c), None
        )
        assert (
            cookie_str is not None
        ), f"derouter_oauth_state cookie not set; got: {cookie_headers}"
    ...
    async def test_redirect_response_omits_oauth_state_cookie_when_pkce_disabled(
        ...
        cookie_headers = response.headers.getlist("set-cookie")
        assert not any(
            "derouter_oauth_state=" in c for c in cookie_headers
        ), f"derouter_oauth_state cookie set on non-PKCE flow; got: {cookie_headers}"
    ...
    async def test_redirect_response_sets_secure_flag_behind_trusted_tls_terminating_proxy(
        ...
        """Regression: derouter sees a plain-HTTP hop when TLS terminates at a reverse
    ...
    async def test_pkce_callback_rejects_missing_cookie(self):
        ...
        callback must reject a request that has no ``derouter_oauth_state``
    ...
    async def test_pkce_callback_rejects_state_cookie_mismatch(self):
        ...
        mock_request.cookies = {"derouter_oauth_state": "victim-browser-state"}
    ...
    async def test_pkce_callback_accepts_matching_state_cookie(self):
        ...
        mock_request.cookies = {"derouter_oauth_state": "matched-state"}
...
async def test_debug_sso_callback_renders_full_jwt_claims():
    ...
    """
    from derouter.proxy.management_endpoints.ui_sso import debug_sso_callback
    ...
    with (
        ...
        patch(
            "derouter.proxy.management_endpoints.ui_sso.get_generic_sso_response",
            side_effect=fake_get_generic_sso_response,
        ),
        patch("derouter.proxy.proxy_server.general_settings", {}),
        patch("derouter.proxy.proxy_server.prisma_client", MagicMock()),
        patch("derouter.proxy.proxy_server.user_api_key_cache", MagicMock()),
        patch("derouter.proxy.proxy_server.jwt_handler", MagicMock(spec=JWTHandler)),
    ):
    ...
    assert payload["raw_claims"]["roles"] == ["derouter.api.user"]
    ...
    assert payload["access_token_claims"]["groups"] == ["derouter-users"]
...
async def _render_debug_page(provider_env, id_jag_registered, force_inert=False):
    ...
    from derouter.proxy.management_endpoints.ui_sso import GoogleSSOHandler, debug_sso_callback
    ...
    stack = [
        ...
        patch(  # test-quality-ok: endpoint test stubs the upstream generic IdP boundary
            "derouter.proxy.management_endpoints.ui_sso.get_generic_sso_response", side_effect=fake_generic
        ),
        ...
        patch(  # test-quality-ok: debug endpoint reads this module global without an injection seam
            "derouter.proxy.management_endpoints.ui_sso.ema_assertion_retention_enabled",
            AsyncMock(return_value=id_jag_registered),
        ),
        patch("derouter.proxy.proxy_server.general_settings", {}),  # test-quality-ok: debug endpoint reads proxy globals
        patch("derouter.proxy.proxy_server.prisma_client", MagicMock()),  # test-quality-ok: debug endpoint reads proxy DB
        patch("derouter.proxy.proxy_server.user_api_key_cache", MagicMock()),  # test-quality-ok: debug endpoint reads proxy globals
        patch("derouter.proxy.proxy_server.jwt_handler", MagicMock(spec=JWTHandler)),  # test-quality-ok: debug endpoint reads proxy globals
    ]
    if force_inert:
        stack.append(
            patch(  # test-quality-ok: force-inert reference isolates the endpoint's pre-change response
                "derouter.proxy.management_endpoints.ui_sso.warn_if_id_jag_capture_gap",
...
async def test_debug_page_survives_a_store_outage(monkeypatch, caplog):
    ...
    not the page."""
    from derouter.proxy.management_endpoints.ui_sso import warn_if_id_jag_capture_gap
...
async def _render_legacy_login_page(env_overrides, general_settings):
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.general_settings", general_settings),
        patch("derouter.proxy.proxy_server.user_api_key_cache", MagicMock()),
        patch("derouter.proxy.proxy_server.user_custom_ui_sso_sign_in_handler", None),
    ):
        ...
        for var in (
            ...
            "GOOGLE_CLIENT_ID",
            "GENERIC_CLIENT_ID",
            "DEROUTER_HIDE_DEFAULT_CREDENTIALS_HINT",
        ):
            os.environ.pop(var, None)
...
async def test_legacy_login_page_hides_credentials_hint_via_env_flag():
    ...
    DEROUTER_HIDE_DEFAULT_CREDENTIALS_HINT is set. The legacy server-rendered
    page previously ignored this flag while the new UI honored it.
    """
    response = await _render_legacy_login_page(
        env_overrides={"DEROUTER_HIDE_DEFAULT_CREDENTIALS_HINT": "true"},
        general_settings={},
    )
...
async def test_saml_callback_blocked_when_admin_ui_disabled():
    ...
    from derouter.proxy.management_endpoints.ui_sso import saml_callback
...
async def test_saml_callback_enforces_free_sso_user_limit_after_validation():
    ...
    async def _fake_handle_acs(**kwargs):
        ...
        return CustomOpenID(
            id="dana@router.delify.vn",
            email="dana@router.delify.vn",
            first_name=None,
            last_name=None,
            display_name="dana",
            picture=None,
    ...
    with patch.dict(os.environ, {"DISABLE_ADMIN_UI": "false"}), patch(
        "derouter.proxy.proxy_server.premium_user", False
    ), patch("derouter.proxy.proxy_server.prisma_client", MagicMock()), patch(
        "derouter.proxy.proxy_server.master_key", "sk-1234"
    ), patch(
        "derouter.proxy.management_endpoints.sso.saml_sso.SAMLAuthHandler.handle_acs",
        new=_fake_handle_acs,
    ), patch(
        "derouter.repositories.user_repository.UserRepository.count_billable_users",
...
async def test_redirect_from_openid_persists_assertion_under_canonical_user_id():
    ...
    from derouter.proxy._experimental.mcp_server.outbound_credentials.sso_assertion_store import (
    ...
    with (
        patch("derouter.proxy.utils.get_prisma_client_or_throw", return_value=MagicMock()),
        patch("derouter.proxy.proxy_server.master_key", "sk-master"),
        patch("derouter.proxy.proxy_server.general_settings", {}),
        patch("derouter.proxy.proxy_server.premium_user", False),
        patch("derouter.proxy.proxy_server.user_custom_sso", None),
        ...
        patch(
            "derouter.proxy.management_endpoints.ui_sso.check_and_update_if_proxy_admin_id",
            AsyncMock(return_value="internal_user"),
        ),
        patch(
            "derouter.proxy.management_endpoints.ui_sso.retain_sso_identity_assertion_for_ema",
            retain_mock,
        ),
    ):
...
async def test_cli_completion_persists_assertion_under_db_user_id():
    ...
    with (
        ...
        patch(
            "derouter.proxy.management_endpoints.ui_sso.fetch_cli_sso_team_details",
            AsyncMock(return_value=[]),
        ),
        patch(
            "derouter.proxy.management_endpoints.ui_sso.build_cli_sso_attribution_metadata",
            return_value={},
        ),
        patch(
            "derouter.proxy.management_endpoints.ui_sso.retain_sso_identity_assertion_for_ema",
...
async def test_browser_funnel_reports_an_uncaptured_assertion(monkeypatch, caplog):
    ...
    with (
        patch(  # test-quality-ok: endpoint test stubs the Prisma client lookup
            "derouter.proxy.utils.get_prisma_client_or_throw", return_value=MagicMock()
        ),
        patch("derouter.proxy.proxy_server.master_key", "sk-master"),  # test-quality-ok: endpoint reads proxy globals
        patch("derouter.proxy.proxy_server.general_settings", {}),  # test-quality-ok: endpoint reads proxy globals
        patch("derouter.proxy.proxy_server.premium_user", False),  # test-quality-ok: endpoint reads proxy globals
        patch("derouter.proxy.proxy_server.user_custom_sso", None),  # test-quality-ok: endpoint reads proxy globals
        patch("derouter.proxy.proxy_server.proxy_logging_obj", MagicMock()),  # test-quality-ok: endpoint reads proxy globals
        patch("derouter.proxy.proxy_server.redis_usage_cache", None),  # test-quality-ok: endpoint reads proxy globals
        patch("derouter.proxy.proxy_server.user_api_key_cache", MagicMock()),  # test-quality-ok: endpoint reads proxy globals
...
class TestSameOriginReturnPath:
    ...
    def test_accepts_relative_paths(self):
        from derouter.proxy.management_endpoints.ui_sso import _is_same_origin_return_path
...
class TestPersistReturnToCookieSharedHelper:
    ...
    def test_sets_cookie_for_same_origin_relative_path(self, monkeypatch):
        ...
        from derouter.proxy.management_endpoints.ui_sso import _persist_return_to_cookie

        monkeypatch.setattr("derouter.proxy.proxy_server.general_settings", {})
        resp = Response()
        _persist_return_to_cookie(resp, "/mcp/authorize?client_id=llm_dcrc_abc", _make_https_request())
        assert "derouter_cp_return_to=" in self._cookie(resp)
    ...
    def test_bad_absolute_with_control_plane_configured_does_not_raise_and_is_not_stored(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "derouter.proxy.proxy_server.general_settings", {"control_plane_url": "https://cp.example.com"}
        )
        resp = Response()
        _persist_return_to_cookie(resp, "https://evil.example.com/steal", _make_https_request())  # must not raise
        assert "derouter_cp_return_to=" not in self._cookie(resp)
    ...
    def test_cookie_is_secure_behind_trusted_tls_terminating_proxy(self, monkeypatch):
        """Regression for the reported bug: TLS terminates at a reverse proxy, derouter only
...
class TestSessionTokenCookie:
    ...
    def test_secure_behind_trusted_tls_terminating_proxy(self, monkeypatch):
        """THE regression: TLS terminates at a reverse proxy, derouter only sees a
    ...
    def test_proxy_base_url_https_overrides_literal_http_scheme(self, monkeypatch):
        ...
        monkeypatch.setenv("PROXY_BASE_URL", "https://derouter.example.com")
        resp = Response()
        set_session_token_cookie(resp, _make_http_request(), "jwt-token-value")
        assert "Secure" in self._cookie(resp)
