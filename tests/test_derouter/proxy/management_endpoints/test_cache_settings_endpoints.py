...
from derouter.types.management_endpoints.cache_settings_endpoints import (
...
async def test_test_cache_connection_calls_cache_test_connection_with_params():
    ...
    # Patch Cache class at the import location (derouter module)
    with patch("derouter.Cache") as mock_cache_class:
...
async def test_update_cache_settings_persists_url_precedence(monkeypatch):
    ...
    mock_prisma.db.derouter_cacheconfig.find_unique = AsyncMock(return_value=None)
    mock_prisma.db.derouter_cacheconfig.upsert = AsyncMock()
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.proxy_config", proxy_config),
...
async def test_get_cache_settings_masks_password_bearing_url():
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_cacheconfig.find_unique = AsyncMock(return_value=cache_row)
...
class TestCacheSettingsManager:
    ...
    async def test_init_cache_settings_in_db_initializes_when_params_changed(self):
        ...
        mock_prisma_client.db.derouter_cacheconfig.find_unique = AsyncMock(return_value=mock_cache_config)
    ...
    async def test_init_cache_settings_in_db_retries_on_transport_error(self):
        ...
        mock_prisma_client = MagicMock()
        mock_prisma_client.db.derouter_cacheconfig.find_unique = AsyncMock(side_effect=_flaky_find_unique)
...
async def test_update_preserves_stored_password_on_redacted_resubmit(monkeypatch):
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_cacheconfig.find_unique = AsyncMock(return_value=existing)
    mock_prisma.db.derouter_cacheconfig.upsert = AsyncMock()
...
async def test_test_cache_connection_survives_saved_lookup_failure(monkeypatch):
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client", bad_prisma),
        patch("derouter.proxy.proxy_server.proxy_config", proxy_config),
        patch("derouter.Cache") as mock_cache_class,
    ):
...
async def test_test_cache_connection_does_not_log_plaintext_credentials(monkeypatch, caplog):
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.proxy_config", proxy_config),
        patch("derouter.Cache") as mock_cache_class,
        caplog.at_level(logging.DEBUG, logger="DeRouter Proxy"),
    ):
        mock_cache_class.return_value = cache_instance
