...
def client_and_mocks(monkeypatch):
    ...
    async def mock_tx():
        tx = types.SimpleNamespace(
            derouter_accessgrouptable=mock_access_group_table,
            derouter_teamtable=mock_team_table,
            derouter_verificationtoken=mock_key_table,
        )
        yield tx
    ...
    mock_db = types.SimpleNamespace(
        ...
        derouter_teamtable=mock_team_table,
        derouter_verificationtoken=mock_key_table,
        derouter_mcpservertable=mock_mcp_server_table,
        derouter_agentstable=mock_agents_table,
        tx=mock_tx,
    )
...
def test_create_access_group_success(client_and_mocks, base_path, payload):
    ...
    mock_prisma.db.derouter_teamtable.find_many = AsyncMock(return_value=[_make_team_record("team-1")])
...
def test_list_access_groups_success_empty(client_and_mocks, base_path):
    ...
    mock_prisma.db.derouter_teamtable.find_many.assert_not_awaited()
...
def test_list_access_groups_attributes_teams_per_group_with_one_query(client_and_mocks, base_path):
    ...
    client, mock_prisma, mock_table, *_ = client_and_mocks
    mock_team_table = mock_prisma.db.derouter_teamtable
...
def test_get_access_group_empty_column_and_no_teams_returns_empty(client_and_mocks):
    ...
    mock_prisma.db.derouter_teamtable.find_many = AsyncMock(return_value=[])
...
def test_delete_access_group_cleans_up_teams_and_keys(client_and_mocks):
    ...
    mock_team_table = mock_prisma.db.derouter_teamtable
    mock_key_table = mock_prisma.db.derouter_verificationtoken
...
def test_delete_access_group_patches_cached_team_and_key(
    ...
    if expected_team_ids_after is not None:
        ...
        if isinstance(written_team, DeRouter_TeamTableCachedObj):
...
def test_record_to_access_group_table():
    """Test _record_to_access_group_table converts Prisma-like record to DeRouter_AccessGroupTable."""
    from derouter.proxy.management_endpoints.access_group_endpoints import (
...
def _mock_resource_tables(mock_prisma, *, mcp_servers=(), agents=(), teams=(), keys=()):
    mock_prisma.db.derouter_mcpservertable.find_many = AsyncMock(return_value=list(mcp_servers))
    mock_prisma.db.derouter_agentstable.find_many = AsyncMock(return_value=list(agents))
    mock_prisma.db.derouter_teamtable.find_many = AsyncMock(return_value=list(teams))
    mock_prisma.db.derouter_verificationtoken.find_many = AsyncMock(return_value=list(keys))
...
def test_get_access_group_resolves_resource_names(client_and_mocks, base_path):
    ...
    mcp_where = mock_prisma.db.derouter_mcpservertable.find_many.call_args.kwargs["where"]
    assert sorted(mcp_where["server_id"]["in"]) == ["mcp-a", "mcp-b", "mcp-ghost"]
    agent_where = mock_prisma.db.derouter_agentstable.find_many.call_args.kwargs["where"]
    assert sorted(agent_where["agent_id"]["in"]) == ["agent-a", "agent-ghost"]
    key_where = mock_prisma.db.derouter_verificationtoken.find_many.call_args.kwargs["where"]
...
def test_list_access_groups_resolves_names_with_one_query_per_table(client_and_mocks):
    ...
    for table, column in (
        (mock_prisma.db.derouter_mcpservertable, "server_id"),
        (mock_prisma.db.derouter_agentstable, "agent_id"),
        (mock_prisma.db.derouter_verificationtoken, "token"),
    ):
...
def test_list_access_groups_skips_lookups_when_nothing_to_resolve(client_and_mocks):
    ...
    mock_prisma.db.derouter_mcpservertable.find_many.assert_not_awaited()
    mock_prisma.db.derouter_agentstable.find_many.assert_not_awaited()
    mock_prisma.db.derouter_verificationtoken.find_many.assert_not_awaited()
...
def test_create_access_group_response_carries_resolved_names(client_and_mocks):
    ...
    mock_prisma.db.derouter_teamtable.find_unique = AsyncMock(return_value=team_record)
    _mock_resource_tables(
        mock_prisma,
