...
from derouter.proxy._types import LitellmTableNames, LitellmUserRoles
...
from derouter.types.management_endpoints.coordination_redis_endpoints import (
...
def _prisma_with_general_settings(general_settings: dict | None) -> MagicMock:
    """A prisma client whose DeRouter_Config `general_settings` row holds ``general_settings``."""
    row = None
    ...
    mock_prisma.db.derouter_config.find_unique = AsyncMock(return_value=row)
...
async def test_get_redacts_every_credential_field():
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.proxy_config", _proxy_config()),
    ):
...
async def test_get_source_is_coordination_redis_when_block_present(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "cache", MagicMock(cache=MagicMock(spec=RedisCache)))
...
async def test_get_source_reads_block_from_yaml_config_when_db_row_absent(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "cache", None)
    monkeypatch.delenv("REDIS_HOST", raising=False)

    with (
        patch("derouter.proxy.proxy_server.prisma_client", _prisma_with_general_settings(None)),
        patch(
...
async def test_get_source_is_cache_backend_when_no_block(monkeypatch, cache_backend_cls):
    ...
    monkeypatch.setattr(derouter, "cache", MagicMock(cache=MagicMock(spec=cache_backend_cls)))
    monkeypatch.setenv("REDIS_HOST", "env-redis")

    with (
        patch("derouter.proxy.proxy_server.prisma_client", _prisma_with_general_settings({})),
        patch("derouter.proxy.proxy_server.proxy_config", _proxy_config()),
    ):
        response = await get_coordination_redis_settings(user_api_key_dict=_admin_auth())
...
async def test_get_source_is_environment_when_no_block_and_non_redis_cache(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "cache", MagicMock(cache=MagicMock()))
    monkeypatch.setenv("REDIS_HOST", "env-redis")
...
async def test_get_source_does_not_build_a_client(monkeypatch):
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client", _prisma_with_general_settings({})),
        patch("derouter.proxy.proxy_server.proxy_config", _proxy_config()),
        patch("derouter.proxy.proxy_server._build_redis_usage_cache") as mock_build,
    ):
        response = await get_coordination_redis_settings(user_api_key_dict=_admin_auth())
...
def test_fields_cover_every_coordination_redis_param():
    ...
    from derouter.proxy._types import CoordinationRedisParams
...
async def test_update_persists_into_the_general_settings_config_row(monkeypatch):
    """Settings land under `general_settings.coordination_redis` in DeRouter_Config
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.store_model_in_db", True),
        patch(
            "derouter.proxy.management_endpoints.coordination_redis_endpoints.invalidate_config_param",
    ...
    upsert_kwargs = mock_prisma.db.derouter_config.upsert.call_args.kwargs
...
async def test_update_persists_os_environ_refs_verbatim(monkeypatch):
    ...
    persisted = json.loads(mock_prisma.db.derouter_config.upsert.call_args.kwargs["data"]["update"]["param_value"])
...
async def test_update_emits_audit_log_with_values_redacted(monkeypatch):
    ...
    with (
        ...
        patch("derouter.proxy.management_helpers.audit_logs.create_audit_log_for_update", new=capture),
    ):
        await update_coordination_redis_settings(
            request=CoordinationRedisSettingsRequest(
...
async def test_connection_test_returns_healthy_on_successful_ping():
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client", _prisma_with_general_settings({})),
        patch("derouter.proxy.proxy_server.proxy_config", _proxy_config()),
        patch("derouter.proxy.proxy_server._build_redis_usage_cache", return_value=mock_client) as mock_build,
    ):
...
async def test_connection_test_reports_unhealthy_without_leaking_the_password():
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client", _prisma_with_general_settings({})),
        patch("derouter.proxy.proxy_server.proxy_config", _proxy_config()),
        patch("derouter.proxy.proxy_server._build_redis_usage_cache", return_value=mock_client),
    ):
        response = await check_coordination_redis_connection(
            request=CoordinationRedisSettingsRequest(
                settings={
...
async def test_connection_test_times_out_instead_of_hanging():
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server._build_redis_usage_cache", return_value=mock_client),
        patch(
            "derouter.proxy.management_endpoints.coordination_redis_endpoints._PING_TIMEOUT_SECONDS",
            0.01,
        ),
    ):
        response = await check_coordination_redis_connection(
