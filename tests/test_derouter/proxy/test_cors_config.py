...
def test_cors_wildcard_disables_credentials():
    """should disable credentials when DEROUTER_CORS_ORIGINS is not set (defaults to wildcard)."""
    from derouter.proxy.proxy_server import _get_cors_config
...
def test_cors_empty_string_disables_credentials():
    """should disable credentials when DEROUTER_CORS_ORIGINS is empty or whitespace."""
    from derouter.proxy.proxy_server import _get_cors_config
...
def test_cors_wildcard_string_in_env_disables_credentials():
    """should disable credentials when DEROUTER_CORS_ORIGINS is explicitly set to '*'."""
    from derouter.proxy.proxy_server import _get_cors_config
...
def test_cors_explicit_credentials_true_overrides_wildcard():
    """should enable credentials when DEROUTER_CORS_ALLOW_CREDENTIALS=true even
...
def test_cors_explicit_credentials_false_overrides_specific_origins():
    """should disable credentials when DEROUTER_CORS_ALLOW_CREDENTIALS=false even
...
def test_cors_explicit_credentials_case_insensitive():
    """should accept TRUE/FALSE case-insensitively for DEROUTER_CORS_ALLOW_CREDENTIALS."""
    from derouter.proxy.proxy_server import _get_cors_config
...
def test_proxy_server_cors_invariant():
    ...
    if os.getenv("DEROUTER_CORS_ALLOW_CREDENTIALS") is None:
