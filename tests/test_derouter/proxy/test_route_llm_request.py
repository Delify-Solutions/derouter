...
from derouter.proxy.route_llm_request import ProxyModelNotFoundError, route_request
...
async def test_route_request_proxy_admin_can_call_all_team_scoped_deployments_without_team_id():
    ...
    with pytest.raises(derouter.BadRequestError, match="multiple teams"):
...
async def test_route_request_no_model_required_with_router_settings_and_no_router():
    ...
    with patch.object(derouter, "acompletion", return_value="fake_response") as mock_completion:
...
async def test_route_request_with_router_settings_override_no_router():
    ...
    falling back to derouter module directly.
    """
    import derouter
    ...
    original_acompletion = derouter.acompletion
    derouter.acompletion = mock_completion

    try:
        response = await route_request(data, None, None, "acompletion")

        assert response == "success"
        # Verify derouter.acompletion was called with merged settings
        call_kwargs = mock_completion.call_args[1]
        assert call_kwargs["fallbacks"] == [{"gpt-3.5-turbo": ["gpt-4"]}]
        assert call_kwargs["num_retries"] == 3
    finally:
        derouter.acompletion = original_acompletion
...
def test_gated_mock_params_cover_mock_router_testing_params():
    ...
    against ``derouter.types.router``. This test guards against drift — if a
    ...
    from derouter.proxy.route_llm_request import GATED_MOCK_PARAM_NAMES
...
def test_mock_params_pass_through_when_allowed(mock_param):
    ...
    from derouter.proxy.route_llm_request import raise_if_mock_testing_params_disallowed
...
async def test_route_request_realtime_wildcard_model_resolves_credentials(
    ...
    with patch(
        "derouter.realtime_api.main.base_llm_http_handler.async_realtime_client_secret_handler",
...
@pytest.mark.asyncio
async def test_route_request_realtime_derouter_credential_name_resolves_api_key(
    openai_realtime_credential,
    monkeypatch,
):
    """
    derouter_credential_name on a wildcard deployment must resolve to the stored
        ...
        model_list=[
            {
                ...
                "derouter_params": {
                    "model": "openai/*",
                    "derouter_credential_name": "openai-realtime-cred",
                },
            }
        ]
    )
...
async def test_route_request_realtime_unresolvable_model_raises_not_found(
    ...
    """
    An unknown model must not silently fall through to derouter with an empty
    OPENAI_API_KEY env var.
    """
    import derouter
    from unittest.mock import AsyncMock, patch

    from derouter.proxy.route_llm_request import ProxyModelNotFoundError
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "openai/gpt-4", "api_key": "other-key"},
            }
        ]
    )
    with patch(
...
async def test_route_request_realtime_calls_resolves_api_base(monkeypatch):
    ...
    with patch(
        "derouter.realtime_api.main.base_llm_http_handler.async_realtime_calls_handler",
        new_callable=AsyncMock,
    ) as mock_handler:
...
async def test_route_request_realtime_transcription_session_resolves_credentials(monkeypatch):
    ...
    with patch(
        "derouter.realtime_api.main.base_llm_http_handler.async_realtime_transcription_session_handler",
        new_callable=AsyncMock,
    ) as mock_handler:
...
async def test_route_request_rejects_chat_completion_without_messages():
    ...
    from derouter.proxy.route_llm_request import ProxyMissingRequiredParamError
...
def _fake_prisma_client_with_models(rows):
    ...
    return SimpleNamespace(db=SimpleNamespace(derouter_proxymodeltable=table)), table
...
def _db_model_row(model_name: str, mock_response: str):
    ...
    return SimpleNamespace(
        ...
        derouter_params={"model": "openai/gpt-4o", "api_key": "fake", "mock_response": mock_response},
        model_info={},
        blocked=False,
    )
...
async def test_route_request_routing_group_name_passes_model_gate():
    ...
    router = Router(
        model_list=[
            {"model_name": "member-a", "derouter_params": {"model": "openai/gpt-4o", "api_key": "sk-test"}},
            {"model_name": "member-b", "derouter_params": {"model": "openai/gpt-4o-mini", "api_key": "sk-test"}},
        ],
        routing_groups=[
...
async def test_route_request_a2a_agent_miss_does_not_consume_model_read_through(monkeypatch):
    ...
    fake_prisma.db.derouter_agentstable = SimpleNamespace(find_unique=agents_find_unique)
    monkeypatch.setattr(proxy_server, "prisma_client", fake_prisma)
    monkeypatch.setattr(proxy_server, "store_model_in_db", True)
    monkeypatch.setattr(proxy_server, "llm_router", router)
