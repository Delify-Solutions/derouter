...
from derouter.proxy.prompts.prompt_registry import InMemoryPromptRegistry, parse_prompt_version
...
def test_resolution_picks_exact_version_and_latest_within_an_environment(isolated_callbacks: list) -> None:
    ...
    assert exact.derouter_params.prompt_data["content"] == "begin every reply with AHOY"

    latest = registry.resolve_prompt_spec("greeting", environment="development")
    assert latest is not None
    assert latest.derouter_params.prompt_data["content"] == "begin every reply with YO"
...
def test_delete_prompts_by_base_id_removes_the_callbacks_from_derouter_callbacks(isolated_callbacks: list) -> None:
    registry = InMemoryPromptRegistry()
