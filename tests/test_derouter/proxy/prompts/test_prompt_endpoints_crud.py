...
from derouter.types.prompts.init_prompts import (
    PromptSpec,
    PromptDeRouterParams,
    PromptInfo,
)
...
async def test_delete_prompt_success():
    ...
    """
    from derouter.proxy.prompts.prompt_endpoints import delete_prompt
    ...
    mock_prisma_client.db.derouter_prompttable.delete_many = AsyncMock(return_value=None)

    # Mock In-Memory Registry
    with patch(
        "derouter.proxy.prompts.prompt_registry.IN_MEMORY_PROMPT_REGISTRY"
    ) as mock_registry:
        ...
        with patch("derouter.proxy.proxy_server.prisma_client", mock_prisma_client):
            ...
            mock_prisma_client.db.derouter_prompttable.delete_many.assert_called_once_with(
...
async def test_delete_prompt_environment_scope_reaches_db_and_registry():
    ...
    with patch(  # test-quality-ok: stubs the collaborator so the test pins what the endpoint deletes
        ...
        mock_registry.resolve_prompt_spec.return_value = PromptSpec(
            prompt_id="test_prompt.v2",
            derouter_params=PromptDeRouterParams(prompt_id="test_prompt", prompt_integration="dotprompt"),
            prompt_info=PromptInfo(prompt_type="db", environment="production"),
        )
        mock_registry.has_config_prompt.return_value = False

        with patch("derouter.proxy.proxy_server.prisma_client", mock_prisma_client):  # test-quality-ok: proxy_server module global is the endpoint's only injection point
            response = await delete_prompt(
                prompt_id="test_prompt.v2",
...
async def test_get_prompt_info_by_base_id():
    ...
    """
    from derouter.proxy.prompts.prompt_endpoints import get_prompt_info
...
async def test_patch_prompt_row_deleted_mid_update_returns_404():
    ...
    from derouter.proxy.prompts.prompt_endpoints import PatchPromptRequest, patch_prompt
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_prompttable.find_many = AsyncMock(
        return_value=[target_row]
    )
    mock_prisma_client.db.derouter_prompttable.update = AsyncMock(return_value=None)
...
async def test_patch_prompt_merges_unsent_fields_from_db_row_not_stale_memory():
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_prompttable.find_many = AsyncMock(return_value=[db_row])
    mock_prisma_client.db.derouter_prompttable.update = AsyncMock(return_value=db_row)
    ...
    written_params = json.loads(mock_prisma_client.db.derouter_prompttable.update.call_args.kwargs["data"]["derouter_params"])
    ...
    assert reloaded_spec.derouter_params.prompt_data["content"] == "Begin every reply with HOWDY"
    assert response.derouter_params.prompt_data["content"] == "Begin every reply with HOWDY"


def test_is_ambiguous_keyed_prompt_data_shapes():
    from derouter.proxy.prompts.prompt_endpoints import is_ambiguous_keyed_prompt_data

    keyed_with_id = PromptDeRouterParams(
        prompt_id="agent-prompt",
        prompt_integration="dotprompt",
        prompt_data={"json_prompt": {"content": "AHOY", "metadata": {}}},
    )
    flat_with_id = PromptDeRouterParams(
        prompt_id="agent-prompt",
        prompt_integration="dotprompt",
        prompt_data={"content": "AHOY", "metadata": {}},
    )
    keyed_without_id = PromptDeRouterParams(
        prompt_integration="dotprompt",
        prompt_data={"json_prompt": {"content": "AHOY", "metadata": {}}},
    )
    no_prompt_data = PromptDeRouterParams(
        prompt_id="agent-prompt", prompt_integration="dotprompt"
    )
    empty_prompt_data = PromptDeRouterParams(
...
async def test_create_prompt_rejects_keyed_prompt_data_with_prompt_id():
    ...
    from derouter.proxy.prompts.prompt_endpoints import (
    ...
    with patch("derouter.proxy.proxy_server.prisma_client", MagicMock()):  # test-quality-ok: proxy_server module global is the endpoint's only injection point
        with pytest.raises(HTTPException) as exc_info:
...
async def test_patch_prompt_info_only_keeps_legacy_keyed_row_patchable():
    ...
    legacy_params = PromptDeRouterParams(
        prompt_id="agent-prompt",
        prompt_integration="dotprompt",
        prompt_data={"json_prompt": {"content": "AHOY", "metadata": {}}},
    )
    ...
    target_row.model_dump.return_value = {
        ...
        "environment": "production",
        "created_by": None,
        "derouter_params": legacy_params.model_dump_json(),
    ...
    mock_prisma_client.db.derouter_prompttable.update = AsyncMock(return_value=updated_row)

    existing_prompt = PromptSpec(
        prompt_id="agent-prompt.v1",
        derouter_params=legacy_params,
        prompt_info=PromptInfo(prompt_type="db"),
    )
    ...
    update_kwargs = mock_prisma_client.db.derouter_prompttable.update.await_args.kwargs
    assert update_kwargs["where"] == {"id": "row-1"}
    assert json.loads(update_kwargs["data"]["prompt_info"])["environment"] == "production"
    assert json.loads(update_kwargs["data"]["derouter_params"])["prompt_data"] == {
...
def test_initialize_prompt_keeps_version_and_created_by():
    import derouter
    from derouter.proxy.prompts.prompt_registry import InMemoryPromptRegistry
    ...
    with patch.object(derouter.logging_callback_manager, "add_derouter_callback"):  # test-quality-ok: keeps initialize_prompt from registering a global callback that would leak across tests
