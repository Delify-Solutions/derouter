...
from derouter.proxy.db.db_transaction_queue.spend_log_cleanup_metrics import (
...
async def test_cleanup_old_spend_logs_batch_deletion():
    ...
    assert 'DELETE FROM "DeRouter_SpendLogs"' in call_args_sql
    ...
    # reusing x-derouter-call-id take out a fresh row alongside the expired one
    ...
    assert 'DELETE FROM "DeRouter_SpendLogToolIndex"' in tool_index_sql

    # The DeRouter_DailyToolSpend rollup must outlive spend-log retention: it is
    ...
    for call in mock_db.execute_raw.call_args_list:
        assert "DeRouter_DailyToolSpend" not in call[0][0]
...
async def test_cleanup_drops_partitions_when_enabled_and_partitioned():
    ...
    partition_manager.drop_partitions_older_than = AsyncMock(
        return_value=["DeRouter_SpendLogs_p20260601"]
    )
    ...
    assert 'DELETE FROM "DeRouter_SpendLogs"' in delete_sql
    ...
    assert any('DELETE FROM "DeRouter_SpendLogToolIndex"' in s for s in all_sql)
...
async def test_delete_old_tool_index_rows_deletes_on_composite_key():
    ...
    assert 'DELETE FROM "DeRouter_SpendLogToolIndex"' in delete_sql
...
async def test_delete_old_logs_continues_after_single_batch_failure(monkeypatch):
    ...
    import derouter.proxy.db.db_transaction_queue.spend_log_cleanup as cleanup_module
...
def test_cleanup_batch_size_env_var(monkeypatch):
    ...
    import derouter.constants as constants_module
    import derouter.proxy.db.db_transaction_queue.spend_log_cleanup as cleanup_module
...
async def test_spend_logs_retention_alone_does_not_touch_the_session_rollup():
    ...
    assert any('"DeRouter_SpendLogs"' in sql for sql in tables)
    assert not any('"DeRouter_AutoRouterSession"' in sql for sql in tables)
    assert not any('"DeRouter_HealthCheckTable"' in sql for sql in tables)
...
async def test_session_retention_alone_cleans_only_the_session_rollup():
    ...
    tables = [call[0][0] for call in client.db.execute_raw.call_args_list]
    assert len(tables) == 1
    assert '"DeRouter_AutoRouterSession"' in tables[0]
...
async def test_health_check_retention_alone_cleans_only_the_health_check_table():
    ...
    tables = [call[0][0] for call in client.db.execute_raw.call_args_list]
    assert len(tables) == 1
    assert '"DeRouter_HealthCheckTable"' in tables[0]
...
async def test_each_retention_key_cuts_off_at_its_own_horizon():
    ...
    cutoffs = {
        (
            "DeRouter_AutoRouterSession"
            if '"DeRouter_AutoRouterSession"' in call[0][0]
            else "DeRouter_HealthCheckTable"
            if '"DeRouter_HealthCheckTable"' in call[0][0]
            else "logs"
        ): call[0][1]
    ...
    assert (now - cutoffs["DeRouter_AutoRouterSession"]).days == 365
    assert (now - cutoffs["DeRouter_HealthCheckTable"]).days == 30
...
async def test_run_budget_is_shared_across_tables_not_granted_per_table():
    ...
    assert "DeRouter_SpendLogs" in tables_touched
...
async def test_cleanup_groups_share_budget_so_health_checks_still_get_a_delete():
    ...
    tables_touched = {call[0][0].split('"')[1] for call in mock_db.execute_raw.call_args_list}
    assert "DeRouter_SpendLogs" in tables_touched
    assert "DeRouter_HealthCheckTable" in tables_touched
...
async def test_partition_maintenance_still_runs_while_the_run_has_budget():
    ...
    partition_manager.is_partitioned = AsyncMock(return_value=True)
    partition_manager.ensure_partitions = AsyncMock()
    partition_manager.drop_partitions_older_than = AsyncMock(return_value=["DeRouter_SpendLogs_p20260601"])
