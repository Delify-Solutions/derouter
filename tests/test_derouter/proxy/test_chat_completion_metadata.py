...
from derouter.proxy.proxy_server import chat_completion, completion, embeddings
...
async def test_chat_completion_metadata_population():
    ...
    with patch(
        "derouter.proxy.proxy_server._read_request_body", new_callable=AsyncMock
    ) as mock_read_body:
...
async def test_embedding_metadata_population():
    ...
    with patch(
        ...
        with patch(
            "derouter.proxy.proxy_server.ProxyBaseLLMRequestProcessing.__init__",
            return_value=None,
