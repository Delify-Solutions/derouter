"""
Tests for DeRouter proxy realtime WebRTC HTTP endpoints:
...
from derouter.proxy.realtime_endpoints.endpoints import (
...
@pytest.fixture
def mock_add_derouter_data() -> AddLitellmDataToRequest:
...
async def test_client_secrets_success_with_mock(
    ...
    try:
        ...
        with (
            patch(
                "derouter.proxy.proxy_server.route_request",
...
async def test_client_secrets_transcription_rejects_disallowed_nested_model(
    ...
    try:
        client = TestClient(proxy_app, raise_server_exceptions=False)
        with (
            patch("derouter.proxy.proxy_server.route_request") as mock_route_request,
            patch("derouter.proxy.proxy_server.proxy_logging_obj") as mock_logging,
        ):
            mock_logging.post_call_failure_hook = AsyncMock()
...
async def test_transcription_sessions_rejects_disallowed_team_model_scope(
    ...
    team = DeRouter_TeamTableCachedObj(
        team_id="team-a",
        models=["gpt-4o-realtime-preview"],
    )
    proxy_app.dependency_overrides[user_api_key_auth] = lambda: UserAPIKeyAuth(
...
async def test_transcription_sessions_rejects_disallowed_team_member_model_scope(
    ...
    team = DeRouter_TeamTableCachedObj(team_id="team-a", models=["*"])
...
async def test_realtime_websocket_phase2_failure_sends_error_event_and_reasoned_close():
    ...
    with (
        patch(
            "derouter.proxy.proxy_server.can_key_call_resolved_model",
            new=AsyncMock(return_value=None),
        ),
        patch(
...
def test_realtime_calls_upstream_rejection_answers_an_openai_typed_error(
    proxy_app: FastAPI,
    mock_add_derouter_data: AddLitellmDataToRequest,
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.route_request", failing_route_request)
    monkeypatch.setattr("derouter.proxy.proxy_server.add_derouter_data_to_request", mock_add_derouter_data)
