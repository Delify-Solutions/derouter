...
async def test_delete_vector_store_checks_access():
    ...
    from derouter.types.vector_stores import VectorStoreDeleteRequest
    ...
    mock_prisma.db.derouter_managedvectorstorestable.find_unique = AsyncMock(return_value=mock_vector_store)
    ...
    with patch(
        ...
        with patch("derouter.vector_store_registry", None):
...
_UNSCOPED: DeRouter_ManagedVectorStore = {
    "vector_store_id": "vs_unscoped",
    "custom_llm_provider": "openai",
    "team_id": None,
}
_TEAM_A_OWNED: DeRouter_ManagedVectorStore = {
    "vector_store_id": "vs_team_a",
    "custom_llm_provider": "openai",
    "team_id": "team_a",
}
_UI_CREATED: DeRouter_ManagedVectorStore = {
...
async def _listed_ids(user_api_key_dict: UserAPIKeyAuth) -> list[str]:
    ...
    with patch(  # test-quality-ok: the list route reads rows through this module-level DB helper, no injection seam
        "derouter.proxy.vector_store_endpoints.management_endpoints.VectorStoreRegistry._get_vector_stores_from_db",
...
async def test_list_vector_stores_hides_ungranted_stores_from_non_admin_keys():
    ...
    assert await _listed_ids(
        UserAPIKeyAuth(
            team_id="team_b",
            object_permission=DeRouter_ObjectPermissionTable(object_permission_id="op-1", vector_stores=["vs_unscoped"]),
        )
    ) == ["vs_unscoped"]
    assert await _listed_ids(
        UserAPIKeyAuth(
...
async def test_list_vector_stores_dashboard_session_resolves_real_teams(
    ...
    from the dashboard (team_id derouter-dashboard) are not visible just because every session shares that team id,
    ...
    alice = UserAPIKeyAuth(
        ...
        object_permission=(
            DeRouter_ObjectPermissionTable(object_permission_id="op-4", vector_stores=session_key_grants)
            if session_key_grants is not None
            else None
        ),
    )
    teams = {
        "team_a": DeRouter_TeamTableCachedObj(team_id="team_a"),
        "team_granted": DeRouter_TeamTableCachedObj(
            team_id="team_granted",
            object_permission=DeRouter_ObjectPermissionTable(object_permission_id="op-3", vector_stores=["vs_unscoped"]),
        ),
    }

    async def fake_get_team_object(team_id: str, **_kwargs: object) -> DeRouter_TeamTableCachedObj:
        return teams[team_id]

    with (
        patch(  # test-quality-ok: team rows come from the module-level prisma client, no injection seam
            "derouter.proxy.auth.auth_checks.get_team_object", new=fake_get_team_object
        ),
        patch(  # test-quality-ok: the user row comes from the module-level prisma client, no injection seam
            "derouter.proxy.vector_store_endpoints.utils.resolve_ui_session_team_ids",
...
async def test_get_vector_store_info_dashboard_session_resolves_real_teams(
    ...
    /vector_store/list shows it, instead of judging the session's reserved derouter-dashboard team id."""
    from derouter.models.team import DeRouter_TeamTableCachedObj
    from derouter.proxy.vector_store_endpoints.management_endpoints import (
        get_vector_store_info,
    )
    from derouter.types.vector_stores import VectorStoreInfoRequest
    ...
    async def fake_get_team_object(team_id: str, **_kwargs: object) -> DeRouter_TeamTableCachedObj:
        return DeRouter_TeamTableCachedObj(team_id=team_id)

    mock_prisma = MagicMock()
    mock_prisma.db.derouter_managedvectorstorestable.find_unique = AsyncMock(
    ...
    with (
        ...
        patch(  # test-quality-ok: the endpoint consults the module-level registry before the DB, no injection seam
            "derouter.vector_store_registry", None
        ),
        patch(  # test-quality-ok: team rows come from the module-level prisma client, no injection seam
            "derouter.proxy.auth.auth_checks.get_team_object", new=fake_get_team_object
        ),
