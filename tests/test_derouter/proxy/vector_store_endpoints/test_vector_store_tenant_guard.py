...
import derouter
from derouter.proxy._types import DeRouter_ManagedVectorStoresTable, UserAPIKeyAuth
...
async def test_vector_store_search_forces_path_id_over_body_id():
    from derouter.proxy.vector_store_endpoints.endpoints import vector_store_search
    ...
    with (
        ...
        patch(
            "derouter.proxy.vector_store_endpoints.endpoints.ProxyBaseLLMRequestProcessing.base_process_llm_request",
...
async def test_vector_store_file_create_forces_path_id_over_body_id():
    ...
    with (
        ...
        patch(
            "derouter.proxy.vector_store_files_endpoints.endpoints.ProxyBaseLLMRequestProcessing.base_process_llm_request",
            new=fake_base_process,
        ),
    ):
...
async def test_vector_store_file_list_resolves_managed_vector_store_before_team_fallback():
    ...
    with (
        patch(
            "derouter.proxy.vector_store_files_endpoints.endpoints.assert_user_can_access_vector_store_id",
            new=AsyncMock(return_value=None),
        ),
        patch("derouter.proxy.proxy_server.llm_router", llm_router),
        patch(
            "derouter.proxy.vector_store_files_endpoints.endpoints.ProxyBaseLLMRequestProcessing.base_process_llm_request",
            new=fake_base_process,
        ),
...
async def test_rag_query_denies_nested_other_team_vector_store():
    from derouter.proxy.rag_endpoints.endpoints import rag_query
    ...
    with (
        patch(
            "derouter.proxy.rag_endpoints.endpoints._read_request_body",
            new=AsyncMock(
                return_value={
                    "model": "gpt-4o-mini",
        ...
        patch.object(derouter, "vector_store_registry", mock_registry),
        patch(
            "derouter.proxy.rag_endpoints.endpoints.derouter.aquery",
            new=AsyncMock(),
        ) as mock_aquery,
    ):
        with pytest.raises(HTTPException) as exc_info:
...
async def test_rag_ingest_denies_nested_other_team_vector_store():
    from derouter.proxy.rag_endpoints.endpoints import rag_ingest
    ...
    with (
        patch(
            "derouter.proxy.rag_endpoints.endpoints.parse_rag_ingest_request",
            new=AsyncMock(
                return_value=(
                    {
                        "vector_store": {
        ...
        patch.object(derouter, "vector_store_registry", mock_registry),
        patch(
            "derouter.proxy.rag_endpoints.endpoints.derouter.aingest",
            new=AsyncMock(),
        ) as mock_aingest,
    ):
        with pytest.raises(HTTPException) as exc_info:
...
def test_rag_payload_scan_rejects_excessive_nesting():
    ...
    from derouter.proxy.rag_endpoints.endpoints import (
...
async def test_get_managed_vector_store_uses_shared_cache_helper_for_db_fallback():
    ...
    mock_registry = MagicMock()
    mock_registry.get_derouter_managed_vector_store_from_registry.return_value = None
    cache_helper = AsyncMock(
        return_value=[
            DeRouter_ManagedVectorStoresTable(
                ...
                created_at=None,
                updated_at=None,
                derouter_credential_name=None,
                derouter_params={"api_base": "https://example.com"},
                team_id="team-a",
                user_id=None,
            )
        ]
    ...
    with (
        ...
        patch(
            "derouter.proxy.auth.auth_checks.get_managed_vector_store_rows_by_uuids",
            new=cache_helper,
        ),
    ):
        vector_store = await get_derouter_managed_vector_store(
            vector_store_id="vs_cached"
        )
...
async def test_get_managed_vector_store_fails_closed_on_lookup_error():
    ...
    mock_registry = MagicMock()
    mock_registry.get_derouter_managed_vector_store_from_registry.side_effect = (
        RuntimeError("registry unavailable")
    )

    with patch.object(derouter, "vector_store_registry", mock_registry):
        with pytest.raises(HTTPException) as exc_info:
            await get_derouter_managed_vector_store(vector_store_id="vs_registry_only")
...
async def test_vertex_discovery_allows_unregistered_provider_native_datastore_id():
    ...
    with (
        patch(
            "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.get_derouter_managed_vector_store",
            new=AsyncMock(return_value=None),
        ) as mock_lookup,
        patch(
            "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints._base_vertex_proxy_route",
...
async def test_milvus_passthrough_denies_other_team_vector_store_index():
    ...
    index_object.derouter_params.vector_store_name = "tenant-b-store"
    index_object.derouter_params.vector_store_index = "tenant_b_collection"
    ...
    mock_vector_registry.get_derouter_managed_vector_store_from_registry_by_name.return_value = {
        ...
        "team_id": "team-b",
        "derouter_params": {"api_base": "https://milvus.example.com"},
    }
    ...
    with (
        ...
        patch.object(derouter, "vector_store_index_registry", mock_index_registry),
        patch.object(derouter, "vector_store_registry", mock_vector_registry),
    ):
        with pytest.raises(HTTPException) as exc_info:
...
async def test_azure_passthrough_denies_other_team_vector_store_index():
    ...
    mock_vector_registry.get_derouter_managed_vector_store_from_registry_by_name.return_value = {
        ...
        "team_id": "team-b",
        "derouter_params": {"api_base": "https://azure.example.com"},
    }
