...
async def test_list_vector_stores_blocked_when_disabled():
    ...
    with patch.dict(
        "derouter.proxy.proxy_server.general_settings", _DISABLED_GS, clear=True
    ):
...
async def test_list_vector_stores_allowed_when_not_disabled():
    ...
    mock_prisma.db.derouter_managedvectorstorestable.find_many = AsyncMock(
        return_value=[]
    )

    with patch.dict(
        "derouter.proxy.proxy_server.general_settings", _ENABLED_GS, clear=True
    ):
        with patch("derouter.proxy.proxy_server.prisma_client", mock_prisma):
            with patch.object(derouter, "vector_store_registry", None):
                with patch(
...
async def test_new_vector_store_blocked_when_disabled():
    ...
    user = _make_internal_user()
    vs = DeRouter_ManagedVectorStore(vector_store_id="vs-1", custom_llm_provider="openai")  # type: ignore[call-arg]
