...
from derouter.proxy._types import ProxyErrorTypes, UserAPIKeyAuth
...
from derouter.proxy.utils import get_custom_url, join_paths


def test_get_custom_url(monkeypatch):
    monkeypatch.setenv("SERVER_ROOT_PATH", "/derouter")
    custom_url = get_custom_url(request_base_url="http://0.0.0.0:4000", route="ui/")
    assert custom_url == "http://0.0.0.0:4000/derouter/ui/"
...
async def test_proxy_only_error_log_marks_no_upstream_llm_call():
    ...
    ``DEROUTER_LOGGING_NO_UPSTREAM_LLM_CALL`` so tracing callbacks don't fabricate
    ...
    def fake_pre_call(self, *args, **kwargs):
        captured["flag"] = self.model_call_details.get(DEROUTER_LOGGING_NO_UPSTREAM_LLM_CALL)
...
@pytest.mark.asyncio
async def test_proxy_only_error_log_keeps_derouter_metadata_in_derouter_params():
    """Responses API requests carry guardrail info under ``derouter_metadata``
    (not ``metadata``). It must land in derouter_params so
    ``merge_derouter_metadata`` can surface ``guardrail_information`` in the
    ...
    try:
        await proxy_logging_obj._handle_logging_proxy_only_error(
            request_data={
                ...
                "derouter_metadata": {"standard_logging_guardrail_information": guardrail_info},
            },
    ...
    assert captured["derouter_params"]["derouter_metadata"]["standard_logging_guardrail_information"] == guardrail_info
    assert "derouter_metadata" not in captured["optional_params"]
...
def _patch_today(monkeypatch, year, month, day):
    ...
    monkeypatch.setattr("derouter.proxy.utils.date", PatchedDate)


def test_get_projected_spend_over_limit_day_one(monkeypatch):
    from derouter.proxy.utils import _get_projected_spend_over_limit
...
def test_enrich_http_exception_with_guardrail_context_dict_detail():
    ...
    from derouter.proxy.utils import _enrich_http_exception_with_guardrail_context
...
class TestPostCallFailureHookLiftsFirstApiCallStartTime:
    ...
    async def test_lifts_to_top_level_and_pops_logging_obj(self):
        ...
        assert "derouter_logging_obj" not in request_data
...
class TestPostCallFailureHookLiftsRecoveredPartialSpend:
    ...
    async def test_lifts_recovered_usage_and_cost(self):
        ...
        request_data = {"derouter_logging_obj": logging_obj, "metadata": {}}
        await self._run(request_data)
...
class TestPostCallFailureHookLiftsStandardLoggingObject:
    ...
    post_call_failure_hook pops derouter_logging_obj before they run. The hook
...
class TestPostCallFailureHookEstimatesDispatchedInputTokens:
    ...
    async def test_dispatched_failure_estimates_input_tokens_with_zero_cost(self):
        ...
        request_data = {
            "derouter_logging_obj": self._logging_obj(
                {
            ...
            "derouter_logging_obj": self._logging_obj(
                {
                    ...
                    "model": "no-such-model",
                    "messages": [{"role": "user", "content": "hi"}],
                    DEROUTER_LOGGING_NO_UPSTREAM_LLM_CALL: True,
                }
            ),
            "metadata": {},
        }
    ...
    async def test_image_message_estimated_without_fetching_image(self):
        ...
        estimated = request_data["combined_usage_object"]
        assert isinstance(estimated, Usage)
        expected = derouter_module.token_counter(
    ...
    async def test_embedding_string_list_input_counted_in_estimate(self):
        ...
        estimated = request_data["combined_usage_object"]
        assert isinstance(estimated, Usage)
        expected = derouter_module.token_counter(model="gpt-3.5-turbo", text="".join(embedding_input))
        assert estimated.prompt_tokens == expected
    ...
    async def test_anthropic_system_prompt_counted_in_estimate(self):
        ...
        ) + derouter_module.token_counter(model="gpt-3.5-turbo", text=system_prompt)
        assert estimated.prompt_tokens == expected
    ...
    async def test_responses_instructions_counted_in_estimate(self):
        ...
        ) + derouter_module.token_counter(model="gpt-3.5-turbo", text=instructions)
        assert estimated.prompt_tokens == expected
    ...
    async def test_optional_params_system_wins_over_request_body_system(self):
        ...
        expected = derouter_module.token_counter(
            model="gpt-3.5-turbo", messages=messages
        ) + derouter_module.token_counter(model="gpt-3.5-turbo", text=dispatched_system)
        assert estimated.prompt_tokens == expected
...
import derouter
from derouter.proxy.utils import create_model_info_response
from derouter.types.utils import ModelInfo
...
def test_create_model_info_response_survives_malformed_limits_registered_by_router():
    ...
    saved_model_cost = dict(derouter.model_cost)
    try:
        router = Router(
            model_list=[
                {
        ...
        derouter.model_cost.update(saved_model_cost)
...
class TestPostCallFailureHookLLMExceptionAlerting:
    ...
    errors (https://github.com/Delify-Solutions/derouter/issues/3395). Already-normalized
    ...
    async def _alerted(self, exc) -> bool:
        ...
        from derouter.proxy._types import AlertType, UserAPIKeyAuth
...
class TestShouldUseSmtpSsl:
    def test_port_465_uses_ssl(self, monkeypatch):
        from derouter.proxy.utils import _should_use_smtp_ssl
...
class TestCreateSmtpConnection:
    def test_port_465_creates_smtp_ssl_with_verified_context(self, monkeypatch):
        ...
        from derouter.proxy.utils import _create_smtp_connection
...
class TestSendEmailStartTls:
    ...
    async def test_starttls_uses_verified_context(self, monkeypatch):
        ...
        from derouter.proxy.utils import send_email
        ...
        with patch("derouter.proxy.utils._create_smtp_connection") as mock_create_connection:
...
def restore_callbacks():
    ...
    ProxyLogging caches callback capabilities keyed on id()s of derouter.callbacks,
    ...
    """
    original = list(derouter.callbacks)
    yield
...
async def test_prisma_health_check_failure_redacts_database_credentials(caplog):
    ...
    client.db.query_raw = AsyncMock(
        side_effect=Exception("could not connect to postgresql://admin:hunter2@db.internal:5432/derouter")
    )
    client.proxy_logging_obj.failure_handler = AsyncMock()
    client._probe_target_wrapper = MagicMock(return_value=client.db)
    client._run_health_probe = partial(PrismaClient._run_health_probe, client)
    ...
    emitted = [record.getMessage() for record in caplog.records if record.name == "DeRouter Proxy"]
...
async def test_update_data_key_branch_stamps_settings_updated_at():
    ...
    client.db.derouter_verificationtoken.update = AsyncMock(return_value=None)

    before = datetime.now(timezone.utc)
    await PrismaClient.update_data(client, token="sk-test-key", data={"models": ["gpt-4"]})
    after = datetime.now(timezone.utc)

    sent = client.db.derouter_verificationtoken.update.call_args.kwargs["data"]
...
def test_a_system_prompt_reads_the_same_whatever_shape_it_arrived_in(system_input, expected):
    from derouter.proxy.utils import _system_prompt_text

    assert _system_prompt_text(system_input) == expected


def test_a_system_prompt_is_counted_on_top_of_the_request():
    from derouter.proxy.utils import _count_request_input_tokens
...
def test_a_failed_dispatch_is_estimated_as_input_only():
    from derouter.proxy.utils import _count_request_input_tokens, _estimate_dispatched_failure_usage
...
def test_nothing_is_estimated_when_there_is_nothing_to_count(request_input):
    from derouter.proxy.utils import _estimate_dispatched_failure_usage
...
def test_usage_the_stream_already_recovered_beats_an_estimate():
    from derouter.proxy.utils import _failure_usage_to_lift
    from derouter.types.utils import Usage
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {
                "derouter_no_upstream_llm_call": True,
                "call_type": "acompletion",
                "model": FAILURE_USAGE_MODEL,
                "messages": ONE_USER_MESSAGE,
            },
...
def test_the_no_upstream_call_key_the_module_uses_is_the_one_asserted_above():
    ...
    assert DEROUTER_LOGGING_NO_UPSTREAM_LLM_CALL == "derouter_no_upstream_llm_call"
...
def test_a_failure_with_no_logging_object_lifts_nothing():
    from derouter.proxy.utils import _failure_fields_to_lift

    assert dict(_failure_fields_to_lift({})) == {}
    assert dict(_failure_fields_to_lift({"derouter_logging_obj": _LoggingObj({})})) == {}
...
def test_a_dispatched_failure_lifts_the_four_fields_the_spend_log_needs():
    ...
    lifted = _failure_fields_to_lift(
        {
            "derouter_logging_obj": _LoggingObj(
                {
...
async def test_proxy_only_error_5xx_keeps_traceback_and_runs_sync_callbacks(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "failure_callback", [_custom_sync_callback])
    proxy_logging_obj = ProxyLogging(user_api_key_cache=DualCache())
    captured = {}
    sync_ran = asyncio.Event()
    loop = asyncio.get_running_loop()
...
def test_convert_mcp_to_llm_format_carries_key_and_team_guardrails(key_metadata, team_metadata, expected_to_run):
    ...
    with patch(  # test-quality-ok: the key-guardrail premium gate reads this proxy_server module global and has no injection seam
        "derouter.proxy.proxy_server.premium_user", True
    ):
