...
AUDIENCE_CASES = (
    ("https://derouter.example.com", False, True),
    (None, True, True),
    (None, False, False),
    ("https://derouter.example.com", True, False),
)
...
def _audit_log(**overrides) -> DeRouter_AuditLogs:
    fields = {
        "id": "audit-1",
    ...
    return DeRouter_AuditLogs(**fields)
...
def test_team_membership_budget_table_optional_no_crash():
    ...
    result = DeRouter_TeamMembership.model_validate(data)
    assert result.derouter_budget_table is None
