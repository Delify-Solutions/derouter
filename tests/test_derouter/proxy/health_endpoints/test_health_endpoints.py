...
import derouter
import derouter.proxy.health_endpoints._health_endpoints as _health_endpoints_module
from derouter.derouter_core_utils.health_check_helpers import TEST_IMAGE_BASE64
...
from tests.test_derouter.proxy.conftest import create_proxy_test_client
...
async def test_health_services_endpoint_sqs(status, error_message):
    ...
    """
    with patch("derouter.integrations.sqs.SQSLogger") as MockSQSLogger:
...
async def test_health_license_endpoint_with_active_license():
    ...
    with (
        patch(
            "derouter.proxy.proxy_server._license_check",
            mock_license_check,
        ),
        patch(
        ...
        patch(
            "derouter.proxy.proxy_server.premium_user_data",
            license_data,
        ),
    ):
...
async def test_test_model_connection_loads_config_from_router():
    ...
    # Mock _update_derouter_params_for_health_check
    def mock_update_params(model_info, derouter_params):
        # Just return params with messages added
        params = derouter_params.copy()
    ...
    with (
        ...
        patch(
            "derouter.proxy.health_endpoints._health_endpoints.derouter.ahealth_check",
            mock_ahealth_check,
        ),
        patch(
            "derouter.proxy.health_endpoints._health_endpoints.run_with_timeout",
            mock_run_with_timeout,
        ),
        patch(
            "derouter.proxy.health_endpoints._health_endpoints._update_derouter_params_for_health_check",
            mock_update_params,
        ),
        patch(
            "derouter.proxy.health_endpoints._health_endpoints._reject_os_environ_references",
...
async def test_test_model_connection_uses_model_info_id_to_disambiguate_duplicate_model_names():
    ...
    def _get_deployment_by_id(model_id):
        if model_id == "deployment-A-id":
            return Deployment(
                model_name="openai/*",
                derouter_params=DeRouter_Params(**deployment_a["derouter_params"]),
                model_info=deployment_a["model_info"],
            )
        if model_id == "deployment-B-id":
            return Deployment(
                model_name="openai/*",
                derouter_params=DeRouter_Params(**deployment_b["derouter_params"]),
...
async def test_test_model_connection_uses_loaded_deployment_team_id():
    ...
    async def fake_find_unique(*, where):
        ...
        if team_id == requester_team_id:
            return SimpleNamespace(
                model_dump=lambda: DeRouter_TeamTable(
                    team_id=requester_team_id,
                    members_with_roles=[
                        {
                            "user_id": "team-b-admin-user",
    ...
    with (
        ...
        patch("derouter.proxy.management_endpoints.model_management_endpoints.TeamRepository") as MockTeamRepo,
    ):
...
async def test_test_model_connection_authorizes_on_params_after_health_check_params_merge():
    ...
    with (
        ...
        patch(  # test-quality-ok: proxy module global, no injection seam
            "derouter.proxy.proxy_server.llm_router", None
        ),
    ...
    authorized_params = passed_model_params.derouter_params.model_dump()
...
async def test_health_services_endpoint_galileo(status, error_message):
    with patch("derouter.integrations.galileo.GalileoObserve") as MockGalileoObserve:
        mock_instance = MagicMock()
        mock_instance.async_health_check = AsyncMock(return_value={"status": status, "error_message": error_message})
...
async def test_health_services_endpoint_datadog_llm_observability():
    ...
    Regression test for: https://github.com/Delify-Solutions/derouter/issues/XXXX
    ...
    with patch("derouter.success_callback", ["datadog_llm_observability"]):
...
async def test_health_services_endpoint_newrelic_blocks_non_admin(role):
    """
    /health/services?service=newrelic emits a real DeRouterConnectionTest event
    ...
    with patch("derouter.integrations.newrelic.newrelic.NewRelicLogger") as MockNewRelicLogger:
        mock_instance = MagicMock()
...
def test_health_readiness_details_returns_diagnostic_fields(monkeypatch):
    ...
    response_data = response.json()
    assert response_data["status"] == "healthy"
    assert "derouter_version" in response_data
...
async def test_health_endpoint_filters_model_list_by_user_access():
    ...
    from derouter.proxy.health_endpoints._health_endpoints import health_endpoint
    ...
    with (
        patch("derouter.proxy.proxy_server.llm_model_list", full_model_list),
        patch("derouter.proxy.proxy_server.llm_router", None),
        patch("derouter.proxy.proxy_server.prisma_client", None),
        patch("derouter.proxy.proxy_server.use_background_health_checks", False),
        patch("derouter.proxy.proxy_server.user_model", None),
        patch("derouter.proxy.proxy_server.health_check_results", {}),
        patch("derouter.proxy.proxy_server.health_check_details", True),
        patch("derouter.proxy.proxy_server.health_check_concurrency", 1),
        patch(
            "derouter.proxy.health_endpoints._health_endpoints._perform_health_check_and_save",
...
async def test_health_endpoint_keeps_full_model_list_for_all_proxy_models():
    ...
    """
    from derouter.proxy._types import SpecialModelNames, UserAPIKeyAuth
    from derouter.proxy.health_endpoints._health_endpoints import health_endpoint
...
async def test_health_endpoint_filters_background_cache_by_user_access():
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.llm_router", None),
        patch("derouter.proxy.proxy_server.prisma_client", None),
        patch("derouter.proxy.proxy_server.use_background_health_checks", True),
        patch("derouter.proxy.proxy_server.user_model", None),
        patch("derouter.proxy.proxy_server.health_check_results", cached_results),
        patch("derouter.proxy.proxy_server.health_check_details", True),
        patch("derouter.proxy.proxy_server.health_check_concurrency", 1),
    ):
        from fastapi import Response
...
async def test_health_readiness_returns_503_when_db_disconnected():
    ...
    from derouter.proxy.health_endpoints._health_endpoints import health_readiness
...
async def test_health_readiness_returns_200_when_db_down_and_allow_requests_on_db_unavailable():
    """
    Regression test for https://github.com/Delify-Solutions/derouter/issues/34934.
    ...
    with (
        patch(  # test-quality-ok: the readiness path reads the proxy-global DB client; it has no injection seam
            "derouter.proxy.proxy_server.prisma_client", mock_prisma
        ),
...
async def test_db_health_readiness_check_bounds_hung_health_check():
    ...
    with patch(  # test-quality-ok: lowers the module-level probe timeout so the hung-call test finishes fast
        "derouter.proxy.health_endpoints._health_endpoints.DB_READINESS_CHECK_TIMEOUT_SECONDS",
        0.05,
    ):
...
async def test_db_health_readiness_check_overall_deadline_bounds_hung_reconnect():
    ...
    with patch(  # test-quality-ok: lowers the module-level probe timeout so the hung-call test finishes fast
        "derouter.proxy.health_endpoints._health_endpoints.DB_READINESS_PROBE_DEADLINE_SECONDS",
        0.05,
    ):
        start = time.monotonic()
        with patch(  # test-quality-ok: the readiness path reads the proxy-global DB client; it has no injection seam
...
def test_clean_endpoint_data_strips_credentials_keeps_routing_fields():
    ...
    """
    from derouter.proxy.health_check import _clean_endpoint_data
...
def test_clean_endpoint_data_never_displays_credential_fields(credential_field, details):
    ...
    carry credential-bearing derouter_params, with or without details.
    """
    from derouter.proxy.health_check import _clean_endpoint_data
...
class TestConfigBaseForHealthCheck:
    ...
    def test_stored_credential_reference_is_dropped_with_the_credentials(self):
        ...
        config = {**self.CONFIG, "derouter_credential_name": "OpenAI-prod"}
        base = self._base(config, {"api_base": "https://caller.example/v1"})
        assert "derouter_credential_name" not in base
        assert "api_key" not in base
    ...
    def test_stored_credential_reference_kept_when_request_sets_no_connection(self):
        ...
        base = self._base(
            config,
            {"model": "openai/gpt-4o", "derouter_credential_name": "OpenAI-prod", "custom_llm_provider": "openai"},
        )
        assert base["derouter_credential_name"] == "OpenAI-prod"
        assert base["api_key"] == "sk-configured"

    def test_request_naming_another_credential_does_not_inherit_config_credentials(self):
        base = self._base(self.CONFIG, {"model": "openai/gpt-4o", "derouter_credential_name": "Another-cred"})
        assert "api_key" not in base
        assert "api_base" not in base
        assert "vertex_credentials" not in base
        assert base["rpm"] == 100
    ...
    def test_blank_credential_name_names_no_credential(self):
        base = self._base(self.CONFIG, {"model": "openai/gpt-4o", "derouter_credential_name": ""})
        assert base["api_key"] == "sk-configured"

    def test_opt_in_does_not_put_config_credentials_over_a_named_credential(self):
        base = self._base(
            self.CONFIG,
            {"model": "openai/gpt-4o", "derouter_credential_name": "Another-cred"},
            allow_client_side_credentials=True,
        )
        assert "api_key" not in base
...
class TestTestConnectionUsesTheNamedCredential:
    ...
    REQUEST = {
        ...
        "derouter_credential_name": "my-xai-cred",
    }
    COMPLETION = {
        "id": "chatcmpl-test",
    ...
    @staticmethod
    def _wildcard_deployment(**derouter_params: str) -> dict:
        return {
            "model_name": "xai/*",
            "derouter_params": {"model": "xai/*", **derouter_params},
    ...
    def _probe(
        ...
        with (
            ...
            patch(  # test-quality-ok: the deployment the probe is matched against is a proxy global; it has no injection seam
                "derouter.proxy.proxy_server.llm_router", router
            ),
    ...
    def test_named_credentials_key_is_sent_not_the_matched_deployments_key(self, monkeypatch):
        monkeypatch.setattr(derouter, "credential_list", [self._credential(api_key=self.CREDENTIAL_KEY)])
...
class TestNoRedisWarning:
    ...
    async def test_warns_when_no_redis_and_no_db_to_count_workers(self, monkeypatch):
        monkeypatch.delenv("DEROUTER_DISABLE_NO_REDIS_WARNING", raising=False)
        with (
            patch("derouter.proxy.proxy_server.redis_usage_cache", None),
            patch("derouter.proxy.proxy_server.llm_router", self._router(None)),
            patch("derouter.proxy.proxy_server.prisma_client", None),
        ):
    ...
    async def test_stays_quiet_for_a_confirmed_single_worker(self, monkeypatch):
        ...
        with (
            patch("derouter.proxy.proxy_server.redis_usage_cache", None),
            patch("derouter.proxy.proxy_server.llm_router", self._router(None)),
            patch("derouter.proxy.proxy_server.prisma_client", self._prisma_with_workers(1)),
        ):
    ...
    async def test_warns_when_multiple_workers_share_the_db(self, monkeypatch, live_workers):
        ...
        with (
            patch("derouter.proxy.proxy_server.redis_usage_cache", None),
            patch("derouter.proxy.proxy_server.llm_router", self._router(None)),
            patch("derouter.proxy.proxy_server.prisma_client", self._prisma_with_workers(live_workers)),
        ):
            assert await _show_no_redis_warning() is True
    ...
    async def test_warns_when_the_worker_census_is_empty(self, monkeypatch):
        ...
        with (
            patch("derouter.proxy.proxy_server.redis_usage_cache", None),
            patch("derouter.proxy.proxy_server.llm_router", self._router(None)),
            patch("derouter.proxy.proxy_server.prisma_client", self._prisma_with_workers(0)),
        ):
            assert await _show_no_redis_warning() is True
    ...
    async def test_stays_quiet_when_a_coordination_redis_is_configured(self, monkeypatch):
        ...
        with (
            patch("derouter.proxy.proxy_server.redis_usage_cache", MagicMock()),
            patch("derouter.proxy.proxy_server.llm_router", self._router(None)),
            patch("derouter.proxy.proxy_server.prisma_client", prisma),
        ):
            assert await _show_no_redis_warning() is False
    ...
    async def test_stays_quiet_when_only_the_router_has_redis(self, monkeypatch):
        ...
        with (
            patch("derouter.proxy.proxy_server.redis_usage_cache", None),
            patch("derouter.proxy.proxy_server.llm_router", self._router(MagicMock())),
            patch("derouter.proxy.proxy_server.prisma_client", self._prisma_with_workers(5)),
        ):
            assert await _show_no_redis_warning() is False
    ...
    async def test_env_var_suppresses_the_warning_despite_multiple_workers(self, monkeypatch, value):
        monkeypatch.setenv("DEROUTER_DISABLE_NO_REDIS_WARNING", value)
        with (
            patch("derouter.proxy.proxy_server.redis_usage_cache", None),
            patch("derouter.proxy.proxy_server.llm_router", self._router(None)),
            patch("derouter.proxy.proxy_server.prisma_client", self._prisma_with_workers(5)),
    ...
    async def test_env_var_set_false_keeps_the_warning_for_multiple_workers(self, monkeypatch):
        monkeypatch.setenv("DEROUTER_DISABLE_NO_REDIS_WARNING", "false")
        with (
            patch("derouter.proxy.proxy_server.redis_usage_cache", None),
            patch("derouter.proxy.proxy_server.llm_router", self._router(None)),
            patch("derouter.proxy.proxy_server.prisma_client", self._prisma_with_workers(2)),
        ):
            assert await _show_no_redis_warning() is True
...
def test_test_model_connection_accepts_image_edit_mode(monkeypatch):
    ...
    with (
        ...
        response = client.post(
            ...
            json={
                "mode": "image_edit",
                "derouter_params": {"model": "openai/gpt-image-2", "api_key": "sk-test"},
            },
        )
