...
from derouter.proxy.client import Client, ModelsManagementClient
from derouter.proxy.client.exceptions import NotFoundError, UnauthorizedError
...
def test_new_without_model_info(client):
    ...
    assert request.json == {"model_name": model_name, "derouter_params": model_params}
...
def test_update_without_model_info(client):
    ...
    # Check request body doesn't include model_info
    assert request.json == {"id": model_id, "derouter_params": model_params}
