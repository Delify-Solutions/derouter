...
from derouter.constants import CLI_JWT_EXPIRATION_HOURS
...
from derouter.derouter_core_utils.cli_token_utils import CliTokenRecord, save_cli_token
from derouter.proxy.client.cli import cli
from derouter.proxy.client.cli.commands.auth import (
    get_stored_api_key,
    login,
    logout,
    print_token,
...
from derouter.proxy.client.cli.commands.claude_settings import SettingsFileOwner
...
def isolated_home(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("DEROUTER_PROXY_URL", raising=False)
    monkeypatch.delenv("DEROUTER_PROXY_API_KEY", raising=False)
    return tmp_path


def _write_home_json(home: Path, filename: str, payload: dict[str, object]) -> None:
    derouter_dir = home / ".derouter"
    derouter_dir.mkdir(exist_ok=True)
    (derouter_dir / filename).write_text(json.dumps(payload))
...
class TestPollingErrorSurfacing:
    def test_client_error_raises_with_server_detail_and_stops_polling(self):
        from derouter.proxy.client.cli.commands.auth import _poll_for_ready_data

        mock_response = Mock()
        mock_response.status_code = 400
        mock_response.json.return_value = {
            "detail": "Your derouter CLI is out of date and uses a login flow this proxy no longer supports."
        }

        with patch("requests.get", return_value=mock_response) as mock_get, patch("time.sleep"):
            with pytest.raises(ValueError, match='Your derouter CLI is out of date and uses a login flow') as exc_info:
                _poll_for_ready_data("http://test/sso/cli/poll/sk-legacy")

        assert mock_get.call_count == 1
        assert (
            "The proxy rejected the login session with HTTP 400: Your derouter CLI is out of date "
...
class TestStartCliSsoFlowErrors:
    def test_endpoint_not_found_explains_version_or_base_url(self):
        from derouter.proxy.client.cli.commands.auth import _start_cli_sso_flow
...
class TestLoginCommand:
    ...
    def test_login_replaces_a_pkce_record_and_revokes_its_refresh_token(self):
        ...
        with (
            ...
            patch("derouter.proxy.client.cli.commands.auth.requests.Session", _FakeSession),
            patch("derouter.proxy.client.cli.commands.auth.load_token", return_value=_pkce_record()),
            patch("derouter.proxy.client.cli.commands.auth.save_token", return_value=SecretStored()) as mock_save,
            patch("derouter.proxy.client.cli.interface.show_commands"),
        ):
    ...
    def test_login_success(self):
        ...
        with (
            ...
            patch("derouter.proxy.client.cli.commands.auth.save_cli_token") as mock_save,
            patch("derouter.proxy.client.cli.interface.show_commands") as mock_show_commands,
        ):
            ...
            assert mock_get.call_args.kwargs["headers"] == {"x-derouter-cli-poll-secret": "poll-secret"}
    ...
    def test_login_prefills_the_code_in_the_browser_when_the_proxy_advertises_it(
        ...
        start_response = _mock_cli_sso_start_response(
            ...
            verification_uri_complete=(
                ...
                "?source=derouter-cli&key=cli-test-uuid-123&user_code=ABCD-EFGH"
            ),
        )
        ...
        assert json.loads((isolated_home / ".derouter" / "token.json").read_text())["user_id"] == "test-user-123"
...
class TestLogoutCommand:
    ...
    def test_logout_success(self, isolated_home, secret_vault_factory):
        ...
        assert not (isolated_home / ".derouter" / "token.json").exists()
    ...
    def test_logout_without_the_keyring_package_does_not_claim_the_keychain_is_clear(
        ...
        assert "pip install 'derouter[cli]'" in result.output
    ...
    def test_logout_on_a_read_only_directory_still_takes_the_secret_out_of_the_file(
        ...
        """A ~/.derouter that will accept no replacement file and no removal still lets the file it
...
class TestWhoamiCommand:
    ...
    def test_whoami_authenticated(self):
        ...
        with patch("derouter.proxy.client.cli.commands.auth.load_cli_token", return_value=token_data):
    ...
    def test_whoami_not_authenticated(self):
        ...
        with patch("derouter.proxy.client.cli.commands.auth.load_cli_token", return_value=None):
            result = self.runner.invoke(whoami)
    ...
    def test_whoami_pkce_record_shows_the_team_and_when_the_key_renews(self):
        ...
        with patch("derouter.proxy.client.cli.commands.auth.load_token", return_value=token_data):
            result = self.runner.invoke(whoami)
    ...
    def test_whoami_no_timestamp(self):
        ...
        with (
            patch(
                "derouter.proxy.client.cli.commands.auth.load_cli_token",
...
class TestCLIKeyRegenerationFlow:
    ...
    def test_login_with_team_selection_flow(self):
        ...
        with (
            ...
            assert mock_get.call_args_list[0].kwargs["headers"] == {"x-derouter-cli-poll-secret": "poll-secret"}
    ...
    def test_login_without_teams_flow(self):
        ...
        with (
            ...
            call_args = mock_browser.call_args[0][0]
            assert "https://test.example.com/sso/key/generate" in call_args
            assert "source=derouter-cli" in call_args
...
class TestPrintTokenCommand:
    ...
    `DEROUTER_PROXY_URL` only enforces the match when a caller explicitly
        ...
        with (
            patch(
                ...
                return_value=CliTokenRecord(
                    base_url="https://derouter-proxy.corp.com",
                    key="sk-prod-fresh",
                    timestamp=time.time(),
                ),
            ),
...
class TestPrintTokenWithConfigFile:
    ...
    DEROUTER_PROXY_URL, so print-token must treat it as an explicit server
...
class TestFileFallbackStorage:
    ...
    def test_owner_only_file_and_directory_with_no_temp_leftovers(self, isolated_home, secret_vault_factory):
        ...
        token_file = isolated_home / ".derouter" / "token.json"
    ...
    def test_a_failed_write_preserves_the_existing_token(self, isolated_home, secret_vault_factory, monkeypatch):
        ...
        monkeypatch.setattr("derouter.derouter_core_utils.private_json.json.dump", _explode)
...
class TestKeychainBackedCommands:
    ...
    def test_login_names_the_kill_switch_instead_of_blaming_the_machine(
        ...
        assert DISABLE_KEYRING_ENV_VAR in result.output
        assert "No OS keychain available" not in result.output
        assert json.loads((isolated_home / ".derouter" / "token.json").read_text())["key"] == "sk-minted"
...
class TestApiKeyPrecedence:
    """`DEROUTER_PROXY_API_KEY` and `--api-key` outrank the stored credential; moving the secret
    into the keychain must not disturb that order."""

    def _resolved_key(self, args, obj=None):
        with patch("derouter.proxy.client.cli.main.print_version") as mock_print_version:
    ...
    def test_env_var_beats_the_stored_credential(self, isolated_home, monkeypatch):
        ...
        monkeypatch.setenv("DEROUTER_PROXY_API_KEY", "sk-from-env")
...
class TestLoginConfigClaude:
    ...
    def _run_login(self, tmp_path, args, base_url="https://test.example.com"):
        ...
        with (
            ...
            patch("requests.post", return_value=_mock_cli_sso_start_response()),
            patch("requests.get", return_value=poll_response),
            patch("derouter.proxy.client.cli.commands.auth.save_cli_token"),
            patch("derouter.proxy.client.cli.interface.show_commands"),
            patch("derouter.proxy.client.cli.commands.auth.CLAUDE_SETTINGS_PATH", settings_path),
            patch(
                "derouter.proxy.client.cli.commands.auth.SETTINGS_FILE_OWNERS",
                (SettingsFileOwner(backup_path, "lite up", "lite down"),),
            ),
            patch(
                "derouter.proxy.client.cli.commands.claude_settings.shutil.which",
...
def _pkce_credential():
    from derouter.proxy.client.cli.commands.pkce_login import PkceCredential
...
class TestPkceLoginCommand:
    ...
    def test_pkce_login_saves_the_new_record_then_revokes_the_refresh_token_it_replaced(self):
        ...
        with (
            patch("derouter.proxy.client.cli.commands.auth.run_pkce_login", return_value=_pkce_credential()),
            patch("derouter.proxy.client.cli.commands.auth.load_token", return_value=_pkce_record(team_id="team-a")),
            patch("derouter.proxy.client.cli.commands.auth.save_token", side_effect=record_posts) as save,
            patch("derouter.proxy.client.cli.commands.auth.requests.Session", _FakeSession),
            patch("derouter.proxy.client.cli.interface.show_commands"),
        ):
    ...
    def test_pkce_login_keeps_the_new_record_when_the_old_refresh_token_cannot_be_revoked(self):
        ...
        with (
            patch("derouter.proxy.client.cli.commands.auth.run_pkce_login", return_value=_pkce_credential()),
            patch("derouter.proxy.client.cli.commands.auth.load_token", return_value=_pkce_record()),
            patch("derouter.proxy.client.cli.commands.auth.save_token", return_value=SecretStored()) as save,
            patch("derouter.proxy.client.cli.commands.auth.requests.Session", _FailingSession),
            patch("derouter.proxy.client.cli.interface.show_commands"),
        ):
            result = self.runner.invoke(login, ["--pkce"], obj={"base_url": PKCE_BASE_URL})
    ...
    def test_pkce_login_without_a_previous_refresh_token_makes_no_revocation_request(self, previous):
        with (
            patch("derouter.proxy.client.cli.commands.auth.run_pkce_login", return_value=_pkce_credential()),
            patch("derouter.proxy.client.cli.commands.auth.load_token", return_value=previous),
            patch("derouter.proxy.client.cli.commands.auth.save_token", return_value=SecretStored()) as save,
            patch("derouter.proxy.client.cli.commands.auth.requests.Session", _FakeSession),
            patch("derouter.proxy.client.cli.interface.show_commands"),
        ):
    ...
    def test_pkce_login_saves_the_refreshable_record_and_skips_the_sso_poll(self):
        with (
            patch("derouter.proxy.client.cli.commands.auth.run_pkce_login", return_value=_pkce_credential()) as run,
            patch("derouter.proxy.client.cli.commands.auth._start_cli_sso_flow") as sso_start,
            patch("derouter.proxy.client.cli.commands.auth.save_token", return_value=SecretStored()) as save,
            patch("derouter.proxy.client.cli.interface.show_commands"),
        ):
    ...
    def test_pkce_login_failure_is_reported_and_nothing_is_saved(self):
        from derouter.proxy.client.cli.commands.pkce_login import PkceFailure

        with (
            patch(
                "derouter.proxy.client.cli.commands.auth.run_pkce_login",
    ...
    def test_login_without_the_flag_never_touches_the_pkce_flow(self):
        with (
            patch("derouter.proxy.client.cli.commands.auth.run_pkce_login") as run,
            patch("derouter.proxy.client.cli.commands.auth._start_cli_sso_flow", side_effect=KeyboardInterrupt),
        ):
...
class TestPkceLogoutCommand:
    ...
    def test_logout_revokes_the_refresh_token_before_clearing(self):
        with (
            patch("derouter.proxy.client.cli.commands.auth.load_token", return_value=_pkce_record()),
            patch("derouter.proxy.client.cli.commands.auth.clear_cli_token", return_value=SecretErased()) as clear,
            patch("derouter.proxy.client.cli.commands.auth.requests.Session", _FakeSession),
        ):
    ...
    def test_logout_still_clears_when_the_proxy_refuses_the_revocation(self):
        ...
        with (
            patch("derouter.proxy.client.cli.commands.auth.load_token", return_value=_pkce_record()),
            patch("derouter.proxy.client.cli.commands.auth.clear_cli_token", return_value=SecretErased()) as clear,
            patch("derouter.proxy.client.cli.commands.auth.requests.Session", _RefusingSession),
        ):
            result = self.runner.invoke(logout)
    ...
    def test_logout_keeps_the_record_when_the_proxy_cannot_record_the_revocation(self):
        ...
        with (
            patch("derouter.proxy.client.cli.commands.auth.load_token", return_value=_pkce_record()),
            patch("derouter.proxy.client.cli.commands.auth.clear_cli_token", return_value=SecretErased()) as clear,
            patch("derouter.proxy.client.cli.commands.auth.requests.Session", _UnavailableSession),
        ):
            result = self.runner.invoke(logout)
    ...
    def test_logout_of_a_classic_token_makes_no_request(self):
        with (
            patch("derouter.proxy.client.cli.commands.auth.load_token", return_value={"key": "sk-classic"}),
            patch("derouter.proxy.client.cli.commands.auth.clear_cli_token", return_value=SecretErased()) as clear,
            patch("derouter.proxy.client.cli.commands.auth.requests.Session", _FakeSession),
        ):
            result = self.runner.invoke(logout)
...
class TestPkcePrintToken:
    ...
    def test_print_token_refreshes_a_near_expiry_key_and_saves_the_rotation(self):
        ...
        with (
            patch("derouter.proxy.client.cli.commands.auth.load_token", return_value=_pkce_record()),
            patch("derouter.proxy.client.cli.commands.auth.save_token") as save,
            patch("derouter.proxy.client.cli.commands.auth.requests.Session", _RefreshingSession),
        ):
            result = self.runner.invoke(print_token, obj={})
    ...
    def test_print_token_prints_a_fresh_pkce_key_without_a_request(self):
        with (
            patch(
                "derouter.proxy.client.cli.commands.auth.load_token",
