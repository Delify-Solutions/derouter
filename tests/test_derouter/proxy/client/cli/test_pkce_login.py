...
from derouter.proxy.client.cli.commands.pkce_login import (
    CallbackCode,
    CallbackDenied,
    CliAuthContract,
    LoopbackServer,
...
DISCOVERY_URL = f"{BASE}/.well-known/derouter-cli-auth"
CONTRACT_DOC = {
    "contract_version": 1,
    "issuer": BASE,
...
def test_loopback_settles_only_on_a_code_for_the_expected_state():
    with LoopbackServer("expected-state") as server:
        ...
        assert status == 200
        assert "Signed in to DeRouter" in body
        thread.join(5)
...
def test_discover_accepts_the_same_proxy_however_the_user_spelled_it(typed_base_url):
    discovery_url = f"{typed_base_url.rstrip('/')}/.well-known/derouter-cli-auth"
    http = _FakeHttp({("GET", discovery_url): _FakeResponse(200, CONTRACT_DOC)})
...
def test_discover_accepts_a_proxy_mounted_under_a_path_only_at_that_path():
    ...
    routes = {
        ("GET", f"{mounted}/.well-known/derouter-cli-auth"): _FakeResponse(200, doc),
