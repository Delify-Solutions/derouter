...
from derouter.proxy.client.cli.commands.autoroute.process import (
    PidRecord,
    ProcessLaunchError,
    UpError,
    clear_pid_record,
...
class TestMissingProxyRuntimeModules:
    def test_flags_absent_modules_only(self, monkeypatch):
        """A thin derouter[cli] install lacks the proxy runtime; the missing ones must be reported
        (by name, for an actionable error) while modules that are importable are not."""
        monkeypatch.setattr(
            process_module,
            "_PROXY_RUNTIME_MODULES",
            ("os", "derouter_autoroute_definitely_absent_pkg", "socket"),
        )

        assert missing_proxy_runtime_modules() == ("derouter_autoroute_definitely_absent_pkg",)
