...
from derouter.proxy.client.cli.commands.autoroute.config import (
...
class TestBuildGeneratedModelList:
    ...
    def test_every_proxy_deployment_points_back_at_customer_proxy(self):
        ...
        for entry in proxy_entries:
            assert entry["derouter_params"]["model"] == f"derouter_proxy/{entry['model_name']}"
            assert entry["derouter_params"]["api_base"] == config.base_url
            assert entry["derouter_params"]["api_key"] == config.api_key
    ...
    def test_no_wildcard_deployment_is_generated(self):
        ...
        # (derouter/router.py:10711-10717), so a "*" entry here would silently never match real
    ...
    def test_complexity_router_config_reflects_llm_classifier(self):
        ...
        router_config = autorouter["derouter_params"]["complexity_router_config"]
    ...
    def test_complexity_router_config_reflects_adaptive(self):
        ...
        autorouter = next(m for m in build_generated_model_list(config) if m["model_name"] == "autorouter")
        assert autorouter["derouter_params"]["complexity_router_config"]["adaptive"] is True
