...
import derouter.proxy.client.cli
...
def test_cli_version_flag(cli_runner):
    ...
    with (
        patch(
            "derouter.proxy.client.health.HealthManagementClient.get_server_version",
            return_value="1.2.3",
        ),
        patch.dict(os.environ, {"DEROUTER_PROXY_URL": "http://localhost:4000"}),
    ):
        result = cli_runner.invoke(cli, ["--version"])
    assert result.exit_code == 0
    assert f"DeRouter Proxy CLI Version: {derouter_version}" in result.output
    assert "DeRouter Proxy Server URL: http://localhost:4000" in result.output
    assert "DeRouter Proxy Server Version: 1.2.3" in result.output
...
def test_cli_source_is_ascii_only():
    ...
    cli_root = Path(derouter.proxy.client.cli.__file__).parent
    offenders = [
...
def isolated_home(monkeypatch, tmp_path):
    """Point HOME at tmp_path so tests never touch the developer's real ~/.derouter."""
...
def test_base_url_read_from_config_file(cli_runner, isolated_home):
    ...
    assert result.exit_code == 0
    assert "DeRouter Proxy Server URL: https://config-proxy.example.com" in result.output


def test_env_var_beats_config_file_base_url(cli_runner, isolated_home, monkeypatch):
    _write_config_file(isolated_home, {"base_url": "https://config-proxy.example.com"})
    monkeypatch.setenv("DEROUTER_PROXY_URL", "http://env-proxy.example.com:5000")

    result = _invoke_version(cli_runner)

    assert result.exit_code == 0
    assert "DeRouter Proxy Server URL: http://env-proxy.example.com:5000" in result.output
...
def test_base_url_flag_beats_env_var_and_config_file(cli_runner, isolated_home, monkeypatch):
    ...
    assert result.exit_code == 0
    assert "DeRouter Proxy Server URL: http://flag-proxy.example.com:9000" in result.output
...
def test_empty_base_url_flag_is_not_treated_as_unset(cli_runner, isolated_home):
    ...
    assert result.exit_code == 0
    assert "DeRouter Proxy Server URL:" not in result.output
...
def test_version_flag_prefers_explicit_base_url_over_config_file(cli_runner, isolated_home):
    ...
    assert result.exit_code == 0
    assert "DeRouter Proxy Server URL: https://flag-proxy.example.com" in result.output
...
def test_version_flag_never_sends_api_key_to_unnamed_server(cli_runner, isolated_home, monkeypatch):
    ...
    _write_config_file(isolated_home, {"base_url": "https://config-proxy.example.com"})
    monkeypatch.setenv("DEROUTER_PROXY_API_KEY", "sk-intended-for-flag-proxy")

    with patch("derouter.proxy.client.http_client.requests.request") as mock_request:
        mock_request.return_value.json.return_value = {"derouter_version": "1.2.3"}
