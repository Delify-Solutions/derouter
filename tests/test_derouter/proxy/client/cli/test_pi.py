...
from derouter.proxy.client.cli.commands.pi import (
    ModelLimits,
    PiSyncError,
    fetch_model_ids,
    fetch_model_limits,
...
class TestSyncModelsJson:
    def test_creates_file_and_parent_dirs(self, tmp_path):
        ...
        assert written["providers"]["derouter"]["baseUrl"] == "http://localhost:4000/v1"
        assert written["providers"]["derouter"]["models"] == [{"id": "m-1"}]
            ...
            json.dumps(
                {
                    ...
                    "providers": {
                        ...
                        "derouter": {"baseUrl": "http://stale:1234/v1", "models": []},
                    },
                }
            )
        )
    ...
    def test_concurrent_syncs_do_not_collide(self, tmp_path):
        ...
        written = json.loads(path.read_text())
        assert written["providers"]["derouter"]["models"] in ([{"id": "m-a"}], [{"id": "m-b"}])
