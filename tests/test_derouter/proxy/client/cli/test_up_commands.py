...
from derouter.proxy.client.cli.commands import up as up_module
from derouter.proxy.client.cli.commands.agents import AgentRunError
from derouter.proxy.client.cli.commands.claude_settings import ClaudeSettingsError
from derouter.proxy.client.cli.commands.up import (
    BackupRecord,
    UpError,
    _ensure_fresh_login,
    down,
...
UP_MODULE = "derouter.proxy.client.cli.commands.up"
AUTH_MODULE = "derouter.proxy.client.cli.commands.auth"
...
class TestResolveApiKeyHelper:
    ...
    def test_windows_keeps_a_spaced_path_and_a_metacharacter_url_as_single_tokens(self, monkeypatch):
        monkeypatch.setattr(shutil, "which", lambda name: "C:\\Program Files\\DeRouter\\lite.EXE")

        helper = resolve_api_key_helper("https://gateway.example.com/?a=1&b=2", platform="win32")

        assert helper == (
            '"C:\\Program Files\\DeRouter\\lite.EXE" "--base-url" "https://gateway.example.com/?a=1&b=2" '
            '"auth" "print-token"'
        )
    ...
    def test_unreachable_proxy_fails_cleanly(self, monkeypatch, tmp_path):
        ...
        with (
            ...
            patch(
                ...
                side_effect=AgentRunError("Could not reach the DeRouter proxy at http://localhost:4000"),
            ),
        ):
            result = self.runner.invoke(up, obj={"base_url": "http://localhost:4000"})

        assert result.exit_code != 0
        assert "Could not reach the DeRouter proxy" in result.output
...
class TestUpCanInvokeTheRealLoginCommand:
    ...
    def test_ctx_invoke_supplies_every_login_parameter(self):
        from derouter.proxy.client.cli.commands.auth import login as real_login
