...
from derouter.proxy.client.cli.commands.agents import (
    AgentRunError,
    ModelSyncSkipped,
    _hand_off,
    _replace_process,
...
AGENTS_MODULE = "derouter.proxy.client.cli.commands.agents"
...
class TestAgentProfile:
    ...
    def test_pi_is_derouter(self):
        assert agent_profile("pi") == ("pi", frozenset({"derouter"}))
...
class TestBuildAgentEnv:
    ...
    def test_derouter_profile_exports_only_the_proxy_key(self):
        env = build_agent_env(
            {}, "http://localhost:4000/", "sk-key", frozenset({"derouter"})
        )
        assert env["DEROUTER_PROXY_API_KEY"] == "sk-key"
        assert "ANTHROPIC_BASE_URL" not in env
        assert "OPENAI_BASE_URL" not in env
        assert "OPENAI_API_KEY" not in env
...
class TestAgentLaunchArgs:
    ...
    def test_codex_points_provider_at_proxy_over_http(self):
        ...
        joined = " ".join(args)
        assert 'model_provider="derouter"' in args
        assert 'model_providers.derouter.base_url="http://localhost:4000/v1"' in args
        assert 'model_providers.derouter.env_key="OPENAI_API_KEY"' in args
        assert 'model_providers.derouter.wire_api="responses"' in args
        assert "model_providers.derouter.supports_websockets=false" in args
...
class TestOpencodeModelSync:
    ...
    def test_declares_proxy_as_derouter_provider_with_listed_models(self):
        ...
        provider = config["provider"]["derouter"]
        assert provider["npm"] == "@ai-sdk/openai-compatible"
        assert provider["name"] == "DeRouter"
    ...
    def test_token_limits_become_opencode_limits(self):
        ...
        models = json.loads(env["OPENCODE_CONFIG_CONTENT"])["provider"]["derouter"]["models"]
    ...
    def test_full_path_opencode_syncs(self):
        ...
        assert "m" in json.loads(env["OPENCODE_CONFIG_CONTENT"])["provider"]["derouter"]["models"]
...
class TestRunAgent:
    ...
    def test_sync_gets_the_launch_inputs_and_runs_after_verify(self):
        ...
        def fake_sync(command, base_env, base_url, api_key, skip_verify):
            ...
            return {"OPENCODE_CONFIG_CONTENT": '{"provider":{"derouter":{}}}'}
    ...
    def test_unreachable_proxy_is_not_asked_for_models(self):
        def failing_verify(*a):
            raise AgentRunError("Could not reach the DeRouter proxy")
    ...
    def test_skip_verify_reaches_the_sync_which_reports_the_skip(self):
        ...
        assert warnings == ["derouter: not syncing OpenCode models from the proxy: offline"]
    ...
    def test_codex_injects_proxy_provider_args_before_user_args(self):
        ...
        assert args.index('model_provider="derouter"') < args.index("exec")
    ...
    def test_pi_prepared_args_precede_user_args_and_env_has_proxy_key(self):
        ...
        run_agent(
            ...
            preparers={"pi": lambda *a: ["--model", "derouter/m-1"]},
        )
        assert calls["args"] == ("pi", "--model", "derouter/m-1", "-p", "hello")
        assert calls["env"]["DEROUTER_PROXY_API_KEY"] == "sk-key"
    ...
    def test_prepare_pi_syncs_models_json_and_pins_first_model(self, tmp_path):
        from derouter.proxy.client.cli.commands.agents import prepare_pi
        ...
        assert pin == ("--model", "derouter/m-first")
        import json

        written = json.loads((tmp_path / "models.json").read_text())
        assert written["providers"]["derouter"]["apiKey"] == "$DEROUTER_PROXY_API_KEY"
        assert written["providers"]["derouter"]["models"] == [
...
class TestHandOff:
    ...
    def test_windows_run_agent_spawns_resolved_binary_with_proxy_args(self):
        ...
        assert '"model_provider=""derouter"""' in command
        assert env["OPENAI_API_KEY"] == "sk-key"
...
class TestAgentCommands:
    ...
    def test_non_interactive_without_key_errors_clearly(self):
        ...
        assert "DEROUTER_PROXY_API_KEY" in result.output
