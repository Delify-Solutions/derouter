...
from derouter.proxy.client.cli.commands.config import (
    get_config_file_path,
    get_config_value,
    load_config,
    save_config,
)
from derouter.derouter_core_utils.private_json import write_private_json
from derouter.proxy.client.cli.interface import show_commands
...
def _config_path(home: Path) -> Path:
    return home / ".derouter" / "config.json"
...
class TestConfigSet:
    ...
    def test_set_base_url_with_path_prefix_accepted(self, cli_runner, isolated_home):
        ...
        result = cli_runner.invoke(cli, ["config", "set", "base_url", "https://proxy.example.com/derouter"])

        assert result.exit_code == 0
        assert json.loads(_config_path(isolated_home).read_text()) == {"base_url": "https://proxy.example.com/derouter"}
...
class TestHiddenCommands:
    ...
    def test_hidden_commands_are_still_invokable(self, cli_runner, isolated_home):
        ...
        with patch("derouter.proxy.client.cli.commands.agents.run_agent") as run_agent_mock:
...
class TestConfigHelpers:
    def test_get_config_file_path_under_home(self, isolated_home):
        assert get_config_file_path() == str(isolated_home / ".derouter" / "config.json")
    ...
    def test_home_unresolvable_does_not_crash_cli(self, cli_runner, isolated_home, monkeypatch):
        ...
        monkeypatch.setattr(
            "derouter.proxy.client.cli.commands.config.get_config_file_path",
...
class TestWritePrivateJson:
    ...
    def test_interrupted_write_removes_temp_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("derouter.derouter_core_utils.private_json.json.dump", _interrupt)
        target = tmp_path / "config.json"
