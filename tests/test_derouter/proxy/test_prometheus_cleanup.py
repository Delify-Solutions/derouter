"""
Tests for derouter.proxy.prometheus_cleanup.wipe_directory and
...
from derouter.proxy.prometheus_cleanup import mark_dead_workers, mark_worker_exit, wipe_directory
from derouter.proxy.proxy_cli import ProxyInitializationHelpers

_WORKER: Final = """
import sys, time
from prometheus_client import Gauge
Gauge("derouter_in_flight", "", multiprocess_mode="livesum").set(float(sys.argv[1]))
...
def _livesum(directory: Path) -> float:
    ...
    value = registry.get_sample_value("derouter_in_flight")
...
class TestMaybeSetupPrometheusMultiprocDir:
    def test_respects_existing_env_var(self, tmp_path):
        ...
        derouter_settings = {"callbacks": ["prometheus"]}
    ...
    def test_handles_string_callbacks(self, derouter_settings):
    ...
    def test_noop_when_setup_not_needed(self, num_workers, derouter_settings):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("PROMETHEUS_MULTIPROC_DIR", None)
            os.environ.pop("prometheus_multiproc_dir", None)
    ...
    def test_auto_creates_dir_when_prometheus_configured(self, derouter_settings):
    ...
    def test_separate_metrics_port_forces_dir_for_single_worker(self, derouter_settings):
