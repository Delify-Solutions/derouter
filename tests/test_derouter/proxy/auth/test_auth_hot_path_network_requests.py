...
def _create_team_object(team_id: str) -> DeRouter_TeamTableCachedObj:
...
def _create_user_object(user_id: str) -> DeRouter_UserTable:
...
async def test_get_team_object_warm_cache():
    ...
    mock_prisma.db.derouter_teamtable.find_unique.assert_not_called()
...
async def test_get_user_object_warm_cache():
    ...
    # DB should NOT have been called
    mock_prisma.db.derouter_usertable.find_unique.assert_not_called()
...
async def test_get_team_membership_warm_cache():
    ...
    mock_prisma.db.derouter_teammembership.find_unique = AsyncMock()
    ...
    # DB should NOT have been called
    mock_prisma.db.derouter_teammembership.find_unique.assert_not_called()
...
async def test_full_hot_path_network_count():
    ...
    team_obj = _create_team_object(team_id)
    user_obj = _create_user_object(user_id)
    membership_data = DeRouter_TeamMembership(
        user_id=user_id,
        team_id=team_id,
        spend=3.0,
        budget_id=None,
...
async def test_get_user_object_missing_user_negative_cache():
    ...
    assert mock_prisma.db.derouter_usertable.find_unique.call_count == 1
...
async def test_get_user_object_missing_user_rechecks_after_expiry():
    ...
    """
    from derouter.proxy.auth.auth_checks import db_cache_expiry, last_db_access_time
    ...
    assert mock_prisma.db.derouter_usertable.find_unique.call_count == 2
