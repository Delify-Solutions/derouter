...
from derouter.proxy.auth.auth_checks import _virtual_key_multi_budget_check
...
async def test_over_first_window_raises():
    ...
    with patch(
        "derouter.proxy.proxy_server.get_current_spend", side_effect=fake_get_spend
    ):
...
async def test_budget_limit_entry_objects_coerced():
    ...
    """
    from derouter.proxy._types import BudgetLimitEntry
