...
import derouter
from derouter.proxy._types import InvitationClaim
...
def _make_invite(
    *, is_accepted: bool, expired: bool = False, claimed: bool = False
    ...
    now = derouter.utils.get_utc_datetime()
    invite = MagicMock()
...
def _make_prisma(invite: MagicMock, user: MagicMock | None = None) -> MagicMock:
    prisma = MagicMock()
    prisma.db.derouter_invitationlink.find_unique = AsyncMock(return_value=invite)
    prisma.db.derouter_invitationlink.update = AsyncMock()
    prisma.db.derouter_invitationlink.update_many = AsyncMock(return_value=1)
...
def _make_onboarding_token(
    *,
    ...
    token_type: str = "derouter_onboarding",
...
async def test_get_token_rejects_already_used_link():
    ...
    """
    from derouter.proxy.proxy_server import onboarding
    ...
    prisma.db.derouter_usertable.find_unique.assert_not_called()
...
async def test_get_token_returns_onboarding_token_without_minting_ui_key():
    ...
    with (
        ...
        patch(
            "derouter.proxy.proxy_server.get_custom_url",
            return_value="http://localhost:4000/",
        ),
        patch(
            "derouter.proxy.proxy_server.get_disabled_non_admin_personal_key_creation",
            return_value=False,
        ),
        patch("derouter.proxy.proxy_server.get_server_root_path", return_value=""),
    ):
    ...
    assert onboarding_claims["token_type"] == "derouter_onboarding"
    ...
    prisma.db.derouter_invitationlink.update_many.assert_not_called()
    prisma.db.derouter_invitationlink.update.assert_not_called()
...
async def test_claim_token_rejects_already_used_link():
    ...
    """
    from derouter.proxy.proxy_server import claim_onboarding_link
...
async def test_claim_token_rejects_concurrent_reuse_before_password_write():
    ...
    invite = _make_invite(is_accepted=False)
    prisma = _make_prisma(invite)
    prisma.db.derouter_invitationlink.update_many = AsyncMock(return_value=0)
...
async def test_claim_token_sets_accepted_at_after_password_written():
    ...
    prisma.db.derouter_invitationlink.update_many.assert_called_once()
    reserve_kwargs = prisma.db.derouter_invitationlink.update_many.call_args.kwargs
    ...
    call_kwargs = prisma.db.derouter_usertable.update.call_args
    ...
    prisma.db.derouter_invitationlink.update.assert_called_once()
    link_update_data = prisma.db.derouter_invitationlink.update.call_args.kwargs["data"]
...
async def test_claim_token_rolls_back_invite_when_session_key_mint_fails():
    ...
    assert prisma.db.derouter_invitationlink.update_many.call_count == 2
    rollback_kwargs = prisma.db.derouter_invitationlink.update_many.call_args_list[
        1
    ].kwargs
