...
from derouter.proxy._types import DeRouter_TeamTable, DeRouter_UserTable, Member
...
def test_get_complete_model_list_expands_team_scoped_wildcard_with_stored_credential(
    ...
    import derouter
    from derouter import Router
    from derouter.proxy.auth import model_checks
    from derouter.proxy.auth.model_checks import get_complete_model_list
    ...
    def fake_get_provider_models(provider, derouter_params=None):
        captured_params["provider"] = provider
        captured_params["api_key"] = derouter_params.api_key
        captured_params["api_base"] = derouter_params.api_base
        captured_params["credential_name"] = derouter_params.derouter_credential_name
        return ["gpt-4o"]
        ...
        model_list=[
            {
                ...
                "derouter_params": {
                    ...
                    "derouter_credential_name": "openai-credential",
                },
                "model_info": {
                    "team_id": "team-1",
...
def test_wildcard_credential_hydration_preserves_deployment_params(
    ...
    def fake_get_provider_models(provider, derouter_params=None):
        captured_params["provider"] = provider
        captured_params["model"] = derouter_params.model
        captured_params["api_key"] = derouter_params.api_key
        captured_params["api_version"] = derouter_params.api_version
        captured_params["credential_name"] = derouter_params.derouter_credential_name
        captured_params["has_unexpected_field"] = hasattr(
            derouter_params, "unexpected_field"
        )
        return ["gpt-4o"]
...
def test_wildcard_custom_prefix_does_not_stack_provider_prefix(monkeypatch):
    ...
    monkeypatch.setattr(
        model_checks,
        "get_provider_models",
        lambda provider, derouter_params=None: ["ollama/gemma3:1b", "ollama/llama3:8b"],
    )
...
def test_wildcard_custom_prefix_keeps_org_segment_for_non_provider_first_segment(
    ...
    If ``get_provider_models`` returns ids whose first segment is an org rather than a derouter
    ...
    monkeypatch.setattr(
        model_checks,
        "get_provider_models",
        lambda provider, derouter_params=None: ["meta-llama/Llama-3-8B"],
    )
...
def test_wildcard_credential_hydration_preserves_missing_credential_name(
    ...
    result = get_known_models_from_wildcard(
        ...
        derouter_params=DeRouter_Params(
            ...
            custom_llm_provider="openai",
            api_key=None,
            derouter_credential_name="missing-credential",
        ),
    )
...
async def test_get_available_models_for_user_expands_query_team_wildcard(
    ...
    def fake_get_provider_models(provider, derouter_params=None):
        assert derouter_params.api_key == "stored-openai-key"
        assert derouter_params.derouter_credential_name is None
        return ["gpt-4o-mini"]
...
def test_expand_wildcard_deployments_non_wildcard_passthrough():
    ...
    deployment = {"model_name": "gpt-4o", "derouter_params": {"model": "gpt-4o"}}
...
def test_expand_wildcard_deployments_openai_wildcard():
    ...
    with patch(
        "derouter.proxy.auth.model_checks.get_known_models_from_wildcard",
    ...
    assert all(r["derouter_params"]["model"] in fake_models for r in result)


def test_expand_wildcard_concrete_model_name_with_wildcard_derouter_params():
    """Concrete model_name must not be overwritten when only derouter_params.model is wildcard."""
    from derouter.proxy.auth.model_checks import (
        expand_wildcard_deployments_for_model_info,
    )
...
def test_expand_wildcard_invalid_derouter_params_passthrough():
    """Deployments with invalid derouter_params must pass through unchanged (no 500)."""
    from derouter.proxy.auth.model_checks import (
        expand_wildcard_deployments_for_model_info,
    )
    ...
    # Even if DeRouter_Params construction fails the deployment should survive
    result = expand_wildcard_deployments_for_model_info([deployment])
    assert result == [deployment]
...
def test_get_complete_model_list_excludes_wildcard_routes_by_default():
    ...
    router = Router(
        model_list=[
            ...
            {
                "model_name": "gpt-4",
                "derouter_params": {"model": "openai/gpt-4"},
            },
        ]
    )
...
def test_add_known_models_refreshes_models_by_provider_for_wildcard_expansion():
    ...
    captured_reference = derouter.models_by_provider
    assert fake_model not in derouter.models_by_provider["vertex_ai"]
    try:
        derouter.add_known_models(
            model_cost_map={
                fake_model: {"derouter_provider": "vertex_ai-language-models", "mode": "chat"}
            }
        )
        assert fake_model in derouter.models_by_provider["vertex_ai"]
        assert derouter.models_by_provider is captured_reference
        assert fake_model in captured_reference["vertex_ai"]
        assert fake_model in get_known_models_from_wildcard("vertex_ai/*")
    finally:
        derouter.vertex_language_models.discard(fake_model)
        derouter.add_known_models(model_cost_map={})
    assert fake_model not in derouter.models_by_provider["vertex_ai"]
...
def test_azure_ai_wildcard_lists_the_foundry_gpt_6_astra_entry(monkeypatch):
    ...
    local_entry = derouter.get_model_cost_map(url="")[foundry_key]
    registered_before = foundry_key in derouter.azure_ai_models
    try:
        derouter.add_known_models(model_cost_map={foundry_key: local_entry})
        assert foundry_key in get_known_models_from_wildcard("azure_ai/*")
    finally:
        if not registered_before:
            derouter.azure_ai_models.discard(foundry_key)
            derouter.add_known_models(model_cost_map={})
