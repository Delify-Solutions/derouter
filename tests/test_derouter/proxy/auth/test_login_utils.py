...
from derouter.proxy.auth.login_utils import (
    LoginResult,
    authenticate_user,
    get_ui_credentials,
)
...
async def test_authenticate_user_admin_login_with_ui_credentials():
    ...
    with patch.dict(
        ...
        with patch(
            "derouter.proxy.auth.login_utils.generate_key_helper_fn",
            ...
            with patch(
                "derouter.proxy.auth.login_utils.user_update",
                new_callable=AsyncMock,
                return_value=None,
            ) as mock_user_update:
                with patch(
                    "derouter.proxy.auth.login_utils.get_secret_bool",
                    return_value=False,
                ):
                    ...
                    assert result.user_id == DEROUTER_PROXY_ADMIN_NAME
...
async def test_authenticate_user_wrong_password():
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_usertable.find_first = AsyncMock(return_value=mock_user)
...
async def test_authenticate_user_email_case_insensitive_login():
    ...
    # `DeRouter_UserTable` does not define a `password` field, but `authenticate_user()`
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_usertable.find_first = AsyncMock(side_effect=mock_find_first)
    ...
    calls = mock_prisma_client.db.derouter_usertable.find_first.await_args_list
    assert len(calls) == 2
...
async def test_authenticate_user_multiple_logins_generate_unique_tokens():
    ...
    with patch.dict(
        ...
        with patch(
            ...
            mock_generate_key.side_effect = [
                {"token": "session-token-1", "user_id": DEROUTER_PROXY_ADMIN_NAME},
                {"token": "session-token-2", "user_id": DEROUTER_PROXY_ADMIN_NAME},
                {"token": "session-token-3", "user_id": DEROUTER_PROXY_ADMIN_NAME},
            ]
...
class TestEncodeUiSessionJwt:
    ...
    def test_encoded_cookie_carries_bounded_exp(self):
        ...
        with patch("derouter.proxy.auth.login_utils.DEROUTER_UI_SESSION_DURATION", "24h"):
    ...
    def test_duration_is_honored_from_env(self):
        ...
        with patch("derouter.proxy.auth.login_utils.DEROUTER_UI_SESSION_DURATION", "1h"):
    ...
    def test_cookie_is_accepted_by_the_exp_requiring_session_reader(self):
        ...
        from derouter.proxy._experimental.mcp_server.byok_oauth_endpoints import (
        ...
        with patch("derouter.proxy.proxy_server.master_key", "sk-master-for-tests"):
...
def _patch_sso_configured(stack: ExitStack, *, configured: bool) -> None:
    stack.enter_context(
        patch(  # test-quality-ok: no HTTP boundary here; same internal the pre-existing tests above already mock
            "derouter.proxy.auth.login_utils.is_sso_provider_fully_configured", return_value=configured
        )
    )
...
def _patch_successful_admin_login_deps(stack: ExitStack) -> None:
    ...
    stack.enter_context(
        patch(  # test-quality-ok: internal orchestration, no HTTP boundary; matches pre-existing tests
            "derouter.proxy.auth.login_utils.generate_key_helper_fn",
            new_callable=AsyncMock,
            return_value={"token": "test-token", "user_id": DEROUTER_PROXY_ADMIN_NAME},
        )
    )
    stack.enter_context(
        patch(  # test-quality-ok: internal orchestration, no HTTP boundary; matches pre-existing tests
...
class TestDisablePasswordLoginWhenSSOEnabled:
    ...
    async def test_rejects_correct_admin_credentials_when_sso_configured(self):
        ...
        mock_prisma_client.db.derouter_usertable.find_first.assert_not_called()
