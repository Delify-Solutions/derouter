...
class TestUnmappedModelBudgetEnforcement:
    ...
    def setup_method(self):
        """Snapshot derouter.model_cost before each test."""
        self._saved_model_cost = copy.deepcopy(derouter.model_cost)

    def teardown_method(self):
        """Restore derouter.model_cost after each test."""
        derouter.model_cost = self._saved_model_cost

    def test_unmapped_model_enforces_budget(self):
        """A model not in derouter.model_cost should have budget enforced."""
        router = Router(
            model_list=[
                {
    ...
    def test_unmapped_model_with_derouter_params_pricing(self):
        """A model with cost=0 in derouter_params (not model_info) should bypass budget."""
        router = Router(
            model_list=[
                {
        ...
        assert (
            result is True
        ), "Model with explicit cost=0 in derouter_params should bypass budget"
    ...
    def test_strategy_router_alias_with_zero_pricing_enforces_budget(self):
        ...
        assert "input_cost_per_token" not in derouter.model_cost.get("alias-id", {})
