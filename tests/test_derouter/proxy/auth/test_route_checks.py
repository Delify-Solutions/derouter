...
from derouter.proxy.auth.auth_checks_organization import _user_is_org_admin
...
def test_virtual_key_allowed_routes_with_derouter_routes_member_name_allowed():
    """Test that virtual key is allowed to call routes when allowed_routes contains DeRouterRoutes member name"""

    # Create a UserAPIKeyAuth with allowed_routes containing a DeRouterRoutes member name
    valid_token = UserAPIKeyAuth(
        user_id="test_user",
        allowed_routes=["openai_routes"],  # This is a member name in DeRouterRoutes enum
    )

    # Test that a route from the openai_routes group is allowed
    result = RouteChecks.is_virtual_key_allowed_to_call_route(
        route="/chat/completions",  # This is in DeRouterRoutes.openai_routes.value
        valid_token=valid_token,
    )
...
def test_model_info_not_classified_as_llm_api(route):
    ...
    x-derouter-team-id attachment; model metadata is a free read and must stay
    outside all of them.
    """
...
def test_realtime_webrtc_http_routes_classified_as_llm_api(route):
    ...
    Regression test for https://github.com/Delify-Solutions/derouter/issues/29923
    """
...
def test_virtual_key_allowed_routes_with_derouter_routes_member_name_denied():
    """Test that virtual key is denied when route is not in the allowed DeRouterRoutes group"""

    # Create a UserAPIKeyAuth with allowed_routes containing a DeRouterRoutes member name
    valid_token = UserAPIKeyAuth(
        user_id="test_user",
        allowed_routes=["info_routes"],  # This is a member name in DeRouterRoutes enum
    )

    # Test that a route NOT in the info_routes group raises an HTTPException
    with pytest.raises(HTTPException) as exc_info:
        RouteChecks.is_virtual_key_allowed_to_call_route(
            route="/chat/completions",  # This is NOT in DeRouterRoutes.info_routes.value
            valid_token=valid_token,
        )
...
def test_virtual_key_llm_api_route_includes_passthrough_prefix(route):
    ...
    Relevant issue: https://github.com/Delify-Solutions/derouter/issues/14017
    """
...
def test_virtual_key_allowed_routes_with_multiple_derouter_routes_member_names():
    """Test that virtual key works with multiple DeRouterRoutes member names in allowed_routes"""

    # Create a UserAPIKeyAuth with multiple DeRouterRoutes member names
    valid_token = UserAPIKeyAuth(
...
def test_virtual_key_allowed_routes_with_mixed_member_names_and_explicit_routes():
    """Test that virtual key works with both DeRouterRoutes member names and explicit routes"""
...
def test_virtual_key_allowed_routes_with_no_member_names_only_explicit():
    ...
    # Create a UserAPIKeyAuth with only explicit routes (no DeRouterRoutes member names)
    valid_token = UserAPIKeyAuth(
        user_id="test_user",
...
def test_vector_store_crud_accessible_to_internal_roles(user_role, method, route):
    ...
    RouteChecks.non_proxy_admin_allowed_routes_check(
        user_obj=DeRouter_UserTable(user_id="test_user", user_role=user_role),
...
def test_videos_route_accessible_to_internal_users():
    ...
    https://github.com/Delify-Solutions/derouter/issues/16470
...
# Sourced from `DeRouterRoutes.global_spend_tracking_routes` so any future
# additions to that list are exercised by these tests automatically.

GLOBAL_SPEND_ROUTES = DeRouterRoutes.global_spend_tracking_routes.value
...
# (ui/derouter-dashboard/src/components/view_logs/) — see the comment on each.
...
class TestModelsRouteExemptFromDisableLLMEndpoints:
    ...
    Relevant issue: https://github.com/Delify-Solutions/derouter/issues/new (UI breaks with DISABLE_LLM_ENDPOINTS)
    """
...
def test_route_in_additional_public_routes_wildcard_match():
    ...
    """
    from derouter.proxy.auth.auth_utils import route_in_additonal_public_routes

    with (
        patch(
            "derouter.proxy.proxy_server.general_settings", {"public_routes": ["/api/*"]}
        ),
        patch("derouter.proxy.proxy_server.premium_user", True),
    ):
...
def _make_org_admin_user(org_id: str) -> DeRouter_UserTable:
    membership = DeRouter_OrganizationMembershipTable(
...
from derouter.proxy.auth.auth_checks_organization import (
...
def test_patch_team_route_has_same_reach_as_team_update():
    ...
    assert RouteChecks.check_route_access(
        route="/team/abc-123", allowed_routes=DeRouterRoutes.org_admin_allowed_routes.value
    )
    assert not RouteChecks.check_route_access(
        route="/team/abc-123", allowed_routes=DeRouterRoutes.internal_user_routes.value
    )
    assert not RouteChecks.check_route_access(
        route="/team/abc-123", allowed_routes=DeRouterRoutes.self_managed_routes.value
    )
...
async def test_initialize_pass_through_registers_wildcard_for_auth_subpath():
    ...
    original_routes = DeRouterRoutes.openai_routes.value[:]
    try:
        with (
            patch(
                "derouter.proxy.proxy_server.app",
                MagicMock(),
            ),
            patch(
            ...
            assert base_path in DeRouterRoutes.openai_routes.value
            assert wildcard_path in DeRouterRoutes.openai_routes.value
            ...
            await initialize_pass_through_endpoints([endpoint_config])
            assert DeRouterRoutes.openai_routes.value.count(base_path) == 1
            assert DeRouterRoutes.openai_routes.value.count(wildcard_path) == 1
            ...
            assert base_path not in DeRouterRoutes.openai_routes.value
            assert wildcard_path not in DeRouterRoutes.openai_routes.value
    finally:
        DeRouterRoutes.openai_routes.value[:] = original_routes
...
def test_organization_daily_activity_not_granted_by_org_admin_request_data_branch():
    ...
    assert not RouteChecks.check_route_access(
        route="/organization/daily/activity",
        allowed_routes=DeRouterRoutes.org_admin_only_routes.value,
    )
...
def test_agent_routes_union_still_covers_both_halves(route):
    ...
    assert (
        RouteChecks.check_route_access(
            route=route, allowed_routes=DeRouterRoutes.agent_routes.value
        )
        is True
    )
...
def test_team_callback_routes_are_self_managed():
    ...
    for template in (
        ...
        assert template in DeRouterRoutes.self_managed_routes.value
