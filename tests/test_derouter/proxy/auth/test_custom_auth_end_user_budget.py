...
async def test_custom_auth_run_post_custom_auth_checks_with_end_user_budget_exceeded():
    ...
    with patch(
        ...
        with patch(
            "derouter.proxy.proxy_server.model_max_budget_limiter.is_end_user_within_model_budget",
            new_callable=AsyncMock,
        ) as mock_budget_check:
            mock_budget_check.side_effect = derouter.BudgetExceededError(
...
async def test_custom_auth_enforces_end_user_budget_when_common_checks_skipped():
    ...
    over_budget_end_user = DeRouter_EndUserTable(
        user_id="customer-1",
        blocked=False,
        spend=0.0,
...
async def test_custom_auth_defers_end_user_budget_to_common_checks_when_enabled():
    ...
    with (
        ...
        patch(
            "derouter.proxy.auth.user_api_key_auth._check_end_user_budget",
            new_callable=AsyncMock,
        ) as mock_check,
        patch(
...
async def test_custom_auth_token_budget_still_loads_and_caches_unrestricted_end_user():
    ...
    from derouter.proxy.auth.user_api_key_auth import _lookup_end_user_and_apply_budget
