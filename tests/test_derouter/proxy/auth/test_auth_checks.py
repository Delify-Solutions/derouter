...
from derouter.caching.redis_cache import RedisCache
...
from derouter.proxy.common_utils.encrypt_decrypt_utils import decrypt_value_helper
...
from derouter.utils import get_utc_datetime
...
def set_salt_key(monkeypatch):
    """Automatically set DEROUTER_SALT_KEY for all tests"""
    monkeypatch.setenv("DEROUTER_SALT_KEY", "sk-1234")
...
def reset_constants_module():
    ...
    from derouter import constants
    from derouter.proxy.auth import auth_checks
...
def test_get_experimental_ui_login_jwt_auth_token_valid(valid_sso_user_defined_values):
    ...
    assert token_data["max_budget"] == derouter.max_ui_session_budget
...
def test_get_experimental_ui_login_jwt_auth_token_uses_10_min_expiry(
    ...
    """Test that Experimental UI token uses fixed 10-minute expiry (does not use DEROUTER_UI_SESSION_DURATION)."""
    token = ExperimentalUIJWTToken.get_experimental_ui_login_jwt_auth_token(valid_sso_user_defined_values)
    decrypted_token = decrypt_value_helper(token, key="ui_hash_key", exception_type="debug")
    assert decrypted_token is not None
    token_data = json.loads(decrypted_token)
...
def test_experimental_ui_token_ignores_derouter_ui_session_duration(
    valid_sso_user_defined_values,
):
    """Regression test: DEROUTER_UI_SESSION_DURATION must NOT affect Experimental UI token expiry.
    ...
    # Default DEROUTER_UI_SESSION_DURATION is "24h" - token must still expire in ~10 min
    token = ExperimentalUIJWTToken.get_experimental_ui_login_jwt_auth_token(valid_sso_user_defined_values)
    decrypted_token = decrypt_value_helper(token, key="ui_hash_key", exception_type="debug")
    assert decrypted_token is not None
    token_data = json.loads(decrypted_token)
    expires = datetime.fromisoformat(token_data["expires"].replace("Z", "+00:00"))
    now = get_utc_datetime()
    # Must be ~10 min, NOT 24h. If DEROUTER_UI_SESSION_DURATION were incorrectly used, this would fail.
    assert expires <= now + timedelta(minutes=11), (
        "Experimental UI must use 10-min expiry, not DEROUTER_UI_SESSION_DURATION"
    )
...
def test_get_key_object_from_ui_hash_key_valid(valid_sso_user_defined_values, monkeypatch):
    ...
    assert key_object.max_budget == derouter.max_ui_session_budget
...
async def test_can_user_call_model_no_default_models_returns_forbidden():
    ...
    from derouter.proxy.auth.auth_checks import can_user_call_model

    user_object = DeRouter_UserTable(
        user_id="test-user",
...
async def test_can_key_call_model_all_team_models_uses_team_allowlist():
    from derouter.proxy._types import SpecialModelNames
    from derouter.proxy.auth.auth_checks import can_key_call_model
...
def test_resolve_key_models_teamless_all_team_models_returns_empty():
    ...
    from derouter.proxy._types import SpecialModelNames
    from derouter.proxy.auth.auth_checks import _resolve_key_models_for_auth_check
...
async def test_enforce_key_access_teamless_all_team_models_passes():
    ...
    from derouter.proxy._types import SpecialModelNames
    from derouter.proxy.auth.user_api_key_auth import _enforce_key_and_fallback_model_access
...
async def test_can_key_call_resolved_model_teamless_all_team_models_passes():
    ...
    from derouter.proxy._types import SpecialModelNames
    from derouter.proxy.auth.auth_checks import can_key_call_resolved_model
    ...
    with patch("derouter.proxy.auth.auth_checks.can_key_call_model", new_callable=AsyncMock) as mock_call:
        with patch("derouter.proxy.proxy_server.prisma_client", None):
            with patch("derouter.proxy.proxy_server.proxy_logging_obj", None):
                with patch("derouter.proxy.proxy_server.user_api_key_cache", None):
...
async def test_can_team_access_model_all_team_models_expands_router_models():
    ...
    from derouter.proxy._types import SpecialModelNames
    from derouter.proxy.auth.auth_checks import can_team_access_model
...
def test_get_cli_jwt_auth_token_custom_expiration(valid_sso_user_defined_values, monkeypatch):
    ...
    monkeypatch.setenv("DEROUTER_CLI_JWT_EXPIRATION_HOURS", "48")
...
def test_get_cli_jwt_auth_token_unique_per_session(valid_sso_user_defined_values):
    ...
    from derouter.constants import CLI_SESSION_KEY_PREFIX
...
def test_get_cli_jwt_auth_token_applies_fallback_budget(valid_sso_user_defined_values):
    token = ExperimentalUIJWTToken.get_cli_jwt_auth_token(
        valid_sso_user_defined_values, max_budget=derouter.max_ui_session_budget
    )
    decrypted = decrypt_value_helper(token, key="ui_hash_key", exception_type="debug")
    assert decrypted is not None
    assert json.loads(decrypted).get("max_budget") == derouter.max_ui_session_budget
...
async def test_default_internal_user_params_with_get_user_object(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "default_internal_user_params", default_params)
    ...
    mock_prisma_client.db.derouter_usertable.create = AsyncMock(return_value=mock_user)
    ...
    try:
        ...
        # this fails since the mock object is a MagicMock and not a DeRouter_UserTable
        print(e)

    # Verify the user was created with the default params
    mock_prisma_client.db.derouter_usertable.create.assert_called_once()
    creation_args = mock_prisma_client.db.derouter_usertable.create.call_args[1]["data"]
...
async def test_get_user_object_upsert_sets_budget_reset_at(monkeypatch, has_budget_duration):
    ...
    mock_prisma_client.db.derouter_usertable.find_unique = AsyncMock(return_value=None)
    mock_prisma_client.db.derouter_usertable.find_first = AsyncMock(return_value=None)
    mock_prisma_client.db.derouter_usertable.create = AsyncMock(return_value=MagicMock(organization_memberships=[]))
...
async def test_get_user_object_wraps_db_outage_as_valueerror_preserving_context():
    ...
    with patch("derouter.proxy.auth.auth_checks._should_check_db", return_value=True):
...
async def test_get_user_object_upsert_includes_user_email():
    ...
    try:
        ...
        # May fail since mock object is not a real DeRouter_UserTable
        print(e)
...
async def test_get_user_object_backfills_null_email_from_cache_hit():
    ...
    """
    cache = UserApiKeyCache()
    existing = DeRouter_UserTable(user_id="jwt-user-1", user_email=None, user_role="internal_user")
    await cache.async_set_cache(key="jwt-user-1", value=existing, model_type=DeRouter_UserTable)

    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_usertable.update_many = AsyncMock(return_value=1)
    mock_prisma_client.db.derouter_usertable.find_unique = AsyncMock(
    ...
    mock_prisma_client.db.derouter_usertable.update_many.assert_called_once()
    update_kwargs = mock_prisma_client.db.derouter_usertable.update_many.call_args.kwargs
    assert update_kwargs["where"] == {"user_id": "jwt-user-1", "user_email": None}
    assert update_kwargs["data"]["user_email"] == "jwt-user-1@example.com"

    refreshed = await cache.async_get_cache(key="jwt-user-1", model_type=DeRouter_UserTable)
...
async def test_get_user_object_backfills_null_email_from_db_read():
    ...
    """
    cache = UserApiKeyCache()
    db_row = DeRouter_UserTable(user_id="jwt-user-3", user_email=None, user_role="internal_user")
    backfilled_row = DeRouter_UserTable(
        user_id="jwt-user-3",
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_usertable.find_unique = AsyncMock(side_effect=[db_row, backfilled_row])
    mock_prisma_client.db.derouter_usertable.find_first = AsyncMock(return_value=None)
    mock_prisma_client.db.derouter_usertable.update_many = AsyncMock(return_value=1)
    ...
    refreshed = await cache.async_get_cache(key="jwt-user-3", model_type=DeRouter_UserTable)
    assert refreshed is not None
...
async def test_get_user_object_does_not_overwrite_existing_email():
    ...
    """
    cache = UserApiKeyCache()
    existing = DeRouter_UserTable(
        user_id="jwt-user-2",
        user_email="operator-set@example.com",
        user_role="internal_user",
    )
    await cache.async_set_cache(key="jwt-user-2", value=existing, model_type=DeRouter_UserTable)

    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_usertable.update_many = AsyncMock(return_value=0)
    ...
    mock_prisma_client.db.derouter_usertable.update_many.assert_not_called()
...
async def test_get_user_object_backfill_race_prefers_db_email():
    ...
    """
    cache = UserApiKeyCache()
    existing = DeRouter_UserTable(user_id="jwt-user-4", user_email=None, user_role="internal_user")
    await cache.async_set_cache(key="jwt-user-4", value=existing, model_type=DeRouter_UserTable)

    winner_row = DeRouter_UserTable(
        user_id="jwt-user-4",
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_usertable.update_many = AsyncMock(return_value=0)
    mock_prisma_client.db.derouter_usertable.find_unique = AsyncMock(return_value=winner_row)
    ...
    refreshed = await cache.async_get_cache(key="jwt-user-4", model_type=DeRouter_UserTable)
    assert refreshed is not None
...
async def test_get_user_object_backfill_caches_persisted_email_not_proposed():
    ...
    """
    cache = UserApiKeyCache()
    existing = DeRouter_UserTable(user_id="jwt-user-5", user_email=None, user_role="internal_user")
    await cache.async_set_cache(key="jwt-user-5", value=existing, model_type=DeRouter_UserTable)

    persisted_row = DeRouter_UserTable(
        user_id="jwt-user-5",
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_usertable.update_many = AsyncMock(return_value=1)
    mock_prisma_client.db.derouter_usertable.find_unique = AsyncMock(return_value=persisted_row)
    ...
    refreshed = await cache.async_get_cache(key="jwt-user-5", model_type=DeRouter_UserTable)
    assert refreshed is not None
...
async def test_get_user_object_upsert_routes_default_team_to_membership(monkeypatch):
    ...
    with patch(
        "derouter.proxy.management_endpoints.internal_user_endpoints.add_new_user_to_default_team",
        ...
        try:
            ...
            # mock_user is a MagicMock, so the post-create DeRouter_UserTable(**dict(...))
...
def test_log_budget_lookup_failure_dry_run():
    ...
    with patch("derouter.proxy.auth.auth_checks.verbose_proxy_logger") as mock_logger:
...
@pytest.mark.asyncio
@patch("derouter.proxy.management_endpoints.team_endpoints.new_team", new_callable=AsyncMock)
async def test_get_team_db_check_calls_new_team_on_upsert(mock_new_team, monkeypatch):
    ...
    mock_db = AsyncMock()
    mock_prisma_client.db = mock_db
    mock_prisma_client.db.derouter_teamtable.find_unique.return_value = None
...
async def test_get_team_db_check_does_not_call_new_team_if_exists(mock_new_team, monkeypatch):
    ...
    mock_db = AsyncMock()
    mock_prisma_client.db = mock_db
    mock_prisma_client.db.derouter_teamtable.find_unique.return_value = MagicMock()
...
async def test_vector_store_access_check_early_returns(prisma_client, vector_store_registry, expected_result):
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client", prisma_client),
        patch("derouter.vector_store_registry", vector_store_registry),
    ):
...
async def test_vector_store_access_check_skips_db_lookup_when_no_vector_stores_requested():
    ...
    mock_prisma_client.db.derouter_objectpermissiontable.find_unique = find_unique
    ...
    with (
        ...
        patch(  # test-quality-ok: production auth reads this module global; no dependency injection seam exists
            "derouter.vector_store_registry", mock_vector_store_registry
        ),
    ):
        result = await vector_store_access_check(
            request_body={"messages": [{"role": "user", "content": "test"}]},
...
async def test_vector_store_access_check_with_permissions():
    ...
    mock_prisma_client.db.derouter_objectpermissiontable.find_unique = AsyncMock(return_value=mock_permissions)
    ...
    with (
        ...
        patch("derouter.vector_store_registry", mock_vector_store_registry),
    ):
        result = await vector_store_access_check(
            request_body=request_body,
            team_object=None,
...
async def test_vector_store_access_check_with_team_permissions():
    ...
    mock_prisma_client.db.derouter_objectpermissiontable.find_unique = AsyncMock(return_value=team_permissions)
...
def test_can_object_call_model_with_alias():
    ...
    from derouter import Router
    from derouter.proxy.auth.auth_checks import _can_object_call_model
...
async def test_check_team_member_model_access_with_access_group():
    ...
    from derouter.proxy.auth.auth_checks import _check_team_member_model_access

    router = _make_team_scoped_router()
    team = DeRouter_TeamTable(team_id="team-a")
    ...
    with patch(
        "derouter.proxy.auth.auth_checks.get_team_membership",
...
async def test_get_tag_objects_batch():
    ...
    """
    from derouter.proxy._types import DeRouter_TagTable
    from derouter.proxy.auth.auth_checks import get_tag_objects_batch
    ...
    # Mock cached tags — must be DeRouter_TagTable instances: the mocked async_get_cache
    ...
    cached_tag_1 = DeRouter_TagTable(
        tag_name="cached-1",
        spend=10.0,
        models=[],
        derouter_budget_table=None,
    )
    cached_tag_2 = DeRouter_TagTable(
        tag_name="cached-2",
        spend=20.0,
        models=[],
        derouter_budget_table=None,
    ...
    uncached_tag_1.derouter_budget_table = None
    ...
    uncached_tag_2.derouter_budget_table = None
    ...
    uncached_tag_3.derouter_budget_table = None
    ...
    mock_prisma.db.derouter_tagtable.find_many = AsyncMock(return_value=[uncached_tag_1, uncached_tag_2, uncached_tag_3])
    ...
    assert mock_prisma.db.derouter_tagtable.find_many.call_count == 2
    registry_call, batch_call = mock_prisma.db.derouter_tagtable.find_many.call_args_list
...
async def test_get_tag_objects_batch_never_queries_db_for_unregistered_tags():
    """
    Regression: a request tag with no DeRouter_TagTable row must not cost a DB read per request.
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_tagtable.find_many = AsyncMock(return_value=[_tag_registry_row("some-other-tag")])
    cache = UserApiKeyCache()
    ...
    mock_prisma.db.derouter_tagtable.find_many.assert_called_once_with(take=TAG_REGISTRY_MAX_SIZE + 1)
    ...
    assert second == {}
    assert mock_prisma.db.derouter_tagtable.find_many.call_count == 1
...
async def test_get_tag_objects_batch_fetches_only_registered_uncached_tags():
    ...
    await cache.async_set_cache(
        ...
        value=DeRouter_TagTable(tag_name="cached-tag", spend=7.0, models=[]),
        model_type=DeRouter_TagTable,
    )
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_tagtable.find_many = AsyncMock(side_effect=fake_find_many)
    ...
    batch_calls = _batch_calls(mock_prisma.db.derouter_tagtable.find_many)
...
async def test_get_tag_objects_batch_caches_empty_registry():
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_tagtable.find_many = AsyncMock(return_value=[])
    cache = UserApiKeyCache()
...
async def test_get_tag_objects_batch_registry_db_error_negative_caches_and_keeps_per_tag_fetch():
    ...
    assert len(_registry_calls(mock_prisma.db.derouter_tagtable.find_many)) == 1
    ...
    assert len(_registry_calls(mock_prisma.db.derouter_tagtable.find_many)) == 2
...
async def test_get_tag_objects_batch_oversized_registry_falls_back_and_stops_refetching():
    ...
    find_many = mock_prisma.db.derouter_tagtable.find_many
...
async def test_tag_max_budget_check_still_enforces_registered_tag_over_budget():
    ...
    with patch("derouter.proxy.proxy_server.get_current_spend", mock_get_current_spend):
...
async def test_get_team_object_raises_404_when_not_found():
    ...
    mock_db = AsyncMock()
    mock_prisma_client.db = mock_db
    mock_prisma_client.db.derouter_teamtable.find_unique = AsyncMock(return_value=None)
...
def _mock_prisma_for_team_lookup(find_unique):
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_teamtable.find_unique = find_unique
...
async def test_get_team_object_distinguishes_absent_team_from_unreadable_row():
    ...
    from derouter.proxy.auth.auth_checks import TeamNotFoundError, get_team_object
...
async def test_common_checks_metadata_route_keeps_key_tags_out_of_provider_metadata(
    ...
    """GH#30629: on routes that track tags in derouter_metadata (bedrock, /v1/messages,
    responses, ...) key-level tags must land in derouter_metadata, never in the
    provider-facing metadata field (Bedrock rejects non-user_id metadata with HTTP 400).
    The auth-time pre-seed keys off DEROUTER_METADATA_ROUTES, so hardcoding a single route
    ...
    assert result is True
    assert request_body["derouter_metadata"]["tags"] == ["engineering"]
...
def _pass_through_request() -> Request:
    ...
    from derouter.types.passthrough_endpoints.pass_through_endpoints import (
        DEROUTER_PASS_THROUGH_ENDPOINT_MARKER,
    )

    def pass_through_endpoint(): ...

    setattr(pass_through_endpoint, DEROUTER_PASS_THROUGH_ENDPOINT_MARKER, True)
...
async def test_common_checks_auth_enforced_pass_through_ignores_upstream_model():
    ...
    team_object = DeRouter_TeamTable(team_id="team-1", models=["gpt-4o"])
    valid_token = UserAPIKeyAuth(
        token="test-token",
        team_id="team-1",
        models=[],
...
async def test_virtual_key_max_budget_alert_check_global_fallback():
    """Test that derouter.default_key_max_budget_alert_emails is used when key metadata has no map"""
    alert_triggered = False
    captured_call_info = None
    ...
    original = derouter.default_key_max_budget_alert_emails
    try:
        derouter.default_key_max_budget_alert_emails = global_config
        await _virtual_key_max_budget_alert_check(
            valid_token=valid_token,
            proxy_logging_obj=MockProxyLogging(),
            user_obj=None,
        ...
        derouter.default_key_max_budget_alert_emails = original
...
async def test_get_fuzzy_user_object_case_insensitive_email():
    ...
    mock_prisma.db.derouter_usertable.find_unique = AsyncMock(return_value=None)
    mock_prisma.db.derouter_usertable.find_first = AsyncMock(return_value=test_user)
    ...
    mock_prisma.db.derouter_usertable.find_first.assert_called_once()
    call_args = mock_prisma.db.derouter_usertable.find_first.call_args
...
async def test_custom_auth_common_checks_opt_in():
    ...
    """
    import derouter.proxy.proxy_server as _proxy_server_mod
    from derouter.proxy.auth.user_api_key_auth import _run_centralized_common_checks
    ...
    try:
        ...
        with patch(
            "derouter.proxy.auth.user_api_key_auth.common_checks",
            new_callable=AsyncMock,
        ) as mock_common:
...
def _patched_spend(value: float):
    ...
    return patch("derouter.proxy.proxy_server.get_current_spend", mock_get_current_spend)
...
def test_throttled_limit(limit, pct, expected):
    from derouter.proxy.auth.budget_throttle import throttled_limit
...
async def test_budget_exceeded_throttles_instead_of_blocking(monkeypatch):
    monkeypatch.setattr(derouter, "budget_exceeded_throttle_percentage", 0.1)
...
async def test_budget_throttle_decision_cleared_before_caching():
    ...
    from derouter.proxy.auth.auth_checks import _copy_user_api_key_auth_for_cache
...
async def test_budget_exceeded_invalid_percentage_blocks(monkeypatch, pct):
    monkeypatch.setattr(derouter, "budget_exceeded_throttle_percentage", pct)
    valid_token = _over_budget_token(
        tpm_limit=1000,
        rpm_limit=100,
        metadata={"throttle_on_budget_exceeded": True},
...
async def test_end_user_budget_check_reads_from_spend_counter():
    ...
    end_user_object = DeRouter_EndUserTable(
        user_id="customer-1",
        blocked=False,
        spend=0.0,
        derouter_budget_table=DeRouter_BudgetTable(max_budget=1.0),
    )
...
async def test_tag_budget_check_reads_from_spend_counter():
    ...
    with (
        patch("derouter.proxy.proxy_server.get_current_spend", mock_get_current_spend),
        patch(
            "derouter.proxy.auth.auth_checks.get_tag_objects_batch",
            new_callable=AsyncMock,
...
async def test_team_member_budget_check_reads_from_spend_counter():
    ...
    from derouter.proxy._types import DeRouter_BudgetTable, DeRouter_TeamMembership
    from derouter.proxy.utils import ProxyLogging

    team_object = DeRouter_TeamTable(team_id="test-team")
    user_object = DeRouter_UserTable(user_id="test-user")
    valid_token = UserAPIKeyAuth(
        token="test-token",
        user_id="test-user",
        team_id="test-team",
    ...
    team_membership = DeRouter_TeamMembership(
        user_id="test-user",
        team_id="test-team",
        spend=0.0,  # stale
        derouter_budget_table=DeRouter_BudgetTable(max_budget=1.0),
...
class TestGuardrailModificationCheck:
    ...
    def _call(self, request_body):
        from derouter.proxy.auth.auth_checks import _guardrail_modification_check
    ...
    def test_rejects_guardrails_list(self):
        ...
        with patch(
            "derouter.proxy.guardrails.guardrail_helpers.can_modify_guardrails",
            return_value=False,
        ):
    ...
    def test_rejects_injection_via_derouter_metadata_key(self):
        ...
        with patch(
            ...
            with pytest.raises(HTTPException) as exc:
                self._call({"derouter_metadata": {"disable_global_guardrails": True}})
            assert exc.value.status_code == 403
    ...
    def test_rejects_string_encoded_derouter_metadata_bypass(self):
        """Same bypass via the derouter_metadata key."""
        import json as _json
        ...
        with patch(
            ...
            with pytest.raises(HTTPException) as exc:
                self._call({"derouter_metadata": _json.dumps(attacker_payload)})
            assert exc.value.status_code == 403
...
async def test_team_member_budget_check_falls_back_to_team_default_budget_id():
    ...
    prisma_client = MagicMock()
    prisma_client.db.derouter_budgettable.find_unique = AsyncMock(return_value=fake_budget_row)
    ...
    prisma_client.db.derouter_budgettable.find_unique.assert_awaited_once()

    # Second call hits the cached budget row, no additional prisma read.
    prisma_client.db.derouter_budgettable.find_unique.reset_mock()
    with (
        patch("derouter.proxy.proxy_server.get_current_spend", mock_get_current_spend),
        patch(
            "derouter.proxy.auth.auth_checks.get_team_membership",
        ...
        with pytest.raises(derouter.BudgetExceededError) as second_exc_info:
            await _check_team_member_budget(
                team_object=team_object,
                user_object=user_object,
                valid_token=valid_token,
    ...
    prisma_client.db.derouter_budgettable.find_unique.assert_not_awaited()
...
async def test_team_member_budget_check_per_member_override_wins_over_team_default():
    ...
    team_membership = DeRouter_TeamMembership(
        ...
        derouter_budget_table=DeRouter_BudgetTable(max_budget=200.0),
    )
...
async def test_team_member_budget_check_null_clone_falls_back_to_team_default():
    ...
    team_membership = DeRouter_TeamMembership(
        ...
        derouter_budget_table=DeRouter_BudgetTable(max_budget=None),
    )
    ...
    prisma_client = MagicMock()
    prisma_client.db.derouter_budgettable.find_unique = AsyncMock(return_value=fake_default_row)
...
async def test_team_member_budget_check_zero_per_member_row_still_blocks():
    ...
    team_membership = DeRouter_TeamMembership(
        ...
        derouter_budget_table=DeRouter_BudgetTable(max_budget=0.0),
    )

    proxy_logging_obj = ProxyLogging(user_api_key_cache=None)

    prisma_client = MagicMock()
    prisma_client.db.derouter_budgettable.find_unique = AsyncMock(return_value=None)
...
def _validate_flag_on(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "validate_end_user_id_in_db", True)
    monkeypatch.setattr(derouter, "max_end_user_budget_id", None)
...
async def test_resolve_end_user_returns_none_for_none_input(_validate_flag_on, monkeypatch):
    from derouter.proxy.auth.auth_checks import resolve_and_validate_end_user_id
...
async def test_resolve_end_user_passes_through_when_flag_disabled(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "validate_end_user_id_in_db", False)
    _patch_validation_helpers(monkeypatch)
    cache = _validation_cache()
...
async def test_resolve_end_user_preserves_id_when_default_budget_configured(_validate_flag_on, monkeypatch):
    """Don't drop unregistered ids when derouter.max_end_user_budget_id is set.
    ...
    monkeypatch.setattr(derouter, "max_end_user_budget_id", "default-budget")
    _patch_validation_helpers(monkeypatch)
    cache = _validation_cache()
...
async def test_cache_team_object_writes_team_id_and_invalidates_team_alias():
    ...
    from derouter.proxy._types import DeRouter_TeamTableCachedObj
    from derouter.proxy.auth.auth_checks import _cache_team_object
    ...
    # ===== team_alias is set =====
    team_table = DeRouter_TeamTableCachedObj(**base_team_row)
    cache = MagicMock()
    cache.async_set_cache = AsyncMock()
    ...
    aliasless = DeRouter_TeamTableCachedObj(**{**base_team_row, "team_alias": None})
    cache2 = MagicMock()
...
async def test_team_update_not_shadowed_by_internal_usage_cache_lit_4391():
    ...
    from derouter.caching.dual_cache import DualCache
    from derouter.proxy._types import DeRouter_TeamTableCachedObj
    from derouter.proxy.auth.auth_checks import _cache_team_object, get_team_object
    ...
    await _cache_team_object(
        team_id=team_id,
        team_table=DeRouter_TeamTableCachedObj(team_id=team_id, models=["model-a"]),
        user_api_key_cache=user_api_key_cache,
        proxy_logging_obj=proxy_logging_obj,
    )
    ...
    await _cache_team_object(
        team_id=team_id,
        team_table=DeRouter_TeamTableCachedObj(team_id=team_id, models=["model-a", "model-b"]),
        user_api_key_cache=user_api_key_cache,
        proxy_logging_obj=proxy_logging_obj,
    )
...
async def test_model_discovery_route_bypasses_team_budget(route):
    ...
    team_object = DeRouter_TeamTable(team_id="test-team", spend=150.0, max_budget=100.0)
...
async def test_model_discovery_route_bypasses_user_budget():
    ...
    user_object = DeRouter_UserTable(user_id="test-user", spend=100.0, max_budget=50.0)
...
async def test_common_checks_budget_reads_run_concurrently():
    ...
    end_user = DeRouter_EndUserTable(
        user_id="eu1",
        blocked=False,
        spend=0.0,
        derouter_budget_table=DeRouter_BudgetTable(max_budget=100.0),
    )
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.get_current_spend", probe),
    ):
...
async def test_common_checks_budget_gather_raises_highest_priority_scope():
    ...
    team = DeRouter_TeamTable(team_id="t1", spend=0.0, max_budget=100.0)
    end_user = DeRouter_EndUserTable(
        user_id="eu1",
        blocked=False,
        spend=0.0,
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client", None),
        patch("derouter.proxy.proxy_server.get_current_spend", _spend_by_counter),
    ):
        # Both team and end-user over budget: team wins on priority.
        _spend_by_counter.team = 999.0
        _spend_by_counter.end_user = 999.0
        with pytest.raises(derouter.BudgetExceededError) as both_over:
            await _run()
        ...
        _spend_by_counter.end_user = 999.0
        with pytest.raises(derouter.BudgetExceededError) as end_user_over:
            await _run()
...
async def test_common_checks_personal_user_budget_blocks_in_gather():
    ...
    user = DeRouter_UserTable(user_id="u1", spend=0.0, max_budget=100.0)
    ...
    with (
        ...
        with pytest.raises(derouter.BudgetExceededError) as over:
            await common_checks(
                request_body={"messages": [{"role": "user", "content": "hi"}]},
                team_object=None,
                user_object=user,
...
async def test_common_checks_personal_user_budget_skipped_for_team_key():
    ...
    user = DeRouter_UserTable(user_id="u1", spend=0.0, max_budget=100.0)
    team = DeRouter_TeamTable(team_id="t1", spend=0.0, max_budget=1000.0)
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client", None),
        patch("derouter.proxy.proxy_server.get_current_spend", _spend_by_counter),
        patch("derouter.proxy.auth.auth_checks.get_team_membership", _no_membership),
    ):
        result = await common_checks(
            request_body={"messages": [{"role": "user", "content": "hi"}]},
            team_object=team,
...
async def test_budget_checks_only_run_on_llm_api_routes(scope, route, expect_blocked):
    ...
    org_table.derouter_budget_table = MagicMock()
    org_table.derouter_budget_table.max_budget = 10.0

    async def _get_org(*a, **kw):
        return org_table

    user = DeRouter_UserTable(user_id="u1", spend=0.0, max_budget=10.0 if scope == "user" else None)
    team = DeRouter_TeamTable(team_id="t1", max_budget=10.0) if scope == "team" else None
    token = UserAPIKeyAuth(
        token="k1",
        user_id="u1",
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.get_current_spend", _spend_by_counter),
        patch("derouter.proxy.auth.auth_checks.get_team_membership", _no_membership),
        patch("derouter.proxy.auth.auth_checks.get_org_object", _get_org),
    ):
        if expect_blocked:
            with pytest.raises(derouter.BudgetExceededError):
                await _run()
...
async def test_spend_capable_non_llm_routes_still_enforce_budget(route):
    ...
    external service: /health and /health/test_connection run derouter.ahealth_check
    ...
    team = DeRouter_TeamTable(team_id="t1", spend=150.0, max_budget=100.0)
...
async def test_get_default_end_user_budget_db_fetch_returns_validated_budget(monkeypatch):
    from derouter.proxy.auth.auth_checks import get_default_end_user_budget

    monkeypatch.setattr(derouter, "max_end_user_budget_id", "budget-default-1")

    budget_row = MagicMock()
    budget_row.dict = lambda: {"budget_id": "budget-default-1", "max_budget": 12.5, "tpm_limit": 100}

    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_budgettable.find_unique = AsyncMock(return_value=budget_row)
    ...
    assert isinstance(result, DeRouter_BudgetTable)
...
async def test_get_team_member_default_budget_caches_json_safe_payload():
    ...
    """
    from derouter.proxy.auth.auth_checks import get_team_member_default_budget
    ...
    assert isinstance(budget, DeRouter_BudgetTable)
    ...
    assert isinstance(cached, DeRouter_BudgetTable)
    assert cached.max_budget == 25.0
    mock_prisma_client.db.derouter_budgettable.find_unique.assert_awaited_once()
...
async def test_get_end_user_object_db_fetch_returns_validated_end_user():
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_endusertable.find_unique = AsyncMock(return_value=end_user_row)
    ...
    assert isinstance(result, DeRouter_EndUserTable)
...
async def test_get_end_user_object_never_queries_db_for_unrestricted_end_users(
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_endusertable.find_many = AsyncMock(return_value=[_end_user_registry_row("eu-blocked")])
    mock_prisma.db.derouter_endusertable.find_unique = AsyncMock(return_value=_end_user_db_row("eu-anon-1"))
    cache = UserApiKeyCache()
    ...
    mock_prisma.db.derouter_endusertable.find_unique.assert_not_awaited()

    mock_prisma.db.derouter_endusertable.find_many.assert_awaited_once()
    registry_call = mock_prisma.db.derouter_endusertable.find_many.call_args
    ...
    mock_prisma.db.derouter_endusertable.find_many.reset_mock()
    assert (
        await get_end_user_object(
    ...
    mock_prisma.db.derouter_endusertable.find_many.assert_not_awaited()
    mock_prisma.db.derouter_endusertable.find_unique.assert_not_awaited()
...
async def test_get_end_user_object_still_fetches_restricted_end_user(end_user_registry_skip_enabled):
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_endusertable.find_many = AsyncMock(return_value=[_end_user_registry_row("eu-blocked")])
    mock_prisma.db.derouter_endusertable.find_unique = AsyncMock(
    ...
    assert isinstance(blocked, DeRouter_EndUserTable)
    assert blocked.blocked is True
    mock_prisma.db.derouter_endusertable.find_unique.assert_awaited_once()
    ...
    mock_prisma.db.derouter_endusertable.find_unique.reset_mock()
...
async def test_get_end_user_object_caches_empty_restricted_registry(end_user_registry_skip_enabled):
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_endusertable.find_many = AsyncMock(return_value=[])
    mock_prisma.db.derouter_endusertable.find_unique = AsyncMock(return_value=_end_user_db_row("eu-anon-1"))
    cache = UserApiKeyCache()
...
async def test_get_end_user_object_registry_db_error_negative_caches_and_keeps_per_id_fetch(
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_endusertable.find_many = AsyncMock(side_effect=Exception("registry query failed"))
    mock_prisma.db.derouter_endusertable.find_unique = AsyncMock(
    ...
    assert mock_prisma.db.derouter_endusertable.find_many.await_count == 2
...
async def test_registry_db_error_is_logged_at_warning(end_user_registry_skip_enabled):
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_endusertable.find_many = AsyncMock(side_effect=Exception("registry query failed"))
    mock_prisma.db.derouter_endusertable.find_unique = AsyncMock(return_value=_end_user_db_row("eu-1", blocked=True))
...
async def test_end_user_registry_load_is_single_flighted_across_concurrent_requests(
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_endusertable.find_many = AsyncMock(side_effect=fake_find_many)
    mock_prisma.db.derouter_endusertable.find_unique = AsyncMock(return_value=_end_user_db_row("eu-anon-1"))
    cache = UserApiKeyCache()
...
async def test_get_end_user_object_oversized_registry_falls_back_and_stops_refetching(
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_endusertable.find_many = AsyncMock(return_value=oversized)
    mock_prisma.db.derouter_endusertable.find_unique = AsyncMock(
        side_effect=lambda **kwargs: _end_user_db_row(kwargs["where"]["user_id"], blocked=True)
    )
    cache = UserApiKeyCache()
    ...
    assert second is not None and second.blocked is True
    mock_prisma.db.derouter_endusertable.find_many.assert_awaited_once()
    assert mock_prisma.db.derouter_endusertable.find_unique.await_count == 2
...
async def test_get_end_user_object_default_budget_gate_keeps_fetching_unrestricted_end_users(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "max_end_user_budget_id", "default-eu-budget")
    monkeypatch.setattr(derouter, "validate_end_user_id_in_db", False)
    ...
    mock_prisma.db.derouter_endusertable.find_many = AsyncMock(return_value=[])
    mock_prisma.db.derouter_endusertable.find_unique = AsyncMock(return_value=_end_user_db_row("eu-anon-1"))
    mock_prisma.db.derouter_budgettable.find_unique = AsyncMock(return_value=budget_row)
    ...
    assert result.derouter_budget_table.max_budget == 25.0
    mock_prisma.db.derouter_endusertable.find_many.assert_not_awaited()
...
async def test_get_end_user_object_token_budget_gate_keeps_fetching_unrestricted_end_users(
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_endusertable.find_many = AsyncMock(return_value=[])
    mock_prisma.db.derouter_endusertable.find_unique = AsyncMock(return_value=_end_user_db_row("eu-anon-1", spend=100.0))
    cache = UserApiKeyCache()
...
async def test_end_user_id_validation_gate_still_resolves_unrestricted_end_users(monkeypatch):
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_endusertable.find_many = AsyncMock(return_value=[])
    mock_prisma.db.derouter_endusertable.find_unique = AsyncMock(return_value=_end_user_db_row("eu-known-1"))
    mock_prisma.db.derouter_usertable.find_unique = AsyncMock(return_value=None)
    mock_prisma.db.derouter_usertable.find_first = AsyncMock(return_value=None)
...
async def test_get_team_membership_db_fetch_returns_validated_membership():
    from derouter.proxy._types import DeRouter_TeamMembership
    from derouter.proxy.auth.auth_checks import get_team_membership
    ...
    assert isinstance(result, DeRouter_TeamMembership)
...
async def test_get_access_object_db_fetch_returns_validated_access_group():
    ...
    from derouter.proxy.auth.auth_checks import get_access_object
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_accessgrouptable.find_unique = AsyncMock(return_value=access_row)
    ...
    assert isinstance(result, DeRouter_AccessGroupTable)
...
async def test_get_team_object_by_alias_db_fetch_returns_cached_obj():
    from derouter.proxy._types import DeRouter_TeamTableCachedObj
    from derouter.proxy.auth.auth_checks import get_team_object_by_alias

    team_row = MagicMock()
    team_row.model_dump = lambda: {"team_id": "t-9", "team_alias": "alias-9", "models": ["gpt-4"]}

    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_teamtable.find_many = AsyncMock(return_value=[team_row])
    ...
    assert isinstance(result, DeRouter_TeamTableCachedObj)
...
async def test_get_org_object_by_alias_db_fetch_returns_validated_org():
    from derouter.proxy._types import DeRouter_OrganizationTable
    from derouter.proxy.auth.auth_checks import get_org_object_by_alias
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_organizationtable.find_many = AsyncMock(return_value=[org_row])
    ...
    assert isinstance(result, DeRouter_OrganizationTable)
...
async def test_get_object_permission_db_fetch_returns_validated_permission():
    from derouter.proxy.auth.auth_checks import get_object_permission

    perm_row = MagicMock()
    perm_row.dict = lambda: {"object_permission_id": "op-1", "vector_stores": ["vs-1"]}

    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_objectpermissiontable.find_unique = AsyncMock(return_value=perm_row)
    ...
    assert isinstance(result, DeRouter_ObjectPermissionTable)
...
async def test_get_managed_vector_store_rows_by_uuids_db_fetch_validates_rows():
    from derouter.proxy._types import DeRouter_ManagedVectorStoresTable
    from derouter.proxy.auth.auth_checks import get_managed_vector_store_rows_by_uuids

    vs_row = MagicMock()
    vs_row.model_dump = lambda: {"vector_store_id": "vs-7", "custom_llm_provider": "openai"}

    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_managedvectorstorestable.find_many = AsyncMock(return_value=[vs_row])
    ...
    assert len(result) == 1
    assert isinstance(result[0], DeRouter_ManagedVectorStoresTable)
...
async def test_get_project_object_db_fetch_returns_cached_obj():
    from derouter.proxy._types import DeRouter_ProjectTableCachedObj
    from derouter.proxy.auth.auth_checks import get_project_object

    project_row = MagicMock()
    project_row.model_dump = lambda: {"project_id": "p-1", "project_alias": "proj", "team_id": "t-1"}

    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_projecttable.find_unique = AsyncMock(return_value=project_row)
    ...
    assert isinstance(result, DeRouter_ProjectTableCachedObj)
...
async def test_project_allowlist_enforced_when_key_models_empty():
    ...
    from derouter.proxy.auth.auth_checks import _run_project_checks, can_key_call_model
    ...
    project = DeRouter_ProjectTableCachedObj(
        project_id="p-1",
        team_id="t-1",
...
def test_is_user_proxy_admin_rejects_view_only_admin():
    ...
    from derouter.proxy.auth.auth_checks import _is_user_proxy_admin

    viewer = DeRouter_UserTable(
        user_id="viewer_user",
        user_email="viewer@example.com",
        user_role=LitellmUserRoles.PROXY_ADMIN_VIEW_ONLY.value,
    )
    admin = DeRouter_UserTable(
        user_id="admin_user",
...
def _make_wildcard_access_group_router():
    ...
    return Router(
        model_list=[
            {
                "model_name": "openai/*",
                "derouter_params": {"model": "openai/*", "api_key": "fake"},
                "model_info": {
                    "id": "wildcard-openai",
            ...
            {
                "model_name": "azure/*",
                "derouter_params": {"model": "azure/*", "api_key": "fake"},
    ...
    return Router(
        model_list=[
            ...
            },
            {
                ...
                "derouter_params": {"model": UNPRICED_UNDERLYING_MODEL, "api_key": "sk-test"},
            },
        ]
    )
...
def test_model_has_no_cost_mapping_priced_model_is_false():
    from derouter.proxy.auth.auth_checks import model_has_no_cost_mapping
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": underlying_model, "api_key": "sk-test"},
            }
        ]
    )
...
def test_model_has_no_cost_mapping_non_token_price_from_derouter_params_is_false():
    from derouter.proxy.auth.auth_checks import model_has_no_cost_mapping
    from derouter.router import Router
...
async def test_common_checks_blocks_unpriced_model_when_enabled(monkeypatch):
    monkeypatch.setattr(derouter, "block_requests_for_models_without_pricing", True)
    router = _router_with_priced_and_unpriced_models()
...
async def test_common_checks_allows_unpriced_model_when_disabled(monkeypatch):
    monkeypatch.setattr(derouter, "block_requests_for_models_without_pricing", False)
    router = _router_with_priced_and_unpriced_models()
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": f"{UNPRICED_UNDERLYING_MODEL}-model-info", "api_key": "sk-test"},
...
def test_team_allowed_routes_wildcard_prefix_matches_unregistered_passthrough_routes(user_route, expected):
    ...
    from derouter.proxy.auth.auth_checks import allowed_routes_check

    assert (
        allowed_routes_check(
            user_role=LitellmUserRoles.TEAM,
            user_route=user_route,
            derouter_proxy_roles=DeRouter_JWTAuth(team_allowed_routes=["/internal-models/*"]),
        )
        is expected
    )
...
def test_team_allowed_routes_exact_route_does_not_become_a_prefix_grant():
    ...
    roles = DeRouter_JWTAuth(team_allowed_routes=["/internal-models/model-a"])

    assert (
        allowed_routes_check(
            user_role=LitellmUserRoles.TEAM, user_route="/internal-models/model-a", derouter_proxy_roles=roles
        )
        is True
    )
    assert (
        allowed_routes_check(
            user_role=LitellmUserRoles.TEAM, user_route="/internal-models/model-b", derouter_proxy_roles=roles
        )
        is False
    )
...
def test_admin_allowed_routes_wildcard_prefix_is_honored():
    ...
    roles = DeRouter_JWTAuth(admin_allowed_routes=["/internal-models/*"])

    assert (
        allowed_routes_check(
            user_role=LitellmUserRoles.PROXY_ADMIN, user_route="/internal-models/anything", derouter_proxy_roles=roles
        )
        is True
    )
    assert (
        allowed_routes_check(
            user_role=LitellmUserRoles.PROXY_ADMIN, user_route="/other/anything", derouter_proxy_roles=roles
        )
        is False
    )
...
def test_team_allowed_routes_named_route_group_still_resolves():
    ...
    roles = DeRouter_JWTAuth(team_allowed_routes=["openai_routes"])

    assert (
        allowed_routes_check(
            user_role=LitellmUserRoles.TEAM, user_route="/v1/chat/completions", derouter_proxy_roles=roles
        )
        is True
    )
    assert (
        allowed_routes_check(user_role=LitellmUserRoles.TEAM, user_route="/key/generate", derouter_proxy_roles=roles)
        is False
    )
...
async def test_invalidate_team_member_spend_state_sets_the_spend_counter_and_clears_both_membership_cache_keys():
    ...
    with patch(  # test-quality-ok: injects a real DualCache for the module global, not a behavior mock
        "derouter.proxy.proxy_server.spend_counter_cache", real_spend_counter_cache
    ):
...
async def test_invalidate_team_member_spend_state_broadcasts_the_spend_counter_to_remote_workers():
    ...
    from derouter.caching.dual_cache import DualCache
    from derouter.caching.in_memory_cache import InMemoryCache
    from derouter.proxy.common_utils.auth_cache_invalidation_pubsub import AuthCacheInvalidationSubscriber
    from derouter.proxy.common_utils.user_api_key_cache import UserApiKeyCache
    ...
    with (
        patch(  # test-quality-ok: injects a real DualCache for the module global, not a behavior mock
            "derouter.proxy.proxy_server.spend_counter_cache", local_spend_counter_cache
        ),
        patch(  # test-quality-ok: injects a fake pub/sub-capable redis cache; no live redis in this unit test
            "derouter.proxy.common_utils.auth_cache_invalidation_pubsub.coordination_redis_cache",
...
async def test_delete_cache_key_object_is_best_effort_when_the_cache_backend_fails(caplog):
    ...
    for operators whose Redis ACL denies `DEL` on DeRouter's unprefixed token-hash keys. The
    in-memory entry is already dropped by then, so raising never made the cache less stale.
    """
    import logging
    from unittest.mock import AsyncMock, MagicMock

    from derouter.proxy.auth.auth_checks import _delete_cache_key_object

    hashed_token = "a" * 64
    caplog.set_level(logging.WARNING, logger="DeRouter Proxy")
...
async def _run_key_budget_check(key_name: str) -> str:
    ...
    with pytest.raises(derouter.BudgetExceededError, match="Budget has been exceeded") as exc_info:
        await _virtual_key_max_budget_check(
            valid_token=valid_token,
...
async def test_enforced_model_allowlists_treats_a_missing_user_row_as_unrestricted():
    from derouter.proxy.auth.auth_checks import enforced_model_allowlists
    from derouter.proxy.common_utils.user_api_key_cache import UserApiKeyCache
    from derouter.proxy.utils import ProxyLogging
...
async def test_enforced_model_allowlists_reads_every_level_from_cache():
    ...
    proxy_logging_obj = ProxyLogging(user_api_key_cache=cache)
    await cache.async_set_cache(
        key="team_id:team-fake", value=DeRouter_TeamTableCachedObj(team_id="team-fake", models=["gpt-4o"])
    )
    await cache.async_set_cache(
        key=team_membership_reservation_cache_key(user_id="user-fake", team_id="team-fake"),
        value=DeRouter_TeamMembership(
            user_id="user-fake",
            team_id="team-fake",
            derouter_budget_table=DeRouter_BudgetTable(allowed_models=["gpt-4o-mini"]),
        ),
    )
    await cache.async_set_cache(
        key="project_id:project-fake",
        value=DeRouter_ProjectTableCachedObj(project_id="project-fake", models=["gpt-4.1"]),
    )
    await cache.async_set_cache(key="user-fake", value=DeRouter_UserTable(user_id="user-fake", models=["o3"]))
