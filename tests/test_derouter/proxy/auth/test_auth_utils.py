...
class TestCustomAuthCommonChecksWarning:
    ...
    def test_warns_when_custom_auth_configured_and_checks_off(self):
        ...
        assert "https://router.delify.vn/docs/proxy/custom_auth" in warning
...
class TestWarnOnceIfCustomAuthSkipsCommonChecks:
    ...
    def _reset_sentinel(self, monkeypatch):
        monkeypatch.setattr(
            "derouter.proxy.auth.auth_utils._custom_auth_common_checks_warning_emitted",
            False,
        )
...
class TestLogOnceIfBudgetReservationDisabled:
    ...
    def _reset_sentinel(self, monkeypatch):
        monkeypatch.setattr(
            "derouter.constants.budget_reservation_disabled_info_emitted",
            False,
        )
...
class TestGetKeyModelTpmLimit:
    ...
    def test_skips_deployments_with_malformed_limit_value(self):
        ...
        mock_router.get_model_list.return_value = [
            {
                "model_name": "model1",
                "derouter_params": {"default_api_key_tpm_limit": "not-a-number"},
            },
...
class TestGetCustomerIdFromStandardHeaders:
    ...
    def test_should_return_customer_id_from_x_derouter_customer_id_header(self):
        """Should extract customer ID from x-derouter-customer-id header."""
        headers = {"x-derouter-customer-id": "customer-123"}
        result = _get_customer_id_from_standard_headers(request_headers=headers)
        assert result == "customer-123"

    def test_should_return_customer_id_from_x_derouter_end_user_id_header(self):
        """Should extract customer ID from x-derouter-end-user-id header."""
        headers = {"x-derouter-end-user-id": "end-user-456"}
        result = _get_customer_id_from_standard_headers(request_headers=headers)
...
class TestGetEndUserIdFromRequestBodyWithStandardHeaders:
    ...
    def test_should_prioritize_standard_header_over_body_user(self):
        ...
        headers = {"x-derouter-customer-id": "header-customer"}
...
def _pass_through_endpoint():
    ...
    setattr(endpoint, DEROUTER_PASS_THROUGH_ENDPOINT_MARKER, True)
    return endpoint
...
def test_get_model_from_request_skips_pass_through_dispatched_request():
    ...
    the body `model` names an upstream model and must not be treated as a DeRouter
...
def test_get_model_from_request_includes_file_endpoint_header_model():
    assert (
        get_model_from_request(
            request_data={},
            route="/v1/files",
            request_headers={"X-DeRouter-Model": "restricted-model"},
        )
        == "restricted-model"
    )
...
def test_get_model_from_request_authorizes_all_file_routing_model_sources():
    models = get_model_from_request(
        ...
        route="/v1/files",
        request_headers={"x-derouter-model": "header-model"},
...
def test_get_model_from_request_extracts_unified_file_id_models():
    raw_unified_file_id = (
        "derouter_proxy:application/octet-stream;unified_id,test-id;"
...
def test_get_model_from_request_extracts_video_id_model():
    from derouter.types.videos.utils import encode_video_id_with_provider
...
_MANAGED_BATCH_ID = _encode_managed_id(
    f"derouter_proxy;model_id:{_BATCH_DEPLOYMENT_ID};llm_batch_id:provider-batch-123"
)
...
async def test_managed_batch_routes_pass_team_model_access_check(route, request_data):
    ...
    assert (
        await can_team_access_model(
            model=model,
            team_object=DeRouter_TeamTable(team_id="team-batch", models=["bedrock-batch-model"]),
            llm_router=llm_router,
        )
        is True
    )
    ...
    with pytest.raises(Exception, match="team not allowed to access model"):
        await can_team_access_model(
            model=model,
            team_object=DeRouter_TeamTable(team_id="team-other", models=["some-other-model"]),
            llm_router=llm_router,
        )


def test_get_model_from_request_resolves_character_id_model_with_router():
    from derouter.types.videos.utils import encode_character_id_with_provider
...
def test_get_model_from_request_only_runs_media_decoders_for_matching_fields():
    with (
        patch(
            "derouter.types.videos.utils.decode_video_id_with_provider",
            return_value={"model_id": "video-model"},
        ) as video_decoder,
        patch(
            "derouter.types.videos.utils.decode_character_id_with_provider",
...
def test_get_model_from_request_handles_managed_id_decoder_failures():
    with (
        patch(
            "derouter.proxy.openai_files_endpoints.common_utils.decode_model_from_file_id",
            side_effect=Exception("decode failed"),
        ),
        patch(
            "derouter.llms.base_llm.managed_resources.utils.parse_unified_id",
...
def test_get_model_from_request_extracts_realtime_session_model(route):
    ...
    HTTP routes (https://github.com/Delify-Solutions/derouter/issues/29923).
    """
    assert (
        get_model_from_request(
...
def test_get_customer_user_header_returns_none_when_no_customer_role():
    from derouter.proxy.auth.auth_utils import get_customer_user_header_from_mapping
...
def test_get_customer_user_header_returns_none_for_single_non_customer_mapping():
    ...
    mapping = {"header_name": "X-Only-Internal", "derouter_user_role": "internal_user"}
...
def test_get_customer_user_header_returns_customers_header_in_config_order_when_multiple_exist():
    ...
    mappings = [
        {"header_name": "X-OpenWebUI-User-Id", "derouter_user_role": "internal_user"},
        {"header_name": "X-OpenWebUI-User-Email", "derouter_user_role": "customer"},
        {"header_name": "X-User-Id", "derouter_user_role": "customer"},
    ]
    result = get_customer_user_header_from_mapping(mappings)
...
def test_get_end_user_id_returns_id_from_user_header_mappings():
    from derouter.proxy.auth.auth_utils import get_end_user_id_from_request_body
    ...
    with (
        patch(
            "derouter.proxy.auth.auth_utils._get_customer_id_from_standard_headers",
            return_value=None,
        ),
...
class TestCoerceUserIdToStr:
    ...
    def test_plain_string_is_returned_verbatim(self):
        from derouter.proxy.auth.auth_utils import _coerce_user_id_to_str
    ...
    def test_json_encoded_dict_string_passes_through_by_default(self):
        ...
        original = derouter.validate_end_user_id_in_db
        derouter.validate_end_user_id_in_db = False
        try:
            assert _coerce_user_id_to_str(blob) == blob
        finally:
            derouter.validate_end_user_id_in_db = original
    ...
    def test_json_encoded_dict_string_returns_none_when_validation_enabled(self):
        ...
        original = derouter.validate_end_user_id_in_db
        derouter.validate_end_user_id_in_db = True
        try:
...
class TestGetEndUserIdDropsMalformedBodyValues:
    ...
    def test_dict_user_falls_through_to_derouter_metadata(self):
        request_body = {
            "user": {
                "device_id": "abc",
                "session_id": "c284b8cb",
            },
            "derouter_metadata": {"user": "alice@example.com"},
        }
    ...
    def test_json_encoded_user_string_passes_through_by_default(self):
        ...
        Gating behind ``derouter.validate_end_user_id_in_db`` keeps existing
...
def _make_deployment_dict(
    ...
    derouter_params: dict = {"model": model_name}
    if tpm is not None:
        derouter_params["default_api_key_tpm_limit"] = tpm
    if rpm is not None:
        derouter_params["default_api_key_rpm_limit"] = rpm
    return {"model_name": model_name, "derouter_params": derouter_params}


_ROUTER_PATCH = "derouter.proxy.proxy_server.llm_router"
...
class TestDeploymentDefaultRpmLimit:
    ...
    def test_skips_deployments_with_malformed_limit_value(self):
        ...
        mock_router.get_model_list.return_value = [
            {
                "model_name": "model1",
                "derouter_params": {"default_api_key_rpm_limit": "not-a-number"},
            },
...
class TestCheckCompleteCredentialsBlocksSSRF:
    ...
    The check only runs when ``derouter.user_url_validation`` is True, so
    ...
    def _enable_url_validation(self, monkeypatch):
        ...
        monkeypatch.setattr(derouter, "user_url_validation", True, raising=False)
    ...
    def test_rejects_private_or_metadata_targets(self, url_field, blocked_url):
        from derouter.derouter_core_utils.url_utils import SSRFError

        with patch(
            "derouter.proxy.auth.auth_utils.validate_url",
    ...
    def test_skips_url_validation_when_toggle_is_off(self, monkeypatch):
        ...
        monkeypatch.setattr(derouter, "user_url_validation", False, raising=False)
        with patch(
            "derouter.proxy.auth.auth_utils.validate_url",
        ) as mocked:
            result = check_complete_credentials(
...
class TestGetDynamicLitellmParamsClearsAdminConfigOnBaseOverride:
    ...
    def test_clears_admin_organization_and_extra_body_on_base_override(self):
        ...
        out = get_dynamic_derouter_params(
            derouter_params=dict(admin_params),
            request_kwargs={
                "api_key": "sk-attacker",
    ...
    def test_field_echo_does_not_preserve_admin_value(self):
        ...
        # admin's value in ``derouter_params``.
        from derouter.router_utils.clientside_credential_handler import (
            get_dynamic_derouter_params,
        )
...
class TestIsRequestBodySafeBlocksFallbackSmuggle:
    ...
    def test_nested_api_base_caught_across_router_fallback_rounds(self):
        ...
        with pytest.raises(ValueError, match="api_base"):
            is_request_body_safe(
                request_body=_rounds_deep_api_base_payload(derouter.ROUTER_MAX_FALLBACKS - 1, "fallbacks"),
                general_settings={},
                llm_router=None,
                model="gpt-4",
            )
...
class TestIsRequestBodySafeBlocksRustOptIn:
    ...
    the router, and ``rust`` is a derouter param, so it lands in
    ``derouter_params`` and the gate honours it: without this entry any
...
class TestIsRequestBodySafeBlocksNVCFFunctionOverride:
    ...
    def test_admin_opt_in_per_deployment_allows_nvcf_function_id(self, monkeypatch):
        ...
        from derouter.proxy.auth import auth_utils
...
class TestIsRequestBodySafeNestedConfig:
    ...
    ``derouter_embedding_config`` as ``**kwargs`` into ``derouter.embedding(...)``
    ...
    def test_nested_api_base_in_embedding_config_blocked(self):
        """Smuggling ``api_base`` inside ``derouter_embedding_config`` is
    ...
    def test_nested_nvcf_function_id_in_metadata_blocked(self):
        ...
        with pytest.raises(ValueError, match="nvcf_function_id"):
            is_request_body_safe(
                request_body={
                    "model": "nvidia_riva/parakeet",
                    "derouter_metadata": {"nvcf_function_id": "attacker-via-metadata"},
                },
                general_settings={},
                llm_router=None,
                model="nvidia_riva/parakeet",
    ...
    def test_non_dict_nested_config_does_not_break_check(self):
        """A bogus type for ``derouter_embedding_config`` (string, list,
        None) must not crash the validator — it should just fall through."""
        assert (
            is_request_body_safe(
                request_body={"derouter_embedding_config": "not-a-dict"},
                general_settings={},
                llm_router=None,
                model="x",
            )
    ...
    def test_deeply_nested_config_does_not_recurse(self):
        ...
        a deeply-nested ``derouter_embedding_config`` cannot exhaust the
        Python call stack to trigger a 500 ``RecursionError``. Build a
        body 1000 levels deep; the validator must complete in O(1)
        descent."""
        body = {"derouter_embedding_config": {}}
        cur = body["derouter_embedding_config"]
        for _ in range(1000):
            cur["derouter_embedding_config"] = {}
            cur = cur["derouter_embedding_config"]
...
class TestObservabilityCallbackBans:
    ...
    in ``metadata`` / ``derouter_metadata``, or in a JSON-string-encoded
    metadata blob (multipart/``extra_body`` path).

    The ban list is derived from
    ``derouter.derouter_core_utils.initialize_dynamic_callback_params._supported_callback_params``
    ...
    def test_observability_field_in_metadata_dict_is_rejected(
        ...
        # or ``derouter_metadata`` is just as dangerous as the same field
    ...
    def test_observability_field_in_derouter_params_metadata_is_rejected(self):
...
class TestPricingInjectionBlocked:
    ...
    derouter.model_cost registry by supplying pricing fields in the request
    body.  Any CustomPricingDeRouterParams field (input_cost_per_token etc.)
    passed to completion() is forwarded to register_model(), which overwrites
    the shared global dict for ALL users on the instance.

    Fix: all CustomPricingDeRouterParams fields are in _BANNED_REQUEST_BODY_PARAMS,
    ...
    def test_all_custom_pricing_fields_are_banned(self):
        from derouter.proxy.auth.auth_utils import _BANNED_REQUEST_BODY_PARAMS
        from derouter.types.utils import CustomPricingDeRouterParams

        banned = set(_BANNED_REQUEST_BODY_PARAMS)
        for field in CustomPricingDeRouterParams.model_fields:
            assert field in banned, (
                f"CustomPricingDeRouterParams.{field} is not in "
        ...
        with pytest.raises(ValueError, match="model_list is not allowed"):
            is_request_body_safe(
                request_body={
                    "model": "gpt-4",
                    "messages": [{"role": "user", "content": "hi"}],
                    "model_list": [{"model_name": "x", "derouter_params": {}}],
                },
                general_settings={},
                llm_router=None,
                model="gpt-4",
...
class TestIsRequestBodySafeChecksBracketNotationMetadata:
    ...
    def test_bracket_notation_api_base_is_rejected(self):
        with pytest.raises(ValueError, match="api_base"):
            is_request_body_safe(
                request_body={"derouter_metadata[api_base]": "https://example.invalid"},
                general_settings={},
                llm_router=None,
                model="gpt-4",
            )
    ...
    def test_bracket_notation_allowed_under_proxy_wide_opt_in(self):
        assert (
            is_request_body_safe(
                request_body={"derouter_metadata[langfuse_host]": "https://byok.example"},
                general_settings={"allow_client_side_credentials": True},
                llm_router=None,
                model="gpt-4",
            )
    ...
    def test_benign_bracket_notation_metadata_is_allowed(self):
        assert (
            is_request_body_safe(
                request_body={
                    "purpose": "assistants",
                    "derouter_metadata[spend_logs_metadata][owner]": "john",
                    "derouter_metadata[tags]": "production",
                },
                general_settings={},
                llm_router=None,
                model="gpt-4",
    ...
    def test_bracket_notation_matches_json_encoding_for_deeper_nesting(self):
        ...
        deep_bracket = {
            "derouter_metadata[spend_logs_metadata][langfuse_host]": "https://example.invalid"
        }
        deep_json = {
            "derouter_metadata": {"spend_logs_metadata": {"langfuse_host": "https://example.invalid"}}
        }
...
class TestHasUserSetupSso:
    ...
    def test_false_when_no_sso_env(self):
        from derouter.proxy.auth.auth_utils import has_user_setup_sso
...
class TestIsSsoProviderFullyConfigured:
    ...
    def test_false_when_nothing_configured(self):
        from derouter.proxy.auth.auth_utils import is_sso_provider_fully_configured
