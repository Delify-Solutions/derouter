...
def test_model_with_fallbacks_returns_complete_list():
    ...
    with patch(
        "derouter.proxy.auth.model_checks.get_fallback_model_group"
...
def test_invalid_fallback_type_returns_empty_list():
    ...
    with patch("derouter.proxy.auth.model_checks.verbose_proxy_logger") as mock_logger:
        result = get_all_fallbacks(
...
def test_exception_handling_returns_empty_list():
    ...
    with patch(
        ...
        with patch(
            "derouter.proxy.auth.model_checks.verbose_proxy_logger"
        ) as mock_logger:
            result = get_all_fallbacks("claude-4-sonnet", llm_router=router)
