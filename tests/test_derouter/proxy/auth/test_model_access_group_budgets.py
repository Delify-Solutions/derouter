...
MODEL_LIST = [
    {
        "model_name": "gpt-4o",
        "derouter_params": {"model": "openai/gpt-4o", "api_key": "k"},
        "model_info": {"access_groups": ["tier-a", "tier-b"]},
    },
    {
        "model_name": "claude-sonnet",
        "derouter_params": {"model": "anthropic/claude-sonnet", "api_key": "k"},
    ...
    if member_allowed_models:
        await cache.async_set_cache(
            ...
            value=DeRouter_TeamMembership(
                ...
                derouter_budget_table=DeRouter_BudgetTable(allowed_models=list(member_allowed_models)),
            ),
...
async def _matched(
    *,
    ...
    return await collect_matched_model_access_groups(
        ...
        team_object=(
            DeRouter_TeamTable(team_id=TEAM_ID, models=team_models, organization_id=team_org_id)
            if team_models is not None
            else None
        ),
        project_object=(
            DeRouter_ProjectTableCachedObj(project_id="project-1", models=project_models)
...
class _MagBudgetRow:
    """One ``DeRouter_ModelAccessGroupBudgetTable`` row as prisma hands it back."""

    def __init__(self, access_group_name: str, spend: float = 0.0, max_budget: float | None = None) -> None:
        self.access_group_name = access_group_name
        self.spend = spend
        self.derouter_budget_table = None if max_budget is None else SimpleNamespace(max_budget=max_budget)
...
async def _enforce(
    ...
    *rows: _MagBudgetRow,
    ...
    # one collaborator it reaches by a lazy `from derouter.proxy.proxy_server import`, with no parameter.
    with patch("derouter.proxy.proxy_server.get_current_spend", read):  # test-quality-ok: get_current_spend is lazily imported inside _model_access_group_max_budget_check and has no injection point
...
async def test_a_database_error_does_not_block_the_request():
    class _FailingPrismaClient:
        def __init__(self) -> None:
            self.db = SimpleNamespace(
                derouter_modelaccessgroupbudgettable=SimpleNamespace(find_many=self._boom)
            )
...
async def _common_checks_with_over_budget_group(*, skip_budget_checks: bool) -> bool:
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.prisma_client", prisma_client),  # test-quality-ok: common_checks lazily imports prisma_client from proxy_server and takes no client parameter
        patch("derouter.proxy.proxy_server.user_api_key_cache", cache),  # test-quality-ok: common_checks lazily imports user_api_key_cache from proxy_server and takes no cache parameter
        patch("derouter.proxy.proxy_server.get_current_spend", read),  # test-quality-ok: get_current_spend is lazily imported inside the budget check and has no injection point
    ):
