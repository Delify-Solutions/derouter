...
from derouter.constants import INVALID_VIRTUAL_KEY_ERROR_MARKER
from derouter.exceptions import BudgetExceededError
from derouter.proxy._types import ProxyErrorTypes, ProxyException, UserAPIKeyAuth
...
async def test_handle_authentication_error_permanent_fault_503_is_not_worded_as_transient(prisma_error):
    ...
    with patch(  # test-quality-ok: the handler reads general_settings off the proxy module, no injection seam
        "derouter.proxy.proxy_server.general_settings", {"allow_requests_on_db_unavailable": False}
    ):
        with pytest.raises(ProxyException) as exc_info:
...
async def test_handle_authentication_error_db_infra_error_returns_503(db_error):
    ...
    with (
        patch(
            "derouter.proxy.proxy_server.proxy_logging_obj.post_call_failure_hook",
...
async def test_auth_failure_without_resolved_identity_still_logs():
    ...
    # as derouter.api_key.hash and leak a real sk-... to anyone reading the trace.
...
async def test_auth_failure_logs_requester_ip_address(
    ...
    """401s and budget 429s are rejected before `add_derouter_data_to_request` stamps
    ...
    with (
        patch("derouter.proxy.auth.auth_exception_handler.seed_request_identity"),
        patch(
            "derouter.proxy.proxy_server.proxy_logging_obj.post_call_failure_hook",
            new_callable=AsyncMock,
            return_value=None,
...
@pytest.mark.asyncio
async def test_auth_failure_ip_uses_derouter_metadata_when_present():
    """Routes that keep proxy metadata under `derouter_metadata` (e.g. /responses) must
    ...
    with (
        ...
        with pytest.raises(ProxyException):
            await UserAPIKeyAuthExceptionHandler._handle_authentication_error(
                ...
                _http_request(),
                {"derouter_metadata": {}, "metadata": {"user_supplied": "keep-me"}},
                "/v1/responses",
                None,
                "sk-bad-key",
            )
    ...
    logged_request_data = mock_hook.call_args[1]["request_data"]
    assert logged_request_data["derouter_metadata"]["requester_ip_address"] == "10.1.2.3"
...
def _marked_malformed_key_error() -> HTTPException:
    ...
    error = HTTPException(status_code=401, detail="DeRouter Virtual Key expected. Received=test")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            HTTPException(status_code=401, detail="DeRouter Virtual Key expected. Received=test"),
            False,
            "ERROR",
...
async def test_handle_authentication_error_traceback_only_for_unexpected_errors(auth_error, expect_traceback, expect_level, caplog):
    ...
    with (
        ...
        try:
            try:
                ...
                with caplog.at_level(expect_level, logger="DeRouter Proxy"), pytest.raises((ProxyException, HTTPException)):
                    await handler._handle_authentication_error(
                        caught,
                        MagicMock(),
                        {},
    ...
    expected_logger_name = "DeRouter Proxy.stdout" if expect_level == "WARNING" else "DeRouter Proxy"
