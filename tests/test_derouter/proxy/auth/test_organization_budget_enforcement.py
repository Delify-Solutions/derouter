...
async def test_organization_budget_exceeded_blocks_request():
    ...
    with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
        with patch("derouter.proxy.proxy_server.user_api_key_cache") as mock_cache:
            with patch(
                "derouter.proxy.auth.auth_checks.get_org_object", new_callable=AsyncMock
            ) as mock_get_org:
...
async def test_organization_budget_fields_are_checked():
    ...
    org_over_budget = DeRouter_OrganizationTable(
        organization_id="test-org",
        budget_id="budget-1",
...
async def test_both_team_and_org_budget_enforced():
    ...
    team_over_budget = DeRouter_TeamTable(
        team_id="team-over",
        max_budget=100.0,
        spend=150.0,  # Over budget
        models=["gpt-4"],
    ...
    team_under_budget = DeRouter_TeamTable(
        team_id="team-under",
        organization_id="org-over",
        max_budget=50.0,
