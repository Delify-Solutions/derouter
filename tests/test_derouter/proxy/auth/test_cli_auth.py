"""
Tests for derouter/proxy/client/cli/commands/auth.py
...
@patch("derouter.proxy.client.cli.commands.auth.requests.post")
def test_start_cli_sso_flow_rejects_invalid_response(request_mock):
    ...
    with pytest.raises(ValueError, match="missing required field\\(s\\): poll_secret"):
        _start_cli_sso_flow("https://derouter.com")


@pytest.mark.asyncio
@patch(
    "derouter.proxy.client.cli.commands.auth.requests.get",
    side_effect=[Mock(status_code=404)],
)
@patch("derouter.proxy.client.cli.commands.auth.time.sleep")
async def test_poll_for_ready_404(sleep_mock, request_mock):
    ...
    with pytest.raises(ValueError, match="rejected the login session with HTTP 404"):
        _poll_for_ready_data(
            "https://derouter.com", poll_interval=1, total_timeout=1, request_timeout=42
        )
    request_mock.assert_called_once_with("https://derouter.com", headers=None, timeout=42)
...
@patch("derouter.proxy.client.cli.commands.auth.click.echo")
@patch("derouter.proxy.client.cli.commands.auth.time.sleep")
...
async def test_poll_for_ready_single_pending(sleep_mock, click_mock, request_mock):
    """Test poll_for_ready function"""
    actual = _poll_for_ready_data(
        "https://derouter.com", poll_interval=1, total_timeout=2, request_timeout=42
    )
    assert actual == {"status": "ready", "json": "data"}
    click_mock.assert_not_called()
    request_mock.assert_has_calls(
        [
            call("https://derouter.com", headers=None, timeout=42),
            call("https://derouter.com", headers=None, timeout=42),
        ]
    )
...
@pytest.mark.asyncio
@patch("derouter.proxy.client.cli.commands.auth._handle_team_selection_during_polling")
@patch("derouter.proxy.client.cli.commands.auth._poll_for_ready_data", return_value=None)
@patch("derouter.proxy.client.cli.commands.auth.click.echo")
async def test_poll_for_authentication_no_data(click_mock, poll_mock, handle_mock):
    """Test poll_for_authentication function"""
    actual = _poll_for_authentication("https://derouter.com", "key-123", "poll-secret")
    assert actual is None
    poll_mock.assert_called_once_with(
        "https://derouter.com/sso/cli/poll/key-123",
        headers={"x-derouter-cli-poll-secret": "poll-secret"},
...
@patch("derouter.proxy.client.cli.commands.auth._handle_team_selection_during_polling")
@patch(
    "derouter.proxy.client.cli.commands.auth._poll_for_ready_data",
...
@pytest.mark.asyncio
@patch(
    "derouter.proxy.client.cli.commands.auth._handle_team_selection_during_polling",
