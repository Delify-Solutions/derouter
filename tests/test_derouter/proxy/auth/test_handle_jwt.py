...
from derouter.proxy.auth.handle_jwt import (
    JWKS_FETCH_ATTEMPTS,
...
async def test_map_user_to_teams_user_already_in_team():
    ...
    # Setup test data
    user = DeRouter_UserTable(user_id="test_user_1")
...
async def test_map_user_to_teams_add_new_user():
    ...
    # Setup test data
    user = DeRouter_UserTable(user_id="test_user_1")
    team = DeRouter_TeamTable(team_id="test_team_1", members_with_roles=[])
...
async def test_map_user_to_teams_handles_already_in_team_exception():
    ...
    with patch(
        ...
        with patch("derouter.proxy.auth.handle_jwt.verbose_proxy_logger") as mock_logger:
...
async def test_map_user_to_teams_null_inputs():
    ...
    await JWTAuthManager.map_user_to_teams(
        user_object=None, team_object=DeRouter_TeamTable(team_id="test_team_1")
    )

    # Test with null team
    await JWTAuthManager.map_user_to_teams(
        user_object=DeRouter_UserTable(user_id="test_user_1"), team_object=None
    )
...
async def test_find_team_with_model_access_reports_passthrough_allowlist_denial():
    ...
    with (
        patch(
            "derouter.proxy.auth.handle_jwt.get_team_object",
            new_callable=AsyncMock,
            return_value=team,
        ),
        patch(
            "derouter.proxy.auth.handle_jwt.can_team_access_model",
            new_callable=AsyncMock,
            return_value=True,
        ),
        patch(
            "derouter.proxy.auth.handle_jwt.allowed_routes_check",
            return_value=True,
        ),
        patch(
            "derouter.proxy.auth.handle_jwt.RouteChecks.is_auth_enforced_pass_through_route",
            return_value=True,
        ) as mock_is_auth_enforced_pass_through_route,
        patch(
            "derouter.proxy.auth.handle_jwt.RouteChecks.check_passthrough_route_access",
            return_value=False,
...
async def test_check_admin_access_result_includes_user_email():
    ...
    jwt_handler = JWTHandler()
    jwt_handler.derouter_jwtauth = DeRouter_JWTAuth(
        admin_jwt_scope="derouter_proxy_admin",
...
async def test_sync_user_role_and_teams():
    ...
    jwt_handler.update_environment(
        ...
        derouter_jwtauth=DeRouter_JWTAuth(
            jwt_derouter_role_map=[
                JWTDeRouterRoleMap(
                    jwt_role="ADMIN", derouter_role=LitellmUserRoles.PROXY_ADMIN
                )
            ],
    ...
    prisma.db.derouter_usertable.update.assert_called_once()
...
async def test_sync_user_role_and_teams_cache_invalidation_on_role_change():
    ...
    assert isinstance(call_kwargs.kwargs["value"], DeRouter_UserTable)
    assert call_kwargs.kwargs["value"].user_role == LitellmUserRoles.PROXY_ADMIN.value
    assert call_kwargs.kwargs["model_type"] == DeRouter_UserTable
...
@pytest.mark.asyncio
async def test_map_jwt_role_to_derouter_role():
    """Test JWT role mapping to DeRouter roles with various patterns"""
    from unittest.mock import MagicMock
    ...
    jwt_handler.update_environment(
        ...
        derouter_jwtauth=DeRouter_JWTAuth(
            jwt_derouter_role_map=[
                ...
                # Wildcard patterns
                JWTDeRouterRoleMap(
                    jwt_role="user_*", derouter_role=LitellmUserRoles.INTERNAL_USER
                ),
                JWTDeRouterRoleMap(
                    jwt_role="team_?", derouter_role=LitellmUserRoles.TEAM
                ),
                JWTDeRouterRoleMap(
                    jwt_role="dev_[123]", derouter_role=LitellmUserRoles.INTERNAL_USER
                ),
            ],
            roles_jwt_field="roles",
        ),
    ...
    # Test exact match
    token = {"roles": ["ADMIN"]}
    result = jwt_handler.map_jwt_role_to_derouter_role(token)
    ...
    jwt_handler.derouter_jwtauth.jwt_derouter_role_map = None
    token = {"roles": ["ADMIN"]}
    result = jwt_handler.map_jwt_role_to_derouter_role(token)
    assert result is None

    # Test empty role mappings
    jwt_handler.derouter_jwtauth.jwt_derouter_role_map = []
    token = {"roles": ["ADMIN"]}
    result = jwt_handler.map_jwt_role_to_derouter_role(token)
    assert result is None
    ...
    jwt_handler.derouter_jwtauth.jwt_derouter_role_map = [
        JWTDeRouterRoleMap(jwt_role="team_?", derouter_role=LitellmUserRoles.TEAM),
    ]
...
async def test_nested_jwt_field_access():
    ...
    jwt_handler.derouter_jwtauth = DeRouter_JWTAuth(user_id_jwt_field="user.sub")
    assert jwt_handler.get_user_id(nested_token, None) == "u123"

    # Test 1b: user_id_jwt_field with flat access (backward compatibility)
    jwt_handler.derouter_jwtauth = DeRouter_JWTAuth(user_id_jwt_field="sub")
    assert jwt_handler.get_user_id(flat_token, None) == "u123"

    # Test 2: user_email_jwt_field with nested access
    jwt_handler.derouter_jwtauth = DeRouter_JWTAuth(user_email_jwt_field="user.email")
    ...
    jwt_handler.derouter_jwtauth = DeRouter_JWTAuth(user_email_jwt_field="email")
    assert jwt_handler.get_user_email(flat_token, None) == "user@example.com"

    # Test 3: team_ids_jwt_field with nested access
    jwt_handler.derouter_jwtauth = DeRouter_JWTAuth(team_ids_jwt_field="groups")
    ...
    jwt_handler.derouter_jwtauth = DeRouter_JWTAuth(end_user_id_jwt_field="end_user_id")
    assert jwt_handler.get_end_user_id(flat_token, None) == "customer123"

    # Test 7: team_id_jwt_field with nested access
    jwt_handler.derouter_jwtauth = DeRouter_JWTAuth(team_id_jwt_field="tenant.team_id")
    ...
    jwt_handler.derouter_jwtauth = DeRouter_JWTAuth(team_id_jwt_field="team_id")
...
async def test_find_team_with_model_access_model_group(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "derouter.proxy.proxy_server", proxy_server_module)

    team = DeRouter_TeamTable(team_id="team-1", models=["test-group"])

    async def mock_get_team_object(*args, **kwargs):  # type: ignore
        return team

    monkeypatch.setattr(
        "derouter.proxy.auth.handle_jwt.get_team_object", mock_get_team_object
    )
...
async def test_find_team_with_model_access_v1_messages_default_routes(monkeypatch):
    ...
    x-derouter-team-id header. /v1/messages lives in `anthropic_routes`, so when a
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "claude-sonnet-4-6"},
    ...
    team = DeRouter_TeamTable(team_id="coding-team", models=["coding_only_models"])
...
async def test_auth_builder_returns_team_membership_object():
    ...
    mock_team_membership = DeRouter_TeamMembership(
        user_id=_user_id,
        team_id=_team_id,
    ...
    team_object = DeRouter_TeamTable(team_id=_team_id)
...
def test_get_team_id_from_header():
    ...
    result = JWTAuthManager.get_team_id_from_header(
        request_headers={"x-derouter-team-id": "team-1"},
    ...
    with pytest.raises(HTTPException) as exc_info:
        JWTAuthManager.get_team_id_from_header(
            request_headers={"x-derouter-team-id": "invalid-team"},
            allowed_team_ids={"team-1", "team-2"},
        )
    assert exc_info.value.status_code == 403
...
async def test_auth_builder_uses_team_from_header_e2e():
    """Test auth_builder e2e flow: selects team from x-derouter-team-id header."""
    from derouter.caching import DualCache
    from derouter.proxy.utils import ProxyLogging
    ...
    team_object = DeRouter_TeamTable(team_id="team-2")
    user_object = DeRouter_UserTable(
    ...
    with (
        ...
        patch(
            "derouter.proxy.auth.handle_jwt.get_team_object", new_callable=AsyncMock
        ) as mock_get_team,
        patch.object(
            JWTAuthManager,
            "get_objects",
        ...
        result = await JWTAuthManager.auth_builder(
            ...
            parent_otel_span=None,
            proxy_logging_obj=ProxyLogging(user_api_key_cache=user_api_key_cache),
            request_headers={"x-derouter-team-id": "team-2"},
        )
...
async def test_auth_builder_header_team_denies_auth_passthrough_without_allowlist():
    ...
    team_object = DeRouter_TeamTable(team_id="team-2", metadata={})
...
async def test_auth_builder_specific_team_denies_auth_passthrough_without_allowlist():
    ...
    team_object = DeRouter_TeamTable(team_id="team-1", metadata={})
...
async def test_auth_builder_rbac_team_denies_passthrough_without_allowlist():
    ...
    team_object = DeRouter_TeamTable(team_id="team-rbac", metadata={})
...
async def test_auth_builder_admin_on_llm_route_honors_team_header():
    """JWT proxy_admin + x-derouter-team-id on an LLM API route -> team context is
    ...
    team_object = DeRouter_TeamTable(team_id="team-low", tpm_limit=100, rpm_limit=2)
    ...
    with (
        ...
        result = await JWTAuthManager.auth_builder(
            ...
            parent_otel_span=None,
            proxy_logging_obj=ProxyLogging(user_api_key_cache=user_api_key_cache),
            request_headers={"x-derouter-team-id": "team-low"},
        )
...
async def test_auth_builder_admin_on_mgmt_route_ignores_team_header():
    """JWT proxy_admin + x-derouter-team-id on an admin management route -> header
    ...
    with (
        ...
        result = await JWTAuthManager.auth_builder(
            ...
            parent_otel_span=None,
            proxy_logging_obj=ProxyLogging(user_api_key_cache=user_api_key_cache),
            request_headers={"x-derouter-team-id": "totally-made-up-team"},
        )
...
async def test_auth_builder_admin_on_llm_route_without_header_unchanged():
    """JWT proxy_admin on an LLM API route without x-derouter-team-id -> no team
...
async def test_get_team_alias_with_nested_fields():
    ...
    jwt_handler.derouter_jwtauth = DeRouter_JWTAuth(team_alias_jwt_field="team_name")
    ...
    jwt_handler.derouter_jwtauth = DeRouter_JWTAuth()  # team_alias_jwt_field is None
...
async def test_find_and_validate_specific_team_id_with_team_alias():
    ...
    from derouter.caching import DualCache
    from derouter.proxy._types import DeRouter_JWTAuth, DeRouter_TeamTable
    from derouter.proxy.auth.handle_jwt import JWTAuthManager, JWTHandler
    from derouter.proxy.utils import ProxyLogging
    ...
    jwt_handler.update_environment(
        prisma_client=None,
        user_api_key_cache=user_api_key_cache,
        derouter_jwtauth=DeRouter_JWTAuth(team_alias_jwt_field="team_alias"),
    )
    ...
    team_object = DeRouter_TeamTable(team_id="resolved-team-id", team_alias="my-team")

    with patch(
        "derouter.proxy.auth.handle_jwt.get_team_object_by_alias", new_callable=AsyncMock
...
async def test_find_and_validate_team_id_takes_precedence_over_name():
    ...
    team_object = DeRouter_TeamTable(team_id="direct-team-id")
    ...
    with (
        ...
        patch(
            "derouter.proxy.auth.handle_jwt.get_team_object_by_alias",
            new_callable=AsyncMock,
        ) as mock_get_by_alias,
    ):
...
async def test_get_org_alias_with_nested_fields():
    ...
    # Test flat access
    jwt_handler.derouter_jwtauth = DeRouter_JWTAuth(org_alias_jwt_field="org_name")
...
async def test_get_objects_resolves_org_by_name():
    ...
    """
    from derouter.caching import DualCache
    from derouter.proxy._types import DeRouter_JWTAuth, DeRouter_OrganizationTable
    from derouter.proxy.auth.handle_jwt import JWTAuthManager, JWTHandler
    from derouter.proxy.utils import ProxyLogging
    ...
    jwt_handler.update_environment(
        prisma_client=None,
        user_api_key_cache=user_api_key_cache,
        derouter_jwtauth=DeRouter_JWTAuth(org_alias_jwt_field="org_alias"),
    )

    # Mock org object returned by get_org_object_by_alias
    org_object = DeRouter_OrganizationTable(
    ...
    with patch(
        "derouter.proxy.auth.handle_jwt.get_org_object_by_alias", new_callable=AsyncMock
    ) as mock_get_by_alias:
...
def _make_jwt_handler(team_id_jwt_field: str) -> JWTHandler:
    ...
    handler.update_environment(
        prisma_client=None,
        user_api_key_cache=DualCache(),
        derouter_jwtauth=DeRouter_JWTAuth(team_id_jwt_field=team_id_jwt_field),
    )
    return handler
...
async def test_find_and_validate_specific_team_id_hints_bracket_notation():
    ...
    using 'roles' instead (and explain DeRouter auto-unwraps list values).
    """
    from unittest.mock import MagicMock
    ...
    with pytest.raises(Exception, match="is not supported\\. Use 'roles' instead — DeRouter") as exc_info:
        await JWTAuthManager.find_and_validate_specific_team_id(
            jwt_handler=handler,
...
async def test_auth_builder_single_team_db_fallback_when_jwt_has_no_team(
    ...
    if len(user_teams) == 1 and get_team_object_return == "resolved_row":
        only = user_teams[0]
        team_table = DeRouter_TeamTable(team_id=only)
        membership = DeRouter_TeamMembership(
            user_id=user_id, team_id=only, derouter_budget_table=None
        )
    ...
    with (
        ...
        patch(
            "derouter.proxy.auth.handle_jwt.get_team_membership",
            new_callable=AsyncMock,
...
async def test_auth_builder_single_team_fallback_membership_error_skips_no_raise():
    ...
    team_table = DeRouter_TeamTable(team_id=team_id_val)
    jwt_handler = JWTHandler()
    jwt_handler.derouter_jwtauth = DeRouter_JWTAuth()
...
# but do not resolve to a DeRouter team.
# ---------------------------------------------------------------------------
...
async def test_find_team_with_model_access_unresolved_group_claim_returns_none(
    ...
    monkeypatch.setattr("derouter.proxy.auth.handle_jwt.get_team_object", raise_404)

    jwt_handler = JWTHandler()
    jwt_handler.derouter_jwtauth = DeRouter_JWTAuth(team_claim_fallback=True)
...
async def test_find_team_with_model_access_enforce_team_based_access_still_raises():
    ...
    jwt_handler = JWTHandler()
    jwt_handler.derouter_jwtauth = DeRouter_JWTAuth(enforce_team_based_model_access=True)
...
async def test_find_team_with_model_access_resolved_team_without_model_still_raises_403(
    ...
    DeRouter team but that team does not grant the requested model, keep the
    ...
    team = DeRouter_TeamTable(team_id="real-team", models=["gpt-3.5-turbo"])
...
async def test_find_team_with_model_access_unresolved_group_claim_default_raises(
    ...
    to any DeRouter team must still raise 403 — preserves the strict
...
def test_canonical_user_id_rebinds_to_legacy_uuid():
    ...
    user_object = DeRouter_UserTable(user_id=legacy_uuid, user_email=jwt_email)
...
def test_canonical_user_id_no_change_when_ids_match():
    ...
    user_object = DeRouter_UserTable(user_id=same, user_email=same)
...
async def test_auth_jwt_expired_token_raises_401_jwk_path():
    ...
    with (
        ...
        patch(
            "derouter.proxy.auth.handle_jwt.jwt.get_unverified_header",
            return_value={"kid": "test-kid"},
        ),
        patch(
            "derouter.proxy.auth.handle_jwt.PyJWK.from_dict",
            return_value=MagicMock(key="fake-key"),
        ),
        patch(
            "derouter.proxy.auth.handle_jwt.jwt.decode",
...
async def test_auth_jwt_expired_token_raises_401_pem_cert_path():
    ...
    with (
        ...
        patch(
            "derouter.proxy.auth.handle_jwt.x509.load_pem_x509_certificate",
...
def _get_jwt_handler_with_issuer_keys(issuers: list, keys_by_url: dict) -> JWTHandler:
    ...
    for jwks_url, keys in keys_by_url.items():
        cache.set_cache(
            key=f"derouter_jwt_auth_keys_{jwks_url}",
            value=keys,
        )
    ...
    jwt_handler.update_environment(
        prisma_client=None,
        user_api_key_cache=cache,
        derouter_jwtauth=DeRouter_JWTAuth(issuers=issuers),
    )
    return jwt_handler
...
async def test_get_public_key_fetches_and_caches_jwks_response():
    ...
    jwt_handler.update_environment(
        prisma_client=None,
        user_api_key_cache=cache,
        derouter_jwtauth=DeRouter_JWTAuth(public_key_ttl=123),
    )
    ...
    cached_keys = await cache.async_get_cache(
        key="derouter_jwt_auth_keys_https://issuer.example.com/keys"
    )
...
async def test_get_public_key_serves_stale_keys_when_jwks_refresh_fails():
    ...
    await cache.async_delete_cache(key=f"derouter_jwt_auth_keys_{jwks_url}")
...
async def test_stale_jwks_window_is_the_configured_grace_past_a_long_public_key_ttl():
    ...
    active_key = f"derouter_jwt_auth_keys_{jwks_url}"
...
async def test_removed_signing_key_stops_being_trusted_once_the_stale_window_expires(monkeypatch):
    ...
    active_cache_key = f"derouter_jwt_auth_keys_{jwks_url}"
...
async def test_zero_public_key_stale_ttl_fails_closed_instead_of_serving_stale_keys():
    ...
    assert await cache.async_get_cache(key=f"{STALE_CACHE_KEY_PREFIX}derouter_jwt_auth_keys_{jwks_url}") is None
...
async def test_public_key_stale_ttl_defaults_to_one_hour():
    ...
    assert DeRouter_JWTAuth().public_key_stale_ttl == 3600
...
async def test_get_public_key_tries_next_jwks_url_when_kid_missing(monkeypatch):
    ...
    cache = DualCache()
    cache.set_cache(key=f"derouter_jwt_auth_keys_{first_jwks_url}", value=[first_jwk])
    cache.set_cache(key=f"derouter_jwt_auth_keys_{second_jwks_url}", value=[second_jwk])
    jwt_handler = JWTHandler()
    jwt_handler.update_environment(
        prisma_client=None,
        user_api_key_cache=cache,
...
def test_get_jwks_url_for_issuer_falls_back_to_discovery_document():
    jwt_handler = JWTHandler()
    issuer_config = DeRouter_JWTAuth(
        issuers=[
            {
...
async def test_get_objects_team_membership_uses_rebound_user_id():
    ...
    resolved_user = DeRouter_UserTable(user_id=legacy_uuid, user_email=jwt_email)
    captured = {}
    ...
    with (
        patch(
            "derouter.proxy.auth.handle_jwt.get_user_object",
...
async def test_multi_issuer_jwt_validates_selected_issuer_and_maps_claims(
    ...
    token = _encode_rsa_jwt(
        ...
        extra_claims={
            ...
            "repository": "example-org/derouter-fork",
        },
    )

    claims = await jwt_handler.auth_jwt(token=token)

    assert claims[JWTHandler.DEROUTER_JWT_ISSUER_CLAIM] == issuer_two
...
async def test_multi_issuer_jwt_missing_mapped_claim_leaves_user_id_unset(
    ...
    """Mapped issuer claims behave like the global ``derouter_jwtauth`` path —
    present claims override the normalised value, missing ones simply leave
    the corresponding DeRouter-internal claim absent (rather than failing the
    ...
    assert claims[jwt_handler.DEROUTER_JWT_ISSUER_CLAIM] == issuer
    assert jwt_handler.DEROUTER_USER_ID_CLAIM not in claims
...
async def test_global_jwt_ignores_user_supplied_internal_claims(monkeypatch):
    ...
    cache = DualCache()
    cache.set_cache(key=f"derouter_jwt_auth_keys_{jwks_url}", value=[jwk])
    ...
    token = _encode_rsa_jwt(
        ...
        extra_claims={
            ...
            JWTHandler.DEROUTER_JWT_ISSUER_CLAIM: "https://issuer.example.com",
            JWTHandler.DEROUTER_USER_ID_CLAIM: "victim-user",
            JWTHandler.DEROUTER_USER_EMAIL_CLAIM: "victim@example.com",
            JWTHandler.DEROUTER_TEAM_ID_CLAIM: "victim-team",
            JWTHandler.DEROUTER_TEAM_IDS_CLAIM: ["victim-team"],
            JWTHandler.DEROUTER_ORG_ID_CLAIM: "victim-org",
            JWTHandler.DEROUTER_END_USER_ID_CLAIM: "victim-end-user",
        },
    )
...
async def test_multi_issuer_jwt_strips_unmapped_internal_claims(monkeypatch):
    ...
    assert JWTHandler.DEROUTER_USER_ID_CLAIM not in claims
    assert JWTHandler.DEROUTER_TEAM_ID_CLAIM not in claims
...
# claims (config flag on DeRouter_JWTAuth)
# ---------------------------------------------------------------------------


def test_get_team_id_from_header_defers_to_db_membership_only_without_jwt_claims():
    """With fallback_to_db_teams=True, an x-derouter-team-id header is accepted
    ...
    deferred = JWTAuthManager.get_team_id_from_header(
        request_headers={"x-derouter-team-id": "team-from-db"},
    ...
    with pytest.raises(HTTPException) as exc_info:
        JWTAuthManager.get_team_id_from_header(
            request_headers={"x-derouter-team-id": "team-x"},
            allowed_team_ids={"team-1", "team-2"},
            fallback_to_db_teams=True,
        )
    assert exc_info.value.status_code == 403
...
def _db_fallback_handler(derouter_jwtauth: Optional[DeRouter_JWTAuth] = None) -> JWTHandler:
    handler = JWTHandler()
    handler.derouter_jwtauth = derouter_jwtauth or DeRouter_JWTAuth()
    return handler
...
async def test_resolve_db_team_fallback_skips_unresolvable_membership():
    ...
    resolved = DeRouter_TeamTable(team_id="real_team")
...
async def test_auth_builder_db_team_fallback_when_jwt_has_no_team(
    ...
    team, honors a valid x-derouter-team-id header, and rejects a header team the
    ...
    request_headers = {"x-derouter-team-id": header_team_id} if header_team_id else None

    async def fake_get_team(team_id, **kwargs):
        return DeRouter_TeamTable(team_id=team_id)
    ...
    async def call_auth_builder():
        with (
            ...
            patch(
                "derouter.proxy.auth.handle_jwt.get_team_membership",
                new_callable=AsyncMock,
                return_value=DeRouter_TeamMembership(
                    user_id=user_id,
...
async def test_resolve_db_team_fallback_skips_team_without_model_access():
    ...
    teams = {
        "restricted_team": DeRouter_TeamTable(
            team_id="restricted_team", models=["claude-3"]
        ),
        "allowed_team": DeRouter_TeamTable(team_id="allowed_team", models=["gpt-4"]),
    }
...
async def test_resolve_db_team_fallback_enforces_team_allowed_routes():
    ...
    team = DeRouter_TeamTable(team_id="team_a", models=["gpt-4"])
    handler = _db_fallback_handler(DeRouter_JWTAuth(team_allowed_routes=["openai_routes"]))
...
def test_validate_header_team_in_db_membership_does_not_leak_team_ids():
    """The 403 raised for an x-derouter-team-id header outside the user's DB
...
async def _run_auth_builder_with_header_team(
    jwt_auth_config: DeRouter_JWTAuth,
    token: dict,
    header_team_id: str,
    user_object: DeRouter_UserTable,
    fake_get_team,
    ...
    jwt_handler = JWTHandler()
    jwt_handler.derouter_jwtauth = jwt_auth_config
    with (
        patch.object(
        ...
        return await JWTAuthManager.auth_builder(
            ...
            parent_otel_span=None,
            proxy_logging_obj=None,
            request_headers={"x-derouter-team-id": header_team_id},
        )
...
async def test_auth_builder_header_team_not_found_matches_non_membership_denial() -> (
    ...
    """A provisional x-derouter-team-id naming a nonexistent team must produce
...
async def test_resolve_db_team_fallback_loads_team_membership():
    ...
    would silently skip DeRouter_TeamMembership budget checks for every request."""
    user_object = DeRouter_UserTable(
...
async def test_auth_builder_db_fallback_does_not_validate_rbac_team_against_db_membership():
    ...
    x-derouter-team-id header). That RBAC-asserted team must not be re-validated
...
async def test_resolve_db_team_fallback_distinguishes_no_membership_vs_model_denied():
    ...
    membership_user = DeRouter_UserTable(
        user_id="u_no_model",
        user_role=LitellmUserRoles.INTERNAL_USER,
        teams=["only_team"],
    )
    no_membership_user = DeRouter_UserTable(
        user_id="u_empty",
        user_role=LitellmUserRoles.INTERNAL_USER,
        teams=[],
    )
    ...
    async def fake_get_team(team_id, **kwargs):
        return DeRouter_TeamTable(team_id=team_id, models=["other"])
...
async def test_auth_builder_alias_only_token_resolves_alias_not_db_fallback():
    ...
    async def fake_get_team_by_alias(team_alias, **kwargs):
        return DeRouter_TeamTable(team_id="alias_resolved_team", team_alias=team_alias)
...
async def test_find_and_validate_specific_team_id_team_id_default_used_without_alias():
    ...
    jwt_token = {"sub": "user-1"}
    default_team = DeRouter_TeamTable(team_id="config_default_team")
...
async def test_auth_builder_db_fallback_enforces_passthrough_route_access():
    ...
    jwt_handler = JWTHandler()
    jwt_handler.derouter_jwtauth = DeRouter_JWTAuth(fallback_to_db_teams=True)
...
async def test_auth_builder_provisional_header_team_is_not_upserted():
    """A provisional x-derouter-team-id (accepted only because the JWT carries no
    ...
    with (
        ...
        result = await JWTAuthManager.auth_builder(
            ...
            parent_otel_span=None,
            proxy_logging_obj=None,
            request_headers={"x-derouter-team-id": header_team},
        )
...
async def test_auth_builder_header_cannot_override_rbac_team_under_db_fallback():
    ...
    by sending x-derouter-team-id for any other team they happen to belong to:
    ...
    with (
        ...
        with pytest.raises(HTTPException) as exc_info:
            await JWTAuthManager.auth_builder(
                ...
                parent_otel_span=None,
                proxy_logging_obj=None,
                request_headers={"x-derouter-team-id": other_team},
            )
...
async def test_auth_builder_header_team_enforces_team_allowed_routes_under_db_fallback():
    """A claimless JWT with x-derouter-team-id under fallback_to_db_teams must
