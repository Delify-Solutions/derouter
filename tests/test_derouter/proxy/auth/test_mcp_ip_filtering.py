...
class TestResolveNumTrustedHops:
    ...
    def test_below_minimum_warns_so_misconfig_is_visible(self):
        with patch(
            "derouter.proxy.auth.ip_address_utils.verbose_proxy_logger"
        ) as mock_logger:
            assert IPAddressUtils._resolve_num_trusted_hops(0) == _HopCountInvalid()
...
class TestXffPresentButDisabledWarning:
    ...
    def test_warns_and_does_not_fail_when_xff_present_but_disabled(self):
        ...
        with patch(
            "derouter.proxy.auth.ip_address_utils.verbose_proxy_logger.error"
        ) as mock_error:
            result = IPAddressUtils.get_mcp_client_ip(
...
class TestIsRequestHttps:
    ...
    derouter only sees a plain-HTTP hop when TLS terminates at a reverse
    ...
    def test_proxy_base_url_http_overrides_literal_https_scheme(self, monkeypatch):
        ...
        monkeypatch.setenv("PROXY_BASE_URL", "http://derouter.internal")
        request = _make_scheme_request("https")
        assert IPAddressUtils.is_request_https(request, general_settings={}) is False
