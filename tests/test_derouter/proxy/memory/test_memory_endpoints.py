...
from derouter.proxy.memory.memory_endpoints import _visibility_filter, router
...
class _InMemoryMemoryTable:
    """Tiny fake of prisma_client.db.derouter_memorytable used by the endpoints."""
...
class _InMemoryTeamTable:
    """
    Tiny fake of `prisma_client.db.derouter_teamtable` — only `find_unique`
...
def _make_team(team_id: str, *, admin_user_ids: List[str]) -> Any:
    ...
    now = datetime.now(timezone.utc)
    return prisma_models.DeRouter_TeamTable(
        team_id=team_id,
...
def _make_prisma() -> MagicMock:
    client = MagicMock()
    client.db = MagicMock()
    client.db.derouter_memorytable = _InMemoryMemoryTable()
    client.db.derouter_teamtable = _InMemoryTeamTable()
    return client
...
def _patch_prisma(prisma: Any):
    ...
    return patch(
        "derouter.proxy.memory.memory_endpoints._require_prisma",
        return_value=prisma,
    )
...
class TestMemoryEndpoints:
    ...
    def test_create_memory_with_metadata_jsonifies_for_prisma(self):
        ...
        table = self.prisma.db.derouter_memorytable
    ...
    def test_put_memory_creates_when_missing(self):
        ...
        assert len(self.prisma.db.derouter_memorytable.rows) == 1
    ...
    def test_put_memory_team_admin_can_modify_pure_team_row(self):
        ...
        self.prisma.db.derouter_teamtable.teams.append(
    ...
    def test_internal_error_does_not_leak_db_details(self):
        ...
        async def boom(*_args, **_kwargs):
            raise Exception(
                ...
                '"DeRouter_MemoryTable.value" does not exist '
        ...
        body_text = resp.text
        for leak in ("DeRouter_MemoryTable", "db.internal", "PrismaClient"):
            assert (
