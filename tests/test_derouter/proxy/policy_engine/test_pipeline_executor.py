...
from derouter.proxy.guardrails.guardrail_hooks.custom_code.custom_code_guardrail import (
    CustomCodeGuardrail,
)
from derouter.proxy.policy_engine.pipeline_executor import PipelineExecutor, UndeliverableStreamRewrite
...
async def test_terminal_block_carries_guardrail_information_to_request(
    ...
    monkeypatch.setattr(derouter, "callbacks", [guard])
    data = {
...
async def test_terminal_block_merges_guardrail_information_without_duplicates(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [first, second])
...
async def test_escalation_step1_fails_step2_blocks(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [simple_guard, advanced_guard])
...
async def test_passthrough_guardrail_failure_can_pipeline_block(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [passthrough_guard])
...
async def test_custom_code_guardrail_failure_can_pipeline_block(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [custom_guard])
...
async def test_data_forwarding_pii_masking(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [pii_guard, content_guard])
...
async def test_scan_raw_request_step_sees_pre_pipeline_content(monkeypatch):
    """
    veria-ai finding on Delify-Solutions/derouter#34940: a scan_raw_request=True guardrail
...
async def test_on_error_next_fallback_on_api_outage_on_fail_blocks_content(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [primary, fallback])
...
async def test_on_fail_next_on_content_on_error_block_stops_api_fallback(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [primary_content, fallback])
    ...
    fallback.calls = 0
    monkeypatch.setattr(derouter, "callbacks", [primary_api, fallback])
    result = await PipelineExecutor.execute_steps(
        steps=pipeline_content.steps,
        mode=pipeline_content.mode,
...
async def test_guardrail_not_found_with_next_continues(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [pass_guard])
...
async def test_allow_restores_independent_guardrails_list(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [pipeline_guard])
...
class _TextTranslation:
    ...
    async def process_output_streaming_response(
        self, responses_so_far, guardrail_to_apply, derouter_logging_obj=None, user_api_key_dict=None, request_data=None
    ):
...
async def test_streaming_step_discards_text_rewrite_when_translation_lacks_write_back(monkeypatch, caplog):
    monkeypatch.setattr(derouter, "callbacks", [_TextReturningGuardrail(["hello [MASKED]"])])
...
async def test_streaming_step_unchanged_texts_in_another_container_allow(monkeypatch, caplog):
    monkeypatch.setattr(derouter, "callbacks", [_TextReturningGuardrail(("hello world",))])
...
async def test_streaming_step_in_place_rewrite_is_discarded_without_write_back(monkeypatch, caplog):
    monkeypatch.setattr(derouter, "callbacks", [_InPlaceMutatingGuardrail()])
    chunks = [_chunk()]
...
async def test_streaming_step_delivers_text_rewrite_through_writing_translation(monkeypatch, caplog):
    monkeypatch.setattr(derouter, "callbacks", [_TextAndToolCallRewritingGuardrail(rewrite_tool_call=False)])
    chunks = [_chunk()]
...
async def test_streaming_step_discards_tool_call_rewrite_and_restores_written_text(monkeypatch, caplog):
    monkeypatch.setattr(derouter, "callbacks", [_TextAndToolCallRewritingGuardrail(rewrite_tool_call=True)])
    chunks = [_chunk()]
...
async def test_streaming_step_records_guardrail_information_once_on_block(monkeypatch):
    monkeypatch.setattr(derouter, "callbacks", [_BlockingStreamGuardrail()])
