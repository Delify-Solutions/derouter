...
import derouter.proxy.policy_engine.policy_endpoints as policy_endpoints
from derouter.proxy.policy_engine.attachment_registry import AttachmentRegistry
...
class TestListPoliciesIncludesConfig:
    ...
    async def test_merges_db_rows_with_config_and_keeps_db_row_shape(self, policy_registry, monkeypatch):
        ...
        prisma = MagicMock()
        prisma.db.derouter_policytable.find_many = AsyncMock(return_value=[row])
    ...
    async def test_stale_registry_provenance_does_not_hide_config_policy(self, policy_registry, monkeypatch):
        ...
        sync_prisma.db.derouter_policytable.find_many = AsyncMock(side_effect=[[production_row], []])
        ...
        fresh_prisma.db.derouter_policytable.find_many = AsyncMock(return_value=[])
...
class TestListAttachmentsIncludesConfig:
    ...
    async def test_merges_db_attachments_with_config_and_keeps_db_row_shape(self, attachment_registry, monkeypatch):
        ...
        prisma = MagicMock()
        prisma.db.derouter_policyattachmenttable.find_many = AsyncMock(return_value=[row])
        _set_prisma(monkeypatch, prisma)
        attachment_registry.load_attachments([{"policy": "config-policy", "scope": "*"}])
