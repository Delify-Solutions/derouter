...
`derouter.completion` accepts pricing fields (`input_cost_per_token`,
`output_cost_per_token`, the rest of `CustomPricingDeRouterParams`,
...
their own per-request cost and — via `derouter.register_model` — mutate
`derouter.model_cost` for every subsequent caller in the worker. The proxy
...
class TestStripClientPricingOverrides:
    def test_pricing_field_set_tracks_pydantic_model(self):
        ...
        assert _CLIENT_PRICING_CONTROL_FIELDS == frozenset(
            CustomPricingDeRouterParams.model_fields.keys()
        )
    ...
    def test_metadata_strip_handles_non_dict_metadata(self):
        ...
        _strip_client_pricing_overrides({"derouter_metadata": ["a", "b"]})
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_strips_root_pricing_fields():
    data = {
        "model": "gpt-4",
        "messages": [{"role": "user", "content": "hi"}],
        "input_cost_per_token": 0.0,
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_strips_client_disconnect_metadata():
    data = {
        "model": "gpt-4",
        "messages": [{"role": "user", "content": "hi"}],
        "metadata": {
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_strips_metadata_model_info():
    data = {
        "model": "gpt-4",
        "messages": [{"role": "user", "content": "hi"}],
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_skips_strip_with_key_opt_in():
    data = {
        "model": "gpt-4",
        "messages": [{"role": "user", "content": "hi"}],
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_strips_json_string_derouter_metadata():
    """``derouter_metadata`` may arrive as a JSON-encoded string (multipart/
    ...
    data = {
        "model": "gpt-4",
        "messages": [{"role": "user", "content": "hi"}],
        "derouter_metadata": json.dumps({"model_info": {"input_cost_per_token": 0.0}}),
    }
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_skips_strip_with_team_opt_in():
    data = {
        "model": "gpt-4",
        "messages": [{"role": "user", "content": "hi"}],
        "input_cost_per_token": 0.0001,
...
async def test_global_model_cost_unmutated_after_stripped_request(monkeypatch):
    """After a stripped request, ``derouter.model_cost`` must not carry the
    caller's submitted pricing for the model. The mutation only happens when
    the pricing fields reach ``derouter.completion``; the strip prevents that."""
    snapshot = dict(derouter.model_cost)
    data = {
    ...
    assert "test-pricing-canary-model" not in derouter.model_cost
    # And no other entries were mutated as a side effect.
    assert derouter.model_cost == snapshot
