...
class TestHasPostCallGuardrails:
    def test_returns_true_for_post_call_guardrail(self):
        with patch("derouter.callbacks", [PostCallGuardrail()]):
    ...
    def test_returns_true_for_list_with_post_call(self):
        ...
        with patch("derouter.callbacks", [ListGuardrail()]):
            assert ProxyBaseLLMRequestProcessing._has_post_call_guardrails() is True
...
class TestHasPostCallGuardrailsForPassthrough:
    ...
    def test_request_scoped_guardrail_not_configured_for_key(self):
        ...
        with patch("derouter.callbacks", [OptInPostCall()]):
...
class TestDeferredStreamingClosure:
    ...
    async def test_closure_runs_only_guardrail_hooks(self):
        ...
        with patch("derouter.callbacks", [tracking_guardrail, tracking_logger]):
            resp = await derouter.acompletion(
                model="gpt-3.5-turbo",
                messages=[{"role": "user", "content": "hi"}],
                mock_response="Hello!",
            ...
            request_data = {"derouter_logging_obj": mock_logging_obj}
    ...
    async def test_closure_passes_guardrail_modified_response_to_logging(self):
        ...
        with patch("derouter.callbacks", [guardrail]):
            await ProxyBaseLLMRequestProcessing._run_deferred_stream_guardrails(
    ...
    async def test_hooks_receive_merged_guardrail_data(self):
        ...
        with (
            patch("derouter.callbacks", [guardrail]),
            patch(
                "derouter.proxy.utils._check_and_merge_model_level_guardrails",
    ...
    async def test_multiple_guardrails_all_receive_merged_data(self):
        ...
        with (
            patch("derouter.callbacks", [guardrail_a, guardrail_b]),
            patch(
                "derouter.proxy.utils._check_and_merge_model_level_guardrails",
                side_effect=mock_merge,
            ),
    ...
    async def test_deferred_logging_forces_async_for_sync_classified_call_type(self):
        ...
        type is classified as a sync SDK request by _is_sync_derouter_request.

        Without prefer_async_handlers=True, an async proxy stream whose
        derouter_params lacks a recognized async marker would enter the sync
        ...
        logging_obj = DeRouterLoggingObj(
            ...
            derouter_call_id="test-id",
            function_id="fn",
        )
        # derouter_params with no recognized async marker -> classified sync.
        logging_obj.model_call_details["derouter_params"] = {}
        assert DeRouterLoggingObj._is_sync_derouter_request({}) is True
        ...
        with (
            ...
            patch("derouter.callbacks", [PostCallGuardrail()]),
        ):
            await ProxyBaseLLMRequestProcessing._run_deferred_stream_guardrails(
...
class TestFireDeferredStreamLogging:
    ...
    async def test_noop_when_no_logging_obj(self):
        ...
        derouter_logging_obj is missing from request_data."""
        request_data = {}
        # Should not raise
        ProxyLogging._fire_deferred_stream_logging(request_data)
...
class TestArmDeferredStreamDispatch:
    ...
    (DeRouterCompletionStreamingIterator) shares its inner CustomStreamWrapper's
    ...
    async def test_bridged_responses_iterator_gets_csw_arg_shape(self):
        ...
        bridged = object.__new__(DeRouterCompletionStreamingIterator)
        ...
        ProxyLogging._fire_deferred_stream_logging({"derouter_logging_obj": logging_obj})
        await asyncio.sleep(0)
    ...
    async def test_router_wrapped_bridged_iterator_gets_csw_arg_shape(self):
        ...
        logging_obj, recorded = self._dispatch_recording_logging_obj()
        wrapped = HiddenParamsAsyncIteratorWrapper(object.__new__(DeRouterCompletionStreamingIterator))
