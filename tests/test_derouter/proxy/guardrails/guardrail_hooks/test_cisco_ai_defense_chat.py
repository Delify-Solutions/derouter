from tests.test_derouter.proxy.guardrails.guardrail_hooks._cisco_ai_defense_test_utils import (
    Any,
    AsyncMock,
    CHAT_URL,
    Choices,
...
def test_init_registers_on_both_callbacks_and_success_callback(monkeypatch):
    ...
    def _has_our_guardrail(callback_list):
        from derouter.proxy.guardrails.guardrail_hooks.cisco_ai_defense import (
            CiscoAIDefenseGuardrail,
        )
    ...
    assert _has_our_guardrail(derouter.callbacks), (
        "Cisco guardrail missing from derouter.callbacks — proxy's "
        "pre_call/during_call/post_call dispatch will skip it."
    )
    assert _has_our_guardrail(derouter.success_callback), (
        "Cisco guardrail missing from derouter.success_callback — "
        "derouter_logging.async_post_mcp_tool_call_hook will skip it, "
        ...
        with (
            ...
            if inspection_type == "chat" and direction == "output":
                ...
                await g.async_pre_call_hook(
                    user_api_key_dict=UserAPIKeyAuth(),
                    cache=DualCache(),
                    data=_mcp_request(name="tool", args={"x": 1}, derouter_call_id="c"),
                    call_type="mcp_call",
                )
