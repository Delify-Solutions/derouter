...
from derouter.proxy.guardrails.guardrail_hooks.cato_networks.cato_networks import (
...
from derouter.proxy.proxy_server import UserAPIKeyAuth
from derouter.types.utils import ModelResponse, ResponsesAPIResponse
...
def test_cato_guard_config(monkeypatch):
    ...
    cato_guardrails = [callback for callback in derouter.callbacks if isinstance(callback, CatoNetworksGuardrail)]
...
async def test_block_callback(mode: str):
    ...
    cato_guardrails = [
        callback for callback in derouter.callbacks if isinstance(callback, CatoNetworksGuardrail)
    ]
    assert len(cato_guardrails) == 1
...
def test_initialize_guardrail_forwards_ssl_verify(monkeypatch):
    ...
    guard = initialize_guardrail(derouter_params, {"guardrail_name": "cato-guard"})
...
def test_build_cato_headers_only_required_when_optionals_missing():
    ...
    assert headers["x-cato-derouter-hook"] == "pre_call"
    assert "x-cato-derouter-version" in headers
...
def test_build_cato_headers_includes_all_optionals_when_present():
    ...
    assert headers["x-cato-derouter-hook"] == "output"
...
async def test_call_cato_guardrail_forwards_user_email_from_auth():
    ...
    assert sent_headers["x-cato-derouter-hook"] == "pre_call"
...
def test_get_config_model_returns_pydantic_class():
    from derouter.types.proxy.guardrails.guardrail_hooks.cato_networks import (
...
async def test_streaming_iterator_yields_verified_chunks_and_cancels_sender():
    ...
    with patch(
        "derouter.proxy.guardrails.guardrail_hooks.cato_networks.cato_networks.connect",
        ...
        chunks = [
            ...
            async for chunk in guard.async_post_call_streaming_iterator_hook(
                ...
                request_data={"derouter_call_id": "stream-call"},
            )
        ]
    assert len(chunks) == 1
...
async def test_streaming_iterator_raises_on_connection_closed():
    guard = _make_guardrail()
    from derouter.proxy.proxy_server import StreamingCallbackError
...
async def test_cato_skips_embeddings_without_calling_the_guardrail(hook: str, call_type: str):
    ...
    with patch(  # test-quality-ok: transport is derouter's aiohttp-backed handler; respx cannot intercept it
        "derouter.llms.custom_httpx.http_handler.AsyncHTTPHandler.post",
        new_callable=AsyncMock,
    ) as mock_post:
        if hook == "pre_call":
