...
from derouter.types.guardrails import LitellmParams, PiiAction, PiiEntityType
...
async def test_presidio_filter_scope_initializer(monkeypatch):
    ...
    class DummyGuardrail:
        ...
        def update_in_memory_derouter_params(self, derouter_params):
            pass
    ...
    class DummyManager:
        ...
        def add_derouter_callback(self, cb):
            self.added.append(cb)

    mgr = DummyManager()
    monkeypatch.setattr(derouter, "logging_callback_manager", mgr, raising=False)
    import derouter.proxy.guardrails.guardrail_hooks.presidio as presidio_mod
    import derouter.proxy.guardrails.guardrail_initializers as gi
    ...
    # input-only
    created.clear()
    from derouter.proxy.guardrails.guardrail_initializers import initialize_presidio
...
def test_update_in_memory_applies_score_thresholds():
    """
    update_in_memory_derouter_params should refresh score thresholds.
    """
    guardrail = _OPTIONAL_PresidioPIIMasking(mock_testing=True)
    ...
    guardrail.update_in_memory_derouter_params(params)
...
async def test_apply_to_output_streaming_mixed_chunks_flushes_and_warns():
    ...
    mock_user_api_key = UserAPIKeyAuth(api_key="test-key")
    received = []
    with patch("derouter.proxy.guardrails.guardrail_hooks.presidio.verbose_proxy_logger") as mock_logger:
        async for chunk in guardrail.async_post_call_streaming_iterator_hook(
            user_api_key_dict=mock_user_api_key,
            response=mock_stream(),
            request_data={},
...
def test_presidio_analyze_chunk_size_default_and_validation():
    from derouter.constants import DEFAULT_PRESIDIO_ANALYZE_CHUNK_SIZE_BYTES
...
def test_update_in_memory_keeps_output_masker_from_unmasking():
    ...
    masker.update_in_memory_derouter_params(params)
    unmasker.update_in_memory_derouter_params(params)
...
async def test_chunked_analyze_concurrency_is_bounded():
    from derouter.constants import PRESIDIO_ANALYZE_CHUNK_CONCURRENCY
