...
from derouter._version import version as derouter_version
from derouter.exceptions import GuardrailRaisedException, Timeout
from derouter.proxy._types import UserAPIKeyAuth
from derouter.proxy.guardrails.guardrail_hooks.generic_guardrail_api import (
    GenericGuardrailAPI,
)
from derouter.proxy.guardrails.guardrail_hooks.generic_guardrail_api.generic_guardrail_api import (
...
class TestGenericGuardrailAPIConfiguration:
    ...
    def test_init_with_config(self):
        ...
        assert (
            guardrail.api_base
            == "https://api.test.guardrail.com/beta/derouter_basic_guardrail_api"
        )
    ...
    def test_init_with_env_vars(self):
        ...
        with patch.dict(
            ...
            assert (
                guardrail.api_base
                == "https://env.api.guardrail.com/beta/derouter_basic_guardrail_api"
            )
    ...
    def test_api_base_appends_endpoint(self):
        ...
        assert (
            guardrail.api_base
            == "https://api.test.guardrail.com/v1/beta/derouter_basic_guardrail_api"
        )

    def test_api_base_not_duplicated(self):
        """Test that endpoint path is not duplicated if already present"""
        guardrail = GenericGuardrailAPI(
            api_base="https://api.test.guardrail.com/beta/derouter_basic_guardrail_api",
        )
        assert (
            guardrail.api_base
            == "https://api.test.guardrail.com/beta/derouter_basic_guardrail_api"
...
class TestMetadataExtraction:
    ...
    async def test_extract_metadata_from_output_response(
        ...
        """Test extracting metadata from output response (derouter_metadata field)"""
        ...
        for key, value in user_dict.items():
            if value is not None and not key.startswith("_"):
                if key.startswith("user_api_key_"):
                    ...
                    derouter_metadata[f"user_api_key_{key}"] = value
        ...
        with patch.object(
            ...
            # Verify metadata was extracted from request_data["derouter_metadata"]
    ...
    @pytest.mark.asyncio
    async def test_inbound_headers_and_derouter_version_forwarded_and_sanitized(
        self, generic_guardrail, mock_request_data_input
    ):
        """
        ...
        with patch.object(
            ...
            assert json_payload["derouter_version"] == derouter_version
...
class TestErrorHandling:
    ...
    async def test_timeout_fail_open_allows_flow(self, mock_request_data_input):
        """Test derouter.Timeout allows flow when unreachable_fallback=fail_open"""
        guardrail = GenericGuardrailAPI(
            api_base="https://api.test.guardrail.com",
            headers={"Authorization": "Bearer test-key"},
            unreachable_fallback="fail_open",
...
def _make_stream_chunk(content: str, finish_reason=None):
    ...
    from derouter.types.utils import Delta, ModelResponseStream

    return ModelResponseStream(
        model="gpt-4",
        choices=[
            derouter.StreamingChoices(
                index=0,
...
def _make_assembled_model_response(content: str) -> ModelResponse:
    return ModelResponse(
        ...
        choices=[
            derouter.Choices(
                index=0,
                message=derouter.Message(role="assistant", content=content),
                finish_reason="stop",
            )
        ],
    )
...
class TestGenericGuardrailAPIStreamingConfig:
    ...
    def test_optional_params_streaming_sampling_rate_ge_one(self):
        ...
        from derouter.types.proxy.guardrails.guardrail_hooks.generic_guardrail_api import (
    ...
    def test_initialize_guardrail_forwards_streaming_flags(self):
        ...
        derouter_params.streaming_end_of_stream_only = False  # type: ignore[attr-defined]
        derouter_params.streaming_sampling_rate = 3  # type: ignore[attr-defined]
        ...
        with patch(
            ...
            guardrail = initialize_guardrail(derouter_params, guardrail_config)
    ...
    def test_initialize_guardrail_optional_params_defaults_do_not_shadow_top_level(
        ...
        derouter_params.streaming_end_of_stream_only = True  # type: ignore[attr-defined]
        derouter_params.streaming_sampling_rate = 2  # type: ignore[attr-defined]
        # Sibling optional_params only; streaming fields stay at Pydantic default None.
        derouter_params.optional_params = GenericGuardrailAPIOptionalParams(  # type: ignore[attr-defined]
    ...
    def test_initialize_guardrail_explicit_optional_params_streaming_wins(self):
        ...
        derouter_params.streaming_end_of_stream_only = False  # type: ignore[attr-defined]
        derouter_params.streaming_sampling_rate = 9  # type: ignore[attr-defined]
        derouter_params.optional_params = GenericGuardrailAPIOptionalParams(  # type: ignore[attr-defined]
            streaming_end_of_stream_only=True,
    ...
    def test_initialize_guardrail_dict_optional_params_streaming_wins(self):
        ...
        derouter_params.optional_params = {  # type: ignore[attr-defined]
            "streaming_end_of_stream_only": True,
            "streaming_sampling_rate": 1,
        }
...
class TestGenericGuardrailAPIStreamingViaUnified:
    ...
    async def test_streaming_safe_content_yields_all_chunks(self):
        ...
        with (
            ...
            patch(
                "derouter.llms.openai.chat.guardrail_translation.handler.stream_chunk_builder",
