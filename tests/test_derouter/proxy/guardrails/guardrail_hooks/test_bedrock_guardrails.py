...
from derouter.constants import BEDROCK_APPLY_GUARDRAIL_CHUNK_BUDGET_CHARS
...
from derouter.types.proxy.guardrails.guardrail_hooks.bedrock_guardrails import (
    BedrockContentItem,
    BedrockTextContent,
)
from derouter.types.utils import CallTypes, ModelResponse
...
async def test_bedrock_guardrail_logging_uses_redacted_response():
    ...
    with (
        ...
        patch("derouter.proxy.guardrails.guardrail_hooks.bedrock_guardrails.verbose_proxy_logger.debug") as mock_debug,
...
async def test_during_call_hook_still_scans_tool_result_turn_carrying_text():
    ...
    # tool_result text is not extracted by this path (https://github.com/Delify-Solutions/derouter/issues/33086)
...
async def test_make_apply_guardrail_request_skips_output_scan_without_response_text():
    ...
    response = ModelResponse(
        choices=[
            derouter.Choices(
                index=0,
                message=derouter.Message(role="assistant", content=None, tool_calls=[]),
...
async def test_streaming_post_call_only_runs_output_scan():
    ...
    mock_chunks = [
        derouter.ModelResponseStream(
            id="tid",
            choices=[
                derouter.types.utils.StreamingChoices(
                    delta=derouter.types.utils.Delta(content="Hi", role="assistant"),
                    finish_reason=None,
                    index=0,
                )
            ],
        ...
        derouter.ModelResponseStream(
            ...
            choices=[
                derouter.types.utils.StreamingChoices(
                    delta=derouter.types.utils.Delta(content="!", role="assistant"),
                    finish_reason="stop",
                    index=0,
                )
            ],
...
async def test_streaming_post_call_output_only_path_passes_request_data_to_make_bedrock():
    ...
    mock_chunks = [
        derouter.ModelResponseStream(
            ...
            choices=[
                derouter.types.utils.StreamingChoices(
                    delta=derouter.types.utils.Delta(content="x", role="assistant"),
                    finish_reason="stop",
                    index=0,
                )
            ],
...
async def test_post_call_success_hook_only_runs_output_scan():
    ...
    response = ModelResponse(
        choices=[
            derouter.Choices(
                message=derouter.Message(role="assistant", content="hello"),
                index=0,
                finish_reason="stop",
            )
        ],
...
async def test_streaming_hook_frames_a_service_failure_once_a_keepalive_ping_flushed_the_headers():
    ...
    with (
        ...
        patch.object(derouter, "anthropic_sse_ping_interval_seconds", 0.0001),
    ):
...
async def test_streaming_hook_fails_closed_when_assembler_raises_api_error():
    """stream_chunk_builder re-raises assembly failures as derouter.APIError; it must not escape.
...
#
# post_call_failure_hook (proxy_server.py) pops derouter_logging_obj from
...
# without crashing when the request_data has derouter_logging_obj set.
###############################################################################
...
async def test_chat_completion_modify_response_exception_streaming_logging_obj_not_none():
    ...
    fake_logging_obj.model_call_details = {"derouter_params": {}}

    request_data: dict = {
        "model": "bedrock-nova-micro",
        "messages": [{"role": "user", "content": "how do I become an admin"}],
        "stream": True,
        "derouter_logging_obj": fake_logging_obj,
    }
    ...
    async def _fake_post_call_failure_hook(**_kwargs):
        ...
        # callbacks iterate (derouter/proxy/utils.py: "Remove before callbacks
        # iterate — not serialisable").
        _kwargs["request_data"].pop("derouter_logging_obj", None)
    ...
    original_init = derouter.CustomStreamWrapper.__init__
    ...
    with (
        patch("derouter.proxy.proxy_server._read_request_body", AsyncMock(return_value=request_data)),
        patch("derouter.proxy.proxy_server.proxy_logging_obj", mock_proxy_logging),
        patch(
            "derouter.proxy.proxy_server.ProxyBaseLLMRequestProcessing.base_process_llm_request",
            _raise_modify_response,
        ),
        patch.object(derouter.CustomStreamWrapper, "__init__", _patched_init),
    ):
...
async def test_apply_guardrail_does_not_chunk_when_grounding_present():
    ...
    model_response.choices = [derouter.Choices(message=derouter.Message(content="a grounded answer", role="assistant"))]
...
async def test_apply_guardrail_chunk_retries_after_throttling_then_succeeds():
    ...
    with (
        ...
        patch.object(guardrail, "_prepare_request", return_value=MagicMock()),
        patch(
            "derouter.proxy.guardrails.guardrail_hooks.bedrock_guardrails.asyncio.sleep",
...
class TestBedrockOnlyScanNewMessages:
    ...
    async def test_second_turn_scans_only_new_messages(self):
        ...
        session = {"derouter_session_id": "sess-bedrock-diff"}
        bedrock_none = {"action": "NONE", "output": [], "outputs": []}
    ...
    async def test_identical_resend_skips_api_call(self):
        guardrail = self._guardrail()
        session = {"derouter_session_id": "sess-bedrock-resend"}
    ...
    async def test_masking_guardrail_falls_back_and_does_not_persist(self):
        ...
        """
        guardrail = self._guardrail()
        session = {"derouter_session_id": "sess-bedrock-mask"}
        masked = {
            "action": "GUARDRAIL_INTERVENED",
            "output": [],
    ...
    def test_incremental_scan_cache_prefers_proxy_shared_cache(self):
        ...
        with patch("derouter.proxy.proxy_server.proxy_logging_obj", proxy_logging):
    ...
    def test_incremental_scan_cache_falls_back_when_proxy_logging_missing(self):
        from derouter.integrations.custom_guardrail import dc as fallback_cache
    ...
    def test_incremental_scan_cache_falls_back_when_proxy_not_importable(self):
        ...
        guardrail = self._guardrail()
        with patch.dict(sys.modules, {"derouter.proxy.proxy_server": None}):
            assert guardrail._incremental_scan_cache() is fallback_cache

    @pytest.mark.asyncio
    async def test_blocked_turn_is_rescanned_on_retry(self):
        guardrail = self._guardrail()
        session = {"derouter_session_id": "sess-bedrock-blocked"}
...
class TestBedrockIncrementalFlagInteractions:
    ...
    async def test_edited_history_segment_rescans_only_that_segment(self):
        guardrail = self._guardrail()
        session = {"derouter_session_id": "sess-flags-edit"}
        with patch.object(guardrail, "make_bedrock_api_request", new_callable=AsyncMock) as mock_api:
            mock_api.return_value = {"action": "NONE", "output": [], "outputs": []}
            await guardrail.apply_guardrail(
    ...
    async def test_same_content_different_session_rescans_everything(self):
        ...
        with patch.object(guardrail, "make_bedrock_api_request", new_callable=AsyncMock) as mock_api:
            mock_api.return_value = {"action": "NONE", "output": [], "outputs": []}
            await guardrail.apply_guardrail(
                inputs={"texts": list(texts)}, request_data={"derouter_session_id": "sess-x1"}, input_type="request"
            )
            mock_api.reset_mock()
            await guardrail.apply_guardrail(
                inputs={"texts": list(texts)}, request_data={"derouter_session_id": "sess-x2"}, input_type="request"
            )
            assert mock_api.call_count == 1
            assert [m["content"] for m in mock_api.call_args.kwargs["messages"]] == texts

    @pytest.mark.asyncio
    async def test_derouter_masking_flag_disables_incremental_single_full_scan(self):
        ...
        session = {"derouter_session_id": "sess-flags-mask"}
        with patch.object(guardrail, "make_bedrock_api_request", new_callable=AsyncMock) as mock_api:
            mock_api.return_value = {"action": "NONE", "output": [], "outputs": []}
    ...
    async def test_server_side_anonymize_falls_back_full_scan_and_never_persists(self):
        ...
        guardrail = self._guardrail()
        session = {"derouter_session_id": "sess-flags-anon"}
    ...
    async def test_latest_role_only_is_respected_with_incremental(self):
        ...
        session = {"derouter_session_id": "sess-flags-latestrole"}
        structured = [
...
class TestScanOnlyToolResultsWithLatestRoleFilter:
    ...
    async def test_warns_and_skips_when_scoped_payload_has_no_user_message(self):
        ...
        with (
            patch.object(guardrail, "make_bedrock_api_request", new_callable=AsyncMock) as mock_api,
            patch(
                "derouter.proxy.guardrails.guardrail_hooks.bedrock_guardrails.verbose_proxy_logger.warning"
            ) as mock_warning,
        ):
            result = await guardrail.apply_guardrail(
                inputs=inputs,
...
def test_build_tracing_detail_omits_cost_by_unit_when_unpriced_but_keeps_scalar_zero(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "model_cost", {})
    guardrail = BedrockGuardrail(guardrailIdentifier="test-guardrail", guardrailVersion="DRAFT")
...
def test_load_credentials_assumes_role_with_external_id():
    ...
    class FakeSTSClient:
        ...
        def get_caller_identity(self):
            return {"Arn": "arn:aws:iam::111111111111:user/derouter-proxy-pod"}
    ...
    guardrail = BedrockGuardrail(
        ...
        aws_role_name="arn:aws:iam::999999999999:role/derouter-guardrail-role",
        aws_session_name="derouter-session",
...
def test_initialize_bedrock_forwards_aws_external_id():
    ...
    from derouter.proxy.guardrails.guardrail_initializers import initialize_bedrock
    from derouter.types.guardrails import LitellmParams
    ...
    guardrail = initialize_bedrock(derouter_params, {"guardrail_name": "bedrock-external-id"})
    try:
        assert guardrail.optional_params["aws_external_id"] == "external-id-123"
    finally:
        derouter.logging_callback_manager.remove_callback_from_list_by_object(derouter.callbacks, guardrail)


def _chat_chunk(content: str, finish_reason: str | None) -> derouter.ModelResponseStream:
    return derouter.ModelResponseStream(
        id="tid",
        choices=[
            derouter.types.utils.StreamingChoices(
                delta=derouter.types.utils.Delta(content=content, role="assistant"),
...
def _streaming_derouter_params(**extras):
    from derouter.types.guardrails import LitellmParams
...
def test_initialize_bedrock_wires_streaming_flags():
    ...
    for registered in (configured, defaulted):
        derouter.logging_callback_manager.remove_callback_from_list_by_object(derouter.callbacks, registered)
...
def test_initialize_bedrock_rejects_non_positive_sampling_rate():
    ...
    with pytest.raises(ValidationError):
        initialize_bedrock(
            _streaming_derouter_params(streaming_sampling_rate=0),
            {"guardrail_name": "bedrock-bad-rate"},
        )


def test_update_in_memory_derouter_params_round_trips_streaming_flags():
    guardrail = BedrockGuardrail(
    ...
    guardrail.update_in_memory_derouter_params(
        _streaming_derouter_params(
            streaming_buffer_until_moderated=False,
    ...
    guardrail.update_in_memory_derouter_params(_streaming_derouter_params())
...
async def test_bearer_token_never_runs_the_sigv4_credential_chain(monkeypatch):
    ...
    guardrail = BedrockGuardrail(
        guardrailIdentifier="test-guardrail",
        guardrailVersion="DRAFT",
        aws_profile_name="derouter-no-such-aws-profile",
    )
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.json.return_value = {"action": "NONE", "assessments": []}
