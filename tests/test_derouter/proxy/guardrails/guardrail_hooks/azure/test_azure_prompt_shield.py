...
def test_update_in_memory_derouter_params_applies_new_pricing_from_raw_dict():
    """The immediate PUT sync hands the raw DB dict to update_in_memory_derouter_params;
    the pricing extras must reach the live instance (base vars() loop never sees
    pydantic extras and rejects dicts outright)."""
    guardrail = _priced_shield_guardrail(cost_tier="paid", price_per_1000_text_records=0.38)

    guardrail.update_in_memory_derouter_params({"cost_tier": "paid", "price_per_1000_text_records": 0.76})

    assert guardrail.price_per_1000_text_records == 0.76
    assert guardrail.cost_tier == "paid"


def test_update_in_memory_derouter_params_rejects_invalid_pricing_untouched():
    ...
    with pytest.raises(ValueError, match="requires a positive price"):
        guardrail.update_in_memory_derouter_params({"cost_tier": "paid", "price_per_1000_text_records": None})

    assert guardrail.cost_tier == "paid"
    assert guardrail.price_per_1000_text_records == 0.38


def test_update_in_memory_derouter_params_reads_extras_from_pydantic_object():
...
def test_update_in_memory_derouter_params_resolves_env_credential_references(monkeypatch):
...
def test_update_in_memory_derouter_params_dead_env_credential_rejected_untouched(monkeypatch):
