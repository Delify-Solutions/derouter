...
from derouter.proxy.guardrails.guardrail_hooks.block_code_execution import (
    DEFAULT_EVENT_HOOKS,
    BlockCodeExecutionGuardrail,
    initialize_guardrail,
)
from derouter.proxy.guardrails.guardrail_hooks.block_code_execution.block_code_execution import (
...
```'''
    ...
    async def test_detection_includes_confidence_and_action_taken(self):
        ...
        meta = (
            request_data.get("metadata") or request_data.get("derouter_metadata") or {}
        )
    ...
    def test_initialize_guardrail_default_mode_is_both(self):
        ...
        derouter_params.guardrail = "block_code_execution"
        derouter_params.blocked_languages = ["python"]
        derouter_params.action = "block"
        derouter_params.confidence_threshold = 0.7
        derouter_params.default_on = False
        derouter_params.mode = None  # not set
        guardrail = {"guardrail_name": "block-code-test"}
        instance = initialize_guardrail(derouter_params, guardrail)
