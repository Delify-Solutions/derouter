...
from derouter.proxy.guardrails.guardrail_hooks.noma import (
    NomaGuardrail,
    NomaV2Guardrail,
    initialize_guardrail,
)
import derouter.proxy.guardrails.guardrail_hooks.noma.noma as noma_legacy_module
...
class TestNomaGuardrailConfiguration:
    ...
    def test_initialize_guardrail_function(self):
        ...
        with patch("derouter.logging_callback_manager.add_derouter_callback") as mock_add:
            result = initialize_guardrail(derouter_params, guardrail)
...
class TestNomaApplicationIdResolution:
    ...
    @pytest.mark.asyncio
    async def test_application_id_defaults_to_derouter(
        self, noma_guardrail, mock_user_api_key_dict, mock_request_data
    ):
        request_data = self._clone_request_data(mock_request_data)
        user_auth = self._clone_user_auth(mock_user_api_key_dict)
        ...
        assert application_id == "derouter"
...
class TestBackgroundProcessing:
    ...
    async def test_handle_verdict_background_allowed(self, monitor_mode_guardrail):
        ...
        with patch("derouter._logging.verbose_proxy_logger.info") as mock_info:
            await monitor_mode_guardrail._handle_verdict_background(
...
class TestNomaImageProcessing:
    ...
    async def test_image_content_blocked(self, noma_guardrail, mock_user_api_key_dict):
        ...
        from derouter.proxy.guardrails.guardrail_hooks.noma.noma import (
            NomaBlockedMessage,
        )
...
class TestIntegration:
    ...
    async def test_full_guardrail_flow(self):
        ...
        with patch.dict(
            ...
            custom_loggers = (
                derouter.logging_callback_manager.get_custom_loggers_for_type(
                    callback_type=derouter.integrations.custom_guardrail.CustomGuardrail
                )
            )
