...
from derouter.proxy.guardrails.guardrail_hooks.singulr.singulr import SingulrGuardrail
...
class TestSingulrBuildPayloadRequestData:
    ...
    def test_derouter_metadata_is_forwarded(self, singulr_guardrail):
        request_data = {"model": "gpt-4o", "derouter_metadata": {"user_api_key_hash": "abc123"}}
        payload = singulr_guardrail._build_payload(request_data, {"texts": []}, "request")
        assert payload["request_data"]["derouter_metadata"] == {"user_api_key_hash": "abc123"}
    ...
    def test_internal_logging_object_is_not_forwarded(self, singulr_guardrail):
        ...
        request_data = {
            ...
            "derouter_logging_obj": _NotSerializable(),
        }
        payload = singulr_guardrail._build_payload(request_data, {"texts": ["hi"]}, "request")

        # Must not raise.
        _json.dumps(payload)
        assert "derouter_logging_obj" not in payload["request_data"]
...
class TestSingulrRequestWiring:
    ...
    async def test_sends_configured_timeout(self):
        """derouter_params.timeout must reach the httpx call so operators can
...
class TestSingulrInitializer:
    def test_guardrail_initializer_registry_has_entry(self):
        from derouter.proxy.guardrails.guardrail_hooks.singulr import (
    ...
    def test_initialize_guardrail_reads_singulr_prefixed_fields(self):
        ...
        cb = initialize_guardrail(derouter_params, guardrail)
