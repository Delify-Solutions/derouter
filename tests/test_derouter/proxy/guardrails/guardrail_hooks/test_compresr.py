...
- recovery: hash marker appended, compresr_retrieve tool injected, originals
  stored per derouter_call_id, agentic loop returns the original content and
...
from derouter.proxy.guardrails.guardrail_hooks.compresr.compresr import (
...
def _logging_obj(call_id: str) -> SimpleNamespace:
    ...
    return SimpleNamespace(
        ...
        model_call_details={
            "derouter_params": {"metadata": {"user_api_key_auth": UserAPIKeyAuth(api_key="hash-default")}}
        },
    )
...
def _logging_obj_with_key(call_id: str, user_api_key: str, meta_key: str = "metadata") -> SimpleNamespace:
    ...
    return SimpleNamespace(
        derouter_call_id=call_id,
        model_call_details={"derouter_params": {meta_key: {"user_api_key_auth": UserAPIKeyAuth(api_key=user_api_key)}}},
    )
...
async def test_apply_guardrail_ignores_user_supplied_call_id(guardrail: CompresrGuardrail):
    ...
    with patch.object(guardrail.async_handler, "post", mock_post):
        await guardrail.apply_guardrail(
            inputs=_apply_inputs(AGENT_MESSAGES),
            request_data={"model": "gpt-4o", "derouter_call_id": attacker_call_id},
            input_type="request",
...
async def test_agentic_plan_ignores_user_supplied_call_id(guardrail: CompresrGuardrail):
    ...
    plan = await guardrail.async_build_agentic_loop_plan(
        ...
        stream=False,
        kwargs={"derouter_call_id": "victim-tenant-call-id"},
    )
...
async def test_recovery_store_partitioned_by_caller_identity(guardrail: CompresrGuardrail):
    """Two tenants that set the SAME client-forgeable x-derouter-call-id must not
...
@pytest.mark.asyncio
async def test_caller_scope_read_from_derouter_metadata(guardrail: CompresrGuardrail):
    """/v1/messages and /v1/responses carry the auth object under
    derouter_metadata rather than metadata; the store key must be scoped by it
    there too, without relying on upstream's metadata backfill."""
    logging_obj = _logging_obj_with_key("call-lm", "hash-tenant-lm", meta_key="derouter_metadata")
    mock_post = AsyncMock(return_value=_make_single_compress_response())
...
async def test_caller_scope_rejects_forged_user_api_key_string(guardrail: CompresrGuardrail):
    ...
    logging_obj = SimpleNamespace(
        derouter_call_id="call-forge",
        model_call_details={"derouter_params": {"metadata": {"user_api_key": "victim-tenant-hash"}}},
    )
    mock_post = AsyncMock(return_value=_make_single_compress_response())
...
def test_extract_compresr_tool_calls_tolerates_missing_keys():
    ...
    with patch(
        "derouter.proxy.guardrails.guardrail_hooks.compresr.compresr.get_tool_calls_from_response",
...
async def test_agentic_plan_strips_other_guardrails_executed_markers(guardrail: CompresrGuardrail):
    ...
    kwargs = {
        ...
        "derouter_metadata": {PRE_CALL_EXECUTED_GUARDRAILS_KEY: ["token:other_guardrail"]},
    }
    ...
    assert PRE_CALL_EXECUTED_GUARDRAILS_KEY not in out["derouter_metadata"]
    assert kwargs["metadata"][PRE_CALL_EXECUTED_GUARDRAILS_KEY] == [own_marker, "token:pii_guardrail"]
    assert kwargs["derouter_metadata"][PRE_CALL_EXECUTED_GUARDRAILS_KEY] == ["token:other_guardrail"]
...
def test_display_hash_strips_control_characters():
    ...
    from derouter.proxy.guardrails.guardrail_hooks.compresr.compresr import _display_hash
...
def test_originals_store_caps_total_bytes_across_calls(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        "derouter.proxy.guardrails.guardrail_hooks.compresr.compresr._MAX_TOTAL_STORE_BYTES",
        10_000,
    )
...
async def test_recovery_disabled_without_call_id():
    # enable_retrieval defaults True, but with no framework derouter_call_id we
...
async def test_recovery_disabled_when_no_caller_scope():
    ...
    logging_obj.derouter_call_id = "call-abc"
    logging_obj.model_call_details = {"derouter_params": {"metadata": {}}}
    mock_post = AsyncMock(return_value=_make_single_compress_response())
    with patch.object(guardrail.async_handler, "post", mock_post):
        result = await guardrail.apply_guardrail(
            inputs=_apply_inputs(AGENT_MESSAGES),
...
async def test_warns_once_per_interval_when_recovery_skipped_without_scope():
    ...
    with patch.object(guardrail.async_handler, "post", mock_post):
        with patch("derouter.proxy.guardrails.guardrail_hooks.compresr.compresr.verbose_proxy_logger") as mock_log:
            for _ in range(3):
                await guardrail.apply_guardrail(
                    inputs=_apply_inputs(AGENT_MESSAGES),
                    request_data={"model": "gpt-4o"},
