...
from derouter.types.integrations.custom_logger import HEADROOM_CONVERTED_STREAM_KEY
...
def _recorded_guardrail_entries(request_data: dict) -> list:
    for container_key in ("metadata", "derouter_metadata"):
...
async def test_async_build_agentic_loop_plan_calls_retrieve_and_builds_messages(
    ...
    with patch.object(
        ...
        plan = await guardrail.async_build_agentic_loop_plan(
            ...
            logging_obj=None,
            stream=False,
            kwargs={"derouter_call_id": "call-1"},
        )
...
async def test_async_build_agentic_loop_plan_rejects_hash_with_no_known_call(
    ...
    with patch.object(
        ...
        plan = await guardrail.async_build_agentic_loop_plan(
            ...
            logging_obj=None,
            stream=False,
            kwargs={"derouter_call_id": "call-unknown"},
        )
...
async def test_async_build_agentic_loop_plan_rejects_hash_issued_for_different_call(
    ...
    back in its own messages -- retrieval must be scoped per derouter_call_id,
    ...
    with patch.object(
        ...
        plan = await guardrail.async_build_agentic_loop_plan(
            ...
            logging_obj=None,
            stream=False,
            kwargs={"derouter_call_id": "call-B"},
        )
...
async def test_ccr_retrieval_disabled_ignores_service_declared_hashes(monkeypatch: pytest.MonkeyPatch):
    ...
    from derouter.proxy.guardrails.guardrail_hooks.headroom import initialize_guardrail
    from derouter.types.guardrails import LitellmParams

    monkeypatch.setattr(derouter.logging_callback_manager, "add_derouter_callback", lambda callback: None)
    params = LitellmParams(guardrail="headroom", mode="pre_call", api_base=FAKE_API_BASE, ccr_retrieval=False)
    guardrail = initialize_guardrail(params, {"guardrail_name": "headroom", "derouter_params": params})
...
async def test_responses_request_sends_compressed_input_and_retrieve_tool_upstream(
    ...
    from derouter.llms.openai.responses.guardrail_translation.handler import OpenAIResponsesHandler
...
async def test_apply_guardrail_http_status_error_raises():
    """Regression test: derouter's async httpx client calls raise_for_status()
...
@pytest.mark.asyncio
async def test_apply_guardrail_derouter_timeout_raises_when_fail_closed():
    guardrail = _make_guardrail()
...
@pytest.mark.asyncio
async def test_apply_guardrail_derouter_timeout_fail_open_forwards_uncompressed():
    guardrail = _make_guardrail(unreachable_fallback="fail_open")
...
def test_raw_retrieve_call_ids_covers_both_shapes_and_ignores_others():
    ...
    yield nothing."""
    from derouter.proxy.guardrails.guardrail_hooks.headroom.headroom import _raw_retrieve_call_ids
...
async def _plan_for(guardrail: HeadroomGuardrail, response, messages: list):
    ...
    logging_obj.derouter_call_id = "call-1"
...
async def test_configured_timeout_overrides_the_default():
    """Headroom accepted derouter_params.timeout and ignored it."""
