...
from derouter.proxy.guardrails.guardrail_hooks.noma import NomaV2Guardrail
from derouter.proxy.guardrails.guardrail_hooks.noma.noma import NomaBlockedMessage
from derouter.types.proxy.guardrails.guardrail_hooks.noma import (
...
class TestNomaV2Configuration:
    ...
    def test_build_scan_payload_sends_raw_available_data(self, noma_v2_guardrail):
        ...
        request_data = {
            ...
            "derouter_metadata": {"user_api_key_alias": "derouter-alias"},
            "derouter_call_id": "call-id-1",
        }
    ...
    def test_build_scan_payload_drops_conversation_duplicated_in_request_data(
        ...
        assert payload["request_data"]["derouter_call_id"] == "call-id-1"
    ...
    def test_build_scan_payload_passes_model_call_details_without_conversation(
        ...
        class _LoggingObj:
            def __init__(self) -> None:
                self.model_call_details = {
                    ...
                    "derouter_call_id": "call-id-123",
                    "function_id": "fn-id-456",
                    "derouter_trace_id": "trace-id-789",
                    "api_key": "included-as-is",
                }

        request_data = {"derouter_logging_obj": "<Logging object>"}
        payload = noma_v2_guardrail._build_scan_payload(
            inputs={"texts": ["hello"]},
            request_data=request_data,
            input_type="request",
        ...
        assert payload["request_data"]["derouter_logging_obj"] == {
            "complete_streaming_response": {"status": "completed"},
            "model": "gpt-4.1-mini",
            "stream": False,
            "call_type": "acompletion",
        ...
        assert request_data["derouter_logging_obj"] == "<Logging object>"
    ...
    def test_build_scan_payload_forwards_non_dict_model_call_details_unchanged(
        ...
        payload = noma_v2_guardrail._build_scan_payload(
            inputs={"texts": ["hello"]},
            request_data={"derouter_call_id": "call-id-1"},
            input_type="request",
            logging_obj=_LoggingObjWithoutDetails(),
            application_id="test-app",
        )

        assert payload["request_data"]["derouter_logging_obj"] is None
    ...
    def test_sanitize_payload_for_transport_falls_back_to_safe_dumps(
        ...
        with patch(
            "derouter.proxy.guardrails.guardrail_hooks.noma.noma_v2.json.dumps",
            side_effect=TypeError("cannot serialize"),
        ):
            with patch(
                "derouter.proxy.guardrails.guardrail_hooks.noma.noma_v2.safe_dumps",
    ...
    def test_sanitize_payload_for_transport_logs_warning_when_payload_becomes_empty(
        ...
        with patch(
            "derouter.proxy.guardrails.guardrail_hooks.noma.noma_v2.safe_json_loads",
            return_value={},
        ):
            with patch(
                "derouter.proxy.guardrails.guardrail_hooks.noma.noma_v2.verbose_proxy_logger.warning"
            ) as mock_warning:
                sanitized = noma_v2_guardrail._sanitize_payload_for_transport(
                    {"inputs": {"texts": ["hello"]}}
                )
...
class TestNomaV2ApplicationIdResolution:
    ...
    @pytest.mark.asyncio
    async def test_apply_guardrail_falls_back_to_key_alias_from_derouter_metadata(
        self, noma_v2_guardrail
    ):
        ...
        request_data = {
            "metadata": {},
            "derouter_metadata": {"user_api_key_alias": "test-key-alias"},
        }
        with patch.object(
            noma_v2_guardrail,
            "get_guardrail_dynamic_request_body_params",
