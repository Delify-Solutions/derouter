...
from derouter.proxy.guardrails.guardrail_hooks.alice.alice import (
    GUARDRAIL_NAME,
    AliceGuardrail,
...
def _verdict(payload: dict[str, object], status_code: int = 200) -> Response:
    return Response(
        ...
        json=payload,
        request=Request("POST", "https://api.alice.io/v2/evaluate/derouter"),
    )
...
def test_alice_guardrail_config(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(derouter, "guardrail_name_config_map", {})
        ...
        all_guardrails=[
            {
                ...
                "derouter_params": {"guardrail": "alice", "mode": "pre_call", "default_on": True},
            }
        ],
        config_file_path="",
    )

    registered = [cb for cb in derouter.callbacks if isinstance(cb, AliceGuardrail)]
...
class TestAliceGuardrailInitialization:
    ...
    def test_reads_credentials_from_environment(self, monkeypatch: pytest.MonkeyPatch):
        ...
        assert guardrail.api_base == "https://env.alice.test/v2/evaluate/derouter"

    def test_defaults_the_api_base(self):
        assert _guardrail().api_base == "https://api.alice.io/v2/evaluate/derouter"

    def test_trailing_slash_does_not_double_up(self):
        assert _guardrail(api_base="https://api.alice.io/").api_base == ("https://api.alice.io/v2/evaluate/derouter")
...
class TestAliceCredentialStripping:
    ...
    async def test_nested_credentials_are_stripped_at_every_depth(self):
        ...
        request_data = {
            ...
            "derouter_metadata": {"headers": {"authorization": "Bearer derouter-metadata-secret"}},
        }
        ...
        assert "derouter-metadata-secret" not in serialized
        ...
        assert "headers" not in sent_request_data["derouter_metadata"]
        assert "secret_fields" not in sent_request_data
...
class TestAliceTransportFailures:
    ...
    async def test_a_timeout_is_unreachable(self):
        guardrail = _guardrail()
        guardrail.async_handler.post = AsyncMock(
            side_effect=derouter.exceptions.Timeout(message="slow", model="gpt-4o", llm_provider="openai")
        )
