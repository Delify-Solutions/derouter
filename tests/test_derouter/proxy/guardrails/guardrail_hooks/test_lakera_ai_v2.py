...
PR checklist requires at least one test in tests/test_derouter/.
...
from derouter.proxy.guardrails.guardrail_hooks.lakera_ai_v2 import (
    LakeraAIGuardrail,
    _build_lakera_inspection_messages,
    humanize_lakera_block_reasons,
)
from derouter.types.guardrails import LitellmParams, Mode
...
class TestBuildLakeraInspectionMessages:
    """Bugbot/veria-ai findings on Delify-Solutions/derouter#34940: the Responses-API
    instructions field must be inspected (derouter later converts it into the
...
class TestFilterSkippedMessages:
    ...
    def test_keeps_system_when_flag_false_and_no_global_default(self, monkeypatch):
        monkeypatch.setattr(derouter, "skip_system_message_in_guardrail", False)
    ...
    def test_global_default_used_when_per_instance_flag_is_none(self, monkeypatch):
        monkeypatch.setattr(derouter, "skip_system_message_in_guardrail", True)
...
class TestSharedFilterMessagesBySkipFlagsUtil:
    def test_importable_directly_from_shared_utils_module(self):
        from derouter.llms.base_llm.guardrail_translation import utils as guardrail_utils
    ...
    def test_lakera_delegates_to_shared_function(self):
        ...
        with patch(  # test-quality-ok: asserts delegation to the specific shared collaborator, not an HTTP boundary
            "derouter.proxy.guardrails.guardrail_hooks.lakera_ai_v2.filter_messages_by_skip_flags",
...
class TestAsyncModerationHookWiring:
    ...
    async def test_includes_responses_instructions_in_lakera_request(self):
        """
        Veria-ai finding on Delify-Solutions/derouter#34940: async_moderation_hook (the
...
class TestPiiMaskingSafetyGuard:
    ...
    async def test_pii_only_violation_on_tool_message_masks_while_preserving_tool_call_id(self):
        """
        Regression (maintainer finding on Delify-Solutions/derouter#34940): mask-in-place must
    ...
    async def test_pii_only_violation_preserves_tool_calls_none_and_name_and_cache_control(self):
        """
        Regression (maintainer finding on Delify-Solutions/derouter#34940): a message carrying
    ...
    async def test_pii_only_violation_with_combined_messages_and_input_blocks_instead_of_masking(self):
        ...
        with (
            ...
            patch(  # test-quality-ok: asserts the wholesale write-back path is never reached for this unsafe case
                "derouter.proxy.guardrails.guardrail_hooks.lakera_ai_v2.apply_redacted_messages_back"
    ...
    async def test_pii_only_violation_with_responses_instructions_blocks_instead_of_masking(self):
        """
        Veria-ai finding on Delify-Solutions/derouter#34940: the Responses-API
    ...
    async def test_pii_only_violation_with_responses_instructions_and_skip_system_message_masks_instead_of_blocking(
        ...
        """
        Bugbot finding on Delify-Solutions/derouter#34940: _has_responses_instructions
    ...
    async def test_pii_only_violation_with_skipped_system_message_masks_and_leaves_system_message_untouched(self):
        """
        Regression (maintainer finding on Delify-Solutions/derouter#34940): setting
    ...
    async def _monitor_unmasked(self, guardrail, data, lakera_response, caplog, call_type="completion"):
        ...
        with (
            patch.object(guardrail, "call_v2_guard", new_callable=AsyncMock) as mock_call,
            caplog.at_level(logging.ERROR, logger="DeRouter Proxy"),
        ):
            mock_call.return_value = (lakera_response, {})
            result = await guardrail.async_pre_call_hook(
                user_api_key_dict=UserAPIKeyAuth(api_key="test_key"),
    ...
    async def test_pii_only_violation_with_uppercase_skipped_role_masks_without_raising(self):
        """
        Greptile finding on Delify-Solutions/derouter#34940: filter_messages_by_skip_flags
    ...
    async def test_moderation_hook_pii_only_violation_blocks_since_masking_cannot_reach_dispatch(self):
        """
        Greptile finding (P1, security) on Delify-Solutions/derouter#34940: during_call runs
...
class TestAdvisorySystemMessageValidation:
    ...
    on Delify-Solutions/derouter#34940: this check previously ran unconditionally, so a
    ...
    def test_malformed_template_with_block_mode_constructs_without_error(self):
        """Maintainer finding on Delify-Solutions/derouter#34940: on_flagged='block' never
    ...
    def test_in_memory_update_to_block_mode_with_malformed_template_is_allowed(self):
        ...
        guardrail.update_in_memory_derouter_params(derouter_params=updated_params)
        assert guardrail.on_flagged == "block"


class TestAdvisoryModeDuringCallDegradesGracefully:
    """Maintainer finding on Delify-Solutions/derouter#34940: rejecting on_flagged=
    ...
    def test_in_memory_update_moving_off_during_call_in_the_same_update_is_allowed(self):
        """Bugbot finding on Delify-Solutions/derouter#34940: validation checked the live,
    ...
    def test_in_memory_update_actually_moves_dispatch_off_during_call(self):
        """
        Veria-ai finding on Delify-Solutions/derouter#34940: LitellmParams has no field
...
class TestAdvisoryModeRequiresPayloadAndBreakdown:
    """Veria-ai finding on Delify-Solutions/derouter#34940: the mixed-violation masking
...
class TestAdvisoryModeWiring:
    ...
    async def test_pre_call_pii_only_flag_masks_instead_of_appending_advisory(self):
        """
        Regression (maintainer finding on Delify-Solutions/derouter#34940): advisory mode must
    ...
    async def test_pre_call_mixed_violation_masks_pii_before_appending_advisory(self):
        """
        Bugbot finding on Delify-Solutions/derouter#34940: a mixed violation (PII plus a
    ...
    async def test_pre_call_blocks_instead_of_advisory_when_pii_is_not_maskable(self):
        """
        Bugbot finding on Delify-Solutions/derouter#34940: a PII-only or mixed violation on
    ...
    async def test_pre_call_delivers_advisory_for_non_pii_violation_on_non_maskable_input(self):
        """
        Bugbot finding on Delify-Solutions/derouter#34940: blocking on non-maskable input
    ...
    async def test_moderation_hook_pure_prompt_injection_does_not_reassign_messages(self):
        """
        Bugbot finding on Delify-Solutions/derouter#34940: unlike async_pre_call_hook (gated
    ...
    async def test_moderation_hook_pii_only_flag_blocks_since_masking_cannot_reach_dispatch(self):
        """
        Greptile finding (P1, security) on Delify-Solutions/derouter#34940: during_call's
    ...
    async def test_moderation_hook_blocks_instead_of_advisory_when_pii_is_not_maskable(self):
        """
        Greptile finding (P1, security) on Delify-Solutions/derouter#34940: a PII violation
