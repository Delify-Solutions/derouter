...
from derouter.proxy.guardrails.guardrail_hooks.grayswan import grayswan as grayswan_module
from derouter.proxy.guardrails.guardrail_hooks.grayswan.grayswan import (
    GraySwanGuardrail,
...
def test_prepare_payload_forwards_only_scan_id_header(
    ...
    request_data = {
        ...
        "derouter_metadata": {"request_id": "request-123"},
    }

    payload = grayswan_guardrail._prepare_payload(messages, {}, request_data)

    assert payload["derouter_metadata"] == {
...
def test_prepare_payload_sanitizes_headers_when_derouter_metadata_absent(
    ...
    assert "derouter_metadata" not in payload
...
def test_prepare_payload_includes_derouter_metadata(
    grayswan_guardrail: GraySwanGuardrail,
) -> None:
    """Verify _prepare_payload forwards derouter_metadata from request_data."""
    messages = [{"role": "user", "content": "hello"}]
    request_data = {
        "derouter_metadata": {
    ...
    assert "derouter_metadata" in payload
    assert payload["derouter_metadata"]["user_api_key_user_id"] == "user-123"
    assert payload["derouter_metadata"]["user_api_key_team_id"] == "team-456"


def test_ensure_derouter_metadata_populates_from_user_api_key_dict() -> None:
    """Verify _ensure_derouter_metadata populates derouter_metadata."""
    from derouter.proxy.guardrails.guardrail_hooks.unified_guardrail.unified_guardrail import (
        _ensure_derouter_metadata,
    )

    user_auth = UserAPIKeyAuth(user_id="u1", team_id="t1", api_key="sk-test-hashed")
    data: dict = {}

    _ensure_derouter_metadata(data, user_auth)

    assert "derouter_metadata" in data
    assert data["derouter_metadata"]["user_api_key_user_id"] == "u1"
    assert data["derouter_metadata"]["user_api_key_team_id"] == "t1"


def test_ensure_derouter_metadata_noop_when_already_present() -> None:
    """Verify _ensure_derouter_metadata does not overwrite existing derouter_metadata."""
    from derouter.proxy.guardrails.guardrail_hooks.unified_guardrail.unified_guardrail import (
        _ensure_derouter_metadata,
    )

    user_auth = UserAPIKeyAuth(user_id="should-not-appear")
    data: dict = {"derouter_metadata": {"existing": "value"}}

    _ensure_derouter_metadata(data, user_auth)

    assert data["derouter_metadata"] == {"existing": "value"}
