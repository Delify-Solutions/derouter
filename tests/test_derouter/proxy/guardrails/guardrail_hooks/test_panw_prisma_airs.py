...
This test file follows DeRouter's testing patterns and covers:
...
from derouter.types.guardrails import GuardrailEventHooks, LitellmParams
...
def mock_panw_client():
    ...
    with patch(
        "derouter.proxy.guardrails.guardrail_hooks.panw_prisma_airs.panw_prisma_airs.get_async_httpx_client"
    ) as mock_client:
...
class TestPanwAirsInitialization:
    ...
    def test_initialize_guardrail_function(self):
        ...
        with patch("derouter.logging_callback_manager.add_derouter_callback"):
            handler = initialize_guardrail(derouter_params, guardrail_config)
...
class TestPanwAirsPromptScanning:
    ...
    async def test_empty_prompt_handling(self, base_handler, user_api_key_dict):
        ...
        empty_data = {
            ...
            "messages": [],
            "user": "test_user",
            "derouter_call_id": "test-call-id-empty",
        }
...
class TestPanwAirsMaskingFunctionality:
    ...
    async def test_response_masking_on_block(self):
        ...
        user_api_key_dict = UserAPIKeyAuth()
        data = {"model": "gpt-3.5-turbo", "derouter_call_id": "test-call-id"}
        response = ModelResponse(
            id="test_id",
            choices=[
                Choices(
...
class TestPanwAirsAdvancedFeatures:
    ...
    async def test_streaming_hook_adds_guardrail_header(self):
        ...
        with patch.object(
            ...
            with patch(
                "derouter.proxy.guardrails.guardrail_hooks.panw_prisma_airs.panw_prisma_airs.add_guardrail_to_applied_guardrails_header"
            ) as mock_header:
                mock_api.return_value = mock_scan_result
...
class TestPanwAirsDeduplication:
    ...
    async def test_duplicate_pre_call_scan_prevented(self):
        ...
        data = {
            "model": "gpt-3.5-turbo",
            "messages": [{"role": "user", "content": "Test"}],
            "derouter_call_id": "test-call-123",
        }
    ...
    async def test_duplicate_post_call_scan_prevented(self):
        ...
        data = {
            "model": "gpt-3.5-turbo",
            "derouter_call_id": "test-call-456",
        }
        response = ModelResponse(
            id="test_id",
            choices=[
    ...
    async def test_duplicate_streaming_scan_prevented(self):
        ...
        request_data = {
            "model": "gpt-3.5-turbo",
            "derouter_call_id": "test-call-789",
            "messages": [{"role": "user", "content": "test"}],
        }
...
class TestPanwAirsSessionTracking:
    """Test session tracking with derouter_trace_id."""

    @pytest.mark.asyncio
    async def test_tr_id_always_call_id_with_trace_in_metadata(self, mock_panw_client):
        """Test that tr_id is always call_id even when metadata has derouter_trace_id."""
        handler = make_handler()
        ...
        assert payload["metadata"]["derouter_trace_id"] == trace_id

    @pytest.mark.asyncio
    async def test_fallback_to_call_id_when_trace_id_missing(self, mock_panw_client):
        """Test fallback to call_id when derouter_trace_id is missing."""
        handler = make_handler()
    ...
    async def test_trace_id_extraction_from_request_data(self):
        """Test that derouter_trace_id is extracted from request data."""
        handler = make_handler()
        ...
        assert "derouter_trace_id" in metadata
        assert metadata["derouter_trace_id"] == trace_id

    def test_trace_id_extraction_from_nested_metadata(self):
        """Test derouter_trace_id extraction from data['metadata'] (proxy path).

        The proxy stores user-supplied derouter_trace_id inside
        data["metadata"]["derouter_trace_id"], NOT at data["derouter_trace_id"].
        ...
        data = {
            ...
            "metadata": {
                "derouter_trace_id": trace_id,
                "requester_metadata": {"derouter_trace_id": trace_id},
            },
        }
    ...
    def test_trace_id_extraction_from_requester_metadata(self):
        """Test derouter_trace_id extraction from requester_metadata fallback.

        For /v1/messages routes, user metadata is deep-copied into
        requester_metadata. If derouter_trace_id is only there, we must find it.
        """
        handler = make_handler()
    ...
    def test_trace_id_extraction_from_header_key(self):
        """Test derouter_trace_id extraction from x-derouter-trace-id header.

        derouter_pre_call_utils stores the x-derouter-trace-id header value
        as metadata["trace_id"] (not "derouter_trace_id"). We must find it.
        """
        handler = make_handler()
        ...
        data = {
            "model": "gpt-3.5-turbo",
            "metadata": {
                "trace_id": trace_id,  # as stored by derouter_pre_call_utils
            },
        }
...
class TestPanwAirsAppUserMetadata:
    ...
    async def test_app_user_priority_chain(self):
        """Test that app_user follows priority: app_user > user > derouter_user."""
        handler = make_handler()
        ...
        test_cases = [
            ...
            ({}, "derouter_user", "derouter_user is default"),
        ]
...
class TestPanwAirsDeduplicationMissingCallId:
    """Test _check_and_mark_scanned fallback behavior when derouter_call_id is missing."""

    def test_check_and_mark_scanned_synthesizes_call_id_when_missing(self):
        """Test that _check_and_mark_scanned synthesizes derouter_call_id when missing."""
        handler = make_handler()
        ...
        assert data["derouter_call_id"]
        assert (
            data["derouter_metadata"][f"_panw_pre_scanned_{data['derouter_call_id']}"]
            is True
        )
...
class TestPanwAirsApplyGuardrail:
    ...
    async def test_apply_guardrail_allow(self, handler):
        ...
        request_data = {"derouter_call_id": "test-call-id", "model": "gpt-4"}
    ...
    async def test_apply_guardrail_warns_when_tool_results_scope_leaves_nothing_scannable(self, handler):
        ...
        with (
            patch.object(handler, "_call_panw_api", new_callable=AsyncMock) as mock_api,
            patch(
                "derouter.proxy.guardrails.guardrail_hooks.panw_prisma_airs.panw_prisma_airs.verbose_proxy_logger.warning"
            ) as mock_warning,
        ):
            result = await handler.apply_guardrail(
                inputs=inputs,
    ...
    async def test_apply_guardrail_missing_call_id_synthesizes_fallback(self, handler):
        """Missing derouter_call_id is synthesized (not a hard fail)."""
        inputs: GenericGuardrailAPIInputs = {"texts": ["Test content"]}
        request_data = {"model": "gpt-4"}  # No derouter_call_id
        ...
        with patch.object(
            ...
            assert "derouter_call_id" in request_data
            assert len(request_data["derouter_call_id"]) == 36  # UUID4 format
            assert mock_api.call_count == 1
    ...
    async def test_apply_guardrail_synthesizes_call_id_for_direct_endpoint(
        ...
        with patch.object(
            ...
            assert mock_api.call_count == 1
            assert (
                mock_api.call_args.kwargs["call_id"] == request_data["derouter_call_id"]
            )

    @pytest.mark.asyncio
    async def test_apply_guardrail_call_id_from_logging_obj(self, handler):
        """Test derouter_call_id resolved from logging_obj when missing from request_data."""
        inputs: GenericGuardrailAPIInputs = {"texts": ["Hello world"]}
        request_data = {"model": "gpt-4"}  # No derouter_call_id

        logging_obj = MagicMock()
        logging_obj.derouter_call_id = "logging-call-id"
    ...
    async def test_apply_guardrail_response_side_missing_call_id(self, handler):
        """Response-side with no derouter_call_id synthesizes a UUID fallback."""
        response = ModelResponse(
            id="chatcmpl-test",
        ...
        request_data: dict = {"response": response}  # No derouter_call_id
        ...
        with patch.object(
            ...
            assert "derouter_call_id" in request_data
            assert len(request_data["derouter_call_id"]) == 36
    ...
    async def test_scan_tool_calls_post_call_scans_args_as_response_text(self):
        ...
        with patch.object(
            ...
            await handler._scan_tool_calls_for_guardrail(
                ...
                metadata={"derouter_call_id": "test"},
                call_id="test-call-id",
                request_data={},
...
class TestPanwAirsStreamingBytesScan:
    ...
    async def test_streaming_bytes_scan(self, action):
        ...
        request_data = {
            ...
            "derouter_call_id": "test-bytes-call-id",
        }
    ...
    async def test_bytes_streaming_success_adds_observability_header(self):
        ...
        request_data = {
            "messages": [{"role": "user", "content": "test"}],
            "model": "claude-3-5-sonnet",
            "derouter_call_id": "test-obs-bytes-id",
        }
...
class TestPanwAirsStreamingPydanticEventsScan:
    ...
    async def test_streaming_pydantic_events_scan(self, action):
        ...
        request_data = {
            "messages": [{"role": "user", "content": "test"}],
            "model": "gpt-4",
            "derouter_call_id": "test-pydantic-call-id",
        }
    ...
    async def test_pydantic_streaming_success_adds_observability_header(self):
        ...
        request_data = {
            "messages": [{"role": "user", "content": "test"}],
            "model": "gpt-4",
            "derouter_call_id": "test-obs-pydantic-id",
        }
...
class TestPanwAirsApplyGuardrailMetadataEnrichment:
    ...
    async def test_apply_guardrail_metadata_enrichment(self):
        ...
        request_data = {"response": mock_response, "derouter_call_id": "test-enrich-id"}

        # logging_obj carries the original metadata
        logging_obj = MagicMock()
        logging_obj.derouter_call_id = "test-enrich-id"
        logging_obj.model = "gpt-4"
    ...
    async def test_tool_call_args_sent_as_prompt_content(self, handler):
        ...
        with patch.object(
            ...
            await handler._scan_tool_calls_for_guardrail(
                ...
                metadata={"user": "test", "model": "gpt-4"},
                call_id="test-call-id",
                request_data={"derouter_call_id": "test-call-id"},
                start_time=datetime.now(),
            )
...
class TestPanwAirsChatStreamingPostCall:
    ...
    async def test_model_response_stream(self, action):
        ...
        request_data = {
            "messages": [{"role": "user", "content": "test"}],
            "model": "gpt-4",
            "derouter_call_id": "test-stream-chat",
        }
...
class TestPanwAirsTrIdOverride:
    """Test tr_id override from explicit derouter_trace_id in metadata."""
    ...
    async def test_tr_id_header_only_no_override(self, mock_panw_client):
        ...
        # Simulate header-derived trace_id (stored as "trace_id" by derouter_pre_call_utils)
        data = {
            "model": "gpt-3.5-turbo",
            "metadata": {
        ...
        assert metadata["derouter_trace_id"] == header_trace
        ...
        payload = mock_panw_client.client.post.call_args.kwargs["json"]
        assert payload["tr_id"] == call_id
        assert payload["metadata"]["derouter_trace_id"] == header_trace

    @pytest.mark.asyncio
    async def test_tr_id_uses_call_id_with_requester_metadata_trace(
        self, mock_panw_client
    ):
        """requester_metadata.derouter_trace_id is correlation-only, tr_id is always call_id."""
        handler = PanwPrismaAirsHandler(
            guardrail_name="test_panw_airs",
            api_key="test_api_key",
            profile_name="test_profile",
        ...
        assert "_panw_tr_id_override" not in metadata
        # derouter_trace_id still extracted for correlation
        assert metadata["derouter_trace_id"] == trace_id
    ...
    @pytest.mark.asyncio
    async def test_top_level_derouter_trace_id_is_correlation_only(
        self, mock_panw_client
    ):
        """Top-level data['derouter_trace_id'] is correlation-only, NOT a tr_id override."""
        handler = PanwPrismaAirsHandler(
            guardrail_name="test_panw_airs",
            api_key="test_api_key",
            profile_name="test_profile",
        ...
        # Only top-level derouter_trace_id, NO metadata.derouter_trace_id
        data = {
            "model": "gpt-3.5-turbo",
            "derouter_trace_id": top_level_trace,
            "metadata": {},
        }
        ...
        assert metadata["derouter_trace_id"] == top_level_trace
        ...
        payload = mock_panw_client.client.post.call_args.kwargs["json"]
        assert payload["tr_id"] == call_id
        # derouter_trace_id still forwarded for correlation
        assert payload["metadata"]["derouter_trace_id"] == top_level_trace
...
class TestPanwAirsDictChunkStreaming:
    ...
    async def test_streaming_hook_dict_chunks_scanned(self):
        ...
        request_data = {
            "messages": [{"role": "user", "content": "test"}],
            "model": "gpt-4",
            "derouter_call_id": "test-dict-chunk-call-id",
        }
...
class TestPanwAirsRawStreamingMaskingWarning:
    ...
    async def test_raw_streaming_block_with_masking_logs_warning(self):
        ...
        request_data = {
            "messages": [{"role": "user", "content": "test"}],
            "model": "gpt-4",
            "derouter_call_id": "test-raw-mask-call-id",
        }
        ...
        with patch.object(
            ...
            with patch(
                "derouter.proxy.guardrails.guardrail_hooks.panw_prisma_airs.panw_prisma_airs.verbose_proxy_logger"
            ) as mock_logger:
                with pytest.raises(HTTPException) as exc_info:
...
class TestPanwAirsLatestRoleMessageOnly:
    ...
    async def test_flag_unset_anthropic_defaults_latest_only(
        ...
        dumped = derouter_params.model_dump(exclude_unset=True)
        handler = PanwPrismaAirsHandler(
            **{
                **dumped,
                "guardrail_name": "test_panw_airs",
...
class TestPanwAirsMcpToolCallWithoutCallId:
    ...
    derouter_call_id — the bug fix for _convert_mcp_to_llm_format synthetic data."""
    ...
    async def test_mcp_tool_call_request_without_call_id(self, handler):
        """MCP tool call with no derouter_call_id should NOT raise 500.

        This is the core regression test: _convert_mcp_to_llm_format produces
        synthetic request_data without derouter_call_id, and logging_obj is None.
        ...
        with patch.object(
            ...
            # Assert 4: derouter_call_id backfilled into request_data
            assert request_data.get("derouter_call_id") == call_kwargs["call_id"]
    ...
    async def test_mcp_tool_call_with_logging_obj_call_id_uses_parent_id(self, handler):
        """When logging_obj has derouter_call_id, the handler should use it as tr_id
        ...
        request_data = {
            ...
            # NO derouter_call_id in request_data
        }
        mock_logging_obj = MagicMock()
        mock_logging_obj.derouter_call_id = "parent-call-id-123"
        ...
        with patch.object(
            ...
            # call_id should be the parent's derouter_call_id
            call_kwargs = mock_api.call_args.kwargs
    ...
    async def test_direct_apply_guardrail_empty_request_data_synthesizes_plain_uuid(
        ...
        with patch.object(
            ...
            synth_id = request_data.get("derouter_call_id")
    ...
    async def test_non_mcp_request_without_call_id_synthesizes_uuid(self, handler):
        ...
        request_data = {
            ...
            "derouter_call_id": None,  # explicitly missing
        }
        ...
        with patch.object(
            ...
            # UUID was synthesized and injected
            assert request_data["derouter_call_id"] is not None
            assert len(request_data["derouter_call_id"]) == 36
    ...
    async def test_mcp_rest_name_fallback_synthesizes_tr_id(self, handler):
        ...
        request_data = {
            ...
            "name": "web_search_exa",
            "arguments": {"path": "/tmp"},
            # NO mcp_tool_name, NO derouter_call_id
        }
...
class TestPanwAirsStreamingFallbackFix:
    ...
    async def test_streaming_transient_returns_tuple_without_raising(self, handler):
        ...
        request_data = _simple_data(derouter_call_id="test-call-id")
...
class TestPanwAirsTimeoutCoercion:
    ...
    def test_derouter_params_coerces_string_timeout(self):
    ...
    def test_derouter_params_rejects_garbage_timeout(self):
    ...
    def test_derouter_params_empty_string_timeout_becomes_none(self):
    ...
    def test_legacy_initializer_handles_unset_timeout(self):
        ...
        from derouter.proxy.guardrails.guardrail_initializers import (
...
class TestPanwAirsScanIdExposure:
    ...
    def _recorded_scan_ids(request_data):
        metadata = {**request_data.get("metadata", {}), **request_data.get("derouter_metadata", {})}
    ...
    async def test_pre_call_allow_records_scan_id(self, user_api_key_dict):
        ...
        data = _simple_data(derouter_call_id="test-call-id", metadata={})
    ...
    async def test_post_call_allow_records_response_scan_id(self, user_api_key_dict):
        handler = self._handler(self.ALLOW_SCAN_RESULT)
        data = {"model": "gpt-4", "derouter_call_id": "test-call-id", "metadata": {}}
    ...
    async def test_apply_guardrail_allow_records_scan_id(self):
        handler = self._handler(self.ALLOW_SCAN_RESULT)
        inputs: GenericGuardrailAPIInputs = {"texts": ["Hello world"]}
        request_data = {"derouter_call_id": "test-call-id", "model": "gpt-4", "metadata": {}}
    ...
    async def test_allowed_scan_id_becomes_response_header(self, user_api_key_dict):
        from derouter.proxy.common_utils.callback_utils import get_logging_caching_headers
        ...
        assert headers["x-derouter-guardrail-scan-id"] == "scan-abc-123"
        assert "x-derouter-guardrail-scan-metadata" not in headers
    ...
    async def test_request_and_response_scan_ids_are_both_exposed(self, user_api_key_dict):
        ...
        headers = get_logging_caching_headers(data)
        assert headers["x-derouter-guardrail-scan-id"] == "scan-abc-123,scan-response-456"
