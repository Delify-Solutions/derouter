...
from derouter.exceptions import GuardrailRaisedException, ModifyResponseException
from derouter.proxy.guardrails.guardrail_hooks.straiker import initialize_guardrail
from derouter.proxy.guardrails.guardrail_hooks.straiker.straiker import (
    StraikerGuardrail,
    _build_usage,
...
def _logging_obj() -> MagicMock:
    obj = MagicMock()
    obj.derouter_call_id = "call-123"
    obj.derouter_trace_id = "trace-456"
...
async def test_request_envelope_transport_and_shape():
    ...
    assert headers["X-Straiker-Webhook-Format"] == "derouter"
    ...
    assert payload["context"]["derouter_call_id"] == "call-123"
    assert payload["identity"]["derouter_key"] == "team-key"
...
async def test_webhook_metadata_session_id_and_opaque_passthrough():
    ...
    await g.apply_guardrail(
        ...
        request_data={
            "model": "m",
            "derouter_session_id": "sess-from-derouter",
            "metadata": {
    ...
    assert payload["application"] == {"source": "chatbot-app", "name": "Chatbot"}
    assert payload["identity"]["derouter_key"] == "team-key"
    assert payload["context"]["session_id"] == "sess-from-derouter"
...
async def test_context_mode_from_tagged_mode_is_flattened_and_deduped():
    from derouter.types.guardrails import Mode
...
async def test_identity_key_and_team_coalesce_alias_over_id():
    ...
    assert identity["derouter_key"] == "prod-key"
    assert identity["derouter_team"] == "growth"
...
async def test_identity_key_and_team_fall_back_to_hash_and_id():
    ...
    identity = _posted_payload(g)["identity"]
    assert identity["derouter_key"] == "hash-abc"
    assert identity["derouter_team"] == "team-9"
...
async def test_identity_end_user_from_resolved_metadata():
    ...
    assert identity["derouter_user_id"] == "default_user_id"
...
async def test_application_source_from_agent_id():
    g = _make_guardrail(source="derouter")
    g.async_handler.post.return_value = _mock_response("NONE")
    await g.apply_guardrail(
        inputs={"texts": ["x"]},
...
@pytest.mark.parametrize(
    "request_data",
    [
        {"input": ["ssn 123-45-6789"], "derouter_metadata": {"user_api_key_request_route": "/vllm/v1/embeddings"}},
        {"input": [[1, 2, 3]], "derouter_metadata": {}},
        {"input": "confidential memo", "derouter_metadata": {}},
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"messages": [{"role": "user", "content": "hi"}], "derouter_metadata": {}},
