...
from derouter.proxy.guardrails.guardrail_hooks.mcp_security import initialize_guardrail
...
class TestExtractMCPServerNames:
    def test_extracts_derouter_proxy_mcp_servers(self):
        tools = [
            {"type": "mcp", "server_url": "derouter_proxy/mcp/zapier"},
            {"type": "mcp", "server_url": "derouter_proxy/mcp/github"},
...
class TestMCPSecurityGuardrailPreCall:
    ...
    async def test_blocks_unregistered_server_chat_completions(
        ...
        data = {
            "tools": [
                {"type": "mcp", "server_url": "derouter_proxy/mcp/zapier"},
                {"type": "mcp", "server_url": "derouter_proxy/mcp/evil_server"},
            ],
            "model": "gpt-4",
    ...
    async def test_blocks_unregistered_server_responses_api(
        ...
        data = {
            "tools": [
                {"type": "mcp", "server_url": "derouter_proxy/mcp/unknown_server"},
            ],
            "model": "gpt-4o",
...
class TestInitializeGuardrail:
    ...
    def test_on_violation_from_derouter_params(
        self,
        configured: Literal["block", "alert", "warn", "end_session"] | None,
        expected: Literal["block", "alert"],
    ):
        derouter_params = LitellmParams(guardrail="mcp_security", mode="pre_call", on_violation=configured)
        guardrail = Guardrail(guardrail_name="mcp-security-block", derouter_params=derouter_params)

        result = initialize_guardrail(derouter_params=derouter_params, guardrail=guardrail)

        assert isinstance(result, MCPSecurityGuardrail)
        assert result.on_violation == expected
        assert result in derouter.callbacks
