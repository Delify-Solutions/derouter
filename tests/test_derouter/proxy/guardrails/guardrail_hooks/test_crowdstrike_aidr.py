...
from derouter.proxy.guardrails.guardrail_hooks.crowdstrike_aidr import initialize_guardrail
from derouter.proxy.guardrails.guardrail_hooks.crowdstrike_aidr.crowdstrike_aidr import (
...
from derouter.types.utils import Delta, GenericGuardrailAPIInputs, ModelResponse, ModelResponseStream
...
def test_initialize_guardrail_wires_fail_on_error_and_defaults_closed(configured: dict, expected: bool) -> None:
    ...
    guardrail = Guardrail(guardrail_name="crowdstrike-aidr-guard", derouter_params=derouter_params)

    handler = initialize_guardrail(derouter_params=derouter_params, guardrail=guardrail)
...
def _initialize_from_config(**derouter_params_kwargs: object) -> CrowdStrikeAIDRHandler:
    derouter_params = LitellmParams(
        guardrail="crowdstrike_aidr",
        api_key="pts_crowdstrike_tokenid",
        api_base="https://api.crowdstrike.com/aidr/aiguard",
        default_on=True,
        **derouter_params_kwargs,
    )
    guardrail = Guardrail(guardrail_name="crowdstrike-aidr-guard", derouter_params=derouter_params)
    return initialize_guardrail(derouter_params=derouter_params, guardrail=guardrail)
...
def test_update_in_memory_derouter_params_reapplies_streaming_params() -> None:
    handler = _initialize_from_config(mode="post_call")

    handler.update_in_memory_derouter_params(
        LitellmParams(
            guardrail="crowdstrike_aidr",
            mode="post_call",
...
async def _guard_calls_for_stream(handler: CrowdStrikeAIDRHandler, chunk_texts: list[str]) -> int:
    ...
    from derouter.proxy.guardrails.guardrail_hooks.unified_guardrail.unified_guardrail import UnifiedLLMGuardrails
