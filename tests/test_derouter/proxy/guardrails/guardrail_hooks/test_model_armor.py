...
from derouter.llms.custom_httpx.http_handler import MaskedHTTPStatusError
from derouter.proxy.guardrails.anthropic_sse import anthropic_sse_error_frames
from derouter.proxy._types import UserAPIKeyAuth
from derouter.proxy.guardrails.guardrail_hooks.model_armor import ModelArmorGuardrail
...
async def test_model_armor_post_call_hook_sanitization():
    ...
    with patch.object(
        ...
        mock_llm_response = derouter.ModelResponse()
...
async def test_model_armor_post_call_hook_blocked():
    ...
    with patch.object(
        ...
        mock_llm_response.choices = [
            derouter.Choices(
                message=derouter.Message(content="Here is some harmful content...")
            )
        ]
                ...
                derouter.ModelResponseStream(
                    choices=[
                        derouter.types.utils.StreamingChoices(
                            delta=derouter.types.utils.Delta(content="Sensitive ")
                        )
                    ]
                ),
                derouter.ModelResponseStream(
                    choices=[
                        derouter.types.utils.StreamingChoices(
                            delta=derouter.types.utils.Delta(content="information")
                        )
                    ]
                ),
            ]
        ...
        async def mock_stream():
            chunks = [
                derouter.ModelResponseStream(
                    choices=[
                        derouter.types.utils.StreamingChoices(
                            delta=derouter.types.utils.Delta(content="My password is ")
                        )
                    ]
                ),
                derouter.ModelResponseStream(
                    choices=[
                        derouter.types.utils.StreamingChoices(
                            delta=derouter.types.utils.Delta(content="hunter2")
                        )
                    ]
                ),
            ]
...
async def test_model_armor_api_error_fail_open_moderation_and_post_call(fail_on_error: bool):
    ...
    mock_llm_response = derouter.ModelResponse()
    mock_llm_response.choices = [
        derouter.Choices(message=derouter.Message(content="model output"))
    ]
...
async def test_model_armor_api_error_fail_open_streaming(fail_on_error: bool):
    ...
    async def mock_stream():
        yield derouter.ModelResponseStream(
            choices=[
                derouter.types.utils.StreamingChoices(
                    delta=derouter.types.utils.Delta(content="streamed output")
                )
            ]
        )
    ...
    if fail_on_error:
        ...
        assert len(chunks) == 1
        assert isinstance(chunks[0], derouter.ModelResponseStream)
...
def test_model_armor_hot_reload_null_stays_sanitized():
    """update_in_memory_derouter_params assigns raw fields; an explicit null in a
...
async def test_model_armor_post_call_logging_redacts_scanned_content(sanitize: bool):
    ...
    with patch(
        "derouter.proxy.common_utils.callback_utils.add_guardrail_response_to_standard_logging_object"
    ) as add_logging:
        await guardrail.async_post_call_success_hook(
            data=request_data,
            user_api_key_dict=UserAPIKeyAuth(),
...
async def test_model_armor_match_found_sanitizes_caller_and_logging(sanitize: bool):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await guardrail.async_pre_call_hook(
            ...
            cache=MagicMock(spec=DualCache),
            data=request_data,
            call_type=derouter.types.utils.CallTypes.call_mcp_tool.value,
        )
...
def test_model_armor_ui_friendly_name():
    ...
    from derouter.types.proxy.guardrails.guardrail_hooks.model_armor import (
...
async def test_model_armor_missing_sanitized_text():
    ...
    with patch.object(
        ...
        mock_llm_response = derouter.ModelResponse()
        mock_llm_response.choices = [
            derouter.Choices(message=derouter.Message(content="Original content"))
        ]
...
def test_initialize_guardrail_forwards_skip_unscannable_attachments():
    """skip_unscannable_attachments configured in derouter_params reaches the guardrail instance."""
    from derouter.proxy.guardrails.guardrail_hooks.model_armor import initialize_guardrail
...
async def test_pre_call_blocks_file_over_size_limit():
    ...
    from derouter.proxy.guardrails.guardrail_hooks.model_armor.file_scanning import (
...
async def test_scan_result_reaches_the_logger_on_a_seeded_route():
    """On routes that seed `derouter_metadata` the scan result must land in that bucket
    ...
    request_data = {
        ...
        "derouter_metadata": {"guardrails": ["model-armor-test"]},
    }
    ...
    assert "_model_armor_response" in request_data["derouter_metadata"]

    before = len(request_data["derouter_metadata"].get("standard_logging_guardrail_information", []))
    guardrail._process_response(response=None, request_data=request_data)

    logged = request_data["derouter_metadata"]["standard_logging_guardrail_information"]
...
def _chat_completion_chunks():
    ...
    return (
        derouter.types.utils.ModelResponseStream(
            choices=[
                derouter.types.utils.StreamingChoices(
                    index=0,
                    delta=derouter.types.utils.Delta(content="my card is 4111-1111-1111-1111"),
                )
            ]
        ),
        derouter.types.utils.ModelResponseStream(
            choices=[
                derouter.types.utils.StreamingChoices(
                    index=0,
                    delta=derouter.types.utils.Delta(content=""),
                    finish_reason="stop",
                )
            ]
        ),
...
def test_is_sse_error_stream_only_matches_a_stream_that_is_nothing_but_refusals(chunks, expected, case):
    ...
    from derouter.proxy.guardrails.anthropic_sse import is_sse_error_stream
