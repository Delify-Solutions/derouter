...
from derouter.types.utils import Choices, GenericGuardrailAPIInputs, Message
...
def test_onyx_guard_config(monkeypatch: pytest.MonkeyPatch):
    ...
    registered = [c for c in derouter.callbacks if isinstance(c, OnyxGuardrail)]
...
def test_onyx_guard_with_custom_timeout_from_kwargs(monkeypatch: pytest.MonkeyPatch):
    ...
    with patch(
        "derouter.proxy.guardrails.guardrail_hooks.onyx.onyx.get_async_httpx_client"
    ) as mock_get_client:
...
class TestOnyxGuardrail:
    ...
    async def test_apply_guardrail_request_no_violations(self, monkeypatch: pytest.MonkeyPatch):
        ...
        assert (
            call_args.args[0]
            == f"{guardrail.api_base}/guard/evaluate/v1/{guardrail.api_key}/derouter"
        )
        assert (
...
class TestOnyxIntegration:
    ...
    async def test_full_guardrail_flow(self, monkeypatch: pytest.MonkeyPatch):
        ...
        custom_loggers = derouter.logging_callback_manager.get_custom_loggers_for_type(
