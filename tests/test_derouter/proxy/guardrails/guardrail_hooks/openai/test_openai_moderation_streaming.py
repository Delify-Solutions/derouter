...
from derouter.types.utils import ModelResponseStream, ModelResponse
...
async def test_openai_moderation_streaming_end_of_stream_request_data_passthrough():
    ...
    with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
        ...
        # unified hook seeds derouter_metadata, so read the bucket the resolver names
    ...
    with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
        ...
        mock_model_response = ModelResponse(
            ...
            choices=[
                derouter.Choices(
                    index=0,
                    message=derouter.Message(role="assistant", content="ABCDEFGHIJ"),
                    finish_reason="stop",
                )
            ],
        )
    ...
    with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
        ...
        mock_model_response = ModelResponse(
            ...
            choices=[
                derouter.Choices(
                    index=0,
                    message=derouter.Message(role="assistant", content="ABCDEF"),
                    finish_reason="stop",
                )
            ],
        )
