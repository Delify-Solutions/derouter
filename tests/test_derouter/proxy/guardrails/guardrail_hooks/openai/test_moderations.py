...
from derouter.types.llms.openai import OpenAIModerationResponse, OpenAIModerationResult
...
@pytest.mark.asyncio
async def test_openai_moderation_guardrail_adds_to_derouter_callbacks():
    """Test that OpenAI moderation guardrail adds itself to derouter callbacks during initialization"""
    import derouter
    from derouter.proxy.guardrails.guardrail_hooks.openai import (
    ...
    original_callbacks = derouter.callbacks.copy()
    derouter.logging_callback_manager._reset_all_callbacks()

    try:
        with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
            guardrail_derouter_params = LitellmParams(
            ...
            guardrail = openai_initialize_guardrail(
                derouter_params=guardrail_derouter_params,
                guardrail=Guardrail(
                    guardrail_name="test-openai-moderation",
                    derouter_params=guardrail_derouter_params,
                ),
            ...
            # Check that the guardrail was added to derouter callbacks
            assert guardrail in derouter.callbacks
            assert len(derouter.callbacks) == 1

            # Verify it's the correct guardrail
            callback = derouter.callbacks[0]
...
async def test_openai_moderation_guardrail_streaming_harmful_content():
    ...
    with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
        ...
        # Mock for stream_chunk_builder - use real derouter types so isinstance checks pass
    ...
    with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
        ...
        llm_response = ModelResponse(
            ...
            choices=[
                derouter.Choices(
                    index=0,
                    message=derouter.Message(role="assistant", content="Hello world"),
                    finish_reason="stop",
                )
            ],
        )
        ...
        # seeds derouter_metadata, so read the bucket the resolver names rather than
...
async def test_openai_moderation_initialize_guardrail_forwards_streaming_flags():
    """initialize_guardrail forwards streaming knobs from derouter_params (extra='allow')."""
    import derouter
    from derouter.proxy.guardrails.guardrail_hooks.openai import (
        initialize_guardrail as openai_initialize_guardrail,
    )
