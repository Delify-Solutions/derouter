...
from derouter.proxy.guardrails.guardrail_hooks.vigil_guard import (
    VigilGuardGuardrail,
    guardrail_class_registry,
    guardrail_initializer_registry,
    initialize_guardrail,
)
from derouter.proxy.guardrails.guardrail_hooks.vigil_guard.vigil_guard import (
    _DEFAULT_VIGIL_TIMEOUT,
    VigilGuardMissingConfig,
)
from derouter.types.guardrails import LitellmParams, SupportedGuardrailIntegrations
...
def _transient_exceptions() -> List[BaseException]:
    ...
    return [
        ...
        DeRouterTimeout(message="t", model="m", llm_provider="vigil_guard"),
    ]
...
async def test_metadata_source_precedence_and_derouter_metadata_fallback():
    handler = FakeHandler([_resp({"decision": "ALLOWED"})])
    g = _make_guardrail(handler)
    request_data = {
        "user_id": "top",
        "metadata": {"user_id": "nested"},
        "derouter_metadata": {"tenant_id": "lm-tenant"},
    }
    await g.apply_guardrail(
        inputs={"texts": ["x"]}, request_data=request_data, input_type="request"
    )
...
async def test_call_id_forwarded_from_logging_obj():
    handler = FakeHandler([_resp({"decision": "ALLOWED"})])
    g = _make_guardrail(handler)
    logging_obj = SimpleNamespace(derouter_call_id="call-123")
    await g.apply_guardrail(
        inputs={"texts": ["x"]},
        request_data={},
        input_type="request",
    ...
    assert handler.calls[0].json["metadata"]["derouter_call_id"] == "call-123"
...
async def test_call_id_forwarded_from_request_data():
    ...
    await g.apply_guardrail(
        inputs={"texts": ["x"]},
        request_data={"derouter_call_id": "rd-1"},
        input_type="request",
        logging_obj=None,
    )
    assert handler.calls[0].json["metadata"]["derouter_call_id"] == "rd-1"
...
async def test_call_id_forwarded_from_request_metadata():
    ...
    await g.apply_guardrail(
        inputs={"texts": ["x"]},
        request_data={"metadata": {"derouter_call_id": "md-1"}},
        input_type="request",
        logging_obj=None,
    )
    assert handler.calls[0].json["metadata"]["derouter_call_id"] == "md-1"


async def test_call_id_logging_obj_takes_precedence():
    handler = FakeHandler([_resp({"decision": "ALLOWED"})])
    g = _make_guardrail(handler)
    logging_obj = SimpleNamespace(derouter_call_id="log-1")
    await g.apply_guardrail(
        inputs={"texts": ["x"]},
        request_data={"derouter_call_id": "rd-1"},
        input_type="request",
    ...
    assert handler.calls[0].json["metadata"]["derouter_call_id"] == "log-1"
...
def test_derouter_params_includes_config_model():
