...
from derouter.llms.base_llm.ocr.transformation import OCRPage, OCRResponse
...
from derouter.proxy._types import DeRouterRoutes, UserAPIKeyAuth
from derouter.proxy.guardrails.guardrail_hooks.unified_guardrail import (
...
from derouter.types.utils import CallTypes, Delta, ModelResponseStream, StreamingChoices
...
class _NoopTranslation(BaseTranslation):
    ...
    async def process_input_messages(self, data, guardrail_to_apply, derouter_logging_obj=None):  # type: ignore[override]
        return data
...
class TestUnifiedLLMGuardrails:
    class TestSkipSystemMessageForChatCompletions:
        ...
        def test_effective_skip_respects_per_guardrail_over_global(self, monkeypatch):
            monkeypatch.setattr(
                derouter, "skip_system_message_in_guardrail", True, raising=False
            )
    ...
    class TestSkipToolMessageForChatCompletions:
        ...
        def test_effective_skip_tool_respects_per_guardrail_over_global(
            ...
            monkeypatch.setattr(
                derouter, "skip_tool_message_in_guardrail", True, raising=False
            )
    ...
    class TestAsyncPostCallStreamingIteratorHook:
        ...
        async def test_streaming_content_not_lost_on_sampled_chunks(self, monkeypatch):
            ...
            class _ContentClearingTranslation(BaseTranslation):
                ...
                async def process_output_response(self, response, guardrail_to_apply, derouter_logging_obj=None, user_api_key_dict=None):  # type: ignore[override]
                    return response
        ...
        def test_openai_prefixed_aliases_are_authorized_like_canonical_routes(self) -> None:
            ...
            for route in (
                ...
                assert route in openai_routes, (
                    f"{route!r} missing from DeRouterRoutes.openai_routes; team and "
...
class TestStreamingGuardrailInformationBucket:
    ...
    ``derouter_metadata`` key first, flipping the bucket so every later
    ...
    async def test_chat_eos_scan_writes_guardrail_information_to_metadata(self):
        ...
        assert "derouter_metadata" not in request_data
