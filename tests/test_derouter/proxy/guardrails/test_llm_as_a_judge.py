...
def _make_derouter_params(**overrides):
    params = MagicMock()
...
def _make_guardrail_dict(name="g", **derouter_params_overrides):
    raw = {"judge_model": "gpt-4o-mini", "criteria": CRITERIA_100, "on_failure": "block", "overall_threshold": 80.0}
    raw.update(derouter_params_overrides)
    return {"guardrail_name": name, "derouter_params": raw}


@patch("derouter.proxy.guardrails.guardrail_hooks.llm_as_a_judge.derouter.logging_callback_manager")
def test_initialize_guardrail_ok(mock_mgr):
    lp = _make_derouter_params()
    g = _make_guardrail_dict()
    instance = initialize_guardrail(lp, g)
    assert isinstance(instance, LLMAsAJudgeGuardrail)
    mock_mgr.add_derouter_callback.assert_called_once_with(instance)


def test_initialize_guardrail_missing_judge_model():
    lp = _make_derouter_params()
    g = _make_guardrail_dict(judge_model=None)
    g["derouter_params"].pop("judge_model")
...
@pytest.mark.asyncio
@patch("derouter.proxy.guardrails.guardrail_hooks.llm_as_a_judge.derouter.acompletion")
...
@pytest.mark.parametrize(
    ...
    [
        (
            [{"model_name": "my-judge-alias", "derouter_params": {"model": "anthropic/claude-sonnet-4-6", "api_key": "sk-ant-test"}}],
            {},
            "my-judge-alias",
        ),
        (
            [{"model_name": "anthropic/*", "derouter_params": {"model": "anthropic/*", "api_key": "sk-ant-test"}}],
            {},
            "anthropic/claude-sonnet-4-6",
        ),
        (
            [{"model_name": "backing-group", "derouter_params": {"model": "anthropic/claude-sonnet-4-6", "api_key": "sk-ant-test"}}],
...
@pytest.mark.asyncio
@patch("derouter.proxy.guardrails.guardrail_hooks.llm_as_a_judge.derouter.acompletion", new_callable=AsyncMock)
...
async def test_judge_call_falls_back_to_sdk_when_model_not_in_router(mock_sdk_completion):
    ...
    router = _real_router(
        [{"model_name": "some-other-model", "derouter_params": {"model": "openai/gpt-4o-mini", "api_key": "sk-test"}}]
    )
...
def test_default_router_provider_returns_none_when_proxy_not_importable():
    ...
    from derouter.proxy.guardrails.guardrail_hooks.llm_as_a_judge import _default_router_provider
...
def test_default_router_provider_reads_global_router():
    ...
    sentinel = object()
    with patch("derouter.proxy.proxy_server.llm_router", sentinel):
