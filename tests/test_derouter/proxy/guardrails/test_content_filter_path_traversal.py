...
class TestContentFilterPathTraversal:
    ...
    def test_valid_category_file_inside_categories_dir_allowed(self):
        ...
        categories_dir = os.path.join(
            os.path.dirname(
                __import__(
                    "derouter.proxy.guardrails.guardrail_hooks.derouter_content_filter.content_filter",
    ...
    def test_allow_external_paths_env_var_bypasses_jail(self, tmp_path):
        """DEROUTER_CONTENT_FILTER_ALLOW_EXTERNAL_PATHS=true skips the directory jail."""
        import os as _os
        from derouter.proxy.guardrails.guardrail_hooks.derouter_content_filter.content_filter import (
            ContentFilterGuardrail,
        )
        ...
        with patch.dict(
            _os.environ, {"DEROUTER_CONTENT_FILTER_ALLOW_EXTERNAL_PATHS": "true"}
        ):
    ...
    def test_traversal_blocked_when_allow_external_not_set(self):
        ...
        with patch.dict(_os.environ, {}, clear=False):
            _os.environ.pop("DEROUTER_CONTENT_FILTER_ALLOW_EXTERNAL_PATHS", None)
            with pytest.raises(ValueError, match="outside the allowed categories"):
                guardrail._resolve_category_file_path("/etc/passwd")
