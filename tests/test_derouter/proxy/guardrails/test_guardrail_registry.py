...
from derouter.types.guardrails import GuardrailEventHooks, Guardrail, LitellmParams
...
def test_noma_registry_resolution():
    from derouter.proxy.guardrails.guardrail_hooks.noma.noma import NomaGuardrail
    from derouter.proxy.guardrails.guardrail_hooks.noma.noma_v2 import NomaV2Guardrail
    from derouter.proxy.guardrails.guardrail_registry import (
...
def test_initialize_guardrail_run_in_parallel_preserves_constructor_default(configured, expected):
    ...
    def _initializer(derouter_params, guardrail):
    ...
    try:
        ...
        result = handler.initialize_guardrail(
            guardrail={"guardrail_name": "cf-parallel-default", "derouter_params": params},
        )
...
def _register_mode_following_initializer(guardrail_type: str):
    """Registers like the shipped initializers do: construct, then add the instance to derouter's callbacks."""
    import derouter
    from derouter.proxy.guardrails import guardrail_registry as registry_module
    ...
    def _initializer(derouter_params, guardrail):
        callback = CustomGuardrail(
            ...
            event_hook=GuardrailEventHooks(derouter_params.mode),
            default_on=True,
        )
...
def _mode_following_db_row(guardrail_id: str, mode: str, description: str = "") -> Guardrail:
    """The raw row GuardrailRegistry.update_guardrail_in_db hands back: derouter_params is a plain dict."""
    return Guardrail(
        guardrail_id=guardrail_id,
        guardrail_name="mode-following",
        derouter_params={"guardrail": "mode_following_test", "mode": mode, "default_on": True},
...
def test_update_in_memory_guardrail_raw_db_row_mode_change_gates_at_the_new_stage():
    ...
    try:
        ...
        assert handler.IN_MEMORY_GUARDRAILS["123"]["derouter_params"].mode == "post_call"
...
def test_update_in_memory_guardrail_invalid_row_keeps_the_previous_instance_enforcing():
    ...
    try:
        ...
        assert handler.IN_MEMORY_GUARDRAILS["123"]["derouter_params"].mode == "pre_call"
        assert _live_instances_named("mode-following") == 1
    finally:
        registry_module.guardrail_initializer_registry.pop("mode_following_test", None)
        for cb_list, snapshot in zip(lists, snapshots):
...
def _make_guardrail(guardrail_id: str, name: str = "g") -> Guardrail:
    return Guardrail(
        guardrail_id=guardrail_id,
        guardrail_name=name,
        derouter_params=LitellmParams(guardrail=name, mode="pre_call", default_on=False),
    )
...
def test_initialize_guardrail_early_return_updates_source_marker():
    ...
    g = Guardrail(
        ...
        derouter_params=LitellmParams(guardrail="bedrock", mode="pre_call", default_on=False),
    )
...
def _db_derouter_params() -> dict:
    """
    Shape produced by GuardrailRegistry.get_all_guardrails_from_db: derouter_params
...
def test_unchanged_db_params_do_not_register_as_changed():
    """
    A DB poll returns derouter_params as a raw dict while the in-memory copy is a
    ...
    """
    handler = InMemoryGuardrailHandler()
    raw = _db_derouter_params()
    gid = "11111111-1111-1111-1111-111111111111"
    handler.IN_MEMORY_GUARDRAILS[gid] = Guardrail(
        guardrail_id=gid,
        guardrail_name="cf",
        derouter_params=LitellmParams(**raw),
    )

    new = Guardrail(guardrail_id=gid, guardrail_name="cf", derouter_params=dict(raw))
...
def test_changed_db_params_register_as_changed():
    ...
    new = Guardrail(guardrail_id=gid, guardrail_name="cf", derouter_params=changed)
...
def test_unnormalizable_db_params_register_as_changed_without_raising():
    """
    A DB row whose derouter_params fail LitellmParams validation must not crash the
    ...
    new = Guardrail(guardrail_id=gid, guardrail_name="cf", derouter_params=malformed)
    assert handler._has_guardrail_params_changed(gid, new) is True
...
def test_delete_in_memory_guardrail_removes_callback_from_all_lists():
    """
    Request handling promotes guardrail callbacks from derouter.callbacks into the
...
def test_repeated_db_sync_does_not_accumulate_runner_instances():
    ...
    def db_guardrail(word: str) -> Guardrail:
        ...
        return Guardrail(guardrail_id=gid, guardrail_name=name, derouter_params=params)

    def promote_into_request_lists() -> None:
        manager = derouter.logging_callback_manager
        for callback in list(derouter.callbacks):
            manager.add_derouter_success_callback(callback)
            manager.add_derouter_failure_callback(callback)
            manager.add_derouter_async_success_callback(callback)
            manager.add_derouter_async_failure_callback(callback)

    def distinct_runner_instances() -> int:
        seen = set()
        for callback in derouter.logging_callback_manager._get_all_callbacks():
...
def test_presidio_siblings_are_tracked_and_deleted_together():
    ...
    try:
        ...
        registered = _presidio_callbacks_in(derouter.callbacks)
...
def test_update_in_memory_guardrail_rebuilds_presidio_siblings_and_keeps_their_stage():
    ...
    try:
        ...
        tracked = _presidio_callbacks_in(derouter.callbacks)
        roles_before = [
        ...
        rebuilt = _presidio_callbacks_in(derouter.callbacks)
...
def test_repeated_db_sync_replaces_presidio_siblings_instead_of_leaking_stale_ones():
    ...
    try:
        ...
        for cycle in range(4):
            ...
            for cb_list in lists[1:]:
                cb_list.extend(_presidio_callbacks_in(derouter.callbacks))
...
def test_db_synced_judge_guardrail_uses_lazy_router_provider():
    ...
    from derouter.proxy.guardrails.guardrail_hooks.llm_as_a_judge import (
...
def _lakera_guardrail(guardrail_id: str, **derouter_params_overrides) -> Guardrail:
    params = {"guardrail": "lakera_v2", "mode": "pre_call", "on_flagged": "block", **derouter_params_overrides}
    return Guardrail(
        guardrail_id=guardrail_id,
        guardrail_name="lakera-test",
        derouter_params=LitellmParams(**params),
    )


class TestReinitializeGuardrailRestoresOnFailure:
    """Maintainer finding on Delify-Solutions/derouter#34940: reinitialize_guardrail deletes
    ...
    def test_invalid_update_leaves_dict_metadata_matching_the_restored_instance(self):
        ...
        try:
            ...
            assert handler.IN_MEMORY_GUARDRAILS["lakera-restore-meta"]["derouter_params"].on_flagged == "block"
        finally:
            for cb_list, snapshot in zip(lists, snapshots):
                cb_list[:] = snapshot
    ...
    def _initialize(self, name: str, params: dict):
        ...
        try:
            return InMemoryGuardrailHandler().initialize_guardrail(
                guardrail={"guardrail_name": name, "derouter_params": params},
            )
        finally:
            for cb_list, snapshot in zip(lists, snapshots):
                cb_list[:] = snapshot
...
async def test_update_guardrail_in_db_raises_when_row_missing():
    ...
    prisma_client.db.derouter_guardrailstable.update = AsyncMock(return_value=None)
    ...
    with pytest.raises(
        ...
        await GuardrailRegistry().update_guardrail_in_db(
            ...
            guardrail=Guardrail(
                ...
                derouter_params=LitellmParams(guardrail="bedrock", mode="pre_call"),
            ),
...
def test_reinitialize_guardrail_restores_previous_on_failure():
    ...
    def _initializer(derouter_params, guardrail):
        if derouter_params.api_key == "boom":
    ...
    try:
        ...
        created = handler.initialize_guardrail(
            guardrail={
                ...
                "derouter_params": {"guardrail": "restore_test", "mode": "pre_call", "api_key": "ok"},
            },
        )
        ...
        with pytest.raises(ValueError, match="invalid updated params"):
            handler.reinitialize_guardrail(
                guardrail={
                    "guardrail_id": guardrail_id,
                    "guardrail_name": "restore-me",
                    "derouter_params": {"guardrail": "restore_test", "mode": "pre_call", "api_key": "boom"},
                },
            )
...
def test_reinitialize_guardrail_raises_value_error_for_non_value_error_init_failures():
    ...
    def _initializer(derouter_params, guardrail):
        if derouter_params.api_key == "bad-regex":
            re.compile("([")
        return CustomGuardrail(
            guardrail_name=guardrail["guardrail_name"],
            event_hook=GuardrailEventHooks.pre_call,
    ...
    try:
        ...
        created = handler.initialize_guardrail(
            guardrail={
                ...
                "derouter_params": {"guardrail": "regex_test", "mode": "pre_call", "api_key": "ok"},
            },
        )
        guardrail_id = created["guardrail_id"]
        ...
        with pytest.raises(ValueError, match="Guardrail initialization failed") as excinfo:
            handler.reinitialize_guardrail(
                guardrail={
                    "guardrail_id": guardrail_id,
                    "guardrail_name": "regex-me",
                    "derouter_params": {"guardrail": "regex_test", "mode": "pre_call", "api_key": "bad-regex"},
                },
            )
...
def test_sync_guardrail_from_db_applies_db_dict_params_to_live_instance():
    """
    Regression for PUT /guardrails/{id}: the DB row arrives with derouter_params as
