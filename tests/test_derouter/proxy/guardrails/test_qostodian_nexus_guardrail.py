...
def test_qostodian_nexus_initialization_with_defaults():
    ...
    from derouter.proxy.guardrails.guardrail_hooks.qohash import QostodianNexus
...
def test_qostodian_nexus_in_guardrail_initializer_registry():
    ...
    from derouter.proxy.guardrails.guardrail_hooks.qohash import (
...
def test_qostodian_nexus_initializer_function():
    ...
    from derouter.proxy.guardrails.guardrail_hooks.qohash import initialize_guardrail
    from derouter.types.guardrails import LitellmParams, Guardrail
    from unittest.mock import patch

    # Mock derouter.logging_callback_manager
    with patch("derouter.logging_callback_manager") as mock_manager:
        mock_manager.add_derouter_callback = MagicMock()
        ...
        # Call initializer
        result = initialize_guardrail(derouter_params, guardrail_config)

        # Verify callback was added
        mock_manager.add_derouter_callback.assert_called_once()
...
def test_qostodian_nexus_guardrail_name_constant():
    ...
    from derouter.proxy.guardrails.guardrail_hooks.qohash.qohash import GUARDRAIL_NAME
...
def test_qostodian_nexus_unified_detection():
    """
    Test that QostodianNexus is properly detected by DeRouter's unified guardrail system.
    ...
    # Create an instance (this is how DeRouter uses it)
    instance = QostodianNexus(api_base="http://test:8800")

    # Test the exact detection logic used in derouter/proxy/utils.py:868
