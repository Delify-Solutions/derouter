...
def test_initialize_presidio_guardrail():
    ...
    assert result["derouter_params"].guardrail == SupportedGuardrailIntegrations.PRESIDIO.value
    assert result["derouter_params"].mode == "pre_call"
...
def test_initialize_guardrail_sets_run_in_parallel(config_value, expected):
    """run_in_parallel from derouter_params must reach the built guardrail instance."""
    derouter_params = {
        "guardrail": SupportedGuardrailIntegrations.PRESIDIO.value,
        "mode": "pre_call",
        "presidio_analyzer_api_base": "https://fakelink.com/v1/presidio/analyze",
    ...
    if config_value is not None:
        derouter_params["run_in_parallel"] = config_value

    guardrail_handler = InMemoryGuardrailHandler()
    result = guardrail_handler.initialize_guardrail(
        guardrail={"guardrail_name": "test_parallel_flag", "derouter_params": derouter_params},
    )
...
def test_initialize_guardrail_sets_scan_raw_request(config_value, expected):
    """scan_raw_request from derouter_params must reach the built guardrail instance,
    ...
    if config_value is not None:
        derouter_params["scan_raw_request"] = config_value

    guardrail_handler = InMemoryGuardrailHandler()
    result = guardrail_handler.initialize_guardrail(
        guardrail={"guardrail_name": "test_scan_raw_request_flag", "derouter_params": derouter_params},
    )
...
def test_init_guardrails_v2_skips_invalid_guardrail_instead_of_crashing_boot():
    """
    Regression: one guardrail with an invalid derouter_params combination (Lakera's
    ...
    here anymore (maintainer finding on Delify-Solutions/derouter#34940): that combination is
...
def test_init_guardrails_v2_accepts_during_call_advisory_mode():
    """
    Maintainer finding on Delify-Solutions/derouter#34940: on_flagged='inject_system_message'
