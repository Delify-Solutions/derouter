...
def _make_signer(**kwargs: Any):
    ...
    import derouter.proxy.guardrails.guardrail_hooks.mcp_jwt_signer.mcp_jwt_signer as mod
...
def test_jwks_public_key_can_verify_signed_jwt():
    ...
    signer = _make_signer(issuer="https://derouter.example.com", audience="mcp")
    now = int(time.time())
    claims = {
        "iss": "https://derouter.example.com",
        "aud": "mcp",
        "iat": now,
        "exp": now + 300,
    }
    ...
    decoded = jwt.decode(
        ...
        algorithms=["RS256"],
        audience="mcp",
        issuer="https://derouter.example.com",
    )
    assert decoded["iss"] == "https://derouter.example.com"
...
def test_build_claims_standard_fields():
    ...
    signer = _make_signer(
        issuer="https://derouter.example.com", audience="mcp", ttl_seconds=300
    )
    ...
    assert claims["iss"] == "https://derouter.example.com"
...
def test_build_claims_act_fallback_to_derouter_proxy():
    """_build_claims() falls back to 'derouter-proxy' when team_id and org_id are absent."""
    signer = _make_signer()
    user_dict = _make_user_api_key_dict()
    ...
    assert claims["act"]["sub"] == "derouter-proxy"
...
def test_build_claims_sub_fallback_to_derouter_proxy_when_no_token():
    """_build_claims() falls back to 'derouter-proxy' when user_id and token are both absent."""
    signer = _make_signer()
    user_dict = _make_user_api_key_dict(user_id="")
    user_dict.user_id = None
    ...
    assert claims["sub"] == "derouter-proxy"
...
def test_end_user_claim_sources_token_sub():
    ...
    signer = _make_signer(end_user_claim_sources=["token:sub", "derouter:user_id"])
    user_dict = _make_user_api_key_dict(user_id="derouter-user")
...
def test_end_user_claim_sources_falls_back_to_derouter_user_id():
    """Falls back to derouter:user_id when token:sub is absent."""
    signer = _make_signer(end_user_claim_sources=["token:sub", "derouter:user_id"])
    user_dict = _make_user_api_key_dict(user_id="derouter-user")
    ...
    assert claims["sub"] == "derouter-user"
...
def test_end_user_claim_sources_derouter_email():
    """derouter:email resolves from UserAPIKeyAuth.user_email."""
    signer = _make_signer(end_user_claim_sources=["derouter:email"])
...
async def test_debug_header_injected_when_enabled():
    """x-derouter-mcp-debug header is injected when debug_headers=True."""
    ...
    assert isinstance(result, dict)
    assert "x-derouter-mcp-debug" in result["extra_headers"]
    debug_val = result["extra_headers"]["x-derouter-mcp-debug"]
...
async def test_debug_header_absent_when_disabled():
    """x-derouter-mcp-debug is NOT injected when debug_headers=False (default)."""
    signer = _make_signer()
    user_dict = _make_user_api_key_dict()
    data = {"mcp_tool_name": "tool"}
    ...
    assert isinstance(result, dict)
    assert "x-derouter-mcp-debug" not in result["extra_headers"]
...
async def test_sub_resolved_from_user_api_key_dict_jwt_claims():
    ...
    the guardrail resolves sub from jwt_claims (DeRouter-decoded JWT path).
    """
    signer = _make_signer(end_user_claim_sources=["token:sub", "derouter:user_id"])
    user_dict = _make_user_api_key_dict(user_id="derouter-fallback")
    # jwt_claims populated by DeRouter's JWT auth machinery
...
def test_initialize_guardrail_passes_all_params():
    ...
    from derouter.proxy.guardrails.guardrail_hooks.mcp_jwt_signer import (
        initialize_guardrail,
    )

    derouter_params = MagicMock()
    derouter_params.mode = "pre_mcp_call"
    derouter_params.default_on = True
    derouter_params.optional_params = None
    # Set every non-default param directly on derouter_params
    derouter_params.issuer = "https://derouter.example.com"
    derouter_params.audience = "mcp-test"
    derouter_params.ttl_seconds = 120
    derouter_params.access_token_discovery_uri = (
        "https://idp.example.com/.well-known/openid-configuration"
    )
    derouter_params.token_introspection_endpoint = "https://idp.example.com/introspect"
    derouter_params.verify_issuer = "https://idp.example.com"
    derouter_params.verify_audience = "api://test"
    derouter_params.end_user_claim_sources = ["token:email", "derouter:user_id"]
    derouter_params.add_claims = {"deployment_id": "prod"}
    derouter_params.set_claims = {"env": "production"}
    derouter_params.remove_claims = ["nbf"]
    derouter_params.channel_token_audience = "bedrock-gateway"
    derouter_params.channel_token_ttl = 60
    derouter_params.required_claims = ["sub", "email"]
    derouter_params.optional_claims = ["groups"]
    derouter_params.debug_headers = True
    derouter_params.allowed_scopes = ["mcp:tools/call"]

    guardrail = {"guardrail_name": "mcp-jwt-signer"}

    with patch("derouter.logging_callback_manager.add_derouter_callback"):
        signer = initialize_guardrail(derouter_params, guardrail)

    assert signer.issuer == "https://derouter.example.com"
    ...
    assert signer.end_user_claim_sources == ["token:email", "derouter:user_id"]
...
import derouter.proxy.guardrails.guardrail_hooks.mcp_jwt_signer.mcp_jwt_signer as _signer_mod
...
async def test_get_oidc_discovery_caches_when_jwks_uri_present():
    ...
    with patch(
        "derouter.proxy.guardrails.guardrail_hooks.mcp_jwt_signer.mcp_jwt_signer._fetch_oidc_discovery",
...
async def test_verify_incoming_jwt_returns_payload_on_valid_token():
    ...
    with patch.object(
        ...
        with patch(
            "derouter.proxy.guardrails.guardrail_hooks.mcp_jwt_signer.mcp_jwt_signer._fetch_jwks",
            new_callable=AsyncMock,
...
async def test_inject_mcp_jwt_signs_for_list_tools_path():
    ...
    _make_signer(issuer="https://derouter.example.com", audience="mcp", ttl_seconds=300)
    user_dict = UserAPIKeyAuth(api_key="sk-test", user_id="alice")
