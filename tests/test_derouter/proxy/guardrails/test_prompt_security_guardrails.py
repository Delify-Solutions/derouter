...
import derouter
from derouter.proxy.guardrails.guardrail_hooks.prompt_security.prompt_security import (
...
def test_prompt_security_guard_config(monkeypatch: pytest.MonkeyPatch):
    ...
    registered = [c for c in derouter.callbacks if isinstance(c, PromptSecurityGuardrail)]
...
async def test_user_api_key_alias_forwarding(monkeypatch: pytest.MonkeyPatch):
    ...
    request_data = {
        ...
        "derouter_metadata": {"user_api_key_alias": "vk-alias"},
    }
    ...
    assert headers.get("X-DeRouter-Key-Alias") == "vk-alias"
