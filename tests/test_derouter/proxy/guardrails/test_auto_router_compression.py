"""Unit tests for derouter.proxy.guardrails.auto_router_compression."""
...
class TestPolicyFromLitellmParams:
    def test_neither_key_set_is_no_policy(self):
        assert policy_from_derouter_params({}) is None

    def test_routing_only(self):
        policy = policy_from_derouter_params({"auto_router_routing_compression": "headroom-a"})
    ...
    def test_none_sentinel_normalizes_to_no_compression(self):
        policy = policy_from_derouter_params(
    ...
    def test_none_sentinel_is_case_insensitive(self):
        policy = policy_from_derouter_params({"auto_router_routing_compression": "NONE"})
...
class _FakeRouter:
    """Minimal stand-in for derouter.Router.get_model_list, for policy_for_model."""
...
class TestPolicyForModel:
    ...
    def test_no_marker_deployment_returns_none(self):
        router = _FakeRouter([{"model_name": "smart-router", "derouter_params": {"model": "openai/gpt-4o-mini"}}])
    ...
    def test_marker_deployment_without_policy_returns_none(self):
        router = _FakeRouter(
            [{"model_name": "smart-router", "derouter_params": {"model": "auto_router/complexity_router"}}]
        )
        assert policy_for_model(llm_router=router, model_alias="smart-router", team_id=None, request_tags=()) is None
...
class TestArmPreCall:
    ...
    async def test_policy_suppresses_active_compression_guardrails(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "derouter.proxy.guardrails.auto_router_compression.COMPRESSION_GUARDRAIL_PROVIDERS",
            frozenset({"fake-provider"}),
        )
        import derouter

        always_on = _RecordingCompressionGuardrail(guardrail_name="always-on-compression")
        derouter.logging_callback_manager.add_derouter_callback(always_on)
        try:
            router = _FakeRouter(
                [
                    {
            ...
            derouter.logging_callback_manager.remove_callback_from_all_lists(always_on)
    ...
    async def test_model_side_guardrail_is_requested_even_when_not_default_on(self, monkeypatch):
        ...
        derouter.logging_callback_manager.add_derouter_callback(active)
        router = _FakeRouter(
            [
                {
                    "model_name": "smart-router",
        ...
        try:
            ...
            derouter.logging_callback_manager.remove_callback_from_all_lists(active)
...
class TestMessagesForRouting:
    ...
    async def test_a_non_compression_guardrail_is_never_invoked_for_routing(self, monkeypatch):
        ...
        derouter.logging_callback_manager.add_derouter_callback(other)
        try:
            ...
            derouter.logging_callback_manager.remove_callback_from_all_lists(other)
