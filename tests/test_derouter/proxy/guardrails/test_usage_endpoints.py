...
which read the derouter_guardrailstable Prisma table — could not see them:
...
from derouter.proxy.guardrails.guardrail_registry import InMemoryGuardrailHandler
from derouter.proxy.guardrails.usage_endpoints import (
...
def _yaml_guardrail(
    ...
    return Guardrail(
        ...
        guardrail_name=name,
        derouter_params=LitellmParams(guardrail=provider, mode="pre_call"),
...
def _db_row(guardrail_id: str = "db-1", name: str = "db-guard", provider: str = "aim") -> Any:
    """A Prisma-style row: attribute access, derouter_params/guardrail_info as plain dicts."""
    row = MagicMock(spec=["guardrail_id", "guardrail_name", "derouter_params", "guardrail_info"])
    row.guardrail_id = guardrail_id
    row.guardrail_name = name
    row.derouter_params = {"guardrail": provider, "mode": "pre_call"}
...
def _prisma(
    *,
    ...
    client = MagicMock()
    db = client.db
    db.derouter_guardrailstable.find_many = AsyncMock(return_value=find_many or [])
    db.derouter_guardrailstable.find_unique = AsyncMock(return_value=find_unique)
    db.derouter_dailyguardrailmetrics.find_many = AsyncMock(return_value=metrics or [])
    db.derouter_dailyguardrailusageunits.find_many = AsyncMock(return_value=units or [])
    db.derouter_spendlogguardrailindex.find_many = AsyncMock(return_value=index_find_many or [])
    db.derouter_spendlogguardrailindex.count = AsyncMock(return_value=0)
    db.derouter_spendlogs.find_many = AsyncMock(return_value=[])
    return client
...
def _patches(prisma: MagicMock, handler: InMemoryGuardrailHandler):
    return (
        ...
        patch("derouter.proxy.guardrails.guardrail_registry.IN_MEMORY_GUARDRAIL_HANDLER", handler),
    )
...
async def test_overview_reports_usage_units_per_row_and_total():
    ...
    units_where = prisma.db.derouter_dailyguardrailusageunits.find_many.call_args.kwargs["where"]
...
def _units_table_missing() -> TableNotFoundError:
    return TableNotFoundError(
        data={"user_facing_error": {"meta": {"table": "public.DeRouter_DailyGuardrailUsageUnits"}}}
    )
...
async def test_overview_degrades_units_to_empty_when_units_table_is_missing():
    ...
    prisma.db.derouter_dailyguardrailusageunits.find_many = AsyncMock(side_effect=_units_table_missing())
    handler = _config_handler(_yaml_guardrail(guardrail_id="yaml-uuid", name="yaml-pii"))
    p1, p2 = _patches(prisma, handler)
    with p1, p2:
        resp = await guardrails_usage_overview(start_date=START, end_date=END, user_api_key_dict=ADMIN)
...
async def test_logs_resolves_config_guardrail_logical_name():
    ...
    where = prisma.db.derouter_spendlogguardrailindex.find_many.call_args.kwargs["where"]
...
async def test_logs_reports_flagged_action_for_guardrail_flagged_status():
    ...
    prisma.db.derouter_spendlogs.find_many = AsyncMock(
        return_value=[
...
async def test_policies_overview_returns_a_full_row_and_totals():
    ...
    prisma = _prisma()
    prisma.db.derouter_policytable.find_many = AsyncMock(return_value=[policy])
    prisma.db.derouter_dailypolicymetrics.find_many = AsyncMock(return_value=[metric])
    p1, p2 = _patches(prisma, _config_handler())
    with p1, p2:
...
async def test_detail_prev_trend_query_is_bounded():
    ...
    wheres = [c.kwargs["where"] for c in prisma.db.derouter_dailyguardrailmetrics.find_many.await_args_list]
