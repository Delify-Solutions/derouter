...
def _prisma() -> MagicMock:
    client = MagicMock()
    db = client.db
    db.derouter_dailyguardrailmetrics.upsert = AsyncMock()
    db.derouter_dailyguardrailusageunits.upsert = AsyncMock()
    db.derouter_spendlogguardrailindex.create_many = AsyncMock()
    return client
...
def _units_upserts(prisma: MagicMock) -> dict[tuple, int]:
    calls = prisma.db.derouter_dailyguardrailusageunits.upsert.call_args_list
    out: dict[tuple, int] = {}
    for c in calls:
...
async def test_flagged_status_counts_as_flagged_not_passed_or_blocked():
    ...
    create = prisma.db.derouter_dailyguardrailmetrics.upsert.call_args.kwargs["data"]["create"]
...
async def test_one_failing_upsert_does_not_drop_remaining_writes():
    ...
    """
    prisma = _prisma()
    prisma.db.derouter_dailyguardrailmetrics.upsert.side_effect = httpx.ConnectError("db down")
    prisma.db.derouter_dailyguardrailusageunits.upsert.side_effect = [httpx.ConnectError("db down"), None, None]
    sleep, _ = _fake_sleep()
    logs = [
...
async def test_transient_upsert_failure_is_retried_with_backoff_for_failed_rows_only():
    ...
    """
    prisma = _prisma()
    prisma.db.derouter_dailyguardrailusageunits.upsert.side_effect = [httpx.ConnectError("blip"), None, None]
...
async def test_persistent_upsert_failure_stops_after_three_retries():
    ...
    assert prisma.db.derouter_dailyguardrailmetrics.upsert.call_count == 4
    assert delays == [1, 2, 4]
    assert prisma.db.derouter_dailyguardrailusageunits.upsert.call_count == 1
...
async def test_retry_exhausted_rows_are_requeued_and_land_on_the_next_flush():
    ...
    pending = PendingRollups()
    down = _prisma()
    down.db.derouter_dailyguardrailmetrics.upsert.side_effect = httpx.ConnectError("db down")
    down.db.derouter_dailyguardrailusageunits.upsert.side_effect = httpx.ConnectError("db down")
    sleep, _ = _fake_sleep()
    ...
    metrics_create = recovered.db.derouter_dailyguardrailmetrics.upsert.call_args.kwargs["data"]["create"]
...
async def test_ambiguous_failures_are_never_requeued():
    ...
    pending = PendingRollups()
    prisma = _prisma()
    prisma.db.derouter_dailyguardrailusageunits.upsert.side_effect = httpx.ReadTimeout("maybe committed")
    sleep, delays = _fake_sleep()
...
def _units_upsert_wheres(prisma: MagicMock) -> list[tuple]:
    return [
        ...
        for c in prisma.db.derouter_dailyguardrailusageunits.upsert.call_args_list
    ]
...
async def test_post_send_failure_is_never_retried_so_increments_cannot_double_count():
    ...
    """
    prisma = _prisma()
    prisma.db.derouter_dailyguardrailusageunits.upsert.side_effect = [
...
async def test_generic_upsert_exception_is_terminal_for_that_row_only():
    prisma = _prisma()
    prisma.db.derouter_dailyguardrailmetrics.upsert.side_effect = RuntimeError("constraint violation")
    sleep, delays = _fake_sleep()

    await process_spend_logs_guardrail_usage(prisma, [_payload("r1", usage={"topicPolicyUnits": 1})], sleep=sleep)

    assert prisma.db.derouter_dailyguardrailmetrics.upsert.call_count == 1
    assert delays == []
    assert _units_upserts(prisma) == {
        ("bedrock-guard", "2026-08-17", "team-a", "hashed-key-1", "topicPolicyUnits"): 1,
    }
...
async def test_payload_without_request_id_is_skipped_like_the_metrics_path():
    ...
    assert prisma.db.derouter_dailyguardrailmetrics.upsert.call_args.kwargs["data"]["create"]["requests_evaluated"] == 1
