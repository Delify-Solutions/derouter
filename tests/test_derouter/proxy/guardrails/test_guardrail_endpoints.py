...
from derouter.proxy.guardrails.guardrail_endpoints import (
...
MOCK_GUARDRAIL = Guardrail(
    ...
    derouter_params=LitellmParams(**MOCK_CONFIG_GUARDRAIL["derouter_params"]),
...
MOCK_PATCH_REQUEST = PatchGuardrailRequest(
    ...
    derouter_params={"guardrail": "updated.guardrail", "mode": "post_call"},
...
def mock_prisma_client(mocker):
    ...
    mock_client.db.derouter_guardrailstable = mocker.Mock()
    mock_client.db.derouter_guardrailstable.find_many = AsyncMock(
        return_value=[MOCK_DB_GUARDRAIL]
    )
    mock_client.db.derouter_guardrailstable.find_unique = AsyncMock(
        return_value=MOCK_DB_GUARDRAIL
    )
    return mock_client
...
async def test_list_guardrails_v2_with_db_and_config(
    ...
    mocker.patch(
        "derouter.proxy.guardrails.guardrail_registry.IN_MEMORY_GUARDRAIL_HANDLER",
    ...
    assert isinstance(db_guardrail.derouter_params, BaseLitellmParams)
    ...
    assert isinstance(config_guardrail.derouter_params, BaseLitellmParams)
...
async def test_list_guardrails_v2_skips_stale_db_backed_in_memory_entries(mocker):
    ...
    stale_guardrail = {
        ...
        "derouter_params": {"guardrail": "bedrock", "mode": "pre_call"},
        "guardrail_info": {},
    }
    mock_prisma_client = mocker.Mock()
    mock_prisma_client.db = mocker.Mock()
    mock_prisma_client.db.derouter_guardrailstable = mocker.Mock()
    mock_prisma_client.db.derouter_guardrailstable.find_many = AsyncMock(return_value=[])
...
async def test_get_guardrail_info_404s_stale_db_backed_entry(
    ...
    mock_prisma_client.db.derouter_guardrailstable.find_unique = AsyncMock(
        return_value=None
    )
...
async def test_list_guardrails_v2_masks_sensitive_data_in_db_guardrails(mocker):
    """Test that sensitive derouter_params are masked for DB guardrails in list response"""
    ...
    mock_prisma_client.db = mocker.Mock()
    mock_prisma_client.db.derouter_guardrailstable = mocker.Mock()
    mock_prisma_client.db.derouter_guardrailstable.find_many = AsyncMock(
    ...
    derouter_params = guardrail.derouter_params
    if isinstance(derouter_params, dict):
        params = derouter_params
    else:
        params = (
            derouter_params.model_dump()
            if hasattr(derouter_params, "model_dump")
            else dict(derouter_params)
        )
...
async def test_list_guardrails_v2_masks_sensitive_data_in_config_guardrails(mocker):
    """Test that sensitive derouter_params are masked for in-memory/config guardrails in list response"""
...
async def test_list_guardrails_v2_admin_viewer_sees_guardrails_of_teams_they_are_not_in(
    ...
    mock_get_user_team_ids = mocker.patch(
        "derouter.proxy.guardrails.guardrail_endpoints._get_user_team_ids",
...
async def test_list_guardrails_v2_masks_sensitive_data_for_admin_viewer(mocker):
    ...
    params = guardrail.derouter_params.model_dump()
...
async def test_get_guardrail_info_from_db(mocker, mock_prisma_client):
    ...
    assert isinstance(response.derouter_params, BaseLitellmParams)
...
async def test_list_guardrails_v2_without_prisma_returns_config_guardrails(
    ...
    """
    mocker.patch("derouter.proxy.proxy_server.prisma_client", None)
    mocker.patch(
        "derouter.proxy.guardrails.guardrail_registry.IN_MEMORY_GUARDRAIL_HANDLER",
        mock_in_memory_handler,
    )
        ...
        [
            {
                ...
                "derouter_params": {
                    "guardrail": "derouter_content_filter",
                    "mode": "pre_call",
                },
            }
        ]
...
def test_get_provider_specific_params():
    ...
    from derouter.proxy.guardrails.guardrail_endpoints import _get_fields_from_model
    from derouter.proxy.guardrails.guardrail_hooks.azure import (
...
async def test_provider_specific_params_includes_embedding_toggle():
    from derouter.proxy.guardrails.guardrail_endpoints import get_provider_specific_params
...
def test_optional_params_not_returned_when_not_overridden():
    ...
    from derouter.proxy.guardrails.guardrail_endpoints import _get_fields_from_model
    from derouter.types.proxy.guardrails.guardrail_hooks.base import GuardrailConfigModel
...
async def test_create_guardrail_endpoint(
    ...
    if scenario == "success_with_sync":
        ...
        mocker.patch("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
        mocker.patch(
            "derouter.proxy.guardrails.guardrail_endpoints.GUARDRAIL_REGISTRY",
            mock_guardrail_registry,
        )
        mocker.patch(
            "derouter.proxy.guardrails.guardrail_registry.IN_MEMORY_GUARDRAIL_HANDLER",
        ...
        mock_logger = mocker.patch(
            "derouter.proxy.guardrails.guardrail_endpoints.verbose_proxy_logger"
        )
...
async def test_update_guardrail_endpoint(
    ...
    if scenario == "success_with_sync":
        ...
        mocker.patch("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)  # test-quality-ok: reused pattern
...
async def test_patch_guardrail_endpoint(
    ...
    if scenario == "success_with_sync":
        ...
        # Maintainer finding on Delify-Solutions/derouter#34940: a ValueError from
...
async def test_patch_guardrail_rejects_mcp_only_on_violation_with_422(mocker, mock_guardrail_registry):
    mocker.patch("derouter.proxy.proxy_server.prisma_client", mocker.Mock())  # test-quality-ok: endpoint has no DI seam
    mocker.patch(  # test-quality-ok: endpoint has no DI seam
        "derouter.proxy.guardrails.guardrail_endpoints.GUARDRAIL_REGISTRY", mock_guardrail_registry
    )
    request = PatchGuardrailRequest(derouter_params=BaseLitellmParams(on_violation="block"))
...
async def test_apply_guardrail_not_found(mocker):
    ...
    mocker.patch(
        "derouter.proxy.guardrails.guardrail_endpoints.GUARDRAIL_REGISTRY", mock_registry
    )

    mock_proxy_logging = mocker.Mock()
    mock_proxy_logging.post_call_failure_hook = AsyncMock()
    mocker.patch("derouter.proxy.proxy_server.proxy_logging_obj", mock_proxy_logging)
    mocker.patch("derouter.proxy.proxy_server.general_settings", {})
    mocker.patch("derouter.proxy.proxy_server.proxy_config", mocker.Mock())
    mocker.patch("derouter.proxy.proxy_server.version", "test")
...
async def test_apply_guardrail_execution_error(mocker):
    ...
    mocker.patch("derouter.proxy.proxy_server.proxy_config", mocker.Mock())
    mocker.patch("derouter.proxy.proxy_server.version", "test")
    mocker.patch("derouter.derouter_core_utils.thread_pool_executor.executor")
...
async def test_apply_guardrail_invokes_logging_pipeline(mocker):
    ...
    mocker.patch(
        "derouter.derouter_core_utils.thread_pool_executor.executor", mock_executor
    )
...
async def test_get_guardrail_info_endpoint_config_guardrail(mocker):
    ...
    """
    from derouter.proxy.guardrails.guardrail_endpoints import get_guardrail_info

    # Mock prisma_client to not be None (patch at the source where it's imported from)
    mock_prisma = mocker.Mock()
    mocker.patch("derouter.proxy.proxy_server.prisma_client", mock_prisma)
    ...
    mocker.patch(
        "derouter.derouter_core_utils.derouter_logging._get_masked_values",
...
class TestBuildFieldDict:
    ...
    def test_build_field_dict_with_string_ui_type(self):
        ...
        from derouter.proxy.guardrails.guardrail_endpoints import _build_field_dict
    ...
    def test_build_field_dict_with_enum_ui_type(self):
        ...
        from derouter.proxy.guardrails.guardrail_endpoints import _build_field_dict
        from derouter.types.guardrails import GuardrailParamUITypes
...
async def test_register_guardrail_success(mocker):
    ...
    mock_prisma = mocker.Mock()
    mock_prisma.db.derouter_guardrailstable.find_unique = AsyncMock(return_value=None)
    ...
    mock_prisma.db.derouter_guardrailstable.create = AsyncMock(return_value=created_row)
    mocker.patch("derouter.proxy.proxy_server.prisma_client", mock_prisma)
    ...
    mock_prisma.db.derouter_guardrailstable.create.assert_called_once()
    call_data = mock_prisma.db.derouter_guardrailstable.create.call_args[1]["data"]
...
async def test_register_guardrail_rejects_non_generic_api(mocker):
    """Register returns 400 when derouter_params.guardrail is not generic_guardrail_api."""
    mocker.patch("derouter.proxy.proxy_server.prisma_client", mocker.Mock())
...
async def test_register_guardrail_non_admin_cross_team_allowed(mocker):
    ...
    mock_prisma.db.derouter_guardrailstable.create = AsyncMock(return_value=created)
    mocker.patch("derouter.proxy.proxy_server.prisma_client", mock_prisma)
    mocker.patch(
        "derouter.proxy.guardrails.guardrail_endpoints._get_user_team_ids",
    ...
    req = RegisterGuardrailRequest(
        guardrail_name=MOCK_REGISTER_REQUEST.guardrail_name,
        team_id="team-beta",
        derouter_params=MOCK_REGISTER_REQUEST.derouter_params,
    )
    user = UserAPIKeyAuth(
...
async def test_register_guardrail_duplicate_name(mocker):
    ...
    mock_prisma = mocker.Mock()
    mock_prisma.db.derouter_guardrailstable.find_unique = AsyncMock(
...
async def test_list_guardrail_submissions_non_admin_scoped_to_own_teams(mocker):
    ...
    mock_prisma.db.derouter_guardrailstable.find_many = find_many
    mocker.patch("derouter.proxy.proxy_server.prisma_client", mock_prisma)
    mocker.patch(
        "derouter.proxy.guardrails.guardrail_endpoints._get_user_team_ids",
...
async def test_list_guardrail_submissions_success(mocker):
    ...
    mock_prisma.db.derouter_guardrailstable.find_many = AsyncMock(return_value=[row])
    mocker.patch("derouter.proxy.proxy_server.prisma_client", mock_prisma)
...
async def test_get_guardrail_submission_non_admin_own_team(mocker):
    ...
    mock_prisma.db.derouter_guardrailstable.find_unique = AsyncMock(return_value=row)
    mocker.patch("derouter.proxy.proxy_server.prisma_client", mock_prisma)
    mocker.patch(
        "derouter.proxy.guardrails.guardrail_endpoints._get_user_team_ids",
        AsyncMock(return_value=["team-mine"]),
...
async def test_approve_guardrail_submission_success(mocker):
    ...
    mock_prisma.db.derouter_guardrailstable.find_unique = AsyncMock(return_value=row)
    mock_prisma.db.derouter_guardrailstable.update = AsyncMock()
    mocker.patch("derouter.proxy.proxy_server.prisma_client", mock_prisma)
    ...
    mock_prisma.db.derouter_guardrailstable.update.assert_called_once()
    call_data = mock_prisma.db.derouter_guardrailstable.update.call_args[1]["data"]
...
async def test_list_submissions_summary_counts_unaffected_by_filters(mocker):
    ...
    mock_prisma.db.derouter_guardrailstable.find_many = AsyncMock(return_value=all_rows)
    mocker.patch("derouter.proxy.proxy_server.prisma_client", mock_prisma)
    user = UserAPIKeyAuth(user_role=LitellmUserRoles.PROXY_ADMIN)
...
async def test_get_guardrail_ui_settings_returns_per_provider_supported_modes():
    ...
    assert "pre_mcp_call" in modes_by_provider["derouter_content_filter"]
    assert "during_mcp_call" not in modes_by_provider["derouter_content_filter"]
...
def test_model_armor_runtime_supported_event_hooks_match_classmethod():
    ...
    """
    from derouter.proxy.guardrails.guardrail_hooks.model_armor.model_armor import (
        ModelArmorGuardrail,
    )
...
def test_strict_guardrail_modes_flag_controls_raise_vs_warn(monkeypatch, caplog):
    ...
    mode should be able to set DEROUTER_STRICT_GUARDRAIL_MODES=false and boot
    ...
    monkeypatch.delenv("DEROUTER_STRICT_GUARDRAIL_MODES", raising=False)
    with pytest.raises(ValueError, match="not in the supported event hooks"):
        ContentFilterGuardrail(
    ...
    monkeypatch.setenv("DEROUTER_STRICT_GUARDRAIL_MODES", "false")
