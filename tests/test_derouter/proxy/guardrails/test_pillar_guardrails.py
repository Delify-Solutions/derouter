"""
Pillar Security Guardrail Tests for DeRouter

Tests for the Pillar Security guardrail integration using pytest fixtures
and following DeRouter testing patterns and best practices.
"""
...
from derouter.proxy.guardrails.guardrail_hooks.pillar import (
    PillarGuardrail,
    PillarGuardrailAPIError,
    PillarGuardrailMissingSecrets,
)
from derouter.proxy.guardrails.guardrail_hooks.pillar.pillar import (
...
def setup_and_teardown():
    """
    Standard DeRouter fixture that reloads derouter before every function
    ...
    # 1. derouter not in sys.modules (parallel worker removed it)
    # 2. derouter already imported (normal case)
    _module = importlib.import_module("derouter")
    derouter = importlib.reload(_module)

    # Set up async loop
    loop = asyncio.get_event_loop_policy().new_event_loop()
    asyncio.set_event_loop(loop)

    # Set up derouter state
...
def test_pillar_guard_config_missing_api_key(pillar_guardrail_config, monkeypatch):
    ...
    pillar_guardrail_config["derouter_params"].pop("api_key", None)
...
# =========================================================================
# DEROUTER KEY CONTEXT HEADER TESTS
# =========================================================================


@pytest.mark.asyncio
async def test_derouter_context_headers_automatically_added(
    sample_request_data,
    user_api_key_dict_with_context,
    dual_cache,
    pillar_clean_response,
):
    """Test that DeRouter context headers are automatically added (always enabled)."""
    guardrail = PillarGuardrail(
    ...
    # Verify DeRouter context headers are present
    assert "X-DeRouter-Key-Name" in captured_headers
    assert captured_headers["X-DeRouter-Key-Name"] == "production-api-key"
    assert "X-DeRouter-Key-Alias" in captured_headers
    assert captured_headers["X-DeRouter-Key-Alias"] == "prod-key"
    assert "X-DeRouter-User-Id" in captured_headers
    assert captured_headers["X-DeRouter-User-Id"] == "user-123"
    assert "X-DeRouter-User-Email" in captured_headers
    assert captured_headers["X-DeRouter-User-Email"] == "test@example.com"
    assert "X-DeRouter-Team-Id" in captured_headers
    assert captured_headers["X-DeRouter-Team-Id"] == "team-456"
    assert "X-DeRouter-Team-Name" in captured_headers
    assert captured_headers["X-DeRouter-Team-Name"] == "engineering-team"
    assert "X-DeRouter-Org-Id" in captured_headers
    assert captured_headers["X-DeRouter-Org-Id"] == "org-789"

    # Metadata is NOT sent (may contain sensitive information)
    assert "X-DeRouter-Metadata" not in captured_headers


@pytest.mark.asyncio
async def test_derouter_context_with_partial_fields(
    sample_request_data,
    dual_cache,
    pillar_clean_response,
):
    """Test that partial DeRouter context (only some fields present) is handled correctly."""
    ...
    assert "X-DeRouter-User-Id" in captured_headers
    assert captured_headers["X-DeRouter-User-Id"] == "user-only"
    assert "X-DeRouter-Team-Id" in captured_headers
    assert captured_headers["X-DeRouter-Team-Id"] == "team-only"

    # Verify empty fields are not present
    assert "X-DeRouter-Key-Name" not in captured_headers
    assert "X-DeRouter-User-Email" not in captured_headers
...
def test_metadata_inclusion(pillar_guardrail_instance):
    ...
    assert payload["metadata"]["source"] == "derouter"
