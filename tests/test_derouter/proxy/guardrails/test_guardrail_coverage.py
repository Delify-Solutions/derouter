...
async def test_lasso_multimodal_falls_back_to_classify(user_api_key, monkeypatch):
    ...
    from derouter.proxy.guardrails.guardrail_hooks.lasso.lasso import LassoGuardrail
...
def test_banned_keywords_blocks_multimodal_content(monkeypatch):
    ...
    """
    monkeypatch.setattr("derouter.banned_keywords_list", ["forbidden"], raising=False)
...
async def test_azure_content_safety_pre_call_fires_on_runtime_call_types(
    ...
    from derouter.proxy.hooks.azure_content_safety import _PROXY_AzureContentSafety
...
async def test_openai_moderation_inspects_multimodal_content(monkeypatch, user_api_key):
    ...
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.llm_router", fake_router, raising=False
    )
