...
from derouter.proxy.enterprise_billing import billing_metrics as bm
from derouter.proxy.middleware.billable_request_metrics_middleware import BillableCategory
...
def test_not_premium_returns_none(tmp_path, monkeypatch):
    ...
    assert bm.build_billing_metrics_recorder(premium=False, license_data=None, derouter_version="1.0") is None


def test_premium_without_config_returns_none(monkeypatch):
    assert bm.build_billing_metrics_recorder(premium=True, license_data={"user_id": "x"}, derouter_version="1.0") is None
...
def test_premium_with_missing_cert_files_returns_none(monkeypatch, tmp_path):
    ...
    assert bm.build_billing_metrics_recorder(premium=True, license_data=None, derouter_version="1.0") is None
...
def test_premium_with_full_config_builds_recorder(monkeypatch, tmp_path):
    ...
    recorder = bm.build_billing_metrics_recorder(
        premium=True, license_data={"user_id": "org-1"}, derouter_version="1.0"
    )
...
def test_building_the_recorder_logs_an_affirmative_line(monkeypatch, tmp_path):
    ...
    recorder = bm.build_billing_metrics_recorder(premium=True, license_data={"user_id": "org-1"}, derouter_version="1.0")
...
def test_shutdown_flushes_active_recorder_once(monkeypatch, tmp_path):
    ...
    recorder = bm.build_billing_metrics_recorder(premium=True, license_data=None, derouter_version="1.0")
    assert recorder is not None
...
def test_load_config_carries_license_id(monkeypatch, tmp_path):
    _set_full_env(monkeypatch, tmp_path)
    config = bm.load_billing_metrics_config(license_data={"user_id": "org-42"}, derouter_version="9.9")
    assert config is not None and config.license_id == "org-42" and config.derouter_version == "9.9"
...
def test_load_config_with_empty_string_env_is_disabled(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv(bm.CLIENT_CERT_ENV, paths[bm.CLIENT_CERT_ENV])
    monkeypatch.setenv(bm.CLIENT_KEY_ENV, paths[bm.CLIENT_KEY_ENV])
    assert bm.load_billing_metrics_config(license_data=None, derouter_version="1.0") is None
...
def test_load_config_materializes_inline_pem_content(monkeypatch):
    ...
    config = bm.load_billing_metrics_config(license_data=None, derouter_version="1.0")
...
def test_resource_attributes_include_license_id(tmp_path):
    ...
    assert attrs["service.name"] == "derouter-proxy"
    assert attrs["derouter.version"] == "1.2.3"
    assert attrs["derouter.license.id"] == "org-7"
...
def test_resource_attributes_omit_license_id_when_absent(tmp_path):
    ...
    assert "derouter.license.id" not in attrs
...
def test_record_increments_counter_with_attributes():
    ...
    by_category = {point.attributes["derouter.endpoint.category"]: point.value for point in points}
