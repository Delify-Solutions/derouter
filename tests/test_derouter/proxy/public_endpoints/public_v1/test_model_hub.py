...
def _publish(monkeypatch, infos: Sequence[ModelGroupInfo], prisma_client: object | None = None) -> None:
    monkeypatch.setattr(derouter, "public_model_groups", [info.model_group for info in infos])
    monkeypatch.setattr(
...
def test_the_route_is_registered_as_a_public_route():
    ...
    assert MODEL_HUB_PATH in DeRouterRoutes.public_routes.value
...
def test_a_repeated_sort_field_is_rejected_rather_than_sorted_twice(monkeypatch):
    ...
    assert response.status_code == 400
    assert response.headers["content-type"].startswith("application/problem+json")
    assert response.json()["type"] == "urn:derouter:error:duplicate-sort-field"
...
def guarded(monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.master_key", "sk-1234")
...
def test_no_published_model_groups_yields_an_empty_but_coherent_envelope(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "public_model_groups", None)
...
def test_no_router_answers_with_a_problem_rather_than_the_openai_error_shape(monkeypatch):
    ...
    assert response.status_code == 400
    assert response.headers["content-type"].startswith("application/problem+json")
    assert response.json()["type"] == "urn:derouter:error:no-llm-router"
...
def test_an_unexpected_router_failure_answers_as_a_problem_not_the_openai_error_shape(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "public_model_groups", ["boom"])
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", _Exploding())
    ...
    assert response.headers["content-type"].startswith("application/problem+json")
    assert response.json()["type"] == "urn:derouter:error:internal-server-error"
...
def test_a_facet_is_reachable_without_a_key(facet):
    assert f"{MODEL_HUB_PATH}/{facet}" in DeRouterRoutes.public_routes.value
