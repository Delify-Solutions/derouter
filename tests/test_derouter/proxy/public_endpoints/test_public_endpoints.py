...
from derouter.proxy.public_endpoints import router
...
def test_get_provider_create_fields():
    ...
    assert "derouter_provider" in first_provider
...
def test_get_derouter_model_cost_map_returns_cost_map():
    app = FastAPI()
    app.include_router(router)
    client = TestClient(app)

    response = client.get("/public/derouter_model_cost_map")
...
def test_bedrock_mantle_provider_fields():
    ...
    # bedrock_mantle models; derouter_provider must be the backend slug.
    assert mantle["provider_display_name"] == "Amazon Bedrock Mantle"
    assert mantle["derouter_provider"] == "bedrock_mantle"
...
def test_vllm_provider_display_names_are_distinct():
    ...
    assert hosted["derouter_provider"] == "hosted_vllm"
    assert local["derouter_provider"] == "vllm"
...
def test_nvidia_riva_provider_fields():
    ...
    assert riva["derouter_provider"] == LlmProviders.NVIDIA_RIVA.value
...
def test_cognition_provider_fields():
    ...
    assert cognition["derouter_provider"] == "cognition"
...
def test_chatgpt_provider_fields():
    ...
    assert chatgpt["derouter_provider"] == LlmProviders.CHATGPT.value
...
def test_every_backend_provider_is_listed_in_add_model_or_frozen_as_unlisted():
    ...
    response = test_client.get("/public/providers/fields")
    assert response.status_code == 200
    listed = {p["derouter_provider"] for p in response.json()}
...
def test_google_ai_studio_provider_fields_expose_api_base():
    ...
    assert google_ai["derouter_provider"] == "gemini"
...
def test_public_model_hub_with_healthy_model():
    ...
    with (
        patch("derouter.public_model_groups", ["gpt-3.5-turbo"]),
        patch("derouter.proxy.proxy_server._get_model_group_info") as mock_get_info,
        patch("derouter.proxy.proxy_server.llm_router", mock_llm_router),
        patch("derouter.proxy.proxy_server.prisma_client", mock_prisma),
        patch(
            "derouter.proxy.health_endpoints._health_endpoints._convert_health_check_to_dict"
        ) as mock_convert,
    ):
...
def test_public_model_hub_with_unhealthy_model():
    ...
    with (
        patch("derouter.public_model_groups", ["gpt-4"]),
        patch("derouter.proxy.proxy_server._get_model_group_info") as mock_get_info,
        patch("derouter.proxy.proxy_server.llm_router", mock_llm_router),
        patch("derouter.proxy.proxy_server.prisma_client", mock_prisma),
        patch(
...
def test_public_model_hub_without_health_check():
    ...
    with (
        patch("derouter.public_model_groups", ["claude-3"]),
        patch("derouter.proxy.proxy_server._get_model_group_info") as mock_get_info,
        patch("derouter.proxy.proxy_server.llm_router", mock_llm_router),
        patch("derouter.proxy.proxy_server.prisma_client", mock_prisma),
    ):
...
def test_public_model_hub_mixed_health_statuses():
    ...
    with (
        patch("derouter.public_model_groups", ["gpt-3.5-turbo", "gpt-4", "claude-3"]),
        patch("derouter.proxy.proxy_server._get_model_group_info") as mock_get_info,
        patch("derouter.proxy.proxy_server.llm_router", mock_llm_router),
        patch("derouter.proxy.proxy_server.prisma_client", mock_prisma),
        patch(
...
def test_public_agent_hub_rewrites_upstream_url_to_proxy():
    ...
    with (
        patch("derouter.public_agent_groups", ["agent-123"]),
        patch(
...
def test_public_agent_hub_serializes_http_security_scheme_without_bearer_format():
    ...
    ``securitySchemes.DeRouterKey`` of ``{"type": "http", "scheme": "bearer"}``
        ...
        agent_card_params={
            ...
            "securitySchemes": {
                "DeRouterKey": {
                    "type": "http",
                    "scheme": "bearer",
                    "description": "DeRouter virtual key",
                }
            },
        },
    )
    ...
    payload = response.json()
    assert len(payload) == 1
    scheme = payload[0]["securitySchemes"]["DeRouterKey"]
...
def test_public_agent_hub_returns_empty_when_no_public_groups():
    ...
    with (
        patch("derouter.public_agent_groups", None),
        patch(
            "derouter.proxy.agent_endpoints.agent_registry.global_agent_registry",
            mock_registry,
        ),
...
def test_bedrock_agentcore_runtime_arn_validation_pattern_accepts_full_resource_path():
    ...
    assert arn_field["include_in_derouter_params"] is False
...
import derouter.proxy.public_endpoints.public_endpoints as _pe_module
from derouter.proxy.public_endpoints.public_endpoints import (
    _build_endpoints,
    _clean_display_name,
)
...
def test_get_supported_endpoints_is_cached(reset_endpoints_cache):
    ...
    client = _make_client()
    with patch(
        "derouter.proxy.public_endpoints.public_endpoints._load_endpoints",
...
def test_public_mcp_hub_returns_only_whitelisted_servers():
    ...
    derouter.public_mcp_servers, mirroring /public/model_hub and
    ...
    with (
        patch("derouter.public_mcp_servers", ["listed"]),
        patch(
...
def reset_autorouter_presets_cache():
    from derouter.proxy.public_endpoints.public_endpoints import _AutoRouterPresetsCache
...
def test_get_autorouter_presets_local_mode_serves_bundled_catalog(
    ...
    monkeypatch.setenv("DEROUTER_LOCAL_AUTOROUTER_PRESETS", "True")
    app = FastAPI()
    app.include_router(router)
    client = TestClient(app)
    ...
    assert payload["1m_context"]["complexity_router_config"]["tier_model_configs"] == {
        "REASONING": [{"model_name": "claude-opus-5", "derouter_params": {"reasoning_effort": "high"}}]
    }
...
async def test_get_autorouter_presets_fetches_once_per_process(
    ...
    monkeypatch.delenv("DEROUTER_LOCAL_AUTOROUTER_PRESETS", raising=False)
...
async def test_get_autorouter_presets_caches_bundled_fallback_on_remote_failure(
    ...
    from derouter.proxy.public_endpoints.public_endpoints import get_autorouter_presets
...
async def test_autorouter_presets_adapter_rejects_wrong_shapes():
    ...
    from derouter.proxy.public_endpoints.public_endpoints import _AUTOROUTER_PRESETS_ADAPTER
...
async def test_fetch_remote_autorouter_presets_parses_and_rejects_empty(monkeypatch):
    import derouter.llms.custom_httpx.http_handler as http_handler_module
    from derouter.proxy.public_endpoints.public_endpoints import _fetch_remote_autorouter_presets
