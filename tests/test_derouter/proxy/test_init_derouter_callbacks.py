"""
Unit tests for ProxyLogging._init_derouter_callbacks.

Validates that string callbacks in derouter.callbacks are replaced in-place
with their initialized instances, preventing duplicate entries (string + instance)
that caused double-counting of metrics like derouter_proxy_total_requests_metric.
"""
...
class TestInitLitellmCallbacks:
    """Tests for ProxyLogging._init_derouter_callbacks."""
    ...
    @patch(
        "derouter.proxy.utils.ProxyLogging._add_proxy_hooks",
        new_callable=lambda: lambda self, *a, **kw: None,
    )
    def test_should_replace_string_callback_with_instance(self, _mock_hooks):
        """
        When derouter.callbacks contains a string callback (e.g. "lago"),
        _init_derouter_callbacks should replace the string with the initialized
        CustomLogger instance, not leave both the string and instance in the list.
        """
        fake_logger = FakeCustomLogger()

        # Start with a string callback in derouter.callbacks
        derouter.callbacks = ["lago"]  # type: ignore

        proxy_logging = self._make_proxy_logging()

        with patch(
            "derouter.derouter_core_utils.derouter_logging._init_custom_logger_compatible_class",
        ...
        string_entries = [c for c in derouter.callbacks if isinstance(c, str)]
        instance_entries = [
            c for c in derouter.callbacks if isinstance(c, FakeCustomLogger)
        ]
        ...
        # Clean up
        derouter.callbacks = []  # type: ignore
    ...
    def test_should_not_duplicate_existing_instance_callbacks(self, _mock_hooks):
        """
        When derouter.callbacks already contains a CustomLogger instance (not a string),
        _init_derouter_callbacks should not create a duplicate.
        """
        existing_logger = FakeCustomLogger()

        derouter.callbacks = [existing_logger]  # type: ignore

        proxy_logging = self._make_proxy_logging()

        proxy_logging._init_derouter_callbacks(llm_router=None)

        # Count how many FakeCustomLogger instances are in derouter.callbacks
        instance_count = sum(
            1 for c in derouter.callbacks if isinstance(c, FakeCustomLogger)
        )
        assert instance_count == 1, (
            f"Expected exactly 1 FakeCustomLogger instance, found {instance_count}. "
            f"derouter.callbacks = {derouter.callbacks}"
        )
    ...
    def test_should_handle_unrecognized_string_callback(self, _mock_hooks):
        ...
        the string should remain in derouter.callbacks (not crash).
        """
        derouter.callbacks = ["unknown_callback"]  # type: ignore
        ...
        assert "unknown_callback" in derouter.callbacks
    ...
    def test_should_replace_multiple_string_callbacks(self, _mock_hooks):
        """
        When derouter.callbacks contains multiple string callbacks,
        ...
        derouter.callbacks = ["callback_a", "callback_b"]  # type: ignore
