...
def _patch_proxy_globals(monkeypatch):
    ...
    monkeypatch.setattr(derouter.proxy.proxy_server, "prisma_client", _make_mock_prisma())
...
async def _read_stored_manifest(name: str) -> dict:
    table = derouter.proxy.proxy_server.prisma_client.db.derouter_claudecodeplugintable
