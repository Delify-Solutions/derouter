...
class TestAnthropicEndpoints(unittest.TestCase):
    @patch("derouter.derouter_core_utils.safe_json_dumps.safe_dumps")
    @pytest.mark.asyncio
...
class TestBlockedResponseUsage:
    ...
    def test_uses_original_response_usage(self):
        from derouter.proxy.anthropic_endpoints.endpoints import _blocked_response_usage
    ...
    async def test_blocked_endpoint_response_carries_original_usage(self):
        ...
        import derouter.proxy.anthropic_endpoints.endpoints as ep
...
class TestFailureHookRequestData:
    ...
    async def test_failure_hook_gets_post_setup_data_with_logging_obj(self):
        ...
        async def fake_process(self, **kwargs):
            self.data = {**self.data, "derouter_logging_obj": "logging-obj-sentinel"}
        ...
        assert hook_request_data["derouter_logging_obj"] == "logging-obj-sentinel"
...
class TestStripTotalTokens(unittest.TestCase):
    ...
    DeRouter injects it internally; the helper must remove it from the wire
...
class TestStripTotalTokensFeatureFlag(unittest.TestCase):
    """The strip is gated behind `derouter.strip_anthropic_total_tokens`.

    Default off (backward compat). Greptile P1 on #30382 required a
    user-controlled flag so existing clients reading the DeRouter-shaped
    ...
    def test_flag_defaults_off(self):
        ...
        assert derouter.strip_anthropic_total_tokens is False
