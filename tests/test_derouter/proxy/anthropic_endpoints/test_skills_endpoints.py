...
import derouter
from derouter.llms.derouter_proxy.skills.skill_search import skill_search_text
from derouter.proxy._types import DeRouter_SkillsTable, LitellmUserRoles, UserAPIKeyAuth
from derouter.proxy.anthropic_endpoints.skills_endpoints import router
...
def embedding_router(monkeypatch: pytest.MonkeyPatch, key_limits: MagicMock) -> MagicMock:
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", embedding_router)
...
class TestGetSkillsQuery:
    def test_query_ranks_and_scores_and_truncates(
        ...
        response = _client(LitellmUserRoles.PROXY_ADMIN).get(
            "/v1/skills",
            params={"custom_llm_provider": "derouter_proxy", "query": "language translation", "top_k": 2},
    ...
    def test_restricted_key_only_ranks_the_skills_it_can_access(
        ...
        response = _client(LitellmUserRoles.INTERNAL_USER).get(
            "/v1/skills",
            params={"custom_llm_provider": "derouter_proxy", "query": "language translation"},
            headers={"Authorization": "Bearer k"},
        )
        assert response.status_code == 200
    ...
    def test_no_accessible_skills_is_a_no_match_empty_result(
        ...
        response = _client(LitellmUserRoles.INTERNAL_USER).get(
            "/v1/skills",
            params={"custom_llm_provider": "derouter_proxy", "query": "anything"},
            headers={"Authorization": "Bearer k"},
        )
        assert response.status_code == 200
    ...
    def test_missing_embedding_model_is_a_400(
        ...
        monkeypatch.setattr(derouter, "skill_search_embedding_model", None)
        response = _client(LitellmUserRoles.PROXY_ADMIN).get(
            "/v1/skills",
            params={"custom_llm_provider": "derouter_proxy", "query": "anything"},
            headers={"Authorization": "Bearer k"},
    ...
    def test_top_k_is_validated(self, accessible_skills: AsyncMock, embedding_router: MagicMock) -> None:
        response = _client(LitellmUserRoles.PROXY_ADMIN).get(
            "/v1/skills",
            params={"custom_llm_provider": "derouter_proxy", "query": "anything", "top_k": 0},
            headers={"Authorization": "Bearer k"},
        )
