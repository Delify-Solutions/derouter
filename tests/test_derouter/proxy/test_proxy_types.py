...
def test_audit_log_masking():
    ...
    from derouter.proxy._types import DeRouter_AuditLogs
...
def test_team_membership_null_budget_table():
    """
    Regression test for: DeRouter_TeamMembership.derouter_budget_table missing = None.
    ...
    Related: https://github.com/Delify-Solutions/derouter/issues/28689
    """
    from derouter.proxy._types import DeRouter_TeamMembership

    membership = DeRouter_TeamMembership(user_id="u1", team_id="t1")
    assert membership.derouter_budget_table is None

    membership_explicit = DeRouter_TeamMembership(
        user_id="u1", team_id="t1", derouter_budget_table=None
    )
    assert membership_explicit.derouter_budget_table is None
...
def test_internal_jobs_user_has_proxy_admin_role():
    ...
    Regression test for: https://github.com/Delify-Solutions/derouter/pull/21896
    """
    ...
    system_user = UserAPIKeyAuth.get_derouter_internal_jobs_user_api_key_auth()
...
def test_a_jwt_issuer_must_pick_audience_validation_or_opt_out():
    ...
    from derouter.proxy._types import JWTIssuerConfig
    ...
    with pytest.raises(ValidationError, match="cannot set audience and disable_audience_validation"):
        JWTIssuerConfig(
            ...
            audience="derouter-proxy",
            disable_audience_validation=True,
        )

    assert JWTIssuerConfig(issuer="https://issuer.example.com", audience="derouter-proxy").audience == "derouter-proxy"
    assert (
...
def test_an_organization_member_can_only_take_a_role_the_organization_has():
    ...
    from derouter.proxy._types import LitellmUserRoles, OrganizationMemberUpdateRequest
...
def test_an_llm_backed_injection_check_needs_the_call_it_would_make():
    ...
    for missing in ("llm_api_name", "llm_api_system_prompt", "llm_api_fail_call_string"):
        ...
        with pytest.raises(ValidationError, match=f"{missing} must be provided"):
            DeRouterPromptInjectionParams(llm_api_check=True, **complete)

    assert DeRouterPromptInjectionParams(llm_api_check=False).llm_api_name is None
