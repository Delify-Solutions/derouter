...
from derouter.proxy.image_endpoints import endpoints
...
async def test_image_generation_prompt_rerouting(monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.proxy_config", {})
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.proxy_logging_obj", fake_proxy_logger
    )
    monkeypatch.setattr("derouter.proxy.proxy_server.user_model", None)
    monkeypatch.setattr("derouter.proxy.proxy_server.version", "test-version")
    monkeypatch.setattr(
        "derouter.proxy.common_request_processing.ProxyBaseLLMRequestProcessing.get_custom_headers",
        classmethod(lambda *args, **kwargs: {}),
    )
    monkeypatch.setattr(
        "derouter.proxy.image_endpoints.endpoints.route_request", fake_route_request
    )
...
async def test_a_model_the_router_cannot_serve_answers_an_openai_typed_error(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.user_model", None)
    monkeypatch.setattr("derouter.proxy.proxy_server.version", "test-version")
    monkeypatch.setattr("derouter.proxy.image_endpoints.endpoints.route_request", failing_route_request)
