"""
Test that x-derouter-model-id header is propagated correctly on error responses.
...
def test_maybe_get_model_id_from_derouter_params():
    """
    Test extraction of model_id from logging_obj.derouter_params (used by /v1/chat/completions).
    """
    # Create a ProxyBaseLLMRequestProcessing instance
    processor = ProxyBaseLLMRequestProcessing(data={})

    # Create a mock logging object with model_info in derouter_params
    mock_logging_obj = MagicMock()
    mock_logging_obj.derouter_params = {
        "model_info": {"id": "test-model-id-from-derouter-params"}
    }

    # Test extraction
    model_id = processor.maybe_get_model_id(mock_logging_obj)

    assert model_id == "test-model-id-from-derouter-params"


def test_maybe_get_model_id_from_derouter_params_nested():
    """
    Test extraction of model_id from nested metadata in logging_obj.derouter_params.
    """
    processor = ProxyBaseLLMRequestProcessing(data={})
...
def test_maybe_get_model_id_from_kwargs():
    ...
    mock_logging_obj = MagicMock()
    mock_logging_obj.derouter_params = None
    mock_logging_obj.kwargs = {
        "derouter_params": {"model_info": {"id": "test-model-id-from-kwargs"}}
    }
...
def test_maybe_get_model_id_from_data():
    ...
    processor = ProxyBaseLLMRequestProcessing(
        data={"derouter_metadata": {"model_info": {"id": "test-model-id-from-data"}}}
    )
...
def test_maybe_get_model_id_no_logging_obj():
    ...
    processor = ProxyBaseLLMRequestProcessing(
        data={
            "derouter_metadata": {"model_info": {"id": "test-model-id-no-logging-obj"}}
        }
    )
...
def test_maybe_get_model_id_priority_derouter_params_over_data():
    """
    Test that model_id from logging_obj.derouter_params takes priority over self.data.
    """
    # Create a processor with model_info in both places
    processor = ProxyBaseLLMRequestProcessing(
        data={"derouter_metadata": {"model_info": {"id": "model-id-from-data"}}}
    )

    # Create a mock logging object with model_info
    mock_logging_obj = MagicMock()
    mock_logging_obj.derouter_params = {
        "model_info": {"id": "model-id-from-derouter-params"}
    }

    # Test extraction - should prefer derouter_params
    model_id = processor.maybe_get_model_id(mock_logging_obj)

    assert model_id == "model-id-from-derouter-params"


def test_get_custom_headers_includes_model_id():
    """
    Test that get_custom_headers includes x-derouter-model-id when model_id is provided.
    """
    ...
    assert "x-derouter-model-id" in headers
    assert headers["x-derouter-model-id"] == "test-model-123"
...
def test_get_custom_headers_without_model_id():
    ...
    # x-derouter-model-id should not be in headers (or should be empty/None)
    if "x-derouter-model-id" in headers:
        assert headers["x-derouter-model-id"] in [None, ""]


class _FakeLoggingObj:
    def __init__(self, derouter_params):
        self.derouter_params = derouter_params
        self.derouter_call_id = "test-call-id"
...
def test_get_custom_headers_includes_deployment_model_name(metadata_key):
    """
    x-derouter-model-name should expose the underlying deployment model string,
    which the router records under derouter_params[metadata]["deployment"].
    """
    mock_user_api_key_dict = MagicMock()
    mock_user_api_key_dict.tpm_limit = 1000
    mock_user_api_key_dict.rpm_limit = 100
    ...
    logging_obj = _FakeLoggingObj(
        derouter_params={metadata_key: {"deployment": "azure/gpt-4o-2024-08-06"}}
    )
    ...
    assert headers["x-derouter-model-name"] == "azure/gpt-4o-2024-08-06"
    assert headers["x-derouter-model-id"] == "deployment-uuid"


def test_get_custom_headers_omits_model_name_when_deployment_missing():
    """
    Without a deployment model string, x-derouter-model-name must not be emitted
    ...
    logging_obj = _FakeLoggingObj(derouter_params={"metadata": {}})
    ...
    assert "x-derouter-model-name" not in headers
...
def test_get_custom_headers_with_empty_string_model_id():
    ...
    # x-derouter-model-id should not be in headers (or should be empty)
    if "x-derouter-model-id" in headers:
        assert headers["x-derouter-model-id"] == ""
