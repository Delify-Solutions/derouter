# tests/derouter/proxy/common_utils/test_upsert_budget_membership.py
import types
...
def mock_tx():
    ...
    tx = MagicMock()
    tx.derouter_teammembership = membership
    tx.derouter_budgettable = budget
    return tx
...
def budget_row(**fields):
    """A fake derouter_budgettable row whose model_dump returns the given fields."""
    row = MagicMock()
...
async def test_empty_patch_is_noop(mock_tx, fake_user):
    ...
    mock_tx.derouter_teammembership.update.assert_not_called()
    mock_tx.derouter_teammembership.upsert.assert_not_called()
    mock_tx.derouter_budgettable.update.assert_not_called()
    mock_tx.derouter_budgettable.create.assert_not_called()
...
async def test_clearing_all_limits_disconnects(mock_tx, fake_user):
    mock_tx.derouter_budgettable.find_unique = AsyncMock(
    ...
    mock_tx.derouter_teammembership.update.assert_awaited_once_with(
...
async def test_clear_one_field_keeps_others(mock_tx, fake_user):
    ...
    mock_tx.derouter_teammembership.update.assert_not_called()
    mock_tx.derouter_budgettable.update.assert_awaited_once_with(
...
async def test_update_in_place_seeds_reset_at(mock_tx, fake_user):
    ...
    mock_tx.derouter_budgettable.update.assert_awaited_once()
    call = mock_tx.derouter_budgettable.update.await_args
...
async def test_create_seeds_reset_at_and_links(mock_tx, fake_user):
    ...
    mock_tx.derouter_budgettable.create.assert_awaited_once()
    data = mock_tx.derouter_budgettable.create.await_args.kwargs["data"]
    assert data["budget_duration"] == "7d"
    assert data["created_by"] == fake_user.user_id
    assert data["updated_by"] == fake_user.user_id
    assert_future_reset_time(data["budget_reset_at"])

    new_budget_id = mock_tx.derouter_budgettable.create.return_value.budget_id
    mock_tx.derouter_teammembership.upsert.assert_awaited_once_with(
        where={"user_id_team_id": {"user_id": "user-new", "team_id": "team-new"}},
        data={
            "create": {
                "user_id": "user-new",
                "team_id": "team-new",
                "derouter_budget_table": {"connect": {"budget_id": new_budget_id}},
            },
            "update": {
                "derouter_budget_table": {"connect": {"budget_id": new_budget_id}},
            },
...
async def test_clone_on_write_from_shared_default(mock_tx, fake_user):
    ...
    mock_tx.derouter_budgettable.update.assert_not_called()
    mock_tx.derouter_budgettable.create.assert_awaited_once()
    create_data = mock_tx.derouter_budgettable.create.await_args.kwargs["data"]
