...
from derouter.proxy.common_utils import reset_budget_job as reset_budget_job_module
...
from derouter.proxy.common_utils.timezone_utils import BudgetResetSettings
...
class MockBatcher:
    ...
    def __init__(self):
        ...
        self.derouter_verificationtoken = _Table("key", self)
        self.derouter_usertable = _Table("user", self)
        self.derouter_teamtable = _Table("team", self)
        self.derouter_budgettable = _Table("budget", self)
        self.derouter_teammembership = _Table("team_membership", self)
        self.derouter_organizationtable = _Table("org", self)
        self.derouter_tagtable = _Table("tag", self)
        self.derouter_modelaccessgroupbudgettable = _Table("model_access_group", self)
        self.derouter_endusertable = _Table("enduser", self)
...
class MockDB:
    def __init__(self):
        self.derouter_teammembership = MockTable()
        self.derouter_verificationtoken = MockTable()
        self.derouter_endusertable = MockTable()
        self.derouter_organizationtable = MockTable()
        self.derouter_tagtable = MockTable()
        self.derouter_modelaccessgroupbudgettable = MockTable()
    ...
    async def get_data(self, table_name, query_type, **kwargs):
        ...
        if table_name == "enduser" and query_type == "find_all" and "budget_id_list" in kwargs:
            ...
            return [
                item
                for item in data
                if hasattr(item, "derouter_budget_table")
                and hasattr(item.derouter_budget_table, "budget_id")
                and item.derouter_budget_table.budget_id in budget_id_list
            ]
...
def test_write_key_reset_updates_skips_none_token_and_still_writes_the_rest(reset_budget_job, mock_prisma_client):
    ...
    keys = [
        DeRouter_VerificationToken(token=None, budget_reset_at=reset_at),
        DeRouter_VerificationToken(token="tok-ok", budget_reset_at=reset_at),
    ]
...
def test_reset_budget_for_key(reset_budget_job, mock_prisma_client):
    ...
    # Run the test
    asyncio.run(reset_budget_job.reset_budget_for_derouter_keys())
...
def test_reset_budget_for_key_honors_injected_reset_time(mock_prisma_client, mock_proxy_logging):
    ...
    asyncio.run(job.reset_budget_for_derouter_keys())
...
def test_reset_budget_for_user(reset_budget_job, mock_prisma_client):
    ...
    # Run the test
    asyncio.run(reset_budget_job.reset_budget_for_derouter_users())
...
def test_reset_budget_for_team(reset_budget_job, mock_prisma_client):
    ...
    # Run the test
    asyncio.run(reset_budget_job.reset_budget_for_derouter_teams())
...
def test_reset_budget_for_enduser(reset_budget_job, mock_prisma_client):
    ...
    test_enduser = type(
        ...
        {
            "spend": 20.0,
            "derouter_budget_table": test_budget,
    ...
    asyncio.run(reset_budget_job.reset_budget_for_derouter_budget_table())
...
def test_budget_table_reset_zeroes_spend_on_every_linked_table(
    ...
    double-reset by reset_budget_for_derouter_keys(), and the payload must stay
...
def _run_reset_at_fixed_now(job, fixed_now):
    ...
    with patch("derouter.proxy.common_utils.timezone_utils.datetime") as mock_dt:
        mock_dt.now.return_value = fixed_now
        mock_dt.side_effect = lambda *args, **kwargs: datetime(*args, **kwargs)
        asyncio.run(job.reset_budget_for_derouter_budget_table())
...
def test_reset_budget_resets_endusers_with_null_budget_id(reset_budget_job, mock_prisma_client):
    """
    When derouter.max_end_user_budget_id is configured and that budget is
    ...
    mock_prisma_client.db.derouter_endusertable.set_find_many_results([enduser_no_budget_row])
    ...
    find_many_calls = mock_prisma_client.db.derouter_endusertable.find_many_calls
...
def test_reset_budget_skips_null_budget_id_endusers_when_default_not_configured(reset_budget_job, mock_prisma_client):
    """
    When derouter.max_end_user_budget_id is NOT configured, end users with
...
def test_reset_budget_skips_null_budget_id_endusers_when_default_not_in_reset_list(
    ...
    """
    When derouter.max_end_user_budget_id IS configured but the corresponding
    ...
    now = datetime.now(timezone.utc)
    derouter.max_end_user_budget_id = "default-budget-not-expired"
...
def _make_reset_budget_windows_job(
    ...
    `derouter.proxy.proxy_server` module exposing a stub `spend_counter_cache`.
    ...
    async def fake_query_raw(query: str, *args, **kwargs):
        ...
        if '"DeRouter_VerificationToken"' in query:
            return key_rows
        if '"DeRouter_TeamTable"' in query:
            return team_rows
    ...
    prisma_client.db.derouter_verificationtoken.update = AsyncMock(return_value=None)
    prisma_client.db.derouter_teamtable.update = AsyncMock(return_value=None)

    # Stub out derouter.proxy.proxy_server so the in-function
    # `from derouter.proxy.proxy_server import spend_counter_cache` resolves
    ...
    fake_module = types.ModuleType("derouter.proxy.proxy_server")
    fake_module.spend_counter_cache = spend_counter_cache
    monkeypatch.setitem(sys.modules, "derouter.proxy.proxy_server", fake_module)
...
def test_reset_budget_windows_uses_is_not_null_filter(monkeypatch):
    ...
    assert '"DeRouter_VerificationToken"' in key_query
    assert "budget_limits IS NOT NULL" in key_query
    assert '"DeRouter_TeamTable"' in team_query
...
def test_reset_budget_windows_resets_expired_key_window(monkeypatch):
    ...
    prisma_client.db.derouter_verificationtoken.update.assert_awaited_once()
    call_kwargs = prisma_client.db.derouter_verificationtoken.update.await_args.kwargs
...
def _window_spend_rolls(prisma_client):
    return [
        ...
        if "DeRouter_BudgetWindowSpend" in call.args[0]
    ]
...
def test_reset_budget_windows_rolls_the_key_window_spend_row(monkeypatch):
    ...
    written_windows = json.loads(
        prisma_client.db.derouter_verificationtoken.update.await_args.kwargs["data"]["budget_limits"]
    )
    new_reset_at = datetime.fromisoformat(written_windows[0]["reset_at"].replace("Z", "+00:00")).replace(tzinfo=None)
...
def test_reset_budget_windows_survives_a_failed_window_spend_roll(monkeypatch):
    """The row is an optimization over aggregating DeRouter_SpendLogs; a DB
...
def test_reset_budget_windows_skips_unexpired_key_window(monkeypatch):
    ...
    prisma_client.db.derouter_verificationtoken.update.assert_not_awaited()
...
def test_reset_budget_windows_resets_expired_team_window(monkeypatch):
    ...
    prisma_client.db.derouter_teamtable.update.assert_awaited_once()
    call_kwargs = prisma_client.db.derouter_teamtable.update.await_args.kwargs
...
def _make_counter_invalidation_job(monkeypatch):
    ...
    Both caches are looked up via ``from derouter.proxy.proxy_server import
...
def test_reset_budget_for_proxy_budget_row_invalidates_global_spend_cache(
    ...
    ("derouter-proxy-budget") must also drop the cached global-spend
...
def test_reset_does_not_zero_counter_when_db_write_fails(monkeypatch):
    ...
    batcher = MagicMock()
    batcher.derouter_verificationtoken.update = MagicMock()
...
_INVALIDATION_CASES = [
    ...
    (
        "derouter_modelaccessgroupbudgettable",
...
def test_budget_table_reset_invalidates_every_tag_not_just_the_first(reset_budget_job, mock_prisma_client, monkeypatch):
    ...
    counter_cache = _make_counter_invalidation_job(monkeypatch)
    mock_prisma_client.data["budget"] = [_budget_row(budget_id="budget-1")]
    mock_prisma_client.db.derouter_tagtable.set_find_many_results(
...
def test_budget_table_reset_commits_even_when_cache_eviction_fails(reset_budget_job, mock_prisma_client, monkeypatch):
    ...
    mock_prisma_client.data["budget"] = [_budget_row(budget_id="budget-1")]
    mock_prisma_client.db.derouter_tagtable.set_find_many_results([type("Tag", (), {"tag_name": "tenant-42"})])
...
def _model_access_group_row(name: str = "gpt-4-group", spend: float = 12.0, budget_id: str = "budget-1"):
    """A DeRouter_ModelAccessGroupBudgetTable row, shaped like prisma hands it back."""
...
def test_access_group_reset_only_matches_rows_that_have_spend(reset_budget_job, mock_prisma_client, monkeypatch):
    ...
    mock_prisma_client.db.derouter_modelaccessgroupbudgettable.set_find_many_results(
    ...
    assert mock_prisma_client.db.derouter_modelaccessgroupbudgettable.find_many_calls == [{"where": expected_where}]
...
def test_access_groups_are_untouched_when_no_budget_is_due(reset_budget_job, mock_prisma_client, monkeypatch):
    ...
    counter_cache = _make_counter_invalidation_job(monkeypatch)
    mock_prisma_client.db.derouter_modelaccessgroupbudgettable.set_find_many_results([_model_access_group_row()])

    asyncio.run(reset_budget_job.reset_budget_for_derouter_budget_table())

    assert mock_prisma_client.db.derouter_modelaccessgroupbudgettable.find_many_calls == []
...
class FailingTeamMembershipDB(MockDB):
    ...
    def batch_(self):
        ...
        batcher.derouter_teammembership.update_many = _fail
        return batcher
...
def _job_with_expired_budget(db, proxy_logging=None):
    ...
    db.derouter_tagtable.set_find_many_results([type("Tag", (), {"tag_name": "tenant-42"})])
    job = ResetBudgetJob(
...
def test_budget_reset_at_is_not_advanced_when_the_cascade_fails(db_factory, monkeypatch):
    ...
    asyncio.run(job.reset_budget_for_derouter_budget_table())  # swallowed, retried next tick
...
def test_budget_cascade_writes_land_in_a_single_transaction(reset_budget_job, mock_prisma_client, monkeypatch):
    ...
    mock_prisma_client.data["enduser"] = [
        type("EndUser", (), {"spend": 5.0, "derouter_budget_table": budget, "user_id": "enduser-1", "budget_id": "budget-1"})
    ]
...
def test_failed_cascade_is_logged_as_a_cascade_failure(monkeypatch):
    ...
    with patch("derouter.proxy.common_utils.reset_budget_job.verbose_proxy_logger.exception") as mock_exception:
        asyncio.run(job.reset_budget_for_derouter_budget_table())
...
@pytest.mark.parametrize(
    ...
    [
        ("reset_budget_for_derouter_users", "user", lambda uid: _user_row(uid)),
        ("reset_budget_for_derouter_teams", "team", lambda tid: _team_row(tid)),
    ],
    ids=["users", "teams"],
)
...
def test_key_reset_keeps_paging_when_some_rows_in_a_chunk_fail(monkeypatch):
    ...
    _run_and_drain_hooks(job.reset_budget_for_derouter_keys)
...
@pytest.mark.parametrize(
    ...
    [
        ("reset_budget_for_derouter_users", "user", _user_row, "reset_budget_users"),
        ("reset_budget_for_derouter_teams", "team", _team_row, "reset_budget_teams"),
    ],
    ids=["users", "teams"],
)
...
def test_reset_budget_releases_the_lease_when_a_phase_raises(monkeypatch):
    ...
    monkeypatch.setattr(job, "reset_budget_for_derouter_keys", boom)
...
def _paginating_window_job(monkeypatch, pages_by_table: Dict[str, List[List[Dict[str, Any]]]]):
    ...
    async def fake_query_raw(query: str, *args, **kwargs):
        table = "key" if '"DeRouter_VerificationToken"' in query else "team"
...
def test_reset_budget_windows_survives_one_table_failing(monkeypatch):
    ...
    assert any('"DeRouter_TeamTable"' in sql for sql in queried)
...
@pytest.mark.parametrize(
    ...
    [
        ("reset_budget_for_derouter_keys", "key", "reset_budget_read_keys_failure"),
        ("reset_budget_for_derouter_users", "user", "reset_budget_read_users_failure"),
        ("reset_budget_for_derouter_teams", "team", "reset_budget_read_teams_failure"),
    ],
...
@pytest.mark.parametrize(
    "phase, table_name, reason",
    [
        ("reset_budget_for_derouter_keys", "key", "reset_budget_write_keys_failure"),
        ("reset_budget_for_derouter_users", "user", "reset_budget_write_users_failure"),
        ("reset_budget_for_derouter_teams", "team", "reset_budget_write_teams_failure"),
    ],
    ids=["keys", "users", "teams"],
)
...
def test_transport_error_on_window_read_reconnects_and_still_resets(monkeypatch):
    ...
    async def failing_once_query_raw(query: str, *args, **kwargs):
        if '"DeRouter_VerificationToken"' in query and not reconnect_reasons:
...
def test_connect_error_on_window_write_reconnects_and_writes(monkeypatch):
    ...
    prisma_client.db.derouter_teamtable.update = AsyncMock(side_effect=failing_once_update)
    prisma_client.attempt_db_reconnect = record_reconnect

    asyncio.run(job.reset_budget_windows())

    assert reconnect_reasons == ["reset_budget_write_team_windows_failure"]
    assert prisma_client.db.derouter_teamtable.update.await_count == 2
...
def rollover_enabled(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "budget_rollover", True)
...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda job: job.reset_budget_for_derouter_keys(),
            "key",
            "token",
            "tok-roll",
            lambda now: type(
        ...
        (
            lambda job: job.reset_budget_for_derouter_users(),
            "user",
            "user_id",
            "user-roll",
            lambda now: type(
        ...
        (
            lambda job: job.reset_budget_for_derouter_teams(),
            "team",
            "team_id",
            "team-roll",
            lambda now: type(
...
def test_budget_cascade_carries_overage_per_tier_when_rollover_enabled(
    ...
    mock_prisma_client.db.derouter_teammembership.set_find_many_results([membership])
...
def test_budget_cascade_carries_enduser_overage_when_rollover_enabled(
    ...
    mock_prisma_client.data["enduser"] = [
        type(
            "EndUser",
            (),
            {"spend": 15.0, "derouter_budget_table": budget, "user_id": "enduser-roll", "budget_id": "budget-roll"},
        )
    ]
...
def test_budget_cascade_carries_default_tier_enduser_counter_when_rollover_enabled(
    ...
    counter_cache: Final = _make_counter_invalidation_job(monkeypatch)
    monkeypatch.setattr(derouter, "max_end_user_budget_id", "default-enduser-budget")
    mock_prisma_client.data["budget"] = [
    ...
    mock_prisma_client.db.derouter_endusertable.set_find_many_results([implicit_enduser])
