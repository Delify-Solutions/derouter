...
from derouter.constants import (
    ...
    DEROUTER_EXPIRED_UI_SESSION_KEY_CLEANUP_BATCH_SIZE,
...
class TestExpiredUISessionKeyCleanupManager:
    ...
    async def test_find_expired_ui_session_keys_filters_dashboard_team_and_expiry(self):
        ...
        with patch(
            "derouter.proxy.common_utils.expired_ui_session_key_cleanup_manager.datetime"
        ) as mock_datetime:
        ...
        mock_prisma_client.db.derouter_verificationtoken.find_many.assert_called_once_with(
            ...
            take=DEROUTER_EXPIRED_UI_SESSION_KEY_CLEANUP_BATCH_SIZE,
        )
    ...
    async def test_cleanup_expired_keys_uses_existing_delete_path(self):
        ...
        expired_key = DeRouter_VerificationToken(
            token="expired-dashboard-token",
            team_id=UI_SESSION_TOKEN_TEAM_ID,
        ...
        with patch(
            "derouter.proxy.common_utils.expired_ui_session_key_cleanup_manager.delete_verification_tokens",
            ...
            with patch(
                "derouter.proxy.common_utils.expired_ui_session_key_cleanup_manager.KeyManagementEventHooks.async_key_deleted_hook",
                new_callable=AsyncMock,
        ...
        assert (
            call_kwargs["derouter_changed_by"]
            == DEROUTER_INTERNAL_JOBS_SERVICE_ACCOUNT_NAME
        )
        ...
        assert (
            hook_kwargs["derouter_changed_by"]
            == DEROUTER_INTERNAL_JOBS_SERVICE_ACCOUNT_NAME
        )
