...
def _salt_key(monkeypatch):
    # Dominant convention in the test_derouter/ tree: set the key via env.
    monkeypatch.setenv("DEROUTER_SALT_KEY", "sk-salt-aes-1234")
...
def test_callback_prefix_composes_with_v2(monkeypatch):
    """derouter_enc:: + v2:gcm:... round-trips through the callback read path.

    Callback vars are stored as ``derouter_enc::<helper output>``; the read path
    strips ``derouter_enc::`` then calls the helper, so the value handed to the
