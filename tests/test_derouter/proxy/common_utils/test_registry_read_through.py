...
from derouter.proxy.common_utils.registry_read_through import RegistryReadThrough
...
async def test_get_agent_with_read_through_recovers_agent_created_on_sibling_replica(
    ...
    from derouter.proxy.common_utils.registry_read_through import get_agent_with_read_through
    ...
    prisma_client.db.derouter_agentstable.find_unique.assert_awaited_once_with(
...
async def test_get_agent_with_read_through_recovers_agent_by_name(clean_agent_registry, monkeypatch):
    ...
    prisma_client.db.derouter_agentstable.find_unique.assert_awaited_with(
...
async def test_get_agent_with_read_through_returns_none_for_unknown_agent(clean_agent_registry, monkeypatch):
    ...
    prisma_client: Final = MagicMock()
    prisma_client.db.derouter_agentstable.find_unique = AsyncMock(return_value=None)
    monkeypatch.setattr(proxy_server, "prisma_client", prisma_client)
    monkeypatch.setattr(proxy_server, "store_model_in_db", True)

    assert await get_agent_with_read_through("agent-nobody-created") is None
    assert prisma_client.db.derouter_agentstable.find_unique.await_count == 2
...
async def test_resync_agents_already_registered_skips_db(clean_agent_registry, monkeypatch):
    ...
    import derouter.proxy.proxy_server as proxy_server
    from derouter.proxy.common_utils.registry_read_through import _resync_agents
    ...
    assert await _resync_agents(agent_id) is True
    assert prisma_client.db.derouter_agentstable.find_unique.await_count == 1
...
async def test_get_guardrail_with_read_through_recovers_guardrail_created_on_sibling_replica(monkeypatch):
    ...
    prisma_client: Final = MagicMock()
    prisma_client.db.derouter_guardrailstable.find_first = AsyncMock(
    ...
    try:
        ...
        prisma_client.db.derouter_guardrailstable.find_first.assert_awaited_once_with(
...
async def test_get_guardrail_with_read_through_returns_none_for_unknown_guardrail(monkeypatch):
    ...
    prisma_client: Final = MagicMock()
    prisma_client.db.derouter_guardrailstable.find_first = AsyncMock(return_value=None)
    monkeypatch.setattr(proxy_server, "prisma_client", prisma_client)
    monkeypatch.setattr(proxy_server, "store_model_in_db", True)
...
async def test_resync_guardrails_never_loads_non_active_rows(monkeypatch):
    ...
    import derouter.proxy.proxy_server as proxy_server
    from derouter.proxy.common_utils.registry_read_through import _resync_guardrails
...
async def test_resync_guardrails_syncs_under_guardrail_reconcile_lock(monkeypatch):
    ...
    import derouter.proxy.common_utils.registry_read_through as read_through_module
    import derouter.proxy.proxy_server as proxy_server
    from derouter.proxy.common_utils.registry_read_through import _resync_guardrails
...
async def test_resync_model_deployments_mutates_router_under_model_reconcile_lock(monkeypatch):
    ...
    import derouter.proxy.proxy_server as proxy_server
    from derouter.proxy.common_utils.registry_read_through import _resync_model_deployments

    prisma_client: Final = MagicMock()
    prisma_client.db.derouter_proxymodeltable.find_many = AsyncMock(return_value=[MagicMock()])
...
async def test_resync_model_deployments_loads_db_credentials_before_reconciling_models(
    ...
    prisma_client: Final = MagicMock()
    prisma_client.db.derouter_proxymodeltable.find_many = AsyncMock(return_value=rows)
    router: Final = MagicMock()
    router.get_model_list.return_value = []
...
async def test_resync_model_deployments_respects_supported_db_objects(monkeypatch):
    ...
    prisma_client: Final = MagicMock()
    prisma_client.db.derouter_proxymodeltable.find_many = AsyncMock(
...
async def test_resync_guardrails_respects_supported_db_objects(monkeypatch):
    ...
    prisma_client: Final = MagicMock()
    prisma_client.db.derouter_guardrailstable.find_unique = AsyncMock(
        side_effect=AssertionError("db hit for an object type this replica does not load")
    )
    monkeypatch.setattr(proxy_server, "prisma_client", prisma_client)
    monkeypatch.setattr(proxy_server, "store_model_in_db", True)
...
async def test_resync_agents_waits_for_agent_reload_and_skips_duplicate_registration(clean_agent_registry, monkeypatch):
    ...
    import derouter.proxy.proxy_server as proxy_server
    from derouter.proxy.agent_endpoints.agent_registry import AGENT_RECONCILE_LOCK
    from derouter.proxy.common_utils.registry_read_through import _resync_agents
