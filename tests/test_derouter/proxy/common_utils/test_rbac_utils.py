"""
Tests for derouter/proxy/common_utils/rbac_utils.py
...
from derouter.proxy.common_utils.rbac_utils import (
...
# general_settings is imported from derouter.proxy.proxy_server inside the
# function, so we patch it via patch.dict on the original dict.
_GS_PATH = "derouter.proxy.proxy_server.general_settings"
...
async def test_agents_disabled_team_admin_allowed():
    ...
    with patch.dict(
        ...
        with patch(
            "derouter.proxy.management_endpoints.common_utils._user_has_admin_privileges",
