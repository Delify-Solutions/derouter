...
class TestKeyRotationManager:
    ...
    async def test_should_rotate_key_logic(self):
        ...
        key_no_rotation_time = DeRouter_VerificationToken(
            token="test-token-1",
            auto_rotate=True,
        ...
        key_future_rotation = DeRouter_VerificationToken(
            token="test-token-2",
            auto_rotate=True,
            rotation_interval="30s",
        ...
        key_past_rotation = DeRouter_VerificationToken(
            token="test-token-3",
            auto_rotate=True,
            rotation_interval="30s",
            key_rotation_at=now - timedelta(seconds=10),
        ...
        key_exact_rotation = DeRouter_VerificationToken(
            token="test-token-4",
            auto_rotate=True,
            rotation_interval="30s",
            key_rotation_at=now,
        ...
        key_no_interval = DeRouter_VerificationToken(
            token="test-token-5",
            auto_rotate=True,
    ...
    async def test_find_keys_needing_rotation(self):
        ...
        mock_prisma_client.db.derouter_verificationtoken.find_many.return_value = (
            mock_keys
        )
        ...
        with patch(
            "derouter.proxy.common_utils.key_rotation_manager.datetime"
        ) as mock_datetime:
    ...
    async def test_rotate_key_updates_database(self):
        ...
        # Mock key to rotate
        key_to_rotate = DeRouter_VerificationToken(
            token="old-token",
            auto_rotate=True,
            rotation_interval="30s",
        ...
        call_args = mock_prisma_client.db.derouter_verificationtoken.update.call_args
    ...
    async def test_cleanup_expired_deprecated_keys(self):
        ...
        """
        mock_prisma_client = AsyncMock()
        mock_prisma_client.db.derouter_deprecatedverificationtoken.delete_many.return_value = (
            3
        )
        manager = KeyRotationManager(mock_prisma_client)
        ...
        mock_prisma_client.db.derouter_deprecatedverificationtoken.delete_many.assert_called_once()
        call_args = (
            mock_prisma_client.db.derouter_deprecatedverificationtoken.delete_many.call_args
        )
    ...
    async def test_rotate_key_passes_grace_period(self):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "derouter.proxy.common_utils.key_rotation_manager.DEROUTER_KEY_ROTATION_GRACE_PERIOD",
                    "48h",
                ):
                    await manager._rotate_key(key_to_rotate)
