...
from derouter.proxy.common_utils.key_rotation_manager import KeyRotationManager
...
class TestKeyRotationErrorResilience:
    ...
    async def test_one_key_fails_others_still_rotate(self):
        ...
        key3 = DeRouter_VerificationToken(
            token="token-3",
            auto_rotate=True,
            rotation_interval="30s",
            key_rotation_at=None,
    ...
    async def test_regenerate_key_fn_failure_is_caught(self):
        ...
        with patch(
            "derouter.proxy.common_utils.key_rotation_manager.regenerate_key_fn",
    ...
    async def test_hook_failure_does_not_prevent_db_update(self):
        ...
        with patch(
            ...
            with patch(
                "derouter.proxy.common_utils.key_rotation_manager.KeyManagementEventHooks.async_key_rotated_hook",
                new_callable=AsyncMock,
        ...
        mock_prisma.db.derouter_verificationtoken.update.assert_called_once()
        update_data = mock_prisma.db.derouter_verificationtoken.update.call_args[1][
            "data"
        ]
...
class TestKeyRotationFullFlow:
    ...
    async def test_full_rotation_flow_with_lock(self):
        ...
        # Mock cleanup
        mock_prisma.db.derouter_deprecatedverificationtoken.delete_many.return_value = 1
        # Mock find keys
        mock_prisma.db.derouter_verificationtoken.find_many.return_value = [key]
        ...
        # 2. Cleanup ran
        mock_prisma.db.derouter_deprecatedverificationtoken.delete_many.assert_called_once()

        # 3. Keys were queried
        mock_prisma.db.derouter_verificationtoken.find_many.assert_called_once()

        # 4. DB was updated with new rotation info
        mock_prisma.db.derouter_verificationtoken.update.assert_called_once()
        update_args = mock_prisma.db.derouter_verificationtoken.update.call_args[1]
    ...
    async def test_rotation_count_increments_across_multiple_rotations(self):
        ...
        for expected_count in range(3):
            ...
            mock_prisma.db.derouter_verificationtoken.update.reset_mock()
    ...
    async def test_no_keys_to_rotate_skips_gracefully(self):
        ...
        """
        mock_prisma = AsyncMock()
        mock_prisma.db.derouter_deprecatedverificationtoken.delete_many.return_value = 0
        mock_prisma.db.derouter_verificationtoken.find_many.return_value = []
        ...
        mock_prisma.db.derouter_verificationtoken.update.assert_not_called()
...
class TestKeyRotationInitialization:
    ...
    async def test_lock_pattern_matches_spend_log_cleanup(self):
        ...
        from derouter.constants import KEY_ROTATION_JOB_NAME
...
class TestDeprecatedKeyLookupDbE2E:
    ...
    async def test_deprecated_key_grace_period_cache_hit_path(self):
        ...
        - old key hash resolves through DeRouter_DeprecatedVerificationToken
        ...
        try:
            await prisma_client.db.derouter_verificationtoken.create(
                data={
            ...
            await prisma_client.db.derouter_deprecatedverificationtoken.create(
                data={
            ...
            try:
                await prisma_client.db.derouter_deprecatedverificationtoken.delete_many(
                    where={"token": old_token_hash}
                )
            except Exception:
                pass
            try:
                await prisma_client.db.derouter_verificationtoken.delete_many(
