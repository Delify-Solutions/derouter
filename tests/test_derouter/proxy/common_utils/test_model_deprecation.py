...
class TestCollectModelDeprecations:
            ...
            [
                {
                    ...
                    "derouter_params": {"model": "openai/deprecated-model"},
                    "model_info": {"id": "1"},
                },
                {
                    "model_name": "imminent-alias",
                    "derouter_params": {"model": "imminent-model"},
                    "model_info": {"id": "2"},
                },
                {
                    "model_name": "upcoming-alias",
                    "derouter_params": {"model": "openai/upcoming-model"},
                    "model_info": {"id": "3"},
                },
            ]
        )
    ...
    def test_should_prefer_explicit_deployment_override(self, monkeypatch):
        ...
        router = _make_router(
            [
                {
                    "model_name": "my-alias",
                    "derouter_params": {"model": "some-model"},
                    "model_info": {
                        "id": "x",
    ...
    def test_should_dedupe_duplicate_deployments_in_same_group(self, monkeypatch):
        ...
        router = _make_router(
            [
                {
                    "model_name": "alias",
                    "derouter_params": {"model": "shared-model"},
                    "model_info": {"id": "1"},
                },
                {
                    "model_name": "alias",
    ...
    def test_should_resolve_via_unprefixed_model_name(self, monkeypatch):
        ...
        assert [m.derouter_model for m in snapshot.imminent] == ["gpt-4o"]
    ...
    def test_should_resolve_via_base_model(self, monkeypatch):
        ...
        router = _make_router(
            [
                {
                    "model_name": "alias",
                    "derouter_params": {"model": "azure/some-deployment-name"},
        ...
        assert len(snapshot.imminent) == 1
        assert snapshot.imminent[0].derouter_model == "base-thing"
    ...
    def test_should_render_imminent_and_deprecated_sections(self, monkeypatch):
        ...
        router = _make_router(
            [
                {
                    "model_name": "dead",
                    "derouter_params": {"model": "dead-model"},
                    "model_info": {"id": "1"},
                },
                {
                    "model_name": "soon",
                    "derouter_params": {"model": "soon-model"},
                    "model_info": {"id": "2"},
                },
                {
                    "model_name": "later",
                    "derouter_params": {"model": "later-model"},
                    "model_info": {"id": "3"},
                },
            ]
        )
            ...
            [
                {
                    ...
                    "derouter_params": {"model": "openai/whatever"},
                    "model_info": {
                        "id": "1",
                        "deprecation_date": "2026-06-10",
                        "derouter_provider": "<https://evil.example|openai> & co",
                    },
                }
            ]
        )
