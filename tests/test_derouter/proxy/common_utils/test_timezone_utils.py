...
def test_get_budget_reset_timezone_reads_derouter_attr():
    """
    Test that get_budget_reset_timezone reads from derouter.timezone attribute.
    """
    original = getattr(derouter, "timezone", None)
    try:
        derouter.timezone = "Asia/Tokyo"
        assert get_budget_reset_timezone() == "Asia/Tokyo"
    finally:
        if original is None:
            if hasattr(derouter, "timezone"):
                delattr(derouter, "timezone")
        else:
            derouter.timezone = original


def test_get_budget_reset_timezone_fallback_utc():
    """
    Test that get_budget_reset_timezone falls back to UTC when derouter.timezone is not set.
    """
    original = getattr(derouter, "timezone", None)
    try:
        if hasattr(derouter, "timezone"):
...
def test_get_budget_reset_timezone_fallback_on_none():
    """
    Test that get_budget_reset_timezone falls back to UTC when derouter.timezone is None.
    """
    original = getattr(derouter, "timezone", None)
    try:
        derouter.timezone = None
        assert get_budget_reset_timezone() == "UTC"
    finally:
        if original is None:
            if hasattr(derouter, "timezone"):
...
def test_get_budget_reset_settings_reads_globals():
    orig_tz = getattr(derouter, "timezone", None)
    orig_rt = getattr(derouter, "budget_reset_time", None)
    try:
        derouter.timezone = "Asia/Jerusalem"
        derouter.budget_reset_time = "12:00"
        settings = get_budget_reset_settings()
        assert settings.timezone == "Asia/Jerusalem"
        assert settings.reset_time_of_day == time(12, 0)
    finally:
        _restore_attr(derouter, "timezone", orig_tz)
        _restore_attr(derouter, "budget_reset_time", orig_rt)
