...
class TestKeyRotationManagerPassesKeyAlias:
    ...
    async def test_rotate_key_passes_key_alias_to_regenerate_request(self):
        ...
        mock_prisma.db.derouter_verificationtoken.update = AsyncMock(
        ...
        async def capture_regenerate_key_fn(
            data, user_api_key_dict, derouter_changed_by
        ):
...
class TestKeyRotationSecretNamingStability:
    ...
    async def test_rotation_hook_uses_initial_secret_name_fallback(
        ...
        existing_key = MagicMock(spec=DeRouter_VerificationToken)
        ...
        with patch(
            "derouter.proxy.hooks.key_management_event_hooks.KeyManagementEventHooks._rotate_virtual_key_in_secret_manager",
            new_callable=AsyncMock,
        ) as mock_rotate:
    ...
    async def test_set_key_rotation_fields_requires_alias(self):
        ...
        with patch("derouter._key_management_settings", mock_settings):
