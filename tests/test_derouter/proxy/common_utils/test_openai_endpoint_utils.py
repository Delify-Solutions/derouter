...
@pytest.mark.parametrize(
    ...
    [
        # Test case 1: Empty derouter_params
        (
            {"model_name": "test-model", "derouter_params": {}},
            {"model_name": "test-model", "derouter_params": {}},
        ),
        ...
        (
            ...
            {
                "model_name": "claude-3",
                "derouter_params": {"model": "anthropic/claude-3", "temperature": 0.5},
            },
        ),
...
def test_remove_sensitive_info_from_deployment_with_excluded_keys():
    ...
    assert sanitized_config["derouter_params"]["access_token"] != "token-12345"
    assert "*" in sanitized_config["derouter_params"]["access_token"]

    # With excluded_keys, derouter_credentials_name should NOT be masked.
    ...
    sanitized_config = remove_sensitive_info_from_deployment(
        copy.deepcopy(base_config), excluded_keys={"derouter_credentials_name"}
    )
    assert (
        sanitized_config["derouter_params"]["derouter_credentials_name"]
        == "my-credential-name"
    )
    ...
    assert "api_key" not in sanitized_config["derouter_params"]
