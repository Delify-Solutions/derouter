...
async def test_publish_noops_when_redis_cache_is_none() -> None:
    await publish_config_change(redis_cache=None, object_type="derouter_proxymodeltable")
...
async def test_publish_sends_object_type_json_on_channel() -> None:
    ...
    await publish_config_change(redis_cache=cache, object_type="derouter_proxymodeltable")

    assert len(client.published) == 1
    channel, message = client.published[0]
    assert channel == "derouter_proxy.config_change"
    assert json.loads(message) == {"object_type": "derouter_proxymodeltable"}
...
async def test_publish_uses_namespaced_channel() -> None:
    ...
    await publish_config_change(redis_cache=cache, object_type="derouter_credentialstable")

    assert client.published[0][0] == "prod-eu:derouter_proxy.config_change"
...
async def test_publish_swallows_client_init_errors() -> None:
    await publish_config_change(redis_cache=_ExplodingRedisCache(), object_type="derouter_proxymodeltable")
...
async def test_subscriber_runs_injected_callbacks_in_order_on_message() -> None:
    ...
    subscriber.start()
    pubsub.queue.put_nowait(json.dumps({"object_type": "derouter_proxymodeltable"}))
...
async def test_burst_within_debounce_window_coalesces_into_one_resync() -> None:
    burst = [json.dumps({"object_type": "derouter_proxymodeltable"}) for _ in range(5)]
...
async def test_subscriber_subscribes_on_namespaced_channel_and_resyncs() -> None:
    ...
    assert pubsub.subscribed_channels == ["prod-eu:derouter_proxy.config_change"]
    assert resyncs == ["resync"]
...
async def test_debounce_sleep_adds_jitter_from_injected_rng() -> None:
    pubsub = _QueuePubSub(initial_messages=[json.dumps({"object_type": "derouter_proxymodeltable"})])
    cache = _FakeRedisCache(_ScriptedPubSubRedisClient([pubsub]))
    sleeps: List[float] = []
    fired = asyncio.Event()
...
async def test_redis_error_leads_to_backoff_and_resubscribe() -> None:
    ...
    healthy = _QueuePubSub(initial_messages=[json.dumps({"object_type": "derouter_credentialstable"})])
    cache = _FakeRedisCache(_ScriptedPubSubRedisClient([broken, healthy]))
    resyncs: List[str] = []
    fired = asyncio.Event()
    subscriber = ConfigSyncSubscriber(
...
def test_wrapper_passes_through_unsynced_tables() -> None:
    ...
    wrapped = wrap_table_actions_for_config_sync(actions=actions, table_name="derouter_spendlogs")
...
async def test_wrapper_publishes_table_name_after_write() -> None:
    ...
    assert calls == [("write", "create"), ("publish", "derouter_proxymodeltable")]
...
def test_tool_telemetry_table_writes_pass_through_unwrapped() -> None:
    ...
    wrapped = wrap_table_actions_for_config_sync(actions=actions, table_name="derouter_tooltable")
...
async def test_wrapper_publishes_for_every_write_action(action_name: str) -> None:
    ...
    assert publish_calls == [("publish", "derouter_guardrailstable")]


async def test_model_repository_write_publishes_via_live_coordination_cache() -> None:
    from derouter.proxy import proxy_server
    from derouter.proxy.proxy_server import _set_redis_usage_cache
    from derouter.repositories.model_repository import ModelRepository

    client = _RecordingRedisClient()
    prisma_client = MagicMock()
    prisma_client.db.derouter_proxymodeltable.update = AsyncMock(return_value={"model_id": "m-1"})
    ...
    prisma_client.db.derouter_proxymodeltable.update.assert_awaited_once_with(
...
async def test_evict_config_param_does_not_publish() -> None:
    from derouter.proxy import proxy_server
    from derouter.proxy.proxy_server import _set_redis_usage_cache
    from derouter.proxy.utils import evict_config_param
...
def _reload_config_prisma_client() -> MagicMock:
    ...
    prisma_client.db.derouter_config.find_unique = AsyncMock(return_value=config_record)
    prisma_client.db.derouter_config.upsert = AsyncMock(return_value=config_record)
    prisma_client.db.derouter_config.update_many = AsyncMock(return_value=1)
    return prisma_client


async def test_model_cost_map_reload_does_not_publish_config_change() -> None:
    from derouter.proxy import proxy_server
    from derouter.proxy.proxy_server import ProxyConfig, _set_redis_usage_cache
    ...
    prisma_client.db.derouter_config.update_many.assert_awaited_once()
    assert client.published == []
...
async def test_anthropic_beta_headers_reload_does_not_publish_config_change() -> None:
    ...
    prisma_client.db.derouter_config.upsert.assert_awaited_once()
    assert client.published == []
