...
from derouter.constants import PTU_ROLLUP_JOB_ID, PTU_ROLLUP_LOCK_TTL_SECONDS
from derouter.proxy._types import ScheduledJobStaggerSettings
...
def test_identity_prefers_pod_name_and_separates_workers_on_one_host(monkeypatch):
    monkeypatch.setenv("POD_NAME", "derouter-abc")
    monkeypatch.setenv("HOSTNAME", "derouter-abc")
    identity = resolve_stagger_identity(None)

    assert identity.startswith("derouter-abc:")
    assert identity == f"derouter-abc:{os.getpid()}"

    monkeypatch.delenv("POD_NAME")
    assert resolve_stagger_identity(None).startswith("derouter-abc:")
