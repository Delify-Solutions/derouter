...
from derouter.models.managed_files import DeRouter_ManagedVectorStoresTable
...
class TestCacheCodecRoundTripPreservesNoneFields:
    ...
    def test_managed_vector_store_row_round_trips_with_optional_fields_none(self):
        vs = DeRouter_ManagedVectorStoresTable(
        ...
        wire = CacheCodec.serialize(vs, model_type=DeRouter_ManagedVectorStoresTable)
        assert wire.get("vector_store_name", "MISSING") is None
        assert wire.get("team_id", "MISSING") is None
        restored = CacheCodec.deserialize(wire, model_type=DeRouter_ManagedVectorStoresTable)
        assert restored == vs
