...
def _mock_prisma(row=None, upserted_revision=1):
    ...
    prisma_client.db.derouter_config.find_unique = AsyncMock(return_value=row)
    prisma_client.db.derouter_config.upsert = AsyncMock(return_value=_row(reload_revision=upserted_revision))
    prisma_client.db.derouter_config.update_many = AsyncMock(return_value=1)
    return prisma_client


class _FakeConfigTable:
    """In-memory stand-in for the prisma DeRouter_Config actions, faithful on the parts the
...
def _fake_prisma(table):
    prisma_client = MagicMock()
    prisma_client.db.derouter_config = table
    return prisma_client
...
async def test_write_reload_interval_touches_only_param_value():
    ...
    data = prisma_client.db.derouter_config.upsert.await_args.kwargs["data"]
...
async def test_record_reload_run_updates_last_run_without_creating_or_bumping():
    ...
    kwargs = prisma_client.db.derouter_config.update_many.await_args.kwargs
    assert kwargs == {"data": {"last_run_at": LAST_RUN}, "where": {"param_name": "model_cost_map_reload_config"}}
    prisma_client.db.derouter_config.upsert.assert_not_called()
