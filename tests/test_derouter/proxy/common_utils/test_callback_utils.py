...
from derouter.proxy.common_utils.callback_utils import process_callback
...
def test_get_remaining_tokens_and_requests_from_request_data():
    ...
    casedata = {
        "metadata": {
            ...
            f"derouter-key-remaining-requests-{model_group}": 100,
            f"derouter-key-remaining-tokens-{model_group}": 200,
        }
    }
    ...
    assert headers == {
        f"x-derouter-key-remaining-requests-{expected_name}": 100,
        f"x-derouter-key-remaining-tokens-{expected_name}": 200,
    }
...
def test_add_policy_to_applied_policies_header_uses_derouter_metadata_bucket():
    request_data = {
    ...
    assert request_data["derouter_metadata"]["applied_policies"] == ["global-baseline"]
...
def test_get_logging_caching_headers_merges_metadata_and_derouter_metadata():
    request_data = {
    ...
    assert headers["x-derouter-applied-policies"] == "global-baseline"
    assert headers["x-derouter-applied-guardrails"] == "pii_blocker"
    assert headers["x-derouter-policy-sources"] == "global-baseline=team_default"


def test_add_guardrail_scan_id_dedupes_and_becomes_response_header():
    request_data = {"derouter_metadata": {}}
    ...
    assert request_data["derouter_metadata"]["guardrail_scan_ids"] == ("scan-1", "scan-2")
    assert get_logging_caching_headers(request_data)["x-derouter-guardrail-scan-id"] == "scan-1,scan-2"


def test_get_logging_caching_headers_omits_scan_id_header_without_scans():
    assert "x-derouter-guardrail-scan-id" not in get_logging_caching_headers({"derouter_metadata": {}})
...
def test_initialize_callbacks_on_proxy_instantiates_compression_interception(
    ...
    monkeypatch.setattr(
        "derouter.integrations.compression_interception.handler.CompressionInterceptionLogger.initialize_from_proxy_config",
        lambda derouter_settings, callback_specific_params: dummy_callback,
    )

    original_callbacks = (
        list(derouter.callbacks) if isinstance(derouter.callbacks, list) else []
    )
    derouter.callbacks = []
    try:
        initialize_callbacks_on_proxy(
            ...
            premium_user=False,
            config_file_path=".",
            derouter_settings={"compression_interception_params": {"enabled": True}},
            callback_specific_params={},
        )
        assert dummy_callback in derouter.callbacks
        assert "compression_interception" not in derouter.callbacks
    finally:
...
def test_initialize_callbacks_on_proxy_lakera_ignores_non_dict_callback_settings(
    ...
    ** must be a mapping, not str. The branch now guards on isinstance(dict),
    ...
    fake_lakera = ModuleType("derouter.proxy.guardrails.guardrail_hooks.lakera_ai")
    fake_lakera.lakeraAI_Moderation = _DummyLakera
    monkeypatch.setitem(
        sys.modules,
        "derouter.proxy.guardrails.guardrail_hooks.lakera_ai",
        fake_lakera,
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    try:
        ...
        assert any(isinstance(c, _DummyLakera) for c in derouter.callbacks)
    finally:
        derouter.callbacks = original_callbacks
...
def test_initialize_callbacks_on_proxy_non_dict_callback_specific_params_root(
    ...
    try:
        ...
        assert any(
            isinstance(c, CompressionInterceptionLogger) for c in derouter.callbacks
        )
    finally:
        derouter.callbacks = original_callbacks
...
def probe_config_path(tmp_path):
    ...
    derouter.callbacks members, so an entry left behind here can be read back by an
    ...
    original_callbacks = (
        list(derouter.callbacks) if isinstance(derouter.callbacks, list) else derouter.callbacks
    )
    derouter.callbacks = []
...
def test_initialize_callbacks_on_proxy_rejects_class_valued_entry(probe_config_path):
    ...
    with pytest.raises(ValueError, match='derouter_settings\\.callbacks entry') as exc_info:
...
async def test_initialize_callbacks_on_proxy_instance_entry_runs_pre_call_hook(probe_config_path):
    ...
    assert isinstance(derouter.callbacks[0], CustomLogger)
...
def test_initialize_callbacks_on_proxy_accepts_plain_function_callback(probe_config_path):
    ...
    assert [getattr(cb, "__name__", None) for cb in derouter.callbacks] == ["log_event_fn"]
