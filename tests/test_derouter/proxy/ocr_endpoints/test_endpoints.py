...
(`x-req-format: native | derouter`) and return the provider's native payload.
"""
...
from derouter.proxy.ocr_endpoints.endpoints import _native_response, _parse_ocr_request
...
async def test_should_prefer_body_req_format_over_header():
    ...
    assert (await _parse_ocr_request(request))["req_format"] == "derouter"
...
def test_should_return_native_payload_with_derouter_response_headers():
    fastapi_response = MagicMock()
    fastapi_response.headers = {"x-derouter-response-cost": "0.0015"}
    ...
    assert native.headers["x-derouter-response-cost"] == "0.0015"
