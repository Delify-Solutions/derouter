...
class TestResponsesAPIEndpoints(unittest.TestCase):
    ...
    @patch("derouter.proxy.proxy_server.user_api_key_auth")
    ...
    async def test_cursor_chat_completions_route(self, mock_auth, mock_router):
        ...
        from derouter.types.utils import ResponseOutputMessage, ResponseOutputText
    ...
    async def test_responses_api_key_spend_header_includes_response_cost(self, mock_auth, mock_router):
        ...
        # Verify x-derouter-key-spend header includes current request cost
        assert "x-derouter-key-spend" in response.headers
        key_spend_value = float(response.headers["x-derouter-key-spend"])
        ...
        # Verify x-derouter-response-cost header is present
        assert "x-derouter-response-cost" in response.headers
        response_cost_value = float(response.headers["x-derouter-response-cost"])
...
class TestManagedResponsesWSFirstMessage:
    ...
    async def test_first_message_processed_before_loop(self):
        ...
        """
        from derouter.responses.streaming_iterator import ManagedResponsesWebSocketHandler
...
class TestResponsesWSStreamingFirstMessage:
    ...
    async def test_client_to_backend_replays_first_message(self):
        ...
        """
        from derouter.responses.streaming_iterator import ResponsesWebSocketStreaming
...
class TestWSModelExtraction:
    ...
    def test_flat_format_extracts_model(self):
        from derouter.proxy.response_api_endpoints.endpoints import (
...
class TestResponsesWSFirstFrameModelAuth:
    ...
    async def test_endpoint_enforces_auth_after_model_from_first_frame(self):
        ...
        with (
            patch(
                "derouter.proxy.response_api_endpoints.endpoints._enforce_responses_ws_first_frame_model_auth",
                new_callable=AsyncMock,
            ) as mock_model_auth,
            patch(
                "derouter.proxy.response_api_endpoints.endpoints.ProxyBaseLLMRequestProcessing",
                return_value=processor,
            ),
            patch(
                "derouter.proxy.route_llm_request.route_request",
                new_callable=AsyncMock,
    ...
    async def test_reruns_model_auth_for_first_frame_model(self):
        ...
        with (
            patch(
                "derouter.proxy.auth.user_api_key_auth._enforce_key_and_fallback_model_access",
                new_callable=AsyncMock,
            ) as mock_key_check,
            patch(
            ...
            patch(
                "derouter.proxy.proxy_server.llm_model_list",
                [],
            ),
            patch("derouter.proxy.proxy_server.master_key", "sk-test"),
            patch("derouter.proxy.proxy_server.user_custom_auth", None),
...
def test_cursor_chat_completions_messages_body_uses_chat_pipeline():
    ...
    mock_router.acompletion = AsyncMock(
        return_value=derouter.ModelResponse(
...
def test_cursor_chat_completions_input_body_uses_responses_pipeline_and_strips_stream_options():
    ...
    try:
        with (
            ...
            patch(
                "derouter.proxy.response_api_endpoints.endpoints._read_request_body",
...
class TestNestFlatChatTools:
    def test_flat_custom_tool_is_nested(self):
        from derouter.proxy.response_api_endpoints.endpoints import _convert_tool_envelope
...
class TestCursorMessagesArmToolNormalization:
    ...
    async def test_flat_custom_tool_nested_before_chat_completion_delegation(self):
        ...
        try:
            with patch("derouter.proxy.proxy_server.chat_completion", new=fake_chat_completion):
                client = TestClient(app)
                response = client.post(
                    "/cursor/chat/completions",
                    json={
...
class TestNormalizeToolDialectCoversBothFields:
    ...
    def test_tools_and_tool_choice_convert_together(self, to_chat):
        from derouter.proxy.response_api_endpoints.endpoints import _normalize_tool_dialect
...
class TestCursorInputArmFlattening:
    ...
    async def test_nested_chat_shapes_in_input_body_reach_aresponses_flattened(self):
        ...
        app.dependency_overrides[user_api_key_auth] = lambda: UserAPIKeyAuth(api_key="sk-1234")
        try:
            with patch("derouter.proxy.proxy_server.llm_router") as mock_router:
                mock_router.aresponses = AsyncMock(return_value=mock_response)
                client = TestClient(app)
                response = client.post(
                    "/cursor/chat/completions",
...
class TestChatCompletionsBodyDetection:
    def test_routing_matrix(self):
        from derouter.proxy.response_api_endpoints.endpoints import _is_chat_completions_body
...
class TestParseCursorModelVariant:
    ...
    def test_parse_matrix(self, model, expected_base, expected_effort):
        from derouter.proxy.response_api_endpoints.endpoints import _parse_cursor_model_variant
...
class TestResolveCursorModelVariant:
    ...
    def wildcard_router(self):
        ...
        return Router(
            model_list=[
                {"model_name": "anthropic/*", "derouter_params": {"model": "anthropic/*", "api_key": "fake"}},
                {"model_name": "openai/*", "derouter_params": {"model": "openai/*", "api_key": "fake"}},
                {
                    "model_name": "explicit-alias-thinking-high",
                    "derouter_params": {"model": "anthropic/claude-opus-5", "api_key": "fake"},
                },
            ]
        )

    def test_chat_body_suffix_stripped_into_reasoning_effort(self, wildcard_router):
        from derouter.proxy.response_api_endpoints.endpoints import _resolve_cursor_model_variant
...
class TestCursorModelSuffixResolutionEndToEnd:
    ...
    async def test_chat_arm_rewrites_suffixed_model_before_delegation(self):
        ...
        try:
            with (
                patch("derouter.proxy.proxy_server.llm_router", new=_router_serving_only("claude-opus-5")),
                patch("derouter.proxy.proxy_server.chat_completion", new=fake_chat_completion),
            ):
                client = TestClient(app)
                response = client.post(
                    "/cursor/chat/completions",
    ...
    async def test_responses_arm_rewrites_suffixed_model_before_routing(self):
        ...
        app.dependency_overrides[user_api_key_auth] = lambda: UserAPIKeyAuth(api_key="sk-1234")
        try:
            with patch("derouter.proxy.proxy_server.llm_router", new=mock_router):
                client = TestClient(app)
                response = client.post(
                    "/cursor/chat/completions",
                    json={
...
def _cursor_budget_auth_env(base_model: str, spend: float):
    ...
    router = Router(
        model_list=[{"model_name": "anthropic/*", "derouter_params": {"model": "anthropic/*", "api_key": "fake"}}]
    )
...
def _post_cursor_with_real_auth(valid_token, proxy_server_attrs, request_model: str):
    with (
        patch.multiple("derouter.proxy.proxy_server", **proxy_server_attrs),
        patch(
...
class TestCursorVariantResolvedBeforeAuth:
    ...
    def _run_with_recording_auth(self, mock_router, request_model: str):
        ...
        try:
            with (
                patch("derouter.proxy.proxy_server.llm_router", new=mock_router),
                patch("derouter.proxy.proxy_server.chat_completion", new=fake_chat_completion),
            ):
                client = TestClient(app)
                response = client.post(
...
class TestCursorGateRecognizesRoutingGroups:
    def test_group_name_variant_is_not_mangled(self):
        ...
        router = Router(
            model_list=[{"model_name": "member-fast", "derouter_params": {"model": "openai/gpt-4o", "api_key": "fake"}}],
            routing_groups=[
...
class TestGuardrailBlockedResponsesUsage:
    """Regression tests for https://github.com/Delify-Solutions/derouter/issues/36880.
    ...
    def _post_blocked_responses(self, original_response):
        ...
        try:
            with (
                patch(
                    "derouter.proxy.response_api_endpoints.endpoints.ProxyBaseLLMRequestProcessing.base_process_llm_request",
    ...
    def test_post_call_block_maps_bridged_chat_usage(self):
        ...
        original.usage = derouter.Usage(prompt_tokens=14, completion_tokens=18, total_tokens=32)
...
class TestResponsesInputTokens:
    ...
    def _post_input_tokens(
        ...
        from derouter.proxy._types import UserAPIKeyAuth
        from derouter.proxy.auth.user_api_key_auth import user_api_key_auth
        from derouter.proxy.response_api_endpoints.endpoints import _proxy_token_counter
