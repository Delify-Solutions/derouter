...
only) and the flush that writes DeRouter_SpendLogToolIndex plus the
DeRouter_DailyToolSpend rollup in one transaction.
"""
...
class _FakeBatcher:
    def __init__(self) -> None:
        self.derouter_spendlogtoolindex = MagicMock()
        self.derouter_dailytoolspend = MagicMock()
...
class TestBuildToolUsageTransaction:
    ...
    def test_date_matches_daily_spend_writer_derivation(self):
        ...
        # in UTC but must bucket as 2026-07-25 to match DeRouter_DailyUserSpend.
...
class TestFlushToolUsageTransactions:
    ...
    async def test_multi_tool_request_attributes_full_spend_to_each_tool(self):
        ...
        index_rows = batcher.derouter_spendlogtoolindex.create_many.call_args.kwargs["data"]
        assert [(r["request_id"], r["tool_name"]) for r in index_rows] == [("r1", "tool_a"), ("r1", "tool_b")]
        assert batcher.derouter_spendlogtoolindex.create_many.call_args.kwargs["skip_duplicates"] is True

        upserts = {
            c.kwargs["where"]["date_tool_name"]["tool_name"]: c.kwargs["data"]
            for c in batcher.derouter_dailytoolspend.upsert.call_args_list
        }
    ...
    async def test_same_day_same_tool_aggregates_within_batch(self):
        ...
        assert batcher.derouter_dailytoolspend.upsert.call_count == 1
        data = batcher.derouter_dailytoolspend.upsert.call_args.kwargs["data"]
        assert data["create"] == {
            "date": "2026-07-25",
            "tool_name": "tool_a",
    ...
    async def test_index_rows_and_rollup_share_one_transaction(self):
        ...
        batcher.derouter_spendlogtoolindex.create_many.assert_called_once()
        batcher.derouter_dailytoolspend.upsert.assert_called_once()
    ...
    async def test_connection_errors_retry_and_succeed(self, monkeypatch):
        ...
        monkeypatch.setattr("derouter.proxy.db.spend_log_tool_index.asyncio.sleep", fake_sleep)
