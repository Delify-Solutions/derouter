...
from derouter.proxy.db.prisma_client import PrismaManager, PrismaWrapper, should_update_prisma_schema
...
def test_db_push_applies_replica_identity_full_when_requested(monkeypatch, fake_prisma_cli, unset_database_url):
    """`prisma db push` bypasses derouter-proxy-extras, so it needs its own call
    into the opt-in REPLICA IDENTITY FULL step."""
    from derouter.proxy.db.prisma_client import PrismaManager
    from derouter_proxy_extras.replica_identity import REPLICA_IDENTITY_FULL_ENV_VAR
    from derouter_proxy_extras.utils import ProxyExtrasDBManager
...
def test_azure_entra_mint_writes_an_encoded_url_into_the_db_url_env_var(azure_env):
    ...
    assert db_url == (
        f"postgresql://derouter%40contoso.onmicrosoft.com:{urllib.parse.quote(token, safe='')}"
        "@pg.postgres.database.azure.com:5432/derouter_db"
    )
...
def test_token_refresh_keeps_the_connection_params_of_the_url_it_replaces(
    ...
    monkeypatch.setenv(
        "DATABASE_URL",
        f"postgresql://derouter%40contoso.onmicrosoft.com:{urllib.parse.quote(old_token, safe='')}"
        f"@pg.postgres.database.azure.com:5432/derouter_db?{previous_query}",
    )
...
def test_token_refresh_keeps_the_reader_url_params_separate_from_the_writer(azure_env, monkeypatch):
    from derouter.proxy.db.token_auth import IAMEndpoint

    monkeypatch.setenv("DATABASE_URL", "postgresql://w:t@pg:5432/derouter_db?max_idle_connection_lifetime=45")
    monkeypatch.setenv(
        "DATABASE_URL_READ_REPLICA", "postgresql://r:t@replica:5432/derouter_db?max_idle_connection_lifetime=60"
    )
    reader = _azure_wrapper(
        _entra_jwt(3600),
        db_url_env_var="DATABASE_URL_READ_REPLICA",
        iam_endpoint=IAMEndpoint(host="replica", port="5432", user="r", name="derouter_db", schema=None),
    )
...
def test_db_push_timeout_takes_its_process_tree_with_it(fake_prisma_cli, unset_database_url, monkeypatch):
    ...
    """
    monkeypatch.delenv("DEROUTER_SET_REPLICA_IDENTITY_FULL", raising=False)
...
def test_db_push_without_the_prisma_runner_fails_the_migration_instead_of_crashing_boot(
    ...
    """
    monkeypatch.setitem(sys.modules, "derouter_proxy_extras.prisma_toolchain", None)
