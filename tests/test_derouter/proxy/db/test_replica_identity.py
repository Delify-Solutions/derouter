...
def test_hands_the_alter_statement_to_the_prisma_cli():
    ...
    with patch(
        "derouter_proxy_extras.replica_identity.run_prisma", side_effect=capture
    ):
    ...
    assert r"c.relname LIKE 'DeRouter\_%'" in sql
...
def test_every_failure_is_reported_instead_of_raised(failure):
    with patch(
        "derouter_proxy_extras.replica_identity.run_prisma", side_effect=failure
    ):
        assert (
            apply_replica_identity_full(
                schema_path="/somewhere/schema.prisma",
...
def test_an_unusable_migrations_dir_skips_the_step_instead_of_killing_the_run(
    ...
    """DEROUTER_MIGRATION_DIR makes the step copy the migrations tree before it
    ...
    monkeypatch.setenv("DEROUTER_MIGRATION_DIR", str(blocker / "migrations"))
