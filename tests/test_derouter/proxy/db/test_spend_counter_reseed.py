...
The maintained ``DeRouter_BudgetWindowSpend`` row replaces a per-request
``DeRouter_SpendLogs`` range scan, so these pin *when* the aggregate is still
...
class _FakePrismaClient:
    def __init__(
        ...
        self.db = SimpleNamespace(
            derouter_budgetwindowspend=_FakeFindUniqueTable(row=row, error=error),
            derouter_spendlogs=_FakeSpendLogsTable(total=spend_logs_total),
            derouter_endusertable=_FakeFindUniqueTable(row=end_user_row, error=end_user_error),
        )
...
async def test_window_from_table_reads_row_by_primary_key():
    ...
    assert result == 4.5
    assert prisma.db.derouter_budgetwindowspend.where_clauses == [
        {
...
async def test_window_from_table_maps_team_entity_type():
    ...
    assert result == 9.0
    inner = prisma.db.derouter_budgetwindowspend.where_clauses[0]["entity_type_entity_id_window_duration"]
...
async def test_window_from_db_prefers_the_row_over_the_spend_logs_aggregate():
    ...
    assert result == 4.5
    assert prisma.db.derouter_spendlogs.call_count == 0
...
async def test_window_from_db_falls_back_to_spend_logs(row):
    ...
    assert result == 7.25
    assert prisma.db.derouter_spendlogs.call_count == 1
...
async def test_end_user_from_db_reads_the_end_user_row_by_user_id():
    ...
    assert result == 0.0
    assert prisma.db.derouter_endusertable.where_clauses == [{"user_id": "customer-42"}]
...
async def test_end_user_from_db_ignores_other_counter_kinds_without_touching_the_db(counter_key):
    ...
    assert prisma.db.derouter_endusertable.where_clauses == []
