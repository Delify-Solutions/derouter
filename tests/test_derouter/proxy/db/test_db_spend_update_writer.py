...
import derouter
from derouter.proxy._types import Litellm_EntityType
...
async def test_daily_spend_tracking_with_disabled_spend_logs():
    ...
    with (
        patch("derouter.proxy.proxy_server.disable_spend_logs", True),
        patch("derouter.proxy.proxy_server.prisma_client", MagicMock()),
        patch("derouter.proxy.proxy_server.user_api_key_cache", MagicMock()),
        patch("derouter.proxy.proxy_server.derouter_proxy_budget_name", "test-budget"),
    ):
        # Test data
        test_data = {
            "token": "test-token",
...
async def test_update_database_enqueues_tool_usage_for_invoked_tools():
    ...
    with (
        patch("derouter.proxy.proxy_server.disable_spend_logs", False),
        patch("derouter.proxy.proxy_server.prisma_client", prisma),
        patch("derouter.proxy.proxy_server.derouter_proxy_budget_name", "test-budget"),
        patch(
            "derouter.proxy.spend_tracking.spend_tracking_utils.get_logging_payload",
...
async def test_update_daily_spend_retries_connect_errors(monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.db.db_spend_update_writer.asyncio.sleep", fake_sleep)
    await DBSpendUpdateWriter._update_daily_spend(
        n_retry_times=3,
        prisma_client=prisma_client,
        proxy_logging_obj=proxy_logging,
...
async def test_update_daily_spend_tag_with_request_id():
    ...
    assert '"request_id" = COALESCE(EXCLUDED."request_id", "DeRouter_DailyTagSpend"."request_id")' in sql
...
async def test_update_tag_db_with_valid_tags():
    ...
    """
    from derouter.proxy._types import Litellm_EntityType, SpendUpdateQueueItem
...
async def test_commit_spend_updates_to_db_increments_agent_spend():
    """
    Test that _commit_spend_updates_to_db calls derouter_agentstable.update_many
    with spend increment when agent_list_transactions is present.
    """
    db_writer = DBSpendUpdateWriter()

    mock_batcher = MagicMock()
    mock_batcher.derouter_verificationtoken = MagicMock()
    mock_batcher.derouter_verificationtoken.update_many = MagicMock()
    mock_batcher.derouter_usertable = MagicMock()
    mock_batcher.derouter_usertable.update_many = MagicMock()
    mock_batcher.derouter_teamtable = MagicMock()
    mock_batcher.derouter_teamtable.update_many = MagicMock()
    mock_batcher.derouter_teammembership = MagicMock()
    mock_batcher.derouter_teammembership.update_many = MagicMock()
    mock_batcher.derouter_organizationtable = MagicMock()
    mock_batcher.derouter_organizationtable.update_many = MagicMock()
    mock_batcher.derouter_tagtable = MagicMock()
    mock_batcher.derouter_tagtable.update_many = MagicMock()
    mock_batcher.derouter_agentstable = MagicMock()
    mock_batcher.derouter_agentstable.update_many = MagicMock()
    ...
    with patch("derouter.proxy.utils._raise_failed_update_spend_exception"):
    ...
    mock_batcher.derouter_agentstable.update_many.assert_called_once()
    call_kwargs = mock_batcher.derouter_agentstable.update_many.call_args[1]
...
async def test_commit_spend_updates_to_db_increments_team_member_spend_and_total_spend():
    ...
    and total_spend (non-resetting) on DeRouter_TeamMembership in a single
    ...
    mock_batcher.derouter_teammembership.update_many.assert_called_once()
    call_kwargs = mock_batcher.derouter_teammembership.update_many.call_args[1]
...
async def test_add_spend_log_transaction_to_daily_tag_transaction_with_request_id():
    ...
    This tests that request_id is included in the DailyTagSpendTransaction for the DeRouter_DailyTagSpend table.
    """
    writer = DBSpendUpdateWriter()
    mock_prisma = MagicMock()
...
async def test_update_daily_spend_logs_detailed_error_on_batch_upsert_failure():
    ...
    with patch.object(verbose_proxy_logger, "error") as mock_error_logger:
        ...
        assert "Table: DeRouter_DailyUserSpend" in formatted
...
async def test_commit_key_spend_updates_includes_last_active():
    ...
    mock_batcher.derouter_verificationtoken.update_many.assert_called_once()
    call_kwargs = mock_batcher.derouter_verificationtoken.update_many.call_args[1]
...
async def test_update_database_creates_single_task():
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.user_api_key_cache", MagicMock()),
        patch("derouter.proxy.proxy_server.derouter_proxy_budget_name", "test-budget"),
        patch("derouter.proxy.db.db_spend_update_writer.asyncio.create_task") as mock_create_task,
    ):
        await db_writer.update_database(
            token="test-token",
            user_id="test-user",
...
async def test_batch_database_updates_isolation_on_failure():
    ...
    await db_writer._batch_database_updates(
        ...
        derouter_proxy_budget_name="budget",
        payload={"key": "value"},
    )
...
async def test_update_database_does_not_deepcopy_on_request_path():
    ...
    with (
        ...
        patch(
            "derouter.proxy.db.db_spend_update_writer.copy.deepcopy",
            counting_deepcopy,
        ),
    ):
        await db_writer.update_database(
...
async def test_spend_update_path_never_queries_user_cache_with_none_user_id():
    ...
    with (
        patch.object(derouter, "max_budget", 0),
        patch("derouter.proxy.proxy_server.disable_spend_logs", True),
        patch("derouter.proxy.proxy_server.prisma_client", MagicMock()),
        patch("derouter.proxy.proxy_server.user_api_key_cache", strict_redis_backed_cache),
        patch("derouter.proxy.proxy_server.derouter_proxy_budget_name", "derouter-proxy-budget"),
        patch(
            "derouter.proxy.spend_tracking.spend_tracking_utils.get_logging_payload",
            return_value={
                "startTime": "2024-01-01T00:00:00",
...
async def test_update_user_db_enqueues_user_spend_without_cache_dependency():
    ...
    with patch.object(derouter, "max_budget", 0):
...
async def test_daily_transaction_carries_compression_saved_tokens():
    ...
    metadata = {
        ...
        "derouter_gateway_injected_cache": "dep-of-the-compression-row",
        "compression_savings": {
            "tokens_before": 12000,
            "tokens_after": 5000,
            "tokens_saved": 7000,
    ...
    model_info = derouter.get_model_info(model="claude-sonnet-5", custom_llm_provider="anthropic")
...
# ---------------------------------------------------------------------------
# Budget window spend flush (DeRouter_BudgetWindowSpend)
# ---------------------------------------------------------------------------
...
class _WindowSpendFakeDB:
    ...
    async def query_raw(self, query, *args):
        ...
        if "DeRouter_BudgetWindowSpend" in query:
...
def _window_spend_upserts(db):
    return [params for query, params in db.batcher.calls if "DeRouter_BudgetWindowSpend" in query]
...
async def test_failed_window_spend_commit_requeues_the_increments_and_continues_the_flush():
    ...
    against DeRouter_SpendLogs, so a dropped increment would let the key spend
...
def _deadlock_error():
    ...
    return RawQueryError(
        data={"user_facing_error": {"error_code": "P2034", "meta": {"table": "DeRouter_VerificationToken"}}}
    )
...
async def test_commit_spend_updates_retries_deadlock_then_commits(monkeypatch):
    ...
    slept = []
    monkeypatch.setattr(
        "derouter.proxy.db.db_spend_update_writer.asyncio.sleep",
...
async def test_commit_spend_updates_raises_after_exhausting_deadlock_retries(monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.db.db_spend_update_writer.asyncio.sleep", AsyncMock(return_value=None))
...
async def test_commit_spend_updates_does_not_retry_non_deadlock_data_error(monkeypatch):
    ...
    non_deadlock = UniqueViolationError(
        data={"user_facing_error": {"error_code": "P2002", "meta": {"table": "DeRouter_VerificationToken"}}}
    )
    mock_prisma_client = MagicMock()
...
def _spend_logs_prisma(inserted: int, existing: object, taken_over: int = 1) -> MagicMock:
    ...
    prisma.db.derouter_spendlogs.create_many = AsyncMock(return_value=inserted)
    prisma.db.derouter_spendlogs.find_unique = AsyncMock(return_value=existing)
    prisma.db.derouter_spendlogs.update_many = AsyncMock(return_value=taken_over)
    return prisma
...
async def _update_database_with(
    ...
    with (
        patch(  # test-quality-ok: update_database reads this proxy_server global at call time, no seam
            "derouter.proxy.proxy_server.disable_spend_logs", disable_spend_logs
        ),
        patch(  # test-quality-ok: update_database reads this proxy_server global at call time, no seam
            "derouter.proxy.proxy_server.prisma_client", prisma
        ),
        patch(  # test-quality-ok: update_database reads this proxy_server global at call time, no seam
            "derouter.proxy.proxy_server.derouter_proxy_budget_name", "test-budget"
        ),
...
async def test_update_database_charges_a_batch_only_from_the_retrieve_that_wrote_its_row(
    ...
    claimed_rows = prisma.db.derouter_spendlogs.create_many.await_args.kwargs
...
async def test_update_database_charges_a_batch_whose_row_a_pre_upgrade_poll_left_at_zero(
    ...
    taken = prisma.db.derouter_spendlogs.update_many.await_args.kwargs
    assert taken["where"] == {
        "request_id": "batch_abc_batch_cost",
        "call_type": "aretrieve_batch",
        "status": "success",
...
async def test_update_database_leaves_a_batch_whose_zero_row_it_could_not_take_over_to_the_next_retrieve():
    ...
    prisma.db.derouter_spendlogs.update_many = AsyncMock(side_effect=RuntimeError("db unreachable"))
...
async def test_update_database_leaves_a_batch_that_cost_nothing_to_the_retrieve_that_wrote_its_row():
    ...
    prisma.db.derouter_spendlogs.update_many.assert_not_called()
    assert db_writer._batch_database_updates.await_count == 0
...
async def test_update_database_charges_a_batch_once_even_with_spend_logs_disabled(
    ...
    assert prisma.db.derouter_spendlogs.create_many.await_count == 1
    assert db_writer._batch_database_updates.await_count == (1 if charged else 0)
...
async def test_update_database_writes_no_ordinary_spend_row_with_spend_logs_disabled():
    ...
    prisma.db.derouter_spendlogs.create_many.assert_not_called()
    assert prisma.spend_log_transactions == []
...
def _logged_batch_cost_payload() -> dict:
    return {
        **_batch_cost_payload(),
        ...
        "proxy_server_request": '{"headers": {"user-agent": "derouter-batch-cost-check"}}',
    }
...
async def test_update_database_claims_a_batch_without_logging_the_request_that_polled_it(
    ...
    claimed = prisma.db.derouter_spendlogs.create_many.await_args.kwargs["data"][0]
...
async def test_update_database_queues_only_the_claim_for_a_batch_it_could_not_write_with_logs_disabled():
    ...
    prisma.db.derouter_spendlogs.create_many = AsyncMock(side_effect=RuntimeError("db unreachable"))
...
async def test_caching_savings_are_attributed_to_the_deployment_that_was_injected(injected_deployment, attributed):
    ...
    bucket and one derouter_call_id, so a marker written by the leg that injected is
        ...
        "metadata": json.dumps(
            {
                ...
                "derouter_gateway_injected_cache": injected_deployment,
            }
        ),
    }
...
async def test_daily_transaction_attributes_caching_savings_only_with_an_injection_marker():
    """Cached usage with no derouter_gateway_injected_cache marker is still a real saving.
    ...
    empty, which is what separates what caching saved from what derouter can claim.
    """
    writer = DBSpendUpdateWriter()
    mock_prisma = MagicMock()
    mock_prisma.get_request_status = MagicMock(return_value="success")
