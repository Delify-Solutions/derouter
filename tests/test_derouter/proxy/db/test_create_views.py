...
async def test_create_views_reraises_connection_error():
    ...
    from derouter.proxy.db.create_views import create_missing_views
...
async def test_create_views_reraises_permission_error():
    ...
    mock_db.query_raw = AsyncMock(
        side_effect=Exception(
            "permission denied for table DeRouter_VerificationTokenView"
        )
    )
    mock_db.execute_raw = AsyncMock()
...
async def test_create_views_creates_view_on_does_not_exist():
    ...
    mock_db.query_raw = AsyncMock(
        side_effect=[
            Exception('relation "DeRouter_VerificationTokenView" does not exist'),
    ...
    assert 'CREATE VIEW "DeRouter_VerificationTokenView"' in created_sql
...
async def test_create_views_creates_view_on_undefined_error():
    ...
    mock_db.query_raw = AsyncMock(
        side_effect=[
            Exception("undefined table DeRouter_VerificationTokenView"),
            None,
            None,
            None,
            None,
...
async def test_should_create_missing_views_reltuples_zero():
    ...
    from derouter.proxy.db.create_views import should_create_missing_views
...
async def test_create_view_tolerating_race_swallows_only_already_exists():
    from derouter.proxy.db.create_views import create_view_tolerating_race
