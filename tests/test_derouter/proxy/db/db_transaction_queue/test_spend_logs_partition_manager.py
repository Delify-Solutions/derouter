...
from derouter.proxy.db.db_transaction_queue.spend_logs_partition_manager import (
...
def test_partition_name_uses_period_start_date():
    assert partition_name(date(2026, 6, 1)) == "DeRouter_SpendLogs_p20260601"
...
def test_upcoming_partitions_count_and_contiguous_ranges():
    ...
    assert names == [
        "DeRouter_SpendLogs_p20260601",
        "DeRouter_SpendLogs_p20260602",
        "DeRouter_SpendLogs_p20260603",
        "DeRouter_SpendLogs_p20260604",
    ]
...
async def test_drop_partitions_older_than_drops_expired_only():
    ...
    client.db.query_raw = AsyncMock(
        return_value=[
            {
                "name": "DeRouter_SpendLogs_p20260601",
                "bound": "FOR VALUES FROM ('2026-06-01 00:00:00') TO ('2026-06-02 00:00:00')",
            },
            {
                "name": "DeRouter_SpendLogs_p20260609",
                "bound": "FOR VALUES FROM ('2026-06-09 00:00:00') TO ('2026-06-10 00:00:00')",
            },
            {"name": "DeRouter_SpendLogs_pdefault", "bound": "DEFAULT"},
        ]
    )
    ...
    assert dropped == ["DeRouter_SpendLogs_p20260601"]
    executed = " ".join(call.args[0] for call in client.db.execute_raw.call_args_list)
    assert 'DROP TABLE IF EXISTS "DeRouter_SpendLogs_p20260601"' in executed
...
async def test_ensure_partitions_issues_create_for_each_period():
    ...
    assert 'PARTITION OF "DeRouter_SpendLogs"' in first_sql
...
async def test_drop_partitions_continues_when_one_drop_fails():
    ...
    client.db.query_raw = AsyncMock(
        return_value=[
            ...
            {
                "name": "DeRouter_SpendLogs_p20260602",
    ...
    assert dropped == ["DeRouter_SpendLogs_p20260602"]
