...
def test_per_model_reads_route_to_reader_writes_to_writer():
    ...
    writer, writer_inner, reader, reader_inner = _make_wrappers()
    writer_inner.derouter_usertable = _model_actions_mock("writer_users")
    reader_inner.derouter_usertable = _model_actions_mock("reader_users")
    routing = RoutingPrismaWrapper(writer=writer, reader=reader)

    actions = routing.derouter_usertable

    # Reads → reader actions.
    assert actions.find_many is reader_inner.derouter_usertable.find_many
    assert actions.find_unique is reader_inner.derouter_usertable.find_unique
    assert actions.find_first is reader_inner.derouter_usertable.find_first
    assert actions.count is reader_inner.derouter_usertable.count
    assert actions.group_by is reader_inner.derouter_usertable.group_by

    # Writes → writer actions.
    assert actions.create is writer_inner.derouter_usertable.create
    assert actions.update is writer_inner.derouter_usertable.update
    assert actions.upsert is writer_inner.derouter_usertable.upsert
    assert actions.delete is writer_inner.derouter_usertable.delete
    assert actions.update_many is writer_inner.derouter_usertable.update_many
    assert actions.delete_many is writer_inner.derouter_usertable.delete_many
...
def test_writer_pinned_client_bypasses_reader_routing():
    ...
    from derouter.proxy.db.routing_prisma_wrapper import RoutingPrismaWrapper, WriterPinnedClient

    writer, writer_inner, reader, reader_inner = _make_wrappers()
    writer_inner.derouter_proxymodeltable = _model_actions_mock("writer_models")
    reader_inner.derouter_proxymodeltable = _model_actions_mock("reader_models")
    routing = RoutingPrismaWrapper(writer=writer, reader=reader)

    pinned = WriterPinnedClient(routing)

    assert pinned.db is writer
    assert pinned.db.derouter_proxymodeltable.find_many is writer_inner.derouter_proxymodeltable.find_many
...
def test_writer_pinned_client_yields_to_routed_reads_when_writer_down():
    ...
    assert pinned.db.derouter_proxymodeltable.find_many is reader_inner.derouter_proxymodeltable.find_many
...
def test_routed_actions_falls_back_to_writer_for_unknown_methods():
    from derouter.proxy.db.routing_prisma_wrapper import _RoutedActions
...
async def test_reads_route_to_writer_when_reader_unavailable():
    ...
    actions = routing.derouter_usertable
    assert actions.find_many is writer_inner.derouter_usertable.find_many
    assert actions.find_unique is writer_inner.derouter_usertable.find_unique
...
def test_per_model_accessor_falls_back_when_reader_lacks_attr():
    ...
    writer_inner.derouter_usertable = _model_actions_mock("writer_users")
    reader_inner = _PartialPrisma()  # deliberately missing derouter_usertable
    ...
    assert actions is writer_inner.derouter_usertable
...
def test_parse_iam_endpoint_from_url_extracts_all_fields():
    from derouter.proxy.db.prisma_client import parse_iam_endpoint_from_url

    ep = parse_iam_endpoint_from_url(
        "postgresql://derouter_user:initial-token@aurora-reader.example.com:6543/derouter?schema=public"
    )
    assert ep.host == "aurora-reader.example.com"
    assert ep.port == "6543"
    assert ep.user == "derouter_user"
    assert ep.name == "derouter"
...
def test_iam_endpoint_build_url_inserts_token_verbatim():
    from derouter.proxy.db.prisma_client import IAMEndpoint
...
async def test_getattr_does_not_block_inside_running_loop_on_expired_token(monkeypatch):
    ...
    monkeypatch.setenv(
        ...
        "postgresql://reader:placeholder@reader.aurora.local:5432/derouter",
    )
...
def test_writer_get_rds_iam_token_defaults_port_when_unset(monkeypatch, unset_database_url):
    ...
    monkeypatch.setenv("DATABASE_NAME", "derouter")
    ...
    monkeypatch.setitem(sys.modules, "derouter.proxy.auth.rds_iam_token", fake_module)
    ...
    assert ":5432/derouter" in (new_url or "")
...
def test_writer_get_rds_iam_token_uses_database_host_env_vars(monkeypatch, unset_database_url):
    ...
    assert new_url == (
        "postgresql://derouter:WRITER-TOKEN@writer.aurora.local:5432/derouter?schema=public"
    )
...
def test_reader_iam_refresh_uses_parsed_endpoint(monkeypatch):
    ...
    from derouter.proxy.db.prisma_client import IAMEndpoint, PrismaWrapper
    ...
    assert new_url.startswith(
        "postgresql://lit:FRESH-TOKEN@reader.aurora.local:5432/derouter"
    )
...
def test_prisma_client_init_falls_back_to_writer_when_reader_iam_token_fails(
    ...
    monkeypatch.setenv(
        "DATABASE_URL_READ_REPLICA",
        "postgresql://reader_user@reader.aurora.local:5432/derouter",
    )
    ...
    monkeypatch.setitem(
        sys.modules, "derouter.proxy.auth.rds_iam_token", fake_iam_module
    )
    ...
    with caplog.at_level(logging.WARNING, logger="DeRouter Proxy"):
        client = PrismaClient(
            database_url="postgresql://writer@writer.aurora.local:5432/derouter",
...
def test_prisma_client_premints_an_entra_token_for_the_reader(monkeypatch):
    ...
    from derouter.proxy.db.prisma_client import PrismaWrapper
    from derouter.proxy.db.routing_prisma_wrapper import RoutingPrismaWrapper
    from derouter.proxy.db.token_auth import AzureEntraTokenAuth

    monkeypatch.setenv("AZURE_POSTGRESQL_AUTH", "true")
    monkeypatch.delenv("IAM_TOKEN_DB_AUTH", raising=False)
    monkeypatch.setenv(
        "DATABASE_URL_READ_REPLICA",
        "postgresql://derouter%40contoso.com@reader.postgres.database.azure.com:5432/derouter",
    )
    ...
    with patch(
        ...
        client = PrismaClient(
            database_url="postgresql://derouter@writer.postgres.database.azure.com:5432/derouter",
            proxy_logging_obj=MagicMock(),
        )

    assert isinstance(client.db, RoutingPrismaWrapper)
    assert captured_kwargs["datasource"] == {
        "url": "postgresql://derouter%40contoso.com:ENTRA-TOKEN@reader.postgres.database.azure.com:5432/derouter"
    }
