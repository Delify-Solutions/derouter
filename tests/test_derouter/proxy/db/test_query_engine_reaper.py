...
from derouter.proxy.db.query_engine_reaper import (
    REAPER_THREAD_NAME,
    _read_comm_and_ppid,
    _reaper_loop,
    _send_signal,
    ...
    def test_matches_platform_capability(self):
        result: Final = subprocess.run(
            [
                ...
                "-c",
                "import sys; "
                "from derouter.proxy.db.query_engine_reaper import set_child_subreaper; "
...
class TestReaperLoop:
    def test_survives_scan_failure_and_continues(self):
        ...
        with (
            patch(
                "derouter.proxy.db.query_engine_reaper.reap_orphaned_engines",
                side_effect=flaky_scan,
            ),
            patch("derouter.proxy.db.query_engine_reaper.time.sleep"),
...
class TestReapOrphanedEngines:
    def test_terminates_each_orphan(self, tmp_path):
        ...
        with patch("derouter.proxy.db.query_engine_reaper.terminate_and_reap_all") as mock_terminate:
...
class TestStartQueryEngineReaper:
    def test_noop_on_non_linux(self):
        with patch("derouter.proxy.db.query_engine_reaper.sys.platform", "darwin"):
            assert start_query_engine_reaper() is None

    def test_starts_daemon_thread_on_linux(self):
        with (
            patch("derouter.proxy.db.query_engine_reaper.sys.platform", "linux"),
            patch(
                "derouter.proxy.db.query_engine_reaper.threading.enumerate",
                return_value=[],
            ),
            patch("derouter.proxy.db.query_engine_reaper.set_child_subreaper") as mock_subreaper,
            patch("derouter.proxy.db.query_engine_reaper.threading.Thread") as mock_thread_cls,
        ):
