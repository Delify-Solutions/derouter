...
def _prisma_client_returning(team_record: object) -> MagicMock:
    ...
    prisma_client.db.derouter_teamtable.find_unique = AsyncMock(return_value=team_record)
...
async def test_fetch_mcp_servers_by_team(team_record, expected):
    ...
    prisma_client.db.derouter_teamtable.find_unique.assert_awaited_once_with(
...
def _prisma_client_with_missing_mcp_server_row() -> MagicMock:
    prisma_client = MagicMock()
    prisma_client.db.derouter_mcpservertable.update = AsyncMock(return_value=None)
    return prisma_client
