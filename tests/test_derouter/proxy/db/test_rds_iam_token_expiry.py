...
Run these tests:
    uv run pytest tests/test_derouter/proxy/db/test_rds_iam_token_expiry.py -v -s
"""
...
async def demonstrate_fix():
    ...
    os.environ["DATABASE_NAME"] = "derouter"
    ...
    encoded_token = urllib.parse.quote(token, safe="")
    os.environ["DATABASE_URL"] = (
        f"postgresql://iam_user:{encoded_token}@mock-rds:5432/derouter"
    )
