...
emitted by the ``helm/derouter`` chart, before Prisma initializes. It covers
...
from derouter.proxy.db.token_auth import AzureEntraTokenAuth, RdsIamTokenAuth
...
def _stub_iam_token(token: str = "FAKE_TOKEN"):
    ...
    return patch(
        "derouter.proxy.auth.rds_iam_token.generate_iam_auth_token",
        return_value=token,
    )
...
def _stub_entra_token(token: str = "FAKE_TOKEN"):
    ...
    return patch(
        "derouter.secret_managers.get_azure_ad_token_provider.get_azure_ad_token_provider",
...
def test_assembles_writer_url_when_iam_enabled(monkeypatch):
    ...
    monkeypatch.setenv("DATABASE_USER", "derouter")
    monkeypatch.setenv("DATABASE_NAME", "derouter_db")
    ...
    assert (
        ...
        == "postgresql://derouter:WRITER_TOKEN@writer.example.com:5432/derouter_db?max_idle_connection_lifetime=60"
    )
...
def test_a_pre_encoded_iam_user_survives_url_assembly(monkeypatch):
    ...
    assert os.environ["DATABASE_URL"] == (
        "postgresql://svc%40corp:WRITER_TOKEN@writer.example.com:5432/derouter_db?max_idle_connection_lifetime=60"
    )
...
def test_reader_url_assembled_when_host_set_and_url_unset(monkeypatch):
    ...
    assert (
        ...
        == "postgresql://derouter:READER_TOKEN@reader.example.com:5432/derouter_db?max_idle_connection_lifetime=60"
    )
...
def test_reader_url_not_clobbered_when_already_set(monkeypatch):
    ...
    monkeypatch.setenv(
        "DATABASE_URL_READ_REPLICA",
        "postgresql://app:secret@reader.example.com:5432/derouter_db",
    )
    ...
    assert (
        os.environ["DATABASE_URL_READ_REPLICA"]
        == "postgresql://app:secret@reader.example.com:5432/derouter_db?max_idle_connection_lifetime=60"
    )
...
def test_reader_field_fallbacks_default_to_writer_values(monkeypatch):
    ...
    assert (
        os.environ["DATABASE_URL_READ_REPLICA"]
        == "postgresql://derouter:READER_TOKEN@reader.example.com:5432/derouter_db"
...
def test_assembles_writer_url_when_azure_entra_enabled(monkeypatch):
    ...
    monkeypatch.setenv("DATABASE_USER", "derouter@contoso.onmicrosoft.com")
    monkeypatch.setenv("DATABASE_NAME", "derouter_db")
    ...
    assert os.environ["DATABASE_URL"] == (
        "postgresql://derouter%40contoso.onmicrosoft.com:ENTRA_TOKEN"
        "@writer.postgres.database.azure.com:5432/derouter_db?max_idle_connection_lifetime=60"
    )
...
def test_azure_reader_url_assembled_from_writer_fallbacks(monkeypatch):
    ...
    assert os.environ["DATABASE_URL_READ_REPLICA"] == (
        "postgresql://derouter%40contoso.onmicrosoft.com:ENTRA_TOKEN"
        "@reader.postgres.database.azure.com:5432/derouter_db?schema=public&max_idle_connection_lifetime=60"
    )
...
def test_the_toggle_agrees_with_the_refresh_loop_on_every_spelling(monkeypatch):
    ...
    from derouter.proxy.db.token_auth import resolve_database_token_auth
...
def test_assembles_writer_url_from_password(monkeypatch):
    ...
    assert (
        os.environ["DATABASE_URL"]
        == "postgresql://derouter:s3cr3t@writer.example.com:5432/derouter_db?max_idle_connection_lifetime=60"
    )
...
def test_writer_password_is_percent_encoded(monkeypatch):
    ...
    assert (
        os.environ["DATABASE_URL"]
        == "postgresql://derouter:p%40ss%2Fw%3Ard@writer.example.com:5432/derouter_db?max_idle_connection_lifetime=60"
    )
...
def test_writer_url_not_clobbered_when_already_set(monkeypatch):
    ...
    monkeypatch.setenv(
        "DATABASE_URL", "postgresql://pinned:url@db.example.com:5432/derouter_db"
    )
    monkeypatch.setenv("DATABASE_HOST", "writer.example.com")
    monkeypatch.setenv("DATABASE_USER", "derouter")
    monkeypatch.setenv("DATABASE_NAME", "derouter_db")
    ...
    assert (
        os.environ["DATABASE_URL"]
        == "postgresql://pinned:url@db.example.com:5432/derouter_db?max_idle_connection_lifetime=60"
    )
...
def test_writer_url_passwordless(monkeypatch):
    ...
    assert (
        os.environ["DATABASE_URL"]
        == "postgresql://derouter@writer.example.com:5432/derouter_db?max_idle_connection_lifetime=60"
    )
...
def test_database_username_alias(monkeypatch):
    ...
    monkeypatch.setenv("DATABASE_HOST", "writer.example.com")
    monkeypatch.setenv("DATABASE_USERNAME", "derouter")
    monkeypatch.setenv("DATABASE_NAME", "derouter_db")
    monkeypatch.setenv("DATABASE_PASSWORD", "s3cr3t")
...
def test_password_reader_falls_back_to_writer_password(monkeypatch):
    ...
    assert (
        os.environ["DATABASE_URL_READ_REPLICA"]
        == "postgresql://derouter:s3cr3t@reader.example.com:5432/derouter_db?max_idle_connection_lifetime=60"
    )
...
def test_password_reader_uses_own_credentials(monkeypatch):
    ...
    monkeypatch.setenv("DATABASE_PASSWORD", "s3cr3t")
    monkeypatch.setenv("DATABASE_HOST_READ_REPLICA", "reader.example.com")
    monkeypatch.setenv("DATABASE_USER_READ_REPLICA", "derouter_ro")
    monkeypatch.setenv("DATABASE_PASSWORD_READ_REPLICA", "ro_pw")

    assert _apply() is True
    assert (
        os.environ["DATABASE_URL_READ_REPLICA"]
        == "postgresql://derouter_ro:ro_pw@reader.example.com:5432/derouter_db?max_idle_connection_lifetime=60"
    )
...
@pytest.mark.parametrize(
    "url,scheme",
    [
        ("sqlite:///data/derouter.db", "sqlite"),
...
def test_unsupported_db_scheme_does_not_echo_schemeless_credentials():
    ...
    leaky = "derouter:s3cr3t_password@db.internal:5432/derouter"
...
def test_apply_to_env_rejects_pinned_sqlite_writer(monkeypatch):
    ...
    monkeypatch.setenv("DATABASE_URL", "sqlite:///data/derouter.db")

    with pytest.raises(RuntimeError, match="sqlite"):
        _apply()

    # The bad URL must not have been propagated as a usable connection string.
    assert os.environ["DATABASE_URL"] == "sqlite:///data/derouter.db"
...
def test_apply_to_env_rejects_pinned_sqlite_direct_url(monkeypatch):
    ...
    monkeypatch.setenv("DIRECT_URL", "sqlite:///data/derouter.db")
...
def test_assembled_reader_url_inherits_writer_connection_params(monkeypatch):
    ...
    assert reader_url.startswith("postgresql://derouter:s3cr3t@reader.example.com:5432/derouter_db?")
...
def test_disable_prepared_statements_appends_pgbouncer_to_assembled_writer(monkeypatch):
    ...
    assert _apply() is True
    assert os.environ["DATABASE_URL"] == (
        "postgresql://derouter:s3cr3t@writer.example.com:5432/derouter_db?pgbouncer=true&max_idle_connection_lifetime=60"
    )
...
def test_disable_prepared_statements_appends_pgbouncer_to_pinned_writer(monkeypatch):
    ...
    monkeypatch.setenv("DATABASE_DISABLE_PREPARED_STATEMENTS", "true")
    monkeypatch.setenv("DATABASE_URL", "postgresql://u:p@db.example.com:5432/derouter_db")

    assert _apply() is False
    assert os.environ["DATABASE_URL"] == (
        "postgresql://u:p@db.example.com:5432/derouter_db?pgbouncer=true&max_idle_connection_lifetime=60"
    )


def test_disable_prepared_statements_respects_a_pinned_pgbouncer_value(monkeypatch):
    monkeypatch.setenv("DATABASE_DISABLE_PREPARED_STATEMENTS", "true")
    monkeypatch.setenv("DATABASE_URL", "postgresql://u:p@db.example.com:5432/derouter_db?pgbouncer=false")

    _apply()

    assert os.environ["DATABASE_URL"] == (
        "postgresql://u:p@db.example.com:5432/derouter_db?pgbouncer=false&max_idle_connection_lifetime=60"
    )
...
def test_disable_prepared_statements_applies_to_direct_url(monkeypatch):
    monkeypatch.setenv("DATABASE_DISABLE_PREPARED_STATEMENTS", "true")
    monkeypatch.setenv("DATABASE_URL", "postgresql://u:p@db.example.com:5432/derouter_db")
    monkeypatch.setenv("DIRECT_URL", "postgresql://u:p@direct.example.com:5432/derouter_db")

    _apply()

    assert os.environ["DIRECT_URL"] == (
        "postgresql://u:p@direct.example.com:5432/derouter_db?pgbouncer=true&max_idle_connection_lifetime=60"
    )
...
def test_disable_prepared_statements_off_leaves_urls_alone(monkeypatch):
    ...
    assert os.environ["DATABASE_URL"] == (
        "postgresql://u:p@db.example.com:5432/derouter_db?max_idle_connection_lifetime=60"
    )
...
def test_libpq_verify_full_and_sslrootcert_become_prisma_strict_sslcert(monkeypatch):
    ...
    monkeypatch.setenv(
        "DATABASE_URL",
        "postgresql://u:p@db.example.com:5432/derouter_db?sslmode=verify-full&sslrootcert=/certs/rds-bundle.pem",
    )
...
def test_libpq_verify_ca_becomes_prisma_strict(monkeypatch):
    monkeypatch.setenv("DATABASE_URL", "postgresql://u:p@db.example.com:5432/derouter_db?sslmode=verify-ca")
...
def test_sslrootcert_alone_turns_on_strict_verification(monkeypatch):
    ...
    monkeypatch.setenv(
        "DATABASE_URL", "postgresql://u:p@db.example.com:5432/derouter_db?sslmode=require&sslrootcert=/certs/ca.pem"
    )
...
def test_pinned_prisma_ssl_params_win_over_libpq_translation(monkeypatch):
    monkeypatch.setenv(
        "DATABASE_URL",
        "postgresql://u:p@db.example.com:5432/derouter_db"
...
def test_default_idle_lifetime_applied_to_pinned_writer_and_direct_url(monkeypatch):
    ...
    assert os.environ["DIRECT_URL"] == (
        "postgresql://u:p@direct.example.com:5432/derouter_db?max_idle_connection_lifetime=60"
    )
...
def test_url_pinned_idle_lifetime_wins_over_default_and_env_knob(monkeypatch):
    ...
    monkeypatch.setenv(
        "DATABASE_URL", "postgresql://u:p@db.example.com:5432/derouter_db?max_idle_connection_lifetime=300"
    )

    _apply()

    assert os.environ["DATABASE_URL"] == (
        "postgresql://u:p@db.example.com:5432/derouter_db?max_idle_connection_lifetime=300"
    )
...
def test_env_knob_overrides_default_idle_lifetime(monkeypatch):
    ...
    assert os.environ["DATABASE_URL"] == (
        "postgresql://u:p@db.example.com:5432/derouter_db?max_idle_connection_lifetime=45"
    )
    assert os.environ["DIRECT_URL"] == (
        "postgresql://u:p@direct.example.com:5432/derouter_db?max_idle_connection_lifetime=45"
    )
