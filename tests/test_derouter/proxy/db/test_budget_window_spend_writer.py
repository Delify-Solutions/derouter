...
class _RecordingAggregate:
    """Stands in for the DeRouter_SpendLogs seed aggregate. before_batch
...
class _SpendLogsFake:
    """Sums the DeRouter_SpendLogs rows (request_id, spend, startTime) it holds,
...
async def test_existing_row_is_never_reseeded():
    """The seed is a full DeRouter_SpendLogs scan; running it for a row that is
...
async def test_upsert_sql_adds_for_a_current_window_and_replaces_for_a_newer_one():
    ...
    assert (
        'spend = CASE WHEN "DeRouter_BudgetWindowSpend".window_start >= EXCLUDED.window_start '
        'THEN "DeRouter_BudgetWindowSpend".spend + $6 ELSE EXCLUDED.spend END' in normalized
    )
    assert 'window_start = GREATEST("DeRouter_BudgetWindowSpend".window_start, EXCLUDED.window_start)' in normalized
...
async def test_unknown_entity_type_contributes_no_seed():
    """Only key and team windows have a DeRouter_SpendLogs column to aggregate;
...
async def test_seed_aggregate_sql_splits_the_window_at_the_batch_start(entity_type, expected_column):
    ...
    assert 'FROM "DeRouter_SpendLogs"' in normalized
