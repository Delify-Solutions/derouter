...
``derouter/proxy/db/token_auth.py`` decides where the proxy's Postgres password
...
def test_rds_mint_delegates_to_the_sigv4_token_generator():
    endpoint = _endpoint(host="writer.aurora.local", user="derouter_rds")
...
def test_build_url_encodes_a_upn_user_and_the_schema():
    endpoint = _endpoint(user="derouter@contoso.onmicrosoft.com", name="derouter db", schema="app/schema")

    assert endpoint.build_url("TOKEN") == (
        "postgresql://derouter%40contoso.onmicrosoft.com:TOKEN"
        "@pg.postgres.database.azure.com:5432/derouter%20db?schema=app%2Fschema"
    )
...
def test_build_url_inserts_the_token_verbatim():
    ...
    assert _endpoint().build_url(rds_token) == (
        f"postgresql://derouter:{rds_token}@pg.postgres.database.azure.com:5432/derouter_db"
    )
...
@pytest.mark.parametrize(
    "endpoint",
    [
        IAMEndpoint(host="h.example.com", port="5432", user="derouter", name="derouter_db"),
        IAMEndpoint(host="h.example.com", port="6543", user="derouter@contoso.com", name="db", schema="public"),
        IAMEndpoint(host="h.example.com", port="5432", user="u", name="derouter db", schema="app schema"),
    ],
)
