...
from derouter.proxy._types import DBSpendUpdateTransactions, Litellm_EntityType, SpendUpdateQueueItem
from derouter.proxy.db.db_spend_update_writer import DBSpendUpdateWriter, debitable_model_access_groups
from derouter.proxy.db.db_transaction_queue.daily_spend_update_queue import DailySpendUpdateQueue
...
from derouter.proxy.spend_tracking.spend_tracking_utils import get_request_model_access_groups
...
def test_access_groups_read_from_request_metadata():
    kwargs = {"derouter_params": {"metadata": {MODEL_ACCESS_GROUP_METADATA_KEY: ["pool-a", "pool-b", "pool-a"]}}}
    assert get_request_model_access_groups(kwargs) == ("pool-a", "pool-b")


def test_access_groups_read_from_derouter_metadata():
    kwargs = {"derouter_params": {"derouter_metadata": {MODEL_ACCESS_GROUP_METADATA_KEY: ["pool-a"]}}}
    assert get_request_model_access_groups(kwargs) == ("pool-a",)


def test_standard_logging_payload_wins_over_metadata():
    kwargs = {
        "derouter_params": {"metadata": {MODEL_ACCESS_GROUP_METADATA_KEY: ["from-metadata"]}},
...
def test_non_list_access_group_metadata_is_ignored(stamped):
    kwargs = {"derouter_params": {"metadata": {MODEL_ACCESS_GROUP_METADATA_KEY: stamped}}}
...
def test_key_absent_from_metadata_yields_no_groups():
    ...
    kwargs = {"derouter_params": {"metadata": {"user_api_key_user_id": "u-1"}}}
    assert get_request_model_access_groups(kwargs) == ()
...
def test_explicit_none_yields_no_groups():
    ...
    kwargs = {"derouter_params": {"metadata": {MODEL_ACCESS_GROUP_METADATA_KEY: None}}}
    assert get_request_model_access_groups(kwargs) == ()
...
async def test_neither_absent_nor_none_metadata_debits_anything(metadata):
    ...
    await writer._update_model_access_group_db(
        response_cost=0.5,
        request_model_access_groups=get_request_model_access_groups({"derouter_params": {"metadata": metadata}}),
        served_model_id="deployment-1",
        prisma_client=object(),
        router=_FakeRouter({"deployment-1": ["premium-pool"]}),
    )
...
def test_detached_sub_call_falls_back_to_the_auth_object():
    ...
    kwargs = {"derouter_params": {"metadata": {"user_api_key_auth": _Auth()}}}
...
def test_non_string_entries_are_dropped():
    kwargs = {"derouter_params": {"metadata": {MODEL_ACCESS_GROUP_METADATA_KEY: ["pool-a", None, "", 3]}}}
    assert get_request_model_access_groups(kwargs) == ("pool-a",)
...
def test_missing_metadata_yields_no_groups():
    ...
    assert get_request_model_access_groups({"derouter_params": {}}) == ()
...
async def test_commit_increments_spend_on_the_model_access_group_budget_table():
    ...
    assert prisma_client.batcher.tables["derouter_modelaccessgroupbudgettable"].calls == [
        ({"access_group_name": "pool-a"}, {"spend": {"increment": 0.25}}),
        ({"access_group_name": "pool-b"}, {"spend": {"increment": 0.5}}),
    ]
    assert "derouter_tagtable" not in prisma_client.batcher.tables
...
async def test_batch_database_updates_enqueues_access_group_spend():
    ...
    await writer._batch_database_updates(
        ...
        end_user_id=None,
        prisma_client=object(),
        derouter_proxy_budget_name=None,
