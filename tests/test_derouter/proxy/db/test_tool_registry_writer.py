...
that exposes derouter_tooltable.upsert / find_many / find_unique.
"""
...
def _make_prisma(
    *,
    ...
    """Return a mock prisma_client with derouter_tooltable.upsert, find_many, find_unique."""
    prisma = MagicMock()
    prisma.db.derouter_tooltable = MagicMock()
    prisma.db.derouter_tooltable.upsert = AsyncMock(return_value=upsert_return)
    prisma.db.derouter_tooltable.find_many = AsyncMock(
        return_value=find_many_rows if find_many_rows is not None else []
    )
    prisma.db.derouter_tooltable.find_unique = AsyncMock(return_value=find_unique_row)
    return prisma
...
async def test_batch_upsert_tools_calls_upsert():
    ...
    prisma.db.derouter_tooltable.upsert.assert_awaited_once()
    call_kw = prisma.db.derouter_tooltable.upsert.call_args.kwargs
...
async def test_batch_upsert_tools_empty_list():
    ...
    prisma.db.derouter_tooltable.upsert.assert_not_awaited()
...
async def test_batch_upsert_multiple_tools_calls_upsert_per_tool():
    ...
    await batch_upsert_tools(prisma, items)
    assert prisma.db.derouter_tooltable.upsert.await_count == 2
    calls = prisma.db.derouter_tooltable.upsert.call_args_list
...
async def test_list_tools_no_filter():
    ...
    prisma.db.derouter_tooltable.find_many.assert_awaited_once()
    call_kw = prisma.db.derouter_tooltable.find_many.call_args.kwargs
...
async def test_get_tool_found():
    ...
    prisma.db.derouter_tooltable.find_unique.assert_awaited_once_with(
...
async def test_update_tool_policy_calls_upsert_then_get_tool():
    ...
    prisma.db.derouter_tooltable.find_unique.assert_awaited_with(
        where={"tool_name": "my_tool"}
    )
...
async def test_get_tools_by_names_returns_policy_map():
    ...
    prisma.db.derouter_tooltable.find_many.assert_awaited_once_with(
...
async def test_get_tools_by_names_empty_list():
    ...
    assert result == {}
    prisma.db.derouter_tooltable.find_many.assert_not_awaited()
...
async def test_tool_policy_registry_sync_and_get_effective_policies():
    ...
    prisma.db.derouter_objectpermissiontable.find_many = AsyncMock(
        return_value=[
...
async def test_sync_tool_policy_from_db_retries_on_transport_error_first_read():
    ...
    mock_prisma_client.db.derouter_tooltable.find_many = AsyncMock(
        side_effect=_flaky_find_many
    )
    mock_prisma_client.db.derouter_objectpermissiontable.find_many = AsyncMock(
        return_value=[]
    )
...
async def test_sync_tool_policy_from_db_retries_on_transport_error_second_read():
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_tooltable.find_many = AsyncMock(return_value=[])
    mock_prisma_client.db.derouter_objectpermissiontable.find_many = AsyncMock(
