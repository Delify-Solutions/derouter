...
def _configured_servers(*servers: MCPServer):
    return patch.dict(
        "derouter.proxy._experimental.mcp_server.mcp_server_manager.global_mcp_server_manager.config_mcp_servers",
...
class TestLoggingSafeMcpHeaders:
    ...
    def test_strips_custom_derouter_key_header(self):
        """general_settings.derouter_key_header_name carries the proxy virtual key, so it must
    ...
    def test_strips_client_controlled_redaction_opt_out(self):
        """derouter-disable-message-redaction is read back out of the logged metadata to turn off
        redaction, so leaving it in place lets any MCP client undo what an admin configured."""
        safe = logging_safe_mcp_headers({"derouter-disable-message-redaction": "true", "x-nuid": "nuid-1"})
    ...
    @pytest.mark.parametrize(
        "configured",
        [
            [{"header_name": "X-User", "derouter_user_role": "customer"}],
            {"header_name": "X-User", "derouter_user_role": "customer"},
        ],
...
class TestBuildSyntheticMcpRequest:
    def test_forwards_client_headers_without_upstream_credentials(self):
        """The synthetic request feeds add_derouter_data_to_request, which derives
    ...
    def test_drops_custom_derouter_key_header(self):
    ...
    def test_drops_caller_host_so_the_logged_url_is_not_client_steerable(self):
        """add_derouter_data_to_request records str(request.url) as proxy_server_request.url, and
    ...
    def test_keeps_identity_header_so_end_user_attribution_survives(self):
        """add_derouter_data_to_request reads user_header_name off this request to fill
