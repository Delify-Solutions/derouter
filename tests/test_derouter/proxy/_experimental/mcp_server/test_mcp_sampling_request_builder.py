...
class TestBuildSamplingRequest:
    ...
    def test_should_forward_raw_headers(self):
        ...
        raw = {
            "x-derouter-tags": "tag1,tag2",
            "x-derouter-trace-id": "trace-abc-123",
        ...
        assert headers.get("x-derouter-tags") == "tag1,tag2"
        assert headers.get("x-derouter-trace-id") == "trace-abc-123"
    ...
    def test_server_should_default_to_derouter_port(self):
        """Server tuple should use port 4000 (DeRouter default), not 0."""
        req = _build_sampling_request()
        _host, _port = req.scope["server"]
        assert _port == 4000, f"Expected default DeRouter port 4000, got {_port}"
    ...
    def test_should_forward_x_derouter_api_key(self):
        """x-derouter-api-key header must be forwarded for auth."""
        raw = {"x-derouter-api-key": "sk-proxy-key-123"}
        req = _build_sampling_request(raw_headers=raw)
        headers = dict(req.headers)
        assert headers.get("x-derouter-api-key") == "sk-proxy-key-123"
