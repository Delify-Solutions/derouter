...
class TestMCPRequestHandler:
    ...
    async def test_admitted_subject_not_zeroed_by_require_key_mcp_access_defined(self):
        ...
        with (
            ...
            patch("derouter.proxy.proxy_server.general_settings", {"require_key_mcp_access_defined": True}),
        ):
            result = await MCPRequestHandler.get_allowed_mcp_servers(auth)
    ...
    async def test_team_toolset_only_does_not_inherit_org_full_server_list(self):
        ...
        with (
            ...
            patch(  # test-quality-ok: team-server resolution requires the proxy's module-global prisma client
                "derouter.proxy.proxy_server.prisma_client", MagicMock()
            ),
            patch(  # test-quality-ok: stub the DB team loader to drive the real team-server resolution path
                "derouter.proxy.auth.auth_checks.get_team_object", AsyncMock(return_value=team_obj)
            ),
            patch(  # test-quality-ok: access-group lookup hits the DB, not under test here
                "derouter.proxy.auth.auth_checks._get_mcp_server_ids_from_access_groups",
    ...
    async def test_declares_toolsets_gate_falls_back_to_db_for_unhydrated_key(self):
        ...
        with (
            ...
            patch(  # test-quality-ok: stub the level's perm loader; the resolver reads module globals with no injection seam
                "derouter.proxy.auth.auth_checks.get_object_permission",
    ...
    async def test_permission_inheritance_edge_cases(self):
        ...
        mock_prisma_client.db.derouter_objectpermissiontable.find_unique.return_value = None
        ...
        mock_prisma_client.db.derouter_objectpermissiontable.find_unique.side_effect = Exception("DB Error")
    ...
    async def test_access_group_extras_returns_empty_without_access_group_ids(self):
        ...
        with (
            ...
            patch("derouter.proxy._experimental.mcp_server.mcp_server_manager.global_mcp_server_manager") as mock_mgr,
        ):
    ...
    @pytest.mark.parametrize(
        ...
        [
            # Test case 1: x-derouter-api-key header present
            (
                [(b"x-derouter-api-key", b"test-api-key-123")],
                "test-api-key-123",
                None,
                {},
            ),
            ...
            (
                [
                    (b"x-derouter-api-key", b"primary-key"),
            ...
            (
                [(b"X-DEROUTER-API-KEY", b"case-insensitive-key")],
    ...
    async def test_process_mcp_request_with_server_auth_headers(
        ...
        with patch(
            "derouter.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp.user_api_key_auth",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                [
                    (b"x-derouter-api-key", b"test-api-key"),
            ...
            # Test case 6: Using Authorization header instead of x-derouter-api-key
            (
                [
    ...
    async def test_header_extraction(self, headers, expected_result):
        ...
        api_key = MCPRequestHandler.get_derouter_api_key_from_headers(extracted_headers)
...
class TestMCPOAuth2AuthFlow:
    ...
    as DeRouter API keys, causing auth failures and empty tool listings.
    """

    async def test_oauth2_token_in_authorization_header_fallback(self):
        """
        When only the Authorization header is present with a non-DeRouter OAuth2
        token AND the target server delegates auth to upstream, DeRouter skips its
        ...
        with (
            ...
            # The upstream token is never validated as a DeRouter key ...
    ...
    async def test_explicit_derouter_key_with_oauth2_authorization(self):
        """
        When both x-derouter-api-key AND Authorization header are present,
        DeRouter key should be used for auth and Authorization preserved for OAuth2.
        """
        scope = {
            "type": "http",
            "method": "POST",
            "path": "/mcp/atlassian_mcp",
            "headers": [
                (b"x-derouter-api-key", b"sk-derouter-valid-key"),
        ...
        with patch(
            ...
            # DeRouter key should be used for auth
            mock_auth.assert_called_once()
            call_args = mock_auth.call_args
            assert call_args.kwargs["api_key"] == "Bearer sk-derouter-valid-key"
    ...
    @pytest.mark.parametrize(
        "header_value",
        [b"sk-derouter-valid-key", b"Bearer sk-derouter-valid-key", b"bearer sk-derouter-valid-key"],
    )
    async def test_x_derouter_api_key_survives_bearer_only_strip(self, header_value):
        from derouter.proxy.auth.user_api_key_auth import _get_bearer_token

        scope = {
            "type": "http",
            "method": "POST",
            "path": "/mcp/some_server",
            "headers": [(b"x-derouter-api-key", header_value)],
        }
        ...
        mock_auth.assert_called_once()
        assert _get_bearer_token(api_key=mock_auth.call_args.kwargs["api_key"]) == "sk-derouter-valid-key"
        assert auth_result.user_id == "test-user"

    async def test_derouter_key_in_authorization_backward_compat(self):
        """
        Backward compatibility: when only Authorization header is present
        with a valid DeRouter key (not OAuth2), auth should succeed normally.
        """
        scope = {
            "type": "http",
            "method": "POST",
            "path": "/mcp/some_server",
            "headers": [
                (b"authorization", b"Bearer sk-derouter-valid-key"),
            ],
        }
        ...
        with patch(
            ...
            # Should succeed with the DeRouter key from Authorization header
            from derouter.proxy.utils import hash_token

            assert auth_result.api_key == hash_token("sk-derouter-valid-key")
            mock_auth.assert_called_once()
    ...
    async def test_proxy_exception_non_delegate_oauth2_challenges_with_per_server_metadata(self):
        ...
        a gateway-managed oauth2 server the bearer is treated as a DeRouter
...
class TestMCPPassthroughColdStartAdmission:
    ...
    async def test_cold_start_ignores_header_without_path_target(self):
        ...
        with (
            ...
            patch("derouter.proxy._experimental.mcp_server.mcp_server_manager.global_mcp_server_manager") as mock_mgr,
            patch(
                "derouter.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp._is_mcp_passthrough_cold_start"
            ) as mock_cold_start,
        ):
    ...
    async def test_cold_start_fails_closed_when_client_ip_hides_server(self):
        ...
        with (
            ...
            patch(
                "derouter.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp.IPAddressUtils.get_mcp_client_ip",
...
class TestMCPOAuth2FallbackTargetGating:
    ...
    is operator-configured for ``auth_type=oauth2``. A failed DeRouter-auth
    ...
    async def test_non_delegate_oauth2_does_not_fall_back_to_anonymous(self):
        ...
        ``delegate_auth_to_upstream`` must not exchange a failed DeRouter auth for
    ...
    async def test_fallback_allowed_when_target_is_passthrough(self):
        ...
        DeRouter key but the target is a pass-through server, so admission
...
class TestMCPDelegateAuthToUpstream:
    ...
    When set on an ``auth_type=oauth2`` MCP server, DeRouter must skip its own
    ...
    async def test_delegate_skips_derouter_auth_with_no_authorization(self):
        """
    ...
    async def test_delegate_with_upstream_token_in_authorization_skips_derouter_auth(
        self,
    ):
        """
        oauth2 + delegate_auth_to_upstream=True with an upstream OAuth token in
        ``Authorization``: the delegate gate fires before any DeRouter validation,
    ...
    async def test_delegate_off_still_requires_derouter_auth(self):
        """
        oauth2 server but delegate flag is OFF → existing behaviour: a missing
        / invalid DeRouter key still 401s (no anonymous fast-path).
        """
        from fastapi import HTTPException
    ...
    async def test_explicit_derouter_key_takes_precedence_over_delegate(self):
        """
        When ``x-derouter-api-key`` is present, normal auth runs even for a
        ...
        fires when no DeRouter key is supplied.
        """
        from derouter.types.mcp import MCPAuth
        ...
        scope = {
            ...
            "method": "POST",
            "path": "/mcp/delegated_oauth_server",
            "headers": [(b"x-derouter-api-key", b"Bearer sk-1234")],
        }
    ...
    async def test_authorization_bearer_on_delegate_server_treated_as_upstream(self):
        ...
        upstream token rather than a DeRouter key — even when it is sk-shaped. It
        is forwarded upstream without DeRouter validation, so ``user_api_key_auth``
        is not called and no DeRouter identity is resolved. Callers who need
        DeRouter identity / spend tracking on a delegate server must supply
        ``x-derouter-api-key`` (see
        test_explicit_derouter_key_takes_precedence_over_delegate).
        """
        from derouter.types.mcp import MCPAuth
    ...
    async def test_delegate_ignored_for_client_credentials_server(self):
        ...
        → bypass must NOT fire; normal DeRouter auth must be attempted.
        ...
        caller invoke tools as DeRouter's service account.
        """
        from fastapi import HTTPException
    ...
    async def test_delegate_ignored_for_unstamped_m2m_shaped_server(self):
        ...
        reopen the anonymous bypass to a server that runs upstream as DeRouter's
    ...
    async def test_delegate_bypass_for_internal_server(self):
        """
        Delegate + oauth2 interactive servers bypass DeRouter auth even when
    ...
    async def test_true_passthrough_skips_derouter_auth_anonymously(self):
    ...
    def test_extract_target_server_names_matches_routing_parser(self):
        ...
        targeting ``<delegated>`` by the auth gate (bypassing DeRouter auth)
    ...
    async def test_delegate_does_not_bypass_on_extra_path_segment(self):
        ...
        resolve to a delegate-enabled server, so normal DeRouter auth runs.
        """
        from fastapi import HTTPException
...
class TestMCPCustomHeaderName:
    ...
    def test_get_mcp_client_side_auth_header_name(self, env_var, general_setting, expected_header_name):
        ...
        with patch("derouter.secret_managers.main.get_secret_str") as mock_get_secret:
            with patch("derouter.proxy.proxy_server.general_settings") as mock_general_settings:
                # Configure mocks
                ...
                for call in mock_get_secret.call_args_list:
                    assert call.args == ("DEROUTER_MCP_CLIENT_SIDE_AUTH_HEADER_NAME",)
    ...
    def test_get_mcp_server_auth_headers_from_headers(self):
        ...
        headers = Headers({"x-derouter-api-key": "test-key", "content-type": "application/json"})
        ...
        headers = Headers({"x-derouter-api-key": "test-key", "x-mcp-github-authorization": ""})
        result = MCPRequestHandler._get_mcp_server_auth_headers_from_headers(headers)
        assert result == {"github": {"Authorization": ""}}

        # Test case 9: Edge case - very long header value
        long_token = "Bearer " + "x" * 1000
        headers = Headers({"x-derouter-api-key": "test-key", "x-mcp-github-authorization": long_token})
        result = MCPRequestHandler._get_mcp_server_auth_headers_from_headers(headers)
...
def test_mcp_path_based_server_segregation(monkeypatch):
    ...
    from derouter.proxy._experimental.mcp_server.server import app, get_auth_context
    ...
    monkeypatch.setattr(
        "derouter.proxy._experimental.mcp_server.server.session_manager_stateless",
        MagicMock(handle_request=dummy_handle_request),
    )
    monkeypatch.setattr(
        "derouter.proxy._experimental.mcp_server.server.session_manager_stateful",
        MagicMock(handle_request=dummy_handle_request),
    )
    monkeypatch.setattr(
        "derouter.proxy._experimental.mcp_server.server.initialize_session_managers",
        AsyncMock(),
    )
    ...
    response = client.get("/mcp/zapier,group1/tools", headers={"x-derouter-api-key": "test"})
...
async def test_get_team_object_permission_with_already_loaded_permission():
    ...
    """
    from derouter.proxy._types import DeRouter_ObjectPermissionTable, DeRouter_TeamTable

    # Create mock object permission
    mock_object_permission = DeRouter_ObjectPermissionTable(
    ...
    mock_team_obj = DeRouter_TeamTable(
        team_id="team-123",
    ...
    with patch(
        ...
        with patch("derouter.proxy.auth.auth_checks.get_team_object") as mock_get_team:
            with patch("derouter.proxy.auth.auth_checks.get_object_permission") as mock_get_perm:
...
async def test_get_team_object_permission_ui_session_team_skips_db_lookup():
    """
    UI session tokens carry the virtual team_id "derouter-dashboard" (UI_TEAM_ID),
    ...
    """
    from derouter.proxy._types import UI_TEAM_ID
    ...
    with patch("derouter.proxy.proxy_server.prisma_client", mock_prisma):
        with patch(
            ...
            side_effect=HTTPException(
                status_code=404,
                detail={"error": "Team doesn't exist in db. Team=derouter-dashboard."},
            ),
        ):
...
async def test_get_allowed_mcp_servers_for_team_uses_helper():
    ...
    from derouter.proxy._types import DeRouter_ObjectPermissionTable, DeRouter_TeamTable
    from derouter.types.mcp import MCPTransport
    from derouter.types.mcp_server.mcp_server_manager import MCPServer
...
async def test_get_allowed_mcp_servers_for_key_guard_conditions(user_api_key_auth, prisma_client_value, scenario):
    ...
    with patch(
        ...
        with patch("derouter.proxy.proxy_server.prisma_client", prisma_client_value):
            result = await MCPRequestHandler._get_allowed_mcp_servers_for_key(user_api_key_auth)
...
async def test_get_allowed_mcp_servers_for_key_prefers_in_memory_permission():
    ...
    from derouter.proxy._types import DeRouter_ObjectPermissionTable
    from derouter.types.mcp import MCPTransport
    from derouter.types.mcp_server.mcp_server_manager import MCPServer
    ...
    try:
        perms = DeRouter_ObjectPermissionTable(
...
class TestAgentMCPPermissions:
    ...
    async def test_get_agent_object_permission_uses_shared_helper(self):
        ...
        prisma_client.db.derouter_agentstable.find_unique = AsyncMock(return_value=agent_row)
        user_api_key_auth = UserAPIKeyAuth(
            api_key="test-key",
            user_id="test-user",
        ...
        with (
            ...
            prisma_client.db.derouter_agentstable.find_unique.reset_mock()
            await MCPRequestHandler._get_agent_object_permission(user_api_key_auth)
            prisma_client.db.derouter_agentstable.find_unique.assert_not_called()
    ...
    async def test_get_agent_object_permission_caches_missing_permission(self):
        ...
        with (
            ...
            prisma_client.db.derouter_agentstable.find_unique.assert_awaited_once()
...
async def test_tool_permission_servers_included_in_allowed_servers():
    ...
    Regression test for https://github.com/Delify-Solutions/derouter/issues/21954
    """
    from derouter.proxy._experimental.mcp_server.mcp_server_manager import (
        global_mcp_server_manager,
    )
...
class TestOrgMCPPermissions:
    ...
    async def test_get_org_object_permission_no_prisma(self):
        ...
        with patch(
            "derouter.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp.MCPRequestHandler._get_org_object_permission",
            new_callable=AsyncMock,
            return_value=None,
        ):
...
async def test_team_access_group_ids_resolve_to_mcp_servers():
    ...
    Reproduction of https://github.com/Delify-Solutions/derouter/issues/27657:
...
def test_expand_permission_list_does_not_honor_all_proxy_sentinel():
    ...
    from derouter.proxy._types import SpecialMCPServerName
    from derouter.types.mcp import MCPTransport
    from derouter.types.mcp_server.mcp_server_manager import MCPServer
...
async def test_get_allowed_mcp_servers_for_team_expands_all_proxy_sentinel_dynamically():
    ...
    try:
        team_perm = DeRouter_ObjectPermissionTable(
...
async def test_key_with_all_proxy_sentinel_does_not_grant_all_servers():
    ...
    try:
        key_perm = DeRouter_ObjectPermissionTable(
...
class TestMCPDcrBridgeDelegateAdmission:
    ...
    _KEY_HASH = "hashed-derouter-key-abc123"
    ...
    def _reloaded_key(**overrides):
        ...
        defaults = dict(
            ...
            object_permission=DeRouter_ObjectPermissionTable(
    ...
    def _patch_key_reload(*, return_value=None, side_effect=None, team_blocked=False, owner=None, project_object=None):
        ...
        patchers = [
            patch("derouter.proxy.auth.auth_checks.get_key_object", get_key_object),
            patch("derouter.proxy.proxy_server.prisma_client", MagicMock()),
            patch("derouter.proxy.proxy_server.user_api_key_cache", MagicMock()),
        ]
        if team_blocked:
            patchers.append(
                patch(
                    "derouter.proxy.auth.user_api_key_auth.get_team_object",
                    AsyncMock(return_value=MagicMock(blocked=True)),
                )
            )
        if owner is not None:
            patchers.append(patch("derouter.proxy.auth.auth_checks.get_user_object", AsyncMock(return_value=owner)))
        if project_object is not None:
            patchers.append(
                patch(
                    "derouter.proxy.auth.user_api_key_auth.get_project_object",
    ...
    def _patch_user_reload(*, return_value=None, side_effect=None):
        ...
        with (
            patch("derouter.proxy.auth.auth_checks.get_user_object", get_user_object),
            patch("derouter.proxy.proxy_server.prisma_client", MagicMock()),
            patch("derouter.proxy.proxy_server.user_api_key_cache", MagicMock()),
        ):
    ...
    async def test_valid_envelope_reloads_live_key_and_admits_its_authorization_context(self):
        ...
        with (
            ...
            patch("derouter.proxy._experimental.mcp_server.mcp_server_manager.global_mcp_server_manager") as mock_mgr,
            patch("derouter.proxy.proxy_server.master_key", self._MASTER_KEY),
    ...
    async def test_user_subject_envelope_carries_the_users_mcp_object_permission(self):
        ...
        get_allowed_mcp_servers grants the user their derouter-granted servers, rather than admitting a
    ...
    async def test_user_subject_envelope_permanent_db_fault_is_503_not_worded_as_transient(self):
        ...
        with (
            ...
            patch(  # test-quality-ok: the envelope opener reads master_key off the proxy module, no injection seam
                "derouter.proxy.proxy_server.master_key", self._MASTER_KEY
            ),
            self._patch_user_reload(
    ...
    _POLICY_GATE = "derouter.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp._run_centralized_common_checks"
    ...
    async def test_over_budget_admission_surfaces_429_not_401(self):
        ...
        mapped = await self._enforce_with_gate_error(derouter.BudgetExceededError(current_cost=10.0, max_budget=1.0))
    ...
    async def test_envelope_rejected_by_proxy_wide_pre_db_gates_403(self):
        ...
        with (
            patch("derouter.proxy._experimental.mcp_server.mcp_server_manager.global_mcp_server_manager") as mock_mgr,
            patch("derouter.proxy.proxy_server.master_key", self._MASTER_KEY),
            patch("derouter.proxy.proxy_server.general_settings", {"allowed_routes": ["/chat/completions"]}),
        ):
            mock_mgr.get_mcp_server_by_name.return_value = self._bridge_delegate_server()
            with pytest.raises(HTTPException) as exc_info:
                await MCPRequestHandler.process_mcp_request(scope)
    ...
    async def test_injection_key_agrees_with_egress_lookup(self, server_name, alias):
        ...
        from derouter.proxy._experimental.mcp_server.utils import lookup_mcp_server_auth_in_headers
    ...
    async def test_valid_derouter_authorization_key_uses_standard_admission(self):
        scope = {
            "type": "http",
            "method": "POST",
            "path": "/mcp/bridge_delegate_server",
            "headers": [(b"authorization", b"Bearer sk-valid-derouter-key")],
        }
        admitted = UserAPIKeyAuth(api_key="hashed-key", user_id="derouter-key-user")
        ...
        assert mcp_server_auth_headers == {}
        assert mock_auth.await_args.kwargs["api_key"] == "Bearer sk-valid-derouter-key"

    async def test_non_401_derouter_key_failure_is_not_converted_to_oauth_challenge(self):
        scope = {
            "type": "http",
            "method": "POST",
            "path": "/mcp/bridge_delegate_server",
            "headers": [(b"authorization", b"Bearer sk-blocked-derouter-key")],
        }
    ...
    async def test_explicit_derouter_key_wins_over_envelope_arm(self):
        """An explicit x-derouter-api-key is always a DeRouter credential and its arm precedes the
        ...
        scope = {
            ...
            "path": "/mcp/bridge_delegate_server",
            "headers": [
                (b"x-derouter-api-key", b"sk-explicit-derouter-key"),
                (b"authorization", f"Bearer {envelope}".encode("latin-1")),
            ],
        }

        async def mock_user_api_key_auth(api_key, request):
            return UserAPIKeyAuth(api_key=api_key, user_id="derouter-key-user")
        ...
        mock_auth.assert_called_once()
        assert mock_auth.call_args.kwargs["api_key"] == "Bearer sk-explicit-derouter-key"
        # The explicit-key arm admitted; the envelope arm never ran, so no inner token is injected.
        assert auth_result.user_id == "derouter-key-user"
        assert mcp_server_auth_headers == {}
    ...
    async def test_admit_helper_raises_500_when_master_key_missing(self):
        ...
        envelope = self._mint_bridge_envelope()
        with patch("derouter.proxy.proxy_server.master_key", None):
            with pytest.raises(HTTPException) as exc_info:
...
class TestAggregateGatewayDcrChallenge:
    ...
    targets, explicit derouter keys, or non-401 failures."""

    _AUTH_PATCH_TARGET = "derouter.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp.user_api_key_auth"
    ...
    async def test_challenge_invalid_token_on_failed_bearer(self):
        """A bearer that fails DeRouter admission at aggregate scope (an expired
    ...
    async def test_challenge_inserts_server_root_path(self):
        ...
        with (
            patch.dict(os.environ, {"SERVER_ROOT_PATH": "/derouter"}),
            patch(self._AUTH_PATCH_TARGET, side_effect=self._auth_401()),
        ):
            with pytest.raises(HTTPException) as exc_info:
                await MCPRequestHandler.process_mcp_request(self._scope())
        www_authenticate = (exc_info.value.headers or {})["WWW-Authenticate"]
        assert (
            'resource_metadata="http://testserver/.well-known/oauth-protected-resource/derouter/mcp"' in www_authenticate
        )

    async def test_no_challenge_for_explicit_derouter_key(self):
        """An explicit x-derouter-api-key declares a derouter-key client; a typo
        ...
        with (
            ...
            with pytest.raises(ProxyException):
                await MCPRequestHandler.process_mcp_request(
                    self._scope(extra_headers=((b"x-derouter-api-key", b"sk-typo"),))
                )
    ...
    async def test_per_server_challenge_keeps_spelling_under_server_root_path(self):
        ...
        ``/derouter/{server}/mcp``; matching that against the root-relative ``/{server}/mcp`` shape
        ...
        for original_path, expected_metadata_path in (
            ("/derouter/mcp/github", "/derouter/.well-known/oauth-protected-resource/derouter/mcp/github"),
            ("/derouter/github/mcp", "/derouter/.well-known/oauth-protected-resource/derouter/github/mcp"),
        ):
            scope = {
    ...
    def test_challenge_target_excludes_every_non_gateway_managed_mode(self):
        ...
        with patch("derouter.proxy._experimental.mcp_server.mcp_server_manager.global_mcp_server_manager") as mock_mgr:
...
class TestGatewaySessionAdmission:
    ...
    the LIVE derouter user it references at any MCP scope (aggregate, per-server path, or
    ...
    async def test_bad_session_bearer_fails_closed(self, scenario, expect_challenge):
        ...
        with patch("derouter.proxy.proxy_server.master_key", master_key):
            with pytest.raises(HTTPException) as exc_info:
...
def _make_team(team_id, mcp_servers, *, org_id=None, tool_perms=None, members=("sso-user",)):
    from derouter.proxy._types import DeRouter_ObjectPermissionTable, DeRouter_TeamTable, Member
...
def _make_admitted_subject(user_id, *, org_id=None, own_servers=None, own_tool_perms=None):
    ...
    if own_servers is not None or own_tool_perms is not None:
        op = DeRouter_ObjectPermissionTable(
...
class TestUserSubjectTeamUnion:
    ...
    def _patch(self, *, teams_by_id, user_teams=None, orgs_by_id=None):
        ...
        with (
            patch("derouter.proxy.auth.auth_checks.get_team_object", _get_team_object),
            patch("derouter.proxy.auth.auth_checks.get_user_object", _get_user_object),
            patch("derouter.proxy.auth.auth_checks.get_org_object", _get_org_object),
            patch("derouter.proxy.auth.auth_checks._get_mcp_server_ids_from_access_groups", AsyncMock(return_value=[])),
            patch("derouter.proxy.proxy_server.get_current_spend", _spend_from_fallback),
            patch("derouter.proxy.proxy_server.prisma_client", MagicMock()),
            patch("derouter.proxy.proxy_server.user_api_key_cache", MagicMock()),
            patch("derouter.proxy.proxy_server.proxy_logging_obj", MagicMock()),
        ):
    ...
    async def test_org_outage_is_not_treated_as_a_missing_org(self):
        ...
        from derouter.proxy.auth.auth_checks import OrganizationNotFoundError
        ...
        with self._patch(teams_by_id=teams, user_teams=["t1"]):
            with patch("derouter.proxy.auth.auth_checks.get_org_object", absent):
        ...
        with self._patch(teams_by_id=teams, user_teams=["t1"]):
            with patch("derouter.proxy.auth.auth_checks.get_org_object", outage):
                reachable = await MCPRequestHandler.get_allowed_mcp_servers(auth)
    ...
    async def test_org_ceiling_fault_fails_closed_for_admitted_but_open_for_keys(self):
        ...
        auth = _make_admitted_subject("sso-user")
        auth.org_id = "org-a"
        auth.object_permission = DeRouter_ObjectPermissionTable(object_permission_id="op-u", mcp_servers=["srv1"])
        with self._patch(teams_by_id={}, user_teams=[]):
    ...
    async def test_only_the_attributing_team_bucket_is_charged(self):
        ...
        from derouter.proxy.hooks.parallel_request_limiter_v3 import _PROXY_MaxParallelRequestsHandler_v3
    ...
    def _manager_with(self, server_ids, allow_all=()):
        from derouter.proxy._experimental.mcp_server.mcp_server_manager import MCPServerManager
        from derouter.types.mcp import MCPTransport
        from derouter.types.mcp_server.mcp_server_manager import MCPServer
    ...
    async def test_billing_auth_carries_team_and_org_onto_the_spend_object(self):
        ...
        with self._patch(teams_by_id={"t-grant": t_grant}, user_teams=["t-grant"]):
            with patch(
                "derouter.proxy._experimental.mcp_server.mcp_server_manager.global_mcp_server_manager._get_mcp_server_from_tool_name",
    ...
    async def test_admitted_tools_never_run_the_single_credential_prelude(self):
        ...
        auth.object_permission = DeRouter_ObjectPermissionTable(object_permission_id="op-u", mcp_toolsets=["ts-1"])
        boom = AsyncMock(side_effect=RuntimeError("toolset resolution exploded"))
        with self._patch(teams_by_id=teams, user_teams=["t1"]):
            with patch(
                "derouter.proxy._experimental.mcp_server.mcp_server_manager.global_mcp_server_manager.resolve_toolset_tool_permissions",
                boom,
            ):
    ...
    async def test_admitted_own_byom_servers_stay_open(self):
        ...
        db_default_perm = DeRouter_ObjectPermissionTable(object_permission_id="op-u", mcp_servers=[])
    ...
    async def test_admitted_admin_explicit_scope_still_wins(self):
        ...
        from derouter.proxy._types import DeRouter_ObjectPermissionTable, LitellmUserRoles
        ...
        scoped_key = UserAPIKeyAuth(
            ...
            object_permission=DeRouter_ObjectPermissionTable(object_permission_id="op-k", mcp_servers=[]),
        )
    ...
    async def test_admitted_admin_db_default_empty_scope_still_gets_registry(self):
        ...
        admitted = _make_admitted_subject("admin-user")
        admitted.user_role = LitellmUserRoles.PROXY_ADMIN
        admitted.object_permission = DeRouter_ObjectPermissionTable(object_permission_id="op-u", mcp_servers=[])
        with patch.object(MCPRequestHandler, "get_allowed_mcp_servers", AsyncMock(return_value=[])):
    ...
    async def test_admitted_admin_tools_ride_own_source_on_ungranted_server(self):
        ...
        with self._patch(teams_by_id={}, user_teams=[]):
            with patch(
                "derouter.proxy._experimental.mcp_server.mcp_server_manager.global_mcp_server_manager.operator_open_server_ids",
    ...
    async def test_admitted_opt_out_via_wrapper_keeps_team_servers(self):
        ...
        from derouter.proxy._types import DeRouter_ObjectPermissionTable, SpecialMCPServerNames

        manager = self._manager_with(["srv-team"])
        opt_out = DeRouter_ObjectPermissionTable(
    ...
    async def test_open_channel_confers_reachability_not_a_ceiling_waiver(self):
        ...
        auth.object_permission = DeRouter_ObjectPermissionTable(
    ...
    async def test_team_in_over_budget_org_grants_nothing(self):
        ...
        org = MagicMock(object_permission_id=None, derouter_budget_table=MagicMock(max_budget=5.0), spend=9.0)
        auth = _make_admitted_subject("sso-user")
    ...
    async def test_one_faulting_team_does_not_deny_the_other_sources(self):
        ...
        auth = _make_admitted_subject("sso-user")
        auth.object_permission = DeRouter_ObjectPermissionTable(object_permission_id="op-u", mcp_servers=["srv-own"])
        ...
        with self._patch(teams_by_id=teams, user_teams=["t-boom", "t-ok"]):
            with patch("derouter.proxy.auth.auth_checks.get_team_object", _team_or_boom):
                servers = set(await MCPRequestHandler.get_allowed_mcp_servers(auth))
                tools = await MCPRequestHandler.get_allowed_tools_for_server("srv1", auth)
    ...
    async def test_key_org_tool_ceiling_fault_keeps_key_restrictions(self):
        ...
        key_auth.object_permission = DeRouter_ObjectPermissionTable(
    ...
    async def test_org_list_caps_a_source_but_never_becomes_a_grant(self):
        ...
        org_perm = AsyncMock(
            return_value=DeRouter_ObjectPermissionTable(object_permission_id="op-org-a", mcp_servers=["srv1", "srv2"])
        )
        with self._patch(teams_by_id={}, user_teams=[]):
    ...
    async def test_blocked_team_grants_no_servers_to_admitted_subject(self):
        ...
        blocked = DeRouter_TeamTable(
            team_id="team-blocked",
            blocked=True,
            members_with_roles=[Member(user_id="sso-user", role="user")],
            access_group_ids=[],
            object_permission=DeRouter_ObjectPermissionTable(object_permission_id="op-blk", mcp_servers=["srv-secret"]),
        )
...
class TestAdmittedSubjectPerTeamOrgCap:
    ...
    def _patch(self, *, teams_by_id, user_teams, org_perms=None, registry=None):
        """org_perms: {org_id: DeRouter_ObjectPermissionTable | None | LOAD_FAILS}.
        ...
        cms = [
            ...
            patch("derouter.proxy.auth.auth_checks.get_user_object", _get_user_object),
            patch("derouter.proxy.auth.auth_checks.get_org_object", _get_org_object),
            patch("derouter.proxy.auth.auth_checks.get_object_permission", _get_object_permission),
            patch(
                "derouter.proxy.auth.auth_checks._get_mcp_server_ids_from_access_groups",
                AsyncMock(return_value=[]),
            ),
    ...
    async def test_team_grant_capped_by_its_own_org(self):
        ...
        org_perms = {"org-a": DeRouter_ObjectPermissionTable(object_permission_id="orgop-org-a", mcp_servers=["srv1"])}
        auth = _make_admitted_subject("sso-user")
    ...
    async def test_cross_org_teams_each_capped_by_own_org(self):
        ...
        org_perms = {
            "org-a": DeRouter_ObjectPermissionTable(object_permission_id="orgop-org-a", mcp_servers=["srv1"]),
            "org-b": DeRouter_ObjectPermissionTable(object_permission_id="orgop-org-b", mcp_servers=["srv3"]),
        }
        auth = _make_admitted_subject("sso-user")
    ...
    async def test_all_proxy_grant_capped_by_org(self):
        from derouter.proxy._types import DeRouter_ObjectPermissionTable, SpecialMCPServerName
    ...
    async def test_direct_grants_unioned_with_team_and_capped_by_user_org(self):
        ...
        org_perms = {
            ...
            "org-u": DeRouter_ObjectPermissionTable(object_permission_id="orgop-org-u", mcp_servers=["srvD", "srv1"]),
        }
    ...
    async def test_single_team_key_uses_primary_org_cap_not_per_team(self):
        ...
        teams = {"team-a": _make_team("team-a", ["srv1", "srv2"], org_id="org-a")}
        org_perms = {
            "org-a": DeRouter_ObjectPermissionTable(object_permission_id="orgop-org-a", mcp_servers=["srv2"]),
            "org-k": DeRouter_ObjectPermissionTable(object_permission_id="orgop-org-k", mcp_servers=["srv1"]),
        }
    ...
    async def test_org_tool_ceiling_binds_when_team_places_no_tool_restriction(self):
        ...
        teams = {"team-a": _make_team("team-a", ["srv1"], org_id="org-a")}
        org_perms = {
            "org-a": DeRouter_ObjectPermissionTable(
    ...
    async def test_org_less_team_grant_capped_by_user_primary_org(self):
        ...
        org_perms = {"org-U": DeRouter_ObjectPermissionTable(object_permission_id="orgop-org-U", mcp_servers=["srv1"])}
    ...
    async def test_config_defined_oauth_server_by_alias_reached_and_org_capped(self):
        ...
        org_perms = {
            "org-a": DeRouter_ObjectPermissionTable(object_permission_id="orgop-org-a", mcp_servers=["linear_cfg"])
        }
        auth = _make_admitted_subject("sso-user")
        with self._patch(
            teams_by_id=teams,
    ...
    async def test_config_oauth_server_alias_resolution_feeds_the_org_cap(self):
        ...
        org_perms = {
            "org-b": DeRouter_ObjectPermissionTable(object_permission_id="orgop-org-b", mcp_servers=["control-id"])
        }
        auth = _make_admitted_subject("sso-user")
        with self._patch(
            teams_by_id=teams,
...
class TestSessionBearerEgressScrub:
    ...
    async def test_session_bearer_misdirected_to_passthrough_is_scrubbed(self):
        ...
        mock_auth.assert_not_called()  # true_passthrough → DeRouter auth skipped (anonymous arm, no marker)
...
class TestUserMCPEntitlement:
    ...
    def _perm(self, *, servers=None, access_groups=None, tool_permissions=None) -> DeRouter_ObjectPermissionTable:
...
class TestGetUserObjectPermission:
    ...
    def _prisma_with_user(self, user_row):
        prisma_client = MagicMock()
        prisma_client.db.derouter_usertable.find_unique = AsyncMock(return_value=user_row)
    ...
    async def test_resolves_through_the_shared_permission_cache(self):
        ...
        with (
            patch("derouter.proxy.proxy_server.prisma_client", prisma_client),
            patch("derouter.proxy.proxy_server.user_api_key_cache", DualCache()),
            patch("derouter.proxy.proxy_server.proxy_logging_obj", MagicMock()),
            patch(
                "derouter.proxy.auth.auth_checks.get_object_permission",
                new_callable=AsyncMock,
            ...
            prisma_client.db.derouter_usertable.find_unique.reset_mock()
            await MCPRequestHandler._get_user_object_permission(auth)
            prisma_client.db.derouter_usertable.find_unique.assert_not_called()
    ...
    async def test_caches_a_sentinel_for_a_human_with_no_entitlement(self):
        ...
        with (
            ...
            patch("derouter.proxy.proxy_server.user_api_key_cache", DualCache()),
            patch("derouter.proxy.proxy_server.proxy_logging_obj", MagicMock()),
            patch("derouter.proxy.auth.auth_checks.get_object_permission", new_callable=AsyncMock) as mock_get_perm,
        ):
            assert await MCPRequestHandler._get_user_object_permission(auth) is None
            assert await MCPRequestHandler._get_user_object_permission(auth) is None
            mock_get_perm.assert_not_awaited()
            prisma_client.db.derouter_usertable.find_unique.assert_awaited_once()
    ...
    async def test_unreadable_user_row_places_no_ceiling(self):
        ...
        prisma_client = MagicMock()
        prisma_client.db.derouter_usertable.find_unique = AsyncMock(side_effect=Exception("db down"))
...
def _agent_prisma(object_permission_id=None, side_effect=None):
    prisma_client = MagicMock()
    prisma_client.db.derouter_agentstable.find_unique = AsyncMock(
...
def _entitlement_fault_globals(prisma_client=None):
    ...
    with (
        patch("derouter.proxy.proxy_server.prisma_client", prisma_client or MagicMock()),
        patch("derouter.proxy.proxy_server.user_api_key_cache", DualCache()),
        patch("derouter.proxy.proxy_server.proxy_logging_obj", MagicMock()),
    ):
        yield
...
class TestEntitlementFaultSemantics:
    ...
    async def test_end_user_named_but_unloadable_permission_denies(self):
        ...
        with _entitlement_fault_globals():
            with (
                patch("derouter.proxy.auth.auth_checks.get_end_user_object", AsyncMock(return_value=end_user)),
                patch("derouter.proxy.auth.auth_checks.get_object_permission", AsyncMock(return_value=None)),
            ):
    ...
    async def test_end_user_without_an_entitlement_places_no_ceiling(self):
        ...
        for lookup, shape in (
            ...
            with _entitlement_fault_globals():
                with patch("derouter.proxy.auth.auth_checks.get_end_user_object", lookup):
                    allowed = await MCPRequestHandler.get_allowed_mcp_servers(auth)
    ...
    async def test_agent_named_but_unloadable_permission_denies(self):
        ...
        with _entitlement_fault_globals(_agent_prisma(object_permission_id="op-agent")):
            with patch("derouter.proxy.auth.auth_checks.get_object_permission", AsyncMock(return_value=None)):
                allowed = await MCPRequestHandler.get_allowed_mcp_servers(auth)
    ...
    async def test_org_named_but_unloadable_ceiling_denies(self):
        ...
        with _entitlement_fault_globals():
            with (
                patch("derouter.proxy.auth.auth_checks.get_org_object", AsyncMock(return_value=org)),
                patch("derouter.proxy.auth.auth_checks.get_object_permission", AsyncMock(return_value=None)),
            ):
                allowed = await MCPRequestHandler.get_allowed_mcp_servers(auth)
    ...
    async def test_org_without_a_resolvable_entitlement_places_no_ceiling(self):
        ...
        for lookup, shape in (
            ...
            with _entitlement_fault_globals():
                with patch("derouter.proxy.auth.auth_checks.get_org_object", lookup):
                    allowed = await MCPRequestHandler.get_allowed_mcp_servers(auth)
    ...
    async def test_keyless_org_ceiling_denies_on_either_fault_class(self):
        ...
        org = MagicMock(object_permission_id="op-org")
        with _entitlement_fault_globals():
            with patch("derouter.proxy.auth.auth_checks.get_org_object", AsyncMock(return_value=org)):
                with patch("derouter.proxy.auth.auth_checks.get_object_permission", AsyncMock(return_value=None)):
                    named_unloadable = await MCPRequestHandler.get_allowed_mcp_servers(auth)
            with patch(
                "derouter.proxy.auth.auth_checks.get_org_object",
    ...
    async def test_keyless_source_never_consults_the_end_user_or_agent_levels(self):
        ...
        with _entitlement_fault_globals(_agent_prisma(object_permission_id="op-agent")):
            with (
                patch("derouter.proxy.auth.auth_checks.get_end_user_object", AsyncMock(side_effect=AssertionError)),
                patch("derouter.proxy.auth.auth_checks.get_object_permission", AsyncMock(return_value=None)),
            ):
                allowed = await MCPRequestHandler.get_allowed_mcp_servers(auth)
