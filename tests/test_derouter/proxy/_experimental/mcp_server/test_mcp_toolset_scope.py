...
class TestApplyToolsetScope:
    ...
    async def test_restricts_to_toolset_servers_and_tools(self):
        from derouter.proxy._experimental.mcp_server.server import _apply_toolset_scope
        ...
        with patch(
            "derouter.proxy._experimental.mcp_server.server."
...
class TestFetchMCPToolsetsAccess:
    ...
    async def test_non_admin_empty_grants_returns_empty(self):
        ...
        with (
            ...
            patch(
                "derouter.proxy.management_endpoints.mcp_management_endpoints.list_mcp_toolsets",
...
class TestToolsetPrefixResolution:
    ...
    async def _resolve(server, server_id, stored):
        ...
        with (
            patch(
                "derouter.proxy._experimental.mcp_server.mcp_server_manager."
                "global_mcp_server_manager.get_mcp_server_by_id",
                return_value=server,
            ),
...
class TestMCPActiveToolsetContextVar:
    ...
    def test_contextvar_default_is_none(self):
        from derouter.proxy._experimental.mcp_server.server import _mcp_active_toolset_id
    ...
    async def test_client_header_is_stripped_in_scope(self):
        ...
        with (
            ...
            patch(
                "derouter.proxy._experimental.mcp_server.server.IPAddressUtils",
            ...
            patch(
                "derouter.proxy._experimental.mcp_server.server.MCPDebug",
                MagicMock(
