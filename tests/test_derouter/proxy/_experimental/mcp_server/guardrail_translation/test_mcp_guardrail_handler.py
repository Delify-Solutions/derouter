...
import derouter
import derouter.llms as derouter_llms
...
from derouter.proxy._experimental.mcp_server.utils import MAX_STRUCTURED_CONTENT_SCAN_DEPTH
...
def restore_callbacks(monkeypatch):
    ...
    derouter.llms memoizes the guardrail translation mappings in a module global, and
    ...
    """
    monkeypatch.setattr(derouter, "callbacks", derouter.callbacks)
    monkeypatch.setattr(
        derouter_llms,
        "endpoint_guardrail_translation_mappings",
        derouter_llms.endpoint_guardrail_translation_mappings,
    )
    yield
...
async def test_two_sequential_guardrails_both_masks_survive(restore_callbacks, monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", _two_interleaving_maskers(run_in_parallel=False))
...
async def test_two_parallel_guardrails_on_separate_arguments_both_masks_survive(restore_callbacks, monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", _two_interleaving_maskers(run_in_parallel=True))
