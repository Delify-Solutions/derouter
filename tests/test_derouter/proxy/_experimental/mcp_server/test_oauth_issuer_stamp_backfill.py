...
def _prisma(rows):
    ...
    prisma_client.db.derouter_mcpservertable.find_many = AsyncMock(return_value=rows)
    prisma_client.db.derouter_mcpservertable.update = AsyncMock()
...
async def test_clears_the_stamp_and_records_its_own_actor():
    ...
    call = prisma_client.db.derouter_mcpservertable.update.call_args
    ...
    where = prisma_client.db.derouter_mcpservertable.find_many.call_args.kwargs["where"]
...
async def test_leaves_rows_alone_that_do_not_carry_the_defect_signature(overrides, reason):
    ...
    prisma_client.db.derouter_mcpservertable.update.assert_not_awaited()
...
async def test_a_failed_row_does_not_abort_the_rest():
    ...
    assert await backfill_discovery_stamped_issuers(prisma_client) == 1
    assert prisma_client.db.derouter_mcpservertable.update.await_count == 2
