...
def _mock_prisma(null_rows, token_rows):
    ...
    mock_prisma.db.derouter_mcpservertable.find_many = AsyncMock(return_value=null_rows)
    mock_prisma.db.derouter_mcpservertable.update_many = AsyncMock(return_value=MagicMock())
    mock_prisma.db.derouter_mcpusercredentials.find_many = AsyncMock(return_value=token_rows)
    return mock_prisma
...
async def test_backfill_only_targets_null_flow_oauth2_rows():
    ...
    assert counts == {}
    mock_prisma.db.derouter_mcpservertable.find_many.assert_awaited_once_with(
        where={"auth_type": "oauth2", "oauth2_flow": None},
    )
    mock_prisma.db.derouter_mcpusercredentials.find_many.assert_not_awaited()
    mock_prisma.db.derouter_mcpservertable.update_many.assert_not_awaited()
...
async def test_backfill_stamps_rows_and_reports_rule_counts():
    ...
    mock_prisma.db.derouter_mcpservertable.update_many.assert_awaited_once()
    call = mock_prisma.db.derouter_mcpservertable.update_many.await_args
