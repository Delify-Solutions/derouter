...
from derouter.proxy._experimental.mcp_server.mcp_context import _mcp_proxy_mode
...
async def test_proxy_scope_exception_emits_failure_log(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    auth = UserAPIKeyAuth(
        ...
        object_permission=DeRouter_ObjectPermissionTable(object_permission_id="denied", mcp_servers=["no-mcp-servers"]),
    )
    ...
    with pytest.raises(HTTPException) as denied:
        await server._dispatch_virtual_mcp_tool(
            ...
            raw_headers={"authorization": "Bearer raw-scope-secret", "x-derouter-call-id": "scope-denial"},
        )
