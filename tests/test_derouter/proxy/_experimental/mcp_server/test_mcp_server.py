...
from derouter.types.mcp_server.mcp_server_manager import MCPOAuthMetadata, MCPServer
...
async def test_mcp_server_tool_call_body_contains_request_data():
    ...
    # Mock the add_derouter_data_to_request function to capture the data
    captured_data = {}

    async def mock_add_derouter_data_to_request(data, request, user_api_key_dict, proxy_config, **kwargs):
    ...
    with patch(
        "derouter.proxy.derouter_pre_call_utils.add_derouter_data_to_request",
        mock_add_derouter_data_to_request,
    ):
        with patch(
            "derouter.proxy._experimental.mcp_server.server.call_mcp_tool",
            mock_call_mcp_tool,
        ):
            with patch(
...
async def test_mcp_server_tool_call_forwards_client_headers_to_logging():
    ...
    with patch(
        ...
        with patch(
            ...
            with patch("derouter.proxy.proxy_server.proxy_config", MagicMock()):
...
@pytest.mark.asyncio
async def test_mcp_server_tool_call_strips_custom_derouter_key_header():
    """The deployment can rename the proxy key header via general_settings.derouter_key_header_name.
    ...
    async def capturing_add_derouter_data_to_request(**kwargs):
        data = await add_derouter_data_to_request(**kwargs)
        captured_data.update(data)
        return data
    ...
    with patch(
        "derouter.proxy.derouter_pre_call_utils.add_derouter_data_to_request",
        capturing_add_derouter_data_to_request,
    ):
        with patch(
            "derouter.proxy._experimental.mcp_server.server.call_mcp_tool",
            mock_call_mcp_tool,
            ...
            with patch("derouter.proxy.proxy_server.proxy_config", MagicMock()):
                with patch.dict(
                    ...
                    {"derouter_key_header_name": "x-company-key"},
                    clear=False,
                ):
                    await mcp_server_tool_call("test_tool", {"param": "value"})
...
async def test_mcp_server_tool_call_relays_upstream_auth_error_as_iserror():
    ...
    try:
        from derouter.proxy._experimental.mcp_server.exceptions import MCPUpstreamAuthError
        from derouter.proxy._experimental.mcp_server.server import (
            mcp_server_tool_call,
            set_auth_context,
        )
    ...
    with patch(
        ...
        with patch(
            ...
            with patch("derouter.proxy.proxy_server.proxy_config", MagicMock()):
                with patch("derouter.proxy._experimental.mcp_server.server.verbose_logger", mock_logger):
...
def test_prepare_mcp_server_headers_case_insensitive_extra_headers():
    ...
    server_auth_header, extra_headers = _prepare_mcp_server_headers(
        ...
        oauth2_headers=None,
        raw_headers={
            "x-derouter-api-key": "Bearer sk-derouter-key",
...
def test_prepare_mcp_server_headers_group_header_defaults_for_members_only():
    ...
    def resolve(mcp_server: MCPServer):
        server_auth_header, _ = _prepare_mcp_server_headers(
            ...
            mcp_auth_header=None,
            oauth2_headers=None,
            raw_headers={"x-derouter-api-key": "Bearer sk-derouter-key"},
        )
...
def test_prepare_mcp_server_headers_passthrough_strips_authorization_without_admission_header():
    ...
    server_auth_header, extra_headers = _prepare_mcp_server_headers(
        ...
        oauth2_headers=None,
        raw_headers={
            "authorization": "Bearer sk-derouter-key",
...
def test_prepare_mcp_server_headers_passthrough_strips_authorization_for_authenticated_admission():
    """When admission validated ``Authorization`` as a DeRouter key
    (``user_api_key_auth.api_key`` is set, no explicit ``x-derouter-api-key``),
    ...
    server_auth_header, extra_headers = _prepare_mcp_server_headers(
        ...
        user_api_key_auth=UserAPIKeyAuth(api_key="sk-derouter-key"),
    )
...
def test_prepare_mcp_server_headers_oauth2_m2m_omits_derouter_caller_authorization():
    """M2M OAuth must not put caller Bearer (DeRouter API key) into extra_headers (#23652)."""
    try:
        from derouter.proxy._experimental.mcp_server.server import (
            _prepare_mcp_server_headers,
        )
    ...
    caller_key = {"Authorization": "Bearer sk-derouter-caller"}
...
async def test_get_prompts_from_mcp_servers_success():
    ...
    with (
        patch(
            "derouter.proxy._experimental.mcp_server.server._get_allowed_mcp_servers",
            AsyncMock(return_value=[server_a, server_b]),
        ) as mock_allowed,
        patch(
            "derouter.proxy._experimental.mcp_server.server._prepare_mcp_server_headers",
            return_value=(None, None),
        ) as mock_headers,
        patch(
            "derouter.proxy._experimental.mcp_server.server.global_mcp_server_manager",
        ) as mock_manager,
    ):
...
async def test_mcp_get_prompt_success():
    try:
        from derouter.proxy._experimental.mcp_server.server import mcp_get_prompt
    except ImportError:
        pytest.skip("MCP server not available")
...
async def test_mcp_read_resource_success():
    try:
        from derouter.proxy._experimental.mcp_server.server import mcp_read_resource
    except ImportError:
        pytest.skip("MCP server not available")
...
async def test_get_tools_from_mcp_servers_continues_when_one_server_fails():
    ...
    with patch(
        ...
        with patch(
            "derouter.proxy._experimental.mcp_server.server.verbose_logger",
        ) as mock_logger:
...
async def _denied_scoped_list(
    ...
    from derouter.proxy._experimental.mcp_server.server import _get_tools_from_mcp_servers
...
async def test_scoped_list_denied_by_agent_binding_raises_403_naming_agent():
    ...
    pytest.importorskip("derouter.proxy._experimental.mcp_server.server")
...
async def test_handle_list_tools_converts_permission_httpexception_to_mcp_error():
    ...
    try:
        from derouter.proxy._experimental.mcp_server.server import handle_list_tools
    except ImportError:
        pytest.skip("MCP server not available")
    ...
    with (
        patch(  # test-quality-ok: the protocol handler reads auth from module context; no injection seam
            "derouter.proxy._experimental.mcp_server.server.get_or_extract_auth_context",
        ...
        patch(  # test-quality-ok: the listing helper is the handler's only collaborator; the suite's seam
            "derouter.proxy._experimental.mcp_server.server._list_mcp_tools",
...
async def test_mcp_server_tool_call_renders_denial_message_not_detail_dict():
    try:
        from derouter.proxy._experimental.mcp_server.server import mcp_server_tool_call
    except ImportError:
        pytest.skip("MCP server not available")
...
async def test_concurrent_initialize_session_managers():
    ...
    import derouter.proxy._experimental.mcp_server.server as mcp_server
    ...
    try:
        ...
        with (
            ...
            patch("derouter.proxy._experimental.mcp_server.server.verbose_logger"),
        ):
...
async def test_streamable_http_session_manager_is_stateless():
    ...
    assert session_manager_stateless.stateless is True, (
        ...
        "See: https://github.com/Delify-Solutions/derouter/issues/20242"
    )
...
async def test_mcp_routing_initialize_to_stateful_no_session_to_stateless():
    ...
    async def make_request(method_body: bytes, path: str = "/mcp/progress_test"):
        ...
        with (
            patch(
                "derouter.proxy._experimental.mcp_server.server.extract_mcp_auth_context",
                new_callable=AsyncMock,
                return_value=(MagicMock(), None, ["progress_test"], None, None, None),
            ),
            patch(
                "derouter.proxy._experimental.mcp_server.server.set_auth_context",
            ),
            patch(
                "derouter.proxy._experimental.mcp_server.server._SESSION_MANAGERS_INITIALIZED",
                True,
            ),
            patch.object(
                session_manager_stateless,
...
async def test_mcp_routing_caps_body_peek_for_oversized_chunked_body():
    ...
    """
    try:
        from derouter.proxy._experimental.mcp_server import server as mcp_server
        from derouter.proxy._experimental.mcp_server.server import (
            handle_streamable_http_mcp,
            session_manager_stateful,
            session_manager_stateless,
    ...
    with (
        ...
        patch("derouter.proxy._experimental.mcp_server.server.set_auth_context"),
        patch(
            "derouter.proxy._experimental.mcp_server.server._SESSION_MANAGERS_INITIALIZED",
            True,
        ),
...
async def test_mcp_routing_with_conflicting_alias_and_group_name():
    ...
    with (
        patch(
            "derouter.proxy._experimental.mcp_server.server.global_mcp_server_manager.get_allowed_mcp_servers",
            mock_get_allowed,
        ),
        patch(
            "derouter.proxy._experimental.mcp_server.server.MCPRequestHandler._get_mcp_servers_from_access_groups",
            mock_db_lookup,
        ),
        patch(
            "derouter.proxy._experimental.mcp_server.server.global_mcp_server_manager._get_tools_from_server",
            mock_get_tools_spy,
        ),
    ):
...
async def test_oauth2_caller_headers_not_forwarded_for_migrated_server():
    ...
    with (
        ...
        patch(
            "derouter.proxy._experimental.mcp_server.server._prefetch_oauth_creds_for_user",
            new_callable=AsyncMock,
            return_value={},
        ),
        patch(
            "derouter.proxy._experimental.mcp_server.server._get_user_oauth_extra_headers_from_db",
            new_callable=AsyncMock,
            return_value=None,
        ),
    ):
...
async def test_mcp_manager_allows_public_servers_without_permissions():
    ...
    with (
        patch(
            "derouter.proxy.management_endpoints.common_utils._user_has_admin_view",
            return_value=False,
        ),
        patch(
            "derouter.proxy._experimental.mcp_server.mcp_server_manager.MCPRequestHandler.get_allowed_mcp_servers",
...
async def test_mcp_manager_merges_public_and_restricted_servers():
    try:
        from derouter.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp import MCPServerAccess
        from derouter.proxy._experimental.mcp_server.mcp_server_manager import (
            MCPServerManager,
        )
        from derouter.proxy._types import MCPTransport
    ...
    with (
        ...
        patch(
            "derouter.proxy._experimental.mcp_server.mcp_server_manager.MCPRequestHandler.get_mcp_server_access",
...
async def test_call_mcp_tool_user_unauthorized_access():
    ...
    from derouter.proxy._experimental.mcp_server.server import call_mcp_tool
    from derouter.proxy._types import UserAPIKeyAuth
    ...
    with (
        patch(
            "derouter.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp.MCPRequestHandler.get_allowed_mcp_servers",
            AsyncMock(return_value=["allowed_server", "another_server"]),
        ),
        patch(
            "derouter.proxy._experimental.mcp_server.server.global_mcp_server_manager.get_mcp_server_by_id",
...
async def test_list_tools_filters_by_key_team_permissions():
    ...
    try:
        ...
        from derouter.proxy._types import DeRouter_ObjectPermissionTable, UserAPIKeyAuth
    except ImportError:
        pytest.skip("MCP server not available")
...
async def test_list_tools_with_team_tool_permissions_inheritance():
    ...
    team_object_permission = DeRouter_ObjectPermissionTable(
    ...
    key_object_permission = DeRouter_ObjectPermissionTable(
    ...
    with patch(
        ...
        with patch(
            "derouter.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp.MCPRequestHandler._get_team_object_permission",
...
async def test_list_tools_strips_prefix_when_matching_permissions():
    ...
    Tools from MCP servers are prefixed (e.g., "GITMCP-fetch_derouter_documentation"),
    but allowed tools in DB are stored without prefix (e.g., "fetch_derouter_documentation").
    ...
    object_permission = DeRouter_ObjectPermissionTable(
        ...
        mcp_tool_permissions={
            "gitmcp_server": [
                "fetch_derouter_documentation",  # No prefix in DB
                "search_derouter_code",  # No prefix in DB
            ],
        },
    )
    ...
    async def mock_get_tools_from_server(
        ...
        **kwargs,
        ...
        tool1 = MagicMock()
        tool1.name = "GITMCP-fetch_derouter_documentation"  # Prefixed
        tool1.description = "Fetch docs"
        tool1.inputSchema = {}

        tool2 = MagicMock()
        tool2.name = "GITMCP-search_derouter_documentation"  # Prefixed, not in allowed list
        tool2.description = "Search docs"
        tool2.inputSchema = {}

        tool3 = MagicMock()
        tool3.name = "GITMCP-search_derouter_code"  # Prefixed
    ...
    assert tool_names == [
        "GITMCP-fetch_derouter_documentation",
        "GITMCP-search_derouter_code",
    ]
...
def test_apply_tool_overrides():
    ...
    from derouter.proxy._experimental.mcp_server.server import apply_tool_overrides
    from derouter.types.mcp import MCPTransport
    from derouter.types.mcp_server.mcp_server_manager import MCPServer
...
def _make_db_mcp_server(server_id: str, updated_at: datetime) -> DeRouter_MCPServerTable:
    return DeRouter_MCPServerTable(
        server_id=server_id,
        server_name="server",
        alias="server",
...
class TestMCPServerManagerReload:
    ...
    async def test_reuses_existing_server_when_updated_at_matches(self):
        ...
        mock_prisma.db.derouter_mcpservertable.find_many = AsyncMock(return_value=[db_row])
        with (
            patch(
    ...
    async def test_skips_server_when_build_from_database_fails(self, caplog):
        ...
        mock_prisma = MagicMock()
        mock_prisma.db.derouter_mcpservertable.find_many = AsyncMock(
        ...
        with (
            ...
            caplog.at_level("ERROR", logger="DeRouter"),
        ):
            await manager.reload_servers_from_database()
    ...
    async def test_skips_server_when_openapi_registration_fails(self, caplog):
        ...
        mock_prisma = MagicMock()
        mock_prisma.db.derouter_mcpservertable.find_many = AsyncMock(return_value=[healthy_row, bad_openapi_row])
        with (
            patch(
                "derouter.proxy.management_endpoints.mcp_management_endpoints.get_prisma_client_or_throw",
                return_value=mock_prisma,
...
async def test_call_mcp_tool_logs_failure_via_post_call_failure_hook():
    ...
    try:
        ...
        from derouter.proxy._types import MCPTransport, UserAPIKeyAuth
        from derouter.types.mcp_server.mcp_server_manager import MCPServer
    except ImportError:
        pytest.skip("MCP server not available")
    ...
    with (
        ...
        patch(
            "derouter.proxy._experimental.mcp_server.server._get_allowed_mcp_servers_from_mcp_server_names",
            new_callable=AsyncMock,
            return_value=[mock_server],
        ),
        patch(
            "derouter.proxy._experimental.mcp_server.server.execute_mcp_tool",
            new_callable=AsyncMock,
            side_effect=Exception("boom"),
        ),
        patch(
            "derouter.proxy.proxy_server.proxy_logging_obj",
            proxy_logging_mock,
        ),
    ):
...
async def test_get_tools_from_mcp_servers_logs_list_tools_to_spendlogs_when_enabled():
    ...
    with (
        ...
        patch(
            "derouter.proxy._experimental.mcp_server.server.filter_tools_by_allowed_tools",
            side_effect=lambda tools, _server: tools,
        ),
        patch(
            "derouter.proxy._experimental.mcp_server.server.filter_tools_by_key_team_permissions",
            new=AsyncMock(side_effect=lambda tools, **_: tools),
        ),
        patch(
            "derouter.proxy._experimental.mcp_server.server.function_setup",
...
def test_tool_name_matches_case_insensitive():
    ...
    """
    try:
        from derouter.proxy._experimental.mcp_server.server import _tool_name_matches
    except ImportError:
        pytest.skip("MCP server not available")
...
def test_filter_tools_by_allowed_tools_case_insensitive():
    ...
    try:
        ...
        from derouter.types.mcp_server.tool_registry import MCPTool
    except ImportError:
        pytest.skip("MCP server not available")
...
def test_filter_tools_by_allowed_tools_no_filter():
    ...
    tools = [
        MCPTool(
            name="fusion_derouter_mcp-model_list",
            description="List models",
            input_schema={"type": "object"},
            handler=mock_handler,
        ),
        MCPTool(
            name="fusion_derouter_mcp-chat_completion",
...
class TestGatewayCreateInitializationOptions:
    ...
    def test_no_contextvar_returns_default_options(self):
        ...
        try:
            from derouter.proxy._experimental.mcp_server.mcp_context import (
                _mcp_gateway_initialize_instructions,
                _mcp_gateway_server_name,
            )
            from derouter.proxy._experimental.mcp_server.server import server
        except ImportError:
            pytest.skip("MCP server not available")
        ...
        try:
            ...
            assert opts.server_name == "derouter-mcp-server"
        finally:
    ...
    async def test_scoped_request_uses_configured_server_alias(self):
        ...
        assert server.create_initialization_options().server_name == "derouter-mcp-server"
    ...
    async def test_sse_handler_scopes_server_name_from_single_server_path(self):
        ...
        with (
            ...
            patch(
                "derouter.proxy._experimental.mcp_server.server._raise_preemptive_401_for_unauthenticated_servers",
                new_callable=AsyncMock,
            ),
            patch(
                "derouter.proxy._experimental.mcp_server.server._check_passthrough_upstream_auth",
                new_callable=AsyncMock,
            ),
            patch(
                "derouter.proxy._experimental.mcp_server.server._SESSION_MANAGERS_INITIALIZED",
...
async def test_probe_upstream_auth_returns_upstream_status():
    ...
    from derouter.proxy._experimental.mcp_server.server import _probe_upstream_auth
    ...
    with patch(
        "derouter.proxy._experimental.mcp_server.server.get_async_httpx_client",
...
def test_get_forwarded_auth_from_scope_extracts_header():
    """Returns Authorization value when x-derouter-api-key is also present."""
    from derouter.proxy._experimental.mcp_server.server import (
    ...
    scope = {
        "headers": [
            (b"content-type", b"application/json"),
            (b"x-derouter-api-key", b"sk-derouter-proxy-key"),
...
def test_get_forwarded_auth_from_scope_skips_when_no_derouter_key_header():
    """Skip when ``x-derouter-api-key`` is absent.

    Without ``x-derouter-api-key``, the ``Authorization`` header may itself be
    the DeRouter proxy API key (backward-compat). Forwarding it upstream would
...
def _patch_delegate_resolver(server: MCPServer, *resolvable_names: str):
    ...
    return patch(
        "derouter.proxy._experimental.mcp_server.server.global_mcp_server_manager.get_mcp_server_by_name",
...
async def test_delegate_bad_token_gets_connect_time_401():
    ...
    Delegate-mode clients send only ``Authorization`` (no ``x-derouter-api-key``),
    ...
    with (
        ...
        patch(
            "derouter.proxy._experimental.mcp_server.server._probe_upstream_auth",
...
async def test_bare_authorization_never_probes_passthrough_servers():
    """A bare ``Authorization`` header may be a DeRouter key (backward-compat), so
    ...
    unambiguous ``x-derouter-api-key`` + ``Authorization`` pair."""
    from derouter.proxy._experimental.mcp_server.server import (
        _check_passthrough_upstream_auth,
    )
    from derouter.proxy._types import UserAPIKeyAuth
...
async def test_delegate_not_probed_when_named_only_via_server_id():
    ...
    DeRouter key by ``process_mcp_request`` (its ``get_mcp_server_by_name`` misses),
    so the bare ``Authorization`` header is that DeRouter key. The probe must resolve
    ...
    scope = {
        ...
        "scheme": "http",
        "server": ("localhost", 4000),
        "headers": [(b"authorization", b"Bearer sk-derouter-proxy-key")],
    }
...
async def test_delegate_preflight_with_unpatched_probe():
    ...
    mocked only at the httpx-client boundary (tests/test_derouter is mocked-only; the
...
async def test_execute_mcp_tool_rest_server_id_authoritative_for_unprefixed_tool():
    ...
    from derouter.proxy._experimental.mcp_server import server as mcp_module
...
async def test_execute_mcp_tool_rest_server_id_injects_requested_server_credentials():
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.proxy_logging_obj", None),
    ):
        await mcp_module.execute_mcp_tool(
            name="echo",
            arguments={"message": "hello"},
...
async def test_execute_mcp_tool_sets_model_in_model_call_details():
    ...
    from derouter.utils import Rules, function_setup
    ...
    derouter_logging_obj, _ = function_setup(
    ...
    assert derouter_logging_obj.model_call_details.get("model") is None
    ...
    with (
        ...
        patch(
            "derouter.proxy._experimental.mcp_server.server._handle_local_mcp_tool",
            new=AsyncMock(return_value=[]),
        ),
        patch(
            "derouter.proxy._experimental.mcp_server.server.MCPRequestHandler.is_tool_allowed",
            return_value=True,
        ),
    ):
        await mcp_module.execute_mcp_tool(
    ...
    assert derouter_logging_obj.model_call_details["model"] == "MCP: list_pets"
    assert derouter_logging_obj.model == "MCP: list_pets"
...
async def test_get_allowed_mcp_servers_from_mcp_server_names_unknown_name_fails_closed():
    ...
    with patch(
        "derouter.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp."
...
class TestStreamableHttpAuthErrorMapping:
    ...
    async def test_streamable_http_propagates_proxy_exception_as_401(self):
        ...
        scope = {
            ...
            "headers": [(b"x-derouter-api-key", b"sk-bad")],
        }
...
class TestMCPMetaTraceCarrier:
    ...
    def test_extracts_trace_context_and_excludes_baggage_and_other_meta(self):
        ...
        (e.g. ``derouter.team.id``). Dropping it at the source is the regression guard."""
        from types import SimpleNamespace
        ...
        meta = RequestParams.Meta.model_validate(
            {
                ...
                "tracestate": "rojo=1",
                "baggage": "derouter.team.id=spoofed-team,derouter.metadata.user_api_key_user_id=attacker",
                "progressToken": "p1",
            }
        )
...
async def test_stateful_mcp_tool_call_uses_current_requests_otel_destinations() -> None:
    ...
    from derouter.integrations.otel.plumbing.context import (
...
async def test_get_allowed_mcp_servers_includes_active_servers_submitted_by_user():
    ...
    with (
        ...
        patch(
            "derouter.proxy._experimental.mcp_server.db.get_active_submitted_mcp_server_ids_for_user",
...
async def test_get_active_submitted_mcp_server_ids_for_user_queries_active_rows():
    ...
    from derouter.proxy._types import MCPApprovalStatus

    row = MagicMock()
    row.server_id = "submitted-1"
    prisma_client = MagicMock()
    prisma_client.db.derouter_mcpservertable.find_many = AsyncMock(return_value=[row])

    result = await get_active_submitted_mcp_server_ids_for_user(prisma_client, "submitter-user")

    assert result == ["submitted-1"]
    prisma_client.db.derouter_mcpservertable.find_many.assert_awaited_once_with(
        where={
...
async def test_get_active_submitted_mcp_server_ids_for_user_empty_user_id_skips_db():
    ...
    prisma_client = MagicMock()
    prisma_client.db.derouter_mcpservertable.find_many = AsyncMock()

    assert await get_active_submitted_mcp_server_ids_for_user(prisma_client, "") == []
    prisma_client.db.derouter_mcpservertable.find_many.assert_not_awaited()
...
def test_extract_mcp_tool_result_error_message():
    from derouter.proxy._experimental.mcp_server.utils import (
...
async def test_fire_mcp_tool_call_logging_iserror_logs_failure():
    ...
    from derouter.proxy._experimental.mcp_server.exceptions import MCPToolResultError

    logging_obj = _mock_mcp_logging_obj()
    proxy_logging_mock = _mock_mcp_proxy_logging()
    user_auth = UserAPIKeyAuth(api_key="test-key", user_id="test-user")

    with patch("derouter.proxy.proxy_server.proxy_logging_obj", proxy_logging_mock):
        await _fire_mcp_tool_call_logging(
            ...
            user_api_key_auth=user_auth,
            request_data={"derouter_call_id": "cid"},
        )
...
async def test_fire_mcp_tool_call_logging_strips_credentials_from_failure_hook():
    ...
    assert hook_request_data == {"name": "explode", "derouter_call_id": "cid"}
...
async def test_fire_mcp_tool_call_logging_iserror_emits_otel_error_span(monkeypatch):
    ...
    import derouter
    from derouter.integrations.otel import OpenTelemetryV2Config
    from derouter.integrations.otel.logger import OpenTelemetryV2
    from derouter.integrations.otel.plumbing import providers
    from derouter.proxy._experimental.mcp_server.server import (
        _fire_mcp_tool_call_logging,
    )
    ...
    monkeypatch.setattr(derouter, "failure_callback", [])
    monkeypatch.setattr(derouter, "_async_failure_callback", [otel_logger])
    monkeypatch.setattr(derouter, "success_callback", [])
    monkeypatch.setattr(derouter, "_async_success_callback", [otel_logger])
...
def test_redact_mcp_resource_url_strips_credentials(url, expected):
    ...
    from derouter.proxy._experimental.mcp_server.server import _redact_mcp_resource_url
...
async def test_call_mcp_tool_skips_failure_hook_for_upstream_auth_error():
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.proxy_logging_obj", proxy_logging_mock),
    ):
...
async def test_aggregate_listing_reports_per_server_outcomes():
    ...
    from derouter.proxy._experimental.mcp_server.exceptions import MCPServerListError
    from derouter.proxy._experimental.mcp_server.faults.list_outcomes import ServerListFault
...
async def test_outcome_keys_use_display_prefix_never_canonical_names():
    ...
    try:
        from derouter.proxy._experimental.mcp_server.server import _aggregate_server_key
    except ImportError:
        pytest.skip("MCP server not available")
...
async def test_handle_list_tools_attaches_outcome_meta():
    ...
    from derouter.proxy._experimental.mcp_server.faults.list_outcomes import (
...
class TestPreemptive401ModeAware:
    ...
    Regression guard for the bug where a DeRouter virtual key presented as
    ...
    DEROUTER_KEY_HEADERS = {"Authorization": "Bearer sk-derouter-virtual-key"}
    ...
    async def _run(self, server, oauth2_headers, has_stored_token: bool):
        from derouter.proxy._experimental.mcp_server import server as server_module
        ...
        with (
            ...
            await server_module._raise_preemptive_401_for_unauthenticated_servers(
                ...
                oauth2_headers=oauth2_headers,
                mcp_server_auth_headers=None,
                user_api_key_auth=UserAPIKeyAuth(api_key="sk-derouter-virtual-key"),
                client_ip=None,
            )
    ...
    @pytest.mark.asyncio
    async def test_gateway_managed_interactive_no_token_challenges_with_x_derouter_api_key(self):
        """No stored token, key in x-derouter-api-key (oauth2_headers empty): 401."""
    ...
    @pytest.mark.parametrize(
        ...
        (
            ("/derouter/mcp/interactive", "/derouter/.well-known/oauth-authorization-server/derouter/mcp/interactive"),
            ("/derouter/interactive/mcp", "/derouter/.well-known/oauth-authorization-server/derouter/interactive"),
        ),
    )
    ...
    async def test_gateway_managed_interactive_with_stored_token_does_not_challenge(self):
        ...
        await self._run(_make_oauth2_server("interactive"), self.DEROUTER_KEY_HEADERS, has_stored_token=True)
    ...
    async def test_m2m_never_challenges(self):
        ...
        await self._run(m2m, self.DEROUTER_KEY_HEADERS, has_stored_token=False)
    ...
    async def test_unstamped_m2m_shape_never_challenges(self):
        ...
        await self._run(unstamped, self.DEROUTER_KEY_HEADERS, has_stored_token=False)
    ...
    async def test_delegate_challenges_only_when_bearer_absent(self):
        ...
        assert exc.value.status_code == 401
        await self._run(delegate, self.DEROUTER_KEY_HEADERS, has_stored_token=False)
    ...
    async def _run(self, server: MCPServer, mcp_servers: list[str], preflight: AsyncMock) -> None:
        ...
        with (
            ...
            await server_module._raise_preemptive_401_for_unauthenticated_servers(
                ...
                mcp_servers=mcp_servers,
                oauth2_headers={"Authorization": "Bearer sk-derouter-virtual-key"},
                mcp_server_auth_headers=None,
                user_api_key_auth=UserAPIKeyAuth(api_key="sk-derouter-virtual-key", user_id="u-1"),
                client_ip=None,
            )
...
async def test_post_mcp_call_guardrails_run_without_a_logging_object():
    ...
    A tool call dispatched without a derouter_logging_obj (tool search, and any
...
async def test_post_mcp_call_guardrails_propagate_a_block():
    ...
    from derouter.exceptions import BlockedPiiEntityError
    from derouter.proxy._experimental.mcp_server.server import (
        _run_post_mcp_call_guardrails,
    )
    ...
    with patch("derouter.proxy.proxy_server.proxy_logging_obj", proxy_logging_mock):
        with pytest.raises(BlockedPiiEntityError):
            await _run_post_mcp_call_guardrails(
                ...
                derouter_logging_obj=_mock_mcp_logging_obj(),
                user_api_key_auth=None,
                request_data={},
            )
...
class TestListFiltersHonorThePrefixBoundary:
    ...
    def _alias_less_server(**overrides):
        from derouter.proxy._experimental.mcp_server.mcp_server_manager import MCPServer
    ...
    def test_alias_form_entry_matches_a_tool_published_under_the_short_prefix(self, monkeypatch):
        ...
        monkeypatch.setenv("DEROUTER_USE_SHORT_MCP_TOOL_PREFIX", "true")
        server = MCPServer(
    ...
    async def test_key_team_listing_and_dispatch_agree(self, grants, expected):
        ...
        with (
            ...
            patch("derouter.proxy._experimental.mcp_server.server.global_mcp_server_manager") as mock_manager,
        ):
...
async def test_list_tools_injects_byok_credential_for_non_oauth2_auth_types(auth_type):
    ...
    with (
        ...
        patch(
            "derouter.proxy._experimental.mcp_server.server._get_byok_credential",
