...
When clients reconnect to DeRouter's MCP endpoint after a server restart or reload,
...
async def test_per_user_oauth_missing_stored_token_returns_preemptive_401():
    ...
    with (
        ...
        patch(
            "derouter.proxy._experimental.mcp_server.server._handle_stale_mcp_session",
            new_callable=AsyncMock,
            return_value=False,
        ),
        patch(
            "derouter.proxy._experimental.mcp_server.server.global_mcp_server_manager.has_user_oauth_token",
            new_callable=AsyncMock,
            return_value=False,
        ) as mock_has_token,
        patch(
...
async def test_admitted_subject_missing_stored_token_challenged_with_resource_metadata():
    ...
    for a keyless client (its token request carries no derouter credential), so sending an
...
async def test_client_credentials_server_is_not_preemptively_challenged(m2m_fields):
    ...
    with (
        ...
        patch("derouter.proxy._experimental.mcp_server.server._SESSION_MANAGERS_INITIALIZED", True),
        patch(
            "derouter.proxy._experimental.mcp_server.server._handle_stale_mcp_session",
            new_callable=AsyncMock,
            return_value=False,
...
async def test_handle_streamable_http_mcp_delegated_server_surfaces_upstream_challenge():
    ...
    scope = {
        ...
        "query_string": b"",
        "root_path": "",
        "server": ("derouter.example.com", 443),
        "headers": [
            (b"content-type", b"application/json"),
            (b"host", b"derouter.example.com"),
        ],
    }
    receive = AsyncMock(
        return_value={
...
async def test_handle_streamable_http_mcp_oauth_delegate_without_token_returns_gateway_proxied_401():
    """oauth_delegate is admitted with the DeRouter key but still owns upstream
...
async def test_handle_streamable_http_mcp_oauth_delegate_with_forwarded_token_skips_challenge():
    """When the oauth_delegate caller carries both the DeRouter key and a separate
    ...
    scope = _passthrough_mode_scope(
        "od_server",
        extra_headers=[
            (b"x-derouter-api-key", b"Bearer sk-1234"),
...
async def test_handle_streamable_http_mcp_per_server_header_skips_preemptive_challenge(auth_type):
    ...
    try:
        from derouter.proxy._experimental.mcp_server.server import handle_streamable_http_mcp  # noqa: F401
    except ImportError:
        pytest.skip("MCP server not available")
