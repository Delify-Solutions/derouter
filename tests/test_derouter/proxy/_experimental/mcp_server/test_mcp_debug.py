...
from derouter.proxy._experimental.mcp_server.mcp_debug import (
...
class TestIsDebugEnabled:
    ...
    def test_case_insensitive_header_name(self):
        assert MCPDebug.is_debug_enabled({"X-DeRouter-MCP-Debug": "true"}) is True
...
class TestMask:
    ...
    def test_derouter_key_masked(self):
...
class TestBuildDebugHeaders:
    ...
    def test_derouter_key_in_dedicated_header(self):
        headers = MCPDebug.build_debug_headers(
            inbound_headers={
                "x-derouter-api-key": "Bearer sk-1234567890abcdef",
                "host": "localhost",
            },
            oauth2_headers=None,
            derouter_api_key="Bearer sk-1234567890abcdef",
            auth_resolution="no-auth",
            server_url="https://mcp.example.com",
            server_auth_type="oauth2",
        )
        assert "x-derouter-api-key=" in headers["x-mcp-debug-inbound-auth"]
        assert headers["x-mcp-debug-oauth2-token"] == "(none)"

    def test_same_key_flagged(self):
        """When Authorization and x-derouter-api-key carry the same token."""
        headers = MCPDebug.build_debug_headers(
            inbound_headers={
        ...
        assert "SAME_AS_DEROUTER_KEY" in headers["x-mcp-debug-oauth2-token"]

    def test_different_tokens_not_flagged(self):
        """When OAuth2 token is different from DeRouter key."""
        headers = MCPDebug.build_debug_headers(
            inbound_headers={
                "x-derouter-api-key": "Bearer sk-derouter-key-here",
                "authorization": "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.atlassian",
            },
            oauth2_headers={
                "Authorization": "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.atlassian"
            },
            derouter_api_key="Bearer sk-derouter-key-here",
            auth_resolution="oauth2-passthrough",
            server_url="https://mcp.atlassian.com/v1/mcp",
            server_auth_type="oauth2",
        )
        assert "SAME_AS_DEROUTER_KEY" not in headers["x-mcp-debug-oauth2-token"]
        assert headers["x-mcp-debug-auth-resolution"] == "oauth2-passthrough"

    def test_m2m_resolution(self):
        headers = MCPDebug.build_debug_headers(
            inbound_headers={"x-derouter-api-key": "Bearer sk-key"},
            oauth2_headers=None,
            derouter_api_key="Bearer sk-key",
