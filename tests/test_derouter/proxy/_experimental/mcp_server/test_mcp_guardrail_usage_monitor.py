...
``derouter_logging_obj`` and bridge those records onto it. These tests pin both the
seeding (which unified guardrails consume off ``data["derouter_logging_obj"]``) and
...
from derouter.proxy._experimental.mcp_server import mcp_server_manager as MOD


class _FakeLoggingObj:
    """Minimal stand-in for ``DeRouterLoggingObj``.

    ``_sync_guardrail_info_to_logging_obj`` reads exactly these two attributes,
    and the spend-log payload is built from ``derouter_params["metadata"]``, so a
    real ``Logging`` instance would add setup cost without adding coverage.
    """

    def __init__(self) -> None:
        self.derouter_params: dict[str, Any] = {"metadata": {}}
        self.model_call_details: dict[str, Any] = {"derouter_params": self.derouter_params}

    @property
    def recorded_guardrails(self) -> list:
        return self.derouter_params["metadata"].get("standard_logging_guardrail_information", [])
...
async def test_pre_call_seeds_request_logging_obj_for_unified_guardrails():
    """Unified guardrails read ``data["derouter_logging_obj"]`` and pass it into
    ...
    assert capture["data"]["derouter_logging_obj"] is logging_obj
...
async def test_pre_call_without_logging_obj_is_unchanged():
    ...
    assert result == {}
    assert capture["data"]["derouter_logging_obj"] is None
...
async def test_call_tool_threads_logging_obj_into_both_hooks():
    ...
    async def _fake_pre_call_tool_check(**kwargs):
        seen["pre_call"] = kwargs.get("derouter_logging_obj")
        return {}

    def _fake_during_hook_task(**kwargs):
        seen["during_call"] = kwargs.get("derouter_logging_obj")
