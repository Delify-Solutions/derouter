...
def test_refresh_jwt_wearing_the_access_prefix_is_rejected_at_the_edge():
    ...
    from derouter.proxy._experimental.mcp_server.outbound_credentials.envelope import REFRESH_ENVELOPE_PREFIX
...
def test_resolve_non_envelope_is_not_bridge_envelope():
    keys = envelope_keys_from_master_key(_MASTER_KEY)
    assert isinstance(resolve_bridge_envelope("Bearer sk-some-derouter-key", keys, _NOW, _SERVER_ID), NotBridgeEnvelope)
...
def test_is_bridge_envelope_shaped_rejects_non_envelope_bearer():
    assert is_bridge_envelope_shaped("Bearer sk-some-derouter-key") is False
