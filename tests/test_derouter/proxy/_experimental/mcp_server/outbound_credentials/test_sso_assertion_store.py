...
from derouter.proxy.common_utils.encrypt_decrypt_utils import decrypt_value_helper, encrypt_value_helper
...
def _make_prisma(stored: dict, db_has_id_jag_server: bool = False):
    ...
    prisma = MagicMock()
    prisma.db.derouter_mcpservertable.find_first = AsyncMock(return_value=MagicMock() if db_has_id_jag_server else None)
    ...
    prisma.db.derouter_ssoidentityassertion.upsert = AsyncMock(side_effect=_upsert)
    prisma.db.derouter_ssoidentityassertion.find_unique = AsyncMock(side_effect=_find_unique)
    prisma.db.derouter_ssoidentityassertion.find_many = AsyncMock(side_effect=_find_many)
    prisma.db.derouter_ssoidentityassertion.update = AsyncMock(side_effect=_update)
    return prisma
...
async def test_retention_gate_requires_an_id_jag_server():
    with (
        patch("derouter.proxy._experimental.mcp_server.mcp_server_manager.global_mcp_server_manager") as manager,
        patch("derouter.proxy.proxy_server.prisma_client", _make_prisma({}, db_has_id_jag_server=False)),
    ):
...
async def test_retention_gate_reads_the_db_when_config_declares_no_id_jag_server():
    ...
    with patch("derouter.proxy._experimental.mcp_server.mcp_server_manager.global_mcp_server_manager") as manager:
        manager.config_mcp_servers = {"s1": _server_with_auth(MCPAuth.oauth2)}
        db_backed = _make_prisma({}, db_has_id_jag_server=True)
        with patch("derouter.proxy.proxy_server.prisma_client", db_backed):
            assert await ema_assertion_retention_enabled() is True
        db_backed.db.derouter_mcpservertable.find_first.assert_awaited_once_with(
...
async def test_fetch_serves_second_read_from_cache_without_db_read():
    ...
    token = _make_id_token()
    assertion = assertion_from_sso_login(token, "rt_1")
    with patch("derouter.proxy.proxy_server.prisma_client", prisma):  # test-quality-ok: fake DB seam has no injection boundary
    ...
    prisma.db.derouter_ssoidentityassertion.find_unique.assert_awaited_once()
...
async def test_fetch_racing_a_relogin_does_not_cache_the_previous_assertion():
    ...
    unpaused_find_unique = prisma.db.derouter_ssoidentityassertion.find_unique
    ...
    prisma.db.derouter_ssoidentityassertion.find_unique = _paused_find_unique
    with patch("derouter.proxy.proxy_server.prisma_client", prisma):  # test-quality-ok: fake DB seam has no injection boundary
        await persist_sso_identity_assertion("user-a", assertion_from_sso_login(first_token, None), cache=cache)
...
async def test_fetch_does_not_cache_a_missing_row():
    ...
    assert prisma.db.derouter_ssoidentityassertion.find_unique.await_count == 2
...
async def test_retain_noop_when_no_id_jag_server():
    ...
    prisma.db.derouter_ssoidentityassertion.upsert.assert_not_called()
    assert stored == {}
...
async def test_retain_none_assertion_never_consults_gate_or_store():
    gate = MagicMock()
    with patch(
        "derouter.proxy._experimental.mcp_server.outbound_credentials.sso_assertion_store.ema_assertion_retention_enabled",
        gate,
    ):
...
async def test_retain_swallows_store_failure():
    prisma = MagicMock()
    prisma.db.derouter_ssoidentityassertion.upsert = AsyncMock(side_effect=RuntimeError("db down"))
    with (
        patch("derouter.proxy._experimental.mcp_server.mcp_server_manager.global_mcp_server_manager") as manager,
        patch("derouter.proxy.proxy_server.prisma_client", prisma),
    ):
...
async def test_db_store_converts_a_driver_failure_into_assertion_store_unavailable():
    ...
    prisma = MagicMock()
    prisma.db.derouter_ssoidentityassertion.find_unique = AsyncMock(side_effect=RuntimeError("connection refused"))
    with patch("derouter.proxy.proxy_server.prisma_client", prisma):
...
async def test_db_store_returns_none_for_a_user_with_no_stored_assertion():
    ...
    with patch.dict(os.environ, {"DEROUTER_SALT_KEY": SALT_KEY}):
        with patch("derouter.proxy.proxy_server.prisma_client", _make_prisma({})):
