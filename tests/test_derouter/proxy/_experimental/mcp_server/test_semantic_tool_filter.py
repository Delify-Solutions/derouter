...
async def test_semantic_filter_basic_filtering():
    ...
    """
    from derouter.proxy._experimental.mcp_server.semantic_tool_filter import (
    ...
    from derouter.types.utils import Embedding, EmbeddingResponse
    ...
    filter_instance = SemanticMCPToolFilter(
        ...
        derouter_router_instance=mock_router,
        top_k=3,
...
async def test_semantic_filter_hook_preserves_native_tools():
    ...
    assert "derouter_semantic_filter_stats" in result["metadata"]

    # Stats should report MCP-only counts, not inflated with native tools
    stats = result["metadata"]["derouter_semantic_filter_stats"]
...
async def test_semantic_filter_hook_all_native_tools():
    ...
    Then:  All tools pass through, and NO spurious semantic filter response
           headers are emitted (no derouter_semantic_filter_stats in metadata).
    """
    from derouter.proxy._experimental.mcp_server.semantic_tool_filter import (
        SemanticMCPToolFilter,
    )
    ...
    assert (
        "derouter_semantic_filter_stats" not in result["metadata"]
...
@requires_semantic_router
@pytest.mark.asyncio
async def test_semantic_filter_hook_filters_expanded_derouter_proxy_tools():
    """
    Regression test (LIT-4214): derouter_proxy MCP references must be
    semantically filtered after expansion, with real filter stats.

    Given: A /v1/responses-style request whose tools are a single
           {"type": "mcp", "server_url": "derouter_proxy"} reference that
           expands to 5 flat OpenAI function dicts
    When:  The hook processes the request
    Then:  The expanded tools go through the semantic filter (top_k=2)
           and derouter_semantic_filter_stats reports pre/post counts, so
           the x-derouter-semantic-filter header shows how many tools
    ...
    assert len(mcp_references) == 1, "The derouter_proxy MCP reference must be preserved for the MCP gateway to expand"
    ...
    assert (
        "derouter_semantic_filter_stats" in result["metadata"]
    ), "Filter stats must be emitted for the derouter_proxy expansion path"
    stats = result["metadata"]["derouter_semantic_filter_stats"]
    ...
    print(f"✅ Expanded derouter_proxy tools filtered: {len(expanded_tools)} -> {len(allowed_tools)}, stats={stats}")
...
async def test_semantic_filter_hook_narrows_mcp_reference_for_chat_completions():
    """
    Regression test (LIT-4451): the hook must narrow the derouter_proxy MCP
    ...
    assert forwarded[0]["server_url"] == "derouter_proxy", "The MCP reference must keep routing to the gateway"
...
async def test_semantic_filter_hook_zero_matches_exposes_all_tools_on_both_paths():
    ...
    carries a derouter_proxy MCP reference or plain MCP tool objects.
...
async def test_semantic_filter_hook_expansion_skips_filter_when_disabled():
    ...
    assert (
        "derouter_semantic_filter_stats" not in data["metadata"]
...
class TestGetToolsByNames:
    ...
    the incoming name is ``derouter_fc_web_search-firecrawl_scrape``.
    ...
    def test_client_prefix_with_underscore_separator(self):
        ...
        client_name = "derouter_" + canonical
    ...
    def test_exact_match_preferred_over_prefixed(self):
        ...
        available_tools = [
            ...
            {"name": "derouter_" + canonical, "description": "wrapped"},
        ]
    ...
    def test_same_tool_not_returned_twice(self):
        ...
        separator-anchored suffixes of ``derouter_api-fs-read_file``.
        """
        filter_instance = self._make_filter()
        available_tools = [{"name": "derouter_api-fs-read_file", "description": "read"}]
    ...
    def test_ordering_follows_router_output(self):
        ...
        filter_instance = self._make_filter()
        available_tools = [
            {"name": "derouter_fs-read", "description": "read"},
            {"name": "derouter_fs-write", "description": "write"},
            {"name": "derouter_fs-delete", "description": "delete"},
        ]
...
async def test_semantic_filter_headers_hook_emits_only_complete_tool_names():
    ...
    The x-derouter-semantic-filter-tools header used to be sliced mid-name at
    ...
    """
    from derouter.constants import MAX_MCP_SEMANTIC_FILTER_TOOLS_HEADER_LENGTH
    from derouter.proxy._experimental.mcp_server.semantic_tool_filter import (
        SemanticMCPToolFilter,
    )
    from derouter.proxy.hooks.mcp_semantic_filter import SemanticToolFilterHook
    ...
    data = {
        "metadata": {
            "derouter_semantic_filter_stats": "40->8",
            "derouter_semantic_filter_tools": ",".join(tool_names),
        }
    }
    ...
    assert headers["x-derouter-semantic-filter"] == "40->8"

    tools_header = headers["x-derouter-semantic-filter-tools"]
...
def test_truncate_csv_at_tool_name_boundary_edges():
    from derouter.proxy.hooks.mcp_semantic_filter.hook import (
...
def _make_context_window_raising_router(state):
    """
    Mock derouter Router whose embedding call raises ContextWindowExceededError
...
def _make_context_window_filter(state, top_k: int = 3):
    ...
    return SemanticMCPToolFilter(
        embedding_model="text-embedding-3-small",
        derouter_router_instance=_make_context_window_raising_router(state),
        top_k=top_k,
        similarity_threshold=0.3,
        enabled=True,
    )
...
async def test_semantic_filter_hook_fails_closed_on_expanded_tools_context_window_error():
    """
    Regression test (LIT-4284): the derouter_proxy MCP expansion path (driven
...
def test_is_context_window_error_detection_variants():
    ...
    cwe = derouter.ContextWindowExceededError(
        message="Invalid 'input[0]': maximum input length is 8192 tokens.",
        model="text-embedding-3-small",
        llm_provider="openai",
    )
    ...
    with pytest.raises(ValueError, match="Internal_derouter_router API call failed") as explicitly_chained:
        raise ValueError("Internal_derouter_router API call failed") from cwe
    ...
    def wrap_without_explicit_chaining():
        ...
        except derouter.ContextWindowExceededError:
...
def test_is_context_window_error_sees_through_trees_the_chain_walk_missed():
    ...
    def _cwe() -> derouter.ContextWindowExceededError:
        return derouter.ContextWindowExceededError(message="overflow", model="m", llm_provider="openai")
...
def _make_keyword_embedding_router(recorded_inputs):
    """
    Mock derouter Router whose embeddings are deterministic keyword one-hots:
...
def _make_keyword_filter(recorded_inputs, top_k: int = 3):
    ...
    return SemanticMCPToolFilter(
        embedding_model="text-embedding-3-small",
        derouter_router_instance=_make_keyword_embedding_router(recorded_inputs),
        top_k=top_k,
        similarity_threshold=0.3,
        enabled=True,
    )
...
async def test_semantic_filter_hook_narrows_only_references_the_gateway_serves():
    ...
    gateway_reference = {"type": "mcp", "server_url": "derouter_proxy", "require_approval": "never"}
    external_tool = {
        "type": "mcp",
        "server_label": "zapier",
