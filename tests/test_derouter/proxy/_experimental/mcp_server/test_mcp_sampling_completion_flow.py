...
class TestRunGuardrailsAndCallLlm:
    ...
    async def test_should_propagate_guardrail_rejection(self):
        ...
        with patch("derouter.proxy.proxy_server.proxy_logging_obj", plo):
...
class TestHandleSamplingCreateMessagePipeline:
    async def test_should_return_message_result_on_success(self):
        ...
        with (
            patch(
                "derouter.proxy._experimental.mcp_server.sampling_handler._resolve_model_from_preferences",
                return_value="gpt-4o",
            ),
            patch(
                "derouter.proxy._experimental.mcp_server.sampling_handler._check_model_access",
                new_callable=AsyncMock,
                return_value=None,
            ),
            patch(
                "derouter.proxy._experimental.mcp_server.sampling_handler._run_budget_checks",
                new_callable=AsyncMock,
                return_value=None,
            ),
            patch(
                "derouter.proxy._experimental.mcp_server.sampling_handler._build_completion_kwargs",
                new_callable=AsyncMock,
                return_value={"model": "gpt-4o", "messages": []},
            ),
            patch(
                "derouter.proxy._experimental.mcp_server.sampling_handler._run_guardrails_and_call_llm",
                new_callable=AsyncMock,
