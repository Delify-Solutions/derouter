...
from derouter.constants import MCP_TOOL_LISTING_TIMEOUT
from derouter.proxy._experimental.mcp_server import rest_endpoints
from derouter.proxy._experimental.mcp_server.auth import (
...
class TestExecuteWithMcpClient:
    ...
    async def test_m2m_drops_incoming_oauth2_headers(self, monkeypatch):
        ...
        the derouter API key) must NOT be forwarded as extra_headers — otherwise
        ...
        incoming_oauth2 = {"Authorization": "Bearer sk-derouter-api-key"}
        result = await rest_endpoints._execute_with_mcp_client(
            payload,
            ok_operation,
    ...
    async def test_m2m_does_not_build_presented_store(self, monkeypatch):
        ...
        result = await rest_endpoints._execute_with_mcp_client(
            payload,
            ok_operation,
            oauth2_headers={"Authorization": "Bearer sk-derouter-api-key"},
        )
...
class TestTestConnection:
    ...
    async def test_forwards_staged_oauth2_bearer(self, monkeypatch):
        ...
        request = _build_request(
            {"x-derouter-api-key": "sk-admin-session", "authorization": "Bearer upstream-oauth-token"},
    ...
    async def test_forwards_staged_auth_value(self, monkeypatch):
        ...
        captured = self._capture_execute(monkeypatch)
        request = _build_request({"x-derouter-api-key": "sk-admin-session"}, path="/mcp-rest/test/connection")
        payload = NewMCPServerRequest(
            server_name="example",
    ...
    async def test_does_not_forward_authorization_that_satisfied_admission(self, monkeypatch):
        """With no x-derouter-api-key, the Authorization value is the caller's DeRouter key and
        must never reach the upstream."""
        from derouter.proxy._types import LitellmUserRoles

        captured = self._capture_execute(monkeypatch)
        request = _build_request({"authorization": "Bearer sk-derouter-admission-key"}, path="/mcp-rest/test/connection")
...
class TestTestToolsList:
    ...
    async def test_extracts_oauth2_headers(self, monkeypatch):
        ...
        request = _build_request({"authorization": "Bearer incoming", "x-derouter-api-key": "sk-admission"})
        payload = NewMCPServerRequest(
            server_name="example",
            url="https://example.com",
            auth_type=MCPAuth.oauth2,
    ...
    async def test_extracts_oauth2_headers_for_client_forwarded_modes(self, monkeypatch, auth_type):
        ...
        request = _build_request({"authorization": "Bearer upstream-token", "x-derouter-api-key": "sk-admission"})
        payload = NewMCPServerRequest(
            server_name="example",
            url="https://example.com",
            auth_type=auth_type,
    ...
    async def test_does_not_forward_authorization_that_satisfied_admission(self, monkeypatch, auth_type):
        """Authorization is also the admission fallback: with no x-derouter-api-key on the request,
        the Authorization value is the caller's DeRouter key, so forwarding it would send the
        ...
        request = _build_request({"authorization": "Bearer sk-derouter-admission-key"})
        payload = NewMCPServerRequest(
            server_name="example",
            url="https://example.com",
            auth_type=auth_type,
...
class TestListToolsRestAPI:
    ...
    async def test_single_server_response_includes_paginated_upstream_tools(
        ...
        import derouter.experimental_mcp_client.client as mcp_client_module
        from mcp.types import ListToolsResult, PaginatedRequestParams
        from mcp.types import Tool as MCPTool

        from derouter.proxy._experimental.mcp_server.server import MCPServer
...
class TestCallToolRestAPI:
    ...
    async def test_rejects_disallowed_server(self, monkeypatch):
        ...
        async def fake_add_derouter_data_to_request(**kwargs):
        ...
        monkeypatch.setattr(
            "derouter.proxy.proxy_server.add_derouter_data_to_request",
            fake_add_derouter_data_to_request,
            raising=False,
        )
    ...
    async def test_returns_guardrail_rewritten_tool_result(self, monkeypatch):
        ...
        monkeypatch.setattr("derouter.proxy.proxy_server.proxy_config", {}, raising=False)
    ...
    async def test_local_permission_denial_keeps_error_level_logging(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "derouter.proxy.proxy_server.add_derouter_data_to_request", fake_add_derouter_data_to_request, raising=False
        )
        monkeypatch.setattr("derouter.proxy.proxy_server.proxy_config", {}, raising=False)
        monkeypatch.setattr(rest_endpoints, "execute_mcp_tool", fake_execute_mcp_tool, raising=False)
        mock_logger = MagicMock()
    ...
    async def test_virtual_mcp_tool_call_relays_upstream_auth_failure(self, monkeypatch, upstream_status):
        ...
        import derouter.proxy._experimental.mcp_server.tool_search as tool_search_mod
        from derouter.proxy._experimental.mcp_server.exceptions import MCPUpstreamAuthError
        from derouter.proxy._types import DeRouter_ObjectPermissionTable
        ...
        monkeypatch.setattr("derouter.proxy.proxy_server.proxy_config", {}, raising=False)
        monkeypatch.setattr("derouter.proxy.proxy_server.general_settings", {}, raising=False)
