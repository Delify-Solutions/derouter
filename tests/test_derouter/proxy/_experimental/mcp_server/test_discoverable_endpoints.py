...
async def test_vendor_credential_state_agrees_with_egress_token_resolution(monkeypatch, fields, egress_has_token):
    from derouter.proxy._experimental.mcp_server import db as mcp_db
    from derouter.proxy._experimental.mcp_server import discoverable_endpoints
...
def mock_mcp_client_ip():
    ...
    """
    with patch(
        "derouter.proxy._experimental.mcp_server.discoverable_endpoints.IPAddressUtils.get_mcp_client_ip",
        return_value=None,
    ):
        yield
...
def isolate_global_mcp_registry():
    ...
    """
    from derouter.proxy._experimental.mcp_server.mcp_server_manager import global_mcp_server_manager
...
def _unresolved_oauth_server():
    from derouter.proxy._types import MCPTransport
    from derouter.types.mcp_server.mcp_server_manager import MCPServer
...
def _resolved_oauth_metadata():
    from derouter.types.mcp_server.mcp_server_manager import MCPOAuthMetadata
...
async def test_authorize_resolves_cold_oauth_metadata(monkeypatch):
    ...
    monkeypatch.setenv("DEROUTER_SALT_KEY", "sk-test-salt-for-lit-6255")
    server = _unresolved_oauth_server()
    global_mcp_server_manager.registry[server.server_id] = server
    global_mcp_server_manager._set_oauth_discovery_deferred(server.server_id, True)
    request = _mock_callback_request("https://derouter.example.com/")
...
def trust_xff():
    ...
    """
    with patch(
        "derouter.proxy._experimental.mcp_server.discoverable_endpoints.IPAddressUtils.is_request_from_trusted_proxy",
        return_value=True,
    ):
        yield
...
async def test_authorize_endpoint_includes_response_type():
    ...
    try:
        ...
        from derouter.proxy._experimental.mcp_server.discoverable_endpoints import (
            authorize,
        )
    ...
    mock_request.base_url = "https://derouter.example.com/"
    mock_request.headers = {}

    # Mock the encryption functions to avoid needing a signing key
    with patch("derouter.proxy._experimental.mcp_server.discoverable_endpoints.encrypt_value_helper") as mock_encrypt:
...
async def test_authorize_endpoint_forwards_pkce_parameters():
    ...
    # Mock request
    mock_request = MagicMock(spec=Request)
    mock_request.base_url = "https://derouter-proxy.example.com/"
    mock_request.headers = {}
...
async def test_token_endpoint_forwards_code_verifier():
    ...
    with patch(
        "derouter.proxy._experimental.mcp_server.discoverable_endpoints.get_async_httpx_client"
    ) as mock_get_client:
...
async def test_register_client_without_mcp_server_name_returns_dummy():
    ...
    mock_request = MagicMock(spec=Request)
    mock_request.base_url = "https://proxy.derouter.example/"
    mock_request.headers = {}
    with patch(
        "derouter.proxy._experimental.mcp_server.discoverable_endpoints._read_request_body",
    ...
    assert result == {
        ...
        "client_secret": "dummy",
        "redirect_uris": ["https://proxy.derouter.example/callback"],
    }
...
async def test_register_client_remote_registration_success():
    ...
    assert call_args.kwargs["json"]["redirect_uris"] == ["https://proxy.derouter.example/callback"]
...
async def test_register_client_non_bridge_returns_client_redirect_not_gateway_callback():
    ...
    from derouter.proxy._experimental.mcp_server.discoverable_endpoints import register_client
    from derouter.proxy._experimental.mcp_server.mcp_server_manager import global_mcp_server_manager
    from derouter.proxy._types import MCPTransport
    from derouter.types.mcp import MCPAuth
    from derouter.types.mcp_server.mcp_server_manager import MCPServer
    ...
    assert mock_async_client.post.call_args.kwargs["json"]["redirect_uris"] == [
        "https://proxy.derouter.example/callback"
    ]
...
async def test_dcr_full_loop_lands_on_client_redirect_not_gateway_callback(monkeypatch):
    ...
    monkeypatch.setenv("DEROUTER_SALT_KEY", "sk-test-salt-33699")
...
async def test_authorize_rejects_untrusted_cross_origin_redirect_with_allowlist_hint(monkeypatch):
    ...
    from derouter.proxy._experimental.mcp_server.discoverable_endpoints import authorize
    from derouter.proxy._experimental.mcp_server.mcp_server_manager import global_mcp_server_manager
    from derouter.proxy._types import MCPTransport
    from derouter.types.mcp import MCPAuth
    from derouter.types.mcp_server.mcp_server_manager import MCPServer
...
async def test_register_client_malformed_redirect_uris_falls_back_to_gateway_callback(malformed_redirect_uris):
    ...
    assert result["redirect_uris"] == ["https://proxy.derouter.example/callback"]
...
async def test_register_client_persists_dcr_client_identity():
    ...
    with (
        ...
        patch("derouter.proxy._experimental.mcp_server.db.update_mcp_server", new=mock_update),
    ...
    assert update_data.credentials["redirect_uris"] == ["https://proxy.derouter.example/callback"]
...
async def test_register_client_persists_only_to_its_own_row_when_another_server_shares_the_url():
    ...
    with (
        ...
        patch("derouter.proxy.utils.get_prisma_client_or_throw", return_value=MagicMock()),
        patch("derouter.proxy._experimental.mcp_server.db.get_mcp_server", new=AsyncMock(side_effect=_get_row)),
        patch("derouter.proxy._experimental.mcp_server.db.update_mcp_server", new=mock_update),
        patch.object(global_mcp_server_manager, "update_server", new=AsyncMock()),
    ):
        response = await register_client_with_server(
...
async def test_register_client_reuses_persisted_client_id_for_non_admin_when_registry_is_stale():
    ...
    with (
        ...
        patch("derouter.proxy.utils.get_prisma_client_or_throw", return_value=MagicMock()),
        patch(
            "derouter.proxy._experimental.mcp_server.db.get_mcp_server",
            new=mock_get_mcp_server,
        ),
        patch(
            "derouter.proxy._experimental.mcp_server.db.update_mcp_server",
...
async def test_register_client_re_registers_when_persisted_redirect_uri_no_longer_matches_origin():
    ...
    assert register_payload["redirect_uris"] == ["https://proxy.derouter.example/callback"]
...
async def test_authorize_endpoint_respects_x_forwarded_proto():
    ...
    mock_request = MagicMock(spec=Request)
    mock_request.base_url = "http://derouter.example.com/"  # HTTP
    ...
    assert (
        "redirect_uri=https%3A%2F%2Fderouter.example.com%2Fcallback" in location
        or "redirect_uri=https://derouter.example.com/callback" in location
    )
...
async def test_token_endpoint_respects_x_forwarded_proto():
    ...
    # Mock request with http base_url but X-Forwarded-Proto: https
    mock_request = MagicMock(spec=Request)
    mock_request.base_url = "http://derouter-proxy.example.com/"  # HTTP
    mock_request.headers = {"X-Forwarded-Proto": "https"}  # Behind HTTPS proxy
    ...
    call_args = mock_async_client.post.call_args
    assert call_args[1]["data"]["redirect_uri"] == "https://derouter-proxy.example.com/callback"
...
async def test_oauth_protected_resource_respects_x_forwarded_proto():
    ...
    assert response["authorization_servers"][0].startswith("https://derouter.example.com/")
...
async def test_oauth_authorization_server_respects_x_forwarded_proto():
    ...
    # Verify response uses HTTPS URLs
    assert response["authorization_endpoint"].startswith("https://derouter.example.com/")
    assert response["token_endpoint"].startswith("https://derouter.example.com/")
    assert response["registration_endpoint"].startswith("https://derouter.example.com/")
...
async def test_register_client_respects_x_forwarded_proto():
    ...
    # Mock request with http base_url but X-Forwarded-Proto: https
    mock_request = MagicMock(spec=Request)
    mock_request.base_url = "http://proxy.derouter.example/"  # HTTP
    mock_request.headers = {"X-Forwarded-Proto": "https"}  # Behind HTTPS proxy
...
def test_xff_misconfig_warning_emitted_once(caplog):
    ...
    try:
        ...
        from derouter.proxy import auth as proxy_auth_pkg  # noqa: F401
        from derouter.proxy._experimental.mcp_server.discoverable_endpoints import (
            get_request_base_url,
        )
        from derouter.proxy.auth import ip_address_utils
    except ImportError:
        pytest.skip("MCP discoverable endpoints not available")
    ...
    with (
        caplog.at_level(logging.WARNING, logger="DeRouter Proxy"),
        patch("derouter.proxy.proxy_server.general_settings", misconfig, create=True),
    ):
        for _ in range(3):
...
def test_get_request_base_url_honors_proxy_base_url_env(monkeypatch):
    try:
        ...
        from derouter.proxy._experimental.mcp_server.oauth_utils import (
            get_request_base_url,
        )
    except ImportError:
        pytest.skip("MCP discoverable endpoints not available")
    ...
    mock_request = MagicMock(spec=Request)
    mock_request.base_url = "http://derouter-internal:4000/"
    mock_request.client = MagicMock()
    mock_request.client.host = "10.0.0.7"
    headers = {
        "X-Forwarded-Proto": "https",
        "X-Forwarded-Host": "derouter-internal:4000",
        "X-Forwarded-Port": "9999",
    }
    mock_request.headers.get = lambda name, default=None: headers.get(name, default)

    monkeypatch.setenv("PROXY_BASE_URL", "https://derouter.example.com")
    assert get_request_base_url(mock_request) == "https://derouter.example.com"
...
def test_validate_trusted_redirect_uri_logs_diagnostic_on_rejection(caplog, monkeypatch):
    ...
    headers = {
        "X-Forwarded-Proto": "https",
        "X-Forwarded-Host": "derouter.example.com",
        "X-Forwarded-Port": "443",
        "Host": "derouter-internal:4000",
    }
    mock_request.headers.get = lambda name, default=None: headers.get(name, default)
    ...
    with (
        caplog.at_level(logging.WARNING, logger="DeRouter"),
        patch("derouter.proxy.proxy_server.general_settings", {}, create=True),
    ):
        with pytest.raises(HTTPException) as exc_info:
            validate_trusted_redirect_uri(
                mock_request,
                "https://derouter.example.com/ui/mcp/oauth/callback",
            )
        assert exc_info.value.status_code == 400
    ...
    assert "https://derouter.example.com/ui/mcp/oauth/callback" in msg
    assert "derouter-internal:4000" in msg
...
def test_validate_trusted_redirect_uri_follows_the_xff_trust_gate(direct_ip, expect_accepted, monkeypatch):
    ...
    with patch("derouter.proxy.proxy_server.general_settings", general_settings, create=True):
        if expect_accepted:
...
@pytest.mark.parametrize(
    "bad_value",
    [
        "derouter.example.com",
        "derouter.example.com/",
        "://derouter.example.com",
        "ftp://derouter.example.com",
        "https://",
        "not a url at all",
    ],
)
def test_get_request_base_url_rejects_malformed_proxy_base_url(bad_value, monkeypatch, caplog):
    try:
        ...
        from derouter.proxy._experimental.mcp_server import oauth_utils
        from derouter.proxy._experimental.mcp_server.oauth_utils import (
            get_request_base_url,
        )
    except ImportError:
    ...
    assert result == "http://derouter-internal:4000", (
...
def test_get_request_base_url_malformed_proxy_base_url_warning_is_one_shot(monkeypatch, caplog):
    ...
    monkeypatch.setenv("PROXY_BASE_URL", "derouter.example.com")
...
async def test_oauth_protected_resource_gateway_managed_oauth2_advertises_gateway_as():
    ...
    try:
        ...
        for name in ("github_mcp", "m2m_mcp"):
            ...
            assert standard["authorization_servers"] == ["https://derouter.example.com/mcp"], name
            assert standard["resource"] == f"https://derouter.example.com/mcp/{name}"
            ...
            assert legacy["authorization_servers"] == ["https://derouter.example.com/mcp"], name
            assert legacy["resource"] == f"https://derouter.example.com/{name}/mcp"
        ...
        assert relay_response["authorization_servers"] == ["https://derouter.example.com/relay_mcp"]
        ...
        assert relay_legacy_response["authorization_servers"] == ["https://derouter.example.com/relay_mcp"]
        ...
        assert delegated_response["authorization_servers"] == ["https://derouter.example.com/delegated_mcp"]
    finally:
        global_mcp_server_manager.registry.clear()
...
async def test_oauth_protected_resource_root_resolved_single_server_keeps_relay_as():
    ...
    try:
        ...
        assert response["authorization_servers"] == ["https://derouter.example.com/solo_mcp"]
    finally:
        global_mcp_server_manager.registry.clear()
...
async def test_authorize_root_resolves_single_oauth2_server():
    ...
    try:
        with patch(
            "derouter.proxy._experimental.mcp_server.discoverable_endpoints.encrypt_value_helper"
        ) as mock_encrypt:
            mock_encrypt.return_value = "mocked_encrypted_state"
...
async def test_register_root_does_aggregate_dcr_not_single_server_resolution():
    ...
    try:
        with (
            ...
            patch("derouter.proxy.proxy_server.master_key", "sk-test-salt-for-lit3637"),
        ):
...
async def test_oauth_callback_redirects_with_state():
    ...
    with patch("derouter.proxy._experimental.mcp_server.discoverable_endpoints.decode_state_hash") as mock_decode:
...
async def test_authorize_forwards_short_state_and_round_trips_via_cookie(monkeypatch):
    ...
    monkeypatch.setenv("DEROUTER_SALT_KEY", "sk-test-salt-for-LIT-4197")
...
async def test_authorize_bridge_relay_passes_client_params_verbatim(auth_type_value):
    ...
    assert "derouter.example.com" not in location
...
async def test_authorize_bridge_short_circuit_keeps_callback_and_redirect_trust():
    ...
    assert query["redirect_uri"] == ["https://derouter.example.com/callback"]
...
async def test_token_non_bridge_keeps_gateway_callback():
    ...
    assert data["redirect_uri"] == "https://derouter.example.com/callback"
...
def test_oauth_authorization_server_metadata_served_for_bridge_server():
    ...
    assert result["authorization_endpoint"] == "https://derouter.example.com/bridge_srv/authorize"
    assert result["token_endpoint"] == "https://derouter.example.com/bridge_srv/token"
    assert result["registration_endpoint"] == "https://derouter.example.com/bridge_srv/register"
...
async def _bridge_register_response(server, request_payload, persist_credentials=False):
    ...
    with (
        ...
        patch(
            "derouter.proxy._experimental.mcp_server.discoverable_endpoints._persist_dcr_client_registration",
            new_callable=AsyncMock,
        ) as mock_persist,
        patch(
            "derouter.proxy._experimental.mcp_server.discoverable_endpoints._reuse_persisted_dcr_client_if_available",
            new_callable=AsyncMock,
            return_value=False,
        ),
    ):
...
async def _exchange_for_bridge_server(server, upstream_body, key_hash, code="auth-code", fake_client_out=None):
    from derouter.proxy._experimental.mcp_server.bridge_token_flow import _ResolvedKey
    from derouter.proxy._experimental.mcp_server.discoverable_endpoints import exchange_token_with_server
    ...
    fake_http_client = MagicMock()
    fake_http_client.post = AsyncMock(return_value=fake_http_response)
    # The mint consumes _resolve_active_derouter_key's tagged result: an active key resolves to a
    ...
    with (
        ...
        patch(
            "derouter.proxy._experimental.mcp_server.bridge_token_flow._resolve_active_derouter_key",
            new=key_resolver,
        ),
        patch("derouter.proxy.proxy_server.master_key", _BRIDGE_MASTER_KEY),
    ):
        response = await exchange_token_with_server(
            request=_bridge_mock_request(),
            mcp_server=server,
    ...
    from derouter.proxy._experimental.mcp_server.discoverable_endpoints import is_bridge_authorization_code

    # The key_hash path resolves the presented derouter key; the interactive SSO path recovers identity
...
async def test_oauth_delegate_bridge_token_exchange_mints_envelope_not_raw_token():
    ...
    from derouter.proxy._experimental.mcp_server.outbound_credentials.bridge_credentials import (
    ...
    response = await _exchange_for_bridge_server(server, upstream, key_hash="hashed-derouter-key-77")
    ...
    assert opened.identity.subject == "hashed-derouter-key-77"
...
def test_bridge_authorization_code_round_trips_and_rejects_hostile_input():
    ...
    with patch("derouter.proxy.proxy_server.master_key", _BRIDGE_MASTER_KEY):
        sealed = seal_bridge_authorization_code(
            upstream_code="up-code", derouter_user_id="sso-user-9", mcp_server_id="srv-1"
        )
        ...
        assert opened.derouter_user_id == "sso-user-9"
...
async def test_interactive_bridge_token_exchange_mints_user_subject_envelope():
    ...
    callback sealed, and NO derouter key) mints an envelope bound to the SSO-captured user: it opens
    ...
    with patch("derouter.proxy.proxy_server.master_key", _BRIDGE_MASTER_KEY):
        gateway_code = seal_bridge_authorization_code(
            upstream_code="REAL-UPSTREAM-CODE", derouter_user_id="sso-user-42", mcp_server_id=server.server_id
        )
    upstream = {"access_token": "UPSTREAM-SECRET-TOKEN", "token_type": "Bearer", "expires_in": 3600}
    captured: dict = {}
...
async def test_interactive_bridge_gateway_code_for_another_server_is_rejected_400():
    ...
    with patch("derouter.proxy.proxy_server.master_key", _BRIDGE_MASTER_KEY):
        gateway_code = seal_bridge_authorization_code(
            upstream_code="up-code", derouter_user_id="sso-user-42", mcp_server_id="a-different-server-id"
        )
    upstream = {"access_token": "UPSTREAM-SECRET-TOKEN", "token_type": "Bearer", "expires_in": 3600}
    with pytest.raises(HTTPException) as exc:
...
async def test_interactive_bridge_authorize_seals_sso_user_into_state():
    ...
    from derouter.proxy._experimental.mcp_server.discoverable_endpoints import authorize_with_server
    from derouter.proxy._experimental.mcp_server.mcp_server_manager import (
        global_mcp_server_manager,
    )
    ...
    with (
        patch(
            "derouter.proxy._experimental.mcp_server.byok_oauth_endpoints._user_id_from_session_cookie",
            return_value="sso-user-42",
        ),
        patch(
            "derouter.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp.MCPRequestHandler.reload_admitted_user",
        ...
        patch(
            "derouter.proxy._experimental.mcp_server.discoverable_endpoints.encode_state_with_base_url",
    ...
    assert captured["derouter_user_id"] == "sso-user-42"
...
async def test_interactive_bridge_authorize_without_session_redirects_to_login():
    ...
    authorize sends the browser through derouter login instead of proceeding to the upstream."""
    from derouter.proxy._experimental.mcp_server.discoverable_endpoints import authorize_with_server
    from derouter.types.mcp import MCPAuth
...
async def test_interactive_bridge_callback_seals_user_into_gateway_code():
    ...
    state_data = {
        ...
        "derouter_user_id": "sso-user-42",
        "mcp_server_id": "bridge_srv",
    }
    with (
        patch(
            "derouter.proxy._experimental.mcp_server.discoverable_endpoints._resolve_encoded_oauth_state",
            return_value="enc",
        ),
        patch(
            "derouter.proxy._experimental.mcp_server.discoverable_endpoints.decode_state_hash",
            return_value=state_data,
        ),
        patch(
            "derouter.proxy._experimental.mcp_server.discoverable_endpoints._get_validated_client_redirect_uri",
...
@pytest.mark.asyncio
async def test_oauth_delegate_bridge_token_exchange_fails_closed_without_derouter_identity():
    """Without a resolvable derouter identity on the token request, the exchange must not mint an
...
async def test_bridge_envelope_unrepresentable_upstream_lifetime_is_502():
    ...
    response = await _exchange_for_bridge_server(
        server,
        upstream,
        key_hash="hashed-derouter-key-77",
    )
...
async def test_bridge_access_envelope_never_carries_upstream_refresh_token():
    ...
    from derouter.proxy._experimental.mcp_server.outbound_credentials.envelope import (
        OpenedEnvelope,
        open_envelope,
    )
    from derouter.types.mcp import MCPAuth
...
async def _refresh_for_bridge_server(
    ...
    with (
        ...
        patch(
            "derouter.proxy._experimental.mcp_server.bridge_token_flow._revalidate_active_subject",
...
async def test_bridge_refresh_grant_with_revoked_key_is_invalid_grant_before_upstream():
    """A valid refresh envelope whose sealed derouter key has since been revoked cannot keep refreshing:
...
async def test_bridge_refresh_grant_renews_a_user_subject_envelope():
    ...
    from derouter.proxy._experimental.mcp_server.outbound_credentials.envelope import user_identity
    from derouter.types.mcp import MCPAuth
...
async def test_revalidate_active_subject_dispatches_on_subject_type():
    ...
    from derouter.proxy._experimental.mcp_server.bridge_token_flow import (
        _ResolvedKey,
        _revalidate_active_subject,
    )
    from derouter.proxy._experimental.mcp_server.outbound_credentials.envelope import key_hash_identity, user_identity

    with (
        patch(
            "derouter.proxy._experimental.mcp_server.bridge_token_flow._reload_active_key_by_hash",
            new=AsyncMock(return_value=_ResolvedKey(key_hash="kh", key=MagicMock())),
        ) as key_reload,
        patch(
            "derouter.proxy._experimental.mcp_server.bridge_token_flow._reload_active_user_by_id",
...
def test_upstream_refresh_credential_expired_refresh_token_is_not_sealed():
    ...
    from derouter.proxy._experimental.mcp_server.bridge_token_flow import _upstream_refresh_credential
...
async def test_revalidate_key_subject_revoked_when_owner_scim_deactivated(proxy_globals):
    ...
    from derouter.proxy._experimental.mcp_server.bridge_token_flow import _ResolvedKey, _revalidate_active_subject
    from derouter.proxy._experimental.mcp_server.outbound_credentials.envelope import key_hash_identity
...
def test_bridge_reported_expires_in_can_be_zero_at_jwt_exp_boundary():
    ...
    ready = _BridgeMintReady(
        identity=key_hash_identity(server_id="bridge_srv", key_hash="hashed-derouter-key-77"),
        keys=envelope_keys_from_master_key(_BRIDGE_MASTER_KEY),
    )
...
def test_classify_upstream_lifetime():
    ...
    from derouter.proxy._experimental.mcp_server.bridge_token_flow import _classify_upstream_lifetime
...
def test_bridge_grant_honors_and_rejects_upstream_lifetime():
    ...
    from derouter.proxy._experimental.mcp_server.bridge_token_flow import _bridge_grant_from_token_response
...
async def test_true_passthrough_bridge_token_exchange_returns_raw_upstream_token():
    ...
    to the client, since that mode has no derouter identity to bind and the caller owns the token."""
    from derouter.types.mcp import MCPAuth
...
async def _exchange_persistence_attempted_for_auth_type(auth_type) -> bool:
    ...
    with (
        ...
        patch(
            "derouter.proxy._experimental.mcp_server.discoverable_endpoints._extract_user_id_from_request",
            new_callable=AsyncMock,
            return_value="admin-user",
        ),
        patch(
            "derouter.proxy._experimental.mcp_server.discoverable_endpoints._store_per_user_token_server_side",
            new_callable=AsyncMock,
        ) as mock_store,
    ):
        await exchange_token_with_server(
...
_OBO_RESOURCE = "https://derouter.example.com/mcp/obo_mcp"
_PATCH_ISSUERS = "derouter.proxy._experimental.mcp_server.discoverable_endpoints._jwt_auth_issuers"
...
async def test_build_oauth_protected_resource_response_obo_end_to_end():
    ...
    try:
        ...
        assert response["resource"] == "https://derouter.example.com/mcp/obo_mcp"
    finally:
        global_mcp_server_manager.registry.clear()
...
@pytest.mark.asyncio
async def test_extract_user_id_reads_x_derouter_api_key_header(proxy_globals):
    """The DeRouter key arrives on x-derouter-api-key (what Claude Desktop/Code send), not
    Authorization. Reading only Authorization dropped the identity, so the per-user token was
    never stored and the egress 401'd forever. Resolution must honor x-derouter-api-key."""
    from derouter.proxy._experimental.mcp_server.bridge_token_flow import (
        _extract_user_id_from_request,
    )
    from derouter.proxy._types import UserAPIKeyAuth, hash_token
    from derouter.proxy.common_utils.user_api_key_cache import UserApiKeyCache
    ...
    request = _token_request({"x-derouter-api-key": f"Bearer {key}"})
...
async def test_extract_user_id_falls_back_to_db_on_cache_miss(proxy_globals):
    ...
    request = _token_request({"x-derouter-api-key": key})
    assert await _extract_user_id_from_request(request) == "db-bob"


@pytest.mark.asyncio
async def test_extract_user_id_none_without_derouter_key(proxy_globals):
    """No DeRouter key on the request resolves to None without consulting the resolver."""
    from derouter.proxy._experimental.mcp_server.bridge_token_flow import (
        _extract_user_id_from_request,
    )
    from derouter.proxy.common_utils.user_api_key_cache import UserApiKeyCache
...
async def test_extract_user_id_rejects_blocked_key(proxy_globals):
    """A blocked DeRouter key must not resolve an identity. get_key_object returns the DB row without
    ...
    request = _token_request({"x-derouter-api-key": "sk-blocked-key"})
    assert await _extract_user_id_from_request(request) is None


@pytest.mark.asyncio
async def test_extract_user_id_rejects_expired_key(proxy_globals):
    """An expired DeRouter key must not resolve an identity, for the same reason as a blocked key."""
    from datetime import datetime, timedelta, timezone
    ...
    request = _token_request({"x-derouter-api-key": "sk-expired-key"})
    assert await _extract_user_id_from_request(request) is None


@pytest.mark.asyncio
async def test_resolve_active_derouter_key_returns_resolved_key_for_active_key(proxy_globals):
    ...
    request = _token_request({"x-derouter-api-key": f"Bearer {key}"})
    resolved = await _resolve_active_derouter_key(request)
    assert isinstance(resolved, _ResolvedKey)
    assert resolved.key_hash == hash_token(key)


@pytest.mark.asyncio
async def test_resolve_active_derouter_key_resolves_key_without_user_id(proxy_globals):
...
@pytest.mark.asyncio
async def test_resolve_active_derouter_key_rejects_blocked_key(proxy_globals):
    ...
    request = _token_request({"x-derouter-api-key": "sk-blocked-key"})
    assert await _resolve_active_derouter_key(request) == "no_active_key"


@pytest.mark.asyncio
async def test_resolve_active_derouter_key_fails_closed_on_malformed_expiry(proxy_globals):
    ...
    request = _token_request({"x-derouter-api-key": "sk-bad-expiry-key"})
    assert await _resolve_active_derouter_key(request) == "no_active_key"


@pytest.mark.asyncio
async def test_resolve_active_derouter_key_no_active_key_without_derouter_key(proxy_globals):
    """No DeRouter key on the request yields no hash without consulting the resolver."""
    from derouter.proxy._experimental.mcp_server.bridge_token_flow import (
        _resolve_active_derouter_key,
    )
    from derouter.proxy.common_utils.user_api_key_cache import UserApiKeyCache
...
@pytest.mark.asyncio
async def test_resolve_active_derouter_key_db_outage_is_unavailable(proxy_globals):
    ...
    request = _token_request({"x-derouter-api-key": "sk-during-outage"})
    assert await _resolve_active_derouter_key(request) == "unavailable"


@pytest.mark.asyncio
async def test_resolve_active_derouter_key_permanent_engine_fault_is_faulted(proxy_globals):
    ...
    request = _token_request({"x-derouter-api-key": "sk-during-engine-fault"})
    assert await _resolve_active_derouter_key(request) == "faulted"


@pytest.mark.asyncio
async def test_resolve_active_derouter_key_transport_error_over_permanent_fault_is_faulted(proxy_globals):
    ...
    request = _token_request({"x-derouter-api-key": "sk-during-failed-reconnect"})
    assert await _resolve_active_derouter_key(request) == "faulted"


@pytest.mark.asyncio
async def test_resolve_active_derouter_key_no_database_is_unresolvable(proxy_globals):
    ...
    request = _token_request({"x-derouter-api-key": "sk-no-db"})
    assert await _resolve_active_derouter_key(request) == "unresolvable"


def _wrapped_user_lookup_error(original: BaseException) -> ValueError:
    """Reproduce get_user_object's real exception contract (derouter/proxy/auth/auth_checks.py): it
...
async def test_reload_active_user_by_id_missing_user_is_no_active_key(proxy_globals):
    ...
    from derouter.proxy._experimental.mcp_server.bridge_token_flow import _reload_active_user_by_id
    from derouter.proxy.common_utils.user_api_key_cache import UserApiKeyCache
...
async def test_oauth_protected_resource_passthrough_none_auth_not_404():
    ...
    try:
        with patch(
            "derouter.proxy._experimental.mcp_server.discoverable_endpoints.fetch_upstream_oauth_protected_resource",
...
async def test_store_per_user_token_server_side_invalidates_v2_token_cache():
    ...
    until its TTL."""
    from derouter.proxy._experimental.mcp_server import mcp_server_manager as manager_module
    from derouter.proxy._experimental.mcp_server.discoverable_endpoints import (
    ...
    with (
        ...
        patch(
            "derouter.proxy._experimental.mcp_server.db.store_user_oauth_credential",
            new=AsyncMock(return_value=None),
        ),
        patch(
            "derouter.proxy._experimental.mcp_server.oauth2_token_cache.mcp_per_user_token_cache.set",
            new=cache_set_mock,
        ),
        patch.object(
...
async def test_token_exchange_pairs_client_secret_with_server_client_id():
    ...
    with patch(
        ...
        await exchange_token_with_server(
            ...
            grant_type="authorization_code",
            code="auth-code",
            redirect_uri="https://derouter.example.com/ui/mcp/oauth/callback",
            client_id="srv-1",
            client_secret="dummy",
            code_verifier="verifier",
        )
...
async def test_token_exchange_relays_rejection_when_http_client_raises():
    """derouter's AsyncHTTPHandler.post raise_for_status()es internally and raises MaskedHTTPStatusError
...
async def test_persist_dcr_client_for_config_server_uses_side_store():
    """A config.yaml-declared OAuth2 DCR server has no DeRouter_MCPServerTable row, so
    ...
    with (
        ...
        patch(
            "derouter.proxy._experimental.mcp_server.db.get_mcp_server_oauth_client_credentials",
            new=AsyncMock(return_value=None),
        ),
        patch(
            "derouter.proxy._experimental.mcp_server.db.upsert_mcp_server_oauth_client_credentials",
            new=mock_upsert,
        ),
        patch.object(global_mcp_server_manager, "update_server", new=mock_update_server),
    ):
        result = await _persist_dcr_client_registration(
            ...
            current_redirect_uri="https://proxy.derouter.example/callback",
        )
    ...
    assert stored["redirect_uris"] == ["https://proxy.derouter.example/callback"]
...
async def test_hydrate_config_server_applies_stored_dcr_client(monkeypatch):
    ...
    import derouter.proxy.common_utils.encrypt_decrypt_utils as enc
    from derouter.derouter_core_utils.safe_json_dumps import safe_dumps
    from derouter.proxy._experimental.mcp_server.db import encrypt_credentials
    from derouter.proxy._experimental.mcp_server.discoverable_endpoints import (
...
async def test_reuse_config_server_reads_store_with_real_crypto(monkeypatch):
    ...
    with (
        patch.object(global_mcp_server_manager, "is_config_declared_server", return_value=True),
        patch("derouter.proxy.utils.get_prisma_client_or_throw", return_value=MagicMock()),
        patch("derouter.proxy._experimental.mcp_server.db.get_mcp_server", new=AsyncMock(return_value=None)),
        patch(
            "derouter.proxy._experimental.mcp_server.db.get_mcp_server_oauth_client_credentials",
            new=store_lookup,
        ),
...
async def test_temp_server_is_not_persisted_to_store():
    ...
    with (
        ...
        patch("derouter.proxy.utils.get_prisma_client_or_throw", return_value=MagicMock()),
        patch("derouter.proxy._experimental.mcp_server.db.update_mcp_server", new=AsyncMock(return_value=None)),
        patch("derouter.proxy._experimental.mcp_server.db.get_mcp_server", new=AsyncMock(return_value=None)),
        patch("derouter.proxy._experimental.mcp_server.db.upsert_mcp_server_oauth_client_credentials", new=upsert),
        patch(
            "derouter.proxy._experimental.mcp_server.db.get_mcp_server_oauth_client_credentials",
            new=store_read,
        ),
...
async def test_reuse_does_not_inherit_store_client_when_a_row_exists():
    """Security: a server that HAS a DeRouter_MCPServerTable row reads its DCR client only from that
...
async def test_reload_servers_from_database_hydrates_dcr_clients():
    ...
    prisma = MagicMock()
    prisma.db.derouter_mcpservertable.find_many = AsyncMock(return_value=[])

    hydrate_spy = AsyncMock()
    with (
        patch(
            "derouter.proxy.management_endpoints.mcp_management_endpoints.get_prisma_client_or_throw",
            return_value=prisma,
        ),
...
def test_aggregate_wellknown_routes_serve_gateway_metadata():
    ...
    from derouter.proxy._experimental.mcp_server.discoverable_endpoints import router
    from derouter.proxy._experimental.mcp_server.mcp_server_manager import (
        global_mcp_server_manager,
    )
...
def test_well_known_root_suffix_reflects_server_root_path():
    ...
    from derouter.proxy._experimental.mcp_server.oauth_utils import well_known_root_suffix

    with patch.dict(os.environ, {"SERVER_ROOT_PATH": ""}):
        assert well_known_root_suffix() == ""
    with patch.dict(os.environ, {"SERVER_ROOT_PATH": "/"}):
        assert well_known_root_suffix() == ""
    with patch.dict(os.environ, {"SERVER_ROOT_PATH": "/derouter"}):
        assert well_known_root_suffix() == "/derouter"
...
def test_gateway_dcr_flow_routing_engages_only_for_llm_dcrc_clients(monkeypatch):
    ...
    monkeypatch.setenv("DEROUTER_SALT_KEY", "sk-test-salt-for-lit3637")
    monkeypatch.setattr("derouter.proxy.proxy_server.master_key", "sk-test-salt-for-lit3637", raising=False)
    global_mcp_server_manager.registry.clear()
    app = FastAPI()
    app.include_router(router)
    client = TestClient(app)
...
async def test_authorize_wall_names_the_fix_for_urlless_servers():
    ...
    from derouter.types.mcp import MCPAuth, MCPTransport
    from derouter.types.mcp_server.mcp_server_manager import MCPServer
...
async def test_token_exchange_with_configured_token_url_never_joins_discovery(monkeypatch):
    ...
    from derouter.proxy._experimental.mcp_server import (
        discoverable_endpoints,
        mcp_server_manager,
    )
    from derouter.proxy._experimental.mcp_server.discoverable_endpoints import (
...
async def test_bridge_authorize_relays_with_registration_url_resolved_by_deferred_discovery(monkeypatch):
    ...
    from derouter.proxy._experimental.mcp_server import mcp_server_manager
    from derouter.proxy._experimental.mcp_server.discoverable_endpoints import (
        authorize_with_server,
    )
    from derouter.types.mcp import MCPAuth, MCPTransport
...
async def test_authorize_bridge_server_with_ephemeral_client_takes_short_circuit_arm():
    ...
    assert params["redirect_uri"] == ["https://derouter.example.com/callback"]
...
async def test_callback_forwards_raw_code_when_dcr_state_lacks_server_binding():
    ...
    from derouter.proxy._experimental.mcp_server.discoverable_endpoints import callback
...
async def test_resolve_ephemeral_dcr_client_mint_set_is_exact(auth_type_value, dcr_bridge):
    ...
    with patch(
        "derouter.proxy._experimental.mcp_server.discoverable_endpoints.mint_ephemeral_dcr_client",
        mint_mock,
    ):
...
async def test_mint_ephemeral_dcr_client_posts_rfc7591_and_returns_client():
    ...
    assert register_data["redirect_uris"] == ["https://derouter.example.com/callback"]
...
async def test_token_exchange_authenticates_with_the_sealed_clients_own_auth_method(
    ...
    with patch(
        ...
        await exchange_token_with_server(
            ...
            grant_type="authorization_code",
            code="up-code",
            redirect_uri="https://derouter.example.com/callback",
            client_id="minted-77",
            client_secret="mint-secret",
            code_verifier="verifier",
...
def test_resolve_upstream_resource_tristate_and_canonicalization(url, configured, expected):
    ...
    from derouter.proxy._experimental.mcp_server.oauth_utils import resolve_upstream_resource
...
def test_build_upstream_oauth2_token_request_bundles_resource_with_client_auth(configured, url, expected_resource):
    ...
    from derouter.proxy._experimental.mcp_server.oauth_utils import build_upstream_oauth2_token_request
...
class TestPerRequestRootPathDiscovery:
    ...
    def test_user_oauth_challenge_url_routes_and_resource_matches_client_url(
        ...
        from derouter.proxy._experimental.mcp_server.discoverable_endpoints import router
        from derouter.proxy._experimental.mcp_server.outbound_credentials.adapter import (
...
def _native_client_app(monkeypatch):
    ...
    from derouter.proxy._experimental.mcp_server.discoverable_endpoints import router
    from derouter.proxy._experimental.mcp_server.gateway_dcr_flow import ConsentTeam, MintedProxyCredential
    from derouter.proxy._experimental.mcp_server.mcp_server_manager import global_mcp_server_manager

    monkeypatch.setenv("DEROUTER_SALT_KEY", _NATIVE_CLIENT_MASTER_KEY)
    monkeypatch.setattr("derouter.proxy.proxy_server.master_key", _NATIVE_CLIENT_MASTER_KEY, raising=False)
    minted = []
    ...
    monkeypatch.setattr(
        "derouter.proxy._experimental.mcp_server.discoverable_endpoints.mint_proxy_credential", fake_mint
    )
    monkeypatch.setattr(
        "derouter.proxy._experimental.mcp_server.discoverable_endpoints.lookup_consent_teams", fake_lookup
    )
    global_mcp_server_manager.registry.clear()
    app = FastAPI()
    app.include_router(router)
...
def test_native_client_login_walks_discovery_consent_token_refresh_and_revoke(monkeypatch):
    ...
    discovery = client.get("/.well-known/derouter-cli-auth")
    assert discovery.status_code == 200
...
def test_introspect_route_requires_virtual_key_auth_and_is_advertised():
    ...
    assert "/introspect" in DeRouterRoutes.mcp_routes.value
...
def test_introspect_route_answers_for_authenticated_caller(monkeypatch):
    ...
    from derouter.proxy._experimental.mcp_server.discoverable_endpoints import router
    from derouter.proxy._experimental.mcp_server.outbound_credentials.session_credentials import (
        session_keys_from_master_key,
    )
    from derouter.proxy._experimental.mcp_server.outbound_credentials.session_token import (
        SessionPrincipal,
        mint_session_token,
    )
    from derouter.proxy._types import UserAPIKeyAuth
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.master_key", introspect_master_key, raising=False)

    async def fake_reload(user_id: str):
        return None

    monkeypatch.setattr(
        "derouter.proxy._experimental.mcp_server.discoverable_endpoints._reload_active_user_by_id", fake_reload
    )
    app = FastAPI()
    app.include_router(router)
