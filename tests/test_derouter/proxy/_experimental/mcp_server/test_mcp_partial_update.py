...
from derouter.proxy._types import NewMCPServerRequest, UpdateMCPServerRequest
...
def _mock_prisma():
    ...
    mock_prisma.db.derouter_mcpservertable = AsyncMock()
    row = models.DeRouter_MCPServerTable.model_construct(
        server_id="test-server", transport="http", env={}, env_vars=[]
    )
    mock_prisma.db.derouter_mcpservertable.update = AsyncMock(return_value=row)
    mock_prisma.db.derouter_mcpservertable.create = AsyncMock(return_value=row)
    return mock_prisma
...
async def _run_update(data: UpdateMCPServerRequest, fields_set=None) -> dict:
    ...
    return mock_prisma.db.derouter_mcpservertable.update.call_args[1]["data"]
...
async def _run_update_with_existing(data: UpdateMCPServerRequest, existing_auth_type: str) -> dict:
    ...
    mock_prisma.db.derouter_mcpservertable.find_unique = AsyncMock(return_value=existing)
...
async def test_create_still_writes_defaults():
    ...
    data_dict = mock_prisma.db.derouter_mcpservertable.create.call_args[1]["data"]
