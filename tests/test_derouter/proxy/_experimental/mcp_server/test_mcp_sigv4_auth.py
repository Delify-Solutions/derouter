...
from derouter.experimental_mcp_client.client import MCPSigV4Auth, MCPClient
from derouter.types.mcp import MCPAuth, MCPTransport
from prisma import models


def _updated_row() -> models.DeRouter_MCPServerTable:
    return models.DeRouter_MCPServerTable.model_construct(server_id="test-server", transport="http", env={}, env_vars=[])
...
class TestMCPSigV4AssumeRole:
    ...
    def test_assume_role_with_ambient_credentials(self):
        ...
        assert call_kwargs["RoleSessionName"].startswith("derouter-mcp-")
    ...
    def test_assume_role_with_custom_session_name(self):
        ...
        with patch("boto3.client", return_value=mock_sts):
            MCPSigV4Auth(
                aws_role_name="arn:aws:iam::123456789012:role/TestRole",
                aws_session_name="regeneron-derouter-prod",
            )

        call_kwargs = mock_sts.assume_role.call_args[1]
        assert call_kwargs["RoleSessionName"] == "regeneron-derouter-prod"
    ...
    async def test_load_config_with_aws_role_name(self):
        ...
        config = {
            "agentcore_tools": {
                ...
                "auth_type": "aws_sigv4",
                "aws_role_name": "arn:aws:iam::123456789012:role/TestRole",
                "aws_session_name": "derouter-prod",
                "aws_region_name": "us-east-1",
            }
        }
        ...
        assert server.aws_session_name == "derouter-prod"
...
class TestSigV4CredentialEncryption:
    ...
    def test_encrypt_credentials_handles_aws_fields(self):
        ...
        with patch(
            "derouter.proxy._experimental.mcp_server.db.encrypt_value_helper",
...
class TestCredentialMergeOnUpdate:
    ...
    async def test_partial_update_preserves_existing_credentials(self):
        ...
        from derouter.proxy._experimental.mcp_server.db import update_mcp_server
        ...
        mock_prisma.db.derouter_mcpservertable.find_unique = AsyncMock(return_value=existing_record)
        mock_prisma.db.derouter_mcpservertable.update = AsyncMock(return_value=_updated_row())
        ...
        with (
            patch(
                "derouter.proxy._experimental.mcp_server.db._get_salt_key",
                return_value=None,
            ),
            patch(
                "derouter.proxy._experimental.mcp_server.db.encrypt_value_helper",
        ...
        update_call = mock_prisma.db.derouter_mcpservertable.update
    ...
    async def test_update_without_credentials_preserves_all(self):
        ...
        data_dict = mock_prisma.db.derouter_mcpservertable.update.call_args[1]["data"]
...
class TestSigV4BuildFromTable:
    ...
    async def test_build_mcp_server_from_table_with_sigv4_credentials(self):
        ...
        with patch(
            "derouter.proxy._experimental.mcp_server.mcp_server_manager.decrypt_value_helper",
...
class TestDecryptCredentials:
    ...
    def test_decrypt_credentials_handles_all_secret_fields(self):
        ...
        from derouter.proxy._experimental.mcp_server.db import decrypt_credentials
        ...
        with patch(
            "derouter.proxy._experimental.mcp_server.db.decrypt_value_helper",
...
class TestRotateCredentials:
    ...
    async def test_rotation_decrypts_then_reencrypts(self):
        ...
        mock_prisma = MagicMock()
        mock_prisma.db.derouter_mcpservertable.find_many = AsyncMock(return_value=[server])
        mock_prisma.db.derouter_mcpservertable.update = AsyncMock()
        mock_prisma.db.derouter_mcpserveroauthclient.find_many = AsyncMock(return_value=[])
