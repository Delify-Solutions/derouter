...
``call_mcp_tool`` is wrapped by ``@client`` (``derouter.utils.client``), which uses
...
``from derouter.proxy.proxy_server import proxy_logging_obj``; the real
...
from derouter.proxy._experimental.mcp_server import server


class _RecordingLoggingObj:
    """Stands in for ``DeRouterLoggingObj``, recording the failure flush the fix
...
async def _call_block(logging_obj, order: list, *, user_api_key_auth=mock.sentinel.auth):
    ...
    fake_proxy_server = types.ModuleType("derouter.proxy.proxy_server")
    fake_proxy_server.proxy_logging_obj = proxy_logging_obj  # pyright: ignore[reportAttributeAccessIssue]

    with mock.patch.dict(sys.modules, {"derouter.proxy.proxy_server": fake_proxy_server}):
