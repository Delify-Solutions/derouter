...
def _salt_key(monkeypatch):
    monkeypatch.setenv("DEROUTER_SALT_KEY", MASTER_KEY)
...
def test_pkce_mismatched_challenge_returns_false_never_raises(code_challenge):
    ...
    from derouter.proxy._experimental.mcp_server.gateway_dcr_flow import _pkce_verifier_matches
...
async def test_single_use_guard_in_memory_is_single_use_within_process():
    ...
    from derouter.proxy._experimental.mcp_server.gateway_dcr_flow import _SingleUseGuard
...
_MANAGER_PATCH = "derouter.proxy._experimental.mcp_server.mcp_server_manager.global_mcp_server_manager"
...
async def test_resolve_scoped_resource_server_matrix():
    ...
    from derouter.proxy._experimental.mcp_server.gateway_dcr_flow import resolve_scoped_resource_server
...
def test_native_client_auth_contract_points_every_endpoint_at_this_proxy():
    assert json.loads(json.dumps(native_client_auth_contract(_request("/.well-known/derouter-cli-auth")))) == {
        "contract_version": 1,
...
async def test_introspect_accepts_rs256_signed_tokens_under_configured_signing(monkeypatch):
    ...
    from derouter.proxy._experimental.mcp_server.outbound_credentials.session_token import AsymmetricSessionKeys
