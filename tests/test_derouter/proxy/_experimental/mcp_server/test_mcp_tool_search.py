...
from derouter.proxy.common_utils.semantic_text_index import EmbeddingFailed, SemanticTextIndex, Vector
from derouter.types.mcp import MCPToolSearchSettings
...
def _make_perm(**kwargs: Any) -> DeRouter_ObjectPermissionTable:
    return DeRouter_ObjectPermissionTable(object_permission_id="test", **kwargs)
...
class TestListToolRestApiWithToolSearch:
    ...
    async def test_returns_only_virtual_tools_when_flag_enabled(self) -> None:
        from derouter.proxy._experimental.mcp_server.rest_endpoints import router
    ...
    async def test_returns_full_catalog_when_flag_disabled(self) -> None:
        ...
        with (
            patch(
                "derouter.proxy._experimental.mcp_server.rest_endpoints.build_effective_auth_contexts",
                new_callable=AsyncMock,
                return_value=[user_api_key_dict],
            ),
            patch("derouter.proxy._experimental.mcp_server.rest_endpoints.global_mcp_server_manager") as mock_manager,
            patch(
                "derouter.proxy._experimental.mcp_server.rest_endpoints._get_tools_for_single_server",
                new_callable=AsyncMock,
                return_value=fake_tools,
            ),
            patch("derouter.proxy._experimental.mcp_server.rest_endpoints.IPAddressUtils"),
            patch(
                "derouter.proxy._experimental.mcp_server.rest_endpoints._prefetch_user_oauth_creds",
                new_callable=AsyncMock,
                return_value={},
            ),
            patch(
                "derouter.proxy._experimental.mcp_server.rest_endpoints._v1_resolved_oauth2_server_ids",
                return_value=[],
            ),
            patch(
                "derouter.proxy._experimental.mcp_server.rest_endpoints._get_server_auth_header",
                return_value=None,
            ),
            patch(
                "derouter.proxy._experimental.mcp_server.rest_endpoints._get_user_oauth_extra_headers",
                new_callable=AsyncMock,
                return_value=None,
            ),
        ):
...
class TestCallToolRestApiVirtualTools:
    ...
    async def test_mcp_tool_call_executes_discovered_tool(self) -> None:
        ...
        with (
            ...
            patch(
                "derouter.proxy._experimental.mcp_server.rest_endpoints._fire_mcp_tool_call_logging",
                new_callable=AsyncMock,
    ...
    async def test_mcp_tool_call_forwards_client_ip_for_ip_filtering(self) -> None:
        ...
        with (
            patch(
                "derouter.proxy._experimental.mcp_server.rest_endpoints.IPAddressUtils.get_mcp_client_ip",
    ...
    async def test_agent_search_call_ranks_accessible_agents(self) -> None:
        from derouter.proxy.agent_endpoints.agent_search import AgentSearchHit, AgentSearchHits
        ...
        with (
            patch(  # test-quality-ok: the tool resolves agent access through proxy_server globals, no injection seam
                "derouter.proxy.agent_endpoints.auth.agent_permission_handler.accessible_agents",
                new_callable=AsyncMock,
            ...
            patch(  # test-quality-ok: the embedding router only resolves via proxy_server globals, no injection seam
                "derouter.proxy.agent_endpoints.agent_search.search_agents",
                new_callable=AsyncMock,
    ...
    async def test_skill_search_call_tolerates_malformed_top_k(self) -> None:
        ...
        with patch(  # test-quality-ok: the embedding router only resolves via proxy_server globals, no injection seam
            "derouter.proxy._experimental.mcp_server.tool_search.handle_skill_search",
            new_callable=AsyncMock,
            return_value=fake_result,
        ) as mock_search:
            result = await self._get_call_fn()(request=request, user_api_key_dict=user_api_key_dict)
    ...
    async def test_agent_search_call_reports_missing_embedding_model_as_tool_error(self) -> None:
        from derouter.proxy.agent_endpoints.agent_search import AgentSearchNotConfigured
    ...
    async def test_mcp_tool_search_ranks_the_callers_catalog_with_the_configured_embedding_model(
        ...
        monkeypatch.setattr(derouter, "mcp_tool_search", {"embedding_model": "emb", "similarity_threshold": 0.5})
        user_api_key_dict = UserAPIKeyAuth(
        ...
        with (
            ...
            patch(  # test-quality-ok: the proxy's key-limit hooks are a module global; the embedding call runs them like /embeddings does
                "derouter.proxy.proxy_server.proxy_logging_obj", key_limits
            ),
    ...
    async def test_mcp_tool_search_reports_missing_router_as_tool_error(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(derouter, "mcp_tool_search", {"embedding_model": "emb"})
        user_api_key_dict = UserAPIKeyAuth(api_key="k", object_permission=_make_perm(mcp_tool_search_enabled=True))
    ...
    async def test_mcp_tool_search_reports_invalid_settings_as_tool_error(
        ...
        monkeypatch.setattr(derouter, "mcp_tool_search", {"top_k": 0})
        user_api_key_dict = UserAPIKeyAuth(api_key="k", object_permission=_make_perm(mcp_tool_search_enabled=True))
        result = await self._get_call_fn()(request=self._semantic_request(), user_api_key_dict=user_api_key_dict)
        assert result.isError is True
...
class TestDispatchVirtualMcpTool:
    ...
    async def test_routes_search_with_client_ip(self) -> None:
        from derouter.proxy._experimental.mcp_server import server as srv

        uak = UserAPIKeyAuth(api_key="k", object_permission=_make_perm(mcp_tool_search_enabled=True))
        with patch(
            "derouter.proxy._experimental.mcp_server.tool_search.handle_mcp_tool_search",
            new_callable=AsyncMock,
    ...
    async def test_routes_agent_search_to_its_handler(self) -> None:
        ...
        with patch(  # test-quality-ok: dispatch routing is the subject; the handler is faked like its siblings here
            "derouter.proxy._experimental.mcp_server.tool_search.handle_agent_search",
            new_callable=AsyncMock,
    ...
    async def test_agent_search_rejected_when_flag_disabled(self) -> None:
        from derouter.proxy._experimental.mcp_server.server import _dispatch_virtual_mcp_tool
    ...
    async def test_routes_call_with_client_ip(self) -> None:
        ...
        uak = UserAPIKeyAuth(api_key="k", object_permission=_make_perm(mcp_tool_search_enabled=True))
        with patch(
            "derouter.proxy._experimental.mcp_server.tool_search.handle_mcp_tool_call",
            new_callable=AsyncMock,
    ...
    async def test_call_builds_and_forwards_logging_obj(self) -> None:
        ...
        assert mock_call.await_args.kwargs["derouter_logging_obj"] is sentinel_logging_obj
    ...
    async def test_call_handler_forwards_auth_headers_to_execute(self) -> None:
        ...
        with (
            ...
            await handle_mcp_tool_call(
                ...
                oauth2_headers={"Authorization": "Bearer oauth"},
                raw_headers={"x-mcp-auth": "tok"},
                derouter_logging_obj=sentinel_logging_obj,
            )
        ...
        assert kw["derouter_logging_obj"] is sentinel_logging_obj
...
class TestMcpServerToolCallErrorHandling:
    ...
    async def test_virtual_tool_error_returns_iserror_not_raised(self) -> None:
        ...
        with (
            ...
            patch(
                "derouter.proxy._experimental.mcp_server.server._dispatch_virtual_mcp_tool",
                new_callable=AsyncMock,
...
async def test_handle_mcp_tool_call_scoped_denial_names_the_binding_agent() -> None:
    ...
    from derouter.proxy._experimental.mcp_server.tool_search import handle_mcp_tool_call
