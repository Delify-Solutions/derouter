...
async def test_token_endpoint_success():
    ...
    with (
        patch(
            "derouter.proxy._experimental.mcp_server.byok_oauth_endpoints.store_user_credential",
            mock_store,
        ),
        patch(
            "derouter.proxy._experimental.mcp_server.byok_oauth_endpoints.router",
        ),
    ):
        ...
        with patch(
            ...
            import derouter.proxy._experimental.mcp_server.byok_oauth_endpoints as mod
            ...
            with (
                ...
                patch("derouter.proxy.proxy_server.master_key", test_master_key),
            ):
...
async def test_token_endpoint_invalid_code():
    from derouter.proxy._experimental.mcp_server.byok_oauth_endpoints import byok_token

    mock_request = MagicMock()
    with (
        patch("derouter.proxy.proxy_server.prisma_client", MagicMock()),
        patch("derouter.proxy.proxy_server.master_key", "key"),
    ):
        result = await byok_token(
            request=mock_request,
            grant_type="authorization_code",
...
async def test_check_byok_credential_not_byok():
    ...
    from derouter.proxy._experimental.mcp_server.server import _check_byok_credential
...
async def test_check_byok_credential_missing_credential():
    ...
    with (
        patch(
            "derouter.proxy._experimental.mcp_server.db.get_user_credential",
...
def test_authorize_post_accepts_ui_session_cookie(unauthenticated_client):
    ...
    with patch("derouter.proxy.proxy_server.master_key", "test-master-key"):
...
def test_authorize_post_rejects_cookie_signed_with_wrong_key(unauthenticated_client):
    ...
    with patch("derouter.proxy.proxy_server.master_key", "real-master-key"):
        forged = _jwt.encode(
...
async def test_token_endpoint_accepts_spec_compliant_client_id():
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.master_key", "test-master"),
        patch("derouter.proxy.proxy_server.prisma_client", MagicMock()),
    ):
        result = await byok_token(
            request=MagicMock(),
...
async def test_token_endpoint_accepts_oauth21_client_omitting_redirect_uri():
    ...
    DeRouter must accept that and rely on PKCE + client_id binding for
...
def test_validate_trusted_redirect_uri_rejects_unlisted_native_callback(
    ...
    monkeypatch.setattr(
        "derouter.proxy._experimental.mcp_server.oauth_utils._DEFAULT_NATIVE_REDIRECT_URIS",
        [],
    )
    req = _make_trusted_request("http://localhost:4000/")
    with pytest.raises(HTTPException) as exc:
...
def test_strip_default_port_empty_netloc():
    ...
    from derouter.proxy._experimental.mcp_server.oauth_utils import _strip_default_port
