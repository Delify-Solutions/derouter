...
``derouter.proxy._experimental.mcp_server.utils`` and do not require a DB
...
import derouter.proxy._experimental.mcp_server.utils as _mcp_utils
...
async def test_load_user_env_vars_caches_within_ttl(env_vars_salt_key, monkeypatch):
    ...
    from derouter.proxy._experimental.mcp_server import mcp_server_manager as mgr_mod
    from derouter.proxy._experimental.mcp_server.mcp_server_manager import (
        MCPServerManager,
    )
    from derouter.types.mcp_server.mcp_server_manager import MCPServer
    ...
    assert prisma.db.derouter_mcpuserenvvars.find_unique.await_count == 1
...
async def test_load_user_env_vars_force_refresh_bypasses_cache(
    ...
    prisma.db.derouter_mcpuserenvvars.find_unique = AsyncMock(
    ...
    assert prisma.db.derouter_mcpuserenvvars.find_unique.await_count == 2
...
def env_vars_salt_key(monkeypatch):
    monkeypatch.setenv("DEROUTER_SALT_KEY", _SALT_KEY)
...
def _mock_env_vars_prisma(row=None):
    ...
    prisma = MagicMock()
    prisma.db.derouter_mcpuserenvvars.find_unique = AsyncMock(return_value=row)
    prisma.db.derouter_mcpuserenvvars.find_many = AsyncMock(return_value=[])
    prisma.db.derouter_mcpuserenvvars.upsert = AsyncMock()
...
def _transactional_env_vars_prisma(read_delay: float = 0.0):
    ...
    class _Tx:
        def __init__(self, store, delay):
            self._store = store
            self._held = None
            self.derouter_mcpuserenvvars = _Table(store, delay=delay)
    ...
    class _DB:
        def __init__(self, store, delay):
            self._store = store
            self._delay = delay
            self.derouter_mcpuserenvvars = _Table(store)
...
async def test_merge_user_env_vars_does_not_persist_plaintext(env_vars_salt_key):
    ...
    row = await prisma.db.derouter_mcpuserenvvars.find_unique(
...
async def test_get_user_env_vars_round_trip(env_vars_salt_key):
    ...
    from derouter.proxy._experimental.mcp_server.db import get_user_env_vars
...
async def test_decode_user_env_vars_warns_when_undecryptable(
    ...
    (not just a debug line) and decode to ``{}`` so a rotated ``DEROUTER_SALT_KEY``
    ...
    import derouter.proxy._experimental.mcp_server.db as mcp_db
    from derouter.proxy._experimental.mcp_server.db import _decode_user_env_vars

    blob = _encrypted_user_env_blob({"CORP_PASSWORD": "s3cret"})

    monkeypatch.setenv("DEROUTER_SALT_KEY", "a-totally-different-salt-key-0000")
    logger = MagicMock()
...
async def test_get_user_env_vars_bulk_distributes_results(env_vars_salt_key):
    ...
    from derouter.proxy._experimental.mcp_server.db import get_user_env_vars_bulk
    ...
    prisma = _mock_env_vars_prisma()
    prisma.db.derouter_mcpuserenvvars.find_many = AsyncMock(return_value=[row1, row2])
...
async def test_get_user_env_vars_bulk_empty_ids_short_circuits():
    ...
    assert prisma.db.derouter_mcpuserenvvars.find_many.await_count == 0
...
async def test_delete_user_env_vars_is_idempotent_delete_many():
    ...
    from derouter.proxy._experimental.mcp_server.db import delete_user_env_vars

    prisma = _mock_env_vars_prisma()
    await delete_user_env_vars(prisma, "alice", "srv-1")
    prisma.db.derouter_mcpuserenvvars.delete_many.assert_awaited_once()
    call = prisma.db.derouter_mcpuserenvvars.delete_many.call_args
...
async def test_merge_user_env_vars_preserves_existing_and_prunes_disallowed(
    ...
    from derouter.proxy._experimental.mcp_server.db import merge_user_env_vars
...
async def test_merge_user_env_vars_acquires_lock_without_deserializing_void(
    ...
    class _Tx:
        def __init__(self):
            self.stored = None
            self.derouter_mcpuserenvvars = self
...
async def test_delete_mcp_server_removes_orphaned_user_env_vars():
    ...
    prisma = _mock_env_vars_prisma()
    prisma.db.derouter_mcpservertable.delete = AsyncMock(return_value=object())
...
async def test_delete_mcp_server_skips_env_var_cleanup_when_server_missing():
    ...
    prisma.db.derouter_mcpuserenvvars.delete_many.assert_not_awaited()
...
async def test_delete_mcp_server_succeeds_when_orphan_cleanup_fails():
    ...
    deleted = object()
    prisma = _mock_env_vars_prisma()
    prisma.db.derouter_mcpservertable.delete = AsyncMock(return_value=deleted)
    prisma.db.derouter_mcpuserenvvars.delete_many = AsyncMock(
...
async def test_delete_mcp_server_removes_orphaned_user_credentials():
    ...
    prisma.db.derouter_mcpusercredentials.delete_many.assert_awaited_once()
    call = prisma.db.derouter_mcpusercredentials.delete_many.call_args
    assert call.kwargs["where"] == {"server_id": "srv-1"}
...
def test_prepare_mcp_server_data_encrypts_global_env_var_values(env_vars_salt_key):
    ...
    from derouter.proxy._types import MCPEnvVar
...
def test_prepare_mcp_server_data_skips_unset_env_vars_on_partial_update():
    ...
    from derouter.proxy._types import MCPEnvVar, UpdateMCPServerRequest
...
async def test_build_mcp_server_from_table_decrypts_global_env_vars(env_vars_salt_key):
    ...
    from derouter.proxy._types import DeRouter_MCPServerTable, MCPEnvVar
...
async def test_create_mcp_server_decrypts_env_vars_when_prisma_returns_json_string(
    ...
    def _prisma_row_with_json_string_env_vars():
        ...
        from prisma.models import DeRouter_MCPServerTable

        return DeRouter_MCPServerTable.model_validate({
    ...
    mock_prisma.db.derouter_mcpservertable.create = AsyncMock(
    ...
    mock_prisma_upd.db.derouter_mcpservertable.update = AsyncMock(
        return_value=_prisma_row_with_json_string_env_vars()
    )
...
async def test_rotate_mcp_user_env_vars_logs_rotated_and_skipped_counts(
    ...
    prisma = MagicMock()
    prisma.db.derouter_mcpuserenvvars.find_many = AsyncMock(
    ...
    update = prisma.db.derouter_mcpuserenvvars.update
...
def test_decrypt_global_env_var_drops_undecryptable_value(
    ...
    ``DEROUTER_SALT_KEY`` can't silently leak ciphertext into ``${NAME}`` headers."""
    import json
    from unittest.mock import MagicMock
