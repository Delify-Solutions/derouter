            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch.object(
                        ...
                        import derouter.proxy._experimental.mcp_server.mcp_server_manager as mgr_mod
...
class TestHookHeaderMergePriority:
    ...
    @pytest.mark.asyncio
    async def test_m2m_oauth2_does_not_forward_derouter_caller_authorization(self):
        """M2M must not put caller Bearer (DeRouter API key) into extra_headers (#23652)."""
        manager = MCPServerManager()
        server = MCPServer(
            server_id="test-id",
            name="Test Server",
...
class TestUserAPIKeyAuthJwtClaims:
    ...
    def test_jwt_claims_field_accepts_dict(self):
        claims = {"sub": "user-123", "iss": "derouter", "exp": 9999999999}
...
class TestOpenApiByokCallTool:
    ...
    async def test_call_tool_openapi_byok_injects_request_auth_contextvar(self):
        ...
        with patch.object(manager, "_resolve_mcp_server_for_tool_call", return_value=server):
            with patch(
                "derouter.proxy._experimental.mcp_server.mcp_server_manager._resolve_byok_mcp_auth_header",
...
class TestResolveByokMcpAuthHeader:
    ...
    async def test_byok_server_checks_credential_and_keeps_caller_header_when_supplied(self):
        ...
        with patch(
            "derouter.proxy._experimental.mcp_server.server._check_byok_credential",
            new=check_mock,
        ):
...
class TestOpenApiResolvedUpstreamAuth:
    ...
    async def test_caller_oauth2_headers_never_become_resolved_for_byok_server(self):
        ...
        resolved, forwarded = await manager.resolve_openapi_upstream_auth(
            mcp_server=server,
            oauth2_headers={"Authorization": "Bearer sk-derouter-gateway-key"},
            raw_headers=None,
            ...
            with patch.object(manager, "check_tool_permission_for_key_team", new_callable=AsyncMock):
                with patch.object(manager, "validate_allowed_params"):
                    await manager.pre_call_tool_check(
                        ...
                        proxy_logging_obj=proxy_logging,
                        server=server,
                        raw_headers={"x-nuid": "nuid-1", "x-derouter-api-key": "sk-proxy"},
                    )
