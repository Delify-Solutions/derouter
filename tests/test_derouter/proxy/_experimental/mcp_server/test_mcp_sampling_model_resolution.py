...
class TestHintMatching:
    ...
    @patch("derouter.model_list", [{"model_name": "gpt-4o"}, {"model_name": "claude-3"}])
    ...
    @patch("derouter.proxy.proxy_server.llm_router", None)
    @patch("derouter.model_list", ["gpt-4o", "claude-3"])
    ...
    @patch("derouter.model_list", None)
    ...
    @patch("derouter.proxy.proxy_server.llm_router", None)
    @patch("derouter.model_list", [{"model_name": "gpt-4o"}])
...
class TestFallbackChain:
    @patch("derouter.proxy.proxy_server.llm_router", None)
    @patch(
        "derouter.model_list", [{"model_name": "first-model"}, {"model_name": "second"}]
    )
    ...
    @patch("derouter.proxy.proxy_server.llm_router", None)
    @patch("derouter.model_list", [])
    def test_should_use_configured_default_sampling_model(self, monkeypatch):
        import derouter

        monkeypatch.setattr(
            derouter, "default_mcp_sampling_model", "fallback-model", raising=False
        )
        prefs = _prefs()
    ...
    def test_should_raise_when_nothing_resolvable(self, monkeypatch):
        ...
        monkeypatch.setattr(derouter, "default_mcp_sampling_model", None, raising=False)
        prefs = _prefs()
