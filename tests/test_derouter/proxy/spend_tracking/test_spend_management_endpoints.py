...
def make_ui_spend_logs_mock_prisma(mock_spend_logs, filter_fn, team_lookup_fn=None, query_observer=None):
    ...
    Args:
        ...
        team_lookup_fn: Optional async callable for team RBAC (find_unique).
                        If provided, adds derouter_teamtable to db.
    """
    ...
    class MockPrismaClient:
        def __init__(self):
            self.db = MockDB()
            self.db.derouter_spendlogs = self.db
            if team_lookup_fn is not None:
                self.db.derouter_teamtable = self
...
from derouter.proxy.management_endpoints import common_utils
from derouter.proxy.proxy_server import app
from derouter.proxy.spend_tracking import spend_management_endpoints
from derouter.router import Router
from derouter.types.utils import BudgetConfig
...
async def test_can_team_member_view_log_none_team_id():
    ...
    class MockPrisma:
        class DB:
            ...
            def __init__(self):
                self.derouter_teamtable = self.TeamTable()
...
async def test_assert_user_can_view_request_id_rejects_both_users_none():
    ...
    class MockDB:
        def __init__(self):
            self.derouter_spendlogs = MockSpendLogs()
...
ignored_keys = [
    ...
    "metadata.derouter_overhead_time_ms",
...
def disable_budget_sync(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.router_strategy.budget_limiter.RouterBudgetLimiting.periodic_sync_in_memory_spend_with_redis",
        noop,
    )
...
async def test_ui_view_spend_logs_sort_by_and_sort_order(
    ...
    class MockPrismaClient:
        def __init__(self):
            self.db = MagicMock()
            self.db.derouter_spendlogs = MagicMock()
            self.db.derouter_spendlogs.count = AsyncMock(side_effect=mock_count)
            self.db.query_raw = AsyncMock(side_effect=mock_query_raw)

    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", MockPrismaClient())
    monkeypatch.setattr(
        "derouter.proxy.spend_tracking.spend_management_endpoints._is_admin_view_safe",
...
async def test_ui_view_spend_logs_sort_validation_errors(
    ...
    class MockPrismaClient:
        def __init__(self):
            self.db = MagicMock()
            self.db.derouter_spendlogs = MagicMock()
            self.db.derouter_spendlogs.find_many = AsyncMock(return_value=[])
            self.db.derouter_spendlogs.count = AsyncMock(side_effect=mock_count)
...
async def test_ui_view_spend_logs_exclude_internal_health_checks(client, monkeypatch):
    ...
    def observe_query(sql_query, params):
        if 'FROM "DeRouter_SpendLogs"' in sql_query:
...
async def test_ui_view_spend_logs_explicit_user_filter_cannot_escape_own_scope(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.proxy.spend_tracking.spend_management_endpoints._get_permitted_team_ids_for_spend_logs",
...
async def test_ui_view_spend_logs_request_id_blocks_non_owner(client, monkeypatch):
    ...
    class _DB:
        def __init__(self):
            self.derouter_spendlogs = _SpendLogs()

    class _Prisma:
        def __init__(self):
            self.db = _DB()

    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", _Prisma())
    app.dependency_overrides[ps.user_api_key_auth] = lambda: UserAPIKeyAuth(
...
async def test_ui_view_spend_logs_search_keeps_non_admin_scope(client, monkeypatch, search, expected_request_ids):
    ...
    monkeypatch.setattr(
        "derouter.proxy.spend_tracking.spend_management_endpoints._assert_user_can_view_request_id",
        ownership_check,
    )
    app.dependency_overrides[ps.user_api_key_auth] = lambda: UserAPIKeyAuth(
        user_role=LitellmUserRoles.INTERNAL_USER, user_id="internal_user_1"
...
async def test_ui_view_spend_logs_with_model_id(client, monkeypatch):
    """Test that the model_id query param filters spend logs by derouter model deployment id."""
    mock_spend_logs = [
        {
            "id": "log1",
            "request_id": "req1",
...
class TestSpendLogsPayload:
    def setup_method(self):
        self._original_callbacks = derouter.callbacks[:]
        self._original_cache = derouter.cache
        derouter.cache = None

    def teardown_method(self):
        derouter.callbacks = self._original_callbacks
        derouter.cache = self._original_cache

    @pytest.mark.asyncio
    async def test_spend_logs_payload_e2e(self):
        derouter.callbacks = [_ProxyDBLogger(message_logging=False)]
        # derouter._turn_on_debug()

        with (
            patch.object(
                derouter.proxy.db.db_spend_update_writer.DBSpendUpdateWriter,
                "_insert_spend_log_to_db",
            ) as mock_client,
            patch.object(derouter.proxy.proxy_server, "prisma_client"),
        ):
            ...
            expected_payload = SpendLogsPayload(
                **{
                    ...
                    "user": "",
                    "team_id": "",
                    "metadata": '{"applied_guardrails": [], "attempted_fallbacks": null, "original_model_group": null, "batch_models": null, "batch_successful_requests": null, "batch_failed_requests": null, "mcp_tool_call_metadata": null, "vector_store_request_metadata": null, "routing_decision": null, "internal_call_origin": null, "guardrail_information": null, "compression_savings": null, "derouter_gateway_injected_cache": null, "router_metadata": null, "usage_object": {"completion_tokens": 20, "prompt_tokens": 10, "total_tokens": 30, "completion_tokens_details": null, "prompt_tokens_details": null}, "model_map_information": {"model_map_key": "gpt-4o", "model_map_value": {"key": "gpt-4o", "max_tokens": 16384, "max_input_tokens": 128000, "max_output_tokens": 16384, "input_cost_per_token": 2.5e-06, "cache_creation_input_token_cost": null, "cache_read_input_token_cost": 1.25e-06, "input_cost_per_character": null, "input_cost_per_token_above_128k_tokens": null, "input_cost_per_token_above_200k_tokens": null, "input_cost_per_query": null, "input_cost_per_second": null, "input_cost_per_audio_token": null, "input_cost_per_token_batches": 1.25e-06, "output_cost_per_token_batches": 5e-06, "output_cost_per_token": 1e-05, "output_cost_per_audio_token": null, "output_cost_per_character": null, "output_cost_per_token_above_128k_tokens": null, "output_cost_per_character_above_128k_tokens": null, "output_cost_per_token_above_200k_tokens": null, "output_cost_per_second": null, "output_cost_per_reasoning_token": null, "output_cost_per_image": null, "output_vector_size": null, "derouter_provider": "openai", "mode": "chat", "supports_system_messages": true, "supports_response_schema": true, "supports_vision": true, "supports_function_calling": true, "supports_tool_choice": true, "supports_assistant_prefill": false, "supports_prompt_caching": true, "supports_audio_input": false, "supports_audio_output": false, "supports_pdf_input": false, "supports_embedding_image_input": false, "supports_native_streaming": null, "supports_web_search": true, "supports_reasoning": false, "search_context_cost_per_query": {"search_context_size_low": 0.03, "search_context_size_medium": 0.035, "search_context_size_high": 0.05}, "tpm": null, "rpm": null, "supported_openai_params": ["frequency_penalty", "logit_bias", "logprobs", "top_logprobs", "max_tokens", "max_completion_tokens", "modalities", "prediction", "n", "presence_penalty", "seed", "stop", "stream", "stream_options", "temperature", "top_p", "tools", "tool_choice", "function_call", "functions", "max_retries", "extra_headers", "parallel_tool_calls", "audio", "response_format", "user"]}}, "additional_usage_values": {"completion_tokens_details": null, "prompt_tokens_details": null}}',
                    "cache_key": "Cache OFF",
...
async def test_view_spend_logs_bounds_row_count(client, monkeypatch):
    ...
    try:
        ...
        assert "x-derouter-spend-logs-truncated" not in response.headers
        ...
        assert response.headers["x-derouter-spend-logs-truncated"] == "true"
    finally:
        app.dependency_overrides.pop(ps.user_api_key_auth, None)
...
async def test_view_spend_tags(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.proxy.spend_tracking.spend_management_endpoints.get_spend_by_tags",
...
async def test_provider_budget_provider_budgets(disable_budget_sync):
    ...
    with patch("derouter.proxy.proxy_server.llm_router", router):
...
async def test_build_ui_spend_logs_response_dict_rows_session_counts():
    ...
    mock_prisma.db.derouter_spendlogs.group_by = AsyncMock()
    ...
    mock_prisma.db.derouter_spendlogs.group_by.assert_not_called()
...
async def test_view_spend_logs_internal_user_combines_user_with_api_key(
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_client)
...
class _SpendScopeMockPrismaClient:
    ...
    def __init__(self, get_data_returns=None, find_many_returns=None):
        ...
        class _DB:
            def __init__(self):
                self.derouter_verificationtoken = _VerificationTokenTable()
...
def _cold_storage_handler(payload):
    from derouter.proxy.spend_tracking.cold_storage_handler import ColdStorageHandler
...
async def test_cold_storage_handler_returns_none_when_no_logger_configured(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "cold_storage_custom_logger", None, raising=False)
...
async def test_cold_storage_handler_resolves_configured_logger_from_registry(
    ...
    monkeypatch.setattr(derouter, "cold_storage_custom_logger", "s3_v2", raising=False)
    monkeypatch.setattr(
        derouter.logging_callback_manager,
...
def test_ui_view_request_response_reads_from_cold_storage(client, monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", fake_prisma)
...
def _spend_report_mock_prisma(query_raw_returns=None, team_rows=None, user_row=None):
    ...
    pc.db.derouter_teamtable.find_many = AsyncMock(
        return_value=team_rows if team_rows is not None else []
    )
    pc.db.derouter_usertable.find_unique = AsyncMock(return_value=user_row)
    return pc
...
def test_scoped_spend_report_routes_reachable_by_non_admin_roles():
    ...
    for path in _SCOPED_SPEND_REPORT_PATHS:
        assert path in DeRouterRoutes.spend_tracking_routes.value
        assert path in DeRouterRoutes.internal_user_routes.value
        assert path in DeRouterRoutes.internal_user_view_only_routes.value
        assert path in DeRouterRoutes.org_admin_allowed_routes.value
