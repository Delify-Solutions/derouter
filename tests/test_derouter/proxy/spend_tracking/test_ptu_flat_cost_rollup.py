...
import derouter.proxy.spend_tracking.ptu_flat_cost_rollup as ptu_rollup
from derouter.constants import PTU_ROLLUP_MAX_BACKFILL_DAYS, PTU_SENTINEL_API_KEY
...
def _prisma_with_models(rows, existing_sentinel_rows=()):
    ...
    prisma.db = types.SimpleNamespace(derouter_proxymodeltable=model_table, derouter_dailyteamspend=daily)
    return prisma, daily
...
def _router_entry(model_id="dep-1", model_name="gpt-4o-ptu", model_info=None, with_start=True):
    ...
    return {
        ...
        "derouter_params": {"model": "azure/gpt-4o"},
        "model_info": info,
    }
...
async def test_scheduled_rollup_releases_the_lock_even_when_the_run_raises():
    prisma, table = _prisma_with_models([])
    prisma.db.derouter_proxymodeltable.find_many = AsyncMock(side_effect=RuntimeError("db down"))
    lock = _pod_lock(acquired=True)
...
class _FakeSentinelTable:
    """In-memory DeRouter_DailyTeamSpend that honours the sentinel key and prune predicate."""
...
def _prisma_for(model_rows, daily_table):
    ...
    prisma.db = types.SimpleNamespace(derouter_proxymodeltable=model_table, derouter_dailyteamspend=daily_table)
    return prisma
...
async def test_backfill_counts_a_charge_that_never_landed():
    ...
    prisma.db.derouter_dailyteamspend.upsert = AsyncMock(side_effect=RuntimeError("db down"))
...
async def test_a_failing_backfill_does_not_lose_the_days_rollup_result():
    ...
    table = _FakeSentinelTable()
    prisma = _prisma_for([_windowed_row()], table)
    prisma.db.derouter_dailyteamspend.find_many = AsyncMock(side_effect=RuntimeError("read replica down"))
...
async def test_a_config_declared_deployment_is_priced():
    ...
    DeRouter_ProxyModelTable, so a DB-only scan bills the provider's reservation to nobody."""
