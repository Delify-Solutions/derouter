...
from derouter.constants import (
    DEROUTER_TRUNCATED_PAYLOAD_FIELD,
    DEROUTER_TRUNCATION_DB_SAFEGUARD_NOTE,
...
from derouter.proxy.spend_tracking.spend_tracking_utils import (
...
from derouter.proxy._types import SpendLogsPayload
...
def _get_additional_usage_values_for_usage(usage: derouter.Usage) -> dict:
    payload = get_logging_payload(
        kwargs={
            "model": "gpt-4o-mini",
            "derouter_params": {"metadata": {"user_api_key": "test-key"}},
        },
...
def _trace_only_session_id(omit_when_missing: bool) -> str | None:
    """get_derouter_params copies metadata.trace_id into derouter_session_id, so every field echoes the trace id."""
    return _get_session_id_for_spend_log(
        kwargs={"derouter_trace_id": "trace-abc", "derouter_session_id": "trace-abc"},
...
def test_omit_records_metadata_session_id():
    session_id: Final = _get_session_id_for_spend_log(
        kwargs={"derouter_session_id": "chain-1"},
...
def test_batch_lifecycle_rows_derive_the_same_session_from_the_batch_id():
    ...
    from derouter.proxy.spend_tracking.spend_tracking_utils import _get_batch_trace_session_id
...
def test_batch_session_outranks_the_per_request_trace_id():
    ...
    session_id: Final = _get_session_id_for_spend_log(
        kwargs={"derouter_trace_id": "trace-abc"},
        metadata={"trace_id": "trace-abc"},
        standard_logging_payload=_TRACE_ONLY_STANDARD_LOGGING,
...
def test_get_logging_payload_groups_batch_create_and_cost_rows_in_one_session():
    def _payload(call_type: str) -> SpendLogsPayload:
        return get_logging_payload(
            ...
            response_obj=derouter.ModelResponse(id="batch-uid-1", choices=[], usage=derouter.Usage()),
            start_time=datetime.datetime.now(timezone.utc),
            end_time=datetime.datetime.now(timezone.utc),
        )
...
def test_get_logging_payload_reads_omit_decision_stamped_on_request(
    ...
    with patch(  # test-quality-ok: proves log time ignores proxy config; general_settings is yaml, not an HTTP boundary
        "derouter.proxy.proxy_server.general_settings", {"missing_session_id": "generate"}
    ):
        payload: SpendLogsPayload = get_logging_payload(
            kwargs={
                "model": "gpt-4o-mini",
                "derouter_trace_id": "trace-abc",
                "derouter_params": {"derouter_session_id": "trace-abc", "metadata": request_metadata},
                "standard_logging_object": _TRACE_ONLY_STANDARD_LOGGING,
            },
            response_obj=derouter.ModelResponse(id="chatcmpl-test", choices=[]),
            start_time=datetime.datetime.now(timezone.utc),
            end_time=datetime.datetime.now(timezone.utc),
        )
...
def test_get_logging_payload_applies_omit_to_requests_that_carry_no_stamp(policy: str | None):
    ...
    with patch(  # test-quality-ok: general_settings is proxy config, loaded from yaml, not an HTTP boundary
        "derouter.proxy.proxy_server.general_settings", {} if policy is None else {"missing_session_id": policy}
    ):
        payload: SpendLogsPayload = get_logging_payload(
            kwargs={
                "model": "claude-opus-4",
                "derouter_trace_id": "trace-abc",
                "derouter_params": {"derouter_session_id": "trace-abc", "metadata": {"trace_id": "trace-abc"}},
                "standard_logging_object": _TRACE_ONLY_STANDARD_LOGGING,
            },
            response_obj=derouter.ModelResponse(id="chatcmpl-test", choices=[]),
            start_time=datetime.datetime.now(timezone.utc),
...
def test_sanitize_request_body_for_spend_logs_payload_long_string():
    from derouter.constants import MAX_STRING_LENGTH_PROMPT_IN_DB
    ...
    expected_truncation_message = f"... ({DEROUTER_TRUNCATED_PAYLOAD_FIELD} skipped {skipped_chars} chars. {DEROUTER_TRUNCATION_DB_SAFEGUARD_NOTE}) ..."
...
@patch("derouter.proxy.spend_tracking.spend_tracking_utils._should_store_prompts_and_responses_in_spend_logs")
...
def test_get_response_for_spend_logs_payload_truncates_large_base64(mock_should_store):
    ...
    assert DEROUTER_TRUNCATED_PAYLOAD_FIELD in truncated_value
...
def test_truncation_includes_db_safeguard_note():
    ...
    assert DEROUTER_TRUNCATED_PAYLOAD_FIELD in truncated
    assert DEROUTER_TRUNCATION_DB_SAFEGUARD_NOTE in truncated
...
def test_response_truncation_logs_info_message(mock_should_store):
    ...
    with patch("derouter.proxy.spend_tracking.spend_tracking_utils.verbose_proxy_logger") as mock_logger:
...
def test_request_body_truncation_logs_info_message(mock_should_store):
    ...
    derouter_params = {"proxy_server_request": {"body": {"messages": [{"role": "user", "content": large_prompt}]}}}

    with patch("derouter.proxy.spend_tracking.spend_tracking_utils.verbose_proxy_logger") as mock_logger:
        _get_proxy_server_request_for_spend_logs_payload(metadata={}, derouter_params=derouter_params, kwargs={})
        mock_logger.info.assert_called_once()
        log_msg = mock_logger.info.call_args[0][0]
...
@pytest.mark.asyncio
@patch("derouter.proxy.proxy_server.master_key", "sk-master-key")
@patch("derouter.proxy.proxy_server.general_settings", {})
...
def test_get_logging_payload_includes_overhead_in_spend_logs_metadata():
    """
    Test that get_logging_payload extracts derouter_overhead_time_ms from hidden_params
    ...
    standard_logging_payload = StandardLoggingPayload(
        ...
        hidden_params=StandardLoggingHiddenParams(
            ...
            api_base="https://api.openai.com",
            response_cost="0.001",
            derouter_overhead_time_ms=test_overhead_ms,
            additional_headers=None,
            batch_models=None,
            derouter_model_name=None,
            usage_object=None,
    ...
    assert metadata.get("derouter_overhead_time_ms") == test_overhead_ms, (
        f"Expected overhead '{test_overhead_ms}', got '{metadata.get('derouter_overhead_time_ms')}'"
    )
...
def test_get_logging_payload_handles_missing_overhead_gracefully():
    ...
    standard_logging_payload = StandardLoggingPayload(
        ...
        hidden_params=StandardLoggingHiddenParams(
            ...
            api_base="https://api.openai.com",
            response_cost="0.001",
            derouter_overhead_time_ms=None,  # No overhead
            additional_headers=None,
            batch_models=None,
            derouter_model_name=None,
            usage_object=None,
    ...
    # When overhead is None, derouter_overhead_time_ms should be None or not present
    assert metadata.get("derouter_overhead_time_ms") is None, (
        "derouter_overhead_time_ms should be None when overhead is not provided"
    )
...
def test_spend_logs_redacts_request_and_response_when_turn_off_message_logging_enabled(
    ...
    request_result = _get_proxy_server_request_for_spend_logs_payload(
        metadata=metadata, derouter_params=derouter_params, kwargs=kwargs
    )

    parsed_request = json.loads(request_result)
    assert parsed_request["messages"] == [{"role": "user", "content": "redacted-by-derouter"}]
    ...
    assert parsed_response["choices"][0]["message"]["content"] == "redacted-by-derouter"
    assert parsed_response["choices"][0]["message"]["role"] == "assistant"


@patch("derouter.secret_managers.main.get_secret_bool")
...
def test_sanitize_guardrail_information_redacts_all_prompt_carrying_fields_when_flag_false(
    ...
    in-tree writers (derouter_content_filter, block_code_execution) inline
...
def test_sanitize_guardrail_information_redacts_prompt_fields_when_flag_false(
    ...
    land in DeRouter_SpendLogs.metadata, while every other field on the
...
def test_get_logging_payload_guardrail_info_when_no_standard_logging_payload():
    ...
    with patch("derouter.proxy.proxy_server.master_key", "sk-master"):
        with patch(
            "derouter.proxy.proxy_server.general_settings",
            {"store_prompts_in_spend_logs": True},
        ):
...
def test_get_logging_payload_includes_request_duration_ms():
    ...
    kwargs = {
        "model": "gpt-4",
        "derouter_params": {"api_base": "https://api.openai.com"},
...
def test_proxy_server_request_payload_excludes_secret_fields(mock_should_store):
    ...
    result = _get_proxy_server_request_for_spend_logs_payload(metadata={}, derouter_params=derouter_params, kwargs={})
    parsed = json.loads(result)
...
def test_sanitize_error_information_caps_size_regardless_of_prompt_flag(
    ...
    assert DEROUTER_TRUNCATED_PAYLOAD_FIELD in sanitized["error_message"]
    assert DEROUTER_TRUNCATED_PAYLOAD_FIELD in sanitized["traceback"]
...
def test_sanitize_error_information_reproduces_lit_2992(mock_should_store):
    ...
    error_message = (
        "derouter.RateLimitError: RateLimitError: OpenAIException - "
...
def test_redact_logged_api_key_master_key_alias_passes_through():
    ...
    result = _redact_logged_api_key(DEROUTER_PROXY_MASTER_KEY_ALIAS, already_redacted=True)
    assert result == DEROUTER_PROXY_MASTER_KEY_ALIAS


def test_redact_logged_api_key_master_key_alias_without_provenance_is_hashed():
    from derouter.constants import DEROUTER_PROXY_MASTER_KEY_ALIAS

    result = _redact_logged_api_key(DEROUTER_PROXY_MASTER_KEY_ALIAS)
    assert result == hash_token(DEROUTER_PROXY_MASTER_KEY_ALIAS)
    assert result != DEROUTER_PROXY_MASTER_KEY_ALIAS
...
def test_get_spend_logs_metadata_keeps_master_key_alias_readable():
    ...
    meta = _get_spend_logs_metadata(
        {
            "user_api_key": DEROUTER_PROXY_MASTER_KEY_ALIAS,
            "user_api_key_hash": DEROUTER_PROXY_MASTER_KEY_ALIAS,
        }
    )
    assert meta["user_api_key"] == DEROUTER_PROXY_MASTER_KEY_ALIAS
...
def test_get_logging_payload_batch_attribution_keeps_verification_token_hash():
    ...
    DeRouter_VerificationToken for api_key_alias and user_email. Regression: without
...
def test_get_logging_payload_uses_recovered_combined_usage_on_failure():
    ...
    kwargs = {
        ...
        "call_type": "acompletion",
        "derouter_params": {"metadata": {"user_api_key": "sk-test"}},
...
def test_get_logging_payload_sets_derouter_call_id_for_correlation():
    """LIT-3868: a successful spend log must carry the x-derouter-call-id (the
    ...
    kwargs = {
        ...
        "call_type": "acompletion",
        "derouter_call_id": trace_call_id,
        "derouter_params": {"metadata": {"user_api_key": "sk-test"}},
    }
    response_obj = {
    ...
    assert metadata["derouter_call_id"] == trace_call_id
    assert metadata["derouter_call_id"] != payload["request_id"]


def test_get_logging_payload_derouter_call_id_falls_back_to_derouter_params():
    """derouter_call_id may only be present in derouter_params; it must still land
    ...
    assert json.loads(payload["metadata"])["derouter_call_id"] == trace_call_id


def test_get_logging_payload_derouter_call_id_when_response_has_no_id():
...
def test_get_logging_payload_cache_hit_keeps_raw_derouter_call_id():
    """On a cache hit request_id is suffixed to stay unique, but the metadata
    derouter_call_id stays the raw trace id so the row still points at its trace.
    """
    ...
    assert json.loads(payload["metadata"])["derouter_call_id"] != payload["request_id"]
...
def test_get_logging_payload_keeps_master_key_alias_readable():
    ...
    assert payload["api_key"] == DEROUTER_PROXY_MASTER_KEY_ALIAS
    parsed_meta = json.loads(payload["metadata"])
    assert parsed_meta["user_api_key"] == DEROUTER_PROXY_MASTER_KEY_ALIAS
...
def test_get_logging_payload_keeps_internal_service_account_key_readable(service_account: str):
    ...
    kwargs = {
        ...
        "messages": [{"role": "user", "content": "Hello"}],
        "call_type": "acompletion",
        "derouter_params": {"metadata": data["metadata"]},
    }
    payload = get_logging_payload(
        kwargs=kwargs,
        response_obj=Exception("error"),
...
async def test_compression_savings_survive_to_spend_log_payload_metadata(monkeypatch):
    ...
    kwargs' derouter_metadata, the logging object captures it via
    ...
    monkeypatch.setattr(
        "derouter.integrations.compression_interception.handler.compress",
        lambda **kwargs: {
    ...
    call_kwargs = {
        ...
        "max_tokens": 512,
        "derouter_call_id": "test-compression-call-id",
        "derouter_metadata": {
...
def test_no_routing_decision_key_defaults_to_none_in_spend_log_metadata():
    payload = get_logging_payload(
        ...
        response_obj=derouter.ModelResponse(id="chatcmpl-no-routing-decision", choices=[], usage=derouter.Usage()),
        start_time=datetime.datetime.now(timezone.utc),
        end_time=datetime.datetime.now(timezone.utc),
    )
    metadata = json.loads(payload["metadata"])
...
@pytest.mark.parametrize("bucket", ["metadata", "derouter_metadata"])
def test_internal_call_origin_survives_into_spend_log_metadata(bucket):
    ...
    returns derouter_metadata when present and metadata otherwise, and the classifier
    ...
    payload = get_logging_payload(
        ...
        response_obj=derouter.ModelResponse(id="chatcmpl-classifier", choices=[], usage=derouter.Usage()),
        start_time=datetime.datetime.now(timezone.utc),
        end_time=datetime.datetime.now(timezone.utc),
    )
    metadata = json.loads(payload["metadata"])
...
def test_user_traffic_carries_no_internal_call_origin():
    ...
    payload = get_logging_payload(
        ...
        response_obj=derouter.ModelResponse(id="chatcmpl-user-traffic", choices=[], usage=derouter.Usage()),
        start_time=datetime.datetime.now(timezone.utc),
        end_time=datetime.datetime.now(timezone.utc),
    )
    metadata = json.loads(payload["metadata"])
...
REDACTED_RESPONSE_PLACEHOLDER: Final = {"text": "redacted-by-derouter"}
...
def test_get_spend_logs_id_stays_unique_when_the_response_is_a_redaction_placeholder(call_type):
    """request_id is the DeRouter_SpendLogs primary key and the flush inserts with
    ...
    first = get_spend_logs_id(call_type, dict(REDACTED_RESPONSE_PLACEHOLDER), {"derouter_call_id": "call-id-1"})
    second = get_spend_logs_id(call_type, dict(REDACTED_RESPONSE_PLACEHOLDER), {"derouter_call_id": "call-id-2"})
...
def test_get_spend_logs_id_prefers_the_response_id_for_batch_and_file_calls(call_type):
    ...
    assert get_spend_logs_id(call_type, {"id": "batch_abc123"}, {"derouter_call_id": "call-id-1"}) == expected
...
def test_get_spend_logs_id_keys_off_batch_identity_when_the_body_was_redacted(call_type):
    ...
    first = get_spend_logs_id(
        ...
        {"derouter_call_id": "call-id-1", "standard_logging_object": standard_logging_object},
    )
    second = get_spend_logs_id(
        call_type,
        dict(REDACTED_RESPONSE_PLACEHOLDER),
        {"derouter_call_id": "call-id-2", "standard_logging_object": standard_logging_object},
    )
...
def test_get_spend_logs_id_separates_distinct_batches_whose_bodies_were_both_redacted():
    ...
    ids = [
        get_spend_logs_id(
            "aretrieve_batch",
            dict(REDACTED_RESPONSE_PLACEHOLDER),
            {"derouter_call_id": f"call-id-{index}", "standard_logging_object": {"id": batch_id}},
        )
...
def test_get_spend_logs_id_prefers_the_response_id_over_the_standard_logging_id():
    ...
    assert (
        get_spend_logs_id(
            ...
            {"derouter_call_id": "call-id-1", "standard_logging_object": {"id": "id-from-standard-payload"}},
        )
...
async def test_spend_log_request_id_is_the_message_id_a_bridged_streaming_caller_was_streamed():
    ...
    from derouter.derouter_core_utils.derouter_logging import Logging
    from derouter.llms.anthropic.experimental_pass_through.responses_adapters.streaming_iterator import (
    ...
    logging_obj = Logging(
        ...
        call_type="anthropic_messages",
        start_time=datetime.datetime.now(timezone.utc),
        derouter_call_id="6825cafe-0000-4000-8000-000000000001",
        function_id="1234",
    )
...
async def test_spend_log_request_id_is_untouched_when_no_message_id_was_streamed():
    ...
    logging_obj = Logging(
        ...
        call_type="anthropic_messages",
        start_time=datetime.datetime.now(timezone.utc),
        derouter_call_id="6825cafe-0000-4000-8000-000000000002",
        function_id="1234",
    )
    logging_obj.optional_params = {}
...
def test_batch_cost_row_does_not_collide_with_the_batch_creation_row():
    ...
    creation_row_id = get_spend_logs_id("acreate_batch", {"id": batch_id}, {"derouter_call_id": "call-create"})
    cost_row_id = get_spend_logs_id(
        "aretrieve_batch",
        dict(REDACTED_RESPONSE_PLACEHOLDER),
        {"derouter_call_id": "call-poller", "standard_logging_object": {"id": batch_id}},
    )
...
def test_batch_cost_row_id_is_stable_across_repeated_accounting():
    ...
    ids = [
        get_spend_logs_id(
            "aretrieve_batch",
            dict(REDACTED_RESPONSE_PLACEHOLDER),
            {"derouter_call_id": f"call-{index}", "standard_logging_object": standard_logging_object},
        )
...
def test_get_logging_payload_failed_request_falls_back_to_standard_logging_payload():
    ...
    payload = get_logging_payload(
        kwargs={
            "model": "group-x",
            "derouter_params": {"metadata": {"user_api_key": "test-key", "status": "failure"}},
...
class _ModelRouterSpendLogKwargs(TypedDict):
    model: ReadOnly[str]
    derouter_params: ReadOnly[dict[str, dict[str, str]]]
...
def _model_router_spend_log_kwargs(slp_model: str | None) -> _ModelRouterSpendLogKwargs:
    ...
    return {
        ...
        "derouter_params": {"metadata": {"user_api_key": "sk-test-key"}},
        "standard_logging_object": standard_logging_payload,
    }
...
def test_autorouter_savings_flow_from_logging_payload_into_spend_log_metadata():
    ...
    payload = get_logging_payload(
        ...
        response_obj=derouter.ModelResponse(id="chatcmpl-ar-savings", choices=[], usage=derouter.Usage()),
        start_time=datetime.datetime.now(timezone.utc),
        end_time=datetime.datetime.now(timezone.utc),
    )
    metadata = json.loads(payload["metadata"])
...
def test_caller_forged_autorouter_savings_is_discarded(bucket):
    ...
    payload = get_logging_payload(
        kwargs={
            "model": "gpt-4o-mini",
            "derouter_params": {bucket: {"user_api_key": "test-key", "autorouter_savings": 999.0}},
        },
        response_obj=derouter.ModelResponse(id="chatcmpl-forged-savings", choices=[], usage=derouter.Usage()),
        start_time=datetime.datetime.now(timezone.utc),
        end_time=datetime.datetime.now(timezone.utc),
    )
    metadata = json.loads(payload["metadata"])
...
def test_injected_cache_breakpoints_survive_into_spend_log_metadata(bucket):
    ...
    stamp metadata while /v1/messages routes stamp derouter_metadata, and
    ...
    payload = get_logging_payload(
        kwargs={
            ...
            "derouter_params": {
                bucket: {
                    "user_api_key": "test-key",
                    "derouter_gateway_injected_cache": "dep-of-this-row",
                }
            },
        },
        response_obj=derouter.ModelResponse(id="chatcmpl-injected", choices=[], usage=derouter.Usage()),
        start_time=datetime.datetime.now(timezone.utc),
        end_time=datetime.datetime.now(timezone.utc),
    )
    metadata = json.loads(payload["metadata"])
    assert metadata["derouter_gateway_injected_cache"] == "dep-of-this-row"
...
def test_passthrough_caching_carries_no_injection_marker():
    ...
    payload = get_logging_payload(
        ...
        response_obj=derouter.ModelResponse(id="chatcmpl-passthrough", choices=[], usage=derouter.Usage()),
        start_time=datetime.datetime.now(timezone.utc),
        end_time=datetime.datetime.now(timezone.utc),
    )
    metadata = json.loads(payload["metadata"])
    assert metadata["derouter_gateway_injected_cache"] is None
...
def _routed_call_kwargs(model_info: Mapping[str, object]) -> dict[str, object]:
    return {
        ...
        "derouter_call_id": "router-corr-123",
        "derouter_params": {
            "metadata": {
                "user_api_key": "test-key",
...
def test_router_metadata_stamped_for_internal_router_model_deployment():
    ...
    payload = get_logging_payload(
        ...
        response_obj=derouter.ModelResponse(id="chatcmpl-router-meta", choices=[], usage=derouter.Usage()),
        start_time=datetime.datetime.now(timezone.utc),
        end_time=datetime.datetime.now(timezone.utc),
    )
    metadata = json.loads(payload["metadata"])
...
def test_router_metadata_absent_without_internal_router_model_flag():
    payload = get_logging_payload(
        ...
        response_obj=derouter.ModelResponse(id="chatcmpl-unflagged", choices=[], usage=derouter.Usage()),
        start_time=datetime.datetime.now(timezone.utc),
        end_time=datetime.datetime.now(timezone.utc),
    )
    metadata = json.loads(payload["metadata"])
...
def test_caller_forged_router_metadata_is_discarded(bucket):
    ...
    payload = get_logging_payload(
        ...
        response_obj=derouter.ModelResponse(id="chatcmpl-forged-router-meta", choices=[], usage=derouter.Usage()),
        start_time=datetime.datetime.now(timezone.utc),
        end_time=datetime.datetime.now(timezone.utc),
    )
    metadata = json.loads(payload["metadata"])
...
def _anthropic_messages_logging_obj(*, stream: bool) -> Any:
    ...
    logging_obj = Logging(
        ...
        call_type="anthropic_messages",
        start_time=datetime.datetime.now(timezone.utc),
        derouter_call_id="6806cafe-0000-4000-8000-000000000001",
        function_id="1234",
    )
    logging_obj.optional_params = {}
...
def test_spend_log_request_id_is_the_message_id_a_streaming_messages_caller_received():
    ...
    to stamp derouter_call_id over the msg_ id carried by the message_start event.
    """
...
def test_spend_log_request_id_still_falls_back_to_derouter_call_id_without_a_provider_id():
    """
    Anthropic-compatible upstreams that omit `id` must keep landing on derouter_call_id rather
    ...
    AnthropicPassthroughLoggingHandler._create_anthropic_response_logging_payload(
        derouter_model_response=derouter.ModelResponse(id="chatcmpl-generated"),
        model="claude-haiku-4-5",
        kwargs={},
        start_time=datetime.datetime.now(timezone.utc),
        end_time=datetime.datetime.now(timezone.utc),
...
def test_spend_log_request_id_for_chat_completions_is_untouched():
    ...
    assert (
        _spend_log_request_id(
            response_obj=derouter.ModelResponse(id="chatcmpl-EJvWIw3DAhuKYuwp3jJI4Pnhp2vjv", choices=[]),
            kwargs={
                "call_type": "acompletion",
                "model": "gpt-5.6",
                "derouter_call_id": "6806cafe-0000-4000-8000-000000000002",
                "derouter_params": {"metadata": {"user_api_key": "test-key"}},
            },
        )
    ...
    assert (
        _spend_log_request_id(
            ...
            kwargs={
                "call_type": "anthropic_messages",
                "model": "gpt-5.6",
                "derouter_call_id": "6806cafe-0000-4000-8000-000000000003",
                "derouter_params": {"metadata": {"user_api_key": "test-key"}},
            },
        )
