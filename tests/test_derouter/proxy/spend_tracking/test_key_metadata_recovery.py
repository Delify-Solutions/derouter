...
def _query_raw_by_table(
    ...
    async def query_raw(sql: str, *params: object) -> list[dict[str, str | None]]:
        if '"DeRouter_VerificationToken"' in sql:
            return list(active_rows)
        if '"DeRouter_DeletedVerificationToken"' in sql:
...
async def test_recover_permanent_miss_costs_two_digest_lookups_and_no_table_walk():
    ...
    mock_prisma.db.derouter_verificationtoken.find_many.assert_not_called()
...
async def test_fill_missing_api_key_aliases_updates_null_alias_and_email_rows():
    ...
    assert mock_prisma.db.derouter_usertable.find_many.call_args.kwargs["where"] == {"user_id": {"in": ["bob"]}}
...
async def test_recover_key_metadata_from_spend_logs_drops_the_owner_of_a_digest_shared_by_several_users():
    ...
    query_raw = _spend_log_transaction(
        mock_prisma,
        _query_raw_spend_logs(
            [{**_spend_log_row(shared_ui_digest, "ui-token", "derouter-dashboard", None), "first_owner": "alice", "last_owner": "bob"}]
        ),
    )
    ...
    assert result[shared_ui_digest] == {"key_alias": "ui-token", "team_id": "derouter-dashboard", "user_id": None}
