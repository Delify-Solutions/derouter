...
async def test_delete_cloudzero_settings_success(client, monkeypatch):
    ...
    mock_derouter_config = MagicMock()
    mock_derouter_config.find_first = AsyncMock(return_value=mock_config)
    mock_derouter_config.delete = AsyncMock(return_value=mock_config)

    mock_prisma = MagicMock()
    mock_prisma.db = MagicMock()
    mock_prisma.db.derouter_config = mock_derouter_config
    ...
    try:
        ...
        mock_derouter_config.find_first.assert_awaited_once()
        mock_derouter_config.delete.assert_awaited_once()
    finally:
...
async def test_delete_cloudzero_settings_not_found(client, monkeypatch):
    mock_derouter_config = MagicMock()
    mock_derouter_config.find_first = AsyncMock(return_value=None)
    ...
    try:
        ...
        mock_derouter_config.find_first.assert_awaited_once()
        mock_derouter_config.delete.assert_not_called()
    finally:
        app.dependency_overrides.pop(ps.user_api_key_auth, None)
...
async def test_get_cloudzero_settings_success(client, monkeypatch):
    ...
    with patch(
        "derouter.proxy.spend_tracking.cloudzero_endpoints.decrypt_value_helper"
    ) as mock_decrypt:
        mock_decrypt.return_value = "decrypted_api_key"

        # Mock the masker
        with patch(
            "derouter.proxy.spend_tracking.cloudzero_endpoints._sensitive_masker"
        ) as mock_masker:
