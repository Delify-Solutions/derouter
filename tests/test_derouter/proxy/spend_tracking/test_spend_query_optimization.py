...
async def test_spend_logs_ui_wraps_params_in_at_time_zone_utc(monkeypatch):
    ...
    mock_prisma.db = MagicMock()
    mock_prisma.db.query_raw = AsyncMock(return_value=[])
    mock_prisma.db.derouter_spendlogs = MagicMock()
    mock_prisma.db.derouter_spendlogs.count = AsyncMock(return_value=0)
...
async def test_spend_logs_ui_uses_bounded_count_not_full_scan(monkeypatch):
    ...
    whole time window of a huge DeRouter_SpendLogs table (Aurora ACU spike,
    ...
    mock_prisma.db.derouter_spendlogs.count.assert_not_called()
...
async def test_spend_logs_ui_group_by_session_offset_pages_for_other_sorts(monkeypatch):
    ...
    """
    from derouter.proxy._types import LitellmUserRoles, UserAPIKeyAuth
    from derouter.proxy.spend_tracking.spend_management_endpoints import ui_view_spend_logs
