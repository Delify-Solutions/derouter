...
def _anthropic_costs(model: str) -> tuple[float, float]:
    info = derouter.get_model_info(model=model, custom_llm_provider="anthropic")
...
def test_caching_savings_agree_with_biller_on_the_request_pricing_basis(
    ...
    pricing: Final = derouter.get_model_info(model=model, custom_llm_provider=provider)
    usage: Final = Usage(
...
def test_unpublished_one_hour_price_uses_the_ordinary_write_price() -> None:
    ...
    pricing: Final = derouter.get_model_info(model=model, custom_llm_provider="anthropic")
...
def test_zero_cache_write_price_is_read_as_unpublished():
    ...
    """
    info = derouter.get_model_info(model="deepseek-chat", custom_llm_provider="deepseek")
...
def test_switching_models_mid_conversation_charges_the_cold_cache_write():
    ...
    sonnet = derouter.get_model_info("claude-sonnet-5", "anthropic")
    haiku = derouter.get_model_info("claude-haiku-4-5", "anthropic")
    warm_baseline = (
...
def test_a_cold_switch_never_beats_turning_caching_off():
    ...
    opus = derouter.get_model_info("claude-opus-5", "anthropic")
    haiku = derouter.get_model_info("claude-haiku-4-5", "anthropic")
...
def test_model_without_cache_read_pricing_yields_no_caching_savings():
    ...
    assert derouter.get_model_info(model=model).get("cache_read_input_token_cost") is None
    result = compute_savings_spend(
        model=model,
...
def test_a_baseline_that_prices_caching_implicitly_still_pays_for_its_prompt():
    ...
    gpt5 = derouter.get_model_info("gpt-5", "openai")
...
def _priced_chat_model_without_cache_read_rate() -> tuple[str, str, str]:
    ...
    for key in sorted(derouter.model_cost):
        entry = derouter.model_cost[key]
        provider = entry.get("derouter_provider")
...
def test_a_baseline_with_no_cache_read_rate_is_charged_its_input_rate():
    ...
    baseline = derouter.get_model_info(baseline_name, baseline_provider)
...
def test_the_baseline_is_priced_on_the_basis_the_request_was_billed_at(basis, expected_multiplier):
    ...
    """
    gpt = derouter.get_model_info("gpt-5.5", "openai")
    haiku = derouter.get_model_info("claude-haiku-4-5", "anthropic")
...
def test_the_baseline_is_priced_on_the_vertex_location_the_request_was_billed_at(monkeypatch):
    ...
    gemini = derouter.get_model_info("gemini-3.5-flash", "vertex_ai")
    haiku = derouter.get_model_info("claude-haiku-4-5", "anthropic")
...
def test_a_leftover_configured_baseline_does_not_override_the_recorded_one(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "autorouter_savings_baseline_model", "claude-sonnet-5", raising=False)
    result = compute_savings_spend(
        model="claude-haiku-4-5",
        custom_llm_provider="anthropic",
        compression_saved_tokens=0,
...
def test_rows_written_before_the_field_shipped_recompute():
    ...
    from derouter.proxy.spend_tracking.savings import autorouter_savings_for_request
...
def test_marks_gateway_injection_credits_only_the_deployment_that_was_injected():
    ...
    derouter_call_id, so the deployment is what tells those legs apart. A marker naming a
    ...
    assert marks_gateway_injection({"derouter_gateway_injected_cache": "dep-a"}, "dep-a") is True
    assert marks_gateway_injection({"derouter_gateway_injected_cache": "dep-a"}, "dep-b") is False
    assert marks_gateway_injection({"derouter_gateway_injected_cache": "dep-a"}, None) is False
    # injected before a deployment was chosen, so it is in the payload every leg sends
    assert marks_gateway_injection({"derouter_gateway_injected_cache": ""}, "dep-a") is True
    assert marks_gateway_injection({"derouter_gateway_injected_cache": ""}, None) is True
    assert marks_gateway_injection({"derouter_call_id": "c1"}, "dep-a") is False
    assert marks_gateway_injection({"derouter_gateway_injected_cache": True}, "dep-a") is False
