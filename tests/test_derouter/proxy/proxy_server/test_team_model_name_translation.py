...
These live in tests/test_derouter/proxy/proxy_server/ (not the top-level
...
def _team_row() -> dict:
    return {
        ...
        "derouter_params": {"model": "azure/gpt-5.2-low-rpm-testing"},
        "model_info": {
            "id": "byok-id-1",
...
async def test_model_info_v2_translates_team_model_name(monkeypatch):
    ...
    monkeypatch.setattr(
        ps, "_enrich_model_info_with_derouter_data", lambda model, **kw: model
    )
    import derouter.proxy.agent_endpoints.model_list_helpers as mlh
...
async def test_model_info_v1_list_path_translates_team_model_name(monkeypatch):
    """/v1/model/info list path (no derouter_model_id) must include team-scoped
    ...
    resp = await ps.model_info_v1(user_api_key_dict=admin, derouter_model_id=None)
...
async def test_model_info_v1_unrestricted_key_returns_all_deployments(monkeypatch):
    ...
    resp = await ps.model_info_v1(user_api_key_dict=caller, derouter_model_id=None)
...
async def test_model_info_v1_unrestricted_key_hides_other_team_byok(monkeypatch):
    ...
    prisma_client.db.derouter_usertable.find_unique = AsyncMock(
...
async def test_model_info_v1_team_key_sees_own_byok_regardless_of_user_lookup(
    ...
    prisma_client = MagicMock()
    prisma_client.db.derouter_usertable.find_unique = find_unique
...
async def test_populate_team_access_gives_view_only_admin_full_admin_scope(monkeypatch):
    ...
    prisma_client = MagicMock()
    prisma_client.db.derouter_usertable.find_unique = AsyncMock(
        return_value=DeRouter_UserTable(user_id="viewer", teams=[], models=[])
    )
...
async def test_populate_team_access_grants_config_access_group_model():
    ...
    prisma_client = MagicMock()
    prisma_client.db.derouter_teamtable.find_many = AsyncMock(return_value=[team_db_object])
...
async def test_model_info_v1_team_id_without_db_fails_fast(monkeypatch):
    ...
    monkeypatch.setattr(ps, "_enrich_model_info_with_derouter_data", enrich_spy)
    ...
    with pytest.raises(ps.HTTPException) as exc_info:
        await ps.model_info_v1(
            user_api_key_dict=admin, derouter_model_id=None, teamId="team-abc-123"
        )
...
async def test_model_info_v1_include_team_models_without_db_fails_fast(monkeypatch):
    ...
    with pytest.raises(ps.HTTPException) as exc_info:
        await ps.model_info_v1(
            user_api_key_dict=admin, derouter_model_id=None, include_team_models=True
        )
...
@pytest.mark.asyncio
async def test_model_info_v1_derouter_model_id_team_id_without_db_fails_fast(
    monkeypatch,
):
    """`derouter_model_id` + `teamId` without a connected DB must raise 500 too, not
...
@pytest.mark.asyncio
async def test_model_info_v1_derouter_model_id_include_team_models_filters_inaccessible(
    monkeypatch,
):
    """`derouter_model_id` + `include_team_models` must drop a model the caller cannot
...
@pytest.mark.asyncio
async def test_model_info_v1_derouter_model_id_team_id_applies_team_filter(monkeypatch):
    """`derouter_model_id` + `teamId` must run the teamId filter on the single model
...
async def test_v1_models_translates_team_model_for_access_group_key(monkeypatch):
    ...
    team_dep = {
        ...
        "derouter_params": {"model": "azure/gpt-4.1"},
        "model_info": {
            "id": "id1",
            "team_id": "teamX",
...
async def test_retrieve_model_by_public_name_returns_200(monkeypatch):
    ...
    import derouter
    import derouter.proxy.utils as proxy_utils

    team_row = _team_row()
    router = _public_named_router(team_row)
    deployment = MagicMock()
    deployment.derouter_params.model = "azure/gpt-5.2-low-rpm-testing"
    ...
    monkeypatch.setattr(
        derouter, "get_llm_provider", lambda model: (model, "openai", None, None)
    )
...
def test_get_direct_access_models_resolves_explicit_model_names():
    ...
    user = DeRouter_UserTable(user_id="u", models=["gpt-4o"], teams=[])
...
def test_get_direct_access_models_empty_models_grants_all_non_team_models():
    ...
    user = DeRouter_UserTable(user_id="u", models=[], teams=[])
...
def test_get_direct_access_models_all_proxy_models_key_keeps_team_scoped_user_grant():
    ...
    user = DeRouter_UserTable(user_id="u", models=["byok-model"], teams=[])
...
def test_get_direct_access_models_expands_access_group_grant():
    ...
    user = DeRouter_UserTable(user_id="u", models=["beta-models"], teams=[])
