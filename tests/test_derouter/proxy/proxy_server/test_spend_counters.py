...
def _make_prisma_with_end_user_row(spend: float | None):
    ...
    prisma.db.derouter_endusertable.find_unique = AsyncMock(
...
async def test_get_current_spend_end_user_floor_admits_after_a_reset_on_a_stale_worker(monkeypatch):
    """The reset job zeroes DeRouter_EndUserTable.spend and the shared counter, but it
    ...
    assert result == 0.0
    prisma.db.derouter_endusertable.find_unique.assert_awaited_once_with(where={"user_id": "customer-42"})
    fake_cache.redis_cache.async_set_max.assert_not_called()
...
def _make_window_spend_prisma(row=None, spend_logs_total=0.0):
    prisma = MagicMock()
    prisma.db.derouter_budgetwindowspend.find_unique = AsyncMock(return_value=row)
    prisma.db.derouter_spendlogs.group_by = AsyncMock(
...
async def test_get_current_spend_floors_window_against_maintained_row(monkeypatch):
    ...
    assert result == 15.0
    fake_prisma.db.derouter_spendlogs.group_by.assert_not_awaited()
    fake_cache.redis_cache.async_set_max.assert_awaited_once_with(
        key=counter_key, value=15.0
    )
...
async def test_get_current_spend_floors_window_against_logs_when_row_stale(monkeypatch):
    ...
    assert result == 15.0
    fake_prisma.db.derouter_spendlogs.group_by.assert_awaited_once()
