...
from derouter.router_utils import pattern_match_deployments
...
def patched_token_counter(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "disable_token_counter", False, raising=False)
    monkeypatch.setattr(
        derouter.utils,
        "_select_tokenizer",
    ...
    monkeypatch.setattr(derouter, "token_counter", lambda **kwargs: 7)
    yield
...
def test_token_counter_counts_off_the_event_loop(client, auth_as, patched_token_counter, monkeypatch):
    ...
    monkeypatch.setattr(derouter, "token_counter", recording_counter)
...
def test_supported_openai_params_resolves_router_alias(client, auth_as, monkeypatch):
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "anthropic/claude-opus-4-6", "api_key": "sk-test"},
            }
        ]
    )
    ...
    assert response.status_code == 200
    expected = derouter.get_supported_openai_params(model="claude-opus-4-6", custom_llm_provider="anthropic")
...
def test_supported_openai_params_never_runs_oauth_for_authenticating_providers(client, auth_as, monkeypatch, tmp_path):
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "github_copilot/gpt-4o"},
            },
            {
                "model_name": "openai/*",
    ...
    monkeypatch.setattr(derouter, "get_llm_provider", _oauth_tripwire)
    monkeypatch.setattr(pattern_match_deployments, "get_llm_provider", _oauth_tripwire)
    expected = derouter.get_supported_openai_params(model="gpt-4o", custom_llm_provider="github_copilot")
...
def test_supported_openai_params_invalid_model(client, auth_as, monkeypatch):
    ...
    monkeypatch.setattr(proxy_server, "llm_router", None)
    monkeypatch.setattr(derouter, "get_llm_provider", _raise)
    with auth_as():
...
def patched_transform(monkeypatch):
    ...
    monkeypatch.setattr("derouter.utils.return_raw_request", _fake_return_raw_request)
    yield
...
def test_token_counter_fallback_counts_tools_system_and_anthropic_blocks(client, auth_as, monkeypatch):
    """The ``derouter.token_counter`` fallback counts the request's tools and system prompt, and Anthropic ``image``/``document`` blocks, instead of 500ing."""
    monkeypatch.setattr(proxy_server, "llm_router", None)
    monkeypatch.setattr(derouter, "disable_token_counter", False, raising=False)
    ...
    assert bare == derouter.token_counter(model="claude-fable-5", messages=messages)
    assert full == derouter.token_counter(
...
def test_token_counter_fallback_prompt_with_tools_does_not_500(client, auth_as, monkeypatch):
    ...
    assert response.status_code == 200, response.text
    assert response.json()["total_tokens"] == derouter.token_counter(model="claude-fable-5", text=prompt)
