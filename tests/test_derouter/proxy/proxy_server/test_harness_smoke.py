...
def test_mock_prisma_has_team_table(mock_prisma):
    assert hasattr(mock_prisma.db, "derouter_teamtable")
    assert callable(mock_prisma.db.derouter_teamtable.find_unique)
    assert callable(mock_prisma.db.derouter_teamtable.find_many)


def test_mock_prisma_has_key_table(mock_prisma):
    assert hasattr(mock_prisma.db, "derouter_verificationtoken")
    assert callable(mock_prisma.db.derouter_verificationtoken.find_unique)
...
def _write_cov_xml(tmp_path: Path, line_rate: float, branch_rate: float) -> Path:
    xml = textwrap.dedent(f"""\
        ...
        <coverage version="7.0">
          <packages>
            <package name="derouter.proxy">
              <classes>
                <class filename="derouter/proxy/proxy_server.py"
