...
async def test_flush_spend_logs_queue_on_shutdown_drains_before_disconnect(monkeypatch):
    ...
    drain = AsyncMock()
    import derouter.proxy.utils as utils_mod
...
def test__redact_worker_config_for_logging_dict_masks_all_secret_shapes():
    ...
    """
    from derouter.proxy.proxy_server import _redact_worker_config_for_logging
...
def test_load_from_azure_key_vault_disabled_no_side_effect(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "secret_manager_client", sentinel_secret_mgr, raising=False)

    result = load_from_azure_key_vault(use_azure_key_vault=False)

    observed = {
        "return_value": result,
        "secret_manager_unchanged": derouter.secret_manager_client is sentinel_secret_mgr,
        "called_with": False,
    }
    assert normalize(observed) == {
        "return_value": None,
...
def test_cost_tracking_adds_db_and_shadow_eval_callbacks_when_prisma_set(monkeypatch):
    ...
    before_callbacks = len(derouter.callbacks)
    before_async = len(derouter._async_success_callback)

    cost_tracking()
    cost_tracking()

    observed = {
        "added_to_callbacks": len(derouter.callbacks) - before_callbacks,
        "added_to_async_success": len(derouter._async_success_callback) - before_async,
        "shadow_eval_loggers": sum(isinstance(cb, ShadowEvalLogger) for cb in derouter.callbacks),
        "prisma_was_set": True,
    }
    assert normalize(observed) == {
...
def test_cost_tracking_no_op_when_prisma_missing(monkeypatch):
    ...
    assert derouter._async_success_callback == []
...
def test_get_derouter_model_info_uses_base_model_for_lookup(monkeypatch):
    import derouter

    expected_info = {"max_tokens": 8192, "input_cost_per_token": 0.00003}
    fake_get = MagicMock(return_value=expected_info)
    monkeypatch.setattr(derouter, "get_model_info", fake_get, raising=False)

    model = {
        "model_info": {"base_model": "gpt-4"},
        "derouter_params": {"model": "azure/my-deployment"},
    }
    result = get_derouter_model_info(model=model)
...
def test_get_derouter_model_info_invalid_empty_dict_returns_empty():
    """Empty input means model_to_lookup is None — internal exception is caught
    and the function returns {}."""
    result = get_derouter_model_info(model={})
    assert result == {}
...
def test_startup_warns_for_global_budget_without_database(caplog):
    ...
    assert "derouter.max_budget=100.0" in caplog.text
...
def test_startup_does_not_warn_for_global_budget_with_database(caplog):
    ...
    assert "derouter.max_budget" not in caplog.text
...
def test_proxy_startup_event_warns_for_global_budget_without_database():
    ...
    wrapped = getattr(proxy_startup_event, "__wrapped__", proxy_startup_event)
    source = inspect.getsource(wrapped)
    budget_check_pos = source.find("if prisma_client is not None and derouter.max_budget > 0:")
...
async def test_tuning_baseline_v2_is_created_alongside_the_legacy_row():
    from derouter.router_utils.auto_router_tuning_baseline import DEFAULT_TUNING_FINGERPRINT

    prisma_client = MagicMock()
    prisma_client.db.derouter_config.find_unique = AsyncMock(return_value=None)
    prisma_client.db.derouter_config.create = AsyncMock()
    deployment = {
        "model_name": "a",
        "derouter_params": {"model": "auto_router/complexity_router", "complexity_router_config": {}},
    }
    ...
    assert prisma_client.db.derouter_config.create.await_args.kwargs["data"] == {
...
async def test_tuning_baseline_waits_for_a_complete_db_model_census(monkeypatch):
    ...
    assert result is None
    prisma_client.db.derouter_config.find_unique.assert_not_called()
