...
def _install_derouter_config(mock_prisma: MagicMock) -> MagicMock:
    """Ensure mock_prisma.db.derouter_config exists with async methods (the
    conftest only stubs ``derouter_configtable`` — this is a different table)."""
    table = MagicMock()
...
def test_config_update_happy_admin(client, auth_as, mock_prisma, monkeypatch):
    ...
    _install_derouter_config(mock_prisma)
...
def test_config_update_persists_optional_pre_call_checks(client, auth_as, mock_prisma, monkeypatch):
    ...
    table = _install_derouter_config(mock_prisma)
    monkeypatch.setattr(ps, "prisma_client", mock_prisma)
    fake_proxy_config = MagicMock()
    fake_proxy_config.add_deployment = AsyncMock()
    monkeypatch.setattr(ps, "proxy_config", fake_proxy_config)
...
def test_config_field_info_redacts_nested_secret_for_view_only_admin(
    ...
    assert value["user_table_name"] == "DeRouter_UserTable"
...
def test_config_field_info_redacts_top_level_scalar_for_view_only(
    ...
    table = _install_derouter_config(mock_prisma)
    row = MagicMock()
    row.param_value = {"database_url": "postgresql://admin:p4ss@db:5432/derouter"}
    table.find_first = AsyncMock(return_value=row)
    monkeypatch.setattr(ps, "prisma_client", mock_prisma)
...
def test_config_callback_delete_happy_admin(client, auth_as, mock_prisma, monkeypatch):
    ...
    fake_proxy_config.get_config = AsyncMock(
        return_value={"derouter_settings": {"success_callback": ["langfuse", "slack"]}}
    )
...
def test_config_callback_delete_not_found(client, auth_as, mock_prisma, monkeypatch):
    ...
    fake_proxy_config = MagicMock()
    fake_proxy_config.get_config = AsyncMock(
        return_value={"derouter_settings": {"success_callback": ["slack"]}}
    )
    monkeypatch.setattr(ps, "proxy_config", fake_proxy_config)
...
def test_get_config_callbacks_happy(client, auth_as, mock_prisma, monkeypatch):
    ...
    fake_proxy_config.get_config = AsyncMock(
        return_value={
            "derouter_settings": {"success_callback": []},
            "general_settings": {},
...
def _install_callbacks_config(monkeypatch, mock_prisma):
    ...
    fake_proxy_config.get_config = AsyncMock(
        return_value={
            "derouter_settings": {"success_callback": ["langfuse", "datadog", "otel"]},
