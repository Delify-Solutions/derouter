...
def test_get_client_requested_model_for_streaming_prefers_client_requested():
    request_data = {
        "_derouter_client_requested_model": "gpt-4",
        "model": "openai/internal-gpt-4",
        "derouter_call_id": "abc",
    }
    ...
    snapshot = {
        "result": result,
        "client_field_preserved": request_data["_derouter_client_requested_model"],
...
def test_restamp_streaming_chunk_model_overrides_model_on_basemodel():
    ...
    new_chunk, logged = _restamp_streaming_chunk_model(
        ...
        request_data={"derouter_call_id": "id-1"},
...
def test_get_streaming_fallback_metadata_reads_headers():
    fallback_errors = [
        {
            "message": "derouter.RateLimitError: upstream limited request",
            "type": "RateLimitError",
            "param": None,
            "code": "429",
        }
    ...
    stream = _FakeStream(
        ...
        hidden_params={
            "additional_headers": {
                ...
                "x-derouter-model-group": "fallback-model",
                "x-derouter-fallback-errors": json.dumps(fallback_errors),
            }
        },
    )
...
def test_get_streaming_fallback_metadata_zero_fallback_count():
    stream = _FakeStream(
        [],
        hidden_params={"additional_headers": {"x-derouter-attempted-fallbacks": 0}},
    )
    assert _get_streaming_fallback_metadata(stream) == (False, None, [])
...
def test_restamp_streaming_chunk_model_setattr_exception_logs_and_returns():
    ...
    new_chunk, logged = _restamp_streaming_chunk_model(
        chunk=chunk,
        requested_model_from_client="gpt-4",
        request_data={"derouter_call_id": "test-id"},
        model_mismatch_logged=False,
    )
...
def test_format_fallback_metadata_sse_event():
    ...
    assert payload["derouter_fallback"] == {
        "fallback_model": "fallback-model",
        "errors": fallback_errors,
    }
    assert payload["id"] == "derouter-fallback-metadata"
...
async def test_async_data_generator_emits_fallback_error_metadata_event(monkeypatch):
    ...
    assert metadata_payload["derouter_fallback"] == {
        "fallback_model": "fallback-model",
        "errors": fallback_errors,
    }
    assert metadata_payload["id"] == "derouter-fallback-metadata"
...
from derouter.proxy.proxy_server import (  # noqa: E402
    _iter_with_keepalive,
    _keepalive_from_deployment_config,
    _make_keepalive_resolver,
    _resolve_keepalive_seconds,
)
from derouter.proxy.proxy_server import _KEEPALIVE_MAX_SECONDS, _KEEPALIVE_MIN_SECONDS  # noqa: E402
...
def test_resolve_keepalive_seconds_client_value_ignored_without_override_permission(monkeypatch):
    ...
    deployment.derouter_params.keepalive_seconds = 15.0
    deployment.derouter_params.allow_client_keepalive_override = False
...
def test_resolve_keepalive_seconds_request_value_wins_when_override_allowed(monkeypatch):
    ...
    deployment = MagicMock()
    deployment.derouter_params.keepalive_seconds = None
    deployment.derouter_params.allow_client_keepalive_override = True
...
def test_resolve_keepalive_seconds_explicit_zero_disables_when_override_allowed(monkeypatch):
    ...
    deployment = MagicMock()
    deployment.derouter_params.keepalive_seconds = 20.0
    deployment.derouter_params.allow_client_keepalive_override = True
...
def test_resolve_keepalive_seconds_deployment_disable_cannot_be_overridden_by_request(monkeypatch):
    ...
    deployment = MagicMock()
    deployment.derouter_params.keepalive_seconds = 0
    deployment.derouter_params.allow_client_keepalive_override = True
...
def test_resolve_keepalive_seconds_global_default_applies_when_unconfigured(monkeypatch):
    """derouter_settings.sse_keepalive_ping_interval_seconds is the operator's
    ...
    monkeypatch.setattr(ps, "llm_router", None)
    monkeypatch.setattr(derouter, "sse_keepalive_ping_interval_seconds", 15.0)
...
def test_resolve_keepalive_seconds_global_default_is_clamped(monkeypatch):
    ...
    monkeypatch.setattr(ps, "llm_router", None)
    monkeypatch.setattr(derouter, "sse_keepalive_ping_interval_seconds", 900.0)
...
def test_resolve_keepalive_seconds_deployment_value_beats_global_default(monkeypatch):
    ...
    deployment = MagicMock()
    deployment.derouter_params.keepalive_seconds = 30.0
    deployment.derouter_params.allow_client_keepalive_override = False
...
def test_keepalive_from_deployment_config_reads_by_model_id(monkeypatch):
    ...
    deployment = MagicMock()
    deployment.derouter_params.keepalive_seconds = 45.0
    deployment.derouter_params.allow_client_keepalive_override = True
...
def test_keepalive_from_deployment_config_stale_model_id_does_not_fall_through(monkeypatch):
    ...
    router.get_model_list.return_value = [
        {"derouter_params": {"keepalive_seconds": 20.0}},
    ]
...
def test_keepalive_from_deployment_config_fallback_by_name_agreeing_deployments(monkeypatch):
    ...
    router.get_deployment.return_value = None
    router.get_model_list.return_value = [
        {"derouter_params": {"keepalive_seconds": 20.0, "allow_client_keepalive_override": True}},
        {"derouter_params": {"keepalive_seconds": 20.0, "allow_client_keepalive_override": True}},
    ]
...
def test_keepalive_from_deployment_config_fallback_by_name_conflicting_deployments(monkeypatch):
    ...
    router.get_model_list.return_value = [
        {"derouter_params": {"keepalive_seconds": 20.0}},
        {"derouter_params": {"keepalive_seconds": 0}},
    ]
...
def test_make_keepalive_resolver_caches_by_model_id(monkeypatch):
    ...
    deployment = MagicMock()
    deployment.derouter_params.keepalive_seconds = 5.0
    deployment.derouter_params.allow_client_keepalive_override = False
...
def test_make_keepalive_resolver_reresolves_on_model_id_change(monkeypatch):
    ...
    before = MagicMock()
    before.derouter_params.keepalive_seconds = 5.0
    before.derouter_params.allow_client_keepalive_override = False

    after = MagicMock()
    after.derouter_params.keepalive_seconds = 30.0
    after.derouter_params.allow_client_keepalive_override = False
...
def test_make_keepalive_resolver_missing_model_id_never_cached(monkeypatch):
    ...
    router = MagicMock()
    router.get_deployment.return_value = None
    router.get_model_list.return_value = [{"derouter_params": {"keepalive_seconds": 12.0}}]
    monkeypatch.setattr(ps, "llm_router", router)
...
def test_make_keepalive_resolver_expires_cache_after_ttl(monkeypatch):
    ...
    before = MagicMock()
    before.derouter_params.keepalive_seconds = 20.0
    before.derouter_params.allow_client_keepalive_override = False

    after = MagicMock()
    after.derouter_params.keepalive_seconds = 0
    after.derouter_params.allow_client_keepalive_override = False
...
def test_keepalive_seconds_in_all_derouter_params():
    from derouter.types.utils import all_derouter_params

    assert "keepalive_seconds" in all_derouter_params


def test_allow_client_keepalive_override_in_all_derouter_params():
    """allow_client_keepalive_override is a deployment-only control flag: if it's
    missing from all_derouter_params, it leaks straight through into the actual
    ...
    assert "allow_client_keepalive_override" in all_derouter_params
...
async def test_async_data_generator_emits_ping_heartbeat(monkeypatch):
    ...
    router = MagicMock()
    router.get_deployment.return_value = None
    router.get_model_list.return_value = [{"derouter_params": {"allow_client_keepalive_override": True}}]
    monkeypatch.setattr(ps, "llm_router", router)
...
async def test_async_data_generator_resolves_deployment_once_per_steady_stream(monkeypatch):
    ...
    router = MagicMock()
    router.get_deployment.return_value = deployment
    router.get_model_list.return_value = [{"derouter_params": {}}]
    monkeypatch.setattr(ps, "llm_router", router)
...
async def _run_thread_streaming(monkeypatch, interval, delay=0.3, fails_with=None):
    ...
    monkeypatch.setattr(ps, "add_derouter_data_to_request", _add_data)
