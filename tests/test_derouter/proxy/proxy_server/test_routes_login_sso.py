...
def _install_login_mocks(monkeypatch, raise_on_auth: bool = False) -> None:
    ...
    monkeypatch.setattr("derouter.proxy.auth.login_utils.authenticate_user", _fake_auth)
    monkeypatch.setattr("derouter.proxy.auth.login_utils.create_ui_token_object", _fake_token_object)
...
def test_fallback_login_shows_credentials_hint_by_default(client, monkeypatch):
    ...
    monkeypatch.delenv("UI_USERNAME", raising=False)
    monkeypatch.delenv("DEROUTER_HIDE_DEFAULT_CREDENTIALS_HINT", raising=False)
    response = client.get("/fallback/login")
...
def test_fallback_login_hides_credentials_hint_via_env_flag(client, monkeypatch):
    """Pin: DEROUTER_HIDE_DEFAULT_CREDENTIALS_HINT removes the hint on /fallback/login."""
    monkeypatch.delenv("UI_USERNAME", raising=False)
    monkeypatch.setenv("DEROUTER_HIDE_DEFAULT_CREDENTIALS_HINT", "true")
    response = client.get("/fallback/login")
    assert response.status_code == 200
...
def test_v3_login_exchange_success_returns_token_and_redirect(client, monkeypatch):
    ...
    cached_payload = {
        "token": "jwt-token-xyz",
        "redirect_url": "https://derouter.example.invalid/ui?login=success",
    }
    fake_cache = MagicMock()
...
def test_login_form_honors_same_origin_return_to_cookie(client, monkeypatch):
    """The aggregate DCR connect flow preserves a same-origin return_to in the derouter_cp_return_to
    ...
    response = client.post(
        "/login",
        data={"username": "admin", "password": "password"},
        cookies={"derouter_cp_return_to": return_to},
        follow_redirects=False,
    )
...
def test_login_form_honors_control_plane_return_to_cookie(client, monkeypatch):
    ...
    response = client.post(
        "/login",
        data={"username": "admin", "password": "password"},
        cookies={"derouter_cp_return_to": "https://cp.example.com/console"},
        follow_redirects=False,
    )
    location = response.headers.get("location", "")
    assert response.status_code == 303
...
def test_login_form_survives_stale_control_plane_return_to(client, monkeypatch):
    ...
    response = client.post(
        "/login",
        data={"username": "admin", "password": "password"},
        cookies={"derouter_cp_return_to": "https://old-cp.example.com/console"},
        follow_redirects=False,
    )
...
def test_login_form_ignores_open_redirect_return_to(client, monkeypatch):
    ...
    response = client.post(
        "/login",
        data={"username": "admin", "password": "password"},
        cookies={"derouter_cp_return_to": "https://evil.example.com/steal"},
        follow_redirects=False,
    )
    assert response.status_code == 303
    location = response.headers.get("location", "")
