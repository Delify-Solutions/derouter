...
from derouter.derouter_core_utils.get_model_cost_map import get_model_cost_map_provenance
...
def _attach_derouter_config(mock_prisma):
    """Attach a derouter_config table mock (not in conftest's _PRISMA_TABLES)."""
    table = MagicMock()
    ...
    mock_prisma.db.derouter_config = table
    return table
...
def test_reload_model_cost_map_happy(client, auth_as, monkeypatch, mock_prisma):
    ...
    table = _attach_derouter_config(mock_prisma)
    ...
    monkeypatch.setattr("derouter.add_known_models", lambda model_cost_map=None: None)
    monkeypatch.setattr("derouter.model_cost", {}, raising=False)
    monkeypatch.setattr(
        "derouter.proxy.proxy_server._invalidate_model_cost_lowercase_map",
        lambda: None,
        raising=False,
    )
...
def test_reload_model_cost_map_surfaces_the_blob_id_of_the_bytes_served_on_every_status_surface(
    ...
    import derouter
    from derouter.derouter_core_utils.get_model_cost_map import git_blob_id
    from derouter.proxy import proxy_server as ps
    from derouter.proxy._types import LitellmUserRoles

    _attach_derouter_config(mock_prisma)
    monkeypatch.setattr(ps, "prisma_client", mock_prisma)
    monkeypatch.delenv("DEROUTER_LOCAL_MODEL_COST_MAP", raising=False)
    ...
    monkeypatch.setattr(
        "derouter.derouter_core_utils.get_model_cost_map._default_reload_client",
    ...
    public_response = client.get("/public/derouter_model_cost_map")
    ...
    assert reload_body["models_count"] == len(derouter.model_cost)
...
def test_reload_model_cost_map_fetch_failure_502_keeps_map(
    ...
    monkeypatch.setattr("derouter.model_cost", sentinel_map, raising=False)
    monkeypatch.setattr(
        "derouter.derouter_core_utils.get_model_cost_map.refetch_model_cost_map",
        AsyncMock(
    ...
    assert derouter_module.model_cost is sentinel_map
...
def test_get_model_cost_map_source_happy(client, auth_as, monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.derouter_core_utils.get_model_cost_map.get_model_cost_map_source_info",
        lambda: fake_info,
    )
    monkeypatch.setattr("derouter.model_cost", {"a": 1, "b": 2, "c": 3}, raising=False)
...
def test_get_model_cost_map_source_admin_view_only_allowed(
    ...
    monkeypatch.setattr("derouter.model_cost", {"a": 1}, raising=False)
