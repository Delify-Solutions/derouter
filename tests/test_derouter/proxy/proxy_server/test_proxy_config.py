...
def test__scrub_db_overlay_remote_module_loads_strips_lists_and_strs():
    ...
    out = _scrub_db_overlay_remote_module_loads("derouter_settings", db_value)
...
def test__scrub_db_overlay_remote_module_loads_invalid_non_dict_returns_input():
    ...
    assert _scrub_db_overlay_remote_module_loads("derouter_settings", "raw") == "raw"
...
def test_validate_deployment_complexity_router_placement_leaves_valid_deployments_alone(derouter_params):
    ...
    model = {"model_name": "m", "derouter_params": dict(derouter_params)}

    validate_deployment_complexity_router_placement(model)

    assert model["derouter_params"] == derouter_params
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([{"model_name": "gpt-4o", "derouter_params": {"model": "gpt-4o"}}], 1),
...
async def test_ProxyConfig_load_config_takes_the_heuristic_v2_limit_from_the_license_only(
    ...
    monkeypatch.delenv("DEROUTER_CONFIG_BUCKET_NAME", raising=False)
    monkeypatch.setattr(
        "derouter.proxy.proxy_server._license_check.auto_router_capability_limit", lambda: license_limit
    )
...
async def test_ProxyConfig_load_config_router_refuses_a_db_heuristic_v2_router_beyond_the_license(
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.store_model_in_db", False)
    monkeypatch.delenv("DEROUTER_CONFIG_BUCKET_NAME", raising=False)
    monkeypatch.setattr("derouter.proxy.proxy_server._license_check.auto_router_capability_limit", lambda: 1)
...
def test_validate_deployment_max_agentic_loops_allows_a_deployment_without_the_key():
    model = {"model_name": "gpt-4o", "derouter_params": {"model": "gpt-4o"}}

    validate_deployment_max_agentic_loops(model)

    assert "max_agentic_loops" not in model["derouter_params"]


def test_validate_deployment_max_agentic_loops_leaves_a_valid_ceiling_alone():
    model = {"model_name": "gpt-4o", "derouter_params": {"model": "gpt-4o", "max_agentic_loops": 5}}

    validate_deployment_max_agentic_loops(model)

    assert model["derouter_params"]["max_agentic_loops"] == 5
...
def test_validate_deployment_max_agentic_loops_rejects_zero():
    ...
    with pytest.raises(ValueError, match="must be at least 1, got 0"):
        validate_deployment_max_agentic_loops(
            {"model_name": "gpt-4o", "derouter_params": {"model": "gpt-4o", "max_agentic_loops": 0}}
        )
...
def test_validate_deployment_max_agentic_loops_rejects_a_non_integer():
    ...
    with pytest.raises(TypeError, match="must be an integer"):
        validate_deployment_max_agentic_loops(
            {"model_name": "gpt-4o", "derouter_params": {"model": "gpt-4o", "max_agentic_loops": "three"}}
        )
...
def test_validate_deployment_max_agentic_loops_rejects_a_bool():
    with pytest.raises(TypeError, match="must be an integer"):
        validate_deployment_max_agentic_loops(
            {"model_name": "gpt-4o", "derouter_params": {"model": "gpt-4o", "max_agentic_loops": True}}
        )
...
def test_validate_deployment_max_agentic_loops_names_the_offending_model():
    with pytest.raises(ValueError, match="on model 'claude-sonnet-4-5'"):
        validate_deployment_max_agentic_loops(
            {"model_name": "claude-sonnet-4-5", "derouter_params": {"max_agentic_loops": -1}}
        )
...
def test_ProxyConfig__process_includes_merges_files(tmp_path):
    ...
    pc = ProxyConfig()
    cfg = {"include": ["models.yaml"], "model_list": [], "derouter_settings": {}}
...
async def test_ProxyConfig_save_config_writes_yaml_when_no_db(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.user_config_file_path", str(target))
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", None)
    monkeypatch.setattr("derouter.proxy.proxy_server.store_model_in_db", False)
    monkeypatch.setattr("derouter.proxy.proxy_server.general_settings", {})
    pc = ProxyConfig()
    cfg = {"model_list": [], "general_settings": {"a": 1}, "derouter_settings": {}}
...
async def test_ProxyConfig_save_config_invalid_path_raises(monkeypatch):
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.user_config_file_path",
...
async def test_ProxyConfig_save_config_db_omits_environment_variables_by_default(monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.master_key", "sk-test-salt-key")
    ...
    assert written["derouter_settings"] == {"success_callback": ["langfuse"]}
...
async def test_ProxyConfig_save_config_db_persists_environment_variables_when_opted_in(monkeypatch):
    ...
    pc = ProxyConfig()
    cfg = {"derouter_settings": {}, "environment_variables": {"OPENAI_API_KEY": "sk-explicit"}}
...
def _install_fake_config_repo(monkeypatch, existing_row):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.ConfigRepository", _FakeRepo)
    monkeypatch.setattr("derouter.proxy.proxy_server.invalidate_config_param", AsyncMock())
    return captured
...
async def test_ProxyConfig_save_environment_variables_merges_sets_and_deletes(monkeypatch):
    ...
    captured = _install_fake_config_repo(
        monkeypatch,
        existing_row={"EXISTING_KEY": "ciphertext-existing", "UI_LOGO_PATH": "old-logo", "DEROUTER_FAVICON_URL": "old"},
    )
    ...
    pc = ProxyConfig()
    await pc.save_environment_variables({"UI_LOGO_PATH": "new-logo", "DEROUTER_FAVICON_URL": None})
    ...
    assert "DEROUTER_FAVICON_URL" not in written
...
def test_ProxyConfig_load_team_config_no_settings_returns_empty():
    ...
    pc.config = {"derouter_settings": {"default_team_settings": [{"no_id": True}]}}
    with pytest.raises(Exception, match="team_id missing from team"):
...
def test_ProxyConfig__init_cache_sets_derouter_cache(monkeypatch):
    pc = ProxyConfig()
    monkeypatch.setattr(derouter, "cache", None, raising=False)
    pc._init_cache(cache_params={"type": "local"})
    snapshot = {
        "cache_is_set": derouter.cache is not None,
        "cache_type_name": type(derouter.cache).__name__,
        "params_used": "local",
    }
    assert snapshot == {
        "cache_is_set": True,
...
def test_ProxyConfig_switch_on_llm_response_caching_sets_flag(monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", fake_router)
    monkeypatch.setattr(derouter, "cache", fake_cache, raising=False)
...
from derouter.integrations.custom_secret_manager import CustomSecretManager

VAULT = {"DEROUTER_MASTER_KEY": "master-from-vault", "MY_PROVIDER_KEY": "provider-from-vault"}
...
general_settings:
  master_key: os.environ/DEROUTER_MASTER_KEY
...
async def test_ProxyConfig_get_config_resolves_keys_held_only_by_the_secret_manager(tmp_path, monkeypatch):
    ...
    assert {
        ...
        "api_key": cfg["model_list"][0]["derouter_params"]["api_key"],
        "hosted_keys": derouter._key_management_settings.hosted_keys,
    } == {
        "master_key": "master-from-vault",
        "api_key": "provider-from-vault",
        "hosted_keys": ["DEROUTER_MASTER_KEY", "MY_PROVIDER_KEY"],
    }
...
async def test_ProxyConfig_get_config_reuses_an_already_initialized_secret_manager(tmp_path, monkeypatch):
    ...
    first_client = derouter.secret_manager_client
    second = await ProxyConfig().get_config(config_file_path=config_file_path)

    assert {
        "client_reused": derouter.secret_manager_client is first_client,
...
async def test_ProxyConfig_get_config_without_key_management_system_leaves_secret_manager_unset(
    ...
    warn = MagicMock()
    monkeypatch.setattr("derouter.proxy.proxy_server.verbose_proxy_logger.warning", warn)
    ...
    assert {
        "master_key": cfg["general_settings"]["master_key"],
        "api_key": cfg["model_list"][0]["derouter_params"]["api_key"],
        "client": derouter.secret_manager_client,
...
async def test_ProxyConfig_get_config_does_not_warn_for_a_name_outside_hosted_keys(tmp_path, monkeypatch):
    ...
    assert {
        "api_key": cfg["model_list"][0]["derouter_params"]["api_key"],
        "client_is_up": derouter.secret_manager_client is not None,
        "warned_about": [call.args[1] for call in warn.call_args_list],
...
def test_ProxyConfig_update_config_state_and_get_config_state_roundtrip():
    pc = ProxyConfig()
    cfg = {"model_list": [], "general_settings": {"x": 1}, "derouter_settings": {}}
        ...
        "search_tools": [
            {
                ...
                "derouter_params": {"search_provider": "google"},
            }
        ]
    }
...
def test_ProxyConfig_merge_config_and_db_search_tools_returns_superset():
    config_tools = [
        {
            ...
            "derouter_params": {"search_provider": "tavily"},
        }
    ]
    db_tools = [
        {
    ...
    assert merged[1]["derouter_params"]["api_key"] == "fake-db-key"
...
def test_ProxyConfig_merge_config_and_db_search_tools_prefers_db_duplicate():
    config_tools = [
        ...
        },
        {
            ...
            "derouter_params": {"search_provider": "perplexity"},
        },
    ]
    db_tools = [
        {
    ...
    assert merged[1]["derouter_params"]["search_provider"] == "exa_ai"
    assert merged[1]["derouter_params"]["api_key"] == "fake-db-key"
...
async def test_ProxyConfig__init_search_tools_in_db_loads_merged_tools(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.proxy.search_endpoints.search_tool_registry.SearchToolRegistry.get_all_search_tools_from_db",
        mock_get_db_tools,
    )
    ...
    assert update_kwargs["search_tools"][1]["derouter_params"]["api_key"] == "fake-db-key"
...
async def test_ProxyConfig__init_search_tools_in_db_clears_router_when_last_tool_is_deleted(monkeypatch):
    ...
    pc.update_config_state({})
    fake_router = MagicMock()
    fake_router.search_tools = [{"search_tool_name": "deleted-search", "derouter_params": {}}]
...
async def test_ProxyConfig_reload_search_tools_from_db_serializes_overlapping_refreshes(monkeypatch):
    ...
    snapshots = iter(
        (
            [{"search_tool_name": "doomed-search", "derouter_params": {}}],
            [],
        )
    )
...
async def test_load_config_logs_disabled_budget_reservation_once(tmp_path, monkeypatch, caplog, setting):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", None)
    monkeypatch.setattr("derouter.proxy.proxy_server.store_model_in_db", False)
    monkeypatch.setattr("derouter.constants.budget_reservation_disabled_info_emitted", False)
    monkeypatch.delenv("DEROUTER_CONFIG_BUCKET_NAME", raising=False)
    config = ProxyConfig()

    with caplog.at_level(logging.INFO, logger="DeRouter Proxy"):
        for _ in range(3):
...
async def test_ProxyConfig_load_config_wires_general_settings_url_validation(tmp_path, monkeypatch):
    """Regression for #26599: SSRF settings in general_settings must reach derouter globals."""
    f = tmp_path / "c.yaml"
    f.write_text(
        "model_list: []\n"
        "general_settings:\n"
    ...
    original_validation = derouter.user_url_validation
    original_hosts = list(derouter.user_url_allowed_hosts)
    original_provider_hosts = list(derouter.provider_url_destination_allowed_hosts)
    try:
        await ProxyConfig().load_config(router=None, config_file_path=str(f))
        assert derouter.user_url_validation is False
        assert derouter.user_url_allowed_hosts == ["internal.corp"]
        assert derouter.provider_url_destination_allowed_hosts == ["api.example.com"]
    finally:
        derouter.user_url_validation = original_validation
        derouter.user_url_allowed_hosts = original_hosts
        derouter.provider_url_destination_allowed_hosts = original_provider_hosts
...
async def test_ProxyConfig_load_config_wires_config_reload_interval(tmp_path, monkeypatch):
    ...
    import derouter.proxy.proxy_server as proxy_server
...
async def test_ProxyConfig_load_config_forwards_callback_specific_params(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.initialize_callbacks_on_proxy",
...
async def test_ProxyConfig_load_config_blank_callback_settings_does_not_crash(tmp_path, monkeypatch):
    ...
    original_callbacks = list(derouter.callbacks) if isinstance(derouter.callbacks, list) else []
    derouter.callbacks = []
    try:
        pc = ProxyConfig()
        await pc.load_config(router=None, config_file_path=str(f))

        assert any(isinstance(c, CompressionInterceptionLogger) for c in derouter.callbacks)
    finally:
...
async def test_ProxyConfig__init_non_llm_configs_worker_registry_requires_premium(monkeypatch):
    ...
    pc = ProxyConfig()
    with pytest.raises(ValueError, match='Trying to use `worker_registry`You must be a DeRouter') as exc_info:
        await pc._init_non_llm_configs(
            config={
                "worker_registry": [
...
async def test_ProxyConfig__init_non_llm_configs_no_worker_registry_is_never_gated(monkeypatch, premium):
    monkeypatch.setattr("derouter.proxy.proxy_server.premium_user", premium)
    pc = ProxyConfig()
    await pc._init_non_llm_configs(config={}, config_file_path=None)
    assert pc.worker_registry == []
...
def test_ProxyConfig__load_alerting_settings_does_not_log_general_settings_dict(monkeypatch):
    ...
    ``DEROUTER_DISABLE_REDACT_SECRETS=true`` operators, downstream handlers
    ...
    import derouter._logging as _logging_module
...
def test_ProxyConfig__warn_on_misplaced_jwt_keys_warns_on_top_level_keys():
    ...
    result, warnings = _capture_proxy_warnings(
        {"enable_jwt_auth": True, "derouter_jwtauth": {"team_id_jwt_field": "client_id"}}
    )

    assert result == ("enable_jwt_auth", "derouter_jwtauth")
    assert len(warnings) == 1
    assert "enable_jwt_auth" in warnings[0]
    assert "derouter_jwtauth" in warnings[0]
...
def test_ProxyConfig__warn_on_misplaced_jwt_keys_silent_when_correctly_placed():
    ...
    result, warnings = _capture_proxy_warnings(
        {"general_settings": {"enable_jwt_auth": True, "derouter_jwtauth": {}}}
    )
...
def test_ProxyConfig__add_deployment_invalid_derouter_params_skips(monkeypatch):
    fake_router = MagicMock()
    fake_router.upsert_deployment = MagicMock(return_value=None)
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", fake_router)
    pc = ProxyConfig()
    bad = SimpleNamespace(derouter_params="not-a-dict", model_name="x", model_id="x")
...
def test_ProxyConfig__add_deployment_resolves_env_refs_after_db_decrypt(monkeypatch):
    ...
    behavior where only fields in ``_DB_DEROUTER_PARAM_ENV_REF_KEYS``
    resolved: the whitelist has been removed so the resolver applies to
    every string field."""
    monkeypatch.setenv("DEROUTER_DB_MODEL_API_KEY", "resolved-secret")
    monkeypatch.setenv("DEROUTER_MASTER_KEY", "master-secret")
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.decrypt_value_helper",
    ...
    db_model = SimpleNamespace(
        ...
        derouter_params={
            "model": "openai/gpt-4o-mini",
            "api_key": "os.environ/DEROUTER_DB_MODEL_API_KEY",
            "api_base": "os.environ/DEROUTER_MASTER_KEY",
        },
        blocked=False,
    )
    ...
    assert added == 1
    assert deployment.derouter_params.api_key == "resolved-secret"
    assert deployment.derouter_params.api_base == "master-secret"
...
def test_ProxyConfig__add_deployment_resolves_team_env_refs(monkeypatch):
    ...
    db_model = SimpleNamespace(
        ...
        derouter_params={
            "model": "openai/gpt-4o-mini",
            "api_key": "os.environ/DEROUTER_MASTER_KEY",
    ...
    assert added == 1
    assert deployment.derouter_params.api_key == "master-secret"
    assert deployment.derouter_params.api_base == "https://team.example"


def test_ProxyConfig__resolve_db_derouter_param_skips_non_string_values(monkeypatch):
    ...
    assert pc._resolve_db_derouter_param(key="tpm", value=100) == 100
...
def test_ProxyConfig__add_deployment_resolves_env_refs_for_aws_bedrock_auth_params(
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", fake_router)
    pc = ProxyConfig()
    derouter_params: Dict[str, Any] = {"model": "bedrock/anthropic.claude-v2"}
    for key, (env_name, _) in aws_env.items():
        derouter_params[key] = f"os.environ/{env_name}"
    db_model = SimpleNamespace(
        model_id="model-1",
    ...
    for key, (_, expected) in aws_env.items():
        assert getattr(deployment.derouter_params, key) == expected, key
...
def test_ProxyConfig__add_deployment_resolves_env_refs_on_arbitrary_field(monkeypatch):
    ...
    assert added == 1
    assert deployment.derouter_params.some_future_field == "resolved-custom-value"
...
def test_ProxyConfig__add_deployment_keeps_loading_rows_after_a_non_flag_drop_params(monkeypatch):
    ...
    assert added == 2
    assert [d.derouter_params.drop_params for d in deployments] == [None, True]


@pytest.mark.asyncio
@pytest.mark.parametrize("configured, expected", [("true", True), ("false", False)])
async def test_ProxyConfig_load_config_turns_derouter_settings_drop_params_string_into_bool(
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.store_model_in_db", False)
    monkeypatch.delenv("DEROUTER_CONFIG_BUCKET_NAME", raising=False)
    monkeypatch.setattr(derouter, "drop_params", not expected)

    await ProxyConfig().load_config(router=None, config_file_path=str(f))

    assert derouter.drop_params is expected


@pytest.mark.asyncio
async def test_ProxyConfig_load_config_resolves_a_derouter_settings_drop_params_env_ref(tmp_path, monkeypatch):
    f = tmp_path / "c.yaml"
    ...
    assert derouter.drop_params is True


@pytest.mark.asyncio
async def test_ProxyConfig_load_config_warns_and_turns_off_a_non_flag_derouter_settings_drop_params(
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.store_model_in_db", False)
    monkeypatch.delenv("DEROUTER_CONFIG_BUCKET_NAME", raising=False)
    monkeypatch.setattr(derouter, "drop_params", True)

    with caplog.at_level(logging.WARNING, logger="DeRouter Proxy"):
        await ProxyConfig().load_config(router=None, config_file_path=str(f))

    assert derouter.drop_params is False
    assert "derouter_settings.drop_params='ture' is not a flag value, treating it as off" in caplog.text
...
def test_ProxyConfig_decrypt_model_list_from_db_returns_decrypted(monkeypatch):
    ...
    m = SimpleNamespace(
        ...
        model_name="gpt-4",
        model_info={"id": "m-1"},
        derouter_params={"api_key": "sk-x", "model": "gpt-4"},
        blocked=False,
    )
    ...
    snapshot = {
        ...
        "params_model": out[0]["derouter_params"]["model"],
...
def test_ProxyConfig_decrypt_model_list_from_db_resolves_env_refs_after_db_decrypt(
    ...
    monkeypatch.setattr(
        ...
        lambda value, key, return_original_value: (
            "os.environ/DEROUTER_DB_MODEL_API_KEY"
            if key == "api_key"
            else "os.environ/DEROUTER_MASTER_KEY"
            if key == "api_base"
            else value
        ),
    )
    ...
    assert out[0]["derouter_params"]["api_key"] == "resolved-secret"
    assert out[0]["derouter_params"]["api_base"] == "master-secret"
...
def test_ProxyConfig_decrypt_model_list_from_db_resolves_team_env_refs_after_db_decrypt(
    ...
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.decrypt_value_helper",
        lambda value, key, return_original_value: "os.environ/DEROUTER_MASTER_KEY" if key == "api_key" else value,
    )
    pc = ProxyConfig()
    m = SimpleNamespace(
        model_id="model-1",
    ...
    assert out[0]["derouter_params"]["api_key"] == "master-secret"
    assert out[0]["derouter_params"]["api_base"] == "https://team.example"


def test_ProxyConfig_decrypt_model_list_from_db_invalid_params_skips():
    pc = ProxyConfig()
    bad = SimpleNamespace(model_id="m-1", model_name="x", model_info={}, derouter_params="not-a-dict")
...
async def test_ProxyConfig__update_llm_router_no_models_smoke(monkeypatch):
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", None)
    monkeypatch.setattr("derouter.proxy.proxy_server.master_key", "sk-master")
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", None)
    monkeypatch.setattr("derouter.proxy.proxy_server.general_settings", {})
    pc = ProxyConfig()
...
async def test_ProxyConfig__update_llm_router_bad_proxy_logging_raises(monkeypatch):
    ...
    monkeypatch.setattr(pc, "get_config", fake_get_config)
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", fake_router)
    monkeypatch.setattr("derouter.proxy.proxy_server.master_key", "sk-x")
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", None)
    monkeypatch.setattr("derouter.proxy.proxy_server.general_settings", {"alerting": ["email"]})
    monkeypatch.setattr("derouter.proxy.proxy_server.proxy_config", pc)
...
# ---------------------------------------------------------------------------
# ProxyConfig._add_callback_from_db_to_in_memory_derouter_callbacks
# ---------------------------------------------------------------------------


def test_ProxyConfig__add_callback_from_db_to_in_memory_derouter_callbacks_adds(
    monkeypatch,
):
    monkeypatch.setattr(derouter, "callbacks", [], raising=False)
    pc = ProxyConfig()
    pc._add_callback_from_db_to_in_memory_derouter_callbacks(
        callback="my_custom_cb",
        event_types=["success", "failure"],
        existing_callbacks=[],
    )
    snapshot = {
        "in_callbacks": "my_custom_cb" in derouter.callbacks,
        "count": len(derouter.callbacks),
        "method_called": True,
    }
    assert snapshot == {"in_callbacks": True, "count": 1, "method_called": True}


def test_ProxyConfig__add_callback_from_db_to_in_memory_derouter_callbacks_invalid_event_raises(
    monkeypatch,
):
    monkeypatch.setattr(derouter, "callbacks", [], raising=False)
    pc = ProxyConfig()
...
def test_ProxyConfig__add_callbacks_from_db_config_processes_lists(monkeypatch):
    monkeypatch.setattr(derouter, "callbacks", [], raising=False)
    monkeypatch.setattr(derouter, "success_callback", [], raising=False)
    monkeypatch.setattr(derouter, "failure_callback", [], raising=False)
    pc = ProxyConfig()
    cfg = {
        "derouter_settings": {
            "callbacks": ["cb_a"],
    ...
    snapshot = {
        "cb_added": "cb_a" in derouter.callbacks,
        "success_added": "s_a" in derouter.success_callback,
        "failure_added": "f_a" in derouter.failure_callback,
    }
    assert snapshot == {
        "cb_added": True,
        "success_added": True,
...
async def test_ProxyConfig__get_hierarchical_router_settings_key_shadows_team_entirely(monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.get_team_object", team_lookup)
...
async def test_ProxyConfig__add_router_settings_from_db_config_updates_router():
    ...
    fake_prisma.db.derouter_config.find_first = AsyncMock(
...
def _stub_add_deployment_collaborators(
    ...
    monkeypatch.setattr(proxy_server, "prisma_client", fake_prisma)
    monkeypatch.setattr(proxy_server, "proxy_config", pc)
    monkeypatch.delenv("DEROUTER_SALT_KEY", raising=False)
...
def _fake_prisma_with_encrypted_credential(credential_name: str, api_key: str) -> MagicMock:
    fake_prisma = MagicMock()
    fake_prisma.db.derouter_credentialstable.find_many = AsyncMock(
...
async def test_ProxyConfig_add_deployment_loads_db_credentials_before_reconciling_models(
    ...
    pc = ProxyConfig()
    fake_prisma = MagicMock()
    fake_prisma.db.derouter_credentialstable.find_many = AsyncMock(return_value=[])
    ...
    async def read_models_while_a_credential_lands(prisma_client: object) -> list[MagicMock]:
        fake_prisma.db.derouter_credentialstable.find_many.return_value = [
    ...
    request_kwargs = {"derouter_credential_name": "openai-cred"}
    load_credentials_from_list(request_kwargs)
    assert request_kwargs == {"derouter_credential_name": "openai-cred", "api_key": "sk-from-db"}
...
async def test_ProxyConfig_get_credentials_reads_from_writer_not_replica(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    writer_inner.derouter_credentialstable.find_many = AsyncMock(
        return_value=[_encrypted_credential_row("openai-cred", "sk-from-writer")]
    )
    reader_inner.derouter_credentialstable.find_many = AsyncMock(return_value=[])
    fake_prisma = MagicMock()
    ...
    reader_inner.derouter_credentialstable.find_many.assert_not_awaited()
...
async def test_ProxyConfig__reschedule_spend_log_cleanup_job_no_scheduler(monkeypatch):
    monkeypatch.setattr("derouter.proxy.proxy_server.scheduler", None)
    pc = ProxyConfig()
    try:
...
async def test_ProxyConfig__reschedule_spend_log_cleanup_job_invalid_cron(monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.scheduler", fake_scheduler)
    monkeypatch.setattr(
...
async def test_ProxyConfig__update_general_settings_updates_health_check_retention(monkeypatch):
    settings = {}
    monkeypatch.setattr("derouter.proxy.proxy_server.general_settings", settings)
    pc = ProxyConfig()
...
async def test_ProxyConfig__update_config_from_db_does_not_log_general_settings_secrets(
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.decrypt_value_helper", _fake_decrypt_value_helper)
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.get_config_param", _fake_get_config_param)
...
@pytest.mark.asyncio
async def test_ProxyConfig_load_config_redacts_secret_derouter_setting_keeps_plain(tmp_path, monkeypatch):
    """Regression for LIT-4152 on the ``derouter_settings`` apply loop.

    ``load_config`` logged ``setting derouter.<key>=<value>`` verbatim at DEBUG,
    ...
    api_key_secret = "sk-lit4152-derouter-settings-secret-abcdef1234567890"
    f = tmp_path / "c.yaml"
    f.write_text(
    ...
    handler.setLevel(logging.DEBUG)
    original_level = verbose_proxy_logger.level
    original_api_key = getattr(derouter, "api_key", None)
    original_num_retries = getattr(derouter, "num_retries", None)
    verbose_proxy_logger.setLevel(logging.DEBUG)
    verbose_proxy_logger.addHandler(handler)
    try:
        await ProxyConfig().load_config(router=None, config_file_path=str(f))
        ...
        verbose_proxy_logger.removeHandler(handler)
        verbose_proxy_logger.setLevel(original_level)
        derouter.api_key = original_api_key
        derouter.num_retries = original_num_retries
...
async def test_ProxyConfig__init_agents_in_db_keeps_config_defined_agents(clean_agent_registry):
    ...
    prisma_client = MagicMock()
    prisma_client.db.derouter_agentstable.find_many = AsyncMock(return_value=[_FakeAgentRow("db-id", "db-agent")])
...
def _db_guardrail_row(guardrail_id: str, guardrail_type: str) -> dict[str, object]:
    return {
        ...
        "derouter_params": {"guardrail": guardrail_type, "mode": "pre_call"},
        "guardrail_info": None,
        "team_id": None,
    }
...
async def test_ProxyConfig__init_guardrails_in_db_skips_only_the_unloadable_row(monkeypatch):
    ...
    from derouter.proxy.guardrails import guardrail_registry as registry_module
    from derouter.types.guardrails import Guardrail, GuardrailEventHooks, LitellmParams
    ...
    def _initializer(derouter_params: LitellmParams, guardrail: Guardrail) -> CustomGuardrail:
    ...
    prisma_client = MagicMock()
    prisma_client.db.derouter_guardrailstable.find_many = AsyncMock(
        return_value=[
            _db_guardrail_row("first", "lit5367_ok"),
            _db_guardrail_row("broken", "derouter_tool_permission"),
