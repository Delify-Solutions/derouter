...
def _make_prisma_with_config(
    ...
    """Build a MagicMock prisma_client with a ``db.derouter_config`` namespace.

    The conftest's ``mock_prisma`` fixture stubs ``derouter_configtable`` but
    the anthropic-beta routes use ``prisma_client.db.derouter_config`` —
...
def _stub_reload_beta_headers(monkeypatch, return_value=None):
    """Replace ``derouter.anthropic_beta_headers_manager.reload_beta_headers_config``
    ...
    import derouter.anthropic_beta_headers_manager as mgr
...
def test_reload_anthropic_beta_headers_admin_success(client, auth_as, monkeypatch):
    ...
    prisma.db.derouter_config.upsert.assert_awaited_once()
...
def test_reload_anthropic_beta_headers_preserves_existing_interval(
    ...
    call_kwargs = prisma.db.derouter_config.upsert.await_args.kwargs
...
def test_cancel_anthropic_beta_headers_reload_admin_success(
    ...
    """Admin cancel: deletes the DeRouter_Config row and returns success dict."""
    from derouter.proxy._types import LitellmUserRoles
    ...
    prisma.db.derouter_config.delete.assert_awaited_once_with(
