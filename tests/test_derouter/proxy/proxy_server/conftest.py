"""Shared fixtures for tests/test_derouter/proxy/proxy_server/.
...
# uv this is defensive — `derouter` already resolves through site-packages
...
def app():
    ...
    """
    os.environ.setdefault("DEROUTER_LOG", "ERROR")
    from derouter.proxy.proxy_server import app as _app
...
_PRISMA_TABLES: List[str] = [
    ...
    "derouter_organizationmembership",
    "derouter_proxymodeltable",
    "derouter_modeltable",
    "derouter_budgettable",
    "derouter_spendlogs",
    "derouter_invitationlink",
    "derouter_credentialstable",
    "derouter_mcpservertable",
    "derouter_objectpermissiontable",
    "derouter_configtable",
    "derouter_audit_log",
    "derouter_dailyuserspend",
    "derouter_dailyteamspend",
    "derouter_dailytagspend",
    "derouter_managed_object_table",
    "derouter_managed_vector_stores_table",
    "derouter_promptstable",
    "derouter_guardrailstable",
    "derouter_managed_files",
    "derouter_session_token_table",
    "derouter_passthrough_endpoint_table",
    "derouter_cron_job",
    "derouter_passthrough_logs",
    "derouter_health_check_table",
    "derouter_mcpusercredentials",
]
...
def mock_prisma() -> MagicMock:
    ...
    count -> 0. Override in a test with::
        ...
        mock_prisma.db.derouter_teamtable.find_unique.return_value = ...
    """
...
def mock_router() -> MagicMock:
    ...
    router.model_list = [
        {"model_name": "gpt-4", "derouter_params": {"model": "gpt-4"}},
        {
            ...
            "derouter_params": {"model": "anthropic/claude-3-5-sonnet-latest"},
        },
        {
            "model_name": "bedrock-claude",
            "derouter_params": {"model": "bedrock/anthropic.claude-3-5-sonnet"},
        },
    ]
...
def mock_callbacks_disabled(monkeypatch) -> None:
    """Wipe ``derouter.callbacks`` and friends so tests don't leak side effects."""
    import derouter
    ...
    for attr in (
        ...
        if hasattr(derouter, attr):
            monkeypatch.setattr(derouter, attr, [], raising=False)
