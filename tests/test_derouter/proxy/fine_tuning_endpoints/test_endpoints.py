"""
require_managed_files enforcement for derouter/proxy/fine_tuning_endpoints/endpoints.py

Ownership rows only exist for DeRouter managed ids. A raw provider id sent to these
...
import derouter
import derouter.proxy.fine_tuning_endpoints.endpoints as endpoints
...
from derouter.types.utils import DeRouterFineTuningJob, SpecialEnums
...
def _unified_job_id() -> str:
    unified = SpecialEnums.DEROUTER_MANAGED_GENERIC_RESPONSE_COMPLETE_STR.value.format("gpt-4o-mini-id", RAW_JOB_ID)
    return base64.urlsafe_b64encode(unified.encode()).decode().rstrip("=")


def _job() -> DeRouterFineTuningJob:
    job = DeRouterFineTuningJob(
        id=RAW_JOB_ID,
...
class Seams:
    def __init__(self, router: MagicMock, derouter_calls: dict[str, AsyncMock], logging: MagicMock):
        self.router = router
        self.derouter_calls = derouter_calls
        self.logging = logging

    def assert_no_provider_call(self) -> None:
        for name, mock in self.derouter_calls.items():
            assert mock.call_count == 0, f"derouter.{name} was called"
...
def seams():
    ...
    with ExitStack() as stack:
        ...
        for name, mock in derouter_calls.items():
            stack.enter_context(patch.object(derouter, name, mock))
        ...
        yield Seams(router=router, derouter_calls=derouter_calls, logging=logging)
...
async def test_create__raw_training_file_allowed_when_managed_files_not_required(seams):
    ...
    assert seams.derouter_calls["acreate_fine_tuning_job"].call_count == 1
