...
def patched_model_info_v1(monkeypatch):
    ...
    unhealthy_row = {
        ...
        "derouter_params": {"model": "anthropic/claude-sonnet"},
...
async def test_retrieve_model_default_serves_unhealthy_model(patched_model_list, monkeypatch):
    ...
    deployment.derouter_params.model = "anthropic/claude-sonnet"
    patched_model_list.get_deployment_by_model_group_name.return_value = deployment
    monkeypatch.setattr(derouter, "get_llm_provider", lambda model: (model, "anthropic", None, None))
...
@pytest.mark.asyncio
async def test_model_info_v1_derouter_model_id_lookup_ignores_health_filter(patched_model_info_v1, monkeypatch):
    ...
    response = await proxy_server.model_info_v1(
        user_api_key_dict=_admin_key(),
        derouter_model_id="unhealthy-id",
    )
