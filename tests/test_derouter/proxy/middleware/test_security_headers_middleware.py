...
that HSTS is opt-in via DEROUTER_ENABLE_HSTS.
"""
...
def test_hsts_absent_by_default(monkeypatch):
    monkeypatch.delenv("DEROUTER_ENABLE_HSTS", raising=False)
    resp = _make_client(_ok).get("/")
    assert "strict-transport-security" not in resp.headers


def test_hsts_present_when_enabled(monkeypatch):
    monkeypatch.setenv("DEROUTER_ENABLE_HSTS", "true")
    resp = _make_client(_ok).get("/")
...
def test_hsts_not_enabled_by_arbitrary_value(monkeypatch):
    monkeypatch.setenv("DEROUTER_ENABLE_HSTS", "1")
    resp = _make_client(_ok).get("/")
    assert "strict-transport-security" not in resp.headers
