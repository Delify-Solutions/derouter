...
from derouter.proxy.db.gateway_request_tracking import GatewayRequestAccumulator
...
def _make_app(recorder: Optional[FakeRecorder], status_code: int = 200, model_id: Optional[str] = None) -> Starlette:
    async def handler(request: Request) -> Response:
        headers = {"x-derouter-model-id": model_id} if model_id else {}
...
def test_extract_model_id_present():
    headers = [(b"content-type", b"application/json"), (b"x-derouter-model-id", b"deploy-123")]
    assert _extract_model_id(headers) == "deploy-123"


def test_extract_model_id_case_insensitive():
    assert _extract_model_id([(b"X-DeRouter-Model-Id", b"deploy-9")]) == "deploy-9"
...
def test_billable_middleware_is_registered_inside_the_in_flight_tracker():
    ...
    from derouter.proxy.proxy_server import app as proxy_app
...
def test_varying_model_ids_fold_into_a_single_persisted_key():
    ...
    x-derouter-model-id header, and a caller has some say in which deployment
