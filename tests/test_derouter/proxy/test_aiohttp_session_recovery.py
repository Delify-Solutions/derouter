...
Fixes: https://github.com/Delify-Solutions/derouter/issues/23806
"""
...
async def test_add_shared_session_attaches_open_session():
    ...
    from derouter.proxy.route_llm_request import add_shared_session_to_data

    mock_session = MagicMock()
    mock_session.closed = False

    with patch("derouter.proxy.proxy_server.shared_aiohttp_session", mock_session):
        data = {}
...
async def test_add_shared_session_recreates_closed_session():
    ...
    import derouter.proxy.route_llm_request as route_module
...
async def test_add_shared_session_no_session_available():
    ...
    with patch("derouter.proxy.proxy_server.shared_aiohttp_session", None):
        data = {}
        await add_shared_session_to_data(data)
        assert "shared_session" not in data
