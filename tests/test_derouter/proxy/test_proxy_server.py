...
import derouter
import derouter.proxy.proxy_server as proxy_server_module
from derouter.caching.caching import RedisCache
from derouter.caching.redis_cluster_cache import RedisClusterCache
from derouter.derouter_core_utils.get_model_cost_map import ModelCostMapReloaded
from derouter.caching.dual_cache import DualCache
from derouter.proxy._types import LitellmUserRoles, UserAPIKeyAuth
from derouter.proxy.auth.user_api_key_auth import user_api_key_auth
from derouter.proxy.proxy_server import app, initialize
...
def mock_patch_aembedding():
    return mock.patch(
        "derouter.proxy.proxy_server.llm_router.aembedding",
...
def client_no_auth():
    # Assuming derouter.proxy.proxy_server is an object
    from derouter.proxy.proxy_server import cleanup_router_config_variables
...
def test_cors_exposes_cache_key_header_to_browser_js():
    ...
    from derouter.constants import DEROUTER_UI_ALLOW_HEADERS

    cors_middleware = next(m for m in app.user_middleware if m.cls is CORSMiddleware)
    assert cors_middleware.kwargs["expose_headers"] is DEROUTER_UI_ALLOW_HEADERS
    assert "x-derouter-cache-key" in cors_middleware.kwargs["expose_headers"]
...
def test_login_v2_returns_redirect_url_and_sets_cookie(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.proxy.auth.login_utils.authenticate_user",
        mock_authenticate_user,
    )
    monkeypatch.setattr(
        "derouter.proxy.auth.login_utils.create_ui_token_object",
        mock_create_ui_token_object,
    )
    monkeypatch.setattr("jwt.encode", mock_jwt_encode)
    monkeypatch.setattr("derouter.proxy.proxy_server.master_key", "test-master-key")
    monkeypatch.setattr("derouter.proxy.proxy_server.general_settings", {})
    monkeypatch.setattr("derouter.proxy.proxy_server.premium_user", False)
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma_client)
    monkeypatch.setattr("derouter.proxy.utils.get_server_root_path", lambda: "")
    monkeypatch.setattr("derouter.proxy.utils.get_proxy_base_url", lambda: None)
...
def _mock_login_v2_deps(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.master_key", "test-master-key")
    monkeypatch.setattr("derouter.proxy.proxy_server.premium_user", False)
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", MagicMock())
    monkeypatch.setattr("derouter.proxy.utils.get_server_root_path", lambda: "")
    monkeypatch.setattr("derouter.proxy.utils.get_proxy_base_url", lambda: None)
...
def test_login_v2_sets_secure_cookie_behind_trusted_tls_terminating_proxy(monkeypatch):
    """THE regression: derouter only sees a plain-HTTP hop when TLS terminates at a
...
def test_login_v3_returns_code(monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.proxy_config", mock_config)
    monkeypatch.setattr("derouter.proxy.utils.get_server_root_path", lambda: "")
    monkeypatch.setattr("derouter.proxy.utils.get_proxy_base_url", lambda: None)
...
def test_login_v3_exchange_sets_secure_cookie_behind_trusted_tls_terminating_proxy(monkeypatch):
    ...
    TLS-terminating reverse proxy even though derouter only sees a plain-HTTP hop."""
    mock_prisma_client = MagicMock()
    monkeypatch.setattr(
        "derouter.proxy.auth.login_utils.authenticate_user",
        AsyncMock(return_value={"user_id": "test-user"}),
...
@pytest.mark.parametrize(
    "ui_logo_path",
    [
        "/etc/derouter/secret-config.json",
...
def test_sso_key_generate_shows_deprecation_banner(client_no_auth, monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.proxy.management_endpoints.ui_sso.show_missing_vars_in_env",
        lambda: None,
    )
    monkeypatch.setattr(
        "derouter.proxy.management_endpoints.ui_sso.SSOAuthenticationHandler.get_redirect_url_for_sso",
        lambda *args, **kwargs: "http://test/redirect",
    )
    monkeypatch.setattr(
        "derouter.proxy.management_endpoints.ui_sso.SSOAuthenticationHandler._get_cli_state",
        lambda *args, **kwargs: None,
    )
    monkeypatch.setattr(
        "derouter.proxy.management_endpoints.ui_sso.SSOAuthenticationHandler.should_use_sso_handler",
        lambda *args, **kwargs: False,
    )
    # Mock premium_user to bypass enterprise check (prevents 403 Forbidden)
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.premium_user",
        True,
    )
...
def test_restructure_ui_html_files_handles_nested_routes(tmp_path):
    ...
    """
    from derouter.proxy import proxy_server
    ...
    write_file(ui_root / "derouter-asset-prefix" / "ignore.html", "asset")
    ...
    assert (ui_root / "derouter-asset-prefix" / "ignore.html").read_text() == "asset"
...
def test_admin_ui_export_serves_nested_extensionless_routes():
    out_dir = Path(derouter.__file__).parent / "proxy" / "_experimental" / "out"
    ...
    nested_html_offenders = [
        ...
        and "derouter-asset-prefix" not in path.parts
    ]
...
def test_restructure_always_happens(monkeypatch):
    """
    Test that restructuring logic always executes regardless of DEROUTER_NON_ROOT setting.
    In development (is_non_root=False), restructuring happens directly in _experimental/out.
    In non-root Docker (is_non_root=True), restructuring happens in /var/lib/derouter/ui.
    """
    # Test Case 1: is_non_root is True - restructuring happens in /var/lib/derouter/ui
    monkeypatch.setenv("DEROUTER_NON_ROOT", "true")

    runtime_ui_path = "/var/lib/derouter/ui"
    packaged_ui_path = "/some/packaged/ui/path"

    # Simulate the logic from proxy_server.py
    is_non_root = os.getenv("DEROUTER_NON_ROOT", "").lower() == "true"
    if is_non_root:
    ...
    monkeypatch.delenv("DEROUTER_NON_ROOT", raising=False)
...
async def test_initialize_scheduled_jobs_loads_credentials_only_through_add_deployment(
    ...
    from derouter.proxy.proxy_server import ProxyStartupEvent
    from derouter.proxy.utils import ProxyLogging
    ...
    with (
        patch("derouter.proxy.proxy_server.proxy_config", mock_proxy_config),
        patch("derouter.proxy.proxy_server.store_model_in_db", False),
    ):
    ...
    try:
        with (
            patch("derouter.proxy.proxy_server.proxy_config", mock_proxy_config),
            patch("derouter.proxy.proxy_server.store_model_in_db", True),
            patch("derouter.proxy.proxy_server.AsyncIOScheduler", return_value=scheduler),
        ):
            await ProxyStartupEvent.initialize_scheduled_background_jobs(
                general_settings={},
                prisma_client=mock_prisma_client,
...
async def test_periodic_reload_job_scheduled_without_store_model_in_db(monkeypatch):
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_config.find_first = AsyncMock(return_value=None)
    mock_proxy_logging = MagicMock(spec=ProxyLogging)
    mock_proxy_logging.slack_alerting_instance = MagicMock()
    mock_proxy_logging.db_spend_update_writer = MagicMock()
    mock_proxy_config = _mock_scheduled_proxy_config()
    ...
    try:
        with (
            patch("derouter.proxy.proxy_server.proxy_config", mock_proxy_config),
            patch("derouter.proxy.proxy_server.store_model_in_db", False),
            patch("derouter.proxy.proxy_server.get_secret_bool", return_value=False),
            patch("derouter.proxy.proxy_server.AsyncIOScheduler", return_value=scheduler),
        ):
            await ProxyStartupEvent.initialize_scheduled_background_jobs(
                general_settings={},
...
async def test_initialize_scheduled_jobs_uses_configured_config_reload_interval(monkeypatch):
    ...
    with (
        patch("derouter.proxy.proxy_server.proxy_config", mock_proxy_config),
        patch("derouter.proxy.proxy_server.store_model_in_db", True),
        patch("derouter.proxy.proxy_server.get_secret_bool", return_value=True),
        patch(
            "derouter.proxy.proxy_server.proxy_config_reload_interval_seconds",
            configured_interval,
        ),
        patch("derouter.proxy.proxy_server.AsyncIOScheduler", return_value=mock_scheduler),
    ):
        await ProxyStartupEvent.initialize_scheduled_background_jobs(
            general_settings={},
            prisma_client=mock_prisma_client,
...
async def test_initialize_scheduled_jobs_rejects_non_positive_config_reload_interval(monkeypatch):
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.store_model_in_db", True),
        patch("derouter.proxy.proxy_server.get_secret_bool", return_value=True),
        patch("derouter.proxy.proxy_server.proxy_config_reload_interval_seconds", 0),
        patch("derouter.proxy.proxy_server.AsyncIOScheduler", return_value=mock_scheduler),
    ):
        await ProxyStartupEvent.initialize_scheduled_background_jobs(
            general_settings={},
...
async def test_init_mcp_servers_from_db_respects_supported_db_objects(monkeypatch):
    ...
    """
    from derouter.proxy.proxy_server import ProxyConfig
...
def test_get_config_custom_callback_api_env_vars(monkeypatch):
    ...
    """
    from derouter.proxy.proxy_server import app, proxy_config, user_api_key_auth

    # Mock config with custom_callback_api enabled and generic logger env vars present
    config_data = {
        "derouter_settings": {"success_callback": ["custom_callback_api"]},
        "general_settings": {},
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", mock_router)
...
@patch(
    "derouter.proxy.common_utils.callback_utils.CustomLogger.get_callback_env_vars",
...
def test_get_config_callbacks_fall_back_to_process_env(mock_env_vars, monkeypatch):
    ...
    config_data = {
        "derouter_settings": {"success_callback": ["langfuse"]},
        "general_settings": {},
        "environment_variables": {},
    }
    mock_router = MagicMock()
...
def test_get_config_returns_email_settings(monkeypatch):
    """
    Regression for https://github.com/Delify-Solutions/derouter/issues/19221
...
def test_get_config_returns_email_settings_set_only_in_process_env(monkeypatch):
    ...
    live at runtime because derouter/proxy/utils.py::send_email resolves every
...
def test_get_config_returns_slack_webhook(monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.proxy_logging_obj", mock_logging)
    monkeypatch.setattr(proxy_config, "get_config", AsyncMock(return_value=config_data))
...
@patch(
    "derouter.proxy.proxy_server.ProxyStartupEvent._setup_prisma_client",
...
async def test_aaaproxy_startup_master_key(mock_prisma, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.PrismaClient", MockPrisma)
    ...
    async with proxy_startup_event(app):
        from derouter.proxy.proxy_server import master_key
    ...
    monkeypatch.setenv("DEROUTER_MASTER_KEY", test_env_master_key)
...
async def test_get_all_team_models():
    ...
    from derouter.proxy._types import DeRouter_TeamTable
    from derouter.proxy.proxy_server import get_all_team_models
    ...
    mock_prisma_client = MagicMock()
    mock_db = MagicMock()
    mock_derouter_teamtable = MagicMock()

    mock_prisma_client.db = mock_db
    mock_db.derouter_teamtable = mock_derouter_teamtable

    # Make find_many async
    mock_derouter_teamtable.find_many = AsyncMock()
    ...
    mock_derouter_teamtable.find_many.return_value = [mock_team1, mock_team2]

    with patch("derouter.proxy.proxy_server.DeRouter_TeamTable") as mock_team_table_class:
        ...
        mock_derouter_teamtable.find_many.assert_called_with()
    ...
    mock_derouter_teamtable.reset_mock()
    mock_router.reset_mock()
    mock_router.get_model_list.side_effect = mock_get_model_list

    # Only return team1 for specific team query
    mock_derouter_teamtable.find_many.return_value = [mock_team1]
    ...
    with patch("derouter.proxy.proxy_server.DeRouter_TeamTable") as mock_team_table_class:
        ...
        mock_derouter_teamtable.find_many.assert_called_with(where={"team_id": {"in": ["team1"]}})
    ...
    mock_derouter_teamtable.reset_mock()
    mock_router.reset_mock()
    mock_derouter_teamtable.find_many.return_value = []
    ...
    mock_derouter_teamtable.find_many.assert_called_with(where={"team_id": {"in": []}})
...
def test_add_team_models_to_all_models():
    ...
    """
    from derouter.proxy._types import DeRouter_TeamTable
    from derouter.proxy.proxy_server import _add_team_models_to_all_models

    team_db_objects_typed = MagicMock(spec=DeRouter_TeamTable)
...
def test_add_team_models_to_all_models_resolves_config_access_group():
    ...
    team = MagicMock(spec=DeRouter_TeamTable)
    team.team_id = "team-a"
...
async def test_non_admin_all_models_returns_user_models_when_user_row_missing():
    """
    Regression test: /key/generate mints keys without a DeRouter_UserTable row, so
    ...
    """
    from derouter.proxy.proxy_server import non_admin_all_models

    user_added_model = {"model_name": "my-model", "model_info": {"id": "user-model-1"}}
    prisma_client = MagicMock()
    prisma_client.db.derouter_usertable.find_unique = AsyncMock(return_value=None)
    prisma_client.db.derouter_proxymodeltable.find_unique = AsyncMock(return_value=MagicMock(created_by="ghost-user"))
...
async def test_apply_search_filter_matches_team_public_model_name():
    ...
    """
    from derouter.proxy.proxy_server import _apply_search_filter_to_models

    byok_model = {
        "model_name": "model_name_team-abc-123_4a6b8",
        "derouter_params": {"model": "claude-sonnet-4-5"},
        "model_info": {
            "id": "byok-id-1",
...
async def test_apply_search_filter_scopes_byok_to_caller_teams():
    ...
    caller_team_byok = {
        ...
        "derouter_params": {"model": "claude-sonnet"},
        "model_info": {
            "id": "byok-mine",
            "team_id": "team-mine",
    ...
    prisma_client = MagicMock()
    prisma_client.db.derouter_proxymodeltable.count = AsyncMock(return_value=2)
    prisma_client.db.derouter_proxymodeltable.find_many = AsyncMock(return_value=[db_caller_row, db_other_row])
    caller_user_row = MagicMock()
    caller_user_row.teams = ["team-mine"]
    prisma_client.db.derouter_usertable.find_unique = AsyncMock(return_value=caller_user_row)
...
async def test_apply_search_filter_bounds_db_fetch_by_page_and_cap():
    ...
    * Sorted searches: `find_many(take=cap)` falls back to
    ...
    prisma_client = MagicMock()
    prisma_client.db.derouter_proxymodeltable.count = AsyncMock(return_value=10_000)
    prisma_client.db.derouter_proxymodeltable.find_many = AsyncMock(return_value=[])
    ...
    take = prisma_client.db.derouter_proxymodeltable.find_many.call_args.kwargs["take"]
    ...
    prisma_client.db.derouter_proxymodeltable.find_many.reset_mock()
    await _apply_search_filter_to_models(
        all_models=[],
        search="model",
        prisma_client=prisma_client,
...
async def test_apply_search_filter_honours_exact_model_name_in_db_query():
    ...
    prisma_client = MagicMock()
    prisma_client.db.derouter_proxymodeltable.count = AsyncMock(return_value=0)
    prisma_client.db.derouter_proxymodeltable.find_many = AsyncMock(return_value=[])
    proxy_config = MagicMock()
    proxy_config.decrypt_model_list_from_db = lambda rows: []
    ...
    where = prisma_client.db.derouter_proxymodeltable.count.call_args.kwargs["where"]
    assert where["model_name"] == "anthropic-sonnet-5"
    assert prisma_client.db.derouter_proxymodeltable.find_many.call_args.kwargs["where"] == where

    prisma_client.db.derouter_proxymodeltable.count.reset_mock()
    ...
    prisma_client.db.derouter_proxymodeltable.count.assert_not_called()
...
async def test_filter_models_by_team_id_excludes_viewer_direct_access():
    ...
    """
    from derouter.proxy.proxy_server import _filter_models_by_team_id
    ...
    team222_byok = {
        ...
        "derouter_params": {"model": "claude-haiku"},
        "model_info": {
            "id": "byok-team-222",
            "team_id": "team-222",
    ...
    prisma.db.derouter_teamtable.find_unique = AsyncMock(return_value=team_db)
    prisma.db.derouter_proxymodeltable.find_many = AsyncMock(return_value=[])
...
async def test_filter_models_by_team_id_rejects_non_member():
    ...
    prisma.db.derouter_usertable.find_unique = AsyncMock(return_value=user_row)
...
async def test_caller_byok_team_scope_treats_view_only_admin_as_unscoped():
    ...
    """
    from derouter.proxy._types import LitellmUserRoles, UserAPIKeyAuth
    from derouter.proxy.proxy_server import _get_caller_byok_team_scope
...
async def test_add_access_group_models_to_team_models():
    ...
    """
    from derouter.proxy._types import DeRouter_TeamTable
    from derouter.proxy.proxy_server import _add_access_group_models_to_team_models

    # Team with specific models AND access groups
    team_with_access_groups = MagicMock(spec=DeRouter_TeamTable)
    ...
    team_without_access_groups = MagicMock(spec=DeRouter_TeamTable)
    ...
    team_empty_access_groups = MagicMock(spec=DeRouter_TeamTable)
    ...
    team_all_access = MagicMock(spec=DeRouter_TeamTable)
    ...
    team_all_proxy = MagicMock(spec=DeRouter_TeamTable)
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_accessgrouptable.find_many = AsyncMock(return_value=[mock_ag_row])
    ...
    mock_prisma_client.db.derouter_accessgrouptable.find_many.assert_called_once()
    call_args = mock_prisma_client.db.derouter_accessgrouptable.find_many.call_args
...
async def test_add_access_group_models_multiple_teams_shared_group():
    ...
    team_a = MagicMock(spec=DeRouter_TeamTable)
    team_a.team_id = "team-a"
    team_a.models = ["gpt-4"]
    team_a.access_group_ids = ["shared-group"]

    team_b = MagicMock(spec=DeRouter_TeamTable)
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_accessgrouptable.find_many = AsyncMock(return_value=[mock_shared_row, mock_extra_row])
...
async def test_add_access_group_models_no_eligible_teams():
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_accessgrouptable.find_many = AsyncMock()
    ...
    # No DB call made
    mock_prisma_client.db.derouter_accessgrouptable.find_many.assert_not_called()
...
async def test_get_all_team_models_with_access_groups():
    ...
    mock_prisma_client.db = mock_db
    mock_db.derouter_teamtable = mock_derouter_teamtable
    mock_derouter_teamtable.find_many = AsyncMock(return_value=[mock_team1])
    mock_db.derouter_accessgrouptable = MagicMock()
    mock_db.derouter_accessgrouptable.find_many = AsyncMock(return_value=[mock_ag_row])
    ...
    with patch("derouter.proxy.proxy_server.DeRouter_TeamTable") as mock_tt_class:
...
async def test_delete_deployment_type_mismatch():
    ...
    with (
        patch("derouter.proxy.proxy_server.llm_router", mock_llm_router),
        patch("derouter.proxy.proxy_server.user_config_file_path", "test_config.yaml"),
    ):
...
async def test_get_config_from_file(tmp_path, monkeypatch):
    ...
    test_config = {
        "model_list": [{"model_name": "gpt-4", "derouter_params": {"model": "gpt-4"}}],
        "general_settings": {"master_key": "sk-test"},
        "router_settings": {"enable_pre_call_checks": True},
        "derouter_settings": {"drop_params": True},
    }
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.user_config_file_path", None)

    result = await proxy_config._get_config_from_file(str(config_file))
    assert result == test_config

    # Verify that user_config_file_path was set
    from derouter.proxy.proxy_server import user_config_file_path
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.user_config_file_path", str(config_file))
...
def test_normalize_datetime_for_sorting():
    ...
    """
    from derouter.proxy.proxy_server import _normalize_datetime_for_sorting
...
async def test_add_proxy_budget_to_db_only_creates_user_no_keys():
    ...
    which should prevent key creation in DeRouter_VerificationToken table.

    Also guards the row identity: the budget must land on the proxy-wide
    aggregate row "derouter-proxy-budget" (the one the spend writer increments
    ...
    # Set up required derouter settings
    derouter.budget_duration = "30d"
    derouter.max_budget = 100.0

    derouter_proxy_budget_name = "derouter-proxy-budget"
    ...
    with patch("derouter.proxy.proxy_server.generate_key_helper_fn", mock_generate_key_helper):
        # Call the function under test
        ...
        assert call_args.kwargs["user_id"] == derouter_proxy_budget_name
...
async def test_add_proxy_budget_to_db_backfills_budget_reset_at():
    ...
    budget row stays at NULL and reset_budget_for_derouter_users never matches
    ...
    mock_prisma.db.derouter_usertable.update_many = AsyncMock(return_value={"count": 1})
    ...
    with (
        patch(
            "derouter.proxy.proxy_server.generate_key_helper_fn",
            mock_generate_key_helper,
        ),
        patch("derouter.proxy.proxy_server.prisma_client", mock_prisma),
    ):
    ...
    mock_prisma.db.derouter_usertable.update_many.assert_called_once()
    backfill_call = mock_prisma.db.derouter_usertable.update_many.call_args
    assert backfill_call.kwargs["where"]["user_id"] == derouter_proxy_budget_name
    ...
    # reset_budget_for_derouter_users would still skip the row.
...
async def test_custom_ui_sso_sign_in_handler_config_loading():
    ...
    try:
        with patch(
            "derouter.proxy.proxy_server.get_instance_fn",
            ...
            from derouter.proxy.proxy_server import user_custom_ui_sso_sign_in_handler
...
@pytest.mark.asyncio
async def test_startup_initializes_string_callbacks_after_all_derouter_settings_load(tmp_path, monkeypatch):
    from derouter.integrations.s3_v2 import S3Logger
    from derouter.derouter_core_utils import derouter_logging
    from derouter.proxy.proxy_server import ProxyConfig
    from derouter.proxy.utils import ProxyLogging
    ...
    monkeypatch.setattr(derouter, "s3_callback_params", None)
    monkeypatch.setattr(derouter_logging, "_in_memory_loggers", [])

    await ProxyConfig().load_config(router=MagicMock(), config_file_path=str(config_file))
    ProxyLogging(user_api_key_cache=MagicMock())._init_derouter_callbacks(llm_router=None)

    success_loggers = [cb for cb in derouter._async_success_callback if isinstance(cb, S3Logger)]
    failure_loggers = [cb for cb in derouter._async_failure_callback if isinstance(cb, S3Logger)]
    ...
    assert "s3_v2" not in derouter.success_callback
    assert "s3_v2" not in derouter.failure_callback
...
async def test_load_config_max_budget_env_var_coerced_to_float(tmp_path, monkeypatch):
    ...
    test_config = {
        "model_list": [],
        "derouter_settings": {"max_budget": "os.environ/MAX_BUDGET"},
    }
    config_file = tmp_path / "config.yaml"
    config_file.write_text(yaml.dump(test_config))

    original_max_budget = derouter.max_budget
    try:
        proxy_config = ProxyConfig()
        ...
        assert derouter.max_budget == 10.0
        assert derouter.max_budget > 0
    finally:
        derouter.max_budget = original_max_budget
...
def test_max_ui_session_budget_default_is_one_dollar():
    ...
    assert derouter.max_ui_session_budget == 1.0
...
async def test_load_config_max_ui_session_budget_applied_and_coerced(tmp_path, monkeypatch):
    ...
    test_config = {
        "model_list": [],
        "derouter_settings": {"max_ui_session_budget": "os.environ/UI_SESSION_BUDGET"},
    }
    config_file = tmp_path / "config.yaml"
    config_file.write_text(yaml.dump(test_config))

    original_budget = derouter.max_ui_session_budget
    try:
        proxy_config = ProxyConfig()
        await proxy_config.load_config(router=MagicMock(), config_file_path=str(config_file))
        assert isinstance(derouter.max_ui_session_budget, float)
        assert derouter.max_ui_session_budget == 2.5
    finally:
        derouter.max_ui_session_budget = original_budget
...
async def test_load_config_max_ui_session_budget_none_disables_cap(tmp_path):
    ...
    test_config = {
        "model_list": [],
        "derouter_settings": {"max_ui_session_budget": None},
    }
    config_file = tmp_path / "config.yaml"
    config_file.write_text(yaml.dump(test_config))
    ...
    try:
        proxy_config = ProxyConfig()
        await proxy_config.load_config(router=MagicMock(), config_file_path=str(config_file))
        assert derouter.max_ui_session_budget is None
    finally:
        derouter.max_ui_session_budget = original_budget
...
async def test_load_config_default_internal_user_params_max_budget_scientific_notation(tmp_path):
    ...
    original_params = derouter.default_internal_user_params
    try:
        await ProxyConfig().load_config(router=MagicMock(), config_file_path=str(config_file))
        assert derouter.default_internal_user_params == {
            "user_role": "internal_user",
            "max_budget": 1000000000.0,
            "budget_duration": "30d",
            "auto_create_key": False,
        }
        assert isinstance(derouter.default_internal_user_params["max_budget"], float)
    finally:
        derouter.default_internal_user_params = original_params
...
async def test_load_config_default_internal_user_params_without_max_budget(tmp_path):
    ...
    try:
        ...
        assert derouter.default_internal_user_params == {"user_role": "internal_user"}
...
async def test_load_config_user_url_validation_handles_null_and_string_false(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(derouter, "user_url_validation", True)
    monkeypatch.setattr(derouter, "user_url_allowed_hosts", ["internal.example"])
    monkeypatch.setattr(derouter, "provider_url_destination_allowed_hosts", ["provider.example"])
    null_config_file = tmp_path / "null_config.yaml"
    null_config_file.write_text(
        yaml.dump(
            {
    ...
    await ProxyConfig().load_config(router=MagicMock(), config_file_path=str(null_config_file))
    assert derouter.user_url_validation is True
    assert derouter.user_url_allowed_hosts is None
    assert derouter.provider_url_destination_allowed_hosts is None
    ...
    assert derouter.user_url_validation is False
...
async def test_load_environment_variables_direct_and_os_environ():
    ...
    with patch("derouter.proxy.proxy_server.get_secret_str", return_value=mock_secret_value) as mock_get_secret:
...
@pytest.mark.asyncio
async def test_load_environment_variables_derouter_license_and_edge_cases():
    """
    Test _load_environment_variables method with DEROUTER_LICENSE special handling and edge cases
    """
    from unittest.mock import MagicMock, patch
    ...
    # Test Case 1: DEROUTER_LICENSE in environment_variables
    test_config_with_license = {
        "environment_variables": {
            "DEROUTER_LICENSE": "test_license_key",
    ...
    with patch("derouter.proxy.proxy_server._license_check", mock_license_check):
        with patch.dict(os.environ, {}, clear=False):
            # Call the method under test
            proxy_config._load_environment_variables(test_config_with_license)

            # Verify DEROUTER_LICENSE was set in environment
            assert os.environ["DEROUTER_LICENSE"] == "test_license_key"
    ...
    with patch("derouter.proxy.proxy_server.get_secret_str", return_value=None):
        with patch.dict(os.environ, {}, clear=False):
            # Call the method under test
...
async def test_write_config_to_file(monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.store_model_in_db", True)
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.general_settings", mock_general_settings)

    # Mock user_config_file_path
    test_config_path = "/tmp/test_config.yaml"
    monkeypatch.setattr("derouter.proxy.proxy_server.user_config_file_path", test_config_path)
...
async def test_write_config_to_file_when_store_model_in_db_false(monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.store_model_in_db", False)

    # Mock prisma_client to be None (so file path is taken)
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", None)
...
async def test_async_data_generator_midstream_error():
    ...
    from derouter.proxy.proxy_server import async_data_generator
    from derouter.proxy.utils import ProxyLogging
    ...
    with patch("derouter.proxy.proxy_server.proxy_logging_obj", mock_proxy_logging_obj):
...
async def test_chat_completion_result_no_nested_none_values():
    ...
    import derouter
    from derouter.proxy._types import UserAPIKeyAuth
    from derouter.proxy.proxy_server import chat_completion

    # Create a mock ModelResponse with nested None values
    mock_model_response = derouter.ModelResponse()
    ...
    setattr(
        mock_model_response,
        "usage",
        derouter.Usage(prompt_tokens=10, completion_tokens=5, total_tokens=15),
    )
    ...
    with (
        patch(
            "derouter.proxy.proxy_server._read_request_body",
            return_value={"model": "gpt-3.5-turbo", "messages": []},
        ),
        patch(
            "derouter.proxy.proxy_server.ProxyBaseLLMRequestProcessing",
...
def _reload_schedule_row(
    ...
    *,
    ...
    """DeRouter_Config row shape: admin-owned interval in param_value, run state in dedicated columns"""
...
class TestPriceDataReloadAPI:
    ...
    def client_with_auth(self):
        ...
        from derouter.proxy._types import LitellmUserRoles
        from derouter.proxy.proxy_server import cleanup_router_config_variables
    ...
    def test_reload_model_cost_map_admin_access(self, client_with_auth):
        ...
        # (derouter.model_cost = new_model_cost_map) does not contaminate
        # subsequent tests running in the same worker process.
        from derouter.derouter_core_utils.get_model_cost_map import ModelCostMapReloaded

        original_model_cost = derouter.model_cost.copy()
        try:
            with patch(
                "derouter.derouter_core_utils.get_model_cost_map.refetch_model_cost_map",
                new=AsyncMock(
                    return_value=ModelCostMapReloaded(model_cost_map={"gpt-3.5-turbo": {"input_cost_per_token": 0.001}})
                ),
            ):
                # Mock the database connection
                with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
                    mock_prisma.db.derouter_config.upsert = AsyncMock(
    ...
    def test_get_model_cost_map_public_access(self, client_no_auth):
        ...
        with patch("derouter.model_cost", {"gpt-3.5-turbo": {"input_cost_per_token": 0.001}}):
            response = client_no_auth.get("/public/derouter_model_cost_map")
    ...
    def test_reload_model_cost_map_error_handling(self, client_with_auth):
        ...
        with patch(
            ...
            # Mock the database connection
            with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
                mock_prisma.db.derouter_config.find_unique = AsyncMock(return_value=None)
                mock_prisma.db.derouter_config.upsert = AsyncMock(
                    return_value=_reload_schedule_row({}, reload_revision=1)
                )
    ...
    def test_schedule_model_cost_map_reload_admin_access(self, client_with_auth):
        ...
        with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
            ...
            mock_prisma.db.derouter_config.upsert = AsyncMock(return_value=_reload_schedule_row({}, reload_revision=1))
            ...
            call_args = mock_prisma.db.derouter_config.upsert.call_args
    ...
    def test_cancel_model_cost_map_reload_admin_access(self, client_with_auth):
        ...
        with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
            mock_prisma.db.derouter_config.update_many = AsyncMock(return_value=1)
            mock_prisma.db.derouter_config.delete = AsyncMock(return_value=None)
            ...
            assert "message" in data
            assert "timestamp" in data
            assert json.loads(mock_prisma.db.derouter_config.update_many.await_args.kwargs["data"]["param_value"]) == {
                "interval_hours": None
            }
            mock_prisma.db.derouter_config.delete.assert_not_called()
    ...
    def test_get_model_cost_map_reload_status_admin_access(self, client_with_auth):
        ...
        with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
            mock_prisma.db.derouter_config.find_unique = AsyncMock(
...
class TestPriceDataReloadIntegration:
        ...
        try:
            with patch(
                ...
                with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
                    ...
                    # Test get endpoint
                    response = client_with_auth.get("/public/derouter_model_cost_map")
                    assert response.status_code == 200
        finally:
            derouter.model_cost = original_model_cost
            _invalidate_model_cost_lowercase_map()
    ...
    def test_pod_data_clock_seeded_from_actual_cost_map_load(self):
        ...
        with patch(
            "derouter.derouter_core_utils.get_model_cost_map.get_model_cost_map_loaded_at",
    ...
    def test_distributed_reload_check_function(self):
        ...
        mock_prisma = MagicMock()
        mock_prisma.db.derouter_config.upsert = AsyncMock(return_value=_reload_schedule_row({}, reload_revision=1))
        mock_prisma.db.derouter_config.update_many = AsyncMock(return_value=None)
        mock_prisma.db.derouter_config.find_unique = AsyncMock(return_value=None)
        ...
        mock_prisma.db.derouter_config.update_many.assert_not_called()
        ...
        try:
            with (
                patch(
                    "derouter.derouter_core_utils.get_model_cost_map.refetch_model_cost_map", new_callable=AsyncMock
                ) as mock_get_map,
                patch("derouter.proxy.proxy_server.utc_now", return_value=frozen_now),
            ):
                ...
                assert derouter.model_cost["gpt-3.5-turbo"] == {"input_cost_per_token": 0.001}
                assert proxy_config.model_cost_map_loaded_at == frozen_now
                assert mock_prisma.db.derouter_config.update_many.call_args[1] == {
                    "data": {"last_run_at": frozen_now},
                    "where": {"param_name": "model_cost_map_reload_config"},
                }
                mock_prisma.db.derouter_config.upsert.assert_not_called()
    ...
    def test_periodic_reload_uses_pod_local_data_age(self):
        ...
        try:
            with (
                ...
                assert derouter.model_cost["gpt-4-test"] == {"input_cost_per_token": 0.5}
                assert proxy_config.model_cost_map_loaded_at == frozen_now
                assert mock_prisma.db.derouter_config.update_many.call_args[1]["data"] == {"last_run_at": frozen_now}

                mock_get_map.reset_mock()
                mock_prisma.db.derouter_config.update_many.reset_mock()
    ...
    def test_every_pod_applies_a_manual_revision_exactly_once(self):
        ...
        mock_prisma = MagicMock()
        mock_prisma.db.derouter_config.update_many = AsyncMock(return_value=None)
        mock_prisma.db.derouter_config.find_unique = AsyncMock(return_value=_reload_schedule_row({}, reload_revision=1))
        for pod in pods:
    ...
    def test_distributed_reload_stamps_last_run_without_creating_row(self):
        ...
        proxy_config = ProxyConfig()
        mock_prisma = MagicMock()
        mock_prisma.db.derouter_config.find_unique = AsyncMock(return_value=_reload_schedule_row({"interval_hours": 24}))
        mock_prisma.db.derouter_config.upsert = AsyncMock(return_value=_reload_schedule_row({}, reload_revision=1))
        mock_prisma.db.derouter_config.update_many = AsyncMock(return_value=None)
        frozen_now = datetime(2024, 1, 1, 7, 0, tzinfo=timezone.utc)
        ...
        try:
            with (
                ...
                mock_prisma.db.derouter_config.upsert.assert_not_called()
                mock_prisma.db.derouter_config.create.assert_not_called()
        finally:
            derouter.model_cost = original_model_cost
            _invalidate_model_cost_lowercase_map()
    ...
    def test_distributed_reload_leaves_request_unserved_when_status_write_fails(self):
        ...
        mock_prisma.db.derouter_config.update_many = AsyncMock(side_effect=Exception("connection reset"))
    ...
    def test_distributed_reload_keeps_current_map_when_fetch_fails(self):
        ...
        Regression: a 429/network failure used to silently replace derouter.model_cost with
        ...
        mock_prisma.db.derouter_config.update_many = AsyncMock(return_value=None)
        mock_prisma.db.derouter_config.upsert = AsyncMock(return_value=None)
        ...
        assert derouter.model_cost is original_model_cost, (
    ...
    def test_scheduled_reload_replays_runtime_registrations(self):
        ...
        original_model_cost = derouter.model_cost
        original_registry = dict(derouter_utils._runtime_registered_model_cost)
        try:
            derouter.register_model(
                model_cost={"custom/deployment-model": {"derouter_provider": "custom", "max_input_tokens": 4321}}
            )
                ...
                patch(
                    ...
                    new=AsyncMock(
                        return_value=ModelCostMapReloaded(
                            model_cost_map={"gpt-4o": {"derouter_provider": "openai", "mode": "chat"}}
                        )
                    ),
                ),
                patch("derouter.proxy.proxy_server.utc_now", return_value=frozen_now),
                patch("derouter.proxy.proxy_server.verbose_proxy_logger") as mock_logger,
            ):
                asyncio.run(proxy_config._check_and_reload_model_cost_map(mock_prisma))

            mock_logger.exception.assert_not_called()
            assert derouter.model_cost["custom/deployment-model"]["max_input_tokens"] == 4321
            assert "gpt-4o" in derouter.model_cost
            assert proxy_config.model_cost_map_applied_revision == 7
        finally:
            derouter.model_cost = original_model_cost
            derouter_utils._runtime_registered_model_cost.clear()
            derouter_utils._runtime_registered_model_cost.update(original_registry)
            _invalidate_model_cost_lowercase_map()
    ...
    def test_swap_in_model_cost_map_counts_the_fetched_catalog_only(self):
        ...
        from derouter import utils as derouter_utils
        from derouter.proxy.proxy_server import _swap_in_model_cost_map
        ...
        try:
            ...
            models_count = _swap_in_model_cost_map({"gpt-4o": {"derouter_provider": "openai", "mode": "chat"}})
    ...
    def test_manual_reload_preserves_interval_hours(self):
        ...
        try:
            with (
                ...
                mock_prisma.db.derouter_config.find_unique.assert_not_called()
                call_args = mock_prisma.db.derouter_config.upsert.call_args
    ...
    def test_anthropic_beta_headers_reload_preserves_interval_hours(self):
        ...
        """
        from derouter.proxy.proxy_server import ProxyConfig
        from derouter.proxy.utils import derouter_config_cache

        derouter_config_cache.flush_cache()
        proxy_config = ProxyConfig()
        mock_prisma = MagicMock()
        ...
        mock_prisma.db.derouter_config.find_unique = AsyncMock(return_value=mock_config)
        ...
        with patch("derouter.anthropic_beta_headers_manager.reload_beta_headers_config") as mock_reload:
            ...
            mock_prisma.db.derouter_config.upsert.assert_called()
            call_args = mock_prisma.db.derouter_config.upsert.call_args
    ...
    def test_anthropic_beta_headers_manual_reload_preserves_interval_hours(self):
        ...
        with patch("derouter.anthropic_beta_headers_manager.reload_beta_headers_config") as mock_reload:
            ...
            with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
                ...
                mock_prisma.db.derouter_config.find_unique = AsyncMock(return_value=mock_existing)
                mock_prisma.db.derouter_config.upsert = AsyncMock(
                    return_value=_reload_schedule_row({}, reload_revision=1)
                )
...
async def test_add_router_settings_from_db_config_merge_logic():
    ...
    # Mock prisma client
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_config.find_first = AsyncMock(return_value=mock_db_config)
    ...
    mock_prisma_client.db.derouter_config.find_first.assert_called_once_with(where={"param_name": "router_settings"})
...
async def test_add_router_settings_from_db_config_edge_cases():
    ...
    mock_prisma_client.db.derouter_config.find_first = AsyncMock(return_value=mock_db_config_invalid)
...
async def test_model_info_v1_oci_secrets_not_leaked():
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.llm_model_list", [mock_model_data]),
        ...
        result = await model_info_v1(user_api_key_dict=mock_user_api_key_dict, derouter_model_id=None)
        ...
        derouter_params = model_info["derouter_params"]

        # Verify that sensitive OCI fields are masked
        assert "****" in derouter_params["oci_key"], "oci_key should be masked"
        assert "****" in derouter_params["oci_fingerprint"], "oci_fingerprint should be masked"
        assert "****" in derouter_params["oci_tenancy"], "oci_tenancy should be masked"
        assert "****" in derouter_params["oci_key_file"], "oci_key_file should be masked"

        # Verify that non-sensitive fields are NOT masked
        assert derouter_params["model"] == "oci/xai.grok-4", "model field should not be masked"
        assert derouter_params["oci_region"] == "us-phoenix-1", "oci_region should not be masked"
        assert derouter_params["drop_params"] is True, "drop_params should not be masked"

        # Verify the model field specifically is not masked (this was the original issue)
        assert "****" not in derouter_params["model"], "model field should never be masked"
        assert derouter_params["model"].startswith("oci/"), "model should retain its full value"
...
def test_add_callback_from_db_to_in_memory_derouter_callbacks():
    """
    Test that _add_callback_from_db_to_in_memory_derouter_callbacks correctly adds callbacks
    ...
    with patch("derouter.proxy.proxy_server.derouter") as mock_derouter:
        # Set up mock derouter attributes
        mock_derouter._known_custom_logger_compatible_callbacks = []
        mock_derouter.logging_callback_manager = mock_callback_manager

        # Test Case 1: Add success callback
        mock_success_callbacks = []
        proxy_config._add_callback_from_db_to_in_memory_derouter_callbacks(
            callback="prometheus",
            event_types=["success"],
            existing_callbacks=mock_success_callbacks,
        )
        mock_callback_manager.add_derouter_success_callback.assert_called_once_with("prometheus")
        ...
        mock_callback_manager.add_derouter_failure_callback.assert_called_once_with("langfuse")
        mock_callback_manager.reset_mock()
        ...
        mock_callback_manager.add_derouter_callback.assert_called_once_with("s3")
        mock_callback_manager.reset_mock()
        ...
        mock_callback_manager.add_derouter_success_callback.assert_not_called()
...
def test_should_load_db_object_with_supported_db_objects():
    ...
    with patch("derouter.proxy.proxy_server.general_settings", {}):
...
async def test_tag_cache_update_called():
    ...
    from derouter.proxy.proxy_server import user_api_key_cache

    cache = DualCache()

    setattr(
        derouter.proxy.proxy_server,
        "user_api_key_cache",
        cache,
    )
    ...
    with patch.object(cache, "async_get_cache", new=AsyncMock(return_value=mock_tag_obj)) as mock_get_cache:
        with patch.object(cache, "async_set_cache_pipeline", new=AsyncMock()) as mock_set_cache:
            await derouter.proxy.proxy_server.update_cache(
                token=None,
                user_id=None,
                end_user_id=None,
                team_id=None,
...
async def test_update_cache_pipeline_honors_user_api_key_cache_ttl():
    ...
    original_cache = derouter.proxy.proxy_server.user_api_key_cache
    cache = DualCache(default_in_memory_ttl=300)
    setattr(derouter.proxy.proxy_server, "user_api_key_cache", cache)
    try:
        with patch.object(
            cache,
            "async_get_cache",
        ...
        setattr(derouter.proxy.proxy_server, "user_api_key_cache", original_cache)
...
async def test_spend_tracking_never_writes_the_auth_object_back():
    ...
    """
    from derouter.proxy.common_utils.user_api_key_cache import UserApiKeyCache
...
async def test_update_cache_global_proxy_spend_scalar_stays_shared():
    ...
    ``{derouter_proxy_admin_name}:spend`` cache entry between authoritative DB
    ...
    admin_name = derouter.proxy.proxy_server.derouter_proxy_admin_name
...
async def test_init_sso_settings_in_db():
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_ssoconfig.find_unique = AsyncMock(return_value=mock_sso_config)
    ...
    with patch.object(proxy_config, "_decrypt_and_set_db_env_variables") as mock_decrypt_and_set:
        ...
        mock_prisma_client.db.derouter_ssoconfig.find_unique.assert_awaited_once_with(where={"id": "sso_config"})
...
async def test_init_sso_settings_in_db_no_settings():
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_ssoconfig.find_unique = AsyncMock(return_value=None)
...
async def test_init_sso_settings_in_db_error_handling():
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_ssoconfig.find_unique = AsyncMock(side_effect=Exception("Database connection error"))
...
async def test_init_sso_settings_in_db_retries_on_transport_error():
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_ssoconfig.find_unique = AsyncMock(side_effect=_flaky_find_unique)
...
async def test_init_sso_settings_in_db_propagates_when_reconnect_fails():
    ...
    proxy_config = ProxyConfig()
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_ssoconfig.find_unique = AsyncMock(side_effect=prisma.errors.ClientNotConnectedError())
...
async def test_init_hashicorp_vault_config_override_retries_on_transport_error():
    ...
    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_configoverrides.find_unique = AsyncMock(side_effect=_flaky_find_unique)
    mock_prisma_client.attempt_db_reconnect = AsyncMock(return_value=True)
    mock_prisma_client._db_auth_reconnect_timeout_seconds = 2.0
    mock_prisma_client._db_auth_reconnect_lock_timeout_seconds = 0.1
...
def test_encrypt_env_variables_for_db_is_idempotent(monkeypatch):
    ...
    monkeypatch.setenv("DEROUTER_SALT_KEY", "sk-test-salt-key")
...
def test_get_prompt_spec_for_db_prompt_with_versions():
    ...
    mock_prompt_v1.model_dump.return_value = {
        ...
        "version": 1,
        "derouter_params": '{"prompt_id": "chat_prompt", "prompt_integration": "dotprompt", "model": "gpt-3.5-turbo", "messages": [{"role": "user", "content": "v1 content"}]}',
    ...
    mock_prompt_v2.model_dump.return_value = {
        ...
        "prompt_id": "chat_prompt",
        "version": 2,
        "derouter_params": '{"prompt_id": "chat_prompt", "prompt_integration": "dotprompt", "model": "gpt-4", "messages": [{"role": "user", "content": "v2 content"}]}',
        "prompt_info": '{"prompt_type": "db"}',
...
def test_root_redirect_when_docs_url_not_root_and_redirect_url_set(monkeypatch):
    ...
    from derouter.proxy.proxy_server import cleanup_router_config_variables
    from derouter.proxy.utils import _get_docs_url
...
async def test_get_image_non_root_uses_var_lib_assets_dir(monkeypatch):
    """
    Test that get_image uses /var/lib/derouter/assets when DEROUTER_NON_ROOT is true.
    """
    from unittest.mock import patch

    from derouter.proxy.proxy_server import get_image

    # Set DEROUTER_NON_ROOT to true
    monkeypatch.setenv("DEROUTER_NON_ROOT", "true")
    monkeypatch.delenv("UI_LOGO_PATH", raising=False)

    # Mock os.path operations - exists=False for assets_dir so makedirs gets called
    def exists_side_effect(path):
        return False if path == "/var/lib/derouter/assets" else True

    with (
        patch("derouter.proxy.proxy_server.os.makedirs") as mock_makedirs,
        patch("derouter.proxy.proxy_server.os.path.exists", side_effect=exists_side_effect),
        patch("derouter.proxy.proxy_server.os.access", return_value=True),
        patch("derouter.proxy.proxy_server.os.getenv") as mock_getenv,
        patch("derouter.proxy.proxy_server.FileResponse") as mock_file_response,
    ):
        # Setup mock_getenv to return empty string for UI_LOGO_PATH
        def getenv_side_effect(key, default=""):
            if key == "UI_LOGO_PATH":
                return ""
            elif key == "DEROUTER_NON_ROOT":
                return "true"
            return default
        ...
        # Verify makedirs was called with /var/lib/derouter/assets
        mock_makedirs.assert_called_once_with("/var/lib/derouter/assets", exist_ok=True)
...
async def test_get_image_non_root_fallback_to_default_logo(monkeypatch):
    ...
    in /var/lib/derouter/assets for non-root case.
    """
    from unittest.mock import patch
    ...
    # Track path.exists calls to verify it checks /var/lib/derouter/assets/logo.jpg
    exists_calls = []

    def exists_side_effect(path):
        exists_calls.append(path)
        # Return False for /var/lib/derouter/assets* so: makedirs is called, logo fallback
        # triggers, and we don't return early with cached file
        if "/var/lib/derouter/assets" in path:
            return False
        return True
    ...
    with (
        ...
        # Verify that exists was called to check /var/lib/derouter/assets/logo.jpg
        assets_logo_path = "/var/lib/derouter/assets/logo.jpg"
...
async def test_get_image_root_case_uses_current_dir(monkeypatch):
    """
    Test that get_image uses current_dir when DEROUTER_NON_ROOT is not true.
    """
    from unittest.mock import patch

    from derouter.proxy.proxy_server import get_image

    # Don't set DEROUTER_NON_ROOT (or set it to false)
    monkeypatch.delenv("DEROUTER_NON_ROOT", raising=False)
    monkeypatch.delenv("UI_LOGO_PATH", raising=False)
    ...
    with (
        patch("derouter.proxy.proxy_server.os.makedirs") as mock_makedirs,
        patch("derouter.proxy.proxy_server.os.path.exists", return_value=True),
        patch("derouter.proxy.proxy_server.os.getenv") as mock_getenv,
        patch("derouter.proxy.proxy_server.FileResponse") as mock_file_response,
    ):
        # Setup mock_getenv
        ...
        # Verify makedirs was NOT called with /var/lib/derouter/assets (should not create it for root case)
        var_lib_assets_calls = [call for call in mock_makedirs.call_args_list if "/var/lib/derouter/assets" in str(call)]
        assert len(var_lib_assets_calls) == 0, "Should not create /var/lib/derouter/assets for root case"
...
async def test_get_image_custom_local_logo_bypasses_cache(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("DEROUTER_NON_ROOT", raising=False)
    monkeypatch.delenv("DEROUTER_ASSETS_PATH", raising=False)
    ...
    with (
        patch("derouter.proxy.proxy_server.FileResponse", side_effect=fake_file_response),
    ):
        await get_image()
...
async def test_get_image_default_logo_ignores_stale_cache(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("UI_LOGO_PATH", raising=False)
    monkeypatch.delenv("DEROUTER_NON_ROOT", raising=False)
    monkeypatch.setenv("DEROUTER_ASSETS_PATH", str(tmp_path))
...
class TestInvitationEndpoints:
    ...
    def test_invitation_endpoints_proxy_admin_success(self, client_with_auth, endpoint, payload, mock_return):
        ...
        with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
            mock_prisma.db.derouter_invitationlink = MagicMock()
            if endpoint == "/invitation/new":
                mock_create = AsyncMock(return_value=mock_return)
                with patch(
                    "derouter.proxy.management_helpers.user_invitation.create_invitation_for_user",
                    mock_create,
                ):
                    response = client_with_auth.post(endpoint, json=payload)
            else:
                mock_prisma.db.derouter_invitationlink.find_unique = AsyncMock(
                    return_value={**mock_return, "created_by": "admin-user-id"}
                )
                mock_prisma.db.derouter_invitationlink.delete = AsyncMock(return_value=mock_return)
                response = client_with_auth.post(endpoint, json=payload)
    ...
    def test_invitation_endpoints_non_admin_denied(self, client_with_auth, endpoint, payload):
        ...
        with patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma:
            ...
            with patch(
                "derouter.proxy.proxy_server._user_has_admin_privileges",
                new_callable=AsyncMock,
                return_value=False,
            ):
                response = client_with_auth.post(endpoint, json=payload)
...
async def test_async_data_generator_buffers_split_google_native_sse_json_frame():
    ...
    mock_request_data = {
        ...
        "_derouter_skip_openai_stream_done": True,
        "_derouter_raw_sse_stream": True,
    }
    payload = (
...
async def test_async_data_generator_flushes_raw_sse_stream_without_trailing_delimiter():
    ...
    with (
        patch("derouter.proxy.proxy_server.proxy_logging_obj", mock_proxy_logging_obj),
...
async def test_async_data_generator_errors_when_raw_sse_frame_exceeds_buffer_limit():
    ...
    with (
        patch("derouter.proxy.proxy_server.proxy_logging_obj", mock_proxy_logging_obj),
        patch("derouter.proxy.proxy_server._MAX_RAW_SSE_BUFFER_CHARS", 8),
        patch.object(ProxyLogging, "_fire_deferred_stream_logging"),
    ):
        yielded_data = []
        async for data in async_data_generator(mock_response, mock_user_api_key_dict, mock_request_data):
...
def test_store_model_in_db_in_config_general_settings():
    ...
    """
    from derouter.proxy._types import ConfigGeneralSettings
...
async def test_update_general_settings_store_model_in_db_true():
    ...
    with (
        patch("derouter.proxy.proxy_server.store_model_in_db", False) as mock_store,
        patch("derouter.proxy.proxy_server.general_settings", {}) as mock_gs,
    ):
        await proxy_config._update_general_settings(db_general_settings={"store_model_in_db": True})

        import derouter.proxy.proxy_server as ps
...
async def test_update_general_settings_store_model_in_db_false():
    ...
    with (
        patch("derouter.proxy.proxy_server.store_model_in_db", True),
        patch("derouter.proxy.proxy_server.general_settings", {}),
    ):
...
async def test_update_general_settings_keeps_a_yaml_set_spend_log_cleanup_bound():
    ...
    with patch("derouter.proxy.proxy_server.general_settings", {"maximum_spend_logs_cleanup_run_budget": "90s"}):
        await proxy_config._update_general_settings(db_general_settings={"store_model_in_db": True})
...
async def test_update_general_settings_clearing_a_db_override_falls_back_to_the_yaml_bound():
    ...
    with patch("derouter.proxy.proxy_server.general_settings", {"maximum_spend_logs_cleanup_run_budget": "30s"}):
        await proxy_config._update_general_settings(db_general_settings={"store_model_in_db": True})
...
async def test_update_general_settings_apply_user_budget_to_team_keys_yaml_wins():
    ...
    with patch("derouter.proxy.proxy_server.general_settings", {"apply_user_budget_to_team_keys": True}):
...
async def test_batch_cost_poller_is_confirmed_before_serving(monkeypatch):
    monkeypatch.delenv("STORE_MODEL_IN_DB", raising=False)
    from derouter.proxy.openai_files_endpoints.common_utils import batch_cost_poller_is_active
    from derouter.proxy.proxy_server import ProxyStartupEvent
    from derouter.proxy.utils import ProxyLogging

    mock_prisma_client = MagicMock()
    mock_prisma_client.db.derouter_config.find_first = AsyncMock(return_value=None)
    mock_prisma_client.db.derouter_managedobjecttable.find_first = AsyncMock(return_value=None)
    mock_proxy_logging = MagicMock(spec=ProxyLogging)
    mock_proxy_logging.slack_alerting_instance = MagicMock()
    mock_proxy_logging.db_spend_update_writer = MagicMock()

    with (
        patch("derouter.proxy.proxy_server.proxy_config", _mock_scheduled_proxy_config()),
        patch("derouter.proxy.proxy_server.store_model_in_db", False),
        patch("derouter.proxy.proxy_server.llm_router", MagicMock()),
        patch("derouter.proxy.proxy_server.PROXY_BATCH_POLLING_ENABLED", True),
        patch("derouter.constants.PROXY_BATCH_POLLING_ENABLED", True),
        patch("derouter.proxy.proxy_server.get_secret_bool", return_value=False),
    ):
        await ProxyStartupEvent.initialize_scheduled_background_jobs(
            general_settings={},
        ...
        probe_where = mock_prisma_client.db.derouter_managedobjecttable.find_first.call_args[1]["where"]
...
async def test_store_model_in_db_db_override_when_config_false():
    ...
    mock_prisma_client.db.derouter_config.find_first = AsyncMock(return_value=mock_db_record)
...
async def test_store_model_in_db_db_failure_graceful(monkeypatch):
    ...
    mock_prisma_client = MagicMock()
    # Simulate DB failure
    mock_prisma_client.db.derouter_config.find_first = AsyncMock(side_effect=Exception("DB connection error"))
...
async def test_get_current_spend_reads_redis_first():
    ...
    try:
        from derouter.proxy.proxy_server import get_current_spend
...
async def test_increment_spend_counters_initializes_and_increments():
    ...
    """
    from derouter.caching.dual_cache import DualCache
    from derouter.proxy._types import DeRouter_VerificationTokenView, hash_token
    ...
    cached_key = DeRouter_VerificationTokenView(
        token=hashed_token,
        spend=5.0,
        max_budget=10.0,
    )
    ...
    try:
        from derouter.proxy.proxy_server import increment_spend_counters
...
async def test_increment_spend_counters_team_and_member():
    ...
    # Cached team object
    team_obj = DeRouter_TeamTable(team_id="team-1", spend=2.0)
...
async def test_init_and_increment_spend_counter_reseeds_from_db_on_counter_miss():
    ...
    fake_prisma.db.derouter_teamtable.find_unique = AsyncMock(return_value=db_row)
    ...
    import derouter.proxy.proxy_server as ps
    from derouter.proxy.proxy_server import _init_and_increment_spend_counter
    ...
    try:
        ...
        fake_prisma.db.derouter_teamtable.find_unique.assert_awaited_once_with(where={"team_id": "team-9"})
...
async def test_primary_spend_counter_redis_concurrent_seed_does_not_double_seed():
    ...
    """
    from derouter.caching.dual_cache import DualCache
    from derouter.proxy.db.spend_counter_reseed import SpendCounterReseed
    ...
    fake_prisma = MagicMock()
    fake_prisma.db.derouter_teamtable.find_unique = AsyncMock(side_effect=slow_find_unique)
...
async def test_reseed_spend_from_db_user_and_org_prefixes():
    ...
    fake_prisma = MagicMock()
    fake_prisma.db.derouter_usertable.find_unique = AsyncMock(return_value=user_row)
    fake_prisma.db.derouter_endusertable.find_unique = AsyncMock()
    fake_prisma.db.derouter_tagtable.find_unique = AsyncMock()
    fake_prisma.db.derouter_organizationtable.find_unique = AsyncMock(return_value=org_row)

    assert await SpendCounterReseed.from_db(fake_prisma, "spend:user:alice") == 17.0
    fake_prisma.db.derouter_usertable.find_unique.assert_awaited_once_with(where={"user_id": "alice"})
    ...
    fake_prisma.db.derouter_endusertable.find_unique.assert_not_awaited()

    assert await SpendCounterReseed.from_db(fake_prisma, "spend:tag:paid-tag") is None
    fake_prisma.db.derouter_tagtable.find_unique.assert_not_awaited()

    assert await SpendCounterReseed.from_db(fake_prisma, "spend:org:acme") == 305.0
    fake_prisma.db.derouter_organizationtable.find_unique.assert_awaited_once_with(where={"organization_id": "acme"})
...
async def test_reseed_spend_from_db_skips_window_variant_keys():
    ...
    fake_prisma = MagicMock()
    fake_prisma.db.derouter_verificationtoken.find_unique = AsyncMock()
    fake_prisma.db.derouter_teamtable.find_unique = AsyncMock()
    ...
    fake_prisma.db.derouter_verificationtoken.find_unique.assert_not_awaited()
    fake_prisma.db.derouter_teamtable.find_unique.assert_not_awaited()


@pytest.mark.asyncio
async def test_window_spend_counter_reseeds_from_spend_logs_on_counter_miss():
    from derouter.caching.dual_cache import DualCache
    from derouter.proxy.proxy_server import _init_and_increment_window_spend_counter

    counter_cache = DualCache()
    window_start = datetime.now(timezone.utc) - timedelta(hours=1)
    fake_prisma = MagicMock()
    fake_prisma.db.derouter_budgetwindowspend.find_unique = AsyncMock(return_value=None)
    fake_prisma.db.derouter_spendlogs.group_by = AsyncMock(
    ...
    try:
        ...
        fake_prisma.db.derouter_spendlogs.group_by.assert_awaited_once_with(
            by=["api_key"],
...
async def test_init_spend_counter_redis_clean_miss_skips_stale_in_memory():
    ...
    try:
        ...
        fake_prisma.db.derouter_teamtable.find_unique.assert_awaited_once_with(where={"team_id": "team-stale-local"})
...
async def test_window_spend_counter_does_not_seed_zero_when_db_unavailable():
    from derouter.caching.dual_cache import DualCache
    from derouter.proxy.proxy_server import _ensure_window_spend_counter_initialized
...
async def test_increment_spend_counters_finalizes_after_unreserved_increments():
    ...
    try:
        with patch(
            "derouter.proxy.proxy_server._init_and_increment_spend_counter",
...
async def test_increment_spend_counter_invalidates_stale_cache_on_redis_failure():
    from derouter.caching.dual_cache import DualCache
    from derouter.proxy.proxy_server import _increment_spend_counter_cache
...
async def test_get_current_spend_reseeds_from_db_when_counter_missing():
    ...
    db_row.spend = 362.0
    fake_prisma = MagicMock()
    fake_prisma.db.derouter_teammembership.find_unique = AsyncMock(return_value=db_row)
...
async def test_get_current_spend_coalesces_concurrent_reseeds():
    ...
    fake_prisma = MagicMock()
    fake_prisma.db.derouter_teammembership.find_unique = AsyncMock(side_effect=slow_find_unique)
...
async def test_reseed_locks_dict_is_bounded():
    ...
    """
    import derouter.constants as constants
    from derouter.proxy.db.spend_counter_reseed import SpendCounterReseed
    ...
    import derouter.proxy.db.spend_counter_reseed as scr
...
async def test_reseed_warms_cache_even_on_zero_db_spend():
    ...
    fake_prisma = MagicMock()
    fake_prisma.db.derouter_teammembership.find_unique = AsyncMock(side_effect=find_unique)
...
# update_config (derouter/proxy/proxy_server.py): targeted per-section writes,
# the removal of the store_model_in_db gate, env var encryption, and the
# success_callback / derouter_settings merge semantics. All other branches
...
class _FakePrismaClient:
    def __init__(self, initial_rows=None):
        ...
        self.db.derouter_config = _FakeLitellmConfig(initial_rows=initial_rows)
...
def _update_config_setup(monkeypatch):
    ...
    from derouter.proxy._types import LitellmUserRoles, UserAPIKeyAuth
    from derouter.proxy.auth.user_api_key_auth import user_api_key_auth as auth_dep

    def _install(initial_rows=None, store_model_in_db=True):
        prisma = _FakePrismaClient(initial_rows=initial_rows)
        monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", prisma)
        monkeypatch.setattr("derouter.proxy.proxy_server.store_model_in_db", store_model_in_db)
        monkeypatch.setattr(
            "derouter.proxy.proxy_server.encrypt_value_helper",
            lambda value, **_: f"enc:{value}",
        )
        monkeypatch.setattr(
            "derouter.proxy.proxy_server.invalidate_config_param",
            AsyncMock(return_value=None),
        )
        from derouter.proxy.proxy_server import proxy_config as real_proxy_config
...
def test_update_config_writes_only_sent_section(_update_config_setup):
    ...
    try:
        ...
        written = {name for name, _ in prisma.db.derouter_config.upsert_calls}
        assert written == {"general_settings"}
        assert prisma.db.derouter_config.rows["derouter_settings"] == {"drop_params": True}
        assert prisma.db.derouter_config.rows["environment_variables"] == {"FOO": "enc:bar"}
    finally:
        restore()
...
def test_update_config_env_var_round_trip_not_double_encrypted(_update_config_setup, monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.decrypt_value_helper", _fake_decrypt)
    ...
    try:
        ...
        assert resp.status_code == 200
        stored = prisma.db.derouter_config.rows["environment_variables"]
...
def test_update_config_can_flip_store_model_in_db_when_currently_false(
    ...
    try:
        ...
        assert resp.status_code == 200
        assert prisma.db.derouter_config.rows["general_settings"]["store_model_in_db"] is True
    finally:
        restore()
...
def test_update_config_derouter_settings_request_wins_for_non_callback_keys(
    _update_config_setup,
):
    ...
    client, prisma, restore = _update_config_setup(
        initial_rows={
            "derouter_settings": {"drop_params": True, "set_verbose": True},
        }
    )
    try:
        resp = client.post("/config/update", json={"derouter_settings": {"drop_params": False}})
        assert resp.status_code == 200
        stored = prisma.db.derouter_config.rows["derouter_settings"]
...
def test_update_config_success_callback_normalizes_existing_mixed_case(
    ...
    client, prisma, restore = _update_config_setup(
        initial_rows={"derouter_settings": {"success_callback": ["Langfuse", "SQS"]}}
    )
    try:
        resp = client.post(
            "/config/update",
            json={"derouter_settings": {"success_callback": ["langfuse"]}},
        )
        assert resp.status_code == 200
        stored = prisma.db.derouter_config.rows["derouter_settings"]["success_callback"]
...
class TestLazyFeatureRegistry:
    ...
    def test_registry_entries_have_required_fields(self):
        from derouter.proxy._lazy_features import LAZY_FEATURES, LazyFeature
    ...
    def test_registry_names_unique(self):
        from derouter.proxy._lazy_features import LAZY_FEATURES
    ...
    def test_matches_covers_prefix_and_suffix(self):
        ...
        from derouter.proxy._lazy_features import LazyFeature
...
class TestLazyFeaturesNotImportedAtStartup:
    ...
    def test_heavy_modules_absent_at_startup(self):
        ...
        proxy_server_src = (Path(__file__).resolve().parents[3] / "derouter/proxy/proxy_server.py").read_text()
...
class TestLazyFeatureMiddleware:
    ...
    async def test_first_request_triggers_load_subsequent_does_not(self):
        ...
        from derouter.proxy._lazy_features import (
            LazyFeature,
...
class TestInjectLazyStubs:
    ...
    def test_imported_but_unregistered_module_still_gets_stub(self):
        ...
        from derouter.proxy._lazy_features import LazyFeature, inject_lazy_stubs
    ...
    def test_snapshot_fragments_injected_for_boot_imported_features(self):
        from derouter.proxy._lazy_features import LAZY_FEATURES, inject_lazy_stubs
        from derouter.proxy._lazy_openapi_snapshot import load_snapshot
    ...
    def test_loaded_lazy_modules_reads_app_state(self):
        ...
        from derouter.proxy._lazy_features import loaded_lazy_modules

        app = FastAPI()
        assert loaded_lazy_modules(app) == frozenset()

        app.state.lazy_loaded = {"derouter.proxy.spend_tracking.cloudzero_endpoints"}
        assert loaded_lazy_modules(app) == frozenset({"derouter.proxy.spend_tracking.cloudzero_endpoints"})
...
async def test_get_current_spend_redis_error_falls_back_to_in_memory():
    ...
    fake_prisma = MagicMock()
    fake_prisma.db.derouter_teammembership.find_unique = AsyncMock(return_value=MagicMock(spend=999.0))
    ...
    try:
        ...
        fake_prisma.db.derouter_teammembership.find_unique.assert_not_awaited()
    finally:
        ps.spend_counter_cache = orig_counter
        ps.prisma_client = orig_prisma
...
def test_realtime_websocket_route_aliases_registered():
    ...
    in `DeRouterRoutes.openai_routes` (so non-admin / team / key-scoped
    ...
    from derouter.proxy._types import DeRouterRoutes
    from derouter.proxy.proxy_server import app
    from derouter.types.utils import API_ROUTE_TO_CALL_TYPES, CallTypes

    websocket_paths = {route.path for route in app.routes if isinstance(route, WebSocketRoute)}
    openai_routes = DeRouterRoutes.openai_routes.value
    ...
    for expected in ("/openai/v1/realtime", "/v1/realtime", "/realtime"):
        ...
        assert expected in openai_routes, (
            f"{expected!r} missing from DeRouterRoutes.openai_routes; "
...
async def _lit6973_drive_realtime_session(
    ...
    *,
    ...
    no counter store."""
    from derouter.derouter_core_utils.realtime_streaming import REALTIME_SESSION_SUCCESS_LOGGED_KEY
    from derouter.proxy import proxy_server as ps
    ...
    from derouter.proxy._types import ProxyException
...
async def test_release_or_invalidate_falls_back_to_invalidating_the_counters():
    ...
    from derouter.proxy import proxy_server as ps
    from derouter.proxy.spend_tracking import budget_reservation as br
...
class TestSortModelsByDisplayName:
    ...
    def test_byok_models_sort_by_team_public_model_name(self):
        from derouter.proxy.proxy_server import _sort_models
...
class TestDeleteDeploymentSync:
    ...
    async def test_delete_deployment_evicts_model_when_all_db_models_deleted(self):
        ...
        with patch("derouter.proxy.proxy_server.llm_router", mock_router):
    ...
    async def test_get_models_from_db_returns_none_on_exception(self):
        ...
        proxy_config = ProxyConfig()
        mock_prisma = MagicMock()
        mock_prisma.db.derouter_proxymodeltable.find_many = AsyncMock(side_effect=Exception("DB connection lost"))
    ...
    async def test_get_models_from_db_reads_from_writer_not_replica(self):
        ...
        from derouter.proxy.db.prisma_client import PrismaWrapper
        from derouter.proxy.db.routing_prisma_wrapper import RoutingPrismaWrapper
        from derouter.proxy.proxy_server import ProxyConfig
        ...
        writer_inner.derouter_proxymodeltable.find_many = AsyncMock(return_value=[committed_row])
        reader_inner.derouter_proxymodeltable.find_many = AsyncMock(return_value=[])
        ...
        reader_inner.derouter_proxymodeltable.find_many.assert_not_awaited()
    ...
    async def test_get_models_from_db_falls_back_to_replica_when_writer_down(self):
        ...
        writer_inner.derouter_proxymodeltable = SimpleNamespace(
            find_many=AsyncMock(side_effect=RuntimeError("writer unreachable")),
            create=MagicMock(name="writer_create"),
        )
        reader_inner.derouter_proxymodeltable = SimpleNamespace(
        ...
        writer_inner.derouter_proxymodeltable.find_many.assert_not_awaited()
...
def test_get_config_list_includes_cancel_on_disconnect(monkeypatch):
    ...
    mock_prisma.db = types.SimpleNamespace(derouter_config=mock_config_table)
...
def test_get_config_list_includes_budget_exceeded_throttle_percentage(monkeypatch):
    """The throttle fraction is a derouter_settings scalar surfaced on the General
    Settings table as a Float field so it sits with the other global limits; it
    must appear in /config/list reading its live derouter.<attr> value."""
    import types
    from unittest.mock import AsyncMock, MagicMock
    ...
    mock_prisma.db = types.SimpleNamespace(derouter_config=mock_config_table)
    monkeypatch.setattr(ps, "prisma_client", mock_prisma)
    monkeypatch.setattr(derouter, "budget_exceeded_throttle_percentage", 0.15)
    app.dependency_overrides[ps.user_api_key_auth] = lambda: UserAPIKeyAuth(
        user_id="admin", user_role=LitellmUserRoles.PROXY_ADMIN
    )
    try:
...
@pytest.mark.asyncio
async def test_update_config_field_throttle_persists_to_derouter_settings(monkeypatch):
    """Editing the throttle Float row on the General Settings table routes to
    derouter_settings (not general_settings): it sets derouter.<attr> live and
    persists under derouter_settings so the runtime read is unchanged."""
    from unittest.mock import MagicMock
    ...
    from derouter.proxy.proxy_server import update_config_general_settings

    saved: dict = {}

    async def fake_get_config():
        return {"derouter_settings": {}}
    ...
    monkeypatch.setattr(derouter, "store_audit_logs", False)
    monkeypatch.setattr(derouter, "budget_exceeded_throttle_percentage", None)
    ...
    assert derouter.budget_exceeded_throttle_percentage == 0.1
    assert saved["derouter_settings"]["budget_exceeded_throttle_percentage"] == 0.1


def test_get_config_list_includes_anthropic_prompt_caching_fields(monkeypatch):
    """The auto prompt caching flag and its ttl are derouter_settings globals surfaced on the
    ...
    mock_prisma.db = types.SimpleNamespace(derouter_config=mock_config_table)
    monkeypatch.setattr(ps, "prisma_client", mock_prisma)
    monkeypatch.setattr(derouter, "enable_anthropic_prompt_caching", True)
    monkeypatch.setattr(derouter, "anthropic_prompt_caching_ttl", "1h")
    app.dependency_overrides[ps.user_api_key_auth] = lambda: UserAPIKeyAuth(
        user_id="admin", user_role=LitellmUserRoles.PROXY_ADMIN
    )
    try:
...
def test_general_settings_ui_fields_are_db_overridable():
    """Every field the Admin UI can edit is a `derouter.<attr>` set via setattr on the handling
    worker (`_persist_general_settings_ui_derouter_field`). Unless it is also in
    DEROUTER_SETTINGS_SAFE_DB_OVERRIDES, a config reload on a peer worker merges the DB value but
    ...
    """
    from derouter.constants import DEROUTER_SETTINGS_SAFE_DB_OVERRIDES
    from derouter.proxy.proxy_server import _GENERAL_SETTINGS_UI_DEROUTER_FIELDS

    missing = set(_GENERAL_SETTINGS_UI_DEROUTER_FIELDS) - set(DEROUTER_SETTINGS_SAFE_DB_OVERRIDES)
    assert not missing, (
        f"UI-editable derouter_settings fields missing from DEROUTER_SETTINGS_SAFE_DB_OVERRIDES: {sorted(missing)}. "
...
async def test_update_config_field_max_ui_session_budget_sets_live_value(monkeypatch):
    ...
    express a dollar budget), apply live via setattr, and persist under derouter_settings."""
    from unittest.mock import MagicMock
    ...
    monkeypatch.setattr(ps, "prisma_client", MagicMock())
    monkeypatch.setattr(derouter, "store_audit_logs", False)
    monkeypatch.setattr(derouter, "max_ui_session_budget", 1.0)
    ...
    assert derouter.max_ui_session_budget == 25.0
    assert saved["derouter_settings"]["max_ui_session_budget"] == 25.0
...
def test_validate_max_ui_session_budget_rejects_malformed(bad_value):
    ...
    from derouter.proxy.proxy_server import _validate_general_settings_ui_derouter_value

    with pytest.raises(HTTPException) as exc_info:
        _validate_general_settings_ui_derouter_value("max_ui_session_budget", bad_value)
...
def test_validate_max_ui_session_budget_empty_restores_default(empty_value):
    ...
    assert _validate_general_settings_ui_derouter_value("max_ui_session_budget", empty_value) == 1.0
...
def test_general_settings_ui_defaults_unchanged_for_existing_fields():
    ...
    from derouter.proxy.proxy_server import (
        _GENERAL_SETTINGS_UI_DEROUTER_FIELDS,
        _general_settings_ui_derouter_default,
    )

    assert (
        _general_settings_ui_derouter_default(_GENERAL_SETTINGS_UI_DEROUTER_FIELDS["budget_exceeded_throttle_percentage"])
        is None
    )
    assert (
        _general_settings_ui_derouter_default(_GENERAL_SETTINGS_UI_DEROUTER_FIELDS["enable_anthropic_prompt_caching"])
        is False
    )
    assert (
        _general_settings_ui_derouter_default(_GENERAL_SETTINGS_UI_DEROUTER_FIELDS["anthropic_prompt_caching_ttl"])
        is None
    )
...
def test_prompt_caching_settings_propagate_on_config_reload(monkeypatch, field_name, db_value):
    ...
    monkeypatch.setattr(derouter, field_name, False if isinstance(db_value, bool) else None)

    pc = ps.ProxyConfig()
    pc._update_config_fields(
        current_config={"derouter_settings": {}},
        param_name="derouter_settings",
        db_param_value={field_name: db_value},
    )

    assert getattr(derouter, field_name) == db_value
...
def test_get_config_list_marks_untouched_prompt_caching_flag_as_not_set(monkeypatch):
    ...
    mock_prisma.db = types.SimpleNamespace(derouter_config=mock_config_table)
    monkeypatch.setattr(ps, "prisma_client", mock_prisma)
    monkeypatch.setattr(derouter, "enable_anthropic_prompt_caching", False)
    app.dependency_overrides[ps.user_api_key_auth] = lambda: UserAPIKeyAuth(
        user_id="admin", user_role=LitellmUserRoles.PROXY_ADMIN
    )
    try:
...
@pytest.mark.asyncio
async def test_update_config_field_prompt_caching_persists_to_derouter_settings(monkeypatch, field_name, field_value):
    """Toggling either row must set derouter.<attr> live and persist under derouter_settings,
    ...
    monkeypatch.setattr(ps, "prisma_client", MagicMock())
    monkeypatch.setattr(derouter, "store_audit_logs", False)
    monkeypatch.setattr(derouter, field_name, None)
    ...
    assert getattr(derouter, field_name) == field_value
    assert saved["derouter_settings"][field_name] == field_value
...
async def test_update_config_field_prompt_caching_rejects_invalid(monkeypatch, field_name, bad_value):
    ...
    assert getattr(derouter, field_name) is None
...
async def test_reset_config_field_restores_type_default(monkeypatch, field_name, expected_default):
    ...
    from derouter.proxy.proxy_server import delete_config_general_settings

    saved: dict = {}

    async def fake_get_config():
        return {"derouter_settings": {field_name: "stale"}}
    ...
    monkeypatch.setattr(ps, "prisma_client", MagicMock())
    monkeypatch.setattr(derouter, "store_audit_logs", False)
    monkeypatch.setattr(derouter, field_name, "stale")
    ...
    assert getattr(derouter, field_name) is expected_default
    assert field_name not in saved["derouter_settings"]
...
async def test_update_config_field_throttle_rejects_invalid(monkeypatch, bad_value):
    ...
    assert exc.value.status_code == 400
    assert derouter.budget_exceeded_throttle_percentage is None
...
def test_preserve_redacted_plugin_keys_keeps_stored_credential():
    ...
    from derouter.proxy.proxy_server import _preserve_redacted_plugin_keys
...
def _config_field_info_client(monkeypatch, user_role):
    ...
    db_record = types.SimpleNamespace(
        param_value={
            ...
            "database_url": "postgresql://user:p4ssw0rd@db:5432/derouter",
            "pass_through_endpoints": [
                {
                    "path": "/upstream",
...
def _fake_prisma_with_config(existing_param_value):
    """MagicMock prisma whose derouter_config row returns existing_param_value and
    whose derouter_auditlog.create records the written audit row."""
    fake = MagicMock()
    config_row = MagicMock()
    config_row.param_value = existing_param_value
    fake.db.derouter_config.find_first = AsyncMock(return_value=config_row)
    fake.db.derouter_config.upsert = AsyncMock(return_value=config_row)
    fake.db.derouter_auditlog.create = AsyncMock()
    return fake


def test_dump_redacted_config_redacts_secret_leaves():
    from derouter.proxy.proxy_server import _dump_redacted_config
...
async def test_create_config_audit_log_writes_redacted_entry(monkeypatch):
    import derouter.proxy.proxy_server as proxy_server_module
    from derouter.proxy._types import LitellmTableNames
    from derouter.proxy.proxy_server import create_config_audit_log
    ...
    monkeypatch.setattr(derouter, "store_audit_logs", True)
    ...
    fake.db.derouter_auditlog.create.assert_awaited_once()
    written = fake.db.derouter_auditlog.create.call_args.kwargs["data"]
...
async def test_create_config_audit_log_noop_when_store_audit_logs_disabled(monkeypatch):
    ...
    fake.db.derouter_auditlog.create.assert_not_called()
...
async def test_update_config_general_settings_emits_audit_log(monkeypatch):
    import derouter.proxy.proxy_server as proxy_server_module
    from derouter.proxy._types import ConfigFieldUpdate
    from derouter.proxy.proxy_server import update_config_general_settings
    ...
    fake.db.derouter_auditlog.create.assert_awaited_once()
    written = fake.db.derouter_auditlog.create.call_args.kwargs["data"]
    assert written["table_name"] == "DeRouter_Config"
...
async def test_update_config_field_accepts_valid_alerting_args(monkeypatch):
    ...
    written = json.loads(fake.db.derouter_config.upsert.call_args.kwargs["data"]["update"]["param_value"])
...
async def test_update_config_general_settings_applies_ssrf_globals(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "store_audit_logs", False)
    monkeypatch.setattr(derouter, "user_url_validation", True)
    monkeypatch.setattr(derouter, "user_url_allowed_hosts", [])
    monkeypatch.setattr(derouter, "provider_url_destination_allowed_hosts", [])
    ...
    assert derouter.user_url_validation is False
    assert derouter.user_url_allowed_hosts == ["internal.example"]
    assert derouter.provider_url_destination_allowed_hosts == ["provider.example"]
...
async def test_delete_config_general_settings_emits_deleted_audit_log(monkeypatch):
    import derouter.proxy.proxy_server as proxy_server_module
    from derouter.proxy._types import ConfigFieldDelete
    from derouter.proxy.proxy_server import delete_config_general_settings
...
def test_update_config_audits_every_written_section(_update_config_setup, monkeypatch):
    ...
    derouter_settings, router_settings) is mutation-protected. derouter_settings
    is the row that holds default_internal_user_params ("default user settings")."""
    import derouter.proxy.proxy_server as proxy_server_module

    client, prisma, restore = _update_config_setup(initial_rows={"derouter_settings": {"drop_params": True}})
    audit_create = AsyncMock()
    prisma.db.derouter_auditlog.create = audit_create
    monkeypatch.setattr(proxy_server_module, "premium_user", True)
    monkeypatch.setattr(derouter, "store_audit_logs", True)
    try:
        resp = client.post(
            ...
            json={
                ...
                "derouter_settings": {"default_internal_user_params": {"max_budget": 10}},
        ...
        assert audited == {
            ...
            "derouter_settings": "updated",
            "router_settings": "updated",
        }
        for call in audit_create.await_args_list:
            assert call.kwargs["data"]["table_name"] == "DeRouter_Config"
            assert call.kwargs["data"]["changed_by"] == "test_admin"

        ls_call = next(c for c in audit_create.await_args_list if c.kwargs["data"]["object_id"] == "derouter_settings")
...
def test_delete_callback_audits_derouter_settings_deletion(_update_config_setup, monkeypatch):
    """/config/callback/delete must emit a deleted audit row for derouter_settings
    ...
    monkeypatch.setattr(
        real_proxy_config,
        "get_config",
        AsyncMock(return_value={"derouter_settings": {"success_callback": ["langfuse", "datadog"]}}),
    )
    ...
    try:
        ...
        assert written["object_id"] == "derouter_settings"
        assert written["action"] == "deleted"
        before = json.loads(written["before_value"])
        after = json.loads(written["updated_values"])
...
def test_update_config_redacts_all_environment_variable_values(_update_config_setup, monkeypatch):
    ...
    secret must never reach DeRouter_AuditLog."""
    import derouter.proxy.proxy_server as proxy_server_module
    ...
    try:
        resp = client.post(
            ...
            json={
                "environment_variables": {
                    "DATABASE_URL": "postgresql://u:p@db.internal:5432/derouter",
                    "LOG_LEVEL": "debug",
                }
            },
        )
...
def _patched_coordination_redis_module_state(
    *,
    ...
    """Stub every `derouter.proxy.proxy_server` global that
    ...
    with (
        ...
        patch.object(proxy_server_module, "derouter_config_cache", config_cache),
...
def _run_init_cache_with_backend(cache_backend, redis_env_kwargs):
    ...
    mock_derouter_cache = MagicMock()
    mock_derouter_cache.cache = cache_backend
    ...
    with (
        ...
        patch(
            "derouter._redis._redis_kwargs_from_environment",
            return_value=redis_env_kwargs,
        ),
        patch("derouter.Cache", return_value=mock_derouter_cache),
    ):
        derouter.cache = None
...
async def _run_init_coordination_redis_env_fallback(
    derouter_settings, redis_env_kwargs, redis_cache_class=_EnvBuiltRedisCache
):
...
async def test_init_coordination_redis_env_fallback_builds_from_environment():
    """A deployment with no coordination_redis block and no derouter_settings.cache
...
def test_env_fallback_builds_cluster_client_from_cluster_nodes_env():
    ...
    with (
        ...
        patch.object(proxy_server_module, "RedisClusterCache", _EnvBuiltClusterCache),
        patch("derouter._redis._redis_kwargs_from_environment", return_value={}),
...
async def test_startup_ignores_database_coordination_redis_without_connection_target():
    ...
    with (
        ...
        patch.object(proxy_server_module, "derouter_config_cache", types.SimpleNamespace(redis_cache=None)),
        patch.object(proxy_server_module, "RedisCache", _EnvBuiltRedisCache),
        patch.object(
            proxy_server_module,
            "get_persisted_coordination_redis_settings",
...
def _stream_usage_test_chunks():
    from derouter.types.utils import Delta, ModelResponseStream, StreamingChoices, Usage
...
def _stream_usage_generator_chunks():
    from derouter.types.utils import ModelResponseStream
...
def test_is_injected_stream_usage_artifact():
    from derouter.proxy.proxy_server import _is_injected_stream_usage_artifact
    from derouter.types.utils import ModelResponseStream, Usage
...
async def test_async_data_generator_strips_injected_usage_chunk():
    frames = await _collect_async_data_generator_frames({"model": "gpt-5.4-nano", "_derouter_strip_stream_usage": True})
...
async def test_config_field_update_rejects_mock_testing_flag():
    ...
    from derouter.proxy._types import ConfigFieldUpdate
    from derouter.proxy.proxy_server import update_config_general_settings
    from derouter.proxy.route_llm_request import MOCK_TESTING_CONFIG_KEY
...
def test_config_update_body_drops_mock_testing_flag():
    ...
    from derouter.proxy._types import ConfigYAML
    from derouter.proxy.route_llm_request import MOCK_TESTING_CONFIG_KEY
...
def test_startup_warns_when_mock_testing_params_enabled(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="DeRouter Proxy"):
...
# ---------------------------------------------------------------------------
# Budget window spend row enqueue (DeRouter_BudgetWindowSpend writer)
# ---------------------------------------------------------------------------
...
async def test_window_spend_row_is_enqueued_even_when_the_counter_was_reserved():
    ...
    from derouter.proxy.proxy_server import increment_spend_counters
    import derouter.proxy.spend_tracking.budget_reservation as br
...
async def test_sliding_window_without_reset_at_is_not_enqueued():
    ...
    using its DeRouter_SpendLogs fallback instead."""
    from derouter.proxy.proxy_server import increment_spend_counters
...
async def test_window_spend_row_carries_the_request_start_time():
    """The seed sums DeRouter_SpendLogs only up to this point, so it must be the
...
async def _run_setup_prisma_client(mock_client):
    ...
    with patch.object(proxy_server_module, "PrismaClient", return_value=mock_client):
        result = await ProxyStartupEvent._setup_prisma_client(
            database_url="postgresql://derouter:derouter@localhost:5432/derouter",
            proxy_logging_obj=MagicMock(),
...
async def test_ptu_rollup_job_registered_at_startup(monkeypatch):
    ...
    monkeypatch.delenv("STORE_MODEL_IN_DB", raising=False)
    from derouter.proxy.spend_tracking.ptu_feature_flag import PTU_COST_ATTRIBUTION_ENV_VAR
...
async def test_ptu_rollup_job_hands_the_rollup_the_proxys_router(monkeypatch):
    ...
    monkeypatch.delenv("STORE_MODEL_IN_DB", raising=False)
    from derouter.proxy.spend_tracking import ptu_flat_cost_rollup
    from derouter.proxy.spend_tracking.ptu_feature_flag import PTU_COST_ATTRIBUTION_ENV_VAR
    from derouter.proxy.spend_tracking.ptu_flat_cost_rollup import PTU_ROLLUP_JOB_ID
...
async def test_ptu_rollup_job_not_registered_without_opt_in(monkeypatch):
    """Without DEROUTER_ENABLE_PTU_COST_ATTRIBUTION the rollup never runs, so no sentinel row
...
async def test_moderations_reraises_proxy_exception_unwrapped():
    ...
    with (
        patch.object(proxy_server_module, "add_derouter_data_to_request", new=AsyncMock(side_effect=exc)),
...
async def test_init_prompts_in_db_reloads_rows_patched_on_another_worker(monkeypatch):
    from derouter.proxy.prompts.prompt_registry import IN_MEMORY_PROMPT_REGISTRY
    from derouter.proxy.proxy_server import ProxyConfig
...
async def test_init_prompts_in_db_syncs_remaining_rows_when_one_row_fails(monkeypatch):
    ...
    try:
        ...
        assert derouter.callbacks == [healthy_callback]
    finally:
...
def _prompt_db_row(prompt_id: str, derouter_params: str) -> MagicMock:
    row = MagicMock()
    row.model_dump.return_value = {
        "prompt_id": prompt_id,
        "version": 1,
...
async def test_init_prompts_in_db_unloads_rows_deleted_on_another_worker(monkeypatch):
    ...
    try:
        ...
        prisma_client.db.derouter_prompttable.find_many = AsyncMock(return_value=[])
        await ProxyConfig()._init_prompts_in_db(prisma_client=prisma_client)

        assert IN_MEMORY_PROMPT_REGISTRY.resolve_prompt_spec("greeting_del") is None
        assert derouter.callbacks == []
    finally:
...
async def test_init_prompts_in_db_keeps_config_prompts_when_their_id_has_no_db_row(monkeypatch):
    from derouter.proxy.prompts.prompt_registry import IN_MEMORY_PROMPT_REGISTRY
    from derouter.proxy.proxy_server import ProxyConfig
    from derouter.types.prompts.init_prompts import PromptInfo, PromptDeRouterParams, PromptSpec

    monkeypatch.setattr(derouter, "callbacks", [])

    config_prompt = PromptSpec(
        prompt_id="greeting_cfg",
        derouter_params=PromptDeRouterParams(
            prompt_id="greeting_cfg",
            prompt_integration="dotprompt",
            prompt_data={"content": "Begin every reply with AHOY", "metadata": {}},
        ),
    ...
    try:
        ...
        assert len(derouter.callbacks) == 1
    finally:
...
async def test_init_prompts_in_db_keeps_the_in_memory_copy_when_a_row_fails_to_parse(monkeypatch):
    ...
    try:
        ...
        assert derouter.callbacks == [loaded_callback]
    finally:
...
async def test_init_prompts_in_db_keeps_a_prompt_created_while_the_sync_was_reading(monkeypatch):
    ...
    try:
        ...
        prisma_client.db.derouter_prompttable.find_many = AsyncMock(side_effect=create_prompt_behind_the_select)
        await ProxyConfig()._init_prompts_in_db(prisma_client=prisma_client)
        ...
        assert derouter.callbacks == [surviving_callback]
    finally:
...
class TestEmbeddingsFailureHookRequestData:
    ...
    async def test_failure_hook_gets_post_setup_data_with_logging_obj(self):
        ...
        async def fake_process(self, **kwargs):
            self.data = {**self.data, "derouter_logging_obj": logging_obj_sentinel}
        ...
        assert hook_request_data["derouter_logging_obj"] is logging_obj_sentinel
...
async def test_authoritative_floor_spend_keeps_a_reset_marker_written_during_the_db_read():
    ...
    from derouter.proxy.proxy_server import _authoritative_floor_spend
...
async def test_load_config_router_authorizes_fallback_targets_against_the_calling_key(tmp_path):
    ...
    config_file = tmp_path / "config.yaml"
    config_file.write_text(
        yaml.dump({"model_list": [{"model_name": "m", "derouter_params": {"model": "openai/m", "api_key": "k"}}]})
    )
...
def test_production_app_docs_urls_are_wired_to_the_real_env_helpers():
    ...
    """
    from derouter.proxy import utils as proxy_utils
...
async def test_update_general_settings_propagates_openai_websocket_passthrough(db_general_settings, expected):
    ...
    with patch("derouter.proxy.proxy_server.general_settings", {"enable_openai_websocket_passthrough": True}):
...
async def test_update_general_settings_keeps_yaml_openai_websocket_passthrough():
    ...
    with patch("derouter.proxy.proxy_server.general_settings", {"enable_openai_websocket_passthrough": False}):
