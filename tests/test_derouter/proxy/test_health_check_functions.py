...
def mock_prisma():
    ...
    client = MagicMock()
    client.db.derouter_healthchecktable.create = AsyncMock(
        return_value={"id": "test-id"}
    )
    client.db.derouter_healthchecktable.find_many = AsyncMock(
...
async def test_save_health_check_result(
    ...
    if not should_succeed:
        mock_prisma.db.derouter_healthchecktable.create.side_effect = Exception(
            "DB Error"
        )
    ...
    if should_succeed:
        mock_prisma.db.derouter_healthchecktable.create.assert_called_once()
    else:
...
async def test_get_health_check_history(mock_prisma):
    ...
    mock_prisma.db.derouter_healthchecktable.find_many.assert_called_once()
    assert len(result) == 1
...
def test_build_model_param_to_info_mapping():
    ...
    model_list = [
        ...
        },
        {
            ...
            "derouter_params": {"model": "gpt-3.5-turbo"},  # Same model param
        },
    ]
...
async def test_get_all_latest_health_checks_with_model_id(mock_prisma):
    ...
    mock_prisma.db.derouter_healthchecktable.find_many = AsyncMock(
...
async def test_perform_health_check_and_save_passes_model_id_to_perform_health_check():
    ...
    with patch(
        "derouter.proxy.health_endpoints._health_endpoints.perform_health_check",
...
def test_parse_background_health_check_model_groups_unset_returns_none():
    from derouter.proxy.health_check import parse_background_health_check_model_groups
...
def test_filter_deployments_to_model_groups():
    from derouter.proxy.health_check import filter_deployments_to_model_groups
