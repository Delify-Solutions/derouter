...
class TestSwaggerChatCompletions:
    ...
    def test_openapi_schema_includes_chat_completions_request_body(self, client):
        ...
        # Check for DeRouter-specific fields added by ProxyChatCompletionRequest
        expected_derouter_fields = [
            "guardrails",
            "caching",
            "num_retries",
        ...
        for field in expected_derouter_fields:
            assert (
                field in properties
            ), f"Expected DeRouter field '{field}' not found in ProxyChatCompletionRequest schema"
    ...
    def test_chat_completions_endpoints_have_expanded_request_body(self, client):
        ...
        # Should have DeRouter-specific fields
        derouter_fields = ["guardrails", "caching", "fallbacks", "num_retries"]
        for field in derouter_fields:
            assert (
                field in properties
            ), f"DeRouter field '{field}' should be in expanded properties"
    ...
    @patch(
        "derouter.proxy.common_utils.custom_openapi_spec.CustomOpenAPISpec.add_chat_completion_request_schema"
    )
    ...
    def test_proxy_chat_completion_request_pydantic_model_works(self):
        ...
        and includes the expected DeRouter-specific fields.
        """
        from derouter.proxy._types import ProxyChatCompletionRequest
        ...
        # Check for DeRouter-specific fields
        derouter_fields = [
            "guardrails",
            "caching",
            "num_retries",
        ...
        for field in derouter_fields:
            assert (
                field in properties
            ), f"DeRouter field '{field}' should be in ProxyChatCompletionRequest schema"
    ...
    def test_openapi_schema_servers_url_with_root_path(self):
        ...
        from derouter.proxy.proxy_server import app, custom_openapi, get_openapi_schema

        # Test cases: (server_root_path, expected_servers_url)
        # Note: empty string is falsy in Python, so servers won't be set
        test_cases = [
            ("/derouter", "/derouter"),
            ("/derouter/", "/derouter"),  # trailing slash should be removed
            ("derouter", "/derouter"),  # missing leading slash should be added
            ("/api/v1", "/api/v1"),
        ]
        ...
        for root_path, expected_url in test_cases:
            ...
            with patch("derouter.proxy.proxy_server.server_root_path", root_path):
    ...
    def test_should_make_duplicate_operation_ids_unique_by_method(self):
        from derouter.proxy.proxy_server import ensure_unique_openapi_operation_ids
    ...
    def test_should_not_add_method_suffix_to_multi_method_route_base_id(self):
        ...
        from derouter.proxy.proxy_server import _generate_stable_operation_id
