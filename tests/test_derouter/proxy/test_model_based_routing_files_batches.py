...
class TestPrepareDataWithCredentials:
    def test_preserves_trusted_internal_credentials_snapshot(self):
        ...
        assert isinstance(
            data["_derouter_internal_model_credentials"], type(MappingProxyType({}))
        )
        assert (
            data["_derouter_internal_model_credentials"]["s3_bucket_name"]
            == "safe-bucket"
        )
...
class TestRoundTrip:
    ...
    def test_roundtrip_vertex_gcs_uri_file_id(self):
        ...
        original = "gs://vertex-bucket/derouter-files/output.jsonl"
        model = "gemini-2.5-pro"
        encoded = encode_file_id_with_model(original, model)
