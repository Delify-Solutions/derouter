...
Tests verify that guardrails configured via derouter_params.guardrails on a
...
class TestCheckAndMergeModelLevelGuardrails:
    ...
    def test_merge_adds_model_guardrails_to_metadata(self):
        ...
        mock_deployment.derouter_params.get.return_value = ["openai-moderation"]
    ...
    def test_merge_combines_with_existing_guardrails(self):
        ...
        mock_router = MagicMock()
        mock_deployment = MagicMock()
        mock_deployment.derouter_params.get.return_value = ["model-guardrail"]
        mock_router.get_deployment.return_value = mock_deployment
    ...
    def test_returns_data_unchanged_when_deployment_has_no_guardrails(self):
        ...
        mock_router = MagicMock()
        mock_deployment = MagicMock()
        mock_deployment.derouter_params.get.return_value = None
        mock_router.get_deployment.return_value = mock_deployment
    ...
    def test_returns_new_data_dict(self):
        ...
        mock_router = MagicMock()
        mock_deployment = MagicMock()
        mock_deployment.derouter_params.get.return_value = ["new-guardrail"]
        mock_router.get_deployment.return_value = mock_deployment
...
async def test_pre_call_hook_runs_once_with_model_level_guardrails():
    """
    A guardrail attached at the model level (derouter_params.guardrails) is
    ...
    with patch("derouter.callbacks", [guardrail]):
        ...
        # the top-level kwargs, then derouter.acompletion fires the deployment hook
...
async def test_deployment_hook_runs_pre_call_without_proxy_loop():
    """
    Direct-SDK usage (derouter.acompletion(..., guardrails=[...]) without the
...
async def test_post_call_success_hook_runs_model_level_guardrail():
    ...
    mock_router = MagicMock()
    mock_deployment = MagicMock()
    mock_deployment.derouter_params.get.return_value = ["test-model-guardrail"]
    mock_router.get_deployment.return_value = mock_deployment
...
async def test_post_call_success_hook_skips_guardrail_not_on_model():
    ...
    mock_router = MagicMock()
    mock_deployment = MagicMock()
    mock_deployment.derouter_params.get.return_value = ["some-other-guardrail"]
    mock_router.get_deployment.return_value = mock_deployment
...
async def test_pre_call_merges_model_level_guardrails_before_pre_call_hook():
    ...
    # model_info has been stripped by add_derouter_data_to_request (see
    ...
    mock_router = MagicMock()
    mock_router.get_deployment.return_value = None
    mock_router.get_model_list.return_value = [{"derouter_params": {"guardrails": ["my-pre-call-guardrail"]}}]
    ...
    async def passthrough_add_derouter_data(*, data, **kwargs):
        return data
    ...
    with (
        patch(
            ...
            side_effect=passthrough_add_derouter_data,
        ),
        patch(
            "derouter.proxy.common_request_processing.derouter.utils.function_setup",
