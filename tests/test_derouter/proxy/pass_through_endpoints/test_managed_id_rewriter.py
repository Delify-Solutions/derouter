...
from derouter.proxy.pass_through_endpoints.managed_id_codec import decode, new_managed_id
...
async def test_list_batches_out_of_range_limit_raises_400(limit, expected_message, expected_openai_code):
    ...
    pc.db.derouter_managedobjecttable.find_many.assert_not_called()
...
async def test_list_files_drops_batch_guardrail_key_persisted_by_an_older_proxy():
    ...
    row.file_object = {**row.file_object, "derouter_batch_guardrail": None}
    ...
    assert result is not None
    assert "derouter_batch_guardrail" not in result["data"][0]
...
async def test_streamed_response_is_owned_and_rewritten_across_chunk_boundaries(chunk_size: int):
    ...
    created = pc.db.derouter_managedobjecttable.upsert.await_args.kwargs["data"]["create"]
...
async def test_streamed_response_with_cr_only_frame_delimiters_is_still_owned_and_rewritten():
    ...
    pc.db.derouter_managedobjecttable.upsert.assert_awaited_once()
    managed_id = pc.db.derouter_managedobjecttable.upsert.await_args.kwargs["data"]["create"]["unified_object_id"]
    assert RAW_RESPONSE_ID.encode() not in output
...
async def test_streamed_response_stays_raw_and_intact_when_the_row_cannot_be_persisted():
    pc = _prisma_client()
    pc.db.derouter_managedobjecttable.upsert = AsyncMock(side_effect=RuntimeError("db down"))
    payload = _response_stream_bytes()
