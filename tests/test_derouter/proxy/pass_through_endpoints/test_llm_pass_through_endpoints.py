...
from derouter.proxy._types import LitellmUserRoles, SpecialHeaders, UserAPIKeyAuth
from derouter.proxy.auth.handle_jwt import JWTHandler
from derouter.types.passthrough_endpoints.vertex_ai import VertexPassThroughCredentials
...
class TestBaseOpenAIPassThroughHandler:
    def test_join_url_paths(self):
        ...
        result = _join_url_paths(
            base_url, path, derouter.LlmProviders.OPENAI.value
        )
    ...
    @patch(  # test-quality-ok: patching derouter internal for unit test isolation
        "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.create_pass_through_route"
    )
    async def test_base_openai_pass_through_handler(self, mock_create_pass_through):
        ...
        result = await BaseOpenAIPassThroughHandler._base_openai_pass_through_handler(
            ...
            api_key="test_api_key",
            custom_llm_provider=derouter.LlmProviders.OPENAI.value,
        )
...
class TestVertexAIPassThroughHandler:
    ...
    async def test_vertex_passthrough_with_credentials(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.passthrough_endpoint_router",
            pass_through_router,
        )
        ...
        with (
            mock.patch(
                "derouter.llms.vertex_ai.vertex_llm_base.VertexBase.load_auth"
            ) as mock_load_auth,
            mock.patch(
                "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.create_pass_through_route"
            ) as mock_create_route,
            mock.patch(
                "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.get_derouter_virtual_key"
            ) as mock_get_virtual_key,
            mock.patch(
                "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.user_api_key_auth"
            ) as mock_user_auth,
            mock.patch(
                "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.get_vertex_pass_through_handler"
            ) as mock_get_handler,
        ):
    ...
    async def test_vertex_passthrough_with_no_default_credentials(self, monkeypatch):
        ...
        with (
            mock.patch(
                "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.vertex_llm_base._ensure_access_token_async"
            ) as mock_ensure_token,
            mock.patch(
                "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.vertex_llm_base._get_token_and_url"
            ) as mock_get_token,
            mock.patch(
                "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.create_pass_through_route"
            ) as mock_create_route,
    ...
    async def test_async_vertex_proxy_route_api_key_auth(self):
        ...
        arrives in x-derouter-api-key and must reach user_api_key_auth. With no Vertex
        credential configured, that virtual key must not be forwarded to Google, so
        the request fails with a clean 401 (LIT-5997).
        """
        # Mock dependencies
        mock_request = Mock()
        mock_request.headers = {"x-derouter-api-key": "test-key-123"}
        mock_request.method = "POST"
        mock_response = Mock()
    ...
    def test_vertex_passthrough_handler_multimodal_embedding_response(self):
        ...
        mock_logging_obj = Mock(spec=DeRouterLoggingObj)
        mock_logging_obj.optional_params = {}
        mock_logging_obj.derouter_call_id = "test-call-id-123"
        ...
        with patch(
            "derouter.llms.vertex_ai.multimodal_embeddings.transformation.VertexAIMultimodalEmbeddingConfig"
            ...
            from derouter.types.utils import Embedding, EmbeddingResponse, Usage
    ...
    def test_vertex_passthrough_handler_predict_cost_tracking(self):
        ...
        with patch("derouter.completion_cost") as mock_completion_cost:
    ...
    def test_vertex_passthrough_handler_embed_content_response(self):
        ...
        mock_logging_obj = Mock(spec=DeRouterLoggingObj)
        mock_logging_obj.optional_params = {}
        mock_logging_obj.derouter_call_id = "test-call-id-embed"
        mock_logging_obj.model_call_details = {}
    ...
    def test_vertex_passthrough_handler_batch_embed_contents_response(self):
        ...
        mock_logging_obj = Mock(spec=DeRouterLoggingObj)
        mock_logging_obj.optional_params = {}
        mock_logging_obj.derouter_call_id = "test-call-id-batch"
        mock_logging_obj.model_call_details = {}
    ...
    def test_vertex_passthrough_handler_embed_content_google_ai_studio_url(self):
        ...
        mock_logging_obj = Mock(spec=DeRouterLoggingObj)
        mock_logging_obj.optional_params = {}
        mock_logging_obj.derouter_call_id = "test-call-id-gemini-studio"
        mock_logging_obj.model_call_details = {}
    ...
    def test_vertex_passthrough_handler_prices_regional_endpoint_with_uplift(self, monkeypatch, streaming):
        ...
        monkeypatch.setattr(
            ...
            "model_cost",
            {
                **derouter.get_model_cost_map(url=""),
...
class TestVertexAIDiscoveryPassThroughHandler:
    ...
    async def test_vertex_discovery_proxy_route_api_key_auth(self):
        """
        The virtual key arrives in x-derouter-api-key and must reach user_api_key_auth.
...
async def test_mistral_passthrough_accepts_multipart_without_json_parsing():
    boundary = "----derouter-test-boundary"
    body = (
        f"--{boundary}\r\n"
    ...
    with (
        patch(
            "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.passthrough_endpoint_router.get_credentials",
...
class TestBedrockLLMProxyRoute:
    ...
    async def test_bedrock_llm_proxy_route_application_inference_profile(self):
        ...
        with (
            patch(
                "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints._read_request_body",
                return_value=mock_request_body,
            ),
            patch(
                "derouter.proxy.common_request_processing.ProxyBaseLLMRequestProcessing",
    ...
    async def test_bedrock_error_handling_returns_actual_error(self):
        ...
        with patch(
            "derouter.proxy.common_request_processing.ProxyBaseLLMRequestProcessing.base_passthrough_process_llm_request",
            new_callable=AsyncMock,
    ...
    async def test_bedrock_passthrough_uses_model_specific_credentials(self):
        ...
        with patch.dict(
            ...
            # Test 1: Verify get_derouter_params extracts AWS credentials from kwargs
            kwargs_with_creds = {
            ...
            derouter_params = get_derouter_params(**kwargs_with_creds)

            # Verify credentials are extracted
            assert derouter_params.get("aws_access_key_id") == model_access_key
            assert derouter_params.get("aws_secret_access_key") == model_secret_key
            assert derouter_params.get("aws_region_name") == model_region
            assert derouter_params.get("aws_session_token") == model_session_token
            ...
            deployment_derouter_params = deployment.get("derouter_params", {})

            # Verify model-specific credentials are in the deployment
            assert (
                deployment_derouter_params.get("aws_access_key_id") == model_access_key
            )
            assert (
                deployment_derouter_params.get("aws_secret_access_key")
                == model_secret_key
            )
            assert deployment_derouter_params.get("aws_region_name") == model_region
            assert (
                deployment_derouter_params.get("aws_session_token")
                == model_session_token
            )

            # Verify environment variables are NOT in the deployment
            assert deployment_derouter_params.get("aws_access_key_id") != env_access_key
            assert (
                deployment_derouter_params.get("aws_secret_access_key") != env_secret_key
            )
            assert deployment_derouter_params.get("aws_region_name") != env_region
            ...
            with (
                patch(
                    "derouter.passthrough.main.llm_passthrough_route",
                    new_callable=AsyncMock,
                ...
                # The key verification is that get_derouter_params extracts the credentials
    ...
    async def test_key_guardrail_blocks_bedrock_converse_passthrough(self):
        ...
        from derouter.llms.pass_through.guardrail_translation import (
            guardrail_translation_mappings,
        )
        from derouter.types.utils import CallTypes, GenericGuardrailAPIInputs
...
class TestBedrockAgentRuntimePassthroughToggle:
    ...
    def _patched_dispatch(self, general_settings: Mapping[str, object]):
        ...
        with (
            ...
            patch(
                "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.get_secret_str",
                return_value="us-east-1",
            ),
            patch("derouter.llms.bedrock.chat.BedrockConverseLLM", return_value=bedrock_llm),
            patch(
                "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.create_request_copy",
                Mock(),
            ),
            patch(
                "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.create_pass_through_route",
    ...
    async def test_model_invoke_still_routed_when_agent_runtime_disabled(self):
        with (
            patch("derouter.proxy.proxy_server.general_settings", self.DISABLED),
            patch(
                "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.get_secret_str",
                return_value="us-east-1",
            ),
            ...
            patch(
                "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.bedrock_llm_proxy_route",
...
class TestLLMPassthroughFactoryProxyRoute:
    ...
    async def test_llm_passthrough_factory_proxy_route_success(self):
        ...
        with (
            patch(
                "derouter.utils.ProviderConfigManager.get_provider_model_info"
            ...
            mock_get_provider.assert_called_once_with(
                provider=derouter.LlmProviders(LlmProviders.VLLM), model=None
            )
...
class TestVLLMProxyRoute:
    @pytest.mark.asyncio
    @patch(  # test-quality-ok: patching derouter internal for unit test isolation
        "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.get_request_body",
        return_value={"model": "router-model", "stream": False},
    )
    @patch(  # test-quality-ok: patching derouter internal for unit test isolation
        "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.is_passthrough_request_using_router_model",
        return_value=True,
    )
    @patch("derouter.proxy.proxy_server.llm_router")  # test-quality-ok: patching derouter internal for unit test isolation
    ...
    @patch(  # test-quality-ok: patching derouter internal for unit test isolation
        "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.llm_passthrough_factory_proxy_route"
    )
...
class TestGigachatProxyRoute:
    ...
    async def test_gigachat_router_handler_keeps_cached_body_and_payload_metadata_pristine(self):
        ...
        from derouter.proxy.common_utils.http_parsing_utils import get_request_body
        from derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints import (
        ...
        with patch(  # test-quality-ok: patching derouter internal for unit test isolation
            "derouter.proxy.common_request_processing.ProxyBaseLLMRequestProcessing",
            _CapturingProcessor,
        ):
    ...
    @patch(  # test-quality-ok: patching derouter internal for unit test isolation
        "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.is_streaming_request_fn",
        new_callable=AsyncMock,
        return_value=False,
    )
    @patch(  # test-quality-ok: patching derouter internal for unit test isolation
        "derouter.llms.gigachat.authenticator.get_access_token",
    ...
    async def test_allm_passthrough_streaming_preserves_upstream_headers(self):
        ...
        processor = ProxyBaseLLMRequestProcessing(
            data={
                ...
                "stream": True,
                "derouter_call_id": "call-123",
                "derouter_logging_obj": MagicMock(derouter_call_id="call-123"),
            }
        )
        ...
        with patch.object(
            ...
            new=AsyncMock(
                return_value=(
                    processor.data,
                    processor.data["derouter_logging_obj"],
                )
            ),
        ), patch(  # test-quality-ok: patching derouter internal for unit test isolation
            "derouter.proxy.common_request_processing.route_request",
            new=_fake_route_request,
        ), patch(  # test-quality-ok: patching derouter internal for unit test isolation
            "derouter.proxy.common_request_processing.ProxyBaseLLMRequestProcessing.get_custom_headers",
            return_value={"x-derouter-call-id": "call-123"},
        ):
        ...
        assert result.headers["x-derouter-call-id"] == "call-123"
...
class TestForwardHeaders:
    ...
    async def test_pass_through_request_with_forward_headers_true(self):
        ...
        custom_headers = {
            "x-derouter-header": "derouter-value",
        }
        ...
        with (
            patch(
                "derouter.proxy.pass_through_endpoints.pass_through_endpoints._read_request_body",
                return_value=mock_request_body,
            ),
            patch(
                "derouter.proxy.pass_through_endpoints.pass_through_endpoints.get_async_httpx_client"
            ) as mock_get_client,
            patch("derouter.proxy.proxy_server.proxy_logging_obj") as mock_logging_obj,
        ):
            ...
            assert sent_headers["x-derouter-header"] == "derouter-value"
    ...
    async def test_pass_through_request_with_forward_headers_false(self):
        ...
        custom_headers = {
            "x-derouter-header": "derouter-value",
            "authorization": "Bearer derouter-token",
        }
        ...
        with (
            ...
            assert sent_headers["x-derouter-header"] == "derouter-value"
            assert sent_headers["authorization"] == "Bearer derouter-token"
...
class TestMilvusProxyRoute:
    ...
    async def test_milvus_proxy_route_success(self):
        ...
        mock_index_object.derouter_params.vector_store_name = vector_store_name
        mock_index_object.derouter_params.vector_store_index = vector_store_index
        ...
        with (
            ...
            patch(
                "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.ProviderConfigManager.get_provider_vector_stores_config"
            ) as mock_get_config,
            patch(
                "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.is_allowed_to_call_vector_store_endpoint"
            ) as mock_is_allowed,
            patch(
                "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints._safe_set_request_parsed_body"
            ) as mock_safe_set,
            patch(
                "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.create_pass_through_route"
            ) as mock_create_route,
            patch.object(derouter, "vector_store_index_registry") as mock_index_registry,
            patch.object(derouter, "vector_store_registry") as mock_vector_registry,
        ):
            # Setup mocks
            mock_provider_config = MagicMock()
            ...
            mock_vector_registry.get_derouter_managed_vector_store_from_registry_by_name.return_value = (
                mock_vector_store
            )
    ...
    async def test_milvus_proxy_route_no_index_registry(self):
        ...
        with (
            ...
            patch.object(derouter, "vector_store_index_registry", None),
        ):
            mock_get_config.return_value = MagicMock()
    ...
    async def test_milvus_proxy_route_not_managed_index(self):
        ...
        with (
            ...
            patch.object(derouter, "vector_store_index_registry") as mock_index_registry,
            patch.object(derouter, "vector_store_registry", MagicMock()),
        ):
            mock_get_config.return_value = MagicMock()
            ...
            assert exc_info.value.status_code == 400
            assert (
                f"Collection {collection_name} is not a derouter managed vector store index"
                in str(exc_info.value.detail)
            )
    ...
    async def test_milvus_proxy_route_no_api_base(self):
        ...
        mock_vector_store = {"derouter_params": {}}  # No api_base
    ...
    async def test_milvus_proxy_route_endpoint_without_leading_slash(self):
        ...
        mock_vector_store = {"derouter_params": {"api_base": api_base}}
...
class TestOpenAIPassthroughRoute:
    ...
    async def test_openai_passthrough_responses_api(self):
        ...
        routed to DeRouter's native implementation instead of passthrough
        """
...
class TestCursorProxyRoute:
    ...
    async def test_cursor_proxy_route_raises_on_missing_api_key(self):
        ...
        with (
            ...
            patch(
                "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.derouter.credential_list",
                [],
            ),
        ):
    ...
    async def test_cursor_proxy_route_uses_ui_credential(self):
        """should use credentials added via UI (derouter.credential_list) when env var is not set"""
...
class TestVertexRawPredictStreamingClassification:
    ...
    async def _capture_passthrough_kwargs(self, endpoint: str, body: object) -> dict:
        ...
        request = Request(
            {
                ...
                "headers": [
                    ...
                    (b"x-derouter-api-key", b"test-key"),
        ...
        module = "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints"
        with (
            mock.patch(
                "derouter.llms.vertex_ai.vertex_llm_base.VertexBase.load_auth",
            ...
            mock.patch(f"{module}.get_derouter_virtual_key", return_value="Bearer test-key"),
...
class TestVertexCredentiallessPassthroughVirtualKeyLeak:
    ...
    to Google, including whichever header carried the caller's DeRouter virtual key.
    DeRouter accepts that key from several headers (``Authorization``,
    ``x-derouter-api-key``, ``x-goog-api-key``, ``api-key``, ``x-api-key``), and
    ...
    proxy-only auth headers Google never consumes (``x-derouter-api-key``,
    ...
    ``general_settings.derouter_key_header_name``, whatever that header is named.

    The by-value strip keys off what actually authenticated the caller (the
    master key, or the DeRouter key whose hash ``user_api_key_auth`` resolved as
    ...
    async def _run(
        ...
        monkeypatch.setattr("derouter.proxy.proxy_server.master_key", master_key)
    ...
    @pytest.mark.asyncio
    async def test_x_derouter_api_key_virtual_key_is_rejected_not_forwarded(self, monkeypatch):
        raised, forwarded = await self._run(
            monkeypatch,
            [(b"x-derouter-api-key", self.VKEY.encode()), (b"content-type", b"application/json")],
        )
        assert forwarded is None, "credential-less request must never reach the upstream forwarder"
        assert raised is not None and raised.status_code == 401
    ...
    async def test_x_goog_api_key_carrying_virtual_key_is_rejected_not_forwarded(self, monkeypatch):
        raised, forwarded = await self._run(
            monkeypatch,
            [
                (b"x-derouter-api-key", self.VKEY.encode()),
    ...
    async def test_byo_google_oauth_token_still_forwards_without_virtual_key(self, monkeypatch):
        ...
        assert "x-derouter-api-key" not in forwarded
    ...
    @pytest.mark.parametrize(
        "credential_header",
        sorted(
            SpecialHeaders.derouter_credential_header_names()
            - {"authorization", "x-goog-api-key", "x-derouter-api-key"}
        ),
    )
    ...
    async def test_virtual_key_in_operator_configured_header_is_stripped(self, monkeypatch):
        with mock.patch.dict(  # test-quality-ok: general_settings is the real proxy config surface for derouter_key_header_name; no injection seam exists on this route
    ...
    async def test_virtual_key_in_pass_through_configured_header_is_dropped_and_rejected(self, monkeypatch):
        with mock.patch.dict(  # test-quality-ok: general_settings is the real proxy config surface for pass_through_endpoints; no injection seam exists on this route
            "derouter.proxy.proxy_server.general_settings",
            {"pass_through_endpoints": [{"headers": {"derouter_user_api_key": "x-company-key"}}]},
        ):
            raised, forwarded = await self._run(
                monkeypatch,
                [
    ...
    @pytest.mark.asyncio
    async def test_virtual_key_in_mapped_route_derouter_user_api_key_header_is_stripped(self, monkeypatch):
        raised, forwarded = await self._run(
            monkeypatch,
            [
                (b"derouter_user_api_key", self.VKEY.encode()),
                (b"authorization", b"Bearer ya29.byo-google-oauth"),
                (b"x-goog-api-key", b"AIza-real-google-api-key"),
                (b"content-type", b"application/json"),
            ],
        ...
        assert "derouter_user_api_key" not in forwarded
        assert self.VKEY not in " ".join(f"{name}:{value}" for name, value in forwarded.items())

    @pytest.mark.asyncio
    async def test_virtual_key_in_mapped_route_derouter_user_api_key_header_alone_is_rejected(self, monkeypatch):
        raised, forwarded = await self._run(
            monkeypatch,
            [
                (b"derouter_user_api_key", self.VKEY.encode()),
        ...
        assert forwarded is None, "a virtual key in the mapped-route derouter_user_api_key header must be dropped, not forwarded"
        assert raised is not None and raised.status_code == 401

    GOOGLE_OAUTH_TOKEN = "ya29.byo-google-oauth-token"

    DEROUTER_JWT_CLAIMS = MappingProxyType({"sub": "jwt-subject", "iss": "https://idp.example.com"})
    DEROUTER_JWT = _unsigned_jwt(DEROUTER_JWT_CLAIMS)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                "sk-master-1234",
                UserAPIKeyAuth(api_key=None, user_id="jwt-subject", jwt_claims=dict(DEROUTER_JWT_CLAIMS)),
                id="jwt-auth",
            ),
    ...
    async def test_google_token_in_authorization_is_forwarded_when_auth_did_not_consume_it(
        ...
        assert raised is None, f"the caller's own Google token must not be mistaken for a DeRouter key: {raised}"
        assert forwarded is not None
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                DEROUTER_JWT,
                UserAPIKeyAuth(api_key=DEROUTER_JWT, user_id="jwt-subject"),
            ...
            pytest.param(
                ...
                UserAPIKeyAuth(
                    ...
                    jwt_claims={
                        **DEROUTER_JWT_CLAIMS,
                        JWTHandler.DEROUTER_JWT_ISSUER_CLAIM: "https://idp.example.com",
                        JWTHandler.DEROUTER_USER_ID_CLAIM: "jwt-subject",
                    },
                ),
    ...
    async def test_non_sk_derouter_credential_that_authenticated_is_rejected_not_forwarded(
    ...
    async def test_jwt_authenticated_caller_keeps_a_different_byo_google_jwt(self, monkeypatch):
        raised, forwarded = await self._run(
            monkeypatch,
            [
                (b"x-derouter-api-key", self.DEROUTER_JWT.encode()),
                (b"authorization", f"Bearer {self.GOOGLE_SERVICE_ACCOUNT_JWT}".encode()),
                (b"content-type", b"application/json"),
            ],
            authenticated=UserAPIKeyAuth(api_key=None, user_id="jwt-subject", jwt_claims=dict(self.DEROUTER_JWT_CLAIMS)),
        )
    ...
    async def test_master_key_in_authorization_alone_is_rejected(self, monkeypatch):
        raised, forwarded = await self._run(
            ...
            authenticated=UserAPIKeyAuth(api_key=DEROUTER_PROXY_MASTER_KEY_ALIAS, user_role=LitellmUserRoles.PROXY_ADMIN),
        )
...
class TestVertexPassthroughDefaultLocationOnShortRoutes:
    ...
    async def _forward(
        ...
        module: Final = "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints"
...
class TestAzureProxyRouteCrossIndexAuthorization:
    ...
    async def test_authorizes_the_targeted_index(self):
        ...
        index_object.derouter_params.vector_store_name = "my-store"
        vector_store = {"derouter_params": {"api_base": "https://svc.search.windows.net"}}
        ...
        with (
            ...
            patch(
                "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.assert_user_can_access_vector_store",
                new=AsyncMock(),
            ),
            patch(
                "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints.BaseOpenAIPassThroughHandler._base_openai_pass_through_handler",
            ...
            mock_vector_registry.get_derouter_managed_vector_store_from_registry_by_name.return_value = vector_store
    ...
    async def test_trailing_index_segment_does_not_authorize_a_different_index(self):
        with (
            ...
            assert mock_handler.await_args.kwargs["custom_llm_provider"] == derouter.LlmProviders.AZURE
...
class TestComprehendMedicalProxyRoute:
    ...
    async def test_signs_and_forwards_detect_entities_v2(self):
        ...
        from derouter.types.passthrough_endpoints.pass_through_endpoints import (
            DEROUTER_PASS_THROUGH_CUSTOM_BODY_STATE_KEY,
            DEROUTER_PASS_THROUGH_RAW_BODY_STATE_KEY,
        )
        ...
        with (
            ...
            patch(
                "derouter.llms.bedrock.base_aws_llm.BaseAWSLLM.get_credentials",
        ...
        assert getattr(mock_request.state, DEROUTER_PASS_THROUGH_CUSTOM_BODY_STATE_KEY) == request_body
        assert json.loads(getattr(mock_request.state, DEROUTER_PASS_THROUGH_RAW_BODY_STATE_KEY)) == request_body
        mock_endpoint_func.assert_awaited_once()
    ...
    def test_comprehendmedical_is_a_mapped_pass_through_route(self):
        ...
        assert "/comprehendmedical" in DeRouterRoutes.mapped_pass_through_routes.value
...
class TestVertexAILiveWebsocketPassthrough:
    ...
    async def test_uses_db_deployment_credentials_without_query_params(self, monkeypatch):
        from derouter.proxy.pass_through_endpoints import (
        ...
        monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", llm_router)
        monkeypatch.setattr(
    ...
    async def test_default_vertex_config_outranks_db_deployment(self, monkeypatch):
        ...
        from derouter.types.passthrough_endpoints.vertex_ai import (
    ...
    async def test_credential_failure_close_names_configuration_options(self, monkeypatch):
        ...
        with (
            patch.object(passthrough_module.vertex_llm_base, "_ensure_access_token_async", ensure_token),
            patch("derouter.proxy.proxy_server.proxy_logging_obj") as mock_proxy_logging,
        ):
...
class TestPassthroughRouterModelBudgetReservation:
    ...
    def _install_recording_router(self, monkeypatch, body: dict) -> list[dict]:
        import derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints as ep
    ...
    def _assert_metadata_carries_attribution(self, captured: list[dict], user_api_key_dict: UserAPIKeyAuth) -> None:
        ...
        assert captured[0].get("metadata") is None, (
            "attribution must ride the derouter_metadata bucket the router canonicalizes on; "
            "the plain metadata bucket is dropped for every non-user_api_key field"
        )
        derouter_metadata = captured[0]["derouter_metadata"]
        assert derouter_metadata["user_api_key"] == user_api_key_dict.api_key
        assert derouter_metadata["user_api_key_budget_reservation"] is user_api_key_dict.budget_reservation
        assert derouter_metadata["user_api_key_user_id"] == user_api_key_dict.user_id
        assert derouter_metadata["user_api_key_team_id"] == user_api_key_dict.team_id
        assert derouter_metadata["agent_id"] == user_api_key_dict.agent_id
        assert derouter_metadata["user_api_end_user_max_budget"] == user_api_key_dict.end_user_max_budget
...
class TestAzureRouterModelStreamingDispatch:
    ...
    async def test_azure_router_model_streaming_returns_streaming_response(self, monkeypatch):
        import derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints as ep
        import derouter.proxy.proxy_server as proxy_server
        from derouter.passthrough.main import AsyncPassthroughStreamingResponse
        ...
        from derouter.router_utils.add_retry_fallback_headers import prepare_response_for_header_attachment
...
class TestAzureRouterModelStreamingKeepalive:
    async def _dispatch(self, monkeypatch, interval, headers_delay=0.0, body_delay=0.0) -> StreamingResponse:
        ...
        monkeypatch.setattr(derouter, "sse_keepalive_ping_interval_seconds", interval)
...
async def test_bedrock_count_tokens_error_forwards_provider_headers():
    ...
    with patch(  # test-quality-ok: the route's BedrockError branch is only reachable when the handler raises
        "derouter.llms.bedrock.count_tokens.handler.BedrockCountTokensHandler.handle_count_tokens_request",
