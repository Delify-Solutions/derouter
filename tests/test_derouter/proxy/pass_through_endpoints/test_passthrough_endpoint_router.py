...
def _flagged_deployment(model: str, **derouter_params) -> dict:
    return {
        "model_name": model.split("/", 1)[-1],
        "derouter_params": {"model": model, "use_in_pass_through": True, **derouter_params},
    }


def _passthrough_router(llm_router: derouter.Router | None) -> PassthroughEndpointRouter:
...
def test_credential_loaded_after_deployment_registration_still_resolves():
    llm_router = derouter.Router(
        model_list=[_flagged_deployment("openai/gpt-4o", derouter_credential_name="cred_openai")]
    )
...
def test_deleted_deployment_stops_serving_its_key(monkeypatch):
    llm_router = derouter.Router(model_list=[_flagged_deployment("openai/gpt-4o", api_key="sk-inline")])
    passthrough_router = _passthrough_router(llm_router)
...
def test_missing_credential_and_no_inline_key_falls_back_to_env(monkeypatch):
    llm_router = derouter.Router(
        model_list=[_flagged_deployment("openai/gpt-4o", derouter_credential_name="cred_deleted")]
    )
    passthrough_router = _passthrough_router(llm_router)
    monkeypatch.setenv("OPENAI_API_KEY", "sk-from-env")
...
def test_unflagged_deployment_does_not_match():
    llm_router = derouter.Router(
        model_list=[
            {
                "model_name": "gpt-4o",
                "derouter_params": {"model": "openai/gpt-4o", "api_key": "sk-not-flagged"},
            }
        ]
    )
    passthrough_router = _passthrough_router(llm_router)
...
def _vertex_deployment(model_name: str, model: str, **derouter_params) -> dict:
    return {
...
def test_vertex_deployment_resolves_via_named_credential():
    ...
    llm_router = derouter.Router(
        model_list=[
            _vertex_deployment(
                "gemini-live", "vertex_ai/gemini-live-2.5-flash", derouter_credential_name="cred_gcp"
            )
        ]
    )
    passthrough_router = _passthrough_router(llm_router)
...
def test_vertex_deployment_resolves_from_inline_derouter_params():
    llm_router = derouter.Router(
        model_list=[
            _vertex_deployment(
                "gemini-live",
...
def _two_vertex_deployments_router() -> derouter.Router:
...
def test_vertex_named_credential_keeps_dict_service_account():
    ...
    llm_router = derouter.Router(
        model_list=[
            _vertex_deployment(
                "gemini-live", "vertex_ai/gemini-live-2.5-flash", derouter_credential_name="cred_gcp_dict"
            )
        ]
    )
    passthrough_router = _passthrough_router(llm_router)
...
def test_vertex_deployment_with_deleted_credential_is_skipped(monkeypatch):
    ...
    llm_router = derouter.Router(
        model_list=[
            _vertex_deployment(
                "gemini-live", "vertex_ai/gemini-live-2.5-flash", derouter_credential_name="cred_gone"
            )
        ]
    )
    passthrough_router = _passthrough_router(llm_router)
