...
@pytest.mark.asyncio
async def test_get_derouter_virtual_key():
    """
    Test that the get_derouter_virtual_key function correctly handles the API key authentication
    """
    # Test with x-derouter-api-key
    mock_request = Mock()
    mock_request.headers = {"x-derouter-api-key": "test-key-123"}
    result = get_derouter_virtual_key(mock_request)
    ...
    # Test with both headers (x-derouter-api-key should take precedence)
    mock_request.headers = {
        "x-derouter-api-key": "test-key-123",
