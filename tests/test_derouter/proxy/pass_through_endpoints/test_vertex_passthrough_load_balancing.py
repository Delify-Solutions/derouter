...
async def test_vertex_passthrough_load_balancing():
    ...
    with (
        patch(
            "derouter.llms.vertex_ai.common_utils.get_vertex_model_id_from_url",
            return_value="gemini-pro",
        ),
        patch("derouter.proxy.proxy_server.llm_router", mock_router),
        patch(
            "derouter.llms.vertex_ai.common_utils.get_vertex_project_id_from_url",
            return_value=None,
        ),
        patch(
            "derouter.llms.vertex_ai.common_utils.get_vertex_location_from_url",
            return_value=None,
        ),
        patch(
        ...
        patch(
            "derouter.proxy.pass_through_endpoints.llm_passthrough_endpoints._prepare_vertex_auth_headers",
...
def test_get_available_deployment_for_pass_through_filters_correctly():
    ...
    assert deployment["derouter_params"]["vertex_project"] == "project-1"
    assert deployment["derouter_params"]["use_in_pass_through"] is True
...
def test_get_available_deployment_for_pass_through_load_balancing():
    ...
    for _ in range(100):
        ...
        project = deployment["derouter_params"]["vertex_project"]
...
async def test_vertex_passthrough_forwards_anthropic_beta_header():
    ...
    with (
        ...
        (
            ...
            vertex_location="us-central1",
            user_api_key_dict=UserAPIKeyAuth(api_key="sk-derouter-secret-key"),
        )
...
async def test_vertex_passthrough_drops_anthropic_beta_only_on_count_tokens(
    ...
    with (
        ...
        mock_auth.return_value = UserAPIKeyAuth(api_key="sk-derouter-secret-key")
...
@pytest.mark.asyncio
async def test_vertex_passthrough_does_not_forward_derouter_auth_token():
    """
    Test that the DeRouter authorization header is NOT forwarded to Vertex AI.

    This test validates the fix for the issue where both the DeRouter auth token
    ...
    The incoming request has:
      - authorization: Bearer <derouter_token>  (should NOT be forwarded)
    ...
    # Create a mock request with ONLY the derouter auth token (no other headers)
    mock_request = MagicMock()
    mock_request.headers = Headers(
        {
            "authorization": "Bearer sk-derouter-secret-key",  # DeRouter token - should NOT be forwarded
            "Authorization": "Bearer sk-derouter-secret-key-uppercase",  # Also try uppercase
        }
    )
    ...
    with (
        ...
        # The DeRouter token should NOT be present (neither lowercase nor as a duplicate)
        assert "authorization" not in headers
        assert headers.get("Authorization") != "Bearer sk-derouter-secret-key"
        assert headers.get("Authorization") != "Bearer sk-derouter-secret-key-uppercase"
...
def test_forward_headers_from_request_x_pass_prefix():
    ...
    """
    from derouter.passthrough.utils import BasePassthroughUtils
