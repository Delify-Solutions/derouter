...
async def test_non_streaming_http_request_handler_multipart_with_non_empty_parsed_body():
    """
    Regression: pass_through_request injects derouter_logging_obj into _parsed_body before
    ...
    await HttpPassThroughEndpointHelpers.non_streaming_http_request_handler(
        ...
        headers={},
        requested_query_params=None,
        _parsed_body={"derouter_logging_obj": MagicMock()},
...
async def test_pass_through_request_failure_handler():
    ...
    Critical Test: When a users pass through endpoint request fails, we must log the failure code, exception in derouter spend logs.
    """
    with patch("derouter.proxy.proxy_server.proxy_logging_obj") as mock_proxy_logging:
        with patch("derouter.llms.custom_httpx.http_handler.get_async_httpx_client") as mock_get_client:
            with patch(
                "derouter.proxy.pass_through_endpoints.pass_through_endpoints.ProxyBaseLLMRequestProcessing"
            ) as mock_processing:
...
async def test_custom_passthrough_predict_path_logs_via_generic_handler():
    ...
    mock_logging_obj = MagicMock(spec=DeRouterLoggingObj)
    ...
    with patch(
        "derouter.proxy.pass_through_endpoints.success_handler.VertexPassthroughLoggingHandler.vertex_passthrough_handler"
...
async def test_pass_through_handler_rejects_unregistered_method():
    ...
    with (
        ...
        patch(
            "derouter.proxy.auth.auth_utils.get_request_route",
            return_value="/test/path",
        ),
        patch(
            "derouter.proxy.pass_through_endpoints.pass_through_endpoints._parse_request_data_by_content_type",
            new_callable=AsyncMock,
            return_value=({}, {}, None, False),
        ),
        patch(
            "derouter.proxy.pass_through_endpoints.pass_through_endpoints._registered_pass_through_routes",
            {
...
async def test_initialize_pass_through_endpoints_with_include_subpath():
    ...
    with patch(
        "derouter.proxy.pass_through_endpoints.pass_through_endpoints.InitPassThroughEndpointHelpers.add_exact_path_route"
    ) as mock_add_exact_route:
        with patch(
            "derouter.proxy.pass_through_endpoints.pass_through_endpoints.InitPassThroughEndpointHelpers.add_subpath_route"
            ...
            with patch(
                ...
                with patch(
                    "derouter.proxy.pass_through_endpoints.pass_through_endpoints.set_env_variables_in_header"
                ) as mock_set_env:
...
async def test_create_pass_through_route_with_cost_per_request():
    ...
    with (
        patch("derouter.proxy.pass_through_endpoints.pass_through_endpoints.pass_through_request") as mock_pass_through,
        patch(
            "derouter.proxy.pass_through_endpoints.pass_through_endpoints.InitPassThroughEndpointHelpers.is_registered_pass_through_route"
        ) as mock_is_registered,
        patch(
            "derouter.proxy.pass_through_endpoints.pass_through_endpoints.InitPassThroughEndpointHelpers.get_registered_pass_through_route"
...
def test_resolve_llm_passthrough_timeout_precedence():
    ...
    assert (
        resolve_llm_passthrough_timeout(
            ...
            derouter_params={"timeout": 60},
        )
        == 30.0
    )
    assert (
        resolve_llm_passthrough_timeout(
            derouter_params={"timeout": 90},
        )
        == 90.0
    )
...
async def test_pass_through_request_contains_proxy_server_request_in_kwargs():
    ...
    with patch("derouter.proxy.proxy_server.proxy_logging_obj") as mock_proxy_logging:
        with patch(
            "derouter.proxy.pass_through_endpoints.pass_through_endpoints.HttpPassThroughEndpointHelpers.non_streaming_http_request_handler"
        ) as mock_http_handler:
            with patch(
                "derouter.proxy.pass_through_endpoints.pass_through_endpoints.ProxyBaseLLMRequestProcessing"
            ) as mock_processing:
                with patch(
                    "derouter.proxy.pass_through_endpoints.pass_through_endpoints.pass_through_endpoint_logging.pass_through_async_success_handler"
                ) as mock_success_handler:
                    with patch(
                        "derouter.proxy.pass_through_endpoints.pass_through_endpoints.get_response_body"
                        ...
                        # Verify that derouter_params exists in kwargs
                        assert "derouter_params" in call_kwargs
                        derouter_params = call_kwargs["derouter_params"]

                        # Verify that proxy_server_request exists in derouter_params
                        assert "proxy_server_request" in derouter_params
                        proxy_server_request = derouter_params["proxy_server_request"]
...
async def test_pass_through_request_streaming_marks_logging_obj_as_stream():
    ...
    with patch("derouter.proxy.proxy_server.proxy_logging_obj") as mock_proxy_logging:
        with patch(
            ...
            with patch(
                "derouter.proxy.pass_through_endpoints.pass_through_endpoints.PassThroughStreamingHandler.chunk_processor"
                ...
                logging_obj = mock_chunk_processor.call_args.kwargs["derouter_logging_obj"]
...
async def test_pass_through_request_streamed_response_is_owned_by_the_caller():
    ...
    """
    import derouter
    from derouter.llms.custom_httpx.http_handler import get_async_httpx_client
    from derouter.types.llms.custom_http import httpxSpecialProvider
    ...
    prisma_client.db.derouter_managedobjecttable.find_first = AsyncMock(return_value=None)
    prisma_client.db.derouter_managedobjecttable.upsert = AsyncMock(return_value=None)
    prisma_client.db.derouter_managedfiletable.find_first = AsyncMock(return_value=None)
    ...
    cache_dict = derouter.in_memory_llm_clients_cache.cache_dict
    ...
    proxy_server_globals = (
        patch("derouter.proxy.proxy_server.proxy_logging_obj", mock_proxy_logging),  # test-quality-ok: read at call time
        patch("derouter.proxy.proxy_server.general_settings", flag_on),  # test-quality-ok: read at call time
        patch("derouter.proxy.proxy_server.prisma_client", prisma_client),  # test-quality-ok: read at call time
    )
    ...
    prisma_client.db.derouter_managedobjecttable.upsert.assert_awaited_once()
    created = prisma_client.db.derouter_managedobjecttable.upsert.await_args.kwargs["data"]["create"]
...
async def test_create_pass_through_endpoint():
    ...
    with patch("derouter.proxy.proxy_server.get_config_general_settings") as mock_get_config:
        with patch("derouter.proxy.proxy_server.update_config_general_settings") as mock_update_config:
...
async def test_create_pass_through_endpoint_auth_true_enforces_allowlist():
    ...
    with (
        patch("derouter.proxy.proxy_server.get_config_general_settings") as mock_get_config,
        patch("derouter.proxy.proxy_server.update_config_general_settings"),
        patch(
            "derouter.proxy.pass_through_endpoints.pass_through_endpoints._registered_pass_through_routes",
            registry,
        ),
...
async def test_update_pass_through_endpoint_preserves_auth_false():
    ...
    with (
        patch("derouter.proxy.proxy_server.get_config_general_settings") as mock_get_config,
        patch("derouter.proxy.proxy_server.update_config_general_settings") as mock_update_config,
        patch(
            "derouter.proxy.pass_through_endpoints.pass_through_endpoints._registered_pass_through_routes",
            registry,
        ),
...
async def test_delete_pass_through_endpoint_not_found():
    ...
    from derouter.proxy._types import ConfigFieldInfo, UserAPIKeyAuth
    from derouter.proxy.pass_through_endpoints.pass_through_endpoints import (
        delete_pass_through_endpoints,
    )
...
async def test_get_pass_through_endpoints_includes_config_and_db():
    ...
    with patch(
        ...
        with patch(
            "derouter.proxy.pass_through_endpoints.pass_through_endpoints._get_pass_through_endpoints_from_db",
            new_callable=AsyncMock,
        ) as mock_get_db:
            with patch(
                "derouter.proxy.pass_through_endpoints.pass_through_endpoints._get_pass_through_endpoints_from_config"
            ) as mock_get_config:
...
def test_get_pass_through_endpoints_from_config_skips_malformed():
    ...
    with patch(
        "derouter.proxy.proxy_server.config_passthrough_endpoints",
        config_passthrough_endpoints,
    ):
...
class _FakeManagedFilesHook:
    ...
    async def get_unified_file_id(self, file_id: str, derouter_parent_otel_span=None) -> SimpleNamespace:
...
async def _run_pass_through_and_capture_wire_url(
    ...
    try:
        with ExitStack() as stack:
            stack.enter_context(patch("derouter.proxy.proxy_server.proxy_logging_obj", mock_proxy_logging))
            if managed_files_hook is not None:
                ...
                stack.enter_context(patch("derouter.proxy.proxy_server.prisma_client", None))
            response = await pass_through_request(
                request=mock_request,
                target=target,
                custom_headers={},
...
async def test_pass_through_request_without_merge_replaces_target_query():
    ...
    assert dict(wire_url.params) == {"q": "derouter"}
...
async def test_pass_through_request_merge_query_params_rewrites_managed_ids_on_the_wire():
    ...
    """
    from derouter.proxy.pass_through_endpoints.managed_id_codec import new_managed_id
...
async def test_filter_endpoints_by_team_allowed_routes_with_filter():
    ...
    """
    from derouter.proxy._types import PassThroughGenericEndpoint
    from derouter.proxy.pass_through_endpoints.pass_through_endpoints import (
    ...
    mock_prisma_client.db.derouter_teamtable.find_unique = AsyncMock(return_value=mock_team)
    ...
    mock_prisma_client.db.derouter_teamtable.find_unique.assert_called_once_with(where={"team_id": "test-team-123"})
...
async def test_bedrock_router_passthrough_metadata_initialization():
    ...
    derouter_params.proxy_server_request were missing for /bedrock passthrough
    ...
    with patch("derouter.proxy.common_request_processing.ProxyBaseLLMRequestProcessing") as mock_processing_class:
        # Setup mock instance
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_adds_headers_to_metadata():
    """
    Test that add_derouter_data_to_request adds headers to metadata for guardrails.
    ...
    # Verify headers are added to derouter_metadata for guardrails.
    # Bedrock passthrough uses derouter_metadata to prevent key-level
    # tags from leaking into the provider payload (GH#30629).
    assert "derouter_metadata" in result, "derouter_metadata should be present in result"
    assert "headers" in result["derouter_metadata"], "headers should be present in derouter_metadata"
    assert isinstance(result["derouter_metadata"]["headers"], dict), "headers should be a dictionary"

    # Verify specific headers are accessible (important for guardrails)
    headers = result["derouter_metadata"]["headers"]
...
async def test_create_pass_through_route_custom_body_url_target():
    ...
    request.state[DEROUTER_PASS_THROUGH_CUSTOM_BODY_STATE_KEY]; it is forwarded to
    ...
    with (
        ...
        setattr(mock_request.state, DEROUTER_PASS_THROUGH_CUSTOM_BODY_STATE_KEY, bedrock_body)
...
async def test_pass_through_request_non_streaming_uses_content_for_state_raw_body():
    ...
    mock_request.headers = Headers({"Content-Type": "application/json"})
    mock_request.state = SimpleNamespace()
    setattr(mock_request.state, DEROUTER_PASS_THROUGH_RAW_BODY_STATE_KEY, raw_signed)
    ...
    with (
        ...
        patch(
            "derouter.proxy.proxy_server.proxy_logging_obj.pre_call_hook",
            new=AsyncMock(side_effect=_hook_mutates_body),
        ),
        patch(
            "derouter.proxy.proxy_server.proxy_logging_obj.post_call_response_headers_hook",
...
def test_is_registered_pass_through_route_with_custom_root():
    ...
    with patch("derouter.proxy.utils.get_server_root_path", return_value="/proxy"):
    ...
    with patch("derouter.proxy.utils.get_server_root_path", return_value="/"):
        assert InitPassThroughEndpointHelpers.is_registered_pass_through_route("/api/endpoint") is True
...
def test_get_registered_pass_through_route_with_custom_root():
    ...
    with patch("derouter.proxy.utils.get_server_root_path", return_value="/derouter"):
        # Prefixed incoming route
        result = InitPassThroughEndpointHelpers.get_registered_pass_through_route("/derouter/chat/completions")
...
def test_db_registered_pass_through_route_bare_path_convention(
    ...
    with patch(
        "derouter.proxy.utils.get_server_root_path",
...
def test_mapped_pass_through_routes_with_server_root_path():
    ...
    Regression test for https://github.com/Delify-Solutions/derouter/issues/22272
    """
    from derouter.proxy.pass_through_endpoints.pass_through_endpoints import (
        InitPassThroughEndpointHelpers,
    )
    ...
    with patch("derouter.proxy.utils.get_server_root_path", return_value="/derouter"):
        ...
        assert (
            InitPassThroughEndpointHelpers.is_registered_pass_through_route("/derouter/vertex_ai/v1/projects/foo")
            is True
        )
        assert InitPassThroughEndpointHelpers.is_registered_pass_through_route("/derouter/bedrock/model/invoke") is True
...
class TestStaleRouteCleanupOnReload:
    ...
    def _patches():
        ...
        stack.enter_context(
            patch(
                "derouter.proxy.pass_through_endpoints.pass_through_endpoints.SafeRouteAdder.add_api_route_if_not_exists"
            )
        )
        stack.enter_context(patch("derouter.proxy.proxy_server.premium_user", True))
        mock_set_env = stack.enter_context(
            patch("derouter.proxy.pass_through_endpoints.pass_through_endpoints.set_env_variables_in_header")
        )
        mock_set_env.return_value = {}
        return stack
...
_PT_MODULE = "derouter.proxy.pass_through_endpoints.pass_through_endpoints"
...
async def _drive_pass_through_block(raised_exception):
    ...
    patches = [
        patch("derouter.proxy.proxy_server.proxy_logging_obj", proxy_logging),
        ...
        patch(
            "derouter.proxy.pass_through_endpoints.passthrough_guardrails."
...
async def test_pass_through_request_mid_stream_upstream_drop_fires_failure_hook():
    ...
    try:
        with patch(  # test-quality-ok: proxy_logging_obj is a proxy_server module global read inside pass_through_request; there is no injection seam
            "derouter.proxy.proxy_server.proxy_logging_obj", mock_proxy_logging
        ):
            response = await pass_through_request(
                request=mock_request,
    ...
    assert request_data["derouter_call_id"]
    assert request_data["model"] == "gpt-5.6"
    assert isinstance(request_data["derouter_logging_obj"], DeRouterLoggingObj)
...
def _inject_fake_passthrough_client(transport, timeout):
    ...
    cache = derouter.in_memory_llm_clients_cache
    cache_key = next(
...
def _enter_relay_logging_mocks(stack, parsed_body):
    ...
    mock_proxy_logging = stack.enter_context(patch("derouter.proxy.proxy_server.proxy_logging_obj"))
...
async def test_pass_through_request_relays_non_json_body_without_buffering():
    ...
    handler returns, upstream status/headers plus x-derouter-* headers preserved,
    ...
    try:
        with ExitStack() as stack:
            ...
            assert "x-derouter-call-id" in response.headers
...
def _recording_success_callback():
    ...
    original = derouter._async_success_callback
    derouter._async_success_callback = [*original, recorder]
    try:
        yield recorder
    finally:
        derouter._async_success_callback = original
...
async def test_passthrough_records_cost_and_tokens_reported_by_upstream():
    ...
    x-derouter-response-cost / x-derouter-total-tokens. DeRouter must record those
...
async def test_passthrough_records_zero_when_upstream_reports_zero():
    payloads, _ = await _run_upstream_reporting_passthrough(
        {"x-derouter-response-cost": "0", "x-derouter-total-tokens": "0"}
    )
...
async def test_passthrough_records_upstream_reported_cost_on_error_response():
    ...
    _, mock_proxy_logging = await _run_upstream_reporting_passthrough(
        {
            "x-derouter-response-cost": "0.00021",
            "x-derouter-total-tokens": "930",
        },
        status_code=500,
    )
    ...
    assert request_data["combined_usage_object"] == derouter.Usage(total_tokens=930)
    ...
    fake_client, cleanup = _inject_fake_passthrough_client(
        _FakeUpstreamTransport(
            ...
            headers={
                ...
                "x-derouter-response-cost": "0.00312",
                "x-derouter-total-tokens": "4021",
            },
...
async def test_unusable_upstream_cost_records_zero_not_the_flat_estimate():
    ...
    """
    payloads, _ = await _run_upstream_reporting_passthrough(
        {"x-derouter-response-cost": "not-a-number", "x-derouter-total-tokens": "1874"},
...
async def test_websocket_passthrough_forwards_non_ascii_first_frame():
    ...
    with (
        ...
        patch(
            "derouter.proxy.pass_through_endpoints.pass_through_endpoints.connect",
            return_value=FakeUpstreamConnect(upstream_ws),
        ),
        patch("derouter.proxy.pass_through_endpoints.pass_through_endpoints.GLOBAL_LOGGING_WORKER") as mock_worker,
    ):
        mock_proxy_logging.pre_call_hook = AsyncMock(return_value={})
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "vertex_ai/gemini-live-2.5-flash"},
            }
        ]
    )
...
async def test_websocket_passthrough_rewrites_alias_of_unrecognised_upstream_model():
    ...
    llm_router.get_model_list.return_value = [
        {"model_name": "gemini-live", "derouter_params": {"model": "self-hosted-live-endpoint"}}
    ]
...
async def test_websocket_passthrough_does_not_close_twice_when_success_logging_fails():
    ...
    with (
        ...
        patch(
            "derouter.proxy.pass_through_endpoints.pass_through_endpoints."
...
def _passthrough_kwargs_for_reservation(
    ...
    return HttpPassThroughEndpointHelpers._init_kwargs_for_pass_through_endpoint(
        ...
        derouter_call_id="lit-5425-call-id",
    )
...
async def _track_cost_for_passthrough_kwargs(kwargs: dict) -> AsyncMock:
    ...
    with (
        patch("derouter.proxy.proxy_server.proxy_logging_obj") as mock_proxy_logging,
        patch(
            "derouter.proxy.proxy_server.increment_spend_counters",
            increment_spend_counters,
        ),
        patch("derouter.proxy.proxy_server.update_cache", new_callable=AsyncMock),
    ):
...
async def test_passthrough_success_reconciles_budget_reservation():
    ...
    assert kwargs["derouter_params"]["metadata"]["user_api_key_budget_reservation"] is reservation
...
async def test_passthrough_body_cannot_forge_budget_reservation():
    ...
    kwargs = _passthrough_kwargs_for_reservation(
        user_api_key_dict,
        parsed_body={"derouter_metadata": {"user_api_key_budget_reservation": forged}},
    )
    assert kwargs["derouter_params"]["metadata"]["user_api_key_budget_reservation"] is None
...
async def _drive_streaming_pass_through(upstream_content_type, chunk_delay_seconds, client_asked_for_stream=True):
    ...
    with ExitStack() as stack:
        ...
        mock_get_client = stack.enter_context(
            patch("derouter.proxy.pass_through_endpoints.pass_through_endpoints.get_async_httpx_client")
        )
...
async def test_pass_through_sse_stream_stays_silent_when_keepalive_is_unconfigured():
    with patch.object(derouter, "sse_keepalive_ping_interval_seconds", None):
        collected = await _drive_streaming_pass_through(
...
async def test_pass_through_route_pings_while_the_upstream_call_is_still_running(configured_interval, expect_ping):
    ...
    module = "derouter.proxy.pass_through_endpoints.pass_through_endpoints"
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(patch.object(derouter, "sse_keepalive_ping_interval_seconds", configured_interval))
...
def test_passthrough_carries_the_per_model_budgets():
    ...
    add_derouter_data_to_request. Without these three keys the post-call increment
    ...
    metadata = kwargs["derouter_params"]["metadata"]
...
def _marked_pass_through_endpoint():
    ...
    setattr(_endpoint, DEROUTER_PASS_THROUGH_ENDPOINT_MARKER, True)  # noqa: B010  # name is a module constant
    return _endpoint
...
def test_user_defined_passthrough_is_neither_tracked_nor_enforced():
    ...
    model rather than a DeRouter-managed one, and enforcing key/team allowlists
    ...
    scoped to a DeRouter model that merely shares the name. Tracking and
...
def test_builtin_provider_routes_do_not_carry_the_user_defined_marker(handler_name):
    ...
    """
    from derouter.proxy.pass_through_endpoints import llm_passthrough_endpoints
    from derouter.types.passthrough_endpoints.pass_through_endpoints import (
        DEROUTER_PASS_THROUGH_ENDPOINT_MARKER,
    )

    handler = getattr(llm_passthrough_endpoints, handler_name)
    assert getattr(handler, DEROUTER_PASS_THROUGH_ENDPOINT_MARKER, False) is False, (
...
def test_the_marker_check_distinguishes_the_two_route_kinds():
    ...
    from derouter.proxy.auth.auth_utils import request_dispatched_to_pass_through_endpoint
    from derouter.proxy.pass_through_endpoints import llm_passthrough_endpoints
...
async def _drive_passthrough_request_and_capture_logging(
    user_api_key_dict: UserAPIKeyAuth,
    on_pre_call: Callable[[DeRouterLoggingObj | None], None] | None = None,
) -> tuple[int, DeRouterLoggingObj | None]:
    import derouter
    from derouter.llms.custom_httpx.http_handler import get_async_httpx_client
    from derouter.types.llms.custom_http import httpxSpecialProvider
    ...
    async def capture_pre_call_hook(user_api_key_dict, data, call_type):
        ...
        if on_pre_call is not None:
            on_pre_call(data.get("derouter_logging_obj"))
        return data
    ...
    return response.status_code, captured_data.get("derouter_logging_obj")
...
async def test_pass_through_request_wires_team_callbacks():
    ...
    same way add_derouter_data_to_request does for normal LLM routes."""
    user_api_key_dict = UserAPIKeyAuth(
        api_key="test-key",
        team_id="test-team",
        team_metadata={
...
async def test_pass_through_request_leaves_guardrail_readable_metadata():
    ...
    from derouter.proxy.guardrails.guardrail_hooks.hiddenlayer.hiddenlayer import (
    ...
    def read_headers_the_way_a_guardrail_does(logging_obj: DeRouterLoggingObj | None) -> None:
        assert logging_obj is not None
        try:
...
@pytest.mark.parametrize("client_metadata_key", ["derouter_metadata", "metadata"])
def test_passthrough_client_cannot_forge_session_id_omission(client_metadata_key: str):
    ...
    from derouter.constants import SESSION_ID_OMITTED_METADATA_KEY
    ...
    kwargs = HttpPassThroughEndpointHelpers._init_kwargs_for_pass_through_endpoint(
        ...
        derouter_call_id="lit-6694-call-id",
    )
...
async def test_chat_completion_pass_through_endpoint_answers_an_openai_typed_error_for_an_unknown_model(
    ...
    async def fake_add_derouter_data_to_request(**kwargs: object) -> object:
        return kwargs["data"]

    monkeypatch.setattr("derouter.proxy.proxy_server.proxy_logging_obj", proxy_logging)
    monkeypatch.setattr("derouter.proxy.proxy_server.add_derouter_data_to_request", fake_add_derouter_data_to_request)
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", None)
    monkeypatch.setattr("derouter.proxy.proxy_server.user_model", None)
