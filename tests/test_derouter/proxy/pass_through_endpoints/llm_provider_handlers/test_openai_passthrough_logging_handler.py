...
from derouter.proxy.pass_through_endpoints.llm_provider_handlers.openai_passthrough_logging_handler import (
...
class TestOpenAIPassthroughLoggingHandler:
    ...
    def _create_mock_logging_obj(self) -> DeRouterLoggingObj:
    ...
    @patch("derouter.derouter_core_utils.derouter_logging.get_standard_logging_object_payload")
    ...
    def test_openai_passthrough_handler_with_user_tracking(self, mock_get_standard_logging, mock_completion_cost):
        ...
        # Verify user information is included in derouter_params
        assert "derouter_params" in result["kwargs"]
        assert "proxy_server_request" in result["kwargs"]["derouter_params"]
        assert "body" in result["kwargs"]["derouter_params"]["proxy_server_request"]
        assert result["kwargs"]["derouter_params"]["proxy_server_request"]["body"]["user"] == "test_user_123"
    ...
    def test_build_complete_streaming_response(self):
        ...
        result = self.handler._build_complete_streaming_response(
            ...
            derouter_logging_obj=self._create_mock_logging_obj(),
            model="gpt-4o",
        )
    ...
    @patch("derouter.completion_cost", return_value=3.3e-06)
    ...
    @patch(f"{OpenAIPassthroughLoggingHandler.__module__}.get_standard_logging_object_payload")
    @patch("derouter.completion_cost", return_value=2.1e-06)
    ...
    @patch(f"{OpenAIPassthroughLoggingHandler.__module__}.get_standard_logging_object_payload")
    @patch("derouter.completion_cost", return_value=1.4e-06)
    ...
    def test_azure_passthrough_tags_metadata_model_provider(self, mock_get_standard_logging, mock_completion_cost):
        ...
        # Set up kwargs with existing derouter_params containing metadata tags
        kwargs = {
            "passthrough_logging_payload": passthrough_payload,
            "model": "gpt-4o",
        ...
        # Verify metadata tags are preserved in derouter_params
        assert "derouter_params" in result["kwargs"]
        assert "metadata" in result["kwargs"]["derouter_params"]
        assert "tags" in result["kwargs"]["derouter_params"]["metadata"]
        assert result["kwargs"]["derouter_params"]["metadata"]["tags"] == [
            "production",
            "azure-deployment",
        ]
        assert result["kwargs"]["derouter_params"]["metadata"]["user_id"] == "user_123"
    ...
    @patch("derouter.completion_cost")
    @patch("derouter.derouter_core_utils.derouter_logging.get_standard_logging_object_payload")
    @patch("derouter.llms.openai.responses.transformation.OpenAIResponsesAPIConfig.transform_response_api_response")
...
class TestOpenAIPassthroughIntegration:
    ...
    def test_is_supported_openai_endpoint_includes_responses_api(self):
        ...
        `DeRouter_SpendLogs` with zero tokens / zero spend even though the
    ...
    @patch(
        "derouter.proxy.pass_through_endpoints.llm_provider_handlers.openai_passthrough_logging_handler.OpenAIPassthroughLoggingHandler.passthrough_chat_handler"
    )
    @patch("derouter.completion_cost")
    ...
    @patch(
        "derouter.proxy.pass_through_endpoints.llm_provider_handlers.openai_passthrough_logging_handler.OpenAIPassthroughLoggingHandler.openai_passthrough_handler"
    )
    @pytest.mark.asyncio
    ...
    @patch("derouter.cost_calculator.default_image_cost_calculator")
...
class TestOpenAIPassthroughResponsesStreamingSpendLog:
    ...
    def setup_method(self):
        self.start_time = datetime.now()
        self.end_time = datetime.now()
        rates = derouter.model_cost[self.MODEL_MAP_KEY]
        self.expected_spend = (
    ...
    def _logging_obj(self) -> DeRouterLoggingObj:
        logging_obj = DeRouterLoggingObj(
            ...
            call_type="pass_through_endpoint",
            start_time=self.start_time,
            derouter_call_id="323dfe4f-2741-4473-b6e2-000000000000",
            function_id="1234",
        )
        logging_obj.model_call_details["custom_llm_provider"] = "openai"
        return logging_obj
...
class TestOpenAIPassthroughEmbeddingsSpendLog:
    ...
    def setup_method(self):
        self.start_time = datetime.now()
        self.end_time = datetime.now()
        self.expected_spend = self.PROMPT_TOKENS * derouter.model_cost[self.MODEL]["input_cost_per_token"]
        self.response_body = {
            "object": "list",
