...
class TestAzureAnthropicCostCalculation:
    ...
    @patch("derouter.completion_cost")
    ...
    @patch("derouter.completion_cost")
    def test_cost_calculation_resolves_unknown_model_from_derouter_params(
        self, mock_completion_cost
    ):
        """When the body model is the "unknown" sentinel, the deployment model
        from derouter_params must be used for costing, not "unknown" (which makes
    ...
    def test_cost_calculation_resolves_unknown_model_from_model_group(
        ...
        """With only model_group available (no deployment derouter_params.model),
    ...
    @patch("derouter.completion_cost")
    def test_cost_calculation_skips_unknown_derouter_params_model_for_model_group(
        self, mock_completion_cost
    ):
        """When derouter_params.model is itself the "unknown" sentinel, the
    ...
    def test_streaming_cost_calculation_resolves_model_from_message_start_chunk(
        ...
        """On the bare /anthropic passthrough path derouter_params carries no model
    ...
    def test_passthrough_logging_sets_response_cost_with_server_tool_use_dict(self):
        ...
        kwargs = AnthropicPassthroughLoggingHandler._create_anthropic_response_logging_payload(
            derouter_model_response=response,
            model="claude-3-7-sonnet-20250219",
            kwargs={},
            start_time=datetime.now(),
            end_time=datetime.now(),
...
class TestAnthropicBatchPassthroughCostTracking:
    ...
    def mock_logging_obj(self):
        """Mock logging object"""
        mock = MagicMock()
        mock.derouter_call_id = "test-call-id-123"
    ...
    @patch(
        "derouter.proxy.pass_through_endpoints.llm_provider_handlers.anthropic_passthrough_logging_handler.AnthropicPassthroughLoggingHandler._store_batch_managed_object"
    )
    @patch(
        "derouter.proxy.pass_through_endpoints.llm_provider_handlers.anthropic_passthrough_logging_handler.AnthropicPassthroughLoggingHandler.get_actual_model_id_from_router"
    )
    @patch("derouter.llms.anthropic.batches.transformation.AnthropicBatchesConfig")
    def test_batch_creation_handler_success(
        ...
        mock_batch_response = DeRouterBatch(
            id="msgbatch_01Wj7gkQk7gn4MpAKR8ZEDU2",
            object="batch",
    ...
    def test_store_batch_managed_object_propagates_user_identity_from_metadata(
        ...
        request's derouter_params.metadata, not the (always-empty) top-level
        kwargs lookup. Falls back to "default-user" only when metadata is
        absent."""
        mock_managed_files_hook = MagicMock()
        with (
            patch("derouter.proxy.proxy_server.proxy_logging_obj") as mock_pl,
            patch(
                "derouter.proxy.pass_through_endpoints.llm_provider_handlers.anthropic_passthrough_logging_handler.verbose_proxy_logger"
            ),
        ):
    ...
    async def _store_with_metadata(self, mock_logging_obj, metadata):
        ...
        with (
            patch("derouter.proxy.proxy_server.proxy_logging_obj") as mock_pl,
            patch(
                "derouter.proxy.pass_through_endpoints.llm_provider_handlers.batch_attribution.verbose_proxy_logger"
            ),
        ):
            mock_pl.get_proxy_hook.return_value = mock_managed_files_hook
            AnthropicPassthroughLoggingHandler._store_batch_managed_object(
    ...
    async def test_failed_create_write_is_reported_not_swallowed(self, mock_logging_obj):
        ...
        with (
            ...
            AnthropicPassthroughLoggingHandler._store_batch_managed_object(
                ...
                model_object_id="b1",
                logging_obj=mock_logging_obj,
                derouter_params={"metadata": {"user_api_key": "hashed-key-a"}},
            )
            await _drain_tasks()
...
class TestPureTextFastPathParity:
    ...
    def _assert_parity(self, raw_frames):
        ...
        k_legacy = AnthropicPassthroughLoggingHandler._create_anthropic_response_logging_payload(
            derouter_model_response=legacy,
            model="claude-3-5-sonnet-20241022",
            kwargs={},
            start_time=start,
            end_time=end,
...
class TestInterruptedStreamOutputTokenRecovery:
    ...
    def test_interrupted_stream_retokenizes_buffered_output(self):
        ...
        expected = derouter.token_counter(
            model=self._MODEL,
...
class TestStreamFalseDeduplication:
    ...
    @staticmethod
    def _make_logging_obj(stream: bool = False) -> DeRouterLoggingObj:
...
class TestNonStreamingResponseRedaction:
    ...
    @staticmethod
    def _make_logging_obj(stream: bool) -> DeRouterLoggingObj:
        logging_obj = DeRouterLoggingObj(
            model="claude-3-5-sonnet-20241022",
            messages=[{"role": "user", "content": "hello"}],
            stream=stream,
    ...
    def test_non_streaming_response_is_redacted_when_message_logging_off(self):
        ...
        logging_obj.model_call_details["derouter_params"] = {
            "metadata": {"headers": {"x-derouter-enable-message-redaction": True}}
        }
...
class TestAnthropicUsageOnlyFallback:
    ...
    def _real_logging_obj(self):
        from derouter.derouter_core_utils.derouter_logging import Logging as RealLoggingObj
    ...
    def test_handler_falls_back_when_assembly_raises(self, mock_assemble, mock_cost):
        ...
        mock_assemble.side_effect = derouter.APIError(
            status_code=500,
            message="boom",
...
class TestAnthropicPassthroughFastMode:
    ...
    def _logging_obj(self) -> DeRouterLoggingObj:
        return DeRouterLoggingObj(
            model=self.MODEL,
            messages=[],
            stream=True,
            call_type="pass_through_endpoint",
            start_time=datetime.now(),
            derouter_call_id="fast-mode",
    ...
    def _cost(self, response) -> float:
        ...
        return derouter.completion_cost(completion_response=response, model=f"anthropic/{self.MODEL}")
...
class TestRecordPartialUsageForFailure:
    ...
    @staticmethod
    def _make_logging_obj() -> DeRouterLoggingObj:
        return DeRouterLoggingObj(
            ...
            start_time=datetime.now(),
            derouter_call_id="test-partial-usage-failure",
