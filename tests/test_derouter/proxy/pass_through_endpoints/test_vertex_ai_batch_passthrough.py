...
class TestVertexAIBatchPassthroughHandler:
    ...
    def mock_httpx_response(self):
        ...
        mock_response.json.return_value = {
            ...
            "displayName": "derouter-vertex-batch-test",
    ...
    def test_batch_prediction_jobs_handler_success(
        ...
        with patch(
            "derouter.proxy.pass_through_endpoints.llm_provider_handlers.vertex_passthrough_logging_handler.verbose_proxy_logger"
        ) as mock_logger:
            with patch(
                "derouter.proxy.pass_through_endpoints.llm_provider_handlers.vertex_passthrough_logging_handler.VertexPassthroughLoggingHandler.get_actual_model_id_from_router"
            ) as mock_get_model_id:
                with patch(
                    "derouter.proxy.pass_through_endpoints.llm_provider_handlers.vertex_passthrough_logging_handler.VertexPassthroughLoggingHandler._store_batch_managed_object"
                ) as mock_store:
                    with patch(
                        "derouter.llms.vertex_ai.batches.transformation.VertexAIBatchTransformation"
    ...
    def test_get_actual_model_id_from_router_with_router(self):
        ...
        with patch("derouter.proxy.proxy_server.llm_router") as mock_router:
            with patch(
                "derouter.proxy.pass_through_endpoints.llm_provider_handlers.vertex_passthrough_logging_handler.VertexPassthroughLoggingHandler.extract_model_name_from_vertex_path"
            ) as mock_extract:
    ...
    def test_unified_object_id_generation(self):
        ...
        unified_id_string = (
            SpecialEnums.DEROUTER_MANAGED_BATCH_COMPLETE_STR.value.format(
                model_id, batch_id
            )
        )
        expected_unified_id = (
    ...
    @pytest.mark.parametrize(
        "metadata, expected",
        [
            # a request that sent its own tags (x-derouter-tags header or body metadata)
    ...
    def test_batch_cost_calculation_integration(self):
        ...
        from derouter.batches.batch_utils import calculate_vertex_ai_batch_cost_and_usage
...
class TestVertexAIBatchCostCalculation:
    ...
    async def test_openai_shaped_output_records_nonzero_cost_and_usage(self):
        ...
        """
        import derouter
        from derouter.batches.batch_utils import calculate_batch_cost_and_usage
        ...
        original_flag = getattr(
            derouter, "disable_vertex_batch_output_transformation", False
        )
        try:
            derouter.disable_vertex_batch_output_transformation = False
            ...
            derouter.disable_vertex_batch_output_transformation = original_flag
    ...
    async def test_raw_vertex_output_still_works_when_transformation_disabled(self):
        ...
        try:
            derouter.disable_vertex_batch_output_transformation = True
