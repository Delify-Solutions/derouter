...
async def test_chunk_processor_routes_logging_through_logging_worker():
    ...
    with (
        ...
        async for chunk in PassThroughStreamingHandler.chunk_processor(
            response=response,
            request_body={"model": "claude-3-haiku"},
            derouter_logging_obj=_unarmed_logging_obj(),
            endpoint_type=EndpointType.GENERIC,
            start_time=datetime.now(),
...
async def test_chunk_processor_stamps_completion_start_time_on_cost_injection_path():
    ...
    import derouter as derouter_mod
    ...
    original = getattr(derouter_mod, "include_cost_in_streaming_usage", False)
    derouter_mod.include_cost_in_streaming_usage = True
    try:
        with patch.object(
            PassThroughStreamingHandler,
            "_route_streaming_logging_to_handler",
        ...
        derouter_mod.include_cost_in_streaming_usage = original
...
async def test_chunk_processor_injects_cost_into_openai_passthrough_usage_frame(monkeypatch):
    ...
    pricing = derouter.model_cost["gpt-4o-mini"]
...
async def test_chunk_processor_logs_failure_not_success_on_mid_stream_exception():
    ...
    logging_obj = DeRouterLoggingObj(
        ...
        start_time=datetime.now(),
        derouter_call_id="test-mid-stream-timeout",
...
async def test_chunk_processor_bills_partial_google_usage_on_mid_stream_exception(endpoint_type, url_route):
    ...
    logging_obj = DeRouterLoggingObj(
        ...
        start_time=datetime.now(),
        derouter_call_id=f"test-google-mid-stream-timeout-{endpoint_type.value}",
