...
import derouter
from derouter.proxy._types import AddTeamCallback, ProxyException, TeamCallbackMetadata, UserAPIKeyAuth
from derouter.proxy.derouter_pre_call_utils import (
    KeyAndTeamLoggingSettings,
    DeRouterProxyRequestSetup,
    ...
    add_derouter_data_to_request,
    add_provider_specific_headers_to_request,
    check_if_token_is_service_account,
    clean_headers,
)
from derouter.derouter_core_utils.core_helpers import get_derouter_metadata_from_kwargs
from derouter.derouter_core_utils.internal_call_metadata import MODEL_ACCESS_GROUP_METADATA_KEY
from derouter.derouter_core_utils.get_provider_specific_headers import (
    ProviderSpecificHeaderUtils,
)
from derouter.derouter_core_utils.initialize_dynamic_callback_params import (
    TRUSTED_CALLBACK_VARS_FIELD,
)
from derouter.constants import SESSION_ID_GENERATED_METADATA_KEY, SESSION_ID_OMITTED_METADATA_KEY
from derouter.llms.bedrock.base_aws_llm import BaseAWSLLM
from derouter.llms.fireworks_ai.common_utils import get_fireworks_session_id
...
class TestGetMetadataVariableName:
    ...
    def test_returns_derouter_metadata_for_thread_routes(self):
        request = self._make_request("/v1/threads/thread_123/messages")
        assert _get_metadata_variable_name(request) == "derouter_metadata"

    def test_returns_derouter_metadata_for_assistant_routes(self):
        request = self._make_request("/v1/assistants/asst_123")
        assert _get_metadata_variable_name(request) == "derouter_metadata"

    def test_returns_derouter_metadata_for_batches_route(self):
        request = self._make_request("/v1/batches")
        assert _get_metadata_variable_name(request) == "derouter_metadata"

    def test_returns_derouter_metadata_for_messages_route(self):
        request = self._make_request("/v1/messages")
        assert _get_metadata_variable_name(request) == "derouter_metadata"

    def test_returns_derouter_metadata_for_files_route(self):
    ...
    def test_returns_derouter_metadata_for_bedrock_invoke(self):
        # GH#30629: bedrock passthrough must use derouter_metadata
    ...
    def test_returns_derouter_metadata_for_bedrock_converse(self):
...
def test_get_enforced_params(general_settings, user_api_key_dict, expected_enforced_params):
    from derouter.proxy.derouter_pre_call_utils import _get_enforced_params
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_parses_string_metadata():
    ...
    # Call
    updated_data = await add_derouter_data_to_request(
        data=data,
    ...
    # Assert
    derouter_metadata = updated_data.get("metadata", {})
    assert isinstance(derouter_metadata, dict)
...
@pytest.mark.asyncio
async def test_arrival_time_prefers_derouter_received_at_over_time_time():
    ...
    queue-time window. request.state.derouter_received_at (stamped at the top of
    ...
    request_mock.state = SimpleNamespace(derouter_received_at=received_at)
...
@pytest.mark.asyncio
async def test_arrival_time_falls_back_to_time_time_without_derouter_received_at():
    ...
    request_mock.client = MagicMock()
    request_mock.client.host = "127.0.0.1"
    request_mock.state = SimpleNamespace()  # no derouter_received_at attribute
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_strips_admin_injection_slots():
    ...
    metadata key (e.g. derouter_metadata while the proxy writes to metadata).
    """
    from derouter.proxy.derouter_pre_call_utils import add_derouter_data_to_request
    ...
    updated = await add_derouter_data_to_request(
        data=data,
        request=request_mock,
        user_api_key_dict=user_api_key_dict,
        proxy_config=MagicMock(),
    ...
    other = updated.get("derouter_metadata") or {}
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_strips_all_user_api_key_prefix_keys():
    ...
    data = {
        ...
        "derouter_metadata": {**attacker_injected},
    }
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_string_metadata_does_not_crash():
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("derouter_metadata", "abc", "a string"),
        ("metadata", 42, "an integer"),
        ("derouter_metadata", [1, 2], "an array"),
        ("metadata", True, "a boolean"),
    ],
)
async def test_add_derouter_data_to_request_rejects_non_object_metadata(field, value, received_type):
    """Regression for https://github.com/Delify-Solutions/derouter/issues/37147: a
    non-object metadata was silently dropped with a 200, and a non-object
    derouter_metadata crashed later with a 500 ('str' object has no attribute
    ...
    with pytest.raises(ProxyException) as exc_info:
        await add_derouter_data_to_request(
            data=data,
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_removes_every_invalid_metadata_field_before_raising():
    ...
    data = {"input_file_id": "file-abc", "metadata": "abc", "derouter_metadata": "xyz"}
    ...
    assert "derouter_metadata" not in data


@pytest.mark.asyncio
async def test_add_derouter_data_to_request_parses_json_object_string_derouter_metadata():
    data = {"input_file_id": "file-abc", "derouter_metadata": json.dumps({"cost_centre": "research"})}
    ...
    assert updated["derouter_metadata"]["cost_centre"] == "research"


@pytest.mark.asyncio
async def test_add_derouter_data_to_request_proxy_server_request_body_is_post_strip():
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_body_snapshot_excludes_secret_fields():
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_body_snapshot_excludes_proxy_server_request():
...
def test_refresh_proxy_server_request_body_snapshot_picks_up_guardrail_masking():
    ...
    add_derouter_data_to_request BEFORE guardrails (e.g. Presidio PII masking) run
    ...
    class _FakeLoggingObj:
        ...
        derouter.utils.function_setup stamps onto `data` between the initial
    ...
    data = {
        ...
        "derouter_logging_obj": _FakeLoggingObj(),
    ...
    assert "derouter_logging_obj" not in refreshed_body
    assert "123-45-6789" not in json.dumps(refreshed_body)


@pytest.mark.asyncio
async def test_add_derouter_data_to_request_strips_string_encoded_admin_injection():
    ...
    data = {
        ...
        "derouter_metadata": json.dumps(attacker_payload),
    }
    ...
    other = updated.get("derouter_metadata") or {}
    # After the strip, derouter_metadata has no admin-injection slots.
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_strips_user_control_fields():
    ...
    malicious_metadata = {
        ...
        "derouter_gateway_injected_cache": "forged-deployment-id",
    ...
    data = {
        ...
        "derouter_metadata": copy.deepcopy(malicious_metadata),
    }
    ...
    assert "derouter_gateway_injected_cache" not in updated
    ...
    stripped_keys = {
        ...
        "policy_sources",
        "routing_decision",
        "derouter_gateway_injected_cache",
        "_session_deployment_affinity_ttl",
        "internal_call_origin",
        "_guardrail_pipelines",
        "_pipeline_managed_guardrails",
    }
    assert "derouter_metadata" not in updated
...
async def test_add_derouter_data_to_request_strips_callback_control_fields(
    control_field,
):
    """``callbacks`` / ``service_callback`` / ``logger_fn`` get appended to
    the worker-wide ``derouter.{input,success,failure,_async_*,service}_callback``
    lists and ``derouter.user_logger_fn`` from inside ``function_setup`` —
    one request poisons every subsequent caller in that worker.
    ``derouter_disabled_callbacks`` is the inverse: a request-body value
    ...
    sample_values = {
        ...
        "derouter_disabled_callbacks": ["langfuse"],
...
async def test_add_derouter_data_to_request_marks_body_timeout_as_client_side(timeout_field):
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_ignores_forged_client_side_timeout():
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_allows_client_mock_response_with_admin_opt_in():
    request_mock = MagicMock(spec=Request)
    request_mock.url.path = "/v1/chat/completions"
    request_mock.url = MagicMock()
    request_mock.url.__str__.return_value = "http://localhost/v1/chat/completions"
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_strips_client_redaction_bypass_controls():
    request_mock = MagicMock(spec=Request)
    request_mock.url.path = "/v1/chat/completions"
    request_mock.url = MagicMock()
    request_mock.url.__str__.return_value = "http://localhost/v1/chat/completions"
    ...
    request_mock.headers = {
        "Content-Type": "application/json",
        "derouter-disable-message-redaction": "true",
    }
    request_mock.client = MagicMock()
    request_mock.client.host = "127.0.0.1"

    original_turn_off_message_logging = derouter.turn_off_message_logging
    ...
    try:
        updated = await add_derouter_data_to_request(
            data={
                ...
                "metadata": {
                    "headers": {"derouter-disable-message-redaction": "true"},
                    "turn_off_message_logging": False,
                },
                "derouter_metadata": json.dumps(
                    {
                        "headers": {"DeRouter-Disable-Message-Redaction": "true"},
                        "turn_off_message_logging": "false",
                    }
        ...
        derouter.turn_off_message_logging = original_turn_off_message_logging

    assert "turn_off_message_logging" not in updated
    assert "turn_off_message_logging" not in (updated.get("derouter_params") or {}).get("metadata", {})
    assert "turn_off_message_logging" not in updated["metadata"]
    assert "turn_off_message_logging" not in (updated.get("derouter_metadata") or {})
    assert "derouter-disable-message-redaction" not in {header.lower() for header in updated["metadata"]["headers"]}
    assert "derouter-disable-message-redaction" not in {
    ...
    assert "derouter-disable-message-redaction" not in {
        header.lower() for header in (updated.get("derouter_metadata") or {}).get("headers", {})
    }
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_admin_callback_vars_turn_off_message_logging_overrides_global(
    admin_metadata_kwargs,
):
    from derouter.derouter_core_utils.initialize_dynamic_callback_params import (
        initialize_standard_callback_dynamic_params,
    )
    from derouter.derouter_core_utils.redact_messages import should_redact_message_logging
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_admin_callback_vars_turn_off_message_logging_enables_redaction_when_global_off(
    admin_metadata_kwargs,
):
    from derouter.derouter_core_utils.initialize_dynamic_callback_params import (
        initialize_standard_callback_dynamic_params,
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_allows_redaction_opt_out_with_admin_opt_in(
    auth_kwargs,
):
    request_mock = MagicMock(spec=Request)
    request_mock.url.path = "/v1/chat/completions"
    ...
    try:
        updated = await add_derouter_data_to_request(
            data={
                ...
                "derouter_metadata": json.dumps({"headers": {"DeRouter-Disable-Message-Redaction": "true"}}),
            },
            request=request_mock,
    ...
    assert "derouter-disable-message-redaction" in {header.lower() for header in updated["metadata"]["headers"]}
    assert "derouter-disable-message-redaction" in {
        header.lower() for header in updated["metadata"]["requester_metadata"].get("headers", {})
    }
    assert "derouter-disable-message-redaction" in {
        header.lower() for header in updated["proxy_server_request"]["headers"]
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_honors_header_tags():
    ...
    request_mock.headers = {
        "Content-Type": "application/json",
        "x-derouter-tags": "production,ab-test",
    }
    request_mock.client = MagicMock()
    request_mock.client.host = "127.0.0.1"
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_preserves_caller_metadata_tags():
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_unions_caller_header_tags_with_static_key_tags():
    """Caller-supplied `x-derouter-tags` must union with static key-level
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_unions_caller_header_tags_with_static_team_tags():
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_unions_dedups_overlapping_caller_and_static_tags():
    ...
    request_mock.headers = {
        "Content-Type": "application/json",
        "x-derouter-tags": "env:prod,tenant:7",
    }
    request_mock.client = MagicMock()
    request_mock.client.host = "127.0.0.1"
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_user_spend_and_budget():
    from derouter.proxy.derouter_pre_call_utils import add_derouter_data_to_request
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_audio_transcription_multipart():
    from derouter.proxy.derouter_pre_call_utils import add_derouter_data_to_request
    ...
    derouter_metadata = updated_data.get("derouter_metadata", {})
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_disabled_callbacks():
    """
    Test that derouter_disabled_callbacks from key metadata is properly added to the request data.
    """
    from derouter.proxy.derouter_pre_call_utils import add_derouter_data_to_request
    ...
    user_api_key_dict = UserAPIKeyAuth(
        ...
        user_id="test_user_id",
        org_id="test_org_id",
        metadata={"derouter_disabled_callbacks": ["langfuse", "langsmith", "datadog"]},
    )
    ...
    # Call add_derouter_data_to_request
    result = await add_derouter_data_to_request(
        data=data,
        request=request_mock,
        user_api_key_dict=user_api_key_dict,
    ...
    # Verify that derouter_disabled_callbacks was added to the request data
    assert "derouter_disabled_callbacks" in result
    assert result["derouter_disabled_callbacks"] == ["langfuse", "langsmith", "datadog"]
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_disabled_callbacks_empty():
    """
    Test that derouter_disabled_callbacks is not added when it's empty.
    """
    from derouter.proxy.derouter_pre_call_utils import add_derouter_data_to_request
    ...
    user_api_key_dict = UserAPIKeyAuth(
        ...
        user_id="test_user_id",
        org_id="test_org_id",
        metadata={"derouter_disabled_callbacks": []},
    )
    ...
    # Verify that derouter_disabled_callbacks is not added when empty
    assert "derouter_disabled_callbacks" not in result
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_disabled_callbacks_not_present():
    """
    Test that derouter_disabled_callbacks is not added when it's not present in metadata.
    """
    from derouter.proxy.derouter_pre_call_utils import add_derouter_data_to_request
    ...
    user_api_key_dict = UserAPIKeyAuth(
        ...
        user_id="test_user_id",
        org_id="test_org_id",
        metadata={},  # No derouter_disabled_callbacks
    )
    ...
    # Verify that derouter_disabled_callbacks is not added when not present
    assert "derouter_disabled_callbacks" not in result
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_disabled_callbacks_invalid_type():
    """
    Test that derouter_disabled_callbacks is not added when it's not a list.
    """
    from derouter.proxy.derouter_pre_call_utils import add_derouter_data_to_request
    ...
    user_api_key_dict = UserAPIKeyAuth(
        ...
        user_id="test_user_id",
        org_id="test_org_id",
        metadata={"derouter_disabled_callbacks": "not_a_list"},  # Should be a list
    )
    ...
    # Verify that derouter_disabled_callbacks is not added when invalid type
    assert "derouter_disabled_callbacks" not in result
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_disabled_callbacks_with_logging_settings():
    """
    Test that derouter_disabled_callbacks works correctly alongside logging settings.
    """
    from derouter.proxy.derouter_pre_call_utils import add_derouter_data_to_request
    ...
    user_api_key_dict = UserAPIKeyAuth(
        ...
        metadata={
            ...
            "derouter_disabled_callbacks": ["langsmith", "datadog"],
        },
    )
    ...
    assert "derouter_disabled_callbacks" in result
    assert result["derouter_disabled_callbacks"] == ["langsmith", "datadog"]
...
def test_key_dynamic_logging_settings_decrypts_callback_vars(monkeypatch):
    ...
    from derouter.proxy.common_utils.callback_utils import encrypt_callback_vars
...
def test_get_num_retries_from_request():
    """
    Test DeRouterProxyRequestSetup._get_num_retries_from_request method
    """
    # Test case 1: Header is present with valid integer string
    headers_with_retries = {"x-derouter-num-retries": "3"}
    result = DeRouterProxyRequestSetup._get_num_retries_from_request(headers_with_retries)
    assert result == 3

    # Test case 2: Header is not present
    headers_without_retries = {"Content-Type": "application/json"}
    result = DeRouterProxyRequestSetup._get_num_retries_from_request(headers_without_retries)
    assert result is None

    # Test case 3: Empty headers dictionary
    empty_headers = {}
    result = DeRouterProxyRequestSetup._get_num_retries_from_request(empty_headers)
    assert result is None

    # Test case 4: Header present with zero value
    headers_with_zero = {"x-derouter-num-retries": "0"}
    result = DeRouterProxyRequestSetup._get_num_retries_from_request(headers_with_zero)
    assert result == 0

    # Test case 5: Header present with large number
    headers_with_large_number = {"x-derouter-num-retries": "100"}
    result = DeRouterProxyRequestSetup._get_num_retries_from_request(headers_with_large_number)
    assert result == 100

    # Test case 6: Multiple headers with num retries header
    headers_multiple = {
        "Content-Type": "application/json",
        "x-derouter-num-retries": "5",
        "Authorization": "Bearer token",
    }
    result = DeRouterProxyRequestSetup._get_num_retries_from_request(headers_multiple)
    assert result == 5

    # Test case 7: Header present with invalid value (should raise ValueError when int() is called)
    headers_with_invalid = {"x-derouter-num-retries": "invalid"}
    with pytest.raises(ValueError, match="invalid literal for int\\(\\) with base"):
        DeRouterProxyRequestSetup._get_num_retries_from_request(headers_with_invalid)

    # Test case 8: Header present with float string (should raise ValueError when int() is called)
    headers_with_float = {"x-derouter-num-retries": "3.5"}
    with pytest.raises(ValueError, match="invalid literal for int\\(\\) with base"):
        DeRouterProxyRequestSetup._get_num_retries_from_request(headers_with_float)

    # Test case 9: Header present with negative number
    headers_with_negative = {"x-derouter-num-retries": "-1"}
    result = DeRouterProxyRequestSetup._get_num_retries_from_request(headers_with_negative)
    assert result == -1


def test_get_keepalive_seconds_from_request():
    """
    Test DeRouterProxyRequestSetup._get_keepalive_seconds_from_request method
    """
    # Header present with valid float string
    headers_with_keepalive = {"x-derouter-keepalive-seconds": "15"}
    result = DeRouterProxyRequestSetup._get_keepalive_seconds_from_request(headers_with_keepalive)
    assert result == 15.0

    # Header not present
    result = DeRouterProxyRequestSetup._get_keepalive_seconds_from_request({"Content-Type": "application/json"})
    assert result is None

    # Empty headers dictionary
    result = DeRouterProxyRequestSetup._get_keepalive_seconds_from_request({})
    assert result is None

    # Header present with a fractional value
    result = DeRouterProxyRequestSetup._get_keepalive_seconds_from_request({"x-derouter-keepalive-seconds": "1.5"})
    assert result == 1.5

    # Header present with invalid value raises ValueError, matching the other
    # x-derouter-* numeric header helpers (_get_timeout_from_request, etc.)
    with pytest.raises(ValueError, match="could not convert string to float: 'not-a-number"):
        DeRouterProxyRequestSetup._get_keepalive_seconds_from_request({"x-derouter-keepalive-seconds": "not-a-number"})


def test_add_derouter_data_for_backend_llm_call_merges_keepalive_seconds_header():
    """
    The x-derouter-keepalive-seconds header must be merged into the data dict
    that add_derouter_data_to_request later data.update()s onto the request body,
    the same way x-derouter-timeout/x-derouter-num-retries already are.
    """
    result = DeRouterProxyRequestSetup.add_derouter_data_for_backend_llm_call(
        headers={"x-derouter-keepalive-seconds": "20"},
        request_data={},
...
def test_add_user_api_key_auth_to_request_metadata():
    ...
    data = {
        "model": "gpt-3.5-turbo",
        "messages": [{"role": "user", "content": "Hello"}],
        "derouter_metadata": {},  # This will be the metadata variable name
    }
    ...
    # Call the function
    result = DeRouterProxyRequestSetup.add_user_api_key_auth_to_request_metadata(
        data=data,
        user_api_key_dict=user_api_key_dict,
...
def test_stamped_auth_object_carries_no_callback_credentials():
    ...
    data = {"derouter_metadata": {}}

    result = DeRouterProxyRequestSetup.add_user_api_key_auth_to_request_metadata(
        data=data,
        user_api_key_dict=user_api_key_dict,
        _metadata_variable_name="derouter_metadata",
    )

    stamped = result["derouter_metadata"]["user_api_key_auth"]
    emitted = json.dumps(
        {
...
def test_stamping_does_not_mutate_the_cached_auth_object():
    ...
    DeRouterProxyRequestSetup.add_user_api_key_auth_to_request_metadata(
        data={"derouter_metadata": {}},
        user_api_key_dict=user_api_key_dict,
        _metadata_variable_name="derouter_metadata",
    )
...
def test_management_endpoint_metadata_drops_callback_credentials():
    ...
    result = DeRouterProxyRequestSetup.add_management_endpoint_metadata_to_request_metadata(
        data=data,
    ...
    auth_metadata = result["derouter_metadata"]["user_api_key_auth_metadata"]
...
def test_add_headers_to_llm_call_by_model_group(data, model_group_settings, expected_headers_added):
    """
    Test DeRouterProxyRequestSetup.add_headers_to_llm_call_by_model_group method
    ...
    original_model_group_settings = getattr(derouter, "model_group_settings", None)
    derouter.model_group_settings = model_group_settings

    try:
        # Mock the add_headers_to_llm_call method to return expected headers
        expected_returned_headers = {
            "X-DeRouter-User": "test-user",
            "X-DeRouter-Org": "test-org",
        }
        ...
        with patch.object(
            ...
            result = DeRouterProxyRequestSetup.add_headers_to_llm_call_by_model_group(
        ...
        derouter.model_group_settings = original_model_group_settings
...
def test_add_headers_to_llm_call_by_model_group_empty_headers_returned():
    ...
    original_model_group_settings = getattr(derouter, "model_group_settings", None)
    derouter.model_group_settings = mock_settings
...
def test_add_headers_to_llm_call_by_model_group_existing_headers_in_data():
    ...
    try:
        new_headers = {"X-DeRouter-User": "test-user"}
...
from derouter.proxy.common_request_processing import ProxyBaseLLMRequestProcessing
...
@pytest.mark.asyncio
async def test_add_derouter_metadata_from_request_headers():
    """
    Test that add_derouter_metadata_from_request_headers properly adds derouter metadata from request headers,
    ...
    Relevant issue: https://github.com/Delify-Solutions/derouter/issues/14008
    """
    # Set up test logger
    derouter._turn_on_debug()
    test_logger = TestCustomLogger()
    original_callbacks = derouter.callbacks
    derouter.callbacks = [test_logger]

    try:
        # Prepare test data (ensure no streaming, add mock_response and api_key to route to derouter.acompletion)
        headers = {
            "x-derouter-spend-logs-metadata": '{"user_id": "12345", "project_id": "proj_abc", "request_type": "chat_completion", "timestamp": "2025-09-02T10:30:00Z"}'
        }
        data = {
            "model": "gpt-4",
            "messages": [{"role": "user", "content": "Hello"}],
        ...
        assert SPEND_LOGS_METADATA == dict(json.loads(headers["x-derouter-spend-logs-metadata"])), (
...
def test_add_derouter_metadata_from_request_headers_x_derouter_trace_id_sets_chain_id():
    """x-derouter-trace-id sets both metadata and top-level derouter_session_id/derouter_trace_id for call chaining."""
    headers = {"x-derouter-trace-id": "foo"}
    data = {"metadata": {}}
    DeRouterProxyRequestSetup.add_derouter_metadata_from_request_headers(
    ...
    assert data["derouter_session_id"] == "foo"
    assert data["derouter_trace_id"] == "foo"


def test_add_derouter_metadata_from_request_headers_x_derouter_session_id_sets_chain_id():
    """x-derouter-session-id sets both metadata and top-level derouter_session_id/derouter_trace_id for call chaining."""
    headers = {"x-derouter-session-id": "bar"}
    data = {"metadata": {}}
    DeRouterProxyRequestSetup.add_derouter_metadata_from_request_headers(
        headers=headers, data=data, _metadata_variable_name="metadata"
    )
    ...
    assert data["derouter_session_id"] == "bar"
    assert data["derouter_trace_id"] == "bar"


def test_add_derouter_metadata_from_request_headers_both_headers_trace_id_precedence():
    """When both x-derouter-trace-id and x-derouter-session-id are present, trace-id takes precedence for chain_id."""
    headers = {
        "x-derouter-trace-id": "trace-value",
        "x-derouter-session-id": "session-value",
    }
    data = {"metadata": {}}
    DeRouterProxyRequestSetup.add_derouter_metadata_from_request_headers(
        headers=headers, data=data, _metadata_variable_name="metadata"
    ...
    assert data["derouter_session_id"] == "trace-value"
    assert data["derouter_trace_id"] == "trace-value"


def test_add_derouter_metadata_from_request_headers_generic_session_id_header():
    """A generic x-<vendor>-session-id header is used when no explicit derouter header is set."""
    ...
    assert data["derouter_session_id"] == "e96634a3-fa28-4083-b354-55542e2dca01"
    assert data["derouter_trace_id"] == "e96634a3-fa28-4083-b354-55542e2dca01"


def test_add_derouter_metadata_from_anthropic_user_id_sets_session_id():
    ...
    assert data["metadata"]["session_id"] == "e96634a3-fa28-4083-b354-55542e2dca01"
    assert data["derouter_session_id"] == "e96634a3-fa28-4083-b354-55542e2dca01"
    assert "derouter_trace_id" not in data


def test_add_derouter_metadata_from_anthropic_user_id_dict_sets_session_id():
    data = {
        "metadata": {
            "user_id": {
                "device_id": "device",
    ...
    assert data["derouter_session_id"] == "sess_4f8c1d2a-1234"
    assert "derouter_trace_id" not in data


def test_add_derouter_metadata_from_headers_session_id_beats_anthropic_user_id():
    data = {
        "metadata": {
    ...
    DeRouterProxyRequestSetup.add_derouter_metadata_from_request_headers(
        headers={"x-derouter-session-id": "header-session-id"},
        data=data,
        _metadata_variable_name="metadata",
    )
    assert data["metadata"]["session_id"] == "header-session-id"
    assert data["derouter_session_id"] == "header-session-id"
    assert data["derouter_trace_id"] == "header-session-id"


def test_add_derouter_metadata_from_headers_session_id_beats_anthropic_user_id_dict():
    data = {
        "metadata": {
            "user_id": {
...
def test_add_derouter_metadata_from_anthropic_user_id_ignores_invalid_session_id(user_id: str):
...
def test_add_derouter_metadata_from_anthropic_user_id_dict_ignores_invalid_session_id(
    user_id: object,
):
    data = {"metadata": {"user_id": user_id}}
    DeRouterProxyRequestSetup.add_derouter_metadata_from_request_headers(
...
def test_add_derouter_metadata_from_request_headers_explicit_header_beats_generic():
    """Explicit x-derouter-trace-id wins over a generic x-*-session-id header."""
    headers = {
        "x-derouter-trace-id": "explicit-trace-id-value",
    ...
    assert data["derouter_session_id"] == "explicit-trace-id-value"
    assert data["derouter_trace_id"] == "explicit-trace-id-value"
...
def test_get_chain_id_from_headers_generic_vendor_session_id():
    ...
    from derouter.proxy.derouter_pre_call_utils import get_chain_id_from_headers
    ...
    assert (
        get_chain_id_from_headers(
            {
                "x-derouter-trace-id": "explicit-id-value",
                "x-claude-code-session-id": "e96634a3-fa28-4083-b354-55542e2dca01",
            }
        )
        == "explicit-id-value"
...
def test_is_codex_user_agent_accepts_every_first_party_originator(user_agent: str):
    ...
    from derouter.proxy.derouter_pre_call_utils import is_codex_user_agent
...
def test_add_derouter_metadata_groups_codex_turns_into_one_session():
    ...
    turns = [{"derouter_metadata": {}}, {"derouter_metadata": {}}]
    for turn in turns:
        DeRouterProxyRequestSetup.add_derouter_metadata_from_request_headers(
            headers=headers, data=turn, _metadata_variable_name="derouter_metadata"
        )

    for turn in turns:
        assert turn["derouter_session_id"] == CODEX_SESSION_UUID
        assert turn["derouter_trace_id"] == CODEX_SESSION_UUID
        assert turn["derouter_metadata"]["session_id"] == CODEX_SESSION_UUID
...
def test_add_derouter_metadata_groups_opencode_turns_into_one_session():
    ...
    for turn in turns:
        ...
        assert turn["derouter_session_id"] == OPENCODE_SESSION_ID
        assert turn["derouter_trace_id"] == OPENCODE_SESSION_ID
...
def test_trace_id_from_traceparent_valid():
    from derouter.proxy.derouter_pre_call_utils import _trace_id_from_traceparent
...
def test_session_id_from_baggage_valid():
    from derouter.proxy.derouter_pre_call_utils import _session_id_from_baggage
...
def test_add_derouter_metadata_from_request_headers_traceparent_sets_trace_id_only():
    """A bare traceparent header (no derouter-specific headers) sets derouter_trace_id
    from its trace-id component and leaves derouter_session_id unset."""
    headers = {"traceparent": "00-4bf92f3577b34da6a3ce929d0e0e4736-00f067aa0ba902b7-01"}
    data = {"metadata": {}}
    DeRouterProxyRequestSetup.add_derouter_metadata_from_request_headers(
        headers=headers, data=data, _metadata_variable_name="metadata"
    )
    assert data["derouter_trace_id"] == "4bf92f3577b34da6a3ce929d0e0e4736"
    assert data["metadata"]["trace_id"] == "4bf92f3577b34da6a3ce929d0e0e4736"
    assert "derouter_session_id" not in data


def test_add_derouter_metadata_from_request_headers_baggage_sets_session_id_only():
    """A bare baggage header (no derouter-specific headers) sets derouter_session_id
    from its session.id entry and leaves derouter_trace_id unset."""
    headers = {"baggage": "session.id=baggage-session-42,user.id=7"}
    data = {"metadata": {}}
    DeRouterProxyRequestSetup.add_derouter_metadata_from_request_headers(
        headers=headers, data=data, _metadata_variable_name="metadata"
    )
    assert data["derouter_session_id"] == "baggage-session-42"
    assert data["metadata"]["session_id"] == "baggage-session-42"
    assert "derouter_trace_id" not in data


def test_add_derouter_metadata_from_request_headers_baggage_session_id_not_logged_raw(caplog):
    ...
    data = {"metadata": {}}
    with caplog.at_level(logging.DEBUG, logger="DeRouter Proxy"):
        DeRouterProxyRequestSetup.add_derouter_metadata_from_request_headers(
            headers=headers, data=data, _metadata_variable_name="metadata"
        )
    assert data["derouter_session_id"] == poisoned
    assert not any(poisoned in record.getMessage() for record in caplog.records)


def test_add_derouter_metadata_from_request_headers_traceparent_and_baggage_together():
...
def test_add_derouter_metadata_from_request_headers_explicit_trace_id_beats_traceparent():
    """x-derouter-trace-id must win over a traceparent header carrying a
    different trace-id - explicit derouter headers are always highest priority."""
    headers = {
        "x-derouter-trace-id": "explicit-trace-id-value",
        "traceparent": "00-4bf92f3577b34da6a3ce929d0e0e4736-00f067aa0ba902b7-01",
    }
...
def test_add_derouter_metadata_from_request_headers_anthropic_metadata_beats_baggage():
...
def test_get_internal_user_header_from_mapping_returns_expected_header():
    mappings = [
        {"header_name": "X-OpenWebUI-User-Id", "derouter_user_role": "internal_user"},
        {"header_name": "X-OpenWebUI-User-Email", "derouter_user_role": "customer"},
    ]

    header_name = DeRouterProxyRequestSetup.get_internal_user_header_from_mapping(mappings)
    assert header_name == "X-OpenWebUI-User-Id"


def test_get_internal_user_header_from_mapping_none_when_absent():
    mappings = [{"header_name": "X-OpenWebUI-User-Email", "derouter_user_role": "customer"}]
    header_name = DeRouterProxyRequestSetup.get_internal_user_header_from_mapping(mappings)
    assert header_name is None

    single = {"header_name": "X-Only-Customer", "derouter_user_role": "customer"}
    header_name = DeRouterProxyRequestSetup.get_internal_user_header_from_mapping(single)
    assert header_name is None
        ...
        "user_header_mappings": [
            {
                ...
                "derouter_user_role": "internal_user",
            },
            {"header_name": "X-OpenWebUI-User-Email", "derouter_user_role": "customer"},
        ]
    }

    result = DeRouterProxyRequestSetup.add_internal_user_from_user_mapping(general_settings, user_api_key_dict, headers)
...
def test_add_internal_user_from_user_mapping_no_header_or_mapping_returns_unchanged():
    ...
    result = DeRouterProxyRequestSetup.add_internal_user_from_user_mapping(
    ...
    general_settings = {
        "user_header_mappings": [{"header_name": "X-OpenWebUI-User-Id", "derouter_user_role": "internal_user"}]
    }
    result = DeRouterProxyRequestSetup.add_internal_user_from_user_mapping(
...
def test_get_sanitized_user_information_from_key_includes_guardrails_metadata():
    ...
    result = DeRouterProxyRequestSetup.get_sanitized_user_information_from_key(user_api_key_dict=user_api_key_dict)
...
def test_user_and_team_spend_and_budget_flow_to_standard_logging_metadata():
    ...
    """
    from derouter.derouter_core_utils.derouter_logging import StandardLoggingPayloadSetup
    ...
    sanitized = DeRouterProxyRequestSetup.get_sanitized_user_information_from_key(user_api_key_dict=user_api_key_dict)
...
async def test_team_guardrails_append_to_key_guardrails():
    ...
    with patch("derouter.proxy.utils._premium_user_check"):
        updated_data = await add_derouter_data_to_request(
            data=data,
            request=request_mock,
            user_api_key_dict=user_api_key_dict,
...
async def test_request_guardrails_do_not_override_key_guardrails():
    ...
    with patch("derouter.proxy.utils._premium_user_check"):
        updated_data_empty = await add_derouter_data_to_request(
...
async def test_embedding_header_forwarding_with_model_group():
    ...
    import derouter.proxy.derouter_pre_call_utils as pre_call_utils_module

    # Reload the module to ensure it has a fresh reference to derouter
    # This is necessary because conftest.py reloads derouter at module scope,
    # which can cause the module's derouter reference to become stale
    ...
    # Use string-based patch to ensure we patch the current sys.modules['derouter']
    ...
    with patch("derouter.model_group_settings", mock_settings):
        # Call add_derouter_data_to_request which includes header forwarding logic
        updated_data = await add_derouter_data_to_request(
            data=data,
            request=request_mock,
            user_api_key_dict=user_api_key_dict,
...
async def test_add_guardrails_from_policy_engine():
    ...
    """
    from derouter.proxy.policy_engine.attachment_registry import get_attachment_registry
    from derouter.proxy.policy_engine.policy_registry import get_policy_registry
    from derouter.types.proxy.policy_engine import (
        Policy,
        PolicyAttachment,
        PolicyGuardrails,
    )
...
async def test_add_guardrails_from_policy_engine_accepts_dynamic_policies_and_pops_from_data():
    ...
    This is critical because 'policies' is a DeRouter proxy-specific parameter that should
...
async def test_add_guardrails_from_policy_engine_policy_version_by_id():
    ...
    from derouter.proxy.policy_engine.attachment_registry import get_attachment_registry
    from derouter.proxy.policy_engine.policy_registry import get_policy_registry
    from derouter.types.proxy.policy_engine import Policy, PolicyGuardrails
...
async def test_bearer_token_not_in_debug_logs():
    ...
    Calls add_derouter_data_to_request with a Bearer token in the request
    ...
    appears in any log message — covering the exact paths the client reported:
      - "Request Headers: ..."
      - "receiving data: ..."
      - "[PROXY] returned data from derouter_pre_call_utils: ..."
    """
    import logging
    ...
    logger = logging.getLogger("DeRouter Proxy")
...
def setup_test_credentials():
    """Populate derouter.credential_list with test credentials and enable feature flag, clean up after."""
    original = derouter.credential_list[:]
    original_flag = derouter.enable_model_config_credential_overrides
    derouter.enable_model_config_credential_overrides = True
    derouter.credential_list.extend(
        [
            CredentialItem(
    ...
    yield
    derouter.credential_list[:] = original
    derouter.enable_model_config_credential_overrides = original_flag
...
def test_extract_credential_from_entry_azure():
    entry = {"azure": {"derouter_credentials": "my-cred"}}
...
def test_resolve_project_model_specific_wins():
    project_config = {
        "gpt-4": {"azure": {"derouter_credentials": "proj-gpt4"}},
        "defaultconfig": {"azure": {"derouter_credentials": "proj-default"}},
    }
    team_config = {
        "gpt-4": {"azure": {"derouter_credentials": "team-gpt4"}},
        "defaultconfig": {"azure": {"derouter_credentials": "team-default"}},
    }
...
def test_resolve_model_not_in_any_config():
    project_config = {"gpt-4": {"azure": {"derouter_credentials": "x"}}}
...
def test_apply_overrides_project_model_specific(setup_test_credentials):
    ...
    user_api_key_dict = UserAPIKeyAuth(
        ...
        team_metadata={
            "model_config": {
                "defaultconfig": {"azure": {"derouter_credentials": "hotel-azure-eastus"}},
                "gpt-4": {"azure": {"derouter_credentials": "hotel-azure-westus"}},
            }
        },
        project_metadata={
            "model_config": {
                "defaultconfig": {"azure": {"derouter_credentials": "hotel-rec-azure"}},
                "gpt-4-vision": {"azure": {"derouter_credentials": "hotel-rec-vision"}},
            }
        },
    )
...
def test_apply_overrides_clientside_credentials_take_precedence(
    ...
    user_api_key_dict = UserAPIKeyAuth(
        api_key="test-key",
        team_metadata={"model_config": {"defaultconfig": {"azure": {"derouter_credentials": "hotel-azure-eastus"}}}},
    )
    _apply_credential_overrides_from_model_config(data=data, user_api_key_dict=user_api_key_dict)
...
def test_apply_overrides_missing_credential_name(setup_test_credentials):
    ...
    user_api_key_dict = UserAPIKeyAuth(
        api_key="test-key",
        team_metadata={"model_config": {"gpt-4": {"azure": {"derouter_credentials": "nonexistent-credential"}}}},
    )
    _apply_credential_overrides_from_model_config(data=data, user_api_key_dict=user_api_key_dict)
    assert "api_base" not in data
    assert "api_key" not in data
...
def test_apply_overrides_no_model_in_data(setup_test_credentials):
    ...
    user_api_key_dict = UserAPIKeyAuth(
        api_key="test-key",
        team_metadata={"model_config": {"defaultconfig": {"azure": {"derouter_credentials": "some-cred"}}}},
    )
    _apply_credential_overrides_from_model_config(data=data, user_api_key_dict=user_api_key_dict)
    assert "api_base" not in data
...
def test_apply_overrides_clientside_api_version_preserved(setup_test_credentials):
    ...
    user_api_key_dict = UserAPIKeyAuth(
        api_key="test-key",
        team_metadata={"model_config": {"gpt-4-vision": {"azure": {"derouter_credentials": "hotel-rec-vision"}}}},
    )
    _apply_credential_overrides_from_model_config(data=data, user_api_key_dict=user_api_key_dict)
...
def test_resolve_non_dict_model_config_ignored():
    ...
    result = _resolve_credential_from_model_config(
        "gpt-4",
        "invalid",
        {"gpt-4": {"azure": {"derouter_credentials": "valid-cred"}}},
    )
...
def test_resolve_post_alias_name_takes_priority():
    ...
    team_config = {
        "gpt-4": {"azure": {"derouter_credentials": "pre-alias-cred"}},
        "gpt-4o-team-1": {"azure": {"derouter_credentials": "post-alias-cred"}},
    }
    # Team scope
...
def test_apply_overrides_with_alias(setup_test_credentials):
    ...
    user_api_key_dict = UserAPIKeyAuth(
        ...
        team_metadata={
            "model_config": {
                "my-gpt4": {"azure": {"derouter_credentials": "hotel-azure-eastus"}},
            }
        },
    )
    _apply_credential_overrides_from_model_config(
...
def test_apply_overrides_feature_flag_disabled_by_default():
    ...
    assert derouter.enable_model_config_credential_overrides is False
    data = {"model": "gpt-4"}
    user_api_key_dict = UserAPIKeyAuth(
        api_key="test-key",
        team_metadata={"model_config": {"gpt-4": {"azure": {"derouter_credentials": "hotel-azure-eastus"}}}},
    )
    _apply_credential_overrides_from_model_config(data=data, user_api_key_dict=user_api_key_dict)
    assert "api_base" not in data
    assert "api_key" not in data
...
def test_extract_credential_provider_hint_prefers_exact_match():
    ...
    entry = {
        "openai": {"derouter_credentials": "openai-cred"},
        "azure": {"derouter_credentials": "azure-cred"},
    }
...
def test_clean_headers_preserves_x_api_key_when_byok_enabled():
    ...
    headers = Headers(
        {
            ...
            "x-derouter-api-key": "sk-proxy-virtual-key",
            "content-type": "application/json",
        }
    )
    ...
    result = clean_headers(
        headers=headers,
        derouter_key_header_name="x-derouter-api-key",
        forward_llm_provider_auth_headers=True,
        authenticated_with_header="x-derouter-api-key",
    )

    # x-api-key must be preserved for BYOK
    assert result.get("x-api-key") == "sk-ant-api03-client-key"
    # x-derouter-api-key must NOT leak to the upstream
    assert "x-derouter-api-key" not in result
...
def test_clean_headers_strips_x_api_key_when_byok_enabled_but_x_api_key_was_auth_header():
    ...
    # as the DeRouter auth header (anti-replay guard).
    assert "x-api-key" not in result
...
async def test_team_guardrail_merges_with_global_policy():
    ...
    The bug: get_guardrail_from_metadata checked derouter_metadata before
    metadata. When the request contained a non-empty derouter_metadata field
    ...
    from derouter.proxy.policy_engine.attachment_registry import get_attachment_registry
    from derouter.proxy.policy_engine.policy_registry import get_policy_registry
    from derouter.proxy.derouter_pre_call_utils import move_guardrails_to_metadata
    from derouter.types.proxy.policy_engine import (
        Policy,
        PolicyAttachment,
        PolicyGuardrails,
    ...
    data = {
        "model": "gpt-4",
        "messages": [{"role": "user", "content": "Hello"}],
        # Simulate a request that carries derouter_metadata (without guardrails)
        # which previously shadowed data["metadata"]["guardrails"].
        "derouter_metadata": {"some_user_field": "some_value"},
        "metadata": {},
    }
    ...
    try:
        ...
        # when derouter_metadata is present (the bug: it returned [] before fix)
        ...
        assert "team-direct-guardrail" in returned, (
            f"get_guardrail_from_metadata shadowed by derouter_metadata; got: {returned}"
        )
...
@pytest.mark.asyncio
async def test_get_guardrail_from_metadata_prefers_metadata_over_derouter_metadata():
    """
    Unit test: get_guardrail_from_metadata must read from data["metadata"] first.
    A non-empty data["derouter_metadata"] without a 'guardrails' key must not
    ...
    data = {
        ...
        "derouter_metadata": {"some_field": "some_value"},  # no 'guardrails' key
    }
...
def test_get_guardrail_from_metadata_reads_derouter_metadata_when_no_metadata():
    """
    get_guardrail_from_metadata must still read from derouter_metadata when
    ...
    data = {
        ...
        "derouter_metadata": {"guardrails": ["my-guardrail"]},
    }

    result = dummy.get_guardrail_from_metadata(data)
    assert result == ["my-guardrail"], f"Expected guardrails from derouter_metadata fallback, got: {result}"
...
class TestApplyClientTagPolicyPreAuth:
    """Tests for ``DeRouterProxyRequestSetup.apply_client_tag_policy_pre_auth``.

    Regression coverage for the bug where ``x-derouter-tags`` header was
    invisible to ``_tag_max_budget_check`` because the merge happened
    post-auth in ``add_derouter_data_to_request``.
    """

    def test_merges_header_tags_into_metadata(self):
        request_mock = _build_request_mock_with_headers({"x-derouter-tags": "tenant:acme,env:prod"})
        data = {"model": "gpt-3.5-turbo"}
        user_api_key_dict = UserAPIKeyAuth(
            api_key="hashed-key",
            metadata={},
        ...
        DeRouterProxyRequestSetup.apply_client_tag_policy_pre_auth(
            request=request_mock,
            request_data=data,
            user_api_key_dict=user_api_key_dict,
        )
    ...
    def test_preserves_body_tags(self):
        ...
        request_mock = _build_request_mock_with_headers({"x-derouter-tags": "tenant:acme"})
        data = {
            "model": "gpt-3.5-turbo",
            "tags": ["root-tag"],
            "derouter_metadata": {"tags": ["derouter-meta-tag"]},
        }
        user_api_key_dict = UserAPIKeyAuth(
            api_key="hashed-key",
            metadata={},
        ...
        # derouter_metadata is the active metadata key (it's present), so
        # header tags merge into it and union with existing tags there.
        assert data["derouter_metadata"]["tags"] == [
            "derouter-meta-tag",
            "tenant:acme",
        ]

    def test_uses_derouter_metadata_when_present(self):
        request_mock = _build_request_mock_with_headers({"x-derouter-tags": "tenant:acme"})
        data = {
            "model": "gpt-3.5-turbo",
            "derouter_metadata": {"foo": "bar"},
        }
        user_api_key_dict = UserAPIKeyAuth(
            api_key="hashed-key",
            metadata={},
        ...
        # get_metadata_variable_name_from_kwargs returns "derouter_metadata"
        # when present, so header tags should land there to be visible to
        # _tag_max_budget_check.
        assert data["derouter_metadata"]["tags"] == ["tenant:acme"]
    ...
    def test_string_metadata_tags_survive_header_merge(self):
        ...
        request_mock = _build_request_mock_with_headers({"x-derouter-tags": "free"})
        data = {
            "model": "gpt-3.5-turbo",
            "metadata": '{"tags": ["paid"]}',
        }
    ...
    async def test_string_metadata_does_not_bypass_tag_max_budget_check(self):
        ...
        be silently overwritten when an x-derouter-tags header is present."""
        from derouter.proxy._types import DeRouter_BudgetTable, DeRouter_TagTable
        from derouter.proxy.auth.auth_checks import _tag_max_budget_check
        from derouter.proxy.utils import ProxyLogging
        ...
        paid_tag = DeRouter_TagTable(
            tag_name="paid",
            spend=0.0,
            derouter_budget_table=DeRouter_BudgetTable(max_budget=0.10),
        )
        ...
        with (
            patch(
                "derouter.proxy.proxy_server.get_current_spend",
                mock_get_current_spend,
            ),
            patch(
                "derouter.proxy.auth.auth_checks.get_tag_objects_batch",
    ...
    async def test_header_tags_visible_to_tag_max_budget_check_on_metadata_route(self, route):
        """Regression: on DEROUTER_METADATA_ROUTES (bedrock, /v1/messages, ...),
        common_checks pre-seeds ``derouter_metadata`` and writes key tags there
        ...
        resolving to ``derouter_metadata``) silently ignores them. This test mirrors
        the actual auth-time call order and verifies that an over-budget
        header-supplied tag still trips ``_tag_max_budget_check``.
        """
        from derouter.proxy._types import DeRouter_BudgetTable, DeRouter_TagTable
        from derouter.proxy.auth.auth_checks import common_checks
        from derouter.proxy.utils import ProxyLogging
        ...
        DeRouterProxyRequestSetup.pre_seed_derouter_metadata_for_route(
            request_data=data,
            route=route,
        )
        DeRouterProxyRequestSetup.apply_client_tag_policy_pre_auth(
...
class TestApplyKeyTagsPreAuth:
    def test_merges_key_tags_into_metadata(self):
        ...
        DeRouterProxyRequestSetup.apply_key_tags_pre_auth(
            request_data=data,
            user_api_key_dict=user_api_key_dict,
        )
    ...
    def test_uses_derouter_metadata_when_present(self):
        ...
        assert data["derouter_metadata"]["tags"] == ["key-tag"]
        assert "tags" not in data.get("metadata", {})
...
def test_resolve_provider_from_deployment_uses_derouter_params_model():
    """When custom_llm_provider is unset, fall back to the prefix of model."""
    router = MagicMock()
    deployment = MagicMock()
    deployment.derouter_params.model = "bedrock/us.anthropic.claude-sonnet-4-6"
    deployment.derouter_params.custom_llm_provider = None
...
def test_resolve_provider_from_deployment_prefers_custom_llm_provider():
    ...
    router = MagicMock()
    deployment = MagicMock()
    deployment.derouter_params.model = "us.anthropic.claude-sonnet-4-6"
    deployment.derouter_params.custom_llm_provider = "bedrock"
    router.get_deployment_by_model_group_name.return_value = deployment
...
def test_resolve_provider_from_deployment_falls_back_to_pre_alias():
    ...
    router = MagicMock()
    deployment = MagicMock()
    deployment.derouter_params.model = "bedrock/anthropic.claude-sonnet-4-6"
    deployment.derouter_params.custom_llm_provider = None
...
def test_apply_overrides_multi_provider_default_picks_correct_provider(
    ...
    """
    derouter.credential_list.append(
        CredentialItem(
        ...
        team_metadata={
            "model_config": {
                "defaultconfig": {
                    ...
                    "gemini": {"derouter_credentials": "gemini-team-1"},
                    "bedrock": {"derouter_credentials": "bedrock-team-1"},
                }
            }
        },
    )
...
def test_apply_overrides_provider_prefix_in_model_skips_router_lookup(
    ...
    user_api_key_dict = UserAPIKeyAuth(
        ...
        team_metadata={
            "model_config": {
                "defaultconfig": {
                    "azure": {"derouter_credentials": "hotel-azure-eastus"},
                    "bedrock": {"derouter_credentials": "hotel-rec-azure"},
                }
            }
        },
    )
...
async def test_add_derouter_data_to_request_agentic_cli_drop_params(
    user_agent, request_drop_params, operator_drop_params, expected_drop_params
):
    ...
    explicit caller value, an explicit operator-level derouter_settings value,
    ...
    proxy_config = MagicMock()
    proxy_config.config = (
        {"derouter_settings": {"drop_params": operator_drop_params}}
        if operator_drop_params is not None
        else {"derouter_settings": {}}
    )
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_merges_metadata_tags_on_responses_route():
    """Regression for #31584: user-supplied metadata.tags must be merged into
    derouter_metadata.tags on /v1/responses so they reach SpendLogs.request_tags."""
    from derouter.proxy.derouter_pre_call_utils import add_derouter_data_to_request
    ...
    assert "cost-center-1" in updated["derouter_metadata"]["tags"]
    assert "team-alpha" in updated["derouter_metadata"]["tags"]


@pytest.mark.asyncio
async def test_add_derouter_data_to_request_unions_metadata_tags_with_header_tags_on_responses_route():
    """On /v1/responses, tags from metadata.tags AND x-derouter-tags header
    must both appear in derouter_metadata.tags."""
    from derouter.proxy.derouter_pre_call_utils import add_derouter_data_to_request
    ...
    tags = updated["derouter_metadata"]["tags"]
...
async def test_overwrite_user_with_key_hash_clobbers_caller_supplied_user(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "overwrite_user_with_key_hash", True)
...
async def test_overwrite_user_with_key_hash_disabled_preserves_caller_user():
    assert derouter.overwrite_user_with_key_hash is False
...
async def test_overwrite_user_with_key_hash_stamps_master_key_alias(monkeypatch):
    ...
    from derouter.constants import DEROUTER_PROXY_MASTER_KEY_ALIAS

    monkeypatch.setattr(derouter, "overwrite_user_with_key_hash", True)

    user_api_key_dict = UserAPIKeyAuth(api_key=DEROUTER_PROXY_MASTER_KEY_ALIAS)
    user_api_key_dict.via_virtual_key = True
    ...
    assert updated_data["user"] == DEROUTER_PROXY_MASTER_KEY_ALIAS
...
def test_get_sanitized_user_information_from_key_drops_callback_config():
            ...
            "logging": [
                {
                    ...
                    "callback_vars": {"langsmith_api_key": "derouter_enc::ciphertext"},
                }
            ],
            "callback_settings": {"callback_vars": {"langfuse_secret_key": "derouter_enc::other"}},
            "priority": "high",
        },
    )
    ...
    assert "derouter_enc::" not in json.dumps(auth_metadata)
...
def test_team_alias_targeting_deleted_team_deployment_keeps_requested_model(monkeypatch):
    ...
    """
    import derouter.proxy.derouter_pre_call_utils as pre_call_utils
    from derouter.proxy.derouter_pre_call_utils import _update_model_if_team_alias_exists

    monkeypatch.delenv("DEROUTER_ENABLE_TEAM_STALE_ALIAS_BYPASS", raising=False)
    ...
    with patch("derouter.proxy.proxy_server.llm_router", _MockRouter()):
...
@pytest.mark.parametrize(
    ...
    [
        ("/v1/messages", "derouter_metadata"),
        ("/v1/chat/completions", "metadata"),
    ],
)
async def test_add_derouter_data_to_request_redacts_oauth_header_from_logging_copies(path, metadata_variable_name):
    ...
    request_mock = _make_request_mock(
        path,
        {
            ...
            "x-derouter-api-key": "Bearer sk-virtual-key",
        },
    )
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_keeps_every_forwarded_credential_out_of_logging_copies():
    ...
    secrets = {
        ...
        "cookie": "derouter_jwt=session-token-lit5108",
...
def test_redact_credential_headers_classifies_each_header(header, expected_redacted):
    from derouter.proxy.derouter_pre_call_utils import redact_credential_headers
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_debug_log_does_not_print_credentials():
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_strips_caller_supplied_callback_credentials():
    """
    ...
    data = {
        ...
        **caller_destinations,
        ...
        "derouter_metadata": dict(caller_destinations),
        "derouter_params": {"metadata": dict(caller_destinations)},
    }
    ...
    assert "dd_site" not in updated["derouter_params"]["metadata"]
    assert updated["metadata"]["safe_user_metadata"] == "kept"


@pytest.mark.asyncio
async def test_add_derouter_data_to_request_strips_caller_supplied_callback_credentials_with_clientside_creds_allowed():
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_omits_trusted_callback_vars_without_team_callbacks():
...
def test_trusted_callback_vars_never_reach_the_provider():
    """
    The stamped field rides the request body, so it has to be a recognised derouter param;
    otherwise the OpenAI param builder sweeps it into extra_body and the provider 400s.
    """
    from derouter.utils import get_non_default_completion_params
...
class TestPromotedTraceControlFields:
    """LIT-5137: caller metadata trace fields must reach derouter_metadata."""
    ...
    async def _run(self, path: str, data: dict, headers: dict | None = None) -> dict:
        ...
        return await add_derouter_data_to_request(
            data=data,
            request=request,
            user_api_key_dict=UserAPIKeyAuth(api_key="hashed-key"),
            proxy_config=MagicMock(),
    ...
    def test_returns_derouter_metadata_for_responses_route(self):
        assert _get_metadata_variable_name(self._make_request("/v1/responses")) == "derouter_metadata"
    ...
    def test_existing_derouter_metadata_value_wins(self):
        promoted = _promoted_trace_control_fields(
            requester_metadata={"trace_id": "from-body", "session_id": "from-body", "trace_name": "from-body"},
            derouter_metadata={"trace_id": "from-header", "session_id": "from-header"},
        )

        assert dict(promoted) == {"trace_name": "from-body"}

    def test_empty_requester_metadata_promotes_nothing(self):
        assert _promoted_trace_control_fields(requester_metadata={}, derouter_metadata={}) == ()
    ...
    async def test_responses_route_end_to_end(self):
        ...
        derouter_metadata = updated["derouter_metadata"]
        for key, value in caller_metadata.items():
            assert derouter_metadata[key] == value
    ...
    async def test_messages_route_end_to_end(self):
        ...
        assert updated["derouter_metadata"]["trace_id"] == "msg-trace-1"
        assert updated["derouter_metadata"]["session_id"] == "msg-session-1"
    ...
    async def test_session_id_header_beats_body_metadata(self):
        updated = await self._run(
            ...
            headers={"x-derouter-session-id": "from-header-12345678"},
        )

        assert updated["derouter_metadata"]["session_id"] == "from-header-12345678"
    ...
    async def test_forged_user_api_key_fields_are_not_promoted(self):
        ...
        derouter_metadata = updated["derouter_metadata"]
        assert derouter_metadata["trace_id"] == "trace-1"
        assert derouter_metadata.get("user_api_key_user_id") != "forged"
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_inherited_tags_excludes_caller_tags():
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_inherited_tags_survives_pre_auth_header_merge():
    """Regression: apply_client_tag_policy_pre_auth (run from user_api_key_auth,
    for _tag_max_budget_check) merges the caller's x-derouter-tags header into the
    ...
    request_mock.method = "POST"
    request_mock.query_params = {}
    request_mock.headers = {"Content-Type": "application/json", "x-derouter-tags": "caller-invented-tag"}
    request_mock.client = MagicMock()
    request_mock.client.host = "127.0.0.1"
    ...
    # same data dict, before add_derouter_data_to_request is ever called.
    DeRouterProxyRequestSetup.apply_client_tag_policy_pre_auth(
        request=request_mock,
        request_data=data,
        user_api_key_dict=user_api_key_dict,
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_caller_tags_excludes_key_and_team_tags():
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_caller_tags_includes_header_tags():
    """The x-derouter-tags header is as much a caller-controlled input as the
    ...
    request_mock.method = "POST"
    request_mock.query_params = {}
    request_mock.headers = {"Content-Type": "application/json", "x-derouter-tags": "header-tag"}
    request_mock.client = MagicMock()
    request_mock.client.host = "127.0.0.1"
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_caller_tags_empty_when_caller_sends_nothing():
...
async def test_newrelic_team_callback_vars_reach_trusted_field():
    ...
    from derouter.integrations.otel.presets import dynamic_otlp_endpoint, dynamic_otlp_headers
...
def test_newrelic_vars_scoped_to_newrelic_callback_entry():
    ...
    from derouter.proxy._types import AddTeamCallback
    from derouter.proxy.derouter_pre_call_utils import convert_key_logging_metadata_to_callback
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_strips_router_reserved_stamps_from_both_buckets():
    ...
    data = {
        ...
        "derouter_metadata": dict(_PLANTED_STAMPS),
    }
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_strips_router_reserved_stamps_from_json_string_derouter_metadata():
    from derouter.proxy.derouter_pre_call_utils import add_derouter_data_to_request

    data = {
        "model": "gpt-3.5-turbo",
        "messages": [{"role": "user", "content": "hi"}],
        "derouter_metadata": json.dumps(_PLANTED_STAMPS),
    }
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_strips_router_reserved_stamps_despite_pricing_override_opt_in():
    ...
    data = {
        "model": "gpt-3.5-turbo",
        "messages": [{"role": "user", "content": "hi"}],
        "derouter_metadata": {**_PLANTED_STAMPS, "model_info": {"input_cost_per_token": 0.0}},
    }
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_strips_router_reserved_stamps_on_responses_route():
    """On the Responses family the proxy-owned bucket is derouter_metadata and the client's
    ...
    for bucket in ("metadata", "derouter_metadata"):
...
async def test_router_keeps_proxy_metadata_bucket_identity_after_reserved_stamp_strip():
    ...
    derouter_metadata made the router hand downstream a scrubbed copy, so the proxy's
    ...
    ``metadata``, since the boundary folds client ``derouter_metadata`` into it."""
    from derouter.proxy.derouter_pre_call_utils import add_derouter_data_to_request
    ...
    request_data = await add_derouter_data_to_request(
        data=data,
        request=_reserved_stamp_request("/v1/chat/completions"),
        user_api_key_dict=_reserved_stamp_key(),
        proxy_config=MagicMock(),
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "gpt-3.5-turbo", "mock_response": "hi"},
            }
        ]
    )
    forwarded_buckets = []
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_folds_derouter_metadata_into_metadata_on_chat_routes():
    data = {
        "model": "gpt-3.5-turbo",
        "metadata": {"tags": ["from-metadata"]},
        "derouter_metadata": {"trace_id": "abc", "tags": ["from-derouter-metadata"]},
    }
    ...
    assert updated["metadata"]["tags"] == ["from-metadata", "from-derouter-metadata"]


@pytest.mark.asyncio
async def test_add_derouter_data_to_request_keeps_derouter_metadata_on_derouter_metadata_routes():
    data = {"model": "claude-sonnet-5", "derouter_metadata": {"trace_id": "abc"}}
    ...
    assert updated["derouter_metadata"]["trace_id"] == "abc"
...
def _stamp_model_access_groups(matched_model_access_groups, metadata_variable_name="metadata"):
    ...
    return DeRouterProxyRequestSetup.add_user_api_key_auth_to_request_metadata(
...
def test_stamped_model_access_groups_survive_the_derouter_metadata_merge():
    """
    The key must keep its ``user_api_key`` prefix: when a request carries both metadata dicts,
    get_derouter_metadata_from_kwargs returns derouter_metadata and copies a key over from metadata
    ...
    kwargs = {
        "derouter_params": {
            ...
            "derouter_metadata": {"trace_id": "abc"},
        }
    }

    assert get_derouter_metadata_from_kwargs(kwargs)[MODEL_ACCESS_GROUP_METADATA_KEY] == ["tier-a"]
...
def _spend_log_session_id(data: dict[str, object], metadata_key: str = "metadata") -> str | None:
    """Resolve session_id the way DeRouter_SpendLogs does, reading the omit decision stamped on the request."""
    from derouter.derouter_core_utils.get_derouter_params import get_derouter_params
    from derouter.derouter_core_utils.derouter_logging import StandardLoggingPayloadSetup
    from derouter.proxy.spend_tracking.spend_tracking_utils import _get_session_id_for_spend_log

    metadata = data[metadata_key]
    assert isinstance(metadata, dict)
    derouter_params = get_derouter_params(
        derouter_session_id=str(data["derouter_session_id"]) if "derouter_session_id" in data else None,
        derouter_trace_id=str(data["derouter_trace_id"]) if "derouter_trace_id" in data else None,
        metadata=metadata,
    )
    trace_id = StandardLoggingPayloadSetup.get_standard_logging_payload_trace_id(
        logging_obj=SimpleNamespace(derouter_trace_id="per-call-random-trace-id"),
...
async def test_missing_session_id_generate_makes_spend_log_and_callback_session_ids_agree(
    ...
    monkeypatch.setattr(derouter, "request_correlation_in_logs", request_correlation_in_logs)
    data = {"model": "gpt-4o", "messages": [{"role": "user", "content": "hi"}]}
    ...
    assert (
        get_fireworks_session_id({"derouter_session_id": updated["derouter_session_id"], "metadata": updated["metadata"]})
        is None
    )
...
async def test_missing_session_id_unset_keeps_legacy_divergence():
    ...
    assert "derouter_session_id" not in updated
...
async def test_missing_session_id_omit_leaves_spend_log_session_id_null():
    ...
    assert "session_id" not in updated["metadata"]
    assert updated["derouter_trace_id"] == "4bf92f3577b34da6a3ce929d0e0e4736"
...
async def test_missing_session_id_omit_keeps_client_supplied_session_id():
    request = _request_for("/v1/chat/completions")
    request.headers = {"x-derouter-session-id": "client-session-1"}
...
async def test_missing_session_id_policy_keeps_client_supplied_session_id(policy: str):
    ...
    assert updated["derouter_session_id"] == "client-session-1"
    assert updated["metadata"]["session_id"] == "client-session-1"
    assert _spend_log_session_id(updated) == "client-session-1"
    assert SESSION_ID_GENERATED_METADATA_KEY not in updated["metadata"]
    assert (
        get_fireworks_session_id({"derouter_session_id": "client-session-1", "metadata": updated["metadata"]})
        == "client-session-1"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("/v1/chat/completions", "derouter_metadata", "metadata", {}),
        ("/v1/chat/completions", "derouter_metadata", "metadata", {"missing_session_id": "generate"}),
        ("/v1/messages", "derouter_metadata", "derouter_metadata", {}),
        ("/v1/messages", "metadata", "derouter_metadata", {}),
        ("/mcp/tools", "metadata", "metadata", {"missing_session_id": "omit"}),
        ("/mcp/tools", "derouter_metadata", "metadata", {"missing_session_id": "omit"}),
    ],
)
