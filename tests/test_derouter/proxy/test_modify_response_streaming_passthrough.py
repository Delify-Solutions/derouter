...
``derouter_logging_obj``) and NOT from the outer request body returned by
``_read_request_body`` -- the two diverge at ``function_setup`` and only the
processor copy gets ``derouter_logging_obj`` attached.
...
async def _run_streaming_block_and_get_wrapper(exception):
    ...
    request that does NOT carry derouter_logging_obj -- mirroring production,
    ...
    with patch(
        ...
        "derouter.proxy.proxy_server.select_data_generator",
        return_value=iter([]),
    ), patch(
        "derouter.CustomStreamWrapper"
    ) as mock_csw:
...
async def test_streaming_modify_response_uses_request_data_logging_obj():
    sentinel_logging_obj = MagicMock(name="derouter_logging_obj")
