...
from derouter.files.types import FileContentStreamingResult
from derouter.proxy._types import DeRouter_UserTableFiltered, UserAPIKeyAuth
from derouter.proxy.hooks import get_proxy_hook
from derouter.proxy.management_endpoints.internal_user_endpoints import ui_view_users
from derouter.proxy.openai_files_endpoints.file_content_streaming_handler import (
...
def setup_proxy_logging_object(monkeypatch, llm_router: Router) -> ProxyLogging:
    ...
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.proxy_logging_obj", proxy_logging_object
    )
...
async def test_stream_file_content_with_logging_closes_inner_iterator_on_early_exit():
    ...
    generator = FileContentStreamingHandler.stream_file_content_with_logging(
        ...
        data={"derouter_call_id": "call-123"},
    )
...
def test_get_file_content_rejects_raw_cloud_storage_uri(llm_router: Router):
    ...
    s3_file_id = "s3://my-bucket/derouter-batch-outputs/job-123/input.jsonl.out"
    response = client.get(
...
def test_mock_create_audio_file(mocker: MockerFixture, monkeypatch, llm_router: Router):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.master_key", None)
    ...
    mock_create_file = mocker.patch(
        "derouter.files.main.create_file", new=mocker.AsyncMock()
    )
    ...
    from derouter.llms.base_llm.files.transformation import BaseFileEndpoints
    ...
    class DummyManagedFiles(BaseFileEndpoints):
        async def acreate_file(
            ...
            # Call the mocked derouter.files.main.create_file to ensure asserts work
            await derouter.files.main.create_file(
                custom_llm_provider="azure",
                model="azure/chatgpt-v-2",
                api_key="AZURE_AI_API_KEY",
                file=file_data[1],
        ...
        async def afile_retrieve(self, file_id, derouter_parent_otel_span, llm_router):
        ...
        async def afile_delete(
            self, file_id, derouter_parent_otel_span, llm_router, **data
        ):
            raise NotImplementedError("Not implemented for test")
...
def test_create_file_batch_streams_from_upload_spool(monkeypatch, llm_router: Router):
    ...
    import derouter.proxy.proxy_server as ps
    from derouter.proxy._types import LitellmUserRoles
    from derouter.proxy.openai_files_endpoints import files_endpoints as fe
    from derouter.types.llms.openai import OpenAIFileObject
...
def test_target_storage_invokes_storage_backend(
    ...
    mocker.patch(
        "derouter.proxy.openai_files_endpoints.storage_backend_service.StorageBackendFileService.upload_file_to_storage_backend",
        new=async_mock,
    )
...
@pytest.mark.skip(reason="function migrated to derouter/proxy/hooks/managed_files.py")
def test_create_file_for_each_model(
    ...
    from derouter import CreateFileRequest
    ...
    from derouter.proxy.utils import ProxyLogging
    from derouter.types.llms.openai import OpenAIFileObject, OpenAIFilesPurpose
...
def test_managed_files_with_loadbalancing(
    ...
    monkeypatch.setattr("derouter.enable_loadbalancing_on_batch_endpoints", True)
    ...
    class ManagedFilesWithLoadbalancing(BaseFileEndpoints):
        async def acreate_file(
            ...
            return OpenAIFileObject(
                id="derouter_managed_file_abc123",
                object="file",
                bytes=100,
                created_at=1234567890,
    ...
    try:
        ...
        app.dependency_overrides.pop(ps.user_api_key_auth, None)
    result = response.json()
    assert result["id"] == "derouter_managed_file_abc123"
...
def test_create_file_with_nested_derouter_metadata(
    mocker: MockerFixture, monkeypatch, llm_router: Router
):
    """
    Test that nested derouter_metadata is correctly parsed from form data in bracket notation.

    Regression test for: derouter_metadata[spend_logs_metadata][owner] format should be
    ...
    class DummyManagedFiles(BaseFileEndpoints):
        async def acreate_file(
            ...
            # Capture derouter_metadata for verification
            if isinstance(create_file_request, dict):
                captured_derouter_metadata.update(
                    create_file_request.get("derouter_metadata", {})
                )
            else:
                captured_derouter_metadata.update(
                    getattr(create_file_request, "derouter_metadata", {})
                )
    ...
    # Test with nested derouter_metadata in bracket notation
    response = client.post(
        "/v1/files",
        files={"file": test_file},
        data={
            "purpose": "fine-tune",
            "target_model_names": "gpt-3.5-turbo",
            "derouter_metadata[spend_logs_metadata][owner]": "john_doe",
            "derouter_metadata[spend_logs_metadata][team]": "engineering",
            "derouter_metadata[environment]": "prod",
        },
        headers={"Authorization": "Bearer test-key"},
    )
    ...
    assert "spend_logs_metadata" in captured_derouter_metadata
    assert captured_derouter_metadata["spend_logs_metadata"]["owner"] == "john_doe"
    assert captured_derouter_metadata["spend_logs_metadata"]["team"] == "engineering"
    assert captured_derouter_metadata["environment"] == "prod"


def test_create_file_with_deep_nested_derouter_metadata(
    mocker: MockerFixture, monkeypatch, llm_router: Router
):
    """
    Test that deeply nested derouter_metadata is correctly parsed from form data.

    Regression test for: derouter_metadata[a][b][c] format should be correctly parsed.
    """
    import derouter.proxy.proxy_server as ps
    from derouter.llms.base_llm.files.transformation import BaseFileEndpoints
    from derouter.proxy._types import LitellmUserRoles
    ...
    try:
        ...
        response = client.post(
            ...
            data={
                "purpose": "batch",
                "target_model_names": "gpt-3.5-turbo",
                "derouter_metadata[config][database][host]": "localhost",
                "derouter_metadata[config][database][port]": "5432",
                "derouter_metadata[config][cache][enabled]": "true",
            },
            headers={"Authorization": "Bearer test-key"},
        )
        ...
        assert "config" in captured_derouter_metadata
        assert "database" in captured_derouter_metadata["config"]
        assert captured_derouter_metadata["config"]["database"]["host"] == "localhost"
        assert captured_derouter_metadata["config"]["database"]["port"] == "5432"
        assert "cache" in captured_derouter_metadata["config"]
        assert captured_derouter_metadata["config"]["cache"]["enabled"] == "true"
    finally:
        app.dependency_overrides.pop(ps.user_api_key_auth, None)
...
def test_get_file_content_streams_openai_direct_path(
    ...
    monkeypatch.setattr(derouter, "afile_content", _mock_afile_content)
    monkeypatch.setattr(
        "derouter.proxy.openai_files_endpoints.files_endpoints.handle_model_based_routing",
...
def test_require_managed_files_rejects_missing_target_model_names(
    ...
    monkeypatch.setattr("derouter.require_managed_files", True)
    monkeypatch.setattr("derouter.proxy.proxy_server.master_key", None)
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", None)
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", llm_router)
    setup_proxy_logging_object(monkeypatch, llm_router)

    mock_acreate_file = mocker.patch("derouter.acreate_file", new=mocker.AsyncMock())
    ...
    try:
        ...
        app.dependency_overrides.pop(ps.user_api_key_auth, None)
        monkeypatch.setattr("derouter.require_managed_files", False)
...
def test_require_managed_files_allows_managed_file_upload(
    ...
    assert response.status_code == 200, response.text
    assert response.json()["id"] == "derouter_managed_file_abc123"
    mock_acreate_file.assert_not_called()
...
def test_require_managed_files_rejects_model_param_bypass(
    ...
    derouter.acreate_file directly instead of the managed-files hook.
    """
    import derouter.proxy.proxy_server as ps
    from derouter.proxy._types import LitellmUserRoles
...
def test_require_managed_files_accepts_target_model_names_bracket_form(
    ...
    class DummyManagedFiles(BaseFileEndpoints):
        async def acreate_file(
            ...
            return OpenAIFileObject(
                id="derouter_managed_file_bracket",
                object="file",
                bytes=3,
                created_at=1234567890,
                filename="test.txt",
    ...
    assert response.status_code == 200, response.text
    assert response.json()["id"] == "derouter_managed_file_bracket"
...
def test_require_managed_files_accepts_repeated_target_model_names_bracket_form(
    ...
    class DummyManagedFiles(BaseFileEndpoints):
        async def acreate_file(
            ...
            return OpenAIFileObject(
                id="derouter_managed_file_repeated",
                object="file",
                bytes=3,
                created_at=1234567890,
                filename="test.txt",
    ...
    assert response.status_code == 200, response.text
    assert response.json()["id"] == "derouter_managed_file_repeated"
...
def test_list_files_resolves_wildcard_deployment_credentials(
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.master_key", None)
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", None)
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", wildcard_router)
    proxy_logging_obj.update_request_status = mocker.AsyncMock()
    ...
    monkeypatch.setattr(derouter, "afile_list", _mock_afile_list)
...
def test_unscoped_list_files_uses_managed_file_store(
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", None)
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", llm_router)
    provider_list = mocker.patch.object(derouter, "afile_list", new=mocker.AsyncMock())
...
def _setup_unscoped_list_files_route(mocker, monkeypatch, llm_router: Router, afile_list):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", None)
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", llm_router)
    mocker.patch.object(derouter, "afile_list", new=mocker.AsyncMock())
...
def test_list_files_with_all_proxy_models_team_uses_openai_deployment(
    ...
    """
    import derouter.proxy.proxy_server as ps
    from derouter.proxy._types import LitellmUserRoles, SpecialModelNames
        ...
        model_list=[
            {
                ...
                "derouter_params": {
                    ...
                    "derouter_credential_name": "vertex-named-cred",
                },
            }
        ]
    )
...
def test_create_file_provider_only_resolves_named_vertex_credentials(
    ...
    monkeypatch.setattr(derouter, "acreate_file", _mock_acreate_file)
...
def test_get_file_provider_only_resolves_named_vertex_credentials(
    ...
    monkeypatch.setattr(derouter, "afile_retrieve", _mock_afile_retrieve)
...
def test_delete_file_provider_only_resolves_named_vertex_credentials(
    ...
    monkeypatch.setattr(derouter, "afile_delete", _mock_afile_delete)
...
@pytest.mark.parametrize(
    "http_method, url, patched_derouter_call",
    [
        ("get", "/v1/files/file-victim-abc123", "derouter.afile_retrieve"),
        ("get", "/v1/files/file-victim-abc123/content", "derouter.afile_content"),
        ("delete", "/v1/files/file-victim-abc123", "derouter.afile_delete"),
    ],
)
def test_require_managed_files_rejects_raw_provider_file_id(
    ...
    mock_call = mocker.patch(patched_derouter_call, new=mocker.AsyncMock())
...
def test_get_file_content_model_routed_attaches_trusted_model_credentials(monkeypatch):
    ...
    bucket only from _derouter_internal_model_credentials, so without the snapshot every
    ...
    import derouter.proxy.proxy_server as ps
    from derouter.proxy._types import LitellmUserRoles
    from derouter.types.utils import SpecialEnums
    ...
    prisma_stub.db.derouter_managedfiletable.find_first = AsyncMock(return_value=managed_file_row)

    monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", prisma_stub)
    ...
    unified_id = SpecialEnums.DEROUTER_MANAGED_FILE_COMPLETE_STR.value.format(
        "application/jsonl",
        "unified-output-id",
    ...
    snapshot = calls[0].get("_derouter_internal_model_credentials")
...
def test_batch_upload_runs_guardrails_on_each_record(
    ...
    import derouter
    import derouter.proxy.openai_files_endpoints.files_endpoints as fe
    import derouter.proxy.proxy_server as ps
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", llm_router)
    setup_proxy_logging_object(monkeypatch, llm_router)
    monkeypatch.setattr(derouter, "callbacks", [_Redactor(guardrail_name="g", default_on=True)])
...
def test_batch_upload_redacts_per_record(monkeypatch, llm_router: Router):
    ...
    try:
        ...
        report = resp.json()["derouter_batch_guardrail"]
...
def test_create_file_omits_batch_guardrail_field_when_guardrail_made_no_changes(monkeypatch, llm_router: Router):
    ...
    forwarded_calls = _setup_batch_upload_endpoint(monkeypatch, llm_router)
    monkeypatch.setattr(derouter, "callbacks", [_Passthrough(guardrail_name="noop", default_on=True)])
    ProxyLogging._callback_capabilities_cache.clear()
    try:
        response = client.post(
            "/v1/files",
...
def test_batch_upload_closes_the_spools_it_opened(monkeypatch, llm_router: Router):
    ...
    import derouter
    import derouter.proxy.openai_files_endpoints.batch_guardrails as bg
    import derouter.proxy.openai_files_endpoints.files_endpoints as fe
    import derouter.proxy.proxy_server as ps
    from derouter.integrations.custom_guardrail import CustomGuardrail
    from derouter.proxy._types import LitellmUserRoles
...
def _setup_unscoped_list_files_route_over_real_hook(
    ...
    import derouter.proxy.proxy_server as ps
    from derouter.proxy._types import LitellmUserRoles
    from derouter_enterprise.proxy.hooks.managed_files import (
        _PROXY_DeRouterManagedFiles,
    )

    for env_var in ("OPENAI_API_KEY", "OPENAI_ADMIN_KEY", "OPENAI_ORGANIZATION"):
        monkeypatch.delenv(env_var, raising=False)
    monkeypatch.setattr(derouter, "api_key", None, raising=False)
    monkeypatch.setattr(derouter, "openai_key", None, raising=False)

    managed_files = _PROXY_DeRouterManagedFiles(
        internal_usage_cache=MagicMock(), prisma_client=MagicMock()
    )
    managed_files.prisma_client.db.derouter_managedfiletable = _ManagedFileTableOverRows(rows)
...
def _setup_create_file_over_pre_call_hook(monkeypatch, llm_router, hook):
    setup_proxy_logging_object(monkeypatch, llm_router)
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", llm_router)
    monkeypatch.setattr(derouter, "callbacks", [hook])
    monkeypatch.setattr(
        "derouter.proxy.openai_files_endpoints.files_endpoints.files_config",
...
def _setup_managed_file_stored_in_an_unknown_storage_backend(
    ...
    monkeypatch.setattr(
        "derouter.proxy.openai_files_endpoints.files_endpoints.ManagedFileRepository", lambda _prisma: repository
    )
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", llm_router)
