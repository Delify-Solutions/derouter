...
async def test_guardrail_writing_bookkeeping_into_metadata_is_not_a_redaction():
    def _touch_metadata(data):
        data["derouter_metadata"]["applied_guardrails"] = ["some-guard"]
...
async def test_request_metadata_is_narrowed_to_what_guardrails_read():
    ...
    metadata = {
        ...
        "derouter_parent_otel_span": threading.RLock(),
        "user_api_key": "sk-secret",
    }

    failure = await _scan(
        _jsonl(_record("a")),
        FakeProxyLogging(lambda d: seen.append(dict(d["derouter_metadata"]))),
        metadata=metadata,
    )
...
async def test_records_are_scanned_under_the_headers_the_upload_carried():
    ...
    await _scan(
        _jsonl(_record("a")),
        FakeProxyLogging(lambda d: seen.append(d["derouter_metadata"].get("headers"))),
        metadata={"guardrails": ["g"], "headers": {"x-noma-application-id": "app-1"}},
    )
...
async def test_a_file_the_upload_validation_accepts_is_a_file_the_scan_can_read(prefix, label):
    ...
    from derouter.proxy.openai_files_endpoints.batch_file_validation import check_batch_file_upload
...
def test_load_balancing_finds_the_routing_record_in_any_file_the_upload_accepts(prefix):
    ...
    from derouter.proxy.openai_files_endpoints.batch_file_validation import check_batch_file_upload
    from derouter.proxy.openai_files_endpoints.files_endpoints import get_first_json_object
...
def test_a_reported_custom_id_can_always_be_rendered(custom_id, expected):
    ...
    from derouter.proxy.openai_files_endpoints.batch_guardrails import _custom_id_of
...
async def test_scan_does_not_mutate_the_parsed_record():
    ...
    from derouter.proxy.openai_files_endpoints.batch_guardrails import _ParsedRecord, _scan_record
...
async def test_scan_is_bounded_so_a_huge_file_cannot_fan_out_without_limit():
    ...
    from derouter.proxy.openai_files_endpoints.batch_guardrails import _SCAN_WINDOW
...
async def test_redact_mode_keeps_a_masked_record_instead_of_rejecting():
    ...
    assert "derouter_metadata" not in rewritten["body"], "proxy metadata must not reach the uploaded file"
    assert result.submitted_records == 3


@pytest.mark.asyncio
@pytest.mark.parametrize("status_code", [400, 403, 422], ids=["content_policy", "akto", "llm_as_a_judge"])
async def test_every_status_derouter_calls_a_block_drops_the_record(status_code):
    """Follows CustomGuardrail._is_guardrail_intervention, so drop matches what derouter logs as a block."""
    source = _jsonl(_record("a"), _record("b", content="tripwire"))
...
async def test_derouter_native_block_exceptions_drop_the_record(exc):
...
async def test_raising_a_native_block_exception_drops_whatever_status_it_carries():
    """Raising this type IS the block signal in derouter, so the drop set matches what it logs as a block."""
...
async def test_a_guardrail_subclass_that_blocks_content_drops_only_that_record():
    ...
    from derouter.proxy.guardrails.guardrail_hooks.ovalix.ovalix import OvalixGuardrailBlockedException
...
async def test_a_record_a_guardrail_rerouted_aborts_rather_than_shipping_to_the_original_provider():
    ...
    from derouter.proxy.openai_files_endpoints.batch_guardrails import UnroutableRecord
...
async def test_a_real_non_guardrail_enforcement_hook_drops_its_record(monkeypatch):
    ...
    """
    import derouter
    from derouter.proxy.hooks.prompt_injection_detection import _OPTIONAL_PromptInjectionDetection
    from derouter.proxy._types import DeRouterPromptInjectionParams

    hook = _OPTIONAL_PromptInjectionDetection(
        prompt_injection_params=DeRouterPromptInjectionParams(heuristics_check=True)
    )
...
async def test_a_redacted_record_keeps_its_own_body_metadata():
    ...
    assert "derouter_metadata" not in body


@pytest.mark.asyncio
async def test_a_redacted_record_keeps_its_own_derouter_metadata():
    """Tags ride in derouter_metadata; a guardrail firing must not change how the record is attributed."""
    record = _record("m", content="my secret is here")
    record["body"]["derouter_metadata"] = {"tags": ["cost-center-42"]}

    body = await _rewritten_body(record, _redact_containing("secret"))

    assert body["derouter_metadata"] == {"tags": ["cost-center-42"]}
...
async def test_a_dropped_record_names_the_guardrail_from_an_enriched_http_detail():
    """derouter stamps guardrail_name into a block's detail dict; the report should carry it through."""
