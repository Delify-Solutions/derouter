...
from derouter.constants import RETURN_RAW_MODEL_NAME_METADATA_KEY
from derouter.integrations.custom_logger import CustomLogger
from derouter.integrations.opentelemetry import UserAPIKeyAuth
...
from derouter.proxy.dd_span_tagger import DDSpanTagger
from derouter.proxy._types import ProxyException
from derouter.proxy._types import UserAPIKeyAuth as ProxyUserAPIKeyAuth
from derouter.proxy.utils import ProxyLogging


class TestProxyBaseLLMRequestProcessing:
    @pytest.mark.asyncio
    async def test_base_passthrough_process_llm_request_preserves_derouter_headers_for_non_streaming_response(
        self, monkeypatch
    ):
        ...
        async def fake_base_process_llm_request(**kwargs):
            ...
            passthrough_response.headers["x-derouter-call-id"] = "test-call-id"
            passthrough_response.headers["x-derouter-version"] = "test-version"
        ...
        assert result.headers["x-derouter-call-id"] == "test-call-id"
        assert result.headers["x-derouter-version"] == "test-version"
    ...
    async def test_handle_non_streaming_allm_passthrough_route_forwards_upstream_headers(
        ...
        x-amzn-requestid) alongside the x-derouter-* headers, matching the
        ...
        result = await processing_obj._handle_non_streaming_allm_passthrough_route(
            ...
            user_api_key_dict=MagicMock(spec=UserAPIKeyAuth),
            custom_headers={"x-derouter-call-id": "test-call-id"},
            request_headers={},
        )
    ...
    async def test_handle_event_stream_allm_passthrough_route_forwards_upstream_headers(
        ...
        headers alongside the x-derouter-* headers."""
        processing_obj = ProxyBaseLLMRequestProcessing(
            data={"custom_llm_provider": "bedrock"}
        )
        monkeypatch.setattr(
    ...
    async def test_handle_non_streaming_allm_passthrough_route_applies_response_headers_hook(
        ...
        proxy_logging_obj.post_call_response_headers_hook = AsyncMock(
            return_value={"x-derouter-custom": "from-hook"}
        )
        ...
        assert isinstance(result, Response)
        assert result.headers["x-derouter-custom"] == "from-hook"
        assert result.headers["x-derouter-call-id"] == "test-call-id"
    ...
    @pytest.mark.asyncio
    async def test_common_processing_pre_call_logic_pre_call_hook_receives_derouter_call_id(self, monkeypatch):
        processing_obj = ProxyBaseLLMRequestProcessing(data={})
        mock_request = MagicMock(spec=Request)
        mock_request.headers = {}

        async def mock_add_derouter_data_to_request(*args, **kwargs):
            return {}
        ...
        monkeypatch.setattr(
            derouter.proxy.common_request_processing,
            "add_derouter_data_to_request",
            mock_add_derouter_data_to_request,
        )
        ...
        assert "derouter_call_id" in data_passed
        try:
            uuid.UUID(data_passed["derouter_call_id"])
        except ValueError:
            pytest.fail("derouter_call_id is not a valid UUID")
        assert data_passed["derouter_call_id"] == returned_data["derouter_call_id"]
    ...
    async def test_common_processing_pre_call_logic_refreshes_proxy_server_request_body_after_guardrails(
        ...
        # derouter_logging_obj is stamped onto `data` by function_setup between the
        ...
        assert "derouter_logging_obj" not in persisted_body
    ...
    async def test_common_processing_pre_call_logic_arms_auto_router_compression_before_guardrails(
        ...
        derouter.logging_callback_manager.add_derouter_callback(active_guardrail)
        ...
        try:
            ...
            derouter.logging_callback_manager.remove_callback_from_all_lists(active_guardrail)

        assert seen_metadata.get("guardrails") == ["headroom-model"]

    def test_add_dd_apm_tags_for_derouter_call_id_uses_dd_tracing_helper(self, monkeypatch):
        mock_set_active_span_tag = MagicMock(return_value=True)
        import derouter.proxy.dd_span_tagger

        monkeypatch.setattr(
            derouter.proxy.dd_span_tagger,
            "set_active_span_tag",
        ...
        mock_set_active_span_tag.assert_called_once_with("derouter.call_id", "test-call-id")
    ...
    async def test_stream_timeout_header_processing(self):
        """
        Test that x-derouter-stream-timeout header gets processed and added to request data as stream_timeout.
        """
        from derouter.proxy.derouter_pre_call_utils import DeRouterProxyRequestSetup

        # Test with stream timeout header
        headers_with_timeout = {"x-derouter-stream-timeout": "30.5"}
        result = DeRouterProxyRequestSetup._get_stream_timeout_from_request(headers_with_timeout)
        assert result == 30.5

        # Test without stream timeout header
        headers_without_timeout = {}
        result = DeRouterProxyRequestSetup._get_stream_timeout_from_request(headers_without_timeout)
        assert result is None

        # Test with invalid header value (should raise ValueError when converting to float)
        headers_with_invalid = {"x-derouter-stream-timeout": "invalid"}
        with pytest.raises(ValueError, match="could not convert string to float: 'invalid"):
            DeRouterProxyRequestSetup._get_stream_timeout_from_request(headers_with_invalid)

    @pytest.mark.asyncio
    async def test_build_derouter_proxy_success_headers_from_llm_response(self):
        """
        Google native :generateContent uses this helper instead of base_process_llm_request;
        ensure x-derouter-* headers and callback hooks merge like the main proxy path.
        """
        mock_request = MagicMock(spec=Request)
        mock_request.headers = {}
        ...
        logging_obj.derouter_call_id = "call-id-test"
        ...
        headers = await ProxyBaseLLMRequestProcessing.build_derouter_proxy_success_headers_from_llm_response(
        ...
        assert headers["x-derouter-call-id"] == "call-id-test"
        assert headers["x-derouter-model-id"] == "deployment-model-id"
        assert headers["x-derouter-version"] == "9.9.9"
        assert headers["llm_provider-ratelimit-requests"] == "1000"
        assert headers["x-ratelimit-remaining-requests"] == "999"
        proxy_logging_obj.post_call_response_headers_hook.assert_awaited_once()

    @pytest.mark.asyncio
    async def test_build_derouter_proxy_success_headers_streaming_style_iterator(self):
        ...
        mock_request.headers = {}
        logging_obj = MagicMock()
        logging_obj.derouter_call_id = "cid-stream"
        mock_user = MagicMock()
        mock_user.tpm_limit = None
        mock_user.rpm_limit = None
        mock_user.max_budget = None
        ...
        assert headers["x-derouter-model-id"] == "stream-model-id"
        assert headers["x-derouter-model-api-base"] == ("https://generativelanguage.googleapis.com/v1beta")
        assert headers["llm_provider-x"] == "y"

    @pytest.mark.asyncio
    async def test_build_derouter_proxy_success_headers_no_hidden_params_metadata_fallback(
        self,
    ):
        """When response has no _hidden_params, model_id can still come from derouter_metadata."""
        ...
        mock_request.headers = {}
        logging_obj = MagicMock()
        logging_obj.derouter_call_id = "cid-meta"
        mock_user = MagicMock()
        mock_user.tpm_limit = None
        mock_user.rpm_limit = None
        mock_user.max_budget = None
        ...
        headers = await ProxyBaseLLMRequestProcessing.build_derouter_proxy_success_headers_from_llm_response(
            ...
            request_data={
                ...
                "derouter_metadata": {"model_info": {"id": "meta-model-id"}},
            },
            request=mock_request,
            user_api_key_dict=mock_user,
            logging_obj=logging_obj,
        ...
        assert headers["x-derouter-model-id"] == "meta-model-id"

    @pytest.mark.asyncio
    async def test_add_derouter_data_to_request_with_stream_timeout_header(self):
        """
        Test that x-derouter-stream-timeout header gets processed and added to request data
        when calling add_derouter_data_to_request.
        """
        ...
        mock_request = MagicMock(spec=Request)
        mock_request.headers = {"x-derouter-stream-timeout": "45.0"}
        ...
        result_data = await add_derouter_data_to_request(
            data=test_data,
            request=mock_request,
    ...
    def test_get_custom_headers_with_discount_info(self):
        ...
        assert "x-derouter-response-cost" in headers
        assert float(headers["x-derouter-response-cost"]) == 0.000095

        assert "x-derouter-response-cost-original" in headers
        assert float(headers["x-derouter-response-cost-original"]) == 0.0001

        assert "x-derouter-response-cost-discount-amount" in headers
        assert float(headers["x-derouter-response-cost-discount-amount"]) == 0.000005
    ...
    def test_get_custom_headers_without_discount_info(self):
        ...
        assert "x-derouter-response-cost" in headers
        assert float(headers["x-derouter-response-cost"]) == 0.0001

        # Discount headers should not be in the final dict
        assert "x-derouter-response-cost-original" not in headers
        assert "x-derouter-response-cost-discount-amount" not in headers
    ...
    def test_get_custom_headers_with_margin_info(self):
        ...
        logging_obj = DeRouterLoggingObj(
            ...
            call_type="completion",
            start_time=None,
            derouter_call_id="test-call-id-margin",
        ...
        assert "x-derouter-response-cost" in headers
        assert float(headers["x-derouter-response-cost"]) == 0.00011

        assert "x-derouter-response-cost-margin-amount" in headers
        assert float(headers["x-derouter-response-cost-margin-amount"]) == 0.00001

        assert "x-derouter-response-cost-margin-percent" in headers
        assert float(headers["x-derouter-response-cost-margin-percent"]) == 0.10
    ...
    def test_get_custom_headers_without_margin_info(self):
        ...
        logging_obj = DeRouterLoggingObj(
            ...
            call_type="completion",
            start_time=None,
            derouter_call_id="test-call-id-no-margin",
            function_id="test-function",
        )
        logging_obj.set_cost_breakdown(
            input_cost=0.00005,
        ...
        assert "x-derouter-response-cost-margin-amount" not in headers
        assert "x-derouter-response-cost-margin-percent" not in headers
    ...
    def test_get_custom_headers_per_component_cost_breakdown(self):
        ...
        logging_obj = DeRouterLoggingObj(
            ...
            call_type="completion",
            start_time=None,
            derouter_call_id="test-call-id-components",
            function_id="test-function",
        )
        ...
        assert "x-derouter-response-cost" in headers
        assert float(headers["x-derouter-response-cost"]) == pytest.approx(total_cost)

        assert "x-derouter-response-cost-input" in headers
        assert float(headers["x-derouter-response-cost-input"]) == pytest.approx(uncached_input_cost)

        assert "x-derouter-response-cost-output" in headers
        assert float(headers["x-derouter-response-cost-output"]) == pytest.approx(output_cost)

        assert "x-derouter-response-cost-cache-read" in headers
        assert float(headers["x-derouter-response-cost-cache-read"]) == pytest.approx(cache_read_cost)

        assert "x-derouter-response-cost-cache-creation" in headers
        assert float(headers["x-derouter-response-cost-cache-creation"]) == pytest.approx(cache_creation_cost)

        assert "x-derouter-response-cost-reasoning" in headers
        assert float(headers["x-derouter-response-cost-reasoning"]) == pytest.approx(reasoning_cost)

        assert "x-derouter-response-cost-tool-usage" in headers
        assert float(headers["x-derouter-response-cost-tool-usage"]) == pytest.approx(tool_usage_cost)

        component_sum: Final = (
            float(headers["x-derouter-response-cost-input"])
            + float(headers["x-derouter-response-cost-cache-read"])
            + float(headers["x-derouter-response-cost-cache-creation"])
            + float(headers["x-derouter-response-cost-output"])
            + float(headers["x-derouter-response-cost-tool-usage"])
        )
        assert component_sum == pytest.approx(float(headers["x-derouter-response-cost"]))
        assert float(headers["x-derouter-response-cost-reasoning"]) <= float(headers["x-derouter-response-cost-output"])

    def test_get_custom_headers_without_cost_breakdown_omits_component_headers(self):
        """Test that when derouter_logging_obj has no cost_breakdown, component headers are omitted."""
        from derouter.derouter_core_utils.derouter_logging import (
            Logging as DeRouterLoggingObj,
        )
        ...
        logging_obj = DeRouterLoggingObj(
            ...
            call_type="completion",
            start_time=None,
            derouter_call_id="test-call-id-no-breakdown",
            function_id="test-function",
        )
        ...
        assert "x-derouter-response-cost" in headers
        assert "x-derouter-response-cost-input" not in headers
        assert "x-derouter-response-cost-output" not in headers
        assert "x-derouter-response-cost-cache-read" not in headers
        assert "x-derouter-response-cost-cache-creation" not in headers
        assert "x-derouter-response-cost-reasoning" not in headers
        assert "x-derouter-response-cost-tool-usage" not in headers
    ...
    def test_get_custom_headers_per_component_with_discount_and_margin(self):
        ...
        logging_obj = DeRouterLoggingObj(
            ...
            call_type="completion",
            start_time=None,
            derouter_call_id="test-call-id-combined",
            function_id="test-function",
        )
        ...
        assert float(headers["x-derouter-response-cost"]) == pytest.approx(0.000105)
        assert float(headers["x-derouter-response-cost-original"]) == pytest.approx(0.0001)
        assert float(headers["x-derouter-response-cost-discount-amount"]) == pytest.approx(0.000005)
        assert float(headers["x-derouter-response-cost-margin-amount"]) == pytest.approx(0.00001)
        assert float(headers["x-derouter-response-cost-margin-percent"]) == pytest.approx(0.10)
        assert float(headers["x-derouter-response-cost-input"]) == pytest.approx(0.00006)
        assert float(headers["x-derouter-response-cost-output"]) == pytest.approx(0.00004)
        assert "x-derouter-response-cost-cache-read" not in headers
        assert "x-derouter-response-cost-cache-creation" not in headers
        assert "x-derouter-response-cost-reasoning" not in headers
        assert float(headers["x-derouter-response-cost-tool-usage"]) == pytest.approx(0.0)
    ...
    def test_get_custom_headers_classifier_cost_from_routing_decision(self, metadata_key):
        ...
        x-derouter-response-cost stays the final routed call's cost (it feeds the
        ...
        bucket is metadata on chat-style routes and derouter_metadata on messages-style
        ...
        assert headers["x-derouter-classifier-cost"] == "8.1e-05"
        assert float(headers["x-derouter-response-cost"]) == 0.00023
    ...
    def test_get_custom_headers_omits_classifier_cost_without_a_priced_decision(self, request_data):
        ...
        assert "x-derouter-classifier-cost" not in headers
    ...
    def test_get_cost_breakdown_from_logging_obj_helper(self):
        ...
        logging_obj_with_margin = DeRouterLoggingObj(
            model="gpt-4",
            messages=[{"role": "user", "content": "test"}],
            stream=False,
            call_type="completion",
        ...
        logging_obj_no_discount = DeRouterLoggingObj(
            model="gpt-3.5-turbo",
            messages=[{"role": "user", "content": "test"}],
            stream=False,
            call_type="completion",
            start_time=None,
            derouter_call_id="test-call-id-2",
        ...
        logging_obj_with_cache = DeRouterLoggingObj(
            model="claude-haiku-4-5",
            messages=[{"role": "user", "content": "test"}],
            stream=False,
            call_type="completion",
            start_time=None,
            derouter_call_id="test-call-id-cache",
    ...
    def test_get_custom_headers_key_spend_includes_response_cost(self):
        """
        Test that x-derouter-key-spend header includes the current request's response_cost.
        ...
        assert "x-derouter-key-spend" in headers_1
        expected_spend_1 = 0.001 + 0.0005  # Initial spend + current request cost
        assert float(headers_1["x-derouter-key-spend"]) == pytest.approx(expected_spend_1, abs=1e-10)
        assert float(headers_1["x-derouter-response-cost"]) == response_cost_1
        ...
        assert "x-derouter-key-spend" in headers_2
        expected_spend_2 = 0.001 + 0.0003  # Initial spend + current request cost
        assert float(headers_2["x-derouter-key-spend"]) == pytest.approx(expected_spend_2, abs=1e-10)
        ...
        assert "x-derouter-key-spend" in headers_3
        assert float(headers_3["x-derouter-key-spend"]) == 0.001  # Should use original spend
        ...
        assert "x-derouter-key-spend" in headers_4
        assert float(headers_4["x-derouter-key-spend"]) == 0.001  # Should remain unchanged for 0 cost
        ...
        assert "x-derouter-key-spend" in headers_5
        assert float(headers_5["x-derouter-key-spend"]) == 0.0002  # 0.0 + 0.0002
        ...
        assert "x-derouter-key-spend" in headers_6
        assert float(headers_6["x-derouter-key-spend"]) == 0.001  # Should use original spend
        ...
        assert "x-derouter-key-spend" in headers_7
        assert float(headers_7["x-derouter-key-spend"]) == 0.001  # Should use original spend on error
    ...
    async def test_queue_time_seconds_is_set_in_metadata(self, monkeypatch):
        ...
        after add_derouter_data_to_request populates arrival_time.

        This verifies the fix for the bug where queue_time_seconds was always None
        because arrival_time was read BEFORE add_derouter_data_to_request set it.
        """
        processing_obj = ProxyBaseLLMRequestProcessing(data={})
        mock_request = MagicMock(spec=Request)
        mock_request.headers = {}
        ...
        async def mock_add_derouter_data_to_request(*args, **kwargs):
            ...
            # Simulate what add_derouter_data_to_request does: set arrival_time
            import time
        ...
        # (the same start_time derouter_request_total_latency_metric's window
...
class TestCommonRequestProcessingHelpers:
    ...
    async def test_proxy_exception_from_http_exception_helper(self):
        ...
        result = proxy_exception_from_http_exception(exc, {"x-derouter-call-id": "abc"})
        ...
        assert result.headers == {"x-derouter-call-id": "abc"}
    ...
    async def test_create_streaming_response_refresh_headers_after_first_chunk(self):
        ...
        refresh_headers: Final = AsyncMock(
            return_value={"x-derouter-model-id": "fallback-deployment", "llm_provider-x-request-id": "req-FALLBACK"}
        )

        response = await create_response(
            mock_generator(),
            "text/event-stream",
            {"x-derouter-model-id": "failed-deployment", "llm_provider-x-request-id": "req-FAILED"},
            refresh_headers=refresh_headers,
        )
        assert isinstance(response, StreamingResponse)
        assert refresh_headers.await_count == 1
        assert response.headers["x-derouter-model-id"] == "fallback-deployment"
    ...
    async def test_create_streaming_response_refreshes_only_after_the_first_chunk(self):
        ...
        async def refresh_headers():
            ...
            return {"x-derouter-model-id": served}

        response = await create_response(
            mock_generator(),
            "text/event-stream",
            {"x-derouter-model-id": "failed-deployment"},
            refresh_headers=refresh_headers,
        )
        assert response.headers["x-derouter-model-id"] == "fallback-deployment"
    ...
    async def test_create_streaming_response_empty_stream_uses_refreshed_headers(self):
        ...
        async def refresh_headers():
            return {"x-derouter-model-id": "fallback-deployment"}
    ...
    async def test_create_streaming_response_without_refresh_headers_is_unchanged(self):
        ...
        assert response.headers["x-derouter-model-id"] == "failed-deployment"
    ...
    async def test_create_streaming_response_all_chunks_have_dd_trace(self):
        ...
        with (
            patch("derouter.proxy.common_request_processing.tracer", mock_tracer),
            patch(
                "derouter.proxy.common_request_processing._DD_STREAMING_TRACE_ENABLED",
                True,
            ),
        ):
            response = await create_response(mock_generator(), "text/event-stream", {})
    ...
    async def test_create_streaming_response_dd_trace_with_error_chunk(self):
        ...
        with patch("derouter.proxy.common_request_processing.tracer", mock_tracer):
            response = await create_response(mock_generator(), "text/event-stream", {})
...
class TestGuardrailBlockErrorPayloadNeverStringifiesNone:
    ...
    def test_streaming_block_frame_carries_a_real_type_and_null_param(self):
        from derouter.proxy.common_request_processing import sse_error_payload
...
class TestOverrideOpenAIResponseModel:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ({"derouter_metadata": {RETURN_RAW_MODEL_NAME_METADATA_KEY: True}}, True),
        ],
    )
    ...
    def test_override_model_preserves_fallback_model_when_fallback_occurred_object(
        ...
        """
        Test that when a fallback occurred (x-derouter-attempted-fallbacks > 0),
        ...
        response_obj._hidden_params = {"additional_headers": {"x-derouter-attempted-fallbacks": 1}}
    ...
    def test_override_model_preserves_fallback_model_multiple_fallbacks(self):
        ...
        response_obj._hidden_params = {
            "additional_headers": {
                "x-derouter-attempted-fallbacks": 2  # Multiple fallbacks
            }
        }
    ...
    def test_override_model_overrides_when_attempted_fallbacks_is_zero(self):
        ...
        response_obj._hidden_params = {
            "additional_headers": {
                "x-derouter-attempted-fallbacks": 0  # Zero means no fallback occurred
            }
        }
    ...
    def test_override_model_overrides_when_attempted_fallbacks_is_none(self):
        ...
        response_obj = MagicMock()
        response_obj.model = downstream_model
        response_obj._hidden_params = {"additional_headers": {"x-derouter-attempted-fallbacks": None}}
    ...
    def test_override_model_uses_winning_model_for_fastest_response(self):
        ...
        response_obj._hidden_params = {
            ...
            "additional_headers": {
                "x-derouter-model-group": winning_model_group,
            },
        }
    ...
    def test_override_model_normal_when_fastest_response_not_set(self):
        ...
        response_obj._hidden_params = {
            "additional_headers": {
                "x-derouter-model-group": "openai/gpt-4o",
            },
        }
    ...
    def test_skips_model_override_when_response_has_no_model_attribute(self):
        from derouter.llms.base_llm.search.transformation import SearchResponse, SearchResult
...
class TestStreamingOverheadHeader:
    """
    Tests that x-derouter-overhead-duration-ms is emitted in streaming responses.
    ...
    def test_get_custom_headers_includes_overhead_when_set(self):
        """
        get_custom_headers() returns x-derouter-overhead-duration-ms
        when derouter_overhead_time_ms is in hidden_params.
        """
        mock_user_api_key_dict = MagicMock(spec=UserAPIKeyAuth)
        mock_user_api_key_dict.tpm_limit = None
        mock_user_api_key_dict.rpm_limit = None
        ...
        hidden_params = {
            "derouter_overhead_time_ms": 42.5,
            "_response_ms": 500.0,
        ...
        assert "x-derouter-overhead-duration-ms" in headers
        assert headers["x-derouter-overhead-duration-ms"] == "42.5"
    ...
    def test_get_custom_headers_reads_timing_from_logging_obj_when_response_has_no_hidden_params(self):
        ...
        headers = ProxyBaseLLMRequestProcessing.get_custom_headers(
            ...
            call_id="test-call-id",
            hidden_params={},
            derouter_logging_obj=self._timing_logging_obj(
                {"_response_ms": 500.0, "derouter_overhead_time_ms": 42.5}
            ),
        )

        assert headers["x-derouter-response-duration-ms"] == "500.0"
        assert headers["x-derouter-overhead-duration-ms"] == "42.5"
    ...
    def test_get_custom_headers_skips_logging_obj_timing_on_the_failure_path(self):
        ...
        assert "x-derouter-response-duration-ms" not in headers
        assert "x-derouter-overhead-duration-ms" not in headers
    ...
    def test_get_custom_headers_takes_both_timing_values_from_one_source(self):
        ...
        assert headers["x-derouter-response-duration-ms"] == "300.0"
        assert "x-derouter-overhead-duration-ms" not in headers
    ...
    def test_get_custom_headers_survives_a_logging_object_without_timing_metrics(self):
        ...
        headers = ProxyBaseLLMRequestProcessing.get_custom_headers(
            ...
            call_id="test-call-id",
            hidden_params={},
            derouter_logging_obj=_NoTimingLoggingObj(),
        )
    ...
    def test_get_custom_headers_prefers_response_hidden_params_over_logging_obj_timing(self):
        ...
        headers = ProxyBaseLLMRequestProcessing.get_custom_headers(
            user_api_key_dict=mock_user_api_key_dict,
            call_id="test-call-id",
            hidden_params={"_response_ms": 300.0, "derouter_overhead_time_ms": 7.5},
            derouter_logging_obj=self._timing_logging_obj(
                {"_response_ms": 500.0, "derouter_overhead_time_ms": 42.5}
            ),
        )
        ...
        assert headers["x-derouter-response-duration-ms"] == "300.0"
        assert headers["x-derouter-overhead-duration-ms"] == "7.5"
    ...
    def test_get_custom_headers_omits_overhead_when_none(self):
        """
        get_custom_headers() omits x-derouter-overhead-duration-ms
        when derouter_overhead_time_ms is not in hidden_params.
        """
        mock_user_api_key_dict = MagicMock(spec=UserAPIKeyAuth)
        mock_user_api_key_dict.tpm_limit = None
        mock_user_api_key_dict.rpm_limit = None
    ...
    def test_update_response_metadata_sets_overhead_on_stream_wrapper(self):
        """
        update_response_metadata() sets derouter_overhead_time_ms on
        ...
        mock_logging_obj.derouter_call_id = "test-call-id"
        ...
        assert "derouter_overhead_time_ms" in stream_result._hidden_params
        overhead = stream_result._hidden_params["derouter_overhead_time_ms"]
    ...
    async def test_streaming_response_includes_overhead_header(self):
        ...
        x-derouter-overhead-duration-ms in its headers.
        """
        ...
        headers = {
            "x-derouter-overhead-duration-ms": "42.5",
            "x-derouter-call-id": "test-call-id",
            "x-derouter-model-id": "test-model-id",
        }
        ...
        assert isinstance(response, StreamingResponse)
        assert response.headers.get("x-derouter-overhead-duration-ms") == "42.5"
    ...
    def test_streaming_overhead_header_in_custom_headers_from_stream_hidden_params(
        ...
        response's hidden_params (containing derouter_overhead_time_ms),
        the x-derouter-overhead-duration-ms header is correctly populated.
        ...
        hidden_params = {
            ...
            "api_base": "https://api.openai.com",
            "additional_headers": {},
            "derouter_overhead_time_ms": 55.3,  # set by update_response_metadata
            "_response_ms": 280.0,
            "derouter_call_id": "test-call-id",
            "response_cost": 0.002,
            "cache_key": None,
        ...
        assert "x-derouter-overhead-duration-ms" in custom_headers, (
            "x-derouter-overhead-duration-ms header must be emitted during streaming. "
            "It was missing — this is the streaming overhead header regression."
        )
        assert custom_headers["x-derouter-overhead-duration-ms"] == "55.3"
...
class TestDDSpanTaggerTagRequest:
    ...
    def test_tags_key_alias_and_model(self):
        ...
        with patch("derouter.proxy.dd_span_tagger.set_active_span_tag") as mock_set_tag:
            DDSpanTagger.tag_request(
                user_api_key_dict=user_key,
                requested_model="gpt-4o",
            )

        mock_set_tag.assert_any_call("derouter.key_alias", "my-prod-key")
        mock_set_tag.assert_any_call("derouter.key_hash", "hashed123")
        mock_set_tag.assert_any_call("derouter.requested_model", "gpt-4o")
    ...
    def test_only_model_tagged_when_no_key_info(self):
        ...
        mock_set_tag.assert_called_once_with("derouter.requested_model", "claude-3-5-sonnet")
...
class TestHandleLLMApiExceptionDictDetail:
    ...
    async def _invoke(self, exc: Exception, callback_headers: Optional[dict] = None):
        from derouter.proxy._types import ProxyException, UserAPIKeyAuth
    ...
    async def test_not_found_error_preserves_404(self):
        ...
        from derouter.exceptions import NotFoundError
    ...
    async def test_exception_with_status_code_propagates(self):
        ...
        from derouter.llms.vertex_ai.common_utils import VertexAIError
    ...
    async def test_unclassified_exception_redacts_internal_details_from_client_message(self):
        ...
        exc = RuntimeError(
            "Failed to connect to postgresql://derouter_internal:S3cr3tPGPass@10.20.30.40:5432/derouter_prod "
            "(config file /etc/derouter/secrets/db.yaml)"
        )
        proxy_exc = await self._invoke(exc)
        assert proxy_exc.code == "500"
        assert "S3cr3tPGPass" not in proxy_exc.message
        assert "derouter_internal" not in proxy_exc.message
        assert "10.20.30.40" not in proxy_exc.message
        assert "/etc/derouter/secrets/db.yaml" not in proxy_exc.message
...
class TestStreamCloseOnDisconnect:
    ...
    async def test_async_streaming_data_generator_redacts_internal_details_on_error(
        ...
        assert "/etc/derouter/secrets/db.yaml" not in message
...
class TestAsyncStreamingDataGeneratorFastPath:
    ...
    async def test_slow_path_runs_per_chunk_hook(self, monkeypatch):
        ...
        cb = _StreamingCb()
        monkeypatch.setattr(derouter, "callbacks", [cb])
        ProxyLogging._callback_capabilities_cache.clear()
...
class TestDisconnectGatherCleanup:
    ...
    async def test_base_process_llm_request_raises_499_on_client_disconnect(
        ...
        import derouter.proxy.common_request_processing as cpr
    ...
    async def test_cancel_pending_gather_tasks_skips_already_done_tasks(self):
        ...
        from derouter.proxy.common_request_processing import _cancel_pending_gather_tasks
...
class TestStreamingClientDisconnectLogging:
    ...
    async def test_record_streaming_client_disconnect_sets_error_information(self):
        ...
        mock_logging_obj = MagicMock()
        mock_logging_obj.model_call_details = {"derouter_params": {}, "metadata": {}}
        mock_request = MagicMock(spec=Request)
        mock_request.is_disconnected = AsyncMock(return_value=True)
        request_data = {
            "derouter_call_id": "test-call-id",
            "derouter_logging_obj": mock_logging_obj,
            "metadata": {},
    ...
    async def test_record_streaming_client_disconnect_handles_none_request_data_metadata(self):
        ...
        assert request_data["metadata"]["client_disconnected"] is True
        assert (
            request_data["derouter_params"]["metadata"]["client_disconnected"] is True
        )
    ...
    async def test_finalize_streaming_generator_cleanup_fires_deferred_logging(
        ...
        fire_spy = MagicMock()
        monkeypatch.setattr(
            "derouter.proxy.utils.ProxyLogging._fire_deferred_stream_logging",
            fire_spy,
        )
        ...
        request_data = {
            "metadata": {},
            "derouter_params": {"metadata": {}},
            "derouter_logging_obj": MagicMock(model_call_details={"metadata": {}, "derouter_params": {}}),
        }
    ...
    async def test_finalize_streaming_generator_cleanup_skips_disconnect_after_completion(
        ...
        mock_response = MagicMock()
        mock_response.aclose = AsyncMock()
        request_data = {"metadata": {}, "derouter_params": {"metadata": {}}}
    ...
    async def test_async_streaming_data_generator_records_499_on_early_aclose(
        ...
        request_data = {
            ...
            "derouter_logging_obj": MagicMock(
                model_call_details={"metadata": {}, "derouter_params": {}}
            ),
        }
...
class TestCancelOnDisconnect:
    ...
    async def _drive_base_process_llm_request(
        ...
        logging_obj = MagicMock()
        logging_obj.derouter_call_id = "test-cancel-on-disconnect"
        logging_obj._defer_async_logging = False
        logging_obj._on_deferred_stream_complete = None
        logging_obj.cost_breakdown = None

        processor = ProxyBaseLLMRequestProcessing(
            data={"model": "fake-model", "derouter_logging_obj": logging_obj}
        )
...
class TestEventStreamAllmPassthroughRoute:
    ...
    async def test_bedrock_provider_dispatches_to_handler(self):
        ...
        with patch(
            "derouter.llms.bedrock.passthrough.guardrail_translation.handler.BedrockPassthroughGuardrailHandler.de_anonymize_event_stream",
    ...
    async def test_non_streaming_response_includes_custom_headers(self):
        ...
        custom_headers = {
            "x-derouter-call-id": "test-call-123",
            "x-derouter-model-id": "bedrock/claude",
            "content-length": "99",
        }
        ...
        assert result.headers.get("x-derouter-call-id") == "test-call-123"
        assert result.headers.get("x-derouter-model-id") == "bedrock/claude"
...
class TestAllmPassthroughStreamingProviderGate:
    ...
    def _build_processing_obj(
        ...
        logging_obj = MagicMock()
        logging_obj.derouter_call_id = "call-123"
        logging_obj.cost_breakdown = None
        data = {
    ...
    async def _run(self, processing_obj, monkeypatch, chunks, stream=None):
        import derouter.proxy.common_request_processing as crp
        from derouter.proxy._types import UserAPIKeyAuth as RealUserAPIKeyAuth
    ...
    async def test_bedrock_converse_stream_is_buffered_through_handler(
        ...
        with patch.object(
            ...
            "derouter.llms.bedrock.passthrough.guardrail_translation.handler."
...
class TestResponseCostHeaderForTypedDictResponses:
    """
    Regression for LIT-4076. x-derouter-response-cost went missing on Anthropic
    ...
    def _build_logging_obj(self, *, model_call_details, response_cost_calculator):
        logging_obj = MagicMock()
        logging_obj.derouter_call_id = "call-lit4076"
        logging_obj.cost_breakdown = None
    ...
    async def _drive_non_streaming(
        ...
        fastapi_response = Response()
        processing_obj = ProxyBaseLLMRequestProcessing(
            data={"derouter_logging_obj": logging_obj, **({"model": client_model} if client_model else {})}
        )
    ...
    async def test_messages_typeddict_emits_cost_header_from_stored_cost(self, monkeypatch):
        from derouter.types.utils import AnthropicMessagesResponse
        ...
        assert fastapi_response.headers["x-derouter-response-cost"] == "0.00123"
    ...
    async def test_messages_cost_recompute_prices_provider_model_not_client_alias(self, monkeypatch):
        ...
        assert fastapi_response.headers["x-derouter-response-cost"] == "0.003"
    ...
    async def test_generate_content_typeddict_emits_cost_header_via_recompute(self, monkeypatch):
        ...
        assert fastapi_response.headers["x-derouter-response-cost"] == "0.00456"
        recompute.assert_called_once()
    ...
    async def test_generate_content_emits_real_nonzero_cost_header_from_usage_metadata(self, monkeypatch):
        ...
        real_logging = DeRouterLoggingObj(
            model="gemini-2.5-flash",
            messages=[{"role": "user", "content": "hi"}],
            stream=False,
            call_type="agenerate_content",
            start_time=None,
            derouter_call_id="call-lit4076-real",
            function_id="fn",
        )
        ...
        assert float(fastapi_response.headers["x-derouter-response-cost"]) == pytest.approx(expected_cost)
    ...
    async def test_generate_content_with_hidden_params_emits_cost_header(self, monkeypatch):
        ...
        attribute (which is why x-derouter-model-group / x-derouter-model-api-base
        ...
        x-derouter-response-cost went missing even though the cost was computed.
        """
        from types import SimpleNamespace

        response = SimpleNamespace(
            _hidden_params={
                "additional_headers": {"x-derouter-model-group": "gemini-2.5-flash"},
            }
        )
        recompute = MagicMock(return_value=999.0)
        logging_obj = self._build_logging_obj(
        ...
        assert fastapi_response.headers["x-derouter-response-cost"] == "0.0004521"
        assert fastapi_response.headers["x-derouter-model-group"] == "gemini-2.5-flash"
        recompute.assert_not_called()
    ...
    async def test_generate_content_with_hidden_params_zero_cost_drops_header(self, monkeypatch):
        ...
        x-derouter-response-cost: 0.0 where nothing was emitted before.
        """
        from types import SimpleNamespace
        ...
        assert "x-derouter-response-cost" not in fastapi_response.headers
        recompute.assert_not_called()
    ...
    async def test_object_response_with_hidden_params_is_unaffected(self, monkeypatch):
        ...
        assert fastapi_response.headers["x-derouter-response-cost"] == "0.009"
        recompute.assert_not_called()
...
class TestCostHeadersForCallsPricedAtZero:
    ...
    zero dropped the entire x-derouter-response-cost family off those replies: the header
    ...
    def _logging_obj(*, call_type, recovered_cost=0.0):
        logging_obj = MagicMock()
        logging_obj.derouter_call_id = "call-lit5602"
    ...
    async def _drive(self, *, monkeypatch, response, logging_obj, route_type):
        ...
        fastapi_response = Response()
        processing_obj = ProxyBaseLLMRequestProcessing(data={"derouter_logging_obj": logging_obj})
    ...
    async def test_responses_read_emits_the_cost_header_family_at_zero(self, monkeypatch):
        ...
        assert fastapi_response.headers["x-derouter-response-cost"] == "0.0"
        for component in (
            "original",
            "discount-amount",
            "margin-amount",
            ...
            assert fastapi_response.headers[f"x-derouter-response-cost-{component}"] == "0.0"
    ...
    async def test_reading_a_background_response_keeps_its_real_cost(self, monkeypatch):
        ...
        assert float(fastapi_response.headers["x-derouter-response-cost"]) == pytest.approx(0.00042)
    ...
    def test_cost_breakdown_reports_zero_components_for_a_call_priced_at_zero(self):
        breakdown = _get_cost_breakdown_from_logging_obj(
            derouter_logging_obj=self._logging_obj(call_type="aget_responses")
        )
    ...
    def test_cost_breakdown_stays_empty_for_an_inference_call(self):
        breakdown = _get_cost_breakdown_from_logging_obj(
            derouter_logging_obj=self._logging_obj(call_type="acompletion")
        )
...
class TestPreCallWithFallbacksOnLocalRateLimit:
    ...
    async def test_fallback_triggered_on_local_rate_limit(self):
        from derouter.proxy.common_utils.proxy_rate_limit_error import ProxyRateLimitError
        from derouter.proxy.common_request_processing import ProxyBaseLLMRequestProcessing
    ...
    async def test_real_parallel_request_limiter_model_tpm_limit_triggers_fallback(self):
        ...
        from derouter.proxy.common_utils.proxy_rate_limit_error import (
            ProxyRateLimitError,
        )
        from derouter.proxy.hooks.parallel_request_limiter import (
            _PROXY_MaxParallelRequestsHandler,
        )
        from derouter.proxy.utils import InternalUsageCache
        ...
        await limiter.internal_usage_cache.async_set_cache(
            ...
            derouter_parent_otel_span=None,
            local_only=True,
        )
        ...
        with patch(
            "derouter.proxy.hooks.parallel_request_limiter.datetime", _FrozenClock
        ):
            with patch.object(
                processor,
                "common_processing_pre_call_logic",
...
class TestStreamingClientDisconnectBilling:
    ...
    async def test_completed_stream_does_not_double_bill_on_late_disconnect(self):
        ...
        try:
            ...
            await ProxyBaseLLMRequestProcessing._finalize_streaming_generator_cleanup(
                request=None,
                request_data={"derouter_logging_obj": response.logging_obj},
                response=response,
                stream_completed=False,
                client_disconnected=True,
            )
            ...
            model_list=[
                {
                    ...
                    "derouter_params": {"model": "openai/gpt-4o-mini", "api_key": "test-key"},
                }
            ]
        )
        recorder = _RecordingSuccessLogger()
    ...
    async def test_disconnect_billing_does_not_double_release_slot(self):
        ...
        original_callbacks = derouter.callbacks
        derouter.callbacks = [_RecordingSuccessLogger()]
        try:
            response = await self._start_partial_stream()
            ...
            billed = await _bill_partial_streamed_spend_on_disconnect(
                {"derouter_logging_obj": response.logging_obj}, response
            )
    ...
    async def _bill_and_collect_success_event(self, prepare=None, request_data=None):
        ...
        try:
            ...
            if prepare is not None:
                prepare(response)
            billed = await _bill_partial_streamed_spend_on_disconnect(
                {"derouter_logging_obj": response.logging_obj, **(request_data or {})}, response
            )
            assert billed is True
            for _ in range(50):
                if recorder.success_events:
    ...
    async def test_disconnect_billing_prices_alias_restamped_chunks_at_real_model(self):
        assert "openai/my-public-alias" not in derouter.model_cost
    ...
    async def test_disconnect_billing_keeps_the_routed_model_when_request_data_model_was_rewritten(self):
        ...
        event = await self._bill_and_collect_success_event(
            ...
            request_data={
                ...
                "_derouter_client_requested_model": "azure-model-router",
            },
        )
...
def _apply_stream_usage_tracking(
    ...
    from derouter.proxy.common_request_processing import _stream_usage_tracking_updates
...
class TestApplyStreamUsageTracking:
    def test_default_injects_usage_and_marks_strip_for_chat_completions(self):
        ...
        assert data["_derouter_strip_stream_usage"] is True
    ...
    def test_client_requested_usage_is_left_untouched_and_not_stripped(self):
        ...
        assert data["stream_options"] == {"include_usage": True}
        assert "_derouter_strip_stream_usage" not in data
    ...
    def test_client_supplied_strip_marker_is_neutralized(self):
        data = {
            ...
            "_derouter_strip_stream_usage": True,
        }

        _apply_stream_usage_tracking(data=data, general_settings={}, route_type="acompletion")

        assert data["_derouter_strip_stream_usage"] is False
        assert data["stream_options"] == {"include_usage": True}
    ...
    def test_client_supplied_strip_marker_is_neutralized_on_non_streaming_request(self):
        data = {"_derouter_strip_stream_usage": True}
    ...
    def test_deployment_on_provider_rejecting_stream_options_is_not_injected(self):
        router = derouter.Router(
            model_list=[
                {
                    "model_name": "tiny",
                    "derouter_params": {"model": "bytez/openai-community/gpt2", "api_key": "fake"},
                }
            ]
        )
    ...
    def test_mixed_provider_model_group_is_not_injected(self):
        router = derouter.Router(
            model_list=[
                {
                    "model_name": "mixed",
                    "derouter_params": {"model": "openai/gpt-4o", "api_key": "fake"},
                },
                {
                    "model_name": "mixed",
                    "derouter_params": {"model": "oci/cohere.command-r-plus", "api_key": "fake"},
                },
            ]
        )
            ...
            model_list=[
                {
                    ...
                    "derouter_params": {"model": "azure/gpt-5.4-nano", "api_key": "fake"},
                    "model_info": {
                        "team_id": "team-1",
...
class TestPerRequestModelGroupAlias:
    ...
    def _router() -> derouter.Router:
        return derouter.Router(
            model_list=[
                ...
                {
                    "model_name": "group-b",
                    "derouter_params": {"model": "openai/gpt-4o-mini", "api_key": "sk-fake"},
                },
            ]
        )
    ...
    async def test_team_level_alias_rewrites_the_requested_model(self, monkeypatch):
        ...
        from derouter.proxy.proxy_server import ProxyConfig as RealProxyConfig
        ...
        monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", MagicMock())
        monkeypatch.setattr(
            "derouter.proxy.proxy_server.get_team_object",
...
class TestInjectCostIntoUsageDict:
    ...
    def _expected_cost(model, prompt_tokens, completion_tokens):
        pricing = derouter.model_cost[model]
    ...
    def test_openai_chunk_with_flex_service_tier_uses_flex_pricing(self):
        ...
        assert result is not None
        pricing = derouter.model_cost["gpt-5-mini"]
    ...
    def test_pricing_a_frame_leaves_the_real_logging_obj_unchanged(self):
        ...
        logging_obj = DeRouterLoggingObj(
            ...
            call_type="completion",
            start_time=None,
            derouter_call_id="lit4902-breakdown-test",
            function_id="lit4902-breakdown-test",
        )
        logging_obj.update_environment_variables(derouter_params={}, optional_params={})
    ...
    def test_pricing_a_frame_restores_a_breakdown_the_request_already_had(self):
        ...
        logging_obj = DeRouterLoggingObj(
            ...
            call_type="completion",
            start_time=None,
            derouter_call_id="lit4902-breakdown-restore",
...
class TestProcessChunkWithCostInjection:
    def test_complete_usage_frame_chunk_is_injected(self, monkeypatch):
        monkeypatch.setattr(derouter, "include_cost_in_streaming_usage", True)
        chunk = (
...
def test_ttft_keepalive_interval_only_arms_for_a_streaming_request(request_data, global_interval, expected):
    with patch.object(derouter, "sse_keepalive_ping_interval_seconds", global_interval):
...
async def test_base_process_llm_request_pings_while_the_upstream_call_is_still_running(
    ...
    with patch.object(derouter, "sse_keepalive_ping_interval_seconds", 0.05):
...
async def test_base_process_llm_request_honours_a_deployment_hard_disable(
    ...
    llm_router.get_model_list = MagicMock(return_value=[{"model_name": "m", "derouter_params": params}])
...
def test_log_llm_api_exception_traceback_only_for_unexpected_errors(exc, expect_traceback, caplog):
    ...
    from derouter.proxy.common_request_processing import _log_llm_api_exception

    verbose_proxy_logger.propagate = True
    try:
        with caplog.at_level("ERROR", logger="DeRouter Proxy"):
            try:
                raise exc
...
class TestDetachedStreamFailureHook:
    ...
    def _logging_obj():
        logging_obj = MagicMock()
        logging_obj.derouter_call_id = "call-lit3798"
    ...
    async def test_streaming_messages_arms_the_detached_failure_hook(self, monkeypatch):
        ...
        user_api_key_dict = RealUserAPIKeyAuth(api_key="sk-test")
        processing_obj = ProxyBaseLLMRequestProcessing(
            data={"derouter_logging_obj": logging_obj, "model": "claude-sonnet-4-5"}
        )
...
class TestStreamingResponseHeadersFollowFallback:
    ...
    async def test_streaming_headers_name_the_deployment_that_served(self, monkeypatch):
        ...
        logging_obj = MagicMock()
        logging_obj.derouter_call_id = "lit-6767-call"
        logging_obj._defer_async_logging = False
        logging_obj._on_deferred_stream_complete = None
        logging_obj.cost_breakdown = None

        processor = ProxyBaseLLMRequestProcessing(
            data={"model": "oa-midfail", "stream": True, "derouter_logging_obj": logging_obj}
        )
        ...
        monkeypatch.setattr(
            derouter.proxy.common_request_processing, "route_request", fake_route_request
        )
        ...
        assert isinstance(result, StreamingResponse)
        assert result.headers["x-derouter-model-id"] == "served-deployment"
        assert result.headers["x-derouter-model-api-base"] == "https://api.openai.com"
...
class TestPassthroughHeadersAcceptImmutableMappings:
    ...
    def test_merge_passthrough_streaming_headers_accepts_a_read_only_mapping(self):
        merged = ProxyBaseLLMRequestProcessing._merge_passthrough_streaming_headers(
            ...
            custom_headers=MappingProxyType({"x-derouter-model-id": "served-deployment"}),
        )

        assert merged["x-derouter-model-id"] == "served-deployment"
