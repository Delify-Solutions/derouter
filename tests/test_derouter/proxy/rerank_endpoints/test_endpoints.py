...
import derouter.proxy.proxy_server as proxy_server_mod
from derouter.proxy._types import ProxyException, UserAPIKeyAuth
from derouter.proxy.rerank_endpoints.endpoints import rerank
from derouter.types.utils import RerankResponse
...
HIDDEN_PARAMS = {
    ...
    "response_cost": 0.002,
    "_response_ms": 1500.5,
    "derouter_overhead_time_ms": 12.5,
...
async def _call_rerank(hidden_params: dict = HIDDEN_PARAMS) -> Response:
    ...
    async def fake_add_derouter_data_to_request(**kwargs):
        return {**kwargs["data"], "derouter_call_id": "call-123"}
    ...
    with (
        patch.object(proxy_server_mod, "add_derouter_data_to_request", fake_add_derouter_data_to_request),  # test-quality-ok: the rerank route reads these proxy_server module globals; no injection seam on the FastAPI handler
...
async def test_rerank_emits_latency_and_cost_headers():
    ...
    assert fastapi_response.headers["x-derouter-call-id"] == "call-123"
    assert fastapi_response.headers["x-derouter-response-duration-ms"] == "1500.5"
    assert fastapi_response.headers["x-derouter-overhead-duration-ms"] == "12.5"
    assert fastapi_response.headers["x-derouter-callback-duration-ms"] == "1.25"
    assert fastapi_response.headers["x-derouter-response-cost"] == "0.002"


@pytest.mark.asyncio
async def test_rerank_emits_detailed_timing_headers_when_enabled():
    """DEROUTER_DETAILED_TIMING must also work on /rerank, not just /chat/completions."""
    with patch.object(common_request_processing_mod, "DEROUTER_DETAILED_TIMING", True):  # test-quality-ok: DEROUTER_DETAILED_TIMING is a module constant; toggling it is the behavior under test
        fastapi_response = await _call_rerank()

    assert fastapi_response.headers["x-derouter-timing-llm-api-ms"] == "1488.0"
    assert fastapi_response.headers["x-derouter-timing-pre-processing-ms"] == "10.0"
    assert fastapi_response.headers["x-derouter-timing-post-processing-ms"] == "2.5"
    assert fastapi_response.headers["x-derouter-timing-message-copy-ms"] == "0.01"
...
async def test_rerank_omits_detailed_timing_headers_when_disabled():
    with patch.object(common_request_processing_mod, "DEROUTER_DETAILED_TIMING", False):  # test-quality-ok: DEROUTER_DETAILED_TIMING is a module constant; toggling it is the behavior under test
        fastapi_response = await _call_rerank()

    assert "x-derouter-timing-llm-api-ms" not in fastapi_response.headers
...
async def _rerank_failure(
    ...
    monkeypatch.setattr(proxy_server_mod, "add_derouter_data_to_request", fake_add_derouter_data_to_request)
