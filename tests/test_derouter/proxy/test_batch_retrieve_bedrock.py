...
batch_id, it did not forward `model` as a kwarg to `derouter.aretrieve_batch`.
That caused derouter to skip the `BedrockBatchesConfig` provider_config path
...
def _make_in_progress_batch_response(batch_id: str) -> DeRouterBatch:
...
def test_retrieve_batch_passes_model_for_bedrock_encoded_id(
    ...
    """Encoded batch_id → proxy must pass `model` to derouter.aretrieve_batch
    so BedrockBatchesConfig is loaded.

    Without this, derouter falls into the legacy provider switch and raises
    'DeRouter doesn't support bedrock for retrieve_batch'.
    """
    ...
    monkeypatch.setattr(derouter, "aretrieve_batch", mock_aretrieve_batch)
    ...
    assert captured_kwargs.get("model") == BEDROCK_MODEL, (
        "model must be forwarded to derouter.aretrieve_batch so the bedrock "
...
def test_file_content_routes_to_bedrock_for_encoded_output_file_id(
    ...
    must reach `derouter.afile_content` with `custom_llm_provider="bedrock"`,
    ...
    call `derouter.file_content(...)` directly with hand-rolled AWS args.
    """
    _setup_proxy(monkeypatch, bedrock_router)
    ...
    monkeypatch.setattr(derouter, "afile_content", mock_afile_content)
    ...
    assert captured_kwargs.get("file_id") == BEDROCK_OUTPUT_S3_URI, (
        ...
        "derouter.afile_content; got kwargs: " + repr(captured_kwargs)
    )
