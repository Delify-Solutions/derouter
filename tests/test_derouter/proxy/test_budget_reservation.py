...
from derouter.constants import STREAM_SSE_KEEPALIVE_PING_BYTES
...
from derouter.proxy.common_utils.reset_budget_job import _model_access_group_counter_key
...
async def test_should_shrink_second_tag_reservation_to_remaining_budget(
    ...
    prisma_client.db.derouter_tagtable.find_many = AsyncMock(return_value=[])
...
async def test_should_seed_and_update_end_user_and_tag_counters_without_reservation(
    ...
    await key_cache.async_set_cache(
        ...
        value=DeRouter_EndUserTable(
            user_id="customer-1",
            blocked=False,
            spend=4.0,
            derouter_budget_table=DeRouter_BudgetTable(max_budget=10.0),
        ).model_dump(),
    )
    await key_cache.async_set_cache(
        key="tag:paid-tag",
...
async def test_should_reserve_team_member_and_org_budget_counters(spend_counter_state):
    ...
    await key_cache.async_set_cache(
        ...
        value=DeRouter_OrganizationTable(
...
async def test_should_not_reserve_user_budget_counter_for_team_key(spend_counter_state):
    ...
    team_object = DeRouter_TeamTable(team_id="team-no-budget", spend=0.0, max_budget=None)
    user_object = DeRouter_UserTable(user_id="user-on-team", spend=0.0, max_budget=5.0)
...
async def test_should_reserve_user_budget_counter_for_team_key_when_flag_enabled(spend_counter_state):
    ...
    team_object = DeRouter_TeamTable(team_id="team-no-budget", spend=0.0, max_budget=None)
    user_object = DeRouter_UserTable(user_id="user-on-team-flagged", spend=0.0, max_budget=5.0)
...
async def test_should_clamp_reservation_to_default_when_output_cap_missing(
    ...
    with patch(
        "derouter.proxy.spend_tracking.budget_reservation._get_model_cost_info",
        return_value={
...
def test_tiered_reservation_is_all_or_nothing_with_output_tier_from_input_length():
    ...
    with (
        ...
        patch(
            "derouter.proxy.spend_tracking.budget_reservation._estimate_input_tokens",
            return_value=input_tokens,
        ),
        patch(
            "derouter.proxy.spend_tracking.budget_reservation._estimate_output_tokens",
...
def test_reservation_uses_most_expensive_deployment_in_group():
    ...
    with (
        ...
        patch(
            "derouter.proxy.spend_tracking.budget_reservation._get_deployment_tiered_pricing_tables",
...
async def test_should_skip_reservation_when_counter_increment_fails(
    ...
    with (
        ...
        patch(
            "derouter.proxy.spend_tracking.budget_reservation.verbose_proxy_logger.warning"
        ) as mock_warning,
    ):
        reservation = await reserve_budget_for_request(
            request_body=_request_body(),
...
async def test_should_skip_reservation_when_counter_initialization_fails(
    ...
    with (
        ...
        patch(
            "derouter.proxy.proxy_server._ensure_spend_counter_initialized",
...
async def test_should_release_tracked_entry_when_reservation_fails_after_increment(
    ...
    with (
        ...
        patch(
            "derouter.proxy.proxy_server._increment_spend_counter_cache",
...
async def test_should_preserve_budget_error_and_continue_partial_cleanup(
    ...
    with (
        ...
        patch(
            "derouter.proxy.spend_tracking.budget_reservation.verbose_proxy_logger.exception"
...
async def test_release_budget_reservation_on_cancel_gives_back_counter(
    ...
    with (
        ...
        patch(
            "derouter.proxy.spend_tracking.budget_reservation.estimate_request_input_cost",
            return_value=0.5,
        ),
    ):
        reservation = await reserve_budget_for_request(
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "anthropic/claude-haiku-4-5", "api_key": "sk-test"},
            }
        ]
    )
...
async def test_release_budget_reservation_on_cancel_swallows_release_errors():
    ...
    with patch(
        "derouter.proxy.spend_tracking.budget_reservation.reconcile_budget_reservation",
...
async def test_streaming_cancel_in_slow_path_before_yield_refunds(spend_counter_state):
    ...
    with patch.object(derouter, "include_cost_in_streaming_usage", True, create=True):
        async def _drain():
            async for chunk in generator:
                received.append(chunk)
...
def _tiered_router() -> Router:
    return Router(
        model_list=[
            {
                "model_name": "dashscope/qwen3-max",
                "derouter_params": {"model": "dashscope/qwen3-max", "api_key": "sk-fake"},
                "model_info": {
                    "max_input_tokens": 258048,
                    "max_output_tokens": 65536,
                    "tiered_pricing": [
...
async def test_reservation_tokenizes_the_prompt_once(spend_counter_state):
    ...
    real_token_counter = derouter.token_counter
    calls = []
    ...
    with patch.object(derouter, "token_counter", counting_token_counter):
        reservation = await reserve_budget_for_request(
            request_body=request_body,
            route="/chat/completions",
...
async def test_large_prompt_is_tokenized_off_the_event_loop(spend_counter_state):
    ...
    with patch.object(derouter, "token_counter", recording_token_counter):
        reservation = await reserve_budget_for_request(
            request_body=request_body,
            route="/chat/completions",
            llm_router=None,
...
class _ModelAccessGroupBudgetPrisma:
    """Serves ``DeRouter_ModelAccessGroupBudgetTable`` rows, recording what reached the database."""
    ...
    def __init__(self, **max_budget_by_group) -> None:
        self.rows = {
            group: SimpleNamespace(
                ...
                spend=7.0,
                derouter_budget_table=None if max_budget is None else SimpleNamespace(max_budget=max_budget),
            )
            for group, max_budget in max_budget_by_group.items()
        }
        self.batches = []
        self.db = SimpleNamespace(
            derouter_modelaccessgroupbudgettable=SimpleNamespace(find_many=self._find_many)
        )
