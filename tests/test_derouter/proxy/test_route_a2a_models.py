...
Maps to: derouter/proxy/agent_endpoints/a2a_routing.py
"""
...
from derouter.proxy.agent_endpoints.a2a_routing import route_a2a_agent_request
from derouter.proxy.route_llm_request import route_request


@pytest.mark.asyncio
async def test_route_a2a_model_bypasses_router():
    """Test that a2a/ prefixed models bypass router and go directly to derouter with api_base"""
    ...
    # Mock derouter.acompletion to verify it's called
    ...
    with patch("derouter.acompletion", mock_acompletion):
        with patch(
            ...
            # Verify derouter.acompletion was called with api_base injected
...
async def test_route_a2a_model_read_through_recovers_agent_created_on_sibling_replica(monkeypatch):
    ...
    prisma_client.db.derouter_agentstable.find_unique.assert_awaited()
