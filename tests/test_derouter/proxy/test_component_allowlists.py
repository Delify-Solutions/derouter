...
# Importing ``derouter.proxy.proxy_server`` runs its module-level setup, which
# reads ``DATABASE_URL`` (Prisma) and ``DEROUTER_MASTER_KEY``. Tier-zero CI
...
_THROWAWAY_ENV = {
    ...
    "DEROUTER_MASTER_KEY": "sk-test-component-allowlist",
}
...
# gateway/ and backend/ live at the repo root, not inside derouter/.
...
def test_gateway_trim_keeps_metrics_mount():
    ...
    Regression test for https://github.com/Delify-Solutions/derouter/issues/30291:
...
def test_gateway_drops_ui_and_swagger_mounts():
    ...
    for path in ("/ui", "/_next", "/derouter-asset-prefix/_next", "/swagger"):
...
def test_every_app_mount_is_assigned_to_a_component():
    ...
    silently dropped.
    """
    ui_served_prefixes = ("/ui", "/_next", "/derouter-asset-prefix")
