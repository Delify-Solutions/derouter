...
class TestKeyMaskingInAuthErrors:
    ...
    def test_assert_message_masks_key_without_sk_prefix(self):
        ...
        """
        from derouter.proxy.auth.auth_utils import abbreviate_api_key
    ...
    def test_key_not_starting_with_sk_raises_masked_error(self):
        ...
        error_message = "DeRouter Virtual Key expected. Received={}, expected to start with 'sk-'.".format(
            _masked_key
        )
...
class TestKeyMaskingInKeyManagement:
    ...
    def test_invalid_key_format_error_is_masked(self):
        ...
        error_msg = f"Invalid key format. DeRouter Virtual Key must start with 'sk-'. Received: {_masked}"
