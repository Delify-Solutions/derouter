...
import derouter.proxy.management_endpoints.team_endpoints as team_endpoints
...
from derouter.proxy.management_endpoints.team_endpoints import team_member_update
...
async def test_ateam_member_update_admin_requires_premium(monkeypatch):
    ...
    expected_msg = (
        "Assigning team admins is a premium feature. You must be a DeRouter Enterprise user to use this feature. "
        "If you have a license please set `DEROUTER_LICENSE` in your env. Get a 7 day trial key here: https://www.router.delify.vn/#trial. "
...
def happy_path_upsert(monkeypatch):
    ...
    prisma_client.db.derouter_teamtable.find_unique = AsyncMock(return_value=team_row)
    prisma_client.db.derouter_teamtable.update = AsyncMock()
