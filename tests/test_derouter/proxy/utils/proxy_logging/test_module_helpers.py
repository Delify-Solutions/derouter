...
``_accepts_derouter_call_info``, ``_enrich_http_exception_with_guardrail_context``,
...
def test_print_verbose_when_set_verbose_true_prints_redacted(monkeypatch, capsys):
    ...
    snapshot = {
        "out_has_prefix": "DeRouter Proxy:" in captured.out,
...
class _CbAcceptsInfo:
    async def async_post_call_response_headers_hook(self, *, derouter_call_info=None):
        return None
...
def test_accepts_derouter_call_info_matrix(monkeypatch):
    ...
    snapshot = {
        "cache_hit_returns_true": _accepts_derouter_call_info(_CbAcceptsInfo()),
...
def test_accepts_derouter_call_info_signature_inspection(monkeypatch):
    monkeypatch.setattr(utils_mod, "_CALLBACK_ACCEPTS_CALL_INFO", {})
    snapshot = {
        "accepts_param_true": _accepts_derouter_call_info(_CbAcceptsInfo()),
        "rejects_param_false": _accepts_derouter_call_info(_CbRejectsInfo()),
...
def test_accepts_derouter_call_info_error_on_callback_without_hook_raises(monkeypatch):
    monkeypatch.setattr(utils_mod, "_CALLBACK_ACCEPTS_CALL_INFO", {})
    ...
    with pytest.raises(AttributeError):
        _accepts_derouter_call_info(_Bad())  # type: ignore[arg-type]
...
async def test_lookup_deprecated_key_returns_active_token_id_and_caches(monkeypatch):
    ...
    db = MagicMock()
    db.derouter_deprecatedverificationtoken.find_first = AsyncMock(return_value=deprecated_row)
...
async def test_lookup_deprecated_key_returns_none_when_not_found(monkeypatch):
    ...
    db = MagicMock()
    db.derouter_deprecatedverificationtoken.find_first = AsyncMock(return_value=None)
...
async def test_lookup_deprecated_key_db_error_returns_none(monkeypatch):
    ...
    monkeypatch.setattr(utils_mod, "_deprecated_key_cache", LimitedSizeOrderedDict(max_size=10))
    db = MagicMock()
    db.derouter_deprecatedverificationtoken.find_first = AsyncMock(side_effect=RuntimeError("db down"))
...
async def test_lookup_deprecated_key_uses_cache_within_ttl(monkeypatch):
    ...
    db.derouter_deprecatedverificationtoken.find_first.assert_not_called()
