...
from derouter.proxy._types import AlertType, ProxyErrorTypes
...
async def test_post_call_failure_hook_no_callbacks_returns_none(
    ...
    request_data = {"derouter_call_id": "abc", "model": "m", "messages": []}
    ...
    snapshot = {
        ...
        "derouter_logging_obj_popped": "derouter_logging_obj" not in request_data,
        "call_id_preserved": request_data["derouter_call_id"] == "abc",
        "first_api_call_start_time_present": "first_api_call_start_time" in request_data,
    }
    assert snapshot == {
        "out_is_none": True,
        "derouter_logging_obj_popped": True,
...
async def test_post_call_failure_hook_callback_returns_http_exception(
    ...
    proxy_logging.alert_types = []
    out = await proxy_logging.post_call_failure_hook(
        request_data={"derouter_call_id": "abc"},
        original_exception=ValueError("oops"),
        user_api_key_dict=make_user_api_key_auth(),
    )
...
async def test_handle_logging_proxy_only_path_no_logging_obj_creates_one(
    ...
    monkeypatch.setattr(derouter.utils, "function_setup", fake_function_setup)
