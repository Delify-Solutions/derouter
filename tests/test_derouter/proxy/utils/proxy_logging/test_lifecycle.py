...
def test_startup_event_initializes_slack_and_callbacks(proxy_logging):
    ...
    proxy_logging._init_derouter_callbacks = MagicMock()
    ...
    snapshot = {
        ...
        "init_called": proxy_logging._init_derouter_callbacks.called,
...
def test_startup_event_propagates_init_callbacks_failure_raises(proxy_logging):
    proxy_logging.slack_alerting_instance = MagicMock()
    proxy_logging.slack_alerting_instance.alert_types = []
    proxy_logging._init_derouter_callbacks = MagicMock(side_effect=RuntimeError("boom"))
...
def test_add_proxy_hooks_registers_callbacks(proxy_logging, monkeypatch):
    ...
    from derouter.proxy import utils as utils_mod
...
def test_init_derouter_callbacks_replaces_string_with_instance(proxy_logging, monkeypatch):
    from derouter.proxy import utils as utils_mod

    sentinel_instance = MagicMock(spec=derouter.integrations.custom_logger.CustomLogger)
    sentinel_instance.__class__ = derouter.integrations.custom_logger.CustomLogger

    monkeypatch.setattr(derouter, "callbacks", ["some-string-logger"])
    monkeypatch.setattr(
    ...
    proxy_logging._init_derouter_callbacks(llm_router=None)
    snapshot = {
        "replaced_first_item": derouter.callbacks[0] is sentinel_instance,
        "callbacks_grew_with_service": len(derouter.callbacks) >= 2,
        "service_logging_appended": any("ServiceLogging" in type(c).__name__ for c in derouter.callbacks),
    }
    assert snapshot == {
...
def test_init_derouter_callbacks_string_resolution_failure_keeps_string(proxy_logging, monkeypatch):
    from derouter.proxy import utils as utils_mod

    monkeypatch.setattr(derouter, "callbacks", ["unknown-logger"])
    monkeypatch.setattr(
        derouter.derouter_core_utils.derouter_logging,
        "_init_custom_logger_compatible_class",
        lambda *a, **kw: None,
    ...
    assert derouter.callbacks[0] == "unknown-logger"


def test_init_derouter_callbacks_propagates_resolver_error_raises(proxy_logging, monkeypatch):
    from derouter.proxy import utils as utils_mod

    monkeypatch.setattr(derouter, "callbacks", ["raises-on-init"])
    monkeypatch.setattr(
        derouter.derouter_core_utils.derouter_logging,
        "_init_custom_logger_compatible_class",
...
async def test_update_request_status_when_alerting_set_writes_cache(proxy_logging):
    ...
    await proxy_logging.update_request_status(derouter_call_id="call-1", status="success")
    snapshot = {
        "key": captured["key"],
...
async def test_update_request_status_cache_error_raises(proxy_logging):
    ...
    with pytest.raises(ConnectionError):
        await proxy_logging.update_request_status(derouter_call_id="x", status="fail")
