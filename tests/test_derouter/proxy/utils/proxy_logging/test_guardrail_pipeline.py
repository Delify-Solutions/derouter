...
from derouter.proxy.common_utils.callback_utils import add_guardrail_to_applied_guardrails_header
from derouter.proxy.utils import ProxyLogging, _streamable_post_call_pipelines
from derouter.proxy.guardrails.guardrail_hooks.derouter_content_filter.content_filter import ContentFilterGuardrail
from derouter.types.guardrails import BlockedWord, ContentFilterAction, GuardrailEventHooks
from derouter.types.proxy.policy_engine.pipeline_types import (
    GuardrailPipeline,
    PipelineStep,
)
...
def test_process_guardrail_metadata_calls_header_helper(proxy_logging, monkeypatch):
    ...
    from derouter.proxy.common_utils import callback_utils
...
async def test_maybe_execute_pipelines_skips_pipelines_with_other_mode(proxy_logging, make_user_api_key_auth, monkeypatch):
    ...
    executed = MagicMock()
    monkeypatch.setattr(
        "derouter.proxy.policy_engine.pipeline_executor.PipelineExecutor.execute_steps", executed
    )
    out, replacement = await proxy_logging._maybe_execute_pipelines(
        data=data,
        user_api_key_dict=make_user_api_key_auth(),
...
@pytest.mark.parametrize(
    ...
    [
        ("derouter_metadata", "metadata", "anthropic_messages"),
        ("metadata", "derouter_metadata", "completion"),
    ],
)
@pytest.mark.asyncio
async def test_maybe_execute_pipelines_finds_policy_state_when_caller_sends_own_metadata(
    ...
    """The route picks the bucket the policy engine writes to (``derouter_metadata`` on
    ...
    monkeypatch.setattr(derouter, "callbacks", [BlockingGuardrail(guardrail_name="gr-1")])
...
async def test_maybe_execute_pipelines_blocks_on_block_terminal_action_raises(
    ...
    monkeypatch.setattr(
        "derouter.proxy.policy_engine.pipeline_executor.PipelineExecutor.execute_steps",
        fake_execute_steps,
    )
    with pytest.raises(HTTPException):
        await proxy_logging._maybe_execute_pipelines(
...
async def test_run_guardrail_with_metrics_passes_result_and_records_success(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [prom])
...
async def test_during_call_hook_records_latency_metric(
    ...
    prom = _prometheus_callback()
    monkeypatch.setattr(derouter, "callbacks", [prom, cb])
...
async def test_post_call_success_hook_records_latency_metric(
    ...
    prom = _prometheus_callback()
    monkeypatch.setattr(derouter, "callbacks", [prom, cb])
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", None, raising=False)
...
async def test_process_prompt_template_no_op_when_no_prompt_spec(proxy_logging, monkeypatch):
    from derouter.proxy.prompts import prompt_registry
...
async def test_process_prompt_template_applies_when_spec_resolves(proxy_logging, monkeypatch):
    ...
    prompt_spec.derouter_params = MagicMock(prompt_id="resolved-id")
...
async def test_process_prompt_template_async_get_prompt_error_raises(proxy_logging, monkeypatch):
    ...
    custom_logger = MagicMock()
    prompt_spec = MagicMock()
    prompt_spec.derouter_params = MagicMock(prompt_id="x")
    monkeypatch.setattr(
        prompt_registry.IN_MEMORY_PROMPT_REGISTRY,
        "get_prompt_callback_for_prompt",
        lambda *a, **kw: custom_logger,
...
async def test_process_prompt_template_resolves_the_requested_environment(proxy_logging, monkeypatch):
    ...
    prompt_spec = MagicMock()
    prompt_spec.derouter_params = MagicMock(prompt_id="greeting")
...
async def test_post_call_pipeline_managed_default_on_guardrail_runs_exactly_once(
    ...
    await proxy_logging.post_call_success_hook(
        data=data, response=derouter.ModelResponse(), user_api_key_dict=make_user_api_key_auth()
    )
...
def test_handle_pipeline_result_allow_on_post_call_keeps_metadata_writes_only():
    ...
    out = ProxyLogging._handle_pipeline_result(
        result=result, data=data, policy_name="p", original_response=derouter.ModelResponse()
    )
...
async def test_pre_call_hook_stays_quiet_on_background_request_without_post_call_pipeline(
    ...
    seen: Dict[str, Any] = {}
    monkeypatch.setattr(derouter, "callbacks", [_unified_stream_guardrail(seen)])
...
def _stream_chunks() -> List[Any]:
    return [
        derouter.ModelResponseStream(choices=[{"index": 0, "delta": {"content": "hello "}, "finish_reason": None}]),
        derouter.ModelResponseStream(choices=[{"index": 0, "delta": {"content": "world"}, "finish_reason": "stop"}]),
    ]
...
def test_streamable_post_call_pipelines_keeps_supported_and_drops_unsupported(
    ...
    monkeypatch.setattr(derouter, "callbacks", [supported, native_only])
...
def test_streamable_post_call_pipelines_is_empty_on_route_without_translation(
    ...
    monkeypatch.setattr(derouter, "callbacks", [_unified_stream_guardrail({})])
    governed = GuardrailPipeline(mode="post_call", steps=[PipelineStep(guardrail="gr-post", on_fail="block")])
...
async def test_streaming_iterator_hook_pipeline_block_withholds_all_chunks(
    ...
    seen: Dict[str, Any] = {}
    monkeypatch.setattr(derouter, "callbacks", [_unified_stream_guardrail(seen, block=True)])
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", None, raising=False)
    data = _post_call_pipeline_data(stream=True)
    delivered: List[Any] = []
...
def _tool_call_stream_chunks() -> List[Any]:
    ...
    return [
        ...
        derouter.ModelResponseStream(choices=[{"index": 0, "delta": {}, "finish_reason": "tool_calls"}]),
    ]
...
async def test_streaming_iterator_hook_pipeline_releases_originals_on_runtime_tool_call_rewrite(
    ...
    monkeypatch.setattr(derouter, "callbacks", [_rewriting_stream_guardrail(transform)])
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", None, raising=False)
...
async def test_pipeline_executor_discards_text_rewrite_when_translation_lacks_write_back(monkeypatch, caplog):
    ...
    class NoWriteBackTranslation(BaseTranslation):
        async def process_input_messages(self, data, guardrail_to_apply, derouter_logging_obj):
            return data

        async def process_output_response(self, response, guardrail_to_apply, derouter_logging_obj, **kwargs):
            return response
...
async def test_per_chunk_streaming_hook_skips_pipeline_managed_guardrail(
    ...
    monkeypatch.setattr(derouter, "callbacks", [managed, free])
    monkeypatch.setattr("derouter.proxy.proxy_server.llm_router", None, raising=False)
    data = _post_call_pipeline_data(stream=True)
