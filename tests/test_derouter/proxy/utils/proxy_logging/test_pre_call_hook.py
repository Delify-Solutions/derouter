...
async def test_pre_call_hook_invokes_pre_call_override(proxy_logging, make_user_api_key_auth, monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [_Cb()])
    proxy_logging.slack_alerting_instance = MagicMock(alerting=None)
    out = await proxy_logging.pre_call_hook(
        user_api_key_dict=make_user_api_key_auth(),
...
async def test_pre_call_hook_propagates_callback_error_raises(proxy_logging, make_user_api_key_auth, monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [_BadCb()])
    proxy_logging.slack_alerting_instance = MagicMock(alerting=None)
...
async def test_guardrails_only_skips_non_guardrail_pre_call_callbacks(
    ...
    monkeypatch.setattr(derouter, "callbacks", [_RateLimiterLike()])
    proxy_logging.slack_alerting_instance = MagicMock(alerting=None)
...
async def test_guardrails_only_skips_prompt_template_rewriting(proxy_logging, make_user_api_key_auth, monkeypatch):
    ...
    await proxy_logging.pre_call_hook(
        user_api_key_dict=make_user_api_key_auth(),
        data={"messages": [{"x": "input"}], "model": "m", "prompt_id": "p1", "derouter_logging_obj": MagicMock()},
...
async def test_aresponses_call_type_applies_prompt_templates_before_routing(proxy_logging, make_user_api_key_auth, monkeypatch):
    ...
    await proxy_logging.pre_call_hook(
        user_api_key_dict=make_user_api_key_auth(),
        data={"input": "hi", "model": "m", "prompt_id": "p1", "derouter_logging_obj": MagicMock()},
...
async def test_a_content_enforcer_runs_in_both_walks(proxy_logging, monkeypatch, guardrails_only):
    enforcer = _Enforcer()
    monkeypatch.setattr(derouter, "callbacks", [enforcer])
...
async def test_an_accounting_hook_is_skipped_by_a_guardrails_only_walk(proxy_logging, monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [accountant])
...
def test_has_pre_call_guardrails_counts_a_content_enforcer(proxy_logging, monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [_Accountant()])
    assert proxy_logging.has_pre_call_guardrails({}) is False

    monkeypatch.setattr(derouter, "callbacks", [_Enforcer()])
...
def test_every_pre_call_customlogger_is_deliberately_classified():
    ...
    counts_or_shapes_the_request = {
        ...
        "_PROXY_DeRouterManagedVectorStores",
    }

    from derouter.proxy.hooks import PROXY_HOOKS
    ...
    for name, cls in (
        ...
        ("detect_prompt_injection", _load("derouter.proxy.hooks.prompt_injection_detection", "_OPTIONAL_PromptInjectionDetection")),
        ("azure_content_safety", _load("derouter.proxy.hooks.azure_content_safety", "_PROXY_AzureContentSafety")),
    ):
        if cls is not None:
            registered[name] = cls
...
async def test_yaml_order_changes_enforcement_without_scan_raw_request(
    ...
    monkeypatch.setattr(derouter, "callbacks", [_RedactingGuardrail(), _BlockOnSecretGuardrail()])
    proxy_logging.slack_alerting_instance = MagicMock(alerting=None)
    out = await proxy_logging.pre_call_hook(
        user_api_key_dict=make_user_api_key_auth(),
...
async def test_reversed_yaml_order_blocks_the_same_request(proxy_logging, make_user_api_key_auth, monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [_BlockOnSecretGuardrail(), _RedactingGuardrail()])
    proxy_logging.slack_alerting_instance = MagicMock(alerting=None)
...
async def test_scan_raw_request_makes_blocking_order_independent(proxy_logging, make_user_api_key_auth, monkeypatch):
    """Maintainer finding on Delify-Solutions/derouter#34940: with scan_raw_request=True
    ...
    monkeypatch.setattr(
        derouter, "callbacks", [_RedactingGuardrail(), _BlockOnSecretGuardrail(scan_raw_request=True)]
    )
    proxy_logging.slack_alerting_instance = MagicMock(alerting=None)
    with pytest.raises(HTTPException, match="blocked"):
        await proxy_logging.pre_call_hook(
...
async def test_scan_raw_request_guardrail_does_not_undo_later_masking(
    ...
    monkeypatch.setattr(
        derouter, "callbacks", [_BlockOnSecretGuardrail(scan_raw_request=True), _PiiRedactor()]
    )
    proxy_logging.slack_alerting_instance = MagicMock(alerting=None)
    out = await proxy_logging.pre_call_hook(
        user_api_key_dict=make_user_api_key_auth(),
...
class _Unpicklable:
    ...
    safe_deep_copy exists to handle (see derouter_core_utils/core_helpers.py)."""
...
async def test_scan_raw_request_snapshot_survives_unpicklable_metadata(
    ...
    """
    Bugbot finding on Delify-Solutions/derouter#34940: the scan_raw_request snapshot
    used a bare copy.deepcopy, which raises on request payloads carrying
    unpicklable objects (e.g. metadata["derouter_parent_otel_span"] when
    ...
    """
    monkeypatch.setattr(derouter, "callbacks", [_BlockOnSecretGuardrail(scan_raw_request=True)])
    proxy_logging.slack_alerting_instance = MagicMock(alerting=None)
    data = {
        "messages": [{"role": "user", "content": "hello, nothing flagged here"}],
        "model": "m",
        "metadata": {"derouter_parent_otel_span": _Unpicklable()},
    }
    out = await proxy_logging.pre_call_hook(
        user_api_key_dict=make_user_api_key_auth(),
        data=data,
...
async def test_scan_raw_request_isolation_survives_unpicklable_top_level_field(
    ...
    """
    Bugbot finding on Delify-Solutions/derouter#34940: real proxy requests carry
    data["derouter_logging_obj"] (a Logging instance nesting a live OTel span
    ...
    passes (none of them set derouter_logging_obj). The isolation guarantee
    ...
    proxy_logging.slack_alerting_instance = MagicMock(alerting=None)
    data = _secret_request()
    data["derouter_logging_obj"] = _Unpicklable()
    with pytest.raises(HTTPException, match="blocked"):
        await proxy_logging.pre_call_hook(
            user_api_key_dict=make_user_api_key_auth(),
            data=data,
...
async def test_scan_raw_request_snapshot_taken_before_pipelines(
    ...
    """
    veria-ai finding on Delify-Solutions/derouter#34940: the raw snapshot was taken
...
async def test_scan_raw_request_warns_when_guardrail_mutation_discarded(
    ...
    """
    veria-ai finding on Delify-Solutions/derouter#34940: scan_raw_request is accepted
    ...
    from derouter.proxy import utils as proxy_utils_module

    mock_logger = MagicMock()
    monkeypatch.setattr(proxy_utils_module, "verbose_proxy_logger", mock_logger)
    monkeypatch.setattr(derouter, "callbacks", [_MutatingScanner()])
    proxy_logging.slack_alerting_instance = MagicMock(alerting=None)
    await proxy_logging.pre_call_hook(
        user_api_key_dict=make_user_api_key_auth(),
        data=_secret_request(),
...
async def test_scan_raw_request_baseline_does_not_leak_marker_under_safe_memory_mode(
    ...
    """
    veria-ai finding on Delify-Solutions/derouter#34940: safe_deep_copy returns the
    original object unchanged when derouter.safe_memory_mode is True, so
    ...
    """
    monkeypatch.setattr(derouter, "safe_memory_mode", True)
    ...
    monkeypatch.setattr(derouter, "callbacks", [callback])
    proxy_logging.slack_alerting_instance = MagicMock(alerting=None)
    out = await proxy_logging.pre_call_hook(
        user_api_key_dict=make_user_api_key_auth(),
        data=_secret_request(),
...
async def test_scan_raw_request_stamps_live_request_when_guardrail_actually_ran(
    ...
    """
    Bugbot finding on Delify-Solutions/derouter#34940: a scan_raw_request guardrail only
...
async def test_scan_raw_request_does_not_warn_when_guardrail_only_blocks(
    ...
    """
    Bugbot finding on Delify-Solutions/derouter#34940: _process_guardrail_callback always
...
async def test_scan_raw_request_warns_on_in_place_mutation_returning_none(
    ...
    mock_logger = MagicMock()
    monkeypatch.setattr(proxy_utils_module, "verbose_proxy_logger", mock_logger)
    monkeypatch.setattr(derouter, "callbacks", [_ScanningRedactor()])
    proxy_logging.slack_alerting_instance = MagicMock(alerting=None)
    await proxy_logging.pre_call_hook(
        user_api_key_dict=make_user_api_key_auth(),
        data=_secret_request(),
