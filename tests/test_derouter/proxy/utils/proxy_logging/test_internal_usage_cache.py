...
exactly the same arguments, mapping ``derouter_parent_otel_span`` to the
...
async def test_async_get_cache_forwards_args():
    ...
    result = await cache.async_get_cache(key="k", derouter_parent_otel_span="span", local_only=True, extra="x")
...
async def test_async_get_cache_propagates_underlying_error_raises():
    ...
    with pytest.raises(RuntimeError, match="redis down"):
        await cache.async_get_cache(key="k", derouter_parent_otel_span=None)
...
async def test_async_set_cache_forwards_args():
    ...
    await cache.async_set_cache(key="k", value="v", derouter_parent_otel_span="span", local_only=False, ttl=60)
...
async def test_async_set_cache_propagates_error_raises():
    ...
    with pytest.raises(ValueError, match="bad value"):
        await cache.async_set_cache(key="k", value="v", derouter_parent_otel_span=None)
...
async def test_async_batch_set_cache_forwards_pipeline():
    ...
    pairs = [("a", 1), ("b", 2)]
    await cache.async_batch_set_cache(cache_list=pairs, derouter_parent_otel_span=None, local_only=True, ttl=10)
...
async def test_async_batch_set_cache_propagates_error_raises():
    ...
    with pytest.raises(ConnectionError):
        await cache.async_batch_set_cache(cache_list=[], derouter_parent_otel_span=None)
...
async def test_async_increment_cache_forwards_args():
    ...
    cache = InternalUsageCache(dual_cache=inner)
    result = await cache.async_increment_cache(key="counter", value=1.5, derouter_parent_otel_span="span")
...
async def test_async_increment_cache_propagates_error_raises():
    ...
    with pytest.raises(OverflowError):
        await cache.async_increment_cache(key="x", value=1.0, derouter_parent_otel_span=None)
