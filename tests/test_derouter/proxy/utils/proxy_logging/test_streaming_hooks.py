...
``post_call_response_headers_hook``, ``_build_derouter_call_info``.
"""
...
# ---------------------------------------------------------------------------
# _build_derouter_call_info
# ---------------------------------------------------------------------------


def test_build_derouter_call_info_pulls_from_hidden_params_and_metadata(proxy_logging):
    response = MagicMock()
    ...
    info = proxy_logging._build_derouter_call_info(
...
def test_build_derouter_call_info_fallbacks_to_derouter_metadata(proxy_logging):
    response = MagicMock()
    response._hidden_params = {"custom_llm_provider": "azure"}
    info = proxy_logging._build_derouter_call_info(
        data={"derouter_metadata": {"model_info": {"alias": "azure-gpt"}}},
        response=response,
    )
    snapshot = {
...
def test_build_derouter_call_info_invalid_data_raises(proxy_logging):
    with pytest.raises(AttributeError):
        proxy_logging._build_derouter_call_info(data=None, response=MagicMock())  # type: ignore[arg-type]
...
async def test_init_response_taking_too_long_task_runs_when_alerting(proxy_logging):
    ...
    payload = {"req": "y", "derouter_call_id": "c1", "model": "m"}
...
async def test_async_post_call_streaming_hook_fast_path_returns_response(proxy_logging, mock_callbacks_disabled, make_user_api_key_auth):
    ...
    snapshot = {
        ...
        "callbacks_empty": len(derouter.callbacks) == 0,
    }
    assert snapshot == {
        "out_is_input": True,
        "out_value": "chunk-1",
...
async def test_async_post_call_streaming_hook_callback_error_raises(proxy_logging, make_user_api_key_auth, monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [_Per()])
...
async def test_async_post_call_streaming_iterator_hook_with_override_chains_callback(proxy_logging, make_user_api_key_auth, monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [_IterOverride()])
...
async def test_native_messages_stream_logging_fires_after_guardrail_end_of_stream_scan(
    ...
    monkeypatch.setattr(derouter, "callbacks", [_EndOfStreamScanGuardrail()])
...
async def test_native_messages_stream_logging_fires_when_guardrail_blocks_after_stream_end(
    ...
    monkeypatch.setattr(derouter, "callbacks", [_BlockingGuardrail()])
...
async def test_fire_deferred_stream_logging_fires_callback():
    ...
    ProxyLogging._fire_deferred_stream_logging(request_data={"derouter_logging_obj": logging_obj})
    await asyncio.sleep(0)
    snapshot = {
        "arg": captured["arg"],
...
async def test_post_call_response_headers_hook_merges_callback_headers(proxy_logging, make_user_api_key_auth, monkeypatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [_Cb(), _Cb2()])
    response = MagicMock()
    response._hidden_params = {}
    out = await proxy_logging.post_call_response_headers_hook(
