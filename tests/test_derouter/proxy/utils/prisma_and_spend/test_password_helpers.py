...
async def test_migrate_passwords_skips_when_no_plaintext() -> None:
    ...
    pc.db.derouter_usertable.find_many = AsyncMock(
        return_value=[
            _make_user("a", "scrypt:abc"),
            _make_user("b", sha),
        ]
    )
    pc.db.derouter_usertable.update = AsyncMock()

    result = await migrate_passwords_to_scrypt_async(pc)
    outcome = {
        "message": result,
        "updates": pc.db.derouter_usertable.update.await_count,
        "find_called": pc.db.derouter_usertable.find_many.await_count,
        "fetch_filter": pc.db.derouter_usertable.find_many.await_args.kwargs["where"],
    }
    assert outcome == {
...
async def test_migrate_passwords_upgrades_only_plaintext_rows() -> None:
    ...
    pc.db.derouter_usertable.find_many = AsyncMock(return_value=users)
    pc.db.derouter_usertable.update = AsyncMock()
    ...
    updated_user_ids = sorted(
        ...
        for call in pc.db.derouter_usertable.update.await_args_list
    )
    ...
    outcome = {
        "message": result,
        "update_count": pc.db.derouter_usertable.update.await_count,
