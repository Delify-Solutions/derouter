"""Pin the DeRouter_Config cached-read layer.
...
def _swap_config_cache(
    ...
    monkeypatch.setattr(utils_mod, "derouter_config_cache", mock_dual_cache)
...
def test_config_cache_key_uses_documented_prefix() -> None:
    ...
    assert actual == {
        "key": "derouter_config:param:max_budget",
        "another": "derouter_config:param:disable_spend_updates",
        "prefix": "derouter_config",
    }
...
async def test_invalidate_config_param_evicts_from_cache(
    ...
    assert actual == {
        "store_empty": True,
        "delete_calls": 1,
        "delete_arg": "derouter_config:param:p4",
    }
...
async def test_prefetch_config_params_populates_cache_for_each_name(
    ...
    prisma = MagicMock()
    prisma.db.derouter_config.find_many = AsyncMock(return_value=rows)
...
async def test_prefetch_config_params_empty_list_is_noop(
    ...
    prisma = MagicMock()
    prisma.db.derouter_config.find_many = AsyncMock(return_value=[])
    await prefetch_config_params(prisma, [])
    assert prisma.db.derouter_config.find_many.await_count == 0
...
async def test_prefetch_config_params_swallows_db_error_without_caching(
    ...
    prisma = MagicMock()
    prisma.db.derouter_config.find_many = AsyncMock(side_effect=RuntimeError("boom"))
