...
async def test_insert_data_hashes_token_and_upserts(prisma_client: PrismaClient) -> None:
    ...
    prisma_client.db.derouter_verificationtoken.upsert = AsyncMock(return_value=response)
    data = {
        "token": token,
        "user_id": "u1",
        "team_id": "t1",
    ...
    upsert_kwargs = prisma_client.db.derouter_verificationtoken.upsert.await_args.kwargs
    actual = {
        "returned": result,
...
async def test_insert_data_strips_null_budget_limits(prisma_client: PrismaClient) -> None:
    prisma_client.db.derouter_verificationtoken.upsert = AsyncMock(return_value=None)
    await prisma_client.insert_data(
        data={"token": "sk-1", "budget_limits": None}, table_name="key"
    )
    create_payload = prisma_client.db.derouter_verificationtoken.upsert.await_args.kwargs[
        "data"
    ]["create"]
...
async def test_insert_data_team_serializes_members(prisma_client: PrismaClient) -> None:
    prisma_client.db.derouter_teamtable.upsert = AsyncMock(
    ...
    create_payload = prisma_client.db.derouter_teamtable.upsert.await_args.kwargs["data"][
        "create"
    ]
...
async def test_insert_data_user_organization_fk_raises_400(
    ...
    prisma_client.db.derouter_usertable.upsert = AsyncMock(side_effect=err)
...
async def test_insert_data_debug_log_hashes_token(
    ...
    expected_hash = hashlib.sha256(token.encode()).hexdigest()
    prisma_client.db.derouter_verificationtoken.upsert = AsyncMock(return_value=SimpleNamespace(token=expected_hash))
...
async def test_insert_data_debug_log_tolerates_none_token(
    ...
    prisma_client.db.derouter_usertable.upsert = AsyncMock(return_value=SimpleNamespace(user_id="u1"))
    with caplog.at_level(logging.DEBUG, logger="DeRouter Proxy"):
...
async def test_insert_data_logs_and_raises_generic_error(
    ...
    prisma_client.db.derouter_verificationtoken.upsert = AsyncMock(
...
async def test_update_data_token_hashes_and_updates(
    ...
    prisma_client.db.derouter_verificationtoken.update = AsyncMock(return_value=response)
    result = await prisma_client.update_data(
        token=token,
        data={"spend": 1.0},
    )
    update_kwargs = prisma_client.db.derouter_verificationtoken.update.await_args.kwargs
...
async def test_update_data_user_upsert_returns_user_envelope(
    ...
    prisma_client.db.derouter_usertable.upsert = AsyncMock(return_value=row)
    result = await prisma_client.update_data(
...
async def test_update_data_team_serializes_members_when_list(
    ...
    prisma_client.db.derouter_teamtable.upsert = AsyncMock(return_value=row)
    ...
    upsert_kwargs = prisma_client.db.derouter_teamtable.upsert.await_args.kwargs
    actual = {
...
async def test_update_data_logs_and_raises_on_error(
    ...
    prisma_client.db.derouter_verificationtoken.update = AsyncMock(
...
async def test_delete_data_hashes_sk_tokens_and_calls_delete_many(
    ...
    prisma_client.db.derouter_verificationtoken.delete_many = AsyncMock(
        return_value=deleted
    )
    tokens = ["sk-one", "sk-two", "raw-hashed-token"]
    result = await prisma_client.delete_data(tokens=tokens)
    where = prisma_client.db.derouter_verificationtoken.delete_many.await_args.kwargs[
        "where"
    ]
    ...
    actual = {
        ...
        "delete_call_count": prisma_client.db.derouter_verificationtoken.delete_many.await_count,
    }
    assert actual == {
...
async def test_delete_data_team_calls_team_delete_many(
    ...
    prisma_client.db.derouter_teamtable.delete_many = AsyncMock()
    result = await prisma_client.delete_data(
        team_id_list=["t1", "t2"], table_name="team"
    )
    where = prisma_client.db.derouter_teamtable.delete_many.await_args.kwargs["where"]
