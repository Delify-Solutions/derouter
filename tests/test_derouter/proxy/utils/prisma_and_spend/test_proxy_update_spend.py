...
from derouter.proxy.db.spend_log_batching import spend_log_row_bytes
from derouter.proxy.utils import PrismaClient, ProxyUpdateSpend, enqueue_spend_logs
...
async def test_update_end_user_spend_upserts_each_end_user(
    ...
    batcher = MagicMock()
    batcher.derouter_endusertable.upsert = MagicMock()
    ...
    calls = batcher.derouter_endusertable.upsert.call_args_list
...
def _end_user_deadlock_error() -> Exception:
    ...
    return RawQueryError(data={"user_facing_error": {"error_code": "P2034", "meta": {"table": "DeRouter_EndUserTable"}}})
...
async def test_update_end_user_spend_retries_on_deadlock_then_commits(
    ...
    batcher.derouter_endusertable.upsert.assert_called_once()
    assert batcher.derouter_endusertable.upsert.call_args.kwargs["where"] == {"user_id": "end-user-1"}
    assert len(sleeps) == 1
    assert 1.0 <= sleeps[0] <= 2.0
...
async def test_update_spend_logs_writes_batches_via_create_many(
    ...
    mock_prisma_client.db.derouter_spendlogs.create_many = AsyncMock()
    proxy_logging = MagicMock()
    proxy_logging.failure_handler = AsyncMock()
    ...
    kwargs = mock_prisma_client.db.derouter_spendlogs.create_many.await_args.kwargs
    pinned = {
        "calls": mock_prisma_client.db.derouter_spendlogs.create_many.await_count,
...
async def test_update_spend_logs_bounds_each_statement_by_payload_bytes(
    ...
    calls = mock_prisma_client.db.derouter_spendlogs.create_many.await_args_list
...
async def test_update_spend_logs_uses_spend_logs_url_when_set(
    ...
    pinned = {
        ...
        "create_many_calls": mock_prisma_client.db.derouter_spendlogs.create_many.await_count,
    }
    assert pinned == {
        "post_calls": 1,
...
async def test_update_spend_logs_pops_logs_when_logs_to_process_is_none(
    ...
    assert mock_prisma_client.db.derouter_spendlogs.create_many.await_count == 1
...
async def test_update_spend_logs_failure_raises_after_retries(
    ...
    mock_prisma_client.db.derouter_spendlogs.create_many = AsyncMock(side_effect=httpx.ReadError("network blip"))
    proxy_logging = MagicMock()
    proxy_logging.failure_handler = AsyncMock()
...
async def test_update_spend_logs_isolates_poison_row_and_persists_good_rows(
    ...
    mock_prisma_client.db.derouter_spendlogs.create_many = AsyncMock(side_effect=_create_many)
    proxy_logging = MagicMock()
    proxy_logging.failure_handler = AsyncMock()
...
async def test_update_spend_logs_reraises_connection_masquerade_dataerror(
    ...
    mock_prisma_client.db.derouter_spendlogs.create_many = AsyncMock(side_effect=err)
    proxy_logging = MagicMock()
    proxy_logging.failure_handler = AsyncMock()
...
async def test_update_spend_logs_retries_and_requeues_batch_on_db_outage(
    ...
    assert mock_prisma_client.db.derouter_spendlogs.create_many.await_count == 3
...
async def test_update_spend_logs_retries_deadlock_and_keeps_every_row(
    ...
    mock_prisma_client.db.derouter_spendlogs.create_many = create_many
    proxy_logging = MagicMock()
    proxy_logging.failure_handler = AsyncMock()
    mock_prisma_client.spend_log_transactions = []
...
async def test_update_spend_logs_requeues_batch_once_deadlock_retries_exhaust(
    ...
    monkeypatch.setattr(utils_mod.asyncio, "sleep", _fake_sleep)
    mock_prisma_client.db.derouter_spendlogs.create_many = AsyncMock(side_effect=_deadlock_error())
    proxy_logging = MagicMock()
    proxy_logging.failure_handler = AsyncMock()
    ...
    assert mock_prisma_client.db.derouter_spendlogs.create_many.await_count == 2
    assert [row["request_id"] for row in mock_prisma_client.spend_log_transactions] == ["a", "b", "c"]
...
async def test_update_spend_logs_does_not_requeue_non_transport_failures(
    ...
    """
    mock_prisma_client.db.derouter_spendlogs.create_many = AsyncMock(side_effect=ValueError("bad payload"))
    proxy_logging = MagicMock()
    proxy_logging.failure_handler = AsyncMock()
    mock_prisma_client.spend_log_transactions = []
...
async def test_update_spend_logs_caps_isolation_attempts_under_poison_flood(
    ...
    mock_prisma_client.db.derouter_spendlogs.create_many = AsyncMock(side_effect=_always_poison)
    proxy_logging = MagicMock()
    proxy_logging.failure_handler = AsyncMock()
    ...
    attempts = mock_prisma_client.db.derouter_spendlogs.create_many.await_count
...
async def _flush_and_count_create_many(
    ...
    return int(mock_prisma_client.db.derouter_spendlogs.create_many.await_count)
