...
async def test_update_spend_logs_job_skips_when_queue_empty(
    ...
    assert mock_prisma_client.db.derouter_spendlogs.create_many.await_count == 0
...
async def test_update_spend_logs_job_drains_tool_queue_when_spend_queue_empty(
    ...
    import derouter.proxy.db.spend_log_tool_index as tool_mod
    import derouter.proxy.guardrails.usage_tracking as guard_mod
...
async def test_update_spend_logs_job_processes_and_clears_queue(
    ...
    pinned = {
        ...
        "first_data_request_id": mock_prisma_client.db.derouter_spendlogs.create_many.await_args.kwargs[
            "data"
        ][0]["request_id"],
        "skip_duplicates_set": mock_prisma_client.db.derouter_spendlogs.create_many.await_args.kwargs[
            "skip_duplicates"
        ],
    }
    assert pinned == {
...
async def test_drain_spend_logs_queue_flushes_rows_queued_while_draining(
    ...
    mock_prisma_client.db.derouter_spendlogs.create_many = AsyncMock(side_effect=_write)
...
async def test_drain_spend_logs_queue_gives_up_after_max_passes(
    ...
    assert (
        mock_prisma_client.db.derouter_spendlogs.create_many.await_count
        == MAX_SPEND_LOG_DRAIN_ITERATIONS
    )
...
async def test_monitor_spend_logs_queue_invokes_job_when_queue_nonempty(
    ...
    import derouter.proxy.utils as utils_mod
    import derouter.constants as constants_mod
...
async def test_monitor_spend_logs_queue_flushes_as_soon_as_one_is_requested(
    ...
    import derouter.constants as constants_mod
    import derouter.proxy.utils as utils_mod
    from derouter.proxy.utils import request_spend_log_flush
