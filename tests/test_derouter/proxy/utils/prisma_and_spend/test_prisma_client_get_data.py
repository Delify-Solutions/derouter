...
async def test_get_generic_data_dispatches_by_table(
    ...
    prisma_client.db.derouter_usertable.find_first = AsyncMock(return_value=row)
    result = await prisma_client.get_generic_data(
        key="user_id", value="u1", table_name="users"
    )
    actual = {
        "result_is_row": result is row,
        "find_first_count": prisma_client.db.derouter_usertable.find_first.await_count,
        "where_kwarg": prisma_client.db.derouter_usertable.find_first.await_args.kwargs[
            "where"
        ],
...
async def test_get_generic_data_logs_failure_handler_and_raises_on_error(
    ...
    prisma_client.db.derouter_usertable.find_first = AsyncMock(
...
async def test_query_first_with_cached_plan_fallback_reconnects_then_retries_identical_query(
    ...
    original_query = 'SELECT * FROM "DeRouter_VerificationToken" WHERE v.token = $1'
    expected = {"token": "abc", "team_spend": 1.0, "team_max_budget": 5.0}
    manager = MagicMock()
    ...
    # https://github.com/Delify-Solutions/derouter/issues/36418: without this the healthy
...
async def test_check_view_exists_noop_when_all_views_present(
    ...
    actual = {
        ...
        "view_query_contains_token_view": "DeRouter_VerificationTokenView"
...
async def test_check_view_exists_creates_token_view_when_missing(
    ...
    actual = {
        ...
        .strip()
        .startswith('CREATE VIEW "DeRouter_VerificationTokenView"'),
    }
    assert actual == {
        "result": None,
        "create_called": 1,
...
async def test_get_data_token_find_unique_returns_record(
    ...
    prisma_client.db.derouter_verificationtoken.find_unique = AsyncMock(
        return_value=record
    )
    ...
    actual = {
        ...
        "where_arg": prisma_client.db.derouter_verificationtoken.find_unique.await_args.kwargs[
            "where"
        ],
        "include_arg": prisma_client.db.derouter_verificationtoken.find_unique.await_args.kwargs[
            "include"
        ],
    ...
    assert actual == {
        ...
        "include_arg": {"derouter_budget_table": True},
...
async def test_get_data_user_find_unique_returns_user_row(
    ...
    prisma_client.db.derouter_usertable.find_unique = AsyncMock(return_value=row)
    result = await prisma_client.get_data(user_id="u-7", table_name="user")
    actual = {
        "result_is_row": result is row,
        "where_arg": prisma_client.db.derouter_usertable.find_unique.await_args.kwargs[
            "where"
        ],
        "include_arg": prisma_client.db.derouter_usertable.find_unique.await_args.kwargs[
            "include"
        ],
        "spend": row.spend,
    }
...
async def test_get_data_combined_view_returns_view_for_deprecated_key(
    ...
    prisma_client.db.derouter_deprecatedverificationtoken = MagicMock()
    prisma_client.db.derouter_deprecatedverificationtoken.find_first = AsyncMock(
    ...
    assert isinstance(response, DeRouter_VerificationTokenView)
...
async def test_get_data_team_keys_forward_limit_as_take(
    ...
    """
    prisma_client.db.derouter_verificationtoken.find_many = AsyncMock(return_value=[])
    ...
    assert prisma_client.db.derouter_verificationtoken.find_many.await_args.kwargs == {
        "take": limit,
...
async def test_query_first_with_cached_plan_fallback_reports_pre_query_engine_generation(
    ...
    (https://github.com/Delify-Solutions/derouter/issues/36418). Reading it after the
...
async def test_query_first_with_cached_plan_fallback_reports_the_reader_generation(
    ...
    poisoned (https://github.com/Delify-Solutions/derouter/issues/36418). The two
