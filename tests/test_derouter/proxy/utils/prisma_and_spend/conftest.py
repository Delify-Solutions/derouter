"""Shared fixtures for tests/test_derouter/proxy/utils/prisma_and_spend/.
...
_PRISMA_TABLES: List[str] = [
    ...
    "derouter_spendlogs",
    "derouter_config",
    "derouter_usernotifications",
    "derouter_healthchecktable",
...
def mock_prisma_client() -> MagicMock:
    """Bare ``db`` mock with all common DeRouter_* tables stubbed.

    Override individual return values in a test::

        mock_prisma_client.db.derouter_usertable.find_unique.return_value = user
    """
...
def patched_prisma_import(monkeypatch: pytest.MonkeyPatch) -> Iterator[MagicMock]:
    ...
    import derouter.proxy.utils as _utils_mod
...
def make_spend_log_row() -> Callable[..., Dict[str, Any]]:
    """Factory for fake DeRouter_SpendLogs rows."""
