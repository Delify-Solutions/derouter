...
def _router_with_deployment(guardrails, *, by_alias: bool = False):
    ...
    deployment = SimpleNamespace(derouter_params={"guardrails": guardrails})
    router = MagicMock()
    router.get_deployment.return_value = deployment
    router.get_model_list.return_value = [{"derouter_params": {"guardrails": guardrails}}] if by_alias else []
    return router
...
def _router_with_deployments(group_guardrails):
    ...
    router.get_model_list.return_value = [
        {"derouter_params": {"guardrails": g} if g is not None else {}} for g in group_guardrails
    ]
    return router
...
def test_check_and_merge_model_level_guardrails_ignores_client_model_info_id_when_distrusted():
    ...
    add_derouter_data_to_request preserves the client-supplied
    ...
    spoofed_deployment = SimpleNamespace(derouter_params={"guardrails": []})
    router.get_deployment.return_value = spoofed_deployment
    # The real alias group has a guarded deployment.
    router.get_model_list.return_value = [{"derouter_params": {"guardrails": ["alias-secret-scan"]}}]
...
def test_check_and_merge_model_level_guardrails_post_call_accepts_bare_string_guardrail():
    ...
    deployment = SimpleNamespace(derouter_params={"guardrails": "scalar-guardrail"})
    router = MagicMock()
    router.get_deployment.return_value = deployment
...
def test_check_and_merge_model_level_guardrails_alias_union_accepts_bare_string_guardrail():
    ...
    router = MagicMock()
    router.get_deployment.return_value = None
    router.get_model_list.return_value = [{"derouter_params": {"guardrails": "scalar-alias-guardrail"}}]
...
def test_check_and_merge_model_level_guardrails_alias_fallback_passes_team_id():
    ...
    router = MagicMock()
    router.get_deployment.return_value = None
    router.get_model_list.return_value = [{"derouter_params": {"guardrails": ["team-guardrail"]}}]
    data = {
...
def test_check_and_merge_model_level_guardrails_alias_fallback_reads_team_id_from_derouter_metadata():
    """Backstop: some call sites stash the team id on derouter_metadata
    ...
    data = {
        "model": "alias-m",
        "metadata": {"model_info": {}},
        "derouter_metadata": {"user_api_key_team_id": "team-xyz"},
    }
