...
def test_is_known_vector_store_index_happy_path(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "vector_store_index_registry", registry)
    summary = {
...
def test_is_known_vector_store_index_error_path_no_registry(monkeypatch):
    monkeypatch.setattr(derouter, "vector_store_index_registry", None)
...
def test_create_model_info_response_with_metadata_default_general(monkeypatch):
    monkeypatch.setattr(
        "derouter.proxy.auth.model_checks.get_all_fallbacks",
...
def test_create_model_info_response_with_explicit_fallback_type(monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.auth.model_checks.get_all_fallbacks", _capture)
    result = create_model_info_response(
        model_id="gpt-4o",
        provider="openai",
        include_metadata=True,
...
async def test_get_available_models_for_user_happy_path_returns_complete_list(
    ...
    monkeypatch.setattr(
        "derouter.proxy.auth.model_checks.get_key_models",
        lambda **_k: ["gpt-4o"],
    )
    monkeypatch.setattr(
        "derouter.proxy.auth.model_checks.get_team_models",
        lambda **_k: ["claude-haiku"],
    )
    monkeypatch.setattr(
        "derouter.proxy.auth.model_checks.get_complete_model_list",
...
async def test_get_available_models_for_user_error_path_complete_list_raises(
    ...
    monkeypatch.setattr(
        "derouter.proxy.auth.model_checks.get_complete_model_list", _boom
    )
    user_api_key_dict = UserAPIKeyAuth(
        api_key="sk-test-key",
        user_id="user-1",
...
async def test_get_available_models_for_user_resolves_team_access_group_models(
    ...
    access_group = DeRouter_AccessGroupTable(
    ...
    monkeypatch.setattr("derouter.proxy.auth.auth_checks.get_team_object", _get_team_object)
    monkeypatch.setattr("derouter.proxy.auth.auth_checks.get_access_object", _get_access_object)
...
async def test_get_available_models_for_user_without_access_groups_grants_nothing(
    ...
    async def _get_team_object(**_kwargs):
        return DeRouter_TeamTableCachedObj(team_id="team-1", models=["no-default-models"])
