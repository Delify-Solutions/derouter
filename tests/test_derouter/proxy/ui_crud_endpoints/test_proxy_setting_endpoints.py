...
from derouter.proxy._types import DefaultInternalUserParams, LitellmUserRoles
...
class TestProxySettingEndpoints:
    def test_get_internal_user_settings(self, mock_proxy_config, mock_auth):
        ...
        values = data["values"]
        mock_params = mock_proxy_config["config"]["derouter_settings"][
            "default_internal_user_params"
        ]
    ...
    def test_update_internal_user_settings(
        ...
        # Mock derouter.default_internal_user_params

        import derouter

        monkeypatch.setattr("derouter.proxy.proxy_server.store_model_in_db", True)
        monkeypatch.setattr(derouter, "default_internal_user_params", {})
        ...
        updated_config = mock_proxy_config["config"]["derouter_settings"][
            "default_internal_user_params"
        ]
    ...
    def test_update_default_team_settings(
        ...
        # Mock derouter.default_team_params
        import derouter

        monkeypatch.setattr("derouter.proxy.proxy_server.store_model_in_db", True)
        monkeypatch.setattr(derouter, "default_team_params", {})
    ...
    def test_get_sso_settings(self, mock_proxy_config, mock_auth, monkeypatch):
        ...
        mock_prisma.db.derouter_ssoconfig.find_unique.assert_called_once()
        call_args = mock_prisma.db.derouter_ssoconfig.find_unique.call_args
    ...
    def _mock_sso_db_record(self, monkeypatch, sso_settings):
        ...
        mock_prisma.db.derouter_ssoconfig.find_unique = AsyncMock(return_value=mock_db_record)
        monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma)

        # The resolver decrypts stored values via decrypt_value_helper; make it an
        # identity so the plaintext fixtures round-trip.
        monkeypatch.setattr(
            "derouter.proxy.config_resolvers.sso.decrypt_value_helper",
    ...
    def test_update_sso_settings(self, mock_proxy_config, mock_auth, monkeypatch):
        ...
        monkeypatch.setenv("DEROUTER_SALT_KEY", "test_salt_key")
        monkeypatch.setattr("derouter.proxy.proxy_server.store_model_in_db", True)

        # Mock the prisma client
        mock_prisma = MagicMock()
        mock_prisma.db.derouter_ssoconfig.find_unique = AsyncMock(return_value=None)
        mock_prisma.db.derouter_ssoconfig.upsert = AsyncMock()
        mock_prisma.db.derouter_config = MagicMock()
        mock_prisma.db.derouter_config.find_unique = AsyncMock(return_value=None)
        mock_prisma.db.derouter_config.update = AsyncMock()
        monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma)
        ...
        assert mock_prisma.db.derouter_ssoconfig.upsert.called
        call_args = mock_prisma.db.derouter_ssoconfig.upsert.call_args
    ...
    def test_update_sso_settings_maps_saml_fields_to_env_vars(
        ...
        try:
            ...
            stored = json.loads(
                mock_prisma.db.derouter_ssoconfig.upsert.call_args.kwargs["data"]["create"]["sso_settings"]
            )
    ...
    def test_update_sso_settings_audits_when_env_cleanup_fails(
        ...
        monkeypatch.setattr(
            "derouter.proxy.proxy_server.create_config_audit_log",
        ...
        assert (
            json.loads(
                mock_prisma.db.derouter_ssoconfig.upsert.call_args.kwargs["data"][
                    "create"
                ]["sso_settings"]
            )["google_client_id"]
            == "new_google_client_id"
    ...
    def test_update_sso_settings_ui_access_mode_handling(
        ...
        assert mock_prisma.db.derouter_ssoconfig.upsert.call_count == 2
        call_args = mock_prisma.db.derouter_ssoconfig.upsert.call_args
        create_data = call_args.kwargs["data"]["create"]
        create_sso_settings = json.loads(create_data["sso_settings"])
    ...
    def test_update_ui_theme_settings_with_favicon(
        ...
        assert os.environ["UI_LOGO_PATH"] == "https://example.com/new-logo.png"
        assert os.environ["DEROUTER_FAVICON_URL"] == "https://example.com/custom-favicon.ico"
        # Only the owned keys are persisted, each with its new value
        assert mock_proxy_config["env_updates"]() == [
            {
                "UI_LOGO_PATH": "https://example.com/new-logo.png",
                "UI_LOGO_PATH_DARK": None,
                "DEROUTER_FAVICON_URL": "https://example.com/custom-favicon.ico",
            }
        ]
    ...
    def test_update_ui_theme_settings_clear_favicon(
        ...
        data = response.json()
        assert data["status"] == "success"
        assert "DEROUTER_FAVICON_URL" not in os.environ
    ...
    def test_get_ui_theme_settings_with_favicon_configured(self, mock_proxy_config):
        ...
        mock_proxy_config["config"]["derouter_settings"]["ui_theme_config"] = {
            "logo_url": "https://example.com/logo.png",
    ...
    def test_get_ui_theme_settings_falls_back_to_process_env(
        ...
        A deployment that sets UI_LOGO_PATH / DEROUTER_FAVICON_URL via IaC and
        ...
        monkeypatch.delenv("DEROUTER_FAVICON_URL", raising=False)
        monkeypatch.setenv("UI_LOGO_PATH", "https://cdn.example.com/logo.png")
        monkeypatch.setenv("DEROUTER_FAVICON_URL", "https://cdn.example.com/favicon.ico")
    ...
    def test_get_ui_theme_settings_stored_value_wins_over_env(
        ...
        monkeypatch.setenv("DEROUTER_FAVICON_URL", "https://env.example.com/favicon.ico")
    ...
    def test_get_ui_theme_settings_does_not_disclose_local_path_env_value(
        ...
        monkeypatch.setenv("DEROUTER_FAVICON_URL", "file:///etc/favicon.ico")
    ...
    def test_get_ui_settings(self, mock_auth, monkeypatch):
        ...
        mock_prisma.db.derouter_uisettings.find_unique = AsyncMock(
            return_value=mock_db_record
        )
        monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma)
        ...
        mock_prisma.db.derouter_uisettings.find_unique.assert_called_once_with(
    ...
    def test_get_ui_settings_schema_description_preserved_with_extensions(
        ...
        from derouter.proxy.ui_crud_endpoints import proxy_setting_endpoints
        from derouter.proxy.ui_crud_endpoints.proxy_setting_endpoints import (
        ...
        try:
            ...
            mock_prisma = MagicMock()
            mock_prisma.db.derouter_uisettings.find_unique = AsyncMock(return_value=None)
            monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma)
    ...
    def test_get_ui_settings_allows_internal_roles(self, monkeypatch, user_role):
        ...
        class MockUser:
            def __init__(self, role):
                self.user_role = role
                self.team_id = "derouter-dashboard"
                self.allowed_routes = []
    ...
    def test_update_ui_settings_allowlisted_value(self, mock_auth, monkeypatch):
        ...
        monkeypatch.setattr("derouter.proxy.proxy_server.store_model_in_db", True)
        mock_prisma = MagicMock()
        mock_prisma.db.derouter_uisettings.upsert = AsyncMock()
        mock_prisma.db.derouter_uisettings.find_unique = AsyncMock(return_value=None)
        monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma)
        ...
        assert mock_prisma.db.derouter_uisettings.upsert.called
        call_args = mock_prisma.db.derouter_uisettings.upsert.call_args
    ...
    def test_update_ui_settings_persists_forward_llm_provider_auth_headers(
        ...
        """BYOK flag must be allowlisted and persisted to derouter_uisettings."""
        from unittest.mock import AsyncMock, MagicMock
    ...
    def test_update_ui_settings_syncs_forward_llm_provider_auth_headers_to_general_settings(
        ...
        monkeypatch.setattr(
            "derouter.proxy.proxy_server.general_settings", general_settings
        )
    ...
    def test_get_sso_settings_from_database(
        ...
        monkeypatch.setattr(
            "derouter.proxy.config_resolvers.sso.decrypt_value_helper", mock_decrypt
        )
    ...
    def test_update_sso_settings_to_database(
        ...
        mock_prisma.db.derouter_ssoconfig.find_unique = AsyncMock(return_value=None)
        mock_prisma.db.derouter_ssoconfig.upsert = upsert_mock
        mock_prisma.db.derouter_config = MagicMock()
        mock_prisma.db.derouter_config.find_unique = AsyncMock(return_value=None)
        mock_prisma.db.derouter_config.update = AsyncMock()
    ...
    def test_update_sso_settings_removes_sso_env_vars_from_config(
        ...
        mock_prisma.db.derouter_ssoconfig = MagicMock()
        mock_prisma.db.derouter_ssoconfig.find_unique = AsyncMock(return_value=None)
        mock_prisma.db.derouter_ssoconfig.upsert = AsyncMock()
        ...
        assert response.status_code == 200
        mock_prisma.db.derouter_config.find_unique.assert_called_once()
        mock_prisma.db.derouter_config.update.assert_called_once()
        update_call = mock_prisma.db.derouter_config.update.call_args
    ...
    def test_setup_role_mappings_custom_logic_with_env_vars(self, monkeypatch):
        ...
        from derouter.proxy._types import LitellmUserRoles
        from derouter.proxy.management_endpoints.ui_sso import _setup_role_mappings
...
def test_update_internal_user_settings_writes_audit_log(mock_proxy_config, monkeypatch):
    ...
    fake_prisma.db.derouter_auditlog.create = audit_create
...
def test_update_sso_settings_writes_redacted_audit_log(mock_proxy_config, monkeypatch):
    ...
    fake_prisma = MagicMock()
    fake_prisma.db.derouter_auditlog.create = audit_create
    fake_prisma.db.derouter_ssoconfig.upsert = AsyncMock()
    # No prior SSO row, so before_value resolves to None.
    fake_prisma.db.derouter_ssoconfig.find_unique = AsyncMock(return_value=None)
    fake_prisma.db.derouter_config.find_unique = AsyncMock(return_value=None)
    ...
    try:
        ...
        assert written["table_name"] == "DeRouter_SSOConfig"
        assert written["changed_by"] == "audit-admin"
...
def test_update_sso_settings_audit_captures_redacted_before_snapshot(
    ...
    fake_prisma.db.derouter_ssoconfig.find_unique = AsyncMock(return_value=existing_record)
    fake_prisma.db.derouter_config.find_unique = AsyncMock(return_value=None)
...
def test_update_ui_settings_writes_audit_log(monkeypatch):
    ...
    fake_prisma = MagicMock()
    fake_prisma.db.derouter_auditlog.create = audit_create
    fake_prisma.db.derouter_uisettings.find_unique = AsyncMock(return_value=None)
    fake_prisma.db.derouter_uisettings.upsert = AsyncMock()
    ...
    try:
        ...
        assert written["table_name"] == "DeRouter_UISettings"
        assert written["changed_by"] == "audit-admin"
        after = json.loads(written["updated_values"])
...
def mock_team_lookup(monkeypatch):
    ...
    fake_prisma = MagicMock()
    fake_prisma.db.derouter_teamtable.find_many = find_many
    ...
    monkeypatch.setattr(derouter, "default_internal_user_params", {})
    monkeypatch.setattr(
        "derouter.proxy.ui_crud_endpoints.proxy_setting_endpoints.update_default_team_member_budget",
...
def mock_organization_lookup(monkeypatch):
    ...
    fake_prisma = MagicMock()
    fake_prisma.db.derouter_organizationtable.find_unique = find_unique
...
def test_update_default_team_settings_rejects_unknown_organization(
    ...
    assert derouter.default_team_params == {}
...
def test_update_default_team_settings_saves_when_organization_exists(
    ...
    assert derouter.default_team_params["organization_id"] == "real-org"
...
class TestMcpToolSearchSettingsEndpoints:
    """`derouter_settings.mcp_tool_search` drives the native `mcp_tool_search` virtual tool, so the UI must round-trip it."""
    ...
    def test_get_returns_stored_values_and_field_schema(self, mock_proxy_config, mock_auth, monkeypatch):
        monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", object())
        mock_proxy_config["config"]["derouter_settings"]["mcp_tool_search"] = {
    ...
    def test_update_persists_and_applies_in_memory(self, mock_proxy_config, monkeypatch):
        ...
        monkeypatch.setattr("derouter.proxy.proxy_server.store_model_in_db", True)
        monkeypatch.setattr(derouter, "mcp_tool_search", None)
        ...
        assert resp.status_code == 200, resp.text
        assert mock_proxy_config["save_call_count"]() == 1
        assert derouter.mcp_tool_search == payload
        assert mock_proxy_config["config"]["derouter_settings"]["mcp_tool_search"] == payload
...
class TestPtuCostAttributionUISetting:
    ...
    def _mock_prisma(monkeypatch, stored=None):
        ...
        mock_prisma.db.derouter_uisettings.find_unique = AsyncMock(return_value=mock_record)
        mock_prisma.db.derouter_uisettings.upsert = AsyncMock()
        monkeypatch.setattr("derouter.proxy.proxy_server.prisma_client", mock_prisma)
        return mock_prisma
    ...
    def test_a_co_submitted_setting_still_applies_alongside_the_derived_key(self, mock_auth, monkeypatch):
        ...
        assert response.status_code == 200
        upsert_data = mock_prisma.db.derouter_uisettings.upsert.call_args.kwargs["data"]
    ...
    def test_patch_rejects_the_derived_setting(self, mock_auth, monkeypatch):
        ...
        assert not mock_prisma.db.derouter_uisettings.upsert.called
