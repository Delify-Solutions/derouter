...
def mock_audit_log(monkeypatch):
    ...
    monkeypatch.setattr("derouter.proxy.proxy_server.create_config_audit_log", audit_mock)
    return audit_mock


def _mock_prisma(monkeypatch, record=None):
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_uisettings.find_unique = AsyncMock(return_value=record)
...
class TestUpdateUserBanner:
    def test_rejects_non_admin(self, internal_user_auth, monkeypatch):
        ...
        mock_prisma.db.derouter_uisettings.upsert.assert_not_awaited()
    ...
    def test_persists_and_round_trips(self, admin_auth, monkeypatch, mock_audit_log):
        ...
        upsert_kwargs = mock_prisma.db.derouter_uisettings.upsert.await_args.kwargs
