...
async def test_regeneration_repoint_update_runs_on_the_writer():
    ...
    assert writer_inner.query_raw.await_args.args[0].startswith('UPDATE "DeRouter_AccessGroupTable"')
...
async def test_membership_attach_and_detach_updates_run_on_the_writer():
    ...
    assert all(
        call.args[0].startswith('UPDATE "DeRouter_AccessGroupTable"') for call in writer_inner.query_raw.await_args_list
    )
    reader_inner.query_raw.assert_not_awaited()
