...
from derouter.proxy._types import DeRouter_AuditLogs, LitellmTableNames
...
from derouter.types.utils import StandardAuditLogPayload
...
def reset_audit_log_callbacks(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(derouter, "audit_log_callbacks", [])
...
def test_is_audit_logging_enabled_precedence(
    ...
    monkeypatch.setattr(derouter, "store_audit_logs", configured_value)
    monkeypatch.setattr("derouter.proxy.proxy_server.premium_user", premium_user)
    if environment_value is None:
        monkeypatch.delenv("DEROUTER_STORE_AUDIT_LOGS", raising=False)
    else:
        monkeypatch.setenv("DEROUTER_STORE_AUDIT_LOGS", environment_value)
...
class TestBuildAuditLogPayload:
    def test_builds_correct_payload(self):
        ...
        assert payload["table_name"] == "DeRouter_TeamTable"
...
class TestDispatchAuditLogToCallbacks:
    ...
    async def test_dispatches_to_custom_logger_instance(self, monkeypatch: pytest.MonkeyPatch):
        ...
        monkeypatch.setattr(derouter, "audit_log_callbacks", [mock_logger])
    ...
    async def test_resolves_string_callback(self, monkeypatch: pytest.MonkeyPatch):
        ...
        monkeypatch.setattr(derouter, "audit_log_callbacks", ["s3_v2"])

        with patch(
            "derouter.proxy.management_helpers.audit_logs._resolve_audit_log_callback",
    ...
    async def test_skips_unresolvable_string_callback(self, monkeypatch: pytest.MonkeyPatch):
        monkeypatch.setattr(derouter, "audit_log_callbacks", ["nonexistent_callback"])
...
class TestCreateAuditLogForUpdateWithCallbacks:
    ...
    async def test_dispatches_to_callbacks_after_db_write(self, monkeypatch: pytest.MonkeyPatch):
        ...
        with (
            ...
            mock_prisma.db.derouter_auditlog.create.assert_called_once()
    ...
    async def test_no_dispatch_when_not_premium(self, monkeypatch: pytest.MonkeyPatch):
        ...
        with (
            ...
            mock_prisma.db.derouter_auditlog.create.assert_not_called()
    ...
    async def test_no_dispatch_when_store_audit_logs_false(self, monkeypatch: pytest.MonkeyPatch):
        ...
        with patch("derouter.store_audit_logs", False):
            audit_log = _make_audit_log()
            await create_audit_log_for_update(audit_log)
            await asyncio.sleep(0.1)
...
class TestAuditLogTaskDoneCallback:
    def test_logs_exception_from_failed_task(self):
        ...
        with patch(
            "derouter.proxy.management_helpers.audit_logs.verbose_proxy_logger"
        ) as mock_logger:
...
class TestS3LoggerAuditLogEvent:
    ...
    async def test_queues_audit_log_with_correct_s3_key(self):
        with patch("derouter.integrations.s3_v2.S3Logger.__init__", return_value=None):
    ...
    async def test_s3_key_format_no_path(self):
        with patch("derouter.integrations.s3_v2.S3Logger.__init__", return_value=None):
            ...
            audit_log = StandardAuditLogPayload(
                ...
                changed_by_api_key="sk-abc",
                action="deleted",
                table_name="DeRouter_VerificationToken",
                object_id="key-1",
                before_value=None,
                updated_values=None,
            )
...
class TestS3AuditCallbackParamsDecoupling:
    ...
    def _isolate_caches_and_globals(self, monkeypatch: pytest.MonkeyPatch):
        from derouter.derouter_core_utils import derouter_logging as ll_logging
        from derouter.proxy.management_helpers import audit_logs as ll_audit_logs

        monkeypatch.setattr(derouter, "s3_callback_params", derouter.s3_callback_params)
        monkeypatch.setattr(
            derouter, "s3_audit_callback_params", getattr(derouter, "s3_audit_callback_params", None)
        )
    ...
    def test_opt_in_constructs_separate_instance_with_audit_config(self, monkeypatch: pytest.MonkeyPatch):
        ...
        monkeypatch.setattr(derouter, "s3_callback_params", {"s3_bucket_name": "normal-bucket"})
        monkeypatch.setattr(derouter, "s3_audit_callback_params", {"s3_bucket_name": "audit-bucket"})
    ...
    def test_opt_out_preserves_singleton_behavior(self, monkeypatch: pytest.MonkeyPatch):
        ...
        monkeypatch.setattr(derouter, "s3_callback_params", {"s3_bucket_name": "shared-bucket"})
        monkeypatch.setattr(derouter, "s3_audit_callback_params", None)
    ...
    def test_empty_dict_opts_in(self, monkeypatch: pytest.MonkeyPatch):
        ...
        monkeypatch.setattr(derouter, "s3_callback_params", {"s3_bucket_name": "normal-bucket"})
        monkeypatch.setattr(derouter, "s3_audit_callback_params", {})
    ...
    def test_reset_audit_log_callback_cache_clears_audit_instance(self, monkeypatch: pytest.MonkeyPatch):
        ...
        monkeypatch.setattr(derouter, "s3_audit_callback_params", {"s3_bucket_name": "first"})
        with patch("asyncio.create_task"):
            ...
            monkeypatch.setattr(derouter, "s3_audit_callback_params", {"s3_bucket_name": "second"})
