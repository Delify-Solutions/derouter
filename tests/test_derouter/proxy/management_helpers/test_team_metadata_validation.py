...
async def _drive_create(metadata, mock_sink=None):
    ...
    from derouter.proxy._types import DeRouter_TeamTable, NewTeamRequest
    from derouter.proxy.management_endpoints.team_endpoints import new_team

    with (
        patch("derouter.proxy.proxy_server.prisma_client") as pc,
        patch("derouter.proxy.proxy_server._license_check") as lic,
        ...
        pc.db.derouter_teamtable.create = AsyncMock(return_value=team_row)
        pc.db.derouter_teamtable.count = AsyncMock(return_value=0)
        pc.db.derouter_teamtable.update = AsyncMock(return_value=team_row)
        pc.db.derouter_usertable.update = AsyncMock(return_value=MagicMock())
        pc.db.derouter_modeltable.create = AsyncMock(return_value=MagicMock(id="model-1"))
        lic.is_team_count_over_limit.return_value = False
        if mock_sink is not None:
            mock_sink["team_create"] = pc.db.derouter_teamtable.create
            mock_sink["model_create"] = pc.db.derouter_modeltable.create
...
async def _drive_update(kind, existing_metadata, payload):
    ...
    from derouter.proxy._types import DeRouter_TeamTable, PatchTeamRequest, UpdateTeamRequest
    from derouter.proxy.management_endpoints.team_endpoints import patch_team, update_team
    ...
    with (
        ...
        pc.db.derouter_teamtable.find_unique = AsyncMock(return_value=existing)
        pc.db.derouter_teamtable.update = AsyncMock(
            return_value=DeRouter_TeamTable(team_id=team_id, team_alias="matrix")
        )
        pc.jsonify_team_object = MagicMock(side_effect=lambda db_data: db_data)
