...
async def test_set_object_permission():
    ...
    mock_prisma_client.db.derouter_objectpermissiontable.create.assert_called_once()

    # Verify the data passed to create excludes None values and object_permission_id
    call_args = mock_prisma_client.db.derouter_objectpermissiontable.create.call_args
...
async def test_set_object_permission_persists_mcp_tool_search_enabled():
    ...
    payload so it persists to DeRouter_ObjectPermissionTable. The field was
    ...
    created_data = (
        mock_prisma_client.db.derouter_objectpermissiontable.create.call_args.kwargs[
            "data"
        ]
    )
...
def _make_team_obj(
    ...
    if (
        ...
        mock_team.object_permission = MagicMock(spec=DeRouter_ObjectPermissionTable)
...
@pytest.mark.asyncio
@patch(
    "derouter.proxy.management_helpers.object_permission_utils._get_allow_all_keys_server_ids",
    return_value=set(),
)
@patch(
...
async def test_validate_db_mcp_server_alias_outside_team_scope_raises_when_registry_empty(
    ...
    mock_prisma_client.db.derouter_mcpservertable.find_many = AsyncMock(
...
async def test_resolve_team_allowed_mcp_servers_string_tool_permissions(
    ...
    mock_perm = MagicMock(spec=DeRouter_ObjectPermissionTable)
...
async def test_resolve_team_all_proxy_sentinel_resolves_dynamically(mock_access_groups):
    ...
    team_perm = MagicMock(spec=DeRouter_ObjectPermissionTable)
...
async def test_enforce_all_proxy_mcp_grant_allows_non_admin_when_sentinel_already_set():
    ...
    with patch(
        "derouter.proxy.management_helpers.object_permission_utils.ObjectPermissionRepository",
...
def _make_ambiguity_prisma(existing_tool_permissions=None):
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_mcpservertable.find_many = AsyncMock(return_value=list(_SHARED_ALIAS_DB_SERVERS))
    mock_prisma.db.derouter_objectpermissiontable.create = AsyncMock(
    ...
    mock_prisma.db.derouter_objectpermissiontable.find_unique = AsyncMock(return_value=existing_row)
    return mock_prisma
...
async def test_set_object_permission_rejects_shared_alias_or_name_tool_permission_key(identifier, colliding_ids):
    ...
    mock_prisma.db.derouter_objectpermissiontable.create.assert_not_called()
...
def test_object_permission_dict_mirrors_pydantic_model():
    ...
    DeRouter_ObjectPermissionBase. If a new field is added to the Pydantic
    model, this test fails until the TypedDict is updated to match."""
    from typing import get_type_hints

    pydantic_fields = set(DeRouter_ObjectPermissionBase.model_fields.keys())
    typeddict_fields = set(get_type_hints(ObjectPermissionDict).keys())
    assert pydantic_fields == typeddict_fields, (
        f"ObjectPermissionDict drifted from DeRouter_ObjectPermissionBase.\n"
