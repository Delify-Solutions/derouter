...
from derouter.proxy.management_helpers.utils import add_new_member
...
async def test_management_otel_span_redacts_mcp_global_env_var_secrets(monkeypatch):
    ...
    from derouter.proxy.management_helpers import utils as mgmt_utils
    ...
    secret = "s3cr3t-p@ss"
    result = DeRouter_MCPServerTable(
        server_id="srv-1",
        alias="echo",
...
async def test_add_new_member_clones_default_team_budget_id():
    ...
    mock_prisma_client.db.derouter_usertable.upsert = AsyncMock(return_value=mock_user_response)
    ...
    mock_prisma_client.db.derouter_teammembership.create = AsyncMock(
    ...
    result_user, result_team_membership = await add_new_member(
        ...
        derouter_proxy_admin_name=test_admin_name,
    ...
    mock_prisma_client.db.derouter_usertable.upsert.assert_called_once()
    mock_prisma_client.db.derouter_teammembership.create.assert_called_once()
    ...
    mock_prisma_client.db.derouter_budgettable.create.assert_called_once()
    cloned_create_data = (
        mock_prisma_client.db.derouter_budgettable.create.call_args.kwargs["data"]
    )
    ...
    team_membership_call_args = (
        mock_prisma_client.db.derouter_teammembership.create.call_args
    )
...
async def test_add_new_member_budget_duration_only_clones_default_max_budget():
    ...
    await add_new_member(
        ...
        team_id="team-dc",
        user_api_key_dict=user_api_key_dict,
        derouter_proxy_admin_name="test_admin",
...
async def test_add_new_member_no_budget_when_no_default_and_no_max_budget():
    ...
    mock_prisma_client.db.derouter_budgettable.find_unique.assert_not_called()
    mock_prisma_client.db.derouter_budgettable.create.assert_not_called()
    mock_prisma_client.db.derouter_teammembership.create.assert_not_called()
...
async def test_add_new_member_creates_new_budget_when_max_budget_provided():
    ...
    2. A new budget is created in the derouter_budgettable
    ...
    mock_prisma_client.db.derouter_budgettable.create.assert_called_once()
    budget_call_args = mock_prisma_client.db.derouter_budgettable.create.call_args
...
async def test_add_new_member_persists_budget_duration():
    ...
    mock_prisma_client.db.derouter_budgettable.create.assert_called_once()
    budget_data = mock_prisma_client.db.derouter_budgettable.create.call_args.kwargs[
        "data"
    ]
...
async def test_attach_object_permission_to_dict_without_object_permission_id():
    ...
    mock_prisma_client.db.derouter_objectpermissiontable.find_unique.assert_not_called()
...
async def test_add_new_member_appends_team_only_if_absent_for_existing_user():
    ...
    mock_prisma_client.db.derouter_usertable.upsert = AsyncMock(return_value=mock_user_after)
    mock_prisma_client.db.derouter_usertable.update_many = AsyncMock()
    ...
    where = mock_prisma_client.db.derouter_usertable.update_many.call_args.kwargs["where"]
    assert where["user_id"] == "existing-user"
    assert where["NOT"] == {"teams": {"has": "team-1"}}
    data = mock_prisma_client.db.derouter_usertable.update_many.call_args.kwargs["data"]
    ...
    mock_prisma_client.db.derouter_usertable.upsert.assert_called_once()
    upsert_update = mock_prisma_client.db.derouter_usertable.upsert.call_args.kwargs["data"]["update"]
...
async def test_add_new_member_creates_missing_user_atomically_via_upsert():
    ...
    mock_prisma_client.db.derouter_usertable.upsert = AsyncMock(return_value=mock_created)
    mock_prisma_client.db.derouter_usertable.update_many = AsyncMock()
    mock_prisma_client.db.derouter_usertable.create = AsyncMock()
    mock_prisma_client.db.derouter_budgettable.find_unique = AsyncMock(return_value=None)
    ...
    mock_prisma_client.db.derouter_usertable.upsert.assert_called_once()
    mock_prisma_client.db.derouter_usertable.create.assert_not_called()
    upsert_data = mock_prisma_client.db.derouter_usertable.upsert.call_args.kwargs["data"]
...
def _member_write_tx() -> MagicMock:
    ...
    tx.derouter_usertable.upsert = AsyncMock(return_value=created_user)
    tx.derouter_usertable.create = AsyncMock(return_value=created_user)
    tx.derouter_usertable.update_many = AsyncMock()
    tx.derouter_usertable.find_many = AsyncMock(return_value=[])
    tx.derouter_budgettable.find_unique = AsyncMock(return_value=None)
...
async def test_add_new_member_runs_every_write_on_the_caller_transaction(new_member):
    ...
    assert tx.derouter_budgettable.create.await_count == 1
    assert tx.derouter_teammembership.create.await_count == 1
    assert tx.derouter_usertable.upsert.await_count + tx.derouter_usertable.create.await_count == 1
