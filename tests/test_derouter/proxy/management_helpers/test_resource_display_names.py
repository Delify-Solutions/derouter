...
async def test_mcp_db_row_beats_config_entry_for_the_same_server():
    ...
    prisma = _prisma(
        derouter_mcpservertable=_table([types.SimpleNamespace(server_id="s1", alias="db-alias", server_name=None)])
    )
...
async def test_mcp_config_only_server_falls_back_alias_then_server_name_then_name(alias, server_name, expected):
    ...
    prisma = _prisma(derouter_mcpservertable=_table())
...
async def test_mcp_db_row_without_alias_or_server_name_yields_no_label():
    ...
    prisma = _prisma(
        derouter_mcpservertable=_table([types.SimpleNamespace(server_id="s1", alias=None, server_name=None)])
    )
...
async def test_mcp_only_requested_ids_are_returned_and_the_query_is_deduped():
    ...
    prisma = _prisma(derouter_mcpservertable=table)
...
async def test_mcp_empty_ids_skip_the_db():
    table = _table()
    names = await mcp_server_display_names(_prisma(derouter_mcpservertable=table), (), {})
    assert dict(names) == {}
...
async def test_agent_db_name_beats_registry_name():
    prisma = _prisma(derouter_agentstable=_table([types.SimpleNamespace(agent_id="a1", agent_name="from-db")]))
...
async def test_agent_legacy_config_id_resolves_to_the_stable_agent_name():
    ...
    prisma = _prisma(derouter_agentstable=_table())
...
async def test_agent_empty_ids_skip_the_db():
    table = _table()
    names = await agent_display_names(_prisma(derouter_agentstable=table), (), _registry_with())
    assert dict(names) == {}
    table.find_many.assert_not_awaited()
...
async def test_key_alias_only_for_keys_that_have_one():
    ...
    names = await key_display_names(_prisma(derouter_verificationtoken=table), ("k1", "k2", "k1"))
...
async def test_key_empty_ids_skip_the_db():
    table = _table()
    assert dict(await key_display_names(_prisma(derouter_verificationtoken=table), ())) == {}
    table.find_many.assert_not_awaited()
