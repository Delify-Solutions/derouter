...
import derouter
from derouter.proxy.proxy_server import _blocked_response_usage
from derouter.types.llms.openai import ResponseAPIUsage, ResponsesAPIResponse


def test_uses_original_response_usage():
    resp = derouter.ModelResponse()
    resp.usage = derouter.Usage(prompt_tokens=42, completion_tokens=7, total_tokens=49)
...
async def test_success_hook_attaches_original_response_on_block():
    ...
    import derouter.proxy.guardrails.guardrail_hooks.unified_guardrail.unified_guardrail as ug
    ...
    response.usage = derouter.Usage(prompt_tokens=15, completion_tokens=3, total_tokens=18)
...
def test_responses_api_blocked_reply_maps_bridged_chat_usage():
    ...
    resp = derouter.ModelResponse()
    resp.usage = derouter.Usage(prompt_tokens=14, completion_tokens=18, total_tokens=32)
...
def test_raise_passthrough_exception_attaches_original_response():
    ...
    resp = derouter.ModelResponse()
    resp.usage = derouter.Usage(prompt_tokens=5, completion_tokens=2, total_tokens=7)
