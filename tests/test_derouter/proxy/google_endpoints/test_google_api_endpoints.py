...
def _patch_base_process(return_value=None):
    ...
    return patch(
        "derouter.proxy.google_endpoints.endpoints.ProxyBaseLLMRequestProcessing.base_process_llm_request",
...
def test_google_stream_generate_content_endpoint():
    ...
    with (
        ...
        patch(
            "derouter.proxy.google_endpoints.endpoints.ProxyBaseLLMRequestProcessing.__init__",
            return_value=None,
        ) as mock_init,
    ):
        response = client.post(
        ...
        assert init_kwargs["data"]["_derouter_raw_sse_stream"] is True
        assert init_kwargs["data"]["_derouter_skip_openai_stream_done"] is True
...
def test_google_generate_content_forwards_call_id_header():
    """The endpoint must forward the x-derouter-call-id header to the processor
    ...
    with _patch_base_process() as mock_base:
        client.post(
            "/v1beta/models/test-model:generateContent",
            json={"contents": [{"role": "user", "parts": [{"text": "Hello"}]}]},
            headers={"x-derouter-call-id": "trace-abc-123"},
        )

        forwarded_request = mock_base.call_args.kwargs["request"]
        assert forwarded_request.headers.get("x-derouter-call-id") == "trace-abc-123"
...
def test_google_count_tokens_unchanged():
    ...
    with patch(
        "derouter.proxy.proxy_server.token_counter",
        new_callable=AsyncMock,
