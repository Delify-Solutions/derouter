...
The proxy must not pass the agent name as ``model`` or DeRouter may route to
...
class TestInteractionsAgentOnlyProviderRouting:
    ...
    @patch("derouter.interactions.main.interactions_http_handler")
    @patch("derouter.interactions.main.get_provider_interactions_api_config")
    @patch("derouter.get_llm_provider")
    def test_agent_only_skips_get_llm_provider(
        ...
        from derouter.interactions.main import create
