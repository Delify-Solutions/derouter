...
def test_token_counter_counts_chat_messages():
    from derouter.batches.batch_utils import _count_entry_tokens
...
async def test_pre_call_rejects_unauthorized_model_in_batch_file():
    ...
    from derouter.proxy.hooks.batch_rate_limiter import _PROXY_BatchRateLimiter
    ...
    with (
        patch(
            "derouter.proxy.auth.auth_checks.can_key_call_model",
...
async def test_pre_call_allows_all_team_models_key_when_model_in_team_allowlist():
    ...
    with patch("derouter.proxy.proxy_server.llm_router", None):
        await rate_limiter._enforce_batch_file_model_access(
            user_api_key_dict=user,
            models=_models(file_dict),
        )
...
async def test_pre_call_uses_current_team_allowlist_for_all_team_models_key():
    from derouter.proxy._types import DeRouter_TeamTable, SpecialModelNames
    from derouter.proxy.hooks.batch_rate_limiter import _PROXY_BatchRateLimiter
...
async def test_pre_call_denies_all_team_models_key_via_member_scope():
    ...
    team_object = DeRouter_TeamTable(team_id="team-123", models=[team_model])
    membership = DeRouter_TeamMembership(
        user_id="alice",
        team_id="team-123",
        derouter_budget_table=DeRouter_BudgetTable(allowed_models=["other-model"]),
    )
...
async def test_pre_call_skips_file_fetch_for_configured_provider():
    ...
    with (
        ...
        patch(
            "derouter.proxy.openai_files_endpoints.common_utils.get_credentials_for_model",
            return_value={"custom_llm_provider": "hosted_vllm"},
        ),
        patch("derouter.afile_content", new=AsyncMock()) as mock_afile_content,
    ):
        result = await rate_limiter.async_pre_call_hook(
            user_api_key_dict=user,
            cache=MagicMock(),
...
async def test_pre_call_does_not_skip_for_spoofed_provider():
    ...
    with (
        ...
        patch(
            "derouter.afile_content", new=AsyncMock(return_value=mock_content)
...
async def test_count_input_file_usage_decodes_model_embedded_file_id():
    ...
    encoded_payload = (
        base64.urlsafe_b64encode(
            f"derouter:{original_file_id};model,my-vllm-batch".encode()
        )
        .decode()
        .rstrip("=")
    )
...
async def test_pre_call_uses_target_model_names_not_stripped_reverse_lookup(
    ...
    deployment_templates = {
        "openai/openai/gpt-5.5": {
            ...
            "derouter_params": {"model": "openai/gpt-5.5"},
        ...
        "us/azure/openai/gpt-5.5": {
            ...
            "derouter_params": {"model": "azure/gpt-5.5"},
            "model_info": {"id": "openai/openai/gpt-5.5", "mode": "chat"},
        },
    }
    mock_router = MagicMock()
    ...
    def _resolve(model_id):
        for deployment in mock_router.model_list:
            actual_model = deployment.get("derouter_params", {}).get("model")
...
def test_get_batch_routing_model_prefers_file_bound_over_request_model():
    ...
    rate_limiter = _make_rate_limiter()
    encoded = (
        base64.urlsafe_b64encode(b"derouter:file-xyz;model,vllm-batch")
        .decode()
        .rstrip("=")
    )
    assert (
...
def test_get_batch_routing_model_uses_unified_file_id_target():
    ...
    with (
        ...
        patch(
            "derouter.proxy.openai_files_endpoints.common_utils.get_models_from_unified_file_id",
...
def test_should_skip_ignores_client_supplied_metadata_flag():
    ...
    ``derouter_metadata.skip_batch_input_file_rate_limiting`` in the request
        ...
        should_skip, descriptors = (
            rate_limiter._should_skip_batch_input_file_processing(
                data={
                    ...
                    "derouter_metadata": {"skip_batch_input_file_rate_limiting": True},
                },
                user_api_key_dict=user,
            )
        )
...
def test_should_not_skip_for_forged_model_embedded_file_id():
    ...
    user = UserAPIKeyAuth(api_key="sk", models=["*"])
    encoded = (
        base64.urlsafe_b64encode(b"derouter:file-xyz;model,gpt-4o-mini")
        .decode()
        .rstrip("=")
    )
    with patch(
...
def test_should_not_skip_when_file_bound_provider_is_rate_limited():
    ...
    user = UserAPIKeyAuth(api_key="sk", models=["*"])
    encoded = (
        base64.urlsafe_b64encode(b"derouter:file-orig;model,vllm-batch")
        .decode()
        .rstrip("=")
    )
...
def test_warns_once_for_unsupported_model_skip_setting():
    ...
    with (
        ...
        patch(
            "derouter.proxy.hooks.batch_rate_limiter.verbose_proxy_logger"
        ) as mock_logger,
    ):
        for _ in range(3):
            rate_limiter._should_skip_batch_input_file_processing(
...
async def test_pre_call_enforces_project_otpm_limit_for_batch():
    ...
    with (
        ...
        patch("derouter.afile_content", new=AsyncMock(return_value=mock_content)),
    ):
        with pytest.raises(HTTPException) as exc:
            await rate_limiter.async_pre_call_hook(
                user_api_key_dict=user,
...
async def test_count_input_file_usage_raises_on_non_bytes_content():
    ...
    with patch("derouter.afile_content", new=AsyncMock(return_value=bad_content)):
...
async def test_count_input_file_usage_streams_without_building_list():
    ...
    with (
        patch("derouter.afile_content", new=AsyncMock(return_value=fake_content)),
        patch(
...
async def test_count_input_file_usage_enforces_models_when_token_counting_fails():
    ...
    with (
        patch("derouter.afile_content", new=AsyncMock(return_value=fake_content)),
        patch("derouter.proxy.hooks.batch_rate_limiter._count_entry_tokens", new=_boom),
        patch("derouter.proxy.auth.auth_checks.can_key_call_model", new=deny),
        patch("derouter.proxy.proxy_server.llm_router", MagicMock(model_list=[])),
    ):
        with pytest.raises(HTTPException) as exc:
            await rate_limiter.count_input_file_usage(
                file_id="file-not-managed",
...
async def test_count_input_file_usage_estimates_tokens_when_counting_fails_for_allowed_model():
    ...
    with (
        patch("derouter.afile_content", new=AsyncMock(return_value=fake_content)),
        patch("derouter.proxy.hooks.batch_rate_limiter._count_entry_tokens", new=_boom),
        patch("derouter.proxy.auth.auth_checks.can_key_call_model", new=allow),
        patch("derouter.proxy.proxy_server.llm_router", MagicMock(model_list=[])),
    ):
        usage = await rate_limiter.count_input_file_usage(
            file_id="file-not-managed",
...
async def test_enqueued_limit_accepts_batch_over_per_minute_limits():
    ...
    from derouter.proxy.hooks.parallel_request_limiter_v3 import get_request_stash
    ...
    with patches[0], patches[1], patches[2], patch("derouter.afile_content", new=afile_content_mock):
...
async def test_disable_flag_still_skips_batch_processing_with_enqueued_limits():
    ...
    with (
        patch("derouter.proxy.proxy_server.general_settings", {"disable_batch_input_file_rate_limiting": True}),
        patch("derouter.proxy.proxy_server.llm_router", MagicMock()),
        patch("derouter.afile_content", new=afile_content_mock),
    ):
        data = {"input_file_id": "file-abc123", "model": "gpt-4o-mini"}
        result = await rate_limiter.async_pre_call_hook(
            user_api_key_dict=opted_in_user,
