...
SKILL_TOOL_NAME = "derouter_skill_e2b8dca8_031a_4481_b034_b9ec7d4eb7bf"
...
def _request_data():
    return {
        ...
        "derouter_metadata": {
            "_derouter_code_execution_enabled": True,
...
@pytest.mark.asyncio
async def test_post_call_success_hook_executes_derouter_skill_tool():
    """DB skill tool names carry the derouter_skill_ prefix and must trigger the execution loop."""
...
@pytest.mark.asyncio
async def test_execute_code_loop_dispatches_derouter_skill_tool():
    """The agentic loop must route derouter_skill_ tool calls to _execute_skill_tool."""
    hook = SkillsInjectionHook()
    ...
    with (
        ...
        patch("derouter.anthropic.acreate", new=AsyncMock(return_value=final_response)),
    ):
...
def test_importing_proxy_hooks_does_not_mutate_derouter_callbacks():
    script: Final = (
        "import derouter; "
        "derouter.callbacks = []; "
        "import derouter.proxy.hooks; "
        "assert len(derouter.callbacks) == 0, f'mutated: {derouter.callbacks}'"
    )
    result: Final = subprocess.run(
        [sys.executable, "-c", script],
        capture_output=True,
        text=True,
        env={**os.environ, "DEROUTER_LOCAL_MODEL_COST_MAP": "True"},
    )
