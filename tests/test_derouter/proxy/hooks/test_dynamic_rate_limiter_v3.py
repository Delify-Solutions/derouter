...
import derouter
from derouter import DualCache, Router
...
async def test_priority_weight_allocation(monkeypatch):
    ...
    monkeypatch.setenv("DEROUTER_LICENSE", "test-license-key")

    # Set up priority reservations
    derouter.priority_reservation = {"high": 0.9, "low": 0.1}
...
async def test_concurrent_pre_call_hooks_stress(monkeypatch):
    ...
    derouter.priority_reservation = {"premium": 0.8, "standard": 0.2}
    ...
    async def mock_get_cache(key, derouter_parent_otel_span=None, local_only=False):
...
async def test_fake_calls_case_1_no_rate_limiting_at_capacity(monkeypatch):
    ...
    # Set up priority reservations
    derouter.priority_reservation = {"key_a": 0.75, "key_b": 0.25}
...
async def test_fake_calls_case_3_spillover_capacity_default_keys(monkeypatch):
    ...
    derouter.priority_reservation = {"key_a": 0.75}
    derouter.priority_reservation_settings.default_priority = 0.25
...
async def test_fake_calls_case_4_over_allocated_with_normalization(monkeypatch):
    ...
    derouter.priority_reservation = {"key_a": 0.60, "key_b": 0.80}
...
async def test_fake_calls_case_5_default_value_priority_reservation(monkeypatch):
    ...
    derouter.priority_reservation = {"key_a": 0.50, "key_b": 0.20, "key_c": 0.05}
    derouter.priority_reservation_settings.default_priority = 0.05
...
async def test_default_priority_shared_pool(monkeypatch):
    ...
    derouter.priority_reservation = {"prod": 0.75}
    derouter.priority_reservation_settings.default_priority = 0.25
...
async def test_async_log_success_event_increments_by_actual_tokens(monkeypatch):
    ...
    monkeypatch.setenv("DEROUTER_LICENSE", "test-license-key")
    derouter.priority_reservation = {"dev": 0.1, "prod": 0.9}
    ...
    with patch(
        "derouter.proxy.common_utils.callback_utils.get_model_group_from_derouter_kwargs",
        return_value=model,
    ):
...
async def test_saturation_check_cache_ttl_configuration(monkeypatch):
    ...
    original_ttl = derouter.priority_reservation_settings.saturation_check_cache_ttl
    derouter.priority_reservation_settings.saturation_check_cache_ttl = 5
    ...
    try:
        ...
        async def mock_get_cache(
            key, derouter_parent_otel_span=None, local_only=False, **kwargs
        ):
        ...
        derouter.priority_reservation_settings.saturation_check_cache_ttl = 30
        ...
        derouter.priority_reservation_settings.saturation_check_cache_ttl = original_ttl
...
async def test_async_log_success_event_uses_team_priority_from_auth_metadata(monkeypatch):
    ...
    monkeypatch.setenv("DEROUTER_LICENSE", "test-license-key")
    derouter.priority_reservation = {"team_priority": 0.8, "default": 0.2}
...
async def test_priority_429_includes_model_name_and_configured_limits(monkeypatch):
    ...
    monkeypatch.setenv("DEROUTER_LICENSE", "test-license-key")
    derouter.priority_reservation = {"prod": 0.5}
...
async def test_tpm_only_model_enforces_priority_and_model_capacity(monkeypatch):
    ...
    monkeypatch.setenv("DEROUTER_LICENSE", "test-license-key")
    derouter.priority_reservation = {"dev": 0.25, "prod": 0.5}
    ...
    async def record_usage(priority: str, total_tokens: int) -> None:
        await handler.async_log_success_event(
            kwargs={
                ...
                "derouter_params": {"metadata": {"model_group": model}},
            },
