...
import derouter
from derouter.proxy._types import NewUserRequest, NewUserResponse, UserAPIKeyAuth
...
class FakePrismaClient:
    def __init__(self, rows: List[Dict[str, Any]]):
        self.db = SimpleNamespace(derouter_usertable=FakeUserTable(rows))
...
async def _run_created_hook(prisma_client: FakePrismaClient, audit_log: AsyncMock) -> None:
    ...
    with (
        patch.dict(sys.modules, {"derouter.proxy.proxy_server": proxy_server}),
        patch.object(derouter, "store_audit_logs", True),
        patch(
