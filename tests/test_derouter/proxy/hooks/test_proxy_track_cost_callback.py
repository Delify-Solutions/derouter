...
from derouter.types.utils import CallTypes, Usage
...
async def test_async_post_call_failure_hook():
    ...
    with patch(
        "derouter.proxy.db.db_spend_update_writer.DBSpendUpdateWriter.update_database",
        ...
        assert "derouter_params" in call_args["kwargs"]
        assert call_args["kwargs"]["derouter_params"]["proxy_server_request"] == {"request_id": "test_request_id"}
        metadata = call_args["kwargs"]["derouter_params"]["metadata"]
...
@pytest.mark.asyncio
async def test_async_post_call_failure_hook_carries_guardrail_info_from_derouter_metadata():
    """
    LIT-5650 regression: on a pre_call guardrail block the unified guardrail
    layer seeds request_data["derouter_metadata"], so the guardrail hook writes
    ...
    with patch(
        ...
        metadata = mock_update_database.call_args[1]["kwargs"]["derouter_params"]["metadata"]
...
async def test_async_post_call_failure_hook_does_not_clobber_guardrail_info_in_metadata():
    ...
    request_data = {
        ...
        "derouter_metadata": {"standard_logging_guardrail_information": [{"guardrail_name": "from-derouter-bucket"}]},
        "proxy_server_request": {"request_id": "test_request_id"},
    }
...
async def test_async_post_call_failure_hook_releases_budget_reservation_before_route_skip():
    ...
    with (
        patch(
            "derouter.proxy.spend_tracking.budget_reservation.release_budget_reservation",
            new_callable=AsyncMock,
...
async def test_should_continue_failure_tracking_when_budget_release_fails():
    ...
    with (
        ...
        patch(
            "derouter.proxy.hooks.proxy_track_cost_callback._invalidate_budget_reservation_counters",
            new_callable=AsyncMock,
        ...
        patch(
            "derouter.proxy.hooks.proxy_track_cost_callback.verbose_proxy_logger.exception",
...
async def test_update_database_and_spend_counters_invalidates_reservation_when_counter_update_fails():
    ...
    with patch(
        "derouter.proxy.spend_tracking.budget_reservation.invalidate_budget_reservation_counters",
        new_callable=AsyncMock,
    ) as mock_invalidate_budget_reservation_counters:
...
async def test_track_cost_callback_charges_a_batch_once_and_only_when_final(  # test-quality-ok: whether the spend writer runs, whether the counters move, and whether the poll's reservation is handed back is the whole observable contract of the gate
    ...
    with (
        patch(  # test-quality-ok: increment_spend_counters is a proxy_server global the callback reads lazily, no seam
            "derouter.proxy.proxy_server.increment_spend_counters", new_callable=AsyncMock
        ...
        patch(  # test-quality-ok: update_cache is a proxy_server global the callback reads lazily, no seam
            "derouter.proxy.proxy_server.update_cache", new_callable=AsyncMock
        ...
        patch(  # test-quality-ok: the release is imported inside the callback's helper, no seam
            "derouter.proxy.spend_tracking.budget_reservation.release_budget_reservation", new_callable=AsyncMock
        ) as mock_release_budget_reservation,
    ):
        mock_proxy_logging.failed_tracking_alert = AsyncMock()
...
def _in_progress_interaction_kwargs(reservation: dict) -> dict:
    return {
        ...
        "model": "gemini/gemini-3-flash-preview",
        "derouter_call_id": "test-call-id",
        "derouter_params": {"metadata": {"user_api_key_budget_reservation": reservation}},
        "stream": False,
    }
...
async def test_track_cost_callback_releases_reservation_for_in_progress_interaction_when_polling_disabled(
    ...
    """
    import derouter.proxy.hooks.proxy_track_cost_callback as callback_module
    from derouter.types.interactions import InteractionsAPIResponse
...
async def test_callback_handles_every_status_the_interactions_api_can_return():
    ...
    """
    from derouter.types.interactions import InteractionsAPIResponse
    from derouter.types.interactions.generated import Status1
...
async def test_async_post_call_failure_hook_propagates_trace_id_from_logging_obj():
    ...
    derouter_logging_obj (set by function_setup) is in request_data and
    ...
    # Simulate a derouter_logging_obj with model_call_details containing
    # the standard_logging_object (as set by _failure_handler_helper_fn)
    mock_logging_obj = MagicMock()
    mock_logging_obj.derouter_trace_id = "trace-id-from-logging-obj"
    ...
    request_data = {
        ...
        # Note: no "standard_logging_object" and no "derouter_trace_id"
    }
    ...
    with patch(
        ...
        # derouter_trace_id should also be propagated as a fallback
        assert call_kwargs.get("derouter_trace_id") == "trace-id-from-logging-obj"
...
async def test_enrich_failure_metadata_with_team_alias():
    ...
    with patch(
        "derouter.proxy.hooks.proxy_track_cost_callback.get_team_object",
        new_callable=AsyncMock,
...
async def test_enrich_failure_metadata_with_full_key_lookup():
    ...
    with (
        patch(
            "derouter.proxy.hooks.proxy_track_cost_callback.get_key_object",
            new_callable=AsyncMock,
...
async def test_track_cost_callback_reads_key_only_for_in_request_logs(call_type, expect_key_read):
    ...
    written = kwargs["derouter_params"]["metadata"]
    if expect_key_read:
...
async def test_async_post_call_failure_hook_uses_actual_start_time():
    ...
    mock_logging_obj.model_call_details = {}
    mock_logging_obj.derouter_trace_id = None
...
async def _invoke_failure_hook_with_raised_exception():
    ...
    with patch(
        ...
        call_args = mock_update_database.call_args[1]
        return call_args["kwargs"]["derouter_params"]["metadata"]
...
async def test_failure_hook_keeps_error_information_traceback_by_default(monkeypatch):
    ...
    monkeypatch.delenv("DEROUTER_SUPPRESS_SPEND_LOG_TRACEBACKS", raising=False)
...
async def test_failure_hook_drops_error_information_traceback_when_env_set(
    ...
    monkeypatch.setenv("DEROUTER_SUPPRESS_SPEND_LOG_TRACEBACKS", "true")
...
async def test_track_cost_callback_enriches_user_id_for_mcp_style_metadata():
    ...
    with (
        ...
        patch(
            "derouter.proxy.proxy_server.update_cache",
            new_callable=AsyncMock,
        ),
        patch(
            "derouter.proxy.proxy_server.proxy_logging_obj",
        ...
        assert kwargs["derouter_params"]["metadata"]["user_api_key_user_id"] == "mcp-user@example.com"
...
async def test_track_cost_callback_keeps_guardrail_cost_on_cache_hit():
    ...
    with (
        patch("derouter.proxy.proxy_server.increment_spend_counters", new_callable=AsyncMock) as mock_increment,  # test-quality-ok: the callback imports this from proxy_server inside its body, so there is no injection seam
        patch("derouter.proxy.proxy_server.update_cache", new_callable=AsyncMock),  # test-quality-ok: same function-body import, no injection seam
        patch("derouter.proxy.proxy_server.proxy_logging_obj") as mock_proxy_logging,  # test-quality-ok: same function-body import, no injection seam
    ):
        mock_proxy_logging.db_spend_update_writer.update_database = AsyncMock()
        mock_proxy_logging.slack_alerting_instance.customer_spend_alert = AsyncMock()
...
def test_should_track_cost_callback_pass_through_without_owner(call_type, expected):
    ...
    DeRouter_SpendLogs. Other call types with no owner stay untracked.
    ...
    user_api_key_user_id/user_api_key_team_id from DeRouter_ManagedObjectTable,
    ...
    such a batch's cost silently never reached DeRouter_SpendLogs."""
    assert (
        _should_track_cost_callback(
            user_api_key=None,
            user_id=None,
...
async def _groups_charged_by_the_callback(kwargs, deployments=None):
    ...
    with (
        ...
        patch(  # test-quality-ok: the arguments to this call are the boundary under test
            "derouter.proxy.hooks.proxy_track_cost_callback._update_database_and_spend_counters",
            new=AsyncMock(),
        ) as mock_update,
        patch(  # test-quality-ok: llm_router is a proxy_server global the callback reads lazily, no seam
            "derouter.proxy.proxy_server.llm_router", new=_FakeDeploymentLookup(deployments or {})
        ),
    ):
        mock_proxy_logging.failed_tracking_alert = AsyncMock()
        mock_proxy_logging.slack_alerting_instance.customer_spend_alert = AsyncMock()
