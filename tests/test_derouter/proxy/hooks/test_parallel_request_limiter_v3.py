...
from derouter.proxy.utils import InternalUsageCache, ProxyLogging, hash_token
...
async def test_sliding_window_rate_limit_v3(monkeypatch, time_controller):
    ...
    """
    monkeypatch.setenv("DEROUTER_RATE_LIMIT_WINDOW_SIZE", "2")
    _api_key = "sk-12345"
...
async def test_normal_router_call_tpm_v3(
    ...
    parallel_request_handler.batch_rate_limiter_script = mock_batch_rate_limiter
    monkeypatch.setattr(derouter, "callbacks", [parallel_request_handler])
...
async def test_token_rate_limit_type_respected_v3(monkeypatch, token_rate_limit_type):
    ...
    monkeypatch.setenv("DEROUTER_RATE_LIMIT_WINDOW_SIZE", "60")
...
async def test_parallel_gauge_uses_atomic_redis_script_v3():
    ...
    assert (
        await handler.internal_usage_cache.async_get_cache(
            key=counter_key, derouter_parent_otel_span=None, local_only=True
        )
        == 3
    )
    gauge_statuses = [
...
async def test_async_log_success_event_with_dict_usage(
    ...
    from derouter.types.utils import BaseDeRouterOpenAIResponseObject

    # Use spec to make isinstance checks work correctly with MagicMock
    mock_response = MagicMock(spec=BaseDeRouterOpenAIResponseObject)
    mock_response.usage = {
        "prompt_tokens": 25,
...
async def test_async_log_success_event_with_dict_usage_missing_fields(monkeypatch):
    ...
    mock_response.__class__ = type(
        "MockResponse", (BaseDeRouterOpenAIResponseObject,), {}
    )
...
async def test_agent_level_rate_limit_descriptors():
    ...
    with patch(
        "derouter.proxy.agent_endpoints.agent_registry.global_agent_registry.get_agent_by_id",
...
class TestGetTotalTokensFromUsageCacheExclusion:
    ...
    but DeRouter was including them, causing up to 10x difference in rate limiting.
    """
...
async def test_pre_call_hook_keeps_internal_stash_out_of_request_body():
    ...
    created or mutated ``metadata`` / ``derouter_metadata`` buckets. The
...
async def test_responses_route_body_untouched_by_pre_call_hook(caller_metadata):
    ...
    ``derouter_metadata`` produced upstream HTTP 400s."""
    import copy
...
async def test_chat_tpm_refund_and_slot_release_via_context_stash(monkeypatch):
    ...
    """
    monkeypatch.delenv("DEROUTER_TPM_TOKEN_RESERVATION_ENABLED", raising=False)
...
async def test_pre_call_hook_ignores_caller_supplied_stash_values():
    ...
    injected = {
        "_derouter_tpm_reserved_tokens": 9999,
        "_derouter_rate_limit_descriptors": victim_descriptors,
    }
    data: Dict[str, Any] = {
        "model": "gpt-4o-mini",
        "messages": [{"role": "user", "content": "hi"}],
        "metadata": dict(injected),
        "derouter_metadata": dict(injected),
    }
...
async def test_log_events_from_nested_calls_leave_owner_stash_alone(monkeypatch):
    """
    A nested DeRouter call made inside the request (LLM-judge guardrail,
    silent experiment) inherits the request context and fires the same global
    logging callbacks with a fresh ``derouter_call_id``. Those callbacks must
    ...
    await handler.async_pre_call_hook(
        ...
        data={
            ...
            "messages": [{"role": "user", "content": "hello"}],
            "max_tokens": 50,
            "derouter_call_id": "owner-call-id",
        },
        call_type="completion",
    )
    ...
    stash = get_request_stash()
    assert stash is not None
    assert stash.owner_derouter_call_id == "owner-call-id"
    reserved = stash.reserved_tokens
    assert reserved > 0

    nested_kwargs = {
        "derouter_call_id": "nested-guardrail-call",
        "standard_logging_object": {"metadata": {"user_api_key_hash": _api_key}},
    }
...
async def test_stash_applies_when_owner_or_callback_call_id_missing():
    ...
    do not thread ``derouter_call_id`` into their logging kwargs.
    """
    local_cache = DualCache()
    handler = _PROXY_MaxParallelRequestsHandler(
        internal_usage_cache=InternalUsageCache(local_cache)
    ...
    await handler.async_log_failure_event(
        kwargs={"derouter_call_id": "any-id", "standard_logging_object": {}},
        response_obj=None,
        start_time=None,
        end_time=None,
    )
    ...
    claimed = RequestRateLimiterStash(
        owner_derouter_call_id="owner-1", reserved_tokens=42
    )
...
async def test_failure_event_refunds_reservation_for_input_only_estimate_v3():
    ...
    await handler.async_log_failure_event(
        kwargs={
            "derouter_call_id": "estimate-call",
            "standard_logging_object": {"metadata": {"user_api_key_hash": _api_key}},
...
@contextmanager
def _override_derouter_callbacks(new_callbacks):
    """Swap derouter.callbacks so _callback_capabilities recomputes deterministically."""
    saved = derouter.callbacks
    derouter.callbacks = new_callbacks
    try:
        yield
    finally:
        derouter.callbacks = saved
...
async def test_async_streaming_data_generator_releases_counter_on_disconnect_v3(
    ...
    with _override_derouter_callbacks([]):
...
async def test_async_data_generator_releases_counter_when_wrapped_v3():
    ...
    try:
        with _override_derouter_callbacks([_PassthroughIteratorOverride()]):
...
def test_tpm_reservation_disabled_via_env(monkeypatch, value):
    monkeypatch.setenv("DEROUTER_TPM_TOKEN_RESERVATION_ENABLED", value)
    handler = _PROXY_MaxParallelRequestsHandler(
        internal_usage_cache=InternalUsageCache(DualCache())
    )
...
async def test_pre_call_hook_skips_reservation_when_disabled(monkeypatch):
    ...
    """
    monkeypatch.setenv("DEROUTER_TPM_TOKEN_RESERVATION_ENABLED", "false")
    handler = _PROXY_MaxParallelRequestsHandler(
        internal_usage_cache=InternalUsageCache(DualCache())
    )
...
async def test_streaming_end_to_end_populates_slp_ratelimit_headers(monkeypatch):
    ...
    # so the success callback can find it via derouter_params.metadata.
    data: Dict[str, Any] = {
        "model": "gpt-4o-mini",
        "metadata": {},
        "stream": True,
    ...
    # completion() call: it becomes kwargs["derouter_params"]["metadata"] by
...
async def test_atomic_check_with_zero_increment_still_enforces_token_limit():
    ...
    """
    from derouter.proxy.hooks.parallel_request_limiter_v3 import RateLimitDescriptor
...
def test_non_integer_output_budgets_still_reserve_their_declared_size(declared):
    """A budget derouter cannot read is a budget it cannot reserve against.
