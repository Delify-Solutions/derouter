...
*before* :func:`derouter.get_llm_provider` runs anywhere else in the request
lifecycle.

Until now, those hooks raised a bare ``HTTPException(429, ...)`` which carries
no ``llm_provider`` / ``model`` attribute. Downstream:

- The Prometheus ``derouter_proxy_failed_requests_metric`` reads
...
``derouter.RateLimitError``), and resolves ``model`` / ``llm_provider`` from
``data["model"]`` via :func:`get_llm_provider`. When the model is missing or
unparseable we fall back to ``llm_provider="derouter_proxy"`` so we never break
...
from derouter.proxy.hooks.max_iterations_limiter import _PROXY_MaxIterationsHandler
...
from derouter.proxy.hooks.rate_limiter_utils import (
...
class TestProxyRateLimitErrorClass:
    ...
    def test_defaults_to_derouter_proxy_provider(self):
...
class TestResolveLLMProviderForRateLimit:
    ...
    def test_router_alias_resolves_to_underlying_provider(self):
        """
        Nearly every real DeRouter proxy deployment uses router aliases:
        ...
        assert provider == "openai", (
            f"Router-alias path must resolve through derouter_params.model, "
            ...
            model_list = [
                {
                    ...
                    "derouter_params": {"model": "anthropic/claude-3-5-sonnet"},
                },
                {
                    "model_name": "claude-pool",
                    "derouter_params": {"model": "anthropic/claude-3-5-haiku"},
                },
            ]
    ...
    def test_router_alias_unknown_falls_back(self):
        ...
        land at the defensive ``derouter_proxy`` fallback rather than
        raising.
        """
    ...
    def test_router_alias_with_malformed_deployment_falls_back(self):
        ...
        ``derouter_params.model`` (or where ``get_llm_provider`` on the
        ...
        class _FakeRouter:
            model_list = [
                {"model_name": "broken", "derouter_params": {}},
                {"model_name": "broken", "derouter_params": {"model": ""}},
                {
                    "model_name": "broken",
                    "derouter_params": {"model": "nonsense-no-provider"},
                },
            ]
...
async def test_parallel_request_limiter_v1_unknown_model_falls_back():
    ...
    ``derouter_proxy`` — and crucially does *not* leak a secondary exception.
    """
    handler = _PROXY_MaxParallelRequestsHandler(
        internal_usage_cache=InternalUsageCache(DualCache())
    )
...
def _make_iter_agent(max_iterations: int) -> AgentResponse:
    return AgentResponse(
        ...
        derouter_params={"max_iterations": max_iterations},
...
def _make_session_budget_agent(max_budget: float) -> AgentResponse:
    return AgentResponse(
        ...
        derouter_params={"max_budget_per_session": max_budget},
...
def test_prometheus_exception_class_name_back_compat_for_proxy_rate_limit_error():
    ...
    # Same back-compat path even when the resolver fell back to derouter_proxy.
...
def test_prometheus_exception_class_name_back_compat_for_budget_exceeded_error():
    ...
    err_no_provider = derouter.BudgetExceededError(current_cost=1.0, max_budget=0.5)
    assert (
