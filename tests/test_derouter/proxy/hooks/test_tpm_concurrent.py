...
from derouter.proxy.utils import InternalUsageCache, hash_token
...
from derouter.types.utils import ModelResponse, PromptTokensDetailsWrapper, Usage
...
async def test_model_scope_refund_targets_reserved_model(rate_limiter):
    ...
    router later writes ``model_group`` into ``derouter_params.metadata``,
    ...
    mock_kwargs = {
        # NOTE: no derouter_params.metadata.model_group — get_model_group_from_derouter_kwargs
...
async def test_reservation_released_on_proxy_rejection(rate_limiter):
    ...
    the tokens leak — `async_log_failure_event` is a derouter completion
...
async def test_itpm_reservation_accounts_for_audio_content_not_just_text(rate_limiter):
    ...
    derouter.token_counter has no per-type handling for `input_audio`
...
async def test_itpm_otpm_reservation_is_kept_on_stream_disconnect(rate_limiter):
    ...
    async def mock_increment(increment_list, derouter_parent_otel_span=None):
        for op in increment_list:
...
async def test_responses_api_multimodal_input_counts_image_content(rate_limiter):
    ...
    derouter.token_counter's `text` argument only joins plain string entries
...
async def test_itpm_only_status_stored_when_no_prior_rate_limit_response(rate_limiter):
    ...
    assert stored is not None, (
        "ITPM status must be stored in derouter_proxy_rate_limit_response"
    )
...
async def test_max_output_tokens_prevents_cap_injection(rate_limiter):
    ...
    data: dict = {
        ...
        "derouter_call_id": "test-max-output-tokens",
        "metadata": {},
    }
...
async def test_rerank_query_and_documents_enforce_project_itpm(
    ...
    monkeypatch.setattr("derouter.token_counter", token_counter)
    user_api_key_dict = UserAPIKeyAuth(
...
async def test_project_io_counters_not_double_charged_when_reservation_disabled(
    ...
    """With DEROUTER_TPM_TOKEN_RESERVATION_ENABLED=false the first
...
async def test_post_call_success_hook_contains_header_merge_failures(
    ...
    await handler.async_post_call_success_hook(
        data={
            "derouter_proxy_rate_limit_response": {
                "overall_code": "OK",
                "statuses": (),
            }
        },
