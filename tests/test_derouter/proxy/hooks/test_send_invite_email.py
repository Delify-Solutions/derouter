...
async def test_v1_user_creation_no_email_when_send_invite_email_false():
    ...
    with patch(
        "derouter.logging_callback_manager.get_custom_loggers_for_type", return_value=[]
    ):
        mock_proxy_server = SimpleNamespace(
            general_settings={"alerting": ["email"]},
            proxy_logging_obj=mock_proxy_logging_obj,
            derouter_proxy_admin_name="admin-user",
        )
...
async def test_v1_key_generation_sends_email_when_send_invite_email_true():
    ...
    with (
        ...
        with patch(
            ...
            with patch.dict(
                sys.modules, {"derouter.proxy.proxy_server": mock_proxy_server}
            ):
