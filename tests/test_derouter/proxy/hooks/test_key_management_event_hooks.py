...
class TestKeyManagementEventHooksIndependentOperations:
    ...
    async def test_email_failure_does_not_block_secret_manager(self):
        ...
        with (
            ...
            patch("derouter.store_audit_logs", False),
            patch(
                "derouter.proxy.hooks.key_management_event_hooks.verbose_proxy_logger"
            ),
        ):
...
async def test_key_generated_audit_log_uses_license_default(
    ...
    from derouter.proxy._types import GenerateKeyRequest, GenerateKeyResponse, UserAPIKeyAuth

    monkeypatch.setattr("derouter.store_audit_logs", None)
...
class TestRotateVirtualKeyInSecretManager:
    ...
    async def test_rotate_virtual_key_with_team_id(self):
        ...
        from derouter.secret_managers.base_secret_manager import BaseSecretManager
        import derouter
        ...
        derouter.secret_manager_client = mock_secret_manager
        ...
        derouter._key_management_settings = KeyManagementSettings(
            ...
            prefix_for_stored_virtual_keys="derouter/",
        )
        ...
        with (
            ...
            patch(
                "derouter.proxy.hooks.key_management_event_hooks.isinstance",
            ...
            assert call_kwargs["current_secret_name"] == "derouter/virtual-key-old"
            assert call_kwargs["new_secret_name"] == "derouter/virtual-key-new"
