...
- derouter/proxy/hooks/batch_rate_limiter.py:181-248
...
- derouter/proxy/hooks/dynamic_rate_limiter_v3.py:463-548
...
from derouter.proxy.hooks.batch_rate_limiter import BatchFileUsage
...
async def test_batch_zero_token_consumes_rpm_only():
    ...
    rpm_val = await internal_usage_cache.async_get_cache(
        key=rpm_key, derouter_parent_otel_span=None, local_only=True
    )
    tpm_val = await internal_usage_cache.async_get_cache(
        key=tpm_key, derouter_parent_otel_span=None, local_only=True
    )
