...
class _FakeUserAPIKeyAuth:
    ...
    derouter_parent_otel_span specially (pops it before copying), but does
...
def test_get_silent_experiment_kwargs():
    ...
    model_list = [
        {
            ...
            "derouter_params": {"model": "gpt-3.5-turbo", "api_key": "fake-key"},
        },
    ]
    ...
    kwargs = {
        "metadata": {
            "foo": "bar",
            "derouter_parent_otel_span": mock_span,
    ...
    assert "derouter_call_id" not in result
    ...
    assert "derouter_parent_otel_span" not in result["metadata"]
    ...
    assert kwargs["metadata"]["derouter_parent_otel_span"] is mock_span
...
async def test_router_silent_experiment_acompletion():
    ...
    mock_response = derouter.ModelResponse(choices=[{"message": {"content": "hello"}}])
    mock_acompletion = AsyncMock(return_value=mock_response)

    # Patch at the derouter.router module level where it's imported and used
    with patch.object(derouter, "acompletion", mock_acompletion):
        ...
        # Verify no silent_model in any call to derouter.acompletion
...
def test_router_silent_experiment_completion():
    ...
    # Patch at the derouter module level
    with (
        patch.object(derouter, "acompletion", mock_acompletion_mock),
        patch.object(derouter, "completion", mock_completion_mock),
    ):
