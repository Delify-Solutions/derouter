...
from derouter.experimental_mcp_client.client import (
...
from derouter.types.mcp import MCPAuth, MCPStdioConfig, MCPTransport
...
class TestMCPClient:
    ...
    @pytest.mark.asyncio
    @patch("derouter.experimental_mcp_client.client.stdio_client")
    @patch("derouter.experimental_mcp_client.client.ClientSession")
    ...
    async def test_mcp_client_ssl_configuration_from_env(self, mock_streamable_http_client):
        ...
        # Mock the session
        with patch("derouter.experimental_mcp_client.client.ClientSession") as mock_session:
            mock_session_instance = AsyncMock()
            mock_session_instance.initialize = AsyncMock()
            mock_session_ctx = AsyncMock()
            mock_session_ctx.__aenter__.return_value = mock_session_instance
...
def test_missing_streamable_http_client_error_names_requirement_and_remedy():
    ...
    assert "pip install 'derouter[mcp]'" in message
...
async def test_http_transport_without_streamable_http_client_raises_actionable_import_error():
    ...
    with patch.object(  # test-quality-ok: simulates mcp<1.24.0 whose module lacks this import-time symbol
        ...
        with pytest.raises(ImportError, match=r"pip install 'derouter\[mcp\]'"):
            await client.list_tools(raise_on_error=True)
