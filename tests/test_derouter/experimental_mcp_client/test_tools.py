...
async def test_pagination_walk_stops_at_page_cap(mock_session, monkeypatch):
    monkeypatch.setattr("derouter.experimental_mcp_client.tools.MCP_TOOL_LISTING_MAX_PAGES", 2)
    mock_session.list_tools.side_effect = [
        ListToolsResult(
...
async def test_pagination_walk_stops_at_whole_walk_deadline(mock_session, monkeypatch):
    ...
    monkeypatch.setattr("derouter.experimental_mcp_client.tools.MCP_CLIENT_TIMEOUT", 0.2)
    monkeypatch.setattr("derouter.experimental_mcp_client.tools.MCP_TOOL_LISTING_TIMEOUT", 0.2)
...
async def test_pagination_walk_honors_explicit_deadline_over_globals(mock_session, monkeypatch):
    ...
    monkeypatch.setattr("derouter.experimental_mcp_client.tools.MCP_CLIENT_TIMEOUT", 0.1)
    monkeypatch.setattr("derouter.experimental_mcp_client.tools.MCP_TOOL_LISTING_TIMEOUT", 0.1)
...
def test_transform_mcp_tool_to_openai_responses_api_tool():
    ...
    assert openai_tool["name"] == "GitMCP-fetch_derouter_documentation"
    assert openai_tool["type"] == "function"
