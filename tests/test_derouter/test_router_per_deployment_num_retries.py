"""
Unit tests for per-deployment num_retries in derouter_params
GitHub Issue: #18968 - Per-deployment max_retries/num_retries in derouter_params is not used in retry logic
"""
...
class TestPerDeploymentNumRetries:
    ...
    def test_update_kwargs_does_not_fill_in_a_num_retries_default(self):
        ...
        derouter_params.num_retries outrank the header/body value.
        """
        router = Router(
            model_list=[
                {
...
class TestNoProviderRetryAmplification:
    ...
    ``derouter_params`` is applied twice - once by the Router loop and once as the
    ...
    path by injecting a counting transport via ``derouter.aclient_session`` (the
    ...
    def _install_counting_upstream() -> dict:
        ...
        derouter.aclient_session = httpx.AsyncClient(transport=httpx.MockTransport(handler))
        return counter
    ...
    async def _isolate_clients(self):
        ...
        yield
        session = derouter.aclient_session
        derouter.aclient_session = None
        derouter.in_memory_llm_clients_cache.flush_cache()
        if session is not None:
            await session.aclose()

    @staticmethod
    def _router(api_base: str, derouter_params: dict, **router_kwargs) -> Router:
        params = {"model": "openai/gpt-4o-mini", "api_base": api_base, "api_key": "sk-fake"}
        params.update(derouter_params)
        return Router(model_list=[{"model_name": "mock", "derouter_params": params}], **router_kwargs)
    ...
    async def test_direct_completion_still_forwards_num_retries_to_provider(self):
        """
        For a NON-routed direct ``derouter.acompletion`` call, ``num_retries`` remains an
        alias for the provider client's ``max_retries`` (the instructor use case). The
        provider SDK therefore retries in addition to derouter's own retry wrapper, so the
...
class TestRequestNumRetriesBeatsGlobal:
    """
    A per-request num_retries (request body or the x-derouter-num-retries header, both of
    which arrive as the num_retries kwarg) must take precedence over the global
    derouter.num_retries (derouter_settings.num_retries on the proxy) during retry handling.

    The regression: the @client wrapper stamped the global derouter.num_retries onto the
    ...
    real retry loop end to end (the failing call flows through the wrapped derouter.acompletion),
    which the kwargs-merge-only test above does not.
    """

    @pytest.fixture(autouse=True)
    def _restore_derouter_globals(self):
        prev_num_retries = derouter.num_retries
        prev_callbacks = derouter.callbacks
        yield
        derouter.num_retries = prev_num_retries
        derouter.callbacks = prev_callbacks
    ...
    async def _count_attempts(self, *, global_num_retries, request_num_retries):
        ...
        derouter.callbacks = [counter]
        derouter.num_retries = global_num_retries
    ...
    async def test_deployment_num_retries_reaches_wrapper_when_no_request_value(self):
        ...
        derouter_params.num_retries reaches the wrapped call, is carried on the raised
        exception, and is applied: deployment 2 -> 1 initial + 2 retries = 3 (not 1).
        """
        counter = _AttemptCounter()
        derouter.callbacks = [counter]
        derouter.num_retries = None
        router = Router(
            model_list=[
                {
                    "model_name": "mock",
...
class TestRequestNumRetriesBeatsDeployment:
    ...
    Documented precedence for num_retries on the proxy:
        ...
        x-derouter-num-retries header > request body > model_list derouter_params > derouter_settings
    ...
    The regression: a failing deployment stamps its own derouter_params.num_retries onto the
    ...
    def _router(*, global_num_retries, deployment_num_retries):
        ...
        if deployment_num_retries is not None:
            derouter_params["num_retries"] = deployment_num_retries
        return Router(
            model_list=[{"model_name": "mock", "derouter_params": derouter_params}],
            num_retries=global_num_retries,
        )
    ...
    async def _count_attempts(
        ...
        *,
        ...
        with patch("asyncio.sleep", return_value=None):
            with pytest.raises((derouter.InternalServerError, derouter.RateLimitError)):
                await router.acompletion(**kwargs)
        return counter.attempts
    ...
    async def test_deployment_num_retries_still_beats_global_when_request_omits_it(self):
        """
        The deployment value keeps its place directly above derouter_settings: global=1 and
...
class TestDeploymentNumRetriesOnNonCompletionEntryPoints:
    ...
    run through the same retry loop as acompletion, so a deployment's derouter_params.num_retries must
    ...
    @pytest.fixture(autouse=True)
    def _isolate(self):
        prev = derouter.num_retries
        derouter.num_retries = None
        derouter.in_memory_llm_clients_cache.flush_cache()
        yield
        derouter.num_retries = prev
        derouter.aclient_session = None
        derouter.in_memory_llm_clients_cache.flush_cache()
    ...
    async def _upstream_count(self, label, deployment_num_retries=None, **call_kwargs):
        ...
        if deployment_num_retries is not None:
            derouter_params["num_retries"] = deployment_num_retries
        router = Router(
            model_list=[{"model_name": "img", "derouter_params": derouter_params}],
            num_retries=0,
        )
        with patch("asyncio.sleep", return_value=None):
            with pytest.raises(derouter.InternalServerError):
