...
DeRouter previously raised rate-limit conditions through *several* unrelated
exception types — :class:`derouter.RateLimitError` (vendor 429s),
:class:`fastapi.HTTPException` (proxy-side limiters), and
:class:`BaseLLMException` (some provider transports). These tests pin down
the new behavior:

1. Every rate-limit exception is a :class:`derouter.RateLimitError` and exposes
...
3. The :class:`RateLimitErrorCategory` constants are exported on the
   ``derouter`` module so user code can import them without reaching into
   internal modules.
"""
...
import derouter
from derouter.exceptions import RateLimitError, RateLimitErrorCategory, RateLimitType
...
class TestRateLimitErrorCategory:
    def test_should_export_category_enum_on_derouter_module(self):
        assert hasattr(derouter, "RateLimitErrorCategory")
        assert derouter.RateLimitErrorCategory is RateLimitErrorCategory

    def test_should_define_all_documented_categories(self):
        # The Linear ticket explicitly lists vendor_rate_limit, derouter_rate_limit
        # and vendor_batch_rate_limit. We additionally expose a derouter_batch_*
        ...
        assert RateLimitErrorCategory.DEROUTER_RATE_LIMIT == "derouter_rate_limit"
        assert (
            RateLimitErrorCategory.DEROUTER_BATCH_RATE_LIMIT
            == "derouter_batch_rate_limit"
        )
...
class TestRateLimitErrorCategoryAttribute:
    ...
    def test_should_accept_enum_category_and_normalize_to_string(self):
        e = RateLimitError(
            message="oops",
            llm_provider="derouter",
            model="gpt-4",
            category=RateLimitErrorCategory.DEROUTER_RATE_LIMIT,
        )
        ...
        assert e.category == "derouter_rate_limit"
...
class TestProxyRateLimitError:
    ...
    def test_should_default_category_to_derouter_rate_limit(self):
        # ProxyRateLimitError is only used by derouter's own proxy-side
        # limiters, so its default category must reflect that. The vendor
        # default lives on the parent RateLimitError.
        e = ProxyRateLimitError(detail="over limit")
        assert e.category == RateLimitErrorCategory.DEROUTER_RATE_LIMIT

    def test_should_accept_derouter_batch_rate_limit_category(self):
        e = ProxyRateLimitError(
            detail="batch over limit",
            category=RateLimitErrorCategory.DEROUTER_BATCH_RATE_LIMIT,
        )
        assert e.category == "derouter_batch_rate_limit"
    ...
    def test_should_be_catchable_as_rate_limit_error(self):
        ...
        assert exc_info.value.category == "derouter_rate_limit"
...
class TestProxyHookCategoryWiring:
    ...
    def test_max_budget_limiter_raises_proxy_rate_limit_error(self):
        ...
        assert exc_info.value.status_code == 429
        assert exc_info.value.category == RateLimitErrorCategory.DEROUTER_RATE_LIMIT
        ...
        from derouter.proxy.hooks import max_budget_limiter
    ...
    @pytest.mark.parametrize(
        "module_path",
        [
            "derouter.proxy.hooks.parallel_request_limiter",
            "derouter.proxy.hooks.parallel_request_limiter_v3",
            "derouter.proxy.hooks.dynamic_rate_limiter",
            "derouter.proxy.hooks.dynamic_rate_limiter_v3",
            "derouter.proxy.hooks.batch_rate_limiter",
            "derouter.proxy.hooks.max_budget_limiter",
            "derouter.proxy.hooks.max_budget_per_session_limiter",
            "derouter.proxy.hooks.max_iterations_limiter",
        ],
    )
...
class TestStandardLoggingPayloadCarriesCategory:
    ...
    def test_should_propagate_category_for_proxy_rate_limit_error(self):
        ...
        assert info["error_rate_limit_category"] == "derouter_rate_limit"
    ...
    def test_should_propagate_derouter_batch_rate_limit_category(self):
        from derouter.derouter_core_utils.derouter_logging import (
            StandardLoggingPayloadSetup,
        )
        ...
        info = StandardLoggingPayloadSetup.get_error_information(e)
        assert info["error_rate_limit_category"] == "derouter_batch_rate_limit"
...
class TestProxyHooksActuallyRaiseProxyRateLimitError:
    ...
    async def test_max_iterations_limiter_raises_proxy_rate_limit_error(self):
        ...
        from derouter.proxy.hooks.max_iterations_limiter import (
            _PROXY_MaxIterationsHandler,
        )
        from derouter.proxy.utils import InternalUsageCache
        from derouter.types.agents import AgentResponse
        ...
        agent = AgentResponse(
            ...
            derouter_params={"max_iterations": 1},
            agent_card_params={"name": "iter-agent", "version": "1.0.0"},
        )
        with patch(
            "derouter.proxy.agent_endpoints.agent_registry.global_agent_registry"
        ) as mock_registry:
    ...
    async def test_max_budget_limiter_raises_proxy_rate_limit_error(self):
        ...
        from derouter.caching.caching import DualCache
        from derouter.proxy._types import UserAPIKeyAuth
        from derouter.proxy.hooks.max_budget_limiter import (
    ...
    async def test_max_budget_per_session_limiter_raises_proxy_rate_limit_error(
        ...
        from derouter.caching.caching import DualCache
        from derouter.proxy._types import UserAPIKeyAuth
        from derouter.proxy.hooks.max_budget_per_session_limiter import (
        ...
        agent = MagicMock()
        agent.derouter_params = {"max_budget_per_session": 1.0}
        with patch(
            "derouter.proxy.agent_endpoints.agent_registry.global_agent_registry"
        ) as mock_registry:
            mock_registry.get_agent_by_id.return_value = agent
    ...
    def test_batch_rate_limiter_helper_raises_with_derouter_batch_category(self):
        """
        Direct invocation of `_PROXY_BatchRateLimiter._raise_rate_limit_error`
        — confirms the batch limiter tags with `DEROUTER_BATCH_RATE_LIMIT`
        instead of the generic `DEROUTER_RATE_LIMIT`.
        """
        from unittest.mock import MagicMock

        from derouter.proxy.hooks.batch_rate_limiter import (
            BatchFileUsage,
        ...
        e = exc_info.value
        assert e.status_code == 429
        # Critical: batch category, NOT the default derouter_rate_limit.
        assert e.category == RateLimitErrorCategory.DEROUTER_BATCH_RATE_LIMIT
        assert isinstance(e, RateLimitError)
        assert isinstance(e, HTTPException)
...
class TestRateLimitType:
    ...
    `category` answers *who* rate-limited (vendor vs. derouter); `type`
    ...
    def test_should_export_type_enum_on_derouter_module(self):
        assert hasattr(derouter, "RateLimitType")
        assert derouter.RateLimitType is RateLimitType
...
class TestBudgetExceededErrorSurfacesUnifiedFields:
    ...
    raises :class:`derouter.BudgetExceededError`, which historically had no
    ...
    def test_should_carry_derouter_rate_limit_category(self):
        e = derouter.BudgetExceededError(current_cost=0.5, max_budget=0.1)
    ...
    def test_should_accept_llm_provider_kwarg(self):
        ...
        e = derouter.BudgetExceededError(
    ...
    def test_should_keep_existing_status_code_and_message(self):
        ...
        e = derouter.BudgetExceededError(current_cost=0.000109, max_budget=0.0001)
        assert e.status_code == 429
    ...
    def test_should_still_be_catchable_as_exception_not_rate_limit_error(self):
        ...
        assert isinstance(e, derouter.BudgetExceededError)
...
class TestBudgetExceededErrorLlmProviderEnrichment:
    ...
    async def _run_handler_and_capture_exception_seen_by_callback(
        ...
        with (
            ...
            patch(
                "derouter.proxy.auth.auth_exception_handler._get_request_ip_address",
    ...
    async def test_should_resolve_llm_provider_from_request_data_when_unset(self):
        err = derouter.BudgetExceededError(current_cost=100, max_budget=10)
    ...
    async def test_should_not_overwrite_llm_provider_when_caller_set_it(self):
        err = derouter.BudgetExceededError(
    ...
    async def test_should_fall_back_to_derouter_proxy_when_model_missing(self):
        err = derouter.BudgetExceededError(current_cost=100, max_budget=10)
        seen = await self._run_handler_and_capture_exception_seen_by_callback(err, {})
        assert seen.llm_provider == "derouter_proxy"
