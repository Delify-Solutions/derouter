"""
Unit tests for derouter/batches/batch_utils.py
...
maps (derouter.completion_cost, batch_cost_calculator), the tokenizer
...
import derouter
import derouter.batches.batch_utils as bu
...
def test_output_models_uses_model_name_override(monkeypatch):
    monkeypatch.setattr(derouter, "completion_cost", lambda **kw: 0.0)
...
def test_aggregate_returns_batch_cost_usage_result_dataclass(monkeypatch):
    import derouter.cost_calculator as cc
...
def test_empty_body_line_does_not_zero_whole_batch():
    """A status-200 row with an empty body makes derouter.completion_cost raise;
    ...
    reconciling with the provider's own request_counts over a derouter-side
...
def _batch(output_file_id):
    from derouter.types.llms.openai import Batch
...
async def test_output_file_content_vertex_fetches_via_afile_content(monkeypatch):
    import derouter.files.main as files_main
    ...
    assert captured["gcs_bucket_name"] == "derouter-bucket"
...
async def test_output_file_content_vertex_unified_file_id_extracts_gcs_uri(monkeypatch):
    ...
    monkeypatch.setattr(files_main, "afile_content", fake_afile_content)
    unified_id = (
        "derouter_proxy:application/jsonl;unified_id,uuid-1;target_model_names,vertex-model;"
        "llm_output_file_id,gs://derouter-bucket/output/predictions.jsonl;llm_output_file_model_id,model-1"
    )
...
async def test_output_file_content_model_encoded_file_id_decoded_to_provider_id(monkeypatch):
    import derouter.files.main as files_main
    from derouter.proxy.openai_files_endpoints.common_utils import encode_file_id_with_model
...
def _vertex_predictions_row(custom_id, prompt_tokens, completion_tokens):
    return {
        "request": {
            ...
            "labels": {"derouter_custom_id": custom_id},
        },
        "status": "",
        "response": {
            "candidates": [
...
async def test_output_file_content_vertex_managed_uri_accepted_by_real_validation(respx_interceptable_httpx_client):
    managed_output_uri = (
        "gs://derouter-bucket/derouter-vertex-files/publishers/google/models/"
    ...
    route = respx.get(url__regex=r"https://storage\.googleapis\.com/storage/v1/b/derouter-bucket/o/.*").mock(
    ...
    assert request.url.raw_path == (
        b"/storage/v1/b/derouter-bucket/o/"
        b"derouter-vertex-files%2Fpublishers%2Fgoogle%2Fmodels%2Fgemini-3.6-flash"
...
async def test_output_file_content_vertex_foreign_bucket_rejected_by_real_validation():
    with pytest.raises(Exception, match="does not match the configured storage bucket"):
        await bu._fetch_batch_output_file_content(
            _batch("gs://attacker-bucket/derouter-vertex-files/x/predictions.jsonl"),
            custom_llm_provider="vertex_ai",
            derouter_params={
                "api_key": "test-token",
                "vertex_project": "proj-1",
...
async def test_handle_completed_vertex_batch_computes_cost_usage_and_models(monkeypatch):
    ...
    result = await bu._handle_completed_batch(
        _batch("gs://derouter-bucket/output/predictions.jsonl"),
        custom_llm_provider="vertex_ai",
        derouter_params={"vertex_project": "proj-1", "vertex_location": "us-central1"},
    )

    pricing = derouter.model_cost["vertex_ai/gemini-3.6-flash"]
...
async def test_output_file_content_fetches_and_parses(monkeypatch):
    import derouter.files.main as files_main
    import derouter.proxy.openai_files_endpoints.common_utils as cu
    ...
    result = await bu._fetch_batch_output_file_content(
        ...
        derouter_params={"api_key": "sk-az", "api_base": "https://az", "model": "x"},
    )
...
async def test_output_file_content_unified_file_id_extraction(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda fid: "derouter_proxy;llm_output_file_id,real-file-99;rest",
    )
...
async def test_handle_completed_batch_orchestration(monkeypatch):
    ...
    async def fake_fetch(batch, custom_llm_provider, derouter_params=None):
...
async def test_handle_completed_batch_decodes_model_encoded_error_file_id(monkeypatch):
    ...
    encoded_error_file_id = "file-" + base64.urlsafe_b64encode(
        f"derouter:{provider_error_file_id};model,model-abc".encode()
    ).decode().rstrip("=")
...
def test_extract_credentials_forwards_the_trusted_model_credential_snapshot():
    ...
    credentials = bu._extract_file_access_credentials({"_derouter_internal_model_credentials": snapshot})

    assert credentials["_derouter_internal_model_credentials"] is snapshot
...
def test_extract_credentials_forwards_the_deployment_aws_credentials():
    ...
    derouter_params, and the S3 read of the output file signs with whatever afile_content
    receives. Dropping them here sent the read to the ambient credential chain, so a
    deployment whose only AWS credentials live in its derouter_params never recorded
...
async def test_output_file_content_bedrock_reads_with_deployment_aws_credentials(monkeypatch):
    ...
    await bu._fetch_batch_output_file_content(
        _batch("s3://configured-bucket/derouter-batch-outputs/job-1/out.jsonl.out"),
        custom_llm_provider="bedrock",
        derouter_params={
            "aws_access_key_id": "AKIA-deployment",
            "aws_secret_access_key": "secret-deployment",
            "aws_session_token": "token-deployment",
            "aws_region_name": "us-west-2",
            "_derouter_internal_model_credentials": snapshot,
            "model": "bedrock/us.anthropic.claude-haiku-4-5-20251001-v1:0",
        },
    )

    assert captured["file_id"] == "s3://configured-bucket/derouter-batch-outputs/job-1/out.jsonl.out"
    ...
    assert captured["_derouter_internal_model_credentials"] is snapshot
    assert "model" not in captured
...
async def test_handle_completed_bedrock_batch_prices_from_deployment_model(monkeypatch) -> None:
    ...
    async def fake_fetch(batch: object, custom_llm_provider: str, derouter_params: dict | None = None) -> bytes:
        return _vertex_jsonl(rows)
