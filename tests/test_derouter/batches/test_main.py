"""
Provider-dispatch contract tests for derouter/batches/main.py
...
import derouter
import derouter.batches.main as bm
...
# must reach the provider handler. Explicit kwargs win over derouter.* globals and
# env vars (they are first in each `optional_params.x or derouter.x or env` chain),
...
def _params(**kw):
    ...
    return GenericDeRouterParams(**kw)
...
def test_retrieve__forwards_trusted_model_credentials_into_derouter_params(seams):
    """The batch's cost is computed by reading its output file after the retrieve, and
    Bedrock resolves that bucket only from this immutable snapshot. get_derouter_params has
    ...
    derouter_params = logging_obj.update_from_kwargs.call_args.kwargs["derouter_params"]
    assert derouter_params["_derouter_internal_model_credentials"] is snapshot
...
def test_retrieve__omits_trusted_model_credentials_when_not_supplied(seams):
    ...
    bm.retrieve_batch(batch_id="batch-1", custom_llm_provider="openai", derouter_logging_obj=logging_obj)

    derouter_params = logging_obj.update_from_kwargs.call_args.kwargs["derouter_params"]
    assert "_derouter_internal_model_credentials" not in derouter_params
