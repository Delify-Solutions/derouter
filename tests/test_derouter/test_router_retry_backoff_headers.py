...
async def test_retry_backoff_uses_current_exception_headers():
    ...
    first_error = derouter.RateLimitError(
        message="Rate limited on first attempt",
        model="gpt-3.5-turbo",
        llm_provider="openai",
    )
    first_error.derouter_response_headers = httpx.Headers({"retry-after": "1"})

    second_error = derouter.RateLimitError(
        message="Rate limited on second attempt",
        model="gpt-3.5-turbo",
        llm_provider="openai",
    )
    second_error.derouter_response_headers = httpx.Headers({"retry-after": "15"})

    third_error = derouter.RateLimitError(
        message="Rate limited on third attempt",
        model="gpt-3.5-turbo",
        llm_provider="openai",
    )
    third_error.derouter_response_headers = httpx.Headers({"retry-after": "30"})
