...
When deployments have `order` set in derouter_params, lower order deployments
...
from derouter.utils import _get_deployment_order, get_order_filtered_deployments
...
async def test_router_order_fallback_does_not_reselect_order_1_when_order_2_is_filtered_out():
    ...
    derouter.callbacks.append(drop_order_2)
    try:
        ...
        derouter.callbacks.remove(drop_order_2)
...
async def test_generic_api_call_strips_target_order_from_provider_kwargs():
    ...
    router = Router(
        model_list=[
            {
                "model_name": "test-model",
                "derouter_params": {"model": "gpt-4o", "api_key": "key", "order": 2},
                "model_info": {"id": "2"},
            },
        ],
    )
