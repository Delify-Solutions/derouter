...
Ensures shared_session is in all_derouter_params to prevent
...
def test_shared_session_in_all_derouter_params():
    """
    CRITICAL: shared_session must be in all_derouter_params.

    If missing, it gets passed to provider APIs causing JSON serialization errors.
    Regression test for commit 819a6b5f18.
    """
    assert "shared_session" in all_derouter_params
...
def test_openai_embedding_passes_shared_session():
    ...
    Full chain: derouter.embedding() -> OpenAI.embedding() -> _get_openai_client()
    ...
    # Step 1: derouter.embedding() extracts and passes shared_session
    main_source = inspect.getsource(derouter.embedding)
