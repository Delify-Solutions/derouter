...
class TestLangfuseInMemoryCache:
    ...
    @patch("derouter.initialized_langfuse_clients", 5)
    def test_langfuse_client_count_decrements_on_eviction(self):
        ...
        with patch(
            "derouter.integrations.langfuse.langfuse.LangFuseLogger", MockLangFuseLogger
        ):
            ...
            initial_count = derouter.initialized_langfuse_clients

            # Trigger eviction
            self.cache.evict_cache()

            # Verify client count was decremented
            assert derouter.initialized_langfuse_clients == initial_count - 1

    @patch("derouter.initialized_langfuse_clients", 3)
