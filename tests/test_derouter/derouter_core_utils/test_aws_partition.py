...
from derouter.derouter_core_utils.aws_partition import (
    AwsPartition,
    contains_aws_arn,
...
from derouter.llms.aws_polly.text_to_speech.transformation import AWSPollyTextToSpeechConfig
...
from derouter.llms.sagemaker.chat.transformation import SagemakerChatConfig
...
ENDPOINT_BUILDERS: Final = {
    ...
    "polly": lambda region: AWSPollyTextToSpeechConfig().get_complete_url(
        model="polly/neural",
        api_base=None,
        derouter_params={"aws_region_name": region},
    ),
...
def _fstring_literal_offenders(needle: str) -> list[str]:
    derouter_root = Path(derouter.__file__).parent
    return [
        f"{path.relative_to(derouter_root)}: {part.value!r}"
        for path in sorted(derouter_root.rglob("*.py"))
