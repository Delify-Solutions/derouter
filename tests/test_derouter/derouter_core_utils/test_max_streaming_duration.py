"""
Tests for DEROUTER_MAX_STREAMING_DURATION_SECONDS — the global cap on streaming response wall-clock time.
...
class TestCustomStreamWrapperMaxDuration:
    def test_should_not_raise_when_duration_is_none(self):
        ...
        with patch("derouter.constants.DEROUTER_MAX_STREAMING_DURATION_SECONDS", None):
    ...
    def test_should_not_raise_when_under_limit(self):
        ...
        wrapper = _make_custom_stream_wrapper()
        with patch("derouter.constants.DEROUTER_MAX_STREAMING_DURATION_SECONDS", 60.0):
            wrapper._check_max_streaming_duration()  # should not raise

    def test_should_raise_timeout_when_exceeded(self):
        """Stream exceeded the limit → derouter.Timeout."""
        wrapper = _make_custom_stream_wrapper()
        wrapper._stream_created_time = time.time() - 20  # simulate 20s elapsed
        with patch("derouter.constants.DEROUTER_MAX_STREAMING_DURATION_SECONDS", 10.0):
            with pytest.raises(derouter.Timeout, match="max streaming duration"):
...
class TestResponsesStreamingIteratorMaxDuration:
    ...
    def test_should_not_raise_when_duration_is_none(self):
        ...
        with patch.object(
            import_module("derouter.responses.streaming_iterator"), "DEROUTER_MAX_STREAMING_DURATION_SECONDS",
            None,
        ):
