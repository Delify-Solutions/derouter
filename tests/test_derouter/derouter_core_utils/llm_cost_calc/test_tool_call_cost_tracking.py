...
from derouter.types.llms.openai import FileSearchTool, ResponsesAPIResponse, WebSearchOptions
from derouter.types.utils import ModelResponse, StandardBuiltInToolsParams
...
def test_web_search_cost_low():
    ...
    model_info = derouter.get_model_info("gpt-4o-search-preview")
...
def test_anthropic_web_search_cost_from_raw_response_dict_when_usage_drops_server_tool_use():
    ...
    per_query_cost = derouter.get_model_info(model)["search_context_cost_per_query"][
...
def test_get_cost_for_vertex_ai_gemini_web_search(model, custom_llm_provider):
    ...
    This tests the fix for: https://github.com/Delify-Solutions/derouter/issues/XXXXX
    ...
    """
    from derouter.types.utils import Choices, Message, PromptTokensDetailsWrapper, Usage
...
def test_gemini_3x_web_search_billed_per_query(model, local_model_cost_map):
    ...
    model_info = derouter.get_model_info(model)
...
def test_openai_responses_web_search_priced_per_call(local_model_cost_map):
    ...
    model = "gpt-5-nano"
    per_call = derouter.get_model_info(model)["search_context_cost_per_query"][
        "search_context_size_medium"
    ]
...
def test_dated_search_preview_entries_carry_search_pricing(local_model_cost_map):
    ...
    for dated, undated in (
        ...
        assert (
            derouter.get_model_info(dated)["search_context_cost_per_query"]
            == derouter.get_model_info(undated)["search_context_cost_per_query"]
        )
...
def test_web_search_gate_reads_server_side_tool_usage_details_without_citations():
    ...
    """
    from derouter.llms.xai.cost_calculator import _DEFAULT_WEB_SEARCH_COST_PER_CALL
    from derouter.types.utils import Usage
...
def test_bedrock_mantle_web_search_billed_per_query(local_model_cost_map, model):
    ...
    pricing = derouter.get_model_info(model)["search_context_cost_per_query"]
    assert pricing == {
    ...
    response = _responses_with_web_search(
        model,
        actions=[{"type": "search", "query": "derouter"}, {"type": "search", "query": "bedrock web search"}],
...
def test_web_search_call_count_prefers_provider_reported_num_requests(local_model_cost_map, num_requests):
    ...
    response = _responses_with_web_search(
        model,
        actions=[
            {"type": "search", "query": "derouter"},
            {"type": "open_page", "url": "https://router.delify.vn/"},
        ],
