...
Regression guard for https://github.com/Delify-Solutions/derouter/issues/34309: the
...
def _logging_obj() -> Logging:
    return Logging(
        ...
        derouter_call_id="responses-cache-breakdown",
        function_id="f",
    )
...
def test_responses_api_cache_costs_are_itemized_in_the_breakdown(local_model_cost_map):
    ...
    rates = derouter.model_cost[MODEL]
    cached_tokens = 4012
    ...
    total = derouter.completion_cost(
...
def test_cache_fields_stay_unset_when_there_was_no_cache_activity(local_model_cost_map):
    ...
    assert breakdown["input_cost"] == pytest.approx(1000 * derouter.model_cost[MODEL]["input_cost_per_token"])
