...
from derouter.llms.gemini.image_generation.cost_calculator import (
    cost_calculator as gemini_image_generation_cost_calculator,
)
from derouter.llms.vertex_ai.image_generation.cost_calculator import (
    cost_calculator as vertex_image_generation_cost_calculator,
)
from derouter.types.llms.openai import FileSearchTool, WebSearchOptions
...
from derouter.types.utils import CacheCreationTokenDetails, Usage
...
def test_reasoning_tokens_no_price_set(_local_model_cost_map):
    ...
    model_cost_map = derouter.model_cost[model]
    usage = Usage(
...
def test_image_tokens_with_custom_pricing():
    ...
    with patch(
        "derouter.derouter_core_utils.llm_cost_calc.utils.get_model_info",
...
def test_video_output_tokens_gemini_omni_flash_preview(_local_model_cost_map):
    ...
    model_cost_map = derouter.model_cost[f"gemini/{model}"]
...
def test_get_model_info_propagates_off_peak_fields():
    ...
    info = derouter.get_model_info(model=model_name)
...
def test_get_token_base_cost_off_peak_wins_over_tiered_pricing():
    ...
    model_name = "derouter-test-off-peak-tiered"
    derouter.register_model(
        {
            model_name: {
                "derouter_provider": "openai",
...
def test_generic_cost_per_token_off_peak_reasoning_rate():
    ...
    model_name = "derouter-test-off-peak-reasoning"
...
def test_generic_cost_per_token_off_peak_block_without_reasoning_rate():
    ...
    _register_off_peak_reasoning_model("derouter-test-off-peak-model-reasoning-rate", block)
    _, with_model_rate = generic_cost_per_token(
        model="derouter-test-off-peak-model-reasoning-rate",
        usage=_off_peak_reasoning_usage(),
        custom_llm_provider="openai",
    ...
    _register_off_peak_reasoning_model("derouter-test-off-peak-no-reasoning-rate", block, reasoning_rate=None)
    _, without_model_rate = generic_cost_per_token(
        model="derouter-test-off-peak-no-reasoning-rate",
        usage=_off_peak_reasoning_usage(),
        custom_llm_provider="openai",
        current_time=inside_window,
    )
...
def test_generic_cost_per_token_off_peak_reasoning_rate_wins_over_the_tier():
    ...
    model_name = "derouter-test-off-peak-tiered-reasoning"
    derouter.register_model(
        {
            model_name: {
                "derouter_provider": "openai",
...
def test_generic_cost_per_token_off_peak_reasoning_rate_wins_over_the_service_tier():
    ...
    model_name = "derouter-test-off-peak-reasoning-service-tier"
    _register_off_peak_reasoning_model(
        model_name,
        {"hours_utc": "16:30-00:30", "output_cost_per_token": 1e-6, "output_cost_per_reasoning_token": 5e-7},
...
def test_apply_off_peak_pricing_treats_bool_as_unset_and_parses_strings():
    ...
    model_name = "derouter-test-off-peak-odd-values"
    _register_off_peak_reasoning_model(
        model_name,
        {
            "hours_utc": "16:30-00:30",
    ...
    rates = apply_off_peak_pricing(
        derouter.get_model_info(model_name, custom_llm_provider="openai"),
...
def test_get_token_type_cost_breakdown_reflects_off_peak_reasoning_and_cache_creation_rates():
    ...
    model_name = "derouter-test-off-peak-breakdown"
    _register_off_peak_reasoning_model(
        model_name,
        {
            "hours_utc": "16:30-00:30",
...
def test_bedrock_mantle_gpt56_sol_cache_write_matches_aws_invoiced_rate(_local_model_cost_map):
    ...
    sol = derouter.model_cost["bedrock_mantle/openai.gpt-5.6-sol"]
...
def test_generic_cost_per_token_honors_non_standard_above_threshold():
    ...
    model = "derouter-test-non-standard-tier"
    custom_llm_provider = "openai"
    derouter.register_model(
        {
            model: {
                "derouter_provider": custom_llm_provider,
                "mode": "chat",
                "input_cost_per_token": 1e-6,
                "output_cost_per_token": 2e-6,
    ...
    try:
        ...
        derouter.model_cost.pop(model, None)
...
def test_generic_cost_per_token_tiered_pricing_charges_cache_creation_at_tier_rate():
    ...
    model = "derouter-test-tiered-cache-creation"
...
def test_generic_cost_per_token_tiered_pricing_is_all_or_nothing():
    ...
    model = "derouter-test-tiered-all-or-nothing"
    custom_llm_provider = "openrouter"
    derouter.register_model(
        {
            model: {
...
def test_generic_cost_per_token_tier_without_an_output_rate_bills_the_model_rate():
    ...
    model = "derouter-test-tiered-input-only"
    custom_llm_provider = "openrouter"
    derouter.register_model(
        {
            model: {
...
def test_generic_cost_per_token_tier_without_cache_rates_bills_cache_at_the_tier_input_rate():
    model = "derouter-test-tiered-no-cache-rates"
    custom_llm_provider = "openrouter"
    derouter.register_model(
        {
            model: {
...
def test_generic_cost_per_token_tier_without_a_1hr_cache_rate_bills_the_tier_cache_creation_rate():
    model = "derouter-test-tiered-no-1hr-cache-rate"
    custom_llm_provider = "openrouter"
    derouter.register_model(
        {
            model: {
...
def test_generic_cost_per_token_tier_without_an_input_rate_is_not_a_priced_tier():
    model = "derouter-test-tiered-no-input-rate"
    custom_llm_provider = "openrouter"
    derouter.register_model(
        {
            model: {
...
def test_router_deployment_with_input_only_tiers_bills_completions_at_the_backend_rate():
    ...
    model_id = "derouter-test-router-tiered-input-only"
    backend_model = "anthropic/claude-haiku-4-5"
    backend_output_rate = derouter.get_model_info(backend_model)["output_cost_per_token"]
    Router(
        model_list=[
            {
    ...
    try:
        ...
        derouter.model_cost.pop(model_id, None)
...
def test_generic_cost_per_token_tiered_pricing_bills_reasoning_at_tier_rate():
    ...
    model = "derouter-test-tiered-reasoning"
    custom_llm_provider = "openrouter"
    derouter.register_model(
        {
            model: {
...
def test_generic_cost_per_token_gpt55(_local_model_cost_map):
    ...
    assert model_cost_map["derouter_provider"] == "openai"
...
def test_gpt_5_6_alias_prices_match_sol(local_model_cost_map):
    ...
    who used the alias."""
    alias = derouter.model_cost["gpt-5.6"]
    sol = derouter.model_cost["gpt-5.6-sol"]
...
def test_generic_cost_per_token_azure_gpt56(_local_model_cost_map, 
    ...
    model_cost_map = derouter.model_cost[model]
    assert model_cost_map["derouter_provider"] == "azure"
    assert model_cost_map["input_cost_per_token"] == input_cost
    assert model_cost_map["output_cost_per_token"] == output_cost
    assert model_cost_map["cache_read_input_token_cost"] == cache_read_cost
    assert model_cost_map["max_input_tokens"] == 922000
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # The JSON must reflect this so DeRouter rejects unsupported values locally
...
def test_gpt55_dated_variants_match_base_reasoning_effort_capabilities(_local_model_cost_map, 
    ...
    base = derouter.model_cost[base_model]
    dated = derouter.model_cost[dated_model]
...
def test_azure_gpt55_entries_present_with_correct_pricing(_local_model_cost_map, 
    ...
    m = derouter.model_cost[model]
    assert m["derouter_provider"] == "azure"
...
def test_generic_cost_per_token_overlapping_cached_and_image_tokens():
    ...
    model = "derouter-test-overlapping-cached-image"
    derouter.register_model(
        {
            model: {
                "derouter_provider": "openai",
...
def test_generic_cost_per_token_warm_prefix_cache_spanning_text_and_image_tokens():
    ...
    0.001274 bill."""
    model = "derouter-test-warm-prefix-cache-overlap"
    derouter.register_model(
        {
            model: {
                "derouter_provider": "openai",
...
def test_calculate_cost_component_with_string_values():
    ...
    from derouter.derouter_core_utils.llm_cost_calc.utils import calculate_cost_component
...
def test_gemini_image_generation_cost_with_zero_text_tokens(_local_model_cost_map, model: str):
    ...
    https://github.com/Delify-Solutions/derouter/issues/17410
    """
...
def test_vertex_image_generation_cost_prefers_token_usage_metadata(_local_model_cost_map):
    ...
    model_info = derouter.get_model_info(model=model, custom_llm_provider="vertex_ai")
...
def test_gemini_image_generation_cost_prefers_token_usage_metadata(_local_model_cost_map):
    ...
    model_info = derouter.get_model_info(model=model, custom_llm_provider="gemini")
...
def test_reasoning_tokens_without_text_tokens_gpt5_nano():
    ...
    https://github.com/Delify-Solutions/derouter/issues/18599

    When OpenAI models (gpt-5-nano, o1, o3) return reasoning_tokens but don't provide
    text_tokens, DeRouter should calculate text_tokens as:
...
def test_service_tier_suffixes_constant_in_sync_with_enum():
    from derouter.derouter_core_utils.llm_cost_calc.utils import _SERVICE_TIER_SUFFIXES
    from derouter.types.utils import ServiceTier
...
def test_get_cost_per_unit_falls_back_from_service_tier_key_to_base():
    from derouter.derouter_core_utils.llm_cost_calc.utils import _get_cost_per_unit
...
def test_threshold_keys_exclude_service_tier_variants():
    ...
    from derouter.derouter_core_utils.llm_cost_calc.utils import _get_token_base_cost
    from derouter.types.utils import ModelInfo, Usage
...
def test_token_type_cost_breakdown_includes_cache_creation_from_top_level_usage(_local_model_cost_map):
    ...
    model_info = derouter.get_model_info(model=model, custom_llm_provider="bedrock")
...
def test_generic_cost_per_token_openai_cache_write_tokens_gpt_5_6(_local_model_cost_map):
    ...
    info = derouter.get_model_info(model=model, custom_llm_provider="openai")
...
def test_gemini_35_flash_lite_flex_cache_read_map_entries(_local_model_cost_map):
    ...
    assert derouter.model_cost["gemini-3.5-flash-lite"]["cache_read_input_token_cost_flex"] == 1.5e-08
    assert derouter.model_cost["vertex_ai/gemini-3.5-flash-lite"]["cache_read_input_token_cost_flex"] == 1.5e-08
    assert derouter.model_cost["gemini/gemini-3.5-flash-lite"]["cache_read_input_token_cost_flex"] == 2e-08
...
def test_priority_reasoning_tokens_bill_at_the_priority_output_rate(_local_model_cost_map):
    ...
    model_info = derouter.get_model_info(model="gemini-3.5-flash", custom_llm_provider="gemini")
...
def test_gemini_38_flash_matches_37_flash_promotional_pricing(prefix, _local_model_cost_map):
    new_model = derouter.model_cost[f"{prefix}gemini-3.8-flash"]
    old_model = derouter.model_cost[f"{prefix}gemini-3.7-flash"]
...
def test_grok_46_launch_pricing(_local_model_cost_map):
    model_cost_map = derouter.model_cost["xai/grok-4.6"]
...
def test_realtime_image_tokens_priced_per_token(model, provider, image_token_rate, _local_model_cost_map):
    ...
    text_rate = derouter.model_cost[model]["input_cost_per_token"]
...
def test_route_image_generation_cost_falls_back_to_requested_quality(
    ...
    def tier(cost):
        return {"derouter_provider": "xai", "mode": "image_generation", "input_cost_per_image": cost}
