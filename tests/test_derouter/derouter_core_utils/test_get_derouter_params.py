"""
Tests for get_derouter_params and related helpers.
...
class TestGetBaseModelFromLitellmCallMetadata:
    def test_none_metadata_returns_none(self):
        assert _get_base_model_from_derouter_call_metadata(None) is None

    def test_empty_metadata_returns_none(self):
        assert _get_base_model_from_derouter_call_metadata({}) is None

    def test_missing_model_info_returns_none(self):
        assert _get_base_model_from_derouter_call_metadata({"foo": "bar"}) is None

    def test_model_info_none_returns_none(self):
        assert _get_base_model_from_derouter_call_metadata({"model_info": None}) is None

    def test_model_info_empty_dict_returns_none(self):
        assert _get_base_model_from_derouter_call_metadata({"model_info": {}}) is None

    def test_returns_base_model(self):
        result = _get_base_model_from_derouter_call_metadata(
...
class TestGetLitellmParamsKwargsExtraction:
    ...
    def test_all_optional_kwargs_extractable(self):
        ...
        result = get_derouter_params(**kwargs)
        for key in _OPTIONAL_KWARGS_KEYS:
...
class TestGetLitellmParamsExplicitFields:
    ...
    def test_no_log_from_kwargs(self):
        ...
        result = get_derouter_params(**{"no-log": True})
        assert result["no-log"] is True

    def test_no_log_from_explicit_param(self):
        result = get_derouter_params(no_log=True)
        assert result["no-log"] is True
...
class TestGetLitellmParamsDataResidency:
    ...
    def test_no_api_base_is_none(self):
        result = get_derouter_params(custom_llm_provider="openai")
        assert result["data_residency"] is None
...
class TestMetadataFallsBackToLitellmMetadata:
    def test_metadata_falls_back_to_derouter_metadata_when_absent(self):
        result = get_derouter_params(derouter_metadata={"trace_id": "trace-1"})
        assert result["metadata"] == {"trace_id": "trace-1"}
        assert result["derouter_metadata"] == {"trace_id": "trace-1"}

    def test_empty_metadata_falls_back_to_derouter_metadata(self):
        result = get_derouter_params(metadata={}, derouter_metadata={"trace_id": "trace-1"})
        assert result["metadata"] == {"trace_id": "trace-1"}

    def test_metadata_wins_when_both_present(self):
        result = get_derouter_params(
            metadata={"trace_id": "from-metadata"},
            derouter_metadata={"trace_id": "from-derouter-metadata"},
        )
        assert result["metadata"] == {"trace_id": "from-metadata"}

    @pytest.mark.parametrize("bad_value", ["not-json-a-string", 12345, ["a"], True])
    def test_non_dict_derouter_metadata_is_ignored(self, bad_value):
        result = get_derouter_params(derouter_metadata=bad_value)
        assert result["metadata"] is None

    def test_metadata_stays_none_without_derouter_metadata(self):
        result = get_derouter_params(api_key="test-key")
        assert result["metadata"] is None

    def test_session_and_trace_id_derived_from_derouter_metadata(self):
        result = get_derouter_params(
            derouter_metadata={"trace_id": "trace-1", "session_id": "session-1"},
        )
        assert result["derouter_session_id"] == "session-1"
        assert result["derouter_trace_id"] == "trace-1"

    def test_explicit_session_and_trace_id_are_not_overridden(self):
        result = get_derouter_params(
            derouter_session_id="explicit-session",
            derouter_trace_id="explicit-trace",
            derouter_metadata={"trace_id": "trace-1", "session_id": "session-1"},
        )
        assert result["derouter_session_id"] == "explicit-session"
        assert result["derouter_trace_id"] == "explicit-trace"

    def test_derouter_metadata_fallback_is_copied_not_aliased(self):
        derouter_metadata = {"trace_id": "trace-1"}

        result = get_derouter_params(derouter_metadata=derouter_metadata)

        assert result["metadata"] == derouter_metadata
        assert result["metadata"] is not derouter_metadata
...
def test_drop_params_strings_reach_derouter_params_as_flags(value, expected):
    assert get_derouter_params(drop_params=value)["drop_params"] is expected
