...
from derouter.llms.xai.realtime.transformation import XAIRealtimeNormalizer
...
async def test_provider_path_suppresses_duplicate_session_created_after_synthetic():
    ...
    logging_obj = MagicMock()
    logging_obj.derouter_trace_id = "trace_1"
    logging_obj.async_success_handler = AsyncMock()
    logging_obj.success_handler = MagicMock()
...
async def test_deferred_setup_flushes_audio_on_backend_session_created(monkeypatch):
    ...
    logging_obj = MagicMock()
    logging_obj.derouter_trace_id = "trace_defer"
    logging_obj.async_success_handler = AsyncMock()
    logging_obj.success_handler = MagicMock()
    config = GeminiRealtimeConfig()
...
def test_store_message_skips_pydantic_for_unlogged_audio_delta():
    ...
    with patch("derouter.derouter_core_utils.realtime_streaming.OpenAIRealtimeStreamResponseBaseObject") as base_obj:
...
async def test_audio_delta_frame_parsed_at_most_once():
    ...
    with patch(
        "derouter.derouter_core_utils.realtime_streaming.json.loads",
...
def test_setup_folds_in_auto_response_disable_when_transcription_guardrail_active(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(derouter, "callbacks", [_transcription_guardrail()])
    streaming = RealTimeStreaming(MagicMock(), MagicMock(), MagicMock())
    setup = json.dumps(
        {
            "setup": {
...
async def test_log_messages_routes_async_logging_through_bounded_worker():
    ...
    with patch("derouter.derouter_core_utils.realtime_streaming.asyncio.create_task") as mock_create_task:
        await streaming.log_messages()
...
async def test_provider_config_path_captures_transcription_usage():
    ...
    from derouter.types.realtime import RealtimeInputAudioTranscriptionUsage, RealtimeResponseTypedDict
...
@pytest.mark.parametrize(
    ...
    (
        ...
        pytest.param("/etc/derouter/service-account.json", id="filesystem-path"),
    ),
)
@pytest.mark.asyncio
