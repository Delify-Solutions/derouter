#### What this tests ####
#    This tests derouter.token_counter.token_counter() function
import importlib
import time
import traceback
...
import derouter
from derouter import create_pretrained_tokenizer, decode, encode, get_modified_max_tokens
from derouter import token_counter as token_counter_old
import derouter.constants
from derouter.derouter_core_utils.token_counter import _get_tiktoken_count_function
from derouter.derouter_core_utils.token_counter import token_counter as token_counter_new
from tests.large_text import text
from tests.test_derouter.derouter_core_utils.messages_with_counts import (
    MESSAGES_TEXT,
...
def test_invalid_chunk_size_config_stays_usable(monkeypatch, configured):
    ...
    try:
        reloaded = importlib.reload(derouter.constants)
        ...
        importlib.reload(derouter.constants)


def test_valid_chunk_size_config_is_honoured(monkeypatch):
    monkeypatch.setenv("TIKTOKEN_ENCODE_CHUNK_SIZE_CHARS", "2048")
    try:
        assert importlib.reload(derouter.constants).TIKTOKEN_ENCODE_CHUNK_SIZE_CHARS == 2048
    finally:
        monkeypatch.delenv("TIKTOKEN_ENCODE_CHUNK_SIZE_CHARS")
        importlib.reload(derouter.constants)
...
def test_get_modified_max_tokens(
    ...
    derouter.token_counter.side_effect = _mock_token_counter
...
def test_gpt_4o_token_counter():
    with patch.object(
        derouter.utils, "openai_token_counter", new=MagicMock()
    ) as mock_client:
        token_counter(
...
def test_img_url_token_counter(img_url, monkeypatch):
    ...
    """
    import base64
    from derouter.derouter_core_utils.token_counter import get_image_dimensions
    ...
    if img_url.startswith(("http://", "https://")):
        ...
        monkeypatch.setattr(
            "derouter.derouter_core_utils.token_counter.safe_get",
...
from derouter.utils import _select_tokenizer_helper, claude_json_str, encoding
...
class TestTokenizerSelection(unittest.TestCase):
    ...
    @patch("derouter.utils.Tokenizer.from_pretrained")
    ...
    def test_cohere_tokenizer_api_failure(self, mock_from_pretrained):
        ...
        derouter.cohere_models = ["command-r-v1"]
    ...
    @patch("derouter.utils.Tokenizer.from_str")
    def test_claude_tokenizer_api_failure(self, mock_from_str):
        ...
        derouter.anthropic_models = ["claude-2"]
    ...
    @patch("derouter.utils._return_huggingface_tokenizer")
    def test_disable_hf_tokenizer_download(self, mock_return_huggingface_tokenizer):
        monkeypatch = pytest.MonkeyPatch()
        monkeypatch.setattr(derouter, "disable_hf_tokenizer_download", True)
        try:
...
def test_count_content_list_rejects_unknown_type():
    ...
    """
    from derouter.derouter_core_utils.token_counter import _count_content_list
...
def test_anthropic_image_source_resolves_to_what_the_image_pricer_reads(source: dict[str, str], expected: str):
    ...
    from derouter.derouter_core_utils.token_counter import _anthropic_image_source_data
