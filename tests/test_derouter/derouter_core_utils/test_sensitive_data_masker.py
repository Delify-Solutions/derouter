...
def test_excluded_keys_exact_match():
    ...
    data = {
        ...
        "derouter_credentials_name": "my-credential-name",
    ...
    # With excluded_keys, derouter_credentials_name should NOT be masked (exact match)
    # This ensures that even if pattern matching logic changes, excluded keys won't be masked
    masked = masker.mask_dict(data, excluded_keys={"derouter_credentials_name"})
    assert masked["derouter_credentials_name"] == "my-credential-name"
    ...
    masked = masker.mask_dict(data, excluded_keys={"DEROUTER_CREDENTIALS_NAME"})
    assert masked["derouter_credentials_name"] != "my-credential-name"
    assert "*" in masked["derouter_credentials_name"]
...
def test_extra_headers_are_masked_recursively():
    ...
    masked = masker.mask_dict(data)
    extra_headers = masked["derouter_params"]["extra_headers"]
