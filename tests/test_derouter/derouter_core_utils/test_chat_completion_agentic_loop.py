...
(`derouter/derouter_core_utils/chat_completion_agentic_loop.py`) and the
...
def restore_callbacks():
    """Save/restore derouter.callbacks so a registered fake logger never pollutes
    other tests in the suite."""
    saved = list(derouter.callbacks)
    try:
        yield
    finally:
...
class _LoggingStub:
    ...
    derouter_call_id off it."""

    derouter_call_id = "call-test"
...
async def test_dispatcher_runs_followup_with_incremented_depth_and_patched_messages(
    ...
    derouter.acompletion exactly once with _agentic_loop_depth == 1 and the
    patched messages. Loop-control state rides as derouter-level kwargs and is
    mirrored into derouter_metadata; the provider-surface transient
    _agentic_loop_api_surface is never forwarded. (Provider-body stripping of
    these derouter-level kwargs is asserted separately in test A.)"""
    ...
    with patch.object(derouter, "acompletion", acompletion_mock):
        result = await maybe_run_chat_completion_agentic_loop(
    ...
    # Loop-control state is carried at the derouter level for the follow-up.
    assert call_kwargs["max_agentic_loops"] >= 1
    assert "_agentic_loop_fingerprints" in call_kwargs
    # Interception markers are mirrored into derouter_metadata for the follow-up.
    assert call_kwargs["derouter_metadata"]["_code_interpreter_interception_active"] is True
...
async def test_dispatcher_raises_on_repeated_tool_call_fingerprint(restore_callbacks):
    ...
    gate_tool_calls = {"tool_calls": [{"id": "call_abc", "name": "derouter_code_execution"}]}
