...
from derouter.derouter_core_utils.health_check_helpers import (
    IMAGE_EDIT_HEALTH_CHECK_PROMPT,
    HealthCheckHelpers,
)
from derouter.main import ahealth_check
from derouter.proxy._types import UserAPIKeyAuth
from derouter.types.utils import LIST_BATCHES_SUPPORTED_PROVIDERS
...
async def test_image_edit_health_check_handler_uses_descriptive_prompt_and_multicolor_png():
    ...
    with patch(  # test-quality-ok: the public health-check path has no dependency injection seam
        "derouter.aimage_edit", new_callable=AsyncMock, return_value={}
    ) as mock_aimage_edit:
...
async def test_ahealth_check_image_edit_treats_content_policy_violation_as_healthy():
    moderation_error = derouter.ContentPolicyViolationError(
        message="Your request was rejected as a result of our safety system.",
        model="gpt-image-1",
        llm_provider="openai",
    )
    with patch(  # test-quality-ok: the public health-check path has no dependency injection seam
        "derouter.aimage_edit", new_callable=AsyncMock, side_effect=moderation_error
    ):
...
async def test_ahealth_check_image_edit_treats_moderation_blocked_code_as_healthy():
    moderation_blocked = derouter.BadRequestError(
        message=(
    ...
    with patch(  # test-quality-ok: the public health-check path has no dependency injection seam
        "derouter.aimage_edit", new_callable=AsyncMock, side_effect=moderation_blocked
    ):
        result = await ahealth_check(
            {"model": "gpt-image-1", "api_key": "sk-test"},
            mode="image_edit",
...
async def test_ahealth_check_image_edit_still_fails_on_non_moderation_errors():
    ...
    with patch(  # test-quality-ok: the public health-check path has no dependency injection seam
        "derouter.aimage_edit", new_callable=AsyncMock, side_effect=auth_error
    ):
        result = await ahealth_check(
...
def test_update_model_params_with_health_check_tracking_information():
    ...
    with patch(
        "derouter.proxy._types.UserAPIKeyAuth.get_derouter_internal_health_check_user_api_key_auth"
    ) as mock_get_auth:
        mock_auth = MagicMock()
        mock_get_auth.return_value = mock_auth

        with patch(
            "derouter.proxy.derouter_pre_call_utils.DeRouterProxyRequestSetup.add_user_api_key_auth_to_request_metadata"
        ) as mock_add_auth:
            ...
            # Verify that derouter_metadata was added
            assert "derouter_metadata" in result
            assert result["derouter_metadata"]["tags"] == [LITTELM_INTERNAL_HEALTH_SERVICE_ACCOUNT_NAME]
            ...
            assert call_args[1]["_metadata_variable_name"] == "derouter_metadata"
...
def test_get_derouter_internal_health_check_user_api_key_auth():
    """Test get_derouter_internal_health_check_user_api_key_auth returns properly configured UserAPIKeyAuth object."""
    result = UserAPIKeyAuth.get_derouter_internal_health_check_user_api_key_auth()
...
async def test_batch_health_check_bridges_metadata_into_logging_obj():
    ...
    model_call_details["derouter_params"]["metadata"]."""
    ...
    with patch("derouter.alist_batches", new_callable=AsyncMock, return_value={}):
    ...
    assert call_kwargs["derouter_params"] == {"api_base": "https://api.openai.com"}


@pytest.mark.asyncio
async def test_batch_health_check_omits_api_base_when_absent():
    """api_base must not appear in derouter_params when the provider resolves
    it implicitly (bedrock, vertex, gemini)."""
    mock_logging_obj = MagicMock()
    mock_logging_obj.update_from_kwargs = MagicMock()

    derouter_metadata = {"tags": [LITTELM_INTERNAL_HEALTH_SERVICE_ACCOUNT_NAME]}
    ...
    with patch("derouter.acompletion", new_callable=AsyncMock, return_value={}):
        await HealthCheckHelpers._batch_health_check(
    ...
    call_kwargs = mock_logging_obj.update_from_kwargs.call_args[1]
    assert call_kwargs["derouter_params"] is None


@pytest.mark.asyncio
async def test_batch_health_check_skips_bridge_when_no_logging_obj():
    """When derouter_logging_obj is absent, dispatch still proceeds."""
    derouter_metadata = {"tags": [LITTELM_INTERNAL_HEALTH_SERVICE_ACCOUNT_NAME]}
    ...
    with patch("derouter.alist_batches", new_callable=AsyncMock, return_value={}) as mock_alist:
        await HealthCheckHelpers._batch_health_check(
            custom_llm_provider="openai",
            model_params={"model": "openai/gpt-4"},
            filtered_model_params=filtered_model_params,
...
async def test_batch_health_check_falls_back_to_acompletion_for_unsupported():
    ...
    with (
        patch("derouter.alist_batches", new_callable=AsyncMock) as mock_alist,
        patch("derouter.acompletion", new_callable=AsyncMock, return_value={}) as mock_acompletion,
    ):
        await HealthCheckHelpers._batch_health_check(
            custom_llm_provider="bedrock",
            model_params=model_params,
...
async def test_realtime_health_check_uses_model_level_vertex_params():
    ...
    import derouter
    from derouter.realtime_api import main as realtime_main
    ...
    with (
        ...
        result = await derouter.ahealth_check(
            model_params={
...
async def test_ocr_health_check_sends_the_document_kind_the_provider_config_accepts(
    ...
    with patch(  # test-quality-ok: the public health-check path has no dependency injection seam
        "derouter.aocr", new_callable=AsyncMock, return_value={}
    ) as mock_aocr:
        await handlers["ocr"]()
