...
from derouter.constants import LOGGING_WORKER_AGGRESSIVE_CLEAR_COOLDOWN_SECONDS
from derouter.derouter_core_utils.logging_worker import LoggingWorker
...
class TestLoggingWorker:
    ...
    async def test_worker_handles_cancellation_gracefully(self, logging_worker):
        ...
        with patch(
            "derouter.derouter_core_utils.logging_worker.verbose_logger"
        ) as mock_logger:
            # Start the worker
            logging_worker.start()
