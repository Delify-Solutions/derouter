...
def _with_flag(model_info, declared=None, enabled=True):
    with patch.dict(os.environ, {"DEROUTER_ENABLE_PTU_COST_ATTRIBUTION": "True" if enabled else ""}, clear=False):
...
def test_a_setting_that_is_not_a_charge_is_left_alone():
    """CustomPricingDeRouterParams also carries configuration, and zeroing one of those
