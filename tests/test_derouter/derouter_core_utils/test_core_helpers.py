"""Tests for derouter_core_utils.core_helpers module."""
...
class TestGetOrCreateMetadataBucket:
    ...
    def test_prefers_derouter_metadata_when_both_present(self):
        request_data = {"metadata": {"user_id": "caller"}, "derouter_metadata": {}}

        key, bucket = get_or_create_metadata_bucket(request_data)

        assert key == "derouter_metadata"
        assert bucket is request_data["derouter_metadata"]

    def test_uses_metadata_when_derouter_metadata_absent(self):
    ...
    def test_replaces_a_non_dict_bucket(self):
        request_data = {"derouter_metadata": None}

        key, bucket = get_or_create_metadata_bucket(request_data)

        assert key == "derouter_metadata"
        assert isinstance(request_data["derouter_metadata"], dict)
        assert bucket is request_data["derouter_metadata"]
...
class TestMapFinishReasonGenericError:
    ...
    def test_lowercase_error_does_not_warn(self, mocker):
        warn = mocker.patch(
            "derouter.derouter_core_utils.core_helpers.verbose_logger.warning"
        )
        assert map_finish_reason("error") == "stop"
...
def test_drop_params_flag_returns_a_bool_without_a_warning(value, expected, caplog):
    with caplog.at_level(logging.WARNING, logger="drop-params-test"):
        assert drop_params_flag(value, "DEROUTER_DROP_PARAMS", logging.getLogger("drop-params-test")) is expected
    assert caplog.text == ""
...
def test_drop_params_flag_treats_non_flag_values_as_off_with_a_warning(value, caplog):
    with caplog.at_level(logging.WARNING, logger="drop-params-test"):
        assert drop_params_flag(value, "DEROUTER_DROP_PARAMS", logging.getLogger("drop-params-test")) is False
    assert f"DEROUTER_DROP_PARAMS={value!r} is not a flag value, treating it as off" in caplog.text
...
@pytest.mark.parametrize(
    ...
    [
        ({}, False),
        ({"DEROUTER_DROP_PARAMS": ""}, False),
        ({"DEROUTER_DROP_PARAMS": "   "}, False),
        ({"DEROUTER_DROP_PARAMS": "true"}, True),
        ({"DEROUTER_DROP_PARAMS": " False "}, False),
        ({"DEROUTER_DROP_PARAMS": "0"}, False),
    ],
)
...
def test_drop_params_env_flag_keeps_a_non_flag_value_on_with_a_warning(configured, caplog):
    with caplog.at_level(logging.WARNING, logger="drop-params-test"):
        assert drop_params_env_flag({"DEROUTER_DROP_PARAMS": configured}, logging.getLogger("drop-params-test")) is True
    assert (
        f"DEROUTER_DROP_PARAMS={configured!r} is not a flag value, treating it as on. Set it to true or false"
        in caplog.text
    )
...
class TestIsExpectedClientError:
    def test_status_ranges(self):
        from derouter.derouter_core_utils.core_helpers import is_expected_client_error
    ...
    def test_provider_originated_4xx_is_not_expected(self):
        ...
        from derouter.exceptions import AuthenticationError, RateLimitError
        from derouter.derouter_core_utils.core_helpers import is_expected_client_error
    ...
    def test_budget_rejection_decorated_with_provider_is_expected(self):
        ...
        from derouter.exceptions import BudgetExceededError, RateLimitError, RateLimitErrorCategory
        from derouter.derouter_core_utils.core_helpers import is_expected_client_error
