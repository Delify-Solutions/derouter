...
def test_iter_client_callback_metadata_dicts_covers_all_read_paths():
    ...
    slots = dict(
        iter_client_callback_metadata_dicts(
            {
                "metadata": md,
                "derouter_metadata": lm,
                "derouter_params": {"metadata": lp_md},
            }
        )
    )
    assert slots == {
...
def test_extractor_reads_turn_off_message_logging_from_every_slot():
    for kwargs in (
        ...
        {"derouter_metadata": {"turn_off_message_logging": True}},
        {"derouter_params": {"metadata": {"turn_off_message_logging": True}}},
    ):
...
def test_derouter_params_metadata_overrides_metadata():
    kwargs = {
        "metadata": {
            "langfuse_public_key": "pk-meta",
        },
        "derouter_params": {
            "metadata": {
                "langfuse_public_key": "pk-derouter-params",
            }
        },
    }
    ...
    assert params.get("langfuse_public_key") == "pk-derouter-params"
...
def test_top_level_kwargs_overrides_metadata_slots():
    kwargs = {
        ...
        "derouter_params": {"metadata": {"langfuse_public_key": "from-derouter-params"}},
    }
    params = initialize_standard_callback_dynamic_params(kwargs)
...
def test_gcs_bucket_name_in_derouter_params_metadata_is_ignored():
    kwargs = {
        "derouter_params": {
            "metadata": {
...
def test_newrelic_callback_params_are_not_extracted_from_request_kwargs():
    kwargs = {
        ...
        "derouter_params": {"metadata": {"newrelic_region": "eu"}},
    }
...
def test_newrelic_trusted_vars_overlay_reaches_standard_params():
    from derouter.types.utils import TRUSTED_CALLBACK_VARS_FIELD
