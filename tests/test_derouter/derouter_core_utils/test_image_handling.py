...
def test_invalid_image_url_raises_bad_request(monkeypatch):
    monkeypatch.setattr(derouter, "module_level_client", DummyClient())
...
def test_completion_with_invalid_image_url(monkeypatch):
    ...
    with pytest.raises(derouter.ImageFetchError) as excinfo:
        derouter.completion(model="gemini/gemini-pro", messages=messages, api_key="test")
...
def test_image_exceeds_size_limit_with_content_length(monkeypatch):
    ...
    """
    monkeypatch.setattr(derouter, "module_level_client", LargeImageClient(size_mb=100))
...
def test_streaming_download_protects_against_huge_files(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "module_level_client", client)
...
def test_image_within_size_limit(monkeypatch):
    ...
    """
    monkeypatch.setattr(derouter, "module_level_client", SmallImageClient())
...
def test_data_url_is_returned_unchanged_without_fetch(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "module_level_client", ExplodingClient())
...
async def test_async_data_url_is_returned_unchanged_without_fetch(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "module_level_aclient", ExplodingAsyncClient())
...
def test_image_size_limit_disabled(monkeypatch):
    ...
    """
    import derouter.derouter_core_utils.prompt_templates.image_handling as image_handling
...
async def test_async_inline_remote_media_cancels_the_other_fetches_when_one_fails(monkeypatch):
    ...
    with pytest.raises(derouter.ImageFetchError, match="Status code: 404"):
...
async def test_async_convert_url_to_base64_hides_the_ssrf_verdict_and_does_not_retry(monkeypatch):
    ...
    for verdict in _SSRF_VERDICTS:
        ...
        with pytest.raises(derouter.ImageFetchError) as raised:
