...
def _bundled_blob_id() -> str:
    path = os.path.join(os.path.dirname(__file__), "../../../derouter/model_prices_and_context_window_backup.json")
...
def _make_models(n: int) -> dict:
    return {
        f"model-{i}": {"derouter_provider": "openai", "mode": "chat"} for i in range(n)
    }
...
def test_shipped_routing_rules_never_match_through_an_unrecognized_namespace():
    """Routing rules decide ``derouter_provider`` for otherwise-unknown ids, and the
    ...
    routing_rules = [r for r in rules if "derouter_provider" in r["model_info"]]
...
def test_shipped_backup_marks_claude_4_6_plus_adaptive_not_4_0():
    ...
    assert "derouter_provider" not in baseline_rule["model_info"]
...
def test_get_model_cost_map_stamps_loaded_at():
    ...
    from derouter.derouter_core_utils import get_model_cost_map as module
...
def _unset_local_cost_map_env(monkeypatch):
    """CI exports DEROUTER_LOCAL_MODEL_COST_MAP=True; clear it so fetch behavior is deterministic."""
...
async def test_refetch_respects_local_env_override(monkeypatch):
    """DEROUTER_LOCAL_MODEL_COST_MAP=True short-circuits to the bundled backup, zero HTTP."""
...
def test_boot_load_respects_local_env_override(monkeypatch):
    """DEROUTER_LOCAL_MODEL_COST_MAP=True still short-circuits to the backup with zero HTTP."""
    monkeypatch.setenv("DEROUTER_LOCAL_MODEL_COST_MAP", "True")
