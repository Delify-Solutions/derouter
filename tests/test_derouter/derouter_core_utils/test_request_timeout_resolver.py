"""Unit tests for derouter.derouter_core_utils.request_timeout_resolver.

The resolver decides whether ``derouter.request_timeout`` was *explicitly configured*
(env REQUEST_TIMEOUT / derouter_settings, or a non-default runtime value) versus left
...
def test_default_value_without_flag_is_unset(restore_request_timeout):
    derouter.request_timeout = DEFAULT_REQUEST_TIMEOUT_SECONDS
...
def test_non_default_runtime_value_treated_as_explicit(restore_request_timeout):
    # SDK users assigning derouter.request_timeout directly (no flag) must keep working.
    derouter.request_timeout = 300
    derouter.request_timeout_explicitly_set = False
    assert get_configured_request_timeout() == 300.0
