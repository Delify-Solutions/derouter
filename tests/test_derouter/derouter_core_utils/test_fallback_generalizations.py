...
Covers the pure module (derouter.derouter_core_utils.fallback_generalizations): the
...
def test_routing_inference_first_match_wins(restore_generalizations):
    restore_generalizations(
        [
            {"name": "first", "pattern": r"^acme-", "model_info": {"derouter_provider": "openai"}},
            {"name": "second", "pattern": r"^acme-pro-", "model_info": {"derouter_provider": "anthropic"}},
        ]
    )
...
def test_routing_match_is_case_insensitive(restore_generalizations):
    restore_generalizations(
        [{"name": "r", "pattern": r"^claude-opus", "model_info": {"derouter_provider": "anthropic"}}]
    )
...
def test_routing_rules_are_excluded_from_capability_results(restore_generalizations):
    restore_generalizations(
        [
            {"name": "route", "pattern": r"^acme-", "model_info": {"derouter_provider": "openai"}},
...
def test_legacy_mixed_rule_acts_as_both_kinds(restore_generalizations):
    """A legacy rule mixing ``derouter_provider`` with capability keys routes AND
        ...
        [
            {
                "name": "legacy-mixed",
                "pattern": r"^acme-",
                "model_info": {"derouter_provider": "anthropic", "supports_vision": True},
            },
...
def test_legacy_main_schema_keeps_unmapped_claude_working(restore_generalizations):
    ...
    _, provider, _, _ = derouter.get_llm_provider(model="claude-opus-9-9")
    assert provider == "anthropic"

    info = derouter.get_model_info("claude-opus-9-9")
    assert info["derouter_provider"] == "anthropic"
    assert info["supports_adaptive_thinking"] is True
    assert info["supports_function_calling"] is True
    assert info["max_input_tokens"] == 200000
    assert not info.get("input_cost_per_token")

    low = derouter.get_model_info("claude-opus-4-0")
    assert low["derouter_provider"] == "anthropic"
...
def test_non_string_provider_rule_warns_and_is_skipped(restore_generalizations, warning_messages):
    restore_generalizations([{"name": "bad-provider", "pattern": r"^acme-", "model_info": {"derouter_provider": 42}}])
...
def test_unknown_model_routes_via_routing_rule(restore_generalizations):
    restore_generalizations([{"name": "myco", "pattern": r"^myco-", "model_info": {"derouter_provider": "openai"}}])
    _, provider, _, _ = derouter.get_llm_provider(model="myco-fast-1")
...
def test_capability_info_backfills_requested_provider(restore_generalizations):
    ...
    derouter.get_model_info.cache_clear()
    info = derouter.get_model_info("beeco-fast-1", custom_llm_provider="groq")
    assert info["derouter_provider"] == "groq"
    assert info["max_input_tokens"] == 12345
    assert info["supports_vision"] is True
    other = derouter.get_model_info("beeco-fast-1", custom_llm_provider="openai")
    assert other["derouter_provider"] == "openai"


def test_routing_only_match_does_not_resolve_model_info(restore_generalizations):
    restore_generalizations([{"name": "route", "pattern": r"^ceeco-", "model_info": {"derouter_provider": "openai"}}])
    derouter.get_model_info.cache_clear()
    with pytest.raises(Exception, match="This model isn't mapped yet"):
        derouter.get_model_info("ceeco-fast-1", custom_llm_provider="openai")
...
def test_exact_entry_takes_precedence_over_rule(restore_generalizations):
    ...
    derouter.get_model_info.cache_clear()
    info = derouter.get_model_info("gpt-4o")
    assert info["derouter_provider"] == "openai"
...
def shipped_cost_map(monkeypatch):
    ...
    original_cost = derouter.model_cost
...
def test_shipped_rules_resolve_unmapped_bedrock_claude_with_bedrock_provider(shipped_cost_map):
    ...
    assert model not in derouter.model_cost
    info = derouter.get_model_info(model, custom_llm_provider="bedrock")
    assert info["derouter_provider"] == "bedrock"
    assert info["supports_adaptive_thinking"] is True
    assert info["supports_function_calling"] is True
    assert info["max_input_tokens"] == 200000
...
def test_shipped_rules_stack_adaptive_and_mid_conversation_flags(shipped_cost_map):
    ...
    assert model not in derouter.model_cost
    info = derouter.get_model_info(model, custom_llm_provider="anthropic")
    assert info["derouter_provider"] == "anthropic"
    assert info["supports_adaptive_thinking"] is True
...
def test_shipped_rules_flag_unmapped_fable_as_always_on_thinking(shipped_cost_map):
    ...
    other = derouter.get_model_info("claude-opus-4-9", custom_llm_provider="anthropic")
...
def test_shipped_rules_are_provider_neutral_for_unmapped_ids(shipped_cost_map, model, provider):
    assert model not in derouter.model_cost
    info = derouter.get_model_info(model, custom_llm_provider=provider)
...
def test_shipped_version_gates_are_family_agnostic_at_4x(shipped_cost_map):
    ...
    high = derouter.get_model_info("claude-newfam-4-9", custom_llm_provider="anthropic")
    ...
    low = derouter.get_model_info("claude-newfam-4-5", custom_llm_provider="anthropic")
    assert low.get("supports_adaptive_thinking") is None
...
def test_shipped_rules_give_bare_majors_the_full_baseline_union(shipped_cost_map):
    ...
    assert model not in derouter.model_cost
    info = derouter.get_model_info(model)
    assert info["derouter_provider"] == "anthropic"
...
def test_shipped_routing_rule_covers_bare_majors(shipped_cost_map):
    _, provider, _, _ = derouter.get_llm_provider(model="claude-newt-5")
    assert provider == "anthropic"
...
def test_shipped_exact_entry_beats_rules(shipped_cost_map):
    ...
    info = derouter.get_model_info(model, custom_llm_provider="bedrock")
    assert info["derouter_provider"] == "bedrock_converse"
...
def test_shipped_rules_lose_to_exact_entries_across_cost_ladder_variants(shipped_cost_map):
    ...
    assert "claude-haiku-4-5-20251001" in derouter.model_cost
    with pytest.raises(Exception, match="This model isn't mapped yet"):
        derouter.get_model_info("claude-haiku-4-5-20251001", custom_llm_provider="bedrock")

    entry = derouter.model_cost["us.anthropic.claude-haiku-4-5-20251001-v1:0"]
...
def test_shipped_adaptive_rule_gates_on_version_not_pricing(shipped_cost_map):
    ...
    assert adaptive not in derouter.model_cost
    assert non_adaptive not in derouter.model_cost
