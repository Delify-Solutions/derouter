...
Delify-Solutions/derouter#31582.

That per-model gate only covers models whose cost-map entry carries the flag, so a
``strict: false`` that derouter itself synthesized still broke unflagged models. Since
``strict: false`` is the Chat Completions default, it is now dropped for every model
rather than forwarded as a no-op the provider can reject. See Delify-Solutions/derouter#33193.
"""
