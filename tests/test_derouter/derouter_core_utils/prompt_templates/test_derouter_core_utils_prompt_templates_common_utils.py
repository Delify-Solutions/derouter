...
def test_get_format_from_file_id():
    unified_file_id = "derouter_proxy:application/pdf;unified_id,cbbe3534-8bf8-4386-af00-f5f6b7e370bf"
...
def test_split_concatenated_json_wholly_invalid_returns_empty():
    ...
    Regression for https://github.com/Delify-Solutions/derouter/issues/18667: a raise
...
# Reusable fixture (decodes to: derouter_proxy:application/pdf;unified_id,...;
...
# --- x-derouter-model upload-path decoding (derouter #29830) -------------------


def _xderouter_encoded(raw_id: str, model: str) -> str:
...
def test_update_messages_with_model_file_ids_decodes_xderouter_encoded_id():
    """x-derouter-model upload returns `file-<b64(derouter:<raw>;model,<m>)>`.
    ...
    encoded_id = _xderouter_encoded(raw_id, "gpt-5.1")
    messages = [
        {
            "role": "user",
            "content": [
...
def test_update_responses_input_with_model_file_ids_decodes_xderouter_encoded_id():
...
def test_update_messages_xderouter_decode_does_not_override_mapping():
...
class TestRequestContainsImageContent:
    ...
    def test_detects_every_image_dialect_including_tool_results(self, part):
        from derouter.derouter_core_utils.prompt_templates.common_utils import request_contains_image_content
