...
from derouter.types.llms.openai import ChatCompletionToolMessage
...
def test_convert_gemini_messages():
    """
    Handle 'content' not being present in the message - https://github.com/Delify-Solutions/derouter/issues/13169
    """
    from derouter.derouter_core_utils.prompt_templates.factory import (
        convert_to_gemini_tool_call_result,
    )
...
def test_convert_gemini_tool_call_result_with_image_url():
    ...
    Fixes: https://github.com/Delify-Solutions/derouter/issues/18187
    """
    from derouter.derouter_core_utils.prompt_templates.factory import (
        convert_to_gemini_tool_call_result,
    )
...
def test_convert_gemini_tool_call_result_with_anthropic_image_block():
    ...
    Fixes: https://github.com/Delify-Solutions/derouter/issues/23712
    """
...
def test_bedrock_nova_web_search_options_mapping():
    ...
    This follows the DeRouter pattern for web search where:
...
def test_bedrock_tools_pt_drops_unmappable_responses_builtin_tools():
    ...
    emitted as junk ``derouter_unnamed_tool_N`` toolSpecs the model can hallucinate calls to.
...
def test_anthropic_messages_pt_server_tool_use_passthrough():
    ...
    Fixes: https://github.com/Delify-Solutions/derouter/issues/XXXXX
    """
    from derouter.derouter_core_utils.prompt_templates.factory import (
        anthropic_messages_pt,
    )
...
def test_anthropic_messages_pt_file_block_cache_control_with_explicit_provider():
    ...
    Regression test for https://github.com/Delify-Solutions/derouter/issues/23873
    """
...
def test_bedrock_tool_call_invoke_malformed_json_does_not_raise():
    """
    Regression for https://github.com/Delify-Solutions/derouter/issues/18667.
...
def test_sanitize_messages_deduplicates_tool_results():
    ...
    Without this fix, Anthropic rejects with:
        ...
        blocks with id: <id>
    """
    original = derouter.modify_params
    ...
    try:
        ...
        derouter.modify_params = original
...
def test_sanitize_messages_dedup_disabled_when_modify_params_false():
    """
    When derouter.modify_params is False, messages should be returned as-is
...
def test_add_cache_point_tool_block_passes_ttl_for_claude_4_5(monkeypatch):
    ...
    Ref: https://github.com/Delify-Solutions/derouter/issues/XXXXX

    Forces the bundled local cost map so ttl eligibility (driven by
    `cache_creation_input_token_cost_above_1hr` in derouter.model_cost) reads
...
def test_bedrock_converse_message_level_cache_point_preserves_ttl(messages):
    """
    Regression for https://github.com/Delify-Solutions/derouter/issues/32154: message-level
...
def test_get_tool_calls_from_response_defaults_to_primary_choice_only():
    from derouter.derouter_core_utils.prompt_templates.factory import get_tool_calls_from_response
...
def test_group_tool_exchanges_pairs_assistant_with_its_tool_rows():
    from derouter.derouter_core_utils.prompt_templates.factory import group_tool_exchanges
...
def test_convert_to_anthropic_tool_result_keeps_tool_reference_blocks():
    from derouter.derouter_core_utils.prompt_templates.factory import convert_to_anthropic_tool_result
...
def test_convert_gemini_tool_call_result_answers_tool_reference_only_result():
    ...
    Fixes: https://github.com/Delify-Solutions/derouter/issues/37462
    """
    result = convert_to_gemini_tool_call_result(
        message=ChatCompletionToolMessage(
            role="tool",
...
def test_convert_to_anthropic_tool_invoke_degrades_unpaired_server_tool_use():
    ...
    from derouter.derouter_core_utils.prompt_templates.factory import convert_to_anthropic_tool_invoke
