...
4. All these exceptions are in DEROUTER_EXCEPTION_TYPES
"""
...
def test_bad_gateway_error_preserves_retry_after_header():
    ...
    error = derouter.BadGatewayError(
...
def test_service_unavailable_error_preserves_retry_after_header():
    ...
    error = derouter.ServiceUnavailableError(
...
def test_internal_server_error_preserves_retry_after_header():
    ...
    error = derouter.InternalServerError(
...
def test_bad_gateway_error_in_exception_types():
    """Test that BadGatewayError is in DEROUTER_EXCEPTION_TYPES."""
    assert derouter.BadGatewayError in derouter.DEROUTER_EXCEPTION_TYPES
...
def test_bad_gateway_error_attributes():
    ...
    assert "DeRouter Retried: 1 times" in str(error)
    assert "DeRouter Max Retries: 3" in str(error)
