...
from derouter.utils import ModelResponseListIterator
...
async def test_streaming_completion_start_time(logging_obj: Logging):
    ...
    mock_callback = MockCallback()
    derouter.success_callback = [mock_callback, "langfuse"]
...
async def test_vertex_streaming_rate_limit_triggers_midstream_fallback(
    ...
    Regression test for https://github.com/Delify-Solutions/derouter/issues/20870
    """
...
async def test_bedrock_midstream_internal_server_error_wraps_for_fallback(
    ...
    """End-to-end regression for https://github.com/Delify-Solutions/derouter/issues/24608:
    ...
    from derouter.exceptions import MidStreamFallbackError
    from derouter.llms.bedrock.chat.invoke_handler import AWSEventStreamDecoder
...
async def test_bedrock_5xx_wraps_for_midstream_fallback(logging_obj: Logging):
    ...
    into MidStreamFallbackError so the Router can run streaming fallback."""
    from derouter.exceptions import MidStreamFallbackError
    from derouter.llms.bedrock.chat.invoke_handler import BedrockError
...
async def test_bedrock_validation_error_raises_directly(logging_obj: Logging):
    ...
    with pytest.raises(Exception, match='derouter\\.BadRequestError: BedrockException') as excinfo:
        await response.__anext__()
...
async def test_in_body_stream_error_400_raises_bad_request(logging_obj: Logging):
    """Regression for https://github.com/Delify-Solutions/derouter/issues/25492: a 400
...
def test_calculate_total_usage_with_cost():
    from derouter.derouter_core_utils.streaming_handler import calculate_total_usage
...
def test_calculate_total_usage_preserves_anthropic_cache_creation_ttl_breakdown():
    ...
    from derouter.derouter_core_utils.streaming_handler import calculate_total_usage
    from derouter.types.utils import CacheCreationTokenDetails
...
async def test_openrouter_streaming_usage_only_chunk_without_stream_options():
    ...
    mock_callback = MockCallback()
    previous_success_callback = derouter.success_callback
    previous_async_success_callback = derouter._async_success_callback
    derouter.success_callback = [mock_callback]
    derouter._async_success_callback = [mock_callback]
    ...
    try:
        ...
        derouter.success_callback = previous_success_callback
        derouter._async_success_callback = previous_async_success_callback
...
def test_openrouter_streaming_cost_propagates_to_hidden_params():
    ...
    _hidden_params["additional_headers"]["llm_provider-x-derouter-response-cost"]
    on the complete streaming response, so derouter's cost calculator uses it.
    """
    import derouter
    ...
    complete_response = derouter.stream_chunk_builder(
    ...
    assert (
        complete_response._hidden_params["additional_headers"][
            "llm_provider-x-derouter-response-cost"
        ]
        == 0.00025
    )

    # Verify the cost calculator would pick this up
    from derouter.cost_calculator import get_response_cost_from_hidden_params
...
def test_xai_streaming_reported_cost_still_takes_the_margin(monkeypatch):
    ...
    assert get_response_cost_from_hidden_params(complete_response._hidden_params) is None
    monkeypatch.setattr(derouter, "cost_margin_config", {"xai": 0.5})
    assert response_cost_calculator(
        response_object=complete_response,
...
def test_is_chunk_non_empty_with_empty_tool_calls(
    ...
    Regression test for https://github.com/Delify-Solutions/derouter/issues/17425
...
def test_is_chunk_non_empty_with_valid_tool_calls(
    ...
    Companion test for https://github.com/Delify-Solutions/derouter/issues/17425
...
_REPETITION_TEST_CASES = [
    # Basic cases
    pytest.param(
        ["same"] * derouter.REPEATED_STREAMING_CHUNK_LIMIT,
        True,
        id="all_identical_raises",
    ),
    pytest.param(
        ["same"] * (derouter.REPEATED_STREAMING_CHUNK_LIMIT - 1),
        False,
        id="below_threshold_no_raise",
    ),
    pytest.param(
        [None] * derouter.REPEATED_STREAMING_CHUNK_LIMIT,
        False,
        id="none_content_no_raise",
    ),
    pytest.param(
        [""] * derouter.REPEATED_STREAMING_CHUNK_LIMIT,
        False,
    ...
    pytest.param(
        ["ab"] * derouter.REPEATED_STREAMING_CHUNK_LIMIT,
        False,
        id="short_content_2chars_ab_no_raise",
    ),
    # All different chunks
    pytest.param(
        ["diff"] * derouter.REPEATED_STREAMING_CHUNK_LIMIT,
        False,
        id="all_different_no_raise",
    ),
    # One chunk different at various positions
    pytest.param(
        ["different_first"] + ["same"] * (derouter.REPEATED_STREAMING_CHUNK_LIMIT - 1),
        False,
        id="first_chunk_different_no_raise",
    ),
    pytest.param(
        ["same"] * (derouter.REPEATED_STREAMING_CHUNK_LIMIT - 1) + ["different_last"],
        False,
        id="last_chunk_different_no_raise",
    ),
    pytest.param(
        ["same"] * (derouter.REPEATED_STREAMING_CHUNK_LIMIT // 2 + 1)
        + ["different_mid"]
        + ["same"]
        * (
            derouter.REPEATED_STREAMING_CHUNK_LIMIT
            - derouter.REPEATED_STREAMING_CHUNK_LIMIT // 2
            + 1
        ),
        False,
    ...
    pytest.param(
        ["same"] * (derouter.REPEATED_STREAMING_CHUNK_LIMIT - 2) + ["diff", "diff"],
        False,
...
def test_raise_on_model_repetition(
    ...
    if should_raise:
        ...
        with pytest.raises(derouter.InternalServerError) as exc_info:
            _feed()
...
def test_raise_on_model_repetition_tolerates_empty_choices(
    ...
    """
    Regression test for https://github.com/Delify-Solutions/derouter/issues/28884
...
def test_dispatch_custom_provider_returns_chunk_early(
    ...
    monkeypatch.setattr(derouter, "_custom_providers", ["my-custom-llm"])
...
async def test_azure_streaming_role_preserved_with_include_usage(sync_mode: bool):
    """
    Regression test for https://github.com/Delify-Solutions/derouter/issues/24221
...
def test_gemini_legacy_vertex_stop_finish_reason_normalised():
    ...
    with patch("derouter.derouter_core_utils.streaming_handler.proto", create=True):
...
def test_chunk_creator_passes_through_model_response_stream(
    ...
    derouter._custom_providers.append("my-custom-provider")
    ...
    derouter._custom_providers.remove("my-custom-provider")
...
def test_chunk_creator_tool_calls_not_dropped_on_finish(
    ...
    """
    from derouter.types.utils import ChatCompletionDeltaToolCall, Function
...
def test_record_partial_usage_for_failure_stashes_usage_and_cost():
    ...
    logging_obj = Logging(
        ...
        call_type="completion",
        start_time=time.time(),
        derouter_call_id="partial-usage-1",
        function_id="1245",
    )
...
def test_record_partial_usage_for_failure_noop_without_chunks():
    ...
    logging_obj = Logging(
        ...
        call_type="completion",
        start_time=time.time(),
        derouter_call_id="partial-usage-2",
        function_id="1245",
    )
    wrapper = CustomStreamWrapper(
        completion_stream=None,
...
def _wrapper_with_partial_chunks(
    ...
    logging_obj = Logging(
        ...
        call_type="completion",
        start_time=time.time(),
        derouter_call_id="partial-usage-alias",
        function_id="1245",
    )
...
def test_record_partial_usage_for_failure_prices_alias_restamped_chunks_at_real_model():
    ...
    assert "bedrock/bedrock-claude-opus-5" not in derouter.model_cost

    wrapper._record_partial_usage_for_failure()

    stashed = logging_obj.model_call_details["combined_usage_object"]
    assert stashed.completion_tokens == 5
    rates = derouter.model_cost["us.anthropic.claude-opus-5"]
...
def test_record_partial_usage_for_failure_prices_corrected_model_not_chunk_model():
    ...
    rates = derouter.model_cost["gpt-4o-mini"]
    expected = 40 * rates["input_cost_per_token"] + 5 * rates["output_cost_per_token"]
    assert logging_obj.model_call_details["response_cost"] == pytest.approx(expected)
...
async def test_stream_chunk_builder_raise_and_usage_recovery_failure_does_not_crash(
    ...
    from derouter.derouter_core_utils import streaming_handler as sh_module
    ...
    response = CustomStreamWrapper(
        ...
        logging_obj=Logging(
            ...
            call_type="completion",
            start_time=time.time(),
            derouter_call_id="raise-recover-fail",
            function_id="1245",
        ),
        stream_options={"include_usage": True},
    )
    ...
    with (
        patch.object(
            derouter, "stream_chunk_builder", side_effect=Exception("assembly failed")
        ),
        patch.object(
...
def test_openai_custom_tool_call_stream_deltas_survive_conversion(logging_obj: Logging):
    ...
    from derouter.types.utils import ChatCompletionDeltaCustomToolCall
...
def test_sync_completion_never_stamps_correlation_context(monkeypatch):
    ...
    caller passes derouter_trace_id/derouter_session_id or the call streams.
    ...
    try:
        derouter.completion(
            ...
            derouter_trace_id="should-never-appear",
            derouter_session_id="should-never-appear-either",
            num_retries=0,
        )
        ...
        response = derouter.completion(
            ...
            mock_response="Hello there!",
            stream=True,
            derouter_trace_id="should-never-appear-stream",
            derouter_session_id="should-never-appear-stream-either",
            num_retries=0,
        )
        for _ in response:
            pass
    ...
    try:
        ...
        def call_a_abandon_stream():
            response = derouter.completion(
                ...
                stream=True,
                derouter_session_id="SESSION-AAA",
                derouter_trace_id="TRACE-AAA",
                num_retries=0,
            )
        ...
        def call_b_non_streaming():
            derouter.completion(
                ...
                derouter_session_id="SESSION-BBB",
                derouter_trace_id="TRACE-BBB",
                num_retries=0,
            )
...
async def test_async_streaming_completion_does_not_reset_context_before_iteration(monkeypatch):
    ...
    try:
        response = await derouter.acompletion(
            ...
            mock_response="Hello there!",
            stream=True,
            derouter_session_id="async-streaming-call-session",
            num_retries=0,
        )
...
def test_stream_wrapper_del_restores_correlation_context():
    ...
    try:
        log_obj = Logging(
            ...
            call_type="completion",
            start_time=None,
            derouter_call_id="abandoned-stream-call",
            function_id="fn-abandoned-stream",
            kwargs={"derouter_session_id": "abandoned-stream-session"},
        )
        wrapper = CustomStreamWrapper(
...
def test_stream_wrapper_del_does_not_clobber_a_newer_active_call():
    ...
    try:
        ...
        Logging(
            ...
            call_type="completion",
            start_time=None,
            derouter_call_id="newer-active-call",
            function_id="fn-newer-active-call",
            kwargs={"derouter_session_id": "newer-active-session"},
        )
        assert trace_id_var.get() != abandoned_log_obj.derouter_trace_id
...
def test_stream_wrapper_del_restores_when_own_session_id_needed_sanitizing():
    ...
    stored in the contextvar, not the raw derouter_session_id/derouter_trace_id
    ...
    try:
        ...
        log_obj = Logging(
            ...
            call_type="completion",
            start_time=None,
            derouter_call_id="abandoned-stream-needs-sanitizing",
            function_id="fn-abandoned-stream-needs-sanitizing",
            kwargs={"derouter_session_id": raw_session_id},
        )
        # Sanity: the contextvar holds the sanitized value, which differs
        # from the raw derouter_session_id this test constructed it with.
        assert session_id_var.get() != raw_session_id
        assert log_obj.derouter_session_id == raw_session_id
...
def test_stream_wrapper_next_keeps_context_active_through_synthesized_finish_reason_chunk():
    ...
    try:
        log_obj = Logging(
            ...
            call_type="completion",
            start_time=None,
            derouter_call_id="finish-reason-call",
            function_id="fn-finish-reason",
            kwargs={"derouter_session_id": "finish-reason-session"},
        )
        wrapper = CustomStreamWrapper(
            completion_stream=iter([]),
            model="gpt-3.5-turbo",
...
def test_stream_wrapper_del_cleans_up_after_synthesized_finish_reason_chunk():
    ...
    try:
        log_obj = Logging(
            ...
            call_type="completion",
            start_time=None,
            derouter_call_id="finish-reason-del-call",
            function_id="fn-finish-reason-del",
            kwargs={"derouter_session_id": "finish-reason-del-session"},
        )
        wrapper = CustomStreamWrapper(
            completion_stream=iter([]),
            model="gpt-3.5-turbo",
...
async def test_stream_wrapper_anext_keeps_context_active_through_synthesized_finish_reason_chunk():
    ...
    try:
        log_obj = Logging(
            ...
            call_type="completion",
            start_time=None,
            derouter_call_id="anext-finish-reason-call",
            function_id="fn-anext-finish-reason",
            kwargs={"derouter_session_id": "anext-finish-reason-session"},
        )
...
async def test_stream_wrapper_anext_max_duration_timeout_restores_consumer_correlation_context(monkeypatch):
    """_check_max_streaming_duration() raises derouter.Timeout when a client keeps
    an async stream open past DEROUTER_MAX_STREAMING_DURATION_SECONDS. That raise
    ...
    monkeypatch.setattr(derouter.constants, "DEROUTER_MAX_STREAMING_DURATION_SECONDS", 1)
    ...
    try:
        log_obj = Logging(
            ...
            call_type="completion",
            start_time=None,
            derouter_call_id="max-duration-call",
            function_id="fn-max-duration",
            kwargs={"derouter_session_id": "max-duration-session"},
        )
        ...
        with pytest.raises(derouter.Timeout):
            await wrapper.__anext__()
...
async def test_stream_wrapper_aclose_restores_consumer_correlation_context():
    ...
    try:
        log_obj = Logging(
            ...
            call_type="completion",
            start_time=None,
            derouter_call_id="aclose-call",
            function_id="fn-aclose",
            kwargs={"derouter_session_id": "aclose-session"},
        )
...
async def test_stream_wrapper_aclose_keeps_context_active_through_close_failure_diagnostic(monkeypatch):
    ...
    try:
        log_obj = Logging(
            ...
            call_type="completion",
            start_time=None,
            derouter_call_id="close-fail-call",
            function_id="fn-close-fail",
            kwargs={"derouter_session_id": "close-fail-session"},
        )
        ...
        assert captured_ids["trace_id"] == log_obj.derouter_trace_id
...
def test_handle_stream_fallback_error_restores_context_only_after_exception_mapping(monkeypatch):
    ...
    try:
        log_obj = Logging(
            ...
            call_type="completion",
            start_time=None,
            derouter_call_id="fallback-error-call",
            function_id="fn-fallback-error",
            kwargs={"derouter_session_id": "fallback-error-session"},
        )
        wrapper = CustomStreamWrapper(
            completion_stream=iter([]),
            model="claude-3-opus",
        ...
        monkeypatch.setattr("derouter.derouter_core_utils.streaming_handler.exception_type", fake_exception_type)
...
def test_chunk_creator_preserves_hidden_provider_specific_fields_from_parsed_chunk():
    ...
    assembled = derouter.stream_chunk_builder(chunks=[result])
...
def test_chunk_creator_keeps_provider_model_private_across_stream():
    ...
    assembled = derouter.stream_chunk_builder(chunks=[first_result, terminal_result])
    assert assembled is not None
...
def test_assembled_stream_uses_later_provider_model_for_cost(
    ...
    assembled = derouter.stream_chunk_builder(
    ...
    assert derouter.completion_cost(
...
async def test_async_stream_assembled_response_keeps_vertex_traffic_type(logging_obj: Logging):
    ...
    assembled = derouter.stream_chunk_builder(chunks=received, messages=[{"role": "user", "content": "hi"}])
    assert assembled is not None
    assert assembled._hidden_params["provider_specific_fields"]["traffic_type"] == "ON_DEMAND_FLEX"
...
async def test_async_fake_stream_final_chunk_carries_hidden_usage(logging_obj: Logging):
    from derouter.llms.base_llm.base_model_iterator import MockResponseIterator
