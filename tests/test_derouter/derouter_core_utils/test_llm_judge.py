...
def _router(alias: tuple[str, ...] = (), deployments: bool = False) -> derouter.Router:
    ...
    router = derouter.Router(
        model_list=[
            {"model_name": name, "derouter_params": {"model": "openai/gpt-4o", "api_key": "fake"}}
...
async def test_judge_acompletion_falls_back_to_sdk_for_unconfigured_model(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(derouter_module, "acompletion", sdk)
    router = _router()
...
def test_judge_target_gives_one_identity_to_a_bare_public_name_and_a_prefixed_deployment() -> None:
    ...
    router = derouter.Router(
        model_list=[{"model_name": "fast-tier", "derouter_params": {"model": "openai/gpt-4o", "api_key": "fake"}}]
    )
