...
from derouter.types.utils import SHADOW_EVAL_ROUTER_CALL_ORIGIN
...
class TestSubCallMetadataSanitization:
    ...
    def test_classifier_buckets_keep_non_spend_fields_on_a_chat_completions_parent(self):
        ...
        get_derouter_metadata_from_kwargs prefers derouter_metadata whenever truthy, so an
        origin-only dict would make an empty derouter_metadata win and silently drop
        ...
        resolved = get_derouter_metadata_from_kwargs(
            {
                "derouter_params": {
                    "metadata": forwarded_internal_call_metadata(parent, "autorouter_classifier"),
                    "derouter_metadata": forwarded_internal_call_metadata(None, "autorouter_classifier"),
                }
            }
        )
