...
from derouter.types.llms.openai import OpenAIRealtimeStreamList
...
from derouter.utils import TranscriptionResponse
...
def test_cost_calculator_with_response_cost_in_additional_headers():
    class MockResponse(BaseModel):
        _hidden_params = {
            "additional_headers": {"llm_provider-x-derouter-response-cost": 1000}
        }
...
def test_vertex_lyria_speech_cost(
    ...
    model_info: Final = derouter.model_cost[model]
    if runtime_state == "missing":
        monkeypatch.delitem(derouter.model_cost, model)
...
def test_baseten_model_api_pricing_entries(_local_model_cost_map):
    ...
    for model_name, (input_cost, output_cost) in expected_pricing.items():
        model_info = derouter.model_cost.get(model_name)
        assert model_info is not None, f"Missing model pricing entry: {model_name}"
        assert model_info["derouter_provider"] == "baseten"
...
def test_wandb_model_api_pricing_entries(_local_model_cost_map):
    ...
    for model_name, (input_cost, output_cost) in expected_pricing.items():
        model_info = derouter.model_cost.get(model_name)
        assert model_info is not None, f"Missing model pricing entry: {model_name}"
        assert model_info["derouter_provider"] == "wandb"
        assert model_info["input_cost_per_token"] == input_cost
        assert model_info["output_cost_per_token"] == output_cost
...
def test_openrouter_qwen36_plus_model_info(_local_model_cost_map):
    ...
    model_info = derouter.model_cost.get("openrouter/qwen/qwen3.6-plus")
...
def test_github_copilot_mai_code_1_flash_pricing(_local_model_cost_map, model):
    ...
    model_info = derouter.model_cost.get(model)

    assert model_info is not None, f"Missing model pricing entry: {model}"
    assert model_info["derouter_provider"] == "github_copilot"
    assert model_info["mode"] == "chat"
...
def test_cost_calculator_with_usage(_local_model_cost_map, monkeypatch):
    ...
    model_info = derouter.model_cost["gemini-2.0-flash-001"]
    ...
    # Invalidate caches after modifying derouter.model_cost
...
def test_transcription_cost_uses_token_pricing(_local_model_cost_map):
    from derouter import completion_cost
...
def test_handle_realtime_stream_cost_calculation():
    from derouter.cost_calculator import RealtimeAPITokenUsageProcessor
    ...
    cost = handle_realtime_stream_cost_calculation(
        ...
        custom_llm_provider="openai",
        derouter_model_name="gpt-3.5-turbo",
    )
...
def test_handle_realtime_stream_cost_calculation_stores_cost_breakdown():
    ...
    logging_obj = Logging(
        ...
        derouter_call_id="realtime-cost-breakdown-test",
    ...
    total_cost = handle_realtime_stream_cost_calculation(
        ...
        combined_usage_object=combined_usage_object,
        custom_llm_provider="openai",
        derouter_model_name="gpt-4o-realtime-preview",
...
def test_realtime_transcription_duration_cost(monkeypatch):
    ...
    logging_obj = Logging(
        ...
        call_type="_arealtime",
        start_time=datetime.now(),
        derouter_call_id="realtime-transcription-cost-breakdown-test",
    ...
    cost = handle_realtime_stream_cost_calculation(
        ...
        custom_llm_provider="openai",
        derouter_model_name="gpt-realtime-whisper",
        derouter_logging_obj=logging_obj,
    )
...
def test_realtime_transcription_duration_cost_resolves_model_from_derouter_name(
    monkeypatch,
):
    """When no session event carries the ASR model, the derouter_model_name is used."""
    monkeypatch.setenv("DEROUTER_LOCAL_MODEL_COST_MAP", "True")
    monkeypatch.setattr(derouter, "model_cost", derouter.get_model_cost_map(url=""))
    ...
    cost = handle_realtime_stream_cost_calculation(
        ...
        derouter_model_name="azure/gpt-realtime-whisper",
    )
...
def test_realtime_transcription_no_completed_events_is_zero(monkeypatch):
    ...
    from derouter.cost_calculator import handle_realtime_transcription_cost_calculation
...
def test_realtime_transcription_token_billed_fallback(monkeypatch):
    ...
    from derouter.cost_calculator import _transcription_usage_cost
...
def test_get_transcription_model_falls_back_to_session_model(monkeypatch):
    ...
    from derouter.cost_calculator import _get_transcription_model_name_from_results
...
def test_custom_pricing_cost_calc_uses_router_model_id_from_derouter_metadata():
    """When custom pricing is in derouter_metadata.model_info,
    ...
    """
    from derouter.cost_calculator import _select_model_name_for_cost_calc
    ...
    derouter.register_model(model_cost={custom_model_id: custom_pricing_info})
    ...
    custom_pricing = use_custom_pricing_for_model(derouter_params)
...
def test_per_request_custom_pricing_with_router():
    ...
    assert router_model_id in derouter.model_cost
    entry = derouter.model_cost[router_model_id]
...
def test_tiered_pricing_only_deployment_selects_router_model_id():
    ...
    assert (
        derouter.model_cost["dashscope/qwen-tier-only-test"].get("tiered_pricing") is None
    )
...
def test_azure_realtime_cost_calculator(_local_model_cost_map):
    ...
    cost = handle_realtime_stream_cost_calculation(
        ...
        custom_llm_provider="azure",
        derouter_model_name="my-custom-azure-deployment",
    )
...
def test_azure_audio_output_cost_calculation(_local_model_cost_map):
    ...
    Reproduces issue: https://github.com/Delify-Solutions/derouter/issues/19764
    Audio tokens should be charged at output_cost_per_audio_token rate,
    not at the text token rate (output_cost_per_token).
    """
    from derouter.types.utils import Choices, CompletionTokensDetailsWrapper, Message
    ...
    model_info = derouter.get_model_info("azure/gpt-audio-2025-08-28")
...
def test_default_image_cost_calculator(monkeypatch):
    from derouter.cost_calculator import default_image_cost_calculator
...
def test_cost_calculator_with_cache_creation():
    from derouter import completion_cost
    from derouter.types.utils import Choices, Message, Usage

    derouter_model_response = ModelResponse(
    ...
    assert derouter_model_response.usage.prompt_tokens_details.cached_tokens == 28491
...
def test_log_context_cost_calculation():
    ...
    from derouter import get_model_info
...
def test_azure_ai_cache_cost_calculation(_local_model_cost_map):
    ...
    model_info = derouter.model_cost[test_model_id]
    expected_input_cost = (
...
def test_azure_gpt_5_6_rates_match_azure_price_page(_local_model_cost_map, model):
    ...
    """
    entry = derouter.model_cost[model]
    ...
    zone = model.split("/")[1]
    if zone in ("us", "eu"):
        global_entry = derouter.model_cost["azure/" + model.split("/", 2)[2]]
...
def test_vertex_regional_deployment_costs_uplift_over_global(monkeypatch):
    """
    Regression for https://github.com/Delify-Solutions/derouter/issues/34393: two Vertex
...
def test_cost_discount_vertex_ai(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "cost_discount_config", {})
    ...
    monkeypatch.setattr(derouter, "cost_discount_config", {"vertex_ai": 0.05})
...
def test_cost_margin_percentage(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "cost_margin_config", {})
    ...
    monkeypatch.setattr(derouter, "cost_margin_config", {"openai": 0.10})
...
def test_cost_margin_fixed_amount(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "cost_margin_config", {"openai": {"fixed_amount": 0.001}})
...
def test_cost_margin_global(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "cost_margin_config", {"global": 0.05})
...
def test_cost_margin_provider_overrides_global(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "cost_margin_config", {"global": 0.05, "openai": 0.10})
...
def test_cost_margin_with_discount(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "cost_discount_config", {"openai": 0.05})
    monkeypatch.setattr(derouter, "cost_margin_config", {"openai": 0.10})
...
def test_gemini_implicit_caching_cost_calculation():
    ...
    See: https://github.com/Delify-Solutions/derouter/issues/16341
    """
    from derouter import completion_cost
    from derouter.llms.vertex_ai.gemini.vertex_and_google_ai_studio_gemini import (
        VertexGeminiConfig,
    )
    from derouter.types.utils import Choices, Message, ModelResponse
    ...
    model_info = derouter.get_model_info("gemini/gemini-2.0-flash")
...
def test_additional_costs_only_for_azure_ai(_local_model_cost_map):
    ...
    """
    from derouter.cost_calculator import _get_additional_costs
...
def test_openrouter_gemini_3_1_flash_lite_preview_pricing(_local_model_cost_map):
    ...
    Regression test for https://github.com/Delify-Solutions/derouter/issues/25604
...
def test_custom_pricing_applies_cache_creation_input_cost_via_cache_write_tokens_alias():
    ...
    from derouter.cost_calculator import cost_per_token
...
def test_extract_cache_read_tokens_anthropic_top_level():
    from derouter.proxy.spend_tracking.savings import extract_cache_read_tokens as _extract_cache_read_tokens
...
def test_extract_cache_creation_tokens_anthropic_top_level():
    from derouter.proxy.spend_tracking.savings import extract_cache_creation_tokens as _extract_cache_creation_tokens
...
def test_completion_cost_logs_reasoning_and_cache_breakdown(_local_model_cost_map):
    ...
    logging_obj = Logging(
        ...
        start_time=datetime.now(),
        derouter_call_id="reasoning-cache-breakdown",
        function_id="f",
    )
...
def test_batch_cost_calculator_prices_cache_creation_tokens_at_cache_write_rate():
    ...
    input tokens.
    """
    from derouter.cost_calculator import batch_cost_calculator
...
def test_completion_cost_bills_interactions_api_response():
    ...
    model_info = derouter.get_model_info(model="gemini-2.5-flash", custom_llm_provider="gemini")
...
def test_completion_cost_bills_interactions_google_search_per_query():
    ...
    model_info = derouter.get_model_info(model="gemini-3-flash-preview", custom_llm_provider="gemini")
    response = InteractionsAPIResponse(
...
def test_completion_cost_bills_interactions_video_output_at_video_rate():
    ...
    model_info = derouter.get_model_info(model="gemini-omni-flash-preview", custom_llm_provider="gemini")
    video_tokens = 5792 * 8
    response = InteractionsAPIResponse(
...
def test_completion_cost_together_metadata_only_model_still_uses_size_bucket(_local_model_cost_map):
    assert "input_cost_per_token" not in derouter.model_cost["together_ai/togethercomputer/CodeLlama-34b-Instruct"]
...
def _bedrock_response_with_private_model(model: str, region_name: str) -> derouter.ModelResponse:
    response = derouter.ModelResponse(
        id="x",
        choices=[
            {
...
def test_completion_cost_nonzero_for_slash_alias_model_name(_local_model_cost_map):
    ...
    response.usage = derouter.Usage(prompt_tokens=100, completion_tokens=50)
...
def test_every_one_hour_cache_write_rate_is_double_its_input_rate():
    ...
    DeRouter tracks (Anthropic, Bedrock, Vertex, Azure) publishes the 1h write at 2x input."""
...
def test_gemini_live_native_audio_ga_realtime_cost(_local_model_cost_map: None) -> None:
    """Regression for https://github.com/Delify-Solutions/derouter/issues/31087."""
    from derouter.types.utils import CompletionTokensDetailsWrapper
    ...
    cost = handle_realtime_stream_cost_calculation(
        ...
        combined_usage_object=combined_usage_object,
        custom_llm_provider="vertex_ai",
        derouter_model_name="vertex_ai/gemini-live-2.5-flash-native-audio",
    )
...
@pytest.mark.parametrize(
    "priceless_entry",
    [
        {"derouter_provider": "vertex_ai", "mode": "realtime"},
        {
            "derouter_provider": "vertex_ai",
            "mode": "realtime",
...
def test_realtime_priceless_deployment_entry_falls_through_to_priced_model(
    ...
    """Regression for https://github.com/Delify-Solutions/derouter/issues/31087 (router-registered priceless entries)."""
    monkeypatch.setitem(
        derouter.model_cost,
    ...
    priced_entry = derouter.model_cost["gemini-live-2.5-flash-preview-native-audio-09-2025"]
    ...
    cost = handle_realtime_stream_cost_calculation(
        ...
        combined_usage_object=combined_usage_object,
        custom_llm_provider="vertex_ai",
        derouter_model_name=priced_model,
    )
...
def test_realtime_explicitly_free_session_model_still_bills_zero(
    ...
    cost = handle_realtime_stream_cost_calculation(
        ...
        combined_usage_object=combined_usage_object,
        custom_llm_provider="vertex_ai",
        derouter_model_name="vertex_ai/gemini-live-2.5-flash-preview-native-audio-09-2025",
    )
...
def test_handle_realtime_stream_cost_calculation_bills_nested_reasoning_tokens_once(
    ...
    total_cost = handle_realtime_stream_cost_calculation(
        ...
        combined_usage_object=combined_usage_object,
        custom_llm_provider="azure",
        derouter_model_name="azure/gpt-realtime-2.1-mini",
    )

    info = derouter.get_model_info(model="azure/gpt-realtime-2.1-mini", custom_llm_provider="azure")
    expected = (
