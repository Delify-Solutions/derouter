"""
Test for DEROUTER_DISABLE_LAZY_LOADING environment variable.

This test verifies that when DEROUTER_DISABLE_LAZY_LOADING is set,
...
Tests that need to clear sys.modules and re-import derouter run in subprocesses
...
def _run_python(
    ...
    env.pop("DEROUTER_DISABLE_LAZY_LOADING", None)
    ...
    return subprocess.run(
        ...
        text=True,
        env=env,
        # Importing derouter can cold-load tiktoken/tokenizer assets and is
...
def test_eager_loading_enabled():
    ...
    result = _run_python(
        """
        import derouter
        assert hasattr(derouter, "encoding"), "Encoding should be available when eager loading is enabled"
        ...
        """,
        env_override={"DEROUTER_DISABLE_LAZY_LOADING": "1"},
    )
    assert (
        result.returncode == 0
...
def test_eager_loading_env_var_values():
    ...
    cold ``import derouter`` process per value (~78 s each on CI).  The
    subprocess re-imports derouter in isolated ``importlib`` reloads so each
    ...
    result = _run_python(
        ...
        for value in values:
            ...
            os.environ["DEROUTER_DISABLE_LAZY_LOADING"] = value
            # Remove cached derouter modules so re-import picks up the new env
            mods_to_remove = [k for k in sys.modules if k == "derouter" or k.startswith("derouter.")]
            for m in mods_to_remove:
                del sys.modules[m]
            import derouter
            assert hasattr(derouter, "encoding"), f"Encoding missing for {value!r}"
            tokens = derouter.encoding.encode("test")
