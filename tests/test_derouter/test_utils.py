...
from derouter.proxy.utils import is_valid_api_key
...
from derouter.types.utils import all_derouter_params, bedrock_batch_derouter_params
from derouter.types.router import CredentialDeRouterParams, GenericDeRouterParams
...
def test_get_utc_datetime_returns_current_aware_utc_time() -> None:
    ...
    result: Final = derouter.utils.get_utc_datetime()
...
def test_get_model_info_surfaces_supports_adaptive_thinking(local_model_cost_map):
    ...
    or unmapped name."""
    explicit = derouter.get_model_info(model="claude-opus-4-8")
    assert explicit["supports_adaptive_thinking"] is True

    generalized = derouter.get_model_info(
...
def test_get_model_info_surfaces_supports_parallel_function_calling(local_model_cost_map):
    ...
    and derouter.supports_parallel_function_calling. Regression: the key was never copied into
    ...
    declared_true = derouter.get_model_info(model="together_ai/zai-org/GLM-5.3-Flash")
    assert declared_true["supports_parallel_function_calling"] is True
    assert derouter.supports_parallel_function_calling(model="together_ai/zai-org/GLM-5.3-Flash") is True

    declared_false = derouter.get_model_info(model="o3-mini")
    assert declared_false["supports_parallel_function_calling"] is False
    assert derouter.supports_parallel_function_calling(model="o3-mini") is False
...
def test_get_model_info_surfaces_supported_endpoints(local_model_cost_map):
    ...
    (LIT-6240)."""
    info = derouter.get_model_info(model="gpt-realtime-whisper", custom_llm_provider="azure")
...
def test_potential_model_names_keeps_provider_prefixed_candidate():
    """A provider whose own model ids repeat the derouter provider name (Perplexity's
    ...
    the leading `perplexity/` as the derouter prefix and strips it away."""
...
def test_get_model_info_resolves_provider_prefixed_model_ids(local_model_cost_map):
    ...
    for model, reasoning in (
        ...
        assert derouter.supports_reasoning(model=model) is reasoning, model

    via_provider = derouter.get_model_info(
        model="perplexity/glm-5.2", custom_llm_provider="perplexity"
    )
...
def test_get_model_info_strips_openai_finetune_ids_without_a_custom_suffix(local_model_cost_map):
    info = derouter.get_model_info(model="ft:gpt-4o-2024-08-06:my-org::abc123", custom_llm_provider="openai")
...
def test_provider_prefixed_lookup_never_outranks_an_existing_row(local_model_cost_map):
    ...
    sonar = derouter.get_model_info(model="sonar", custom_llm_provider="perplexity")
    assert sonar["key"] == "perplexity/sonar"
    assert sonar["mode"] == "chat"
    assert sonar["input_cost_per_token"] == 1e-06

    still_sonar = derouter.get_model_info(
    ...
    for model, provider, expected_key in (
        ...
        assert derouter.get_model_info(model=model, custom_llm_provider=provider)["key"] == expected_key
...
def test_check_provider_match_azure_ai_allows_openai_and_azure():
    ...
    assert (
        _check_provider_match(
            model_info={"derouter_provider": "openai"}, custom_llm_provider="azure_ai"
        )
        is True
    )
    ...
    assert (
        _check_provider_match(
            model_info={"derouter_provider": "azure"}, custom_llm_provider="azure_ai"
        )
        is True
    )
    ...
    assert (
        _check_provider_match(
            model_info={"derouter_provider": "anthropic"}, custom_llm_provider="azure_ai"
        )
        is False
    )
...
def test_check_provider_match_github_allows_upstream_provider_metadata():
    ...
    assert (
        _check_provider_match(
            model_info={"derouter_provider": "openai"},
    ...
    assert (
        _check_provider_match(
            model_info={"derouter_provider": "github"},
            custom_llm_provider="github",
        )
        is True
    )
    ...
    assert (
        _check_provider_match(
            model_info={"derouter_provider": "anthropic"},
            custom_llm_provider="github",
        )
        is True
    )
...
def test_supports_function_calling_github_openai_alias():
    assert derouter.utils.supports_function_calling(model="github/gpt-4o-mini") is True
    assert (
        derouter.utils.supports_function_calling(
...
def test_supports_function_calling_deepinfra_llama():
    ...
    Regression test for https://github.com/Delify-Solutions/derouter/issues/22619
    """
    assert (
        derouter.utils.supports_function_calling(
...
def test_get_optional_params_image_gen():
    from derouter.llms.azure.image_generation import AzureGPTImageGenerationConfig
...
def test_get_optional_params_image_gen_keeps_gpt_image_supported_params(custom_llm_provider):
    """https://github.com/Delify-Solutions/derouter/issues/38649"""
...
def test_gpt_image_provider_detection_covers_existing_family():
    for image_model in ("gpt-image-1", "gpt-image-1-mini", "gpt-image-1.5"):
        model, custom_llm_provider, _, _ = derouter.get_llm_provider(model=image_model)
...
def test_gpt_image_2_provider_and_model_info(local_model_cost_map):
    ...
    model, custom_llm_provider, _, _ = derouter.get_llm_provider(model="gpt-image-2")

    assert model == "gpt-image-2"
    assert custom_llm_provider == "openai"

    model_info = derouter.get_model_info(model="gpt-image-2")
    assert model_info["derouter_provider"] == "openai"
    ...
    assert (
        ...
        in derouter.model_cost["gpt-image-2"]["supported_endpoints"]
    )
    assert (
        "/v1/images/edits" in derouter.model_cost["gpt-image-2"]["supported_endpoints"]
    )
...
def test_gpt_image_2_snapshot_model_info(local_model_cost_map):
    ...
    model_info = derouter.get_model_info(model="gpt-image-2-2026-04-21")
    assert model_info["derouter_provider"] == "openai"
    assert model_info["mode"] == "image_generation"
    assert model_info["output_cost_per_image_token"] == 3e-05
...
def test_azure_gpt_image_2_model_info(local_model_cost_map):
    ...
    assert model_info["derouter_provider"] == "azure"
    assert model_info["mode"] == "image_generation"
    assert model_info["input_cost_per_token"] == 5e-06
    assert model_info["input_cost_per_image_token"] == 8e-06
    assert model_info["output_cost_per_token"] == 0
...
def test_all_model_configs():
    from derouter.llms.vertex_ai.vertex_ai_partner_models.ai21.transformation import (
        VertexAIAi21Config,
    )
    from derouter.llms.vertex_ai.vertex_ai_partner_models.llama3.transformation import (
    ...
    from derouter.llms.fireworks_ai.chat.transformation import FireworksAIConfig
    ...
    from derouter.llms.nvidia_nim.chat.transformation import NvidiaNimConfig
    ...
    from derouter.llms.ollama.chat.transformation import OllamaChatConfig
    ...
    from derouter.llms.predibase.chat.transformation import PredibaseConfig
    ...
    from derouter.llms.codestral.completion.transformation import (
    ...
    from derouter.llms.volcengine.chat.transformation import (
    ...
    from derouter.llms.ai21.chat.transformation import AI21ChatConfig
    ...
    from derouter.llms.azure.chat.gpt_transformation import AzureOpenAIConfig
    ...
    from derouter.llms.bedrock.chat.converse_transformation import AmazonConverseConfig
    ...
    from derouter import AmazonAnthropicClaudeConfig, AmazonAnthropicConfig
    ...
    from derouter.llms.databricks.chat.transformation import DatabricksConfig
    ...
    from derouter.llms.vertex_ai.vertex_ai_partner_models.anthropic.transformation import (
    ...
    from derouter.llms.gemini.chat.transformation import GoogleAIStudioGeminiConfig
...
def test_anthropic_web_search_in_model_info(monkeypatch):
    ...
    for model in supported_models:
        from derouter.utils import get_model_info
...
def test_cohere_embedding_optional_params():
    from derouter import get_optional_params_embeddings
...
def test_get_model_info_bedrock_regional_inference_profile_pricing(local_model_cost_map):
    ...
    regional = derouter.model_cost["au.anthropic.claude-opus-4-8"]
    base = derouter.model_cost["anthropic.claude-opus-4-8"]
    ...
    for model in (
        ...
        info = derouter.get_model_info(model=model)
    ...
    control = derouter.get_model_info(model="au.anthropic.claude-opus-4-8", custom_llm_provider="bedrock")
...
def test_get_model_info_bedrock_regional_profile_without_entry_falls_back_to_base(local_model_cost_map):
    ...
    assert "apac.anthropic.claude-opus-4-8" not in derouter.model_cost
    info = derouter.get_model_info(model="bedrock/apac.anthropic.claude-opus-4-8")
...
def test_get_model_info_bedrock_double_provider_prefix_resolves(local_model_cost_map):
    ...
    info = derouter.get_model_info(model="bedrock/bedrock/us.anthropic.claude-sonnet-4-6")
...
def test_openai_models_in_model_info(monkeypatch):
    ...
    for model, info in model_map.items():
        if (
            info.get("derouter_provider") == "openai"
...
def test_supports_tool_choice_simple_tests():
    ...
    """
    assert derouter.utils.supports_tool_choice(model="gpt-4o") == True
    assert (
        derouter.utils.supports_tool_choice(
    ...
    assert (
        derouter.utils.supports_tool_choice(model="us.amazon.nova-micro-v1:0") is False
    )
    assert (
        derouter.utils.supports_tool_choice(model="bedrock/us.amazon.nova-micro-v1:0")
        is False
    )
    assert (
        derouter.utils.supports_tool_choice(
    ...
    assert derouter.utils.supports_tool_choice(model="perplexity/sonar") is False
...
def test_check_provider_match():
    ...
    model_info = {"derouter_provider": "bedrock"}
    assert derouter.utils._check_provider_match(model_info, "bedrock") is True
    assert derouter.utils._check_provider_match(model_info, "bedrock_converse") is True

    # Test bedrock_converse provider
    model_info = {"derouter_provider": "bedrock_converse"}
    assert derouter.utils._check_provider_match(model_info, "bedrock") is True
    assert derouter.utils._check_provider_match(model_info, "bedrock_converse") is True

    # Test non-matching provider
    model_info = {"derouter_provider": "bedrock"}
    assert derouter.utils._check_provider_match(model_info, "openai") is False


def test_check_provider_match_none_value_matches_any_provider():
    """
    A ``derouter_provider`` of None must be treated the same as a missing
    ...
    Before the fix, ``register_model`` persisted ``derouter_provider: None``
    ...
    assert derouter.utils._check_provider_match({}, "openai") is True
    assert (
        derouter.utils._check_provider_match({"derouter_provider": None}, "openai")
        is True
    )
    assert (
        derouter.utils._check_provider_match({"derouter_provider": None}, "anthropic")
        is True
    )
    # When custom_llm_provider is also None nothing constrains the match.
    assert derouter.utils._check_provider_match({"derouter_provider": None}, None) is True
...
def test_get_provider_rerank_config():
    ...
    """
    from derouter import HostedVLLMRerankConfig
    from derouter.utils import LlmProviders
...
def test_get_provider_text_to_speech_config_vertex_gemini_skips_cloud_tts():
    ...
    from derouter.llms.vertex_ai.text_to_speech.transformation import VertexAITextToSpeechConfig
    from derouter.utils import LlmProviders
...
def test_supports_computer_use_utility(monkeypatch):
    """
    Tests the derouter.utils.supports_computer_use utility function.
    """
    from derouter.utils import supports_computer_use

    # Ensure DEROUTER_LOCAL_MODEL_COST_MAP is set for consistent test behavior,
    # as supports_computer_use relies on get_model_info.
    # This also requires derouter.model_cost to be populated.
    original_env_var = os.getenv("DEROUTER_LOCAL_MODEL_COST_MAP")
    original_model_cost = getattr(derouter, "model_cost", None)

    monkeypatch.setenv("DEROUTER_LOCAL_MODEL_COST_MAP", "True")
    derouter.model_cost = derouter.get_model_cost_map(url="")  # Load with local/backup
    ...
    try:
        ...
        if original_env_var is None:
            del os.environ["DEROUTER_LOCAL_MODEL_COST_MAP"]
        else:
            monkeypatch.setenv("DEROUTER_LOCAL_MODEL_COST_MAP", original_env_var)

        if original_model_cost is not None:
            derouter.model_cost = original_model_cost
        elif hasattr(derouter, "model_cost"):
            delattr(derouter, "model_cost")
...
def test_get_model_info_shows_supports_computer_use(monkeypatch):
    ...
    """
    monkeypatch.setenv("DEROUTER_LOCAL_MODEL_COST_MAP", "True")
    # Ensure derouter.model_cost is loaded, relying on the backup mechanism if primary fails
    ...
    info = derouter.get_model_info(model_known_to_support_computer_use)
    ...
    info_gpt = derouter.get_model_info(model_known_not_to_support_computer_use)
...
def test_pre_process_non_default_params(model, custom_llm_provider):
    ...
    from derouter.utils import pre_process_non_default_params
...
def test_provider_supports_vertex_params(custom_llm_provider, expected):
    from derouter.utils import _provider_supports_vertex_params
...
def test_vertex_params_not_stripped_for_vertex_family(
    ...
    optional_params = derouter.utils.get_optional_params(
        model=model,
        custom_llm_provider=custom_llm_provider,
...
class TestProxyFunctionCalling:
    ...
    @pytest.mark.parametrize(
        ...
        [
            # OpenAI models
            ("gpt-3.5-turbo", "derouter_proxy/gpt-3.5-turbo", True),
            ("gpt-4", "derouter_proxy/gpt-4", True),
            ("gpt-4o", "derouter_proxy/gpt-4o", True),
            ("gpt-4o-mini", "derouter_proxy/gpt-4o-mini", True),
            ("gpt-4-turbo", "derouter_proxy/gpt-4-turbo", True),
            ("gpt-4-1106-preview", "derouter_proxy/gpt-4-1106-preview", True),
            # Azure OpenAI models
            ("azure/gpt-4", "derouter_proxy/azure/gpt-4", True),
            ("azure/gpt-3.5-turbo", "derouter_proxy/azure/gpt-3.5-turbo", True),
            (
                "azure/gpt-4-1106-preview",
                "derouter_proxy/azure/gpt-4-1106-preview",
                True,
            ),
            # Anthropic models (Claude supports function calling)
            (
                "claude-sonnet-4-6",
                "derouter_proxy/claude-sonnet-4-6",
                True,
            ),
            # Google models
            ("gemini-2.5-pro", "derouter_proxy/gemini-2.5-pro", True),
            ("gemini/gemini-2.5-pro", "derouter_proxy/gemini/gemini-2.5-pro", True),
            ("gemini/gemini-2.5-flash", "derouter_proxy/gemini/gemini-2.5-flash", True),
            # Groq models (mixed support)
            ("groq/gemma-7b-it", "derouter_proxy/groq/gemma-7b-it", True),
            (
                "groq/llama-3.3-70b-versatile",
                "derouter_proxy/groq/llama-3.3-70b-versatile",
                True,
            ),
            # Cohere models (generally don't support function calling)
            ("command-nightly", "derouter_proxy/command-nightly", False),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # These will return False because DeRouter cannot determine the underlying model
            (
                "derouter_proxy/bedrock-claude-3-haiku",
                "bedrock/anthropic.claude-3-haiku-20240307-v1:0",
                False,
            ),
            (
                "derouter_proxy/bedrock-claude-3-sonnet",
                "bedrock/anthropic.claude-3-sonnet-20240229-v1:0",
                False,
            ),
            (
                "derouter_proxy/bedrock-claude-3-opus",
                "bedrock/anthropic.claude-sonnet-4-5-20250929-v1:0",
                False,
            ),
            (
                "derouter_proxy/bedrock-claude-instant",
                "bedrock/anthropic.claude-instant-v1",
                False,
            ),
            (
                "derouter_proxy/bedrock-titan-text",
                "bedrock/amazon.titan-text-express-v1",
                False,
            ),
            # Azure with custom deployment names (cannot be resolved)
            ("derouter_proxy/my-gpt4-deployment", "azure/gpt-4", False),
            ("derouter_proxy/production-gpt35", "azure/gpt-3.5-turbo", False),
            ("derouter_proxy/dev-gpt4o", "azure/gpt-4o", False),
            # Custom OpenAI deployments (cannot be resolved)
            ("derouter_proxy/company-gpt4", "gpt-4", False),
            ("derouter_proxy/internal-gpt35", "gpt-3.5-turbo", False),
            # Vertex AI with custom names (cannot be resolved)
            ("derouter_proxy/vertex-gemini-pro", "vertex_ai/gemini-1.5-pro", False),
            ("derouter_proxy/vertex-gemini-flash", "vertex_ai/gemini-1.5-flash", False),
            # Anthropic with custom names (cannot be resolved)
            ("derouter_proxy/claude-prod", "anthropic/claude-3-sonnet-20240229", False),
            ("derouter_proxy/claude-dev", "anthropic/claude-3-haiku-20240307", False),
            # Groq with custom names (cannot be resolved)
            ("derouter_proxy/fast-llama", "groq/llama-3.1-8b-instant", False),
            ("derouter_proxy/groq-gemma", "groq/gemma-7b-it", False),
            # Cohere with custom names (cannot be resolved)
            ("derouter_proxy/cohere-command", "cohere/command-r", False),
            ("derouter_proxy/cohere-command-plus", "cohere/command-r-plus", False),
            # Together AI with custom names (cannot be resolved)
            (
                "derouter_proxy/together-llama",
                "together_ai/meta-llama/Llama-2-70b-chat-hf",
                False,
            ),
            (
                "derouter_proxy/together-mistral",
                "together_ai/mistralai/Mistral-7B-Instruct-v0.1",
                False,
            ),
            # Ollama with custom names (cannot be resolved)
            ("derouter_proxy/local-llama", "ollama/llama2", False),
            ("derouter_proxy/local-mistral", "ollama/mistral", False),
        ],
    )
    def test_proxy_custom_model_names_without_config(
        ...
        Without proxy configuration context, DeRouter cannot resolve custom model names
    ...
    def test_proxy_model_resolution_with_custom_names_documentation(self):
        ...
        custom_model = "derouter_proxy/my-custom-claude"
        ...
        resolvable_model = "derouter_proxy/claude-sonnet-4-5-20250929"
        ...
        ✅ WORKS (with current fallback logic):
           - derouter_proxy/gpt-4
           - derouter_proxy/claude-sonnet-4-5-20250929
           - derouter_proxy/anthropic/claude-3-haiku-20240307
           
        ❌ DOESN'T WORK (requires proxy server config):
           - derouter_proxy/my-custom-gpt4
           - derouter_proxy/bedrock-claude-3-haiku
           - derouter_proxy/production-model
           
        💡 SOLUTION: Use DeRouter proxy server with proper model_list configuration
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("derouter_proxy/gpt-4-with-functions", True),  # Hints at GPT-4
            ("derouter_proxy/claude-3-haiku-prod", True),  # Hints at Claude 3 Haiku
            (
                "derouter_proxy/bedrock-anthropic-claude-3-sonnet",
                True,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("derouter_proxy/gpt-3.5-turbo", True),
            ("derouter_proxy/gpt-4", True),
            ("derouter_proxy/gpt-4o", True),
            ("derouter_proxy/claude-sonnet-4-6", True),
            ("derouter_proxy/gemini/gemini-2.5-pro", True),
            # Test proxy models that should not support function calling
            ("derouter_proxy/command-nightly", False),
            ("derouter_proxy/anthropic.claude-instant-v1", False),
        ],
    )
    ...
    def test_derouter_utils_supports_function_calling_import(self):
        """Test that supports_function_calling can be imported from derouter.utils."""
        try:
            from derouter.utils import supports_function_calling
    ...
    def test_derouter_supports_function_calling_import(self):
        """Test that supports_function_calling can be imported from derouter directly."""
        try:
            import derouter

            assert hasattr(derouter, "supports_function_calling")
            assert callable(derouter.supports_function_calling)
        except Exception as e:
            pytest.fail(f"Failed to access derouter.supports_function_calling: {e}")

    @pytest.mark.parametrize(
        "model_name",
        [
            "derouter_proxy/gpt-3.5-turbo",
            "derouter_proxy/gpt-4",
            "derouter_proxy/claude-sonnet-4-6",
            "derouter_proxy/gemini/gemini-2.5-pro",
        ],
    )
    ...
    def test_edge_cases_and_malformed_proxy_models(self):
        ...
        test_cases = [
            ("derouter_proxy/", False),  # Empty model name after proxy prefix
            ("derouter_proxy", False),  # Just the proxy prefix without slash
            ("derouter_proxy//gpt-3.5-turbo", False),  # Double slash
            ("derouter_proxy/nonexistent-model", False),  # Non-existent model
        ]
    ...
    def test_proxy_model_resolution_demonstration(self):
        ...
        proxy_model = "derouter_proxy/gpt-3.5-turbo"
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "derouter_proxy/bedrock-claude-3-5-sonnet",
            ...
            (
                "derouter_proxy/bedrock-claude-v2",
                "bedrock/anthropic.claude-v2",
                False,
                "Bedrock Claude v2 Legacy API",
            ),
            (
                "derouter_proxy/bedrock-claude-v2-1",
            ...
            (
                "derouter_proxy/prod-claude-haiku",
                "bedrock/converse/anthropic.claude-3-haiku-20240307-v1:0",
                False,
                "Production Claude Haiku",
            ),
            (
                "derouter_proxy/dev-claude-sonnet",
                "bedrock/converse/anthropic.claude-3-sonnet-20240229-v1:0",
                False,
                "Development Claude Sonnet",
            ),
            (
                "derouter_proxy/staging-claude-opus",
                "bedrock/converse/anthropic.claude-sonnet-4-5-20250929-v1:0",
                False,
                "Staging Claude Opus",
            ),
            (
                "derouter_proxy/cost-optimized-claude",
                "bedrock/converse/anthropic.claude-3-haiku-20240307-v1:0",
                False,
                "Cost-optimized Claude deployment",
            ),
            (
                "derouter_proxy/high-performance-claude",
                "bedrock/converse/anthropic.claude-sonnet-4-5-20250929-v1:0",
                False,
            ...
            (
                "derouter_proxy/us-east-claude",
                "bedrock/converse/anthropic.claude-3-sonnet-20240229-v1:0",
                False,
                "US East Claude deployment",
            ),
            (
                "derouter_proxy/eu-west-claude",
                "bedrock/converse/anthropic.claude-3-haiku-20240307-v1:0",
                False,
                "EU West Claude deployment",
            ),
            (
                "derouter_proxy/ap-south-llama",
                "bedrock/converse/meta.llama3-70b-instruct-v1:0",
                False,
    ...
    def test_bedrock_converse_api_proxy_mappings(
        ...
        and cannot be resolved by DeRouter without that context.
        """
        print(f"\nTesting: {description}")
    ...
    def test_real_world_proxy_config_documentation(self):
        ...
        3. DeRouter evaluates the underlying model's capabilities
        4. Returns: True (because Claude 3 Haiku supports function calling)
        
        Without the proxy server configuration context, DeRouter cannot resolve
...
def test_register_model_with_scientific_notation():
    ...
    derouter.register_model(model_cost_dict)

    registered_model = derouter.model_cost[test_model_name]
    print(registered_model)
    assert registered_model["input_cost_per_token"] == 3e-07
    assert registered_model["output_cost_per_token"] == 6e-07
    assert registered_model["derouter_provider"] == "openai"
    assert registered_model["mode"] == "chat"

    # Clean up after test
    if test_model_name in derouter.model_cost:
        del derouter.model_cost[test_model_name]
    _invalidate_model_cost_lowercase_map()
...
def test_register_model_openrouter_without_slash():
    ...
    Fixes https://github.com/Delify-Solutions/derouter/issues/18936
    ...
    derouter.openrouter_models.discard("my-custom-alias")
    derouter.openrouter_models.discard("gpt-4")
    derouter.openrouter_models.discard("openai/gpt-4")
    ...
    derouter.register_model(
        {
            "my-custom-alias": {
                ...
                "derouter_provider": "openrouter",
                "mode": "chat",
            },
        }
    )
    assert "my-custom-alias" in derouter.openrouter_models
    ...
    assert "gpt-4" in derouter.openrouter_models
    ...
    assert "openai/gpt-4-turbo" in derouter.openrouter_models
...
def test_anthropic_claude_4_invoke_chat_provider_config():
    ...
    from derouter.llms.bedrock.chat.invoke_transformations.anthropic_claude3_transformation import (
...
def test_bedrock_application_inference_profile():
    ...
    from derouter.utils import supports_tool_choice
...
def test_image_response_utils():
    ...
    from derouter.utils import ImageResponse
...
def test_generate_gcp_iam_access_token():
    ...
    with patch.dict(
        ...
        from derouter._redis import _generate_gcp_iam_access_token
...
def test_generate_azure_ad_redis_token():
    ...
    with patch.dict(
        ...
        from derouter._redis import _generate_azure_ad_redis_token
...
def test_redis_client_logic_azure_ad_auth():
    ...
    with patch.dict(
        ...
        from derouter._redis import _get_redis_client_logic
...
def test_model_info_for_vertex_ai_deepseek_model():
    ...
    assert model_info is not None
    assert model_info["derouter_provider"] == "vertex_ai-deepseek_models"
...
def test_model_info_for_openrouter_kimi_k2_5():
    ...
    assert model_info["derouter_provider"] == "openrouter"
    assert model_info["mode"] == "chat"
...
def test_gemini_embedding_2_ga_in_cost_map():
    ...
    for key, provider in (
        ...
        assert info["derouter_provider"] == provider
...
def test_gemini_lyria_3_preview_models_in_cost_map():
    ...
    assert clip["derouter_provider"] == "gemini" and pro["derouter_provider"] == "gemini"
...
def test_vertex_ai_lyria_models_in_cost_map():
    ...
    assert lyria_2["derouter_provider"] == "vertex_ai"
    assert clip["derouter_provider"] == "vertex_ai"
    assert pro["derouter_provider"] == "vertex_ai"
...
def test_model_info_for_fireworks_short_form_models():
    ...
    for key in [
        ...
        assert info["derouter_provider"] == "fireworks_ai"
...
class TestGetValidModelsWithCLI:
    ...
    def test_get_valid_models_with_cli_pattern(self):
        """Test get_valid_models with derouter_proxy provider and CLI token pattern"""
        ...
        mock_response.json.return_value = {
            "data": [
                ...
                {"id": "derouter_proxy/gemini/gemini-2.5-flash", "object": "model"},
        ...
        with patch.object(
            ...
            result = derouter.get_valid_models(
            ...
            assert len(result) == 4
            # All models get prefixed with "derouter_proxy/" by the get_models method
            assert "derouter_proxy/gpt-3.5-turbo" in result
            assert "derouter_proxy/gpt-4" in result
            # Note: This model already had the prefix, so it gets double-prefixed
            assert "derouter_proxy/derouter_proxy/gemini/gemini-2.5-flash" in result
            assert "derouter_proxy/claude-3-sonnet" in result
...
class TestProxyLoggingBudgetAlerts:
    ...
    async def test_budget_alerts_soft_budget_with_alert_emails_bypasses_alerting_none(
        ...
        """
        from derouter.caching.caching import DualCache
        from derouter.proxy._types import CallInfo, Litellm_EntityType
        from derouter.proxy.utils import ProxyLogging
...
def test_azure_ai_claude_provider_config():
    ...
    from derouter import AzureAIStudioConfig, AzureAnthropicConfig
...
# Related to issue: https://github.com/Delify-Solutions/derouter/issues/18926
...
def test_any_assistant_message_has_thinking_blocks_with_thinking():
    ...
    from derouter.utils import any_assistant_message_has_thinking_blocks
...
class TestAdditionalDropParamsForNonOpenAIProviders:
    ...
    Fixes https://github.com/Delify-Solutions/derouter/issues/19225
    ...
    def test_additional_drop_params_filters_for_bedrock(self):
        ...
        """
        from derouter.utils import add_provider_specific_params_to_optional_params
...
class TestDropParamsWithPromptCacheKey:
    ...
    def test_prompt_cache_key_in_default_params(self):
        ...
        from derouter.constants import DEFAULT_CHAT_COMPLETION_PARAM_VALUES
    ...
    def test_drop_params_removes_prompt_cache_key_for_bedrock(self):
        ...
        """
        from derouter.utils import get_optional_params
...
class TestCallbackAsyncSyncSeparation:
    ...
    def test_async_success_callback_routed_to_async_list(self):
        ...
        derouter.logging_callback_manager.add_derouter_success_callback(my_async_cb)
        assert my_async_cb in derouter._async_success_callback
        assert my_async_cb not in derouter.success_callback

    def test_sync_success_callback_stays_in_sync_list(self):
        def my_sync_cb(*args, **kwargs):
            pass

        derouter.logging_callback_manager.add_derouter_success_callback(my_sync_cb)
        assert my_sync_cb in derouter.success_callback
        assert my_sync_cb not in derouter._async_success_callback

    def test_string_callback_stays_in_sync_list(self):
        derouter.logging_callback_manager.add_derouter_success_callback("langfuse")
        assert "langfuse" in derouter.success_callback
        assert "langfuse" not in derouter._async_success_callback

    def test_async_failure_callback_routed_to_async_list(self):
        async def my_async_cb(*args, **kwargs):
            pass

        derouter.logging_callback_manager.add_derouter_failure_callback(my_async_cb)
        assert my_async_cb in derouter._async_failure_callback
        assert my_async_cb not in derouter.failure_callback

    def test_sync_failure_callback_stays_in_sync_list(self):
        def my_sync_cb(*args, **kwargs):
            pass

        derouter.logging_callback_manager.add_derouter_failure_callback(my_sync_cb)
        assert my_sync_cb in derouter.failure_callback
        assert my_sync_cb not in derouter._async_failure_callback

    def test_dynamodb_routed_to_async_success(self):
        derouter.logging_callback_manager.add_derouter_success_callback("dynamodb")
        assert "dynamodb" in derouter._async_success_callback
        assert "dynamodb" not in derouter.success_callback

    def test_openmeter_routed_to_async_success(self):
        derouter.logging_callback_manager.add_derouter_success_callback("openmeter")
        assert "openmeter" in derouter._async_success_callback
        assert "openmeter" not in derouter.success_callback

    def test_async_input_callback_routed_to_async_list(self):
        async def my_async_cb(*args, **kwargs):
            pass

        derouter.logging_callback_manager.add_derouter_input_callback(my_async_cb)
        assert my_async_cb in derouter._async_input_callback
        assert my_async_cb not in derouter.input_callback

    def test_sync_input_callback_stays_in_sync_list(self):
        def my_sync_cb(*args, **kwargs):
            pass

        derouter.logging_callback_manager.add_derouter_input_callback(my_sync_cb)
        assert my_sync_cb in derouter.input_callback
        assert my_sync_cb not in derouter._async_input_callback
...
class TestMetadataNoneHandling:
    ...
    def test_metadata_none_model_group_check(self):
        ...
        kwargs = {"metadata": None}
        _is_derouter_router_call = "model_group" in (kwargs.get("metadata") or {})
        assert _is_derouter_router_call is False
    ...
    def test_metadata_present_with_values(self):
        ...
        _is_derouter_router_call = "model_group" in (kwargs.get("metadata") or {})
        assert _is_derouter_router_call is True
    ...
    def test_derouter_params_metadata_none(self):
        """derouter_params.get("metadata") or {} should handle None value."""
        derouter_params = {"metadata": None}
...
class TestValidateAndFixThinkingParam:
    ...
    def test_none_returns_none(self):
        from derouter.utils import validate_and_fix_thinking_param
    ...
    def test_bool_true_maps_to_enabled_with_default_budget(self):
        from derouter.constants import DEFAULT_REASONING_EFFORT_MEDIUM_THINKING_BUDGET
        from derouter.utils import validate_and_fix_thinking_param
...
def test_deepseek_v4_models_in_cost_map():
    ...
    Closes https://github.com/Delify-Solutions/derouter/issues/26709
    """
    import json
    from pathlib import Path
    ...
    for key, expected_input, expected_output, expected_cache in [
        ...
        assert info["derouter_provider"] == "deepseek"
        assert info["mode"] == "chat"
...
def test_deepseek_v4_models_in_backup_cost_map():
    """
    Test that deepseek-v4-flash and deepseek-v4-pro entries are correctly
    configured in derouter/model_prices_and_context_window_backup.json.
    """
    import json
    from pathlib import Path

    json_path = Path(__file__).parents[2] / "derouter" / "model_prices_and_context_window_backup.json"
    with open(json_path) as f:
        model_cost = json.load(f)
...
class TestGetOptionalParamsTencent:
    ...
    def test_tencent_supports_thinking_param(self):
        ...
        with patch(
            "derouter.llms.tencent.chat.transformation.supports_reasoning",
            return_value=True,
        ):
            result = get_optional_params(
    ...
    def test_tencent_messages_config_routing(self):
        ...
        config = ProviderConfigManager.get_provider_anthropic_messages_config(
            ...
            provider=derouter.LlmProviders.TENCENT,
        )
...
class TestValidateEnvironmentTencent:
    ...
    def test_reports_key_present(self):
        with patch.dict(os.environ, {"TENCENT_API_KEY": "sk-tencent"}):
            result = derouter.validate_environment(model="tencent/deepseek-v4-pro")
...
class TestVertexEmbeddingEncodingFormat:
    ...
    def test_encoding_format_float_is_accepted_and_dropped(self):
        optional_params = derouter.utils.get_optional_params_embeddings(
    ...
    def test_encoding_format_base64_still_rejected_without_drop_params(self):
        with pytest.raises(Exception, match='To drop these, set `derouter\\.drop_params=True` or for proxy') as excinfo:
            derouter.utils.get_optional_params_embeddings(
                model="gemini-embedding-001",
...
def test_gemini_image_models_do_not_support_reasoning(
    ...
    assert model in derouter.model_cost, (
        f"{model} is missing from the local model cost map. "
        "Add its entry to derouter/model_prices_and_context_window_backup.json."
    )
    assert derouter.supports_reasoning(model) is False, (
...
def test_anthropic_reexport_entries_carry_explicit_prompt_cache_min_tokens(local_model_cost_map: None) -> None:
    ...
    wrong: Final = {
        model: (derouter.model_cost[model].get("prompt_cache_min_tokens"), get_prompt_cache_min_tokens(model=model))
        for model, expected in ANTHROPIC_REEXPORT_CACHE_MIN.items()
        if derouter.model_cost[model].get("prompt_cache_min_tokens") != expected
...
def test_is_prompt_caching_valid_prompt_uses_per_model_minimum(local_model_cost_map: None) -> None:
    ...
    token_count = derouter.token_counter(
...
def test_custom_logger_guards_ignore_subclass_instances(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(derouter, "success_callback", [UserSubclassLogger()])
    monkeypatch.setattr(derouter, "failure_callback", [UserSubclassLogger()])
    ...
    monkeypatch.setattr(derouter, "success_callback", [BuiltinLogger()])
    monkeypatch.setattr(derouter, "failure_callback", [BuiltinLogger()])
...
async def test_s3_v2_success_callback_registers_alongside_user_subclass(
    ...
    """Regression LIT-4392: with a user S3Logger subclass registered via derouter_settings.callbacks
    ...
    monkeypatch.setattr(derouter, "success_callback", [user_logger, "s3_v2"])
    monkeypatch.setattr(derouter, "_async_success_callback", [user_logger])
    monkeypatch.setattr(derouter, "failure_callback", [])
    monkeypatch.setattr(derouter, "_async_failure_callback", [])

    _add_custom_logger_callback_to_specific_event("s3_v2", "success")

    assert any(type(cb) is S3Logger for cb in derouter.success_callback)
    assert any(type(cb) is S3Logger for cb in derouter._async_success_callback)
    assert "s3_v2" not in derouter.success_callback
    assert user_logger in derouter.success_callback
...
async def test_builtin_string_callback_registers_when_subclass_already_active(
    ...
    """Regression LIT-4392, derouter.callbacks path: the inline dedup in function_setup also
    matched subclass instances, so a built-in name in derouter.callbacks was dropped whenever a
    ...
    user_logger = UserS3Logger()
    monkeypatch.setattr(derouter, "callbacks", ["s3_v2"])
    monkeypatch.setattr(derouter, "input_callback", [])
    monkeypatch.setattr(derouter, "success_callback", [user_logger])
    monkeypatch.setattr(derouter, "failure_callback", [])
    monkeypatch.setattr(derouter, "_async_success_callback", [user_logger])
    monkeypatch.setattr(derouter, "_async_failure_callback", [])
...
def test_reapply_runtime_registrations_replays_register_model_overrides(monkeypatch):
    ...
    price-data reload swaps derouter.model_cost for a freshly fetched catalog,
    ...
    monkeypatch.setattr(
        ...
        dict(derouter_utils._runtime_registered_model_cost),
    )
    ...
    monkeypatch.setattr(derouter_utils._LiveDeploymentReplay, "callback", None)

    saved_model_cost = derouter.model_cost
    try:
        derouter.register_model(
            model_cost={
                "openai/gpt-4o": {
        ...
        assert derouter.model_cost["openai/gpt-4o"]["input_cost_per_token"] == 0.000123
        assert derouter.model_cost["openai/gpt-4o"]["max_input_tokens"] == 4242
    finally:
        derouter.model_cost = saved_model_cost
        _invalidate_model_cost_lowercase_map()
...
def test_reapply_runtime_registrations_drops_request_scoped_registrations(monkeypatch):
    ...
    try:
        derouter.register_model(
            model_cost={"openai/gpt-4o": {"derouter_provider": "openai", "input_cost_per_token": 0.000111}},
            persist_across_reloads=True,
        )
        derouter.register_model(
            model_cost={"openai/gpt-4o-mini": {"derouter_provider": "openai", "input_cost_per_token": 0.000222}},
            persist_across_reloads=False,
        )

        derouter.model_cost = {
            "openai/gpt-4o": {"derouter_provider": "openai", "input_cost_per_token": 0.000999},
            "openai/gpt-4o-mini": {"derouter_provider": "openai", "input_cost_per_token": 0.000888},
        }
        _invalidate_model_cost_lowercase_map()
        reapply_runtime_model_cost_registrations()

        assert derouter.model_cost["openai/gpt-4o"]["input_cost_per_token"] == 0.000111
        assert derouter.model_cost["openai/gpt-4o-mini"]["input_cost_per_token"] == 0.000888
    finally:
        derouter.model_cost = saved_model_cost
        _invalidate_model_cost_lowercase_map()
...
async def test_wrapper_async_restores_originating_task_context_after_success(monkeypatch):
    ...
    finally block (in derouter/utils.py) must separately restore the *originating*
    task's trace_id/session_id, since nothing else does.
    """
    monkeypatch.setattr(derouter, "request_correlation_in_logs", True)
    ...
    try:
        await derouter.acompletion(
            ...
            derouter_session_id="mock-call-session",
            num_retries=0,
        )
    ...
    try:
        with pytest.raises(RuntimeError, match="simulated failure"):
            derouter.completion(
                ...
                messages=[{"role": "user", "content": "hi"}],
                mock_response="Hello there!",
                derouter_session_id="doomed-call-session",
                num_retries=0,
            )
...
def test_websearch_interception_control_fields_never_reach_the_provider():
    ...
    assert set(WEBSEARCH_INTERNAL_CONTROL_FIELDS) <= set(all_derouter_params)


def test_bedrock_batch_params_never_reach_the_provider():
    """A Bedrock managed-batch deployment carries aws_batch_role_arn / s3_* /
    bedrock_tags in its derouter_params, and the same deployment also serves chat.
    ...
    The batch path is unaffected by registering them, because GenericDeRouterParams
    is extra="allow" and preserves them into derouter_params for the batch and files
    transformations that read them.
    """
    configured = {
        field: ([{"key": "team", "value": "configured-value"}] if field == "bedrock_tags" else "configured-value")
        for field in bedrock_batch_derouter_params
    }
    ...
    assert set(bedrock_batch_derouter_params) <= set(all_derouter_params)

    batch_params = dict(GenericDeRouterParams(**kwargs))
    assert all(batch_params.get(field) == configured[field] for field in bedrock_batch_derouter_params), (
        "registering these must not strip them from the batch path: "
        f"{sorted(f for f in bedrock_batch_derouter_params if batch_params.get(f) != configured[f])}"
    )

    normalized = CredentialDeRouterParams.model_validate(
        GenericDeRouterParams(**kwargs).model_dump(exclude_none=True)
    ).model_dump(exclude_none=True)
    assert all(normalized.get(field) == configured[field] for field in bedrock_batch_derouter_params), (
        "credential normalization dropped batch params before the transformation: "
        f"{sorted(f for f in bedrock_batch_derouter_params if normalized.get(f) != configured[f])}"
    )
...
def test_client_side_timeout_marker_never_reaches_the_provider():
    ...
    x-derouter-timeout headers) so the router can skip cooldowns on the resulting 408s.
...
async def test_async_post_call_failure_deployment_hook_calls_custom_logger_callbacks(
    ...
    monkeypatch.setattr(derouter, "callbacks", [recorder])
...
async def test_async_post_call_failure_deployment_hook_swallows_callback_errors(
    ...
    monkeypatch.setattr(derouter, "callbacks", [exploding_logger])
...
async def test_async_post_call_failure_deployment_hook_skips_non_custom_logger_callbacks(
    ...
    monkeypatch.setattr(derouter, "callbacks", [fn_callback])
...
async def test_wrapper_async_fires_post_call_failure_deployment_hook_once_per_failed_attempt(
    ...
    with pytest.raises(derouter.AuthenticationError):
        await derouter.acompletion(
            model="gpt-4o-mini",
            messages=[{"role": "user", "content": "hi"}],
            mock_response=derouter.AuthenticationError(message="bad key", llm_provider="openai", model="gpt-4o-mini"),
        )

    assert len(recorder.calls) == 1
    _, received_exc, call_type, fallback_depth = recorder.calls[0]
    assert isinstance(received_exc, derouter.AuthenticationError)
    assert call_type == CallTypes.acompletion
    assert fallback_depth is None
...
async def test_wrapper_async_raises_original_exception_even_if_hook_callback_errors(
    ...
    monkeypatch.setattr(derouter, "callbacks", [ExplodingLogger()])

    with pytest.raises(derouter.AuthenticationError, match="bad key"):
        await derouter.acompletion(
            model="gpt-4o-mini",
            messages=[{"role": "user", "content": "hi"}],
            mock_response=derouter.AuthenticationError(message="bad key", llm_provider="openai", model="gpt-4o-mini"),
...
async def test_router_fallback_chain_reports_increasing_fallback_depth(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    router = derouter.Router(
        model_list=[
            {"model_name": "bad-group", "derouter_params": {"model": "openai/gpt-4o-mini", "api_key": "bad-a"}},
            {"model_name": "good-group", "derouter_params": {"model": "openai/gpt-4o-mini", "api_key": "bad-b"}},
        ],
        num_retries=0,
...
async def test_router_multi_hop_fallback_chain_reports_depth_per_hop(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    router = derouter.Router(
        model_list=[
            {"model_name": "group-a", "derouter_params": {"model": "openai/gpt-4o-mini", "api_key": "bad-a"}},
            {"model_name": "group-b", "derouter_params": {"model": "openai/gpt-4o-mini", "api_key": "bad-b"}},
            {"model_name": "group-c", "derouter_params": {"model": "openai/gpt-4o-mini", "api_key": "bad-c"}},
        ],
        num_retries=0,
...
async def test_wrapper_async_fires_post_call_failure_deployment_hook_on_internal_calls(
    ...
    try:
        with pytest.raises(derouter.AuthenticationError):
            await derouter.acompletion(
                model="gpt-4o-mini",
                messages=[{"role": "user", "content": "hi"}],
                mock_response=derouter.AuthenticationError(
    ...
    assert len(recorder.calls) == 1
    assert isinstance(recorder.calls[0][1], derouter.AuthenticationError)
...
async def test_wrapper_async_does_not_fire_failure_hook_for_pre_call_budget_error(
    ...
    recorder = _RecordingDeploymentFailureLogger()
    monkeypatch.setattr(derouter, "callbacks", [recorder])
    monkeypatch.setattr(derouter, "max_budget", 0.0001)
    monkeypatch.setattr(derouter, "_current_cost", 100.0)

    with pytest.raises(derouter.BudgetExceededError):
        await derouter.acompletion(
            model="gpt-4o-mini",
            messages=[{"role": "user", "content": "hi"}],
...
async def test_wrapper_async_calls_hook_override_missing_fallback_depth_param(
    ...
    monkeypatch.setattr(derouter, "callbacks", [three_arg_logger])
    ...
    assert isinstance(three_arg_logger.calls[0][1], derouter.AuthenticationError)
...
async def test_wrapper_async_failure_hook_exception_mutation_does_not_change_raised_exception(
    ...
    monkeypatch.setattr(derouter, "callbacks", [StatusCodeMutatingLogger()])
...
async def test_router_fallback_not_skipped_when_failure_hook_callback_touches_attempted_targets(
    ...
    monkeypatch.setattr(derouter, "callbacks", [RecordingAttemptLogger()])
...
async def test_wrapper_async_preserves_original_exception_when_hook_await_is_cancelled(
    ...
    monkeypatch.setattr(derouter, "callbacks", [SlowLogger()])
...
async def test_wrapper_async_failure_hook_latency_does_not_inflate_reported_duration(
    ...
    monkeypatch.setattr(derouter, "callbacks", [SlowLoggerWithDurationCapture()])
...
async def test_wrapper_async_failure_hook_exception_snapshot_preserves_traceback(
    ...
    monkeypatch.setattr(derouter, "callbacks", [TracebackCapturingLogger()])
...
class TestDefaultReasoningEffortHydration:
    ...
    def test_the_declared_default_survives_model_info_hydration(self, local_model_cost_map, model, provider):
        from derouter.utils import _get_model_info_helper
...
class TestHuggingFaceConfigFetch:
    ...
    def test_get_max_tokens_reads_hf_config_with_a_bounded_timeout(self, hf_config_route):
        from derouter.constants import HF_CONFIG_FETCH_TIMEOUT_SECONDS
        from derouter.utils import get_max_tokens
    ...
    def test_get_max_position_embeddings_reads_hf_config_with_a_bounded_timeout(self, hf_config_route):
        from derouter.constants import HF_CONFIG_FETCH_TIMEOUT_SECONDS
        from derouter.utils import _get_max_position_embeddings
...
async def test_success_deployment_hook_chains_past_callback_returning_response(
    ...
    monkeypatch.setattr(derouter, "callbacks", [PassthroughLogger(), replacer, observer])
...
async def test_registered_guardrail_does_not_starve_vector_store_search_results(
    ...
    result = await async_post_call_success_deployment_hook(
        request_data={"derouter_logging_obj": logging_obj},
        response=response,
...
class TestIsVisionExplicitlyDisabled:
    ...
    def test_never_resolves_an_authenticating_prefix(self, model, monkeypatch):
        from derouter.utils import is_vision_explicitly_disabled
...
class TestVerboseRequestLineRedaction:
    """`derouter.set_verbose = True` echoes the caller's kwargs back as a `derouter.completion(...)`
    ...
    def _verbose_request_line(self, capsys, monkeypatch, **kwargs) -> str:
        monkeypatch.setattr(derouter, "set_verbose", True)
        monkeypatch.setattr("derouter._logging.set_verbose", True)
        capsys.readouterr()
        derouter.completion(
            model="gpt-3.5-turbo",
        ...
        return "\n".join(line for line in (captured.out + captured.err).splitlines() if "derouter.completion(" in line)
    ...
    def test_api_key_never_reaches_the_request_line(self, capsys, monkeypatch):
        ...
        assert "derouter.completion(" in printed
...
class TestFinalOptionalParamsLineRedaction:
    ...
    line has to keep printing on both, because `derouter.set_verbose` and the DEBUG logger are independent
    ...
    def test_line_still_reaches_the_logger_when_only_the_debug_logger_is_on(self, capsys, caplog, monkeypatch):
        monkeypatch.setattr(derouter, "set_verbose", False)
...
class TestDropParamsStringCoercion:
    ...
    def test_falsy_drop_params_still_raises(self, drop_params, monkeypatch):
        ...
        monkeypatch.setattr(derouter, "drop_params", False)
        with pytest.raises(derouter.UnsupportedParamsError):
            get_optional_params(
                model="gpt-5-nano",
                custom_llm_provider="openai",
                temperature=0.1,
...
def _credential_warnings(caplog: pytest.LogCaptureFixture) -> list[str]:
    return [record.getMessage() for record in caplog.records if "derouter_credential_name=" in record.getMessage()]


def test_load_credentials_from_list_warns_when_the_named_credential_is_not_loaded(
    monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture
) -> None:
    from derouter.utils import load_credentials_from_list

    monkeypatch.setattr(derouter, "credential_list", [])
    request_kwargs = {"derouter_credential_name": "openai-cred", "model": "openai/gpt-5.4-mini"}
    with caplog.at_level(logging.WARNING, logger=verbose_logger.name):
        load_credentials_from_list(request_kwargs)

    assert request_kwargs == {"derouter_credential_name": "openai-cred", "model": "openai/gpt-5.4-mini"}
    assert _credential_warnings(caplog) == [
        "derouter_credential_name=openai-cred matched none of the 0 loaded credentials; the request runs without it"
    ]
...
def test_load_credentials_from_list_fills_kwargs_from_the_loaded_credential_without_warning(
    ...
    monkeypatch.setattr(derouter, "credential_list", [loaded])
    request_kwargs = {"derouter_credential_name": "openai-cred", "api_base": "https://request.example"}
    with caplog.at_level(logging.WARNING, logger=verbose_logger.name):
        load_credentials_from_list(request_kwargs)

    assert request_kwargs == {
        "derouter_credential_name": "openai-cred",
