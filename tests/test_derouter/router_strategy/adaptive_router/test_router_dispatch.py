...
3. `init_adaptive_router_deployment` must read `input_cost_per_token`
   from `derouter_params` (where users put it), not just `model_info`.
...
from derouter.types.router import DeRouter_Params, RequestType
...
def test_auto_router_check_excludes_adaptive_router_prefix():
    ...
    assert (
        r._is_auto_router_deployment(
            derouter_params=_params(model="auto_router/adaptive_router")
        )
        is False
    )
...
def test_auto_router_check_excludes_complexity_router_prefix():
    ...
    assert (
        r._is_auto_router_deployment(
            derouter_params=_params(model="auto_router/complexity_router")
        )
        is False
    )
...
def test_auto_router_check_still_matches_plain_auto_router_prefix():
    ...
    assert (
        r._is_auto_router_deployment(
            derouter_params=_params(model="auto_router/my-semantic-router")
        )
        is True
    )
...
def test_adaptive_router_check_rejects_other_prefixes():
    r = Router(model_list=[])
    assert (
        r._is_adaptive_router_deployment(derouter_params=_params(model="openai/gpt-4o"))
        is False
    )
...
def test_init_adaptive_router_reads_cost_from_derouter_params():
    r = Router(
        model_list=[
            {
...
def test_init_adaptive_router_falls_back_to_model_info_cost():
    ...
    DeRouter: cost_calculator.py, add_deployment's derouter.model_cost registration) must still
...
def test_init_adaptive_router_prefers_derouter_params_cost_over_model_info():
    r = Router(
        model_list=[
            {
                "model_name": "smart-cheap-router",
...
async def test_async_pre_routing_hook_dispatches_to_adaptive_router():
    ...
    response = await r.async_pre_routing_hook(
        ...
        request_kwargs={"metadata": {"derouter_session_id": "sess-A"}},
...
async def test_async_pre_routing_hook_stashes_chosen_model_in_metadata():
    ...
    `x-derouter-adaptive-router-model` response header.
    """
    r = _router_with_adaptive()
    ...
    request_kwargs: dict = {"metadata": {"derouter_session_id": "sess-A"}}
...
def test_init_adaptive_router_rejects_duplicate_model_name():
    ...
    from derouter.types.router import AdaptiveRouterConfig, Deployment
...
def test_finalize_prunes_stale_adaptive_router_hooks_from_callbacks():
    ...
    AdaptiveRouterPostCallHook instances in `derouter.callbacks` — otherwise
    ...
    pre_hooks = [
        cb for cb in derouter.callbacks if isinstance(cb, AdaptiveRouterPostCallHook)
    ]
    for cb in pre_hooks:
        derouter.callbacks.remove(cb)
    ...
    try:
        ...
        for cb in list(derouter.callbacks):
            if isinstance(cb, AdaptiveRouterPostCallHook):
                derouter.callbacks.remove(cb)
        for cb in pre_hooks:
            derouter.callbacks.append(cb)
