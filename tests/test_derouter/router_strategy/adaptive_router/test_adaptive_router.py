...
from derouter.router_strategy.adaptive_router import adaptive_router as ar_module
...
async def test_load_state_from_db_keeps_a_one_sided_delta_row_sampleable():
    ...
    from derouter.router_strategy.adaptive_router.bandit import thompson_sample
    ...
    prisma = MagicMock()
    prisma.db.derouter_adaptiverouterstate.find_many = AsyncMock(return_value=[one_sided_row])
    await r.load_state_from_db(prisma)
...
async def test_load_state_from_db_handles_unknown_request_type():
    ...
    prisma = MagicMock()
    prisma.db.derouter_adaptiverouterstate.find_many = AsyncMock(return_value=[bad_row, good_row])
    await r.load_state_from_db(prisma)
...
def test_session_state_is_evicted_after_ttl():
    ...
    from derouter.router_strategy.adaptive_router import adaptive_router as ar
