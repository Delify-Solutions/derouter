...
from derouter.router_strategy.adaptive_router.signals import Turn
...
def _make_mock_prisma():
    p = MagicMock()
    p.db.derouter_adaptiverouterstate.find_unique = AsyncMock(return_value=None)
    p.db.derouter_adaptiverouterstate.find_many = AsyncMock(return_value=[])
    p.db.derouter_adaptiverouterstate.upsert = AsyncMock()
    p.db.derouter_adaptiveroutersession.upsert = AsyncMock()
    return p
...
async def test_pick_record_flush_full_cycle():
    ...
    assert n_state == 1
    assert n_session == 1
    state_call = prisma.db.derouter_adaptiverouterstate.upsert.call_args
    ...
    session_call = prisma.db.derouter_adaptiveroutersession.upsert.call_args
...
async def test_load_state_from_db_adds_persisted_delta_to_cold_start():
    ...
    prisma = _make_mock_prisma()
    prisma.db.derouter_adaptiverouterstate.find_many = AsyncMock(return_value=[fake_row])
...
async def test_load_state_from_db_handles_unknown_request_type():
    ...
    prisma = _make_mock_prisma()
    prisma.db.derouter_adaptiverouterstate.find_many = AsyncMock(return_value=[bad_row])
...
async def test_flush_isolates_writes_per_router_session_model():
    ...
    assert n == 2
    assert prisma.db.derouter_adaptiveroutersession.upsert.call_count == 2

    n_state = await router.queue.flush_state_to_db(prisma)
    assert n_state == 2
    assert prisma.db.derouter_adaptiverouterstate.upsert.call_count == 2
...
async def test_repeated_flush_drains_queue_and_subsequent_flush_is_noop():
    ...
    assert prisma.db.derouter_adaptiverouterstate.upsert.call_count == 1
    assert prisma.db.derouter_adaptiveroutersession.upsert.call_count == 1
