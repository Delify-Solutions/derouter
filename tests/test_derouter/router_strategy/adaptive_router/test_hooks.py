...
def test_resolve_session_key_honors_derouter_session_id_on_derouter_params():
    key = _resolve_session_key({"derouter_params": {"derouter_session_id": "sess-A"}})
    assert key == "sess-A"


def test_resolve_session_key_honors_metadata_session_id():
    key = _resolve_session_key(
        {"derouter_params": {"metadata": {"session_id": "sess-B"}}}
    )
    assert key == "sess-B"


def test_resolve_session_key_returns_none_when_no_messages():
    assert _resolve_session_key({"derouter_params": {}}) is None
    assert _resolve_session_key({"derouter_params": {}, "messages": []}) is None
...
async def test_hook_tracks_short_conversation_with_explicit_session_id():
    ...
    kwargs = _kwargs(
        ...
        extra_derouter_params={"derouter_session_id": "explicit-short"},
    )
...
async def test_hook_uses_explicit_session_id_when_provided():
    """Explicit `derouter_session_id` is forwarded as the session key."""
    hook = _make_hook()
    kwargs = _kwargs(
        chosen="fast",
        extra_derouter_params={"derouter_session_id": "explicit-sess"},
    )
    await hook.async_log_success_event(kwargs, _resp_with_content("ok"), 0.0, 1.0)
    assert hook.adaptive_router.record_turn.await_args.kwargs["session_id"] == (
        "explicit-sess"
...
async def test_post_call_response_headers_hook_returns_chosen_model_header():
    """The header hook returns the `x-derouter-adaptive-router-model` header
    ...
    assert headers == {"x-derouter-adaptive-router-model": "smart"}
...
async def test_post_call_response_headers_hook_noop_when_metadata_missing_key():
    hook = _make_hook()
    headers = await hook.async_post_call_response_headers_hook(
        data={"metadata": {"derouter_session_id": "sess-A"}},
        user_api_key_dict=MagicMock(),
        response=MagicMock(),
    )
