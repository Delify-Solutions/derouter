...
from derouter.router_strategy.savings_baseline import (
    Baseline,
    canonical_model,
    _models_in,
    _most_expensive,
...
def parent() -> Router:
    return Router(
        model_list=[
            ...
            {"model_name": "top", "derouter_params": {"model": "anthropic/claude-opus-5"}},
            {"model_name": "pool", "derouter_params": {"model": "anthropic/claude-haiku-4-5"}},
            {"model_name": "pool", "derouter_params": {"model": "anthropic/claude-opus-5"}},
        ]
    )
...
class TestDeploymentsPricedByBaseModel:
    """`derouter_params.model` is not always a model.
    ...
    def test_model_info_base_model_is_preferred_over_the_deployment_name(self):
        router = self._router(
            {
                "model_name": "big",
                "derouter_params": {"model": "azure/my-gpt5-deployment"},
                "model_info": {"base_model": "azure/gpt-4.1"},
            },
        )
        assert [c.model for c in _models_in(router, "big")] == ["azure/gpt-4.1"]

    def test_derouter_params_base_model_is_the_other_accepted_spelling(self):
        router = self._router(
            {
                "model_name": "big",
                "derouter_params": {"model": "azure/my-gpt5-deployment", "base_model": "azure/gpt-4.1"},
            },
        )
        assert [c.model for c in _models_in(router, "big")] == ["azure/gpt-4.1"]

    def test_a_deployment_without_a_base_model_still_prices_by_its_model(self):
        router = self._router({"model_name": "big", "derouter_params": {"model": "anthropic/claude-opus-5"}})
