...
@pytest.mark.asyncio
async def test_get_llm_provider_for_deployment_dict_does_not_require_derouter_params_instantiation(
    disable_budget_sync, monkeypatch
):
    class RaiseOnInit:
        def __init__(self, *args, **kwargs):
            raise AssertionError(
                "DeRouter_Params should not be instantiated in hot path"
            )

    monkeypatch.setattr(
        "derouter.router_strategy.budget_limiter.DeRouter_Params",
        RaiseOnInit,
    )
...
async def test_get_llm_provider_for_deployment_dict_view_supports_mapping_and_attr_access(
    ...
    def _future_style_get_llm_provider(
        ...
        assert derouter_params is not None
        observed["model_attr"] = derouter_params.model
        observed["provider_get"] = derouter_params.get("custom_llm_provider")
        observed["api_base_item"] = derouter_params["api_base"]
        observed["has_api_key"] = "api_key" in derouter_params
        observed["model_dump"] = derouter_params.model_dump()
        return model, "openai", None, None

    monkeypatch.setattr(
        "derouter.router_strategy.budget_limiter.derouter.get_llm_provider",
...
async def test_async_filter_deployments_does_not_recompute_provider_when_resolved_none(
    ...
    healthy_deployments = [
        {
            "model_name": "gpt-4o-mini",
            "derouter_params": {"model": "unknown-provider/model"},
            "model_info": {"id": "deployment-1"},
        }
    ]
...
def _legacy_provider_resolution(deployment):
    ...
    """
    try:
        _derouter_params = DeRouter_Params(
            **deployment.get("derouter_params", {"model": ""})
        )
        _, custom_llm_provider, _, _ = derouter.get_llm_provider(
            model=_derouter_params.model,
...
@pytest.mark.parametrize(
    ...
    [
        {"derouter_params": {"model": "openai/gpt-4o-mini"}},
        {"derouter_params": {"model": "gpt-4o-mini", "custom_llm_provider": "openai"}},
        {"derouter_params": {"model": "unknown-provider/model"}},
    ],
)
@pytest.mark.asyncio
