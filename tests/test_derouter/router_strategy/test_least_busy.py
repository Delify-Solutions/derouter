...
from derouter.router_strategy.least_busy import IN_FLIGHT_COUNT_TTL_SECONDS, LeastBusyLoggingHandler
...
def _call_kwargs(deployment_id: str) -> dict[str, object]:
    return {"derouter_params": {"metadata": {"model_group": GROUP}, "model_info": {"id": deployment_id}}}
...
def test_calls_without_a_deployment_are_ignored() -> None:
    ...
    worker.log_pre_api_call(model="m", messages=[], kwargs={"derouter_params": {"metadata": None}})
