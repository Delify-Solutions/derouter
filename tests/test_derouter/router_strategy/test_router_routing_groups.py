...
from derouter.types.router import RoutingGroup, RoutingStrategy
...
async def test_async_dispatch_uses_group_strategy_for_grouped_model():
    ...
    with (
        ...
        patch(
            "derouter.router.simple_shuffle", wraps=derouter.router.simple_shuffle
        ) as shuffle_spy,
    ):
...
def test_update_settings_does_not_leak_strategy_callbacks(monkeypatch):
    ...
    `derouter.callbacks` / `derouter.input_callback`. Each rebuild owns the
    ...
    assert any(
        c is initial_default_selector for c in derouter.callbacks
    ...
    assert all(
        c is not initial_default_selector for c in derouter.callbacks
    ), "old default selector instance leaked into derouter.callbacks"
    ...
    router_owned = [
        c for c in derouter.callbacks if type(c).__name__ in router_owned_classes
    ]
    ...
    assert all(
        c is not group_selector_v1 for c in derouter.callbacks
    ), "old group selector instance leaked into derouter.callbacks"
...
def test_update_settings_unregisters_group_selectors_when_groups_removed(monkeypatch):
    ...
    assert group_selector in derouter.callbacks
    assert group_selector in derouter.input_callback

    router.update_settings(routing_groups=[])

    assert all(c is not group_selector for c in derouter.callbacks)
    assert all(c is not group_selector for c in derouter.input_callback)
    assert router._group_selectors == {}


def test_two_least_busy_groups_count_a_request_once(monkeypatch):
    """
    Least-busy counts a request up from the pre-call hooks on `derouter.input_callback` and
    back down from the success hooks on `derouter.callbacks`. The success list drops a second
    ...
    for callback in derouter.input_callback:
...
def test_two_routers_in_one_process_each_count_their_own_requests(monkeypatch):
    """
    Least-busy hangs its counting off derouter's global callback lists, and those lists keep one
...
def test_unregister_router_selectors_removes_by_identity(monkeypatch):
    ...
    assert selector in derouter.callbacks
    assert selector in derouter.input_callback
    router._unregister_router_selectors([selector])
    assert all(c is not selector for c in derouter.callbacks)
    assert all(c is not selector for c in derouter.input_callback)
...
def test_strategy_reinit_unregisters_override_selectors():
    ...
    assert not any(cb is override_selector for cb in derouter.callbacks)
...
def test_override_selectors_are_not_registered_process_wide(monkeypatch):
    ...
    assert derouter.input_callback == []
...
def _pass_through_rpm_limited_model_list():
    deployment = _rpm_limited_model_list()[0]
    return [{**deployment, "derouter_params": {**deployment["derouter_params"], "use_in_pass_through": True}}]
...
async def test_group_call_dispatches_via_group_selector():
    ...
    with (
        ...
        patch("derouter.router.simple_shuffle", wraps=derouter.router.simple_shuffle) as shuffle_spy,
    ):
...
def test_real_model_added_later_shadows_group():
    ...
    router.add_deployment(
        Deployment(
            model_name="quality",
            derouter_params={"model": "openai/gpt-4o", "api_key": "sk-test-4", "api_base": "https://example.invalid"},
...
def test_group_rows_cache_invalidated_on_model_list_change():
    ...
    router.add_deployment(
        Deployment(
            model_name="filtered-model",
            derouter_params={"model": "openai/gpt-4o", "api_key": "sk-test-5", "api_base": "https://example.invalid"},
...
async def _call_and_get_cooldowns(router, model):
    from derouter.router_utils.cooldown_handlers import _async_get_cooldown_deployments

    with (
        patch("derouter.router_strategy.simple_shuffle.random.choice", side_effect=_pin_choice_to("deploy-3")),
        pytest.raises(derouter.RateLimitError),
    ):
    ...
    return await _async_get_cooldown_deployments(derouter_router_instance=router, parent_otel_span=None)
