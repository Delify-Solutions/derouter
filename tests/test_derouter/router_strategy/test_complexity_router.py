...
from derouter.router import as_output_cap
...
from derouter.router_strategy.complexity_router.tier_predictor import (
    TierGlobalStatistic,
    TrainedTierArtifact,
)
...
def mock_router_instance():
    """Create a mock DeRouter Router instance."""
    router = MagicMock()
    return router
...
def complexity_router(mock_router_instance, basic_config):
    ...
    return ComplexityRouter(
        ...
        derouter_router_instance=mock_router_instance,
...
class TestModelSelection:
    ...
    def test_get_model_for_tier_list_random_choice(self, mock_router_instance):
        ...
        pool = ["cheap", "premium"]
        with patch(
            "derouter.router_strategy.complexity_router.complexity_router.random.choice",
...
class TestRouterComplexityDeploymentMethods:
    ...
    def test_is_complexity_router_deployment_true(self):
        ...
        params = DeRouter_Params(model="auto_router/complexity_router/my-router")
    ...
    def test_is_complexity_router_deployment_false(self):
        ...
        params = DeRouter_Params(model="openai/gpt-4o-mini")
    ...
    _POOL: dict[str, object] = {
        "model_name": "gpt-4o-mini",
        "derouter_params": {"model": "openai/gpt-4o-mini", "api_key": "k"},
    }
    ...
    def test_failed_edit_of_a_live_v2_router_rolls_back_without_the_ceiling(self) -> None:
        ...
        broken["derouter_params"]["complexity_router_config"]["tiers"] = {}
        assert router.upsert_deployment(Deployment(**broken)) is None

        assert sorted(router.complexity_routers) == ["v2-a", "v2-b"]
        live = router.get_deployment(model_id="id-a")
        assert live is not None and live.derouter_params.complexity_router_config["classifier_type"] == "heuristic_v2"
        assert router.auto_router_capability_violation(HEURISTIC_V2_CAPABILITY) is not None
    ...
    def test_renaming_built_in_tiers_is_not_a_custom_tier_set(self) -> None:
        ...
        def labeled(model_name: str, model_id: str) -> dict[str, object]:
            ...
            row["derouter_params"]["complexity_router_config"]["tier_labels"] = {"SIMPLE": "Cheap", "MEDIUM": "Standard"}
            return row
    ...
    def test_hybrid_adaptive_router_falls_back_to_model_info_cost(self):
        ...
        in DeRouter) must still feed the hybrid adaptive router's cost-weighted scoring, not
    ...
    async def test_hybrid_adaptive_router_pick_model_favors_the_cheaper_model_info_priced_deployment(self):
        ...
        from derouter.types.router import RequestType
    ...
    def test_hybrid_adaptive_router_prefers_derouter_params_cost_over_model_info(self):
        router = Router(
            model_list=[
                {
                    "model_name": "hybrid",
...
class TestComplexityRouterTagBasedRouting:
    """Regression tests for https://github.com/Delify-Solutions/derouter/issues/33655.
...
class TestPreRoutingStrategyRegistry:
    ...
    def _router(self) -> Router:
        return Router(model_list=[{"model_name": "x", "derouter_params": {"model": "openai/gpt-4o-mini"}}])

    @staticmethod
    def _deployment(tags: list) -> Deployment:
        return Deployment(
            model_name="smart",
            derouter_params=DeRouter_Params(model="openai/gpt-4o-mini", tags=tags),
        )

    def test_deployment_tags_normalizes_to_tuple(self):
        router = self._router()
        assert router._deployment_tags(self._deployment(["cn", "row"])) == ("cn", "row")
        untagged = Deployment(model_name="smart", derouter_params=DeRouter_Params(model="openai/gpt-4o-mini"))
    ...
    def _router_with_plain_smart_deployment(enable_tag_filtering: bool) -> Router:
        return Router(
            model_list=[{"model_name": "smart", "derouter_params": {"model": "openai/gpt-4o-mini"}}],
...
class TestAsyncPreRoutingHookMultiFormat:
    ...
    async def test_should_route_with_responses_api_string_input(self, complexity_router):
        ...
        with patch(
            "derouter.llms.load_guardrail_translation_mappings",
...
class TestLLMClassifier:
    ...
    async def test_aclassify_llm_success_captures_classifier_cost(self, llm_complexity_router, mock_router_instance):
        ...
        x-derouter-classifier-cost header), which is otherwise invisible to the caller."""
        mock_router_instance.acompletion = AsyncMock(
    ...
    async def test_aclassify_captures_cost_from_the_real_client_pipeline(self, llm_classifier_config):
        ...
        mock_response, so derouter's own client wrapper (update_response_metadata ->
        ...
        router = ComplexityRouter(
            model_name="test-complexity-router",
            derouter_router_instance=real_router,
            complexity_router_config=llm_classifier_config,
        )
    ...
    async def test_aclassify_forwards_request_metadata_for_spend_tracking(
        ...
        await llm_complexity_router.aclassify("hi", request_kwargs={"derouter_metadata": request_metadata})
        call_kwargs = mock_router_instance.acompletion.call_args.kwargs
    ...
    async def test_aclassify_forwards_metadata_key_used_by_chat_completions(
        ...
        """/v1/chat/completions puts the request metadata under "metadata", not "derouter_metadata".

        Only the routes in DEROUTER_METADATA_ROUTES (/v1/messages, /v1/responses, ...) get a
        "derouter_metadata" bucket; chat completions gets "metadata". Reading only
        "derouter_metadata" leaves the classifier call unattributed on the most common route,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param({"derouter_metadata": {"user_api_key": "sk-abc"}}, id="derouter-metadata-bucket"),
    ...
    async def test_aclassify_reaches_the_llm_for_every_caller_metadata_shape(
        ...
        actually run. The forwarded metadata reaches derouter's own metadata handling, which
    ...
    async def test_aclassify_propagates_metadata_slot_turn_off_message_logging(
        ...
        """turn_off_message_logging set inside metadata/derouter_metadata must also propagate.

        initialize_standard_callback_dynamic_params reads this flag from either the
        top-level request kwargs or the metadata/derouter_metadata dicts (the same slots a
        real HTTP request populates), so the classifier call must resolve it from there too.
        """
        mock_router_instance.acompletion = AsyncMock(return_value=_llm_response('{"tier": "SIMPLE"}'))
        await llm_complexity_router.aclassify(
            "secret prompt", request_kwargs={"derouter_metadata": {"turn_off_message_logging": True}}
        )
        call_kwargs = mock_router_instance.acompletion.call_args.kwargs
        assert call_kwargs["turn_off_message_logging"] is True
    ...
    @pytest.mark.parametrize(
        ...
        [
            ({"derouter_trace_id": "trace-1"}, {"derouter_trace_id": "trace-1"}),
            ({"derouter_session_id": "sess-1"}, {"derouter_session_id": "sess-1"}),
            (
                {"derouter_session_id": "sess-1", "derouter_trace_id": "trace-1"},
                {"derouter_session_id": "sess-1", "derouter_trace_id": "trace-1"},
            ),
            ({}, {}),
        ],
    ...
    async def test_aclassify_chains_classifier_call_into_parent_session(
        ...
        call_kwargs = mock_router_instance.acompletion.call_args.kwargs
        for key in ("derouter_session_id", "derouter_trace_id"):
    ...
    def test_generated_response_format_without_labels_matches_the_shipped_pydantic_schema(self):
        ...
        """
        from derouter.llms.base_llm.base_utils import type_to_response_format_param
        from derouter.router_strategy.complexity_router.complexity_router import (
            TierClassification,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # A model that ignores the rubric and answers in DeRouter's vocabulary should still be
    ...
    async def test_pre_routing_hook_uses_llm_classifier_end_to_end(self, llm_complexity_router, mock_router_instance):
        ...
        result = await llm_complexity_router.async_pre_routing_hook(
            model="test-model",
            request_kwargs={"derouter_metadata": request_metadata},
            messages=[{"role": "user", "content": "hi"}],
        )
        assert result is not None
        assert result.model == "o1-preview"  # REASONING tier model
...
class TestRouterPreRoutingAliasOverrides:
    """
    Regression tests for: derouter_params configured on a complexity-router alias
    ...
    outbound call only ever merges in the tier deployment's own derouter_params,
    never the alias's.
    """
    ...
    @pytest.mark.asyncio
    async def test_alias_derouter_params_applied_to_request_kwargs(self):
    ...
    @pytest.mark.asyncio
    async def test_tier_derouter_params_are_applied_before_deployment_selection(self):
        router = Router(
            model_list=[
                {
                    "model_name": "smart-router",
                        ...
                        "complexity_router_config": {
                            "tiers": {
                                "SIMPLE": {
                                    ...
                                    "derouter_params": {"reasoning_effort": "xhigh"},
                                }
                            }
                        },
                    },
                },
                {"model_name": "gpt-5-mini", "derouter_params": {"model": "openai/gpt-5-mini"}},
            ]
        )
    ...
    def _make_effort_pinned_router(self, tier_derouter_params: Dict) -> Router:
        return Router(
            model_list=[
                {
                    "model_name": "smart-router",
                    "derouter_params": {
                        ...
                        "complexity_router_config": {
                            "tiers": {
                                "SIMPLE": {
                                    "model_name": "gpt-5-mini",
                                    "derouter_params": tier_derouter_params,
                                }
                            }
                        },
                    },
    ...
    async def test_tier_pinned_effort_supersedes_client_carriers_on_pass_through_path(self):
        router = Router(
            model_list=[
                ...
                {
                    "model_name": "gpt-5-mini",
                    "derouter_params": {"model": "openai/gpt-5-mini", "use_in_pass_through": True},
                },
            ]
        )
    ...
    async def test_routing_never_resolves_an_authenticating_provider(self, monkeypatch, tmp_path):
                        ...
                        "complexity_router_config": {
                            "tiers": {
                                "SIMPLE": {
                                    ...
                                    "derouter_params": {"reasoning_effort": "high"},
                                }
                            }
                        },
                    },
                },
                {"model_name": "cop-mixed", "derouter_params": {"model": "openai/gpt-4o-mini", "api_key": "sk-x"}},
                {"model_name": "cop-mixed", "derouter_params": {"model": "github_copilot/gpt-4o"}},
            ]
        )
        real_get_llm_provider = derouter.get_llm_provider
        ...
        monkeypatch.setattr(derouter, "get_llm_provider", _guarded)
        request_kwargs: Dict = {}
    ...
    def test_router_init_only_params_are_never_sent_to_a_provider(self):
        ...
        marker_and_connection_params above) - every other alias derouter_param,
        including router-init-only fields like
        complexity_router_config, flows into request_kwargs unfiltered. That's
        only safe because derouter.completion()/acompletion() itself strips
        anything listed in all_derouter_params before building the provider
        ...
        derouter.completion(..., complexity_router_config={...}) landing in
        ...
        for param in router_init_only_params:
            assert param in all_derouter_params, (
                f"{param} must stay in derouter.types.utils.all_derouter_params - "
    ...
    async def test_adaptive_router_alias_overrides_survive_reload(self):
        """Alias derouter_params are read fresh from self.model_list at request
...
class TestRouterPreRoutingSharedAliasName:
    """
    Regression tests for https://github.com/Delify-Solutions/derouter/issues/36619.
    ...
    def _tier_entry() -> dict:
        return {
            ...
            "derouter_params": {"model": "gemini/gemini-3.6-flash", "api_key": "sk-tier"},
        }
    ...
    async def test_connection_params_on_the_marker_itself_are_not_forwarded(self):
        ...
        marker_with_connection_params = {
            "model_name": "smart",
            "derouter_params": {
                **self._marker_entry()["derouter_params"],
                "api_key": "sk-marker",
    ...
    async def _routed_call_kwargs(router: Router, prompt: str = "hi", **request_params) -> dict:
        mock_acompletion = AsyncMock(return_value=derouter.ModelResponse(choices=[{"message": {"content": "hi"}}]))
        with patch.object(derouter, "acompletion", mock_acompletion):
    ...
    @pytest.mark.asyncio
    async def test_complexity_tier_derouter_params_beat_the_tier_deployments_own_params(self):
        """Per-tier `derouter_params` are deliberate overrides, not forwarded marker params:
        they keep winning over the tier deployment's own value."""
        marker = self._region_marker_entry()
        marker["derouter_params"]["complexity_router_config"] = {
            "tiers": {
                tier: {"model_name": "bedrock-tier", "derouter_params": {"aws_region_name": "us-west-2"}}
    ...
    async def test_a_markers_explicit_flag_beats_the_tiers_pydantic_default(self):
        """Every deployment materializes `DeRouter_Params` defaults such as
        `merge_reasoning_content_in_choices: False`; a default is not the tier setting its own value."""
        marker = self._region_marker_entry()
        marker["derouter_params"]["merge_reasoning_content_in_choices"] = True
    ...
    async def test_sibling_request_sharing_the_metadata_dict_cannot_unpin_the_tier(self):
        ...
        async def record(**kwargs):
            ...
            return derouter.ModelResponse(choices=[{"message": {"content": "hi"}}])

        with (
            patch.object(router, "async_get_healthy_deployments", yield_between_routing_and_dispatch),
            patch.object(derouter, "acompletion", AsyncMock(side_effect=record)),
        ):
    ...
    def test_forwarded_alias_marker_keys_the_deployment_sets(self):
        deployment = {"derouter_params": {"model": "bedrock/x", "aws_region_name": "us-east-1", "timeout": None}}
    ...
    def test_deployment_sets_derouter_param(self):
        params = {"aws_region_name": "us-east-1", "timeout": None, "use_derouter_proxy": False, "custom_flag": False}

        assert Router._deployment_sets_derouter_param(params, "aws_region_name") is True
        assert Router._deployment_sets_derouter_param(params, "timeout") is False
        assert Router._deployment_sets_derouter_param(params, "missing") is False
        assert Router._deployment_sets_derouter_param(params, "use_derouter_proxy") is False
        assert Router._deployment_sets_derouter_param({"use_derouter_proxy": True}, "use_derouter_proxy") is True
        assert Router._deployment_sets_derouter_param(params, "custom_flag") is True
...
class TestAdaptiveSoftFloors:
    ...
    def test_cold_start_randomly_samples_unobserved_classified_tier_models(self, adaptive_router_instance):
        cr = ComplexityRouter(
            model_name="hybrid",
            derouter_router_instance=adaptive_router_instance,
            complexity_router_config={
                "adaptive": True,
                "tiers": {
    ...
    def test_soft_floor_prefers_home_tier_when_posteriors_equal(self, adaptive_router_instance, hybrid_config):
        from derouter.router_strategy.adaptive_router.bandit import BanditCell
        from derouter.types.router import RequestType
        ...
        with patch(
            "derouter.router_strategy.adaptive_router.bandit.thompson_sample",
            return_value=0.5,
        ):
...
def _make_embedding_response(vectors: List[List[float]]) -> "derouter.EmbeddingResponse":
    return derouter.EmbeddingResponse(
        model="fake-embed",
...
class TestSemanticKeywordTierRules:
    ...
    async def test_semantic_match_routes_to_rule_tier(self, basic_config):
        ...
        router = ComplexityRouter(
            model_name="test-router",
            derouter_router_instance=fake_router,
            complexity_router_config=config,
        )
        result = await router.async_pre_routing_hook(
            model="test-model",
    ...
    async def test_semantic_embedding_call_carries_caller_metadata(self, basic_config):
        """The query embedding call must carry the caller's metadata/derouter_metadata
        ...
        caller_derouter_metadata = {"user_api_key": "hash-abc"}
        result = await router.async_pre_routing_hook(
            model="test-model",
            request_kwargs={"metadata": caller_metadata, "derouter_metadata": caller_derouter_metadata},
        ...
        assert fake_router.async_embedding_kwargs[0]["derouter_metadata"] == {**caller_derouter_metadata, **origin}
    ...
    async def test_semantic_embedding_call_strips_budget_reservation(self, basic_config):
        ...
        await router.async_pre_routing_hook(
            model="test-model",
            request_kwargs={"metadata": caller_metadata, "derouter_metadata": dict(caller_metadata)},
            messages=[{"role": "user", "content": "roll out my k8s cluster"}],
        )
        assert fake_router.async_embedding_kwargs, "expected an embedding call for the prompt"
        ...
        assert fake_router.async_embedding_kwargs[0]["derouter_metadata"] == expected
...
class _StubEncoder:
    """Minimal stand-in for DeRouterRouterEncoder.aencode_queries, capturing the kwargs it was called with."""
...
class TestRoutingDecisionCauseLogging:
    ...
    def router_log_capture(self, caplog):
        ...
        caplog.set_level(logging.INFO, logger="DeRouter Router")
...
class TestSessionAffinity:
    ...
    async def test_adaptive_pinned_turn_still_stamps_chosen_model_metadata(self, mock_router_instance):
        ...
        mock_router_instance.model_list = [
            {
                "model_name": "cheap",
                "derouter_params": {"model": "openai/gpt-4o-mini", "input_cost_per_token": 0.0},
                "model_info": {},
            },
        ]
...
class TestClassificationMode:
    ...
    async def test_escalation_keyword_turn_is_a_new_ask(self, mock_router_instance, user_turn_config):
        ...
        second = await router.async_pre_routing_hook(
            ...
            messages=[self.SIMPLE_ASK, self.ASSISTANT_ANSWER, {"role": "user", "content": "DEROUTER ESCALATE"}],
        )
...
class TestRoutingPlugins:
    ...
    https://github.com/Delify-Solutions/derouter/discussions/32168"""
...
class TestClassifierPlugin:
    ...
    async def test_plugin_reads_caller_identity_from_request_metadata(self, mock_router_instance):
        ...
        basic = await router.aclassify(
            "hi", request_kwargs={"derouter_metadata": {"user_api_key_team_id": "team-basic"}}
        )
...
class TestEscalationKeywords:
    ...
    def test_default_escalation_keyword(self, complexity_router):
        assert complexity_router.escalation_keywords == ("DEROUTER ESCALATE",)

    def test_escalation_triggered_is_case_sensitive(self, complexity_router):
        assert complexity_router._matched_escalation_keyword("please DEROUTER ESCALATE now") == "DEROUTER ESCALATE"
        assert complexity_router._matched_escalation_keyword("please derouter escalate now") is None
    ...
    async def test_escalation_bumps_classified_tier(self, mock_router_instance, basic_config):
        ...
        escalated = await router.async_pre_routing_hook(
            model="test-model",
            request_kwargs={},
            messages=[{"role": "user", "content": "DEROUTER ESCALATE Hello there!"}],
        )
    ...
    async def test_escalation_caps_at_highest_tier(self, mock_router_instance, basic_config):
        ...
        result = await router.async_pre_routing_hook(
            ...
            messages=[
                {
                    "role": "user",
                    "content": "DEROUTER ESCALATE Let's think step by step and reason through this carefully.",
                }
            ],
        )
    ...
    async def test_escalation_bumps_keyword_tier_override(self, mock_router_instance, basic_config):
        ...
        escalated = await router.async_pre_routing_hook(
            model="test-model",
            request_kwargs={},
            messages=[{"role": "user", "content": "DEROUTER ESCALATE a billing question"}],
        )
    ...
    async def test_escalation_overrides_session_pin_and_persists(self, mock_router_instance, basic_config):
        ...
        with patch.object(router, "aclassify", wraps=router.aclassify) as spy_aclassify:
            escalated = await router.async_pre_routing_hook(
                model="test-model",
                request_kwargs=request_kwargs,
                messages=[{"role": "user", "content": "DEROUTER ESCALATE"}],
            )
            spy_aclassify.assert_not_called()
        ...
        again = await router.async_pre_routing_hook(
            model="test-model",
            request_kwargs=request_kwargs,
            messages=[{"role": "user", "content": "DEROUTER ESCALATE still not good"}],
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                [{"type": "text", "text": "<system-reminder>context: DEROUTER ESCALATE</system-reminder>"}],
    ...
    async def test_plumbing_turns_do_not_re_escalate_a_pinned_session(
        ...
        conversation = [
            {"role": "user", "content": "DEROUTER ESCALATE"},
            {"role": "assistant", "content": "working on it"},
    ...
    async def test_plumbing_turns_do_not_escalate_without_session_affinity(self, mock_router_instance, basic_config):
        ...
        mid_loop = await router.async_pre_routing_hook(
            ...
            request_kwargs={},
            messages=[
                {"role": "user", "content": "DEROUTER ESCALATE Hello there!"},
                {"role": "assistant", "content": "working on it"},
    ...
    def test_blank_escalation_keywords_are_stripped(self):
        ...
        assert ComplexityRouterConfig(
            tiers={"SIMPLE": "gpt-4o-mini", "MEDIUM": "gpt-4o"},
            escalation_keywords=["  DEROUTER ESCALATE  ", ""],
        ).escalation_keywords == ["DEROUTER ESCALATE"]
    ...
    async def test_session_escalation_at_ceiling_keeps_multi_model_pin(self, mock_router_instance):
        ...
        result = await router.async_pre_routing_hook(
            ...
            messages=[{"role": "user", "content": "DEROUTER ESCALATE do better"}],
        )
...
class TestStallEscalation:
    ...
    async def test_stall_escalation_stacks_with_keyword_escalation(self, mock_router_instance, basic_config):
        ...
        messages = [*_stalled_tool_history(), {"role": "user", "content": "DEROUTER ESCALATE Hello there!"}]
        result = await router.async_pre_routing_hook(model="test-model", request_kwargs={}, messages=messages)
...
class TestRoutingDecisionSurvivesToSpendLogOnEveryMetadataShape:
    ...
    batch-style routes carry it in `derouter_metadata` (so the provider's own `metadata`
    field stays untouched), and a caller may supply either, both, or neither. Logging
    snapshots `derouter_metadata` by value (`function_setup`, derouter/utils.py), so a
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param({"derouter_metadata": {}}, "derouter_metadata", id="derouter-metadata-seeded"),
            pytest.param(
                {"derouter_metadata": {"caller_tag": "x"}}, "derouter_metadata", id="derouter-metadata-with-caller-value"
            ),
            pytest.param(
                {"derouter_metadata": {}, "metadata": {"user_id": "end-user-1"}},
                "derouter_metadata",
                id="both-buckets",
            ),
        ],
    ...
    async def test_decision_reaches_the_spend_log_payload(self, request_kwargs, expected_bucket):
        ...
        if expected_bucket == "derouter_metadata" and isinstance(request_kwargs.get("metadata"), dict):
        ...
        # Mirror function_setup: it copies `derouter_metadata` by value into
        # derouter_params AFTER the router hook has run, so the copy must carry
        # the decision. Reading the stash any earlier would lose it.
        derouter_params: Dict = {}
        if "metadata" in request_kwargs:
            derouter_params["metadata"] = request_kwargs["metadata"]
        if isinstance(request_kwargs.get("derouter_metadata"), dict):
            derouter_params["derouter_metadata"] = request_kwargs["derouter_metadata"].copy()

        payload = get_logging_payload(
            kwargs={"model": "gpt-4o-mini", "derouter_params": derouter_params},
            response_obj=derouter.ModelResponse(id="chatcmpl-shape", choices=[], usage=derouter.Usage()),
...
class TestRoutingDecisionIsPerAttempt:
    ...
    @pytest.mark.parametrize("seed, bucket", [({}, "metadata"), ({"derouter_metadata": {}}, "derouter_metadata")])
    @pytest.mark.asyncio
    ...
    async def test_non_dict_bucket_is_replaced_not_skipped(self, unusable_bucket):
        """A caller can send `derouter_metadata` as a non-dict (unparsed string, null).
        ...
        router = Router(model_list=self.MODEL_LIST)
        request_kwargs: Dict = {"derouter_metadata": unusable_bucket}
        ...
        assert response is not None
        bucket = request_kwargs["derouter_metadata"]
...
class TestRecordRoutingDecision:
    ...
    def test_none_clears_a_previous_decision_from_both_buckets(self):
        request_kwargs: Dict = {
            ...
            "derouter_metadata": {"routing_decision": self.DECISION},
        }
        Router._record_routing_decision(request_kwargs=request_kwargs, routing_decision=None)
        assert "routing_decision" not in request_kwargs["metadata"]
        assert "routing_decision" not in request_kwargs["derouter_metadata"]
    ...
    def test_clearing_the_decision_takes_the_savings_facts_with_it(self):
        ...
        request_kwargs: Dict = {"derouter_metadata": {"routing_decision": decision}}
        Router._record_routing_decision(request_kwargs=request_kwargs, routing_decision=None)
        assert request_kwargs["derouter_metadata"] == {}
...
class TestEscalationIsRecordedConsistently:
    ...
    async def test_scorer_path_at_ceiling_keeps_the_keyword_and_reports_no_bump(self, mock_router_instance):
        ...
        response = await router.async_pre_routing_hook(
            model="test-router",
            request_kwargs={},
            messages=[{"role": "user", "content": "DEROUTER ESCALATE already at the top"}],
        )
        decision = response.routing_decision
        assert decision["tier"] == "REASONING"
        assert decision["escalation_keyword"] == "DEROUTER ESCALATE"
    ...
    async def test_scorer_path_below_ceiling_reports_the_bump(self, complexity_router):
        response = await complexity_router.async_pre_routing_hook(
            model="test-router",
            request_kwargs={},
            messages=[{"role": "user", "content": "DEROUTER ESCALATE what is 2+2"}],
        )
        decision = response.routing_decision
        assert decision["escalation_keyword"] == "DEROUTER ESCALATE"
    ...
    async def test_session_pin_at_ceiling_still_records_the_ask(self, mock_router_instance):
        ...
        response = await router.async_pre_routing_hook(
            model="test-router",
            request_kwargs=request_kwargs,
            messages=[{"role": "user", "content": "DEROUTER ESCALATE go higher"}],
        )
        decision = response.routing_decision
        assert decision["routed_model"] == "o1-preview"
        assert decision["cause"] == "session_affinity_pin"
...
class TestRedactedLoggingDropsPromptText:
    ...
    MESSAGES = [{"role": "user", "content": "DEROUTER ESCALATE please deploy to k8s now"}]
    ...
    async def test_redaction_drops_quoted_prompt_text_but_keeps_the_explanation(self, monkeypatch):
        # The usual deployment shape: `derouter_settings: turn_off_message_logging: true`
    ...
    def test_only_verbatim_prompt_fields_are_classified_as_prompt_text(self, monkeypatch):
        ...
        full = {
            ...
            "escalated": True,
            "tier_derouter_params": {"reasoning_effort": "xhigh"},
            "signals": ["code (python)"],
            "matched_keyword": "deploy to k8s",
            "escalation_keyword": "DEROUTER ESCALATE",
        }
        ...
        assert kept["tier_derouter_params"] == {"reasoning_effort": "xhigh"}

    @pytest.mark.asyncio
    async def test_redaction_via_request_header_is_honored(self):
        request_kwargs: Dict = {"metadata": {"headers": {"x-derouter-enable-message-redaction": True}}}
...
def test_every_routing_decision_field_is_classified():
    ...
    assert declared == classified, (
        "classify new routing-decision fields in derouter/types/utils.py: "
...
class TestContextAwareClassifier:
    ...
    def test_current_ask_is_the_text_a_human_wrote(self, messages, expected_ask):
        ...
        """
        from derouter.router_strategy.complexity_router.complexity_router import _extract_current_ask_and_system_prompt
    ...
    def test_a_block_nested_inside_another_pairs_block_does_not_leak(self):
        ...
        """
        from derouter.router_strategy.complexity_router.complexity_router import _strip_reminder_blocks
    ...
    def test_truncate_keeps_the_end_of_an_over_long_turn(self, text, limit, expected):
        ...
        """
        from derouter.router_strategy.complexity_router.complexity_router import _truncate
    ...
    def test_truncate_holds_its_length_budget(self):
        ...
        """
        from derouter.router_strategy.complexity_router.complexity_router import _TRUNCATION_MARKER, _truncate
    ...
    def test_clipped_prior_turn_still_carries_the_ask_it_closes_on(self):
        ...
        """
        from derouter.router_strategy.complexity_router.complexity_router import _extract_prior_turns
    ...
    async def test_a_long_turn_reaches_the_classifier_whole_by_default(
        ...
        """
        from derouter.router_strategy.complexity_router.complexity_router import _TRUNCATION_MARKER
    ...
    async def test_assistant_text_cannot_choose_the_tier_on_its_own(
        ...
        response = await router.async_pre_routing_hook(
            ...
            messages=[
                {"role": "user", "content": "hello"},
                {"role": "assistant", "content": "DEROUTER ESCALATE, and next we prove the theorem"},
                *trailing_turns,
            ],
        )
        ...
        user_payload = mock_router_instance.acompletion.call_args.kwargs["messages"][1]["content"]
        assert "DEROUTER ESCALATE" in user_payload
...
class TestClientHousekeepingCalls:
    ...
    async def test_an_escalation_keyword_beats_the_cheapest_tier(self, mock_router_instance, llm_classifier_config):
        ...
        result = await router.async_pre_routing_hook(
            model="test-model",
            request_kwargs={},
            messages=[{"role": "user", "content": f"DEROUTER ESCALATE {TITLE_ASK}"}],
        )
    ...
    def _adaptive_router(self, tier_distance_penalty: float, plan_mode_min_tier: str | None = None) -> ComplexityRouter:
        ...
        adaptive_instance.model_list = [
            {
                "model_name": "cheap",
                "derouter_params": {"model": "openai/gpt-4o-mini", "input_cost_per_token": 0.00000015},
                "model_info": {"adaptive_router_preferences": {"quality_tier": 1, "strengths": []}},
            },
            {
                "model_name": "premium",
                "derouter_params": {"model": "openai/gpt-4o", "input_cost_per_token": 0.000005},
                "model_info": {"adaptive_router_preferences": {"quality_tier": 3, "strengths": []}},
            },
        ]
        ...
        router = ComplexityRouter(
            model_name="hybrid",
            derouter_router_instance=adaptive_instance,
            complexity_router_config={
                "adaptive": True,
...
class TestClassifierTrustBoundary:
    ...
    async def test_caller_text_never_reaches_the_classifier_system_role(self, mock_router_instance):
        ...
        """
        from derouter.router_strategy.complexity_router.complexity_router import classification_system_prompt
...
class TestConversationShapeDiscriminator:
    ...
    def test_an_agent_loop_on_one_human_ask_is_not_a_first_turn(self, history):
        ...
        from derouter.router_strategy.complexity_router.complexity_router import _conversation_is_continuing
    ...
    async def test_the_shape_travels_on_every_pre_routing_response(self):
        ...
        from derouter.router_strategy.complexity_router import complexity_router as module
...
class TestCustomClassifierSystemPrompt:
    ...
    def _built_in_sections_router(**config_patch) -> ComplexityRouter:
        ...
        return ComplexityRouter(
            model_name="test-complexity-router", derouter_router_instance=MagicMock(), complexity_router_config=config
        )
...
class TestSavingsBaselineOnDecision:
    ...
    def _router_with_tiers(tiers: dict, **kwargs) -> ComplexityRouter:
        parent = Router(
            model_list=[
                {"model_name": "cheap", "derouter_params": {"model": "anthropic/claude-haiku-4-5"}},
                {"model_name": "mid", "derouter_params": {"model": "anthropic/claude-sonnet-5"}},
                {"model_name": "top", "derouter_params": {"model": "anthropic/claude-fable-5"}},
            ]
        )
        return ComplexityRouter(
            model_name="savings-router",
            derouter_router_instance=parent,
    ...
    def test_a_leftover_proxy_wide_baseline_setting_does_not_disable_derivation(self, monkeypatch):
        """The proxy config loader setattrs unknown derouter_settings keys, so a stale
        autorouter_savings_baseline_model key must stay inert."""
        monkeypatch.setattr(derouter, "autorouter_savings_baseline_model", "claude-opus-5", raising=False)
    ...
    def test_the_decision_record_carries_the_derived_baseline_and_its_deployment(self):
        ...
        router = self._router_with_tiers({"SIMPLE": "cheap", "REASONING": "top"})
        expected_id = router.derouter_router_instance.get_model_list(model_name="top")[0]["model_info"]["id"]
    ...
    def test_the_routing_test_preview_builds_its_router_without_derivation(self):
        ...
        from derouter.proxy.management_endpoints import auto_router_endpoints
...
class TestSavingsBaselinePinnedPerInstance:
    ...
    def _router_and_parent() -> tuple[ComplexityRouter, Router]:
        parent = Router(
            model_list=[
                {"model_name": "cheap", "derouter_params": {"model": "anthropic/claude-haiku-4-5"}},
                {"model_name": "top", "derouter_params": {"model": "anthropic/claude-sonnet-5"}},
            ]
        )
        router = ComplexityRouter(
            model_name="savings-router",
...
class TestTierDefinitions:
    ...
    async def test_escalation_keyword_is_inert_on_a_custom_tier_set(self, custom_tier_router, mock_router_instance):
        """DEROUTER ESCALATE bumps along the built-in ladder, which a custom set does not define:
        the default keyword must neither escalate nor appear in the decision."""
        mock_router_instance.acompletion = AsyncMock(return_value=_llm_response('{"tier": "SIMPLE"}'))
        response = await custom_tier_router.async_pre_routing_hook(
            model="custom-tier-router",
            request_kwargs={},
            messages=[{"role": "user", "content": "DEROUTER ESCALATE say hi"}],
        )
    ...
    def test_router_init_derives_default_model_from_fallback_tier(self):
        ...
        router = Router(
            model_list=[
                {"model_name": "gpt-4o-mini", "derouter_params": {"model": "openai/gpt-4o-mini", "mock_response": "hi"}},
                {
                    "model_name": "claude-sonnet-4-20250514",
                    "derouter_params": {"model": "anthropic/claude-sonnet-4-20250514", "mock_response": "hi"},
                },
                {"model_name": "o1-preview", "derouter_params": {"model": "openai/o1-preview", "mock_response": "hi"}},
                {
                    "model_name": "custom-tier-router",
                    "derouter_params": {
                        "model": "auto_router/complexity_router",
...
class TestPlanModeTierFloor:
    ...
    async def test_floored_classifier_failure_routes_floor_not_default_model(self, mock_router_instance, basic_config):
        ...
        from derouter.router_strategy.complexity_router.complexity_router import ClassificationOutcome
...
def test_tier_model_params_are_normalized_without_changing_model_pools():
    config = ComplexityRouterConfig(
        tiers={
            "SIMPLE": "mini",
            "REASONING": [
                {"model_name": "opus", "derouter_params": {"reasoning_effort": "xhigh"}},
                "abc",
            ],
        }
    )
    ...
    assert config.tier_model_configs["REASONING"][0].derouter_params == {"reasoning_effort": "xhigh"}
    rebuilt = ComplexityRouterConfig.model_validate(config.model_dump())
    assert rebuilt.tier_model_configs["REASONING"][0].derouter_params == {"reasoning_effort": "xhigh"}


def test_tier_model_params_accept_a_single_object():
    config = ComplexityRouterConfig(
        tiers={"REASONING": {"model_name": "opus", "derouter_params": {"thinking": {"type": "enabled"}}}}
    )
...
@pytest.mark.parametrize(
    "tiers",
    [
        {"REASONING": [{"derouter_params": {"reasoning_effort": "xhigh"}}]},
    ],
)
...
def test_tier_model_params_reject_router_settings(misplaced):
    """A tier entry's derouter_params are request params for that deployment: the pre-routing hook
    ...
    with pytest.raises(ValidationError, match="complexity_router_config settings"):
        ComplexityRouterConfig(tiers={"REASONING": [{"model_name": "opus", "derouter_params": misplaced}]})
...
def test_tier_model_params_still_accept_real_request_params(params):
    ...
    config = ComplexityRouterConfig(tiers={"REASONING": [{"model_name": "opus", "derouter_params": params}]})

    assert config.tier_model_configs["REASONING"][0].derouter_params == params
    ...
    with pytest.raises(ValidationError, match="duplicate model_name"):
        ComplexityRouterConfig(
            tiers={
                "REASONING": [
                    {"model_name": "opus", "derouter_params": {"reasoning_effort": "xhigh"}},
                    {"model_name": "opus", "derouter_params": {"reasoning_effort": "low"}},
                ]
            }
        )
...
def test_tier_model_params_are_used_by_pools_and_savings_baseline(mock_router_instance):
    router = ComplexityRouter(
        ...
        complexity_router_config={
            "tiers": {
                "SIMPLE": "mini",
                "REASONING": [{"model_name": "opus", "derouter_params": {"reasoning_effort": "xhigh"}}, "abc"],
            }
        },
    )
    ...
    assert router._derouter_params_for_model(ComplexityTier.REASONING, "opus") == {"reasoning_effort": "xhigh"}
...
async def test_tier_model_params_reach_the_hook_response_and_override_client_values(mock_router_instance):
    router = ComplexityRouter(
        ...
        complexity_router_config={
            "tiers": {
                "REASONING": {
                    "model_name": "opus",
                    "derouter_params": {"reasoning_effort": "xhigh", "max_tokens": 512},
                }
            },
    ...
    assert response is not None
    assert response.derouter_params == {"reasoning_effort": "xhigh", "max_tokens": 512}
    assert response.routing_decision is not None
    assert response.routing_decision["tier_derouter_params"] == response.derouter_params
...
async def test_tier_params_mask_credentials_in_routing_decision(route, mock_router_instance):
    ...
    config = {
        "tiers": {tier.value: {"model_name": "opus", "derouter_params": params} for tier in TIER_SEVERITY_ORDER},
    ...
    assert response is not None
    assert response.derouter_params == params
    assert response.routing_decision is not None
    assert response.routing_decision["tier_derouter_params"] == {
        "reasoning_effort": "xhigh",
        "api_key": "secr*******-key",
    }
...
async def test_session_pin_outside_tiers_does_not_inherit_medium_params(mock_router_instance):
    ...
    router = ComplexityRouter(
        ...
        complexity_router_config={
            "tiers": {
                "SIMPLE": "mini",
                "MEDIUM": {"model_name": "medium", "derouter_params": {"reasoning_effort": "low"}},
            },
            "session_affinity": True,
    ...
    assert response.derouter_params == {}
...
async def test_session_pin_uses_recorded_tier_when_model_is_in_multiple_tiers(mock_router_instance):
    ...
    router = ComplexityRouter(
        ...
        complexity_router_config={
            "tiers": {
                "SIMPLE": {"model_name": "shared", "derouter_params": {"reasoning_effort": "low"}},
                "REASONING": {"model_name": "shared", "derouter_params": {"reasoning_effort": "xhigh"}},
            },
            "session_affinity": True,
        },
    )
    ...
    assert response is not None
    assert response.derouter_params == {"reasoning_effort": "low"}
    assert response.routing_decision is not None
...
def _windowed_router(*deployments: tuple) -> Router:
    ...
    return Router(
        model_list=[
            {
                "model_name": group,
                "derouter_params": {"model": provider_model, "mock_response": "ok"},
...
class TestContextWindowEscalation:
    ...
    async def test_an_oversized_simple_prompt_escalates_to_the_lowest_tier_that_fits(self):
        ...
        router = ComplexityRouter(
            model_name="test-router",
            derouter_router_instance=_windowed_router(_SMALL, _BIG),
    ...
    async def test_the_pick_prefers_a_fitting_group_inside_the_decided_tier(self):
        ...
        router = ComplexityRouter(
            model_name="test-router",
            derouter_router_instance=_windowed_router(_SMALL, ("mid-model", "openai/gpt-4o-mini", 200000), _BIG),
                ...
                model_list=[
                    {
                        ...
                        "derouter_params": {"model": "openai/gpt-3.5-turbo", "mock_response": "ok"},
                        "model_info": {"max_input_tokens": 16385},
                    },
                    {
                        "model_name": "mixed-pool",
                        "derouter_params": {"model": "openai/gpt-4o-mini", "mock_response": "ok"},
    ...
    async def test_unknown_windows_are_never_acted_on(self, deployments, tiers, expected_model):
        ...
        router = ComplexityRouter(
            model_name="test-router",
            derouter_router_instance=_windowed_router(*deployments),
            complexity_router_config={"tiers": tiers},
        )
    ...
    async def test_the_gate_never_resolves_an_authenticating_provider(self, monkeypatch, tmp_path):
        ...
        router = ComplexityRouter(
            ...
            derouter_router_instance=Router(
                model_list=[
                    {"model_name": "cop-pool", "derouter_params": {"model": "github_copilot/gpt-4o"}},
                    {
                        "model_name": "big-model",
                        "derouter_params": {"model": "openai/gpt-4o-mini", "mock_response": "ok"},
                        "model_info": {"max_input_tokens": 200000},
...
class TestModalityRouting:
    ...
    def _router(mock_router_instance, config, vision_by_model):
        ...
        def get_model_list(model_name=None):
            ...
            return [
                {
                    "model_name": model_name,
                    "derouter_params": {"model": f"openai/unmapped-{model_name}"},
    ...
    async def test_placements_across_decision_paths(self, mock_router_instance, path, expected_model, expected_cause):
        ...
        if path == "keyword_tier_escalates":
            ...
            if path == "pin_replacement_gated":
                ...
                messages = [
                    {"role": "user", "content": [{"type": "text", "text": "DEROUTER ESCALATE describe this"}, IMG_PART]}
                ]
    ...
    async def test_plan_floored_decision_never_falls_to_default_model(self, mock_router_instance):
        ...
        router = self._router(mock_router_instance, config, vision)
        with pytest.raises(derouter.BadRequestError, match="no model"):
            await router.async_pre_routing_hook(
                model="m",
    ...
    async def test_mixed_deployment_group_is_treated_text_only(self, mock_router_instance):
        def get_model_list(model_name=None):
            ...
            return [
                {
                    "model_name": model_name,
                    "derouter_params": {"model": f"openai/unmapped-{model_name}-{i}"},
    ...
    def test_modality_escalation_is_never_pinnable(self):
        from derouter.router_strategy.complexity_router.complexity_router import _decision_is_pinnable
...
class TestTierHealthFailover:
    ...
    def _router(
        ...
        async def get_healthy_deployments(
            ...
            if not ids_by_model.get(model):
                raise derouter_module.BadRequestError(
    ...
    async def test_dead_pinned_group_fails_over_to_live_peer_and_reports_the_displacement(self, mock_router_instance):
        ...
        await router.derouter_router_instance.cache.async_set_cache(
    ...
    async def test_a_failed_over_turn_is_never_pinned(self, mock_router_instance):
        ...
        async def pin_after_session(turn: int):
            ...
            return await router.derouter_router_instance.cache.async_get_cache(
    ...
    async def test_an_unpinnable_displaced_cause_stays_unpinnable_after_failover(self, mock_router_instance):
        ...
        stored = await router.derouter_router_instance.cache.async_get_cache(
            key=router._get_session_affinity_cache_key(session_id, {})
        )
    ...
    async def test_failover_will_not_pick_a_peer_that_cannot_hold_the_prompt(self):
        ...
        router = ComplexityRouter(
            ...
            derouter_router_instance=router_instance,
            complexity_router_config={
    ...
    async def test_the_eligibility_probe_never_mutates_the_caller_request_kwargs(self, mock_router_instance):
        ...
        assert all(probed is not request_kwargs for probed in router.derouter_router_instance.probed_kwargs)
    ...
    async def test_a_peer_whose_every_deployment_is_over_its_rpm_is_not_a_failover_target(self, mock_router_instance):
        ...
        from derouter.types.router import RouterRateLimitErrorBasic
    ...
    async def test_the_probe_forwards_input_so_window_checks_run_on_input_only_surfaces(self, mock_router_instance):
        ...
        assert any(
            ...
            for _, probed_input in router.derouter_router_instance.probed_prompts
...
class TestClassifierVision:
    ...
    def _router(mock_router_instance, *, vision, classifier_declares_vision=True, classifier_type="llm", **extra):
        def get_model_list(model_name=None):
            if model_name != "clf":
                return [{"model_name": model_name, "derouter_params": {"model": "openai/gpt-4o"}}]
            declared = classifier_declares_vision
            return [
                {
                    "model_name": "clf",
                    "derouter_params": {"model": "openai/unmapped-classifier"},
                    "model_info": {} if declared is None else {"supports_vision": declared},
                }
            ]
    ...
    async def test_deployment_model_info_enables_a_classifier_the_cost_map_does_not_describe(
        ...
        `_router` gives every deployment an `openai/unmapped-*` derouter_params model, so nothing in
...
class TestMaxTokensFromTierModel:
    ...
    SMALL: Final = {
        "model_name": "small",
        "derouter_params": {"model": "anthropic/claude-haiku-4-5", "api_key": "k"},
        "model_info": {"max_output_tokens": 8192},
    }

    @staticmethod
    def _router(
        tier_derouter_params: dict | None = None,
        max_tokens_from_tier_model: bool | None = None,
        simple_deployments: list[dict] | None = None,
        extra_config: dict | None = None,
    ) -> Router:
        simple_tier: dict = {"model_name": "small"}
        if tier_derouter_params:
            simple_tier["derouter_params"] = tier_derouter_params
        config: dict = {
        ...
        return Router(
            model_list=[
                {
                    "model_name": "smart-router",
                    "derouter_params": {"model": "auto_router/complexity_router", "complexity_router_config": config},
                },
                *(simple_deployments or [TestMaxTokensFromTierModel.SMALL]),
                {
                    "model_name": "big",
                    "derouter_params": {"model": "anthropic/claude-sonnet-5", "api_key": "k"},
    ...
    async def _routed(router: Router, prompt: str = "hi", **request_kwargs) -> dict:
        ...
        return {"model": deployment["derouter_params"]["model"], **request_kwargs}
    ...
    async def _routed_responses(router: Router, prompt: str = "hi", **request_kwargs) -> dict:
        ...
        return {"model": deployment["derouter_params"]["model"], **routed}
    ...
    async def test_operators_own_tier_ceiling_wins_under_the_surface_name(self, tier_params, responses_call):
        router = self._router(tier_derouter_params=tier_params)
        if responses_call:
    ...
    async def test_a_tier_model_with_an_unknown_ceiling_keeps_the_client_value(self):
        unmapped: dict = {"model_name": "small", "derouter_params": {"model": "openai/not-in-any-map", "api_key": "k"}}
    ...
    async def test_a_multi_deployment_tier_model_uses_its_smallest_ceiling(self):
        smaller: dict = {
            **self.SMALL,
            "derouter_params": {**self.SMALL["derouter_params"], "api_key": "k2"},
    ...
    async def test_ceiling_falls_back_to_the_cost_map(self, monkeypatch):
        monkeypatch.setitem(
            ...
            {"derouter_provider": "openai", "mode": "chat", "max_output_tokens": 4242, "max_input_tokens": 100000},
        )
        mapped_only: dict = {
            "model_name": "small",
            "derouter_params": {"model": "openai/auto-cap-probe-model", "api_key": "k"},
        }
    ...
    async def test_the_default_fallback_exit_carries_the_ceiling(self):
        ...
        assert (deployment["derouter_params"]["model"], routed["max_tokens"]) == ("anthropic/claude-sonnet-5", 64000)
    ...
    async def test_a_default_model_landing_with_no_tier_still_gets_its_ceiling(self):
        strategy = ComplexityRouter(
            model_name="smart-router",
            derouter_router_instance=self._router(),
            complexity_router_config={"tiers": {"SIMPLE": "small"}, "default_model": "big"},
        )

        assert dict(strategy._derouter_params_for_model(None, "big")) == {"max_tokens": 64000}
    ...
    async def test_a_fallback_into_a_plain_group_gets_the_callers_ceiling_back(self):
        ...
        plain: dict = {
            "model_name": "plain",
            "derouter_params": {"model": "anthropic/claude-haiku-4-5", "api_key": "k", "mock_response": "ok"},
        }
        router = Router(
            model_list=[
                {
    ...
    async def test_the_pass_through_routing_entry_point_pins_and_restores_the_same_way(self):
        pass_through: dict = {**self.SMALL["derouter_params"], "use_in_pass_through": True}
        small: dict = {**self.SMALL, "derouter_params": pass_through}
        plain: dict = {**small, "model_name": "plain"}
        router = self._router(simple_deployments=[small, plain])
        for deployment in router.model_list:
            deployment["derouter_params"]["use_in_pass_through"] = True
        routed: dict = {"max_tokens": 8192}
        ...
        assert (deployment["derouter_params"]["model"], pinned, routed["max_tokens"]) == (
    ...
    async def test_pinning_stamps_the_callers_carriers_once(self):
        ...
        first = router._pin_tier_params_onto_request(
            model="big", tier_derouter_params={"max_tokens": 64000}, request_kwargs=request_kwargs, responses_call=False
        )
        second = router._pin_tier_params_onto_request(
            model="big", tier_derouter_params={"max_tokens": 32000}, request_kwargs=request_kwargs, responses_call=False
        )
        none = router._pin_tier_params_onto_request(
            model="big", tier_derouter_params=None, request_kwargs=request_kwargs, responses_call=False
        )
