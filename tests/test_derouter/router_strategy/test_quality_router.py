...
Covers:
- Tier index construction from `model_info.derouter_routing_preferences`.
...
from derouter.router_strategy.quality_router.config import (
    DEFAULT_COMPLEXITY_TO_QUALITY,
)
from derouter.router_strategy.quality_router.quality_router import QualityRouter
...
class TestRoutingPreferencesValidation:
    def test_invalid_quality_tier_type_raises_clear_error(self):
        ...
        ml = [
            {
                ...
                "model_info": {
                    "id": "id-haiku",
                    "derouter_routing_preferences": {"quality_tier": "not-an-int"},
                },
            }
        ]
        router = MagicMock()
        ...
        with pytest.raises(ValueError, match="invalid derouter_routing_preferences"):
            _ = qr._tier_to_models
            ...
            model_list=[
                {
                    ...
                    "model_info": {"derouter_routing_preferences": {"quality_tier": 1}},
                },
                {
                    "model_name": "my-qr",
                    "derouter_params": {
...
# ─── Routing-decision metadata (powers x-derouter-quality-router-* headers) ──
...
# ─── Router.set_response_headers lifts decision into x-derouter-quality-* ────


class TestSetResponseHeadersLiftsDecision:
    """
    Verify the Router.set_response_headers helper turns a stashed quality-router
    decision into x-derouter-quality-router-* headers on the response.
    """
    ...
    async def test_lifts_decision_into_additional_headers(self):
        ...
        assert headers["x-derouter-quality-router-model"] == "smart-coder"
        assert headers["x-derouter-quality-router-tier"] == "3"
        assert headers["x-derouter-quality-router-via"] == "keyword"
        assert headers["x-derouter-quality-router-keyword"] == "code"
        # Keyword route short-circuits classification → no complexity header.
        assert "x-derouter-quality-router-complexity" not in headers
        # Existing x-derouter-model-group behavior is unchanged.
        assert headers["x-derouter-model-group"] == "qr"
    ...
    async def test_quality_tier_route_emits_complexity_not_keyword(self):
        ...
        headers = response._hidden_params["additional_headers"]
        assert headers["x-derouter-quality-router-via"] == "quality_tier"
        assert headers["x-derouter-quality-router-complexity"] == "SIMPLE"
        # Quality-tier route → no keyword header.
        assert "x-derouter-quality-router-keyword" not in headers
    ...
    async def test_no_decision_leaves_quality_router_headers_unset(self):
        ...
        headers = response._hidden_params["additional_headers"]
        assert "x-derouter-quality-router-model" not in headers
        assert "x-derouter-quality-router-tier" not in headers
...
class TestRouterQualityDeploymentMethods:
    ...
    def test_is_quality_router_deployment_true(self):
        ...
        params = DeRouter_Params(model="auto_router/quality_router/my-router")
