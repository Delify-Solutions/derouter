...
def _embedding_response():
    return derouter.EmbeddingResponse(
        ...
        object="list",
        usage=derouter.Usage(prompt_tokens=5, completion_tokens=0, total_tokens=5),
    )
    ...
    return derouter.ModelResponse(
        ...
        choices=[
            derouter.Choices(
                finish_reason="stop",
                index=0,
                message=derouter.Message(content="hi", role="assistant"),
            )
        ],
...
STREAMING_DEPLOYMENTS = [
    {"model_info": {"id": FAST_TTFT_ID}, "derouter_params": {}},
    {"model_info": {"id": SLOW_TTFT_ID}, "derouter_params": {}},
]
...
async def test_async_get_available_deployments_treats_missing_samples_as_zero_latency(cached_entry):
    ...
    healthy_deployments = [
        {"model_info": {"id": DEPLOYMENT_ID}, "derouter_params": {}},
        {"model_info": {"id": "slower"}, "derouter_params": {}},
    ]
