...
Discussion: https://github.com/Delify-Solutions/derouter/discussions/32168
"""
    ...
    return [
        {
            ...
            "derouter_params": {"model": "openai/gpt-4o-mini", "mock_response": "cheap openai"},
            "model_info": {"tags": ["openai"]},
        },
        {
            "model_name": "smart-router",
            "derouter_params": {"model": "anthropic/claude-haiku-4-5", "mock_response": "anthropic"},
            "model_info": {"tags": ["anthropic"]},
        },
        {
            "model_name": "smart-router",
            "derouter_params": {"model": "openai/gpt-5.1", "mock_response": "expensive openai"},
            "model_info": {"tags": ["openai"]},
        },
        {
            "model_name": "smart-router",
            "derouter_params": {"model": "ollama/llama-3-70b", "mock_response": "self hosted"},
...
async def test_routing_plugin_pipeline_matches_jeann2013_e2e_scenario():
    """
    https://github.com/Delify-Solutions/derouter/discussions/32168#discussioncomment-17608820
    ...
    # response.model is the bare model name (derouter strips the provider/ prefix
    # on the response), so compare against bare names rather than derouter_params.model
...
async def test_router_without_plugins_is_unaffected():
    ...
    router = Router(
        model_list=[
            {
                "model_name": "smart-router",
                "derouter_params": {"model": "openai/gpt-4o-mini", "mock_response": "hi"},
            },
        ],
    )
    response = await router.acompletion(
...
def test_filter_by_routing_plugin_candidates_narrows_and_raises_when_empty():
    ...
    assert [d["derouter_params"]["model"] for d in narrowed] == ["openai/gpt-4o-mini"]
...
def test_json_default_stable_id_is_stable_across_instances():
    ...
    derouter_params contain a live plugin instance."""
...
def testgenerate_model_id_is_stable_when_derouter_params_contain_a_plugin_instance():
    """End-to-end: a deployment id built from derouter_params containing a routing
    ...
    id1 = router.generate_model_id("smart-router", derouter_params)
