"""Tests for derouter/router_strategy/auto_router/derouter_encoder.py"""
...
class RecordingRouter:
    """Stand-in for the DeRouter Router that records what reached the embedding call.
    ...
    def _response(self, input: list[str]) -> derouter.EmbeddingResponse:
    ...
    def embedding(self, input: list[str], model: str, **kwargs: Any) -> derouter.EmbeddingResponse:
        return self._response(input)

    async def aembedding(self, input: list[str], model: str, **kwargs: Any) -> derouter.EmbeddingResponse:
        return self._response(input)


def _encoder(router: RecordingRouter, **kwargs: Any) -> DeRouterRouterEncoder:
    return DeRouterRouterEncoder(
        derouter_router_instance=router,  # pyright: ignore[reportArgumentType]  # test double stands in for Router
