...
def test_parse_changed_lines_maps_added_lines_per_file():
    ...
    assert changed["derouter/a.py"] == {10, 11, 12}
    assert changed["derouter/b.py"] == {7}


def test_introduced_keeps_only_violations_on_changed_lines():
    violations = [
        Violation("derouter/a.py", 10, "ANN001"),
        Violation("derouter/a.py", 99, "C901"),
    ]
    assert gate.introduced(violations, {"derouter/a.py": {10}}) == [
        Violation("derouter/a.py", 10, "ANN001")
    ]
...
def test_parse_changed_lines_handles_single_and_ranged_hunks(hunk):
    assert gate.parse_changed_lines(f"+++ b/derouter/a.py\n{hunk}\n")["derouter/a.py"]
...
def _ruff_output_for_noqa(code: str, *extra_args: str) -> str:
    proc = subprocess.run(
        [
            ...
            "--no-cache",
            "--stdin-filename",
            "derouter/types/_external_probe.py",
            *extra_args,
            "-",
        ],
        cwd=_REPO_ROOT,
...
def _ruff_output_for_source(source: str) -> str:
    proc = subprocess.run(
        [_RUFF, "check", "--no-cache", "--stdin-filename", "derouter/types/_graduate_probe.py", "-"],
        cwd=_REPO_ROOT,
        input=source,
        capture_output=True,
        text=True,
