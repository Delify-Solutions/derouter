...
See: https://github.com/Delify-Solutions/derouter/pull/22247
"""
...
async def test_eviction_no_unawaited_coroutine_warning():
    ...
    Regression test for https://github.com/Delify-Solutions/derouter/issues/22128
    """
    cache = LLMClientCache(max_size_in_memory=2)
...
@pytest.mark.asyncio
async def test_evicted_derouter_owned_client_is_closed_once_the_grace_window_elapses():
    """
    ...
    client = MockAsyncClient()
    cache.set_cache("client-key", client, derouter_owned_client=True, ttl=600)
...
async def test_evicted_caller_supplied_client_is_never_closed():
    """derouter does not own a client the caller passed in, so it must stay open."""
    clock = _FakeClock()
    cache = LLMClientCache(
        max_size_in_memory=2,
        evicted_client_closer=EvictedClientCloser(grace_seconds=60.0, clock=clock),
