...
from derouter.types.caching import DeRouterCacheType, SemanticCacheScope
...
def test_cache_key_debug_log_does_not_include_prompt_material(caplog):
    cache = Cache(type=DeRouterCacheType.LOCAL)
...
def _semantic_cache(**cache_kwargs):
    return Cache(
        type=DeRouterCacheType.VALKEY_SEMANTIC,
        host="localhost",
        port="6379",
...
@pytest.mark.parametrize(
    "cache_type",
    [DeRouterCacheType.REDIS_SEMANTIC, DeRouterCacheType.VALKEY_SEMANTIC],
)
...
@pytest.mark.parametrize("metadata_field", ["metadata", "derouter_metadata"])
...
def test_semantic_cache_key_reads_tenant_identity_from_derouter_metadata():
    cache = _semantic_cache()
    messages = [{"role": "user", "content": "What color is the sky?"}]
    key_a = cache.get_cache_key(model="gpt-4o-mini", messages=messages, derouter_metadata={"user_api_key": "hash-A"})
    key_b = cache.get_cache_key(model="gpt-4o-mini", messages=messages, derouter_metadata={"user_api_key": "hash-B"})
    key_a_in_derouter_params = cache.get_cache_key(
        model="gpt-4o-mini",
        messages=messages,
        derouter_params={"derouter_metadata": {"user_api_key": "hash-A"}},
    )
    assert key_a != key_b
    assert key_a == key_a_in_derouter_params
