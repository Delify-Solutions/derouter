...
def sync_batch_redis_cache(redis_no_ping):
    with patch(  # test-quality-ok: RedisCache.__init__ builds its client eagerly, with no injection point
        "derouter._redis.get_redis_client", return_value=MagicMock()
    ) as get_client:
...
@pytest.mark.parametrize(
    ...
    [
        ("derouter", "derouter_spend_update_buffer", "derouter:derouter_spend_update_buffer"),
        ("derouter", "derouter_config:param:general_settings", "derouter:derouter_config:param:general_settings"),
        ("derouter", "derouter:3997c4abcdef", "derouter:3997c4abcdef"),
        ("derouter", "spend:key:3997c4abcdef", "derouter:spend:key:3997c4abcdef"),
        (None, "derouter_spend_update_buffer", "derouter_spend_update_buffer"),
        ("", "derouter_spend_update_buffer", "derouter_spend_update_buffer"),
    ],
)
def test_check_and_fix_namespace_prefixes_keys_sharing_the_namespace_prefix(
    ...
    derouter_spend_update_buffer under namespace "derouter") is not namespaced
    ...
    derouter_config:param:* keys reach Redis unprefixed and NOPERM under an ACL
...
@pytest.mark.parametrize("namespace", [None, "derouter"])
@pytest.mark.asyncio
async def test_async_delete_cache_applies_namespace(
    ...
    ACL restricts DEL to the derouter:* pattern."""
    monkeypatch.setenv("REDIS_HOST", "https://my-test-host")
    redis_cache = RedisCache(namespace=namespace)
    ...
    expected_key = "derouter:3997c4abcdef" if namespace else "3997c4abcdef"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            ["derouter_sandbox:{k:v}:tokens", "derouter_sandbox:{k:v}:requests"],
        ),
    ],
)
@pytest.mark.asyncio
...
@pytest.mark.parametrize("namespace", [None, "derouter_sandbox"])
...
async def test_circuit_breaker_opens_when_method_swallows_redis_failure(call_method):
    ...
    """
    from derouter.constants import REDIS_CIRCUIT_BREAKER_FAILURE_THRESHOLD
...
def sync_batch_cache_with_service_logger(redis_no_ping: None) -> Iterator[tuple[RedisCache, ServiceLogging]]:
    ...
    failing_client.mget.side_effect = OSError("redis unavailable")
    with patch(  # test-quality-ok: RedisCache.__init__ builds its client eagerly, with no injection point
        "derouter._redis.get_redis_client", return_value=failing_client
    ):
...
async def test_sync_batch_get_cache_reports_a_failed_read_from_a_running_loop(
    ...
    failure event, derouter_redis_failed_requests_total stops moving during a Redis
...
def test_sync_batch_get_cache_survives_a_service_callback_that_raises(
    ...
    cache, service_logger = sync_batch_cache_with_service_logger
    monkeypatch.setattr(derouter, "service_callback", ["prometheus_system"])
    monkeypatch.setattr(
        service_logger,
...
def test_call_stack_info_skips_breaker_guard_frames():
    ...
    """
    from derouter.caching.redis_cache import (
        RedisCircuitBreaker,
...
def test_call_stack_info_skips_guard_frames_when_deployed_without_sources(monkeypatch):
    ...
    """
    from derouter.caching import redis_cache as redis_cache_module
    from derouter.caching.redis_cache import (
        RedisCircuitBreaker,
        _get_call_stack_info,
        _redis_circuit_breaker_guard_sync,
...
async def test_only_connectivity_failures_open_the_breaker(error, opens_breaker):
    ...
    from derouter.caching.redis_cache import RedisCircuitBreaker, _run_under_circuit_breaker
...
async def test_stale_timeout_does_not_let_sub_threshold_hard_failures_open_the_breaker():
    ...
    from derouter.caching.redis_cache import RedisCircuitBreaker, _is_redis_timeout_failure
...
async def test_breaker_metrics_track_state_and_failure_class():
    ...
    timeout_before = sample("derouter_redis_circuit_breaker_failures_total", {"failure_class": "timeout"})
    hard_before = sample("derouter_redis_circuit_breaker_failures_total", {"failure_class": "connectivity"})
    opened_before = sample("derouter_redis_circuit_breaker_transitions_total", {"state": "open"})
    open_gauge_before = sample("derouter_redis_circuit_breaker_state", {"state": "open"})
    closed_gauge_before = sample("derouter_redis_circuit_breaker_state", {"state": "closed"})
    ...
    assert sample("derouter_redis_circuit_breaker_failures_total", {"failure_class": "timeout"}) == timeout_before + 1
    assert sample("derouter_redis_circuit_breaker_failures_total", {"failure_class": "connectivity"}) == hard_before + 2
    assert sample("derouter_redis_circuit_breaker_transitions_total", {"state": "open"}) == opened_before + 1
    assert sample("derouter_redis_circuit_breaker_state", {"state": "open"}) == open_gauge_before + 1
    assert sample("derouter_redis_circuit_breaker_state", {"state": "closed"}) == closed_gauge_before

    breaker.record_success()
    assert sample("derouter_redis_circuit_breaker_state", {"state": "open"}) == open_gauge_before
    assert sample("derouter_redis_circuit_breaker_state", {"state": "closed"}) == closed_gauge_before + 1
