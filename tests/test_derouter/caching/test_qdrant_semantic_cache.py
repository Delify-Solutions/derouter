...
def test_qdrant_semantic_cache_initialization(monkeypatch):
    ...
    with (
        ...
        patch("derouter.llms.custom_httpx.http_handler.get_async_httpx_client"),
    ):
        ...
        from derouter.caching.qdrant_semantic_cache import QdrantSemanticCache
...
def test_qdrant_semantic_cache_get_cache_hit():
    ...
    with (
        ...
        with patch(
            "derouter.embedding", return_value={"data": [{"embedding": [0.1, 0.2, 0.3]}]}
        ):
...
def test_qdrant_semantic_cache_rejects_unscoped_cache_hit():
    ...
    safely migrated to a generated DeRouter cache key.
    """
    with (
        patch(
            "derouter.llms.custom_httpx.http_handler._get_httpx_client"
...
def test_qdrant_semantic_cache_set_cache():
    ...
    with (
        ...
        # Mock the embedding function
        with patch(
            "derouter.embedding", return_value={"data": [{"embedding": [0.1, 0.1, 0.1]}]}
        ):
            # Test set_cache
...
def test_qdrant_semantic_cache_default_vector_size():
    ...
    with (
        ...
        from derouter.caching.qdrant_semantic_cache import QdrantSemanticCache
        from derouter.constants import QDRANT_VECTOR_SIZE
...
def _router_proxy_module(router, model_name):
    mod = types.ModuleType("derouter.proxy.proxy_server")
...
def test_qdrant_sync_get_cache_routes_through_router(monkeypatch):
    ...
    with patch("derouter.embedding") as direct_embed:
...
def test_qdrant_sync_set_cache_falls_back_to_direct(monkeypatch):
    ...
    fake_proxy = types.ModuleType("derouter.proxy.proxy_server")
    fake_proxy.llm_router = None
    fake_proxy.llm_model_list = None
    monkeypatch.setitem(sys.modules, "derouter.proxy.proxy_server", fake_proxy)
...
def _token_count(model, text):
    ...
    return len(derouter.encode(model=model, text=text))
...
def test_qdrant_semantic_cache_defaults_embedding_timeout():
    from derouter.caching.qdrant_semantic_cache import QdrantSemanticCache
    from derouter.constants import SEMANTIC_CACHE_EMBEDDING_TIMEOUT_SECONDS
