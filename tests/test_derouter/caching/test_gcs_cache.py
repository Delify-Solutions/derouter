...
from derouter.caching.gcs_cache import GCSCache
...
def mock_gcs_dependencies():
    ...
    with (
        patch.object(import_module("derouter.caching.gcs_cache"), "_get_httpx_client", return_value=mock_sync_client
        ),
        patch.object(import_module("derouter.caching.gcs_cache"), "get_async_httpx_client",
            return_value=mock_async_client,
        ),
        patch.object(import_module("derouter.caching.gcs_cache").GCSBucketBase, "sync_construct_request_headers",
            return_value={},
        ),
    ):
        yield {
...
async def test_gcs_cache_async_get_encodes_object_name_in_path(mock_gcs_dependencies):
    """
    Regression test for https://github.com/Delify-Solutions/derouter/issues/30377
