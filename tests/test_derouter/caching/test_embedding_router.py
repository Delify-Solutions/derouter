...
import derouter
from derouter.caching._embedding_router import (
...
def test_resolve_skips_entries_missing_model_name():
    router = MagicMock()
    model_list = [
        {"derouter_params": {"model": "bedrock/x"}},
        {"model_name": "sem-embed"},
    ]
    assert resolve_embedding_router("sem-embed", router, model_list) is router
    assert resolve_embedding_router("other", router, [{"derouter_params": {}}]) is None
...
def test_truncate_embedding_input_keeps_prompt_within_limit():
    ...
    token_count = len(derouter.encode(model="sem-embed", text=prompt))
...
def test_truncate_embedding_input_cuts_prompt_to_token_limit():
    ...
    assert len(derouter.encode(model="sem-embed", text=truncated)) == 50
