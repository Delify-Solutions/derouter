...
def test_redis_semantic_cache_initialization(monkeypatch):
    ...
    with patch.dict(
        ...
        from derouter.caching.redis_semantic_cache import RedisSemanticCache
...
def test_redis_get_embedding_falls_back_to_direct(monkeypatch):
    ...
    with patch(
        "derouter.embedding", return_value={"data": [{"embedding": [0.1, 0.2]}]}
    ) as direct_embed:
...
def test_cache_forwards_semantic_cache_embedding_timeout():
    from derouter.caching.caching import Cache
    from derouter.types.caching import DeRouterCacheType

    with patch.object(import_module("derouter.caching.caching"), "RedisSemanticCache") as backend:
        Cache(
            type=DeRouterCacheType.REDIS_SEMANTIC,
            similarity_threshold=0.8,
            redis_url="redis://localhost:6379",
