...
under derouter/ is mentioned nowhere on the docs site. It only sees a key if one of its
...
def test_derouter_prefixed_get_secret_bool_is_captured() -> None:
    assert gate.extract_env_keys('derouter.get_secret_bool("QSTASH_FLUSH_ON_BOOT")') == {"QSTASH_FLUSH_ON_BOOT"}
...
def test_get_secret_reached_through_the_utils_module_is_captured() -> None:
    assert gate.extract_env_keys('derouter.utils.get_secret("QSTASH_ALPHA")') == {"QSTASH_ALPHA"}
...
def test_previously_supported_call_shapes_are_still_captured() -> None:
    source = "\n".join(
        (
            ...
            'derouter.get_secret("QSTASH_CHARLIE")',
            'derouter.get_secret_str("QSTASH_DELTA", default_value=None)',
        )
    )
...
def test_excluded_keys_are_filtered_for_every_call_shape() -> None:
    source = "\n".join(
        (
            ...
            'get_secret_bool("DEROUTER_RUST")',
            'derouter.get_secret_str("MAVVRIK_FOCUS_FREQUENCY")',
        )
    )
    assert gate.extract_env_keys(source) == frozenset()
