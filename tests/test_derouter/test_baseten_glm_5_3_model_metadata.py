...
from derouter.utils import supports_function_calling, supports_prompt_caching

REPO_ROOT = Path(__file__).parents[2]
MAIN_PATH = REPO_ROOT / "model_prices_and_context_window.json"
BACKUP_PATH = REPO_ROOT / "derouter" / "model_prices_and_context_window_backup.json"
...
def test_baseten_glm_5_3_specs():
    ...
    assert info["derouter_provider"] == "baseten"
...
def test_baseten_glm_5_3_capabilities_are_visible_to_callers(local_model_cost_map):
    ...
    info = derouter.get_model_info(model="zai-org/GLM-5.3", custom_llm_provider="baseten")
    assert info["max_input_tokens"] == 1048576
    assert info["max_output_tokens"] == 262144
...
def test_entry_advertises_only_what_the_baseten_path_accepts(local_model_cost_map):
    ...
    derouter's Baseten path drops it (``drop_params=True``) or raises
    ...
    """
    supported = derouter.get_supported_openai_params(model="zai-org/GLM-5.3", custom_llm_provider="baseten")
    ...
    with pytest.raises(derouter.UnsupportedParamsError):
        derouter.utils.get_optional_params(
