...
from derouter.constants import ANTHROPIC_SKILLS_API_BETA_VERSION
from derouter.llms.anthropic.skills.transformation import AnthropicSkillsConfig
...
class TestAnthropicSkillsConfigURLConstruction:
    ...
    def test_url_falls_back_to_anthropic_default(self):
        with patch(
            "derouter.llms.anthropic.common_utils.AnthropicModelInfo.get_api_base",
...
class TestAnthropicSkillsConfigHeaderValidation:
    ...
    def _make_derouter_params(self, api_key=FAKE_API_KEY):
        return GenericDeRouterParams(api_key=api_key)

    def test_sets_api_key_header(self):
        with patch(
            "derouter.llms.anthropic.common_utils.AnthropicModelInfo.get_api_key",
            return_value=FAKE_API_KEY,
        ):
            headers = self.config.validate_environment(
                headers={}, derouter_params=self._make_derouter_params()
            )
    ...
    def test_merges_existing_beta_header_string(self):
        with patch(
            ...
            headers = self.config.validate_environment(
                ...
                derouter_params=self._make_derouter_params(),
            )
        ...
        with patch(
            ...
            with pytest.raises(ValueError, match="ANTHROPIC_API_KEY"):
                self.config.validate_environment(
                    headers={}, derouter_params=self._make_derouter_params(api_key=None)
                )
...
class TestAnthropicSkillsConfigCreateRequestTransformation:
    def setup_method(self):
        self.config = AnthropicSkillsConfig()
        self.derouter_params = GenericDeRouterParams(api_key=FAKE_API_KEY)
