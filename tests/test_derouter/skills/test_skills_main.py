...
import derouter.skills.main as skills_main
...
def test_create_skill_forwards_description_and_instructions_from_top_level_kwargs(
    ...
    kwargs (not extra_body). Regression for a bug where the derouter_proxy dispatch
    branch of create_skill() dropped both, so every DeRouter-hosted skill was created
...
def test_create_skill_without_description_or_instructions_passes_none(monkeypatch) -> None:
    ...
    skills_main.create_skill(display_title="Bare Skill", custom_llm_provider=LlmProviders.DEROUTER_PROXY.value)
