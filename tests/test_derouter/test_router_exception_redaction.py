"""
Tests for `derouter.expose_router_debug_in_errors`.
...
Five leak sites are gated in `derouter/router.py`:

1. Deployment timeout debug after `derouter.Timeout`
...
def _reset_expose_flag(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(derouter, "expose_router_debug_in_errors", True)


def test_flag_defaults_on():
    assert derouter.expose_router_debug_in_errors is True
...
async def test_flag_off_does_not_leak_received_model_group(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(derouter, "expose_router_debug_in_errors", False)
...
# --- Site 1: Deployment timeout debug on derouter.Timeout --------------------


def _router_with_plain_deployment() -> Router:
    """Plain deployment, no preconfigured mock_response — caller supplies via kwargs.

    Exception instances cannot live in `model_list[*].derouter_params` because
    `Router.__init__` deep-copies model_list and several DeRouter exceptions
...
async def test_flag_off_does_not_leak_deployment_timeout_debug(monkeypatch: pytest.MonkeyPatch):
    ...
    with pytest.raises(derouter.Timeout) as excinfo:
        await router.acompletion(
            model=_INTERNAL_MODEL_GROUP_NAME,
            messages=[{"role": "user", "content": "hi"}],
            mock_timeout=True,
...
def _content_policy_error() -> derouter.ContentPolicyViolationError:
    return derouter.ContentPolicyViolationError(
...
async def test_flag_off_does_not_leak_content_policy_fallback_hint(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(derouter, "expose_router_debug_in_errors", False)
    router = _router_with_plain_deployment()
    with pytest.raises(derouter.ContentPolicyViolationError) as excinfo:
        await router.acompletion(
            model=_INTERNAL_MODEL_GROUP_NAME,
            messages=[{"role": "user", "content": "hi"}],
