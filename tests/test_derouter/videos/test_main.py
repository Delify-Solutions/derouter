"""
Dispatch-contract tests for derouter/videos/main.py
...
from derouter.types.videos.main import CharacterObject, VideoObject
...
def test_video_status__dispatch_and_provider_from_id(seams):
    ...
    assert seams.get_config.call_args.kwargs["provider"] == derouter.LlmProviders.AZURE
...
# derouter_params. Distinct per-field values catch a cross-wired field.
# =========================================================================== #
...
def test_db_yaml_credentials_reach_every_handler(seams, handler_name, invoke):
    ...
    derouter_params = seams.kwargs_of(handler_name)["derouter_params"]
    assert derouter_params.get("api_key") == DB_YAML_CREDS["api_key"]
    assert derouter_params.get("api_base") == DB_YAML_CREDS["api_base"]
    assert derouter_params.get("api_version") == DB_YAML_CREDS["api_version"]
    assert derouter_params.get("vertex_project") == DB_YAML_CREDS["vertex_project"]
