"""
Pure-logic contract tests for derouter/videos/main.py's request utils
(derouter/videos/utils.py: VideoGenerationRequestUtils).
...
provider config's map_openai_params (a provider boundary); filter_out_derouter_params
runs for real, so the "derouter-internal params get stripped" assertions reflect
...
# params. filter_out_derouter_params strips known internal keys for real; the
# values used below were chosen against the live set: seconds/size/user/foo_param/
# vertex_project/extra/a/b survive, api_key/metadata/derouter_* are stripped.
# =========================================================================== #
...
def test_requested__strips_derouter_internal_params():
    result = get_requested(
        {
            "seconds": "8",
            "api_key": "sk-secret",
            "metadata": {"x": 1},
            "derouter_call_id": "id-123",
        }
    )
...
def test_requested__timeout_always_removed():
    # timeout is NOT a derouter-internal param, so only the explicit pop removes it.
...
def test_requested__extra_body_strips_derouter_internal_params():
...
def test_optional__delegates_to_map_openai_params_with_drop_params():
    ...
    config.map_openai_params.assert_called_once_with(
        ...
        model="sora-2",
        drop_params=derouter.drop_params,
    )
