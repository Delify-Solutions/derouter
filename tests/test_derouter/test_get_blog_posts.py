...
<rss version="2.0">
  <channel>
    <title>DeRouter Blog</title>
    <item>
      <title>Test Post</title>
      <link>https://router.delify.vn/blog/test</link>
      <description>A test post.</description>
      <pubDate>Wed, 01 Jan 2026 10:00:00 GMT</pubDate>
    </item>
    <item>
      <title>Second Post</title>
      <link>https://router.delify.vn/blog/second</link>
...
def test_parse_rss_to_posts():
    ...
    assert posts[0]["url"] == "https://router.delify.vn/blog/test"
...
def test_get_blog_posts_success():
    ...
    with patch(
        "derouter.derouter_core_utils.get_blog_posts.httpx.get",
        return_value=mock_response,
    ):
        posts = get_blog_posts(url=derouter.blog_posts_url)
...
def test_get_blog_posts_ttl_cache_not_refetched():
    ...
    with patch(
        "derouter.derouter_core_utils.get_blog_posts.httpx.get", side_effect=mock_get
    ):
        posts = get_blog_posts(url=derouter.blog_posts_url)
...
def test_get_blog_posts_local_env_var_skips_remote(monkeypatch):
    monkeypatch.setenv("DEROUTER_LOCAL_BLOG_POSTS", "true")
    with patch("derouter.derouter_core_utils.get_blog_posts.httpx.get") as mock_get:
        posts = get_blog_posts(url=derouter.blog_posts_url)
