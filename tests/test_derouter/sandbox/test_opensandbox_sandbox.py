...
import derouter
from derouter.llms.base_llm.sandbox.transformation import ContainerHandle
...
def test_static_helpers_cover_defaults_and_fallbacks(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.llms.opensandbox.sandbox.transformation.get_secret_str",
        fake_secret,
    )
...
async def test_create_waits_across_pending_state(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.llms.opensandbox.sandbox.transformation.asyncio.sleep", fake_sleep
    )
...
async def test_public_lifecycle_create_run_delete():
    ...
    container = await derouter.acreate_sandbox(
        provider="opensandbox", api_key="", api_base=TEST_API_BASE, client=client
    )
    result = await derouter.arun_code(
    ...
    ok = await derouter.adelete_sandbox(
        provider="opensandbox",
        container=container,
        api_key="",
        client=client,
...
async def test_code_interpreter_tool_deletes_even_when_run_raises():
    ...
    with pytest.raises(RuntimeError, match="boom"):
        await derouter.acode_interpreter_tool(
            provider="opensandbox",
            code="1/0",
            api_key="",
            api_base=TEST_API_BASE,
