        ...
        [
            {
                ...
                "derouter_params": {"sandbox_provider": "e2b"},
            }
        ]
    )
...
def test_register_skips_malformed_entries_without_crashing():
    ...
    sandbox_tools.register_sandbox_tools(
        [
            {"derouter_params": {"sandbox_provider": "e2b"}},  # missing name
            "not-a-dict",  # wrong type
            {"sandbox_tool_name": "", "derouter_params": {}},  # empty name
            {
...
def test_register_skips_entry_missing_sandbox_provider():
    ...
    sandbox_tools.register_sandbox_tools(
        [
            {"sandbox_tool_name": "no_provider", "derouter_params": {"api_key": "sk-x"}},
            {
                "sandbox_tool_name": "null_provider",
                "derouter_params": {"sandbox_provider": None},
            },
            {
                "sandbox_tool_name": "good",
                "derouter_params": {"sandbox_provider": "e2b"},
...
def test_register_swaps_registry_atomically():
    ...
    _reset()
    sandbox_tools.register_sandbox_tools(
        [{"sandbox_tool_name": "a", "derouter_params": {"sandbox_provider": "e2b"}}]
    )
    before = sandbox_tools._SANDBOX_TOOL_REGISTRY

    sandbox_tools.register_sandbox_tools(
        [
            {"sandbox_tool_name": "b", "derouter_params": {"sandbox_provider": "e2b"}},
            {"sandbox_tool_name": "c", "derouter_params": {"sandbox_provider": "e2b"}},
        ]
    )
