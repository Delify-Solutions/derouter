...
`.github/workflows/test-derouter-ui-unit.yml` narrows the dashboard's Vitest run to
...
WORKFLOW = REPO_ROOT / ".github" / "workflows" / "test-derouter-ui-unit.yml"
...
def _step_script() -> str:
    ...
    resolved = script.replace("${{ github.repository }}", "Delify-Solutions/derouter")
...
def _run_step(tmp_path: Path, changed: list[str], base_sha: str = "basesha") -> tuple[int, str, list[list[str]]]:
    ...
    env["GITHUB_REF_NAME"] = "derouter_internal_staging"
