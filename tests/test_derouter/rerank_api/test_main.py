...
def test_rerank_does_not_log_request_content_at_info(caplog):
    ...
    """
    derouter.cohere_key = "test_api_key"
    caplog.set_level(logging.DEBUG, logger="DeRouter")
    ...
    derouter_records = [r for r in caplog.records if r.name == "DeRouter"]

    info_or_above = [
        r.getMessage()
        for r in derouter_records
    ...
    optional_params_logs = [r for r in derouter_records if "optional_rerank_params" in r.getMessage()]
...
def test_rerank_error_names_provider_and_keeps_body(respx_mock: respx.MockRouter, monkeypatch):
    ...
    with pytest.raises(derouter.NotFoundError) as exc_info:
        derouter.rerank(
...
@pytest.mark.asyncio
async def test_arerank_error_is_mapped_to_derouter_exception(respx_mock: respx.MockRouter, monkeypatch):
    """Regression for arerank's bare re-raise: provider errors escaped as raw
    provider exception classes instead of the mapped derouter exception contract."""
    monkeypatch.delenv("DASHSCOPE_API_BASE", raising=False)
    monkeypatch.delenv("DASHSCOPE_API_BASE_RERANK", raising=False)
...
async def test_arerank_declared_authenticating_provider_skips_resolution(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "get_llm_provider", record_resolution)
    monkeypatch.setattr("derouter.rerank_api.main.rerank", rerank_raises_provider_error)
