"""
Unit tests for derouter.proxy_auth module.

Tests the OAuth2/JWT token management for DeRouter Proxy authentication.
"""
...
from derouter.proxy_auth import (
    AccessToken,
    AzureADCredential,
...
class TestDeRouterIntegration:
    """Tests for integration with derouter module."""

    def test_proxy_auth_variable_exists(self):
        """Test that derouter.proxy_auth variable exists."""
        import derouter

        # Should be None by default
        assert hasattr(derouter, "proxy_auth")

    def test_proxy_auth_can_be_set(self):
        """Test that derouter.proxy_auth can be set to a ProxyAuthHandler."""
        import derouter

        original_value = derouter.proxy_auth
        try:
            cred = MockCredential()
            handler = ProxyAuthHandler(credential=cred, scope="test")
            derouter.proxy_auth = handler

            assert derouter.proxy_auth is handler
        finally:
            derouter.proxy_auth = original_value
