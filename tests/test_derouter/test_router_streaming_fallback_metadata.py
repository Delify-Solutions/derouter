...
import derouter
from derouter.proxy.proxy_server import _should_include_fallback_errors
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "openai/gpt-fake", "api_key": "sk-fake"},
            },
            {
                "model_name": "fallback-model",
                "derouter_params": {"model": "openai/gpt-fake-2", "api_key": "sk-fake"},
            },
        ],
...
async def test_include_fallback_errors_propagates_through_router():
    ...
    assert headers["x-derouter-attempted-fallbacks"] == 1
    errors = json.loads(headers["x-derouter-fallback-errors"])
...
async def test_router_omits_fallback_errors_without_opt_in():
    ...
    headers = _additional_headers(response)
    assert headers["x-derouter-attempted-fallbacks"] == 1
    assert "x-derouter-fallback-errors" not in headers
