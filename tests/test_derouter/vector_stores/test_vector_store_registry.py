...
def test_get_credentials_for_vector_store():
    ...
    vector_store_1 = DeRouter_ManagedVectorStore(
        vector_store_id="test_id_1",
        custom_llm_provider="openai",
        vector_store_name="test_store_1",
        derouter_credential_name="test_creds_1",
        created_at=datetime.now(timezone.utc),
        updated_at=datetime.now(timezone.utc),
    )

    vector_store_2 = DeRouter_ManagedVectorStore(
        vector_store_id="test_id_2",
        custom_llm_provider="bedrockc",
        vector_store_name="test_store_2",
        derouter_credential_name="test_creds_2",
        created_at=datetime.now(timezone.utc),
        updated_at=datetime.now(timezone.utc),
    )
    ...
    with patch(
        "derouter.derouter_core_utils.credential_accessor.CredentialAccessor.get_credential_values"
    ) as mock_get_creds:
...
def test_add_vector_store_to_registry():
    ...
    existing_store_1 = DeRouter_ManagedVectorStore(
    ...
    existing_store_2 = DeRouter_ManagedVectorStore(
    ...
    new_store = DeRouter_ManagedVectorStore(
    ...
    duplicate_store = DeRouter_ManagedVectorStore(
...
def test_search_uses_registry_credentials():
    ...
    try:
        ...
        with (
            ...
            search(vector_store_id="vs1", query="test", derouter_logging_obj=logger)
            mock_get_creds.assert_called_once_with("vs1")
            called_params = mock_handler.call_args.kwargs["derouter_params"]
