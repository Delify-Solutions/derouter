...
def _clear_names_from_globals(names: tuple):
    """Clear all names from derouter globals."""
    # Get the actual globals dict, not a copy
    derouter_globals = sys.modules["derouter"].__dict__
    for name in names:
        if name in derouter_globals:
            del derouter_globals[name]


def _clear_names_from_utils_globals(names: tuple):
    """Clear all names from derouter.utils globals."""
    # Get the actual globals dict, not a copy
    utils_globals = sys.modules["derouter.utils"].__dict__
    for name in names:
...
def _verify_only_requested_name_imported(name: str, all_names: tuple):
    ...
    for other_name in all_names:
        if other_name != name:
            assert (
                other_name not in derouter_globals
...
def test_cost_calculator_lazy_imports():
    ...
    for name in COST_CALCULATOR_NAMES:
        ...
        assert name in derouter_globals
...
def test_derouter_logging_lazy_imports():
    """Test that all derouter_logging items can be lazy imported."""
    # Get the actual globals dict, not a copy
    derouter_globals = sys.modules["derouter"].__dict__
    ...
    for name in DEROUTER_LOGGING_NAMES:
        # Clear all names before importing just one
        _clear_names_from_globals(DEROUTER_LOGGING_NAMES)

        item = _lazy_import_derouter_logging(name)
        assert item is not None
        assert name in derouter_globals

        # Verify only the requested name is in globals, not the others
        _verify_only_requested_name_imported(name, DEROUTER_LOGGING_NAMES)
...
def test_unknown_attribute_raises_error():
    ...
    with pytest.raises(AttributeError):
        _lazy_import_derouter_logging("unknown")
