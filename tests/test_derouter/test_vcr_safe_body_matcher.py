...
def test_normalize_volatile_tokens_collapses_bedrock_batch_job_names():
    a = (
        b'{"jobName":"derouter-batch-aaaaaaaa",'
        b'"outputDataConfig":{"s3OutputDataConfig":'
        b'{"s3Uri":"s3://bucket/derouter-batch-outputs/derouter-batch-aaaaaaaa/"}}}'
    )
    b = (
        b'{"jobName":"derouter-batch-bbbbbbbb",'
        b'"outputDataConfig":{"s3OutputDataConfig":'
        b'{"s3Uri":"s3://bucket/derouter-batch-outputs/derouter-batch-bbbbbbbb/"}}}'
    )
    assert _normalize_volatile_tokens(a) == _normalize_volatile_tokens(b)
...
def test_tolerant_path_normalizes_bedrock_managed_s3_file_uuid():
    ...
    a = Request(
        method="PUT",
        uri=(
            "https://s3.us-west-2.amazonaws.com/derouter-proxy-test/"
            "derouter-bedrock-files/us.anthropic.claude-haiku-4-5-20251001-v1-0-"
...
def test_tolerant_query_skips_telemetry_but_enforces_others():
    ...
    a = _greq(
        "https://us.cloud.langfuse.com/api/public/observations?traceId=derouter-test-AAA"
    )
    b = _greq(
        "https://us.cloud.langfuse.com/api/public/observations?traceId=derouter-test-BBB"
    )
