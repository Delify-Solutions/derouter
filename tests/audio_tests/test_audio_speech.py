...
async def _run_audio_speech_derouter(sync_mode, model, api_base, api_key):
    ...
    if sync_mode:
        ...
        response = await derouter.aspeech(
            model=model,
            voice="alloy",
            input="the quick brown fox jumped over the lazy dogs",
            api_base=api_base,
...
async def test_audio_speech_derouter_azure(sync_mode):
    await _run_audio_speech_derouter(
        sync_mode=sync_mode,
        model="azure/tts",
...
@pytest.mark.asyncio
@pytest.mark.flaky(retries=3, delay=1)
async def test_audio_speech_derouter_openai(sync_mode):
    await _run_audio_speech_derouter(
        sync_mode=sync_mode,
        model="openai/tts-1",
        api_base=None,
...
@pytest.mark.asyncio
@pytest.mark.flaky(retries=3, delay=1)
async def test_audio_speech_derouter_vertex(sync_mode):
...
@pytest.mark.asyncio
async def test_speech_derouter_vertex_async():
    # Mock the response
...
@pytest.mark.asyncio
async def test_speech_derouter_vertex_async_with_voice():
    # Mock the response
    mock_response = AsyncMock()
...
@pytest.mark.asyncio
async def test_speech_derouter_vertex_async_with_voice_ssml():
    # Mock the response
    mock_response = AsyncMock()
...
async def test_aws_polly_tts_real_api():
    ...
    response = await derouter.aspeech(
        ...
        voice="Joanna",
        input="Hello, this is a test of AWS Polly text to speech integration with DeRouter.",
        aws_region_name="us-east-1",
    )
