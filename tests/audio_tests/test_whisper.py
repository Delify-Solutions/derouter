# What is this?
## Tests `derouter.transcription` endpoint. Outside derouter module b/c of audio file used in testing (it's ~700kb).
...
async def _run_transcription(
    ...
    transcript = await derouter.atranscription(
        model=model,
        file=_audio_file(),
        api_key=api_key,
        api_base=api_base,
...
async def test_transcription_caching():
    ...
    response_1 = await derouter.atranscription(
        model="whisper-1",
        file=_audio_file(),
    )
    ...
    response_2 = await derouter.atranscription(
        model="whisper-1",
        file=_audio_file(),
    )
    ...
    response_3 = await derouter.atranscription(
        model="whisper-1",
        file=_audio_file2(),
    )
...
async def test_gpt_4o_transcribe_model_mapping():
    ...
    response = await derouter.atranscription(
    ...
    response2 = await derouter.atranscription(
        model="openai/gpt-4o-transcribe", file=_audio_file(), response_format="json"
    )
    ...
    response3 = await derouter.atranscription(
...
async def test_azure_transcribe_model_mapping():
    ...
    with patch(
        "derouter.llms.azure.audio_transcriptions.AzureAudioTranscription.get_azure_openai_client",
