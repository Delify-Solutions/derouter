...
def check_for_derouter_module_deletion(base_dir):
    """
    Checks for code patterns that delete derouter modules from sys.modules
    in the test_derouter directory.

    Specifically looks for patterns like:
    for module in list(sys.modules.keys()):
        if module.startswith("derouter"):
            del sys.modules[module]
    """
    problematic_files = []
    test_dir = os.path.join(base_dir, "test_derouter")
    ...
    for root, _, files in os.walk(test_dir):
        for file in files:
            if file.endswith(".py"):
                ...
                # Check for derouter module deletion patterns
                if has_derouter_module_deletion(tree):
                    relative_path = os.path.relpath(file_path, base_dir)
                    problematic_files.append(relative_path)
                    print(f"Found derouter module deletion in: {relative_path}")

    return problematic_files


def has_derouter_module_deletion(tree):
    """
    Checks if the AST contains patterns that delete derouter modules from sys.modules.

    Looks for:
    1. Loops over sys.modules.keys()
    2. Conditions checking if module startswith "derouter"
    3. del sys.modules[module] statements
    """

    class DeRouterDeletionVisitor(ast.NodeVisitor):
        def __init__(self):
            self.has_sys_modules_loop = False
            self.has_derouter_check = False
        ...
        def visit_If(self, node):
            # Check for conditions like module.startswith("derouter")
            if (
                ...
                and node.test.args[0].value == "derouter"
            ):

                self.has_derouter_check = True
    ...
    visitor = DeRouterDeletionVisitor()
    visitor.visit(tree)

    return (
        visitor.has_sys_modules_loop
        and visitor.has_derouter_check
...
def main():
    """
    Main function to check for derouter module deletion patterns in test files.
    """
    # local dir
    # tests_dir = "../../tests/"
    ...
    problematic_files = check_for_derouter_module_deletion(tests_dir)

    if problematic_files:
        print("\nERROR: Found files that delete derouter modules from sys.modules:")
        for file_path in problematic_files:
            print(f"  - {file_path}")

        raise Exception(
            f"Found {len(problematic_files)} file(s) that delete derouter modules from sys.modules. "
            f"This can cause import issues and test failures. Files: {problematic_files}"
        )
    else:
        print("✓ No derouter module deletion patterns found in test_derouter directory.")
