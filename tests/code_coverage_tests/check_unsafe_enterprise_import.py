...
class EnterpriseImportFinder(ast.NodeVisitor):
    ...
    def visit_Import(self, node):
        # Check for direct imports of derouter_enterprise
        for name in node.names:
            if "derouter_enterprise" in name.name or "enterprise" in name.name:
    ...
    def visit_ImportFrom(self, node):
        # Check for from derouter_enterprise imports
        if node.module and (
            "derouter_enterprise" in node.module or "enterprise" in node.module
        ):
            if not self.in_try_block:
                self.unsafe_imports.append(
                    {
...
if __name__ == "__main__":
    # Check for unsafe enterprise imports in the derouter directory
