...
def get_function_names_from_file(file_path: str) -> List[str]:
    """
    Extracts all static method names from derouter_logging.py
    """
...
def main():
    logging_file = "./derouter/derouter_core_utils/derouter_logging.py"
    tests_dir = "./tests/"
    ...
    if untested_functions:
        ...
        raise Exception(
            f"{untested_perc * 100:.2f}% of functions in derouter_logging.py are not tested: {untested_functions}"
        )
    else:
        print("All functions in derouter_logging.py are covered by tests.")
