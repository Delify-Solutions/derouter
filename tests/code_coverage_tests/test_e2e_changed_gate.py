...
@pytest.mark.parametrize(
    ("changed", "expected"),
    (
        (("tests/e2e/logging/test_datadog_e2e.py", "derouter/router.py"), ("tests/e2e/logging/test_datadog_e2e.py",)),
...
def test_harness_changes_run_the_canary_suite(harness_file: str) -> None:
    assert select_tests((harness_file, "derouter/router.py")) == CANARY
