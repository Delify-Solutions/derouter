...
# Root of the derouter package
DEROUTER_ROOT = os.path.join(os.path.dirname(__file__), "..", "..", "derouter")
...
def _collect_python_files():
    """Collect all Python files under the derouter package."""
    python_files = []
    for root, dirs, files in os.walk(DEROUTER_ROOT):
            ...
            for line_num, line in enumerate(f, start=1):
                for match in BASIC_AUTH_PATTERN.finditer(line):
                    ...
                    if _is_real_base64_credentials(b64_value):
                        rel_path = os.path.relpath(filepath, DEROUTER_ROOT)
