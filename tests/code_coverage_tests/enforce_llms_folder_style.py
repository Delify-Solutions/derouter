...
def get_unique_names_from_llms_dir(base_dir: str):
    """
    Returns a set of unique file and folder names from the root level of derouter/llms directory,
...
def run_lint_check(unique_names):
    _all_derouter_providers = [str(provider.value) for provider in derouter.LlmProviders]
    violations = []
    for name in unique_names:
        if (
            name.lower() not in _all_derouter_providers
    ...
    if len(violations) > 0:
        raise ValueError(
            f"There are {len(violations)} violations in the llms folder. \n\n {violations}. \n\n Valid providers: {_all_derouter_providers}"
        )


def main():
    # llms_dir = "./derouter/llms/"  # Update this path if needed
    llms_dir = "derouter/derouter/llms"  # LOCAL TESTING
