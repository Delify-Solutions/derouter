...
1. Every provider folder in derouter/llms/ has a corresponding entry in provider_endpoints_support.json
2. Every provider in derouter/llms/openai_like/providers.json is documented in provider_endpoints_support.json
"""
...
def get_repo_root() -> Path:
    ...
    # Check if derouter directory exists in current working directory
    cwd = Path.cwd()
    if (cwd / "derouter").exists() and (cwd / "derouter").is_dir():
...
def get_llm_provider_folders() -> Set[str]:
    """Get all provider folder names from derouter/llms directory."""
    repo_root = get_repo_root()
    llms_dir = repo_root / "derouter" / "llms"
...
def get_openai_like_providers() -> Set[str]:
    """Get all provider names from derouter/llms/openai_like/providers.json."""
    repo_root = get_repo_root()
    providers_file = repo_root / "derouter" / "llms" / "openai_like" / "providers.json"
...
def main():
    ...
    # Check 1: Provider folders in derouter/llms
    print("\n📁 Checking provider folders in derouter/llms/...")
    ...
    if undocumented_folders:
        ...
        for folder in undocumented_folders:
            error_msg += f"  - derouter/llms/{folder}/\n"
        ...
        for folder in undocumented_folders[:3]:
            ...
            error_msg += (
                f'      "url": "https://router.delify.vn/docs/providers/{folder}",\n'
            )
            error_msg += '      "endpoints": {\n'
    ...
    if undocumented_openai_like:
        ...
        for provider in undocumented_openai_like[:3]:
            ...
            error_msg += (
                f'      "url": "https://router.delify.vn/docs/providers/{normalized}",\n'
            )
            error_msg += '      "endpoints": {\n'
            error_msg += '        "chat_completions": true,\n'
            error_msg += '        "messages": true,\n'
