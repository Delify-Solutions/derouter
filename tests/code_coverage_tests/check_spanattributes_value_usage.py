...
# Add parent directory to path so we can import derouter
import derouter
...
def main():
    ...
    target_file = os.path.join("derouter", "integrations", "opentelemetry.py")

    if not os.path.exists(target_file):
        # Try alternate path for local development
        target_file = os.path.join(
            "..", "..", "derouter", "integrations", "opentelemetry.py"
        )
