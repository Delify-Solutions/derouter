...
ALLOWED_FILES = [
    # local files
    "../../derouter/derouter_core_utils/derouter.logging_callback_manager.py",
    "../../derouter/proxy/common_utils/callback_utils.py",
...
def check_for_callback_modifications(file_path):
    """
    Checks if any direct modifications to specific derouter callback lists are made in the given file.
    ...
    for node in ast.walk(tree):
        # Check for attribute calls like derouter.callbacks.append()
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute):
            ...
            # Check if the attribute chain starts with 'derouter' and modifies a protected list
            if (
                len(attr_chain) >= 3
                and attr_chain[0] == "derouter"
                ...
                if (
                    ...
                    violations.append(
                        ...
                        f"Direct modification of 'derouter.{protected_list}' using '{operation}' is not allowed. "
...
def test_no_unauthorized_callback_modifications():
    ...
    """
    base_dir = "./derouter"  # Adjust this path as needed
    # base_dir = "../../derouter"  # LOCAL TESTING
