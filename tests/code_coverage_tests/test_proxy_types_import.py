...
def test_proxy_types_not_imported():
    """
    Test that proxy._types is not directly imported in derouter/__init__.py
    by examining the source code using AST parsing.
    """
    # Read the derouter/__init__.py file
    # local_init_file = "../derouter/"
    init_file_path = os.path.join("./derouter", "__init__.py")
    ...
    if found_imports:
        print(
            "❌ BAD, this can import time to import derouter. Found direct imports of proxy._types in derouter/__init__.py:"
        )
        print("=" * 80)
        ...
        raise Exception(
            f"Found {len(found_imports)} direct import(s) of proxy._types in derouter/__init__.py"
        )

    print("✓ No direct imports of proxy._types found in derouter/__init__.py")
    return True
...
def main():
    ...
    """
    print("=" * 60)
    print("Testing derouter import performance")
    print(
        "Checking that proxy._types is not directly imported from derouter/__init__.py"
    )
    print("=" * 60)
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print(
            "✓ Test passed! proxy._types is not directly imported from derouter/__init__.py"
        )
        print("=" * 60)
