...
ALLOWED_FILE = os.path.normpath("derouter/_uuid.py")
...
def scan_directory_for_fastuuid(base_dir: str) -> List[Dict[str, Any]]:
    ...
    scan_root = os.path.join(base_dir, "derouter")
...
def main() -> None:
    ...
    if violations:
        print(
            "\n🚨 fastuuid must only be imported inside derouter/_uuid.py. Found violations:"
        )
        for v in violations:
            print(f"* {v['module']} ({v['file']}:{v['line']}) -> {v['import']}")
        print("\n")
        raise Exception(
            "Found fastuuid imports outside derouter/_uuid.py. Use derouter._uuid.uuid or derouter._uuid.uuid4 instead."
        )
    else:
