...
from derouter._logging import handler as derouter_log_handler
...
class HarnessOutputFilter(logging.Filter):
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        return not (record.name == "DeRouter" and record.getMessage().startswith(noisy_prefixes))
...
def _start_output_filtering() -> None:
    derouter_log_handler.addFilter(_HARNESS_OUTPUT_FILTER)


def _stop_output_filtering() -> None:
    derouter_log_handler.removeFilter(_HARNESS_OUTPUT_FILTER)
