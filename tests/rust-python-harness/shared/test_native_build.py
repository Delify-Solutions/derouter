...
def test_newest_source_mtime_tracks_rust_sources_and_skips_target(tmp_path: Final) -> None:
    source: Final = tmp_path / "derouter-rust" / "crates" / "bridge" / "src"
    source.mkdir(parents=True)
    (source / "lib.rs").write_text("fn main() {}\n")
    os.utime(source / "lib.rs", (1_000, 1_000))
    manifest: Final = tmp_path / "derouter-rust" / "crates" / "bridge" / "Cargo.toml"
    manifest.write_text("[package]\n")
    os.utime(manifest, (2_000, 2_000))
    lockfile: Final = tmp_path / "derouter-rust" / "Cargo.lock"
    lockfile.write_text("")
    os.utime(lockfile, (1_500, 1_500))
    target: Final = tmp_path / "derouter-rust" / "target" / "debug" / "junk.rs"
...
def test_ensure_trace_bridge_rebuilds_when_stale(
    ...
    source: Final = tmp_path / "derouter-rust" / "crates" / "bridge" / "src" / "lib.rs"
