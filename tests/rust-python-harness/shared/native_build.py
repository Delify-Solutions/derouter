...
from derouter.rust_bridge import get_native_bridge, reset_native_bridge_cache

MATURIN_SPEC: Final = "maturin==1.15.0"
BRIDGE_FEATURE: Final = "trace-parity"
_RUST_ROOT: Final = "derouter-rust"
...
def _native_module_path() -> Path | None:
    try:
        spec: Final = importlib.util.find_spec("derouter.rust_bridge._native")
...
def _drop_imported_bridge() -> None:
    ...
    for name in tuple(sys.modules):
        if name.startswith("derouter.rust_bridge._native"):
