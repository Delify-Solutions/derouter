...
class _Response(_CassetteModel):
    ...
    headers: dict[str, tuple[str, ...]]
    body: _Body
    chunk_lengths: tuple[int, ...] | None = Field(default=None, alias="x-derouter-chunk-lengths")
...
class ParityCassette(_CassetteModel):
    ...
    parity: _ParityMetadata = Field(alias="x-derouter")
...
def _response_dict(response: RecordedResponse) -> dict[str, object]:
    ...
    return {
        ...
        "headers": headers,
        "body": {"string": body},
        **({"x-derouter-chunk-lengths": list(map(len, chunks))} if chunks is not None else {}),
    }
