...
def test_parity_rejects_request_difference() -> None:
    ...
    rust: Final = _execution(body={"model": "different"}, user_agent="derouter-rust")
...
def test_parity_rejects_response_difference() -> None:
    python: Final = _execution(user_agent=SENTINEL)
    rust: Final = _execution(markdown="different", user_agent="derouter-rust")
...
def test_parity_distinguishes_unobserved_callbacks_from_zero_events() -> None:
    python: Final = _execution(user_agent=SENTINEL)
    rust: Final = _execution(user_agent="derouter-rust").model_copy(update={"callbacks": ()})
...
def test_parity_rejects_callback_payload_difference() -> None:
    ...
    rust: Final = _execution(user_agent="derouter-rust").model_copy(
...
def test_parity_rejects_error_difference() -> None:
    python: Final = Execution(
        requests=(),
        report=SDKError(
            exception_type="derouter.exceptions.BadRequestError",
            message="bad request",
            status_code=400,
            code=None,
            error_type=None,
...
def test_model_parity_compares_public_values_and_ignores_private_attrs() -> None:
    ...
    python.set_hidden_param("derouter_call_id", "python-id")
    rust.set_hidden_param("derouter_call_id", "rust-id")
