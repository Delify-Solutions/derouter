...
PYTHON_VARIANT: Final = ExecutionVariant(name="Python", environment=(("DEROUTER_RUST", "0"),))
RUST_VARIANT: Final = ExecutionVariant(name="Rust", environment=(("DEROUTER_RUST", "1"),))
...
class RecordingCallback(CustomLogger):
    ...
    def _record(
        ...
        raw_derouter_params: Final = kwargs.get("derouter_params")
        derouter_params: Final[Mapping[str, object]] = (
            cast(Mapping[str, object], raw_derouter_params) if isinstance(raw_derouter_params, Mapping) else {}
        )
        raw_metadata: Final = derouter_params.get("metadata")
        ...
        raw_call_id: Final = kwargs.get("derouter_call_id")
        self._observations.put(
            CallbackObservation(
                ...
                derouter_call_id=raw_call_id if isinstance(raw_call_id, str) else None,
                metadata=metadata,
...
INVALID_OCR_CASES: Final = (
    InvalidOcrCase(
        ...
        expected_exception_type="derouter.exceptions.APIConnectionError",
    ...
    InvalidOcrCase(
        ...
        document={"type": "document_url", "document_url": "data:application/pdf;base64,AA=="},
        expected_exception_type="derouter.exceptions.BadRequestError",
    ...
    InvalidOcrCase(
        ...
        model="mistral/mistral-ocr-latest",
        document={"type": "document_url", "document_url": "data:application/pdf;base64,AA=="},
        expected_exception_type="derouter.exceptions.UnsupportedParamsError",
        expected_status_code=400,
...
def _call_kwargs(sdk_input: OcrSdkInput, mock_url: str, route: SDKRoute) -> dict[str, object]:
    return {
        **sdk_input.as_sdk_kwargs(),
        "api_base": mock_url,
        "api_key": API_KEY,
        "extra_headers": {"x-derouter-parity-route": route.value},
    }
...
def _execute_sdk_call(
    ...
    try:
        if route is SDKRoute.OCR:
            sync_route: Final = cast(Callable[..., OCRResponse], derouter.ocr)
            response: Final = sync_route(**call_kwargs)
            return SDKSuccess(response=response.model_dump(mode="json"))
        async_route: Final = cast(Callable[..., Coroutine[object, object, OCRResponse]], derouter.aocr)
...
def _execute_callback_sdk_case(
    ...
    call_kwargs: Final = {
        **_call_kwargs(case.derouter_input, mock_url, route),
        "callbacks": [callback],
        "derouter_call_id": _callback_call_id(route, terminal),
        "derouter_trace_id": _callback_call_id(route, terminal),
...
def _assert_callback_lifecycle(
    ...
    for observation in observations:
        ...
        assert observation.derouter_call_id == expected_call_id
...
def _recorded_check_name(fixture: OcrParityCase, route: SDKRoute) -> str:
    case_input: Final = fixture.derouter_input
...
def _callback_fixture(
    ...
    matching: Final = tuple(
        ...
        if fixture.derouter_input.contract == "mistral"
        and (
    ...
    return min(matching, key=lambda fixture: fixture_id(fixture.derouter_input, fixture.derouter_input.model))
...
def parity_checks() -> Generator[tuple[E2ECheck, ...]]:
    fixtures: Final = tuple(
        ...
        if fixture.derouter_input.contract not in {"reducto_v3", "reducto_legacy"}
    )
    ...
    with tempfile.TemporaryDirectory(prefix="derouter-ocr-parity-") as raw_directory:
    ...
    try:
        ...
        match worker_case:
            case RecordedOcrWorkerCase(case=recorded):
                return WorkerSuccess(report=_execute_sdk_case(recorded.derouter_input, route, mock_url, event_loop))
