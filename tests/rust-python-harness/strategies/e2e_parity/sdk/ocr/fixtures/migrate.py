...
import derouter
from derouter.rust_bridge.ocr import rust, set_rust_ocr
...
def _invoke(provider_url: str, case_input: OcrSdkInput) -> object:
    sdk_call: Final = cast(OcrSdkCall, derouter.ocr)
...
def migrate_fixture(path: Path) -> Path:
    ...
    with replay_server() as provider:
        ...
        captured: Final = record_upstream_interactions(UpstreamEndpoint(provider.url), case.derouter_input, _invoke)
