...
class OcrParityCase(ParityCase[OcrSdkInput]):
    ...
    def load_legacy_contract(cls, value: object) -> object:
        ...
        derouter_input: Final = fixture.get("derouter_input")
        if not isinstance(derouter_input, Mapping) or "contract" in derouter_input:
            return fixture
        legacy_input: Final = cast(Mapping[str, object], derouter_input)
        ...
        if not isinstance(model, str):
            return fixture
        return {**fixture, "derouter_input": {"contract": _legacy_contract(model), **legacy_input}}
