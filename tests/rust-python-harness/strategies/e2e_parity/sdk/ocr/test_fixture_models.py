...
from derouter.llms.azure_ai.ocr.document_intelligence.transformation import AzureDocumentIntelligenceOCRConfig
from derouter.llms.azure_ai.ocr.transformation import AzureAIOCRConfig
from derouter.llms.base_llm.ocr.transformation import (
    BaseOCRConfig,
    DocumentType,
    OCRRequestData,
)
from derouter.llms.mistral.ocr.transformation import MistralOCRConfig
from derouter.llms.reducto.ocr.transformation import ReductoParseLegacyConfig, ReductoParseV3Config
from derouter.llms.vertex_ai.ocr.deepseek_transformation import VertexAIDeepSeekOCRConfig
...
def test_fixture_catalogs_match_active_registered_ocr_models() -> None:
    ...
    active_registered: Final = frozenset(
        ...
        if raw_metadata.get("mode") == "ocr" and raw_metadata.get("derouter_provider") in SUPPORTED_OCR_PROVIDERS
...
def test_ocr_parity_case_migrates_legacy_contract_metadata(legacy_key: str | None) -> None:
    derouter_input: Final[dict[str, object]] = {
        "model": "mistral/mistral-ocr-latest",
        "document": {"type": "document_url", "document_url": "https://example.com/document.pdf"},
    }
    if legacy_key is not None:
        derouter_input[legacy_key] = "mistral"

    fixture: Final = OcrParityCase.model_validate({"derouter_input": derouter_input, "provider_responses": ()})

    assert fixture.derouter_input.contract == "mistral"
...
def test_reducto_parity_cases_are_non_strict_xfails(
    ...
    marks: Final = ocr_fixture_marks(OcrParityCase(derouter_input=sdk_input, provider_responses=()))
...
def test_supported_parity_cases_have_no_marks() -> None:
    ...
    assert ocr_fixture_marks(OcrParityCase(derouter_input=sdk_input, provider_responses=())) == ()
...
def test_reducto_legacy_strategy_generates_valid_derouter_inputs(sdk_input: ReductoParseLegacySdkInput) -> None:
...
def test_azure_document_intelligence_strategy_only_generates_derouter_inputs(
    sdk_input: AzureDocumentIntelligenceOcrSdkInput,
) -> None:
    assert sdk_input.req_format == "derouter"
...
def test_vertex_deepseek_strategy_only_generates_derouter_inputs(
