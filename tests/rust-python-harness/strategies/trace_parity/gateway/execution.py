...
def _collect_python(fixture: RouteFixture) -> tuple[FunctionTraceEvent, ...]:
    ...
    from derouter.proxy.anthropic_endpoints.endpoints import user_api_key_auth
    ...
    proxy_server.llm_router = derouter.Router(
        model_list=[
            {
    ...
    try:
        with profile_python(Path(derouter.__file__).parent, threads=True) as profiler:
...
def _collect_rust(fixture: RouteFixture) -> tuple[FunctionTraceEvent, ...]:
    from derouter.rust_bridge import get_native_bridge
