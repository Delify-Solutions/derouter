...
async def test_async_chat_openai_stream():
    try:
        ...
        derouter.success_callback = [tmp_function.async_test_logging_fn]
...
def test_async_custom_handler_stream():
    try:
        ...
        derouter.callbacks = [customHandler2]
        derouter.set_verbose = False
        messages = [
            {"role": "system", "content": "You are a helpful assistant."},
            {
                "role": "user",
                "content": "write 1 sentence about derouter being amazing",
            },
        ]
        complete_streaming_response = ""
...
def test_azure_completion_stream():
    ...
    try:
        ...
        messages = [
            ...
            {
                "role": "user",
                "content": f"write 1 sentence about derouter being amazing {time.time()}",
            },
        ]
        complete_streaming_response = ""
...
async def test_async_custom_handler_embedding():
    try:
        ...
        derouter.callbacks = [customHandler_embedding]
        # success
...
async def test_async_custom_handler_embedding_optional_param():
    ...
    derouter.callbacks = [customHandler_optional_params]
    response = await derouter.aembedding(
