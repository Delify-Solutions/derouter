...
from derouter.proxy.guardrails.guardrail_hooks.aim.aim import (
    AimGuardrail,
    AimGuardrailMissingSecrets,
)
from derouter.proxy.proxy_server import StreamingCallbackError, UserAPIKeyAuth
...
async def test_block_callback(mode: str):
    ...
    aim_guardrails = [
        callback for callback in derouter.callbacks if isinstance(callback, AimGuardrail)
    ]
...
async def test_post_call_stream__all_chunks_are_valid(monkeypatch, length: int):
    ...
    monkeypatch.setattr(
        "derouter.proxy.guardrails.guardrail_hooks.aim.aim.connect", connect_mock
    )
