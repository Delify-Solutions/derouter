...
class MyCustomHandler(CustomLogger):
    ...
    def log_pre_api_call(self, model, messages, kwargs):
        ...
        print(
            f"previous_models: {kwargs['derouter_params']['metadata'].get('previous_models', None)}"
        )
        self.previous_models = len(
            kwargs["derouter_params"]["metadata"].get("previous_models", [])
        )  # {"previous_models": [{"model": derouter_model_name, "exception_type": AuthenticationError, "exception_string": <complete_traceback>}]}
...
async def test_async_fallbacks_max_retries_per_request():
    derouter.set_verbose = False
    derouter.num_retries_per_request = 0
    model_list = [
        {  # list of model deployments
            "model_name": "azure/gpt-3.5-turbo",  # openai model name
            "derouter_params": {  # params for derouter completion/embedding call
        ...
        model_list = [
            {
                ...
                "derouter_params": get_azure_params("chatgpt-v-3"),
                "model_info": {"id": 1},
                "rpm": AZURE_FAST_RPM,
            },
            {
            ...
            },
            {
                ...
                "derouter_params": get_openai_params("gpt-3.5-turbo"),
                "model_info": {"id": 3},
                "rpm": OPENAI_RPM,
            },
            {
                "model_name": "anthropic-claude-haiku-4-5-20251001",
                "derouter_params": get_anthropic_params("claude-haiku-4-5-20251001"),
                "model_info": {"id": 4},
                "rpm": ANTHROPIC_RPM,
            },
        ]
...
@pytest.mark.parametrize("sync_mode", [True, False])
@pytest.mark.parametrize("derouter_module_fallbacks", [True, False])
@pytest.mark.asyncio
async def test_default_model_fallbacks(sync_mode, derouter_module_fallbacks):
    """
    Related issue - https://github.com/Delify-Solutions/derouter/issues/3623

    If model misconfigured, setup a default model for generic fallback
    """
    if derouter_module_fallbacks:
        derouter.default_fallbacks = ["my-good-model"]
    router = Router(
        model_list=[
            {
        ...
        default_fallbacks=(
            ["my-good-model"] if derouter_module_fallbacks is False else None
        ),
    )
...
async def test_router_content_policy_fallbacks(
    ...
    os.environ["DEROUTER_LOG"] = "DEBUG"

    if content_filter_response_exception:
        mock_response = Exception("content filtering policy")
    else:
        mock_response = derouter.ModelResponse(
            choices=[derouter.Choices(finish_reason="content_filter")],
            model="gpt-3.5-turbo",
            usage=derouter.Usage(prompt_tokens=10, completion_tokens=0, total_tokens=10),
        )
    router = Router(
        model_list=[
            {
...
async def test_using_default_fallback(sync_mode):
    ...
    from derouter._logging import verbose_logger, verbose_router_logger

    verbose_logger.setLevel(logging.DEBUG)
    verbose_router_logger.setLevel(logging.DEBUG)
    derouter.default_fallbacks = ["very-bad-model"]
    router = Router(
        model_list=[
            {
                "model_name": "openai/*",
...
async def test_using_default_working_fallback(sync_mode):
    ...
    verbose_logger.setLevel(logging.DEBUG)
    verbose_router_logger.setLevel(logging.DEBUG)
    derouter.default_fallbacks = ["openai/gpt-3.5-turbo"]
    router = Router(
        model_list=[
            {
                "model_name": "openai/*",
...
def test_router_fallbacks_with_custom_model_costs():
    ...
    default_model_info = derouter.get_model_info(model="claude-sonnet-4-5-20250929")
    ...
    model_info = derouter.get_model_info(model="claude-sonnet-4-5-20250929")

    print(f"key: {model_info['key']}")

    assert model_info["derouter_provider"] == "anthropic"
...
async def test_router_fallbacks_default_and_model_specific_fallbacks(sync_mode):
    ...
    with pytest.raises(Exception, match='derouter\\.AuthenticationError: AuthenticationError') as exc_info:
        await _call_bad_model()
    assert isinstance(
        exc_info.value, derouter.AuthenticationError
    ), f"Expected AuthenticationError, but got {type(exc_info.value).__name__}"


@pytest.mark.asyncio
async def test_router_disable_fallbacks_dynamically():
    from derouter.router import run_async_fallback
...
def test_router_fallbacks_with_model_id():
    router = Router(
        model_list=[
            {
                "model_name": "gpt-3.5-turbo",
                "derouter_params": {"model": "gpt-3.5-turbo", "rpm": 1},
                "model_info": {
                    "id": "123",
                },
            }
...
async def test_router_attempted_fallbacks_in_response(expected_attempted_fallbacks):
    ...
    - Test cases: works on first try, `x-derouter-attempted-fallbacks` is 0
    - Works on 1st fallback, `x-derouter-attempted-fallbacks` is 1
    - Works on 3rd fallback, `x-derouter-attempted-fallbacks` is 3
    """
    router = Router(
        model_list=[
            {
    ...
    if expected_attempted_fallbacks == 0:
        ...
        assert (
            resp._hidden_params["additional_headers"]["x-derouter-attempted-fallbacks"]
