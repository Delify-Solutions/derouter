...
from derouter.derouter_core_utils.prompt_templates.factory import anthropic_messages_pt
...
def test_response_model_none():
    """
    Addresses:https://github.com/Delify-Solutions/derouter/issues/2972
    """
    x = completion(
        model="mymodel",
    ...
    print(f"x: {x}")
    assert isinstance(x, derouter.ModelResponse)
...
def _openai_mock_response(*args, **kwargs) -> derouter.ModelResponse:
...
def test_completion_claude_3_empty_response():
    ...
    try:
        ...
        print(response)
    except derouter.InternalServerError as e:
...
def test_completion_claude_3_function_call(model):
    ...
    except derouter.InternalServerError:
        pass
    except Exception as e:
        pytest.fail(f"Error occurred: {e}")
...
async def test_model_function_invoke(model, sync_mode, api_key, api_base):
    try:
        ...
        if sync_mode:
            response = derouter.completion(**data)
        else:
            response = await derouter.acompletion(**data)
...
async def test_anthropic_no_content_error():
    """
    https://github.com/Delify-Solutions/derouter/discussions/3440#discussioncomment-9323402
    """
    try:
...
def test_parse_xml_params():
    from derouter.derouter_core_utils.prompt_templates.factory import parse_xml_params
...
def test_completion_mistral_api():
    try:
        ...
        cost = derouter.completion_cost(completion_response=response)
...
def test_completion_mistral_api_mistral_large_function_call():
    ...
    try:
        ...
        print(second_response)
    except derouter.RateLimitError:
        pass
    except Exception as e:
        pytest.fail(f"Error occurred: {e}")
...
def test_completion_openai_response_headers():
    """
    Tests if DeRouter reurns response hea
    """
    derouter.return_response_headers = True
    ...
    streaming_response = derouter.completion(
        model="gpt-4o-mini",
        messages=messages,
        stream=True,
    )
    ...
    # embedding
    embedding_response = derouter.embedding(
    ...
    derouter.return_response_headers = False
...
async def test_async_completion_openai_response_headers():
    ...
    streaming_response = await derouter.acompletion(
        model="gpt-4o-mini",
        messages=messages,
        stream=True,
    )
    ...
    # embedding
    embedding_response = await derouter.aembedding(
        model="text-embedding-ada-002",
        input="hello",
    )
...
def test_lm_studio_completion(monkeypatch):
    ...
    except derouter.AuthenticationError as e:
        pytest.fail(f"Error occurred: {e}")
    except derouter.APIError as e:
        print(e)
...
# this should throw an exception, to trigger https://logs.router.delify.vn/
...
def test_completion_openai():
    try:
        ...
        derouter.api_key = os.environ["OPENAI_API_KEY"]
        response = completion(
            model="gpt-3.5-turbo",
    ...
    try:
        ...
        for _ in range(3):
            try:
                ...
                break
            except derouter.JSONSchemaValidationError:
...
def custom_callback(
    ...
    try:
        print("DEROUTER: in custom callback function")
        print("\nkwargs\n", kwargs)
        model = kwargs["model"]
...
def test_completion_openai_with_optional_params():
    ...
    try:
        derouter.set_verbose = True
        derouter.success_callback = [custom_callback]
        response = completion(
        ...
        print(response)
        derouter.success_callback = []  # unset callbacks
...
def test_completion_logprobs():
    """
    This function is used to test the derouter.completion logprobs functionality.
...
def test_completion_openai_derouter_key():
    try:
        derouter.set_verbose = True
        ...
        ###### reset environ key
        os.environ["OPENAI_API_KEY"] = derouter.api_key
...
# test_ completion_openai_derouter_key()
...
def test_completion_ollama_hosted():
    ...
    try:
        derouter.request_timeout = 20  # give ollama 20 seconds to response
        derouter.set_verbose = True
        response = completion(
            model="ollama/phi",
            messages=messages,
...
def test_completion_ollama_function_call(model):
    ...
    try:
        derouter.set_verbose = True
        response = derouter.completion(model=model, messages=messages, tools=tools)
        print(response)
...
async def test_completion_functions_param():
    ...
    try:
        ...
        with patch.object(client, "post", side_effect=gemini_mock_post) as mock_client:
            response: derouter.ModelResponse = await derouter.acompletion(
            ...
            assert (
                "derouter_param_is_function_call"
...
def test_completion_azure_extra_headers():
    ...
    with patch.object(http_client, "send", new=MagicMock()) as mock_client:
        derouter.client_session = http_client
        try:
            response = completion(
...
def test_azure_openai_ad_token():
    ...
    # we use derouter.input_callbacks for this test
    def tester(
        kwargs,  # kwargs to completion
    ):
        print("inside kwargs")
    ...
    derouter.input_callback = [tester]
    try:
        response = derouter.completion(
...
def test_completion_azure3():
    ...
    try:
        print("azure gpt-3.5 test\n\n")
        derouter.set_verbose = True
        derouter.api_base = os.environ["AZURE_AI_API_BASE"]
        derouter.api_key = os.environ["AZURE_AI_API_KEY"]
        derouter.api_version = os.environ["AZURE_API_VERSION"]
        ...
        os.environ["AZURE_AI_API_BASE"] = derouter.api_base
        os.environ["AZURE_API_VERSION"] = derouter.api_version
        os.environ["AZURE_AI_API_KEY"] = derouter.api_key
...
# new azure test for using derouter. vars,
# use the following vars in this test and make an azure_api_call
#  derouter.api_type = self.azure_api_type
#  derouter.api_base = self.AZURE_AI_API_BASE
#  derouter.api_version = self.azure_api_version
#  derouter.api_key = self.api_key
def test_completion_azure_with_derouter_key():
    try:
        print("azure gpt-3.5 test\n\n")
        import openai

        #### set derouter vars
        derouter.api_type = "azure"
        derouter.api_base = os.environ["AZURE_AI_API_BASE"]
        derouter.api_version = os.environ["AZURE_API_VERSION"]
        derouter.api_key = os.environ["AZURE_AI_API_KEY"]
        ...
        ######### UNSET derouter vars
        derouter.api_type = None
        derouter.api_base = None
        derouter.api_version = None
        derouter.api_key = None
...
async def test_completion_replicate_llama3(sync_mode):
    ...
    try:
        ...
        # Add any assertions here to check the response
        assert isinstance(response, derouter.ModelResponse)
...
def test_replicate_custom_prompt_dict():
    ...
    try:
        ...
        pytest.fail(f"An exception occurred - {str(e)}")
    print(f"response: {response}")
    derouter.custom_prompt_dict = {}  # reset
...
def test_bedrock_deepseek_custom_prompt_dict():
    ...
    assert model in derouter.known_tokenizer_config
...
def test_customprompt_together_ai():
    try:
        ...
        print(derouter.success_callback)
        print(derouter._async_success_callback)
        response = completion(
            model="together_ai/openai/gpt-oss-20b",
            messages=messages,
            roles={
        ...
        print(response)
    except derouter.exceptions.Timeout as e:
...
def response_format_tests(response: derouter.ModelResponse):
    ...
    assert isinstance(choice, derouter.Choices)
    assert isinstance(choice.get("index"), int)

    message = choice.get("message")
    assert isinstance(message, derouter.Message)
    ...
    assert isinstance(response.usage, derouter.Usage)  # type: ignore
...
#     # send a request to this proxy server: https://replit.com/@Delify-Solutions/openai-proxy#main.py
...
#             # see https://replit.com/@Delify-Solutions/openai-proxy#main.py
...
#     model_name = "qvv0xeq"
#     derouter.api_base = "https://app.baseten.co"
#     try:
...
def test_completion_deep_infra(drop_params):
    ...
    with patch(
        "derouter.llms.openai.openai.OpenAIChatCompletion.make_sync_openai_chat_completion_request",
        ...
        if drop_params is False:
            ...
            with pytest.raises(derouter.exceptions.UnsupportedParamsError):
                completion(
                    model=model_name,
                    messages=messages,
                    temperature=0,
...
async def test_acompletion_gemini():
    ...
    messages = [{"role": "user", "content": "Hey, how's it going?"}]
    try:
        response = await derouter.acompletion(model=model_name, messages=messages)
        # Add any assertions here to check the response
        print(f"response: {response}")
    except derouter.Timeout as e:
        pass
...
def test_unified_auth_params(provider, model, project, region_name, token):
    ...
    translated_optional_params = derouter.utils.get_optional_params(**data)

    if provider == "azure":
        special_auth_params = (
            derouter.AzureOpenAIConfig().get_mapped_special_auth_params()
        )
    elif provider == "bedrock":
        special_auth_params = (
            derouter.AmazonBedrockGlobalConfig().get_mapped_special_auth_params()
        )
    elif provider == "vertex_ai":
        special_auth_params = derouter.VertexAIConfig().get_mapped_special_auth_params()
    elif provider == "watsonx":
        special_auth_params = (
            derouter.IBMWatsonXAIConfig().get_mapped_special_auth_params()
        )
...
#     try:
#         derouter.config_path = "../config.json"
#         derouter.set_verbose = True
#         response = derouter.config_completion(messages=messages)
#         # Add any assertions here to check the response
#         print(response)
#         derouter.config_path = None
#     except Exception as e:
#         pytest.fail(f"Error occurred: {e}")
...
def test_completion_together_ai_stream():
    derouter.set_verbose = True
    user_message = "Write 1pg about YC & derouter"
    messages = [{"content": user_message, "role": "user"}]
    try:
        response = completion(
            model="together_ai/openai/gpt-oss-20b",
...
def test_moderation():
    response = derouter.moderation(input="i'm ishaan cto of derouter")
    print(response)
...
def test_openai_hallucinated_tool_call_util(function_name, expect_modification):
    ...
    2. Iterate over tool_uses array to call functions:
        ...
        - parameters will be JSON object for function arguments
    """
    from derouter.utils import _handle_invalid_parallel_tool_calls
    from derouter.types.utils import ChatCompletionMessageToolCall
...
def test_completion_openai_metadata(monkeypatch, enable_preview_features):
    ...
    monkeypatch.setattr(derouter, "enable_preview_features", enable_preview_features)
    with patch.object(
...
def test_edit_note():
    derouter.callbacks = ["langfuse_otel"]
    response = completion(
        model="gpt-4o",
        messages=[
            {
