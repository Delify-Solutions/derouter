...
def test_add_pattern_vertex_ai():
    ...
    deployment = Deployment(
        ...
        derouter_params=DeRouter_Params(model="vertex_ai/gemini-1.5-flash-latest"),
        model_info=ModelInfo(),
    )
...
async def test_route_with_no_matching_pattern():
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "bedrock/meta.llama3*"},
            }
        ]
    )
    ...
    ## FAILS
    with pytest.raises(derouter.BadRequestError) as e:
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "anthropic/*", "api_key": "test"},
            }
        ]
    )
...
def test_sorted_patterns():
    ...
    """
    from derouter.router_utils.pattern_match_deployments import PatternUtils
...
def test_wildcard_priority_over_deployment_names():
    """
    Test that wildcard routes take priority over deployment_names (derouter_params.model) matching.
    ...
    NOT deployment 1 (even though deployment 1's derouter_params.model matches).

    Priority order should be:
    1. Exact model_name match
    2. Wildcard model_name match
    3. deployment_names (derouter_params.model) match
    """
    router = Router(
        model_list=[
            {
    ...
    assert (
        deployments[0]["derouter_params"]["api_base"] == "http://localhost:8082/openai"
    ), f"Expected wildcard deployment (8082), got {deployments[0]['derouter_params']['api_base']}"
    ...
    assert len(deployments) == 1, f"Expected 1 deployment, got {len(deployments)}"
    assert (
        deployments[0]["derouter_params"]["api_base"] == "http://localhost:8080/openai"
    ), f"Expected exact match deployment (8080), got {deployments[0]['derouter_params']['api_base']}"
    ...
    assert len(deployments) == 1, f"Expected 1 deployment, got {len(deployments)}"
    assert (
        deployments[0]["derouter_params"]["api_base"] == "http://localhost:8081/openai"
    ), f"Expected '*' wildcard deployment (8081), got {deployments[0]['derouter_params']['api_base']}"
