...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"api_base": "^https://derouter.*direct\.fireworks\.ai/v1$"},
            {"api_base": "https://derouter-dev.direct.fireworks.ai/v1"},
            True,
        ),
        (
            {"api_base": "^https://derouter.*novice\.fireworks\.ai/v1$"},
            {"api_base": "https://derouter-dev.direct.fireworks.ai/v1"},
            False,
        ),
    ],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Test 9: Multiple fallback sources - derouter_metadata
        (
            {},
            {"user_header_name": "X-User-ID"},
            {"derouter_metadata": {"user": "derouter-user-789"}},
            "derouter-user-789",
        ),
        ...
        (
            ...
            {
                "user": "body-user",
                "derouter_metadata": {"user": "derouter-user"},
...
def test_get_end_user_id_from_request_body_with_user_header_name(
    ...
    with patch("derouter.proxy.proxy_server.general_settings", general_settings_config):
        end_user_id = get_end_user_id_from_request_body(
            request_body, dict(mock_request.headers)
        )
...
def test_get_end_user_id_from_request_body_backwards_compatibility():
    ...
    # Test with derouter_metadata
    request_body = {"derouter_metadata": {"user": "derouter-user-456"}}
    end_user_id = get_end_user_id_from_request_body(request_body)
    assert end_user_id == "derouter-user-456"
...
def test_get_internal_user_header_from_mapping_returns_internal_header():
    ...
    result = DeRouterProxyRequestSetup.get_internal_user_header_from_mapping(mappings)
...
def test_get_internal_user_header_from_mapping_no_internal_returns_none():
    ...
    single_mapping = {"header_name": "X-Only-Customer", "derouter_user_role": "customer"}
    result = DeRouterProxyRequestSetup.get_internal_user_header_from_mapping(
        single_mapping
    )
    assert result is None
