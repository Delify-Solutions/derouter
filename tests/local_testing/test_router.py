...
from derouter.types.router import DeploymentTypedDict
...
def test_router_deployment_typing():
    deployment_typed_dict = DeploymentTypedDict(
        model_name="hi", derouter_params={"model": "hello-world"}
    )
...
def test_router_azure_ad_token_provider():
    ...
    for azure_cred in ["DefaultAzureCredential", "AzureCliCredential"]:
        ...
        derouter.enable_azure_ad_token_refresh = True
        router = Router(model_list=[_deployment])
...
async def test_router_retries(sync_mode):
    ...
    model_list = [
        {
            "model_name": "gpt-3.5-turbo",
            "derouter_params": {"model": "gpt-3.5-turbo", "api_key": "bad-key"},
        },
        {
            "model_name": "gpt-3.5-turbo",
            "derouter_params": {
    ...
    try:
        ...
        async def embedding_call():
            ...
            response = await router._aembedding(
                model="text-embedding-ada-002",
                input=["good morning from derouter 2"],
            )
            print(response)
            router.reset()
...
def test_azure_embedding_on_router():
    ...
    try:
        ...
        async def embedding_call():
            response = await router.aembedding(
                model="text-embedding-ada-002", input=["good morning from derouter"]
            )
            print(response)
        ...
        response = router.embedding(
            model="text-embedding-ada-002",
            input=["test 2 from derouter. async embedding"],
        )
        print(response)
        router.reset()
    except Exception as e:
...
async def test_mistral_on_router():
    ...
    response = await router.acompletion(
        ...
        messages=[
            {
                "role": "user",
                "content": "hello from derouter test",
            }
        ],
    )
    print(response)
    ...
    try:
        ...
        async def test():
            ...
            response = await router.acompletion(
                ...
                messages=[
                    {
                        "role": "user",
                        "content": f"hello from derouter test {time.time()}",
                    }
                ],
                stream=True,
            )
        ...
        response = router.completion(
            ...
            messages=[
                {
                    "role": "user",
                    "content": "hello from derouter test2",
                }
            ],
        )
        print(response)
        ...
        response = router.completion(
            ...
            messages=[
                {
                    "role": "user",
                    "content": "hello from derouter test3",
                }
            ],
            stream=True,
        )
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "cohere.command-r-plus-v1:0"},
            }
        ]
    )
...
def test_consistent_model_id():
    """
    - For a given model group + derouter params, assert the model id is always the same
...
def test_reading_keys_os_environ():
    ...
    try:
        ...
        for model in router.model_list:
            assert (
                model["derouter_params"]["api_key"] == os.environ["AZURE_AI_API_KEY"]
            ), f"{model['derouter_params']['api_key']} vs {os.environ['AZURE_AI_API_KEY']}"
            assert (
                model["derouter_params"]["api_base"] == os.environ["AZURE_AI_API_BASE"]
            ), f"{model['derouter_params']['api_base']} vs {os.environ['AZURE_AI_API_BASE']}"
            assert (
                model["derouter_params"]["api_version"]
                == os.environ["AZURE_API_VERSION"]
            ), f"{model['derouter_params']['api_version']} vs {os.environ['AZURE_API_VERSION']}"
            assert float(model["derouter_params"]["timeout"]) == float(
                os.environ["AZURE_TIMEOUT"]
            ), f"{model['derouter_params']['timeout']} vs {os.environ['AZURE_TIMEOUT']}"
            assert float(model["derouter_params"]["stream_timeout"]) == float(
                os.environ["AZURE_STREAM_TIMEOUT"]
            ), f"{model['derouter_params']['stream_timeout']} vs {os.environ['AZURE_STREAM_TIMEOUT']}"
            assert int(model["derouter_params"]["max_retries"]) == int(
                os.environ["AZURE_MAX_RETRIES"]
            ), f"{model['derouter_params']['max_retries']} vs {os.environ['AZURE_MAX_RETRIES']}"
...
def test_reading_openai_keys_os_environ():
    ...
    try:
        ...
        for model in router.model_list:
            assert (
                model["derouter_params"]["api_key"] == os.environ["OPENAI_API_KEY"]
            ), f"{model['derouter_params']['api_key']} vs {os.environ['AZURE_AI_API_KEY']}"
            assert float(model["derouter_params"]["timeout"]) == float(
                os.environ["AZURE_TIMEOUT"]
            ), f"{model['derouter_params']['timeout']} vs {os.environ['AZURE_TIMEOUT']}"
...
def test_router_add_deployment():
    ...
    router.add_deployment(
        deployment=Deployment(
            ...
            derouter_params=DeRouter_Params(model="gpt-3.5-turbo-instruct"),
        ...
        for _ in range(300):
            tasks.append(
                router.atext_completion(
                    model="fake-openai-endpoint",
                    prompt="hello from derouter test",
                )
            )
...
@pytest.fixture
def mock_response() -> derouter.ModelResponse:
...
async def test_router_model_usage(mock_response):
    ...
    setattr(
        mock_response,
        "usage",
        derouter.Usage(prompt_tokens=5, completion_tokens=5, total_tokens=10),
    )
...
def test_router_context_window_pre_call_check(model, base_model, llm_provider):
    ...
    try:
        ...
        derouter.token_counter = MagicMock()

        def token_counter_side_effect(*args, **kwargs):
            # Process args and kwargs if needed
            return 1000000

        derouter.token_counter.side_effect = token_counter_side_effect
        try:
...
def test_router_cooldown_api_connection_error():
    from derouter.router_utils.cooldown_handlers import _is_cooldown_required

    with pytest.raises(derouter.APIConnectionError) as exc_info:
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "vertex_ai/gemini-1.5-pro"},
            }
        ]
    )
...
async def test_aaarouter_dynamic_cooldown_message_retry_time(sync_mode):
    ...
    mock_exception = derouter.RateLimitError(
    ...
    if sync_mode:
        cooldown_deployments = _get_cooldown_deployments(
            derouter_router_instance=router, parent_otel_span=None
        )
    else:
    ...
    if sync_mode:
        with pytest.raises(derouter.types.router.RouterRateLimitError) as exc_info:
            router.embedding(
                model="text-embedding-ada-002",
                input="Hello world!",
...
def test_model_group_alias(hidden):
    _model_list = [
        ...
        {"model_name": "gpt-4", "derouter_params": {"model": "gpt-4"}},
    ]
    router = Router(
...
def test_get_team_specific_model():
    ...
    deployment = DeploymentTypedDict(
        model_name="model-z", derouter_params={}, model_info=ModelInfo()
    )
    assert router._get_team_specific_model(deployment, "team1") is None
...
def test_router_dynamic_credentials():
    ...
    assert deployment is not None
    assert deployment.derouter_params.api_key == original_api_key
