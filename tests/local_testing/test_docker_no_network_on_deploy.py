...
This test verifies that the DeRouter proxy container does not make outbound
...
class TestDockerNoNetworkOnDeploy:
    ...
    TEST_IMAGE_NAME = "derouter-no-network-test"
    TEST_CONTAINER_NAME = "derouter-no-network-test-container"
    ...
    def _build_test_image(self) -> bool:
        ...
        """
        # Check if main derouter image exists
        result = subprocess.run(
            ["docker", "images", "-q", "derouter/derouter"],
            capture_output=True,
            text=True,
            timeout=10,
        )
...
"""
        ...
        config_path = "/tmp/derouter_test_config.yaml"
        with open(config_path, "w") as f:
            f.write(minimal_config)

        image_to_use = "derouter/derouter"
        ...
        run_cmd = [
            ...
            "-e",
            "DEROUTER_MASTER_KEY=sk-test-1234",
            "-e",
