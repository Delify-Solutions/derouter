...
import derouter
from derouter import aembedding, completion, embedding
...
async def test_dual_cache_async_batch_get_cache():
    ...
    """
    from derouter.caching.caching import DualCache, InMemoryCache, RedisCache
...
def test_caching_dynamic_args():  # test in memory cache
    try:
        ...
        derouter.cache = None  # disable cache
...
async def test_caching_with_cache_controls(sync_flag):
    try:
        ...
        if sync_flag:
            ...
            ## TTL = 0
            response1 = await derouter.acompletion(
                model="gpt-3.5-turbo",
                messages=messages,
                cache={"ttl": 0},
                mock_response="Hello world",
            ...
            response2 = await derouter.acompletion(
                model="gpt-3.5-turbo",
                messages=messages,
                cache={"s-maxage": 10},
                mock_response="Hello world",
...
def test_caching_with_models_v2():
    messages = [
        {"role": "user", "content": "who is ishaan CTO of derouter from derouter 2023"}
    ]
    derouter.cache = Cache()
...
def c():
    derouter.enable_caching_on_provider_specific_optional_params = True
    messages = [
        {"role": "user", "content": "who is ishaan CTO of derouter from derouter 2023"}
    ]
    derouter.cache = Cache()
    ...
    if (
        ...
        print(f"response2: {response2}")
        pytest.fail(f"Error occurred:")
    derouter.enable_caching_on_provider_specific_optional_params = False
...
def test_embedding_caching_azure():
    ...
    embedding1 = embedding(
        ...
        input=["good morning from derouter", "this is another item"],
        api_key=api_key,
        api_base=api_base,
...
async def test_embedding_caching_redis_ttl():
    ...
    issue: https://github.com/Delify-Solutions/derouter/issues/6010
    """
    derouter.set_verbose = True
    ...
    with patch("redis.asyncio.Redis.pipeline", return_value=mock_pipeline):
        ...
        embedding_val_1 = await derouter.aembedding(
            model="text-embedding-ada-002",
            input=inputs,
            encoding_format="base64",
        )
...
async def test_redis_cache_basic():
    ...
    cache_key = derouter.cache.get_cache_key(
        model="gpt-3.5-turbo",
        messages=messages,
    )
    print(f"cache_key: {cache_key}")
    derouter.cache.add_cache(result=response1, cache_key=cache_key)
    print(f"cache key pre async get: {cache_key}")
    stored_val = await derouter.cache.async_get_cache(
        model="gpt-3.5-turbo",
        messages=messages,
    )
...
async def test_redis_batch_cache_write():
    ...
    response4 = await derouter.acompletion(
        model="gpt-3.5-turbo",
        messages=messages,
    )
...
async def test_redis_cache_cluster_init_unit_test():
    try:
        ...
        resp = derouter.Cache(type="redis", redis_startup_nodes=startup_nodes)
...
async def test_redis_cache_cluster_init_with_env_vars_unit_test():
    try:
        ...
        resp = derouter.Cache(type="redis")
...
async def test_redis_cache_atext_completion():
    try:
        ...
        response1 = await derouter.atext_completion(
        ...
        response2 = await derouter.atext_completion(
...
async def test_redis_cache_acompletion_stream_bedrock():
    ...
    try:
        ...
        derouter.cache = Cache(type="redis")
        print("test for caching, streaming + completion")
        response_1_content = ""
        response_2_content = ""
...
async def test_s3_cache_acompletion_azure():
    ...
    try:
        ...
        derouter.cache = Cache(
            type="s3",
            s3_bucket_name="derouter-my-test-bucket-2",
...
def test_custom_redis_cache_with_key():
    ...
    derouter.cache.get_cache_key = custom_get_cache_key
    ...
    derouter.cache.cache.set_cache = set_cache
    derouter.cache.cache.get_cache = get_cache
...
def test_cache_override():
    # test if we can override the cache, when `caching=False` but derouter.cache = Cache() is set
...
def test_custom_redis_cache_params():
    ...
    try:
        ...
        print(derouter.cache.cache.redis_client)
        derouter.cache = None
        derouter.success_callback = []
        derouter._async_success_callback = []
    except Exception as e:
    ...
    try:
        ...
        cache_key = cache_instance.get_cache_key(
            **{
                ...
                "temperature": 0.2,
                "stream": True,
                "derouter_call_id": "ffe75e7e-8a07-431f-9a74-71a5b9f35f0b",
        ...
        embedding_cache_key_2 = cache_instance.get_cache_key(
            **{
                ...
                "derouter_call_id": "2642e009-b3cd-443d-b5dd-bb7d56123b0e",
                "derouter_logging_obj": "<derouter.utils.Logging object at 0x12f1bddb0>",
            }
        )
...
def test_cache_context_managers():
    ...
    assert derouter.cache == None
    assert "cache" not in derouter.success_callback
    assert "cache" not in derouter._async_success_callback
    ...
    assert derouter.cache != None
    assert derouter.cache.type == "redis"

    print("VARS of derouter.cache", vars(derouter.cache))
...
def test_caching_redis_simple(caplog, capsys):
    """
    Relevant issue - https://github.com/Delify-Solutions/derouter/issues/4511
    """
    derouter.set_verbose = True  ## REQUIRED FOR TEST.
    derouter.cache = Cache(
...
async def test_cache_default_off_acompletion():
    ...
    from derouter.caching.caching import CacheMode
    ...
    response3 = await derouter.acompletion(
        model="gpt-3.5-turbo",
        messages=[
            {
                "role": "user",
...
async def test_redis_sentinel_caching():
    ...
    derouter.cache.add_cache(result=response1, cache_key=cache_key)
    print(f"cache key pre async get: {cache_key}")
    stored_val = derouter.cache.get_cache(
        model="gpt-3.5-turbo",
        messages=messages,
    )
    ...
    stored_val_2 = await derouter.cache.async_get_cache(
        model="gpt-3.5-turbo",
        messages=messages,
    )
...
async def test_redis_proxy_batch_redis_get_cache():
    ...
    from derouter.caching.caching import Cache, DualCache
    from derouter.proxy._types import UserAPIKeyAuth
    from derouter.proxy.hooks.batch_redis_get import _PROXY_BatchRedisRequests
...
async def test_logging_turn_off_message_logging_streaming(sync_mode):
    ...
    with (
        ...
        if sync_mode is True:
            ...
            resp = await derouter.acompletion(
                model="gpt-3.5-turbo",
                messages=[{"role": "user", "content": "hi"}],
                mock_response="hello",
                stream=True,
...
def test_basic_caching_import():
    from derouter.caching import Cache
...
async def test_caching_kwargs_input(sync_mode):
    ...
    input = {
        ...
        "kwargs": {
            ...
            "caching": True,
            "derouter_call_id": "fae2aa4f-9f75-4f11-8c9c-63ab8d9fae26",
...
async def test_audio_caching(stream):
    derouter.cache = Cache(type="local")

    ## CALL 1 - no cache hit
    completion = await derouter.acompletion(
...
def test_redis_caching_default_ttl():
    ...
    derouter.default_redis_ttl = 120
...
async def test_redis_get_ttl():
    ...
    test that derouter redis caching wrapper handles -1 and -2 values and returns them as None
    """
    try:
        from derouter.caching.redis_cache import RedisCache
...
def test_redis_caching_multiple_namespaces():
    ...
    messages = [{"role": "user", "content": f"what is derouter? {test_uuid}"}]

    # Mock the Redis client creation from the _redis module
    with (
        patch("derouter._redis.get_redis_client") as mock_get_redis_client,
        patch(
            "derouter._redis.get_redis_connection_pool"
...
def test_caching_with_reasoning_content():
    ...
    try:
        messages = [{"role": "user", "content": f"what is derouter? {uuid.uuid4()}"}]
        derouter.cache = Cache()
        ...
        response_1 = completion(
            ...
            mock_response="DeRouter is a unified API interface for LLMs.",
        )
...
async def test_cache_key_in_hidden_params_acompletion():
    ...
    Validates fix for missing x-derouter-cache-key header on proxy cache hits.
    """
    derouter.cache = Cache(
        type="redis",
        host=os.environ["REDIS_HOST"],
