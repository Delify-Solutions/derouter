...
import derouter
from derouter import create_thread, get_thread
from derouter.llms.openai.openai import (
...
async def test_get_assistants(provider, sync_mode, assistant_client):
    ...
    if sync_mode:
        assistants = derouter.get_assistants(**data)
        assert isinstance(assistants, SyncCursorPage)
    else:
        assistants = await derouter.aget_assistants(**data)
...
async def test_create_delete_assistants(provider, sync_mode, assistant_client):
    ...
    if sync_mode:
        assistant = derouter.create_assistants(**data)
        assert isinstance(assistant, Assistant)
        assert assistant.instructions == ASSISTANT_INSTRUCTIONS
        assert assistant.id is not None

        response = derouter.delete_assistant(
            **_request_data(
                provider,
                assistant_client,
        ...
        assistant = await derouter.acreate_assistants(**data)
        assert isinstance(assistant, Assistant)
        assert assistant.instructions == ASSISTANT_INSTRUCTIONS
        assert assistant.id is not None

        response = await derouter.adelete_assistant(
            **_request_data(
                provider,
                assistant_client,
                assistant_id=assistant.id,
...
async def _create_thread_derouter(sync_mode, provider, assistant_client) -> Thread:
    ...
    if sync_mode:
        ...
        new_thread = await derouter.acreate_thread(**data)
...
@pytest.mark.parametrize("sync_mode", [True, False])
@pytest.mark.asyncio
async def test_create_thread_derouter(sync_mode, provider, assistant_client):
    await _create_thread_derouter(sync_mode, provider, assistant_client)


@pytest.mark.parametrize("provider", ["openai", "azure"])
@pytest.mark.parametrize("sync_mode", [True, False])
@pytest.mark.asyncio
async def test_get_thread_derouter(provider, sync_mode, assistant_client):
    new_thread = await _create_thread_derouter(sync_mode, provider, assistant_client)
    data = _request_data(provider, assistant_client, thread_id=new_thread.id)

    if sync_mode:
        received_thread = get_thread(**data)
    else:
        received_thread = await derouter.aget_thread(**data)
...
@pytest.mark.parametrize("sync_mode", [True, False])
@pytest.mark.asyncio
async def test_add_message_derouter(sync_mode, provider, assistant_client):
    new_thread = await _create_thread_derouter(sync_mode, provider, assistant_client)
    message: MessageData = {"role": "user", "content": "Hey, how's it going?"}  # type: ignore
    data = _request_data(provider, assistant_client, thread_id=new_thread.id, **message)

    if sync_mode:
        added_message = derouter.add_message(**data)
    else:
        added_message = await derouter.a_add_message(**data)
...
@pytest.mark.asyncio
async def test_aarun_thread_derouter(
    sync_mode, provider, is_streaming, assistant_client
):
    get_assistants_data = _request_data(provider, assistant_client)
    if sync_mode:
        assistants = derouter.get_assistants(**get_assistants_data)
    else:
        assistants = await derouter.aget_assistants(**get_assistants_data)
    ...
    if sync_mode:
        added_message = derouter.add_message(**message_data)
        assert isinstance(added_message, Message)

        if is_streaming:
            run = derouter.run_thread_stream(assistant_id=assistant_id, **thread_data)
            with run as run:
                assert isinstance(run, AssistantEventHandler)
                run.until_done()
        else:
            run = derouter.run_thread(
                assistant_id=assistant_id, stream=is_streaming, **thread_data
            )
            assert run.status == "completed"
            messages = derouter.get_messages(**thread_data)
            assert isinstance(messages.data[0], Message)
    else:
        added_message = await derouter.a_add_message(**message_data)
        assert isinstance(added_message, Message)

        if is_streaming:
            run = derouter.arun_thread_stream(assistant_id=assistant_id, **thread_data)
            async with run as run:
                assert isinstance(run, AsyncAssistantEventHandler)
                await run.until_done()
        else:
            run = await derouter.arun_thread(
            ...
            assert run.status == "completed"
            messages = await derouter.aget_messages(**thread_data)
            assert isinstance(messages.data[0], Message)
