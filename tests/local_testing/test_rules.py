...
def test_pre_call_rule():
    try:
        derouter.pre_call_rules = [my_pre_call_rule]
        ### completion
        response = completion(
            model="gpt-3.5-turbo",
...
def my_post_call_rule(input: str):
    ...
    if len(input) < 200:
        return {
            "decision": False,
            "message": "This violates DeRouter Proxy Rules. Response too short",
        }
...
def test_post_call_rule():
    derouter.pre_call_rules = []
    derouter.post_call_rules = [my_post_call_rule]

    ### completion
    with pytest.raises(Exception, match=re.escape("This violates DeRouter Proxy Rules. Response too short")) as exc_info:
        completion(
            model="gpt-3.5-turbo",
            messages=[{"role": "user", "content": "say sorry"}],
            max_tokens=2,
        )
    assert exc_info.value.message == "This violates DeRouter Proxy Rules. Response too short"
...
def test_post_call_rule_streaming():
    derouter.pre_call_rules = []
    derouter.post_call_rules = [my_post_call_rule_2]
    ### completion
    response = completion(
        model="gpt-3.5-turbo",
        messages=[{"role": "user", "content": "say sorry"}],
    ...
    with pytest.raises(Exception, match=re.escape("This violates DeRouter Proxy Rules. Response too short")) as exc_info:
        list(response)
    assert "This violates DeRouter Proxy Rules. Response too short" in exc_info.value.message
