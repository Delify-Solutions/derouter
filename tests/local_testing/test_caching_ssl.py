#### What this tests ####
#    This tests using caching w/ derouter which requires SSL=True
...
import pytest
import derouter
from derouter import embedding, completion, Router
...
def test_caching_router():
    """
    Test scenario where derouter.cache is set but kwargs("caching") is not. This should still return a cache hit.
    """
    try:
        model_list = [
            {
...
async def test_redis_with_ssl():
    ...
    """
    from derouter._redis import get_redis_connection_pool, get_redis_async_client
