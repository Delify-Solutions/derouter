...
import derouter
from derouter import RateLimitError, completion
...
#     derouter.HuggingfaceConfig(max_new_tokens=200)
#     derouter.set_verbose=True
#     try:
#         # OVERRIDE WITH DYNAMIC MAX TOKENS
#         response_1 = derouter.completion(
...
#         # USE CONFIG TOKENS
#         response_2 = derouter.completion(
#             model="huggingface/mistralai/Mistral-7B-Instruct-v0.1",
#             messages=[{ "content": "Hello, how are you?","role": "user"}],
#             api_base="https://n9ox93a8sv5ihsow.us-east-1.aws.endpoints.huggingface.cloud",
#         )
...
def claude_test_completion():
    derouter.AnthropicConfig(max_tokens_to_sample=200)
    # derouter.set_verbose=True
    try:
        # OVERRIDE WITH DYNAMIC MAX TOKENS
        response_1 = derouter.completion(
        ...
        try:
            response_3 = derouter.completion(
...
def replicate_test_completion():
    derouter.ReplicateConfig(max_new_tokens=200)
    # derouter.set_verbose=True
    try:
        # OVERRIDE WITH DYNAMIC MAX TOKENS
        response_1 = derouter.completion(
...
def cohere_test_completion():
    # derouter.CohereConfig(max_tokens=200)
    derouter.set_verbose = True
    try:
        # OVERRIDE WITH DYNAMIC MAX TOKENS
        response_1 = derouter.completion(
...
def togetherai_test_completion():
    derouter.TogetherAIConfig(max_tokens=10)
    derouter.set_verbose = True
    try:
        # OVERRIDE WITH DYNAMIC MAX TOKENS
        response_1 = derouter.completion(
...
def nlp_cloud_test_completion():
    derouter.NLPCloudConfig(max_length=10)
    # derouter.set_verbose=True
    try:
        # OVERRIDE WITH DYNAMIC MAX TOKENS
        response_1 = derouter.completion(
...
def aleph_alpha_test_completion():
    derouter.AlephAlphaConfig(maximum_tokens=10)
    # derouter.set_verbose=True
    try:
        # OVERRIDE WITH DYNAMIC MAX TOKENS
        response_1 = derouter.completion(
...
#     derouter.PetalsConfig(max_new_tokens=10)
#     # derouter.set_verbose=True
#     try:
#         # OVERRIDE WITH DYNAMIC MAX TOKENS
#         response_1 = derouter.completion(
...
#     derouter.VertexAIConfig(max_output_tokens=10)
#     # derouter.set_verbose=True
#     try:
#         # OVERRIDE WITH DYNAMIC MAX TOKENS
#         response_1 = derouter.completion(
...
def sagemaker_test_completion():
    derouter.SagemakerConfig(max_new_tokens=10)
    # derouter.set_verbose=True
    try:
        # OVERRIDE WITH DYNAMIC MAX TOKENS
        response_1 = derouter.completion(
...
def bedrock_test_completion():
    derouter.AmazonCohereConfig(max_tokens=10)
    # derouter.set_verbose=True
    try:
        # OVERRIDE WITH DYNAMIC MAX TOKENS
        response_1 = derouter.completion(
...
def openai_test_completion():
    derouter.OpenAIConfig(max_tokens=10)
    # derouter.set_verbose=True
    try:
        # OVERRIDE WITH DYNAMIC MAX TOKENS
        response_1 = derouter.completion(
...
def openai_text_completion_test():
    derouter.OpenAITextCompletionConfig(max_tokens=10)
    # derouter.set_verbose=True
    try:
        # OVERRIDE WITH DYNAMIC MAX TOKENS
        response_1 = derouter.completion(
...
def azure_openai_test_completion():
    derouter.AzureOpenAIConfig(max_tokens=10)
    # derouter.set_verbose=True
    try:
        # OVERRIDE WITH DYNAMIC MAX TOKENS
        response_1 = derouter.completion(
