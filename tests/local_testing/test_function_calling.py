...
def test_parallel_function_call_anthropic_error_msg(
    ...
    - Bedrock **Converse** still raises ``UnsupportedParamsError`` unless
      ``derouter.modify_params`` is enabled (dummy tool is only added there).
    ...
    Reference Issue: https://github.com/Delify-Solutions/derouter/issues/5747, https://github.com/Delify-Solutions/derouter/issues/5388
    """
    ...
    try:
        ...
        if expect_unsupported_params_error:
            with pytest.raises(derouter.UnsupportedParamsError) as e:
                derouter.completion(
                    model=model,
                    messages=messages,
                    temperature=0.2,
