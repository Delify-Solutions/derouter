...
def test_update_kwargs_before_fallbacks_unit_test():
    ...
    assert kwargs["derouter_trace_id"] is not None
...
async def test_update_kwargs_before_fallbacks(call_type):
    ...
    if call_type.value.startswith("a"):
        with patch.object(router, "async_function_with_fallbacks") as mock_client:
            ...
            assert mock_client.call_args.kwargs["derouter_trace_id"] is not None
...
async def test_router_model_group_headers():
    ...
    assert (
        resp._hidden_params["additional_headers"]["x-derouter-model-group"]
...
def test_router_redis_cache():
    router = Router(
        model_list=[{"model_name": "gemini/*", "derouter_params": {"model": "gemini/*"}}]
    )
...
def test_router_handle_clientside_credential():
    ...
    assert new_deployment.derouter_params.api_key == "123"
...
def test_router_get_deployment_credentials():
    router = Router(
        model_list=[
            {
                "model_name": "gemini/*",
                "derouter_params": {"model": "gemini/*", "api_key": "123"},
                "model_info": {"id": "1"},
            }
        ]
    )
