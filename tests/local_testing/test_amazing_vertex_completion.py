...
async def test_get_response():
    ...
    try:
        ...
        pass
    except derouter.UnprocessableEntityError as e:
        pass
...
def test_avertex_ai():
    ...
    load_vertex_ai_credentials()
    test_models = (
        derouter.vertex_chat_models
        | derouter.vertex_code_chat_models
        | derouter.vertex_text_models
        | derouter.vertex_code_text_models
    )
    derouter.set_verbose = False
    vertex_ai_project = "pathrise-convert-1606954137718"

    test_models = random.sample(list(test_models), 1)
    test_models += list(derouter.vertex_language_models)  # always test gemini-pro
    for model in test_models:
        try:
            ...
            assert response.choices[0].finish_reason in derouter._openai_finish_reasons
        except derouter.RateLimitError as e:
            pass
...
def test_avertex_ai_stream():
    load_vertex_ai_credentials()
    derouter.set_verbose = True
    derouter.vertex_project = "pathrise-convert-1606954137718"
    import random
...
async def test_async_vertexai_response_basic():
    ...
    except derouter.NotFoundError as e:
        pass
    except derouter.RateLimitError as e:
        pass
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "us-central1",
        ),  # critical - we had this issue: https://github.com/Delify-Solutions/derouter/issues/13888
        ("vertex_ai/openai/gpt-oss-20b-maas", "us-central1"),
    ],
)
@pytest.mark.parametrize(
...
async def test_gemini_pro_json_schema_args_sent_httpx(
    ...
    with patch.object(client, "post", new=httpx_response) as mock_call:
        ...
        except derouter.JSONSchemaValidationError as e:
...
async def test_anthropic_message_via_anthropic_messages():
    from derouter.llms.custom_httpx.llm_http_handler import AsyncHTTPHandler
    ...
    with patch.object(client, "post", new=httpx_response) as mock_call:
        messages = [{"role": "user", "content": "List 5 cookie recipes"}]
        response = await derouter.anthropic_messages(
    ...
    with patch.object(client, "post", new=httpx_response) as mock_call:
        response_2 = await derouter.acompletion(
            model="vertex_ai/claude-3-5-sonnet@20240620",
            messages=messages,
            max_tokens=100,
            client=client,
...
async def test_gemini_pro_function_calling_streaming(sync_mode):
    ...
    try:
        if sync_mode == True:
            ...
            for chunk in response:
                chunks.append(chunk)
                assert isinstance(chunk, derouter.ModelResponseStream)
        else:
            response = await derouter.acompletion(**data)
            print(f"completion: {response}")

            assert isinstance(response, derouter.CustomStreamWrapper)
        ...
        complete_response = derouter.stream_chunk_builder(chunks=chunks)
        assert (
...
async def test_vertexai_embedding(sync_mode):
    try:
        ...
        input_text = ["good morning from derouter", "this is another item"]
...
async def test_vertexai_multimodal_embedding():
    ...
    with patch(
        ...
        # Act: Call the derouter.aembedding function
        response = await derouter.aembedding(
...
async def test_completion_fine_tuned_model():
    ...
    with patch(
        ...
        # Act: Call the derouter.completion function
        ...
        assert (
            url[0]
            == "https://us-central1-aiplatform.googleapis.com/v1/projects/derouter-ci-cd/locations/us-central1/endpoints/4965075652664360960:generateContent"
        )
...
async def test_gemini_context_caching_disabled_flag(sync_mode):
    ...
    original_flag_value = derouter.disable_anthropic_gemini_context_caching_transform

    try:
        # Enable the disable flag
        derouter.disable_anthropic_gemini_context_caching_transform = True
        ...
        derouter.disable_anthropic_gemini_context_caching_transform = original_flag_value
...
async def test_partner_models_httpx_ai21():
    ...
    with patch(
        ...
        assert (
            url[0]
            == "https://us-central1-aiplatform.googleapis.com/v1beta1/projects/derouter-ci-cd/locations/us-central1/publishers/ai21/models/jamba-1.5-mini@001:rawPredict"
        )
...
def test_gemini_function_call_parameter_in_messages():
    ...
    with patch(
        "derouter.llms.vertex_ai.vertex_llm_base.VertexBase._ensure_access_token",
...
async def test_vertexai_embedding_finetuned(respx_mock: MockRouter):
    ...
    - Response is properly parsed into derouter's embedding response format
    """
    load_vertex_ai_credentials()
    derouter.set_verbose = True
    derouter.disable_aiohttp_transport = (
    ...
    expected_request = {
        "instances": [
            {"inputs": "good morning from derouter"},
...
async def test_vertexai_model_garden_model_completion(
    ...
    """
    Relevant issue: https://github.com/Delify-Solutions/derouter/issues/6480
    ...
    derouter.module_level_aclient = httpx.AsyncClient()
    load_vertex_ai_credentials()
    derouter.set_verbose = True
...
def test_derouter_api_base(monkeypatch, provider, route):
    from derouter.llms.custom_httpx.http_handler import HTTPHandler
    ...
    monkeypatch.setattr(derouter, "api_base", "https://derouter.com")
    ...
    with patch.object(client, "post", new=MagicMock()) as mock_client:
        ...
        mock_client.assert_called()
        assert mock_client.call_args.kwargs["url"].startswith("https://derouter.com")
...
def test_vertex_ai_response_id():
    """Test that derouter preserves the response ID from Vertex AI's API for non-streaming responses"""
    from derouter.llms.custom_httpx.http_handler import HTTPHandler
...
def test_vertex_ai_streaming_response_id():
    """Test that derouter preserves the response ID from Vertex AI's API for streaming responses"""
    from derouter.llms.custom_httpx.http_handler import HTTPHandler
...
def test_gemini_google_maps_tool_simple():
    ...
    except (derouter.RateLimitError, derouter.InternalServerError) as e:
        pytest.skip(f"Transient Vertex-side failure, not a DeRouter bug: {e}")
    except Exception as e:
        pytest.fail(f"Error occurred: {e}")
