...
def langfuse_client():
    ...
    _cached_client = derouter.in_memory_llm_clients_cache.get_cache(_langfuse_cache_key)
    if _cached_client:
        ...
        derouter.in_memory_llm_clients_cache.set_cache(
...
def create_async_task(**completion_kwargs):
    """
    Creates an async task for the derouter.acompletion function.
    ...
    Any kwargs passed to this function will be passed to the derouter.acompletion function.
    By default a standard set of arguments are used for the derouter.acompletion function.
    """
    completion_args = {
    ...
    return asyncio.create_task(derouter.acompletion(**completion_args))
...
async def test_langfuse_logging_without_request_response(stream, langfuse_client):
    try:
        ...
        _unique_trace_name = f"derouter-test-{str(uuid.uuid4())}"
        derouter.set_verbose = True
        ...
        assert _trace_data[0].input == {
            "messages": [{"content": "redacted-by-derouter", "role": "user"}]
        }
        assert _trace_data[0].output == {
            "role": "assistant",
            "content": "redacted-by-derouter",
            "function_call": None,
            "tool_calls": None,
        }
...
async def test_langfuse_logging_audio_transcriptions(langfuse_client):
    ...
    assert generations[0].name == "derouter-atranscription"
...
async def test_langfuse_masked_input_output(langfuse_client):
    ...
    for mask_value in [True, False]:
        ...
        print(response)
        expected_input = "redacted-by-derouter" if mask_value else "This is a test"
        expected_output = (
            "redacted-by-derouter" if mask_value else "This is a test response"
        )
        langfuse_client.flush()
...
async def test_aaalangfuse_logging_metadata(langfuse_client):
    ...
    session_id = f"derouter-test-session-{run_id}"
    trace_common_metadata = {
        "session_id": session_id,
        "tags": ["derouter-test-tag1", "derouter-test-tag2"],
        "update_trace_keys": [
            "output",
            "trace_metadata",
        ...
        "trace_release": "derouter-test-release",
        "version": "derouter-test-version",
    }
    for trace_num in range(1, 3):  # Two traces
        metadata = copy.deepcopy(trace_common_metadata)
        trace_id = f"derouter-test-trace{trace_num}-{run_id}"
        ...
        for generation_num in range(
            ...
            metadata["trace_user_id"] = f"derouter-test-user{generation_num}-{run_id}"
            generation_id = (
                f"derouter-test-trace{trace_num}-generation-{generation_num}-{run_id}"
            )
...
@pytest.mark.asyncio
@pytest.mark.skip(
    reason="local only test, use this to verify if we can send request to derouter proxy server"
)
...
generation_params = {
    ...
    "metadata": {
        ...
        "hidden_params": {
            ...
            "derouter_call_id": "1fa31658-20af-40b5-9ac9-60fd7b5ad98c",
        ...
        "derouter_response_cost": 0.00038,
...
def test_langfuse_prompt_type(prompt):
    ...
    from derouter.integrations.langfuse.langfuse import _add_prompt_to_generation_params
...
def test_langfuse_logging_metadata():
    from derouter.integrations.langfuse.langfuse import log_requester_metadata
