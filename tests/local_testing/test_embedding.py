...
import derouter
from derouter import completion, completion_cost, embedding
...
def test_openai_embedding():
    try:
        ...
        derouter_response = dict(response)
        derouter_response_keys = set(derouter_response.keys())
        derouter_response_keys.discard("_response_ms")

        print(derouter_response_keys)
        print("DeRouter Response\n")
        # print(derouter_response)
        ...
        assert (
            derouter_response_keys == openai_response_keys
        )  # ENSURE the Keys in derouter response is exactly what the openai package returns
        assert (
            len(derouter_response["data"]) == 2
        )  # expect two embedding responses from derouter_response since input had two
        print(openai_response_keys)
...
async def test_together_ai_embedding(model, api_base, api_key, sync_mode):
    try:
        ...
        assert set(["usage", "model", "object", "data"]) == set(
            response_keys
        )  # assert derouter response has expected keys from OpenAI embedding response
        ...
        assert isinstance(response.usage, derouter.Usage)
...
async def test_azure_ai_embedding_image(sync_mode):
    ...
    try:
        ...
        request_cost = derouter.completion_cost(completion_response=response)
...
async def test_bedrock_embedding_titan(model, sync_mode):
    try:
        ...
        if sync_mode:
            response = embedding(
                model=model,
                input=f"good morning from derouter, attempting to embed data {current_time}",  # input should always be a string in this test
...
def test_bedrock_embedding_cohere():
    try:
        ...
        response = embedding(
            ...
            input=[
                "good morning from derouter, attempting to embed data",
...
def test_demo_tokens_as_input_to_embeddings_fails_for_titan():
    ...
    with pytest.raises(
        ...
        match=re.escape(
            'derouter.BadRequestError: BedrockException - {"message":"Malformed input request: '
            'expected type: String, found: JSONArray, please reformat your input and try again."}'
        ),
    ):
        derouter.embedding(model="amazon.titan-embed-text-v1", input=[[1]])
...
def tgi_mock_post(*args, **kwargs):
    ...
    expected_data = {
        "inputs": {
            "source_sentence": "good morning from derouter",
...
@pytest.mark.asyncio
@patch(
    "derouter.llms.huggingface.embedding.handler.async_get_hf_task_embedding_for_model"
)
@patch("derouter.llms.huggingface.embedding.handler.get_hf_task_embedding_for_model")
@pytest.mark.parametrize("sync_mode", [True, False])
    ...
    try:
        ...
        with patch.object(client, "post", side_effect=tgi_mock_post) as mock_client:
            ...
            if sync_mode is True:
                ...
                response = await derouter.aembedding(**data)
...
@pytest.mark.parametrize("sync_mode", [True, False])
@pytest.mark.parametrize(
    "input", ["good morning from derouter", ["good morning from derouter"]]  #
)
@pytest.mark.asyncio
...
def test_embedding_response_ratelimit_headers(model):
    ...
    # if upstream api returns this header, we need to verify the header is transformed by derouter
    if (
    ...
    with patch.object(client, "post") as mock_post:
        try:
            if sync_mode:
                ...
                await derouter.aembedding(**data)
        except Exception as e:
            print(e)
...
def test_jina_ai_img_embeddings(input_data, expected_payload_input):
    ...
    This test verifies that when derouter.embedding is called with a jina_ai model,
    ...
    with patch("derouter.llms.custom_httpx.http_handler.HTTPHandler.post") as mock_post:
        # Configure the mock to return a successful, minimal valid response.
        # This prevents derouter from raising an error when processing the response.
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "object": "list",
        ...
        try:
            derouter.embedding(model="jina_ai/jina-embeddings-v4", input=input_data)
        except Exception as e:
            pytest.fail(
                f"derouter.embedding call failed with an unexpected exception: {e}"
            )
...
def test_encoding_format_omitted_by_default_for_openai_sdk(monkeypatch):
    """
    When encoding_format is not provided, DeRouter leaves it out of the upstream request.

    Optional global override: `DEROUTER_DEFAULT_EMBEDDING_ENCODING_FORMAT`.
    """
    monkeypatch.delenv("DEROUTER_DEFAULT_EMBEDDING_ENCODING_FORMAT", raising=False)
    captured_bodies = []
...
def test_encoding_format_explicit_value_preserved():
    ...
    """
    with patch(
        "derouter.llms.openai.openai.OpenAIChatCompletion._get_openai_client"
    ) as mock_get_client:
