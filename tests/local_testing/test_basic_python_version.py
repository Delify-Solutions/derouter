...
def test_using_derouter():
    try:
        import derouter

        print("derouter imported successfully")
    except Exception as e:
        pytest.fail(f"Error occurred: {e}. Installing derouter failed please retry")


def test_derouter_proxy_server():
    # Sync the local derouter[proxy] dependencies into the project environment
    _run_uv("sync", "--frozen", "--extra", "proxy")

    # Import through the uv-managed interpreter that uv sync populated.
    try:
        _run_uv("run", "--no-sync", "python", "-c", "import derouter.proxy.proxy_server")
    except subprocess.CalledProcessError:
        pytest.fail("Failed to import derouter.proxy.proxy_server")
...
def test_package_dependencies():
    ...
    try:
        ...
        # Get the derouter package root path
        derouter_path = pathlib.Path(derouter.__file__).parent.parent
        pyproject_path = derouter_path / "pyproject.toml"
...
def test_cli_extra_is_a_thin_client_install():
    ...
    pyproject_path = pathlib.Path(derouter.__file__).parent.parent / "pyproject.toml"
    with open(pyproject_path, "rb") as f:
...
def _declared_aiohttp_specifier():
    ...
    pytest.fail("aiohttp is no longer a declared runtime dependency of derouter")
...
def test_declared_aiohttp_floor_excludes_pool_poisoning_releases():
    ...
    assert not admitted, (
        f"derouter declares aiohttp{specifier}, which still admits {admitted}. "
...
def _run_proxy_server_smoke_test(extra_proxy_args=None):
    ...
    try:
        ...
        pytest.fail("Failed to import derouter.proxy_server")
...
def test_derouter_proxy_server_config_no_general_settings():
    """Exercises the default (v1) migration resolver."""
    _run_proxy_server_smoke_test()


def test_derouter_proxy_server_config_no_general_settings_v2_resolver():
