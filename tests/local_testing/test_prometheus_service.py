...
import pytest
from derouter import acompletion, Cache
from derouter._service_logger import ServiceLogging
from derouter.integrations.prometheus_services import PrometheusServicesLogger
from derouter.proxy.utils import ServiceTypes
...
async def test_completion_with_caching():
    ...
    derouter.service_callback = ["prometheus_system"]

    sl = ServiceLogging(mock_testing=True)
    sl.prometheusServicesLogger.mock_testing = True
    derouter.cache.cache.service_logger_obj = sl
        ...
        model_list = [
            {
                ...
                "derouter_params": get_openai_params(),
                "tpm": 100,
            },
            {
                "model_name": "azure/gpt-4",
...
def test_get_metric_existing():
    ...
    retrieved_metric = pl._get_metric("derouter_test_service_test_type_of_request")
...
def test_create_histogram_new():
    ...
    assert pl._get_metric("derouter_test_service_test_type_of_request") is hist
...
def test_create_histogram_existing():
    ...
    assert pl._get_metric("derouter_test_service_test_type_of_request") is hist1
...
def test_create_counter_new():
    ...
    assert pl._get_metric("derouter_test_service_test_type_of_request") is counter
...
def test_create_counter_existing():
    ...
    assert pl._get_metric("derouter_test_service_test_type_of_request") is counter1
