...
import derouter
from derouter import Cache, completion, embedding
...
## 1: Pre-API-Call
## 2: Post-API-Call
## 3: On DeRouter Call success
## 4: On DeRouter Call failure
## 5. Caching
...
# Test interfaces
## 1. derouter.completion() + derouter.embeddings()
## refer to test_custom_callback_input_router.py for the router +  proxy tests


class CompletionCustomHandler(
    CustomLogger
):  # https://router.delify.vn/docs/observability/custom_callback#callback-class
    """
    ...
    def log_pre_api_call(self, model, messages, kwargs):
        try:
            ...
            assert isinstance(kwargs["derouter_params"], dict)
            assert isinstance(kwargs["start_time"], (datetime, type(None)))
            assert isinstance(kwargs["stream"], bool)
            assert isinstance(kwargs["user"], (str, type(None)))
            ### METADATA
            metadata_value = kwargs["derouter_params"].get("metadata")
            assert metadata_value is None or isinstance(metadata_value, dict)
            if metadata_value is not None:
                if derouter.turn_off_message_logging is True:
                    assert (
                        metadata_value["raw_request"]
                        is DeRouterCommonStrings.redacted_by_derouter.value
                    )
                else:
        ...
        try:
            ...
            assert (
                isinstance(
                    ...
                    (str, derouter.CustomStreamWrapper, BaseModel),
                )
    ...
    async def async_log_stream_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            ## RESPONSE OBJECT
            assert isinstance(response_obj, derouter.ModelResponseStream)
            ## KWARGS
            assert isinstance(kwargs["model"], str)
            ...
            assert (
                isinstance(
                    kwargs["original_response"], (str, derouter.CustomStreamWrapper)
                )
                or inspect.isasyncgen(kwargs["original_response"])
                or inspect.iscoroutine(kwargs["original_response"])
            )
        ...
        try:
            ...
            assert isinstance(
                ...
                (
                    ...
                    derouter.EmbeddingResponse,
                    derouter.ImageResponse,
                ),
            )
            ## KWARGS
            assert isinstance(kwargs["model"], str)
            ...
            assert isinstance(kwargs["optional_params"], dict)
            assert isinstance(kwargs["derouter_params"], dict)
            assert isinstance(kwargs["derouter_params"]["api_base"], str)
            ...
            assert isinstance(
                kwargs["original_response"],
                (str, derouter.CustomStreamWrapper, BaseModel),
            ), "Original Response={}. Allowed types=[str, derouter.CustomStreamWrapper, BaseModel]".format(
                kwargs["original_response"]
            )
            assert isinstance(kwargs["additional_args"], (dict, type(None)))
            assert isinstance(kwargs["log_event_type"], str)
    ...
    def log_failure_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            assert isinstance(kwargs["optional_params"], dict)
            assert isinstance(kwargs["derouter_params"], dict)
            assert isinstance(kwargs["derouter_params"]["metadata"], Optional[dict])
            assert isinstance(kwargs["start_time"], (datetime, type(None)))
            assert isinstance(kwargs["stream"], bool)
            assert isinstance(kwargs["user"], (str, type(None)))
            assert (
        ...
        try:
            ...
            assert isinstance(
                ...
                (
                    derouter.ModelResponse,
                    derouter.EmbeddingResponse,
                    derouter.TextCompletionResponse,
                ),
            )
            ## KWARGS
            assert isinstance(kwargs["model"], str)
...
async def test_async_text_completion_bedrock():
    try:
        customHandler = CompletionCustomHandler()
        derouter.callbacks = [customHandler]
        response = await derouter.atext_completion(
...
async def test_async_embedding_openai():
    try:
        ...
        derouter.callbacks = [customHandler_success]
        ...
        derouter.callbacks = [customHandler_failure]
        try:
            response = await derouter.aembedding(
                model="text-embedding-ada-002",
                input=["good morning from derouter"],
...
def test_amazing_sync_embedding():
    try:
        ...
        response = derouter.embedding(
            model="azure/text-embedding-ada-002", input=["good morning from derouter"]
        )
        print(f"customHandler_success.errors: {customHandler_success.errors}")
        print(f"customHandler_success.states: {customHandler_success.states}")
        wait_until(
...
def test_image_generation_openai():
    try:
        ...
        pass
    except derouter.ContentPolicyViolationError:
...
def test_standard_logging_payload(model, turn_off_message_logging):
    ...
    derouter.turn_off_message_logging = turn_off_message_logging
    ...
    with patch.object(
        ...
        if turn_off_message_logging:
            print("checks redacted-by-derouter")
            assert "redacted-by-derouter" == slobject["messages"][0]["content"]
            ...
            if "choices" in response:
                ...
                assert response["text"] == "redacted-by-derouter"
    ...
    with patch.object(
        ...
        if turn_off_message_logging:
            ...
            if "choices" in response:
                ...
                if stream:
                    assert redacted_content == "redacted-by-derouter"
                else:
...
def test_logging_async_cache_hit_sync_call(turn_off_message_logging):
    ...
    derouter.callbacks = [primingHandler]
...
def test_logging_standard_payload_failure_call():
    ...
    with patch.object(
        ...
        except derouter.AuthenticationError:
            pass
...
def test_logging_key_masking_gemini():
    ...
    with patch.object(
        ...
        api_base = mock_client.call_args.kwargs["kwargs"]["derouter_params"]["api_base"]
...
def test_derouter_logging_no_log_param(monkeypatch, disable_no_log_param):
    monkeypatch.setattr(derouter, "global_disable_no_log_param", disable_no_log_param)
    ...
    should_run = derouter_logging_obj.should_run_callback(
        callback="langfuse",
        derouter_params={"no-log": True},
