...
# Pattern matches tests/test_derouter/conftest.py:
#   - Function-scoped fixture saves/restores derouter globals (no reload)
...
# `derouter.num_retries = 3`).  The function-scoped fixture resets globals to
...
# ``derouter.model_cost`` is loaded at import time from the URL pinned to ``main``
# (``DEROUTER_MODEL_COST_MAP_URL``).  The in-tree backup ships with this branch
...
for _k, _v in GetModelCostMap.load_local_model_cost_map().items():
    if _k in RESERVED_TOP_LEVEL_KEYS:
        continue
    derouter.model_cost.setdefault(_k, _v)
...
# module's top-level code (e.g. `derouter.num_retries = 3`) executes, so
# the values here are guaranteed to be the real package defaults.
# ---------------------------------------------------------------------------
_SCALAR_DEFAULTS = {
    "num_retries": getattr(derouter, "num_retries", None),
    "num_retries_per_request": getattr(derouter, "num_retries_per_request", None),
    "request_timeout": getattr(derouter, "request_timeout", None),
    "set_verbose": getattr(derouter, "set_verbose", False),
    "cache": getattr(derouter, "cache", None),
    "allowed_fails": getattr(derouter, "allowed_fails", 3),
    "default_fallbacks": getattr(derouter, "default_fallbacks", None),
    "enable_azure_ad_token_refresh": getattr(
        derouter, "enable_azure_ad_token_refresh", None
    ),
    "tag_budget_config": getattr(derouter, "tag_budget_config", None),
    "model_cost": getattr(derouter, "model_cost", None),
    "token_counter": getattr(derouter, "token_counter", None),
    "disable_aiohttp_transport": getattr(derouter, "disable_aiohttp_transport", False),
    "force_ipv4": getattr(derouter, "force_ipv4", False),
    "drop_params": getattr(derouter, "drop_params", None),
    "modify_params": getattr(derouter, "modify_params", False),
    "api_base": getattr(derouter, "api_base", None),
    "api_key": getattr(derouter, "api_key", None),
}
...
def isolate_derouter_state():
    ...
    Resets derouter globals to their true defaults before each test and
    ...
    # module-level code like `derouter.num_retries = 3` in test files)
    for attr, default_val in _SCALAR_DEFAULTS.items():
        if hasattr(derouter, attr):
            setattr(derouter, attr, default_val)
    ...
    for attr, original_value in original_state.items():
        if hasattr(derouter, attr):
            setattr(derouter, attr, original_value)


@pytest.fixture(scope="module", autouse=True)
def setup_and_teardown():
    """
    Module-scoped setup. Reloads derouter only in single-process mode
