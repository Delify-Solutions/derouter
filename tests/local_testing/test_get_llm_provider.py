...
def test_get_llm_provider():
    _, response, _, _ = derouter.get_llm_provider(model="anthropic.claude-v2:1")
...
def test_get_llm_provider_fireworks():  # tests finetuned fireworks models - https://github.com/Delify-Solutions/derouter/issues/4923
...
def test_get_llm_provider_catch_all():
    _, response, _, _ = derouter.get_llm_provider(model="*")
    assert response == "openai"


def test_get_llm_provider_gpt_instruct():
    _, response, _, _ = derouter.get_llm_provider(model="gpt-3.5-turbo-instruct-0914")

    assert response == "text-completion-openai"


def test_get_llm_provider_mistral_custom_api_base():
    model, custom_llm_provider, dynamic_api_key, api_base = derouter.get_llm_provider(
...
def test_default_api_base():
    ...
    with patch.dict(os.environ, {}, clear=True):
        for provider in derouter.openai_compatible_providers:
...
# -------- Tests for force_use_derouter_proxy ---------


def test_get_derouter_proxy_custom_llm_provider():
    """
    Tests force_use_derouter_proxy uses DEROUTER_PROXY_API_BASE and DEROUTER_PROXY_API_KEY from env.
    """
    ...
    with patch.dict(
        os.environ,
        {
            "DEROUTER_PROXY_API_BASE": expected_api_base,
            "DEROUTER_PROXY_API_KEY": expected_api_key,
        },
        clear=True,
    ):
        (
            ...
            key,
            base,
        ) = derouter.DeRouterProxyChatConfig().derouter_proxy_get_custom_llm_provider_info(
            model=test_model
        )

    assert model == test_model
    assert provider == "derouter_proxy"
    assert key == expected_api_key
    assert base == expected_api_base


def test_get_derouter_proxy_with_args_override_env_vars():
    """
    Tests force_use_derouter_proxy uses api_base and api_key args over environment variables.
    """
    test_model = "gpt-4"
    ...
    with patch.dict(
        os.environ,
        {"DEROUTER_PROXY_API_BASE": env_api_base, "DEROUTER_PROXY_API_KEY": env_api_key},
        clear=True,
    ):
        (
            model,
...
def test_get_derouter_proxy_model_prefix_stripping():
    """
    Tests force_use_derouter_proxy strips 'derouter_proxy/' prefix from model name.
    """
    original_model = "derouter_proxy/claude-2"
...
# -------- Tests for get_llm_provider triggering use_derouter_proxy ---------


def test_get_llm_provider_DEROUTER_PROXY_ALWAYS_true():
    """
    Tests get_llm_provider uses derouter_proxy when USE_DEROUTER_PROXY is "True".
    """
    ...
    with patch.dict(
        os.environ,
        {
            "USE_DEROUTER_PROXY": "True",
            "DEROUTER_PROXY_API_BASE": proxy_api_base,
            "DEROUTER_PROXY_API_KEY": proxy_api_key,
        },
        clear=True,
    ):
        model, provider, key, base = derouter.get_llm_provider(model=test_model_input)
...
def test_get_llm_provider_DEROUTER_PROXY_ALWAYS_true_model_prefix():
    """
    Tests get_llm_provider with USE_DEROUTER_PROXY="True" and model prefix "derouter_proxy/".
    """
    test_model_input = "derouter_proxy/gpt-4-turbo"
...
def test_get_llm_provider_use_proxy_arg_true():
    """
    Tests get_llm_provider uses derouter_proxy when use_proxy=True argument is passed.
    """
    test_model_input = "mistral/mistral-large"
    expected_model_output = (
        "mistral/mistral-large"  # force_use_derouter_proxy keep the model name
    )
    proxy_api_base = "http://my-arg-proxy.com"
    proxy_api_key = "arg_proxy_key"

    # Ensure DEROUTER_PROXY_ALWAYS is not set or False
    with patch.dict(
        os.environ,
        {
            "DEROUTER_PROXY_API_BASE": proxy_api_base,
        ...
        clear=True,
    ):  # clear=True removes DEROUTER_PROXY_ALWAYS if it was set by other tests
        model, provider, key, base = derouter.get_llm_provider(
            model=test_model_input,
            derouter_params=DeRouter_Params(
                use_derouter_proxy=True, model=test_model_input
            ),
        )
...
def test_get_llm_provider_use_proxy_arg_true_with_direct_args():
    ...
    These args should be passed to force_use_derouter_proxy and override env vars.
    """
    ...
    with patch.dict(
        os.environ,
        {
            "DEROUTER_PROXY_API_BASE": env_proxy_api_base,
            "DEROUTER_PROXY_API_KEY": env_proxy_api_key,
        },
        clear=True,
    ):
        model, provider, key, base = derouter.get_llm_provider(
...
class TestClaudeModelPatternMatching:
    ...
    def test_routing_comes_from_the_rule_not_python(self, shipped_generalizations):
        ...
        with pytest.raises(derouter.BadRequestError):
            derouter.get_llm_provider(model="claude-opus-4-9")
