...
from derouter.utils import ModelResponseIterator
...
class MyCustomLLM(CustomLLM):
    def completion(
        ...
        client: Optional[derouter.HTTPHandler] = None,
    ) -> ModelResponse:
    ...
    async def acompletion(
        ...
        headers={},
        timeout: Optional[Union[float, openai.Timeout]] = None,
        client: Optional[derouter.AsyncHTTPHandler] = None,
...
async def test_simple_image_generation_async():
    ...
    resp = await derouter.aimage_generation(
        model="custom_llm/my-fake-model",
        prompt="Hello world",
    )
...
async def test_simple_image_edit_async():
    ...
    resp = await derouter.aimage_edit(
        model="custom_llm/my-fake-model",
        image=b"fake_image_bytes",
        prompt="Edit this image",
    )
...
def test_get_supported_openai_params():
    ...
    from derouter import get_supported_openai_params
...
def test_simple_embedding():
    ...
    resp = derouter.embedding(
        model="custom_llm/my-fake-model",
        input=["good morning from derouter", "good night from derouter"],
    )
...
async def test_simple_aembedding():
    ...
    resp = await derouter.aembedding(
        model="custom_llm/my-fake-model",
        input=["good morning from derouter", "good night from derouter"],
    )
