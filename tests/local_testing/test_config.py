...
import derouter
from derouter.proxy.common_utils.encrypt_decrypt_utils import encrypt_value
from derouter.proxy.proxy_server import ProxyConfig
from derouter.proxy.utils import DualCache, ProxyLogging
...
async def test_delete_deployment():
    ...
    encrypted_derouter_params = derouter_params.dict(exclude_none=True)

    master_key = "sk-1234"

    setattr(derouter.proxy.proxy_server, "master_key", master_key)

    for k, v in encrypted_derouter_params.items():
        if isinstance(v, str):
            encrypted_value = encrypt_value(v, master_key)
            encrypted_derouter_params[k] = base64.b64encode(encrypted_value).decode(
                "utf-8"
            )

    deployment = Deployment(model_name="gpt-3.5-turbo", derouter_params=derouter_params)
    deployment_2 = Deployment(
        model_name="gpt-3.5-turbo-2", derouter_params=derouter_params
    )
    ...
    db_model = DBModel(
        ...
        derouter_params=encrypted_derouter_params,
...
async def test_db_error_new_model_check():
    ...
    Relevant issue: https://github.com/Delify-Solutions/derouter/blob/ddfe687b13e9f31db2fb2322887804e3d01dd467/derouter/proxy/proxy_server.py#L2461
    """
    import base64
...
deployment = Deployment(model_name="gpt-3.5-turbo", derouter_params=derouter_params)
deployment_2 = Deployment(model_name="gpt-3.5-turbo-2", derouter_params=derouter_params)
...
def _create_model_list(flag_value: Literal[0, 1], master_key: str):
    ...
    new_derouter_params = DeRouter_Params(
    ...
    encrypted_derouter_params = new_derouter_params.dict(exclude_none=True)
...
async def test_add_and_delete_deployments(llm_router, model_list_flag_value):
    ...
    llm_router = getattr(derouter.proxy.proxy_server, "llm_router")
...
from derouter import DEROUTER_CHAT_PROVIDERS, LlmProviders
...
def _check_provider_config(config: BaseConfig, provider: LlmProviders):
    ...
    if (
        provider != derouter.LlmProviders.OPENAI
        and provider != derouter.LlmProviders.OPENAI_LIKE
        and provider != derouter.LlmProviders.CUSTOM_OPENAI
    ):
        assert (
...
#     for provider in DEROUTER_CHAT_PROVIDERS:
#         if (
