...
async def test_dynamic_cooldowns():
    """
    Assert kwargs for completion/embedding have 'cooldown_time' as a derouter_param
    """
    # derouter.set_verbose = True
    tmp_mock = MagicMock()

    derouter.failure_callback = [tmp_mock]
    ...
    assert "cooldown_time" in tmp_mock.call_args[0][0]["derouter_params"]
    assert tmp_mock.call_args[0][0]["derouter_params"]["cooldown_time"] == 0
