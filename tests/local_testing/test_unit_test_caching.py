...
from derouter.caching.caching_handler import LLMCachingHandler, CachingHandlerResponse
...
def test_preset_cache_key():
    """
    Test that the preset cache key is used if it is set in kwargs["derouter_params"]
    """
    cache = Cache()
    kwargs = {
        "model": "gpt-3.5-turbo",
        "messages": [{"role": "user", "content": "Hello, world!"}],
        "temperature": 0.7,
        "derouter_params": {"preset_cache_key": "preset-cache-key"},
    }
