...
import derouter.cost_calculator
...
import json
import httpx
from derouter.types.utils import PromptTokensDetails
from derouter.derouter_core_utils.derouter_logging import CustomLogger
...
async def test_custom_pricing(sync_mode):
    ...
    derouter.callbacks = [new_handler]
    if sync_mode:
...
def test_custom_pricing_as_completion_cost_param():
    ...
    from derouter.utils import Usage
...
def test_cost_ft_gpt_35():
    try:
        # this tests if derouter.completion_cost can calculate cost for ft:gpt-3.5-turbo:my-org:custom_suffix:id
        # it needs to lookup  ft:gpt-3.5-turbo in the derouter model_cost map to get the correct cost
        from derouter import Choices, Message, ModelResponse
        from derouter.utils import Usage
...
def test_cost_azure_gpt_35():
    try:
        # this tests if derouter.completion_cost can calculate cost for azure/chatgpt-deployment-2 which maps to azure/gpt-3.5-turbo
...
def test_cost_azure_embedding():
    try:
        ...
        async def _test():
            response = await derouter.aembedding(
                model="azure/text-embedding-ada-002",
                input=["good morning from derouter", "gm"],
            )
...
def test_whisper_openai():
    ...
    cost = derouter.completion_cost(model="whisper-1", completion_response=transcription)

    print(f"cost: {cost}")
    print(f"whisper dict: {derouter.model_cost['whisper-1']}")
    expected_cost = round(
        derouter.model_cost["whisper-1"]["output_cost_per_second"]
...
def test_dalle_3_azure_cost_tracking():
    ...
    response = derouter.ImageResponse(
        created=1710265780,
        data=[
            {
                "b64_json": None,
...
def test_replicate_llama3_cost_tracking():
    ...
    response = derouter.ModelResponse(
        ...
        choices=[
            derouter.utils.Choices(
                finish_reason="stop",
                index=0,
                message=derouter.utils.Message(
    ...
    cost = round(cost, 5)
    expected_cost = round(
        derouter.model_cost["replicate/meta/meta-llama-3-8b-instruct"][
...
def test_groq_response_cost_tracking(is_streaming):
    ...
    response_cost = derouter.response_cost_calculator(
...
def test_together_ai_qwen_completion_cost():
    input_kwargs = {
        "completion_response": derouter.ModelResponse(
            **{
    ...
    response = derouter.cost_calculator.get_model_params_and_category(
...
def test_gemini_completion_cost(provider):
    ...
    prompt_tokens = 128.0
    output_tokens = 228.0
    ## GET MODEL FROM DEROUTER.MODEL_INFO
    model_info = derouter.get_model_info(model=model_name, custom_llm_provider=provider)
...
def test_vertex_ai_completion_cost():
    ...
    model_info = derouter.get_model_info(model="gemini-2.0-flash")
...
def test_vertex_ai_claude_completion_cost():
    ...
    derouter.set_verbose = True
    input_tokens = derouter.token_counter(
        model="vertex_ai/claude-3-sonnet@20240229",
        messages=[{"role": "user", "content": "Hey, how's it going?"}],
    )
    print(f"input_tokens: {input_tokens}")
    output_tokens = derouter.token_counter(
        model="vertex_ai/claude-3-sonnet@20240229",
...
def test_vertex_ai_embedding_completion_cost(caplog):
    """
    Relevant issue - https://github.com/Delify-Solutions/derouter/issues/4630
    """
    os.environ["DEROUTER_LOCAL_MODEL_COST_MAP"] = "True"
    derouter.model_cost = derouter.get_model_cost_map(url="")
    ...
    model_info = derouter.get_model_info(model="vertex_ai/text-embedding-004")
    ...
    for item in captured_logs:
        print("\nitem:{}\n".format(item))
        if (
            "derouter.derouter_core_utils.llm_cost_calc.google.cost_per_character(): Exception occured "
            in item
        ):
...
#     model_info = derouter.get_model_info(model="vertex_ai/textembedding-gecko")
...
#     ## CALCULATED COST
#     resp = derouter.embedding(model="textembedding-gecko", input=[text])
...
def test_completion_cost_azure_common_deployment_name():
    ...
    with patch.object(
        derouter.cost_calculator, "completion_cost", new=MagicMock()
    ) as mock_client:
        _ = derouter.response_cost_calculator(
            response_object=response,
            model="gpt-4-0314",
            custom_llm_provider="azure",
            call_type=CallTypes.acompletion.value,
...
def test_completion_cost_prompt_caching(model, custom_llm_provider):
    ...
    from derouter.utils import Choices, Message, ModelResponse, Usage
    ...
    _model_info = derouter.get_model_info(
...
def test_completion_cost_databricks(model):
    ...
    resp = derouter.completion(model=model, messages=messages)  # works fine
...
from derouter.llms.fireworks_ai.cost_calculator import get_base_model_for_pricing
...
def test_completion_cost_fireworks_ai(model):
    ...
    with patch.object(HTTPHandler, "post", return_value=mock_response):
        resp = derouter.completion(model=model, messages=messages, client=sync_handler)
...
def test_completion_cost_azure_ai_rerank(model):
    from derouter import RerankResponse, rerank
...
def test_together_ai_embedding_completion_cost():
    from derouter.utils import Choices, EmbeddingResponse, Message, ModelResponse, Usage
...
def test_completion_cost_params():
    """
    Relevant Issue: https://github.com/Delify-Solutions/derouter/issues/6133
    """
    derouter.set_verbose = True
...
def test_completion_cost_params_2():
    ...
    model_info = derouter.get_model_info("gemini-2.0-flash")
...
def test_completion_cost_params_gemini_3():
    ...
    from derouter.llms.vertex_ai.cost_calculator import cost_per_character
...
async def test_test_completion_cost_gpt4o_audio_output_from_model(stream):
    ...
    model_info = derouter.get_model_info("gpt-4o-audio-preview")
...
def test_completion_cost_model_response_cost(response_model, custom_llm_provider):
    """
    Relevant issue: https://github.com/Delify-Solutions/derouter/issues/6310
    """
    from derouter import ModelResponse
...
def test_moderations():
    from derouter import moderation

    os.environ["DEROUTER_LOCAL_MODEL_COST_MAP"] = "True"
    derouter.model_cost = derouter.get_model_cost_map(url="")
    derouter.add_known_models()

    assert "omni-moderation-latest" in derouter.model_cost
    print(
        f"derouter.model_cost['omni-moderation-latest']: {derouter.model_cost['omni-moderation-latest']}"
    )
    assert "omni-moderation-latest" in derouter.open_ai_chat_completion_models
...
def test_cost_calculator_azure_embedding():
    from derouter.cost_calculator import response_cost_calculator
    from derouter.types.utils import EmbeddingResponse, Usage
...
def test_add_known_models():
    derouter.add_known_models()
    assert (
        "bedrock/us-west-1/meta.llama3-70b-instruct-v1:0" not in derouter.bedrock_models
    )
...
def test_bedrock_cost_calc_with_region():
    ...
    hidden_params = {
        ...
        "optional_params": {},
        "derouter_call_id": "cf371a5d-679b-410f-b862-8084676d6d59",
        "model_id": None,
        "api_base": None,
    ...
    bedrock_models = derouter.bedrock_models + derouter.bedrock_converse_models

    for model in bedrock_models:
        if derouter.model_cost[model]["mode"] == "chat":
            response = {
                "id": "cmpl-55db75e0b05344058b0bd8ee4e00bf84",
                "choices": [
                    {
...
@pytest.mark.parametrize("base_model_arg", ["derouter_param", "model_info"])
def test_cost_calculator_with_base_model_with_router(base_model_arg):
    ...
    if base_model_arg == "derouter_param":
        model_item["derouter_params"][
            "base_model"
...
def test_cost_calculator_with_base_model_with_router_embedding(base_model_arg):
    ...
    if base_model_arg == "derouter_param":
        model_item["derouter_params"]["base_model"] = "cohere.embed-english-v3"
    elif base_model_arg == "model_info":
        model_item["model_info"] = {
...
async def test_cost_calculator_with_custom_pricing_router(model_item, custom_pricing):
    ...
    if custom_pricing == "derouter_params":
        model_item["derouter_params"]["input_cost_per_token"] = 0.0000008
        model_item["derouter_params"]["output_cost_per_token"] = 0.0000032
...
def test_json_valid_model_cost_map():
    ...
    model_cost = derouter.get_model_cost_map(url="")
