...
async def test_run_async_fallback(function_name):
    ...
    original_exception = derouter.exceptions.InternalServerError(
    ...
    if function_name == "_acompletion":
        assert isinstance(result, derouter.ModelResponse)
    elif function_name == "_atext_completion":
        assert isinstance(result, derouter.TextCompletionResponse)
    elif function_name == "_aembedding":
        assert isinstance(result, derouter.EmbeddingResponse)
...
async def test_failed_fallbacks_raise_most_recent_exception(function_name):
    ...
    with pytest.raises(derouter.exceptions.RateLimitError):
