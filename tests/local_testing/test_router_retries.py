...
async def test_router_retry_policy(error_type):
    from derouter.router import AllowedFailsPolicy, RetryPolicy
...
def test_timeout_for_rate_limit_error_with_healthy_deployments(
    ...
    rate_limit_error = derouter.RateLimitError(
        ...
        response=httpx.Response(
            ...
            request=httpx.Request(method="tpm_rpm_limits", url="https://github.com/Delify-Solutions/derouter"),  # type: ignore
        ),
    )
    model_list = [
        {
...
def test_no_retry_for_bad_request_error_400():
    ...
    # Act & Assert
    error = derouter.BadRequestError(
...
def test_no_retry_for_unprocessable_entity_error_422():
    ...
    # Act & Assert
    error = derouter.UnprocessableEntityError(
...
internal_server_error = derouter.InternalServerError(
...
service_unavailable_error = derouter.ServiceUnavailableError(
    message="service unavailable error",
    model="gpt-12",
    llm_provider="azure",
)

timeout_error = derouter.Timeout(
    ...
    with patch.object(router, "make_call", side_effect=mock_make_call):
        with patch.object(
            ...
            with patch.object(
                ...
                error_str = str(e)
                assert (
                    "DeRouter Retried: 3 times" in error_str
                ), f"Error message should indicate 3 retries: {error_str}"
                assert (
                    "DeRouter Max Retries: 3" in error_str
