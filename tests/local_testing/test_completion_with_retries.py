...
import openai
import derouter
from derouter import completion_with_retries, completion, acompletion_with_retries
from derouter import responses_with_retries, aresponses_with_retries
from derouter.responses.main import responses, aresponses
...
async def test_completion_with_retry_policy(sync_mode):
    ...
    with patch.object(derouter, target_function) as mock_completion_with_retries:
        data = {
...
async def test_completion_with_retries(sync_mode):
    """
    If completion_with_retries is called with num_retries=3, and max_retries=0, then derouter.completion should receive num_retries , max_retries=0
    """
    from unittest.mock import patch, MagicMock, AsyncMock
    ...
    with patch.object(derouter, target_function) as mock_completion:
        if sync_mode:
...
async def test_responses_with_retries(sync_mode):
    ...
    then derouter.responses should receive num_retries=0, max_retries=0
    """
    from unittest.mock import patch, MagicMock, AsyncMock
    ...
    with patch(
        "derouter.responses.main.responses"
        if sync_mode
        else "derouter.responses.main.aresponses"
    ) as mock_responses:
        if sync_mode:
...
async def test_responses_retry_on_auth_error(sync_mode):
    ...
    if sync_mode:
        with patch.object(derouter, "responses_with_retries") as mock_retry:
        ...
        with patch.object(derouter, "aresponses_with_retries") as mock_retry:
            mock_retry.return_value = None
            try:
                await aresponses(
                    model="gpt-4o",
