#### What this tests ####
#    This tests mock request calls to derouter
...
def test_mock_request():
    try:
        ...
        response = derouter.mock_completion(model=model, messages=messages, stream=False)
        print(response)
        print(type(response))
    except Exception:
        traceback.print_exc()
...
def test_streaming_mock_request():
    try:
        model = "gpt-3.5-turbo"
        messages = [{"role": "user", "content": "Hey, I'm a mock request"}]
        response = derouter.mock_completion(model=model, messages=messages, stream=True)
        complete_response = ""
...
async def test_async_mock_streaming_request():
    generator = await derouter.acompletion(
        messages=[{"role": "user", "content": "Why is DeRouter amazing?"}],
        mock_response="DeRouter is awesome",
        stream=True,
        model="gpt-3.5-turbo",
    )
    complete_response = ""
    ...
    assert (
        complete_response == "DeRouter is awesome"
...
def test_mock_request_n_greater_than_1():
    try:
        model = "gpt-3.5-turbo"
        messages = [{"role": "user", "content": "Hey, I'm a mock request"}]
        response = derouter.mock_completion(model=model, messages=messages, n=5)
