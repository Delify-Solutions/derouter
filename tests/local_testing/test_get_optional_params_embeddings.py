...
import pytest
import derouter
from derouter import embedding
from derouter.utils import get_optional_params_embeddings, get_llm_provider
...
def test_openai_non_text_embedding_3_without_allowed_openai_params_raises():
    ...
    prev_drop_params = derouter.drop_params
    ...
    try:
        ...
        derouter.drop_params = prev_drop_params


def test_openai_non_text_embedding_3_drop_params_per_call():
    """
    Regression for https://github.com/Delify-Solutions/derouter/issues/26787
    ...
    """
    prev_drop_params = derouter.drop_params
    derouter.drop_params = False  # ensure only per-call flag is in effect
    try:
        model, custom_llm_provider, _, _ = get_llm_provider(
...
def test_openai_non_text_embedding_3_drop_params_global():
    ...
    When `derouter.drop_params = True` is set globally, `dimensions` should be
