...
class testCustomCallbackProxy(CustomLogger):
    def __init__(self):
        ...
        print(f"{blue_color_code}Initialized DeRouter custom logger")
        try:
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        ...
        # Access derouter_params passed to derouter.completion(), example access `metadata`
        derouter_params = kwargs.get("derouter_params", {})
        metadata = derouter_params.get(
            "metadata", {}
        )  # headers passed to DeRouter proxy, can be found here

        # Calculate cost using  derouter.completion_cost()
        cost = derouter.completion_cost(completion_response=response_obj)
