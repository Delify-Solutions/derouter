...
def test_completion_azure_stream_content_filter_no_delta():
    ...
    try:
        ...
        chunk_list = []
        for chunk in chunks:
            new_chunk = derouter.ModelResponseStream(id=chunk["id"])
            if "choices" in chunk and isinstance(chunk["choices"], list):
                new_choices = []
                for choice in chunk["choices"]:
                    if isinstance(choice, derouter.utils.StreamingChoices):
                        _new_choice = choice
                    elif isinstance(choice, dict):
                        _new_choice = derouter.utils.StreamingChoices(**choice)
        ...
        response = derouter.CustomStreamWrapper(
            completion_stream=completion_stream,
            model="gpt-4-0613",
            custom_llm_provider="cached_response",
            logging_obj=derouter.Logging(
                model="gpt-4-0613",
...
def test_completion_azure_stream():
    try:
        ...
        for idx, init_chunk in enumerate(response):
            ...
            if finished:
                assert isinstance(init_chunk.choices[0], derouter.utils.StreamingChoices)
                break
        if complete_response.strip() == "":
            raise Exception("Empty response received")
    except Exception as e:
...
def test_vertex_ai_stream(provider):
    ...
    for model in test_models:
        try:
            ...
            pass
        except derouter.exceptions.MidStreamFallbackError as e:
            # Streaming 429s are wrapped in MidStreamFallbackError so the
            # Router can fall back; treat as a transient rate-limit pass.
            if not isinstance(e.original_exception, derouter.RateLimitError):
                pytest.fail(f"Error occurred: {e}")
        except Exception as e:
            pytest.fail(f"Error occurred: {e}")
...
async def test_completion_replicate_llama3_streaming(sync_mode):
    ...
    try:
        if sync_mode:
            final_chunk: Optional[derouter.ModelResponse] = None
            response: derouter.CustomStreamWrapper = completion(  # type: ignore
                model=model_name,
                messages=messages,
            ...
            response: derouter.CustomStreamWrapper = await derouter.acompletion(  # type: ignore
                model=model_name,
                messages=messages,
...
async def test_parallel_streaming_requests(sync_mode, model):
    ...
    try:
        ...
        def sync_test_streaming():
            response: derouter.CustomStreamWrapper = derouter.completion(  # type: ignore
                model=model,
                messages=messages,
                stream=True,
                max_tokens=10,
...
def test_sagemaker_weird_response():
    ...
    try:
        ...
        from derouter.llms.sagemaker.completion.handler import TokenIterator
...
async def test_openai_stream_options_call(model, sync):
    ...
    assert isinstance(last_chunk.usage, derouter.Usage)
...
def test_openai_stream_options_call_text_completion():
    ...
    for idx in range(3):
        try:
            response = derouter.text_completion(
...
def validate_first_function_call_chunk_structure(item):
    if not (isinstance(item, dict) or isinstance(item, derouter.ModelResponse)):
    ...
    for choice in item["choices"]:
        if not (
            ...
            or isinstance(choice, derouter.utils.StreamingChoices)
        ):
        ...
        if not (
            ...
            or isinstance(choice["delta"], derouter.utils.Delta)
        ):
            raise Exception(
...
def validate_final_function_call_chunk_structure(data):
    if not (isinstance(data, dict) or isinstance(data, derouter.ModelResponse)):
        raise Exception("Incorrect format")
    ...
    for choice in data["choices"]:
        if not (
            isinstance(choice, dict) or isinstance(choice["delta"], derouter.utils.Delta)
        ):
            raise Exception("Incorrect format")
        for key in required_keys_in_choices_array:
            if key not in choice:
...
def test_streaming_and_function_calling(model):
    ...
    try:
        # derouter.set_verbose = True
        response: derouter.CustomStreamWrapper = completion(
            model=model,
            tools=tools,
            messages=messages,
            stream=True,
...
def test_success_callback_streaming():
    ...
    derouter.success_callback = [success_callback]

    messages = [{"role": "user", "content": "hello"}]
    print("TESTING DEROUTER COMPLETION CALL")
    response = derouter.completion(
        model="gpt-3.5-turbo",
        messages=messages,
        stream=True,
...
async def test_acompletion_function_call_with_streaming(model):
    ...
    except derouter.ServiceUnavailableError:
        pass
    except Exception as e:
        pytest.fail(f"Error occurred: {e}")
...
def test_unit_test_custom_stream_wrapper():
    ...
    chunk = derouter.ModelResponseStream(**chunk)
...
@pytest.mark.parametrize(
    "loop_amount",
    [
        derouter.REPEATED_STREAMING_CHUNK_LIMIT + 1,
        derouter.REPEATED_STREAMING_CHUNK_LIMIT - 1,
    ],
)
@pytest.mark.parametrize(
...
def test_unit_test_custom_stream_wrapper_repeating_chunk(
    ...
    if (loop_amount > derouter.REPEATED_STREAMING_CHUNK_LIMIT) and expected_chunk_fail:
        def _drain():
            for chunk in response:
                continue

        with pytest.raises(
            (derouter.InternalServerError, derouter.exceptions.MidStreamFallbackError)
        ):
            _drain()
    else:
        for chunk in response:
...
def test_unit_test_custom_stream_wrapper_openai():
    ...
    chunk = {
        ...
        "choices": [
            derouter.utils.StreamingChoices(
...
def test_aamazing_unit_test_custom_stream_wrapper_n():
    ...
    Relevant issue: https://github.com/Delify-Solutions/derouter/issues/3276
    """
    chunks = [
        {
...
def test_unit_test_custom_stream_wrapper_function_call():
    ...
    """
    from derouter.types.llms.openai import ChatCompletionDeltaChunk
    ...
    response = derouter.CustomStreamWrapper(
        ...
        model="gpt-3.5-turbo",
        custom_llm_provider="cached_response",
        logging_obj=derouter.derouter_core_utils.derouter_logging.Logging(
            model="gpt-3.5-turbo",
            messages=[{"role": "user", "content": "Hey"}],
            stream=True,
            call_type="completion",
    ...
    initial_model_response = derouter.ModelResponse(
...
def test_mock_response_iterator_tool_use():
    """
    Relevant Issue: https://github.com/Delify-Solutions/derouter/issues/7364
    """
    from derouter.llms.bedrock.chat.invoke_handler import MockResponseIterator
    from derouter.types.utils import (
...
def test_streaming_with_cost_calculation():
    ...
    derouter.include_cost_in_streaming_usage = True
