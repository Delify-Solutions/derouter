...
import derouter
from derouter import aembedding, completion, embedding, aresponses, responses
from derouter.caching.caching import Cache
from derouter.responses.streaming_iterator import CachedResponsesAPIStreamingIterator
...
chat_completion_response = derouter.ModelResponse(
    id=str(uuid.uuid4()),
    choices=[
        derouter.Choices(
...
text_completion_response = derouter.TextCompletionResponse(
    id=str(uuid.uuid4()),
    choices=[derouter.utils.TextChoices(text="Hello, how can I help you today?")],
)
...
async def test_async_set_get_cache(response):
    ...
    original_function = (
        ...
        else derouter.atext_completion
    )
    if isinstance(response, derouter.ModelResponse):
...
async def test_async_get_cache_updates_request_kwargs_for_streaming_responses():
    ...
    assert caching_handler.request_kwargs["cache_key"] == derouter.cache.get_cache_key(
