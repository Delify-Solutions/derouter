...
derouter.success_callback = ["helicone"]
...
def test_helicone_removes_otel_span_from_metadata():
    """
    Test that HeliconeLogger removes derouter_parent_otel_span from metadata
    to prevent JSON serialization errors.
    """
    from derouter.integrations.helicone import HeliconeLogger
    ...
    metadata = {
        ...
        "derouter_parent_otel_span": mock_span,  # This would cause JSON serialization error
    ...
    derouter_params = {"proxy_server_request": {"headers": {}}}
    result_metadata = logger.add_metadata_from_header(derouter_params, metadata)

    # Verify that derouter_parent_otel_span was removed
    assert "derouter_parent_otel_span" not in result_metadata
    ...
    print(
        "✅ Test passed: derouter_parent_otel_span was successfully removed from metadata"
    )
