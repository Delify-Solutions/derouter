...
def test_get_model_info_custom_llm_with_same_name_vllm(monkeypatch):
    ...
    model_info = derouter.get_model_info(model, custom_llm_provider=provider)
    print("model_info", model_info)
    assert model_info["input_cost_per_token"] == 0.0


def test_get_model_info_gemini_pro():
    info = derouter.get_model_info("gemini-2.0-flash")
    print("info", info)
    assert info["key"] == "gemini-2.0-flash"


def test_get_model_info_ollama_chat():
    from derouter.llms.ollama.completion.transformation import OllamaConfig

    with patch.object(
        derouter.module_level_client,
        "post",
...
def test_get_model_info_bedrock_region():
    ...
    derouter.model_cost.pop("us.anthropic.claude-haiku-4-5-20251001-v1:0", None)
    info = derouter.get_model_info(**args)
    print("info", info)
...
def _enforce_bedrock_converse_models(
    ...
    for model, info in derouter.model_cost.items():
        if (
            info["derouter_provider"] == "bedrock"
            and info["mode"] == "chat"
            and model not in whitelist_models
        ):
            raise AssertionError(
                f"New bedrock chat model detected: {model}. Please set `derouter_provider='bedrock_converse'` for this model."
            )
...
def test_model_info_bedrock_converse(monkeypatch):
    ...
    _enforce_bedrock_converse_models(
        model_cost=derouter.model_cost, whitelist_models=whitelist_models
    )
...
def test_model_info_bedrock_converse_enforcement(monkeypatch):
    ...
    derouter.model_cost["fake.bedrock-chat-model"] = {
...
def test_get_model_info_custom_provider():
    # Custom provider example copied from https://router.delify.vn/docs/providers/custom_llm_server:
    import derouter
    from derouter import CustomLLM, completion

    class MyCustomLLM(CustomLLM):
        def completion(self, *args, **kwargs) -> derouter.ModelResponse:
    ...
    derouter.custom_provider_map = [  # 👈 KEY STEP - REGISTER HANDLER
    ...
    derouter.register_model(model_info)
...
def test_get_model_info_bedrock_models():
    ...
    for k, v in derouter.model_cost.items():
        if v["derouter_provider"] == "bedrock":
            k = k.replace("*/", "")
            ...
            base_model_key = (
                base_model
                if base_model in derouter.model_cost
                else f"bedrock/{base_model}"
            )
            if base_model_key not in derouter.model_cost:
                continue
            base_model_info = derouter.model_cost[base_model_key]
...
def test_get_model_info_bedrock_cross_region_capability_parity():
    """
    Cross-region inference profiles carry derouter_provider "bedrock_converse", so the
    ...
    for k, v in derouter.model_cost.items():
        if not str(v.get("derouter_provider", "")).startswith("bedrock"):
            continue
        base_model_key = next(
            (k[len(p) :] for p in prefixes if k.startswith(p)),
            None,
        )
        if base_model_key is None or base_model_key not in derouter.model_cost:
            continue
        checked += 1
        for cap, base_value in derouter.model_cost[base_model_key].items():
...
def test_get_model_info_huggingface_models(monkeypatch):
    ...
    info = derouter.get_model_info("huggingface/meta-llama/Meta-Llama-3-8B-Instruct")
    print("info", info)
    assert info is not None
...
def test_get_model_info_cost_calculator_bedrock_region_cris_stripped(model, provider):
    ...
    Relevant Issue: https://github.com/Delify-Solutions/derouter/issues/8115
    """
...
def test_get_model_info_case_insensitive_lookup(monkeypatch):
    ...
    info_lower = derouter.get_model_info(
        model="qwen/qwen3-next-80b-a3b-thinking", custom_llm_provider="together_ai"
    )
    ...
    info_mixed = derouter.get_model_info(
        model="QWEN/qwen3-NEXT-80b-a3b-thinking", custom_llm_provider="together_ai"
    )
