...
from derouter.router_strategy.lowest_tpm_rpm_v2 import (
...
def test_tpm_rpm_updated():
    ...
    standard_logging_payload["hidden_params"]["derouter_model_name"] = deployment
    kwargs = {
        "derouter_params": {
            "model": deployment,
            "metadata": {
    ...
    derouter_deployment_dict: DeploymentTypedDict = {
        "model_name": model_group,
        "derouter_params": {"model": deployment},
        "model_info": {"id": deployment_id},
    }
    ...
    end_time = time.time()
    lowest_tpm_logger.pre_call_check(deployment=derouter_deployment_dict)
...
def test_router_get_available_deployments():
    ...
    total_tokens = 50
    standard_logging_payload["total_tokens"] = total_tokens
    standard_logging_payload["hidden_params"]["derouter_model_name"] = "azure/gpt-turbo"
    kwargs = {
        "derouter_params": {
            "metadata": {
...
async def test_router_caching_ttl():
    ...
    Relevant issue: https://github.com/Delify-Solutions/derouter/issues/5609
    """
    messages = [
        {"role": "user", "content": "Hello, can you generate a 500 words poem?"}
    ]
...
async def test_tpm_rpm_routing_model_name_checks():
    ...
    with (
        ...
        assert (
            mock_object.call_args[0][0]["derouter_params"]["model"]
        ...
        assert (
            standard_logging_payload["hidden_params"]["derouter_model_name"]
            == "azure/gpt-4.1-mini"
        )
