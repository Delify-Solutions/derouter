...
def test_model_alias_map(caplog):
    try:
        derouter.model_alias_map = model_alias_map
        response = completion(
            "good-model",
        ...
        for rec in caplog.records:
            if rec.levelname == "ERROR" and rec.name.startswith("DeRouter"):
                pytest.fail(f"Unexpected derouter ERROR log: {rec.getMessage()}")
