...
from derouter.proxy.proxy_server import initialize_pass_through_endpoints
...
def client():
    ...
    remove_rerank_route(
        app=app
    )  # remove the native rerank route on the derouter proxy - since we're testing the pass through endpoints
...
async def test_pass_through_endpoint_no_headers(client, monkeypatch):
    ...
    general_settings: dict = (
        getattr(derouter.proxy.proxy_server, "general_settings", {}) or {}
    )
...
async def test_pass_through_endpoint_rpm_limit(
    ...
    from derouter.proxy.proxy_server import ProxyLogging, hash_token, user_api_key_cache

    proxy_logging_obj = ProxyLogging(user_api_key_cache=user_api_key_cache)
    proxy_logging_obj._init_derouter_callbacks()
...
async def test_aaapass_through_endpoint_pass_through_keys_langfuse(
    ...
    original_user_api_key_cache = getattr(
        derouter.proxy.proxy_server, "user_api_key_cache", None
    )
    original_master_key = getattr(derouter.proxy.proxy_server, "master_key", None)
    original_prisma_client = getattr(derouter.proxy.proxy_server, "prisma_client", None)
    original_proxy_logging_obj = getattr(
        derouter.proxy.proxy_server, "proxy_logging_obj", None
    )
    ...
    try:
            ...
            "batch": [
                {
                    ...
                    "body": {
                        ...
                        "name": "test-trace-derouter-proxy-passthrough",
                    },
        ...
        # Format: base64(public_key:secret_key) where public_key is the DeRouter API key
        import base64
        ...
        setattr(derouter.proxy.proxy_server, "general_settings", old_general_settings)
    finally:
        ...
        setattr(derouter.proxy.proxy_server, "master_key", original_master_key)
        setattr(derouter.proxy.proxy_server, "prisma_client", original_prisma_client)
        setattr(
            derouter.proxy.proxy_server, "proxy_logging_obj", original_proxy_logging_obj
        )
