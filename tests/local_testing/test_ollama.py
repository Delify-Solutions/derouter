...
from derouter.utils import EmbeddingResponse, get_llm_provider, get_optional_params
...
@mock.patch(
    "derouter.llms.ollama.completion.handler.ollama_embeddings",
...
@mock.patch(
    "derouter.llms.ollama.completion.handler.ollama_aembeddings",
    return_value=mock_ollama_embedding_response,
)
def test_ollama_aembeddings(mock_aembeddings):
    ...
    try:
        embeddings = asyncio.run(
            derouter.aembedding(model="ollama/nomic-embed-text", input=["hello world"])
        )
        print(embeddings)
...
def test_ollama_ssl_verify():
    ...
    client: HTTPHandler = derouter.in_memory_llm_clients_cache.get_cache(
...
async def test_async_ollama_ssl_verify(stream):
    ...
    client: AsyncHTTPHandler = derouter.in_memory_llm_clients_cache.get_cache(
    ...
    derouter_created_session = client.client._transport._get_valid_client_session()
    print("derouter_created_session=", derouter_created_session)
    # check session ssl
    print("derouter_created_session ssl=", derouter_created_session.connector._ssl)
    ...
    assert derouter_created_session.connector._ssl is False
    assert derouter_created_session.connector._ssl == aiohttp_session.connector._ssl
