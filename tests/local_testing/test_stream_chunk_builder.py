...
from derouter.types.utils import StreamingChoices, ChatCompletionAudioResponse
...
from derouter import completion, stream_chunk_builder
...
def test_stream_chunk_builder_derouter_function_call():
    try:
        derouter.set_verbose = False
...
# test_stream_chunk_builder_derouter_function_call()


def test_stream_chunk_builder_derouter_tool_call():
    try:
...
# test_stream_chunk_builder_derouter_tool_call()


def test_stream_chunk_builder_derouter_tool_call_regular_message():
    try:
...
# test_stream_chunk_builder_derouter_tool_call_regular_message()


def test_stream_chunk_builder_derouter_mixed_calls():
...
def test_stream_chunk_builder_derouter_empty_chunks():
    with pytest.raises(derouter.APIError):
...
def test_stream_chunk_builder_multiple_tool_calls():
    ...
    for chunk in init_chunks:
        chunks.append(derouter.ModelResponseStream(**chunk))
    ...
    expected_response = derouter.ModelResponse(**completed_response)
...
def test_stream_chunk_builder_openai_prompt_caching():
    ...
    chunks: List[derouter.ModelResponse] = []
    usage_obj = None
    for chunk in chat_completion:
        chunks.append(derouter.ModelResponseStream(**chunk.model_dump()))

    print(f"chunks: {chunks}")

    usage_obj: derouter.Usage = chunks[-1].usage  # type: ignore
...
def test_stream_chunk_builder_openai_audio_output_usage():
    ...
    usage_obj: Optional[derouter.Usage] = None
...
def load_env():
    ...
    tools = [
        {
            "type": "function",
            "function": derouter.utils.function_to_dict(get_current_weather),
        }
    ]
    OPENAI_GPT4oMINI = {
        "messages": messages,
...
def execute_completion(opts: dict):
    ...
    response_gen = derouter.completion(**opts)
    ...
    print("\n\n")
    assembly = derouter.stream_chunk_builder(partial_streaming_chunks)
    print(
    ...
    for tool_call in assembly.choices[0].message.tool_calls:
        ...
        )  # assert valid json - https://github.com/Delify-Solutions/derouter/issues/10034
