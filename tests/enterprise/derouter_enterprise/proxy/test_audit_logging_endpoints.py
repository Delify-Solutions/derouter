...
from derouter_enterprise.proxy.audit_logging_endpoints import router as audit_router
from derouter_enterprise.types.proxy.audit_logging_endpoints import AuditLogResponse
...
def mock_prisma_client():
    with patch("derouter.proxy.proxy_server.prisma_client") as mock:
        mock.db.derouter_auditlog.find_many = AsyncMock()
        mock.db.derouter_auditlog.find_unique = AsyncMock()
        mock.db.derouter_auditlog.count = AsyncMock()
        yield mock
...
async def test_get_audit_logs(mock_prisma_client):
    ...
    mock_prisma_client.db.derouter_auditlog.find_many.return_value = [
        AuditLogResponse(**MOCK_AUDIT_LOG)
    ]
    mock_prisma_client.db.derouter_auditlog.count.return_value = 1

    # Mock the auth dependency
    with patch("derouter.proxy.auth.user_api_key_auth.user_api_key_auth") as mock_auth:
...
async def test_get_audit_log_by_id(mock_prisma_client):
    ...
    mock_prisma_client.db.derouter_auditlog.find_unique.return_value = AuditLogResponse(
        **MOCK_AUDIT_LOG
    )
...
async def test_get_audit_log_by_id_not_found(mock_prisma_client):
    ...
    mock_prisma_client.db.derouter_auditlog.find_unique.return_value = None
...
def _list_audit_logs_where(mock_prisma_client: MagicMock, query: str) -> dict[str, object]:
    mock_prisma_client.db.derouter_auditlog.find_many.return_value = []
    mock_prisma_client.db.derouter_auditlog.count.return_value = 0

    response: Final = client.get(f"/audit?{query}")

    assert response.status_code == 200, response.text
    find_many_where: Final = mock_prisma_client.db.derouter_auditlog.find_many.call_args.kwargs["where"]
    assert mock_prisma_client.db.derouter_auditlog.count.call_args.kwargs["where"] == find_many_where
