...
from derouter_enterprise.proxy.management_endpoints.project_endpoints import (
    new_project,
    update_project,
    delete_project,
    project_info,
...
async def test_update_project(prisma_client):
    ...
    try:
        ...
        assert update_response.derouter_budget_table is not None
        assert update_response.derouter_budget_table.max_budget == 200.0
...
async def test_project_info(prisma_client):
    ...
    try:
        ...
        assert info_response.derouter_budget_table is not None
        assert info_response.derouter_budget_table.max_budget == 150.0
...
def test_premium_user_gate():
    ...
    setattr(derouter.proxy.proxy_server, "premium_user", False)
...
def test_project_cached_obj_has_last_refreshed_at():
    """
    Test that DeRouter_ProjectTableCachedObj has last_refreshed_at field
    matching DeRouter_TeamTableCachedObj pattern.
    """
    from derouter.proxy._types import (
    ...
    # Verify inheritance
    assert issubclass(DeRouter_ProjectTableCachedObj, DeRouter_ProjectTable)

    # Verify last_refreshed_at field exists and defaults to None
    obj = DeRouter_ProjectTableCachedObj(
        project_id="test",
        created_by="admin",
        updated_by="admin",
    )
...
async def test_project_max_budget_check_fires_alert():
    ...
    """
    from derouter.proxy.auth.auth_checks import _project_max_budget_check
    from derouter.proxy._types import (
...
async def test_project_soft_budget_check():
    ...
    """
    from derouter.proxy.auth.auth_checks import _project_soft_budget_check
    from derouter.proxy._types import (
        DeRouter_BudgetTable,
        DeRouter_ProjectTableCachedObj,
    )
    ...
    project = DeRouter_ProjectTableCachedObj(
        ...
        created_by="admin",
        updated_by="admin",
        derouter_budget_table=DeRouter_BudgetTable(soft_budget=75.0),
    )
...
def test_derouter_entity_type_has_project():
    """
...
async def test_list_projects_returns_timestamps():
    ...
    from derouter_enterprise.proxy.management_endpoints.project_endpoints import list_projects
    from derouter.proxy._types import DeRouter_ProjectTable
    ...
    mock_prisma.db.derouter_projecttable.find_many = AsyncMock(
...
def test_derouter_project_table_has_timestamp_fields():
    """
    Test that DeRouter_ProjectTable model includes created_at and updated_at fields,
    so the /project/list response_model exposes them.
    """
    from derouter.proxy._types import DeRouter_ProjectTable

    fields = DeRouter_ProjectTable.model_fields
    assert "created_at" in fields, "DeRouter_ProjectTable must have created_at field"
    assert "updated_at" in fields, "DeRouter_ProjectTable must have updated_at field"
...
async def test_update_project_invalidates_cached_project_object(monkeypatch):
    ...
    mock_prisma.db.derouter_projecttable.find_unique = AsyncMock(return_value=stale_row)
    ...
    mock_prisma.db.derouter_projecttable.find_unique = AsyncMock(return_value=existing_row)
    mock_prisma.db.derouter_projecttable.update = AsyncMock(return_value=updated_row)
    ...
    monkeypatch.setattr(derouter.proxy.proxy_server, "user_api_key_cache", cache)
    ...
    mock_prisma.db.derouter_projecttable.find_unique = AsyncMock(return_value=updated_row)
...
async def test_delete_project_invalidates_cached_project_object(monkeypatch):
    ...
    mock_prisma = MagicMock()
    mock_prisma.db.derouter_projecttable.find_unique = AsyncMock(return_value=row)
    ...
    mock_prisma.db.derouter_verificationtoken.find_many = AsyncMock(return_value=[])
    mock_prisma.db.derouter_projecttable.delete = AsyncMock(return_value=row)
    ...
    mock_prisma.db.derouter_projecttable.find_unique = AsyncMock(return_value=None)
    assert (
...
async def test_update_project_succeeds_when_cache_eviction_fails(monkeypatch):
    ...
    monkeypatch.setattr(derouter.proxy.proxy_server, "premium_user", True)
    monkeypatch.setattr(derouter.proxy.proxy_server, "prisma_client", mock_prisma)
    monkeypatch.setattr(derouter.proxy.proxy_server, "user_api_key_cache", failing_cache)
...
async def test_project_eviction_publishes_cross_worker_invalidation(monkeypatch):
    ...
    from derouter.proxy.auth.auth_checks import delete_cached_project_object
    from derouter.proxy.common_utils.user_api_key_cache import UserApiKeyCache
...
def test_update_quota_adds_model_without_quota_rejected():
    ...
    from derouter.proxy._types import UpdateProjectRequest
    from derouter_enterprise.proxy.management_endpoints.project_endpoints import (
...
def _enforced_new_project_mocks(monkeypatch, team_models: list[str], llm_router: mock.MagicMock | None) -> None:
    from derouter.proxy._types import DeRouter_TeamTable
    from derouter_enterprise.proxy.management_endpoints import project_endpoints as pe

    team = DeRouter_TeamTable(team_id="test-team", models=team_models)
    monkeypatch.setattr(derouter.proxy.proxy_server, "prisma_client", mock.MagicMock())
    monkeypatch.setattr(derouter.proxy.proxy_server, "premium_user", True)
    monkeypatch.setattr(derouter.proxy.proxy_server, "llm_router", llm_router)
    monkeypatch.setattr(derouter.proxy.proxy_server, "general_settings", {"enforce_project_model_quota": True})
...
def _project_update_mocks(monkeypatch, stored_metadata: dict) -> mock.MagicMock:
    ...
    mock_prisma.jsonify_object = lambda data: data
    mock_prisma.db.derouter_projecttable.find_unique = mock.AsyncMock(return_value=existing_row)
    mock_prisma.db.derouter_projecttable.update = mock.AsyncMock(return_value=mock.MagicMock())
...
def _written_project_data(mock_prisma: mock.MagicMock) -> dict:
    return mock_prisma.db.derouter_projecttable.update.await_args.kwargs["data"]
