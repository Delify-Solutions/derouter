...
from derouter_enterprise.proxy.management_endpoints.internal_user_endpoints import router
...
def mock_user_api_key_auth():
    ...
    with patch(
        "derouter_enterprise.proxy.management_endpoints.internal_user_endpoints.user_api_key_auth"
    ) as mock_auth:
...
class TestAvailableEnterpriseUsers:
    ...
    async def test_available_users_with_max_users_set(
        ...
        with (
            ...
            # Mock database count
            mock_prisma.db.derouter_usertable.count = _user_count(5)
            mock_prisma.db.derouter_teamtable.count = AsyncMock(return_value=2)
    ...
    async def test_available_users_excludes_scim_deactivated(
        ...
        with (
            ...
            mock_prisma.db.derouter_usertable.count = _user_count(5, deactivated=2)
            mock_prisma.db.derouter_teamtable.count = AsyncMock(return_value=2)
    ...
    async def test_available_users_without_max_users_set(
        ...
        with (
            ...
            # Mock database count
            mock_prisma.db.derouter_usertable.count = _user_count(3)
            mock_prisma.db.derouter_teamtable.count = AsyncMock(return_value=1)
    ...
    async def test_available_users_negative_remaining_bug(
        ...
        with (
            ...
            mock_prisma.db.derouter_usertable.count = _user_count(8)
            mock_prisma.db.derouter_teamtable.count = AsyncMock(return_value=3)
