...
from derouter.proxy._types import CallTypes
...
def test_get_file_ids_from_messages():
    proxy_managed_files = _PROXY_DeRouterManagedFiles(
...
async def test_async_pre_call_hook_batch_retrieve():
    ...
    prisma_client.db.derouter_managedobjecttable.find_first.return_value = return_value
    proxy_managed_files = _PROXY_DeRouterManagedFiles(
...
async def test_list_user_batches_limit_zero_returns_empty_page_without_db_query():
    ...
    proxy_managed_files = _PROXY_DeRouterManagedFiles(DualCache(), prisma_client=prisma_client)
    ...
    prisma_client.db.derouter_managedobjecttable.find_many.assert_not_called()


@pytest.mark.asyncio
async def test_async_pre_call_deployment_hook_resolves_model_id_from_derouter_metadata():
    """
    For batch operations the router stores model_info under
    kwargs["derouter_metadata"]["model_info"] (not top-level kwargs["model_info"]).
    ...
    # model_info is nested under derouter_metadata (batch path)
    kwargs = {
...
async def test_async_pre_call_deployment_hook_prefers_top_level_model_info():
    ...
    should use it without falling back to derouter_metadata.
    """
    proxy_managed_files = _PROXY_DeRouterManagedFiles(
        DualCache(), prisma_client=MagicMock()
    )
    ...
    ), "Should prefer top-level model_info over derouter_metadata"


@pytest.mark.asyncio
async def test_async_pre_call_deployment_hook_no_model_info_leaves_file_id_unchanged():
    """
    When model_info is absent from both top-level and derouter_metadata,
...
#     proxy_managed_files = _PROXY_DeRouterManagedFiles(DualCache())
...
async def test_async_post_call_success_hook_for_unified_finetuning_job():
    from derouter.types.utils import DeRouterFineTuningJob
    ...
    provider_ft_job = DeRouterFineTuningJob(
    ...
    assert isinstance(response, DeRouterFineTuningJob)
...
async def test_can_user_call_unified_file_id(call_type):
    ...
    return_value = MagicMock()
    return_value.created_by = "123"
    prisma_client.db.derouter_managedfiletable.find_first.return_value = return_value
    proxy_managed_files = _PROXY_DeRouterManagedFiles(
...
async def test_router_acreate_batch_only_selects_from_file_id_mapping(monkeypatch):
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "gpt-5-mini"},
                "model_info": {"id": "1234"},
            },
            {
                "model_name": "gpt-5-mini",
    ...
    with patch.object(
        derouter, "acreate_batch", return_value=AsyncMock()
...
async def test_output_file_id_for_batch_retrieve():
    ...
    batch._hidden_params = {
        "derouter_call_id": "dcd789e0-c0ad-4244-9564-4e611448d650",
        "api_base": "https://api.openai.com",
        "model_id": "12345679",
        "response_cost": 0.0,
        "additional_headers": {},
        "derouter_model_name": "gpt-5.5",
        "unified_batch_id": "derouter_proxy;model_id:12345679;llm_batch_id:batch_685c5e5d63988190b85bdb2147ba131d",
    }
    proxy_managed_files = _PROXY_DeRouterManagedFiles(
        DualCache(), prisma_client=AsyncMock()
    )
    ...
    assert not cast(DeRouterBatch, response).output_file_id.startswith("file-")
...
async def test_output_file_id_preserves_target_model_names_when_model_name_missing():
    ...
    from derouter.proxy._types import UserAPIKeyAuth
    from derouter.types.llms.openai import OpenAIFileObject
    from derouter.types.utils import DeRouterBatch
    ...
    managed_input_file_payload = (
        "derouter_proxy:application/octet-stream;"
    ...
    batch._hidden_params = {
        "model_id": "model-id-1",
        "unified_batch_id": "derouter_proxy;model_id:model-id-1;llm_batch_id:batch_123",
    ...
    with patch("derouter.afile_retrieve", new_callable=AsyncMock) as mock_retrieve:
    ...
    decoded_output_file_id = _is_base64_encoded_unified_file_id(
        cast(DeRouterBatch, response).output_file_id
    )
...
async def test_error_file_id_for_failed_batch():
    ...
    batch._hidden_params = {
        ...
        "additional_headers": {},
        "derouter_model_name": "gpt-5.5",
        "unified_batch_id": "derouter_proxy;model_id:test-model-id;llm_batch_id:batch_abc123",
    }
    ...
    assert cast(DeRouterBatch, response).error_file_id is not None
    assert not cast(DeRouterBatch, response).error_file_id.startswith("error-")
    # Verify it's a base64 encoded managed file ID
    assert _is_base64_encoded_unified_file_id(
        cast(DeRouterBatch, response).error_file_id
    )
...
async def test_async_post_call_success_hook_twice_assert_no_unique_violation():
    ...
    with patch("asyncio.create_task") as mock_create_task:
        ...
        if tasks:
            ...
            assert isinstance(response, DeRouterBatch)
            assert _is_base64_encoded_unified_file_id(response.id)
...
def test_update_responses_input_with_unified_file_id():
    ...
    # This decodes to: derouter_proxy:application/pdf;unified_id,6c0b5890-8914-48e0-b8f4-0ae5ed3c14a5;target_model_names,gpt-4o;llm_output_file_id,file-ECBPW7ML9g7XHdwGgUPZaM;llm_output_file_model_id,e26453f9e76e7993680d0068d98c1f4cc205bbad0967a33c664893568ca743c2
...
def test_update_responses_input_with_model_file_id_mapping():
    ...
    managed_file_id = "derouter_proxy_file_123"
...
def test_update_responses_tools_with_model_file_id_mapping():
    ...
    # Managed file IDs
    managed_file_id_1 = "derouter_proxy_file_123"
    managed_file_id_2 = "derouter_proxy_file_456"
...
async def test_store_unified_file_id_with_none_file_object():
    ...
    prisma_client = AsyncMock()
    prisma_client.db.derouter_managedfiletable.upsert = AsyncMock(
        return_value=MagicMock()
    )
    internal_usage_cache = MagicMock()
    ...
    prisma_client.db.derouter_managedfiletable.upsert.assert_called_once()
    call_args = prisma_client.db.derouter_managedfiletable.upsert.call_args
...
async def test_store_unified_file_id_updates_file_metadata_on_existing_row():
    ...
    first_update = prisma_client.db.derouter_managedfiletable.upsert.await_args_list[
        0
    ].kwargs["data"]["update"]
    second_update = prisma_client.db.derouter_managedfiletable.upsert.await_args_list[
        1
    ].kwargs["data"]["update"]
...
async def test_afile_delete_returns_provider_response_when_stored_file_object_none():
    ...
    prisma_client.db.derouter_managedfiletable.find_first = AsyncMock(
        return_value=db_record
    )
    prisma_client.db.derouter_managedfiletable.delete = AsyncMock()
...
async def test_afile_retrieve_fetches_from_provider_when_file_object_none():
    ...
    with patch("derouter.afile_retrieve", new_callable=AsyncMock) as mock_afile_retrieve:
...
async def test_afile_retrieve_raises_error_when_no_router_and_file_object_none():
    ...
    with pytest.raises(Exception, match='DeRouter Managed File object with id=test-unified-file-id') as exc_info:
...
async def test_afile_retrieve_raises_error_for_non_managed_file():
    ...
    with pytest.raises(Exception, match='DeRouter Managed File object with id=non-existent-file-id') as exc_info:
        await proxy_managed_files.afile_retrieve(
...
async def test_list_batches_from_managed_objects_table():
    ...
    prisma_client.db.derouter_managedobjecttable.find_many.return_value = [
        batch_record_1,
        batch_record_2,
    ]
    ...
    prisma_client.db.derouter_managedobjecttable.find_many.assert_called_once_with(
...
def _create_unified_batch_id(model_id: str, batch_id: str) -> str:
    ...
    unified_str = f"derouter_proxy;model_id:{model_id};llm_batch_id:{batch_id}"
...
async def test_list_batches_registers_and_returns_unified_output_file_ids():
    ...
    prisma_client.db.derouter_managedfiletable.find_many = AsyncMock(
    ...
    bulk_lookup = prisma_client.db.derouter_managedfiletable.find_many.await_args
    ...
    assert decoded_output.startswith("derouter_proxy")
    ...
    upsert_calls = prisma_client.db.derouter_managedfiletable.upsert.await_args_list
    stored_raw_ids = {
...
async def test_list_batches_resolves_existing_managed_rows_without_minting():
    ...
    unified_input_file_id = base64.urlsafe_b64encode(
        b"derouter_proxy:application/octet-stream;unified_id,in-9;target_model_names,gpt-5-batch"
    ).decode()
    raw_output_file_ids = ["file-list-out-existing-1", "file-list-out-existing-2"]
    existing_unified_output_ids = [
        base64.urlsafe_b64encode(
            f"derouter_proxy:application/json;unified_id,u-{i};llm_output_file_id,{raw_id}".encode()
        ).decode()
    ...
    prisma_client = AsyncMock()
    prisma_client.db.derouter_managedobjecttable.find_many.return_value = records
    ...
    prisma_client.db.derouter_managedfiletable.find_many.assert_awaited_once()
    prisma_client.db.derouter_managedfiletable.find_first.assert_not_awaited()
    prisma_client.db.derouter_managedfiletable.upsert.assert_not_awaited()
...
async def test_list_batches_caps_page_size_at_100():
    ...
    assert (
        prisma_client.db.derouter_managedobjecttable.find_many.await_args.kwargs["take"]
        == 101
    )
    assert result["data"] == []
...
async def test_list_batches_from_managed_objects_table_filters_by_created_by():
    ...
    prisma_client.db.derouter_managedobjecttable.find_many.return_value = [batch_user1]
    ...
    prisma_client.db.derouter_managedobjecttable.find_many.assert_called_with(
    ...
    prisma_client.db.derouter_managedobjecttable.find_many.return_value = [batch_user2]
...
async def test_list_batches_pagination_uses_unified_object_id_cursor():
    ...
    prisma_client = AsyncMock()
    prisma_client.db.derouter_managedobjecttable.find_first.return_value = MagicMock()
    prisma_client.db.derouter_managedobjecttable.find_many.return_value = []
    ...
    _, call_kwargs = prisma_client.db.derouter_managedobjecttable.find_many.call_args
...
async def test_list_batches_pagination_walks_all_pages_without_loops_or_gaps():
    ...
    def _unified_id(i: int) -> str:
        raw = f"derouter_proxy;model_id:gpt-4o-batch;llm_batch_id:batch_{i:03d}"
    ...
    prisma_client = AsyncMock()
    prisma_client.db.derouter_managedobjecttable.find_many = AsyncMock(
        side_effect=fake_find_many
    )
    prisma_client.db.derouter_managedobjecttable.find_first = AsyncMock(
...
def _managed_batch_row(index, file_object=None):
    ...
    raw = f"derouter_proxy;model_id:gpt-4o-batch;llm_batch_id:batch_{index:03d}"
...
async def test_list_batches_rejects_unknown_after_cursor():
    ...
    prisma_client.db.derouter_managedobjecttable.find_many = AsyncMock(return_value=[])
    ...
    prisma_client.db.derouter_managedobjecttable.find_many.assert_not_called()
    prisma_client.db.derouter_managedobjecttable.find_first.assert_called_once_with(
        where={
            "file_purpose": "batch",
            "created_by": "test-user",
...
async def test_list_batches_treats_empty_after_as_no_cursor():
    ...
    prisma_client.db.derouter_managedobjecttable.find_first.assert_not_called()
    _, call_kwargs = prisma_client.db.derouter_managedobjecttable.find_many.call_args
...
async def test_list_batches_has_more_false_on_exactly_full_final_page():
    ...
    assert prisma_client.db.derouter_managedobjecttable.find_many.call_count == 2
...
async def test_list_batches_bounds_the_queries_a_deep_unparseable_run_costs():
    ...
    assert (
        prisma_client.db.derouter_managedobjecttable.find_many.call_count
...
async def test_list_batches_reads_one_chunk_when_the_first_one_fills_the_page():
    ...
    assert prisma_client.db.derouter_managedobjecttable.find_many.call_count == 1
...
async def test_return_unified_file_id_includes_expires_at():
    ...
    result = await _PROXY_DeRouterManagedFiles.return_unified_file_id(
...
# Reference: https://github.com/Delify-Solutions/derouter/pull/17401/files
...
async def test_user_b_cannot_retrieve_user_a_batch():
    ...
    prisma_client.db.derouter_managedobjecttable.find_first.return_value = batch_record
...
async def test_user_b_cannot_retrieve_user_a_file():
    ...
    prisma_client.db.derouter_managedfiletable.find_first.return_value = file_record
...
async def test_list_batches_only_returns_user_own_batches():
    ...
    prisma_client.db.derouter_managedobjecttable.find_many.return_value = [batch_user_a]
...
async def test_team_b_cannot_access_team_a_provider_format_batch(
    ...
    Regression test: before this check only unified (derouter_proxy-prefixed)
    ...
    prisma_client = AsyncMock()
    prisma_client.db.derouter_managedobjecttable.find_first.return_value = (
    ...
    assert exc_info.value.status_code == 403
    prisma_client.db.derouter_managedobjecttable.find_first.assert_awaited_once_with(
...
async def test_provider_format_batch_without_ownership_row_stays_accessible():
    ...
    prisma_client = AsyncMock()
    prisma_client.db.derouter_managedobjecttable.find_first.return_value = None
    proxy_managed_files = _PROXY_DeRouterManagedFiles(
        DualCache(), prisma_client=prisma_client
    )
...
async def test_fine_tuning_provider_format_id_not_enforced():
    ...
    prisma_client.db.derouter_managedobjecttable.find_first.assert_not_awaited()
...
async def test_team_b_cannot_access_team_a_provider_format_file(
    ...
    prisma_client = AsyncMock()
    prisma_client.db.derouter_managedfiletable.find_first.return_value = (
        _owned_record(created_by="user_a", team_id="team_a")
    )
    proxy_managed_files = _PROXY_DeRouterManagedFiles(
        MagicMock(), prisma_client=prisma_client
    ...
    assert exc_info.value.status_code == 403
    prisma_client.db.derouter_managedfiletable.find_first.assert_awaited_once_with(
...
async def test_provider_format_file_without_ownership_row_stays_accessible():
    ...
    prisma_client = AsyncMock()
    prisma_client.db.derouter_managedfiletable.find_first.return_value = None
    proxy_managed_files = _PROXY_DeRouterManagedFiles(
        MagicMock(), prisma_client=prisma_client
    )
...
async def test_post_call_batch_create_stores_ownership_row(batch_id):
    ...
    upsert_call = prisma_client.db.derouter_managedobjecttable.upsert.await_args
    ...
    prisma_client.db.derouter_managedobjecttable.update_many.assert_not_awaited()
...
async def test_post_call_batch_sync_does_not_claim_ownership():
    ...
    prisma_client = AsyncMock()
    prisma_client.db.derouter_managedobjecttable.update_many.return_value = 0
    proxy_managed_files = _PROXY_DeRouterManagedFiles(
        MagicMock(async_set_cache=AsyncMock()), prisma_client=prisma_client
    )
    ...
    prisma_client.db.derouter_managedobjecttable.upsert.assert_not_awaited()
    prisma_client.db.derouter_managedobjecttable.update_many.assert_awaited_once()
...
async def test_post_call_batch_sync_updates_existing_row():
    ...
    prisma_client = AsyncMock()
    prisma_client.db.derouter_managedobjecttable.update_many.return_value = 1
    prisma_client.db.derouter_managedobjecttable.find_first.return_value = (
        _owned_record(created_by="user_a", team_id="team_a")
    )
    proxy_managed_files = _PROXY_DeRouterManagedFiles(
    ...
    update_call = prisma_client.db.derouter_managedobjecttable.update_many.await_args
...
async def test_post_call_batch_sync_stores_output_file_ownership_from_batch_row():
    ...
    file_upsert = prisma_client.db.derouter_managedfiletable.upsert.await_args
...
async def test_file_list_cursors_are_scoped_to_the_caller():
    ...
    prisma_client = AsyncMock()
    prisma_client.db.derouter_managedfiletable.find_many.return_value = []
    proxy_managed_files = _PROXY_DeRouterManagedFiles(
        DualCache(), prisma_client=prisma_client
    )
...
async def test_file_list_cursors_follow_the_owner_scoped_page():
    ...
    managed_row.unified_file_id = "derouter_proxy:mine"
    ...
    prisma_client = AsyncMock()
    prisma_client.db.derouter_managedfiletable.find_many.return_value = [managed_row]
    proxy_managed_files = _PROXY_DeRouterManagedFiles(
        DualCache(), prisma_client=prisma_client
    )
    ...
    assert [file_object.id for file_object in response.data] == ["derouter_proxy:mine"]
    assert response.first_id == "derouter_proxy:mine"
    assert response.last_id == "derouter_proxy:mine"
    assert response.has_more is False
