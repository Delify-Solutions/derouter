...
from derouter.types.utils import BudgetConfig, GenericBudgetConfigType
...
async def test_async_prometheus_success_logging_with_callbacks(prometheus_logger):
    ...
    diff = await op()
    await asyncio.sleep(2)
    assert diff["derouter_requests_metric_total"] == 1.0
...
async def test_async_prometheus_budget_logging_with_callbacks(prometheus_logger):
    ...
    # TODO: Should implement `derouter_provider_remaining_budget_metric` in prometheus.py
    assert diff.get("derouter_provider_remaining_budget_metric", 50.0) == 50.0
...
async def test_prometheus_metric_tracking():
    ...
    derouter._async_success_callback = [mock_prometheus]
...
class CustomPrometheusLogger(PrometheusLogger):
    ...
    def increment_deployment_cooled_down(
        ...
        self.deployment_cooled_downs.append(
            [derouter_model_name, model_id, api_base, api_provider, exception_status]
        )
