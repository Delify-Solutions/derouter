...
def test_initialize_budget_metrics_cron_job():
    ...
    derouter.callbacks = [prometheus_logger]
...
def test_end_user_not_tracked_for_all_prometheus_metrics():
    ...
    original_setting = getattr(
        derouter, "enable_end_user_cost_tracking_prometheus_only", None
    )
    derouter.enable_end_user_cost_tracking_prometheus_only = None  # Default behavior
    ...
    try:
        ...
        derouter.enable_end_user_cost_tracking_prometheus_only = True
        ...
        derouter.enable_end_user_cost_tracking_prometheus_only = original_setting
...
def test_prometheus_config_parsing():
    ...
    # Set configuration
    derouter.prometheus_metrics_config = test_config
...
def test_get_metric_labels():
    ...
    test_config = [
        {
            "group": "service_metrics",
            "metrics": ["derouter_deployment_failure_responses"],
            "include_labels": ["derouter_model_name", "api_provider"],
        }
    ]
    ...
    # Get filtered labels
    labels = logger.get_labels_for_metric("derouter_deployment_failure_responses")

    # Verify only configured labels are returned
    assert "derouter_model_name" in labels
    assert "api_provider" in labels
    # These should be filtered out even if they're in the default labels
    assert (
        len([l for l in labels if l not in ["derouter_model_name", "api_provider"]]) == 0
    )
...
def test_no_prometheus_config():
    ...
    derouter.prometheus_metrics_config = None
...
def test_prometheus_metrics_config_type():
    ...
    valid_config = PrometheusMetricsConfig(
        group="service_metrics",
        metrics=["derouter_deployment_failure_responses"],
        include_labels=["derouter_model_name"],
    )

    assert valid_config.group == "service_metrics"
    assert valid_config.metrics == ["derouter_deployment_failure_responses"]
    assert valid_config.include_labels == ["derouter_model_name"]
...
def test_basic_functionality():
    ...
    assert derouter.prometheus_metrics_config is not None
    assert len(derouter.prometheus_metrics_config) == 1
    assert derouter.prometheus_metrics_config[0]["group"] == "service_metrics"
    assert (
        "derouter_deployment_failure_responses"
        in derouter.prometheus_metrics_config[0]["metrics"]
    )
    ...
    test_config = [
        {
            ...
            "metrics": [
                ...
                "derouter_deployment_total_requests",  # valid metric
            ],
            "include_labels": ["derouter_model_name"],
        }
    ]
...
def test_invalid_labels_validation():
    ...
    test_config = [
        {
            "group": "service_metrics",
            "metrics": ["derouter_deployment_total_requests"],
            "include_labels": [
                "derouter_model_name",  # valid label
...
def test_valid_configuration_passes_validation():
    ...
    try:
        ...
        assert "derouter_deployment_total_requests" in logger.enabled_metrics
        assert "derouter_deployment_failure_responses" in logger.enabled_metrics
...
def reset_prometheus_exclude_settings():
    ...
    prev_metrics = derouter.prometheus_exclude_metrics
    prev_labels = derouter.prometheus_exclude_labels
    prev_config = derouter.prometheus_metrics_config
    try:
        yield
    finally:
        derouter.prometheus_exclude_metrics = prev_metrics
        derouter.prometheus_exclude_labels = prev_labels
        derouter.prometheus_metrics_config = prev_config
...
def test_exclude_metrics_disables_only_listed_metrics(reset_prometheus_exclude_settings):
    ...
    from derouter.types.integrations.prometheus import NoOpMetric

    clear_prometheus_registry()
    derouter.prometheus_metrics_config = None
    derouter.prometheus_exclude_labels = None
    derouter.prometheus_exclude_metrics = [
    ...
    assert isinstance(logger.derouter_spend_metric, NoOpMetric)
    assert isinstance(logger.derouter_input_tokens_metric, NoOpMetric)
    # A metric not in the exclude list is still a real prometheus metric
    assert not isinstance(logger.derouter_output_tokens_metric, NoOpMetric)
...
def test_exclude_metrics_wins_over_include_config(reset_prometheus_exclude_settings):
    ...
    clear_prometheus_registry()
    derouter.prometheus_exclude_labels = None
    derouter.prometheus_metrics_config = [
        {
            "group": "tokens",
            "metrics": ["derouter_input_tokens_metric", "derouter_output_tokens_metric"],
        }
    ]
    derouter.prometheus_exclude_metrics = ["derouter_input_tokens_metric"]
...
def test_exclude_labels_dropped_globally(reset_prometheus_exclude_settings):
    ...
    clear_prometheus_registry()
    derouter.prometheus_metrics_config = None
    derouter.prometheus_exclude_metrics = None
    derouter.prometheus_exclude_labels = ["hashed_api_key", "api_key_alias"]

    logger = PrometheusLogger()

    for metric_name in ("derouter_spend_metric", "derouter_input_tokens_metric"):
...
def test_exclude_labels_intersect_with_include_labels(reset_prometheus_exclude_settings):
    ...
    derouter.prometheus_metrics_config = [
        {
            "group": "spend",
            "metrics": ["derouter_spend_metric"],
            "include_labels": ["hashed_api_key", "team", "api_provider"],
        }
    ]
    derouter.prometheus_exclude_labels = ["hashed_api_key"]

    logger = PrometheusLogger()

    labels = logger.get_labels_for_metric("derouter_spend_metric")
    assert "hashed_api_key" not in labels
...
def test_no_exclude_settings_is_backward_compatible(reset_prometheus_exclude_settings):
    ...
    assert not isinstance(logger.derouter_spend_metric, NoOpMetric)
    default_labels = PrometheusMetricLabels.get_labels("derouter_spend_metric")
    assert logger.get_labels_for_metric("derouter_spend_metric") == default_labels
...
def test_invalid_exclude_metric_name_raises(reset_prometheus_exclude_settings):
    ...
    derouter.prometheus_metrics_config = None
    derouter.prometheus_exclude_labels = None
    derouter.prometheus_exclude_metrics = ["not_a_real_metric"]
...
def test_invalid_exclude_label_name_raises(reset_prometheus_exclude_settings):
    ...
    derouter.prometheus_metrics_config = None
    derouter.prometheus_exclude_metrics = None
    derouter.prometheus_exclude_labels = ["not_a_real_label"]
...
def test_exclude_hardcoded_label_name_is_accepted(reset_prometheus_exclude_settings, hardcoded_label):
    ...
    derouter.prometheus_metrics_config = None
    derouter.prometheus_exclude_metrics = None
    derouter.prometheus_exclude_labels = [hardcoded_label]
...
def test_exclude_labels_dropped_from_hardcoded_metric(reset_prometheus_exclude_settings):
    ...
    derouter.prometheus_metrics_config = None
    derouter.prometheus_exclude_metrics = None
    derouter.prometheus_exclude_labels = ["guardrail_name"]

    logger = PrometheusLogger()

    labelnames = logger.derouter_guardrail_latency_metric._metric._labelnames
...
def test_hardcoded_metric_emission_omits_excluded_label(reset_prometheus_exclude_settings):
    ...
    logger = PrometheusLogger()
    logger.derouter_guardrail_latency_metric.labels(
    ...
    assert "derouter_guardrail_latency_seconds_bucket" in scrape
...
def test_exclude_only_hardcoded_label_drops_all_labels(reset_prometheus_exclude_settings):
    ...
    derouter.prometheus_metrics_config = None
    derouter.prometheus_exclude_metrics = None
    derouter.prometheus_exclude_labels = ["result", "api_provider"]

    logger = PrometheusLogger()

    assert logger.derouter_managed_file_deleted_total._metric._labelnames == ()
    assert logger.derouter_provider_remaining_budget_metric._metric._labelnames == ()

    logger.derouter_managed_file_deleted_total.labels(result="blocked").inc()
    logger.derouter_provider_remaining_budget_metric.labels("anthropic").set(5.0)
...
def test_exclude_labels_does_not_touch_unrelated_metrics(reset_prometheus_exclude_settings):
    ...
    from derouter.integrations.prometheus import _ExcludedLabelMetric
    ...
    assert not isinstance(logger.derouter_spend_metric, _ExcludedLabelMetric)
    assert not isinstance(logger.derouter_provider_remaining_budget_metric, _ExcludedLabelMetric)
    assert isinstance(logger.derouter_guardrail_latency_metric, _ExcludedLabelMetric)
...
def mock_prometheus_logger():
    ...
    with patch("derouter.proxy.proxy_server.premium_user", True):
        ...
        logger.derouter_proxy_total_requests_metric = MockCounter(
            "derouter_proxy_total_requests_metric"
        )
        logger.derouter_tokens_metric = MockCounter("derouter_total_tokens")
        logger.derouter_input_tokens_metric = MockCounter("derouter_input_tokens")
        logger.derouter_output_tokens_metric = MockCounter("derouter_output_tokens")
        logger.derouter_spend_metric = MockCounter("derouter_spend_metric")
        logger.derouter_requests_metric = MockCounter("derouter_requests_metric")
...
async def test_request_counter_semantic_validation(mock_prometheus_logger):
    ...
    This test specifically catches the bug where derouter_proxy_total_requests_metric
    ...
    total_requests_metric = mock_prometheus_logger.derouter_proxy_total_requests_metric
    assert (
    ...
    tokens_metric = mock_prometheus_logger.derouter_tokens_metric
    assert (
...
async def test_multiple_requests_counter_semantics(mock_prometheus_logger):
    ...
    total_request_increments = sum(
        mock_prometheus_logger.derouter_proxy_total_requests_metric.inc_calls
    )
    total_token_increments = sum(mock_prometheus_logger.derouter_tokens_metric.inc_calls)
...
async def test_streaming_request_counter_semantics(mock_prometheus_logger):
    ...
    for (
        inc_value
    ) in mock_prometheus_logger.derouter_proxy_total_requests_metric.inc_calls:
        assert (
            inc_value == 1
...
async def test_spend_counter_semantics(mock_prometheus_logger):
    ...
    spend_metric = mock_prometheus_logger.derouter_spend_metric
...
def test_callback_failure_metric_increments(prometheus_logger):
    ...
    This tests the derouter_callback_logging_failures_metric counter.
    """
    # Get initial value
    initial_value = 0
    try:
        initial_value = (
            prometheus_logger.derouter_callback_logging_failures_metric.labels(
    ...
    current_value = prometheus_logger.derouter_callback_logging_failures_metric.labels(
        callback_name="S3Logger"
    )._value.get()
    ...
    langfuse_value = prometheus_logger.derouter_callback_logging_failures_metric.labels(
    ...
    s3_value = prometheus_logger.derouter_callback_logging_failures_metric.labels(
        callback_name="S3Logger"
    )._value.get()
...
def test_callback_failure_metric_different_callbacks(prometheus_logger):
    ...
    for callback_name in callbacks_to_test:
        ...
        initial = 0
        try:
            initial = prometheus_logger.derouter_callback_logging_failures_metric.labels(
        ...
        # Verify incremented
        current = prometheus_logger.derouter_callback_logging_failures_metric.labels(
            callback_name=callback_name
        )._value.get()
...
async def test_langfuse_callback_failure_metric(prometheus_logger):
    ...
    derouter_callback_logging_failures_metric is incremented with callback_name="langfuse".
    """
    from unittest.mock import MagicMock
    ...
    initial_value = 0
    try:
        initial_value = prometheus_logger.derouter_callback_logging_failures_metric.labels(
    ...
    with patch("derouter.integrations.langfuse.langfuse_prompt_management.langfuse_client_init"):
        langfuse_logger = LangfusePromptManagement()
    
    # Mock the log_event_on_langfuse to raise an exception
    with patch(
        "derouter.integrations.langfuse.langfuse_prompt_management.LangFuseHandler.get_langfuse_logger_for_request"
    ) as mock_get_logger:
...
async def test_langfuse_otel_callback_failure_metric(prometheus_logger):
    ...
    derouter_callback_logging_failures_metric is incremented with callback_name="langfuse_otel".
    """
    from unittest.mock import MagicMock
    ...
    with patch("derouter.integrations.opentelemetry.OpenTelemetry.__init__", return_value=None):
