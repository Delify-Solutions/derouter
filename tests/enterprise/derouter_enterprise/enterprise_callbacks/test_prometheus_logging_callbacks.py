...
def known_model_router():
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "openai/gpt-5-mini", "api_key": "fake-key"},
            },
            {
    ...
    with patch("derouter.proxy.proxy_server.llm_router", router, create=True):  # test-quality-ok: production reads proxy_server.llm_router lazily, no injection seam
        yield router
...
async def test_async_log_success_event(prometheus_logger):
    ...
    prometheus_logger.derouter_requests_metric = MagicMock()
    prometheus_logger.derouter_spend_metric = MagicMock()

    # Token Metrics
    prometheus_logger.derouter_tokens_metric = MagicMock()
    prometheus_logger.derouter_input_tokens_metric = MagicMock()
    prometheus_logger.derouter_output_tokens_metric = MagicMock()

    # Remaining Budget Metrics
    prometheus_logger.derouter_remaining_team_budget_metric = MagicMock()
    prometheus_logger.derouter_remaining_api_key_budget_metric = MagicMock()

    # Virtual Key Rate limit Metrics
    prometheus_logger.derouter_remaining_api_key_requests_for_model = MagicMock()
    prometheus_logger.derouter_remaining_api_key_tokens_for_model = MagicMock()

    # Latency Metrics
    prometheus_logger.derouter_llm_api_time_to_first_token_metric = MagicMock()
    prometheus_logger.derouter_llm_api_latency_metric = MagicMock()
    prometheus_logger.derouter_request_total_latency_metric = MagicMock()
    ...
    prometheus_logger.derouter_requests_metric.labels.assert_called()
    prometheus_logger.derouter_spend_metric.labels.assert_called()

    # Token Metrics
    prometheus_logger.derouter_tokens_metric.labels.assert_called()
    prometheus_logger.derouter_input_tokens_metric.labels.assert_called()
    prometheus_logger.derouter_output_tokens_metric.labels.assert_called()

    # Remaining Budget Metrics
    prometheus_logger.derouter_remaining_team_budget_metric.labels.assert_called()
    prometheus_logger.derouter_remaining_api_key_budget_metric.labels.assert_called()

    # Virtual Key Rate limit Metrics
    prometheus_logger.derouter_remaining_api_key_requests_for_model.labels.assert_called()
    prometheus_logger.derouter_remaining_api_key_tokens_for_model.labels.assert_called()

    # Latency Metrics
    prometheus_logger.derouter_llm_api_time_to_first_token_metric.labels.assert_called()
    prometheus_logger.derouter_llm_api_latency_metric.labels.assert_called()
    prometheus_logger.derouter_request_total_latency_metric.labels.assert_called()
...
def test_increment_token_metrics(prometheus_logger):
    ...
    enum_values = UserAPIKeyLabelValues(
        derouter_model_name=standard_logging_payload["model"],
    ...
    prometheus_logger.derouter_tokens_metric.labels.assert_called_once_with(
        end_user=None,
        user=None,
        user_email=None,
    ...
    prometheus_logger.derouter_tokens_metric.labels().inc.assert_called_once_with(100)

    prometheus_logger.derouter_input_tokens_metric.labels.assert_called_once_with(
        end_user=None,
        user=None,
        user_email=None,
        hashed_api_key="test_hash",
    ...
    prometheus_logger.derouter_input_tokens_metric.labels().inc.assert_called_once_with(
        50
    )

    prometheus_logger.derouter_output_tokens_metric.labels.assert_called_once_with(
        end_user=None,
        user=None,
        user_email=None,
        hashed_api_key="test_hash",
    ...
    prometheus_logger.derouter_output_tokens_metric.labels().inc.assert_called_once_with(
        50
    )
...
async def test_increment_remaining_budget_metrics(prometheus_logger):
    ...
    prometheus_logger.derouter_remaining_team_budget_metric = MagicMock()
    prometheus_logger.derouter_remaining_api_key_budget_metric = MagicMock()
    prometheus_logger.derouter_team_max_budget_metric = MagicMock()
    prometheus_logger.derouter_api_key_max_budget_metric = MagicMock()
    prometheus_logger.derouter_team_budget_remaining_hours_metric = MagicMock()
    prometheus_logger.derouter_api_key_budget_remaining_hours_metric = MagicMock()
    ...
    with (
        patch("derouter.proxy.auth.auth_checks.get_team_object") as mock_get_team,
        patch("derouter.proxy.auth.auth_checks.get_key_object") as mock_get_key,
    ):
        ...
        prometheus_logger.derouter_remaining_team_budget_metric.labels.assert_called_once_with(
            team="team1", team_alias="team_alias1"
        )
        prometheus_logger.derouter_remaining_team_budget_metric.labels().set.assert_called_once_with(
            40  # 100 - (50 + 10)
        )

        prometheus_logger.derouter_remaining_api_key_budget_metric.labels.assert_called_once_with(
            hashed_api_key="key1", api_key_alias="alias1"
        )
        prometheus_logger.derouter_remaining_api_key_budget_metric.labels().set.assert_called_once_with(
            40  # 75 - (25 + 10)
        )

        # Test max budget metrics
        prometheus_logger.derouter_team_max_budget_metric.labels.assert_called_once_with(
            team="team1", team_alias="team_alias1"
        )
        prometheus_logger.derouter_team_max_budget_metric.labels().set.assert_called_once_with(
            100
        )

        prometheus_logger.derouter_api_key_max_budget_metric.labels.assert_called_once_with(
            hashed_api_key="key1", api_key_alias="alias1"
        )
        prometheus_logger.derouter_api_key_max_budget_metric.labels().set.assert_called_once_with(
            75
        )

        # Test remaining hours metrics
        prometheus_logger.derouter_team_budget_remaining_hours_metric.labels.assert_called_once_with(
            team="team1", team_alias="team_alias1"
        )
        # The remaining hours should be approximately 10 (with some small difference due to test execution time)
        remaining_hours_call = prometheus_logger.derouter_team_budget_remaining_hours_metric.labels().set.call_args[
            0
        ][
            0
        ]
        ...
        prometheus_logger.derouter_api_key_budget_remaining_hours_metric.labels.assert_called_once_with(
            hashed_api_key="key1", api_key_alias="alias1"
        )
        # The remaining hours should be approximately 10 (with some small difference due to test execution time)
        remaining_hours_call = prometheus_logger.derouter_api_key_budget_remaining_hours_metric.labels().set.call_args[
            0
        ][
            0
        ]
...
def test_set_latency_metrics(prometheus_logger):
    ...
    prometheus_logger.derouter_llm_api_time_to_first_token_metric.labels.assert_called_once_with(
        end_user=None,
        user="test_user",
        hashed_api_key="test_hash",
        api_key_alias="test_alias",
    ...
    prometheus_logger.derouter_llm_api_time_to_first_token_metric.labels().observe.assert_called_once_with(
        0.5
    )

    # end_time - api_call_start_time
    prometheus_logger.derouter_llm_api_latency_metric.labels.assert_called_once_with(
        end_user=None,
        user="test_user",
        hashed_api_key="test_hash",
        api_key_alias="test_alias",
    ...
    prometheus_logger.derouter_llm_api_latency_metric.labels().observe.assert_called_once_with(
        1.5
    )

    # total latency for the request
    prometheus_logger.derouter_request_total_latency_metric.labels.assert_called_once_with(
        end_user=None,
        user="test_user",
        hashed_api_key="test_hash",
        api_key_alias="test_alias",
    ...
    prometheus_logger.derouter_request_total_latency_metric.labels().observe.assert_called_once_with(
        2.0
    )
...
def test_set_latency_metrics_missing_timestamps(prometheus_logger):
    ...
    prometheus_logger.derouter_llm_api_time_to_first_token_metric.labels.assert_not_called()

    # Other metrics should still be called
    prometheus_logger.derouter_llm_api_latency_metric.labels.assert_called_once()
    prometheus_logger.derouter_request_total_latency_metric.labels.assert_called_once()
...
def test_set_latency_metrics_missing_api_call_start(prometheus_logger):
    ...
    prometheus_logger.derouter_llm_api_time_to_first_token_metric.labels.assert_not_called()
    prometheus_logger.derouter_llm_api_latency_metric.labels.assert_not_called()
...
def test_increment_top_level_request_and_spend_metrics(prometheus_logger):
    ...
    - derouter_requests_metric is incremented by 1
    - derouter_spend_metric is incremented by the response cost in the standard logging payload
    """
    standard_logging_payload = create_standard_logging_payload()
    enum_values = UserAPIKeyLabelValues(
        derouter_model_name=standard_logging_payload["model"],
    ...
    prometheus_logger.derouter_requests_metric.labels.assert_called_once_with(
        end_user=None,
        user=None,
        user_email=None,
        hashed_api_key="test_hash",
    ...
    prometheus_logger.derouter_requests_metric.labels().inc.assert_called_once()

    # The spend metric uses keyword arguments (same as requests metric)
    prometheus_logger.derouter_spend_metric.labels.assert_called_once_with(
        end_user=None,
        user=None,
        user_email=None,
        hashed_api_key="test_hash",
    ...
    prometheus_logger.derouter_spend_metric.labels().inc.assert_called_once_with(0.1)
...
async def test_async_log_failure_event(prometheus_logger):
    ...
    # Mock the metrics
    prometheus_logger.derouter_llm_api_failed_requests_metric = MagicMock()
    prometheus_logger.derouter_deployment_failure_responses = MagicMock()
    prometheus_logger.derouter_deployment_total_requests = MagicMock()
    ...
    # derouter_llm_api_failed_requests_metric incremented
    # Labels: end_user, hashed_api_key, api_key_alias, model, team, team_alias, user, model_id
    prometheus_logger.derouter_llm_api_failed_requests_metric.labels.assert_called_once_with(
        end_user=None,
        hashed_api_key="test_hash",
        api_key_alias="test_alias",
        model="gpt-5-mini",
    ...
    prometheus_logger.derouter_llm_api_failed_requests_metric.labels().inc.assert_called_once()

    # deployment should be marked in partial outage
    prometheus_logger.set_deployment_partial_outage.assert_called_once_with(
        derouter_model_name="gpt-5-mini",
        model_id="model-123",
        api_base="https://api.openai.com",
        api_provider="openai",
    )
    ...
    prometheus_logger.derouter_deployment_failure_responses.labels.assert_called_once()
    actual_failure_labels = (
        prometheus_logger.derouter_deployment_failure_responses.labels.call_args.kwargs
    )
    ...
    prometheus_logger.derouter_deployment_failure_responses.labels().inc.assert_called_once()

    # deployment total requests incremented - verify key labels are populated
    prometheus_logger.derouter_deployment_total_requests.labels.assert_called_once()
    actual_total_labels = (
        prometheus_logger.derouter_deployment_total_requests.labels.call_args.kwargs
    )
    ...
    prometheus_logger.derouter_deployment_total_requests.labels().inc.assert_called_once()


@pytest.mark.asyncio
async def test_async_log_failure_event_derouter_side_rate_limit(prometheus_logger, known_model_router):
    """DeRouter-side reject (no deployment picked) routes the requested model
    ...
    rate_limit_exc = Exception("DeRouter rate limit exceeded")
    ...
    assert actual_failure_labels["derouter_model_name"] == ""
...
async def test_async_post_call_failure_hook(prometheus_logger, known_model_router):
    ...
    it should increment the derouter_proxy_failed_requests_metric and derouter_proxy_total_requests_metric
    """
    ...
    original_emit = derouter.prometheus_emit_rate_limit_labels
    derouter.prometheus_emit_rate_limit_labels = True
    prometheus_logger._cached_metric_labels.clear()

    # Mock the prometheus metrics
    prometheus_logger.derouter_proxy_failed_requests_metric = MagicMock()
    prometheus_logger.derouter_proxy_total_requests_metric = MagicMock()

    # Create test data
    request_data = {"model": "gpt-5-mini"}

    original_exception = derouter.RateLimitError(
    ...
    try:
        ...
        prometheus_logger.derouter_proxy_failed_requests_metric.labels.assert_called_once_with(
            end_user=None,
            user="test_user",
            user_email=None,
        ...
        derouter.prometheus_emit_rate_limit_labels = original_emit
        prometheus_logger._cached_metric_labels.clear()
    prometheus_logger.derouter_proxy_failed_requests_metric.labels().inc.assert_called_once()

    # Assert total requests metric was incremented with correct labels
    prometheus_logger.derouter_proxy_total_requests_metric.labels.assert_called_once_with(
        end_user=None,
        hashed_api_key="test_key",
        api_key_alias="test_alias",
        requested_model="gpt-5-mini",
    ...
    prometheus_logger.derouter_proxy_total_requests_metric.labels().inc.assert_called_once()
...
async def test_async_post_call_success_hook(prometheus_logger):
    ...
    derouter_proxy_total_requests_metric is NOT incremented here to avoid double-counting.
    ...
    prometheus_logger.derouter_proxy_total_requests_metric.labels.assert_not_called()


def test_set_llm_deployment_success_metrics(prometheus_logger):
    # Mock all the metrics used in the method
    prometheus_logger.derouter_remaining_requests_metric = MagicMock()
    prometheus_logger.derouter_remaining_tokens_metric = MagicMock()
    prometheus_logger.derouter_deployment_success_responses = MagicMock()
    prometheus_logger.derouter_deployment_total_requests = MagicMock()
    prometheus_logger.derouter_deployment_latency_per_output_token = MagicMock()
    prometheus_logger.set_deployment_healthy = MagicMock()
    prometheus_logger.derouter_overhead_latency_metric = MagicMock()
    ...
    standard_logging_payload["hidden_params"]["derouter_overhead_time_ms"] = 100
    ...
    prometheus_logger.derouter_remaining_requests_metric.labels.assert_called_once_with(
        ...
        derouter_model_name="gpt-5-mini",  # actual model used - derouter model name
        hashed_api_key=standard_logging_payload["metadata"]["user_api_key_hash"],
        api_key_alias=standard_logging_payload["metadata"]["user_api_key_alias"],
        model_id="model-123",
    )

    prometheus_logger.derouter_remaining_requests_metric.labels().set.assert_called_once_with(
        123
    )

    # Verify remaining tokens metric
    prometheus_logger.derouter_remaining_tokens_metric.labels.assert_called_once_with(
        api_base="https://api.openai.com",
        api_key_alias=standard_logging_payload["metadata"]["user_api_key_alias"],
        api_provider="openai",
        hashed_api_key=standard_logging_payload["metadata"]["user_api_key_hash"],
    ...
    prometheus_logger.derouter_remaining_tokens_metric.labels().set.assert_called_once_with(
        4321
    )
    ...
    prometheus_logger.derouter_deployment_success_responses.labels.assert_called_once_with(
        derouter_model_name="gpt-5-mini",
        model_id="model-123",
        api_base="https://api.openai.com",
        api_provider="openai",
    ...
    prometheus_logger.derouter_deployment_success_responses.labels().inc.assert_called_once()

    # Verify total requests metric
    prometheus_logger.derouter_deployment_total_requests.labels.assert_called_once_with(
        derouter_model_name="gpt-5-mini",
        model_id="model-123",
        api_base="https://api.openai.com",
        api_provider="openai",
    ...
    prometheus_logger.derouter_deployment_latency_per_output_token.labels.assert_called_once_with(
        derouter_model_name="gpt-5-mini",
        model_id="model-123",
        api_base="https://api.openai.com",
        api_provider="openai",
    ...
    prometheus_logger.derouter_overhead_latency_metric.labels.assert_called_once_with(
        api_base="https://api.openai.com",
        api_key_alias=standard_logging_payload["metadata"]["user_api_key_alias"],
        api_provider="openai",
        hashed_api_key=standard_logging_payload["metadata"]["user_api_key_hash"],
    ...
    prometheus_logger.derouter_deployment_latency_per_output_token.labels().observe.assert_called_once_with(
...
async def test_log_success_fallback_event(prometheus_logger, known_model_router):
    prometheus_logger.derouter_deployment_successful_fallbacks = MagicMock()
    ...
    prometheus_logger.derouter_deployment_successful_fallbacks.labels.assert_called_once_with(
    ...
    prometheus_logger.derouter_deployment_successful_fallbacks.labels().inc.assert_called_once()


@pytest.mark.asyncio
async def test_log_failure_fallback_event(prometheus_logger, known_model_router):
    prometheus_logger.derouter_deployment_failed_fallbacks = MagicMock()
    ...
    prometheus_logger.derouter_deployment_failed_fallbacks.labels.assert_called_once_with(
        requested_model=original_model_group,
        fallback_model="gpt-5.5",
        hashed_api_key="test_hash",
        api_key_alias="test_alias",
    ...
    prometheus_logger.derouter_deployment_failed_fallbacks.labels().inc.assert_called_once()


def test_deployment_state_management(prometheus_logger):
    prometheus_logger.derouter_deployment_state = MagicMock()
    ...
    prometheus_logger.derouter_deployment_state.labels.assert_called_with(
        derouter_model_name=test_params["derouter_model_name"],
        model_id=test_params["model_id"],
        api_base=test_params["api_base"],
        api_provider=test_params["api_provider"],
    )
    prometheus_logger.derouter_deployment_state.labels().set.assert_called_with(0)

    # Test set_deployment_partial_outage (state=1)
    prometheus_logger.set_deployment_partial_outage(**test_params)
    prometheus_logger.derouter_deployment_state.labels().set.assert_called_with(1)

    # Test set_deployment_complete_outage (state=2)
    prometheus_logger.set_deployment_complete_outage(**test_params)
    prometheus_logger.derouter_deployment_state.labels().set.assert_called_with(2)
...
def test_increment_deployment_cooled_down(prometheus_logger):
    ...
    prometheus_logger.derouter_deployment_cooled_down = MagicMock()
    prometheus_logger.derouter_deployment_cooled_down.labels = MagicMock(
    ...
    prometheus_logger.derouter_deployment_cooled_down.labels.assert_called_once_with(
...
def test_prometheus_factory(monkeypatch, enable_end_user_cost_tracking_prometheus_only):
    from derouter.integrations.prometheus import prometheus_label_factory
...
def test_get_custom_labels_from_metadata(monkeypatch):
    monkeypatch.setattr(
        "derouter.custom_prometheus_metadata_labels", ["metadata.foo", "metadata.bar"]
    )
...
def test_get_custom_labels_from_metadata_tags(monkeypatch):
    monkeypatch.setattr("derouter.custom_prometheus_metadata_labels", [])
    metadata = {"foo": "bar", "bar": "baz", "taz": "qux"}
    assert get_custom_labels_from_metadata(metadata) == {}
...
def test_get_custom_labels_from_top_level_metadata(monkeypatch):
    ...
    """
    monkeypatch.setattr(
        "derouter.custom_prometheus_metadata_labels",
...
async def test_async_log_success_event_with_top_level_metadata(
    ...
    monkeypatch.setattr(
        "derouter.custom_prometheus_metadata_labels", ["requester_ip_address"]
    )
    ...
    prometheus_logger.derouter_requests_metric = create_mock_metric()
    prometheus_logger.derouter_spend_metric = create_mock_metric()
    prometheus_logger.derouter_tokens_metric = create_mock_metric()
    prometheus_logger.derouter_input_tokens_metric = create_mock_metric()
    prometheus_logger.derouter_output_tokens_metric = create_mock_metric()
    prometheus_logger.derouter_remaining_team_budget_metric = create_mock_metric()
    prometheus_logger.derouter_remaining_api_key_budget_metric = create_mock_metric()
    prometheus_logger.derouter_remaining_user_budget_metric = create_mock_metric()
    prometheus_logger.derouter_user_max_budget_metric = create_mock_metric()
    prometheus_logger.derouter_user_budget_remaining_hours_metric = create_mock_metric()
    prometheus_logger.derouter_remaining_api_key_requests_for_model = (
        create_mock_metric()
    )
    prometheus_logger.derouter_remaining_api_key_tokens_for_model = create_mock_metric()
    prometheus_logger.derouter_llm_api_time_to_first_token_metric = create_mock_metric()
    prometheus_logger.derouter_llm_api_latency_metric = create_mock_metric()
    prometheus_logger.derouter_request_total_latency_metric = create_mock_metric()
    # Cache metrics
    prometheus_logger.derouter_cache_hits_metric = create_mock_metric()
    prometheus_logger.derouter_cache_misses_metric = create_mock_metric()
    prometheus_logger.derouter_cached_tokens_metric = create_mock_metric()
    # Deployment metrics
    prometheus_logger.derouter_deployment_state = create_mock_metric()
    prometheus_logger.derouter_deployment_success_responses = create_mock_metric()
    prometheus_logger.derouter_deployment_total_requests = create_mock_metric()
    prometheus_logger.derouter_deployment_latency_per_output_token = create_mock_metric()
    prometheus_logger.derouter_remaining_requests_metric = create_mock_metric()
    prometheus_logger.derouter_remaining_tokens_metric = create_mock_metric()
    prometheus_logger.derouter_overhead_latency_metric = create_mock_metric()
    prometheus_logger.derouter_proxy_total_requests_metric = create_mock_metric()
    ...
    assert prometheus_logger.derouter_requests_metric.labels.called
    assert prometheus_logger.derouter_spend_metric.labels.called
    ...
    call_args = prometheus_logger.derouter_requests_metric.labels.call_args
...
def test_get_custom_labels_from_tags(monkeypatch):
    from derouter.integrations.prometheus import get_custom_labels_from_tags

    monkeypatch.setattr(
        "derouter.custom_prometheus_tags", ["prod", "test-env", "batch.job"]
    )
...
def test_get_custom_labels_from_tags_empty_config(monkeypatch):
    ...
    monkeypatch.setattr("derouter.custom_prometheus_tags", [])
    tags = ["prod", "debug"]
    result = get_custom_labels_from_tags(tags)
    assert result == {}
...
def test_get_custom_labels_from_tags_no_tags(monkeypatch):
    ...
    monkeypatch.setattr("derouter.custom_prometheus_tags", ["prod", "test"])
    tags = []
    result = get_custom_labels_from_tags(tags)
    assert result == {
        "tag_prod": "false",
...
def test_get_custom_labels_from_tags_wildcard_patterns(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.custom_prometheus_tags",
        [
            "User-Agent: curl/*",
...
def test_tag_matches_wildcard_configured_pattern():
    ...
    from derouter.integrations.prometheus import _tag_matches_wildcard_configured_pattern
...
async def test_initialize_remaining_budget_metrics(prometheus_logger):
    ...
    """
    derouter.prometheus_initialize_budget_metrics = True
    # Mock the prisma client and get_paginated_teams function
    with (
        patch("derouter.proxy.proxy_server.prisma_client") as mock_prisma,
        patch(
            "derouter.proxy.management_endpoints.team_endpoints.get_paginated_teams"
        ) as mock_get_teams,
    ):
        ...
        prometheus_logger.derouter_remaining_team_budget_metric.assert_has_calls(
            expected_budget_calls, any_order=True
        )

        # Get all the calls made to the hours metric
        hours_calls = (
            prometheus_logger.derouter_team_budget_remaining_hours_metric.mock_calls
        )
        ...
        prometheus_logger.derouter_team_budget_remaining_hours_metric.assert_has_calls(
...
async def test_initialize_remaining_budget_metrics_exception_handling(
    ...
    with (
        ...
        # The code accesses prisma_client.db.derouter_usertable.find_many and count
        ...
        # Mock derouter_teamtable to raise an exception for team count metrics
        mock_teamtable = MagicMock()
        mock_teamtable.count = MagicMock(side_effect=Exception("Team count error"))

        # Mock derouter_organizationtable to raise an exception for org budget metrics
        ...
        mock_db = MagicMock()
        mock_db.derouter_usertable = mock_usertable
        mock_db.derouter_teamtable = mock_teamtable
        mock_db.derouter_organizationtable = mock_orgtable
        mock_prisma.db = mock_db

        # Mock the Prometheus metrics
        prometheus_logger.derouter_remaining_team_budget_metric = MagicMock()
        prometheus_logger.derouter_remaining_api_key_budget_metric = MagicMock()
        prometheus_logger.derouter_remaining_user_budget_metric = MagicMock()
        prometheus_logger.derouter_remaining_org_budget_metric = MagicMock()
        prometheus_logger.derouter_total_users_metric = MagicMock()
        prometheus_logger.derouter_teams_count_metric = MagicMock()

        # Mock the logger to capture the error
        with patch("derouter._logging.verbose_logger.exception") as mock_logger:
            # Call the function
            await prometheus_logger._initialize_remaining_budget_metrics()
        ...
        prometheus_logger.derouter_remaining_team_budget_metric.assert_not_called()
        prometheus_logger.derouter_remaining_api_key_budget_metric.assert_not_called()
        prometheus_logger.derouter_remaining_user_budget_metric.assert_not_called()
        prometheus_logger.derouter_remaining_org_budget_metric.assert_not_called()
        prometheus_logger.derouter_total_users_metric.assert_not_called()
        prometheus_logger.derouter_teams_count_metric.assert_not_called()
...
async def test_initialize_api_key_budget_metrics(prometheus_logger):
    ...
    with (
        ...
        prometheus_logger.derouter_remaining_api_key_budget_metric.assert_has_calls(
            expected_budget_calls, any_order=True
        )

        # Get all the calls made to the hours metric
        hours_calls = (
            prometheus_logger.derouter_api_key_budget_remaining_hours_metric.mock_calls
        )
        ...
        prometheus_logger.derouter_api_key_max_budget_metric.assert_has_calls(
...
def test_set_team_budget_metrics_multiple_teams(prometheus_logger):
    ...
    prometheus_logger.derouter_team_max_budget_metric.assert_has_calls(
        expected_max_budget_calls, any_order=True
    )
    ...
    assert (
        prometheus_logger.derouter_team_budget_remaining_hours_metric.labels.call_count
        == 3
    )
    assert (
        prometheus_logger.derouter_team_budget_remaining_hours_metric.labels().set.call_count
        == 3
    )
...
def test_set_team_budget_metrics_null_values(prometheus_logger):
    ...
    prometheus_logger.derouter_team_max_budget_metric.assert_not_called()

    # Verify reset metric is not set when budget_reset_at is None
    prometheus_logger.derouter_team_budget_remaining_hours_metric.assert_not_called()
...
def test_set_team_budget_metrics_with_custom_labels(prometheus_logger, monkeypatch):
    ...
    monkeypatch.setattr("derouter.custom_prometheus_metadata_labels", custom_labels)
...
def test_prometheus_label_factory_with_custom_tags(monkeypatch):
    ...
    monkeypatch.setattr("derouter.custom_prometheus_tags", ["prod", "test-env"])
...
def test_get_exception_class_name(prometheus_logger):
    ...
    auth_error = derouter.AuthenticationError(
    ...
    context_window_error = derouter.ContextWindowExceededError(
...
def test_set_llm_deployment_success_metrics_with_label_filtering():
    ...
    # Mock derouter.prometheus_metrics_config
    with patch("derouter.prometheus_metrics_config", config):
        ...
        prometheus_logger.derouter_overhead_latency_metric.labels.assert_called_once()
        overhead_labels = (
            prometheus_logger.derouter_overhead_latency_metric.labels.call_args[1]
        )
        ...
        prometheus_logger.derouter_remaining_requests_metric.labels.assert_called_once()
        requests_labels = (
            prometheus_logger.derouter_remaining_requests_metric.labels.call_args[1]
        )
        ...
        prometheus_logger.derouter_remaining_tokens_metric.labels.assert_called_once()
        tokens_labels = (
            prometheus_logger.derouter_remaining_tokens_metric.labels.call_args[1]
        )
        ...
        prometheus_logger.derouter_deployment_success_responses.labels.assert_called_once()
        success_labels = (
            prometheus_logger.derouter_deployment_success_responses.labels.call_args[1]
        )
        ...
        prometheus_logger.derouter_deployment_total_requests.labels.assert_called_once()
        total_labels = (
            prometheus_logger.derouter_deployment_total_requests.labels.call_args[1]
        )
        ...
        prometheus_logger.derouter_overhead_latency_metric.labels().observe.assert_called_once()
        prometheus_logger.derouter_remaining_requests_metric.labels().set.assert_called_once_with(
            123
        )
        prometheus_logger.derouter_remaining_tokens_metric.labels().set.assert_called_once_with(
...
async def test_prometheus_token_metrics_with_prometheus_config():
    ...
    - derouter_total_tokens -> derouter_total_tokens_metric
    - derouter_input_tokens -> derouter_input_tokens_metric
    - derouter_output_tokens -> derouter_output_tokens_metric
    ...
    with patch("derouter.prometheus_metrics_config", config):
        ...
        # Expected values
        expected_values = {
            "derouter_total_tokens_metric": 1500.0,
            "derouter_input_tokens_metric": 900.0,
            "derouter_output_tokens_metric": 600.0,
            "derouter_requests_metric": 1.0,
        }
