...
@pytest.mark.skip(
    reason="Flaky in CI: /team/info?team_id=... intermittently returns 404/400 mid-loop after add_team_member calls. Single-member coverage in test_add_single_member is sufficient; team-member CRUD is also covered by tests/test_derouter/proxy/management_endpoints/."
)
...
@pytest.mark.skip(
    reason="Flaky in CI: /team/info?team_id=... intermittently returns 404 after add_team_member calls, same race documented for test_add_multiple_members. Duplicate-prevention is covered by test_update_team_members_list_duplicate_prevention in tests/test_derouter/proxy/management_endpoints/test_team_endpoints.py."
)
