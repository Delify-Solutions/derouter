"""
E2E tests for Claude Agent SDK with DeRouter Proxy using Bedrock models.
...
def derouter_proxy_config():
    """Configure connection to DeRouter proxy"""
    proxy_url = os.getenv("DEROUTER_PROXY_URL", "http://localhost:4000")
    api_key = os.getenv("DEROUTER_API_KEY", "sk-1234")
...
async def test_claude_agent_sdk_streaming(
    derouter_proxy_config, model_name, model_description
):
    """
    Test streaming messages with Claude Agent SDK through DeRouter proxy.

    This validates:
    1. Claude Agent SDK can connect to DeRouter proxy
