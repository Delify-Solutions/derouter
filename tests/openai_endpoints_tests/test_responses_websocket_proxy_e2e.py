"""
E2E tests for OpenAI Responses API WebSocket mode through the DeRouter proxy.

Connects to ws://0.0.0.0:4000/v1/responses, sends response.create events,
and validates the streamed response events.

Requires:
  - Proxy running: python -m derouter.proxy.proxy_cli --config <config> --port 4000
...
# ── Configuration ─────────────────────────────────────────────────────────────
PROXY_BASE_URL = os.environ.get("DEROUTER_PROXY_BASE_URL", "ws://0.0.0.0:4000")
PROXY_MASTER_KEY = os.environ.get("DEROUTER_PROXY_KEY", "sk-1234")
PROXY_MODEL = os.environ.get("DEROUTER_PROXY_RESPONSES_MODEL", "gpt-5-mini")
# ──────────────────────────────────────────────────────────────────────────────
...
async def test_responses_websocket_proxy_basic():
    ...
    try:
        ...
        pytest.skip(
            ...
            "Start proxy: python -m derouter.proxy.proxy_cli --config <config> --port 4000"
        )
