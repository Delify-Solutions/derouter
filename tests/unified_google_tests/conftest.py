...
def _start_proxy_server(
    ...
    while not server.started:
        if not thread.is_alive():
            raise RuntimeError("DeRouter proxy failed to start")
        if time.time() - start_time > PROXY_START_TIMEOUT_S:
            raise TimeoutError("DeRouter proxy did not start in time")
        time.sleep(0.05)
...
def google_genai_proxy_url() -> Iterator[str]:
    ...
    os.environ["DEROUTER_MASTER_KEY"] = PROXY_MASTER_KEY
