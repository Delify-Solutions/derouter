...
def test_bedrock_embedding_models(model, input_type, embed_response):
    ...
    with patch.object(client, "post") as mock_post:
        ...
        input_data = (
            img_base_64 if input_type == "image" else "Hello world from derouter"
        )
...
def test_e2e_bedrock_embedding():
    ...
    response = derouter.embedding(
        ...
        input=["Hello world from DeRouter with TwelveLabs Marengo!"],
    )

    # Validate response structure
    assert isinstance(
        response, derouter.EmbeddingResponse
...
def test_e2e_bedrock_async_invoke_embedding_twelvelabs_marengo():
    ...
    with patch(
        "derouter.llms.bedrock.embed.embedding.BedrockEmbedding._make_sync_call"
    ) as mock_call:
        ...
        response = derouter.embedding(
            ...
            input=["Hello world from DeRouter async invoke!"],
            aws_region_name="us-east-1",
            inputType="text",
...
async def test_e2e_bedrock_async_invoke_embedding_async_twelvelabs_marengo():
    ...
    with patch(
        "derouter.llms.bedrock.embed.embedding.BedrockEmbedding._make_async_call"
    ) as mock_call:
        mock_call.return_value = {
        ...
        response = await derouter.aembedding(
            model="bedrock/async_invoke/us.twelvelabs.marengo-embed-2-7-v1:0",
            input=["Hello world from DeRouter async invoke async!"],
            aws_region_name="us-east-1",
            inputType="text",
            output_s3_uri="s3://test-bucket/async-invoke-output/",
        )
...
def test_bedrock_embedding_uses_correct_region_when_specified():
    ...
    relevant issue: https://github.com/Delify-Solutions/derouter/issues/16517
    """
