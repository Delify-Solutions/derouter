...
async def test_deepseek_provider_async_completion(stream):
    ...
    with patch(
        ...
        mock_response_data = derouter.ModelResponse(
            choices=[
                derouter.Choices(
                    message=derouter.Message(content="Hello!"),
                    index=0,
                    finish_reason="stop",
                )
            ]
...
def test_completion_cost_deepseek():
    ...
    try:
        response_1 = derouter.completion(model=model_name, messages=messages_1)
        response_2 = derouter.completion(model=model_name, messages=message_2)
...
def test_deepseek_fill_reasoning_content_multiturn():
    ...
    """
    from derouter.llms.deepseek.chat.transformation import DeepSeekChatConfig
