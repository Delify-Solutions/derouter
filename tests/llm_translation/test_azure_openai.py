...
import httpx
import pytest
from derouter.llms.azure.common_utils import process_azure_headers
...
def test_azure_extra_headers(input, call_type, header_value):
    from derouter import embedding, image_generation
...
class TestAzureEmbedding(BaseLLMEmbeddingTest):
    ...
    def get_custom_llm_provider(self) -> derouter.LlmProviders:
        return derouter.LlmProviders.AZURE
...
@patch(
    "derouter.main.azure_chat_completions.make_sync_azure_openai_chat_completion_request"
)
...
@pytest.mark.parametrize("max_retries", [0, 4])
@pytest.mark.parametrize("stream", [True, False])
@patch("derouter.main.azure_chat_completions.make_azure_openai_chat_completion_request")
@pytest.mark.asyncio
...
@patch("derouter.llms.azure.common_utils.select_azure_base_url_or_endpoint")
@pytest.mark.asyncio
async def test_azure_instruct(
    mock_select_azure_base_url_or_endpoint, max_retries, stream, sync_mode
):
    import derouter
    from derouter import completion, acompletion
...
async def test_azure_embedding_max_retries_0(
    ...
    import derouter
    from derouter import aembedding, embedding
...
def test_azure_openai_responses_bridge():
    ...
    with patch.object(derouter, "responses") as mock_responses:
        try:
            response = completion(
...
def test_azure_with_content_safety_error():
    ...
    """
    from derouter import completion
    from derouter.exceptions import ContentPolicyViolationError
