...
from derouter.derouter_core_utils.prompt_templates.factory import _bedrock_tools_pt
...
def bedrock_session_token_creds():
    ...
    if aws_session_token is not None:
        ...
        # TODO: This is using ai.moda's IAM role, we should use DeRouter's IAM role eventually
        aws_role_name = (
            "arn:aws:iam::335785316107:role/derouter-github-unit-tests-circleci"
        )
...
def process_stream_response(res, messages):
    ...
    if isinstance(res, derouter.utils.CustomStreamWrapper):
        chunks = []
        for part in res:
            chunks.append(part)
            text = part.choices[0].delta.content or ""
            print(text, end="")
        res = derouter.stream_chunk_builder(chunks, messages=messages)
    else:
...
def test_bedrock_ptu():
    ...
    Reference: https://github.com/Delify-Solutions/derouter/issues/3805
    """
    client = HTTPHandler()
...
async def test_bedrock_custom_api_base():
    ...
    Reference: https://github.com/Delify-Solutions/derouter/issues/3805, https://github.com/Delify-Solutions/derouter/issues/5389#issuecomment-2313677977
...
async def test_bedrock_extra_headers(model):
    """
    Relevant Issue: https://github.com/Delify-Solutions/derouter/issues/9106
    """
    client = AsyncHTTPHandler()
...
async def test_bedrock_custom_prompt_template():
    ...
    Reference: https://github.com/Delify-Solutions/derouter/issues/4415
    """
    client = AsyncHTTPHandler()
...
def test_bedrock_converse_tools_pt_converts_custom_schema_type_to_object():
    ...
    assert result[2]["toolSpec"]["name"] == "derouter_unnamed_tool_2"


def test_bedrock_tools_transformation_valid_params():
    from derouter.types.llms.bedrock import ToolJsonSchemaBlock
...
def test_not_found_error():
    with pytest.raises(derouter.NotFoundError):
        completion(
...
def test_bedrock_converse_translation_tool_message():
    from derouter.types.utils import ChatCompletionMessageToolCall, Function
...
def test_bedrock_completion_test_4(modify_params):
    derouter.set_verbose = True
    derouter.modify_params = modify_params
    ...
    if modify_params:
        ...
        with pytest.raises(Exception, match=r"derouter\.modify_params") as e:
            derouter.completion(**data)
        assert "derouter.modify_params" in str(e.value)


def test_bedrock_context_window_error():
    with pytest.raises(derouter.ContextWindowExceededError) as e:
        derouter.completion(
...
def test_bedrock_prompt_caching_message(messages, expected_cache_control):
    ...
    transformed_messages = derouter.AmazonConverseConfig()._transform_request(
...
def test_bedrock_supports_tool_call(model, expected_supports_tool_call):
    supported_openai_params = (
        derouter.AmazonConverseConfig().get_supported_openai_params(model=model)
    )
...
class TestBedrockConverseChatCrossRegion(BaseLLMChatTest):
    ...
    def test_completion_cost(self):
        ...
        derouter.model_cost.pop(bedrock_model, None)
...
class TestBedrockRerank(BaseLLMRerankTest):
    def get_custom_llm_provider(self) -> derouter.LlmProviders:
        return derouter.LlmProviders.BEDROCK
...
def test_bedrock_nova_topk(top_k_param):
    ...
    original_transform = derouter.AmazonConverseConfig()._transform_request
    captured_data = None
    ...
    with patch(
        "derouter.AmazonConverseConfig._transform_request", side_effect=mock_transform
    ):
        derouter.completion(**data)
...
def test_nova_optional_params_tool_choice():
    ...
    except derouter.APIConnectionError:
        pass
...
def test_bedrock_error_handling_streaming(exception_type, expected_status_code):
    ...
    from derouter.llms.bedrock.chat.invoke_handler import (
...
def test_handle_top_k_value_helper(model, expected_output):
    assert (
        derouter.AmazonConverseConfig()._handle_top_k_value(model, {"topK": 3})
        == expected_output
    )
    assert (
        derouter.AmazonConverseConfig()._handle_top_k_value(model, {"top_k": 3})
        == expected_output
    )
...
def test_bedrock_invoke_provider():
    assert (
        derouter.AmazonInvokeConfig().get_bedrock_invoke_provider(
    ...
    assert (
        derouter.AmazonInvokeConfig().get_bedrock_invoke_provider("amazon.nova-pro-v1:0")
        == "nova"
    )
    assert (
        derouter.AmazonInvokeConfig().get_bedrock_invoke_provider(
...
async def test_bedrock_thinking_in_assistant_message(sync_mode):
    derouter._turn_on_debug()
    from derouter.llms.custom_httpx.http_handler import HTTPHandler, AsyncHTTPHandler
    ...
    with patch.object(client, "post") as mock_post:
        try:
            if sync_mode:
                response = derouter.completion(**params, client=client)
            else:
                response = await derouter.acompletion(**params, client=client)
        except Exception as e:
            print(e)
...
async def test_bedrock_passthrough(sync_mode: bool):
    ...
    if sync_mode:
        response = derouter.llm_passthrough_route(
            model="bedrock/us.anthropic.claude-haiku-4-5-20251001-v1:0",
            method="POST",
        ...
        response = await derouter.allm_passthrough_route(
            model="bedrock/us.anthropic.claude-haiku-4-5-20251001-v1:0",
            method="POST",
            endpoint="/model/us.anthropic.claude-haiku-4-5-20251001-v1:0/invoke",
            data=data,
...
async def test_bedrock_passthrough_router():
    """
    Test bedrock passthrough using derouter.Router with async mode.
...
async def test_bedrock_converse__streaming_passthrough(monkeypatch):
    ...
    if os.environ.get("DEROUTER_RUN_LIVE_BEDROCK_PASSTHROUGH_TESTS") != "1":
    ...
    monkeypatch.setattr(derouter, "callbacks", [mock_custom_logger])
