...
from derouter.types.utils import GenericStreamingChunk, ChatCompletionToolCallChunk
...
from derouter.llms.anthropic.chat.handler import AnthropicChatCompletion
...
def test_anthropic_beta_header(
    ...
    headers = derouter.AnthropicConfig().get_anthropic_headers(
...
def test_anthropic_tool_with_image():
    from derouter.derouter_core_utils.prompt_templates.factory import prompt_factory
    import json
...
def test_anthropic_map_openai_params_tools_and_json_schema():
    ...
    mapped_params = derouter.AnthropicConfig().map_openai_params(
...
from derouter.constants import RESPONSE_FORMAT_TOOL_NAME
...
def test_anthropic_json_mode_and_tool_call_response(
    ...
    result, _, _ = derouter.AnthropicConfig()._resolve_json_mode_non_streaming(
        json_mode=json_mode,
...
def test_map_stop_sequences(stop_input, expected_output, drop_params):
    ...
    derouter.drop_params = drop_params
    config = AnthropicConfig()
...
async def test_anthropic_structured_output():
    ...
    Relevant Issue: https://github.com/Delify-Solutions/derouter/issues/8291
    """
...
def test_anthropic_thinking_in_assistant_message(model):
    ...
    response = derouter.completion(**params)
...
def _make_transform_request(optional_params: dict, derouter_params: dict) -> dict:
    from derouter.llms.anthropic.chat.transformation import AnthropicConfig
...
def test_metadata_user_id_from_derouter_params_strips_extras():
    """user_id from derouter_params metadata is extracted; extra keys are not forwarded."""
    data = _make_transform_request(
        optional_params={},
        derouter_params={"metadata": {"user_id": "abc123", "trace_id": "xyz"}},
    )
    assert data.get("metadata") == {"user_id": "abc123"}
...
def test_metadata_filter_applies_to_azure_anthropic():
    ...
    from derouter.llms.azure_ai.anthropic.transformation import AzureAnthropicConfig
