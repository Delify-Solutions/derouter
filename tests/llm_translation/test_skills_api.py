...
class BaseSkillsAPITest(ABC):
    ...
    def test_create_skill(self):
        ...
        skill_name = "test-skill-derouter"
        ...
        with create_skill_zip(skill_name, unique_suffix=ts) as zip_file:
            response = derouter.create_skill(
    ...
    def test_get_skill(self):
        ...
        list_response = derouter.list_skills(
            limit=1,
            custom_llm_provider=custom_llm_provider,
            api_key=api_key,
            api_base=api_base,
    ...
    def test_delete_skill(self):
        ...
        with create_skill_zip(skill_name, unique_suffix=ts) as zip_file:
            created_skill = derouter.create_skill(
                display_title=unique_title,
                files=[zip_file],
                custom_llm_provider=custom_llm_provider,
                api_key=api_key,
        ...
        response = derouter.delete_skill(
            skill_id=skill_id,
            custom_llm_provider=custom_llm_provider,
            api_key=api_key,
            api_base=api_base,
...
#   tests/test_derouter/test_anthropic_skills_transformation.py
