...
os.environ["DEROUTER_LOCAL_MODEL_COST_MAP"] = "True"  # Load from local file
...
import importlib
import derouter.derouter_core_utils.get_model_cost_map
import derouter

# Reload modules to pick up environment variable
importlib.reload(derouter.derouter_core_utils.get_model_cost_map)
...
class TestBedrockGovCloudSupport:
    ...
    @patch("derouter.llms.bedrock.common_utils.init_bedrock_client")
    def test_govcloud_client_initialization(self, mock_init_client):
        ...
        from derouter.llms.bedrock.common_utils import init_bedrock_client
    ...
    def test_govcloud_model_in_bedrock_models_list(self):
        ...
        assert not any("us-gov-east-1" in model for model in derouter.bedrock_models)
        assert not any("us-gov-west-1" in model for model in derouter.bedrock_models)
    ...
    def test_govcloud_cost_per_token_with_region(self):
        ...
        from derouter import cost_per_token
        from derouter.utils import Usage
