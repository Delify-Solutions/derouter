...
import derouter
from derouter.types.llms.bedrock import BedrockInvokeNovaRequest
...
class TestBedrockInvokeNovaJson(BaseLLMChatTest):
    ...
    def test_json_response_pydantic_obj(self):
        if os.environ.get("DEROUTER_RUN_LIVE_BEDROCK_NOVA_JSON_TESTS") != "1":
...
def test_nova_invoke_remove_empty_system_messages():
    ...
    derouter.AmazonInvokeNovaConfig()._remove_empty_system_messages(input_request)
...
def test_nova_invoke_filter_allowed_fields():
    ...
    result = derouter.AmazonInvokeNovaConfig()._filter_allowed_fields(input_request)
