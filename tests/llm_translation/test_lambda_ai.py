...
import derouter
from derouter.llms.lambda_ai.chat.transformation import LambdaAIChatConfig
...
def test_lambda_ai_in_provider_lists():
    ...
    assert "lambda_ai" in derouter.openai_compatible_providers
    assert "lambda_ai" in derouter.provider_list
    assert "https://api.lambda.ai/v1" in derouter.openai_compatible_endpoints
...
def test_lambda_ai_model_list_populated():
    ...
    for model in expected_models:
        assert (
            model in derouter.lambda_ai_models
