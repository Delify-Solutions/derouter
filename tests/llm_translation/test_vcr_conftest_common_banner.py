...
def vcr_enabled(monkeypatch):
    ...
    monkeypatch.delenv("DEROUTER_VCR_DISABLE", raising=False)
...
def test_diagnostic_log_dedupes_repeated_blocks(tmp_path, monkeypatch):
    monkeypatch.setenv("DEROUTER_VCR_DIAG_DIR", str(tmp_path))
...
def test_diagnostic_log_silent_when_no_dir(tmp_path, monkeypatch):
    monkeypatch.setenv("DEROUTER_VCR_DIAG_DIR", str(tmp_path / "does-not-exist"))
    reporter = _FakeTerminalReporter()
...
# ---------------------------------------------------------------------------
# Telemetry-leak suppression. Several modules set ``derouter.success_callback``
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # telemetry-named tests: derouter's background flush makes them rotate
...
def test_load_guard_patch_is_idempotent():
    ...
    assert getattr(cassette_mod.Cassette._load, "_derouter_load_guarded", False)
