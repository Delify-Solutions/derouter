...
def test_gemini_flash_image_preview_models(model_name: str):
    ...
    from derouter.types.utils import ImageResponse, ImageObject
    ...
    with patch(
        "derouter.llms.custom_httpx.llm_http_handler.HTTPHandler.post"
    ) as mock_post:
...
def test_gemini_image_generation_image_config_takes_precedence_over_size():
    from derouter.llms.gemini.image_generation.transformation import GoogleImageGenConfig
...
def test_gemini_image_generation_openai_size_maps_to_google_table(
    ...
    from derouter.llms.gemini.common_utils import (
...
def test_gemini_thinking():
    derouter._turn_on_debug()
    from derouter.types.utils import Message, CallTypes
...
def test_gemini_with_grounding():
    from derouter import completion, Usage, stream_chunk_builder
...
async def test_claude_tool_use_with_gemini():
    """
    Tests that tool use via derouter.anthropic.messages.acreate with a non-Anthropic model
...
async def test_gemini_image_generation_async():
    ...
    response = await derouter.acompletion(
        messages=[
            {
                "role": "user",
                "content": "Generate an image of a banana wearing a costume that says DeRouter",
            }
        ],
        model="gemini/gemini-2.5-flash-image",
    )
...
def test_gemini_with_thinking():
    ...
    if tool_calls:
        ...
        second_response = derouter.completion(
            model=model,
            messages=messages,
            seed=22,
            reasoning_effort="low",
...
def test_gemini_exception_message_format():
    ...
    from derouter import BadRequestError
...
def l(status_code, expected_exception):
    ...
    to the correct DeRouter exception types with GeminiException prefix.
    """
    import httpx
    from unittest.mock import Mock
    from derouter.derouter_core_utils.exception_mapping_utils import exception_type
...
def test_reasoning_effort_none_mapping():
    ...
    Related issue: https://github.com/Delify-Solutions/derouter/issues/16420
    """
...
def test_gemini_function_args_preserve_unicode():
    ...
    https://github.com/Delify-Solutions/derouter/issues/16533
...
def test_anthropic_thinking_param_to_gemini_3_provider_defaults():
    ...
    original_force_low_flag = derouter.enable_gemini_default_thinking_level_low
    derouter.enable_gemini_default_thinking_level_low = False
    ...
    try:
        ...
        derouter.enable_gemini_default_thinking_level_low = original_force_low_flag
...
def test_anthropic_thinking_param_to_gemini_3_force_low_feature_flag():
    ...
    original_force_low_flag = derouter.enable_gemini_default_thinking_level_low
    derouter.enable_gemini_default_thinking_level_low = True
...
def test_gemini_image_size_limit_exceeded(monkeypatch):
    ...
    tests/test_derouter/derouter_core_utils/test_image_handling.py) so the test
    deterministically exercises the size-limit rejection path without any
    external network dependency.
    """
    from httpx import Request, Response

    from derouter.derouter_core_utils.prompt_templates import image_handling
    ...
    monkeypatch.setattr(derouter, "module_level_client", LargeImageClient())
    ...
    with pytest.raises(derouter.ImageFetchError) as excinfo:
