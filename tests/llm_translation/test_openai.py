...
import derouter
from derouter import Choices, Message, ModelResponse
...
def test_openai_prediction_param():
    ...
    completion = derouter.completion(
        model="gpt-4o-mini",
        messages=[
            {
                "role": "user",
...
async def test_openai_prediction_param_with_caching():
    ...
    """
    from derouter.caching.caching import DeRouterCacheType
    import logging
    ...
    derouter.set_verbose = True
    derouter.cache = derouter.Cache(type=DeRouterCacheType.LOCAL)
    code = """
    /// <summary>
    /// Represents a user with a first name, last name, and username.
    /// </summary>
    ...
    completion_response_1 = derouter.completion(
        model="gpt-4o-mini",
        messages=[
            {
                "role": "user",
    ...
    # cache hit
    completion_response_2 = derouter.completion(
        model="gpt-4o-mini",
        messages=[
            {
                "role": "user",
    ...
    completion_response_3 = derouter.completion(
        model="gpt-4o-mini",
        messages=[
...
@patch("derouter.main.openai_chat_completions._get_openai_client")
...
def test_openai_image_generation_forwards_organization(mock_get_openai_client):
    ...
    org = "org_test_123"
    resp = derouter.image_generation(
        model="gpt-image-1",
...
def validate_web_search_annotations(annotations: ChatCompletionAnnotation):
    """validates derouter response contains web search annotations"""
...
def test_openai_web_search():
    ...
    print("derouter response: ", response.model_dump_json(indent=4))
...
def test_openai_web_search_streaming():
    ...
    for chunk in response:
        print("derouter response chunk: ", chunk)
        if (
...
class TestOpenAIGPT4OAudioTranscription(BaseLLMAudioTranscriptionTest):
    ...
    def get_custom_llm_provider(self) -> derouter.LlmProviders:
        return derouter.LlmProviders.OPENAI


@pytest.mark.asyncio
@pytest.mark.parametrize("model", ["gpt-4o"])
async def test_openai_pdf_url(model):
    from derouter.utils import return_raw_request, CallTypes
...
async def test_openai_codex_stream(sync_mode):
    ...
    chunks = []
    if sync_mode:
        response = derouter.completion(**kwargs)
        for chunk in response:
            chunks.append(chunk)
    else:
        response = await derouter.acompletion(**kwargs)
...
def test_openai_tool_calling():
    ...
    response = derouter.completion(**completion_params)
...
def test_openai_gpt_5_codex_reasoning():
    ...
    response = derouter.completion(**completion_kwargs)
    print("response: ", response)
    for chunk in response:
        print("chunk: ", chunk)
...
# Regression test for: https://github.com/Delify-Solutions/derouter/issues/8977
@pytest.mark.parametrize("model", ["gpt-4o"])
@pytest.mark.asyncio
...
def test_responses_gpt54_with_xhigh_reasoning():
    ...
    """
    with patch("derouter.responses") as mock_responses:
