...
def test_document_inlining_example(disable_add_transform_inline_image_block):
    """
    Fireworks document inlining has been removed from the platform. DeRouter
...
def test_transform_inline_no_longer_added(content, expected_url):
    ...
    result = derouter.FireworksAIConfig()._transform_messages_helper(
        messages=messages,
...
def test_global_disable_flag_no_longer_adds_transform_inline(is_disabled):
    url = "http://example.com/image.png"
    derouter.disable_add_transform_inline_image_block = is_disabled
    messages = [
        {
            "role": "user",
    ...
    derouter.disable_add_transform_inline_image_block = False  # Reset for other tests
...
def test_global_disable_flag_with_transform_messages_helper(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "disable_add_transform_inline_image_block", True)
