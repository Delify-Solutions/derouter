...
- Verifies derouter.completion() still works even with a broken cost map
- Verifies derouter.get_model_info() raises the expected error for unmapped models
...
class TestGetModelCostMapFallback:
    ...
    def test_should_fallback_when_fetched_map_shrinks_dramatically(self):
        ...
        tiny_map = {f"model-{i}": {"derouter_provider": "test"} for i in range(11)}
        mock_response = MagicMock()
        mock_response.raise_for_status = MagicMock()
    ...
    def test_should_use_local_map_when_env_var_set(self):
        """DEROUTER_LOCAL_MODEL_COST_MAP=True should skip remote fetch entirely."""
        with patch.dict(os.environ, {"DEROUTER_LOCAL_MODEL_COST_MAP": "True"}):
...
class TestBadHostedModelCostMap:
    ...
    - derouter.completion() still works
    """
    ...
    def test_should_model_info_pass_after_bad_hosted_map(self):
        ...
        original = derouter.model_cost
        derouter.model_cost = fallback_map
        try:
            ...
            derouter.model_cost = original

    def test_should_completion_pass_after_bad_hosted_map(self):
        """
        If the hosted map is bad, derouter.completion() should still work.

        Uses derouter's built-in mock_response param so the real completion
