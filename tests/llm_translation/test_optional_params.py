...
import derouter
from derouter.derouter_core_utils.prompt_templates.factory import map_system_message_pt
...
### Scenarios: w/ optional params + derouter.drop_params = True


def test_supports_system_message():
    """
    Check if derouter.completion(...,supports_system_message=False)
    """
    messages = [
...
@patch("derouter.main.openai_chat_completions.completion")
def test_requester_metadata_forwarded_to_openai(mock_completion):
    ...
    original_api_key = derouter.api_key
    derouter.api_key = "sk-test"
    original_preview_flag = derouter.enable_preview_features
    derouter.enable_preview_features = True
    ...
    try:
        ...
        derouter.enable_preview_features = original_preview_flag
...
def test_allowed_openai_params_does_not_forward_unset_params():
    """
    Regression test for https://github.com/Delify-Solutions/derouter/issues/25697

    When a user lists a param in ``allowed_openai_params`` but does not
    actually send that param in the request, derouter must not forward it
...
def test_vertex_ai_llama_3_optional_params():
    derouter.vertex_llama3_models = ["meta/llama3-405b-instruct-maas"]
    derouter.drop_params = True
    optional_params = get_optional_params(
...
def test_vertex_ai_mistral_optional_params():
    derouter.vertex_mistral_models = ["mistral-large@2407"]
    derouter.drop_params = True
    optional_params = get_optional_params(
...
def test_dynamic_drop_params_e2e():
    with patch(
        "derouter.llms.custom_httpx.http_handler.HTTPHandler.post", new=MagicMock()
    ) as mock_response:
        try:
            response = derouter.completion(
                model="command-r",
...
def test_drop_params_parallel_tool_calls(model, provider, should_drop):
    """
    https://github.com/Delify-Solutions/derouter/issues/4584
    """
    response = derouter.utils.get_optional_params(
        model=model,
...
def test_get_optional_params_image_gen():
    response = derouter.utils.get_optional_params_image_gen(
...
def test_get_optional_params_num_retries():
    """
    Relevant issue - https://github.com/Delify-Solutions/derouter/issues/5124
    """
    with patch(
        "derouter.main.get_optional_params",
...
def test_vertex_safety_settings(provider):
    derouter.vertex_ai_safety_settings = [
        {
...
def test_drop_nested_params_add_prop_and_strict(provider, model):
    """
    Relevant issue - https://github.com/Delify-Solutions/derouter/issues/5288

    Relevant issue - https://github.com/Delify-Solutions/derouter/issues/6136
    """
    tools = [
        {
            "type": "function",
...
def test_hosted_vllm_tool_param():
    """
    Relevant issue - https://github.com/Delify-Solutions/derouter/issues/6228
    """
    optional_params = get_optional_params(
        model="my-vllm-model",
...
def test_forward_user_param():
    from derouter.utils import get_supported_openai_params, get_optional_params
...
def test_gemini_frequency_penalty_listed_in_vertex_ai_supported_params():
    from derouter.utils import get_supported_openai_params
...
def test_derouter_proxy_claude_3_5_sonnet():
    tools = [
        {
            "type": "function",
            "function": {
...
def test_is_vertex_anthropic_model():
    assert (
        derouter.VertexAIAnthropicConfig().is_supported_model(
            model="claude-3-5-sonnet", custom_llm_provider="derouter_proxy"
        )
        is False
    )
...
def test_azure_prediction_param():
    optional_params = get_optional_params(
        ...
        prediction={
            "type": "content",
            "content": "DeRouter is a very useful way to connect to a variety of LLMs.",
        },
    )
...
def test_derouter_proxy_thinking_param():
    optional_params = get_optional_params(
        model="gpt-4o",
        custom_llm_provider="derouter_proxy",
        thinking={"type": "enabled", "budget_tokens": 1024},
...
def test_azure_response_format_param():
    optional_params = derouter.get_optional_params(
...
def test_anthropic_unified_reasoning_content(model, provider):
    from derouter.constants import DEFAULT_REASONING_EFFORT_HIGH_THINKING_BUDGET
...
def test_optional_params_responses_api_allowed_openai_params():
    from derouter import responses
...
def test_validate_openai_optional_params_disable_stop_sequence_limit():
    ...
    When derouter.disable_stop_sequence_limit is True, stop sequences should not be truncated.
    """
    # Save original value
    original_value = derouter.disable_stop_sequence_limit

    try:
        # Test with disable_stop_sequence_limit = True - should NOT truncate
        derouter.disable_stop_sequence_limit = True
        stop_sequences = ["stop1", "stop2", "stop3", "stop4", "stop5", "stop6"]
        result = validate_openai_optional_params(stop=stop_sequences)
        ...
        derouter.disable_stop_sequence_limit = False
        stop_sequences = ["stop1", "stop2", "stop3", "stop4", "stop5", "stop6"]
        result = validate_openai_optional_params(stop=stop_sequences)
        assert result == ["stop1", "stop2", "stop3", "stop4"]
        assert len(result) == 4
        ...
        derouter.disable_stop_sequence_limit = original_value
...
def test_validate_openai_optional_params_integration():
    ...
    try:
        with patch("derouter.llms.openai.openai.OpenAI") as mock_client:
...
def test_drop_store_param_for_anthropic():
    ...
    Ref: https://github.com/Delify-Solutions/derouter/issues/19700
    """
    optional_params = get_optional_params(
...
def test_store_in_openai_chat_completion_params():
    ...
    Ref: https://github.com/Delify-Solutions/derouter/issues/19700
    """
    from derouter.constants import OPENAI_CHAT_COMPLETION_PARAMS

    assert "store" in OPENAI_CHAT_COMPLETION_PARAMS

    # Verify get_standard_openai_params recognizes store
    from derouter.utils import get_standard_openai_params
