...
def mock_provider_mapping():
    with patch(
        "derouter.llms.huggingface.chat.transformation._fetch_inference_provider_mapping"
    ) as mock:
...
def clear_lru_cache():
    from derouter.llms.huggingface.common_utils import _fetch_inference_provider_mapping
...
def mock_http_handler():
    ...
    with patch("derouter.llms.custom_httpx.http_handler.HTTPHandler.post") as mock:
...
class TestHuggingFace(BaseLLMChatTest):
    ...
    def test_completion_non_streaming(self):
        ...
        response = derouter.completion(model=self.model, messages=messages, stream=False)
    ...
    def test_completion_streaming(self):
        ...
        response = derouter.completion(model=self.model, messages=messages, stream=True)
    ...
    def test_get_complete_url(self, model, expected_url):
        ...
        from derouter.llms.huggingface.chat.transformation import HuggingFaceChatConfig
    ...
    def test_build_chat_completion_url_function(self):
        ...
        from derouter.llms.huggingface.chat.transformation import (
