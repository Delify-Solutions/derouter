...
import httpx
import derouter
from derouter import completion, embedding
import pytest
...
@pytest.mark.asyncio
async def test_derouter_gateway_from_sdk():
    ...
    with patch.object(
        ...
        try:
            completion(
                model="derouter_proxy/my-vllm-model",
                messages=messages,
...
@pytest.mark.asyncio
async def test_derouter_gateway_from_sdk_structured_output():
    ...
    with patch.object(
        ...
        try:
            derouter.completion(
                model="derouter_proxy/openai/gpt-4o",
                messages=[
                ...
                base_url="https://derouter.ml-serving-internal.scale.com",
                client=openai_client,
            )
        except Exception as e:
            print(e)
...
@pytest.mark.asyncio
async def test_derouter_gateway_from_sdk_embedding(is_async):
    derouter.set_verbose = True
...
@pytest.mark.parametrize("is_async", [False, True])
@pytest.mark.asyncio
async def test_derouter_gateway_from_sdk_image_generation(is_async):
    derouter._turn_on_debug()
    ...
    with patch.object(patch_target.__self__, patch_target.__name__, new=mock_method):
        try:
            if is_async:
                response = await derouter.aimage_generation(
                    model="derouter_proxy/dall-e-3",
...
@pytest.mark.parametrize("is_async", [False, True])
@pytest.mark.asyncio
async def test_derouter_gateway_image_generation_direct(is_async):
    """Test image generation using the derouter_proxy provider directly."""
    derouter._turn_on_debug()
    ...
    if is_async:
        ...
        with patch(
            "derouter.llms.openai.openai.AsyncOpenAI", return_value=mock_async_client
        ...
        with patch(
            "derouter.llms.openai.openai.OpenAI", return_value=mock_sync_client
...
@pytest.mark.parametrize("is_async", [False, True])
@pytest.mark.asyncio
async def test_derouter_gateway_from_sdk_image_edit(is_async):
    derouter._turn_on_debug()
    ...
    if is_async:
        ...
        patch_target = "derouter.llms.custom_httpx.http_handler.AsyncHTTPHandler.post"
    else:
        mock_post = MagicMock(return_value=MockResponse(mock_response, 200))
        patch_target = "derouter.llms.custom_httpx.http_handler.HTTPHandler.post"

    with patch(patch_target, new=mock_post):
        if is_async:
            await derouter.aimage_edit(
                model="derouter_proxy/gpt-image-1",
...
@pytest.mark.parametrize("is_async", [False, True])
@pytest.mark.asyncio
async def test_derouter_gateway_from_sdk_transcription(is_async):
    derouter.set_verbose = True
    derouter._turn_on_debug()
    ...
    with patch.object(patch_target.__self__, patch_target.__name__, new=mock_method):
        try:
            if is_async:
                await derouter.atranscription(
                    model="derouter_proxy/whisper-1",
                    file=b"sample_audio",
                    client=openai_client,
                    api_base="my-custom-api-base",
                )
            else:
                derouter.transcription(
                    model="derouter_proxy/whisper-1",
                    file=b"sample_audio",
                    client=openai_client,
                    api_base="my-custom-api-base",
...
@pytest.mark.parametrize("is_async", [False, True])
@pytest.mark.asyncio
async def test_derouter_gateway_from_sdk_speech(is_async):
    derouter.set_verbose = True
...
@pytest.mark.parametrize("is_async", [False, True])
@pytest.mark.asyncio
async def test_derouter_gateway_from_sdk_rerank(is_async):
    derouter.set_verbose = True
    derouter._turn_on_debug()
        ...
        try:
            if is_async:
                response = await derouter.arerank(
                    model="derouter_proxy/rerank-english-v2.0",
...
def test_derouter_gateway_from_sdk_with_response_cost_in_additional_headers():
    derouter.set_verbose = True
    derouter._turn_on_debug()
    ...
    mock_response = MagicMock()
    mock_response.headers = {"x-derouter-response-cost": "120"}
    mock_response.parse.return_value = derouter.ModelResponse(
        **{
...
def test_derouter_gateway_from_sdk_with_thinking_param():
    with pytest.raises(Exception, match=re.escape("Connection error.")) as exc_info:
        response = derouter.completion(
            model="derouter_proxy/anthropic.claude-sonnet-4-5-20250929-v1:0",
            messages=[{"role": "user", "content": "Hello world"}],
            api_base="http://0.0.0.0:4000",
            api_key="sk-PIp1h0RekR",
            # client=openai_client,
