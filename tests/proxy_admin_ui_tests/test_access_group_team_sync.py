...
_DELETE_SEEDED = 'DELETE FROM "DeRouter_AccessGroupTable" WHERE access_group_id = ANY($1::TEXT[])'
_DELETE_TEAMS = 'DELETE FROM "DeRouter_TeamTable" WHERE team_id = ANY($1::TEXT[])'
...
async def _seed(db, assignments):
    for group_id, team_ids in assignments.items():
        await db.derouter_accessgrouptable.create(
            data={
...
async def _read(db):
    rows = await db.query_raw(
        'SELECT access_group_id, assigned_team_ids FROM "DeRouter_AccessGroupTable" '
...
async def _set_team_groups(db, team_id, access_group_ids):
    ...
    if access_group_ids is None:
        ...
        return
    await db.derouter_teamtable.upsert(
...
async def test_reconcile_handles_a_null_array_column():
    ...
    async with _clean_db() as db:
        ...
        await db.execute_raw(
            'UPDATE "DeRouter_AccessGroupTable" SET assigned_team_ids = NULL WHERE access_group_id = $1',
            GROUPS[0],
        )
    ...
    async with _clean_db() as db:
        ...
        async def _blow_up_after_reconcile():
            async with db.tx() as tx:
                await tx.derouter_teamtable.create(data={"team_id": TEAM, "access_group_ids": [GROUPS[0]]})
        ...
        assert await db.derouter_teamtable.find_unique(where={"team_id": TEAM}) is None
        ...
        try:
            async with blocker.tx(timeout=timedelta(seconds=30)) as held:
                ...
                await held.execute_raw(
                    'UPDATE "DeRouter_TeamTable" SET access_group_ids = $1 WHERE team_id = $2',
                    [GROUPS[1]],
                    TEAM,
                )
