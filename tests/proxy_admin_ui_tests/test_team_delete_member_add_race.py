...
_DELETE_SEEDED = 'DELETE FROM "DeRouter_TeamMembership" WHERE team_id = $1'
_DELETE_USER = 'DELETE FROM "DeRouter_UserTable" WHERE user_id = $1'
_DELETE_TEAM = 'DELETE FROM "DeRouter_TeamTable" WHERE team_id = $1'
...
async def test_member_add_blocked_by_delete_writes_no_dangling_reference():
    ...
    async with _clean_db(team_id, user_id) as db:
        await db.derouter_teamtable.create(data={"team_id": team_id, "team_alias": team_id, "members_with_roles": "[]"})
        ...
        async with _real_prisma_client() as prisma_client:
            ...
            async def add_member():
                await _add_team_members_to_team(
                    ...
                    complete_team_data=DeRouter_TeamTable(team_id=team_id, members_with_roles=[]),
                    prisma_client=prisma_client,
                    user_api_key_dict=_admin_auth(),
                    derouter_proxy_admin_name="lit5544-admin",
                )
        ...
        user_row = await db.derouter_usertable.find_unique(where={"user_id": user_id})
        assert user_row is None, "member_add must not have written a user row for a team that was gone under its lock"

        membership_row = await db.derouter_teammembership.find_first(where={"team_id": team_id, "user_id": user_id})
...
async def test_member_delete_blocked_by_member_add_removes_from_the_fresh_roster():
    ...
    async with _clean_db(team_id, user_id) as db:
        await db.derouter_teamtable.create(
            ...
            try:
                ...
                try:
                    async with blocker.tx(timeout=timedelta(seconds=30)) as held:
                        ...
                        await held.derouter_teamtable.update(
        ...
        team_row = await db.derouter_teamtable.find_unique(where={"team_id": team_id})
...
async def test_delete_blocked_by_member_add_sweeps_the_fresh_reference():
    ...
    import derouter.proxy.proxy_server as proxy_server_module
    from derouter.proxy._types import DeRouter_TeamTable
    from derouter.proxy.management_endpoints.team_endpoints import delete_team
    ...
    async with _clean_db(team_id, user_id) as db:
        ...
        async with _real_prisma_client() as prisma_client:
            ...
            original_prisma_client = proxy_server_module.prisma_client
            original_admin_name = proxy_server_module.derouter_proxy_admin_name
            ...
            proxy_server_module.prisma_client = prisma_client
            proxy_server_module.derouter_proxy_admin_name = "lit5544-admin"
            ...
            async def restore():
                proxy_server_module.prisma_client = original_prisma_client
                proxy_server_module.derouter_proxy_admin_name = original_admin_name
            ...
            try:
                ...
                async def run_delete():
                    return await delete_team(
                        ...
                        user_api_key_dict=_admin_auth(),
                        derouter_changed_by="lit5544-admin",
                    )
                ...
                try:
                    async with blocker.tx(timeout=timedelta(seconds=30)) as held:
                        ...
                        await held.derouter_usertable.upsert(
                        ...
                        await held.derouter_teammembership.create(data={"team_id": team_id, "user_id": user_id})
                        await held.derouter_teamtable.update(
                            where={"team_id": team_id},
