...
async def test_get_users(prisma_client):
    ...
    for user in result["users"]:
        assert isinstance(user, DeRouter_UserTable)

    # Clean up test users
    for user in test_users:
        await prisma_client.db.derouter_usertable.delete(where={"user_id": user.user_id})
...
async def test_get_users_filters_dashboard_keys(prisma_client):
    """
    Tests that /users/list endpoint doesn't return keys with team_id='derouter-dashboard'
    ...
    # Create two keys for the user - one with team_id="derouter-dashboard" and one without
    regular_key = await generate_key_helper_fn(
        user_id=test_user.user_id,
        request_type="key",
        team_id="derouter-dashboard",  # This key should be included in the response
        models=[],
        aliases={},
        config={},
        spend=0,
    ...
    await prisma_client.db.derouter_usertable.delete(
...
async def test_get_users_key_count(prisma_client):
    ...
    # Clean up test user and keys
    await prisma_client.db.derouter_usertable.delete(where={"user_id": test_user_id})


async def cleanup_existing_teams(prisma_client):
    all_teams = await prisma_client.db.derouter_teamtable.find_many()
...
def test_is_team_key():
    from derouter.proxy.management_endpoints.key_management_endpoints import _is_team_key
...
def test_team_key_generation_team_member_check():
    ...
    derouter.key_generation_settings = {
...
def test_key_generation_required_params_check(
    ...
    if key_type == "team_key":
        derouter.key_generation_settings = StandardKeyGenerationConfig(
...
async def test_list_key_helper_team_filtering(prisma_client):
    ...
    1. Create keys with different team_ids (None, derouter-dashboard, other)
    2. Verify filtering excludes derouter-dashboard keys
    ...
    # Create 2 keys with team_id=derouter-dashboard
    for i in range(2):
        key = await generate_key_fn(
            data=GenerateKeyRequest(
    ...
    try:
        # Test 1: Get all keys with pagination (exclude derouter-dashboard)
        all_keys = []
        page = 1
        ...
        # Verify no derouter-dashboard keys are present
        dashboard_keys = [k for k in all_keys if k.team_id == "derouter-dashboard"]
        assert len(dashboard_keys) == 0, "Should not include derouter-dashboard keys"
...
@pytest.mark.asyncio
@patch("derouter.proxy.management_endpoints.key_management_endpoints.get_team_object")
async def test_key_generate_always_db_team(mock_get_team_object):
    from derouter.proxy.management_endpoints.key_management_endpoints import (
        generate_key_fn,
    )

    setattr(derouter.proxy.proxy_server, "prisma_client", MagicMock())
