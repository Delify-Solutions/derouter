...
from derouter.proxy.proxy_server import app, ProxyLogging, hash_token
...
def override_env_settings(monkeypatch):
    ...
    monkeypatch.setenv("DEROUTER_MASTER_KEY", MASTER_KEY)
    monkeypatch.setenv("DEROUTER_LOG", "DEBUG")
...
async def test_master_key_not_inserted(test_client):
    ...
    stored_tokens = {
        row.token
        for row in await prisma_client.db.derouter_verificationtoken.find_many()
    }

    for leaked in (hash_token(MASTER_KEY), MASTER_KEY):
        assert leaked not in stored_tokens, (
            "SECURITY ALERT: the master key was found in the derouter_verificationtoken "
    ...
    assert (
        not stored_tokens
    ), f"startup unexpectedly wrote token(s) to derouter_verificationtoken: {stored_tokens}"
