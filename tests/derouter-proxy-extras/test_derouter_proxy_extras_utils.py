...
sys.path.insert(
    0,
    os.path.abspath(
        os.path.join(os.path.dirname(__file__), "../../derouter-proxy-extras")
    ),
)
...
_MIGRATIONS_DIR = os.path.abspath(
    os.path.join(
        ...
        "../../derouter-proxy-extras/derouter_proxy_extras/migrations",
    )
)
...
def test_custom_prisma_dir(monkeypatch):
    ...
    monkeypatch.setenv("DEROUTER_MIGRATION_DIR", temp_dir)
...
_PRE_GUARD_MIGRATIONS = frozenset({
    ...
    "20260713230852_add_key_type_to_derouter_verification_token",
...
ALTER TABLE "DeRouter_BudgetTable" ADD COLUMN     "updated_by" TEXT;

-- AlterTable
ALTER TABLE "DeRouter_SpendLogs" DROP CONSTRAINT "DeRouter_SpendLogs_pkey",
ADD COLUMN     "created_at" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
ADD COLUMN     "updated_at" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
ADD CONSTRAINT "DeRouter_SpendLogs_pkey" PRIMARY KEY ("request_id");

-- DropTable
DROP TABLE "DeRouter_SpendLogs_legacy";
"""


class TestPartitionedSpendLogsDriftFilter:
    """A doc-partitioned DeRouter_SpendLogs (db_scripts/partition_spend_logs.sql) has a
    ...
    def test_pk_rewrite_and_runbook_artifact_drops_are_removed(self):
        ...
        assert 'DROP CONSTRAINT "DeRouter_SpendLogs_pkey"' not in filtered
        assert 'PRIMARY KEY ("request_id")' not in filtered
        assert "DeRouter_SpendLogs_legacy" not in filtered

    def test_legitimate_statements_in_the_same_script_are_kept(self):
        filtered = filter_partitioned_spend_logs_diff(_PARTITIONED_DRIFT_SQL)
        assert 'ALTER TABLE "DeRouter_BudgetTable" ADD COLUMN     "updated_by" TEXT;' in filtered
        ...
        assert filtered.count('ALTER TABLE "DeRouter_SpendLogs"') == 1
    ...
    def test_other_tables_pk_changes_are_untouched(self):
        sql = (
            'ALTER TABLE "DeRouter_TeamTable" DROP CONSTRAINT "DeRouter_TeamTable_pkey",\n'
            'ADD CONSTRAINT "DeRouter_TeamTable_pkey" PRIMARY KEY ("team_id");\n'
        )
        filtered = filter_partitioned_spend_logs_diff(sql)
        assert 'DROP CONSTRAINT "DeRouter_TeamTable_pkey"' in filtered
...
class TestResolveAllMigrationsLedger:
    def _run(self, monkeypatch, tmp_path, partitioned, execute_fails):
        ...
        import derouter_proxy_extras.utils as utils_module
    ...
    def test_partitioned_spend_logs_gets_the_filtered_drift_script(self, monkeypatch, tmp_path):
        ...
        assert "DeRouter_SpendLogs_legacy" not in executed_sql
        assert 'ALTER TABLE "DeRouter_BudgetTable" ADD COLUMN     "updated_by" TEXT;' in executed_sql
...
class TestSpendLogsPartitionDetectionSchemaScope:
    """A same-named DeRouter_SpendLogs in another schema must not trip the
...
class TestSpendLogsPartitionDetectionMissingPsycopg:
    ...
    False here is what let a genuinely partitioned DeRouter_SpendLogs hit the
    ...
    def test_missing_psycopg_logs_a_warning(self, monkeypatch, caplog):
        monkeypatch.setitem(sys.modules, "psycopg", None)
        monkeypatch.setenv("DATABASE_URL", "postgresql://u:p@localhost:5432/db")
        with caplog.at_level("WARNING", logger="derouter_proxy_extras"):
