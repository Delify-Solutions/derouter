...
from models import ChatResponse, DeRouterParamsBody, SpendLogs, SpendLogsParams
...
def test_streaming_messages_via_responses_bridge_tracks_spend(
    ...
    model is bridged through derouter's anthropic-messages -> Responses adapter, and
...
def test_failure_call_writes_failure_status_row(
    ...
    model_id = client.proxy.create_model(
        model,
        DeRouterParamsBody(model="openai/gpt-5.5", api_key="sk-invalid-e2e-failure-row"),
    )
