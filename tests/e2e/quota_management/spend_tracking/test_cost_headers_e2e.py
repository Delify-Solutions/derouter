"""Live e2e: the per-component x-derouter-response-cost-* headers keep their contract.

Pins the header contract shipped in #36965: alongside the x-derouter-response-cost
...
from models import ChatBody, ChatMessage, ChatResponse, DeRouterParamsBody
    ...
    def test_component_cost_headers_sum_to_total(
        ...
        def prime_then_reread() -> StreamingResponse | None:
            ...
            for _ in range(CACHE_REREADS):
                ...
                if _header_cost(response, "x-derouter-response-cost-cache-read") > 0:
                    return response
            return None
        ...
        assert total is not None and total > 0, (
            f"x-derouter-response-cost missing or zero: {measured.headers}"
        )
        ...
        reasoning = _header_cost(measured, "x-derouter-response-cost-reasoning")
        output = _header_cost(measured, "x-derouter-response-cost-output")
        ...
        assert approx_equal(
            _header_cost(measured, "x-derouter-response-cost-cache-read"),
            cached_tokens * CACHE_READ_RATE,
        ), (
            f"cache-read header {measured.headers.get('x-derouter-response-cost-cache-read')} != "
            f"{cached_tokens} cached tokens * {CACHE_READ_RATE}"
        )
        fresh_tokens = (usage.prompt_tokens or 0) - cached_tokens - cache_creation_tokens
        assert approx_equal(
            _header_cost(measured, "x-derouter-response-cost-input"), fresh_tokens * INPUT_RATE
        ), (
            f"input header {measured.headers.get('x-derouter-response-cost-input')} != "
