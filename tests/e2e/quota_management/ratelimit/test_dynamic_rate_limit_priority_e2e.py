...
The proxy under test must run with this config (and DEROUTER_LICENSE set, since
...
from models import KeyGenerateBody, KeyMetadata, DeRouterParamsBody
...
REQUIRED_CONFIG_HINT = (
    "the proxy must run derouter_settings.callbacks=['dynamic_rate_limiter_v3'] with "
...
def _dynamic_limited_model(client: QuotaClient, resources: ResourceManager, label: str) -> _Fixture:
    ...
    model_id = client.proxy.create_model(
        model,
        DeRouterParamsBody(model=BACKEND, api_key="os.environ/ANTHROPIC_API_KEY", tpm=MODEL_TPM),
    )
    ...
    def test_strict_mode_blocks_saturated_priority_but_serves_the_other(
        ...
        while True:
            ...
            if outcome.status_code == 429:
                ...
                assert outcome.headers.get("x-derouter-priority") == DEV_PRIORITY, (
                    f"the 429 must attribute the blocked priority, headers: "
                    f"{ {k: v for k, v in outcome.headers.items() if 'derouter' in k} }"
                )
                break
            require_successful_call(outcome)
            dev_spent += _total_tokens(outcome)
