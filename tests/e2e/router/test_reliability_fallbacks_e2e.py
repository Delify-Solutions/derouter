...
in the x-derouter-attempted-fallbacks header. Empty content is accepted only when
...
def _assert_served_by_fallback(resp: StreamingResponse) -> None:
    ...
    attempted = resp.headers.get("x-derouter-attempted-fallbacks")
    assert attempted is not None, "response is missing the x-derouter-attempted-fallbacks header"
    assert int(attempted) >= 1, f"x-derouter-attempted-fallbacks should be >= 1, got {attempted!r}"
