...
and returns no x-derouter-cache-key), the second is an exact hit (the proxy serves
from cache and returns x-derouter-cache-key). This relies on the standard Redis
...
class TestReliabilityCache:
    ...
    def test_exact_cache_returns_cached(self, client: ComplexityRouterClient, scoped_key: str) -> None:
        ...
        assert "x-derouter-cache-key" not in first.headers, (
        ...
        assert "x-derouter-cache-key" in second.headers, (
