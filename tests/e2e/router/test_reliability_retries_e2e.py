...
def assert_retry_landed_on_backup(resp: StreamingResponse) -> None:
    ...
    attempted = resp.headers.get("x-derouter-attempted-retries")
    assert attempted is not None, "response is missing the x-derouter-attempted-retries header"
    assert int(attempted) >= 1, (
        f"x-derouter-attempted-retries is {attempted!r}; a 200 with no retry means the request never "
