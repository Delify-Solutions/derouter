...
# what derouter maps to ContextWindowExceededError.
...
def create_bad_base_deployment(proxy: ProxyClient, name: str) -> str:
    ...
    return proxy.create_model(
        name, DeRouterParamsBody(model=REAL_MODEL, api_key=REAL_KEY, api_base="http://127.0.0.1:9/v1")
    )
...
def create_timeout_deployment(proxy: ProxyClient, name: str) -> str:
    ...
    return proxy.create_model(name, DeRouterParamsBody(model=REAL_MODEL, api_key=REAL_KEY, timeout=0.001))
...
def create_small_context_deployment(proxy: ProxyClient, name: str) -> str:
    ...
    return proxy.create_model(name, DeRouterParamsBody(model=SMALL_CONTEXT_MODEL, api_key=REAL_KEY))
...
def create_always_timing_out_deployment(proxy: ProxyClient, name: str) -> str:
    ...
    return proxy.register_model(
        ModelNewBody(
            model_name=name,
            derouter_params=DeRouterParamsBody(model=REAL_MODEL, api_key=REAL_KEY, timeout=0.001, weight=1),
...
def create_always_picked_small_context_deployment(proxy: ProxyClient, name: str) -> str:
    ...
    return proxy.register_model(
        ModelNewBody(
            model_name=name,
            derouter_params=DeRouterParamsBody(model=SMALL_CONTEXT_MODEL, api_key=REAL_KEY, weight=1),
...
def create_zero_weight_backup_deployment(proxy: ProxyClient, name: str) -> str:
    ...
    return proxy.register_model(
        ModelNewBody(
            model_name=name,
            derouter_params=DeRouterParamsBody(model=REAL_MODEL, api_key=REAL_KEY, weight=0),
            model_info=ModelInfoBody(),
        )
    )
