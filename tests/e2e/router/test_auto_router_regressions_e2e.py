...
- GitHub issue #36621: a request tagged via the ``x-derouter-tags`` header
...
- GitHub issues #36620/#36621 on /v1/responses: the same tag split holds for
  string and list input, whether the tag arrives in derouter_metadata or the
  x-derouter-tags header.
...
class TaggedAuthHeaders(AuthHeaders):
    x_derouter_tags: str | None = Field(default=None, serialization_alias="x-derouter-tags")
...
class ResponsesBody(BaseModel):
    ...
    derouter_metadata: ResponsesTagMetadata | None = None
...
def split(proxy: ProxyClient) -> Iterator[TagSplitDeployments]:
    ...
    marker_params_a: Final = DeRouterParamsBody(
        model="auto_router/complexity_router",
        complexity_router_config=_uniform_tier_config(deployments.tier_a),
        tags=[deployments.tag_a],
    )
    marker_params_b: Final = DeRouterParamsBody(
        model="auto_router/complexity_router",
        complexity_router_config=_uniform_tier_config(deployments.tier_b),
        tags=[deployments.tag_b],
    )
    registrations: Final[tuple[tuple[str, DeRouterParamsBody], ...]] = (
        (deployments.shared_a, DeRouterParamsBody(model=PLAIN_MODEL, api_key=anthropic_key)),
        (deployments.tier_a, DeRouterParamsBody(model=CHEAP_MODEL, api_key=anthropic_key, tags=[deployments.tag_a])),
        (deployments.shared_a, marker_params_a),
        (deployments.shared_b, marker_params_b),
        (deployments.tier_b, DeRouterParamsBody(model=CHEAP_MODEL, api_key=anthropic_key)),
        (deployments.shared_b, DeRouterParamsBody(model=PLAIN_MODEL, api_key=anthropic_key)),
    )
...
def zero_priced_alias(proxy: ProxyClient) -> Iterator[ZeroPricedAlias]:
    ...
    alias_params: Final = DeRouterParamsBody(
        model="auto_router/complexity_router",
    ...
    registrations: Final[tuple[tuple[str, DeRouterParamsBody], ...]] = (
        (named.tier, DeRouterParamsBody(model=CHEAP_MODEL, api_key=_provider_key("ANTHROPIC_API_KEY"))),
...
def heuristic_split(proxy: ProxyClient) -> Iterator[HeuristicSplit]:
    ...
    registrations: Final[tuple[tuple[str, DeRouterParamsBody], ...]] = (
        (named.cheap, DeRouterParamsBody(model=CHEAP_MODEL, api_key=_provider_key("ANTHROPIC_API_KEY"))),
        (named.strong, DeRouterParamsBody(model=STRONG_MODEL, api_key=_provider_key("OPENAI_API_KEY"))),
        (named.alias, DeRouterParamsBody(model="auto_router/complexity_router", complexity_router_config=config)),
    )
    created: Final = tuple(proxy.create_model(name, params) for name, params in registrations)
    try:
        yield named
...
def semantic_auto_router(proxy: ProxyClient) -> Iterator[SemanticAutoRouter]:
    ...
    marker_params: Final = DeRouterParamsBody(
    ...
    registrations: Final[tuple[tuple[str, DeRouterParamsBody], ...]] = (
        (named.embedding, DeRouterParamsBody(model=EMBEDDING_MODEL, api_key=_provider_key("OPENAI_API_KEY"))),
        (named.target, DeRouterParamsBody(model=CHEAP_MODEL, api_key=_provider_key("ANTHROPIC_API_KEY"))),
        (named.fallback, DeRouterParamsBody(model=PLAIN_MODEL, api_key=_provider_key("ANTHROPIC_API_KEY"))),
...
class TestUntaggedTierDeployments:
    ...
    def test_header_tagged_messages_routes_through_the_marker_to_an_untagged_tier(
        ...
        x-derouter-tags header selects the tagged marker, and the rewrite still
        ...
        headers: Final = TaggedAnthropicHeaders(authorization=f"Bearer {key}", x_derouter_tags=split.tag_b)
        answer: Final = unwrap(
...
class TestResponsesApiTagRouting:
    ...
    def test_header_tagged_responses_with_string_input_routes_to_the_tier(
        ...
        the x-derouter-tags header, succeeds and routes through the tagged
        marker to its tier."""
        key: Final = _key_for(proxy, resources, [split.shared_a, split.tier_a], tag_filtering=True)
        headers: Final = TaggedAuthHeaders(authorization=f"Bearer {key}", x_derouter_tags=split.tag_a)
    ...
    def test_body_tagged_responses_with_list_input_routes_to_the_tier(
        ...
        /v1/responses with derouter_metadata.tags and structured input items
        ...
        body: Final = ResponsesBody(
            ...
            derouter_metadata=ResponsesTagMetadata(tags=[split.tag_a]),
        )
        answer: Final = unwrap(
            proxy.transport.post(
                "/v1/responses",
