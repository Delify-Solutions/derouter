...
derouter_params; the proxy fronts it at /a2a/{id}, serving a proxy-owned agent card
at /.well-known/agent-card.json and accepting A2A JSON-RPC calls at /a2a/{id}. This
suite registers agents backed by the derouter_completion_bridge (custom_llm_provider
...
class A2ABridgeParams(BaseModel):
    """derouter_params that route the agent through the completion bridge: an A2A
    message/send is transformed into a derouter.acompletion against this provider."""
...
class AgentRegisterBody(BaseModel):
    ...
    derouter_params: A2ABridgeParams | None = None
