...
derouter_params route it through the completion bridge; a caller then discovers the
...
# No api_key: derouter resolves ANTHROPIC_API_KEY from the proxy's own environment
...
# bridge forwards derouter_params to acompletion() without expanding "os.environ/"
...
def _register(client: A2AClient, resources: ResourceManager, protocol_version: str) -> AgentResponse:
    ...
    body = AgentRegisterBody(
        ...
        agent_card_params=AgentCardParams(
            ...
            description="e2e agent backed by the derouter completion bridge",
            version="1.0.0",
            skills=[A2ASkill(id="chat", name="Chat", description="general chat", tags=["chat"])],
        ),
        derouter_params=BRIDGE,
    )
