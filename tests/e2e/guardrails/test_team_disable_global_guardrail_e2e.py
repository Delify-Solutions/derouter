...
Uses a local derouter_content_filter (keyword match, no external service) so the
...
class TestTeamDisableGlobalGuardrail:
    @pytest.mark.covers(
        "guardrail.derouter_content_filter.pre_call.blocks",
    ...
    @pytest.mark.covers(
        "guardrail.derouter_content_filter.pre_call.allows",
        exercised_on=["chat_completions"],
    )
