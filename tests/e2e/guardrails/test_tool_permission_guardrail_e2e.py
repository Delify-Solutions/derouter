...
- allows: a request declaring only the permitted tool is served normally, comes
  ...
  `x-derouter-applied-guardrails` header naming the guardrail, which is what
...
def _applied_guardrails(outcome: StreamingResponse) -> str:
    return outcome.headers.get("x-derouter-applied-guardrails", "")
...
class TestToolPermissionPreCall:
    ...
    def test_pre_call_allows_permitted_tool(
        ...
        assert name in applied, (
            "the allowed call must carry x-derouter-applied-guardrails naming the guardrail; "
