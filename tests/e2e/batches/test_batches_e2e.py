"""Live e2e for the Batches API across every provider DeRouter supports.
...
from models import KeyGenerateBody, KeyMetadata, DeRouterParamsBody, SpendLogRow
...
@pytest.mark.skip(
    reason=(
        ...
        "to count tokens by awaiting derouter.afile_content with no timeout, so a slow Files "
...
class TestOpenAIFiles:
    ...
    @pytest.mark.skip(
        reason=(
            "LIT-4820 (https://linear.app/derouter-ai/issue/LIT-4820): GET /v1/files omits "
...
def _assume_role_params(role_arn: str, session_name: str) -> DeRouterParamsBody:
    return DeRouterParamsBody(
...
class TestBedrockBatchAssumeRole:
    ...
    Provisions a bedrock batch deployment whose derouter_params carry
...
def _vllm_params(api_base: str, api_key: str | None, model_id: str) -> DeRouterParamsBody:
    return DeRouterParamsBody(
...
class TestBatchSecondHop:
    """Two-proxy batch routing: a derouter_proxy deployment chained to the gateway
    itself (LIT-5347, PR #36240).

    The hop deployment's derouter_params point derouter_proxy/<inner model> at this
    ...
    def test_unified_create_and_retrieve_via_chained_gateway(
        ...
        model_id = client.create_model(
            hop_name,
            DeRouterParamsBody(
                model=f"derouter_proxy/{OPENAI_BATCH_MODEL}",
...
TERMINAL_MARKER_KEY = "derouter_e2e_suite"
