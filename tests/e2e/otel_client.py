...
Traces are fetched server-side by the ``derouter.call_id`` tag the gen-AI span
carries (the request's x-derouter-call-id response header), so read-back is
...
JAEGER_SERVICE = "derouter"
#: Span tag carrying the request's x-derouter-call-id (stamped on the gen-AI span).
CALL_ID_TAG = "derouter.call_id"
