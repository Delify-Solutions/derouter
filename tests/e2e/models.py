...
class DeRouterBudgetTable(BaseModel):
    max_budget: float | None = None
    soft_budget: float | None = None
    budget_duration: str | None = None
...
class KeyInfo(BaseModel):
    ...
    budget_reset_at: str | None = None
    budget_id: str | None = None
    derouter_budget_table: DeRouterBudgetTable | None = None
...
class AnthropicServerTool(BaseModel):
    ...
    `input_schema`; `type` is the SDK-version-pinned identifier DeRouter keys its
...
class AnthropicMessagesResponse(BaseModel):
    ...
    shape; `choices` is the OpenAI-normalized shape DeRouter emits for some
...
class CustomPricing(BaseModel):
    ...
    derouter_params - the token-cost subset of derouter's CustomPricingDeRouterParams
...
class ModelInfoEntry(BaseModel):
    """One /model/info row. `derouter_params` is the configured deployment (carries
    ...
    model_name: str
    derouter_params: CustomPricing = CustomPricing()
...
class DeRouterParamsBody(BaseModel):
    """POST /model/new derouter_params: `model` is the only required field; `api_key`
...
class ModelNewBody(BaseModel):
    model_config = ConfigDict(protected_namespaces=())
    model_name: str
    derouter_params: DeRouterParamsBody
...
class ModelUpdateBody(BaseModel):
    ...
    `derouter_params` to merge over its stored params. The handler overlays only the
...
class ReadinessDetailsResponse(ReadinessResponse):
    ...
    derouter_version: str | None = None
