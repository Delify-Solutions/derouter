...
def render_prometheus(report: CoverageReport) -> str:
    lines = [
        "# HELP derouter_e2e_coverage_cells E2E coverage registry cells by module and state.",
        "# TYPE derouter_e2e_coverage_cells gauge",
    ]
    for module in report.modules:
        label = _label_value(module.module)
        lines.append(
            f'derouter_e2e_coverage_cells{{module="{label}",state="covered"}} {module.covered}'
        )
        lines.append(
            f'derouter_e2e_coverage_cells{{module="{label}",state="total"}} {module.total}'
        )
    lines.extend(
        [
            f'derouter_e2e_coverage_cells{{module="ALL",state="covered"}} {report.covered}',
            f'derouter_e2e_coverage_cells{{module="ALL",state="total"}} {report.total}',
            "# HELP derouter_e2e_coverage_percent E2E coverage percent by module.",
            "# TYPE derouter_e2e_coverage_percent gauge",
        ]
    )
    for module in report.modules:
        label = _label_value(module.module)
        lines.append(
            f'derouter_e2e_coverage_percent{{module="{label}"}} {module.coverage_percent:.6f}'
        )
    lines.extend(
        [
            f'derouter_e2e_coverage_percent{{module="ALL"}} {report.coverage_percent:.6f}',
            "# HELP derouter_e2e_coverage_orphan_markers Coverage markers not found in the registry.",
            "# TYPE derouter_e2e_coverage_orphan_markers gauge",
            f"derouter_e2e_coverage_orphan_markers {len(report.orphan_markers)}",
            "# HELP derouter_e2e_coverage_skipped_markers Registry cells claimed only by skipped tests.",
            "# TYPE derouter_e2e_coverage_skipped_markers gauge",
            f"derouter_e2e_coverage_skipped_markers {len(report.skipped_markers)}",
            "# HELP derouter_e2e_coverage_collection_errors Pytest nodes that failed during collection.",
            "# TYPE derouter_e2e_coverage_collection_errors gauge",
            f"derouter_e2e_coverage_collection_errors {len(report.collection_errors)}",
        ]
    )
    return "\n".join(lines)
