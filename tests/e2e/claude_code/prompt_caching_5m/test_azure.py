...
Drive the real `claude` CLI against a running DeRouter proxy that routes
...
variant is *not* supported on Foundry — DeRouter strips that field
before forwarding (see `_remove_scope_from_cache_control` in
`derouter/llms/azure_ai/anthropic/messages_transformation.py`) — but
...
def test_prompt_caching_5m_azure(compat_result):
    """Drive the `claude` CLI against the DeRouter proxy and assert the
