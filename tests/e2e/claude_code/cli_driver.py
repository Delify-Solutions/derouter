...
DEFAULT_TIMEOUT_SECONDS = float(
    os.environ.get("DEROUTER_COMPAT_CLI_TIMEOUT_SECONDS") or 120
)
...
DEFAULT_RATE_LIMIT_RETRIES = int(
    os.environ.get("DEROUTER_COMPAT_RATE_LIMIT_RETRIES") or 2
)
DEFAULT_RATE_LIMIT_BACKOFF_SECONDS = float(
    os.environ.get("DEROUTER_COMPAT_RATE_LIMIT_BACKOFF_SECONDS") or 65
)
...
def run_claude(
    *,
    ...
    The CLI is pointed at a DeRouter proxy via `ANTHROPIC_BASE_URL` /
...
def run_claude_models_parallel(
    *,
    ...
    windows can reset; both default to the `DEROUTER_COMPAT_RATE_LIMIT_*`
