#!/usr/bin/env bash
# Run the Claude Code compat matrix end-to-end against a live DeRouter
...
#   DEROUTER_PROXY_URL        e.g. http://localhost:4000
#   DEROUTER_MASTER_KEY       e.g. sk-1234
#
# Optional env (rate limits, all default to 5 req/s; 0 disables a column):
#   DEROUTER_COMPAT_RATE_ANTHROPIC
#   DEROUTER_COMPAT_RATE_AZURE
#   DEROUTER_COMPAT_RATE_VERTEX_AI
#   DEROUTER_COMPAT_RATE_BEDROCK_CONVERSE
#   DEROUTER_COMPAT_RATE_BEDROCK_INVOKE
#   DEROUTER_COMPAT_RATE_OPENAI
#   DEROUTER_COMPAT_RATE_AZURE_OPENAI
#   DEROUTER_COMPAT_RATE_BEDROCK_MANTLE
#   DEROUTER_COMPAT_RATE_BURST            override per-bucket burst
#
...
if [[ -z "${DEROUTER_PROXY_URL:-}" || -z "${DEROUTER_MASTER_KEY:-}" ]]; then
    echo "error: DEROUTER_PROXY_URL and DEROUTER_MASTER_KEY must be set" >&2
    exit 64
fi
...
state_dir="${DEROUTER_COMPAT_RATE_STATE_DIR:-${TMPDIR:-/tmp}/derouter-claude-compat-ratelimit}"
if [[ -d "$state_dir" ]]; then
    rm -rf "$state_dir"
fi
...
for provider in ANTHROPIC AZURE VERTEX_AI BEDROCK_CONVERSE BEDROCK_INVOKE OPENAI AZURE_OPENAI BEDROCK_MANTLE; do
    var="DEROUTER_COMPAT_RATE_${provider}"
    echo "  ${provider}=${!var:-default(5/s)}"
done
echo "  BURST=${DEROUTER_COMPAT_RATE_BURST:-default(=rate)}"
