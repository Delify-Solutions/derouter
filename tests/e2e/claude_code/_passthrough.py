...
layer: Claude Code speaks the first-party Anthropic wire and DeRouter
transforms the request per provider. This row instead exercises
DeRouter's *native passthrough* routes -- the "LLM gateway"
...
disabled via CLAUDE_CODE_SKIP_<PROVIDER>_AUTH, and the DeRouter virtual
...
def vertex_extra_env(proxy_base_url: str) -> Dict[str, str]:
    ...
    the override carries it. DeRouter's /vertex_ai route reuses the
