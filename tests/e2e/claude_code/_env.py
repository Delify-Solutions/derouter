...
Uses the same ``DEROUTER_PROXY_URL`` / ``DEROUTER_MASTER_KEY`` names as
...
PRIMARY_BASE_URL_ENV = "DEROUTER_PROXY_URL"
PRIMARY_API_KEY_ENV = "DEROUTER_MASTER_KEY"
    ...
    compat_result.set(
        {
            ...
            "error": (
                ...
                f"{PRIMARY_API_KEY_ENV} to point at a running DeRouter proxy"
            ),
        }
    )
    pytest.fail(
