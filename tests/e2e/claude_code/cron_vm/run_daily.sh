...
#
# Runs from the GCP VM `derouter-compatibility-matrix-populator` via the
# systemd timer in this directory. The flow is:
#
#   1. Resolve the latest DeRouter final release tag from the GitHub
#      Releases API.
...
#   6. `gh repo clone` derouter-docs, write the JSON to a deterministic
#      branch (`compat-matrix/<derouter>-<claude>-<UTC-date>`), commit,
#      push the branch straight to Delify-Solutions/derouter-docs (mateo-berri has
...
# Required state: a derouter checkout at $DEROUTER_REPO (this file lives in
...
DEROUTER_REPO="${DEROUTER_REPO:-${HOME}/derouter/derouter}"
WORKTREE="${DEROUTER_WORKTREE:-${HOME}/derouter-cron-worktree}"
PROXY_PORT="${PROXY_PORT:-4100}"
PROXY_API_KEY="${PROXY_API_KEY:-sk-cron-matrix}"
DOCS_REPO="${DOCS_REPO:-Delify-Solutions/derouter-docs}"
...
# Merge method for auto-merge. Delify-Solutions/derouter-docs only allows squash
...
WORKDIR="$(mktemp -d -t derouter-compat-matrix.XXXXXX)"
...
#   2. SIGKILL anything from `pgrep -f "derouter.*--port ${PROXY_PORT}"`
...
cleanup() {
  ...
  pgrep -f "derouter.*--port[ =]?${PROXY_PORT}([^0-9]|$)" 2>/dev/null \
    | xargs -r kill -KILL 2>/dev/null || true
  pgrep -f "${WORKTREE}/.uv-bin/uv.*run derouter" 2>/dev/null \
    | xargs -r kill -KILL 2>/dev/null || true
  rm -rf "${WORKDIR}"
  exit "${rc}"
}
...
# Publishing pushes the branch straight to Delify-Solutions/derouter-docs and opens
...
if [[ "${SKIP_PUBLISH}" != "1" ]]; then
  [[ -n "${GITHUB_TOKEN:-}" ]] \
    || die "publish token required: /etc/derouter-compat-matrix-github-token via LoadCredential under systemd, or an exported GITHUB_TOKEN for manual runs (or set SKIP_PUBLISH=1)"
fi
...
# Newest PEP 440 *final* release on Delify-Solutions/derouter. DeRouter moved off
...
# (default page_size=30). DeRouter ships multiple pre-releases per day, so
...
for page in 1 2 3 4 5; do
  ...
  curl -fsS \
    ...
    -H 'User-Agent: derouter-compat-matrix' \
    "${GH_AUTH_HEADER[@]}" \
    "https://api.github.com/repos/Delify-Solutions/derouter/releases?per_page=100&page=${page}" \
    >"${PAGE_JSON}"
...
[[ -n "${DEROUTER_VERSION}" ]] || die "could not resolve latest PEP 440 final release (vX.Y.Z) in 5 pages of releases"
log "resolved derouter: ${DEROUTER_VERSION}"
...
if [[ ! -d "${WORKTREE}/.git" ]]; then
  log "first run: cloning derouter into ${WORKTREE}"
  mkdir -p "$(dirname "${WORKTREE}")"
  git clone https://github.com/Delify-Solutions/derouter.git "${WORKTREE}"
fi

log "updating worktree to ${DEROUTER_VERSION}"
...
git -C "${WORKTREE}" checkout --force "${DEROUTER_VERSION}"

# Always rebuild tests/e2e/ in the worktree from the dev checkout,
# regardless of what the resolved ${DEROUTER_VERSION} tag ships. Two
# reasons:
#
...
#     ${DEROUTER_REPO}, not whenever the next stable release happens.
...
if [[ ! -d "${DEROUTER_REPO}/tests/e2e/claude_code" ]]; then
  die "no shim source at ${DEROUTER_REPO}/tests/e2e/claude_code"
fi
for helper in "${E2E_HELPER_FILES[@]}"; do
  [[ -f "${DEROUTER_REPO}/tests/e2e/${helper}" ]] \
    || die "missing shim helper: ${DEROUTER_REPO}/tests/e2e/${helper}"
done
log "shimming tests/e2e/claude_code/ + helpers from ${DEROUTER_REPO} (always-overwrite)"
rm -rf "${WORKTREE}/tests/e2e"
mkdir -p "${WORKTREE}/tests/e2e"
cp -r "${DEROUTER_REPO}/tests/e2e/claude_code" "${WORKTREE}/tests/e2e/"
for helper in "${E2E_HELPER_FILES[@]}"; do
  cp "${DEROUTER_REPO}/tests/e2e/${helper}" "${WORKTREE}/tests/e2e/"
done

# derouter pins an exact uv version in pyproject.toml's [tool.uv]
...
# `--extra proxy` pulls fastapi/uvicorn/etc. so `uv run derouter` can
...
# the test env set `DEROUTER_PROXY_URL=http://127.0.0.1:...`),
# so there's no reason to expose it on the VM's external interfaces.
# Without `--host`, `derouter` defaults to 0.0.0.0, which combined with
# the predictable default `DEROUTER_MASTER_KEY=sk-cron-matrix` would
...
setsid env DEROUTER_MASTER_KEY="${PROXY_API_KEY}" bash -c '
  echo "$$" > "$0"
  cd "$1"
  exec "$2" run derouter --config "$3" --host 127.0.0.1 --port "$4"
...
(
  cd "${WORKTREE}" \
    && DEROUTER_PROXY_URL="http://127.0.0.1:${PROXY_PORT}" \
       DEROUTER_MASTER_KEY="${PROXY_API_KEY}" \
...
(
  cd "${WORKTREE}" \
    && "${WORKTREE_UV}" run python "${POPULATOR_DIR}/build_matrix.py" \
       ...
       --output "${MATRIX_JSON}" \
       --derouter-version "${DEROUTER_VERSION}" \
...
if [[ "${SKIP_PUBLISH}" == "1" ]]; then
  cp "${MATRIX_JSON}" "${DEROUTER_REPO}/compatibility-matrix.json"
  log "SKIP_PUBLISH=1; matrix written to ${DEROUTER_REPO}/compatibility-matrix.json"
  exit 0
fi

DATE_UTC="$(date -u +%Y-%m-%d)"
BRANCH_NAME="compat-matrix/${DEROUTER_VERSION}-${CLAUDE_CODE_VERSION}-${DATE_UTC}"
DOCS_CLONE="${WORKDIR}/derouter-docs"

log "cloning ${DOCS_REPO}@${DOCS_BRANCH}"
gh repo clone "${DOCS_REPO}" "${DOCS_CLONE}" -- --depth 1 --branch "${DOCS_BRANCH}"

cd "${DOCS_CLONE}"
git config user.email "derouter-bot@berri.ai"
git config user.name "derouter-compat-matrix-bot"
...
derouter_version: ${DEROUTER_VERSION}
...
# Push the branch straight to Delify-Solutions/derouter-docs. mateo-berri has write
...
PR_TITLE="chore(compat-matrix): refresh for ${DEROUTER_VERSION} + claude-code ${CLAUDE_CODE_VERSION}"
PR_BODY="$(cat <<EOF
