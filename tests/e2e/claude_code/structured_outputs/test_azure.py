...
flag, route through a DeRouter proxy aimed at Azure (Microsoft Foundry), and assert that
...
DeRouter's `output_config`-stripping fixes (2.1.122, 2.1.81) surface in
...
def _validate_against_schema(
    ...
    not a DeRouter-proxy bug, so a deeper check would only add false
...
def test_structured_outputs_azure(compat_result):
    """Drive `claude --json-schema ...` against the DeRouter proxy and
