...
# on), while ``DeRouterParamsBody`` names them without the ``_ai`` infix
...
# raw ``DeRouterParamsBody(**entry)`` would produce a body with the
...
def load_all_deployments(
    ...
    return tuple(
        CompatDeployment(
            ...
            derouter_params=DeRouterParamsBody(
                **_normalize_params(entry["derouter_params"])
            ),
        )
