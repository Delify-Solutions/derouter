...
Drive the real `claude` CLI in headless mode against a running DeRouter
...
in Microsoft Foundry on 2025-11-18; DeRouter exposes them via the
...
# Per-model aliases registered in the DeRouter proxy's routing config to
...
def test_basic_messaging_non_streaming_azure(compat_result):
    """Drive the `claude` CLI against the DeRouter proxy and assert a reply.
