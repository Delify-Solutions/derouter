...
HTTP POST against a DeRouter proxy endpoint, parsed and shape-checked
...
the upstream, or DeRouter 500 on a transformation bug).
"""
...
# The discovery tool's `_20251119`-suffixed type is what DeRouter keys its
...
def probe_count_tokens(
    *,
    ...
    The Anthropic / DeRouter `count_tokens` endpoint accepts a request body whose
...
def probe_tool_search(
    *,
    ...
    DeRouter's `is_tool_search_used` helper keys on the `_20251119`-suffixed type
...
def assert_tool_search_shape(result: Result[AnthropicMessagesResponse]) -> str | None:
    ...
    Acceptance criteria:
      ...
      2. The body has either `content` (Anthropic-shape passthrough) or `choices`
         (DeRouter normalized OpenAI-shape, used by Bedrock Converse). Either is
