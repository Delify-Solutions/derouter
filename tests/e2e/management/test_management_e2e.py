...
class TestModelRoutes:
    ...
    def test_update_persists_input_cost_to_model_info(
        ...
        initial = before.derouter_params.input_cost_per_token
        ...
        client.proxy.update_model(
            model_id,
            DeRouterParamsBody(model="gpt-4o-mini", input_cost_per_token=_UPDATED_INPUT_COST),
        )
        ...
        def updated() -> ModelInfoEntry | None:
            ...
            if entry is None:
                return None
            cost = entry.derouter_params.input_cost_per_token
    ...
    def test_delete_removes_from_model_info_catalog(
        ...
        model_name = f"e2e-mgmt-model-{unique_marker()}"
        model_id = client.proxy.create_model(model_name, DeRouterParamsBody(model="openai/gpt-5.5", api_key="dummy"))
        resources.defer(lambda: client.proxy.delete_model(model_id))
    ...
    def test_new_persists_to_model_info_catalog(
        ...
        model_id = client.proxy.create_model(
            model_name,
            DeRouterParamsBody(model="openai/gpt-5.5", api_key="e2e-dummy-key"),
        )
        resources.defer(lambda: client.proxy.delete_model(model_id))
