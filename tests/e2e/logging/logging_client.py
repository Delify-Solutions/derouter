...
``derouter_request``; correlate by unique prompt marker and ``user_api_key_alias``
...
# Not a DeRouter virtual key; OpenAI must reject it after the proxy accepts the call.
...
class GuardrailSpec(BaseModel):
    guardrail_name: str
    derouter_params: GuardrailLitellmParams
...
def completion_response_id(body: str) -> str | None:
    """SpendLogs.request_id is the chat completion body id, not x-derouter-call-id."""
...
def _matches_run(obs: LangfuseObservation, *, key_alias: str, prompt_marker: str) -> bool:
    ...
    langfuse_otel names generations ``derouter_request`` (not ``derouter:{alias}``).
    ...
    if key_alias and key_alias in meta_blob:
        return True
    if obs.name == f"derouter:{key_alias}":
        return True
    return False
...
class LoggingClient:
    ...
    def create_model(self, model_name: str, derouter_params: DeRouterParamsBody) -> str:
        return self.proxy.create_model(model_name, derouter_params)
    ...
    def messages_raw(
        ...
        status/body/headers, for tests that need x-derouter-call-id. With
    ...
    def responses_raw(
        ...
        status/body/headers, for tests that need x-derouter-call-id.
    ...
    def find_langfuse_observation(
        ...
        *,
        ...
        # langfuse_otel generations are named derouter_request; classic SDK used
        # derouter:{key_alias}. Search both, then a recent unfiltered page.
        for name in ("derouter_request", f"derouter:{key_alias}"):
