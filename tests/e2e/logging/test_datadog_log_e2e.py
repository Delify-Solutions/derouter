...
against the x-derouter-response-cost header of the very response the caller
received).
"""
...
def _assert_exactly_one_event(
    ...
    *,
    ...
    sourced from derouter, whose payload names the model group and call type,
    counts real tokens, and carries the same cost as ``cost_anchor`` - the
    x-derouter-response-cost header for non-streaming calls, or the /spend/logs
    ...
    event = events[0]
    assert "source:derouter" in event.tags, (
        f"the ingested event must carry the derouter source (shipped as ddsource), got tags {event.tags!r}"
    )
...
class TestDataDogLogDelivery:
    ...
    def test_chat_completions_emits_one_log_event(
        ...
        assert outcome.response_cost is not None and outcome.response_cost > 0, (
            f"the response must report x-derouter-response-cost, got {outcome.response_cost!r}"
        )
...
def _assert_exactly_one_failure_event(events: list[DdLogEvent], *, model_group: str) -> _DdMessagePayload:
    ...
    event for the deployment, sourced from derouter, indexed at an error-grade
