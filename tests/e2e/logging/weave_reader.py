...
``derouter_request`` op and to calls that started after the test's own request,
...
#: The op every derouter LLM call lands under. The proxy also exports a root
...
DEROUTER_REQUEST_OP: Final = "derouter_request"
...
class WeaveReader:
    ...
    def calls_matching(self, marker: str, *, since: float, op: str = DEROUTER_REQUEST_OP) -> tuple[WeaveCall, ...]:
    ...
    def poll_calls_matching(self, marker: str, *, since: float, op: str = DEROUTER_REQUEST_OP) -> tuple[WeaveCall, ...]:
