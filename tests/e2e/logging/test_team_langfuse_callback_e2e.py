...
with the cost agreeing with the x-derouter-response-cost header), while traffic
    ...
    def test_team_callback_delivers_and_isolates(
        ...
        for _attempt in range(2):
            ...
            assert team_outcome.response_cost is not None and team_outcome.response_cost > 0, (
                f"the response must report x-derouter-response-cost, got {team_outcome.response_cost!r}"
            )
