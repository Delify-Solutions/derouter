...
from models import DeRouterParamsBody
...
def _assert_otel_destination_configured(client: LoggingClient) -> None:
    ...
    assert OTEL_V2_LOGGER_NAME in details.success_callbacks, (
        ...
        f"(DEROUTER_OTEL_V2 + arize_phoenix preset in the compose config); got: {details.success_callbacks}"
    )
...
EXPECTED_ERROR_SPAN_ATTRIBUTES: dict[str, str] = {
    ...
    "derouter.provider.error.code": "401",
    "derouter.provider.error.llm_provider": "anthropic",
}
...
def _assert_error_span_contract(span: JaegerSpan) -> None:
    ...
    assert provider_error.error.type == "authentication_error", (
        f"the span must carry anthropic's own auth error object rather than a derouter "
    ...
    stack = _tag(span, "derouter.provider.error.stack_trace")
    assert isinstance(stack, str) and stack, "the error span must carry a non-empty derouter.provider.error.stack_trace"
...
class TestOtelTraceCompleteness:
    ...
    def test_chat_completions_exports_complete_trace(
        ...
        assert outcome.call_id is not None, "success response must carry x-derouter-call-id"
    ...
    def test_chat_completions_stream_exports_complete_trace(
        ...
        * The gen-AI span contains `derouter.request.streaming=true`.
        """
        route = "/chat/completions"
        _assert_otel_destination_configured(client)
        ...
        assert _tag(served, "derouter.request.streaming") is True, (
            "the gen-AI span must record derouter.request.streaming=true; its absence means "
    ...
    def test_failed_chat_completions_error_span_attributes(
        ...
        allows the request to pass DeRouter’s proxy authentication and fail at
        ...
        model_id = client.create_model(
            model_name,
            DeRouterParamsBody(model="anthropic/claude-haiku-4-5", api_key=INVALID_UPSTREAM_API_KEY),
        )
        ...
        assert outcome.call_id is not None, "failed responses must still carry x-derouter-call-id"
    ...
    def test_failed_messages_error_span_attributes(
        ...
        derouter.provider.error.llm_provider attribute.
