...
x-derouter-response-cost header of the very response the caller received).
"""
...
def _assert_gcs_configured(client: LoggingClient) -> None:
    ...
    assert GCS_LOGGER_NAME in body, (
        ...
        f"(derouter_settings.callbacks: ['gcs_bucket'] + GCS_BUCKET_NAME env + enterprise license); "
        f"got: {body[:400]}"
    )
