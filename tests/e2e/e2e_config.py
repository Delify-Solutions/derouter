"""Generic configuration for live e2e tests against a running DeRouter proxy.
...
PROXY_BASE_URL = os.environ.get("DEROUTER_PROXY_URL", "http://localhost:4000").rstrip("/")
MASTER_KEY = os.environ.get("DEROUTER_MASTER_KEY", "sk-1234")
...
# Set DEROUTER_CONTROL_PLANE_URL only when management is a different base than
# the LLM host and you are not going through an ingress that path-routes.
CONTROL_PLANE_BASE_URL = os.environ.get(
    "DEROUTER_CONTROL_PLANE_URL", PROXY_BASE_URL
).rstrip("/")
...
PROXY_REPLICA_URLS: Final = parse_replica_urls(os.environ.get("DEROUTER_PROXY_REPLICA_URLS", ""), PROXY_BASE_URL)
