...
def _azure_params(model: str) -> DeRouterParamsBody:
...
def _vertex_params(model: str, location: str) -> DeRouterParamsBody:
    return DeRouterParamsBody(
        model=model,
...
def _register_deployment(
    client: EndpointsClient, resources: ResourceManager, params: DeRouterParamsBody
) -> str:
...
#:   derouter -> anthropic/claude-opus-4-8   7013  preserved
#:   derouter -> vertex_ai/claude-opus-4-8      0  destroyed
...
MID_CONVERSATION_CACHE_SKIP_REASON = (
    ...
    "both direct to Anthropic and through derouter's first-party anthropic path. Product bug, not "
