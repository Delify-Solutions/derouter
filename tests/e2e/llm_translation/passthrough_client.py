...
derouter virtual key is passed as the provider key; the proxy swaps in the real env
credential. SpendLogs.request_id == the x-derouter-call-id response header. The
...
class VertexHeaders(Headers):
    # Only the derouter virtual key; the /vertex_ai passthrough mints the Vertex token
    ...
    x_derouter_api_key: str = Field(serialization_alias="x-derouter-api-key")
    content_type: str = Field(
        default="application/json", serialization_alias="Content-Type"
    )
...
class OpenAIChatBody(BaseModel):
    ...
    # 'max_completion_tokens' instead." derouter's drop_params/translation does not
    # apply on this route.
...
class PassthroughClient:
    ...
    def vertex_generate(
        ...
        return self.proxy.transport.send(
            path,
            headers=VertexHeaders(x_derouter_api_key=key),
            json=GeminiGenerateBody(
                contents=[GeminiContent(parts=[GeminiPart(text=text)])]
            ),
        )
