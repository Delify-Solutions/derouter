...
def _managed_deployment(client: EndpointsClient, resources: ResourceManager) -> str:
    ...
    model_id = client.create_model(
        model,
        DeRouterParamsBody(model=UPSTREAM_MODEL, api_key="os.environ/GEMINI_API_KEY"),
    )
...
class TestGoogleNativeGenerateContent:
    ...
    def test_generate_content_returns_response_cost_header(
        ...
        assert result.call_id, "generateContent must stamp x-derouter-call-id"
        assert result.response_cost is not None, (
            "generateContent returned no x-derouter-response-cost header; "
            "google-native traffic cannot be reconciled against spend without it"
        )
        assert result.response_cost > 0, f"x-derouter-response-cost must be a real cost, got {result.response_cost}"
