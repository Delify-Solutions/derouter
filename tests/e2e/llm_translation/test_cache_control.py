...
- Vertex (gemini-2.5-flash): explicit context caching via ``cache_control``
  with a 5-minute ttl. derouter builds the Vertex cache before the generate
...
from models import ChatBody, ChatMessage, ChatResponse, DeRouterParamsBody, Usage
...
class TestCacheControl:
    ...
    def test_bedrock_prompt_caching_reads_cache(
        ...
        model_id = client.proxy.create_model(
            model,
            DeRouterParamsBody(model=BEDROCK_MODEL, aws_region_name="us-east-1"),
        )
    ...
    def test_anthropic_prompt_caching_reads_cache(
        ...
        model_id = client.proxy.create_model(
            model,
            DeRouterParamsBody(model=ANTHROPIC_MODEL, api_key="os.environ/ANTHROPIC_API_KEY"),
        )
        resources.defer(lambda: client.proxy.delete_model(model_id))
        key = resources.key()
        _assert_cache_read_on_second_call(model, lambda prefix: _cache_chat(client, key, model, prefix))
    ...
    def test_openai_prompt_caching_reads_cache(
        ...
        model_id = client.proxy.create_model(
            model,
            DeRouterParamsBody(model=OPENAI_MODEL, api_key="os.environ/OPENAI_API_KEY"),
        )
        resources.defer(lambda: client.proxy.delete_model(model_id))
        key = resources.key()
