...
interval, and the DeRouter proxy does not answer them, so the connection is closed
with a 1011 before the run completes. ``DeRouterRealtimeLLMService`` carries the
...
class DeRouterRealtimeLLMService(OpenAIRealtimeLLMService):
    """Minimal DeRouter-aware realtime service for tests.

    Three overrides carried from bot.py:
      1. _connect   - disables websockets keepalive pings (DeRouter proxy
      ...
      3. _handle_evt_session_created - immediately marks the session ready
                      ...
                      DeRouter Gemini bridge does not send one).
    """
