...
the x-derouter-call-id header on gemini, the `msg_...` message id on anthropic.
...
@pytest.mark.skip(reason="stage red: product gap, native passthrough returns no x-derouter-response-cost or x-ratelimit-* headers")
def test_gemini_passthrough_returns_the_same_header_contract_as_the_managed_route(
    ...
    /chat/completions: x-derouter-response-cost so the call reconciles against
    ...
    assert result.call_id, "passthrough must stamp x-derouter-call-id"
    assert result.response_cost is not None, (
        "passthrough generateContent returned no x-derouter-response-cost header, so a "
        "native call cannot be reconciled against spend the way /chat/completions can"
    )
    assert result.response_cost > 0, (
        f"x-derouter-response-cost must be a real cost, got {result.response_cost}"
    )
...
class TestOpenAIPassthroughSpend:
    ...
    def test_embeddings_call_logs_its_cost(
        ...
        require_successful_call(result)
        assert result.call_id, "embeddings passthrough returned no x-derouter-call-id"
