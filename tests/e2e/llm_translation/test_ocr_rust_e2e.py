...
from models import DeRouterParamsBody, OcrBody, OcrDocument, OcrResponse
...
TEST_PDF_URL = (
    "https://cdn.jsdelivr.net/gh/Delify-Solutions/derouter"
...
class OcrProvider(Protocol):
    ...
    def derouter_params(self) -> DeRouterParamsBody: ...
...
class MistralOcr:
    ...
    def derouter_params(self) -> DeRouterParamsBody:
        return DeRouterParamsBody(model=self.model, api_key="os.environ/MISTRAL_API_KEY")
...
class AzureAiOcr:
    ...
    to Azure verbatim), so we omit them and let derouter read the env vars by name."""

    model: str

    def derouter_params(self) -> DeRouterParamsBody:
        return DeRouterParamsBody(model=self.model)
...
class VertexOcr:
    ...
    vertex_project verbatim from derouter_params and never unwraps an `os.environ/*`
    ...
    def derouter_params(self) -> DeRouterParamsBody:
        return DeRouterParamsBody(model=self.model, vertex_location=self.location)
...
class TestRustOcrGateway:
    ...
    def test_rust_ocr_response(
        ...
        model_id = endpoints_client.create_model(model, case.provider.derouter_params())
    ...
    def test_missing_document_returns_error(
        ...
        model_id = endpoints_client.create_model(model, MistralOcr().derouter_params())
        resources.defer(lambda: endpoints_client.delete_model(model_id))
        key = resources.key()
