...
so the request never reached DeRouter and no spend was recorded; the bypass, not
...
its derouter virtual key in x-derouter-api-key and no upstream bearer, and the proxy
...
model, spend > 0), correlated by the x-derouter-call-id header.
"""
...
class _ModelNewBody(BaseModel):
    model_name: str
    derouter_params: _VertexDeploymentParams
...
def _costed_row(client: PassthroughClient, call_id: str | None) -> SpendLogRow:
    ...
    a billed Vertex call that DeRouter did not track is the exact regression #31689
    guards against."""
    assert call_id, "vertex passthrough response had no x-derouter-call-id header"
