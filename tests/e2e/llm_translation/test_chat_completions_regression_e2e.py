...
def _bedrock_params() -> DeRouterParamsBody:
...
class TestCohereChat:
    ...
    def test_cohere_chat_returns_content(
        ...
        model_id = client.proxy.create_model(
            model,
            DeRouterParamsBody(model=COHERE_BACKEND, api_key=cohere_key),
        )
...
class TestGeminiChatCompletions:
    ...
    def test_gemini_chat_returns_content_and_logs_cost(
        ...
        model_id = client.proxy.create_model(
            model,
            DeRouterParamsBody(model=GEMINI_BACKEND, api_key="os.environ/GEMINI_API_KEY"),
        )
        resources.defer(lambda: client.proxy.delete_model(model_id))
        key = resources.key()
...
class TestOpenAIChatCompletions:
    ...
    def test_openai_chat_streams_real_content(
        ...
        model_id = client.proxy.create_model(
            model, DeRouterParamsBody(model=OPENAI_BACKEND, api_key="os.environ/OPENAI_API_KEY")
        )
        resources.defer(lambda: client.proxy.delete_model(model_id))
        key = resources.key()
            ...
            client.proxy.chat(
                ...
                ChatBody(
                    ...
                    messages=[
                        ...
                        # What is under test is that derouter surfaces the field, so the
    ...
    def test_openai_chat_vision_describes_image(
        ...
        model_id = client.proxy.create_model(
            model, DeRouterParamsBody(model=OPENAI_VISION_BACKEND, api_key="os.environ/OPENAI_API_KEY")
        )
        resources.defer(lambda: client.proxy.delete_model(model_id))
        key = resources.key()
...
class TestAnthropicChatCompletions:
    ...
    def _register(self, client: PassthroughClient, resources: ResourceManager, prefix: str) -> str:
        model = f"{prefix}-{unique_marker()}"
        model_id = client.proxy.create_model(
            model, DeRouterParamsBody(model=ANTHROPIC_BACKEND, api_key="os.environ/ANTHROPIC_API_KEY")
        )
        resources.defer(lambda: client.proxy.delete_model(model_id))
        return model
