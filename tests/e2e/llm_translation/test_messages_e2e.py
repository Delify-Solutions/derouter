...
derouter-regression-tests/tests/test_inference_endpoints.py.
"""
...
def _anthropic_params() -> DeRouterParamsBody:
    """The Anthropic deployment, wired through the record/replay edge when a fixture
    mode is active (LIT-5974). The mount base carries no ``/v1``: derouter's Anthropic
...
class TestAnthropicMessages:
    def _register(
        ...
        params: DeRouterParamsBody | None = None,
    ) -> tuple[str, str]:
    ...
    def test_messages_logs_cost_matching_the_response_header(
        ...
        assert header_cost is not None and header_cost > 0, (
            "x-derouter-response-cost header missing or non-positive on /v1/messages; "
        ...
        assert row.spend is not None and _approx_equal(row.spend, header_cost), (
            f"logged spend {row.spend} disagrees with the x-derouter-response-cost header {header_cost}; "
