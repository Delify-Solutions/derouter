...
class TestCustomPricing:
    ...
    def test_model_info_reports_custom_pricing(
        ...
        assert entry.derouter_params.input_cost_per_token == CUSTOM_INPUT_RATE, (
            f"/model/info derouter_params input rate "
            f"{entry.derouter_params.input_cost_per_token} != configured {CUSTOM_INPUT_RATE}"
        )
        assert entry.derouter_params.output_cost_per_token == CUSTOM_OUTPUT_RATE, (
            f"/model/info derouter_params output rate "
            f"{entry.derouter_params.output_cost_per_token} != configured {CUSTOM_OUTPUT_RATE}"
        )
