...
class ContentFilterMcpParams(BaseModel):
    """derouter_content_filter params scoped to the MCP tool-call hook. mode is
    ...
    guardrail: str = "derouter_content_filter"
...
class GuardrailSpecBody(BaseModel):
    guardrail_name: str
    derouter_params: ContentFilterMcpParams
...
class McpClient:
    ...
    def list_tools(self, key: str) -> Result[McpToolsListResponse]:
        return self.proxy.transport.get(
            "/mcp-rest/tools/list",
            headers=ApiKeyHeaders(x_derouter_api_key=key),
            params=NoBody(),
