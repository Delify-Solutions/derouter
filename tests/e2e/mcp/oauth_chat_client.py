...
class _HeaderInjectingTransport(httpx.AsyncBaseTransport):
    """Adds the caller's DeRouter key header to every outgoing SDK request
    ...
    like a production MCP host configured with a DeRouter key header."""
...
class ChatMcpClient:
    ...
    def seed_user_token(self, alias: str, key: str, storage_state_path: str) -> tuple[str, ...]:
        ...
        just-created server and key propagate asynchronously. The DeRouter key
        rides x-derouter-api-key so the gateway binds the token to that user.
        Returns the upstream tool names the dance listed, proof the token works."""
        headers = {"x-derouter-api-key": f"Bearer {key}"}
    ...
    def chat_with_mcp(self, headers: AuthHeaders, body: ChatBody) -> ChatResponse:
        """POST /chat/completions carrying the DeRouter key in `headers` (either
