...
MCP attached. The gateway resolves the user from the DeRouter key, lists Linear's
...
documented ingress header (x-derouter-api-key and Authorization).
...
class TestMcpChatCompletionOauth:
    ...
    def test_chat_completion_uses_linear_with_x_derouter_api_key_header(
        ...
        response = chat_client.chat_with_mcp(
            AuthHeaders.model_validate({"x-derouter-api-key": f"Bearer {key}"}),
            ChatBody(
                model=CHEAP_ANTHROPIC_MODEL,
                messages=[ChatMessage(role="user", content=LINEAR_PROMPT)],
                tools=[
                    McpChatTool(
                        server_url=f"derouter_proxy/mcp/{alias}",
                        server_label=alias,
