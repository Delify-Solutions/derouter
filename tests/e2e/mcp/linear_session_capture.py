...
to the terminal and press Enter:
    ...
    DEROUTER=~/derouter-mcpe2e
    "$DEROUTER"/.venv/bin/python "$DEROUTER"/tests/e2e/mcp/linear_session_capture.py

The session is written to ``E2E_LINEAR_STORAGE_STATE`` (default
``~/.derouter-e2e/linear_storage_state.json``), outside the repo. It is a
...
DEFAULT_STATE_PATH = Path.home() / ".derouter-e2e" / "linear_storage_state.json"
