...
class Headers(BaseModel):
    ...
    (e.g. ``x-derouter-api-key``); serialization uses by_alias."""

    model_config = ConfigDict(populate_by_name=True)


class AuthHeaders(Headers):
    # derouter accepts either; set whichever the call needs, leave the other None.
    authorization: str | None = None
    x_derouter_api_key: str | None = Field(default=None, alias="x-derouter-api-key")
...
class UnauthorizedError(BaseModel):
    ...
    # derouter 401s for key auth, model access, and tag routing alike, so keep the body to tell them apart.
    body: str = ""
...
class RateLimitedError(BaseModel):
    ...
    # derouter overloads 429 for budget_exceeded too, so keep the body to tell them apart.
    body: str = ""
...
class StreamingResponse(BaseModel):
    ...
    x-derouter-call-id header, the x-derouter-response-cost header (StandardLogging
    ...
    status_code: int
    call_id: str | None = None  # x-derouter-call-id header
    response_cost: float | None = None  # x-derouter-response-cost header
...
def request_with_retry[T: RetryableResponse](
    ...
    status derouter provably never originates, with an observed flake in hand.
...
def _parse_response_cost(resp: SseResponse) -> float | None:
    raw = _hdr(resp, "x-derouter-response-cost")
...
def streaming_outcome(
    ...
    call_id: Final = _hdr(resp, "x-derouter-call-id")
...
def send(
    ...
    *,
    ...
    x-derouter-call-id header. For native/passthrough bodies and for calls judged by
...
def stream(
    ...
    x-derouter-call-id + content-type headers. Body is elided."""
...
def stream_binary(
    ...
    *,
    ...
    with resp:
        ...
        call_id = _hdr(resp, "x-derouter-call-id")
...
def download(
    ...
    schema. Returns the decoded body and the x-derouter-call-id header."""
    try:
