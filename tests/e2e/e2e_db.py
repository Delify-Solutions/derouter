...
reset_spend_logs truncates DeRouter_SpendLogs and cannot be undone, so the
...
def reset_spend_logs() -> None:
    """Truncate DeRouter_SpendLogs for a clean slate. No proxy endpoint deletes
    ...
    url = os.environ.get(
        "DATABASE_URL",
        "postgresql://llmproxy:dbpassword9090@localhost:5432/derouter",
    )
    with psycopg.connect(url) as conn:
        _ = conn.execute('TRUNCATE TABLE "DeRouter_SpendLogs"')
