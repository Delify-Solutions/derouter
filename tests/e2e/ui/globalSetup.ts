...
async function globalSetup() {
  ...
  // The proxy runs with DEROUTER_LICENSE in CI, so enable it the same way
  // the admin UI toggle does; the projects migration smoke needs the link.
  const masterKey = process.env.DEROUTER_MASTER_KEY || "sk-1234";
  ...
  for (const role of Object.values(Role)) {
    ...
    try {
      ...
      // The login flow stores a post-login return URL in the derouter_return_url
      ...
      await page.context().clearCookies({ name: "derouter_return_url" });
