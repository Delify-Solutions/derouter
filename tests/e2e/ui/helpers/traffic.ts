...
export const masterKey = (): string => process.env.DEROUTER_MASTER_KEY || "sk-1234";
...
interface ChatOptions {
  ...
  endUser?: string;
  /** Sent as `derouter_trace_id`, which lands in the spend log's session_id column. */
  traceId?: string;
}
...
export async function sendChatCompletion(request: APIRequestContext, opts: ChatOptions): Promise<string> {
  const res = await request.post(`${rootPath()}/v1/chat/completions`, {
    ...
    data: {
      ...
      ...(opts.traceId ? { derouter_trace_id: opts.traceId } : {}),
    },
  });
