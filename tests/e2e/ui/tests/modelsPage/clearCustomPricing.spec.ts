 ...
 * the override from both `derouter_params` and `model_info`.
 *
 * Pre-fix, the UI sent the old pricing back on every save (the spread of
 * `values.derouter_params` re-injected it), and the backend's `exclude_none=True`
...
test.describe("Clear custom pricing on a deployment", () => {
  ...
  test("UI sends null for cleared pricing and backend removes the override", async ({ page }) => {
    ...
    expect(patchBody.derouter_params.input_cost_per_token, "UI sends explicit null for cleared input cost").toBeNull();
    expect(patchBody.derouter_params.output_cost_per_token, "UI sends explicit null for cleared output cost").toBeNull();
    expect(
      patchBody.derouter_params.cache_read_input_token_cost,
      "UI sends explicit null for cleared cache_read cost",
    ).toBeNull();
    expect(
      patchBody.derouter_params.cache_creation_input_token_cost,
    ...
    expect("input_cost_per_token" in row.derouter_params, "derouter_params.input_cost_per_token key removed").toBe(false);
    expect("output_cost_per_token" in row.derouter_params, "derouter_params.output_cost_per_token key removed").toBe(
      false,
    );
    expect(
      "cache_read_input_token_cost" in row.derouter_params,
      "derouter_params.cache_read_input_token_cost key removed",
    ).toBe(false);
    expect(
      "cache_creation_input_token_cost" in row.derouter_params,
      "derouter_params.cache_creation_input_token_cost key removed",
    ).toBe(false);
    expect(
