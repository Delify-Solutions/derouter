...
/** GET /model/info?derouter_model_id= returns {data: [row]}, the deployment as stored. */
async function readDeployment(page: PlaywrightPage, modelId: string): Promise<Record<string, any> | undefined> {
  const body = await readBack<{ data: Record<string, any>[] }>(page, `/model/info?derouter_model_id=${modelId}`);
  return body.data[0];
}
...
test.describe("Add Model", () => {
  ...
  test("Edit team model TPM and RPM limits", async ({ page }) => {
    ...
    // handleModelUpdate PATCHes the whole derouter_params blob, so pin what goes on the wire.
    ...
    expect(Number(patch.derouter_params?.tpm), "new TPM on the wire").toBe(999);
    expect(Number(patch.derouter_params?.rpm), "new RPM on the wire").toBe(888);
    ...
    await expect
      .poll(
        async () => {
          ...
          return [Number(stored?.derouter_params?.tpm), Number(stored?.derouter_params?.rpm)];
        },
    ...
    expect(after?.derouter_params?.model, "upstream model untouched by a limits edit").toBe("openai/fake-gpt-4");
  ...
  test("Add a model through the UI, pass Test Connect, and serve traffic with it", async ({ page, request }) => {
    ...
    expect(created.derouter_params?.api_base, "the api base survives the form").toBe(MOCK_LLM_BASE);
  ...
  test("Add a model with a stored credential, pass Test Connect, and serve traffic", async ({ page, request }) => {
          ...
          const probe = await page.request.post("/health/test_connection", {
            ...
            data: {
              derouter_params: {
                ...
                derouter_credential_name: credentialName,
              },
              model_info: {},
              mode: "chat",
            },
    ...
    try {
      ...
      expect(created.derouter_params?.derouter_credential_name, "the picked credential goes on the wire").toBe(
        credentialName,
      );
      expect(created.derouter_params?.api_key, "no raw api key goes on the wire").toBeUndefined();
  ...
  test("Add specific model and verify it appears in All Models", async ({ page }) => {
    ...
    expect(created.derouter_params?.model, "the model name goes on the wire").toBe("claude-haiku-4-5");
    expect(created.derouter_params?.custom_llm_provider, "the picked provider goes on the wire").toBe("anthropic");
    ...
    expect(stored?.derouter_params?.model, "stored deployment keeps the model name").toBe("claude-haiku-4-5");
    expect(stored?.derouter_params?.custom_llm_provider, "stored deployment keeps its provider").toBe("anthropic");
  });
  ...
  test("Add team-only model via Team-BYOK toggle and verify it appears with the team", async ({ page, request }) => {
    ...
    test.skip(!process.env.DEROUTER_LICENSE, "DEROUTER_LICENSE not set in test env — Team-BYOK switch is disabled");
  ...
  test("Add wildcard route and verify it appears in All Models", async ({ page }) => {
    ...
    expect(stored?.derouter_params?.model, "stored deployment keeps the wildcard route").toBe("cohere/*");
  });
});
