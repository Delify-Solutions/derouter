...
interface TeamInfoResponse {
  ...
  team_memberships: {
    user_id: string;
    derouter_budget_table: { max_budget: number | null } | null;
  }[];
}
...
function budgetOf(info: TeamInfoResponse, userId: string): number | null | undefined {
  return info.team_memberships.find((membership) => membership.user_id === userId)?.derouter_budget_table?.max_budget;
}
