...
/**
 * Sanity check that DEROUTER_LICENSE is being forwarded to the proxy when set
...
test.describe("Premium license wiring", () => {
  test("admin session JWT carries premium_user=true when DEROUTER_LICENSE is set", () => {
    test.skip(!process.env.DEROUTER_LICENSE, "DEROUTER_LICENSE not set in test env — proxy is running unlicensed");
