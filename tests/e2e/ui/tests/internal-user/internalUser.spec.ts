...
test.describe("Internal User", () => {
  ...
  test("Virtual Keys page does not surface derouter-dashboard team keys", async ({ page }) => {
    await navigateToPage(page, Page.ApiKeys);
    ...
    // The derouter-dashboard team is the proxy's internal bookkeeping team —
    // its keys must never leak into an internal user's Virtual Keys table.
    await expect(page.getByRole("row").filter({ hasText: "derouter-dashboard" })).toHaveCount(0);
  });
});
