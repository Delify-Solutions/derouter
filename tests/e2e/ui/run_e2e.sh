...
DASHBOARD_DIR="$REPO_ROOT/ui/derouter-dashboard"
IS_CI="${CI:-false}"
CONTAINER_NAME="derouter-e2e-postgres-$$"
MOCK_PID=""
MOCK_PRESIDIO_PID=""
PROXY_PID=""
PROXY_LOG=""
...
if [ "$IS_CI" = "false" ]; then
  ...
  #
  # So select a Node that satisfies ui/derouter-dashboard's engines.node, and if
  ...
  if [ -s "$HOME/.nvm/nvm.sh" ]; then
    ...
    if [ -n "$required_major" ]; then
      ...
      if [ -z "$current_major" ] || [ "$current_major" -lt "$required_major" ]; then
        ...
        current_major="$(node --version 2>/dev/null | sed -E 's/^v([0-9]+).*/\1/')"
        if [ -z "$current_major" ] || [ "$current_major" -lt "$required_major" ]; then
          echo "Error: ui/derouter-dashboard requires Node >= v${required_major}, and no such version is installed."
...
if [ "$IS_CI" = "false" ]; then
  ...
  export POSTGRES_DB="derouter_e2e"
...
# --- Credentials ---
export DEROUTER_MASTER_KEY="sk-1234"
...
# Forward DEROUTER_LICENSE if set in the outer env so premium-gated UI flows
# (e.g. Team-BYOK Model switch) can be exercised. Tests that depend on a
# premium proxy gate themselves on process.env.DEROUTER_LICENSE.
export DEROUTER_LICENSE="${DEROUTER_LICENSE:-}"
...
cp -r "$DASHBOARD_DIR/out/" "$REPO_ROOT/derouter/proxy/_experimental/out/"
...
find "$REPO_ROOT/derouter/proxy/_experimental/out" -name '*.html' ! -name 'index.html' | while read -r htmlfile; do
...
uv run --no-sync python -m prisma generate --schema derouter/proxy/schema.prisma

echo "=== Pushing Prisma schema to database ==="
uv run --no-sync python -m prisma db push --schema derouter/proxy/schema.prisma --accept-data-loss
...
# --- DeRouter proxy ---
echo "=== Starting DeRouter proxy ==="
cd "$REPO_ROOT"
PROXY_LOG="${TMPDIR:-/tmp}/derouter-e2e-proxy-$$.log"
uv run --no-sync python -m derouter.proxy.proxy_cli \
...
for i in $(seq 1 180); do
  ...
  HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:${PROXY_PORT}/health -H "Authorization: Bearer $DEROUTER_MASTER_KEY" 2>/dev/null || true)
