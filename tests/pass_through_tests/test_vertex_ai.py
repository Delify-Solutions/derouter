...
async def test_basic_vertex_ai_pass_through_with_spendlog():
    ...
    # so the call never reaches DeRouter and no spend is logged. A direct request always
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        call_id = response.headers.get("x-derouter-call-id")
        assert call_id, "proxy response missing x-derouter-call-id header"
