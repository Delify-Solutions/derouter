...
from derouter.proxy.guardrails.guardrail_hooks.zscaler_ai_guard import ZscalerAIGuard
import asyncio
...
@pytest.mark.asyncio
@patch(
    "derouter.proxy.guardrails.guardrail_hooks.zscaler_ai_guard.ZscalerAIGuard.make_zscaler_ai_guard_api_call",
...
@pytest.mark.asyncio
async def test_should_resolve_from_derouter_metadata_during_post_call():
    """Test that user_api_key_alias is resolved from derouter_metadata during post-call"""
    request_data = {"derouter_metadata": {"user_api_key_alias": "test-alias-post-call"}}
...
async def test_should_resolve_user_api_key_key_alias_mapping():
    """Test key_alias -> user_api_key_key_alias mapping in derouter_metadata"""
    request_data = {"derouter_metadata": {"user_api_key_key_alias": "test-key-alias"}}
    result = ZscalerAIGuard._resolve_metadata_value(request_data, "user_api_key_alias")
...
async def test_apply_guardrail_block_does_not_log_error(mock_api_call):
    ...
    with patch(
        "derouter.proxy.guardrails.guardrail_hooks.zscaler_ai_guard.zscaler_ai_guard.verbose_proxy_logger"
    ) as mock_logger:
        with pytest.raises(HTTPException) as exc_info:
            await guardrail.apply_guardrail(inputs, request_data, "request")
...
async def test_send_request_uses_default_timeout_when_unconfigured():
    ...
    with patch(
        "derouter.proxy.guardrails.guardrail_hooks.zscaler_ai_guard.zscaler_ai_guard.get_async_httpx_client"
    ) as mock_get_client:
        mock_client = Mock()
...
def test_initialize_guardrail_forwards_configured_timeout():
    ...
    """
    from derouter.proxy.guardrails.guardrail_hooks.zscaler_ai_guard import (
    ...
    guardrail = initialize_guardrail(
        derouter_params, {"guardrail_name": "zscaler-configured-timeout"}
    )
...
def test_update_in_memory_derouter_params_keeps_timeout_resolved():
    """
