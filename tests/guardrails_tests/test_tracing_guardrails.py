...
async def test_standard_logging_payload_includes_guardrail_information():
    ...
    # 2. call derouter.acompletion
    response = await derouter.acompletion(**request_data)
...
async def test_bedrock_guardrail_status_blocked():
    ...
    with patch.object(
        ...
        # Call derouter.acompletion to trigger logging callbacks
...
async def test_bedrock_guardrail_status_success():
    ...
    with patch.object(
        ...
        # Call derouter.acompletion to trigger logging
        response = await derouter.acompletion(**request_data)
        await asyncio.sleep(1)
...
def test_guardrail_status_fields_computation():
    ...
    """
    from derouter.derouter_core_utils.derouter_logging import _get_status_fields
