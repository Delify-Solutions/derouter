...
from derouter.proxy.guardrails.guardrail_hooks.akto.akto import AktoGuardrail
...
def test_resolve_metadata_value():
    ...
    assert (
        AktoGuardrail.resolve_metadata_value(
            {"derouter_metadata": {"user_api_key_team_id": "t1"}},
...
def test_resolve_metadata_value_non_dict_containers():
    assert (
        AktoGuardrail.resolve_metadata_value(
            {"metadata": "invalid", "derouter_metadata": ["bad"]},
            "some_key",
        )
        is None
    )
