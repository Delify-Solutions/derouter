import os
import pytest
from derouter import mock_completion
...
from derouter.types.guardrails import PiiEntityType, PiiAction
...
async def test_output_parsing():
    ...
    derouter.output_parse_pii = True
    pii_masking = _OPTIONAL_PresidioPIIMasking(mock_testing=True)
...
async def test_presidio_pii_masking_logging_output_only_logged_response_guardrails_config():
    ...
    import derouter
    from derouter.proxy.guardrails.init_guardrails import initialize_guardrails
    ...
    derouter_settings = {"guardrails": guardrails_config}

    assert len(derouter.guardrail_name_config_map) == 0
    ...
    assert len(derouter.guardrail_name_config_map) == 1
