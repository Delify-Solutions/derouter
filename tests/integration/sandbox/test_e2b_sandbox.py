...
tests/test_derouter/sandbox/test_e2b_sandbox.py.
...
async def test_integration_ephemeral_real_e2b():
    result = await derouter.acode_interpreter_tool(
...
async def test_integration_lifecycle_roundtrip_real_e2b():
    container = await derouter.acreate_sandbox(provider="e2b")
    try:
        ...
        assert await derouter.adelete_sandbox(provider="e2b", container=container)
