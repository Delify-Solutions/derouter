...
async def test_async_streaming(m: _M, oci_params):
    ...
    chunks = []
    async for chunk in await derouter.acompletion(
        model=f"oci/{m.model}",
        messages=[{"role": "user", "content": "Count to 3."}],
        max_tokens=m.max_tokens,
        stream=True,
...
def test_tool_use(m: _M, oci_params):
    ...
    resp = derouter.completion(**call_kwargs)
...
async def test_async_tool_use(m: _M, oci_params):
    ...
    resp = await derouter.acompletion(**call_kwargs)
    _assert_tool_call(resp)
...
def test_reasoning_tokens_in_usage(oci_params):
    ...
    DeRouter should surface it on Usage.completion_tokens_details."""
    import derouter
