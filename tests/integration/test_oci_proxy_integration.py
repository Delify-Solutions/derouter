...
Spins up the DeRouter proxy (`derouter --config oci_proxy_test_config.yaml`) as a
...
def _wait_for_health(base_url: str, proc: subprocess.Popen, deadline: float) -> None:
    ...
    while time.monotonic() < deadline:
        if proc.poll() is not None:
            ...
            raise RuntimeError(
                f"derouter proxy exited early with code {proc.returncode}\n--- proxy output ---\n{output}"
            )
        try:
        ...
        time.sleep(0.5)
    raise RuntimeError(
        f"derouter proxy did not become ready within {STARTUP_TIMEOUT_S}s"
    )


def _oci_env_from_profile() -> dict[str, str]:
    """Translate the active OCI profile into the OCI_* env vars the derouter
    ...
    if "security_token_file" in cfg:
        pytest.skip(
            ...
            "derouter's OCI provider needs an API-key profile for env-driven config"
        )
...
def _serve(config_path: str) -> Iterator[str]:
    """Boot the derouter proxy with the given config and yield its base URL."""
    env = os.environ.copy()
    ...
    # Prefer the `derouter` console script that lives next to the active
    # Python so we inherit the test virtualenv. Fall back to PATH.
    cli = Path(sys.executable).parent / "derouter"
    if not cli.exists():
        cli = "derouter"
...
def test_embedding_via_proxy(proxy_url: str) -> None:
    ...
    r = httpx.post(
        ...
        headers=_auth_headers(),
        json={"model": "oci-embed", "input": ["hello from the derouter proxy"]},
        timeout=REQUEST_TIMEOUT_S,
    )
