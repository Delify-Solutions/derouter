...
from derouter.constants import STREAM_SSE_DONE_STRING
...
class TestBaseResponsesAPIStreamingIterator:
    ...
    async def test_responses_streaming_iterator_parses_u2028_in_sse_json(self):
        ...
        iterator = ResponsesAPIStreamingIterator(
            ...
            derouter_metadata={"model_info": {"id": "model_123"}},
        ...
        with (
            ...
            patch("derouter.responses.streaming_iterator.executor"),
        ):
    ...
    def test_handle_logging_completed_response_with_unpickleable_objects(self):
        ...
        with (
            ...
            patch("derouter.responses.streaming_iterator.executor") as mock_executor,
        ):
            try:
    ...
    def test_stop_iteration_not_logged_as_failure(self):
        ...
        Regression test for: https://github.com/Delify-Solutions/derouter/issues/XXXXX
        """
    ...
    def test_process_chunk_response_failed_calls_failure_handler(self):
        ...
        with (
            ...
            patch(
                "derouter.responses.streaming_iterator.run_async_function"
            ) as mock_run_async,
            patch("derouter.responses.streaming_iterator.executor") as mock_executor,
        ):
            result = iterator._process_chunk(json.dumps(test_chunk_data))
