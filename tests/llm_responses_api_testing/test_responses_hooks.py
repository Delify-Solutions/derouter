...
from derouter.responses import streaming_iterator as streaming_module
...
async def test_responses_streaming_triggers_hooks(monkeypatch):
    ...
    iterator = ResponsesAPIStreamingIterator(
        ...
        request_data={"foo": "bar", "derouter_params": {}},
    ...
    assert seen["request_data"].get("derouter_params") is not None
...
async def test_responses_streaming_calls_post_streaming_deployment_hook(monkeypatch):
    ...
    derouter.callbacks = [_HookLogger()]
...
def test_process_chunk_wraps_encrypted_content_with_model_id():
    ...
    class _EncryptedConfig:
        def transform_streaming_response(self, **kwargs):
            return openai_types.OutputItemAddedEvent(
                ...
                output_index=0,
                item=openai_types.BaseDeRouterOpenAIResponseObject(
                    id="rs_123",
                    type="reasoning",
    ...
    assert event.item.encrypted_content.startswith("derouter_enc:")
...
def test_process_chunk_completed_response_updates_id_and_usage_cost(monkeypatch):
    original_include_cost = derouter.include_cost_in_streaming_usage
    ...
    iterator = ResponsesAPIStreamingIterator(
        ...
        logging_obj=logging_obj,
        derouter_metadata={"model_info": {"id": "model-123"}},
    ...
    try:
        ...
        derouter.include_cost_in_streaming_usage = original_include_cost
...
async def test_responses_streaming_completed_event_persists_async_cache():
    ...
    derouter.cache = SimpleNamespace(
    ...
    caching_handler = SimpleNamespace(
        ...
        original_function=derouter.aresponses,
    ...
    derouter.cache.async_add_cache.assert_called_once()
    assert derouter.cache.async_add_cache.call_args.kwargs["stream"] is True
    assert (
        derouter.cache.async_add_cache.call_args.kwargs["cache_key"]
        == "responses-stream-cache-key"
    )
    assert "metadata" not in derouter.cache.async_add_cache.call_args.kwargs
    assert "custom_llm_provider" not in derouter.cache.async_add_cache.call_args.kwargs
    assert (
        json.loads(derouter.cache.async_add_cache.call_args.args[0])["id"]
...
def test_responses_streaming_completed_event_persists_sync_cache():
    ...
    caching_handler = SimpleNamespace(
        ...
        preset_cache_key="responses-stream-cache-key",
        original_function=derouter.responses,
    ...
    derouter.cache.add_cache.assert_called_once()
    assert derouter.cache.add_cache.call_args.kwargs["stream"] is True
    assert (
        derouter.cache.add_cache.call_args.kwargs["cache_key"]
        == "responses-stream-cache-key"
    )
    assert "metadata" not in derouter.cache.add_cache.call_args.kwargs
    assert "custom_llm_provider" not in derouter.cache.add_cache.call_args.kwargs
    assert (
        json.loads(derouter.cache.add_cache.call_args.args[0])["id"]
        == iterator.completed_response.response.id
    )
    derouter.cache = original_cache
...
def test_persist_completed_response_to_cache_guard_branches(monkeypatch, scenario):
    ...
    if scenario == "already_cached":
        ...
        if scenario == "missing_cache_backend":
            monkeypatch.setattr(streaming_module.derouter, "cache", None)
        else:
            monkeypatch.setattr(
                streaming_module.derouter,
                "cache",
...
async def test_cached_responses_stream_async_hit_triggers_success_callbacks(
    ...
    assert hook_calls["post_call"] == 1
    assert hook_calls["metadata"] == 1
    derouter.cache.async_add_cache.assert_not_called()
    derouter.cache.add_cache.assert_not_called()
    derouter.cache = original_cache
