...
def validate_standard_logging_payload(
    ...
    Args:
        ...
        response (dict): The derouter response to compare against
...
def test_basic_openai_responses_api_streaming_with_logging():
    ...
    derouter.callbacks = [test_custom_logger]
    request_model = "gpt-5.5"
    response = derouter.responses(
        model=request_model,
        input="hi",
        stream=True,
    )
    ...
    for event in response:
        ...
        print("derouter response=", json.dumps(event, indent=4, default=str))
...
def validate_responses_match(slp_response, derouter_response):
    """Validate that the standard logging payload OpenAI response matches the derouter response"""
    # Validate core fields
    assert slp_response["id"] == derouter_response["id"], "ID mismatch"
    assert slp_response["model"] == derouter_response["model"], "Model mismatch"
    assert (
        slp_response["created_at"] == derouter_response["created_at"]
    ), "Created at mismatch"

    # Validate usage
    assert (
        slp_response["usage"]["prompt_tokens"]
        == derouter_response["usage"]["input_tokens"]
    ), "Input tokens mismatch"
    assert (
        slp_response["usage"]["completion_tokens"]
        == derouter_response["usage"]["output_tokens"]
    ), "Output tokens mismatch"
    assert (
        slp_response["usage"]["total_tokens"]
        == derouter_response["usage"]["total_tokens"]
    ), "Total tokens mismatch"

    # Validate output/messages
    assert len(slp_response["output"]) == len(
        derouter_response["output"]
    ), "Output length mismatch"
    for slp_msg, derouter_msg in zip(slp_response["output"], derouter_response["output"]):
        assert slp_msg["role"] == derouter_msg.role, "Message role mismatch"
        # Access the content's text field for the derouter response
        derouter_content = derouter_msg.content[0].text if derouter_msg.content else ""
        assert (
            slp_msg["content"][0]["text"] == derouter_content
        ), f"Message content mismatch. Expected {derouter_content}, Got {slp_msg['content']}"
        assert slp_msg["status"] == derouter_msg.status, "Message status mismatch"
...
async def test_basic_openai_responses_api_non_streaming_with_logging():
    ...
    print("derouter response=", json.dumps(response, indent=4, default=str))
...
async def test_openai_responses_api_returns_headers(sync_mode):
    ...
    Related issue: DeRouter responses API should return OpenAI headers like chat completions does
    """
    derouter._turn_on_debug()
    derouter.set_verbose = True
...
def validate_stream_event(event):
    """
    Validate that a streaming event from derouter.responses() or derouter.aresponses()
...
@pytest.mark.parametrize("sync_mode", [True, False])
@pytest.mark.asyncio
async def test_openai_responses_derouter_router(sync_mode):
    """
    Test the OpenAI responses API with DeRouter Router in both sync and async modes
    """
    derouter._turn_on_debug()
...
@pytest.mark.parametrize("sync_mode", [True, False])
@pytest.mark.asyncio
async def test_openai_responses_derouter_router_streaming(sync_mode):
    """
    Test the OpenAI responses API with streaming through DeRouter Router
    """
    derouter._turn_on_debug()
    router = derouter.Router(
        model_list=[
...
@pytest.mark.asyncio
async def test_openai_responses_derouter_router_no_metadata():
    """
...
@pytest.mark.asyncio
async def test_openai_responses_derouter_router_with_metadata():
    """
...
@pytest.mark.asyncio
async def test_openai_responses_derouter_router_with_prompt():
    """Test that prompt object is passed through the Router for responses API"""

    prompt_obj = {
        "id": "pmpt_abc123",
        "version": "2",
        "variables": {"random_variable": "ishaan_from_derouter"},
    }
...
def test_bad_request_bad_param_error():
    ...
    try:
        derouter.responses(model="gpt-5.5", input="This should fail", temperature=2000)
        pytest.fail("Expected BadRequestError but no exception was raised")
    except derouter.BadRequestError as e:
...
async def test_openai_o1_pro_response_api(sync_mode):
    """
    Test that DeRouter correctly handles an incomplete response from OpenAI's o1-pro model
    ...
    with patch(
        ...
        if sync_mode:
            ...
            with patch(
                ...
                for event in response:
                    print(
                        f"Sync derouter response #{event_count}:",
            ...
            async for event in response:
                print(
                    f"Async derouter response #{event_count}:",
                    json.dumps(event, indent=4, default=str),
                )
                event_count += 1
...
def test_basic_computer_use_preview_tool_call():
    """
    Test that DeRouter correctly handles a computer_use_preview tool call where the environment is set to "linux"

    linux is an unsupported environment for the computer_use_preview tool, but derouter users should still be able to pass it to openai
    """
...
def test_mcp_tools_with_responses_api():
    ...
    try:
        response = derouter.responses(model=MODEL, tools=MCP_TOOLS, input=USER_QUERY)
        print(response)
        ...
        if mcp_approval_id:
            response_with_mcp_call = derouter.responses(
                model=MODEL,
                tools=MCP_TOOLS,
                input=[
                    {
...
async def test_openai_responses_api_field_types():
    ...
    response_without_store = await derouter.aresponses(model="gpt-5.5", input="hi")
...
async def test_aresponses_service_tier_and_safety_identifier():
    ...
    when using derouter.aresponses.
    """
    mock_response = {
...
async def test_openai_responses_api_token_limit_error():
    """
    Relevant issue: https://github.com/Delify-Solutions/derouter/issues/15785
    ...
    The iterator now surfaces the event as derouter.APIError with status 400
...
async def test_openai_compact_responses_api(sync_mode):
    ...
    try:
        if sync_mode:
            response = derouter.compact_responses(
                model="openai/gpt-5.5",
                input=input_messages,
                instructions="Be helpful and concise",
            )
        else:
            response = await derouter.acompact_responses(
                model="openai/gpt-5.5",
                input=input_messages,
                instructions="Be helpful and concise",
            )
