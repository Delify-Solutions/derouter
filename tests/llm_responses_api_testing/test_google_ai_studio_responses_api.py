...
async def test_mock_basic_google_ai_studio_responses_api_with_tools():
    """
    - Ensure that this is the request that derouter.completion gets when we pass web search options

    derouter.acompletion(messages=[{'role': 'user', 'content': 'what is the latest version of supabase python package and when was it released?'}], model='gemini-2.5-flash', tools=[], web_search_options={'search_context_size': 'low', 'user_location': None})
    """
    ...
    with patch("derouter.acompletion", new_callable=AsyncMock) as mock_acompletion:
        ...
        print(
            "call kwargs to derouter.completion=",
...
async def test_gemini_3_responses_api_streaming_with_thought_signatures():
    ...
    response_stream = await derouter.aresponses(
        model=request_model,
        input="What is the weather in Mumbai?",
        tools=tools,
        stream=True,
