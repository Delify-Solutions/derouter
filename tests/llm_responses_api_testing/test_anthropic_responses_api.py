...
def test_multiturn_tool_calls():
    ...
    follow_up_response = derouter.responses(
        model="anthropic/claude-haiku-4-5-20251001",
...
def test_response_api_handler_merges_metadata_and_service_tier_without_error():
    ...
    handler = DeRouterCompletionTransformationHandler()

    with patch("derouter.completion", new_callable=MagicMock) as mock_completion:
...
async def test_async_response_api_handler_merges_trace_id_without_error():
    ...
    async def fake_session_handler(previous_response_id, derouter_completion_request):
        derouter_completion_request["derouter_trace_id"] = "session-trace"
    ...
    with patch.object(
        ...
        with patch("derouter.acompletion", new_callable=AsyncMock) as mock_acompletion:
            ...
            await handler.async_response_api_handler(
                derouter_completion_request={"model": "test"},
                request_input="hi",
                responses_api_request={"previous_response_id": "123"},
                derouter_trace_id="original-trace",
            )
            # ensure acompletion called once with merged trace_id
            assert mock_acompletion.call_count == 1
            assert (
                mock_acompletion.call_args.kwargs["derouter_trace_id"] == "session-trace"
            )
...
async def test_aresponses_forwards_timeout_to_acompletion():
    ...
    forwarded to `derouter_completion_transformation_handler.response_api_handler`,
