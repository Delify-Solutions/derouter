...
class TestResponsePollingHandler:
    ...
    def test_generate_polling_id_has_correct_prefix(self):
        ...
        assert polling_id.startswith("derouter_poll_")
        assert len(polling_id) > len("derouter_poll_")  # Has UUID after prefix
    ...
    def test_get_cache_key_format(self):
        ...
        polling_id = "derouter_poll_abc123"
        cache_key = ResponsePollingHandler.get_cache_key(polling_id)

        assert cache_key == "derouter:polling:response:derouter_poll_abc123"
    ...
    async def test_create_initial_state_returns_queued_status(self):
        ...
        polling_id = "derouter_poll_test123"
        request_data = {
            "model": "gpt-4o",
            "input": "Hello",
            "metadata": {"test": "value"},
    ...
    async def test_create_initial_state_stores_in_redis(self):
        ...
        assert (
            call_args.kwargs["key"] == "derouter:polling:response:derouter_poll_test123"
        )
    ...
    async def test_get_state_returns_cached_state(self):
        ...
        result = await handler.get_state("derouter_poll_test")
    ...
    async def test_get_state_returns_none_for_missing_state(self):
        ...
        result = await handler.get_state("derouter_poll_nonexistent")
    ...
    async def test_cancel_polling_updates_status_to_cancelled(self):
        ...
        result = await handler.cancel_polling("derouter_poll_test")
    ...
    async def test_delete_polling_removes_from_cache(self):
        ...
        result = await handler.delete_polling("derouter_poll_test")

        assert result is True
        mock_async_client.delete.assert_called_once_with(
            "derouter:polling:response:derouter_poll_test"
        )
...
class TestBackgroundStreamingModule:
    ...
    def test_module_exports_from_init(self):
        ...
        from derouter.proxy.response_polling import (
...
class TestProviderResolutionForPolling:
    ...
    def test_provider_resolution_from_router_single_deployment(self):
        ...
        for idx in indices:
            ...
            derouter_params = deployment_dict.get("derouter_params", {})

            dep_provider = derouter_params.get("custom_llm_provider")
            if not dep_provider:
                dep_model = derouter_params.get("model", "")
                if "/" in dep_model:
    ...
    def test_provider_resolution_model_not_in_router(self):
        ...
        model_list = [
            {"model_name": "gpt-5", "derouter_params": {"model": "openai/gpt-5"}}
        ]
...
class TestPollingConditionChecks:
    ...
    def test_polling_with_router_lookup_no_match(self):
        ...
        mock_router.model_list = [
            {
                "model_name": "claude-3",
                "derouter_params": {"model": "anthropic/claude-3-sonnet"},
            }
        ]
        ...
        mock_router.model_list = [
            {
                ...
                "derouter_params": {"model": "openai/o4-mini-deep-research"},
            }
        ]
...
class TestBackgroundStreamingTerminalEvents:
    ...
    async def test_response_failed_sets_failed_status_and_error(self):
        ...
        with patch(
            "derouter.proxy.response_polling.background_streaming.ProxyBaseLLMRequestProcessing"
        ) as MockProcessor:
...
class TestEdgeCases:
    ...
    def test_polling_id_detection_edge_cases(self):
        ...
        assert ResponsePollingHandler.is_polling_id("derouter_poll_") is True

        # Similar but different prefix
        assert ResponsePollingHandler.is_polling_id("derouter_polling_abc") is False

        # Case sensitivity
        assert ResponsePollingHandler.is_polling_id("DEROUTER_POLL_abc") is False
