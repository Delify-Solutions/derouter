...
from derouter.proxy.image_endpoints import image_generation
...
async def test_call_with_valid_model_using_all_models(prisma_client):
    ...
    """
    # Make a call to a key with model = `all-proxy-models` this is an Alias from DeRouter Admin UI
    setattr(derouter.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(derouter.proxy.proxy_server, "master_key", "sk-1234")
    try:
        await derouter.proxy.proxy_server.prisma_client.connect()
...
def test_call_with_user_over_budget(prisma_client):
    ...
    async def test():
        ...
        from derouter import Choices, Message, ModelResponse, Usage
        from derouter.proxy.proxy_server import _ProxyDBLogger
...
def test_call_with_end_user_over_budget(prisma_client):
    ...
    # we only check this when derouter.max_end_user_budget is set
    import random

    setattr(derouter.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(derouter.proxy.proxy_server, "master_key", "sk-1234")
    setattr(derouter, "max_end_user_budget", 0.00001)
...
def test_call_with_proxy_over_budget(prisma_client):
    ...
    setattr(derouter.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(derouter.proxy.proxy_server, "master_key", "sk-1234")
    derouter_proxy_budget_name = f"derouter-proxy-budget-{time.time()}"
    setattr(
        derouter.proxy.proxy_server,
        "derouter_proxy_admin_name",
        derouter_proxy_budget_name,
    )
    setattr(derouter, "max_budget", 0.00001)
    from derouter.proxy.proxy_server import user_api_key_cache

    user_api_key_cache.set_cache(
        key="{}:spend".format(derouter_proxy_budget_name), value=0
    )
...
def test_call_with_proxy_over_budget_stream(prisma_client):
    ...
    async def test():
        ...
        # request = NewUserRequest(
        #     max_budget=0.00001, user_id=derouter_proxy_budget_name
        # )
        request = NewUserRequest()
        key = await new_user(
            data=request,
...
def test_delete_key(prisma_client):
    ...
    setattr(derouter.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(derouter.proxy.proxy_server, "master_key", "sk-1234")
    setattr(derouter.proxy.proxy_server, "user_custom_auth", None)
    try:
...
def test_generate_and_call_key_info(prisma_client):
    ...
    try:
        ...
        async def test():
            await derouter.proxy.proxy_server.prisma_client.connect()
            request = NewUserRequest(
                metadata={"team": "derouter-team3", "project": "derouter-project3"}
            )
            key = await new_user(
                data=request,
                user_api_key_dict=UserAPIKeyAuth(
...
def test_generate_and_update_key(prisma_client):
    ...
    try:
        ...
        async def test():
            ...
            # create team "derouter-core-infra@gmail.com""
            print("creating team derouter-core-infra@gmail.com")
            _team_1 = "derouter-core-infra@gmail.com_{}".format(uuid.uuid4())
            await new_team(
                NewTeamRequest(
                    team_id=_team_1,
                ),
            ...
            request = NewUserRequest(
                metadata={"project": "derouter-project3"},
                team_id=_team_1,
            )
...
def test_key_generate_with_custom_auth(prisma_client):
    ...
    async def custom_generate_key_fn(data: GenerateKeyRequest) -> dict:
        ...
        Returns:
            ...
            {
                "decision": False,
                "message": "This violates DeRouter Proxy Rules. No team id provided.",
            }
        """
        ...
        if team_id is not None and team_id == "derouter-core-infra@gmail.com":
            # only team_id="derouter-core-infra@gmail.com" can make keys
            return {
                "decision": True,
            }
        else:
    ...
    setattr(derouter.proxy.proxy_server, "master_key", "sk-1234")
    setattr(
        derouter.proxy.proxy_server, "user_custom_key_generate", custom_generate_key_fn
    )
    try:
        ...
        async def test():
            ...
            with pytest.raises(
                Exception, match=re.escape("This violates DeRouter Proxy Rules. No team id provided.")
            ) as exc_info:
                key = await generate_key_fn(
                    request,
                    user_api_key_dict=UserAPIKeyAuth(
            ...
            assert (
                "This violates DeRouter Proxy Rules. No team id provided."
                in e.message
            )

            request_2 = GenerateKeyRequest(
                team_id="derouter-core-infra@gmail.com",
            )
...
def test_call_with_key_over_budget_no_cache(prisma_client):
    ...
    # Related to this: https://github.com/Delify-Solutions/derouter/issues/3920
    setattr(derouter.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(derouter.proxy.proxy_server, "master_key", "sk-1234")
    ...
    async def test():
        ...
        setattr(derouter.proxy.proxy_server, "proxy_batch_write_at", 1)
...
async def test_aasync_call_with_key_over_model_budget(
    ...
    from derouter.proxy.proxy_server import model_max_budget_limiter
...
async def test_key_name_set(prisma_client):
    ...
    setattr(derouter.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(derouter.proxy.proxy_server, "master_key", "sk-1234")
    setattr(derouter.proxy.proxy_server, "general_settings", {"allow_user_auth": True})
    await derouter.proxy.proxy_server.prisma_client.connect()
    try:
        request = GenerateKeyRequest()
        key = await generate_key_fn(
...
async def test_default_key_params(prisma_client):
    ...
    setattr(derouter.proxy.proxy_server, "master_key", "sk-1234")
    setattr(derouter.proxy.proxy_server, "general_settings", {"allow_user_auth": True})
    derouter.default_key_generate_params = {"max_budget": 0.000122}
    await derouter.proxy.proxy_server.prisma_client.connect()
    try:
        request = GenerateKeyRequest()
        key = await generate_key_fn(
...
async def test_upperbound_key_param_larger_budget(prisma_client):
    ...
    setattr(derouter.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(derouter.proxy.proxy_server, "master_key", "sk-1234")
    derouter.upperbound_key_generate_params = DeRouter_UpperboundKeyGenerateParams(
...
async def test_user_api_key_auth_without_master_key(prisma_client):
    ...
    try:
        ...
        setattr(derouter.proxy.proxy_server, "prisma_client", prisma_client)
        setattr(derouter.proxy.proxy_server, "master_key", None)
        setattr(
            derouter.proxy.proxy_server, "general_settings", {"allow_user_auth": True}
        )
        await derouter.proxy.proxy_server.prisma_client.connect()
...
async def test_key_with_no_permissions(prisma_client):
    ...
    setattr(derouter.proxy.proxy_server, "prisma_client", prisma_client)
    setattr(derouter.proxy.proxy_server, "master_key", "sk-1234")
    setattr(derouter.proxy.proxy_server, "general_settings", {"allow_user_auth": False})
    await derouter.proxy.proxy_server.prisma_client.connect()
    try:
        response = await generate_key_helper_fn(
            request_type="key",
            **{"duration": "1hr", "key_max_budget": 0, "models": [], "aliases": {}, "config": {}, "spend": 0, "user_id": "ishaan", "team_id": "derouter-dashboard"},  # type: ignore
        )
...
async def test_master_key_hashing(prisma_client):
    try:
        ...
        setattr(derouter.proxy.proxy_server, "prisma_client", prisma_client)
        setattr(derouter.proxy.proxy_server, "master_key", master_key)
...
async def test_custom_api_key_header_name(prisma_client):
    ...
    setattr(
        derouter.proxy.proxy_server,
        "general_settings",
        {"derouter_key_header_name": "x-derouter-key"},
    )
    await derouter.proxy.proxy_server.prisma_client.connect()
    ...
    request = Request(
        {
            ...
            "path": api_route.path,
            "headers": [
                (b"x-derouter-key", b"Bearer sk-1234"),
            ],
        }
    )

    # this should pass because we pass the master key as X-Litellm-Key and derouter_key_header_name="X-Litellm-Key" in general settings
...
async def test_team_access_groups(prisma_client):
    ...
    derouter_router = derouter.Router(
        model_list=[
            {
    ...
    setattr(derouter.proxy.proxy_server, "llm_router", derouter_router)
...
async def test_user_api_key_auth_db_unavailable():
    ...
    setattr(derouter.proxy.proxy_server, "prisma_client", MockPrismaClient())
    setattr(derouter.proxy.proxy_server, "user_api_key_cache", MockDualCache())
    setattr(derouter.proxy.proxy_server, "master_key", "sk-1234")
    setattr(
        derouter.proxy.proxy_server,
        "general_settings",
...
async def test_user_api_key_auth_db_unavailable_not_allowed():
    ...
    setattr(derouter.proxy.proxy_server, "prisma_client", MockPrismaClient())
    setattr(derouter.proxy.proxy_server, "user_api_key_cache", MockDualCache())
    setattr(derouter.proxy.proxy_server, "general_settings", {})
    setattr(derouter.proxy.proxy_server, "master_key", "sk-1234")
    ...
    # Run test with a sample API key
    with pytest.raises(derouter.proxy._types.ProxyException):
        await user_api_key_auth(
            request=request,
            api_key="Bearer sk-123456789",
        )
...
@pytest.mark.asyncio
@mock.patch(
    "derouter.secret_managers.aws_secret_manager_v2.AWSSecretsManagerV2.async_write_secret"
)
@mock.patch(
    "derouter.secret_managers.aws_secret_manager_v2.AWSSecretsManagerV2.async_read_secret"
)
@mock.patch(
    "derouter.secret_managers.aws_secret_manager_v2.AWSSecretsManagerV2.async_delete_secret"
)
async def test_key_generate_with_secret_manager_call(
    ...
    """
    from derouter.secret_managers.aws_secret_manager_v2 import AWSSecretsManagerV2
    from derouter.types.secret_managers.main import (
        KeyManagementSystem,
        KeyManagementSettings,
    )

    from derouter.proxy.hooks.key_management_event_hooks import (
        DEROUTER_PREFIX_STORED_VIRTUAL_KEYS,
    )
    ...
    derouter.secret_manager_client = aws_secret_manager_client
    derouter._key_management_system = KeyManagementSystem.AWS_SECRET_MANAGER
    ...
    setattr(derouter.proxy.proxy_server, "general_settings", general_settings)
    setattr(derouter.proxy.proxy_server, "master_key", "sk-1234")
    setattr(derouter.proxy.proxy_server, "prisma_client", prisma_client)
    await derouter.proxy.proxy_server.prisma_client.connect()
    ############################################################################
    ...
    result = await aws_secret_manager_client.async_read_secret(
        secret_name=f"{derouter._key_management_settings.prefix_for_stored_virtual_keys}{key_alias}"
    )
...
def test_should_track_cost_callback():
    ...
    """
    from derouter.proxy.hooks.proxy_track_cost_callback import (
...
async def test_get_paginated_teams(prisma_client):
    ...
    """
    from derouter.proxy.management_endpoints.team_endpoints import get_paginated_teams
...
async def test_reset_budget_job(prisma_client, entity_type):
    ...
    if entity_type == "key":
        ...
        updated = await prisma_client.db.derouter_verificationtoken.update_many(
        ...
        await prisma_client.db.derouter_usertable.update_many(
        ...
        await prisma_client.db.derouter_teamtable.update(
    ...
    if entity_type == "key":
        entity_before = await prisma_client.db.derouter_verificationtoken.find_unique(
            where={"token": entity_id}
        )
    elif entity_type == "user":
        entity_before = await prisma_client.db.derouter_usertable.find_unique(
            where={"user_id": entity_id}
        )
    elif entity_type == "team":
        entity_before = await prisma_client.db.derouter_teamtable.find_unique(
    ...
    if entity_type == "key":
        entity_after = await prisma_client.db.derouter_verificationtoken.find_unique(
            where={"token": entity_id}
        )
    elif entity_type == "user":
        entity_after = await prisma_client.db.derouter_usertable.find_unique(
            where={"user_id": entity_id}
        )
    elif entity_type == "team":
        entity_after = await prisma_client.db.derouter_teamtable.find_unique(
            where={"team_id": entity_id}
        )
...
def test_delete_nonexistent_key_returns_404(prisma_client):
    ...
    try:
        ...
        async def test():
            ...
            result = await derouter.proxy.proxy_server.user_api_key_auth(
                request=request, api_key=bearer_token
            )
            result.user_role = LitellmUserRoles.PROXY_ADMIN
            with pytest.raises(ProxyException) as exc_info:
