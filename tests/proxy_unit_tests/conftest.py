...
# Top-level assignments of these types are the ones importlib.reload(derouter)
...
def _collect_flushable_caches():
    ...
    targets = []
    for module in (derouter, derouter.proxy.proxy_server):
        for attr in list(vars(module)):
            if attr.startswith("_"):
                continue
            try:
...
_DEROUTER_STATE = _snapshot_mutable_state(derouter)
_PROXY_SERVER_STATE = _snapshot_mutable_state(derouter.proxy.proxy_server)
_FLUSHABLE_CACHES = _collect_flushable_caches()


@pytest.fixture(scope="function", autouse=True)
def setup_and_teardown():
    """Reset mutable module state on derouter and proxy_server before each test.

    Replaces a previous importlib.reload(derouter) approach that cost ~17s
    per test (re-executing the full derouter __init__ import chain).
    ...
    """
    _restore_mutable_state(derouter, _DEROUTER_STATE)
    _restore_mutable_state(derouter.proxy.proxy_server, _PROXY_SERVER_STATE)
