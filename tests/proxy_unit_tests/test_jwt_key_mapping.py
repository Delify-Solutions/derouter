...
from derouter.proxy.management_endpoints.jwt_key_mapping_endpoints import (
    _to_response,
...
async def test_jwt_to_virtual_key_mapping_resolution():
    ...
    prisma_client.db.derouter_jwtkeymapping.find_first = AsyncMock()

    # Mock finding a mapping
    mock_mapping = MagicMock()
    mock_mapping.token = "sk-1234"
    mock_mapping.is_active = True
    prisma_client.db.derouter_jwtkeymapping.find_first.return_value = mock_mapping
    ...
    with patch(
        ...
        prisma_client.db.derouter_jwtkeymapping.find_first.assert_called_once()

        # Test Cache hit
        prisma_client.db.derouter_jwtkeymapping.find_first.reset_mock()
        ...
        prisma_client.db.derouter_jwtkeymapping.find_first.assert_not_called()
...
async def test_jwt_to_virtual_key_mapping_no_mapping():
    ...
    """
    jwt_handler = JWTHandler()
    jwt_handler.derouter_jwtauth = DeRouter_JWTAuth(virtual_key_claim_field="email")
    jwt_claims = {"email": "unknown@example.com"}

    prisma_client = MagicMock()
    prisma_client.db.derouter_jwtkeymapping.find_first = AsyncMock()
    prisma_client.db.derouter_jwtkeymapping.find_first.return_value = None
...
async def test_virtual_key_mapping_oidc_enabled_jwt_token_uses_auth_jwt():
    ...
    if jwt_handler.derouter_jwtauth.oidc_userinfo_enabled and not jwt_handler.is_jwt(
        token=api_key
    ):
...
def _mock_prisma():
    prisma = MagicMock()
    prisma.db.derouter_jwtkeymapping.create = AsyncMock()
    prisma.db.derouter_jwtkeymapping.find_unique = AsyncMock()
    prisma.db.derouter_jwtkeymapping.find_many = AsyncMock()
    prisma.db.derouter_jwtkeymapping.update = AsyncMock()
    prisma.db.derouter_jwtkeymapping.delete = AsyncMock()
    prisma.db.derouter_jwtkeymapping.count = AsyncMock(return_value=0)
    return prisma
...
async def test_create_returns_409_on_unique_violation():
    ...
    from derouter.proxy._types import CreateJWTKeyMappingRequest

    mock_prisma = _mock_prisma()
    mock_prisma.db.derouter_jwtkeymapping.create.side_effect = Exception(
...
async def test_delete_returns_404_when_not_found():
    ...
    from derouter.proxy._types import DeleteJWTKeyMappingRequest

    mock_prisma = _mock_prisma()
    mock_prisma.db.derouter_jwtkeymapping.find_unique.return_value = None
    mock_cache = AsyncMock()
...
async def test_update_returns_404_when_not_found():
    ...
    from derouter.proxy._types import UpdateJWTKeyMappingRequest
...
async def test_update_returns_404_when_row_deleted_before_write():
    ...
    mock_prisma = _mock_prisma()
    mock_prisma.db.derouter_jwtkeymapping.find_unique.return_value = _mock_mapping()
    mock_prisma.db.derouter_jwtkeymapping.update.return_value = None
    mock_cache = AsyncMock()
    ...
    with (
        ...
        patch("derouter.proxy.proxy_server.user_api_key_cache", mock_cache),  # test-quality-ok: proxy_server module global is the endpoint's only injection point
    ):
        with pytest.raises(HTTPException) as exc_info:
            await update_jwt_key_mapping(
                data=data, user_api_key_dict=_make_admin_auth()
...
async def test_create_success_returns_response_without_token():
    ...
    mock_prisma = _mock_prisma()
    mock_prisma.db.derouter_jwtkeymapping.create.return_value = _mock_mapping()
    mock_cache = AsyncMock()
...
async def test_reject_behavior_raises_403_on_no_mapping():
    ...
    """
    from derouter.proxy._types import UnregisteredJWTClientBehavior
    ...
    prisma_client = MagicMock()
    prisma_client.db.derouter_jwtkeymapping.find_first = AsyncMock(return_value=None)
...
async def test_auto_register_returns_pending_signal_without_creating_key():
    ...
    """
    from derouter.proxy._types import UnregisteredJWTClientBehavior
    from derouter.proxy.auth.user_api_key_auth import _PendingAutoRegister
    ...
    prisma_client.db.derouter_jwtkeymapping.create.assert_not_called()
...
async def test_auto_register_creates_key_and_mapping_when_helper_invoked():
    ...
    DeRouter_VerificationToken), not the plaintext key.
    """
    ...
    call_data = prisma_client.db.derouter_jwtkeymapping.create.call_args[1]["data"]
...
async def test_auto_register_race_condition_unique_conflict():
    ...
    second hits a unique-constraint violation on create, it must:
      1) delete the orphaned virtual key it just created (so orphans don't
         accumulate in DeRouter_VerificationToken under sustained concurrency),
      2) fall back to the winner's mapping,
      3) not surface an error.
    """
    from derouter.proxy.auth.user_api_key_auth import _auto_register_jwt_mapping
    from derouter.proxy._types import UnregisteredJWTClientBehavior, hash_token
    ...
    prisma_client.db.derouter_verificationtoken.delete = AsyncMock()
    ...
    assert result == mock_key_obj
    # The orphaned loser key must be deleted from DeRouter_VerificationToken
    prisma_client.db.derouter_verificationtoken.delete.assert_called_once_with(
...
async def test_auto_register_race_conflict_tolerates_delete_failure():
    ...
    assert result == mock_key_obj
    prisma_client.db.derouter_verificationtoken.delete.assert_called_once()
...
def test_jwt_client_id_field_alias_maps_to_virtual_key_claim_field():
    ...
    """
    auth = DeRouter_JWTAuth(jwt_client_id_field="azp")
...
async def test_delete_evicts_cache_after_row_is_gone():
    ...
    """
    from derouter.proxy._types import DeleteJWTKeyMappingRequest
    from derouter.proxy.auth.auth_checks import jwt_key_mapping_cache_key
    ...
    mock_prisma.db.derouter_jwtkeymapping.delete.side_effect = concurrent_reader_repopulates

    with (
        patch("derouter.proxy.proxy_server.prisma_client", mock_prisma),  # test-quality-ok: proxy_server module global is the endpoint's only injection point
        patch("derouter.proxy.proxy_server.user_api_key_cache", user_api_key_cache),  # test-quality-ok: proxy_server module global is the endpoint's only injection point
    ):
...
async def test_update_evicts_old_and_new_cache_keys_after_write():
    ...
    mock_prisma.db.derouter_jwtkeymapping.update.side_effect = concurrent_reader_repopulates
