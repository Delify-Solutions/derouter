"""
Unit tests for derouter/proxy/google_endpoints/agents_endpoints.py
...
(api_key, api_base, …) supplied via the JSON-encoded derouter_params_template
...
from derouter.proxy.google_endpoints.agents_endpoints import (
...
class TestMergeQueryParamsIntoData:
    ...
    def test_derouter_params_template_json_is_expanded(self):
        template = json.dumps(
            {"api_key": "AIzaFromTemplate", "api_base": "https://example.com"}
        )
        from urllib.parse import quote

        request = _make_request(f"derouter_params_template={quote(template)}")
        data = {"custom_llm_provider": "gemini"}
        _merge_query_params_into_data(data, request)
        ...
        assert "derouter_params_template" not in data

    def test_derouter_params_template_does_not_overwrite_existing(self):
        template = json.dumps(
    ...
    def test_invalid_derouter_params_template_json_is_ignored(self):
        request = _make_request("derouter_params_template=NOT_VALID_JSON")
        data = {"custom_llm_provider": "gemini"}
        _merge_query_params_into_data(data, request)
    ...
    def test_template_only_no_flat_params_merged(self):
        """Only derouter_params_template is expanded; unknown flat params are dropped."""
        template = json.dumps({"api_key": "FromTemplate"})
        from urllib.parse import quote

        qs = f"derouter_params_template={quote(template)}&vertex_project=my-project"
        request = _make_request(qs)
        data = {"custom_llm_provider": "gemini"}
        _merge_query_params_into_data(data, request)
        assert data["api_key"] == "FromTemplate"
        # flat vertex_project is ignored since it wasn't in derouter_params_template
...
def mock_srv():
    ...
    with patch(
        "derouter.proxy.google_endpoints.agents_endpoints._proxy_server_imports",
        return_value=srv,
    ):
        yield srv
...
async def test_list_gemini_agents_passes_api_key_to_processor(
    ...
    from derouter.proxy.google_endpoints.agents_endpoints import list_gemini_agents

    template = json.dumps({"api_key": "AIzaListTest"})

    with patch(
        "derouter.proxy.google_endpoints.agents_endpoints.ProxyBaseLLMRequestProcessing"
    ) as MockProcessor:
        instance = MockProcessor.return_value
        instance.base_process_llm_request = AsyncMock(return_value=MagicMock())

        request = _make_endpoint_request(f"derouter_params_template={quote(template)}")
...
async def test_get_gemini_agent_passes_api_key_to_processor(
    ...
    from derouter.proxy.google_endpoints.agents_endpoints import get_gemini_agent
...
async def test_delete_gemini_agent_passes_api_key_to_processor(
    ...
    from derouter.proxy.google_endpoints.agents_endpoints import delete_gemini_agent
...
async def test_get_gemini_agent_name_not_overwritten_by_query_param(
    ...
    with patch(
        ...
        request = _make_endpoint_request(
            f"name=INJECTED&derouter_params_template={quote(template)}"
        )
        await get_gemini_agent(
            request=request,
            name="real-agent",
...
async def test_list_agents_template_via_query_param(mock_srv, user_api_key_dict):
    """derouter_params_template in query string is expanded."""
    from derouter.proxy.google_endpoints.agents_endpoints import list_gemini_agents
    from urllib.parse import quote
    ...
    with patch(
        ...
        assert "derouter_params_template" not in init_data
...
async def test_create_agent_non_admin_without_api_key_is_rejected(
    ...
    from derouter.proxy.google_endpoints.agents_endpoints import create_gemini_agent
    ...
    with (
        ...
        patch(
            "derouter.proxy.google_endpoints.agents_endpoints._read_request_body",
...
def test_validate_environment_rejects_api_base_override_without_explicit_key(
    ...
    """
    from derouter.llms.gemini.agents.transformation import GeminiAgentsConfig
    ...
    with pytest.raises(ValueError, match="api_base"):
        cfg.validate_environment(
            headers={},
            derouter_params={"api_base": "https://attacker.example"},
        )
...
def test_validate_environment_env_fallback_when_no_api_base_override(monkeypatch):
    ...
    cfg = GeminiAgentsConfig()
    headers = cfg.validate_environment(headers={}, derouter_params={})
