...
async def test_e2e_size_of_redis_buffer():
    ...
    from derouter.proxy.db.db_transaction_queue.base_update_queue import BaseUpdateQueue
    ...
    setattr(derouter.proxy.proxy_server, "use_redis_transaction_buffer", True)
    ...
    with patch(
        "derouter.proxy.db.db_spend_update_writer.PodLockManager.acquire_lock",
        return_value=True,
    ):
