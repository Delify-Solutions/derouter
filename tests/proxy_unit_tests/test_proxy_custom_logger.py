...
import pytest, time
import derouter
from derouter import embedding, completion, completion_cost, Timeout
from derouter import RateLimitError
...
def test_embedding(client):
    try:
        ...
        derouter.callbacks = [my_custom_logger]
        # Your test data
        print("initialized proxy")
        # import the initialized custom logger
        print(derouter.callbacks)

        # assert len(derouter.callbacks) == 1 # assert derouter is initialized with 1 callback
        ...
        metadata = derouter_params.get("metadata", None)
        print("\n\n Metadata in custom logger kwargs", derouter_params.get("metadata"))
        assert metadata is not None, "metadata should be present in derouter_params"
        ...
        proxy_server_request = derouter_params.get("proxy_server_request")
        ...
        model_info = derouter_params.get("model_info")
...
def test_chat_completion(client):
    try:
        ...
        print("DeRouter Callbacks", derouter.callbacks)
        print("my_custom_logger", my_custom_logger)
        ...
        test_data = {
            ...
            "messages": [
                {"role": "user", "content": "write a derouter poem"},
            ],
            "max_tokens": 10,
        }
        ...
        derouter_params = my_custom_logger.async_completion_kwargs.get("derouter_params")
        ...
        config_model_info = derouter_params.get("model_info")
        ...
        proxy_server_request_object = derouter_params.get("proxy_server_request")
        assert (
...
def test_chat_completion_stream(client):
    try:
        ...
        test_data = {
            "model": "Azure OpenAI GPT-4 Canada",
            "messages": [
                {"role": "user", "content": "write 1 line poem about DeRouter"},
            ],
            "max_tokens": 40,
