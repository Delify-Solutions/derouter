...
class TestCheckResponsesCost:
    ...
    async def test_check_responses_cost_no_jobs(
        ...
        find_many_call = (
            mock_prisma_client.db.derouter_managedobjecttable.find_many.call_args
        )
    ...
    async def test_cleanup_stale_managed_objects(
        ...
        from derouter.constants import STALE_OBJECT_CLEANUP_BATCH_SIZE
    ...
    async def test_check_responses_cost_with_completed_response(
        ...
        # Run the check with mocked derouter.aget_responses
        with patch("derouter.aget_responses", new_callable=AsyncMock) as mock_aget:
    ...
    async def test_encoded_response_id_is_fetched_through_router(
        ...
        """
        Regression test for https://github.com/Delify-Solutions/derouter/issues/35131
        ...
        derouter.aget_responses directly only sees provider env vars, fails, and
    ...
    async def test_encrypted_response_id_is_fetched_through_router(
        ...
        monkeypatch.setenv("DEROUTER_SALT_KEY", "sk-test-salt-key-for-response-ids")
        ...
        encrypted_response_id = "resp_" + str(
            encrypt_value_helper(
                value=SpecialEnums.DEROUTER_MANAGED_RESPONSE_API_RESPONSE_ID_COMPLETE_STR.value.format(
    ...
    async def test_response_id_without_model_id_uses_sdk(
        ...
        with patch("derouter.aget_responses", new_callable=AsyncMock) as mock_sdk_aget:
    ...
    async def test_check_responses_cost_no_model_in_file_object(
        ...
        assert "model" not in call_kwargs.get("derouter_metadata", {})
        assert "model_group" not in call_kwargs.get("derouter_metadata", {})
    ...
    async def test_poll_stamps_internal_call_origin_so_the_read_is_billed(
        ...
        metadata = mock_aget.call_args[1]["derouter_metadata"]
