...
Two bugs are confirmed in DeRouter v1.83.7-stable (upstream Delify-Solutions/derouter#27193).
Both live in _lookup_deprecated_key() (derouter/proxy/utils.py):
...
def _make_db(active_token_id: Optional[str]) -> MagicMock:
    ...
    db = MagicMock()
    db.derouter_deprecatedverificationtoken = MagicMock()
    db.derouter_deprecatedverificationtoken.find_first = AsyncMock(
...
async def test_lookup_deprecated_key_db_miss_returns_none():
    ...
    from derouter.proxy.utils import _lookup_deprecated_key, _deprecated_key_cache
    ...
    db.derouter_deprecatedverificationtoken.find_first.assert_called_once()
...
async def test_lookup_deprecated_key_cache_hit_returns_on_second_call():
    ...
    assert db.derouter_deprecatedverificationtoken.find_first.call_count == 1
