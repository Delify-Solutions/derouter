...
Tests that search API requests are properly logged to DeRouter_SpendLogs
...
from derouter.proxy.spend_tracking.spend_management_endpoints import view_spend_logs
from derouter.proxy.utils import ProxyLogging, hash_token, update_spend
...
def prisma_client():
    ...
    proxy_server.derouter_proxy_budget_name = f"derouter-proxy-budget-{time.time()}"
...
async def test_search_api_logging_and_cost_tracking(prisma_client):
    ...
    with patch("derouter.search.main.asearch", new_callable=AsyncMock) as mock_asearch:
        ...
        kwargs = {
            ...
            "derouter_call_id": request_id,  # Set request_id in kwargs
            "derouter_params": {
                "metadata": {
