...
from derouter.proxy.google_endpoints.endpoints import google_generate_content
...
async def test_google_gemini_httpx_request_direct():
    ...
    from derouter.llms.gemini.google_genai.transformation import GoogleGenAIConfig
...
async def test_generationconfig_to_config_mapping(sample_request_payload):
    ...
    with patch(
        "derouter.google_genai.main.base_llm_http_handler.generate_content_handler"
...
async def test_gemini_proxy_config_with_custom_api_base():
    ...
    for model in test_models:
        ...
        auth_header, result_url = vertex_base._check_custom_proxy(
            api_base=model_config["derouter_params"]["api_base"],
            custom_llm_provider="gemini",
            gemini_api_key=model_config["derouter_params"]["api_key"],
            endpoint="generateContent",
            stream=False,
            auth_header=None,
        ...
        expected_url = f"{model_config['derouter_params']['api_base']}/models/{model}:generateContent"
        assert (
            result_url == expected_url
        ), f"Expected {expected_url}, got {result_url} for model {model}"
        expected_auth_header = {
            "x-goog-api-key": model_config["derouter_params"]["api_key"]
        }
        assert (
