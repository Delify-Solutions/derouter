...
class TestIsModelCostZero:
    ...
    def test_paid_model_in_router(self, mock_router_with_zero_cost_model):
        ...
        with patch("derouter.get_model_info") as mock_get_model_info:
...
class TestEndUserBudgetBypass:
    ...
    async def test_end_user_over_budget_with_zero_cost_model_allowed(
        ...
        end_user_budget = DeRouter_BudgetTable(max_budget=20.0)
        end_user_object = DeRouter_EndUserTable(
            user_id="end-user-123",
            spend=50.0,
            derouter_budget_table=end_user_budget,
            blocked=False,
        )
...
class TestTeamMemberBudgetBypass:
    ...
    async def test_team_member_over_budget_with_zero_cost_model_allowed(
        ...
        member_budget = DeRouter_BudgetTable(max_budget=30.0)
        team_membership = DeRouter_TeamMembership(
            user_id="test-user",
            team_id="test-team",
            spend=60.0,
            derouter_budget_table=member_budget,
        )
