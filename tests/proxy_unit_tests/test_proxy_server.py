...
import derouter
from derouter import RateLimitError, Timeout, completion, completion_cost, embedding
...
from derouter.proxy.proxy_server import (  # Replace with the actual module where your FastAPI router is defined
    app,
    initialize,
    save_worker_config,
)
...
def mock_patch_acompletion():
    return mock.patch(
        "derouter.proxy.proxy_server.llm_router.acompletion",
...
def mock_patch_aimage_generation():
    return mock.patch(
        "derouter.proxy.proxy_server.llm_router.aimage_generation",
...
def test_chat_completion(mock_acompletion, client_no_auth):
    ...
    try:
        ...
        mock_acompletion.assert_called_once_with(
            ...
            max_tokens=10,
            derouter_call_id=mock.ANY,
            derouter_logging_obj=mock.ANY,
        ...
        pytest.fail(f"DeRouter Proxy test failed. Exception - {str(e)}")
...
def test_chat_completion_malformed_messages_returns_400(client_no_auth):
    ...
    {role, content} objects, DeRouter returns a 400 invalid_request_error instead
...
def test_get_settings_request_timeout(client_no_auth):
    """
    When no timeout is set, it should use the derouter.request_timeout value
    """
    # Set a known value for derouter.request_timeout
    import derouter
    ...
    assert settings["derouter.request_timeout"] == derouter.request_timeout
...
def test_add_headers_to_request(derouter_key_header_name):
    ...
    request_headers = clean_headers(headers, derouter_key_header_name)
    forwarded_headers = DeRouterProxyRequestSetup._get_forwardable_headers(
        request_headers
    )
...
def test_chat_completion_forward_headers(
    mock_acompletion, client_no_auth, derouter_key_header_name, forward_headers
):
    global headers
    try:
        if forward_headers:
            gs = getattr(derouter.proxy.proxy_server, "general_settings")
            gs["forward_client_headers_to_llm_api"] = True
            setattr(derouter.proxy.proxy_server, "general_settings", gs)
        if derouter_key_header_name is not None:
            gs = getattr(derouter.proxy.proxy_server, "general_settings")
            gs["derouter_key_header_name"] = derouter_key_header_name
            setattr(derouter.proxy.proxy_server, "general_settings", gs)
        # Your test data
        test_data = {
            "model": "gpt-3.5-turbo",
        ...
        if derouter_key_header_name is not None:
            headers_to_not_forward = {derouter_key_header_name: "Bearer 1234"}
        else:
...
async def test_team_disable_guardrails(mock_acompletion, client_no_auth):
    ...
    from derouter.proxy.proxy_server import hash_token, user_api_key_cache
    ...
    setattr(derouter.proxy.proxy_server, "user_api_key_cache", user_api_key_cache)
    setattr(derouter.proxy.proxy_server, "master_key", "sk-1234")
    setattr(derouter.proxy.proxy_server, "prisma_client", "hello-world")
...
def test_custom_logger_failure_handler(mock_acompletion, client_no_auth):
    ...
    derouter.callbacks = [mock_logger, mock_logger_unit_tests]
    proxy_logging_obj._init_derouter_callbacks(llm_router=None)

    setattr(derouter.proxy.proxy_server, "user_api_key_cache", user_api_key_cache)
    setattr(derouter.proxy.proxy_server, "master_key", "sk-1234")
    setattr(derouter.proxy.proxy_server, "prisma_client", "FAKE-VAR")
    setattr(derouter.proxy.proxy_server, "proxy_logging_obj", proxy_logging_obj)
...
def test_embedding(mock_aembedding, client_no_auth):
    global headers
    from derouter.proxy.proxy_server import user_custom_auth

    try:
        test_data = {
            "model": "azure/text-embedding-ada-002",
            "input": ["good morning from derouter"],
        }
...
def test_chat_completion_optional_params(mock_acompletion, client_no_auth):
    ...
    # This tests if all the /chat/completion params are passed to derouter
    try:
        # Your test data
        ...
        derouter.callbacks = [customHandler]
        ...
        pytest.fail("DeRouter Proxy test failed. Exception", e)
...
@mock.patch("derouter.proxy.proxy_server.derouter.Cache")
def test_load_router_config(mock_cache, fake_env_vars):
    ...
    try:
        ...
        # tests for derouter.cache set from config
        ...
        assert derouter.cache is not None
        assert "redis_client" in vars(
            derouter.cache.cache
        )  # it should default to redis on proxy
        assert derouter.cache.supported_call_types == [
            "completion",
            "acompletion",
            "embedding",
            "aembedding",
        ...
        assert derouter.cache is not None
        print(derouter.cache)
        print(derouter.cache.supported_call_types)
        print(vars(derouter.cache.cache))
        assert "redis_client" in vars(
            derouter.cache.cache
        )  # it should default to redis on proxy
        assert derouter.cache.supported_call_types == [
...
async def test_team_update_redis():
    ...
    with patch.object(
        ...
        await _cache_team_object(
            team_id="1234",
            team_table=DeRouter_TeamTableCachedObj(team_id="1234"),
...
async def test_create_user_default_budget(prisma_client, user_role):  # noqa: F811  # pytest fixture, not a redefinition
    ...
    setattr(derouter.proxy.proxy_server, "master_key", "sk-1234")
    setattr(derouter, "max_internal_user_budget", 10)
    setattr(derouter, "internal_user_budget_duration", "5m")
    ...
    with patch.object(
        derouter.proxy.proxy_server.prisma_client, "insert_data", new=AsyncMock()
    ) as mock_client:
        await new_user(
            request,
        )
        ...
        if user_role == LitellmUserRoles.INTERNAL_USER.value:
            assert (
                ...
                == derouter.max_internal_user_budget
            )
            assert (
                mock_client.call_args.kwargs["data"]["budget_duration"]
                == derouter.internal_user_budget_duration
            )
...
def _member_add_tx_cm(team_table):
    ...
    class _Tx:
        ...
        derouter_teamtable = team_table

        def __getattr__(self, table_name):
            return getattr(derouter.proxy.proxy_server.prisma_client.db, table_name)
...
async def test_create_team_member_add(prisma_client, new_member_method):  # noqa: F811  # pytest fixture, not a redefinition
    ...
    await derouter.proxy.proxy_server.prisma_client.connect()
    user = f"ishaan {uuid.uuid4().hex}"
    _team_id = "derouter-test-client-id-new"
    team_obj = DeRouter_TeamTableCachedObj(
        team_id=_team_id,
        blocked=False,
        last_refreshed_at=time.time(),
    ...
    with (
        patch(
            "derouter.proxy.proxy_server.prisma_client.db.derouter_usertable",
            new_callable=AsyncMock,
        ) as mock_derouter_usertable,
        patch(
            "derouter.proxy.auth.auth_checks._get_team_object_from_user_api_key_cache",
            new=AsyncMock(return_value=team_obj),
        ) as mock_team_obj,
        patch(
            "derouter.proxy.proxy_server.prisma_client.get_data",
        ...
        mock_derouter_usertable.upsert = mock_client
        mock_derouter_usertable.find_many = AsyncMock(return_value=[])
        # Mock find_first for user_email validation (returns None for new users)
        mock_derouter_usertable.find_first = AsyncMock(return_value=None)
        # Mock find_unique for user_id validation (returns None for new users)
        mock_derouter_usertable.find_unique = AsyncMock(return_value=None)
        team_mock_client = AsyncMock()
        original_val = getattr(
            derouter.proxy.proxy_server.prisma_client.db, "derouter_teamtable"
        )
        derouter.proxy.proxy_server.prisma_client.db.derouter_teamtable = team_mock_client

        team_mock_client.update = AsyncMock(
            return_value=DeRouter_TeamTableCachedObj(team_id="1234")
        )

        tx_cm = _member_add_tx_cm(team_mock_client)
        original_tx = derouter.proxy.proxy_server.prisma_client.tx
        derouter.proxy.proxy_server.prisma_client.tx = MagicMock(
            return_value=tx_cm
        )
        ...
        derouter.proxy.proxy_server.prisma_client.db.derouter_teamtable = original_val
        derouter.proxy.proxy_server.prisma_client.tx = original_tx
...
async def test_create_team_member_add_team_admin_user_api_key_auth(
    ...
    from derouter.proxy._types import DeRouter_TeamTableCachedObj, Member
...
async def test_create_team_member_add_team_admin(
    ...
    """
    Relevant issue - https://github.com/Delify-Solutions/derouter/issues/5300
    ...
    with (
        ...
        with (
            patch.object(
                derouter.proxy.proxy_server.prisma_client.db,
                "derouter_teamtable",
                team_mock_client,
            ),
            patch.object(
...
async def test_user_info_team_list(prisma_client):  # noqa: F811  # pytest fixture, not a redefinition
    ...
    from derouter.proxy.management_endpoints.internal_user_endpoints import user_info

    with patch(
        "derouter.proxy.management_endpoints.team_endpoints.list_team",
        new_callable=AsyncMock,
    ) as mock_client:
...
async def test_add_callback_via_key(prisma_client):  # noqa: F811  # pytest fixture, not a redefinition
    ...
    try:
        ...
        with patch.object(
            ...
            assert "user_api_key_metadata" in kwargs["derouter_params"]["metadata"]
            assert (
                "logging"
                in kwargs["derouter_params"]["metadata"]["user_api_key_metadata"]
            )
            checked_keys = False
            for item in kwargs["derouter_params"]["metadata"]["user_api_key_metadata"][
                "logging"
            ]:
...
async def test_add_callback_via_key_derouter_pre_call_utils(
    mock_prisma_client,
    callback_type,
    ...
    setattr(derouter.proxy.proxy_server, "prisma_client", mock_prisma_client)
    setattr(derouter.proxy.proxy_server, "master_key", "sk-1234")

    proxy_config = getattr(derouter.proxy.proxy_server, "proxy_config")
    ...
    new_data = await add_derouter_data_to_request(**data)
...
async def test_add_callback_via_key_derouter_pre_call_utils_gcs_bucket(
    mock_prisma_client,
    callback_type,
    expected_success_callbacks,
    expected_failure_callbacks,
...
async def test_add_callback_via_key_derouter_pre_call_utils_langsmith(
    mock_prisma_client,
    callback_type,
    expected_success_callbacks,
    expected_failure_callbacks,
...
async def test_model_info_alias_without_prisma(hidden):
    ...
    setattr(derouter.proxy.proxy_server, "llm_router", router)
    setattr(derouter.proxy.proxy_server, "llm_model_list", _model_list)
    setattr(derouter.proxy.proxy_server, "prisma_client", None)
...
async def test_proxy_model_group_alias_checks(prisma_client, hidden):  # noqa: F811  # pytest fixture, not a redefinition
    ...
    from derouter.proxy.proxy_server import model_group_info, model_info_v1, model_list
    ...
    _model_list = [
        {
            ...
            "derouter_params": {"model": "cohere/rerank-english-v3.0"},
            "model_info": {
                "mode": "rerank",
            },
        }
    ]
    router = derouter.Router(model_list=_model_list)
    setattr(derouter.proxy.proxy_server, "llm_router", router)
    setattr(derouter.proxy.proxy_server, "llm_model_list", _model_list)
...
#         new_team,
#     )
#     from derouter.proxy._types import TeamMemberAddRequest, Member, NewTeamRequest
...
async def test_proxy_server_prisma_setup():
    from derouter.proxy.proxy_server import ProxyStartupEvent, proxy_state
    from derouter.proxy.utils import ProxyLogging
    ...
    with patch.object(
        derouter.proxy.proxy_server, "PrismaClient", new=MagicMock()
...
async def test_get_ui_settings_spend_logs_threshold():
    ...
    from fastapi import Request
    from derouter.constants import MAX_SPENDLOG_ROWS_TO_QUERY
...
def test_get_timeout_from_request():
    ...
    timeout = DeRouterProxyRequestSetup._get_timeout_from_request(headers)
    assert timeout == 90

    headers = {
        "x-derouter-timeout": "90.5",
    }
    timeout = DeRouterProxyRequestSetup._get_timeout_from_request(headers)
    assert timeout == 90.5


def test_add_derouter_data_for_backend_llm_call_marks_client_side_timeout():
    """A caller-supplied x-derouter-timeout must be marked with client_side_timeout=True,
    ...
    data = DeRouterProxyRequestSetup.add_derouter_data_for_backend_llm_call(
        headers={"x-derouter-timeout": "0.001"},
        request_data={},
    ...
    data_without_header = DeRouterProxyRequestSetup.add_derouter_data_for_backend_llm_call(
        headers={},
        request_data={},
        user_api_key_dict=user_api_key_dict,
    )
...
def test_add_derouter_data_for_backend_llm_call_marks_client_side_timeout_from_body(
    request_data,
):
    ...
    of those directly in the request body, not just via the x-derouter-timeout header.
...
def test_add_derouter_data_for_backend_llm_call_ignores_forged_client_side_timeout():
...
def test_non_root_ui_path_logic(monkeypatch, tmp_path, ui_exists, ui_has_content):
    ...
    Tests that when DEROUTER_NON_ROOT is set to "true":
    ...
    # Create a temporary directory to act as /tmp/derouter_ui
    test_ui_path = tmp_path / "derouter_ui"
    ...
    if os.getenv("DEROUTER_NON_ROOT", "").lower() == "true":
...
async def test_update_config_success_callback_normalization():
    ...
    existing_derouter_settings = {"success_callback": ["langfuse"]}
    ...
    async def fake_find_first(where=None):
        if where and where.get("param_name") == "derouter_settings":
            return FakeRow("derouter_settings", existing_derouter_settings)
        return None
    ...
    class MockPrisma:
        def __init__(self):
            self.db = MagicMock()
            self.db.derouter_config = MagicMock()
            self.db.derouter_config.find_first = AsyncMock(side_effect=fake_find_first)
            self.db.derouter_config.upsert = AsyncMock(side_effect=fake_upsert)
    ...
    config_update = ConfigYAML(derouter_settings={"success_callback": ["SQS", "sQs"]})
    ...
    request = MagicMock()
    request.json = AsyncMock(return_value={"derouter_settings": {"success_callback": ["SQS", "sQs"]}})
    await proxy_server.update_config(config_update, request=request, user_api_key_dict=admin_user)

    assert (
        "derouter_settings" in upserted
    ), "derouter_config.upsert was not called for derouter_settings"
    callbacks = upserted["derouter_settings"]["success_callback"]
    ...
    [
        {
            "model": {
                ...
                "derouter_params": {"model": "azure/gpt-4.1-mini"},
        ...
        },
        {
            "model": {
                ...
                "derouter_params": {"model": "openai/gpt-4.1-mini"},
            },
        ...
        },
        {
            "model": {
                ...
                "derouter_params": {"model": "anthropic/claude-sonnet-4-5@20250929"},
        ...
        },
        {
            "model": {
                ...
                "derouter_params": {"model": "gemini/gemini-2.5-flash@001"},
...
def test_get_derouter_model_info(data):
    from derouter.proxy.proxy_server import get_derouter_model_info

    model = data["model"]
    get_info_mock = MagicMock()

    with mock.patch(
        "derouter.get_model_info",
        new=get_info_mock,
    ):
        get_derouter_model_info(model=model)
