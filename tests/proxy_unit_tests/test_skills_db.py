"""
Test DeRouter Skills SDK with custom_llm_provider=derouter_proxy

Tests the SDK-level skills methods when using the DeRouter database backend:
...
async def test_create_skill_sdk(prisma_client):
    """
    Test creating a skill using SDK with custom_llm_provider=derouter_proxy.
    ...
    from derouter.skills.main import acreate_skill, adelete_skill
    ...
    skill = await acreate_skill(
        ...
        custom_llm_provider=LlmProviders.DEROUTER_PROXY.value,
    )

    # Verify skill was created correctly
    assert skill is not None
    assert skill.id is not None
    assert skill.id.startswith("derouter_skill")
...
async def test_list_skills_sdk(prisma_client):
    """
    Test listing skills using SDK with custom_llm_provider=derouter_proxy.
    ...
    from derouter.skills.main import acreate_skill, adelete_skill, alist_skills
...
async def test_get_skill_sdk(prisma_client):
    """
    Test getting a skill by ID using SDK with custom_llm_provider=derouter_proxy.
    ...
    from derouter.skills.main import acreate_skill, adelete_skill, aget_skill
...
async def test_delete_skill_sdk(prisma_client):
    """
    Test deleting a skill using SDK with custom_llm_provider=derouter_proxy.
