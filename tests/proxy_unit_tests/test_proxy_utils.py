...
from derouter.proxy.utils import _get_docs_url, _get_openapi_url, _get_redoc_url
...
def mock_request(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.proxy.derouter_pre_call_utils.add_derouter_data_to_request", mock_request
    )
    return mock_request
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_thread_endpoint(endpoint, mock_request):
    ...
    assert "derouter_metadata" in data
...
@pytest.mark.asyncio
async def test_add_derouter_data_to_request_non_thread_endpoint(endpoint, mock_request):
    mock_request.url.path = endpoint
    user_api_key_dict = UserAPIKeyAuth(
        api_key="test_api_key", user_id="test_user_id", org_id="test_org_id"
    )
...
async def test_traceparent_not_added_by_default(endpoint, mock_request):
    ...
    setattr(derouter.proxy.proxy_server, "open_telemetry_logger", otel_logger)
    ...
    setattr(derouter.proxy.proxy_server, "open_telemetry_logger", None)
...
async def test_add_key_or_team_level_spend_logs_metadata_to_request(
    ...
    print(data)
    new_data = await add_derouter_data_to_request(
        data, mock_request, user_api_key_dict, proxy_config
    )
...
def test_reading_openai_org_id_from_headers():
    ...
    org_id = DeRouterProxyRequestSetup.get_openai_org_id_from_headers(headers)
...
def test_add_derouter_data_for_backend_llm_call(
    headers, general_settings, expected_data
):
    import json
    ...
    data = DeRouterProxyRequestSetup.get_user_from_headers(
        headers=headers,
...
def test_foward_derouter_user_info_to_backend_llm_call():
    import json

    derouter.add_user_information_to_llm_headers = True
    ...
    data = DeRouterProxyRequestSetup.add_headers_to_llm_call(
        headers={},
    ...
    expected_data = {
        "x-derouter-user_api_key_user_id": "test_user_id",
        "x-derouter-user_api_key_org_id": "test_org_id",
        "x-derouter-user_api_key_hash": "test_api_key",
        "x-derouter-user_api_key_spend": "0.0",
        "x-derouter-user_api_key_auth_metadata": "{}",
    }
...
def test_update_internal_user_params():
    ...
    assert updated_data_json["models"] == derouter.default_internal_user_params["models"]
    assert (
        updated_data_json["max_budget"]
        == derouter.default_internal_user_params["max_budget"]
    )
    assert (
        updated_data_json["budget_duration"]
        == derouter.default_internal_user_params["budget_duration"]
    )
...
async def test_prepare_key_update_data():
    ...
    existing_key_row = MagicMock(spec=DeRouter_VerificationToken)
...
def test_merge_tags(request_tags, tags_to_add, expected_tags):
    ...
    result = DeRouterProxyRequestSetup._merge_tags(
...
async def test_add_derouter_data_to_request_duplicate_tags(
    key_tags, request_tags, expected_tags
):
    """
    ...
    DeRouter_SpendLogs table request_tags column. This can mess up aggregation logic"
    """
    mock_request = Mock(spec=Request)
    mock_request.url.path = "/chat/completions"
...
def test_enforced_params_check(
    ...
    from derouter.proxy.derouter_pre_call_utils import _enforced_params_check
...
def test_get_key_models():
    ...
    from derouter.proxy.auth.model_checks import get_key_models
...
def test_get_team_models():
    ...
    from derouter.proxy.auth.model_checks import get_team_models
...
def test_update_config_fields():
    ...
    all_team_config = updated_config["derouter_settings"]["default_team_settings"]
...
def test_update_config_fields_default_internal_user_params(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "default_internal_user_params", None)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ["openai/*"],
            [{"model_name": "openai/*", "derouter_params": {"model": "openai/*"}}],
            "openai",
        ),
        (
            ["bedrock/*"],
            [{"model_name": "bedrock/*", "derouter_params": {"model": "bedrock/*"}}],
            "bedrock",
        ),
        (
            ["anthropic/*"],
            [{"model_name": "anthropic/*", "derouter_params": {"model": "anthropic/*"}}],
            "anthropic",
        ),
        (
            ["cohere/*"],
            [{"model_name": "cohere/*", "derouter_params": {"model": "cohere/*"}}],
            "cohere",
        ),
    ],
)
...
def test_team_callback_metadata_all_none_values():
    from derouter.proxy._types import TeamCallbackMetadata
...
def test_proxy_config_state_post_init_callback_call(monkeypatch):
    ...
    Addresses issue: https://github.com/Delify-Solutions/derouter/issues/6787
    ...
    callback_metadata = DeRouterProxyRequestSetup.add_team_based_callbacks_from_config(
        team_id="test",
        proxy_config=pc,
    )
    ...
    assert config["derouter_settings"]["default_team_settings"][0]["team_id"] == "test"
...
@pytest.mark.parametrize(
    ...
    [
        (
            {
                "derouter_budget_table_max_budget": None,
                "derouter_budget_table_tpm_limit": None,
                "derouter_budget_table_rpm_limit": 1,
                "derouter_budget_table_model_max_budget": None,
            },
            "rpm_limit",
            1,
        ),
        ...
        (
            {
                "derouter_budget_table_max_budget": None,
                "derouter_budget_table_tpm_limit": None,
                "derouter_budget_table_rpm_limit": None,
                "derouter_budget_table_model_max_budget": {"gpt-4o": 100},
            },
            "model_max_budget",
            {"gpt-4o": 100},
        ),
...
def test_derouter_verification_token_view_response_with_budget_table(
    associated_budget_table,
    expected_user_api_key_auth_key,
    expected_user_api_key_auth_value,
):
    from derouter.proxy._types import DeRouter_VerificationTokenView
    ...
    resp = DeRouter_VerificationTokenView(**args)
...
def test_derouter_verification_token_view_budget_does_not_override_key_model_max_budget():
    """
    ...
    args = {
        ...
        "team_model_aliases": None,
        "team_member": None,
        "derouter_budget_table_model_max_budget": {
...
def test_get_model_group_info():
    from derouter import Router
    from derouter.proxy.proxy_server import _get_model_group_info
...
class MockDb:
    ...
    async def query_raw(self, query: str, *args):
        ...
        filtered_keys = [
            ...
            if k["team_id"] != "derouter-dashboard" or k["team_id"] is None
        ]
...
class MockPrismaClientDB:
    ...
    async def get_data(
        ...
        # Return a proper DeRouter_UserTable object when querying by user_id
        if user_id:
...
async def test_get_user_info_for_proxy_admin(mock_team_data, mock_key_data):
    ...
    from derouter.proxy._types import UserAPIKeyAuth, UserInfoResponse
...
def test_get_temp_budget_increase():
    ...
    from derouter.proxy._types import UserAPIKeyAuth
    from derouter.proxy.auth.user_api_key_auth import _get_temp_budget_increase
...
async def test_health_check_not_called_when_disabled(monkeypatch):
    ...
    monkeypatch.setattr(
        "derouter.proxy.proxy_server.PrismaClient", lambda **kwargs: mock_prisma
    )
...
@patch(
    "derouter.proxy.proxy_server.get_openapi_schema",
    return_value={
        "paths": {
...
def test_custom_openapi(mock_get_openapi_schema):
    from derouter.proxy.proxy_server import custom_openapi

    openapi_schema = custom_openapi()
    assert openapi_schema is not None


from derouter.proxy.utils import ProxyUpdateSpend
...
async def test_spend_logs_cleanup_after_error():
    ...
    mock_client.db.derouter_spendlogs.create_many = AsyncMock(
...
#     from derouter.types.utils import ProviderSpecificHeader
...
@pytest.mark.parametrize(
    "wildcard_model, derouter_params, expected_models",
    [
        (
            "anthropic/*",
            {"model": "anthropic/*"},
...
def test_get_known_models_from_wildcard(
    ...
    from derouter.proxy.auth.model_checks import get_known_models_from_wildcard
    from derouter.types.router import DeRouter_Params

    wildcard_models = get_known_models_from_wildcard(
        wildcard_model=wildcard_model, derouter_params=DeRouter_Params(**derouter_params)
    )
...
def test_get_known_models_from_wildcard_without_derouter_params():
    """
    Test wildcard expansion without derouter_params (BYOK case - team has openai/*
    but no deployment in router config).
    """
    from derouter.proxy.auth.model_checks import get_known_models_from_wildcard

    wildcard_models = get_known_models_from_wildcard(
        wildcard_model="openai/*", derouter_params=None
    )
...
def test_team_alias_stale_bypass_enabled_by_flag(monkeypatch):
    ...
    monkeypatch.setenv("DEROUTER_ENABLE_TEAM_STALE_ALIAS_BYPASS", "true")
...
def mock_prisma_client():
    client = MagicMock()
    client.db = MagicMock()
    client.db.derouter_teamtable = AsyncMock()
    return client
...
async def test_get_admin_team_ids(
    test_id: str,
    user_info: Optional[DeRouter_UserTable],
    user_role: str,
    ...
    # Setup
    mock_prisma_client.db.derouter_teamtable.find_many.return_value = mock_teams
    user_api_key_dict = UserAPIKeyAuth(
    ...
    if should_query_db:
        mock_prisma_client.db.derouter_teamtable.find_many.assert_called_once_with(
            where={"team_id": {"in": user_info.teams}}
        )
    else:
        mock_prisma_client.db.derouter_teamtable.find_many.assert_not_called()
...
async def test_post_call_failure_hook_auth_error_key_info_route():
    ...
    from derouter.proxy._types import ProxyErrorTypes
    ...
    with patch.object(
        ...
        request_data = {
            ...
            "derouter_call_id": "test_call_id_123",
        }
...
async def test_handle_logging_proxy_only_error_syncs_normalized_call_type(
    ...
    original_function_setup = derouter.utils.function_setup
    ...
    with (
        patch(
            "derouter.proxy.utils.derouter.utils.function_setup",
        ...
        patch("derouter.proxy.utils.threading.Thread") as mock_thread,
    ):
...
async def test_during_call_hook_parallel_execution():
    ...
    try:
        derouter.callbacks = [TestGuardrail(f"g{i}") for i in range(3)]
...
async def test_during_call_hook_parallel_execution_with_error():
    ...
    try:
        derouter.callbacks = [FailingGuardrail()]
...
async def test_pre_call_hook_sequential_mutations_precede_parallel_batch():
    ...
    try:
        derouter.callbacks = [parallel_observer, MaskingGuardrail()]
...
async def test_pre_call_hook_parallel_guardrail_blocks_request():
    ...
    try:
        derouter.callbacks = [BlockingGuardrail()]
...
async def test_pre_call_hook_parallel_block_wins_over_reroute():
    ...
    from derouter.caching.caching import DualCache
    from derouter.exceptions import SensitiveDataRouteException
    from derouter.proxy.utils import ProxyLogging
    ...
    try:
        derouter.callbacks = [FastRerouteGuardrail(), SlowBlockingGuardrail()]
...
async def test_pre_call_hook_parallel_awaits_all_when_one_blocks():
    ...
    try:
        derouter.callbacks = [FastBlockingGuardrail(), SlowGuardrail()]
...
async def test_post_call_hook_parallel_guardrail_blocks_response():
    ...
    try:
        derouter.callbacks = [BlockingPostCallGuardrail()]
...
async def test_post_call_hook_parallel_awaits_all_when_one_blocks():
    ...
    try:
        derouter.callbacks = [FastBlockingPostCall(), SlowPostCall()]
...
@pytest.mark.asyncio
async def test_derouter_logging_obj_excluded_from_optional_params():
    """Ensure derouter_logging_obj is excluded from _optional_params to prevent
    ...
    assert "derouter_logging_obj" not in logging_obj.model_call_details
...
def test_handle_exception_on_proxy_preserves_status_code():
    """
    OpenAI batch creation returns 429 for rate limits. DeRouter wraps this as a
    ...
    """
    from derouter.proxy.utils import handle_exception_on_proxy
