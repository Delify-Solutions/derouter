...
async def test_read_config_file_with_os_environ_vars():
    ...
    # Add assertions
    assert config["derouter_settings"]["default_internal_user_params"]["user_role"] == "admin"
    assert config["derouter_settings"]["s3_callback_params"]["s3_aws_access_key_id"] == "1234567890"
    assert config["derouter_settings"]["s3_callback_params"]["s3_aws_secret_access_key"] == "1234567890"

    for model in config["model_list"]:
        if "azure" in model["derouter_params"]["model"]:
            assert model["derouter_params"]["api_key"] == "1234567890"
        elif "fireworks" in model["derouter_params"]["model"]:
            assert model["derouter_params"]["api_key"] == "1234567890"
...
async def test_basic_include_directive():
    ...
    assert config["derouter_settings"]["callbacks"] == ["prometheus"]
...
def test_add_callbacks_from_db_config():
    ...
    # Reset derouter callbacks before test
    ...
    # 1 instance of LangfusePromptManagement should exist in derouter.success_callback
    num_langfuse_instances = sum(
        isinstance(callback, LangfusePromptManagement) for callback in derouter.success_callback
    )
    assert num_langfuse_instances == 1
    assert len(derouter.success_callback) == 2
    assert len(derouter.failure_callback) == 1
...
def test_add_callbacks_invalid_input():
    ...
    # Test Case 2: Missing derouter_settings
    config_data = {}
    proxy_config._add_callbacks_from_db_config(config_data)
...
async def test_json_logs_calls_turn_on_json():
    """
    Test that json_logs: true in derouter_settings calls derouter._turn_on_json()
    ...
    config_content = {
        ...
        "derouter_settings": {"json_logs": True},
    }
    ...
    try:
        ...
        with mock.patch("derouter._turn_on_json") as mock_turn_on_json:
        ...
        assert derouter.json_logs is True

    finally:
        # Cleanup
        os.unlink(temp_file_path)
        derouter.json_logs = False
...
class TestYamlStorePromptsDbOverride:
    ...
    When store_model_in_db=true, DeRouter persists general_settings to the DB.
    ...
    async def test_yaml_value_takes_precedence_over_db(self):
        ...
        with mock.patch("derouter.proxy.proxy_server.general_settings", test_general_settings):
