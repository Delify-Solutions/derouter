...
class TestPollingEndpointPreCallGuard:
    ...
    async def test_rate_limit_error_prevents_polling_id_creation(self):
        ...
        from derouter.proxy.response_api_endpoints.endpoints import responses_api
        from derouter.proxy.response_polling.polling_handler import (
            ResponsePollingHandler,
        )

        rate_limit_exc = derouter.RateLimitError(
            message="TPM limit exceeded",
            llm_provider="",
            model="gpt-4",
        )
        generate_polling_id_mock = MagicMock(return_value="derouter_poll_test")

        proxy_server_patches = {
            "derouter.proxy.proxy_server._read_request_body": AsyncMock(
                return_value={"model": "gpt-4", "background": True}
            ),
            "derouter.proxy.proxy_server.general_settings": {},
            "derouter.proxy.proxy_server.llm_router": MagicMock(),
            "derouter.proxy.proxy_server.native_background_mode": None,
            "derouter.proxy.proxy_server.polling_cache_ttl": 3600,
            "derouter.proxy.proxy_server.polling_via_cache_enabled": True,
            "derouter.proxy.proxy_server.proxy_config": MagicMock(),
            "derouter.proxy.proxy_server.proxy_logging_obj": AsyncMock(),
            "derouter.proxy.proxy_server.redis_usage_cache": AsyncMock(),
            "derouter.proxy.proxy_server.select_data_generator": None,
            "derouter.proxy.proxy_server.user_api_base": None,
            "derouter.proxy.proxy_server.user_max_tokens": None,
            "derouter.proxy.proxy_server.user_model": None,
            "derouter.proxy.proxy_server.user_request_timeout": None,
            "derouter.proxy.proxy_server.user_temperature": None,
            "derouter.proxy.proxy_server.version": "1.0.0",
        }
        ...
        with (
            ...
            patch(
                "derouter.proxy.response_polling.polling_handler.should_use_polling_for_request",
                return_value=True,
            ),
            patch.object(
