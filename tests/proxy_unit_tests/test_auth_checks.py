...
from derouter.proxy.utils import CallInfo
...
async def test_get_end_user_object(customer_spend, customer_budget):
    ...
    _budget = DeRouter_BudgetTable(max_budget=customer_budget)
    end_user_obj = DeRouter_EndUserTable(
        user_id=end_user_id,
        spend=customer_spend,
        derouter_budget_table=_budget,
        blocked=False,
    )
...
async def test_can_key_call_model(model, expect_to_work):
    ...
    router = derouter.Router(model_list=llm_model_list)
    args = {
        "model": model,
...
async def test_can_team_call_model(model, expect_to_work):
    from derouter.proxy.auth.auth_checks import model_in_access_group
    from fastapi import HTTPException
...
async def test_wildcard_access_after_cost_map_reload(key_models, model, expect_to_work):
    ...
    add_known_models(model_cost_map=new_map) must update derouter.anthropic_models
    ...
    derouter.model_cost but never re-ran add_known_models(), so the provider sets
    stayed stale and wildcard matching failed for new models.

    Fix: each reload now calls derouter.add_known_models(model_cost_map=new_map)
    ...
    new_cost_map = dict(derouter.model_cost)
    new_cost_map[model] = {
    ...
    derouter.model_cost = new_cost_map

    # Confirm the model is NOT yet in the provider set before reload propagation.
    assert model not in derouter.anthropic_models

    # Simulate what proxy_server.py now does after every reload.
    derouter.add_known_models(model_cost_map=new_cost_map)

    # After add_known_models(), the model must be in the set.
    assert model in derouter.anthropic_models

    llm_model_list = [
        {
            "model_name": "anthropic/*",
            "derouter_params": {"model": "anthropic/*", "api_key": "test-api-key"},
            "model_info": {"id": "test-id-anthropic-wildcard", "db_model": False},
        },
        {
            "model_name": "openai/*",
            "derouter_params": {"model": "openai/*", "api_key": "test-api-key"},
    ...
    try:
        ...
        derouter.anthropic_models.discard(model)
...
async def test_add_known_models_explicit_map_updates_provider_sets():
    ...
    derouter.add_known_models(model_cost_map=new_model_cost_map) instead of
    ...
    assert fake_new_model not in derouter.anthropic_models
    ...
    try:
        assert fake_new_model in derouter.anthropic_models, (
            "add_known_models(model_cost_map=...) did not add the new model to "
            "derouter.anthropic_models — wildcard access checks would fail."
        )
    finally:
        # Clean up: restore original state.
        derouter.model_cost = original_model_cost
        derouter.anthropic_models.discard(fake_new_model)


@pytest.mark.asyncio
async def test_is_valid_fallback_model():
    from derouter.proxy.auth.auth_checks import is_valid_fallback_model
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "openai/gpt-3.5-turbo"},
            }
        ]
    )
...
async def test_virtual_key_max_budget_check(
    ...
    """
    from derouter.proxy.auth.auth_checks import _virtual_key_max_budget_check
        ...
        model_list=[
            {
                ...
                "derouter_params": {"model": "anthropic/anthropic-claude"},
            },
            {
                "model_name": "gpt-3.5-turbo",
                "derouter_params": {"model": "gpt-3.5-turbo", "api_key": "test-api-key"},
            },
        ]
    )
...
async def test_can_user_call_model_with_no_default_models():
    from derouter.proxy.auth.auth_checks import can_user_call_model
    from derouter.proxy._types import ProxyException, SpecialModelNames
...
async def test_get_fuzzy_user_object():
    from derouter.proxy.auth.auth_checks import _get_fuzzy_user_object
    ...
    mock_prisma.db.derouter_usertable.find_unique = AsyncMock(return_value=test_user)
    result = await _get_fuzzy_user_object(
        prisma_client=mock_prisma, sso_user_id="sso_123", user_email="test@example.com"
    )
    assert result == test_user
    mock_prisma.db.derouter_usertable.find_unique.assert_called_with(
    ...
    assert result == test_user
    mock_prisma.db.derouter_usertable.find_first.assert_called_with(
    ...
    mock_prisma.db.derouter_usertable.update.assert_called_with(
...
async def test_cache_access_object():
    ...
    from derouter.proxy.auth.auth_checks import _cache_access_object
    from derouter.proxy._types import DeRouter_AccessGroupTable

    cache = DualCache()
    ag_id = "ag-test-123"
    ag_table = DeRouter_AccessGroupTable(
...
async def test_delete_cache_access_object():
    ...
    from derouter.proxy.auth.auth_checks import _delete_cache_access_object
    from derouter.proxy._types import DeRouter_AccessGroupTable
...
async def test_get_models_from_access_groups_empty_ids():
    ...
    from derouter.proxy.auth.auth_checks import _get_models_from_access_groups
...
async def test_can_team_access_model_via_access_group_ids():
    ...
    with patch(
        "derouter.proxy.auth.auth_checks._get_models_from_access_groups",
        new_callable=AsyncMock,
        return_value=["gpt-4"],
    ):
        result = await can_team_access_model(
...
async def test_can_key_call_model_via_access_group_ids():
    ...
    router = derouter.Router(
        model_list=[
            {
                "model_name": "gpt-4",
                "derouter_params": {"model": "openai/gpt-4", "api_key": "test"},
            }
        ]
    )
...
async def test_key_access_group_grants_model_when_team_authorized():
    ...
    from derouter.proxy.auth.auth_checks import _key_access_group_grants_model
