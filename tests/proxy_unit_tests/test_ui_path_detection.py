...
Tests the new DEROUTER_UI_PATH and DEROUTER_ASSETS_PATH functionality
...
class TestUIPathEnvironmentVariable:
    """Test DEROUTER_UI_PATH environment variable handling."""

    def test_custom_ui_path_env_var(self):
        """Test that DEROUTER_UI_PATH overrides default."""
        custom_path = "/custom/ui/path"

        with mock.patch.dict(
            os.environ, {"DEROUTER_UI_PATH": custom_path, "DEROUTER_NON_ROOT": "true"}
        ):
            is_non_root = os.getenv("DEROUTER_NON_ROOT", "").lower() == "true"
            default_runtime_ui_path = (
                "/var/lib/derouter/ui" if is_non_root else "/default/packaged/path"
            )
            runtime_ui_path = os.getenv("DEROUTER_UI_PATH", default_runtime_ui_path)

            assert runtime_ui_path == custom_path

    def test_default_ui_path_non_root(self):
        """Test default UI path in non-root mode."""
        with mock.patch.dict(os.environ, {"DEROUTER_NON_ROOT": "true"}, clear=False):
            # Clear DEROUTER_UI_PATH if it exists
            env_copy = os.environ.copy()
            if "DEROUTER_UI_PATH" in env_copy:
                del env_copy["DEROUTER_UI_PATH"]
            ...
            with mock.patch.dict(os.environ, env_copy, clear=True):
                ...
                assert runtime_ui_path == "/var/lib/derouter/ui"


class TestAssetsPathEnvironmentVariable:
    """Test DEROUTER_ASSETS_PATH environment variable handling."""

    def test_custom_assets_path_env_var(self):
        """Test that DEROUTER_ASSETS_PATH overrides default."""
        custom_path = "/custom/assets/path"

        with mock.patch.dict(
            os.environ,
            {"DEROUTER_ASSETS_PATH": custom_path, "DEROUTER_NON_ROOT": "true"},
        ):
            is_non_root = os.getenv("DEROUTER_NON_ROOT", "").lower() == "true"
            default_assets_dir = (
                "/var/lib/derouter/assets" if is_non_root else "/default/current/dir"
            )
    ...
    def test_default_assets_path_non_root(self):
        ...
        env_copy = os.environ.copy()
        env_copy["DEROUTER_NON_ROOT"] = "true"
        if "DEROUTER_ASSETS_PATH" in env_copy:
            del env_copy["DEROUTER_ASSETS_PATH"]
        ...
        with mock.patch.dict(os.environ, env_copy, clear=True):
            ...
            assert assets_dir == "/var/lib/derouter/assets"
...
class TestUIDetectionLogic:
    ...
    def test_marker_file_exists(self):
        ...
        marker_path = os.path.join(self.temp_dir, ".derouter_ui_ready")
