...
def test_check_valid_ip(allowed_ips: Optional[List[str]], client_ip: Optional[str], expected_result: bool):
    from derouter.proxy.auth.auth_utils import _check_valid_ip
...
async def test_check_blocked_team():
    ...
    team_obj = DeRouter_TeamTableCachedObj(team_id=_team_id, blocked=False, last_refreshed_at=time.time())
...
async def test_team_object_has_object_permission_id():
    ...
    setattr(derouter.proxy.proxy_server, "user_api_key_cache", user_api_key_cache)
    setattr(derouter.proxy.proxy_server, "master_key", "sk-1234")
    setattr(derouter.proxy.proxy_server, "prisma_client", "test-client")

    request = Request(scope={"type": "http"})
    request._url = URL(url="/chat/completions")

    with patch("derouter.proxy.auth.user_api_key_auth.common_checks", new_callable=AsyncMock) as mock_common_checks:
...
async def test_returned_user_api_key_auth(user_role, expected_role):
    ...
    new_obj = await _return_user_api_key_auth_obj(
        user_obj=DeRouter_UserTable(user_role=user_role, user_id="", max_budget=None, user_email=""),
        api_key="hello-world",
...
async def test_aaauser_personal_budgets(key_ownership):
    ...
    user_obj = DeRouter_UserTable(user_id=_user_id, spend=11, max_budget=10, user_email="")
    ...
    test_user_cache = getattr(derouter.proxy.proxy_server, "user_api_key_cache")
...
def test_ui_token_route_access(route, user_role, should_be_allowed):
    """
    Verify that UI tokens (team_id=derouter-dashboard) go through the same
    RBAC checks as API tokens. Non-admin dashboard users must not be able
    to access admin-only routes like /config/update.
    """
    from derouter.proxy.auth.auth_checks import _is_api_route_allowed
...
def _assert_api_key_from_custom_header(headers, custom_header_name, expected_api_key):
    ...
    api_key = get_api_key_from_custom_header(request=request, custom_derouter_key_header_name=custom_header_name)
...
def test_allowed_route_inside_route(user_role, auth_user_id, requested_user_id, expected_result):
    from derouter.proxy.auth.auth_checks import allowed_route_check_inside_route
    from derouter.proxy._types import UserAPIKeyAuth, LitellmUserRoles
...
async def test_soft_budget_alert():
    ...
    setattr(derouter.proxy.proxy_server, "user_api_key_cache", user_api_key_cache)
    setattr(derouter.proxy.proxy_server, "master_key", "sk-1234")
    setattr(derouter.proxy.proxy_server, "prisma_client", AsyncMock())
    ...
    alert_called = False
    original_budget_alerts = derouter.proxy.proxy_server.proxy_logging_obj.budget_alerts
...
def test_get_user_role(user_obj, expected_role):
    from derouter.proxy.auth.user_api_key_auth import _get_user_role
...
async def test_user_api_key_auth_websocket():
    ...
    with patch("derouter.proxy.auth.user_api_key_auth.user_api_key_auth", autospec=True) as mock_user_api_key_auth:
...
async def test_jwt_user_api_key_auth_builder_enforce_rbac(enforce_rbac, monkeypatch):
    from derouter.proxy.auth.handle_jwt import JWTHandler, JWTAuthManager
    ...
    monkeypatch.setenv("JWT_AUDIENCE", "api://DeRouter_Proxy-dev")
    ...
    local_cache.set_cache(
        key="derouter_jwt_auth_keys_my-fake-url",
        value=keys,
    )

    derouter_jwtauth = DeRouter_JWTAuth(
        **{
            "admin_jwt_scope": "derouter_proxy_endpoints_access",
...
def test_can_rbac_role_call_model():
    from derouter.proxy.auth.handle_jwt import JWTAuthManager
    from derouter.proxy._types import RoleBasedPermissions
...
def test_get_model_from_request(route, request_data, expected_model):
    from derouter.proxy.auth.user_api_key_auth import get_model_from_request
...
async def test_jwt_non_admin_team_route_access(monkeypatch):
    ...
    monkeypatch.setattr(derouter.proxy.proxy_server, "general_settings", {"enable_jwt_auth": True})

    # Initialize jwt_handler with a default DeRouter_JWTAuth so that the
    # virtual_key_claim_field check in user_api_key_auth doesn't fail with
    # "JWTHandler has no attribute 'derouter_jwtauth'"
    from derouter.proxy._types import DeRouter_JWTAuth
    ...
    with (
        ...
        patch(
            "derouter.proxy.auth.handle_jwt.JWTAuthManager.auth_builder",
...
@pytest.mark.asyncio
async def test_x_derouter_api_key():
    """
    Check if auth can pick up x-derouter-api-key header, even if Bearer token is provided.

    On a master-key match, ``UserAPIKeyAuth.api_key`` (and the derived
    ``token``) are now the stable alias ``DEROUTER_PROXY_MASTER_KEY_ALIAS``
    ...
    valid_token = await user_api_key_auth(
        ...
        custom_derouter_key_header=master_key,
    )
    assert valid_token.token == DEROUTER_PROXY_MASTER_KEY_ALIAS
...
async def test_check_user_model_budget(user_id, user_model_max_budget, expected_calls):
    ...
    stored on DeRouter_UserTable, accepted by /user/new and /user/update, and read
    by nothing, so a user-level per-model budget never blocked anything.
    """
    from derouter.proxy.auth.user_api_key_auth import _check_user_model_budget
...
async def test_user_model_budget_is_enforced_through_user_api_key_auth(over_budget, expect_refusal):
    ...
    from derouter.proxy._types import DeRouter_UserTable, Litellm_EntityType, UserAPIKeyAuth
    from derouter.proxy.auth.user_api_key_auth import user_api_key_auth
    from derouter.proxy.hooks.model_max_budget_limiter import model_budget_spend_cache_key
    from derouter.proxy.proxy_server import (
        hash_token,
    ...
    setattr(derouter.proxy.proxy_server, "user_api_key_cache", user_api_key_cache)
    setattr(derouter.proxy.proxy_server, "master_key", "sk-1234")
    setattr(derouter.proxy.proxy_server, "prisma_client", "present")
...
async def test_jwt_user_model_budget_is_enforced_before_the_jwt_path_returns(over_budget, expect_refusal):
    ...
    """
    from derouter.proxy._types import Litellm_EntityType, UserAPIKeyAuth
    from derouter.proxy.auth.user_api_key_auth import _check_user_model_budget
    from derouter.proxy.hooks.model_max_budget_limiter import model_budget_spend_cache_key
    ...
    if expect_refusal:
        with pytest.raises(derouter.BudgetExceededError) as exc:
            await _check_user_model_budget(
                valid_token=valid_token,
...
def test_jwt_path_enforces_the_user_model_budget_before_returning():
    ...
    from derouter.proxy.auth import user_api_key_auth as auth_module
...
async def test_user_budget_lookup_tolerates_an_unreadable_user():
    ...
    """
    from derouter.caching.dual_cache import DualCache
    from derouter.proxy.auth.user_api_key_auth import _read_user_model_max_budget
