...
_IS_B64 = "derouter.proxy.openai_files_endpoints.common_utils._is_base64_encoded_unified_file_id"
...
def _unmanaged_vertex_file_object(
    input_file_id="gs://bucket/derouter-vertex-files/publishers/google/models/gemini-2.5-flash/abc.jsonl",
    status="validating",
):
    """A DeRouterBatch JSON blob shaped like what the managed-files hook stores for an
...
def _unmanaged_bedrock_file_object(
    input_file_id=(
        "s3://bucket/derouter-bedrock-files-us.anthropic.claude-sonnet-4-20250514-v1-0"
        "-74b61828-9191-4d80-addb-5a0f9ab0ec6a.jsonl"
    ),
    status="validating",
):
    """A DeRouterBatch JSON blob shaped like what gets stored for an unmanaged Bedrock
...
class TestCheckBatchCost:
    ...
    def mock_prisma_client(self):
        client = MagicMock()
        client.db = MagicMock()
        client.db.derouter_managedobjecttable = MagicMock()
        client.db.derouter_usertable = MagicMock()
        return client
    ...
    async def test_cleanup_scoped_to_batch_file_purpose(
        ...
        mock_prisma_client.db.derouter_managedobjecttable.update_many = AsyncMock(
            return_value=1
        )
        # Return empty so the main poll loop exits immediately
        mock_prisma_client.db.derouter_managedobjecttable.find_many = AsyncMock(
            return_value=[]
        )
        ...
        calls = (
            mock_prisma_client.db.derouter_managedobjecttable.update_many.call_args_list
        )
        stale_call = calls[0]
    ...
    async def test_startup_probe_marks_column_absent(
        ...
        mock_prisma_client.db.derouter_managedobjecttable.find_first = AsyncMock(
    ...
    async def test_find_many_uses_pagination_and_excludes_stale(
        ...
        from derouter.constants import MAX_OBJECTS_PER_POLL_CYCLE
        ...
        find_call = mock_prisma_client.db.derouter_managedobjecttable.find_many.call_args
    ...
    async def test_fallback_query_used_when_batch_processed_missing(
        ...
        calls = (
            mock_prisma_client.db.derouter_managedobjecttable.find_many.call_args_list
        )
        assert len(calls) == 2
    ...
    async def test_column_absence_cached_across_cycles(
        ...
        assert (
            mock_prisma_client.db.derouter_managedobjecttable.find_many.call_count == 1
        )
        fallback_where = (
            mock_prisma_client.db.derouter_managedobjecttable.find_many.call_args[1][
                "where"
            ]
        )
        assert "batch_processed" not in fallback_where
    ...
    async def test_fallback_completion_update_omits_batch_processed(
        ...
        mock_prisma_client.db.derouter_managedobjecttable.update = AsyncMock()
        ...
        mock_deployment.derouter_params.custom_llm_provider = "openai"
        mock_deployment.derouter_params.model = "gpt-4"
        ...
        with (
            patch(
                "derouter.proxy.openai_files_endpoints.common_utils._is_base64_encoded_unified_file_id",
                side_effect=[decoded_id, None],
            ),
            patch(
                "derouter.proxy.openai_files_endpoints.common_utils.get_model_id_from_unified_batch_id",
                return_value="model-123",
            ),
            patch(
                "derouter.proxy.openai_files_endpoints.common_utils.get_batch_id_from_unified_batch_id",
                return_value="batch-456",
            ),
            patch(
                "derouter.files.main.afile_content",
                new_callable=AsyncMock,
                return_value=mock_file_content,
            ),
            patch(
                "derouter.batches.batch_utils._get_file_content_as_dictionary",
                return_value=[{"id": "req-1"}],
            ),
            patch(
                "derouter.batches.batch_utils.calculate_batch_cost_and_usage",
                new_callable=AsyncMock,
            ...
            patch(
                "derouter.derouter_core_utils.get_llm_provider_logic.get_llm_provider",
                return_value=("gpt-4", "openai", None, None),
            ),
            patch(
                "derouter.derouter_core_utils.derouter_logging.Logging"
            ) as mock_logging_cls,
        ):
        ...
        assert (
            mock_prisma_client.db.derouter_managedobjecttable.update.call_count == 1
        ), "Expected update() to be called exactly once for the completed job"
        update_data = mock_prisma_client.db.derouter_managedobjecttable.update.call_args[
            1
        ]["data"]
        assert (
    ...
    async def test_output_fetch_passes_deployment_credentials_as_trusted_snapshot(
        ...
        Spreading the credentials as plain kwargs is not enough: get_derouter_params drops
        s3_bucket_name, so without _derouter_internal_model_credentials the cost poller
        cannot read the output file and every completed Bedrock batch stays unbilled.
        """
        from types import MappingProxyType
        from unittest.mock import patch

        mock_prisma_client.db.derouter_managedobjecttable.update_many = AsyncMock(return_value=1)
        mock_prisma_client.db.derouter_managedobjecttable.update = AsyncMock()
        ...
        mock_job.unified_object_id = "dW5pZmllZF9iYXRjaF9pZA=="
        mock_job.created_by = "user-1"
        mock_prisma_client.db.derouter_managedobjecttable.find_many = AsyncMock(return_value=[mock_job])
        ...
        mock_deployment = MagicMock()
        mock_deployment.derouter_params.custom_llm_provider = "bedrock"
        mock_deployment.derouter_params.model = "bedrock/anthropic.claude-haiku-4-5-20251001-v1:0"
        mock_deployment.model_info.model_dump.return_value = {}
        mock_llm_router.get_deployment = MagicMock(return_value=mock_deployment)
        ...
        with (
            ...
            patch("derouter.derouter_core_utils.derouter_logging.Logging") as mock_logging_cls,
        ):
            mock_logging_obj = MagicMock()
            mock_logging_obj.async_success_handler = AsyncMock()
            mock_logging_cls.return_value = mock_logging_obj
        ...
        snapshot = passed_kwargs.get("_derouter_internal_model_credentials")
    ...
    async def test_poller_prices_with_deployment_registered_batch_rates(
        ...
        The deployment's raw model_info dict carries no derouter_params pricing, so passing
        ...
        mock_deployment = MagicMock()
        mock_deployment.derouter_params.custom_llm_provider = "bedrock"
        mock_deployment.derouter_params.model = "bedrock/us.anthropic.claude-haiku-4-5-20251001-v1:0"
        mock_deployment.model_info.model_dump.return_value = {}
        mock_llm_router.get_deployment = MagicMock(return_value=mock_deployment)
    ...
    async def test_poller_masks_api_base_credentials_before_logging(
        ...
        mock_job.unified_object_id = base64.urlsafe_b64encode(
            b"derouter_proxy;model_id:model-123;llm_batch_id:batch-456"
        ).decode()
        mock_job.created_by = "user-1"
        mock_prisma_client.db.derouter_managedobjecttable.find_many = AsyncMock(return_value=[mock_job])
        ...
        mock_deployment = MagicMock()
        mock_deployment.derouter_params.custom_llm_provider = "openai"
        mock_deployment.derouter_params.model = "gpt-5.4-mini"
        mock_deployment.derouter_params.api_base = "https://gateway.example.com/v1?key=AIzaSyVERYSECRET7890"
        mock_deployment.model_info.model_dump.return_value = {}
        mock_llm_router.get_deployment = MagicMock(return_value=mock_deployment)
        ...
        logged_api_base = cost_row_calls[0].args[0].derouter_params["api_base"]
    ...
    async def test_completed_batch_with_no_attributable_owner_still_writes_spend_log(
        ...
        created_by=None and team_id=None on DeRouter_ManagedObjectTable (the table
        ...
        DeRouter_SpendLogs row was ever written.

        Unlike the other tests in this file, this one does NOT mock
        derouter_logging.Logging or async_success_handler -- it runs the real
        ...
        # A real DeRouterBatch (not a bare MagicMock): this test runs the real
        # derouter_logging.Logging pipeline, which type-checks the result via
        # isinstance(..., DeRouterBatch) before it will compute/attach a cost.
        from derouter.types.utils import DeRouterBatch

        mock_response = DeRouterBatch(
            id="batch-1",
            completion_window="24h",
            created_at=1,
            endpoint="/v1/chat/completions",
        ...
        # derouter_logging.Logging pipeline, which calls
        ...
        with (
            ...
            patch.object(derouter, "_async_success_callback", [db_logger]),
            patch(
            ...
            patch("derouter.proxy.proxy_server.increment_spend_counters", AsyncMock()),
            patch("derouter.proxy.proxy_server.update_cache", AsyncMock()),
        ):
            await check_batch_cost_instance.check_batch_cost()

        mock_update_database.assert_awaited_once()
        assert mock_update_database.call_args.kwargs["response_cost"] == 0.01
        assert mock_prisma_client.db.derouter_managedobjecttable.update.call_count == 1, (
    ...
    async def test_cost_tracking_failure_leaves_job_unprocessed(
        ...
        assert (
            mock_prisma_client.db.derouter_managedobjecttable.update.call_count == 0
    ...
    async def test_terminal_status_persists_managed_output_file_ids(
        ...
        unified_input_file_id = base64.urlsafe_b64encode(
            b"derouter_proxy:application/octet-stream;unified_id,in-1;target_model_names,gpt-5-batch"
        ).decode()
        unified_error_file_id = base64.urlsafe_b64encode(
            f"derouter_proxy:application/octet-stream;unified_id,u-2;llm_output_file_id,{raw_error_file_id}".encode()
        ).decode()
        ...
        mock_prisma_client.db.derouter_managedfiletable.find_first = AsyncMock(
        ...
        assert mock_prisma_client.db.derouter_managedobjecttable.update.call_count == 1
        update_call = mock_prisma_client.db.derouter_managedobjecttable.update.call_args
    ...
    async def test_raw_output_file_id_converted_to_managed_id(
        ...
        mock_deployment = MagicMock()
        mock_deployment.derouter_params.custom_llm_provider = "azure"
        mock_deployment.derouter_params.model = "azure/gpt-5-mini"
...
class TestUnmanagedVertexRouting:
    ...
    def _vertex_deployment(self):
        ...
        deployment.derouter_params.custom_llm_provider = "vertex_ai"
        deployment.derouter_params.model = "vertex_ai/gemini-2.5-flash"
        return deployment
    ...
    def test_flag_on_routes_fine_tuned_endpoint_to_vertex_deployment(self):
        ...
        job = self._job(
            file_object=_unmanaged_vertex_file_object(
                input_file_id=f"gs://bucket/derouter-vertex-files/endpoints/{endpoint_id}/abc.jsonl"
            )
        )
    ...
    def test_flag_on_skips_non_vertex_deployment_sharing_model_group(self):
        ...
        non_vertex_deployment.derouter_params.custom_llm_provider = "openai"
        non_vertex_deployment.derouter_params.model = "gpt-4o"
    ...
    def test_flag_on_uses_later_vertex_deployment_with_matching_suffix(self):
        ...
        non_vertex_deployment = MagicMock()
        non_vertex_deployment.derouter_params.custom_llm_provider = "azure"
        non_vertex_deployment.derouter_params.model = "azure/gemini-2.5-flash"
        router.get_deployment = MagicMock(return_value=non_vertex_deployment)
        router.get_model_list.return_value = [
            {
    ...
    async def test_end_to_end_costs_unmanaged_batch(self):
        ...
        prisma.db = MagicMock()
        prisma.db.derouter_managedobjecttable = MagicMock()
        prisma.db.derouter_managedobjecttable.update_many = AsyncMock(return_value=1)
        prisma.db.derouter_managedobjecttable.update = AsyncMock()
        prisma.db.derouter_managedobjecttable.find_many = AsyncMock(
            return_value=[self._job()]
        )
        prisma.db.derouter_usertable = MagicMock()
        prisma.db.derouter_usertable.find_unique = AsyncMock(return_value=None)
        ...
        assert prisma.db.derouter_managedobjecttable.update.call_count == 1
        update_data = prisma.db.derouter_managedobjecttable.update.call_args[1]["data"]
        assert update_data["batch_processed"] is True
        assert update_data["status"] == "complete"
...
class TestUnmanagedBedrockRouting:
    ...
    def _bedrock_deployment(self):
        ...
        deployment.derouter_params.model = "bedrock/us.anthropic.claude-sonnet-4-20250514-v1:0"
        return deployment
    ...
    def test_flag_on_skips_non_bedrock_deployment_sharing_model_group(self):
        ...
        non_bedrock_deployment.derouter_params.custom_llm_provider = "anthropic"
        non_bedrock_deployment.derouter_params.model = "claude-sonnet-4-20250514"
    ...
    def test_flag_on_non_s3_input_is_not_unmanaged_bedrock(self):
        """Flag on, but input_file_id is not a derouter-bedrock-files- s3:// key: treat as
...
class TestManagedOutputFileIdEncodesPublicModelGroup:
    ...
    def _managed_input_file_id(model_group: str) -> str:
        ...
        unified_id = (
            "derouter_proxy:application/octet-stream;unified_id,c4843482-b176-4901-8292-7523fd0f2c6e;"
    ...
    async def _run(self, job: MagicMock) -> str:
        ...
        from derouter.types.utils import DeRouterBatch
        from enterprise.derouter_enterprise.proxy.hooks.managed_files import (
            _PROXY_DeRouterManagedFiles,
        )
        ...
        deployment = MagicMock()
        deployment.derouter_params.custom_llm_provider = "azure"
        deployment.derouter_params.model = "azure/gpt-5.5"
        ...
        hook.get_unified_output_file_id = (
            lambda output_file_id, model_id, model_name: _PROXY_DeRouterManagedFiles.get_unified_output_file_id(
        ...
        with (
            ...
            patch("derouter.derouter_core_utils.derouter_logging.Logging") as logging_cls,
        ):
    ...
    async def test_target_model_names_comes_from_input_file_not_provider_model(self):
        from derouter.proxy.openai_files_endpoints.common_utils import (
...
class TestBatchCostAttribution:
    ...
    def _instance(self, key_row=None, team_row=None, user_row=None):
        from derouter_enterprise.proxy.common_utils.check_batch_cost import CheckBatchCost

        prisma = MagicMock()
        prisma.db.derouter_verificationtoken.find_unique = AsyncMock(return_value=key_row)
    ...
    async def test_metadata_keeps_key_when_team_key_has_no_user(self):
        ...
        instance.prisma_client.db.derouter_usertable.find_unique.assert_not_called()
    ...
    async def test_key_alias_lookup_failure_does_not_break_attribution(self):
        ...
        attribute it."""
        instance = self._instance()
        instance.prisma_client.db.derouter_verificationtoken.find_unique = AsyncMock(
...
class TestPollPageStarvation:
    ...
    def _prisma(self, jobs):
        prisma = MagicMock()
        prisma.db.derouter_managedobjecttable.update_many = AsyncMock(return_value=0)
        prisma.db.derouter_managedobjecttable.update = AsyncMock()
        prisma.db.derouter_managedobjecttable.find_many = AsyncMock(return_value=jobs)
        prisma.db.derouter_usertable.find_unique = AsyncMock(return_value=None)
        return prisma
    ...
    async def test_unified_id_without_model_id_is_retired(self):
        ...
        prisma = self._prisma(
            [self._job("job-no-model", self._encode("derouter_proxy;llm_batch_id:poison-no-model"))]
        )
        llm_router = MagicMock()
        ...
        prisma.db.derouter_managedobjecttable.update.assert_awaited_once()
        call = prisma.db.derouter_managedobjecttable.update.call_args[1]
    ...
    async def test_provider_404_retires_job(self):
        ...
        prisma = self._prisma(
            [
                self._job(
                    "job-gone",
                    self._encode("derouter_proxy;model_id:model-123;llm_batch_id:batch_deadbeef"),
                )
            ]
        )
        llm_router = MagicMock()
        llm_router.aretrieve_batch = AsyncMock(
            side_effect=derouter.NotFoundError(
        ...
        prisma.db.derouter_managedobjecttable.update.assert_awaited_once()
        assert prisma.db.derouter_managedobjecttable.update.call_args[1]["data"] == {
    ...
    async def test_provider_404_with_deployment_gone_keeps_job(self):
        ...
        prisma = self._prisma(
            [
                self._job(
                    "job-misrouted",
                    self._encode("derouter_proxy;model_id:model-gone;llm_batch_id:batch_alive"),
                )
            ]
        )
        llm_router = MagicMock()
        ...
        prisma.db.derouter_managedobjecttable.update.assert_not_awaited()
    ...
    async def test_transient_provider_error_keeps_job_for_retry(self):
        ...
        prisma = self._prisma(
            [
                self._job(
                    "job-flaky",
                    self._encode("derouter_proxy;model_id:model-123;llm_batch_id:batch_flaky"),
                )
            ]
        )
        llm_router = MagicMock()
    ...
    async def test_retirement_falls_back_to_status_without_batch_processed_column(self):
        ...
        prisma = self._prisma(
            [self._job("job-legacy", self._encode("derouter_proxy;llm_batch_id:poison-no-model"))]
        )
    ...
    async def test_stale_cleanup_gives_up_on_never_costed_completed_rows(self):
        ...
        calls = prisma.db.derouter_managedobjecttable.update_many.call_args_list
    ...
    async def test_newer_batch_is_polled_once_dead_rows_are_retired(self):
        ...
        live_row = self._job(
            "job-live",
            self._encode("derouter_proxy;model_id:model-123;llm_batch_id:batch_live"),
        )
        ...
        async def _retrieve(model, batch_id, derouter_metadata):
            if batch_id == "batch_deadbeef":
                raise derouter.NotFoundError(
        ...
        retired = [
            call[1]["where"]["id"]
            for call in prisma.db.derouter_managedobjecttable.update.call_args_list
        ]
    ...
    async def test_404_that_does_not_name_the_batch_keeps_job_for_retry(self):
        ...
        prisma = self._prisma(
            [
                self._job(
                    "job-bad-deployment",
                    self._encode("derouter_proxy;model_id:model-123;llm_batch_id:batch_real"),
                )
            ]
        )
        llm_router = MagicMock()
...
class _FakeManagedObjectTable:
    """A DeRouter_ManagedObjectTable double backed by one real, mutable row.
...
class TestMultiPodBatchCostClaim:
    ...
    the shared DeRouter_ManagedObjectTable, so a completed batch must be claimed atomically
    ...
    def _prisma(row: _FakeManagedObjectRow, journal: list):
        prisma = MagicMock()
        prisma.db.derouter_managedobjecttable = _FakeManagedObjectTable(row, journal)
        prisma.db.derouter_managedfiletable.find_many = AsyncMock(return_value=[])
        prisma.db.derouter_managedfiletable.find_first = AsyncMock(return_value=None)
        prisma.db.derouter_usertable.find_unique = AsyncMock(return_value=None)
        prisma.db.derouter_verificationtoken.find_unique = AsyncMock(return_value=None)
        return prisma
    ...
    def _router():
        ...
        deployment = MagicMock()
        deployment.derouter_params.custom_llm_provider = "openai"
        deployment.derouter_params.model = "gpt-4"
        deployment.model_info.model_dump.return_value = {}
    ...
    def _billing_patches(journal: list, during_fetch=None, bill_error=None):
        ...
        with (
            ...
            patch("derouter.files.main.afile_content", new=AsyncMock(side_effect=_afile_content)),
            patch(
                "derouter.batches.batch_utils._get_file_content_as_dictionary",
                return_value=[{"id": "req-1"}],
            ),
            ...
            patch("derouter.derouter_core_utils.derouter_logging.Logging", return_value=logging_obj),
        ):
            yield logging_obj
    ...
    def _claim_calls(prisma) -> list:
        return [
            call.kwargs
            for call in prisma.db.derouter_managedobjecttable.update_many.call_args_list
    ...
    async def _run_deletion_guard(prisma, file_id: str) -> None:
        ...
        from derouter_enterprise.proxy.hooks.managed_files import _PROXY_DeRouterManagedFiles

        cache = MagicMock()
        cache.async_get_cache = AsyncMock(return_value=None)
        cache.async_set_cache = AsyncMock()
        guard = _PROXY_DeRouterManagedFiles(internal_usage_cache=cache, prisma_client=prisma)

        scheduler = MagicMock()
        scheduler.get_job.return_value = MagicMock()
        with patch("derouter.proxy.proxy_server.scheduler", scheduler):
