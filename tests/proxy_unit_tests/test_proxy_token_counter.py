...
from derouter.proxy._types import ProxyException, TokenCountRequest
from derouter.proxy.anthropic_endpoints.endpoints import (
    count_tokens as anthropic_count_tokens,
)
from derouter.proxy.proxy_server import token_counter
...
async def test_vLLM_token_counting():
    ...
    setattr(derouter.proxy.proxy_server, "llm_router", llm_router)
...
async def test_anthropic_messages_count_tokens_endpoint():
    ...
    """
    from derouter.proxy.anthropic_endpoints.endpoints import count_tokens
    ...
    import derouter.proxy.anthropic_endpoints.endpoints as anthropic_endpoints
...
async def test_internal_token_counter_anthropic_provider_detection():
    ...
    assert response.total_tokens > 0
    assert response.tokenizer_type == "openai_tokenizer"  # Should use DeRouter tokenizer
...
async def test_factory_anthropic_endpoint_calls_anthropic_counter():
    ...
    with patch(
        "derouter.llms.anthropic.count_tokens.token_counter.anthropic_count_tokens_handler",
        mock_handler,
    ):
            ...
            mock_router.model_list = [
                {
                    ...
                    "derouter_params": {"model": "anthropic/claude-3-5-sonnet-20241022"},
                    "model_info": {},
                }
            ]
...
async def test_factory_gpt4_endpoint_does_not_call_anthropic_counter():
    ...
    with patch(
        ...
        # Mock derouter token counter
        with patch("derouter.token_counter") as mock_derouter_counter:
            mock_derouter_counter.return_value = 50
...
async def test_factory_normal_token_counter_endpoint_does_not_call_anthropic():
    ...
    with patch(
        ...
        # Mock derouter token counter
        with patch("derouter.token_counter") as mock_derouter_counter:
            mock_derouter_counter.return_value = 35
...
async def test_factory_registration():
    ...
    non_anthropic_deployment = {"derouter_params": {"model": "openai/gpt-4"}}
...
async def test_bedrock_count_tokens_endpoint():
    ...
    async def mock_count_tokens_handler(request_data, derouter_params, resolved_model):
...
async def test_vertex_ai_anthropic_token_counting():
    ...
    with patch(
        "derouter.llms.vertex_ai.vertex_ai_partner_models.count_tokens.handler.VertexAIPartnerModelsTokenCounter.handle_count_tokens_request"
...
def test_vertex_ai_partner_models_token_counting_endpoint(vertex_location):
    ...
    """
    from derouter.llms.vertex_ai.vertex_ai_partner_models.count_tokens.handler import (
...
async def test_bedrock_token_counter_error_propagation_bedrock_error():
    ...
    with patch.object(
        ...
        with patch(
            "derouter.llms.bedrock.count_tokens.bedrock_token_counter.BedrockCountTokensHandler"
        ) as MockHandler:
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch.object(
                        ...
                        with patch(
                            "derouter.llms.bedrock.count_tokens.handler.get_async_httpx_client"
                        ) as mock_client:
...
async def test_token_counter_httpx_status_error_raises_proxy_exception():
    ...
    original_get_provider_token_counter = (
        derouter.proxy.proxy_server._get_provider_token_counter
    )
    original_router = derouter.proxy.proxy_server.llm_router
    ...
    try:
        ...
        derouter.proxy.proxy_server.llm_router = original_router
...
async def test_proxy_token_counter_error_raises_exception_when_disabled():
    ...
    setattr(derouter.proxy.proxy_server, "llm_router", mock_router)

    # Save original value and function
    original_disable = derouter.disable_token_counter
    original_get_provider_token_counter = (
        derouter.proxy.proxy_server._get_provider_token_counter
    )

    try:
        derouter.disable_token_counter = True
        ...
        derouter.disable_token_counter = original_disable
        derouter.proxy.proxy_server._get_provider_token_counter = (
            original_get_provider_token_counter
        )
...
async def test_proxy_token_counter_error_falls_back_when_enabled():
    ...
    try:
        derouter.disable_token_counter = False
