...
def test_load_config_with_custom_role_names():
    config = {
        "general_settings": {
            "derouter_proxy_roles": {"admin_jwt_scope": "derouter-proxy-admin"}
        }
    }
    proxy_roles = DeRouter_JWTAuth(
        **config.get("general_settings", {}).get("derouter_proxy_roles", {})
    )

    print(f"proxy_roles: {proxy_roles}")

    assert proxy_roles.admin_jwt_scope == "derouter-proxy-admin"
...
async def test_token_single_public_key(monkeypatch):
    ...
    await cache.async_set_cache(
        key="derouter_jwt_auth_keys_https://example.com/public-key",
...
@pytest.mark.parametrize("audience", [None, "derouter-proxy"])
@pytest.mark.asyncio
async def test_valid_invalid_token(audience, monkeypatch):
    ...
    await cache.async_set_cache(
        key="derouter_jwt_auth_keys_https://example.com/public-key", value=[public_jwk]
    )
...
async def test_team_token_output(prisma_client, audience, monkeypatch):
    ...
    jwt_handler.derouter_jwtauth = DeRouter_JWTAuth(team_id_jwt_field="client_id")
    ...
    setattr(derouter.proxy.proxy_server, "jwt_handler", jwt_handler)
    try:
...
async def aaaatest_user_token_output(
    ...
    import derouter
    from derouter.proxy._types import NewTeamRequest, NewUserRequest, UserAPIKeyAuth
    ...
    jwt_handler.derouter_jwtauth.user_id_jwt_field = "sub"
    jwt_handler.derouter_jwtauth.team_id_default = default_team_id
    jwt_handler.derouter_jwtauth.user_id_upsert = user_id_upsert

    if team_id_set:
        jwt_handler.derouter_jwtauth.team_id_jwt_field = "client_id"
    ...
    ## 1. INITIAL TEAM CALL - should fail
    # use generated key to auth in
    setattr(derouter.proxy.proxy_server, "general_settings", {"enable_jwt_auth": True})
    setattr(derouter.proxy.proxy_server, "jwt_handler", jwt_handler)
    try:
        result = await user_api_key_auth(request=request, api_key=bearer_token)
        pytest.fail("Team doesn't exist. This should fail")
...
async def test_allowed_routes_admin(
    ...
    pseudo_routes = jwt_handler.derouter_jwtauth.admin_allowed_routes

    actual_routes = []
    for route in pseudo_routes:
        if route in DeRouterRoutes.__members__:
            actual_routes.extend(DeRouterRoutes[route].value)
...
async def test_allow_access_by_email(
    ...
    Relevant issue: https://github.com/Delify-Solutions/derouter/issues/5605
    """
    import jwt
    from starlette.datastructures import URL
    ...
    setattr(derouter.proxy.proxy_server, "jwt_handler", jwt_handler)
    setattr(derouter.proxy.proxy_server, "prisma_client", {})
    ...
    # ),
    with patch.object(
        derouter.proxy.auth.handle_jwt,
        "get_user_object",
...
async def test_end_user_jwt_auth(monkeypatch):
    ...
    from derouter.types.router import RouterGeneralSettings
    ...
    setattr(derouter.proxy.proxy_server, "prisma_client", {})
    setattr(derouter.proxy.proxy_server, "jwt_handler", jwt_handler)
    from derouter.proxy.proxy_server import cost_tracking
    ...
    mock_response = derouter.ModelResponse(
        id="chatcmpl-mock",
        choices=[
            derouter.Choices(
                finish_reason="stop",
    ...
    with patch(
        "derouter.acompletion", new=AsyncMock(return_value=mock_response)
    ) as mock_completion:
...
def test_check_scope_based_access(requested_model, should_work):
    from derouter.proxy.auth.handle_jwt import JWTAuthManager
    from derouter.proxy._types import ScopeMapping
    ...
    args = {
        "scope_mappings": [
            ScopeMapping(
                ...
                scope="derouter.api.consumer",
            ),
            ScopeMapping(
                models=["gpt-3.5-turbo-testing"],
                routes=None,
                scope="derouter.api.gpt_3_5_turbo",
            ),
        ],
        "scopes": [
            "profile",
            "groups-scope",
            "email",
            "derouter.api.gpt_3_5_turbo",
            "derouter.api.consumer",
        ],
        "request_data": {
...
async def test_custom_validate_called():
    ...
    jwt_handler.derouter_jwtauth = MagicMock(
...
async def test_multi_issuer_jwt_missing_mapped_claim_is_optional(monkeypatch):
    ...
    is just absent — matching the global ``derouter_jwtauth`` behaviour.
    """
    monkeypatch.delenv("JWT_AUDIENCE", raising=False)
    monkeypatch.delenv("JWT_PUBLIC_KEY_URL", raising=False)
    ...
    assert claims[JWTHandler.DEROUTER_JWT_ISSUER_CLAIM] == issuer
