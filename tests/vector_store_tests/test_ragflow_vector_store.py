...
from derouter.llms.ragflow.vector_stores.transformation import RAGFlowVectorStoreConfig
from derouter.derouter_core_utils.derouter_logging import Logging as DeRouterLoggingObj
from derouter.types.vector_stores import VectorStoreCreateOptionalRequestParams
...
class TestRAGFlowVectorStore(BaseVectorStoreTest):
    ...
    def test_get_auth_credentials(self):
        ...
        derouter_params = {"api_key": "test-api-key-123"}
        credentials = config.get_auth_credentials(derouter_params)
    ...
    def test_get_complete_url(self):
        ...
        derouter_params = {"api_base": "http://custom-host:9999"}
        url = config.get_complete_url(api_base=None, derouter_params=derouter_params)
        assert url == "http://custom-host:9999/api/v1/datasets"

        # Test with api_base parameter
        url = config.get_complete_url(
            api_base="http://test-host:8888", derouter_params={}
        )
    ...
    def test_validate_environment(self):
        ...
        # Test with api_key in derouter_params
        ...
        with pytest.raises(ValueError, match="RAGFLOW_API_KEY"):
            config.validate_environment({}, GenericDeRouterParams())
    ...
    async def test_basic_create_vector_store(self, sync_mode):
        ...
        try:
            if sync_mode:
                response = derouter.vector_stores.create(
                    name=f"test-ragflow-{int(__import__('time').time())}", **create_args
                )
            else:
                response = await derouter.vector_stores.acreate(
                    name=f"test-ragflow-{int(__import__('time').time())}", **create_args
                )
        ...
        print("derouter create response=", json.dumps(response, indent=4, default=str))
