...
from derouter.llms.bedrock.vector_stores.transformation import BedrockVectorStoreConfig
...
class TestBedrockVectorStore(BaseVectorStoreTest):
    ...
    def test_get_filename_from_metadata(self):
        ...
        metadata_with_path = {
            "x-amz-bedrock-kb-source-uri": "https://router.delify.vn/tutorial/setup.html"
        }
        ...
        metadata_domain_only = {"x-amz-bedrock-kb-source-uri": "https://www.router.delify.vn"}
        filename = config._get_filename_from_metadata(metadata_domain_only)
        assert filename == "www.router.delify.vn"
...
async def test_bedrock_search_with_credentials_managed_registry():
    ...
    original_credential_list = getattr(derouter, "credential_list", [])

    try:
        # Set up test AWS credentials in the credential system
        test_credentials = CredentialItem(
            credential_name="bedrock-derouter-website-knowledgebase",
            credential_info={
                "provider": "aws",
        ...
        derouter.credential_list = [test_credentials]
        ...
        vector_store = DeRouter_ManagedVectorStore(
            ...
            derouter_credential_name="bedrock-derouter-website-knowledgebase",
        )

        # Set up registry
        registry = VectorStoreRegistry([vector_store])
        derouter.vector_store_registry = registry
        ...
        with patch.object(
            ...
            with patch(
                "derouter.vector_stores.main.base_llm_http_handler.vector_store_search_handler"
            ) as mock_handler:
                ...
                derouter_params = call_kwargs.get("derouter_params", {})
                ...
                assert hasattr(
                    derouter_params, "aws_access_key_id"
                ), "aws_access_key_id should be in derouter_params"
                assert hasattr(
                    derouter_params, "aws_secret_access_key"
                ), "aws_secret_access_key should be in derouter_params"
                assert hasattr(
                    derouter_params, "aws_region_name"
                ), "aws_region_name should be in derouter_params"
        ...
        derouter.credential_list = original_credential_list
