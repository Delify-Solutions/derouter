...
import derouter
from derouter.llms.azure_ai.vector_stores.transformation import AzureAIVectorStoreConfig
from derouter.types.utils import EmbeddingResponse
from derouter.vector_stores import (
...
async def test_basic_search_vector_store(sync_mode):
    ...
    base_request_args = {
        ...
        "derouter_embedding_model": "azure_ai/text-embedding-3-large",
...
async def test_transform_uses_injected_executor_without_embedding_config(respx_mock: respx.MockRouter):
    ...
    logging_details = transform_kwargs["derouter_logging_obj"].model_call_details
...
def test_transform_falls_back_to_sdk_embedding_without_executor(
    ...
    transform_kwargs["derouter_params"] = {
        "derouter_embedding_model": "openai/text-embedding-3-small",
        "derouter_embedding_config": {"api_base": "https://embedding.example/v1", "api_key": "store-key"},
    }
...
def test_transform_requires_embedding_model():
    ...
    transform_kwargs["derouter_params"] = {"derouter_embedding_config": {"api_key": "store-key"}}
