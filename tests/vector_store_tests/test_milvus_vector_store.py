...
from derouter.llms.milvus.vector_stores.transformation import MilvusVectorStoreConfig
from derouter.types.utils import EmbeddingResponse
from derouter.vector_stores import asearch as vector_store_asearch
from derouter.vector_stores import search as vector_store_search
...
class TestMilvusVectorStore:
    ...
    async def test_basic_search_with_mock_async(self):
        ...
        with patch("derouter.aembedding", new_callable=AsyncMock) as mock_embedding:
            ...
            with patch(
                ...
                response = await vector_store_asearch(
                    ...
                    derouter_embedding_model="text-embedding-3-large",
    ...
    def test_backend_configured_db_and_partition_are_forwarded(self):
        """milvus_db_name / milvus_partition_names from derouter_params must be sent."""
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = MOCK_MILVUS_SEARCH_RESPONSE
        mock_response.text = json.dumps(MOCK_MILVUS_SEARCH_RESPONSE)
...
#         pytest.skip("Skipping test due to derouter.InternalServerError")
...
def test_sdk_search_with_router_kwarg_resolves_bare_embedding_alias_sync(
    ...
    response = derouter.vector_stores.search(router=_alias_router(), **ALIAS_SEARCH_KWARGS)
...
async def test_sdk_search_with_router_kwarg_resolves_bare_embedding_alias_async(
    ...
    response = await derouter.vector_stores.asearch(router=_alias_router(), **ALIAS_SEARCH_KWARGS)
...
async def test_sdk_search_with_router_kwarg_rejects_team_alias_without_team_metadata(
    ...
    with pytest.raises(derouter.APIConnectionError):
        await derouter.vector_stores.asearch(router=_team_alias_router(), **ALIAS_SEARCH_KWARGS)
...
async def test_transform_uses_injected_executor_without_embedding_config(respx_mock: respx.MockRouter):
    ...
    transform_kwargs = {
        ...
        "derouter_params": {"derouter_embedding_model": "multilingual-e5-large", "milvus_db_name": "docs"},
...
def test_transform_falls_back_to_sdk_embedding_without_executor_or_config(
    ...
    _, body = MilvusVectorStoreConfig().transform_search_vector_store_request(
        ...
        api_base="https://milvus.example",
        derouter_logging_obj=logging_obj,
        derouter_params={"derouter_embedding_model": "openai/text-embedding-3-small"},
    )
...
def test_transform_requires_embedding_model():
    with pytest.raises(ValueError, match="derouter_embedding_model is required"):
        MilvusVectorStoreConfig().transform_search_vector_store_request(
            ...
            derouter_params={"derouter_embedding_config": {"api_key": "store-key"}},
