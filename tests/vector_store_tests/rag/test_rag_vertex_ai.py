...
class TestRAGVertexAI(BaseRAGTest):
    ...
    async def query_vector_store(
        ...
        """
        Query Vertex AI RAG corpus using DeRouter's vector store search.
        ...
        try:
            # Use DeRouter's vector store search
            search_response = await derouter.vector_stores.asearch(
    ...
    async def test_create_corpus_and_ingest(self):
        ...
        try:
            response = await derouter.rag.aingest(
            ...
            pytest.skip(f"Skipping test due to derouter.InternalServerError: {e}")
        except Exception as e:
