...
async def test_mcp_server_manager_https_server():
    ...
    with patch(
        "derouter.proxy._experimental.mcp_server.mcp_server_manager.MCPClient",
...
async def test_sse_mcp_handler_mock():
    ...
    with (
        ...
        patch(
            "derouter.proxy._experimental.mcp_server.server.sse_session_manager",
        ...
        from derouter.proxy._experimental.mcp_server.server import handle_sse_mcp
...
async def test_list_tools_rest_api_server_not_found():
    ...
    with patch(
        "derouter.proxy._experimental.mcp_server.rest_endpoints.global_mcp_server_manager"
    ) as mock_manager:
...
async def test_get_tools_from_mcp_servers():
    ...
    try:
        ...
        with patch(
            ...
            with patch(
                "derouter.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp.MCPRequestHandler._get_mcp_servers_from_access_groups",
...
async def test_mcp_server_manager_access_groups_from_config():
    ...
    import derouter.proxy._experimental.mcp_server.mcp_server_manager as mcp_server_manager_mod
...
async def test_mcp_server_manager_config_integration_with_database():
    ...
    db_server = DeRouter_MCPServerTable(
    ...
    # Test 3: Test config server conversion to DeRouter_MCPServerTable format
...
def test_get_server_prefix_with_alias():
    ...
    """
    from derouter.proxy._experimental.mcp_server.utils import get_server_prefix
...
async def test_add_update_server_with_alias():
    ...
    # Create a mock DeRouter_MCPServerTable with alias
...
async def test_add_update_server_without_alias():
    ...
    # Create a mock DeRouter_MCPServerTable without alias
    mock_mcp_server = MagicMock()
    mock_mcp_server.server_id = "test-server-123"
...
async def test_add_update_server_fallback_to_server_id():
    ...
    # Create a mock DeRouter_MCPServerTable without alias or server_name
    mock_mcp_server = MagicMock()
    mock_mcp_server.server_id = "test-server-123"
    mock_mcp_server.alias = None
...
def test_normalize_server_name():
    ...
    """
    from derouter.proxy._experimental.mcp_server.utils import normalize_server_name
...
def test_add_server_prefix_to_name():
    ...
    from derouter.proxy._experimental.mcp_server.utils import add_server_prefix_to_name
...
async def test_list_tool_rest_api_with_server_specific_auth():
    ...
    from derouter.proxy._experimental.mcp_server.rest_endpoints import list_tool_rest_api
    from derouter.proxy._experimental.mcp_server.auth.user_api_key_auth_mcp import (
        MCPRequestHandler,
    )
    from derouter.proxy._types import UserAPIKeyAuth
...
async def test_call_mcp_tool_uses_manager_permission_lookup():
    ...
    with (
        ...
        patch(
            "derouter.proxy._experimental.mcp_server.server.global_mcp_tool_registry"
        ) as mock_tool_registry,
        patch(
            "derouter.proxy._experimental.mcp_server.server._handle_managed_mcp_tool",
            new_callable=AsyncMock,
