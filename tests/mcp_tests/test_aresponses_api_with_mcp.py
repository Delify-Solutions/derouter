...
import derouter
from derouter.responses.mcp.derouter_proxy_mcp_handler import DeRouter_Proxy_MCP_Handler
...
async def test_mcp_helper_methods():
    """Test the core MCP helper methods in DeRouter_Proxy_MCP_Handler"""

    # Test _should_use_derouter_mcp_gateway
    mcp_tools: List[Any] = [
        {"type": "mcp", "server_url": "derouter_proxy", "require_approval": "never"}
    ]
    ...
    # Should return True for MCP tools with derouter_proxy
    assert DeRouter_Proxy_MCP_Handler._should_use_derouter_mcp_gateway(mcp_tools) == True

    # Should return False for other tools
    assert (
        DeRouter_Proxy_MCP_Handler._should_use_derouter_mcp_gateway(other_tools) == False
    )

    # Should return False for None
    assert DeRouter_Proxy_MCP_Handler._should_use_derouter_mcp_gateway(None) == False

    # Test _parse_mcp_tools
    mixed_tools = mcp_tools + other_tools
    mcp_parsed, other_parsed = DeRouter_Proxy_MCP_Handler._parse_mcp_tools(mixed_tools)
    ...
    assert DeRouter_Proxy_MCP_Handler._should_auto_execute_tools(mcp_tools_never) == True
    assert (
        DeRouter_Proxy_MCP_Handler._should_auto_execute_tools(mcp_tools_always) == False
    )
    ...
    assert DeRouter_Proxy_MCP_Handler._should_auto_execute_tools(mcp_tools_mixed) == False
    mcp_tools_manual = [{"require_approval": "never"}, {"require_approval": "manual"}]
    assert DeRouter_Proxy_MCP_Handler._should_auto_execute_tools(mcp_tools_manual) == False
    assert DeRouter_Proxy_MCP_Handler._should_auto_execute_tools([]) == False
...
async def test_mcp_output_elements_addition():
    ...
    updated_response = DeRouter_Proxy_MCP_Handler._add_mcp_output_elements_to_response(
...
async def test_aresponses_api_with_mcp_mock_integration():
    ...
    """
    # Define MCP tools with derouter_proxy server_url and require_approval="never"
    mcp_tools: List[OpenAIMcpServerTool] = [
        {
            "type": "mcp",
            "server_url": "derouter_proxy",
    ...
    should_use_mcp = DeRouter_Proxy_MCP_Handler._should_use_derouter_mcp_gateway(
        cast(Any, mcp_tools)
    )
    assert (
        should_use_mcp == True
    ), "Should detect MCP tools with derouter_proxy server_url"

    # Test 2: Verify auto-execution detection works
    should_auto_execute = DeRouter_Proxy_MCP_Handler._should_auto_execute_tools(
        cast(Any, mcp_tools)
    )
    assert (
    ...
    mcp_parsed, other_parsed = DeRouter_Proxy_MCP_Handler._parse_mcp_tools(
        cast(Any, mcp_tools)
    )
    ...
    assert mcp_parsed[0]["server_url"] == "derouter_proxy", "Should preserve server_url"
    assert (
...
async def test_aresponses_api_with_mcp_passes_mcp_server_auth_headers_to_process_tools():
    ...
    """
    from derouter.responses.main import aresponses_api_with_mcp
    ...
    mcp_tools = [{"type": "mcp", "server_url": "derouter_proxy"}]
    secret_fields = {
    ...
    with (
        ...
        patch(
            "derouter.responses.main.aresponses",
...
async def test_mcp_allowed_tools_filtering():
    ...
    filtered_tools = DeRouter_Proxy_MCP_Handler._filter_mcp_tools_by_allowed_tools(
        mcp_tools=mock_mcp_tools_from_server,
        mcp_tools_with_derouter_proxy=cast(
    ...
    filtered_tools_all = DeRouter_Proxy_MCP_Handler._filter_mcp_tools_by_allowed_tools(
        mcp_tools=mock_mcp_tools_from_server,
        mcp_tools_with_derouter_proxy=cast(
    ...
    mock_mcp_tools_with_duplicates = [
        ...
        type(
            ...
            (),
            {
                "name": "GitMCP-fetch_derouter_documentation",
                "description": "Fetch entire documentation file from GitHub repository: Delify-Solutions/derouter. Useful for general questions. Always call this tool first if asked about Delify-Solutions/derouter.",
                "inputSchema": {
                    "type": "object",
                    "properties": {},
        ...
        type(
            ...
            (),
            {
                "name": "GitMCP-search_derouter_documentation",
                "description": "Semantically search within the fetched documentation from GitHub repository: Delify-Solutions/derouter. Useful for specific queries.",
                "inputSchema": {
                    "type": "object",
                    "properties": {"query": {"type": "string"}},
                    "required": ["query"],
    ...
    mcp_tool_config_with_duplicates = [
        {
            "type": "mcp",
            "server_label": "derouter",
            "server_url": "derouter_proxy/mcp",
            "require_approval": "never",
            "allowed_tools": ["GitMCP-fetch_derouter_documentation"],
        }
    ]

    # First filter by allowed tools
    filtered_tools_with_duplicates = (
        DeRouter_Proxy_MCP_Handler._filter_mcp_tools_by_allowed_tools(
    ...
    filtered_tools_deduplicated, _ = DeRouter_Proxy_MCP_Handler._deduplicate_mcp_tools(
        filtered_tools_with_duplicates, []
    )
    ...
    assert (
        filtered_tools_deduplicated[0].name == "GitMCP-fetch_derouter_documentation"
    ), f"Expected GitMCP-fetch_derouter_documentation, got {filtered_tools_deduplicated[0].name}"

    print("✓ Test Case 3: duplicate tools are properly deduplicated")

    # Test Case 3b: Test standalone deduplication method
    standalone_deduplicated, _ = DeRouter_Proxy_MCP_Handler._deduplicate_mcp_tools(
        mock_mcp_tools_with_duplicates, allowed_mcp_servers
    )

    # Should return 2 unique tools (GitMCP-fetch_derouter_documentation and GitMCP-search_derouter_documentation)
    assert (
    ...
    expected_unique_names = [
        ...
        "GitMCP-search_derouter_documentation",
    ]
    ...
    filtered_tools_multiple = (
        DeRouter_Proxy_MCP_Handler._filter_mcp_tools_by_allowed_tools(
            mcp_tools=mock_mcp_tools_from_server,
            mcp_tools_with_derouter_proxy=cast(List[ToolParam], multiple_mcp_configs),
        )
    )
    ...
    filtered_tools_empty = DeRouter_Proxy_MCP_Handler._filter_mcp_tools_by_allowed_tools(
        mcp_tools=mock_mcp_tools_from_server,
        mcp_tools_with_derouter_proxy=cast(List[ToolParam], mcp_config_empty_allowed),
    )
...
async def test_streaming_mcp_events_validation():
    ...
    from derouter.types.llms.openai import ResponsesAPIStreamEvents
    ...
    mock_mcp_tools = [
        type(
            ...
            {
                "name": "search_repo",
                "description": "Search Delify-Solutions/derouter repository for information",
                "inputSchema": {
                    "type": "object",
                    "properties": {
    ...
    with (
                    ...
                    results.append(
                        {
                            ...
                            "result": "DeRouter is a unified interface for 100+ LLMs.",
                        }
                    )
            return results
        ...
        mcp_tool_config = {
            "type": "mcp",
            "server_url": "derouter_proxy/mcp/test_server",
            ...
            input=[
                {
                    "role": "user",
                    "type": "message",
                    "content": "What is DeRouter? Give me a brief overview.",
                }
            ],
            stream=True,
        )
...
async def test_streaming_responses_api_with_mcp_tools(
    ...
    """
    Test the streaming responses API with MCP tools when using server_url="derouter_proxy"

    Under the hood the follow occurs

    - MCP: responses called derouter MCP manager.list_tools (MOCKED)
    ...
    with caplog.at_level(logging.ERROR):
        with (
            ...
            mock_get_tools.return_value = (mock_mcp_tools, ["derouter_proxy"])
                ...
                for tool_call in tool_calls:
                    ...
                    if call_id:
                        results.append(
                            {
                                "tool_call_id": call_id,
                                "result": "DeRouter is a unified interface for 100+ LLMs that translates inputs to provider-specific completion endpoints and provides consistent OpenAI-format output.",
                            }
                        )
                return results
                ...
                input=[
                    {
                        "role": "user",
                        "type": "message",
                        "content": "give me a TLDR of what Delify-Solutions/derouter is about",
                    }
                ],
                stream=True,
            )
    ...
    lite_errors = [
        ...
        and ("DeRouter" in record.name or "DeRouter" in record.getMessage())
    ]
    assert not lite_errors, "Unexpected DeRouter errors: " + ", ".join(
...
async def test_mcp_parameter_preparation_helpers():
    ...
    initial_params_auto = DeRouter_Proxy_MCP_Handler._prepare_initial_call_params(
    ...
    initial_params_no_auto = DeRouter_Proxy_MCP_Handler._prepare_initial_call_params(
    ...
    follow_up_params = DeRouter_Proxy_MCP_Handler._prepare_follow_up_call_params(
    ...
    request_params = DeRouter_Proxy_MCP_Handler._build_request_params(
        input=input_data,
        model=model,
        all_tools=tools,
    ...
    request_params_no_prev = DeRouter_Proxy_MCP_Handler._build_request_params(
        input=input_data,
        model=model,
        all_tools=tools,
        call_params=call_params,
...
async def test_mcp_tool_execution_events_creation():
    ...
    mock_tool_calls = [
        {
            "id": "call_abc123",
            "name": "search_repo",
            "arguments": '{"query": "DeRouter overview"}',
            "type": "function_call",
        },
        {
            "id": "call_def456",
            "name": "get_repo_info",
            "arguments": '{"repo_name": "Delify-Solutions/derouter"}',
            "type": "function_call",
        },
    ]
    ...
    mock_tool_results = [
        ...
        },
        {
            ...
            "result": "Repository: Delify-Solutions/derouter - Python library for LLM integration",
        },
    ]

    # Create tool execution events
    execution_events = DeRouter_Proxy_MCP_Handler._create_tool_execution_events(
    ...
    empty_events = DeRouter_Proxy_MCP_Handler._create_tool_execution_events(
...
async def test_no_duplicate_mcp_tools_in_streaming_e2e():
    ...
    with (
        ...
        patch("derouter.aresponses", side_effect=capture_llm_tools) as mock_aresponses,
    ):
