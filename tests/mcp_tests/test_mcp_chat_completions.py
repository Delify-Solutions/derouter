...
async def test_acompletion_mcp_auto_exec(monkeypatch):
    ...
    async def fake_process(user_api_key_auth, mcp_tools_with_derouter_proxy, **kwargs):
    ...
    response = await derouter.acompletion(
        ...
        tools=[
            {
                "type": "mcp",
                "server_url": "derouter_proxy/mcp/local",
                "server_label": "local",
...
async def test_completion_mcp_with_streaming_no_timeout_error(monkeypatch):
    """
    Test that derouter.completion with stream=True and MCP tools does not raise
    ...
    with patch("derouter.acompletion", side_effect=mock_acompletion):
