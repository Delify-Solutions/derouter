...
from derouter.proxy._experimental.mcp_server.tool_search import handle_mcp_proxy_tool
from derouter.proxy._types import DeRouter_ObjectPermissionTable, ProxyException, UserAPIKeyAuth
...
def _clear_proxy_database_env() -> typing.Iterator[None]:
    ...
    # the DEROUTER_MASTER_KEY env var, overriding whatever initialize() set from
    # the config file. We must set it here so the lifespan doesn't reset it to None.
    mp.setenv("DEROUTER_MASTER_KEY", "sk-1234")
    try:
        yield
    finally:
        mp.undo()
...
def _proxy_server(
    ...
    config["derouter_settings"]["callbacks"] = [f"{__name__}.proxy_call_recorder"]
...
class TestProxyMcpStatelessBehavior:
    """
    Verify that the DeRouter MCP proxy operates in stateless mode.
    ...
    Regression test for https://github.com/Delify-Solutions/derouter/issues/20242
    """
...
async def authorize_proxy_key(request: Request, api_key: str) -> UserAPIKeyAuth:
    permissions = {
        "sk-1234": DeRouter_ObjectPermissionTable(object_permission_id="open", mcp_servers=["math_stdio"]),
        "sk-restricted": DeRouter_ObjectPermissionTable(
            object_permission_id="restricted", mcp_servers=["math_restricted"]
        ),
        "sk-none": DeRouter_ObjectPermissionTable(object_permission_id="none", mcp_servers=["no-mcp-servers"]),
        "sk-add-only": DeRouter_ObjectPermissionTable(
...
class TestProxyMcpAuthorizationScope:
    ...
    async def test_proxy_scope_exception_returns_iserror_and_emits_failure_log(self, proxy_server_url: str) -> None:
        async with _scoped_session(
            proxy_server_url,
            "sk-none",
            **{"x-mcp-servers": "math_restricted", "x-derouter-call-id": "proxy-scope-denial"},
        ) as session:
