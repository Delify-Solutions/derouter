...
from derouter.proxy._experimental.mcp_server.oauth2_token_cache import (  # noqa: E402
    MCPPerUserTokenCache,
    _compute_per_user_token_ttl,
    mcp_per_user_token_cache,
)
from derouter.types.mcp import MCPAuth, MCPTransport  # noqa: E402
from derouter.types.mcp_server.mcp_server_manager import MCPServer  # noqa: E402
...
class TestComputePerUserTokenTtl:
    ...
    def test_default_ttl_when_expires_in_none(self):
        from derouter.constants import MCP_PER_USER_TOKEN_DEFAULT_TTL
...
class TestMCPPerUserTokenCache:
    ...
    async def test_get_returns_none_on_miss(self, cache, mock_dual_cache):
        with (
            patch(
                "derouter.proxy._experimental.mcp_server.oauth2_token_cache.decrypt_value_helper"
            ) as mock_decrypt,
            patch("derouter.proxy.proxy_server.user_api_key_cache", mock_dual_cache),
        ):
    ...
    async def test_set_encrypts_before_storing(self, cache, mock_dual_cache):
        ...
        with (
            patch(
                "derouter.proxy._experimental.mcp_server.oauth2_token_cache.encrypt_value_helper",
    ...
    async def test_delete_calls_async_delete_cache(self, cache, mock_dual_cache):
        ...
        with patch("derouter.proxy.proxy_server.user_api_key_cache", mock_dual_cache):
...
class TestRefreshUserOauthToken:
    ...
    async def test_returns_none_when_no_refresh_token(self, server):
        from derouter.proxy._experimental.mcp_server.db import refresh_user_oauth_token
    ...
    async def test_returns_none_on_http_error(self, server, cred):
        ...
        with patch(
            "derouter.proxy._experimental.mcp_server.db.get_async_httpx_client",
    ...
    async def test_stores_and_returns_new_credential(self, server, cred):
        ...
        with (
            ...
            patch(
                "derouter.proxy._experimental.mcp_server.db.get_user_oauth_credential",
                new_callable=AsyncMock,
