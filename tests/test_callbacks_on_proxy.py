...
def _summarize(all_derouter_callbacks) -> Counter:
    return Counter(_normalize_callback(str(c)) for c in all_derouter_callbacks)
...
async def get_active_callbacks(session):
    ...
    async with session.get(url, headers=headers) as response:
        ...
        all_derouter_callbacks = _json_response["all_derouter_callbacks"]

        print("current number of callbacks: ", _num_callbacks)
        print("current number of alerts: ", _num_alerts)
        return _num_callbacks, _num_alerts, all_derouter_callbacks
