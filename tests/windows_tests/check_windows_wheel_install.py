"""Reproduce a default-Windows ``pip install derouter`` to catch the 260-char
...
def main():
    ...
    if _run(["uv", "pip", "install", "--python", python, wheel]) != 0:
        ...
        return 1
    if _run([python, "-c", "import derouter; import derouter.types.utils"]) != 0:
        print("::error::derouter did not import after install (half-unpacked package)")
        return 1
