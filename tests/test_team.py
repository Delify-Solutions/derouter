    ...
    async with aiohttp.ClientSession() as session:
        ...
        for k, v in new_team_data["data"].items():
            if k == "members_with_roles":
                ...
                or k == "model_id"
                or k == "derouter_organization_table"
                or k == "object_permission_id"
                or k == "object_permission"
                or k == "derouter_model_table"
                or k == "policies"
...
async def test_users_in_team_budget():
    ...
    async with aiohttp.ClientSession() as session:
            ...
            for team_info in user_info_after["teams"]:
                if team_info.get("team_id") == team["team_id"]:
                    ...
                    for membership in team_info.get("team_memberships", []):
                        ...
                        if "derouter_budget_table" in membership:
                            budget_table = membership["derouter_budget_table"]
                    ...
                    if team_info.get("team_id") == team["team_id"]:
                        for membership in team_info.get("team_memberships", []):
                            if "derouter_budget_table" in membership:
                                ...
                                error_msg += f"Team member max budget: {membership['derouter_budget_table'].get('max_budget')}\n"
        ...
        assert (
            user_info["teams"][0]["team_memberships"][0]["derouter_budget_table"][
                "max_budget"
            ]
            == 0.0000001
        )
