...
def analyze_function(func_info: FunctionInfo) -> Dict:
    ...
    for name, type_name in func_info.parameters:
        if type_name.endswith("Request") or type_name.endswith("Response"):
            pydantic_model = getattr(derouter.proxy._types, type_name, None)
...
def main():
    ...
    # directory = "../../derouter/proxy/management_endpoints"  # LOCAL
    directory = "./derouter/proxy/management_endpoints"
