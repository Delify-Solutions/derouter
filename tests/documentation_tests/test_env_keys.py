...
# Define the base directory for the derouter repository and documentation path
repo_base = "./derouter"  # Change this to your actual path
...
ENV_KEY_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"(?<![\w.])(?:derouter\.(?:utils\.)?)?get_secret(?:_str|_bool)?" + _GET_SECRET_ARGS),
)
...
# Directories to skip (dependencies, venvs, caches) - only scan derouter source
SKIP_DIRS = {
    ".venv",
    "venv",
    "__pycache__",
...
def main() -> None:
    if not os.path.isdir(DOCS_BASE):
        raise Exception(f"No documentation found at {DOCS_BASE}; check out Delify-Solutions/derouter-docs into docs/my-website")
