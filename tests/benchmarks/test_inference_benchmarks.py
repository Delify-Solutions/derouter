...
def test_completion_simple_message():
    ...
    derouter.completion(model="gpt-4o", messages=SIMPLE_MESSAGES, mock_response=MOCK_RESPONSE)


@pytest.mark.benchmark
def test_completion_multi_turn():
    """Benchmark a multi-turn completion through the full SDK path."""
    derouter.completion(model="gpt-4o", messages=MULTI_TURN_MESSAGES, mock_response=MOCK_RESPONSE)
