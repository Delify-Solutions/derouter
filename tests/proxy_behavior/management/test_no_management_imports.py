...
FORBIDDEN_IMPORT = re.compile(r"^\s*from\s+derouter\.proxy\.management_endpoints\b")
...
def test_no_management_endpoint_imports():
    ...
    assert not violations, (
        "tests/proxy_behavior/ must not import from derouter.proxy.management_endpoints. "
        "Violations:\n  " + "\n  ".join(violations)
    )
