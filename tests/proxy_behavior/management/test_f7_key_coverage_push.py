...
async def _seed_token(
    ...
    *,
    ...
    await prisma.db.derouter_verificationtoken.create(data=data)
    return cleartext
...
async def test_regenerate_happy_path(proxy_client, prisma, scratch, world):
    ...
    old_row = await prisma.db.derouter_verificationtoken.find_unique(
        where={"token": old_hash}
    )
    assert old_row is None, "old token still present after regenerate"

    # New token should be active.
    new_row = await prisma.db.derouter_verificationtoken.find_unique(
...
# DeRouter_DeprecatedVerificationToken; assert it lands there.
# ---------------------------------------------------------------------------
...
async def test_regenerate_with_grace_period_inserts_deprecated_row(
    ...
    deprecated_row = await prisma.db.derouter_deprecatedverificationtoken.find_unique(
        where={"token": old_hash}
    )
    ...
    await prisma.db.derouter_deprecatedverificationtoken.delete(
        where={"token": old_hash}
    )
