...
DeRouter_TeamMembership row reflects the resolved user_id (the regression
...
async def test_member_add_both_none_rejected(proxy_client, prisma, scratch, world):
    ...
    rows = await prisma.db.derouter_teammembership.find_many(where={"team_id": team_id})
...
async def test_member_add_email_only_resolves_user_id(
    ...
    # derouter_teammembership rows are only written when a per-member budget
...
async def test_member_add_unknown_user_id_upserted(
    ...
    pre = await prisma.db.derouter_usertable.find_unique(where={"user_id": new_user_id})
    assert pre is None
    seeder = world.keys[Actor.PROXY_ADMIN].cleartext
    resp = await proxy_client.post(
        "/team/member_add",
    ...
    assert resp.status_code == 200, resp.text
    post = await prisma.db.derouter_usertable.find_unique(where={"user_id": new_user_id})
