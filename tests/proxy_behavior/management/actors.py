...
async def _wipe_world(prisma: PrismaClient) -> None:
    ...
    await prisma.db.derouter_budgettable.delete_many(where={"budget_id": BUDGET_ID})


async def seed_world(prisma: PrismaClient) -> World:
    await _wipe_world(prisma)

    await prisma.db.derouter_budgettable.create(
        data={
            "budget_id": BUDGET_ID,
    ...
    for org_id, alias in [(ORG_A, "alpha"), (ORG_B, "beta")]:
        await prisma.db.derouter_organizationtable.create(
            data={
    ...
    await prisma.db.derouter_teamtable.create(
        data={
            "team_id": TEAM_ALPHA,
            "team_alias": "alpha-1",
            "organization_id": ORG_A,
    ...
    for actor, team_id in [
        ...
        await prisma.db.derouter_teammembership.create(
    ...
    for actor, profile in profiles.items():
        ...
        token_data: Dict[str, Any] = {
            ...
            "user_id": user_ids[actor],
            # DeRouter_VerificationTokenView's models field rejects NULL even
