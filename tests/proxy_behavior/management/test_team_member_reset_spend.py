...
async def test_team_member_reset_spend_authz_matrix(
    ...
    row = await prisma.db.derouter_teammembership.find_unique(
...
async def test_team_member_reset_spend_missing_membership_is_404(
    ...
    """A well-formed team but a user_id with no DeRouter_TeamMembership row is 404."""
...
async def test_team_member_reset_spend_team_admin_cannot_reset_own_spend(
    ...
    """A team admin targeting their own DeRouter_TeamMembership row is 403: unchecked, an
