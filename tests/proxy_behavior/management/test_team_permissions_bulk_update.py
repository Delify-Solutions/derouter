...
async def test_bulk_update_specific_team_only_mutates_listed(
    ...
    for tid in world_team_ids + [bystander]:
        row = await prisma.db.derouter_teamtable.find_unique(where={"team_id": tid})
    ...
    target_row = await prisma.db.derouter_teamtable.find_unique(
    ...
    for tid, before_perms in before.items():
        after_row = await prisma.db.derouter_teamtable.find_unique(
            where={"team_id": tid}
        )
...
async def test_bulk_update_unknown_team_id_rejected(
    ...
    before = list(
        (
            await prisma.db.derouter_teamtable.find_unique(where={"team_id": existing})
        ).team_member_permissions
        or []
    )
    seeder = world.keys[Actor.PROXY_ADMIN].cleartext
    ...
    after_real = await prisma.db.derouter_teamtable.find_unique(
...
async def test_bulk_update_apply_to_all_mutates_every_team(
    ...
    all_teams = await prisma.db.derouter_teamtable.find_many()
    ...
    try:
        ...
        post = await prisma.db.derouter_teamtable.find_many()
        for team in post:
        ...
        for team_id, prior_perms in snapshot.items():
            current = await prisma.db.derouter_teamtable.find_unique(
