...
async def test_org_team_guaranteed_throughput_model_over_bound_rejected(
    ...
    rows = await prisma.db.derouter_teamtable.find_many(where={"team_id": team_id})
    assert rows == []
...
async def test_key_generate_with_unknown_project_id_rejected(
    ...
    rows = await prisma.db.derouter_verificationtoken.find_many(
...
async def _seed_key_for_relocation(
    ...
    await prisma.db.derouter_verificationtoken.create(
        data={
...
async def test_team_new_with_team_member_budget_creates_budget_row(
    ...
    Observable end-state: a derouter_budgettable row is created and the
    ...
    assert resp.status_code == 200, resp.text
    team_row = await prisma.db.derouter_teamtable.find_unique(where={"team_id": team_id})
    ...
    try:
        budget_row = await prisma.db.derouter_budgettable.find_unique(
        ...
        await prisma.db.derouter_teamtable.update(
            where={"team_id": team_id},
            data={"metadata": Json({})},
        )
        await prisma.db.derouter_budgettable.delete(where={"budget_id": budget_id})
...
async def test_key_team_change_accepted_by_target_team_admin(
    ...
    assert resp.status_code == 200, resp.text
    row = await prisma.db.derouter_verificationtoken.find_unique(
