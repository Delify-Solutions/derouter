...
async def proxy_app():
    ...
    # proxy_startup_event re-reads master_key from DEROUTER_MASTER_KEY and
    ...
    # ambient DEROUTER_MASTER_KEY with a different value would make the proxy
    # authenticate on that key while the tests still send MASTER_KEY.
    os.environ["DEROUTER_MASTER_KEY"] = MASTER_KEY
...
async def create_scratch_team(
    ...
    *,
    ...
    columns (Phase 4 F1+F3) — they live directly on DeRouter_TeamTable, no
    ...
    await prisma.db.derouter_teamtable.create(data=data)
    return team_id
...
async def create_scratch_user(
    ...
    *,
    ...
    await prisma.db.derouter_usertable.create(data=data)
    return user_id
...
async def create_scratch_org(
    ...
    *,
    ...
    The org's `budget_id` points at a fresh `derouter_budgettable` row that
    carries the per-org limits (`_check_org_key_limits` and the team budget
    helpers read `org_table.derouter_budget_table.<limit>`, not columns on the
    ...
    await prisma.db.derouter_budgettable.create(data=budget_data)
    ...
    await prisma.db.derouter_organizationtable.create(data=org_data)
    return org_id
...
async def create_scratch_actor(
    ...
    *,
    ...
    derouter_usertable, derouter_verificationtoken, and
    derouter_organizationmembership by scratch prefix — no bespoke cleanup
    needed. Does NOT write derouter_teammembership against world teams: the
    ...
    models=[] satisfies DeRouter_VerificationTokenView.
    """
    user_id = f"{scratch_prefix}-{suffix}"
    cleartext = "sk-" + uuid.uuid4().hex
    hashed = hash_token(cleartext)
    await prisma.db.derouter_usertable.create(
        data={
            "user_id": user_id,
            "user_role": user_role,
    ...
    await prisma.db.derouter_verificationtoken.create(data=token_data)
    for org_id in org_admin_of:
        await prisma.db.derouter_organizationmembership.create(
            data={
                "user_id": user_id,
                "organization_id": org_id,
                "user_role": "org_admin",
...
async def scratch(prisma):
    ...
    try:
        ...
        await prisma.db.derouter_verificationtoken.delete_many(
            where={
                "OR": [
        ...
        await prisma.db.derouter_teammembership.delete_many(
            where={"team_id": {"startswith": handle.prefix}}
        )
        await prisma.db.derouter_organizationmembership.delete_many(
            where={"user_id": {"startswith": handle.prefix}}
        )
        await prisma.db.derouter_teamtable.delete_many(
            where={"team_id": {"startswith": handle.prefix}}
        )
        ...
        await prisma.db.derouter_usertable.delete_many(
            where={
                "OR": [
        ...
        # the only other writer of DeRouter_OrganizationTable and uses the
        ...
        await prisma.db.derouter_organizationtable.delete_many(
            where={"organization_id": {"startswith": handle.prefix}}
        )
        await prisma.db.derouter_budgettable.delete_many(
            where={"budget_id": {"startswith": handle.prefix}}
        )
        # /team/member_add writes DeRouter_UserTable.teams; the available-team
        ...
        polluted = await prisma.db.derouter_usertable.find_many(
        ...
        for user in polluted:
            ...
            if cleaned != list(user.teams):
                await prisma.db.derouter_usertable.update(
