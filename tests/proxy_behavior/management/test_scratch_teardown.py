...
async def test_a2_scratch_actor_lands_in_db(proxy_client, prisma, scratch):
    ...
    user_row = await prisma.db.derouter_usertable.find_unique(
    ...
    memberships = await prisma.db.derouter_organizationmembership.find_many(
        where={"user_id": actor.user_id}
    )
...
async def test_b_scratch_namespace_is_clean(prisma):
    tokens = await prisma.db.derouter_verificationtoken.find_many(
        where={"key_alias": {"startswith": SCRATCH_PREFIX}}
    )
    users = await prisma.db.derouter_usertable.find_many(
