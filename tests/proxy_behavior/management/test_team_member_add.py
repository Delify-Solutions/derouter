...
async def test_team_member_add_authz_matrix(
    ...
    row = await prisma.db.derouter_teamtable.find_unique(
...
async def test_team_member_add_unprovisioned_user_id_is_proxy_admin_only(
    ...
    assert (
        await prisma.db.derouter_usertable.find_unique(
    ...
    assert row is not None
    created = await prisma.db.derouter_usertable.find_unique(
        where={"user_id": unprovisioned_id}
    )
    if expected_status == 200:
...
async def test_team_member_add_email_invite_open_to_team_admin(
    ...
    invited = await prisma.db.derouter_usertable.find_first(
...
# in derouter.default_internal_user_params["available_teams"], but the bypass
...
async def test_team_member_add_available_team_self_join(
    ...
    monkeypatch.setattr(
        derouter, "default_internal_user_params", {"available_teams": [scratch.prefix]}
    )
