"""
Behavior tests for the DeRouter_AutoRouterSession conditional upsert and the benchmarks
...
tests/test_derouter/proxy/db/test_autorouter_session_rollup.py.
"""
...
async def _row(db, key: str, session_id: str = "s1", router: str = "auto-1") -> dict:
    rows = await db.query_raw(
        'SELECT * FROM "DeRouter_AutoRouterSession" WHERE api_key = $1 AND session_id = $2 AND router_name = $3',
        key,
        session_id,
        router,
    )
