...
async def make_text_completion_request():
    return await derouter.atext_completion(
        model="openai/gpt-4o",
...
async def test_acompletion_memory():
    """Test memory usage for derouter.acompletion"""
...
async def test_atext_completion_memory():
    """Test memory usage for derouter.atext_completion"""
...
async def make_router_atext_completion_request():
    return await derouter_router.atext_completion(
        model="text-gpt-4o",
        temperature=0.5,
...
async def make_router_acompletion_request():
    return await derouter_router.acompletion(
        model="chat-gpt-4o",
        messages=[{"role": "user", "content": "Test message for memory usage"}],
        api_base="https://exampleopenaiendpoint-production.up.railway.app/",
    )
