...
def test_default_deployment_isolation():
    ...
    2. derouter_params dict

    Deeper nested objects are intentionally shared for performance (safe because
    the router only modifies the 'model' field at derouter_params level).

    Critical behavior verified:
    1. Each deployment gets independent model value
    2. Original default_deployment unchanged for derouter_params fields
    3. Shared fields (api_key) accessible in all copies
    4. Adding new derouter_params fields is isolated per deployment
    ...
    assert deployment1["derouter_params"]["model"] == "custom-model-1"  # type: ignore
    assert deployment2["derouter_params"]["model"] == "custom-model-2"  # type: ignore

    # Assert: Original default_deployment must remain unchanged (not mutated by requests)
    assert router.default_deployment["derouter_params"]["model"] == "gpt-5-mini"  # type: ignore

    # Assert: Shared fields should still be accessible in all copies
    assert deployment1["derouter_params"]["api_key"] == "test-key"  # type: ignore
    assert deployment2["derouter_params"]["api_key"] == "test-key"  # type: ignore

    # Assert: Modifying derouter_params in one deployment doesn't affect others
    # This tests the shallow copy properly isolated the derouter_params dict level
    deployment1["derouter_params"]["temperature"] = 0.9  # type: ignore
    assert "temperature" not in deployment2["derouter_params"]  # type: ignore
    assert "temperature" not in router.default_deployment["derouter_params"]  # type: ignore

    # Assert: Deep nested objects ARE shared (intentional trade-off for 100x perf gain)
    # Safe because router only modifies top-level derouter_params fields
    deployment1["derouter_params"]["custom_config"]["nested_setting"] = "modified"  # type: ignore
    assert deployment2["derouter_params"]["custom_config"]["nested_setting"] == "modified"  # type: ignore
    assert router.default_deployment["derouter_params"]["custom_config"]["nested_setting"] == "modified"  # type: ignore
