...
async def test_router_acancel_batch(router):
    """Test that router.acancel_batch() calls derouter.acancel_batch with correct params"""
    mock_response = MagicMock()
    mock_response.id = "batch_123"
    mock_response.status = "cancelled"

    with patch.object(derouter, "acancel_batch", new_callable=AsyncMock) as mock_cancel:
...
async def test_router_acancel_batch_resolves_credential_name():
        ...
        model_list=[
            {
                ...
                "derouter_params": {
                    ...
                    "derouter_credential_name": "openai-test-credential",
                },
            }
        ]
    )
    ...
    try:
        with patch.object(
            derouter, "acancel_batch", new_callable=AsyncMock
        ) as mock_cancel:
            mock_cancel.return_value = mock_response
        ...
        assert "derouter_credential_name" not in call_kwargs
    finally:
        derouter.credential_list = []
...
async def test_router_acancel_batch_removes_unresolved_credential_name():
        ...
        model_list=[
            {
                ...
                "derouter_params": {
                    "model": "openai/gpt-5.5",
                    "derouter_credential_name": "missing-openai-credential",
                },
            }
        ]
    )
    ...
    with (
        ...
        patch.object(derouter, "acancel_batch", new_callable=AsyncMock) as mock_cancel,
    ):
        mock_cancel.return_value = mock_response
