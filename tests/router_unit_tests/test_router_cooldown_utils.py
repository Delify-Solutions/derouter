...
from derouter.router import Deployment, DeRouter_Params
    ...
    return Router(
        model_list=[
            ...
            },
            {
                ...
                "derouter_params": {"model": "openai/test_deployment"},
                "model_id": "test_deployment_2",
            },
            {
                "model_name": "test_deployment",
                "derouter_params": {"model": "openai/test_deployment-2"},
...
def test_should_run_cooldown_logic(testing_derouter_router):
    testing_derouter_router.disable_cooldowns = True
    # don't run cooldown logic if disable_cooldowns is True
    assert (
        _should_run_cooldown_logic(
            testing_derouter_router, "test_deployment", 500, Exception("Test")
        )
        is False
    )

    # don't cooldown if deployment is None
    testing_derouter_router.disable_cooldowns = False
    assert (
        _should_run_cooldown_logic(testing_derouter_router, None, 500, Exception("Test"))
        is False
    )

    # don't cooldown if it's a provider default deployment
    testing_derouter_router.provider_default_deployment_ids = ["test_deployment"]
    assert (
        _should_run_cooldown_logic(
            testing_derouter_router, "test_deployment", 500, Exception("Test")
        )
...
def single_deployment_router():
    ...
    "dep-1" (unlike `testing_derouter_router`'s top-level "model_id" key, which
...
def test_should_run_cooldown_logic_generic_bad_request_excluded_by_default(
    ...
    exc = derouter.BadRequestError("bad request", "openai", "gpt-5-mini")
    assert (
...
def test_should_run_cooldown_logic_explicit_deployment_level_policy_overrides_content_policy_exclusion(
    ...
    exc = derouter.ContentPolicyViolationError("flagged content", "openai", "gpt-5-mini")
...
class TestHasExplicitAllowedFailsPolicyForException:
    ...
    def test_router_level_policy_for_matching_exception_returns_false(
        ...
        exc = derouter.RateLimitError("rate limited", "openai", "gpt-5-mini")
        single_deployment_router.allowed_fails_policy = AllowedFailsPolicy(
    ...
    def test_deployment_level_policy_for_matching_exception_returns_true(
        ...
        exc = derouter.ContentPolicyViolationError("flagged", "openai", "gpt-5-mini")
        deployment_dict = single_deployment_router.get_model_info(id="dep-1")
        deployment_dict["model_info"]["allowed_fails_policy"] = {
            "ContentPolicyViolationErrorAllowedFails": 0
        }
...
def test_should_cooldown_deployment_rate_limit_error(testing_derouter_router):
    """
    ...
    _exception = derouter.exceptions.RateLimitError(
        "Rate limit", "openai", "gpt-5-mini"
    )
    assert (
        _should_cooldown_deployment(
            testing_derouter_router, "test_deployment", 429, _exception
        )
        is True
    )


def test_should_cooldown_deployment_auth_limit_error(testing_derouter_router):
    """
    ...
    _exception = derouter.exceptions.AuthenticationError(
        "Unauthorized", "openai", "gpt-5-mini"
    )
    assert (
        _should_cooldown_deployment(
            testing_derouter_router, "test_deployment", 401, _exception
        )
        is True
    )
...
@pytest.mark.asyncio
async def test_should_cooldown_deployment(testing_derouter_router):
    """
    ...
    available_deployment = testing_derouter_router.get_available_deployment(
    ...
    for _ in range(40):
        increment_deployment_successes_for_current_minute(
            derouter_router_instance=testing_derouter_router, deployment_id=deployment_id
        )
    ...
    for _ in range(41):
        tasks.append(
            testing_derouter_router.acompletion(
                model=deployment_id,
    ...
    assert (
        _should_cooldown_deployment(
            testing_derouter_router, deployment_id, 500, Exception("Test")
        )
        is True
    )
...
def test_increment_deployment_successes_for_current_minute_does_not_write_to_redis(
    ...
    testing_derouter_router.cache = DualCache(
        redis_cache=mock_redis_cache, in_memory_cache=InMemoryCache()
    )

    # Call the function we're testing
    increment_deployment_successes_for_current_minute(
        derouter_router_instance=testing_derouter_router, deployment_id="test_deployment"
    )
    ...
    print(
        ...
        testing_derouter_router.cache.in_memory_cache.cache_dict,
    )
    assert (
        testing_derouter_router.cache.in_memory_cache.get_cache(
...
@patch(
    "derouter.router_utils.cooldown_handlers.get_deployment_successes_for_current_minute"
)
@patch(
    "derouter.router_utils.cooldown_handlers.get_deployment_failures_for_current_minute"
)
def test_should_cooldown_high_traffic_all_fails(mock_failures, mock_successes, router):
    # Simulate 10 failures, 0 successes
    from derouter.constants import SINGLE_DEPLOYMENT_TRAFFIC_FAILURE_THRESHOLD
...
def test_is_cooldown_required_empty_string_exception_status(testing_derouter_router):
    """
    Test that _is_cooldown_required returns False when exception_status is an empty string
    """
    result = _is_cooldown_required(
        derouter_router_instance=testing_derouter_router,
        model_id="test_deployment",
        exception_status="",
    )
...
def test_should_cooldown_deployment_minimum_request_threshold(testing_derouter_router):
    """
    ...
    """
    from derouter.constants import DEFAULT_FAILURE_THRESHOLD_MINIMUM_REQUESTS
    ...
    _exception = derouter.exceptions.InternalServerError(
        "Internal error", "openai", "gpt-5-mini"
    )

    # With only 1 request, should NOT cooldown (below minimum threshold)
    should_cooldown = _should_cooldown_deployment(
        testing_derouter_router, deployment_id, 500, _exception
    )
    assert (
        should_cooldown is False
