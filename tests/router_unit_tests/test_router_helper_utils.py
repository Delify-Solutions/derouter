...
def test_routing_strategy_init(model_list):
    ...
    from derouter.types.router import RoutingStrategy
...
def test_print_deployment(model_list):
    ...
    assert 10 * "*" in printed_deployment["derouter_params"]["api_key"]
...
def test_print_deployment_with_redact_enabled(model_list):
    ...
    original_setting = derouter.redact_user_api_key_info
    try:
        derouter.redact_user_api_key_info = True
        printed_deployment = router.print_deployment(deployment)

        assert "*" in printed_deployment["derouter_params"]["aws_access_key_id"]
        assert "*" in printed_deployment["derouter_params"]["aws_secret_access_key"]
        assert "us-west-2" == printed_deployment["derouter_params"]["aws_region_name"]
    finally:
        derouter.redact_user_api_key_info = original_setting
...
async def test_router_schedule_atext_completion(model_list):
    ...
    from derouter.types.utils import TextCompletionResponse
...
def test_update_kwargs_with_default_derouter_params(model_list):
    """Test if the '_update_kwargs_with_default_derouter_params' function is working correctly"""
    router = Router(
        model_list=model_list,
        default_derouter_params={"api_key": "test", "metadata": {"key": "value"}},
    )
    kwargs: dict = {"metadata": {"key2": "value2"}}
    router._update_kwargs_with_default_derouter_params(kwargs=kwargs)
...
@pytest.mark.parametrize(
    ...
    [
        ("mock_testing_fallbacks", derouter.InternalServerError),
        ("mock_testing_context_fallbacks", derouter.ContextWindowExceededError),
        ("mock_testing_content_policy_fallbacks", derouter.ContentPolicyViolationError),
    ],
)
...
async def test_deployment_callback_on_success_tracks_tpm_for_io_deployment():
    ...
    response = derouter.ModelResponse(model="openai/gpt-4o-mini", usage={"total_tokens": 100})
...
def test_deployment_callback_respects_cooldown_time(model_list):
    ...
    with patch("derouter.router._set_cooldown_deployments") as mock_set:
...
async def test_routing_strategy_pre_call_checks(model_list, sync_mode):
    ...
    if sync_mode:
        ...
        await router.async_routing_strategy_pre_call_checks(
            deployment, derouter_logging_obj
        )
        ...
        with patch.object(
            ...
            AsyncMock(
                side_effect=derouter.RateLimitError(
...
def test_create_deployment(
    ...
    if set_supported_environments:
        os.environ["DEROUTER_ENVIRONMENT"] = "staging"
...
def test_upsert_deployment(model_list):
    ...
    deployment.derouter_params.model = "gpt-5.5"
...
async def test_get_model_group_io_token_usage_sums_across_deployments():
    ...
    """
    from derouter.types.router import RouterCacheEnum
...
async def test_get_remaining_model_group_usage_merges_io_and_tpm_headers(model_list):
    ...
    from derouter.types.router import ModelGroupInfo
...
async def test_set_response_headers_dict_anthropic_messages_response(model_list):
    ...
    headers = resp["_hidden_params"]["additional_headers"]
    assert headers["x-derouter-model-group"] == "io-itpm-strict"
...
@pytest.mark.parametrize(
    ...
    [
        (derouter.exceptions.BadRequestError, "BadRequestError", 3),
        (derouter.exceptions.AuthenticationError, "AuthenticationError", 4),
        (derouter.exceptions.RateLimitError, "RateLimitError", 6),
        (
            derouter.exceptions.ContentPolicyViolationError,
...
def test_get_num_retries_from_retry_policy(
    ...
    from derouter.router import RetryPolicy
...
def test_initialize_alerting(model_list):
    ...
    from derouter.types.router import AlertingConfig
...
def test_discard(model_list):
    ...
    derouter.service_callback = []

    router = Router(model_list=model_list)
    router.discard()

    # Verify all callback lists are empty
    assert len(derouter.callbacks) == 0
    assert len(derouter.success_callback) == 0
    assert len(derouter.failure_callback) == 0
    assert len(derouter._async_success_callback) == 0
    assert len(derouter._async_failure_callback) == 0
    assert len(derouter.input_callback) == 0
    assert len(derouter.service_callback) == 0
...
def test_pass_through_assistants_endpoint_factory(model_list):
    ...
    router._pass_through_assistants_endpoint_factory(
        original_function=derouter.acreate_assistants,
        custom_llm_provider="openai",
        client=None,
        **{},
    )
...
def test_factory_function(model_list):
    ...
    router = Router(model_list=model_list)
    router.factory_function(derouter.acreate_assistants)
...
def test_get_deployment_by_derouter_model(model_list):
    """Test if the 'get_deployment_by_derouter_model' function is working correctly"""
    router = Router(model_list=model_list)
    deployment = router._get_deployment_by_derouter_model(model="gpt-5-mini")
    assert deployment is not None
...
#         matched_pattern=match, derouter_deployment_derouter_model="openai/*"
#     )
#     assert updated_model == "openai/fo::hi:static::hello"


@pytest.mark.parametrize(
    "user_request_model, model_name, derouter_model, expected_model",
    [
...
def test_pattern_match_deployment_set_model_name(
    ...
    updated_model = pattern_router.set_deployment_model_name(match, derouter_model)
        ...
        deployments=[
            {
                ...
                "derouter_params": {"model": derouter_model},
            }
        ],
    )

    for model in updated_models:
        assert model["derouter_params"]["model"] == expected_model
...
async def test_pass_through_moderation_endpoint_factory(model_list):
    ...
    response = await router._pass_through_moderation_endpoint_factory(
        original_function=derouter.amoderation,
...
def test_mock_router_testing_params_str_to_bool_conversion(
    ...
    from derouter.types.router import MockRouterTestingParams
...
def test_is_auto_router_deployment(model_list):
    ...
    derouter_params_auto = DeRouter_Params(model="auto_router/my-auto-router")
    assert router._is_auto_router_deployment(derouter_params_auto) is True

    # Test case 2: Model doesn't start with "auto_router/" - should return False
    derouter_params_regular = DeRouter_Params(model="gpt-5-mini")
    assert router._is_auto_router_deployment(derouter_params_regular) is False

    # Test case 3: Model is empty string - should return False
    derouter_params_empty = DeRouter_Params(model="")
    assert router._is_auto_router_deployment(derouter_params_empty) is False

    # Test case 4: Model contains "auto_router/" but doesn't start with it - should return False
    derouter_params_contains = DeRouter_Params(model="prefix_auto_router/something")
    assert router._is_auto_router_deployment(derouter_params_contains) is False


@patch("derouter.router_strategy.auto_router.auto_router.AutoRouter")
...
def testgenerate_model_id_with_deployment_model_name(model_list):
    ...
    # Test case 1: Normal case with valid model_group and derouter_params
    model_group = "gpt-4.1"
    derouter_params = {
        "model": "gpt-4.1",
        "api_key": "test_key",
    ...
    try:
        result = router.generate_model_id(
            model_group=model_group, derouter_params=derouter_params
        )
    ...
    with pytest.raises(TypeError) as exc_info:
        router.generate_model_id(model_group=None, derouter_params=derouter_params)
    ...
    # Test case 3: Edge case with None key in derouter_params
    derouter_params_with_none_key = {
        "model": "gpt-4.1",
        "api_key": "test_key",
    ...
    try:
        result = router.generate_model_id(
            model_group=model_group, derouter_params=derouter_params_with_none_key
        )
        assert isinstance(result, str)
        assert len(result) > 0
        print(f"✓ Success with None key in derouter_params: {result}")
    except Exception as e:
        pytest.fail(f"Failed with None key in derouter_params: {e}")

    # Test case 4: Edge case with empty derouter_params
    try:
        result = router.generate_model_id(model_group=model_group, derouter_params={})
        assert isinstance(result, str)
        assert len(result) > 0
        print(f"✓ Success with empty derouter_params: {result}")
    except Exception as e:
        pytest.fail(f"Failed with empty derouter_params: {e}")
...
def test_handle_clientside_credential_with_deployment_model_name(model_list):
    ...
    deployment = {
        "model_name": "gpt-4.1",
        "derouter_params": {"model": "gpt-4.1", "api_key": "test_key"},
    }
    ...
    # Mock dynamic_derouter_params that would be returned by get_dynamic_derouter_params
    dynamic_derouter_params = {
        "api_key": "client_side_key",
        "api_base": "https://api.openai.com/v1",
    }
    ...
    try:
        ...
        result = router.generate_model_id(
            model_group=model_group, derouter_params=dynamic_derouter_params
        )
        assert isinstance(result, str)
        assert len(result) > 0
...
def test_sync_generic_api_call_preserves_requested_model_group_in_logs():
    ...
    try:
        ...
        assert captured_kwargs["derouter_metadata"]["model_group"] == "claude-sonnet-4-6"
        assert (
            captured_kwargs["derouter_metadata"]["deployment"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("_ageneric_api_call_with_fallbacks", "derouter_metadata"),
        ("batch", "derouter_metadata"),
        ("completion", "metadata"),
        ("acreate_file", "derouter_metadata"),
        ("aget_file", "derouter_metadata"),
    ],
)
def test_handle_clientside_credential_metadata_loading(
    ...
    # Verify the derouter_params contain the clientside credentials
    assert result_deployment.derouter_params.api_key == "client_side_key"
    assert result_deployment.derouter_params.api_base == "https://api.openai.com/v1"
    ...
    # For _ageneric_api_call_with_fallbacks/batch, it should use "derouter_metadata"
    if function_name == "acompletion":
        assert "metadata" in kwargs
        assert "derouter_metadata" not in kwargs
        ...
        # Note: acompletion would not have derouter_metadata, but other functions might have both
...
def test_handle_clientside_credential_metadata_variable_name(
    ...
    from derouter.router_utils.batch_utils import _get_router_metadata_variable_name
...
def test_handle_clientside_credential_with_responses_function(model_list):
    ...
    # Mock kwargs with clientside credentials and derouter_metadata (for responses function)
    kwargs = {
        "api_key": "client_side_key",
        "api_base": "https://api.openai.com/v1",
        "derouter_metadata": {
    ...
    print(
        "✓ Success with _ageneric_api_call_with_fallbacks function name and derouter_metadata"
    )
...
def test_get_metadata_variable_name_from_kwargs(model_list):
    ...
    # Test case 1: kwargs contains derouter_metadata - should return "derouter_metadata"
    kwargs_with_derouter_metadata = {
        "derouter_metadata": {"user": "test"},
        "metadata": {"other": "data"},
    }
    result = router._get_metadata_variable_name_from_kwargs(
        kwargs_with_derouter_metadata
    )
    assert result == "derouter_metadata"
...
async def test_asearch_with_fallbacks_helper_missing_search_provider():
    ...
    is found but doesn't have search_provider in its derouter_params.
    """
    ...
    with pytest.raises(ValueError, match="search_provider not found in derouter_params"):
        await router._asearch_with_fallbacks_helper(
            model="bad-tool",
            original_generic_function=mock_original_function,
            query="test query",
...
def test_get_first_default_fallback():
    ...
    model_list = [
        {
            "model_name": "gpt-5-mini",
            "derouter_params": {"model": "gpt-5-mini", "api_key": "fake-key"},
        }
    ]
...
def test_resolve_model_name_from_model_id():
    ...
    # Test case 3: model_id matches derouter_params.model exactly
    model_list = [
        {
...
def test_get_router_model_info_with_deployment_object():
    """Test get_router_model_info accepts Deployment object directly and reuses DeRouter_Params"""
    router = Router(
        model_list=[
            {
                "model_name": "gpt-5.5",
                "derouter_params": {"model": "gpt-5.5", "api_key": "test-key"},
                "model_info": {"id": "test-id"},
            }
        ]
    )
    ...
    assert isinstance(deployment.derouter_params, DeRouter_Params)

    # Pass Deployment directly (not .model_dump()) - this exercises the isinstance check
    # that reuses the existing DeRouter_Params instead of reconstructing it
