...
transport (httpx.AsyncClient.send), so requests run derouter's real
...
class FakeAnthropicUpstream:
    ...
    clients, so the transport is the seam that exercises the real derouter pipeline."""
...
FABLE_TIER = {
    ...
    "derouter_params": {"model": "anthropic/claude-fable-5", "api_key": "sk-test"},
}
OPUS_TARGET = {
    "model_name": "opus-target",
    "derouter_params": {"model": "anthropic/claude-opus-5", "api_key": "sk-test"},
}
...
async def test_caller_forged_tier_stamp_cannot_pick_the_streaming_fallback_chain():
    ...
    with fake.install():
        stream = await router.aanthropic_messages(
            ...
            derouter_metadata={PRE_ROUTING_SELECTED_MODEL_KEY: "forged-tier"},
        )
        body = await _collect(stream)
...
def test_record_pre_routing_selection_writes_only_the_internal_bucket():
    ...
    kwargs = {"metadata": {"user_id": "u1"}, "derouter_metadata": {}}

    record_pre_routing_selection(kwargs, "tier-x")

    assert kwargs["derouter_metadata"] == {PRE_ROUTING_SELECTED_MODEL_KEY: "tier-x"}
...
def test_refusal_fallback_available_arms_on_generic_rows_only_without_content_policy():
    ...
    stamped = {"derouter_metadata": {PRE_ROUTING_SELECTED_MODEL_KEY: "tier-group"}}
...
def test_refusal_gate_keys_on_pre_routing_tier_stamp():
    ...
    refusal_kwargs = {"derouter_metadata": {PRE_ROUTING_SELECTED_MODEL_KEY: "tier-group"}}
    assert (
