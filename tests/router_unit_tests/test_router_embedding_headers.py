...
default_derouter_params (including headers) to be propagated.
"""
...
class TestRouterEmbeddingHeaders:
    ...
    def test_embedding_calls_update_kwargs_before_fallbacks(self):
        ...
        with patch.object(
            ...
            with patch("derouter.embedding") as mock_derouter_embedding:
                mock_derouter_embedding.return_value = MagicMock(
    ...
    async def test_aembedding_calls_update_kwargs_before_fallbacks(self):
        ...
        with patch.object(
            ...
            with patch(
                "derouter.aembedding", new_callable=AsyncMock
            ) as mock_derouter_aembedding:
                mock_derouter_aembedding.return_value = MagicMock(
                    data=[{"embedding": [0.1, 0.2, 0.3]}]
                )
    ...
    def test_embedding_propagates_default_derouter_params(self):
        """
        Test that embedding calls properly propagate default_derouter_params including headers.

        This is the main fix - ensuring that headers set in default_derouter_params
        ...
        # Create router with default_derouter_params containing headers
        router = Router(
            model_list=model_list,
            default_derouter_params={
        ...
        with patch("derouter.embedding") as mock_derouter_embedding:
            ...
            # Verify that derouter.embedding was called with the headers
            mock_derouter_embedding.assert_called_once()
            call_kwargs = mock_derouter_embedding.call_args[1]
    ...
    @pytest.mark.asyncio
    async def test_aembedding_propagates_default_derouter_params(self):
        """
        Test that async embedding calls properly propagate default_derouter_params including headers.
        """
        custom_headers = {"X-Custom-Header": "test-value", "X-API-Version": "v2"}
        ...
        with patch(
            ...
            # Verify that derouter.aembedding was called with the headers
            mock_derouter_aembedding.assert_called_once()
            call_kwargs = mock_derouter_aembedding.call_args[1]
    ...
    def test_embedding_sets_derouter_trace_id(self):
        """
        Test that embedding calls include a derouter_trace_id.
        ...
        with patch("derouter.embedding") as mock_derouter_embedding:
            ...
            # Verify derouter_trace_id was set
            assert "derouter_trace_id" in call_kwargs
            assert isinstance(call_kwargs["derouter_trace_id"], str)
            assert len(call_kwargs["derouter_trace_id"]) > 0
    ...
    def test_embedding_consistency_with_completion(self):
        ...
        router = Router(
            model_list=model_list, default_derouter_params={"headers": custom_headers}
        )

        # Test completion
        with patch("derouter.completion") as mock_completion:
        ...
        # Test embedding
        with patch("derouter.embedding") as mock_embedding:
        ...
        # Both should have headers from default_derouter_params
        ...
        # Both should have derouter_trace_id
        assert "derouter_trace_id" in completion_kwargs
        assert "derouter_trace_id" in embedding_kwargs
...
if __name__ == "__main__":
    ...
    test.test_embedding_propagates_default_derouter_params()
    test.test_embedding_metadata_includes_model_group()
    test.test_embedding_sets_derouter_trace_id()
