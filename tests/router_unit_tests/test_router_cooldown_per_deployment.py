...
from derouter.router_utils.cooldown_cache import CooldownCache, CooldownCacheValue
...
from derouter.router_utils.fallback_event_handlers import _trigger_cooldown_for_failed_deployment
...
class TestDeploymentLevelAllowedFails:
    def test_deployment_level_allowed_fails_overrides_router_level(self):
        ...
        _exception = derouter.RateLimitError("Rate limit", "openai", "gpt-4")
...
class TestDeploymentLevelAllowedFailsPolicyByExceptionType:
    def test_rate_limit_error_triggers_cooldown_with_zero_threshold(self):
        ...
        rate_limit_exc = derouter.RateLimitError("Rate limit", "openai", "gpt-4")
        should_cooldown = _should_cooldown_deployment(
            derouter_router_instance=router,
            deployment="primary",
            exception_status=429,
    ...
    def test_internal_server_error_respects_per_exception_threshold(self):
        ...
        ise = derouter.InternalServerError("Internal error", "openai", "gpt-4")
...
class TestFallbackDeploymentCooldown:
    ...
    def test_trigger_cooldown_falls_back_to_derouter_params_cooldown_time(self):
        """
        cooldown_time has pre-existing derouter_params support on the primary
        ...
        with patch("derouter.router_utils.fallback_event_handlers._set_cooldown_deployments") as mock_set_cooldown:
            ...
            call_kwargs = mock_set_cooldown.call_args[1]
            assert call_kwargs["time_to_cooldown"] == 30.0, (
                "derouter_params.cooldown_time must still be honored as a fallback"
            )

    def test_trigger_cooldown_prefers_model_info_cooldown_time_over_derouter_params(self):
        mock_router = MagicMock()
        mock_router.cooldown_time = 300.0
        mock_router.get_model_info.return_value = {
            "model_info": {"cooldown_time": 15.0},
            "derouter_params": {"cooldown_time": 30.0},
        }
...
class TestResolveAllowedFailsFromPolicyFallsThrough:
    def test_none_value_on_first_match_falls_through_to_next_type(self):
        ...
        exc = derouter.ContentPolicyViolationError("flagged", "openai", "gpt-4")
...
class TestDeploymentCallbackOnFailureCooldownTimePrecedence:
    def test_model_info_cooldown_time_used_in_primary_sync_path(self):
        ...
        also honor a model_info.cooldown_time, not just derouter_params.cooldown_time.
        """
        router = _make_router(
            model_list=[
                {
        ...
        with patch("derouter.router._set_cooldown_deployments") as mock_set_cooldown:
    ...
    def test_derouter_params_cooldown_time_still_honored_as_fallback(self):
        """cooldown_time has pre-existing derouter_params support on this primary
        path; it must keep working when model_info doesn't set it."""
        router = _make_router(
            model_list=[
                {
                    "model_name": "gpt-4",
                    "derouter_params": {"model": "openai/gpt-4", "cooldown_time": 20.0},
                    "model_info": {"id": "primary"},
                },
            ],
        )
        ...
        with patch("derouter.router._set_cooldown_deployments") as mock_set_cooldown:
            ...
            call_kwargs = mock_set_cooldown.call_args[1]
            assert call_kwargs["time_to_cooldown"] == 20.0, "derouter_params.cooldown_time must still be honored"
...
class TestNewAllowedFailsPolicyFields:
    def test_service_unavailable_error_matched_by_policy(self):
        ...
        exc = derouter.ServiceUnavailableError("Service unavailable", "openai", "gpt-4")
        result = _resolve_allowed_fails_from_policy(policy=policy, exception=exc)
        assert result == 0
    ...
    def test_bad_gateway_error_matched_by_policy(self):
        ...
        exc = derouter.BadGatewayError("Bad gateway", "openai", "gpt-4")
        result = _resolve_allowed_fails_from_policy(policy=policy, exception=exc)
        assert result == 2
...
class TestRouterLevelGetAllowedFailsFromPolicy:
    ...
    def test_internal_server_error_returned(self):
        ...
        exc = derouter.InternalServerError("500 error", "openai", "gpt-4")
    ...
    def test_service_unavailable_error_returned(self):
        ...
        exc = derouter.ServiceUnavailableError("503 error", "openai", "gpt-4")
    ...
    def test_bad_gateway_error_returned(self):
        ...
        exc = derouter.BadGatewayError("502 error", "openai", "gpt-4")
    ...
    def test_not_found_error_returned(self):
        ...
        exc = derouter.NotFoundError("404 error", "openai", "gpt-4")
