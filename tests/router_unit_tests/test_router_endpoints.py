...
from derouter import Router, CustomLogger
...
# This file includes the custom callbacks for DeRouter Proxy
...
async def test_audio_speech_router(mode):
    ...
    from derouter.llms.openai.openai import HttpxBinaryResponseContent
...
async def test_aspeech_fallbacks_on_deployment_failure():
    router = Router(
        model_list=[
            {
                "model_name": "tts-main",
                "derouter_params": {"model": "openai/tts-1", "api_key": "fake-key"},
            },
            {
                "model_name": "tts-backup",
                "derouter_params": {"model": "openai/tts-1-hd", "api_key": "fake-key"},
            },
        ],
    ...
    with patch("derouter.aspeech", side_effect=mock_aspeech):
...
async def test_aspeech_success_returns_response():
    ...
    with patch("derouter.aspeech", return_value=mock_response) as mock_aspeech:
        response = await router.aspeech(
            model="tts",
            input="the quick brown fox jumped over the lazy dogs",
            voice="alloy",
...
async def test_moderation_endpoint_with_api_base():
    ...
    with patch(
        "derouter.main.openai_chat_completions._get_openai_client"
    ) as mock_get_client:
...
async def test_router_with_empty_choices(model_list):
    """
    https://github.com/Delify-Solutions/derouter/issues/8306
    """
    router = Router(model_list=model_list)
...
async def test__aadapter_completion():
    ...
    """
    # Create a mock response for derouter.aadapter_completion
    mock_response = {
        "id": "adapter_resp_123",
        "object": "adapter.completion",
        "created": 1677858242,
    ...
    # Create a router with a mocked derouter.aadapter_completion
    with patch(
        "derouter.aadapter_completion", new_callable=AsyncMock
        ...
        # Verify derouter.aadapter_completion was called with the right parameters
...
async def test_init_containers_api_endpoints_create_with_unknown_model_passes_through(monkeypatch):
    ...
    router = Router(
        model_list=[
            {
                "model_name": "gpt-5.4",
                "derouter_params": {"model": "openai/gpt-5.4", "api_key": "sk-model-list-key"},
            }
        ]
    )
    mock_original_function = AsyncMock(return_value={"id": "cntr_test"})
