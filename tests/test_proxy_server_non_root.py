...
def test_restructure_ui_html_files_skipped_in_non_root(monkeypatch):
    ...
    - DEROUTER_NON_ROOT is "true"
    - ui_path is "/var/lib/derouter/ui"
    """
    ...
    with patch(
        "derouter.proxy.proxy_server._restructure_ui_html_files",
        create=True,
        spec=False,
    ) as mock_restructure:
        # Simulate the logic we added in proxy_server.py
        if is_non_root and ui_path == "/var/lib/derouter/ui":
            # Skipping...
            pass
        else:
