...
class BaseLLMImageEditTest(ABC):
    ...
    def image_edit_function(self):
        return derouter.image_edit

    @property
    def async_image_edit_function(self):
        return derouter.aimage_edit
    ...
    @pytest.mark.asyncio
    async def test_openai_image_edit_derouter_sdk(self, sync_mode):
        """
...
_DEROUTER_SITE_BYTES = _read_image_bytes("derouter_site.png")


def _make_test_images() -> list:
    return [_ISHAAN_GITHUB_BYTES, _DEROUTER_SITE_BYTES]
...
@pytest.mark.flaky(retries=3, delay=2)
@pytest.mark.asyncio
async def test_openai_image_edit_derouter_router():
    derouter._turn_on_debug()
    try:
        prompt = """
        Create a studio ghibli style image that combines all the reference images. Make sure the person looks like a CTO.
...
async def test_openai_image_edit_with_bytesio():
    ...
    from derouter import image_edit, aimage_edit
...
def test_recraft_image_edit_config():
    ...
    """
    from derouter.llms.recraft.image_edit.transformation import RecraftImageEditConfig
