...
from derouter.proxy.proxy_server import LitellmUserRoles
from derouter.caching.caching import DualCache
from derouter.types.proxy.claude_code_endpoints import RegisterPluginRequest
...
def mock_prisma_client():
    ...
    mock_client.db.derouter_claudecodeplugintable = mock_table
...
async def test_register_plugin(mock_prisma_client):
    ...
    stored_plugin = (
        await mock_prisma_client.db.derouter_claudecodeplugintable.find_unique(
    ...
    await mock_prisma_client.db.derouter_claudecodeplugintable.delete(
        where={"name": plugin_name}
    )
