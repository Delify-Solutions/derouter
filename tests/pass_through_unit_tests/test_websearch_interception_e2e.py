...
import derouter
from derouter.integrations.websearch_interception import (
...
async def test_websearch_interception_non_streaming():
    ...
    derouter.callbacks = [websearch_logger]
    ...
    try:
        ...
        print("\n📞 Making derouter.messages.acreate() call...")
        print(f"   Model: bedrock/us.anthropic.claude-haiku-4-5-20251001-v1:0")
        print(f"   Query: 'What is DeRouter?'")
        ...
        if has_tool_use:
            ...
            if "derouter" in text_content.lower():
                print("\n" + "=" * 80)
                print("✅ TEST 1 PASSED!")
                print("=" * 80)
                print("✅ User made ONE derouter.messages.acreate() call")
                ...
                print("\n⚠️  Got text response but doesn't mention DeRouter")
                return False
        else:
...
async def test_websearch_interception_streaming():
    ...
    try:
        ...
        print("\n📞 Making derouter.messages.acreate() call with stream=True...")
        print(f"   Model: bedrock/us.anthropic.claude-haiku-4-5-20251001-v1:0")
        print(f"   Query: 'What is DeRouter?'")
        print(f"   Tools: WebSearch")
        ...
        if has_tool_use:
            ...
            if "derouter" in text_content.lower():
                ...
                print("=" * 80)
                print(
                    "✅ User made ONE derouter.messages.acreate() call with stream=True"
                )
...
async def test_websearch_interception_no_tool_call_streaming():
    ...
    try:
        ...
        if len(chunks) > 0:
            ...
            print("✅ TEST 3 PASSED!")
            print("=" * 80)
            print("✅ User made ONE derouter.messages.acreate() call with stream=True")
...
async def test_claude_code_native_websearch():
    ...
    try:
        ...
        print(f"   Query: 'Perform a web search for the query: derouter what is it'")
        ...
        response = await messages.acreate(
            ...
            messages=[
                {
                    "role": "user",
                    "content": "Perform a web search for the query: derouter what is it",
                }
            ],
            tools=[
        ...
        if has_tool_use:
            ...
            if "derouter" in text_content.lower():
                ...
                print("✅ Tool was converted to DeRouter standard format")
                print("✅ User made ONE derouter.messages.acreate() call")
...
async def test_derouter_standard_websearch_tool():
    """
    PRIORITY TEST #1: Test with the canonical derouter_web_search tool format.

    This validates that using get_derouter_web_search_tool() directly
    works end-to-end without any conversion needed.
    """
    print("\n" + "=" * 80)
    print("E2E TEST: DeRouter Standard WebSearch Tool")
    print("=" * 80)

    from derouter.integrations.websearch_interception import get_derouter_web_search_tool
    ...
    try:
        ...
        print(f"   Tool: derouter_web_search (standard format, no conversion needed)")
        print(f"   Stream: False")
        ...
        response = await messages.acreate(
            ...
            tools=[get_derouter_web_search_tool()],
            max_tokens=1024,
            stream=False,
        )
        ...
        if has_tool_use:
            ...
            print("✅ TEST PASSED!")
            print("=" * 80)
            print("✅ DeRouter standard tool format works without conversion")
            print("✅ Agentic loop executed transparently")
            print("=" * 80)
            return True
        else:
...
async def test_claude_code_native_websearch_streaming():
    ...
    Validates:
    - Native tool conversion (web_search_20250305 → derouter_web_search)
    ...
    try:
        ...
        if has_tool_use:
            ...
            print("✅ TEST PASSED!")
            print("=" * 80)
            print("✅ Native tool converted to derouter_web_search")
...
def test_is_web_search_tool_detection():
    ...
    from derouter.integrations.websearch_interception import is_web_search_tool

    test_cases = [
        ({"name": "derouter_web_search"}, True, "DeRouter standard tool"),
        (
            {"type": "web_search_20250305", "name": "web_search", "max_uses": 8},
            True,
...
async def test_pre_request_hook_modifies_request_body():
    ...
    2. Native web_search_20250305 tool is converted to derouter_web_search
    ...
    with patch(
        "derouter.llms.anthropic.experimental_pass_through.messages.handler.anthropic_messages_handler",
        ...
        if tools and len(tools) > 0:
            ...
            if tool_name == DEROUTER_WEB_SEARCH_TOOL_NAME:
                print(
                    f"   ✅ Tool converted: web_search_20250305 → {DEROUTER_WEB_SEARCH_TOOL_NAME}"
                )
            else:
                print(
                    f"   ❌ Tool NOT converted: expected {DEROUTER_WEB_SEARCH_TOOL_NAME}, got {tool_name}"
                )
                return False
        else:
