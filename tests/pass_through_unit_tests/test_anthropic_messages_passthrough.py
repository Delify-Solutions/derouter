...
class TestAnthropicOpenAIAPI(BaseAnthropicMessagesTest):
    ...
    @pytest.mark.asyncio
    async def test_anthropic_messages_derouter_router_streaming_with_logging(self):
        """
...
async def test_anthropic_messages_streaming_with_bad_request():
    ...
    error = None
    try:
        response = await derouter.anthropic.messages.acreate(
...
@pytest.mark.asyncio
async def test_anthropic_messages_derouter_router_non_streaming():
    """
...
@pytest.mark.asyncio
async def test_anthropic_messages_derouter_router_routing_strategy():
    """
...
@pytest.mark.asyncio
async def test_anthropic_messages_derouter_router_latency_metadata_tracking():
    """
    Test the anthropic_messages with routing strategy and verify that _latency_per_deployment
    field is passed in derouter_metadata when calling derouter.anthropic_messages
    """
    with unittest.mock.patch("derouter.anthropic_messages") as mock_anthropic_messages:
        ...
        # Verify that derouter.anthropic_messages was called
        ...
        # Verify that derouter_metadata was passed and contains _latency_per_deployment
        assert (
            "derouter_metadata" in call_kwargs
        ), "derouter_metadata should be passed to anthropic_messages"

        derouter_metadata = call_kwargs["derouter_metadata"]
        assert derouter_metadata is not None, "derouter_metadata should not be None"
        assert isinstance(
            derouter_metadata, dict
        ), "derouter_metadata should be a dictionary"

        # Verify _latency_per_deployment is present
        assert (
            "_latency_per_deployment" in derouter_metadata
        ), "derouter_metadata should contain _latency_per_deployment field"

        # Verify the structure of _latency_per_deployment
        latency_per_deployment = derouter_metadata["_latency_per_deployment"]
        assert isinstance(
        ...
        # Verify other expected fields in derouter_metadata
        assert "model_group" in derouter_metadata
        assert derouter_metadata["model_group"] == MODEL_GROUP
        assert "deployment" in derouter_metadata
        assert "model_info" in derouter_metadata
        ...
        print(
            "✅ Successfully verified that _latency_per_deployment is passed in derouter_metadata to anthropic_messages"
        )
...
@pytest.mark.asyncio
async def test_anthropic_messages_derouter_router_non_streaming_with_logging():
    """
    Test the anthropic_messages with non-streaming request
...
#     Issue: When calling Anthropic models via the Messages API, DeRouter makes a call to
...
#     with unittest.mock.patch("derouter.llms.custom_httpx.llm_http_handler.get_async_httpx_client") as mock_get_client:
...
def test_sync_openai_messages():
    ...
    """
    derouter._turn_on_debug()
    response = derouter.anthropic.messages.create(
        messages=[{"role": "user", "content": "Hello, can you tell me a short joke?"}],
        model="openai/gpt-4.1-mini",
        max_tokens=100,
    )
