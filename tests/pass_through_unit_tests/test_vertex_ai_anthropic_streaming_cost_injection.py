...
async def test_vertex_ai_anthropic_streaming_cost_injection_enabled():
    ...
    original_value = getattr(derouter, "include_cost_in_streaming_usage", False)
    ...
    try:
        ...
        derouter_logging_obj = MagicMock(spec=DeRouterLoggingObj)
        derouter_logging_obj.model_call_details = {"model": "claude-sonnet-4@20250514"}
        derouter_logging_obj.completion_start_time = None
        derouter_logging_obj.async_success_handler = AsyncMock()
        ...
        with patch("derouter.completion_cost", return_value=0.00015):
            received_chunks = []
        ...
        derouter.include_cost_in_streaming_usage = original_value
...
async def test_vertex_ai_anthropic_streaming_model_extraction():
    ...
    try:
        ...
        with patch("derouter.completion_cost") as mock_cost:
