...
def test_get_response_headers_filters_excluded_custom_headers():
    ...
    assert result["x-derouter-version"] == "1.84.0"
    assert result["x-derouter-call-id"] == "call-123"
...
def test_init_kwargs_for_pass_through_endpoint_basic(
    ...
    assert result["derouter_call_id"] == "test-call-id"
    assert result["passthrough_logging_payload"] == passthrough_payload

    #########################################################
    # Check metadata
    #########################################################
    assert result["derouter_params"]["metadata"]["user_api_key"] == "test-key"
    assert result["derouter_params"]["metadata"]["user_api_key_hash"] == "test-key"
    assert result["derouter_params"]["metadata"]["user_api_key_alias"] is None
    assert result["derouter_params"]["metadata"]["user_api_key_user_email"] is None
    assert result["derouter_params"]["metadata"]["user_api_key_user_id"] == "test-user"
    assert result["derouter_params"]["metadata"]["user_api_key_team_id"] == "test-team"
    assert result["derouter_params"]["metadata"]["user_api_key_org_id"] is None
    assert result["derouter_params"]["metadata"]["user_api_key_team_alias"] is None
    assert (
        result["derouter_params"]["metadata"]["user_api_key_end_user_id"] == "test-user"
    )
    assert result["derouter_params"]["metadata"]["user_api_key_request_route"] is None


def test_init_kwargs_with_derouter_metadata(mock_request, mock_user_api_key_dict):
    """
    Expected behavior: derouter_metadata should be merged with default metadata

    see usage example here: https://router.delify.vn/docs/pass_through/anthropic_completion#send-derouter_metadata-tags
    """
    request = mock_request()
    parsed_body = {
        "derouter_metadata": {"custom_field": "custom_value", "tags": ["tag1", "tag2"]}
    }
    passthrough_payload = PassthroughStandardLoggingPayload(
        url="https://test.com",
        request_body={},
    ...
    # Check that derouter_metadata was merged with default metadata
    metadata = result["derouter_params"]["metadata"]
...
athropic_request_body = {
    ...
    "derouter_metadata": {"tags": ["hi", "hello"]},
}
...
async def test_pass_through_request_logging_failure(
    ...
    with (
        patch(
            "derouter.proxy.pass_through_endpoints.pass_through_endpoints.PassThroughEndpointLogging.pass_through_async_success_handler",
...
async def test_pass_through_request_logging_failure_with_stream(
    ...
    with (
        patch(
            "derouter.proxy.pass_through_endpoints.streaming_handler.PassThroughStreamingHandler._route_streaming_logging_to_handler",
            new=mock_logging_failure,
        ),
        patch(
            "httpx.AsyncClient.send",
...
def test_init_kwargs_filters_pricing_params(mock_request, mock_user_api_key_dict):
    ...
    # Verify pricing parameters are stored in derouter_params for internal use
    derouter_params = result["derouter_params"]
    assert derouter_params["input_cost_per_token"] == 0.00002
    assert derouter_params["output_cost_per_token"] == 0.00002
...
def test_custom_pricing_used_in_cost_calculation():
    """
    Test that when custom pricing parameters are provided in derouter_params,
    they are actually used for cost calculation.

    This ensures that the custom pricing functionality works end-to-end:
    1. Pricing params are stored in derouter_params
    2. These params are used by completion_cost() to calculate costs

    Regression test for: LIT-1221
    """
    from derouter import completion_cost, Choices, Message, ModelResponse
...
def test_init_kwargs_no_authenticated_identity_field_is_client_settable(
    ...
    expected = dict(
        DeRouterProxyRequestSetup.get_sanitized_user_information_from_key(
            user_api_key_dict=authenticated_key
        )
    )
    ...
    result = HttpPassThroughEndpointHelpers._init_kwargs_for_pass_through_endpoint(
        ...
        _parsed_body={"derouter_metadata": dict(spoofed), "metadata": dict(spoofed)},
    )
