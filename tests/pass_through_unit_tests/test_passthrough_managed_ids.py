...
from derouter.proxy.pass_through_endpoints.managed_id_codec import (
    decode,
    encode,
    is_managed,
    new_managed_id,
...
def _prisma_client() -> MagicMock:
    ...
    pc = MagicMock()
    pc.db = MagicMock()
    pc.db.derouter_managedfiletable = MagicMock()
    pc.db.derouter_managedfiletable.find_first = AsyncMock(return_value=None)
    pc.db.derouter_managedfiletable.find_many = AsyncMock(return_value=[])
    pc.db.derouter_managedfiletable.create = AsyncMock(return_value=None)
    pc.db.derouter_managedobjecttable = MagicMock()
    pc.db.derouter_managedobjecttable.find_first = AsyncMock(return_value=None)
    pc.db.derouter_managedobjecttable.upsert = AsyncMock(return_value=None)
    pc.db.derouter_managedobjecttable.update = AsyncMock(return_value=None)
    return pc
...
class TestCodec:
    ...
    def test_decode_returns_none_for_unified_endpoint_id(self):
        # A unified-endpoint ID: starts with derouter_proxy: but lacks passthrough;
        plaintext = "derouter_proxy:application/octet-stream;unified_id,123;target_model_names,gpt-4"
...
class TestManagedIdProviderScope:
    ...
    def test_openai_scope(self):
        ...
        assert (
            resolve_passthrough_managed_id_provider(derouter.LlmProviders.OPENAI)
            == "openai"
        )
    ...
    def test_azure_scope(self):
        ...
        assert (
            resolve_passthrough_managed_id_provider(derouter.LlmProviders.AZURE)
            == "azure"
        )
    ...
    def test_azure_ai_collapses_to_azure(self):
        ...
        assert (
            resolve_passthrough_managed_id_provider(derouter.LlmProviders.AZURE_AI)
            == "azure"
        )
...
class TestResolveOne:
    ...
    async def test_valid_batch_id_resolves_via_object_table(self):
        ...
        pc.db.derouter_managedobjecttable.find_first = AsyncMock(return_value=obj_row)
...
class TestRewriteResponseIds:
    ...
    async def test_dedup_reuses_existing_file_row(self):
        ...
        pc.db.derouter_managedfiletable.find_many = AsyncMock(
    ...
    async def test_cross_owner_file_retrieve_raises_404(self):
        ...
        pc.db.derouter_managedfiletable.find_many = _owner_scoped_file_find_many(
            other_owner_row
        )
        hook = _managed_files_hook()
    ...
    async def test_batch_reuse_refreshes_stored_snapshot(self):
        ...
        pc = _prisma_client()
        pc.db.derouter_managedobjecttable.find_first = AsyncMock(
            return_value=existing_row
        )
        ...
        assert result["id"] == existing_managed_id
        pc.db.derouter_managedobjecttable.upsert.assert_not_awaited()
        # The stored snapshot is refreshed with the completed batch body
        pc.db.derouter_managedobjecttable.update.assert_awaited_once()
        update_kwargs = pc.db.derouter_managedobjecttable.update.call_args.kwargs
    ...
    async def test_cross_provider_batch_collision_mints_new_id(self):
        ...
        call_data = pc.db.derouter_managedobjecttable.upsert.call_args.kwargs["data"]
        assert (
    ...
    async def test_batch_create_persist_failure_leaves_raw_id(self):
        ...
        pc = _prisma_client()
        pc.db.derouter_managedobjecttable.find_first = AsyncMock(return_value=None)
        pc.db.derouter_managedobjecttable.upsert = AsyncMock(
        ...
        pc.db.derouter_managedobjecttable.upsert.assert_awaited_once()
    ...
    async def test_concurrent_create_converges_on_winner_managed_id(self):
        ...
        assert pc.db.derouter_managedobjecttable.find_first.await_count == 2
    ...
    async def test_cross_provider_batch_collision_dedup_uses_namespaced_key(self):
        ...
        lookup_where = pc.db.derouter_managedobjecttable.find_first.call_args.kwargs[
            "where"
        ]
    ...
    async def test_batch_snapshot_stores_managed_nested_file_ids(self):
        ...
        stored = pc.db.derouter_managedobjecttable.upsert.call_args.kwargs["data"][
            "create"
        ]["file_object"]
...
class TestRewriteBodyIds:
    ...
    @pytest.mark.asyncio
    async def test_derouter_internal_key_preserved(self):
        """derouter_logging_obj and similar keys are never walked."""
        logging_obj = object()
        body = {"derouter_logging_obj": logging_obj, "model": "gpt-4o"}
        result = await rewrite_body_ids(body, "openai", _user(), None, None)
        # Internal key preserved by reference
        assert result["derouter_logging_obj"] is logging_obj  # type: ignore[index]
...
class TestRawProviderIdInputGuard:
    ...
    async def test_raw_file_owned_by_caller_passes_through(self):
        ...
        pc.db.derouter_managedfiletable.find_many = AsyncMock(return_value=[own_row])
        result = await rewrite_path_ids(
    ...
    async def test_cross_provider_raw_file_not_blocked(self):
        ...
        pc.db.derouter_managedfiletable.find_many = AsyncMock(return_value=[azure_row])
        result = await rewrite_path_ids(
            "/openai/v1/files/file-victim",
            "openai",
            _user("attacker", "attacker-team"),
...
class TestRawProviderIdGuardBudget:
    ...
    async def test_many_distinct_raw_ids_capped(self):
        ...
        assert (
            pc.db.derouter_managedfiletable.find_many.call_count
            == _MAX_RAW_ID_GUARD_LOOKUPS
        )
    ...
    async def test_repeated_raw_id_deduped(self):
        ...
        assert result is body
        assert pc.db.derouter_managedfiletable.find_many.call_count == 1
    ...
    async def test_distinct_ids_under_cap_not_rejected(self):
        ...
        assert result is body
        assert pc.db.derouter_managedfiletable.find_many.call_count == 5
...
def _prisma_with_list(file_rows=None, batch_rows=None) -> MagicMock:
    ...
    if file_rows is not None:
        pc.db.derouter_managedfiletable.find_many = AsyncMock(side_effect=_file_filter)
    if batch_rows is not None:
        pc.db.derouter_managedobjecttable.find_many = AsyncMock(
...
class TestListPassthroughIdsFromDb:
    ...
    async def test_list_files_admin_gets_all_rows(self):
        ...
        call_kwargs = pc.db.derouter_managedfiletable.find_many.call_args.kwargs
    ...
    async def test_list_files_pushes_provider_scope_to_db(self):
        ...
        assert result["data"] == []
        assert result["has_more"] is False
        where = pc.db.derouter_managedfiletable.find_many.call_args.kwargs["where"]
        assert where["flat_model_file_ids"] == {
            "has": _passthrough_provider_marker("openai")
        }
        assert pc.db.derouter_managedfiletable.find_many.await_count == 1
    ...
    async def test_list_ignores_cross_provider_cursor(self):
        ...
        pc.db.derouter_managedfiletable.find_first = AsyncMock(return_value=cursor_row)
    ...
    async def test_list_cursor_does_not_drop_created_at_ties(self):
        ...
        pc = _prisma_client()
        pc.db.derouter_managedfiletable.find_many = AsyncMock(side_effect=_find_many)
        pc.db.derouter_managedfiletable.find_first = AsyncMock(side_effect=_find_first)
    ...
    async def test_list_batches_pushes_provider_scope_to_db(self):
        ...
        assert result is not None
        assert len(result["data"]) == 1
        where = pc.db.derouter_managedobjecttable.find_many.call_args.kwargs["where"]
        assert where["model_object_id"] == {"startswith": "passthrough:azure:"}
        assert pc.db.derouter_managedobjecttable.find_many.await_count == 1
