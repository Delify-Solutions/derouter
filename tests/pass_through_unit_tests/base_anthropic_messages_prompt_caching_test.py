...
Tests that prompt caching works correctly via derouter.anthropic.messages interface
...
class BaseAnthropicMessagesPromptCachingTest(ABC):
    ...
    async def test_prompt_caching_returns_cache_read_tokens_on_second_call(self):
        ...
        response1 = await derouter.anthropic.messages.acreate(
            model=self.get_model(),
            messages=messages,
            max_tokens=100,
        )
        ...
        response2 = await derouter.anthropic.messages.acreate(
            model=self.get_model(),
            messages=messages,
            max_tokens=100,
        )
