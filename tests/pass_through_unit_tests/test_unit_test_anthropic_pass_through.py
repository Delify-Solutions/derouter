...
async def test_anthropic_passthrough_handler(
    ...
    """
    Unit test - Assert that the anthropic passthrough handler calls the derouter logging object's async_success_handler
    """
    start_time = datetime.now()
    ...
    assert isinstance(result["result"], derouter.ModelResponse)


@pytest.mark.parametrize(
    "metadata_params",
    [{"metadata": {"user_id": "test"}}, {"derouter_metadata": {"user": "test"}}, {}],
)
def test_create_anthropic_response_logging_payload(mock_logging_obj, metadata_params):
    ...
    result = AnthropicPassthroughLoggingHandler._create_anthropic_response_logging_payload(
        derouter_model_response=model_response,
        model="claude-opus-4-7",
        kwargs={
            "derouter_params": {
                "metadata": {
            ...
            "derouter_call_id": "5cf924cb-161c-4c1d-a565-31aa71ab50ab",
...
@pytest.mark.parametrize(
    "end_user_id",
    [{"derouter_metadata": {"user": "test"}}, {"metadata": {"user_id": "test"}}],
)
