...
async def test_route_streaming_logging_runs_async_handler_for_sdk_passthrough():
    ...
    derouter_params carries no ``acompletion`` flag, so only the sync success_handler
    ...
    logging_obj.model_call_details["derouter_params"] = {"anthropic_messages": True}
...
async def test_handle_logging_runs_async_handler_for_passthrough():
    ...
    call_type that _is_sync_derouter_request would classify as sync (here
    "completion" with no async marker in derouter_params) must still reach
