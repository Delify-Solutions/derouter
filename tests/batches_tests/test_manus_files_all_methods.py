...
async def test_manus_files_api_e2e_all_methods():
    ...
    created_file = await derouter.acreate_file(
    ...
    retrieved_file = await derouter.afile_retrieve(
        file_id=file_id,
        custom_llm_provider="manus",
        api_key=api_key,
    )
    ...
    files_list = await derouter.afile_list(
        custom_llm_provider="manus",
        api_key=api_key,
    )
    ...
    deleted_file = await derouter.afile_delete(
        file_id=file_id,
        custom_llm_provider="manus",
        api_key=api_key,
    )
