...
async def cancel_batch_unless_already_terminal(batch_id: str, provider: str) -> None:
    try:
        cancel_batch_response = await derouter.acancel_batch(batch_id=batch_id, custom_llm_provider=provider)
        ...
        if "Cannot cancel a batch with status 'failed'" not in str(e):
            raise
        failed_batch = await derouter.aretrieve_batch(batch_id=batch_id, custom_llm_provider=provider)
...
async def test_create_batch(provider, tmp_path):
    ...
    with open(file_path, "rb") as batch_file:
        file_obj = await derouter.acreate_file(
            file=batch_file,
            purpose="batch",
    ...
    create_batch_response = await derouter.acreate_batch(
    ...
    print("response from derouter.create_batch=", create_batch_response)
    ...
    retrieved_batch = await derouter.aretrieve_batch(
    ...
    # list all batches
    list_batches = await derouter.alist_batches(custom_llm_provider=provider, limit=2)
    print("list_batches=", list_batches)

    file_content = await derouter.afile_content(
...
def cleanup_azure_files():
    ...
    """
    azure_files = derouter.file_list(
    ...
    for _file in azure_files:
        ...
        delete_file_response = derouter.file_delete(
            file_id=_file.id,
            custom_llm_provider="azure",
            api_key=os.getenv("AZURE_FT_API_KEY"),
            api_base=os.getenv("AZURE_FT_API_BASE"),
...
async def test_async_create_batch(provider, tmp_path):
    ...
    derouter.callbacks = [custom_logger, "datadog"]
    create_batch_response = await derouter.acreate_batch(
        completion_window="24h",
        endpoint="/v1/chat/completions",
        input_file_id=batch_input_file_id,
        custom_llm_provider=provider,
        metadata={"key1": "value1", "key2": "value2"},
        # derouter specific param - used for logging metadata on logging callback
        derouter_metadata=extra_metadata_field,
    )
    ...
    # file obj
    file_obj = await derouter.afile_retrieve(
        file_id=batch_input_file_id, custom_llm_provider=provider
    )
    ...
    # delete file
    delete_file_response = await derouter.afile_delete(
        file_id=batch_input_file_id, custom_llm_provider=provider
    )
    ...
    all_files_list = await derouter.afile_list(
        custom_llm_provider=provider,
    )
...
mock_file_response = {
    "kind": "storage#object",
    "id": "derouter-local/derouter-vertex-files/publishers/google/models/gemini-1.5-flash-001/5f7b99ad-9203-4430-98bf-3b45451af4cb/1739598666670574",
    "selfLink": "https://www.googleapis.com/storage/v1/b/derouter-local/o/derouter-vertex-files%2Fpublishers%2Fgoogle%2Fmodels%2Fgemini-1.5-flash-001%2F5f7b99ad-9203-4430-98bf-3b45451af4cb",
    "mediaLink": "https://storage.googleapis.com/download/storage/v1/b/derouter-local/o/derouter-vertex-files%2Fpublishers%2Fgoogle%2Fmodels%2Fgemini-1.5-flash-001%2F5f7b99ad-9203-4430-98bf-3b45451af4cb?generation=1739598666670574&alt=media",
    "name": "derouter-vertex-files/publishers/google/models/gemini-1.5-flash-001/5f7b99ad-9203-4430-98bf-3b45451af4cb",
    "bucket": "derouter-local",
...
mock_vertex_batch_response = {
    ...
    "displayName": "derouter_batch_job",
    "model": "projects/123456789/locations/us-central1/models/gemini-1.5-flash-001",
    "modelVersionId": "v1",
    "inputConfig": {
        "gcsSource": {
            "uris": [
                "gs://derouter-local/derouter-vertex-files/publishers/google/models/gemini-1.5-flash-001/5f7b99ad-9203-4430-98bf-3b45451af4cb"
            ]
        }
    },
    "outputConfig": {
        "gcsDestination": {"outputUriPrefix": "gs://derouter-local/batch-outputs/"}
    },
    "dedicatedResources": {
        "machineSpec": {
...
async def test_avertex_batch_prediction(monkeypatch):
    monkeypatch.setenv("GCS_BUCKET_NAME", "derouter-local")
    ...
    with (
        ...
        assert "/b/derouter-local/o" in upload_url
        assert (
        ...
        with patch(
            "derouter.llms.custom_httpx.http_handler.AsyncHTTPHandler.get"
        ) as mock_get:
...
async def test_vertex_list_batches(monkeypatch):
    monkeypatch.setenv("GCS_BUCKET_NAME", "derouter-local")
    monkeypatch.setenv("VERTEXAI_PROJECT", "derouter-test-project")
    monkeypatch.setenv("VERTEXAI_LOCATION", "us-central1")

    monkeypatch.setattr(
        "derouter.llms.vertex_ai.batches.handler.VertexAIBatchPrediction._ensure_access_token",
        lambda self, credentials, project_id, custom_llm_provider: (
            "mock-token",
            "derouter-test-project",
        ),
    )
    ...
    with patch(
        ...
        list_response = await derouter.alist_batches(
            custom_llm_provider="vertex_ai",
            limit=2,
        )
...
async def test_delete_batch_output_file():
    ...
    if retrieved_batch.output_file_id:
        ...
        delete_output_file_response = await derouter.afile_delete(
    ...
    delete_input_file_response = await derouter.afile_delete(
