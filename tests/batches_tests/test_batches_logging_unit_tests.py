...
import derouter
from derouter import create_batch, create_file
...
def test_get_batch_job_total_usage_from_file_content(sample_file_content_dict):
    with patch("derouter.completion_cost", return_value=0.0):
...
async def test_batch_cost_calculator(sample_file_content_dict):
    ...
    """
    with patch("derouter.cost_calculator.batch_cost_calculator", return_value=(0.3, 0.2)):
        result = _aggregate_batch_cost_usage_models(
...
async def test_batch_retrieve_cost_tracking_with_completed_batch_no_explicit_cost():
    ...
    with patch(
        "derouter.derouter_core_utils.derouter_logging._handle_completed_batch",
        new=AsyncMock(
...
async def test_handle_completed_batch_computes_real_cost_from_output_file(
    ...
    """
    from derouter.batches.batch_utils import _handle_completed_batch
    from derouter.types.utils import DeRouterBatch
    ...
    with patch(
        "derouter.batches.batch_utils._fetch_batch_output_file_content",
    ...
    pricing = derouter.model_cost["gpt-4o-mini-2024-07-18"]
    expected_cost = (
...
async def test_batch_retrieve_explicit_cost_split_sets_cost_breakdown():
    ...
    from derouter.derouter_core_utils.derouter_logging import Logging
    from derouter.types.utils import CallTypes, DeRouterBatch
    ...
    logging_obj = Logging(
        ...
        stream=False,
        call_type=CallTypes.aretrieve_batch.value,
        derouter_call_id="test-call-breakdown",
        function_id="test-function",
        start_time=time.time(),
        dynamic_success_callbacks=[],
    )
    ...
    await logging_obj.async_success_handler(
        ...
        end_time=time.time() + 1,
        batch_cost=0.10,
        batch_usage=derouter.Usage(prompt_tokens=200, completion_tokens=100, total_tokens=300),
...
async def test_batch_retrieve_cost_tracking_with_unified_file_id_incomplete_batch():
    ...
    import base64
    from derouter.derouter_core_utils.derouter_logging import Logging
    from derouter.types.utils import CallTypes, SpecialEnums
    from derouter.types.utils import DeRouterBatch
    from unittest.mock import AsyncMock, patch
...
async def test_batch_retrieve_cost_tracking_with_partial_explicit_data():
    ...
    logging_obj = Logging(
        ...
        stream=False,
        call_type=CallTypes.aretrieve_batch.value,
        derouter_call_id="test-call-partial",
        function_id="test-function",
        start_time=time.time(),
        dynamic_success_callbacks=[],
    )
