...
def get_expected_batch_file_usage(file_path: str) -> tuple[int, int]:
    ...
    for item in file_contents:
        ...
        if messages:
            item_tokens = derouter.token_counter(model=model, messages=messages)
...
{"custom_id": "request-3", "method": "POST", "url": "/v1/chat/completions", "body": {"model": "gpt-3.5-turbo", "messages": [{"role": "user", "content": "Hey"}]}}"""
    ...
    try:
        ...
        with open(small_file_path, "rb") as batch_file:
            file_obj_small = await derouter.acreate_file(
                file=batch_file,
                purpose="batch",
                custom_llm_provider=CUSTOM_LLM_PROVIDER,
            )
    ...
    with open(large_file_path, "rb") as batch_file:
        file_obj_large = await derouter.acreate_file(
            file=batch_file,
            purpose="batch",
            custom_llm_provider=CUSTOM_LLM_PROVIDER,
        )
...
async def test_batch_rate_limit_multiple_requests(tmp_path):
    ...
    try:
        ...
        with open(file_path_1, "rb") as batch_file:
            file_obj_1 = await derouter.acreate_file(
                file=batch_file,
                purpose="batch",
                custom_llm_provider=CUSTOM_LLM_PROVIDER,
            )
    ...
    with open(file_path_2, "rb") as batch_file:
        file_obj_2 = await derouter.acreate_file(
            file=batch_file,
            purpose="batch",
            custom_llm_provider=CUSTOM_LLM_PROVIDER,
        )
...
async def test_batch_rate_limiter_with_managed_files(tmp_path):
    ...
    try:
        ...
        original_afile_content = derouter.afile_content
        ...
        with patch("derouter.afile_content", side_effect=mock_afile_content):
            data = {
                "model": "gpt-3.5-turbo",
...
async def test_batch_rate_limiter_managed_files_regression():
    ...
    with patch.dict(
        "sys.modules",
        {
            "derouter.proxy.proxy_server": MagicMock(
    ...
    with patch("derouter.afile_content") as mock_afile_content:
        mock_response = httpx.Response(
            status_code=200,
            content=batch_content,
            headers={"content-type": "application/octet-stream"},
        ...
        # Verify derouter.afile_content was called
        assert (
            mock_afile_content.called
        ), "REGRESSION: derouter.afile_content not called for non-managed files"
    ...
    with patch.object(batch_limiter, "_fetch_managed_file_content") as mock_fetch:
        ...
        try:
            with patch("derouter.afile_content") as mock_afile_content:
                # If derouter.afile_content is called for managed files, bug exists
                ...
                # This should use _fetch_managed_file_content, not derouter.afile_content
                usage = await batch_limiter.count_input_file_usage(
                    file_id=managed_file_id,
                    custom_llm_provider="openai",
                    user_api_key_dict=user_api_key_dict,
...
async def test_batch_logging_azure_credentials_regression():
    ...
    Fix: Pass derouter_params (containing credentials) from the logging object
    ...
    from derouter.types.llms.openai import Batch, HttpxBinaryResponseContent
    import httpx
    ...
    # Setup: Azure credentials (as they would be in derouter_params)
    azure_credentials = {
    ...
    with patch(
        "derouter.files.main.afile_content", side_effect=mock_afile_content_tracker
    ):
        result = await _fetch_batch_output_file_content(
            batch=mock_batch,
            custom_llm_provider="azure",
            derouter_params=azure_credentials,
        )
    ...
    with patch("derouter.files.main.afile_content") as mock_afile_content_fail:
    ...
    with patch("derouter.files.main.afile_content") as mock_afile_content:
        mock_response = httpx.Response(
            status_code=200,
            content=batch_output,
            headers={"content-type": "application/octet-stream"},
        ...
        # Call without derouter_params (should still work for OpenAI)
        result = await _fetch_batch_output_file_content(
            batch=mock_batch,
            custom_llm_provider="openai",
        ...
        print("   ✓ Works without derouter_params for OpenAI")
