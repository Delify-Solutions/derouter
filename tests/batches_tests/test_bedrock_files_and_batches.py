...
async def test_async_create_file():
    ...
    with (
        ...
        file_obj = await derouter.acreate_file(
            ...
            s3_bucket_name="derouter-proxy-941277531214",
    ...
    assert put_call["url"].startswith(
        "https://s3.us-west-2.amazonaws.com/derouter-proxy-941277531214/"
    )
    assert "/derouter-bedrock-files-us.anthropic.claude-haiku-4-5-20251001-v1-0-" in (
        put_call["url"]
    )
    ...
    assert file_obj.id.startswith(
        "s3://derouter-proxy-941277531214/derouter-bedrock-files-"
    )
    ...
    with patch.dict(os.environ, _BEDROCK_TEST_AWS_ENV):
        ...
        with patch(
            ...
            with patch("boto3.client", return_value=mock_bedrock_client):
                retrieve_batch_response = await derouter.aretrieve_batch(
...
async def test_bedrock_retrieve_batch():
    ...
    with (
        ...
        patch(
            "derouter.llms.bedrock.batches.transformation.BedrockBatchesConfig.get_credentials",
            return_value=mock_creds,
        ),
    ):
        batch_response = await derouter.aretrieve_batch(
...
def test_bedrock_batch_with_encryption_key_in_post_request():
    ...
    with (
        ...
        response = derouter.create_batch(
            completion_window="24h",
            endpoint="/v1/chat/completions",
