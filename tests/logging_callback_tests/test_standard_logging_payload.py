...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Test with derouter.Usage object
        (
...
def test_get_model_cost_information():
    ...
    derouter_info_gpt_3_5_turbo_model_map_value = derouter.get_model_info(
    ...
    assert result["model_map_value"] == derouter_info_gpt_3_5_turbo_model_map_value
    # assert all fields in StandardLoggingModelInformation are present
    assert all(
        field in result for field in StandardLoggingModelInformation.__annotations__
    )
...
def test_standard_logging_payload_uses_deployment_when_no_base_model():
    ...
    logging_obj = Logging(
        ...
        derouter_call_id="test-deploy-fallback",
        function_id="test-fn",
    )
...
def test_get_final_response_obj():
    ...
    # Test redaction when derouter.turn_off_message_logging is True
    derouter.turn_off_message_logging = True
    try:
        model_response = derouter.ModelResponse(
            choices=[
                derouter.Choices(message=derouter.Message(content="sensitive content"))
            ]
        )
        ...
        assert result["choices"][0]["message"]["content"] == "redacted-by-derouter"
        # Verify that redaction occurred in kwargs
        assert kwargs["messages"][0]["content"] == "redacted-by-derouter"
    finally:
        # Reset derouter.turn_off_message_logging to its original value
...
def testget_standard_logging_payload_trace_id():
    ...
    # Test case 1: When derouter_trace_id is provided in derouter_params
    from unittest.mock import MagicMock

    # Create a mock Logging object
    mock_logging_obj = MagicMock()
    mock_logging_obj.derouter_trace_id = "default-trace-id"

    # Test when derouter_trace_id is in derouter_params
    derouter_params = {"derouter_trace_id": "dynamic-trace-id"}
    result = StandardLoggingPayloadSetup.get_standard_logging_payload_trace_id(
        logging_obj=mock_logging_obj, derouter_params=derouter_params
    )
    assert result == "dynamic-trace-id"

    # Test case 2: When derouter_trace_id is not provided in derouter_params
    derouter_params = {}
    result = StandardLoggingPayloadSetup.get_standard_logging_payload_trace_id(
        logging_obj=mock_logging_obj, derouter_params=derouter_params
    )
    ...
    # Test case 3: When derouter_params is None
    result = StandardLoggingPayloadSetup.get_standard_logging_payload_trace_id(
        logging_obj=mock_logging_obj, derouter_params={}
    )
    assert result == "default-trace-id"

    # Test case 4: When derouter_trace_id in params is not a string
    derouter_params = {"derouter_trace_id": 12345}
    result = StandardLoggingPayloadSetup.get_standard_logging_payload_trace_id(
        logging_obj=mock_logging_obj, derouter_params=derouter_params
    )
    assert result == "12345"
...
def testget_standard_logging_payload_trace_id_prioritizes_trace_id_when_flag_on(monkeypatch):
    """With request_correlation_in_logs on, an explicit derouter_trace_id wins over derouter_session_id."""
    from unittest.mock import MagicMock
    ...
    derouter_params = {"derouter_trace_id": "the-trace-id", "derouter_session_id": "the-session-id"}
    result = StandardLoggingPayloadSetup.get_standard_logging_payload_trace_id(
        logging_obj=mock_logging_obj, derouter_params=derouter_params
    )
...
def testget_standard_logging_payload_trace_id_prioritizes_session_id_when_flag_off(monkeypatch):
    ...
    derouter_session_id still wins over derouter_trace_id."""
    from unittest.mock import MagicMock

    monkeypatch.setattr(derouter, "request_correlation_in_logs", False)
    mock_logging_obj = MagicMock()
    mock_logging_obj.derouter_trace_id = "default-trace-id"
...
def testget_standard_logging_payload_session_id_when_flag_on(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "request_correlation_in_logs", True)
    mock_logging_obj = MagicMock()
    mock_logging_obj.derouter_session_id = ""

    # Test case 1: derouter_session_id provided directly in derouter_params
    derouter_params = {"derouter_session_id": "dynamic-session-id"}
    result = StandardLoggingPayloadSetup.get_standard_logging_payload_session_id(
        logging_obj=mock_logging_obj, derouter_params=derouter_params
    )
    assert result == "dynamic-session-id"

    # Test case 2: falls back to metadata.session_id when not in derouter_params directly
    derouter_params = {"metadata": {"session_id": "metadata-session-id"}}
    result = StandardLoggingPayloadSetup.get_standard_logging_payload_session_id(
        logging_obj=mock_logging_obj, derouter_params=derouter_params
    )
    assert result == "metadata-session-id"

    # Test case 3: falls back to logging_obj.derouter_session_id when nothing else is set
    mock_logging_obj.derouter_session_id = "obj-session-id"
    result = StandardLoggingPayloadSetup.get_standard_logging_payload_session_id(
        logging_obj=mock_logging_obj, derouter_params={}
    )
    ...
    derouter_params = {"derouter_session_id": 98765}
    result = StandardLoggingPayloadSetup.get_standard_logging_payload_session_id(
        logging_obj=mock_logging_obj, derouter_params=derouter_params
    )
    assert result == "98765"
    ...
    derouter_params = {"derouter_trace_id": "some-trace-id"}
    result = StandardLoggingPayloadSetup.get_standard_logging_payload_session_id(
        logging_obj=mock_logging_obj, derouter_params=derouter_params
    )
    assert result == ""
...
def testget_standard_logging_payload_session_id_empty_when_flag_off(monkeypatch):
    ...
    even if derouter_session_id was explicitly supplied - preserves the pre-existing
...
def test_get_error_information():
    ...
    # Test with derouter exception from provider
    derouter_exception = derouter.exceptions.RateLimitError(
        message="Test error",
        llm_provider="openai",
        model="gpt-5-mini",
        response=None,
        derouter_debug_info=None,
        max_retries=None,
        num_retries=None,
    )
    ...
    assert result["error_message"] == "derouter.RateLimitError: Test error"
...
def test_usage_dict_roundtrip_in_payload(use_combined_usage_object):
    ...
    logging_obj = Logging(
        ...
        call_type="completion",
        start_time=datetime.now(),
        derouter_call_id="test-usage-roundtrip",
        function_id="test-fn",
    )
...
def test_standard_logging_payload_uses_actual_model_for_azure_router():
    ...
    logging_obj = Logging(
        ...
        call_type="completion",
        start_time=datetime.now(),
        derouter_call_id="test-azure-router-opt-in",
        function_id="test-fn",
    )
...
def test_standard_logging_payload_uses_actual_model_for_azure_router_with_underscore():
    ...
    logging_obj = Logging(
        ...
        call_type="completion",
        start_time=datetime.now(),
        derouter_call_id="test-azure-router-underscore",
        function_id="test-fn",
    )
...
def test_merge_derouter_metadata_basic():
    """
    Test that merge_derouter_metadata correctly merges metadata and derouter_metadata.
    User API key fields (from metadata) should take precedence over model-related fields (from derouter_metadata).
    """
    derouter_params = {
        "metadata": {
    ...
    result = StandardLoggingPayloadSetup.merge_derouter_metadata(derouter_params)
...
def test_merge_derouter_metadata_precedence():
    """
    Test that metadata fields take precedence over derouter_metadata when there are conflicts.
    """
    derouter_params = {
        "metadata": {
        ...
        "derouter_metadata": {
            ...
            "custom_field": "from_derouter_metadata",  # This should NOT overwrite
    ...
    # derouter_metadata values should only be included if not in metadata
    assert result["model_group"] == "gpt-4-group"


def test_merge_derouter_metadata_skip_non_serializable():
    """
...
def test_merge_derouter_metadata_empty_params():
    """
    Test that merge_derouter_metadata handles empty or missing metadata gracefully.
    """
    # Test with empty derouter_params
    result = StandardLoggingPayloadSetup.merge_derouter_metadata({})
    assert result == {}
    ...
    # Test with only derouter_metadata
    derouter_params = {
        "derouter_metadata": {
            "model_group": "gpt-4-group",
        }
...
def test_merge_derouter_metadata_bedrock_passthrough_scenario():
    """
    Test merge_derouter_metadata in a Bedrock passthrough scenario where both
