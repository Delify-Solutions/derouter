...
class TestOpentelemetryUnitTests(BaseLoggingCallbackTest):
    ...
    async def test_opentelemetry_integration(self):
        """
        Unit test to confirm external parent otel spans are NOT ended by DeRouter.

        External spans (passed via metadata) should be managed by their creators,
        not by DeRouter. This prevents premature closure of spans from Langfuse,
        ...
        derouter.callbacks = ["otel"]

        await derouter.acompletion(
            model="gpt-5-mini",
            messages=[{"role": "user", "content": "Hello, world!"}],
            mock_response="Hey!",
            metadata={"derouter_parent_otel_span": parent_otel_span},
        )

        await asyncio.sleep(1)

        # Verify external span was NOT ended by DeRouter
    ...
    def test_record_exception_on_span(self):
        ...
        from opentelemetry.sdk.trace import TracerProvider
        from derouter.integrations.opentelemetry import OpenTelemetry
        from derouter.integrations._types.open_inference import ErrorAttributes
