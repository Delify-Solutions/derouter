...
async def use_callback_in_llm_call(
    ...
    if callback == "dynamic_rate_limiter":
        ...
        derouter.argilla_transformation_object = {}
        ...
        derouter.global_bitbucket_config = {
            "workspace": "test-workspace",
            "repository": "test-repo",
            "access_token": "test-token",
            "branch": "main",
        }
        derouter.global_gitlab_config = {
    ...
    if used_in == "callbacks":
        ...
        derouter.success_callback = [callback]
    ...
    for _ in range(5):
        ...
        if used_in == "callbacks":
            assert isinstance(derouter._async_success_callback[0], expected_class)
            assert isinstance(derouter._async_failure_callback[0], expected_class)
            assert isinstance(derouter.success_callback[0], expected_class)
            assert isinstance(derouter.failure_callback[0], expected_class)

            assert (
                len(derouter._async_success_callback) == 1
            ), f"Got={derouter._async_success_callback}"
            assert len(derouter._async_failure_callback) == 1
            assert len(derouter.success_callback) == 1
            assert len(derouter.failure_callback) == 1
            assert len(derouter.callbacks) == 1
        elif used_in == "success_callback":
            print(f"derouter.success_callback: {derouter.success_callback}")
            print(f"derouter._async_success_callback: {derouter._async_success_callback}")
            assert isinstance(derouter.success_callback[0], expected_class)
            assert len(derouter.success_callback) == 1  # ["lago", LagoLogger]
            assert isinstance(derouter._async_success_callback[0], expected_class)
        ...
        if callback == "bitbucket":
            ...
            if hasattr(derouter, "global_bitbucket_config"):
                delattr(derouter, "global_bitbucket_config")
            patch.stopall()
...
def test_dynamic_logging_global_callback():
    ...
    from derouter.types.utils import ModelResponse, Choices, Message, Usage

    cl = CustomLogger()

    derouter_logging = DeRouterLoggingObj(
        model="claude-opus-4-7",
        messages=[{"role": "user", "content": "hi"}],
        stream=False,
        call_type="completion",
        start_time=datetime.now(),
        derouter_call_id="123",
        function_id="456",
        kwargs={
    ...
    with patch.object(cl, "log_success_event") as mock_log_success_event:
        ...
        derouter.success_callback = [cl]

        try:
            derouter_logging.success_handler(
