...
def test_langfuse_e2e_sync(monkeypatch):
    ...
    monkeypatch.setattr(derouter, "success_callback", ["langfuse"])
    ...
    with respx.mock:
        ...
        completion(
            ...
            messages=[{"role": "user", "content": "hello from derouter"}],
            stream=False,
            mock_response="Hello from derouter 2",
        )
...
def test_masking_function_isolated_from_other_loggers():
    ...
    # Simulate derouter_params with masking function in metadata
    derouter_params = {
        "metadata": {
    ...
    result = scrub_sensitive_keys_in_metadata(derouter_params)
