...
import asyncio
import derouter
import derouter.vector_stores.main
import gzip
import json
import logging
import time
...
def setup_vector_store_registry():
    from derouter.vector_stores.vector_store_registry import (
        VectorStoreRegistry,
...
async def test_vector_store_hook_routes_search_through_proxy_router(
    ...
    logging_obj.model_call_details = {
        "derouter_params": {"metadata": {"user_api_key_team_id": "team-a"}}
    }

    with patch("derouter.proxy.proxy_server.llm_router", proxy_router):
        _, messages, _ = await VectorStorePreCallHook().async_get_chat_completion_prompt(
            model="chat-model",
            messages=[{"role": "user", "content": "what is derouter?"}],
...
async def test_e2e_bedrock_knowledgebase_retrieval_with_completion(
    ...
    print("value of derouter.vector_store_registry:", derouter.vector_store_registry)
        ...
        mock_response.text = json.dumps(
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": "DeRouter is a library that simplifies LLM API access.",
                    }
                ],
...
async def test_e2e_bedrock_knowledgebase_retrieval_with_llm_api_call_with_tools_and_filters(
    ...
    # The logs above show:  derouter.asearch(... filters={'key': 'user_id', 'value': 'fake-user-id', 'operator': 'eq'})
...
async def test_bedrock_kb_request_body_has_transformed_filters(
        ...
        return VectorStoreSearchResponse(
            ...
            data=[
                VectorStoreSearchResult(
                    ...
                    content=[
                        VectorStoreResultContent(
                            text="DeRouter is a library", type="text"
                        )
                    ],
                )
            ],
    ...
    with patch.object(
        derouter.vector_stores.main.base_llm_http_handler,
...
async def test_openai_with_vector_store_ids_in_tool_call_mock_openai(
    ...
    with patch.object(
        ...
        # assert that the tool call was not sent to the upstream llm API if it's a derouter vector store
...
#     assert vector_store_request_metadata.get("query") == "what is derouter?"
...
#     assert vector_store_search_response.get("search_query") == "what is derouter?"
...
async def test_e2e_bedrock_knowledgebase_retrieval_with_vector_store_not_in_registry(
    ...
    if derouter.vector_store_registry is not None:
        print("Registry iniitalized:", derouter.vector_store_registry.vector_stores)
    else:
...
async def test_provider_specific_fields_in_proxy_http_response(
    ...
    mock_message = derouter.Message(
        content="DeRouter is a tool that simplifies working with multiple LLMs.",
        role="assistant",
    ...
    mock_choice = derouter.Choices(finish_reason="stop", index=0, message=mock_message)

    mock_response.choices = [mock_choice]
    mock_response.usage = derouter.Usage(
        prompt_tokens=10, completion_tokens=20, total_tokens=30
    )

    # Patch the completion call at the proxy level
    with mock_patch("derouter.acompletion", new=AsyncMock(return_value=mock_response)):
