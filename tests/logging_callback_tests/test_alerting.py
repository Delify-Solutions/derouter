...
from derouter.integrations.SlackAlerting.slack_alerting import (
    DeploymentMetrics,
    SlackAlerting,
)
from derouter.proxy._types import CallInfo, Litellm_EntityType, WebhookEvent
from derouter.proxy.utils import ProxyLogging
from derouter.router import AlertingConfig, Router
from derouter.utils import get_api_base
...
async def test_get_api_base():
    ...
    derouter_params = {
        ...
        "derouter_call_id": "68f46d2d-714d-4ad8-8137-69600ec8755c",
        "model_alias_map": {},
...
async def test_response_taking_too_long_callback(slack_alerting):
    ...
    kwargs = {"model": "test_model", "messages": "test_messages", "derouter_params": {}}
...
async def test_alerting_metadata(slack_alerting):
    ...
    kwargs = {
        ...
        "derouter_params": {"metadata": {"alerting_metadata": {"hello": "world"}}},
    }
    with patch.object(slack_alerting, "send_alert", new=AsyncMock()) as mock_send_alert:
...
async def test_daily_reports_completion(slack_alerting):
    with patch.object(slack_alerting, "send_alert", new=AsyncMock()) as mock_send_alert:
        derouter.callbacks = [slack_alerting]
        ...
        router = derouter.Router(
            model_list=[
                {
                    "model_name": "gpt-5.5",
                    "derouter_params": {"model": "gpt-5-mini", "api_key": "bad_key"},
                }
            ]
        )
...
async def test_send_daily_reports_ignores_zero_values():
    ...
    router.get_model_info.side_effect = lambda x: {"derouter_params": {"model": x}}
...
async def test_outage_alerting_called(
    ...
    if error_code == 400:
        ...
        error_to_raise = derouter.BadRequestError(
...
async def test_langfuse_trace_id():
    ...
    trace_url = await _add_langfuse_trace_id_to_alert(
        request_data={"derouter_logging_obj": derouter_logging_obj}
    )
    ...
    assert returned_trace_id == derouter_logging_obj._get_trace_id(
...
async def test_print_alerting_payload_warning():
    ...
    from derouter.integrations.SlackAlerting.batching_handler import send_to_webhook
    import logging
