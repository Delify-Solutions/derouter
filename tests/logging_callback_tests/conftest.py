...
#
# Key design: capture derouter's true default values at conftest import time
...
# globals like `derouter.num_retries = 3` which pollute state for all tests
...
def isolate_derouter_state():
    ...
    Resets derouter state to the true defaults captured at conftest import time,
    then restores after the test. This prevents module-level mutations (e.g.
    `derouter.num_retries = 3` at the top of test_langfuse_e2e_test.py) from
    ...
    for attr in _LIST_ATTRS:
        if attr in _DEFAULTS:
            ...
            setattr(
                derouter, attr, default.copy() if isinstance(default, list) else default
            )

    for attr in _SCALAR_ATTRS:
        if attr in _DEFAULTS:
            setattr(derouter, attr, _DEFAULTS[attr])
