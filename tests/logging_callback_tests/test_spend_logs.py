...
def test_spend_logs_payload(model_id: Optional[str]):
    ...
    input_args: dict = {
        "kwargs": {
            ...
            "derouter_params": {
                ...
                "derouter_call_id": "b9929bf6-7b80-4c8c-b486-034e6ac0c8b7",
                "model_alias_map": {},
    ...
    # This is crucial - used in PROD, it should pass, related issue: https://github.com/Delify-Solutions/derouter/issues/4334
    assert (
...
def test_spend_logs_payload_whisper():
    ...
    kwargs: dict = {
        ...
        "derouter_call_id": "05921cf7-33f9-421c-aad9-33310c1e2702",
    ...
    response = derouter.utils.TranscriptionResponse(
...
def test_spend_logs_payload_with_prompts_enabled(monkeypatch):
        ...
        "response_obj": derouter.ModelResponse(
            ...
            choices=[
                derouter.Choices(
                    finish_reason="stop",
                    index=0,
                    message=derouter.Message(content="Hi there!", role="assistant"),
                )
            ],
            model="gpt-5-mini",
            usage=derouter.Usage(completion_tokens=2, prompt_tokens=1, total_tokens=3),
        ),
        "start_time": datetime.datetime.now(),
        "end_time": datetime.datetime.now(),
    }
    ...
    input_args["kwargs"]["derouter_params"] = derouter_params
...
def test_large_request_no_truncation_threshold():
    ...
    assert DEROUTER_TRUNCATED_PAYLOAD_FIELD in truncated_content
...
def test_configurable_string_length_env_var(monkeypatch):
    ...
    import derouter.proxy.spend_tracking.spend_tracking_utils

    importlib.reload(derouter.constants)
    importlib.reload(derouter.proxy.spend_tracking.spend_tracking_utils)
