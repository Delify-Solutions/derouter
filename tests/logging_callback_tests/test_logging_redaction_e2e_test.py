...
async def test_global_redaction_on():
    ...
    assert standard_logging_payload["messages"][0]["content"] == "redacted-by-derouter"
    print(
...
async def test_dynamic_turn_off_message_logging_overrides_global_on(dynamic_turn_off, expect_redacted):
    ...
    expected_response_content = "redacted-by-derouter" if expect_redacted else "hello"
    expected_message_content = "redacted-by-derouter" if expect_redacted else "hi"
...
async def test_redaction_with_custom_logger_streaming():
    ...
    try:
        derouter_logging_obj = LoggingWithoutSyncSuccessHandler(
            model="gpt-5-mini",
            messages=[{"role": "user", "content": "hi"}],
            stream=True,
        ...
        assert async_complete_streaming_response.choices[0].message.content == "redacted-by-derouter"
    finally:
        derouter.turn_off_message_logging = False
...
async def test_streaming_redaction_scoped_to_opted_out_logger():
    ...
    derouter.callbacks = [opted_out_logger, compliant_logger]
    ...
    try:
        ...
        assert opted_out_logger.response_obj.choices[0].message.content == "redacted-by-derouter"
        ...
        if "content" in output_item and isinstance(output_item["content"], list):
            for content_item in output_item["content"]:
                if "text" in content_item:
                    assert (
                        content_item["text"] == "redacted-by-derouter"
...
async def test_redaction_responses_api_with_reasoning_summary():
    ...
    import derouter
    from derouter.derouter_core_utils.redact_messages import perform_redaction

    response = derouter.ResponsesAPIResponse(
        id="resp_123",
    ...
    assert (
        model_call_details["messages"][0]["content"] == "redacted-by-derouter"
...
async def test_redaction_with_coroutine_objects():
    ...
    assert result == {"text": "redacted-by-derouter"}
...
async def test_disable_redaction_header_responses_api():
    """
    Test that DeRouter-Disable-Message-Redaction header works for Responses API.

    This test verifies the fix for the issue where the header wasn't respected
    because Responses API uses 'derouter_metadata' instead of 'metadata'.
    """
    derouter.turn_off_message_logging = True
    test_custom_logger = TestCustomLogger()
    derouter.callbacks = [test_custom_logger]

    # Pass the header via derouter_metadata (as the proxy does for Responses API)
    response = await derouter.aresponses(
        model="gpt-5-mini",
        input="hi",
        mock_response="This is a test response",
        derouter_metadata={"headers": {"derouter-disable-message-redaction": "true"}},
    )
...
async def test_redaction_with_metadata_completion_api():
    ...
    # based on the global setting (derouter.turn_off_message_logging = True)
    response = await derouter.acompletion(
        model="gpt-5-mini",
        messages=[{"role": "user", "content": "hi"}],
        mock_response="hello",
