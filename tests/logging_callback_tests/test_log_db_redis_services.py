...
from derouter.proxy.utils import log_db_metrics, ServiceTypes
...
async def test_log_db_metrics_event_metadata_is_safe():
    ...
    with patch("derouter.proxy.proxy_server.proxy_logging_obj") as mock_proxy_logging:
        ...
        assert call_args["event_metadata"] == {"table_name": "DeRouter_SpendLogs"}
...
async def test_dd_log_db_spend_failure_metrics():
    ...
    with patch.object(dd_logger, "async_service_failure_hook", new_callable=AsyncMock):
        ...
        derouter.service_callback = [dd_logger]
