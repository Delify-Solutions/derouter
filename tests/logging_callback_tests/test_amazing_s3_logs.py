...
async def test_basic_s3_logging(sync_mode, streaming):
    ...
    derouter.success_callback = ["s3"]
    derouter.s3_callback_params = {
...
async def test_basic_s3_v2_logging(streaming):
    ...
    derouter.callbacks = [s3_v2_logger]
...
def test_s3_logging():
    ...
    # on circle ci - we only test derouter.acompletion()
    try:
        ...
        derouter.success_callback = ["s3"]
        derouter.s3_callback_params = {
            "s3_bucket_name": "derouter-logs-2",
            "s3_aws_secret_access_key": "os.environ/AWS_SECRET_ACCESS_KEY",
            "s3_aws_access_key_id": "os.environ/AWS_ACCESS_KEY_ID",
        }
        derouter.set_verbose = True
