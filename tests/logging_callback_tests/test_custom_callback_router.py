...
import derouter
from derouter import Cache, Router
...
class CompletionCustomHandler(
    ...
    def log_pre_api_call(self, model, messages, kwargs):
        try:
            ...
            assert isinstance(kwargs["derouter_params"]["metadata"], dict)
            assert isinstance(kwargs["derouter_params"]["metadata"]["model_group"], str)
            assert isinstance(kwargs["derouter_params"]["metadata"]["deployment"], str)
            assert isinstance(kwargs["derouter_params"]["model_info"], dict)
            assert isinstance(kwargs["derouter_params"]["model_info"]["id"], str)
            assert isinstance(
                kwargs["derouter_params"]["proxy_server_request"], (str, type(None))
            )
            assert isinstance(
                kwargs["derouter_params"]["preset_cache_key"], (str, type(None))
            )
            assert isinstance(kwargs["derouter_params"]["stream_response"], dict)
    ...
    def log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            assert isinstance(end_time, datetime)
            ## RESPONSE OBJECT
            assert isinstance(response_obj, derouter.ModelResponse)
            ## KWARGS
            assert isinstance(kwargs["model"], str)
            assert isinstance(kwargs["messages"], list) and isinstance(
                kwargs["messages"][0], dict
    ...
    async def async_log_success_event(self, kwargs, response_obj, start_time, end_time):
        try:
            ...
            ## RESPONSE OBJECT
            assert isinstance(
                response_obj, (derouter.ModelResponse, derouter.EmbeddingResponse)
            )
            ## KWARGS
            assert isinstance(kwargs["model"], str)

            # checking we use base_model for azure cost calculation
            base_model = derouter.utils._get_base_model_from_metadata(
            ...
            if (
                ...
                # this checks response_cost == derouter.cost_per_token(model=base_model)
                ...
                prompt_price, completion_price = derouter.cost_per_token(
                    model=base_model,
...
async def test_async_chat_azure():
    try:
        ...
        derouter.callbacks = [customHandler_completion_azure_router]
        ...
        derouter.callbacks = [customHandler_streaming_azure_router]
...
async def test_async_embedding_azure():
    try:
        ...
        response = await router.aembedding(
            model="azure-embedding-model", input=["hello from derouter!"]
        )
...
async def test_async_chat_azure_with_fallbacks():
    try:
        ...
        derouter.callbacks = [customHandler_fallbacks]
        derouter.set_verbose = True
        # with fallbacks
        model_list = [
            {
        ...
        router = Router(
            ...
            retry_policy=derouter.router.RetryPolicy(
...
async def test_async_completion_azure_caching():
    ...
    derouter.callbacks = [customHandler_caching]
...
async def test_async_completion_azure_caching_streaming():
    ...
    # Use Router instead of direct derouter.acompletion to get router-specific metadata
    model_list = [
        {
...
async def test_async_embedding_azure_caching():
    ...
    response1 = await router.aembedding(
        ...
        input=[f"good morning from derouter1 {unique_time}"],
        caching=True,
    )
...
async def test_rate_limit_error_callback():
    ...
    Relevant issue: https://github.com/Delify-Solutions/derouter/issues/4096
    """
    ...
    derouter_logging_obj = DeRouterLogging(
        model="my-test-gpt",
    ...
    with patch.object(
        ...
        except (derouter.RateLimitError, ValueError):
            pass
