...
from derouter.types.utils import StandardLoggingPayload, Usage, ModelInfoBase
...
async def test_stream_token_counting_gpt_4o():
    ...
    derouter.logging_callback_manager.add_derouter_callback(custom_logger)
...
async def test_stream_token_counting_without_include_usage():
    ...
    by default, derouter passes `include_usage=True` for OpenAI API
    """
    custom_logger = TestCustomLogger()
    derouter.logging_callback_manager.add_derouter_callback(custom_logger)
...
async def test_stream_token_counting_with_redaction():
    """
    When derouter.turn_off_message_logging=True is used, the usage tracked == usage from llm api chunk
    """
...
async def test_stream_token_counting_anthropic_with_include_usage():
    ...
    print("input_tokens_derouter", custom_logger.recorded_usage.prompt_tokens)
    print("output_tokens_derouter", custom_logger.recorded_usage.completion_tokens)
