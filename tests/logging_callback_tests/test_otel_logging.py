...
from derouter.integrations.arize.arize_phoenix import ArizePhoenixLogger
...
EXPECTED_SPAN_NAMES = ["derouter_request", "raw_gen_ai_request"]
...
async def test_async_otel_callback(streaming):
    ...
    derouter.callbacks = [OpenTelemetry(config=OpenTelemetryConfig(exporter=exporter))]
    ...
    for span in spans:
        ...
        if span.name == "derouter_request":
            validate_derouter_request(span)
...
def validate_derouter_request(span):
    expected_attributes = [
...
async def test_awesome_otel_with_message_logging_off(streaming, global_redact):
    ...
    tests when derouter.turn_off_message_logging is set to True
    ...
    derouter.callbacks = [otel_logger]
...
async def test_arize_phoenix_creates_nested_spans_on_dedicated_provider():
    ...
    ``derouter_proxy_request`` parent span and a ``derouter_request`` child span
    ...
    derouter.callbacks = [phoenix_logger]
    derouter.success_callback = []
    derouter.failure_callback = []
    ...
    # - "derouter_proxy_request" (parent) — created by _get_phoenix_context
    # - "derouter_request" (child)       — the LLM call span
    # - "raw_gen_ai_request"            — raw request sub-span
    assert (
        "derouter_proxy_request" in span_names
    ), f"Expected proxy parent span, got: {span_names}"
    assert (
        DEROUTER_REQUEST_SPAN_NAME in span_names
