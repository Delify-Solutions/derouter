...
def test_is_internal_derouter_proxy_callback():
    """
    Ensure we can determine if a callback is an internal derouter proxy callback

    eg. `_PROXY_MaxBudgetLimiter`, `_PROXY_CacheControlCheck`
    """
    logging = setup_logging()

    assert logging._is_internal_derouter_proxy_callback(_PROXY_MaxBudgetLimiter) == True

    # Test non-internal callbacks
    def regular_callback():
        pass

    assert logging._is_internal_derouter_proxy_callback(regular_callback) == False

    # Test string callback
    assert logging._is_internal_derouter_proxy_callback("callback_string") == False
...
def test_should_run_sync_callbacks_for_async_calls():
    ...
    derouter.success_callback = [regular_callback]
    assert logging._should_run_sync_callbacks_for_async_calls() == True

    # Test with internal callback only
    derouter.success_callback = [_PROXY_MaxBudgetLimiter]
    assert logging._should_run_sync_callbacks_for_async_calls() == False


def test_remove_internal_derouter_callbacks():
    logging = setup_logging()
    ...
    filtered = logging._remove_internal_derouter_callbacks(callbacks)
