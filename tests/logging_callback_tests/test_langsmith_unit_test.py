...
from derouter.integrations.langsmith import (
    LangsmithLogger,
...
async def test_is_serializable():
    from derouter.integrations.langsmith import is_serializable
...
async def test_langsmith_key_based_logging():
    """
    In key based logging langsmith_api_key and langsmith_project are passed directly to derouter.acompletion
    """
    try:
        ...
        mock_get_client = patch(
            "derouter.integrations.langsmith.get_async_httpx_client",
            return_value=mock_async_httpx_handler,
        )
        mock_get_client.start()

        derouter.set_verbose = True
        derouter.DEFAULT_FLUSH_INTERVAL_SECONDS = 1

        derouter.callbacks = [LangsmithLogger()]
...
async def test_langsmith_queue_logging():
    try:
        ...
        derouter.callbacks = [test_langsmith_logger]
