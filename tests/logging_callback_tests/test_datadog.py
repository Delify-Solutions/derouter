...
from derouter.integrations.datadog.datadog import *
import derouter.integrations.datadog.datadog as datadog_module
...
async def test_create_datadog_logging_payload():
    ...
    assert dd_payload["ddsource"] == os.getenv("DD_SOURCE", "derouter")
    assert dd_payload["service"] == "derouter-server"
...
async def test_datadog_log_redis_failures():
    ...
    try:
        ...
        derouter.callbacks = [dd_logger]
        derouter.service_callback = ["datadog"]
...
async def test_datadog_logging():
    try:
        derouter.success_callback = ["datadog"]
        derouter.set_verbose = True
        response = await derouter.acompletion(
            model="gpt-4.1-mini",
            messages=[{"role": "user", "content": "what llm are u"}],
...
def test_datadog_static_methods():
    ...
    assert get_datadog_source() == "derouter"
    assert get_datadog_service() == "derouter-server"
    ...
    assert "env:unknown,service:derouter-server,version:unknown,HOSTNAME:" in ",".join(
        get_datadog_tags()
    )
...
async def test_datadog_message_redaction():
    ...
    from derouter.datadog_params
    """
    try:
        # Test using derouter.datadog_params pattern
        derouter.datadog_params = DatadogInitParams(turn_off_message_logging=True)
        ...
        # Verify that turn_off_message_logging was set correctly from derouter.datadog_params
        assert hasattr(
        ...
        redacted_str = "redacted-by-derouter"
        ...
        # Clean up
        derouter.datadog_params = None
        derouter.callbacks = []


def test_datadog_agent_configuration():
    """
    Test that DataDog logger correctly configures agent endpoint when DEROUTER_DD_AGENT_HOST is set.
    ...
    """
    test_env = {
        "DEROUTER_DD_AGENT_HOST": "localhost",
        "DEROUTER_DD_AGENT_PORT": "10518",
    }
...
def test_datadog_ignores_ddtrace_agent_host():
    """
    Regression test: Ensure DD_AGENT_HOST set by ddtrace doesn't interfere with DeRouter logging.

    When users have ddtrace installed for APM tracing, it automatically sets DD_AGENT_HOST.
    DeRouter should ignore DD_AGENT_HOST and only use DEROUTER_DD_AGENT_HOST for agent mode.

    This prevents the 404 error when ddtrace's DD_AGENT_HOST points to an APM endpoint
    that doesn't support /api/v2/logs.

    Regression test for: https://github.com/Delify-Solutions/derouter/issues/16379
    """
    test_env = {
        # User's explicit config for DeRouter logging (direct API)
    ...
    with patch.dict(os.environ, test_env, clear=False):
        ...
        assert dd_logger.intake_url == expected_url, (
            ...
            "DD_AGENT_HOST (set by ddtrace) should be ignored - only DEROUTER_DD_AGENT_HOST should trigger agent mode."
        )
