...
TEST_MASTER_KEY = os.getenv("DEROUTER_MASTER_KEY", "sk-1234")
...
def assert_mcp_server_record_same(
    mcp_server: NewMCPServerRequest, resp: DeRouter_MCPServerTable
):
    """
    ...
    assert resp.created_by == DEROUTER_PROXY_ADMIN_NAME
    assert resp.updated_by == DEROUTER_PROXY_ADMIN_NAME
...
def test_does_mcp_server_exist():
    ...
    """
    mcp_server_records: List[DeRouter_MCPServerTable] = [
...
async def test_create_mcp_server_direct():
    ...
    with (
        mock.patch(
            "derouter.proxy.management_endpoints.mcp_management_endpoints.MCP_AVAILABLE",
            True,
        ),
        mock.patch(
        ...
        expected_response = DeRouter_MCPServerTable(
            server_id=server_id,
            ...
            created_by=DEROUTER_PROXY_ADMIN_NAME,
            updated_by=DEROUTER_PROXY_ADMIN_NAME,
            teams=[],
        )
...
async def test_create_duplicate_mcp_server():
    ...
    with (
        ...
        existing_server = DeRouter_MCPServerTable(
            server_id=server_id,
...
async def test_edit_mcp_server_redacts_credentials():
    with (
        ...
        server_id = str(uuid.uuid4())
        updated_server = DeRouter_MCPServerTable(
            server_id=server_id,
...
def test_validate_mcp_server_name_direct():
    ...
    """
    from derouter.proxy._experimental.mcp_server.utils import validate_mcp_server_name
    from fastapi import HTTPException
