...
async def test_chat_completion_bad_model_with_spend_logs():
    ...
    with httpx.Client() as client:
        ...
        # Extract the derouter call ID from headers
        derouter_call_id = response.headers.get("x-derouter-call-id")
        print(f"Status code: {response.status_code}")
        print(f"Headers: {dict(response.headers)}")
        print(f"DeRouter Call ID: {derouter_call_id}")
    ...
    assert (
        derouter_call_id is not None
    ), "Failed to get DeRouter Call ID from response headers"
    print("waiting for flushing error log to db....")
    await asyncio.sleep(15)

    # Now query the spend logs
    url = "http://0.0.0.0:4000/spend/logs?request_id=" + derouter_call_id
    ...
    with httpx.Client() as client:
        ...
        log_entry = spend_logs[0]  # Should be the specific log for our derouter_call_id

        # Verify the structure of the log entry
        assert log_entry["request_id"] == derouter_call_id
