...
async def obtain_cli_sso_token_via_poll_flow(
    ...
    *,
    ...
    """
    Obtain a CLI SSO JWT through the same HTTP flow as `derouter-proxy login`:
    ...
    poll_headers = {
        "x-derouter-cli-poll-secret": poll_secret,
    }
...
async def test_team_budget_enforcement_cli_sso_token():
    """
    Team budget enforcement for CLI SSO session tokens (derouter-proxy login JWT).
