...
async def test_proxy_failure_metrics():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        expected_patterns = [
            "derouter_proxy_failed_requests_metric_total{",  # New metric
            "derouter_llm_api_failed_requests_metric_total{",  # Deprecated but may still be used
        ]

        # Master-key auth substitutes DEROUTER_PROXY_MASTER_KEY_ALIAS for
        ...
        expected_hashed_api_key = DEROUTER_PROXY_MASTER_KEY_ALIAS
        ...
        for pattern in expected_patterns:
            for line in metrics.split("\n"):
                ...
                if "derouter_proxy_failed_requests_metric_total{" in line:
                    if (
                ...
                elif "derouter_llm_api_failed_requests_metric_total{" in line:
                    if (
        ...
        assert (
            found_metric
        ), f"Expected failure metric not found in /metrics. Looking for either derouter_proxy_failed_requests_metric_total or derouter_llm_api_failed_requests_metric_total with required fields"

        # Check total requests metric similarly
        # The derouter_proxy_total_requests_metric_total should be present
        total_requests_pattern = "derouter_proxy_total_requests_metric_total{"
...
async def test_proxy_success_metrics():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for line in metrics.split("\n"):
            if (
                "derouter_request_total_latency_metric_bucket{" in line
        ...
        ), "Expected derouter_request_total_latency_metric_bucket not found in /metrics"

        # Check for llm_api_latency_metric with required fields
        found_api_latency = False
        for line in metrics.split("\n"):
            if (
                "derouter_llm_api_latency_metric_bucket{" in line
                and 'api_key_alias="None"' in line
                and f'hashed_api_key="{expected_hashed_api_key}"' in line
                and 'requested_model="fake-openai-endpoint"' in line
                and 'le="0.005"' in line
        ...
        assert (
            found_api_latency
        ), "Expected derouter_llm_api_latency_metric_bucket not found in /metrics"
...
def verify_latency_metrics(metrics: str):
    ...
    - derouter_request_total_latency_metric_bucket
    - derouter_llm_api_latency_metric_bucket
    ...
    metric_names = [
        "derouter_request_total_latency_metric_bucket",
        "derouter_llm_api_latency_metric_bucket",
        "derouter_overhead_latency_metric_bucket",
    ]
...
async def test_proxy_fallback_metrics():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for line in metrics.split("\n"):
            if (
                "derouter_deployment_successful_fallbacks_total{" in line
                and 'api_key_alias="None"' in line
                and 'exception_class="Openai.RateLimitError"' in line
                and 'exception_status="429"' in line
        ...
        ), "Expected derouter_deployment_successful_fallbacks_total metric not found in /metrics"

        # Check if failed fallback metric is incremented - use flexible matching
        found_failed_fallback = False
        for line in metrics.split("\n"):
            if (
                "derouter_deployment_failed_fallbacks_total{" in line
                and 'api_key_alias="None"' in line
                and 'exception_class="Openai.RateLimitError"' in line
                and 'exception_status="429"' in line
        ...
        ), "Expected derouter_deployment_failed_fallbacks_total metric not found in /metrics"
...
def extract_budget_metrics(metrics_text: str, team_id: str) -> Dict[str, float]:
    ...
    remaining_pattern = f'derouter_remaining_team_budget_metric{{team="{team_id}",team_alias="[^"]*"}} ([0-9.]+)'
    ...
    # Get total budget
    total_pattern = f'derouter_team_max_budget_metric{{team="{team_id}",team_alias="[^"]*"}} ([0-9.]+)'
    total_match = re.search(total_pattern, metrics_text)
    metrics["total"] = float(total_match.group(1)) if total_match else None

    # Get remaining hours
    hours_pattern = f'derouter_team_budget_remaining_hours_metric{{team="{team_id}",team_alias="[^"]*"}} ([0-9.]+)'
...
def extract_key_budget_metrics(metrics_text: str, key_id: str) -> Dict[str, float]:
    ...
    # Get remaining budget
    remaining_pattern = f'derouter_remaining_api_key_budget_metric{{api_key_alias="[^"]*",hashed_api_key="{key_id}"}} ([0-9.]+)'
    remaining_match = re.search(remaining_pattern, metrics_text)
    metrics["remaining"] = float(remaining_match.group(1)) if remaining_match else None

    # Get total budget
    total_pattern = f'derouter_api_key_max_budget_metric{{api_key_alias="[^"]*",hashed_api_key="{key_id}"}} ([0-9.]+)'
    total_match = re.search(total_pattern, metrics_text)
    metrics["total"] = float(total_match.group(1)) if total_match else None

    # Get remaining hours
    hours_pattern = f'derouter_api_key_budget_remaining_hours_metric{{api_key_alias="[^"]*",hashed_api_key="{key_id}"}} ([0-9.]+)'
    hours_match = re.search(hours_pattern, metrics_text)
    metrics["remaining_hours"] = float(hours_match.group(1)) if hours_match else None
...
def extract_user_budget_metrics(metrics_text: str, user_id: str) -> Dict[str, float]:
    ...
    remaining_pattern = rf'derouter_remaining_user_budget_metric{{[^}}]*user="{escaped_user_id}"[^}}]*}} ([0-9.]+)'
    remaining_match = re.search(remaining_pattern, metrics_text)
    metrics["remaining"] = float(remaining_match.group(1)) if remaining_match else None

    # Get total budget
    total_pattern = rf'derouter_user_max_budget_metric{{[^}}]*user="{escaped_user_id}"[^}}]*}} ([0-9.]+)'
    total_match = re.search(total_pattern, metrics_text)
    metrics["total"] = float(total_match.group(1)) if total_match else None

    # Get remaining hours
    hours_pattern = rf'derouter_user_budget_remaining_hours_metric{{[^}}]*user="{escaped_user_id}"[^}}]*}} ([0-9.]+)'
    hours_match = re.search(hours_pattern, metrics_text)
    metrics["remaining_hours"] = float(hours_match.group(1)) if hours_match else None
...
async def test_user_email_metrics():
    ...
    3. Verify user email is being tracked correctly in `derouter_user_email_metric`
    """
    async with aiohttp.ClientSession() as session:
...
async def test_user_email_in_all_required_metrics():
    ...
    - derouter_proxy_total_requests_metric_total
    - derouter_proxy_failed_requests_metric_total
    - derouter_input_tokens_metric_total
    - derouter_output_tokens_metric_total
    - derouter_requests_metric_total
    - derouter_spend_metric_total
    """
    async with aiohttp.ClientSession() as session:
        # Create a user with user_email
        ...
        failure_pattern = rf'derouter_proxy_failed_requests_metric_total{{[^}}]*user_email="{re.escape(user_email)}"[^}}]*}}'
        failure_matches = re.findall(failure_pattern, metrics_text)
        assert (
            len(failure_matches) > 0
        ), f"derouter_proxy_failed_requests_metric_total should contain user_email={user_email}"
