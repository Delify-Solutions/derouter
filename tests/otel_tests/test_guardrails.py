...
async def test_llm_guard_triggered_safe_request():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        assert "x-derouter-applied-guardrails" in headers

        assert (
            headers["x-derouter-applied-guardrails"]
...
async def test_no_llm_guard_triggered():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        assert "x-derouter-applied-guardrails" not in headers
...
async def test_guardrails_with_api_key_controls():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        assert "x-derouter-applied-guardrails" in headers
        assert headers["x-derouter-applied-guardrails"] == "bedrock-pre-guard"
...
async def test_custom_guardrail_during_call_triggered():
    ...
    """
    async with aiohttp.ClientSession() as session:
        with pytest.raises(Exception, match="Guardrail failed words - `derouter` detected") as exc_info:
            response, headers = await chat_completion(
                session,
                "sk-1234",
                model="fake-openai-endpoint",
                messages=[{"role": "user", "content": f"Hello do you like derouter?"}],
                guardrails=["custom-during-guard"],
            )
        e = exc_info.value
        print(e)
        assert "Guardrail failed words - `derouter` detected" in str(e)
...
async def test_guardrail_load_balancing():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for i in range(num_requests):
            ...
            assert "x-derouter-applied-guardrails" in headers
            assert headers["x-derouter-applied-guardrails"] == "lb-test-guard"
