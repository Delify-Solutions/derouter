...
async def model_info_get_call(session, key, model_id: str):
    # make get call pass "derouter_model_id" in query params
    url = f"http://0.0.0.0:4000/model/info?derouter_model_id={model_id}"
    headers = {
        "Authorization": f"Bearer {key}",
        "Content-Type": "application/json",
    }
...
async def test_team_tag_routing():
    async with aiohttp.ClientSession() as session:
        ...
        for _ in range(5):
            ...
            print(headers)
            assert (
                headers["x-derouter-model-id"] == "team-a-model"
        ...
        for _ in range(5):
            ...
            print(headers)
            assert (
                headers["x-derouter-model-id"] == "team-b-model"
