"""Smoke-check that a base ``pip install derouter`` (no extras) is importable and usable.
...
def check_import() -> str:
    ...
    _require(bool(derouter.__file__), "derouter has no __file__")
    return f"imported derouter {version('derouter')}"
...
def check_bundled_model_metadata() -> str:
    ...
    max_input_tokens = derouter.get_model_info("gpt-4o")["max_input_tokens"]
    _require(
        isinstance(max_input_tokens, int) and max_input_tokens > 0,
        f"get_model_info returned max_input_tokens={max_input_tokens!r}",
    )
    prompt_cost, completion_cost = derouter.cost_per_token(model="gpt-4o", prompt_tokens=1000, completion_tokens=1000)
    _require(
...
def check_token_counter() -> str:
    ...
    count = derouter.token_counter(model="gpt-4o", text="hello world")
...
def main() -> int:
    ...
    for label, check in CHECKS:
        ...
        if not passed:
            print(f"FAIL  {label}:\n{detail}")
            print(f"A base `pip install derouter` is broken at: {label}")
