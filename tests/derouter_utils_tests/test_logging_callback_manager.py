...
from derouter.derouter_core_utils.logging_callback_manager import LoggingCallbackManager
...
def test_add_string_callback():
    """
    Test adding a string callback to derouter.callbacks - only 1 instance of the string callback should be added
    """
    manager = LoggingCallbackManager()
    test_callback = "test_callback"

    # Add string callback
    manager.add_derouter_callback(test_callback)
    assert test_callback in derouter.callbacks

    # Test duplicate prevention
    manager.add_derouter_callback(test_callback)
    assert derouter.callbacks.count(test_callback) == 1
...
def test_duplicate_langfuse_logger_test():
    ...
    for _ in range(10):
        ...
        manager.add_derouter_success_callback(langfuse_logger)
    print("derouter.success_callback: ", derouter.success_callback)
...
def test_duplicate_multiple_loggers_test():
    ...
    for _ in range(10):
        ...
        manager.add_derouter_success_callback(langfuse_logger)
        manager.add_derouter_success_callback(otel_logger)
    print("derouter.success_callback: ", derouter.success_callback)
    assert len(derouter.success_callback) == 2

    # Check exactly one instance of each logger type
    langfuse_count = sum(
        1
        for callback in derouter.success_callback
...
def test_add_function_callback():
    ...
    manager.add_derouter_callback(test_func)
    assert test_func in derouter.callbacks

    # Test duplicate prevention
    manager.add_derouter_callback(test_func)
    assert derouter.callbacks.count(test_func) == 1
...
def test_add_custom_logger(mock_custom_logger):
    ...
    # Add custom logger
    manager.add_derouter_callback(mock_custom_logger)
    assert mock_custom_logger in derouter.callbacks
...
def test_add_multiple_callback_types(mock_custom_logger):
    ...
    manager.add_derouter_callback(string_callback)
    manager.add_derouter_callback(test_func)
    manager.add_derouter_callback(mock_custom_logger)

    assert string_callback in derouter.callbacks
    assert test_func in derouter.callbacks
    assert mock_custom_logger in derouter.callbacks
    assert len(derouter.callbacks) == 3
...
def test_success_failure_callbacks():
    ...
    # Add callbacks
    manager.add_derouter_success_callback(success_callback)
    manager.add_derouter_failure_callback(failure_callback)

    assert success_callback in derouter.success_callback
    assert failure_callback in derouter.failure_callback
...
def test_async_callbacks():
    ...
    # Add async callbacks
    manager.add_derouter_async_success_callback(async_success)
    manager.add_derouter_async_failure_callback(async_failure)

    assert async_success in derouter._async_success_callback
    assert async_failure in derouter._async_failure_callback
...
def test_remove_callback_from_list_by_object():
    ...
    def TestObject():
        def __init__(self):
            manager.add_derouter_callback(self.callback)
            manager.add_derouter_success_callback(self.callback)
            manager.add_derouter_failure_callback(self.callback)
            manager.add_derouter_async_success_callback(self.callback)
            manager.add_derouter_async_failure_callback(self.callback)
    ...
    manager.remove_callback_from_list_by_object(derouter.callbacks, obj)
    manager.remove_callback_from_list_by_object(derouter.success_callback, obj)
    manager.remove_callback_from_list_by_object(derouter.failure_callback, obj)
    manager.remove_callback_from_list_by_object(derouter._async_success_callback, obj)
    manager.remove_callback_from_list_by_object(derouter._async_failure_callback, obj)
...
def test_remove_callback_from_all_lists():
    ...
    obj = TestLogger()
    manager.add_derouter_callback(obj)
    manager.add_derouter_success_callback(obj)
    manager.add_derouter_failure_callback(obj)
    manager.add_derouter_async_success_callback(obj)
    manager.add_derouter_async_failure_callback(obj)

    manager.remove_callback_from_all_lists(obj)

    assert obj not in derouter.callbacks
    assert obj not in derouter.success_callback
    assert obj not in derouter.failure_callback
    assert obj not in derouter._async_success_callback
    assert obj not in derouter._async_failure_callback


def test_reset_callbacks(callback_manager):
    # Add various callbacks
    callback_manager.add_derouter_callback("test")
    callback_manager.add_derouter_success_callback("success")
    callback_manager.add_derouter_failure_callback("failure")
    callback_manager.add_derouter_async_success_callback("async_success")
    callback_manager.add_derouter_async_failure_callback("async_failure")
...
async def test_slack_alerting_callback_registration(callback_manager):
    """
    Test that derouter callbacks are correctly registered for slack alerting
    ...
    with patch(
        "derouter.integrations.SlackAlerting.slack_alerting.get_async_httpx_client"
    ) as mock_http:
        ...
        assert isinstance(derouter.callbacks[0], SlackAlerting)
        ...
        callback_manager._reset_all_callbacks()  # Reset callbacks
        proxy_logging.update_values(alerting=["slack"], alert_types=["outage_alerts"])
        assert len(derouter.callbacks) == 1  # Regular callback for outage alerts
        assert isinstance(derouter.callbacks[0], SlackAlerting)
        ...
        assert len(derouter._async_success_callback) == 1
        assert derouter._async_success_callback[0] == response_taking_too_long_callback
...
async def test_generic_api_callback_settings_retry_config():
    ...
    derouter.callback_settings[callback_name] = {
    ...
    try:
        ...
        derouter.callback_settings.pop(callback_name, None)
        _generic_api_logger_cache.pop(callback_name, None)
