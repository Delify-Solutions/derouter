...
from derouter.secret_managers.hashicorp_secret_manager import HashicorpSecretManager
...
def test_hashicorp_secret_manager_get_secret(hashicorp_secret_manager):
    with patch("derouter.llms.custom_httpx.http_handler.HTTPHandler.get") as mock_get:
...
async def test_hashicorp_secret_manager_delete_secret(hashicorp_secret_manager):
    with patch(
        "derouter.llms.custom_httpx.http_handler.AsyncHTTPHandler.delete"
    ) as mock_delete:
        # Configure the mock response
        mock_response = MagicMock()
        mock_response.raise_for_status.return_value = None
