...
def check_aws_credentials():
    ...
    if os.getenv("DEROUTER_RUN_LIVE_AWS_SECRET_MANAGER_TESTS") != "1":
...
async def test_write_and_read_simple_secret():
    ...
    test_secret_name = f"derouter_test_{uuid.uuid4().hex[:8]}"
    ...
    try:
        ...
        write_response = await secret_manager.async_write_secret(
            ...
            description="DeRouter Test Secret",
        )
...
async def test_write_and_read_json_secret():
    ...
    secret_manager = AWSSecretsManagerV2()
    test_secret_name = f"derouter_test_{uuid.uuid4().hex[:8]}_json"
    test_secret_value = {
        "api_key": "test_key",
        "model": "gpt-4",
        "temperature": 0.7,
        "metadata": {"team": "ml", "project": "derouter"},
    }
    ...
    try:
        ...
        write_response = await secret_manager.async_write_secret(
            ...
            description="DeRouter JSON Test Secret",
        )
...
async def test_read_nonexistent_secret():
    ...
    secret_manager = AWSSecretsManagerV2()
    nonexistent_secret = f"derouter_nonexistent_{uuid.uuid4().hex}"
...
async def test_primary_secret_functionality():
    ...
    secret_manager = AWSSecretsManagerV2()
    primary_secret_name = f"derouter_test_primary_{uuid.uuid4().hex[:8]}"
    ...
    try:
        ...
        write_response = await secret_manager.async_write_secret(
            ...
            description="DeRouter Test Primary Secret",
        )
...
async def test_write_secret_with_description_and_tags():
    ...
    secret_manager = AWSSecretsManagerV2()
    test_secret_name = f"derouter_test_{uuid.uuid4().hex[:8]}_tags"
    test_secret_value = "test_value_with_tags"

    test_description = "DeRouter Secret with Description and Tags"
    test_tags = {
        "Environment": "Test",
...
def test_load_aws_secret_manager_with_settings():
    ...
    try:
        ...
        assert derouter.secret_manager_client is not None
        assert isinstance(derouter.secret_manager_client, AWSSecretsManagerV2)

        # Verify settings were passed through
        assert derouter.secret_manager_client.aws_role_name == settings.aws_role_name
        assert derouter.secret_manager_client.aws_region_name == settings.aws_region_name
        assert (
            derouter.secret_manager_client.aws_session_name == settings.aws_session_name
        )
    finally:
        # Cleanup
...
async def test_end_to_end_iam_role_secret_write():
    ...
    test_secret_name = f"derouter_test_iam_{uuid.uuid4().hex[:8]}"
