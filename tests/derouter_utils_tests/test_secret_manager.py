...
import pytest
import derouter
from derouter.llms.azure.azure import get_azure_ad_token_from_oidc
...
def test_aws_secret_manager():
    ...
    secret_val = get_secret("derouter_master_key")
    ...
    assert secret_val["derouter_master_key"] == "sk-1234"
...
@pytest.mark.skip(
    reason="Quarantined: Flaky test - fails with InvalidIdentityToken, OIDC provider no longer configured in AWS account. TODO: Switch to DeRouter's own IAM role"
)
def test_oidc_circle_v1_with_amazon():
    ...
    aws_role_name = "arn:aws:iam::335785316107:role/derouter-github-unit-tests-circleci-v1-assume-only"
...
def test_oidc_file(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        monkeypatch.setenv("DEROUTER_OIDC_ALLOWED_CREDENTIAL_DIRS", temp_dir)
...
def test_google_secret_manager():
    ...
    """
    os.environ["GOOGLE_SECRET_MANAGER_PROJECT_ID"] = "derouter-ci-cd"

    from derouter.secret_managers.google_secret_manager import GoogleSecretManager
    ...
    with (
        ...
        assert "projects/derouter-ci-cd/secrets/OPENAI_API_KEY" in call_url
...
def test_should_read_secret_from_secret_manager():
    ...
    derouter.secret_manager_client = None
    ...
    derouter.secret_manager_client = "dummy_client"
    derouter._key_management_settings = KeyManagementSettings(access_mode="read_only")
    ...
    derouter._key_management_settings = KeyManagementSettings(access_mode="write_only")
    assert _should_read_secret_from_secret_manager() is False
...
def test_key_management_settings_defaults():
    ...
    assert settings.prefix_for_stored_virtual_keys == "derouter/"
...
def test_key_management_settings_custom_values():
    ...
    custom_description = "DeRouter-managed API key for development"

    settings = KeyManagementSettings(
        store_virtual_keys=True,
        prefix_for_stored_virtual_keys="derouter/custom/",
        access_mode="read_and_write",
        primary_secret_name="primary/derouter/keys",
        description=custom_description,
        tags=custom_tags,
    )

    assert settings.store_virtual_keys is True
    assert settings.prefix_for_stored_virtual_keys == "derouter/custom/"
    assert settings.access_mode == "read_and_write"
    assert settings.primary_secret_name == "primary/derouter/keys"
...
async def test_async_write_secret_receives_description_and_tags(monkeypatch):
    ...
    """
    from derouter import derouter
    from derouter.secret_managers.aws_secret_manager_v2 import AWSSecretsManagerV2
    from derouter.types.secret_managers.main import KeyManagementSettings

    # Mock out AWS network calls
    mock_async_write = AsyncMock(return_value={"Name": "derouter/test_secret"})
    monkeypatch.setattr(AWSSecretsManagerV2, "async_write_secret", mock_async_write)

    # Setup settings
    derouter._key_management_settings = KeyManagementSettings(
        store_virtual_keys=True,
        description="DeRouter Unit Test Secret",
        tags={"Owner": "UnitTest", "Purpose": "Validation"},
    )

    # Instantiate fake client
    derouter.secret_manager_client = AWSSecretsManagerV2()
    ...
    assert kwargs["description"] == "DeRouter Unit Test Secret"
...
def test_key_management_settings_serialization_roundtrip():
    ...
    original = KeyManagementSettings(
        store_virtual_keys=True,
        prefix_for_stored_virtual_keys="derouter/dev/",
        access_mode="read_and_write",
    ...
    assert reloaded.prefix_for_stored_virtual_keys == "derouter/dev/"
