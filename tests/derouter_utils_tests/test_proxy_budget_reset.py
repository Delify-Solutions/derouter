...
from derouter.proxy._types import DeRouter_BudgetTableFull
...
# In a real-world scenario, these would be instances of DeRouter_VerificationToken, DeRouter_UserTable, etc.
...
def _wire_batcher_for_test(prisma_client, fail_commit=False):
    ...
    def make_batcher():
        ...
        batcher = MagicMock()
        batcher.derouter_verificationtoken = _Table("key")
        batcher.derouter_usertable = _Table("user")
        batcher.derouter_teamtable = _Table("team")
        batcher.derouter_budgettable = _Table("budget")
        batcher.derouter_teammembership = _Table("team_membership")
        batcher.derouter_organizationtable = _Table("org")
        batcher.derouter_tagtable = _Table("tag")
        batcher.derouter_endusertable = _Table("enduser")
...
async def test_reset_budget_keys_partial_failure():
    ...
    with patch.object(
        ...
        await job.reset_budget_for_derouter_keys()
...
async def test_reset_budget_users_partial_failure():
    ...
    with patch.object(
        ...
        await job.reset_budget_for_derouter_users()
        await asyncio.sleep(0.1)
...
async def test_reset_budget_endusers_cascade_failure_is_all_or_nothing():
    ...
    budget1 = DeRouter_BudgetTableFull(
        **{
            "budget_id": "budget1",
            "max_budget": 65.0,
            "budget_duration": "2d",
    ...
    await job.reset_budget_for_derouter_budget_table()
    await asyncio.sleep(0.1)
...
async def test_reset_budget_teams_partial_failure():
    ...
    with patch.object(
        ...
        await job.reset_budget_for_derouter_teams()
        await asyncio.sleep(0.1)
...
async def test_service_logger_keys_success():
    ...
    with patch.object(
        ...
        with patch(
            "derouter.proxy.common_utils.reset_budget_job.verbose_proxy_logger.exception"
        ) as mock_verbose_exc:
            await job.reset_budget_for_derouter_keys()
...
@pytest.mark.asyncio
async def test_reset_budget_for_derouter_team_members_called():
    """
    Test that when reset_budget_for_derouter_budget_table is called, team
