...
def _assert_overhead_is_smaller_than_total(response, total_time_ms):
    derouter_overhead_ms = response._hidden_params["derouter_overhead_time_ms"]
    overhead_percent = derouter_overhead_ms * 100 / total_time_ms

    assert derouter_overhead_ms > 0
    assert derouter_overhead_ms < 1000
    assert derouter_overhead_ms < total_time_ms
    assert overhead_percent < 40


@pytest.fixture(autouse=True)
def reset_derouter_state():
    derouter.cache = None
...
@pytest.mark.asyncio
async def test_derouter_overhead_non_streaming(monkeypatch):
...
@pytest.mark.asyncio
async def test_derouter_overhead_stream(monkeypatch):
    calls = _mock_openai_completion_transport(
...
@pytest.mark.asyncio
async def test_derouter_overhead_cache_hit(monkeypatch):
    ...
    assert response2._hidden_params["derouter_overhead_time_ms"] > 0
    assert (
        response2._hidden_params["derouter_overhead_time_ms"]
