...
async def test_azure_health_check():
    response = await derouter.ahealth_check(
        model_params={
...
def test_update_derouter_params_for_health_check():
    """
    Test if _update_derouter_params_for_health_check correctly:
    ...
    """
    from derouter.proxy.health_check import _update_derouter_params_for_health_check
    ...
    for prefix in cris_prefixes:
        ...
        updated_params = _update_derouter_params_for_health_check(
            model_info, derouter_params
        )
        assert (
...
async def test_perform_health_check_filters_by_model_id():
    ...
    """
    from derouter.proxy.health_check import perform_health_check
    ...
    model_list = [
        {
            ...
            "derouter_params": {"model": "gpt-5.5", "api_key": "fake-key-1"},
        },
        {
            "model_name": "gpt-5.5",
            "model_info": {"id": "deployment-id-2"},
            "derouter_params": {"model": "gpt-5.5", "api_key": "fake-key-2"},
        },
    ]
    ...
    async def mock_perform_health_check(m_list, details=True, **kwargs):
        ...
        return (
            [{"model": "gpt-5.5", "api_key": m_list[0]["derouter_params"]["api_key"]}],
            [],
            {},
        )

    with patch(
        "derouter.proxy.health_check._perform_health_check",
    ...
    model_list = [
        {
            "model_name": "a",
            "model_info": {"id": "id-a"},
            "derouter_params": {"model": "m-a", "api_key": "k1"},
        },
        {
            "model_name": "b",
            "model_info": {
            ...
            "derouter_params": {"model": "m-b", "api_key": "k2"},
        },
    ]
    captured = []
...
async def test_perform_health_check_with_health_check_model():
    ...
    """
    from derouter.proxy.health_check import _perform_health_check

    # Mock model list with health_check_model specified
    model_list = [
        {
            "derouter_params": {"model": "openai/*", "api_key": "fake-key"},
            "model_info": {
                "mode": "chat",
    ...
    async def mock_health_check(derouter_params, **kwargs):
        health_check_calls.append(derouter_params["model"])
        return {"status": "healthy"}

    with patch("derouter.ahealth_check", side_effect=mock_health_check):
...
async def test_health_check_bad_model():
    ...
    with patch(
        "derouter.ahealth_check", side_effect=mock_health_check
...
async def test_health_check_respects_concurrency_limit():
    ...
    model_list = [
        {"derouter_params": {"model": f"openai/gpt-4o-mini-{i}", "api_key": "fake-key"}}
        for i in range(6)
    ]
...
async def test_health_check_creates_only_bounded_initial_tasks():
    ...
    with (
        patch("derouter.ahealth_check", side_effect=mock_health_check),
        patch(
            "derouter.proxy.health_check.asyncio.create_task",
...
async def test_timeout_does_not_cancel_other_health_checks():
    ...
    model_list = [
        {
            "derouter_params": {"model": "openai/slow-model", "api_key": "fake-key"},
            "model_info": {"health_check_timeout": 0.05},
        },
        {
            "derouter_params": {"model": "openai/fast-model", "api_key": "fake-key"},
            "model_info": {"health_check_timeout": 1},
        },
    ]

    async def mock_health_check(derouter_params, **kwargs):
        if derouter_params["model"] == "openai/slow-model":
...
async def test_image_generation_health_check_prompt(monkeypatch):
    ...
    import importlib
    import derouter.constants as derouter_constants
    import derouter.proxy.health_check as health_check

    def reload_modules():
        reloaded_constants = importlib.reload(derouter_constants)
    ...
    async def run_health_check(health_check_module):
        ...
        async def mock_health_check(derouter_params, mode=None, prompt=None, input=None):
            health_check_calls.append(
                {
                    "mode": mode,
                    "prompt": prompt,
                    "model": derouter_params.get("model"),
                }
            )
            return {"status": "healthy"}
        ...
        model_list = [
            {
                "derouter_params": {"model": "gpt-image-1", "api_key": "fake-key"},
                "model_info": {
                    "mode": "image_generation",
                },
            }
        ...
        with patch(
            "derouter.proxy.health_check.derouter.ahealth_check",
...
async def test_health_check_with_custom_llm_provider():
    ...
    with patch("derouter.acompletion", return_value=mock_response):
