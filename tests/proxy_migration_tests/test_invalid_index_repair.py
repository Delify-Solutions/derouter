...
import pytest
from derouter_proxy_extras.utils import INDEX_REPAIR_ADVISORY_LOCK_KEY, ProxyExtrasDBManager
...
HEALTH_TABLE: Final = "DeRouter_HealthCheckTable"
HEALTH_INDEX: Final = "DeRouter_HealthCheckTable_model_id_model_name_checked_at_idx"
HEALTH_INDEX_COLUMNS: Final = '"model_id", "model_name", "checked_at" DESC'
LOOKALIKE_TABLE: Final = "DeRouterLookalikeTable"
LOOKALIKE_INDEX: Final = "DeRouterLookalikeTable_id_idx"
PARTITIONED_TABLE: Final = "DeRouter_PartitionedTable"
PARTITIONED_INDEX: Final = "DeRouter_PartitionedTable_id_idx"
...
def fresh_database(monkeypatch: pytest.MonkeyPatch) -> Iterator[str]:
    ...
    so a DeRouter schema already pushed into public makes them skip and then
...
@requires_db
def test_repair_rebuilds_invalid_derouter_indexes_and_leaves_lookalike_tables_alone(scratch_schema: str) -> None:
...
def test_repair_defaults_to_the_public_schema(monkeypatch: pytest.MonkeyPatch) -> None:
    table: Final = f"DeRouter_ScratchTable_{uuid.uuid4().hex[:8]}"
