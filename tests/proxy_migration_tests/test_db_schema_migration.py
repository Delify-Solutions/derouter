...
def test_schema_migration_in_sync():
    ...
    temp_base = Path(tempfile.mkdtemp(prefix="derouter_schema_migration_"))
    ...
    try:
        ...
        if not any(migrations_dir.iterdir()):
            pytest.fail(
                "No existing migrations found. Run `python derouter/ci_cd/baseline_db_migration.py`."
            )
        ...
        if diff.returncode == 2:
            pytest.fail(
                ...
                "`python derouter/ci_cd/run_migration.py <migration_name>`.\n\n"
                + diff.stdout
            )
