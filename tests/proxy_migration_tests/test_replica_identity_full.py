...
def scratch_schema(monkeypatch):
    """A schema holding two DeRouter tables and one foreign table, all at the default."""
    schema = f"replica_identity_{uuid.uuid4().hex[:8]}"
    with psycopg.connect(_base_url(), autocommit=True) as conn:
        conn.execute(f'CREATE SCHEMA "{schema}"')
        conn.execute(
            f'CREATE TABLE "{schema}"."DeRouter_ScratchTable" (id TEXT PRIMARY KEY, note TEXT)'
        )
        conn.execute(f'CREATE TABLE "{schema}"."DeRouter_ScratchSibling" (id TEXT PRIMARY KEY)')
...
@requires_db
def test_applies_full_to_derouter_tables_only(scratch_schema, monkeypatch):
    ...
    assert identities["DeRouter_ScratchTable"] == "f"
    assert identities["DeRouter_ScratchSibling"] == "f"
...
def test_a_locked_table_does_not_block_the_others(scratch_schema, monkeypatch):
    ...
    with psycopg.connect(_base_url()) as holder:
        holder.execute(f'SELECT * FROM "{scratch_schema}"."DeRouter_ScratchTable"')
        assert ProxyExtrasDBManager.apply_replica_identity_full_if_requested() is True

    identities = _replica_identities(scratch_schema)
    assert identities["DeRouter_ScratchTable"] == "d"
    assert identities["DeRouter_ScratchSibling"] == "f"
...
def test_leaves_tables_alone_when_not_requested(scratch_schema, monkeypatch):
    ...
    assert _replica_identities(scratch_schema)["DeRouter_ScratchTable"] == "d"
...
def test_is_idempotent_across_runs(scratch_schema, monkeypatch):
    ...
    assert _replica_identities(scratch_schema)["DeRouter_ScratchTable"] == "f"
