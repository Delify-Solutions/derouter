...
Gated on DEROUTER_IMAGE (the tag of the image to exercise) so it is skipped in
...
IMAGE = os.getenv("DEROUTER_IMAGE")
POSTGRES_IMAGE = os.getenv("DEROUTER_TEST_POSTGRES_IMAGE", "postgres:16-alpine")
CURL_IMAGE = os.getenv("DEROUTER_TEST_CURL_IMAGE", "curlimages/curl:8.11.1")
COMPONENT_PORT = os.getenv("DEROUTER_COMPONENT_PORT", "4000")
NON_ROOT_UID = "12345:0"
STARTUP_TIMEOUT_SECONDS = int(os.getenv("DEROUTER_COMPONENT_STARTUP_TIMEOUT", "180"))

pytestmark = [
    pytest.mark.skipif(IMAGE is None, reason="requires a built image (set DEROUTER_IMAGE)"),
...
def offline_stack():
    ...
    these images deliberately omit, and with DEROUTER_LOCAL_MODEL_COST_MAP, or the
    ...
    try:
        _docker(
            ...
            "-e", "POSTGRES_PASSWORD=pw", "-e", "POSTGRES_DB=derouter",
            POSTGRES_IMAGE,
        )
        ...
        _docker(
            ...
            "--user", NON_ROOT_UID,
            "-e", f"DATABASE_URL=postgresql://postgres:pw@{pg}:5432/derouter",
            "-e", "DEROUTER_MASTER_KEY=sk-component-serve-test",
            "-e", "DISABLE_SCHEMA_UPDATE=true",
            "-e", "DEROUTER_LOCAL_MODEL_COST_MAP=True",
            IMAGE,
        )
...
def _wait_until_postgres_ready(pg: str, attempts: int = 60) -> None:
    for _ in range(attempts):
        ...
        ready = _docker(
            "exec", pg, "pg_isready", "-U", "postgres", "-d", "derouter", check=False
        )
