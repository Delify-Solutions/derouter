...
MIN_TABLES = int(os.getenv("DEROUTER_TEST_MIN_TABLES", "20"))
NON_ROOT_UID = "12345:0"  # arbitrary uid in GID 0, as OpenShift restricted-v2 assigns

MIGRATION_INTERPRETER = os.getenv("DEROUTER_MIGRATION_INTERPRETER", "python")
MIGRATION_SCRIPT = os.getenv(
    "DEROUTER_MIGRATION_SCRIPT", "derouter/proxy/prisma_migration.py"
)
...
def _table_count(pg: str) -> int:
    result = _docker(
        "exec", pg, "psql", "-U", "postgres", "-d", "derouter", "-tAc",
...
def test_migration_offline_as_non_root_uid(offline_postgres):
    ...
    migrate = _docker(
        ...
        "-e", "DEROUTER_MASTER_KEY=sk-offline-migration-test",
