"""
Test for A2A to DeRouter Completion Bridge.

Tests the SDK-level functions that route A2A requests through derouter.acompletion.
...
async def test_a2a_completion_bridge_non_streaming():
    ...
    # Validate response is DeRouterSendMessageResponse
...
async def test_vertex_agent_engine_non_streaming():
    """
    Test non-streaming request to Vertex AI Agent Engine via derouter.acompletion.
    ...
    # Call via derouter.acompletion with vertex_ai/agent_engine/ prefix
...
async def test_vertex_agent_engine_streaming():
    """
    Test streaming request to Vertex AI Agent Engine via derouter.acompletion.

    Uses the Reasoning Engine resource ID to call a hosted agent with streaming.
    """
    # derouter._turn_on_debug()

    # Call via derouter.acompletion with streaming
    response = await derouter.acompletion(
        model=f"vertex_ai/agent_engine/{VERTEX_AGENT_RESOURCE_NAME}",
        messages=[{"role": "user", "content": "Hello! What can you do?"}],
        stream=True,
