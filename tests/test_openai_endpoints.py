...
async def test_chat_completion_different_deployments():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            ...
            init_model_id = results[0]["x-derouter-model-id"]
            deployments_shuffled = False
            for result in results[1:]:
                if init_model_id != result["x-derouter-model-id"]:
...
async def test_proxy_all_models():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        await chat_completion(
            session=session, key=DEROUTER_MASTER_KEY, model="groq/openai/gpt-oss-120b"
        )
