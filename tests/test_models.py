...
async def get_model_info(session, key, derouter_model_id=None):
    """
    Make sure only models user has access to are returned
    """
    if derouter_model_id:
        url = f"http://0.0.0.0:4000/model/info?derouter_model_id={derouter_model_id}"
    else:
...
async def test_get_specific_model():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for idx, m in enumerate(models):
            assert m == "gpt-4"
            derouter_model_id = response["data"][idx]["model_info"]["id"]
            model_specific_info = response["data"][idx]
        assert derouter_model_id is not None
        response = await get_model_info(
            session=session, key=key, derouter_model_id=derouter_model_id
        )
        assert response["data"][0]["model_info"]["id"] == derouter_model_id
        assert (
...
@pytest.mark.skip(
    reason="Requires live proxy + OPENAI_API_KEY. Deterministic mock version in tests/test_derouter/proxy/management_endpoints/test_model_management_endpoints.py::TestAddAndDeleteModelLifecycle"
)
@pytest.mark.asyncio
