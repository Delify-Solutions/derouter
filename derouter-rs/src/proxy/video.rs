//! Proxy video — /v1/videos/generations. Phase 1.
...
pub async fn handle_video_generation(pool: DbPool, body: axum::body::Bytes, headers: HeaderMap) -> Response {
    ...
    for conn in &connections_list {
        ...
        match executor.complete(conn, body, headers.clone()).await {
            ...
            Ok(base::UpstreamResponse::Error { status, message }) => {
                tracing::warn!("Video gen error via {}: {} - {}", conn.id, status, message);
                continue;
            }
            _ => continue,
        }
    ...
    error_resp(StatusCode::SERVICE_UNAVAILABLE, "All providers failed for video generation")
}
