//! Proxy images — /v1/images/generations. Phase 1.
//! Port of src/sse/handlers/imageGeneration.js.
...
pub async fn handle_image_generation(pool: DbPool, body: axum::body::Bytes, headers: HeaderMap) -> Response {
    ...
    for conn in &connections_list {
        ...
        // Images are always non-streaming
        match executor.complete(conn, body, headers.clone()).await {
            ...
            Ok(base::UpstreamResponse::Error { status, message }) => {
                tracing::warn!("Image gen error via {}: {} - {}", conn.id, status, message);
                continue;
            }
            _ => continue,
        }
    ...
    error_resp(StatusCode::SERVICE_UNAVAILABLE, "All providers failed for image generation")
}
