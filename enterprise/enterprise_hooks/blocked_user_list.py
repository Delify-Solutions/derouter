...
from derouter.proxy._types import UserAPIKeyAuth, DeRouter_EndUserTable
...
class _ENTERPRISE_BlockedUserList(CustomLogger):
    ...
    def __init__(self, prisma_client: Optional[PrismaClient]):
        ...
        blocked_user_list = derouter.blocked_user_list
    ...
    async def async_pre_call_hook(
        ...
        try:
            ...
            if "user_id" in data or "user" in data:
                ...
                cache_key = f"derouter:end_user_id:{user}"
                end_user_cache_obj: Optional[DeRouter_EndUserTable] = cache.get_cache(  # type: ignore
                    key=cache_key
                )
                if end_user_cache_obj is None and self.prisma_client is not None:
                    # check db
                    end_user_obj = (
                        await self.prisma_client.db.derouter_endusertable.find_unique(
                            where={"user_id": user}
                        )
                    )
                    if end_user_obj is None:  # user not in db - assume not blocked
                        end_user_obj = DeRouter_EndUserTable(user_id=user, blocked=False)
            ...
            verbose_proxy_logger.exception(
                "derouter.enterprise.enterprise_hooks.blocked_user_list::async_pre_call_hook - Exception occurred - {}".format(
                    str(e)
                )
            )
