...
class _ENTERPRISE_GoogleTextModeration(CustomLogger):
    ...
    def __init__(self):
        ...
        default_confidence_threshold = (
            derouter.google_moderation_confidence_threshold or 0.8
        )  # by default require a high confidence (80%) to fail

        for category in self.confidence_categories:
            if hasattr(derouter, f"{category}_confidence_threshold"):
                setattr(
                    self,
                    f"{category}_confidence_threshold",
                    getattr(derouter, f"{category}_confidence_threshold"),
                )
            else:
                setattr(
                    self,
