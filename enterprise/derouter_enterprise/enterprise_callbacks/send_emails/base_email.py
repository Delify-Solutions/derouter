...
from derouter.integrations.email_templates.key_created_email import (
    KEY_CREATED_EMAIL_TEMPLATE,
)
from derouter.integrations.email_templates.key_rotated_email import (
    KEY_ROTATED_EMAIL_TEMPLATE,
)
from derouter.integrations.email_templates.templates import (
    MAX_BUDGET_ALERT_EMAIL_TEMPLATE,
    SOFT_BUDGET_ALERT_EMAIL_TEMPLATE,
    TEAM_SOFT_BUDGET_ALERT_EMAIL_TEMPLATE,
)
from derouter.integrations.email_templates.user_invitation_email import (
...
from derouter.types.integrations.slack_alerting import DEROUTER_LOGO_URL
...
class BaseEmailLogger(CustomLogger):
    DEFAULT_DEROUTER_EMAIL = "notifications@alerts.router.delify.vn"
    DEFAULT_SUPPORT_EMAIL = "support@berri.ai"
    DEFAULT_SUBJECT_TEMPLATES = {
        EmailEvent.new_user_invitation: "DeRouter: {event_message}",
        EmailEvent.virtual_key_created: "DeRouter: {event_message}",
        EmailEvent.virtual_key_rotated: "DeRouter: {event_message}",
    }
    ...
    async def send_user_invitation_email(self, event: WebhookEvent):
        ...
        await self.send_email(
            from_email=self.DEFAULT_DEROUTER_EMAIL,
    ...
    async def _get_email_params(
        ...
        logo_url = get_custom_or_default(custom_logo, DEROUTER_LOGO_URL, "logo URL")
        ...
        if email_event == EmailEvent.new_user_invitation:
            ...
            subject_template = "DeRouter: {event_message}"

        subject = (
            subject_template.format(event_message=event_message)
            if event_message
            else "DeRouter Notification"
        )
        ...
        if unused_custom_fields:
            ...
            warning_msg = (
                ...
                "This is an Enterprise feature. To use custom email fields, please upgrade to DeRouter Enterprise. "
                "Schedule a meeting here: https://enterprise.router.delify.vn/demo"
            )
    ...
    async def _lookup_user_email_from_db(self, user_id: Optional[str]) -> Optional[str]:
        ...
        user_row = await prisma_client.db.derouter_usertable.find_unique(
    ...
    async def _get_or_create_invitation(self, user_id: str):
        ...
        try:
            ...
            existing_invitations = (
                await prisma_client.db.derouter_invitationlink.find_many(
                    where={"user_id": user_id},
