...
from derouter.proxy.management_endpoints.common_utils import _set_object_metadata_field
from derouter.proxy.management_helpers.utils import (
    management_endpoint_wrapper,
)
from derouter.proxy.utils import PrismaClient, handle_exception_on_proxy
...
from derouter.repositories.verification_token_repository import VerificationTokenRepository
...
def _team_table(prisma_client: PrismaClient) -> TableActions["prisma_models.DeRouter_TeamTable"]:
    return TeamRepository(prisma_client).table


def _project_table(prisma_client: PrismaClient) -> TableActions["prisma_models.DeRouter_ProjectTable"]:
...
def _budget_table(prisma_client: PrismaClient) -> TableActions["prisma_models.DeRouter_BudgetTable"]:
    return BudgetRepository(prisma_client).table


def _object_permission_table(
    prisma_client: PrismaClient,
) -> TableActions["prisma_models.DeRouter_ObjectPermissionTable"]:
    return ObjectPermissionRepository(prisma_client).table


def _user_table(prisma_client: PrismaClient) -> TableActions["prisma_models.DeRouter_UserTable"]:
...
async def _validate_team_exists(
    team_id: str,
    prisma_client: PrismaClient,
) -> "prisma_models.DeRouter_TeamTable":
...
def _check_team_project_limits(
    ...
    # unlike Project which uses a separate DeRouter_BudgetTable relation
...
async def _create_budget_for_project(
    ...
    budget_params = DeRouter_BudgetTable.model_fields.keys()
    _json_data: dict[str, object] = data.model_dump(exclude_none=True)
    _budget_data = {k: v for k, v in _json_data.items() if k in budget_params}
    budget_row = DeRouter_BudgetTable.model_validate(_budget_data)
    ...
    _budget: Final = await _budget_table(prisma_client).create(
        data={
            **new_budget,
            "created_by": user_id or derouter_proxy_admin_name,
            "updated_by": user_id or derouter_proxy_admin_name,
        }
    )
...
async def _set_project_object_permission(
    ...
    """
    Creates the DeRouter_ObjectPermissionTable record for the project.
...
def _remove_budget_fields_from_project_data(project_data: dict[str, object]) -> dict[str, object]:
    ...
    Budget fields belong to DeRouter_BudgetTable, not DeRouter_ProjectTable.
    Keep budget_id as it's a foreign key.

    Following the pattern from organization_endpoints.py
    """
    budget_fields = DeRouter_BudgetTable.model_fields.keys()
...
async def new_project(
    ...
    - object_permission: Optional[DeRouter_ObjectPermissionBase] - project-specific object permission. Example - {"vector_stores": ["vector_store_1", "vector_store_2"]}. IF null or {} then no object permission.
    ...
    try:
        ...
        _check_team_project_limits(
            team_object=DeRouter_TeamTable.model_validate(team_object.model_dump()),
            data=data,
        )
        ...
        project_row = DeRouter_ProjectTable.model_validate(
            {
                **project_row_payload,
        ...
        verbose_proxy_logger.exception(
            "derouter.proxy.management_endpoints.project_endpoints.new_project(): Exception occured - {}".format(str(e))
        )
...
@router.post(
    ...
    tags=["project management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=DeRouter_ProjectTable,
)
@management_endpoint_wrapper
async def update_project(
    ...
    - object_permission: Optional[DeRouter_ObjectPermissionBase] - Updated object permission
    ...
    try:
        ...
        existing_project: (
            prisma_models.DeRouter_ProjectTable | None
        ...
        if data.team_id is not None and data.team_id != existing_project.team_id:
            can_assign_to_target = await _check_user_permission_for_project(
                ...
                prisma_client=prisma_client,
                team_object=(
                    DeRouter_TeamTable.model_validate(target_team_obj.model_dump()) if target_team_obj else None
                ),
            )
        ...
        if target_team_obj is not None:
            _check_team_project_limits(
                team_object=DeRouter_TeamTable.model_validate(target_team_obj.model_dump()),
                data=data,
            )
        ...
        verbose_proxy_logger.exception(
            "derouter.proxy.management_endpoints.project_endpoints.update_project(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
...
@router.delete(
    ...
    tags=["project management"],
    dependencies=[Depends(user_api_key_auth)],
    response_model=list[DeRouter_ProjectTable],
)
@management_endpoint_wrapper
async def delete_project(
    ...
    try:
        ...
        deleted_projects: list[prisma_models.DeRouter_ProjectTable | None] = []
        ...
        for project_id in data.project_ids:
            ...
            associated_keys: Sequence[
                prisma_models.DeRouter_VerificationToken
        ...
        verbose_proxy_logger.exception(
            "derouter.proxy.management_endpoints.project_endpoints.delete_project(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
...
async def project_info(
    ...
    try:
        ...
        # Fetch project
        project: prisma_models.DeRouter_ProjectTable | None = await _project_table(prisma_client).find_unique(
        ...
        verbose_proxy_logger.exception(
            "derouter.proxy.management_endpoints.project_endpoints.project_info(): Exception occured - {}".format(str(e))
        )
        raise handle_exception_on_proxy(e)
...
async def list_projects(
    ...
    try:
        ...
        verbose_proxy_logger.exception(
            "derouter.proxy.management_endpoints.project_endpoints.list_projects(): Exception occured - {}".format(
                str(e)
            )
        )
        raise handle_exception_on_proxy(e)
