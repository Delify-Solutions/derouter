# What is this?
## This hook is used to check for DeRouter managed files in the request body, and replace them with model-specific file id
...
import derouter
from derouter import Router, verbose_logger
...
from derouter.llms.base_llm.managed_resources.isolation import (
    build_list_page,
    build_owner_filter,
    can_access_resource,
...
from derouter.proxy.pass_through_endpoints.llm_provider_handlers.batch_attribution import (
    request_tags_from_metadata,
)
from derouter.types.llms.openai import (  # pyright: ignore[reportAttributeAccessIssue]
    AllMessageValues,
    AsyncCursorPage,
...
if TYPE_CHECKING:
    ...
    from prisma.models import (
        DeRouter_ManagedObjectTable as PrismaManagedObjectRow,
    )

    from derouter.proxy.utils import InternalUsageCache as _InternalUsageCache
    from derouter.proxy.utils import PrismaClient as _PrismaClient
...
def _parse_managed_batch_row(row: "PrismaManagedObjectRow") -> Optional[DeRouterBatch]:
    try:
        batch_obj: Final = DeRouterBatch.model_validate(_decode_json_blob(row.file_object))
...
def _managed_file_table(prisma_client: PrismaClient) -> _ManagedFileTableActions:
    return prisma_client.db.derouter_managedfiletable


def _managed_object_table(prisma_client: PrismaClient) -> _ManagedObjectTableActions:
    return prisma_client.db.derouter_managedobjecttable


class _PROXY_DeRouterManagedFiles(CustomLogger, BaseFileEndpoints):
    ...
    def _get_prometheus_logger():
        """Find PrometheusLogger from derouter.callbacks, if registered."""
    ...
    async def store_unified_file_id(
        ...
        derouter_parent_otel_span: Optional[Span],
        model_mappings: Dict[str, str],
        user_api_key_dict: UserAPIKeyAuth,
    ) -> None:
        verbose_logger.info(f"Storing DeRouter Managed File object with id={file_id} in cache")
        if file_object is not None:
            derouter_managed_file_object = DeRouter_ManagedFileTable(
            ...
            await self.internal_usage_cache.async_set_cache(
                key=file_id,
                value=derouter_managed_file_object.model_dump(),
        ...
        verbose_logger.debug(f"DeRouter Managed File object with id={file_id} stored in db: {result}")
    ...
    async def store_unified_object_id(
        ...
        file_object: Union[DeRouterBatch, DeRouterFineTuningJob, "ResponsesAPIResponse"],
        derouter_parent_otel_span: Optional[Span],
        model_object_id: str,
        ...
        """
        verbose_logger.info(f"Storing DeRouter Managed {file_purpose} object with id={unified_object_id} in cache")
        derouter_managed_object = DeRouter_ManagedObjectTable(
        ...
        await self.internal_usage_cache.async_set_cache(
            ...
            value=derouter_managed_object.model_dump(),
            derouter_parent_otel_span=derouter_parent_otel_span,
        )
    ...
    async def get_unified_file_id(
        self, file_id: str, derouter_parent_otel_span: Optional[Span] = None
    ) -> Optional[DeRouter_ManagedFileTable]:
        ## CHECK CACHE
        result = cast(
            Optional[dict],
        ...
        if result:
            return DeRouter_ManagedFileTable.model_validate(result)

        ## CHECK DB
        db_object = await _managed_file_table(self.prisma_client).find_first(where={"unified_file_id": file_id})

        if db_object:
            return DeRouter_ManagedFileTable.model_validate(db_object.model_dump())
        return None
    ...
    async def delete_unified_file_id(
        ...
        if initial_value is None:
            raise Exception(f"DeRouter Managed File object with id={file_id} not found")
        ## delete old value
        await self.internal_usage_cache.async_set_cache(
            key=file_id,
            value=None,
    ...
    async def enforce_batch_object_access(
        ...
        if self.prisma_client is None:
            return
        managed_object = (
            await self.prisma_client.db.derouter_managedobjecttable.find_first(
    ...
    async def enforce_provider_file_access(
        ...
        if self.prisma_client is None:
            return
        managed_file = (
            await self.prisma_client.db.derouter_managedfiletable.find_first(
    ...
    async def store_batch_output_file_ownership(
        self, response: DeRouterBatch, derouter_parent_otel_span: Optional[Span]
    ) -> None:
    ...
    async def list_user_batches(
        ...
        # A hash of the model name + derouter_params for the model name is encoded as the model id. This is not sufficient to reliably map the target model names to the model ids.
    ...
    async def _collect_listed_batches(
        ...
        matches: tuple[DeRouterBatch, ...] = ()  # rebind-ok: accumulates survivors across chunks
    ...
    async def _resolve_listed_rows(
        ...
        resolved: Final[list[DeRouterBatch]] = []  # mutable-ok: resolution stops as soon as the page is full
    ...
    async def _resolve_listed_batch(
        ...
        batch_obj: DeRouterBatch,
        unified_id_by_raw_id: Mapping[str, str],
        user_api_key_dict: UserAPIKeyAuth,
    ) -> Optional[DeRouterBatch]:
    ...
    async def async_pre_call_hook(
        ...
        """
        - Detect derouter_proxy/ file_id
        - add dictionary of mappings of derouter_proxy/ file_id -> provider_file_id => {derouter_proxy/file_id: {"model_id": id, "file_id": provider_file_id}}
        """
        ...
        if is_completion_call:
                ...
                if file_ids:
                    ...
                    if is_vertex_ai:
                        await self._convert_storage_files_to_base64(
                            messages=messages,
                            file_ids=file_ids,
                            derouter_parent_otel_span=user_api_key_dict.parent_otel_span,
                        )
            ...
            if potential_llm_object_id and retrieve_object_id:
                ...
                if potential_model_id is None:
                    raise Exception(
                        f"DeRouter Managed {accessor_key} with id={retrieve_object_id} is invalid - does not contain encoded model_id."
                    )
    ...
    async def async_pre_call_deployment_hook(
        ...
        if accessor_key:
            ...
            # model_info may be at top-level or nested under derouter_metadata
            # (batch/file operations use derouter_metadata)
            model_id = cast(Optional[str], kwargs.get("model_info", {}).get("id", None))
            if model_id is None:
                model_id = cast(
                    Optional[str],
                    kwargs.get("derouter_metadata", {}).get("model_info", {}).get("id", None),
                )
    ...
    def get_vector_store_ids_from_file_search_tools(self, tools: List[Dict[str, object]]) -> List[str]:
        ...
        Only returns IDs that are DeRouter-managed (base64 unified IDs).
        Native provider IDs are skipped — they have no DeRouter access record.
        """
    ...
    async def check_vector_store_ids_access(
        ...
        """
        Verify the caller's team can access each DeRouter-managed vector store.
    ...
    async def get_model_file_id_mapping(self, file_ids: List[str], derouter_parent_otel_span: Span) -> dict:
        """
        Get model-specific file IDs for a list of proxy file IDs.
        Returns a dictionary mapping derouter_proxy/ file_id -> model_id -> model_file_id

        1. Get all the derouter_proxy/ file_ids from the messages
        2. For each file_id, search for cache keys matching the pattern file_id:*
        3. Return a dictionary of mappings of derouter_proxy/ file_id -> model_id -> model_file_id

        Example:
        {
            "derouter_proxy/file_id": {
        ...
        derouter_managed_file_ids = []

        for file_id in file_ids:
            ## CHECK IF FILE ID IS MANAGED BY LITELM
            is_base64_unified_file_id = _is_base64_encoded_unified_file_id(file_id)
            if is_base64_unified_file_id:
                derouter_managed_file_ids.append(file_id)

        if derouter_managed_file_ids:
            # Get all cache keys matching the pattern file_id:*
            for file_id in derouter_managed_file_ids:
                # Search for any cache key starting with this file_id
                unified_file_object = await self.get_unified_file_id(file_id, derouter_parent_otel_span)
    ...
    async def create_file_for_each_model(
        ...
        derouter_parent_otel_span: Span,
    ) -> List[OpenAIFileObject]:
    ...
    async def acreate_file(
        ...
        response = await _PROXY_DeRouterManagedFiles.return_unified_file_id(
    ...
    async def return_unified_file_id(
        ...
        unified_file_id = SpecialEnums.DEROUTER_MANAGED_FILE_COMPLETE_STR.value.format(
            file_type,
            str(uuid.uuid4()),
    ...
    def get_unified_generic_response_id(self, model_id: str, generic_response_id: str) -> str:
        unified_generic_response_id = SpecialEnums.DEROUTER_MANAGED_GENERIC_RESPONSE_COMPLETE_STR.value.format(
    ...
    def get_unified_batch_id(self, batch_id: str, model_id: str) -> str:
        unified_batch_id = SpecialEnums.DEROUTER_MANAGED_BATCH_COMPLETE_STR.value.format(model_id, batch_id)
    ...
    def get_unified_output_file_id(self, output_file_id: str, model_id: str, model_name: Optional[str]) -> str:
        ...
        unified_output_file_id = SpecialEnums.DEROUTER_MANAGED_FILE_COMPLETE_STR.value.format(
            "application/json",
    ...
    async def async_post_call_success_hook(
        ...
        if isinstance(response, DeRouterBatch):
                    ...
                    if file_id_value and model_id:
                        ...
                        try:
                            ...
                            if _llm_router is not None and model_id:
                                ...
                                file_object = await derouter.afile_retrieve(
            ...
            request_metadata: Final = data.get("derouter_metadata")
        ...
        elif isinstance(response, DeRouterFineTuningJob):
            ## Check if unified_file_id is in the response
            unified_file_id = response._hidden_params.get("unified_file_id")  # managed file id
    ...
    async def afile_retrieve(
        self, file_id: str, derouter_parent_otel_span: Optional[Span], llm_router: Optional[Router] = None
    ) -> OpenAIFileObject:
        stored_file_object = await self.get_unified_file_id(file_id, derouter_parent_otel_span)
        ...
        if not llm_router:
            raise Exception(
                f"DeRouter Managed File object with id={file_id} has no file_object "
        ...
        try:
            ...
            response = await derouter.afile_retrieve(file_id=model_file_id, **credentials)
    ...
    async def _get_batches_referencing_file(self, file_id: str) -> List[Dict[str, object]]:
        ...
        try:
            model_file_id_mapping = await self.get_model_file_id_mapping([file_id], derouter_parent_otel_span=None)
    ...
    async def afile_delete(
        ...
        **data: Dict,
        ...
        model_file_id_mapping = await self.get_model_file_id_mapping([file_id], derouter_parent_otel_span)
        ...
        stored_file_object = await self.delete_unified_file_id(file_id, derouter_parent_otel_span)
    ...
    async def afile_content(
        ...
        **data: Dict,
        ...
        model_file_id_mapping = model_file_id_mapping or await self.get_model_file_id_mapping(
            [file_id], derouter_parent_otel_span
        )
        ...
        if specific_model_file_id_mapping:
            ...
            for model_id, provider_file_id in specific_model_file_id_mapping.items():
                try:
                    ...
                    if credentials is not None:
                        data["_derouter_internal_model_credentials"] = cast(Dict, MappingProxyType(dict(credentials)))
                    else:
                        data.pop("_derouter_internal_model_credentials", None)
            ...
            raise Exception(
                f"DeRouter Managed File object with id={file_id} not found. Checked model id's: {specific_model_file_id_mapping.keys()}. Errors: {exception_dict}"
            )
        else:
            raise Exception(f"DeRouter Managed File object with id={file_id} not found")
    ...
    async def _convert_storage_files_to_base64(
        ...
        for file_id in file_ids:
            ...
            try:
                from derouter.llms.base_llm.files.storage_backend_factory import (
                    get_storage_backend,
                )
