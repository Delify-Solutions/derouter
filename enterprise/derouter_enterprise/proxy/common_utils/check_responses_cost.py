"""
Polls DeRouter_ManagedObjectTable to check if the response is complete.
...
from derouter.types.utils import BACKGROUND_RESPONSE_COST_POLL_CALL_ORIGIN
...
class CheckResponsesCost:
    ...
    async def _get_response(
        self,
        response_id: str,
        derouter_metadata: Dict[str, str],
    ) -> ResponsesAPIResponse:
        """Fetch the upstream response, using deployment credentials when available.

        DeRouter-encoded response IDs carry the ``model_id`` of the deployment that
        ...
        ``GET /v1/responses/{id}`` does. ``derouter.aget_responses`` on its own only
        ...
        if model_id is None or self.llm_router.get_deployment(model_id=model_id) is None:
            return await derouter.aget_responses(response_id=response_id, derouter_metadata=derouter_metadata)
        router_response = await self.llm_router.aget_responses(
            response_id=response_id, derouter_metadata=derouter_metadata
        )
    ...
    async def _expire_stale_rows(
        ...
        return await self.prisma_client.db.execute_raw(
            """
            UPDATE "DeRouter_ManagedObjectTable"
            SET "status" = 'stale_expired'
            WHERE "id" IN (
                SELECT "id" FROM "DeRouter_ManagedObjectTable"
    ...
    async def check_responses_cost(self):
        ...
        for job in jobs:
            ...
            try:
                from derouter.proxy.hooks.responses_id_security import (
                    ResponsesIDSecurity,
                )
                ...
                if model_name:
                    derouter_metadata["model"] = model_name
                    derouter_metadata["model_group"] = model_name  # Use same value for model_group
