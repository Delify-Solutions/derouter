"""
Polls DeRouter_ManagedObjectTable to check if the batch job is complete, and if the cost has been tracked.
"""
...
if TYPE_CHECKING:
    ...
    from derouter.proxy._types import DeRouter_ManagedObjectTable
...
CHECK_BATCH_COST_USER_AGENT = "DeRouter Proxy/CheckBatchCost"
...
class CheckBatchCost:
    ...
    async def _get_user_info(self, batch_id: str, user_id: Optional[str]) -> dict[str, str | None]:
        ...
        if not user_id:
            return {}
        try:
            user_row: prisma_models.DeRouter_UserTable | None = (
                await self.prisma_client.db.derouter_usertable.find_unique(
    ...
    async def _get_key_alias(self, batch_id: str, api_key: str | None) -> str | None:
        ...
        if not api_key:
            return None
        try:
            key_row: prisma_models.DeRouter_VerificationToken | None = (
                await self.prisma_client.db.derouter_verificationtoken.find_unique(
                    where={"token": api_key}
                )
            )
    ...
    async def _get_team_alias(self, team_id: str | None) -> str | None:
        ...
        if not team_id:
            return None
        try:
            team_row: prisma_models.DeRouter_TeamTable | None = (
                await self.prisma_client.db.derouter_teamtable.find_unique(
    ...
    async def _get_org_id(self, job: "DeRouter_ManagedObjectTable", batch_id: str) -> str | None:
    ...
    async def _build_creator_attribution_metadata(
        self, job: "DeRouter_ManagedObjectTable", batch_id: str
    ) -> dict[str, object]:
        """
    ...
    async def _cleanup_stale_managed_objects(self) -> None:
        ...
        result: Final = await self.prisma_client.db.derouter_managedobjecttable.update_many(
            where={
                "file_purpose": "batch",
        ...
        retired: Final = await self.prisma_client.db.derouter_managedobjecttable.update_many(
            where={
                "file_purpose": "batch",
    ...
    async def _fallback_find_jobs(self) -> list:
        ...
        return await self.prisma_client.db.derouter_managedobjecttable.find_many(
            where={
                "file_purpose": "batch",
                "status": {
                    "not_in": [
    ...
    async def _retire_job(self, job: "DeRouter_ManagedObjectTable", reason: str) -> None:
        """
        ...
        try:
            await self.prisma_client.db.derouter_managedobjecttable.update(
                where={"id": job.id},
                data=data,
            )
    ...
    async def _claim_job_for_costing(self, job: "DeRouter_ManagedObjectTable") -> bool:
        """
        ...
        if not self._has_batch_processed_column:
            return True
        try:
            claimed: Final = await self.prisma_client.db.derouter_managedobjecttable.update_many(
    ...
    async def _release_job_claim(self, job: "DeRouter_ManagedObjectTable") -> None:
        ...
        if not self._has_batch_processed_column:
            return
        try:
            await self.prisma_client.db.derouter_managedobjecttable.update_many(
    ...
    @staticmethod
    def _has_unified_id_without_model(job: "DeRouter_ManagedObjectTable") -> bool:
    ...
    async def _finalize_unbilled_terminal_job(
        self, job: "prisma_models.DeRouter_ManagedObjectTable", response: "DeRouterBatch"
    ) -> None:
    ...
    def _resolve_job_routing(
        self,
        job: "DeRouter_ManagedObjectTable",
        ...
        DeRouter's own /v1/batches with a raw input_file_id) store the raw provider job id as
        ...
        if self._track_unmanaged_batch_cost:
            from derouter.llms.bedrock.batches.transformation import (
    ...
    def _get_deployment_id_from_matching_deployments(
        ...
        for deployment in self.llm_router.get_model_list(model_name=None) or []:
            derouter_params = deployment.get("derouter_params") or {}
            ...
            try:
                _, deployment_llm_provider, _, _ = get_llm_provider(
                    model=actual_model,
                    custom_llm_provider=derouter_params.get("custom_llm_provider"),
                )
            except Exception:
                continue
    ...
    def _get_deployment_id_for_provider(
        ...
        for deployment_id in self.llm_router.get_model_ids(model_name=model_group):
            ...
            try:
                _, deployment_llm_provider, _, _ = get_llm_provider(
                    model=deployment_info.derouter_params.model,
                    custom_llm_provider=deployment_info.derouter_params.custom_llm_provider,
                )
            except Exception:
                continue
            if deployment_llm_provider == llm_provider:
    ...
    @staticmethod
    def _get_input_file_id(job: "DeRouter_ManagedObjectTable") -> Optional[str]:
        import json
        ...
        if not isinstance(file_object, dict):
            return None
        try:
            return DeRouterBatch.model_validate(file_object).input_file_id
        except Exception:
            return None
    ...
    async def _track_completed_batch_cost(
        ...
        later poll.
        """
        from derouter.batches.batch_utils import (
            count_error_file_failed_requests,
            _get_file_content_as_dictionary,
            calculate_batch_cost_and_usage,
        )
        from derouter.files.main import afile_content
        from derouter.derouter_core_utils.get_llm_provider_logic import get_llm_provider
        from derouter.derouter_core_utils.derouter_logging import Logging as DeRouterLogging
        from derouter.derouter_core_utils.derouter_logging import deployment_pricing_model_info, mask_api_base_credentials
        from derouter.proxy.openai_files_endpoints.common_utils import (
            _is_base64_encoded_unified_file_id,
        )
        ...
        #
        # CheckBatchCost builds its own DeRouterLogging object (logging_obj below) and
        ...
        _file_content = await afile_content(
            ...
            _derouter_internal_model_credentials=MappingProxyType(dict(credentials)),
            **credentials,
        )
        ...
        if deployment_info is None:
            ...
            return None
        custom_llm_provider = deployment_info.derouter_params.custom_llm_provider
        derouter_model_name = deployment_info.derouter_params.model
        ...
        # Pass the deployment's router-registered pricing (derouter_params custom
        ...
        deployment_model_info = deployment_pricing_model_info(
            model_id=model_id,
            deployment_model=derouter_model_name,
        )
        ...
        deployment_api_base: Final = deployment_info.derouter_params.api_base
    ...
    async def check_batch_cost(self):
        ...
        if self._has_batch_processed_column:
            try:
                ...
                jobs = await self.prisma_client.db.derouter_managedobjecttable.find_many(
                    where={
                        "file_purpose": "batch",
                        "batch_processed": False,
                        "status": {
