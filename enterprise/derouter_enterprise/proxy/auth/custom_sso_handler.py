"""
Enterprise Custom SSO Handler for DeRouter Proxy
...
Use this when you have an OAuth proxy in front of DeRouter (where the OAuth proxy
...
class EnterpriseCustomSSOHandler:
    ...
    async def handle_custom_ui_sso_sign_in(
        ...
        Use this when you have an OAuth proxy in front of DeRouter (where the OAuth proxy has already authenticated the user)

        Args:
            request: The FastAPI request object containing headers and other request data

        Returns:
            RedirectResponse: Redirect response that sends the user to the DeRouter UI with authentication token
        ...
        from derouter.proxy.auth.trusted_proxy_utils import (
