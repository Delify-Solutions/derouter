-- One-shot backfill of the DeRouter_DailyToolSpend rollup from the per-request
-- DeRouter_SpendLogToolIndex x DeRouter_SpendLogs tables.
--
...
INSERT INTO "DeRouter_DailyToolSpend" (date, tool_name, spend, total_tokens, request_count, created_at, updated_at)
SELECT
    ...
    now() AS created_at,
    now() AS updated_at
FROM "DeRouter_SpendLogToolIndex" ti
JOIN "DeRouter_SpendLogs" sl ON sl.request_id = ti.request_id
