...
// (prevents EBUSY on `npm i -g derouter@latest` while MITM is running).
...
const ENCRYPT_SALT = "derouter-mitm-pwd";
...
async function startServer(apiKey, sudoPassword, forceKillPort443 = false) {
  ...
  if (!effectiveServerPath || !fs.existsSync(effectiveServerPath)) {
    ...
    if (!effectiveServerPath || !fs.existsSync(effectiveServerPath)) {
      throw new Error(`MITM server.js not found at ${effectiveServerPath}. Reinstall derouter.`);
    }
  }
