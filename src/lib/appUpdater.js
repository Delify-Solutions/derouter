...
// Collect PIDs of all derouter-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from derouter install dir or wrapper cli.js
        const isAppProcess = lower.includes("derouter") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("derouter") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "derouter");
  }
  return path.join(os.homedir(), ".derouter");
}
...
// Resolve npx/derouter binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx derouter` — works regardless of global bin path changes after npm i -g
