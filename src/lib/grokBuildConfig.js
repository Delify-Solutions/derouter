...
const previousDefaultRegExp = /^# derouter-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# derouter-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via derouter gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# derouter-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# derouter-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}
