...
let reloadCallbacks: LocaleChangeCallback[] = [];
...
// Translate with type passthrough for non-string values
export function translateT<T>(text: T): T {
  if (typeof text !== "string") return text;
  return translate(text) as T;
}
...
interface OriginalTextNode extends Text {
...
function processTextNode(node: Text): void {
  ...
  const origNode = node as OriginalTextNode;
  if (!origNode._originalText) {
    origNode._originalText = node.nodeValue;
  }

  // Use original text for translation
  const original = origNode._originalText;
