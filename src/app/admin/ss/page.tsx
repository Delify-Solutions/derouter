...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const data = await apiPost<LoginApiResponse>("/api/auth/login", { password });
      ...
      if (err instanceof Error && "status" in err) {
        const apiErr = err as { status: number; body?: LoginApiResponse };
        setError(apiErr.body?.error || "Invalid password");
        if (apiErr.body?.resetHint) setResetHint(apiErr.body.resetHint);
        if (apiErr.body?.retryAfter) setRetryAfter(Number(apiErr.body.retryAfter));
      } else {
  ...
  const handleSetNewPassword = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      await apiPatch<SettingsPatchResponse>("/api/settings", {
      ...
      if (err instanceof Error && "status" in err) {
        const apiErr = err as { status: number; body?: SettingsPatchResponse };
        setError(apiErr.body?.error || "Failed to set password");
      } else {
        setError("An error occurred. Please try again.");
      }
    } finally {
    ...
    <div className="min-h-screen flex items-center justify-center bg-bg p-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-primary mb-2">derouter</h1>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {resetHint && (
                    <p className="text-xs text-text-muted">
                      Forgot password? Open <code className="bg-sidebar px-1 rounded">derouter</code> CLI on the host → <b>Settings</b> → <b>Reset Password to Default</b>.
                    </p>
                  )}
                </div>
