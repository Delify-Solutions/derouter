...
type PricingData = Record<string, PricingRates> | null;

export default function PricingSettingsPage() {
  const router = useRouter();
  const [showModal, setShowModal] = useState(false);
  const [currentPricing, setCurrentPricing] = useState<PricingData>(null);
  ...
  const loadPricing = async (): Promise<void> => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCurrentPricing(data as PricingData);
      }
    } catch (error) {
  ...
  const handlePricingUpdated = (): void => {
    loadPricing();
  };
