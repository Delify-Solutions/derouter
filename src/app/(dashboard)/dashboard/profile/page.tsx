...
import { apiGet, apiPatch, apiPost, ApiError } from "@/shared/api/client";
...
export default function ProfilePage() {
    ...
    apiGet<Settings>("/api/settings")
      .then((data) => {
        ...
        setSamlForm({
          ...
          samlIssuer: data?.samlIssuer || "urn:derouter:sp",
  ...
  const updateOutboundProxy = async (e) => {
    ...
    try {
      const data = await apiPatch<Settings>("/api/settings", {
      ...
      setProxyStatus({ type: "error", message: err instanceof Error ? err.message : "Failed to update proxy settings" });
    } finally {
  ...
  const testOutboundProxy = async () => {
    ...
    try {
      const data = await apiPost<{ ok: boolean; status: number; elapsedMs: number; error?: string }>("/api/settings/proxy-test", { proxyUrl });
      if (data?.ok) {
        setProxyStatus({
          type: "success",
      ...
      setProxyStatus({ type: "error", message: err instanceof Error ? err.message : "An error occurred" });
    } finally {
  ...
  const updateOutboundProxyEnabled = async (outboundProxyEnabled: boolean) => {
    ...
    try {
      const data = await apiPatch<Settings>("/api/settings", { outboundProxyEnabled });
      setSettings((prev) => ({ ...prev, ...data }));
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPassStatus({ type: "error", message: err instanceof Error ? err.message : "Failed to update password" });
    } finally {
  ...
  const updateFallbackStrategy = async (strategy: string) => {
    try {
      await apiPatch("/api/settings", { fallbackStrategy: strategy });
  ...
  const updateComboStrategy = async (strategy: string) => {
    try {
      await apiPatch("/api/settings", { comboStrategy: strategy });
  ...
  const updateStickyLimit = async (limit: string) => {
    ...
    try {
      await apiPatch("/api/settings", { stickyRoundRobinLimit: numLimit });
  ...
  const updateComboStickyLimit = async (limit: string) => {
    ...
    try {
      await apiPatch("/api/settings", { comboStickyRoundRobinLimit: numLimit });
  ...
  const updateRequireLogin = async (requireLogin: boolean) => {
    try {
      await apiPatch("/api/settings", { requireLogin });
  ...
  const saveOidcSettings = async (authMode: string = oidcForm.authMode || "password") => {
    ...
    try {
      ...
      const data = await apiPatch<Settings>("/api/settings", payload);
      setSettings((prev) => ({ ...prev, ...data }));
      setOidcForm({
      ...
      setOidcStatus({ type: "error", message: err instanceof Error ? err.message : "Failed to save OIDC settings" });
    } finally {
  ...
  const testOidcConnection = async () => {
    ...
    try {
      const saved = await apiPatch<Settings>("/api/settings", {
      ...
      if (!saved) {
        setOidcTestStatus({
          type: "error",
          message: "Failed to save OIDC settings before testing",
        });
        return;
      }
    ...
    reader.onload = (e) => {
      try {
        ...
        setSamlForm((prev) => ({
          ...
          samlIssuer: prev.samlIssuer || "urn:derouter:sp",
  ...
  const saveSamlSettings = async (targetAuthMode = oidcForm.authMode || "password") => {
    ...
    try {
      const payload = {
        ...
        samlIssuer: samlForm.samlIssuer.trim() || "urn:derouter:sp",
      ...
      setSamlStatus({ type: "error", message: err instanceof Error ? err.message : "Failed to save SAML settings" });
    } finally {
  ...
  const updateObservabilityEnabled = async (enabled: boolean) => {
    try {
      await apiPatch("/api/settings", { enableObservability: enabled });
  ...
  const handleExportDatabase = async (password: string) => {
    ...
    try {
      // Export needs custom header — use raw fetch for this one
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:20128'}/api/settings/database`, {
        credentials: "include",
        headers: { "x-dr-password": password },
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
      ...
      anchor.href = url;
      anchor.download = `derouter-backup-${stamp}.json`;
  ...
  const runImportDatabase = async (password: string) => {
    ...
    try {
      ...
      await apiPost("/api/settings/database", { ...payload, password });
  ...
  const handleShutdown = async () => {
    ...
    try {
      await apiPost("/api/version/shutdown");
    } catch {
  ...
  const handleLogout = async () => {
    try {
      ...
      window.location.assign("/admin/ss");
    } catch (err) {
          ...
          <div className="flex flex-col gap-3 pt-4 border-t border-border">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 rounded-lg bg-bg border border-border gap-2">
              <div>
                ...
                <p className="text-xs sm:text-sm text-text-muted font-mono break-all">~/.derouter/db/data.sqlite</p>
              </div>
            </div>
                        ...
                        <div className="p-2.5 rounded border border-primary/20 bg-primary/5 text-primary text-xs">
                          ...
                          <ul className="list-disc pl-4 space-y-1 font-mono text-[11px]">
                            ...
                            <li>
                              ...
                              <code className="bg-bg px-1 py-0.5 rounded break-all">{samlForm.samlIssuer || "urn:derouter:sp"}</code>
                            </li>
                            <li>
                              <b>NameID Format:</b>{" "}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
                          <div className="p-3 rounded border border-border bg-bg/50 flex flex-col gap-1.5">
                            ...
                            <ol className="list-decimal pl-4 text-text-muted space-y-1">
                              ...
                              <li>Set <b>Application SAML audience</b> to <code className="text-text-main font-mono">{samlForm.samlIssuer || "urn:derouter:sp"}</code>.</li>
                          ...
                          <div className="p-3 rounded border border-border bg-bg/50 flex flex-col gap-1.5">
                            ...
                            <ol className="list-decimal pl-4 text-text-muted space-y-1">
                              ...
                              <li><b>Identifier (Entity ID):</b> <code className="text-text-main font-mono">{samlForm.samlIssuer || "urn:derouter:sp"}</code></li>
                          ...
                          <div className="p-3 rounded border border-border bg-bg/50 flex flex-col gap-1.5">
                            ...
                            <ol className="list-decimal pl-4 text-text-muted space-y-1">
                              ...
                              <li><b>Audience URI (SP Entity ID):</b> <code className="text-text-main font-mono">{samlForm.samlIssuer || "urn:derouter:sp"}</code></li>
                          ...
                          <div className="p-3 rounded border border-border bg-bg/50 flex flex-col gap-1.5">
                            ...
                            <ol className="list-decimal pl-4 text-text-muted space-y-1">
                              ...
                              <li><b>Client ID:</b> <code className="text-text-main font-mono">{samlForm.samlIssuer || "urn:derouter:sp"}</code></li>
                  ...
                  <div className="flex flex-col gap-2 p-3 rounded-lg border border-border bg-bg text-xs sm:text-sm text-text-muted">
                    ...
                    <div className="flex items-center justify-between gap-2 pt-2 border-t border-border/50">
                      ...
                      <a
                        ...
                        download="derouter-sp-metadata.xml"
                ...
                <div className="flex flex-col gap-4 pt-2 border-t border-border/50">
                  <div className="grid grid-cols-1 gap-4">
                    <div className="flex flex-col gap-2">
                      ...
                      <Input
                        placeholder="https://auth.example.com/application/o/derouter/"
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <Input
                        placeholder="derouter-dashboard"
