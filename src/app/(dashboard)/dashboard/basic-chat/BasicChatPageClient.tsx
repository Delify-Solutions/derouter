...
type MessageStatus = "streaming" | "done" | "error" | undefined;
...
function buildUserContent(message: ChatMessage): string | UserContentItem[] {
...
function readAssistantText(chunk: unknown): string {
  ...
  const pieces = [delta.content, choice?.message?.content, c.output_text, c.text]
    .map(textValue)
    .filter(Boolean);
  return pieces[0] || "";
}
...
function getProviderLabel(connection: ProviderConnection | undefined | null): string {
  if (!connection) return "provider";
  return connection.name || humanize(connection.provider || connection.id || "provider");
}

function normalizeStaticModel(model: StaticModel, connection: ProviderConnection): ModelOption | null {
...
function normalizeLiveModel(model: unknown, connection: ProviderConnection): ModelOption | null {
  const m = (typeof model === "object" && model !== null ? model : {}) as LiveModel;
  const rawId = typeof model === "string" ? model : (m.id || m.name || m.model || "");
  if (!rawId) return null;

  const displayName = typeof model === "string"
    ? model
    : (m.name || m.displayName || rawId);
...
function parseProviderModelsPayload(data: unknown): unknown[] {
  ...
  if (Array.isArray(d.models)) return d.models as unknown[];
...
export default function BasicChatPageClient() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        const providersData = await providersRes.json().catch(() => ({})) as { connections?: ProviderConnection[] };
        ...
        for (const connection of connections) {
          ...
          const staticModels: ModelOption[] = getModelsByProviderId(providerId)
            .map((model: StaticModel) => normalizeStaticModel(model, connection))
        ...
        if (!cancelled) {
          setLoadError(textValue((error as Error)?.message) || "Failed to load providers/models.");
          setProviderGroups([]);
        }
      } finally {
  ...
  const ensureSessionForModel = (model: ModelOption | null): ChatSession | null => {
  ...
  const finalizeSessionTitle = (sessionId: string, titleSeed: unknown) => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const errMsg = errorData.error || errorData.message || `Request failed (${response.status})`;
        throw new Error(textValue(errMsg));
      }

      const reader = response.body?.getReader();
      if (!reader) {
        const data = await response.json().catch(() => ({})) as StreamJsonResponse;
        const fallbackText = textValue(data?.choices?.[0]?.message?.content || data?.output_text || data?.error || data?.message || "");
        updateSession(sessionId, (currentSession) => ({
          ...currentSession,
          messages: currentSession.messages.map((message) => (message.id === assistantMessageId ? { ...message, content: fallbackText, status: "done" as const } : message)),
          updatedAt: new Date().toISOString(),
        }));
        return;
      }
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            updateSession(sessionId, (currentSession) => ({
              ...currentSession,
              messages: currentSession.messages.map((message) => (message.id === assistantMessageId ? { ...message, content: assistantText, status: "streaming" as const } : message)),
              updatedAt: new Date().toISOString(),
            }));
          } catch {
      ...
      updateSession(sessionId, (currentSession) => ({
        ...currentSession,
        messages: currentSession.messages.map((message) => (message.id === assistantMessageId ? { ...message, content: assistantText || message.content, status: "done" as const } : message)),
        updatedAt: new Date().toISOString(),
      }));
      ...
      if (err.name !== "AbortError") {
        const errorText = textValue(err?.message || error);
        updateSession(sessionId, (currentSession) => ({
          ...currentSession,
          messages: currentSession.messages.map((message) => (message.id === assistantMessageId ? { ...message, content: message.content || `Error: ${errorText}`, status: "error" as const } : message)),
          updatedAt: new Date().toISOString(),
        }));
