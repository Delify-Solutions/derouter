...
import { apiGet, apiPost, apiPatch, apiPut, apiDelete, ApiError } from "@/shared/api/client";
import type { Settings, ApiKey, KeyListResponse } from "@/shared/types";
import {
  TUNNEL_BENEFITS,
 ...
 // Cloudflare Tunnel state
  ...
  const loadSettings = async () => {
    ...
    try {
      ...
      if (settingsData) {
        setRequireApiKey(!!settingsData.requireApiKey);
        setRequireLogin(settingsData.requireLogin !== false);
        setHasPassword(!!settingsData.hasPassword);
        setTunnelDashboardAccess(!!settingsData.tunnelDashboardAccess);
      }
      if (statusRes.ok) {
        const data = await statusRes.json();
        const tEnabled = data.tunnel?.settingsEnabled ?? data.tunnel?.enabled ?? false;
  ...
  const handleTunnelDashboardAccess = async (value: boolean) => {
    try {
      await apiPatch("/api/settings", { tunnelDashboardAccess: value });
  ...
  const handleRequireApiKey = async (value: boolean) => {
    try {
      await apiPatch("/api/settings", { requireApiKey: value });
  ...
  const fetchData = async () => {
    try {
      const fetchKeys = async (): Promise<ApiKey[]> => {
        const data = await apiGet<KeyListResponse>("/api/keys").catch(() => null);
        if (!data) return [];
        return data.keys || [];
      };
      ...
      if (existing.length === 0) {
        try {
          await apiPost("/api/keys", { name: "Default Key" });
  ...
  const loadGroups = useCallback(async () => {
    try {
      const data = await apiGet<{ groups?: any[] }>("/api/groups");
  ...
  const handleCreateKey = async () => {
    ...
    try {
      const data = await apiPost<{ key: ApiKey } & Record<string, unknown>>("/api/keys", body);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const data = await apiPut<ApiKey>(`/api/keys/${editingKey.id}`, body);
      await fetchData();
  ...
  const handleToggleKey = async (id, isActive) => {
    try {
      await apiPut(`/api/keys/${id}`, { isActive });
          ...
          <div className="bg-surface-2 border border-border-subtle rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-text-muted">
                  Expose your local derouter to the internet. No port forwarding, no static IP needed. Share endpoint URL with your team or use it in Cursor, Cline, and other AI tools from anywhere.
                </p>
              </div>
            </div>
          </div>
