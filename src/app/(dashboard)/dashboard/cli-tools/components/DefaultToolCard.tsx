...
import type { ApiKey, ProviderConnection } from "@/shared/types";
...
export default function DefaultToolCard({ toolId, tool, isExpanded, onToggle, baseUrl, apiKeys = [], activeProviders = [], cloudEnabled = false, tunnelEnabled = false }: DefaultToolCardProps) {
  ...
  const replaceVars = (text: string): string => {
    const keyToUse = (selectedApiKey && selectedApiKey.trim())
      ? selectedApiKey
      : (!cloudEnabled ? "sk_derouter" : "your-api-key");
  ...
  const hasActiveProviders = (activeProviders?.length ?? 0) > 0;
  ...
  const renderNotes = (): React.ReactNode => {
    const notes = tool.notes as Note[] | undefined;
  ...
  const canShowGuide = (): boolean => {
    ...
    if ((tool as Record<string, unknown>).requiresCloud && !cloudEnabled) return false;
    return true;
  };

  const renderGuideSteps = (): React.ReactNode => {
    const guideSteps = tool.guideSteps as GuideStep[] | undefined;
    ...
    return (
      <div className="flex flex-col gap-4">
        ...
        {canShowGuide() && !!(tool as Record<string, unknown>).codeBlock && (
          <div className="mt-2">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-text-muted uppercase tracking-wide">{(tool as Record<string, { language: string; code: string }>).codeBlock.language}</span>
              <button
                onClick={() => handleCopy((tool as Record<string, { language: string; code: string }>).codeBlock.code, "codeblock")}
            ...
            <pre className="p-4 bg-bg-secondary rounded-lg border border-border overflow-x-auto">
              <code className="text-sm font-mono whitespace-pre">{replaceVars((tool as Record<string, { language: string; code: string }>).codeBlock.code)}</code>
            </pre>
          </div>
        )}
      </div>
