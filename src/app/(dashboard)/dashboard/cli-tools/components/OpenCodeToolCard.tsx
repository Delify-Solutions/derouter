...
interface OpenCodeStatus extends CliToolStatus {
  opencode?: { models?: string[]; activeModel?: string };
  config?: {
    provider?: { derouter?: { options?: { baseURL?: string } } };
    agent?: { explorer?: { model?: string } };
  };
}

export default function OpenCodeToolCard({
  tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl,
}: OpenCodeToolCardProps) {
  const [status, setStatus] = useState<OpenCodeStatus | null>(initialStatus as OpenCodeStatus || null);
  ...
  useEffect(() => {
    if (initialStatus) setStatus(initialStatus as OpenCodeStatus);
  }, [initialStatus]);
  ...
  useEffect(() => {
    ...
    if (status?.config?.agent?.explorer?.model?.startsWith("derouter/")) {
      setSubagentModel(status.config.agent.explorer.model.replace("derouter/", ""));
    }
  }, [status]);
  ...
  const saveModels = async (models: string[]) => {
    try {
      const keyToUse = (selectedApiKey && selectedApiKey.trim())
        ? selectedApiKey
        : (!cloudEnabled ? "sk_derouter" : selectedApiKey);
      const validActiveModel = models.includes(activeModel) ? activeModel : (models[0] || "");
      await apiPost<{ error?: string }>("/api/cli-tools/opencode", {
  ...
  const currentBaseUrl = status?.config?.provider?.["derouter"]?.options?.baseURL || "";

  const getConfigStatus = (): "configured" | "not_configured" | "other" | null => {
    if (!status?.installed) return null;
    if (!status.config) return "not_configured";
    if (!status.hasderouter) return "not_configured";
    const url = status.config?.provider?.["derouter"]?.options?.baseURL || "";
  ...
  const checkStatus = async () => {
    setChecking(true);
    try {
      const data = await apiGet<OpenCodeStatus>("/api/cli-tools/opencode");
      setStatus(data);
    } catch (error) {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      const data = await apiPost<{ error?: string }>("/api/cli-tools/opencode", {
        baseUrl: getEffectiveBaseUrl(),
        apiKey: keyToUse,
  ...
  const handleReset = async () => {
    ...
    setMessage(null);
    try {
      const data = await apiDelete<{ error?: string }>("/api/cli-tools/opencode");
      if (data.error) {
        setMessage({ type: "error", text: data.error });
      } else {
  ...
  const getManualConfigs = () => {
    ...
    return [{
      ...
      content: JSON.stringify({
        ...
        model: `derouter/${activeModelToShow}`,
        agent: {
          explorer: {
            description: "Fast explorer subagent for codebase exploration",
            mode: "subagent",
            model: `derouter/${effectiveSubagentModel}`
          }
        }
      }, null, 2),
    }];
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted">Manual configuration is still available if derouter is deployed on a remote server.</p>
                  </div>
                </div>
          ...
          {!checking && status?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {status?.config?.provider?.["derouter"]?.options?.baseURL && (
                          ...
                          <span
                            ...
                            onClick={async () => {
                              if (model === activeModel) {
                                try {
                                  const data = await apiPatch<{ error?: string }>("/api/cli-tools/opencode", { clearActiveModel: true });
                                  if (!data.error) {
                                    setActiveModel("");
                                    checkStatus();
                                  }
                          ...
                          >
                            ...
                            <button
                              onClick={async (e) => {
                                e.stopPropagation();
                                try {
                                  const res = await fetch(`/api/cli-tools/opencode?model=${encodeURIComponent(model)}`, { method: "DELETE" });
                                  if (res.ok) {
                                  ...
                                  console.log("Error removing model:", (error as Error).message);
                                }
                              }}
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!status.hasderouter} loading={restoring}>
