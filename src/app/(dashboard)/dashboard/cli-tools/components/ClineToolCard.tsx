...
interface ClineStatus extends CliToolStatus {
  settings?: { openAiBaseUrl?: string; openAiModelId?: string };
}
...
export default function ClineToolCard({
  ...
  const [status, setStatus] = useState<ClineStatus | null>((initialStatus as ClineStatus) || null);
  ...
  useEffect(() => {
    if (apiKeys && apiKeys.length > 0 && !selectedApiKey) setSelectedApiKey(apiKeys[0].key);
  }, [apiKeys, selectedApiKey]);

  useEffect(() => {
    if (initialStatus) setStatus(initialStatus as ClineStatus);
  }, [initialStatus]);
  ...
  const checkStatus = async () => {
    setChecking(true);
    try {
      const data = await apiGet<ClineStatus>("/api/cli-tools/cline");
      setStatus(data);
    } catch (error) {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      const data = await apiPost<{ error?: string }>("/api/cli-tools/cline", {
  ...
  const handleReset = async () => {
    ...
    setMessage(null);
    try {
      const data = await apiDelete<{ error?: string }>("/api/cli-tools/cline");
      if (data.error) {
        setMessage({ type: "error", text: data.error });
      } else {
    ...
    <Card padding="xs" className="overflow-hidden">
          ...
          {!checking && status?.installed && (
            <>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                <Button variant="primary" size="sm" onClick={handleApply} disabled={(!selectedApiKey && (cloudEnabled && (apiKeys?.length ?? 0) > 0)) || !selectedModel} loading={applying}>
      ...
      {modalOpen && (
        <ModelSelectModal
          ...
          onSelect={(model: { value: string }) => { setSelectedModel(model.value); setModalOpen(false); }}
