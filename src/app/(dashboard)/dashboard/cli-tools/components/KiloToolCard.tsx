...
export default function KiloToolCard({
  ...
  const [status, setStatus] = useState<CliToolStatus | null>(initialStatus || null);
  ...
  useEffect(() => {
    if ((apiKeys?.length ?? 0) > 0 && !selectedApiKey) setSelectedApiKey(apiKeys![0].key);
  ...
  const getConfigStatus = (): "configured" | "not_configured" | null => {
    if (!status?.installed) return null;
    return status.hasderouter ? "configured" : "not_configured";
  };
  ...
  const checkStatus = async () => {
    setChecking(true);
    try {
      const data = await apiGet<CliToolStatus>("/api/cli-tools/kilo");
      setStatus(data);
    } catch (error) {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      const data = await apiPost<{ error?: string }>("/api/cli-tools/kilo", {
  ...
  const handleReset = async () => {
    ...
    setMessage(null);
    try {
      const data = await apiDelete<{ error?: string }>("/api/cli-tools/kilo");
      if (data.error) {
        setMessage({ type: "error", text: data.error });
      } else {
