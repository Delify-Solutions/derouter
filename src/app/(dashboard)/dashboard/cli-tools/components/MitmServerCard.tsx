...
import type { CliToolStatus, ToolMessage } from "./cliTools.types";
import type { ApiKey } from "@/shared/types";

const DEFAULT_MITM_ROUTER_BASE = "http://localhost:20128";

interface MitmStatus extends CliToolStatus {
  running?: boolean;
  certExists?: boolean;
  certTrusted?: boolean;
  isWin?: boolean;
...
interface MitmServerCardProps {
  ...
  onStatusChange?: (data: MitmStatus) => void;
}
...
export default function MitmServerCard({ apiKeys, cloudEnabled, onStatusChange }: MitmServerCardProps) {
  ...
  const [pendingAction, setPendingAction] = useState<"start" | "stop" | "trust-cert" | null>(null);
  const [modalError, setModalError] = useState<ToolMessage | null>(null);
  const [actionError, setActionError] = useState<ToolMessage | null>(null);
  ...
  const fetchStatus = useCallback(async () => {
    try {
      const data = await apiGet<MitmStatus>("/api/cli-tools/antigravity-mitm");
      setStatus(data);
  ...
  const handleAction = (action: string) => {
    ...
    if (canRunWithoutPassword) {
      ...
      setPendingAction(action as "start" | "stop" | "trust-cert");
  ...
  const doAction = async (action: string, password: string, forceKillPort443 = false) => {
    ...
    setActionError(null);
    try {
      let data: { error?: string; code?: string; portOwner?: { name?: string; pid?: number } } | undefined;
      if (action === "trust-cert") {
        data = await apiPatch<{ error?: string; code?: string; portOwner?: { name?: string; pid?: number } }>("/api/cli-tools/antigravity-mitm", { action: "trust-cert", sudoPassword: password });
      } else if (action === "start") {
        const keyToUse = selectedApiKey?.trim()
          || (apiKeys && apiKeys.length > 0 ? apiKeys[0].key : null)
          || (!cloudEnabled ? "sk_derouter" : null);
        data = await apiPost<{ error?: string; code?: string; portOwner?: { name?: string; pid?: number } }>("/api/cli-tools/antigravity-mitm", {
          apiKey: keyToUse,
        ...
        // DELETE with body — apiDelete doesn't support body, so use raw fetch
        ...
        if (!res.ok) {
          const errData = await res.json().catch(() => ({})) as { error?: string; code?: string; portOwner?: { name?: string; pid?: number } };
          data = errData;
        }
      }
      ...
      if (data?.code === "PORT_443_BUSY" && data.portOwner) {
        ...
        setPort443Conflict({ owner: { name: data.portOwner.name || "unknown", pid: data.portOwner.pid || 0 }, password });
        return;
      }
      if (data?.error) {
        setActionError({ type: "error", text: data.error || `Failed to ${action} MITM server` });
        return;
      }
      setShowPasswordModal(false);
      setSudoPassword("");
      ...
      setActionError({ type: "error", text: (e as Error).message || "Network error" });
    } finally {
      setLoading(false);
  ...
  const handleConfirmPassword = () => {
    if (!sudoPassword.trim()) {
      setModalError({ type: "error", text: "Sudo password is required" });
      return;
    }
    if (pendingAction) {
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="px-2 py-2 rounded-lg bg-surface/50 border border-border/50 flex flex-col gap-2">
            <p className="text-[11px] text-text-muted leading-relaxed">
              <span className="font-medium text-text-main">Purpose:</span> Use Antigravity IDE & GitHub Copilot → with ANY provider/model from derouter
            </p>
            <p className="text-[11px] text-text-muted leading-relaxed">
              <span className="font-medium text-text-main">How it works:</span> Antigravity/Copilot IDE request → DNS redirect to localhost:443 → MITM proxy intercepts → derouter → response to Antigravity/Copilot
            </p>
          </div>
          ...
          <div className="flex flex-col gap-2">
            <div className="grid gap-1 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
              <span className="text-xs font-semibold text-text-main sm:text-right sm:text-sm">derouter Base URL</span>
            ...
            {!isRunning && (
              <div className="grid gap-1 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                ...
                <input
                  ...
                  placeholder={cloudEnabled ? "Enter or pick API key" : "sk_derouter (default)"}
          ...
          {serverIsWindows && !isAdmin && (
            <div className="flex items-center gap-2 px-2 py-1.5 rounded text-xs bg-red-500/10 text-red-600 border border-red-500/20">
              ...
              <span>Administrator required — restart derouter as Administrator to use MITM</span>
            </div>
          )}
        </div>
      </Card>
