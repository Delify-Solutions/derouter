...
interface CopilotStatus extends CliToolStatus {
...
export default function CopilotToolCard({
  ...
  const [status, setStatus] = useState<CopilotStatus | null>(initialStatus as CopilotStatus || null);
  ...
  useEffect(() => {
    if (initialStatus) setStatus(initialStatus as CopilotStatus);
  }, [initialStatus]);
  ...
  useEffect(() => {
    if (status?.config && Array.isArray(status.config) && selectedModels.length === 0) {
      const entry = status.config.find((e) => e.name === "derouter");
  ...
  const saveModels = async (models: string[]) => {
    try {
      ...
      await apiPost<{ error?: string; message?: string }>("/api/cli-tools/copilot", {
  ...
  const checkStatus = async () => {
    setChecking(true);
    try {
      const data = await apiGet<CopilotStatus>("/api/cli-tools/copilot");
      setStatus(data);
    } catch (error) {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      const data = await apiPost<{ error?: string; message?: string }>("/api/cli-tools/copilot", {
        baseUrl: getEffectiveBaseUrl(),
        apiKey: keyToUse,
  ...
  const handleReset = async () => {
    ...
    setMessage(null);
    try {
      const data = await apiDelete<{ error?: string }>("/api/cli-tools/copilot");
      if (data.error) {
        setMessage({ type: "error", text: data.error });
      } else {
