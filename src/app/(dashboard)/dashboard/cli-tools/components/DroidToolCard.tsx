...
interface DroidStatus extends CliToolStatus {
  settings?: {
    customModels?: Array<{ id?: string; model?: string; index?: number; baseUrl?: string }>;
  };
}
...
export default function DroidToolCard({
  ...
  const [droidStatus, setDroidStatus] = useState<DroidStatus | null>(initialStatus as DroidStatus || null);
  ...
  const currentBaseUrl = droidStatus?.settings?.customModels?.find((m) => m.id?.startsWith("custom:derouter"))?.baseUrl || "";

  const getConfigStatus = (): "configured" | "not_configured" | "other" | null => {
    if (!droidStatus?.installed) return null;
    // Check for any derouter model entry (support multi-model: custom:derouter-0, custom:derouter-1, ...)
    const currentConfig = droidStatus.settings?.customModels?.find(m => m.id?.startsWith("custom:derouter"));
  ...
  useEffect(() => {
    if ((apiKeys?.length ?? 0) > 0 && !selectedApiKey) {
      setSelectedApiKey(apiKeys![0].key);
    }
  }, [apiKeys, selectedApiKey]);

  useEffect(() => {
    if (initialStatus) setDroidStatus(initialStatus as DroidStatus);
  }, [initialStatus]);
  ...
  useEffect(() => {
    if (droidStatus?.installed && !hasInitializedModel.current) {
      ...
      const existingModels = (droidStatus.settings?.customModels || [])
        .filter(m => m.id?.startsWith("custom:derouter"))
        .sort((a, b) => (a.index || 0) - (b.index || 0))
        .map(m => m.model);
      if (existingModels.length > 0) {
        setModelList(existingModels.filter((m): m is string => !!m));
      } else {
        // Legacy: single model stored as custom:derouter-0
        const legacy = droidStatus.settings?.customModels?.find(m => m.id === "custom:derouter-0");
        if (legacy?.model) {
  ...
  const checkDroidStatus = async () => {
    ...
    try {
      const data = await apiGet<DroidStatus>("/api/cli-tools/droid");
      setDroidStatus(data);
    } catch (error) {
  ...
  const handleApplySettings = async () => {
    ...
    try {
      const keyToUse = selectedApiKey?.trim()
        || ((apiKeys?.length ?? 0) > 0 ? apiKeys![0].key : null)
        || (!cloudEnabled ? "sk_derouter" : null);

      const data = await apiPost<{ error?: string }>("/api/cli-tools/droid", {
  ...
  const handleResetSettings = async () => {
    ...
    setMessage(null);
    try {
      const data = await apiDelete<{ error?: string }>("/api/cli-tools/droid");
      if (data.error) {
        setMessage({ type: "error", text: data.error });
      } else {
          ...
          {!checkingDroid && droidStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {droidStatus?.settings?.customModels?.find(m => m.id?.startsWith("custom:derouter"))?.baseUrl && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!droidStatus?.hasderouter} loading={restoring}>
