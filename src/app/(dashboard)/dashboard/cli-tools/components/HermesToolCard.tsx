...
interface HermesStatus extends CliToolStatus {
  settings?: { model?: { base_url?: string; default?: string } };
}

const ENDPOINT = "/api/cli-tools/hermes";
...
export default function HermesToolCard({
  ...
  const [hermesStatus, setHermesStatus] = useState<HermesStatus | null>(initialStatus as HermesStatus || null);
  ...
  useEffect(() => {
    if (initialStatus) setHermesStatus(initialStatus as HermesStatus);
  }, [initialStatus]);
  ...
  const checkStatus = async () => {
    setChecking(true);
    try {
      const data = await apiGet<HermesStatus>(ENDPOINT);
          ...
          {!checking && hermesStatus?.installed && (
            <>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!hermesStatus?.hasderouter} loading={restoring} className="w-full sm:w-auto">
