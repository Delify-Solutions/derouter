...
interface GrokStatus extends CliToolStatus {
  settings?: {
    model?: GrokModelConfig;
    subagentModels?: Record<string, { model?: string; context_window?: number }>;
  };
}

const ENDPOINT = "/api/cli-tools/grok-build";
const MODEL_SLOT = "derouter";
const SUBAGENT_TYPES: { id: string; label: string; help: string }[] = [
...
export default function GrokBuildToolCard({
  ...
  const getContextWindow = (model: string): number | null => getCaps(model)?.contextWindow || null;
  const initialModel = (initialStatus?.settings?.model as { model?: string } | undefined)?.model || "";
  const initialSubagents = Object.fromEntries(
    SUBAGENT_TYPES
      .map((type) => [type.id, (initialStatus?.settings?.subagentModels as Record<string, { model?: string }> | undefined)?.[type.id]?.model])
      .filter(([, model]) => Boolean(model)),
  );
  const [grokStatus, setGrokStatus] = useState<GrokStatus | null>(initialStatus as GrokStatus || null);
  ...
  const hasFetchedStatus = useRef<boolean>(Boolean(initialStatus));
  ...
  const hydrateForm = useCallback((status: GrokStatus | null) => {
  ...
  const checkStatus = useCallback(async ({ hydrate = false }: { hydrate?: boolean } = {}) => {
    setChecking(true);
    try {
      const status = await apiGet<GrokStatus>(ENDPOINT);
  ...
  return (
    <Card padding="xs" className="overflow-hidden">
          ...
          {!checking && grokStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {SUBAGENT_TYPES.map((type) => (
                  <ModelField
                    ...
                    onChange={(value: string) => setSubagentModels((current) => ({ ...current, [type.id]: value }))}
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!grokStatus?.hasderouter} loading={restoring} className="w-full sm:w-auto"><span className="material-symbols-outlined text-[14px] mr-1">restore</span>Reset</Button>
      ...
      <ManualConfigModal isOpen={showManualConfigModal} onClose={() => setShowManualConfigModal(false)} title="Grok Build - Manual Configuration" configs={getManualConfigs() as never} />
    </Card>
  );
}
