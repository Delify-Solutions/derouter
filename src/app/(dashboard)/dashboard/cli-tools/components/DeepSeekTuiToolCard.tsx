...
const ENDPOINT = "/api/cli-tools/deepseek-tui";
...
interface DeepSeekStatus extends CliToolStatus {
  settings?: {
    "providers.openai"?: {
      base_url?: string;
      model?: string;
...
export default function DeepSeekTuiToolCard({
  ...
  const [deepseekStatus, setDeepseekStatus] = useState<DeepSeekStatus | null>(initialStatus as DeepSeekStatus || null);
  ...
  useEffect(() => {
    if (initialStatus) setDeepseekStatus(initialStatus as DeepSeekStatus);
  }, [initialStatus]);
  ...
  const checkStatus = async () => {
    setChecking(true);
    try {
      const data = await apiGet<DeepSeekStatus>(ENDPOINT);
          ...
          {!checking && deepseekStatus?.installed && (
            <>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleReset} disabled={!deepseekStatus?.hasderouter} loading={restoring}>
