...
interface JcodeStatus extends CliToolStatus {
  config?: {
    providers?: {
      derouter?: {
        base_url?: string;
...
export default function JcodeToolCard({
  ...
  const [jcodeStatus, setJcodeStatus] = useState<JcodeStatus | null>(initialStatus as JcodeStatus || null);
  ...
  const currentBaseUrl = jcodeStatus?.config?.providers?.["derouter"]?.base_url || "";

  const getConfigStatus = (): "configured" | "not_configured" | "other" | null => {
    if (!jcodeStatus?.installed) return null;
    if (!jcodeStatus?.hasderouter) return "not_configured";
    const currentProvider = jcodeStatus.config?.providers?.["derouter"];
  ...
  useEffect(() => {
    if (initialStatus) setJcodeStatus(initialStatus as JcodeStatus);
  }, [initialStatus]);
  ...
  useEffect(() => {
    if (jcodeStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = jcodeStatus.config?.providers?.["derouter"];
      if (provider) {
  ...
  const checkJcodeStatus = async () => {
    ...
    try {
      const data = await apiGet<JcodeStatus>("/api/cli-tools/jcode");
      setJcodeStatus(data);
    } catch (error) {
  ...
  const handleApplySettings = async () => {
    ...
    try {
      ...
      const data = await apiPost<{ error?: string }>("/api/cli-tools/jcode", {
  ...
  const handleResetSettings = async () => {
    ...
    setMessage(null);
    try {
      const data = await apiDelete<{ error?: string }>("/api/cli-tools/jcode");
      if (data.error) {
        setMessage({ type: "error", text: data.error });
      } else {
  ...
  const getManualConfigs = () => {
    ...
    const configToml = `[providers.derouter]
type = "openai-compatible"
base_url = "${getEffectiveBaseUrl()}"
auth = "bearer"
api_key_env = "JCODE_DEROUTER_API_KEY"
env_file = "provider-derouter.env"
default_model = "${selectedModel || "cc/claude-opus-4-7"}"
requires_api_key = true

[[providers.derouter.models]]
id = "${selectedModel || "cc/claude-opus-4-7"}"`;

    const envContent = `JCODE_DEROUTER_API_KEY="${keyToUse}"`;
    ...
    return [
      ...
      {
        filename: "~/.config/jcode/provider-derouter.env",
        content: envContent,
      },
    ];
  };
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-text-muted mt-2">Manual configuration is still available if derouter is deployed on a remote server.</p>
                  </div>
                </div>
          ...
          {!checkingJcode && jcodeStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {jcodeStatus?.config?.providers?.["derouter"]?.base_url && (
                ...
                <div className="flex flex-col gap-1 p-3 bg-blue-500/5 border border-blue-500/20 rounded-lg">
                  ...
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile derouter</code>
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile derouter --model {selectedModel || "cc/claude-opus-4-7"}</code>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!jcodeStatus?.hasderouter} loading={restoring}>
