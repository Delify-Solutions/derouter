...
interface ClaudeStatus extends CliToolStatus {
...
export default function ClaudeToolCard({
  ...
  const [claudeStatus, setClaudeStatus] = useState<ClaudeStatus | null>(initialStatus as ClaudeStatus || null);
  ...
  useEffect(() => {
    if (initialStatus) {
      setClaudeStatus(initialStatus as ClaudeStatus);
  ...
  useEffect(() => {
    apiGet<Record<string, unknown>>("/api/settings").then(data => {
  ...
  const handleCcFilterNamingToggle = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      await apiPatch("/api/settings", { ccFilterNaming: value });
    } catch { /* ignore */ }
  };
  ...
  const checkClaudeStatus = async () => {
    ...
    try {
      const data = await apiGet<ClaudeStatus>("/api/cli-tools/claude");
  ...
  const handleApplySettings = async () => {
    ...
    try {
      ...
      const data = await apiPost<{ error?: string }>("/api/cli-tools/claude", { env, exaMcpEnabled, maxContextTokens });
      if (data.error) {
  ...
  const handleResetSettings = async () => {
    ...
    setMessage(null);
    try {
      const data = await apiDelete<{ error?: string }>("/api/cli-tools/claude");
      if (data.error) {
        setMessage({ type: "error", text: data.error });
      } else {
  ...
  return (
    <Card padding="xs" className="overflow-hidden">
          ...
          {!checkingClaude && claudeStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  ...
                  <label className="flex items-center gap-1.5 cursor-pointer select-none">
                    ...
                    <Tooltip text="Intercepts Claude Code's topic-naming requests and returns a fake response locally, saving API tokens." color={undefined as never}>
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  ...
                  <label className="flex items-center gap-1.5 cursor-pointer select-none">
                    ...
                    <Tooltip text="Injects Exa MCP into ~/.claude.json so non-Claude models gain web search. Restart Claude Code after Apply." color={undefined as never}>
                      <span className="material-symbols-outlined text-text-muted text-[14px] cursor-help">info</span>
                    </Tooltip>
                  </label>
                </div>
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!claudeStatus?.hasderouter} loading={restoring}>
      ...
      {modalOpen && (
        <ModelSelectModal isOpen={modalOpen} onClose={() => setModalOpen(false)} onSelect={handleModelSelect} selectedModel={currentEditingAlias ? modelMappings[currentEditingAlias] : undefined} activeProviders={activeProviders as never} modelAliases={modelAliases as never} title={`Select model for ${currentEditingAlias}`}
