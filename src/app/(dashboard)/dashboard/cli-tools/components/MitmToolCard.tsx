...
import { apiGet, apiPut, apiPatch } from "@/shared/api/client";
...
export default function MitmToolCard({
  ...
  const [pendingDnsAction, setPendingDnsAction] = useState<"enable" | "disable" | null>(null);
  const [modalError, setModalError] = useState<string | null>(null);
  const [modelMappings, setModelMappings] = useState<Record<string, string>>({});
  const [modalOpen, setModalOpen] = useState(false);
  const [currentEditingAlias, setCurrentEditingAlias] = useState<string | null>(null);

  const mitmHosts = TOOL_HOSTS[tool.id as keyof typeof TOOL_HOSTS] ?? [];
  ...
  const loadSavedMappings = async () => {
    try {
      const data = await apiGet<{ aliases?: Record<string, string> }>(`/api/cli-tools/antigravity-mitm/alias?tool=${tool.id}`);
      if (data.aliases && Object.keys(data.aliases).length > 0) setModelMappings(data.aliases);
    } catch { /* ignore */ }
  };

  const saveMappings = useCallback(async (mappings: Record<string, string>) => {
    try {
      await apiPut<{ error?: string }>("/api/cli-tools/antigravity-mitm/alias", { tool: tool.id, mappings });
    } catch { /* ignore */ }
  }, [tool.id]);
  ...
  const doDnsAction = async (action: string, password: string) => {
    ...
    setWarning(null);
    try {
      const data = await apiPatch<{ error?: string }>("/api/cli-tools/antigravity-mitm", { tool: tool.id, action, sudoPassword: password });
      if (data.error) throw new Error(data.error || "Failed to toggle DNS");
        ...
        {isExpanded && (
          <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-0.5 text-[11px] text-text-muted px-1">
              <p>Toggle DNS to redirect {tool.name} traffic through derouter via MITM.</p>
              {!dnsActive && (
