...
interface OpenClawStatus extends CliToolStatus {
  settings?: {
    models?: {
      providers?: {
        derouter?: {
...
export default function OpenClawToolCard({
  ...
  const [openclawStatus, setOpenclawStatus] = useState<OpenClawStatus | null>(initialStatus as OpenClawStatus || null);
  ...
  const currentBaseUrl = openclawStatus?.settings?.models?.providers?.["derouter"]?.baseUrl || "";

  const getConfigStatus = (): "configured" | "not_configured" | "other" | null => {
    if (!openclawStatus?.installed) return null;
    const currentProvider = openclawStatus.settings?.models?.providers?.["derouter"];
  ...
  useEffect(() => {
    if (initialStatus) setOpenclawStatus(initialStatus as OpenClawStatus);
  }, [initialStatus]);
  ...
  useEffect(() => {
    if (openclawStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = openclawStatus.settings?.models?.providers?.["derouter"];
      if (provider) {
        const primaryModel = openclawStatus.settings?.agents?.defaults?.model?.primary;
        if (primaryModel) setSelectedModel(primaryModel.replace("derouter/", ""));
  ...
  const checkOpenclawStatus = async () => {
    ...
    try {
      const data = await apiGet<OpenClawStatus>("/api/cli-tools/openclaw");
  ...
  const handleApplySettings = async () => {
    ...
    try {
      ...
      const data = await apiPost<{ error?: string }>("/api/cli-tools/openclaw", {
  ...
  const handleResetSettings = async () => {
    ...
    setMessage(null);
    try {
      const data = await apiDelete<{ error?: string }>("/api/cli-tools/openclaw");
      if (data.error) {
        setMessage({ type: "error", text: data.error });
      } else {
  ...
  const getManualConfigs = () => {
    ...
    const settingsContent = {
      agents: {
        defaults: {
          model: {
            primary: `derouter/${selectedModel || "provider/model-id"}`,
          },
        },
      },
      models: {
          ...
          {!checkingOpenclaw && openclawStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {openclawStatus?.settings?.models?.providers?.["derouter"]?.baseUrl && (
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                ...
                <Button variant="outline" size="sm" onClick={handleResetSettings} disabled={!openclawStatus?.hasderouter} loading={restoring}>
