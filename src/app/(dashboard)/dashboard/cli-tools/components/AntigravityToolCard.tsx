...
interface AntigravityStatus extends CliToolStatus {
  running?: boolean;
  certExists?: boolean;
  certTrusted?: boolean;
...
export default function AntigravityToolCard({
  ...
  const [status, setStatus] = useState<AntigravityStatus | null>(initialStatus as AntigravityStatus || null);
  ...
  useEffect(() => {
    if (initialStatus) setStatus(initialStatus as AntigravityStatus);
  }, [initialStatus]);
  ...
  const loadSavedMappings = async () => {
    try {
      const data = await apiGet<{ aliases?: Record<string, string> }>("/api/cli-tools/antigravity-mitm/alias?tool=antigravity");
  ...
  const fetchStatus = async () => {
    try {
      const data = await apiGet<AntigravityStatus>("/api/cli-tools/antigravity-mitm");
      setStatus(data);
    } catch (error) {
  ...
  const doStart = async (password: string) => {
    ...
    try {
      ...
      const data = await apiPost<{ error?: string }>("/api/cli-tools/antigravity-mitm", {
        apiKey: keyToUse,
  ...
  const handleSaveMappings = async () => {
    ...
    try {
      await apiPut<{ error?: string }>("/api/cli-tools/antigravity-mitm/alias", {
        tool: "antigravity",
    ...
    <Card padding="xs" className="overflow-hidden">
        ...
        <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
              ...
              <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                ...
                {apiKeys && apiKeys.length > 0 ? (
                  ...
                  <span className="min-w-0 rounded bg-surface/40 px-2 py-2 text-xs text-text-muted sm:py-1.5">
                    {cloudEnabled ? "No API keys - Create one in Keys page" : "sk_derouter (default)"}
                  </span>
                )}
              </div>
          ...
          {!isRunning && serverIsWindows && (
            <div className="flex items-center gap-2 px-2 py-1.5 rounded text-xs bg-yellow-500/10 text-yellow-600 border border-yellow-500/20">
              ...
              <span>Windows: Run terminal (derouter) as Administrator to enable MITM</span>
            </div>
          )}
          ...
          {!isRunning && (
            <div className="flex flex-col gap-1.5 px-1">
              <p className="text-xs text-text-muted">
                <span className="font-medium text-text-main">How it works:</span> Intercepts Antigravity traffic via DNS redirect, letting you reroute models through derouter.
              </p>
              <div className="flex flex-col gap-0.5 text-[11px] text-text-muted">
                ...
                <span>3. Maps Antigravity models to any provider via derouter</span>
              </div>
            </div>
          )}
        </div>
      ...
      {modalOpen && (
        <ModelSelectModal
          ...
          selectedModel={currentEditingAlias ? modelMappings[currentEditingAlias] : undefined}
