...
import type { EndpointPreset } from "./cliTools.types";
...
const buildOptions = (args: BuildOptionsArgs): SelectOption[] => {
  const opts: SelectOption[] = [];
  const wrap = (url: string) => (args.withV1 ? ensureV1(url) : (url || "").replace(/\/+$/, ""));
  if (!args.requiresExternalUrl) {
    const localUrl = wrap(`http://127.0.0.1:${UPDATER_CONFIG.appPort}`);
    opts.push({ value: "local", label: localUrl, url: localUrl });
  }
  if (args.tunnelEnabled && args.tunnelPublicUrl) {
    const u = wrap(args.tunnelPublicUrl);
    opts.push({ value: "tunnel", label: u, url: u });
  }
  if (args.tailscaleEnabled && args.tailscaleUrl) {
    const u = wrap(args.tailscaleUrl);
    opts.push({ value: "tailscale", label: u, url: u });
  }
  if (args.cloudEnabled && args.cloudUrl) {
    const u = wrap(args.cloudUrl);
    opts.push({ value: "cloud", label: u, url: u });
  }
  args.savedPresets.forEach((p) => {
...
export default function BaseUrlSelect({
  ...
  const options = useMemo(
    () => buildOptions({ requiresExternalUrl, tunnelEnabled, tunnelPublicUrl: tunnelPublicUrl || "", tailscaleEnabled, tailscaleUrl: tailscaleUrl || "", cloudEnabled, cloudUrl: cloudUrl || "", savedPresets, withV1 }),
  ...
  const handleSelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (next === SAVE_VALUE) {
      ...
      try { defaultName = new URL(trimmed).host; } catch { /* keep default */ }
