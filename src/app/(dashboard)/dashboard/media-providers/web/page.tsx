...
import type { ProviderConnection, Combo, ProviderListResponse, ComboListResponse } from "@/shared/types";
...
function getEffectiveStatus(conn: ProviderConnection & { testStatus?: string; [key: string]: unknown }): string {
...
interface ProviderCardProps {
  ...
  kind: string;
  connections: (ProviderConnection & { testStatus?: string; [key: string]: unknown })[];
}
...
function Section({ title, icon, kind, providers, connections, combos, onCreateCombo }: SectionProps) {
  return (
    <div>
...
export default function WebProvidersPage() {
  ...
  const [connections, setConnections] = useState<(ProviderConnection & { testStatus?: string; [key: string]: unknown })[]>([]);
  ...
  const fetchAll = async () => {
    try {
      ...
      if (connsRes) setConnections(connsRes.connections as (ProviderConnection & { testStatus?: string; [key: string]: unknown })[]);
      if (combosRes) setCombos(combosRes.combos as ComboEntry[]);
    } catch { /* noop */ }
  };
  ...
  const searchProviders = getProvidersByKind("webSearch") as WebProviderInfo[];
  const fetchProviders = getProvidersByKind("webFetch") as WebProviderInfo[];
  const searchCombos = combos.filter((c) => (c as Combo).kind === "webSearch");
  const fetchCombos = combos.filter((c) => (c as Combo).kind === "webFetch") as unknown as ComboEntry[];
  ...
  return (
    <div className="flex flex-col gap-8">
      <Section
        ...
        providers={searchProviders} connections={connections} combos={searchCombos as unknown as ComboEntry[]}
