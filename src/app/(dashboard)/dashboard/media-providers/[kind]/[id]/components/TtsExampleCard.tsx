...
import { useState, useEffect, type ChangeEvent, type MouseEvent, type CSSProperties } from "react";
...
interface TtsConfig {
  ...
  apiEndpoint?: string;
  languageOptions?: string[] | { id: string; name: string }[];
}
...
interface TTSProviderModelConfig {
  models?: TtsModelInfo[];
}
...
export function TtsExampleCard({ providerId }: TtsExampleCardProps) {
  ...
  const config: TtsConfig = (TTS_PROVIDER_CONFIG as Record<string, TtsConfig>)[providerId] || (TTS_PROVIDER_CONFIG as Record<string, TtsConfig>)["edge-tts"];

  // Voice state
  const [selectedVoice, setSelectedVoice]         = useState<string>(config.defaultVoiceId || "");
  const [selectedVoiceName, setSelectedVoiceName] = useState<string>("");
  const [voiceId, setVoiceId]                     = useState<string>(config.defaultVoiceId || ""); // editable voice id (elevenlabs/config providers)
  ...
  const [selectedModel, setSelectedModel]   = useState<string>(() => {
    const cfgModels = (AI_PROVIDERS as Record<string, { ttsConfig?: TTSProviderModelConfig }>)[providerId]?.ttsConfig?.models;
  ...
  // Form state
  const [input, setInput]               = useState<string>("Hello, this is a text to speech test.");
  const [style, setStyle]               = useState<string>(""); // style/voice instructions (e.g. MiMo voicedesign)
  ...
  const [responseFormat, setResponseFormat] = useState<string>("mp3"); // mp3 | json
  ...
  const [byLang, setByLang]             = useState<Record<string, TtsLanguage>>({});
  ...
  useEffect(() => {
    ...
    fetch("/api/providers", { cache: "no-store" })
      .then((r) => r.json())
      .then((d: { connections?: Array<{ provider?: string; isActive?: boolean }> }) => { setConnectionCount((d.connections || []).filter((c) => c.provider === providerId && c.isActive !== false).length); })
      .catch(() => {});
    ...
    if (config.voiceSource === "hardcoded") {
      ...
      const voices: TtsVoice[] = (config.voicesPerModel && defaultModel)
        ...
        : getModelsByProviderId(config.voiceKey || providerId).filter((m: TtsModelInfo) => getModelKind(m) === "tts");
      if (voices.length) {
  ...
  const openModal = async () => {
    ...
    try {
      if (config.voiceSource === "hardcoded") {
        ...
        const voices = getModelsByProviderId(voiceKey).filter((m: TtsModelInfo) => getModelKind(m) === "tts");
        const byLangMap: Record<string, TtsLanguage> = {};
        for (const v of voices) {
          if (!byLangMap[v.id]) byLangMap[v.id] = { code: v.id, name: v.name || v.id, voices: [{ id: v.id, name: v.name || v.id }] };
        }
        setByLang(byLangMap);
        ...
        const d = await r.json() as { error?: string; languages?: TtsLanguage[]; byLang?: Record<string, TtsLanguage> };
  ...
  const ttsBody = (() => {
    const b: Record<string, unknown> = { model: modelFull, input };
    ...
    <>
      <Card>
        ...
        <div className="flex flex-col gap-2.5">
          ...
          {config.hasModelSelector && (config.modelKey || getModelsByProviderId(providerId).some((m: TtsModelInfo) => getModelKind(m) === "tts")) && (
            <Row label="Model">
              <select
                value={selectedModel}
              ...
              >
                {(() => {
                  const ttsModels = getModelsByProviderId(providerId).filter((m: TtsModelInfo) => getModelKind(m) === "tts");
                  return (ttsModels.length ? ttsModels : getModelsByProviderId(config.modelKey!) || []).map((m: TtsModelInfo) => (
          ...
          {config.hasLanguageHint && (
            <Row label="Language">
              <select
                value={languageHint}
                onChange={(e: ChangeEvent<HTMLSelectElement>) => setLanguageHint(e.target.value)}
                className="w-full px-3 py-1.5 text-sm border border-border rounded-lg bg-background focus:outline-none focus:border-primary"
              >
            ...
            <Row label="Language">
              <select
                ...
                onChange={(e: ChangeEvent<HTMLSelectElement>) => {
                  const m = getModelsByProviderId(providerId).filter((mm: TtsModelInfo) => getModelKind(mm) === "tts").find((mm: TtsModelInfo) => mm.id === e.target.value);
                  setSelectedVoice(e.target.value);
                  setSelectedVoiceName(m?.name || e.target.value);
                }}
                className="w-full px-3 py-1.5 text-sm border border-border rounded-lg bg-background focus:outline-none focus:border-primary"
              >
                {getModelsByProviderId(providerId).filter((m: TtsModelInfo) => getModelKind(m) === "tts").map((m: TtsModelInfo) => (
                  <option key={m.id} value={m.id}>{m.name || m.id}</option>
                ))}
              </select>
            </Row>
          ...
          {config.hasStyleInput && (
            <Row label={translate("Style")}>
              <div className="relative">
                <textarea
                  value={style}
                  onChange={(e: ChangeEvent<HTMLTextAreaElement>) => setStyle(e.target.value)}
          ...
          <Row label="Output Format">
            <select
              ...
              onChange={(e: ChangeEvent<HTMLSelectElement>) => setResponseFormat(e.target.value)}
              className="w-full px-3 py-1.5 text-sm border border-border rounded-lg bg-background focus:outline-none focus:border-primary"
            >
      ...
      {modalOpen && (
        <div
          ...
          style={{ backgroundColor: "rgba(0,0,0,0.6)", backdropFilter: "blur(2px)" } as CSSProperties}
          onClick={() => setModalOpen(false)}
        >
          <div
            className="border border-border rounded-xl shadow-2xl w-full max-w-md mx-4 flex flex-col max-h-[80vh]"
            style={{ backgroundColor: "var(--color-bg)", isolation: "isolate" } as CSSProperties}
          ...
          >
            ...
            <div className="px-4 py-2.5 border-b border-border shrink-0">
              <input
                autoFocus
                value={modalSearch}
                onChange={(e: ChangeEvent<HTMLInputElement>) => setModalSearch(e.target.value)}
