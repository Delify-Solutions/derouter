...
type SttResult = string | Record<string, unknown>;

export function SttExampleCard({ providerId }: SttExampleCardProps) {
  const providerAlias = getProviderAlias(providerId);
  const builtinSttModels = getModelsByProviderId(providerId).filter((m: SttModelInfo) => getModelKind(m) === "stt");
  const [customSttModels, setCustomSttModels] = useState<CustomModelEntry[]>([]);
  const sttModels: SttModelInfo[] = [...builtinSttModels, ...customSttModels];

  const [selectedModel, setSelectedModel] = useState<string>(builtinSttModels[0]?.id ?? "");
  const selectedModelObj = sttModels.find((m) => m.id === selectedModel);
  const allowedParams: string[] = Array.isArray(selectedModelObj?.params) ? (selectedModelObj as SttModelInfo).params! : [];
  ...
  useEffect(() => {
    ...
    const loadCustom = () => {
      fetch("/api/models/custom", { cache: "no-store" })
        .then((r) => r.json())
        .then((d: { models?: CustomModelEntry[] }) => {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      const data: SttResult = ct.includes("application/json") ? await res.json() : await res.text();
      if (!res.ok) {
        if (typeof data === "object" && data !== null) {
          const errObj = (data as Record<string, unknown>).error as { message?: string } | string | undefined;
          const errMsg = typeof errObj === "object" && errObj !== null ? errObj.message : (typeof errObj === "string" ? errObj : "");
          setError(errMsg || `HTTP ${res.status}`);
        } else {
          setError(String(data) || `HTTP ${res.status}`);
        }
        return;
      }
      setResult(data);
