...
// Minimal model shape for embedding models from the shared constants
...
export function EmbeddingExampleCard({ providerId, customAlias }: EmbeddingExampleCardProps) {
  ...
  const embeddingModels = isCustom ? [] : getModelsByProviderId(providerId).filter((m: EmbeddingModelInfo) => getModelKind(m) === "embedding");

  const [selectedModel, setSelectedModel] = useState<string>(embeddingModels[0]?.id ?? "");
  ...
  const buildBody = (): Record<string, unknown> => {
    const body: Record<string, unknown> = { model: modelFull, input: input.trim() };
  ...
  const formatResultJson = (data: unknown): string => {
    ...
    (clone.data || []).forEach((item) => {
      if (Array.isArray(item.embedding) && item.embedding.length > 4) {
        item.embedding = [...item.embedding.slice(0, 4).map((v) => parseFloat(String(v).toString()).toFixed(6) as unknown as number), `... (${item.embedding.length} dims)`];
      }
    });
      ...
      <div className="flex flex-col gap-2.5">
        ...
        <Row label="Model">
          {isCustom ? (
            ...
            >
              {embeddingModels.map((m: EmbeddingModelInfo) => (
        ...
        <Row label="Endpoint">
          <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
            <input
              value={endpoint}
              onChange={(e: ChangeEvent<HTMLInputElement>) => useTunnel ? setTunnelEndpoint(e.target.value) : setLocalEndpoint(e.target.value)}
