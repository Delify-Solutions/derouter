...
import type { Combo, ComboListResponse, ProviderConnection, ProviderListResponse, Settings, PricingMap, ComboTestResult } from "@/shared/types";
...
interface CapEntry { enabled: boolean; roundRobin: boolean; models: string[] }
...
export default function CombosPage() {
  ...
  const [comboStrategies, setComboStrategies] = useState<Record<string, unknown>>({});
  const [capacityAdapter, setCapacityAdapter] = useState(EMPTY_CAPACITY_ADAPTER);
  const [poolPricing, setPoolPricing] = useState<PricingMap>({}); // { provider: { model: {5 fields} } }
  const [comboPricing, setComboPricing] = useState<PricingMap>({}); // { comboName: {5 fields} }
  const { getCaps } = useModelCaps();
  const [confirmState, setConfirmState] = useState<{ title: string; message: string; onConfirm: () => Promise<void> } | null>(null);
  const [testState, setTestState] = useState<{ comboName: string; loading: boolean; result: ComboTestResult | null } | null>(null);
  ...
  const fetchData = async () => {
    try {
      const [combosData, providersData, settingsData, poolPricingData, comboPricingData] = await Promise.all([
        apiGet<ComboListResponse>("/api/combos").catch(() => null),
        apiGet<ProviderListResponse>("/api/providers").catch(() => null),
        apiGet<Settings>("/api/settings").catch(() => null),
        apiGet<PricingMap>("/api/pricing").catch(() => ({})),
        apiGet<PricingMap>("/api/pricing?combo=1").catch(() => ({})),
      ]);

      // Only LLM combos here - webSearch/webFetch combos belong to media-providers/web
      if (combosData) setCombos((combosData.combos || []).filter(c => !c.kind || c.kind === "llm"));
      if (poolPricingData) setPoolPricing(poolPricingData);
      if (comboPricingData) setComboPricing(comboPricingData);
      if (providersData) {
        setActiveProviders(providersData.connections || []);
      }
      if (settingsData) {
        setComboStrategies((settingsData.comboStrategies as Record<string, unknown>) || {});
  ...
  const handleSetCapacityAdapter = async (next: typeof EMPTY_CAPACITY_ADAPTER) => {
    setCapacityAdapter(next);
    try {
      await apiPatch("/api/settings", { capacityAdapter: next });
    } catch (error) {
  ...
  const handleTestCombo = async (comboName: string) => {
    if (testState?.loading) return;
    setTestState({ comboName, loading: true, result: null });
    try {
      const data = await apiPost<{ ok: boolean; status: number; latencyMs?: number; content?: string; error?: string }>("/api/models/test", {
        model: comboName,
      }).catch(() => ({ ok: false, error: "Invalid response from test endpoint" }));
      setTestState({ comboName, loading: false, result: data });
    } catch (error) {
      const errMsg = error instanceof Error ? error.message : "Network error";
      setTestState({ comboName, loading: false, result: { ok: false, error: errMsg } });
    }
  };
  ...
  const handleSetComboStrategy = async (comboName: string, patch: Record<string, unknown>) => {
    try {
      ...
      const next = { ...(updated[comboName] as Record<string, unknown> || {}), ...patch };
...
function ComboFormModal({ isOpen, combo, onClose, onSave, activeProviders, kindFilter = null, poolPricing = {}, comboPricing = {} }) {
  ...
  const fetchModalData = async () => {
    try {
      const aliasesData = await apiGet<{ aliases: Record<string, string> }>("/api/models/alias");
