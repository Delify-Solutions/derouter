...
// ---- Local types (token-saver-specific) ----
...
export default function TokenSaverClient() {
  ...
  const [pxpipeStatus, setPxpipeStatus] = useState<PxpipeStatusState>({
    installed: false,
    installing: false,
    running: false,
    version: null,
  ...
  const refreshHeadroomStatus = useCallback(async () => {
    ...
    try {
      ...
      setHeadroomStatus({ ...data, loading: false } as HeadroomStatus);
      if (!data?.installed) {
        setHeadroomExtras({
          version: null,
          extras: { code: false, ml: false },
  ...
  const refreshPxpipeStatus = useCallback(async () => {
    ...
    try {
      ...
      const data = (await res.json()) as PxpipeStatusState;
  ...
  const runPxpipeHealth = useCallback(async () => {
    try {
      ...
      setPxpipeHealth((await res.json()) as PxpipeHealth);
    } catch (e: unknown) {
    ...
    const loadSettings = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setHeadroomUrl((data.headroomUrl as string) || "http://localhost:8787");
          ...
          setCavemanLevel((data.cavemanLevel as string) || "full");
          setPonytailEnabled(!!data.ponytailEnabled);
          setPonytailLevel((data.ponytailLevel as string) || "full");
    ...
    <div className="space-y-6 p-6">
        ...
        {headroomStatus.installed && (
          <div className="mb-3 ml-1 pl-3 pb-4 border-l-2 border-border">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {headroomExtras.available.map((extra: string) => {
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <Input
              value={headroomUrl}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setHeadroomUrl(e.target.value)}
          ...
          <div className="flex flex-col gap-1">
            ...
            <Input
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setHeadroomTimeoutMs(Number(e.target.value))}
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          {pxpipeHealth?.checks && pxpipeHealth.checks.length > 0 && (
          ...
          {!pxpipeStatus.installed ? (
            <div className="flex flex-col gap-2">
              ...
              <p className="text-xs text-text-muted">
                ...
                the derouter data directory. May take a few minutes.
              </p>
            </div>
          ) : (
          ...
          <div className="flex flex-col gap-1">
            ...
            <Input
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPxpipeMinChars(Number(e.target.value))}
