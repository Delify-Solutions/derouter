...
import { apiGet, apiPatch, apiDelete, ApiError } from "@/shared/api/client";
import type { PricingMap, PricingFields } from "@/shared/types";
...
function isOverrideRow(merged: PricingMap, defaults: PricingMap, provider: string, model: string): boolean {
  const d = defaults?.[provider]?.[model];
  if (!d) return true; // not in built-in → user-added
  const m = merged?.[provider]?.[model];
  for (const f of ["input", "output", "cached", "reasoning", "cache_creation"] as const) {
...
function flatten(merged: PricingMap, defaults: PricingMap): PricingRow[] {
...
export default function PricingPageClient() {
  ...
  const [merged, setMerged] = useState<PricingMap>({}); // GET /api/pricing
  const [defaults, setDefaults] = useState<PricingMap>({}); // GET /api/pricing?defaults=1
  ...
  const [confirmReset, setConfirmReset] = useState<{ provider: string; model: string } | null>(null);

  const loadData = useCallback(async () => {
    try {
      const [mergedData, defaultsData] = await Promise.all([
        apiGet<PricingMap>("/api/pricing").catch(() => ({})),
        apiGet<PricingMap>("/api/pricing?defaults=1").catch(() => ({})),
      ]);
  ...
  const saveEdit = async () => {
    if (!editing) return;
    let parsed: PricingFields;
    try { parsed = parsePricingDraft(editDraft); }
    catch (e) { setStatus({ type: "error", message: e instanceof Error ? e.message : String(e) }); return; }
    setSavingRow(true);
    try {
      const body = { [editing.provider]: { [editing.model]: parsed } };
      const updated = await apiPatch<PricingMap>("/api/pricing", body);
      setMerged(updated || merged);
      setStatus({ type: "success", message: `Saved price for ${editing.provider}/${editing.model}` });
      cancelEdit();
    } catch (e) {
      setStatus({ type: "error", message: e instanceof Error ? e.message : "Failed to save price" });
    } finally {
      setSavingRow(false);
    }
  };

  const resetOverride = async (provider: string, model: string) => {
    setSavingRow(true);
    try {
      const updated = await apiDelete<PricingMap>(`/api/pricing?provider=${encodeURIComponent(provider)}&model=${encodeURIComponent(model)}`);
      setMerged(updated || {});
      setStatus({ type: "success", message: `Reset ${provider}/${model} to built-in default` });
    } catch (e) {
      setStatus({ type: "error", message: e instanceof Error ? e.message : "Failed to reset" });
    } finally {
      setSavingRow(false);
  ...
  const saveAdd = async () => {
    ...
    try {
      ...
      setStatus({ type: "error", message: e instanceof Error ? e.message : "Failed to add price" });
    } finally {
      setSavingAdd(false);
    }
  };
