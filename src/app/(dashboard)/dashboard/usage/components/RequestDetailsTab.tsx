...
import type { ProviderNode, ProviderNodeListResponse, UsageByProvider, UsageByProviderResponse, ApiKey, KeyListResponse } from "@/shared/types";

let providerNameCache: Record<string, string> | null = null;
...
async function fetchProviderNames() {
  ...
  const nodesData = await apiGet<ProviderNodeListResponse>("/api/provider-nodes");
...
export default function RequestDetailsTab() {
  ...
  const fetchProviders = useCallback(async () => {
    try {
      const data = await apiGet<UsageByProviderResponse>("/api/usage/providers");
  ...
  const fetchKeys = useCallback(async () => {
    try {
      ...
      const list = Array.isArray(data) ? data : (data.keys || (data as unknown as Record<string, unknown[]>).items || []);
  ...
  const fetchDetails = useCallback(async () => {
    ...
    try {
      ...
      const data = await apiGet<{ details?: any[]; pagination?: any }>(
        `/api/usage/request-details?${params}`,
      );
