...
interface ChartResponse {
  total?: { requests: number; input: number; output: number; cost: number };
  buckets?: Array<{ timestamp: string; input: number; output: number; cost: number }>;
}
...
const fmtCost = (n: number): string => `$${(n || 0).toFixed(4)}`;

function toChartBuckets(buckets: ChartResponse["buckets"]): ChartBucket[] {
  if (!buckets) return [];
  return buckets.map((b) => {
    const d = new Date(b.timestamp);
    return {
      ...b,
      tokens: (b.input || 0) + (b.output || 0),
      label: d.toLocaleDateString("en", { month: "short", day: "numeric" }) + " " + d.toLocaleTimeString("en", { hour: "2-digit", minute: "2-digit" }),
    };
  });
}
...
export default function UsageChart({ period = "7d" }: { period?: string }) {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const json = await apiGet<ChartResponse>(`/api/usage/chart?period=${period}`);
      setData(toChartBuckets(json.buckets));
    } catch (e) {
          ...
          <AreaChart data={data} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
            ...
            <Tooltip
              ...
              formatter={((value: number, name: string) =>
                name === "tokens" ? [fmtTokens(value), "Tokens"] : [fmtCost(value), "Cost"]) as any}
            />
            {viewMode === "tokens" ? (
              <Area
                type="monotone"
