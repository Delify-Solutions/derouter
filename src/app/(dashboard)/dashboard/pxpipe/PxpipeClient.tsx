...
// ---- Local types (pxpipe-specific, not shared across pages) ----
...
const fmtUptime = (ms: number | null | undefined): string => {
  if (!ms || ms <= 0) return "—";
...
export default function PxpipeClient() {
  ...
  const [windowId, setWindowId] = useState<string>("last7d");
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setStatus((await statusRes.json()) as PxpipeStatus);
      setStats((await statsRes.json()) as PxpipeStats);
      setLogs((await logsRes.json()) as PxpipeLogs);
      const healthRes = await fetch("/api/pxpipe/health", { method: "POST" });
      setHealth((await healthRes.json()) as PxpipeHealth);
    } catch {
        ...
        {stats?.timeline?.some((d) => d.tokensSavedEst > 0) ? (
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={stats.timeline} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
              ...
              <YAxis tick={{ fontSize: 11 }} tickFormatter={(v: unknown) => fmtTokens(Number(v))} width={48} />
              <Tooltip formatter={(v: unknown) => [fmtTokens(Number(v)), "Tokens saved"]} labelFormatter={(d: unknown) => String(d)} />
                ...
                <tr key={`${ev.ts}-${i}`} className="border-b border-border/50">
                  ...
                  <td className="py-1.5">
                    <span
                      ...
                      title={ev.detail || ""}
                    >
                      {ev.applied ? "Compressed" : (ev.reason && REASON_LABELS[ev.reason]) || ev.reason || "—"}
                    </span>
                  </td>
                </tr>
              ))}
