...
import type { ProviderListResponse, ProviderInfo } from "@/shared/types";
import ModelAvailabilityBadge from "./components/ModelAvailabilityBadge";
import AddCompatibleModal from "./components/AddCompatibleModal";
import { STATUS_FILTER_OPTIONS, matchesStatusFilter } from "./utils";

// Extended connection shape with runtime fields used by the UI
interface ProviderConnectionExt {
  id: string;
  provider: string;
  authType: string;
  name: string | null;
...
function getStatusDisplay(connected: number, error: number, errorCode: string | null): React.ReactNode {
...
function getConnectionErrorTag(connection: ProviderConnectionExt | null): string | null {
...
export default function ProvidersPage() {
  const [connections, setConnections] = useState<ProviderConnectionExt[]>([]);
  const [providerNodes, setProviderNodes] = useState<ProviderNodeExt[]>([]);
  ...
  const [testResults, setTestResults] = useState<TestBatchResult | null>(null);
  const [statusFilter, setStatusFilter] = useState("all");
  const notify = useNotificationStore();
  const searchQuery = useHeaderSearchStore((s: { query: string }) => s.query);
  const registerSearch = useHeaderSearchStore((s: { register: (p: string) => void }) => s.register);
  const unregisterSearch = useHeaderSearchStore((s: { unregister: () => void }) => s.unregister);
  ...
  const matchSearch = (name: string | undefined | null): boolean => {
  ...
  const getProviderStats = (providerId: string, authType: string | string[]): ProviderStats => {
    ...
    const getEffectiveStatus = (conn: ProviderConnectionExt): string => {
  ...
  const matchStatus = (stats: ProviderStats, isNoAuth: boolean): boolean =>
  ...
  const handleToggleProvider = async (
    ...
    const authTypes = Array.isArray(authType) ? authType : [authType];
    const matches = (c: ProviderConnectionExt) =>
    ...
    await Promise.allSettled(
      providerConns.map((c) =>
        apiPut(`/api/providers/${c.id}`, { isActive: newActive }),
      ),
    );
  };
  ...
  const handleBatchTest = async (mode: string, providerId: string | null = null) => {
    ...
    setTestResults(null);
    try {
      const data = await apiPost<TestBatchResult>("/api/providers/test-batch", { mode, providerId });
      setTestResults(data);
      if (data.summary) {
    ...
    const fetchData = async () => {
      try {
        const [connectionsRes, nodesRes] = await Promise.all([
          apiGet<ProviderListResponse>("/api/providers"),
          apiGet<{ nodes?: ProviderNodeExt[] }>("/api/provider-nodes"),
        ]);
        if (connectionsRes.connections) {
          setConnections(connectionsRes.connections as unknown as ProviderConnectionExt[]);
        }
        if (nodesRes.nodes) setProviderNodes(nodesRes.nodes);
      } catch {
        // ignore fetch errors
      } finally {
        setLoading(false);
  ...
  const dualAuthTypes = (info: ProviderInfo | undefined, key: string): string | string[] => {
  ...
  const oauthEntries = sortByPriority(
    (Object.entries(OAUTH_PROVIDERS) as [string, ProviderInfo][]).filter(
      ([key, info]) =>
        !info.hidden &&
  ...
  const freeEntries = (Object.entries(FREE_PROVIDERS) as [string, ProviderInfo][])
    .filter(
      ([key, info]) =>
        !info.hidden &&
        matchSearch(info.name) &&
  ...
  const freeTierEntries = (Object.entries(FREE_TIER_PROVIDERS) as [string, ProviderInfo][])
    .filter(
      ([key, info]) =>
        !info.hidden &&
        matchSearch(info.name) &&
  ...
  const apikeyEntries = (Object.entries(APIKEY_PROVIDERS) as [string, ProviderInfo][])
    .filter(
      ([key, info]) =>
        !info.hidden &&
        (info.serviceKinds ?? ["llm"]).includes("llm") &&
  ...
  const hasAnyResult =
    ...
    (providerNodes
      .filter((node) => node.type === "openai-compatible" || node.type === "anthropic-compatible")
      .length > 0);
  ...
  const compatibleProviders = providerNodes
    ...
    .filter(
      (p) => matchSearch(p.name) && matchStatus(getProviderStats(p.id, "apikey"), false),
    );
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6 px-1 sm:px-0">
          ...
          {oauthEntries.map(([key, info]) => {
            ...
            return (
              <ProviderCard
                ...
                authType="oauth"
                onToggle={(active: boolean) => handleToggleProvider(key, authTypes, active)}
              />
            );
          })}
        </div>
          ...
          {freeTierEntries.map(([key, info]) => {
            ...
            return (
              <ApiKeyProviderCard
                ...
                onToggle={(active: boolean) => handleToggleProvider(key, freeAuthTypes, active)}
              />
            );
          })}
        </div>
      ...
      <AddCompatibleModal
        ...
        onCreated={(node) => {
          setProviderNodes((prev) => [...prev, node as ProviderNodeExt]);
...
function ProviderCard({ providerId, provider, stats, authType, onToggle }: ProviderCardProps) {
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            >
              <ProviderIcon
                ...
                fallbackColor={provider.color || "#666"}
              />
            </div>
