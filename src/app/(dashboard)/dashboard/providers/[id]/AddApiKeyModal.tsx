...
export default function AddApiKeyModal({ isOpen, provider, providerName, isCompatible, isAnthropic, authType, authHint, website, proxyPools, error, existingNames, onSave, onBulkDone, onClose }: AddApiKeyModalProps) {
  ...
  const providerConfig = provider ? (AI_PROVIDERS as Record<string, ProviderConfig> | undefined)?.[provider] : undefined;
  const providerRegions = providerConfig?.regions || null;
  const defaultRegion = providerConfig?.defaultRegion || providerRegions?.[0]?.id || "";
  ...
  const handleBulkSubmit = async (): Promise<void> => {
    ...
    const plan = planBulkAdd(lines, existingNames, { isCloudflareAi }) as BulkPlanEntry[];
    ...
    for (const entry of plan) {
      try {
        ...
        try {
          ...
          const vdata = await vres.json().catch(() => ({}) as { valid?: boolean });
          isValid = !!vdata.valid;
        } catch {
          isValid = false;
        }
