...
export default function ProviderDetailPage() {
  ...
  const [selectedConnection, setSelectedConnection] = useState<(Connection & { id: string }) | null>(null);
  ...
  const [liveModels, setLiveModels] = useState<BuiltInModel[]>([]);
  const [kiloFreeModels, setKiloFreeModels] = useState<BuiltInModel[]>([]);
  ...
  const staticModels = getModelsByProviderId(providerId) as BuiltInModel[];
  const models: BuiltInModel[] = providerId === "cursor" && liveModels.length > 0
    ? liveModels
    : staticModels;
  ...
  const providerThinkingLevels = (() => {
    ...
    const addLevels = (modelId: string): void => {
  ...
  const fetchDisabledModels = useCallback(async (): Promise<void> => {
    try {
  ...
  const handleDisableModel = async (modelId: string): Promise<void> => {
    try {
  ...
  const handleEnableModel = async (modelId: string): Promise<void> => {
    try {
  ...
  const handleDisableAll = async (ids: string[]): Promise<void> => {
  ...
  const handleEnableAll = async (): Promise<void> => {
    try {
  ...
  const fetchAliases = useCallback(async (): Promise<void> => {
    try {
  ...
  const fetchCustomModels = useCallback(async (): Promise<void> => {
    try {
      const res = await fetch("/api/models/custom", { cache: "no-store" });
      const data = await res.json() as { models?: CustomModelEntry[] };
      if (res.ok) {
  ...
  useEffect(() => {
    ...
    fetch("/api/providers/kilo/free-models")
      .then((res) => res.json())
      .then((data: { models?: BuiltInModel[] }) => { if (data.models?.length) setKiloFreeModels(data.models); })
      .catch(() => {});
  }, [providerId]);
  ...
  const fetchConnections = useCallback(async (): Promise<void> => {
    try {
      ...
      const connectionsData = await connectionsRes.json() as { connections?: Connection[] };
      const nodesData = await nodesRes.json() as { nodes?: ProviderNode[] };
      const proxyPoolsData = await proxyPoolsRes.json() as { proxyPools?: ProxyPool[] };
      ...
      const providerStrategies = (settingsData.providerStrategies || {}) as Record<string, { fallbackStrategy?: string; stickyRoundRobinLimit?: number }>;
      const override = providerStrategies[providerId] || {};
      setProviderStrategy(override.fallbackStrategy || null);
      setProviderStickyLimit(override.stickyRoundRobinLimit != null ? String(override.stickyRoundRobinLimit) : "1");
      // Load per-provider thinking config
      const providerThinking = (settingsData.providerThinking || {}) as Record<string, { mode?: string }>;
      const thinkingCfg = providerThinking[providerId] || {};
      setThinkingMode(thinkingCfg.mode || "auto");
      const autoPingSettingsKey = AUTO_PING_SETTINGS_KEYS[providerId];
      const apCfg: AutoPingState = autoPingSettingsKey ? (settingsData[autoPingSettingsKey] as AutoPingState | undefined) || { enabled: false, connections: {} } : { enabled: false, connections: {} };
      ...
      if (nodesRes.ok) {
        ...
        if (!node && isCompatible) {
          for (let attempt = 0; attempt < 3; attempt += 1) {
            ...
            const retryData = await retryRes.json() as { nodes?: ProviderNode[] };
  ...
  const handleUpdateNode = async (formData: Record<string, unknown>): Promise<void> => {
    try {
      ...
      if (res.ok) {
        setProviderNode(data.node || null);
  ...
  const saveProviderStrategy = async (strategy: string | null, stickyLimit: string): Promise<void> => {
    try {
      const settingsRes = await fetch("/api/settings", { cache: "no-store" });
      const settingsData = settingsRes.ok ? await settingsRes.json() as Record<string, unknown> : {};
      const current = (settingsData.providerStrategies || {}) as Record<string, { fallbackStrategy?: string; stickyRoundRobinLimit?: number }>;

      // Build override: null strategy means remove override, use global
      const override: { fallbackStrategy?: string; stickyRoundRobinLimit?: number } = {};
  ...
  const handleRoundRobinToggle = (enabled: boolean): void => {
    const strategy = enabled ? "round-robin" : null;
    const sticky = enabled ? (providerStickyLimit || "1") : providerStickyLimit;
    if (enabled && !providerStickyLimit) setProviderStickyLimit("1");
    setProviderStrategy(strategy);
    void saveProviderStrategy(strategy, sticky);
  };

  const handleStickyLimitChange = (value: string): void => {
    setProviderStickyLimit(value);
    void saveProviderStrategy("round-robin", value);
  };

  const saveThinkingConfig = async (mode: string): Promise<void> => {
    try {
      const settingsRes = await fetch("/api/settings", { cache: "no-store" });
      const settingsData = settingsRes.ok ? await settingsRes.json() as Record<string, unknown> : {};
      const current = (settingsData.providerThinking || {}) as Record<string, { mode?: string }>;
      const updated = { ...current };
      if (!mode || mode === "auto") {
        delete updated[providerId];
      } else {
  ...
  const handleThinkingModeChange = (mode: string): void => {
    setThinkingMode(mode);
    void saveThinkingConfig(mode);
  };

  const saveAutoPing = async (next: AutoPingState): Promise<void> => {
    const autoPingSettingsKey = AUTO_PING_SETTINGS_KEYS[providerId];
  ...
  const handleAutoPingConnection = (connectionId: string, on: boolean): void => {
    void saveAutoPing({ ...autoPing, connections: { ...autoPing.connections, [connectionId]: on } });
  };
  ...
  useEffect(() => {
    ...
    fetch(`/api/providers/${connection.id}/models`, { cache: "no-store" })
      .then(async (res) => ({ ok: res.ok, data: await res.json() as { models?: BuiltInModel[] } }))
      .then(({ ok, data }) => {
  ...
  useEffect(() => {
    ...
    if (!fetcher) return;
    void fetchSuggestedModels(fetcher).then(setSuggestedModels);
  }, [providerId]);

  const handleSetAlias = async (modelId: string, alias: string, providerAliasOverride: string = providerAlias): Promise<void> => {
  ...
  const handleAddCustomModel = async (modelId: string, type: string = "llm", providerAliasOverride: string = providerStorageAlias, caps?: CapsMap): Promise<void> => {
    try {
  ...
  const handleDeleteCustomModel = async (modelId: string, type: string = "llm", providerAliasOverride: string = providerStorageAlias): Promise<void> => {
    try {
  ...
  const handleImportQoderModels = async (): Promise<void> => {
    ...
    try {
      ...
      const data = await res.json() as { models?: Array<Record<string, string>>; error?: string };
      if (!res.ok) {
  ...
  const handleRunOneByOneTest = async (): Promise<void> => {
    if (oneByOneRunning || connections.length === 0) return;

    const queuedState = Object.fromEntries(
      connections.map((connection) => [connection.id, { state: "queued", error: undefined } as OneByOneResult]),
    );
    ...
    try {
      for (let index = 0; index < connections.length; index += 1) {
        ...
        setOneByOneResults((prev) => ({
          ...prev,
          [connection.id]: { state: "testing", error: undefined },
        }));
        ...
        try {
          ...
          setOneByOneResults((prev) => ({
            ...
            [connection.id]: {
              ...
              error: valid ? undefined : (data.error || undefined),
            },
          }));
        } catch (error) {
          failed += 1;
  ...
  const handleStopOneByOneTest = (): void => {
  ...
  const handleBulkDelete = (): void => {
    ...
    setConfirmState({
      ...
      onConfirm: async () => {
        ...
        if (failedCount > 0) alert(`Deleted ${idsToDelete.length - failedCount} connection(s), ${failedCount} failed.`);
      }
    });
  };

  const handleOAuthSuccess = (): void => {
    void fetchConnections();
    setShowOAuthModal(false);
  };

  const handleIFlowCookieSuccess = (): void => {
    void fetchConnections();
    setShowIFlowCookieModal(false);
  };

  const handleSaveApiKey = async (formData: Record<string, unknown>): Promise<void> => {
  ...
  const handleUpdateConnection = async (formData: Record<string, unknown>): Promise<void> => {
  ...
  const handleUpdateConnectionStatus = async (id: string, isActive: boolean): Promise<void> => {
    try {
  ...
  const handleSwapPriority = async (index1: number, index2: number): Promise<void> => {
  ...
  const toggleSelectConnection = (connectionId: string): void => {
  ...
  const toggleSelectAllConnections = (): void => {
    if (allSelected) {
      setSelectedConnectionIds([]);
      return;
    }
  ...
  const closeBulkProxyModal = (): void => {
    if (bulkUpdatingProxy) return;
    setShowBulkProxyModal(false);
  };

  const applyProxyAssignments = async (assignments: Array<{ connectionId: string; proxyPoolId: string | null }>): Promise<void> => {
    ...
    try {
      ...
      if (failedCount > 0) alert(`Updated with ${failedCount} failed request(s).`);
      await fetchConnections();
      setShowBulkProxyModal(false);
    } finally {
  ...
  const handleApplySinglePool = (proxyPoolId: string | null): Promise<void> => {
    const targets = connections.map((c) => ({ connectionId: c.id, proxyPoolId }));
    return applyProxyAssignments(targets);
  };

  const handleApplyOneToOne = (): Promise<void> => {
    const activePoolsList = proxyPools.filter((p) => p.isActive === true);
    if (activePoolsList.length === 0) {
      alert("No active proxy pools available.");
      return Promise.resolve();
    }
    const targets = connections.map((c, i) => ({
      connectionId: c.id,
      proxyPoolId: activePoolsList[i % activePoolsList.length].id,
    }));
    return applyProxyAssignments(targets);
  };

  const isSelected = (connectionId: string): boolean => selectedConnectionIds.includes(connectionId);

  const triggerOAuthConnection = (): void => {
  ...
  const triggerApiKeyConnection = (): void => {
    setAddConnectionError("");
    setShowAddApiKeyModal(true);
  };

  const triggerAddConnection = (): void => {
    if (isOAuth) {
  ...
  const handleAgRiskConfirm = (): void => {
          ...
          <div key={conn.id} className="flex min-w-0 items-stretch">
            ...
            <div className="flex-1 min-w-0">
              <ConnectionRow
                ...
                onMoveUp={() => void handleSwapPriority(index, index - 1)}
                onMoveDown={() => void handleSwapPriority(index, index + 1)}
                onToggleActive={(isActive: boolean) => void handleUpdateConnectionStatus(conn.id, isActive)}
    ...
    >
      <div className="flex flex-col gap-3">
        <div className="flex flex-col">
          <button
            onClick={() => void handleApplyOneToOne()}
          ...
          <button
            onClick={() => void handleApplySinglePool(null)}
          ...
          {proxyPools.map((pool) => (
            <button
              key={pool.id}
              onClick={() => void handleApplySinglePool(pool.id)}
  ...
  const renderModelsSection = (): React.ReactNode => {
    if (isCompatible) {
      return (
    ...
    return (
      <div className="flex flex-wrap gap-3">
          ...
          <ModelRow
            ...
            onDeleteAlias={() => {
              if (model.source === "custom") {
                void handleDeleteCustomModel(model.id, "llm", providerStorageAlias);
              } else {
                void handleDeleteAlias(model.alias || "");
              }
            }}
            testStatus={modelTestResults[model.id]}
            onTest={connections.length > 0 || isFreeNoAuth ? () => void handleTestModel(model.id) : undefined}
            isTesting={testingModelIds.has(model.id)}
            isCustom
            isFree={false}
            caps={getCaps(`${providerId}/${model.id}`) as Record<string, boolean> | null}
        ...
        {displayModels.map((model) => {
          ...
          return (
            <ModelRow
              ...
              copied={copied}
              onCopy={copy}
              onDeleteAlias={() => existingAlias && void handleDeleteAlias(existingAlias)}
              testStatus={modelTestResults[model.id]}
              onTest={connections.length > 0 || isFreeNoAuth ? () => void handleTestModel(model.id) : undefined}
              isTesting={testingModelIds.has(model.id)}
              isFree={model.isFree}
              onDisable={() => void handleDisableModel(model.id)}
              caps={getCaps(`${providerId}/${model.id}`) as Record<string, boolean> | null}
              thinkingSuffix={resolveThinkingSuffix(model.id) ?? undefined}
            />
          );
        ...
        {providerId === "qoder" && connections.some((conn) => conn.isActive !== false) && (
          <button
            onClick={() => void handleImportQoderModels()}
          ...
          <div className="w-full mt-2">
            ...
            <div className="flex flex-wrap gap-2">
              {disabledDisplayModels.map((m) => (
                <button
                  key={m.id}
                  onClick={() => void handleEnableModel(m.id)}
  ...
  const getHeaderIconPath = (): string | null => {
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6 px-1 sm:gap-8 sm:px-0">
              ...
              {connections.length > 0 && (
                <>
                  ...
                  <Button
                    ...
                    variant="secondary"
                    icon="sync"
                    onClick={() => void handleRunOneByOneTest()}
            ...
            return (
              <div className="flex gap-2">
                {disabledModelIds.length > 0 && (
                  <Button size="sm" variant="secondary" icon="restart_alt" onClick={() => void handleEnableAll()}>
                    Active All
                  </Button>
                )}
                {activeIds.length > 0 && (
                  <Button size="sm" variant="secondary" icon="block" onClick={() => void handleDisableAll(activeIds)}>
                    Disable All
                  </Button>
                )}
              </div>
      ...
      <AddApiKeyModal
        ...
        onBulkDone={() => void fetchConnections()}
        onClose={() => {
          setAddConnectionError("");
          setShowAddApiKeyModal(false);
        }}
      ...
      {!isCompatible && (
        <AddCustomModelModal
          ...
          providerAlias={providerStorageAlias}
          providerDisplayAlias={providerDisplayAlias}
          onSave={async (modelId: string, caps: CapsMap) => {
      ...
      {providerId === "codex" && (
        <BulkImportCodexModal
          ...
          onSuccess={() => void fetchConnections()}
        />
      )}
      ...
      <ConfirmModal
        ...
        onConfirm={confirmState ? () => { void confirmState.onConfirm(); } : undefined}
