...
function parseAccountsInput(rawText: string): unknown[] {
  ...
  if (parsed && typeof parsed === "object") {
    if (Array.isArray((parsed as Record<string, unknown>).accounts)) {
      return (parsed as Record<string, unknown[]>).accounts;
    }
    return [parsed];
  }
...
export default function BulkImportGrokCliModal({ isOpen, onClose, onSuccess }: BulkImportGrokCliModalProps) {
  ...
  const processFiles = async (files: FileList | null): Promise<void> => {
    ...
    try {
      ...
      setParseError(`${translate("Error reading files")}: ${err instanceof Error ? err.message : String(err)}`);
    }
  };
