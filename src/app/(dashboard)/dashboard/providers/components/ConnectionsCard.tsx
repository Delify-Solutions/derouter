...
function ConnectionRow({ connection, proxyPools, isOAuth, isFirst, isLast, onMoveUp, onMoveDown, onToggleActive, onUpdateProxy, onEdit, onDelete }: ConnectionRowProps) {
  ...
  const boundProxyPoolId = data?.proxyPoolId || null;
  const boundProxyPool = boundProxyPoolId ? proxyPoolMap.get(boundProxyPoolId) : null;
  const hasLegacyProxy = data?.connectionProxyEnabled === true && !!data?.connectionProxyUrl;
  const hasAnyProxy = !!boundProxyPoolId || hasLegacyProxy;

  const proxyDisplayText = boundProxyPool
    ? `Pool: ${boundProxyPool.name}`
    : boundProxyPoolId ? `Pool: ${boundProxyPoolId} (inactive/missing)`
    : hasLegacyProxy ? `Legacy: ${data?.connectionProxyUrl}` : "";

  let maskedProxyUrl = "";
  const rawProxyUrl = boundProxyPool?.proxyUrl || data?.connectionProxyUrl;
  if (rawProxyUrl) {
    try {
  ...
  const noProxyText = boundProxyPool?.noProxy || data?.connectionNoProxy || "";
  let proxyBadgeVariant: BadgeVariant = "default";
  if (boundProxyPool?.isActive === true) proxyBadgeVariant = "success";
  else if (boundProxyPoolId || hasLegacyProxy) proxyBadgeVariant = "error";

  const modelLockUntil = Object.entries(connection)
    .filter(([k]) => k.startsWith("modelLock_"))
    .map(([, v]) => v).filter((v): v is string => !!v).sort()[0] || null;

  useEffect(() => {
    const check = () => {
      const until = Object.entries(connection)
        .filter(([k]) => k.startsWith("modelLock_"))
        .map(([, v]) => v).filter((v): v is string => typeof v === "string" && !!v && new Date(v).getTime() > Date.now()).sort()[0] || null;
      setIsCooldown(!!until);
    };
    check();
  ...
  const getStatusVariant = (): BadgeVariant => getConnectionStatusVariant(connection.isActive ?? false, effectiveStatus) as BadgeVariant;

  const displayName = isOAuth
    ? connection.name || connection.email || connection.displayName || "OAuth Account"
    : connection.name;

  const handleSelectProxy = async (poolId: string): Promise<void> => {
            ...
            <div className="relative" ref={proxyDropdownRef}>
              ...
              {showProxyDropdown && (
                <div className="absolute right-0 top-full mt-1 z-50 bg-bg border border-border rounded-lg shadow-lg py-1 min-w-[160px]">
                  <button onClick={() => void handleSelectProxy("__none__")} className={`w-full text-left px-3 py-1.5 text-sm hover:bg-black/5 dark:hover:bg-white/5 ${!boundProxyPoolId ? "text-primary font-medium" : "text-text-main"}`}>None</button>
                  {(proxyPools || []).map((pool) => (
                    <button key={pool.id} onClick={() => void handleSelectProxy(pool.id)} className={`w-full text-left px-3 py-1.5 text-sm hover:bg-black/5 dark:hover:bg-white/5 ${boundProxyPoolId === pool.id ? "text-primary font-medium" : "text-text-main"}`}>{pool.name}</button>
                  ))}
                </div>
              )}
            </div>
...
// ── AddApiKeyModal ─────────────────────────────────────────────
interface AddApiKeyFormData {
  name: string;
  apiKey: string;
  priority: number;
  proxyPoolId: string | null;
  testStatus: "active" | "unknown";
}
...
interface AddApiKeyModalProps {
  ...
  providerName?: string;
  proxyPools?: ProxyPool[];
  onSave: (payload: AddApiKeyFormData) => Promise<void>;
  onClose: () => void;
}
...
export default function ConnectionsCard({ providerId, isOAuth }: ConnectionsCardProps) {
  ...
  const fetch_ = useCallback(async (): Promise<void> => {
    try {
      ...
      const proxyData = await proxyRes.json() as { proxyPools?: ProxyPool[] };
      const settingsData = settingsRes.ok ? await settingsRes.json() as { providerStrategies?: Record<string, { fallbackStrategy?: string; stickyRoundRobinLimit?: number }> } : {};
  ...
  const saveStrategy = async (strategy: string | null, stickyLimit: string): Promise<void> => {
    try {
      const res = await fetch("/api/settings", { cache: "no-store" });
      const data = res.ok ? await res.json() as { providerStrategies?: Record<string, { fallbackStrategy?: string; stickyRoundRobinLimit?: number }> } : {};
  ...
  const handleSwapPriority = async (i1: number, i2: number): Promise<void> => {
  ...
  const handleUpdateProxy = async (connId: string, proxyPoolId: string | null): Promise<void> => {
    try {
  ...
  const handleSaveApiKey = async (formData: AddApiKeyFormData): Promise<void> => {
    try {
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Toggle
              ...
              onChange={(enabled: boolean) => {
                ...
                void saveStrategy(strategy, enabled ? (providerStickyLimit || "1") : providerStickyLimit);
              }}
            />
            {providerStrategy === "round-robin" && (
              <div className="flex flex-wrap items-center gap-1.5">
                ...
                <input
                  ...
                  onChange={(e) => { setProviderStickyLimit(e.target.value); void saveStrategy("round-robin", e.target.value); }}
            ...
            <div className="flex flex-col divide-y divide-black/[0.03] dark:divide-white/[0.03] max-h-[500px] overflow-y-auto pr-1">
              {connections.map((conn, idx) => (
                <ConnectionRow
                  ...
                  onMoveUp={() => void handleSwapPriority(idx, idx - 1)}
                  onMoveDown={() => void handleSwapPriority(idx, idx + 1)}
                  onToggleActive={(isActive: boolean) => void handleToggleActive(conn.id!, isActive)}
                  onUpdateProxy={async (poolId: string | null) => { await handleUpdateProxy(conn.id!, poolId); }}
                  onEdit={() => { setSelectedConnection(conn as Connection & { id: string }); setShowEditModal(true); }}
