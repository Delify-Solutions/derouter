...
export function ModelRow({ model, fullModel, alias: _alias, copied, onCopy, onSetAlias: _onSetAlias, testStatus, isCustom, isFree, onDeleteAlias, onTest, isTesting }: ModelRowProps) {
...
export default function ModelsCard({ providerId, kindFilter, providerAliasOverride }: ModelsCardProps) {
  ...
  const fetchData = useCallback(async (): Promise<void> => {
    try {
      ...
      const aliasData = await aliasRes.json() as { aliases?: Record<string, string> };
      const customData = await customRes.json() as { models?: CustomModel[] };
  ...
  const handleSetAlias = async (modelId: string, alias: string): Promise<void> => {
  ...
  const handleAddCustomModel = async (modelId: string): Promise<void> => {
    try {
  ...
  const handleDeleteCustomModel = async (modelId: string): Promise<void> => {
    try {
  ...
  const allBuiltIn = getModelsByProviderId(providerId) as BuiltInModel[];
  const builtInModels: BuiltInModel[] = kindFilter
    ? allBuiltIn.filter((m) => {
        ...
        <div className="flex flex-wrap gap-3">
          {displayModels.map((model) => {
            ...
            return (
              <ModelRow
                ...
                copied={copied}
                onCopy={copy}
                onSetAlias={(alias: string) => void handleSetAlias(model.id, alias)}
          ...
          {myCustomModels.map((model) => (
            <ModelRow
              ...
              onCopy={copy}
              onSetAlias={() => {}}
              onDeleteAlias={() => void handleDeleteCustomModel(model.id)}
              testStatus={modelTestResults[model.id]}
              onTest={() => void handleTestModel(model.id)}
              isTesting={testingModelId === model.id}
              isCustom
