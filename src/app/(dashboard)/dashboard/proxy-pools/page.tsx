...
import { apiGet, apiPost, apiPut, apiDelete, ApiError } from "@/shared/api/client";
import type { ProxyPool, ProxyPoolListResponse } from "@/shared/types";
...
function normalizeFormData(data: Partial<ProxyPool> | null = null): ProxyPoolFormData {
  return {
    name: data?.name || "",
    proxyUrl: data?.proxyUrl || "",
    noProxy: data?.noProxy || "",
    isActive: data ? data.isActive !== false : true,
...
export default function ProxyPoolsPage() {
  ...
  const [formData, setFormData] = useState<ProxyPoolFormData>(normalizeFormData());
  ...
  const fetchProxyPools = useCallback(async () => {
    try {
      const data = await apiGet<ProxyPoolListResponse>("/api/proxy-pools?includeUsage=true");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEdit) {
        await apiPut(`/api/proxy-pools/${editingProxyPool!.id}`, payload);
      } else {
      ...
      if (error instanceof ApiError) {
        notify.error(error.message || "Failed to save proxy pool");
      } else {
  ...
  const handleDelete = async (proxyPool: ProxyPool) => {
    setConfirmState({
      ...
      onConfirm: async () => {
        ...
        try {
          await apiDelete(`/api/proxy-pools/${proxyPool.id}`);
          setProxyPools((prev) => prev.filter((item) => item.id !== proxyPool.id));
          notify.success("Proxy pool deleted");
        } catch (error) {
          if (error instanceof ApiError && error.status === 409) {
            notify.warning(`Cannot delete: connections are still using this pool.`);
          } else if (error instanceof ApiError) {
            notify.error(error.message || "Failed to delete proxy pool");
          } else {
  ...
  const handleTest = async (proxyPoolId: string) => {
    ...
    try {
      const data = await apiPost<{ ok: boolean; error?: string }>(`/api/proxy-pools/${proxyPoolId}/test`);
      await fetchProxyPools();
      notify.success(data.ok ? "Proxy test passed" : "Proxy test failed");
    } catch (error) {
      if (error instanceof ApiError) {
        notify.error(error.message || "Failed to test proxy");
      } else {
  ...
  const handleToggleActive = async (pool: ProxyPool) => {
    ...
    try {
      await apiPut(`/api/proxy-pools/${pool.id}`, { isActive: next });
    } catch (error) {
  ...
  const bulkSetActive = async (isActive: boolean) => {
    ...
    try {
      ...
      for (const id of targets) {
        try {
          await apiPut(`/api/proxy-pools/${id}`, { isActive });
          ok += 1;
        } catch { failed += 1; }
      }
      await fetchProxyPools();
        ...
        try {
          ...
          for (const id of selectedIds) {
            try {
              ...
              if (error instanceof ApiError && error.status === 409) blocked += 1;
              else failed += 1;
            }
          }
          await fetchProxyPools();
  ...
  const handleHealthCheck = async () => {
    ...
    const worker = async () => {
      while (queue.length > 0) {
        ...
        if (!pool) break;
        try {
          const data = await apiPost<{ ok: boolean }>(`/api/proxy-pools/${pool.id}/test`);
          if (data.ok) alive += 1; else deadIds.push(pool.id);
        } catch {
        ...
        onConfirm: async () => {
          ...
          try {
            for (const id of deadIds) {
              try {
                await apiPut(`/api/proxy-pools/${id}`, { isActive: false });
              } catch {}
            }
            await fetchProxyPools();
  ...
  const handleVercelDeploy = async () => {
    ...
    setDeploying(true);
    try {
      const data = await apiPost<{ deployUrl?: string; error?: string }>("/api/proxy-pools/vercel-deploy", vercelForm);
      await fetchProxyPools();
      closeVercelModal();
  ...
  const handleCloudflareDeploy = async () => {
    ...
    setDeploying(true);
    try {
      const data = await apiPost<{ deployUrl?: string; error?: string }>("/api/proxy-pools/cloudflare-deploy", cloudflareForm);
      await fetchProxyPools();
  ...
  const handleDenoDeploy = async () => {
    ...
    setDeploying(true);
    try {
      const data = await apiPost<{ deployUrl?: string; error?: string }>("/api/proxy-pools/deno-deploy", denoForm);
      await fetchProxyPools();
      closeDenoModal();
      notify.success(`Deployed: ${data.deployUrl}`);
    } catch (error) {
  ...
  const parseProxyLine = (line: string): { proxyUrl: string; name: string } | null => {
