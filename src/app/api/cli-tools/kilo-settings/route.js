...
const hasderouterConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["derouter"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("derouter");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasderouter: hasderouterConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "derouter", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "derouter settings removed from Kilo Code" });
  } catch (error) {
