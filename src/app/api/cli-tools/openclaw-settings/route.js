...
// Check if settings has derouter config
const hasderouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["derouter"];
};

// Read per-agent models.json and return current model id (without "derouter/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["derouter"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasderouter: hasderouterConfig(settings),
...
// POST - Update derouter settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `derouter/${model}`;

    // Remove all old derouter/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("derouter/"))
    ...
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`derouter/${m}`] = {};
    });

    // Remove old derouter model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("derouter/")) {
    ...
    settings.models.providers["derouter"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `derouter/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove derouter settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["derouter"];
    ...
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("derouter/"));
    ...
    if (settings.agents?.defaults?.model?.primary?.startsWith("derouter/")) {
