...
const hasderouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "derouter");
};

const getderouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "derouter") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getderouterEntry(config);
...
// POST - Apply derouter config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    const idx = config.findIndex((e) => e.name === "derouter");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "derouter");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));
