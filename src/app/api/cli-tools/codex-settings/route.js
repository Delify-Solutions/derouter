...
// Check if config has derouter settings
const hasderouterConfig = (config) => {
  if (!config) return false;
  return config.includes("model_provider = \"derouter\"") || config.includes("[model_providers.derouter]");
};
...
// POST - Update derouter settings (merge with existing config)
export async function POST(request) {
  try {
    ...
    // Update only derouter related fields (api_key goes to auth.json, not config.toml)
    parsed.model = model;
    parsed.model_provider = "derouter";

    // Update or create derouter provider section (no api_key - Codex reads from auth.json)
    // Ensure /v1 suffix is added only once
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;
    // Custom providers ignore auth.json - the key must travel as a static header
    setNestedSection(parsed, "model_providers.derouter", {
      name: "derouter",
...
export async function DELETE() {
  try {
    ...
    if (parsed.model_provider === "derouter") {
      delete parsed.model;
      delete parsed.model_provider;
    }

    // Remove derouter provider section
    deleteNestedSection(parsed, "model_providers.derouter");
