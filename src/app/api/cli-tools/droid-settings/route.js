...
// Check if settings has derouter customModels
const hasderouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:derouter"));
};
...
// POST - Update derouter customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:derouter"));
...
// DELETE - Remove derouter customModels only (keep other settings)
