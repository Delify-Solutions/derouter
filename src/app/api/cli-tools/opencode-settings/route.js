...
const hasderouterConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["derouter"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["derouter"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasderouter: hasderouterConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("derouter/") ? config.model.replace(/^derouter\//, "") : null,
...
// POST - Apply derouter as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_derouter";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing derouter provider entry and its models
    const existingProvider = config.provider["derouter"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["derouter"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `derouter/${finalActive}`;
      }
    }
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("derouter/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove derouter provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["derouter"]?.models) {
      delete config.provider["derouter"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["derouter"].models).length === 0) {
        delete config.provider["derouter"];
        if (config.model?.startsWith("derouter/")) delete config.model;
      } else if (config.model === `derouter/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["derouter"].models);
        config.model = `derouter/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire derouter provider
      if (config.provider) delete config.provider["derouter"];
      if (config.model?.startsWith("derouter/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("derouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "derouter settings removed from OpenCode",
    });
  } catch (error) {
