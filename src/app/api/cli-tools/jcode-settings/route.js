...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-derouter.env");
};
...
const hasderouterConfig = (config) => {
  ...
  if (providers["derouter"]) return true;
...
export async function GET() {
  ...
  const hasderouter = hasderouterConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_DEROUTER_API_KEY = apiKey;
...
export async function DELETE() {
  try {
    ...
    delete config.providers["derouter"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_DEROUTER_API_KEY;
    await writeProviderEnv(env);
