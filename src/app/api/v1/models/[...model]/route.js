import { buildModelsList, resolveKeyScopeForModels } from "../route.js";
...
export async function GET(request, { params }) {
  try {
    ...
    // Resolve the calling key's scope (401 if requireApiKey and missing/invalid;
    // allowedModels = null for unlimited keys / open proxy). Same scoping as the
    // /v1/models listing, so a key can only look up models it can use.
    const scope = await resolveKeyScopeForModels(request);
    if (scope.response) return json(
      { error: { message: "Authentication failed", type: "invalid_request_error" } },
      { status: 401 }
    );
