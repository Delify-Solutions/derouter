...
interface CursorAutoImportResponse {
  found: boolean;
  accessToken?: string;
  machineId?: string;
...
interface CursorImportResponse {
  success?: boolean;
  error?: string;
}
...
export default function CursorAuthModal({ isOpen, onSuccess, onClose }: CursorAuthModalProps) {
  ...
  const runAutoDetect = async (): Promise<void> => {
    ...
    try {
      ...
      const data: CursorAutoImportResponse = await res.json();
  ...
  const handleImportToken = async (): Promise<void> => {
    ...
    try {
      ...
      const data: CursorImportResponse = await res.json();
        ...
        {!autoDetecting && (
          <>
            ...
            <div>
              ...
              <Input
                value={machineId}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setMachineId(e.target.value)}
