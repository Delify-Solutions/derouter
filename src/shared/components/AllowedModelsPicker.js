 ...
 *
 * Props:
 *   value: string[]              - selected combo names
 *   onChange: (string[]) => void
 *   placeholder?: string
 */
export default function AllowedModelsPicker({ value = [], onChange, placeholder = "Search combos…" }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/combos")
      ...
      .then((data) => {
        if (cancelled) return;
        const llm = (data.combos || []).filter((c) => !c.kind || c.kind === "llm");
        setCombos(llm);
        setLoading(false);
      })
      .catch(() => {
