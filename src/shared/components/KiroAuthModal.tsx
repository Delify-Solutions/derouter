...
interface KiroAutoImportResponse {
  found: boolean;
  refreshToken?: string;
  clientId?: string;
  clientSecret?: string;
...
export interface KiroAuthModalProps {
  isOpen: boolean;
  onMethodSelect: (method: string, config?: MethodConfig) => void;
  onClose: () => void;
}
...
export default function KiroAuthModal({ isOpen, onMethodSelect, onClose }: KiroAuthModalProps) {
  ...
  useEffect(() => {
    ...
    const autoDetect = async (): Promise<void> => {
      ...
      try {
        ...
        const data: KiroAutoImportResponse = await res.json();
  ...
  const handleMethodSelect = (method: SelectedMethod): void => {
  ...
  const handleImportToken = async (): Promise<void> => {
    ...
    try {
      ...
      const data: KiroImportResponse = await res.json();
  ...
  const handleImportCliProxyJson = async (): Promise<void> => {
  ...
  const handleApiKeyImport = async (): Promise<void> => {
    if (!apiKey.trim()) {
        ...
        {selectedMethod === "idc" && (
          <div className="space-y-4">
            <div>
              ...
              <Input
                value={idcStartUrl}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setIdcStartUrl(e.target.value)}
            ...
            <div>
              ...
              <Input
                value={idcRegion}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setIdcRegion(e.target.value)}
        ...
        {selectedMethod === "api-key" && (
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                value={apiKeyRegion}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setApiKeyRegion(e.target.value)}
                placeholder="us-east-1"
                className="font-mono text-sm"
              />
              <p className="text-xs text-text-muted mt-1">
        ...
        {selectedMethod === "import-cli-proxy" && (
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                value={cliProxyJson}
                onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setCliProxyJson(e.target.value)}
