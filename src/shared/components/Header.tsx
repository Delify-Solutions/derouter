...
const getPageInfo = (pathname: string | null): PageInfo => {
  ...
  if (mediaDetailMatch) {
    ...
    const kindConfig = MEDIA_PROVIDER_KINDS.find((k: { id: string }) => k.id === kindId);
    const provider = (AI_PROVIDERS as Record<string, { name?: string } | undefined>)[providerId];
    return {
  ...
  if (providerMatch) {
    ...
    const providerInfo =
      (OAUTH_PROVIDERS as Record<string, { name?: string; id?: string } | undefined>)[providerId] ||
      (APIKEY_PROVIDERS as Record<string, { name?: string; id?: string } | undefined>)[providerId];
    if (providerInfo) {
      return {
        ...
        breadcrumbs: [
          { label: "Providers", href: "/dashboard/providers" },
          {
            ...
            image: getProviderIconSrc(providerInfo.id || providerId),
          },
        ],
      };
    }
  ...
  if (pathname.includes("/usage"))
    return { title: "Usage & Analytics", description: "Monitor your API usage, token consumption, and request logs", icon: "bar_chart", breadcrumbs: [] };
  if (pathname.includes("/auth-files"))
    return { title: "Auth Files", description: "Map provider credentials stored in the local database", icon: "vpn_key", breadcrumbs: [] };
  if (pathname.includes("/quota"))
    return { title: "Quota Tracker", description: "Track and manage your API quota limits", icon: "data_usage", breadcrumbs: [] };
  if (pathname.includes("/mitm"))
    return { title: "MITM Proxy", description: "Intercept CLI tool traffic and route through derouter", icon: "security", breadcrumbs: [] };
  ...
  if (pathname.includes("/skills"))
    return { title: "Agent Skills", description: "Copy a link and paste to your AI to use derouter — no install needed", icon: "extension", breadcrumbs: [] };
  ...
  useEffect(() => {
    ...
    async function loadAuthStatus() {
      try {
        ...
        if (!res.ok) return;
        const data = await res.json() as { displayName?: string; samlName?: string; samlEmail?: string; oidcName?: string; oidcEmail?: string; loginMethod?: string };
        if (!cancelled) {
...
function HeaderSearch() {
  const visible = useHeaderSearchStore((s: { visible: boolean }) => s.visible);
  const query = useHeaderSearchStore((s: { query: string }) => s.query);
  const placeholder = useHeaderSearchStore((s: { placeholder: string }) => s.placeholder);
  const setQuery = useHeaderSearchStore((s: { setQuery: (v: string) => void }) => s.setQuery);
