...
import type { McpPlugin } from "@/shared/types";
...
export default function McpMarketplaceModal({ isOpen, onClose, onAdd, addedNames = [] }: McpMarketplaceModalProps) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    apiGet<RegistryResponse>(REGISTRY_ENDPOINT)
      .then((d) => {
  ...
  const filtered = useMemo<RegistryServer[]>(() => {
    const q = search.trim().toLowerCase();
    return servers.filter((s: RegistryServer) => {
  ...
  const fetchTools = async (server: RegistryServer): Promise<void> => {
    if (toolsCache[server.url]) return;
    setToolsLoading((p) => ({ ...p, [server.url]: true }));
    try {
      const d = await apiPost<ToolsResponse>(TOOLS_ENDPOINT, { url: server.url });
      const tools = d.tools || [];
      const fallback = Array.isArray(server.toolNames) ? server.toolNames : [];
      const toolNames = tools.length > 0 ? tools.map((t: ToolItem) => t.name) : fallback;
      ...
      setToolsCache((p) => ({ ...p, [server.url]: { tools: [], error: e instanceof Error ? e.message : "Unknown error" } }));
    } finally {
      setToolsLoading((p) => ({ ...p, [server.url]: false }));
    }
  };

  const expandServer = (server: RegistryServer): void => {
  ...
  const toggleTool = (url: string, tool: string): void => {
    setToolSelection((prev) => ({ ...prev, [url]: { ...prev[url], [tool]: !prev[url]?.[tool] } }));
  };

  const setAllTools = (url: string, value: boolean): void => {
    const sel = toolSelection[url] || {};
    setToolSelection((prev) => ({ ...prev, [url]: Object.fromEntries(Object.keys(sel).map((t: string) => [t, value])) }));
  };

  const confirmAdd = (server: RegistryServer): void => {
    const sel = toolSelection[server.url] || {};
    const enabled = Object.keys(sel).filter((t: string) => sel[t]);
    onAdd?.({
                ...
                <div key={s.url} className="rounded border border-transparent hover:border-border">
                  <div className="flex items-start gap-2 px-2 py-2 hover:bg-black/5 dark:hover:bg-white/5">
                    {s.iconUrl ? (
                      ...
                      <img src={s.iconUrl} alt="" className="size-7 rounded shrink-0 object-contain" loading="lazy" decoding="async" />
                    ) : (
