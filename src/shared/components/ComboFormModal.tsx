...
import type { Combo, ModelAliasResponse } from "@/shared/types";
...
function ModelItem({ index, model, isFirst, isLast, onEdit, onMoveUp, onMoveDown, onRemove }: ModelItemProps) {
  ...
  return (
    <div className="group flex min-w-0 items-center gap-1.5 rounded-md bg-black/[0.02] px-2 py-1 transition-colors hover:bg-black/[0.04] dark:bg-white/[0.02] dark:hover:bg-white/[0.04]">
      ...
      {editing ? (
        <input autoFocus value={draft} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setDraft(e.target.value)} onBlur={commit} onKeyDown={handleKeyDown}
...
export default function ComboFormModal({ isOpen, combo, onClose, onSave, activeProviders = [], kindFilter = null, forcePrefix = "", title }: ComboFormModalProps) {
  ...
  useEffect(() => {
    if (!isOpen) return;
    apiGet<ModelAliasResponse>("/api/models/alias")
      .then((d) => { if (d && d.aliases) setModelAliases(d.aliases); })
      .catch(() => {});
  }, [isOpen]);
  ...
  const handleAddModel = (model: ModelOption): void => {
    if (!models.includes(model.value)) setModels([...models, model.value]);
  };
  const handleDeselectModel = (model: ModelOption): void => {
    setModels(models.filter((m: string) => m !== model.value));
  };
  const handleRemoveModel = (i: number): void => setModels(models.filter((_, idx: number) => idx !== i));
  const handleMoveUp = (i: number): void => {
    if (i === 0) return;
    const a = [...models]; [a[i - 1], a[i]] = [a[i], a[i - 1]]; setModels(a);
  };
  const handleMoveDown = (i: number): void => {
              ...
              <div className="flex max-h-[55vh] min-w-0 flex-col gap-1 overflow-y-auto sm:max-h-[350px]">
                {models.map((model: string, index: number) => (
                  <ModelItem key={index} index={index} model={model}
                    ...
                    onEdit={(v: string) => { const a = [...models]; a[index] = v; setModels(a); }}
