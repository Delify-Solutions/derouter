...
import type { ProxyPool, Settings } from "@/shared/types";
...
export default function NoAuthProxyCard({ providerId }: NoAuthProxyCardProps) {
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([
      apiGet<{ proxyPools: ProxyPool[] }>("/api/proxy-pools?isActive=true").catch(() => ({ proxyPools: [] })),
      apiGet<Settings>("/api/settings").catch(() => ({} as Settings)),
    ]).then(([poolData, settingsData]) => {
      if (cancelled) return;
      setProxyPools(poolData.proxyPools || []);
      const providerStrategies = (settingsData as Settings).providerStrategies as Record<string, Record<string, unknown>> | undefined;
      const override = providerStrategies?.[providerId] || {};
      setProxyPoolId((override.proxyPoolId as string) || NONE_PROXY_POOL_VALUE);
      setRotateStrategy((override.rotateStrategy as string) || "none");
    }).catch(() => {});
    return () => { cancelled = true; };
  }, [providerId]);

  const save = useCallback(async (poolId: string, strategy: string): Promise<void> => {
    setSaving(true);
    try {
      const data = await apiGet<Settings>("/api/settings").catch(() => ({} as Settings));
      const current = (data.providerStrategies || {}) as Record<string, Record<string, unknown>>;
  ...
  const handlePoolChange = (newPoolId: string): void => {
  ...
  return (
    <Card>
      ...
      <Select
        label="Proxy Pool"
        value={proxyPoolId}
        onChange={(e: React.ChangeEvent<HTMLSelectElement>) => handlePoolChange(e.target.value)}
        disabled={saving || isRotation}
        options={[
          { value: NONE_PROXY_POOL_VALUE, label: "None (direct)" },
          ...proxyPools.map((pool: ProxyPool) => ({ value: pool.id, label: pool.name })),
        ]}
      ...
      <div className="flex flex-col gap-2 mt-4">
        ...
        <select
          ...
          onChange={(e: React.ChangeEvent<HTMLSelectElement>) => handleStrategyChange(e.target.value)}
          disabled={saving}
