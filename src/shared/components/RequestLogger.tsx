...
import type { RequestLogsResponse } from "@/shared/types";
...
export default function RequestLogger() {
  ...
  const fetchLogs = async (showLoading: boolean = true): Promise<void> => {
    if (showLoading) setLoading(true);
    try {
      const data = await apiGet<RequestLogsResponse | string[]>("/api/usage/request-logs");
      if (Array.isArray(data)) {
        setLogs(data as string[]);
      } else if (data && Array.isArray((data as RequestLogsResponse).logs)) {
        setLogs((data as RequestLogsResponse).logs);
      }
    } catch (error) {
