...
const PROXY_OAUTH_PROVIDERS = new Set<string>(["trae", "windsurf", "zed"]);

interface PasteTokenConfig {
  label: string;
  instructions: string;
  placeholder: string;
  ideName: string;
...
const PASTE_TOKEN_PROVIDERS: Record<string, PasteTokenConfig> = {
  trae: {
    label: "Cloud-IDE-JWT",
    instructions:
...
type AuthStep = "waiting" | "input" | "success" | "error";
type AuthMode = "browser" | "paste-token";
...
export default function OAuthModal({ isOpen, provider, providerInfo, onSuccess, onClose, oauthMeta, idcConfig }: OAuthModalProps) {
  const [step, setStep] = useState<AuthStep>("waiting");
  ...
  // Exchange tokens
  const exchangeTokens = useCallback(async (code: string, state: string | null): Promise<void> => {
    if (!authData) return;
    try {
  ...
  const completeXaiManualCode = useCallback(async (code: string): Promise<void> => {
  ...
  const startPolling = useCallback(async (deviceCode: string, codeVerifier: string, interval: number, extraData: ExtraData | null, deadlineMs?: number): Promise<void> => {
    ...
    const deadline = startedAt + (Number.isFinite(deadlineMs) && (deadlineMs ?? 0) > 0 ? (deadlineMs as number) : 120_000);
    ...
    while (Date.now() < deadline) {
      ...
      await new Promise<void>((r) => setTimeout(r, currentInterval * 1000));
      ...
      try {
        ...
        const data: DevicePollResponse = await res.json();
  ...
  const startProxyFlow = useCallback(async (providerId: string): Promise<void> => {
    ...
    const startData: ProxyStartResponse = await startRes.json();
    ...
    const regBody: Record<string, string> = { state: authData.state || "" };
  ...
  const startOAuthFlow = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const deviceCodeProviders: string[] = [
        "github",
        "kiro",
        "kimi",
        "kimi-coding",
      ...
      if (deviceCodeProviders.includes(provider)) {
        ...
        const extraData: ExtraData | null = provider === "kiro"
          ? {
      ...
      if (provider === "codex") {
        try {
          ...
          const proxyData: ProxyStartResponse = await proxyRes.json();
  ...
  useEffect(() => {
    if (isOpen && provider) {
      ...
      if (provider && PASTE_TOKEN_PROVIDERS[provider]) {
        fetch(`/api/oauth/${provider}/ide-status`)
          .then((r) => r.json())
          .then((data: IdeStatus) => setIdeStatus(data))
    ...
    const pollProvider = authData?.codexServerSide
      ...
      : authData?.xaiServerSide
        ...
        : authData?.proxyProvider
          ? (authData.proxyProvider as string)
          : null;
  ...
  useEffect(() => {
    ...
    const handleCallback = async (data: CallbackData): Promise<void> => {
      ...
      if (token || code) {
        callbackProcessedRef.current = true;
        await exchangeTokens(token || (code as string), state || null);
      }
    };
    ...
    const handleMessage = (event: MessageEvent): void => {
      ...
      const eventData = event.data as { type?: string; data?: CallbackData } | null;
      if (eventData?.type === "oauth_callback") {
        handleCallback(eventData.data as CallbackData);
      }
    };
    ...
    try {
      ...
      channel.onmessage = (event: MessageEvent) => handleCallback(event.data as CallbackData);
    } catch {
    ...
    const handleStorage = (event: StorageEvent): void => {
      if (event.key === "oauth_callback" && event.newValue) {
        try {
          const data: CallbackData = JSON.parse(event.newValue);
          handleCallback(data);
    ...
    try {
      ...
      if (stored) {
        const data: CallbackData = JSON.parse(stored);
        ...
        {PROXY_OAUTH_PROVIDERS.has(provider) && (step === "waiting" || step === "input" || step === "error") && (
          <>
              ...
              <>
                ...
                {step === "input" && (
                  <div className="space-y-3">
                    ...
                    <Input
                      value={callbackUrl}
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCallbackUrl(e.target.value)}
            ...
            {authMode === "paste-token" && (
              <div className="space-y-3">
                {ideStatus && !ideStatus.installed && provider && PASTE_TOKEN_PROVIDERS[provider] && (
                ...
                {provider && PASTE_TOKEN_PROVIDERS[provider] && (
                  <p className="text-sm text-text-muted">{PASTE_TOKEN_PROVIDERS[provider].instructions}</p>
                )}
                <Input
                  value={pasteToken}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPasteToken(e.target.value)}
                  placeholder={provider && PASTE_TOKEN_PROVIDERS[provider] ? PASTE_TOKEN_PROVIDERS[provider].placeholder : ""}
                  className="font-mono text-xs"
                />
                <div className="flex gap-2">
