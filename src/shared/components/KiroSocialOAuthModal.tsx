...
type AuthStep = "loading" | "input" | "success" | "error";
...
interface SocialAuthorizeResponse {
  authUrl: string;
  codeVerifier?: string;
  error?: string;
}
...
export default function KiroSocialOAuthModal({ isOpen, provider, onSuccess, onClose }: KiroSocialOAuthModalProps) {
  ...
  const [authData, setAuthData] = useState<SocialAuthData | null>(null);
  ...
  useEffect(() => {
    ...
    const initAuth = async (): Promise<void> => {
      try {
        ...
        const data: SocialAuthorizeResponse = await res.json();

        if (!res.ok) {
          throw new Error(data.error || "Authorization failed");
        }

        setAuthData({ authUrl: data.authUrl, codeVerifier: data.codeVerifier });
  ...
  const handleManualSubmit = async (): Promise<void> => {
    try {
      ...
      const data: SocialExchangeResponse = await res.json();
